...
/**
 * Declares functions for a memory file, ie, a file that is allocated in memory
 * and never dumped to disk. Used for result sets.
 */
 ...
 */
void
mfSetPos(XFile* xFile, int32 position);
 ...
 * @return <code>true</code> if it was possible to allocate the new memory file
 * buffer; <code>false</code>, otherwise.
 * @throws OutOfMemoryError If there is not enougth memory allocate memory.
 */
bool
mfGrowTo(Context context, XFile* xFile, uint32 newSize);
 ...
 */
bool
mfReadBytes(Context context, XFile* xFile, uint8* buffer, int32 count);
 ...
 */
bool
mfWriteBytes(Context context, XFile* xFile, uint8* buffer, int32 count);
 ...
 * @param context The thread context where the function is being executed.
 * @param xFile The memory file.
 * @return Always <code>true</code>. It has a return value in order to be
 * compatible with <code>nfClose()</code>.
 */
bool
mfClose(Context context, XFile* xFile);
 ...
 */
void
test_mfClose(TestSuite* testSuite, Context currentContext);
 ...
 */
void
test_mfGrowTo(TestSuite* testSuite, Context currentContext);
 ...
 */
void
test_mfReadBytes(TestSuite* testSuite, Context currentContext);
 ...
 */
void
test_mfSetPos(TestSuite* testSuite, Context currentContext);
 ...
 */
void
test_mfWriteBytes(TestSuite* testSuite, Context currentContext);
