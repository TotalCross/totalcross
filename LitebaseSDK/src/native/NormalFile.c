...
bool nfCreateFile(Context context, CharP name, bool isCreation, bool useCrypto, TCHARP sourcePath,
	...
	if (cacheSize != -1
		&& !(xFile->cache = xmalloc(xFile->cacheInitialSize = cacheSize))) { // Creates the cache.
...
bool refreshCache(Context context, XFile* xFile, int32 count) {
	...
	if (!xFile->cache || xFile->cacheInitialSize < count) // Increases the cache size if necessary.
		if (!(xFile->cache = xrealloc(xFile->cache, xFile->cacheInitialSize = MAX(CACHE_INITIAL_SIZE,
									  count << 2)))) {
			TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
			return false;
		}
	...
	if ((ret = lbfileSetPos(xFile->file, xFile->cachePos))
		|| (ret = lbfileReadBytes(xFile->file, (CharP)xFile->cache, 0, xFile->cacheInitialSize, &bytes))) {
...
bool flushCache(Context context, XFile* xFile) {
	...
	if ((ret = lbfileSetPos(xFile->file, xFile->cacheDirtyIni)) || (ret = lbfileWriteBytes(xFile->file,
			(CharP)&xFile->cache[xFile->cacheDirtyIni - xFile->cacheIni], 0,
			xFile->cacheDirtyEnd - xFile->cacheDirtyIni, &written))) {
		goto error;
	}
