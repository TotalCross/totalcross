             ...
             int32 cacheSize)
{
  ...
  if (cacheSize != -1 &&
      !(xFile->cache =
          xmalloc(xFile->cacheInitialSize = cacheSize))) { // Creates the cache.
...
#else
  if ((ret = lbfileCreate(
         &xFile->file, buffer, isCreation ? CREATE_EMPTY : READ_WRITE))
#endif
...
nfReadBytes(Context context, XFile* xFile, uint8* buffer, int32 count)
{
  ...
  // juliana@202_4: Removed a possible reset or GPF if there is not enough
  // memory to create the file cache on Windows 32, Windows CE, Palm OS, and
  // iPhone.
  if ((xFile->cacheInitialSize < count || xFile->cachePos < xFile->cacheIni ||
       (xFile->cachePos + count) > xFile->cacheEnd) &&
...
nfWriteBytes(Context context, XFile* xFile, uint8* buffer, int32 count)
{
  ...
  xmemmove(&xFile->cache[(cachePos = xFile->cachePos) - xFile->cacheIni],
           buffer,
           count);
...
nfGrowTo(Context context, XFile* xFile, uint32 newSize)
{
  ...
  // The index files grow a bunch per time, so it is necessary to check here if
  // the growth is really needed. If so, enlarges the file.
  if ((ret = lbfileSetSize(&xFile->file, newSize))) {
    goto error;
  }

  // juliana@227_23: solved possible crashes when using a table recovered which
  // was being used with setRowInc().
#if !defined(POSIX) && !defined(ANDROID)
  if (newSize - xFile->size >
      0) { // juliana@230_18: removed possible garbage in table files.
    uint8 zeroBuf[1024];
    int32 remains = newSize - xFile->size, written;
    ...
    while (remains > 0) {
      if ((ret = lbfileWriteBytes(xFile->file,
                                  zeroBuf,
                                  0,
                                  remains > 1024 ? 1024 : remains,
                                  &written))) {
        goto error;
      }
      remains -= written;
...
nfClose(Context context, XFile* xFile)
{
  ...
  if (fileIsValid(xFile->file)) {
    ...
    // juliana@201_5: the .dbo file must be cropped so that it wont't be too
    // large with zeros at the end of the file.
    if (xFile->finalPos &&
        (ret |= lbfileSetSize(&xFile->file, xFile->finalPos))) {
      fileError(context, ret, xFile->name);
    }
...
refreshCache(Context context, XFile* xFile, int32 count)
{
  ...
  if (xFile->cacheIsDirty &&
      !flushCache(context, xFile)) { // Flushes the cache if necessary.
    return false;
  }

  // juliana@223_14: solved possible memory problems.
  if (!xFile->cache ||
      xFile->cacheInitialSize < count) // Increases the cache size if necessary.
    if (!(xFile->cache = xrealloc(xFile->cache,
                                  xFile->cacheInitialSize =
                                    MAX(CACHE_INITIAL_SIZE, count << 2)))) {
      TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
      return false;
    }

    // juliana@closeFiles_1: removed possible problem of the IOException with
    // the message "Too many open files". Some files might have been closed if
    // the maximum number of opened files was reached.
#if defined(POSIX) || defined(ANDROID)
  if ((ret = reopenFileIfNeeded(context, xFile))) {
    goto error;
  }
  ...
  if ((ret = lbfileSetPos(xFile->file, xFile->cachePos)) ||
      (ret = lbfileReadBytes(xFile->file,
                             (CharP)xFile->cache,
                             0,
                             xFile->cacheInitialSize,
                             &bytes))) {
...
flushCache(Context context, XFile* xFile)
{
  ...
  if ((ret = lbfileSetPos(xFile->file, xFile->cacheDirtyIni)) ||
      (ret = lbfileWriteBytes(
         xFile->file,
         (CharP)&xFile->cache[xFile->cacheDirtyIni - xFile->cacheIni],
         0,
         xFile->cacheDirtyEnd - xFile->cacheDirtyIni,
         &written))) {
    goto error;
  }
  ...
  // juliana@227_3: improved table files flush dealing.
  // juliana@226a_22: solved a problem on Windows CE of file data being lost
...
openFile(Context context, XFile* xFile, int32 mode)
{
  ...
  if (filesList.count < MAX_OPEN_FILES) { // There is space in the list.
  ...
  } else { // No space: the last used file must be removed from the list and
           // closed.
    int32 ret = MAX_OPEN_FILES, minStamp, oldest = 0;
    ...
    if ((file = list[oldest])->cacheIsDirty &&
        !(ret = flushCache(context, file))) {
      UNLOCKVAR(files);
      return 1;
    }
...
{
  if (fileIsValid(
        xFile->file)) { // If the file is opened, just updates its time stamp.
    xFile->timeStamp = TC_getTimeStamp();
    return 0;
  } else {
    return openFile(context,
                    xFile,
                    READ_WRITE); // If the file was closed, reopens it.
  }
}
