               ...
               bool isCaseless)
{
  ...
  if (isCaseless) {
    JChar char1, char2;
    while (n--) {
      if ((char1 = TC_JCharToLower(*string1)) ==
          (char2 = TC_JCharToLower(*string2))) {
        string1++;
        string2++;
      } else {
        return (int32)char1 - (int32)char2;
      }
    }
  } else {
    while (n--) {
      if (*string1 == *string2) {
        ...
        return (int32)*string1 - (int32)*string2;
      }
    }
  }
             ...
             bool isCaseless)
{
  TRACE("str16IndexOf")
  JChar c;
  int32 j = (charsLen - subLen), i = 0;
  uint32 len;
  bool found = false;
  JCharP string1, string2;
  ...
  if (isCaseless) {
    ...
    while (!found) {
      while (i <= j &&
             TC_JCharToLower(*charsStr) !=
               c) { // Searches for the next ocurrence of the first char.
        i++;
        charsStr++;
      }
...
{ // rnovais@567_2
  ...
  if (0 <= year && year < 20) { // From 2000 to 2019
    year += 2000;
  } else if (20 <= year && year < 100) { // From 1920 to 1999
    year += 1900;
  }

  // Checks if the day and month values are valid, if the number of days in the
  // month is valid and if the year is not too big.
  if (day > 0 && month >= 1 && month <= 12 &&
      day <= getDaysInMonth(month, year) && year < 3000) {
...
testAndPrepareDate(CharP chars)
{
  ...
  int32 i = -1, j = 0, start = 0, len = 0, n = xstrlen(chars), pos;
  int32 p[3];
  bool err;
  char c;
...
testAndPrepareTime(CharP chars)
{
  ...
  int32 i = -1, j = 0, start = 0, len = 0, n = xstrlen(chars), hour, minutes,
        seconds, millis, pos;
  int32 p[4];
  char c;
  bool err;
  ...
  if (n > 0 && n <= 13) {
    ...
    if ((hour = p[0]) < 0 || hour > 23 || (minutes = p[1]) < 0 ||
        minutes > 59 || (seconds = p[2]) < 0 || seconds > 59 ||
        (millis = p[3]) < 0 || millis > 999) {
      return -1;
    }
  ...
  if (type == DATE_TYPE && (value->asInt = testAndPrepareDate(str)) == -1) {
    ...
    if (posSpace) {
      *posSpace = 0;
      ...
      value->asTime =
        testAndPrepareTime(strTrim(posSpace + 1)); // Gets the time part.
    } else {
...
newIntVector(int32 count, Heap heap)
{
  ...
  iv.length = count;
  iv.size = 0;
  iv.items = (int32*)TC_heapAlloc(iv.heap = heap,
                                  count << 2); // Allocates in the heap.
  return iv;
}
...
IntVectorAdd(IntVector* intVector, int32 value)
{
  ...
  if (intVector->size == intVector->length) {
    ...
    int32* items = (int32*)TC_heapAlloc(
      intVector->heap, (length + 1) << 3); // Allocates in the heap.
...
findNextBitSet(IntVector* intVector, int32 start)
{
  ...
  start &= 31;
  while (1) {
    if ((n = intVector->size - index) > 0 &&
        !intVector->items[index]) { // guich@104
      start = 0;                    // guich@104
      while (n > 0 &&
             !intVector->items[index]) { // Finds the next int with any bit set.
        n--;
        index++;
      }
    }
               ...
               SQLResultSetField** sortFieldList)
{
  ...
  int32 i = -1, index, result;
  SQLResultSetField* field;

  while (++i < sortFieldListCount) { // Compares the records, using the sequence
                                     // used by the sort column List.
    index = (field = sortFieldList[i])->tableColIndex;

    // Compares the elements checking if they are null.
    result = valueCompareTo(null,
                            record1[index],
                            record2[index],
                            field->dataType,
                            isBitSet(nullsRecord1, index),
...
getFullFileName(CharP fileName, TCHARP sourcePath, TCHARP buffer)
{
  ...
  // juliana@223_6: Corrected a bug that would create spourious paths if they
  // had a stress on Windows CE.
  tcscpy(buffer, sourcePath);
  if (sourcePath[endChar] != PATH_SEPARATOR &&
      sourcePath[endChar] != NO_PATH_SEPARATOR) {
    tcscat(buffer, TEXT("/"));
  }
...
{
  TRACE("getTimeLong")
  return (int64)year * (int64)1000000000L * (int64)10L + month * 100000000L +
                      ...
                      int32 asciiLen)
{
  ...
  if (asciiLen > unicodeLen) { // If the substring is greater than the string,
                               // the result i false.
    return false;
  }
...
date2JCharP(int32 year, int32 month, int32 day, JCharP buffer)
{
  ...
  TC_CharP2JCharPBuf(dateTimeBuf,
                     10,
                     buffer,
                     false); // juliana@238_1: corrected the end quote not
                             // appearing in the log files after dates.
}
                ...
                JCharP buffer)
{
  ...
  xstrprintf(dateTimeBuf, "%04d/%02d/%02d", year, month, day);
  xstrprintf(
    &dateTimeBuf[11], "%02d:%02d:%02d:%03d", hour, minute, second, millis);
  dateTimeBuf[10] = ' ';
...
str2short(CharP chars, bool* error)
{
  ...
  // juliana@227_18: corrected a possible insertion of a negative short column
  // being recovered in the select as positive. juliana@225_15: when using short
  // values, if it is out of range an exception must be thrown.
...
str2float(CharP chars, bool* error)
{
  ...
  if ((value = (value < 0) ? -value : value) &&
      (value < MIN_FLOAT_VALUE || value > MAX_FLOAT_VALUE)) {
    *error = true;
  }
...
{
  TCObject object = params->retO =
    TC_createObject(params->currentContext, "totalcross.util.Date");
...
{
  TCObject object = params->retO =
    TC_createObject(params->currentContext, "totalcross.sys.Time");
...
{
  int32 oldCapacity = table->hash + 1, i = oldCapacity, index,
        newCapacity = oldCapacity << 1;
  MemoryUsageEntry** oldTable = table->items;
  MemoryUsageEntry** newTable =
    (MemoryUsageEntry**)xmalloc(TSIZE * newCapacity);
...
muPut(MemoryUsageHT* table, int32 key, int32 dbSize, int32 dboSize)
{
  ...
  if (table->size >=
      table->threshold) { // Rehashs the table if the threshold is exceeded.
    muRehash(table);
    index = key & table->hash;
  }

  if (!(entry = (MemoryUsageEntry*)xmalloc(
          sizeof(MemoryUsageEntry)))) { // Creates the new entry.
    return false;
  }
  entry->key = key;
  entry->dbSize = dbSize;
