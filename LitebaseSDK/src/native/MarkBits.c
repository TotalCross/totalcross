...
int32 onKey(Context context, Key* key, MarkBits* markBits) {
	...
	if (leftOp == OP_REL_EQUAL || leftOp == OP_REL_GREATER_EQUAL || (leftOp == OP_REL_GREATER
			&& markBits->isNoLongerEqual)) {
		...
		if (str16StartsWith(valStr, patStr, valLen, keys0->length, 0,
