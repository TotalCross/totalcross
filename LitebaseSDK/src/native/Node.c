...
bool nodeLoad(Context context, Node* node) {
	...
	xmemset(&children[n + 1], LEAF,
			(index->btreeMaxNodes - n) << 1); // Fills the non-used indexes with TERMINAL.
...
bool nodeSaveDirtyKey(Context context, Node* node, int32 currPos) {
	...
	nfSetPos(fnodes, node->idx * index->nodeRecSize + 2 + index->keyRecSize * currPos +
...
int32 nodeFindIn(Context context, Node* node, Key* key, bool isInsert) { // juliana@201_3
	...
	while (left <= right) {
		if (!(comp = keyCompareTo(context, key, &keys[middle = (left + right) >> 1], numberColumns,
								  plainDB))) {
			return middle;
		} else if (comp < 0) {
			right = middle - 1;
