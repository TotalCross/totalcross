...
/**
 * Defines functions to manipulate a B-Tree. It is used to store the table
...
createNode(Index* index)
{
  ...
  int32 i = (node->index = index)->btreeMaxNodes, j,
  ...
  while (--i >= 0) {
    ...
    while (--j >= 0)
      if (colSizes[j]) {
        key->keys[j].asChars =
          (JCharP)TC_heapAlloc(heap, (colSizes[j] << 1) + 2);
      }
  }
  return node;
}
...
nodeLoad(Context context, Node* node)
{
  ...
  xmemmove(children,
           dataStream,
           ((node->size = n) + 1) << 1); // Loads the node children.

  // juliana@202_3: Solved a bug that could cause a GPF when using composed
  // indices.
  xmemset(&children[n + 1],
          LEAF,
          (index->btreeMaxNodes - n)
            << 1); // Fills the non-used indexes with TERMINAL.
...
nodeSaveDirtyKey(Context context, Node* node, int32 currPos)
{
  ...
  nfSetPos(fnodes,
           node->idx * index->nodeRecSize + 2 + index->keyRecSize * currPos +
             (index->keyRecSize - VALREC_SIZE));
...
nodeSave(Context context, Node* node, bool isNew, int32 left, int32 right)
{
  ...
  Key* keys = node->keys;
  uint8* dataStream = index->basbuf;
  int32 i = right - left, idx = node->idx, nodeRecSize = index->nodeRecSize;

  if (isNew) {
    if ((idx = index->nodeCount++) >= MAX_IDX) { // The index got too large!
      TC_throwExceptionNamed(
        context, "litebase.DriverException", getMessage(ERR_INDEX_LARGE));
      return -1;
    }

    if (index->isWriteDelayed) { // Grows more than 1 record per time.
      if (idx * nodeRecSize == fnodes->size &&
          !nfGrowTo(context, fnodes, (idx + RECGROWSIZE) * nodeRecSize)) {
        return -1;
      }
    } else if (!nfGrowTo(context,
                         fnodes,
                         (idx + 1) *
                           nodeRecSize)) { // Opens space for the node.
      return -1;
    }
  }
  ...
  // juliana@225_2: corrected a possible index corruption when updating each
  // node children. Saves the children;
  ...
  xmemzero(dataStream,
           nodeRecSize -
             (dataStream - index->basbuf)); // Fills the rest with zeros.
...
{ // juliana@201_3
  ...
  Key* keys = node->keys;
  int32 right = node->size - 1, middle, comp,
        numberColumns = index->numberColumns,

        // juliana@201_3: If the insertion is ordered, the position being
        // seached is the last.
    left = (isInsert && index->isOrdered && right > 0) ? right : 0;

  while (left <= right) {
    if (!(comp = keyCompareTo(context,
                              key,
                              &keys[middle = (left + right) >> 1],
                              numberColumns,
                              plainDB))) {
      return middle;
    } else if (comp < 0) {
           ...
           int32 insPos)
{
  ...
  node->size++;
  if (node->index->isWriteDelayed) { // Only saves the key if it is not to be
                                     // saved later.
    node->isDirty = true;
  } else {
...
nodeSetWriteDelayed(Context context, Node* node, bool delayed)
{
  ...
  if (node && node->index->isWriteDelayed && node->isDirty && !delayed &&
      nodeSave(context, node, false, 0, node->size) < 0) {
    return false;
  }
