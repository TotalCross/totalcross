 ...
 */
int32 verifyIfIndexAlreadyExists(Context context, Table* table, uint8* columnNumbers,
								 int32 indexCount) {
	...
	if (indexCount == 1) { // Simple index.
		if (table->columnIndexes[idx = *columnNumbers]) {
			TC_throwExceptionNamed(context, "litebase.AlreadyCreatedException",
								   getMessage(ERR_INDEX_ALREADY_CREATED), table->columnNames[idx]);
			return -1;
		}
		return 0;
	} else { // Composed index.
		...
		while (--i >= 0) {
			...
			if (j == indexCount) {
				...
				if (j < 0) {
					...
					TC_throwExceptionNamed(context, "litebase.AlreadyCreatedException",
										   getMessage(ERR_INDEX_ALREADY_CREATED), errorMsg);
					return -1;
				}
			}
		}
...
bool driverDropIndex(Context context, Table* table, int32 column) {
	...
	if (!index) {
		TC_throwExceptionNamed(context, "litebase.DriverException",
							   getMessage(ERR_COLUMN_DOESNOT_HAVE_AN_INDEX), table->columnNames[column]);
		return false;
	}
 ...
 */
bool driverDropComposedIndex(Context context, Table* table, uint8* columns, int32 size,
							 int32 indexId, bool saveMD) {
	...
	if (i >= 0 && compIndex) { // Removes the index.
		...
		if (table->numberComposedIndexes) {
			(table->composedIndexes[i] = table->composedIndexes[--table->numberComposedIndexes])->indexId = i +
					1;
		} else {
		...
		TC_throwExceptionNamed(context, "litebase.DriverException",
							   getMessage(ERR_COLUMN_DOESNOT_HAVE_AN_INDEX), errorMsg);
		return false;
	}
	if (saveMD) {
...
int32 deleteAllIndexes(Context context, Table* table) {
	...
	while (--i >= 0)
		if (i != primaryKey) {
			ret &= driverDropComposedIndex(context, table, composedIndexes[i]->columns,
										   composedIndexes[i]->numberColumns, i, false);
			count++;
		}
 ...
 */
bool computeColumnOffsets(Context context,
						  Table* table) { // rnovais@568_10: changed from static to global.
			...
			while (--n >= 0) {
				...
				if (TC_htGet32Inv(htName2index, columnHashes[n]) >= 0) {
					...
					TC_throwExceptionNamed(context, "litebase.SQLParseException",
										   getMessage(ERR_DUPLICATED_COLUMN_NAME), (table->columnNames[n]));
					return false;
				}
				TC_htPut32(htName2index, columnHashes[n], n);
			}
...
bool tableLoadMetaData(Context context, Table* table, bool throwException) { // juliana@220_5
	...
	if (!(((ptr[0] & USE_CRYPTO) == 0) ^ plainDB->db.useCrypto) && ptr[1] == ptr[2] == ptr[3] == 0
	...
	if (!plainDB->headerSize) { // The header size can't be zero.
		...
		nfClose(context, dbFile);
		TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_TABLE_CORRUPTED),
							   &table->name[5]);
		return false;
	}
	...
	if (!(flags &= IS_SAVED_CORRECTLY)) {
		if (throwException) {
			// juliana@222_1: the table should not be marked as closed properly if it was not previously closed correctly.
			nfClose(context, dbFile);
			TC_throwExceptionNamed(context, "litebase.TableNotClosedException",
								   getMessage(ERR_TABLE_NOT_CLOSED), &table->name[5]);
			goto error;
		} else {
	...
	while (--i >= 0) { // Loads the indices.
		if ((columnAttrs[i] & ATTR_COLUMN_HAS_INDEX)) {
			...
			IF_HEAP_ERROR(idxHeap) {
				TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
				heapDestroy(idxHeap);
				table->columnIndexes[i] =
					null; // juliana@270_22: solved a possible crash when the table is corrupted.
				goto error;
			}
	...
	if ((table->numberComposedIndexes = *ptr++) > 0) { // Reads the composed indices.
		...
		while (++i < numberComposedIndexes) {
			...
			if (!indexCreateComposedIndex(context, table, table->name, columns, columnSizesIdx, columnTypesIdx,
										  numColumns, indexId, false, exist,
										  idxHeap)
				|| (!exist && flags
					&& (!tableReIndex(context, table, -1, false, table->composedIndexes[indexId - 1])
						|| !setModified(context, table)))) {
				heapDestroy(idxHeap);
				table->composedIndexes[indexId] =
					null; // juliana@270_22: solved a possible crash when the table is corrupted.
	...
	if ((columnCount = table->numberComposedPKCols = *ptr++) >
		...
		xmemmove(table->composedPrimaryKeyCols = (uint8*)TC_heapAlloc(heap, columnCount), ptr,
				 columnCount);
	}
...
bool tableSaveMetaData(Context context, Table* table, int32 saveType) {
	...
	if (saveType == TSMD_EVERYTHING)
		size += getStringsTotalSize(table->columnNames,
									table->columnCount) + computeDefaultValuesMetadataSize(table)
	...
	*ptr = (plainDB->db.useCrypto ? (plainDB->useOldCrypto ? 1 : USE_CRYPTO) :
			0); // juliana@253_8: now Litebase supports weak cryptography.
	xmove2(ptr + 4, &plainDB->headerSize); // Saves the header size.
	ptr += 6;
	*ptr++ = plainDB->isAscii ? IS_ASCII | !table->isModified :
			 !table->isModified; // juliana@226_4: table is not saved correctly yet if modified.
	...
	if (saveType !=
		TSMD_ONLY_DELETEDROWSCOUNT) { // More things other than the deleted rows count must be saved.
		...
		if (saveType != TSMD_ONLY_AUXROWID) { // More things other than the auxiliary row id must be saved.
			...
			*ptr++ = 0;
			...
			if (saveType !=
				TSMD_ONLY_PRIMARYKEYCOL) { // More things other than the primary key col must be saved.
				...
				if (saveType == TSMD_EVERYTHING) { // Stores the rest.
					...
					ptr = writeStringArray(ptr += (n << 2), table->columnNames,
					...
					while (++i < n) { // Saves the default values.
						if ((columnAttrs[i] & ATTR_COLUMN_HAS_DEFAULT))
							switch (columnTypes[i]) {
								case CHARS_NOCASE_TYPE:
								case CHARS_TYPE:
									ptr = writeString16(ptr, defaultValues[i]->asChars, MIN((int32)defaultValues[i]->length,
														columnSizes[i]));
									break;
					...
					n = *ptr++ =
							table->numberComposedPKCols; // Number of columns on composed primary key. If 0, there's no composed primary key.
	...
	if (!plainWriteMetaData(context, plainDB, ptr0, (int32)(ptr - ptr0))) {
		...
		flushCache(context,
				   &plainDB->db);    // juliana@223_11: table meta data is now always flushed imediately after being changed.
	}
	if (size > SECTOR_SIZE) {
		xfree(ptr0);
	}
 ...
 */
bool tableSetMetaData(Context context, Table* table, CharP* names, int32* hashes, int8* types,
					  int32* sizes, uint8* attrs, uint8* composedPKCols,
					  SQLValue** defaultValues, int32 primaryKeyCol, int32 composedPK, int32 columnCount,
	...
	if (*table->name) { // juliana@201_14: It is not necessary to save the meta data in the .db for memory tables.
		if (table->db.db.size) { // The table can't be already created.
			TC_throwExceptionNamed(context, "litebase.AlreadyCreatedException",
								   getMessage(ERR_TABLE_ALREADY_CREATED), table->name);
			return false;
		}
		...
		return tableSaveMetaData(context, table,
 ...
 */
bool reorder(Context context, Table* table, CharP* fields, SQLValue** record, uint8* storeNulls,
			 uint8* nValues, uint8* paramIndexes) {
	TRACE("reorder")
	...
	SQLValue* outRecord[MAXIMUMS +
								 1]; // Just to store the temporary values, which will be copied over later.
	...
	while (++i < length) { // Makes sure that the fields are in table creation order.
		...
		if (idx < 0) {
			TC_throwExceptionNamed(context, "litebase.SQLParseException", getMessage(ERR_INVALID_COLUMN_NAME),
								   fields[i]);
			return false;
		}

		setBit(tableStoreNulls, idx, isBitSet(storeNulls, i));
		if ((value = outRecord[idx] = record[i]) && (asChars = value->asChars) && asChars[0] == (JChar)'?'
			&& !asChars[1]) {
 ...
 */
bool sortTable(Context context, Table* table, SQLColumnListClause* groupByClause,
	...
	if ((orderByClause
		 && !bindColumnsSQLColumnListClause(context, orderByClause, &table->htName2index, table->columnTypes,
											null, 0))
		|| (groupByClause
			&& !bindColumnsSQLColumnListClause(context, groupByClause, &table->htName2index, table->columnTypes,
					null, 0))) {
		heapDestroy(heap);
		return false;
	}
	...
	return quickSort(context, table, newSQLValues(count, heap), newSQLValues(count, heap),
					 newSQLValues(count, heap), sortListClause->fieldList,
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 */
bool quickSort(Context context, Table* table, SQLValue** pivot, SQLValue** someRecord1,
			   SQLValue** someRecord2, SQLResultSetField** fieldList,
	...
	while (--count >= 0) { // Only loads columns used by the sorting process.
		...
		if ((columnSize = field->table->columnSizes[TC_htGet32(&field->table->htName2index,
						  field->tableColHashCode)])) {
	...
	while (size) { // guich@212_3: removed recursion (storing in a IntVector).
		...
		if (!readRecord(context, table, pivot,
						pivotIndex = (last = high) == (first = low) ? last : randBetween(first, last), columnNulls3,
						fieldList,
		...
		while (true) {
			while (high >= low) { // Finds the partitions.
				...
				if (!readRecord(context, table, someRecord1, low, columnNulls1, fieldList, fieldsCount, true, heap,
								stringArray)) {
					goto error;
				}
			...
			while (high >= low) {
				if (!readRecord(context, table, someRecord2, high, columnNulls2, fieldList, fieldsCount, true, heap,
								stringArray)) {
					goto error;
				}
 ...
 */
int32 compareSortRecords(int32 recSize, SQLValue** vals1, SQLValue** vals2,
						 int8* types) { // juliana@201_3
	TRACE("compareSortRecords")
	int32 i = -1,
		  result;

	while (++i <
		   recSize) // Does the comparison between the values till one of them is different from zero.
 ...
 */
int64 radixPass(int32 start, SQLValue*** source, SQLValue*** dest, int32* count, int32* index,
				int32 type, int32 length) { // juliana@201_3
	TRACE("radixPass")
	int32 i = 0,
		  n = length,
		  ibits = 0,
 ...
 */
Table* tableCreate(Context context, CharP name, TCHARP sourcePath, bool create, bool isAscii,
				   bool useCrypto, int32* nodes,
	...
	if (!createPlainDB(context, &table->db, name, create, useCrypto,
 ...
 */
Table* driverCreateTable(Context context, TCObject driver, CharP tableName, CharP* names,
						 int32* hashes, int8* types, int32* sizes, uint8* attrs,
						 SQLValue** defaultValues, int32 primaryKeyCol, int32 composedPK, uint8* composedPKCols,
						 int32 composedPKColsSize, int32 count, Heap heap) {
	...
	if (!tableName) { // Temporary table.
		...
		if (!(table = tableCreate(context, null, sourcePath, true, false, false, getLitebaseNodes(driver),
								  true, heap))) {
			return null;
		}
		...
		if (!tableSetMetaData(context, table, null, hashes, types, sizes, null, null, null, -1, -1, count,
							  0)) {
			goto error;
		}
	} else { // Normal table.
		...
		// juliana@220_5
		// juliana@253_8: now Litebase supports weak cryptography.
		if (!(table = tableCreate(context, name, sourcePath, true, OBJ_LitebaseIsAscii(driver),
		...
		if (!TC_htPutPtr(htTables,  TC_hashCode(tableName), table)
			|| !tableSetMetaData(context, table, names, hashes, types, sizes, attrs, composedPKCols,
								 defaultValues, primaryKeyCol, composedPK, count,
								 composedPKColsSize)
			|| (primaryKeyCol != NO_PRIMARY_KEY
				&& !driverCreateIndex(context, table, &hashes[primaryKeyCol], 0, 1, null))
			|| (composedPKColsSize > 0
				&& !driverCreateIndex(context, table, null, 0, composedPKColsSize, composedPKCols))) {
			TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
			goto error;
		}
	}
 ...
 */
bool renameTable(Context context, TCObject driver, Table* table,
				 CharP newTableName) { // rnovais@566_10
	TRACE("renameTable")
	...
	if (!TC_htPutPtr(htTables, TC_hashCode(newTableName),
 ...
 * @throws OutOfMemoryError If a memory allocation fails.
 */
bool renameTableColumn(Context context, Table* table, CharP oldColumn, CharP newColumn,
					   bool reuseSpace) { // rnovais@566_17
	...
	if (newIdx >= 0) {
		TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_COLUMN_ALREADY_EXIST),
							   newColumn);
		return false;
	}
 ...
 * @throws OutOfMemoryError If a heap memory allocation fails.
 */
bool tableReIndex(Context context, Table* table, int32 column, bool isPKCreation,
	...
	if (index->isOrdered && !composedIndex) { // Simple index using rowid.
		...
		while (++i < n) {
			...
			// juliana@230_12
			if (!readValue(context, plainDB, &emptyValue, 0, INT_TYPE, basbuf, false, false, false, -1,
						   null) // juliana@220_3
		...
		while (++i < n) {
			...
			if (column != -1) {
				// juliana@230_12
				if (!readValue(context, plainDB, *values[k], offset, type, basbuf, false,
							   isNull = isBitSet(columnNulls0, column), false, -1, heap)) {
					goto error2;
				}
				...
				while (--j >= 0) {
					// juliana@230_12
					if (!readValue(context, plainDB, values[k][j], columnOffsets[columns[j]], types[j], basbuf, false,
								   isNull |= isBitSet(columnNulls0, columns[j]), false, -1, heap)) {
						goto error2;
					}
		...
		if (!index->isOrdered) {
			...
			if (indexSize == 1 && (type == SHORT_TYPE || type == INT_TYPE || type == LONG_TYPE
								   || type == DATE_TYPE)) {
		...
		while (++k < rows) {
			...
			if (isPKCreation && k > 0
				&& (!(compare = compareSortRecords(indexSize, values[k], values[k - 1], types))
					|| compare == MIN_SHORT_VALUE
					|| compare == MAX_SHORT_VALUE)) {
				TC_throwExceptionNamed(context, "litebase.PrimaryKeyViolationException",
									   getMessage(ERR_STATEMENT_CREATE_DUPLICATED_PK), table->name);
	...
	return indexSetWriteDelayed(context, index,
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 */
bool indexCreateIndex(Context context, Table* table, CharP fullTableName, int32 columnIndex,
					  int32* columnSizes, int8* columnTypes, bool exist, Heap heap) {
	...
	// rnovais@113_1
	if (!(table->columnIndexes[columnIndex] = createIndex(context, table, columnTypes, columnSizes,
			indexName, 1, exist, heap))) {
		return false;
	}
 ...
 */
bool indexCreateComposedIndex(Context context, Table* table, CharP fullTableName,
							  uint8* columnIndexes, int32* columnSizes, int8* columnTypes,
	...
	int32 tableLen = xstrlen(fullTableName),
		  ...
		  id = (newIndexNumber < 0) ? -newIndexNumber :
			   newIndexNumber; // newIndexNumber < 0 means that this was a index.
	ComposedIndex* composedIndex;
	...
	if (!(composedIndex->index = createIndex(context, table, columnTypes, columnSizes, indexName,
								 numberColumns, exist, heap))) {
		return false;
	}
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 */
bool readRecord(Context context, Table* table, SQLValue** record, int32 recPos, uint8* columnNulls,
	...
	xmemmove(columnNulls, basbuf + columnOffsets[table->columnCount],
	...
	if (fieldList) // Reads only the columns used during sorting.
		while (--i >= 0) {
			...
			if ((columnTypes[j] != CHARS_TYPE && columnTypes[j] != CHARS_NOCASE_TYPE)
				...
				// juliana@230_12
				if (!readValue(context, plainDB, record[j], columnOffsets[j], columnTypes[j], basbuf, isTemp,
							   isBitSet(columnNulls, j), isTempBlob, -1,
							   null)) {
					return false;
				}
				if (record[j]->length) {
	...
	// juliana@230_12: improved recover table to take .dbo data into consideration.
		...
		while (--i >= 0)
			if (!readValue(context, plainDB, record[i], columnOffsets[i], columnTypes[i], basbuf, isTemp,
						   isBitSet(columnNulls, i), isTempBlob, columnSizes[i], heap)) {
				return false;
			}
	return true;
}
...
bool writeRecord(Context context, Table* table, SQLValue** values, int32 recPos, Heap heap) {
	...
	if (addingNewRecord) { // Adding a record?
		...
		if (!plainRead(context, plainDB,
					   recPos)) { // May have to read the value before deleting the index value.
			return false;
		}
		writePos = recPos;
		xmove4(&rowid, basbuf);
	...
	while (++i < columnCount) {
		...
		if (!addingNewRecord) { // Reads the previous value if it is an update.
			...
			if (!readValue(context, plainDB, vOlds[i], offset, type, basbuf, false,
						   isNullVOld = isBitSet(columnNulls1, i), false, -1, heap)) {
				return false;
			}
		...
		// Writes the value.
		if (!writeValue(context, plainDB, values[i], &basbuf[offset], type, columnSizes[i], valueOk,
						addingNewRecord || changePos, isNull, false)) {
			return false;
		}
		...
		if (hasIndex) {
			...
			if (addingNewRecord || valueCompareTo(null, vOlds[i], values[i], type, isNullVOld, isNull, null)) {
				if (!isNull) { // If it is updating a 'non-null value' to 'null value', only removes it.
					...
					if (!indexAddKey(context, idx, &tempRecord,
									 writePos)) { // juliana@223_14: solved possible memory problems.
						return false;
					}
					nfSetPos(db, oldPos);
				}
		...
		while (--i >= 0) {
			...
			while (--j >= 0) {
				...
				if (values[column]) { // juliana@201_18: can't reuse values. Otherwise, it will spoil the next update.
					...
					if (addingNewRecord
						|| valueCompareTo(null, values[column], vOlds[column], columnTypes[column], !remove, !valueOk,
										  null)) {
						change = true;
					}
				} else {
			...
			// juliana@252_2: corrected a bug of possible composed index corruption when updating or deleting data.
			if (valueOk
				&& change) { // juliana@201_4: corrected a bug that could corrupt the table when updating the composed index.
	...
	basbuf[3] =
		k; // juliana@270_23: Corrected a RowIterator bug of an update changing an updated row to synced again.
	...
	// Flushs .db and .dbo.
	if (!db->dontFlush)
		if ((db->cacheIsDirty && !flushCache(context, db)) || (dbo->cacheIsDirty
				&& !flushCache(context, dbo))) {
			return false;
		}
...
bool writeRSRecord(Context context, Table* table, SQLValue** values) {
	...
	while (--n >= 0)
		if (!writeValue(context, plainDB, values[n], &basbuf[offsets[n]], types[n], sizes[n],
						values[n] != null, true,
 ...
 */
bool checkPrimaryKey(Context context, Table* table, SQLValue** values, int32 recPos, bool newRecord,
					 Heap heap) {
	...
	if (!newRecord) { // An update.
		...
		while (--i >= 0) {
			...
			if (!readValue(context, plainDB, &oldValues[i], offsets[columns[i]], types[i], basbuf, false, false,
						   false, -1, heap)) {
				return false;
			}
 ...
 */
bool verifyNullValues(Context context, Table* table, SQLValue** record, int32 statementType,
					  int32 nValues) {
	...
	if (statementType == CMD_INSERT) { // Insert statement.
		...
		if ((i != NO_PRIMARY_KEY) && (isBitSet(storeNulls, i) || ((!record[i] || record[i]->isNull)
									  && !defaultValues[i]))) {
			TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_PK_CANT_BE_NULL));
			return false;
		}
		...
		while (--i)
			if ((!record[i] || record[i]->isNull) && !defaultValues[i]
				...
				TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_FIELD_CANT_BE_NULL),
									   table->columnNames[i]);
				return false;
			}
		...
		while (--i >= 0)
			if (isBitSet(storeNulls, i)
				&& definedAsNotNull(attrs[i])) { // If it is to store a null but a null can't be stored.
				TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_FIELD_CANT_BE_NULL),
									   table->columnNames[i]);
				return false;
			}
...
bool freeTable(Context context, Table* table, bool isDelete, bool updatePos) {
	...
	if (table) {
		...
		if (*table->name
			&& (i = table->numberComposedIndexes)) { // Frees the composed indices in a normal table.
			while (--i >= 0) {
...
bool getTableColValue(Context context, ResultSet* resultSet, int32 column, SQLValue* value) {
	...
	// juliana@230_12
	return readValue(context, &table->db, value, table->columnOffsets[column],
					 table->columnTypes[column], table->db.basbuf, !*table->name,
...
bool tableExistsByName(Context context, TCObject driver, CharP name) {
	...
	if (lbfileExists(fullName)) {
		...
		TC_throwExceptionNamed(context, "litebase.AlreadyCreatedException",
							   getMessage(ERR_TABLE_ALREADY_CREATED), name);
		return true;
	}
	return false;
}
...
Table* getTable(Context context, TCObject driver, CharP tableName) {
	...
	TC_CharPToLower(
		tableName); // The hash code must be in lower case so that the program is more efficient.
	...
	if (!(table = (Table*)TC_htGetPtr(htTables, hashCode = TC_hashCode(tableName)))) {
		...
		if (getDiskTableName(context, appCrid, tableName, name)) {
			...
			// juliana@220_5
			// juliana@253_8: now Litebase supports weak cryptography.
			if ((table = tableCreate(context, name, getLitebaseSourcePath(driver), false,
									 OBJ_LitebaseIsAscii(driver), OBJ_LitebaseUseCrypto(driver),
									 getLitebaseNodes(driver), true, heap)) && table->db.db.size) {
				if (!TC_htPutPtr(htTables, hashCode,
								 table)) { // Puts the table hash code in the hash table of opened tables.
					freeTable(context, table, false, false);
					TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
					return null;
				}
