...
/**
 * This has the function definitions for a database in a plain binary file. The
 * data and the metadata (header) is written in one file (.db). The strings and
 * the blobs are written in the .dbo file. The current number of records inside
 * the database is discovered only when the database is open by getting its size
 * and discounting the header size. This has a double advantage: it is not
 * necessary to waste space storing the current record count, and it is not
 * needed to save the record count at each insertion.
 *
 * This also has function definitions for a temporary database for
 * <code>ResultSet</code> tables.
 */
...
/**
 * Creates a new <code>PlainDB</code>, loading or creating the table with the
 * given name or creating a temporary table.
 *
 * @param context The thread context where the function is being executed.
 * @param plainDB Receives the new <code>PlainDB</code> or <code>null</code> if
 * an error occurs.
              ...
              TCHARP sourcePath)
{
  ...
  if (name) {
    ...
    if (nfCreateFile(
          context, buffer, create, useCrypto, sourcePath, &plainDB->db, -1) &&
        xstrcat(buffer, "o") &&
        nfCreateFile(
          context, buffer, create, useCrypto, sourcePath, &plainDB->dbo, -1)) {
      return true;
    }
  } else {
  ...
  plainClose(context,
             plainDB,
             false); // Closes the table files if an error occurs.
  return false;
}
 ...
 */
void
plainSetRowSize(PlainDB* plainDB, int32 newRowSize, uint8* buffer)
{
  ...
  if (plainDB->db.size >=
      plainDB->headerSize) { // Finds how many records are there.
    plainDB->rowCount =
      (plainDB->db.size - plainDB->headerSize) / plainDB->rowSize;
  }
}

/**
 * Adds a new record. The file pointer is positioned in the record's beginning
 * so that the data can be written. Usually the record is first added, then the
 ...
 */
bool
plainAdd(Context context, PlainDB* plainDB)
{
  TRACE("plainAdd")

  if (--plainDB->rowAvail <=
      0) { // Checks if there are no more space pre-allocated.
    if (!plainDB->growTo(context,
                         &plainDB->db,
                         (plainDB->rowCount + plainDB->rowInc) *
                             plainDB->rowSize +
                           plainDB->headerSize)) {
      return false;
    }
  ...
  return plainSetPos(
    context,
    plainDB,
    plainDB->rowCount); // Sets the position to the start of the record.
}
 ...
 */
bool
plainWrite(Context context, PlainDB* plainDB)
{
  TRACE("plainWrite")
  if (plainDB->writeBytes(
        context, &plainDB->db, plainDB->basbuf, plainDB->rowSize)) {
    plainDB->rowCount++;
    return true;
  }
  return false;
 ...
 */
bool
plainRead(Context context, PlainDB* plainDB, int32 record)
{
  TRACE("plainRead")
  return plainSetPos(context, plainDB, record) &&
         plainDB->readBytes(
           context, &plainDB->db, plainDB->basbuf, plainDB->rowSize);
}
 ...
 */
bool
plainRewrite(Context context, PlainDB* plainDB, int32 record)
{
  TRACE("plainRewrite")
  return plainSetPos(context, plainDB, record) &&
         plainDB->writeBytes(
 ...
 */
bool
plainRename(Context context, PlainDB* plainDB, CharP newName, TCHARP sourcePath)
{
  TRACE("plainRename")
  char buffer[DBNAME_SIZE];
  ...
  if (nfRename(context,
               &plainDB->db,
               buffer,
               sourcePath)) { // Renames the .db file.
    xstrcat(buffer, "o");
    if (!nfRename(context,
                  &plainDB->dbo,
                  buffer,
                  sourcePath)) { // Renames the .dbo file.
      // If the file could not be renamed, which is unlikely to occur, the .db
      // file should be renamed back.
 ...
 */
bool
plainWriteMetaData(Context context,
                   PlainDB* plainDB,
                   uint8* buffer,
                   int32 length)
{
  ...
  if (!plainDB->db.size) { // The metadata size must have a free space for
                           // future composed indices or composed primary key.
    // juliana@230_7: corrected a possible exception or crash when the table has
    // too many columns and composed indices or PKs.
    ...
    // juliana@223_15: solved a bug that could corrupt tables created with a
    // very large metadata size.
    buffer[4] = (uint8)headerSize;
    buffer[5] = (uint8)(headerSize >> 8);
  }
  nfSetPos(db, 0);

  if (db->useCrypto) { // juliana@253_8: now Litebase supports weak
                       // cryptography.
    int32 i = 4;
    while (--i >= 0) {
      *buffer++ ^= 0xAA;
 ...
 */
uint8*
plainReadMetaData(Context context, PlainDB* plainDB, uint8* buffer)
{
  ...
  if (plainDB->db.useCrypto) { // juliana@253_8: now Litebase supports weak
                               // cryptography.
    int32 i = 4;
    while (--i >= 0) {
      *buffer++ ^= 0xAA;
 ...
 * @param context The thread context where the function is being executed.
 * @param plainDB The <code>PlainDB</code>.
 * @param updatePos Indicates if <code>finalPos</code> must be re-calculated to
 * shrink the file.
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 */
bool
plainClose(Context context, PlainDB* plainDB, bool updatePos)
{
  TRACE("plainClose")
  bool ret = true;
  ...
  if (plainDB) {
    if (plainDB->db.fbuf || fileIsValid(plainDB->db.file) ||
      ...
      if (*plainDB->name) {
        ...
        xmemzero(buffer, 4);
        *pointer =
          (plainDB->db.useCrypto ? (plainDB->useOldCrypto ? 1 : USE_CRYPTO)
                                 : 0);
        xmove2(pointer + 4, &plainDB->headerSize);
        pointer += 6;

        // The table format must also be saved.
        *pointer++ = plainDB->isAscii
                       ? IS_ASCII | !plainDB->wasNotSavedCorrectly
                       : !plainDB->wasNotSavedCorrectly;

        ret = plainWriteMetaData(context, plainDB, buffer, 7);

        if (updatePos) { // Calculates .db used space: .db won't have zeros at
                         // the end.
          plainDB->db.finalPos =
            plainDB->rowCount * plainDB->rowSize + plainDB->headerSize;
        }
      }
      ret &= plainDB->close(context, &plainDB->db); // Closes .db.
    }
    if (plainDB->dbo.fbuf || fileIsValid(plainDB->dbo.file) ||
        plainDB->dbo.cache) { // Closes .dbo if it's open.
 ...
 */
bool
plainRemove(Context context, PlainDB* plainDB, TCHARP sourcePath)
{
  TRACE("plainRemove")
  bool ret = true;
 ...
 */
bool
plainSetPos(Context context, PlainDB* plainDB, int32 record)
{
  TRACE("plainSetPos")
  if (record >= 0 && record < (plainDB->rowCount + plainDB->rowAvail)) {
    // If the table grows too much, the real position can be negative because
    // the integer size has a limit.
    int32 value =
      record * plainDB->rowSize + (*plainDB->name ? plainDB->headerSize : 0);
    if (value < 0) {
      TC_throwExceptionNamed(context,
                             "litebase.DriverException",
                             getMessage(ERR_INVALID_POS),
                             record);
      return false;
    }
  ...
  if (record >= 0) {
    TC_throwExceptionNamed(
      context, "litebase.DriverException", getMessage(ERR_INVALID_POS), record);
  }
  return false;
}
...
/**
 * Compresses the memory file buffers at the current position. This is necessary
 * so that the memory tables do not have unused space after computing the
 * select.
 *
 * @param context The thread context where the function is being executed.
 * @param plainDB The <code>PlainDB</code>.
 ...
 */
bool
plainShrinkToSize(Context context, PlainDB* plainDB)
{
  ...
  if (plainDB->rowCount > 0 && plainDB->rowAvail > 0) {
    ...
    if ((int32)plainDB->dbo.size != plainDB->dbo.finalPos) {
      if (mfGrowTo(context,
                   &plainDB->dbo,
                   plainDB->dbo.finalPos)) { // Shrinks the .dbo.
 ...
 * @param isTemporary Indicates if this is a result set table or the value or
 * the integer of a rowid index is to be loaded.
  ...
  if (!isNull)
    switch (colType) {
      ...
      case CHARS_TYPE: {
        ...
        if (position <
            0) { // juliana@270_22: solved a possible crash when the table is
        ...
        value->asBlob =
          (uint8*)plainDB; // Holds the plainDB pointer so the string don't
                           // need to be loaded in the temporary table.
        ...
        if (!value->asChars) { // Allocates the string if it was not previoulsy
                               // allocated.
      ...
      case BLOB_TYPE:     // juliana@220_3: blobs are not loaded anymore in the
                          // temporary table and sortings.
        if (isTempBlob) { // A blob is being read to a temporary table.
          ...
          if (position > dbo->finalPos) {
            value->length = 0;
            return true;
          } else if (position < 0) { // juliana@270_22: solved a possible crash
                                     // when the table is corrupted on Android
          ...
          if (!plainDB->readBytes(context,
                                  dbo,
                                  (uint8*)&length,
                                  4)) { // Reads the blob size;
            return false;
          }
          ...
          if ((value->length = length) > 0 && value->asBlob &&
              !plainDB->readBytes(context, dbo, value->asBlob, length)) {
            return false;
          }
        }
    }
  ...
  if (isValueOk && !isNull)
    switch (colType) {
      ...
      case CHARS_TYPE:
        if (isTempBlob) {
          ...
          if ((dbo->finalPos + (size = TSIZE + 4)) > (int32)dbo->size &&
              !plainDB->growTo(
                context, dbo, dbo->size + size * MAX(16, plainDB->rowInc))) {
            return false;
          }
          ...
          if (!plainDB->writeBytes(context, dbo, (uint8*)&value->asInt, 4) ||
              !plainDB->writeBytes(
                context, dbo, (uint8*)&value->asBlob, TSIZE)) {
            return false;
          }
          ...
          int32 length =
                  value
                    ->length, // juliana@225_7: the string that is bigger than
                              // its field definiton was already trimmed.
            size = plainDB->isAscii
                     ? (2 + length)
                     : (2 + (length << 1)), // Computes the string size.
            ret = true;
          XFile* dbo = &plainDB->dbo;
          ...
          // This removes possible bugs when doing updates. guich@201_8: grows
          // using rowInc instead of 16 if rowInc > 16.
          if ((dbo->finalPos + size) > (int32)dbo->size &&
              !plainDB->growTo(
                context, dbo, dbo->size + size * MAX(16, plainDB->rowInc))) {
            return false;
          }
          ...
          // Writes the string.
          if (plainDB->isAscii) { // juliana@210_2: now Litebase supports tables
                                  // with ascii strings.
            int32 i = length;
            CharP from = (CharP)value->asChars, to = (CharP)value->asChars;

            while (--i >=
                   0) { // Transforms the unicode string into an ascii string.
              *to++ = *from;
              from += 2;
            }

            if (!plainDB->writeBytes(context, dbo, (uint8*)&length, 2) ||
                (length && !plainDB->writeBytes(
                             context, dbo, (uint8*)value->asChars, length))) {
              ret = false;
            }

            // The string MUST be transformed back. Otherwise, it may mess a
            // Java string up.
            ...
            while (--i >= 0) {
              ...
              *from-- = 0;
              to -= 2;
                     ...
                     (length &&
                      !plainDB->writeBytes(
                        context, dbo, (uint8*)value->asChars, length << 1))) {
            return false;
          }

          dbo->finalPos = dbo->position; // juliana@202_21: the final positon
                                         // now is always the new positon.
          return ret;
        }
      ...
      case BLOB_TYPE: {
        ...
        if (isTempBlob) { // juliana@220_3: blobs are not loaded anymore in the
          ...
          int32 oldPos = 0, length = MIN((int32)value->length, colSize);
          ...
          if (addingNewRecord && (dbo->finalPos + size) > (int32)dbo->size &&
              !plainDB->growTo(
                ...
                dbo->size +
                  size * MAX(16,
                             plainDB->rowInc))) { // guich@201_8: grow using
                                                  // rowInc instead of 16 if
                                                  // rowInc > 16
            return false;
          }

          // It is an insert or the size of the blob is greater then the old,
          // writes the blob at the end of the .dbo.
          if (addingNewRecord) {
            plainDB->setPos(dbo, dbo->finalPos);
          } else {
          ...
          if (!plainDB->writeBytes(context, dbo, (uint8*)&length, 4) ||
              (length &&
               !plainDB->writeBytes(context, dbo, value->asBlob, length))) {
            return false;
          }

          if (addingNewRecord) { // It is an insert or the size of the blob is
                                 // greater then the old one, the final positon
                                 // is the new positon.
            dbo->finalPos = dbo->position;
 ...
 * @return <code>false</code> if the record is deleted; <code>true</code>
 * otherwise.
 */
int32
recordNotDeleted(uint8* buffer)
{
  TRACE("recordNotDeleted")
  int32 attr;

  xmove4(&attr, buffer);
  return (attr & ROW_ATTR_MASK) !=
         ROW_ATTR_DELETED; // When a row is deleted, its rowid is set to 0.
}
 ...
 */
bool
loadString(Context context, PlainDB* plainDB, JCharP string, int32 length)
{
  TRACE("loadString")

  if (plainDB->isAscii) { // juliana@210_2: now Litebase supports tables with
                          // ascii strings.
    int32 i = length - 1;
    CharP str = (CharP)string, from = str + i, to = from + i;

    if (!plainDB->readBytes(context,
                            &plainDB->dbo,
                            (uint8*)str,
                            length)) { // Reads the string.
      return false;
    }
  ...
  } else if (!plainDB->readBytes(context,
                                 &plainDB->dbo,
                                 (uint8*)string,
                                 length << 1)) { // Reads the string.
    return false;
  }
  return true;
}
