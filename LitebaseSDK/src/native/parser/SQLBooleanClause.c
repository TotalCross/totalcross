...
SQLBooleanClause* initSQLBooleanClause(Heap heap) {
	...
	SQLBooleanClause* sqlBooleanClause = (SQLBooleanClause*)TC_heapAlloc(heap,
										 sizeof(SQLBooleanClause));
 ...
 */
bool applyTableIndexes(SQLBooleanClause* booleanClause, Index** tableIndexes, int32 columnsCount,
	...
	while (curTree) {
						...
						if (count >= 2) { // It has an AND operator on at least 2 fields?
							...
							while (--i >= 0) {
								...
								if (appliedComposedIndex) {
									...
									curTree = applyComposedIndexToBranch(booleanClause, originalTree, columns, operators,
																		 indexesValueTree, currCompIndex);
									break;
								} else {
		...
		if (booleanClause->appliedIndexesCount ==
			MAX_NUM_INDEXES_APPLIED) { // If the number of indexes to be applied reached the limit leaves the loop.
			break;
		}
 ...
 */
void applyIndexToBranch(SQLBooleanClause* booleanClause, SQLBooleanClauseTree* branch,
						Index** indexesMap, bool isLeft) {
		...
		if (indexesMap[column]) { // Checks if the column is indexed.
			...
			if (parent) {
				...
				booleanClause->expressionTree =
					null;    // The branch has no parent. So, no expression tree will be left.
			}
		}
	}
}
 ...
 */
SQLBooleanClauseTree* applyComposedIndexToBranch(SQLBooleanClause* booleanClause,
		SQLBooleanClauseTree* branch, uint8* columns, uint8* operators,
...
void applyIndexToBranchJoin(SQLBooleanClause* booleanClause, SQLBooleanClauseTree* branch,
	...
	if (branch->bothAreIdentifier
		&& fieldList[fieldIndex = getFieldIndex(right)]->table->columnIndexes[right->colIndex]) {
		right->hasIndex = true;
	}
	...
	if (leftIsColumn != rightIsColumn) {
		...
		if ((table = (field = fieldList[fieldIndex = getFieldIndex(tree)])->table)->columnIndexes[column]
	...
	if (booleanClause) { // guich@504_14
		...
		while (--i >= 0)
			if (!paramList[i]->isParamValueDefined) {
				TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_PARAMETER_NOT_DEFINED),
									   i);
				return false;
			}
	}
 ...
 */
int32 sqlBooleanClauseSatisfied(Context context, SQLBooleanClause* booleanClause,
								ResultSet* resultSet, Heap heap) {
 ...
 */
bool bindColumnsSQLBooleanClause(Context context, SQLBooleanClause* booleanClause,
	...
	if (tableList != null) { // The having clause has already been verified.
		if (!verifyColumnNamesOnTableList(context, booleanClause->fieldList, booleanClause->fieldsCount,
										  tableList, tableListSize)) {
			return false;
		}
		...
		while (--i >= 0) {
			field = fieldList[i];
			field->tableColIndex = TC_htGet32Inv(names2Index,
	...
	return bindColumnsSQLBooleanClauseTree(context,
										   booleanClause->expressionTree); // Binds the field information in the tree to the table columns.
}
 ...
 */
bool verifyColumnNamesOnTableList(Context context, SQLResultSetField** fieldList, int32 fieldsCount,
	...
	while (--fieldsCount >= 0) {
		if ((field = fieldList[fieldsCount])->tableName) {
			...
			if (!currentTable
				|| (index = TC_htGet32Inv(&currentTable->htName2index, TC_hashCode(field->tableColName))) < 0) {
 ...
 * @return <code>true</code>, if the boolean clause was bound successfully; <code>false</code>, otherwise.
 */
bool bindColumnsSQLBooleanClauseSimple(Context context, SQLBooleanClause* clause,
									   SQLResultSetTable* rsTable, Heap heap) {
	...
	return bindColumnsSQLBooleanClauseTree(context,
										   clause->expressionTree); // Binds the field information in the tree to the table columns.
}
 ...
 * @throws SQLParseException If there is an unknown or an ambiguos column name.
 */
bool verifyColumnNamesOnTable(Context context, SQLResultSetField** fieldList, int32 fieldsCount,
