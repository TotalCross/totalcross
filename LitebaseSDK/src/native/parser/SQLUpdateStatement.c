 ...
 */
SQLUpdateStatement* initSQLUpdateStatement(Context context, TCObject driver, LitebaseParser* parser,
		bool isPrepared) {
	TRACE("sqlUpdateStatement")
	Heap heap = parser->heap;

	// Creates a new update statement.
	SQLUpdateStatement* updateStmt = (SQLUpdateStatement*)TC_heapAlloc(heap,
									 sizeof(SQLUpdateStatement));
	...
	if (isPrepared) { // Some structures from the parser does not need to be reallocated when not using prepared statements.
		updateStmt->rsTable = initSQLResultSetTable((*parser->tableList)->tableName,
							  (*parser->tableList)->aliasTableName, heap);
	...
	if (!(updateStmt->rsTable->table = table = getTable(context, driver,
	...
	while (--i >= 0) {
		...
		if (TC_hashCode(fields[i]) == HCROWID) {
			TC_throwExceptionNamed(context, "litebase.SQLParseException",
								   getMessage(ERR_ROWID_CANNOT_BE_CHANGED), 0);
			return null;
		}
...
bool setNumericParamValueUpd(Context context, SQLUpdateStatement* updateStmt, int32 index,
	...
	if (checkUpdateIndex(context, updateStmt, index)) { // Checks if the index is within the range.
		if (index < updateStmt->paramCount) { // The parameter is in the update clause.
			...
			return setNumericParamValue(context,
										updateStmt->whereClause->paramList[index - updateStmt->paramCount], value, type);
		}
	}
	return false;
}
...
bool setStrBlobParamValueUpd(Context context, SQLUpdateStatement* updateStmt, int32 index,
	...
	if (checkUpdateIndex(context, updateStmt, index)) { // Checks if the index is within the range.
		if (index < updateStmt->paramCount) { // The parameter is in the update clause.
			...
			if ((!isStr
				 && updateStmt->rsTable->table->columnTypes[i = updateStmt->paramIndexes[index]] != BLOB_TYPE)
				|| (isStr && updateStmt->rsTable->table->columnTypes[i = updateStmt->paramIndexes[index]] ==
					BLOB_TYPE)) {
				TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_INCOMPATIBLE_TYPES), 0);
				return false;
			}
			...
			if (!value) {
				...
				return setParamValueString(context,
										   updateStmt->whereClause->paramList[index - updateStmt->paramCount], value, length);
			}
		}
	}
	return false;
...
bool checkUpdateIndex(Context context, SQLUpdateStatement* updateStmt, int32 index) {
	...
	if (index < 0
		|| index >= updateStmt->paramCount + (updateStmt->whereClause ?
				updateStmt->whereClause->paramCount : 0)) {
		TC_throwExceptionNamed(context, "java.lang.IllegalStateException",
							   getMessage(ERR_INVALID_PARAMETER_INDEX), index);
		return false;
	}
	return true;
}
...
bool litebaseBindUpdateStatement(Context context, SQLUpdateStatement* updateStmt) {
	...
	if (valuesCount > MAXIMUMS) {
		TC_throwExceptionNamed(context, "litebase.SQLParseException",
							   getMessage(ERR_MAX_NUM_FIELDS_REACHED), 0);
		return false;
	}
	...
	if (!convertStringsToValues(context, table, record, updateStmt->nValues)
		|| (updateStmt->whereClause
			&& !bindColumnsSQLBooleanClause(context, updateStmt->whereClause, &table->htName2index,
											table->columnTypes, &updateStmt->rsTable, 1, updateStmt->heap))) {
		return false;
	}
