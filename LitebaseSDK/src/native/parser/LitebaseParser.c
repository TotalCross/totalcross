...
int32 lbError(int32 error, LitebaseParser* parser) {
	...
	TC_throwExceptionNamed(parser->context, "litebase.SQLParseException", errorMessage,
						   parser->yyposition);
	return PARSER_ERROR;
}
...
int32 lbErrorWithMessage(CharP error, CharP message, LitebaseParser* parser) {
	...
	xstrcat(errorMessage, getMessage(ERR_MESSAGE_POSITION));
	TC_throwExceptionNamed(parser->context, "litebase.SQLParseException", errorMessage, message,
						   parser->yyposition);
	return PARSER_ERROR;
}
 ...
 */
LitebaseParser* initLitebaseParser(Context context, JCharP sqlStr, int32 sqlLen, bool isSelect,
								   Heap heap) {
...
SQLBooleanClauseTree* setOperandType(int32 operandType, LitebaseParser* parser) {
	...
	SQLBooleanClauseTree* tree = initSQLBooleanClauseTree(getInstanceBooleanClause(parser),
								 parser->heap);
 ...
 */
SQLFieldDefinition* initSQLFieldDefinition(CharP fieldName, int32 fieldType, int32 fieldSize,
		bool isPrimaryKey, JCharP defaultValue, bool isNotNull,
		Heap heap) {
	TRACE("initSQLFieldDefinition")
	SQLFieldDefinition* sqlFieldDefinition = (SQLFieldDefinition*)TC_heapAlloc(heap,
			sizeof(SQLFieldDefinition));
...
SQLResultSetField* initSQLResultSetField(Heap heap) {
	...
	SQLResultSetField* sqlResultSetField = (SQLResultSetField*)TC_heapAlloc(heap,
										   sizeof(SQLResultSetField));
...
SQLResultSetTable* initSQLResultSetTable(CharP tableName, CharP aliasTableName, Heap heap) {
	...
	SQLResultSetTable* sqlResultSetTable = (SQLResultSetTable*)TC_heapAlloc(heap,
										   sizeof(SQLResultSetTable));
	...
	sqlResultSetTable->aliasTableNameHashCode = TC_hashCode((sqlResultSetTable->aliasTableName =
...
bool yyparse(LitebaseParser* parser) {
	...
	switch (yylex(parser)) {
		case TK_ALTER: // Alter table.
			...
			parser->tableList[0] = initSQLResultSetTable(parser->yylval, null,
			...
			switch (yylex(parser)) {
				case TK_ADD: // Adds a primary key or a new column.
					...
					if ((token = createColumn(parser)) == TK_PRIMARY) { // Adds a primary key.
						...
						if (field->isNotNull
							&& !field->defaultValue) { // A field declared as not null must have a default value.
						...
						if (field->isPrimaryKey) { // The new field can't be declared as a primary key when being added.
							if (field->defaultValue) { // All the keys would be the same.
								return lbErrorWithMessage(getMessage(ERR_STATEMENT_CREATE_DUPLICATED_PK),
														  parser->tableList[0]->tableName, parser);
							}
		...
		case TK_CREATE: {
			switch (yylex(parser)) {
				case TK_TABLE: // Create table.
					...
					// Primary key.
					if ((token = createColumnCommalist(parser)) == TK_PRIMARY && yylex(parser) == TK_KEY
						&& yylex(parser) == TK_OPEN && colnameCommaList(parser) == TK_CLOSE) {
		...
		case TK_INSERT: // Insert.
			...
			if (parser->fieldNamesSize && parser->fieldNamesSize != parser->fieldValuesSize) {
				char error[MAXIMUMS];
				xstrprintf(error, "%s (%d != %d) ", getMessage(ERR_NUMBER_FIELDS_AND_VALUES_DOES_NOT_MATCH),
		...
		case TK_UPDATE: { // Update.
			...
			token = optWhereClause(updateExpCommalist(parser),
			...
			if (parser->secondFieldUpdateTableName) // Verifies if there was an error on field.tableName.
				return lbErrorWithMessage(getMessage(ERR_INVALID_COLUMN_NAME), xstrcmp(tableName,
										  parser->firstFieldUpdateTableName) ?
				...
				return lbErrorWithMessage(getMessage(ERR_INVALID_COLUMN_NAME), parser->firstFieldUpdateAlias,
										  parser);
			}
...
int32 createColumn(LitebaseParser* parser) {
	...
	if ((token = yylex(parser)) ==
		TK_PRIMARY) { // The next token after ',' is a primary key declaration. This is not treated here.
		return token;
	}
	...
	if (type == CHARS_TYPE || type == BLOB_TYPE) { // Size and multiplier.
		if (yylex(parser) == TK_OPEN && yylex(parser) == TK_NUMBER) {
			bool error;
			IntBuf buffer;
			if ((size = TC_str2int(TC_JCharP2CharPBuf((JCharP)parser->yylval, -1, buffer), &error)) <= 0
				|| error) {
	...
	if (token == TK_DEFAULT) { // Default value.
		...
		if (((type == CHARS_TYPE || type == CHARS_NOCASE_TYPE || type == DATE_TYPE
			  || type == DATETIME_TYPE) && token == TK_NUMBER)
	...
	parser->fieldList[parser->fieldListSize++] = initSQLFieldDefinition(columnName, type, size,
			isPrimaryKey, strDefault, isNotNull, parser->heap);
	return token;
}
...
int32 factor(int32 token, LitebaseParser* parser) {
	...
	if ((token = singleExp(token, parser)) == TK_EQUAL || token == TK_LESS || token == TK_DIFF
		|| token == TK_GREATER || token == TK_GREATER_EQUAL
		|| token == TK_LESS_EQUAL) {
		switch (token) {
			case TK_LESS:
	...
	if (token == TK_IS) { // factor = single expression is [not] null.
		...
		(tree->rightTree = setOperandType(OP_PAT_NULL,
										  parser))->parent = (tree->leftTree = parser->auxTree)->parent = tree;
		parser->auxTree = tree;
...
int32 singleExp(int32 token, LitebaseParser* parser) {
	...
	if (token == TK_NUMBER) { // single expression = number
		// juliana@226a_20
		(tree = parser->auxTree = initSQLBooleanClauseTree(getInstanceBooleanClause(parser),
								  parser->heap))->operandValue.asChars = parser->yylval;
		return yylex(parser);
	} else if (token == TK_STR) { // single expression = string
		setOperandStringLiteral((tree = parser->auxTree = initSQLBooleanClauseTree(getInstanceBooleanClause(
											parser), parser->heap)), (JCharP)parser->yylval);
		return yylex(parser);
		...
		if (parser->isWhereClause) {
			(parser->whereParamList[booleanClause->paramCount++] = parser->auxTree = initSQLBooleanClauseTree(
						booleanClause, parser->heap))->isParameter = true;
		} else {
			(parser->havingParamList[booleanClause->paramCount++] = parser->auxTree = initSQLBooleanClauseTree(
						booleanClause, parser->heap))->isParameter = true;
		}
		return yylex(parser);
		...
		SQLResultSetField* paramField = field->parameter = initSQLResultSetField(
											parser->heap); // Creates the parameter field.

		(parser->auxTree = tree = initSQLBooleanClauseTree(booleanClause,
								  parser->heap))->operandType = OP_IDENTIFIER;
		hashCode = tree->nameSqlFunctionHashCode = tree->nameHashCode = TC_hashCode((
					   tree->operandName = field->tableColName));
		...
		paramField->alias = paramField->tableColName = field->alias = field->tableColName =
								tree->operandName;
		paramField->aliasHashCode = paramField->tableColHashCode = field->tableColHashCode =
										field->aliasHashCode = tree->nameHashCode;
...
int32 fieldExp(int32 token, LitebaseParser* parser) {
	...
	if (token == TK_ASTERISK) { // All fields.
		...
		do {
			...
			parser->auxField->aliasHashCode = TC_hashCode(parser->auxField->alias =
												  alias); // Assigns the alias.
		} while (token == TK_COMMA);
	}
	return token;
}
...
int32 field(int32 token, LitebaseParser* parser) {
	...
	if (token == TK_IDENT) { // A pure field.
		...
		if ((tokenAux = dataFunction(token, parser)) >= 0) { // A function applied to a field.
			SQLResultSetField* paramField = (field = parser->auxField)->parameter = initSQLResultSetField(
												parser->heap);
			...
			field->tableColHashCode = paramField->aliasHashCode = paramField->tableColHashCode;
										   ...
										   parser)) >= 0) { // An aggregation function applied to a field.
			// Sets the field.
			field = parser->auxField;
...
int32 pureField(int32 token, LitebaseParser* parser) {
	...
	if ((token = yylex(parser)) == TK_DOT) { // table.fieldName
		...
		alias = field->alias = (CharP)TC_heapAlloc(parser->heap,
							   xstrlen(field->tableName) + xstrlen(field->tableColName = (CharP)parser->yylval) + 2);
