 ...
 * @throws SQLParseException To throw an exception if the error message that
 * occurred.
 */
int32
lbError(int32 error, LitebaseParser* parser)
{
  TRACE("lbError")
  ...
  TC_throwExceptionNamed(parser->context,
 ...
 */
int32
lbErrorWithMessage(CharP error, CharP message, LitebaseParser* parser)
{
 ...
 * @return A pointer to a <code>LitebaseParser</code> structure if the string
 * does not have parser errors or <code>null</code>, otherwise.
 */
LitebaseParser*
initLitebaseParser(Context context,
                   JCharP sqlStr,
                   int32 sqlLen,
                   bool isSelect,
                   Heap heap)
{
  ...
  } else if (!(parser = context->litebasePtr =
                 (LitebaseParser*)xmalloc(sizeof(LitebaseParser)))) {
 ...
 * @param parser A pointer to the parser structure.
 * @return A pointer to a <code>SQLBooleanTree</code> structure for a expression
 * tree.
 */
SQLBooleanClauseTree*
setOperandType(int32 operandType, LitebaseParser* parser)
{
  TRACE("setOperandType")
  SQLBooleanClauseTree* tree =
    initSQLBooleanClauseTree(getInstanceBooleanClause(parser), parser->heap);
 ...
 *
 * @param isAscending Indicates if the order by or group by sorting is in
 * ascending or descending order.
 * @param isOrderBy Indicates if the field list where to add a list is a order
 * or group by.
 * @param parser A pointer to the parser structure, which contains the list(s).
 * @return <code>true</code> if the number of fields has not reached its limit;
 * <code>false</code>, otherwise.
 */
bool
addColumnFieldOrderGroupBy(bool isAscending,
                           bool isOrderBy,
                           LitebaseParser* parser)
{
  ...
  SQLColumnListClause* listClause =
    isOrderBy ? &parser->orderBy : &parser->groupBy;
  SQLResultSetField** fieldList =
    isOrderBy ? parser->orderByfieldList : parser->groupByfieldList;
...
/**
 * Gets a especific boolean clause: it will get a where clause if there is a
 * where clause; otherwise, it will get a having clause.
 *
 * @param parser A pointer to the parser structure.
 * @return A pointer to a <code>SQLBooleanClause</code>.
 */
SQLBooleanClause*
getInstanceBooleanClause(LitebaseParser* parser)
{
 ...
 * @param fieldSize The size of the new field (it is zero if the type is not
 * CHARS or CHARS_NOCASE).
 * @param isPrimaryKey Indicates if the new field is the primary key.
 * @param defaultValue The default value of the new field (it can be null if
 * there is no default value).
 * @param isNotNull Indicates if the new field can be null or not.
 * @return a pointer to the new result set table structure.
 */
SQLFieldDefinition*
initSQLFieldDefinition(CharP fieldName,
                       int32 fieldType,
                       int32 fieldSize,
                       bool isPrimaryKey,
                       JCharP defaultValue,
                       ...
                       Heap heap)
{
  ...
  SQLFieldDefinition* sqlFieldDefinition =
    (SQLFieldDefinition*)TC_heapAlloc(heap, sizeof(SQLFieldDefinition));
 ...
 */
SQLResultSetField*
initSQLResultSetField(Heap heap)
{
  TRACE("initSQLResultSetField")
  SQLResultSetField* sqlResultSetField =
    (SQLResultSetField*)TC_heapAlloc(heap, sizeof(SQLResultSetField));
...
/**
 * Initializes a result set table structure using a table name and its optional
 * alias name.
 *
 * @param tableName the name of the new table.
 * @param aliasTableName the optional alias name of the new table (it can be
 * null).
 * @return a pointer to the new result set table structure.
 */
SQLResultSetTable*
initSQLResultSetTable(CharP tableName, CharP aliasTableName, Heap heap)
{
  TRACE("initSQLResultSetTable")

  // Allocates the table structure.
  SQLResultSetTable* sqlResultSetTable =
    (SQLResultSetTable*)TC_heapAlloc(heap, sizeof(SQLResultSetTable));
  ...
  sqlResultSetTable->aliasTableNameHashCode =
    TC_hashCode((sqlResultSetTable->aliasTableName =
                   (!aliasTableName) ? tableName : aliasTableName));
 ...
 * @return <code>true</code> if there are parser errors; <code>false</code>,
 * otherwise.
 */
bool
yyparse(LitebaseParser* parser)
{
  TRACE("yyparse")
  ...
  switch (yylex(parser)) {
    case TK_ALTER: // Alter table.
      ...
      parser->tableList[0] = initSQLResultSetTable(
        parser->yylval,
        null,
        parser->heap); // There's no alias table name here.

      switch (yylex(parser)) {
        case TK_ADD: // Adds a primary key or a new column.

          // juliana@253_22: added command ALTER TABLE ADD column.
          if ((token = createColumn(parser)) ==
              TK_PRIMARY) { // Adds a primary key.
            if (parser->fieldListSize == 1) {
              return lbError(ERR_SYNTAX_ERROR, parser);
            }
            if (yylex(parser) != TK_KEY || yylex(parser) != TK_OPEN ||
                colnameCommaList(parser) != TK_CLOSE) {
              return lbError(ERR_SYNTAX_ERROR, parser);
            }
            ...
            if (field->isNotNull &&
                !field->defaultValue) { // A field declared as not null must
                                        // have a default value.
              return lbError(ERR_NOT_NULL_DEFAULT, parser);
            }
            if (field->isPrimaryKey) {   // The new field can't be declared as a
                                         // primary key when being added.
              if (field->defaultValue) { // All the keys would be the same.
                return lbErrorWithMessage(
                  getMessage(ERR_STATEMENT_CREATE_DUPLICATED_PK),
                  parser->tableList[0]->tableName,
                  parser);
              }
              return lbError(ERR_PK_CANT_BE_NULL,
                             parser); // All the keys would be null.
            }
    ...
    case TK_CREATE: {
      switch (yylex(parser)) {
        case TK_TABLE: // Create table.
          ...
          // Primary key.
          if ((token = createColumnCommalist(parser)) == TK_PRIMARY &&
              yylex(parser) == TK_KEY && yylex(parser) == TK_OPEN &&
              colnameCommaList(parser) == TK_CLOSE) {
        ...
        case TK_INDEX: // Create index.
          if (yylex(parser) != TK_IDENT || yylex(parser) != TK_ON ||
              yylex(parser) != TK_IDENT) {
            return lbError(ERR_SYNTAX_ERROR, parser);
          }
          parser->tableList[0] = initSQLResultSetTable(
            parser->yylval,
            null,
            parser->heap); // There's no alias table name here.
          if (yylex(parser) != TK_OPEN ||
              colnameCommaList(parser) != TK_CLOSE) { // Column name list.
            return lbError(ERR_SYNTAX_ERROR, parser);
          }
    ...
    case TK_DELETE: // Delete.
      if (!(((token = yylex(parser)) == TK_FROM && yylex(parser) == TK_IDENT) ||
            token == TK_IDENT)) {
        return lbError(ERR_SYNTAX_ERROR, parser);
      }
      ...
      if ((token = yylex(parser)) == TK_IDENT) { // Alias table name.
        parser->tableList[0] =
          initSQLResultSetTable(tableName, parser->yylval, parser->heap);
        token = yylex(parser);
      } else {
        parser->tableList[0] =
          initSQLResultSetTable(tableName, null, parser->heap);
      }
    ...
    case TK_DROP:
      switch (yylex(parser)) {
        ...
        case TK_INDEX: // Drop index.
          if ((token = colnameCommaList(parser)) != TK_ON ||
              yylex(parser) != TK_IDENT) {
            return lbError(ERR_SYNTAX_ERROR, parser);
          }
          parser->tableList[0] = initSQLResultSetTable(
    ...
    case TK_INSERT: // Insert.
      ...
      if (token != TK_VALUES || yylex(parser) != TK_OPEN ||
          listValues(parser) != TK_CLOSE) { // Reads the value list.
        return lbError(ERR_SYNTAX_ERROR, parser);
      }

      // If the default order is not used, the number of values must be equal to
      // the number of fields.
      if (parser->fieldNamesSize &&
          parser->fieldNamesSize != parser->fieldValuesSize) {
        char error[MAXIMUMS];
        xstrprintf(error,
                   "%s (%d != %d) ",
                   getMessage(ERR_NUMBER_FIELDS_AND_VALUES_DOES_NOT_MATCH),
                   parserTP->fieldNamesSize,
    ...
    case TK_SELECT: // Select.
      ...
      token = optWhereClause(tableList(parser),
                             parser); // Table list and where clause.
      ...
      // Checks if the first field is the wild card. If so, assigns null to
      // list, to indicate that all fields must be included.
    ...
    case TK_UPDATE: { // Update.
      ...
      token =
        optWhereClause(updateExpCommalist(parser),
                       parser); // Update expression list and where clause.

      if (parser->secondFieldUpdateTableName) // Verifies if there was an error
                                              // on field.tableName.
        return lbErrorWithMessage(
          getMessage(ERR_INVALID_COLUMN_NAME),
          xstrcmp(tableName, parser->firstFieldUpdateTableName)
            ? parser->firstFieldUpdateAlias
            : parser->secondFieldUpdateAlias,
          parser);
      else if (parser->firstFieldUpdateTableName &&
               xstrcmp(tableAlias, parser->firstFieldUpdateTableName)) {
        return lbErrorWithMessage(getMessage(ERR_INVALID_COLUMN_NAME),
                                  parser->firstFieldUpdateAlias,
                                  parser);
      }
      ...
      parser->tableList[0] =
        initSQLResultSetTable(tableName, tableAlias, parser->heap);
      break;
    }
 ...
 */
int32
colnameCommaList(LitebaseParser* parser)
{
  int32 token, fieldNamesSize = parser->fieldNamesSize;
  CharP* fieldNames = parser->fieldNames;

  do {
    if ((token = yylex(parser)) ==
        TK_ASTERISK) { // This is necessary for dropping all indices.
 ...
 */
int32
createColumnCommalist(LitebaseParser* parser)
{
  int32 token;
 ...
 */
int32
createColumn(LitebaseParser* parser)
{
  int32 token, type, size = 0;
  bool isPrimaryKey = false, isNotNull = false;
  CharP columnName;
  JCharP strDefault = null;

  if ((token = yylex(parser)) ==
      TK_PRIMARY) { // The next token after ',' is a primary key declaration.
  ...
  if (type == CHARS_TYPE || type == BLOB_TYPE) { // Size and multiplier.
    if (yylex(parser) == TK_OPEN && yylex(parser) == TK_NUMBER) {
      ...
      IntBuf buffer;
      if ((size =
             TC_str2int(TC_JCharP2CharPBuf((JCharP)parser->yylval, -1, buffer),
                        &error)) <= 0 ||
          error) {
    ...
    // juliana@253_15: now an exception is thrown if the size of a CHAR or
    // VARCHAR is greater than 65535.
    else if (type == CHARS_TYPE && size > (MAX_SHORT_VALUE << 1) + 1) {
      return lbErrorWithMessage(
        getMessage(ERR_INVALID_NUMBER), "unsigned short", parser);
    }
  }
  ...
  if (token == TK_DEFAULT) { // Default value.
    ...
    // A numeric type must have a number as a default value. A string, date or
    // datetime type must have a string as a default value.
    if (((type == CHARS_TYPE || type == CHARS_NOCASE_TYPE ||
          type == DATE_TYPE || type == DATETIME_TYPE) &&
         token == TK_NUMBER) ||
  ...
  parser->fieldList[parser->fieldListSize++] = initSQLFieldDefinition(
    columnName, type, size, isPrimaryKey, strDefault, isNotNull, parser->heap);
  return token;
}
 ...
 */
int32
expression(int32 token, LitebaseParser* parser)
{
  if ((token = term(token, parser)) ==
      TK_OR) { // expression = term or expression | term
    // juliana@213_1: changed the way a tree with ORs is built in order to speed
    // up queries with indices.
 ...
 */
int32
term(int32 token, LitebaseParser* parser)
{
  if ((token = factor(token, parser)) ==
      TK_AND) { // term = factor or factor | term
 ...
 */
int32
factor(int32 token, LitebaseParser* parser)
{
  ...
  if (token == TK_NOT) {
    ...
    // The parent node will be the negation operator and the expression will be
    // the right tree.
  ...
  // factor = single expression (< | > | = | <> | != | <= | >=) single
  // expression
  if ((token = singleExp(token, parser)) == TK_EQUAL || token == TK_LESS ||
      token == TK_DIFF || token == TK_GREATER || token == TK_GREATER_EQUAL ||
      token == TK_LESS_EQUAL) {
    switch (token) {
      case TK_LESS:
  ...
  if (token == TK_IS) { // factor = single expression is [not] null.
    ...
    (tree->rightTree = setOperandType(OP_PAT_NULL, parser))->parent =
      (tree->leftTree = parser->auxTree)->parent = tree;
    parser->auxTree = tree;
  ...
  if (token == TK_LIKE) {
    if ((token = yylex(parser)) == TK_STR) { // string
      ...
      if (booleanClause->paramCount ==
          MAXIMUMS) { // There is a maximum number of parameters.
    ...
    (tree->rightTree = rightTree)->parent =
      (tree->leftTree = parser->auxTree)->parent = tree;
    parser->auxTree = tree;
 ...
 */
int32
singleExp(int32 token, LitebaseParser* parser)
{
  int32 auxToken;
  SQLBooleanClauseTree* tree;
  ...
  if (token == TK_NUMBER) { // single expression = number
    ...
    (tree = parser->auxTree =
       initSQLBooleanClauseTree(getInstanceBooleanClause(parser), parser->heap))
      ->operandValue.asChars = parser->yylval;
    return yylex(parser);
  } else if (token == TK_STR) { // single expression = string
    setOperandStringLiteral((tree = parser->auxTree = initSQLBooleanClauseTree(
                               getInstanceBooleanClause(parser), parser->heap)),
                            (JCharP)parser->yylval);
    return yylex(parser);
    ...
    if (parser->isWhereClause) {
      (parser->whereParamList[booleanClause->paramCount++] = parser->auxTree =
         initSQLBooleanClauseTree(booleanClause, parser->heap))
        ->isParameter = true;
    } else {
      (parser->havingParamList[booleanClause->paramCount++] = parser->auxTree =
         initSQLBooleanClauseTree(booleanClause, parser->heap))
        ->isParameter = true;
    }
    return yylex(parser);
  } else if ((auxToken = dataFunction(token, parser)) !=
             -1) { // single expression = function(...)
    SQLBooleanClause* booleanClause = getInstanceBooleanClause(parser);
    int32 i = 1, index = booleanClause->fieldsCount, hashCode;
    SQLResultSetField* field = parser->auxField;
    Hashtable* fieldName2Index = &booleanClause->fieldName2Index;
    SQLResultSetField* paramField = field->parameter =
      initSQLResultSetField(parser->heap); // Creates the parameter field.

    (parser->auxTree = tree =
       initSQLBooleanClauseTree(booleanClause, parser->heap))
      ->operandType = OP_IDENTIFIER;
    hashCode = tree->nameSqlFunctionHashCode = tree->nameHashCode =
    ...
    paramField->alias = paramField->tableColName = field->alias =
      field->tableColName = tree->operandName;
    paramField->aliasHashCode = paramField->tableColHashCode =
      field->tableColHashCode = field->aliasHashCode = tree->nameHashCode;
    ...
    (parser->auxTree = tree =
       initSQLBooleanClauseTree(booleanClause, parser->heap))
      ->operandType = OP_IDENTIFIER;
    hashCode = field->tableColHashCode = tree->nameSqlFunctionHashCode =
      tree->nameHashCode =
        TC_hashCode((tree->operandName = field->tableColName));
    ...
    field->aliasHashCode =
      TC_hashCode(field->alias); // Sets the hash code of the field alias.
 ...
 */
int32
listValues(LitebaseParser* parser)
{
  int32 token, size = 0;
 ...
 */
int32
tableList(LitebaseParser* parser)
{
  int32 token, size = 0, hash;
  CharP tableName, tableAlias;
  ...
  do {
    if ((token = yylex(parser)) != TK_IDENT) { // Not a table name, return.
      if (!(parser->select.tableListSize =
              size)) { // There must be at least a table.
        return lbError(ERR_SYNTAX_ERROR, parser);
      }
      return token;
    }
    ...
    if (TC_htGet32Inv(&parserTP->tables, (hash = TC_hashCode(tableAlias))) !=
        -1) {
      return lbErrorWithMessage(
        getMessage(ERR_NOT_UNIQUE_ALIAS_TABLE), tableAlias, parser);
    } else {
 ...
 */
int32
fieldExp(int32 token, LitebaseParser* parser)
{
  ...
  if (token == TK_ASTERISK) { // All fields.
    ...
    (resultFieldList[select->fieldsCount++] =
       initSQLResultSetField(parser->heap))
      ->isWildcard = true;
    token = yylex(parser);
    ...
    do {
      ...
      if ((token = field(token, parser)) == TK_AS) { // There is an alias.
        ...
        // If the alias_name is null, the alias must be the name of the column.
        // This was already done before. If the alias is null and the field is a
        // virtual column, raises an exception, since virtual columns require
        // explicit aliases.
        if (parser->auxField->isVirtual) {
          return lbError(ERR_REQUIRED_ALIAS, parser);
        }

        alias = parser->auxField
                  ->alias; // The null alias name is filled as tableColName or
                           // tableName.tableColName, which was set before.
      } else {
        return PARSER_ERROR;
      }
      ...
      while (--i >= 0)
        if (strEq(resultFieldList[i]->alias, alias)) {
          return lbErrorWithMessage(
            getMessage(ERR_DUPLICATE_ALIAS), alias, parser);
        }

      parser->auxField->aliasHashCode =
        TC_hashCode(parser->auxField->alias = alias); // Assigns the alias.
    } while (token == TK_COMMA);
  }
  return token;
}
 ...
 */
int32
updateExpCommalist(LitebaseParser* parser)
{
  int32 token, size = 0;
  JCharP* values = parser->fieldValues;
  ...
  do {
    ...
    // New value.
    if ((token = yylex(parser)) == TK_STR ||
        token == TK_NUMBER) { // A string or a number.
      values[size++] = (JCharP)parser->yylval;
    ...
    if (!parser->firstFieldUpdateTableName) { // After the table name
                                              // verification, the associated
                                              // table name on the field name is
                                              // discarded.
      if (field->tableName) {
    ...
    // There is an error: update has just one table. This error will raise an
    // exception later on.
    {
 ...
 */
int32
field(int32 token, LitebaseParser* parser)
{
  int32 tokenAux;
  SQLSelectClause* select = &parser->select;
  ...
  if (token == TK_IDENT) { // A pure field.
    ...
    if (select->fieldsCount ==
        MAXIMUMS) { // The  maximum number of fields can't be reached.
    ...
    if ((tokenAux = dataFunction(token, parser)) >=
        0) { // A function applied to a field.
      SQLResultSetField* paramField = (field = parser->auxField)->parameter =
        initSQLResultSetField(parser->heap);
      ...
      field->tableColHashCode = paramField->aliasHashCode =
        paramField->tableColHashCode;
    } else if ((tokenAux = aggFunction(token,
                                       parser)) >=
               0) { // An aggregation function applied to a field.
      // Sets the field.
      field = parser->auxField;
    ...
    parser->selectFieldList[select->fieldsCount++] =
      field; // Sets the select statement.
  }
 ...
 */
int32
pureField(int32 token, LitebaseParser* parser)
{
  SQLResultSetField* field = parser->auxField =
    initSQLResultSetField(parser->heap);
  ...
  if ((token = yylex(parser)) == TK_DOT) { // table.fieldName
    ...
    alias = field->alias = (CharP)TC_heapAlloc(
      parser->heap,
      xstrlen(field->tableName) +
        xstrlen(field->tableColName = (CharP)parser->yylval) + 2);
 ...
 */
int32
dataFunction(int32 token, LitebaseParser* parser)
{
  int32 function;
  ...
  if (yylex(parser) != TK_OPEN ||
      pureField(yylex(parser), parser) != TK_CLOSE) {
    return lbError(ERR_SYNTAX_ERROR, parser);
  }
 ...
 */
int32
aggFunction(int32 token, LitebaseParser* parser)
{
  int function;
  ...
  if (token == TK_COUNT) {
    if (yylex(parser) != TK_OPEN || yylex(parser) != TK_ASTERISK ||
        yylex(parser) != TK_CLOSE) {
      return lbError(ERR_SYNTAX_ERROR, parser);
    }
    parser->auxField = initSQLResultSetField(parser->heap);
  } else if (yylex(parser) != TK_OPEN ||
             pureField(yylex(parser), parser) != TK_CLOSE) {
    return lbError(ERR_SYNTAX_ERROR, parser);
  }
  parser->auxField->sqlFunction = function;
 ...
 * @param parser The parser structure.
 * @return The token received if it is not a where clause or the token after the
 * where clause.
 */
int32
optWhereClause(int32 token, LitebaseParser* parser)
{
  if (token == TK_WHERE) { // Where clause.
    SQLBooleanClause* whereClause = getInstanceBooleanClause(parser);
    token = expression(yylex(parser), parser);
    whereClause->expressionTree = whereClause->origExpressionTree =
      parser->auxTree;
    whereClause->isWhereClause =
      true; // It indicates that it is a where clause.
  }
  return token;
}
 ...
 */
int32
orderByClause(LitebaseParser* parser)
{
  int32 token;
  bool direction;
  SQLSelectClause* select = &parser->select;
 ...
 */
int32
groupByClause(LitebaseParser* parser)
{
  int32 token;
  SQLSelectClause* select = &parser->select;
  ...
  if (token == TK_HAVING) { // Adds the expression tree of the where clause.
    parser->isWhereClause =
      false; // Indicates if the clause is a where or a having clause.
    token = expression(yylex(parser), parser);
...
/**
 * Tests if the function <code>lbErrorWithMessage()</code> in fact creates an
 * exception.
 *
 * @param testSuite The test structure.
 * @param currentContext The thread context where the test is being executed.
...
TESTCASE(initLitebaseParser)
{
  ...
  // Null string.
  ASSERT1_EQUALS(Null,
                 initLitebaseParser(currentContext, null, 0, false, heap));
  ASSERT1_EQUALS(NotNull, currentContext->thrownException);
  currentContext->thrownException = null;
  ...
  buffer[0] = 0;
  ASSERT1_EQUALS(Null,
                 initLitebaseParser(currentContext, buffer, 0, false, heap));
  ASSERT1_EQUALS(NotNull, currentContext->thrownException);
  currentContext->thrownException = null;

  TC_CharP2JCharPBuf(" ", 1, buffer, true);
  ASSERT1_EQUALS(Null,
                 initLitebaseParser(currentContext, buffer, 1, false, heap));
  ASSERT1_EQUALS(NotNull, currentContext->thrownException);
  currentContext->thrownException = null;

  TC_CharP2JCharPBuf("  ", 2, buffer, true);
  ASSERT1_EQUALS(Null,
                 initLitebaseParser(currentContext, buffer, 2, false, heap));
  ASSERT1_EQUALS(NotNull, currentContext->thrownException);
  currentContext->thrownException = null;

  string = "create table bookentry(name char(30), address char(50), phone "
           "char(20), birthday int, salary float, married short, gender short, "
           "lastUpdated long)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ...
  string = "select rowid, name, address, phone, birthday, salary, married, "
           "gender, lastUpdated from bookentry";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, true, heap));
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  ...
  string =
    "update bookentry set name = ?, address = ?, phone = ?, birthday = ?, "
    "salary = ?, married = ?, gender = ?, lastUpdated = ? where rowid = ?";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  ...
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    Null, initLitebaseParser(currentContext, buffer, length, false, heap));
  ASSERT1_EQUALS(NotNull, currentContext->thrownException);
  currentContext->thrownException = null;
  ...
  string =
    "create table blob0 (value blob(100), age int, primary key(value, age))";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  ...
  string =
    "create table blob0 (value blob(100), age int, primary key(age, value))";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  ...
  string = "INSERT INTO ACT_CLIENTE VALUES (38L, '38', 'ADENILZA "
           "xxxxxxxxxxxxxxxxxxxxxE', 'ADENILZA 111111111111111111111E', "
           "'11.111.222//4443-22', '', 1, 1, '', 4, '3423423421', '', '', '1', "
           "20051110101123, 20051110101123, 1)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  ...
  string = "INSERT INTO ACT_CLIENTE VALUES (114L, '114', 'NIETO "
           "yyyyyyyyyyyyyyyyyyyyE', 'NIETO 2222222222222222222EE', "
           "'22.222.333//3333-33', '', 1, 1, '', 4, '4342342423', '', '', '1', "
           "20051110101123, 20051110101123, 1)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ...
  string = "INSERT INTO ACT_CLIENTE VALUES (161L, '161', 'ANTONIO "
           "bbbbbbbbbbbbbbbbbbbbbbE', 'ANTONIO 33333333333333333333333', "
           "'44.444.444//4441-44', '', 1, 1, '', 4, '5435656458', '', '', '1', "
           "20051110101124, 20051110101124, 1)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  ...
  string = "INSERT INTO ACT_CLIENTE VALUES (421L, '421', 'CLAUDIOMIR "
           "cccccccccccccccccMEE', 'CLAUDIOMIR 444444444444444444EE', "
           "'55.555.555//5555-26', '', 1, 1, '', 4, '', '', '', '1', "
           "20051110101124, 20051110101124, 1)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ...
  string = "INSERT INTO ACT_CLIENTE VALUES (443L, '443', 'MARIA "
           "dddddddddddddddddSO', 'MARIA 55555555555555555SO', "
           "'777.777.777-20', '', 1, 1, '', 4, '6756756756', '', '', '2', "
           "20051110101124, 20051110101124, 1)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ...
  string =
    "INSERT INTO ACT_CLIENTE VALUES (941L, '941', "
    "'J.B.eeeeeeeeeeeeeeeeeeeeeeeeeeE', 'J.B.6666666666666666666666666EE', "
    "'88.888.888//8889-83', '', 1, 1, '', 4, '8655676565', '', '', '1', "
    "20051110101124, 20051110101124, 1)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ...
  string = "INSERT INTO ACT_CLIENTE VALUES (968L, '968', 'JARDELIO "
           "fffffffffffffffffffffffE', 'JARDELIO "
           "7777777777777777777777EE','99.999.999//9999-82', '', 1, 1, '', 4, "
           "'7656456547', '', '', '1', 20051110101124, 20051110101124, 1)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  ...
  string =
    "INSERT INTO ACT_CLIENTE VALUES (1217L, '1217', 'N.gggggggggggggggE', "
    "'N.C.8888888888888E', '00.000.000//1111-16', '', 1, 1, '', 4, "
  ...
  string = "INSERT INTO ACT_CLIENTE VALUES (1450L, '1450', 'OTTO "
           "COMhhhhhhhhhhhhhhhhhhhhhhhE', 'OTTO 9999999999999999999999999EE', "
           "'22.222.222//2222-17', '', 1, 1, '', 4, '4535345340', '', '', '1', "
           "20051110101124, 20051110101124, 1)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ...
  string = "INSERT INTO ACT_CLIENTE VALUES (1585L, '1585', 'OROSINO "
           "iiiiiiiiiiiiiiiiiiiE', 'OROSINO 0000000000000000000E', "
           "'33.333.333//3333-33', '', 1, 1, '', 4, '5435345357', '', '30      "
           "  ', '1', 20051110101124, 20051110101124, 1)";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, false, heap));
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  ...
  string = "create table act_cliente (actclienteid long, actcodcliente "
           "char(30), actrazaosocial char(100), actnomefantasia char(100), "
           "actie char(20), actcnpj char(20), actstatus long, acttipocliente "
           "long, actmail char(255), actvendedorid long, acttelefone char(18), "
           "actobservacao char(255), actfax char(18), actcodtabela char(20), "
  ...
  string = "select actrazaosocial from act_cliente where "
           "actrazaosocial='ADENILZA xxxxxxxxxxxxxxxxxxxxxE'";
  TC_CharP2JCharPBuf(string, length = xstrlen(string), buffer, true);
  ASSERT1_EQUALS(
    NotNull, initLitebaseParser(currentContext, buffer, length, true, heap));
  ASSERT1_EQUALS(Null, currentContext->thrownException);
