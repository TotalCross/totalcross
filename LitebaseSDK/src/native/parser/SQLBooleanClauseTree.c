...
SQLBooleanClauseTree* initSQLBooleanClauseTree(SQLBooleanClause* booleanClause, Heap heap) {
	...
	SQLBooleanClauseTree* tree = (SQLBooleanClauseTree*)TC_heapAlloc(heap,
								 sizeof(SQLBooleanClauseTree));
 ...
 */
bool setNumericParamValue(Context context, SQLBooleanClauseTree* booleanClauseTree, VoidP value,
						  int32 type) {
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 */
bool setParamValueString(Context context, SQLBooleanClauseTree* booleanClauseTree, JCharP value,
						 int32 length) {
...
void weightTheTree(SQLBooleanClauseTree* booleanClauseTree) {
	...
	switch (booleanClauseTree->operandType) { // Checks the type of the operand.
		...
		default: { // The others.
			...
			SQLResultSetField* leftField = fieldList[TC_htGet32(fieldName2Index,
										   leftTree->nameSqlFunctionHashCode)];
			SQLResultSetField* rightField = fieldList[TC_htGet32(fieldName2Index,
											rightTree->nameSqlFunctionHashCode)];
 ...
 */
void getBranchProperties(SQLBooleanClauseTree* booleanClauseTree, uint8* columns, uint8* operators,
	...
	if (leftTree->operandType ==
		OP_IDENTIFIER) { // One of the elements of the branch must be an identifier.
...
bool getOperandValue(Context context, SQLBooleanClauseTree* booleanClauseTree, SQLValue* value) {
	...
	if (booleanClauseTree->operandType == OP_IDENTIFIER) {
		...
		xmemmove(table->columnNulls, table->db.basbuf + table->columnOffsets[table->columnCount],
		...
		if ((field = booleanClause->fieldList[getFieldIndex(booleanClauseTree)])->sqlFunction !=
			FUNCTION_DT_NONE) {
...
int32 compareNumericOperands(Context context, SQLBooleanClauseTree* booleanClauseTree) {
	...
	int32 leftValueType = leftTree->valueType,
		  ...
		  assignType = booleanClauseTree->isFloatingPointType ? DOUBLE_TYPE
					   : leftValueType != LONG_TYPE && rightValueType != LONG_TYPE
					   && leftValueType != DATETIME_TYPE ? INT_TYPE : LONG_TYPE,
	...
	if (leftValueType == -1
		|| rightValueType == -1) { // Prevents problems when doing comparisons like 1 == 2.
		bool error;
		DoubleBuf buffer;
		...
		if (rightValueAsString[last = TC_JCharPLen(rightValueAsString) - 1] == 'l'
		...
		leftTree->operandValue.asDouble = leftValue.asDouble = leftValueAsDouble = TC_str2double(buffer,
										  &error);
		if (error) {
			TC_throwExceptionNamed(context, "litebase.SQLParseException", getMessage(ERR_INVALID_NUMBER),
								   buffer, "double");
		...
		rightTree->operandValue.asDouble = rightValue.asDouble = rightValueAsDouble = TC_str2double(buffer,
										   &error);
		if (error) {
			TC_throwExceptionNamed(context, "litebase.SQLParseException", getMessage(ERR_INVALID_NUMBER),
								   buffer, "double");
 ...
 * @return The evaluation of the comparison expression or -1 if an error occurs.
 */
int32 matchStringOperands(Context context, SQLBooleanClauseTree* booleanClauseTree, bool ignoreCase,
						  Heap heap) {
	...
	if (!leftValue.asChars) {
		leftTree->valueJoin.asChars = leftValue.asChars = (JCharP)TC_heapAlloc(heap,
									  (booleanClauseTree->booleanClause->resultSet->table->columnSizes[leftTree->colIndex] << 1) + 2);
	}
	if (!getOperandValue(context, leftTree, &leftValue)) {
		return -1;
	}
	...
	if (leftTree->valueType == DATE_TYPE) {
		...
		dateTime2JCharP(asDate / 10000, asDate / 100 % 100, asDate % 100,
						asTime / 10000000, asTime / 100000 % 100, asTime / 1000 % 100, asTime % 1000,
	...
	switch (matchType) {
		...
		case PAT_MATCH_ENDS_WITH:
			result = str16StartsWith(leftStringStr, strToMatchStr, leftStringLen, strToMatchLen,
									 leftStringLen - strToMatchLen, ignoreCase);
			break;
		...
		case PAT_MATCH_MIDDLE: // rnovais@568_1
			...
			result = str16StartsWith(leftStringStr, strToMatchStr, leftStringLen, strToMatchLen = pos - 1, 0,
									 ignoreCase) ?
					 str16StartsWith(leftStringStr, &strToMatchStr[pos + 1], leftStringLen, strEndLen,
									 leftStringLen - strEndLen, ignoreCase) : 0;
			break;

		case PAT_MATCH_EQUAL: // rnovais@568_1
			if (ignoreCase) {
				result = TC_JCharPEqualsIgnoreCaseJCharP(leftStringStr, strToMatchStr, leftStringLen,
						 strToMatchLen);
			} else {
 ...
 * @return The evaluation of the comparison expression or -1 if an error occurs.
 */
int32 compareStringOperands(Context context, SQLBooleanClauseTree* booleanClauseTree,
							bool ignoreCase, Heap heap) {
	...
	if (!rightTree->valueJoin.asChars) {
		rightTree->valueJoin.asChars = (JCharP)TC_heapAlloc(heap,
									   (columnSizes[rightTree->colIndex] << 1) + 2);
	}
	...
	if (!booleanClauseTree->bothAreIdentifier) {
		if (!leftTree->valueJoin.asChars && leftTree->operandType == OP_IDENTIFIER) {
			leftTree->valueJoin.asChars = (JCharP)TC_heapAlloc(heap,
										  (columnSizes[leftTree->colIndex] << 1) + 2);
		}
	...
	result = str16CompareTo(leftValue.asChars, rightValue.asChars, leftValue.length, rightValue.length,
							ignoreCase);
...
int32 booleanTreeEvaluate(Context context, SQLBooleanClauseTree* booleanClauseTree, Heap heap) {
	...
	switch (booleanClauseTree->operandType) { // Checks the operand type of the tree.
		...
		case OP_PAT_MATCH_LIKE:
		case OP_PAT_MATCH_NOT_LIKE:
			return matchStringOperands(context, booleanClauseTree,
									   booleanClauseTree->valueType == CHARS_NOCASE_TYPE, heap);
		...
		case OP_BOOLEAN_AND: { // AND connector.
			...
			if (booleanClauseTree->leftTree && booleanClauseTree->rightTree
				&& (result = booleanTreeEvaluate(context, booleanClauseTree->leftTree, heap)) == true) {
...
bool bindColumnsSQLBooleanClauseTree(Context context, SQLBooleanClauseTree* booleanClauseTree) {
	...
	if (booleanClauseTree->operandType ==
		OP_IDENTIFIER) { // If operand type is identifier, binds to a column in the table.
		int32 dtParameter, // rnovais@568_10
			  fieldIndex = getFieldIndex(
							   booleanClauseTree), // Stores the binding information also in the boolean clause field list.
						   colIndex;
		SQLResultSetField* field;
		SQLBooleanClause* booleanClause =
			booleanClauseTree->booleanClause; // Stores the binding information also in the boolean clause field list.
		...
		booleanClauseTree->colIndex = colIndex = (field =
										  booleanClause->fieldList[fieldIndex])->tableColIndex;

		if (field->sqlFunction ==
			FUNCTION_DT_NONE) { // rnovais@568_10: if it does not have a data function, stores the correct value type.
	...
	if (leftTree && rightTree && (leftTree->valueType == DATE_TYPE
...
bool inferOperationValueType(Context context, SQLBooleanClauseTree* booleanClauseTree) {
	...
	if (leftIsParameter || rightIsParameter) {
		...
		if (leftIsChar !=
			rightIsChar) { // Can not mix a character type with a non-character type, except for DATE and DATETIME.
			TC_throwExceptionNamed(context, "litebase.SQLParseException", getMessage(ERR_INCOMPATIBLE_TYPES));
			return false;
		}
	...
	booleanClauseTree->isFloatingPointType = booleanClauseTree->valueType == DOUBLE_TYPE
			|| booleanClauseTree->valueType == FLOAT_TYPE;
	return true;
}
...
bool convertValue(Context context, SQLBooleanClauseTree* booleanClauseTree, int32 type) {
	...
	if (type < SHORT_TYPE
		|| type > DOUBLE_TYPE) { // juliana@201_10: non-numerical types don't need to be converted to numbers.
		return true;
	}
...
int32 getFieldIndex(SQLBooleanClauseTree* booleanClauseTree) {
	...
	return (TC_htGet32Inv(&(booleanClauseTree)->booleanClause->fieldName2Index,
						  (booleanClauseTree)->nameSqlFunctionHashCode ? (booleanClauseTree)->nameSqlFunctionHashCode :
 ...
 */
SQLBooleanClauseTree* cloneTree(SQLBooleanClauseTree* booleanClauseTree,
								SQLBooleanClauseTree* destTree, Heap heap) {
	TRACE("cloneTree")
	SQLBooleanClause* booleanClause = booleanClauseTree->booleanClause;
	...
	if (booleanClauseTree->leftTree) {
		(tree->leftTree = cloneTree(booleanClauseTree->leftTree, destTree ? destTree->leftTree : null,
									heap))->parent = tree;
	}
	if (booleanClauseTree->rightTree) {
		(tree->rightTree = cloneTree(booleanClauseTree->rightTree, destTree ? destTree->rightTree : null,
									 heap))->parent = tree;
	}
	return tree;
}
