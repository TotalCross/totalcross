 ...
 */
SQLSelectStatement*
initSQLSelectStatement(LitebaseParser* parser, bool isPrepared)
{
  ...
  SQLBooleanClause* whereClause = selectStmt->whereClause =
    parser->whereClause; // Sets the where clause.
  SQLBooleanClause* havingClause = selectStmt->havingClause =
    parser->havingClause; // Sets the having clause.
  ...
  selectClause = selectStmt->selectClause =
    (SQLSelectClause*)TC_heapAlloc(heap, sizeof(SQLSelectClause));
  xmemmove(selectClause, &parser->select, sizeof(SQLSelectClause));
  selectClause->fieldList = (SQLResultSetField**)TC_heapAlloc(
    heap,
    count =
      ((selectClause->fieldsCount ? selectClause->fieldsCount : MAXIMUMS) *
       TSIZE));
  xmemmove(selectClause->fieldList, parser->selectFieldList, count);
  selectClause->tableList = (SQLResultSetTable**)TC_heapAlloc(
    heap, count = (parser->select.tableListSize * TSIZE));
  xmemmove(selectClause->tableList, parser->tableList, count);

  if (isPrepared) { // It is only necessary to re-allocate the parser structures
                    // if the statement is from a prepared statement.
    if (parser->groupBy.fieldsCount) { // Sets the group by clause.
      listClause = selectStmt->groupByClause =
        (SQLColumnListClause*)TC_heapAlloc(heap, sizeof(SQLColumnListClause));
      xmemmove(listClause, &parser->groupBy, sizeof(SQLColumnListClause));
      listClause->fieldList = (SQLResultSetField**)TC_heapAlloc(
        heap, count = (parser->groupBy.fieldsCount * TSIZE));
      xmemmove(listClause->fieldList, parser->groupByfieldList, count);
    }

    if (parser->orderBy.fieldsCount) { // Sets the order by clause.
      listClause = selectStmt->orderByClause =
        (SQLColumnListClause*)TC_heapAlloc(heap, sizeof(SQLColumnListClause));
      xmemmove(listClause, &parser->orderBy, sizeof(SQLColumnListClause));
      listClause->fieldList = (SQLResultSetField**)TC_heapAlloc(
        heap, count = (parser->orderBy.fieldsCount * TSIZE));
      xmemmove(listClause->fieldList, parser->orderByfieldList, count);
    }

    if (whereClause) {
      whereClause->fieldList = (SQLResultSetField**)TC_heapAlloc(
        heap, count = (whereClause->fieldsCount * TSIZE));
      xmemmove(whereClause->fieldList, parser->whereFieldList, count);
      whereClause->paramList = (SQLBooleanClauseTree**)TC_heapAlloc(
        heap, count = (whereClause->paramCount * TSIZE));
      xmemmove(whereClause->paramList, parser->whereParamList, count);
    }

    if (havingClause) {
      havingClause->fieldList = (SQLResultSetField**)TC_heapAlloc(
        heap, count = (havingClause->fieldsCount * TSIZE));
      xmemmove(havingClause->fieldList, parser->havingFieldList, count);
      havingClause->paramList = (SQLBooleanClauseTree**)TC_heapAlloc(
        heap, count = (havingClause->paramCount * TSIZE));
 ...
 */
bool
setNumericParamValueSel(Context context,
                        SQLSelectStatement* selectStmt,
                        int32 index,
                        VoidP value,
                        int32 type)
{
  ...
  if (index < 0 || index >= (whereParamCount +
                             (havingClause ? havingClause->paramCount : 0))) {
    TC_throwExceptionNamed(context,
                           "litebase.DriverException",
                           getMessage(ERR_INVALID_PARAMETER_INDEX),
                           index);
    return false;
  } else if (index <
             whereParamCount) { // Sets the parameter value in its proper place.
    return setNumericParamValue(
      context, whereClause->paramList[index], value, type);
  } else {
    return setNumericParamValue(
      context, havingClause->paramList[index - whereParamCount], value, type);
  }
}
 ...
 */
bool
setParamValueStringSel(Context context,
                       SQLSelectStatement* selectStmt,
                       int32 index,
                       JCharP value,
                       int32 length)
{
  ...
  if (index < 0 || index >= (whereParamCount +
    ...
    TC_throwExceptionNamed(
      context, "litebase.SQLParseException", getMessage(ERR_PARAM_NULL));
    return false;
  } else if (index <
             whereParamCount) { // Sets the parameter value in its proper place.
    return setParamValueString(
      context, whereClause->paramList[index], value, length);
  } else {
    return setParamValueString(
      context, havingClause->paramList[index - whereParamCount], value, length);
  }
}
 ...
 */
void
clearParamValuesSel(SQLSelectStatement* selectStmt)
{
  TRACE("clearParamValuesSel")
  int32 i;
  SQLBooleanClause* clause;
  SQLBooleanClauseTree** paramList;
  if ((clause = selectStmt->whereClause)) { // Clears all the parameters of the
                                            // where clause.
    i = clause->paramCount;
  ...
  if ((clause = selectStmt->havingClause)) { // Clears all the parameters of the
                                             // having clause.
    i = clause->paramCount;
    paramList = clause->paramList;
    while (--i >= 0) {
 ...
 */
bool
allParamValuesDefinedSel(SQLSelectStatement* selectStmt)
{
  TRACE("allParamValuesDefinedSel")
  int32 i;
  SQLBooleanClause* clause;
  SQLBooleanClauseTree** paramList;
  if ((clause = selectStmt->whereClause)) { // Checks if all the where clause
  ...
  if ((clause = selectStmt->havingClause)) { // Checks if all the having clause
                                             // parameters are defined.
    i = clause->paramCount;
    paramList = clause->paramList;
    while (--i >= 0)
 ...
 */
TCObject
litebaseDoSelect(Context context,
                 TCObject driver,
                 SQLSelectStatement* selectStmt)
{
  ...
  while (--i >= 0) {
    if (!tableList[i]->table) {
      TC_throwExceptionNamed(context,
                             "litebase.DriverException",
                             getMessage(ERR_CANT_READ),
                             tableList[i]->tableName);
      return null;
    }
  }

  // juliana@212_4: if the select fields are in the table order beginning with
  // rowid, do not build a temporary table. juliana@230_14: removed temporary
  // tables when there is no join, group by, order by, and aggregation.
  // juliana@210_1: select * from table_name does not create a temporary table
  // anymore.
  if (!selectStmt->groupByClause && !selectStmt->havingClause &&
      !selectStmt->orderByClause && !selectStmt->whereClause &&
      selectClause->tableListSize == 1 && !selectClause->hasAggFunctions) {
    isSimpleSelect = true;
    rsBaseTable = (*tableList)->table;
    rsBaseTable->answerCount = -1;
  } else { // Generates the result set and stores it in a temporary table if
           // necessary.
    SQLResultSetField** fieldList = selectClause->fieldList;
    int32 fieldListLen = selectClause->fieldsCount;
    if (!(rsBaseTable = generateResultSetTable(
            context, driver, selectStmt))) { // Temporary table.
      return null;
    }

    if (!*rsBaseTable->name) {
      // Remaps the table column names to use the aliases of the select
      // statement instead of the original column names. Releases the unused
      // memory (rowinc is 100 by default for result sets). This must be used
      // only for temporary tables.
      if (!remapColumnsNames2Aliases(
            context, rsBaseTable, fieldList, fieldListLen) ||
          !plainShrinkToSize(
            context,
            &rsBaseTable->db)) { // guich@201_9: always shrink the .db and
                                 // .dbo memory files.
  ...
  finish:
    heapDestroy(heap);
    if (!*rsBaseTable
            ->name) { // juliana@223_14: solved possible memory problems.
      freeTable(context, rsBaseTable, false, true);
    }
  ...
  // Creates the result set without passing any fields or WHERE clause, since
  // all the records and all the fields in the temporary table are part of the
  // result set.
  ...
  bag->isSimpleSelect =
    isSimpleSelect; // juliana@210_1: select * from table_name does not create
                    // a temporary table anymore.
  bag->driver = driver; // juliana@227_2

  // juliana@223_13: corrected a bug that could break the application when
  // freeing a result set of a prepared statement.
  bag->isPrepared = selectClause->isPrepared;

  if (rsBaseTable->answerCount >= 0) {
    bag->answerCount = rsBaseTable->answerCount;

    // juliana@263_3: corrected a bug where a new result set data could overlap
    // an older result set data if both were related to the same table.
    if (!(bag->allRowsBitmap =
            (uint8*)xmalloc(rsBaseTable->allRowsBitmapLength))) {
      goto finish;
    }
    xmemmove(bag->allRowsBitmap,
             rsBaseTable->allRowsBitmap,
             rsBaseTable->allRowsBitmapLength);
  }
 ...
 */
bool
litebaseBindSelectStatement(Context context,
                            TCObject driver,
                            SQLSelectStatement* selectStmt)
{
  ...
  while (--i >= 0) { // Gets all tables from the select statement.
    if (!(table = tableList[i]->table =
            getTable(context, driver, tableList[i]->tableName))) {
      return false;
    }
  }

  if (!bindSelectStatement(context,
                           selectStmt)) { // Validates and binds the select
                                          // statement to the table list.
    return false;
  }
  orderTablesToJoin(
    selectStmt); // Finds the best table order for the join operation.
...
/**
 * Tries to put as inner table a table that has an index used more often in the
 * where clause, when the where clause has a comparison between fields from
 * different tables. e.g.: <code>select * from table1, table2 where
 * table1.field1 = table2.field2 </code> If only <code>table1.field1</code> has
 * index, changes the select to: <code>select * from table2, table1 where
 * table1.field1 = table2.field2</code>. If both tables has the same level of
 * index using, sorts them by the row count.
 *
 * @param selectStmt A SQL select statement.
 */
void
orderTablesToJoin(SQLSelectStatement* selectStmt)
{
  ...
  int32 size = selectClause->tableListSize, i = size, j, k, highest;
  uint8 startedIndex[MAXIMUMS], changedTo[MAXIMUMS];
  ...
  while (--i >= 0) { // Reorders the tables according to the weight.
    ...
    while (--j >= 0)
      ...
      if (tableAux1->weight >
            (tableAux2 = (rsTableAux2 = tableList[j])->table)->weight ||
          (tableAux1->weight == tableAux2->weight &&
           tableAux1->db.rowCount > tableAux2->db.rowCount)) {
 ...
 * @param context The thread context where the function is being executed.
 * @param selectStmt A SQL select statement.
 * @return <code>true</code> if the statement could be corrected bound;
 * <code>false</code>, otherwise.
 */
bool
bindSelectStatement(Context context, SQLSelectStatement* selectStmt)
{
  ...
  SQLResultSetTable** tableList = selectClause->tableList;
  int32 count = 0, size = selectClause->tableListSize, i = size;
  Table* table;
  ...
  i = size;
  while (--i >=
         0) { // Adds the column types properties of all tables to a big array.
  ...
  if (!bindColumnsSQLSelectClause(context, selectClause) ||
      (selectStmt->whereClause &&
       !bindColumnsSQLBooleanClause(context,
                                    selectStmt->whereClause,
                                    names2Index,
                                    columnTypes,
                                    tableList,
                                    size,
                                    selectStmt->selectClause->heap)) ||
      (selectStmt->groupByClause &&
       !bindColumnsSQLColumnListClause(context,
                                       selectStmt->groupByClause,
                                       names2Index,
                                       columnTypes,
                                       tableList,
 ...
 * @return <code>false</code> if a <code>SQLParseException</code> occurs;
 * <code>true</code>, otherwise.
 * @throws SQLParseException If the order by and group by clauses do not match,
 * if a query with group by is not well-formed, if there is a having clause
 * without an aggregation, a field in the having clause is not in the select
 * clause, there is no order by and there are aggregated functions mixed with
 * real columns, or there is an aggregation with an order by clause and no group
 * by clause.
 */
bool
validateSelectStatement(Context context, SQLSelectStatement* selectStmt)
{
  TRACE("validateSelectStatement")
  SQLSelectClause* selectClause = selectStmt->selectClause;
  int32 selectCount = selectClause->fieldsCount, i = selectCount;
  ...
  if (groupByClause && orderByClause &&
      !sqlcolumnlistclauseEquals(groupByClause, orderByClause)) {
    TC_throwExceptionNamed(context,
                           "litebase.SQLParseException",
                           getMessage(ERR_ORDER_GROUPBY_MUST_MATCH),
                           0);
    return false;
  }
  ...
  if (groupByClause) { // Validates the group by clause if it exists.
    while (--i >=
           0) { // Checks if all fields referenced in the HAVING clause are
                // listed as aliased aggregated functions in the SELECT clause.
      field1 = selectFields[i];

      // For now, there is no support for queries with GROUP BY and virtual
      // columns in the SELECT clause that are not aggregated functions.
      if (!field1->isAggregatedFunction && field1->isVirtual) {
        TC_throwExceptionNamed(context,
                               "litebase.SQLParseException",
                               getMessage(ERR_VIRTUAL_COLUMN_ON_GROUPBY),
                               0);
        return false;
      }

      // Checks if every non-aggregated function field that is listed in the
      // SELECT clause is listed in the GROUP BY clause.
      if (!field1->isAggregatedFunction &&
          !sqlcolumnlistclauseContains(groupByClause, field1->tableColIndex)) {
        TC_throwExceptionNamed(context,
                               "litebase.SQLParseException",
                               getMessage(ERR_AGGREG_FUNCTION_ISNOT_ON_SELECT),
                               0);
        return false;
      }
    }

    if (havingClause) { // Checks if all fields referenced in the HAVING clause
                        // are listed as aliased aggregated functions in the
                        // SELECT clause.
      int32 havingFieldsCount = havingClause->fieldsCount, j;
      ...
      while (--i >= 0) {
        ...
        j = selectCount;
        while (--j >= 0) {
          if (field1->aliasHashCode ==
              (field2 = selectFields[j])->aliasHashCode) {
            ...
            TC_throwExceptionNamed(context,
                                   "litebase.SQLParseException",
                                   getMessage(ERR_IS_NOT_AGGREG_FUNCTION),
                                   field1->alias);
            return false;
          }
        }

        if (!found) { // The having clause fields must be in the select clause
                      // fields.
          TC_throwExceptionNamed(
            context,
            "litebase.SQLParseException",
            getMessage(ERR_WAS_NOT_LISTED_ON_AGGREG_FUNCTION),
            field1->alias);
          return false;
        }
      }
    ...
    // If there is no 'GROUP BY' clause, there can not be aggregated functions
    // mixed with real columns in the SELECT clause.
    if (selectClause->hasRealColumns && selectClause->hasAggFunctions) {
      TC_throwExceptionNamed(context,
                             "litebase.SQLParseException",
                             getMessage(ERR_CANNOT_MIX_AGGREG_FUNCTION),
                             0);
      return false;
    }

    // If there are aggregate functions with an ORDER BY clause with no GROUP BY
    // clause, also raises an exception.
    if (selectClause->hasAggFunctions && orderByClause) {
      TC_throwExceptionNamed(context,
                             "litebase.SQLParseException",
                             getMessage(ERR_CANNOT_HAVE_AGGREG_AND_NO_GROUPBY),
                             0);
      return false;
    }
  }
 ...
 */
Table*
generateResultSetTable(Context context,
                       TCObject driver,
                       SQLSelectStatement* selectStmt)
{
  TRACE("generateResultSetTable")
  SQLSelectClause* selectClause = selectStmt->selectClause;
  int32 numTables = selectClause->tableListSize, i, j, count = 0,
        totalRecords = 0, size = 0,
        selectFieldsCount = selectClause->fieldsCount,
        aggFunctionsColsCount = 0, groupCount, row = -1, numberRows,
        answerCount, numOfBytes;
  bool aggFunctionExist, writeDelayed, isTableTemporary,
    countQueryWithWhere = false, useIndex = true;
  SQLResultSetTable** tableList = selectClause->tableList;
  SQLBooleanClause* whereClause = selectStmt->whereClause;
  SQLColumnListClause* groupByClause = selectStmt->groupByClause;
  SQLColumnListClause* orderByClause = selectStmt->orderByClause;
  ...
  SQLBooleanClause* havingClause = selectStmt->havingClause;
  SQLResultSetField** fieldList = selectClause->fieldList;
  SQLResultSetField** groupList =
    groupByClause ? groupByClause->fieldList : null;
  Table* tempTable1 = null;
  Table* tempTable2 = null;
  Table* tempTable3 = null;
  CharP countAlias = null;
  int8 columnTypes[MAXIMUMS], aggFunctionsCodes[MAXIMUMS];
  uint8 nullsCurRecord[NUMBEROFBYTES(MAXIMUMS + 1)];
  int16 columnIndexes[MAXIMUMS];
  int32 columnHashes[MAXIMUMS], columnSizes[MAXIMUMS],
    aggFunctionsRealParamCols[MAXIMUMS], groupCountCols[MAXIMUMS],
  ...
  Hashtable
    colHashesTable; // juliana@270_24: corrected a possible application crash
                    // or exception when using order/group by with join.
  Heap heap = null, heap_1 = null, heap_2 = null, heap_3 = null;

  if (numTables == 1) { // The query is not a join.
    (tempTable1 = (*tableList)->table)->answerCount = -1;
  }

  // juliana@226_13: corrected a bug where a query of the form "select
  // year(field) as years from table" could be confunded with "select count(*)
  // as years from table". Optimization for queries that just wants to count the
  // number of records of a table ("SELECT COUNT(*) FROM TABLE").
  if (selectFieldsCount == 1 && !groupByClause &&
      (*fieldList)->sqlFunction == FUNCTION_AGG_COUNT &&
      (*fieldList)->isAggregatedFunction) {
  ...
  while (++i < selectFieldsCount) {
    ...
    // Decides which hash code to use as the column name in the temp table and
    // which data type to assign to the temporary table.
    if (field->isVirtual) {
      if (field->isAggregatedFunction) {
        // Finds the fields that are parameter for a MAX() and MIN() function
        // that can use an index. juliana@230_21: MAX() and MIN() now use
        // indices on simple queries.
      ...
      if ((param = field->parameter)) {
        ...
        // juliana@270_24: corrected a possible application crash or exception
        // when using order/group by with join.
        TC_htPut32(&colHashesTable,
                   param->aliasHashCode,
                   1); // juliana@253_1: corrected a bug when sorting if the
                       // sort field is in a function.
      } else {         // Uses the parameter hash and data type.
        columnTypes[size] = field->dataType; // Uses the field data type.
        columnHashes[size] =
          field->aliasHashCode; // Uses the alias hash code instead.
        columnIndexes[size] =
          -1; // This is just a place holder, since this column does not map
      ...
      TC_htPut32(&colHashesTable,
                 field->aliasHashCode,
                 0); // juliana@253_1: corrected a bug when sorting if the sort
                     // field is in a function.
  ...
  if (sortListClause) {
    findSortIndex(sortListClause); // juliana@230_29: order by and group by now
                                   // use indices on simple queries.

    // Checks if all columns listed in the order by/group by clause were
    // selected. If not, includes the ones that are missing. It must be
    // remembered that, if both are present, group by and order by must match.
    // So, it does not matter which one is picked.
    ...
    while (++i < count) {
      // juliana@270_24: corrected a possible application crash or exception
      // when using order/group by with join.
      if (!TC_htGet32Inv(&colHashesTable,
                         (field = fieldList[i])->aliasHashCode)) {
        continue;
      }
  ...
  // Creates the temporary table to store the records that satisfy the WHERE
  // clause. For optimization, the first temporary table will NOT be created, in
  // case there is no WHERE clause and sort clause (either ORDER BY or GROUP BY)
  // and the SELECT clause contains aggregated functions. In this case, the
  // calculation of the aggregated functions will be made on the existing table.
  if (!whereClause && !sortListClause && selectClause->hasAggFunctions &&
      numTables == 1) {
    // In this case, there is no need to create the temporary table. Just points
    // the necessary structures of the original table.
    totalRecords = tempTable1->db.rowCount;

    // The index should not be used for MAX() and MIN() if not all the fields
    // are MAX() and MIN() or one of the parametes cannot use an index.
    i = -1;
    while (++i < selectFieldsCount)
    ...
    if (!createListResultSetForSelect(
          context, tableList, numTables, whereClause, listRsTemp, heap)) {
      goto error;
    }

    rsTemp = *listRsTemp;

    // The index should not be used for MAX() and MIN() if there is a join, a
    // sort or the indices do not resolve all the query.
    if (!sortListClause && (!whereClause || !whereClause->expressionTree) &&
        numTables == 1) {
      // The index should not be used for MAX() and MIN() if not all the fields
      // are MAX() and MIN() or one of the parametes cannot use an index.
      i = -1;
    ...
    if (sortListClause && ((whereClause && whereClause->expressionTree) ||
                           selectClause->hasAggFunctions || numTables != 1)) {
      sortListClause->index = -1;
    }

    // juliana@230_14: removed temporary tables when there is no join, group by,
    // order by, and aggregation.
    if ((sortListClause && sortListClause->index == -1) ||
        countQueryWithWhere || numTables != 1) {
      // Optimization for queries of type "SELECT COUNT(*) FROM TABLE WHERE..."
      // Just counts the records of the result set and write it to a table.
      ...
      // Creates a temporary table to store the result set records and writes
      // the result set records to the temporary table..
      if ((tempTable1 =
             driverCreateTable(context,
                               driver,
                               null,
                               null,
                               duplicateIntArray(columnHashes, size, heap_1),
                               duplicateByteArray(columnTypes, size, heap_1),
                               duplicateIntArray(columnSizes, size, heap_1),
                               null,
                               null,
                               NO_PRIMARY_KEY,
                               NO_PRIMARY_KEY,
        ...
        totalRecords =
          writeResultSetToTable(context,
                                ...
                                selectClause,
                                columnIndexesTables,
                                whereClause ? whereClause->type : -1,
                                heap);
      } else {      // guich@570_97
      ...
      if (selectClause->type == COUNT_WITH_WHERE) {
        ...
        return createIntValueTable(context, driver, totalRecords, countAlias);
               ...
               !sortListClause) { // A query that use index for MAX() and MIN()
                                  // should not check now which rows are
                                  // answered.
      ...
      if (newLength > oldLength)
        if (!(tempTable1->allRowsBitmap = allRowsBitmap = xrealloc(
                allRowsBitmap, tempTable1->allRowsBitmapLength = newLength))) {
          TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
          goto error;
        }
      ...
      if (!selectClause->hasAggFunctions) { // Nothing more to be done. Just
                                            // returns the result.
        heapDestroy(heap);
        return tempTable1;
      }
  ...
  if (sortListClause) { // Sorts the temporary table, if required.
    if (sortListClause->index == -1) {
      ...
      if ((tempTable1 =
        ...
        if (!(plainDB->rowAvail =
                (rsTemp->rowsBitmap.items
                   ? bitCount(rsTemp->rowsBitmap.items,
                              rsTemp->rowsBitmap.length)
        ...
        while (--i >= 0)
          if (types[i] == CHARS_TYPE || types[i] == CHARS_NOCASE_TYPE) {
            record[i]->asChars =
              (JCharP)TC_heapAlloc(heap, (sizes[i] << 1) + 1);
          }
        ...
        if (!mfGrowTo(
              context, &plainDB->db, plainDB->rowAvail++ * plainDB->rowSize)) {
          goto error;
        }
        ...
        } else if (!sortRecordsDesc(context,
                                    index,
                                    &rsTemp->rowsBitmap,
                                    tempTable1,
                                    record,
  ...
  // There is still one new temporary table to be created, if the select clause
  // has aggregate functions or here is a group by clause.
  ...
  // When creating the new temporary table, removes the extra fields that were
  // created to perform the sorting. juliana@270_24: corrected a possible
  // application crash or exception when using order/group by with join.
  if (sortListClause && count != selectFieldsCount) {
    size = selectFieldsCount;
  }

  // Also updates the types and hashcodes to reflect the types and aliases of
  // the final temporary table, since they may still reflect the aggregated
  // functions paramList types and hashcodes.
  columnSizesItems1 = columnSizes;

  // First preserves the original types, since they will be needed in the
  // aggregated functions running totals calculation.
  ...
  if (!(tempTable2 =
          driverCreateTable(context,
                            ...
                            null,
                            null,
                            duplicateIntArray(columnHashes, size, heap_2),
                            duplicateByteArray(columnTypes, size, heap_2),
                            duplicateIntArray(columnSizes, size, heap_2),
                            null,
                            null,
                            NO_PRIMARY_KEY,
                            NO_PRIMARY_KEY,
  ...
  if ((aggFunctionExist = selectClause->hasAggFunctions) &&
      !useIndex) { // Initializes the aggregated functions running totals.
    aggFunctionsRunTotals =
      (SQLValue*)TC_heapAlloc(heap, sizeof(SQLValue) * selectFieldsCount);

    // If the temporary table points to a real table, stores also the column
    // indexes of aggregate functions parameter list of the real table.
    if (*tempTable1->name) {
      i = aggFunctionsColsCount;
      while (--i >= 0) {
        aggFunctionsRealParamCols[i] =
  ...
  curRecord = record1 =
    newSQLValues(i = (count = tempTable1->columnCount) > selectFieldsCount
                       ? count
                       : selectFieldsCount,
                 heap);
  ...
  if (useIndex) {
    ...
    if (!(tempTable1->db.rowCount - tempTable1->deletedRowsCount) ||
        (whereClause && !bitCount(rowsBitmap->items, rowsBitmap->length))) {
      heapDestroy(heap);
      return tempTable2;
    }
  ...
  // Loops through the records of the temporary table, to calculate the
  // agregated values and/or write the group records.
  paramCols = (isTableTemporary = !*tempTable1->name)
                ? aggFunctionsParamCols
                : aggFunctionsRealParamCols;

  xmemzero(groupCountCols,
           aggFunctionsColsCount
             << 2); // Each column has a groupCount because of the null values.

  // juliana@230_20: solved a possible crash when using aggregation functions
  // with strings. Allocates the total space for the strings at once so that
  // they do not need to be reallocated.
  columnSizesItems1 = tempTable1->columnSizes;
  columnSizesItems2 = tempTable2->columnSizes;

  i = selectFieldsCount;
  while (--i >= 0)
    if ((i < count && columnSizesItems2[i] > columnSizesItems1[i]) ||
        columnSizesItems2[i]) {
      record1[i]->asChars =
        (JCharP)TC_heapAlloc(heap, (columnSizesItems2[i] << 1) + 2);
      record2[i]->asChars =
        (JCharP)TC_heapAlloc(heap, (columnSizesItems2[i] << 1) + 2);
    }
  ...
  i = count;
  while (--i >= 0)
    if ((i < selectFieldsCount &&
         columnSizesItems1[i] > columnSizesItems2[i]) ||
        (columnSizesItems1[i] && !record1[i]->asChars)) {
      record1[i]->asChars =
        (JCharP)TC_heapAlloc(heap, (columnSizesItems1[i] << 1) + 2);
      record2[i]->asChars =
        (JCharP)TC_heapAlloc(heap, (columnSizesItems1[i] << 1) + 2);
    }
  ...
  i = aggFunctionsColsCount;
  while (--i >= 0) {
    // juliana@227_12: corrected a possible bug with MAX() and MIN() with
    // strings.
    if (paramCols[i] >= 0 && columnSizesItems1[paramCols[i]]) {
      aggFunctionsRunTotals[i].asChars =
        (JCharP)TC_heapAlloc(heap, (columnSizesItems1[paramCols[i]] << 1) + 2);
    }
  }
  ...
  // juliana@253_17: correted a possible crash or wrong result when using
  // aggregation functions without using indices on a table with many columns.
  ...
  for (i = -1, groupCount = 0; ++i < totalRecords; groupCount++) {
    ...
    // Because it is possible to be pointing to a real table, skips deleted
    // records.
    ...
    // In case there is a group by, checks if there was a change in the group
    // record composition.
    if (groupByClause && groupCount &&
        compareRecords(prevRecord,
                       curRecord,
                       nullsPrevRecord,
                       nullsCurRecord,
                       count,
                       groupList)) {
      if (aggFunctionExist) // Checks if there are aggregate functions and
                            // concludes any aggregated function calculation.
        endAggFunctionsCalc(prevRecord,
                            groupCount,
                            aggFunctionsRunTotals,
                            aggFunctionsCodes,
                            aggFunctionsParamCols,
                            paramCols,
                            aggFunctionsColsCount,
                            origColumnTypesItems,
                            groupCountCols);

      // Flushes the previous record and starts a new group counting, taking the
      // null values for the non-aggregate fields into consideration.
    ...
    // Checks if there are aggregate functions and performs the calculation of
    // the aggregate functions..
    if (aggFunctionExist)
      performAggFunctionsCalc(context,
                              curRecord,
                              nullsCurRecord,
                              aggFunctionsRunTotals,
                              aggFunctionsCodes,
  ...
  if (writeDelayed &&
      groupCount >
        0) { // If there was adelayed writing, flushes the last record.
    if (aggFunctionExist)
      // Concludes any aggregated function calculation.
      endAggFunctionsCalc(prevRecord,
                          groupCount,
    ...
    xmemmove(columnNulls0,
             nullsCurRecord,
             numOfBytes); // Takes the null values for the non-aggregate fields
                          // into consideration.
    j = aggFunctionsColsCount;
    while (--j >= 0) {
      setBit(columnNulls0, aggFunctionsParamCols[j], !groupCountCols[j]);
  ...
  if (!*tempTable1->name) { // Drops the first temporary table, if it is really
                            // a temporary table.
  ...
  if (!havingClause) { // If there is no having clause, returns the temp
                       // table 2.
    heapDestroy(heap);
    return tempTable2;
  }
  ...
  // juliana@223_14: solved possible memory problems.
  // Creates the last and third temporary table to hold the records that comply
  ...
  if (!(tempTable3 =
          driverCreateTable(context,
                            ...
                            null,
                            null,
                            duplicateIntArray(columnHashes, size, heap_3),
                            duplicateByteArray(columnTypes, size, heap_3),
                            duplicateIntArray(columnSizes, size, heap_3),
                            null,
                            null,
                            NO_PRIMARY_KEY,
                            NO_PRIMARY_KEY,
  ...
  // The HAVING clause only works with aliases. So, remaps the table column
  // names, to use the aliases of the select statement instead of the original
  // column names.
  if (!remapColumnsNames2Aliases(
        context, tempTable3, fieldList, selectClause->fieldsCount) ||
      !bindColumnsSQLBooleanClause(context,
                                   havingClause,
                                   &tempTable3->htName2index,
                                   tempTable3->columnTypes,
                                   null,
                                   0,
                                   heap_3)) {
    goto error;
  ...
  if (!(rsTemp =
          createResultSetForSelect(context, tempTable2, havingClause, heap))) {
    goto error;
  }

  // juliana@223_14: solved possible memory problems.
  // Writes the result set to the temporary table 3.
  if (writeResultSetToTable(context,
                            &rsTemp,
                            1,
                            tempTable3,
                            null,
 ...
 * @return <code>false</code>if an error occurs when appling the indices;
 * <code>true</code>, otherwise.
 */
bool
createListResultSetForSelect(Context context,
                             ...
                             Heap heap)
{
  ...
  while (++i < size) {
    resultSet =
      createResultSet((table = tableList[i]->table), whereClause, heap);
  ...
  if (whereClause) { // Tries to apply the table indices to generate a bitmap of
                     // the rows to be returned.
    if (size > 1) {  // Join.
      setIndexRsOnTree((*rsList)->whereClause->expressionTree);
    }
    if (!generateIndexedRowsMap(
          context, rsList, size, hasComposedIndex, heap)) {
      return false;
    }
  }
  return true;
 ...
 * @return <code>true</code> if the function executed correctly;
 * <code>false</code>, otherwise.
 */
bool
generateIndexedRowsMap(Context context,
                       ResultSet** rsList,
                       int32 size,
                       bool hasComposedIndex,
                       Heap heap)
{
  ...
  if (whereClause) {
    ...
    rsList[0]->markBits = markBits = TC_heapAlloc(heap, sizeof(MarkBits));
    markBits->leftKey.keys =
      (SQLValue*)TC_heapAlloc(heap, sizeof(SQLValue) * maxSize);
    markBits->rightKey.keys =
      (SQLValue*)TC_heapAlloc(heap, sizeof(SQLValue) * maxSize);
    ...
    if (!computeIndex(
          context, rsList, size, size > 1, -1, null, -1, -1, heap)) {
      return false;
    }

    if (!whereClause->expressionTree)
      while (--size >= 0) { // There is no where clause left, since all rows can
 ...
 * @param indexRsOnTheFly The index of the result set or -1 if the query is
 * being indexed on the fly.
 ...
 */
bool
computeIndex(Context context,
             ResultSet** rsList,
             int32 size,
             bool isJoin,
             int32 indexRsOnTheFly,
             SQLValue* value,
             int32
             operator,
             int32 colIndex,
             Heap heap)
{
  ...
  // with the indexes values to search for and the bool operation to apply.
  TRACE("computeIndex")
  ResultSet* rsBag;
  ...
  int32 i, j, count = 1, booleanOp, recordCount, col, op;
  ...
  if (isJoin) { // Puts the result set bag in order with the tables.
    ...
    while (--i >= 0) {
      ...
      while (--j >= 0)
        if (rsList[j]->table == appliedIndexTables[i]) {
          ...
          rsListPointer[i]->rowsBitmap =
            newIntBits(recordCount = rsBag->table->db.rowCount - 1, heap);
          break;
        }
    }
  } else {
    if (onTheFly) {
      ...
      if (!resultSet->auxRowsBitmap.items) {
        ...
        xmemzero(resultSet->auxRowsBitmap.items,
                 resultSet->auxRowsBitmap.size << 2);
      }
    } else {
  ...
  // Loops through all applied indexes, and records the indexed rows in the
  // bitmap.
  i = -1;
  while (++i < count) {
    isCI = false;
    ...
    if (onTheFly) {
      ...
      if ((isCI = (appliedComposedIndexes[i] != null))) {
        ...
        while (--j >= 0) {
          if (!(isMatch = (relationalOps[i + j] == OP_PAT_MATCH_NOT_LIKE ||
                           relationalOps[i + j] == OP_PAT_MATCH_LIKE)) &&
              !getOperandValue(
                context, indexedValues[i + j], &markBits.leftKey.keys[j])) {
            return false;
          }
        }
      } else {
        col = indexedCols[i];
        if (!(isMatch = (op = relationalOps[i]) == OP_PAT_MATCH_NOT_LIKE ||
                        op == OP_PAT_MATCH_LIKE) &&
            !getOperandValue(
              context, indexedValues[i], &markBits.leftKey.keys[0])) {
          return false;
        }
      }
    }
    ...
    index =
      (isCI) ? appliedComposedIndexes[i]->index : table->columnIndexes[col];
    markBitsReset(
      &markBits,
      (rsBag->indexCount > 1)
        ? &auxBitmap
        : (onTheFly ? &rsBag->auxRowsBitmap
    ...
    while (--j >= 0) {
      ...
      // if the operation is 'like x%', then replaces the operand by the value
      // without the % mask.
      if (isMatch) {
        // juliana@230_10: solved a bug that could crash the application when
        // more than one index is applied.
      ...
      // Checks if this is a "between" operation.
      else if (booleanOp == OP_BOOLEAN_AND && i < (count - 1) &&
               indexedCols[i + 1] == col &&
               (op == OP_REL_GREATER || op == OP_REL_GREATER_EQUAL) &&
               (relationalOps[i + j + 1] == OP_REL_LESS ||
                relationalOps[i + j + 1] == OP_REL_LESS_EQUAL)) {
        if (!getOperandValue(
              context, indexedValues[j + i + 1], &markBits.rightKey.keys[0])) {
          return false;
        }
        ...
        markBits.rightOp[j] =
          relationalOps[j + i++ +
                        1]; // The next operation is already processed.
      } else {
        switch (col = op) { // When searching for !=, <, <=, we'll use the
        ...
        if (op != col) { // All the rows will be marked and only resets the rows
                         // that satisfy the opposite operation.
          xmemset(
            markBits.indexBitmap->items, 0xFF, markBits.indexBitmap->size << 2);
    ...
    switch (op) { // Finally, marks all rows that match this value / range of
                  // values.
      case OP_REL_EQUAL:
    ...
    // If it is the first index, assigns the index bitmap to the resulting
    // bitmap. Otherwise, merges the index bitmap with the existing bitmap,
    // using the boolean operator.
    if (rsBag->indexCount > 1) {
      mergeBitmaps((onTheFly ? &rsBag->auxRowsBitmap : &rsBag->rowsBitmap),
                   markBits.indexBitmap,
                   booleanOp);
    }

    rsBag->rowsBitmapBoolOp =
      booleanOp; // juliana@230_15: corrected a bug when using joins with
                 // indices which could cause an OutOfMemoryExcepion.
    if (isCI) {
      i += (size - 1);
    }
  }
 ...
 */
void
mergeBitmaps(IntVector* bitmap1, IntVector* bitmap2, int32 booleanOp)
{
  TRACE("mergeBitmaps")
...
/**
 * Concludes the calculation of the given aggregated function running totals
 * based on the given record and the group count.
 *
 ...
 * @param aggFunctionsParamCols The columns that are parameters to the
 * aggregated functions.
 * @param aggFunctionsRealParamCols The real columns that are parameters to the
 * aggregated functions.
 * @param aggFunctionsColsCount The number of columns that are parameters to the
 * aggregated functions.
 * @param columnTypes The types of the columns.
 * @param groupCountCols The count for the groups.
 */
void
endAggFunctionsCalc(SQLValue** record,
                    int32 groupCount,
                    SQLValue* aggFunctionsRunTotals,
                    int8* aggFunctionsCodes,
                    int32* aggFunctionsParamCols,
                    int32* aggFunctionsRealParamCols,
                    int32 aggFunctionsColsCount,
                    int8* columnTypes,
                    int32* groupCountCols)
{
  TRACE("endAggFunctionsCalc")
  int32 j = aggFunctionsColsCount, colIndex, realColIndex, sqlAggFunction,
        colType;
  SQLValue* aggValue;
  SQLValue* value;

  while (--j >= 0) { // Concludes the calculation of the aggregate functions
                     // running totals.
    ...
    switch (sqlAggFunction = aggFunctionsCodes[j]) {
      ...
      // juliana@226_5: the aggregation functions MAX() and MIN() now work for
      // CHAR, VARCHAR, CHAR NOCASE, and VARCHAR NOCASE column types.
      case FUNCTION_AGG_MAX:
      case FUNCTION_AGG_MIN: {
        switch (colType =
                  columnTypes[realColIndex]) { // Checks the type of the column.
          case SHORT_TYPE:
          ...
          // table when building result sets. juliana@227_12: corrected a
          // possible bug with MAX() and MIN() with strings.
          case CHARS_TYPE:
          case CHARS_NOCASE_TYPE: {
            xmemmove(value->asChars,
                     aggValue->asChars,
                     (value->length = aggValue->length) << 1);
            value->asInt = aggValue->asInt;
 ...
 * @return The table if the method executes correctlty; <code>null</code>,
 * otherwise.
 * @throws OutOfMemoryError If there is not enougth memory alloc memory.
 */
Table*
createIntValueTable(Context context,
                    TCObject driver,
                    int32 intValue,
                    CharP colName)
{
  ...
  *colType = INT_TYPE;
  ...
  // juliana@223_14: solved possible memory problems.
  if ((table = driverCreateTable(context,
                                 driver,
                                 null,
                                 null,
                                 colHash,
                                 ...
                                 1,
                                 heap)) &&
      writeRSRecord(context, table, &record)) {
    return table;
  }
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 * @throws SQLParseException In case of an unknown or ambiguous column name, the
 * parameter and the function data types are incompatible, or the total number
 * of fields of the select exceeds the maximum.
 */
bool
bindColumnsSQLSelectClause(Context context, SQLSelectClause* clause)
{ // guich@512_2: added columnnames
  ...
  int32 tableListSize = clause->tableListSize,
        fieldListSize = clause->fieldsCount, i, j, index, columnCount;
  ...
  if (!fieldListSize) { // If the select clause has a wild card (is null), then
                        // expands the list using the column information from
                        // the given tables.
    ...
    // juliana@250_7: now a select * will cause a SQLParseException if the total
    // number of columns is more than 254.
    if (count > MAXIMUMS) {
      TC_throwExceptionNamed(
        context, "litebase.SQLParseException", getMessage(ERR_FIELDS_OVERFLOW));
      return false;
    }
    ...
    while (++j < tableListSize) {
      ...
      while (++i <
             columnCount) { // guich@503_10: changed loop to exclude the rowid.
        // SQLResultSetField - guich@503_10: added -1 to exclude the rowid.
        (field = fieldList[count++] = initSQLResultSetField(clause->heap))
          ->alias = columnNames[i]; // guich@_512_2: stores the column name too.
        ...
        TC_htPut32(&htName2index,
                   TC_hashCodeFmt("sss", tableName, ".", columnNames[i]),
                   position);
        TC_htPut32IfNew(&htName2index, columnHashes[i], position++);
      }
    }
  } else {
    int32 hashAliasTableName, hash1, auxIndex, aggFunctionType, dtFunctionType,
      sqlFunction;
    bool foundFirst;
    CharP fieldName;
    Table* auxTable;
    ...
    while (++i < fieldListSize) { // Binds the listed colums to the table.
      ...
      if (!(field = fieldList[i])->isAggregatedFunction ||
        ...
        if (field->tableName) { // Checks the names.
          ...
          if (!currentTable ||
              ((index = TC_htGet32Inv(
                  &currentTable->htName2index,
                  hash1 = TC_hashCode(fieldName = field->tableColName))) < 0)) {
            TC_throwExceptionNamed(context,
                                   "litebase.SQLParseException",
          ...
          if (field->aliasHashCode !=
              TC_hashCodeFmt("sss",
                             tableName = field->tableName,
                             ".",
                             fieldName)) { // Used an explicit alias.
            TC_htPut32(&htName2index,
                       TC_hashCodeFmt("sss", tableName, ".", field->alias),
                       i);
          } else if (TC_htGet32Inv(&htName2index,
                                   hash1) <
                     0) { // Stores the name of the field once; only the first.
          ...
          j = tableListSize;
          while (--j >= 0) {
            if ((auxIndex = TC_htGet32Inv(
                   &(auxTable = (rsTableAux = tableList[j])->table)
                      ->htName2index,
                   field->tableColHashCode)) >= 0) {
              if (foundFirst) {
                TC_throwExceptionNamed(context,
                                       "litebase.SQLParseException",
                                       getMessage(ERR_AMBIGUOUS_COLUMN_NAME),
                                       field->alias);
                return false;
              } else {
                foundFirst = true;
                ...
                TC_htPut32(&htName2index,
                           TC_hashCodeFmt(
                             "sss", tableList[j]->tableName, ".", field->alias),
                           i);
                TC_htPut32(&htName2index, field->aliasHashCode, i);

                // juliana@252_4: corrected the fact that a field used in a
                // function can't be fetched using only the name of the field
      ...
      if (!field->isVirtual) { // If the field is not virtual, it needs to be
                               // mapped directly to the underlying table.
        field->dataType =
          (int8)currentTable->columnTypes[field->tableColIndex = index];
      ...
      // rnovais@568_10
      else if (field->isAggregatedFunction ||
               field
                 ->isDataTypeFunction) { // If it is an aggregated or data
                                         // type function, maps its parameter.
        param = field->parameter;
        aggFunctionType = dtFunctionType =
          UNDEFINED_TYPE; // initialized with an UNDEFINED number.
        ...
        if (param) {
          param->dataType =
            (int8)currentTable->columnTypes[param->tableColIndex = index];
          ...
          if (field->isAggregatedFunction) { // rnovais@568_10
            ...
            if (param->dataType == CHARS_TYPE &&
                (aggFunctionType == INT_TYPE ||
                 aggFunctionType == DOUBLE_TYPE)) {
              TC_throwExceptionNamed(
                context,
                "litebase.SQLParseException",
                getMessage(ERR_DATA_TYPE_FUNCTION),
                (sqlFunction == FUNCTION_AGG_COUNT)
                  ? "count"
                  : (sqlFunction == FUNCTION_AGG_MAX)
                      ? "max"
                      : (sqlFunction == FUNCTION_AGG_MIN)
                          ? "min"
                          : (sqlFunction == FUNCTION_AGG_AVG)
                              ? "avg"
                              : (sqlFunction == FUNCTION_AGG_SUM) ? "sum" : "");
              return false;
            }

            // For aggregated functions, if the function does not have a defined
            // data type, it inherits the parameter size and type.
            ...
            // rnovais@570_5: if UNDEFINED the datatype will be the same of the
            // field thus, it is possible to have functions that can be applyed
            // to diferents fieds type. e.g. ABS(int) returns int, ABS(double)
            // returns double, etc.
            if (dtFunctionType == UNDEFINED_TYPE) {
              field->dataType = param->dataType;
            }

            // rnovais@568_10: checks if the parameter and the data type
            // function data types are compatible.
 ...
 */
bool
remapColumnsNames2Aliases(Context context,
                          Table* table,
                          SQLResultSetField** fieldsList,
                          int32 fieldsCount)
{
  ...
  if (!table->columnNames) { // previously created? don't overrite it
    ...
    while (--fieldsCount >= 0) {
      table->columnNames[fieldsCount] =
        (field = fieldsList[fieldsCount])->alias;

      if (columnHashes[fieldsCount] ==
          field
            ->aliasHashCode) { // Replaces the original mapping, if necessary.
        continue;
      }
      if (!TC_htPut32(tableName2Index,
                      field->aliasHashCode,
                      fieldsCount)) { // Already replaces old values.
        return false;
      }
    }
  }
 ...
 * an <code>OR</code>.
 * @param heap A heap to allocate temporary structures.
 * @return The total number of records added to the table or -1 if an error
 * occurs.
 */
int32
writeResultSetToTable(Context context,
                      ResultSet** list,
                      int32 numTables,
                      Table* table,
                      int16* rs2TableColIndexes,
                      SQLSelectClause* selectClause,
                      size_t* columnIndexesTables,
                      int32 whereClauseType,
                      Heap heap)
{
  ...
  int32 countSelectedField =
          selectClause->fieldsCount, // rnovais@568_10: when it has an order
                                     // by table.columnCount =
                                     // selectClause.fieldsCount + 1.
    i, j, totalRecords = 0, rowSize = tempDB->rowSize,
        rsCount = rsTable->columnCount, bytes = NUMBEROFBYTES(rsCount), dbSize,
        dboSize, size = 0;
  ...
  // Windows CE, Palm, iPhone, and Android. No indices and no where clause:
  // allocs all the records space in the temporary .db.
  if (!resultSet->whereClause && !resultSet->rowsBitmap.size) {
    if (!mfGrowTo(context,
                  memoryDB,
                  (tempDB->rowAvail =
                     1 + selectDB->rowCount -
                     (*selectClause->tableList)->table->deletedRowsCount) *
                    rowSize) ||
  ...
  if (!tempDB->rowAvail &&
      rowsBitmap
        .size) { // Uses the indices if the other approachs can't be useful.
    int32 numberOfBits = bitCount(rowsBitmap.items, rowsBitmap.length) + 1;

    if (numberOfBits > 0) {
      if (!mfGrowTo(context,
                    memoryDB,
                    (tempDB->rowAvail = 1 + numberOfBits) * rowSize) ||
          !mfGrowTo(context, &tempDB->dbo, tempDB->rowAvail * size)) {
        goto error;
      }
    }
  ...
  if (numTables == 1) {
    ...
    while (--i >= 0) { // Allocates the record before using them.
      if ((colIndex = (items) ? items[i] : i) != -1) {
        ...
        if ((rsTypes[colIndex] == CHARS_TYPE ||
             rsTypes[colIndex] == CHARS_NOCASE_TYPE)) {
          values[i]->asChars =
            (JCharP)TC_heapAlloc(heap, (rsSizes[colIndex] << 1) + 2);
        }
      }
    }
    ...
    while (getNextRecord(context, resultSet, heap)) { // No preverify needed.
      ...
      while (++i < count) { // Gets the values of the result set columns.
        ...
        // For columns that do no map directly to the underlying table of the
        // result set, just skips the reading.
        if (colIndex != -1 && isBitUnSet(rsNulls0, colIndex) &&
            !getTableColValue(context,
                              resultSet,
                              colIndex,
                              values[i])) { // juliana@220_3
          goto error;
        }
        ...
        if (colIndex != -1) {
          setBit(nulls0,
                 i,
                 isBitSet(rsNulls0,
                          colIndex)); // Sets the null values of tempTable.
        } else {
          setBitOff(nulls0, i);
        }
    ...
    totalRecords = performJoin(context,
                               list,
                               numTables,
                               table,
                               rs2TableColIndexes,
 ...
 */
int32
bitCount(int32* elements, int32 length)
{
  TRACE("bitCount")
  int32 count = 0, value;
 ...
 * <code>OR</code>.
 * @param heap A heap to allocate temporary structures.
 * @return The number of records written to the temporary table or -1 if an
 * error occurs.
 */
int32
performJoin(Context context,
            ResultSet** list,
            int32 numTables,
            Table* table,
            int16* rs2TableColIndexes,
...
{
  TRACE("performJoin")
  int32 currentIndexTable = 0, totalRecords = 0, ret = NO_RECORD,
        length = table->columnCount, colIndex, position, rsCount;
  bool bitSet;
  ...
  while (--length >= 0) { // Allocates the necessary records before using them.
    ...
    if (types[length] == CHARS_TYPE || types[length] == CHARS_NOCASE_TYPE) {
      values[length]->asChars =
        (JCharP)TC_heapAlloc(heap, (sizes[length] << 1) + 2);
    }
  }
  ...
  while (currentIndexTable >= 0) {
    ...
    switch (ret = getNextRecordJoin(context,
                                    currentIndexTable,
                                    verifyWhereCondition[currentIndexTable],
                                    numTables,
                                    whereClauseType,
                                    list,
                                    heap)) {
      ...
      case VALIDATION_RECORD_INCOMPLETE: {
        ...
        xmemmove(rsNulls0 = rsTable->columnNulls,
                 rsTable->db.basbuf + rsTable->columnOffsets[rsCount],
                 NUMBEROFBYTES(rsCount));

        while (--length >= 0) { // Fills the data of the current ResultSet.
          position = indexes.items[length];

          // If rs2TableColIndexes == null, it indicates that the result set and
          ...
          if ((colIndex != -1) && !bitSet &&
              !getTableColValue(context,
                                currentRs,
                                colIndex,
                                values[position])) { // juliana@220_3
            return -1;
          }
 ...
 * verified.
 * @param totalRs The number of result sets (tables used in the join) in the
 * result set list.
 ...
 * <code>VALIDATION_RECORD_NOT_OK</code>,
 * <code>VALIDATION_RECORD_INCOMPLETE</code>, or -1 if an error occurs.
 */
int32
getNextRecordJoin(Context context,
                  int32 rsIndex,
                  bool verifyWhereCondition,
                  int32 totalRs,
                  int32 whereClauseType,
                  ResultSet** rsList,
                  Heap heap)
{
  ...
  IntVector rowsBitmap = (resultSet->auxRowsBitmap.size > 0)
                           ? resultSet->auxRowsBitmap
                           : resultSet->rowsBitmap;
  SQLBooleanClause* whereClause = resultSet->whereClause;
  int32 rowCountLess1 = plainDB->rowCount - 1, ret;
  ...
  if (rowsBitmap.size && verifyWhereCondition) {
    ...
    if (resultSet->pos < rowCountLess1) {
      if (!resultSet->whereClause || resultSet->auxRowsBitmap.size) {
        ...
        while ((position = findNextBitSet(&rowsBitmap, resultSet->pos + 1)) !=
                 -1 &&
               position <= rowCountLess1) {
          if (plainRead(context, plainDB, resultSet->pos = position)) {
            if (recordNotDeleted(
                  basbuf)) { // juliana@230_45: join should not take deleted
              ...
              if (resultSet->auxRowsBitmap.size && verifyWhereCondition &&
                ...
                return booleanTreeEvaluateJoin(
                  context, whereClause->expressionTree, rsList, totalRs, heap);
              }
              if (whereClauseType == WC_TYPE_AND_DIFF_RS) {
                return (totalRs == resultSet->indexRs + 1)
        ...
        // 1) The relationship between the bitmap and the WHERE clause is an AND
        // relationship. 2) The relationship between the bitmap and the WHERE
        // clause is an OR relationship.
        if (resultSet->rowsBitmapBoolOp == OP_BOOLEAN_AND) {
          // AND case - Walks through the bits that are set in the bitmap and
          // checks if the rows satisfy the where clause.
          while ((position = findNextBitSet(&rowsBitmap, resultSet->pos + 1)) !=
                   -1 &&
                 position <= rowCountLess1)
            if (plainRead(context, plainDB, resultSet->pos = position)) {
          ...
          while (resultSet->pos < rowCountLess1 &&
                 plainRead(context, plainDB, ++resultSet->pos)) {
            if (IntVectorisBitSet(&rowsBitmap, resultSet->pos)) {
              return VALIDATION_RECORD_OK;
            }
            whereClause->resultSet = resultSet;
            if (recordNotDeleted(basbuf) &&
                (ret = booleanTreeEvaluateJoin(context,
      ...
      if (recordNotDeleted(basbuf)) {
        ...
        if ((ret = booleanTreeEvaluateJoin(
          ...
          while (++rsIndex < totalRs)
            if (rsList[rsIndex]->auxRowsBitmap.size ||
                rsList[rsIndex]->rowsBitmap.size) {
 ...
 */
int32
booleanTreeEvaluateJoin(Context context,
                        ...
                        Heap heap)
{
  ...
  int32 indexRs = resultSet->indexRs, indexTree = tree->indexRs;
    ...
    if (indexTree > indexRs) { // juliana@211_5: solved a bug with joins which
      ...
      if (tree->bothAreIdentifier &&
        ...
        if (!valueJoin->asChars) {
          valueJoin->asChars = (JCharP)TC_heapAlloc(
            heap, 2 * resultSet->table->columnSizes[leftTree->colIndex] + 2);
        }
        ...
        if (rightTree->hasIndex && boolOp <= 1) {
          ...
          // juliana@230_39: join now can be much faster if the query is smartly
          // written.
  ...
  switch (tree->operandType) { // Checks what is the operand type of the tree.
    ...
    case OP_REL_GREATER_EQUAL:
    case OP_REL_LESS_EQUAL:
      switch (tree->valueType) { // Calls the right operation accordingly to the
                                 // values type.
        case SHORT_TYPE:
        case INT_TYPE:
        case LONG_TYPE:
        ...
        case DATE_TYPE:
        case DATETIME_TYPE:
          return compareNumericOperands(context, tree)
                   ? VALIDATION_RECORD_OK
                   : VALIDATION_RECORD_NOT_OK;
          return -1;
        case CHARS_TYPE:
          return compareStringOperands(context, tree, false, heap)
                   ? VALIDATION_RECORD_OK
                   : VALIDATION_RECORD_NOT_OK;
        case CHARS_NOCASE_TYPE:
          return compareStringOperands(context, tree, true, heap)
                   ? VALIDATION_RECORD_OK
                   : VALIDATION_RECORD_NOT_OK;
      }
    ...
    case OP_PAT_MATCH_LIKE:
    case OP_PAT_MATCH_NOT_LIKE:
      return matchStringOperands(
               context, tree, tree->valueType == CHARS_NOCASE_TYPE, heap)
               ? VALIDATION_RECORD_OK
               : VALIDATION_RECORD_NOT_OK;
    ...
    case OP_BOOLEAN_AND:           // AND connector.
      if (leftTree && rightTree) { // Expects both trees not be null.
        switch (
          booleanTreeEvaluateJoin(context, leftTree, rsList, totalRs, heap)) {
          case VALIDATION_RECORD_NOT_OK:
            return VALIDATION_RECORD_NOT_OK;
          case VALIDATION_RECORD_INCOMPLETE:
            if (booleanTreeEvaluateJoin(context,
                                        rightTree,
                                        rsList,
                                        totalRs,
                                        heap) ==
          ...
          case VALIDATION_RECORD_OK:
            switch (
              booleanTreeEvaluateJoin(context,
                                      rightTree,
                                      rsList,
                                      totalRs,
                                      heap)) { // The left side returned true,
                                               // so verifies the right branch.
              case VALIDATION_RECORD_NOT_OK:
                return VALIDATION_RECORD_NOT_OK;
              case VALIDATION_RECORD_INCOMPLETE_OK:
              case VALIDATION_RECORD_OK:
        ...
        switch (
          ...
          case VALIDATION_RECORD_NOT_OK:
            ...
            switch (booleanTreeEvaluateJoin(
              context, rightTree, rsList, totalRs, heap)) {
              case VALIDATION_RECORD_NOT_OK:
                return VALIDATION_RECORD_NOT_OK;

              // juliana@270_21: solved a very old join problem when using OR
              // and false constants comparison which would make the join return
              // no results.
              case VALIDATION_RECORD_INCOMPLETE_OK:
                return VALIDATION_RECORD_INCOMPLETE_OK;
    ...
    case OP_PAT_IS:
    case OP_PAT_IS_NOT:
      return compareNullOperands(tree) ? VALIDATION_RECORD_OK
                                       : VALIDATION_RECORD_NOT_OK;
  }
 ...
 * @param aggFunctionsRunTotals The current totals for the aggregation
 * functions.
 ...
 * @param aggFunctionsColsCount The number of columns that use aggregation
 * functions.
 * @param columnTypes The types of the columns.
 * @param groupCountCols The columns that use count.
 */
void
performAggFunctionsCalc(Context context,
                        SQLValue** record,
                        uint8* nullsRecord,
                        SQLValue* aggFunctionsRunTotals,
                        int8* aggFunctionsCodes,
                        ...
                        int32* groupCountCols)
{
  ...
  int32 i = aggFunctionsColsCount, colIndex, sqlAggFunction, colType;
  SQLValue* aggValue;
  SQLValue* value;
    ...
    switch (sqlAggFunction = aggFunctionsCodes[i]) {
      ...
      case FUNCTION_AGG_SUM: {
        switch (colType) { // Checks the type of the column.
          ...
          case DATETIME_TYPE:
            TC_throwExceptionNamed(context,
                                   "litebase.SQLParseException",
                                   getMessage(ERR_SUM_AVG_WITH_DATE_DATETIME));
            return;
        }
        break;
      }
      ...
      case FUNCTION_AGG_MAX: {
        switch (colType) { // Checks the type of the column.
          ...
          case CHARS_TYPE:
          case CHARS_NOCASE_TYPE:
            if (groupCountCols[i] == 1 || str16CompareTo(aggValue->asChars,
                                                         value->asChars,
                                                         aggValue->length,
                                                         value->length,
                                                         colType) < 0) {
              xmemmove(aggValue->asChars,
                       value->asChars,
                       (aggValue->length = value->length) << 1);
          ...
          case DATETIME_TYPE: // rnovais@567_2
            if (groupCountCols[i] == 1) {
              ...
              if (aggValue->asDate < value->asDate ||
                  aggValue->asTime <
                    value->asTime) { // The date or time is smaller.
                aggValue->asDate = value->asDate;
                aggValue->asTime = value->asTime;
              }
            }
        ...
        switch (colType) { // Checks the type of the column.
          ...
          case DATETIME_TYPE:
            if (groupCountCols[i] == 1) {
              ...
              if (aggValue->asDate > value->asDate ||
                  aggValue->asTime > value->asTime) { // The date is greater.
                aggValue->asDate = value->asDate;
                aggValue->asTime = value->asTime;
              }
            }
...
// order by, and aggregation.
/**
 * Calculates the answer of a select without aggregation, join, order by, or
 * group by without using a temporary table.
 *
 * @param context The thread context where the function is being executed.
 ...
 */
void
computeAnswer(Context context, ResultSet* resultSet, Heap heap)
{
  TRACE("computeAnswer")
  int32 i;
 ...
 */
void
findMaxMinIndex(SQLResultSetField* field)
{
  TRACE("findMaxMinIndex")
  Table* table = field->table;
  int32 column = field->parameter ? field->parameter->tableColIndex
                                  : field->tableColIndex,
        i = table->numberComposedIndexes;
  ComposedIndex** composedIndices = table->composedIndexes;

  if (table->columnIndexes[column]) { // If the field has a simple index, uses
                                      // it.
    field->index = column;
    field->isComposed = false;
  } else
    while (--i >= 0)
      if (*composedIndices[i]->columns ==
          column) { // Else, if the field is the first field of a composed
                    // index, uses it.
        field->index = i;
