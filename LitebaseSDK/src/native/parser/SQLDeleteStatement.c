...
SQLDeleteStatement* initSQLDeleteStatement(LitebaseParser* parser, bool isPrepared) {
	...
	SQLDeleteStatement* deleteStmt = (SQLDeleteStatement*)TC_heapAlloc(parser->heap,
									 sizeof(SQLDeleteStatement));
	...
	if (isPrepared) { // It is only necessary to re-allocate the parser structures if the statement is from a prepared statement.
		deleteStmt->rsTable = initSQLResultSetTable((*parser->tableList)->tableName,
			...
			while (getNextRecord(context, rs, heap)) {
				...
				while (--i >= 0) // Simple indexes.
					if ((index = columnIndexes[i]) && isBitUnSet(nulls, i)) {
						if (!readValue(context, plainDB, keys[0], columnOffsets[i], columnTypes[i], basbuf, false, false,
									   false, -1, null)) {
							goto error;
						}
				...
				if ((i = numberComposedIndexes)) // Composed index.
					while (--i >= 0) {
						...
						while (--id >= 0)
							if (!readValue(context, plainDB, keys[id], columnOffsets[columns[id]], colIdxTypes[id], basbuf,
...
bool litebaseBindDeleteStatement(Context context, TCObject driver, SQLDeleteStatement* deleteStmt) {
	...
	Table* table = deleteStmt->rsTable->table = getTable(context, driver,
				   deleteStmt->rsTable->tableName);

	// Binds the delete statement to its table.
	if (!table || (deleteStmt->whereClause
				   && !bindColumnsSQLBooleanClauseSimple(context, deleteStmt->whereClause, deleteStmt->rsTable,
						   deleteStmt->heap))) {
		return false;
	}
