...
LB_API bool LibOpen(OpenParams params) {
	...
	if (!ranTests) {
		...
		// The test results.
		TC_alert("%02d test total\n%02d succeeded\n%02d failed", 30, 30 - testSuite.failed,
				 testSuite.failed);
	}
#endif
	return true;
...
LB_API void LibClose() {
	...
	TC_htFreeContext(TC_getMainContext(), &htCreatedDrivers,
					 (VisitElementContextFunc)freeLitebase); // Flushs pending data and closes all tables.
...
bool initVars(OpenParams params) {
	...
	if (!(htCreatedDrivers = TC_htNew(10,
									  null)).items // Allocates a hash table for the loaded connections.
...
TCObject create(Context context, int32 crid, TCObject objParams) {
	...
	if (logger) { // juliana@230_30: reduced log files size.
		...
		if (objParams) {
			if (!TC_appendJCharP(context, logSBuffer, String_charsStart(objParams),
	...
	if (objParams) {
		...
		while (--i >= 0) { // The parameters order does not matter.
			...
			if (xstrstr(tempParams[i], "chars_type")) { // Chars type param.
				...
				path = TC_CharP2TCHARPBuf(tempParams[0],
				...
				TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_INVALID_PARAMETER),
									   tempParams[i]);
				return null;
			}
		}
	...
	if (!(driver = TC_htGetPtr(&htCreatedDrivers, (hash = TC_hashCodeFmt("ixiis", crid, context->thread,
							   isAscii, useCrypto, sourcePath ? TC_TCHARP2CharPBuf(sourcePath, params) : "null"))))) {
		Hashtable htTables,
				  htPS;
...
void freeLitebase(Context context, size_t driver) {
	...
	int32* nodes = getLitebaseNodes(
					   driver); // juliana@253_6: the maximum number of keys of a index was duplicated.
	...
	TC_htRemove(&htCreatedDrivers,
				OBJ_LitebaseKey((TCObject)
								driver)); // fdie@555_2: removes this instance from the drivers hash table.
...
void litebaseExecute(Context context, TCObject driver, JCharP sqlStr, uint32 sqlLen) {
	...
	if (parser->command == CMD_CREATE_TABLE) {
		...
		if (xstrlen(parser->tableList[0]->tableName) >
			MAX_TABLE_NAME_LENGTH) { // rnovais@112_3 rnovais@570_114: The table name can't be infinite.
			TC_throwExceptionNamed(context, "litebase.SQLParseException",
								   getMessage(ERR_MAX_TABLE_NAME_LENGTH));
			goto error; // juliana@201_25: memory leak.
		}

		xstrcpy(tableName, parser->tableList[0]->tableName);
		if (tableExistsByName(context, driver,
		...
		while (--i) { // Creates the other columns
			...
			if (field->defaultValue) { // Default values: default null has no effect. This is handled by the parser.
				...
				switch (types[i]) {
					...
					case DOUBLE_TYPE:
						defaultValueI->asDouble = TC_str2double(TC_JCharP2CharPBuf(defaultValue, sqlLen, doubleBuf),
																&error);
						break;
				...
				if (error) {
					TC_throwExceptionNamed(context, "litebase.SQLParseException", getMessage(ERR_INVALID_NUMBER),
										   defaultValue, "number");
					goto error;
				}
			}
		...
		if ((numberComposedPKCols = parser->fieldNamesSize) > 0) { // Gets the composed primary keys.
			...
			while (++i < numberComposedPKCols) {
				...
				if (pos == -1) { // Column not found.
					TC_throwExceptionNamed(context, "litebase.SQLParseException", getMessage(ERR_UNKNOWN_COLUMN),
										   fields[i]);
					goto error;
				}
				...
				while (++j < i) // Verifies if there's a duplicate definition.
					if (composedPKCols[j] == pos) {
						TC_throwExceptionNamed(context, "litebase.SQLParseException",
											   getMessage(ERR_DUPLICATED_COLUMN_NAME), fields[i]);
						goto error;
					}
		...
		driverCreateTable(context, driver, tableName, names, hashes, types, sizes, columnAttrs,
						  defaultValues, primaryKeyCol, composedPK, composedPKCols, numberComposedPKCols, count, heap);
		...
		if (table) {
			...
			while (--i >= 0) {
				...
				if (TC_htGet32(&htTable, hashes[i] = TC_hashCode(names[i]))) {
					TC_throwExceptionNamed(context, "litebase.SQLParseException",
										   getMessage(ERR_DUPLICATED_COLUMN_NAME), names[i]);
					goto error;
				}
...
void litebaseExecuteDropTable(Context context, TCObject driver, LitebaseParser* parser) {
	...
	if ((table = (Table*)TC_htGetPtr(htTables,
		...
		while (--count >= 0) { // Erases the table files.
			...
			if (xstrstr(value, fileName) == value || xstrstr(value, fileSimpIdxName) == value
		...
		if (!deleted) { // If there is no file to be erased, an exception must be raised.
			TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_TABLE_NAME_NOT_FOUND),
								   tableName);
			goto finish;
		}
	}
	...
	if (fieldNames[0][0] == '*' && !fieldNames[0][1]) { // Drops all the indices.
		...
		if (fieldNamesSize == 1) { // Simple index.
			if (column < 0) { // Unknown column.
				TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_INVALID_COLUMN_NAME),
									   fieldNames[0]);
				goto finish;
			}
			...
			while (--i >= 0) {
				if ((column = TC_htGet32Inv(&table->htName2index,
											TC_hashCode(fieldNames[i]))) < 0) { // Unknown column.
					TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_INVALID_COLUMN_NAME),
										   fieldNames[i]);
					goto finish;
				}
	...
	switch (parser->command) {
		...
		case CMD_ALTER_ADD_PK: { // ADD PRIMARY KEY
			...
			if (table->primaryKeyCol != NO_PRIMARY_KEY
				|| table->composedPK != NO_PRIMARY_KEY) { // There can't be two primary keys.
				TC_throwExceptionNamed(context, "litebase.AlreadyCreatedException",
			...
			while (++i < size) {
				...
				while (--j >= 0)
					if (composedPKCols[j] == colIndex) {
						TC_throwExceptionNamed(context, "litebase.SQLParseException",
											   getMessage(ERR_DUPLICATED_COLUMN_NAME), fieldNames[i]);
						goto finish;
					}
			...
			if (size == 1) { // Simple primary key.
				...
				if (table->columnIndexes[colIndex]) {
					IntBuf intBuf;
					TC_throwExceptionNamed(context, "litebase.AlreadyCreatedException",
										   getMessage(ERR_INDEX_ALREADY_CREATED), TC_int2str(colIndex, intBuf));
					break;
		...
		case CMD_ALTER_RENAME_TABLE: // RENAME TABLE
			...
			if (!renameTable(context, driver, table,
							 fieldNames[0])) { // juliana@223_14: solved possible memory problems.
		...
		case CMD_ALTER_ADD_COLUMN: {
			...
			if (TC_htGet32Inv(&table->htName2index, hash) >= 0) {
				TC_throwExceptionNamed(context, "litebase.SQLParseException",
									   getMessage(ERR_DUPLICATED_COLUMN_NAME), field->fieldName);
				...
				if (((oldCount + 8) >> 3) >
					bytes) { // Increases the nulls fields if the number of bytes must be increased.
				...
				(table->columnOffsets = newOffsets)[newCount] = (newOffsets[oldCount] +
						typeSizes[field->fieldType]);
				...
				i = -1;
				while (++i < size) {
					if (!readRecord(context, table, record, i, columnNulls, null, 0, false, null,
					...
					while (++j < newCount)
						if (!writeValue(context, &newDB, record[j], &newBuffer[newOffsets[j]], newTypes[j], newSizes[j],
										true, true, (columnNulls[j >> 3] & (1 << (j & 7))), false)) {
							goto free;
						}
				...
				while (--i >= 0) // Recreates the simple indices.
					if (newIndices[i] && (newTypes[i] == CHARS_TYPE || newTypes[i] == CHARS_NOCASE_TYPE)
...
void getByIndex(NMParams p, int32 type) {
	...
	if (testRIClosed(p)) { // The row iterator and its driver can't be closed.
		...
		if (column < 0 || column >= table->columnCount) { // Checks if the column index is within range.
			TC_throwExceptionNamed(context, "java.lang.IllegalArgumentException",
...
bool testRIClosed(NMParams params) {
	...
	if (OBJ_LitebaseDontFinalize(OBJ_RowIteratorDriver(
		...
		TC_throwExceptionNamed(params->currentContext, "java.lang.IllegalStateException",
...
bool bindFunctionDataType(int32 parameterDataType, int32 sqlFunction) { // rnovais@568_10
	...
	if (parameterDataType < 0 || parameterDataType > DATETIME_TYPE || sqlFunction <= FUNCTION_DT_NONE
		|| sqlFunction > FUNCTION_DT_LOWER) {
		return false;
	}
	...
	if (cridObj)
		if (pathObj)
			if (String_charsLen(pathObj) >= MAX_PATHNAME - 4 -
				...
				TC_throwExceptionNamed(params->currentContext, "litebase.DriverException",
				...
				if ((error = TC_listFiles(pathStr, -1, &list, &count, heap,
				...
				while (--i >= 0) {
					...
					if (xstrstr(value, cridStr) == value && xstrstr(value, ".db") && !xstrstr(value, ".dbo")) {
						...
						if ((toEncrypt ? bytes[0] != 0 : (bytes[0] != 1 && bytes[0] != 3)) || bytes[1] != 0
							|| bytes[2] != 0 || bytes[3] != 0) {
							TC_throwExceptionNamed(params->currentContext, "litebase.DriverException",
...
TESTCASE(LibOpen) {
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[4],
								  "Maximum number of parameters in the 'WHERE/HAVING' clause was reached."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[5],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[7],
								  "The maximum number of fields in a SELECT clause was exceeded."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[8],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[12],
	...
	// Primary key errors.
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[16],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[18],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[33],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[37],
								  "It is not possible to open a table within a connection with a different string format."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[38],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[42],
	...
	// Rename errors.
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[45],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[48],
								  "This alias is already being used in this expression: %s."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[49],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[58],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[63],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[68],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[73],
								  "A properly closed table can't be used in recoverTable(): %s.")); // juliana@222_2
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[74], "Can't find index record position on delete."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[75],
								  "The table format (%d) is incompatible with Litebase version. Please update your tables."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[76],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[80],
								  "An opened table can't be recovered or converted: %s."));

	// BLOB errors.
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[81],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[87],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[89],
								  "It is only possible to insert or update a BLOB through prepared statements using setBlob()."));
	...
	// Limit errors.
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[3],
								  "N�mero m�ximo de campos diferentes foi alcan�ado."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[4],
								  "N�mero m�ximo da lista de par�metros na cl�usula 'WHERE/HAVING' foi alcan�ado."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[5],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[7],
								  "O n�mero m�ximo de campos na cl�usula SELECT foi excedido."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[8],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[12],
	...
	// Primary key errors.
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[16],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[29],
								  "ResultSetMetaData n�o pode ser usado depois que o ResultSet estiver fechado."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[33],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[37],
								  "N�o � poss�vel abrir uma tabela com uma conex�o com um tipo de strings diferente."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[38],
								  "N�o � poss�vel abrir uma tabela com uma conex�o com um tipo de criptografia diferente."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[48],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[55],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[58],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[62],
								  "N�o � poss�vel remover uma chave prim�ria usando drop index."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[63],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[66],
								  "Um par�metro em uma where clause n�o pode ser NULL."));

	// Result set errors.
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[67],
								  "ResultSet em uma posi��o de registro inv�lida %d."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[68],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[73],
								  "Uma tabela fechada corretamente n�o pode ser usada no recoverTable(): %s.")); // juliana@222_2
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[76],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[80],
								  "Uma tabela aberta n�o pode ser recuperada ou convertida: %s."));

	// BLOB errors.
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[81],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[83],
								  "Um tipo BLOB n�o pode ser parte de uma chave prim�ria."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[84],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[86],
								  "Um BLOB n�o pode ser convertido em uma string."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[87],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[89],
								  "S� � poss�vel inserir ou atualizar um BLOB atrav�s prepared statements usando setBlob()."));
	...
	ASSERT2_EQUALS(I32, is['_'] & (IS_ALPHA | IS_DIGIT),
...
TESTCASE(initVars) {
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[28],
								  "ResultSetMetaData cannot be used after the ResultSet is closed."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[29],
								  "The application id must be four characters long."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[32],
								  "Prepared statement closed. Please prepare it again."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[36],
								  "It is not possible to open a table within a connection with a different string format."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[37],
								  "It is not possible to open a table within a connection with a different cryptography format."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[41],
								  "Not all parameters of the query had their values defined."));
	...
	// Rename errors.
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[44],
								  "Can't rename table. This table already exists: %s."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[47],
								  "This alias is already being used in this expression: %s."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[48],
								  "An alias is required for the aggregate function column."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[62],
								  "Index too large. It can't have more than 65534 nodes."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[67],
								  "Invalid value for decimal places: %d. It must range from -1 to 40."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[72],
								  "A properly closed table can't be used in recoverTable(): %s.")); // juliana@222_2
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[73], "Can't find index record position on delete."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[74],
								  "The table format (%d) is incompatible with Litebase version. Please update your tables."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[75],
								  "The table format is not the previous one: %s.")); // juliana@220_11
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[79],
								  "An opened table can't be recovered or converted: %s."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[86],
								  "Blobs types can't be in ORDER BY or GROUP BY clauses."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[87], "It is not possible to compare BLOBs."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[88],
								  "It is only possible to insert or update a BLOB through prepared statements using setBlob()."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[28],
								  "ResultSetMetaData n�o pode ser usado depois que o ResultSet estiver fechado."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[32],
								  "Prepared statement fechado. Por favor, prepare-o novamente."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[36],
								  "N�o � poss�vel abrir uma tabela com uma conex�o com um tipo de strings diferente."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[37],
								  "N�o � poss�vel abrir uma tabela com uma conex�o com um tipo de criptografia diferente."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[40],
								  "Comando SQL n�o executa uma atualiza��o no banco de dados."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[41],
								  "Nem todos os par�metros da consulta tiveram seus valores definidos."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[42],
	...
	// Rename errors.
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[44],
								  "N�o � poss�vel renomear a tabela. Esta tabela j� existe: %s."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[47],
								  "Este alias j� est� sendo utilizado no sql: %s."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[48],
								  "Um alias � necess�rio para colunas com fun��o de agrega��o."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[61],
								  "N�o � poss�vel remover uma chave prim�ria usando drop index."));
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[62],
								  "�ndice muito grande. Ele n�o pode ter mais do que 65534 n�s."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[65],
								  "Um par�metro em uma where clause n�o pode ser NULL."));
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[71],
								  "Tabela n�o foi fechada corretamente: %s.")); // juliana@220_2
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[72],
								  "Uma tabela fechada corretamente n�o pode ser usada no recoverTable(): %s.")); // juliana@222_2
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[73],
	...
	ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[79],
								  "Uma tabela aberta n�o pode ser recuperada ou convertida: %s."));
