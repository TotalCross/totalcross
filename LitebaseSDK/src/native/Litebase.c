...
/**
 * A list of objects used to hold prepared statements that uses a specific
 * table.
 */
...
/**
 * Flushs all pending data and destroy all Litebase structures when closing the
 * application.
 */
LB_API void
LibClose()
{
  TRACE("LibClose")
  getMainContextFunc TC_getMainContext =
    TC_getProcAddress(null, "getMainContext");

  TC_htFreeContext(
    TC_getMainContext(),
    &htCreatedDrivers,
    (VisitElementContextFunc)
      freeLitebase);          // Flushs pending data and closes all tables.
...
initVars(OpenParams params)
{
  ...
  if (!(htCreatedDrivers = TC_htNew(10,
                                    null))
         .items // Allocates a hash table for the loaded connections.
      || !(memoryUsage = muNew(100))
            .items     // Allocates a hash table for select statistics.
...
/**
 * Creates a LitebaseConnection for the given creator id and with the given
 * connection param list. This method avoids the creation of more than one
 * instance with the same creator id and parameters, which would lead to
 * performance and memory problems.
 *
 * @param context The thread context where the function is being executed.
 * @param crid The creator id, which may be the same one of the current
 * application and MUST be 4 characters long.
 * @param objParams Only the folder where it is desired to store the tables,
 ...
 * everything to the memory card or to the NVFS volume. <p>Due to the slowness
 * of a memory card and the NVFS volume, all queries will be stored in the main
 * memory; only tables and indexes will be stored on the card or on the NVFS
 * volume. <p> An exception will be raised if tables created with an ascii kind
 * of connection are oppened with an unicode connection and vice-versa.
 * @return A Litebase instance.
 * @throws OutOfMemoryError If memory allocation fails.
 */
TCObject
create(Context context, int32 crid, TCObject objParams)
{
  TRACE("create")
  TCObject driver, logger = litebaseConnectionClass->objStaticValues[1];
  int32 hash;
  bool isAscii = false, useCrypto = false;
  ...
  if (logger) { // juliana@230_30: reduced log files size.
    ...
    if (!TC_appendCharP(context, logSBuffer, "new LitebaseConnection(") ||
    ...
    if (objParams) {
      if (!TC_appendJCharP(context,
                           logSBuffer,
                           String_charsStart(objParams),
                           String_charsLen(objParams))) {
        goto error;
      }
  ...
  error:
    UNLOCKVAR(log);
    if (context->thrownException) { // juliana@223_14: solved possible memory
                                    // problems.
      return null;
    }
  }
  ...
  if (objParams) {
    ...
    // juliana@250_4: now getInstance() can receive only the parameter
    // chars_type = ... juliana@210_2: now Litebase supports tables with ascii
    // strings.
    TC_JCharP2CharPBuf(
      String_charsStart(objParams), String_charsLen(objParams), params);
    tempParams[0] = params;
    ...
    while (--i >= 0) { // The parameters order does not matter.
      ...
      if (xstrstr(tempParams[i], "chars_type")) { // Chars type param.
        ...
        path = TC_CharP2TCHARPBuf(
          tempParams[0],
          sourcePath); // Things do not change if there is only one parameter.
      } else { // juliana@253_11: now a DriverException will be throw if an
               // incorrect parameter is passed in
               // LitebaseConnection.getInstance().
        TC_throwExceptionNamed(context,
                               "litebase.DriverException",
                               getMessage(ERR_INVALID_PARAMETER),
                               tempParams[i]);
        return null;
      }
    }
  ...
  // juliana@221_3: solved a small bug that could make Litebase crash on Windows
  // 32, Windows CE, Palm, iPhone, and Android when passing 2 arguments to
  ...
  if (!(driver = TC_htGetPtr(
          ...
          (hash = TC_hashCodeFmt(
             ...
             isAscii,
             useCrypto,
             sourcePath ? TC_TCHARP2CharPBuf(sourcePath, params) : "null"))))) {
    Hashtable htTables, htPS;
    ...
    // SourcePath.
    if (!(setLitebaseSourcePath(
          driver, xmalloc(sizeof(TCHAR) * (tcslen(sourcePath) + 1))))) {
    error1:
 ...
 *
 * @param context The thread context where the function is being executed.
 * @param driver The driver as an int because this function may be called from a
 * hash table.
 */
void
freeLitebase(Context context, size_t driver)
{
  TRACE("freeLitebase")
  TCHARP sourcePath = getLitebaseSourcePath(driver);
  int32* nodes = getLitebaseNodes(driver); // juliana@253_6: the maximum number
                                           // of keys of a index was duplicated.
  ...
  if (htPs) { // juliana@230_19: removed some possible memory problems with
              // prepared statements and ResultSet.getStrings().
    TC_htFree(htPs, (VisitElementFunc)freePreparedStatement);
    xfree(htPs);
  }

  xfree(sourcePath); // Frees the source path.
  xfree(nodes);      // juliana@253_6: the maximum number of keys of a index was
                     // duplicated.
  TC_htRemove(
    &htCreatedDrivers,
    OBJ_LitebaseKey((TCObject)driver)); // fdie@555_2: removes this instance
                                        // from the drivers hash table.
  OBJ_LitebaseDontFinalize((TCObject)driver) =
    true; // This object shouldn't be finalized again.
}
 ...
 * @throws SQLParseException If the table name or a default string is too big,
 * there is an invalid default value, or an unknown (on a create table) or
 * repeated column name, or an invalid number occurs.
 * @throws AlreadyCreatedException If the table or index is already created.
 * @throws OutOfMemoryError If a memory allocation fails.
 */
void
litebaseExecute(Context context, TCObject driver, JCharP sqlStr, uint32 sqlLen)
{
  ...
  int32* hashes;
  CharP* names;
  Heap heapParser = heapCreate(), heap = null;
  ...
  if (parser->command == CMD_CREATE_TABLE) {
    int32 count = parser->fieldListSize + 1, // fieldListSize + rowid
      ...
      composedPK = NO_PRIMARY_KEY, numberComposedPKCols, type;
    bool error = false;
    uint8* columnAttrs;
    ...
    if (xstrlen(parser->tableList[0]->tableName) >
        MAX_TABLE_NAME_LENGTH) { // rnovais@112_3 rnovais@570_114: The table
                                 // name can't be infinite.
      TC_throwExceptionNamed(context,
    ...
    if (tableExistsByName(
          context,
          driver,
          tableName)) { // guich@105: verifies if it is already created.
      goto error;       // juliana@201_25: memory leak.
    }
    ...
    while (--i) { // Creates the other columns
      ...
      if (field->defaultValue) { // Default values: default null has no effect.
                                 // This is handled by the parser.
        defaultValueI = defaultValues[i] =
          (SQLValue*)TC_heapAlloc(heap, sizeof(SQLValue));
        ...
        if (((type = types[i]) != CHARS_TYPE && type != CHARS_NOCASE_TYPE &&
             sqlLen > 39) ||
            ((type == CHARS_TYPE || type == CHARS_NOCASE_TYPE) &&
             (int32)sqlLen > sizes[i])) {
          TC_throwExceptionNamed(
            context,
            "litebase.SQLParseException",
            getMessage(ERR_LENGTH_DEFAULT_VALUE_IS_BIGGER));
          goto error;
        }
        ...
        switch (types[i]) {
          ...
          case SHORT_TYPE:
            defaultValueI->asShort = str2short(
              TC_JCharP2CharPBuf(defaultValue, sqlLen, doubleBuf), &error);
            break;

          case INT_TYPE:
            defaultValueI->asInt = TC_str2int(
              TC_JCharP2CharPBuf(defaultValue, sqlLen, doubleBuf), &error);
            break;

          case LONG_TYPE:
            defaultValueI->asLong = TC_str2long(
              TC_JCharP2CharPBuf(defaultValue, sqlLen, doubleBuf), &error);
            break;

          case FLOAT_TYPE:
            defaultValueI->asFloat = str2float(
              TC_JCharP2CharPBuf(defaultValue, sqlLen, doubleBuf), &error);
            break;

          case DOUBLE_TYPE:
            defaultValueI->asDouble = TC_str2double(
              TC_JCharP2CharPBuf(defaultValue, sqlLen, doubleBuf), &error);
            break;
          ...
          case DATETIME_TYPE:
            ...
            if (!testAndPrepareDateAndTime(
                  context, defaultValues[i], doubleBuf, type)) {
              goto error;
            }
        }
    ...
    if ((numberComposedPKCols = parser->fieldNamesSize) >
        0) { // Gets the composed primary keys.
      int32 pos = -1, j, hashCol;
      ...
      while (++i < numberComposedPKCols) {
        ...
        if (types[pos] == BLOB_TYPE) { // A blob can't be in a composed PK.
          TC_throwExceptionNamed(context,
                                 "litebase.SQLParseException",
                                 getMessage(ERR_BLOB_PRIMARY_KEY));
          goto error;
        }
        j = -1;
        while (++j < i) // Verifies if there's a duplicate definition.
          if (composedPKCols[j] == pos) {
            TC_throwExceptionNamed(context,
                                   "litebase.SQLParseException",
                                   getMessage(ERR_DUPLICATED_COLUMN_NAME),
                                   fields[i]);
            goto error;
          }
    ...
    xstrcpy(tableName,
            parser->tableList[0]
              ->tableName); // indexTableName ignored - formed internally.
    table = getTable(context, driver, tableName);

    if (table) {
      Hashtable htTable =
        TC_htNew((i = parser->fieldNamesSize) + 1, heapParser);
      ...
      while (--i >= 0) {
        // juliana@225_8: it was possible to create a composed index with
      ...
      driverCreateIndex(
        context, table, hashes, 0, parser->fieldNamesSize, null);
    }
  } else {
    TC_throwExceptionNamed(context,
                           "litebase.SQLParseException",
                           getMessage(ERR_ONLY_CREATE_TABLE_INDEX_IS_ALLOWED));
  }
  heapDestroy(heapParser);
}
 ...
 * @param sqlStr The SQL creation command.
 * @param sqlLen The SQL string length.
 * @return The number of rows affected or <code>0</code> if a drop or alter
 * operation was successful.
 * @throws OutOfMemoryError If a memory allocation fails.
 */
int32
litebaseExecuteUpdate(Context context,
                      TCObject driver,
                      JCharP sqlStr,
                      int32 sqlLen)
{
  ...
  switch (parser->command) {
    ...
    case CMD_INSERT: {
      SQLInsertStatement* insertStmt =
        initSQLInsertStatement(context, driver, parser);
    ...
    case CMD_UPDATE: {
      SQLUpdateStatement* updateStmt =
        initSQLUpdateStatement(context, driver, parser, false);
 ...
 * @param sqlStr The SQL creation command.
 * @param sqlLen The SQL string length.
 * @return A result set with the values returned from the query or
 * <code>null</code> if an error occurs.
 * @throws OutOfMemoryError If a memory allocation fails.
 */
TCObject
litebaseExecuteQuery(Context context,
                     TCObject driver,
                     JCharP strSql,
                     int32 length)
{
  ...
  // juliana@223_9: improved Litebase temporary table allocation on Windows 32,
  // Windows CE, Palm, iPhone, and Android.
  locked = true;
  LOCKVAR(parser);
 ...
 * @throws DriverException If the table does not exist, if its name is greater
 * than the maximum possible or it is not possible to remove it.
 */
void
litebaseExecuteDropTable(Context context,
                         TCObject driver,
                         LitebaseParser* parser)
{
  ...
  if (xstrlen(tableName) >
      23) { // The table name has a maximum length because of palm os.
    TC_throwExceptionNamed(context,
                           "litebase.DriverException",
                           getMessage(ERR_MAX_TABLE_NAME_LENGTH));
    goto finish;
  ...
  if ((table = (Table*)TC_htGetPtr(
         htTables,
         hashCode = TC_hashCode(tableName)))) { // The table is open.
    ...
    if ((i = table->numberComposedIndexes - 1) >= 0)
      while (--i >= 0) { // Drops its composed indices.
        if ((index = composedIndexes[i]->index) &&
            !indexRemove(context, index)) {
          goto finish;
        }
    ...
    int32 deleted = 0, count = 0;
    char fileName[DBNAME_SIZE], fileSimpIdxName[DBNAME_SIZE],
    ...
    // juliana@220_12: drop table was dropping a closed table and all tables
    // starting with the same name of the dropped one.
    ...
    if ((i = TC_listFiles(sourcePath,
                          -1,
                          &list,
                          &count,
                          heap,
                          0))) { // Lists all the path files.
      fileError(context, i, "");
      goto finish;
    }
    ...
    while (--count >= 0) { // Erases the table files.
      ...
      if (xstrstr(value, fileName) == value ||
          xstrstr(value, fileSimpIdxName) == value ||
          xstrstr(value, fileCompIdxName) == value) {
    ...
    if (!deleted) { // If there is no file to be erased, an exception must be
                    // raised.
      TC_throwExceptionNamed(context,
                             "litebase.DriverException",
                             getMessage(ERR_TABLE_NAME_NOT_FOUND),
                             tableName);
      goto finish;
    }
  }
 ...
 * @param driver The current Litebase connection.
 * @param parser The parser.
 * @return <code>-1</code> if an error occured; the number of indices removed,
 * otherwise.
 * @throws DriverException If a column does not have an index, is invalid, or if
 * the columns to have the index dropped are from a primary key.
 */
int32
litebaseExecuteDropIndex(Context context,
                         TCObject driver,
                         LitebaseParser* parser)
{
  ...
  // juliana@226_4: now a table won't be marked as not closed properly if the
  // application stops suddenly and the table was not modified since its last
  // opening.
  ...
  if (fieldNames[0][0] == '*' && !fieldNames[0][1]) { // Drops all the indices.
    ...
    int32 column =
            TC_htGet32Inv(&table->htName2index, TC_hashCode(fieldNames[0])),
    ...
    if (fieldNamesSize == 1) { // Simple index.
      if (column < 0) {        // Unknown column.
        TC_throwExceptionNamed(context,
                               "litebase.DriverException",
                               getMessage(ERR_INVALID_COLUMN_NAME),
                               fieldNames[0]);
        goto finish;
      }
      if (column ==
          table->primaryKeyCol) { // Can't use drop index to drop a primary key.
        TC_throwExceptionNamed(context,
                               "litebase.DriverException",
                               getMessage(ERR_DROP_PRIMARY_KEY));
        goto finish;
      }
      ...
      i = fieldNamesSize;
      while (--i >= 0) {
        if ((column = TC_htGet32Inv(&table->htName2index,
                                    TC_hashCode(fieldNames[i]))) <
            0) { // Unknown column.
          TC_throwExceptionNamed(context,
                                 "litebase.DriverException",
                                 getMessage(ERR_INVALID_COLUMN_NAME),
      ...
      driverDropComposedIndex(
        context, table, columns, fieldNamesSize, -1, true);
      count = 1;
    }
  }
 ...
 * @param driver The current Litebase connection.
 * @param parser The parser.
 * @throws DriverException If there is no primary key to be dropped, or an
 * invalid column name.
 * @throws AlreadyCreatedException If one tries to add another primary key, or a
 * simple primary key is added to a column that already has an index.
 * @throws SQLParseException If there is a blob in a primary key definition or
 * there is a duplicated column name in the primary key definition.
 * @throws OutOfMemoryError If a memory allocation fails.
 */
void
litebaseExecuteAlter(Context context, TCObject driver, LitebaseParser* parser)
{
  TRACE("litebaseExecuteAlter")
  Table* table = getTable(context, driver, parser->tableList[0]->tableName);
  Heap heap, parserHeap = parser->heap;
  CharP* fieldNames = parser->fieldNames;
  int32 i;
  ...
  switch (parser->command) {
    case CMD_ALTER_DROP_PK:                         // DROP PRIMARY KEY
      if (table->primaryKeyCol != NO_PRIMARY_KEY) { // Simple primary key.
        ...
        // juliana@230_17: solved a possible crash or exception if the table is
        // not closed properly after dropping a composed primary key. The meta
        // data is saved.
        int32 number = table->numberComposedPKCols;
        table->numberComposedPKCols = 0;
        table->composedPK = NO_PRIMARY_KEY;
        driverDropComposedIndex(
          context, table, table->composedPrimaryKeyCols, number, -1, true);
      } else { // There's no primary key.
        TC_throwExceptionNamed(context,
                               "litebase.DriverException",
                               getMessage(ERR_TABLE_DOESNOT_HAVE_PRIMARY_KEY));
      }
      break;

    case CMD_ALTER_ADD_PK: { // ADD PRIMARY KEY
      int32 size = parser->fieldNamesSize, j, colIndex = -1;
      ...
      if (table->primaryKeyCol != NO_PRIMARY_KEY ||
          table->composedPK !=
            NO_PRIMARY_KEY) { // There can't be two primary keys.
        TC_throwExceptionNamed(context,
                               "litebase.AlreadyCreatedException",
                               getMessage(ERR_PRIMARY_KEY_ALREADY_DEFINED));
        break;
      }

      i = -1;
      while (++i < size) {
        if ((colIndex = TC_htGet32Inv(
               htName2index, hashCols[i] = TC_hashCode(fieldNames[i]))) < 0) {
          TC_throwExceptionNamed(context,
                                 "litebase.DriverException",
                                 getMessage(ERR_INVALID_COLUMN_NAME),
                                 fieldNames[i]);
        ...
        if (types[colIndex] == BLOB_TYPE) {
          TC_throwExceptionNamed(
            context, "litebase.SQLParseException", getMessage(ERR_BLOB_INDEX));
          goto finish;
        }
      }

      if (size == 1) { // Simple primary key.
        // juliana@230_41: an AlreadyCreatedException is now thrown when trying
        // to add a primary key for a column that already has a simple index.
        if (table->columnIndexes[colIndex]) {
          IntBuf intBuf;
          TC_throwExceptionNamed(context,
                                 "litebase.AlreadyCreatedException",
                                 getMessage(ERR_INDEX_ALREADY_CREATED),
                                 TC_int2str(colIndex, intBuf));
          break;
        ...
        // juliana@202_20: Corrected a bug that would not create the composed PK
        ...
        if (!driverCreateIndex(
              context, table, hashCols, 1, size, composedPKCols)) {
          table->composedPK = -1;
          break;
        }
      }
    ...
    case CMD_ALTER_RENAME_TABLE: // RENAME TABLE
      if (tableExistsByName(
            context,
            driver,
            fieldNames[0])) { // The new table name is stored in the field list.
        break;
      }
      if (!renameTable(context,
                       driver,
                       table,
                       fieldNames[0])) { // juliana@223_14: solved possible
                                         // memory problems.
    ...
    case CMD_ALTER_RENAME_COLUMN: { // RENAME COLUMN
      CharP oldColumn = fieldNames[1], newColumn;
      ...
      if (reuseSpace) { // If the new column name length is smaller than the
                        // previous one, reuses its space.
    ...
    case CMD_ALTER_ADD_COLUMN: {
      ...
      int32 oldCount = table->columnCount, newCount = oldCount + 1,
            bytes = (oldCount + 7) >> 3, hash = TC_hashCode(field->fieldName);
      ...
      if (TC_htGet32Inv(&table->htName2index, hash) >= 0) {
        ...
        TC_throwExceptionNamed(context,
                               "litebase.SQLParseException",
                               getMessage(ERR_MAX_NUM_FIELDS_REACHED));
      } else {
        ...
        SQLValue** newDefaultValues =
          (SQLValue**)TC_heapAlloc(heap, newCount * TSIZE);
        SQLValue** record =
          (SQLValue**)TC_heapAlloc(parserHeap, newCount * TSIZE);
        ...
        int32 crc32, j, k, size, length, blobLength;

        if (((oldCount + 8) >> 3) >
            bytes) { // Increases the nulls fields if the
                     // number of bytes must be increased.
        ...
        (table->columnAttrs = newAttrs)[oldCount] =
          (field->defaultValue ? ATTR_COLUMN_HAS_DEFAULT : 0) |
          (field->isNotNull ? ATTR_COLUMN_IS_NOT_NULL : 0);

        // Column hashes.
        xmemmove(newHashes, table->columnHashes, oldCount << 2);
        TC_htPut32(&table->htName2index,
                   (table->columnHashes = newHashes)[oldCount] = hash,
                   oldCount);

        // Column offsets.
        xmemmove(newOffsets, table->columnOffsets, newCount << 1);
        (table->columnOffsets = newOffsets)[newCount] =
          (newOffsets[oldCount] + typeSizes[field->fieldType]);
        ...
        (table->columnNames = newNames)[oldCount] =
          TC_heapAlloc(heap, i = (xstrlen(field->fieldName) + 1));
        ...
        if (defaultValue) { // Sets the new default value if it exists.
          ...
          newDefaultValue = newDefaultValues[oldCount] =
            (SQLValue*)TC_heapAlloc(heap, sizeof(SQLValue));
          ...
          // juliana@222_9: Some string conversions to numerical values could
          // return spourious values if the string range were greater than the
          // type range.
          switch (field->fieldType) {
            case CHARS_TYPE:
            case CHARS_NOCASE_TYPE:
              newDefaultValue->asChars =
                (JCharP)TC_heapAlloc(heap, (i + 1) << 1);
              xmemmove(newDefaultValue->asChars,
                       defaultValue,
                       (newDefaultValue->length = i) << 1);
              break;
            case SHORT_TYPE:
              newDefaultValue->asShort = str2short(
                TC_JCharP2CharPBuf(defaultValue, i, doubleBuf), &error);
              break;
            case INT_TYPE:
              newDefaultValue->asInt = TC_str2int(
                TC_JCharP2CharPBuf(defaultValue, i, doubleBuf), &error);
              break;
            case LONG_TYPE:
              newDefaultValue->asLong = TC_str2long(
                TC_JCharP2CharPBuf(defaultValue, i, doubleBuf), &error);
              break;
            case FLOAT_TYPE:
              newDefaultValue->asFloat = str2float(
                TC_JCharP2CharPBuf(defaultValue, i, doubleBuf), &error);
              break;
            case DOUBLE_TYPE:
              newDefaultValue->asDouble = TC_str2double(
                TC_JCharP2CharPBuf(defaultValue, i, doubleBuf), &error);
              break;
            case DATE_TYPE:
            case DATETIME_TYPE:
              ...
              if (!testAndPrepareDateAndTime(
                    context, newDefaultValue, doubleBuf, type)) {
                goto finish;
              }
          }
        }
        ...
        if (!(createPlainDB(
              context, &newDB, tempName, true, useCrypto, table->sourcePath))) {
          goto finish;
        }
        newDB.isAscii = oldDB.isAscii; // juliana@210_2: now Litebase supports
                                       // tables with ascii strings.
        newDB.headerSize = oldDB.headerSize;
        newBuffer = newDB.basbuf = TC_heapAlloc(
          heap, i = newOffsets[newCount] + ((newCount + 7) >> 3) + 4);
        plainSetRowSize(&newDB, i, newBuffer);
        newDB.rowInc = oldDB.rowCount;

        record[oldCount] =
          newDefaultValue; // Sets the record for the new column.
        ...
        while (--i >= 0) {
          ...
          if (newTypes[i] == CHARS_TYPE || newTypes[i] == CHARS_NOCASE_TYPE) {
            record[i]->asChars =
              (JCharP)TC_heapAlloc(parserHeap, (newSizes[i] << 1) + 2);
        ...
        while (++i < size) {
          if (!readRecord(context,
                          ...
                          false,
                          null,
                          null)) { // juliana@220_3 juliana@227_20
          free:
          ...
          if (isNull) {
            columnNulls[oldCount >> 3] |=
              (1 << (oldCount & 7)); // Sets the column as null.
          }
          ...
          while (++j < newCount)
            if (!writeValue(context,
                            &newDB,
                            record[j],
                            &newBuffer[newOffsets[j]],
                            newTypes[j],
                            newSizes[j],
                            true,
                            true,
                            (columnNulls[j >> 3] & (1 << (j & 7))),
                            false)) {
              goto free;
            }
          ...
          // consideration. juliana@223_8: corrected a bug on purge that would
          // not copy the crc32 codes for the rows. juliana@220_4: added a crc32
          // code for every record. Please update your tables.
          k = newBuffer[3];
          newBuffer[3] = 0; // juliana@222_5: The crc was not being calculated
                            // correctly for updates.
          newDB.useOldCrypto = false;

          // Computes the crc for the record and stores at the end of the
          // record.
          crc32 = updateCRC32(newBuffer, length, 0);
          if (table->version == VERSION_TABLE) {
            j = newCount;
            while (--j > 0)
              if ((newTypes[j] == CHARS_TYPE ||
                   newTypes[j] == CHARS_NOCASE_TYPE) &&
                  (columnNulls[j >> 3] & (1 << (j & 7))) == 0) {
                crc32 = updateCRC32(
                  (uint8*)record[j]->asChars, record[j]->length << 1, crc32);
              } else if (newTypes[j] == BLOB_TYPE &&
                         (columnNulls[j >> 3] & (1 << (j & 7))) == 0) {
                blobLength = record[j]->length;
                crc32 = updateCRC32((uint8*)&blobLength, 4, crc32);
              }
          }
          xmove4(&newBuffer[length],
        ...
        if (!plainRemove(context, &oldDB, table->sourcePath) ||
            !plainRename(context, &newDB, table->name, table->sourcePath)) {
          goto finish;
        }
        ...
        while (--i >= 0) // Recreates the simple indices.
          if (newIndices[i] &&
              (newTypes[i] == CHARS_TYPE || newTypes[i] == CHARS_NOCASE_TYPE) &&
              !tableReIndex(context, table, i, false, null)) {
            goto finish;
          }

        if ((i = table->numberComposedIndexes) >
            0) // Recreates the composed indices.
          while (--i >= 0)
            if (!tableReIndex(
                  context, table, -1, false, table->composedIndexes[i])) {
              goto finish;
            }
      }
    }
 ...
 */
void
getByIndex(NMParams p, int32 type)
{
  TRACE("getByIndex")
  ...
  if (testRIClosed(p)) { // The row iterator and its driver can't be closed.
    ...
    // If the column type is char nocase, the column is compatible with the char
    // type.
    ...
    switch (type) {
      case SHORT_TYPE: // juliana@227_18: corrected a possible insertion of a
                       // negative short column being recovered in the select as
                       // positive.
        p->retI = 0;
      ...
      case CHARS_NOCASE_TYPE: {
        ...
        xmove4(&i,
               &data[table->columnOffsets[column]]); // Finds the string
                                                     // position in the .dbo.
 ...
 */
bool
testRIClosed(NMParams params)
{
  TRACE("testRIClosed")
  TCObject rowIterator = params->obj[0];

  if (OBJ_LitebaseDontFinalize(OBJ_RowIteratorDriver(
        rowIterator))) { // The connection with Litebase can't be closed.
    TC_throwExceptionNamed(params->currentContext,
  ...
  if (!(getRowIteratorTable(rowIterator))) { // Row iterator closed.
    TC_throwExceptionNamed(params->currentContext,
                           "java.lang.IllegalStateException",
                           getMessage(ERR_ROWITERATOR_CLOSED));
    return false;
  }
  return true;
}
...
/**
 * Checks if the path passed as a parameter is valid and uses an internal path
 * if it is null.
 *
 * @param context The thread context where the function is being executed.
 * @param sourcePath Receives the path that Litebase will use to store and
 * access tables.
 ...
 */
bool
checkApppath(Context context, TCHARP sourcePath, TCHARP pathParam)
{ // juliana@214_1
  TRACE("checkApppath")
  TCHAR buffer[MAX_PATHNAME];
  int32 endCh, lenAppPath, ret = 0;
  ...
  if (pathParam) {
    ...
    // tcscpy(sourcePath, tstrTrim(pathParam)); - guich: was crashing on
    // LaudoMovel
    {
      TCHARP src = tstrTrim(pathParam);
      xmemmove(sourcePath, src, sizeof(TCHAR) * (tcslen(src) + 1));
    }
  ...
  } else { // Since no path was passed by the user, gets it from dataPath or
           // appPath.
  ...
  if (!sourcePath[0] || (sourcePath[0] && !lbfileExists(sourcePath) &&
                         (ret = lbfileCreateDir(sourcePath)))) {
    char pathCharP[1024];
 ...
 * @return <code>true</code> If the function can be applied to a data type
 * field; <code>false</code>, otherwise.
 */
bool
bindFunctionDataType(int32 parameterDataType, int32 sqlFunction)
{ // rnovais@568_10
  ...
  if (parameterDataType < 0 || parameterDataType > DATETIME_TYPE ||
      sqlFunction <= FUNCTION_DT_NONE || sqlFunction > FUNCTION_DT_LOWER) {
    return false;
  }
 ...
 */
CharP
dataTypeFunctionsName(int32 sqlFunction)
{ // rnovais@568_10
...
/**
 * Checks if the driver is opened and another parameter is not null when they
 * are sent as parameters in some native methods.
 *
 ...
 */
bool
checkParamAndDriver(NMParams params, CharP parameter)
{
...
/**
 * Encrypts or decrypts all the tables of a connection given from the
 * application id.
 *
 * @param p->obj[0] The application id of the database.
 * @param p->obj[1] The path where the files are stored.
 * @throws DriverException If a file error occurs, not all the tables use the
 * desired cryptography format.
 * @throws NullPointerException If one of the string parameters is null.
 */
void
encDecTables(NMParams params, bool toEncrypt)
{
  TRACE("encDecTables")
  TCObject cridObj = params->obj[0], pathObj = params->obj[1];
  ...
  if (cridObj)
    if (pathObj)
      if (String_charsLen(pathObj) >=
        ...
        char cridStr[5], value[DBNAME_SIZE];
        int32 i, j, k, error, count = 0, encByte = toEncrypt ? 0 : 1;
        ...
        {
          TC_throwExceptionNamed(
            params->currentContext, "java.lang.OutOfMemoryError", null);
        ...
        TC_JCharP2CharPBuf(
          String_charsStart(cridObj), String_charsLen(cridObj), cridStr);
        TC_JCharP2TCHARPBuf(
          String_charsStart(pathObj), String_charsLen(pathObj), pathStr);

        if ((error = TC_listFiles(pathStr,
                                  -1,
                                  &list,
                                  &count,
                                  heap,
        ...
        // Before doing anything, checks if all the .db files are marked as
        // decrypted or encrypted.
        i = count;
        while (--i >= 0) {
          TC_TCHARP2CharPBuf(list->value, value);
          if (xstrstr(value, cridStr) == value && xstrstr(value, ".db") &&
              !xstrstr(value, ".dbo")) {
            ...
            if ((error = lbfileReadBytes(file, (CharP)bytes, 0, 4, &j)) ||
                (error = lbfileClose(&file))) {
              goto fileError;
            }
            if ((toEncrypt ? bytes[0] != 0
                           : (bytes[0] != 1 && bytes[0] != 3)) ||
                bytes[1] != 0 || bytes[2] != 0 || bytes[3] != 0) {
              TC_throwExceptionNamed(params->currentContext,
                                     "litebase.DriverException",
                                     getMessage(ERR_WRONG_CRYPTO_FORMAT));
              goto error;
            }
          }
        ...
        while (--i >= 0) {
          ...
          if (xstrstr(value, cridStr) == value) {
            ...
            count = 0;
            if (xstrstr(value,
                        ".db")) { // Changes the .db file crypto information.
              if ((error = lbfileReadBytes(file, (CharP)bytes, 0, 4, &j))) {
                goto fileError;
              }
              bytes[0] = encByte;
              if ((error = lbfileSetPos(file, 0)) ||
                  (error = lbfileWriteBytes(file, (CharP)bytes, 0, 4, &j))) {
                goto fileError;
              }
              count = 4;
            }
            ...
            while (!(error = lbfileReadBytes(
                       file, (CharP)bytes, 0, CACHE_INITIAL_SIZE, &j)) &&
                   j) {
              k = j;
              while (--k >= 0) {
                bytes[k] ^= 0xAA;
              ...
              if ((error = lbfileSetPos(file, count)) ||
                  (error = lbfileWriteBytes(file, (CharP)bytes, 0, j, &j))) {
                goto fileError;
              }
              count += j;
            }
...
TESTCASE(LibOpen)
{
  ...
  // Limit errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[3],
                         "Maximum number of different fields was reached."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[4],
                         "Maximum number of parameters in the "
                         "'WHERE/HAVING' clause was reached."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[5],
                         "Maximum number of composed indices 32 was reached."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[6], "Table name too big: must be <= 23."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[7],
            "The maximum number of fields in a SELECT clause was exceeded."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[8],
                         "Maximum number of columns exceeded in "
  ...
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[11], "Invalid column number: %d."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[12],
                         "The following column(s) does (do) "
                         "not have an associated index %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[13], "Column name in field list is ambiguous: %s."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[14], "Column not found %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[15], "Duplicated column name: %s."));

  // Primary key errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[16],
                         "A primary key was already defined for this table."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[17], "Table does not have a primary key."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[18],
                         "Statement creates a duplicated primary key in %s."));

  // Type errors.
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[19], "Incompatible types."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[20], "Field size must be a positive interger value."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[21],
            "Value %s is not a valid number for the desired type: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[22],
                         "Incompatible data type for the function call: %s"));

  // Number of fields errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[23],
            "The number of fields does not match the number of values "));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[24],
                         "The given number of values does not "
                         "match the table definition %d."));

  // Default value errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[25],
            "Length of default value is bigger than column size."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[26],
                         "An added column declared as NOT NULL "
                         "must have a not null default value."));

  // Driver errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[27],
                         "This driver instance was closed and cannot be used "
                         "anymore. Please get a new instance of it."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[28], "ResultSet already closed!"));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[29],
            "ResultSetMetaData cannot be used after the ResultSet is closed."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[30],
                         "The application id must be four characters long."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[31], "The increment must be greater than 0 or -1."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[32], "Iterator already closed."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[33],
            "Prepared statement closed. Please prepare it again."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[34], "Invalid connection parameter: %s."));

  // Table errors.
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[35], "Table name not found: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[36], "Table already created: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[37],
                         "It is not possible to open a table within a "
                         "connection with a different string format."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[38],
                         "It is not possible to open a table within a "
                         "connection with a different cryptography format."));

  // ROWID errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[39], "ROWID can't be changed by the user!"));

  // Prepared Statement errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[40], "Query does not return result set."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[41],
                         "Query does not perform updates in the database."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[42],
            "Not all parameters of the query had their values defined."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[43], "A value was not defined for the parameter %d."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[44], "Invalid parameter index."));

  // Rename errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[45],
                         "Can't rename table. This table already exists: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[46], "Column already exists: %s."));

  // Alias errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[47], "Not unique table/alias: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[48],
            "This alias is already being used in this expression: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[49],
            "An alias is required for the aggregate function column."));

  // Litebase.execute() error.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[50],
                         "Only CREATE TABLE and CREATE INDEX can "
                         "be used in Litebase.execute()."));

  // Order by and group by errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[51], "ORDER BY and GROUP BY clauses must match."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[52],
                         "No support for virtual columns in SQL "
                         "queries with GROUP BY clause."));

  // Function errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[53],
                         "All non-aggregation function columns in the SELECT "
                         "clause must also be in the GROUP BY clause."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[54],
                         "%s is not an aggregation function. All fields "
                         "present in a HAVING clause must be listed in the "
                         "SELECT clause as aliased aggregation functions."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[55],
                         "Can't mix aggregation functions with real columns in "
                         "the SELECT clause without a GROUP BY clause."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[56],
                         "Can't have aggregation functions with ORDER "
                         "BY clause and no GROUP BY clause."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[57],
                         "%s was not listed in the SELECT clause. All fields "
                         "present in a HAVING clause must be listed in the "
                         "SELECT clause as aliased aggregation funtions."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[58],
                         "SUM and AVG aggregation functions are not "
                         "used with DATE and DATETIME type fields."));

  // DATE and DATETIME errors.
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[59], "Value is not a DATE: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[60], "Value is not a DATETIME: %s."));

  // Index error.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[61], "Index already created for column %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[62],
                         "Can't drop a primary key index withdrop index."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[63],
            "Index too large. It can't have more than 65534 nodes."));

  // NOT NULL errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[64], "Primary key can't have null."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[65], "Field can't be null: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[66], "A parameter in a where clause can't be null."));

  // Result set errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[67], "ResultSet in invalid record position: %d."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[68],
                         "Invalid value for decimal places: "
                         "%d. It must range from -1 to 40."));
  ...
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[72],
                         "Table not closed properly: %s.")); // juliana@220_2
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[73],
                         "A properly closed table can't be used in "
                         "recoverTable(): %s.")); // juliana@222_2
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[74], "Can't find index record position on delete."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[75],
                         "The table format (%d) is incompatible with Litebase "
                         "version. Please update your tables."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[76],
            "The table format is not the previous one: %s.")); // juliana@220_11
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[77], "Invalid path: %s.")); // juliana@214_1
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[78], "Invalid file position: %d."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[79], "Database not found."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[80],
            "An opened table can't be recovered or converted: %s."));

  // BLOB errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[81],
            "The total size of a blob can't be greater then 10 Mb."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[82], "This is not a valid size multiplier."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[83], "A blob type can't be part of a primary key."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[84], "A BLOB column can't be indexed."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[85], "A BLOB can't be in the where clause."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[86], "A BLOB can't be converted to a string."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[87],
            "Blobs types can't be in ORDER BY or GROUP BY clauses."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[88], "It is not possible to compare BLOBs."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[89],
                         "It is only possible to insert or update a BLOB "
                         "through prepared statements using setBlob()."));
  ...
  // Limit errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[3],
                         "N�mero m�ximo de campos diferentes foi alcan�ado."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[4],
                         "N�mero m�ximo da lista de par�metros na "
                         "cl�usula 'WHERE/HAVING' foi alcan�ado."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[5],
            "Numero m�ximo de �ndices compostos 32 foi alcan�ado."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[6], "Nome da tabela muito grande: deve ser <= 23"));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[7],
            "O n�mero m�ximo de campos na cl�usula SELECT foi excedido."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[8],
                         "O n�mero m�ximo de campos na cl�usula "
                         "'ORDER BY/GROUP BY' foi excedido."));

  // Column errors.
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[9], "Coluna desconhecida %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[10], "Nome de coluna inv�lido: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[11], "N�mero de coluna inv�lido: %d."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[12],
            "A(s) coluna(s) a seguir n�o tem (t�m) um ind�ce associado %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[13], "Nome de coluna amb�guo: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[14], "Coluna n�o encontrada: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[15], "Nome de coluna duplicado: %s."));

  // Primary key errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[16],
            "Uma chave prim�ria j� foi definida para esta tabela."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[17], "Tabela n�o tem chave prim�ria."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[18],
                         "Comando cria uma chave prim�ria duplicada em %s."));

  // Type errors.
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[19], "Tipos incompativeis."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[20],
            "Tamanho do campo deve ser um valor inteiro positivo."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[21],
            "O valor %s n�o � um n�mero v�lido para o tipo desejado: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[22],
            "Tipo de dados incompat�vel para a chamada de fun��o: %s"));

  // Number of fields errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[23],
            "O n�mero de campos � diferente do n�mero de valores "));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[24],
                         "O n�mero de valores dado n�o coincide "
                         "com a defini��o da tabela %d."));

  // Default value errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[25],
                         "Tamanho do valor padr�o � maior que o "
                         "tamanho definido para a coluna."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[26],
                         "Uma coluna adicionada declarada como NOT NULL "
                         "deve ter um valor padr�o n�o nulo."));

  // Driver errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[27],
            "Esta inst�ncia do driver est� fechada e n�o pode ser "
            "mais utilizada. Por favor, obtenha uma nova inst�ncia."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[28], "ResultSet j� est� fechado!"));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[29],
                         "ResultSetMetaData n�o pode ser usado depois "
                         "que o ResultSet estiver fechado."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[30], "O id da aplica��o de ter 4 characteres."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[31], "O incremento deve ser maior do que 0 ou -1."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[32], "Iterador j� foi fechado."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[33],
            "Prepared statement fechado. Por favor, prepare-o novamente."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[34], "Par�metro de conex�o inv�lido: %s."));

  // Table errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[35], "Nome da tabela n�o encontrado: %s."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[36], "Tabela j� existe: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[37],
                         "N�o � poss�vel abrir uma tabela com uma "
                         "conex�o com um tipo de strings diferente."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[38],
                         "N�o � poss�vel abrir uma tabela com uma conex�o com "
                         "um tipo de criptografia diferente."));

  // ROWID errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[39], "ROWID n�o pode ser mudado pelo usu�rio!"));

  // Prepared Statement errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[40], "Comando SQL n�o retorna um ResultSet."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[41],
            "Comando SQL n�o executa uma atualiza��o no banco de dados."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[42],
                         "Nem todos os par�metros da consulta "
                         "tiveram seus valores definidos."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[43],
                         "N�o foi definido um valor para o par�metro %d."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[44], "Invalid parameter index."));

  // Rename errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[45],
            "N�o � poss�vel renomear a tabela. Esta tabela j� existe: %s."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[46], "Coluna j� existe: %s."));

  // Alias errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[47], "Nome de tabela/alias repetido: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[48],
                         "Este alias j� est� sendo utilizado no sql: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[49],
            "Um alias � necess�rio para colunas com fun��o de agrega��o."));

  // Litebase.execute() error.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[50],
                         "Apenas CREATE TABLE e CREATE INDEX s�o "
                         "permitidos no Litebase.execute()"));

  // Order by and group by errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[51],
                         "Cl�usulas ORDER BY e GROUP BY devem coincidir."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(
      errorMsgs_pt[52],
      "SQL com cl�usula GROUP BY n�o tem suporte para colunas virtuais."));

  // Function errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[53],
                         "Todas colunas que n�os�o fun��es de agrega��o na "
                         "cl�usula SELECT devem estar na cl�usula GROUP BY."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[54],
            "%s n�o � uma fun��o de agrega��o. Todos as colunas da cl�usula "
            "HAVING devem ser listadas no SELECT utilizando alias."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[55],
                         "N�o � possivel misturar colunas reais e de "
                         "agrega��o no SELECT sem cl�usula GROUP BY."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[56],
                         "N�o � poss�vel ter fun��es de agrega��o com "
                         "cl�usula ORDER BY sem cl�usula GROUP BY."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[57],
            "%s n�o foi listado no SELECT. Todas as colunas da cl�usula "
            "HAVING devem ser listadas no SELECT utilizando alias."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[58],
                         "Fun��es de agrega��o SUM e AVG n�o s�o usadas "
                         "com colunas do tipo DATE e DATETIME."));

  // DATE and DATETIME errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[59], "Valor n�o � um tipo DATE v�lido: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[60], "Valor n�o � um tipo DATETIME v�lido: %s."));

  // Index error.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[61], "�ndice j� criado para a coluna %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[62],
            "N�o � poss�vel remover uma chave prim�ria usando drop index."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[63],
            "�ndice muito grande. Ele n�o pode ter mais do que 65534 n�s."));

  // NOT NULL errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[64], "Chave prim�ria n�o pode ter NULL."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[65], "Coluna n�o pode ser NULL: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[66],
            "Um par�metro em uma where clause n�o pode ser NULL."));

  // Result set errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[67],
                         "ResultSet em uma posi��o de registro inv�lida %d."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[68],
                         "Valor inv�lido para casas decimais: "
                         "%d. Deve ficar entre - 1 e 40."));

  // File errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[69], "N�o � poss�vel ler da tabela %s."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[70], "N�o � poss�vel carregar n� folha!"));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[71], "Tabela est� corrompida: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[72],
            "Tabela n�o foi fechada corretamente: %s.")); // juliana@220_2
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[73],
                         "Uma tabela fechada corretamente n�o pode ser usada "
                         "no recoverTable(): %s.")); // juliana@222_2
  ASSERT1_EQUALS(
    False,
    xstrcmp(
      errorMsgs_pt[74],
      "N�o � poss�vel achar a posi��o de registro no �ndice na exclus�o."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[75],
            "O formato de tabela (%d) n�o � compat�vel com a vers�o "
            "do Litebase. Por favor, atualize suas tabelas."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[76],
            "O formato de tabela n�o � o anterior: %s.")); // juliana@220_11
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[77],
                         "Caminho inv�lido: %s.")); // juliana@214_1
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[78], "Posi��o inv�lida no arquivo: %d."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[79], "Base de dados n�o encontrada."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[80],
            "Uma tabela aberta n�o pode ser recuperada ou convertida: %s."));

  // BLOB errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[81],
            "O tamanho total de um BLOB n�o pode ser maior do que 10 Mb."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[82], "O multiplicador de tamanho n�o � v�lido."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[83],
            "Um tipo BLOB n�o pode ser parte de uma chave prim�ria."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[84],
                         "Uma coluna do tipo BLOB n�o pode ser indexada."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[85], "Um BLOB n�o pode estar na cl�usula WHERE."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[86],
                         "Um BLOB n�o pode ser convertido em uma string."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[87],
            "Tipos BLOB n�o podem estar em cl�usulas ORDER BY ou GROUP BY."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[88], "N�o � poss�vel comparar BLOBs."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[89],
                         "S� � poss�vel inserir ou atualizar um BLOB atrav�s "
                         "prepared statements usando setBlob()."));
  ...
  ASSERT2_EQUALS(I32,
                 is['_'] & (IS_ALPHA | IS_DIGIT),
                 (IS_ALPHA | IS_DIGIT)); // '_' can be part of an identifier.
  ...
  ASSERT2_EQUALS(
    I32, TC_htGet32(&reserved, TC_hashCode("datetime")), TK_DATETIME);
  ASSERT2_EQUALS(I32, TC_htGet32(&reserved, TC_hashCode("day")), TK_DAY);
  ASSERT2_EQUALS(
    I32, TC_htGet32(&reserved, TC_hashCode("default")), TK_DEFAULT);
  ...
  ASSERT2_EQUALS(
    I32, TC_htGet32(&reserved, TC_hashCode("distinct")), TK_DISTINCT);
  ...
  ASSERT2_EQUALS(
    I32, TC_htGet32(&reserved, TC_hashCode("primary")), TK_PRIMARY);
  ...
  ASSERT2_EQUALS(
    I32, TC_htGet32(&reserved, TC_hashCode("varchar")), TK_VARCHAR);
...
/**
 * Tests the function <code>bindFunctionDataType()</code> works properly. It is
 * tested with all possible data types and data function types, including
 * invalid values.
 *
 * @param testSuite The test structure.
 * @param currentContext The thread context where the test is being executed.
...
TESTCASE(bindFunctionDataType)
{
  ...
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(UNDEFINED_TYPE, FUNCTION_DT_UPPER));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(UNDEFINED_TYPE, FUNCTION_DT_LOWER));
  ...
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(UNDEFINED_TYPE, FUNCTION_DT_MONTH));
  ...
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(UNDEFINED_TYPE, FUNCTION_DT_MINUTE));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(UNDEFINED_TYPE, FUNCTION_DT_SECOND));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(UNDEFINED_TYPE, FUNCTION_DT_LOWER + 1));
  ...
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_TYPE, FUNCTION_DT_LOWER + 1));
  ...
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(SHORT_TYPE, FUNCTION_DT_LOWER + 1));
  ...
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(FLOAT_TYPE, FUNCTION_DT_LOWER + 1));
  ...
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(DOUBLE_TYPE, FUNCTION_DT_LOWER + 1));

  // Tests CHARS_NOCASE type, which is only used for UPPER and LOWER.
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_NONE));
  ASSERT1_EQUALS(True,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_UPPER));
  ASSERT1_EQUALS(True,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_LOWER));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_ABS));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_YEAR));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_MONTH));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_DAY));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_HOUR));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_MINUTE));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_SECOND));
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_MILLIS));
  ASSERT1_EQUALS(
    False, bindFunctionDataType(CHARS_NOCASE_TYPE, FUNCTION_DT_LOWER + 1));
  ...
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(BOOLEAN_TYPE, FUNCTION_DT_LOWER + 1));
  ...
  // Tests DATETIME type, which is only used for YEAR, MONTH, DAY, HOUR, MINUTE,
  // SECOND, and MILLIS.
  ...
  ASSERT1_EQUALS(False,
                 bindFunctionDataType(DATETIME_TYPE, FUNCTION_DT_LOWER + 1));
...
/**
 * Tests if the function <code>checkApppath()</code> accepts paths if and only
 * if they are valid.
 *
 * @param testSuite The test structure.
 * @param currentContext The thread context where the test is being executed.
...
TESTCASE(checkApppath)
{
  ...
  // Correct paths.
  ASSERT1_EQUALS(True,
                 checkApppath(currentContext, sourcePath, defaultAppPath));
  ASSERT2_EQUALS(Sz, sourcePath, defaultAppPath);
  xstrcpy(path, "/");
...
/**
 * Tests if the function <code>dataTypeFunctionsName()</code> returns the
 * strings with the correct types.
 *
 * @param testSuite The test structure.
 * @param currentContext The thread context where the test is being executed.
 */
...
TESTCASE(initVars)
{
  ...
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[30], "The increment must be greater than 0 or -1."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[31], "Iterator already closed."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[32],
            "Prepared statement closed. Please prepare it again."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[33], "Invalid connection parameter: %s."));

  // Table errors.
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[34], "Table name not found: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[35], "Table already created: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[36],
                         "It is not possible to open a table within a "
                         "connection with a different string format."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[37],
  ...
  // ROWID errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[38], "ROWID can't be changed by the user!"));

  // Prepared Statement errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[39], "Query does not return result set."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[40],
                         "Query does not perform updates in the database."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[41],
  ...
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[42], "A value was not defined for the parameter %d."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[43], "Invalid parameter index."));

  // Rename errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[44],
                         "Can't rename table. This table already exists: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[45], "Column already exists: %s."));

  // Alias errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[46], "Not unique table/alias: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[47],
            "This alias is already being used in this expression: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[48],
  ...
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[50], "ORDER BY and GROUP BY clauses must match."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[51],
                         "No support for virtual columns in SQL "
                         "queries with GROUP BY clause."));
  ...
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[59], "Value is not a DATETIME: %s."));

  // Index error.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[60], "Index already created for column %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[61],
                         "Can't drop a primary key index withdrop index."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[62],
  ...
  // NOT NULL errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[63], "Primary key can't have null."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[64], "Field can't be null: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[65], "A parameter in a where clause can't be null."));

  // Result set errors.
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[66], "ResultSet in invalid record position: %d."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[67],
                         "Invalid value for decimal places: "
                         "%d. It must range from -1 to 40."));
  ...
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[73], "Can't find index record position on delete."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[74],
                         "The table format (%d) is incompatible with Litebase "
                         "version. Please update your tables."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[75],
            "The table format is not the previous one: %s.")); // juliana@220_11
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[76], "Invalid path: %s.")); // juliana@214_1
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[77], "Invalid file position: %d."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_en[78], "Database not found."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[79],
            "An opened table can't be recovered or converted: %s."));
  ...
  ASSERT1_EQUALS(
    ...
    xstrcmp(errorMsgs_en[80],
            "The total size of a blob can't be greater then 10 Mb."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[81], "This is not a valid size multiplier."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[82], "A blob type can't be part of a primary key."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[83], "A BLOB column can't be indexed."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[84], "A BLOB can't be in the where clause."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[85], "A BLOB can't be converted to a string."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_en[86],
            "Blobs types can't be in ORDER BY or GROUP BY clauses."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_en[87], "It is not possible to compare BLOBs."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_en[88],
                         "It is only possible to insert or update a BLOB "
                         "through prepared statements using setBlob()."));
  ...
  ASSERT1_EQUALS(
    ...
    xstrcmp(errorMsgs_pt[26],
            ...
            "mais utilizada. Por favor, obtenha uma nova inst�ncia."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[27], "ResultSet j� est� fechado!"));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[28],
                         "ResultSetMetaData n�o pode ser usado depois "
                         "que o ResultSet estiver fechado."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[29], "O id da aplica��o de ter 4 characteres."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[30], "O incremento deve ser maior do que 0 ou -1."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[31], "Iterador j� foi fechado."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[32],
            "Prepared statement fechado. Por favor, prepare-o novamente."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[33], "Par�metro de conex�o inv�lido: %s."));

  // Table errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[34], "Nome da tabela n�o encontrado: %s."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[35], "Tabela j� existe: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[36],
                         "N�o � poss�vel abrir uma tabela com uma "
                         "conex�o com um tipo de strings diferente."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[37],
  ...
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[38], "ROWID n�o pode ser mudado pelo usu�rio!"));

  // Prepared Statement errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[39], "Comando SQL n�o retorna um ResultSet."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[40],
            "Comando SQL n�o executa uma atualiza��o no banco de dados."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[41],
                         "Nem todos os par�metros da consulta "
  ...
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[44],
            "N�o � poss�vel renomear a tabela. Esta tabela j� existe: %s."));
  ASSERT1_EQUALS(False, xstrcmp(errorMsgs_pt[45], "Coluna j� existe: %s."));

  // Alias errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[46], "Nome de tabela/alias repetido: %s."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[47],
                         "Este alias j� est� sendo utilizado no sql: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[48],
  ...
  // DATE and DATETIME errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[58], "Valor n�o � um tipo DATE v�lido: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[59], "Valor n�o � um tipo DATETIME v�lido: %s."));

  // Index error.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[60], "�ndice j� criado para a coluna %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[61],
            "N�o � poss�vel remover uma chave prim�ria usando drop index."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[62],
  ...
  // NOT NULL errors.
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[63], "Chave prim�ria n�o pode ter NULL."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[64], "Coluna n�o pode ser NULL: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[65],
            "Um par�metro em uma where clause n�o pode ser NULL."));
  ...
  // File errors.
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[68], "N�o � poss�vel ler da tabela %s."));
  ASSERT1_EQUALS(
    False, xstrcmp(errorMsgs_pt[69], "N�o � poss�vel carregar n� folha!"));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[70], "Tabela est� corrompida: %s."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[71],
            "Tabela n�o foi fechada corretamente: %s.")); // juliana@220_2
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[72],
                         "Uma tabela fechada corretamente n�o pode ser usada "
  ...
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[76],
                         "Caminho inv�lido: %s.")); // juliana@214_1
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[77], "Posi��o inv�lida no arquivo: %d."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[78], "Base de dados n�o encontrada."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[79],
            "Uma tabela aberta n�o pode ser recuperada ou convertida: %s."));
  ...
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[81], "O multiplicador de tamanho n�o � v�lido."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[82],
            "Um tipo BLOB n�o pode ser parte de uma chave prim�ria."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[83],
                         "Uma coluna do tipo BLOB n�o pode ser indexada."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[84], "Um BLOB n�o pode estar na cl�usula WHERE."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[85],
                         "Um BLOB n�o pode ser convertido em uma string."));
  ASSERT1_EQUALS(
    False,
    xstrcmp(errorMsgs_pt[86],
            "Tipos BLOB n�o podem estar em cl�usulas ORDER BY ou GROUP BY."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[87], "N�o � poss�vel comparar BLOBs."));
  ASSERT1_EQUALS(False,
                 xstrcmp(errorMsgs_pt[88],
                         "S� � poss�vel inserir ou atualizar um BLOB atrav�s "
                         "prepared statements usando setBlob()."));
