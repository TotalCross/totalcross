...
/**
 * Implements the function used to load the pointers to TotalCross functions
 * used by Litebase and delcares the function names.
 */
...
initTCVMLib()
{
  ...
  TC_JCharPEqualsIgnoreCaseJCharP =
    GETPROCADDRESS(JCharPEqualsIgnoreCaseJCharP);
  ...
  TC_createStringObjectFromTCHARP =
    GETPROCADDRESS(createStringObjectFromTCHARP);
  ;
