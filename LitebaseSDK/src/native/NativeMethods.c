...
LB_API void lLC_privateDeleteLogFiles(NMParams p) // litebase/LitebaseConnection public static native int deleteLogFiles();
{
   ...
   if ((i = TC_listFiles(pathTCHARP, 1, &list, &count, heap, 0))) // Lists all the files of the folder. 
   {
      fileError(context, i, "");
      goto finish;
   }
   ...
   while (--count >= 0)
   {
      ...
      if (xstrstr(value, "LITEBASE") == value && xstrstr(value, ".LOGS")) // Deletes only the closed log files.
      {
         getFullFileName(value, pathCharP, fullPath);
         if (!fileDelete(null, fullPath, 1, false))
            i++;
      }
