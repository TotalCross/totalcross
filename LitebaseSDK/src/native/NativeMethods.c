...
LB_API void lRI_next(NMParams p) { // litebase/RowIterator public native boolean next();
	...
	if (testRIClosed(p)) {
		...
		if (++rowNumber < plainDB->rowCount && plainRead(p->currentContext, plainDB, rowNumber)) {
			...
			OBJ_RowIteratorAttr(rowIterator) = ((id & ROW_ATTR_MASK) >> ROW_ATTR_SHIFT) &
		...
		xmemmove(table->columnNulls, basbuf + table->columnOffsets[table->columnCount],
 ...
 */
LB_API void lRI_nextNotSynced(NMParams
							  p) { // litebase/RowIterator public native boolean nextNotSynced();
...
LB_API void lRI_setSynced(NMParams p) { // litebase/RowIterator public native void setSynced();
	...
	if (testRIClosed(p)) {
		...
		if ((newAttr = OBJ_RowIteratorAttr(rowIterator) = ROW_ATTR_SYNCED_MASK) != (oldAttr = ((
					id & ROW_ATTR_MASK) >> ROW_ATTR_SHIFT) & 3)
 ...
 * @param p->obj[0] The row iterator.
 */
LB_API void lRI_setNotSynced(NMParams
							 p) { // litebase/RowIterator public native void setNotSynced();
	...
	if (testRIClosed(p)) {
		...
		// guich@560_19 // juliana@230_16: solved a bug with row iterator.
		if ((newAttr = OBJ_RowIteratorAttr(rowIterator) = ROW_ATTR_NEW) != (oldAttr = ((
					id & ROW_ATTR_MASK) >> ROW_ATTR_SHIFT) & 3)
			&& oldAttr != (int32)ROW_ATTR_DELETED_MASK) {
			id = (id & ROW_ID_MASK) | newAttr; // Sets the new attribute.
			xmove4(basbuf, &id);
 ...
 */
LB_API void lRI_getShort_i(NMParams
						   p) { // litebase/RowIterator public native short getShort(int column);
 ...
 */
LB_API void lRI_getLong_i(NMParams
						  p) { // litebase/RowIterator public native long getLong(int column);
	TRACE("lRI_getLong_i")
	MEMORY_TEST_START
 ...
 */
LB_API void lRI_getFloat_i(NMParams
						   p) { // litebase/RowIterator public native double getFloat(int column);
 ...
 * @param p->retD Receives the value of the column or 0 if the column is <code>null</code>.
 */
LB_API void lRI_getDouble_i(NMParams
							p) { // litebase/RowIterator public native double getDouble(int column);
 ...
 */
LB_API void lRI_getString_i(NMParams
							p) { // litebase/RowIterator public native String getString(int column);
 ...
 * @param p->retO Receives the value of the column or <code>null</code> if the column is <code>null</code>.
 */
LB_API void lRI_getBlob_i(NMParams
						  p) { // litebase/RowIterator public native byte[] getBlob(int column);
	TRACE("lRI_getBlob_i")
	MEMORY_TEST_START
 ...
 * @param p->retO Receives the value of the column or <code>null</code> if the column is <code>null</code>.
 */
LB_API void lRI_getDate_i(NMParams
						  p) { // litebase/RowIterator public native totalcross.util.Date getDate(int column);
	TRACE("lRI_getDate_i")
	MEMORY_TEST_START
 ...
 * @param p->retO Receives the value of the column or <code>null</code> if the column is <code>null</code>.
 */
LB_API void lRI_getDateTime_i(NMParams
							  p) { // litebase/RowIterator public native totalcross.sys.Time getDateTime(int column);
 ...
 */
LB_API void lRI_isNull_i(NMParams
						 p) { // litebase/RowIterator public native boolean isNull(int column) IllegalArgumentException;
	TRACE("lRI_isNull_i")
	...
	if (testRIClosed(p)) {
		...
		if (column < 0 || column >= table->columnCount) { // Checks if the column index is within range.
			TC_throwExceptionNamed(p->currentContext, "java.lang.IllegalArgumentException",
								   getMessage(ERR_INVALID_COLUMN_NUMBER), column);
		} else {
			p->retI = isBitSet(table->columnNulls,
							   column);    // juliana@223_5: now possible null values are treated in RowIterator.
		}
	}
	MEMORY_TEST_END
}
 ...
 */
LB_API void lLC_privateGetInstance(NMParams
								   p) { // litebase/LitebaseConnection public static native litebase.LitebaseConnection privateGetInstance();
...
LB_API void lLC_privateGetInstance_s(NMParams p) {
	...
	if (!appCrid) { // The application can't be null.
		...
		TC_throwExceptionNamed(p->currentContext, "litebase.DriverException",
 ...
 */
LB_API void lLC_getSourcePath(NMParams
							  p) { // litebase/LitebaseConnection public native String getSourcePath() throws IllegalStateException;
	...
	if (OBJ_LitebaseDontFinalize(driver)) { // The driver can't be closed.
		TC_throwExceptionNamed(p->currentContext, "java.lang.IllegalStateException",
							   getMessage(ERR_DRIVER_CLOSED));
	} else {
		TC_setObjectLock(p->retO = TC_createStringObjectFromTCHARP(p->currentContext,
								   getLitebaseSourcePath(driver), -1), UNLOCKED);
	}
 ...
 */
LB_API void lLC_execute_s(NMParams
						  p) { // litebase/LitebaseConnection public native void execute(String sql);
	TRACE("lLC_execute_s")
 ...
 */
LB_API void lLC_executeUpdate_s(NMParams
								p) { // litebase/LitebaseConnection public native int executeUpdate(String sql);
	...
	if (checkParamAndDriver(p, "sql")) { // The sql can't be null and the driver can't be closed.
		...
		p->retI = litebaseExecuteUpdate(context, driver, String_charsStart(sqlString),
 ...
 */
LB_API void lLC_executeQuery_s(NMParams
							   p) { // litebase/LitebaseConnection public native litebase.ResultSet executeQuery(String sql);
	...
	if (checkParamAndDriver(p, "sql")) { // The sql can't be null and the driver can't be closed.
		...
		TC_setObjectLock(p->retO = litebaseExecuteQuery(context, driver, String_charsStart(sqlString),
								   String_charsLen(sqlString)), UNLOCKED);
	}
 ...
 */
LB_API void lLC_prepareStatement_s(NMParams
								   p) { // litebase/LitebaseConnection public native litebase.PrepareStatement prepareStatement(String sql) throws OutOfMemoryError;
	...
	if (checkParamAndDriver(p, "sql")) { // The sql can't be null and the driver can't be closed.
		...
		// juliana@253_18: now it is possible to log only changes during Litebase operation.
		if (logger
			&& !litebaseConnectionClass->i32StaticValues[6]) { // juliana@230_30: reduced log files size.
		...
		if ((prepStmt = p->retO = p->obj[0] = TC_htGetPtr(htPS, hashCode = TC_JCharPHashCode(sqlChars,
											  sqlLength)))
			&& !OBJ_PreparedStatementDontFinalize(prepStmt)
			&& (oldSqlObj = OBJ_PreparedStatementSqlExpression(prepStmt))
			&& TC_JCharPEqualsJCharP(String_charsStart(oldSqlObj), sqlChars, String_charsLen(oldSqlObj),
									 sqlLength)) {
		...
		if (xstrstr(command, "create")) {
			OBJ_PreparedStatementType(p->retO) = CMD_CREATE_TABLE;
				   ...
				   || (isSelect = (xstrstr(command, "select") != null)) || xstrstr(command, "update")) {
			bool locked = false;
			Table* table;
		...
		if (!(OBJ_PreparedStatementObjParams(prepStmt) = TC_createArrayObject(context, "[java.lang.Object",
				numParams))) {
			goto finish;
		}
		...
		if (getPreparedStatementStatement(prepStmt) && logger) {
			...
			if (numParams > 0) {
				...
				setPreparedStatementParamsAsStrs(prepStmt, (paramsAsStrs = (JCharP*)TC_heapAlloc(heapParser,
												 numParams * TSIZE)));

				// Creates the array of the parameters length
				setPreparedStatementParamsLength(prepStmt, (paramsLength = (int16*)TC_heapAlloc(heapParser,
												 numParams << 1)));
			...
			setPreparedStatementParamsPos(prepStmt, (paramsPos = (int16*)TC_heapAlloc(heapParser,
										  (numParams + 1) << 1)));
...
LB_API void lLC_getCurrentRowId_s(NMParams p) {
	...
	if (checkParamAndDriver(p,
							"tableName")) { // The driver can't be closed and the table name can't be null.
		Context context = p->currentContext;
		TCObject driver = p->obj[0],
				 tableName = p->obj[1],
				 logger = litebaseConnectionClass->objStaticValues[1];
 ...
 */
LB_API void lLC_exists_s(NMParams
						 p) { // litebase/LitebaseConnection public native boolean exists(String tableName) throws DriverException;
	TRACE("lLC_exists_s")
 ...
 * @throws IllegalStateException If the driver is closed.
 */
LB_API void lLC_closeAll(NMParams
						 p) { // litebase/LitebaseConnection public native void closeAll() throws IllegalStateException;
	TRACE("lLC_closeAll")
	TCObject driver = p->obj[0];
	Context context = p->currentContext;
...
LB_API void lLC_purge_s(NMParams p) {
	...
	if (checkParamAndDriver(p,
		...
		if (table && ((deleted = table->deletedRowsCount) > 0
			...
			if (willRemain) {
				...
				xstrcat(buffer, "_");
				if (!nfCreateFile(context, buffer, true, useCrypto, sourcePath, &newdbo,
				...
				i = -1;
				while (++i < rowCount) {
					if (!readRecord(context, table, record, i, columnNulls0, null, 0, false, null,
									null)) { // juliana@227_20
						goto free;
					}
					...
					if ((record[0]->asInt & ROW_ATTR_MASK) != ROW_ATTR_DELETED) { // is record ok?
						...
						j = -1;
						while (++j < columnCount)
							if (!writeValue(context, plainDB, record[j], &basbuf[columnOffsets[j]], columnTypes[j],
											columnSizes[j], true, true, false,
											false)) {
						...
						if (table->version == VERSION_TABLE) {
							k = columnCount;
							while (--k >= 0)
								if (((type = columnTypes[k]) == CHARS_TYPE || type == CHARS_NOCASE_TYPE)
						...
						xmove4(&basbuf[length],
							   &crc32); // Computes the crc for the record and stores at the end of the record.
						basbuf[3] = j;
				...
				dbo->finalPos = dbFile->finalPos = dbFile->size = dbo->size = plainDB->rowAvail = plainDB->rowCount
												   = 0;
			}

			table->deletedRowsCount = 0; // Empties the deletedRows.
			table->wasUpdated =
				false; // juliana@270_27: now purge will also really purge the table if it only suffers updates.
			...
			plainDB->rowAvail = 0;
			if ((i = lbfileSetSize(&dbFile->file,
								   dbFile->size = plainDB->rowCount * plainDB->rowSize + plainDB->headerSize))
 ...
 */
LB_API void lLC_getRowCountDeleted_s(NMParams
									 p) { // litebase/LitebaseConnection public native int getRowCountDeleted(String tableName);
 ...
 */
LB_API void lLC_getRowIterator_s(NMParams
								 p) { // litebase/LitebaseConnection public native litebase.RowIterator getRowIterator(String tableName);
 ...
 */
LB_API void lLC_privateGetLogger(NMParams
								 p) { // litebase/LitebaseConnection public static native totalcross.util.Logger getLogger();
 ...
 */
LB_API void lLC_privateSetLogger_l(NMParams
								   p) { // litebase/LitebaseConnection public static native void setLogger(totalcross.util.Logger logger);
...
LB_API void lLC_privateGetDefaultLogger(NMParams p) {
	...
	if (!FIELD_I32(FIELD_OBJ(logger, loggerClass, 1),
		...
		TC_CharP2TCHARPBuf(TC_long2str(getTimeLong(year, month, day, hour, minute, second), timeLong),
						   &name[tcslen(name)]);
		tcscat(name, TEXT("."));
...
finish:
	...
	if (context->thrownException
		&& TC_areClassesCompatible(context, OBJ_CLASS(context->thrownException),
 ...
 * @param p->retI receives the number of files deleted.
 */
LB_API void lLC_privateDeleteLogFiles(NMParams
									  p) { // litebase/LitebaseConnection public static native int deleteLogFiles();
	...
	if ((ret = TC_listFiles(path, 1, &list, &count, heap,
	...
	if (count) {
		...
		if (logger) {
			nameObj = ((TCObject*)ARRAYOBJ_START(FIELD_OBJ(FIELD_OBJ(logger, loggerClass, 1),
												 OBJ_CLASS(FIELD_OBJ(logger, loggerClass, 1)), 0)))[0];
...
LB_API void lLC_privateProcessLogs_Ssb(NMParams p) {
	...
	if (sql)
		while (++i < length) {
			...
			if (JCharPStartsWithCharP(sqlStr = String_charsStart(string), "new LitebaseConnection",
									  sqlLen = String_charsLen(string), 22)) {
...
LB_API void lLC_recoverTable_s(NMParams p) {
	...
	if (checkParamAndDriver(p,
		...
		if (String_charsLen(tableName) > MAX_TABLE_NAME_LENGTH) {
			...
			uint32 j;
			int32 auxRowId =
				-1, // juliana@270_26: solved a possible duplicate rowid after issuing LitebaseConnection.recoverTable() on a table.
				currentRowId =
					-1; // juliana@270_26: solved a possible duplicate rowid after issuing LitebaseConnection.recoverTable() on a table.
			int8* types;
			...
			TC_CharPToLower(
				&name[5]); // juliana@227_19: corrected a bug in convert() and recoverTable() which could not find the table .db file.
			...
			if (read !=
				1) { // juliana@226_8: a table without metadata (with an empty .db, for instance) can't be recovered: it is corrupted.
				lbfileClose(&tableDb);
			...
			if (!(table = tableCreate(context, name, sourcePath, false, (bool)OBJ_LitebaseIsAscii(driver),
									  useCrypto, getLitebaseNodes(driver),
									  false, heap))) {
				goto finish;
			}
			...
			while (--i >= 0) { // Checks all table records.
				...
				if (isZero(basbuf, crcPos +
						   4)) { // juliana@268_3: Now does not do anything if there are only zeros in a row and removes them.
					rows--;
					continue;
				}
	...
	if (checkParamAndDriver(p,
		...
		if (String_charsLen(tableName) > MAX_TABLE_NAME_LENGTH) {
			...
			if (j != VERSION_TABLE - 1) {
				TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_WRONG_PREV_VERSION),
									   name);
				lbfileClose(&tableDb);
				goto finish;
			}
 ...
 */
LB_API void lLC_isOpen_s(NMParams
						 p) { // litebase/LitebaseConnection public native boolean isOpen(String tableName) throws DriverException;
	TRACE("lLC_isOpen_s")
...
LB_API void lLC_dropDatabase_ssi(NMParams p) {
	...
	if (cridObj)
		if (pathObj)
			if (String_charsLen(pathObj) >= MAX_PATHNAME - 4 -
				DBNAME_SIZE) { // The path length can't be greater than the buffer size.
				TC_throwExceptionNamed(p->currentContext, "litebase.DriverException",
				...
				if ((i = TC_listFiles(fullPath, -1, &list, &count, heap,
									  0))) { // Lists all the files of the folder.
...
LB_API void lLC_isTableProperlyClosed_s(NMParams p) {
	...
	if (!p->currentContext->thrownException
		...
		if (String_charsLen(tableName) > MAX_TABLE_NAME_LENGTH) {
			...
			if (read !=
				1) { // juliana@226_8: a table without metadata (with an empty .db, for instance) is corrupted.
				fileError(context, j, name);
				lbfileClose(&tableDb);
				TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_TABLE_CORRUPTED), name);
				goto finish;
 ...
 */
LB_API void lRS_getResultSetMetaData(NMParams
									 p) { // litebase/ResultSet public native litebase.ResultSetMetaData getResultSetMetaData();
	...
	if (testRSClosed(p->currentContext, resultSet)
		&& (p->retO = rsMetaData = TC_createObject(p->currentContext, "litebase.ResultSetMetaData"))) {
 ...
 */
LB_API void lRS_close(NMParams
					  p) { // litebase/ResultSet private native void rsClose() throws IllegalStateException;
	TRACE("lRS_close")
	TCObject resultSet = p->obj[0];
 ...
 */
LB_API void lRS_getShort_i(NMParams
						   p) { // litebase/ResultSet public native short getShort(int col) throws DriverException;
 ...
 * @param p->retI receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>0</code>.
 */
LB_API void lRS_getShort_s(NMParams
						   p) { // litebase/ResultSet public native short getShort(String colName) throws DriverException;
 ...
 * @param p->retI receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>0</code>.
 */
LB_API void lRS_getInt_i(NMParams
						 p) { // litebase/ResultSet public native int getInt(int col) throws DriverException;
	TRACE("lRS_getInt_i");
	MEMORY_TEST_START
 ...
 * @param p->retI receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>0</code>.
 */
LB_API void lRS_getInt_s(NMParams
						 p) { // litebase/ResultSet public native int getInt(String colName) throws DriverException;
	TRACE("lRS_getInt_s");
	MEMORY_TEST_START
 ...
 */
LB_API void lRS_getLong_i(NMParams
						  p) { // litebase/ResultSet public native long getLong(int col) throws DriverException;
	TRACE("lRS_getLong_i");
	MEMORY_TEST_START
 ...
 * @param p->retL receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>0</code>.
 */
LB_API void lRS_getLong_s(NMParams
						  p) { // litebase/ResultSet public native long getLong(String colName) throws DriverException;
	TRACE("lRS_getLong_s");
	MEMORY_TEST_START
 ...
 */
LB_API void lRS_getFloat_i(NMParams
						   p) { // litebase/ResultSet public native double getFloat(int col) throws DriverException;
 ...
 * @param p->retD receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>0.0</code>.
 */
LB_API void lRS_getFloat_s(NMParams
						   p) { // litebase/ResultSet public native double getFloat(String colName) throws DriverException;
 ...
 * @param p->retD receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>0.0</code>.
 */
LB_API void lRS_getDouble_i(NMParams
							p) { // litebase/ResultSet public native double getDouble(int col) throws DriverException;
 ...
 * @param p->retD receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>0.0</code>.
 */
LB_API void lRS_getDouble_s(NMParams
							p) { // litebase/ResultSet public native double getDouble(String colName) throws DriverException;
 ...
 */
LB_API void lRS_getChars_i(NMParams
						   p) { // litebase/ResultSet public native char[] getChars(int col) throws DriverException;
 ...
 * @param p->retO receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>null</code>.
 */
LB_API void lRS_getChars_s(NMParams
						   p) { // litebase/ResultSet public native char[] getChars(String colName) throws DriverException;
 ...
 * @param p->retO receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>null</code>
 */
LB_API void lRS_getString_i(NMParams
							p) { // litebase/ResultSet public native String getString(int col) throws DriverException;
 ...
 * @param p->retO receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>null</code>
 */
LB_API void lRS_getString_s(NMParams
							p) { // litebase/ResultSet public native String getString(String colName) throws DriverException;
 ...
 * @param p->retO receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>null</code>.
 */
LB_API void lRS_getBlob_i(NMParams
						  p) { // litebase/ResultSet public native uint8[] getBlob(int col) throws DriverException;
	TRACE("lRS_getBlob_i");
	MEMORY_TEST_START
 ...
 * @param p->retO receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>null</code>.
 */
LB_API void lRS_getBlob_s(NMParams
						  p) { // litebase/ResultSet public native uint8[] getBlob(String colName) throws DriverException;
	TRACE("lRS_getBlob_s");
	MEMORY_TEST_START
 ...
 */
LB_API void lRS_getStrings_i(NMParams
							 p) { // litebase/ResultSet public native String[][] getStrings(int count);
	TRACE("lRS_getStrings_i")
	MEMORY_TEST_START
	getStrings(p,
			   p->i32[0]); // juliana@201_2: corrected a bug that would let garbage in the number of records parameter.
	MEMORY_TEST_END
}
 ...
 * <code>setDecimalPlaces()</code> settings. If the value is SQL <code>NULL</code> or a <code>blob</code>, the value returned is <code>null</code>.
 */
LB_API void lRS_getStrings(NMParams
						   p) { // litebase/ResultSet public native String[][] getStrings();
	TRACE("lRS_getStrings")
	MEMORY_TEST_START
	getStrings(p,
			   -1); // juliana@201_2: corrected a bug that would let garbage in the number of records parameter.
	MEMORY_TEST_END
}
 ...
 * @param p->retO receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>null</code>.
 */
LB_API void lRS_getDate_i(NMParams
						  p) { // litebase/ResultSet public native totalcross.util.Date getDate(int col);
	TRACE("lRS_getDate_i")
	MEMORY_TEST_START
 ...
 * @param p->retO receives the column value; if the value is SQL <code>NULL</code>, the value returned is <code>null</code>.
 */
LB_API void lRS_getDate_s(NMParams
						  p) { // litebase/ResultSet public native totalcross.util.Date getDate(String colName);
	TRACE("lRS_getDate_s")
	MEMORY_TEST_START
 ...
 */
LB_API void lRS_getDateTime_i(NMParams
							  p) { // litebase/ResultSet public native totalcross.sys.Time getDateTime(int colIdx);
 ...
 * @param p->retO receives the time of the DATETIME. If the DATETIME value is SQL <code>NULL</code>, the value returned is <code>null</code>.
 */
LB_API void lRS_getDateTime_s(NMParams
							  p) { // litebase/ResultSet public native totalcross.sys.Time getDateTime(String colName);
	...
	if (testRSClosed(context, resultSet)) { // The driver and the result set can't be closed.
		...
		if (rowsBitmap != null) {
			...
			if ((p->retI = plainRead(context, plainDB, rsBag->pos = i - 1))) {
				xmemmove(table->columnNulls, plainDB->basbuf + table->columnOffsets[table->columnCount],
						 NUMBEROFBYTES(table->columnCount));
			} else {
				goto finish;
			}
		} else if (table->deletedRowsCount >
				   0) { // juliana@210_1: select * from table_name does not create a temporary table anymore.
			int32 rowCount = 0;
			...
			while (rowCount <= rowCountLess1 && rowCount <= row) {
				...
				if ((i & ROW_ATTR_MASK) ==
					ROW_ATTR_DELETED) { // If it was deleted, one more row will be read in total.
					row++;
				}
			}
			xmemmove(table->columnNulls, plainDB->basbuf + table->columnOffsets[table->columnCount],
 ...
 */
LB_API void lRS_relative_i(NMParams
						   p) { // litebase/ResultSet public native bool relative(int rows);
	...
	if (testRSClosed(context, resultSet)) { // The driver and the result set can't be closed.
		...
		if (rowsBitmap) { // juliana@210_1: select * from table_name does not create a temporary table anymore.
			...
			table->deletedRowsCount) { // juliana@210_1: select * from table_name does not create a temporary table anymore.
			int32 value;
			uint8* basbuf = plainDB->basbuf;
			...
			if (rows > 0)
				while (--rows >= 0)
					while (pos++ <
						   rowCountLess1) { // juliana@210_1: select * from table_name does not create a temporary table anymore.
			...
			if (rsBag->pos !=
				newPos) { // If there are no deleted rows, just reads the row in the right position.
				rsBag->pos = newPos;
 ...
 */
LB_API void lRS_setDecimalPlaces_ii(NMParams
									p) { // litebase/ResultSet public native void setDecimalPlaces(int col, int places) throws DriverException;
	...
	if (testRSClosed(p->currentContext, resultSet)) { // The driver and the result set can't be closed.
		...
		if (column < 0
			|| column >=
			rsBag->selectClause->fieldsCount) { // The columns given by the user ranges from 1 to n.
			TC_throwExceptionNamed(p->currentContext, "litebase.DriverException",
								   getMessage(ERR_INVALID_COLUMN_NUMBER), column);
		} else {
			int32 type;
			...
			if (places < -1 || places > 40) { // Invalid value for decimal places.
				TC_throwExceptionNamed(p->currentContext, "litebase.DriverException",
									   getMessage(ERR_RS_DEC_PLACES_START), places);
			} else if (type == FLOAT_TYPE
					   || type == DOUBLE_TYPE) { // Only sets the decimal places if the type is FLOAT or DOUBLE.
...
LB_API void lRS_getRowCount(NMParams p) { // litebase/ResultSet public native int getRowCount();
	...
	if (testRSClosed(p->currentContext, resultSet)) { // The driver and the result set can't be closed.
		...
		p->retI = rsBag->allRowsBitmap ? rsBag->answerCount : rsBag->table->db.rowCount -
				  rsBag->table->deletedRowsCount;
	}
 ...
 */
LB_API void lRS_isNull_s(NMParams
						 p) { // litebase/ResultSet public native boolean isNull(String colName) throws NullPointerException;
	TRACE("lRS_isNull_s")
	TCObject resultSet = p->obj[0],
			 colName = p->obj[1];
 ...
 */
LB_API void lRSMD_getColumnCount(NMParams
								 p) { // litebase/ResultSetMetaData public native int getColumnCount();
...
LB_API void lRSMD_getColumnLabel_i(NMParams p) {
	...
	if (testRSClosed(p->currentContext, resultSet)) { // The driver and the result set can't be closed.
		...
		if (column <= 0 || column > rsBag->selectClause->fieldsCount) {
			...
			TC_setObjectLock(p->retO = TC_createStringObjectFromCharP(p->currentContext,
									   rsBag->selectClause->fieldList[column - 1]->alias, -1),
							 UNLOCKED);
	}
...
LB_API void lRSMD_getColumnTableName_i(NMParams p) {
	...
	if (testRSClosed(p->currentContext, resultSet)) { // The driver and the result set can't be closed.
		...
		if (column < 0 || column >= rsBag->selectClause->fieldsCount) {
			...
			TC_setObjectLock(p->retO = fields[column]->tableName ? TC_createStringObjectFromCharP(
		...
		if (columnNameStr) {
			...
			while (++i < length) { // Gets the name of the table or its alias given the column name.
				if ((((tableColName = fields[i]->tableColName)
					  && JCharPEqualsCharP(columnNameJCharP, tableColName, columnNameLength, xstrlen(tableColName),
										   true)))
					|| ((tableColName = fields[i]->alias)
						&& JCharPEqualsCharP(columnNameJCharP, tableColName, columnNameLength, xstrlen(tableColName),
											 true))) {
					TC_setObjectLock(p->retO = (tableName = fields[i]->tableName) ? TC_createStringObjectFromCharP(
			...
			if (i == length) { // Column name or alias not found.
				...
				TC_throwExceptionNamed(p->currentContext, "litebase.DriverException",
									   getMessage(ERR_COLUMN_NOT_FOUND), tableColName ? tableColName : "");
				xfree(tableColName);
			}
...
LB_API void lRSMD_hasDefaultValue_i(NMParams p) {
	...
	if (!p->currentContext->thrownException && (nameObj = p->retO)) {
		...
		if ((table = getTable(p->currentContext, rsBag->driver, nameCharP))) {
			...
			p->retI = (table->columnAttrs[field->parameter ? field->parameter->tableColIndex :
														   field->tableColIndex]
		...
		IntBuf buffer;
		TC_throwExceptionNamed(p->currentContext, "litebase.DriverException",
							   getMessage(ERR_COLUMN_NOT_FOUND), TC_int2str(p->i32[0], buffer));
	}
 ...
 * @throws DriverException If the column does not have an underlining table.
 */
LB_API void lRSMD_isNotNull_i(NMParams
							  p) { // litebase/ResultSetMetaData public native boolean isNotNull(int columnIndex) throws DriverException;
	...
	if (testRSClosed(context, resultSet)) { // The driver and the result set can't be closed.
		...
		if (tableNameStr) {
			...
			if (table->primaryKeyCol != NO_PRIMARY_KEY) { // Simple primary key.
				...
				array = (TCObject*)ARRAYOBJ_START(p->retO);
				if (!(array[0] = TC_createStringObjectFromCharP(context, table->columnNames[table->primaryKeyCol],
								 -1))) {
					goto finish;
				}
...
LB_API void lRSMD_getDefaultValue_i(NMParams p) {
	...
	if (!context->thrownException && (nameObj = p->retO)) {
		...
		TC_setObjectLock(p->retO = getDefault(context, rsBag, nameCharP,
											  field->parameter ? field->parameter->tableColIndex : field->tableColIndex), UNLOCKED);
	} else if (!nameObj) { // The column does not have an underlining table.
		IntBuf buffer;
		TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_COLUMN_NOT_FOUND),
							   TC_int2str(p->i32[0], buffer));
			...
			while (++i < length) { // Gets the name of the table or its alias given the column name.
				if ((((tableColName = (field = fields[i])->tableColName)
					...
					if (field->tableName) { // Returns the default value of the column or the parameter of a function.
						TC_setObjectLock(p->retO = getDefault(context, rsBag, field->tableName,
															  field->parameter ? field->parameter->tableColIndex : field->tableColIndex), UNLOCKED);
					} else {
						i = length;
					}
...
LB_API void lPS_executeQuery(NMParams p) {
	...
	if (testPSClosed(p)) {
		...
		if (OBJ_PreparedStatementType(stmt) != CMD_SELECT) { // The statement must be a select.
			TC_throwExceptionNamed(context, "litebase.DriverException",
								   getMessage(ERR_QUERY_DOESNOT_RETURN_RESULTSET));
		} else {
			SQLSelectStatement* selectStmt = (SQLSelectStatement*)getPreparedStatementStatement(
												 stmt); // The select statement.
			TCObject driver = OBJ_PreparedStatementDriver(stmt);

			if (!allParamValuesDefinedSel(
					selectStmt)) { // All the parameters of the select statement must be defined.
				TC_throwExceptionNamed(context, "litebase.DriverException",
				...
				// juliana@253_18: now it is possible to log only changes during Litebase operation.
				if (logger
					&& !litebaseConnectionClass->i32StaticValues[6]) { // If log is on, adds information to it.
					LOCKVAR(log);
				...
				if (p->retO) {
					...
					if (!muPut(&memoryUsage, selectStmt->selectClause->sqlHashCode, plainDB->db.size,
							   plainDB->dbo.size)) {
						TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
					}
					UNLOCKVAR(parser);
 ...
 */
LB_API void lPS_executeUpdate(NMParams
							  p) { // litebase/PreparedStatement public native int executeUpdate() throws DriverException;
	...
	if (testPSClosed(p)) {
		...
		if (OBJ_PreparedStatementType(stmt) == CMD_SELECT) { // The statement must be a select.
			...
			switch (OBJ_PreparedStatementType(
						stmt)) { // Returns the number of rows affected or if the command was successfully executed.
				case CMD_INSERT: {
					SQLInsertStatement* insertStmt = (SQLInsertStatement*)getPreparedStatementStatement(stmt);

					rearrangeNullsInTable(insertStmt->table, insertStmt->record, insertStmt->storeNulls,
				...
				case CMD_UPDATE: {
					...
					rearrangeNullsInTable(updateStmt->rsTable->table, updateStmt->record, updateStmt->storeNulls,
					...
					if (allParamValuesDefinedUpd(updateStmt)
						&& convertStringsToValues(context, updateStmt->rsTable->table, updateStmt->record,
												  updateStmt->nValues)) {
				...
				default: { // alter table or drop
					TCObject sqlExpression = OBJ_PreparedStatementSqlExpression(stmt);
					p->retI = litebaseExecuteUpdate(context, driver, String_charsStart(sqlExpression),
 ...
 * @throws OutOfMemoryError If a memory allocation fails.
 */
LB_API void lPS_setString_is(NMParams
							 p) { // litebase/PreparedStatement public native void setString(int index, String value) OutOfMemoryError;
	...
	if (testPSClosed(p)) {
		...
		if (statement) { // Only sets the parameter if the statement is not null.
			...
			if (psSetStringParamValue(p->currentContext, stmt, string, index,
									  string ? String_charsLen(string) : 0)) { // Sets the string parameter.
 ...
 */
LB_API void lPS_setBlob_iB(NMParams
						   p) { // litebase/PreparedStatement public native void setBlob(int index, uint8 []value) throws SQLParseException;
	...
	if (testPSClosed(p)) {
		...
		if (statement) { // Only sets the parameter if the statement is not null.
			...
			switch (statement->type) { // Sets the parameter.
				case CMD_INSERT:
					if (!setStrBlobParamValueIns(p->currentContext, (SQLInsertStatement*)statement, index, blobArray,
												 blobLength, false)) {
						goto finish;
					}
					break;
				case CMD_UPDATE:
					if (!setStrBlobParamValueUpd(p->currentContext, (SQLUpdateStatement*)statement, index, blobArray,
												 blobLength, false)) {
						goto finish;
					}
					break;
			...
			if (OBJ_PreparedStatementStoredParams(
					stmt)) { // Only stores the parameter if there are parameters to be stored.
	...
	if (testPSClosed(p)) {
		...
		if (statement) { // Only sets the parameter if the statement is not null.
			...
			if (date) {
				...
				date2JCharP(FIELD_I32(date, 2), FIELD_I32(date, 1), FIELD_I32(date, 0),
 ...
 * @param p->obj[1] The value of the parameter.
 */
LB_API void lPS_setDateTime_id(NMParams
							   p) { // litebase/PreparedStatement public native void setDate(int index, totalcross.Util.Date);
		...
		if (statement) { // Only sets the parameter if the statement is not null.
			...
			if (time) {
				if (!dateTimeBufObj || String_charsLen(dateTimeBufObj) < 23) {
					...
					objParams[index] =
						dateTimeBufObj; // juliana@222_8: stores the object so that it won't be collected.
				} else {
					xmemzero(String_charsStart(dateTimeBufObj), String_charsLen(dateTimeBufObj) << 1);
				}
				dateTime2JCharP(Time_year(time), Time_month(time), Time_day(time), Time_hour(time),
								Time_minute(time), Time_second(time),
								Time_millis(time), stringChars = String_charsStart(dateTimeBufObj));
			}

			psSetStringParamValue(p->currentContext, stmt, dateTimeBufObj, index,
 ...
 */
LB_API void lPS_setNull_i(NMParams
						  p) { // litebase/PreparedStatement public native void setNull(int index) throws SQLParseException;
	TRACE("lPS_setNull_i")
 ...
 * @param p->obj[0] The prepared statement.
 */
LB_API void lPS_clearParameters(NMParams
								p) { // litebase/PreparedStatement public native void clearParamValues();
 ...
 */
LB_API void lPS_toString(NMParams
						 p) { // litebase/PreparedStatement public native String toString();
	TRACE("lPS_toString")
	...
	if (testPSClosed(p)) {
		...
		if (OBJ_PreparedStatementStoredParams(
				statement)) { // There are no parameters o the logger is not being used.
			TCObject string;
			...
			while (++i < storedParams) { // Concatenates each string part with the next parameter.
				...
				debugLen += length;
				xmemmove(&charsStart[debugLen], &sql[paramsPos[i] + 1],
