...
class SQLUpdateStatement extends SQLStatement {
	...
	void setParamValue(int index, String val) throws DriverException, SQLParseException,
		...
		if (index < paramCount) { // The parameter is in the update clause.
			...
			Utils.setBit(storeNulls, idx, record[idx].isNull = (val ==
						 null)); // Sets whether the value is or not null.
			paramDefined[index] = true;
		} else { // The parameter is in the where clause.
	...
	void clearParamValues() {
		...
		Convert.fill(paramDefined, 0, paramDefined.length,
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	int litebaseDoUpdate(LitebaseConnection driver) throws IOException, InvalidDateException,
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	SQLUpdateStatement litebaseBindUpdateStatement(LitebaseConnection driver) throws IOException,
	...
	void checkIndex(int index) throws IllegalArgumentException {
		if (index < 0
			|| index >= (paramCount + (whereClause == null ? 0 :
