...
class SQLUpdateStatement extends SQLStatement
{
  ...
  SQLUpdateStatement(LitebaseParser parser) throws SQLParseException
  {
    ...
    String[] fieldsAux = fields =
      new String[nValues]; // Create an array of fields.
    ...
    // Allocates space for the list of the parameters. Worst case: all fields
    // are parameters. juliana@253_14: corrected a possible AIOBE if the number
    // of parameters of a prepared statement were greater than 128.
    ...
    while (--nValues >= 0) {
      ...
      if (fieldsAux[nValues].hashCode() == SQLElement.hcRowId)
        throw new SQLParseException(LitebaseMessage.getMessage(
          LitebaseMessage.ERR_ROWID_CANNOT_BE_CHANGED));
    ...
    if (clause != null) { // Process the where clause, if it exists.
      ...
      SQLBooleanClauseTree[] compactParamList =
        new SQLBooleanClauseTree[clause.paramCount];
  ...
  void setParamValue(int index, short val) throws DriverException
  {
    ...
    if (index < paramCount) { // The parameter is in the update clause.
      int idx = paramIndexes[index] & 0xFF; // guich@lb225_1: masks out the sign
                                            // bit in all reads of paramIndexes.

      if (rsTable.table.columnTypes[idx] !=
          SQLElement.SHORT) { // The type must be short.
      ...
      Utils.setBit(storeNulls,
                   idx,
                   value.isNull = false); // The value is not null.
  ...
  void setParamValue(int index, int val) throws DriverException
  {
    ...
    if (index < paramCount) { // The parameter is in the update clause.
      ...
      if (rsTable.table.columnTypes[idx] !=
          SQLElement.INT) { // The type must be int.
        throw new DriverException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_INCOMPATIBLE_TYPES));
      }
  ...
  void setParamValue(int index, long val) throws DriverException
  {
    ...
    if (index < paramCount) { // The parameter is in the update clause.
      ...
      if (rsTable.table.columnTypes[idx] !=
          SQLElement.LONG) { // The type must be long.
        throw new DriverException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_INCOMPATIBLE_TYPES));
      }
  ...
  void setParamValue(int index, float val) throws DriverException
  {
    ...
    if (index < paramCount) { // The parameter is in the update clause.
      ...
      if (rsTable.table.columnTypes[idx] !=
          SQLElement.FLOAT) { // The type must be float.
        throw new DriverException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_INCOMPATIBLE_TYPES));
      }
  ...
  void setParamValue(int index, double val) throws DriverException
  {
    ...
    if (index < paramCount) { // The parameter is in the update clause.
      ...
      if (rsTable.table.columnTypes[idx] !=
          SQLElement.DOUBLE) { // The type must be double.
        throw new DriverException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_INCOMPATIBLE_TYPES));
      }
           ...
           InvalidDateException
  {
    ...
    if (index < paramCount) { // The parameter is in the update clause.
      ...
      if (rsTable.table.columnTypes[idx] ==
          SQLElement.BLOB) { // The type can't be a blob.
        throw new DriverException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_BLOB_STRING));
      }

      record[idx].asString =
        val; // Sets the values of the parameter in its list.
      Utils.setBit(storeNulls,
                   idx,
                   record[idx].isNull =
                     (val == null)); // Sets whether the value is or not null.
      paramDefined[index] = true;
    } else {             // The parameter is in the where clause.
  ...
  void setParamValue(int index, byte[] val)
    throws DriverException, SQLParseException
  {
    checkIndex(index); // Checks if the index is within the range.

    if (index >= paramCount) // // The parameter is in the where clause.
                             // Since blobs can't be in the were clause, an
    ...
    if (rsTable.table.columnTypes[idx] !=
        SQLElement.BLOB) { // The type must be blob.
      throw new DriverException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_INCOMPATIBLE_TYPES));
    }
  ...
  void clearParamValues()
  {
    ...
    // juliana@253_14: corrected a possible AIOBE if the number of parameters of
    // a prepared statement were greater than 128.
    short[] paramIndexesAux = paramIndexes;

    byte[] storeNullsAux = storeNulls;

    Convert.fill(paramDefined,
                 0,
                 paramDefined.length,
                 false); // Cleans the parameter values of the update clause.
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  int litebaseDoUpdate(LitebaseConnection driver)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    ...
    ResultSet rs = table.createSimpleResultSet(
      where); // Creates the result set that will be used to update the rows.
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  SQLUpdateStatement litebaseBindUpdateStatement(LitebaseConnection driver)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    Table table = rsTable.table =
      driver.getTable(rsTable.tableName); // Gets the statement table.
    int valuesCount = record.length;

    // juliana@262_1: now it is not allowed duplicated fields in an update
    // statement.
    if (valuesCount >= table.columnCount) {
      throw new SQLParseException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_DUPLICATED_COLUMN_NAME));
    }
    ...
    while (++i < valuesCount) { // Checks if there are undefined values.
      String string;
      if ((string = record[i].asString) != null &&
          string.equals("?")) { // Identifies the values that are placeholders
                                // for parameters.
        paramIndexes[paramCount++] = (short)i;
      }
    }

    table.reorder(this); // Makes sure the fields are in correct order, aligned
                         // with the table order.
    table.convertStringsToValues(
      record); // Converts the values to be updated into its correct type.
  ...
  void checkIndex(int index) throws IllegalArgumentException
  {
    if (index < 0 ||
        index >=
          (paramCount + (whereClause == null
                           ? 0
                           : whereClause.paramCount))) // Checks if the index is
                                                       // within the range.
