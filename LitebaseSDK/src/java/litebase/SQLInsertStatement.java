...
class SQLInsertStatement extends SQLStatement {
	 ...
	 */
	SQLInsertStatement(LitebaseParser parser, LitebaseConnection driver) throws SQLParseException,
		...
		SQLValue[] recordAux = record = SQLValue.newSQLValues(nFields = parser.fieldValuesSize +
										1);  // Gets the values and stores them.
	...
	void setParamValue(int index, short val) {
		int columnIndex = paramIndexes[index] &
						  0xFF; // guich@lb225_1: masks out the sign bit in all reads of paramIndexes.
		checkIndex(index, columnIndex, SQLElement.SHORT,
	...
	void setParamValue(int index, int val) {
		int columnIndex = paramIndexes[index] &
						  0xFF; // guich@lb225_1: masks out the sign bit in all reads of paramIndexes.
		checkIndex(index, columnIndex, SQLElement.INT,
				   SQLElement.UNDEFINED); // Checks the parameter index and type.
	...
	void setParamValue(int index, long val) {
		int columnIndex = paramIndexes[index] &
						  0xFF; // guich@lb225_1: masks out the sign bit in all reads of paramIndexes.
		checkIndex(index, columnIndex, SQLElement.LONG,
				   SQLElement.UNDEFINED); // Checks the parameter index and type.
	...
	void setParamValue(int index, float val) {
		int columnIndex = paramIndexes[index] &
						  0xFF; // guich@lb225_1: masks out the sign bit in all reads of paramIndexes.
		checkIndex(index, columnIndex, SQLElement.FLOAT,
				   SQLElement.UNDEFINED); // Checks the parameter index and type.
	...
	void setParamValue(int index, double val) throws DriverException {
		int columnIndex = paramIndexes[index] &
						  0xFF; // guich@lb225_1: masks out the sign bit in all reads of paramIndexes.
		checkIndex(index, columnIndex, SQLElement.DOUBLE,
				   SQLElement.UNDEFINED); // Checks the parameter index and type.
	...
	void setParamValue(int index, String val) throws DriverException, IllegalArgumentException {
		if (index < 0
			|| index >= paramCount) { // Checks if the index is within the range of the parameter count.
		...
		SQLValue value = record[columnIndex];
		value.asString = val;
		Utils.setBit(storeNulls, columnIndex,
					 value.isNull = (val == null)); // Sets whether the value is or not null.
	...
	void setParamValue(int index, byte[] val) {
		int columnIndex = paramIndexes[index] &
						  0xFF; // guich@lb225_1: masks out the sign bit in all reads of paramIndexes.
		checkIndex(index, columnIndex, SQLElement.BLOB,
				   SQLElement.UNDEFINED); // Checks the parameter index and type.
	...
	void setNull(int index) throws DriverException {
		...
		value.asString = null;
		Utils.setBit(storeNulls, columnIndex,
					 value.isNull = paramDefined[index] = true); // Sets whether the value is or not null.
	}
	...
	void litebaseDoInsert(LitebaseConnection driver) throws IOException, InvalidDateException {
		...
		if (dbFile ==
			null) { // juliana@201_28: If a table is re-created after the prepared statement is parsed, there won't be a NPE.
	 ...
	 */
	SQLInsertStatement litebaseBindInsertStatement() throws SQLParseException, IOException,
		...
		while (++i < valuesCount) { // Checks if there are undefined values.
			...
			if ((string = record[i].asString) != null
				&& string.equals("?")) { // Identifies the values that are placeholders for parameters.
		...
		if (record.length !=
			baseTable.columnCount) // The record to be inserted size must math the table record size.
			throw new SQLParseException(LitebaseMessage.getMessage(
	 ...
	 */
	void checkIndex(int index, int columnIndex, int type1, int type2) throws IllegalArgumentException,
		DriverException {
