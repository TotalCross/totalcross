...
class SQLInsertStatement extends SQLStatement
{
  ...
  // juliana@253_14: corrected a possible AIOBE if the number of parameters of a
  // prepared statement were greater than 128.
  /**
   ...
   */
  SQLInsertStatement(LitebaseParser parser, LitebaseConnection driver)
    throws SQLParseException, InvalidDateException, IOException
  {
    ...
    // On Litebase, a table has no alias name on insert. This has no sense. So
    // the same name of the table will be used as an alias. The parser must be
    // changed to understand the alias table name.

    table = driver.getTable(
      tableName =
        parser.tableList[0].tableName); // Gets the statement base table.

    if (parser.fieldNamesSize !=
        0) { // Checks if it is not using the default order.
      ...
      while (--nFields > 0)
        ...
        if ((fieldsAux[nFields] = fieldNames[nFields - 1]).hashCode() ==
            SQLElement.hcRowId)
          throw new SQLParseException(LitebaseMessage.getMessage(
            LitebaseMessage.ERR_ROWID_CANNOT_BE_CHANGED));
    } else { // The nulls info does not need to be recreated when all the fields
             // are used in the insert.
      Convert.fill(storeNulls = table.storeNulls, 0, storeNulls.length, 0);
    }

    // Number of fields + rowid.
    SQLValue[] recordAux = record = SQLValue.newSQLValues(
      nFields = parser.fieldValuesSize + 1); // Gets the values and stores them.
    ...
    while (--nFields > 0)
      if ((value = fieldValues[nFields - 1]) !=
          null) { // Only stores values that are not null.
  ...
  {
    int columnIndex =
      paramIndexes[index] & 0xFF; // guich@lb225_1: masks out the sign bit in
                                  // all reads of paramIndexes.
    checkIndex(index,
               columnIndex,
               SQLElement.SHORT,
               SQLElement.UNDEFINED); // Checks the parameter index and type.
  ...
  {
    if (index < 0 || index >= paramCount) { // Checks if the index is within the
                                            // range of the parameter count.
    ...
    if (table.columnTypes[columnIndex] ==
  ...
  void setNull(int index) throws DriverException
  {
    ...
    Utils.setBit(storeNulls,
                 columnIndex,
                 value.isNull = paramDefined[index] =
                   true); // Sets whether the value is or not null.
  }
  ...
  /**
   * Does nothing. If a parameter is not defined, <code>null</code> will be
   * inserted instead.
   */
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  void litebaseDoInsert(LitebaseConnection driver)
    throws IOException, InvalidDateException
  {
    ...
    if (dbFile == null) { // juliana@201_28: If a table is re-created after the
   ...
   * @throws SQLParseException If the number of values does not match the number
   *     of table fields.
   * @throws IOException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   * @throws InvalidNumberException  If an internal method throws it.
   */
  SQLInsertStatement litebaseBindInsertStatement()
    throws SQLParseException, IOException, InvalidDateException,
           InvalidNumberException
  {
    int valuesCount = record.length, i = 0;
    ...
    if (record.length !=
        baseTable.columnCount) // The record to be inserted size must math the
                               // table record size.
      throw new SQLParseException(LitebaseMessage.getMessage(
        LitebaseMessage.ERR_NUMBER_VALUES_DIFF_TABLE_DEFINITION));

    baseTable.convertStringsToValues(
      record); // Converts the string values to their right types.
    return this;
  }
   ...
   */
  void checkIndex(int index, int columnIndex, int type1, int type2)
    throws IllegalArgumentException, DriverException
  {
    if (index < 0 ||
        index >= paramCount) // Checks if the index is within the range.
      throw new IllegalArgumentException(LitebaseMessage.getMessage(
        LitebaseMessage.ERR_INVALID_PARAMETER_INDEX));
