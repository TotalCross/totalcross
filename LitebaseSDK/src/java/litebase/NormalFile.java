...
class NormalFile extends XFile
{
  ...
  {
    f = new File(name,
                 isCreation ? File.CREATE_EMPTY
                            : File.READ_WRITE) { // Opens or creates the file.
      // flsobral@lb201_1: Overriding finalize method prevents files from being
      // finalized before the LitebaseConnection.
  ...
  {
    if (cacheInitialSize < count || pos < cacheIni ||
        (pos + count) > cacheEnd) {
      refreshCache(count);
    }
  ...
  /**
   * Enlarges the file. This method MUST be called to grow the file - otherwise,
   * <code>getSize()</code> won't work correctly.
   *
  ...
  void growTo(int newSize) throws IOException
  {
    ...
    // juliana@227_23: solved possible crashes when using a table recovered
    // which was being used with setRowInc().
    if (newSize - size >
   ...
   * @throws IOException If an internal method throws it.
   * @throws DriverException If the table is corrupted and its access tries to
   *     read/write after the file end.
   */
  void setPos(int newPos) throws IOException, DriverException
  {
    if (newPos > size) {
      throw new DriverException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_TABLE_CORRUPTED));
    }
  ...
  void flushCache() throws IOException
  {
    ...
    cacheIsDirty = false;
    f.writeBytes(
      cache, cacheDirtyIni - cacheIni, cacheDirtyEnd - cacheDirtyIni);
  }
   ...
   * @throws IOException If an internal method throws it or it is not possible
  ...
  private void refreshCache(int count) throws IOException, DriverException
  {
    ...
    if (cacheInitialSize < count) {
      cache =
        new byte[cacheInitialSize = Math.max(CACHE_INITIAL_SIZE, count << 2)];
    }
    if (size != 0) {
      f.setPos(cacheDirtyEnd = cacheIni = pos);

      // juliana@212_8: when reading a file, an exception must not be thrown
      // when reading zero bytes.
      if (f.readBytes(cache, 0, cacheInitialSize) == -1 && pos != size &&
          count > 0) {
        throw new IOException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_CANT_READ));
      }

      // Uses the cache size even if less was read, otherwise when filling a
      // table, the cache will have to be refreshed all the times.
  ...
  public void close() throws IOException
  {
    ...
    if (finalPos > 0) // juliana@210a_11: fixed a problem that could crop data
                      // from database indices.
      f.setSize(
        finalPos); // juliana@201_5: the .dbo file must be cropped so that it
                   // wont't be too large with zeros at the end of the file.
    f.close();
  }

  /**
   * If there's enough memory, loads this file into memory by increasing the
   * cache size.
   *
   * @param turnOn <code>true</code> to use a very big cache size;
   *     <code>false</code> to use the default cahce size.
   * @throws IOException If an internal method throws it.
   */
  void loadIntoMemory(boolean turnOn) throws IOException
  {
    if (!turnOn) {
      ...
      int cacheSize =
        size == 0
          ? 8192
          : size; // If nothing was loaded, starts with a good cache size.
      if (cacheInitialSize < cacheSize &&
          totalcross.sys.Vm.getFreeMemory() > cacheSize << 2) {
  ...
  void rename(String newName) throws IOException
  {
    ...
    f = new File(newName, File.READ_WRITE) { // Opens or creates the file.
      protected synchronized void finalize()
      {
        // flsobral@lb201_1: Overriding finalize method prevents files from
        // being finalized before the LitebaseConnection.
        if (LitebaseConnection.htDrivers.size() == 0) {
          super.finalize();
        }
      }
