...
class SQLValue
{
  ...
  // rnovais@568_10
  // You must to handle the null values before. If the value is null, this
   ...
   *     compared is greater than the one being compared against; otherwise,
   * a negative number.
   * @throws IOException If an internal method throws it.
   */
  int valueCompareTo(SQLValue value,
                     int type,
                     boolean isNull1,
                     boolean isNull2,
                     PlainDB plainDB) throws IOException
  {
    if (isNull1 || isNull2) { // A null value is always considered to be the
                              // greatest value.
   ...
   */
  void validateDateTime(Date tempDate, int valueType)
  ...
  // juliana@202_2: corrected a bug that might let Litebase issue an
  // InvalidDateException if a date was inserted in a datetime field.
  /**
   ...
   */
  void parseDateTime(Date tempDate, String strVal)
    throws SQLParseException, InvalidDateException
  {
    strVal = strVal.trim();
    int pos = strVal.lastIndexOf(' '); // By using lastIndexOf another trim for
    ...
    if (pos == -1) {                   // only date?
      ...
      asShort = Utils.testAndPrepareTime(
        strVal.substring(pos + 1)); // Gets the time part, skipping the space.
    }
  }
}
