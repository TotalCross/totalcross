...
public class DBListBox extends ListBox {
	...
	public void setSelectedIndex(int i) {
		if (0 <= i && i < itemCount
			&& i != selectedIndex/* && height != 0*/) { // flsobral@220_14: commented height!=0 otherwise combobox are not properly set. (same problem observed with listbox)
			offset = i;
			int vi = sbar.getVisibleItems();
			int ma = sbar.getMaximum();
			if (offset + vi >
				ma) // astein@200b4_195: fix list items from being lost when the comboBox.setSelectedIndex() method is used
	...
	private void qsort(int first, int last) { // guich@220_34
		...
		while (true) {
			while (high >= low
				   && mid.compareTo(sitems[low][dataCol])  >
				   0) { // guich@_566_25: added "high > low" here and below - guich@_568_5: changed to >=
				low++;
			}
