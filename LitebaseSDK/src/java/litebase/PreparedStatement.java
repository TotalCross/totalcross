...
public class PreparedStatement {
	 ...
	 */
	void prepare(LitebaseConnection newDriver, String sql) throws IOException, InvalidDateException,
		InvalidNumberException, SQLParseException {
		String tempSQL = sqlExpression =
							 sql; // guich@503_9: Assign this, or it will become unusable. This is necessary for logging.
		driver = newDriver;
		...
		if (statement != null && LitebaseConnection.logger != null) {
			...
			if (length >
				0) { // If there is some '?' in the sql command string, there are parameters to be stored.
				int i = sql.length();
	...
	public ResultSet executeQuery() throws DriverException, SQLParseException {
		...
		try {
			...
			if (LitebaseConnection.logger != null
				&& !LitebaseConnection.logOnlyChanges) // If log is on, adds information to it.
	...
	public int executeUpdate() throws DriverException, SQLParseException {
		...
		if (type ==
			SQLElement.CMD_SELECT) // The statement musn't be a select. executeQuery() must be used instead.
			throw new DriverException(LitebaseMessage.getMessage(
	...
	private void rearrangeNullsInTable(Table table, SQLStatement stmt, boolean isPreparedStmt) {
		...
		int len = record.length < paramIndexes.length ? record.length :
				  paramIndexes.length; // Finds the smallest length.

		while (--len >= 0) // If a parameter is not defined, set it as null.
			if (!paramDefined[len]
				&& (value = record[paramIndexes[len]]) !=
				null) { // juliana@201_17: NPE ocurred if record[paramIndexes[len]] == null.
				value.isNull = true;
			}
	...
	public void setDate(int index,
						Date date) { // juliana@220_6: setDate() and setDateTime() must accept null values.
		setString(index, date == null ? null : date.toString(
					  Settings.DATE_YMD)); // Formats the date so that it can be accepted by SQL.
	}
	 ...
	 * @param date The value of the parameter, which can be null.
	 */
	public void setDateTime(int index, Date
							date) { // juliana@220_6: setDate() and setDateTime() must accept null values.
		setString(index, date == null ? null : date.toString(
					  Settings.DATE_YMD)); // Formats the date so that it can be accepted by SQL.
	}
	 ...
	 */
	public void setDateTime(int index, Time
							time) { // juliana@220_6: setDate() and setDateTime() must accept null values.
		...
		if (time == null) {
			...
			setString(index, sbuf.append(time.year).append('/').append(time.month).append('/').append(
						  time.day).append(' ').append(time.hour)
					  .append(':').append(time.minute).append(':').append(time.second).append(':').append(
