...
/**
 * Used to iterate through the rows of a database. It can access some attributes
 * from the row that eases the control of which row was changed, deleted or is
 * newer since a synchronization.
 */
public class RowIterator
{
  ...
  /**
   * The attribute for this row. It is necessary to use the constants beginning
   * with <B><CODE>ROW_ATTR_</B><CODE> to compare or assign.
   */
  public int attr;

  /**
   * The number of the row. Note: this must be READ ONLY. Changing it will
   ...
   */
  protected DataStreamLB
    basds; // juliana@253_8: now Litebase supports weak cryptography.

  /**
   * An iterator cannot be constructed directly; it must be created throught the
   * method <code>LitebaseConnection.getRowIterator()</code>.
   *
   * @param driver The Litebase driver.
   * @param tableName The name of the table for which the row iterator will be
   *     created.
   * @throws IOException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   */
  protected RowIterator(LitebaseConnection driver, String tableName)
    throws IOException, InvalidDateException
  {
    ...
    basds = new DataStreamLB(
      bas = new ByteArrayStream(data = new byte[table.db.rowSize]),
      table.db.useCrypto);
  }
   ...
   *
   * @return <code>true</code> if it is possible to iterate to the next record.
   ...
   *
   * @return <code>true</code> if it is possible to iterate to a next record not
   *     synced. Otherwise, it will return
   *         <code>false</code>.
   * @throws DriverException If an <code>IOException</code> occurs.
   */
  ...
  public void setSynced() throws DriverException
  {
    ...
    try { // The record is assumed to have been already read.
      int id = plainDb.basds.readInt();
      int oldAttr =
        (((id & Utils.ROW_ATTR_MASK) >> ROW_ATTR_SHIFT) & 3); // guich@560_19
      int newAttr = attr = ROW_ATTR_SYNCED;
      if (newAttr != oldAttr && oldAttr != ROW_ATTR_DELETED) {
        plainDb.bas.reset();
        plainDb.basds.writeInt((id & Utils.ROW_ID_MASK) |
                               newAttr); // Sets the new attribute.
   ...
   * row. Returns 0 if the column is null.
   *
   * @param column The double precision floating point number column index,
   *     starting from 1.
   * @return The value of the column or 0 if the column is <code>null</code>.
   * @throws DriverException If an <code>IOException</code> occurs.
   */
  ...
  /**
   * Returns a string contained in the current row. Note that this method must
   * be used only with CHAR types.
   *
   * @param column The string column index, starting from 1.
   * @return The value of the column or <code>null</code> if the column is
   *     <code>null</code>.
   * @throws DriverException If an <code>IOException</code> occurs.
   */
  public String getString(int column) throws DriverException
  {
    try {
      ...
      PlainDB plainDB = table.db;
      plainDB.dbo.setPos(
        basds.readInt()); // Finds the string position in the .dbo.
  ...
  public byte[] getBlob(int column) throws DriverException
  {
    try {
      ...
      PlainDB db = table.db;
      DataStreamLB ds =
        db.dsdbo; // juliana@253_8: now Litebase supports weak cryptography.
   ...
   * @return The value of the column  or <code>null</code> if the column is
   *     <code>null</code>
   * @throws DriverException If an <code>IOException</code> or an
   *     <code>InvalidDateException</code> occurs.
   */
  ...
  // closed.
  /**
   * Returns a datetime contained in the current row. Returns null if the column
   * is null.
   *
  ...
  private void checkState()
  {
    if (table == null) {
      throw new IllegalStateException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_ROWITERATOR_CLOSED));
    }
    if (table.db == null) {
      throw new IllegalStateException(
  ...
  /**
   * Checks if the driver or the row iterator is closed, if the column index is
   * invalid or if the type of the column requested is imcompatible with the get
   * method type and reads the requested value.
   *
   ...
   * @return <code>true</code> if the value stored is SQL <code>NULL</code>;
   *     <code>false</code>, otherwise.
   * @throws DriverException If the column is not of type requested.
   * @throws IllegalArgumentException If the column index is invalid.
   * @throws IOException If an internal method throws it.
   */
  private boolean readColumn(int column, int type1, int type2)
    throws IllegalArgumentException, DriverException, IOException
  {
    checkState(); // juliana@230_27
