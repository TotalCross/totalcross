...
public class RowIterator {
	 ...
	 */
	protected RowIterator(LitebaseConnection driver, String tableName) throws IOException,
		InvalidDateException {
		table = driver.getTable(tableName);
		rowNumber = -1;

		// juliana@253_8: now Litebase supports weak cryptography.
		basds = new DataStreamLB(bas = new ByteArrayStream(data = new byte[table.db.rowSize]),
								 table.db.useCrypto);
	}
	...
	private void checkState() {
		if (table == null) {
			throw new IllegalStateException(LitebaseMessage.getMessage(
												LitebaseMessage.ERR_ROWITERATOR_CLOSED));
		}
		if (table.db == null) {
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	private boolean readColumn(int column, int type1, int type2) throws IllegalArgumentException,
		DriverException,
		IOException {
		checkState(); // juliana@230_27
