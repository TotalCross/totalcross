...
/**
 * This class has useful constants, fields, and methods for the other Litebase
 * classes.
 */
class Utils
{
  /**
   * The default row increment, that is, how many empty rows are inserted in a
   * .db file at once.
   */
  static final int DEFAULT_ROW_INC = 10;

  // juliana@114_9: the absence of primary key can't be zero because the rowid
  // may be a primary key or it can have an index.
  /**
  ...
  /**
   * When saving the meta data, indicates that only the deleted rows count was
   * changed.
   */
  ...
  /**
   * When saving the meta data, indicates that only the auxiliary rowid was
   * changed.
   */
   ...
   */
  private static final byte[] bitsInNibble = { 0, 1, 1, 2, 1, 2, 2, 3,
                                               1, 2, 2, 3, 2, 3, 3, 4 };
  ...
  static int findNextBitSet(int[] items, int start)
  {
    ...
    while (true) {
      ...
      if (n > 0 && items[index] == 0) {
        start = 0;
        while (n > 0 &&
               items[index] == 0) { // Finds the next int with any bit set.
          n--;
          index++;
        }
      }
   ...
   * @return 0 if the arrays are identical; a positive number if
   *     <code>v1[]</code> is greater than <code>v2[]</code>; otherwise, a
   *     negative number.
   * @throws IOException If an internal method throws it.
   */
  static int arrayValueCompareTo(SQLValue[] v1,
                                 SQLValue[] v2,
                                 byte[] types,
                                 PlainDB plainDB) throws IOException
  {
    int size = v1.length, r, i = -1;
    while (++i < size) // juliana@210a_12: corrected wrong comparison order.
      if ((r = v1[i].valueCompareTo(v2[i], types[i], false, false, plainDB)) !=
          0) {
        return r;
      }
    return 0;
   ...
   *     number if <code>record1[]</code> is greater than
   * <code>record2[]</code>; otherwise, a negative number.
   * @throws IOException If an internal method throws it.
   */
  static int compareRecords(SQLValue[] record1,
                            SQLValue[] record2,
                            byte[] nullsRecord1,
                            byte[] nullsRecord2,
    ...
    throws IOException
  {
    ...
    int i = -1, n = sortFieldList.length, result, index;
    SQLResultSetField field;

    while (++i < n) { // Compares the records, using the sequence used by the
                      // sort column List.
      field = sortFieldList[i];
      index = field.tableColIndex;

      // Compares the elements checking if they are null.
      result = record1[index].valueCompareTo(
        record2[index],
        field.dataType,
        (nullsRecord1[index >> 3] & (1 << (index & 7))) != 0,
   ...
   * @return path + file name if path is not empty, null or the root; otherwise,
   *     only the table name.
   */
  static String getFullFileName(String fileName, String sourcePath)
  {
    if (sourcePath.length() > 0 &&
        !sourcePath.equals("./")) { // guich@102a_4: better logic, and fix.
   ...
   * int datetime. The time ranges from 00:00:00:000 to 23:59:59:9999 (it
   * accepts dots and colons). This method is very flexible. For instance:
   * 2:-:8:10 is the same as 2:0:8:10 and returns 20008010; 02:.:8:1 is the same
   * as 02:0.0:8 and returns 20000008; :4:8:19 is the same as 0:4:8:19 and
   * returns; 408019 2.4.a.876 is the same as 2.4.0.876 and returns 20400876.
   *
   ...
   */
  static int testAndPrepareTime(String strTime) throws SQLParseException
  { // rnovais@567_2
    int n = strTime.length(), j = 2, hh, mm, ss, ms, multiplier = 1;
    int p[] = new int[3];
    char c;
    ...
    if (n > 0 && n <= 13) {
      ...
      while (--n >= 0) { // Going back to front and computing the value.
        ...
        if ('0' <= c && c <= '9') {
          ...
          multiplier *= 10;
        } else if (j < 0) // juliana@270_20: solved a possible AIOOBE when
                          // passing an invalid time to DATETIME.
          throw new SQLParseException(
            LitebaseMessage.getMessage(
              LitebaseMessage.ERR_VALUE_ISNOT_DATETIME) +
            strTime);
        else {
          p[j--] = value;
          value = 0;
    ...
    throw new SQLParseException(
      LitebaseMessage.getMessage(LitebaseMessage.ERR_VALUE_ISNOT_DATETIME) +
      strTime);
  }

  /**
   * Formats an int "intTime" into a TIME hh:mm:ss:mmm and appends it to a
  ...
  /**
   * Formats an pecific integer date as a string into a
   * <code>StringBuffer</code>.
   *
   * @param sBuffer The string buffer parameter.
  ...
  {
    sBuffer.append(intDate / 10000000)
      .append(intDate / 1000000 % 10)
      .append(intDate / 100000 % 10)
      .append(intDate / 10000 % 10)
      .append('/')
      .append(intDate / 1000 % 10)
      .append(intDate / 100 % 10)
      .append('/')
      .append(intDate / 10 % 10)
      .append(intDate % 10);
  }

  /**
   * Formats a date or a datetime as integers as a string using a
   * <code>StringBuffer</code>.
   *
   * @param sBuffer The string buffer parameter.
   * @param type The type of the value, DATE or DATETIME.
   * @param sqlValue The record which stores the DATE, DATETIME, and the
   *     resulting string.
   * @return The string representing the date or datetime.
   */
  static String formatDateDateTime(StringBuffer sBuffer,
                                   int type,
                                   SQLValue sqlValue)
  {
   ...
   * @throws SQLParseException If the function can't be applied to the data type
   *     field.
   */
  static void bindFunctionDataType(int parameterDataType, int sqlFunction)
  ...
  static float toFloat(String string) throws InvalidNumberException
  {
    try {
      ...
      throw new InvalidNumberException("Error: " + string +
   ...
   * @param initialIdx The initial index of the string to calculate the hash
   *     code.
   * @return The hash code of the substring.
   */
  static int subStringHashCode(String string, int initialIdx)
  {
    int hashCode = 0, length = string.length() - initialIdx;
