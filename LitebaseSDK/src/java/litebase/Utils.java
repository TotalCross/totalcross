...
class Utils {
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	static int compareRecords(SQLValue[] record1, SQLValue[] record2, byte[] nullsRecord1,
							  byte[] nullsRecord2,
							  SQLResultSetField[] sortFieldList) throws IOException {
		...
		while (++i < n) { // Compares the records, using the sequence used by the sort column List.
			...
			result = record1[index].valueCompareTo(record2[index], field.dataType,
			...
			while (--n >= 0) { // Going back to front and computing the value.
				...
				if ('0' <= c && c <= '9') {
					...
					multiplier *= 10;
						   ...
						   0) // juliana@270_20: solved a possible AIOOBE when passing an invalid time to DATETIME.
					throw new SQLParseException(LitebaseMessage.getMessage(LitebaseMessage.ERR_VALUE_ISNOT_DATETIME) +
												strTime);
				else {
					p[j--] = value;
					value = 0;
	...
	static void formatTime(StringBuffer sBuffer, int intTime) {
		...
		sBuffer.append(h / 10).append(h % 10).append(Settings.timeSeparator).append(minute / 10).append(
			minute % 10).append(Settings.timeSeparator)
		.append(second / 10).append(second % 10).append(Settings.timeSeparator).append(mills / 100).append(
			mills / 10 % 10).append(mills % 10);
	...
	static void formatDate(StringBuffer sBuffer, int intDate) {
		sBuffer.append(intDate / 10000000).append(intDate / 1000000 % 10).append(
			intDate / 100000 % 10).append(intDate / 10000 % 10)
		.append('/').append(intDate / 1000 % 10).append(intDate / 100 % 10).append('/').append(
			intDate / 10 % 10).append(intDate % 10);
	}
	 ...
	 */
	static void bindFunctionDataType(int parameterDataType,
									 int sqlFunction) throws SQLParseException { // rnovais@568_10
