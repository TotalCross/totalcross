...
class SQLDeleteStatement extends SQLStatement
{
  ...
  SQLDeleteStatement(LitebaseParser parser)
  {
    ...
    SQLBooleanClause booleanClause = whereClause =
      parser.whereClause; // Gets the where clause.
    if (booleanClause != null &&
        booleanClause.fieldList.length != booleanClause.fieldsCount) {
      // Compacts the resulting field list.
      SQLResultSetField[] compactFieldList =
        new SQLResultSetField[booleanClause.fieldsCount];
      Vm.arrayCopy(booleanClause.fieldList,
                   0,
                   compactFieldList,
                   0,
      ...
      SQLBooleanClauseTree[] compactParamList =
        new SQLBooleanClauseTree[booleanClause.paramCount];
      Vm.arrayCopy(booleanClause.paramList,
                   0,
                   compactParamList,
                   0,
                   booleanClause.paramCount);
   ...
   * @throws DriverException If the parameter index is invalid or the column
   ...
   * @throws NullPointerException If a <code>null</code> is used as a parameter
   *     of a where clause.
   * @throws InvalidNumberException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   */
  void setParamValue(int index, String val)
    throws NullPointerException, InvalidNumberException, InvalidDateException
  {
    checkIndex(index); // Checks if the index is within the range.
   ...
   * index. Since blobs can't be in a where clause and a delete clause only has
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  int litebaseDoDelete(LitebaseConnection driver)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    if (rsTable.table.db.db ==
        null) { // juliana@201_28: If a table is re-created after the prepared
                // statement is parsed, there won't be a NPE.
    ...
    DataStreamLB ds =
      plainDB.basds; // juliana@253_8: now Litebase supports weak cryptography.
    int nn = 0, columnCount = table.columnCount, i = columnCount, j;
    Index index;
    ComposedIndex ci;
    ...
    // table was not modified since its last opening.
    table.setModified(); // Sets the table as not closed properly.

    // juliana@270_32: corrected a bug of a delete not updating the number of
    // total deleted rows in the metadata when there is an index corruption.
    try {
      if (wholeTable) {   // Deletes the whole table.
        if (hasIndices) { // If the whole table is being deleted, just empties
                          // all indexes.
          i = columnCount;
          while (--i >= 0)
          ...
          if ((i = table.numberComposedIndices) >
              0) // juliana@201_6: it now deletes the erases the composed index
                 // when deleting the whole table.
            while (--i >= 0) {
              composedIndices[i].index.deleteAllRows();
            }
        }

        // juliana@227_10: Corrected a bug of a delete with no where clause not
        // taking the already deleted rows into consideration when returning the
        ...
        // guich@300: now all records are just marked as deleted instead of
        // physical removal.
        int column;
        ...
        if (hasIndices)
          while (rs.getNextRecord()) {
            i = columnCount; // juliana@201_35: Would not remove key from the
                             // index of the last column.

            // juliana@227_11: corrected a bug of an exception being thrown when
            // trying to delete a row with a null in column which has an index.
            while (--i >= 0) // Simple index.
              if (columnIndices[i] != null &&
                  (nulls[i >> 3] & (1 << (i & 7))) == 0) {
                index = columnIndices[i];
                bas.reset(); // juliana@116_1: if reset is not done, the value
                             // read is wrong.
                table.readValue(driver.sqlv,
                                offsets[i],
                                types[i],
                                false,
            ...
            if ((i = table.numberComposedIndices) > 0) // Composed index.
              while (--i >= 0) {
                ...
                index = ci.index;
                keys2 =
                  SQLValue.newSQLValues(j = ci.columns.length); // juliana@201_6
                while (--j >= 0) {
                  // juliana@116_1: if reset is not done, the value read is
                  // wrong.
                  bas.reset();

                  // juliana@220_3
                  table.readValue(keys2[j],
                                  offsets[column = ci.columns[j]],
                                  types[column],
                                  false,
   ...
   */
  SQLDeleteStatement litebaseBindDeleteStatement(LitebaseConnection driver)
    throws InvalidDateException, InvalidNumberException, IOException
  {
    ...
    if (whereClause != null) { // Binds the delete statement to its table.
      whereClause.bindColumnsSQLBooleanClause(
        table.htName2index, table.columnTypes, rsTable);
    }
    return this;
  }
  ...
  {
    if (index < 0 || whereClause == null ||
        index >=
          whereClause.paramCount) // Checks if the index is within the range.
      throw new IllegalArgumentException(LitebaseMessage.getMessage(
