...
class SQLDeleteStatement extends SQLStatement {
	 ...
	 */
	void setParamValue(int index, String val) throws NullPointerException, InvalidNumberException,
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	int litebaseDoDelete(LitebaseConnection driver) throws IOException, InvalidDateException,
			...
			if (wholeTable) { // Deletes the whole table.
				if (hasIndices) { // If the whole table is being deleted, just empties all indexes.
					...
					if ((i = table.numberComposedIndices) >
						0) // juliana@201_6: it now deletes the erases the composed index when deleting the whole table.
						while (--i >= 0) {
						...
						if ((i = table.numberComposedIndices) > 0) // Composed index.
							while (--i >= 0) {
								...
								while (--j >= 0) {
									...
									// juliana@220_3
									table.readValue(keys2[j], offsets[column = ci.columns[j]], types[column], false,
	 ...
	 */
	SQLDeleteStatement litebaseBindDeleteStatement(LitebaseConnection driver) throws
		InvalidDateException,
		InvalidNumberException, IOException {
	...
	void checkIndex(int index) throws IllegalArgumentException {
		if (index < 0 || whereClause == null
			|| index >= whereClause.paramCount) // Checks if the index is within the range.
