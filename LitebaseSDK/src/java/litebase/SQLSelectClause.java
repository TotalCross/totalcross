			...
			while (++j < n) {
				...
				while (++i < m) { // guich@503_10: excludes the rowid.
					(field = new SQLResultSetField()).alias =
						columnNames[i]; // guich@_512_2: added column name so the ResultSetMetaData can work.
			...
			while (++i < n) { // Binds the listed colums to the table.
				...
				if (!field.isAggregatedFunction || field.sqlFunction != SQLElement.FUNCTION_AGG_COUNT) {
					if ((tableName = field.tableName) != null) { // Checks the names.
						...
						if (field.aliasHashCode != Convert.hashCode(sbufnf.append(tableName).append('.').append(
									field.tableColName))) {
							sbufnf.setLength(0);
						...
						j = tableList.length;
						while (--j >= 0) {
							auxIndex = (auxTable = (rsTableAux = tableList[j]).table).htName2index.get(field.tableColHashCode,
									   -1);
							if (auxIndex != -1) {
								if (foundFirst)
									...
									index = auxIndex;
									sbufnf.setLength(0);
									htName2index.put(Convert.hashCode(sbufnf.append(tableList[j].tableName).append('.').append(
																		  field.alias)), i);
									htName2index.put(field.aliasHashCode, i);
