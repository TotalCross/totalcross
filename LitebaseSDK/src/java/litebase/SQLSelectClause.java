...
class SQLSelectClause
{
  ...
  /**
   * Indicates if the query is of the form <code>select count(*) from table
   * where...</code>.
   */
  static final int COUNT_WITH_WHERE = 1;

  // juliana@250_8: now the maximum number of columns, fields, tables, etc is
  // 254 instead of 128 except on palm.
  /**
  ...
  // This will be better used when implement a PLANNER for litebase is
  // implemented. Up to now this only indicates if the select clause has a
  // count(*).
  /**
   ...
   * @throws SQLParseException In case of an unknown or ambiguous column name,
   *     the parameter and the function data types are incompatible, or the
   * total number of fields of the select exceeds the maximum.
   */
  void bindColumnsSQLSelectClause(LitebaseConnection driver)
    throws SQLParseException
  {
    ...
    // If the select clause has a wild card (is null), then expands the list
    // using the column information from the given tables.
    if (fieldList == null) {
      String tableName;
      String[] columnNames;
      int[] columnHashes;
      ...
      // juliana@250_7: now a select * will cause a SQLParseException if the
      // total number of columns is more than 254.
      ...
      while (++j < n) {
        ...
        while (++i < m) { // guich@503_10: excludes the rowid.
          (field = new SQLResultSetField()).alias =
            columnNames[i]; // guich@_512_2: added column name so the
                            // ResultSetMetaData can work.
          ...
          htName2index.put(
            Convert.hashCode(
              sbufnf.append(tableName).append('.').append(field.alias)),
            pos);
      ...
      int index, hash, auxIndex, aggFunctionType,
      ...
      while (++i < n) { // Binds the listed colums to the table.
          ...
          if ((tableName = field.tableName) != null) { // Checks the names.
            ...
            if (table == null)
              throw new SQLParseException(
                ...
                field.alias);
            if ((index = table.htName2index.get(
                   hash = field.tableColName.hashCode(), -1)) == -1)
              throw new SQLParseException(
                LitebaseMessage.getMessage(LitebaseMessage.ERR_UNKNOWN_COLUMN) +
                field.alias);
            ...
            if (field.aliasHashCode !=
                Convert.hashCode(sbufnf.append(tableName).append('.').append(
                  field.tableColName))) {
              sbufnf.setLength(0);
              htName2index.put(
                Convert.hashCode(
                  sbufnf.append(tableName).append('.').append(field.alias)),
                i);
            } else if (!htName2index.exists(hash)) {
              htName2index.put(
                hash,
                i); // Stores the name of the field once; only the first.
            }
          } else { // Verifies if the column name in the field list is
                   // ambiguous.
            table = null;
            rsTable = null;
            foundFirst = false;
            j = tableList.length;
            while (--j >= 0) {
              auxIndex = (auxTable = (rsTableAux = tableList[j]).table)
                           .htName2index.get(field.tableColHashCode, -1);
              if (auxIndex != -1) {
                if (foundFirst)
                  throw new SQLParseException(
                  ...
                  sbufnf.setLength(0);
                  htName2index.put(
                    Convert.hashCode(sbufnf.append(tableList[j].tableName)
                                       .append('.')
                                       .append(field.alias)),
                    i);
                  htName2index.put(field.aliasHashCode, i);
        ...
        if (!field.isVirtual) { // If the field is not virtual, it needs to be
          ...
          htName2index.put(field.aliasHashCode, i);
                   ...
                   field.isDataTypeFunction) { // If it is an aggregated or data
                                               // type function, maps its
                                               // parameter.
          param = field.parameter;
          aggFunctionType = dtFunctionType =
            SQLElement.UNDEFINED; // initialized with an UNDEFINED number.
          ...
          if (param != null) {
            ...
            if (field.isAggregatedFunction) { // rnovais@568_10
              // Checks if the parameter and aggregated function data types are
              // compatible. juliana@226_5
              if ((aggFunctionType == SQLElement.INT ||
                   aggFunctionType == SQLElement.DOUBLE) &&
                  (param.dataType == SQLElement.CHARS ||
                   param.dataType == SQLElement.CHARS_NOCASE))
                throw new SQLParseException(
                  LitebaseMessage.getMessage(
                    LitebaseMessage.ERR_INCOMPATIBLE_TYPES) +
                  ' ' + SQLElement.aggregateFunctionsNames[sqlFunction]);

              // For aggregated functions, if the function does not have a
              // defined data type, it inherits the parameter size and type.
              ...
              // rnovais@570_5: if UNDEFINED the datatype will be the same of
              // the field thus, it is possible to have functions that can be
              // applyed to diferents fieds type. e.g. ABS(int) returns int,
              // ABS(double) returns double, etc.
              if (dtFunctionType == SQLElement.UNDEFINED) {
                field.dataType = param.dataType;
              }

              // Checks if the parameter and the data type function data types
              // are compatible.
