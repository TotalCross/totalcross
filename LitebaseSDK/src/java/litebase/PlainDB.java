...
class PlainDB {
	...
	PlainDB(String aName, String sourcePath, boolean create) throws IOException {
		...
		try {
			dbo = openFile(aName == null ? null : aName + NormalFile.DBO_EXT, create, sourcePath);
					 ...
					 exception) { // juliana@222_7: .db should be closed if .dbo cannot be openned on desktop and BlackBerry.
			db.close();
			throw exception;
		}
	}
	...
	void writeMetaData(byte[] buf, int len) throws IOException {
		...
		if (dbFile.size ==
			0) { // The metadata size must have a free space for future composed indices or composed primary key.
			int size = headerSize;
	...
	void close(boolean updatePos) throws IOException {
		...
		if (updatePos)
			db.finalPos = rowCount * rowSize +
						  headerSize; // Calculates .db used space: .db won't have zeros at the end.
	 ...
	 */
	int readValue(SQLValue value, int offset, int colType, DataStreamLB stream, boolean isTemporary,
				  boolean isNull, boolean isTempBlob)
					...
					if (pos < dbo.finalPos && pos >= 0) {
						...
						if (value.asInt != -1) {
							...
							if (value.asBlob.length >
								0) { // juliana@212_8: when reading a file, an exception must not be thrown when reading zero bytes.
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	void writeValue(int type, SQLValue value, DataStreamLB ds, boolean valueOk, boolean addingNewRecord,
					int colSize, int offset, boolean isTemporary)
	throws IOException {
		if (!valueOk) { // Only writes non-null values and values being changed.
			ds.skipBytes(
				Utils.typeSizes[type]);    // If the value is null or is not updated, just skips its size.
		} else
			switch (type) {
				// juliana@226_9: strings are not loaded anymore in the temporary table when building result sets.
				case SQLElement.CHARS_NOCASE:
				case SQLElement.CHARS: {
					...
					if (isTemporary) {
						...
						if ((dboFile.finalPos + size) > dboFile.size)
							dboFile.growTo(dboFile.size + 2 + size * (rowInc > 16 ? rowInc :
						...
						if (isAscii) { // juliana@210_2: now Litebase supports tables with ascii strings.
							...
							int i = -1;
							dsdbo.writeShort(
								c); // juliana@214_5: must trim ascii strings if they are longer than the field size definition.
							while (++i < c) {
				...
				case SQLElement.BLOB: {
					...
					if (isTemporary) { // The position of a blob and its table is being written to the temporary table.
						...
						int size = Math.min(value.asBlob.length,
											colSize), // Trims a blob that is bigger than its field definiton.
								   oldPos = 0;
						...
						if (addingNewRecord && (dboFile.finalPos + size + 4) >= (dboFile.size + 1))
							dboFile.growTo(dboFile.size + 4 + size * (rowInc > 16 ? rowInc :
										   16)); // If the .dbo is full, grows it.
						...
						if (size >
							0) { // juliana@212_8: when reading a file, an exception must not be thrown when writing zero bytes.
