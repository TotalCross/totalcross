...
/**
 * This has the implementation of a database in a plain binary file. The data
 * and the metadata (header) is written in one file (.db). The strings and the
 * blobs are written in the .dbo file. The current number of records inside the
 * database is discovered only when the database is open by getting its size and
 * discounting the header size. This has a double advantage: it is not necessary
 * to waste space storing the current record count, and it is not needed to save
 * the record count at each insertion.
 *
 * This also has the implementation of a temporary database for
...
class PlainDB
{
  ...
  PlainDB(String aName, String sourcePath, boolean create) throws IOException
  {
    ...
    db = openFile(aName == null ? null : (name = aName) + NormalFile.DB_EXT,
                  create,
                  sourcePath);
    ...
    try {
      dbo = openFile(
        aName == null ? null : aName + NormalFile.DBO_EXT, create, sourcePath);
    } catch (
      IOException exception) { // juliana@222_7: .db should be closed if .dbo
                               // cannot be openned on desktop and BlackBerry.
      db.close();
      throw exception;
    }
  }
  ...
  void setRowSize(int newRowSize, byte[] buffer)
  {
    ...
    basds =
      new DataStreamLB(bas = new ByteArrayStream(basbuf = buffer), useCrypto);
   ...
   * @throws IOException If an internal method throws it.
   */
  XFile openFile(String name, boolean create, String sourcePath)
    throws IOException
  {
    if (name == null) {
      ...
      return new NormalFile(
        Utils.getFullFileName(name, sourcePath), create, -1);
    }
  }

  /**
   * Adds a new record. The file pointer is positioned in the record's beginning
   * so that the data can be written. Usually the record is first added, then
  ...
  void add() throws IOException
  {
    ...
    db.setPos(headerSize +
              rowCount *
                rowSize); // Sets the position to the start of the record.
  ...
  // juliana@253_19: corrected a possible table corruption after a purge or a
  // rename table only on Java SE.
  /**
  ...
  {
    String newFullName =
      Utils.getFullFileName(newName, sourcePath) + NormalFile.DB_EXT;

    ((NormalFile)db)
      .rename(newFullName); // rnovais@570_75  // Renames the .db file.
    try {
      ((NormalFile)dbo)
        .rename(newFullName + 'o'); // Renames the .dbo file. // rnovais@570_75
  ...
  void writeMetaData(byte[] buf, int len) throws IOException
  {
    ...
    if (dbFile.size == 0) { // The metadata size must have a free space for
                            // future composed indices or composed primary key.
      int size = headerSize;

      // juliana@230_7: corrected a possible exception or crash when the table
      // has too many columns and composed indices or PKs.
      ...
      // very large metadata size. juliana@253_8: now Litebase supports weak
      // cryptography.
      dbFile.setPos(4);
   ...
   *
   * @param updatePos Indicates if <code>finalPos</code> must be re-calculated
   *     to shrink the file.
   * @throws IOException If an internal method throws it.
   */
  void close(boolean updatePos) throws IOException
  {
    ...
    DataStreamLB tsmdDs =
      new DataStreamLB(tsmdBas, useCrypto); // Creates a new stream.
    ...
    tsmdDs.writeByte(isAscii ? (Table.IS_ASCII | Table.IS_SAVED_CORRECTLY)
    ...
    if (updatePos)
      db.finalPos =
        rowCount * rowSize + headerSize; // Calculates .db used space: .db
                                         // won't have zeros at the end.
  ...
  // building result sets. juliana@230_14: removed temporary tables when there
  // is no join, group by, order by, and aggregation. juliana@253_8: now
  // Litebase supports weak cryptography.
  /**
   ...
   * @param isTempBlob Indicates if the blob is being read for a temporary
   *     table.
   * @return The total offset in the row.
   * @throws IOException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   */
  int readValue(SQLValue value,
                int offset,
                int colType,
                DataStreamLB stream,
                boolean isTemporary,
    ...
    switch (colType) {
      ...
      case SQLElement.CHARS_NOCASE:
        if (isTemporary) {
          ...
          if ((value.asInt = stream.readInt()) < dbo.finalPos &&
              value.asInt >= 0) {
            dbo.setPos(value.asInt); // Reads the string position in the .dbo
      ...
      case SQLElement.INT:
        ...
        // juliana@230_38: corrected possible indices problems when updating an
        // integer field on JavaSE and BlackBerry.
        if (((ByteArrayStream)stream.getStream()).getPos() == 4 &&
            !isTemporary) { // Is it the row id?
          value.asInt =
            value.asInt & Utils.ROW_ID_MASK; // Masks out the attributes.
        }
        break;
      ...
      case SQLElement.BLOB: // juliana@220_3: blobs are not loaded anymore in
        ...
        if (isTempBlob) { // A blob is being read to a temporary table.
          ...
          if (pos < dbo.finalPos && pos >= 0) {
            dbo.setPos(pos); // Reads the blob position in the .dbo and sets its
                             // position.
            if (value.asInt != -1) {
              value.asBlob =
                new byte[dsdbo.readInt()]; // Creates the blob with its size.
              if (value.asBlob.length >
                  0) { // juliana@212_8: when reading a file, an exception must
                       // not be thrown when reading zero bytes.
  ...
  // juliana@220_3: blobs are not loaded anymore in the temporary table when
  // building result sets. juliana@253_8: now Litebase supports weak
  // cryptography.
  /**
   ...
   * @throws IOException If an internal method throws it.
   */
  void writeValue(int type,
                  SQLValue value,
                  DataStreamLB ds,
                  boolean valueOk,
                  boolean addingNewRecord,
                  int colSize,
                  int offset,
                  boolean isTemporary) throws IOException
  {
    if (!valueOk) { // Only writes non-null values and values being changed.
      ds.skipBytes(Utils.typeSizes[type]); // If the value is null or is not
                                           // updated, just skips its size.
    } else
      switch (type) {
        // juliana@226_9: strings are not loaded anymore in the temporary table
        // when building result sets.
        ...
        case SQLElement.CHARS: {
          ...
          if (isTemporary) {
            if ((dboFile.finalPos + 8) > dboFile.size) {
              dboFile.growTo(dboFile.size +
                             8 * (rowInc > 16
                                    ? rowInc
                                    : 16)); // If the .dbo is full, grows it.
            }
            ...
            // juliana@225_7.
            // juliana@210_2: now Litebase supports tables with ascii strings.
            int c = value.asString
                      .length(), // The string that is bigger than its field
                                 // definiton was already trimmed.
              size =
                isAscii ? c + 2 : (c << 1) + 2; // Computes the string size.

            // guich@201_8: grows using rowInc instead of 16 if rowInc > 16.
            // juliana@201_20: only grows .dbo if it is going to be increased.
            // juliana@212_7: The size of the string must be taken into
            // consideration because it can be zero.
            if ((dboFile.finalPos + size) > dboFile.size)
              dboFile.growTo(dboFile.size + 2 +
                             size * (rowInc > 16
                                       ? rowInc
                                       : 16)); // If the .dbo is full, grows it.

            // juliana@202_21: Always writes the string at the end of the .dbo.
            // This removes possible bugs when doing updates.
            dboFile.setPos(dboFile.finalPos);
            ds.writeInt(value.asInt =
                          dboFile.pos); // The string position for an index and
                                        // writes it in the .db

            // Writes the string to the buffer.
            if (isAscii) { // juliana@210_2: now Litebase supports tables with
                           // ascii strings.
              String asString = value.asString;
              int i = -1;
              dsdbo.writeShort(
                c); // juliana@214_5: must trim ascii strings if they are longer
                    // than the field size definition.
              while (++i < c) {
            ...
            dboFile.finalPos = dboFile.pos; // juliana@202_21: the final positon
        ...
        case SQLElement.BLOB: {
          // juliana@220_3: blobs are not loaded anymore in the temporary table
          // when building result sets.
          XFile dboFile = dbo;

          if (isTemporary) { // The position of a blob and its table is being
            ...
            int size = Math.min(value.asBlob.length,
                                colSize), // Trims a blob that is bigger than
                                          // its field definiton.
              oldPos = 0;

            // guich@201_8: grows using rowInc instead of 16 if rowInc > 16.
            // juliana@201_20: only grows .dbo if it is going to be increased.
            // juliana@212_7: The size of the blob must be taken into
            // consideration because it can be zero.
            if (addingNewRecord &&
                (dboFile.finalPos + size + 4) >= (dboFile.size + 1))
              dboFile.growTo(dboFile.size + 4 +
                             size * (rowInc > 16
                                       ? rowInc
                                       : 16)); // If the .dbo is full, grows it.
            ...
            if (size > 0) { // juliana@212_8: when reading a file, an exception
                            // must not be thrown when writing zero bytes.
              dsdbo.writeBytes(value.asBlob,
                               0,
                               size); // Writes the blob itself to .dbo.
            }

            // It is an insert or the size of the blob is greater then the old
            // one, the final positon is the new positon.
            if (addingNewRecord) {
              dboFile.finalPos = dboFile.pos;
            }
  ...
  boolean recordNotDeleted() throws IOException
  {
    ...
    boolean notDeleted =
      (basds.readInt() & Utils.ROW_ATTR_MASK) != Utils.ROW_ATTR_DELETED;
    bas.reset(); // Resets read position.
    return notDeleted;
  }
  ...
  String loadString() throws IOException
  {
    ...
    if (isAscii) { // juliana@210_2: now Litebase supports tables with ascii
                   // strings.
