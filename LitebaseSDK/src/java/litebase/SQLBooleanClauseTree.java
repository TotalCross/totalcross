...
class SQLBooleanClauseTree {
		...
		switch (valueType) {
			...
			case SQLElement.DATETIME: // If the type is DATETIME, checks if it is valid and converts it to 2 ints.
				...
				if (pos == -1) { // If it has only a date...
					...
					value.asInt = booleanClause.tempDate.set(val.substring(0, pos),
	...
	void weightTheTree() {
		switch (operandType) { // Checks the type of the operand.
			...
			default: // The others.
				...
				SQLResultSetField leftField = booleanClauseAux.fieldList[booleanClauseAux.fieldName2Index.get(
												  left.nameSqlFunctionHashCode, 0)],
											  rightField = booleanClauseAux.fieldList[booleanClauseAux.fieldName2Index.get(
	...
	void setIndexRsOnTree() {
		switch (operandType) { // Checks the operand type.
			...
			case SQLElement.OP_REL_DIFF:
				...
				if (left.operandType == SQLElement.OP_IDENTIFIER || right.operandType == SQLElement.OP_IDENTIFIER) {
					int lIdx = clause.fieldName2Index.get(left.nameSqlFunctionHashCode != 0 ?
	 ...
	 */
	void getBranchProperties(byte[] columns, byte[] operators, SQLBooleanClauseTree[] indexesValueTree,
							 int pos) {
	...
	SQLValue getOperandValue() throws IOException, InvalidDateException {
		...
		if (operandType == SQLElement.OP_IDENTIFIER) {
			...
			SQLResultSetField field = clause.fieldList[clause.fieldName2Index.get(nameSqlFunctionHashCode,
									  -1)]; // rnovais@570_108
	...
	boolean compareNullOperands() throws IOException {
		...
		table.readNullBytesOfRecord(0, false, 0);
		boolean isNull = (table.columnNulls[0][leftTree.colIndex >> 3] & (1 <<
						  (leftTree.colIndex & 7))) != 0;
	...
	boolean compareNumericOperands() throws IOException, InvalidDateException, InvalidNumberException {
		...
		int leftValueType = left.valueType,
			...
			assignType = isFloatingPointType ? SQLElement.DOUBLE : leftValueType != SQLElement.LONG
		...
		if (leftValueType == -1
			...
			if (lastChar == 'l'
				|| lastChar == 'L') { // Strips off the final 'L', because toDouble() does not like it.
			...
			compareType =
				SQLElement.DOUBLE; // juliana@225_12: a numeric constant in a boolean clause must have its type considered to be double.
		}
		...
		switch (operandType) { // Then performs the comparison.
			...
			case SQLElement.OP_REL_DIFF:
				switch (compareType) {
					case SQLElement.DOUBLE:
						result = (Convert.doubleToLongBits(leftValueAsDouble) == Convert.doubleToLongBits(
									  rightValueAsDouble));
						break;
					case SQLElement.INT:
	...
	boolean matchStringOperands(boolean ignoreCase) throws IOException, InvalidDateException {
		...
		leftString = Utils.formatDateDateTime(booleanClause.resultSet.table.db.driver.sBuffer,
											  leftTree.valueType, leftValue);

		if (ignoreCase) {
			leftString = leftString.toLowerCase();
			if (strToMatch !=
				null) { // juliana@220_9: solved a NPE when using like '%' with a nocase string column.
		...
		switch (right.patternMatchType) {
			...
			case SQLBooleanClauseTree.PAT_MATCH_MIDDLE: // rnovais@568_1
				...
				result = leftString.startsWith(strToMatch.substring(0,
											   pos)) ? (leftString.endsWith(strToMatch.substring(pos + 1))) : false;
				break;
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	void bindColumnsSQLBooleanClauseTree() throws SQLParseException, InvalidDateException,
		InvalidNumberException {
		if (operandType ==
			SQLElement.OP_IDENTIFIER) { // If operand type is identifier, binds to a column in the table.
			...
			if (field.sqlFunction ==
				-1) { // rnovais@568_10: if it does not have a data function, stores the correct value type.
	...
	void inferOperationValueType() throws SQLParseException, InvalidNumberException {
		...
		if (left.operandName !=
			null) { // rnovais@568_10: if it has a data type function, verifies if it can be applied.
			SQLResultSetField field = left.booleanClause.fieldList[left.booleanClause.fieldName2Index.get(
										  left.nameSqlFunctionHashCode == 0 ?
		...
		if (leftIsParameter || rightIsParameter) {
			...
			boolean leftIsChar = (leftValueType == SQLElement.CHARS || leftValueType == SQLElement.CHARS_NOCASE
								  || leftValueType == SQLElement.DATE
								  || leftValueType == SQLElement.DATETIME),
								 rightIsChar = (rightValueType == SQLElement.CHARS || rightValueType == SQLElement.CHARS_NOCASE
												|| leftValueType == SQLElement.DATE
												|| leftValueType == SQLElement.DATETIME);
				...
				switch (operandType) {
					...
					case SQLElement.OP_REL_DIFF:
						if (leftIsChar || rightIsChar) {
							if (leftValueType == SQLElement.DATE || rightValueType == SQLElement.DATE) { // rnovais@567_2
							...
							} else if (leftValueType == SQLElement.DATETIME || rightValueType == SQLElement.DATETIME) {
								...
								if (leftOperandType == SQLElement.OP_IDENTIFIER && rightOperandType == SQLElement.OP_IDENTIFIER)
									valueType = (leftValueType == SQLElement.CHARS
												 || rightValueType == SQLElement.CHARS) ? SQLElement.CHARS
							...
							} else if (leftValueType == SQLElement.FLOAT || rightValueType == SQLElement.FLOAT) {
								valueType = SQLElement.FLOAT;
							} else if (leftValueType == SQLElement.LONG || rightValueType == SQLElement.LONG) {
								valueType = SQLElement.LONG;
							} else if (leftValueType == SQLElement.INT || rightValueType == SQLElement.INT) {
