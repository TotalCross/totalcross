...
class SQLBooleanClauseTree
{
  ...
  /**
   * Indicates if there are two '%' symbols to compare if a string contains
   * another.
   */
  ...
  /**
   * Indicates if there is a match character in the middle of the string to be
   * matched.
   */
  ...
  /**
   * Indicates if it has an associated index. Used on join <code>table1.field1 =
   * table2.field2</code>.
   */
  boolean hasIndex;
  ...
  /**
   * The current value. Used only on joins comparing table fields like
   * <code>table1.field1 <operator> table2.field2</code>.
   */
  SQLValue valueJoin;
   ...
   */
  void setOperandStringLiteral(String value)
  { // juliana@201_34
   ...
   */
  void setParamValue(short val) throws DriverException
  { // juliana@201_34
   ...
   */
  void setParamValue(int val) throws DriverException
  { // juliana@201_34
   ...
   */
  void setParamValue(long val) throws DriverException
  { // juliana@201_34
   ...
   */
  void setParamValue(float val) throws DriverException
  { // juliana@201_34
   ...
   */
  void setParamValue(double val) throws DriverException
  { // juliana@201_34
    createValueAndTestType(SQLElement.DOUBLE);
    operandValue.asDouble = val;
  }

  // juliana@201_34: setString() for a where clause wouldn't work if the table
  // column wasn't a date, datetime, chars or chars nocase. juliana@222_9: Some
  // string conversions to numerical values could return spourious values if the
  // string range were greater than the type range.
  /**
    ...
    throws InvalidDateException, InvalidNumberException
  {
    ...
    switch (valueType) {
      ...
      case SQLElement.DATE: // rnovais@570_55: If the type is DATE, checks if it
                            // is valid and converts it to int.
        value.asInt = booleanClause.tempDate.set(val.trim(), Settings.DATE_YMD);
        break;

      case SQLElement.DATETIME: // If the type is DATETIME, checks if it is
                                // valid and converts it to 2 ints.
        val = val.trim();
        int pos = val.lastIndexOf(' ');
        if (pos == -1) { // If it has only a date...
          value.asInt = booleanClause.tempDate.set(
            val, Settings.DATE_YMD); // Gets the date part.
          value.asShort = 0;
        } else {
          value.asInt = booleanClause.tempDate.set(
            val.substring(0, pos),
            Settings.DATE_YMD); // Gets the date part.
          value.asShort = Utils.testAndPrepareTime(
            val.substring(pos + 1)); // Gets the time part.
        }
        break;
  ...
  void weightTheTree()
  {
    switch (operandType) { // Checks the type of the operand.
      ...
      default: // The others.
        SQLBooleanClauseTree left = leftTree, right = rightTree;
        SQLBooleanClause booleanClauseAux = booleanClause;
        SQLResultSetField
          leftField =
            booleanClauseAux.fieldList[booleanClauseAux.fieldName2Index.get(
              left.nameSqlFunctionHashCode, 0)],
          rightField =
            booleanClauseAux.fieldList[booleanClauseAux.fieldName2Index.get(
              right.nameSqlFunctionHashCode, 0)];
        Index leftIndex =
                leftField.table.columnIndices[leftField.tableColIndex],
              rightIndex =
                rightField.table.columnIndices[rightField.tableColIndex];

        // field.indexRs is filled on the where clause validation. Both are
        // identifiers.
        if (left.operandType == SQLElement.OP_IDENTIFIER &&
            right.operandType == SQLElement.OP_IDENTIFIER) {
          if (leftIndex != null) {
  ...
  void setIndexRsOnTree()
  {
    switch (operandType) { // Checks the operand type.
      ...
      case SQLElement.OP_REL_DIFF:
        ...
        if (left.operandType == SQLElement.OP_IDENTIFIER ||
            right.operandType == SQLElement.OP_IDENTIFIER) {
          int lIdx = clause.fieldName2Index.get(
                left.nameSqlFunctionHashCode != 0 ? left.nameSqlFunctionHashCode
                                                  : left.nameHashCode,
                -1),
              rIdx =
                clause.fieldName2Index.get(right.nameSqlFunctionHashCode != 0
                                             ? right.nameSqlFunctionHashCode
                                             : right.nameHashCode,
                                           -1);
          if (left.operandType == SQLElement.OP_IDENTIFIER &&
              right.operandType == SQLElement.OP_IDENTIFIER) {
            ...
            // juliana@263_2: corrected a very old bug in a join with
            // comparision between two fields of the same table.
            ...
            indexRs =
              clause
                .fieldList[left.operandType == SQLElement.OP_IDENTIFIER ? lIdx
                                                                        : rIdx]
                .indexRs;
          }
        }
  ...
  /**
   * Used for composed indices to find some properties related to a branch of
   * the expression tree.
   *
   ...
   */
  void getBranchProperties(byte[] columns,
                           byte[] operators,
                           SQLBooleanClauseTree[] indexesValueTree,
                           int pos)
  {
    if (pos >=
        columns.length) { // Does not let an <code>OutOfBoundsException</code>.
      return;
    }
    ...
    } else if (rightTree.operandType == SQLElement.OP_IDENTIFIER) {
  ...
  /**
   * Gets a value from a result set and applies a sql function if there is one
   * to be applied.
   *
   * @return The value returned from the result set or the result of a function
   *     on a value of the result set.
  ...
  SQLValue getOperandValue() throws IOException, InvalidDateException
  {
    ...
    if (operandType == SQLElement.OP_IDENTIFIER) {
      ...
      if ((table.columnNulls[0][colIndex >> 3] & (1 << (colIndex & 7))) !=
          0) { // There is a null value.
        return null;
      }

      // guich@tc100b4: replaced "new SQLValue" by tree.tempValue to avoid
      // allocating this (+ 5000 times in AllTests).
      v = clause.resultSet.sqlwhereclausetreeGetTableColValue(colIndex,
                                                              tempValue);

      // rnovais@568_10: applies data type function.
      SQLResultSetField field =
        clause.fieldList[clause.fieldName2Index.get(nameSqlFunctionHashCode,
                                                    -1)]; // rnovais@570_108
  ...
  boolean compareNullOperands() throws IOException
  {
    ...
    table.readNullBytesOfRecord(0, false, 0);
    boolean isNull = (table.columnNulls[0][leftTree.colIndex >> 3] &
                      (1 << (leftTree.colIndex & 7))) != 0;
   ...
   */
  boolean compareNumericOperands()
    throws IOException, InvalidDateException, InvalidNumberException
  {
    boolean result = false;
    SQLBooleanClauseTree left = leftTree, right = rightTree;
    SQLValue leftValue =
               bothAreIdentifier ? left.valueJoin : left.getOperandValue(),
             rightValue = right.getOperandValue();

    if (leftValue == null ||
        rightValue == null) { // One of the values is a null value.
      return false;
    }

    int leftValueType = left.valueType, rightValueType = right.valueType,
        assignType = isFloatingPointType
                       ? SQLElement.DOUBLE
                       : leftValueType != SQLElement.LONG &&
                             rightValueType != SQLElement.LONG &&
                             leftValueType != SQLElement.DATETIME
                           ? SQLElement.INT
                           : SQLElement.LONG,
        compareType = leftValueType == SQLElement.DATETIME ? SQLElement.DATETIME
                                                           : assignType,
        leftValueAsInt = 0, rightValueAsInt = 0, leftValueAsTime = 0,
        rightValueAsTime = 0;
    long leftValueAsLong = 0L, rightValueAsLong = 0L;
    ...
    switch (rightValueType) { // Getting right value.
      ...
      case SQLElement.CHARS:    // rnovais@567_2 : this is for DATE and DATETIME
                                // typed that are CHARS type in the sql.
    ...
    if (leftValueType == -1 ||
      ...
      String leftValueAsString = leftValue.asString, rightValueAsString =
                                                       rightValue.asString;
      int last = leftValueAsString.length() - 1;
      char lastChar = leftValueAsString.charAt(last);

      if (lastChar == 'l' ||
          lastChar == 'L') { // Strips off the final 'L', because toDouble()
                             // does not like it.
      ...
      leftValue.asDouble = leftValueAsDouble =
        Convert.toDouble(leftValueAsString);
      rightValue.asDouble = rightValueAsDouble =
        Convert.toDouble(rightValueAsString);
      left.valueType = right.valueType = SQLElement.DOUBLE;
      isFloatingPointType = true;

      compareType =
        SQLElement.DOUBLE; // juliana@225_12: a numeric constant in a boolean
                           // clause must have its type considered to be double.
    }
    ...
    switch (operandType) { // Then performs the comparison.
      ...
      case SQLElement.OP_REL_DIFF:
        switch (compareType) {
          case SQLElement.DOUBLE:
            result = (Convert.doubleToLongBits(leftValueAsDouble) ==
                      Convert.doubleToLongBits(rightValueAsDouble));
            break;
          case SQLElement.INT:
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  boolean matchStringOperands(boolean ignoreCase)
    throws IOException, InvalidDateException
  {
    ...
    String leftString = leftValue.asString, strToMatch = right.strToMatch;

    // juliana@230_3: corrected a bug of LIKE using DATE and DATETIME not
    // returning the correct result.
    leftString =
      Utils.formatDateDateTime(booleanClause.resultSet.table.db.driver.sBuffer,
                               leftTree.valueType,
                               leftValue);

    if (ignoreCase) {
      leftString = leftString.toLowerCase();
      if (strToMatch != null) { // juliana@220_9: solved a NPE when using like
                                // '%' with a nocase string column.
    ...
    switch (right.patternMatchType) {
      ...
      case SQLBooleanClauseTree.PAT_MATCH_MIDDLE: // rnovais@568_1
        ...
        result = leftString.startsWith(strToMatch.substring(0, pos))
                   ? (leftString.endsWith(strToMatch.substring(pos + 1)))
                   : false;
        break;
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  boolean compareStringOperands(boolean ignoreCase)
    throws IOException, InvalidDateException
  {
    int result;

    SQLValue leftValue = bothAreIdentifier ? leftTree.valueJoin
                                           : leftTree.getOperandValue(),
    ...
    String leftString = leftValue.asString, rightString = rightValue.asString;
    ...
    throws IOException, InvalidDateException, InvalidNumberException
  {
    ...
    switch (operandType) { // Checks the operand type of the tree.
      ...
      case SQLElement.OP_REL_LESS_EQUAL:
      case SQLElement.OP_REL_DIFF:
        switch (valueType) { // Calls the right operation accordingly to the
                             // values type.
          case SQLElement.SHORT:
          case SQLElement.INT:
          case SQLElement.LONG:
      ...
      case SQLElement.OP_BOOLEAN_AND: // AND connector.
        if (leftTree != null && rightTree != null) {
          ...
          if (leftResult) { // Short circuit: only evaluates the right tree if
                            // left result is TRUE.
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  void bindColumnsSQLBooleanClauseTree()
    throws SQLParseException, InvalidDateException, InvalidNumberException
  {
    if (operandType ==
        SQLElement.OP_IDENTIFIER) { // If operand type is identifier, binds to a
                                    // column in the table.
      int dtParameter;              // rnovais@568_10

      // Stores the binding information also in the boolean clause field list.
      int fieldIndex =
        booleanClause.fieldName2Index.get(nameSqlFunctionHashCode, -1);
      ...
      if (field.sqlFunction ==
          -1) { // rnovais@568_10: if it does not have a data function, stores
    ...
    if (left != null &&
        right != null) { // Infers the operation resulting value type.
      inferOperationValueType();
    }

    // rnovais@567_2: validates date and datetime in the rightTree.
    if ((left != null) && right.operandValue != null &&
        (left.valueType == SQLElement.DATE ||
         left.valueType == SQLElement.DATETIME)) {
      right.operandValue.validateDateTime(booleanClause.tempDate,
                                          left.valueType);
    }
  }
   ...
   *
   * @throws SQLParseException If left and right values have incompatible types
   *     or there is a blob type in the comparison.
   * @throws InvalidNumberException If an internal method throws it.
   */
  void inferOperationValueType()
    throws SQLParseException, InvalidNumberException
  {
    SQLBooleanClauseTree left = leftTree, right = rightTree;
    ...
    if (left.operandName != null) { // rnovais@568_10: if it has a data type
                                    // function, verifies if it can be applied.
      SQLResultSetField field =
        left.booleanClause.fieldList[left.booleanClause.fieldName2Index.get(
          left.nameSqlFunctionHashCode == 0 ? left.nameHashCode
                                            : left.nameSqlFunctionHashCode,
          -1)];
    ...
    if (operandType == SQLElement.OP_BOOLEAN_AND ||
        operandType == SQLElement.OP_BOOLEAN_OR) { // Boolean type.
      valueType = SQLElement.BOOLEAN;
      return;
    }

    int leftOperandType = left.operandType,
        rightOperandType = right.operandType, leftValueType = left.valueType,
        rightValueType = right.valueType;

    if (leftValueType == SQLElement.BLOB ||
        rightValueType == SQLElement.BLOB) { // Blobs can't be compared.
      throw new SQLParseException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_COMP_BLOBS));
    }

    boolean leftIsParameter = left.isParameter, rightIsParameter =
                                                  right.isParameter;

    // In case one of them is a parameter, the tree has the type of the one that
    // is not a parameter. If both are parameters, the type is undefined (which
    ...
    if (leftIsParameter || rightIsParameter) {
      ...
      boolean leftIsChar = (leftValueType == SQLElement.CHARS ||
                            leftValueType == SQLElement.CHARS_NOCASE ||
                            leftValueType == SQLElement.DATE ||
                            leftValueType == SQLElement.DATETIME),
              rightIsChar = (rightValueType == SQLElement.CHARS ||
                             rightValueType == SQLElement.CHARS_NOCASE ||
                             leftValueType == SQLElement.DATE ||
                             leftValueType == SQLElement.DATETIME);
      ...
      if (rightOperandType == SQLElement.OP_IDENTIFIER &&
          leftOperandType != SQLElement.OP_IDENTIFIER) {
        valueType = rightValueType;
        left.convertValue(rightValueType);
      } else if (leftOperandType == SQLElement.OP_IDENTIFIER &&
                 rightOperandType != SQLElement.OP_IDENTIFIER) {
        valueType = leftValueType;
        ...
        switch (operandType) {
          ...
          case SQLElement.OP_REL_DIFF:
            if (leftIsChar || rightIsChar) {
              if (leftValueType == SQLElement.DATE ||
                  rightValueType == SQLElement.DATE) { // rnovais@567_2
                valueType = SQLElement.DATE;
              } else if (leftValueType == SQLElement.DATETIME ||
                         rightValueType == SQLElement.DATETIME) {
                valueType = SQLElement.DATETIME;
              } else {
                // If both are identifiers, it prevails CHARS, in case one of
                // them is CHARS (case sensitive).
                if (leftOperandType == SQLElement.OP_IDENTIFIER &&
                    rightOperandType == SQLElement.OP_IDENTIFIER)
                  valueType = (leftValueType == SQLElement.CHARS ||
                               rightValueType == SQLElement.CHARS)
                                ? SQLElement.CHARS
              ...
              if (leftValueType == SQLElement.DOUBLE ||
                  rightValueType == SQLElement.DOUBLE) {
                valueType = SQLElement.DOUBLE;
              } else if (leftValueType == SQLElement.FLOAT ||
                         rightValueType == SQLElement.FLOAT) {
                valueType = SQLElement.FLOAT;
              } else if (leftValueType == SQLElement.LONG ||
                         rightValueType == SQLElement.LONG) {
                valueType = SQLElement.LONG;
              } else if (leftValueType == SQLElement.INT ||
                         rightValueType == SQLElement.INT) {
          ...
          case SQLElement.OP_PAT_MATCH_NOT_LIKE:
            if (!leftIsChar ||
              ...
              throw new SQLParseException(LitebaseMessage.getMessage(
                LitebaseMessage.ERR_INCOMPATIBLE_TYPES));
            }
            ...
            if (leftValueType == SQLElement.DATE ||
              ...
              if (leftOperandType == SQLElement.OP_IDENTIFIER &&
                  rightOperandType == SQLElement.OP_IDENTIFIER) {
                if (leftValueType == SQLElement.CHARS ||
                    rightValueType == SQLElement.CHARS) {
                  valueType = SQLElement.CHARS;
                } else {
    ...
    isFloatingPointType =
      (valueType == SQLElement.DOUBLE || valueType == SQLElement.FLOAT);
  }
  ...
  // juliana@214_4: removes not from expression trees so that indices can be
  ...
  {
    if (expressionTree ==
        null) { // Does nothing with an empty expression is used.
      return null;
    }
      ...
      switch (right.operandType) {
        case SQLElement.OP_BOOLEAN_AND: // not (A and B) == not A or not B.
          SQLBooleanClauseTree tree =
            new SQLBooleanClauseTree(expressionTree.booleanClause);
  ...
  SQLBooleanClauseTree cloneTree(SQLBooleanClauseTree destTree)
  {
    ...
    if (leftTree != null) {
      (tree.leftTree =
         leftTree.cloneTree(destTree == null ? null : destTree.leftTree))
        .parent = tree;
    }
    if (rightTree != null) {
      (tree.rightTree =
         rightTree.cloneTree(destTree == null ? null : destTree.rightTree))
        .parent = tree;
    }
    return tree;
  }
  ...
  private void createValueAndTestType(int type)
  {
    ...
    // juliana@230_37: solved a possible bug when using prepared statements
    // without issuing PreparedStatement.clearAllParameters().
