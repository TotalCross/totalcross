...
public class ResultSet {
	...
	public String[][] getStrings(int count) throws DriverException, IllegalArgumentException {
		...
		if (pos < 0
			|| pos > last) { // The position of the cursor must be greater then 0 and less then the last position.
		...
		try {
			do {
				...
				validRecords++; // juliana@211_4: solved bugs with result set dealing.
					 ...
					 && next()); // Continues until there are rows to be read and the number of rows read is not the desired.
		} catch (IOException exception) {
			throw new DriverException(exception);
	...
	public void setDecimalPlaces(int col, int places) throws DriverException {
		...
		if (places < -1 || places > 40) // Invalid value for decimal places.
			throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_RS_DEC_PLACES_START) +
									  places
									  + LitebaseMessage.getMessage(LitebaseMessage.ERR_RS_DEC_PLACES_END));

		if (type == SQLElement.FLOAT
			|| type == SQLElement.DOUBLE) { // Only sets the decimal places if the type is FLOAT or DOUBLE.
			byte[] decimals = decimalPlaces;
	...
	public int getRowCount() {
		verifyResultSet(); // The driver or result set can't be closed.
		return allRowsBitmap == null ? lastRecordIndex + 1 - table.deletedRowsCount :
	...
	public String rowToString() throws DriverException {
		...
		if (pos < 0
			|| pos > lastRecordIndex) { // The position of the cursor must be greater then 0 and less then the last position.
			throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_RS_INV_POS));
		}
	 ...
	 */
	SQLValue sqlwhereclausetreeGetTableColValue(int col, SQLValue val) throws IOException,
		InvalidDateException {
		Table tableAux = table;

		// juliana@220_3
		table.readValue(val, tableAux.columnOffsets[col], tableAux.columnTypes[col],
	...
	boolean getNextRecord() throws IOException, InvalidDateException, InvalidNumberException {
		...
		if (rowsBitmapAux != null) { // Desired rows partially computed using the indexes?
			...
			if (pos < last) {
				if (clause == null) {
					...
					if (rowsBitmapBoolOp == SQLElement.OP_BOOLEAN_AND)
						...
						while (pos++ < last) {
							db.read(pos);
							if (rowsBitmapAux.isBitSet(pos) || (db.recordNotDeleted()
			...
			while (pos++ <
				   last) { // juliana@201_27: solved a bug in next() and prev() that would happen after doing a delete from table_name.
				db.read(pos);
	...
	void verifyResultSet() throws IllegalStateException {
		...
		if (driver.htTables ==
			null) { // juliana@227_4: the connection where the result set was created can't be closed while using it.
	...
	private void checkColumn(int column) {
		...
		if (column <= 0 || column > columnCount) // The columns given by the user ranges from 1 to n.
			throw new IllegalArgumentException(LitebaseMessage.getMessage(
												   LitebaseMessage.ERR_INVALID_COLUMN_NUMBER) + column);
	}
	...
	private boolean privateGetFromIndex(int column, int type) {
		...
		if (type != SQLElement.UNDEFINED)
			if (!(field.isDataTypeFunction && type == SQLElement.SHORT && (typeCol == SQLElement.DATE
					|| typeCol == SQLElement.DATETIME))
				&& (typeCol != type
					&& ((typeCol != SQLElement.CHARS_NOCASE && typeCol != SQLElement.CHARS)
