...
class Table
{
  ...
  /**
   * Indicates that the row validation is incomplete and must be continued in
   * join operations.
   */
  private static final int VALIDATION_RECORD_INCOMPLETE = 3;

  /**
   * Used internally on <code>booleanTreeEvaluateJoin()</code>. The current
   * branch was validated as <code>true</code>.
   */
  ...
  /**
   * Indicates that a column old value had null. Used in
   * <code>writeRecord()</code>.
   */
   ...
   */
  static final int USE_CRYPTO =
    3; // juliana@253_8: now Litebase supports weak cryptography.

  /**
   * The counter of the current <code>rowid</code>. The <code>rowid</code> is
   * continuously incremented so that two elements will never have the same one,
   * even if elements are deleted. <p>The record attributes are stored in the
   * first two bits of the <code>rowid</code>.
   */
  int currentRowId = 1;
  ...
  /**
   * Used to return the number of rows that a select without a where clause
   * returned.
   */
  ...
  // opening.
  /**
   * Indicates that a table has been modified and must be marked as not closed
   * properly after opened and before closed.
   */
  boolean isModified;
  ...
  /**
   * Given a column name, returns its index for this table. <code>rowid</code>,
   * a special column, is always column 0.
   */
  IntHashtable htName2index;

  /**
   * Just for the case when the column has a default value but the user
   * explicited the insert or update of a null.
   */
  byte[] storeNulls;
  ...
  /**
   * Column types (<code>SHORT</code>, <code>INT</code>, <code>LONG</code>,
   * <code>FLOAT</code>, <code>DOUBLE</code>, <code>CHARS</code>,
   * CHARS_NOCASE</code>)
   */
  byte[] columnTypes;
  ...
  /**
   * The column names. If <code>null</code>, the column names are not available
  ...
  /**
   * Existing column indices for each column, or <code>null</code> if the column
   * has no index.
   */
  Index[] columnIndices;

  /**
   * Existing composed column indices for each column, or <code>null</code> if
   * the table has no composed index.
   */
  ComposedIndex[] composedIndices =
    new ComposedIndex[SQLBooleanClause.MAX_NUM_INDEXES_APPLIED];
   ...
   */
  private DataStreamLB
    tsmdDs; // juliana@253_8: now Litebase supports weak cryptography.

  /**
   * Stores old values read from the table. This is used by
   * <code>writeRecord()</code> in order to reduce memory allocation.
   */
  SQLValue[] gvOlds;

  /**
   * Stores flags from the record. This is used by <code>writeRecord()</code> in
   * order to reduce memory allocation.
   */
  byte[] ghas;

  /**
   * An array to store the primary key values. Used in
   * <code>writeRecord()</code>.
   */
  private SQLValue[] primaryKeyValues;

  /**
   * An array to store the primary key old values when doing an update. Used in
   * <code>writeRecord()</code>.
   */
   ...
   * @return 0 for simple indices. For composed index, if there was this same
   *     index, it returns the negative number of the this old one; otherwise,
   * it returns the new number.
   * @throws AlreadyCreatedException If an index already exists.
   */
  int verifyIfIndexAlreadyExists(byte[] columnNumbers)
    throws AlreadyCreatedException
  {
    int indexCount = columnNumbers.length, idx = -1;
    ...
    if (indexCount == 1) { // Simple index.
      ...
      while (--i >= 0) {
        ...
        if (j == indexCount) { // juliana@253_2: corrected a bug if a composed
                               // index with less columns were created after one
                               // with more columns.
          while (--j >= 0)
  ...
  void driverDropIndex(int column) throws IOException
  {
    ...
    if (index == null) // Column does not have an index.
      throw new DriverException(
        LitebaseMessage.getMessage(
          LitebaseMessage.ERR_COLUMN_DOESNOT_HAVE_AN_INDEX) +
        columnNames[column]);
    ...
    // juliana@227_6
    columnAttrs[column] &=
      ~Utils.ATTR_COLUMN_HAS_INDEX; // Deletes the INDEX bit from the
                                    // attributes.
   ...
   * @param indexId The id of the composed index or -1 if its position is not
   *     known.
   * @param saveMD Indicates if the meta data is to be saved.
   * @throws IOException If an internal method throws it.
   * @throws DriverException If the table does not have the desired composed
   *     index to be dropped.
   */
  void driverDropComposedIndex(byte[] columns, int indexId, boolean saveMD)
    throws DriverException, IOException
  {
    ...
    if (found && ci != null) { // Removes the index.
      ...
      // juliana@201_16: When a composed index is deleted, its information is
      // now deleted from the metadata.
  ...
  // juliana@227_6: drop index * on table_name wold make the index reapear after
  // closing the driver and reusing table_name.
  /**
  ...
  int deleteAllIndices() throws IOException
  {
    ...
    int i = indices.length, count = 0, pk = primaryKeyCol, cpk = composedPK;
    ...
    // juliana@201_33: When all indices are dropped by the user, the composed
    // primary key can't be deleted. Composed index.
  ...
  private void computeColumnOffsets() throws IOException
  {
    ...
    // the number of bytes necessary to store the columns. Each column in a
    // table correspond to one bit. Added a number of bytes corresponding to the
    // null values and to the crc code.
    sum += ((n + 7) >> 3) + 4; // juliana@220_4

    plainDB.setRowSize(sum,
                       plainDB.basbuf == null || sum > plainDB.basbuf.length
                         ? new byte[sum]
                         : plainDB.basbuf); // Sets the new row size.
    ...
    if (!recomputing) {
      ...
      if (name ==
          null) // The hashes of the columns must be put in a hash table.
        while (--n >= 0) {
   ...
   * @param throwException Indicates that a <code>TableNotClosedException</code>
   *     should be thrown.
   * @throws IOException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   * @throws DriverException If the table is in an incompatible format.
   * @throws TableNotClosedException If the table was not properly close when
   *     opened last time.
   */
  private void tableLoadMetaData(String appCrid,
                                 String sourcePath,
                                 boolean throwException)
    throws IOException, InvalidDateException, DriverException,
           TableNotClosedException
  {
    ...
    DataStreamLB ds =
      new DataStreamLB(new ByteArrayStream(bytes), plainDB.useCrypto);

    // The currentRowId is found from the last non-empty record, not from the
    // metadata.
    if (!(((bytes[0] & USE_CRYPTO) == 0) ^ plainDB.useCrypto) &&
        bytes[1] == 0 && bytes[2] == 0 && bytes[3] == 0 &&
        (bytes[0] == 0 || bytes[0] == 1 || bytes[0] == 3)) {
      // juliana@222_1: the table should not be marked as closed properly if it
      // was not previously closed correctly.
      dbFile.close();
      plainDB.dbo.close();
      throw new DriverException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_WRONG_CRYPTO_FORMAT));
    }
    ...
    ds.skipBytes(4); // It is not necessary to read the last position of the
                     // blobs and strings file.
    ...
    if (plainDB.headerSize == 0) { // The header size can't be zero.
      ...
      plainDB.dbo.close();
      throw new DriverException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_TABLE_CORRUPTED) + name +
        '!');
    }

    // If the header needs to be bigger, re-creates the metadata buffer with the
    // correct size and skips the bytes already read.
    if (plainDB.headerSize != bytes.length)
      (ds =
         new DataStreamLB(new ByteArrayStream(bytes = plainDB.readMetaData()),
                          plainDB.useCrypto))
        .skipBytes(6);

    plainDB.dbo.finalPos =
      plainDB.dbo
        .size; // This does not let the user lose some database objects.

    // Checks if the table strings has the same format of the connection.
    if ((((flags = ds.readByte()) & IS_ASCII) != 0 && !plainDB.isAscii) ||
        ((flags & IS_ASCII) == 0) && plainDB.isAscii) {
      // juliana@222_1: the table should not be marked as closed properly if it
      // was not previously closed correctly.
      dbFile.close();
      plainDB.dbo.close();
      throw new DriverException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_WRONG_STRING_FORMAT));
    }

    // juliana@251_11: removed a possible exception when recovering an ascii
    // table with indices on JavaSE and Blackberry. juliana@220_2: added
    // TableNotCreatedException which will be raised whenever a table is not
    // closed properly. If the table was not correctly closed, throws an
    // specific exception to the user.
    ...
    // consideration.
    if ((version = ds.readShort()) <
        VERSION -
          1) { // The tables version must be the same as Litebase version.
      // juliana@222_1: the table should not be marked as closed properly if it
      // was not previously closed correctly.
      dbFile.close();
      plainDB.dbo.close();
      throw new DriverException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_WRONG_VERSION) +
        (" (" + version) + ')');
    }
    ...
    // juliana@230_5: Corrected a AIOBE when using a table created on Windows
    // 32, Windows CE, Linux, Palm, Android, iPhone, or iPad using primary key
    // on BlackBerry and Eclipse.
    primaryKeyCol =
      ds.readByte(); // juliana@114_9: the simple primary key column.
    ds.skipBytes(1);
    composedPK = ds.readByte(); // The composed primary key index.
    ds.skipBytes(1);
    columnCount = ds.readUnsignedShort(); // Reads the column count.

    int n = columnCount, i = -1, j;
    ...
    // The number of bytes necessary to store the columns. Each column in a
    // table corresponds to one bit. juliana@201_21: The null columns
    // information must be created before openning the indices when reading the
    // table meta data.
    ...
    // juliana@224_1: corrected an inconsistency between java and native
    // versions that would raise an <code>IndexOutOfBoundsException</code> when
    // using a table created on Windows 32, Windows CE, Palm, Android, Linux, or
    // iPhone on Java or BlackBerry.
    ds.skipBytes(2);
    String[] names = columnNames =
      ds.readStringArray(n); // Reads the column names.
    ...
    while (--i >= 0) // Loads the indices.
      if ((attrs[i] & Utils.ATTR_COLUMN_HAS_INDEX) != 0) {
        // juliana@227_21: corrected a bug of recover table not working
        // correctly if the table has indices.
        if ((exist = new File(fullName = Utils.getFullFileName(
                                nameAux + i + ".idk", sourcePath))
                       .exists()) &&
            flags == 0) {
        ...
        indexCreateIndex(tableName,
                         ...
                         exist);
        if (!exist &&
            flags != 0) { // One of the files doesn't exist. juliana@227_21
          // juliana@230_8: corrected a possible index corruption if its files
          // are deleted and the application crashes after recreating it.
    ...
    dbFile.setPos(plainDB.headerSize +
                  (plainDB.rowCount > 0 ? plainDB.rowCount - 1 : 0) *
                    plainDB.rowSize);
    currentRowId =
      (auxRowId != Utils.ATTR_DEFAULT_AUX_ROWID
         ? auxRowId
         : ((new DataStreamLB(dbFile, plainDB.useCrypto).readInt() &
             Utils.ROW_ID_MASK) +
            1)) &
      Utils.ROW_ID_MASK;
    ...
    while (++i < n) // Reads the default values.
      if ((attrs[i] & Utils.ATTR_COLUMN_HAS_DEFAULT) !=
          0) { // Tests if it has default values.
    ...
    int numCompIndices = numberComposedIndices =
      ds.readByte(); // Reads the composed indices.

    if (numCompIndices > 0) {
      int column, aComposedPK = composedPK, indexId, numColumns;
      byte[] columns;
      int[] columnSizes;
      byte[] columnTypes;
      ComposedIndex[] compIndices = composedIndices;
      ...
      while (++i < numCompIndices) {
        ...
        while (++j < numColumns) {
          column = columns[j] =
            ds.readByte(); // Columns of this composed index.
        ...
        // correctly if the table has indices.
        if ((exist = new File(fullName = Utils.getFullFileName(
                                nameAux + indexId + ".idk", sourcePath))
                       .exists()) &&
            flags == 0) {
          idxFile = new File(fullName, File.READ_WRITE);
          idxFile.setSize(0);
        ...
        indexCreateComposedIndex(tableName,
                                 columns,
                                 columnSizes,
                                 columnTypes,
                                 indexId,
        ...
        if (!exist && flags != 0) { // One of the files doesn't exist.
          ...
          // are deleted and the application crashes after recreating it.
          setModified(); // Sets the table as not closed properly.
          tableReIndex(indexId - 1,
                       compIndices[indexId - 1],
                       false); // juliana@227_21
        }
      }
    }
   ...
   *
   * @param saveType The kind of save. It can be one out of
   *     <code><B>TSMD_ONLY_DELETEDROWSCOUNT</B></code>,
   * <code><B>TSMD_ONLY_PRIMARYKEYCOL</B></code>,
   * <code><B>TSMD_EVERYTHING</B></code>, or
   * <code><B>TSMD_ONLY_AUXROWID</B></code>.
   * @throws IOException If an internal method throws it.
   */
  void tableSaveMetaData(int saveType) throws IOException
  {
    // Stores the changeable information.
    int n = columnCount, i = -1, j = isModified ? 0 : Table.IS_SAVED_CORRECTLY,
        numberColumns;
    byte[] types = columnTypes;
    ...
    if (auxBas != null) { // If the buffer is not empty, only resets it.
      ...
      auxDs = tsmdDs = new DataStreamLB(
        auxBas = tsmdBas = new ByteArrayStream(plainDB.headerSize),
        plainDB.useCrypto);

    auxBas.getBuffer()[0] =
      (byte)(plainDB.useCrypto ? (plainDB.useOldCrypto ? 1 : USE_CRYPTO) : 0);
    ...
    auxDs.writeByte(
      plainDB.isAscii
        ? IS_ASCII | j
        : j); // juliana@226_4: table is not saved correctly if modified.
    auxDs.writeShort(version);        // The table format version.
    auxDs.writeInt(deletedRowsCount); // Saves the deleted rows count.

    if (saveType !=
        Utils.TSMD_ONLY_DELETEDROWSCOUNT) { // More things other than the
                                            // deleted rows count must be saved.
      auxDs.writeInt(auxRowId); // rnovais@570_61: saves the auxiliary rowid.

      if (saveType !=
          Utils.TSMD_ONLY_AUXROWID) { // More things other than the auxiliary
                                      // row id must be saved.
        // juliana@230_5: Corrected a AIOBE when using a table created on
        // Windows 32, Windows CE, Linux, Palm, Android, iPhone, or iPad using
        // primary key on BlackBerry and Eclipse.
        auxDs.writeByte(primaryKeyCol); // Saves the primary key col.
        auxDs.writeByte(0);
        auxDs.writeByte(
          composedPK); // juliana@114_9: saves the composed primary key index.
        auxDs.writeByte(0);

        if (saveType !=
            Utils.TSMD_ONLY_PRIMARYKEYCOL) { // More things other than the
                                             // primary key col must be saved.
          ...
          if (saveType == Utils.TSMD_EVERYTHING) { // Stores the rest.
            ...
            auxDs.writeByte(
              n = numberComposedIndices); // Number of composed indices.

            i = -1;
            while (++i < n) { // Stores the composed indices.
              auxDs.writeByte(
                (ci = compIndices[i]).indexId); // The composed index id.
              auxDs.writeByte(
                numberColumns =
                  ci.columns
                    .length);     // Number of columns on the composed index.
            ...
            // Number of columns on composed primary key. If 0, there's no
            ...
            while (++i < n) { // Stores the composed primary key.
              auxDs.writeByte(
                columns[i]); // The column of the composed primary key.
            }
          }
        }
      }
    ...
    ((NormalFile)(plainDB.db))
      .flushCache(); // juliana@223_11: table meta data is now always flushed
                     // imediately after being changed.
  }
  ...
  void reorder(SQLStatement stmt) throws SQLParseException, IOException
  {
    ...
    String[] fields;
    SQLValue[] record;
    short[] paramIndexes; // juliana@253_14: corrected a possible AIOBE if the
                          // number of parameters of a prepared statement were
                          // greater than 128.
    SQLInsertStatement insertStmt = null;
    SQLUpdateStatement updateStmt = null;
    int idx, i = -1, length, numParams = 0;
    boolean isInsert = stmt.type == SQLElement.CMD_INSERT;

    if (isInsert) { // Insert statement.
      nulls = (insertStmt = (SQLInsertStatement)stmt)
                .storeNulls; // Cleans the <code>storeNulls</code>.
      length = (fields = insertStmt.fields).length;
      paramIndexes = insertStmt.paramIndexes;
      record = insertStmt.record;
    } else { // Update statement.
      nulls = (updateStmt = (SQLUpdateStatement)stmt)
                .storeNulls; // Cleans the <code>storeNulls</code>.
    ...
    // juliana@230_9: solved a bug of prepared statement wrong parameter
    // dealing.
    while (++i < length) // Makes sure the fields are in db creation order.
      try {
        outRecord[idx = hashTable.get(fields[i].hashCode())] =
          record[i]; // Finds the index of the field on the table and reorders
                     // the record.
        Utils.setBit(tableNulls, idx, (nulls[i >> 3] & (1 << (i & 7))) != 0);
        if (record[i] != null && record[i].asString != null &&
            record[i].asString.equals("?")) {
   ...
   * @param ascii Indicates if the table strings are to be stored in the ascii
   *     format or in the unicode format.
   * @param crypto Indicates if the table is to be stored encrypted or not.
   * @param throwException Indicates that a TableNotClosedException should be
   *     thrown.
   * @throws IOException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   */
  void tableCreate(String sourcePath,
                   String newName,
                   boolean create,
                   String appCrid,
    ...
    throws IOException, InvalidDateException
  {
    PlainDB plainDB = db =
      new PlainDB(newName,
                  sourcePath,
                  create); // Creates or opens the table files.
    plainDB.driver = driver;
    if (newName != null &&
        (plainDB.db.size != 0 ||
         create)) { // The table is already created if the .db is not empty.
      name = newName;
      plainDB.isAscii = ascii;
      plainDB.useCrypto = crypto;
      if (plainDB.db.size !=
          0) { // If the table is already created, loads its meta data.
  ...
  // rnovais@566_10
  /**
   * Renames a table. This never happens to be a temporary
   * <code>ResultSet</code> memory table.
   *
   ...
   * @throws IOException If an internal method throws it.
   */
  void renameTable(LitebaseConnection driver,
                   String oldTableName,
                   String newTableName) throws IOException
  {
    ...
    driver.htTables.put(newTableName,
                        this); // Adds the new table name to the hash table.
    ...
    while (--i >= 0)
      if (indices[i] != null) {
        index = indices[i];
        newFullName = Utils.getFullFileName(
          nameIndex = (tableFullName + '$') + i, driver.sourcePath);
    ...
    while (--i >= 0) {
      newFullName = Utils.getFullFileName(
        nameIndex = (tableFullName + '&') + (i + 1), driver.sourcePath);
   ...
   * @throws IOException If an internal method throws it.
   * @throws DriverException If the old column does not exist or the new column
   *     already exists.
   */
  void renameTableColumn(String oldColumn, String newColumn)
    throws IOException, DriverException
  {
    ...
    if (ht.exists(newHashCode)) // The new column name can't exist.
      throw new DriverException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_COLUMN_ALREADY_EXIST) +
        newColumn);
   ...
   * @param composedPKIdx The composed primary key index in the composed
   *     indices.
   ...
   * @throws IOException If an internal method throws it.
   */
  void tableSetMetaData(String[] names,
                        int[] hashes,
                        byte[] types,
                        int[] sizes,
                        byte[] attrs,
                        ...
                        int pkCol,
                        int composedPKIdx,
                        byte[] composedPKColums,
                        int ComposedPKColsSize)
    throws AlreadyCreatedException, IOException
  {
    ...
    storeNulls = new byte[bytes = (count + 7) >>
                                  3]; // Initializes the arrays for the nulls.

    // The number of bytes necessary to store the nulls. Each column in a table
    // correspond to one bit.
    ...
    if (name != null) { // juliana@201_14: It is not necessary to save the meta
                        // data in the .db for memory tables.
      ...
      tableSaveMetaData(
        Utils.TSMD_EVERYTHING); // Saves the metadata after everything was set.
    } else {
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  ResultSet createSimpleResultSet(SQLBooleanClause whereClause)
    throws IOException, InvalidDateException
  {
    ResultSet rs = new ResultSet();
    rs.pos = -1; // guich@300: beforeFirst();
    ...
    rs.lastRecordIndex = db.rowCount - 1;
    rs.whereClause = whereClause;
    if (whereClause != null) // tries to apply the table indexes to generate a
                             // bitmap of the rows to be returned.
      SQLSelectStatement.generateIndexedRowsMap(
        new ResultSet[] { rs }, rs.table.numberComposedIndices > 0);
    return rs;
  }
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  ResultSet createResultSetForSelect(SQLBooleanClause whereClause)
    throws IOException, InvalidDateException
  {
  ...
  // juliana@201_3: if an index is created after populating or purging the
  // table, its nodes will be full in order to improve its usage and search
  // speed.
  /**
   * Re-builds an index of a table.
   *
   * @param column The table column number of the index or -1 for a composed
   *     index.
   * @param composedIndex The composed index to be rebuilt or null in case of a
   *     simple index.
   * @param isPrimaryKey Indicates that the index is of a primary key.
   * @throws DriverException If there is a null in the primary key or a
   *     duplicated key.
   * @throws IOException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   */
  void tableReIndex(int column,
    ...
    throws DriverException, IOException, InvalidDateException
  {
    // Gets the index.
    Index index =
      (composedIndex == null) ? columnIndices[column] : composedIndex.index;
    PlainDB plainDb = db;
    int numberColumns = index.types.length, // Gets the number of columns.
      i = -1, j, n = plainDb.rowCount;
    short[] offsets = columnOffsets;
    ...
    // rnovais@570_24 juliana@114_9: checks if the column being reindexed is the
    // primary key columns.
    ...
    try {
      if (index.isOrdered &&
          composedIndex == null) { // Simple index using rowid.
        ...
        while (++i < n) {
          ...
          readValue(vals[0],
                    ...
                    SQLElement.INT,
                    false,
                    false); // juliana@220_3 juliana@230_14
          index.indexAddKey(vals, i);
        }
      } else {
        int rows =
          plainDb.rowCount; // juliana@284_1: Solved a possible application
                            // crash when recreating indices.
        ...
        while (++i < n) {
          ...
          readNullBytesOfRecord(
            0,
            false,
            0); // juliana@201_22: the null columns information wasn't being
                // read when re-creating an index.
          vals[k] = SQLValue.newSQLValues(numberColumns);

          if (composedIndex == null) {
            // juliana@230_14
            // juliana@220_3
            // juliana@202_12: Corrected null values dealing when building an
            // index.
            readValue(vals[k][0],
                      offsets[column],
                      types[0],
                      isNull = (nulls[column >> 3] & (1 << (column & 7))) != 0,
                      false);

            // The primary key can't be null.
            // juliana@202_10: Corrected a bug that would cause a
            // DriverException if there was a null in an index field when
            // creating it after the table is populated.
            if (isPrimaryKey && isNull) {
              throw new DriverException(LitebaseMessage.getMessage(
                LitebaseMessage.ERR_PK_CANT_BE_NULL));
            }
          } else {
            j = numberColumns;
            while (--j >= 0) {
              ...
              readValue(vals[k][j],
                        ...
                        types[j],
                        isNull |=
                        (nulls[columns[j] >> 3] & (1 << (columns[j] & 7))) != 0,
                        false);
          ...
          if (isNull) { // Do not store null records.
            rows--;
          } else // juliana@212_2: DATETIME indices would be recreated
                 // incorrectly on desktop and BlackBerry.
            if (types[0] ==
                SQLElement.LONG) { // The record value is stored in an empty
                                   // field of the first record column value.
            vals[k++][0].asInt = i;
          } else {
            vals[k++][0].asLong = i;
          }
        ...
        rows = k; // juliana@270_22: solved a possible crash when the table is
                  // corrupted on Android and possibly on other platforms.
        if (!index.isOrdered) {
          // A radix sort is done for integer types. It is much more efficient
          // than quick sort.
          if (numberColumns == 1 &&
              (types[0] == SQLElement.SHORT || types[0] == SQLElement.INT ||
               types[0] == SQLElement.LONG || types[0] == SQLElement.DATE)) {
          ...
          index.isOrdered =
            true; // The index elements will be inserted in the right order.
        }
        int count = -1, compare;
        while (++count < rows) {
          // if it is the primary key, checks first if there is violation.
          if (isPrimaryKey && count > 0 &&
              ((compare =
                  compareRecords(vals[count], vals[count - 1], types)) == 0 ||
               compare == Convert.MAX_INT_VALUE ||
               compare == Convert.MIN_INT_VALUE))
            throw new PrimaryKeyViolationException(
              LitebaseMessage.getMessage(
                LitebaseMessage.ERR_STATEMENT_CREATE_DUPLICATED_PK) +
              name);
      ...
      index.setWriteDelayed(
        isDelayed); // Uses the user desired delayed settings again.
  ...
  // juliana@230_14
  // juliana@220_3: blobs are not loaded anymore in the temporary table when
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  void readValue(SQLValue value,
                 int offset,
                 int colType,
                 boolean isNull,
                 boolean isTempBlob) throws IOException, InvalidDateException
  {
    PlainDB plainDB = db;
    ByteArrayStream bas = plainDB.bas;
    bas.skipBytes(offset); // Skips the first columns.
    offset = plainDB.readValue(value,
                               offset,
                               colType,
                               plainDB.basds,
                               name == null,
                               isNull,
                               isTempBlob); // Reads the value
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  void indexCreateIndex(String fullTableName,
                        int column,
                        int[] columnSizes,
                        byte[] columnTypes,
                        String crid,
                        String sourcePath,
                        boolean exist) throws IOException, InvalidDateException
  {
    String name = (fullTableName + '$') + column; // The index name.

    // Creates a new index structure.
    Index index = columnIndices[column] =
      new Index(this, columnTypes, columnSizes, name, sourcePath, exist);
   ...
   * @param crid The application id of the table.
   * @param increaseArray Indicates if the composed indices array must be
   *     increased.
   * @param sourcePath The folder where the table files are stored.
   * @param exist Indicates that the index files already exist.
   * @throws DriverException If the maximum number of composed indices was
   *     achieved.
   * @throws IOException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   */
  void indexCreateComposedIndex(String fullTableName,
                                byte[] columnIndices,
                                int[] columnSizes,
                                byte[] columnTypes,
                                int newIndexNumber,
    ...
    throws DriverException, IOException, InvalidDateException
  {
    ...
    String name =
      (fullTableName + '&') + newIndexNumber; // Passes the newIndex index id.
    ...
    ci.index = new Index(this,
                         columnTypes,
                         columnSizes,
                         name,
                         sourcePath,
                         exist); // Creates the index of the composed index.
  ...
  {
    // Important: this is an optimized version of writeRecord, specially
    // designed to write the values of a resultset.
    int n = columnCount, i;
    PlainDB plainDB = db;
    DataStreamLB ds =
      db.basds; // juliana@253_8: now Litebase supports weak cryptography.
    int[] sizes = columnSizes;
    byte[] types = columnTypes;
    byte[] nulls = columnNulls[0];
   ...
   */
  private void checkPrimaryKey(SQLValue[] vals,
                               int recPos,
                               boolean newRecord,
                               SQLValue[] values)
    throws IOException, InvalidDateException, DriverException
  {
    ...
    // If the column is a simple primary key, gets the index of its column.
    // Otherwise, gets the index of its composed primary key.
    Index index =
      (pKCol != -1) ? columnIndices[pKCol] : composedIndices[composedPK].index;
    ...
    if (!newRecord) {  // An update.
      ...
      while (--i >= 0) {
        // If it is updating a record, reads the old value and checks if a
        ...
        hasChanged |=
          vals[i] != null &&
          vals[i].valueCompareTo(values[i], types[i], false, false, null) != 0;
    ...
    while (--i >= 0)
      if (vals[i] == null) {
        throw new DriverException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_PK_CANT_BE_NULL));
      }

    if (hasChanged ||
        newRecord) { // Sees if the record does not violate the primary key.
   ...
   * @param statementType The type of the statement, which can be
   *     <code>SQLElement.STMT_INSERT</code> or
   *           <code>SQLElement.STMT_UPDATE</code>.
   * @throws DriverException If a primary key is or a <code>NOT NULL</code>
   *     field is is <code>null</code>.
   */
  void verifyNullValues(SQLValue[] record,
                        byte[] storeNullsStmt,
                        int statementType) throws DriverException
  {
    ...
    if (statementType == SQLElement.CMD_INSERT) { // Insert statement.
      ...
      if (primaryKeyCol != Utils.NO_PRIMARY_KEY &&
          (nulls[primaryKeyCol >> 3] & (1 << (primaryKeyCol & 7))) !=
            0) { // The primary key can't be null.
        throw new DriverException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_PK_CANT_BE_NULL));
      }

      while (--len > 0) // A not null field can't have a null.
        if ((record[len] == null || record[len].isNull) &&
            defaults[len] == null &&
            (attrs[len] & Utils.ATTR_COLUMN_IS_NOT_NULL) != 0)
          throw new DriverException(
            LitebaseMessage.getMessage(LitebaseMessage.ERR_FIELD_CANT_BE_NULL) +
            columnNames[len]);
    } else { // Update statement.
      if (primaryKeyCol != Utils.NO_PRIMARY_KEY &&
          (nulls[primaryKeyCol >> 3] & (1 << (primaryKeyCol & 7))) !=
   ...
   * @param columnIndexesTables Has the indices of the tables for each resulting
   *     column.
   * @param whereClauseType Indicates the where clause is an <code>AND</code> or
   *     an <code>OR</code>.
   ...
   */
  int writeResultSetToTable(ResultSet[] list,
                            short[] rs2TableColIndexes,
                            SQLSelectClause selectClause,
                            Table[] columnIndexesTables,
                            int whereClauseType)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    int count = columnCount, i = count, j, colIndex;

    // rnovais@568_10: when it has an order by table.columnCount =
    // selectClause.fieldsCount + 1.
    int totalRecords = 0;
    ResultSet rs;
    ...
    if (!hasJoin) {
      ...
      // If there are indices, grows the result set table to the number of index
      // records which satisfy the query. This reduces a lot the number of
      // "new"s to increase the temporary table.
      if (rs.whereClause == null) {
        PlainDB plainDB = db;

        plainDB.rowAvail = (rs.rowsBitmap == null
                              ? rs.table.db.rowCount - rs.table.deletedRowsCount
                              : Utils.countBits(rs.rowsBitmap.items));
      ...
      // If rs2TableColIndexes == null, that indicates that the result set and
      // the table have the same sequence of columns.
      if (rs2TableColIndexes == null)
        while (rs.getNextRecord()) { // No preverify needed.
          rs.table.readNullBytesOfRecord(0,
                                         false,
                                         0); // Reads the bytes of the nulls.
          Vm.arrayCopy(rsColumnNulls,
                       0,
                       nulls,
                       0,
                       rsColumnNulls.length); // Since all the columns match,
                                              // just copies the nulls.

          i = count;
          while (--i >= 0) // Gets the values of the result set columns.
            if ((rsColumnNulls[i >> 3] & (1 << (i & 7))) ==
                0) { // If it is null, just skips.
        ...
        while (rs.getNextRecord()) { // No preverify needed.
          ...
          i = count;
          while (--i >= 0) { // Gets the values of the result set columns.
            // For columns that do not map directly to the underlying table of
            // the result set, just skips the reading.
            if (!(isNull = ((colIndex = rs2TableColIndexes[i]) == -1 ||
                            (rsColumnNulls[colIndex >> 3] &
                             (1 << (colIndex & 7))) != 0))) {
            ...
            Utils.setBit(nulls,
                         i,
                         isNull); // Sets the null values for tempTable.
          }
      ...
      return performJoin(
        list, rs2TableColIndexes, selectClause, values, whereClauseType);
    }
   ...
   * @param whereClauseType The type of operation used: <code>AND</code> or
   *     <code>OR</code>.
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  private int performJoin(ResultSet[] list,
                          short[] rs2TableColIndexes,
                          SQLSelectClause selectClause,
                          SQLValue[] valuesJoin,
                          int whereClauseType)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    int numTables = list.length, currentIndexTable = 0, ret, totalRecords = 0,
        colIndex, pos, len;
    boolean bitSet;
    ResultSet currentRs;
    Table table;
    ...
    while (currentIndexTable >= 0) {
      ...
      nulls = columnNulls[0];
      ret = getNextRecordJoin(list,
                              currentIndexTable,
                              !ignoreWhereCondition[currentIndexTable],
                              whereClauseType);
      switch (ret) {
        ...
        case VALIDATION_RECORD_INCOMPLETE:
          ...
          while (--len >= 0) {
            ...
            // If rs2TableColIndexes == null, it indicates that the result set
            // and the table have the same sequence of columns.
            colIndex =
              (rs2TableColIndexes == null ? len : rs2TableColIndexes[pos]);

            // For columns that do no map directly to the underlying table of
            // the result set, just skips the reading.
            bitSet = (table.columnNulls[0][colIndex >> 3] &
                      (1 << (colIndex & 7))) != 0;
            if (colIndex != -1 && !bitSet) { // If it is null, just skips.
              currentRs.sqlwhereclausetreeGetTableColValue(colIndex,
                                                           valuesJoin[pos]);
            }
            ...
            Utils.setBit(
              ...
              pos,
              colIndex != -1 &&
                bitSet); // Sets the null values from the temporary table.
          }
        ...
        case NO_RECORD:
          currentIndexTable--;
          currentRs.pos =
            -1; // Restarts the current resultset to the next iteration.
      }
    }
    return totalRecords;
  }
   ...
   *
   * @param list The list of the result sets.
   * @param rsIndex The index of the result set of the list used to get the next
   *     record.
   * @param verifyWhereCondition Indicates if the where clause needs to be
   *     verified.
   * @param whereClauseType The type of expression in the where clause (OR or
   *     AND).
   * @return <code>VALIDATION_RECORD_OK</code>, <code>NO_RECORD</code>,
   *     <code>VALIDATION_RECORD_NOT_OK</code>, or
   * <code>VALIDATION_RECORD_INCOMPLETE</code>.
   * @throws IOException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   * @throws InvalidNumberException If an internal method throws it.
   */
  private int getNextRecordJoin(ResultSet[] list,
                                int rsIndex,
                                boolean verifyWhereCondition,
                                int whereClauseType)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    ResultSet rs = list[rsIndex];
    IntVector auxRowsBitmap = rs.auxRowsBitmap;
    int len = list.length, ret, last = rs.lastRecordIndex;
    PlainDB db = rs.table.db;
    SQLBooleanClause whereClause = rs.whereClause;
    IntVector rowsBitmap =
      (auxRowsBitmap != null) ? auxRowsBitmap : rs.rowsBitmap;
    ...
    if (rowsBitmap != null && verifyWhereCondition) {
      ...
      if (rs.pos < last)
        if (whereClause == null || auxRowsBitmap != null) { // count_index++;
          // juliana@230_45: join should not take deleted rows into
          // consideration. No WHERE clause. Just returns the rows marked in the
          // bitmap.
          while ((p = Utils.findNextBitSet(items, rs.pos + 1)) != -1 &&
                 p <= last) {
            db.read(rs.pos = p);
            if (db.recordNotDeleted()) {
              if (verifyWhereCondition && auxRowsBitmap != null)
                return booleanTreeEvaluateJoin(
                  list,
                  (list[rsIndex].whereClause.resultSet = list[rsIndex])
                    .whereClause.expressionTree);
              if (whereClauseType == Utils.WC_TYPE_AND_DIFF_RS) {
                return (len == rs.indexRs + 1) ? VALIDATION_RECORD_OK
                                               : VALIDATION_RECORD_INCOMPLETE;
              }
          ...
          // 1) The relationship between the bitmap and the WHERE clause is an
          // AND relationship. 2) The relationship between the bitmap and the
          // WHERE clause is an OR relationship.
          if (rs.rowsBitmapBoolOp == SQLElement.OP_BOOLEAN_AND) {
          // AND case - Walks through the bits that are set in the bitmap and
          // checks if the rows satisfy the where clause. juliana@230_45: join
          // should not take deleted rows into consideration.
          while ((p = Utils.findNextBitSet(items, rs.pos + 1)) != -1 &&
                 p <= last) {
            db.read(rs.pos = p);
            if (db.recordNotDeleted())
          ...
          // OR case - Walks through all records. If the corresponding bit is
          // set in the bitmap, does not need to evaluate WHERE clause.
          ...
          while (rs.pos < last) {
            ...
            if (db.recordNotDeleted()) {
              ...
              if ((ret = booleanTreeEvaluateJoin(
                     list,
                     (list[rsIndex].whereClause.resultSet = list[rsIndex])
                       .whereClause.expressionTree)) !=
      ...
      while (rs.pos < last) { // count_noindex++;
        ...
        if (db.recordNotDeleted()) {
          ...
          // juliana@213_2: corrected a bug that could make joins not work with
          // ORs using indices.
          ret =
            booleanTreeEvaluateJoin(list,
                                    (rs.whereClause.resultSet = list[rsIndex])
                                      .whereClause.expressionTree);
          if (ret == VALIDATION_RECORD_NOT_OK &&
              rs.whereClause.appliedIndexesBooleanOp ==
                SQLElement.OP_BOOLEAN_OR)
            while (++rsIndex < len)
              if (list[rsIndex].auxRowsBitmap != null ||
                  list[rsIndex].rowsBitmap != null) {
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  private int booleanTreeEvaluateJoin(ResultSet[] list,
                                      SQLBooleanClauseTree tree)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    int indexRs = tree.booleanClause.resultSet.indexRs, indexTree =
                                                          tree.indexRs;
    SQLBooleanClauseTree leftTree = tree.leftTree, rightTree = tree.rightTree;
    ...
    if (indexTree >= 0) {        // AND, OR and BOOLEAN_NOT have index = -1.
      ...
      if (indexTree > indexRs) { // juliana@211_5: solved a bug with joins which
                                 // would return more answers than desired.
        if (tree.bothAreIdentifier &&
            leftTree.indexRs == indexRs) { // Fills leftTree.value.
          ...
          if (rightTree.hasIndex && boolOp <= 1) {
            // juliana@225_13: join now behaves well with functions in columns
            // with an index.
            ...
            while (--i >= 0)
              if (fieldList[i].tableColIndex == rightTree.colIndex &&
                  fieldList[i].isDataTypeFunction) {
                return VALIDATION_RECORD_INCOMPLETE;
              }

            // Despite this is a join the parameter 'false' is sent because this
            // is a simple index calculation.
            SQLSelectStatement.computeIndex(list,
                                            false,
                                            rightTree.indexRs,
                                            leftTree.valueJoin,
                                            tree.operandType,
            ...
            // juliana@230_39: join now can be much faster if the query is
            // smartly written.
            if (rsBag.rowsBitmap != null && auxRowsBitmap != null &&
                boolOp == 1) {
              SQLSelectStatement.mergeBitmaps(
                auxRowsBitmap.items, rsBag.rowsBitmap.items, 1);
    ...
    switch (tree.operandType) { // Checks what is the operand type of the tree.
      ...
      case SQLElement.OP_REL_DIFF:
        switch (tree.valueType) { // Calls the right operation accordingly to
                                  // the values type.
          case SQLElement.SHORT:
          case SQLElement.INT:
          case SQLElement.LONG:
          ...
          case SQLElement.DATE:
          case SQLElement.DATETIME:
            return tree.compareNumericOperands() ? VALIDATION_RECORD_OK
                                                 : VALIDATION_RECORD_NOT_OK;
          case SQLElement.CHARS:
            return tree.compareStringOperands(false) ? VALIDATION_RECORD_OK
                                                     : VALIDATION_RECORD_NOT_OK;
          case SQLElement.CHARS_NOCASE:
            return tree.compareStringOperands(true) ? VALIDATION_RECORD_OK
                                                    : VALIDATION_RECORD_NOT_OK;
        }
      ...
      case SQLElement.OP_PAT_MATCH_NOT_LIKE:
        return tree.matchStringOperands(tree.valueType ==
      ...
      case SQLElement.OP_BOOLEAN_AND: // AND connector.
        if (leftTree != null &&
            rightTree != null) // Expects both trees to be not null.
          switch (booleanTreeEvaluateJoin(list, leftTree)) {
            case VALIDATION_RECORD_NOT_OK:
              return VALIDATION_RECORD_NOT_OK;
            case VALIDATION_RECORD_INCOMPLETE:
              if (booleanTreeEvaluateJoin(list,
                                          rightTree) ==
                  VALIDATION_RECORD_NOT_OK) { // Verifies the right branch.
                return VALIDATION_RECORD_NOT_OK;
              }

              // All other results return incomplete because the left side has
              // returned incomplete.
              return VALIDATION_RECORD_INCOMPLETE;

            case VALIDATION_RECORD_INCOMPLETE_OK:
            case VALIDATION_RECORD_OK:
              switch (booleanTreeEvaluateJoin(
                list,
                rightTree)) { // The left side returned true, so verifies the
                              // right branch.
                case VALIDATION_RECORD_NOT_OK:
                  return VALIDATION_RECORD_NOT_OK;
                case VALIDATION_RECORD_INCOMPLETE_OK:
                ...
                case VALIDATION_RECORD_INCOMPLETE:
                  ...
                  // If the right side returns incomplete, incomplete must be
                  // returned, despite the left side returned OK.
                  return VALIDATION_RECORD_INCOMPLETE;
              }
          }
      ...
      case SQLElement.OP_BOOLEAN_OR: // OR connector.
        if (leftTree != null &&
          ...
          switch (booleanTreeEvaluateJoin(list, leftTree)) {
            ...
            case VALIDATION_RECORD_INCOMPLETE_OK:
              switch (booleanTreeEvaluateJoin(
                list, rightTree)) { // Verifies the right branch.
                case VALIDATION_RECORD_NOT_OK:
                  return VALIDATION_RECORD_INCOMPLETE_OK; // juliana@263_1:
                                                          // corrected a very
                                                          // old bug in a join
                                                          // with OR.
                case VALIDATION_RECORD_INCOMPLETE:
            ...
            case VALIDATION_RECORD_NOT_OK: {
              // The left side returned false, so continues verifing the right
              // branch.
              switch (booleanTreeEvaluateJoin(list, rightTree)) {
                case VALIDATION_RECORD_NOT_OK:
                  return VALIDATION_RECORD_NOT_OK;

                // juliana@270_21: solved a very old join problem when using OR
                // and false constants comparison which would make the join
                // return no results.
                case VALIDATION_RECORD_INCOMPLETE_OK:
                  return VALIDATION_RECORD_INCOMPLETE_OK;
                case VALIDATION_RECORD_OK:
      ...
      case SQLElement.OP_PAT_IS_NOT:
        return tree.compareNullOperands() ? VALIDATION_RECORD_OK
                                          : VALIDATION_RECORD_NOT_OK;
    }
  ...
  /**
   * Goes to the end of the record, reads the null bytes, and stores them in
   * table.columnNulls. After this, returns the datastream cursor to the
   * previous offset.
   *
   * @param whichColumnNull The column null index. It ranges from 0 to 3.
   * @param dataStreamIsDislocated Indicates if the stream pointer is not
   *     pointing to its beginning,
   * @param col The column index where the stream pointer is pointing to.
   * @throws IOException If an internal method throws it.
   */
  void readNullBytesOfRecord(int whichColumnNull,
                             boolean dataStreamIsDislocated,
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  void sortTable(SQLColumnListClause groupByClause,
    ...
    throws IOException, InvalidDateException
  {
    ...
    if (orderByClause != null) {
      orderByClause.bindColumnsSQLColumnListClause(
        htName2index, columnTypes, null);
    }
    if (groupByClause != null) {
      groupByClause.bindColumnsSQLColumnListClause(
        htName2index, columnTypes, null);
    }

    // Picks one of the Column List clauses as the sort list.
    SQLColumnListClause sortListClause =
      (orderByClause == null ? groupByClause : orderByClause);
    ...
    quickSort(0,
              db.rowCount - 1,
              SQLValue.newSQLValues(count),
              SQLValue.newSQLValues(count),
              SQLValue.newSQLValues(count),
              bufAux,
              sortListClause.fieldList,
              driver);
  }
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  private void quickSort(int first,
                         int last,
                         SQLValue[] pivot,
                         SQLValue[] someRecord1,
                         SQLValue[] someRecord2,
                         byte[] bufAux,
    ...
    throws IOException, InvalidDateException
  {
    ...
    int rowSize = plainDB.rowSize, size = 2, low, high,
        pivotIndex; // guich@212_3: now using random partition (improves worst
                    // case 2000x).
    ...
    while (size >
           0) { // guich@212_3: removed recursion (storing in a IntVector).
      ...
      pivotIndex =
        high == low ? high
                    : r.between(low,
                                high); // guich@212_3: now using random
                                       // partition (improves worst case 2000x).
      ...
      while (true) { // Finds the partitions.
        while (high >= low) {
          ...
          if (Utils.compareRecords(
                someRecord1, pivot, nulls1, nulls3, fieldList) >= 0) {
            break;
          }
          low++;
        }
        ...
        while (high >= low) {
          ...
          if (Utils.compareRecords(
                someRecord2, pivot, nulls2, nulls3, fieldList) <= 0) {
            break;
          }
          high--;
        }
        ...
        if (low <= high) {
          // juliana@114_8: optimized the swap of the records. Now the buffer is
          // written at once.
   ...
   * @param driver The connection with Litebase.
   * @param fieldList A field list that indicates which fields to read from the
   *     table.
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  void readRecord(SQLValue[] record,
                  int recPos,
                  int whichColumnNull,
                  LitebaseConnection driver,
                  SQLResultSetField[] fieldList,
                  ...
                  String[][] strings) throws IOException, InvalidDateException
  {
    ...
    plainDB.read(recPos);
    readNullBytesOfRecord(whichColumnNull,
                          false,
                          0); // Reads the null bytes of the end of the record.

    byte[] nulls = columnNulls[whichColumnNull];
    short[] offsets = columnOffsets;
    byte[] types = columnTypes;

    // juliana@226_12: corrected a bug that could make aggregation function not
    // work properly.
    if ((plainDB.basds.readInt() & Utils.ROW_ATTR_MASK) ==
          Utils.ROW_ATTR_DELETED &&
        name != null) {
  ...
  /**
   * Remaps a table column names, so it uses the alias names of the given field
   * list, instead of the original names.
   *
  ...
  void remapColumnsNames2Aliases(SQLResultSetField[] fieldList)
  {
    ...
    if (names == null) {
      ...
      while (--i >= 0) {
        ...
        if (hashes[i] == field.aliasHashCode) { // Replaces the original
  ...
  // juliana@222_9: Some string conversions to numerical values could return
  // spourious values if the string range were greater than the type range.
  /**
   * Converts the strings of the record into the real values, accordingly to the
   * given table column types.
   *
   * @param record The record whose strings are to be transformed in their real
   *     types.
   * @throws InvalidDateException If an internal method throws it.
   * @throws InvalidNumberException  If an internal method throws it.
   */
  void convertStringsToValues(SQLValue[] record)
    throws InvalidNumberException, InvalidDateException
  {
    int i = record.length, type;
    byte[] types = columnTypes;
    String strVal;
    ...
    while (--i > 0) { // 0 = rowid.
      ...
      // A blob can't be set in a normal statement (if the string is not null,
      // it won't be a "?".
      if ((type = types[i]) == SQLElement.BLOB && strVal != null &&
          !strVal.equals("?")) {
        throw new DriverException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_BLOBS_PREPARED));
      }

      // Ignores null values, blobs or unset parameters.
      if (strVal == null || strVal.equals("?") ||
          type ==
            SQLElement.BLOB) { // guich@tc100b4: also continue if its blob.
        continue;
      }
      ...
      switch (type) {
        ...
        case SQLElement.DATE: // DATE
          record[i].asInt =
            db.driver.tempDate.set(strVal.trim(), Settings.DATE_YMD);
          break;
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  void writeRecord(SQLValue[] values, int recPos)
    throws DriverException, IOException, InvalidDateException
  {
    int n = columnCount, // nowosad@200: the array of values may be larger than
                         // necessary; so use the table field count to truncate
                         // it.
      j, i = n, rowid = 0, writePos = -1, offset = 2;
    ByteArrayStream bas = db.bas;
    DataStreamLB ds =
      db.basds; // juliana@253_8: now Litebase supports weak cryptography.
    Index idx;
    boolean addingNewRecord = recPos == -1, changePos = false;
    byte[] nulls = storeNulls;
    ...
    bas.reset();
    Convert.fill(columnNulls0,
                 0,
                 columnNulls0.length,
                 0); // First of all, clear the columnNulls used.

    while (--i > 0) { // 0 = rowid = never is null.
      // juliana@225_7: a PrimaryKeyViolation was not being thrown when two
      // strings with the same prefix were inserted and the field definition had
      // the size of the prefix and a primary key.
      if ((types[i] == SQLElement.CHARS ||
           types[i] == SQLElement.CHARS_NOCASE) &&
          values[i] != null && values[i].asString != null &&
          values[i].asString.length() > sizes[i]) {
        values[i].asString = values[i].asString.substring(0, sizes[i]);
      }

      if (addingNewRecord) {
        if ((nulls[i >> 3] & (1 << (i & 7))) ==
            0) { // If not explicit to store null.
          if ((attrs[i] & Utils.ATTR_COLUMN_IS_NOT_NULL) == 0) { // Can be null.
            if (values[i] == null || values[i].isNull)
              if (defaults[i] == null) { // It doesn't have a default value.
                columnNulls0[i >> 3] |=
                  (1 << (i & 7)); // Sets the column as null.
              } else {
                values[i] = defaults[i]; // If it doesn't have a value, stores
                                         // the default value.
              }

            // At this moment, if it can't be null, necessarily it has a default
            // value.
    ...
    if (primaryKeyCol !=
        Utils
          .NO_PRIMARY_KEY) { // nowosad@200: if table has a primary key, tests
                             // if writing the record does not violate it.
    ...
    if (composedPKCols != null) {
      ...
      while (--i >= 0) { // A field of the composed primary key can't be null.
        ...
        if ((nulls[j >> 3] & (1 << (j & 7))) != 0 ||
            (values[j] != null && values[j].isNull)) {
          throw new DriverException(
            LitebaseMessage.getMessage(LitebaseMessage.ERR_PK_CANT_BE_NULL));
        }
    ...
    if (addingNewRecord) { // Adding a record?
      ...
      values[0].asInt = rowid =
        currentRowId++; // Writes the rowId, marking the attribute as new.
      resetAuxRowId();
    } else { // May have to read the value before deleting the index value.
      plainDB.read(writePos = recPos);
      rowid = ds.readInt();
      bas.reset();
      wasUpdated = true; // juliana@270_27: now purge will also really purge the
                         // table if it only suffers updates.
    }

    int type;
    boolean isNull, valueOk, hasvolds;
    ...
    while (++i < n) { // 0 = rowid
      ...
      idx = indices[i]; // If a new value is being written, the table index (if
                        // any) needs to be updated.
      ...
      if (!addingNewRecord) { // Reads the previous value if it is an update.
        // Verifies if the value to be read is null. IMPORTANT: the array of
        // bytes can't be stored in table.columnNulls[0]. This variable is in
        // use. Then, it will be read in Table.columnNulls[1].
        readNullBytesOfRecord(1, true, i);

        has[i] |=
          (columnNulls1[i >> 3] & (1 << (i & 7))) != 0 ? ISNULL_VOLDS : 0;
        if (vOlds[i] == null) {
        ...
        vOlds[i].asInt = -1; // This is a flag that indicates that blobs are not
                             // to be loaded.
        ...
        readValue(vOlds[i],
                  0,
                  type,
                  (has[i] & ISNULL_VOLDS) != 0,
                  false); // juliana@220_3 juliana@230_14

        if (valueOk && type == SQLElement.BLOB) {
          // juliana@202_21: Always writes the string at the end of the .dbo.
          // This removes possible bugs when doing updates. A blob in the .dbo
          // must have its position changed if the the new value is greater than
          // the old one.
          j = ((asBlob = vOlds[i].asBlob) == null) ? vOlds[i].asInt
                                                   : asBlob.length;
          if (values[i] != null && values[i].asBlob != null &&
              (((asBlob != null || vOlds[i].asInt != -1) &&
                j < values[i].asBlob.length) ||
               asBlob == null)) {
            changePos = true;
      ...
      // Writes the value.
      plainDB.writeValue(type,
                         values[i],
                         ds,
                         valueOk && !isNull,
                         addingNewRecord || changePos,
                         sizes[i],
                         offset,
                         false); // juliana@220_3

      // If the new and the old values are null and the column is not the rowid,
    ...
    ds.writeBytes(
      ...
      0,
      columnNulls0
        .length); // After the columns, stores the bytes of the null values.

    // juliana@230_12: improved recover table to take .dbo data into
    // consideration. juliana@220_4: added a crc32 code for every record. Please
    // update your tables.
    byte[] buffer = bas.getBuffer();
    byte[] byteArray;

    j = buffer[3]; // juliana@270_23: Corrected a RowIterator bug of an update
                   // changing an updated row to synced again.
    buffer[3] = (plainDB.useCrypto ? (byte)0xAA
    ...
    if (version == Table.VERSION) {
      ...
      while (--i > 0)
        if (types[i] == SQLElement.CHARS ||
          ...
          if (values[i] != null && !values[i].isNull) {
            ...
            crc32 =
              Table.updateCRC32(byteArray, byteArray.length, crc32, false);
                     ...
                     (values[i] == null || !values[i].isNull) &&
                     vOlds[i] != null && !vOlds[i].isNull &&
                     vOlds[i].asString != null) {
          ...
          if (values[i] != null && !values[i].isNull) {
            // juliana@239_4: corrected a non-desired possible row delete when
            // recovering a table with blobs.
            intArray[0] =
              ((values[i].asBlob.length > sizes[i]) ? sizes[i]
                                                    : values[i].asBlob.length);
            crc32 = Table.updateCRC32(
              Convert.ints2bytes(intArray, 4), 4, crc32, false);
          }
          // juliana@265_2: corrected a problem where a row could be wrongly
          // deleted by recovering a table when an update was done and the
          // column which was of type blob remained null on Java SE and
          // BlackBerry.
          else if (!addingNewRecord &&
                   (values[i] == null || !values[i].isNull) &&
                   vOlds[i] != null && !vOlds[i].isNull &&
    ...
    ds.writeInt(crc32); // Computes the crc for the record and stores at the end
                        // of the record.
    buffer[3] = (byte)j; // juliana@270_23: Corrected a RowIterator bug of an
                         // update changing an updated row to synced again.

    if (rowid > 0) { // Now the record's attribute has to be updated.
      bas.reset();
      ds.writeInt(addingNewRecord
                    ? (rowid & Utils.ROW_ID_MASK) | Utils.ROW_ATTR_NEW
                    : rowUpdated(rowid)); // Sets the row as new or updated.
    }
    ...
    while (--n >= 0) // Finally, adds the values to the indices. // 0 = rowid
      ...
      if ((has[n] & HAS_IDX) != 0) {
        ...
        if (addingNewRecord) {
          ...
          if (values[n].valueCompareTo(vOlds[n],
                                       types[n],
                                       isNull,
                                       hasvolds = (has[n] & ISNULL_VOLDS) != 0,
                                       null) != 0) {
          if (!hasvolds) {
          ...
          if (!isNull) { // If it is updating a 'non-null value' to 'null
                         // value', only removes it.
            one[0] = values[n];
            idx.indexAddKey(one, writePos);
          }
        }
    ...
    if ((i = numberComposedIndices) > 0) { // Fills the composed indices.
      // If a composed index has all column values equal to null, it is not
      // possible to store this row in the index. Composed indices that have at
      // least one field that is not null can be stored in the index, but the
      // null values must be handled. This implies in changing the index format.
      // Maybe using the same way like null values are handled on tables. This
      // certainly will decrease the index performance. For simplicity, the key
      // will be stored in a composed index only if all values are not null.
      // This is a project choice.
      int size, column;
      boolean store,
        remove, // juliana@230_43
        change; // juliana@252_2: corrected a bug of possible composed index
                // corruption when updating or deleting data.
      ComposedIndex ci;
      SQLValue[] vals = null;
      ...
      while (--i >= 0) {
        ...
        if (ci.indexId > 0) {
          ...
          j = size;
          while (--j >= 0) {
            if ((has[column = columns[j]] & HAS_NULLVAL) !=
                0) { // Only stores non-null values.
              store = false;
            }

            // juliana@230_43: solved a possible exception if updating a table
            // with composed indices and nulls.
            if ((has[column] & ISNULL_VOLDS) != 0) {
              remove = false;
            }

            // Sets the old and new index values.
            // juliana@282_2: Doesn't update the composed index or PK if the old
            // values are the same of the new ones.
            if (values[column] ==
                null) { // juliana@201_18: can't reuse values. Otherwise, it
                        // will spoil the next update.
              vals[j] = vOlds[column];
            } else {
              if (addingNewRecord ||
                  values[column].valueCompareTo(
                    vOlds[column], types[column], !remove, !store, null) != 0) {
                change = true;
              }
          ...
          // juliana@252_2: corrected a bug of possible composed index
          // corruption when updating or deleting data. juliana@230_43: solved a
          // possible exception if updating a table with composed indices and
          // nulls.
          if (!addingNewRecord && remove &&
              change) { // Removes the old composed index entry.
    ...
    // juliana@270_25: corrected a possible lose of records in recover table
    // when 10 is passed to LitebaseConnection.setRowInc().
    NormalFile dbFile = (NormalFile)plainDB.db, dboFile =
                                                  (NormalFile)plainDB.dbo;
    if (!dbFile.dontFlush) { // juliana@202_23: Flushs the files to disk when
                             // row increment is the default.
   ...
   */
  static int updateCRC32(byte[] buffer,
                         int length,
                         int oldCRC,
                         boolean useCrypto)
  {
    ...
    if (useCrypto)
      while (--length >= 0) {
        oldCRC = crcTable[(oldCRC ^ (buffer[offset++] ^ 0xAA)) & 0xff] ^
                 (oldCRC >>> 8);
      }
    else
      while (--length >= 0) {
   ...
   * @throws IOException If an internal method throws it.
   */
  private void resetAuxRowId() throws IOException
  { // rnovais@570_61
  ...
  private int rowUpdated(int id)
  {
    if ((id & Utils.ROW_ATTR_MASK) == Utils.ROW_ATTR_SYNCED) {
      return (id & Utils.ROW_ID_MASK) |
        Utils.ROW_ATTR_UPDATED; // Sets the row as update.
    }
    return id;
  }

  /**
   * Compares two records. Used for sorting the table to build the indices from
   * scratch.
   *
   ...
   * @return A positive number if vals1 > vals2; 0 if vals1 == vals2; -1,
   *     otherwise. It will return <code>MAX_INT_VALUE</code> if both records
   *     are
   * equal but the record of the first is greater than the second, and
   * <code>MIN_INT_VALUE</code> if both records are equal but the record of the
   * first is less than the second.
   * @throws IOException If an internal method throws it.
   */
  private static int compareRecords(SQLValue[] vals1,
                                    SQLValue[] vals2,
                                    byte[] types) throws IOException
  {
    int n = vals1.length, i = -1, result;

    while (++i < n) // Does the comparison between the values till one of them
                    // is different from zero.
      if ((result = vals1[i].valueCompareTo(
             vals2[i], types[i], false, false, null)) != 0) {
        return result;
      }
    if (types[0] != SQLElement.LONG) {
  ...
  /**
   * Quick sort used for sorting the table to build the indices from scratch.
   * This one is simpler than the sort used for order / gropu by. Uses a stack
   ...
   * @throws IOException If an internal method throws it.
   */
  private void sortRecords(SQLValue[][] sortValues,
                           byte[] types,
                           int first,
                           int last) throws IOException
  {
    ...
    while (size >
      ...
      // juliana@213_3: high can't be equal to low.
      mid =
        sortValues[high == low ? high
                               : r.between(low,
                                           high)]; // guich@212_3: now using
                                                   // random partition (improves
                                                   // worst case 2000x).
      ...
      while (true) { // Finds the partitions.
        ...
        while (high >= low &&
               compareRecords(mid, sortValues[high], types) < 0) {
          high--;
        }
  ...
  /**
   * Does a radix sort on the given SQLValue array. Only integral types are
   * allowed (SHORT, INT, LONG). This is faster than quicksort. Also used to
   * build the indices from scratch.
   *
   * @param source The values to be sorted. Only simple records for simple
   *     indices can be used.
   * @param type The type of the elements.
   * @param temp A temporary array for the sort.
   */
  private static void radixSort(SQLValue source[][],
                                int type,
                                SQLValue[][] temp)
  {
    ...
    SQLValue z[][];
    int byteCount =
          (type == SQLElement.INT) ? 4 : (type == SQLElement.SHORT) ? 2 : 8,
        i = 0,
        length = temp.length; // juliana@227_15: corrected a possible
                              // NullPointerException when creating an index in
                              // a column with null values.
    long mask = 0xFF, bits =
                        radixPass(0, source, temp, count, index, type, length);
    ...
    if (temp != source) { // If the final sorted array is not at the source,
                          // copies to it.
   ...
   * @param dest The dest array where the operations with the source are copied
   *     to.
   ...
   */
  private static long radixPass(int start,
                                SQLValue source[][],
                                SQLValue dest[][],
                                int[] count,
                                int[] index,
  ...
  {
    int i = 0,
        n = length, // juliana@227_15: corrected a possible NullPointerException
                    // when creating an index in a column with null values.
      ibits = 0, b, ishift = start << 3;
    long lbits = 0, lshift = start << 3, lb;
    ...
    switch (type) {
      case SQLElement.INT:
      case SQLElement.DATE: // juliana@214_9: index creation for date types
                            // could create corrupted indices.
        if (start == 0)
          while (--n >= 0) {
          ...
          while (--n >= 0) {
            count[(source[i++][0].asInt >> ishift) & 0xFF]++;
          }
        break;
      case SQLElement.SHORT:
        if (start == 0)
    ...
    switch (type) {
      ...
      case SQLElement.LONG:
        if (start == 0)
          ...
          while (--n >= 0) {
            dest[index[(int)((source[i][0].asLong >> lshift) + 128)]++] =
              source[i];
            i++;
          }
        else
          while (--n >= 0) {
            dest[index[(int)((source[i][0].asLong >> lshift) & 0xFF)]++] =
              source[i];
            i++;
          }
    }
  ...
  {
    PlainDB plainDB = db;
    MemoryFile dbFile = (MemoryFile)plainDB.db, dboFile =
                                                  (MemoryFile)plainDB.dbo;
  ...
  void setModified() throws IOException
  {
    ...
    if (plainDB.useCrypto) { // juliana@253_8: now Litebase supports weak
                             // cryptography.
