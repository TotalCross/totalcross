...
class Table {
		...
		if (indexCount == 1) { // Simple index.
			if (columnIndices[idx = columnNumbers[0]] != null)
				throw new AlreadyCreatedException(LitebaseMessage.getMessage(
													  LitebaseMessage.ERR_INDEX_ALREADY_CREATED) + columnNames[idx]);
			return 0;
		} else { // Composed index.
			...
			while (--i >= 0) {
				...
				if (alreadyExists) {
					...
					throw new AlreadyCreatedException(LitebaseMessage.getMessage(
														  LitebaseMessage.ERR_INDEX_ALREADY_CREATED) + cols);
				}
			}
	...
	void driverDropIndex(int column) throws IOException {
		...
		if (index == null) // Column does not have an index.
			throw new DriverException(LitebaseMessage.getMessage(
										  LitebaseMessage.ERR_COLUMN_DOESNOT_HAVE_AN_INDEX) + columnNames[column]);
	 ...
	 */
	void driverDropComposedIndex(byte[] columns, int indexId, boolean saveMD) throws DriverException,
		IOException {
		...
		if (found && ci != null) { // Removes the index.
			...
			throw new DriverException(LitebaseMessage.getMessage(
										  LitebaseMessage.ERR_COLUMN_DOESNOT_HAVE_AN_INDEX) + cols);
		}
		if (saveMD) {
	...
	private void computeColumnOffsets() throws IOException {
		...
		plainDB.setRowSize(sum, plainDB.basbuf == null
						   || sum > plainDB.basbuf.length ? new byte[sum] : plainDB.basbuf); // Sets the new row size.
	 ...
	 */
	private void tableLoadMetaData(String appCrid, String sourcePath,
		...
		if (plainDB.headerSize != bytes.length)
			(ds = new DataStreamLB(new ByteArrayStream(bytes = plainDB.readMetaData()),
								   plainDB.useCrypto)).skipBytes(6);
		...
		if ((((flags = ds.readByte()) & IS_ASCII) != 0 && !plainDB.isAscii) || ((flags & IS_ASCII) == 0)
			&& plainDB.isAscii) {
			// juliana@222_1: the table should not be marked as closed properly if it was not previously closed correctly.
			dbFile.close();
			plainDB.dbo.close();
		...
		if ((version = ds.readShort()) < VERSION -
			1) { // The tables version must be the same as Litebase version.
			// juliana@222_1: the table should not be marked as closed properly if it was not previously closed correctly.
			dbFile.close();
			plainDB.dbo.close();
			throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_WRONG_VERSION) + (" (" +
									  version) + ')');
		}
		...
		while (--i >= 0) // Loads the indices.
			if ((attrs[i] & Utils.ATTR_COLUMN_HAS_INDEX) != 0) {
				...
				if ((exist = new File(fullName = Utils.getFullFileName(nameAux + i + ".idk", sourcePath)).exists())
					&& flags == 0) {
				...
				indexCreateIndex(tableName, i, new int[] {sizes[i]}, new byte[] {types[i]}, appCrid, sourcePath,
								 exist);
		...
		dbFile.setPos(plainDB.headerSize + (plainDB.rowCount > 0 ? plainDB.rowCount - 1 : 0) *
					  plainDB.rowSize);
		currentRowId = (auxRowId != Utils.ATTR_DEFAULT_AUX_ROWID ? auxRowId
						: ((new DataStreamLB(dbFile, plainDB.useCrypto).readInt() & Utils.ROW_ID_MASK) + 1)) &
					   Utils.ROW_ID_MASK;
		...
		if (numCompIndices > 0) {
			...
			while (++i < numCompIndices) {
				...
				// juliana@227_21: corrected a bug of recover table not working correctly if the table has indices.
				if ((exist = new File(fullName = Utils.getFullFileName(nameAux + indexId + ".idk",
												 sourcePath)).exists()) && flags == 0) {
					idxFile = new File(fullName, File.READ_WRITE);
					idxFile.setSize(0);
					exist = false;
					idxFile.close();
				...
				indexCreateComposedIndex(tableName, columns, columnSizes, columnTypes, indexId, aComposedPK == i,
										 appCrid, false, sourcePath, exist);
	...
	void tableSaveMetaData(int saveType) throws IOException {
		...
		if (auxBas != null) { // If the buffer is not empty, only resets it.
			...
			auxDs = tsmdDs = new DataStreamLB(auxBas = tsmdBas = new ByteArrayStream(plainDB.headerSize),
											  plainDB.useCrypto);
		...
		auxDs.writeByte(plainDB.isAscii ? IS_ASCII | j :
		...
		if (saveType !=
			Utils.TSMD_ONLY_DELETEDROWSCOUNT) { // More things other than the deleted rows count must be saved.
			auxDs.writeInt(auxRowId); // rnovais@570_61: saves the auxiliary rowid.

			if (saveType !=
				Utils.TSMD_ONLY_AUXROWID) { // More things other than the auxiliary row id must be saved.
				// juliana@230_5: Corrected a AIOBE when using a table created on Windows 32, Windows CE, Linux, Palm, Android, iPhone, or iPad using
				// primary key on BlackBerry and Eclipse.
				...
				if (saveType !=
					Utils.TSMD_ONLY_PRIMARYKEYCOL) { // More things other than the primary key col must be saved.
		...
		((NormalFile)(
			 plainDB.db)).flushCache(); // juliana@223_11: table meta data is now always flushed imediately after being changed.
	}
		...
		while (++i < length) // Makes sure the fields are in db creation order.
			try {
				outRecord[idx = hashTable.get(fields[i].hashCode())] =
					record[i]; // Finds the index of the field on the table and reorders the record.
				...
				throw new SQLParseException(LitebaseMessage.getMessage(LitebaseMessage.ERR_INVALID_COLUMN_NAME) +
											fields[i]);
			}
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	void tableCreate(String sourcePath, String newName, boolean create, String appCrid,
					 LitebaseConnection driver, boolean ascii, boolean crypto,
					 boolean throwException) throws IOException, InvalidDateException {
		PlainDB plainDB = db = new PlainDB(newName, sourcePath,
										   create); // Creates or opens the table files.
		plainDB.driver = driver;
		if (newName != null && (plainDB.db.size != 0
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	void renameTable(LitebaseConnection driver, String oldTableName,
	...
	void renameTableColumn(String oldColumn, String newColumn) throws IOException, DriverException {
		...
		if (oldIdx == -1)
			throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_INVALID_COLUMN_NAME) +
									  oldColumn);

		int newHashCode = newColumn.hashCode();

		if (ht.exists(newHashCode)) // The new column name can't exist.
			throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_COLUMN_ALREADY_EXIST) +
									  newColumn);
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	void tableSetMetaData(String[] names, int[] hashes, byte[] types, int[] sizes, byte[] attrs,
						  SQLValue[] values, int pkCol,
						  int composedPKIdx, byte[] composedPKColums, int ComposedPKColsSize) throws AlreadyCreatedException,
		IOException {
		...
		if (name !=
			null) { // juliana@201_14: It is not necessary to save the meta data in the .db for memory tables.
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	ResultSet createSimpleResultSet(SQLBooleanClause whereClause) throws IOException,
		InvalidDateException {
		ResultSet rs = new ResultSet();
		rs.pos = -1; // guich@300: beforeFirst();
		rs.table = this;
		...
		rs.whereClause = whereClause;
		if (whereClause !=
			null) // tries to apply the table indexes to generate a bitmap of the rows to be returned.
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	ResultSet createResultSetForSelect(SQLBooleanClause whereClause) throws IOException,
		InvalidDateException {
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	void tableReIndex(int column, ComposedIndex composedIndex,
					  boolean isPrimaryKey) throws DriverException,
		...
		try {
			if (index.isOrdered && composedIndex == null) { // Simple index using rowid.
				...
				int rows =
					plainDb.rowCount; // juliana@284_1: Solved a possible application crash when recreating indices.
				...
				while (++i < n) {
					...
					readNullBytesOfRecord(0, false,
										  0); // juliana@201_22: the null columns information wasn't being read when re-creating an index.
					...
					if (composedIndex == null) {
						...
						readValue(vals[k][0], offsets[column], types[0],
					...
					if (isNull) { // Do not store null records.
						...
						if (types[0] ==
							SQLElement.LONG) { // The record value is stored in an empty field of the first record column value.
							vals[k++][0].asInt = i;
						} else {
							vals[k++][0].asLong = i;
						}
				...
				if (!index.isOrdered) {
					...
					if (numberColumns == 1
						&& (types[0] == SQLElement.SHORT || types[0] == SQLElement.INT || types[0] == SQLElement.LONG
							|| types[0] == SQLElement.DATE)) {
				...
				while (++count < rows) {
					...
					if (isPrimaryKey && count > 0
						&& ((compare = compareRecords(vals[count], vals[count - 1], types)) == 0
							|| compare == Convert.MAX_INT_VALUE || compare == Convert.MIN_INT_VALUE))
						throw new PrimaryKeyViolationException(LitebaseMessage.getMessage(
								LitebaseMessage.ERR_STATEMENT_CREATE_DUPLICATED_PK) + name);
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	void readValue(SQLValue value, int offset, int colType, boolean isNull,
		...
		offset = plainDB.readValue(value, offset, colType, plainDB.basds, name == null, isNull,
	...
	void indexCreateIndex(String fullTableName, int column, int[] columnSizes, byte[] columnTypes,
		...
		Index index = columnIndices[column] = new Index(this, columnTypes, columnSizes, name, sourcePath,
				exist);
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	void indexCreateComposedIndex(String fullTableName, byte[] columnIndices, int[] columnSizes,
								  byte[] columnTypes, int newIndexNumber, boolean isPK,
								  String crid, boolean increaseArray, String sourcePath, boolean exist) throws DriverException,
		IOException, InvalidDateException {
		ComposedIndex ci;
		...
		ci.index = new Index(this, columnTypes, columnSizes, name, sourcePath,
	...
	void writeRSRecord(SQLValue[] values) throws IOException {
		...
		i = -1;
		while (++i < n)
			plainDB.writeValue(types[i], values[i], ds, (nulls[i >> 3] & (1 << (i & 7))) == 0, true, sizes[i],
							   0, true); // juliana@220_3
	 ...
	 */
	private void checkPrimaryKey(SQLValue[] vals, int recPos, boolean newRecord,
								 SQLValue[] values) throws IOException, InvalidDateException, DriverException {
	 ...
	 */
	void verifyNullValues(SQLValue[] record, byte[] storeNullsStmt,
		...
		if (statementType == SQLElement.CMD_INSERT) { // Insert statement.
			...
			while (--len > 0) // A not null field can't have a null.
				if ((record[len] == null || record[len].isNull) && defaults[len] == null
					&& (attrs[len] & Utils.ATTR_COLUMN_IS_NOT_NULL) != 0)
					throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_FIELD_CANT_BE_NULL) +
											  columnNames[len]);
		} else { // Update statement.
			if (primaryKeyCol != Utils.NO_PRIMARY_KEY
				&& (nulls[primaryKeyCol >> 3] & (1 << (primaryKeyCol & 7))) !=
	 ...
	 */
	int writeResultSetToTable(ResultSet[] list, short[] rs2TableColIndexes,
		...
		if (!hasJoin) {
			...
			if (rs.whereClause == null) {
				...
				plainDB.rowAvail = (rs.rowsBitmap == null ? rs.table.db.rowCount - rs.table.deletedRowsCount :
			...
			if (rs2TableColIndexes == null)
				while (rs.getNextRecord()) { // No preverify needed.
					...
					Vm.arrayCopy(rsColumnNulls, 0, nulls, 0,
								 rsColumnNulls.length); // Since all the columns match, just copies the nulls.
				...
				while (rs.getNextRecord()) { // No preverify needed.
					...
					while (--i >= 0) { // Gets the values of the result set columns.
						...
						if (!(isNull = ((colIndex = rs2TableColIndexes[i]) == -1
										|| (rsColumnNulls[colIndex >> 3] & (1 << (colIndex & 7))) != 0))) {
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	private int performJoin(ResultSet[] list, short[] rs2TableColIndexes, SQLSelectClause selectClause,
							SQLValue[] valuesJoin, int whereClauseType) throws IOException, InvalidDateException,
		...
		while (currentIndexTable >= 0) {
			...
			nulls = columnNulls[0];
			ret = getNextRecordJoin(list, currentIndexTable, !ignoreWhereCondition[currentIndexTable],
									whereClauseType);
			switch (ret) {
				...
				case VALIDATION_RECORD_INCOMPLETE:
					...
					while (--len >= 0) {
						...
						Utils.setBit(nulls, pos, colIndex != -1
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	private int getNextRecordJoin(ResultSet[] list, int rsIndex, boolean verifyWhereCondition,
								  int whereClauseType) throws IOException,
		...
		if (rowsBitmap != null && verifyWhereCondition) {
				...
				if (whereClause == null || auxRowsBitmap != null) { // count_index++;
					...
					if (rs.rowsBitmapBoolOp == SQLElement.OP_BOOLEAN_AND) {
						...
						while ((p = Utils.findNextBitSet(items, rs.pos + 1)) != -1 && p <= last) {
							db.read(rs.pos = p);
							if (db.recordNotDeleted())
								return booleanTreeEvaluateJoin(list,
															   (list[rsIndex].whereClause.resultSet = list[rsIndex]).whereClause.expressionTree);
						}
					} else
			...
			while (rs.pos < last) { // count_noindex++;
				...
				if (db.recordNotDeleted()) {
					...
					ret = booleanTreeEvaluateJoin(list,
												  (rs.whereClause.resultSet = list[rsIndex]).whereClause.expressionTree);
					if (ret == VALIDATION_RECORD_NOT_OK
						&& rs.whereClause.appliedIndexesBooleanOp == SQLElement.OP_BOOLEAN_OR)
						while (++rsIndex < len)
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	private int booleanTreeEvaluateJoin(ResultSet[] list, SQLBooleanClauseTree tree) throws IOException,
		InvalidDateException,
		InvalidNumberException {
			...
			if (indexTree >
				...
				if (tree.bothAreIdentifier && leftTree.indexRs == indexRs) { // Fills leftTree.value.
					...
					if (rightTree.hasIndex && boolOp <= 1) {
						...
						SQLSelectStatement.computeIndex(list, false, rightTree.indexRs, leftTree.valueJoin,
														tree.operandType, rightTree.colIndex);
		...
		switch (tree.operandType) { // Checks what is the operand type of the tree.
			...
			case SQLElement.OP_PAT_MATCH_NOT_LIKE:
				return tree.matchStringOperands(tree.valueType == SQLElement.CHARS_NOCASE) ? VALIDATION_RECORD_OK :
					...
					switch (booleanTreeEvaluateJoin(list, leftTree)) {
						...
						case VALIDATION_RECORD_INCOMPLETE:
							if (booleanTreeEvaluateJoin(list,
														rightTree) == VALIDATION_RECORD_NOT_OK) { // Verifies the right branch.
								return VALIDATION_RECORD_NOT_OK;
							}
						...
						case VALIDATION_RECORD_OK:
							switch (booleanTreeEvaluateJoin(list,
															rightTree)) { // The left side returned true, so verifies the right branch.
								case VALIDATION_RECORD_NOT_OK:
									return VALIDATION_RECORD_NOT_OK;
								case VALIDATION_RECORD_INCOMPLETE_OK:
								case VALIDATION_RECORD_OK:
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	void readNullBytesOfRecord(int whichColumnNull, boolean dataStreamIsDislocated,
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	void sortTable(SQLColumnListClause groupByClause, SQLColumnListClause orderByClause,
				   LitebaseConnection driver) throws IOException,
		InvalidDateException {
		...
		quickSort(0, db.rowCount - 1, SQLValue.newSQLValues(count), SQLValue.newSQLValues(count),
				  SQLValue.newSQLValues(count), bufAux,
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	private void quickSort(int first, int last, SQLValue[] pivot, SQLValue[] someRecord1,
		...
		while (size > 0) { // guich@212_3: removed recursion (storing in a IntVector).
			...
			pivotIndex = high == low ? high : r.between(low,
						 high); // guich@212_3: now using random partition (improves worst case 2000x).
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	void readRecord(SQLValue[] record, int recPos, int whichColumnNull, LitebaseConnection driver,
					SQLResultSetField[] fieldList, boolean isTempBlob,
		...
		if (fieldList == null) // Reads all columns of the table.
			while (--i >= 0)
				readValue(record[i], offsets[i], types[i], (nulls[i >> 3] & (1 << (i & 7))) != 0,
			...
			while (--i >= 0) {
				...
				if ((types[j] != SQLElement.CHARS && types[j] != SQLElement.CHARS_NOCASE)
					...
					readValue(record[j], offsets[j], types[j], (nulls[j >> 3] & (1 << (j & 7))) != 0,
							  false); // juliana@230_14
	...
	void convertStringsToValues(SQLValue[] record) throws InvalidNumberException, InvalidDateException {
		...
		while (--i > 0) { // 0 = rowid.
			...
			if (strVal == null || strVal.equals("?")
				|| type == SQLElement.BLOB) { // guich@tc100b4: also continue if its blob.
				continue;
			}
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	void writeRecord(SQLValue[] values, int recPos) throws DriverException, IOException,
		InvalidDateException {
		...
		if (primaryKeyCol !=
			Utils.NO_PRIMARY_KEY) { // nowosad@200: if table has a primary key, tests if writing the record does not violate it.
		...
		if (addingNewRecord) { // Adding a record?
			...
			bas.reset();
			wasUpdated =
				true; // juliana@270_27: now purge will also really purge the table if it only suffers updates.
		}
		...
		while (++i < n) { // 0 = rowid
			...
			// Writes the value.
			plainDB.writeValue(type, values[i], ds, valueOk && !isNull, addingNewRecord
							   || changePos, sizes[i], offset, false); // juliana@220_3
		...
		ds.writeBytes(columnNulls0, 0,
					  columnNulls0.length); // After the columns, stores the bytes of the null values.
		...
		buffer[3] = (plainDB.useCrypto ? (byte)0xAA :
					 0); // juliana@222_5: The crc was not being calculated correctly for updates.
		...
		if (version == Table.VERSION) {
			...
			while (--i > 0)
				if (types[i] == SQLElement.CHARS || types[i] == SQLElement.CHARS_NOCASE) {
					...
					} else if (!addingNewRecord && (values[i] == null || !values[i].isNull) && vOlds[i] != null
							   && !vOlds[i].isNull && vOlds[i].asString != null) {
					...
					// column which was of type blob remained null on Java SE and BlackBerry.
							 ...
							 && !vOlds[i].isNull && vOlds[i].asInt != -1) {
		...
		buffer[3] = (byte)
					j; // juliana@270_23: Corrected a RowIterator bug of an update changing an updated row to synced again.

		if (rowid > 0) { // Now the record's attribute has to be updated.
			bas.reset();
			ds.writeInt(addingNewRecord ? (rowid & Utils.ROW_ID_MASK) | Utils.ROW_ATTR_NEW : rowUpdated(
							rowid)); // Sets the row as new or updated.
		}
		...
		while (--n >= 0) // Finally, adds the values to the indices. // 0 = rowid
			...
			if ((has[n] & HAS_IDX) != 0) {
				...
				if (addingNewRecord) {
					...
					if (values[n].valueCompareTo(vOlds[n], types[n], isNull, hasvolds = (has[n] & ISNULL_VOLDS) != 0,
												 null) != 0) {
						if (!hasvolds) {
			...
			while (--i >= 0) {
				...
				if (ci.indexId > 0) {
					...
					while (--j >= 0) {
						...
						if (values[column] ==
							null) { // juliana@201_18: can't reuse values. Otherwise, it will spoil the next update.
							vals[j] = vOlds[column];
						} else {
							if (addingNewRecord
								|| values[column].valueCompareTo(vOlds[column], types[column], !remove, !store, null) != 0) {
								change = true;
							}
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	private static int compareRecords(SQLValue[] vals1, SQLValue[] vals2,
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	private void sortRecords(SQLValue[][] sortValues, byte[] types, int first,
		...
		while (size > 0) { // guich@212_3: removed recursion (storing in a IntVector).
			...
			// juliana@213_3: high can't be equal to low.
			mid = sortValues[high == low ? high : r.between(low,
							 high)]; // guich@212_3: now using random partition (improves worst case 2000x).
	 ...
	 */
	private static long radixPass(int start, SQLValue source[][], SQLValue dest[][], int[] count,
								  int[] index, int type,
								  int length) {
		int i = 0,
