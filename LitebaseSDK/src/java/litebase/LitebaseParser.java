...
class LitebaseParser {
	 ...
	 */
	static void parser(String sql, LitebaseParser parser,
					   LitebaseLex lexer) throws InvalidNumberException {
				...
				switch (yylex()) {
					case TK_ADD: // Adds a primary key or a new column.
						...
						if ((token = createColumn()) == TK_PRIMARY) { // Adds a primary key.
							...
							if (field.isNotNull
								&& field.defaultValue == null) { // A field declared as not null must have a default value.
			...
			case TK_CREATE: {
				switch (yylex()) {
					case TK_TABLE: // Create table.
						...
						// Primary key.
						if ((token = createColumnCommalist()) == TK_PRIMARY && yylex() == TK_KEY && yylex() == TK_OPEN
			...
			case TK_UPDATE: // Update.
				...
				if (secondFieldUpdateTableName != null) { // Verifies if there was an error on field.tableName.
					if (!tableAlias.equals(firstFieldUpdateTableName))
						yyerrorWithMessage(LitebaseMessage.getMessage(LitebaseMessage.ERR_INVALID_COLUMN_NAME) +
	...
	private int createColumn() throws InvalidNumberException {
		...
		if (token == TK_DEFAULT) { // Default value.
			...
			if (((type == SQLElement.CHARS || type == SQLElement.CHARS_NOCASE || type == SQLElement.DATE
				  || type == SQLElement.DATETIME) && token == TK_NUMBER)
		...
		fieldList[fieldListSize++] = new SQLFieldDefinition(columnName, type, size, isPrimaryKey,
				strDefault, isNotNull);
		return token;
	}
	...
	private int factor(int token) {
		...
		if ((token = singleExp(token)) == TK_EQUAL || token == TK_LESS || token == TK_DIFF
		...
		if (token == TK_IS) { // factor = single expression is [not] null.
			...
			(tree.rightTree = setOperandType(SQLElement.OP_PAT_NULL)).parent = (tree.leftTree = auxTree).parent
					= tree;
			auxTree = tree;
	...
	private int singleExp(int token) {
		...
		if (token == TK_NUMBER) { // single expression = number
			...
			(whereClause.paramList[whereClause.paramCount++] = auxTree = new SQLBooleanClauseTree(
				whereClause)).isParameter = true;
			return yylex();
			...
			int hashCode = tree.nameSqlFunctionHashCode = tree.nameHashCode = (tree.operandName =
			...
			SQLResultSetField paramField = field.parameter = new
			SQLResultSetField(); // Creates the parameter field.
	...
	private int tableList() {
		...
		do {
			...
			if (tables.exists(hash))
				yyerrorWithMessage(LitebaseMessage.getMessage(LitebaseMessage.ERR_NOT_UNIQUE_ALIAS_TABLE) +
								   tableAlias);
			else {
	...
	private int updateExpCommalist() {
		...
		do {
			...
			if (firstFieldUpdateTableName ==
				null) { // After the table name verification, the associated table name on the field name is discarded.
	 ...
	 */
	private void addColumnFieldOrderGroupBy(SQLResultSetField field, boolean isAscending,
											boolean isOrderBy) {
	...
	void yyerror(int error) throws SQLParseException {
		throw new SQLParseException(LitebaseMessage.getMessage(LitebaseMessage.ERR_MESSAGE_START)
									+ LitebaseMessage.getMessage(error) + LitebaseMessage.getMessage(
										LitebaseMessage.ERR_MESSAGE_POSITION) + lexer.yyposition + '.');
	}
	...
	private void yyerrorWithMessage(String message) {
		throw new SQLParseException(LitebaseMessage.getMessage(LitebaseMessage.ERR_MESSAGE_START) + message
