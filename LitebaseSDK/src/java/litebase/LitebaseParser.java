...
class LitebaseParser
{
   ...
   * The lex main method.
   *
   * @return The token code, -1 if the end of file was reached or 256 if there
   *     was a lexical error.
   */
  private int yylex() { return lexer.yylex(); }

  /**
   * The method which executes the parser process.
   *
   * @param sql The sql command to be parsed.
   * @param parser The parser object which will be filled with the result of the
   *     parsing process.
  ...
  /**
   * Gets an instance of a where clause or a having clause, depending of what
   * clause is used in the <code>SELECT</code> statement.
   *
  ...
  /**
   * Gets an instance of an order by clause used in the <code>SELECT</code>
   * statement.
   *
  ...
  /**
   * Gets an instance of a group by clause used in the <code>SELECT</code>
   * statement.
   *
  ...
  private void yyparse() throws InvalidNumberException
  {
    ...
    switch (yylex()) {
      case TK_ALTER: // Alter table.
        ...
        tableList[0] =
          new SQLResultSetTable(yylval); // There's no alias table name here.
        ...
        switch (yylex()) {
          case TK_ADD: // Adds a primary key or a new column.
            ...
            if ((token = createColumn()) == TK_PRIMARY) { // Adds a primary key.
              ...
              if (yylex() != TK_KEY || yylex() != TK_OPEN ||
                  colnameCommaList() != TK_CLOSE) {
                yyerror(LitebaseMessage.ERR_SYNTAX_ERROR);
              }
              ...
              if (field.isNotNull &&
                  field.defaultValue == null) { // A field declared as not null
              ...
              if (field.isPrimaryKey) { // The new field can't be declared as a
                ...
                if (field.defaultValue !=
                    null) // All the keys would be the same.
                  yyerrorWithMessage(
                    LitebaseMessage.getMessage(
                      LitebaseMessage.ERR_STATEMENT_CREATE_DUPLICATED_PK) +
                    tableList[0].tableName);

                yyerror(LitebaseMessage
                          .ERR_PK_CANT_BE_NULL); // All the keys would be null.
              }
      ...
      case TK_CREATE: {
        switch (yylex()) {
          case TK_TABLE: // Create table.
            ...
            tableList[0] = new SQLResultSetTable(
              yylval); // There's no alias table name here.

            // Primary key.
            if ((token = createColumnCommalist()) == TK_PRIMARY &&
                yylex() == TK_KEY && yylex() == TK_OPEN &&
                colnameCommaList() == TK_CLOSE) {
              if (number_pk == 1) {
          ...
          case TK_INDEX: // Create index.
            ...
            tableList[0] = new SQLResultSetTable(
              yylval); // There's no alias table name here.
            if (yylex() != TK_OPEN ||
                colnameCommaList() != TK_CLOSE) { // Column name list.
              yyerror(LitebaseMessage.ERR_SYNTAX_ERROR);
            }
      ...
      case TK_DELETE: // Delete.
        if (!(((token = yylex()) == TK_FROM && yylex() == TK_IDENT) ||
              token == TK_IDENT)) {
          yyerror(LitebaseMessage.ERR_SYNTAX_ERROR);
        }
        tableName = yylval;
      ...
      case TK_INSERT: // Insert.
        ...
        if (token != TK_VALUES || yylex() != TK_OPEN ||
            listValues() != TK_CLOSE) { // Reads the value list.
          yyerror(LitebaseMessage.ERR_SYNTAX_ERROR);
        }

        // If the default order is not used, the number of values must be equal
        // to the number of fields.
        if (fieldNamesSize != 0 && fieldNamesSize != fieldValuesSize)
          yyerrorWithMessage(
            LitebaseMessage.getMessage(
              LitebaseMessage.ERR_NUMBER_FIELDS_AND_VALUES_DOES_NOT_MATCH) +
      ...
      case TK_SELECT: // Select.
        ...
        if (selectAux.fieldList[0].isWildcard) {
          ...
          SQLResultSetField[] compactFieldList =
            new SQLResultSetField[selectAux.fieldsCount];
          Vm.arrayCopy(
            selectAux.fieldList, 0, compactFieldList, 0, selectAux.fieldsCount);
      ...
      case TK_UPDATE: // Update.
        ...
        token = optWhereClause(
          updateExpCommalist()); // Update expression list and where clause.

        if (secondFieldUpdateTableName !=
            null) { // Verifies if there was an error on field.tableName.
          if (!tableAlias.equals(firstFieldUpdateTableName))
            yyerrorWithMessage(LitebaseMessage.getMessage(
                                 LitebaseMessage.ERR_INVALID_COLUMN_NAME) +
                               firstFieldUpdateAlias);
          else
            yyerrorWithMessage(LitebaseMessage.getMessage(
                                 LitebaseMessage.ERR_INVALID_COLUMN_NAME) +
                               secondFieldUpdateAlias);
        } else if (firstFieldUpdateTableName != null &&
                   !tableAlias.equals(firstFieldUpdateTableName))
          yyerrorWithMessage(LitebaseMessage.getMessage(
                               LitebaseMessage.ERR_INVALID_COLUMN_NAME) +
                             firstFieldUpdateAlias);
  ...
  {
    int token, fieldNamesSizeAux = fieldNamesSize;
  ...
  {
    int token, type, size = 0;
    boolean isPrimaryKey = false, isNotNull = false;
    String columnName, strDefault = null;
    ...
    if (type == SQLElement.CHARS ||
        type == SQLElement.BLOB) { // Size and multiplier.
      if (yylex() == TK_OPEN && yylex() == TK_NUMBER) {
        ...
        if (type == SQLElement.CHARS &&
            size > (Convert.MAX_SHORT_VALUE << 1) + 1) {
    ...
    if (token == TK_DEFAULT) { // Default value.
      ...
      if (((type == SQLElement.CHARS || type == SQLElement.CHARS_NOCASE ||
            type == SQLElement.DATE || type == SQLElement.DATETIME) &&
           token == TK_NUMBER) ||
          ((type > SQLElement.CHARS && type < SQLElement.CHARS_NOCASE) &&
           token == TK_STR)) {
        yyerror(LitebaseMessage.ERR_SYNTAX_ERROR);
      }
    ...
    fieldList[fieldListSize++] = new SQLFieldDefinition(
      columnName, type, size, isPrimaryKey, strDefault, isNotNull);
    return token;
  }
  ...
  private int expression(int token)
  {
    if ((token = term(token)) ==
      ...
      // juliana@213_1: changed the way a tree with ORs is built in order to
  ...
  private int factor(int token)
  {
    ...
    if (token == TK_NOT) {
      ...
      // The parent node will be the negation operator and the expression will
      // be the right tree.
    ...
    // expression
    if ((token = singleExp(token)) == TK_EQUAL || token == TK_LESS ||
    ...
    if (token == TK_IS) { // factor = single expression is [not] null.
      ...
      (tree.rightTree = setOperandType(SQLElement.OP_PAT_NULL)).parent =
        (tree.leftTree = auxTree).parent = tree;
      auxTree = tree;
    ...
    if (token == TK_LIKE) {
      ...
      (tree.rightTree = rightTree).parent = (tree.leftTree = auxTree).parent =
        tree;
      auxTree = tree;
  ...
  private int singleExp(int token)
  {
    ...
    if (token == TK_NUMBER) { // single expression = number
      if ((tree = auxTree =
             new SQLBooleanClauseTree(getInstanceBooleanClause()))
            .operandValue == null) {
      ...
      (auxTree = new SQLBooleanClauseTree(getInstanceBooleanClause()))
        .setOperandStringLiteral(yylval);
      return yylex();
    } else if (token == TK_INTERROGATION) { // single expression = ?
      SQLBooleanClause whereClause = getInstanceBooleanClause();

      (whereClause.paramList[whereClause.paramCount++] = auxTree =
         new SQLBooleanClauseTree(whereClause))
        .isParameter = true;
      return yylex();
    } else if ((auxToken = dataFunction(token)) !=
               -1) { // single expression = function(...)
      SQLBooleanClause booleanClause = getInstanceBooleanClause();
      int i = 1, index = booleanClause.fieldsCount;
      SQLResultSetField field = auxField;

      (auxTree = tree = new SQLBooleanClauseTree(booleanClause)).operandType =
        SQLElement.OP_IDENTIFIER;
      int hashCode = tree.nameSqlFunctionHashCode = tree.nameHashCode =
      ...
      while (
        booleanClause.fieldName2Index.exists(tree.nameSqlFunctionHashCode)) {
      ...
      SQLResultSetField paramField = field.parameter =
        new SQLResultSetField(); // Creates the parameter field.
      ...
      booleanClause.fieldList[booleanClause.fieldsCount++] =
        field; // Puts the field in the field list.
      ...
      (auxTree = tree = new SQLBooleanClauseTree(booleanClause)).operandType =
        SQLElement.OP_IDENTIFIER;
      int hashCode = field.tableColHashCode = tree.nameSqlFunctionHashCode =
        tree.nameHashCode = (tree.operandName = field.tableColName).hashCode();
      ...
      field.aliasHashCode =
        field.alias.hashCode(); // Sets the hash code of the field alias.
      booleanClause.fieldList[booleanClause.fieldsCount++] =
        field; // Puts the field in the field list.
  ...
  private int tableList()
  {
    ...
    do {
      ...
      if (tables.exists(hash))
        yyerrorWithMessage(LitebaseMessage.getMessage(
                             LitebaseMessage.ERR_NOT_UNIQUE_ALIAS_TABLE) +
                           tableAlias);
      else {
  ...
  private int fieldExp(int token)
  {
    ...
    if (token == TK_ASTERISK) { // All fields.
      ...
      (resultFieldList[selectAux.fieldsCount++] = new SQLResultSetField())
        .isWildcard = true;
      token = yylex();
    } else {
      String alias = null;
      ...
      do {
        ...
        if ((token = field(token)) == TK_AS) { // There is an alias.
          ...
          // If the alias_name is null, the alias must be the name of the
          // column. This was already done before. If the alias is null and the
          // field is a virtual column, raises an exception, since virtual
          // columns require explicit aliases.
          if (auxField.isVirtual) {
            yyerror(LitebaseMessage.ERR_REQUIRED_ALIAS);
          }

          alias =
            auxField.alias; // The null alias name is filled as tableColName or
        ...
        while (--i >= 0)
          if (resultFieldList[i].alias.equals(alias)) {
            yyerrorWithMessage(
              LitebaseMessage.getMessage(LitebaseMessage.ERR_DUPLICATE_ALIAS) +
              alias);
          }

        auxField.aliasHashCode =
          (auxField.alias = alias).hashCode(); // Assigns the alias.
      } while (token == TK_COMMA);
    }
    return token;
  }
  ...
  private int updateExpCommalist()
  {
    ...
    do {
      ...
      if (firstFieldUpdateTableName ==
          null) { // After the table name verification, the associated table
                  // name on the field name is discarded.
  ...
  private int field(int token)
  {
    ...
    if (token == TK_IDENT) { // A pure field.
      ...
      if ((tokenAux = dataFunction(token)) !=
          -1) { // A function applied to a field.
        // Sets the field.
        field = auxField;
        field.isDataTypeFunction = field.isVirtual = true;
        field.dataType = SQLElement.dataTypeFunctionsTypes[field.sqlFunction];
        ...
        field.tableColHashCode = paramField.aliasHashCode =
          paramField.tableColHashCode;
                 ...
                 -1) { // An aggregation function applied to a field.
        // Sets the field.
        field = auxField;
      ...
      selectAux.fieldList[select.fieldsCount++] =
   ...
   * @return The token received if it is not a where clause or the token after
   *     the where clause.
   */
  private int optWhereClause(int token)
  {
    if (token == TK_WHERE) { // Where clause.
      ...
      SQLResultSetField[] compactFieldList =
        new SQLResultSetField[clause.fieldsCount];
      Vm.arrayCopy(
        clause.fieldList, 0, compactFieldList, 0, clause.fieldsCount);
  ...
  private int orderByClause()
  {
    ...
    SQLResultSetField[] compactFieldList =
      new SQLResultSetField[orderByAux.fieldsCount];
    Vm.arrayCopy(
      orderByAux.fieldList, 0, compactFieldList, 0, orderByAux.fieldsCount);
  ...
  private int groupByClause()
  {
    ...
    SQLResultSetField[] compactFieldList =
      new SQLResultSetField[groupByAux.fieldsCount];
    Vm.arrayCopy(
      groupByAux.fieldList, 0, compactFieldList, 0, groupByAux.fieldsCount);
   ...
   * @param isOrder by <code>true</code> if the field comes from an order be
   *     clause. <code>false</code>, otherwise.
   */
  private void addColumnFieldOrderGroupBy(SQLResultSetField field,
                                          boolean isAscending,
                                          boolean isOrderBy)
  {
    SQLColumnListClause listClause = isOrderBy
                                       ? getInstanceColumnListClauseOrderBy()
                                       : getInstanceColumnListClauseGroupBy();

    field.tableColHashCode = field.aliasHashCode =
  ...
  void yyerror(int error) throws SQLParseException
  {
    throw new SQLParseException(
      LitebaseMessage.getMessage(LitebaseMessage.ERR_MESSAGE_START) +
      LitebaseMessage.getMessage(error) +
      LitebaseMessage.getMessage(LitebaseMessage.ERR_MESSAGE_POSITION) +
      lexer.yyposition + '.');
  }
  ...
  {
    throw new SQLParseException(
      LitebaseMessage.getMessage(LitebaseMessage.ERR_MESSAGE_START) + message +
      LitebaseMessage.getMessage(LitebaseMessage.ERR_MESSAGE_POSITION) +
      lexer.yyposition + '.');
  }
}
