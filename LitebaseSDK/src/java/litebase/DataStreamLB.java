...
/**
 * Is the same of <code>DataStreamLE</code> except for being able to use
 * cryptography.
 */
class DataStreamLB extends DataStreamLE
{
  ...
  /**
   * Constructs a new <code>DataStreamLB</code> which sits upon the given stream
   * using little endian notation for multibyte values.
   *
   ...
   * @param throwEOF Indicates if an <code>EOFException</code> should be thrown
   *     if an EOF is found.
   * @return The number of bytes read: count.
   * @throws IOException If the stream reaches its end before all bytes are
   *     read.
   */
  protected int readBytesInternal(byte[] buffer,
                                  int start,
                                  int count,
                                  boolean throwEOF) throws IOException
  {
