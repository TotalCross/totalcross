...
class SQLSelectStatement extends SQLStatement {
	...
	SQLSelectStatement(LitebaseParser parser) {
		...
		(selectClause = parser.select).htName2index = new IntHashtable(parser.select.fieldsCount == 0 ? 3 :
				parser.select.fieldsCount);
	...
	void setParamValue(int index, short val) throws DriverException {
		...
		int whereParamCount = (whereClauseAux  == null ? 0 :
							   whereClauseAux.paramCount); // Gets the where clause parameter count.

		if (index < 0
			|| index >= (whereParamCount + (havingClauseAux == null ? 0 :
											havingClauseAux.paramCount))) { // Checks if the index is within the range.
	 ...
	 */
	void setParamValue(int index, String val) throws DriverException, SQLParseException,
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	ResultSet litebaseDoSelect(LitebaseConnection driver) throws IOException, InvalidDateException,
		...
		while (--i >=
			   0) // juliana@201_28: If a table is re-created after the prepared statement is parsed, there won't be a NPE.
		...
		if (groupByClause == null && havingClause == null && orderByClause == null && whereClause == null
			&& !selectClause.hasAggFunctions
		...
		if (rsBaseTable.answerCount >= 0) {
			...
			Vm.arrayCopy(rsBaseTable.allRowsBitmap, 0,
						 rs.allRowsBitmap = new byte[rsBaseTable.allRowsBitmap.length], 0,
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	SQLSelectStatement litebaseBindSelectStatement(LitebaseConnection driver) throws
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	private void bindSelectStatement(LitebaseConnection driver) throws InvalidDateException,
		InvalidNumberException {
				...
				while (--i >= 0) {
					...
					while (--j >= 0) {
						if (field1.aliasHashCode == (field2 = selectFields[j]).aliasHashCode) {
							...
							throw new SQLParseException(field1.alias + LitebaseMessage.getMessage(
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	private Table generateResultSetTable(LitebaseConnection driver) throws IOException,
		...
		if (selectFieldsCount == 1 && groupBy == null
			&& fieldList[0].sqlFunction == SQLElement.FUNCTION_AGG_COUNT && fieldList[0].isAggregatedFunction) {
		...
		if (where == null && sortListClause == null && select.hasAggFunctions && numTables == 1) {
			...
			if (sortListClause != null && ((where != null && where.expressionTree != null)
										   || select.hasAggFunctions || numTables != 1)) {
				sortListClause.index = -1;
			}

			// juliana@230_14: removed temporary tables when there is no join, group by, order by, and aggregation.
			if ((sortListClause != null && sortListClause.index == -1) || countQueryWithWhere
				|| numTables != 1) {
				...
				tempTable = driver.driverCreateTable(null, null, duplicateIntArray(columnHashes, size),
				...
				totalRecords = tempTable.writeResultSetToTable(listRsTemp, duplicateShortArray(columnIndexes, size),
		...
		if (sortListClause != null) { // Sorts the temporary table, if required.
			if (sortListClause.index == -1) {
				...
				if (sortListClause.fieldList[0].isAscending)
					index.sortRecordsAsc(rsTemp.rowsBitmap, tempTable, record, duplicateShortArray(columnIndexes, size),
										 selectClause);
				else
					index.sortRecordsDesc(rsTemp.rowsBitmap, tempTable, record, duplicateShortArray(columnIndexes,
										  size), selectClause);
		...
		byte[] columnTypesItems = duplicateByteArray(columnTypes,
		...
		Table tempTable2 = driver.driverCreateTable(null, null, columnHashesItems, columnTypesItems,
						   columnSizesItems, null, null,
		...
		boolean aggFunctionExist =
			select.hasAggFunctions; // Initializes the aggregated functions running totals.
		...
		SQLValue[] record1 = SQLValue.newSQLValues((count = tempTable.columnCount) > selectFieldsCount ?
							 count : selectFieldsCount);
		SQLValue[] curRecord = record1;

		// juliana@230_21: MAX() and MIN() now use indices on simple queries.
		if (useIndex) {
			// No rows in the answer.
			if (tableOrig.db.rowCount - tableOrig.deletedRowsCount == 0 || (where != null
					&& Utils.countBits(rsTemp.rowsBitmap.items) == 0)) {
				return tempTable2;
			}
		...
		int[] groupCountCols = new
		int[aggFunctionsColsCount]; // Each column has a groupCount because of the null values.
		...
		for (i = -1; ++i < totalRecords; groupCount++) {
			...
			if (!isTableTemporary
				&& !tempTable.db.recordNotDeleted()) { // Because it is possible to be pointing to a real table, skips deleted records.
				groupCount--;
				continue;
			}
			...
			if (groupBy != null && groupCount != 0
				&& Utils.compareRecords(prevRecord, curRecord, nullsPrevRecord, nullsCurRecord,
										groupBy.fieldList) != 0) {
			...
			if (aggFunctionExist) { // Checks if there are aggregate functions.
				...
				while (--j >= 0) {
					...
					if ((colIndex = paramCols[j]) < 0
						|| (nullsCurRecord[colIndex >> 3] & (1 << (colIndex & 7))) != 0) { // The agg functions skip nulls.
						continue;
					}
							...
							switch (colType) { // Checks the type of the column
								...
								case SQLElement.DATETIME: // rnovais@567_2
									if (groupCountCols[j] == 1) {
										aggValue.asInt = value.asInt;
										aggValue.asShort = value.asShort;
									} else if (aggValue.asInt <= value.asInt && (aggValue.asInt < value.asInt
											   || aggValue.asShort < value.asShort)) {
										aggValue.asInt = value.asInt;
										aggValue.asShort = value.asShort;
									}
								...
								case SQLElement.DATETIME: // rnovais@567_2
									if (groupCountCols[j] == 1) {
										...
										aggValue.asShort = value.asShort;
											   ...
											   || aggValue.asShort > value.asShort)) { // the date is greater, or the time is greater
										aggValue.asInt = value.asInt;
										aggValue.asShort = value.asShort;
									}
							}
		...
		if (writeDelayed && groupCount > 0) { // If there was delayed writing, flushs the last record.
			...
			tempTable2.columnNulls[0] =
				nullsCurRecord; // Takes the null values for the non-aggregate fields into consideration.
			j = aggFunctionsColsCount;
		...
		// Creates the second temporary table.
		Table tempTable3 = driver.driverCreateTable(null, null, tempTable2.columnHashes,
						   tempTable2.columnTypes, tempTable2.columnSizes, null, null,
						   Utils.NO_PRIMARY_KEY, Utils.NO_PRIMARY_KEY, null);
		...
		having.bindColumnsSQLBooleanClause(tempTable3.htName2index, tempTable3.columnTypes,
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	private ResultSet[] createListResultSetForSelect(SQLResultSetTable[] tableList,
		...
		if (whereClause !=
			null) { // Tries to apply the table indices to generate a bitmap of the rows to be returned.
			if (size > 1) { // Join.
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	static void generateIndexedRowsMap(ResultSet[] rsList, boolean hasComposedIndex) throws IOException,
		InvalidDateException {
		SQLBooleanClause whereClause = rsList[0].whereClause;

		if (rsList.length >
			1) { // Applies the table indexes to the where clause. If it was not possible, return.
			if (!whereClause.sqlbooleanclauseApplyTableIndexesJoin()) {
				return;
			}
		} else if (!whereClause.sqlbooleanclauseApplyTableIndices(rsList[0].table.columnIndices,
				   hasComposedIndex)) {
			return;
		}
		...
		if (whereClause.expressionTree ==
			null) { // There is no where clause left, since all rows can be returned using the indexes.
			int i = rsList.length;
			while (--i >= 0) {
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	static void computeIndex(ResultSet[] rsList, boolean isJoin, int indexRsOnTheFly, SQLValue value,
							 int operator,
							 int colIndex)
	 ...
	 */
	private void endAggFunctionsCalc(SQLValue[] record, int groupCount,
									 SQLValue[] aggFunctionsRunTotals,
									 int[] aggFunctionsCodes,
									 int[] aggFunctionsParamCols, int[] aggFunctionsRealParamCols, int aggFunctionsColsCount,
									 byte[] columnTypes, int[] groupCountCols) {
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	Table createIntValueTable(LitebaseConnection driver, int intValue,
							  String colName) throws IOException,
		InvalidDateException {
		SQLValue[] record = SQLValue.newSQLValues(1);
		record[0].asInt = intValue;

		Table table = driver.driverCreateTable(null, null, new int[] {colName.hashCode()}, new byte[] {SQLElement.INT},
											   new int[1], null, null,
											   Utils.NO_PRIMARY_KEY, Utils.NO_PRIMARY_KEY, null);
	 ...
	 * @throws InvalidNumberException If an internal method throws it.
	 */
	private void computeAnswer(ResultSet resultSet) throws IOException, InvalidDateException,
		InvalidNumberException {
		int i;
