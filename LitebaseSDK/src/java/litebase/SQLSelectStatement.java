...
class SQLSelectStatement extends SQLStatement
{
  ...
  SQLSelectStatement(LitebaseParser parser)
  {
    ...
    (selectClause = parser.select).htName2index = new IntHashtable(
      parser.select.fieldsCount == 0 ? 3 : parser.select.fieldsCount);
  ...
  {
    SQLBooleanClause whereClauseAux = whereClause, havingClauseAux =
                                                     havingClause;
    int whereParamCount =
      (whereClauseAux == null
         ? 0
         : whereClauseAux.paramCount); // Gets the where clause parameter count.

    if (index < 0 ||
        index >= (whereParamCount +
                  (havingClauseAux == null
                     ? 0
                     : havingClauseAux.paramCount))) { // Checks if the index
                                                       // is within the range.
      throw new DriverException(LitebaseMessage.getMessage(
        LitebaseMessage.ERR_INVALID_PARAMETER_INDEX));
    }
   ...
   * @throws DriverException If the parameter index is invalid.
   * @throws SQLParserException If a <code>null</code> is used as a parameter of
   *     a where clause.
   * @throws InvalidNumberException If an internal method throws it.
   * @throws InvalidDateException If an internal method throws it.
   */
  void setParamValue(int index, String val)
    throws DriverException, SQLParseException, InvalidNumberException,
           InvalidDateException
  {
    ...
    if (val == null) { // A null can't be in a parameter of a where clause.
      throw new SQLParseException(
        LitebaseMessage.getMessage(LitebaseMessage.ERR_PARAM_NULL));
    }
  ...
  /**
   * This was expected to set null in a given field. Since a null can't be in a
   * parameter of a where clause, this method will only raise an exception.
   *
   * @param index The index of the parameter. Not used since a null can't be in
   *     a parameter of a where clause.
   * @throws SQLParserException A null can't be in a parameter of a where
   *     clause.
   */
  ...
  /**
   * This was expected to set the value of a byte array parameter at the given
   * index. Since blobs can't be in a where clause and a select clause only has
   * parameters in a where clause, this method will only raise an exception.
   *
   * @param index The index of the parameter.
  ...
  {
    throw new SQLParseException(
      LitebaseMessage.getMessage(LitebaseMessage.ERR_BLOB_WHERE));
  }
  ...
  void clearParamValues()
  {
    ...
    if ((clause = whereClause) !=
        null) { // Clears all the parameters of the where clause.
    ...
    if ((clause = havingClause) !=
        null) { // Clears all the parameters of the having clause.
      paramList = clause.paramList;
  ...
  void allParamValuesDefined() throws DriverException
  {
    ...
    if ((clause = whereClause) !=
        null) { // Checks if all the where clause parameters are defined.
      paramList = clause.paramList;
      i = clause.paramCount;
      while (--i >= 0)
        if (!paramList[i].isParamValueDefined)
          throw new DriverException(LitebaseMessage.getMessage(
            LitebaseMessage.ERR_NOT_ALL_PARAMETERS_DEFINED));
    }

    if ((clause = havingClause) !=
        null) { // Checks if all the having clause parameters are defined.
      paramList = clause.paramList;
      i = clause.paramCount;
      while (--i >= 0)
        if (!paramList[i].isParamValueDefined)
  ...
  // Modified to use temporary tables except when the query is a simple select
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  ResultSet litebaseDoSelect(LitebaseConnection driver)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    ...
    while (--i >= 0) // juliana@201_28: If a table is re-created after the
                     // prepared statement is parsed, there won't be a NPE.
    ...
    // order by, and aggregation. juliana@114_10: simple selects do not use
    // temporary tables. juliana@212_4: if the select fields are in the table
    // order beginning with rowid, do not build a temporary table.
    if (groupByClause == null && havingClause == null &&
        orderByClause == null && whereClause == null &&
        !selectClause.hasAggFunctions && selectClause.tableList.length == 1) {
      isSimpleSelect = true;
      ...
      if (rsBaseTable.name == null) {
        ...
        // statement instead of the original column names.
        rsBaseTable.remapColumnsNames2Aliases(selectClause.fieldList);

        // This must be used only for temporary tables.
        rsBaseTable.plainShrinkToSize(); // guich@201_9: always shrink the .db
                                         // and .dbo memory files.
      }
    }
    ...
    rs.fields =
      selectClause.fieldList; // Stores the field list for the meta data.
    rs.isSimpleSelect = isSimpleSelect; // juliana@114_10: indicates if it is a
                                        // simple select or not.
    rs.driver = driver;                 // juliana@220_3

    if (rsBaseTable.answerCount >= 0) {
      rs.answerCount = rsBaseTable.answerCount;

      // juliana@263_3: corrected a bug where a new result set data could
      // overlap an older result set data if both were related to the same
      // table.
      Vm.arrayCopy(rsBaseTable.allRowsBitmap,
                   0,
                   rs.allRowsBitmap =
                     new byte[rsBaseTable.allRowsBitmap.length],
                   0,
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  SQLSelectStatement litebaseBindSelectStatement(LitebaseConnection driver)
    throws InvalidDateException, IOException, InvalidNumberException
  {
    ...
    bindSelectStatement(
      driver); // Validates and binds the select statement to the table list.
  ...
  /**
   * Tries to put as inner table a table that has an index used more often in
   * the where clause, when the where clause has a comparison between fields
   * from different tables. e.g.: <code>select * from table1, table2 where
   * table1.field1 = table2.field2 </code> If only <code>table1.field1</code>
   * has index, changes the select to: <code>select * from table2, table1 where
  ...
  private void orderTablesToJoin()
  {
    ...
    SQLResultSetField[] fields =
      whereClause != null ? whereClause.fieldList : null;
    int i = size, j, startedIndexAux, highest;
    SQLResultSetTable rsTableAux1, rsTableAux2;
    Table table1, table2;
      ...
      while (--j >= 0)
        ...
        if (table1.weight > (table2 = (rsTableAux2 = list[j]).table).weight ||
            (table1.weight == table2.weight &&
             table1.db.rowCount > table2.db.rowCount)) {
    ...
    if (whereClause !=
        null) { // Rearranges the indexRs of the where clause fieldlist.
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  private void bindSelectStatement(LitebaseConnection driver)
    throws InvalidDateException, InvalidNumberException
  {
    ...
    int i = listRsTables.length, j = 0, totalLen = 0;
    Table table;
    ...
    if (whereClause != null) { // Binds the where clause if it exists.
      whereClause.bindColumnsSQLBooleanClause(
        names2Index, types, select.tableList);
    }
   ...
   *
   * @throws SQLParseException If a query with group by is not well-formed, if
   *     there is a having clause without an aggregation, a field in the
   * having clause is not in the select clause, there is no order by and there
   * are aggregated functions mixed with real columns, or there is an
   * aggregation with an order by clause and no group by clause.
   */
  private void validateSelectStatement() throws SQLParseException
  {
    SQLColumnListClause groupBy = groupByClause, orderBy = orderByClause;
    SQLSelectClause select = selectClause;
    ...
    int selectCount = select.fieldsCount, i = selectCount, j;
    SQLResultSetField field1, field2;
    ...
    if (groupBy != null) { // Validates the group by clause if it exists.
      ...
      while (--i >= 0) {
        ...
        if (!field1.isAggregatedFunction && field1.isVirtual)
          throw new SQLParseException(LitebaseMessage.getMessage(
            LitebaseMessage.ERR_VIRTUAL_COLUMN_ON_GROUPBY));
      ...
      // Checks if all fields referenced in the HAVING clause are listed as
      // aliased aggregated functions in the SELECT clause.
      if (having != null) {
        ...
        while (--i >= 0) {
          ...
          while (--j >= 0) {
            if (field1.aliasHashCode ==
              ...
              throw new SQLParseException(
                field1.alias + LitebaseMessage.getMessage(
                                 LitebaseMessage.ERR_IS_NOT_AGGREG_FUNCTION));
            }
          }
          ...
          if (!found) // The having clause fields must be in the select clause
            ...
            throw new SQLParseException(
              ...
              LitebaseMessage.getMessage(
                LitebaseMessage.ERR_WAS_NOT_LISTED_ON_AGGREG_FUNCTION));
        }
      }
    } else {
      ...
      if (select.hasRealColumns && select.hasAggFunctions)
        throw new SQLParseException(LitebaseMessage.getMessage(
          LitebaseMessage.ERR_CANNOT_MIX_AGGREG_FUNCTION));

      // If there are aggregate functions with an ORDER BY clause with no GROUP
      // BY clause, also raises an exception.
      if (select.hasAggFunctions && orderBy != null)
        throw new SQLParseException(LitebaseMessage.getMessage(
          LitebaseMessage.ERR_CANNOT_HAVE_AGGREG_AND_NO_GROUPBY));
    }
  }
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  private Table generateResultSetTable(LitebaseConnection driver)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    ResultSet[] listRsTemp;
    SQLSelectClause select = selectClause;
    SQLColumnListClause orderBy = orderByClause, groupBy = groupByClause;
    SQLBooleanClause where = whereClause, having = havingClause;
    ...
    int numTables = tableList.length, i = numTables,
        count = sortListClause != null ? sortListClause.fieldsCount : 0,
        totalRecords, selectFieldsCount = select.fieldsCount;
    ...
    // Optimization for queries that just wants to count the number of records
    // of a table ("SELECT COUNT(*) FROM TABLE").
    boolean countQueryWithWhere = false;
    String countAlias = null;

    // juliana@226_13: corrected a bug where a query of the form "select
    // year(field) as years from table" would be confunded with "select count(*)
    // as years from table".
    if (selectFieldsCount == 1 && groupBy == null &&
        fieldList[0].sqlFunction == SQLElement.FUNCTION_AGG_COUNT &&
    ...
    SQLResultSetField field, param;
    ResultSet rsTemp = null;

    // juliana@270_24: corrected a possible application crash or exception when
    // using order/group by with join.
    IntHashtable colHashesTable =
      new IntHashtable(selectFieldsCount); // The hash table of the columns.

    // Maps the aggregated function parameter column indexes to the aggregate
    // function code.
    ...
    while (++i < selectFieldsCount) {
      ...
      // Decides which hash code to use as the column name in the temporary
      // table and which data type to assign to the temporary table.
      if (field.isVirtual) {
        ...
        if ((param = field.parameter) == null) {
          ...
          // This is just a place holder, since this column does not map to any
          // column in the database.
          columnIndexes[size] = (short)-1;
          columnIndexesTables[size] = null;

          columnTypes[size++] =
            (byte)field.dataType; // Uses the field data type.
        } else {
          ...
          colHashesTable.put(param.aliasHashCode,
                             1); // juliana@253_1: corrected a bug when sorting
                                 // if the sort field is in a function.
        }
      } else {
        ...
        // juliana@270_24: corrected a possible application crash or exception
        // when using order/group by with join.
        colHashesTable.put(field.aliasHashCode,
                           0); // juliana@253_1: corrected a bug when sorting if
                               // the sort field is in a function.
        colHashesTable.put(field.tableColHashCode, 0);
      }
    }

    if (sortListClause != null) {
      sortListClause.findSortIndex(); // juliana@230_29: order by and group by
                                      // now use indices on simple queries.
      ...
      // remembered that, if both present, group by and order by must match. So,
      ...
      i = -1;
      while (++i < count) {
        // juliana@253_1: corrected a sort causing AOOIBE if the sort field is
        // in a function. juliana@270_24: corrected a possible application crash
    ...
    // Creates the temporary table to store records that satisfy the WHERE
    // clause.
    ...
    // For optimization, the first temporary table will NOT be created, in case
    // there is no WHERE clause and sort clause (either ORDER BY or GROUP BY)
    // and the SELECT clause contains aggregated functions. In this case
    // calculation of the aggregated functions will be made on the existing
    // table.
    if (where == null && sortListClause == null && select.hasAggFunctions &&
        numTables == 1) {
      // In this case, there is no need to create the temporary table. Just
      // points to the necessary structures of the original table.
      ...
      if (sortListClause == null &&
          (where == null || where.expressionTree == null) && numTables == 1) {
        // The index should not be used for MAX() and MIN() if not all the
        // fields are MAX() and MIN() or one of the parametes cannot use an
        // index.
        i = -1;
        while (++i < selectFieldsCount)
          if (!(field = fieldList[i]).isAggregatedFunction || field.index < 0 ||
      ...
      // juliana@230_29: order by and group by now use indices on simple
      // queries. Only uses index when sorting if all the indices are applied.
      if (sortListClause != null &&
          ((where != null && where.expressionTree != null) ||
           select.hasAggFunctions || numTables != 1)) {
        sortListClause.index = -1;
      }

      // juliana@230_14: removed temporary tables when there is no join, group
      // by, order by, and aggregation.
      if ((sortListClause != null && sortListClause.index == -1) ||
          countQueryWithWhere || numTables != 1) {
        // Optimization for queries of type "SELECT COUNT(*) FROM TABLE
        // WHERE..." Just counts the records of the result set and writes it to
        // a table.
        ...
        tempTable =
          driver.driverCreateTable(null,
                                   null,
                                   duplicateIntArray(columnHashes, size),
                                   duplicateByteArray(columnTypes, size),
                                   duplicateIntArray(columnSizes, size),
                                   null,
                                   null,
        ...
        totalRecords = tempTable.writeResultSetToTable(
          listRsTemp,
          duplicateShortArray(columnIndexes, size),
          selectClause,
          columnIndexesTables,
      ...
      // A query that use index for MAX() and MIN() should not check now which
      // rows are answered.
      else if (useIndex) {
        tempTable = tableOrig;
    ...
    if (sortListClause != null) { // Sorts the temporary table, if required.
      if (sortListClause.index == -1) {
        ...
        plainDB.rowAvail =
          (rsTemp.rowsBitmap == null
             ? tableOrig.db.rowCount - tableOrig.deletedRowsCount
        ...
        if (sortListClause.fieldList[0].isAscending)
          index.sortRecordsAsc(rsTemp.rowsBitmap,
                               tempTable,
                               record,
                               duplicateShortArray(columnIndexes, size),
                               selectClause);
        else
          index.sortRecordsDesc(rsTemp.rowsBitmap,
                                tempTable,
                                record,
                                duplicateShortArray(columnIndexes, size),
                                selectClause);
        ...
        if (groupBy != null) {
          groupBy.bindColumnsSQLColumnListClause(
            tempTable.htName2index, tempTable.columnTypes, null);
        }
      }
    }
    ...
    // created to perform the sort. juliana@270_24: corrected a possible
    // application crash or exception when using order/group by with join.
    if (sortListClause != null &&
        (count = tempTable.columnCount) != selectFieldsCount) {
    ...
    // functions parameter list types and hashcodes.
    byte[] columnTypesItems =
      duplicateByteArray(columnTypes,
                         size); // Not creating a new array means BUM!
    ...
    Table tempTable2 = driver.driverCreateTable(null,
                                                null,
                                                columnHashesItems,
                                                columnTypesItems,
                                                columnSizesItems,
    ...
    count = totalRecords; // Starts writing the records from the first temporary
                          // table into the second temporary table.
    ...
    int aggFunctionsColsCount = 0,
        groupCount = 0; // Variable to count how many records are currently
                        // listed in the group.
    boolean aggFunctionExist =
      select.hasAggFunctions; // Initializes the aggregated functions running
                              // totals.
    ...
    if (aggFunctionExist && !useIndex) {
      ...
      // indexes of aggregate functions parameter list from the real table.
      if (tempTable.name != null) {
        ...
        while (--i >= 0) {
          ...
          aggFunctionsRealParamCols[i] =
            (param == null ? -1 : param.tableColIndex);
        }
      }
    }

    SQLValue[] record1 = SQLValue.newSQLValues(
      (count = tempTable.columnCount) > selectFieldsCount ? count
                                                          : selectFieldsCount);
    ...
    if (useIndex) {
      ...
      if (tableOrig.db.rowCount - tableOrig.deletedRowsCount == 0 ||
          (where != null && Utils.countBits(rsTemp.rowsBitmap.items) == 0)) {
        return tempTable2;
      }
    ...
    // Loops through the records of the temporary table, to calculate agregated
    // values and/or write the group records.
    boolean isTableTemporary = tempTable.name == null;
    paramCols =
      (isTableTemporary ? aggFunctionsParamCols : aggFunctionsRealParamCols);

    int[] groupCountCols =
      new int[aggFunctionsColsCount]; // Each column has a groupCount because
                                      // of the null values.
    byte[] allRowsBitMap = tempTable.allRowsBitmap;
    int j, colIndex, sqlAggFunction, colType,
      row = -1, numberRows = tempTable.db.rowCount,
      answerCount = tempTable.answerCount;
    SQLValue aggValue, value;

    for (i = -1; ++i < totalRecords; groupCount++) {
      if (answerCount >= 0) {
        while (row++ < numberRows &&
               (allRowsBitMap[row >> 3] & (1 << (row & 7))) == 0)
          ;
        tempTable.readRecord(curRecord, row, 1, driver, null, true, null);
      } else {
        tempTable.readRecord(curRecord,
                             i,
                             1,
                             driver,
                             null,
      ...
      if (!isTableTemporary &&
          !tempTable.db
             .recordNotDeleted()) { // Because it is possible to be pointing
                                    // to a real table, skips deleted records.
        groupCount--;
        continue;
      }
      ...
      if (groupBy != null && groupCount != 0 &&
          Utils.compareRecords(prevRecord,
                               curRecord,
                               nullsPrevRecord,
                               nullsCurRecord,
                               groupBy.fieldList) != 0) {
        ...
        // Takes the null values for the non-aggregate fields into
        // consideration.
        tempTable2.columnNulls[0] = nullsPrevRecord;
        j = aggFunctionsColsCount;
        while (--j >= 0) {
          Utils.setBit(tempTable2.columnNulls[0],
                       aggFunctionsParamCols[j],
                       groupCountCols[j] == 0);
          groupCountCols[j] = 0;
        }
      ...
      if (aggFunctionExist) { // Checks if there are aggregate functions.
        ...
        j = aggFunctionsColsCount;
        while (--j >= 0) {
          // Increments the count(*). NOTE: in the future, implementation of
          // count(field_name) needs verify the null values.
          if (aggFunctionsCodes[j] == 0) {
            groupCountCols[j]++;
            continue;
          }

          if ((colIndex = paramCols[j]) < 0 ||
              (nullsCurRecord[colIndex >> 3] & (1 << (colIndex & 7))) !=
                0) { // The agg functions skip nulls.
            continue;
          }
            ...
            case SQLElement.FUNCTION_AGG_SUM: {
              switch (colType) { // Checks the type of the column.
                ...
                case SQLElement.DATETIME:
                  throw new SQLParseException(LitebaseMessage.getMessage(
                    LitebaseMessage.ERR_SUM_AVG_WITH_DATE_DATETIME));
              }

              break;
            }

            // juliana@226_5: the aggregation functions MAX() and MIN() now work
            // for CHAR, VARCHAR, CHAR NOCASE, and VARCHAR NOCASE column types.
            case SQLElement.FUNCTION_AGG_MAX: {
              switch (colType) { // Checks the type of the column
                case SQLElement
                  .CHARS: // juliana@226_9: strings are not loaded anymore in
                          // the temporary table when building result sets.
                  if (groupCountCols[j] == 1 ||
                      aggValue.asString.compareTo(value.asString) < 0) {
                ...
                // juliana@226_9: strings are not loaded anymore in the
                // temporary table when building result sets.
                case SQLElement.CHARS_NOCASE:
                  String aggValueString = aggValue.asString.toLowerCase(),
                         valueString = value.asString.toLowerCase();
                  if (groupCountCols[j] == 1 ||
                      aggValueString.compareTo(valueString) < 0) {
                    aggValue.asString = value.asString;
                    aggValue.asInt = value.asInt;
                    aggValue.asLong = value.asLong;
                  }
                ...
                case SQLElement.DATETIME: // rnovais@567_2
                  if (groupCountCols[j] == 1) {
                    aggValue.asInt = value.asInt;
                    aggValue.asShort = value.asShort;
                  } else if (aggValue.asInt <= value.asInt &&
                             (aggValue.asInt < value.asInt ||
                              aggValue.asShort < value.asShort)) {
                    aggValue.asInt = value.asInt;
                    aggValue.asShort = value.asShort;
                ...
                case SQLElement.DATETIME: // rnovais@567_2
                  if (groupCountCols[j] == 1) {
                    ...
                    aggValue.asShort = value.asShort;
                             ...
                             (aggValue.asInt > value.asInt ||
                              aggValue.asShort >
                                value.asShort)) { // the date is greater, or the
                                                  // time is greater
                    aggValue.asInt = value.asInt;
                    aggValue.asShort = value.asShort;
                  }
      ...
      Vm.arrayCopy(
        nullsCurRecord, 0, nullsPrevRecord, 0, nullsCurRecord.length);
      curRecord = ((prevRecord = curRecord) == record1) ? record2 : record1;
    }

    if (writeDelayed &&
        groupCount >
          0) { // If there was delayed writing, flushs the last record.
      if (aggFunctionExist)
        // Concludes any aggregated function calculation.
        endAggFunctionsCalc(prevRecord,
                            groupCount,
      ...
      tempTable2.columnNulls[0] =
        nullsCurRecord; // Takes the null values for the non-aggregate fields
                        // into consideration.
      j = aggFunctionsColsCount;
    ...
    if (having == null) { // If there is no having clause, returns the second
                          // temorary table.
      return tempTable2;
    }

    // Creates the last temporary table to hold the records that comply to the
    // "HAVING" clause. Creates the second temporary table.
    Table tempTable3 = driver.driverCreateTable(null,
                                                null,
                                                tempTable2.columnHashes,
                                                tempTable2.columnTypes,
                                                tempTable2.columnSizes,
                                                null,
                                                null,
                                                Utils.NO_PRIMARY_KEY,
                                                Utils.NO_PRIMARY_KEY,
    ...
    having.bindColumnsSQLBooleanClause(tempTable3.htName2index,
                                       tempTable3.columnTypes,
                                       (SQLResultSetTable[])null);

    rsTemp = tempTable2.createResultSetForSelect(
      having); // Creates a result set from the table, using the HAVING clause
    rsTemp.driver = driver;

    // Writes the result set to the third temporary table.
    tempTable3.writeResultSetToTable(
      new ResultSet[] { rsTemp }, null, select, null, -1);
    return tempTable3;
  }
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  private ResultSet[] createListResultSetForSelect(
    SQLResultSetTable[] tableList,
    SQLBooleanClause whereClause) throws IOException, InvalidDateException
  {
    ...
    while (--i >= 0) {
      ...
      if (!hasComposedIndex) { // It is only necessary to have one table with
                               // composed indices.
        hasComposedIndex = t.numberComposedIndices > 0;
      }
    }

    if (whereClause != null) { // Tries to apply the table indices to generate a
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  static void generateIndexedRowsMap(ResultSet[] rsList,
                                     boolean hasComposedIndex)
    throws IOException, InvalidDateException
  {
    SQLBooleanClause whereClause = rsList[0].whereClause;

    if (rsList.length > 1) { // Applies the table indexes to the where clause.
                             // If it was not possible, return.
      if (!whereClause.sqlbooleanclauseApplyTableIndexesJoin()) {
        return;
      }
    } else if (!whereClause.sqlbooleanclauseApplyTableIndices(
                 rsList[0].table.columnIndices, hasComposedIndex)) {
      return;
    }
    ...
    if (whereClause.expressionTree ==
        null) { // There is no where clause left, since all rows can be returned
                // using the indexes.
      int i = rsList.length;
      while (--i >= 0) {
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  static void computeIndex(ResultSet[] rsList,
                           boolean isJoin,
                           int indexRsOnTheFly,
                           SQLValue value,
                           int operator,
    ...
    throws IOException, InvalidDateException
  {
    ...
    int count = 1, booleanOp, i = -1, j, size = rsList.length;
    SQLBooleanClause whereClause = rsList[0].whereClause;
    ...
    while (++i < count) {
      ...
      if (onTheFly) {
        ...
        if (leftVal.length < (size = appliedComposedIndexes[i]
                                       .columns.length)) { // Reuses the buffer.
      ...
      index =
        isComposed ? appliedComposedIndexes[i].index : table.columnIndices[col];

      markBits.reset(index,
                     table.db.rowCount); // Prepares the index row bitmap.
      ...
      while (--j >= 0) {
        ...
        // If operation is 'like x%', then replaces the operand by the value
        // without the % mask. juliana@230_10: solved a bug that could crash the
        // application when more than one index is applied.
        if (op == SQLElement.OP_PAT_MATCH_NOT_LIKE ||
            op == SQLElement.OP_PAT_MATCH_LIKE)
          leftVal[j].asString = (index.types[j] == SQLElement.CHARS_NOCASE)
                                  ? indexedValues[i].strToMatch.toLowerCase()
                                  : indexedValues[i].strToMatch;

        else // Checks if this is a "between" operation.
          if (booleanOp == SQLElement.OP_BOOLEAN_AND && i < count - 1 &&
              indexedCols[i + 1] == col &&
              (op == SQLElement.OP_REL_GREATER ||
               op == SQLElement.OP_REL_GREATER_EQUAL) &&
              (relationalOps[idx + 1] == SQLElement.OP_REL_LESS ||
               relationalOps[idx + 1] == SQLElement.OP_REL_LESS_EQUAL)) {
          markBits.rightKey = new Key(index);

          // Encapsulates.
          markBits.rightKey.set(
            new SQLValue[] { indexedValues[idx + 1].getOperandValue() });
          ...
          switch (op) { // When searching for !=, <, <=, opposite operation will
                        // be used instead.
          ...
          if (op != realOp) { // All rows will be marked and only the rows that
                              // satisfy the opposite operation will be reseted.
      ...
      if (rsBag.indexCount == 1) {
        ...
        mergeBitmaps(
          rsBag.rowsBitmap.items, markBits.indexBitmap.items, booleanOp);
      }
   ...
   * @param aggFunctionsRealParamCols The real columns that are parameters to
   *     the aggregated functions.
   * @param aggFunctionsColsCount The number of columns that are parameters to
   *     the aggregated functions.
   * @param columnTypes The types of the columns.
   * @param groupCountCols The count for the groups.
   */
  private void endAggFunctionsCalc(SQLValue[] record,
                                   int groupCount,
                                   SQLValue[] aggFunctionsRunTotals,
                                   int[] aggFunctionsCodes,
                                   int[] aggFunctionsParamCols,
                                   int[] aggFunctionsRealParamCols,
                                   int aggFunctionsColsCount,
                                   byte[] columnTypes,
                                   int[] groupCountCols)
  {
    ...
    while (--j >= 0) {
      ...
      switch (aggFunctionsCodes[j]) {
        ...
        case SQLElement.FUNCTION_AGG_MIN: {
          switch (
            columnTypes[aggFunctionsRealParamCols[j]]) { // Checks the type
                                                         // of the column.
            case SQLElement.SHORT:
              value.asShort = aggValue.asShort;
              break;

            // juliana@230_31: corrected a bug of MAX() and MIN() not working
            // properly with DATE and DATETIME.
            case SQLElement.DATETIME:
              value.asShort = aggValue.asShort;
            case SQLElement.INT:
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  Table createIntValueTable(LitebaseConnection driver,
                            int intValue,
                            String colName)
    throws IOException, InvalidDateException
  {
    ...
    Table table = driver.driverCreateTable(null,
                                           null,
                                           new int[] { colName.hashCode() },
                                           new byte[] { SQLElement.INT },
                                           new int[1],
                                           null,
                                           null,
                                           Utils.NO_PRIMARY_KEY,
   ...
   * @throws InvalidNumberException If an internal method throws it.
   */
  private void computeAnswer(ResultSet resultSet)
    throws IOException, InvalidDateException, InvalidNumberException
  {
    int i;
    Table table = resultSet.table;

    if (resultSet.whereClause == null && resultSet.rowsBitmap == null &&
   ...
   */
  private byte[] duplicateByteArray(byte[] array, int size)
  { // guich@554_34
   ...
   */
  private short[] duplicateShortArray(short[] array, int size)
  { // guich@554_34
   ...
   */
  private int[] duplicateIntArray(int[] array, int size)
  { // guich@554_34
