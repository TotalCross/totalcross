...
/**
 * This class represents the key of a record. It may be any of the SQL types
 * defined here.
 */
class Key
{
  ...
  void set(SQLValue[] key)
  {
    ...
    while (--i >= 0) {
      ...
      } // juliana@251_12: removed a possible NPE when using indices with null.
    }
   ...
   * Loads a key.
   *
   * @param ds The data stream where the record to be read to find the key value
   *     stored.
  ...
  { // juliana@253_8
    ...
    while (++i < n) {
      ...
      // String keys are not stored in the indices. Only their pointer is
      // stored.
      if (colSizes[i] > 0) {
      ...
      // we can use any PlainDB available. juliana@220_3 // juliana@230_14
      {
        ds.skipBytes(colSizes[i] -
                     db.readValue(key, 0, types[i], ds, true, false, false));
      }
    }
  ...
  { // juliana@253_8
    ...
    while (++i < n) {
      if (colSizes[i] > 0) {
        ds.writeInt(
          keysAux[i].asInt); // Saves only the string position in the .dbo.
      } else
      ...
      // any PlainDB available. juliana@220_3
      {
        index.table.db.writeValue(
          types[i], keysAux[i], ds, true, true, 0, 0, false);
      }
    }
