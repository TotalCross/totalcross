...
class LitebaseLex
{
  ...
  /**
   * Indicates that a character is a possible end of a number. Its values can
   * be: <LU> <LI>'d' or 'D' - double</LI> <LI>'f' or 'F' - float</LI> <LI>'l'
   * or 'L' - long</LI>
   * </LU>
   */
  ...
  /**
   * Indicates that a character is the part of a relational operator. It can
   * be: '!', '>', or '<'.
   */
  private static final int IS_RELATIONAL = 16;

  /**
   * Indicates that a character is a punctuation symbol. It can be: '.', '?',
   * ',', or '='.
   */
  private static final int IS_PUNCT = 32;

  /**
   * Indicates that a character is an operator symbol. It can be: '+', '-', '*',
   * '\', '^', '(', or ')'.
   */
  private static final int IS_OPERATOR = 64;

  /**
   * Indicates that a character is from 'a' to 'z', from 'A' to 'Z', from '0' to
   * '9', or is an underscore '_'.
   */
  private static final int IS_ALPHA_DIGIT = IS_ALPHA | IS_DIGIT;

  /**
   * Indicates that a character is the beginning of a number. It can be a digit
   * or a sign.
   */
   ...
   */
  LitebaseParser
    yyparser; // juliana@224_2: improved memory usage on BlackBerry.
   ...
   */
  private static ReservedHashtable
    reserved; // juliana@213_7: changed to Hashtable.
  ...
  static
  {
    ...
    Convert.fill(is,
                 '0',
                 '9' + 1,
                 IS_DIGIT); // Gives the values for the digits.

    is['_'] = IS_ALPHA_DIGIT;    // '_' can be part of an identifier.
    is['+'] = is['-'] = IS_SIGN; // + and - are only sign of numbers.
    is['*'] = is['('] = is[')'] =
      IS_OPERATOR; // The other operators and brackets.
    is['<'] = is['>'] = is['!'] =
      IS_RELATIONAL; // The symbols that can represent double tokens.
    is['.'] = is[','] = is['?'] = is['='] =
      IS_PUNCT; // The symbols that are treated as punctuations and '='.
  ...
  int yylex()
  {
    int zzlen = zzReaderChars.length(), yybefore, initialPos;
    int value;
    ...
    while (true) {
      ...
      if (yycurrent <=
          YYEOF) { // Ends the lexical analysis if the end of the file is found.
        return YYEOF;
      }

      // Finds identifiers or reserved words.
      if ((is[yycurrent] & IS_ALPHA) !=
          0) { // The first character must be a letter.
        int hashCode = 0;
        ...
        while (yycurrent >= 0 &&
               (is[yycurrent] & IS_ALPHA_DIGIT) !=
                 0) { // The other characters must be a letter, digit, or '_'.
          ...
          yycurrent =
            (yyposition < zzlen) ? zzReaderChars.charAt(yyposition++) : YYEOF;
        }
      ...
      // Finds digits.
      if ((is[yycurrent] & IS_START_DIGIT) !=
          0) { // The start of a digit can be a digit or a sign.
        if (yycurrent == '+') { // juliana@226a_20: now passing +number (ex.
                                // +10) will work on sql clauses.
          yycurrent =
            (yyposition < zzlen) ? zzReaderChars.charAt(yyposition++) : YYEOF;
        }
        ...
        while (yycurrent >= 0 &&
               (is[yycurrent] & IS_DIGIT) !=
                 0) { // The second part of the number must be digits.
          yycurrent =
            (yyposition < zzlen) ? zzReaderChars.charAt(yyposition++) : YYEOF;
        }
        ...
        if (yycurrent == '.') { // Tests if the number is not an integer.
          ...
          while (
            yycurrent >= 0 &&
            (is[yycurrent] & IS_DIGIT) !=
              0) { // Gets the rest of the digits of the non-integer number.
            yycurrent =
              (yyposition < zzlen) ? zzReaderChars.charAt(yyposition++) : YYEOF;
          }
        }
        ...
        yyparser.yylval = zzReaderChars.substring(
          initialPos, yyposition - (yycurrent >= 0 ? 1 : 0));
        return LitebaseParser.TK_NUMBER;
      }

      if ((is[yycurrent] & IS_RELATIONAL) !=
          0) { // Finds tokens with two characters, or '>' or '<'.
        yybefore = yycurrent;
        yycurrent =
          (yyposition < zzlen) ? zzReaderChars.charAt(yyposition++) : YYEOF;
      ...
      if (yycurrent ==
        ...
        while (yycurrent != '\'') {
          ...
          if (yycurrent == '\\') { // Sees if there is an escape in the string.
            ...
            yycurrent =
              (yyposition < zzlen) ? zzReaderChars.charAt(yyposition++) : YYEOF;
            needsNewString = true;
          } else if (yycurrent == YYEOF) { // The string must be closed before
                                           // the end of the file.
            yyparser.yyerror(LitebaseMessage.ERR_SYNTAX_ERROR);
        ...
        if (nameToken.length() == 0) {
          ...
          yyparser.yylval = zzReaderChars.substring(
            initialPos, yyposition - (yycurrent >= 0 ? 2 : 1));
        }
