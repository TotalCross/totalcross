	...
	int yylex() {
		...
		while (true) {
			...
			if ((is[yycurrent] & IS_ALPHA) != 0) { // The first character must be a letter.
				...
				while (yycurrent >= 0
					   && (is[yycurrent] & IS_ALPHA_DIGIT) != 0) { // The other characters must be a letter, digit, or '_'.
			...
			if ((is[yycurrent] & IS_START_DIGIT) != 0) { // The start of a digit can be a digit or a sign.
				...
				while (yycurrent >= 0
					   && (is[yycurrent] & IS_DIGIT) != 0) { // The second part of the number must be digits.
					yycurrent = (yyposition < zzlen) ? zzReaderChars.charAt(yyposition++) : YYEOF;
				}
				...
				if (yycurrent == '.') { // Tests if the number is not an integer.
					...
					while (yycurrent >= 0
						   && (is[yycurrent] & IS_DIGIT) != 0) { // Gets the rest of the digits of the non-integer number.
						yycurrent = (yyposition < zzlen) ? zzReaderChars.charAt(yyposition++) : YYEOF;
					}
				}
