...
public class LitebaseConnection {
	...
	public static LitebaseConnection getInstance(String appCrid, String params) throws DriverException {
		try {
			...
			int key = (appCrid + '|' + params + '|' +
					   Thread.currentThread()).hashCode(); // The key which identifies the Litebase connection.
			...
			if (conn == null) { // If there is no connections with this key, creates a new one.
				...
				if (logger != null)
					synchronized (logger) {
						...
						logger.logInfo(conn.sBuffer.append("new LitebaseConnection(").append(appCrid).append(",").append(
										   params).append(")"));
					}
				...
				if (params != null) {
					...
					while (--len >= 0) { // The parameters order does not matter.
						...
						if (tempParam.startsWith("chars_type")) { // Chars type param.
							...
							throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_INVALID_PARAMETER) +
													  "tempParam");
					}
				}

				// juliana@214_1: relative paths can't be used with Litebase.
				validatePath(path = conn.sourcePath = (path != null) ? path : Settings.dataPath != null
													  && Settings.dataPath.length() != 0 ? Settings.dataPath : Settings.appPath);
				...
				if (!path.endsWith("\\")
					&& !path.endsWith("/")) { // Appends a "/" if the datapath does not end with "\\" or "/".
				...
				synchronized (
					htDrivers) { // juliana@230_13: removed some possible strange behaviours when using threads.
	 ...
	 */
	public void execute(String sql) throws IllegalStateException, DriverException, SQLParseException,
		...
		try {
			...
			if (parser.command == SQLElement.CMD_CREATE_TABLE) { // CREATE TABLE
				...
				if (tableName.length() >
					SQLElement.MAX_TABLE_NAME_LENGTH_AS_PLAIN_FILE) { // rnovais@570_114: The table name can't be infinite.
				...
				if (exists(tableName)) // guich@105: verifies if it is already created.
					throw new AlreadyCreatedException(LitebaseMessage.getMessage(
														  LitebaseMessage.ERR_TABLE_ALREADY_CREATED) + tableName);
				...
				while (--i > 0) { // Creates the other columns.
					...
					if (field.isPrimaryKey) // Checks if there is a primary key definition.
						primaryKeyCol =
							i; // Only one primary key can be defined per table: this is verified during the parsing.

					if ((defaultValue = field.defaultValue) !=
						null) { // Default values: default null has no effect. This is handled by the parser.
						...
						switch (field.fieldType) {
							...
							case SQLElement.CHARS_NOCASE:
								if (defaultValue.length() >
									sizes[i]) // The default value size can't be larger than the size of the field definition.
									throw new SQLParseException(LitebaseMessage.getMessage(
				...
				if (composedPKSize > 0) {
					...
					while (++i < composedPKSize) {
						...
						if (pos == -1) // Column not found.
							throw new SQLParseException(LitebaseMessage.getMessage(LitebaseMessage.ERR_UNKNOWN_COLUMN) +
														composedPK_Fields[i]);
				...
				driverCreateTable(tableName, names, hashes, types, sizes, columnAttrs, defaultValues, primaryKeyCol,
								  composedPK, composedPKCols);
	 ...
	 * @throws DriverException If an <code>IOException</code> occurs.
	 */
	public int executeUpdate(String sql) throws IllegalStateException, SQLParseException,
		DriverException {
		if (htTables == null) { // The driver can't be closed.
			throw new IllegalStateException(LitebaseMessage.getMessage(LitebaseMessage.ERR_DRIVER_CLOSED));
		}
	 ...
	 */
	private void litebaseExecuteAlter(LitebaseParser parser) throws DriverException,
		AlreadyCreatedException,
		SQLParseException, IOException,
		...
		switch (parser.command) {
			case SQLElement.CMD_ALTER_DROP_PK: // DROP PRIMARY KEY
				if (table.primaryKeyCol != Utils.NO_PRIMARY_KEY) { // Simple primary key.
					...
					throw new DriverException(LitebaseMessage.getMessage(
												  LitebaseMessage.ERR_TABLE_DOESNOT_HAVE_PRIMARY_KEY)); // There's no primary key.
				break;
			...
			case SQLElement.CMD_ALTER_ADD_PK: // ADD PRIMARY KEY
				...
				if (size == 1) { // Simple primary key.
					...
					if (table.columnIndices[colIndex] != null) // If there is no index yet for the column, creates it.
						throw new AlreadyCreatedException(LitebaseMessage.getMessage(
															  LitebaseMessage.ERR_INDEX_ALREADY_CREATED) + colIndex);
					try {
			...
			case SQLElement.CMD_ALTER_RENAME_TABLE: // RENAME TABLE
				...
				if (exists(newTableName)) // The new table name can't be in use.
					throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_TABLE_ALREADY_EXIST) +
											  newTableName);
				try {
			...
			case SQLElement.CMD_ALTER_ADD_COLUMN: // ADD COLUMN
				...
				if (oldCount == SQLElement.MAX_NUM_COLUMNS) // The maximum number of columns can't be exceeded.
					throw new SQLParseException(LitebaseMessage.getMessage(LitebaseMessage.ERR_COLUMNS_OVERFLOW) +
												field.fieldName);
				...
				(table.columnAttrs = newAttrs)[oldCount] = (byte)((field.defaultValue != null ?
						Utils.ATTR_COLUMN_HAS_DEFAULT : 0)
				...
				(table.columnOffsets = newOffsets)[newCount] = (short)(newOffsets[oldCount] +
						Utils.typeSizes[field.fieldType]);
				...
				if (defaultValue != null) { // Sets the new default value if it exists.
					...
					switch (field.fieldType) {
						...
						case SQLElement.CHARS_NOCASE:
							if (defaultValue.length() >
								field.fieldSize) // The default value size can't be larger than the size of the field definition.
								throw new SQLParseException(LitebaseMessage.getMessage(
																LitebaseMessage.ERR_LENGTH_DEFAULT_VALUE_IS_BIGGER));
				...
				boolean useCrypto = newDB.useCrypto =
										oldDB.useCrypto; // juliana@crypto_1: now Litebase supports weak cryptography.
				...
				while (++i < size) {
					...
					// juliana@220_3
					while (++j < newCount)
						newDB.writeValue(newTypes[j], record[j], newBasds, (columnNulls0[j >> 3] & (1 << (j & 7))) == 0,
										 true, newSizes[j], 0, false);
					...
					newBuffer[3] = (useCrypto ? (byte)0xAA :
				...
				while (--i >= 0) // Recreates the simple indices.
					if (newIndices[i] != null && (newTypes[i] == SQLElement.CHARS
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	private void litebaseExecuteDropTable(LitebaseParser parser) throws DriverException, IOException,
		...
		if (table != null) { // The table is open.
			...
			while (--numFiles >= 0) // Erases the table files.
				if (listFiles[numFiles].startsWith(name) || listFiles[numFiles].startsWith(nameSimpIdx)
			...
			if (file == null) // If there is no file to be erased, an exception must be raised.
				throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_TABLE_NAME_NOT_FOUND) +
										  tableName);
		}
	}
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	private int litebaseExecuteDropIndex(LitebaseParser parser) throws DriverException, IOException,
		InvalidDateException {
		String tableName = parser.tableList[0].tableName;
	 ...
	 */
	public ResultSet executeQuery(String sql) throws IllegalStateException, DriverException,
		SQLParseException {
		if (htTables == null) { // The driver can't be closed.
			throw new IllegalStateException(LitebaseMessage.getMessage(LitebaseMessage.ERR_DRIVER_CLOSED));
		}
	 ...
	 * @throws SQLParseException If an <code>InvalidDateException</code> or an <code>InvalidNumberException</code> occurs.
	 */
	public PreparedStatement prepareStatement(String sql) throws IllegalStateException,
		SQLParseException, DriverException {
		if (htTables == null) { // The driver can't be closed.
			throw new IllegalStateException(LitebaseMessage.getMessage(LitebaseMessage.ERR_DRIVER_CLOSED));
		}
	 ...
	 * @throws DriverException If an <code>IOException</code> occurs.
	 */
	public void setRowInc(String tableName, int inc) throws DriverException, IllegalStateException,
		...
		try {
			...
			while (--i >=
				   0) { // juliana@202_18: The composed indices must also be written delayed when setting row increment to a value different to -1.
	...
	public boolean exists(String tableName) throws IllegalStateException, DriverException {
		...
		try { // Tests if the .db file exists.
			sBuffer.setLength(0);
			boolean ret = new File(sBuffer.append(sourcePath).append(appCrid).append('-').append(name).append(
									   NormalFile.DB_EXT).toString()).exists();
	...
	private void litebaseClose() throws IOException {
		...
		htTables =
			null; // guich@564_9: makes sure that this instance will fail if someone tries to use it again.
		synchronized (
			htDrivers) { // juliana@201_11: only sets the LitebaseConnection class as finalized if all its objects are finalized.
	...
	public int purge(String tableName) throws IllegalStateException, DriverException {
		...
		try {
			...
			if (deleted > 0
				|| table.wasUpdated) { // juliana@270_27: now purge will also really purge the table if it only suffers updates.
				...
				if (willRemain == 0) { // If no rows will remain, just deletes everyone.
					...
					((NormalFile)plainDB.dbo).finalPos =
						0; // juliana@202_13: .dbo final position must be zeroed when purging all the table.
					...
					boolean useCrypto = newdb.useCrypto =
											plainDB.useCrypto; // juliana@253_8: now Litebase supports weak cryptography.
					...
					while (++i < rows) {
						table.readRecord(record, i, 0, null, null, false, null); // juliana@220_3 juliana@227_20
						if (((record[0].asInt = oldBasds.readInt()) & Utils.ROW_ATTR_MASK) !=
							Utils.ROW_ATTR_DELETED) { // Is record not deleted?
							j = -1;

							// juliana@230_44: solved a NullPointerException when purging a table with a null value on a CHAR or VARCHAR column.
							while (++j < columns)
								newdb.writeValue(columnTypes[j], record[j], newBasds,
												 ((columnNulls0[j >> 3] & (1 << (j & 7))) == 0), true, columnSizes[j], 0, false); // juliana@220_3
							newBasds.writeBytes(columnNulls0, 0, length);
							...
							// juliana@220_4: added a crc32 code for every record. Please update your tables.
							k = oldBuffer[3];
							oldBuffer[3] = (useCrypto ? (byte)0xAA :
											0); // juliana@222_5: The crc was not being calculated correctly for updates.
	...
	public static synchronized Logger getDefaultLogger() throws DriverException {
		Logger logger = Logger.getLogger("litebase", -1,
		...
		try {
			if (logger.getOutputHandlers().length ==
				...
				logger.addOutputHandler(new File(Convert.appendPath(Settings.dataPath != null
												 && Settings.dataPath.length() > 0 ? Settings.dataPath
												 : Settings.appPath, "LITEBASE_" + LitebaseConnection.tempTime.getTimeLong() + '.' +
												 Settings.applicationId + ".LOGS"), File.CREATE_EMPTY));
			}
		} catch (IOException exception) {
			throw new DriverException(exception);
		}
	...
	public static synchronized int deleteLogFiles() throws DriverException {
		String path = Settings.dataPath != null
					  && Settings.dataPath.length() > 0 ? Settings.dataPath : Settings.appPath;
		int count =
			0, // The number of log files.
			i;
			...
			if (list != null) {
				...
				while (--i >= 0) {
					...
					if (current.startsWith(Convert.appendPath(path, "LITEBASE_")) && current.endsWith(".LOGS")) {
						try {
							new File(current,
									 File.READ_WRITE).delete(); // Deletes all closed log files.
							count++;
						} catch (IOException exception) {}
					}
				}
	...
	public static LitebaseConnection processLogs(String[] sql, String params,
			boolean isDebug) throws DriverException {
	...
	public boolean recoverTable(String tableName) throws IllegalStateException, DriverException {
		...
		try {
			...
			File tableDb = new File(sBuffer.append(sourcePath).append(appCrid).append('-').append(
										tableName.toLowerCase()).append(".db").toString(),
									File.READ_WRITE);
			...
			// juliana@253_8: now Litebase supports weak cryptography.
			table.tableCreate(sourcePath, appCrid + '-' + tableName.toLowerCase(), false, appCrid, this,
							  isAscii, useCrypto, false);
			...
			while (--i >= 0) { // Checks all table records.
				...
				if (isZero(
						buffer)) { // juliana@268_3: Now does not do anything if there are only zeros in a row and removes them.
					rows--;
					continue;
				}
			...
			table.tableSaveMetaData(
				Utils.TSMD_EVERYTHING); // Saves information concerning deleted rows and the auxiliary rowid.
	...
	public void convert(String tableName) throws IllegalStateException, DriverException {
		...
		try {
			...
			if ((version = (((bytes[1] & 0xFF) << 8) | (bytes[0] & 0xFF))) != Table.VERSION - 1
				|| version != Table.VERSION - 2) {
				tableDb.close(); // juliana@222_4: The table files must be closed if convert() fails().
				throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_WRONG_PREV_VERSION) +
										  tableName);
			}
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	private void driverCreateIndex(String tableName, String[] columnNames, byte[] numberColumns,
								   boolean isPK)
	throws IOException, DriverException, PrimaryKeyViolationException, InvalidDateException,
		SQLParseException {
		Table table = getTable(tableName);
		PlainDB plainDB = table.db;
		...
		while (--i >= 0) {
			// Column not found.
			if ((idx = columns[i] = (numberColumns == null) ? (byte)table.htName2index.get(
										columnNames[i].hashCode(), -1) : numberColumns[i]) == -1)
		...
		// juliana@253_5: removed .idr files from all indices and changed its format.
		if (indexCount == 1) {
			table.indexCreateIndex(table.name, columns[0], columnSizes, columnTypes, appCrid, sourcePath,
								   false);
			saveType = Utils.TSMD_EVERYTHING;
		} else {
			table.indexCreateComposedIndex(table.name, columns, columnSizes, columnTypes, newIndexNumber, isPK,
										   appCrid, true, sourcePath, false);
			saveType = Utils.TSMD_EVERYTHING;
		}
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	Table driverCreateTable(String tableName, String[] names, int[] hashes, byte[] types, int[] sizes,
							byte[] columnAttrs, SQLValue[] defaultValues,
		...
		// juliana@253_8: now Litebase supports weak cryptography.
		table.tableCreate(sourcePath, tableName == null ? null : appCrid + "-" + tableName, true, appCrid,
						  this, isAscii, useCrypto, true);
		...
		if (tableName == null) { // juliana@201_14
			...
			table.tableSetMetaData(names, hashes, types, sizes, columnAttrs, defaultValues, primaryKeyCol,
								   composedPK, composedPKCols,
	...
	Table getTable(String tableName) throws IOException, InvalidDateException {
		...
		if (table == null) {
			...
			// juliana@224_2: improved memory usage on BlackBerry.
			// juliana@253_8: now Litebase supports weak cryptography.
			table.tableCreate(sourcePath, appCrid + '-' + tableName, false, appCrid, this, isAscii, useCrypto,
							  true); // juliana@220_5
	...
	private static void validatePath(String path) throws DriverException {
		...
		if (auxP != null && (auxP.length() == 2 || auxP.charAt(2) == '/' || (auxP.charAt(2) == '.'
	...
	*/
	 ...
	 */
	private static void encDecTables(String crid, String sourcePath,
