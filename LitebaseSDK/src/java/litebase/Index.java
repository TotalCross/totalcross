...
class Index
{
  ...
  /**
   * The size of the disk sector. Used for calculating the number of keys of a
   * node.
   */
   ...
   */
  Node[] firstLevel; // juliana@230_35: now the first level nodes of a b-tree
                     // index will be loaded in memory.
        ...
        boolean exist) throws IOException, InvalidDateException
  {
    ...
    nodeRecSize =
      2 + btreeMaxNodes * keyRecSize +
      ((btreeMaxNodes + 1) << 1); // int size + key[k] + (Node = int)[k+1]
    ...
    // juliana@213_8: the index node count was not being loaded when loading the
    // indices, which could cause an infinite loop when using them.
    ...
    throws IOException, InvalidDateException, DriverException
  {
    if (!isEmpty) {
      ...
      while (true) {
        keyFound =
          (currKeys =
             curr.keys)[pos = curr.findIn(key,
                                          false)]; // juliana@201_3 // Finds
                                                   // the key position.
        firstChild = (children = curr.children)[0];

        if (pos < (size = curr.size) &&
            Utils.arrayValueCompareTo(keys, keyFound.keys, typesAux, plainDB) ==
              0) {
          while (
            pos >= 0 &&
            Utils.arrayValueCompareTo(
              keys, (keyFound = currKeys[pos]).keys, typesAux, plainDB) == 0 &&
        ...
        if (--nodeCounter < 0) { // juliana@220_16: does not let the index
          ...
          throw new DriverException(
            LitebaseMessage.getMessage(LitebaseMessage.ERR_CANT_LOAD_NODE));
        }
    ...
    throw new DriverException(
      LitebaseMessage.getMessage(LitebaseMessage.ERR_IDX_RECORD_DEL));
  }
    ...
    throws IOException, InvalidDateException, DriverException
  {
    ...
    // juliana@230_35: now the first level nodes of a b-tree index will be
    // loaded in memory. Tries to find the node in the nodes of the first level.
   ...
   * @throws DriverException If the index is corrupted.
   */
  void getValue(Key key, MarkBits markBits)
    throws IOException, InvalidDateException, DriverException
  {
    if (!isEmpty) {
      Node curr = root; // 0 is always the root.
      ...
      while (true) {
        keyFound =
          (currKeys =
             curr.keys)[pos = curr.findIn(key, false)]; // juliana@201_3
        firstChild = (children = curr.children)[0];
        ...
        if (pos < (size = curr.size) &&
          ...
          do {
            pos--;
                   ...
                   Utils.arrayValueCompareTo(
                     keys, currKeys[pos].keys, typesAux, plainDB) == 0);
          while (++pos < size &&
                 Utils.arrayValueCompareTo(
                   keys, currKeys[pos].keys, typesAux, plainDB) == 0) {
            if (markBits != null) {
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  private boolean climbGreaterOrEqual(Node node,
                                      int start,
                                      MarkBits markBits,
                                      boolean stop)
  ...
  /**
   * Starts from the root to find the left key, then climbs from it until the
   * end.
   *
   ...
   * @throws DriverException If the index is corrupted.
   */
  void getGreaterOrEqual(MarkBits markBits)
    throws IOException, InvalidDateException, DriverException
  {
    if (!isEmpty) {
      int pos, nodeCounter = nodeCount, r, count = 0;
      PlainDB plainDB = table.db;
      ...
      while (true) {
        ...
        if ((pos = curr.findIn(left, false)) < curr.size) { // juliana@201_3
          ...
          // If this value is above or equal to the one being looked for, stores
          // it.
          while (--pos >= 0 &&
                 Utils.arrayValueCompareTo(
                   leftKeys, currKeys[pos].keys, typesAux, plainDB) == 0)
            ;
          if ((r = Utils.arrayValueCompareTo(
                 leftKeys, currKeys[++pos].keys, typesAux, plainDB)) <= 0) {
      ...
      if (count > 0) {
        ...
        while (count > 0) {
          stop = false;
          pos = ancestors[--count];
          if ((stop = climbGreaterOrEqual(
                 curr = loadNode(ancestors[--count]), pos, markBits, stop))) {
            break;
          }
        }
      }
   ...
   * ancestors of the node, together with the known insertion position in each
   * of these ancestors.
   *
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  private void splitNode(Node curr, int count)
    throws IOException, InvalidDateException
  {
    int left, right, ins;
    Key keyFound, keyAux = tempKey;
    Node rootAux = root;
    int[] ancestors =
      table.db.driver
        .nodes; // juliana@224_2: improved memory usage on BlackBerry.

    // guich@110_3: curr.size * 3/4 - note that medPos never changes, because
    // the node is always split when the same size is reached. juliana@283_1:
    // solved a bug which would buid corrupted indices when creating or
    // recreating them.
  ...
  {
    fnodes.finalPos =
      nodeCount * nodeRecSize; // Calculated the used space; the file will
                               // have no zeros at the end.
    fnodes.close();
  }
  ...
  void deleteAllRows() throws IOException
  {
    ...
    cacheI = nodeCount = 0; // juliana@220_6: The node count should be reseted
                            // when recreating the indices.
  }
  ...
  void setWriteDelayed(boolean delayed) throws IOException
  {
    ...
    // juliana@230_35: now the first level nodes of a b-tree index will be
    // loaded in memory. Commits the pending first level nodes.
    int i = btreeMaxNodes;
   ...
   * @throws InvalidDateException If an internal method throws it.
   */
  void indexAddKey(SQLValue[] values, int record)
    throws IOException, InvalidDateException
  {
    Key keyAux = tempKey;
    Node rootAux = root;
    ...
    if (isEmpty) {
      ...
      Key[] currKeys;
      PlainDB plainDB = table.db;
      int nodeCountAux = nodeCount, nodeCounter = nodeCountAux,
          maxSize = btreeMaxNodes - 1, pos, size, count = 0;
      int[] ancestors =
        plainDB.driver
          .nodes; // juliana@224_2: improved memory usage on BlackBerry.
      ...
      while (true) {
        keyFound =
          (currKeys =
             curr.keys)[pos = curr.findIn(keyAux, true)]; // juliana@201_3
        children = curr.children;
        ...
        if (pos < (size = curr.size) &&
          ...
          while (++pos < size &&
                 Utils.arrayValueCompareTo(
                   keys, (keyFound = currKeys[pos]).keys, typesAux, plainDB) ==
                   0 &&
                 (keyFound.record < record || keyFound.record == Key.NO_VALUE))
            ;
        }
        ...
        if (children[0] == Node.LEAF) {
          ...
          // keeping track of the ancestors. Note: with k = 50 and 200000
          // values, there are about 1.1 million useless pushes without this
          // redundant insert.
   ...
   * @throws IOException If an internal method throws it.
   */
  void findMinValue(SQLValue sqlValue, IntVector bitMap)
    throws IOException, InvalidDateException
  {
    Node curr;
    Key[] currKeys;
    Key currKey;
    int size, i, nodeCounter = nodeCount + 1, count = 1;
    ...
    while (count > 0) {
      ...
      // Searches for the smallest key of the node marked in the result set or
      // is not deleted.
      size = curr.size;
      i = -1;
      currKeys = curr.keys;
      ...
      while (++i < size)
        if ((currKey = currKeys[i]).record != Key.NO_VALUE &&
            (bitMap == null || bitMap.isBitSet(currKey.record))) {
          currKey.keys[0].cloneSQLValue(sqlValue);
          count = 0; // juliana@284_3: solved a possible wrong result in MAX()
                     // and MIN() if the column searched had an index.
          break;
        }
   ...
   * @throws IOException If an internal method throws it.
   */
  void findMaxValue(SQLValue sqlValue, IntVector bitMap)
    throws IOException, InvalidDateException
  {
    Node curr;
    Key[] currKeys;
    Key currKey;
    int size, i, count = 1, nodeCounter = nodeCount + 1;
    int[] vector = table.db.driver.nodes;
    int[] children;
    ...
    while (count > 0) {
      ...
      // Searches for the greatest key of the node marked in the result set or
      // is not deleted.
      i = size = curr.size;
      currKeys = curr.keys;
      children = curr.children;
      ...
      // Now searches the children nodes whose keys are greater than the one
      // marked or all of them if no one is marked.
      if (children[0] != Node.LEAF)
        while (++i <= size) {
          vector[count++] = children[i];
   ...
   *     first level node, or <code>null</code> if it is not already loaded
  ...
  private Node getLoadedNode(int idx) throws IOException, InvalidDateException
  {
    ...
    if (i < INDEX_CACHE_SIZE) { // Loads the node if there is enough space in
                                // the node cache.
   ...
   * @param columnIndexes Has the indices of the tables for each resulting
   *     column.
   ...
   * @throws IOException If an internal method throws it.
   */
  void sortRecordsAsc(IntVector bitMap,
                      Table tempTable,
                      SQLValue[] record,
                      short[] columnIndexes,
  ...
  {
    int size, i, valRec, node, nodeCounter = nodeCount + 1, count = 1;
    Node curr;
    ...
    while (count > 0) {
      ...
      if (children[0] ==
          Node.LEAF) { // If the node do not have children, just process its
                       // keys in the ascending order.
        i = -1;
        while (++i < size) {
          writeKey(
            keys[i].record, bitMap, tempTable, record, columnIndexes, clause);
        }
   ...
   * @throws IOException If an internal method throws it.
   */
  void sortRecordsDesc(IntVector bitMap,
                       Table tempTable,
                       SQLValue[] record,
                       short[] columnIndexes,
                       SQLSelectClause clause)
    throws DriverException, InvalidDateException, IOException
  {
    ...
    while (count > 0) {
      ...
      } else { // If not, process its children in the descending order and then
               // push its key.
        i = -1;
        while (++i < size) {
   ...
   * @throws IOException If an internal method throws it.
   */
  private void writeKey(int valRec,
                        IntVector bitMap,
                        Table tempTable,
                        SQLValue[] record,
                        short[] columnIndexes,
    ...
    throws IOException, InvalidDateException
  {
    if (valRec != Key.NO_VALUE && (bitMap == null || bitMap.isBitSet(valRec))) {
      ...
      int i = tempTable.columnCount, colIndex;
      boolean isNull;

      tableAux.db.read(valRec); // Reads the record.
      tableAux.readNullBytesOfRecord(0,
                                     false,
      ...
      while (--i >= 0) { // Reads the fields for the temporary table.
        ...
        if (!(isNull =
                (origNulls[colIndex >> 3] & (1 << (colIndex & 7))) != 0)) {
          tableAux.readValue(
            record[i], offsets[colIndex], types[colIndex], false, true);
        }

        Utils.setBit(tempNulls,
                     i,
