...
class Index {
	 ...
	 */
	Index(Table aTable, byte[] keyTypes, int[] newColSizes, String aName, String sourcePath,
		  boolean exist)
	throws IOException, InvalidDateException {
		...
		nodeRecSize = 2 + btreeMaxNodes * keyRecSize + ((btreeMaxNodes + 1) <<
					  1); // int size + key[k] + (Node = int)[k+1]
	...
	void removeValue(Key key, int record) throws IOException, InvalidDateException, DriverException {
		if (!isEmpty) {
			...
			while (true) {
				keyFound = (currKeys = curr.keys)[pos = curr.findIn(key,
														false)]; // juliana@201_3 // Finds the key position.
				firstChild = (children = curr.children)[0];

				if (pos < (size = curr.size)
					&& Utils.arrayValueCompareTo(keys, keyFound.keys, typesAux, plainDB) == 0) {
					while (pos >= 0
						   && Utils.arrayValueCompareTo(keys, (keyFound = currKeys[pos]).keys, typesAux, plainDB) == 0
	 ...
	 * @throws DriverException If the index is corrupted.
	 */
	void getValue(Key key, MarkBits markBits) throws IOException, InvalidDateException,
		DriverException {
		if (!isEmpty) {
			Node curr = root; // 0 is always the root.
			Key keyFound;
			...
			while (true) {
				...
				if (pos < (size = curr.size)
					...
					while (++pos < size
						   && Utils.arrayValueCompareTo(keys, currKeys[pos].keys, typesAux, plainDB) == 0) {
						if (markBits != null) {
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	private boolean climbGreaterOrEqual(Node node, int start, MarkBits markBits,
	 ...
	 * @throws DriverException If the index is corrupted.
	 */
	void getGreaterOrEqual(MarkBits markBits) throws IOException, InvalidDateException,
		DriverException {
		if (!isEmpty) {
			int pos,
				nodeCounter = nodeCount,
			...
			while (true) {
				...
				if ((pos = curr.findIn(left, false)) < curr.size) { // juliana@201_3
					...
					while (--pos >= 0
						   && Utils.arrayValueCompareTo(leftKeys, currKeys[pos].keys, typesAux, plainDB) == 0);
	...
	void close() throws IOException {
		fnodes.finalPos = nodeCount *
						  nodeRecSize; // Calculated the used space; the file will have no zeros at the end.
		fnodes.close();
	}
	...
	void deleteAllRows() throws IOException {
		...
		cacheI = nodeCount =
					 0; // juliana@220_6: The node count should be reseted when recreating the indices.
	}
	...
	void findMinValue(SQLValue sqlValue, IntVector bitMap) throws IOException, InvalidDateException {
		...
		while (count > 0) {
			...
			while (++i < size)
				if ((currKey = currKeys[i]).record != Key.NO_VALUE && (bitMap == null
						|| bitMap.isBitSet(currKey.record))) {
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	void sortRecordsAsc(IntVector bitMap, Table tempTable, SQLValue[] record, short[] columnIndexes,
	...
	throws DriverException, InvalidDateException, IOException {
		...
		while (count > 0) {
			...
			if (children[0] ==
				Node.LEAF) { // If the node do not have children, just process its keys in the ascending order.
				i = -1;
				while (++i < size) {
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	void sortRecordsDesc(IntVector bitMap, Table tempTable, SQLValue[] record, short[] columnIndexes,
						 SQLSelectClause clause)
	throws DriverException, InvalidDateException, IOException {
		int size,
			i,
		...
		while (count > 0) {
			...
			if (children[0] ==
				Node.LEAF) { // If the node do not have children, just process its keys in the descending order.
				writeKey(valRec, bitMap, tempTable, record, columnIndexes, clause);
				i = size;
				while (--i >= 0) {
					writeKey(keys[i].record, bitMap, tempTable, record, columnIndexes, clause);
	 ...
	 * @throws IOException If an internal method throws it.
	 */
	private void writeKey(int valRec, IntVector bitMap, Table tempTable, SQLValue[] record,
						  short[] columnIndexes,
						  SQLSelectClause clause)
	throws IOException, InvalidDateException {
