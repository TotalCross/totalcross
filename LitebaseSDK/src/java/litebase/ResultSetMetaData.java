...
public class ResultSetMetaData {
	...
	public int getColumnType(int column) {
		...
		if (resultSet.allRowsBitmap != null || resultSet.isSimpleSelect) {
			SQLResultSetField field = resultSet.fields[column - 1];
			return resultSet.table.columnTypes[field.parameter == null ? field.tableColIndex :
	...
	public boolean hasDefaultValue(String columnName) throws DriverException {
		...
		while (++i <
			   len) { // Gets the name of the table or its alias given the column name and gets the table column info.
			if (columnName.equalsIgnoreCase((field = fields[i]).tableColName)
				|| columnName.equalsIgnoreCase(fields[i].alias))
				try {
					if (field.tableName != null)
						return ((resultSet.driver.getTable(field.tableName).columnAttrs[field.tableColIndex >= 0 ?
								 field.tableColIndex
								 : field.parameter.tableColIndex]) & Utils.ATTR_COLUMN_HAS_DEFAULT) != 0;
	...
	public String getDefaultValue(int columnIndex) throws DriverException {
		...
		return getDefault(name, field.tableColIndex >= 0 ? field.tableColIndex :
						  field.parameter.tableColIndex);
	}
			...
			if (columnName.equalsIgnoreCase((field = fields[i]).tableColName)
				...
				if (field.tableName != null)
					return getDefault(field.tableName,
									  field.tableColIndex >= 0 ? field.tableColIndex : field.parameter.tableColIndex);
