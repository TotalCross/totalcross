...
class Node {
	 ...
	 */
	static final int MAX_IDX =
		65534; // juliana@253_6: The maximum number of keys of a index was duplicated.
	...
	Node(Index anIndex) {
		...
		children = new int[anIndex.btreeMaxNodes +
						   1]; // Each array has one extra component, to allow for possible overflow.
	}
	...
	void load() throws IOException, InvalidDateException {
		...
		Convert.fill(childrenAux, i + 1, indexAux.btreeMaxNodes + 1,
	...
	void saveDirtyKey(int currPos) throws IOException {
		...
		indexAux.fnodes.setPos(idx * indexAux.nodeRecSize + 2 + indexAux.keyRecSize * currPos +
	...
	int findIn(Key item, boolean isInsert) throws IOException {
		...
		int r = size - 1,
			l = (isInsert && indexAux.isOrdered
				 && r > 0) ? r :
				0, // juliana@201_3: If the insertion is ordered, the position being seached is the last.
				m,
				comp;

		while (l <= r) {
			if ((comp = Utils.arrayValueCompareTo(itemKeys, keysAux[m = (l + r) >> 1].keys, types,
												  plainDB)) == 0) {
				return m;
			} else if (comp < 0) {
				r = m - 1;
