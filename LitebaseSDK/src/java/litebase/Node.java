...
/**
 * This is the implementation of a B-Tree. It is used to store the table
...
class Node
{
   ...
   */
  static final int MAX_IDX = 65534; // juliana@253_6: The maximum number of keys
                                    // of a index was duplicated.
  ...
  Node(Index anIndex)
  {
    ...
    children =
      new int[anIndex.btreeMaxNodes + 1]; // Each array has one extra component,
                                          // to allow for possible overflow.
  }
  ...
  void load() throws IOException, InvalidDateException
  {
    ...
    fnodes.readBytes(indexAux.basbuf,
                     0,
                     indexAux.nodeRecSize); // Reads all the record at once.

    // Loads the keys.
    DataStreamLB ds =
      indexAux.basds; // juliana@253_8: now Litebase supports weak cryptography.
    indexAux.bas.reset();
    ...
    Convert.fill(childrenAux,
                 i + 1,
                 indexAux.btreeMaxNodes + 1,
                 LEAF); // Fills the non-used indexes with TERMINAL.
  }
  ...
  void saveDirtyKey(int currPos) throws IOException
  {
    ...
    indexAux.fnodes.setPos(idx * indexAux.nodeRecSize + 2 +
                           indexAux.keyRecSize * currPos +
                           (indexAux.keyRecSize - Key.VALREC_SIZE));
  ...
  {
    Index indexAux = index;
    int i, idxAux = idx, recSize = indexAux.nodeRecSize;
    NormalFile fnodes = indexAux.fnodes;

    if (isNew) {
      if ((idxAux = indexAux.nodeCount++) >= MAX_IDX) {
        throw new DriverException(
          LitebaseMessage.getMessage(LitebaseMessage.ERR_INDEX_LARGE));
      }

      if (indexAux.isWriteDelayed) {
        if (idxAux * recSize ==
            fnodes.size) { // Grows more than 1 record per time.
    ...
    if (isNew && idxAux > 0 && idxAux <= indexAux.btreeMaxNodes) {
      ...
      Key[] keys = node.keys;
      node.idx = idxAux;
      Vm.arrayCopy(childrenAux,
                   left,
                   node.children,
                   0,
                   (i = node.size = right - left) + 1);
      while (--i >= 0) {
  ...
  /**
   * Returns the index of the leftmost element of this node that is not less
   * than item, using a binary search.
   *
   * @param item The key to be found.
   * @param isInsert Indicates if the method is called by
   *     <code>Index.insert()</code>
  ...
  int findIn(Key item, boolean isInsert) throws IOException
  {
    ...
    int r = size - 1,
        l = (isInsert && indexAux.isOrdered && r > 0)
              ? r
              : 0, // juliana@201_3: If the insertion is ordered, the position
                   // being seached is the last.
      m, comp;

    while (l <= r) {
      if ((comp = Utils.arrayValueCompareTo(
             itemKeys, keysAux[m = (l + r) >> 1].keys, types, plainDB)) == 0) {
        return m;
      } else if (comp < 0) {
        r = m - 1;
      } else {
   ...
   * @throws IOException If an internal method throws it.
   */
  void insert(Key item, int leftChild, int rightChild, int ins)
    throws IOException
  {
    int sizeAux = size, l = sizeAux - ins;
    Key[] keysAux = keys;
    int[] childrenAux = children;
    ...
    if (index.isWriteDelayed) { // Only saves the key if it is not to be saved
                                // later.
      isDirty = true;
    } else {
  ...
  /**
   * Sets the flag that indicates if the not should have its write process
   * delayed or not.
   *
  ...
  {
    if (index.isWriteDelayed && isDirty &&
        !delayed) { // Before changing the flag, flushs the node.
      save(false, 0, size);
    }
  }
}
