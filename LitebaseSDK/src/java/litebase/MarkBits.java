 ...
 * applied to the associated WHERE clause. This class should only be used if the
 * result set has a WHERE clause.
 */
class MarkBits
{
  /**
   ...
   * @return <code>false</code> if the key could not be climbed;
  ...
  boolean onKey(Key k) throws IOException
  {
    ...
    if (key.asString == null &&
        (type == SQLElement.CHARS ||
         type == SQLElement.CHARS_NOCASE)) { // A string may not be loaded.
      db.dbo.setPos(
        key.asInt); // Gets and sets the string position in the .dbo.
      key.asString = db.loadString();
    }

    if (rightKey != null) {
      int comp =
        Utils.arrayValueCompareTo(k.keys,
                                  rightKey.keys,
                                  k.index.types,
                                  null); // Compares the key with the right key.
    ...
    if (l0 == SQLElement.OP_REL_EQUAL ||
        l0 == SQLElement.OP_REL_GREATER_EQUAL ||
        (l0 == SQLElement.OP_REL_GREATER && isNoLongerEqual)) {
      onValue(k.record);                          // Climbs on the value.
    } else if (l0 == SQLElement.OP_REL_GREATER) { // The key can still be equal.
      if (Utils.arrayValueCompareTo(leftKey.keys,
                                    k.keys,
                                    leftKey.index.types,
                                    null) !=
          0) { // Compares the key with the left key.
      ...
      if (val.startsWith(leftKey.keys[0].asString)) { // Only starts with are
                                                      // used with indices.
        onValue(k.record);                            // Climbs on the value.
      } else {
  ...
  void onValue(int record)
  {
    if (record != Key.NO_VALUE) {
      indexBitmap.setBit(
        record, bitValue); // (Un)sets the corresponding bit on the bit array.
    }
  }
}
