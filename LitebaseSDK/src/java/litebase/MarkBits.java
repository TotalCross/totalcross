...
class MarkBits {
	...
	boolean onKey(Key k) throws IOException {
		...
		if (key.asString == null && (type == SQLElement.CHARS
		...
		if (rightKey != null) {
			int comp = Utils.arrayValueCompareTo(k.keys, rightKey.keys, k.index.types,
		...
		if (l0 == SQLElement.OP_REL_EQUAL || l0 == SQLElement.OP_REL_GREATER_EQUAL
			...
			if (Utils.arrayValueCompareTo(leftKey.keys, k.keys, leftKey.index.types,
										  null) != 0) { // Compares the key with the left key.
