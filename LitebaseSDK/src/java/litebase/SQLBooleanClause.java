...
class SQLBooleanClause {
	...
	boolean sqlbooleanclauseApplyTableIndices(Index[] tableIndices, boolean hasComposedIndex) {
		...
		if (!isWhereClause
			|| fieldList.length == 0) { // Indices can only be applied to the where clause with fields.
			return false;
		}
			...
			switch (curOperandType = curTree.operandType) { // Checks the operand type.
				...
				case SQLElement.OP_BOOLEAN_OR:
					...
					if ((leftOperandType >= SQLElement.OP_REL_LESS && leftOperandType <= SQLElement.OP_REL_DIFF)
						...
						if (hasComposedIndex && curOperandType == SQLElement.OP_BOOLEAN_AND
									...
									curTree = curTree.rightTree;
										   ...
										   || (curTree.rightTree.patternMatchType == SQLBooleanClauseTree.PAT_MATCH_STARTS_WITH
											   && (rightOperandType == SQLElement.OP_PAT_MATCH_LIKE
												   || rightOperandType == SQLElement.OP_PAT_MATCH_NOT_LIKE))) {
							...
							if (count >= 2) { // It has an AND operator on at least 2 fields?
								...
								while (--i >= 0) {
									...
									if (appliedComposedIndex) {
										...
										curTree = sqlbooleanclauseApplyComposedIndexToBranch(originalTree, columns, operators,
												  indexesValueTree,
												  currCompIndex);
										break;
									} else {
		...
		if (leftIsColumn != rightIsColumn) {
			...
			while (--i >= 0)
				if (list[i].tableColIndex == column
					&& list[i].isDataTypeFunction) { // An index cannot be applied to a function in the where clause.
					return;
				}
	 ...
	 */
	private SQLBooleanClauseTree sqlbooleanclauseApplyComposedIndexToBranch(SQLBooleanClauseTree branch,
			byte[] columns,
			byte[] operators,
		...
		if (parent == null) {
			...
			if (branch.operandType != SQLElement.OP_BOOLEAN_AND
				&& branch.operandType != SQLElement.OP_BOOLEAN_OR) { // Is the end of the root?
				branch = null;
			}
	...
	void sqlBooleanClausePreVerify() throws DriverException {
		...
		while (--i >= 0)
			if (!paramList[i].isParamValueDefined)
				throw new DriverException(LitebaseMessage.getMessage(LitebaseMessage.ERR_PARAMETER_NOT_DEFINED) +
										  i);
	}
	 ...
	 */
	boolean sqlBooleanClauseSatisfied(ResultSet rs) throws IOException, InvalidDateException,
	 ...
	 * @throws InvalidDateException If an internal method throws it.
	 */
	void bindColumnsSQLBooleanClause(IntHashtable names2Index, byte[] columnTypes,
									 SQLResultSetTable[] tableList) throws InvalidDateException, InvalidNumberException {
		...
		if (tableList != null) { // The having clause has already been verified.
			...
			while (--i >= 0) {
				field = fieldList[i];
				field.tableColIndex = names2Index.get(field.aliasHashCode,
	 ...
	 */
	void verifyColumnNamesOnTableList(SQLResultSetField[] sqlBooleanClauseFieldList,
		...
		while (--size >= 0) {
			...
			if (field.tableName != null) {
				...
				while (--i >= 0) {
					...
					if (index != -1)
						if (foundFirst)
							throw new SQLParseException(LitebaseMessage.getMessage(LitebaseMessage.ERR_AMBIGUOUS_COLUMN_NAME) +
														field.alias);
						else {
							foundFirst = true;
							field.table = currentTable;
	 ...
	 */
	void bindColumnsSQLBooleanClause(IntHashtable names2Index, byte[] columnTypes,
									 SQLResultSetTable rsTable) throws InvalidDateException, InvalidNumberException {
		...
		while (--size >= 0) {
			...
			if (field.tableName != null) {
				...
				if (rsTable.aliasTableNameHashCode != field.tableName.hashCode()
					|| (index = (currentTable = rsTable.table).htName2index.get(field.tableColName.hashCode(),
								-1)) == -1)
					throw new SQLParseException(LitebaseMessage.getMessage(LitebaseMessage.ERR_UNKNOWN_COLUMN) +
												field.alias);
				field.table = currentTable;
