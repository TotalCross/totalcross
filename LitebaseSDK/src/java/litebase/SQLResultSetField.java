...
class SQLResultSetField
{
   ...
   */
  int index =
    -1; // juliana@230_21: MAX() and MIN() now use indices on simple queries.

  /**
   * Indicates that the index to be used is composed or not.
   */
  boolean isComposed; // juliana@230_21: MAX() and MIN() now use indices on
                      // simple queries.
  ...
  void findMaxMinIndex()
  {
    ...
    int column = parameter != null ? parameter.tableColIndex
                                   : tableColIndex, // juliana@230_29
      i = tableAux.numberComposedIndices;
    ComposedIndex[] composedIndices = tableAux.composedIndices;

    if (tableAux.columnIndices[column] !=
        null) { // If the field has a simple index, uses it.
      index = column;
      isComposed = false;
    } else
      while (--i >= 0)
        if (composedIndices[i].columns[0] ==
