	...
	public void testRun() {
		...
		if (!driver.exists("Lancamento")) {
			...
			try {
				...
				assertTrue(
					exception.getMessage().startsWith("It is not possible to open a table within a connection with a different"));
				driver.executeUpdate("drop table lancamento");
				driver.execute("create table Lancamento(data int, ndoc int, valor double, pago short, tipo short, dataExt char(10), origem char(20), "
							   + "obs char(50) nocase)");
			}

		PreparedStatement psListar =
			driver.prepareStatement("select rowid,pago,dataExt,ndoc,valor,tipo from lancamento where data <= ? and "
									+ "(data >= ? or pago=0) order by data");
		assertEquals(1,
					 driver.executeUpdate("insert into lancamento (data,dataext,ndoc,valor,tipo,origem,obs,pago) values "
		...
		PreparedStatement preparedStmt =
			driver.prepareStatement("insert into PERSON values (?, ?, ?, ?, ?, ?) "); // added space at the end.
		...
		assertEquals(8, executeQuery(driver,
		...
		assertEquals(8,
					 driver.executeUpdate("update PERSON set FIRST_NAME = 'x' where salary_prev < 3000 or ((not years_exp_c >= years_exp_java) "
										  + "and not years_exp_java >= 10) or FIRST_NAME > 'r'"));

		// Tests SELECT prepared statements.
		preparedStmt =
			driver.prepareStatement("select * from PERSON where salary_prev < ? or ((not years_exp_c >= years_exp_java) "
		...
		(preparedStmt =
			 driver.prepareStatement("select years_exp_java, count(*) as total from person where years_exp_c > ? and last_name < ? group by "
		...
		(preparedStmt =
			 driver.prepareStatement("update PERSON set FIRST_NAME = ?, last_name = ? where salary_prev < ? or "
									 + "((not years_exp_c >= years_exp_java) and not years_exp_java >= ?) or FIRST_NAME not like ?")).setString(
										 0, "roberto");
		...
		(preparedStmt =
			 driver.prepareStatement("delete person where salary_prev <= ? and not years_exp_c > ?")).setDouble(
				 0, 4000);
		preparedStmt.setInt(1, 10);
		...
		ResultSet rs =
			driver.prepareStatement("select sum(TOTAL) as VALORTOTAL from PEDIDO").executeQuery();
		assertTrue(rs.first());
		...
		PreparedStatement psInsert =
			driver.prepareStatement("insert into ITEMPEDIDO(PEDIDO, PRODUTO, QTDE, UNITARIO, TOTAL) values(?, ?, ?, ?, ?)");
		...
		PreparedStatement psUpdate =
			driver.prepareStatement("update ITEMPEDIDO set QTDE = ?, UNITARIO = ?, TOTAL = ? where PEDIDO = ? and "
									+ "PRODUTO = ?");
		...
		(preparedStmt = driver.prepareStatement("select * from teste where sh1 = ? and x = ?")).setShort(0,
				(short)6);
		preparedStmt.setFloat(1, 8);
		...
		(preparedStmt = driver.prepareStatement("delete from teste where sh1 = ? and x = ?")).setShort(0,
				(short)6);
		preparedStmt.setFloat(1, 8);
		preparedStmt.executeUpdate();
		...
		(preparedStmt =
			 driver.prepareStatement("select * from teste, teste2 where id = id2 and sh1 = ? and x = ?")).setShort(
				 0, (short)6);
		preparedStmt.setFloat(1, 8);
		...
		rs = (preparedStmt =
				  driver.prepareStatement("select field2 from table1 group by field2")).executeQuery();
		assertTrue(rs.next());
		...
		preparedStmt =
			driver.prepareStatement("select * from teste where ( (1 != 1) or (Upper(Nome) = ? and 1 = 1)) and (TipoOp < 3 and idPessoa != -2)");
		...
		assertTrue((rs = (preparedStmt =
							  driver.prepareStatement("select property.value from property where keyP like 'ws%'")).executeQuery()).next());
		rs.close();
		...
		preparedStmt =
			driver.prepareStatement("select * from teste, teste2 where id = id2 and sh1 = ? and x = ?");
		assertTrue(preparedStmt.isValid());
		preparedStmt.close();
		assertFalse(preparedStmt.isValid());
		...
		(preparedStmt =
			 driver.prepareStatement("UPDATE rota SET dt_abertura = ?, id_rota_dia = 'A' WHERE cd_rota = ?")).clearParameters();
		...
		(preparedStmt =
			 driver.prepareStatement("UPDATE rota SET id_rota_dia = 'A', dt_abertura = ? WHERE cd_rota = ?")).clearParameters();
		preparedStmt.setDate(0, date);
		preparedStmt.setInt(1, 1);
		preparedStmt.executeUpdate();
		...
		(preparedStmt =
			 driver.prepareStatement("INSERT INTO exemplo(campo1, campo2) VALUES (?, ?)")).toString();
		assertTrue(preparedStmt.isValid());
		preparedStmt.close();
		assertFalse(preparedStmt.isValid());
		(preparedStmt =
