...
public class TestPreparedStatement extends TestCase
{
  public void testRun()
  {
    ...
    if (!driver.exists("Lancamento")) {
      driver.execute(
        "create table Lancamento(data int, ndoc int, valor double, pago short, tipo short, dataExt char(10), origem char(20), "
        + "obs char(50) nocase)");
    ...
    PreparedStatement psListar = driver.prepareStatement(
      "select rowid,pago,dataExt,ndoc,valor,tipo from lancamento where data <= ? and "
      + "(data >= ? or pago=0) order by data");
    assertEquals(
      1,
      driver.executeUpdate(
        "insert into lancamento (data,dataext,ndoc,valor,tipo,origem,obs,pago) values "
    ...
    // Creates thr table.
    driver.execute(
      "create table PERSON (FIRST_NAME CHAR(30), LAST_NAME CHAR(30), SALARY_CUR DOUBLE, SALARY_PREV DOUBLE, YEARS_EXP_JAVA INT, "
      + "YEARS_EXP_C INT )");

    // Creates the insert prepared statement.
    PreparedStatement preparedStmt = driver.prepareStatement(
      "insert into PERSON values (?, ?, ?, ?, ?, ?) "); // added space at the
                                                        // end.
    ...
    assertEquals(
      10, executeQuery(driver, "select * from PERSON")); // No where clause
    ...
    assertEquals(
      ...
      executeQuery(
        driver,
        "select * from PERSON where salary_prev <= salary_cur and last_name like '%or%'"));
    assertEquals(
      9,
      executeQuery(
        driver,
        "select * from PERSON where salary_prev <= salary_cur or last_name like '%or%'"));
    assertEquals(
      4,
      executeQuery(
        driver,
        "select * from PERSON where salary_prev <= salary_cur and not years_exp_c > 10"));
    assertEquals(
      6,
      executeQuery(
        driver,
        "select * from PERSON where (salary_prev < 3000 and years_exp_java >= 10) or FIRST_NAME > 'r'"));
    assertEquals(
      4,
      executeQuery(
        driver,
    ...
    assertEquals(
      ...
      executeQuery(
        driver,
        "select * from PERSON where (salary_prev < 3000 or not years_exp_c >= years_exp_java) and not "
          + "(years_exp_java >= 10 or FIRST_NAME > 'r')"));
    assertEquals(
      8,
      executeQuery(
        driver,
        "select * from PERSON where salary_prev < 3000 or ((not years_exp_c >= years_exp_java) "
    ...
    assertEquals(
      8,
      driver.executeUpdate(
        "update PERSON set FIRST_NAME = 'x' where salary_prev < 3000 or ((not years_exp_c >= years_exp_java) "
        + "and not years_exp_java >= 10) or FIRST_NAME > 'r'"));

    // Tests SELECT prepared statements.
    preparedStmt = driver.prepareStatement(
      "select * from PERSON where salary_prev < ? or ((not years_exp_c >= years_exp_java) "
    ...
    (preparedStmt = driver.prepareStatement(
       "select years_exp_java, count(*) as total from person where years_exp_c > ? and last_name < ? group by "
    ...
    (preparedStmt = driver.prepareStatement(
       "update PERSON set FIRST_NAME = ?, last_name = ? where salary_prev < ? or "
       +
       "((not years_exp_c >= years_exp_java) and not years_exp_java >= ?) or FIRST_NAME not like ?"))
      .setString(0, "roberto");
    ...
    assertEquals(
      ...
      executeQuery(
        driver,
        "select * from PERSON where first_name = 'roberto' and last_name = 'carlos'"));

    // Tests DELETE prepared statements.
    (preparedStmt = driver.prepareStatement(
       "delete person where salary_prev <= ? and not years_exp_c > ?"))
      .setDouble(0, 4000);
    preparedStmt.setInt(1, 10);
    ...
    assertEquals(
      4,
      executeQuery(driver,
                   "select * from PERSON")); // Checks if DELETE worked.
    ...
    try {
      ...
      assertGreaterOrEqual(
        exception.getMessage().indexOf("had their values defined"), 0);
    }
    ...
    ResultSet rs =
      driver.prepareStatement("select sum(TOTAL) as VALORTOTAL from PEDIDO")
        .executeQuery();
    assertTrue(rs.first());
    ...
    if (!driver.exists("ITEMPEDIDO")) {
      driver.execute(
        "create table ITEMPEDIDO(PEDIDO int, PRODUTO int, QTDE int, UNITARIO double, TOTAL double)");
      driver.execute(
        "create index IDX_ITEMPEDIDO_PEDIDO on ITEMPEDIDO(PEDIDO)");
    } else
      try {
    ...
    PreparedStatement psInsert = driver.prepareStatement(
      "insert into ITEMPEDIDO(PEDIDO, PRODUTO, QTDE, UNITARIO, TOTAL) values(?, ?, ?, ?, ?)");
    ...
    PreparedStatement psUpdate = driver.prepareStatement(
      "update ITEMPEDIDO set QTDE = ?, UNITARIO = ?, TOTAL = ? where PEDIDO = ? and "
      + "PRODUTO = ?");
    ...
    driver.execute(
      "create table teste(id int primary key, sh1 short, x float)");

    (preparedStmt =
       driver.prepareStatement("insert into teste values (?, ?, ?)"))
      .setInt(0, 3);
    ...
    (preparedStmt = driver.prepareStatement("update teste set sh1 = ?, x = ?"))
      .setShort(0, (short)6);
    ...
    (preparedStmt =
       driver.prepareStatement("select * from teste where sh1 = ? and x = ?"))
      .setShort(0, (short)6);
    preparedStmt.setFloat(1, 8);
    ...
    (preparedStmt =
       driver.prepareStatement("delete from teste where sh1 = ? and x = ?"))
      .setShort(0, (short)6);
    preparedStmt.setFloat(1, 8);
    preparedStmt.executeUpdate();
    ...
    (preparedStmt = driver.prepareStatement(
       "select * from teste, teste2 where id = id2 and sh1 = ? and x = ?"))
      .setShort(0, (short)6);
    preparedStmt.setFloat(1, 8);
    ...
    driver.execute(
      "create table table1 (field1 int NOT NULL, field2 char(50) NOT NULL, primary key (field1))");
    preparedStmt =
      driver.prepareStatement("insert into table1 values(?, 'Thiago')");
    i = -1;
    while (++i < 5) {
      preparedStmt.setInt(0, i + 1);
      preparedStmt.executeUpdate();
    }
    rs = (preparedStmt = driver.prepareStatement(
            "select field2 from table1 group by field2"))
           .executeQuery();
    assertTrue(rs.next());
    ...
    driver.execute(
      "create table teste (idPessoa int primary key, Nome varchar(50), TipoOp short)");
    driver.executeUpdate("insert into teste values (-2, 'joao', 0)");
    preparedStmt = driver.prepareStatement(
      "select * from teste where ( (1 != 1) or (Upper(Nome) = ? and 1 = 1)) and (TipoOp < 3 and idPessoa != -2)");
    ...
    driver.execute(
      "create table property (keyP char(100) NOT NULL, value char(200), descr char(100) NOT NULL, status int NOT NULL, "
      + "primary key (keyP))");
    driver.executeUpdate(
      "insert into property (keyP, value, descr, status) values ('ws_prod', NULL, 'Produ��o', 1)");
    driver.executeUpdate(
      "insert into property (keyP, value, descr, status) values ('ws_test', NULL, 'Teste', 2)");
    driver.executeUpdate(
      "insert into property (keyP, value, descr, status) values ('tables_bkp', 'true', 'Criar c�pia de seguran�a', 1)");
    driver.executeUpdate(
      "insert into property (keyP, value, descr, status) values ('tables_drop', NULL, 'Excluir tabelas', 3)");
    driver.executeUpdate(
      "insert into property (keyP, value, descr, status) values ('tables_restore', NULL, 'Recuperar tabelas', 3)");

    assertTrue(
      (rs = (preparedStmt = driver.prepareStatement(
               "select property.value from property where keyP like 'ws%'"))
              .executeQuery())
        .next());
    rs.close();
    ...
    // Tests what happens if a table being used by a prepared statement is
    // dropped.
    driver.executeUpdate("drop table teste");
    try {
    ...
    // Tests update prepared statements with some data to be changed already
    // given.
    if (driver.exists("rota")) {
      driver.executeUpdate("DROP TABLE rota");
    }
    driver.execute(
      "CREATE TABLE rota (id_rota_dia CHAR(1), ds_rota CHAR(20) NOT NULL, cd_rota INT NOT NULL, dt_abertura DATE, PRIMARY KEY(cd_rota))");
    driver.executeUpdate(
      "INSERT INTO rota(ds_rota, cd_rota) VALUES ('Rota 1', 1)");
    driver.executeUpdate(
      "INSERT INTO rota(ds_rota, cd_rota) VALUES ('Rota 2', 2)");
    driver.executeUpdate(
      "INSERT INTO rota(ds_rota, cd_rota) VALUES ('Rota 3', 3)");
    driver.executeUpdate(
      "INSERT INTO rota(ds_rota, cd_rota) VALUES ('Rota 4', 4)");

    Date date = new Date();
    String dateStr = date.toString();

    (preparedStmt = driver.prepareStatement(
       "UPDATE rota SET dt_abertura = ?, id_rota_dia = 'A' WHERE cd_rota = ?"))
      .clearParameters();
    preparedStmt.setDate(0, date);
    preparedStmt.setInt(1, 2);
    preparedStmt.executeUpdate();
    (rs = driver.executeQuery("select dt_abertura from rota where cd_rota = 2"))
      .first();
    ...
    (preparedStmt = driver.prepareStatement(
       "UPDATE rota SET id_rota_dia = 'A', dt_abertura = ? WHERE cd_rota = ?"))
      .clearParameters();
    preparedStmt.setDate(0, date);
    preparedStmt.setInt(1, 1);
    preparedStmt.executeUpdate();
    (rs = driver.executeQuery("select dt_abertura from rota where cd_rota = 1"))
      .first();
    assertEquals(1, rs.getRowCount());
    assertEquals(dateStr, rs.getDate(1).toString());
    assertTrue(preparedStmt.isValid());
    ...
    driver.execute(
      "CREATE TABLE exemplo(campo1 INT, campo2 CHAR(50), PRIMARY KEY(campo1))");

    (preparedStmt = driver.prepareStatement(
       "INSERT INTO exemplo(campo1, campo2) VALUES (?, ?)"))
      .toString();
    assertTrue(preparedStmt.isValid());
    preparedStmt.close();
    assertFalse(preparedStmt.isValid());
    ...
    try {
      driver.prepareStatement("alter table teste2 drop primary key")
        .executeQuery();
      fail("20");
    } catch (DriverException exception) {
    }
  ...
  /**
   * Execute a prepated query and returns the number of rows that answers the
   * query.
   *
