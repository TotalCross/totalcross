...
public class TestReIndex2rowId extends TestCase
{
  public void testRun()
  {
    ...
    driver.execute(
      "create index idxId on person(rowId)"); // Creates an index on rowid
                                              // after some inserts.

    // Do selects.
    ResultSet rs = driver.executeQuery(
      "Select  rowid, name, age from person where rowid = 3");
    ...
    assertEquals(3,
                 (rs = driver.executeQuery(
                    "Select  rowid, name, age from person where rowid > 2"))
                   .getRowCount());
    assertTrue(rs.next());
    assertEquals(3, rs.getInt("rowid"));
    assertEquals("RLN_2", rs.getString("name"));
