...
public class TestLogger extends TestCase
{
  ...
  public void testRun()
  {
    ...
    assertEquals(
      0,
      LitebaseConnection.deleteLogFiles()); // No logger files to be deleted.

    // Creates one logger file and tries to delete it when using and after using
    // it.
    ...
    // Creates three logger files. Just the not currently used ones can be
    // deleted.
    ...
    if (AllTests
          .useLogger) { // If the tests are using logger, turn it on again.
      LitebaseConnection.logger = LitebaseConnection.getDefaultLogger();
    }
  }
}
