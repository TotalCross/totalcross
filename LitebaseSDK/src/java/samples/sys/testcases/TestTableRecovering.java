...
public class TestTableRecovering extends TestCase {
	...
	public void testRun() {
		...
		try {
			while (--record >=
				   0) { // Repeats the test corrupting a different record, from the first (0) till after the last (10).
				...
				assertEquals(record < 10,
							 driver.recoverTable("person")); // If the corruption occurs after the last record, nothing needs to be recovered.
				assertTrue(driver.isTableProperlyClosed("person"));
				...
				try { // Empty file: table corrupted.
					...
					assertTrue((message = exception.getMessage()).indexOf("corrupted") != -1
