...
public class TestTableRecovering extends TestCase
{
  ...
  public void testRun()
  {
    ...
    try {
      while (--record >= 0) { // Repeats the test corrupting a different record,
                              // from the first (0) till after the last (10).
        ...
        driver.execute(
          "create table person (id int not null, name char(30) default 'Maria', cpf long not null, photo blob(2), gender char(1), "
        ...
        prepared = driver.prepareStatement(
        ...
        dbFile =
          new File(tablePath,
                   File.READ_WRITE); // The table is closed after recovering it.
        ...
        if ((pos = 512 + 41 * record + rand.nextInt(33)) + 7 >
            dbFile.getSize()) {
        ...
        assertEquals(record < 10,
                     driver.recoverTable(
                       "person")); // If the corruption occurs after the last
                                   // record, nothing needs to be recovered.
        assertTrue(driver.isTableProperlyClosed("person"));

        ResultSet resultSet = driver.executeQuery("select * from person");

        // If the corruption occurs after the last records, all the records are
        // available.
        ...
        try { // Empty file: table corrupted.
          ...
          assertTrue(
            (message = exception.getMessage()).indexOf("corrupted") != -1 ||
            message.indexOf("format") != -1 || message.indexOf("read") != -1);
        }
        try { // Empty file: table corrupted.
          driver.recoverTable("person");
          fail("8");
        ...
        try { // Blank file: table corrupted.
          ...
          assertTrue((message = exception.getMessage()).indexOf("corrupted") !=
                       -1 ||
                     message.indexOf("format") != -1);
        }
