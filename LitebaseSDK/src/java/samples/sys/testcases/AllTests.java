...
public class AllTests extends TestSuite {
	...
	public AllTests() {
		...
		addTestCase(
			TestAsciiTables.class); // juliana@210_2: now Litebase supports tables with ascii strings.
	...
	public void onEvent(Event event) {
		if (event.type == ControlEvent.PRESSED && event.target == mbar) {
			...
			Settings.appSecretKey = (useLogger ? "y" : "n") + (isAscii ? 'y' : 'n') +
									(useCrypto ? 'y' : 'n') + (LitebaseConnection.logOnlyChanges ? 'y' : 'n');
		}
	...
	static LitebaseConnection getInstance() {
		return LitebaseConnection.getInstance(Settings.applicationId,
											  "chars_type =" + (isAscii ? "ascii" : "unicode") + (useCrypto ? "; crypto" : "") + "; path = " +
											  Settings.appPath);
	}
