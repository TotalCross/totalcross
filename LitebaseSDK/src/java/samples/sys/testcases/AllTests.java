...
public class AllTests extends TestSuite
{
  ...
  /**
   * Indicates if the connections of the tests except for
   * <code>TestAsciiTables</code> and <code>TestSourcePath</code> use ascii
   * connections.
   */
  ...
  /**
   * Indicates if the connections of the tests except for
   * <code>TestAsciiTables</code> and <code>TestSourcePath</code> use
   * cryptography.
   */
  ...
  public AllTests()
  {
    ...
    addTestCase(TestAsciiTables.class); // juliana@210_2: now Litebase supports
    ...
    addTestCase(
      TestResultSet
        .class); // juliana@211_4: solved bugs with result set dealing.
  ...
  { // rnovais@570_77
    ...
    LitebaseConnection.logOnlyChanges =
      appSecretKey != null && appSecretKey.charAt(3) == 'y';
    if (useLogger) {
      LitebaseConnection.logger = LitebaseConnection.getDefaultLogger();
    }
    super.initUI();
    setMenuBar(
      mbar = new MenuBar(new MenuItem[][] {
        mbar.getMenuItems()[0],
        { new MenuItem("Config"),
          new MenuItem("isAscii", isAscii),
          new MenuItem("useCrypto", useCrypto),
          new MenuItem("Drop all tables") },
        { new MenuItem("Logs"),
          new MenuItem("Use Logger", useLogger),
          new MenuItem("Log only changes", LitebaseConnection.logOnlyChanges),
          new MenuItem("Erase All Loggers") } }));
  }
  ...
  public void onEvent(Event event)
  {
    if (event.type == ControlEvent.PRESSED && event.target == mbar) {
      ...
      Settings.appSecretKey = (useLogger ? "y" : "n") + (isAscii ? 'y' : 'n') +
                              (useCrypto ? 'y' : 'n') +
                              (LitebaseConnection.logOnlyChanges ? 'y' : 'n');
    }
  ...
  /**
   * Drops all tables used for the testcases. Notice that all the tables in the
   * folders used by the tests and with the creation id "Test" will be erased
   * even if they are from another application.
   */
  private void dropAllTables()
  {
    ...
    try {
      temporario =
        Convert.appendPath(File.getCardVolume().getPath(), "tempor�rio/");
    ...
    String[] paths = {
      Settings.appPath, Settings.dataPath, tempPath, tempPath + "a/",
      tempPath + "b/",  temporario,        "/"
    };
    ...
    i = 6;
    while (--i >= 2) // Erases the folders, being careful to erase the empty
                     // folders first.
      try {
        folders[i].delete();
      } catch (IOException exception) {
   ...
   *
   * @return A driver connection using ascii or unicode characters depending on
   *     which option was made and the application id as the application id
   * used by Litebase.
   */
  static LitebaseConnection getInstance()
  {
    return LitebaseConnection.getInstance(
      Settings.applicationId,
      "chars_type =" + (isAscii ? "ascii" : "unicode") +
        (useCrypto ? "; crypto" : "") + "; path = " + Settings.appPath);
  }
   ...
   * @return A driver connection using ascii or unicode characters depending on
   *     which option was made and the path given by the user.
   */
