...
/**
 * Tests What happens when the driver is closed and tests
 * <code>processLogs()</code>.
 */
public class TestClosedLitebaseAndProcessLogs extends TestCase
{
  ...
  public void testRun()
  {
    ...
    PreparedStatement psSel =
      driver.prepareStatement("select * from tabps where name = ?");
    ...
    assertEquals(
      1,
      driver.executeUpdate(
        "insert into PRODUTO values(1, '19132',2, 1, '1, 2, 3', 'ABSORVENTE SILHO ABAS', '5', 13, 'PCT', '20X30', 10.f, 0)"));

    // Tests if gc won't make the Litebase instance close.
    int i = 100;
    while (--i >= 0) {
      testTemporaryConnection();  // Gets another Litebase instance.
      driver.exists("tableName"); // The connection can't be closed because of
   ...
   */
  private void testTemporaryConnection() { AllTests.getInstance("Test"); }
}
