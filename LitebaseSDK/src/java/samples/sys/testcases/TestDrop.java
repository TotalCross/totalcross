...
public class TestDrop extends TestCase
{
  ...
  public void testRun()
  {
    ...
    // Drop table can't erase a table whose name is the prefix of the table
    // being dropped.
   ...
   *
   * @return <code>true</code> if sone table file exists; <code>false</code>,
   *     otherwise.
   */
  private boolean TableFileExist()
  {
    try {
      return new File(path + "Test-person.db", File.DONT_OPEN).exists() ||
        new File(path + "Test-person.dbo", File.DONT_OPEN).exists() ||
        new File(path + "Test-person$0.idk", File.DONT_OPEN).exists() ||
   ...
   *
   * @return <code>true</code> if sone index file exists; <code>false</code>,
   *     otherwise.
   */
  private boolean IndexExist()
  {
    try {
      return new File(path + "Test-person$0.idk", File.DONT_OPEN).exists() ||
        new File(path + "Test-person$1.idk", File.DONT_OPEN).exists() ||
        new File(path + "Test-person&1.idk", File.DONT_OPEN).exists() ||
        new File(path + "Test-person&2.idk", File.DONT_OPEN).exists();
    } catch (IOException exception) {
