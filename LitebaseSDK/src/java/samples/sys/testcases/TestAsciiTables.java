...
public class TestAsciiTables extends TestCase
{
  ...
  {
    String tempPath =
      Convert.appendPath(Settings.appPath, "temp/"); // The path used.

    // Creates a table within an ascii connection.
    LitebaseConnection conn = LitebaseConnection.getInstance(
      "Test", "chars_type = ascii; path = " + tempPath);
    ...
    (conn = LitebaseConnection.getInstance(
       "Test", "chars_type = unicode; path = " + tempPath))
      .execute("create table person (name char(10))");
    conn.closeAll();
    ...
    // Creates two connections which only differ in the string types. They must
    // be different.
    LitebaseConnection connAux = LitebaseConnection.getInstance(
      "Test", "path = " + tempPath + " ; chars_type = ascii");
    assertNotEquals(conn = LitebaseConnection.getInstance(
                      "Test", "path = " + tempPath + ";chars_type = unicode"),
                    connAux);

    // Since the paths and the application id are the same, the second
    // connection can't create the table.
    conn.execute("create table person (name char(10))");
    ...
    // Trying to use an unicode table in an ascii connection must fail, but it
    // can't fail in an unicode connection.
    conn = LitebaseConnection.getInstance(
      "Test", "chars_type = unicode; path = " + tempPath);
    connAux = LitebaseConnection.getInstance(
      "Test", "chars_type = ascii; path = " + tempPath);
    try {
      connAux.executeQuery("select * from person");
      fail("5");
    ...
    // It is not possible to use a table within a connection with a different
    // kind of string.
    ...
    rs2.close();
    assertTrue((rs2 = connAux.executeQuery(
                  "select * from person2 where name like '\\''"))
                 .next());
    assertEquals("'", rs2.getString("name"));
    assertFalse(rs2.next());
    rs2.close();
    ...
    try { // The .dbo of the ascii table must be smaller than the .dbo of the
          // unicode table.
      File file1, file2;
      assertLower(
        (file1 = new File(tempPath + "Test-person2.dbo", File.READ_WRITE))
          .getSize(),
        (file2 = new File(tempPath + "Test-person1.dbo", File.READ_WRITE))
          .getSize());
      file1.close();
      file2.close();
