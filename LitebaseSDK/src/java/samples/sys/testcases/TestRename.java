...
/**
 * This test verifies if the "rename table" and "rename column" are working
 * properly.
 */
public class TestRename extends TestCase
{
  ...
  public void testRun()
  {
    ...
    driver.execute(
      "create table person(name char(16) , age int, primary key(name, age))");
    ...
    assertEquals(
      ...
      driver.executeUpdate(
        "ALTER TABLE person RENAME TO Person2")); // Renames the table.
    ...
    assertEquals(-1,
                 selectOldTable()); // The old table does not exist anymore, so
                                    // the select will fail.
    ...
    assertEquals(
      -1,
      createIndexNewTable()); // The index already exists from the old table.
    ...
    assertEquals(
      0,
      driver.executeUpdate(
        "alter table person2 drop primary key")); // Drops the primary key.
    ...
    assertEquals(
      0,
      driver.executeUpdate(
        "ALTER TABLE peRson2 RENAME age TO years ")); // Renames column.
    ...
    assertEquals(
      -1,
      createIndexNewColumnTable()); // Since the new table already has an
                                    // index, the index can't be created.
    assertEquals(
      1,
      driver.executeUpdate(
        "Drop index years on person2"));          // First, it must be dropped.
    assertEquals(1, createIndexNewColumnTable()); // Then, re-created.
    assertEquals(
      1,
      driver.executeUpdate(
        "insert into persOn2(name,years) values ('indira gomes',14)")); // Inserts
                                                                        // a new
                                                                        // row.

    // Does a select using the new column.
    assertEquals(3,
                 (rs = driver.executeQuery("Select name, years from Person2 "))
                   .getRowCount());
    rs.close();

    assertEquals(1,
                 driver.prepareStatement("Drop index years on person2")
                   .executeUpdate()); // Drops the index again.

    // Renames a table and renames it back without using close all between the
    // operations.
    if (driver.exists("person")) {
      driver.executeUpdate("drop table person");
    }
    ...
    driver.prepareStatement("alter table person rename to person2")
      .executeUpdate();
    driver.prepareStatement("alter table person2 rename to person")
      .executeUpdate();
    driver.prepareStatement("alter table person rename to person2")
      .executeUpdate();
    driver.prepareStatement("alter table person2 drop primary key")
      .executeUpdate();
    driver.prepareStatement("CREATE index ageindex on person2(age)")
      .executeUpdate();

    // Renames a table and renames it back using close all between the
    // operations.
    if (driver.exists("person")) {
      driver.executeUpdate("drop table person");
    }
    ...
    try { // Checks if the files of the old table does not exist and that the
          // files of the new table exist.
      ...
      assertTrue(
        new File(path + "Test-person2$2.idk", File.DONT_OPEN).exists());
   ...
   *
   * @return 1 If the index was successully created or -1 if an exception was
   *     thrown.
   */
   ...
   *
   * @return If the index was successully created or -1 if an exception was
   *     thrown.
   */
