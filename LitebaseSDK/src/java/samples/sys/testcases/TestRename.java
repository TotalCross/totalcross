...
public class TestRename extends TestCase {
	...
	public void testRun() {
		...
		assertEquals(-1,
					 selectOldTable()); // The old table does not exist anymore, so the select will fail.
		...
		assertEquals(0,
					 driver.executeUpdate("alter table person2 drop primary key")); // Drops the primary key.
		...
		assertEquals(0,
					 driver.executeUpdate("ALTER TABLE peRson2 RENAME age TO years ")); // Renames column.
		...
		assertEquals(-1,
					 createIndexNewColumnTable()); // Since the new table already has an index, the index can't be created.
		...
		assertEquals(1,
					 driver.executeUpdate("insert into persOn2(name,years) values ('indira gomes',14)")); // Inserts a new row.
		...
		assertEquals(1,
					 driver.prepareStatement("Drop index years on person2").executeUpdate()); // Drops the index again.
