...
/**
 * Tests many <code>ResultSet</code> public methods that are not tested in the
 * previous test cases,
 */
public class TestResultSet extends TestCase
{
  ...
  public void testRun()
  {
    ...
    testResultSet(driver.executeQuery(
      "select * from person where age > 0 order by name")); // Select with
                                                            // temporary
                                                            // table.
    testResultSet(driver.executeQuery(
      "select * from person where age > 0")); // Select without temporary
                                              // table.
    testResultSet(driver.executeQuery(
      "select * from person")); // Select without temporary table.
    testResultSet(driver.executeQuery(
      "select rowid, name, age, birth, years from person")); // Select without
                                                             // temporary
                                                             // table.
    testResultSet(driver.executeQuery(
      "select rowid, name, age, birth, years as anos from person")); // Select
                                                                     // without
                                                                     // temporary
                                                                     // table.
    testResultSet(driver.executeQuery(
      "select rowid, upper(name) as nameAux, age, birth, years as anos from person")); // Select without temporary table.
    testResultSet(driver.executeQuery(
      "select rowid, lower(name) as nameAux, age, birth, years as anos from person")); // Select without temporary table.

    // Drops some records and adds them again so that there are enpty spaces in
    // the select base table.
    ...
    driver.executeUpdate(
      "insert into person values ('Zenes Lima',15, '2005/9/12', '2006/08-21 0:08')");
    driver.executeUpdate(
      "insert into person values ('Juliana Imperial',29, '1979/06/26', '2009/04-14 0:08')");
  ...
  private void testResultSet(ResultSet resultSet)
  {
    ...
    while (resultSet.next()) {
      try { // Checks that the functions lower and upper worked.
        ...
        assertTrue(nameAux.equals(nameAux.toLowerCase()) ||
                   nameAux.equals(nameAux.toUpperCase()));
      } catch (DriverException exception) {
    ...
    resultSet.next();
    resultSet.next();
    resultSet.next(); // next() always returns the last result if it has passed
                      // the last position.
    assertEquals(1, (matrix = resultSet.getStrings()).length);
    assertBetween(4, matrix[0].length, 5);
    ...
    resultSet.next();
    resultSet.next();
    resultSet.next(); // next() always return the last result if it has passed
                      // the last position.
    assertEquals(1, (matrix = resultSet.getStrings()).length);
    assertBetween(4, matrix[0].length, 5);
    ...
    resultSet.prev();
    resultSet.prev();
    resultSet.prev(); // prev() always return the first result if it has passed
                      // the first position.
    assertEquals(4, (matrix = resultSet.getStrings()).length);
    assertBetween(4, matrix[0].length, 5);
    ...
    resultSet.beforeFirst();
    resultSet.relative(0); // relative() always set the position to the first or
                           // last record if it is in an invalid position.
    assertEquals(4, (matrix = resultSet.getStrings()).length);
    assertBetween(4, matrix[0].length, 5);
    resultSet.beforeFirst();
    ...
    assertBetween(4, matrix[0].length, 5);
    resultSet.beforeFirst();
    resultSet.relative(5); // relative() always set the position to the first or
                           // last record if it is in an invalid position.
    assertEquals(1, (matrix = resultSet.getStrings()).length);
    assertBetween(4, matrix[0].length, 5);
    ...
    assertBetween(4, matrix[0].length, 5);
    resultSet.first();
    resultSet.relative(4); // relative() always set the position to the first or
                           // last record if it is in an invalid position.
    assertEquals(1, (matrix = resultSet.getStrings()).length);
    assertBetween(4, matrix[0].length, 5);
    ...
    assertBetween(4, matrix[0].length, 5);
    resultSet.afterLast();
    resultSet.relative(-5); // relative() always set the position to the first
                            // or last record if it is in an invalid position.
    assertEquals(4, (matrix = resultSet.getStrings()).length);
    assertBetween(4, matrix[0].length, 5);
    ...
    assertBetween(4, matrix[0].length, 5);
    resultSet.last();
    resultSet.relative(-4); // relative() always set the position to the first
                            // or last record if it is in an invalid position.
    assertEquals(4, (matrix = resultSet.getStrings()).length);
    assertBetween(4, matrix[0].length, 5);
    ...
    try { // It is not possible use getStrings() with negative numbers other
          // than -1.
    ...
    resultSet.close(); // Closes the result set. All result set methods must
                       // throw an excetion on an atempt to use it,
    try {
      resultSet.absolute(1);
      fail("16");
