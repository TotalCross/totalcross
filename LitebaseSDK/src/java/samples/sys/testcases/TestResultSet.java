...
public class TestResultSet extends TestCase {
	...
	public void testRun() {
		...
		testResultSet(
			driver.executeQuery("select * from person where age > 0 order by name")); // Select with temporary table.
		testResultSet(
			driver.executeQuery("select * from person where age > 0")); // Select without temporary table.
		testResultSet(driver.executeQuery("select * from person")); // Select without temporary table.
		testResultSet(
			driver.executeQuery("select rowid, name, age, birth, years from person")); // Select without temporary table.
		testResultSet(
			driver.executeQuery("select rowid, name, age, birth, years as anos from person")); // Select without temporary table.
		testResultSet(
			driver.executeQuery("select rowid, upper(name) as nameAux, age, birth, years as anos from person")); // Select without temporary table.
		testResultSet(
			driver.executeQuery("select rowid, lower(name) as nameAux, age, birth, years as anos from person")); // Select without temporary table.
	...
	private void testResultSet(ResultSet resultSet) {
		...
		resultSet.beforeFirst();
		resultSet.relative(
			0); // relative() always set the position to the first or last record if it is in an invalid position.
		assertEquals(4, (matrix = resultSet.getStrings()).length);
		assertBetween(4, matrix[0].length, 5);
		resultSet.beforeFirst();
		...
		resultSet.beforeFirst();
		resultSet.relative(
			5); // relative() always set the position to the first or last record if it is in an invalid position.
		assertEquals(1, (matrix = resultSet.getStrings()).length);
		assertBetween(4, matrix[0].length, 5);
		...
		resultSet.first();
		resultSet.relative(
			4); // relative() always set the position to the first or last record if it is in an invalid position.
		assertEquals(1, (matrix = resultSet.getStrings()).length);
		assertBetween(4, matrix[0].length, 5);
