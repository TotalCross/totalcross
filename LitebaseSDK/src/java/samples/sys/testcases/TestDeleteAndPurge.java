...
public class TestDeleteAndPurge extends TestCase {
	public void testRun() {
		...
		try {
			assertEquals(200, driver.executeUpdate("delete CIDADE"));
					 ...
					 exception) { // guich@553_10: this error occured when the table name was not being converted to lowercase.
	...
	private void testPruebas() { // rnovais@570
		...
		ResultSet resultSet =
			driver.executeQuery("select RFS, rackID, modelo,serie ,error , descripcion from motorCatalog where (RFS=231635) and "
								+ "(RackID='16513')");
		...
		assertEquals(8, (resultSet =
							 driver.executeQuery("select RFS, rackID, modelo,serie ,error , descripcion from motorCatalog where (RFS=231635) "
