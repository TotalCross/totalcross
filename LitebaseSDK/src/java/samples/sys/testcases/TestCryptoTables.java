...
public class TestCryptoTables extends TestCase {
	...
	public void testRun() {
		...
		(conn = LitebaseConnection.getInstance("Test",
											   "path = " + tempPath)).execute("create table person (name char(10))");
		conn.closeAll();
		...
		(connAux = LitebaseConnection.getInstance("Test",
				   "crypto; path = " + tempPath)).executeUpdate("drop table person2");
		connAux.closeAll();
