...
public class TestPrimaryKey extends TestCase {
	...
	public void testRun() {
		...
		assertEquals(1,
					 driver.executeUpdate("update tabps set idade = 5, name = 'michelle' where idade = 5")); // guich@564_18
	...
	private void test_person_PK(String sql) {
		...
		assertEquals(1,
					 driver.executeUpdate("update person_pk set first_name = 'madalena' where first_name = 'maria'"));
		assertEquals(1,
					 driver.executeUpdate("update person_pk set city = 'new york' where first_name = 'madalena'"));
		try {
