...
public class TestPrimaryKey extends TestCase
{
  ...
  public void testRun()
  {
    ...
    try { // Tries to create a table with 2 primary keys.
      driver.execute(
        "create table PERSON_PK (FIRST_NAME CHAR(30) PRIMARY KEY, CITY CHAR(30), SALARY_CUR DOUBLE PRIMARY KEY, SALARY_PREV DOUBLE, "
    ...
    test_person_PK(
      "create table PERSON_PK (FIRST_NAME CHAR(30) PRIMARY KEY, CITY CHAR(30), SALARY_CUR DOUBLE, SALARY_PREV DOUBLE, "
      + "YEARS_EXP_JAVA INT, YEARS_EXP_C INT )");
    test_person_PK(
      "create table PERSON_PK (FIRST_NAME CHAR(30), CITY CHAR(30), SALARY_CUR DOUBLE, SALARY_PREV DOUBLE, YEARS_EXP_JAVA INT, "
    ...
    assertEquals(
      ...
      driver.executeUpdate(
        "update tabps set idade = 5, name = 'michelle' where idade = 5")); // guich@564_18

    // Tests that prepared statement won't create a duplicate primary key.
    PreparedStatement ps =
      driver.prepareStatement("insert into tabps values (?, ?)");
    ps.setString(0, "1");
    ps.setString(1, "1");
    ps.executeUpdate();
    ps.setInt(0, 2);
    ...
    ResultSet resultSet = driver.executeQuery(
      "select idade from tabps where name = '1' or name = '2'");
    resultSet.next();
    ...
    try { // Repeated primary key.
      driver.executeUpdate(
        "update tabps set idade = 5, name = 'michelle' where idade = 6"); // guich@564_19
      fail("2");
    ...
    // It is not possible to add a primary key to a column that has already an
    // index or columns that already compose an index in the same order of the
    // key columns.
    try {
    ...
    driver.executeUpdate(
      "alter table tabps add primary key (rowid, x)"); // This must work.
    driver.closeAll();
  }
  ...
  private void test_person_PK(String sql)
  {
    ...
    driver.executeUpdate(
      "alter table PERSON_PK drop primary key"); // Drops the existing primary
                                                 // key.
    ...
    try { // Tries to add the primary key again. It must raise an exception,
          // since now there are duplicated records.
    ...
    assertEquals(
      2,
      driver.executeUpdate("delete person_pk where first_name = 'guilherme'"));
    assertEquals(
      5, driver.executeUpdate("delete person_pk where first_name = 'maria'"));
    assertEquals(
      2, driver.executeUpdate("delete person_pk where first_name = 'paulo'"));
    assertEquals(
      3, driver.executeUpdate("delete person_pk where first_name = 'roberto'"));
    assertEquals(
      2, driver.executeUpdate("delete person_pk where first_name = 'cher'"));

    assertEquals(6, executeQuery("select * from person_pk"));
    assertEquals(6, executeQuery("select * from person_pk where 1 = 1"));

    // Tries one more time to add the primary key again. It should work, since
    // there are no duplicated records anymore.
    driver.executeUpdate("alter table person_pk add primary key(first_name)");
    ...
    // Checks UPDATE statements. The first two should pass; the third should
    // fail.
    assertEquals(
      1,
      driver.executeUpdate(
        "update person_pk set first_name = 'madalena' where first_name = 'maria'"));
    assertEquals(
      1,
      driver.executeUpdate(
        "update person_pk set city = 'new york' where first_name = 'madalena'"));
    try {
      driver.executeUpdate(
        "update person_pk set first_name = 'zanata' where first_name = 'madalena'");
      fail("12");
    } catch (PrimaryKeyViolationException exception) {
    }
  }
