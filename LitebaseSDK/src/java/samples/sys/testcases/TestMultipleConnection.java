...
/**
 * Tests Litebase with multiple connections, to access different databases at
 * the same time.
 */
public class TestMultipleConnection extends TestCase
{
  ...
  private void TestConnections(String tempPath)
  {
    ...
    LitebaseConnection driver1 =
      AllTests.getInstance("Test",
                           dataPath); // The first connection is on datapath.
    ...
    LitebaseConnection driver2 =
      AllTests.getInstance("Test",
                           tempPath); // The second connection is on tempPath.
    ...
    // After creating the table in the first connection, only this one can
    // exists.
    ...
    assertEquals(1,
                 driver1.executeUpdate("insert into person values (0, '0')"));
    assertEquals(
      1, driver2.executeUpdate("insert into person values (1, '1', 1)"));
    ...
    // Inserts the row of the table of the first connection in the one of the
    // second connection.
    assertEquals(1,
                 driver2.executeUpdate("insert into person values('" +
                                       rs1.getString("id") + "','" +
                                       rs1.getString("name") + "', 2)"));
