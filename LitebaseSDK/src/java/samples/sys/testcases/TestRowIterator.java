...
public class TestRowIterator extends TestCase
{
  ...
  public void testRun()
  {
    ...
    int i = -1, updated = 0, deleted = 0, newed = 0, synced = 0;
    ...
    try { // Creates the table.
      driver.execute(
        "CREATE TABLE PERSON (NAME CHAR(500) NOCASE, ADDRESS char(700), id int, age short, cpf long, salary float, "
    ...
    PreparedStatement ps = driver.prepareStatement(
      "INSERT INTO person VALUES (?, ?, ?, ?, ?, ?, ?, '2005/9-12', '2006/08-21 12:08:01:234', ?)");
    ...
    while (it.next()) {
      ...
      if (5 <= i &&
          i <= 15) { // Simulates that these rows have been synchronized.
        it.setSynced();
      }
    }
    ...
    driver.executeUpdate(
      "update person set name ='guilherme' where name like '%1'");
    ...
    while (it.next()) {                   // Iterates through the iterator.
      ...
      assertTrue(it.attr == RowIterator.ROW_ATTR_UPDATED ||
                 it.attr == RowIterator.ROW_ATTR_NEW);
    }
