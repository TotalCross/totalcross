...
public class TestSourcePath extends TestCase
{
  ...
  private void doTest(String tempPath)
  {
    try {
      ...
      try {
        driver.execute(
          "CREATE TABLE twonames (name1 CHAR(100), name2 CHAR(100))");
      ...
      assertEquals(1,
                   driver.executeUpdate(
                     "Insert into twonames values ('guich','michelle\\\\')"));
      ...
      File f;
      if (!(f = new File(tempPath + "Test-twonames.db", File.DONT_OPEN))
             .exists()) {
        fail("File doesn't exist. " + f.getPath());
      }
      if (!(f = new File(tempPath + "Test-twonames.dbo", File.DONT_OPEN))
             .exists()) {
        fail("File doesn't exist. " + f.getPath());
      }
      if (!(f = new File(tempPath + "Test-twonames$1.idk", File.DONT_OPEN))
             .exists()) {
        fail("File doesn't exist. " + f.getPath());
      }
  ...
  public void testRun()
  {
    ...
    // Tests that the an exception will be thrown if a relative path is used on
    // the device.
    try {
      Settings.dataPath = ".";
