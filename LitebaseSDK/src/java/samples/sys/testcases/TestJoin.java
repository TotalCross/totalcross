...
public class TestJoin extends TestCase
{
  ...
  public void testRun()
  {
    driver = AllTests.getInstance(
      "Test");         // First creates the connection with Litebase.
    testJoinSintax();  // Tests the join sintax.
    testSimpleJoins(); // Tests simple joins.
    testComparisons(); // Tests comparison between fields, including null values
                       // and functions.
    ...
    testComparisonsWithIndices();   // Tests comparison between fields, using
                                    // index.
    testPrimaryKeyAndOrdering();    // Tests join with primary key, clause and
                                    // table orders.
    testComparisonInTheSameTable(); // Tests comparison of columns of the same
                                    // table.
    testOrWithFalseConstantComparison(); // Tests join with or and false
  ...
  /**
   * Tests the join sintax. All sql commands with parser error must throw an
   * exception.
   */
  private void testJoinSintax()
  {
    ...
    // Create tables.
    driver.execute(
      "create table person(person_id int primary key, name char(20) default 'Est� sem nome: �^~' not null, age int default 10, "
      + "birth Date default '1981/06/06', years DateTime not null)");
    driver.execute(
      "create table course (name char(20) primary key, person_id int, address char(20))");

    runSelectWithParserError(
      "select * from person p where person.person_id = 1"); // The alias must
                                                            // be used in the
                                                            // where clause.
    runSelectWithParserError(
      "select p.name, person.age from person p"); // The alias must be used if
                                                  // the table has it.

    // The alias must be used in the order by clause.
    runSelectWithParserError(
      "select max(t.person_id) as maxname from course t where t.name = '1' order by course.name ");

    // The alias must be used in the group by clause.
    runSelectWithParserError(
      "select max(t.person_id) as maxname from course t where t.name = '1' group by course.name ");

    // An alias can't be used in the where clause.
    runSelectWithParserError(
      "select max(t.person_id) as maxname, t.name as tn from course t where tn = '1' group by t.name "
      + "having maxname = 'sem nome'");

    runSelectWithParserError(
      "select p.name, p.age from person p where p.xxxx = 1"); // Unknown
                                                              // column xxxx.
    runSelectWithParserError(
      "select name from person, course"); // Ambigous column name.
    runSelectWithParserError(
      "select name as v, person_id as v from person, course"); // Repeated
                                                               // alias.
    runSelectWithParserError(
      "SELECT * FROM person course, course"); // person can't use course as
                                              // alias without giving an alias
                                              // to course.
    runSelectWithParserError(
      "SELECT course.address FROM person course, course person"); // Course is
                                                                  // an alias
                                                                  // for
                                                                  // person.
    ...
    runSelectWithParserError(
      "SELECT * FROM person course, course person where person.address = '' and person.age = 9");

    runSelectWithParserError(
      "SELECT * FROM course, person order by year"); // order by field doesn't
                                                     // exist.

    // c.name is not in the select clause.
    runSelectWithParserError(
      "select max(c.person_id) as maxname, p.name, p.birth, c.address, p.person_id from course c, person p "
      + "where c.name = '1' order by c.name");

    try { // Select sql without sintax errors.
      runSelectWithParserError(
        "SELECT distinct * FROM person course, course person where course.age = 10 and person.address is not null");
      fail();
  ...
  private void testSimpleJoins()
  {
    ...
    ResultSet rs = driver.executeQuery(
      "select name, name2, age, age2 from person, person2 where 'RLN' = person.name order by name2");
    assertNotNull(rs);
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from person, person2  where age2 = 40 and name = 'RLN'"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from person, person2  where name = 'RLN' and person2.age2 = 40 "));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("IAPU", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from person, person2  where person.name = 'RLN' or person2.age2 = 40 "));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from person, person2  where age2 = 40 or person.name = 'RLN'"));
    assertEquals(3, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2 from person, person2 where person.name = person2.name2 and (person.name = 'RLN' or person2.name2 = 'RLN')"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from person, person2  where not age2 <> 40 or not person.name <> 'RLN'"));
    assertEquals(3, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from  person,person2 where not years = 1 or not age2 = 30 "
        + "order by age2"));
    assertEquals(3, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("IOG", rs.getString("name"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from person2,person where name = person2.name2"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
  ...
  private void testComparisons()
  {
    ...
    ResultSet rs = driver.executeQuery(
      "select name, name2, age, age2 from person2, person where lower(person.name) = person2.name2");
    assertNotNull(rs);
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from person2, person where person.age = person2.age2"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("IOG", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
    ...
    driver.executeUpdate(
      "update person2 p set p.age2 = 30 where name2 = 'rln'");
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from person2, person where person.age = person2.age2 or name = name2"));
    assertEquals(3, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, name2, age, age2 from person2, person where person.age = person2.age2 and name = name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person2, person where age is null and years is null and age2 is null"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    ...
    assertNotNull(
      rs = driver.executeQuery("select count(*) as tt from person, person2"));
    assertTrue(rs.next());
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select count(*) as tt from person, person2 where name = 'IND'"));
    assertTrue(rs.next());
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, count(*) as tt from person, person2 group by name"));
    assertEquals(3, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("IND", rs.getString("name"));
    assertEquals(9, rs.getInt("tt"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select name, count(*) as tt from person, person2 group by name having tt > 3"));
    assertEquals(2, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("IND", rs.getString("name"));
    assertEquals(9, rs.getInt("tt"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select age2, lower(name) as tt, age, upper(name2) as rr from person, person2 where name = name2 "
        + "order by age"));
    assertEquals(2, rs.getRowCount());
    assertTrue(rs.next());
  ...
  private void test3TablesJoin()
  {
    ...
    driver.executeUpdate(
      "Insert INTO person3(years3, name3) values (50,'RLN')");
    driver.executeUpdate(
      "INSERT into person3(name3, years3) values ('ZN', 60)");
    ...
    driver.executeUpdate(
      "Insert into person3(name3, years3) values ('ZN', 50)");
    driver.executeUpdate("Insert INTO person3(name3, age3) values ('JN', 75)");

    ResultSet rs =
      driver.executeQuery("select * from person, person2, person3");
    assertNotNull(rs);
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2, person3 where name = name2"));
    assertEquals(32, rs.getRowCount());
    rs.close();

    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2, person3 where name = name2 and person3.age3 is null"));
    assertEquals(12, rs.getRowCount());
    rs.close();

    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2, person3 where name = name2 and person3.age3 is null and name3 = 'IOG' "
        + "and age = 10"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
  ...
  private void testRSColName()
  {
    ...
    assertEquals(
      4,
      (rs = driver.executeQuery(
         "select person.name as tt, age, years as bb, person2.name, person2.age2 from person, person2"))
        .getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString(1));
    assertEquals("20", rs.getString(2));
    ...
    assertEquals((rs = driver.executeQuery(
                    "select person.name, person2.name from person, person2"))
                   .getRowCount(),
                 4);
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString(1));
  ...
  private void testJoinWithIndex()
  {
    ...
    driver.execute("create index idx on person(name)"); // Creates index for
                                                        // column person.name.
  ...
  private void testIndexOnAge()
  {
    ...
    ResultSet rs =
      driver.executeQuery("select * from person,person2 where name = 'RLN'");
    assertNotNull(rs);
    assertEquals(3, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person,person2 where name = 'RLN' and age = 40"));
    assertEquals(3, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person,person2 where name = 'RLN' or age = 40"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person,person2 where name = 'RLN2' and age = 40"));
    assertEquals(3, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN2", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
  ...
  {
    ResultSet rs = driver.executeQuery(
      "select * from person, person2 where name = 'RLN' and name2 = 'rln'");
    assertNotNull(rs);
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2 where name2 = 'rln' and name = 'RLN'"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2 where name = 'RLN' or name2 = 'rln'"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2 where name2 = 'rln' or name = 'RLN'"));
    assertEquals(5, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2 where (name = 'RLN' or name2 = 'rln') or name = 'RLN2'"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2 where (name = 'RLN' or name2 = 'rln') and name = 'RLN2'"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN2", rs.getString("name"));
    assertEquals("rln", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2 where name = 'RLN2' and (name = 'RLN' or name2 = 'rln')"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN2", rs.getString("name"));
    assertEquals("rln", rs.getString("name2"));
    ...
    assertNotNull(rs = driver.executeQuery(
                    "select * from person, person2 where name = name2"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
  ...
  private void testComparisonsWithIndices()
  {
    ...
    ResultSet rs =
      driver.executeQuery("select * from person, person2 where name = name2");
    assertNotNull(rs);
    assertEquals(3, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2 where name = name2 and name = 'IOG'"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("IOG", rs.getString("name"));
    assertEquals("IOG", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2 where name = name2 and age = age2"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "select * from person, person2 where age = age2 and name = name2"));
    assertEquals(1, rs.getRowCount());
    assertTrue(rs.next());
    assertEquals("RLN", rs.getString("name"));
    assertEquals("RLN", rs.getString("name2"));
    ...
    testTableOrder(
      "select * from person, person2, person3 where name = name2 and name = name3");
    testTableOrder(
      "select * from person, person3, person2 where name = name2 and name = name3");
    testTableOrder(
      "select * from person2, person3, person where name = name2 and name = name3");
    testTableOrder(
      "select * from person2, person, person3 where name = name2 and name = name3");
    testTableOrder(
      "select * from person3, person2, person where name = name2 and name = name3");
    testTableOrder(
      "select * from person3, person, person2 where name = name2 and name = name3");
  }
  ...
  private void testPrimaryKeyAndOrdering()
  {
    ...
    // Creates tables.
    driver.execute(
      "create table person (id long primary key, lastname char(20), firstname char(20))");
    driver.execute(
      "create table course (id long primary key, personid long, cname char(20))");

    // Populates the tables
    PreparedStatement ptInsert = driver.prepareStatement(
      "insert into person (id,lastname,firstname) values (?,?,?)");
    PreparedStatement acctInsert = driver.prepareStatement(
      "insert into course (id,personid,cname) values (?,?,?)");
    int i = 101;
    while (--i > 0) {
    ...
    ResultSet rs =
      driver.executeQuery("select person.id from person where person.id = 50");
    assertEquals(1, rs.getRowCount());
    rs.close();

    // All the joins must return only one record because only one person has
    // id 50.
    assertEquals(1,
                 (rs = driver.executeQuery(
                    "select course.id from course where course.personid = 50"))
                   .getRowCount());
    rs.close();
    ...
    assertEquals(
      1,
      (rs = driver.executeQuery(
         "select person.id, course.id from person,course where person.id = course.personid and person.id = 50"))
        .getRowCount());
    rs.next();
    ...
    assertEquals(
      1,
      (rs = driver.executeQuery(
         "select person.id, course.id from course, person where person.id = course.personid and person.id = 50"))
        .getRowCount());
    rs.next();
    assertEquals(50, rs.getLong(1));
    assertEquals(150, rs.getLong(2));
    ...
    assertEquals(
      1,
      (rs = driver.executeQuery(
         "select person.id, course.id from person,course where course.personid = person.id and person.id = 50"))
        .getRowCount());
    rs.next();
    assertEquals(50, rs.getLong(1));
    assertEquals(150, rs.getLong(2));
    ...
    assertEquals(
      1,
      (rs = driver.executeQuery(
         "select person.id, course.id from course, person where course.personid = person.id and person.id = 50"))
        .getRowCount());
    rs.next();
    assertEquals(50, rs.getLong(1));
    assertEquals(150, rs.getLong(2));
    ...
    assertEquals(
      1,
      (rs = driver.executeQuery(
         "select person.id, course.id from person,course where person.id = 50 and course.personid = person.id"))
        .getRowCount());
    rs.next();
    assertEquals(50, rs.getLong(1));
    assertEquals(150, rs.getLong(2));
    ...
    assertEquals(
      1,
      (rs = driver.executeQuery(
         "select person.id, course.id from course, person where person.id = 50 and course.personid = person.id"))
        .getRowCount());
    rs.next();
    assertEquals(50, rs.getLong(1));
    assertEquals(150, rs.getLong(2));
  ...
  void testComparisonInTheSameTable()
  {
    ...
    driverAux.execute(
      "create table M_META_GRUPO_MATERIAL (GMF_CODIGO long NOT NULL, QTD_META double, QTD_CONSUMIDA double)");
    driverAux.executeUpdate(
      "insert into M_META_GRUPO_MATERIAL values (1, 500.6578844, 0)");
    driverAux.executeUpdate(
      "insert into M_META_GRUPO_MATERIAL values (1, 350.50, 350.50)");

    ResultSet resultSet = driver.executeQuery(
      "select * from M_META_GRUPO_MATERIAL MGM where MGM.QTD_CONSUMIDA = MGM.QTD_META");
    ...
    driverAux.execute(
      "create table M_MATERIAL (CODIGO_MATERIAL int NOT NULL, GMF_CODIGO long, primary key(CODIGO_MATERIAL))");
    ...
    assertEquals(
      2,
      (resultSet = driverAux.executeQuery(
         "select * from M_MATERIAL MM, M_META_GRUPO_MATERIAL MGM where MM.GMF_CODIGO=MGM.GMF_CODIGO and MM.CODIGO_MATERIAL=1"))
        .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      1,
      (resultSet = driverAux.executeQuery(
         "select * from M_MATERIAL MM, M_META_GRUPO_MATERIAL MGM where MM.GMF_CODIGO=MGM.GMF_CODIGO and MM.CODIGO_MATERIAL=1 and MGM.QTD_CONSUMIDA = MGM.QTD_META"))
        .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      1,
      (resultSet = driverAux.executeQuery(
         "select * from M_MATERIAL MM, M_META_GRUPO_MATERIAL MGM where MM.GMF_CODIGO=MGM.GMF_CODIGO and MM.CODIGO_MATERIAL=1 and MGM.QTD_CONSUMIDA = 350.5"))
        .getRowCount());
    resultSet.close();
  }
  ...
  private void testOrWithFalseConstantComparison()
  {
    ...
    // Creates the tables.
    driverAux.execute(
      "create table tbpedido (cdpedido short, totalpedido double, cdcliente short)");
    driverAux.execute(
      "create table tbcliente (cdcliente short, nmcliente char(10), flativo char(1))");
    ...
    // Normal select.
    ResultSet resultSet = driverAux.executeQuery(
      "select * from tbpedido ped, tbcliente cli where cli.cdcliente = ped.cdcliente and (ped.cdcliente = 1)");
    ...
    assertEquals(
      2,
      (resultSet = driverAux.executeQuery(
         "select * from tbpedido ped, tbcliente cli where cli.cdcliente = ped.cdcliente and "
         + "(ped.cdcliente = 1 or '1' = '0')"))
        .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      2,
      (resultSet = driverAux.executeQuery(
         "select * from tbpedido ped, tbcliente cli where (ped.cdcliente = 1 or '1' = '0') and "
         + "cli.cdcliente = ped.cdcliente"))
        .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      2,
      (resultSet = driverAux.executeQuery(
         "select * from tbpedido ped, tbcliente cli where ('1' = '0' or ped.cdcliente = 1) and "
         + "cli.cdcliente = ped.cdcliente"))
        .getRowCount());
    resultSet.close();
  }
  ...
  void testOrderGroupBy()
  {
    ...
    // Creates the tables.
    driverAux.execute(
      "CREATE TABLE PERGUNTA (IDPERGUNTA LONG PRIMARY KEY, DESCRICAO VARCHAR(115) NOT NULL, SITUACAO INT NOT NULL, "
      + "FOTO INT NOT NULL)");
    driverAux.execute(
      "CREATE TABLE ASSGRUPOPERGUNTA(IDASSGRUPOPERGUNTA LONG PRIMARY KEY, IDPERGUNTA LONG NOT NULL, "
      +
      "IDGRUPOPERGUNTA LONG NOT NULL, ORDEM INT NOT NULL, SITUACAO INT NOT NULL)");
    driverAux.execute(
      "CREATE TABLE ASSGRPPERGTITULOCHK(IDASSGRPPERGTITULOCHK LONG PRIMARY KEY, IDFORMULARIO LONG NOT NULL, "
      + "IDASSGRUPOPERGUNTA LONG NOT NULL, SITUACAO INT NOT NULL)");

    // Populates the tables.
    driverAux.executeUpdate(
      "insert into pergunta values (57, 'Desligar todas as fontes de tensao', 1, 0)");
    driverAux.executeUpdate(
      "insert into ASSGRUPOPERGUNTA values (262, 57, 14, 1, 1)");
    driverAux.executeUpdate(
      "insert into ASSGRPPERGTITULOCHK values (441, 4, 262, 1)");

    // The joins.
    ResultSet resultSet = driverAux.executeQuery(
      "select ASS.IDASSGRUPOPERGUNTA, P.IDPERGUNTA, P.DESCRICAO, P.FOTO from PERGUNTA P, "
      +
    ...
    assertEquals(
      ...
      (resultSet = driverAux.executeQuery(
         ...
         "ASSGRUPOPERGUNTA ASS, ASSGRPPERGTITULOCHK ASS1 where ASS.IDGRUPOPERGUNTA=14 AND ASS1.IDFORMULARIO = 4 AND ASS.SITUACAO = 1 AND ASS1.SITUACAO = 1 "
         +
         "AND P.SITUACAO = 1 AND ASS.IDPERGUNTA = P.IDPERGUNTA AND ASS1.IDASSGRUPOPERGUNTA = ASS.IDASSGRUPOPERGUNTA group by ASS.IDASSGRUPOPERGUNTA, P.IDPERGUNTA, P.DESCRICAO, P.FOTO"))
        .getRowCount());
    resultSet.close();
  }
}