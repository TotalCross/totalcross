...
/**
 * Tests Litebase methods with invalid arguments. This is necessary to test
 * control conditions.
 */
public class TestInvalidArguments extends TestCase
{
  ...
  public void testRun()
  {
    ...
    testNullString(
      driver); // Tests null strings passed to LitebaseConnection methods.
    testEmptyString(
      driver); // Tests empty strings passed to LitebaseConnection methods.
    testLexicalError(driver); // Tests sql commands with lexical errors.
    testDuplicatedColumn(
      driver); // Tests table creations with duplicated column names.
    testLongTableName(driver); // Tests too long table names.
    testTooManyColumns(
      driver); // Tests the use of tables with too many columns.
    testInvalidCreateIndex(
      driver);               // Tests a create index on a non-existing column.
    testWrongSyntax(driver); // Tests some syntaticaly wrong sql commands.
    testNonExistingTable(
      driver); // Tests Litebase commands that use non-existing tables.
    testNonExistingColumn(
      driver); // Tests Litebase SQLs that use unknown columns.
    testWrongInsertUpdate(driver); // Tests wrong inserts and updates.
    testTooLargeNumbers(
      driver); // Tests some large numbers greater than the declared type.
    testWrongNumberTypesInWhere(
      driver); // Tests bigger strings and wrong types in the where clause.
  ...
  {
    assertFalse(
      driver.exists("")); // juliana@201_31: an empty table can't exist.
  ...
  {
    try { // It is possible to create a table with the maximum table name
          // length.
      if (driver.exists("R1234567891234567890123")) {
        driver.executeUpdate("drop table R1234567891234567890123");
      }
      driver.execute(
        "create table R1234567891234567890123 (FIRST_NAME CHAR(30) PRIMARY KEY, CITY CHAR(30) )");
    } catch (SQLParseException exception) {
      fail("47");
    }

    // rnovais@570_114: Tries to create tables with a name longer than the max
    // size (23).
    try { // Tries to create one with (max size + 1) = 24 characters.
      driver.execute(
        "create table R12345678912345678901234 (FIRST_NAME CHAR(30) PRIMARY KEY, CITY CHAR(30) )");
      fail("48");
    } catch (SQLParseException exception) {
    }
    ...
    try { // Tries to create one with 41 characters.
      driver.execute(
        "create table R12345678901234567890123456789012345678901234567890 (FIRST_NAME CHAR(30) PRIMARY KEY, CITY CHAR(30) )");
      fail("49");
    } catch (SQLParseException exception) {
    }
    ...
    try { // It is possible to use a table with max length.
      driver.executeUpdate(
        "insert into R1234567891234567890123 values('juliana', 'Rio de Janeiro')");
    } catch (SQLParseException exception) {
      fail("51");
    }
    ...
    try { // Tries to do an insert in a table with 41 characters.
      driver.executeUpdate(
        "insert into R12345678901234567890123456789012345678901234567890 values('juliana', 'Rio de Janeiro')");
      fail("53");
    } catch (DriverException exception) {
    }
    ...
    try { // Tries to do a select in a table with 51 characters.
      driver.executeQuery(
        "select * from R123456789012345678901234567890123456789012345678901234567890");
      fail("54");
    } catch (DriverException exception) {
    }
  }
  ...
  private void testTooManyColumns(LitebaseConnection driver)
  {
    ...
    try { // Tries to create a table with too many columns in the composed
          // primary key.
      driver.execute(sBuffer.toString());
      fail("65");
    } catch (ArrayIndexOutOfBoundsException exception) {
    ...
    driver.execute(
      "create table person (q0 int, q1 int, q2 int, q3 int, q4 int, w0 int, w1 int, w2 int, w3 int, w4 int,"
      +
  ...
  private void testInvalidCreateIndex(LitebaseConnection driver)
  {
    ...
    driver.execute(
      "create table person(id int primary key, lastname char(20), firstname char(20), age int, dept int)");
    try {
  ...
  private void testNonExistingColumn(LitebaseConnection driver)
  {
    ...
    driver.execute(
      "create table employee(id int, lastname char(20), firstname char(20), age int, dept int)");
    ...
    try {
      driver.executeUpdate(
        "insert into employee (id,lastname,firstname,years,dept,nocolumn) values(1,'lname1','fname1',1,1,'nocolumn')");
      fail("105");
    } catch (SQLParseException exception) {
    }
    try {
    ...
    try {
      driver.executeUpdate(
        "update employee set middlename='updated' where id = 99");
      fail("107");
    } catch (SQLParseException exception) {
    }
    try {
  ...
  private void testTooLargeNumbers(LitebaseConnection driver)
  {
    ...
    driver.execute(
      "create table bignumbers (s short, i int, l long, c char(1))");
    ...
    try {
      driver.executeQuery(
        "select * from bignumbers where l = 9223372036854775808");
      fail("113");
    } catch (SQLParseException exception) {
    }
    try {
      driver.executeQuery(
        "select * from bignumbers where l = -9223372036854775809");
      fail("114");
    } catch (SQLParseException exception) {
    }
    ...
    driver.executeQuery("select * from bignumbers where i = +2147483647")
      .close();
    driver.executeQuery("select * from bignumbers where i = -2147483648")
      .close();
    driver
      .executeQuery("select * from bignumbers where l = +9223372036854775807")
      .close();
    driver
      .executeQuery("select * from bignumbers where l = -9223372036854775808")
      .close();
    ...
    try {
      driver.executeUpdate(
        "insert into bignumbers (l) values (9223372036854775808)");
      fail("119");
    } catch (SQLParseException exception) {
    }
    try {
      driver.executeUpdate(
        "insert into bignumbers (l) values (-9223372036854775809)");
      fail("120");
    } catch (SQLParseException exception) {
    }
    ...
    // Valid numeric values.
    driver.executeUpdate(
      "insert into bignumbers values (+32767, +2147483647, +9223372036854775807, 'j')");
    driver.executeUpdate(
      "insert into bignumbers values (-32768, -2147483648, -9223372036854775808, 'i')");
  ...
  private void testWrongNumberTypesInWhere(LitebaseConnection driver)
  {
    ...
    PreparedStatement prepStmt =
      driver.prepareStatement("select * from bignumbers where s = ?");
    try {
      prepStmt.setInt(0, 0);
      fail("121");
    } catch (DriverException exception) {
    ...
    driver
      .executeQuery(
        "select * from bignumbers where c = 'Juliana Carpes Imperial'")
      .close();
  }
  ...
  private void testWrongInsertUpdate(LitebaseConnection driver)
  {
    try {
      driver.executeUpdate(
        "insert into employee (id,lastname) values (101,'lname101','fname101',101,1)");
      fail("124");
    } catch (SQLParseException exception) {
    }
    try {
      driver.executeUpdate(
        "insert into employee (id,lastname) values ('lname101', 101)");
      fail("125");
    } catch (SQLParseException exception) {
    }
    ...
    try {
      driver.executeUpdate(
        "update employee set id = 'lname101', lastname = 101)");
      fail("127");
    } catch (SQLParseException exception) {
    }
    try {
      driver.executeUpdate(
        "update employee set id = 'lname101', id = 'lname101', id = 'lname101', id = 'lname101', id = 'lname101', id = 'lname101'");
      fail("128");
    } catch (SQLParseException exception) {
    }
  }
  ...
  private void testInvalidRowidAlter(LitebaseConnection driver)
  {
    try {
      driver.executeUpdate(
        "insert into employee (id, rowid, lastname) values (1, 2, 'imperial')");
      fail("129");
    } catch (SQLParseException exception) {
    }
    try {
      driver.executeUpdate(
        "update employee set id = 1, rowid = 2, lastname = 'imperial'");
      fail("130");
    } catch (SQLParseException exception) {
    }
  }
  ...
  private void testLongPath()
  {
    ...
    LitebaseConnection driver =
      LitebaseConnection.getInstance("Test", path = sBuffer.toString());
    try {
