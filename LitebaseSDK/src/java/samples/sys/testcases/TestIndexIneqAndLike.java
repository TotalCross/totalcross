...
public class TestIndexIneqAndLike extends TestCase
{
  ...
  public void testRun()
  {
    ...
    PreparedStatement ps =
      driver.prepareStatement("insert into cliente values (?,?)");
    ...
    // Select using like.
    testResult(
      driver.executeQuery("select * from cliente where nome like 'nome2%'"),
      10);
    testResult(
      driver.executeQuery("select * from cliente where nome like 'Nome1%'"),
      28);

    // Select using not not like.
    testResult(driver.executeQuery(
                 "select * from cliente where not nome not like 'nome2%'"),
               10);
    testResult(driver.executeQuery(
                 "select * from cliente where not nome not like 'Nome1%'"),
               28);

    // Select using inequalities.
    testResult(driver.executeQuery(
                 "select * from cliente where codigo >= 26 and codigo <= 36"),
               10);
    testResult(
      driver.executeQuery(
        "select * from cliente where not codigo < 26 and not codigo > 36"),
      10);
    testResult(
      driver.executeQuery(
        "select * from cliente where not (codigo < 26 or codigo > 36)"),
      10);
    testResult(driver.executeQuery(
                 "select * from cliente where codigo >  26 and codigo <= 36"),
               9);
    ...
    testResult(driver.executeQuery(
                 "select * from cliente where codigo >  26 and codigo <  36"),
               8);
    testResult(driver.executeQuery("select * from cliente where codigo >= 26"),
               85);
    testResult(driver.executeQuery("select * from cliente where codigo > 26"),
               84);
    testResult(driver.executeQuery("select * from cliente where codigo <= 26"),
    ...
    testResult(
      driver.executeQuery("select * from cliente where nome < 'Nome20'"), 29);
