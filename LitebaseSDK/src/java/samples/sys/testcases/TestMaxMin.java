...
public class TestMaxMin extends TestCase {
	...
	private void testEmptyMaxMin(LitebaseConnection connection) {
		ResultSet resultSet =
			connection.executeQuery("select max(name) as maxn, min(name) as minn, max(cpf) as maxc, min(cpf) as minc from person");
		assertEquals(0, resultSet.getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select max(name) as maxn, min(name) as minn, max(cpf) as maxc, min(cpf) as minc "
		...
		assertEquals(0, (resultSet =
							 connection.executeQuery("select count(*) as c, avg(rowid) as a, max(name) as maxn, min(name) as minn, "
		...
		// Tests datetime.
		assertEquals(0, (resultSet =
							 connection.executeQuery("select max(birth) as maxb, min(birth) as mimb from person")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select max(birth) as maxb, min(birth) as mimb from person where birth is null")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select max(birth) as maxb, min(birth) as mimb from person where birth is not null")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select max(birth) as maxb, min(birth) as mimb from person where birth > '2011/07/19'")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select max(birth) as maxb, min(birth) as mimb from person where birth < '2011/07/20'")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select max(birth) as maxb, min(birth) as mimb from person where birth like '2011/07/19%'")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select max(birth) as maxb, min(birth) as mimb from person where birth like '2011/07/18%'")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select max(id) as maxi, min(id) as mini from person")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();
	}
