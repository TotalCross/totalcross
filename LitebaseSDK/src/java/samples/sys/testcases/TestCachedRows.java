 ...
 */
public class TestCachedRows extends TestCase
{ // Catchs error found in 555_3
  /**
  ...
  public void testRun()
  {
    ...
    if (driver.exists("act_cliente")) {
      driver.executeUpdate("drop table act_cliente");
    }
    driver.execute(
      "create table act_cliente (actclienteid long, actcodcliente char(30), actrazaosocial char(100), "
      +
    ...
    while (++i < numSQL)
      assertEquals(
        ...
        driver.executeUpdate(
          querysSQL[i])); // Delete must return 0; insert must return 1.
    ...
    ResultSet rs =
      (driver = AllTests.getInstance("Test"))
        .executeQuery(
          "select actrazaosocial from act_cliente where actrazaosocial='ADENILZA xxxxxxxxxxxxxxxxxxxxxE'");
    assertEquals(1, rs.getRowCount()); // Would be 6 without 555_3 fix.
    rs.close();
    assertEquals(
      10,
      (rs = driver.executeQuery("select * from act_cliente")).getRowCount());
    rs.close();
    driver.closeAll();
  }
}
