...
public class TestOrderByIndices extends TestCase {
	...
	private void testEmptyOrderBy(LitebaseConnection connection) {
		...
		assertEquals(0, (resultSet =
							 connection.executeQuery("select cpf from person order by cpf")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select birth from person order by birth")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select name, cpf from person order by name, cpf")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select cpf, name from person order by cpf, name")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select birth, rowid from person order by birth, rowid")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select rowid, birth from person order by rowid, birth")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select name from person where name = 'name0' group by name")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select cpf from person where cpf > 'cpf0' group by cpf")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals(0, (resultSet =
							 connection.executeQuery("select birth from person where birth != '2011/07/19' group by birth")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();
		...
		assertEquals(0, (resultSet =
							 connection.executeQuery("select birth, rowid from person where rowid > 0 group by rowid, birth")).getRowCount());
		assertFalse(resultSet.next());
		resultSet.close();
	}
	...
	private void testOrderBy(LitebaseConnection connection) {
		...
		assertTrue((count = (resultSet =
								 connection.executeQuery("select cpf from person order by cpf")).getRowCount()) == 2000
				   || count == 4000);
		testSortAsc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select birth from person order by birth")).getRowCount()) == 2000
				   || count == 4000);
		testSortAsc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select name, cpf from person order by name, cpf"))
							.getRowCount()) == 2000 || count == 4000);
		testSortAsc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select cpf, name from person order by cpf, name"))
							.getRowCount()) == 2000 || count == 4000);
		testSortAsc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select birth, rowid from person order by birth, rowid"))
							.getRowCount()) == 2000 || count == 4000);
		testSortAsc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select rowid, birth from person order by rowid, birth"))
							.getRowCount()) == 2000 || count == 4000);
		testSortAsc(resultSet);
		...
		assertEquals((resultSet =
						  connection.executeQuery("select birth from person where birth != '2011/07/19' group by birth"))
					 .getRowCount(), 1999);
		testSortAsc(resultSet);

		assertEquals((resultSet =
						  connection.executeQuery("select name, cpf from person where name = 'name0' and cpf = 'cpf1999' group by name, cpf"))
					 .getRowCount(), 1);
		testSortAsc(resultSet);

		assertEquals((resultSet =
						  connection.executeQuery("select cpf, name from person where name = 'name0' or cpf = 'cpf0' group by cpf, name"))
					 .getRowCount(), 2);
		testSortAsc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select birth, rowid from person where birth like '2011/0%' group by birth, rowid"))
							.getRowCount()) == 2000 || count == 4000);
		testSortAsc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select birth, rowid from person where rowid > 0 group by rowid, birth"))
							.getRowCount()) == 2000 || count == 4000);
		testSortAsc(resultSet);

		assertEquals((resultSet =
						  connection.executeQuery("select rowid from person where rowid = 0 group by rowid"))
					 .getRowCount(), 0);
		assertFalse(resultSet.next());
		resultSet.close();
	}
	...
	private void testOrderByWithDelete(LitebaseConnection connection) {
		...
		assertTrue((count = (resultSet =
								 connection.executeQuery("select cpf from person order by cpf desc"))
							.getRowCount()) == 1996 || count == 3992);
		testSortDesc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select birth from person order by birth desc"))
							.getRowCount()) == 1996 || count == 3992);
		testSortDesc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select name, cpf from person order by name desc, cpf desc"))
							.getRowCount()) == 1996 || count == 3992);
		testSortDesc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select cpf, name from person order by cpf desc, name desc"))
							.getRowCount()) == 1996 || count == 3992);
		testSortDesc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select birth, rowid from person order by birth desc, rowid desc"))
							.getRowCount()) == 1996 || count == 3992);
		testSortDesc(resultSet);

		assertTrue((count = (resultSet =
								 connection.executeQuery("select rowid, birth from person order by rowid desc, birth desc"))
							.getRowCount()) == 1996 || count == 3992);
		testSortDesc(resultSet);

		assertEquals((resultSet =
						  connection.executeQuery("select name from person where name = 'name1' group by name order by name desc"))
					 .getRowCount(), 1);
		testSortDesc(resultSet);

		assertEquals((resultSet =
						  connection.executeQuery("select cpf from person where cpf > 'cpf1' group by cpf order by cpf desc"))
					 .getRowCount(), 1995);
		testSortDesc(resultSet);

		assertEquals((resultSet =
						  connection.executeQuery("select birth from person where birth != '2011/07/19' group by birth order by birth desc"))
					 .getRowCount(), 1996);
		testSortDesc(resultSet);

		assertEquals((resultSet = connection.executeQuery(
									  "select name, cpf from person where name = 'name0' and cpf = 'cpf1999' group by name, cpf order by name desc, cpf desc")).getRowCount(),
					 0);
		assertFalse(resultSet.next());
		resultSet.close();

		assertEquals((resultSet = connection.executeQuery(
									  "select cpf, name from person where name = 'name1' or cpf = 'cpf1' group by cpf, name order by cpf desc, name desc")).getRowCount(),
					 2);
		testSortDesc(resultSet);
		...
		assertEquals((resultSet =
						  connection.executeQuery("select rowid from person where rowid = 1 group by rowid order by rowid desc"))
					 .getRowCount(), 0);
		assertFalse(resultSet.next());
		resultSet.close();
	}
