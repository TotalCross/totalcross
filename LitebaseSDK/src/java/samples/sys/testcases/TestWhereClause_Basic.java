...
public class TestWhereClause_Basic extends TestCase {
	...
	public void testRun() {
		...
		ResultSet resultSet =
			driver.executeQuery("select first_name, salary_cur from person where rowid = 2");
		assertTrue(resultSet.first());
		assertEquals("raimundo", resultSet.getString(1));
		resultSet.close();
		assertTrue((resultSet =
						driver.executeQuery("select salary_cur, first_name from person where rowid = 2")).first());
		...
		assertEquals(greatThanCount + equalCount, executeQuery(driver,
					 "select * from PERSON where years_exp_java >= 10"));

		// Simple query, with integer comparison >.
		assertEquals(greatThanCount, executeQuery(driver,
					 "select * from PERSON where years_exp_java > 10"));
		...
		// Simple query, with integer comparison <>.
		assertEquals(numRows - equalCount, executeQuery(driver,
					 "select * from PERSON where years_exp_java <> 10"));
		...
		// Simple query, with integer comparison <.
		assertEquals(numRows - equalCount - greatThanCount, executeQuery(driver,
					 "select * from PERSON where years_exp_java < 10"));

		// Simple query, with integer comparison <=.
		assertEquals(numRows - greatThanCount, executeQuery(driver,
					 "select * from PERSON where years_exp_java <= 10"));
		...
		// Simple query, with string comparison =.
		assertEquals(equalCount, executeQuery(driver,
											  "select * from PERSON where FIRST_NAME = 'raimundo'"));
		...
		assertEquals(8, executeQuery(driver,
									 "select * from PERSON where last_name not like '%or%'")); // Not match.
		assertEquals(4, executeQuery(driver,
									 "select * from PERSON where last_name not like '%o%'")); // Not match.
		assertEquals(1, executeQuery(driver,
									 "select * from PERSON where last_name like 'din%ite'")); // Starts and ends with.
		assertEquals(1, executeQuery(driver,
									 "select * from PERSON where last_name like 'sampaio'")); // LIKE as equals.
		...
		assertEquals(8,
					 driver.executeUpdate("update PERSON set FIRST_NAME = 'x' where salary_prev < 3000 or ((not years_exp_c >= years_exp_java) "
										  + "and not years_exp_java >= 10) or FIRST_NAME > 'r'"));
		assertEquals(8,
					 driver.executeUpdate("delete PERSON where salary_prev < 3000 or ((not years_exp_c >= years_exp_java) "
										  + "and not years_exp_java >= 10) or FIRST_NAME > 'r'"));
