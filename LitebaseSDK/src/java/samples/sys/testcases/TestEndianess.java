...
public class TestEndianess extends TestCase
{
  ...
  public void testRun()
  {
    ...
    driver.execute(
      "create table testendianess(ID_PRODUTO int , ID_FORNECEDOR int, NM_NOME char(30), VL_VALOR double, vl_long long, "
      + "vl_float float) ");
    driver.executeUpdate(
      "insert into testendianess values (10 ,20,'guilherme',3.14D," +
      0x1234567890123456L + ",3.15f)");

    ResultSet rs =
      driver.executeQuery("select * from testendianess where ID_PRODUTO = 10");
    assertTrue(rs.next());
    ...
    assertEquals(Convert.toString(0x1234567890123456L),
                 rs.getString("vl_long"));
