...
final class LBThread implements Runnable
{
   ...
   */
  final static boolean doSleep = Settings.platform.equals(Settings.WIN32) ||
                                 Settings.platform.equals(Settings.LINUX) ||
                                 Settings.onJavaSE;
  ...
  public void run()
  {
    try {
      ...
      conn.execute("create table person" + tableId +
      ...
      PreparedStatement stmt = conn.prepareStatement(
        "insert into person" + tableId + " (id,name) values(?, ?)");
      while (++i < 100) {
        if (i == 50 && doSleep) {
          Thread.yield();
        }
      ...
      resultSet =
        conn.executeQuery("select * from person" + tableId + " where id >= 50");
      ...
      LitebaseConnection.setLogger(
        LitebaseConnection.getLogger()); // Tests the logger.
