...
public class TestBlob extends TestCase {
	...
	public void testRun() {
		...
		PreparedStatement preparedStmt2 =
			driver.prepareStatement("insert into blob2 (picture, name) values (?, ?)");
		PreparedStatement preparedStmt3 =
			driver.prepareStatement("insert into blob3 (id, video, name) values (?, ?, ?)");
		...
		(preparedStmt2 =
			 driver.prepareStatement("insert into blob2 (picture, name) values (?, ?)")).setBlob(0, null);
		...
		(preparedStmt2 = driver.prepareStatement("update blob2 set picture = ? where rowid = 11")).setBlob(
			0, bytes[0]);
		assertEquals(1, preparedStmt2.executeUpdate());
		assertEquals(1, (rs2 = driver.executeQuery("select * from blob2 where rowid = 11")).getRowCount());
		rs2.next();
		...
		(preparedStmt2 =
			 driver.prepareStatement("insert into blob2 (picture, name) values (?, ?)")).setNull(0);
		assertEquals(1, preparedStmt2.executeUpdate());
		...
		(preparedStmt3 = driver.prepareStatement("update blob3 set video = ? where rowid = 10")).setBlob(0,
				bytes[9]);
		assertEquals(1, preparedStmt3.executeUpdate());
		assertEquals(1, (rs3 = driver.executeQuery("select * from blob3 where rowid = 10")).getRowCount());
		rs3.next();
		...
		(preparedStmt2 =
			 driver.prepareStatement("insert into blob2 (picture, name) values (?, ?)")).setBlob(0, new byte[0]);
		...
		(preparedStmt2 =
			 driver.prepareStatement("update blob2 set picture = ?, name = '' where rowid = 1")).setBlob(0,
					 new byte[0]);
		assertEquals(1, preparedStmt2.executeUpdate());
