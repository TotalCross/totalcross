...
public class TestSQLFunctions extends TestCase {
	...
	public void testRun() {
		...
		assertNotNull(resultSet =
						  driver.executeQuery("Select year(years) as mon1, years from person where day(years) >= 6"));
		...
		assertNotNull(resultSet =
						  driver.executeQuery("Select hour(years) as h1, day(birth) as d1 from person where month(birth) != 7 "
											  +
											  "                                                                                                 and hour(years) != 0"));
		...
		assertNotNull(resultSet =
						  driver.executeQuery("Select  millis(years) as mil1, minute(years) as sec1 from person where birth > '2005/9-12' "
											  + "order by years"));
		assertEquals(3, resultSet.getRowCount());
		assertTrue(resultSet.next());
		try {
		...
		assertNotNull(resultSet =
						  driver.executeQuery("Select year(birth) as y1, month(birth) as m1, day(birth) as d1 from person " +
		...
		assertNotNull(resultSet =
						  driver.executeQuery("Select hour(years) as h1, minute(years) as m1, second(years) as d1 from person "
											  +
											  "where hour(years) >= 12"));
		assertEquals(2, resultSet.getRowCount());
		assertTrue(resultSet.next());
		...
		// rnovais@570_2: ABS.
		assertNotNull(resultSet =
						  driver.executeQuery("Select amount, abs(amount) as a1 from person where abs(amount)>13"));
		assertEquals(2, resultSet.getRowCount());
		assertTrue(resultSet.next());
		...
		assertNotNull(resultSet =
						  driver.executeQuery("Select amount, abs(amount) as a1, name, lower(name) as u1, upper(name) as u2  from person where"
		...
		// rnovais@570_6
		assertNotNull(resultSet =
						  driver.executeQuery("Select abs(amount) as a0, abs(amount1) as a1, abs(amount2) as a2, abs(amount3) as a3, "
		...
		// rnovais@570_108
		assertNotNull(resultSet =
						  driver.executeQuery("Select amount from person where month(birth) = 7 and year(birth) = 2006 and day(birth) != 9 and"
											  + " hour(years) = 0"));
		assertEquals(1, resultSet.getRowCount());
		assertTrue(resultSet.next());
		...
		assertNotNull(resultSet =
						  driver.executeQuery("Select amount from person where month(birth) < 5 and birth = '2008/4/6'"));
		assertEquals(1, resultSet.getRowCount());
		assertTrue(resultSet.next());
		assertEquals(-20, resultSet.getInt(1));
		assertFalse(resultSet.next());
		...
		assertNotNull(resultSet =
						  driver.executeQuery("Select upper(name) as n1 from person where month(birth) >= 5 and birth != '2005/9/12'"));
		assertEquals(2, resultSet.getRowCount());
		assertTrue(resultSet.next());
		...
		// rnovais@112_1
		assertNotNull(resultSet =
						  driver.executeQuery("Select amount from person where lower(name) >= 'zenes lima'"));
		assertEquals(1, resultSet.getRowCount());
		assertTrue(resultSet.next());
		assertEquals(-15, resultSet.getInt(1));
		assertFalse(resultSet.next());
