 ...
 * FUNCTIONS: year, month, day, hour, minute, second, millis, upper, lower, and
 * abs // rnovais@570_2.
 *
...
public class TestSQLFunctions extends TestCase
{
  ...
  public void testRun()
  {
    ...
    try {
      ...
      driver.execute(
        "create table person(name char(16), amount int, amount1 short, amount2 long, amount3 float, amount4 double, birth Date, "
        + "years DateTime)");
      driver.executeUpdate(
        "insert into person values ('Renato Novais',-1,-2,-100l,-1.2,-456.0, '  2007/5-3  ', ' 2007/11-2    12:08:01:234 ')");
      driver.executeUpdate(
        "insert into person values ('indira gomes',13,-8,-25l,5.2,-154.0, '2006/7/8 ', '2006/08-21 0:08')");
      driver.executeUpdate(
        "insert into person values ('Lucas Novais',-20,-456,48L,-5.9,-954.2, '2008/4/6', ' 2008/06/06  13:45 ')");
      driver.executeUpdate(
        "insert into person values ('Zenes Lima',-15,-54, -5698L,-8.3,-456.5, '2005/9/12 ', '2005/01-4 1:50')");
    ...
    // month(DATE).
    assertNotNull(resultSet = driver.executeQuery(
                    "Select month(years) as mon1, years from person"));
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select year(years) as mon1, years from person where day(years) >= 6"));
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select hour(years) as h1, day(birth) as d1 from person where month(birth) != 7 "
        +
        "                                                                                                 and hour(years) != 0"));
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select  millis(years) as mil1, minute(years) as sec1 from person where birth > '2005/9-12' "
        + "order by years"));
    assertEquals(3, resultSet.getRowCount());
    assertTrue(resultSet.next());
    try {
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select year(birth) as y1, month(birth) as m1, day(birth) as d1 from person "
        +
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select hour(years) as h1, minute(years) as m1, second(years) as d1 from person "
        + "where hour(years) >= 12"));
    assertEquals(2, resultSet.getRowCount());
    assertTrue(resultSet.next());
    try {
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select amount, abs(amount) as a1 from person where abs(amount)>13"));
    assertEquals(2, resultSet.getRowCount());
    assertTrue(resultSet.next());
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select amount, abs(amount) as a1, name, lower(name) as u1, upper(name) as u2  from person where"
    ...
    assertEquals("-20\t20\tLucas Novais\tlucas novais\tLUCAS NOVAIS",
                 resultSet.rowToString());
    assertTrue(resultSet.next());
    assertEquals(-15, resultSet.getInt(1));
    assertEquals(15, resultSet.getInt("a1"));
    ...
    assertEquals("-15\t15\tZenes Lima\tzenes lima\tZENES LIMA",
                 resultSet.rowToString());
    assertFalse(resultSet.next());
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select abs(amount) as a0, abs(amount1) as a1, abs(amount2) as a2, abs(amount3) as a3, "
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select amount from person where month(birth) = 7 and year(birth) = 2006 and day(birth) != 9 and"
        + " hour(years) = 0"));
    assertEquals(1, resultSet.getRowCount());
    assertTrue(resultSet.next());
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select amount from person where month(birth) < 5 and birth = '2008/4/6'"));
    assertEquals(1, resultSet.getRowCount());
    assertTrue(resultSet.next());
    assertEquals(-20, resultSet.getInt(1));
    assertFalse(resultSet.next());
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select upper(name) as n1 from person where month(birth) >= 5 and birth != '2005/9/12'"));
    assertEquals(2, resultSet.getRowCount());
    assertTrue(resultSet.next());
    ...
    assertNotNull(
      resultSet = driver.executeQuery(
        "Select amount from person where lower(name) >= 'zenes lima'"));
    assertEquals(1, resultSet.getRowCount());
    assertTrue(resultSet.next());
    assertEquals(-15, resultSet.getInt(1));
    assertFalse(resultSet.next());
    ...
    try {
      driver.executeQuery(
        "Select  year(birth) as y1, month(birth) as m1, day(birth) as d1 from person where second(birth) = 234");
      fail("10");
    } catch (SQLParseException exception) {
    }
    ...
    output(
      "function that isn't compatible with data type: upper x date"); // rnovais@570_2
    try {
    ...
    output(
      "function that isn't compatible with data type: abs x char"); // rnovais@570_2
    try {
