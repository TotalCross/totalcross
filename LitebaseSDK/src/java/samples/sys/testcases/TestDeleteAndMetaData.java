...
/**
 * This tests if a table was successfully deleted, if the insert of a Long is
 * working and the meta data class.
 */
public class TestDeleteAndMetaData extends TestCase
{
  ...
  private void testMetaData()
  {
    ...
    rs = driver.executeQuery(
      "select time as Tempo, name from tabsync order by rowid");
    ...
    assertEquals(2,
                 (meta = (rs = driver.executeQuery(
                            "select name, time as Tempo from tabsync"))
    ...
    assertEquals(2,
                 (meta = (rs = driver.executeQuery("select * from tabsync"))
                           .getResultSetMetaData())
                   .getColumnCount());
    assertEquals(5, meta.getColumnDisplaySize(2));
    ...
    assertEquals(3,
                 (meta = (rs = driver.executeQuery(
                            "select rowid, time, name from tabsync"))
                           .getResultSetMetaData())
                   .getColumnCount());
    ...
    assertEquals(
      4,
      (meta = (rs = driver.executeQuery("select * from tabsync, tabsync2"))
                .getResultSetMetaData())
        .getColumnCount());
    assertEquals(5, meta.getColumnDisplaySize(2));
    ...
    assertEquals(
      ...
      (meta =
         (rs = driver.executeQuery(
            "select count(*) as c, upper(tabsync.name) as n1, lower(tabsync2.name) as n2, "
            + "abs(tabsync.time) as abt from tabsync, tabsync2"))
           .getResultSetMetaData())
        .getColumnCount());
    assertEquals(11, meta.getColumnDisplaySize(1));
    assertEquals(5, meta.getColumnDisplaySize(2));
  ...
  public void testRun()
  {
    ...
    try {
      driver.execute(
        "CREATE TABLE tabsync (time LONG primary key default 1, NAME CHAR(5) NOCASE not null)");
    } catch (AlreadyCreatedException ace) {
    }
    try {
      driver.execute(
        "CREATE TABLE tabsync2 (life LONG, NAME CHAR(5) NOCASE default 'Juli', primary key(name, life))");
    } catch (AlreadyCreatedException ace) {
    }
    ...
    // Inserts records.
    assertEquals(1,
                 driver.executeUpdate("INSERT INTO tabsync VALUES (1, 'Hi')"));
    assertEquals(1,
                 driver.executeUpdate("INSERT INTO tabsync2 VALUES (2, 'Hi')"));
    driver.closeAll();
    driver = AllTests.getInstance("Test");
    ...
    assertEquals(1,
                 driver.executeUpdate("INSERT INTO tabsync VALUES (2, 'Hi')"));
