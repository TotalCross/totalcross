...
public class TestOrderBy extends TestCase
{
  ...
  public void testRun()
  {
    ...
    driver.execute(
      "create table person (name char(10), age int, district char(10))");

    // Populates the table.
    driver.executeUpdate(
      "insert into person values ('Juliana', 28, 'Maracan�')");
    driver.executeUpdate(
      "insert into person values ('Roberta', 26, 'Maracan�')");
    driver.executeUpdate(
      "insert into person values ('Guilherme', 37, 'Copacabana')");
    ...
    driver.executeUpdate(
      "insert into person values ('Renato', 35, 'Flamengo')");
    driver.executeUpdate(
      "insert into person values ('Ronaldo', 61, 'Maracan�')");
    driver.executeUpdate("insert into person values ('F�bio', 23, 'Jacar�')");
    driver.executeUpdate(
      "insert into person values ('Vin�cius', 28, 'Ipanema')");
    ...
    ResultSet rs = driver.executeQuery(
      "select * from person order by name desc, age asc, district");
    ...
    assertEquals(
      (rs = driver.executeQuery(
         "select * from person order by name asc, district desc, age"))
        .getRowCount(),
      20);
    assertTrue(rs.next());
    assertEquals("Bruno", rs.getString(1));
    ...
    assertEquals(
      (rs = driver.executeQuery(
         "select * from person order by age, name asc, district desc"))
        .getRowCount(),
      20);
    assertTrue(rs.next());
    assertEquals("Bruno", rs.getString(1));
    ...
    assertEquals(
      (rs = driver.executeQuery(
         "select * from person order by age asc, district, name desc"))
        .getRowCount(),
      20);
    ...
    assertEquals(
      (rs = driver.executeQuery(
         "select * from person order by district desc, name asc, age"))
        .getRowCount(),
      20);
    ...
    assertEquals(
      (rs = driver.executeQuery(
         "select * from person order by district asc, age, name desc"))
        .getRowCount(),
      20);
    ...
    try { // Order by and group by must match.
      driver.executeQuery(
        "select * from person group by name order by district");
      fail("1");
    } catch (SQLParseException exception) {
    }
    try { // Order by and group by must match.
      driver.executeQuery(
        "select * from person group by name order by name, district");
      fail("2");
    } catch (SQLParseException exception) {
    }
    ...
    try { // All non-agreggate columns must be in the group by clause.
      driver.executeQuery(
        "select rowid, name, age, district from person group by name, age, district");
      fail("4");
    } catch (SQLParseException exception) {
    }

    // These ones must work.
    driver.executeQuery("select * from person group by name, age, district")
      .close();
    driver
      .executeQuery(
        "select rowid, name, age, district from person group by rowid, name, age, district")
      .close();
    driver
      .executeQuery(
        "select rowid, name, age, district from person group by rowid, name, age, district order by rowid, name, age, district")
      .close();
