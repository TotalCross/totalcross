...
public class TestAddColumn extends TestCase
{
  ...
  private void wrongSyntax()
  {
    ...
    try { // An added column can't be declared as primary key.
      driver.executeUpdate(
        "alter table person add x char(5) default 'x' primary key");
      fail("5");
    } catch (SQLParseException exception) {
    }
    try { // An added column declared as not null must have a not null default
          // value.
    ...
    try { // An added column declared as not null must have a not null default
          // value.
      driver.executeUpdate(
        "alter table person add x char(5) not null default null");
      fail("7");
    } catch (SQLParseException exception) {
    }
    try { // Totally wrong SQL.
      driver.executeUpdate(
        "alter table person add primary key (name) x char(5)");
      fail("8");
    } catch (SQLParseException exception) {
    }
  ...
  private void addAfterInsert()
  {
    ...
    driver.executeUpdate(
      "alter table person add f datetime default '2012/03/20 16:53:42'");
    ...
    driver.executeUpdate(
      "update person set a = 'a', b = 1, c = null, d = 2, e = 3.5, f = '1979/06/26'");
    assertEquals(
      i = 100,
      (resultSet = driver.executeQuery("select * from person")).getRowCount());
    while (resultSet.next()) {
      assertEquals("name" + (--i), resultSet.getString(1));
  ...
  {
    int i = 100;
    PreparedStatement prepStmt = driver.prepareStatement(
  ...
  private void addAndRecover()
  {
    try {
      ...
      File dbFile = new File(
        path, File.READ_WRITE); // The table is closed after recovering it.
  ...
  {
    ResultSet resultSet;
    PreparedStatement psSelect = driver.prepareStatement(
      "select * from person where name = ? and a = ? and b = ? and c = ? and d = ? and e = ? "
