...
public class TestWhereClause_Caseless extends TestCase {
	...
	private void testQueriesCaseless(LitebaseConnection driver) {
		...
		assertEquals(9, executeQuery(driver,
									 "select * from PERSON where FIRST_NAME = 'maria' or LAST_NAME = 'dinamite'"));
		assertEquals(5, executeQuery(driver,
									 "select * from PERSON where FIRST_NAME = 'roberto' or city = 'NATAL'"));
		...
		assertEquals(10, executeQuery(driver,
									  "select * from PERSON where last_name = 'Jose' or years_exp_Java > 10 or years_exp_C > 18"));
		...
		// Complex queries.
		assertEquals(3, executeQuery(driver,
									 "select * from PERSON where (salary_prev < 3000 and years_exp_C = 10) or city = 'ouro preto' "
									 + "or first_name = 'zico'"));
		assertEquals(9, executeQuery(driver,
									 "select * from PERSON where FIRST_NAME = 'maria' or (SALARY_CUR > 3000 and years_exp_C = 6) "
									 + "or city = 'natal'"));
		assertEquals(5, executeQuery(driver,
									 "select * from PERSON where city = 'Rio de Janeiro' and (years_exp_Java > 10 "
									 + "or years_exp_C > 3)"));
		assertEquals(8, executeQuery(driver,
									 "select * from PERSON where first_name = last_name or last_name = 'dinamite'"));
	...
	private void testQueriesCase(LitebaseConnection driver) {
		...
		// Non-exact search with AND.
		assertEquals(0, executeQuery(driver,
									 "select * from PERSON where city = 'rio de Janeiro' and years_exp_Java > 10 and years_exp_C > 3"));
