...
/**
 * This test verifies if the DATE and DATETIME Litebase data fields are working
 * properly.
 */
public class TestDate_DateTime extends TestCase
{
  ...
  private void doSelects()
  {
    ...
    ResultSet rs = driver.executeQuery(
      "Select name, age,birth , years from person where birth = '2001/06/06'");
    assertNotNull(rs);
    assertEquals(0, rs.getRowCount());
    rs.close();
    assertNotNull(
      rs = driver.executeQuery(
        "Select name, age,birth , years from person where years = '2008/06/06 13:45'"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "Select name, age,birth , years from person where birth <= '2005-7/8'"));
    assertEquals(2, rs.getRowCount());
    rs.close();
    assertNotNull(
      rs = driver.executeQuery(
        "Select name, age,birth , years from person where years < '2007/06/06  0.0'"));
    assertEquals(3, rs.getRowCount());
    rs.close();
    assertNotNull(
      rs = driver.executeQuery(
        "Select name, age,birth , years from person where birth > '2003/01/02' and years < '2030/06/06  1.59:40'"));
    ...
    assertNotNull(
      rs = driver.executeQuery(
        "Select years as aName, sum(age) as abirth from person group by years"));
    assertTrue(rs.next());
    ...
    assertNotNull(
      rs = driver.executeQuery("Select year(birth) as years from person"));
    assertTrue(rs.next());
  ...
  private void createTable()
  {
    ...
    driver.execute(
      "create table person(name char(16) , age int, birth Date, years DateTime)");
    driver.execute("create index idx on person(years)");
    driver.executeUpdate(
      "insert into person values ('renato novais',12, '  2005/9-12  ', ' 2006/08-21    12:08:01:234 ')");
    driver.executeUpdate(
      "insert into person values ('indira gomes',13, '2005/7/8 ', '2006/08-21 0:08')");
    driver.executeUpdate(
      "insert into person values ('Lucas Novais',20, '2005/7/8', ' 2008/06/06  13:45 ')");
    driver.executeUpdate(
      "insert into person values ('Zenes Lima',15, '2005/9/12 ', '2006/08-21 0:08')");

    // Invalid date and datetime,
    try {
      driver.executeUpdate(
        "insert into person values ('Juliana Imperial', 29, '', '1979/06-26 2:00')");
      fail("7");
    } catch (SQLParseException exception) {
    ...
    try {
      driver.executeUpdate(
        "insert into person values ('Juliana Imperial', 29, '1979/06/26', '')");
      fail("8");
    } catch (SQLParseException exception) {
    }
    try {
    ...
    try {
      driver.executeUpdate(
        "insert into person values ('Juliana Imperial', 29, '1979/02/29', '1979/06-26 2:00')");
      fail("11");
    } catch (SQLParseException exception) {
    }
    try {
      driver.executeUpdate(
        "insert into person values ('Juliana Imperial', 29, '3000/26/06', '1979/06-26 2:00')");
      fail("12");
    } catch (SQLParseException exception) {
    }
    try {
      driver.executeUpdate(
        "insert into person values ('Juliana Imperial', 29, '1979/26/06', '1979/06-26 2:00:00.000.0')");
      fail("13");
    } catch (SQLParseException exception) {
    }
  }
   ...
   */
  private void testPreparedStatement()
  { // rnovais@_570_56
    ...
    driver
      .prepareStatement(
        " create table person2(name char(16) , age int, birth Date, years DateTime) ")
      .executeUpdate();
    driver.prepareStatement(" create index idx on person2(years) ")
      .executeUpdate();
    driver.prepareStatement(" create index idx on person2(birth) ")
      .executeUpdate();

    PreparedStatement ps =
      driver.prepareStatement(" insert into person2 values(?,?,?,?) ");
    ...
    try {
      ...
      (ps = driver.prepareStatement(
         "update person2 set name = ?  where birth = ?"))
        .setString(0, "danilo");
      ...
      (ps = driver.prepareStatement(
         "update person2 set name = ?  where years = ?"))
        .setString(0, "carol");
      ...
      (ps = driver.prepareStatement("delete from person2 where birth = ?"))
        .setDate(0, new Date("12/09/2005", Settings.DATE_DMY));
      assertEquals(2, ps.executeUpdate());
      assertEquals(2, getRowCount("person2"));

      (ps = driver.prepareStatement("delete from person2 where years = ?"))
        .setDateTime(0, new Time("20060821T00:08:00"));
      assertEquals(1, ps.executeUpdate());
  ...
  /**
   * Does some tests using select prepared statements with indices and
   * functions.
   */
  private void testPreparedStatement2()
  { // rnovais@570_108
    ...
    driver
      .prepareStatement(
        "insert into person2 values ('jo�o pedro',13, '2006/7/8 ', '2006/08-21 0:08')")
      .executeUpdate();
    driver
      .prepareStatement(
        "insert into person2 values ('danilo novais',20, '2008/4/6', ' 2008/06/06  13:45 ')")
      .executeUpdate();
    driver
      .prepareStatement(
        "insert into person2 values ('carol nOVAIS',15, '2005/9/12 ', '2005/01-4 1:50')")
      .executeUpdate();

    try {
      (ps = driver.prepareStatement("select * from person2 where birth = ?"))
        .setDate(0, new Date("8/7/2006", Settings.DATE_DMY));
      ...
      (ps = driver.prepareStatement(
         "select name from person2 where month(birth) = ? and day(birth) = ? and minute(years) = 50 and millis(years) != ?"))
        .setShort(0, (short)9);
      ...
      (ps = driver.prepareStatement(
         "select name from person2 where month(birth) != ? and (day(birth) = ? or minute(years) = ?)"))
        .setShort(0, (short)7);
      ...
      (ps = driver.prepareStatement(
         "select name, birth, years from person2 where (day(birth) = ? and birth != ?) or minute(years) = ?"))
        .setShort(0, (short)6);
  ...
  private void createTablePrimarykeyDateTime()
  {
    ...
    driver.execute(
      "create table person(name char(16), age int, birth date primary key, years DateTime)");

    // Duplicated date.
    assertEquals(
      1,
      driver.executeUpdate(
        "insert into person values ('renato novais',12,'2005/9-12', '2006/08-21 12:08')"));
    try {
      driver.executeUpdate(
        "insert into person values ('indira gomes',13,'2005/9-12', '2007/09/20')");
    ...
    try {
      driver.executeUpdate(
        "insert into person values ('indira gomes',13,'2007/06/15', '2006/08-21 12:08')");
  ...
  /**
   * Tests that setDateTime inserts the correct value of a DATETIME using a
  ...
  private void testSetDateTime()
  {
    ...
    driver.execute(
      "CREATE TABLE time (campo DATETIME primary key, data date default '1979/06/26')");
    PreparedStatement pstmt =
      driver.prepareStatement("INSERT INTO time (campo) VALUES(?)");
    Time now = new Time();
  ...
  /**
   * Tests DATE and DATETIME with like using the pattern mathes EQUALS,
   * STARTS_WITH, and ANYTHING.
   */
  private void testLikeStartsWithEqualsAnythingDateDateTime()
  {
    ResultSet resultSet =
      driver.executeQuery("select * from person where birth like '2005/09/12'");
    assertEquals(1, resultSet.getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where birth like '%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where birth like '2%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where birth like '20%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where birth like '200%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where birth like '2005%'"))
                   .getRowCount());
    resultSet.close();
    ...
    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where birth like '2005/0%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where birth like '2005/09%'"))
                   .getRowCount());
    resultSet.close();
    ...
    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where birth like '2005/09/1%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where birth like '2005/09/12%'"))
                   .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      1,
      (resultSet = driver.executeQuery(
         "select * from person where years like '2006/08/21 12:08:00:000%'"))
        .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '2%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '20%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '200%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '2006%'"))
                   .getRowCount());
    resultSet.close();
    ...
    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '2006/0%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '2006/08%'"))
                   .getRowCount());
    resultSet.close();
    ...
    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '2006/08/2%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '2006/08/21%'"))
                   .getRowCount());
    resultSet.close();
    ...
    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '2006/08/21 1%'"))
                   .getRowCount());
    resultSet.close();

    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '2006/08/21 12%'"))
                   .getRowCount());
    resultSet.close();
    ...
    assertEquals(1,
                 (resultSet = driver.executeQuery(
                    "select * from person where years like '2006/08/21 12:0%'"))
                   .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      1,
      (resultSet = driver.executeQuery(
         "select * from person where years like '2006/08/21 12:08%'"))
        .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      1,
      (resultSet = driver.executeQuery(
         "select * from person where years like '2006/08/21 12:08:0%'"))
        .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      1,
      (resultSet = driver.executeQuery(
         "select * from person where years like '2006/08/21 12:08:00%'"))
        .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      1,
      (resultSet = driver.executeQuery(
         "select * from person where years like '2006/08/21 12:08:00:0%'"))
        .getRowCount());
    resultSet.close();
    ...
    assertEquals(
      1,
      (resultSet = driver.executeQuery(
         "select * from person where years like '2006/08/21 12:08:00:00%'"))
        .getRowCount());
    resultSet.close();
