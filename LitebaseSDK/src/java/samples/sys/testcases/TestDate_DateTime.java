...
public class TestDate_DateTime extends TestCase {
	...
	private void doSelects() {
		...
		ResultSet rs =
			driver.executeQuery("Select name, age,birth , years from person where birth = '2001/06/06'");
		assertNotNull(rs);
		assertEquals(0, rs.getRowCount());
		rs.close();
		assertNotNull(rs =
						  driver.executeQuery("Select name, age,birth , years from person where years = '2008/06/06 13:45'"));
		assertEquals(1, rs.getRowCount());
		rs.close();

		// Where clause with inequalities.
		assertNotNull(rs =
						  driver.executeQuery("Select name, age,birth , years from person where birth <= '2005-7/8'"));
		assertEquals(2, rs.getRowCount());
		rs.close();
		assertNotNull(rs =
						  driver.executeQuery("Select name, age,birth , years from person where years < '2007/06/06  0.0'"));
		...
		assertNotNull(rs =
						  driver.executeQuery("Select years as aName, sum(age) as abirth from person group by years"));
		assertTrue(rs.next());
	...
	private void testPreparedStatement() { // rnovais@_570_56
		...
		try {
			...
			(ps = driver.prepareStatement("update person2 set name = ?  where birth = ?")).setString(0,
					"danilo");
			ps.setDate(1, new Date("8/7/2005", Settings.DATE_DMY));
			assertEquals(2, ps.executeUpdate());
			assertEquals(4, getRowCount("person2"));

			(ps = driver.prepareStatement("update person2 set name = ?  where years = ?")).setString(0,
					"carol");
			...
			(ps = driver.prepareStatement("delete from person2 where birth = ?")).setDate(0,
					new Date("12/09/2005", Settings.DATE_DMY));
			assertEquals(2, ps.executeUpdate());
			assertEquals(2, getRowCount("person2"));

			(ps = driver.prepareStatement("delete from person2 where years = ?")).setDateTime(0,
					new Time("20060821T00:08:00"));
			assertEquals(1, ps.executeUpdate());
	...
	private void testPreparedStatement2() { // rnovais@570_108
		...
		try {
			(ps = driver.prepareStatement("select * from person2 where birth = ?")).setDate(0,
					new Date("8/7/2006", Settings.DATE_DMY));
			ResultSet rs = ps.executeQuery();
			assertTrue(rs.next());
			assertEquals("jo�o pedro", rs.getString(1));
			rs.close();

			(ps = driver.prepareStatement("select name from person2 where month(birth) = ? and day(birth) = ? and minute(years) = 50 and millis(years) != ?")).setShort(
				0, (short)9);
			...
			(ps = driver.prepareStatement("select name from person2 where month(birth) != ? and (day(birth) = ? or minute(years) = ?)")).setShort(
				0, (short)7);
			...
			(ps = driver.prepareStatement("select name, birth, years from person2 where (day(birth) = ? and birth != ?) or minute(years) = ?")).setShort(
				0, (short)6);
	...
	private void createTablePrimarykeyDateTime() {
		...
		// Duplicated date.
		assertEquals(1,
					 driver.executeUpdate("insert into person values ('renato novais',12,'2005/9-12', '2006/08-21 12:08')"));
		try {
	...
	private void testLikeStartsWithEqualsAnythingDateDateTime() {
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where birth like '%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where birth like '2%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where birth like '20%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where birth like '200%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where birth like '2005%'")).getRowCount());
		resultSet.close();
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where birth like '2005/0%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where birth like '2005/09%'")).getRowCount());
		resultSet.close();
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where birth like '2005/09/1%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where birth like '2005/09/12%'")).getRowCount());
		resultSet.close();
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21 12:08:00:000%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '20%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '200%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006%'")).getRowCount());
		resultSet.close();
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/0%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08%'")).getRowCount());
		resultSet.close();
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/2%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21%'")).getRowCount());
		resultSet.close();
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21 1%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21 12%'")).getRowCount());
		resultSet.close();
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21 12:0%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21 12:08%'")).getRowCount());
		resultSet.close();
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21 12:08:0%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21 12:08:00%'")).getRowCount());
		resultSet.close();
		...
		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21 12:08:00:0%'")).getRowCount());
		resultSet.close();

		assertEquals(1, (resultSet =
							 driver.executeQuery("select * from person where years like '2006/08/21 12:08:00:00%'")).getRowCount());
		resultSet.close();
