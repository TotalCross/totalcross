...
public class TestSelectClause_AggFunctions extends TestCase
{
  ...
  public void testRun()
  {
    ...
    // Creates the table.
    driver.execute(
      "create table PERSON (FIRST_NAME CHAR(30), CITY CHAR(30), STATE CHAR(2), SALARY FLOAT, AGE SHORT)");
    ...
    assertEquals(
      7,
      executeQuery(
        driver, "select first_name, age, CITY from PERSON where state ='sp'"));
    ...
    assertEquals(
      ...
      executeQuery(
        driver,
        "select first_name as nome, age as idade, CITY from PERSON where state ='sp'"));

    // Simple COUNT(*)
    ResultSet resultSet =
      driver.executeQuery("select COUNT(*) as total_records from PERSON");
    assertTrue(resultSet.next());

    assertEquals(
      numRows,
      resultSet.getInt("total_records")); // Asserts that query worked properly.
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select max(FIRST_NAME) as max_name, min(FIRST_NAME) as min_name, MAX(city) as max_city, "
         + "MIN(city) as min_city from PERSON"))
        .next());
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select max(FIRST_NAME) as max_name, count(*) as total, state from PERSON group by state "
         + "having max_name >= 'nadia'"))
        .next());
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select min(FIRST_NAME) as min_name, count(*) as total, state from PERSON group by state "
         + "having min_name <= 'guilherme'"))
        .next());
    assertEquals(3, resultSet.getRowCount());
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select COUNT(*) as total_records from PERSON where state = 'sp'"))
        .next());
    assertEquals(
      7,
      resultSet.getInt("total_records")); // Asserts that query worked properly.
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select COUNT(*) as total, min(age) as min_age, max(age) as max_age, sum(salary) as sum_salary, "
         + "avg(salary) as avg_salary from PERSON"))
        .next());
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select COUNT(*) as total, min(age) as min_age, max(age) as max_age, sum(salary) as sum_salary, "
         + "avg(salary) as avg_salary from PERSON where state ='sp'"))
        .next());
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select CITY, state, COUNT(*) as total, min(age) as min_age, avg(age) as avg_age, "
         +
         "sum(salary) as sum_salary, avg(salary) as avg_salary from PERSON group by state,CITY"))
        .next());
    ...
    // Second record: Rio de Janeiro RJ 4 35 47.25 21634.209999999999
    // 5408.5524999999998.
    ...
    // Fourth record: Uruguaiana RS 1 26 26.0 4441.8800000000001
    // 4441.8800000000001.
    ...
    // Sixth record: Sao Paulo SP 2 23 35.5 7976.3400000000001
    // 3988.1700000000001.
    assertEquals(2, resultSet.getInt("total"));
    assertEquals(23, resultSet.getShort("min_age"));
    assertEquals("sp", resultSet.getString("state"));
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select first_name as name, CITY from PERSON order by age, first_name desc"))
        .next());
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select first_name as name, CITY from PERSON where state = 'sp' or state = 'rs' "
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select COUNT(*) as total, CITY from PERSON where age > 26 and salary > 3000F group by CITY"))
        .next());
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select COUNT(*) as total, CITY from PERSON where state = 'sp' or state = 'rj' "
    ...
    // Select with group by but no aggregated function to simulate the usage of
    // a "distinct" in the select.
    assertTrue(
      (resultSet = driver.executeQuery(
         "select CITY from PERSON where state = 'sp' or state = 'rj' group by CITY"))
        .next());
    ...
    assertTrue(
      (resultSet = driver.executeQuery(
         "select CITY, COUNT(*) as total from PERSON where state = 'sp' or state = 'rj' group by CITY "
         + "having total > 2 order by CITY"))
        .next());
    ...
    // Select with aggregated functions and simple columns with group by and
    // having clauses.
    assertTrue(
      (resultSet = driver.executeQuery(
         "select CITY, state, COUNT(*) as total, min(age) as min_age, avg(age) as avg_age, "
         +
         "sum(salary) as sum_salary, avg(salary) as avg_salary from PERSON group by state, CITY having min_age > 25 and sum_salary < 15000"))
        .next());
    ...
    // Second record: Uruguaiana RS 1 26 26.0 4441.8800000000001
    // 4441.8800000000001.
    assertEquals(1, resultSet.getInt("total"));
    assertEquals(26, resultSet.getShort("min_age"));
    assertEquals("rs", resultSet.getString("state"));
    ...
    driver.execute(
      "create table cliente(clienteEmpresaId int not null, clienteId int not null, cpfCnpj varchar(14), tipoPessoa varchar(1), "
      +
    ...
    assertFalse(
      (resultSet =
         driver.prepareStatement(" SELECT max(clienteId) as id FROM cliente ")
           .executeQuery())
        .first());
    resultSet.close();
    driver.closeAll();
