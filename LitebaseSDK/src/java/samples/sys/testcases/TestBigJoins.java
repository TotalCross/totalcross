...
public class TestBigJoins  extends TestCase {
	...
	private void test2Tables() {
		...
		// Prepared statements.
		PreparedStatement psInsertAnimal =
			connection.prepareStatement("INSERT INTO ANIMAL VALUES (?, ?, ?, ?)"),
		...
		id = Vm.getTimeStamp();
		ResultSet resultSet =
			connection.executeQuery("SELECT SUM(ORDENHA.LEITE) AS TOTAL FROM ORDENHA, ANIMAL WHERE ORDENHA.LEITE > 0 "
	...
	private void test3Tables() {
		...
		ResultSet resultSet =
			connection.executeQuery("select * from precos, produtos, estoque where estoque.CODPROD = produtos.CODPROD and produtos.CODPROD = precos.CODPROD and codlista = 'l1'");
		String column2;
