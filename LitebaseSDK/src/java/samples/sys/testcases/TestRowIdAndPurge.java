...
public class TestRowIdAndPurge extends TestCase
{
  ...
  public void testRun()
  {
    ...
    String[] insertRows = {
      ...
      "insert into PERSON3 values ('cher', 'cher', 1000.50, 3400.50, 4, 3)",
      "insert into PERSON3 values ('lero', 'lero', 2001.34, 1000.35, 2, 6)",
      ...
      "insert into PERSON3 values ('paulo', 'falcao', 2800.04, 1.21, 15, 12)",
      "insert into PERSON3 values ('leo', 'junior', 2.50, 3400.50, 4, 5)"
    };
    ...
    driver.execute(
      "create table PERSON3 (FIRST_NAME CHAR(30) NOCASE, LAST_NAME CHAR(30), SALARY_CUR DOUBLE, SALARY_PREV DOUBLE, "
    ...
    ResultSet rs = driver.executeQuery(
      "select rowid, first_name from person3 where first_name > 'A'");
    i = 0;
    while (++i <= numRows) {
      assertTrue(rs.next());
    ...
    assertEquals(numRows,
                 driver.getRowCountDeleted("PERSON3") +
                   driver.getRowCount("person3"));
    assertEquals(numRows, driver.getRowCount("person3"));

    // Does some selects using like and "%".
    assertEquals(1,
                 (rs = driver.executeQuery(
                    "select * from person3 where first_name like 'leo%'"))
                   .getRowCount());
    rs.close();
    assertEquals(1,
                 (rs = driver.executeQuery(
                    "select * from person3 where last_name like '%orr%'"))
                   .getRowCount());
    rs.close();

    // Deletes some records.
    assertEquals(
      2, driver.executeUpdate("delete person3 where first_name like 'l%'"));
