...
public class TestRowIdAndPurge extends TestCase {
	...
	public void testRun() {
		...
		assertEquals(1, (rs =
							 driver.executeQuery("select * from person3 where first_name like 'leo%'")).getRowCount());
		rs.close();
		assertEquals(1, (rs =
							 driver.executeQuery("select * from person3 where last_name like '%orr%'")).getRowCount());
		rs.close();
