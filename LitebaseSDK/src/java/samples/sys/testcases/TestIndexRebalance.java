...
public class TestIndexRebalance extends TestCase {
	...
	public void testRun() {
		...
		PreparedStatement psInsertCliente =
			driver.prepareStatement("insert into CLIENTE(CODIGO, NOME, FANTASIA, BAIRRO) values (?, ?, ?, ?)");
		...
		assertEquals(1960, (resultSet =
								driver.executeQuery("select * from cliente where bairro >= 2")).getRowCount());
		resultSet.close();
		...
		assertEquals(1960, (resultSet =
								driver.executeQuery("select * from cliente where bairro <= 3 and bairro >= 3")).getRowCount());
		resultSet.close();
		assertEquals(1960, (resultSet =
								driver.executeQuery("select * from cliente where nome like 'NOME %'")).getRowCount());
		resultSet.close();
