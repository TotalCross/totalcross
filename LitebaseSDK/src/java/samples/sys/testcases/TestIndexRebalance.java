...
public class TestIndexRebalance extends TestCase
{
  ...
  public void testRun()
  {
    ...
    driver.execute(
      "create table CLIENTE(CODIGO int primary key, NOME char(60), FANTASIA char(60),BAIRRO int)");
    ...
    driver.setRowInc(
      "CLIENTE", 1125); // Requires two increments to fill all the 2250 rows.
    PreparedStatement psInsertCliente = driver.prepareStatement(
      "insert into CLIENTE(CODIGO, NOME, FANTASIA, BAIRRO) values (?, ?, ?, ?)");
    PreparedStatement psDelete =
      driver.prepareStatement("delete from cliente where codigo=?");
    int i = -1;
    ...
    try {
      ...
      while (++i < 2250) { // 250 of them will be deleted.
        ...
        int k;
        while (
          true) { // make sure that a key that was not repeated will be got.
    ...
    ResultSet resultSet =
      driver.executeQuery("select * from cliente where bairro = 2");
    ...
    assertEquals(1960,
                 (resultSet = driver.executeQuery(
                    "select * from cliente where bairro >= 2"))
                   .getRowCount());
    resultSet.close();
    ...
    assertEquals(1960,
                 (resultSet = driver.executeQuery(
                    "select * from cliente where bairro <= 3 and bairro >= 3"))
                   .getRowCount());
    resultSet.close();
    assertEquals(1960,
                 (resultSet = driver.executeQuery(
