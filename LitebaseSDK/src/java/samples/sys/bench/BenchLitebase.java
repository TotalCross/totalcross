...
public class BenchLitebase extends MainWindow {
	...
	private int selectBeforeLast() {
		...
		ResultSet resultSet = driver.executeQuery("select * from person where name = 'a" +
							  (NRECS - 1) + "'");
	...
	private int selectMax() {
		...
		int time = Vm.getTimeStamp();
		ResultSet resultSet =
			driver.executeQuery("select max(name) as mname from person where name >= 'a0'");
	...
	public void initUI() {
		...
		log("total: " + (time1 + time2 + time3 + time4 + time5 + time6 + time7 + time8 + time9 + time10 +
						 time11 + time12));
