		...
		if (event.type == ControlEvent.PRESSED) {
			if (event.target == btnBack) { // Pressed back button: goes to the customer menu.
				...
				onAddAgain();
					   ...
					   btnRemove) { // Pressed removed button: removes the selected customer if possible.
			...
			} else if (event.target == btnEdit) { // Pressed edit button: edits the selected customer.
