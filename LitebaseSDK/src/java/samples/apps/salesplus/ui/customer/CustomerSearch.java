...
public class CustomerSearch extends Container
{
  ...
  public void initUI()
  {
    ...
    // Buttons.
    add(btnSearch = new Button("Search"),
        RIGHT - 5,
        AFTER + 5,
        PREFERRED,
        PREFERRED);
    ...
    add(grid = new Grid(new String[] { "Name", "City", "State" },
                        new int[] { fm.stringWidth("xxxxxxxxxxx"),
                                    fm.stringWidth("xxxxxxxxxx"),
                                    fm.stringWidth("xxx") },
                        new int[] { LEFT, LEFT, CENTER },
                        false),
        LEFT + 5,
        AFTER + 5,
        FILL - 5,
  ...
  public void onEvent(Event event)
  {
    if (event.type == ControlEvent.PRESSED) {
      if (event.target ==
          btnBack) { // Pressed back button: goes to the customer menu.
        MainWindow.getMainWindow().swap(
          SalesPlus.screens[SalesPlus.CUSTOMER_MENU]);
      } else if (event.target ==
                 btnSearch) { // Pressed search button: searches for a customer.
        onAddAgain();
      } else if (event.target ==
                 btnRemove) { // Pressed removed button: removes the selected
        ...
        if (customer != null) {
          if (!OrderDB.hasOrder(customer)) { // The customer can't have a order.
            ...
            new MessageBox(
              "Error",
              "There are orders that belong to | this customer. If you really want |"
                + "to remove it, delete the orders first.")
              .popupNonBlocking();
        }
      } else if (event.target ==
                 btnEdit) { // Pressed edit button: edits the selected customer.
        Customer cli = getSelectedCustomer();
        if (cli != null) {
          NewCustomer.sentCustomer = cli;
          MainWindow.getMainWindow().swap(
            SalesPlus.screens[SalesPlus.NEW_CUSTOMER]);
        }
      }
    }
  }
