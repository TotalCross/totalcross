...
public class NewCustomer extends Container
{
  ...
  public void initUI()
  {
    ...
    add(tabCont = new TabbedContainer(new String[] { "General", "Contact" }),
        LEFT,
        TOP,
        FILL,
        FILL);
    ...
    if (event.type == ControlEvent.PRESSED) {
      if (event.target == btnClear) { // Pressed clear button: clears all data.
        cleanEdit();
                 ...
                 btnBack) { // Pressed back button: goes to the previous menu.
        if (sentCustomer !=
            null) { // There is a selected customer, goes to the customer search
          sentCustomer = null;
          MainWindow.getMainWindow().swap(
            SalesPlus.screens[SalesPlus.CUSTOMER_SEARCH]);
        } else { // No customer were selected, goes to the parent menu.
          MainWindow.getMainWindow().swap(
            SalesPlus.screens[SalesPlus.CUSTOMER_MENU]);
        }
      } else if (event.target ==
                 btnInsert) { // Pressed insert button: checks data and inserts
                              // or updates customer.
        ...
        if (sBuffer.length() > 0)
          ...
          try {
            if (CustomerDB.write(customer, sentCustomer != null) ==
                0) { // Inserts or updates customer.
              new MessageBox("Error", "Customer name can't be repeated.")
                .popup();
              return;
            }
          ...
          cleanEdit();
          if (sentCustomer !=
              null) { // The user may want to update more customers.
            sentCustomer = null;
  ...
  public void onAddAgain()
  {
    ...
    } else { // Update a customer: sets the edits with the selected customer
             // data.
