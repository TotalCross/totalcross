...
public class NewProduct extends Container
{
  ...
  {
    if (sentProduct ==
        null) { // No selected product: a new one will be inserted.
      clearEdit();
      ...
      edPrice.setText(
        Convert.toString((double)sentProduct.unitPrice * 0.01, 2));
  ...
  public void onEvent(Event event)
  {
    if (event.type == ControlEvent.PRESSED) {
      if (event.target == btnInsert) { // The insert button.
        ...
        if (string.length() != 0)
          try {
            product.unitPrice = Convert.toInt(
              new BigDecimal(string).movePointRight(2).toString());
        ...
        try {
          if (ProductDB.write(product, sentProduct != null) ==
              0) { // Inserts or updates product.
        ...
        if (sentProduct != null) { // Updated the product.
          ...
          MainWindow.getMainWindow().swap(
            SalesPlus.screens[SalesPlus.PRODUCT_SEARCH]);
      ...
      if (event.target == btnBack) { // Back button.
        if (sentProduct !=
            null) { // Goes to the search screen if a product was being updated.
          sentProduct = null;
          MainWindow.getMainWindow().swap(
            SalesPlus.screens[SalesPlus.PRODUCT_SEARCH]);
