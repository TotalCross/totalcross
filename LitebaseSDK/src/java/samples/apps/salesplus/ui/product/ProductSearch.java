...
public class ProductSearch extends Container
{
  ...
  public void initUI()
  {
    ...
    // The grid.
    add(grid = new Grid(new String[] { "Name", "Code", "Descr", "Price" },
                        new int[] { fm.stringWidth("xxxxxxxxxxxxxxxx"),
                                    fm.stringWidth("x"),
                                    fm.stringWidth("xxxxxxxxxxxxxxxxx"),
                                    fm.stringWidth("x") },
                        new int[] { LEFT, LEFT, LEFT, RIGHT },
                        false));
  ...
  public void onAddAgain()
  {
    ...
    if (count == 0) { // No elements found: erases the grid.
      ...
      while (++i < count) {
        ...
        strings[i][3] =
          Convert.toString((double)products[i].unitPrice * 0.01, 2);
      }
    ...
    if (event.type == ControlEvent.PRESSED) {
      if (event.target == btnSearch) { // Search button: reloads the grid.
        onAddAgain();
                 ...
                 btnBack) { // Back button: goes back to the product menu.
        MainWindow.getMainWindow().swap(
          SalesPlus.screens[SalesPlus.PRODUCT_MENU]);
      } else if (event.target ==
                 btnEdit) { // Edit button: goes to the new product menu to
                            // update the selected product.
        Product product = getSelectedProduct();
        if (product != null) {
          NewProduct.sentProduct = product;
          MainWindow.getMainWindow().swap(
            SalesPlus.screens[SalesPlus.NEW_PRODUCT]);
        }
      } else if (event.target ==
                 btnRemove) { // Remove button: removes the selected product.
        Product product = getSelectedProduct();
        if (product != null) { // must always be true
          if (!ItemOrderDB.hasProduct(product)) {
            ...
            new MessageBox(
              "Error",
              "There are orders that belong to | this product. If you really want "
                + "| to remove it, delete the orders first.")
              .popup();
        }
      }
    }
