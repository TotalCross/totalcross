...
public class ProductSearch extends Container {
	...
	public void initUI() {
		...
		// The grid.
		add(grid = new Grid(new String[] {"Name", "Code", "Descr", "Price"}, new int[] {fm.stringWidth("xxxxxxxxxxxxxxxx"), fm.stringWidth("x"), fm.stringWidth("xxxxxxxxxxxxxxxxx"), fm.stringWidth("x")},
							new int[] {LEFT, LEFT, LEFT, RIGHT}, false));
		...
		if (event.type == ControlEvent.PRESSED) {
			if (event.target == btnSearch) { // Search button: reloads the grid.
				...
				MainWindow.getMainWindow().swap(SalesPlus.screens[SalesPlus.PRODUCT_MENU]);
					   ...
					   btnEdit) { // Edit button: goes to the new product menu to update the selected product.
