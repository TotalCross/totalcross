...
public class NewOrder extends Container
{
  ...
  public void initUI()
  {
    ...
    container.add(
      cbCustomer = new ComboBox(), edNum.getRect().x, SAME, FILL, PREFERRED);
    container.setRect(
      LEFT + 2, TOP + 2, FILL - 4, cbCustomer.getRect().y2() + 4);

    // The product label and combo box.
    add(new Label("Product "), LEFT + 2, AFTER + 2);
    add(cbProduct = new ComboBox(),
        AFTER,
        SAME,
        PREFERRED + Settings.screenWidth / 5,
        PREFERRED);
    ...
    add(grid = new Grid(new String[] { "Name", "Code", "Qty", "Price" },
                        new int[] { fm.stringWidth("xxxxxxxxxx"),
                                    fm.stringWidth("xxxxxxxx"),
                                    fm.stringWidth("xxx"),
                                    fm.stringWidth("xxxxxxx") },
  ...
  public void onAddAgain()
  {
    ...
    if (names.length ==
        0) { // There are no customer, so there can't add an order.
      ...
      if (names.length ==
          0) { // There are no products, so there can't add an order.
        ...
        if (update) { // The order is being updated.
          ...
          while (++i < length) // Lists all the products and their quantities.
            if ((product = ProductDB.getFromCode(
                   (item = items[i]).productId)) != null) {
  ...
  private void fillGrid()
  {
    ...
    while (++i < count) {
      ...
      strings[i][3] = Convert.toString(
        (double)(product.unitPrice * vQuant.items[i]) * 0.01, 2);
    }
    grid.setItems(strings);
  }

  /**
   * Calculates the total amount of the order based on the products selected and
   * their quantities.
   *
  ...
  /**
   * Cleans this menu, clearing the products and quantity vectors, rewinding the
   * grid, reseting the date, selecting index 0 on both customer and product
   * combo boxes, and reseting the current order number.
   */
  public void clean()
  {
    // Empties the vectors.
    ...
    switch (event.type) {
      case ControlEvent.PRESSED: // A control was pressed.
        ...
        if (event.target == btnAdd &&
            cbProduct.getSelectedIndex() >= 0) { // Adds a product to the order.
          int value =
            spin.getSelectedIndex() + 1; // Gets the selected quantity.
          String name = (String)cbProduct.getSelectedItem();
          boolean found = false;
          int n = products.size();

          while (--n >= 0) // Tries to find the product in the list and updates
                           // its quantity.
          ...
          if (!found) { // The product was not found, so it will be added to the
                        // list.
          ...
          if (update) {
            MainWindow.getMainWindow().swap(
              SalesPlus.screens[SalesPlus.ORDER_SEARCH]);
          } else {
            MainWindow.getMainWindow().swap(
              SalesPlus.screens[SalesPlus.ORDER_MENU]);
          }

          update = false; // The next order won't be updated.
        } else if (event.target ==
                   btnRemove) { // Removes a product from the order.
          String[] strings = getSelectedProduct();
          int n = products.size();
          Product product;

          while (--n >= 0) {
            if ((product = (Product)products.items[n])
                  .name.equals(strings[0])) {
          ...
          edTot.setText(Convert.toString((double)totalValue() * 0.01, 2));
          fillGrid();
        } else if (event.target == btnInsert) { // Inserts an order. This
                                                // actually creates an order.
          int n = products.size();
          ItemOrder item;
          Product product;
          ...
          while (--n >= 0) {
            (item = new ItemOrder()).productId =
              (product = (Product)products.items[n]).code;
          ...
          if (update) {
            n = vDeleted.size();
            while (--n >= 0) {
              ItemOrderDB.remove(currentOrder.orderId,
                                 ((Product)vDeleted.items[n]).code);
            }
          }
      ...
      case ControlEvent
        .FOCUS_OUT: // Corrects the data inserted to the date format.
