...
public class OrderSearch extends Container
{
  ...
  public void initUI()
  {
    ...
    // The grid.
    add(grid =
          new Grid(new String[] { "ID", "Date", "Customer", "Total Amount" },
                   new int[] { fm.stringWidth("xx"),
                               fm.stringWidth("XX/XX/XXXX"),
                               fm.stringWidth("name of the customer"),
                               fm.stringWidth("123456789") },
                   new int[] { LEFT, CENTER, LEFT, RIGHT },
                   false),
        LEFT + 5,
        AFTER + 7,
        FILL - 5,
  ...
  public void onAddAgain()
  {
    ...
    if (count == 0) { // No order found: empties the grid.
      ...
      while (++i < count) {
        ...
        strings[i][2] = nameStr;
        strings[i][3] =
          Convert.toString((double)orders[i].totalAmount * 0.01, 2);
      }
  ...
  public void onEvent(Event event)
  {
    switch (event.type) {
      case ControlEvent.PRESSED:
        if (event.target ==
            btnBack) { // Back button: goes back to the previous menu.
          MainWindow.getMainWindow().swap(
            SalesPlus.screens[SalesPlus.ORDER_MENU]);
        } else if (event.target == btnSearch) { // Search button: updates the
                                                // grid with the results.
          onAddAgain();
        } else if (event.target == btnView) { // View button: views more details
                                              // of the selected order.
          Order order = getSelectedOrder();

          if (order !=
              null) { // If there is a selected order, views it in detail.
            NewOrder.currentOrder = order;
            NewOrder.update = true;
            MainWindow.getMainWindow().swap(
              SalesPlus.screens[SalesPlus.NEW_ORDER]);
          }
        } else if (event.target ==
                   btnRemove) { // Remove button: removes the selected order.
          Order order = getSelectedOrder();
          if (order != null) {
