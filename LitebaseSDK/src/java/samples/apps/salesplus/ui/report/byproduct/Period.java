...
public class Period extends Container
{
  ...
  public void initUI()
  {
    ...
    add(grid = new Grid(new String[] { "Product", "Qty", "Amount" },
                        new int[] { fm.stringWidth("xxxxxxxxx"),
                                    fm.stringWidth("xxxx"),
      ...
      case ControlEvent.PRESSED:
        if (event.target == btnOk) { // Ok button: does the search.
          ...
          if (date1 > date2) {
            new MessageBox(
              "Message",
              "ATTENTION | The date 'to' can | be not lower than | the date 'from'")
              .popup();
            return;
          }
          ...
          if (count1 > 0) {
            ...
            ItemOrder[] items;
            ItemOrder item;
            int i1 = -1, total = 0, i2, count2;

            while (++i1 < count1) {
              strings[i1] = new String[count2 = (items = ItemOrderDB.readAll(
                                                   ((Order)orders[i1]).orderId))
                                                  .length][3];
              i2 = -1;

              while (++i2 < count2) {
                strings[i1][i2][0] =
                  ProductDB.getFromCode((item = items[i2]).productId).name;
                strings[i1][i2][1] = Convert.toString(item.quant);
                strings[i1][i2][2] =
                  Convert.toString((double)item.totalAmount * 0.01, 2);
          ...
          MainWindow.getMainWindow().swap(
            SalesPlus.screens[SalesPlus.BY_PRODUCT_MENU]);
        }
        break;
