...
public abstract class BaseMenu extends Container
{
  ...
  public void initUI()
  {
    ...
    int length = ids.length,
        width =
          (int)(Settings.screenWidth * 0.75), // Buttons fill 75% of the page.
      gap = Settings.screenWidth == 160 ? 2 : 4, i = -1;
    ...
    // Places all controls inside another container so that this container can
    ...
    container.setRect(0,
                      0,
                      width,
                      1000); // Creates a container with a huge height.
    ...
    container.setRect(CENTER,
                      CENTER,
                      width,
                      button.getRect().y2() +
                        gap); // Sets the height to the correct size.
    Button.commonGap = 0;
  }
  ...
  public void back()
  {
    try {
      ...
      MainWindow.exit(
        0); // This is the last screen, so just exits the application.
    }
  }
}