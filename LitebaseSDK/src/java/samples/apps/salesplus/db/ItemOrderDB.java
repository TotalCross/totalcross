...
public class ItemOrderDB
{
  ...
  static
  {
    ...
    if (!driver.exists("itemorder")) {
      driver.execute(
        "create table itemorder(orderid int, productid char(20), quant int, unitPrice int, totalAmount int)");
    ...
    psInsert =
      driver.prepareStatement("insert into itemorder values (?, ?, ?, ?, ?)");
    psUpdate = driver.prepareStatement(
      "update itemorder set orderid = ?, productid = ?, quant = ?, unitPrice = ?, totalAmount = ? "
      + "where productid = ?");
    psDelete = driver.prepareStatement(
      "delete itemorder where productid = ? and orderid = ?");
    psDeleteAll = driver.prepareStatement("delete itemorder where orderid = ?");
    psSelectItems =
      driver.prepareStatement("select * from itemorder where orderid = ?");
    psSelectProduct =
      driver.prepareStatement("select * from itemorder where productid = ?");
  }
  ...
  /**
   * Checks if there is an order whose list has a specific product as its
   * related item. This is useful when it is desired to delete a product but it
   * is already related to an order (consistency).
   *
   * @param product The product.
   * @return <code>true</code> if there is at least one item that has the
   *     product or <code>false</code>, otherwise.
   */
