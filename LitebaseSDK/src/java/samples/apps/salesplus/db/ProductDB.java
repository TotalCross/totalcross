...
public class ProductDB
{
  ...
  static
  {
    ...
    if (!driver.exists("product")) {
      driver.execute(
        "create table product(name char(30), code char(10) primary key, descr char(50), unitprice int)");
    ...
    psInsert =
      driver.prepareStatement("insert into product values (?, ?, ?, ?)");
    psUpdate = driver.prepareStatement(
      "update product set name = ?, code = ?, descr = ?, unitprice = ? where code = ?");
    psSelectNames = driver.prepareStatement("select name from product");
    psDelete = driver.prepareStatement("delete product where code = ?");
    psSelectCode =
      driver.prepareStatement("select * from product where code = ?");
    psSelectName =
      driver.prepareStatement("select * from product where name = ?");
    psSelectCodeName = driver.prepareStatement(
      "select * from product where name = ? and code = ?");
  }
   ...
   * @return <code>1</code> if the record was updated or inserted;
   *     <code>0</code> if no record was updated.
   */
   ...
   * @return The product if found or <code>null</code> if none is found.
   */
  public static Product getFromName(String prodName)
  { // infoName
