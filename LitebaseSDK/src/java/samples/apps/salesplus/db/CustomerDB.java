...
public class CustomerDB
{
  ...
  static
  {
    ...
    if (!driver.exists("customer")) {
      driver.execute(
        "create table customer(name char(30), fedid char (15) primary key, address char (30), city char(15), state char(10), "
        +
    ...
    psUpdate = driver.prepareStatement(
      "update customer set name = ?, fedid = ?, address = ?, city = ?, state = ?, country = ?, zip = ?, "
      +
    ...
    psSelectName =
      driver.prepareStatement("select * from customer where name = ?");
  }
  ...
  public static Customer[] searchCustomers(String name)
  {
    ...
    while (resultSet.next()) {
      (customer = customers[i++] = new Customer()).name =
