...
public class OrderDB {
	...
	static {
		...
		psUpdate =
			driver.prepareStatement("update orders set customer = ?, intdate = ?, totalAmount = ? where rowid = ?");
		psSelectDate =
			driver.prepareStatement("select rowid, customer, intdate, totalamount from orders where intdate = ?");
		psSelectDays =
			driver.prepareStatement("select rowid, customer, intdate, totalamount from orders where intdate >= ? and intdate <= ?");
		psDelete = driver.prepareStatement("delete orders where rowid = ?");
		psSelectNameDate =
			driver.prepareStatement("select rowid, customer, intdate, totalamount from orders where customer = ? and intdate = ?");
		psSelectName =
			driver.prepareStatement("select rowid, customer, intdate, totalamount from orders where customer = ?");
		psSelectId =
			driver.prepareStatement("select rowid, customer, intdate, totalamount from orders where rowid = ?");
	}
