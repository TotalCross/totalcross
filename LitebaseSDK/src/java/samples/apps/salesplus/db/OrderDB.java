...
public class OrderDB
{
  ...
  /**
   * A prepared statement to select all orders of a given customer issued in a
   * given date.
   */
  ...
  static
  {
    ...
    if (!driver.exists("orders")) {
      driver.execute(
        "create table orders(customer char(20), intdate int, totalAmount int)");
    ...
    psUpdate = driver.prepareStatement(
      "update orders set customer = ?, intdate = ?, totalAmount = ? where rowid = ?");
    psSelectDate = driver.prepareStatement(
      "select rowid, customer, intdate, totalamount from orders where intdate = ?");
    psSelectDays = driver.prepareStatement(
      "select rowid, customer, intdate, totalamount from orders where intdate >= ? and intdate <= ?");
    psDelete = driver.prepareStatement("delete orders where rowid = ?");
    psSelectNameDate = driver.prepareStatement(
      "select rowid, customer, intdate, totalamount from orders where customer = ? and intdate = ?");
    psSelectName = driver.prepareStatement(
      "select rowid, customer, intdate, totalamount from orders where customer = ?");
    psSelectId = driver.prepareStatement(
      "select rowid, customer, intdate, totalamount from orders where rowid = ?");
  }
  ...
  /**
   * Removes an order order table. First all the items must be removed and just
   * then the order itself can be removed (consistency).
   *
   ...
   * @return An array with all orders of the given customer issued in the given
   *     date.
   */
  public static Order[] search(String name, String date)
   ...
   * @return <code>true</code> if the customer has issued an order;
