...
public class SQLConsole extends MainWindow
{
  ...
  /**
   * The tabbed container to alternate from the query container to the results
   * container.
   */
  ...
  private MenuItem
    miAscii; // guich@251_2: SQLConsole can now be used with ascii tables.
  ...
  public void initUI()
  {
    try {
      ...
      // The menu.
      MenuItem[] items = { new MenuItem("File"),
                           new MenuItem("Change app id"),
                           new MenuItem("Use default app id"),
                           new MenuItem(),
                           miAscii = new MenuItem(
                             "Is ascii",
                             false), // guich@251_2: SQLConsole can
                                     // now be used with ascii tables.
      ...
      cbsql = new ComboBox();
      container.add(btAdd = new Button("+"),
                    RIGHT,
                    TOP,
                    PREFERRED,
                    cbsql.getPreferredHeight());
      ...
      container.add(btnExe = new Button(
                      new Image("go.gif").smoothScaledFromResolution(320)));
      btnExe.setBorder(Button.BORDER_NONE);
      btnExe.setRect(RIGHT, AFTER + 1, PREFERRED, PREFERRED, btRem);
      container.add(btnClr = new Button(
                      new Image("clear.gif").smoothScaledFromResolution(320)));
      ...
      container.add(pbgcmd = new PushButtonGroup(new String[] { "select",
                                                                "from",
                                                                "where",
                                                                "create",
                                                                "like",
      ...
      container.add(
        pbgoper = new PushButtonGroup(OPERS, 0, 2), LEFT + 1, AFTER + 1);
      pbgoper.setFocusLess(true);
      if (tall) {
        container.add(
          pbgletters = new PushButtonGroup(
            new String[] { "a", "b", "c", "d",  "e", "f", "g", "h", "i",  "j",
                           "k", "l", "m", null, "�", "n", "o", "p", "q",  "r",
                           "s", "t", "u", "v",  "w", "x", "y", "z", null, " " },
            0,
            2),
          LEFT + 1,
          AFTER + 1);
      ...
      if (Settings.appSettings !=
          null) { // Gets the last query and displays it.
        multiEdit.setText(Settings.appSettings);
      }
      if (Settings.appSettingsBin != null) { // Populates the combobox query.
        DataStream ds =
          new DataStream(new ByteArrayStream(Settings.appSettingsBin));
        cbsql.add(ds.readStringArray());
      }
      cbsql.setSelectedIndex(0);
      addGrid(new String[] {
        "No queries executed." }); // No queries were executed yet.
      lStatus.setText("Current database id: " +
                      databaseId); // The current connection id.
  ...
  /**
   * Called just before an application exits. When this is called, all threads
   * are already killed.
   */
  public void onExit()
  {
  ...
  private void processSQL(String sql)
  {
    ...
    if (command.equals("select")) { // Select.
      ...
      try {
        ...
        // attempt to get the name of the tables used on the query to use the
        // recover table
        ...
        String fromClause = idxEndFrom != -1
                              ? sql.substring(idxStartFrom, idxEndFrom)
                              : sql.substring(idxStartFrom);
      ...
      if (rows <= 0) { // Empty answer.
        ...
        // Uses the result set meta data to get the field labels and display
        // settings.
        ResultSetMetaData rsmd = rs.getResultSetMetaData();
        int n = rsmd.getColumnCount(), cw = fm.charWidth('0'), i = 0;
        String[] titles = new String[n];
        int[] w = new int[n], a = new int[n];
    ...
    // Commands that update the database.
             ...
             command.equals("delete") || command.equals("drop") ||
             command.equals("alter")) {
  ...
  public void onEvent(Event event)
  {
    if (event.type == ControlEvent.PRESSED) {
      if (event.target == btCopyResults) {
        ...
        switch (menuBar.getSelectedIndex()) {
          case 1: { // Changes the application id.
            InputBox id = new InputBox("Database ID",
            ...
            if (!answer.equals(databaseId)) {
              if (answer.length() !=
                  4) { // juliana@227_5: The new application id of SQLConsole
          ...
          case 5:          // set/unset isascii
            connChanged(); // guich@251_2: SQLConsole can now be used with ascii
                           // tables.
            break;
      ...
      if (event.target == cbsql &&
          cbsql.getSelectedIndex() >=
            0) { // Gets the selected sql command from the combo box.
        multiEdit.setText(cbsql.getSelectedItem().toString());
        repaint();
        multiEdit.requestFocus();
      } else if (event.target == btnExe && multiEdit.getText().trim().length() >
                                             0) { // Executes the queries.
        execute();
      } else if (event.target == btAdd) { // Adds the queries from the multi
                                          // edit to the combo box.
        cbsql.add(multiEdit.getText());
        cbsql.selectLast();
      } else if (event.target == btRem &&
                 cbsql.getSelectedIndex() >=
                   0) { // Removes a block of queries from the combo box.
        ...
        multiEdit.requestFocus();
                 ...
                 tabCont) { // Changes the tab or sets the focus to it.
      ...
      // Gets the event from the command, operators o letter buttons.
      else if (event.target instanceof PushButtonGroup && getFocus() instanceof
                                                            MultiEdit) {
        ...
        if (string != null) {
          ...
          if (string.equals("�")) {
            ...
            if (pbgroup != pbgletters &&
                !(pbgroup == pbgoper &&
                  pbgroup.getSelectedIndex() >= OPERS.length - 12) &&
                !pbgroup.getSelectedItem().endsWith("_") &&
                !pbgroup.getSelectedItem().endsWith(">") &&
                !pbgroup.getSelectedItem().endsWith("<")) {
              string += " ";
            }
  ...
  /**
   * Replacement for ResultSet.getStrings used to avoid using null values on the
   * grid.
   *
   * @param rs
   * @return
