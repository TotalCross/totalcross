	...
	private void processSQL(String sql) {
		...
		if (command.equals("select")) { // Select.
			...
			try {
				...
				String fromClause = idxEndFrom != -1 ? sql.substring(idxStartFrom,
									idxEndFrom) : sql.substring(idxStartFrom);
		...
		// Commands that update the database.
				 ...
				 || command.equals("drop") || command.equals("alter")) {
	...
	public void onEvent(Event event) {
		if (event.type == ControlEvent.PRESSED) {
				...
				switch (menuBar.getSelectedIndex()) {
					case 1: { // Changes the application id.
						...
						if (!answer.equals(databaseId)) {
							if (answer.length() !=
								4) { // juliana@227_5: The new application id of SQLConsole must be 4 characters long.
			...
			if (event.target == cbsql
				&& cbsql.getSelectedIndex() >= 0) { // Gets the selected sql command from the combo box.
				multiEdit.setText(cbsql.getSelectedItem().toString());
				repaint();
				multiEdit.requestFocus();
			} else if (event.target == btnExe
					   && multiEdit.getText().trim().length() > 0) { // Executes the queries.
				execute();
				...
				cbsql.selectLast();
					   ...
					   && cbsql.getSelectedIndex() >= 0) { // Removes a block of queries from the combo box.
			...
			// Gets the event from the command, operators o letter buttons.
				...
				if (string != null) {
					...
					if (string.equals("�")) {
						...
						if (pbgroup != pbgletters && !(pbgroup == pbgoper
													   && pbgroup.getSelectedIndex() >= OPERS.length - 12)
							&& !pbgroup.getSelectedItem().endsWith("_") && !pbgroup.getSelectedItem().endsWith(">")
