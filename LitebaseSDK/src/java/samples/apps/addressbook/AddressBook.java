...
/**
 * A simple address book application that demonstrates a real case where
 * Litebase can be used. It also shows how to use the grid.
 */
public class AddressBook extends MainWindow implements Grid.DataSource
{
  ...
  public void initUI()
  {
    ...
    String[] gridCaptions = { "id",      "Name",     "Address",
                              "Phone",   "Birthday", "Salary",
                              "Married", "Gender",   "Last Updated" };
    int gridWidths[] = { 0,                                       // id
  ...
  public String[][] getItems(int startIndex, int count)
  {
    if (activeRS != null) {
      ...
      int size = matrix.length, date, time;
      long dateTime;
      ...
      try {
        while (--size >= 0) {
          ...
          sb.setLength(0);
          matrix[size][8] =
            sb.append(tempDate).append(' ').append(tempTime).toString();
        }
      } catch (InvalidNumberException exception) {
  ...
  void invalidateRS()
  {
    ...
    if (psList != null) {
      ...
      if (activeRS
            .first()) { // Maybe the result set is empty (nothing in table).
  ...
  private void createTables()
  {
    try {
      ...
      if (!driver.exists("bookentry")) {
        driver.execute(
          "create table bookentry(name char(30), address char(50), phone char(20), birthday int, salary float, married short, "
          + "gender short, lastUpdated long, photoidx short)");
        driver.execute(
          "CREATE INDEX IDX_0 ON bookentry(rowid)"); // The index names are
                                                     // completely ignored.
      }
      ...
      PreparedStatement ps =
        driver.prepareStatement("insert into photodb values (?)");
    ...
    psList = driver.prepareStatement(
      "select rowid, name, address, phone, birthday, salary, married, gender, lastUpdated, photoidx from "
      + "bookentry");
  }
}
