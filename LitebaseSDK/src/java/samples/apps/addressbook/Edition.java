...
// $Id: Edition.java,v 1.1.2.4 2011-01-12 16:34:31 juliana Exp $
...
class Edition extends Container
{
   ...
   public void initUI()
   {
      int xx = new Label("Birthday").getPreferredWidth() + 1; // Computes the max width so that all can stay at the same position.
      ...
      // birthday
      add(new Label("Birthday"), LEFT + 1, AFTER + 1);
      add(edBirth = new Edit("99/99/9999"), xx, SAME - 1);
      edBirth.setMode(Edit.DATE);
      edBirth.setMaxLength(10);
      ...
      add(pbgAction = new PushButtonGroup(new String[]{"Insert", "Update", "Delete", "Clear"}, false, -1, 0, 4, 0, false, PushButtonGroup.NORMAL), 
                                                                                                                      LEFT, AFTER + 2);
      Button.commonGap = 1; // Makes the OK button ate the same height of the action group.
      add(btnOk = new Button(" Ok "), RIGHT, SAME);
      Button.commonGap = 0;

      // status
      (lbStatus = new Label("", CENTER)).setInvert(true);
      lbStatus.setForeColor(Color.brighter(getForeColor()));
   ...
   {
      if (event.type == ControlEvent.PRESSED && event.target == btnOk)
   ...
   private void doInsertUpdate(boolean isInsert)
   {
      ...
      if (isInsert)
      {
         psInsert.setString(0, name);
         psInsert.setString(1, addr);
         psInsert.setString(2, phone);
         psInsert.setInt(3, birth);
         psInsert.setFloat(4, salary);
         psInsert.setShort(5, (short)married);
         psInsert.setShort(6, (short)gender);
         psInsert.setLong(7, lastUpdated);
         rows = psInsert.executeUpdate();
      }
      else
      {  
         psUpdate.setString(0, name);
         psUpdate.setString(1, addr);
         psUpdate.setString(2, phone);
         psUpdate.setInt(3, birth);
         psUpdate.setFloat(4, salary);
         psUpdate.setShort(5, (short)married);
         psUpdate.setShort(6, (short)gender);
         psUpdate.setLong(7, lastUpdated);
         psUpdate.setInt(8, rowId);
