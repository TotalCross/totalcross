...
class Edition extends Container {
	...
	public void initUI() {
		...
		int xx = labelAux.getPreferredWidth() +
		...
		// action
		add(pbgAction = new PushButtonGroup(new String[] {"Insert", "Update", "Delete", "Clear"}, false, -1,
											0, 4, 0, false, PushButtonGroup.NORMAL),
			LEFT, AFTER + 2);
		...
		psUpdate =
			driver.prepareStatement("update bookentry set name = ?, address = ?, phone = ?, birthday = ?, salary = ?, married = ?, gender = ?,"
	...
	private boolean verifyFields() {
		...
		if (sb.length() > 0) { // Any fields with problems?
			...
			new MessageBox("Attention", "You must fill/correct | the following fields: |" +
						   sb).popupNonBlocking();
			return false;
		}
		return true;
	...
	public void show() {
		...
		try {
			...
			StringBuffer sb = buffer;
			sb.setLength(0);
			lbStatus.setText(sb.append("Last updated: ").append(tempDate).append(' ').append(
								 timeAux).toString());
		} catch (InvalidDateException exception) {}
