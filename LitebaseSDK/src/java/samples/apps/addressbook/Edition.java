...
class Edition extends Container
{
  ...
  public void initUI()
  {
    ...
    int xx =
      labelAux.getPreferredWidth() +
      5; // Computes the max width so that all can stay at the same position.
    ...
    add(rdMale = new Radio("Male", rgGender = new RadioGroupController()),
        xx,
        SAME - 1);
    ...
    // action
    add(pbgAction = new PushButtonGroup(
          new String[] { "Insert", "Update", "Delete", "Clear" },
          false,
          -1,
          0,
          4,
    ...
    psInsert = driver.prepareStatement(
      "insert into bookentry values (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    psUpdate = driver.prepareStatement(
      "update bookentry set name = ?, address = ?, phone = ?, birthday = ?, salary = ?, married = ?, gender = ?,"
      + "lastUpdated = ?, photoidx = ? where rowid = ?");
    psSelect =
      driver.prepareStatement("select * from bookentry where rowid = ?");
   ...
   *
   * @return <code>true</code> if all the field values are valid;
  ...
  private boolean verifyFields()
  {
    ...
    if (sb.length() > 0) {           // Any fields with problems?
      ...
      new MessageBox("Attention",
                     "You must fill/correct | the following fields: |" + sb)
        .popupNonBlocking();
      return false;
    }
    return true;
        ...
        switch (pbgAction.getSelectedIndex()) {
          ...
          default:
            Sound.beep();
                 ...
                 btnPrev) { // Button prev: goes to the previous picture.
        move(-1);
      } else if (event.target ==
                 btnNext) { // Button next: goes to the next picture.
        move(1);
      }
    }
  }
  ...
  {
    String name = edName.getText(), addr = edAddress.getText(),
           phone = edPhone.getText();
    int birth = -1, married = chMarried.isChecked() ? 1 : 0,
        gender = rdMale.isChecked() ? 1 : 0, rows = -1;
    double salary = 0;
  ...
  public void show()
  {
    ...
    rgGender.setSelectedItem(
      gender.charAt(0) == '1' ? rdMale : rdFemale); // Activates the proper one.
    ...
    try {
      ...
      StringBuffer sb = buffer;
      sb.setLength(0);
      lbStatus.setText(sb.append("Last updated: ")
                         .append(tempDate)
                         .append(' ')
                         .append(timeAux)
                         .toString());
