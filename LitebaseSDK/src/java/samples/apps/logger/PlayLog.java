...
// juliana@250_9: added a new sample: LoggerSample. This is to execute a
// Litebase log file and try to reproduce a problem.
/**
      ...
      try {
        ...
        FileChooserBox fcb = new FileChooserBox(new FileChooserBox.Filter() {
          ...
          public boolean accept(File file) throws IOException
          {
            ...
            return file.isDir() ||
              (path.indexOf("LITEBASE_") >= 0 && path.endsWith(".LOGS"));
          }
        });
        ...
        if (loggerName != null && loggerName.length() > 0) {
          LineReader lineReader = new LineReader(
            new File(loggerName, File.READ_ONLY)); // Opens the log file.
          String string;
          int i;

          try { // Erases the test database if it exists.
            LitebaseConnection.dropDatabase("Test",
                                            dataPath,
                                            1); // Erases the previous tables.
          } catch (DriverException exception) {
          }

          // Tells the user that the logger is being executed.
          MessageBox pleaseWait =
            new MessageBox("Please Wait", "Processing log file...", null);
          pleaseWait.popupNonBlocking();

          while ((string = lineReader.readLine()) !=
                 null) { // Executes the possible log sql commands. Do not
                         // simulate prepared statements.
            // Transforms the string into lower case, trims it and removes
            // possible indications that it is a prepared statement.
            if ((string = string.toLowerCase().trim()).startsWith("prep: ")) {
              string = string.substring(6);
            }

            // Changes a blob indication with null. This may difficult finding
            // problems with blobs.
            ...
            try {
              if (string.startsWith("select")) {
                driver.executeQuery(string);
                         ...
                         string.startsWith("insert") ||
                         string.startsWith("delete") ||
                         string.startsWith("drop") ||
                         string.startsWith("alter")) {
                ...
                driver.setRowInc(
                  ...
                  Convert.toInt(string.substring(i + 1, string.length())));
