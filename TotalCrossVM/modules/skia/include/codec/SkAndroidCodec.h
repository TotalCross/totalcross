...
public:
   ...
   *                         This may not return |prefColorSpace| for a couple
   * reasons. (1) Android Principles: 565 must be sRGB, F16 must be linear sRGB,
   * transfer function must be parametric. (2) Codec Limitations: F16 requires a
   * linear color space.
   *
   ...
   */
  SkISize getSampledSubsetDimensions(int sampleSize,
                                     const SkIRect& subset) const;

  /**
   *  Additional options to pass to getAndroidPixels().
   */
  // FIXME: It's a bit redundant to name these AndroidOptions when this class is
  // already
  //        called SkAndroidCodec.  On the other hand, it's may be a bit
  //        confusing to call these Options when SkCodec has a slightly
  //        different set of Options.  Maybe these should be DecodeOptions or
  //        SamplingOptions?
   ...
   *
   *  The AndroidOptions object is also used to specify any requested scaling or
   * subsetting using options->fSampleSize and options->fSubset. If NULL, the
   * defaults (as specified above for AndroidOptions) are used.
   *
   *  @return Result kSuccess, or another value explaining the type of failure.
   */
  // FIXME: It's a bit redundant to name this getAndroidPixels() when this class
  // is already
  //        called SkAndroidCodec.  On the other hand, it's may be a bit
  //        confusing to call this getPixels() when it is a slightly different
  //        API than SkCodec's getPixels(). Maybe this should be decode() or
  //        decodeSubset()?
  ...
  /**
   *  Simplified version of getAndroidPixels() where we supply the default
   * AndroidOptions as specified above for AndroidOptions. It will not perform
  ...
  SkCodec::Result getPixels(const SkImageInfo& info,
                            void* pixels,
                            size_t rowBytes)
  {
...
protected:
  ...
  virtual SkCodec::Result onGetAndroidPixels(const SkImageInfo& info,
                                             void* pixels,
                                             size_t rowBytes,
                                             const AndroidOptions& options) = 0;
