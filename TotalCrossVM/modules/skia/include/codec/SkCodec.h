...
public:
   ...
   *         a no-op.
   *
   *  If a scanline decode is in progress, scanline mode will end, requiring the
   * client to call startScanlineDecode() in order to return to decoding
   * scanlines.
   *
   *  @return Result kSuccess, or another value explaining the type of failure.
   */
  Result getPixels(const SkImageInfo& info,
                   void* pixels,
                   size_t rowBytes,
                   const Options*);
   ...
   *      lines initialized. Only meaningful if this method returns
   * kIncompleteInput. Otherwise the implementation may not set it. Note that
   * some implementations may have initialized this many rows, but not
   * necessarily finished those rows (e.g. interlaced PNG). This may be useful
   * for determining what rows the client needs to initialize.
  ...
  // This transitional definition was added in August 2018, and will eventually
  // be removed.
   ...
   * number does not include the first play through of each frame. For example,
   * a repetition count of 4 means that each frame is played 5 times and then
   * the animation stops.
   *
...
protected:
  ...
  /**
   * @param rowsDecoded When the encoded image stream is incomplete, this
   * function will return kIncompleteInput and rowsDecoded will be set to the
   * number of scanlines that were successfully decoded. This will allow
   * getPixels() to fill the uninitialized memory.
   */
   ...
   *
   *  This is called by getPixels(), getYUV8Planes(), startIncrementalDecode()
   * and startScanlineDecode(). Subclasses may call if they need to rewind at
   * another time.
   */
  ...
  /**
   *  Most images types will be kTopDown and will not need to override this
   * function.
   */
...
private:
   ...
   *
   *  Will be called for the appropriate frame, prior to initializing the
   * colorXform.
   */
  virtual bool conversionSupported(const SkImageInfo& dst,
                                   bool srcIsOpaque,
  ...
  /**
   *  For multi-framed images, return the object with information about the
   * frames.
   */
  virtual const SkFrameHolder* getFrameHolder() const { return nullptr; }

  /**
   *  Check for a valid Options.fFrameIndex, and decode prior frames if
   * necessary.
   */
  Result handleFrameIndex(const SkImageInfo&,
                          void* pixels,
                          size_t rowBytes,
                          const Options&);
  ...
  virtual int onGetScanlines(void* /*dst*/,
                             int /*countLines*/,
                             size_t /*rowBytes*/)
  {
    return 0;
  ...
  /**
   * On an incomplete decode, getPixels() and getScanlines() will call this
   * function to fill any uinitialized memory.
   *
   ...
   */
  void fillIncompleteImage(const SkImageInfo& dstInfo,
                           void* dst,
                           size_t rowBytes,
