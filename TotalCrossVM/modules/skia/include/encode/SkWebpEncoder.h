...
{
  /**
   *  |fCompression| determines whether we will use webp lossy or lossless
   * compression.
   *
   *  |fQuality| must be in [0.0f, 100.0f].
   *  If |fCompression| is kLossy, |fQuality| corresponds to the visual quality
   * of the encoding.  Decreasing the quality will result in a smaller encoded
   * image. If |fCompression| is kLossless, |fQuality| corresponds to the amount
 ...
 */
SK_API bool
Encode(SkWStream* dst, const SkPixmap& src, const Options& options);
}; // namespace SkWebpEncoder
