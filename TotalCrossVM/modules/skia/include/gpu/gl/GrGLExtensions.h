...
/**
 * This helper queries the current GL context for its extensions, remembers
 * them, and can be queried. It supports both glGetString- and
 * glGetStringi-style extension string APIs and will use the latter if it is
...
public:
   ...
   * GrGLInterface. This version of init expects that getString is always
   * non-NULL while getIntegerv and getStringi are non- NULL if on desktop GL
   * with version 3.0 or higher. Otherwise it will fail.
   */
  ...
  /**
   * Queries whether an extension is present. This will fail if init() has not
   * been called.
   */
  bool has(const char[]) const;

  /**
   * Removes an extension if present. Returns true if the extension was present
   * before the call.
   */
