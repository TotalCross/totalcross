...
// It is not guarenteed VkPhysicalDeviceProperties2 will be in the client's
// header so we forward declare it here to be safe.
struct VkPhysicalDeviceFeatures2;

// The BackendContext contains all of the base Vulkan objects needed by the
// GrVkGpu. The assumption is that the client will set these up and pass them to
// the GrVkGpu constructor. The VkDevice created must support at least one
// graphics queue, which is passed in as well. The QueueFamilyIndex must match
// the family of the given queue. It is needed for CommandPool creation, and any
// GrBackendObjects handed to us (e.g., for wrapped textures) needs to be
...
struct SK_API GrVkBackendContext
{
  ...
  // The client can create their VkDevice with either a VkPhysicalDeviceFeatures
  // or VkPhysicalDeviceFeatures2 struct, thus we have to support taking both.
  // The VkPhysicalDeviceFeatures2 struct is needed so we know if the client
  // enabled any extension specific features. If fDeviceFeatures2 is not null
  // then we ignore fDeviceFeatures. If both fDeviceFeatures and
  // fDeviceFeatures2 are null we will assume no features are enabled.
  ...
  // This is deprecated and should be set to false. The client is responsible
  // for managing the lifetime of the VkInstance and VkDevice objects.
