 ...
 * GrBackendObjects for Vulkan textures are really const GrVkImageInfo*
 */
struct GrVkAlloc
{
  GrVkAlloc()
  ...
  GrVkAlloc(VkDeviceMemory memory,
            VkDeviceSize offset,
            VkDeviceSize size,
            uint32_t flags)
    : fMemory(memory)
  ...
  uint32_t fFlags;
  GrVkBackendMemory
    fBackendMemory; // handle to memory allocated via GrVkMemoryAllocator.
  ...
  {
    return fMemory == that.fMemory && fOffset == that.fOffset &&
           fSize == that.fSize && fFlags == that.fFlags &&
           fUsesSystemHeap == that.fUsesSystemHeap;
  }
...
struct GrVkImageInfo
{
  ...
  // This gives a way for a client to update the layout of the Image if they
  // change the layout while we're still holding onto the wrapped texture. They
  // will first need to get a handle to our internal GrVkImageInfo by calling
  // getTextureHandle on a GrVkTexture.
...
using GrVkGetProc =
  std::function<PFN_vkVoidFunction(const char*, // function name
