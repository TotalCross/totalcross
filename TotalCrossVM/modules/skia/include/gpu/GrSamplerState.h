...
public:
  ...
  {
    return WrapMode::kClamp != fWrapModes[0] ||
           WrapMode::kClamp != fWrapModes[1];
  }

  bool operator==(const GrSamplerState& that) const
  {
    return fWrapModes[0] == that.fWrapModes[0] &&
           fWrapModes[1] == that.fWrapModes[1] && fFilter == that.fFilter;
  }
