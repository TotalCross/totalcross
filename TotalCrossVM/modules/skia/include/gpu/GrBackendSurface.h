...
// SkSurface and SkImage rely on a minimal version of these always being
// available
...
public:
  ...
  // If the backend API is GL, these return a pointer to the format and target.
  // Otherwise it returns nullptr.
  const GrGLenum* getGLFormat() const;
  const GrGLenum* getGLTarget() const;

#ifdef SK_VULKAN
  // If the backend API is Vulkan, this returns a pointer to a VkFormat.
  // Otherwise it returns nullptr
  const VkFormat* getVkFormat() const;
#endif

#ifdef SK_METAL
  // If the backend API is Metal, this returns a pointer to a GrMTLPixelFormat.
  // Otherwise it returns nullptr
  const GrMTLPixelFormat* getMtlFormat() const;
#endif

  // If the backend API is Mock, this returns a pointer to a GrPixelConfig.
  // Otherwise it returns nullptr.
...
private:
  ...
  union
  {
    ...
    {
      GrGLenum
        fTarget; // GL_TEXTURE_2D, GL_TEXTURE_EXTERNAL or GL_TEXTURE_RECTANGLE
...
public:
  ...
  // If the backend API is GL, copies a snapshot of the GrGLTextureInfo struct
  // into the passed in pointer and returns true. Otherwise returns false if the
  // backend API is not GL.
  bool getGLTextureInfo(GrGLTextureInfo*) const;

#ifdef SK_VULKAN
  // If the backend API is Vulkan, copies a snapshot of the GrVkImageInfo struct
  // into the passed in pointer and returns true. This snapshot will set the
  // fImageLayout to the current layout state. Otherwise returns false if the
  // backend API is not Vulkan.
  bool getVkImageInfo(GrVkImageInfo*) const;

  // Anytime the client changes the VkImageLayout of the VkImage captured by
  // this GrBackendTexture, they must call this function to notify Skia of the
  // changed layout.
  void setVkImageLayout(VkImageLayout);
#endif

#ifdef SK_METAL
  // If the backend API is Metal, copies a snapshot of the GrMtlTextureInfo
  // struct into the passed in pointer and returns true. Otherwise returns false
  // if the backend API is not Metal.
  bool getMtlTextureInfo(GrMtlTextureInfo*) const;
#endif

  // If the backend API is Mock, copies a snapshot of the GrMockTextureInfo
  // struct into the passed in pointer and returns true. Otherwise returns false
  // if the backend API is not Mock.
...
#if GR_TEST_UTILS
  // We can remove the pixelConfig getter and setter once we remove the
  // GrPixelConfig from the GrBackendTexture and plumb the GrPixelconfig
  // manually throughout our code (or remove all use of GrPixelConfig in
  // general).
...
public:
  ...
  // Anytime the client changes the VkImageLayout of the VkImage captured by
  // this GrBackendRenderTarget, they must call this function to notify Skia of
  // the changed layout.
  void setVkImageLayout(VkImageLayout);
#endif
...
#if GR_TEST_UTILS
  // We can remove the pixelConfig getter and setter once we remove the pixel
  // config from the GrBackendRenderTarget and plumb the pixel config manually
  // throughout our code (or remove all use of GrPixelConfig in general).
  GrPixelConfig pixelConfig() const { return fConfig; }
  void setPixelConfig(GrPixelConfig config) { fConfig = config; }

  static bool TestingOnly_Equals(const GrBackendRenderTarget&,
