...
uint32_t
GrResourceKeyHash(const uint32_t* data, size_t size);

/**
 * Base class for all GrGpuResource cache keys. There are two types of cache
 * keys. Refer to the comments for each key type below.
 */
class GrResourceKey
{
public:
...
protected:
  ...
  public:
    ...
    {
      SkASSERT(fKey);
      SkDEBUGCODE(size_t dataCount =
                    fKey->internalSize() / sizeof(uint32_t) - kMetaDataCnt;)
...
/**
 * A key used for scratch resources. There are three important rules about
 * scratch keys:
 *        * Multiple resources can share the same scratch key. Therefore
 * resources assigned the same scratch key should be interchangeable with
 * respect to the code that uses them.
 *        * A resource can have at most one scratch key and it is set at
 * resource creation by the resource itself.
 *        * When a scratch resource is ref'ed it will not be returned from the
 *          cache for a subsequent cache request until all refs are released.
 * This facilitates using a scratch key for multiple render-to-texture
 * scenarios. An example is a separable blur:
 *
 *  GrTexture* texture[2];
 *  texture[0] = get_scratch_texture(scratchKey);
 *  texture[1] = get_scratch_texture(scratchKey); // texture[0] is already owned
 * so we will get a
 *                                                // different one for
 * texture[1] draw_mask(texture[0], path);        // draws path mask to
 * texture[0] blur_x(texture[0], texture[1]);     // blurs texture[0] in y and
 * stores result in texture[1] blur_y(texture[1], texture[0]);     // blurs
 * texture[1] in y and stores result in texture[0] texture[1]->unref();  //
 * texture 1 can now be recycled for the next request with scratchKey
 *  consume_blur(texture[0]);
 *  texture[0]->unref();  // texture 0 can now be recycled for the next request
 * with scratchKey
 */
...
public:
  ...
  /** Creates an invalid scratch key. It must be initialized using a Builder
   * object before use. */
  GrScratchKey() {}
...
/**
 * A key that allows for exclusive use of a resource for a use case (AKA
 * "domain"). There are three rules governing the use of unique keys:
 *        * Only one resource can have a given unique key at a time. Hence,
 * "unique".
 *        * A resource can have at most one unique key at a time.
 *        * Unlike scratch keys, multiple requests for a unique key will return
 * the same resource even if the resource already has refs. This key type allows
 * a code path to create cached resources for which it is the exclusive user.
 * The code path creates a domain which it sets on its keys. This guarantees
 * that there are no cross-domain collisions.
 *
 * Unique keys preempt scratch keys. While a resource has a unique key it is
 * inaccessible via its scratch key. It can become scratch again if the unique
 * key is removed.
 */
...
public:
  ...
  /** Creates an invalid unique key. It must be initialized using a Builder
   * object before use. */
  GrUniqueKey()
    : fTag(nullptr)
  {}
  ...
  public:
    ...
    Builder(GrUniqueKey* key,
            ...
            int extraData32Cnt,
            const char* tag = nullptr)
      : INHERITED::Builder(key,
                           domain,
                           Data32CntForInnerKey(innerKey) + extraData32Cnt)
    {
      SkASSERT(&innerKey != key);
      // add the inner key to the end of the key so that op[] can be indexed
      // normally.
...
/**
 * It is common to need a frequently reused GrUniqueKey where the only
 * requirement is that the key is unique. These macros create such a key in a
 * thread safe manner so the key can be truly global and only constructed once.
 */
...
#define GR_DEFINE_STATIC_UNIQUE_KEY(name)                                      \
  ...
  static const GrUniqueKey& name =                                             \
    *reinterpret_cast<GrUniqueKey*>(name##_storage.get());
...
public:
  ...
  GrUniqueKeyInvalidatedMessage& operator=(
    const GrUniqueKeyInvalidatedMessage&) = default;
