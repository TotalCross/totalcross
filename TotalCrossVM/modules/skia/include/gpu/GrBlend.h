...
GrBlendCoeffRefsSrc(const GrBlendCoeff coeff)
{
  return kSC_GrBlendCoeff == coeff || kISC_GrBlendCoeff == coeff ||
         kSA_GrBlendCoeff == coeff || kISA_GrBlendCoeff == coeff;
}

static constexpr bool
GrBlendCoeffRefsDst(const GrBlendCoeff coeff)
{
  return kDC_GrBlendCoeff == coeff || kIDC_GrBlendCoeff == coeff ||
         kDA_GrBlendCoeff == coeff || kIDA_GrBlendCoeff == coeff;
}

static constexpr bool
GrBlendCoeffRefsSrc2(const GrBlendCoeff coeff)
{
...
static constexpr bool
GrBlendCoeffsUseSrcColor(GrBlendCoeff srcCoeff, GrBlendCoeff dstCoeff)
{
  return kZero_GrBlendCoeff != srcCoeff || GrBlendCoeffRefsSrc(dstCoeff);
}

static constexpr bool
GrBlendCoeffsUseDstColor(GrBlendCoeff srcCoeff, GrBlendCoeff dstCoeff)
{
  return GrBlendCoeffRefsDst(srcCoeff) || kZero_GrBlendCoeff != dstCoeff;
}

static constexpr bool
GrBlendEquationIsAdvanced(GrBlendEquation equation)
{
  return equation >= kFirstAdvancedGrBlendEquation;
}

static constexpr bool
GrBlendModifiesDst(GrBlendEquation equation,
                   GrBlendCoeff srcCoeff,
                   GrBlendCoeff dstCoeff)
{
  return (kAdd_GrBlendEquation != equation &&
          kReverseSubtract_GrBlendEquation != equation) ||
         kZero_GrBlendCoeff != srcCoeff || kOne_GrBlendCoeff != dstCoeff;
}

/**
 * Advanced blend equations can always tweak alpha for coverage. (See
 ...
 *
 * (Let srcCoeff be negative for reverse subtract.) We can tweak alpha for
 *coverage when the following relationship holds:
 *
 *   (f*S) * srcCoeff' + D * dstCoeff' == f * S * srcCoeff + D * (f * dstCoeff +
 *(1 - f))
 *
 * (Where srcCoeff' and dstCoeff' have any reference to S pre-multiplied by f.)
 *
 * It's easy to see this works for the src term as long as srcCoeff' == srcCoeff
 *(meaning srcCoeff does not reference S). For the dst term, this will work as
 *long as the following is true:
 *|
 *   dstCoeff' == f * dstCoeff + (1 - f)
 *   dstCoeff' == 1 - f * (1 - dstCoeff)
 *
 * By inspection we can see this will work as long as dstCoeff has a 1, and any
 *other term in dstCoeff references S.
 *
 * Moreover, if the blend doesn't modify the dst at all then it is ok to
 *arbitrarily modify the src color so folding in coverage is allowed.
 */
static constexpr bool
GrBlendAllowsCoverageAsAlpha(GrBlendEquation equation,
                             GrBlendCoeff srcCoeff,
                             GrBlendCoeff dstCoeff)
{
