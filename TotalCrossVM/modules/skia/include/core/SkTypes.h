...
*/
SK_API extern void
sk_abort_no_print(void);

#ifndef SkDebugf
SK_API void
SkDebugf(const char format[], ...);
#endif

// SkASSERT, SkASSERTF and SkASSERT_RELEASE can be used as stand alone assertion
// expressions, e.g.
//    uint32_t foo(int x) {
//        SkASSERT(x > 4);
//        return x - 4;
...
static inline constexpr SkFourByteTag
SkSetFourByteTag(char a, char b, char c, char d)
{
...
template<typename T>
constexpr const T&
SkTClamp(const T& x, const T& lo, const T& hi)
{
...
SkTPin(const T& value, const T& min, const T& max)
{
