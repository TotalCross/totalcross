...
/* Generated by tools/bookmaker from include/core/SkPicture.h and
   docs/SkPicture_Reference.bmh on 2018-08-10 12:59:44. Additional documentation
   and examples can be found at: https://skia.org/user/api/SkPicture_Reference

   You may edit either file directly. Structural changes to public interfaces
   require editing both files. After editing docs/SkPicture_Reference.bmh, run:
...
/** \class SkPicture
    SkPicture records drawing commands made to SkCanvas. The command stream may
   be played in whole or in part at a later time.

    SkPicture is an abstract class. SkPicture may be generated by
   SkPictureRecorder or SkDrawable, or from SkPicture previously saved to SkData
   or SkStream.
...
{
public:
  /** Recreates SkPicture that was serialized into a stream. Returns constructed
     SkPicture if successful; otherwise, returns nullptr. Fails if data does not
     permit constructing valid SkPicture.

      procs.fPictureProc permits supplying a custom function to decode
     SkPicture. If procs.fPictureProc is nullptr, default decoding is used.
     procs.fPictureCtx may be used to provide user context to
     procs.fPictureProc; procs.fPictureProc is called with a pointer to data,
     data byte length, and user context.
  ...
  */
  ...
  /** Recreates SkPicture that was serialized into data. Returns constructed
     SkPicture if successful; otherwise, returns nullptr. Fails if data does not
     permit constructing valid SkPicture.
  ...
  */
  ...
  /** \class SkPicture::AbortCallback
      ...
      passed as a parameter to SkPicture::playback, to stop it before all
  ...
  public:
    ...
    */
    ...
    /** Stops SkPicture playback when some condition is met. A subclass of
        AbortCallback provides an override for abort() that can stop
       SkPicture::playback.

        The part of SkPicture drawn when aborted is undefined. SkPicture
       instantiations are free to stop drawing at different points during
       playback.

        If the abort happens inside one or more calls to SkCanvas::save(), stack
        of SkCanvas matrix and SkCanvas clip values is restored to its state
       before SkPicture::playback was called.
  ...
  */
  virtual void playback(SkCanvas* canvas,
                        AbortCallback* callback = nullptr) const = 0;

  /** Returns cull SkRect for this picture, passed in when SkPicture was
     created. Returned SkRect does not specify clipping SkRect for SkPicture;
     cull is hint of SkPicture bounds.
  ...
  */
  ...
  /** Returns storage containing SkData describing SkPicture, using optional
     custom encoders.

      procs.fPictureProc permits supplying a custom function to encode
     SkPicture. If procs.fPictureProc is nullptr, default encoding is used.
     procs.fPictureCtx may be used to provide user context to
     procs.fPictureProc; procs.fPictureProc is called with a pointer to
     SkPicture and user context.
  ...
  */
  ...
  /** Returns a placeholder SkPicture. Result does not draw, and contains only
      cull SkRect, a hint of its bounds. Result is immutable; it cannot be
     changed later. Result identifier is unique.
...
private:
  ...
  void serialize(SkWStream*,
                 const SkSerialProcs*,
                 class SkRefCntSet* typefaces) const;
  ...
  static_assert(MIN_PICTURE_VERSION <= 62,
                "Remove kFontAxes_bad from SkFontDescriptor.cpp");
