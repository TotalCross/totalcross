...
public:
   ...
   *  Setting this to a large value may speed up drawing larger text
   * (repeatedly), but could cause the cache to purge other sizes more often.
   *
   *  This value is a hint to the font engine, and the actual limit may be
   * different due to implementation specific details.
   */
  static int GetFontCachePointSizeLimit();

  /*
   *  Set the maximum point size for text that may be cached, returning the
   * previous value.
   *
   *  Sizes above this will be drawn directly from the font's outline.
   *  Setting this to a large value may speed up drawing larger text
  ...
  /**
   *  These functions get/set the memory usage limit for the resource cache,
   * used for temporary bitmaps and other resources. Entries are purged from the
   * cache when the memory useage exceeds this limit.
   */
   ...
   * avoid the, the client can set a limit for a single allocation. If a
   * cacheable entry would have been cached, but its size exceeds this limit,
   * then we do not attempt to cache it at all.
   *
  ...
  /**
   *  Dumps memory usage of caches using the SkTraceMemoryDump interface. See
   * SkTraceMemoryDump for usage of this method.
   */
   ...
   *
   *  If there are caches associated with GPU context, those will not be
  ...
  /**
   *  To instantiate images from encoded data, first looks at this runtime
   * function-ptr. If it exists, it is called to create an SkImageGenerator from
   * SkData. If there is no function-ptr or there is, but it returns NULL, then
   * skia will call its internal default implementation.
   *
