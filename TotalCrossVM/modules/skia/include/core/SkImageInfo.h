...
/* Generated by tools/bookmaker from include/core/SkImageInfo.h and
   docs/SkImageInfo_Reference.bmh on 2018-07-13 08:15:11. Additional
   ...
   require editing both files. After editing docs/SkImageInfo_Reference.bmh,
   run: bookmaker -b docs -i include/core/SkImageInfo.h -p to create an updated
...
/** Returns true if SkAlphaType equals kOpaque_SkAlphaType. kOpaque_SkAlphaType
   is a hint that the SkColorType is opaque, or that all alpha values are set to
    their 1.0 equivalent. If SkAlphaType is kOpaque_SkAlphaType, and SkColorType
   is not opaque, then the result of drawing any pixel with a alpha value less
...
*/
static inline bool
SkAlphaTypeIsOpaque(SkAlphaType at)
{
...
/** Temporary macro that allows us to add new color types without breaking
 * Chrome compile. */
...
enum SkColorType
{
  ...
  kRGB_565_SkColorType,   //!< pixel with 5 bits red, 6 bits green, 5 bits blue,
                          //!< in 16-bit word
  kARGB_4444_SkColorType, //!< pixel with 4 bits for alpha, red, green, blue; in
                          //!< 16-bit word
  kRGBA_8888_SkColorType, //!< pixel with 8 bits for red, green, blue, alpha; in
                          //!< 32-bit word
  kRGB_888x_SkColorType,  //!< pixel with 8 bits each for red, green, blue; in
                          //!< 32-bit word
  kBGRA_8888_SkColorType, //!< pixel with 8 bits for blue, green, red, alpha; in
                          //!< 32-bit word
  kRGBA_1010102_SkColorType, //!< 10 bits for red, green, blue; 2 bits for
                             //!< alpha; in 32-bit word
  kRGB_101010x_SkColorType,  //!< pixel with 10 bits each for red, green, blue;
                             //!< in 32-bit word
  kGray_8_SkColorType,       //!< pixel with grayscale level in 8-bit byte
  kRGBA_F16_SkColorType,     //!< pixel with half floats for red, green, blue,
                             //!< alpha; in 64-bit word
  kRGBA_F32_SkColorType, //!< pixel using C float for red, green, blue, alpha;
                         //!< in 128-bit word
...
/** Returns the number of bytes required to store a pixel, including unused
   padding. Returns zero if ct is kUnknown_SkColorType or invalid.
...
*/
SK_API bool
SkColorTypeIsAlwaysOpaque(SkColorType ct);

/** Returns true if canonical can be set to a valid SkAlphaType for colorType.
   If there is more than one valid canonical SkAlphaType, set to alphaType, if
   valid. If true is returned and canonical is not nullptr, store valid
   SkAlphaType.

    Returns false only if alphaType is kUnknown_SkAlphaType, color type is not
    kUnknown_SkColorType, and SkColorType is not always opaque. If false is
   returned, canonical is ignored.

    For kUnknown_SkColorType: set canonical to kUnknown_SkAlphaType and return
   true. For kAlpha_8_SkColorType: set canonical to kPremul_SkAlphaType or
    kOpaque_SkAlphaType and return true if alphaType is not
   kUnknown_SkAlphaType. For kRGB_565_SkColorType, kRGB_888x_SkColorType,
   kRGB_101010x_SkColorType, and kGray_8_SkColorType: set canonical to
   kOpaque_SkAlphaType and return true. For kARGB_4444_SkColorType,
   kRGBA_8888_SkColorType, kBGRA_8888_SkColorType, kRGBA_1010102_SkColorType,
   and kRGBA_F16_SkColorType: set canonical to alphaType and return true if
   alphaType is not kUnknown_SkAlphaType.
...
*/
SK_API bool
SkColorTypeValidateAlphaType(SkColorType colorType,
                             SkAlphaType alphaType,
                             SkAlphaType* canonical = nullptr);

/** \enum SkImageInfo::SkYUVColorSpace
    Describes color range of YUV pixels. The color mapping from YUV to RGB
   varies depending on the source. YUV pixels may be generated by JPEG images,
   standard video streams, or high definition video streams. Each has its own
   mapping from YUV and RGB.
...
/** \struct SkImageInfo
    Describes pixel dimensions and encoding. SkBitmap, SkImage, PixMap, and
   SkSurface can be created from SkImageInfo. SkImageInfo can be retrieved from
   SkBitmap and SkPixmap, but not from SkImage and SkSurface. For example,
   SkImage and SkSurface implementations may defer pixel depth, so may not
   completely specify SkImageInfo.
...
public:
  /** Creates an empty SkImageInfo with kUnknown_SkColorType,
     kUnknown_SkAlphaType, a width and height of zero, and no SkColorSpace.
  ...
  */
  ...
  /** Creates SkImageInfo from integral dimensions width and height, SkColorType
     ct, SkAlphaType at, and optionally SkColorSpace cs.

      If SkColorSpace cs is nullptr and SkImageInfo is part of drawing source:
  ...
  */
  static SkImageInfo Make(int width,
                          int height,
                          SkColorType ct,
                          SkAlphaType at,
  ...
  /** Creates SkImageInfo from integral dimensions width and height,
     ...
     kN32_SkColorType will equal either kBGRA_8888_SkColorType or
     kRGBA_8888_SkColorType, whichever is optimal.
     ...
     kPremul_SkAlphaType, kUnpremul_SkAlphaType
      ...
      @return        created SkImageInfo
  */
  static SkImageInfo MakeN32(int width,
                             int height,
                             SkAlphaType at,
                             sk_sp<SkColorSpace> cs = nullptr)
  {
  ...
  */
  ...
  /** Creates SkImageInfo from integral dimensions width and height,
     ...
     SkColorSpace defaults to sRGB, mapping into SkSurface SkColorSpace.
      ...
      @return        created SkImageInfo
  */
  static SkImageInfo MakeN32Premul(int width,
                                   int height,
                                   sk_sp<SkColorSpace> cs = nullptr)
  {
    return Make(
      width, height, kN32_SkColorType, kPremul_SkAlphaType, std::move(cs));
  }

  /** Creates SkImageInfo from integral dimensions width and height,
     kN32_SkColorType, kPremul_SkAlphaType, with SkColorSpace set to nullptr.

      If SkImageInfo is part of drawing source: SkColorSpace defaults to sRGB,
     mapping into SkSurface SkColorSpace.
  ...
  */
  ...
  {
    return Make(
      width, height, kAlpha_8_SkColorType, kPremul_SkAlphaType, nullptr);
  }
  ...
  */
  ...
  {
    return Make(
      width, height, kUnknown_SkColorType, kUnknown_SkAlphaType, nullptr);
  }
  ...
  */
  ...
  /** Returns smart pointer to SkColorSpace, the range of colors. The smart
     pointer tracks the number of objects sharing this SkColorSpace reference so
     the memory is released when the owners destruct.
  ...
  */
  ...
  /** Returns true if associated SkColorSpace is not nullptr, and SkColorSpace
     gamma is approximately the same as sRGB. This includes the
  ...
  */
  ...
  {
    return Make(
      this->width(), this->height(), fColorType, newAlphaType, fColorSpace);
  }
  ...
  */
  ...
  {
    return Make(
      this->width(), this->height(), newColorType, fAlphaType, fColorSpace);
  }
  ...
  */
  ...
  {
    return Make(
      this->width(), this->height(), fColorType, fAlphaType, std::move(cs));
  }
  ...
  */
  ...
  /** Returns minimum bytes per row, computed from pixel width() and
     SkColorType, which specifies bytesPerPixel(). SkBitmap maximum value for
     row bytes must fit in 31 bits.
  ...
  */
  ...
  /** Returns byte offset of pixel from pixel base address.
      ...
      rowBytes is smaller than minRowBytes(), even though result may be
     incorrect.
  ...
  */
  ...
  /** Returns storage required by pixel array, given SkImageInfo dimensions,
     SkColorType, and rowBytes. rowBytes is assumed to be at least as large as
     minRowBytes().
  ...
  */
  ...
  /** Returns true if byteSize equals SIZE_MAX. computeByteSize() and
      ...
      @return          true if computeByteSize() or computeMinByteSize() result
     exceeds size_t
  */
