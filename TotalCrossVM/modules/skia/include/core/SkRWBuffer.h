...
public:
  ...
  public:
    ...
    /**
     *  Returns the number of bytes in the current continguous block of memory,
...
private:
  SkROBuffer(const SkBufferHead* head,
             size_t available,
 ...
 * thread, an RBuffer or StreamAsset can be snapped off (and safely passed to
 * another thread). The RBuffer/StreamAsset snapshot can see the previously
