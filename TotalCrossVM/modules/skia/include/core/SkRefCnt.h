...
public:
  ...
  void validate() const { SkASSERT(getRefCnt() > 0); }
#endif
...
// SkRefCnt. This SkRefCnt should normally derive from SkRefCntBase.
...
/** Check if the argument is non-null, and if so, call obj->ref() and return
 * obj.
 */
template<typename T>
static inline T*
...
// of 8 or 16. There's only benefit to using this if the deriving class does not
// otherwise need a vtable.
...
public:
  ...
  //   - unref() needs a release barrier, and an acquire if it's going to call
  //   delete.
 ...
 *
 *  This can be used for classes inheriting from SkRefCnt, but it also works for
 * other classes that match the interface, but have different internal choices:
 * e.g. the hosted class may have its ref/unref be thread-safe, but that is not
 * assumed/imposed by sk_sp.
 */
template<typename T>
class sk_sp
{
public:
  ...
  /**
   *  Shares the underlying object by calling ref(), so that both the argument
   * and the newly created sk_sp both have a reference to it.
   */
   ...
   * Afterwards only the new sk_sp will have a reference to the object, and the
   * argument will point to null. No call to ref() or unref() will be made.
   */
  sk_sp(sk_sp<T>&& that)
    : fPtr(that.release())
  {}
  ...
  /**
   *  Shares the underlying object referenced by the argument by calling ref()
   * on it. If this sk_sp previously had a reference to an object (i.e. not
   * null) it will call unref() on that object.
   */
  ...
  /**
   *  Move the underlying object from the argument to the sk_sp. If the sk_sp
   * previously held a reference to another object, unref() will be called on
   * that object. No call to ref() will be made.
   */
  ...
  /**
   *  Adopt the new bare pointer, and call unref() on any previously held object
   * (if not null). No call to ref() will be made.
   */
   ...
   *  The caller must assume ownership of the object, and manage its reference
   * count directly. No call to unref() will be made.
   */
...
inline bool
operator==(const sk_sp<T>& a, const sk_sp<U>& b)
{
  return a.get() == b.get();
}
template<typename T>
inline bool
operator==(const sk_sp<T>& a, std::nullptr_t)
{ /*noexcept*/
  return !a;
}
template<typename T>
inline bool
operator==(std::nullptr_t, const sk_sp<T>& b)
{ /*noexcept*/
  return !b;
}
...
auto
operator<<(std::basic_ostream<C, CT>& os, const sk_sp<T>& sp)
  -> decltype(os << sp.get())
{
  return os << sp.get();
}
 ...
 *
 *  This is different than the semantics of the constructor for sk_sp, which
 * just wraps the ptr, effectively "adopting" it.
 */
template<typename T>
sk_sp<T>
sk_ref_sp(T* obj)
