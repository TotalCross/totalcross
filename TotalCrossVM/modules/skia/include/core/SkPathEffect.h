...
public:
   ...
   *
   */
  static sk_sp<SkPathEffect> MakeSum(sk_sp<SkPathEffect> first,
   ...
   */
  static sk_sp<SkPathEffect> MakeCompose(sk_sp<SkPathEffect> outer,
  ...
  */
  ...
  /**
   *  If the PathEffect can be represented as a dash pattern, asADash will
   * return kDash_DashType and None otherwise. If a non NULL info is passed in,
   * the various DashInfo will be filled in if the PathEffect can be a dash
   * pattern. If passed in info has an fCount equal or greater to that of the
   * effect, it will memcpy the values of the dash intervals into the info. Thus
   * the general approach will be call asADash once with default info to get
   * DashType and fCount. If effect can be represented as a dash pattern,
   * allocate space for the intervals in info, then call asADash again with the
   * same info and the intervals will get copied in.
   */
  ...
  static sk_sp<SkPathEffect> Deserialize(const void* data,
                                         size_t size,
                                         const SkDeserialProcs* procs = nullptr)
  {
    return sk_sp<SkPathEffect>(static_cast<SkPathEffect*>(
      SkFlattenable::Deserialize(kSkPathEffect_Type, data, size, procs)
        .release()));
  }
...
protected:
  ...
  virtual bool onAsPoints(PointData*,
                          const SkPath&,
                          const SkStrokeRec&,
                          const SkMatrix&,
                          const SkRect*) const
