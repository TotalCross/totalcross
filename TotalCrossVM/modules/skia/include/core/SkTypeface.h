...
public:
   ...
   * coordinates.
   *  @param coordinateCount the number of entries available through
   * 'coordinates'.
   *
   *  @return The number of axes, or -1 if there is an error.
   *  If 'coordinates != nullptr' and 'coordinateCount >= numAxes' then
   * 'coordinates' will be filled with the variation coordinates describing the
   * position of this typeface in design variation space. It is possible the
   * number of axes can be retrieved but actual position cannot.
   */
   ...
   * parameters.
   *
   *  @param parameters the buffer into which to write the design variation
   * parameters.
   *  @param coordinateCount the number of entries available through
   * 'parameters'.
   *
   *  @return The number of axes, or -1 if there is an error.
   *  If 'parameters != nullptr' and 'parameterCount >= numAxes' then
   * 'parameters' will be filled with the variation parameters describing the
   * position of this typeface in design variation space. It is possible the
   * number of axes can be retrieved but actual parameters cannot.
   */
  ...
  */
  ...
  /** Return a new typeface given a SkData. If the data is null, or is not a
  ...
  */
  ...
  /** Return a new typeface based on this typeface but parameterized as
     specified in the SkFontArguments. If the SkFontArguments does not supply an
     argument for a parameter in the font then the value from this typeface will
     be used as the value for that argument. If the cloned typeface would be
     exaclty the same as this typeface then this typeface may be ref'ed and
     returned. May return nullptr on failure.
  */
  sk_sp<SkTypeface> makeClone(const SkFontArguments&) const;

  /**
   *  A typeface can serialize just a descriptor (names, etc.), or it can also
   * include the actual font data (which can be large). This enum controls how
   * serialize() decides what to serialize.
   */
  ...
  void serialize(
    SkWStream*,
    SerializeBehavior = SerializeBehavior::kIncludeDataIfLocal) const;
   ...
   */
  int charsToGlyphs(const void* chars,
                    Encoding encoding,
                    SkGlyphID glyphs[],
   ...
   *  then 0 is returned.
   */
  size_t getTableData(SkFontTableTag tag,
                      size_t offset,
                      size_t length,
                      void* data) const;
   ...
   */
  bool getKerningPairAdjustments(const SkGlyphID glyphs[],
                                 int count,
...
protected:
  ...
  virtual std::unique_ptr<SkAdvancedTypefaceMetrics> onGetAdvancedMetrics()
    const;
  // For type1 postscript fonts only, set the glyph names for each glyph.
  // destination array is non-null, and points to an array of size
  // this->countGlyphs(). Backends that do not suport type1 fonts should not
  // override.
  ...
  virtual bool onGetKerningPairAdjustments(const SkGlyphID glyphs[],
                                           int count,
                                           int32_t adjustments[]) const;
  ...
  virtual size_t onGetTableData(SkFontTableTag,
                                size_t offset,
                                size_t length,
                                void* data) const = 0;
