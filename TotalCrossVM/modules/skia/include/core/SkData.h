...
public:
   ...
   * modifying the contents when another client/thread has access to the data.
   */
  void* writable_data()
  {
    if (fSize) {
  ...
  /**
   *  Create a new data with uninitialized contents. The caller should call
   * writable_data() to write into the buffer, but this must be done before
   * another ref() is made.
   */
   ...
   */
  static sk_sp<SkData> MakeWithProc(const void* ptr,
                                    size_t length,
                                    ReleaseProc proc,
                                    void* ctx);
   ...
   * lifetime of the SkData. Suitable for with const globals.
   */
  ...
  /**
   *  Attempt to read size bytes into a SkData. If the read succeeds, return the
   ...
   *  src dataref.
   */
  static sk_sp<SkData> MakeSubset(const SkData* src,
                                  size_t offset,
                                  size_t length);
