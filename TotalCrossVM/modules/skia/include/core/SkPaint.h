...
/* Generated by tools/bookmaker from include/core/SkPaint.h and
   docs/SkPaint_Reference.bmh on 2018-08-28 10:32:58. Additional documentation
   and examples can be found at: https://skia.org/user/api/SkPaint_Reference

   You may edit either file directly. Structural changes to public interfaces
   require editing both files. After editing docs/SkPaint_Reference.bmh, run:
...
/** \class SkPaint
    SkPaint controls options applied when drawing and measuring. SkPaint
   collects all options outside of the SkCanvas clip and SkCanvas matrix.

    Various options apply to text, strokes and fills, and images.

    Some options may not be implemented on all platforms; in these cases,
   setting the option has no effect. Some options are conveniences that
   duplicate SkCanvas functionality; for instance, text size is identical to
   matrix scale.

    SkPaint options are rarely exclusive; each option modifies a stage of the
   drawing pipeline and multiple pipeline stages may be affected by a single
   SkPaint.
   ...
   blur, but contains the objects that do so.
    ...
    The objects contained by SkPaint are opaque, and cannot be edited outside of
   the SkPaint to affect it. The implementation is free to defer computations
   associated with the SkPaint, or ignore them altogether. For instance, some
   GPU implementations draw all SkPath geometries with anti-aliasing, regardless
   of how SkPaint::kAntiAlias_Flag is set in SkPaint.

    SkPaint describes a single color, a single font, a single image quality, and
   so on. Multiple colors are drawn either by using multiple paints or with
   objects like SkShader attached to SkPaint.
*/
...
public:
  ...
  */
  ...
  /** Makes a shallow copy of SkPaint. SkTypeface, SkPathEffect, SkShader,
      ...
      between the original paint and the copy. Objects containing SkRefCnt
     increment their references by one.

      The referenced objects SkPathEffect, SkShader, SkMaskFilter,
     SkColorFilter, SkDrawLooper, and SkImageFilter cannot be modified after
     they are created. This prevents objects with SkRefCnt from being modified
     once SkPaint refers to them.
  ...
  */
  ...
  /** Decreases SkPaint SkRefCnt of owned objects: SkTypeface, SkPathEffect,
     SkShader, SkMaskFilter, SkColorFilter, SkDrawLooper, and SkImageFilter. If
     the objects containing SkRefCnt go to zero, they are deleted.
  */
  ~SkPaint();

  /** Makes a shallow copy of SkPaint. SkTypeface, SkPathEffect, SkShader,
      SkMaskFilter, SkColorFilter, SkDrawLooper, and SkImageFilter are shared
      between the original paint and the copy. Objects containing SkRefCnt in
     the prior destination are decreased by one, and the referenced objects are
     deleted if the resulting count is zero. Objects containing SkRefCnt in the
     parameter paint are increased by one. paint is unmodified.
  ...
  */
  ...
  /** Moves the paint to avoid increasing the reference counts
      of objects referenced by the paint parameter. Objects containing SkRefCnt
     in the prior destination are decreased by one; those objects are deleted if
  ...
  */
  ...
  /** Compares a and b, and returns true if a and b are equivalent. May return
     false if SkTypeface, SkPathEffect, SkShader, SkMaskFilter, SkColorFilter,
      SkDrawLooper, or SkImageFilter have identical contents but different
     pointers.
  ...
  */
  ...
  /** Compares a and b, and returns true if a and b are not equivalent. May
     return true if SkTypeface, SkPathEffect, SkShader, SkMaskFilter,
     SkColorFilter, SkDrawLooper, or SkImageFilter have identical contents but
     different pointers.
  ...
  */
  ...
  /** Returns a hash generated from SkPaint values and pointers.
      ...
      equivalent, but differing hashes do not guarantee that the paints have
     differing contents.

      If operator==(const SkPaint& a, const SkPaint& b) returns true for two
     paints, their hashes are also equal.
  ...
  */
  ...
  /** Sets all SkPaint contents to their initial values. This is equivalent to
     replacing SkPaint with the result of SkPaint().
  */
  void reset();

  /** \enum SkPaint::Hinting
      Hinting adjusts the glyph outlines so that the shape provides a uniform
      look at a given point size on font engines that support it. Hinting may
     have a muted effect or no effect at all depending on the platform.

      The four levels roughly control corresponding features on platforms that
     use FreeType as the font engine.
  */
  enum Hinting
  {
  ...
  /** Returns level of glyph outline adjustment.
      ...
      @return  one of: kNo_Hinting, kSlight_Hinting, kNormal_Hinting,
     kFull_Hinting
  */
  ...
  /** Sets level of glyph outline adjustment.
      ...
      @param hintingLevel  one of: kNo_Hinting, kSlight_Hinting,
     kNormal_Hinting, kFull_Hinting
  */
  ...
  */
  enum Flags
  {
    ...
    kEmbeddedBitmapText_Flag =
      0x400,                     //!< mask for setting font embedded bitmaps
  ...
  */
  ...
  /** Replaces SkPaint::Flags with flags, the union of the SkPaint::Flags
     members. All SkPaint::Flags members may be cleared, or one or more may be
     set.
  ...
  */
  ...
  /** Requests, but does not require, that SkPath edge pixels draw opaque or
  ...
  */
  bool isDither() const { return SkToBool(this->getFlags() & kDither_Flag); }
  ...
  */
  ...
  /** Returns true if glyphs at different sub-pixel positions may differ on
     pixel edge coverage.
  ...
  */
  ...
  /** Returns true if SkPaint::Hinting is set to kNormal_Hinting or
     kFull_Hinting, and if platform uses FreeType as the font manager. If true,
  ...
  */
  ...
  /** Sets whether to always hint glyphs.
      If SkPaint::Hinting is set to kNormal_Hinting or kFull_Hinting and
     useAutohinter is set, instructs the font manager to always hint glyphs.
  ...
  */
  ...
  /** Returns true if text advance positions the next glyph below the previous
     glyph instead of to the right of previous glyph.
  ...
  */
  ...
  /** Returns true if approximate bold by increasing the stroke width when
  ...
  */
  ...
  /** \enum SkPaint::Style
      ...
      The stroke and fill
      share all paint attributes; for instance, they are drawn with the same
     color.

      Use kStrokeAndFill_Style to avoid hitting the same pixels twice with a
     stroke draw and a fill draw.
  */
  enum Style
  {
  ...
  */
  ...
  /** Retrieves alpha and RGB, unpremultiplied, packed into 32 bits.
      Use helpers SkColorGetA(), SkColorGetR(), SkColorGetG(), and SkColorGetB()
  ...
  */
  ...
  /** Sets alpha and RGB used when stroking and filling. The color is four
     floating point values, unpremultiplied. The color values are interpreted as
     being in the colorSpace. If colorSpace is nullptr, then color is assumed to
  ...
  */
  ...
  /** Sets color used when drawing solid fills. The color components range from
     0 to 255. The color is unpremultiplied; alpha sets the transparency
     independent of RGB.

      @param a  amount of alpha, from fully transparent (0) to fully opaque
     (255)
  ...
  */
  ...
  /** Sets the thickness of the pen used by the paint to
      ...
      @param width  zero thickness for hairline; greater than zero for pen
     thickness
  */
  ...
  */
  ...
  /** \enum SkPaint::Join
      ...
      affects the four corners of a stroked rectangle, and the connected
     segments in a stroked path.

      Choose miter join to draw sharp corners. Choose round join to draw a
     circle with a radius equal to the stroke width on top of the corner. Choose
     bevel join to minimally connect the thick strokes.

      The fill path constructed to describe the stroked path respects the join
     setting but may not contain the actual join. For instance, a fill path
     constructed with round joins does not necessarily include circles at each
     connected segment.
  */
  enum Join
  {
  ...
  */
  ...
  /** Returns the filled equivalent of the stroked path.
      ...
      @param dst       resulting SkPath; may be the same as src, but may not be
     nullptr
      @param cullRect  optional limit passed to SkPathEffect
      @param resScale  if > 1, increase precision, else if (0 < res < 1) reduce
     precision to favor speed and size
  ...
  */
  ...
  /** Returns the filled equivalent of the stroked path.
      ...
      SkPathEffect, if any, is not culled. stroke width is created with default
     precision.

      @param src  SkPath read to create a filled version
      @param dst  resulting SkPath dst may be the same as src, but may not be
     nullptr
      @return     true if the path represents style fill, or false if it
     represents hairline
  */
  ...
  */
  ...
  /** Sets optional colors used when filling a path, such as a gradient.
      ...
      @param shader  how geometry is filled with color; if nullptr, color is
     used instead
  */
  ...
  */
  ...
  /** Sets SkMaskFilter to maskFilter, decreasing SkRefCnt of the previous
      SkMaskFilter. Pass nullptr to clear SkMaskFilter and leave SkMaskFilter
     effect on mask alpha unaltered.
  ...
  */
  ...
  /** Sets SkTypeface to typeface, decreasing SkRefCnt of the previous
     SkTypeface. Pass nullptr to clear SkTypeface and use the default typeface.
     Increments typeface SkRefCnt by one.
  ...
  */
  ...
  /** Sets SkImageFilter to imageFilter, decreasing SkRefCnt of the previous
      SkImageFilter. Pass nullptr to clear SkImageFilter, and remove
     SkImageFilter effect on drawing.
  ...
  */
  ...
  /** Sets SkDrawLooper to drawLooper, decreasing SkRefCnt of the previous
      drawLooper.  Pass nullptr to clear SkDrawLooper and leave SkDrawLooper
     effect on drawing unaltered.

      Increments drawLooper SkRefCnt by one.

      @param drawLooper  iterates through drawing one or more time, altering
     SkPaint
  */
  ...
  */
  ...
  /** \enum SkPaint::Align
      ...
      Align affects glyphs drawn with: SkCanvas::drawText,
     SkCanvas::drawPosText, SkCanvas::drawPosTextH, SkCanvas::drawTextRSXform,
     SkCanvas::drawTextBlob, and SkCanvas::drawString; as well as calls that
     place text glyphs like getTextWidths() and getTextPath().

      The text position is set by the font for both horizontal and vertical
     text. Typically, for horizontal text, the position is to the left side of
     the glyph on the base line; and for vertical text, the position is the
     horizontal center of the glyph at the caps height.

      Align adjusts the glyph position to center it or move it to abut the
     position using the metrics returned by the font.
  ...
  */
  ...
  /** \enum SkPaint::TextEncoding
      TextEncoding determines whether text specifies character codes and their
     encoded size, or glyph indices. Characters are encoded as specified by the
     Unicode standard.

      Character codes encoded size are specified by UTF-8, UTF-16, or UTF-32.
      All character code formats are able to represent all of Unicode, differing
     only in the total storage required.
      ...
      font manager uses font data to convert character code points into glyph
     indices. A glyph index is a 16-bit word.
  ...
  */
  ...
  /** Returns SkPaint::TextEncoding.
      SkPaint::TextEncoding determines how character code points are mapped to
     font glyph indices.

      @return  one of: kUTF8_TextEncoding, kUTF16_TextEncoding,
     kUTF32_TextEncoding, or kGlyphID_TextEncoding
  */
  ...
  /** Sets SkPaint::TextEncoding to encoding.
      SkPaint::TextEncoding determines how character code points are mapped to
     font glyph indices. Invalid values for encoding are ignored.

      @param encoding  one of: kUTF8_TextEncoding, kUTF16_TextEncoding,
     kUTF32_TextEncoding, or kGlyphID_TextEncoding
  */
  void setTextEncoding(TextEncoding encoding);

  /** \struct SkPaint::FontMetrics
      FontMetrics is filled out by getFontMetrics(). FontMetrics contents
     reflect the values computed by font manager using SkTypeface. Values are
     set to zero if they are not available.

      All vertical values are relative to the baseline, on a y-axis pointing
     down. Zero is on the baseline, negative values are above the baseline, and
     positive values are below the baseline.

      fUnderlineThickness and fUnderlinePosition have a bit set in fFlags if
     their values are valid, since their value may be zero.

      fStrikeoutThickness and fStrikeoutPosition have a bit set in fFlags if
     their values are valid, since their value may be zero.
  */
  struct FontMetrics
  {
    ...
    /** \enum SkPaint::FontMetrics::FontMetricsFlags
        FontMetricsFlags are set in fFlags when underline and strikeout metrics
       are valid; the underline or strikeout metric may be valid and zero. Fonts
       with embedded bitmaps may not have valid underline or strikeout metrics.
    */
    enum FontMetricsFlags
    {
      kUnderlineThicknessIsValid_Flag =
        1 << 0, //!< set if fUnderlineThickness is valid
      kUnderlinePositionIsValid_Flag =
        1 << 1, //!< set if fUnderlinePosition is valid
      kStrikeoutThicknessIsValid_Flag =
        1 << 2, //!< set if fStrikeoutThickness is valid
      kStrikeoutPositionIsValid_Flag =
        1 << 3, //!< set if fStrikeoutPosition is valid
    };
    ...
    /** Returns true if SkPaint::FontMetrics has a valid underline thickness,
       and sets thickness to that value. If the underline thickness is not
       valid, return false, and ignore thickness.
    ...
    */
    ...
    /** Returns true if SkPaint::FontMetrics has a valid underline position, and
       sets position to that value. If the underline position is not valid,
    ...
    */
    ...
    /** Returns true if SkPaint::FontMetrics has a valid strikeout thickness,
       and sets thickness to that value. If the underline thickness is not
       valid, return false, and ignore thickness.
    ...
    */
    ...
    /** Returns true if SkPaint::FontMetrics has a valid strikeout position, and
       sets position to that value. If the underline position is not valid,
        return false, and ignore position.
  ...
  /** Returns SkPaint::FontMetrics associated with SkTypeface.
     ...
     metrics descent, ascent, and leading. If metrics is not nullptr,
     SkPaint::FontMetrics is copied to metrics. Results are scaled by text size
     but does not take into account dimensions required by text scale x, text
     skew x, fake bold, style stroke, and SkPathEffect. Results can be
     additionally scaled by scale; a scale of zero is ignored.

      @param metrics  storage for SkPaint::FontMetrics from SkTypeface; may be
     nullptr
  ...
  */
  ...
  /** Converts text into glyph indices.
      ...
      If byteLength includes a partial character, the partial character is
     ignored.
      ...
      @return            number of glyphs represented by text of length
     byteLength
  */
  ...
  */
  ...
  /** Converts glyphs into text if possible.
      ...
      by SkPaint::TextEncoding; the text values returned are equivalent to
     kUTF32_TextEncoding.
  ...
  */
  void glyphsToUnichars(const SkGlyphID glyphs[],
                        int count,
  ...
  */
  ...
  /** Returns the advance width of text if kVerticalText_Flag is clear,
      ...
      Uses SkPaint::TextEncoding to decode text, SkTypeface to get the font
     metrics, and text size, text scale x, text skew x, stroke width, and
  ...
  */
  ...
  /** Returns the advance width of text if kVerticalText_Flag is clear,
      ...
      The advance is the normal distance to move before drawing additional text.
      Uses SkPaint::TextEncoding to decode text, SkTypeface to get the font
     metrics, and text size to scale the metrics. Does not scale the advance or
     bounds by fake bold or SkPathEffect.
  ...
  */
  ...
  /** Returns the bytes of text that fit within maxWidth.
      If kVerticalText_Flag is clear, the text fragment fits if its advance
     width is less than or equal to maxWidth. If kVerticalText_Flag is set, the
     text fragment fits if its advance height is less than or equal to maxWidth.
      Measures only while the advance is less than or equal to maxWidth.
      Returns the advance or the text fragment in measuredWidth if it not
     nullptr. Uses SkPaint::TextEncoding to decode text, SkTypeface to get the
     font metrics, and text size to scale the metrics. Does not scale the
     advance or bounds by fake bold or SkPathEffect.

      @param text           character codes or glyph indices to be measured
      @param length         number of bytes of text to measure
      @param maxWidth       advance limit; text is measured while advance is
     less than maxWidth
      @param measuredWidth  returns the width of the text less than or equal to
     maxWidth
      @return               bytes of text that fit, always less than or equal to
     length
  */
  size_t breakText(const void* text,
                   size_t length,
                   SkScalar maxWidth,
  ...
  /** Retrieves the advance and bounds for each glyph in text, and returns
      ...
      Uses SkPaint::TextEncoding to decode text, SkTypeface to get the font
     metrics, and text size to scale the widths and bounds. Does not scale the
     advance by fake bold or SkPathEffect. Does include fake bold and
     SkPathEffect in the bounds.

      @param text        character codes or glyph indices to be measured
      @param byteLength  number of bytes of text to measure
      @param widths      returns text advances for each glyph; may be nullptr
      @param bounds      returns bounds for each glyph relative to (0, 0); may
     be nullptr
      @return            glyph count in text
  */
  int getTextWidths(const void* text,
                    size_t byteLength,
                    SkScalar widths[],
                    SkRect bounds[] = nullptr) const;

  /** Returns the geometry as SkPath equivalent to the drawn text.
      Uses SkPaint::TextEncoding to decode text, SkTypeface to get the glyph
     paths, and text size, fake bold, and SkPathEffect to scale and modify the
     glyph paths. All of the glyph paths are stored in path. Uses x, y, and
     SkPaint::Align to position path.
  ...
  */
  void getTextPath(const void* text,
                   size_t length,
                   SkScalar x,
                   SkScalar y,
                   SkPath* path) const;
  ...
  /** Returns the geometry as SkPath equivalent to the drawn text.
      Uses SkPaint::TextEncoding to decode text, SkTypeface to get the glyph
     paths, and text size, fake bold, and SkPathEffect to scale and modify the
     glyph paths. All of the glyph paths are stored in path. Uses pos array and
     SkPaint::Align to position path. pos contains a position for each glyph.
      ...
      @param path    geometry of the glyphs
  */
  void getPosTextPath(const void* text,
                      size_t length,
                      const SkPoint pos[],
                      SkPath* path) const;
  ...
  /** Returns the number of intervals that intersect bounds.
      ...
      The return count is zero or a multiple of two, and is at most twice the
     number of glyphs in the string. Uses SkPaint::TextEncoding to decode text,
     SkTypeface to get the glyph paths, and text size, fake bold, and
     SkPathEffect to scale and modify the glyph paths. Uses x, y, and
     SkPaint::Align to position intervals.
  ...
  */
  int getTextIntercepts(const void* text,
                        size_t length,
                        SkScalar x,
                        SkScalar y,
  ...
  /** Returns the number of intervals that intersect bounds.
     ...
     number of glyphs in the string. Uses SkPaint::TextEncoding to decode text,
     SkTypeface to get the glyph paths, and text size, fake bold, and
     SkPathEffect to scale and modify the glyph paths. Uses pos array and
     SkPaint::Align to position intervals.
      ...
      @return           number of intersections; may be zero
  */
  int getPosTextIntercepts(const void* text,
                           size_t length,
                           const SkPoint pos[],
                           const SkScalar bounds[2],
                           SkScalar* intervals) const;
  ...
  /** Returns the number of intervals that intersect bounds.
     ...
     number of glyphs in the string. Uses SkPaint::TextEncoding to decode text,
     SkTypeface to get the glyph paths, and text size, fake bold, and
     SkPathEffect to scale and modify the glyph paths. Uses xpos array, constY,
     and SkPaint::Align to position intervals.
      ...
      @return           number of intersections; may be zero
  */
  int getPosTextHIntercepts(const void* text,
                            size_t length,
                            const SkScalar xpos[],
                            SkScalar constY,
                            const SkScalar bounds[2],
  ...
  /** Returns the number of intervals that intersect bounds.
      bounds describes a pair of lines parallel to the text advance.
      The return count is zero or a multiple of two, and is at most twice the
     number of glyphs in the string. Uses SkTypeface to get the glyph paths, and
     text size, fake bold, and SkPathEffect to scale and modify the glyph paths.
  ...
  */
  ...
  /**     (to be made private)
      Returns true if SkPaint does not include elements requiring extensive
     computation to compute SkBaseDevice bounds of drawn geometry. For instance,
     SkPaint with SkPathEffect always returns false.
  ...
  /**     (to be made private)
      ...
      SkRect.
          ...
          if (!path.isInverseFillType() && paint.canComputeFastBounds()) {
              SkRect storage;
              if (canvas->quickReject(paint.computeFastBounds(path.getBounds(),
     &storage))) { return; // do not draw the path
              }
          }
          // draw the path
  ...
  */
  ...
  {
    // Things like stroking, etc... will do math on the bounds rect, assuming
    // that it's sorted.
  ...
  */
  ...
  /**     (to be made private)
      Computes the bounds, overriding the SkPaint SkPaint::Style. This can be
     used to account for additional width required by stroking orig, without
      ...
      @return         fast computed bounds
  */
  const SkRect& doComputeFastBounds(const SkRect& orig,
                                    SkRect* storage,
                                    Style style) const;
...
private:
  ...
  typedef const SkGlyph& (*GlyphCacheProc)(SkGlyphCache*,
                                           const char**,
                                           const char*);
  ...
  SkScalar measure_text(SkGlyphCache*,
                        const char* text,
                        size_t length,
                        int* count,
                        SkRect* bounds) const;
  ...
  /*
   * The luminance color is used to determine which Gamma Canonical color to map
   * to.  This is really only used by backends which want to cache glyph masks,
   * and need some way to know if they need to generate new masks based off a
   * given color.
   */
  ...
  static bool TooBigToUseCache(const SkMatrix& ctm,
                               const SkMatrix& textM,
                               SkScalar maxLimit);
