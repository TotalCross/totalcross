...
{
public:
  // Extra information about the output of a filter DAG. For now, this is just
  // the color space (of the original requesting device). This is used when
  // constructing intermediate rendering surfaces, so that we ensure we land in
  // a surface that's similar/compatible to the final consumer of the DAG's
  // output.
  ...
  private:
    ...
    // This will be a pointer to the device's color space, and our lifetime is
    // bounded by the device, so we can store a bare pointer.
  ...
  {
  public:
    Context(const SkMatrix& ctm,
            const SkIRect& clipBounds,
            SkImageFilterCache* cache,
     ...
     *
     *  The SkImageFilterCache Key, for example, requires a finite ctm (no
     * infinities or NaN), so that test is part of isValid.
     */
  ...
  public:
    ...
    /**
     *  Apply this cropRect to the imageBounds. If a given edge of the cropRect
     * is not set, then the corresponding edge from imageBounds will be used. If
     * "embiggen" is true, the crop rect is allowed to enlarge the size of the
     * rect, otherwise it may only reduce the rect. Filters that can affect
     * transparent black should pass "true", while all other filters should pass
     * "false".
     *
     *  Note: imageBounds is in "device" space, as the output cropped rectangle
     * will be, so the matrix is ignored for those. It is only applied the
     * croprect's bounds.
     */
    void applyTo(const SkIRect& imageBounds,
   ...
   */
  sk_sp<SkSpecialImage> filterImage(SkSpecialImage* src,
   ...
   * pixels. In kForward mode it should always be null. If 'inputRect' is null
   * in kReverse mode the resulting answer may be incorrect.
   */
  ...
  /**
   *  Returns whether this image filter is a color filter and puts the color
   * filter into the "filterPtr" parameter if it can. Does nothing otherwise. If
   * this returns false, then the filterPtr is unchanged. If this returns true,
   * then if filterPtr is not null, it must be set to a ref'd colorfitler (i.e.
  ...
  /**
   *  Returns true (and optionally returns a ref'd filter) if this imagefilter
   * can be completely replaced by the returned colorfilter. i.e. the two
   * effects will affect drawing in the same way.
   */
   ...
   *  input image will be processed, and which pixels in the output image will
   * be allowed. The size of the crop rect should be used as the size of the
   * destination image. The origin of this rect should be used to offset access
   * to the input images, and should also be added to the "offset" parameter in
   * onFilterImage.
   */
  ...
  // Can this filter DAG compute the resulting bounds of an object-space
  // rectangle?
  ...
  /**
   *  ImageFilters can natively handle scaling and translate components in the
   * CTM. Only some of them can handle affine (or more complex) matrices. This
   * call returns true iff the filter and all of its (non-null) inputs can
   * handle these more complex matrices.
   */
    ...
    const SkDeserialProcs* procs = nullptr)
  {
    return sk_sp<SkImageFilter>(static_cast<SkImageFilter*>(
      SkFlattenable::Deserialize(kSkImageFilter_Type, data, size, procs)
        .release()));
  }
...
protected:
  ...
  public:
    /**
     *  Attempt to unflatten the cropRect and the expected number of input
     * filters. If any number of input filters is valid, pass -1. If this fails
     * (i.e. corrupt buffer or contents) then return false and common will be
     * left uninitialized. If this returns true, then inputCount() is the number
     * of found input filters, each of which may be NULL or a valid imagefilter.
     */
  ...
  SkImageFilter(sk_sp<SkImageFilter> const* inputs,
                int inputCount,
   ...
   *
   *  @param inputCount    The exact number of inputs expected for this
   * SkImageFilter object. -1 can be used if the filter accepts any number of
   * inputs.
   *  @param rb            SkReadBuffer object from which the SkImageFilter is
   * read.
   */
   ...
   *  src when it is drawn. This is an out-param.
   *
   *  If the result image cannot be created (either because of error or if, say,
   * the result is entirely clipped out), this should return nullptr. Callers
   * that affect transparent black should explicitly handle nullptr results and
   * press on. In the error case this behavior will produce a better result than
   * nothing and is necessary for the clipped out case. If the return value is
   * nullptr then offset should be ignored.
   */
  virtual sk_sp<SkSpecialImage> onFilterImage(SkSpecialImage* src,
                                              const Context&,
   ...
   */
  virtual SkIRect onFilterBounds(const SkIRect&,
                                 const SkMatrix& ctm,
                                 MapDirection,
   ...
   * In kReverse mode, 'inputRect' will be the device space bounds of the input
   * pixels. In kForward mode, 'inputRect' should always be null. If 'inputRect'
   * is null in kReverse mode the resulting answer may be incorrect.
   */
  virtual SkIRect onFilterNodeBounds(const SkIRect&,
                                     const SkMatrix& ctm,
                                     MapDirection,
                                     const SkIRect* inputRect) const;
  ...
  /**
   *  Return true (and return a ref'd colorfilter) if this node in the DAG is
   * just a colorfilter w/o CropRect constraints.
   */
   ...
   * The caller will take care of calling your inputs (and return false if any
   * of them could not handle it).
   */
   ...
   *  future).
   */
  bool applyCropRect(const Context&,
   ...
   *  larger destination.
   */
  sk_sp<SkSpecialImage> applyCropRectAndPad(const Context&,
                                            SkSpecialImage* src,
                                            SkIPoint* srcOffset,
...
#if SK_SUPPORT_GPU
  /**
   *  Returns a version of the passed-in image (possibly the original), that is
   * in a colorspace with the same gamut as the one from the OutputProperties.
   * This allows filters that do many texture samples to guarantee that any
   * color space conversion has happened before running.
   */
  ...
  /**
   *  Returns an image filter transformed into a new color space via the
   * |xformer|.
   */
  ...
  // If 'srcBounds' will sample outside the border of 'originalSrcBounds' (i.e.,
  // the sample will wrap around to the other side) we must preserve the far
  // side of the src along that axis (e.g., if we will sample beyond the left
  // edge of the src, the right side must be preserved for the repeat sampling
  // to work).
...
private:
  ...
  void init(sk_sp<SkImageFilter> const* inputs,
            int inputCount,
            const CropRect* cropRect);
