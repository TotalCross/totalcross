...
static bool
SkStrStartsWith(const char string[], const char prefixStr[])
{
  SkASSERT(string);
  SkASSERT(prefixStr);
...
static bool
SkStrStartsWith(const char string[], const char prefixChar)
{
  SkASSERT(string);
...
bool
SkStrEndsWith(const char string[], const char suffixStr[]);
bool
SkStrEndsWith(const char string[], const char suffixChar);

int
SkStrStartsWithOneOf(const char string[], const char prefixes[]);

static int
SkStrFind(const char string[], const char substring[])
{
...
static int
SkStrFindLastOf(const char string[], const char subchar)
{
...
static bool
SkStrContains(const char string[], const char substring[])
{
  SkASSERT(string);
  SkASSERT(substring);
...
static bool
SkStrContains(const char string[], const char subchar)
{
  SkASSERT(string);
  char tmp[2];
  tmp[0] = subchar;
...
/*
 *  The SkStrAppend... methods will write into the provided buffer, assuming it
 * is large enough. Each method has an associated const (e.g.
 * SkStrAppendU32_MaxSize) which will be the largest value needed for that
 * method's buffer.
 *
 *  char storage[SkStrAppendU32_MaxSize];
 *  SkStrAppendU32(storage, value);
 *
 *  Note : none of the SkStrAppend... methods write a terminating 0 to their
 * buffers. Instead, the methods return the ptr to the end of the written part
 * of the buffer. This can be used to compute the length, and/or know where to
 * write a 0 if that is desired.
 *
...
char*
SkStrAppendU32(char buffer[], uint32_t);
#define SkStrAppendU64_MaxSize 20
char*
SkStrAppendU64(char buffer[], uint64_t, int minDigits);

#define SkStrAppendS32_MaxSize (SkStrAppendU32_MaxSize + 1)
char*
SkStrAppendS32(char buffer[], int32_t);
#define SkStrAppendS64_MaxSize (SkStrAppendU64_MaxSize + 1)
char*
SkStrAppendS64(char buffer[], int64_t, int minDigits);
...
char*
SkStrAppendFloat(char buffer[], float);
...
SkString
SkStringPrintf(const char* format, ...);
/// This makes it easier to write a caller as a VAR_ARGS function where the
/// format string is optional.
static inline SkString
SkStringPrintf()
{
  return SkString();
}
...
enum SkStrSplitMode
{
  // Strictly return all results. If the input is ",," and the separator is ','
  // this will return
  ...
  // will be
  // coalesced. Separators at the beginning and end of the input will be
  // ignored.  If the input is
  // ",," and the separator is ',', this will return an empty vector.
  kCoalesce_SkStrSplitMode
};

// Split str on any characters in delimiters into out.  (Think, strtok with a
// sane API.)
void
SkStrSplit(const char* str,
           const char* delimiters,
           SkStrSplitMode splitMode,
           SkTArray<SkString>* out);
inline void
SkStrSplit(const char* str, const char* delimiters, SkTArray<SkString>* out)
{
