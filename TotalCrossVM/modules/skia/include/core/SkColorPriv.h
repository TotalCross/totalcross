...
static inline U8CPU
SkUnitScalarClampToByte(SkScalar x)
{
 ...
 *  the components are 0..255, and are already premultiplied (i.e. alpha >=
 * color)
 */
static inline SkPMColor
SkPackARGB32(U8CPU a, U8CPU r, U8CPU g, U8CPU b)
{
  SkA32Assert(a);
  SkASSERT(r <= a);
  SkASSERT(g <= a);
 ...
 */
static inline SkPMColor
SkPackARGB32NoCheck(U8CPU a, U8CPU r, U8CPU g, U8CPU b)
{
  return (a << SK_A32_SHIFT) | (r << SK_R32_SHIFT) | (g << SK_G32_SHIFT) |
         (b << SK_B32_SHIFT);
}
...
static inline SkPMColor
SkPremultiplyARGBInline(U8CPU a, U8CPU r, U8CPU g, U8CPU b)
{
  SkA32Assert(a);
  SkR32Assert(r);
  SkG32Assert(g);
...
static SK_ALWAYS_INLINE uint32_t
SkAlphaMulQ(uint32_t c, unsigned scale)
{
...
static inline SkPMColor
SkPMSrcOver(SkPMColor src, SkPMColor dst)
{
...
#define SkGetPackedR16(color)                                                  \
  (((unsigned)(color) >> SK_R16_SHIFT) & SK_R16_MASK)
#define SkGetPackedG16(color)                                                  \
  (((unsigned)(color) >> SK_G16_SHIFT) & SK_G16_MASK)
#define SkGetPackedB16(color)                                                  \
  (((unsigned)(color) >> SK_B16_SHIFT) & SK_B16_MASK)

static inline unsigned
SkR16ToR32(unsigned r)
{
  return (r << (8 - SK_R16_BITS)) | (r >> (2 * SK_R16_BITS - 8));
}

static inline unsigned
SkG16ToG32(unsigned g)
{
  return (g << (8 - SK_G16_BITS)) | (g >> (2 * SK_G16_BITS - 8));
}

static inline unsigned
SkB16ToB32(unsigned b)
{
