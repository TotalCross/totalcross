...
/* Generated by tools/bookmaker from include/core/SkBitmap.h and
   docs/SkBitmap_Reference.bmh on 2018-09-13 13:59:55. Additional documentation
   and examples can be found at: https://skia.org/user/api/SkBitmap_Reference

   You may edit either file directly. Structural changes to public interfaces
   require editing both files. After editing docs/SkBitmap_Reference.bmh, run:
...
/** \class SkBitmap
    SkBitmap describes a two-dimensional raster pixel array. SkBitmap is built
   on SkImageInfo, containing integer width and height, SkColorType and
   SkAlphaType describing the pixel format, and SkColorSpace describing the
   range of colors. SkBitmap points to SkPixelRef, which describes the physical
   array of pixels. SkImageInfo bounds may be located anywhere fully inside
   SkPixelRef bounds.
   ...
   limits some optimizations available to the target platform.
    ...
    If pixel array is primarily written to, use SkSurface for better
   performance.

    Declaring SkBitmap const prevents altering SkImageInfo: the SkBitmap height,
   width, and so on cannot change. It does not affect SkPixelRef: a caller may
   write its pixels. Declaring SkBitmap const affects SkBitmap configuration,
   not its contents.
...
public:
  ...
  /** Creates an empty SkBitmap without pixels, with kUnknown_SkColorType,
      kUnknown_SkAlphaType, and with a width and height of zero. SkPixelRef
     origin is set to (0, 0). SkBitmap is not volatile.
  ...
  */
  ...
  /** Copies settings from src to returned SkBitmap. Shares pixels if src has
     pixels allocated, so both bitmaps reference the same pixels.
  ...
  */
  ...
  /** Copies settings from src to returned SkBitmap. Moves ownership of src
     pixels to SkBitmap.
  ...
  */
  ...
  /** Returns pixel count in each row. Should be equal or less than
      ...
      May be less than pixelRef().width(). Will not exceed pixelRef().width()
     less pixelRefOrigin().fX.
  ...
  */
  ...
  /** Returns pixel row count.
      ...
      Maybe be less than pixelRef().height(). Will not exceed
     pixelRef().height() less pixelRefOrigin().fY.
  ...
  */
  ...
  /** Returns SkColorSpace, the range of colors, associated with SkImageInfo.
     The reference count of SkColorSpace is unchanged. The returned SkColorSpace
     is immutable.
  ...
  */
  ...
  /** Returns smart pointer to SkColorSpace, the range of colors, associated
     with SkImageInfo. The smart pointer tracks the number of objects sharing
     this SkColorSpace reference so the memory is released when the owners
     destruct.
  ...
  */
  ...
  /** Returns number of pixels that fit on row. Should be greater than or equal
     to width().
  ...
  */
  ...
  /** Returns true if SkPixelRef is nullptr.
      ...
      Does not check if width() or height() are zero; call drawsNothing() to
     check width(), height(), and SkPixelRef.
  ...
  */
  ...
  /** Returns row bytes, the interval from one pixel row to the next. Row bytes
      ...
      Returns zero if colorType() is kUnknown_SkColorType, or if row bytes
  ...
  */
  ...
  /** Sets SkAlphaType, if alphaType is compatible with SkColorType.
      Returns true unless alphaType is kUnknown_SkAlphaType and current
     SkAlphaType is not kUnknown_SkAlphaType.

      Returns true if SkColorType is kUnknown_SkColorType. alphaType is ignored,
     and SkAlphaType remains kUnknown_SkAlphaType.

      Returns true if SkColorType is kRGB_565_SkColorType or
     kGray_8_SkColorType. alphaType is ignored, and SkAlphaType remains
     kOpaque_SkAlphaType.
      ...
      alphaType is kUnknown_SkAlphaType and SkAlphaType is not
     kUnknown_SkAlphaType. If SkAlphaType is kUnknown_SkAlphaType, alphaType is
     ignored.

      If SkColorType is kAlpha_8_SkColorType, returns true unless
      alphaType is kUnknown_SkAlphaType and SkAlphaType is not
     kUnknown_SkAlphaType. If SkAlphaType is kUnknown_SkAlphaType, alphaType is
     ignored. If alphaType is kUnpremul_SkAlphaType, it is treated as
     kPremul_SkAlphaType.
  ...
  */
  ...
  /** Returns minimum memory required for pixel storage.
     ...
     width(). Returns zero if result does not fit in size_t. Returns zero if
     height() or width() is 0. Returns height() times rowBytes() if colorType()
     is kUnknown_SkColorType.
  ...
  */
  ...
  /** Sets internal flag to mark SkBitmap as immutable. Once set, pixels can not
     ...
     immutable. Once SkPixelRef is marked immutable, the setting cannot be
     cleared.
  ...
  */
  ...
  /** Returns true if SkAlphaType is set to hint that all pixels are opaque;
     their alpha value is implicitly or explicitly 1.0. If true, and all pixels
     are not opaque, Skia may draw incorrectly.
  ...
  */
  ...
  /** Sets if pixels should be read from SkPixelRef on every access. SkBitmap
     are not volatile by default; a GPU back end may upload pixel values
     expecting them to be accessed repeatedly. Marking temporary SkBitmap as
     volatile provides a hint to SkBaseDevice that the SkBitmap pixels should
     not be cached. This can improve performance by avoiding overhead and
     reducing resource consumption on SkBaseDevice.
  ...
  */
  ...
  /** Resets to its initial state; all fields are set to zero, as if SkBitmap
     had been initialized by SkBitmap().
  ...
  */
  ...
  /** Returns true if all pixels are opaque. SkColorType determines how pixels
      are encoded, and whether pixel describes alpha. Returns true for
     SkColorType without alpha in each pixel; for other SkColorType, returns
     true if all pixels have alpha values equivalent to 1.0 or greater.

      For SkColorType kRGB_565_SkColorType or kGray_8_SkColorType: always
      returns true. For SkColorType kAlpha_8_SkColorType,
     kBGRA_8888_SkColorType, kRGBA_8888_SkColorType: returns true if all pixel
     alpha values are 255. For SkColorType kARGB_4444_SkColorType: returns true
     if all pixel alpha values are 15. For kRGBA_F16_SkColorType: returns true
  ...
  */
  SkIRect getSubset() const
  {
    ...
    return SkIRect::MakeXYWH(
      origin.x(), origin.y(), this->width(), this->height());
  }
  ...
  /** Sets width, height, SkAlphaType, SkColorType, SkColorSpace, and optional
     ...
     imageInfo.colorSpace(). If imageInfo.colorType() is kUnknown_SkColorType,
     imageInfo.alphaType() is set to kUnknown_SkAlphaType. If
     imageInfo.colorType() is kAlpha_8_SkColorType and imageInfo.alphaType() is
      kUnpremul_SkAlphaType, imageInfo.alphaType() is replaced by
     kPremul_SkAlphaType. If imageInfo.colorType() is kRGB_565_SkColorType or
     kGray_8_SkColorType, imageInfo.alphaType() is set to kOpaque_SkAlphaType.
      If imageInfo.colorType() is kARGB_4444_SkColorType,
     kRGBA_8888_SkColorType, kBGRA_8888_SkColorType, or kRGBA_F16_SkColorType:
     imageInfo.alphaType() remains unchanged.

      rowBytes must equal or exceed imageInfo.minRowBytes(). If
     imageInfo.colorSpace() is kUnknown_SkColorType, rowBytes is ignored and
     treated as zero; for all other SkColorSpace values, rowBytes of zero is
     treated as imageInfo.minRowBytes().
     ...
     imageInfo.bytesPerPixel()
      ...
      @param imageInfo  contains width, height, SkAlphaType, SkColorType,
     SkColorSpace
  ...
  */
  ...
  /** Sets SkImageInfo to info following the rules in setInfo() and allocates
     pixel memory. If flags is kZeroPixels_AllocFlag, memory is zeroed.
     ...
     platform implementation of malloc(), if flags is zero, and calloc(), if
     flags is kZeroPixels_AllocFlag.
  ...
  */
  ...
  /** Sets SkImageInfo to info following the rules in setInfo() and allocates
     pixel memory. If flags is kZeroPixels_AllocFlag, memory is zeroed.
      ...
      be zeroed. Abort steps may be provided by the user at compile time by
     defining SK_ABORT.
  ...
  */
  ...
  /** Sets SkImageInfo to width, height, and native color type; and allocates
      pixel memory. If isOpaque is true, sets SkImageInfo to
     kOpaque_SkAlphaType; otherwise, sets to kPremul_SkAlphaType.
      ...
      Use to create SkBitmap that matches SkPMColor, the native pixel
     arrangement on the platform. SkBitmap drawn to output device skips
     converting its pixel format.
      ...
      @return          true if pixel storage is allocated
  */
  bool SK_WARN_UNUSED_RESULT tryAllocN32Pixels(int width,
                                               int height,
                                               bool isOpaque = false)
  {
    SkImageInfo info = SkImageInfo::MakeN32(
      width, height, isOpaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType);
    return this->tryAllocPixels(info);
  }

  /** Sets SkImageInfo to width, height, and the native color type; and
     allocates pixel memory. If isOpaque is true, sets SkImageInfo to
     kPremul_SkAlphaType; otherwise, sets to kOpaque_SkAlphaType.

      Aborts if width exceeds 29 bits or is negative, or height is negative, or
      allocation fails. Abort steps may be provided by the user at compile time
     by defining SK_ABORT.
  ...
  */
  ...
  /** Sets SkImageInfo to info following the rules in setInfo(), and creates
     SkPixelRef containing pixels and rowBytes. releaseProc, if not nullptr, is
     called immediately on failure or when pixels are no longer referenced.
     context may be nullptr.

      If SkImageInfo could not be set, or rowBytes is less than
     info.minRowBytes(): calls releaseProc if present, calls reset(), and
     returns false.

      Otherwise, if pixels equals nullptr: sets SkImageInfo, calls releaseProc
     ...
     SkColorSpace
      ...
      @param releaseProc  function called when pixels can be deleted; may be
     nullptr
      @param context      caller state passed to releaseProc; may be nullptr
      @return             true if SkImageInfo is set to info
  */
  bool installPixels(const SkImageInfo& info,
                     void* pixels,
                     size_t rowBytes,
  ...
  /** Sets SkImageInfo to info following the rules in setInfo(), and creates
     SkPixelRef containing pixels and rowBytes.
      ...
      If SkImageInfo could not be set, or rowBytes is less than
     info.minRowBytes(): calls reset(), and returns false.

      Otherwise, if pixels equals nullptr: sets SkImageInfo, returns true.

      Caller must ensure that pixels are valid for the lifetime of SkBitmap and
     SkPixelRef.
  ...
  */
  ...
  /** Sets SkImageInfo to pixmap.info() following the rules in setInfo(), and
     creates SkPixelRef containing pixmap.addr() and pixmap.rowBytes().
      ...
      Otherwise, if pixmap.addr() equals nullptr: sets SkImageInfo, returns
     true.
  ...
  */
  ...
  /** Allocates pixel memory with HeapAllocator, and replaces existing
     SkPixelRef. The allocation size is determined by SkImageInfo width, height,
     and SkColorType.

      Returns false if info().colorType is kUnknown_SkColorType, or allocation
     fails.
  ...
  */
  void allocPixels() { this->allocPixels((Allocator*)nullptr); }

  /** Allocates pixel memory with allocator, and replaces existing SkPixelRef.
      The allocation size is determined by SkImageInfo width, height, and
     SkColorType. If allocator is nullptr, use HeapAllocator instead.
  ...
  */
  ...
  /** Allocates pixel memory with allocator, and replaces existing SkPixelRef.
     ...
     SkColorType. If allocator is nullptr, use HeapAllocator instead.
      ...
      Aborts if Allocator::allocPixelRef return false. Abort steps may be
     provided by the user at compile time by defining SK_ABORT.
  ...
  */
  ...
  /** Returns SkPixelRef, which contains: pixel base address; its dimensions;
     and rowBytes(), the interval from one row to the next. Does not change
     SkPixelRef reference count. SkPixelRef may be shared by multiple bitmaps.
  ...
  */
  ...
  /** Returns origin of pixels within SkPixelRef. SkBitmap bounds is always
     contained by SkPixelRef bounds, which may be the same size or larger.
     Multiple SkBitmap can share the same SkPixelRef, where each SkBitmap has
     different bounds.

      The returned origin added to SkBitmap dimensions equals or is smaller than
     the SkPixelRef dimensions.
  ...
  */
  ...
  /** Replaces pixel values with c. All pixels contained by bounds() are
     affected. If the colorType() is kGray_8_SkColorType or k565_SkColorType,
     then alpha is ignored; RGB is treated as opaque. If colorType() is
     kAlpha_8_SkColorType, then RGB is ignored.
  ...
  */
  ...
  /** Replaces pixel values with unpremultiplied color built from a, r, g, and
     b. All pixels contained by bounds() are affected. If the colorType() is
     kGray_8_SkColorType or k565_SkColorType, then a is ignored; r, g, and b are
     treated as opaque. If colorType() is kAlpha_8_SkColorType, then r, g, and b
     are ignored.
  ...
  */
  ...
  /** Replaces pixel values inside area with c. If area does not intersect
     bounds(), call has no effect.

      If the colorType() is kGray_8_SkColorType or k565_SkColorType, then alpha
      is ignored; RGB is treated as opaque. If colorType() is
     kAlpha_8_SkColorType, then RGB is ignored.
  ...
  */
  ...
  /** Returns pixel at (x, y) as unpremultiplied color.
     ...
     if built with SK_DEBUG defined; and returns undefined values or may crash
     if SK_RELEASE is defined. Fails if SkColorType is kUnknown_SkColorType or
  ...
  */
  ...
  /** Returns pixel address at (x, y).
     ...
     kUnknown_SkColorType, trigger an assert() if built with SK_DEBUG defined.
     Returns nullptr if SkColorType is kUnknown_SkColorType, or SkPixelRef is
     nullptr.
  ...
  */
  ...
  /** Returns address at (x, y).
      ...
      Input is not validated. Triggers an assert() if built with SK_DEBUG
     defined and:
  ...
  */
  ...
  /** Shares SkPixelRef with dst. Pixels are not copied; SkBitmap and dst point
      ...
      subset may be larger than bounds(). Any area outside of bounds() is
     ignored.
  ...
  */
  ...
  /** Copies a SkRect of pixels from SkBitmap to dstPixels. Copy starts at
     (srcX, srcY), and does not exceed SkBitmap (width(), height()).
     ...
     false if:
      ...
      Pixels are copied only if pixel conversion is possible. If SkBitmap
     colorType() is kGray_8_SkColorType, or kAlpha_8_SkColorType;
     dstInfo.colorType() must match. If SkBitmap colorType() is
     kGray_8_SkColorType, dstInfo.colorSpace() must match. If SkBitmap
     alphaType() is kOpaque_SkAlphaType, dstInfo.alphaType() must match. If
     SkBitmap colorSpace() is nullptr, dstInfo.colorSpace() must match. Returns
      false if pixel conversion is not possible.

      srcX and srcY may be negative to copy only top or left of source. Returns
      false if width() or height() is zero or negative.
      Returns false if abs(srcX) >= Bitmap width(), or if abs(srcY) >= Bitmap
     height().
  ...
  */
  ...
  /** Copies a SkRect of pixels from SkBitmap to dst. Copy starts at (srcX,
     srcY), and does not exceed SkBitmap (width(), height()).
     ...
     Returns false if:
      ...
      Pixels are copied only if pixel conversion is possible. If SkBitmap
     colorType() is kGray_8_SkColorType, or kAlpha_8_SkColorType; dst
     SkColorType must match. If SkBitmap colorType() is kGray_8_SkColorType, dst
     SkColorSpace must match. If SkBitmap alphaType() is kOpaque_SkAlphaType,
     dst SkAlphaType must match. If SkBitmap colorSpace() is nullptr, dst
     SkColorSpace must match. Returns false if pixel conversion is not possible.
  ...
  */
  ...
  /** Copies a SkRect of pixels from src. Copy starts at (dstX, dstY), and does
     not exceed (src.width(), src.height()).

      src specifies width, height, SkColorType, SkAlphaType, SkColorSpace, pixel
     storage, and row bytes of source. src.rowBytes() specifics the gap from one
     source row to the next. Returns true if pixels are copied. Returns false
     if:
      ...
      Pixels are copied only if pixel conversion is possible. If SkBitmap
     colorType() is kGray_8_SkColorType, or kAlpha_8_SkColorType; src
     SkColorType must match. If SkBitmap colorType() is kGray_8_SkColorType, src
     SkColorSpace must match. If SkBitmap alphaType() is kOpaque_SkAlphaType,
     src SkAlphaType must match. If SkBitmap colorSpace() is nullptr, src
     SkColorSpace must match. Returns false if pixel conversion is not possible.

      dstX and dstY may be negative to copy only top or left of source. Returns
      false if width() or height() is zero or negative.
      Returns false if abs(dstX) >= Bitmap width(), or if abs(dstY) >= Bitmap
     height().
  ...
  */
  ...
  /** Copies a SkRect of pixels from src. Copy starts at (0, 0), and does not
     exceed (src.width(), src.height()).
  ...
  */
  ...
  /** Sets dst to alpha described by pixels. Returns false if dst cannot be
     written to or dst pixels cannot be allocated.
      ...
      generates mask alpha from SkBitmap. Uses HeapAllocator to reserve memory
     for dst SkPixelRef. Sets offset to top-left position for dst for alignment
     with SkBitmap; (0, 0) unless SkMaskFilter generates mask.
  ...
  */
  ...
  /** Sets dst to alpha described by pixels. Returns false if dst cannot be
     written to or dst pixels cannot be allocated.
      ...
      If paint is not nullptr and contains SkMaskFilter, SkMaskFilter
      generates mask alpha from SkBitmap. allocator may reference a custom
     allocation class or be set to nullptr to use HeapAllocator. Sets offset to
     top-left position for dst for alignment with SkBitmap; (0, 0) unless
     SkMaskFilter generates mask.

      @param dst        holds SkPixelRef to fill with alpha layer
      @param paint      holds optional SkMaskFilter; may be nullptr
      @param allocator  function to reserve memory for SkPixelRef; may be
     nullptr
      @param offset     top-left position for dst; may be nullptr
      @return           true if alpha layer was constructed in dst SkPixelRef
  */
  ...
  /** Copies SkBitmap pixel address, row bytes, and SkImageInfo to pixmap, if
  ...
  */
  ...
  public:
    /** Allocates the pixel memory for the bitmap, given its dimensions and
        SkColorType. Returns true on success, where success means either
       setPixels() or setPixelRef() was called.

        @param bitmap  SkBitmap containing SkImageInfo as input, and SkPixelRef
       as output
  ...
  /** \class SkBitmap::HeapAllocator
      Subclass of SkBitmap::Allocator that returns a SkPixelRef that allocates
     its pixel memory from the heap. This is the default SkBitmap::Allocator
     invoked by allocPixels().
  */
...
inline uint32_t*
SkBitmap::getAddr32(int x, int y) const
{
...
inline uint16_t*
SkBitmap::getAddr16(int x, int y) const
{
  SkASSERT(fPixmap.addr());
...
inline uint8_t*
SkBitmap::getAddr8(int x, int y) const
{
  SkASSERT(fPixmap.addr());
