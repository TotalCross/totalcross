...
/** \class SkLights
    SkLights encapsulates a set of directional, point and ambient lights for use
   with the SkLightingShader.
*/
...
public:
  ...
  public:
    ...
    static Light MakePoint(const SkColor3f& color,
                           const SkPoint3& pos,
                           SkScalar intensity)
    {
  ...
  private:
    ...
    SkVector3
      fDirOrPos; // For directional lights, holds the direction towards the
  ...
  */
   ...
   *  @return A new SkLights representing the serialized data, or NULL if the
   * buffer is invalid.
   */
