...
public:
  ...
  T* init(Args&&... args)
  {
    ...
    fPtr =
      new (reinterpret_cast<T*>(fStorage.get())) T(std::forward<Args>(args)...);
    return fPtr;
  }
...
/**
 * A helper built on top of SkTLazy to do copy-on-first-write. The object is
 * initialized with a const pointer but provides a non-const pointer accessor.
 * The first time the accessor is called (if ever) the object is cloned.
 *
 ...
 *    thing.writable()->changeMe(); // makes a copy of constThing if we didn't
 * call modifyMe()
 * }
 *
...
public:
  ...
  /**
   * Returns a writable T*. The first time this is called the initial object is
   * cloned.
   */
  T* writable()
  {
