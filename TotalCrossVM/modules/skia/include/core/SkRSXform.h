...
struct SkRSXform
{
  ...
  /*
   *  Initialize a new xform based on the scale, rotation (in radians), final
   * tx,ty location and anchor-point ax,ay within the src quad.
   *
   *  Note: the anchor point is not normalized (e.g. 0...1) but is in pixels of
   * the src image.
   */
  static SkRSXform MakeFromRadians(SkScalar scale,
                                   SkScalar radians,
                                   SkScalar tx,
                                   SkScalar ty,
                                   SkScalar ax,
