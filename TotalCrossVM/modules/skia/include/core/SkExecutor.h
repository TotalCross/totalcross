...
public:
  ...
  // Create a thread pool SkExecutor with a fixed thread count, by default the
  // number of cores.
  ...
  // There is always a default SkExecutor available by calling
  // SkExecutor::GetDefault().
  static SkExecutor& GetDefault();
  static void SetDefault(
    SkExecutor*); // Does not take ownership.  Not thread safe.

  // Add work to execute.
  virtual void add(std::function<void(void)>) = 0;

  // If it makes sense for this executor, use this thread to execute work for a
  // little while.
  virtual void borrow() {}
};
