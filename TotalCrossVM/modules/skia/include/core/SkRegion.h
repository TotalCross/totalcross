...
/* Generated by tools/bookmaker from include/core/SkRegion.h and
   docs/SkRegion_Reference.bmh on 2018-08-10 12:59:44. Additional documentation
   and examples can be found at: https://skia.org/user/api/SkRegion_Reference

   You may edit either file directly. Structural changes to public interfaces
   require editing both files. After editing docs/SkRegion_Reference.bmh, run:
...
/** \class SkRegion
    SkRegion describes the set of pixels used to clip SkCanvas. SkRegion is
   compact, efficiently storing a single integer rectangle, or a run length
   encoded array of rectangles. SkRegion may reduce the current SkCanvas clip,
   or may be drawn as one or more integer rectangles. SkRegion iterator returns
   the scan lines or rectangles contained by it, optionally intersecting a
   bounding rectangle.
*/
...
public:
  ...
  */
  ...
  /** Constructs a copy of an existing region.
      Copy constructor makes two regions identical by value. Internally, region
     and the returned result share pointer values. The underlying SkRect array
     is copied when modified.

      Creating a SkRegion copy is very efficient and never allocates memory.
      SkRegion are always copied by value from the interface; the underlying
  ...
  */
  ...
  /** Releases ownership of any shared data and deletes data if SkRegion is sole
   * owner.
   */
  ~SkRegion();
  ...
  */
  ...
  /** Compares SkRegion and other; returns true if they do not enclose the same
     area.
  ...
  */
  ...
  /** Exchanges SkIRect array of SkRegion and other. swap() internally exchanges
     pointers, so it is lightweight and does not allocate memory.

      swap() usage has largely been replaced by operator=(const SkRegion&
     region). SkPath do not copy their content on assignment until they are
  ...
  */
  ...
  /** Appends outline of SkRegion to path.
      Returns true if SkRegion is not empty; otherwise, returns false, and
     leaves path unmodified.
  ...
  */
  ...
  /** Constructs SkRegion with bounds (left, top, right, bottom).
      Returns true if left is less than right and top is less than bottom;
     otherwise, constructs empty SkRegion and returns false.
  ...
  */
  ...
  /** Constructs SkRegion as the union of SkIRect in rects array. If count is
      zero, constructs empty SkRegion. Returns false if constructed SkRegion is
     empty.
  ...
  */
  ...
  /** Returns true if SkRegion is a single rectangle and contains SkIRect
      ...
      Returns false if SkRegion is empty or SkIRect (left, top, right, bottom)
     is empty. May return false even though SkRegion contains (left, top, right,
     bottom).
  ...
  */
  bool quickContains(int32_t left,
                     int32_t top,
                     int32_t right,
  ...
  */
  ...
  /** Offsets SkRegion by ivector (dx, dy), writing result to dst. SkRegion may
     be passed as dst parameter, translating SkRegion in place. Has no effect if
     dst is nullptr. If SkRegion is empty, sets dst to empty.
  ...
  */
  ...
  /** Replaces SkRegion with the result of SkRegion op SkIRect (left, top,
     right, bottom). Returns true if replaced SkRegion is not empty.
  ...
  */
  ...
  /** \class SkRegion::Iterator
      Returns sequence of rectangles, sorted along y-axis, then x-axis, that
     make up SkRegion.
  */
  class SK_API Iterator
  {
  public:
    /** Initializes SkRegion::Iterator with an empty SkRegion. done() on
       SkRegion::Iterator returns true. Call reset() to initialized
       SkRegion::Iterator at a later time.
    ...
    */
    ...
    /** Returns SkIRect element in SkRegion. Does not return predictable results
       if SkRegion is empty.
  ...
  */
  ...
  public:
    ...
    */
    ...
    /** Returns true if SkRegion::Cliperator is pointing to final SkIRect in
       SkRegion.
    ...
    */
    ...
    /** Returns SkIRect element in SkRegion, intersected with clip passed to
       SkRegion::Cliperator constructor. Does not return predictable results if
       SkRegion is empty.
  ...
  */
  ...
  {
  public:
    /** Sets SkRegion::Spanerator to return line segments in SkRegion on scan
       line.
    ...
    */
    ...
    /** Advances iterator to next span intersecting SkRegion within line segment
       provided in constructor. Returns true if interval was found.
  ...
  /** Writes SkRegion to buffer, and returns number of bytes written.
      If buffer is nullptr, returns number number of bytes that would be
     written.
...
private:
  ...
  static bool RunsAreARect(const SkRegion::RunType runs[],
                           int count,
                           SkIRect* bounds);
