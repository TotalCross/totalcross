...
/** \struct SkYUVAIndex
    Describes from which image source and which channel to read each individual
   YUVA plane.

    SkYUVAIndex contains a index for which image source to read from and a enum
   for which channel to read from.
*/
struct SK_API SkYUVAIndex
{
  ...
  /** The index is a number between -1..3 which definies which image source to
   * read from, where -1 means the image source doesn't exist. The assumption is
   * we will always have image sources for each of YUV planes, but optionally
   * have image source for A plane. */
  int fIndex;
  /** The channel describes from which channel to read the info from. Currently
   * we only deal with YUV and NV12 and channel info is ignored. */
  SkColorChannel fChannel;

  static bool AreValidIndices(const SkYUVAIndex yuvaIndices[4], int* numPlanes)
  {
    // Note that 'numPlanes' is always filled in even if the indices are not
    // valid. This means it can always be used to process the backing resources
    // (but be careful of empty intervening slots).
    int maxSlotUsed = -1;
