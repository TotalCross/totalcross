...
public:
  ...
  public:
     ...
     *  Each time true is returned, the object is drawn (possibly with a
     * modified canvas and/or paint). When false is finally returned, drawing
  ...
  void computeFastBounds(const SkPaint& paint,
                         const SkRect& src,
                         SkRect* dst) const;
   ...
   *
   *  If any of the above are not met, return false and ignore the BlurShadowRec
   * parameter.
   */
  ...
  static sk_sp<SkDrawLooper> Deserialize(const void* data,
                                         size_t size,
                                         const SkDeserialProcs* procs = nullptr)
  {
    return sk_sp<SkDrawLooper>(static_cast<SkDrawLooper*>(
      SkFlattenable::Deserialize(kSkDrawLooper_Type, data, size, procs)
        .release()));
  }
