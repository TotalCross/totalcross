...
/*
 ...
 positioning kLCDRenderText_Flag   = 0x200,  //!< mask to enable subpixel glyph
 renderering kEmbeddedBitmapText_Flag = 0x400, //!< mask to enable embedded
 bitmap strikes kAutoHinting_Flag     = 0x800,  //!< mask to force Freetype's
 autohinter kVerticalText_Flag    = 0x1000,
 };
...
public:
  enum Flags
  {
     ...
     *  This is a last resort hinting method applied only if other hinting
     ...
     * bytecodes first; if that fails (e.g. there are no codes), then attempt to
     * autohint.
     */
     ...
     *  If both bytecode and auto hints are also specified, attempt to use the
     * bitmaps first; if that fails (e.g. there are no bitmaps), then attempt to
     * bytecode or autohint.
     */
    kEmbeddedBitmaps_Flag = 1 << 2,

    /**
     *  Use rounded metric values (e.g. advance).
     *  If either auto or bytecode hinting was used, apply those results to the
     * metrics of the glyphs as well. If no hinting was applied, the metrics
     * will just be rounded to the nearest integer.
     *
     *  This applies to calls that return metrics (e.g. measureText) and to
     * drawing the glyphs (see SkCanvas drawText and drawPosText).
     */
  ...
  static sk_sp<SkFont> Make(sk_sp<SkTypeface>,
                            SkScalar size,
                            MaskType,
                            uint32_t flags);
  static sk_sp<SkFont> Make(sk_sp<SkTypeface>,
  ...
  /**
   *  Return a font with the same attributes of this font, but with the
   * specified size. If size is not supported (e.g. <= 0 or non-finite) NULL
   * will be returned.
   */
  ...
  SkScalar measureText(const void* text,
                       size_t byteLength,
                       SkTextEncoding) const;
