...
{
public:
  // BoneIndices indicates which (of a maximum of 4 bones) a given vertex will
  // interpolate between. To indicate that a slot is not used, the convention is
  // to assign the bone index to 0.
  struct BoneIndices
  {
    uint32_t indices[4];
  ...
  // BoneWeights stores the interpolation weight for each of the (maximum of 4)
  // bones a given vertex interpolates between. To indicate that a slot is not
  // used, the weight for that slot should be 0.
  struct BoneWeights
  {
    float weights[4];
  ...
  /**
   *  Create a vertices by copying the specified arrays. texs, colors,
   * boneIndices, and boneWeights may be nullptr, and indices is ignored if
   * indexCount == 0.
   *
   *  boneIndices and boneWeights must either both be nullptr or both point to
   * valid data. If specified, they must both contain 'vertexCount' entries.
   */
  ...
  {
    return MakeCopy(
      mode, vertexCount, positions, texs, colors, nullptr, nullptr, isVolatile);
  }
  ...
  public:
    ...
    // Detach the built vertices object. After the first call, this will always
    // return null.
  ...
  private:
    ...
    // Extra storage for intermediate vertices in the case where the client
    // specifies indexed triangle fans. These get converted to indexed triangles
  ...
  /**
   *  Pack the vertices object into a byte buffer. This can be used to recreate
   * the vertices by calling Decode() with the buffer.
   */
...
private:
  ...
  // these are needed since we've manually sized our allocation (see
  // Builder::init)
  friend class SkNVRefCnt<SkVertices>;
  void operator delete(void* p);

  static sk_sp<SkVertices> Alloc(int vCount,
                                 int iCount,
                                 uint32_t builderFlags,
                                 size_t* arraySize);

  // we store this first, to pair with the refcnt in our base-class, so we don't
  // have an unnecessary pad between it and the (possibly 8-byte aligned) ptrs.
  uint32_t fUniqueID;
