...
public:
  ...
  void appendStages(SkRasterPipeline*,
                    SkColorSpace*,
                    SkArenaAlloc*,
  ...
  /** Returns the flags for this filter. Override in subclasses to return custom
   * flags.
   */
  ...
  */
   ...
   *  result = this(inner(...))
   *
   *  Due to internal limits, it is possible that this will return NULL, so the
   ...
   */
  static sk_sp<SkColorFilter> MakeMatrixFilterRowMajor255(
    const SkScalar array[20]);

  /** Construct a colorfilter that applies the srgb gamma curve to the RGB
   * channels */
  static sk_sp<SkColorFilter> MakeLinearToSRGBGamma();

  /** Construct a colorfilter that applies the inverse of the srgb gamma curve
   * to the RGB channels
   */
   ...
   * backend. It returns a GrFragmentProcessor that implemets the color filter
   * in GPU shader code.
   *
   *  The fragment processor receives a premultiplied input color and produces a
   * premultiplied output color.
   *
   *  A null return indicates that the color filter isn't implemented for the
   * GPU backend.
   */
    ...
    const SkDeserialProcs* procs = nullptr)
  {
    return sk_sp<SkColorFilter>(static_cast<SkColorFilter*>(
      SkFlattenable::Deserialize(kSkColorFilter_Type, data, size, procs)
        .release()));
  }
...
protected:
  ...
  /**
   *  If this subclass can optimally createa composition with the inner filter,
   * return it as a new filter (which the caller must unref() when it is done).
   * If no such optimization is known, return NULL.
   *
   *  e.g. result(color) == this_filter(inner(color))
   */
  virtual sk_sp<SkColorFilter> onMakeComposed(sk_sp<SkColorFilter>) const
  {
    return nullptr;
  }
...
private:
  /*
   *  Returns 1 if this is a single filter (not a composition of other filters),
   * otherwise it reutrns the number of leaf-node filters in a composition. This
   * should be the same value as the number of GrFragmentProcessors returned by
   * asFragmentProcessors's array parameter.
   *
   *  e.g. compose(filter, compose(compose(filter, filter), filter)) --> 4
   */
  virtual int privateComposedFilterCount() const { return 1; }

  virtual void onAppendStages(SkRasterPipeline*,
                              SkColorSpace*,
                              SkArenaAlloc*,
