...
public:
   ...
   *  If buffer != NULL, copy size bytes into buffer, return how many were
   * copied.
   *  @param buffer when NULL skip size bytes, otherwise copy size bytes into
   * buffer
  ...
  /** Seeks to an relative offset in the stream. If this cannot be done, returns
   * false. If an attempt is made to move to a position outside the stream, the
   * position will be set to the closest point within the stream (beginning or
   * end).
   */
  ...
  // SkStreamMemory
  /** Returns the starting address for the data. If this cannot be done, returns
   * NULL. */
...
/** SkStreamRewindable is a SkStream for which rewind and duplicate are
 * required. */
...
/** SkStreamSeekable is a SkStreamRewindable for which position, seek, move, and
 * fork are required. */
...
private:
  ...
  explicit SkFILEStream(std::shared_ptr<FILE>,
                        size_t size,
                        size_t offset,
...
public:
  ...
  static std::unique_ptr<SkMemoryStream> MakeCopy(const void* data,
                                                  size_t length);

  /** Returns a stream with a bare pointer reference to the input data. */
  static std::unique_ptr<SkMemoryStream> MakeDirect(const void* data,
                                                    size_t length);
  ...
  /** Resets the stream to the specified data and length,
      ...
      if copyData is true, the stream makes a private copy of the data
  */
  virtual void setMemory(const void* data,
                         size_t length,
                         bool copyData = false);
...
public:
  ...
  /** Equivalent to writeToStream() followed by reset(), but may save memory
   * use. */
  bool writeToAndReset(SkWStream* dst);

  /** Equivalent to writeToStream() followed by reset(), but may save memory
     use. When the dst is also a SkDynamicMemoryWStream, the implementation is
     constant time. */
