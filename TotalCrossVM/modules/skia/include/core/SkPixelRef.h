...
/** \class SkPixelRef
   ...
   SkBitmap. This class can be shared/accessed between multiple threads.
*/
...
public:
  ...
  */
  ...
  // Register a listener that may be called the next time our generation ID
  // changes.
  //
  // We'll only call the listener if we're confident that we are the only
  // SkPixelRef with this generation ID.  If our generation ID changes and we
  // decide not to call the listener, we'll never call it: you must add a new
  // listener for each generation ID change.  We also won't call the listener
  // when we're certain no one knows what our generation ID is.
  //
  ...
  // Call when this pixelref is part of the key to a resourcecache entry. This
  // allows the cache to know automatically those entries can be purged when
  // this pixelref is changed or deleted.
...
private:
  ...
  // Set true by caches when they cache content that's derived from the current
  // pixels.
