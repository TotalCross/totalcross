...
/* Generated by tools/bookmaker from include/core/SkCanvas.h and
   docs/SkCanvas_Reference.bmh on 2018-08-28 10:32:58. Additional documentation
   and examples can be found at: https://skia.org/user/api/SkCanvas_Reference

   You may edit either file directly. Structural changes to public interfaces
   require editing both files. After editing docs/SkCanvas_Reference.bmh, run:
...
/** \class SkCanvas
   ...
   and transformed. SkCanvas contains a stack of SkMatrix and clip values.
...
public:
  /** Allocates raster SkCanvas that will draw directly into pixels.
      ...
      rowBytes is zero or large enough to contain info width pixels of
     SkColorType.
     ...
     than info width times bytes required for SkColorType.

      Pixel buffer size should be info height times computed rowBytes.
      Pixels are not initialized.
      To access pixels after drawing, call flush() or peekPixels().

      @param info      width, height, SkColorType, SkAlphaType, SkColorSpace, of
     raster surface; width, or height, or both, may be zero
      @param pixels    pointer to destination pixels buffer
      @param rowBytes  interval from one SkSurface row to the next, or zero
      @param props     LCD striping orientation and setting for device
  ...
  */
  ...
  /** Allocates raster SkCanvas specified by inline image specification.
     Subsequent SkCanvas calls draw into pixels. SkColorType is set to
     kN32_SkColorType. SkAlphaType is set to kPremul_SkAlphaType. To access
     pixels after drawing, call flush() or peekPixels().
      ...
      pixels is not nullptr;
      rowBytes is zero or large enough to contain width pixels of
     kN32_SkColorType.
      ...
      @param width     pixel column count on raster surface created; must be
     zero or greater
      @param height    pixel row count on raster surface created; must be zero
     or greater
      @param pixels    pointer to destination pixels buffer; buffer size should
     be height times rowBytes
      @param rowBytes  interval from one SkSurface row to the next, or zero
      @return          SkCanvas if all parameters are valid; otherwise, nullptr
  */
  static std::unique_ptr<SkCanvas> MakeRasterDirectN32(int width,
                                                       int height,
                                                       SkPMColor* pixels,
                                                       size_t rowBytes)
  {
    return MakeRasterDirect(
      SkImageInfo::MakeN32Premul(width, height), pixels, rowBytes);
  }
  ...
  */
  ...
  /** Creates SkCanvas of the specified dimensions without a SkSurface.
      ...
      direction and order. Since a platform may dynamically change its direction
     when the device is rotated, and since a platform may have multiple monitors
     with different characteristics, it is best not to rely on this legacy
     behavior.
  ...
  */
  ...
  /** Returns SkImageInfo for SkCanvas. If SkCanvas is not associated with
     raster surface or GPU surface, returned SkColorType is set to
  ...
  */
  ...
  /** Copies SkSurfaceProps, if SkCanvas is associated with raster surface or
      GPU surface, and returns true. Otherwise, returns false and leave props
     unchanged.
  ...
  */
  ...
  /** Triggers the immediate execution of all pending draw operations.
      If SkCanvas is associated with GPU surface, resolves all pending GPU
     operations. If SkCanvas is associated with raster surface, has no effect;
     raster draw operations are never deferred.
  */
  void flush();

  /** Gets the size of the base or root layer in global canvas coordinates. The
      origin of the base layer is always (0,0). The area available for drawing
     may be smaller (due to clipping or saveLayer).
  ...
  */
  ...
  /** Creates SkSurface matching info and props, and associates it with
     SkCanvas. Returns nullptr if no match found.

      If props is nullptr, matches SkSurfaceProps in SkCanvas. If props is
     nullptr and SkCanvas does not have SkSurfaceProps, creates SkSurface with
     default SkSurfaceProps.
  ...
  */
  ...
  /** Returns the pixel base address, SkImageInfo, rowBytes, and origin if the
     pixels can be read directly. The returned address is only valid while
     SkCanvas is in scope and unchanged. Any SkCanvas call or SkSurface call may
     invalidate the returned address and other returned values.
      ...
      @param origin    storage for SkCanvas top layer origin, its top-left
     corner; may be nullptr
  ...
  */
  ...
  /** Returns custom context that tracks the SkMatrix and clip.
      ...
      Use SkRasterHandleAllocator to blend Skia drawing with custom drawing,
     typically performed by the host platform user interface. The custom context
     returned is generated by SkRasterHandleAllocator::MakeCanvas, which creates
     a custom canvas with raster storage for the drawing destination.
  ...
  */
  ...
  /** Returns true if SkCanvas has direct access to its pixels.
      ...
      Pixels are readable when SkBaseDevice is raster. Pixels are not readable
     when SkCanvas is returned from GPU surface, returned by
     SkDocument::beginPage, returned by SkPictureRecorder::beginRecording, or
     SkCanvas is the base of a utility class like SkDebugCanvas.
      ...
      @param pixmap  storage for pixel state if pixels are readable; otherwise,
     ignored
      @return        true if SkCanvas has direct access to pixels
  */
  bool peekPixels(SkPixmap* pixmap);

  /** Copies SkRect of pixels from SkCanvas into dstPixels. SkMatrix and clip
     are ignored.

      Source SkRect corners are (srcX, srcY) and (imageInfo().width(),
     imageInfo().height()). Destination SkRect corners are (0, 0) and
     (dstInfo.width(), dstInfo.height()). Copies each readable pixel
     intersecting both rectangles, without scaling, converting to
     dstInfo.colorType() and dstInfo.alphaType() if required.

      Pixels are readable when SkBaseDevice is raster, or backed by a GPU.
      Pixels are not readable when SkCanvas is returned by
     SkDocument::beginPage, returned by SkPictureRecorder::beginRecording, or
     SkCanvas is the base of a utility class like SkDebugCanvas.
     ...
     destination.
      ...
      - SkCanvas pixels could not be converted to dstInfo.colorType() or
     dstInfo.alphaType().
      - SkCanvas pixels are not readable; for instance, SkCanvas is
     document-based.
      - dstRowBytes is too small to contain one row of pixels.

      @param dstInfo      width, height, SkColorType, and SkAlphaType of
     dstPixels
      @param dstPixels    storage for pixels; dstInfo.height() times
     dstRowBytes, or larger
      @param dstRowBytes  size of one destination row; dstInfo.width() times
  ...
  */
  ...
  /** Copies SkRect of pixels from SkCanvas into pixmap. SkMatrix and clip are
      ...
      Source SkRect corners are (srcX, srcY) and (imageInfo().width(),
     imageInfo().height()). Destination SkRect corners are (0, 0) and
     (pixmap.width(), pixmap.height()). Copies each readable pixel intersecting
     both rectangles, without scaling, converting to pixmap.colorType() and
     ...
     SkCanvas is the base of a utility class like SkDebugCanvas.
      ...
      are copied. pixmap pixels contents outside SkRect intersection are
     unchanged.
     ...
     pixmap.
      ...
      Does not copy, and returns false if:
      - Source and destination rectangles do not intersect.
      - SkCanvas pixels could not be converted to pixmap.colorType() or
     pixmap.alphaType().
      - SkCanvas pixels are not readable; for instance, SkCanvas is
     document-based.
  ...
  */
  ...
  /** Copies SkRect of pixels from SkCanvas into bitmap. SkMatrix and clip are
      ...
      Source SkRect corners are (srcX, srcY) and (imageInfo().width(),
     imageInfo().height()). Destination SkRect corners are (0, 0) and
     (bitmap.width(), bitmap.height()). Copies each readable pixel intersecting
     both rectangles, without scaling, converting to bitmap.colorType() and
     ...
     bitmap.
      ...
      Does not copy, and returns false if:
      - Source and destination rectangles do not intersect.
      - SkCanvas pixels could not be converted to bitmap.colorType() or
     bitmap.alphaType().
      - SkCanvas pixels are not readable; for instance, SkCanvas is
     document-based.
  ...
  */
  ...
  /** Copies SkRect from pixels to SkCanvas. SkMatrix and clip are ignored.
      ...
      converting to imageInfo().colorType() and imageInfo().alphaType() if
     required.

      Pixels are writable when SkBaseDevice is raster, or backed by a GPU.
      Pixels are not writable when SkCanvas is returned by
     SkDocument::beginPage, returned by SkPictureRecorder::beginRecording, or
     SkCanvas is the base of a utility class like SkDebugCanvas.
      ...
      imageInfo().alphaType().
      - SkCanvas pixels are not writable; for instance, SkCanvas is
     document-based.
      - rowBytes is too small to contain one row of pixels.

      @param info      width, height, SkColorType, and SkAlphaType of pixels
      @param pixels    pixels to copy, of size info.height() times rowBytes, or
     larger
      @param rowBytes  size of one row of pixels; info.width() times pixel size,
     or larger
      @param x         offset into SkCanvas writable pixels on x-axis; may be
     negative
      @param y         offset into SkCanvas writable pixels on y-axis; may be
     negative
  ...
  */
  ...
  /** Copies SkRect from pixels to SkCanvas. SkMatrix and clip are ignored.
     ...
     SkCanvas is the base of a utility class like SkDebugCanvas.
      ...
      - bitmap pixels could not be converted to SkCanvas imageInfo().colorType()
     or imageInfo().alphaType().
      - SkCanvas pixels are not writable; for instance, SkCanvas is document
     based.
  ...
  */
  ...
  /** Saves SkMatrix and clip.
      ...
      SkMatrix may be changed by translate(), scale(), rotate(), skew(),
     concat(), setMatrix(), and resetMatrix(). Clip may be changed by
  ...
  */
  ...
  /** Saves SkMatrix and clip, and allocates a SkBitmap for subsequent drawing.
      Calling restore() discards changes to SkMatrix and clip, and draws the
     SkBitmap.
     ...
     clipRect(), clipRRect(), clipPath(), clipRegion().
      ...
      SkRect bounds suggests but does not define the SkBitmap size. To clip
     drawing to a specific rectangle, use clipRect().
  ...
  */
  ...
  /** Saves SkMatrix and clip, and allocates a SkBitmap for subsequent drawing.
     ...
     clipRect(), clipRRect(), clipPath(), clipRegion().
      ...
      SkRect bounds suggests but does not define the layer size. To clip drawing
     to a specific rectangle, use clipRect().
  ...
  */
  ...
  /** Saves SkMatrix and clip, and allocates a SkBitmap for subsequent drawing.
     ...
     saves.
      ...
      Draw text on an opaque background so that LCD text blends correctly with
     the prior layer. LCD text drawn on a background with transparency may
     result in incorrect blending.
  ...
  */
  ...
  /** \enum SkCanvas::SaveLayerFlagsSet
      SaveLayerFlags provides options that may be used in any combination in
     SaveLayerRec, defining how layer allocated by saveLayer() operates. It may
     be set to zero, kPreserveLCDText_SaveLayerFlag,
     kInitWithPrevious_SaveLayerFlag, or both flags.
  */
  enum SaveLayerFlagsSet
  {
    kPreserveLCDText_SaveLayerFlag = 1 << 1, //!< creates layer for LCD text
    kInitWithPrevious_SaveLayerFlag =
      1 << 2, //!< initializes with previous contents
  ...
  struct SaveLayerRec
  {
    ...
    */
    ...
    /** Sets fBounds, fPaint, and fSaveLayerFlags; sets fBackdrop to nullptr.
        ...
        @param paint           applied to layer when overlaying prior layer; may
       be nullptr
    ...
    */
    ...
    /** Sets fBounds, fPaint, fBackdrop, and fSaveLayerFlags.
        ...
        @param paint           applied to layer when overlaying prior layer;
                               may be nullptr
        @param backdrop        prior layer copied with SkImageFilter; may be
       nullptr
        @param saveLayerFlags  SaveLayerRec options to modify layer
    ...
    */
    ...
    /** Experimental. Not ready for general use.
        Sets fBounds, fPaint, fBackdrop, fClipMask, fClipMatrix, and
       fSaveLayerFlags. clipMatrix uses alpha channel of image, transformed by
       clipMatrix, to clip layer when drawn to SkCanvas.

        Implementation is not complete; has no effect if SkBaseDevice is
       GPU-backed.

        @param bounds          layer dimensions; may be nullptr
        @param paint           graphics state applied to layer when overlaying
  ...
  */
  ...
  /** Restores state to SkMatrix and clip values when save(), saveLayer(),
     ...
     saveCount.
      ...
      Restores state to initial values if saveCount is less than or equal to
     one.
  ...
  */
  ...
  /** Rotates SkMatrix by degrees about a point at (px, py). Positive degrees
     rotates clockwise.

      Mathematically, constructs a rotation matrix; premultiplies the rotation
     matrix by a translation matrix; then replaces SkMatrix with the resulting
     matrix premultiplied with SkMatrix.
  ...
  */
  ...
  /** Skews SkMatrix by sx on the x-axis and sy on the y-axis. A positive value
     of sx skews the drawing right as y-axis values increase; a positive value
     of sy skews the drawing down as x-axis values increase.

      Mathematically, replaces SkMatrix with a skew matrix premultiplied with
     SkMatrix.
  ...
  */
  ...
  /** Sets the maximum clip rectangle, which can be set by clipRect(),
     ...
     specified rect. The maximum clip affects only future clipping operations;
     it is not retroactive. The clip restriction is not recorded in pictures.
  ...
  */
  ...
  /** Replaces clip with the intersection or difference of clip and path,
      with an aliased or anti-aliased clip edge. SkPath::FillType determines if
     path describes the area inside or outside its contours; and if path contour
     overlaps itself or another path contour, whether the overlaps form part of
     the area. path is transformed by SkMatrix before it is combined with clip.
  ...
  */
  ...
  /** Replaces clip with the intersection or difference of clip and path.
      ...
      describes the area inside or outside its contours; and if path contour
     overlaps itself or another path contour, whether the overlaps form part of
     the area. path is transformed by SkMatrix before it is combined with clip.
  ...
  */
  ...
  /** Replaces clip with the intersection or difference of clip and SkRegion
     deviceRgn. Resulting clip is aliased; pixels are fully contained by the
     clip. deviceRgn is unaffected by SkMatrix.
  ...
  */
  ...
  /** Returns true if SkRect rect, transformed by SkMatrix, can be quickly
     determined to be outside of clip. May return false even though rect is
     outside of clip.
     ...
     draw calls.
      ...
      @return      true if rect, transformed by SkMatrix, does not intersect
     clip
  */
  bool quickReject(const SkRect& rect) const;

  /** Returns true if path, transformed by SkMatrix, can be quickly determined
     to be outside of clip. May return false even though path is outside of
     clip.
     ...
     draw calls.
      ...
      @return      true if path, transformed by SkMatrix, does not intersect
     clip
  */
  bool quickReject(const SkPath& path) const;

  /** Returns bounds of clip, transformed by inverse of SkMatrix. If clip is
     empty, return SkRect::MakeEmpty, where all SkRect sides equal zero.

      SkRect returned is outset by one to account for partial pixel coverage if
  ...
  */
  ...
  /** Returns bounds of clip, transformed by inverse of SkMatrix. If clip is
     empty, return false, and set bounds to SkRect::MakeEmpty, where all SkRect
     sides equal zero.
  ...
  */
  ...
  /** Returns SkIRect bounds of clip, unaffected by SkMatrix. If clip is empty,
      return false, and set bounds to SkRect::MakeEmpty, where all SkRect sides
     equal zero.
  ...
  */
  ...
  /** Makes SkCanvas contents undefined. Subsequent calls that read SkCanvas
     pixels, such as drawing with SkBlendMode, return undefined results.
     discard() does not change clip or SkMatrix.

      discard() may do nothing, depending on the implementation of SkSurface or
     SkBaseDevice that created SkCanvas.
      ...
      any cached data is deleted when owning SkSurface or SkBaseDevice is
     deleted.
  */
  ...
  */
  ...
  /** \enum SkCanvas::PointMode
      Selects if an array of points are drawn as discrete points, as lines, or
     as an open polygon.
  */
  enum PointMode
  {
  ...
  /** Draws pts using clip, SkMatrix and SkPaint paint.
      ...
      mode may be one of: kPoints_PointMode, kLines_PointMode, or
     kPolygon_PointMode.
      ...
      circle of diameter SkPaint stroke width. If paint is set to
     SkPaint::kSquare_Cap or SkPaint::kButt_Cap, each point draws a square of
     ...
     is odd, the final point is ignored.
      ...
      If mode is kPolygon_PointMode, each adjacent pair of points draws a line
     segment. count minus one lines are drawn; the first and last point are used
     once.
      ...
      SkPaint::Join, and unlike drawPath(), does not create a mask from all
     points and lines before drawing.
  ...
  */
  ...
  /** Draws point at (x, y) using clip, SkMatrix and SkPaint paint.
      ...
      SkPaint stroke width. If paint is set to SkPaint::kSquare_Cap or
     SkPaint::kButt_Cap, draw a square of width and height SkPaint stroke width.
      SkPaint::Style is ignored, as if were set to SkPaint::kStroke_Style.
  ...
  */
  ...
  /** Draws SkRegion region using clip, SkMatrix, and SkPaint paint.
      ...
      @param paint   SkPaint stroke or fill, blend, color, and so on, used to
     draw
  */
  ...
  */
  ...
  /** Draws SkRRect rrect using clip, SkMatrix, and SkPaint paint.
     ...
     rectangle, or may have any combination of positive non-square radii for the
     four corners.
  ...
  */
  ...
  /** Draws circle at (cx, cy) with radius using clip, SkMatrix, and SkPaint
     ...
     SkPaint::Style determines if circle is stroked or filled; if stroked,
  ...
  */
  ...
  /** Draws arc using clip, SkMatrix, and SkPaint paint.
     ...
     true, draw a wedge that includes lines from oval center to arc end points.
     If useCenter is false, draw arc between end points.
      ...
      @param paint       SkPaint stroke or fill, blend, color, and so on, used
     to draw
  */
  ...
  /** Draws SkRRect bounded by SkRect rect, with corner radii (rx, ry) using
     clip, SkMatrix, and SkPaint paint.
      ...
      If rx plus ry exceeds rect width or rect height, radii are scaled down to
     fit. If rx and ry are zero, SkRRect is drawn as SkRect and if stroked is
     affected by SkPaint::Join.
  ...
  */
  ...
  /** Draws SkPath path using clip, SkMatrix, and SkPaint paint.
     ...
     closed.
      ...
      In paint: SkPaint::Style determines if SkRRect is stroked or filled:
      if filled, SkPath::FillType determines whether path contour describes
     inside or outside of fill; if stroked, SkPaint stroke width describes the
     line thickness, SkPaint::Cap describes line ends, and SkPaint::Join
  ...
  */
  ...
  /** Draws SkImage image, with its top-left corner at (left, top),
      ...
      If paint is supplied, apply SkColorFilter, alpha, SkImageFilter,
     SkBlendMode, and SkDrawLooper. If image is kAlpha_8_SkColorType, apply
     SkShader. If paint contains SkMaskFilter, generate mask from image bounds.
     If generated mask extends beyond image bounds, replicate image edge colors,
     just as SkShader made from SkImage::makeShader with
     SkShader::kClamp_TileMode set replicates the image edge color when it
     samples outside of its bounds.

      @param image  uncompressed rectangular map of pixels
      @param left   left side of image
      @param top    top side of image
      @param paint  SkPaint containing SkBlendMode, SkColorFilter,
     SkImageFilter, and so on; or nullptr
  */
  ...
  */
  ...
  /** \enum SkCanvas::SrcRectConstraint
      ...
      SkImageFilter in SkPaint may sample multiple pixels in the image. Source
     SkRect restricts the bounds of pixels that may be read. SkImageFilter may
     slow down if it cannot read outside the bounds, when sampling near the edge
     of source SkRect. SrcRectConstraint specifies whether an SkImageFilter is
     allowed to read pixels outside source SkRect.
  */
  ...
  /** Draws SkRect src of SkImage image, scaled and translated to fill SkRect
     dst. Additionally transform draw using clip, SkMatrix, and optional SkPaint
     paint.
     ...
     colors, just as SkShader made from SkImage::makeShader with
     SkShader::kClamp_TileMode set replicates the image edge color when it
     samples outside of its bounds.

      constraint set to kStrict_SrcRectConstraint limits SkPaint SkFilterQuality
     to sample within src; set to kFast_SrcRectConstraint allows sampling
     outside to improve performance.
  ...
  */
  ...
  /** Draws SkIRect isrc of SkImage image, scaled and translated to fill SkRect
     dst. Note that isrc is on integer pixel boundaries; dst may include
     fractional boundaries. Additionally transform draw using clip, SkMatrix,
     ...
     samples outside of its bounds.
      ...
      constraint set to kStrict_SrcRectConstraint limits SkPaint SkFilterQuality
     to sample within isrc; set to kFast_SrcRectConstraint allows sampling
     outside to improve performance.
  ...
  */
  ...
  /** Draws SkImage image, scaled and translated to fill SkRect dst, using clip,
     SkMatrix, and optional SkPaint paint.
     ...
     samples outside of its bounds.
      ...
      constraint set to kStrict_SrcRectConstraint limits SkPaint SkFilterQuality
     to sample within image; set to kFast_SrcRectConstraint allows sampling
     outside to improve performance.
  ...
  */
  ...
  /** Draws SkIRect isrc of SkImage image, scaled and translated to fill SkRect
     dst. isrc is on integer pixel boundaries; dst may include fractional
     boundaries. Additionally transform draw using clip, SkMatrix, and optional
     SkPaint paint.
  ...
  */
  ...
  /** Draws SkImage image stretched proportionally to fit into SkRect dst.
      SkIRect center divides the image into nine sections: four sides, four
     corners, and the center. Corners are unmodified or scaled down
     proportionately if their sides are larger than dst; center and four sides
     are scaled to fit remaining space, if any.

      Additionally transform draw using clip, SkMatrix, and optional SkPaint
     paint.

      If SkPaint paint is supplied, apply SkColorFilter, alpha, SkImageFilter,
      SkBlendMode, and SkDrawLooper. If image is kAlpha_8_SkColorType, apply
     SkShader. If paint contains SkMaskFilter, generate mask from image bounds.
     If paint SkFilterQuality set to kNone_SkFilterQuality, disable pixel
     filtering. For all other values of paint SkFilterQuality, use
     kLow_SkFilterQuality to filter pixels. Any SkMaskFilter on paint is ignored
     as is paint anti-aliasing state.
     ...
     SkImageFilter, and so on; or nullptr
  */
  void drawImageNine(const SkImage* image,
  ...
  /** Draws SkImage image stretched proportionally to fit into SkRect dst.
      SkIRect center divides the image into nine sections: four sides, four
     corners, and the center. Corners are not scaled, or scaled down
     proportionately if their sides are larger than dst; center and four sides
     are scaled to fit remaining space, if any.
  ...
  */
  ...
  /** Draws SkBitmap bitmap, with its top-left corner at (left, top),
      ...
      If SkPaint paint is not nullptr, apply SkColorFilter, alpha,
     SkImageFilter, SkBlendMode, and SkDrawLooper. If bitmap is
     kAlpha_8_SkColorType, apply SkShader. If paint contains SkMaskFilter,
     generate mask from bitmap bounds.

      If generated mask extends beyond bitmap bounds, replicate bitmap edge
     colors, just as SkShader made from SkShader::MakeBitmapShader with
      SkShader::kClamp_TileMode set replicates the bitmap edge color when it
     samples outside of its bounds.
  ...
  */
  ...
  /** Draws SkRect src of SkBitmap bitmap, scaled and translated to fill SkRect
     dst. Additionally transform draw using clip, SkMatrix, and optional SkPaint
     paint.

      If SkPaint paint is supplied, apply SkColorFilter, alpha, SkImageFilter,
      SkBlendMode, and SkDrawLooper. If bitmap is kAlpha_8_SkColorType, apply
     SkShader. If paint contains SkMaskFilter, generate mask from bitmap bounds.
     ...
     SkImageFilter, and so on; or nullptr
      @param constraint  filter strictly within src or draw faster
  */
  void drawBitmapRect(const SkBitmap& bitmap,
                      const SkRect& src,
                      const SkRect& dst,
                      const SkPaint* paint,
                      SrcRectConstraint constraint = kStrict_SrcRectConstraint);
  ...
  /** Draws SkIRect isrc of SkBitmap bitmap, scaled and translated to fill
     SkRect dst. isrc is on integer pixel boundaries; dst may include fractional
     boundaries. Additionally transform draw using clip, SkMatrix, and optional
     SkPaint paint.
  ...
  */
  ...
  /** Draws SkBitmap bitmap, scaled and translated to fill SkRect dst.
     ...
     samples outside of its bounds.
      ...
      constraint set to kStrict_SrcRectConstraint limits SkPaint SkFilterQuality
     to sample within bitmap; set to kFast_SrcRectConstraint allows sampling
     outside to improve performance.
  ...
  */
  ...
  /** Draws SkBitmap bitmap stretched proportionally to fit into SkRect dst.
      SkIRect center divides the bitmap into nine sections: four sides, four
     corners, and the center. Corners are not scaled, or scaled down
     proportionately if their sides are larger than dst; center and four sides
     are scaled to fit remaining space, if any.
  ...
  */
  ...
  /** Draws SkBitmap bitmap stretched proportionally to fit into SkRect dst.
      ...
      Each intersection of an even-numbered row and column is fixed; like the
     corners of drawBitmapNine(), fixed lattice elements never scale larger than
     their initial size and shrink proportionately when all fixed elements
     exceed the bitmap dimension. All other grid elements scale to fill the
     ...
     SkImageFilter, and so on; or nullptr
  */
  void drawBitmapLattice(const SkBitmap& bitmap,
  ...
  */
  ...
  /** Draws text, with origin at (x, y), using clip, SkMatrix, and SkPaint
     paint.

      text meaning depends on SkPaint::TextEncoding; by default, text is encoded
     as UTF-8.

      x and y meaning depends on SkPaint::Align and SkPaint vertical text; by
     default text draws left to right, positioning the first glyph left side
     bearing at x and its baseline at y. Text size is affected by SkMatrix and
     SkPaint text size.

      All elements of paint: SkPathEffect, SkMaskFilter, SkShader,
      SkColorFilter, SkImageFilter, and SkDrawLooper; apply to text. By default,
     draws filled 12 point black glyphs.
  ...
  */
  ...
  /** Draws null terminated string, with origin at (x, y), using clip, SkMatrix,
     and SkPaint paint.
      ...
      string meaning depends on SkPaint::TextEncoding; by default, strings are
     encoded as UTF-8. Other values of SkPaint::TextEncoding are unlikely to
     produce the desired results, since zero bytes may be embedded in the
     string.

      x and y meaning depends on SkPaint::Align and SkPaint vertical text; by
     default string draws left to right, positioning the first glyph left side
     bearing at x and its baseline at y. Text size is affected by SkMatrix and
     SkPaint text size.
  ...
  */
  ...
  /** Draws null terminated string, with origin at (x, y), using clip, SkMatrix,
     ...
     draws filled 12 point black glyphs.
      ...
      @param paint   text size, blend, color, and so on, used to draw
  */
  void drawString(const SkString& string,
                  SkScalar x,
                  SkScalar y,
                  const SkPaint& paint);

  /** Draws each glyph in text with the origin in pos array, using clip,
     SkMatrix, and SkPaint paint. The number of entries in pos array must match
     the number of glyphs described by byteLength of text.

      text meaning depends on SkPaint::TextEncoding; by default, text is encoded
     as UTF-8. pos elements meaning depends on SkPaint vertical text; by default
  ...
  */
  ...
  /** Draws each glyph in text with its (x, y) origin composed from xpos array
     and constY, using clip, SkMatrix, and SkPaint paint. The number of entries
     in xpos array must match the number of glyphs described by byteLength of
     text.

      text meaning depends on SkPaint::TextEncoding; by default, text is encoded
     as UTF-8. xpos elements meaning depends on SkPaint vertical text; by
     default each glyph left side bearing is positioned at an xpos element and
      its baseline is positioned at constY. Text size is affected by SkMatrix
  ...
  */
  ...
  /** Draws text, transforming each glyph by the corresponding SkRSXform,
      ...
      SkRSXform xform array specifies a separate square scale, rotation, and
     translation for each glyph. xform does not affect paint SkShader.

      Optional SkRect cullRect is a conservative bounds of text, taking into
     account SkRSXform and paint. If cullRect is outside of clip, canvas can
     skip drawing.
     ...
     draws filled 12 point black glyphs.
      ...
      @param text        character code points or glyphs drawn
      @param byteLength  byte length of text array
      @param xform       SkRSXform rotates, scales, and translates each glyph
     individually
      @param cullRect    SkRect bounds of text for efficient clipping; or
     nullptr
      @param paint       text size, blend, color, and so on, used to draw
  */
  void drawTextRSXform(const void* text,
                       size_t byteLength,
  ...
  /** Draws SkTextBlob blob at (x, y), using clip, SkMatrix, and SkPaint paint.
      ...
      blob contains glyphs, their positions, and paint attributes specific to
     text: SkTypeface, SkPaint text size, SkPaint text scale x, SkPaint text
     skew x, SkPaint::Align, SkPaint::Hinting, anti-alias, SkPaint fake bold,
      SkPaint font embedded bitmaps, SkPaint full hinting spacing, LCD text,
     SkPaint linear text, and SkPaint subpixel text.
      ...
      SkColorFilter, SkPaint dither, SkDrawLooper, SkMaskFilter, SkPathEffect,
     SkShader, and SkPaint::Style; apply to blob. If SkPaint contains
     SkPaint::kStroke_Style: SkPaint miter limit, SkPaint::Cap, SkPaint::Join,
     and SkPaint stroke width; apply to SkPath created from blob.

      @param blob   glyphs, positions, and their paints' text size, typeface,
     and so on
  ...
  */
  ...
  /** Draws SkPicture picture, using clip and SkMatrix; transforming picture
     with SkMatrix matrix, if provided; and use SkPaint paint alpha,
     SkColorFilter, SkImageFilter, and SkBlendMode, if provided.
     ...
     restore().
      ...
      @param picture  recorded drawing commands to play
      @param matrix   SkMatrix to rotate, scale, translate, and so on; may be
     nullptr
      @param paint    SkPaint to apply transparency, filtering, and so on; may
     be nullptr
  */
  void drawPicture(const SkPicture* picture,
  ...
  */
  ...
  /** Draws SkVertices vertices, a triangle mesh, using clip and SkMatrix.
      If vertices texs and vertices colors are defined in vertices, and SkPaint
     paint contains SkShader, SkBlendMode mode combines vertices colors with
     SkShader.

      @param vertices  triangle mesh to draw
      @param mode      combines vertices colors with SkShader, if both are
     present
      @param paint     specifies the SkShader, used as SkVertices texture; may
     be nullptr
  */
  ...
  */
  ...
  /** Draws SkVertices vertices, a triangle mesh, using clip and SkMatrix. Bone
     data is used to deform vertices with bone weights. If vertices texs and
     vertices colors are defined in vertices, and SkPaint paint contains
     SkShader, SkBlendMode mode combines vertices colors with SkShader. The
     first element of bones should be an object to world space transformation
     matrix that will be applied before performing mesh deformations. If no such
     transformation is needed, it should be the identity matrix. boneCount must
     be at most 80, and thus the size of bones should be at most 80.
  ...
  */
  ...
  /** Draws a Coons patch: the interpolation of four cubics with shared corners,
      ...
      alpha, SkImageFilter, and SkBlendMode. If SkShader is provided it is
     treated as Coons patch texture; SkBlendMode mode combines color colors and
     SkShader if both are provided.

      SkPoint array cubics specifies four SkPath cubic starting at the top-left
     corner, in clockwise order, sharing every fourth point. The last SkPath
     cubic ends at the first point.

      Color array color associates colors with corners in top-left, top-right,
      bottom-right, bottom-left order.

      If paint contains SkShader, SkPoint array texCoords maps SkShader as
     texture to corners in top-left, top-right, bottom-right, bottom-left order.

      @param cubics     SkPath cubic array, sharing common points
      @param colors     color array, one for each corner
      @param texCoords  SkPoint array of texture coordinates, mapping SkShader
     to corners; may be nullptr
      @param mode       SkBlendMode for colors, and for SkShader if paint has
     one
      @param paint      SkShader, SkColorFilter, SkBlendMode, used to draw
  */
  void drawPatch(const SkPoint cubics[12],
  ...
  /** Draws SkPath cubic Coons patch: the interpolation of four cubics with
     shared corners, associating a color, and optionally a texture SkPoint, with
     each corner.
  ...
  */
  ...
  /** Draws a set of sprites from atlas, using clip, SkMatrix, and optional
     SkPaint paint. paint uses anti-alias, alpha, SkColorFilter, SkImageFilter,
     and SkBlendMode to draw, if present. For each entry in the array, SkRect
     tex locates sprite in atlas, and SkRSXform xform transforms it into
     destination space.
     ...
     treating sprite as source and colors as destination. Optional cullRect is a
     conservative bounds of all transformed sprites. If cullRect is outside of
     clip, canvas can skip drawing.
      ...
      @param colors    one per sprite, blended with sprite using SkBlendMode;
     may be nullptr
      @param count     number of sprites to draw
      @param mode      SkBlendMode combining colors and sprites
      @param cullRect  bounds of transformed sprites for efficient clipping; may
     be nullptr
      @param paint     SkColorFilter, SkImageFilter, SkBlendMode, and so on; may
     be nullptr
  */
  void drawAtlas(const SkImage* atlas,
                 const SkRSXform xform[],
                 const SkRect tex[],
  ...
  */
  ...
  {
    this->drawAtlas(
      atlas.get(), xform, tex, colors, count, mode, cullRect, paint);
  }
  ...
  */
  ...
  {
    this->drawAtlas(
      atlas, xform, tex, nullptr, count, SkBlendMode::kDst, cullRect, paint);
  }
  ...
  */
  ...
  /** Draws SkDrawable drawable using clip and SkMatrix, concatenated with
      ...
      so that SkDrawable::draw() can be called when the operation is finalized.
     To force immediate drawing, call SkDrawable::draw() instead.
  ...
  */
  ...
  /** Associates SkRect on SkCanvas with an annotation; a key-value pair, where
     the key is a null-terminated utf8 string, and optional value is stored as
     SkData.
  ...
  */
  ...
  /** Associates SkRect on SkCanvas when an annotation; a key-value pair, where
     the key is a null-terminated utf8 string, and optional value is stored as
     SkData.
     ...
     drawing to document pdf, use annotations.
      ...
      @param value  data holding value stored in annotation
  */
  void drawAnnotation(const SkRect& rect,
                      const char key[],
  ...
  */
  ...
  /**
   *  Returns the global clip as a region. If the clip contains AA, then only
   * the bounds of the clip may be returned.
   */
...
protected:
  ...
  virtual sk_sp<SkSurface> onNewSurface(const SkImageInfo& info,
  ...
  // Overriders should call the corresponding INHERITED method up the
  // inheritance chain. getSaveLayerStrategy()'s return value may suppress full
  // layer allocation.
  ...
  // NOTE: If you are adding a new onDraw virtual to SkCanvas, PLEASE add an
  // override to SkCanvasVirtualEnforcer (in SkCanvasVirtualEnforcer.h). This
  // ensures that subclasses using that mechanism  will be required to implement
  // the new function.
  ...
  virtual void onDrawArc(const SkRect& rect,
                         SkScalar startAngle,
                         SkScalar sweepAngle,
                         bool useCenter,
                         const SkPaint& paint);
  ...
  virtual void onDrawPosTextH(const void* text,
                              ...
                              SkScalar constY,
                              const SkPaint& paint);
  virtual void onDrawTextRSXform(const void* text,
                                 size_t byteLength,
                                 const SkRSXform xform[],
                                 const SkRect* cullRect,
                                 const SkPaint& paint);
  ...
  virtual void onDrawPatch(const SkPoint cubics[12],
                           ...
                           SkBlendMode mode,
                           const SkPaint& paint);
  virtual void onDrawPoints(PointMode mode,
                            size_t count,
                            const SkPoint pts[],
                            const SkPaint& paint);
  ...
  virtual void onDrawImage(const SkImage* image,
                           SkScalar dx,
                           SkScalar dy,
                           const SkPaint* paint);
  virtual void onDrawImageRect(const SkImage* image,
                               const SkRect* src,
                               const SkRect& dst,
                               const SkPaint* paint,
                               SrcRectConstraint constraint);
  virtual void onDrawImageNine(const SkImage* image,
                               const SkIRect& center,
                               const SkRect& dst,
                               const SkPaint* paint);
  virtual void onDrawImageLattice(const SkImage* image,
                                  const Lattice& lattice,
                                  const SkRect& dst,
                                  const SkPaint* paint);

  virtual void onDrawBitmap(const SkBitmap& bitmap,
                            SkScalar dx,
                            SkScalar dy,
                            const SkPaint* paint);
  virtual void onDrawBitmapRect(const SkBitmap& bitmap,
                                const SkRect* src,
                                const SkRect& dst,
                                const SkPaint* paint,
                                SrcRectConstraint constraint);
  virtual void onDrawBitmapNine(const SkBitmap& bitmap,
                                const SkIRect& center,
                                const SkRect& dst,
                                const SkPaint* paint);
  virtual void onDrawBitmapLattice(const SkBitmap& bitmap,
                                   const Lattice& lattice,
                                   const SkRect& dst,
                                   const SkPaint* paint);

  virtual void onDrawAtlas(const SkImage* atlas,
                           const SkRSXform xform[],
                           const SkRect rect[],
                           const SkColor colors[],
                           int count,
  ...
  virtual void onDrawAnnotation(const SkRect& rect,
                                const char key[],
                                SkData* value);
  ...
  bool clipRectBounds(const SkRect* bounds,
                      SaveLayerFlags flags,
...
private:
  ...
  {
    kNone_ShaderOverrideOpacity,   //!< there is no overriding shader (bitmap or
                                   //!< image)
    kOpaque_ShaderOverrideOpacity, //!< the overriding shader is opaque
    kNotOpaque_ShaderOverrideOpacity, //!< the overriding shader may not be
                                      //!< opaque
  };
  ...
  void predrawNotify(const SkRect* rect,
                     const SkPaint* paint,
...
protected:
  // For use by SkNoDrawCanvas (via SkCanvasVirtualEnforcer, which can't be a
  // friend)
...
private:
  ...
  /**
   * Gets the bounds of the top level layer in global canvas coordinates. We
   * don't want this to be public because it exposes decisions about layer sizes
   * that are internal to the canvas.
   */
  SkIRect getTopLayerBounds() const;

  void internalDrawBitmapRect(const SkBitmap& bitmap,
                              const SkRect* src,
                              const SkRect& dst,
                              const SkPaint* paint,
                              SrcRectConstraint);
  ...
  void internalDrawDevice(SkBaseDevice*,
                          int x,
                          int y,
                          const SkPaint*,
                          SkImage* clipImage,
  ...
  /*
   *  Returns true if drawing the specified rect (or all if it is null) with the
   * specified paint (or default if null) would overwrite the entire root device
   * of the canvas (i.e. the canvas' surface if it had one).
   */
  bool wouldOverwriteEntireSurface(const SkRect*,
                                   const SkPaint*,
                                   ShaderOverrideOpacity) const;

  /**
   *  Returns true if the paint's imagefilter can be invoked directly, without
   * needed a layer.
   */
  bool canDrawBitmapAsSprite(SkScalar x,
                             SkScalar y,
                             int w,
                             int h,
                             const SkPaint&);
...
/** \class SkAutoCanvasRestore
    ...
    goes out of scope. Use this to guarantee that the canvas is restored to a
   known state.
*/
class SkAutoCanvasRestore
{
public:
  ...
  */
  ...
  /** Restores SkCanvas to saved state. Destructor is called when container goes
     out of scope.
  */
  ~SkAutoCanvasRestore()
  {
