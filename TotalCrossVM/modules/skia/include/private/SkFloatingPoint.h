...
static inline float
sk_float_pow(float base, float exp)
{
...
#define SK_MaxS64FitsInFloat                                                   \
  (SK_MaxS64 >> (63 - 24) << (63 - 24)) // 0x7fffff8000000000
...
/**
 *  Return the closest int64_t for the given float. Returns SK_MaxS64FitsInFloat
 * for NaN.
 */
static inline int64_t
sk_float_saturate2int64(float x)
{
...
// being cast to float. Clang thinks this is undefined, but it's actually
// implementation defined to return either the largest float or infinity (one of
// the two bracketing representable floats).  Good enough!
...
static inline float
sk_float_rsqrt_portable(float x)
{
...
// Compare to name-brand "1.0f / sk_float_sqrt(x)".  Should be around 10x faster
// on SSE, 2x on NEON.
static inline float
sk_float_rsqrt(float x)
{
  // We want all this inlined, so we'll inline SIMD and just take the hit when
  // we don't know we've got it at compile time.  This is going to be too fast
  // to productively hide behind a function pointer.
  //
  // We do one step of Newton's method to refine the estimates in the NEON and
  // portable paths.  No refinement is faster, but very innacurate.  Two steps
  // is more accurate, but slower than 1/sqrt.
  //
  // Optimized constants in the portable path courtesy of
  // http://rrrola.wz.cz/inv_sqrt.html
#if SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSE1
  return _mm_cvtss_f32(_mm_rsqrt_ss(_mm_set_ss(x)));
#elif defined(SK_ARM_HAS_NEON)
  // Get initial estimate.
  const float32x2_t xx =
    vdup_n_f32(x); // Clever readers will note we're doing everything 2x.
  float32x2_t estimate = vrsqrte_f32(xx);

  // One step of Newton's method to refine.
  const float32x2_t estimate_sq = vmul_f32(estimate, estimate);
  estimate = vmul_f32(estimate, vrsqrts_f32(xx, estimate_sq));
  return vget_lane_f32(estimate,
                       0); // 1 will work fine too; the answer's in both places.
#else
  return sk_float_rsqrt_portable(x);
#endif
}

// This is the number of significant digits we can print in a string such that
// when we read that string back we get the floating point number we expect. The
// minimum value C requires is 6, but most compilers support 9
...
// IEEE defines how float divide behaves for non-finite values and zero-denoms,
// but C does not so we have a helper that suppresses the possible
