...
// SkOnce provides call-once guarantees for Skia, much like
// std::once_flag/std::call_once().
//
// There should be no particularly error-prone gotcha use cases when using
// SkOnce. It works correctly as a class member, a local, a global, a
// function-scoped static, whatever.
...
public:
  ...
  void operator()(Fn&& fn, Args&&... args)
  {
    ...
    if (state == NotStarted &&
        fState.compare_exchange_strong(state,
                                       Claimed,
                                       std::memory_order_relaxed,
                                       std::memory_order_relaxed)) {
      // Great!  We'll run fn() then notify the other threads by releasing Done
      // into fState.
