...
/**
 * Holds the path verbs and points. It is versioned by a generation ID. None of
 * its public methods modify the contents. To modify or append to the
 * verbs/points wrap the SkPathRef in an SkPathRef::Editor object. Installing
 * the editor resets the generation ID. It also performs copy-on-write if the
 * SkPathRef is shared by multiple SkPaths. The caller passes the Editor's
 * constructor a pointer to a sk_sp<SkPathRef>, which may be updated to point to
 * a new SkPathRef after the editor's constructor returns.
 *
 * The points and verbs are stored in a single allocation. The points are at the
 * begining of the allocation while the verbs are stored at end of the
 * allocation, in reverse order. Thus the points and verbs both grow into the
 * middle of the allocation until the meet. To access verb i in the verb array
 * use ref.verbs()[~i] (because verbs() returns a pointer just beyond the first
...
public:
  ...
  public:
    ...
    /**
     * Adds the verb and allocates space for the number of points indicated by
     * the verb. The return value is a pointer to where the points for the verb
     * should be written. 'weight' is only used if 'verb' is kConic_Verb
     */
    SkPoint* growForVerb(int /*SkPath::Verb*/ verb, SkScalar weight = 0)
    {
      SkDEBUGCODE(fPathRef->validate();) return fPathRef->growForVerb(verb,
                                                                      weight);
    }
     ...
     * The return pointer points at the first new point (indexed normally
     * [<i>]). If 'verb' is kConic_Verb, 'weights' will return a pointer to the
    ...
    /**
     * Resets the path ref to a new verb and point count. The new verbs and
     * points are uninitialized.
     */
...
public:
  ...
  */
  ...
  /**
   * Rollsback a path ref to zero verbs and points with the assumption that the
   * path ref will be repopulated with approximately the same number of verbs
   * and points. A new path ref is created only if necessary.
   */
  ...
  /**
   * Returns a pointer one beyond the first logical verb (last verb in memory
   * order).
   */
  const uint8_t* verbs() const { return fVerbs; }

  /**
   * Returns a const pointer to the first verb in memory (which is the last
   * logical verb).
   */
  ...
  void interpolate(const SkPathRef& ending,
                   SkScalar weight,
                   SkPathRef* out) const;

  /**
   * Gets an ID that uniquely identifies the contents of the path ref. If two
   * path refs have the same ID then they have the same verbs and points.
   * However, two path refs may have the same contents but different genIDs.
   */
  ...
  SkPathRef()
  {
    ...
    SkDEBUGCODE(fEditorsAttached = 0;) SkDEBUGCODE(this->validate();)
  }

  void copy(const SkPathRef& ref,
  ...
  void computeBounds() const
  {
    SkDEBUGCODE(this->validate();)
      ...
      // using an inverted rect instead of fBoundsIsDirty and always
      // recalculating fIsFinite.
   ...
   * uninitialized. Also allocates space for reserveVerb additional verbs and
   * reservePoints additional points.*/
  void resetToSize(int verbCount,
                   ...
                   int reservePoints = 0)
  {
    SkDEBUGCODE(this->validate();) fBoundsIsDirty =
    ...
    size_t newReserve =
      sizeof(uint8_t) * reserveVerbs + sizeof(SkPoint) * reservePoints;
   ...
   * specified verb. If 'verb' is kConic_Verb, 'weights' will return a pointer
   * to the uninitialized conic weights.
   */
  SkPoint* growForRepeatedVerb(int /*SkPath::Verb*/ verb,
                               int numVbs,
                               SkScalar** weights);
  ...
  /**
   * Increases the verb count 1, records the new verb, and creates room for the
   * requisite number of additional points. A pointer to the first point is
   * returned. Any new points are uninitialized.
   */
  SkPoint* growForVerb(int /*SkPath::Verb*/ verb, SkScalar weight);

  /**
   * Ensures that the free space available in the path ref is >= size. The verb
   * and point counts are not changed. May allocate extra capacity, unless
   * |exact| is true.
   */
  void makeSpace(size_t size, bool exact = false)
  {
    ...
    // Note that realloc could memcpy more than we need. It seems to be a win
    // anyway. TODO: encapsulate this.
  ...
  {
    return reinterpret_cast<intptr_t>(fVerbs) -
           reinterpret_cast<intptr_t>(fPoints);
  }

  /**
   * Called the first time someone calls CreateEmpty to actually create the
   * singleton.
   */
  ...
  uint8_t* fVerbs;  // points just past the end of the allocation (verbs grow
                    // backwards)
  int fVerbCnt;
  int fPointCnt;
  ...
  SkDEBUGCODE(int32_t fEditorsAttached;) // assert that only one editor in use
                                         // at any time.
  ...
  bool fIsOval;
  bool fIsRRect;
  // Both the circle and rrect special cases have a notion of direction and
  // starting point The next two variables store that information for either.
