...
public:
  ...
  // Instantiates any "threaded" texture proxies that are being prepared
  // elsewhere
  ...
  // Called when this class will survive a flush and needs to truncate its ops
  // and start over.
  ...
  SkDEBUGCODE(virtual void dump(bool printDependencies) const;)
    ...
    void setStencilLoadOp(GrLoadOp loadOp)
  {
    fStencilLoadOp = loadOp;
  }

protected:
  bool isInstantiated() const;

  // In addition to just the GrSurface being allocated, has the stencil buffer
  // been allocated (if it is required)?
  bool isFullyInstantiated() const;

  // This is a backpointer to the GrOpMemoryPool that holds the memory for this
  // opLists' ops. In the DDL case, these back pointers keep the DDL's
  // GrOpMemoryPool alive as long as its constituent opLists survive.
  ...
  // List of texture proxies whose contents are being prepared on a worker
  // thread
