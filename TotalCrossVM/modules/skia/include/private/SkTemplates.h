...
/** \file SkTemplates.h
    ...
    This file contains light-weight template classes for type-safe and
   exception-safe resource management.
*/

/**
 *  Marks a local variable as known to be unused (to avoid warnings).
 *  Note that this does *not* prevent the local variable from being optimized
 * away.
 */
template<typename T>
inline void
sk_ignore_unused_variable(const T&)
{}
...
template<typename D, typename S>
static D*
SkTAddOffset(S* ptr, size_t byteOffset)
{
  // The intermediate char* has the same cv-ness as D as this produces better
  // error messages. This relies on the fact that reinterpret_cast can add
  // constness, but cannot remove it.
  return reinterpret_cast<D*>(
    reinterpret_cast<sknonstd::same_cv_t<char, D>*>(ptr) + byteOffset);
}
...
/** \class SkAutoTCallVProc
   ...
   destructor. If release() is called, the object reference is set to null. If
   the object reference is null when the destructor is called, we do not call
   the function.
*/
...
private:
  ...
  // Stack frame size is limited for SK_BUILD_FOR_GOOGLE3. 4k is less than the
  // actual max, but some functions have multiple large stack allocations.
...
{
public:
  /** Takes ownership of the ptr. The ptr must be a value which can be passed to
   * sk_free. */
  ...
  {
    fPtr.reset(count ? (T*)sk_realloc_throw(fPtr.release(), count * sizeof(T))
                     : nullptr);
  }

  /** Resize the memory area pointed to by the current ptr without preserving
   * contents. */
...
public:
  ...
  // Reallocs the array, can be used to shrink the allocation.  Makes no attempt
  // to be intelligent
  void realloc(size_t count)
  {
    if (count > kCount) {
...
private:
  // Since we use uint32_t storage, we might be able to get more elements for
  // free.
  static const size_t kCountWithPadding =
    SkAlign4(kCountRequested * sizeof(T)) / sizeof(T);
#if defined(SK_BUILD_FOR_GOOGLE3)
  // Stack frame size is limited for SK_BUILD_FOR_GOOGLE3. 4k is less than the
  // actual max, but some functions have multiple large stack allocations.
...
/**
 *  Pass the object and the storage that was offered during SkInPlaceNewCheck,
 * and this will safely destroy (and free if it was dynamically allocated) the
 * object.
 */
template<typename T>
void
SkInPlaceDeleteCheck(T* obj, void* storage)
{
  if (storage == obj) {
    obj->~T();
  } else {
...
/**
 *  Allocates T, using storage if it is large enough, and allocating on the heap
 * (via new) if storage is not large enough.
 *
...
T*
SkInPlaceNewCheck(void* storage, size_t size, Args&&... args)
{
...
using SkAutoFree =
  std::unique_ptr<void, SkFunctionWrapper<void, void, sk_free>>;

template<typename C, std::size_t... Is>
constexpr auto
SkMakeArrayFromIndexSequence(C c, skstd::index_sequence<Is...>)
...
constexpr auto
SkMakeArray(C c) -> std::array<skstd::result_of_t<C(std::size_t)>, N>
{
