...
// The old libstdc++ uses the draft name "monotonic_clock" rather than
// "steady_clock". This might not actually be monotonic, depending on how
// libstdc++ was built. However, this is only currently used for idle resource
// purging so it shouldn't cause a correctness problem.
...
/**
 * Pixel configurations. This type conflates texture formats, CPU pixel formats,
 * and premultipliedness. We are moving away from it towards SkColorType and
 * backend API (GL, Vulkan) texture formats in the pulbic API. Right now this
 * mostly refers to texture formats as we're migrating.
 */
enum GrPixelConfig
{
...
static constexpr int kNumGrPrimitiveTypes =
  (int)GrPrimitiveType::kLinesAdjacency + 1;

static constexpr bool
GrIsPrimTypeLines(GrPrimitiveType type)
{
...
static constexpr bool
GrIsPrimTypeTris(GrPrimitiveType type)
{
...
static constexpr bool
GrPrimTypeRequiresGeometryShaderSupport(GrPrimitiveType type)
{
...
enum GrMaskFormat
{
  ...
  kA565_GrMaskFormat, //!< 2-bytes per pixel, RGB represent 3-channel LCD
                      //!< coverage
 ...
 */
static inline int
GrMaskFormatBytesPerPixel(GrMaskFormat format)
{
  ...
  static_assert(SK_ARRAY_COUNT(sBytesPerPixel) == kMaskFormatCount,
                "array_size_mismatch");
...
enum GrSurfaceFlags
{
  ...
  /**
   * Clears to zero on creation. It will cause creation failure if initial data
   * is supplied to the texture. This only affects the base level if the texture
   * is created with MIP levels.
   */
...
struct GrSurfaceDesc
{
  ...
  /**
   * The number of samples per pixel. Zero is treated equivalently to 1. This
   * only applies if the kRenderTarget_GrSurfaceFlag is set. The actual number
...
{
  /** Skia will assume the client will keep the resource alive and Skia will not
     free it. */
...
enum class GrAAType : unsigned
{
  ...
  kMSAA,
  /**
   * Use "mixed samples" MSAA such that the stencil buffer is multisampled but
   * the color buffer is not.
   */
  kMixedSamples
};

static inline bool
GrAATypeIsHW(GrAAType type)
{
  switch (type) {
    case GrAAType::kNone:
      return false;
...
/**
 * Not all drawing code paths support using mixed samples when available and
 * instead use coverage-based aa.
 */
enum class GrAllowMixedSamples : bool
{
  kNo = false,
  kYes = true
};
...
GrAAType
GrChooseAAType(GrAA, GrFSAAType, GrAllowMixedSamples, const GrCaps&);

/**
 * Controls anti-aliasing of a quad on a per-edge basis. Currently only used by
 * GrTextureOp. This will be moved to public API and renamed when this
...
/**
 * Types of shader-language-specific boxed variables we can create. (Currently
 * only GrGLShaderVars, but should be applicable to other shader languages.)
 */
enum GrSLType
{
  kVoid_GrSLType,
...
/**
 * The type of texture. Backends other than GL currently only use the 2D value
 * but the type must still be known at the API-neutral layer as it used to
 * determine whether MIP maps, renderability, and sampling parameters are legal
 * for proxies that will be instantiated with wrapped textures.
 */
...
/**
 * Precisions of shader language variables. Not all shading languages support
 * precisions or actually vary the internal precision based on the qualifiers.
 * These currently only apply to float types ( including float vectors and
 * matrices).
 */
enum GrSLPrecision
{
  ...
  // Default precision is a special tag that means "whatever the default for the
  // program/type combination is". In other words, it maps to the empty string
  // in shader code. There are some scenarios where kDefault is not allowed (as
  // the default precision for a program, or for varyings, for example).
...
static inline bool
GrSLTypeIsFloatType(GrSLType type)
{
  switch (type) {
...
/** If the type represents a single value or vector return the vector length,
 * else -1. */
static inline int
GrSLTypeVecLength(GrSLType type)
{
  switch (type) {
    case kFloat_GrSLType:
    case kHalf_GrSLType:
...
static inline GrSLType
GrSLCombinedSamplerTypeForTextureType(GrTextureType type)
{
  switch (type) {
...
/** Rectangle and external textures ony support the clamp wrap mode and do not
 * support MIP maps. */
static inline bool
GrTextureTypeHasRestrictedSampling(GrTextureType type)
{
  switch (type) {
    case GrTextureType::k2D:
      return false;
...
static inline bool
GrSLTypeIsCombinedSamplerType(GrSLType type)
{
  switch (type) {
    case kTexture2DSampler_GrSLType:
    case kTextureExternalSampler_GrSLType:
...
static inline bool
GrSLTypeAcceptsPrecision(GrSLType type)
{
  switch (type) {
    case kTexture2DSampler_GrSLType:
    case kTextureExternalSampler_GrSLType:
...
// temporarily accepting (but ignoring) precision modifiers on the new types;
// this will be killed in a future CL
static inline bool
GrSLTypeTemporarilyAcceptsPrecision(GrSLType type)
{
  switch (type) {
    case kShort_GrSLType:
    case kUShort_GrSLType:
...
enum GrVertexAttribType
{
  ...
  kUByte_norm_GrVertexAttribType,  // unsigned byte, e.g. coverage, 0 -> 0.0f,
                                   // 255 -> 1.0f.
  kUByte4_norm_GrVertexAttribType, // vector of 4 unsigned bytes, e.g. colors, 0
                                   // -> 0.0f,
  // 255 -> 1.0f.
  ...
  kUShort2_GrVertexAttribType, // vector of 2 unsigned shorts. 0 -> 0, 65535 ->
                               // 65535.
  kUShort2_norm_GrVertexAttribType, // vector of 2 unsigned shorts. 0 -> 0.0f,
                                    // 65535 -> 1.0f.
...
static inline bool
GrProcessorEdgeTypeIsFill(const GrClipEdgeType edgeType)
{
  return (GrClipEdgeType::kFillAA == edgeType ||
          GrClipEdgeType::kFillBW == edgeType);
}

static inline bool
GrProcessorEdgeTypeIsInverseFill(const GrClipEdgeType edgeType)
{
...
static inline bool
GrProcessorEdgeTypeIsAA(const GrClipEdgeType edgeType)
{
  return (GrClipEdgeType::kFillBW != edgeType &&
          GrClipEdgeType::kInverseFillBW != edgeType);
}

static inline GrClipEdgeType
GrInvertProcessorEdgeType(const GrClipEdgeType edgeType)
{
  switch (edgeType) {
...
/**
 * Indicates the type of pending IO operations that can be recorded for gpu
 * resources.
 */
enum GrIOType
{
...
static inline bool
GrBufferTypeIsVertexOrIndex(GrBufferType type)
{
...
/**
 * Provides a performance hint regarding the frequency at which a data store
 * will be accessed.
 */
enum GrAccessPattern
{
  ...
  /** Data store will be specified once and used many times. (Thus disqualified
     from caching.) */
  kStatic_GrAccessPattern,
  /** Data store will be specified once and used at most a few times. (Also
     can't be cached.) */
...
enum class GrInternalSurfaceFlags
{
  ...
  //    this is enabled whenever MSAA is enabled and GrCaps reports mixed
  //    samples are supported
  // For wrapped resources:
  //    this is disabled for FBO0
  //    but, otherwise, is enabled whenever MSAA is enabled and GrCaps reports
  //    mixed samples
  //        are supported
  kMixedSampled = 1 << 2,
  ...
  // This flag is for use with GL only. It tells us that the internal render
  // target wraps FBO 0.
  kGLRTFBOIDIs0 = 1 << 4,
...
template<typename T>
T* const*
unique_ptr_address_as_pointer_address(std::unique_ptr<T> const* up)
{
  static_assert(sizeof(T*) == sizeof(std::unique_ptr<T>),
                "unique_ptr not expected size.");
...
/**
 * We want to extend the GrPixelConfig enum to add cases for dealing with
 * alpha_8 which is internally either alpha8 or red8. Also for Gray_8 which can
 * be luminance_8 or red_8.
 */
static constexpr GrPixelConfig kAlpha_8_as_Alpha_GrPixelConfig =
  kPrivateConfig1_GrPixelConfig;
static constexpr GrPixelConfig kAlpha_8_as_Red_GrPixelConfig =
  kPrivateConfig2_GrPixelConfig;
static constexpr GrPixelConfig kAlpha_half_as_Red_GrPixelConfig =
  kPrivateConfig3_GrPixelConfig;
static constexpr GrPixelConfig kGray_8_as_Lum_GrPixelConfig =
  kPrivateConfig4_GrPixelConfig;
static constexpr GrPixelConfig kGray_8_as_Red_GrPixelConfig =
  kPrivateConfig5_GrPixelConfig;

/**
 * Refers to the encoding of a GPU buffer as it will be interpreted by the GPU
 * when sampling and blending.
 */
enum class GrSRGBEncoded : bool
{
  kNo = false,
  kYes = true
};
...
/**
 * Describes whether pixel data encoding should be converted to/from linear/sRGB
 * encoding.
 */
...
static inline GrSRGBEncoded
GrPixelConfigIsSRGBEncoded(GrPixelConfig config)
{
  switch (config) {
...
static inline bool
GrPixelConfigIsSRGB(GrPixelConfig config)
{
  return GrSRGBEncoded::kYes == GrPixelConfigIsSRGBEncoded(config);
}

static inline size_t
GrBytesPerPixel(GrPixelConfig config)
{
  switch (config) {
    case kAlpha_8_GrPixelConfig:
    case kAlpha_8_as_Alpha_GrPixelConfig:
...
static inline bool
GrPixelConfigIsOpaque(GrPixelConfig config)
{
  switch (config) {
    case kRGB_565_GrPixelConfig:
    case kRGB_888_GrPixelConfig:
...
static inline bool
GrPixelConfigIsAlphaOnly(GrPixelConfig config)
{
  switch (config) {
    case kAlpha_8_GrPixelConfig:
    case kAlpha_8_as_Alpha_GrPixelConfig:
 ...
 */
static inline GrSLPrecision
GrSLSamplerPrecision(GrPixelConfig config)
{
  switch (config) {
    case kUnknown_GrPixelConfig:
    case kAlpha_8_GrPixelConfig:
...
/**
 * Like SkColorType this describes a layout of pixel data in CPU memory. It
 * specifies the channels, their type, and width. This exists so that the GPU
 * backend can have private types that have no analog in the public facing
 * SkColorType enum and omit types not implemented in the GPU backend. It does
 * not refer to a texture format and the mapping to texture formats may be
 * many-to-many. It does not specify the sRGB encding of the stored values.
 */
enum class GrColorType
{
  ...
  kAlpha_8,
  kRGB_565,
  kABGR_4444, // This name differs from SkColorType. kARGB_4444_SkColorType is
              // misnamed.
  kRGBA_8888,
  kRGB_888x,
  kBGRA_8888,
...
GrColorTypeToSkColorType(GrColorType ct)
{
  switch (ct) {
...
static inline GrColorType
SkColorTypeToGrColorType(SkColorType ct)
{
  switch (ct) {
...
GrColorTypeComponentFlags(GrColorType ct)
{
  switch (ct) {
    case GrColorType::kUnknown:
      return 0;
...
static inline bool
GrColorTypeIsAlphaOnly(GrColorType ct)
{
  return kAlpha_SkColorTypeComponentFlag == GrColorTypeComponentFlags(ct);
}

static inline bool
GrColorTypeHasAlpha(GrColorType ct)
{
  return kAlpha_SkColorTypeComponentFlag & GrColorTypeComponentFlags(ct);
}

static inline int
GrColorTypeBytesPerPixel(GrColorType ct)
{
  switch (ct) {
    case GrColorType::kUnknown:
      return 0;
...
static inline GrColorType
GrPixelConfigToColorTypeAndEncoding(GrPixelConfig config,
...
static inline GrColorType
GrPixelConfigToColorType(GrPixelConfig config)
{
...
GrColorTypeToPixelConfig(GrColorType config, GrSRGBEncoded srgbEncoded)
{
  switch (config) {
    case GrColorType::kUnknown:
...
public:
  ...
  void weak_dispose() const override { fReleaseProc(fReleaseCtx); }
