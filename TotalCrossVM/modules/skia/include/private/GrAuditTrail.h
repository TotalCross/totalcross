...
/*
 * GrAuditTrail collects a list of draw ops, detailed information about those
 * ops, and can dump them to json.
 *
 * Capturing this information is expensive and consumes a lot of memory,
 * therefore it is important to enable auditing only when required and disable
 * it promptly. The AutoEnable class helps to ensure that the audit trail is
 * disabled in a timely fashion. Once the information has been dealt with, be
 * sure to call reset(), or the log will simply keep growing.
 */
class GrAuditTrail
{
public:
  ...
  // Because op combining is heavily dependent on sequence of draw calls, these
  // calls will only produce valid information for the given draw sequence which
  // preceeded them. Specifically, ops of future draw calls may combine with
  // previous ops and thus would invalidate the json. What this means is that
  // for some sequence of draw calls N, the below toJson calls will only produce
  // JSON which reflects N draw calls. This JSON may or may not be accurate for
  // N + 1 or N - 1 draws depending on the actual combining algorithm used.
  ...
  // We could just return our internal bookkeeping struct if copying the data
  // out becomes a performance issue, but until then its nice to decouple
  struct OpInfo
  {
    struct Op
    {
...
private:
  ...
  template<typename T>
  static void JsonifyTArray(SkString* json,
                            const char* name,
                            const T& array,
                            bool addComma);
  ...
  // The client can pass in an optional client ID which we will use to mark the
  // ops
  int fClientID;
  bool fEnabled;
};
...
#define GR_AUDIT_TRAIL_OP_RESULT_NEW(                                          \
  audit_trail, op) // Doesn't do anything now, one day...
