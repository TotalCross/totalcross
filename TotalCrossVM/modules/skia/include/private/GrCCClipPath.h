...
/**
 * These are keyed by SkPath generation ID, and store which device-space paths
 * are accessed and where by clip FPs in a given opList. A single GrCCClipPath
 * can be referenced by multiple FPs. At flush time their coverage count masks
 * are packed into atlas(es) alongside normal DrawPathOps.
 */
class GrCCClipPath
{
public:
  ...
  ~GrCCClipPath()
  {
    // Ensure no clip FP exists with a dangling pointer back into this class.
    // This works because a clip FP will have a ref on the proxy if it exists.
    //
    // This assert also guarantees there won't be a lazy proxy callback with a
    // dangling pointer back into this class, since no proxy will exist after we
    // destruct, if the assert passes.
...
private:
  ...
  SkIVector
    fDevToAtlasOffset; // Translation from device space to location in atlas.
