...
public:
  ...
  void remove(int index, int count = 1)
  {
    ...
    fCount = fCount - count;
    memmove(
      fArray + index, fArray + index + count, sizeof(T) * (fCount - index));
  }
...
private:
  ...
  void adjustCount(int delta)
  {
    ...
    // The sum of fCount and delta is at most 4294967294, which fits fine in
    // uint32_t.
  ...
  void resizeStorageToAtLeast(int count)
  {
    ...
    // We take care to avoid overflow here.
    // The maximum value we can get for reserve here is 2684354563, which fits
    // in uint32_t.
...
template<typename T>
static inline void
swap(SkTDArray<T>& a, SkTDArray<T>& b)
{
  a.swap(b);
}
