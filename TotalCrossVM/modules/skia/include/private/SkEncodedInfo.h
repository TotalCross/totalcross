...
public:
  ...
  static SkEncodedInfo Make(int width,
                            int height,
                            Color color,
                            Alpha alpha,
  ...
  {
    SkASSERT(1 == bitsPerComponent || 2 == bitsPerComponent ||
    ...
    return SkEncodedInfo(
      width, height, color, alpha, bitsPerComponent, std::move(profile));
  }
  ...
  SkImageInfo makeImageInfo() const
  {
    ...
    auto alpha =
      kOpaque_Alpha == fAlpha ? kOpaque_SkAlphaType : kUnpremul_SkAlphaType;
    sk_sp<SkColorSpace> cs =
      fProfile ? SkColorSpace::Make(*fProfile->profile()) : nullptr;
    if (!cs) {
  ...
  {
    auto copy =
      SkEncodedInfo::Make(fWidth, fHeight, fColor, fAlpha, fBitsPerComponent);
    if (fProfile) {
