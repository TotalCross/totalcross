...
// NOTE: SkFixedToFloat is exact. SkFloatToFixed seems to lack a rounding step.
...
// integer then snaps to nearest even). Using double for the rounding math gives
// maximum accuracy for (float -> fixed -> float), but that's usually overkill.
#define SkFixedToFloat(x) ((x)*1.52587890625e-5f)
#define SkFloatToFixed(x) sk_float_saturate2int((x)*SK_Fixed1)

#ifdef SK_DEBUG
static inline SkFixed
SkFloatToFixed_Check(float x)
{
...
/** Converts an integer to a SkFixed, asserting that the result does not
...
SkIntToFixed(int n)
{
  ...
  // Left shifting a negative value has undefined behavior in C, so we cast to
  // unsigned before shifting.
  return (unsigned)n << 16;
}
#else
// Left shifting a negative value has undefined behavior in C, so we cast to
// unsigned before shifting. Then we force the cast to SkFixed to ensure that
// the answer is signed (like the debug version).
...
static inline SkFixed
SkFixedRoundToFixed(SkFixed x)
{
  return (x + SK_FixedHalf) & 0xFFFF0000;
}
static inline SkFixed
SkFixedCeilToFixed(SkFixed x)
{
  return (x + SK_Fixed1 - 1) & 0xFFFF0000;
}
static inline SkFixed
SkFixedFloorToFixed(SkFixed x)
{
  return x & 0xFFFF0000;
}
...
#define SkFixedDiv(numer, denom)                                               \
  SkToS32(SkTPin<int64_t>(                                                     \
    (SkLeftShift((int64_t)(numer), 16) / (denom)), SK_MinS32, SK_MaxS32))

static inline SkFixed
SkFixedMul(SkFixed a, SkFixed b)
{
...
*/
SK_ALWAYS_INLINE SkFixed
SkFloatToFixed_arm(float x)
{
  int32_t y;
...
#define SkFloatToFixed3232(x)                                                  \
  sk_float_saturate2int64((x) * (65536.0f * 65536.0f))
