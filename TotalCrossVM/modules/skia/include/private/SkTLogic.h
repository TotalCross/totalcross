...
// Chromium currently requires gcc 4.8.2 or a recent clang compiler, but uses
// libstdc++4.6.4. Note that Precise actually uses libstdc++4.6.3.
// Unfortunately, libstdc++ STL before libstdc++4.7 do not define
// std::underlying_type. Newer gcc and clang compilers have __underlying_type
// which does not depend on runtime support. See
// https://gcc.gnu.org/onlinedocs/libstdc++/manual/abi.html for __GLIBCXX__
// values. Unfortunately __GLIBCXX__ is a date, but no updates to versions
// before 4.7 are now anticipated.
...
#define SK_GLIBCXX_4_6_4 20121127
  ...
  (__GLIBCXX__ < SK_GLIBCXX_4_7_0 || __GLIBCXX__ == SK_GLIBCXX_4_5_4 ||        \
...
struct make_index_sequence_combine<skstd::index_sequence<I1...>,
                                   skstd::index_sequence<I2...>>
...
// The sknonstd namespace contains things we would like to be proposed and feel
// std-ish.
namespace sknonstd {

// The name 'copy' here is fraught with peril. In this case it means 'append',
// not 'overwrite'. Alternate proposed names are 'propagate', 'augment', or
// 'append' (and 'add', but already taken). std::experimental::propagate_const
// already exists for other purposes in TSv2. These also follow the <dest,
// source> pattern used by boost.
template<typename D, typename S>
struct copy_const
{
  using type =
    skstd::conditional_t<std::is_const<S>::value, skstd::add_const_t<D>, D>;
};
template<typename D, typename S>
...
{
  using type = skstd::
    conditional_t<std::is_volatile<S>::value, skstd::add_volatile_t<D>, D>;
};
template<typename D, typename S>
