 ...
 *
 * TODO: we probably need to expose this class so users can query it for memory
 * usage.
 */
...
public:
#if SK_SUPPORT_GPU
  // This object is the source from which the lazy proxy backing the DDL will
  // pull its backing texture when the DDL is replayed. It has to be separately
  // ref counted bc the lazy proxy can outlive the DDL.
  class LazyProxyData : public SkRefCnt
  {
  public:
    // Upon being replayed - this field will be filled in (by the
    // DrawingManager) with the proxy backing the destination SkSurface. Note
    // that, since there is no good place to clear it it can become a dangling
    // pointer.
...
private:
  friend class GrDrawingManager;              // for access to 'fOpLists' and
                                              // 'fLazyProxyData'
