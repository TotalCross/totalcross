...
SkColorTypeComponentFlags(SkColorType ct)
{
  switch (ct) {
...
SkColorTypeMinRowBytes(SkColorType ct, int width)
{
  return width * SkColorTypeBytesPerPixel(ct);
}

static inline bool
SkColorTypeIsValid(unsigned value)
{
  return value <= kLastEnum_SkColorType;
}

static inline size_t
SkColorTypeComputeOffset(SkColorType ct, int x, int y, size_t rowBytes)
{
...
/**
 *  Returns true if |info| contains a valid combination of width, height,
 * colorType, and alphaType.
 */
static inline bool
SkImageInfoIsValid(const SkImageInfo& info)
{
  ...
  if (kUnknown_SkColorType == info.colorType() ||
      kUnknown_SkAlphaType == info.alphaType()) {
    return false;
  }

  if (kOpaque_SkAlphaType != info.alphaType() &&
      (kRGB_565_SkColorType == info.colorType() ||
       kGray_8_SkColorType == info.colorType())) {
    return false;
  }
...
/**
 *  Returns true if Skia has defined a pixel conversion from the |src| to the
 * |dst|. Returns false otherwise.
 */
static inline bool
SkImageInfoValidConversion(const SkImageInfo& dst, const SkImageInfo& src)
{
