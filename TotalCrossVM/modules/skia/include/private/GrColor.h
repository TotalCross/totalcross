...
/**
 * GrColor is 4 bytes for R, G, B, A, in a specific order defined below. Whether
 * the color is premultiplied or not depends on the context in which it is being
 * used.
 */
 ...
 */
static inline GrColor
GrColorPackRGBA(unsigned r, unsigned g, unsigned b, unsigned a)
{
  SkASSERT((uint8_t)r == r);
  SkASSERT((uint8_t)g == g);
  SkASSERT((uint8_t)b == b);
  SkASSERT((uint8_t)a == a);
  return (r << GrColor_SHIFT_R) | (g << GrColor_SHIFT_G) |
         (b << GrColor_SHIFT_B) | (a << GrColor_SHIFT_A);
}
 ...
 */
static inline GrColor
GrColorPackA4(unsigned a)
{
  SkASSERT((uint8_t)a == a);
  return (a << GrColor_SHIFT_R) | (a << GrColor_SHIFT_G) |
         (a << GrColor_SHIFT_B) | (a << GrColor_SHIFT_A);
}
 ...
 */
static inline void
GrColorIsPMAssert(GrColor SkDEBUGCODE(c))
{
#ifdef SK_DEBUG
...
static inline GrColor
GrColorMul(GrColor c0, GrColor c1)
{
...
static inline void
GrColorToRGBAFloat(GrColor color, float rgba[4])
{
...
static inline SkRGBA4f<kPremul_SkAlphaType>
GrColorToPMColor4f(GrColor color)
{
...
static inline float
GrNormalizeByteToFloat(uint8_t value)
{
  static const float ONE_OVER_255 = 1.f / 255.f;
...
static inline bool
GrColorIsOpaque(GrColor color)
{
  return (color & (0xFFU << GrColor_SHIFT_A)) == (0xFFU << GrColor_SHIFT_A);
}

static inline GrColor
GrPremulColor(GrColor color)
{
  ...
  return GrColorPackRGBA(
    SkMulDiv255Round(r, a), SkMulDiv255Round(g, a), SkMulDiv255Round(b, a), a);
}

/** Returns an unpremuled version of the GrColor. */
static inline GrColor
GrUnpremulColor(GrColor color)
{
  GrColorIsPMAssert(color);
  unsigned r = GrColorUnpackR(color);
  unsigned g = GrColorUnpackG(color);
...
/**
 * Similarly, GrColor4f is 4 floats for R, G, B, A, in that order. And like
 * GrColor, whether the color is premultiplied or not depends on the context.
 */
struct GrColor4f
{
  float fRGBA[4];
  ...
  static GrColor4f OpaqueWhite() { return GrColor4f(1.0f, 1.0f, 1.0f, 1.0f); }
  ...
  {
    return fRGBA[0] == other.fRGBA[0] && fRGBA[1] == other.fRGBA[1] &&
           fRGBA[2] == other.fRGBA[2] && fRGBA[3] == other.fRGBA[3];
  }
  bool operator!=(const GrColor4f& other) const { return !(*this == other); }
  ...
  {
    return fRGBA[3] >=
           1.f; // just in case precision causes a superopaque value.
  }
  ...
  GrColor4f unpremul() const
  {
    ...
    return GrColor4f(
      fRGBA[0] * invAlpha, fRGBA[1] * invAlpha, fRGBA[2] * invAlpha, a);
  }
};
