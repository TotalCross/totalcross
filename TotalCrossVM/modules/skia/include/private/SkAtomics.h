...
template<typename T>
void
sk_atomic_store(T*, T, sk_memory_order = sk_memory_order_seq_cst);

template<typename T>
T
sk_atomic_fetch_add(T*, T, sk_memory_order = sk_memory_order_seq_cst);

template<typename T>
bool
sk_atomic_compare_exchange(T*,
                           T* expected,
                           T desired,
...
// ensure they're always used atomically.  This is our stand-in for
// std::atomic<T>.
// !!! Please _really_ know what you're doing if you change
// default_memory_order. !!!
template<typename T,
         sk_memory_order default_memory_order = sk_memory_order_seq_cst>
...
public:
  ...
  // It is essential we return by value rather than by const&.  fVal may change
  // at any time.
...
template<typename T>
T
sk_atomic_load(const T* ptr, sk_memory_order mo)
{
  SkASSERT(mo == sk_memory_order_relaxed || mo == sk_memory_order_seq_cst ||
           mo == sk_memory_order_acquire || mo == sk_memory_order_consume);
...
template<typename T>
void
sk_atomic_store(T* ptr, T val, sk_memory_order mo)
{
  SkASSERT(mo == sk_memory_order_relaxed || mo == sk_memory_order_seq_cst ||
...
template<typename T>
T
sk_atomic_fetch_add(T* ptr, T val, sk_memory_order mo)
{
...
template<typename T>
bool
sk_atomic_compare_exchange(T* ptr,
                           T* expected,
                           T desired,
                           ...
                           sk_memory_order failure)
{
  ...
  SkASSERT(
    failure == sk_memory_order_relaxed || failure == sk_memory_order_seq_cst ||
    failure == sk_memory_order_acquire || failure == sk_memory_order_consume);
...
inline int32_t
sk_atomic_inc(int32_t* ptr)
{
  return sk_atomic_fetch_add(ptr, +1);
}
inline int32_t
sk_atomic_dec(int32_t* ptr)
{
  return sk_atomic_fetch_add(ptr, -1);
}
