...
// SkArenaAlloc allocates object and destroys the allocated objects when
// destroyed. It's designed to minimize the number of underlying block
// allocations. SkArenaAlloc allocates first out of an (optional) user-provided
// block of memory, and when that's exhausted it allocates on the heap, starting
// with an allocation of extraSize bytes.  If your data (plus a small overhead)
// fits in the user-provided block, SkArenaAlloc never uses the heap, and if it
// fits in extraSize bytes, it'll use the heap only once.  If you pass extraSize
// = 0, it allocates blocks for each call to make<T>.
//
// Examples:
//
...
//
// If mostCasesSize is too large for the stack, you can use the following
// pattern.
//
...
//
// The storage does not necessarily need to be on the stack. Embedding the
// storage in a class also works.
//
//   class Foo {
...
//   };
//
// In addition, the system is optimized to handle POD data including arrays of
// PODs (where POD is really data with no destructors). For POD data it has zero
// overhead per item, and a typical block overhead of 8 bytes. For non-POD
// objects there is a per item overhead of 4 bytes. For arrays of non-POD
// objects there is a per array overhead of typically 8 bytes. There is an
// addition overhead when switching from POD data to non-POD data of typically 8
// bytes.
//
// If additional blocks are needed they are increased exponentially. This
// strategy bounds the recursion of the RunDtorsOnBlock to be limited to O(log
// size-of-memory). Block size grow using the Fibonacci sequence which means
// that for 2^32 memory there are 48 allocations, and for 2^48 there are 71
// allocations.
class SkArenaAlloc
{
public:
  ...
  T* makeArray(size_t count)
  {
    ...
    // If T is primitive then the memory is initialized. For example, an array
    // of chars will be zeroed.
    for (size_t i = 0; i < safeCount; i++) {
      new (&array[i]) T();
    }
    return array;
  ...
  // Only use makeBytesAlignedTo if none of the typed variants are impractical
  // to use.
...
private:
  ...
  void installUint32Footer(FooterAction* action,
                           uint32_t value,
                           uint32_t padding);
  ...
  char* allocObject(uint32_t size, uint32_t alignment)
  {
    ...
    uintptr_t alignedOffset =
      (~reinterpret_cast<uintptr_t>(fCursor) + 1) & mask;
  ...
  char* commonArrayAlloc(uint32_t count)
  {
    ...
    if (std::is_trivially_destructible<T>::value) {
      ...
      AssertRelease(arraySize <=
                    std::numeric_limits<uint32_t>::max() - overhead);
  ...
  // Use the Fibonacci sequence as the growth factor for block size. The size of
  // the block allocated is fFib0 * fExtraSize. Using 2 ^ n * fExtraSize had too
  // much slop for Android.
