...
/**
 * Message must implement bool Message::shouldSend(uint32_t inboxID) const.
 * Perhaps someday we can use std::experimental::is_detected to avoid this
 * requirement by sending to all inboxes when the method is not detected on
 * Message.
 */
...
public:
  ...
  public:
    ...
    // Overwrite out with all the messages we've received since the last call.
    // Threadsafe.
  ...
  private:
    ...
    void receive(
      const Message& m); // SkMessageBus is a friend only to call this.
  };
...
// This must go in a single .cpp file, not some .h, or we risk creating more
// than one global SkMessageBus per type when using shared libraries.  NOTE: at
// most one per file will compile.
...
SkMessageBus<Message>::Inbox::~Inbox()
{
  ...
  SkMessageBus<Message>* bus = SkMessageBus<Message>::Get();
  SkAutoMutexAcquire lock(bus->fInboxesMutex);
  // This is a cheaper fInboxes.remove(fInboxes.find(this)) when order doesn't
  // matter.
...
template<typename Message>
void
SkMessageBus<Message>::Inbox::receive(const Message& m)
{
...
template<typename Message>
void
SkMessageBus<Message>::Inbox::poll(SkTArray<Message>* messages)
{
  SkASSERT(messages);
  messages->reset();
  SkAutoMutexAcquire lock(fMessagesMutex);
...
template<typename Message>
/*static*/ void
SkMessageBus<Message>::Post(const Message& m)
{
  SkMessageBus<Message>* bus = SkMessageBus<Message>::Get();
  SkAutoMutexAcquire lock(bus->fInboxesMutex);
  for (int i = 0; i < bus->fInboxes.count(); i++) {
