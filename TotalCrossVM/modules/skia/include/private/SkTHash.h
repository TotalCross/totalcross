...
// Before trying to use SkTHashTable, look below to see if SkTHashMap or
// SkTHashSet works for you. They're easier to use, usually perform the same,
...
public:
  ...
  // !!!!!!!!!!!!!!!!!                 CAUTION !!!!!!!!!!!!!!!!! set(), find()
  // and foreach() all allow mutable access to table entries. If you change an
  // entry so that it no longer has the same key, all hell will break loose.  Do
  // not do that!
  //
  // Please prefer to use SkTHashMap or SkTHashSet, which do not have this
  // danger.

  // The pointers returned by set() and find() are valid only until the next
  // call to set(). The pointers you receive in foreach() are only valid for its
  // duration.

  // Copy val into the hash table, returning a pointer to the copy now in the
  // table. If there already is an entry in the table with the same key, we
  // overwrite it.
  T* set(T val)
  {
  ...
  // If there is an entry in the table with this key, return a pointer to it. If
  // not, null.
  ...
  void remove(const K& key)
  {
    ...
    for (;;) {
      ...
      // If the empty slot is in between where an element landed, and its native
      // slot, then move it to the empty slot. Don't move it if its native slot
      // is in between where the element landed and the empty slot. [native] <=
      // [empty] < [candidate] == GOOD, can move candidate to empty slot [empty]
      // < [native] < [candidate] == BAD, need to leave candidate where it is
      do {
        index = this->next(index);
        Slot& s = fSlots[index];
        if (s.empty()) {
  ...
  // Call fn on every entry in the table.  You may mutate the entries, but be
  // very careful.
  ...
  T* uncheckedSet(T&& val)
  {
    ...
    for (int n = 0; n < fCapacity; n++) {
      ...
      if (hash == s.hash && key == Traits::GetKey(s.val)) {
        ...
        // Note: this triggers extra copies when adding the same value
        // repeatedly.
        s.val = std::move(val);
        return &s.val;
      }
...
// Maps K->V.  A more user-friendly wrapper around SkTHashTable, suitable for
// most use cases. K and V are treated as ordinary copyable C++ types, with no
...
public:
  ...
  // N.B. The pointers returned by set() and find() are valid only until the
  // next call to set().

  // Set key to val in the table, replacing any previous value with the same
  // key. We copy both key and val, and return a pointer to the value copy now
  // in the table.
  V* set(K key, V val)
  {
  ...
  // If there is key/value entry in the table with this key, return a pointer to
  // the value. If not, return null.
  ...
  // Call fn on every key/value pair in the table.  You may mutate the value but
  // not the key.
  ...
  template<typename Fn> // f(K, V), f(const K&, V), f(K, const V&) or f(const
                        // K&, const V&).
  void foreach (Fn&& fn) const
  {
...
public:
  ...
  // If an item equal to this is in the set, return a pointer to it, otherwise
  // null. This pointer remains valid until the next call to add().
