...
public:
  ...
  void release()
  {
    // The proxy itself may still have multiple refs. It can be owned by an
    // SkImage and multiple SkDeferredDisplayLists at the same time if we are
    // using DDLs.
    ...
    // In the current hybrid world, the proxy and backing surface are
    // ref/unreffed in synchrony. In this instance we're deInstantiating the
    // proxy so, regardless of the number of refs on the backing surface, we're
    // going to remove it. If/when the proxy is re-instantiated all the refs on
    // the proxy (presumably due to multiple uses in ops) will be transfered to
    // the new surface.
...
protected:
  GrIORefProxy()
    ...
    , fPendingWrites(0)
  {}
  GrIORefProxy(sk_sp<GrSurface> surface)
    : fRefCnt(1)
    , fPendingReads(0)
    , fPendingWrites(0)
  {
    // Since we're manually forwarding on refs & unrefs we don't want sk_sp
  ...
  // make all the reffing & unreffing work out we now need to transfer any
  // deferred refs & unrefs to the new GrSurface
  void transferRefs()
  {
    SkASSERT(fTarget);
  ...
  // For deferred proxies this will be null. For wrapped proxies it will point
...
public:
  ...
  enum class LazyState
  {
    ...
    kPartially, // The proxy has a lazy callback but knows basic information
                // about itself.
    kFully, // The proxy has a lazy callback and also doesn't know its width,
            // height, etc.
  };
  ...
  /**
   * Helper that gets the width and height of the surface as a bounding
   * rectangle.
   */
  ...
  /**
   * Helper that gets the worst case width and height of the surface as a
   * bounding rectangle.
   */
  ...
  GrSurfaceOrigin origin() const
  {
    SkASSERT(kTopLeft_GrSurfaceOrigin == fOrigin ||
             kBottomLeft_GrSurfaceOrigin == fOrigin);
    return fOrigin;
  }
   ...
   *      the uniqueID will be different from the real resource, when it is
   * allocated the proxy's uniqueID will not change across the instantiate call
   *
   *    the uniqueIDs of the proxies and the resources draw from the same pool
   *
   * What this boils down to is that the uniqueID of a proxy can be used to
   * consistently track/identify a proxy but should never be used to distinguish
   * between resources and proxies - beware!
   */
  ...
  /**
   * @return the render target proxy associated with the surface proxy, may be
   * NULL.
   */
  ...
  // If this is a texture proxy and the proxy is already instantiated, return
  // its backing GrTexture; if not, return null.
  GrTexture* peekTexture() const
  {
    return fTarget ? fTarget->asTexture() : nullptr;
  }

  // If this is a render target proxy and the proxy is already instantiated,
  // return its backing GrRenderTarget; if not, return null.
  ...
  /**
   * Retrieves the amount of GPU memory that will be or currently is used by
   * this resource in bytes. It is approximate since we aren't aware of
   * additional padding or copies made by the driver.
   *
  ...
  // It always returns a kExact-backed proxy bc it is used when converting an
  // SkSpecialImage to an SkImage. The copy is is not a render target and not
  // multisampled.
  static sk_sp<GrTextureProxy> Copy(GrContext*,
                                    GrSurfaceProxy* src,
                                    GrMipMapped,
                                    SkIRect srcRect,
                                    SkBudgeted);
  ...
  // Copy the entire 'src'
  // It always returns a kExact-backed proxy bc it is used in
  // SkGpuDevice::snapSpecial
  static sk_sp<GrTextureProxy> Copy(GrContext* context,
                                    GrSurfaceProxy* src,
                                    GrMipMapped,
                                    SkBudgeted budgeted);

  // Test-only entry point - should decrease in use as proxies propagate
  static sk_sp<GrSurfaceContext> TestCopy(GrContext* context,
...
protected:
  // Deferred version
  GrSurfaceProxy(const GrSurfaceDesc& desc,
                 GrSurfaceOrigin origin,
                 SkBackingFit fit,
                 SkBudgeted budgeted,
                 GrInternalSurfaceFlags surfaceFlags)
    : GrSurfaceProxy(nullptr,
                     ...
                     surfaceFlags)
  {
    // Note: this ctor pulls a new uniqueID from the same pool at the
    // GrGpuResources
  }

  using LazyInstantiateCallback =
    std::function<sk_sp<GrSurface>(GrResourceProvider*)>;
  ...
  bool hasPendingIO() const { return this->internalHasPendingIO(); }

  bool hasPendingWrite() const { return this->internalHasPendingWrite(); }
  ...
  // In many cases these flags aren't actually known until the proxy has been
  // instantiated. However, Ganesh frequently needs to change its behavior based
  // on these settings. For internally create proxies we will know these
  // properties ahead of time. For wrapped proxies we will copy the properties
  // off of the GrSurface. For lazy proxies we force the call sites to provide
  // the required information ahead of time. At instantiation time we verify
  // that the assumed properties match the actual properties.
  GrInternalSurfaceFlags fSurfaceFlags;

private:
  // For wrapped resources, 'fConfig', 'fWidth', 'fHeight', and 'fOrigin; will
  // always be filled in from the wrapped resource.
  ...
  const UniqueID
    fUniqueID; // set from the backing resource for wrapped resources

  LazyInstantiateCallback fLazyInstantiateCallback;
  // If this is set to kSingleuse, then after one call to
  // fLazyInstantiateCallback we will cleanup the lazy callback and then delete
  // it. This will allow for any refs and resources being held by the standard
  // function to be released. This is specifically useful in non-dll cases where
  ...
  SkDEBUGCODE(size_t getRawGpuMemorySize_debugOnly()
                const { return fGpuMemorySize; })

    virtual size_t onUninstantiatedGpuMemorySize() const = 0;

  bool fNeedsClear;

  // This entry is lazily evaluated so, when the proxy wraps a resource, the
  // resource will be called but, when the proxy is deferred, it will compute
  // the answer itself. If the proxy computes its own answer that answer is
  // checked (in debug mode) in the instantiation method.
  mutable size_t fGpuMemorySize;

  // The last opList that wrote to or is currently going to write to this
  // surface The opList can be closed (e.g., no surface context is currently
  // bound to this proxy). This back-pointer is required so that we can add a
  // dependancy between the opList used to create the current contents of this
  // surface and the opList of a destination surface to which this one is being
  // drawn or copied. This pointer is unreffed. OpLists own a ref on their
  // surface proxies.
