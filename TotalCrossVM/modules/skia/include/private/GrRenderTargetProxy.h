...
public:
  ...
  GrFSAAType fsaaType() const
  {
    ...
    return this->hasMixedSamples() ? GrFSAAType::kMixedSamples
  ...
  /**
   * Returns the number of samples/pixel in the stencil buffer (One if
   * non-MSAA).
   */
  int numStencilSamples() const { return fSampleCnt; }

  /**
   * Returns the number of samples/pixel in the color buffer (One if non-MSAA or
   * mixed sampled).
   */
...
private:
  ...
  SkDEBUGCODE(void onValidateSurface(const GrSurface*) override;)
    ...
    // WARNING: Be careful when adding or removing fields here. ASAN is likely
    // to trigger warnings when instantiating GrTextureRenderTargetProxy. The
    // std::function in GrSurfaceProxy makes each class in the diamond require
    // 16 byte alignment. Clang appears to layout the fields for each class to
    ...
    // alignment, even if the fields in that particular class don't require
    // it. Changing the size of this object can move the start address of
    // other types, leading to this problem.

    int fSampleCnt;
  bool fNeedsStencil;

  // For wrapped render targets the actual GrRenderTarget is stored in the
  // GrIORefProxy class. For deferred proxies that pointer is filled in when we
  // need to instantiate the deferred resource.
