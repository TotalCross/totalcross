...
public:
  ...
  /**
   * Resets to count() = n newly constructed T objects and resets any reserve
   * count.
   */
  void reset(int n)
  {
    ...
    // Set fCount to 0 before calling checkRealloc so that no elements are
    // moved.
    fCount = 0;
    this->checkRealloc(n);
    fCount = n;
  ...
  /**
   * Ensures there is enough reserved space for n additional elements. The is
   * guaranteed at least until the array size grows above n and subsequently
   * shrinks below n, any version of reset() is called, or reserve() is called
   * again.
   */
  void reserve(int n)
  {
  ...
  /** Swaps the contents of this array with that array. Does a pointer swap if
     possible, otherwise copies the T values. */
...
private:
  ...
  /** In the following move and copy methods, 'dst' is assumed to be
   * uninitialized raw storage. In the following move methods, 'src' is
   * destroyed leaving behind uninitialized raw storage.
   */
  void copy(const T* src)
  {
    // Some types may be trivially copyable, in which case we *could* use
    // memcopy; but MEM_MOVE == true implies that the type is trivially movable,
    // and not necessarily trivially copyable (think sk_sp<>).  So short of
    // adding another template arg, we must be conservative and use copy
    // construction.
    for (int i = 0; i < fCount; ++i) {
  ...
  // Helper function that makes space for n objects, adjusts the count, but does
  // not initialize the new objects.
  ...
  void checkRealloc(int delta)
  {
    ...
    // We allow fAllocCount to be in the range [newCount, 3*newCount]. We also
    // never shrink when we're currently using preallocated memory, would
    // allocate less than kMinHeapAllocCount, or a reserve count was specified
    ...
    // Whether we're growing or shrinking, we leave at least 50% extra space for
    // future growth.
    int64_t newAllocCount = newCount + ((newCount + 1) >> 1);
    // Align the new allocation count to kMinHeapAllocCount.
    static_assert(SkIsPow2(kMinHeapAllocCount),
                  "min alloc count not power of two.");
    newAllocCount =
      (newAllocCount + (kMinHeapAllocCount - 1)) & ~(kMinHeapAllocCount - 1);
    // At small sizes the old and new alloc count can both be
    // kMinHeapAllocCount.
...
static inline void
swap(SkTArray<T, M>& a, SkTArray<T, M>& b)
{
  a.swap(b);
}
