...
SkSignBitTo2sCompliment(int32_t x)
{
  if (x < 0) {
    x &= 0x7FFFFFFF;
    x = -x;
 ...
 */
static inline int32_t
Sk2sComplimentToSignBit(int32_t x)
{
  int sign = x >> 31;
  // make x positive
  x = (x ^ sign) - sign;
...
static inline float
SkBits2Float(int32_t floatAsBits)
{
  SkFloatIntUnion data;
...
static inline bool
SkFloatBits_IsFinite(int32_t bits)
{
  return (bits & gFloatBits_exponent_mask) != gFloatBits_exponent_mask;
}

static inline bool
SkFloatBits_IsInf(int32_t bits)
{
  return ((bits & gFloatBits_exponent_mask) == gFloatBits_exponent_mask) &&
 ...
 */
static inline float
Sk2sComplimentAsFloat(int32_t x)
{
