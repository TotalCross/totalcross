...
public:
  ...
  // If the counter is positive, decrement it by 1 and return true, otherwise
  // return false.
  bool try_wait();
...
inline void
SkBaseSemaphore::wait()
{
  // Since this fetches the value before the subtract, zero and below means that
  // there are no resources left, so the thread needs to wait.
