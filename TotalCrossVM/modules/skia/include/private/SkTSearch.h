...
// The most general form of SkTSearch takes an array of T and a key of type K. A
// functor, less, is used to perform comparisons. It has two function operators:
//      bool operator() (const T& t, const K& k)
//      bool operator() (const K& t, const T& k)
template<typename T, typename K, typename LESS>
int
SkTSearch(const T base[], int count, const K& key, size_t elemSize, LESS& less)
{
  SkASSERT(count >= 0);
  if (count <= 0) {
    return ~0;
...
// Specialization for case when T==K and the caller wants to use a function
// rather than functor.
template<typename T, bool(LESS)(const T&, const T&)>
int
SkTSearch(const T base[], int count, const T& target, size_t elemSize)
{
...
// Similar to SkLessFunctionToFunctorAdaptor but makes the functor interface
// take T* rather than T.
template<typename T, bool(LESS)(const T&, const T&)>
...
// Specialization for case where domain is an array of T* and the key value is a
// T*, and you want to compare the T objects, not the pointers.
template<typename T, bool(LESS)(const T&, const T&)>
int
SkTSearch(T* base[], int count, T* target, size_t elemSize)
{
...
int
SkStrSearch(const char* const* base,
            int count,
            const char target[],
            size_t target_len,
            size_t elemSize);
...
/** Like SkStrSearch, but treats target as if it were all lower-case. Assumes
   that base points to a table of lower-case strings.
*/
int
SkStrLCSearch(const char* const* base,
              int count,
              const char target[],
              size_t target_len,
              size_t elemSize);
