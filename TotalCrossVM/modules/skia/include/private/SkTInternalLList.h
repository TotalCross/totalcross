...
/**
 * Helper class to automatically initialize the doubly linked list created
 * pointers.
 */
template<typename T>
class SkPtrWrapper
...
/**
 * This macro creates the member variables required by the SkTInternalLList
 * class. It should be placed in the private section of any class that will be
...
public:
  ...
  /**
   * Inserts a new list entry before an existing list entry. The new entry must
   * not already be a member of this or any other list. If existingEntry is NULL
   * then the new entry is added at the tail.
   */
  ...
  /**
   * Inserts a new list entry after an existing list entry. The new entry must
   * not already be a member of this or any other list. If existingEntry is NULL
   * then the new entry is added at the head.
   */
  ...
  void validate() const
  {
    SkASSERT(!fHead == !fTail);
    Iter iter;
    for (T* item = iter.init(*this, Iter::kHead_IterStart); item;
         item = iter.next()) {
  ...
  /**
   * Debugging-only method that uses the list back pointer to check if 'entry'
   * is indeed in 'this' list.
   */
