...
/**
 * For some classes, it's almost always an error to instantiate one without a
 * name, e.g.
 *   {
 *       SkAutoMutexAcquire(&mutex);
 *       <some code>
 *   }
 * In this case, the writer meant to hold mutex while the rest of the code in
 * the block runs, but instead the mutex is acquired and then immediately
 * released.  The correct usage is
 *   {
 ...
 * SK_REQUIRE_LOCAL_VAR like this: class classname { <your class>
 *   };
 *   #define classname(...) SK_REQUIRE_LOCAL_VAR(classname)
 *
 * This won't work with templates, and you must inline the class' constructors
 * and destructors. Take a look at SkAutoFree and SkAutoMalloc in this file for
 * examples.
 */
...
#if defined(__clang__) // This should work on GCC too, but GCC diagnostic pop
                       // didn't seem to work!
