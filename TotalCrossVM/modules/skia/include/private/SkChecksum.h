...
// #include "SkOpts.h"
// It's sort of pesky to be able to include SkOpts.h here, so we'll just
...
public:
  /**
   * uint32_t -> uint32_t hash, useful for when you're about to trucate this
   * hash but you suspect its low bits aren't well mixed.
   *
   ...
   * hash but you suspect its low bits aren't well mixed.
   *
   *  This version is 2-lines cheaper than Mix, but seems to be sufficient for
   * the font cache.
   */
