...
SK_API CGContextRef
SkCreateCGContext(const SkPixmap&);

/**
 *  Given a CGImage, allocate an SkBitmap and copy the image's pixels into it.
 * If scaleToFit is not null, use it to determine the size of the bitmap, and
 * scale the image to fill the bitmap. Otherwise use the image's width/height.
 *
 *  On failure, return false, and leave bitmap unchanged.
 */
SK_API bool
SkCreateBitmapFromCGImage(SkBitmap* dst, CGImageRef src);

SK_API sk_sp<SkImage> SkMakeImageFromCGImage(CGImageRef);

/**
 *  Copy the pixels from src into the memory specified by
 * info/rowBytes/dstPixels. On failure, return false (e.g. ImageInfo
 * incompatible with src).
 */
SK_API bool
SkCopyPixelsFromCGImage(const SkImageInfo& info,
                        size_t rowBytes,
                        void* dstPixels,
                        CGImageRef src);
static inline bool
SkCopyPixelsFromCGImage(const SkPixmap& dst, CGImageRef src)
{
  return SkCopyPixelsFromCGImage(
    dst.info(), dst.rowBytes(), dst.writable_addr(), src);
}
 ...
 */
SK_API CGImageRef
SkCreateCGImageRefWithColorspace(const SkBitmap& bm, CGColorSpaceRef space);
...
SkCreateCGImageRef(const SkBitmap& bm)
{
 ...
 */
void
SkCGDrawBitmap(CGContextRef, const SkBitmap&, float x, float y);

/**
 *  Return a provider that wraps the specified stream.
 *  When the provider is finally deleted, it will delete the stream.
 */
CGDataProviderRef SkCreateDataProviderFromStream(
  std::unique_ptr<SkStreamRewindable>);
