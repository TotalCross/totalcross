...
public:
   ...
   * or may be newer). The function will return NULL in the event that one of
   * the following conditions are true. 1) the canvas device type is not
   * supported (currently only raster is supported) 2) the canvas clip type is
   * not supported (currently only non-AA clips are supported)
   *
   * It is recommended that the original canvas also not be used until all
   * canvases that have been created using its captured state have been
   * dereferenced.
   *
