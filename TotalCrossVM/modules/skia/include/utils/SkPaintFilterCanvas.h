...
protected:
   ...
   *  The implementation may modify the paint as they wish (using
   ...
   *
   *  Note: The base implementation calls onFilter() for top-level/explicit
   * paints only. To also filter encapsulated paints (e.g. SkPicture,
   * SkTextBlob), clients may need to override the relevant methods (i.e.
   * drawPicture, drawTextBlob).
   */
  virtual bool onFilter(SkTCopyOnFirstWrite<SkPaint>* paint,
                        Type type) const = 0;
