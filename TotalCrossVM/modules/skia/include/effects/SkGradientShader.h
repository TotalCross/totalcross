...
public:
  ...
  /** Returns a shader that generates a linear gradient between the two
     ...
     two points
      @param  pos     May be NULL. array[count] of SkScalars, or NULL, of the
     relative position of each corresponding color in the colors array. If this
     is NULL, the the colors are distributed evenly between the start and end
     point. If this is not null, the values must begin with 0, end with 1.0, and
  ...
  */
  ...
  {
    return MakeLinear(
      pts, colors, std::move(colorSpace), pos, count, mode, 0, nullptr);
  }
  ...
  /** Returns a shader that generates a radial gradient given the center and
     ...
     center and edge of the circle
      @param  pos     May be NULL. The array[count] of SkScalars, or NULL, of
     the relative position of each corresponding color in the colors array. If
     this is NULL, the the colors are distributed evenly between the center and
     edge of the circle. If this is not null, the values must begin with 0, end
     with 1.0, and intermediate values must be strictly increasing.
      @param  count   Must be >= 2. The number of colors (and pos if not NULL)
     entries
      @param  mode    The tiling mode
  */
  static sk_sp<SkShader> MakeRadial(const SkPoint& center,
                                    SkScalar radius,
                                    const SkColor colors[],
                                    const SkScalar pos[],
                                    int count,
  ...
  */
   ...
   */
  static sk_sp<SkShader> MakeTwoPointConical(const SkPoint& start,
  ...
  /** Returns a shader that generates a sweep gradient given a center.
     ...
     the center, within the gradient angle range.
      @param  pos        May be NULL. The array[count] of SkScalars, or NULL, of
     the relative position of each corresponding color in the colors array. If
     this is NULL, then the colors are distributed evenly within the angular
     range. If this is not null, the values must begin with 0, end with 1.0, and
                         intermediate values must be strictly increasing.
      @param  count      Must be >= 2. The number of colors (and pos if not
     NULL) entries
      @param  mode       Tiling mode: controls drawing outside of the gradient
     angular range.
  ...
  */
  ...
  {
    return MakeSweep(
      cx, cy, colors, std::move(colorSpace), pos, count, 0, nullptr);
  }
