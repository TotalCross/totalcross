...
/** Represents a client-created renderable surface and is used to draw text into
 * the surface. */
...
public:
   ...
   * surface when draws are flushed to the SkAtlasTextContext's
   * SkAtlasTextRenderer.
   */
  ...
  /**
   * Enqueues a text draw in the target. The caller provides an array of glyphs
   * and their positions. The meaning of 'color' here is interpreted by the
  ...
  /** Saves the current matrix in a stack. Returns the prior depth of the saved
   * matrix stack. */
  int save();
  /** Pops the top matrix on the stack if the stack is not empty. */
  void restore();
  /**
   * Pops the matrix stack until the stack depth is count. Does nothing if the
   * depth is already less than count.
   */
...
protected:
  SkAtlasTextTarget(sk_sp<SkAtlasTextContext>,
                    int width,
                    int height,
                    void* handle);
