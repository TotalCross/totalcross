...
/**
 * This is the base class for a renderer implemented by the SkAtlasText client.
 * The SkAtlasTextContext issues texture creations, deletions, uploads, and
 * vertex draws to the renderer. The renderer must perform those actions in the
 * order called to correctly render the text drawn to SkAtlasTextTargets.
 */
...
public:
  ...
  struct SDFVertex
  {
    /** Position in device space (not normalized). The third component is w (not
     * z). */
    SkPoint3 fPosition;
   ...
   * 'textureHandle' in the rectangle ['x', 'x' + 'width') x ['y', 'y' +
   * 'height'). 'rowBytes' specifies the byte offset between successive rows in
   * 'data' and will always be a multiple of the number of bytes per pixel. The
   * pixel format of data is the same as that of 'textureHandle'.
   */
  virtual void setTextureData(void* textureHandle,
                              const void* data,
                              int x,
                              int y,
                              int width,
   ...
   */
  virtual void drawSDFGlyphs(void* targetHandle,
                             void* textureHandle,
