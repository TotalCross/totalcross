...
#define GR_GL_DRIVER_VER(major, minor, point)                                  \
  ((static_cast<uint64_t>(major) << 32) |                                      \
   (static_cast<uint64_t>(minor) << 16) | static_cast<uint64_t>(point))
...
enum GrGLRenderer
{
  ...
  kTegra_GrGLRenderer, // Tegra with the same architecture as NVIDIA desktop
                       // GPUs (K1+).
...
GrGLVersion
GrGLGetVersionFromString(const char* versionString);
GrGLStandard
GrGLGetStandardInUseFromString(const char* versionString);
GrGLSLVersion
GrGLGetGLSLVersionFromString(const char* versionString);
GrGLVendor
GrGLGetVendorFromString(const char* vendorString);
GrGLRenderer
GrGLGetRendererFromStrings(const char* rendererString, const GrGLExtensions&);
void
...
void
GrGLGetDriverInfo(GrGLStandard standard,
                  GrGLVendor vendor,
...
GrGLVersion
GrGLGetVersion(const GrGLInterface*);
GrGLSLVersion
GrGLGetGLSLVersion(const GrGLInterface*);
GrGLVendor
GrGLGetVendor(const GrGLInterface*);
GrGLRenderer
GrGLGetRenderer(const GrGLInterface*);
...
void
GrGLCheckErr(const GrGLInterface* gl, const char* location, const char* call);

void
GrGLClearErr(const GrGLInterface* gl);
...
GrGLenum
GrToGLStencilFunc(GrStencilTest test);
