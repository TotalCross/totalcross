...
public class RouteViewer extends MapActivity
{
      ...
      if (!shadow) {
        ...
        switch (mode) {
          ...
          case 3: { // the last path
            ...
            paint.setAlpha(120);
            canvas.drawLine(point.x, point.y, point2.x, point2.y, paint);
            RectF oval = new RectF(point2.x - mRadius,
                                   point2.y - mRadius,
                                   point2.x + mRadius,
                                   point2.y + mRadius);
            /* end point */
  ...
  public void onCreate(Bundle savedInstanceState)
  {
    ...
    StrictMode.setThreadPolicy(
      ThreadPolicy.LAX); // get rid of NetworkOnMainThreadException
    ...
    try {
      ...
      if (latI != 0 && lonI != 0 && latF != 0 && lonF != 0) {
        ...
        for (int i = 0; i < s.length;) {
          pairs.add(new GeoPoint((int)(Double.valueOf(s[i++]) * 1E6),
                                 (int)(Double.valueOf(s[i++]) * 1E6)));
        }
      }
      if (pairs == null) {
      ...
      mapview.getController().zoomToSpan(
        Math.abs(p1.getLatitudeE6() - p2.getLatitudeE6()),
        Math.abs(p1.getLongitudeE6() - p2.getLongitudeE6()));
      mapview.getController().animateTo(new GeoPoint(
        p1.getLatitudeE6() - ((p1.getLatitudeE6() - p2.getLatitudeE6()) / 2),
        p1.getLongitudeE6() -
          ((p1.getLongitudeE6() - p2.getLongitudeE6()) / 2)));
    } catch (Exception e) {
