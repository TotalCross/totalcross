...
{ // guich@tc126_34
  class Preview extends SurfaceView implements SurfaceHolder.Callback
  {
    Preview(Context context)
    {
      ...
      if (allowRotation) {
        switch (getResources().getConfiguration().orientation) {
          ...
          case Configuration.ORIENTATION_LANDSCAPE:
            ...
            if (getWindowManager().getDefaultDisplay().getRotation() ==
      ...
      holder.setType(
        SurfaceHolder
          .SURFACE_TYPE_PUSH_BUFFERS); // DON'T REMOVE THIS! 2.3 (LEVEL 10)
                                       // STILL REQUIRES IT
    }

    // Called once the holder is ready
    public void surfaceCreated(SurfaceHolder holder) { startPreview(); }
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h)
    {
      if (camera != null) {
        ...
        if (Build.VERSION.SDK_INT >= 14 &&
            ...
            !inFocusExclusionList()) {
          parameters.setFocusMode(
            "continuous-picture"); // FOCUS_MODE_CONTINUOUS_PICTURE
        }
        ...
        parameters.setJpegQuality(
          stillQuality == 1 ? 75 : stillQuality == 2 ? 85 : 100);
        if (width != 0 && height != 0) {
      ...
      try {
        ...
        if ((camera = Camera.open()) == null) {
          ...
          if (getNumberOfCameras != null) {
            ...
            Method open =
              android.hardware.Camera.class.getMethod("open", int.class);
            if (open != null)
              while (--i >= 0)
  ...
  {
    try {
      camera.stopPreview(); // stop camera's preview
    } catch (Exception e) {
      e.printStackTrace();
    }
