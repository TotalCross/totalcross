...
public class MapViewer extends MapActivity {
	...
	private class Circle extends MapItem {
		...
		Circle(String s) {
			...
			color = Integer.valueOf(ss[i++]); if ((color & 0xFF000000) == 0) {
				color |= 0xFF000000;
			}
		}
	...
	private class Place extends MapItem {
		...
		Place(String s) {
			...
			String[] ss = s.split("\"");
			caption = ss[1]; if (caption.equals("null")) {
				caption = null;
			}
			details = ss[3].split("\n");
			ss = ss[4].split(",");
			...
			geo = new GeoPoint(lat,lon);
			computeBounds(lat,lon);
			backColor = Integer.valueOf(ss[3]); if ((backColor & 0xFF000000) == 0) {
				backColor |= 0xFF000000;
			}
			capColor = Integer.valueOf(ss[4]); if ((capColor  & 0xFF000000) == 0) {
				capColor |= 0xFF000000;
			}
			detColor = Integer.valueOf(ss[5]); if ((detColor  & 0xFF000000) == 0) {
				detColor |= 0xFF000000;
			}
			pinColor = Integer.valueOf(ss[6]); if ((pinColor  & 0xFF000000) == 0) {
				pinColor |= 0xFF000000;
			}
