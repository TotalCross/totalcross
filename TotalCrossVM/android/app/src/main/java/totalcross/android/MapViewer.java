...
public class MapViewer extends MapActivity
{
  ...
  private class Circle extends MapItem
  {
    ...
    public void draw(Canvas canvas, Projection projection)
    {
      ...
      if (rad > 0) { // in meters?
        ...
        oval = new RectF(
          point.x - frad, point.y - frad, point.x + frad, point.y + frad);
      } else { // in coords delta
        double r = -rad;
        float la = fromCoord(geo.getLatitudeE6()),
              lo = fromCoord(geo.getLongitudeE6());
        Point p1 = projection.toPixels(
          new GeoPoint(toCoord(la - r), toCoord(lo - r)), null);
        Point p2 = projection.toPixels(
          new GeoPoint(toCoord(la + r), toCoord(lo + r)), null);
  ...
  private class Place extends MapItem
  {
    ...
    Place(String s)
    {
      ...
      geo = new GeoPoint(lat, lon);
      computeBounds(lat, lon);
      backColor = Integer.valueOf(ss[3]);
      if ((backColor & 0xFF000000) == 0) {
        backColor |= 0xFF000000;
      }
      capColor = Integer.valueOf(ss[4]);
      if ((capColor & 0xFF000000) == 0) {
        capColor |= 0xFF000000;
      }
      detColor = Integer.valueOf(ss[5]);
      if ((detColor & 0xFF000000) == 0) {
        detColor |= 0xFF000000;
      }
      pinColor = Integer.valueOf(ss[6]);
      if ((pinColor & 0xFF000000) == 0) {
        pinColor |= 0xFF000000;
      }
    ...
    { // above
      ...
      float frad = mRadius * fontPerc /
                   100; // projection.metersToEquatorPixels((float)mRadius);
      Point point = new Point();
      projection.toPixels(geo, point);
      ...
      float yy =
        point.y - frad * 2 * ((hasCap ? 1 : 0) + details.length + 1) - frad;
      ww += textH; // give some extra space

      // draw baloon
      // canvas.drawRect(point.x - ww/2, yy, point.x + ww/2, point.y-frad,
      // mpaint);
      ...
      path.addRoundRect(
        new RectF(
          point.x - ww / 2, yy, point.x + ww / 2, point.y - frad - frad),
        textH / 2,
        textH / 2,
        Path.Direction.CCW);
  ...
  private class MyItemsOverLay extends Overlay
  {
    ...
    public MyItemsOverLay(MapItem[] items)
    { // GeoPoint is a int. (6E)
      this.items = items;
    }
  ...
  public void onCreate(Bundle savedInstanceState)
  {
    ...
    // setup the viewe
    MapView mapview =
      new MapView(this, "0FcAyehwXTAXpaMaXoVn7kGJVJdRSuSI2RsqELQ");
  ...
  private static int toCoord(double v) { return (int)(v * 1e6); }
  private static float fromCoord(int i) { return (float)i / 1e6f; }
