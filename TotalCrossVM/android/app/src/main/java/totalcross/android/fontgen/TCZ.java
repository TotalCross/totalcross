...
public class TCZ {
	...
	public static class Entry {
		...
		public Entry(byte []bytes, String name, int uncompressedSize, Object extra) throws Exception {
			...
			if (name2write.indexOf('.') <
				0) { // no dots on string? note that an image will have a dot, so its correctly kept with / on the name
	...
	public TCZ(Vector<Entry> vout, String outName, short attr) throws IOException {
		...
		size += writeInt(fout, compressedHeaderBytes.length +
	...
	private static void sort(Object []items, int first, int last) {
		...
		while (true) {
			while (high >= low
				   && mid.compareTo(items[low].toString())  >
				   0) { // guich@566_25: added "high > low" here and below - guich@568_5: changed to >=
				low++;
			}
