...
public class TCZ
{
  ...
  public static class Entry
  {
      ...
      throws Exception
    {
      ...
      name2write = name;
      if (name2write.indexOf('.') <
          0) { // no dots on string? note that an image will have a dot, so its
               // correctly kept with / on the name
        name2write =
  ...
  public TCZ(Vector<Entry> vout, String outName, short attr) throws IOException
  {
    ...
    AndroidUtils.debug("header: " + hh.length + " -> " +
                       compressedHeaderBytes.length);
    ...
    size += writeInt(fout,
                     compressedHeaderBytes.length +
                       8); // base offset = compressed header size + 4
  ...
  private static void sort(Object[] items, int first, int last)
  {
    ...
    while (true) {
      while (high >= low && mid.compareTo(items[low].toString()) >
                              0) { // guich@566_25: added "high > low" here
                                   // and below - guich@568_5: changed to >=
        low++;
      }
  ...
  public static int writeSmallString(OutputStream stream, String s)
    throws IOException
  {
