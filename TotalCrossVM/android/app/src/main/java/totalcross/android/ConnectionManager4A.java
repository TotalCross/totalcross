...
public class ConnectionManager4A {
	...
	*/
		...
		switch (type) {
			case GPRS: {
				...
				if (cursor == null || cursor.getCount() <= 0) {
					TelephonyManager tel = (TelephonyManager) Launcher4A.loader.getSystemService(
	...
	public static String getLocalHost() {
		try {
			ConnectivityManager connMgr = (ConnectivityManager) Launcher4A.loader.getSystemService(
	...
	private static String ipAddressToString(int addr) {
		...
		buf.append(addr & 0xff).append('.').append((addr >>>= 8) & 0xff).append('.').append((
					addr >>>= 8) & 0xff).append(
						'.').append((addr >>>= 8) & 0xff);
		return buf.toString();
	}
