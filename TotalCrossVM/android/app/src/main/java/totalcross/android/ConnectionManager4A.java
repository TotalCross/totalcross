...
public class ConnectionManager4A
{
  ...
  /*   private static final int DEFAULT_TRANSPORT_ORDER[] = { TRANSPORT_WIFI,
     TRANSPORT_BES, TRANSPORT_BIS, TRANSPORT_DIRECT_TCP, TRANSPORT_WAP2 };
  ...
  */
    ...
    switch (type) {
      case GPRS: {
        ...
        ContentResolver contentResolver =
          Launcher4A.loader.getContentResolver();
        Cursor cursor =
          contentResolver.query(CONTENT_URI,
                                new String[] { "_id" },
                                "apn = ? and user = ? and password = ?",
                                new String[] { "tim.br", "tim", "tim" },
                                null);
        ...
        if (id > -1) {
          ...
          // provide content://telephony/carriers/preferapn URI mapping
          values.put("apn_id", id);
          contentResolver.update(PREFERRED_APN_URI, values, null, null);
          cursor = contentResolver.query(PREFERRED_APN_URI,
                                         new String[] { "name", "apn" },
                                         "_id=" + id,
                                         null,
                                         null);
  ...
  public static void open() { connRef = null; }
  ...
  public static void close() { connRef = null; }
  ...
  public static String getLocalHost()
  {
    try {
      ConnectivityManager connMgr =
        (ConnectivityManager)Launcher4A.loader.getSystemService(
          Context.CONNECTIVITY_SERVICE);
      int type = connMgr.getActiveNetworkInfo().getType();
      if (type == ConnectivityManager.TYPE_WIFI) {
        WifiManager wifiMgr =
          (WifiManager)Launcher4A.loader.getSystemService(Context.WIFI_SERVICE);
  ...
  public static boolean isInternetAccessible()
  {
    try {
      InetSocketAddress isa =
        new InetSocketAddress(InetAddress.getByName("www.google.com"), 80);
      Socket s = new Socket();
