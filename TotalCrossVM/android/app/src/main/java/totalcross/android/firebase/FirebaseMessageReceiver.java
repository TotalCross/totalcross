...
/**
 ...
 One of the parameters that is sent from the server is the classname, so that
 the correct application can be called. The message is written to the Message
 queue file, and a broadcast is sent, because if the application is running, it
 can get the message. The message is also sent as notification so, if the app is
 not running, it can be opened.
  ...
  public void onMessageReceived(RemoteMessage message)
  {
    ...
    if (data != null) {
      try {
        ...
        b.putByteArray("messageId",
                       messageId == null ? null : messageId.getBytes());
        b.putByteArray("messageType",
                       messageType == null ? null : messageId.getBytes());
        b.putStringArrayList("keys", keys);
        b.putStringArrayList("values", values);
        b.putByteArray("collapseKey",
                       collapseKey == null ? null : collapseKey.getBytes());
        ...
        String classname =
          getClass()
            .getName(); // totalcross.apptapi.firebase.FirebaseMessageReceiver
        classname =
          classname.replace("firebase.FirebaseMessageReceiver", "Loader");

        if (appdata == null) {
          return;
          // throw new Exception("You must suply at least 'appdata' or 'title'
          // parameter with the message; data received\n" + data);
        }
        ...
        if (title != null) {
          ...
          String pack =
            classname.substring(0, dot); //"totalcross.app.totalcrossapi";
