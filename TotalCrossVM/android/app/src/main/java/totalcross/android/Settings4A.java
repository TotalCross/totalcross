...
public final class Settings4A {
	...
	static void fillSettings() {
		...
		if (!Loader.IS_EMULATOR) {
			WifiManager wifiMan = (WifiManager) ctx.getApplicationContext().getSystemService(
			...
			if (wifiMan != null) { // not sure what happens when device has no connectivity at all
				...
				if (macAddress ==
					null) { // if wifi never turned on since last boot, turn it on and off to be able to get the mac (on android, the mac is cached by the O.S.)
		...
		if ((serialNumber == null || "unknown".equalsIgnoreCase(serialNumber)) && !Loader.IS_EMULATOR
			&& macAddress != null) { // no else here!
			serialNumber = String.valueOf(((long)macAddress.replace(":", "").hashCode() & 0xFFFFFFFFFFFFFFL));
		}

		ANDROID_ID = Settings.Secure.getString(Launcher4A.loader.getContentResolver(),
