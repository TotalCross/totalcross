...
public final class Settings4A
{
  ...
  public static void refresh() { settingsRefresh(); }
  ...
  static void fillSettings()
  {
    ...
    } else if (v.length() == 3 && v.charAt(1) == '.') {       // 2.3
    ...
    deviceId = Build.MANUFACTURER.replaceAll("\\P{ASCII}", " ") + " " +
    ...
    // if using a new device, get its serial number. otherwise, create one from
    // the mac-address
    ...
    if (!Loader.IS_EMULATOR) {
      WifiManager wifiMan =
        (WifiManager)ctx.getApplicationContext().getSystemService(
          Context.WIFI_SERVICE);
      if (wifiMan != null) { // not sure what happens when device has no
                             // connectivity at all
        macAddress = wifiMan.getConnectionInfo().getMacAddress();
        if (macAddress ==
            null) { // if wifi never turned on since last boot, turn it on and
                    // off to be able to get the mac (on android, the mac is
                    // cached by the O.S.)
    ...
    if ((serialNumber == null || "unknown".equalsIgnoreCase(serialNumber)) &&
        !Loader.IS_EMULATOR && macAddress != null) { // no else here!
      serialNumber = String.valueOf(
        ((long)macAddress.replace(":", "").hashCode() & 0xFFFFFFFFFFFFFFL));
    }

    ANDROID_ID = Settings.Secure.getString(
      Launcher4A.loader.getContentResolver(), Settings.Secure.ANDROID_ID);
    ...
    dateFormat =
      d.startsWith("25") ? DATE_DMY : d.startsWith("12") ? DATE_MDY : DATE_YMD;
    ...
    is24Hour = d.toLowerCase().indexOf("am") == -1 &&
               d.toLowerCase().indexOf("pm") == -1;

    // graphics
    screenDensity =
      Launcher4A.instance.getResources().getDisplayMetrics().density;

    settingsRefresh();

    if (!Loader.IS_EMULATOR) { // running on emulator, right now there's no way
                               // to retrieve more settings from it.
      ...
      // is24Hour
      is24Hour = Settings.System.getInt(
                   cr, Settings.System.TIME_12_24, is24Hour ? 24 : 12) == 24;

      // date format
      String format =
        Settings.System.getString(cr, Settings.System.DATE_FORMAT);
      if (format != null && format.length() > 0) {
        char firstChar = format.charAt(0);
        dateFormat =
          firstChar == 'd' ? DATE_DMY : firstChar == 'M' ? DATE_MDY : DATE_YMD;
      }
    }
  }
  ...
  public static void fillTelephonySettings()
  {
    ...
    TelephonyManager telephonyMgr =
      (TelephonyManager)Launcher4A.loader.getSystemService(
    ...
    for (int i = mtds.length; --i >= 0;) {
      ...
      if (name.startsWith("getDeviceId") && signat.endsWith("(int)")) {
        try {
          ...
          if (id1 != null && id1.equals(id2)) {
            imei = id1; // some devices return a dumb imei each time getDeviceId
                        // is called
          }
          // dual-sim support
          ...
          if (id1 != null && id1.equals(id2)) {
            imei2 = id1; // some devices return a dumb imei each time
      ...
      } else if (name.startsWith("getSimSerialNumber") &&
                 signat.endsWith("(int)")) {
        try {
          id1 = (String)m.invoke(telephonyMgr, new Integer(0));
          id2 = (String)m.invoke(telephonyMgr, new Integer(0));
          if (id1 != null && id1.equals(id2)) {
            iccid = id1; // some devices return a dumb imei each time
                         // getDeviceId is called
          }
          if (--toFind == 0) {
            break;
