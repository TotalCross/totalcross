...
public class RadioDevice4A
{
  ...
  public static boolean isSupported(int type)
  {
    switch (type) {
      case WIFI:
        return ConnectivityManager.isNetworkTypeValid(
  ...
  public static int getState(int type)
  {
    switch (type) {
      case WIFI: {
        WifiManager wifiMgr =
          (WifiManager)Launcher4A.loader.getApplicationContext()
            .getSystemService(Context.WIFI_SERVICE);
        return wifiMgr.isWifiEnabled() ? RADIO_STATE_ENABLED
      ...
      case BLUETOOTH: {
        ...
        boolean isDisc =
          isOn && Bluetooth4A.isDiscoverable() == Level5.NO_ERROR;
        return isDisc ? BLUETOOTH_STATE_DISCOVERABLE
                      : isOn ? RADIO_STATE_ENABLED : RADIO_STATE_DISABLED;
      }
      default:
        return RADIO_STATE_DISABLED;
    }
  ...
  public static void setState(int type, int state)
  {
    switch (type) {
      ...
      case BLUETOOTH:
        switch (state) {
          case BLUETOOTH_STATE_DISCOVERABLE:
            Bluetooth4A.makeDiscoverable();
            break;
          case RADIO_STATE_ENABLED:
