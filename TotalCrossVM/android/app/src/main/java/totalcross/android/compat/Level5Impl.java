...
public class Level5Impl extends Level5
{
  ...
  // UUID.fromString("e3b9f92c-3226-4ccb-9e0a-218695bd402c");
      ...
      try {
        switch (type) {
          ...
          case BT_CLOSE:
            ...
            break;
          case BT_IS_RADIO_ON: //
            setResponse(btAdapter.getState() == BluetoothAdapter.STATE_ON,
                        null);
            break;
          case BT_IS_DISCOVERABLE: //
            setResponse(btAdapter.getScanMode() ==
  ...
  private void btReadWrite(final boolean isRead,
                           final String btc,
                           ...
                           final int count) throws Exception
  {
    ...
    if (sock == null) {
      throw new IOException("socket for device " + btc +
                            " not found on hashtable");
    }
  ...
  Hashtable<String, BluetoothSocket> htbt =
    new Hashtable<String, BluetoothSocket>(5);
  ...
  */
    ...
    if (sock == null) {
      ...
      while (true)
        try {
          ...
          if (msg != null &&
              msg.indexOf("discovery failed") >= 0) { // invalid password
            if (device.getBondState() == BluetoothDevice.BOND_BONDING) {
              println("STILL BONDING");
              for (int i = 0; i < 200 && device.getBondState() ==
  ...
  { // 000780885D13 -> 00:07:80:88:5D:13
    try {
  ...
  private void btMakeDiscoverable()
  {
    if (btAdapter.getScanMode() !=
      ...
      discoverableIntent.putExtra(BluetoothAdapter.EXTRA_DISCOVERABLE_DURATION,
                                  300);
      Launcher4A.loader.startActivityForResult(discoverableIntent,
                                               BT_MAKE_DISCOVERABLE);
    } else {
  ...
  private void btGetPairedDevices()
  {
    ...
    if (pairedDevices.size() > 0)
      for (BluetoothDevice device : pairedDevices) {
        devices.add(device.getAddress().replace(":", "") + "|" +
                    device.getName());
      }
    String[] ret = null;
    if (devices.size() > 0) {
      devices.copyInto(ret = new String[devices.size()]);
    }
    setResponse(true, ret);
  }

  private static void println(String s) { AndroidUtils.debug("*** " + s); }
  ...
  {
    devices.removeAllElements();
    Launcher4A.loader.registerReceiver(
    ...
    public void run()
    {
      try {
        BluetoothSocket acceptedSocket =
          serverSocket.accept(); // blocks until a connection is accepted
  ...
  BluetoothServerSocket
    serverSocket; // only a single instance can be active on Android
  ...
  private void btserverAccept(String uuid) throws Exception
  {
    ...
    // create new ones
    UUID u = SPP_UUID; // UUID.fromString(uuid.replace("{","").replace("}",""));
                       // // "27648B4D-D854-5674-FA60E4F535E44AF7  // generate
                       // your own UUID at http://www.uuidgenerator.com
    serverSocket =
      unsecure
        ? btAdapter.listenUsingInsecureRfcommWithServiceRecord("MyBluetoothApp",
                                                               u)
        : btAdapter.listenUsingRfcommWithServiceRecord("MyBluetoothApp", u);
  ...
  { // uuid is currently being ignored, since only one
    // serversocket instance is currently allowed
    if (serverSocket != null)
      try {
        serverSocket.close(); // close the listening socket - does not close the
