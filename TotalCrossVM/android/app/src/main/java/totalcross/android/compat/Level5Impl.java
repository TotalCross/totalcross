...
public class Level5Impl extends Level5 {
	...
	private void btReadWrite(final boolean isRead, final String btc, final byte[] byteArray,
							 final int ofs,
	...
	*/
	private void btConnect(String addr) throws Exception {
		...
		if (sock == null) {
			...
			sock = unsecure ? device.createInsecureRfcommSocketToServiceRecord(SPP_UUID) :
	...
	private void btGetUnpairedDevices() {
		...
		Launcher4A.loader.registerReceiver(mReceiver,
	...
	private void btserverAccept(String uuid) throws Exception {
		...
		serverSocket = unsecure ? btAdapter.listenUsingInsecureRfcommWithServiceRecord("MyBluetoothApp",
					   u) : btAdapter.listenUsingRfcommWithServiceRecord("MyBluetoothApp", u);
