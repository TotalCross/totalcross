...
public class GPSHelper
  implements android.location.LocationListener, GpsStatus.Listener,
             LocationListener, ConnectionCallbacks, OnConnectionFailedListener
{
  ...
  public static boolean isGpsOn()
  {
    LocationManager manager =
      (LocationManager)Launcher4A.loader.getSystemService(
    ...
    try {
      ...
      if (provider == null || provider.equals("gps") ||
        ...
        String lat = Double.toString(
          loc.getLatitude()); // flsobral@tc126_57: Decimal separator might be
                              // platform dependent when using
                              // Location.convert with
                              // Location.FORMAT_DEGREES.
        String lon = Double.toString(loc.getLongitude());

        Calendar fix = new GregorianCalendar(TimeZone.getTimeZone(
          "GMT")); // flsobral@tc126_57: Date is deprecated, and apparently
                   // bugged for some devices. Replaced with Calendar.
        fix.setTimeInMillis(loc.getTime());
        String sat = String.valueOf(Math.max(sats, validSatellites));
        String vel = loc.hasSpeed() && loc.getSpeed() != 0d
                       ? String.valueOf(loc.getSpeed())
                       : "";
        String dir = loc.hasBearing() ? String.valueOf(loc.getBearing()) : "";
        String sfix =
          fix.get(Calendar.YEAR) + "/" + (fix.get(Calendar.MONTH) + 1) + "/" +
          fix.get(Calendar.DAY_OF_MONTH) + " " + fix.get(Calendar.HOUR_OF_DAY) +
          ":" + fix.get(Calendar.MINUTE) + ":" + fix.get(Calendar.SECOND);
        float pdop =
          loc.hasAccuracy() ? loc.getAccuracy() : 0; // guich@tc126_66
        lastGps = lat + ";" + lon + ";" + sfix + ";" + sat + ";" + vel + ";" +
                  dir + ";" + pdop + ";";
      }
    } catch (Exception e) {
  ...
  public String gpsGetData()
  {
    ...
    String ret = isHigh ? (gps != null ? lastGps : null)
                        : (googleApiClient != null ? lastGps : null);
    // lastGps = NOGPS;
    Location fusedLocation = null;
    try {
      if ((ret == null || ret.equals(NOGPS)) && googleApiClient != null &&
          (fusedLocation = LocationServices.FusedLocationApi.getLastLocation(
  ...
  {
    if (gps != null && (event == GpsStatus.GPS_EVENT_SATELLITE_STATUS ||
  ...
  public void startGps()
  {
    boolean isHigh = Launcher4A.gpsPrecision == HIGH_GPS_PRECISION;
    if (isHigh && gps == null) {
      gps = (LocationManager)Launcher4A.loader.getSystemService(
        Context.LOCATION_SERVICE);
      try {
        gps.requestLocationUpdates(
          LocationManager.GPS_PROVIDER, 0, 0, instance);
        gps.addGpsStatusListener(instance);
      } catch (SecurityException e) {
        e.printStackTrace();
      }
    }
    if (!isHigh && googleApiClient == null &&
        /*Launcher4A.gpsPrecision == LOW_GPS_PRECISION && */
        checkPlayServices()) {
      ...
      googleApiClient = new GoogleApiClient.Builder(Launcher4A.loader)
      ...
      // try only to get satellite information, since this is not given by
