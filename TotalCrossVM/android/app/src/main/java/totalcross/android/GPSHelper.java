...
public class GPSHelper implements android.location.LocationListener, GpsStatus.Listener,
	...
	public static boolean isGpsOn() {
		LocationManager manager = (LocationManager) Launcher4A.loader.getSystemService(
		...
		try {
			...
			if (provider == null || provider.equals("gps") || provider.equals("fused")) {
				String lat = Double.toString(
								 loc.getLatitude()); //flsobral@tc126_57: Decimal separator might be platform dependent when using Location.convert with Location.FORMAT_DEGREES.
				String lon = Double.toString(loc.getLongitude());

				Calendar fix = new GregorianCalendar(
					TimeZone.getTimeZone("GMT")); //flsobral@tc126_57: Date is deprecated, and apparently bugged for some devices. Replaced with Calendar.
				...
				String sfix = fix.get(Calendar.YEAR) + "/" + (fix.get(Calendar.MONTH) + 1) + "/" + fix.get(
								  Calendar.DAY_OF_MONTH) + " " + fix.get(Calendar.HOUR_OF_DAY) + ":" + fix.get(
								  Calendar.MINUTE) + ":" + fix.get(Calendar.SECOND);
	...
	public String gpsGetData() {
		...
		try {
			if ((ret == null || ret.equals(NOGPS)) && googleApiClient != null
				&& (fusedLocation = LocationServices.FusedLocationApi.getLastLocation(googleApiClient)) != null) {
	...
	public String gpsTurn(boolean on) {
		if (isGpsOn()) {
			...
			if (on) { // if turning on, wait until the message is handled
				...
				return (isHigh && gps.isProviderEnabled(LocationManager.GPS_PROVIDER)) || (!isHigh
						&& gps != null) ? NOGPS : null;
			}
		}
		return null;
	...
	public void startGps() {
		...
		if (!isHigh && googleApiClient == null
			&& /*Launcher4A.gpsPrecision == LOW_GPS_PRECISION && */checkPlayServices()) {
