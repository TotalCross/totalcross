...
public class FontGenerator
{
  ...
  int[] fontsizes = { 7,  8,  9,  10, 11, 12, 13, 14, 15,
  ...
  public FontGenerator(String fontName, String[] extraArgs)
  {
    try {
      ...
      // parse parameters
      if (fontName.indexOf('_') !=
          -1) { // guich@421.27: lets the user pass fonts with spaces
      ...
      Typeface ttfp =
        Typeface
          .DEFAULT; // createFromAsset(Launcher4A.instance.getContext().getAssets(),
                    // "fonts/"+fontName+".ttf");
      Typeface ttfb =
        Typeface
          .DEFAULT_BOLD; // createFromAsset(Launcher4A.instance.getContext().getAssets(),
                         // "fonts/"+fontName+"bd.ttf");

      // create fonts
      println(
        "FontGenerator - Copyright (c) SuperWaba 2002-2014. Processing...");
      ...
      // write the file
      try {
        new File(Environment.getExternalStorageDirectory() + "/" + outName)
  ...
  private void convertFont(Vector<TCZ.Entry> v,
                           Typeface ttf,
                           int size,
                           String fileName,
                           Vector<Range> newRanges,
                           boolean isMono)
  {
    ...
    for (int ri = 0; ri < n; ri++) {
      ...
      for (i = ini; i <= end; i++) {
        ...
        widths[i] = w;
        if (detailed == 1) {
          println("Width of " + ((char)i) + " " + i + " = " + widths[i]);
        }
      }
    }
    ...
    for (int ri = 0; ri < n; ri++) {
      ...
      // println((ri+1)+" of "+n+". Original height: "+maxH+", ascent:
      // "+fm.getMaxAscent()+", descent: "+fm.getMaxDescent()+", leading:
      // "+fm.getLeading());
      ...
      pf.lastChar = end;
      pf.spaceWidth =
        isMono ? maxW : (int)paint.measureText(" "); // guich@tc115_87
      pf.maxWidth = maxW;
      pf.maxHeight = size;
    ...
    if (nullCount > 0) {
      println("A total of " + nullCount + " characters had no glyphs");
    }
  }
  ...
  static public void println(String s) { AndroidUtils.debug(s); }
  ...
  static class PalmFont
  {
    ...
    public TCZ.Entry save()
    {
      try {
        ...
        println("Font " + fileName + " stored compressed (" + fromBytes.length +
                " -> " + toBytes.length + ")");
