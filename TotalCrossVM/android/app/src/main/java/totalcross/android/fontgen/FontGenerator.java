...
public class FontGenerator {
	...
	public FontGenerator(String fontName, String []extraArgs) {
		try {
			...
			Typeface ttfp =
				Typeface.DEFAULT;//createFromAsset(Launcher4A.instance.getContext().getAssets(), "fonts/"+fontName+".ttf");
			Typeface ttfb =
				Typeface.DEFAULT_BOLD;//createFromAsset(Launcher4A.instance.getContext().getAssets(), "fonts/"+fontName+"bd.ttf");
			...
			try {
				new File(Environment.getExternalStorageDirectory() + "/" +
						 outName).delete();   // delete if it exists
			} catch (Exception e) {}
	...
	private void convertFont(Vector<TCZ.Entry> v, Typeface ttf, int size, String fileName,
							 Vector<Range> newRanges,
							 boolean isMono) {
	...
	static class PalmFont {
		...
		public void initTables() {
			rowWidthInBytes = 2 * rowWords * (antialiased == AA_NO ? 1 : antialiased == AA_4BPP ? 4 :
