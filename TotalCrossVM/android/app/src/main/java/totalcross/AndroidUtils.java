...
public class AndroidUtils
{
    ...
    public Config()
    {
      try {
        FileInputStream f = new FileInputStream(
          getRealPath(pinfo.applicationInfo.dataDir) + "/config.bin");
  ...
  public static class StartupTask extends AsyncTask<Object, Integer, Integer>
  {
    ...
    private void initDialog() { publishProgress(0); }
  ...
  {
    try {
      System.load(getRealPath(context.getApplicationInfo().nativeLibraryDir) +
                  "/libtcvm.so");
    } catch (Throwable e1) {
      handleException(e1, false);

      try {
        // to bypass problems of getting access to a file, we create files and
        // folders natively, where we can specify the file attributes.
        String sharedId = pinfo.sharedUserId;
        String tczname = sharedId.substring(sharedId.lastIndexOf('.') + 1);
        System.load(getRealPath("/data/data/totalcross." + tczname + "/lib") +
                    "/libtcvm.so"); // for single apk
      } catch (Throwable e2) {
        handleException(e2, false);

        try {
          System.load(getRealPath("/data/data/totalcross.android/lib") +
                      "/libtcvm.so");
  ...
  public static void checkInstall(Context context, StartupTask task)
    throws Exception
  {
    loadTCVM(context);
  ...
  public static void updateInstall(StartupTask task) throws Exception
  {
    ...
    while ((ze = zis.getNextEntry()) != null) {
      ...
      if (name.endsWith(".tcz")) { // tcz files are never unpacked
        ...
        try {
          if (f.exists()) {
            f.delete(); // delete old tcz files
            debug("deleted old " + dataDir + "/" + name);
          }
        } catch (Exception e) {
        }
        continue;
    ...
    if (fim - ini > 2000) {
      debug("Installation elapsed " + (fim - ini) + " ms");
    }
  }
  ...
  /**
   * Shows a dialog with the error and, optionally exits the application when
   * dismissed.
   */
  ...
  public static void debug(String s) { Log.i("TotalCross", s); }

  private static final String VM_PARAMS =
    getRealPath("/data/data/totalcross.android") +
    "/launcher.params"; // guich@tc127_71: use / instead of \
  ...
  public static void writeVMParameters(Hashtable<String, String> ht)
  {
    try {
      DataOutputStream dos =
        new DataOutputStream(new FileOutputStream(VM_PARAMS));
  ...
  {
    return argl.endsWith(".png") || argl.endsWith(".jpg") ||
      argl.endsWith(".jpeg");
  }
  ...
  public static void copyStreamToFile(InputStream inputStream, String out)
    throws IOException
  {
