...
public class TCEventThread extends Thread
{
  ...
  {
    setPriority(
      Thread.MAX_PRIORITY); // event thread should have maximum priority
    setDaemon(true);
    start();
  }
  ...
  boolean eventAvailable() { return eventQueue.getSize() > 0; }
  ...
  void privatePumpEvents()
  {
    ...
    final TCEvent event = popTime <= 0 ? (TCEvent)eventQueue.pop()
                                       : (TCEvent)eventQueue.popWait(popTime);
    if (event != null) {
      if (event.type == INVOKE_IN_EVENT_THREAD) {
        event.r.run();
        // If they are waiting for this, then notify.
        if (event.synch != null) {
          synchronized (event.synch) { event.synch.notify(); }
        }
      } else {
        win._postEvent(event.type,
                       event.key,
                       event.x,
                       event.y,
                       event.modifiers,
  ...
  public void invokeInEventThread(boolean wait, Runnable r)
  {
    if (!wait) {
      ...
      // avoiding a deadlock if invokeInEventThread() was called from an event
      // handler.
  ...
  private class Node
  {
    ...
    Node(Object obj) { o = obj; }
  }

  /**
   * Implements a simple FIFO queue of unlimited size.  This queue is thread
   * safe, including
   * functionality that allows one thread to wait for another to push an object
   * onto the queue.
   */
  private class Queue
  {
    ...
    /**
     * Returns the oldest object in the queue or null if no objects in the
     * queue
     */
    ...
    /**
     * Returns the oldest object in the queue.  If there are no object in the
     * queue, then this method will block until an object is placed in the queue
     * from another thread, or the specified time expires.  If the time expires
     * and no object has been placed in the queue, this method will return null.
     */
    synchronized Object popWait(int maxTime)
    {
      ...
      try {
        ...
        long timeLeft;
        while (queue == null &&
               (timeLeft = (maxTime - (System.currentTimeMillis() - begin))) >
                 0) {
          wait(timeLeft);
        }
      } catch (Exception ie) {
    ...
    synchronized int getSize() { return size; }
  }
