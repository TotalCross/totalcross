...
final public class Launcher4A extends SurfaceView implements SurfaceHolder.Callback, MainClass,
	OnKeyListener {
			...
			try {
				...
				switch (b.getInt("type")) {
					...
					case GPSFUNC_STOP:
						...
						break;
					case CELLFUNC_START:
						telephonyManager = (TelephonyManager) instance.getContext().getSystemService(
											   Context.TELEPHONY_SERVICE);
						CellLocation.requestLocationUpdate();
						telephonyManager.listen(phoneListener = new PhoneListener(),
												PhoneStateListener.LISTEN_CELL_LOCATION | PhoneStateListener.LISTEN_SIGNAL_STRENGTH);
						break;
					case CELLFUNC_STOP:
	...
	public Launcher4A(Loader context, String tczname, String appPath, String cmdline,
		...
		setWillNotDraw(
			true); // MUST be set to false to allow onDraw to be executed, we set to true and draw using opengl
		setWillNotCacheDrawing(true);
		setFocusableInTouchMode(true);
		requestFocus();
		setOnKeyListener(this);
		Configuration config = getResources().getConfiguration();
		hardwareKeyboardIsVisible = config.hardKeyboardHidden == Configuration.HARDKEYBOARDHIDDEN_NO
									|| config.keyboard ==
									Configuration.KEYBOARD_QWERTY; // motorola titanium returns HARDKEYBOARDHIDDEN_YES but KEYBOARD_QWERTY. In soft inputs, it returns KEYBOARD_NOKEYS
	...
	public void surfaceChanged(final SurfaceHolder holder, int format, int w, int h) {
		...
		if (h < lastScreenH && Loader.isFullScreen && lastScreenH == screenHeight
			&& appTitleH ==
			0) { // 1: surfaceChanged. 0 -> 480. displayH: 480  =>  2: surfaceChanged. 480 -> 455. displayH: 480
	...
	private void sendScreenChangeEvent() {
		...
		eventThread.invokeInEventThread(false, new Runnable() {
			public void run() {
				...
				_postEvent(SCREEN_CHANGED, lastScreenW, lastScreenH, (int)(metrics.xdpi + 0.5),
						   (int)(metrics.ydpi + 0.5), deviceFontHeight);
	...
	class MyInputConnection extends BaseInputConnection {
		...
		public boolean performPrivateCommand(java.lang.String action, android.os.Bundle data) {
			if (action != null
				&& action.contains("hideSoftInputView")) { // for LG's special "hide keyboard" key
				sendCloseSIPEvent();
				sipVisible = false;
			}
	...
	private boolean isSamsungKeyboard() {
		final String currentKeyboard =  Settings.Secure.getString(loader.getContentResolver(),
	...
	public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
		...
		outAttrs.imeOptions = EditorInfo.IME_ACTION_DONE |
							  0x2000000/*EditorInfo.IME_FLAG_NO_FULLSCREEN*/; // the NO_FULLSCREEN flag fixes the problem of keyboard not being shifted correctly in android >= 3.0
		outAttrs.inputType = !wasNumeric ? InputType.TYPE_NULL : InputType.TYPE_CLASS_NUMBER |
	...
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		...
		if (keyCode ==
			KeyEvent.KEYCODE_BACK) { // guich@tc130: if the user pressed the back key on the SIP, don't pass it to the application
		...
		switch (event.getAction()) {
			...
			case KeyEvent.ACTION_DOWN:
				...
				if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN || keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
					eventThread.pushEvent(KEY_PRESS, 0, keyCode == KeyEvent.KEYCODE_VOLUME_UP ? -1000 : -1001, 0,
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		} else if (type != lastType || x != lastX || y != lastY) { // skip identical events
			lastType = type;
			lastX = x;
			lastY = y;
	...
	public static void alert(String msg) { // if called from android package classes, must pass FALSE
		...
		loader.runOnUiThread(new Runnable() {
			public void run() { // guich@tc125_15: use a native dialog box
	...
	public native static void nativeSmsReceived(String displayOriginatingAddress,
			String displayMessageBody, byte[] userData);
	...
	public static void setSIP(int sipOption, boolean numeric) {
		InputMethodManager imm = (InputMethodManager) instance.getContext().getSystemService(
		...
		switch (func) {
			...
			case TURN_SCREEN_ON: {
				if (v == 1 && oldBrightness != 0) { // turn on again?
					if (Settings.System.putInt(loader.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS,
											   oldBrightness)) {
						return 1;
					}
				} else if (v == 0) {
					oldBrightness = Settings.System.getInt(loader.getContentResolver(),
	...
	private static double[] getGeocode(String address) {
		try {
			...
			append("http://maps.googleapis.com/maps/api/geocode/xml?address=").append(address.replace("  ",
					" ").replace(' ', '+')).append("&sensor=false");
			//AndroidUtils.debug(urlString.toString());
			HttpURLConnection urlConnection = (HttpURLConnection) new URL(
												  urlString.toString()).openConnection();
	...
	private static boolean isCallable(Intent intent) {
		return loader.getPackageManager().queryIntentActivities(intent,
				PackageManager.MATCH_DEFAULT_ONLY).size() > 0;
	}

	public static int showRoute(String addressI, String addressF, String coords,
		...
		do {
			try {
				if ((flags & Loader.USE_WAZE) != 0
					&& !isCallable(new Intent(Intent.ACTION_VIEW, Uri.parse("waze://?ll=0,0&navigate=yes")))) {
					return -2;
				}
				...
				if (tryAgain) {
					...
					AndroidUtils.debug("Internet out of range. Trying again to get location (" + tryAgainCount +
									   " of 5)");
				}
			}
		} while (tryAgain);
	...
	public static void appPaused() {
		appPaused = true;
		instance.nativeInitSize(null, -998,
		...
		try {
			...
			while ((ze = zis.getNextEntry()) != null) {
				...
				if (name.equals("assets/tcfiles.zip")) {
					...
					byte[] buf = new byte[Math.max(4096,
												   sis.maxLength * 2)]; // we use a bigger buffer to avoid having a tcz name cut into 2 parts
					...
					for (int i = base, n = tczs.size(); i < n; i++) {
						...
						for (int j = 10;;) {
							...
							if (realOfs == -1) { // if not found, double the read buffer and try again
								...
								AndroidUtils.debug("read from " + pos + " to " + (pos + buf.length) +
	...
	public static int readTCZ(int fileIdx, int ofs, byte[]
							  buf) { // ofs is the relative offset inside the tcz file, not the absolute apk file position
		synchronized (tczs) {
			try {
	...
	public static void soundPlay(String filename) {
		...
		if (ring ==
			AudioManager.RINGER_MODE_NORMAL) { // 4.4 does not returns correct values for volume methods
			...
			for (int i = 0; lastSoundID > 0 && player.play(lastSoundID, volume, volume, 0, 0, 1.0f) == 0
				 && i++ < 10;)
				try {
					Thread.sleep(50);
				} catch (Exception e) {}
