...
final public class Launcher4A extends SurfaceView
  implements SurfaceHolder.Callback, MainClass, OnKeyListener
{
  ...
  static Hashtable<String, String> htPressedKeys =
    ...
    public void handleMessage(Message msg)
    {
      try {
        ...
        switch (b.getInt("type")) {
          ...
          case CELLFUNC_START:
            telephonyManager =
              (TelephonyManager)instance.getContext().getSystemService(
          ...
          case CLIPBOARD: {
            if (clip == null) {
              clip = (android.text.ClipboardManager)loader.getSystemService(
  ...
  public Launcher4A(Loader context,
                    String tczname,
                    String appPath,
                    String cmdline,
                    boolean isSingleAPK)
  {
    ...
    setWillNotDraw(true); // MUST be set to false to allow onDraw to be
                          // executed, we set to true and draw using opengl
    setWillNotCacheDrawing(true);
    setFocusableInTouchMode(true);
    requestFocus();
    setOnKeyListener(this);
    Configuration config = getResources().getConfiguration();
    hardwareKeyboardIsVisible =
      config.hardKeyboardHidden == Configuration.HARDKEYBOARDHIDDEN_NO ||
      config.keyboard ==
        Configuration
          .KEYBOARD_QWERTY; // motorola titanium returns
                            // HARDKEYBOARDHIDDEN_YES but KEYBOARD_QWERTY.
                            // In soft inputs, it returns KEYBOARD_NOKEYS
    lastOrientation = getOrientation();
    String vmPath = context.getApplicationInfo().dataDir;
    initializeVM(context, tczname, appPath, vmPath, cmdline);
    if (GENERATE_FONT) {
      new totalcross.android.fontgen.FontGenerator(
        "tahoma", new String[] { "", "/aa", "/rename:TCFont" });
      exit(0);
    }
  }

  public static Context getAppContext() { return instance.getContext(); }
  ...
  private static class PhoneListener extends PhoneStateListener
  {
    public void onCellLocationChanged(CellLocation location)
    {
      ...
      try {
        ret[0] = Integer.parseInt(operator.substring(0, 3)); // mcc
      } catch (NumberFormatException nfe) {
        ret[0] = 0;
      }
      try {
        ret[1] = Integer.parseInt(operator.substring(3)); // mnc
      } catch (NumberFormatException nfe) {
        ret[1] = 0;
      }
                             ...
                             int h)
  {
    ...
    if (h < lastScreenH && Loader.isFullScreen && lastScreenH == screenHeight &&
        appTitleH == 0) { // 1: surfaceChanged. 0 -> 480. displayH: 480  =>  2:
                          // surfaceChanged. 480 -> 455. displayH: 480
      appTitleH = lastScreenH - h;
    }

    if (sipVisible && !rotated) { // sip changed and no rotation occured?
      instance.nativeInitSize(null,
                              -999,
                              h); // signal vm that the keyboard will appear
    } else {
      instance.nativeInitSize(null,
                              -999,
                              0); // signal vm that the keyboard will hide
      android.view.Surface surface =
        holder == null ? lastSurface : holder.getSurface();
  ...
  private void sendScreenChangeEvent()
  {
    ...
    eventThread.invokeInEventThread(false, new Runnable() {
      public void run()
      {
        ...
        deviceFontHeight = BigDecimal.valueOf(defaultTextSize)
                             .divide(BigDecimal.valueOf(metrics.density),
                                     0,
    ...
    extends ScaleGestureDetector.SimpleOnScaleGestureListener
  {
    ...
    {
      multiTouching = true;
      if (lastWasPenDown) { // if pressed a button and started multitouch, issue
                            // a penup event so button can be released
  ...
  public void surfaceDestroyed(SurfaceHolder holder)
  {
    instance.nativeInitSize(null,
                            0,
                            0); // signal vm that the surface will change
  }
  ...
  class MyInputConnection extends BaseInputConnection
  {
    ...
    public boolean performPrivateCommand(java.lang.String action,
                                         android.os.Bundle data)
    {
      if (action != null &&
          action.contains(
            "hideSoftInputView")) { // for LG's special "hide keyboard" key
        sendCloseSIPEvent();
        sipVisible = false;
      }
    ...
    {
      return "aaaaaa"; // used in TC8000, when user try to do backspace on an
                       // Edit that has text but nothing was typed yet. Android
                       // thinks that there is no characters before the cursor
                       // and does not issue the backspace key.
    }
  }

  private boolean isSamsungKeyboard()
  {
    final String currentKeyboard = Settings.Secure.getString(
      loader.getContentResolver(), Settings.Secure.DEFAULT_INPUT_METHOD);
  ...
  {
    // outAttrs.inputType =
    // android.text.InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS; - this makes
    // android's fullscreen keyboard appear in landscape
    outAttrs.imeOptions = EditorInfo.IME_ACTION_DONE |
                          0x2000000 /*EditorInfo.IME_FLAG_NO_FULLSCREEN*/
      ; // the NO_FULLSCREEN flag fixes the problem of keyboard not being
        // shifted correctly in android >= 3.0
    outAttrs.inputType = !wasNumeric ? InputType.TYPE_NULL
  ...
  public boolean onKey(View v, int keyCode, KeyEvent event)
  {
    ...
    if (keyCode ==
        KeyEvent
          .KEYCODE_BACK) { // guich@tc130: if the user pressed the back key on
                           // the SIP, don't pass it to the application
    ...
    switch (event.getAction()) {
      ...
      case KeyEvent.ACTION_DOWN:
        ...
        if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN ||
          ...
          eventThread.pushEvent(KEY_PRESS,
                                0,
                                keyCode == KeyEvent.KEYCODE_VOLUME_UP ? -1000
                                                                      : -1001,
                                0,
                                event.getMetaState(),
                                0);
        ...
        if ((state & KeyEvent.META_ALT_ON) != 0 &&
            (c = event.getNumber()) != 0) { // handle ALT before pushing event
          keyCode = c;
          state &= ~KeyEvent.META_ALT_ON;
        } else {
        ...
        if (str != null) {
          ...
          for (int i = 0; i < chars.length; i++) {
            eventThread.pushEvent(
              KEY_PRESS, chars[i], 0, 0, event.getMetaState(), 0);
          }
        }
        break;
    }
  ...
  public boolean onTouchEvent(MotionEvent event)
  {
    ...
    if (multiTouching) { // we do this to ignore all touch events until a
                         // definitive pen_up
      if (type == PEN_UP) {
        scaleList.onScaleEnd();
      }
    } else if (type != lastType || x != lastX ||
               y != lastY) { // skip identical events
      lastType = type;
      lastX = x;
      lastY = y;
  ...
  // 1. when the program calls MainWindow.exit, exit below is called before
  // stopVM
  // 2. when the vm is stopped because another program will run, stopVM is
  // called before exit. so, we just have to wait (canQuit=false) in
  // situation 2.
  ...
  { // if called from android package classes, must pass FALSE
    showingAlert = true;
    AndroidUtils.debug(msg);
    final String _msg = msg;
    loader.runOnUiThread(new Runnable() {
      public void run()
      { // guich@tc125_15: use a native dialog box
        new AlertDialog.Builder(loader)
          ...
          .setPositiveButton("Close",
                             new DialogInterface.OnClickListener() {
                               ...
                               {
                                 //               updateScreen(0,0,instance.getWidth(),instance.getHeight(),
                                 //               TRANSITION_NONE);
                                 showingAlert = false;
                               }
                             })
  ...
  native void initializeVM(Context context,
                           String tczname,
                           String appPath,
                           String vmPath,
                           String cmdline);
  public native void nativeInitSize(Surface surface, int w, int h);
  native void nativeOnEvent(int type,
                            int key,
                            int x,
                            int y,
                            int modifiers,
                            int timeStamp);
  public native static void nativeSmsReceived(String displayOriginatingAddress,
                                              String displayMessageBody,
                                              byte[] userData);
  public native static void nativeOnMessageReceived(String messageId,
                                                    String messageType,
                                                    String[] keys,
                                                    String[] values,
                                                    String collapsedKey,
  ...
  // implementation of interface MainClass. Only the _postEvent method is ever
  // called.
  ...
  {
    public SipClosedReceiver() { super(null); }
  ...
  {
    public void run() { sendCloseSIPEvent(); }
  }
  ...
  public static boolean getSIP() { return sipVisible; }

  public static void setSIP(int sipOption, boolean numeric)
  {
    InputMethodManager imm =
      (InputMethodManager)instance.getContext().getSystemService(
      ...
      case SIP_HIDE:
        ...
        if (Loader.isFullScreen) {
          ...
          imm.hideSoftInputFromWindow(
            instance.getWindowToken(), 0, instance.siprecv);
        }
        break;
      case SIP_SHOW:
      case SIP_TOP:
  ...
  public static int getAppHeight() { return instance.getHeight(); }
  ...
  public static int vmFuncI(int func, int v) throws Exception
  {
    switch (func) {
      case VIBRATE: {
        if (vibrator == null) {
          vibrator = (Vibrator)instance.getContext().getSystemService(
      ...
      case REBOOT_DEVICE: {
        // seems that will not work, because the app has to be signed with the
      ...
      case GET_REMAINING_BATTERY: {
        Intent bat = loader.registerReceiver(
      ...
      case TURN_SCREEN_ON: {
        if (v == 1 && oldBrightness != 0) { // turn on again?
          if (Settings.System.putInt(loader.getContentResolver(),
          ...
          oldBrightness = Settings.System.getInt(
            loader.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS);
          if (Settings.System.putInt(loader.getContentResolver(),
                                     Settings.System.SCREEN_BRIGHTNESS,
                                     0)) {
            return 1;
  ...
  {
    if (tonegen == null) {
      soundEnable(true); // at the first time, enable the sound or the guy may
                         // not hear it
    }
    ...
    new Thread() {
      ...
      {
        if (tonegen == null) {
          tonegen = new ToneGenerator(AudioManager.STREAM_MUSIC,
  ...
  {
    AudioManager am = (AudioManager)instance.getContext().getSystemService(
      Context.AUDIO_SERVICE);
    if (oldRingerMode == -1) { // save it so it can be recovered when vm quits
      oldRingerMode = am.getRingerMode();
    }
    am.setRingerMode(enable ? AudioManager.RINGER_MODE_NORMAL
  ...
  private static void restoreSound()
  {
    if (oldRingerMode != -1) {
      AudioManager am =
        (AudioManager)loader.getSystemService(Context.AUDIO_SERVICE);
      am.setRingerMode(oldRingerMode);
      oldRingerMode = -1;
    }
  }

  public static int vmExec(String command,
                           String args,
                           int launchCode,
                           boolean wait)
  {
    ...
    if (command.equals("viewer")) {
      ...
      if ((AndroidUtils.isImage(argl) || argl.endsWith(".pdf")) &&
          !new java.io.File(args).exists()) {
        return -2;
      }
    }
    Message msg = loader.achandler.obtainMessage();
  ...
  {
    try {
      Method getExternalFilesDir = Context.class.getMethod(
        "getExternalFilesDirs", new Class[] { String.class });
      File[] ff =
        (File[])getExternalFilesDir.invoke(loader, new Object[] { null });
      if (ff != null) {
        if (i < 0 || i >= ff.length) {
          return null;
        }
  ...
  private static double[] getLatLon(String address) throws IOException
  {
    if (address.equals("")) { // last known location?
      ...
      LocationManager loc =
        (LocationManager)loader.getSystemService(Context.LOCATION_SERVICE);
      ...
      for (String p : pros) {
        ...
        if (position != null)
          return new double[] { position.getLatitude(),
                                position.getLongitude() };
      }
    } else if (address.startsWith("@")) {
      String[] st = address.substring(1).split(",");
      return new double[] { Double.parseDouble(st[0]),
    ...
    try {
      ...
      StringBuilder urlString =
        new StringBuilder(128)
          .append("http://maps.googleapis.com/maps/api/geocode/xml?address=")
          .append(address.replace("  ", " ").replace(' ', '+'))
          .append("&sensor=false");
      // AndroidUtils.debug(urlString.toString());
      HttpURLConnection urlConnection =
        (HttpURLConnection) new URL(urlString.toString()).openConnection();
  ...
  {
    return loader.getPackageManager()
             .queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY)
             .size() > 0;
  }

  public static int showRoute(String addressI,
                              String addressF,
                              String coords,
                              int flags) throws IOException
  {
    ...
    do {
      try {
        if ((flags & Loader.USE_WAZE) != 0 &&
            !isCallable(new Intent(Intent.ACTION_VIEW,
                                   Uri.parse("waze://?ll=0,0&navigate=yes")))) {
          return -2;
        }
        ...
        double[] llF =
          addressF == null ? new double[] { 0, 0 } : getLatLon(addressF);
        if (llI != null && llF != null) {
          AndroidUtils.debug(llI[0] + "," + llI[1] + " - " + llF[0] + "," +
                             llF[1]);
          // call the loader
          showingMap = true;
          Message msg = loader.achandler.obtainMessage();
          ...
          throw new Exception(
            tryAgainCount + " parse response for address"); // make it try again
        }
      } catch (Exception e) {
        AndroidUtils.handleException(e, false);
        String msg = e.getMessage();
        tryAgain = msg != null && msg.indexOf("parse response") >= 0 &&
                   ++tryAgainCount <= 5; // Unable to parse response from server
        if (tryAgain) {
          try {
            Thread.sleep(250);
          } catch (Exception ee) {
          ...
          AndroidUtils.debug(
            "Internet out of range. Trying again to get location (" +
            tryAgainCount + " of 5)");
        }
      }
    } while (tryAgain);
    ...
    do {
      try {
        ...
        if (ll != null) {
          ...
          throw new Exception(tryAgainCount + " parse response for address " +
                              address); // make it try again
        }
      } catch (Exception e) {
        AndroidUtils.handleException(e, false);
        String msg = e.getMessage();
  ...
  {
    appPaused = true;
    instance.nativeInitSize(
      null,
      -998,
      0); // signal vm to delete the textures while the context is valid
    if (eventThread != null) {
      setSIP(SIP_HIDE, false);
  ...
  public static void appResumed()
  {
    ...
    instance.nativeInitSize(null,
                            -997,
                            0); // signal vm to invalidate the textures
    if (eventThread != null) {
  ...
  // an InputStream that keeps track of position, since RandomAccessFile can't
  // be used with ZipInputStream
      ...
      while ((ze = zis.getNextEntry()) != null) {
        ...
        if (name.equals("assets/tcfiles.zip")) {
          ...
          while ((tcze = tcz.getNextEntry()) != null) {
            ...
            e.name = name;
            e.ofs = sis.pos; // note: this offset is just a guess (upper limit),
                             // since the stream is read in steps. later we'll
                             // find the correct offset
            e.len = tcze.getSize();
            tczs.add(e);
            // AndroidUtils.debug(e.name+" ("+e.len+" bytes) - temp pos:
            // "+e.ofs);
          }
          tcz.close();
          zis.close();
          sis.close();
          // now we open the file again, with random access, and search for the
          // filenames to get the correct offset to them
          byte[] buf = new byte[Math.max(
            4096,
            sis.maxLength * 2)]; // we use a bigger buffer to avoid having a
                                 // tcz name cut into 2 parts
          ...
          for (int i = base, n = tczs.size(); i < n; i++) {
            ...
            for (int j = 10;;) {
              ...
              if (realOfs ==
                  -1) { // if not found, double the read buffer and try again
                if (--j == 0) {
                  raf.close();
                  throw new RuntimeException(
                    "Error: cannot find real offset in APK for " + a.name);
                }
                buf = new byte[buf.length * 2];
                AndroidUtils.debug(
                  "read from " + pos + " to " + (pos + buf.length) +
                  " but not found. doubling buffer to " + buf.length + "!");
              } else {
                a.ofs =
                  pos + realOfs + what.length; // the data comes after the name
    ...
    for (i = 0; i < len; i++)
      if (src[i] == b) { // first letter matches?
        ...
        for (j = 1, k = i + j; j < what.length && found; j++, k++) {
          found &= src[k] == what[j]; // ps: cannot use continue here since
                                      // we're inside 2 loops
        }
        if (found) {
  ...
  public static int readTCZ(int fileIdx, int ofs, byte[] buf)
  { // ofs is the relative offset inside the tcz file, not
    // the absolute apk file position
    synchronized (tczs)
    {
      try {
  ...
  public static String zxing(String mode)
  {
    ...
    try {
      ...
      e.printStackTrace();
      text = "***EXCEPTION"; // note: any tries to get the exception here will
                             // halt the vm (tested in sony ericsson xperia x1)
    }
    return text;
  }
  ...
  public static int getFreeMemory()
  {
    ...
    return mi.availMem > Integer.MAX_VALUE ? Integer.MAX_VALUE
                                           : (int)mi.availMem;
  }
  ///////////////// crash controller //////////////////////

  static boolean createBugreport()
  { // called by MainWindow using Vm.exec
    ...
    try {
      ...
      String bugreportfn = "/sdcard/IssueReport/bugreport" +
      ...
      if (l2 <= 512) { // ignore small reports
        ...
        for (int n; (n = fin.read(buf)) > 0;) {
          if (!containsUsefulInformation) {
            containsUsefulInformation =
              containsUsefulInformation(new String(buf, 0, n));
          }
          zout.write(buf, 0, n);
        }
        zout.closeEntry();
  ...
  private static boolean containsUsefulInformation(String s)
  {
    ...
    //      if (s.indexOf(">>> totalcross") >= 0)
    //         return true;
    //      if (s.indexOf("OutOfMemoryError") != -1 || s.indexOf("(tns") != -1
    //      || s.indexOf("(tnS_") != -1)
    //         return true;
    //      if (s.indexOf("fontDestroy") != -1 || s.indexOf("Unhandled
    //      exception") != -1)
    //         return true;
    //      if (s.indexOf("#00") >= 0)
    //      {
    ...
    //            return true;
    //         if (s.matches(".*(\\(strcmp\\))$")) // #00  pc 0000e108
  ...
  public static void soundPlay(String filename)
  {
    ...
    if (ring ==
        AudioManager.RINGER_MODE_NORMAL) { // 4.4 does not returns correct
                                           // values for volume methods
      ...
      for (int i = 0;
           lastSoundID > 0 &&
           player.play(lastSoundID, volume, volume, 0, 0, 1.0f) == 0 &&
           i++ < 10;)
        try {
          Thread.sleep(50);
        } catch (Exception e) {
  ...
  public static final PermissionHandler PHONE_STATE =
    new PermissionHandler(Loader.PermissionRequestCodes.READ_PHONE_STATE,
                          Manifest.permission.READ_PHONE_STATE);

  public static final PermissionHandler LOCATION =
    new PermissionHandler(Loader.PermissionRequestCodes.ACCESS_FINE_LOCATION,
                          Manifest.permission.ACCESS_FINE_LOCATION,
                          Manifest.permission.ACCESS_COARSE_LOCATION);

  public static final PermissionHandler STORAGE =
    new PermissionHandler(Loader.PermissionRequestCodes.EXTERNAL_STORAGE,
                          Manifest.permission.READ_EXTERNAL_STORAGE,
                          Manifest.permission.WRITE_EXTERNAL_STORAGE);

  public static final PermissionHandler CAMERA =
    new PermissionHandler(Loader.PermissionRequestCodes.CAMERA,
  ...
  public static class PermissionHandler
  {
    ...
    public static final HashMap<Integer, PermissionHandler>
      permissionHandlerMap = new HashMap<>();
