...
public class Stub extends Activity {
	...
	private void runApplication() throws Exception {
		...
		ht.put("fullscreen", "fullscreen:1".equals(getPackageManager().getApplicationInfo(getPackageName(),
				PackageManager.GET_META_DATA).metaData.getString("isFullScreen")) ? "true" :
		...
		int flags = Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS |
					Intent.FLAG_FROM_BACKGROUND; // clear_top is essential to make it work (otherwise, the last called app will be launched instead of the current one)
		boolean sameProgram = lastTCZ != null && tczName.equals(lastTCZ);
		AndroidUtils.debug(sameProgram ? "Program " + tczName + " being resumed" :
						   "Program change detected: " + lastTCZ + " -> " + tczName);
		...
		try {
			...
			if (imp == RunningAppProcessInfo.IMPORTANCE_VISIBLE || (!sameProgram
					&& imp == RunningAppProcessInfo.IMPORTANCE_BACKGROUND)) { // guich@tc123b: if the vm was already running...
