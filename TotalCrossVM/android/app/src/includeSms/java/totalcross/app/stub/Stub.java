...
public class Stub extends Activity
{
  ...
  {
    public LengthyTask(Activity activity) { super(activity); }
  ...
  private void runApplication() throws Exception
  {
    ...
    ht.put(
      "fullscreen",
      "fullscreen:1".equals(getPackageManager()
    ...
    int flags =
      ...
      Intent.FLAG_FROM_BACKGROUND; // clear_top is essential to make it work
                                   // (otherwise, the last called app will be
                                   // launched instead of the current one)
    boolean sameProgram = lastTCZ != null && tczName.equals(lastTCZ);
    AndroidUtils.debug(sameProgram ? "Program " + tczName + " being resumed"
                                   : "Program change detected: " + lastTCZ +
                                       " -> " + tczName);
    if (!sameProgram) { // if its not the same app that was launched, remove all
                        // other tasks
    ...
    try {
      ...
      if (imp == RunningAppProcessInfo.IMPORTANCE_VISIBLE ||
          (!sameProgram &&
           imp == RunningAppProcessInfo
                    .IMPORTANCE_BACKGROUND)) { // guich@tc123b: if the vm was
                                               // already running...
      ...
      AndroidUtils.error(
        "This program requires the TotalCross Virtual Machine to run. Please contact your software's vendor or download the DEMO version from\nwww.totalcross.com/install\nwhich will run during 80 hours.",
        true);
    }
    System.exit(99);
  }
