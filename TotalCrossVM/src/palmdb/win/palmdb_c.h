...
bool PDBCreateFile(TCHARP fullPath, bool createIt, bool readOnly, PDBFileRef* fileRef) {
	return (*fileRef = CreateFile(fullPath,
								  ...
								  FILE_FLAG_RANDOM_ACCESS, // guich@555_9: with FILE_FLAG_WRITE_THROUGH, queries are 50ms faster, but without it, table fillup is 48% faster!
...
bool PDBReadAt(PDBFileRef fileRef, VoidP buf, int32 size, int32 offset, int32* read) {
	...
	off.LowPart = offset;
	return (SetFilePointerEx(fileRef, off, &cur, FILE_BEGIN) != 0) ? PDBRead(fileRef, buf, size,
			read) : false;
#else
	return (SetFilePointer(fileRef, offset, null, FILE_BEGIN) != 0xFFFFFFFFL) ? PDBRead(fileRef, buf,
			size, read) : false;
#endif
}
...
bool PDBWriteAt(PDBFileRef fileRef, VoidP buf, int32 size, int32 offset, int32* written) {
	...
	LARGE_INTEGER off = { 0 }, cur;
	off.LowPart = offset;
	return (SetFilePointerEx(fileRef, off, &cur, FILE_BEGIN) != 0) ? PDBWrite(fileRef, buf, size,
			written) : false;
#else
	return (SetFilePointer(fileRef, offset, null, FILE_BEGIN) != 0xFFFFFFFFL) ? PDBWrite(fileRef, buf,
			size, written) : false;
#endif
}
...
#else
	return (SetFilePointer(fileRef, oldSize + growSize, null,
						   FILE_BEGIN) != 0xFFFFFFFFL) ? SetEndOfFile(fileRef) : false;
#endif
}
