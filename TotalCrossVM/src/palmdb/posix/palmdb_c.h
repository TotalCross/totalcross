...
{
  return (int32)fseek(fileRef, offset, SEEK_SET) == 0 &&
         (*read = (int32)fread(buf,
                               1,
                               size,
                               fileRef)) >
           0; // guich@tc122_53: don't test for the amount of bytes read - no
              // other platforms do this, and will make it impossible to
              // delete an empty pdb
}
...
{
  return (int32)fseek(fileRef, offset, SEEK_SET) == 0 &&
         (*written = (int32)fwrite(buf, 1, size, fileRef)) >
           0; // guich@tc122_53
}
  ...
  if (!stopSearch && recursive) {
    ...
    while (!stopSearch && (entry = readdir(targetDirectory))) {
      ...
      if (isDir && ((name[0] != '.') ||
                    ((name[1] != '\0') &&
                     ((name[1] != '.') ||
                      (name[2] != '\0'))))) { /* warning: order matters! */
