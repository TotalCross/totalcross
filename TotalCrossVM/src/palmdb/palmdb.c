...
static MemHandle getRecord(PDBFile db, uint16 index, CharP buf, int32 ofs, uint32* length,
						   bool queryOnly);
...
Err myDmRecordInfo(DmOpenRef dbP, uint16 index, uint16* attrs) {
	int32 read;
	return !PDBReadAt(((PDBFile) dbP)->fh, attrs, 1, GET_OFFSET(index) + 4,
					  &read) ? PDBGetLastErr() : errNone;
}
...
Err myDmReleaseRecord(DmOpenRef dbP, int32 index, bool dirty) {
	...
	if (isNew || (attr & dmRecAttrBusy)) {
		...
		if (size != originalSize || isNew) {
			...
			if (isNew) {
				shiftOffsets(db, 0, appending ? index : (index + 1),
							 8);    // since we inserted a new index, shift the record offsets 8 bytes down // guich@330_2: without the "appending?:", creating an empty catalog, adding one record and closing it, makes the first record have an offset of 96 instead of 88.
			}
			if (!appending) {
...
Err myDmCloseDatabase(DmOpenRef dbP) {
	...
	if (db->lockedRecords <= 0) {
		dbh.attributes &=
			~dmHdrAttrOpen;    // flsobral@120_30: reset the open flag only if all records are unlocked - guich@tc123_17: save in the correct structure (local dbh variable, not db->dbh!)
	}
...
VoidP myDmFindDatabase(Context currentContext, TCHARP fileName, PDBFile dbId) {
	...
	pdbFound = PDBCreateFile(filePath, false, true,
...
DmOpenRef myDmOpenDatabase(VoidP dbId, uint16 mode) {
	...
	if (!(isFileOpen = PDBCreateFile(db->fullPath, false, false,
...
static MemHandle getRecord(PDBFile db, uint16 index, CharP buf, int32 ofs, uint32* length,
	...
	if (index !=
		db->recordCachedPos) { // guich@550: do a little cache - alltests gets above 15000 cache hits, benchpdbdriver above 50000
	...
	if (length == null) {
		...
		if (*length == 0) {
			*length = db->recordCachedSize;
			...
			size = *length = (db->recordCachedSize < *length) ? db->recordCachedSize :
	...
	PDBReadAt(db->fh, memHandle + sizeof(uint32), size, db->recordCachedHeader.offset + ofs,
			  &bytesRW); // can't change to READ_SIZE_AT bc the macro uses & in the pointer, corrupting memory
