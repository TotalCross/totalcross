...
static void
destroyRecPtrs(PDBFile db);
static void
swapDatabaseHeader(DatabaseHeader* src, DatabaseHeader* dst);
static RecordList*
retrieveRecList(PDBFile db, uint16 index);
static void
shiftFile(PDBFile db, int32 start, int32 end, int32 len);
static void
shiftOffsets(PDBFile db, int32 start, int32 end, int32 delta);
static int32
getRecordOffset(PDBFile db, int32 index);
static bool
destroyRecPtr(PDBFile db, uint16 index);
static MemHandle
resizeBuffer(PDBFile db, uint32 size);
static bool
storeRecPtr(PDBFile db, CharP mh, uint16 index, uint16 size);
static MemHandle
getRecord(PDBFile db,
          uint16 index,
          CharP buf,
          int32 ofs,
          uint32* length,
          bool queryOnly);
static int32
getRecordSize(PDBFile db, uint16 index);
...
Err
myDmRecordInfo(DmOpenRef dbP, uint16 index, uint16* attrs)
{
  int32 read;
  return !PDBReadAt(((PDBFile)dbP)->fh, attrs, 1, GET_OFFSET(index) + 4, &read)
           ? PDBGetLastErr()
           : errNone;
}

Err
myDmSetRecordInfo(DmOpenRef dbP, uint16 index, uint16 attrs)
{
  PDBFile db = (PDBFile)dbP;
  int32 written;

  ++db->dbh.modificationNumber;
  return !PDBWriteAt(db->fh, &attrs, 1, GET_OFFSET(index) + 4, &written)
           ? PDBGetLastErr()
           : errNone;
}

Err
myDmReleaseRecord(DmOpenRef dbP, int32 index, bool dirty)
{
  PDBFile db = (PDBFile)dbP;
  uint16 attr = 0;
  bool isNew;
  ...
  if (isNew || (attr & dmRecAttrBusy)) {
    ...
    if (index == db->recordIndex) { // are we releasing the current record that
                                    // is being written?
    ...
    if (size != originalSize || isNew) {
      ...
      growingSize = isNew ? (size + ((db->fileSize == 82) ? 6 : 8))
                          : (size - originalSize); // guich@550
      ...
      // 2. if not appending, insert space (or remove space) for the new record,
      // shifting the file to the right
      if (!appending) {
        ...
        if (growingSize > 0) {
          if (isNew) {
            ...
            shiftFile(
              db, pos2 + originalSize, db->fileSize - growingSize, growingSize);
          }
        } else {
      ...
      // 5. now shift the offset of all records in the offsets table after this
      // index
      if (isNew) {
        shiftOffsets(
          db,
          0,
          appending ? index : (index + 1),
          8); // since we inserted a new index, shift the record offsets 8
              // bytes down // guich@330_2: without the "appending?:",
              // creating an empty catalog, adding one record and closing it,
              // makes the first record have an offset of 96 instead of 88.
      }
      if (!appending) {
        shiftOffsets(db,
                     index + 1,
                     isNew ? (count + 1) : count,
                     growingSize); // move the others down
      }
...
Err
myDmCloseDatabase(DmOpenRef dbP)
{
  PDBFile db = (PDBFile)dbP;
  DatabaseHeader dbh;
  int32 bytesRW;
  ...
  if (db->recordIndex !=
      -1) { // flsobral@120_30: make sure all records are released.
  ...
  if (db->lockedRecords <= 0) {
    dbh.attributes &=
      ~dmHdrAttrOpen; // flsobral@120_30: reset the open flag only if all
                      // records are unlocked - guich@tc123_17: save in the
                      // correct structure (local dbh variable, not db->dbh!)
  }
...
bool
PDBMatchExact(TCHARP filePath, VoidP userVars)
{
  FileMatches_Vars* vars = (FileMatches_Vars*)userVars;
  DatabaseHeader dbh;
  int32 bytesRW;

  if (!PDBCreateFile(filePath,
                     false,
                     true,
                     &vars->fileRef)) { // createIt - false, readOnly - true
    return false;
  }
...
myDmFindDatabase(Context currentContext, TCHARP fileName, PDBFile dbId)
{
  ...
  pdbFound =
    PDBCreateFile(filePath,
                  false,
                  true,
                  &userVars.fileRef); // createIt - false, readOnly - true
  ...
  if (!pdbFound) {
    ...
    if (getDataPath(dataPathBuf) && *dataPathBuf != 0) {
      ...
      PDBListDatabasesIn(
        fullPath, true, (HandlePDBSearchProcType)PDBMatchExact, &userVars);
...
Err
myDmCreateDatabase(TCHARP fileName, uint32 creator, uint32 type, bool isResDB)
{
  ...
  if (!PDBCreateFile(fullPath,
                     true,
                     false,
                     &fileRef)) { // createIt - true, readOnly - false
    return PDBGetLastErr();
  }
  ...
  if (!PDBWrite(fileRef,
                &pad,
                2,
                &bytesRW)) { // this goes after the table of record headers
    goto error;
  }
...
myDmOpenDatabase(VoidP dbId, uint16 mode)
{
  ...
  if (db->dbh.attributes & dmHdrAttrOpen) {
    ...
    while (count > 16) {
      ...
      for (i = 0; i < 16; i++)
        if (db->rhs[i].attr &
            dmRecAttrBusy) { // no need to swapRecordHeader here
...
MemHandle
myDmQueryRecord(DmOpenRef dbP, uint16 index)
{
  PDBFile db = (PDBFile)dbP;
  int32 currentIndex;
...
uint16
myDmNumRecords(DmOpenRef dbP)
{
  return ((PDBFile)dbP)->dbh.numRecords;
}

/** If the record is open, the MemHandle to it will be returned. If the record
is closed, it will be opened, but a non-zero (but invalid) value will be
returned */
MemHandle
myDmResizeRecord(DmOpenRef dbP, uint16 index, uint32 size)
{
  PDBFile db = (PDBFile)dbP;
  uint8* recPtrTemp;
  uint32 temp;
  ...
  if (db->recordIndex !=
      index) { // resizing a record got with getRecord? (updating a record?)
    rl = retrieveRecList(db, index);
    temp = size + sizeof(uint32);
    handleToBeReleased = null;
    if (rl == null) { // guich@552_25: if the record was not loaded, load and
                      // then release it instead of just returning an error
      handleToBeReleased = myDmGetRecord(dbP, index);
      if (!handleToBeReleased) {
        return null;
      }
      rl = retrieveRecList(db, index);
      if (rl == null) { // if failed for the second time, we have no options
                        // beside just return
        return null;
      }
    }
    ...
    if (handleToBeReleased) { // guich@552_25
      ...
      return (
        MemHandle)1; // just to tell that the record was resized with success
    }
...
myDmRemoveRecord(DmOpenRef dbP, uint16 index)
{
  ...
  // 2. if not removingFromEnd, remove the space of the current record, shifting
  ...
  // 3. records before this one only shift by the removed entry of the records
  // table
  ...
  if (!removingFromEnd) {
    shiftFile(db,
              pos2 + size,
              db->fileSize,
              shrinkingSize); // shrinkingSize is always < 0
  }
...
#else
bool
PDBMatchByTypeCreator(TCHARP fileName, VoidP userVars)
{
  FileMatches_Vars* vars = (FileMatches_Vars*)userVars;
  DatabaseHeader dbh;
  TCHARP entry;
  ...
  vars->resultList =
    TCHARPsAdd(vars->resultList, entry, vars->h); // add entry to list
  (vars->resultListLen)++;
...
listDatabasesByTypeCreator(uint32 type, uint32 creator, int32* count, Heap h)
{
  ...
  if (getDataPath(dataPath)) { // search in Settings.dataPath
    ...
    PDBListDatabasesIn(searchPath,
                       false,
                       (HandlePDBSearchProcType)PDBMatchByTypeCreator,
                       &userVars);
  }
  appPath = getAppPath();
  vmPath = getVMPath();
...
getRecordSize(PDBFile db, uint16 index)
{
  ...
  PDBReadAt(db->fh,
            ...
            8,
            GET_OFFSET(index),
            &bytesRW);                  // get current record header
          ...
          bool queryOnly)
{
  ...
  if (index != db->recordCachedPos) { // guich@550: do a little cache - alltests
                                      // gets above 15000 cache hits,
                                      // benchpdbdriver above 50000
    PDBReadAt(db->fh,
              &db->recordCachedHeader,
              8,
              offset,
              &bytesRW); // get current record header
    db->recordCachedHeader.offset =
      SWAP32_FORCED(db->recordCachedHeader.offset); // swapRecordHeader
  ...
  // flsobral@tc120_30: now we check if there are any locked records before
  // checking if the requested record is busy.
  if (!queryOnly                 // query only? go ahead!
      && (db->lockedRecords > 0) // no records locked? go ahead!
      && (db->recordCachedHeader.attr &
          dmRecAttrBusy)) { // is the record we want already locked?
    return null;
  }
  ...
  if (length == null) {
    ...
    if (*length == 0) {
      *length = db->recordCachedSize;
      ...
      size = *length = (db->recordCachedSize < *length)
                         ? db->recordCachedSize
                         : *length; // wants to read a piece of the record only
    }
  }
  ...
  PDBReadAt(db->fh,
            memHandle + sizeof(uint32),
            size,
            db->recordCachedHeader.offset + ofs,
            &bytesRW); // can't change to READ_SIZE_AT bc the macro uses & in
                       // the pointer, corrupting memory
...
endsWithPDB(TCHARP fName)
{
  ...
  return (len > 3 &&
          (*(fName - 3) == (TCHAR)'p' || *(fName - 3) == (TCHAR)'P') &&
