...
typedef DWORD (__stdcall* CeSetFilePointerProc)(HANDLE hFile, LONG lDistanceToMove,
		PLONG lpDistanceToMoveHigh, DWORD dwMoveMethod);
typedef BOOL (__stdcall* CeReadFileProc)(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead,
		LPDWORD lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped);
typedef BOOL (__stdcall* CeWriteFileProc)(HANDLE hFile, LPCVOID lpBuffer,
...
typedef HANDLE(__stdcall* CeCreateFileProc)(LPCWSTR lpFileName, DWORD dwDesiredAccess,
...
bool PDBCreateFile(TCHARP fullPath, bool createIt, bool readOnly, PDBFileRef* fileRef) {
	return (*fileRef = procCeCreateFile(toWChar(fullPath),
										readOnly ? GENERIC_READ : (GENERIC_READ |
												GENERIC_WRITE), // font files must be open in readonly, or two instances will not be able to run
										FILE_SHARE_READ,
										null,
										createIt ? OPEN_ALWAYS : OPEN_EXISTING,
										/*FILE_ATTRIBUTE_NORMAL|FILE_FLAG_RANDOM_ACCESS|FILE_FLAG_WRITE_THROUGH, TEST IT! */
...
bool inline PDBReadAt(PDBFileRef fileRef, VoidP buf, int32 size, int32 offset, int32* read) {
	return (procCeSetFilePointer(fileRef, offset, null, FILE_BEGIN) != 0xFFFFFFFFL) ? PDBRead(fileRef,
			buf, size, read) : false;
}
...
bool inline PDBWriteAt(PDBFileRef fileRef, VoidP buf, int32 size, int32 offset, int32* written) {
	return (procCeSetFilePointer(fileRef, offset, null, FILE_BEGIN) != 0xFFFFFFFFL) ? PDBWrite(fileRef,
			buf, size, written) : false;
}
...
bool inline PDBGrowFileSize(PDBFileRef fileRef, int32 oldSize, int32 growSize) {
	return (procCeSetFilePointer(fileRef, oldSize + growSize, null,
								 FILE_BEGIN) != 0xFFFFFFFFL) ? (procCeSetEndOfFile(fileRef) == TRUE) : 0;
}
...
bool PDBListDatabasesIn(TCHARP path, bool recursive, HandlePDBSearchProcType proc, VoidP userVars) {
	...
	if ((searchHandle = procCeFindFirstFile(toWChar(searchPath),
...
#endif
				JCharP2TCHARPBuf((JCharP) findFileData.cFileName, lstrlenW(findFileData.cFileName),
