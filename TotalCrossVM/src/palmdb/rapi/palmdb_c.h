...
typedef BOOL(__stdcall* CeFindNextFileProc)(HANDLE hFindFile,
                                            LPCE_FIND_DATA lpFindFileData);
typedef HANDLE(__stdcall* CeFindFirstFileProc)(LPCWSTR lpFileName,
                                               LPCE_FIND_DATA lpFindFileData);
typedef DWORD(__stdcall* CeGetLastErrorProc)(void);
typedef DWORD(__stdcall* CeSetFilePointerProc)(HANDLE hFile,
                                               LONG lDistanceToMove,
                                               PLONG lpDistanceToMoveHigh,
                                               DWORD dwMoveMethod);
typedef BOOL(__stdcall* CeReadFileProc)(HANDLE hFile,
                                        LPVOID lpBuffer,
                                        DWORD nNumberOfBytesToRead,
                                        LPDWORD lpNumberOfBytesRead,
                                        LPOVERLAPPED lpOverlapped);
typedef BOOL(__stdcall* CeWriteFileProc)(HANDLE hFile,
                                         LPCVOID lpBuffer,
...
typedef HANDLE(__stdcall* CeCreateFileProc)(
  LPCWSTR lpFileName,
...
typedef DWORD(__stdcall* CeGetFileSizeProc)(HANDLE hFile,
                                            LPDWORD lpFileSizeHigh);
typedef BOOL(__stdcall* CeMoveFileProc)(LPCWSTR lpExistingFileName,
...
extern "C"
{
  ...
  // wince is 4.5x faster when using the system io, due to the
  // FILE_FLAG_RANDOM_ACCESS flag this is why we have a special version not
  // using POSIX
  ...
  Err inline PDBGetLastErr() { return procCeGetLastError(); }

  bool PDBCreateFile(TCHARP fullPath,
                     bool createIt,
                     bool readOnly,
                     PDBFileRef* fileRef)
  {
    return (*fileRef = procCeCreateFile(
              toWChar(fullPath),
              readOnly
                ? GENERIC_READ
                : (GENERIC_READ |
                   GENERIC_WRITE), // font files must be open in readonly, or
                                   // two instances will not be able to run
              FILE_SHARE_READ,
              null,
              ...
              // FILE_ATTRIBUTE_NORMAL|FILE_FLAG_RANDOM_ACCESS, // guich@555_9:
              // with FILE_FLAG_WRITE_THROUGH, queries are 50ms faster, but
              // without it, table fillup is 48% faster!
  ...
  bool inline PDBReadAt(PDBFileRef fileRef,
                        VoidP buf,
                        int32 size,
                        int32 offset,
                        int32* read)
  {
    return (procCeSetFilePointer(fileRef, offset, null, FILE_BEGIN) !=
            0xFFFFFFFFL)
             ? PDBRead(fileRef, buf, size, read)
             : false;
  }

  bool inline PDBWrite(PDBFileRef fileRef,
                       VoidP buf,
                       int32 size,
                       int32* written)
  {
  ...
  bool inline PDBWriteAt(PDBFileRef fileRef,
                         VoidP buf,
                         int32 size,
                         int32 offset,
                         int32* written)
  {
    return (procCeSetFilePointer(fileRef, offset, null, FILE_BEGIN) !=
            0xFFFFFFFFL)
             ? PDBWrite(fileRef, buf, size, written)
             : false;
  }
  ...
  {
    return (procCeSetFilePointer(
              fileRef, oldSize + growSize, null, FILE_BEGIN) != 0xFFFFFFFFL)
             ? (procCeSetEndOfFile(fileRef) == TRUE)
             : 0;
  }

  bool PDBListDatabasesIn(TCHARP path,
                          bool recursive,
                          ...
                          VoidP userVars)
  {
    ...
    if ((searchHandle = procCeFindFirstFile(
           toWChar(searchPath), &findFileData)) == INVALID_HANDLE_VALUE) {
      return false;
    }
...
#endif
          JCharP2TCHARPBuf((JCharP)findFileData.cFileName,
                           lstrlenW(findFileData.cFileName),
                           searchPath + pathLen + 1);
          // tcscpy(searchPath+pathLen+1, findFileData.cFileName);
          stopSearch =
            PDBListDatabasesIn(searchPath, recursive, proc, userVars);
#if defined(WIN32) && !defined(WINCE)
        }
#endif
      } else if (endsWithPDB(
                   toTCHAR(findFileData.cFileName))) { // it's a pdb file.
        if (proc) {
          JCharP2TCHARPBuf((JCharP)findFileData.cFileName,
                           lstrlenW(findFileData.cFileName),
                           searchPath + pathLen + 1);
  ...
  int32 daysInYear(int32 y) { return 365 + leapYear(y); }
  ...
  int32 date2days(int32 y, int32 m, int32 d)
  {
    ...
    if (m > 1) {
      total += daysAccrued(
        m - 1, y); // it is not necessary count current month neither january
    }
  ...
  uint32 PDBGetNow()
  {
    ...
    GetSystemTime(&st);
    timeX =
      date2days(st.wYear, st.wMonth, st.wDay) * 86400; // convert to seconds
    timeX += st.wHour * 3600 + st.wMinute * 60 +
             st.wSecond; // add the number of seconds in today
    // timeX = 2082844800;
