...
will redirect the call to the vmGlobals structure. This will maintain
compatibility with existing PalmOS code.

The declarations must be customized based on some defines:

Documentation:

        HandlePDBSearchProcType: callback function for DmListDatabases. Return:
0 to continue the search, 1 to stop it DmCloseDatabase:         closes the given
database DmCreateDatabase:        create a database with the given parameters.
Name must have 32 characters (counting \0) DmOpenDatabase:          opens the
given database. It must have the full pathname specified DmOpenDatabaseReadOnly:
opens the given database for READ ONLY. It must have the full pathname specified
        DmReleaseRecord:         release the record. If the database is
read-only, it can have many locked records. Otherwise, if it is read-write, it
can have only one record DmNewRecord:             creates a new record at the
given position. if *atP is < 0, the record is appended DmRenameDatabase: Rename
the given opened database to the new parameters DmDeleteDatabase:        delete
the given opened database DmListDatabases:         lists the databases, calling
the defined proc. If proc is null, only the total files is returned. userVars
can be any pointer to a user defined variable. if recursive is 1, pdb files are
                            searched from SUPERWABA_CORE_PATH recursively
        DmListDatabasesIn:       lists the databases, calling the defined proc.
If proc is null, only the total files is returned. userVars can be any pointer
                            to a user defined variable. if recursive is 1, pdb
files are searched from the given path recursively DmFindAndOpenDB: Finds and
opens the database with the given parameters DmNumRecords:            returns
the total number of records in the database DmWrite:                 writes to
the given opened database DmResizeRecord:          resizes the given record. The
record cannot be locked DmRemoveRecord:          removes the given record
        DmGetRecord:             gets a handle to the given record. The record
is marked as locked and is copied into a buffer. You must call DmReleaseRecord
to unlock this record and also you must free the record by calling xfree
   DmQueryRecord:           gets a handle to the given record. The record may be
locked. size ptr may be NULL, or a variable set to 0 (to load the entire record)
or the number of bytes to load. Note that you must free the allocated MemHandle
by yourself, using xfree DmGetRecordAttributes:   get the record attributes,
even if the record is locked DmSetRecordAttributes:   set the record attributes,
even if the record is locked DmSetAttributes:         set the database
attributes DmGetAttributes:         get the database attributes
        ...
        DmGetDatabaseInfo:       returns the database type and creator for the
given filename. Optionally, returns also the records count and the version
information DmGetName:               returns the internal name of the file. The
buffer must be 32 chars long splitPath:               splits the path from the
filename. path or name may be null
...
typedef struct t_DatabaseHeader
{
   ...
   * null for databases that do not include an sortInfo block.
   */
...
#define dmHdrAttrBackup                                                        \
  0x0008 // Set if database should be backed up to PC if no app-specific
// synchronization conduit has been supplied.
#define dmHdrAttrOKToInstallNewer                                              \
  0x0010 // This tells the backup conduit that it's OK for it to install
...
#define dmHdrAttrResetAfterInstall                                             \
  0x0020 // Device requires a reset after this database is installed.
#define dmHdrAttrCopyPrevention 0x0040 // This database should not be copied to.
#define dmHdrAttrStream                                                        \
  0x0080 // This database is used for file stream implementation.
#define dmHdrAttrHidden                                                        \
  0x0100 // This database should generally be hidden from view used to hide
...
#define dmHdrAttrLaunchableData                                                \
  0x0200 // This data database (not applicable for executables) can be
         // "launched"
// by passing it's name to it's owner app ('appl' database with same
// creator) using the sysAppLaunchCmdOpenNamedDB action code.
#define dmHdrAttrRecyclable                                                    \
  0x0400 // This database (resource or record) is recyclable:
// it will be deleted Real Soon Now, generally the next
// time the database is closed.
#define dmHdrAttrBundle                                                        \
  0x0800 // This database (resource or record) is associated with the
         // application with
...
#define DmFindDatabase(currentContext, fileName, dbId)                         \
  myDmFindDatabase(currentContext, fileName, dbId)
#define DmDatabaseInfo(dbId,                                                   \
                       name,                                                   \
                       attributes,                                             \
                       version,                                                \
                       creationDate,                                           \
...
#define DmCreateDatabase(name, creator, type, isResDB)                         \
  myDmCreateDatabase(name, creator, type, isResDB)
#define DmOpenDatabase(dbId, mode) myDmOpenDatabase(dbId, mode)
#define DmSetDatabaseInfo(dbId,                                                \
                          name,                                                \
                          attributes,                                          \
                          version,                                             \
                          creationDate,                                        \
...
#define DmResizeRecord(dbP, index, newSize)                                    \
  myDmResizeRecord(dbP, index, newSize)
...
#define DmRecordInfo(dbP, index, attrP, uniqueIDP, chunkIDP)                   \
  myDmRecordInfo(dbP, index, attrP)
#define DmSetRecordInfo(dbP, index, attrP, uniqueIDP)                          \
  myDmSetRecordInfo(dbP, index, *attrP)
#define DmWrite(recordP, offset, srcP, bytes)                                  \
  myDmWrite(recordP, offset, srcP, bytes)
...
myDmGetLastErr();
Err
myDmCloseDatabase(DmOpenRef dbref);
Err
myDmReleaseRecord(DmOpenRef dbref, int32 index, bool dirty);
VoidP
myDmFindDatabase(Context currentContext, TCHARP fileName, PDBFile dbId);
Err
myDmDatabaseInfo(VoidP dbId,
                 TCHARP name,
                 uint16* attributes,
                 uint16* version,
                 uint32* creationDate,
                 ...
                 uint32* creator);
Err
myDmDeleteDatabase(VoidP dbId);
Err
myDmCreateDatabase(TCHARP pdbName, uint32 creator, uint32 type, bool isResDB);
DmOpenRef
myDmOpenDatabase(VoidP dbId, uint16 mode);
Err
myDmSetDatabaseInfo(VoidP dbId,
                    TCHARP name,
                    uint16* attributes,
                    uint16* version,
                    uint32* creationDate,
                    ...
                    uint32* creator);
MemHandle
myDmNewRecord(DmOpenRef dbP, uint16* atP, uint32 size);
uint16
myDmNumRecords(DmOpenRef db);
MemHandle
DmResizeRecord(DmOpenRef dbP, uint16 index, uint32 newSize);
MemHandle
myDmGetRecord(DmOpenRef dbP, uint16 index);
Err
myDmRemoveRecord(DmOpenRef dbP, uint16 index);
uint32
myMemHandleSize(MemHandle h);
Err
myDmRecordInfo(DmOpenRef dbref,
               uint16 index,
               uint16* attrs); // int8* attrs);
Err
myDmSetRecordInfo(DmOpenRef dbref,
                  uint16 index,
                  uint16 attrs); // int8 attrs);
Err
myDmWrite(VoidP recordP, uint32 offset, VoidP srcP, uint32 bytes);
MemHandle
myDmQueryRecord(DmOpenRef dbref, uint16 index);
CharP
myMemHandleLock(MemHandle h);

#if defined _RAPI_
int32
listDatabases(TCHARP searchPath,
              HandlePDBSearchProcType proc,
              void* userVars,
              byte recursive);
void
swapDatabaseHeader(DatabaseHeader* src, DatabaseHeader* dst);
#else
TCHARPs*
listDatabasesByTypeCreator(uint32 type, uint32 creator, int32* count, Heap h);
#endif

bool
endsWithPDB(TCHARP fName);
