...
static Handle
tryOpen(const char* prefix, const char* libname)
{
...
executeProgram(char* cmdline)
{
...
#endif
    ,
    "libtcvm"); // load in current folder - otherwise, we'll not be able to
                // debug
  ...
  if (!tcvm) {
    printf("%s\n", dlerror());
    tcvm = tryOpen("/usr/lib/totalcross",
                   "libtcvm"); // load in most common absolute path
  }
  if (!tcvm) {
    printf("%s\n", dlerror());
    return 10000;
...
main(int argc, const char* argv[])
{
  ...
  if (argc > argvIndex ||
      args[0] != '1') { // if there's a commandline passed by the system or one
                        // passed by the user
    xstrcat(cmdline, " /cmd ");
    if (args[0] != '1') {
