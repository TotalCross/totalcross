...
static int executeProgram(char* cmdline) {
	...
	tcvm = LoadLibrary(
			   TEXT("tcvm.dll"));               // load in current folder - otherwise, we'll not be able to debug
	if (!tcvm) {
		TCHAR dir[MAX_PATH],
			  *c; // LoadLibrary does not accept .. in the name, so we have to get the current path and build up the full path
		int n = 2;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow) {
	...
	getTCZName(cmdline);
	if (*lpCmdLine || (*args
					   && *args != '1')) { // if there's a commandline passed by the system or one passed by the user
