...
static TCObject createInfo(Context currentContext) {
	TCObject info = createObjectWithoutCallingDefaultConstructor(currentContext,
					"totalcross.util.Hashtable");
	executeMethod(currentContext, getMethod(OBJ_CLASS(info), true, CONSTRUCTOR_NAME, 1, J_INT), info,
				  10);
	return info;
}
...
static bool putInfo(Context currentContext, TCObject info, CharP key, CharP val) {
	...
	if (info == null || key == null || val == null) {
		...
		if (currentContext->thrownException == null) {
			executeMethod(currentContext, getMethod(OBJ_CLASS(info), true, "put", 2, "java.lang.Object",
													"java.lang.Object"), info, infoKey, infoVal);
...
static void getArtificialHash(char* out) {
	...
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("Software\\Drivers"), 0, KEY_READ, &handle) == NO_ERROR) {
		if (RegQueryValueEx(handle, TEXT("MonkeyArea"), NULL, &type, (byte*) &v,
...
static int32 getDeviceHash(Context currentContext, CharP* deviceHash) {
	...
	if (*serial) {
		...
		if (serialLen >
			12) { // use only the first mac address to keep the hash the same as the previous versions.
	...
	if (!strEq(deviceId, "PALM TREO 750") && !strEqn(deviceId, "MOTOROLA MC", 11)
		&& !strEqn(deviceId, "SYMBOL MC", 9)
		&& !strEq(deviceId,
				  "INTERMEC CN3")) // flsobral@tc122: never use IMEI on these device because it is not available when the device is on airplane mode. (phone off) - guich@tc136: skip all Motorola scanners, changed || to &&
#endif
	{
		if (*imei) {
	...
	res = executeMethod(currentContext, getMethod(loadClass(currentContext, "totalcross.sys.Convert",
						true), true, "bytesToHexString", 1, BYTE_ARRAY), out).asObj;
...
//////////////////////////////////////////////////////////////////////////
TC_API void rU_getConfigInfo(NMParams
							 p) { // ras/Utils native public static totalcross.util.Hashtable getConfigInfo();
...
//////////////////////////////////////////////////////////////////////////
TC_API void rU_getProductInfo(NMParams
							  p) { // ras/Utils native public static totalcross.util.Hashtable getProductInfo();
	//   TCObject obj = p->obj[0];
	TCObject info = createInfo(p->currentContext);
	IntBuf buf;
	char buffer[64];
	...
	putInfo(p->currentContext, info, "COMPILATION_DATE",
...
//////////////////////////////////////////////////////////////////////////
TC_API void rU_getDeviceInfo(NMParams
							 p) { // ras/Utils native public static totalcross.util.Hashtable getDeviceInfo() throws ActivationException;
	//   TCObject obj = p->obj[0];
	TCObject info;
	char serial[128];
	char imei[32];
	...
	switch (getDeviceHash(p->currentContext, &deviceHash)) {
		case GDHERR_IMEI:
			throwExceptionNamed(p->currentContext, "ras.ActivationException",
								"Unable to get unique device information; turn on WIFI and/or phone, if available. Try again or contact us if the problem persists.");
			break;
		case GDHERR_ARTIFICIAL_HASH:
			throwExceptionNamed(p->currentContext, "ras.ActivationException",
								"Unable to get unique device information; turn on WIFI and/or phone, if available.");
			break;
		case GDHERR_OOM:
		...
		case NO_ERROR: {
			...
			if (putInfoObj(p->currentContext, info, "PLATFORM", *getStaticFieldObject(p->currentContext,
						   settingsClass, "platform")) &&
				putInfo(p->currentContext, info, "ID", deviceId) &&
				putInfo(p->currentContext, info, "HASH", deviceHash) &&
				putInfo(p->currentContext, info, "VERSAO_ROM", int2str(*settings->romVersionPtr,
