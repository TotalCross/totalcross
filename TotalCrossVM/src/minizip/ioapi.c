...
static uLong   ZCALLBACK fwrite_file_func OF((voidpf opaque, voidpf stream, const void* buf,
		uLong size));
static ZPOS64_T ZCALLBACK ftell64_file_func OF((voidpf opaque, voidpf stream));
static long    ZCALLBACK fseek64_file_func OF((voidpf opaque, voidpf stream, ZPOS64_T offset,
		int origin));
...
uLong size;
{
	...
	ret = executeMethod(zipNativeP->context, zipNativeP->streamRead, streamObj,  zipNativeP->readBuf, 0,
						size).asInt32;
...
uLong size;
{
	...
	ret = executeMethod(zipNativeP->context, zipNativeP->streamWrite, streamObj,  zipNativeP->writeBuf,
						0, size).asInt32;
...
static long ZCALLBACK fseek64_file_func(voidpf  opaque, voidpf stream, ZPOS64_T offset,
										int origin) {
	int fseek_origin = 0;
	long ret;
	switch (origin) {
