...
extern "C"
{
  ...
  void destroyDebug();
  TC_API bool trace(
    char* s); // used to trace function calls. also prints the memory available
  typedef bool (*traceFunc)(char* s);
  TC_API bool debug(const char* s,
                    ...); // this is a message that the user may see or not
  typedef bool (*debugFunc)(const char* s, ...);
  TC_API bool alert(
    char* s,
    ...); // this is a message that will popup to the user immediately
