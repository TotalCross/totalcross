...
//                                 Math                                  //
///////////////////////////////////////////////////////////////////////////
int toBaseAsDecimal(int num, int baseFrom,
					int baseTo) { // converts a number to the given base and keep it as a decimal number. actually, works with bases <= 10
	int k = 0;
	int m = 1;
	while (num > 0) {
		int n = num % baseTo;
...
double dmod(double c1, double c2) {
	return c1 - ((double)((int64)(c1 / c2))) *
		   c2; // take the integer part of the division - guich@tc114_94: swapped c1 with c2.
}
	...
	while (*c) { // test and go to end of string
		if (*c == '-') { //  guich@566_7: removed '+' - for Java, + is not a valid char
			isNeg = true;
				   ...
				   || n-- == 0) { // guich@300_37: fixed int2str and long2str when an invalid char appeared. - guich@554_23: limit the number of decimals
			if (err) {
				*err = true;
			}
			return 0;
...
TC_API CharP int2str(int32 i, IntBuf buf) {
	if (i == 0) {
		return "0";
	} else if (i == 0x80000000) { // handle the only exception - guich@tc126_69: hex value, not decimal!
		return "-2147483648";
	} else {
		CharP c = buf;
		bool negative = (i < 0);
			...
			if (ignoreLen) {
				*c = 0; // strip the number
				...
				return 0;
				   ...
				   || *c > '9') { // guich@300_37: fixed int2str and long2str when an invalid char appeared. - guich@554_23: limit the number of decimals
			if (err) {
				*err = true;
			}
			return 0;
...
TC_API CharP long2str(int64 i, LongBuf buf) {
	...
	if (i == 0) {
		return "0";
			   ...
			   && i <= 2147483647L) { // guich@566_38: if the number fits in an int, then use the faster routine
...
TC_API double str2double(CharP str, bool* err) { // guich@566_38: new routine
	...
	if (*str == '-') {
		...
		++str;    // guich@350_14: fixes problem when converting numbers that starts with +
	}
	while (*str
		   && *str <=
		   ' ') { // skip starting spaces - seems that JDK allows it. - guich@583_7: check if string is not null
		str++;
	}
	if (*str == 0) {
		if (err) {
...
TC_API CharP double2str(double val, int32 decimalCount, DoubleBuf buffer) {
	...
	if (val == 0) {
		...
		xstrcpy(buf, "NaN");
	} else if (bits == DOUBLE_POSITIVE_INFINITY_VALUE || bits == DOUBLE_NEGATIVE_INFINITY_VALUE) {
		xstrcpy(buf, "-Inf");
		if (val > 0) {
			buf[0] = '+';
		}
		...
		if (DOUBLE_MIN_NON_EXP <= val && val <= DOUBLE_MAX_NON_EXP) { // does it fit without sci notation?
			...
			while (1) {
				...
				if (I64_BITS(mant) ==
					DOUBLE_POSITIVE_INFINITY_VALUE) { // case of converting the minimum double value
					int32 e = exponent < 0 ? -exponent : exponent;
					mant = val;
					if (e > 300) {
						mant *= Pow10(300);    // guich@tc200: fix convertion of MIN_DOUBLE_VALUE
						e -= 300;
					}
		...
		if (decimalCount == 0) {
			...
			*buf++ = '.';
			...
			if (floating)
				...
				if (i > 20) {
					i = 20;    // this should not respect the maximum allowed width, because its just for user formatting
				}
				while (i-- > 0) {
					*buf++ = '0';
				}
