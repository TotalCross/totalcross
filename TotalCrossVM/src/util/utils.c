...
hashCodeFmt(CharP fmt, ...)
{
  ...
  while (*fmt)
    switch (*fmt++) {
      ...
      case 'x': // for pointer, when we start to support 64-bit platforms, move
                // this to case 'l' with the aid of a #ifdef
      case 'i': {
...
///////////////////////////////////////////////////////////////////////////
int
toBaseAsDecimal(int num, int baseFrom, int baseTo)
{ // converts a number to the given base and keep it as a
  // decimal number. actually, works with bases <= 10
  int k = 0;
  int m = 1;
  while (num > 0) {
    int n = num % baseTo;
...
// converts an unsigned value to hex, filling with zeros. outBuf must have
// enough space to hold places+1.
TC_API void
int2hex(int32 b, int32 places, CharP outBuf)
{
...
// enough space to hold places+1.
void
long2hex(int64 b, int32 places, CharP outBuf)
{
  CharP b2h = "0123456789ABCDEF";
  outBuf += places;
  *outBuf-- = 0;
...
bool
radix2int(CharP str, int32 radix, int32* result)
{
...
bool
radix2long(CharP str, int32 radix, int64* result)
{
  int32 strLen = xstrlen(str);
  int32 i;
  char c;
...
int32
min32(int32 i1, int32 i2)
{
  return (i1 <= i2) ? i1 : i2;
}
/* Returns the maximum between the two numbers. */
int32
max32(int32 i1, int32 i2)
{
  return (i1 >= i2) ? i1 : i2;
}
...
double
dmod(double c1, double c2)
{
  return c1 - ((double)((int64)(c1 / c2))) *
                c2; // take the integer part of the division - guich@tc114_94:
                    // swapped c1 with c2.
}

TC_API int32
str2int(CharP str, bool* err)
{
  int32 r = 0;
  int32 m = 1;
  ...
  while (*c) { // test and go to end of string
    if (*c ==
        '-') { //  guich@566_7: removed '+' - for Java, + is not a valid char
      isNeg = true;
    } else if (*c < '0' || *c > '9' ||
               n-- == 0) { // guich@300_37: fixed int2str and long2str when an
                           // invalid char appeared. - guich@554_23: limit the
                           // number of decimals
      if (err) {
        *err = true;
      }
...
TC_API CharP
int2str(int32 i, IntBuf buf)
{
  if (i == 0) {
    return "0";
  } else if (i == 0x80000000) { // handle the only exception - guich@tc126_69:
                                // hex value, not decimal!
    return "-2147483648";
  } else {
    CharP c = buf;
    bool negative = (i < 0);
...
static int64
str2longPriv(CharP str, bool* err, bool ignoreLen)
{ // guich@tc114_40
  int64 r = 0;
  int64 m = 1;
  ...
  while (*c) { // test and go to end of string
    if (*c ==
        '-') { //  guich@566_7: removed '+' - for Java, + is not a valid char
      isNeg = true;
    } else if ((*c == 'L' || *c == 'l') && *(c + 1) == 0) { // last char is L?
      *c = 0;
      break;
    } else if (n-- == 0) {
      if (ignoreLen) {
        *c = 0; // strip the number
        ...
        return 0;
               ...
               *c > '9') { // guich@300_37: fixed int2str and long2str when an
                           // invalid char appeared. - guich@554_23: limit the
                           // number of decimals
      if (err) {
        *err = true;
...
TC_API int64
str2long(CharP str, bool* err)
{
  return str2longPriv(str, err, false);
}

TC_API CharP
long2str(int64 i, LongBuf buf)
{
  CharP c = buf + sizeof(LongBuf) - 2;
  if (i == 0) {
    return "0";
  } else if (i >= -2147483647L &&
             i <= 2147483647L) { // guich@566_38: if the number fits in an int,
                                 // then use the faster routine
...
#define DOUBLE_POSITIVE_INFINITY_VALUE                                         \
  IEEE_I64_VALUE(I64_CONST(0x7ff0000000000000))
#define DOUBLE_NEGATIVE_INFINITY_VALUE                                         \
  IEEE_I64_VALUE(I64_CONST(0xfff0000000000000))
...
static double p1[] = { 1.0,  1e1,  1e2,  1e3,  1e4,  1e5,  1e6,  1e7,
                       1e8,  1e9,  1e10, 1e11, 1e12, 1e13, 1e14, 1e15,
                       1e16, 1e17, 1e18, 1e19, 1e20, 1e21, 1e22, 1e23,
                       1e24, 1e25, 1e26, 1e27, 1e28, 1e29, 1e30, 1e31 };
static double p32[] = { 1.0,   1e32,  1e64,  1e96,  1e128,
                        1e160, 1e192, 1e224, 1e256, 1e288 };
static double np32[] = {
  // guich@570_51
  1.0,    1e-32,  1e-64,  1e-96,  1e-128, 1e-160,
  1e-192, 1e-224, 1e-256, 1e-288, 1e-320,
};
static double
Pow10(int32 exp)
{ // fast pow(10,exp) - thanks to Peter M. Dickerson
  if (exp >= 0) {
    return p1[exp & 31] * p32[(exp >> 5)];
  }
  return np32[-exp >> 5] /
         p1[-exp & 31]; // guich@570_51: when exp >= 320, p32 index is 10, and
                        // 1e320 is impossible
}

/** recognizes exponencial term (e) */
TC_API double
str2double(CharP str, bool* err)
{ // guich@566_38: new routine
  ...
  if (*str == '-') {
    ...
    ++str; // guich@350_14: fixes problem when converting numbers that starts
           // with +
  }
  while (*str &&
         *str <= ' ') { // skip starting spaces - seems that JDK allows it. -
                        // guich@583_7: check if string is not null
    str++;
  }
  if (*str == 0) {
    if (err) {
  ...
  for (c = str; *c; len++, c++) {
    if (*c == 'E' ||
        *c == 'e') { // guich@350_13: handle exponent in uppercase 'E'
      ePtr = c;
  ...
  if (dotPtr && str == dotPtr) { // fix for .25
    ...
    result =
      (double)str2longPriv(str, &err2, true); // convert the first part 'xxx'
    if (err2) {
      if (err) {
        *err = true;
      }
...
static double rounds5[] = { 5e-1,  5e-2,  5e-3,  5e-4,  5e-5,  5e-6,
                            5e-7,  5e-8,  5e-9,  5e-10, 5e-11, 5e-12,
                            5e-13, 5e-14, 5e-15, 5e-16, 5e-17, 5e-18 };

TC_API CharP
double2str(double val, int32 decimalCount, DoubleBuf buffer)
{
  ...
  if (val == 0) {
  ...
  } else if (bits == DOUBLE_NAN_VALUE) {
    xstrcpy(buf, "NaN");
  } else if (bits == DOUBLE_POSITIVE_INFINITY_VALUE ||
             bits == DOUBLE_NEGATIVE_INFINITY_VALUE) {
    xstrcpy(buf, "-Inf");
    if (val > 0) {
      buf[0] = '+';
    }
    ...
    if (floating) {
      decimalCount = DOUBLE_MAX_DIGITS;
    }
    if (val < 0) {
      val = -val; // 1000.5432
      *buf++ = '-';
    ...
    if (DOUBLE_MIN_NON_EXP <= val &&
      ...
      while (1) {
        ...
        if (I64_BITS(mant) ==
            DOUBLE_POSITIVE_INFINITY_VALUE) { // case of converting the minimum
                                              // double value
          int32 e = exponent < 0 ? -exponent : exponent;
          mant = val;
          if (e > 300) {
            mant *=
              Pow10(300); // guich@tc200: fix convertion of MIN_DOUBLE_VALUE
            e -= 300;
          }
        ...
        if (integral == 0 && !adjusted) {
          adjusted = true; // 0.12345 ?
          exponent--;
        } else if (integral >= 10 && !adjusted) {
          adjusted = true; // 10.12345 ?
          exponent++;
        } else {
          val = mant;
          break;
    ...
    if (decimalCount == 0) {
      ...
      *buf++ = '.';
      ...
      else if (i > 0) { // fill with zeros if needed
        if (i > 20) {
          i = 20; // this should not respect the maximum allowed width, because
                  // its just for user formatting
        }
        while (i-- > 0) {
          *buf++ = '0';
        }
...
FILE*
findFile(CharP name, CharP pathOut)
{
  FILE* f;
  ...
  if (f == null) {
    xstrprintf(
      fullName, "P:/gitrepo/TotalCross/TotalCrossSDK/dist/vm/%s", name);
    f = fopen(fullName, "rb");
  }
  // 8. Test.tcz
  if (f == null) {
    xstrprintf(
      fullName,
      "P:/gitrepo/TotalCross/TotalCrossVM/src/tests/java/install/win32/%s",
      name);
    f = fopen(fullName, "rb");
  }
...
#endif
#if !defined(ANDROID) && !defined(FORCE_LIBC_ALLOC) &&                         \
  !defined(ENABLE_WIN32_POINTER_VERIFICATION)
  if (!maxblock) {
...
TC_API TCHARP
CharP2TCHARPBuf(CharP from, TCHARP to)
{
...
TC_API CharP
TCHARP2CharPBuf(TCHARP from, CharP to)
{
#if defined(UNICODE)
...
void
replaceChar(CharP s, char from, char to)
{
  for (; *s; s++)
    if (*s == from) {
      *s = to;
...
#ifdef ANDROID
void
jstring2CharP(jstring src, char* dest)
{
  jstring2CharPEnv(src, dest, getJNIEnv());
}
void
jstring2CharPEnv(jstring src, char* dest, JNIEnv* env)
{
