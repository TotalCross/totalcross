...
TC_API int32 str2int(CharP str, bool *err) {
	...
	while (*c) { // test and go to end of string
		if (*c == '-') { //  guich@566_7: removed '+' - for Java, + is not a valid char
			isNeg = true;
		} else if ((*c < '0') || (*c > '9') || (n-- == 0)) { // guich@300_37: fixed int2str and long2str when an invalid char appeared. - guich@554_23: limit the number of decimals
			if (err) {
				*err = true;
			}
			return 0;
...
static int64 str2longPriv(CharP str, bool *err, bool ignoreLen) { // guich@tc114_40
	...
	while (*c) { // test and go to end of string
		...
		} else if ((*c < '0') || (*c > '9') ) { // guich@300_37: fixed int2str and long2str when an invalid char appeared. - guich@554_23: limit the number of decimals
			if (err) {
				*err = true;
			}
			return 0;
...
TC_API CharP long2str(int64 i, LongBuf buf) {
	...
	if (i == 0) {
		return "0";
	} else if ((i >= -2147483647L) && (i <= 2147483647L) ) { // guich@566_38: if the number fits in an int, then use the faster routine
		...
		if ((DOUBLE_MIN_NON_EXP <= val) && (val <= DOUBLE_MAX_NON_EXP) ) { // does it fit without sci notation?
			...
			while (1) {
				...
				if (I64_BITS(mant) == DOUBLE_POSITIVE_INFINITY_VALUE) { // case of converting the minimum double value
					...
					mant = val;
					if (e > 300) {
						mant *= Pow10(300); e -= 300;
					}                   // guich@tc200: fix convertion of MIN_DOUBLE_VALUE
