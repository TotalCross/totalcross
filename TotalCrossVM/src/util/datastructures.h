...
// Here we concentrate all data structures used, like hashtable, array, list,
// etc. Must turn on "32bit ints" for the target platform
...
extern "C"
{
  ...
  typedef void (*VisitElementFunc)(
    int32 i32,
    VoidP ptr); // i32 is an union with ptr, so be sure to access what you
                // really stored!
  typedef void (*VisitElementKeyFunc)(
    HTKey key,
    int32 i32,
    VoidP ptr); // i32 is an union with ptr, so be sure to access what you
                // really stored!
  typedef bool (*VisitElementContextFunc)(
    Context context,
    VoidP ptr); // i32 is an union with ptr, so be sure to access what you
                // really stored!
  ...
  TC_API Hashtable
  htNew(int32 count,
        Heap heap); // heap is optional; if passed, the rehash function will not
                    // be called if the hashtable gets too big (in this case,
                    // remember to initialize the hashtable with a value big
  ...
  typedef void (*htRemoveFunc)(
    Hashtable* iht,
    HTKey key); // removes the key from the table. If there's a heap, the
                // pointer is NOT freed
  TC_API void htFree(
    Hashtable* iht,
    VisitElementFunc
      freeElement); // if there's a heap, the structures are not freed; you
                    // must free them by destroying the heap yourself
  typedef void (*htFreeFunc)(Hashtable* iht, VisitElementFunc freeElement);
  TC_API bool htFreeContext(
    Context context,
    Hashtable* iht,
    VisitElementContextFunc
      freeElement); // if there's a heap, the structures are not freed; you
                    // must free them by destroying the heap yourself
  typedef bool (*htFreeContextFunc)(Context context,
                                    Hashtable* iht,
                                    VisitElementContextFunc freeElement);
  TC_API bool htInc(Hashtable* iht,
                    HTKey key,
...
//
// putting next/prev first lets you call listGetCount/list2array for any type of
// list. You can optionally pass a created Heap to the list in order to use it
// as the memory allocator. Note that the Remove method does not work with lists
// stored in heaps.
...
#define newArray(sizeofElem, len, mp)                                          \
  privateNewArray(sizeofElem, len, mp, __FILE__, __LINE__)
  void* privateNewArray(int32 sizeofElem,
                        int32 len,
                        Heap mp,
                        const char* file,
                        int line);
...
#define freeArray(p)                                                           \
  do {                                                                         \
    if (p) {                                                                   \
      uint8* b = ((uint8*)p) - TSIZE;                                          \
      xfree(b);                                                                \
      p = null;                                                                \
    }                                                                          \
  } while (0)
#define heapFreeArray(heap, p)                                                 \
  do {                                                                         \
    if (p) {                                                                   \
      uint8* b = ((uint8*)p) - TSIZE;                                          \
      heapFree(heap, b);                                                       \
...
#define newPtrArrayOf(type, len, heap)                                         \
  (type##Array)                                                                \
    newArray(sizeof(type),                                                     \
             len,                                                              \
             heap) // used for primitive types, TCObject and (J)CharP arrays

#define SET_ARRAYLEN(x) (((size_t*)x)[-1])
#define ARRAYLEN(x) ((int32)(((size_t*)x)[-1]))
#define ARRAYLENV(x)                                                           \
  ((x) ? ARRAYLEN(x) : 0) // checks if x is null, returning 0 if yes
