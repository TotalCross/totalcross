...
extern "C"
{
  ...
  TC_API void getDateTime(int32* year,
                          int32* month,
                          int32* day,
                          int32* hour,
                          int32* minute,
                          int32* second,
                          int32* millis);
  typedef void (*getDateTimeFunc)(int32* year,
                                  int32* month,
                                  int32* day,
                                  int32* hour,
                                  int32* minute,
  ...
  int32 toBaseAsDecimal(int32 num,
                        int32 baseFrom,
                        int32 baseTo); // converts a number to octal

  TC_API int32 hashCode(CharP s);
  typedef int32 (*hashCodeFunc)(CharP s);
  int32 hashCodeSlash2Dot(CharP s);

  TC_API int32 hashCodeFmt(
    CharP fmt,
    ...); // Computes the hashcode of the given variable number of types.
          // Allowed types passed in fmt: s (CharP) S (JCharP) x (pointer) i
          // (int32) l (int64), v (CharP converting to lowercase). Ex: int32
          // hash = hashCodeFmt("sii", "Barbara", 30, anInt32variable);
  ...
  /// Search and find a file in the most common paths. Name should be the
  /// filename only, should not contain absolute paths. If pathOut is not null,
  /// it should point to a buffer where the full path will be stored
  FILE* findFile(CharP name, CharP pathOut);

#define LF_NONE 0
#define LF_RECURSIVE 1
  /// List all files on the given folder. In Palm OS, if slot is 0, searches in
  /// main memory, otherwise NVFS is used. If (options & LF_RECURSIVE) is true,
  /// all folders from the given one are searched, and the path is stored with
  /// the file's name; otherwise, only the file name is stored in the list.
  /// <b>Important: count and list must be initialized to 0 before this function
  /// is called!</b> options can be a combination of LF_RECURSIVE
  TC_API Err listFiles(TCHARP path,
                       int32 slot,
                       TCHARPs** list,
                       int32* count,
                       Heap h,
                       int32 options);
  typedef Err (*listFilesFunc)(TCHARP path,
                               int32 slot,
                               TCHARPs** list,
                               int32* count,
                               Heap h,
  ...
  /// Convert a CharP to a primitive type. 'err': if not null, returns true if
  /// an error occurs (must be set to false before calling)
...
/// Pauses the program for the given number of milliseconds
...
#endif
  /// Returns the amount of free memory. Maximum is 2GB. If maxblock is true, it
  /// return the biggest contiguous block size available
  ...
  // Be careful not to hold the result for a long period, since calling this
  // function again will destroy the first result. In all other platforms, no
  // buffer is used and the function just returns "from", since no convertion is
  // needed (TCHAR = char)
...
#define String2CharP(strObj)                                                   \
  JCharP2CharP(String_charsStart(strObj), String_charsLen(strObj))
#define String2CharPBuf(strObj, buf)                                           \
  JCharP2CharPBuf(String_charsStart(strObj), String_charsLen(strObj), buf)
#define String2TCHARP(strObj)                                                  \
  JCharP2TCHARP(String_charsStart(strObj), String_charsLen(strObj))
#define String2TCHARPBuf(strObj, buf)                                          \
  JCharP2TCHARPBuf(String_charsStart(strObj), String_charsLen(strObj), buf)
