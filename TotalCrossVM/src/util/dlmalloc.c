...
#define DEFAULT_GRANULARITY                                                    \
  2 * 1024 *                                                                   \
    1024 // guich@tc124_17: 2MB       - tc330: increased for all platforms
...
#define MAX_RELEASE_CHECK_RATE                                                 \
  MAX_SIZE_T // guich@tc330: prevent memory from being recovered
void
tcabort(char* msg, char* file, int line);
...
#define ABORT tcabort("DLMALLOC", __FILE__, __LINE__)
#define CORRUPTION_ERROR_ACTION(m)                                             \
  tcabort("DLMALLOC CORRUPTION ERROR", __FILE__, __LINE__)
#define USAGE_ERROR_ACTION(m, p)                                               \
  tcabort("DLMALLOC USAGE ERROR", __FILE__, __LINE__)
...
init_malloc_global_mutex()
{
  for (;;) {
    ...
    if (stat == 0 && interlockedcompareexchange(
                       &malloc_global_mutex_status, (LONG)-1, (LONG)0) == 0) {
...
#if MALLOC_INSPECT_ALL
void
dlmalloc_inspect_all(void (*handler)(void* start,
                                     void* end,
                                     size_t used_bytes,
                                     void* callback_arg),
                     void* arg)
...
void**
mspace_independent_calloc(mspace msp,
                          size_t n_elements,
                          size_t elem_size,
                          void* chunks[])
{
...
void**
mspace_independent_comalloc(mspace msp,
                            size_t n_elements,
                            size_t sizes[],
                            void* chunks[])
{
