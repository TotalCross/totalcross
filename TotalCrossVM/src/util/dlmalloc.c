...
#ifdef WINCE
#define EXTRA_B 4 // guich@tc124_17: the actual value used by the system is a pageSize (64K)
#else
#define EXTRA_B 0
#endif
...
{
  void* ptr = VirtualAlloc(0, size+EXTRA_B, MEM_RESERVE|MEM_COMMIT, PAGE_READWRITE); // guich@tc124_17: prevent allocation across block boundaries, which fails on winmo6
...
void dump_memory_map_int(char* fileSuffix, char* blockName, msegmentptr mseg) 
{
   char buf[50];
   unsigned long totalAlloc=0,totalFree=0,maxFree=0,blocksFree=0,totalBlocks=0;
   int sizes[256],i,idx;
   FILE* f;

   memset(sizes,0,sizeof(sizes));
   sprintf(buf, "memmap_%s_%s.txt", blockName, fileSuffix);
   f = fopen(buf, "w+");
   debug(buf);
   ensure_initialization();
   if (f)
   {      
      ...
      while (s != 0) 
      {
         ...
         mchunkptr q = align_as_chunk(s->base);
         size_t size = s->size;
         sprintf(buf, "SEG: %-8ld - %lX\n", (long)size, ((unsigned long)chunk2mem(q))+8); fwrite(buf, strlen(buf), 1, f);
         while (segment_holds(s, q) && q != gm->top && q->head != FENCEPOST_HEAD) 
         {
            size_t sz = chunksize(q);
            int inUse = is_inuse(q);
            char c0 = inUse ? '#' : ' ';
            char c = '!';
            totalBlocks++;
            totalAlloc += sz;
      ...
      // now dump the allocated chunk sizes and some statistics
      sprintf(buf, "total allocated: %ld\n",totalAlloc);   fwrite(buf, 1, strlen(buf), f);
      sprintf(buf, "blocks total   : %ld\n",totalBlocks);  fwrite(buf, 1, strlen(buf), f);
      sprintf(buf, "total free     : %ld\n",totalFree);    fwrite(buf, 1, strlen(buf), f);
      sprintf(buf, "total in use   : %ld\n",totalAlloc-totalFree);    fwrite(buf, 1, strlen(buf), f);
      sprintf(buf, "max block free : %ld\n",maxFree);      fwrite(buf, 1, strlen(buf), f);
      sprintf(buf, "blocks free    : %ld\n",blocksFree);   fwrite(buf, 1, strlen(buf), f);
      sprintf(buf, "================\nBlock sizes\n"); fwrite(buf, strlen(buf), 1, f);
      for (i = 0; i < 255; i++)
         if (sizes[i] > 0)
         {
            sprintf(buf, "%4d: %d\n", (i+1)<<3, sizes[i]); fwrite(buf, strlen(buf), 1, f);
         }
