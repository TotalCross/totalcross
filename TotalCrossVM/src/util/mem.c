...
#else
//#if defined(WIN32) || defined(ANDROID) || defined(darwin) // windows always
// give us aligned blocks
#define EXTRA4ALIGN 4
#endif

#define XMALLOC_MARK_START                                                     \
  8 // note: start must be multiple of 4, otherwise a "datatype misnaligned"
    // will be thrown in wince
...
TC_API void
setCountToReturnNull(int32 n)
{
  allocCount2ReturnNull = n;
}
TC_API int32
getCountToReturnNull()
{
...
#ifdef ENABLE_WIN32_POINTER_VERIFICATION // special memory allocation to find
                                         // bugs in windows 32
...
DbgMalloc(SIZE_T size)
{
  ...
  if (allocCount2ReturnNull > 0 &&
      --allocCount2ReturnNull == 0) { // used on test suites to return null
                                      // after a given number of allocations
    return null;
  }
  ...
  rounded_size =
    (size + 4 + pageSize - 1) & (-pageSize); // round up to the page boundary

  start = (BYTE*)VirtualAlloc(
    NULL, rounded_size + pageSize, MEM_COMMIT, PAGE_READWRITE);
  if (start == null) {
    return null;
  }
  res =
    VirtualProtect(start + rounded_size, pageSize, PAGE_NOACCESS, &old_protect);
...
{
  if (createdHeaps !=
      null) { // no threads are alive at this point, so there's no need to lock
...
destroyMem()
{
  ...
  if (showMemoryMessagesAtExit && (b1 || b2 || warnOnExit)) { // guich@tc114_44
    alert("Memory %s found. Check the\ndebug console for more information.",
          warnOnExit ? "problems" : "leaks");
  }
#endif
}
...
TC_API Heap
privateHeapCreate(bool add2list, const char* file, int32 line)
{
  Heap p = newX(Heap);
  if (p) {
    ...
    xstrcpy(
      p->ex.creationFile,
      max32(0,
            len - (sizeof(p->ex.creationFile) - 1)) +
        (char*)file); // if src is bigger than the buffer, copy the end of the
                      // string (the filename is more important than the path)
...
static MemBlock
createMemBlock(Heap m, uint32 size)
{
  ...
  mb->block = newPtrArrayOf(UInt8,
                            size + EXTRA4ALIGN,
                            0); // to be able to align if necessary
  if (!mb->block) {
    xfree(mb);
    debug("heap error of heap created at %s (%d). setjmp at %s (%d)",
          m->ex.creationFile,
          m->ex.creationLine,
          m->ex.setjmpFile,
          m->ex.setjmpLine);
  ...
  mb->availSize =
    ARRAYLEN(mb->block) -
    EXTRA4ALIGN; // guich@tc114_16: decrease EXTRA4ALIGN from the avail size
  mb->next = m->current;
  m->current = mb;
  m->blocksAlloc++;
...
static uint32
getIdealBlockSize(Heap m)
{ // 100 x mean size of allocated blocks
...
heapAlloc(Heap m, uint32 size)
{
  ...
  m->numAlloc++;
  if (m->current == null ||
      allocFrom->availSize < size) { // need to alloc one more chunk?
    uint32 blockSize = 0;
    if (size >= MEMBLOCK_SIZE) { // must be kept separately, bc the user may
                                 // free this block
      allocFrom = createMemBlock(m, blockSize = size);
    } else if (m->current == null ||
               m->totalAvail < size) { // first block or total available not
                                       // enough to hold the block?
      allocFrom = createMemBlock(
        m,
        blockSize = max32(
          size,
          getIdealBlockSize(m))); // must always have size for the desired block
    } else {
      ...
      if (!allocFrom) {
        allocFrom =
          createMemBlock(m, blockSize = max32(size, getIdealBlockSize(m)));
      }
    }
    if (blockSize > 0) {
...
void
heapFree(Heap m, void* ptr)
{
  MemBlock head, prev;
  for (head = m->current, prev = null; head != null;
...
void
heapFreeAsking(Heap m, AskIfFreeFunc ask)
{
...
TC_API void
heapDestroyPrivate(Heap m, bool added2list)
{
  if (!m) {
    return;
  }
  ...
  // debug("Freeing heap created at %s (%d). setjmp at %s (%d):
  // %X",m->ex.creationFile,m->ex.creationLine,m->ex.setjmpFile,
  // m->ex.setjmpLine, m);
  while (m->current != null) {
    volatile MemBlock mb = m->current->next;
    // xmemzero(m->current->block, ARRAYLEN(m->current->block)); // erase the
    // block to make sure that no pointers inside of it are reused
...
TC_API void
privateHeapError(Heap m, int32 errorCode, const char* file, int32 line)
{
  jmp_buf errorJump;
  int32 len = xstrlen(file);
  xstrcpy(
  ...
  xmemmove(
    ...
    m->ex.errorJump,
    sizeof(
      jmp_buf)); // create a local copy since the original will be destroyed
  // heapDestroy(m); - must be done by the user, otherwise, the error*
  // informations will be destroyed
  longjmp(errorJump, errorCode);
}

void
heapSetFinalizer(Heap m, HeapFinalizerFunc fin, void* bag)
{
  m->finalizerFunc = fin;
  m->finalizerBag = bag;
}
...
TC_API int32
privateHeapSetJump(Heap m, const char* file, int32 line)
{
  int32 len = xstrlen(file);
  xstrcpy(
    m->ex.setjmpFile,
    max32(0,
          len - (sizeof(m->ex.setjmpFile) - 1)) +
      (char*)file); // if src is bigger than the buffer, copy the end of the
                    // string (the filename is more important than the path)
...
static struct htmElem*
htmPut(uint32 addr, int line, const char* src)
{
  struct htmElem* np;
  int32 hashval;
  ...
  if ((np = htmGet(addr)) == NULL) { // duplicated keys not allowed
    ...
    if (memH) {
      ...
      xstrcpy(
        np->src,
        max32(0, len - (sizeof(np->src) - 1)) +
          (char*)src); // if src is bigger than the buffer, copy the end of the
                       // string (the filename is more important than the path)
...
#ifndef darwin
static void*
addMemMarks(uint32 size, uint8* ptr)
{
               ...
               int line)
{
  ...
  if (p) {
    uint8* ptr = (uint8*)p;
    if (*(ptr - 1) != 0x01 || *(ptr - 2) != 0x01 || *(ptr - 3) != 0x01 ||
        *(ptr - 4) != 0x01) {
      struct htmElem* e = htmGet((uint32)p);
      debug("%s - not a memory chunk or chunk's start was overwritten: %lx - "
            "%d. %s at %s, line %d",
            msg,
            (long)p,
            (int)*(ptr - 1),
            e ? "allocated" : "freed",
            e ? e->src : file,
            e ? e->line : line); // this message will be printed inconditionally
      return null;
    } else {
      ...
      if (replaceMarks) {
        *(ptr - 1) = *(ptr - 2) = *(ptr - 3) = *(ptr - 4) =
          2; // set them to 2 so we can find a block that was freed twice
      }
      if (*(ptr + size) != 0x01 || *(ptr + size + 1) != 0x01) {
        struct htmElem* e = htmGet((uint32)p);
        debug("%s - chunk's end was overwritten: %lx (%ld) - %d. %s at %s, "
              "line %d",
              msg,
              (long)p,
              (long)size,
              ...
              e ? e->src : file,
              e ? e->line
                : line); // this message will be printed inconditionally
        return null;
      } else if (replaceMarks) {
...
static uint8*
memError(char* func, int32 origSize, const char* file, int line)
{
#ifdef WIN32
  debug("%s(%d) RETURNING NULL TO %s, line %d. Free memory: %d, avail per "
        "process: %d",
        func,
        (int)origSize,
        file,
        line,
        (int)getFreeMemory(false),
        (int)getFreeMemory(true));
#else
  debug("%s(%d) RETURNING NULL TO %s, line %d. Free memory: %d",
        func,
        (int)origSize,
        file,
        line,
  ...
  if (leakCheckingEnabled) { // no threads are running at this point
    ...
    if (allocCount != freeCount) {
      debug("\nMemory leaked\n Number of xmallocs: %d\n Number of xfrees: "
            "%d\nList of places where the leaks were xmalloc'd:",
            (int)allocCount,
            (int)freeCount); // guich@570_94
    }
#else
    if (totalAllocated != 0) {
...
TC_API uint8*
privateXmalloc(uint32 size, const char* file, int line)
{
#if defined(FORCE_LIBC_ALLOC) || defined(ENABLE_WIN32_POINTER_VERIFICATION)
  uint8* ptr;
  ...
  // debug("xmalloc(%d) at %s (%d). avail after: %d",size, file, line,
  // maxAvail);
#endif
#endif
...
#ifdef ENABLE_TRACE
  // debug("alloc(%d) in %s line %d. Free: %d, used: %d (%lX)",(int)size, file,
  // line, (int)getFreeMemory(false),totalAllocated,(long)p);
#endif
  UNLOCKVAR(alloc);
  return p;
#endif
...
TC_API void
privateXfree(void* p, const char* file, int line)
{
#if defined(FORCE_LIBC_ALLOC) || defined(ENABLE_WIN32_POINTER_VERIFICATION)
  if (leakCheckingEnabled && !htmRemove((uint32)p)) {
    debug(
      "free: %lX NOT REMOVED. xfree called from %s (%d)", (long)p, file, line);
  }
  LOCKVAR(alloc);
  freeCount++;
  free(p);
  ...
  if (p) {
    ...
    if (p) {
      ...
      maxAvail += size;
#ifdef TRACE_OBJCREATION
      // debug("xfree %d at %s (%d). avail after: %d",size + XMALLOC_EXTRASIZE,
      // file, line, maxAvail);
#endif
#endif
      if (leakCheckingEnabled) {
        if (!htmRemove((uint32)pp)) {
          debug("free(%d): %lX NOT REMOVED. xfree called from %s (%d)",
                (int)size,
                (long)pp,
                file,
                line);
          ...
          totalAllocated -=
            size; // guich@585_3: only shrink memory if remove is successfull
          freeCount++;
        }
      } else {
        totalAllocated -= size; // guich@tc100b5: remember to decrease it
      }
      realFree(p);
#ifdef ENABLE_TRACE
      // debug("free: %d, used: %d (%lX)",(int)getFreeMemory(false),
      // totalAllocated, (long)pp);
#endif
    }
  }
#endif
...
TC_API uint8*
privateXrealloc(uint8* ptr, uint32 size, const char* file, int line)
{
  uint32 origSize = size;
#ifndef darwin
  uint32 oldSize = 0;
...
#ifdef ENABLE_TRACE
  // debug("realloc(%d->%d) in %s line %d. Free: %d, used: %d
  // (%lX)",(int)oldSize, (int)size, file, line,
  // (int)getFreeMemory(false),totalAllocated,(long)p);
#endif
  UNLOCKVAR(alloc);
  return p;
#endif
...
TC_API uint8*
privateXcalloc(uint32 NumOfElements,
               uint32 SizeOfElements,
               const char* file,
               int line)
{
#if defined(FORCE_LIBC_ALLOC) || defined(ENABLE_WIN32_POINTER_VERIFICATION)
