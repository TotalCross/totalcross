...
VOID* DbgMalloc(SIZE_T size) {
	...
	if (allocCount2ReturnNull > 0
		&& --allocCount2ReturnNull ==
		0) { // used on test suites to return null after a given number of allocations
		return null;
	}
...
TC_API Heap privateHeapCreate(bool add2list, const char* file, int32 line) {
	...
	if (p) {
		...
		VoidPs* ch;
		xstrcpy(p->ex.creationFile, max32(0,
										  len - (sizeof(p->ex.creationFile) - 1)) + (char*)
				file); // if src is bigger than the buffer, copy the end of the string (the filename is more important than the path)
...
static MemBlock createMemBlock(Heap m, uint32 size) {
	...
	if (!mb->block) {
		xfree(mb);
		debug("heap error of heap created at %s (%d). setjmp at %s (%d)", m->ex.creationFile,
			  m->ex.creationLine, m->ex.setjmpFile, m->ex.setjmpLine);
		HEAP_ERROR(m, HEAP_MEMORY_ERROR);
	}
	mb->current = mb->block;
	mb->availSize = ARRAYLEN(mb->block) -
	...
	if (m->current == null || allocFrom->availSize < size) { // need to alloc one more chunk?
		...
		if (size >= MEMBLOCK_SIZE) { // must be kept separately, bc the user may free this block
			allocFrom = createMemBlock(m, blockSize = size);
				   ...
				   || m->totalAvail < size) { // first block or total available not enough to hold the block?
			allocFrom = createMemBlock(m, blockSize = max32(size,
...
TC_API void privateHeapError(Heap m, int32 errorCode, const char* file, int32 line) {
	jmp_buf errorJump;
	int32 len = xstrlen(file);
	xstrcpy(m->ex.errorFile, max32(0,
								   len - (sizeof(m->ex.errorFile) - 1)) + (char*)
			file); // if src is bigger than the buffer, copy the end of the string (the filename is more important than the path)
	m->ex.errorLine = line;
	m->ex.errorCode = errorCode;
	xmemmove(errorJump, m->ex.errorJump,
			 sizeof(jmp_buf)); // create a local copy since the original will be destroyed
...
TC_API int32 privateHeapSetJump(Heap m, const char* file, int32 line) {
	int32 len = xstrlen(file);
	xstrcpy(m->ex.setjmpFile, max32(0,
									len - (sizeof(m->ex.setjmpFile) - 1)) + (char*)
			file); // if src is bigger than the buffer, copy the end of the string (the filename is more important than the path)
...
static struct htmElem* htmPut(uint32 addr, int line, const char* src) {
	...
	if ((np = htmGet(addr)) == NULL) { // duplicated keys not allowed
		...
		if (memH) {
			...
			np->count = ++alcount;
			len = xstrlen(src);
			xstrcpy(np->src, max32(0, len - (sizeof(np->src) - 1)) + (char*)
					src); // if src is bigger than the buffer, copy the end of the string (the filename is more important than the path)
...
uint8* verifyMemMarks(void* p, char* msg, uint32* _size, bool replaceMarks, const char* file,
	...
	if (p) {
		...
		if (*(ptr - 1) != 0x01 || *(ptr - 2) != 0x01 || *(ptr - 3) != 0x01 || *(ptr - 4) != 0x01) {
			...
			debug("%s - not a memory chunk or chunk's start was overwritten: %lx - %d. %s at %s, line %d", msg,
				  (long)p, (int) * (ptr - 1), e ? "allocated" : "freed", e ? e->src : file,
			...
			if (*(ptr + size) != 0x01 || *(ptr + size + 1) != 0x01) {
				struct htmElem* e = htmGet((uint32)p);
				debug("%s - chunk's end was overwritten: %lx (%ld) - %d. %s at %s, line %d", msg, (long)p,
					  (long)size, (int) * (ptr + size), e ? "allocated" : "freed", e ? e->src : file,
					  e ? e->line : line); // this message will be printed inconditionally
				return null;
...
#ifdef WIN32
	debug("%s(%d) RETURNING NULL TO %s, line %d. Free memory: %d, avail per process: %d", func,
		  (int)origSize, file, line, (int)getFreeMemory(false), (int)getFreeMemory(true));
#else
	debug("%s(%d) RETURNING NULL TO %s, line %d. Free memory: %d", func, (int)origSize, file, line,
	...
	if (leakCheckingEnabled) { // no threads are running at this point
		...
		if (allocCount != freeCount) {
			debug("\nMemory leaked\n Number of xmallocs: %d\n Number of xfrees: %d\nList of places where the leaks were xmalloc'd:",
				  (int)allocCount, (int)freeCount);    // guich@570_94
		}
#else
		if (totalAllocated != 0) {
			debug("\nTotal memory leaked: %d bytes\n Number of xmallocs: %d\n Number of xfrees: %d\nList of places where the leaks were xmalloc'd:",
				  (int)totalAllocated, (int)allocCount, (int)freeCount);    // guich@570_94
		}
#endif
