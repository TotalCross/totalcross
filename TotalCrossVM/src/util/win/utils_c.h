...
static int32 privateGetFreeMemory(bool maxblock) {
	...
	result = maxblock ? ms.dwTotalVirtual :
			 ms.dwAvailVirtual; // guich@tc115_3: now using dwTotalVirtual instead of dwAvailPhys
	return result;
#else
	MEMORYSTATUSEX ms = { 0 };
	ms.dwLength = sizeof(MEMORYSTATUSEX);
	GlobalMemoryStatusEx(&ms);
	return (int32)(ms.ullAvailVirtual > INT_MAX ? ms.ullAvailVirtual - INT_MAX :
				   ms.ullAvailVirtual); // although the correct would be to use INT_MAX if above 2GB, we use a diff to let the user know that the memory is decreasing.
#endif
}
...
static Err privateListFiles(TCHARP path, int32 slot, TCHARPs** list, int32* count, Heap h,
							int32 options) {
...
static void privateGetDateTime(int32* year, int32* month, int32* day, int32* hour, int32* minute,
