...
VoidP privateLoadLibrary(CharP libName) {
	...
	if (library == null && strEq(libName, "litebase")) {
		...
		if (GetEnvironmentVariable(TEXT("TOTALCROSS3_HOME"), litebasePath, MAX_PATHNAME) != 0) {
			tcscat(litebasePath,
				   TEXT("/dist/vm/win32")); //flsobral@tc120_18: fixed path of LitebaseLib.tcz on Win32. Applications should now able to run from anywhere, as long as the Litebase and TotalCross home paths are set.
