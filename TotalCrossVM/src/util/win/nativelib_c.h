...
static VoidP
tryAt(CharP prefix, CharP prefix2, CharP lib, CharP suffix)
{
...
VoidP
privateLoadLibrary(CharP libName)
{
  HINSTANCE library;
  ...
  if (library == null && strEq(libName, "litebase")) {
    ...
    if (GetEnvironmentVariable(
          TEXT("TOTALCROSS3_HOME"), litebasePath, MAX_PATHNAME) != 0) {
      tcscat(litebasePath,
             TEXT("/dist/vm/win32")); // flsobral@tc120_18: fixed path of
                                      // LitebaseLib.tcz on Win32. Applications
                                      // should now able to run from anywhere,
                                      // as long as the Litebase and TotalCross
                                      // home paths are set.
  ...
  //   if (library == null && isWindowsMobile) // flsobral@tc113_27: WindowsCE
  //   doesn't seem to understand paths starting with two slashes.
  //      library = tryAt("//TotalCross/","",libName,".dll");
  return library;
}

void
privateUnloadLibrary(VoidP libPtr)
{
...
VoidP
privateGetProcAddress(const VoidP module, const CharP funcName)
{
#ifdef WINCE
