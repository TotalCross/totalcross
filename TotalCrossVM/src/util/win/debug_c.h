...
SearchWindowProc(HWND hwnd, LPARAM lParam)
{
  ...
  if (lstrcmpi(title, L"ALERT") == 0) {
    ...
    if (lstrcmpi(title, L"ALERT") !=
        0) { // if there's an alert and its not the active window, close it
...
#elif !defined(WP8)
  MessageBox(
    mainHWnd, str, TEXT("ALERT"), MB_OK | MB_TOPMOST | MB_SETFOREGROUND);
#else
