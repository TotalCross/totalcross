...
extern "C"
{
  ...
  /// Gets the pointer of a function, given its module and name. If module is
  /// null, the function is searched in the current module.
  ...
  /// Calls the HandleEvent function for each library. Returns true if the event
  /// was handled in one of them and false otherwise.
  bool handleEvent(VoidP event);
  /// Search in the attached libraries if one of them has the given function
  /// name
  ...
  typedef struct
  {
    VoidP handle;
    getProcAddressFunc
      getProcAddress; // Palm OS dlls must request the address of each API
                      // function, and to do this they require getProcAddress
    alertFunc alert;
    Context currentContext;
    CharP commandLine; // the command line passed to the executable that called
                       // this dll
#ifdef WIN32
    HWND mainHWnd;
#endif
  ...
  /// Must be implemented by a native library. Returns true if library
  /// initialization was successful.
  typedef bool (*NativeLibOpenFunc)(OpenParams params);
  /// May be implemented by a native library. Called by the vm when it is
  /// finishing.
  typedef void (*NativeLibCloseFunc)();
  /// May be implemented by a native library. Called by the vm to let the
  /// library handle an event. Must return true if the event was handled, false
  /// otherwise.
