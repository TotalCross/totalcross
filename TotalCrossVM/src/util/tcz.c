...
static int32 callReadTCZ(int32 apkIdx, uint8* buf, int32 offset, int32 length) {
	JNIEnv* env = getJNIEnv();
	jbyteArray jbytesP = (*env)->NewByteArray(env,
	...
	if (ret > 0) {
		jbyte* jbytes = (*env)->GetByteArrayElements(env, jbytesP,
...
static bool tczReadMore(TCZFile f) {
	...
	if (f->expectedFilePos !=
		f->header->realFilePos) { // if the file position has changed by some other instance, reposition it
...
TCZFile tczFindName(TCZFile tcz,
					CharP name) { // locates the name and also positions the stream at the place to start reading it
	TCZFile ntcz = null;
	int32 pos;
	LOCKVAR(tcz);
...
#endif
		&& version !=
		TCZ_VERSION) { // guich@tc110_70: check for version mismatch. font files doesn't care if the tcz changed
		alert("TCZ version mismatch for %s (defined: %d, loaded: %d). Recompile and deploy your app with the new SDK.",
			  fileName, TCZ_VERSION, version);
		return null;
	}
...
extern void readConstantPool(Context currentContext, ConstantPool t, TCZFile tcz,
							 Heap heap); // class.c
...
TCZFile tczGetFile(CharP filename,
				   bool strict) { // fdie@ strict mode prevents mainClass package name based resolving
	TCZFile found = null;
	VoidPs* list, *head;
	list = head = openTCZs;
	if (head != null)
