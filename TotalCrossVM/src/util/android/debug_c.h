...
privateDebug(char* str)
{
  if (strEq(str, ALTERNATIVE_DEBUG)) { // is the user asking to change the mode?
    ...
    if (!fdebug) {
      ...
      fdebug = fopen(debugPath,
                     "ab+"); // flsobral@tc110: replaced mode "wb" with "ab+".
    }
    if (fdebug) {
...
privateAlert(CharP str)
{
  ...
  // must check if the alert is already being show (maybe by the system itself)
  // prior to calling another alert
