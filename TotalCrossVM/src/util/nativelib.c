...
VoidP
loadLibrary(const char* libName)
{
  return privateLoadLibrary((char*)libName);
}

void
unloadLibrary(VoidP libPtr)
{
  privateUnloadLibrary(libPtr);
}

VoidP
getProcAddress(const VoidP module, const char* funcName)
{
...
bool
attachNativeLib(Context currentContext, CharP name)
{
  NativeLib lib;
  TOpenParams params;
...
attachNativeLib(Context currentContext, CharP name)
{
  ...
  if (h != null && (lib = newX(NativeLib)) != null) {
    ...
    lib->HandleEvent =
      (NativeLibHandleEventFunc)getProcAddress(h, "HandleEvent");
    if (lib->LibOpen) {
      ...
      throwException(
        currentContext,
        RuntimeException,
        "Native library %s does not implement the 'LibOpen' function.",
        name);
    }
    if (ok) {
      openNativeLibs = VoidPsAdd(openNativeLibs, lib, null);
...
VoidP
findProcAddress(CharP funcName, uint32* ref)
{
  VoidPs *list, *head;
  head = list = openNativeLibs;
  if (list != null)
