...
static int32
privateGetFreeMemory(bool maxblock)
{
  ...
  if (host_statistics(
        host_port, HOST_VM_INFO, (host_info_t)&vm_stat, &host_size) !=
      KERN_SUCCESS) {
    return 0;
  }
...
#define BUFFER_SIZE                                                            \
  338 /* We are bothered about only the first 338 bytes of the /proc/meminfo   \
         file */
#define PROC_MEM_FILE "/proc/meminfo"

/* Extract numbers from a string between the start and end indices */
static unsigned long
extract_number(char* str, int start, int end)
{
  int i, j;
  char buf[end - start];
...
privateGetFreeMemory(bool maxblock)
{
  ...
  if ((in = open(PROC_MEM_FILE, O_RDONLY)) < 0) {
    return 2048 *
           1024; // 2mb at least, or some litebase tests will just skip over.
  }
...
static bool
pfileIsDir(TCHARP dir, TCHARP file)
{
#ifdef darwin
                 ...
                 int32 options)
{
  ...
  dir = opendir(path);
  if (!dir) {
    return NO_ERROR; // guich@tc200: don't throw error, let the user handle it
                     // when opening the file. (issue 2254)
  }
  if (recursive) {
  ...
  while ((entry = readdir(dir))) {
    if ((entry->d_name[0] != '.') ||
      ...
      TCHARP fileName = (TCHARP)heapAlloc(
        h,
        sizeof(TCHAR) *
      ...
      if (isDir) {
        tcscat(fileName, TEXT("/"));
               ...
               // once. so, check if it already exists
      for (exists = false, search = *list, si = 0; si < *count && !exists;
           search = search->next, si++) {
      ...
      *list =
        TCHARPsAdd(*list,
                   fileName,
                   h); // add entry to list - if recursive, include the pathname
      (*count)++;
                   ...
                   int32* millis)
{
  ...
  // "The use of the timezone structure (2nd arg of gettimeofday) is obsolete;
  // the tz argument should normally be specified as NULL" as explained here
