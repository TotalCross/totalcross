  ...
  TC_API Hashtable htNew(int32 count, Heap heap)
{
  ...
  iht.size = 0;
  iht.heap = heap;
  iht.items = heap ? (HtEntry**)heapAlloc(heap, count * TSIZE)
                   : (HtEntry**)xmalloc(count * TSIZE);
...
TC_API int32
htGet32(Hashtable* iht, HTKey key)
{
...
TC_API int32
htGet32Inv(Hashtable* iht, HTKey key)
{
  HtEntry* h = htGet(iht, key);
...
TC_API VoidP
htGetPtr(Hashtable* iht, HTKey key)
{
  HtEntry* h = htGet(iht, key);
  return h ? h->ptr : null;
}

/* Once the number of elements gets above the load factor, rehashes the
 hashtable. Does NOT work when the hashtable has a heap.
 */
static bool
htRehash(Hashtable* table)
{
  if (table->heap == null) {
    ...
    int32 newCapacity =
      oldCapacity
      << 1; // in C: for faster hashes, we must always double the hashtable.
            // (((oldCapacity << 1) + oldCapacity) >> 1) + 1;
      ...
      bool ignoreIfNotNew)
{
  ...
  e = iht->heap ? (HtEntry*)heapAlloc(iht->heap, sizeof(HtEntry))
                : (HtEntry*)xmalloc(sizeof(HtEntry));
  if (!e) {
    return false;
  }
  e->key = key;
...
TC_API bool
htPut32(Hashtable* iht, HTKey key, int32 value)
{
  return htPut(iht, key, value, null, true, false);
}

TC_API bool
htPutPtr(Hashtable* iht, HTKey key, void* value)
{
  return htPut(iht, key, 0, value, false, false);
}

TC_API bool
htInc(Hashtable* iht, HTKey key, int32 incValue)
{
  return htPut32(iht, key, incValue + htGet32(iht, key));
}

TC_API bool
htPut32IfNew(Hashtable* iht, HTKey key, int32 value)
{
  return htPut(iht, key, value, null, true, true);
}

TC_API bool
htPutPtrIfNew(Hashtable* iht, HTKey key, void* value)
{
...
TC_API void
htRemove(Hashtable* iht, HTKey key)
{
 ...
 */
TC_API void
htFree(Hashtable* iht, VisitElementFunc freeElement)
{
  if (iht) {
    HtEntry** tab = iht->items;
    HtEntry *e, *next;
...
void
htTraverse(Hashtable* iht, VisitElementFunc visitElement)
{
  HtEntry** tab = iht->items;
  HtEntry* e;
  int32 n = iht->hash;
...
void
htTraverseWithKey(Hashtable* iht, VisitElementKeyFunc visitElement)
{
  HtEntry** tab = iht->items;
  HtEntry* e;
  int32 n = iht->hash;
...
TC_API Stack
newStack(int32 elemCount, int32 elemSize, Heap defHeap)
{
  Stack s;
...
TC_API void
stackPush(Stack s, VoidP p)
{
  if (s->pos == s->blockEnd) {
    if (s->current == null ||
        s->current->next == s->head) { // reached the end? create a new block
      ...
      s->current = s->current->next; // reuse a previously allocated block
    }
...
void*
privateNewArray(int32 sizeofElem,
                int32 len,
                Heap mp,
                const char* file,
                int line)
{
  if (len) {
    ...
    if (p) {
      ...
      debug("newArray(%d,%d,%X) called from %s (%d)",
            sizeofElem,
            len,
            mp,
            file,
...
VoidP
list2array(VoidP li, int32 sizeofElem, int32 extra)
{
