...
static bool htRehash(Hashtable* table) {
	if (table->heap == null) {
		...
		int32 newCapacity = oldCapacity <<
							1; // in C: for faster hashes, we must always double the hashtable. (((oldCapacity << 1) + oldCapacity) >> 1) + 1;
 ...
 */
static bool htPut(Hashtable* iht, HTKey key, int32 i32, VoidP ptr, bool isI32,
				  bool ignoreIfNotNew) {
	HtEntry* e;
	int32 index;
	...
	e = iht->heap ? (HtEntry*)heapAlloc(iht->heap,
										sizeof(HtEntry)) : (HtEntry*)xmalloc(sizeof(HtEntry));
	if (!e) {
		return false;
	}
	e->key = key;
