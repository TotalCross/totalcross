...
bool initWinsock() {
	...
	if (wsLib != null) { // successfully loaded
		...
		WSALookupServiceBeginProc  = (LPFN_WSALOOKUPSERVICEBEGIN) GetProcAddress(wsLib,
									 _WSALookupServiceBegin_);
		WSALookupServiceNextProc   = (LPFN_WSALOOKUPSERVICENEXT) GetProcAddress(wsLib,
									 _WSALookupServiceNext_);
		WSALookupServiceEndProc    = (LPFN_WSALOOKUPSERVICEEND) GetProcAddress(wsLib,
									 TEXT("WSALookupServiceEnd"));
		WSASetServiceProc          = (LPFN_WSASETSERVICE) GetProcAddress(wsLib, _WSASetService_);
	}
#if defined (WINCE)
	if (ossvcsLib != null) {
		pGetWirelessDevices  = (GetWirelessDevicesProc) GetProcAddress(ossvcsLib,
							   MAKEINTRESOURCE(GetWirelessDevice_ORDINAL));
		pChangeRadioState    = (ChangeRadioStateProc)   GetProcAddress(ossvcsLib,
							   MAKEINTRESOURCE(ChangeRadioState_ORDINAL));
		pFreeDeviceList      = (FreeDeviceListProc)     GetProcAddress(ossvcsLib,
							   MAKEINTRESOURCE(FreeDeviceList_ORDINAL));
	}
#endif
#endif
	return true;
