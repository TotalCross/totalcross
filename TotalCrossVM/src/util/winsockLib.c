...
initWinsock()
{
  ...
  if (wsLib == null) {
    ...
    GetVersionEx(&osvi);
    if ((osvi.dwMajorVersion * 100 + osvi.dwMinorVersion) >=
        420) { // try to load ws2.dll
  ...
  if (wsLib != null) { // successfully loaded
    ...
    gethostbynameProc =
      (LPFN_GETHOSTBYNAME)GetProcAddress(wsLib, TEXT("gethostbyname"));
    WSAGetLastErrorProc =
      (LPFN_WSAGETLASTERROR)GetProcAddress(wsLib, TEXT("WSAGetLastError"));
    htonsProc = (LPFN_HTONS)GetProcAddress(wsLib, TEXT("htons"));
    ioctlsocketProc =
      (LPFN_IOCTLSOCKET)GetProcAddress(wsLib, TEXT("ioctlsocket"));
    ...
    closesocketProc =
      (LPFN_CLOSESOCKET)GetProcAddress(wsLib, TEXT("closesocket"));
    ...
    gethostbyaddrProc =
      (LPFN_GETHOSTBYADDR)GetProcAddress(wsLib, TEXT("gethostbyaddr"));
    gethostnameProc =
      (LPFN_GETHOSTNAME)GetProcAddress(wsLib, TEXT("gethostname"));
    ...
    getsocknameProc =
      (LPFN_GETSOCKNAME)GetProcAddress(wsLib, TEXT("getsockname"));

    WSALookupServiceBeginProc = (LPFN_WSALOOKUPSERVICEBEGIN)GetProcAddress(
      wsLib, _WSALookupServiceBegin_);
    WSALookupServiceNextProc =
      (LPFN_WSALOOKUPSERVICENEXT)GetProcAddress(wsLib, _WSALookupServiceNext_);
    WSALookupServiceEndProc = (LPFN_WSALOOKUPSERVICEEND)GetProcAddress(
      wsLib, TEXT("WSALookupServiceEnd"));
    WSASetServiceProc =
      (LPFN_WSASETSERVICE)GetProcAddress(wsLib, _WSASetService_);
  }
#if defined(WINCE)
  if (ossvcsLib != null) {
    pGetWirelessDevices = (GetWirelessDevicesProc)GetProcAddress(
      ossvcsLib, MAKEINTRESOURCE(GetWirelessDevice_ORDINAL));
    pChangeRadioState = (ChangeRadioStateProc)GetProcAddress(
      ossvcsLib, MAKEINTRESOURCE(ChangeRadioState_ORDINAL));
    pFreeDeviceList = (FreeDeviceListProc)GetProcAddress(
      ossvcsLib, MAKEINTRESOURCE(FreeDeviceList_ORDINAL));
  }
#endif
#endif
  return true;
