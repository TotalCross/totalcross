...
#define SWAP16_FORCED(n)                                                       \
  (((((unsigned int)n) << 8) & 0xFF00) | ((((unsigned int)n) >> 8) & 0x00FF))

#define SWAP32_FORCED(n)                                                       \
...
#define __bridge
  ...
  /////////////////////////////////////////////////////////////////////////
  // Basic types
  // explicit signed types (some compilers consider "char" as unsigned by
...
#if !defined(__HSDATATYPES__) &&                                               \
  !(defined(darwin) &&                                                         \
    defined(__OBJC__)) // Palm OS defines these as non-ptr types, so we can't
                       // define them as ptr ones.
...
#define NO_ERROR 0
  ...
  typedef int64 TValue64; // guich@tc111_7: must use int64, because double
                          // assignments validates the number.
...
#define xstrcmp(str1, str2) strcmp(str1, str2)
  CharP xstrncpy(
    CharP dest,
    CharP src,
    int32 len); // this one makes sure that the string is terminated with \0
...
#define xmoveptr(dest, src) xmove4(dest, src);
#else
