...
/****** IMPORTANT: IF YOU GET +30 LINK ERRORS WHEN BUILDING THIS FILE, YOU MUST
 * FORCE THE REBUILD ALL. THIS IS A BUG OF VC6 *******/

#include "Conduit.h"

#define UNDEFINED_ERROR                                                        \
  16394 // Palm error code that means we have no idea of what went wrong.
...
1. ADD C:\TOTALCROSS and c:\PALM to path
  ...
  To build the project, you need to setup eMbedded Visual Tools (or PocketPC
SDK), and setup directories in Visual C++ 6.0. Add ActiveSync inc\ and lib\
folders. Use menu Tools -> Options -> directories tab. In my case, I added
C:\Windows CE Tools\wce300\MS Pocket PC\support\ActiveSync\inc (and \lib)
folders.
********************/
...
extern "C"
{
  ...
  int32 listDatabases(TCHARP searchPath,
                      HandlePDBSearchProcType proc,
                      void* userVars,
                      byte recursive);
}
...
typedef int(__stdcall* CmSetCreatorFileProc)(const char* pCreatorID,
                                             const TCHAR* pFile);
typedef int(__stdcall* CmGetCreatorFileProc)(const char* pCreatorID,
                                             TCHAR* pFile,
                                             int* piSize);
typedef int(__stdcall* CmConvertCreatorIDToStringProc)(DWORD dwID,
                                                       TCHAR* pString,
                                                       int* piSize);
typedef int(__stdcall* CmSetCreatorTitleProc)(const char* pCreatorID,
                                              const TCHAR* pTitle);
typedef int(__stdcall* CmSetCreatorPriorityProc)(const char* pCreatorID,
                                                 DWORD dwPriority);
typedef int(__stdcall* CmSetCreatorDirectoryProc)(const char* pCreatorID,
                                                  const TCHAR* pDirectory);
typedef int(__stdcall* CmGetCreatorDirectoryProc)(const char* pCreatorID,
                                                  TCHAR* pFile,
                                                  int* piSize);
typedef int(__stdcall* CmSetCreatorNameProc)(const char* pCreatorID,
...
typedef long (*LogAddEntryProc)(LPCTSTR pszEntry,
                                Activity act,
                                BOOL bTimeStamp);
...
typedef SInt32 (*SyncReadOpenDbInfoProc)(SyncReadOpenDbInfoParams& rParam,
                                         SyncDatabaseInfoType& rInfo);
typedef SInt32 (*SyncOpenDBProc)(const char* pName,
                                 SInt32 nCardNum,
                                 HSByte& rHandle,
                                 HSByte openMode);
...
typedef SInt32 (*SyncFindDbByTypeCreatorProc)(
...
extern "C"
{
  ...
  SYNC_API bool LibOpen(OpenParams params)
  {
    ...
    if ((rapiModule = LoadLibrary("rapi.dll")) == null) {
      if (params->commandLine[0] == 0 || params->commandLine[2] == 'p' ||
          params->commandLine[2] ==
            'P') { // if no command line passed or targetting palm, we don't
                   // need ActiveSync
        return true;
      }
      throwException(params->currentContext,
                     RuntimeException,
                     "rapi.dll was not found. Was ActiveSync installed?");
      return false;
    }

    procCeRapiInit = (CeRapiInitProc)GetProcAddress(rapiModule, "CeRapiInit");
    procCeRapiUninit =
      (CeRapiUninitProc)GetProcAddress(rapiModule, "CeRapiUninit");
    procCeGetFileAttributes = (CeGetFileAttributesProc)GetProcAddress(
      rapiModule, "CeGetFileAttributes");
    procCeFindClose =
      (CeFindCloseProc)GetProcAddress(rapiModule, "CeFindClose");
    procCeFindNextFile =
      (CeFindNextFileProc)GetProcAddress(rapiModule, "CeFindNextFile");
    procCeFindFirstFile =
      (CeFindFirstFileProc)GetProcAddress(rapiModule, "CeFindFirstFile");
    procCeGetLastError =
      (CeGetLastErrorProc)GetProcAddress(rapiModule, "CeGetLastError");
    procCeSetFilePointer =
      (CeSetFilePointerProc)GetProcAddress(rapiModule, "CeSetFilePointer");
    procCeReadFile = (CeReadFileProc)GetProcAddress(rapiModule, "CeReadFile");
    procCeWriteFile =
      (CeWriteFileProc)GetProcAddress(rapiModule, "CeWriteFile");
    procCeSetEndOfFile =
      (CeSetEndOfFileProc)GetProcAddress(rapiModule, "CeSetEndOfFile");
    procCeCloseHandle =
      (CeCloseHandleProc)GetProcAddress(rapiModule, "CeCloseHandle");
    procCeDeleteFile =
      (CeDeleteFileProc)GetProcAddress(rapiModule, "CeDeleteFile");
    procCeCreateFile =
      (CeCreateFileProc)GetProcAddress(rapiModule, "CeCreateFile");
    procCeGetFileSize =
      (CeGetFileSizeProc)GetProcAddress(rapiModule, "CeGetFileSize");
    procCeMoveFile = (CeMoveFileProc)GetProcAddress(rapiModule, "CeMoveFile");
    procCeRemoveDirectory =
      (CeRemoveDirectoryProc)GetProcAddress(rapiModule, "CeRemoveDirectory");
    procCeSetFileAttributes = (CeSetFileAttributesProc)GetProcAddress(
      rapiModule, "CeSetFileAttributes");
    procCeFindAllFiles =
      (CeFindAllFilesProc)GetProcAddress(rapiModule, "CeFindAllFiles");

    procCeCreateProcess =
      (CeCreateProcessProc)GetProcAddress(rapiModule, "CeCreateProcess");
    procCeRapiGetError =
      (CeRapiGetErrorProc)GetProcAddress(rapiModule, "CeRapiGetError");

    if (!procCeRapiInit || !procCeRapiUninit || !procCeGetFileAttributes ||
        !procCeFindClose || !procCeFindNextFile || !procCeFindFirstFile ||
        !procCeGetLastError || !procCeSetFilePointer || !procCeReadFile ||
        !procCeWriteFile || !procCeSetEndOfFile || !procCeCloseHandle ||
        !procCeDeleteFile || !procCeCreateFile || !procCeGetFileSize ||
        !procCeMoveFile || !procCeRemoveDirectory || !procCeFindAllFiles ||
        !procCeSetFileAttributes || !procCeCreateProcess ||
        !procCeRapiGetError) {
      FreeLibrary(rapiModule);
      throwException(params->currentContext,
                     RuntimeException,
                     "One or more rapi.dll functions were missing. Please be "
                     "sure to be using the correct ActiveSync version.");
      return false;
    }
    return true;
  }
  ...
  static HMODULE loadPalmDll(CharP name)
  {
    ...
    if (ret == NO_ERROR) {
      ...
      size = MAX_PATHNAME - 1;
      ret =
        RegQueryValueEx(handle, "DesktopPath", NULL, NULL, (uint8*)path, &size);
      if (ret == NO_ERROR) {
        xstrcat(path, "\\");
      }
      RegCloseKey(handle);
  ...
  static bool loadHotSyncCfgLibraries()
  {
    ...
    // now get the procs
    procHsRefreshConduitInfo = (HsRefreshConduitInfoProc)GetProcAddress(
      hsApiModule, "HsRefreshConduitInfo");
    procCmSetCreatorFile =
      (CmSetCreatorFileProc)GetProcAddress(condMgrModule, "CmSetCreatorFile");
    procCmGetCreatorFile =
      (CmGetCreatorFileProc)GetProcAddress(condMgrModule, "CmGetCreatorFile");
    procCmConvertCreatorIDToString =
      (CmConvertCreatorIDToStringProc)GetProcAddress(
        condMgrModule, "CmConvertCreatorIDToString");
    procCmSetCreatorTitle =
      (CmSetCreatorTitleProc)GetProcAddress(condMgrModule, "CmSetCreatorTitle");
    procCmSetCreatorPriority = (CmSetCreatorPriorityProc)GetProcAddress(
      condMgrModule, "CmSetCreatorPriority");
    procCmSetCreatorDirectory = (CmSetCreatorDirectoryProc)GetProcAddress(
      condMgrModule, "CmSetCreatorDirectory");
    procCmGetCreatorDirectory = (CmGetCreatorDirectoryProc)GetProcAddress(
      condMgrModule, "CmGetCreatorDirectory");
    procCmSetCreatorName =
      (CmSetCreatorNameProc)GetProcAddress(condMgrModule, "CmSetCreatorName");
    procCmInstallCreator =
      (CmInstallCreatorProc)GetProcAddress(condMgrModule, "CmInstallCreator");
    procCmRemoveConduitByCreatorID =
      (CmRemoveConduitByCreatorIDProc)GetProcAddress(
        condMgrModule, "CmRemoveConduitByCreatorID");

    if (!procHsRefreshConduitInfo || !procCmSetCreatorFile ||
        !procCmGetCreatorFile || !procCmConvertCreatorIDToString ||
        !procCmSetCreatorTitle || !procCmSetCreatorPriority ||
        !procCmSetCreatorDirectory || !procCmSetCreatorName ||
        !procCmInstallCreator || !procCmGetCreatorDirectory ||
        !procCmRemoveConduitByCreatorID) {
      if (procHsRefreshConduitInfo == null)
        MessageBox(null,
                   "Could not load a required function from hsapid.dll.\nPalm "
                   "Desktop 4.14 / HotSync 6.0.1 or newer are required.",
                   "TotalCross Sync",
                   0);
      else
        MessageBox(null,
                   "Could not load a required function from condmgr.dll.\nPalm "
                   "Desktop 4.14 / HotSync 6.0.1 or newer are required.",
                   "TotalCross Sync",
                   0);
      goto error;
...
#define RECEIVE_BUFSIZE                                                        \
  65400 // making it larger will also make the dll larger - CANNOT BE LARGER
        // THAN 65535 !
...
void
swapDatabaseHeader2(DatabaseHeader* src, DatabaseHeader* dst)
{
...
static bool
clcAddFile(TCHAR* fileName, void* userVars)
{
  ...
  if (!PDBCreateFile(fileName,
                     false,
                     true,
                     &fileRef)) { // createIt - false, readOnly - true
    goto finish;
  }
  ...
  xstrprintf(
    vars->stringsP, "%s.%s.%s", vars->uniName, vars->crtrStr, vars->typeStr);
  vars->stringsCount++;
...
#define ggetUInt32(b)                                                          \
  (uint32)((uint32)((b)[0]) << 24 | (uint32)((b)[1]) << 16 |                   \
           (uint32)((b)[2]) << 8 | (uint32)((b)[3]))

#define sizeofPDBHandle(syncTarget)                                            \
  (syncTarget == TARGETING_PALMDESKTOP ? sizeof(HSByte) : sizeof(TPDBFile))
...
static CCallModuleParams*
prepareParams(int32 code, void* inBuf, int32 inBufSize)
{
...
static bool
queryRegistry(HKEY key, TCHAR* subkey, TCHAR* name, char* buf, uint32 size);

static int
execTC(char* param, char* cmdLine, char* className)
{
  HMODULE tcvmModule;
  char buf[256];
  char scr[100];
  TRACE("execTC");
  if (!queryRegistry(HKEY_CURRENT_USER,
                     (cmdLine[1] == 'c')
                       ? "Software\\TotalCross\\ConduitCfgRect"
                       : "Software\\TotalCross\\ConduitSyncRect",
                     className,
                     scr,
                     100)) {     // guich@tc110_27
  ...
  if ((tcvmModule = LoadLibrary("tcvm.dll")) != null) {
    ...
    if ((procExecuteProgram = (executeProgramProc)GetProcAddress(
           tcvmModule, "executeProgram")) != null) {
                    ...
                    uint32 size)
{
  ...
  return CeRegOpenKeyEx(key, subkey, 0, KEY_READ, &handle) == NO_ERROR &&
         CeRegQueryValueEx(
           handle, name, null, &type, (LPBYTE)buf, (LPDWORD)&size) ==
           NO_ERROR &&
...
queryRegistry(HKEY key, TCHAR* subkey, TCHAR* name, char* buf, uint32 size)
{
  ...
  return RegOpenKeyEx(key, subkey, 0, KEY_READ, &handle) == NO_ERROR &&
         RegQueryValueEx(
           handle, name, NULL, &type, (byte*)buf, (LPDWORD)&size) == NO_ERROR &&
         RegCloseKey(handle) == NO_ERROR;
}

static bool
setRegistry(HKEY key, TCHAR* subkey, TCHAR* name, char* buf)
{
  HKEY handle;
  TRACE("setRegistry");
  ...
  return RegOpenKeyEx(key, subkey, 0, KEY_ALL_ACCESS, &handle) == NO_ERROR &&
         RegSetValueEx(handle,
                       ...
                       (byte*)buf,
                       (DWORD)(xstrlen(buf) + 1)) ==
           NO_ERROR && // store the length
         RegCloseKey(handle) == NO_ERROR;
}

static bool
deleteRegistry(HKEY key, TCHAR* subkey, TCHAR* name)
{
  HKEY handle;
...
static bool
setEnviromentVariable(HKEY key, TCHAR* subkey)
{
  HKEY tcKey;
  CharP vmPathP;
  ...
  if (RegOpenKeyEx(key, subkey, 0, KEY_ALL_ACCESS, &tcKey) == NO_ERROR) {
    if ((err = RegQueryValueEx(
           tcKey, "PATH", 0, null, (LPBYTE)pathVar, &pathVarSize)) ==
        NO_ERROR) {
      ...
      ok = xstrstr(pathLower, vmPathLower) != null ||
           xstrstr(pathLower, "totalcross_home") != null;
    }
    ...
    if (!ok) {
      ...
      if (RegSetValueEx(tcKey,
                        "PATH",
                        0,
                        REG_EXPAND_SZ,
                        (BYTE*)pathVar,
                        xstrlen(pathVar)) == NO_ERROR) {
        ok = true;
      }
    }
    SendMessage(
      ...
      WM_WININICHANGE,
      0,
      (LPARAM)&szEnv); // flsobral@tc114_14: Wait until all applications are
                       // notified about the registry update.
  }
  RegCloseKey(tcKey);
  return ok;
}
...
HRESULT
CreateLink(char* path, char* args, char* workingDir, char* linkPath)
{
  HRESULT hres;
  IShellLink* psl;
  TRACE("CreateLink");
...
static bool
isSyncingEnabled(CharP className)
{
  DWORD status = 0;
  TRACE("isSyncingEnabled");
  queryRegistry(HKEY_CURRENT_USER,
                "Software\\TotalCross\\EnableSync",
                className,
                (char*)&status,
                4);
  return status == 1;
...
static Err
getClassName(uint32 ucrid,
             char* className,
             char* path,
             CfgConduitInfoType* info)
{
  TRACE("getClassName");
  if (loadHotSyncCfgLibraries()) {
    ...
    *className = 0;
    ...
    if (info) {
      TCHARP2CharPBuf(info->szUser, getUserName());
    }
                                       ...
                                       CSyncProperties& sync)
{
  ...
  // now get procs
  procLogAddEntry =
    (LogAddEntryProc)GetProcAddress(hsLog20Module, "LogAddEntry");
  procSyncUnRegisterConduit = (SyncUnRegisterConduitProc)GetProcAddress(
    sync20Module, "SyncUnRegisterConduit");
  procSyncYieldCycles =
    (SyncYieldCyclesProc)GetProcAddress(sync20Module, "SyncYieldCycles");
  procSyncReadUserID =
    (SyncReadUserIDProc)GetProcAddress(sync20Module, "SyncReadUserID");
  procSyncGetHHOSVersion =
    (SyncGetHHOSVersionProc)GetProcAddress(sync20Module, "SyncGetHHOSVersion");
  procSyncRegisterConduit = (SyncRegisterConduitProc)GetProcAddress(
    sync20Module, "SyncRegisterConduit");
  procSyncCloseDB =
    (SyncCloseDBProc)GetProcAddress(sync20Module, "SyncCloseDB");
  procSyncReadOpenDbInfo =
    (SyncReadOpenDbInfoProc)GetProcAddress(sync20Module, "SyncReadOpenDbInfo");
  procSyncOpenDB = (SyncOpenDBProc)GetProcAddress(sync20Module, "SyncOpenDB");
  procSyncCreateDB =
    (SyncCreateDBProc)GetProcAddress(sync20Module, "SyncCreateDB");
  procSyncResetSyncFlags =
    (SyncResetSyncFlagsProc)GetProcAddress(sync20Module, "SyncResetSyncFlags");
  procSyncDeleteDB =
    (SyncDeleteDBProc)GetProcAddress(sync20Module, "SyncDeleteDB");
  procSyncGetDBRecordCount = (SyncGetDBRecordCountProc)GetProcAddress(
    sync20Module, "SyncGetDBRecordCount");
  procSyncReadNextModifiedRec = (SyncReadNextModifiedRecProc)GetProcAddress(
    sync20Module, "SyncReadNextModifiedRec");
  procSyncWriteRec =
    (SyncWriteRecProc)GetProcAddress(sync20Module, "SyncWriteRec");
  procSyncReadRecordByIndex = (SyncReadRecordByIndexProc)GetProcAddress(
    sync20Module, "SyncReadRecordByIndex");
  procSyncDeleteRec =
    (SyncDeleteRecProc)GetProcAddress(sync20Module, "SyncDeleteRec");
  procSyncFindDbByTypeCreator = (SyncFindDbByTypeCreatorProc)GetProcAddress(
    sync20Module, "SyncFindDbByTypeCreator");
  procSyncCallRemoteModule = (SyncCallRemoteModuleProc)GetProcAddress(
    sync20Module, "SyncCallRemoteModule");

  if (!procLogAddEntry || !procSyncUnRegisterConduit || !procSyncYieldCycles ||
      !procSyncReadUserID || !procSyncGetHHOSVersion ||
      !procSyncRegisterConduit || !procSyncCloseDB || !procSyncReadOpenDbInfo ||
      !procSyncOpenDB || !procSyncCreateDB || !procSyncResetSyncFlags ||
      !procSyncDeleteDB || !procSyncGetDBRecordCount ||
      !procSyncReadNextModifiedRec || !procSyncWriteRec ||
      !procSyncReadRecordByIndex || !procSyncDeleteRec ||
      !procSyncFindDbByTypeCreator || !procSyncCallRemoteModule) {
    if (procLogAddEntry == null)
      MessageBox(null,
                 "Could not load a required function from hslog20.dll.\nPalm "
                 "Desktop 4.14 / HotSync 6.0.1 or newer are required.",
                 "TotalCross Sync",
                 0);
    else
      MessageBox(null,
                 "Could not load a required function FROM sync20.dll.\nPalm "
                 "Desktop 4.14 / HotSync 6.0.1 or newer are required.",
                 "TotalCross Sync",
                 0);
    return UNDEFINED_ERROR;
  ...
  if (queryRegistry(
        sync.m_hKey, sync.m_Registry, "File0", className, sizeof(className)) &&
      queryRegistry(sync.m_hKey,
                    sync.m_Registry,
                    "Directory",
                    classDir,
                    sizeof(classDir))) {
    ...
    if (isSyncingEnabled(className) &&
        (err = execTC(classPath, "/sp", className)) != 0) {
  ...
  xstrprintf(msg,
             "Conduit not properly registered, registering the conduit "
             "again will probably fix the problem");
...
__declspec(dllexport) long ConfigureConduit(CSyncPreference& pref)
{
  ...
  if (queryRegistry(
        pref.m_hKey, pref.m_Registry, "File0", className, sizeof(className)) &&
      queryRegistry(
        pref.m_hKey,
        pref.m_Registry,
        "Directory",
        classDir,
        sizeof(classDir))) { // guich@tc110_25: now using the directory
...
__declspec(dllexport) long CfgConduit(ConduitCfgEnum cfgType,
                                      void* pArgs,
                                      DWORD* pdwArgsSize)
{
  ...
  if (cfgType != eConfig1) {
    ...
    if ((err = getClassName(info->dwCreatorId, className, classDir, info)) ==
        CONDERR_NONE) {
      xstrprintf(fullPath,
                 "%s/%s",
                 classDir,
                 className); // guich@tc110_25: now using the classDir
                                          ...
                                          DWORD* pdwOutSize)
{
  ...
  //{char buf[100]; sprintf(buf, "ConduitInfoEnum: %d",infoType);
  // MessageBox(null,buf,"alert",0);}
  if (!pOut) {
  ...
  switch (infoType) {
    ...
    case eDefaultAction: {
      ...
      if ((err = getClassName(((ConduitRequestInfoType*)pInArgs)->dwCreatorId,
                              className,
                              null,
                              null)) == CONDERR_NONE) {
...
extern "C"
{
  static void invalidateRemotePDBFile(Context currentContext, TCObject obj)
  {
    ...
    *getStaticFieldInt(
      loadClass(currentContext, "totalcross.io.sync.RemotePDBFile", true),
      "idle") = true; // guich@tc114_95: must set to idle
  }
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisC_finishSync(NMParams p)
  { // totalcross/io/sync/Conduit native boolean finishSync();
    ...
    if (syncTarget == TARGETING_PALMDESKTOP) {
      ...
      CONDHANDLE* conduitHandle =
        (CONDHANDLE*)ARRAYOBJ_START(*conduitHandleObject);
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisC_log_s(NMParams p)
  { // totalcross/io/sync/Conduit native
    ...
    if (text == null) {
      ...
      if (syncTarget == TARGETING_PALMDESKTOP) {
        TCHAR szText[256];
        JCharP2TCHARPBuf(
          String_charsStart(text), String_charsLen(text), szText);
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisC_register_i(NMParams p)
  { // totalcross/io/sync/Conduit native
    // boolean register(int priority);
    TCObject conduit = p->obj[0];
    ...
    if (syncTarget & TARGETING_PALMDESKTOP) {
      ...
      char szCreatorA[5];
      JCharP2TCHARPBuf(String_charsStart(conduitName),
                       String_charsLen(conduitName),
                       szConduitNameT);
      JCharP2CharPBuf(String_charsStart(targetApplicationId),
                      String_charsLen(targetApplicationId),
                      szCreatorA);

      ok = setEnviromentVariable(
        HKEY_LOCAL_MACHINE,
      ...
      if (ok && loadHotSyncCfgLibraries()) {
        ...
        if (procCmInstallCreator(szCreatorA, CONDUIT_APPLICATION) == -1005)
          throwException(p->currentContext,
                         RuntimeException,
                         "Could not install the conduit, the creator id "
          ...
          ok =
            procCmSetCreatorName(szCreatorA, conduitDllPath) == SYNCERR_NONE &&
            procCmSetCreatorDirectory(szCreatorA, conduitAppDir) ==
              SYNCERR_NONE &&
    ...
    if (syncTarget & TARGETING_ACTIVESYNC) {
      ...
      TCHAR szCreatorT[5];
      JCharP2CharPBuf(String_charsStart(conduitName),
                      String_charsLen(conduitName),
                      szConduitNameA);
      JCharP2TCHARPBuf(String_charsStart(targetApplicationId),
                       String_charsLen(targetApplicationId),
                       szCreatorT);
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisC_unregister(NMParams p)
  { // totalcross/io/sync/Conduit native boolean unregister();
    TCObject conduit = p->obj[0];
    int32 syncTarget = *Conduit_syncTarget(conduit);
    int32 ret = 1;
    ...
    if (syncTarget & TARGETING_ACTIVESYNC) {
      ...
      JCharP2TCHARPBuf(String_charsStart(targetApplicationId),
                       ...
                       szCreatorT);
      ret = (ret == 0) ? ret
                       : deleteRegistry(HKEY_LOCAL_MACHINE,
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisC_initSync(NMParams p)
  { // totalcross/io/sync/Conduit native
    ...
    xmemmove(remoteAppPath,
             String_charsStart(targetAppPath),
             targetAppPathLen * sizeof(WCHAR));
    *(remoteAppPath + targetAppPathLen) = 0;

    conduitHandleObject =
      createByteArray(p->currentContext, sizeof(CONDHANDLE));
    CONDHANDLE* conduitHandle =
      (CONDHANDLE*)ARRAYOBJ_START(conduitHandleObject);
    ...
    if (syncTarget == TARGETING_PALMDESKTOP) {
      if (procSyncRegisterConduit(*conduitHandle) != SYNCERR_NONE) {
        procLogAddEntry(
          "Conduit was not allowed to run by the HotSync. Make "
          "sure the application this conduit is registered for is "
      ...
      *conduitHandle = (CONDHANDLE)((procCeRapiInit() == NO_ERROR) ? 1 : 0);
      ...
      if (fileAttributes == 0xFFFFFFFF ||
        ...
        if (fileAttributes == 0xFFFFFFFF ||
            !(fileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
          p->retI = 0; // flsobral@tc114_45: No longer throws an exception if
                       // the application is not found at the device, preventing
                       // the conduit to display errors when connecting to
                       // devices without the target application installed.
          return;
        }
      }
    ...
    *Conduit_targetAppPath(conduit) = createStringObjectFromJCharP(
      p->currentContext, (JCharP)remoteAppPath, -1);

    p->retI = ret;
  }
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisRPDBF_create(NMParams p)
  { // totalcross/io/sync/RemotePDBFile native void create() throws
    ...
    *(remoteAppPath + targetAppPathLen) = 0;
    ...
    TCObject pdbHandleObject =
      createByteArray(p->currentContext, sizeofPDBHandle(syncTarget));
    if (!pdbHandleObject) {
      return;
    }
    ...
    // if create, try to create it; if it already exists, open in read_write
    // mode.
    if (mode == PDBFILE_CREATE) {
      if (syncTarget == TARGETING_PALMDESKTOP) {
        ...
        TCHAR
        szFileName[MAX_PATHNAME]; // flsobral@tc115_11: ensure
                                  // myDmCreateDatabase uses the targetAppPath
                                  // to create the file. Otherwise it may use
                                  // the Conduit's application path and fail.

        String2TCHARPBuf(targetAppPath, szFileName);
        if (szFileName[targetAppPathLen - 1] != '/' &&
            szFileName[targetAppPathLen - 1] != '\\') {
    ...
    if (mode != PDBFILE_CREATE) {
      if (syncTarget == TARGETING_PALMDESKTOP) {
        ...
        if (procSyncOpenDB(
              szPdbNameA, 0, pdbHandle4HotSync(pdbHandle), openMode) !=
            SYNCERR_NONE) {
          ...
          if (procSyncReadOpenDbInfo(params, info) != SYNCERR_NONE ||
              info.baseInfo.m_Creator != creator ||
              info.baseInfo.m_DbType != type) {
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisRPDBF_close(NMParams p)
  { // totalcross/io/sync/RemotePDBFile native public
    // void close() throws totalcross.io.IOException;
    TCObject conduit = getMainClass();
    int32 syncTarget = *Conduit_syncTarget(conduit);
    ...
    if (*pdbHandle) {
      if (syncTarget == TARGETING_PALMDESKTOP) {
        procSyncResetSyncFlags(
          pdbHandle4HotSync(pdbHandle)); // guich@570_14: try through here too
  ...
  SYNC_API void tisRPDBF_delete(NMParams p)
  { // totalcross/io/sync/RemotePDBFile native public
    // void delete() throws totalcross.io.IOException;
    TCObject conduit = getMainClass();
    int32 syncTarget = *Conduit_syncTarget(conduit);
    ...
    if (remotePDB != null) {
      if (*pdbHandle) {
        if (syncTarget == TARGETING_PALMDESKTOP) {
          if ((err = procSyncCloseDB(pdbHandle4HotSync(pdbHandle))) ==
              NO_ERROR) {
            TCObject pdbName = RemotePDBFile_name(remotePDB);
            char szPdbName[42];
            char* dot;
            JCharP2CharPBuf(
              String_charsStart(pdbName), String_charsLen(pdbName), szPdbName);
            dot = strchr(szPdbName, '.');
            if (dot) {
              *dot = 0; // cut off the creator and type.
            }
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisRPDBF_getRecordCount(NMParams p)
  { // totalcross/io/sync/RemotePDBFile native public int
    // getRecordCount() throws totalcross.io.IOException;
    TCObject conduit = getMainClass();
    int32 syncTarget = *Conduit_syncTarget(conduit);
    Err err = NO_ERROR;
    TCObject pdbFile = p->obj[0];
    ...
    if (*pdbHandle) {
      if (syncTarget == TARGETING_PALMDESKTOP) {
        err =
          procSyncGetDBRecordCount(pdbHandle4HotSync(pdbHandle), recordCount);
      } else { // syncTarget == TARGETING_ACTIVESYNC
    ...
    if (*pdbHandle) {
      if (syncTarget == TARGETING_PALMDESKTOP) {
        ...
        while (++lastSearchedRec < recordCount) {
          if (myDmRecordInfo(pdbHandle4ActiveSync(pdbHandle),
                             lastSearchedRec,
                             &attr) != NO_ERROR) {
            ret = -1;
            break;
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisRPDBF_rwRecord_ibb(NMParams p)
  { // totalcross/io/sync/RemotePDBFile native int rwRecord(int
    // idx, totalcross.io.ByteArrayStream bas, boolean read);
    TCObject conduit = getMainClass();
    int32 syncTarget = *Conduit_syncTarget(conduit);
    TCObject pdbFile = p->obj[0];
    ...
    if (bas == null) {
      ...
      if (syncTarget == TARGETING_PALMDESKTOP) {
        ...
        if (isRead) {
          ...
          if (idx < 0 ||
              (idx >= 0 && procSyncReadRecordByIndex(info) ==
                             SYNCERR_NONE)) { // appending or overwriting. if
                                              // overwriting, get the record id
            info.m_RecSize = pos;
            info.m_Attribs =
              0; //&= ~eRecAttrDirty; // guich@570_14: reset the dirty flag
        ...
        if (isRead) {
          ...
          MemHandle recH =
            myDmNewRecord(pdbHandle4ActiveSync(pdbHandle), &uidx, pos);
          if (recH != NULL) {
            boolean ok = false;
            uint16 attr;
            ...
            if (myDmRecordInfo(pdbHandle4ActiveSync(pdbHandle), uidx, &attr) ==
                0)
              myDmSetRecordInfo(pdbHandle4ActiveSync(pdbHandle),
                                uidx,
                                attr &
                                  ~0X40); // guich@570_14: reset dirty attribute
          }
        }
      }
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisRPDBF_deleteRecord_i(NMParams p)
  { // totalcross/io/sync/RemotePDBFile native public void
    // deleteRecord(int index) throws
    // totalcross.io.IllegalArgumentIOException,
    ...
    if (syncTarget == TARGETING_PALMDESKTOP) {
      ...
      if ((err = procSyncReadRecordByIndex(info)) !=
          NO_ERROR) {                  // get record id
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisRPDBF_listPDBs_ii(NMParams p)
  { // totalcross/io/sync/RemotePDBFile native public static
    // String[] listPDBs(int crtr, int type);
    TCObject conduit = getMainClass();
    int32 syncTarget = *Conduit_syncTarget(conduit);
    TCObject targetAppPath = *Conduit_targetAppPath(conduit);
    WCHAR remoteAppPath[MAX_PATH];
    ...
    *(remoteAppPath + targetAppPathLen) = 0;
    ...
    if (crtr || type) { // one of the two must be provided
      if (syncTarget == TARGETING_PALMDESKTOP) {
        ...
        rParams.bSrchFlags =
          SYNC_DB_SRCH_OPT_NEW_SEARCH;    // only set for first search
        ...
        while (1) {
          ...
          rInfo.dwReserved =
            0; // must set to zero before calling function, per documentation
          rInfo.baseInfo.m_dwReserved =
            0; // must set to zero before calling function, per documentation
          ...
          xstrprintf(
            stringsP, "%s.%s.%s", rInfo.baseInfo.m_Name, szCrtr, szType);
          xstrprintf(
            fullName, "%s.%s.%s", rInfo.baseInfo.m_Name, szCrtr, szType);
          stringsCount++;
        ...
        TCHARP searchPath =
          JCharP2TCHARP((JCharP)remoteAppPath, (int32)lstrlenW(remoteAppPath));
      ...
      if (stringsCount != 0) { // no databases found?
        p->retO = stringArray =
          createStringArray(p->currentContext, stringsCount);
        ...
        if (stringArray != NULL) {
          ...
          while (stringsCount-- > 0) {
            ...
            *strings++ =
              createStringObjectFromCharP(p->currentContext, stringsP, len);
            stringsP += len + 1;
          }
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisRF_listFiles_s(NMParams p)
  { // totalcross/io/sync/RemoteFile native public
    // static String[] listFiles(String dir);
    TCObject conduit = getMainClass();
    int32 syncTarget = *Conduit_syncTarget(conduit);
    ...
    if (dir == null) {
      ...
      if (syncTarget == TARGETING_PALMDESKTOP) {
        ...
        CCallModuleParams* params =
          (*szDirA == 0) ? prepareParams(LIST_VOLUMES, null, 0)
                         : prepareParams(LIST_FILES, (void*)szDirA, dirLen + 1);
        if ((err = procSyncCallRemoteModule(params)) != errNone)
          ; // debug("tisRF_listFiles_s error - %d, %d", err,
            // params->m_dwResultCode);
        else if ((stringsCount = params->m_dwResultCode) >
                   0 && // no databases found?
                 (p->retO = stringArray = createStringArray(
                    p->currentContext, stringsCount)) != null) {
          // fill the string array
          TCObject* strings = (TCObject*)ARRAYOBJ_START(stringArray);
          ...
          while (stringsCount-- > 0) {
            ...
            *strings++ =
              createStringObjectFromCharP(p->currentContext, string, stringLen);
            string += stringLen + 1;
          }
          setObjectLock(p->retO, UNLOCKED);
        }
        ...
        CharP2JCharPBuf(szDirA,
                        *(szDirA + dirLen - 1) == '/' ? dirLen - 1 : dirLen,
                        (JCharP)szDirW,
                        true);
        lstrcatW(szDirW, L"/*.*");

        // get the files
        if (!procCeFindAllFiles(
              szDirW, FAF_NAME | FAF_ATTRIBUTES, (LPDWORD)&stringsCount, &fd)) {
          ...
          if (stringArray != null) {
            ...
            for (i = 0; i < (int32)stringsCount; i++) {
              ...
              JCharP2TCHARPBuf(
                (JCharP)fd[i].cFileName, lstrlenW(fd[i].cFileName), fileName);
              if (fd[i].dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                tcscat(fileName, "/");
              }
              *strings++ =
                createStringObjectFromTCHAR(p->currentContext, fileName, -1);
            }
            setObjectLock(p->retO, UNLOCKED);
          }
        }
  ...
  //////////////////////////////////////////////////////////////////////////
  SYNC_API void tisRF_copyToRemote_ss(NMParams p)
  { // totalcross/io/sync/RemoteFile native public static boolean
    // copyToRemote(String srcFile, String dstFile);
    TCObject conduit = getMainClass();
    int32 syncTarget = *Conduit_syncTarget(conduit);
    ...
    if (srcFile == null) {
      ...
      if (syncTarget == TARGETING_PALMDESKTOP) {
        ...
        if ((srcRef = fopen(szSrcFile, "rb")) == NULL) {
          ...
          CCallModuleParams* params = prepareParams(
            WRITE_FILE,
            buffer,
            (uint32)(dataP -
                     dstFileNameP)); // verify if file exists, return its size
                                     // and transfer the first bytes

          if ((read = fread(dataP,
                            1,
                            (RECEIVE_BUFSIZE - (dataP - dstFileNameP)),
                            srcRef)) <= 0) {
            result = 1;
          }
          params->m_dwParamSize = (uint32)((dataP - dstFileNameP) + read);
          if ((err = procSyncCallRemoteModule(params)) != errNone) {
            alert("1.SyncCallRemoteModule in copyTo %X, %d",
                  err,
                  params->m_dwResultCode);
            // do something
          } else {
            ...
            while (!result) {
              ...
              params = prepareParams(
                ...
                buffer,
                (uint32)(dataP -
                         dstFileNameP)); // verify if file exists, return its
                                         // size and transfer the first bytes
              if ((read = fread(dataP,
                                1,
                                (RECEIVE_BUFSIZE - (dataP - dstFileNameP)),
                                srcRef)) <= 0) {
              ...
              params->m_dwParamSize = (uint32)((dataP - dstFileNameP) + read);
              if ((err = procSyncCallRemoteModule(params)) != errNone) {
                alert("2.SyncCallRemoteModule in copyTo %X, %d",
                      err,
                      params->m_dwResultCode);
                break;
                // do something
        ...
        if (hSrc == INVALID_HANDLE_VALUE) {
          ...
          hDest = procCeCreateFile(dstFileW,
                                   GENERIC_WRITE,
                                   FILE_SHARE_READ | FILE_SHARE_WRITE,
                                   NULL,
                                   CREATE_ALWAYS,
  ...
  SYNC_API void tisRF_copyFromRemote_ss(NMParams p)
  { // totalcross/io/sync/RemoteFile native public static boolean
    // copyFromRemote(String srcFile, String dstFile);
    TCObject conduit = getMainClass();
    int32 syncTarget = *Conduit_syncTarget(conduit);
    TCObject srcFile = p->obj[0];
    TCObject dstFile = p->obj[1];
    ...
    if (srcFile == null) {
      ...
      if (syncTarget == TARGETING_PALMDESKTOP) {
        ...
        // the remote file must answer with the total number of bytes
        // transfered.
        ...
        CCallModuleParams* params =
          prepareParams(READ_FILE,
                        buffer,
                        (uint32)(srcFileLen + xstrlen(offsetP) +
                                 2)); // verify if file exists, return its size
                                      // and transfer the first bytes
        if ((err = procSyncCallRemoteModule(params)) != errNone) {
          ...
          while (dstRef != NULL &&
                 read > 0) { // why boring with another if? :-)
            ...
            xstrcpy(offsetP, _ltoa(ofs, bufn, 10));
            params = prepareParams(READ_FILE,
                                   buffer,
                                   (uint32)(srcFileLen + xstrlen(offsetP) +
                                            2)); // open file F at position I
            if ((err = procSyncCallRemoteModule(params)) != errNone) {
        ...
        hSrc = procCeCreateFile(srcFileW,
                                GENERIC_READ,
                                FILE_SHARE_READ | FILE_SHARE_WRITE,
                                NULL,
                                OPEN_EXISTING,
  ...
  SYNC_API void tisRF_delete_s(NMParams p)
  { // totalcross/io/sync/RemoteFile native public
    // static boolean delete(String fileOrFolder);
    TCObject conduit = getMainClass();
    int32 syncTarget = *Conduit_syncTarget(conduit);
    ...
    if (fileOrFolder == null) {
      ...
      JCharP2CharPBuf(
        String_charsStart(fileOrFolder), fileOrFolderLen, szFileOrFolderA);
      replaceSlashes(szFileOrFolderA);

      if (syncTarget == TARGETING_PALMDESKTOP) {
        CCallModuleParams* params = prepareParams(
          DELETE_FILE, szFileOrFolderA, (uint32)fileOrFolderLen + 1);
        if ((err = procSyncCallRemoteModule(params)) != errNone) {
        ...
        DWORD attr;
        CharP2JCharPBuf(
          szFileOrFolderA, fileOrFolderLen, (JCharP)szFileOrFolderW, true);

        // first find the file type
        attr = procCeGetFileAttributes(szFileOrFolderW);
        if ((attr & FILE_ATTRIBUTE_READONLY) !=
            0) // if the file is readonly, reset the attribute before deleting
          if (!procCeSetFileAttributes(szFileOrFolderW,
        ...
        if (err == NO_ERROR) {
          if (((attr & FILE_ATTRIBUTE_DIRECTORY) != 0) &&
              !procCeRemoveDirectory(szFileOrFolderW)) {
            err = procCeGetLastError();
          } else if (!procCeDeleteFile(szFileOrFolderW)) {
            err = procCeGetLastError();
          }
        }
      }
  ...
  { // totalcross/io/sync/RemoteFile native public
    ...
    xmemmove(applicationName,
             String_charsStart(command),
             String_charsLen(command) * sizeof(WCHAR));

    xmemzero(commandLine, sizeof(commandLine));
    xmemmove(commandLine,
             String_charsStart(args),
             String_charsLen(args) * sizeof(WCHAR));

    initRet = procCeRapiInit();
    if (initRet == NO_ERROR || initRet == CERAPI_E_ALREADYINITIALIZED) {
      if ((createProcessRet = procCeCreateProcess(applicationName,
                                                  commandLine,
                                                  null,
                                                  null,
                                                  false,
