...
extern "C" {
	...
	int32 listDatabases(TCHARP searchPath, HandlePDBSearchProcType proc, void* userVars,
						byte recursive);
}
...
typedef int (__stdcall* CmGetCreatorDirectoryProc)(const char* pCreatorID, TCHAR* pFile,
		int* piSize);
...
typedef SInt32(*SyncOpenDBProc)(const char* pName, SInt32 nCardNum, HSByte& rHandle,
								HSByte  openMode);
...
typedef SInt32(*SyncFindDbByTypeCreatorProc)(SyncFindDbByTypeCreatorParams& rParam,
		SyncDatabaseInfoType& rInfo);
...
extern "C" {
	...
	SYNC_API bool LibOpen(OpenParams params) {
		...
		if ((rapiModule = LoadLibrary("rapi.dll")) == null) {
			if (params->commandLine[0] == 0 || params->commandLine[2] == 'p'
				...
				'P') { // if no command line passed or targetting palm, we don't need ActiveSync
				return true;
			}
			throwException(params->currentContext, RuntimeException,
		...
		procCeGetFileAttributes = (CeGetFileAttributesProc) GetProcAddress(rapiModule,
								  "CeGetFileAttributes");
		...
		procCeSetFileAttributes = (CeSetFileAttributesProc) GetProcAddress(rapiModule,
								  "CeSetFileAttributes");
		...
		if (!procCeRapiInit        || !procCeRapiUninit    || !procCeGetFileAttributes
			|| !procCeFindClose        ||
			!procCeFindNextFile    || !procCeFindFirstFile || !procCeGetLastError
			|| !procCeSetFilePointer   ||
			!procCeReadFile        || !procCeWriteFile     || !procCeSetEndOfFile
			|| !procCeCloseHandle      ||
			!procCeDeleteFile      || !procCeCreateFile    || !procCeGetFileSize
			|| !procCeMoveFile         ||
			!procCeRemoveDirectory || !procCeFindAllFiles  || !procCeSetFileAttributes
			|| !procCeCreateProcess    || !procCeRapiGetError) {
			FreeLibrary(rapiModule);
			throwException(params->currentContext, RuntimeException,
						   "One or more rapi.dll functions were missing. Please be sure to be using the correct ActiveSync version.");
			return false;
		}
		return true;
	}
	...
	static HMODULE loadPalmDll(CharP name) {
		...
		TRACE("loadPalmDll");
		ret = RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\U.S. Robotics\\Pilot Desktop\\Core", 0, KEY_READ,
						   &handle);
		if (ret == NO_ERROR) {
			tzero(path);
			size = MAX_PATHNAME - 1;
	...
	static bool loadHotSyncCfgLibraries() {
		...
		// now get the procs
		procHsRefreshConduitInfo = (HsRefreshConduitInfoProc) GetProcAddress(hsApiModule,
		...
		procCmConvertCreatorIDToString = (CmConvertCreatorIDToStringProc) GetProcAddress(condMgrModule,
										 "CmConvertCreatorIDToString");
		procCmSetCreatorTitle = (CmSetCreatorTitleProc) GetProcAddress(condMgrModule, "CmSetCreatorTitle");
		procCmSetCreatorPriority = (CmSetCreatorPriorityProc) GetProcAddress(condMgrModule,
								   "CmSetCreatorPriority");
		procCmSetCreatorDirectory = (CmSetCreatorDirectoryProc) GetProcAddress(condMgrModule,
									"CmSetCreatorDirectory");
		procCmGetCreatorDirectory = (CmGetCreatorDirectoryProc) GetProcAddress(condMgrModule,
		...
		procCmRemoveConduitByCreatorID = (CmRemoveConduitByCreatorIDProc) GetProcAddress(condMgrModule,
		...
		if (!procHsRefreshConduitInfo       || !procCmSetCreatorFile   || !procCmGetCreatorFile      ||
			...
			if (procHsRefreshConduitInfo == null)
				MessageBox(null,
						   "Could not load a required function from hsapid.dll.\nPalm Desktop 4.14 / HotSync 6.0.1 or newer are required.",
						   "TotalCross Sync", 0);
			else
				MessageBox(null,
...
static int execTC(char* param, char* cmdLine, char* className) {
	...
	if (!queryRegistry(HKEY_CURRENT_USER,
					   (cmdLine[1] == 'c') ? "Software\\TotalCross\\ConduitCfgRect" :
					   "Software\\TotalCross\\ConduitSyncRect", className, scr, 100)) { // guich@tc110_27
	...
	if ((tcvmModule = LoadLibrary("tcvm.dll")) != null) {
		...
		if ((procExecuteProgram = (executeProgramProc) GetProcAddress(tcvmModule,
								  "executeProgram")) != null) {
...
static bool setRegistry(HKEY key, TCHAR* subkey, TCHAR* name, char* buf) {
	...
	return RegOpenKeyEx(key, subkey, 0, KEY_ALL_ACCESS, &handle) == NO_ERROR &&
		   RegSetValueEx(handle, name, 0, REG_SZ, (byte*) buf, (DWORD)(xstrlen(buf) + 1)) == NO_ERROR
		   && // store the length
		   RegCloseKey(handle) == NO_ERROR;
}
...
static bool setEnviromentVariable(HKEY key, TCHAR* subkey) {
	...
	if (RegOpenKeyEx(key, subkey, 0, KEY_ALL_ACCESS, &tcKey) == NO_ERROR) {
		...
		SendMessage(HWND_BROADCAST, WM_WININICHANGE, 0,
					(LPARAM) &szEnv); //flsobral@tc114_14: Wait until all applications are notified about the registry update.
	}
	RegCloseKey(tcKey);
	return ok;
}
...
__declspec(dllexport) long OpenConduit(PROGRESSFN progress, CSyncProperties& sync) {
	...
	procSyncUnRegisterConduit = (SyncUnRegisterConduitProc) GetProcAddress(sync20Module,
	...
	procSyncGetHHOSVersion = (SyncGetHHOSVersionProc) GetProcAddress(sync20Module,
							 "SyncGetHHOSVersion");
	procSyncRegisterConduit = (SyncRegisterConduitProc) GetProcAddress(sync20Module,
							  "SyncRegisterConduit");
	procSyncCloseDB = (SyncCloseDBProc) GetProcAddress(sync20Module, "SyncCloseDB");
	procSyncReadOpenDbInfo = (SyncReadOpenDbInfoProc) GetProcAddress(sync20Module,
							 "SyncReadOpenDbInfo");
	...
	procSyncResetSyncFlags = (SyncResetSyncFlagsProc) GetProcAddress(sync20Module,
							 "SyncResetSyncFlags");
	procSyncDeleteDB = (SyncDeleteDBProc) GetProcAddress(sync20Module, "SyncDeleteDB");
	procSyncGetDBRecordCount = (SyncGetDBRecordCountProc) GetProcAddress(sync20Module,
							   "SyncGetDBRecordCount");
	procSyncReadNextModifiedRec = (SyncReadNextModifiedRecProc) GetProcAddress(sync20Module,
								  "SyncReadNextModifiedRec");
	procSyncWriteRec = (SyncWriteRecProc) GetProcAddress(sync20Module, "SyncWriteRec");
	procSyncReadRecordByIndex = (SyncReadRecordByIndexProc) GetProcAddress(sync20Module,
								"SyncReadRecordByIndex");
	procSyncDeleteRec = (SyncDeleteRecProc) GetProcAddress(sync20Module, "SyncDeleteRec");
	procSyncFindDbByTypeCreator = (SyncFindDbByTypeCreatorProc) GetProcAddress(sync20Module,
								  "SyncFindDbByTypeCreator");
	procSyncCallRemoteModule = (SyncCallRemoteModuleProc) GetProcAddress(sync20Module,
	...
	if (!procLogAddEntry || !procSyncUnRegisterConduit || !procSyncYieldCycles ||
		...
		||
		!procSyncReadRecordByIndex || !procSyncDeleteRec || !procSyncFindDbByTypeCreator
		|| !procSyncCallRemoteModule) {
	...
	xstrprintf(msg,
			   "Conduit not properly registered, registering the conduit again will probably fix the problem");
...
__declspec(dllexport) long ConfigureConduit(CSyncPreference& pref) {
	...
	if (queryRegistry(pref.m_hKey, pref.m_Registry, "File0", className, sizeof(className))
		&& queryRegistry(pref.m_hKey, pref.m_Registry, "Directory", classDir,
...
__declspec(dllexport) long GetConduitInfo(ConduitInfoEnum infoType, void* pInArgs, void* pOut,
		DWORD* pdwOutSize) {
	...
	switch (infoType) {
		...
		case eDefaultAction: {
			...
			if ((err = getClassName(((ConduitRequestInfoType*)pInArgs)->dwCreatorId, className, null,
									null)) == CONDERR_NONE) {
...
extern "C" {
	static void invalidateRemotePDBFile(Context currentContext, TCObject obj) {
		...
		*getStaticFieldInt(loadClass(currentContext, "totalcross.io.sync.RemotePDBFile", true),
						   "idle") = true; // guich@tc114_95: must set to idle
	}
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisC_finishSync(NMParams
								  p) { // totalcross/io/sync/Conduit native boolean finishSync();
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisC_log_s(NMParams
							 p) { // totalcross/io/sync/Conduit native static public void log(String text);
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisC_yield(NMParams
							 p) { // totalcross/io/sync/Conduit native static public void yield();
		int32 syncTarget = *Conduit_syncTarget(getMainClass());
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisC_register_i(NMParams
								  p) { // totalcross/io/sync/Conduit native boolean register(int priority);
		TCObject conduit = p->obj[0];
		...
		if (syncTarget & TARGETING_PALMDESKTOP) {
			...
			JCharP2CharPBuf(String_charsStart(targetApplicationId), String_charsLen(targetApplicationId),
							szCreatorA);

			ok = setEnviromentVariable(HKEY_LOCAL_MACHINE,
			...
			if (ok && loadHotSyncCfgLibraries()) {
				...
				if (procCmInstallCreator(szCreatorA, CONDUIT_APPLICATION) == -1005)
					throwException(p->currentContext, RuntimeException,
								   "Could not install the conduit, the creator id specified is already installed");
				else {
		...
		if (syncTarget & TARGETING_ACTIVESYNC) {
			...
			JCharP2TCHARPBuf(String_charsStart(targetApplicationId), String_charsLen(targetApplicationId),
							 szCreatorT);
			...
			if (CreateLink(appPath, "/sw", "", shortcutPath) == S_OK)
				ok = setRegistry(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows CE Services\\AutoStartOnConnect",
								 szCreatorT, shortcutPath);
		}

		p->retI = ok;
	}
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisC_unregister(NMParams
								  p) { // totalcross/io/sync/Conduit native boolean unregister();
		TCObject conduit = p->obj[0];
		int32 syncTarget = *Conduit_syncTarget(conduit);
		int32 ret = 1;
		...
		if (syncTarget & TARGETING_ACTIVESYNC) {
			...
			JCharP2TCHARPBuf(String_charsStart(targetApplicationId), String_charsLen(targetApplicationId),
							 szCreatorT);
			ret = (ret == 0) ? ret : deleteRegistry(HKEY_LOCAL_MACHINE,
													"SOFTWARE\\Microsoft\\Windows CE Services\\AutoStartOnConnect", szCreatorT);
		}

		p->retI = ret;
	}
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisC_initSync(NMParams
								p) { // totalcross/io/sync/Conduit native private boolean initSync();
		TCObject conduit = p->obj[0];
		int32 syncTarget = *Conduit_syncTarget(conduit);
		...
		*(remoteAppPath + targetAppPathLen) = 0;
		...
		if (syncTarget == TARGETING_PALMDESKTOP) {
			if (procSyncRegisterConduit(*conduitHandle) != SYNCERR_NONE) {
				procLogAddEntry("Conduit was not allowed to run by the HotSync. Make sure the application this conduit is registered for is installed on the target device",
								slError, false);
				*conduitHandle = 0;
			} else {
		...
		*Conduit_targetAppPath(conduit) = createStringObjectFromJCharP(p->currentContext,
										  (JCharP) remoteAppPath, -1);

		p->retI = ret;
	}
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRPDBF_create(NMParams
								  p) { // totalcross/io/sync/RemotePDBFile native void create() throws totalcross.io.IllegalArgumentIOException, totalcross.io.FileNotFoundException, totalcross.io.IOException;
		...
		if (mode != PDBFILE_CREATE) {
			if (syncTarget == TARGETING_PALMDESKTOP) {
				...
				if (procSyncOpenDB(szPdbNameA, 0, pdbHandle4HotSync(pdbHandle), openMode) != SYNCERR_NONE) {
					...
					if (procSyncReadOpenDbInfo(params, info) != SYNCERR_NONE || info.baseInfo.m_Creator != creator
				...
				if (pdbHandleObject) {
					...
					if (dbId) { // check if the creator id and type are those expected
						...
						err = myDmDatabaseInfo(dbId, null, null, null, null, null, null, null, null, null, &typeId,
											   &creatorId);
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRPDBF_close(NMParams
								 p) { // totalcross/io/sync/RemotePDBFile native public void close() throws totalcross.io.IOException;
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRPDBF_delete(NMParams
								  p) { // totalcross/io/sync/RemotePDBFile native public void delete() throws totalcross.io.IOException;
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRPDBF_getRecordCount(NMParams
										  p) { // totalcross/io/sync/RemotePDBFile native public int getRecordCount() throws totalcross.io.IOException;
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
		Err err = NO_ERROR;
		TCObject pdbFile = p->obj[0];
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRPDBF_getNextModifiedRecordIn(NMParams
			p) { // totalcross/io/sync/RemotePDBFile native public int getNextModifiedRecordIndex();
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRPDBF_rwRecord_ibb(NMParams
										p) { // totalcross/io/sync/RemotePDBFile native int rwRecord(int idx, totalcross.io.ByteArrayStream bas, boolean read);
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
		TCObject pdbFile = p->obj[0];
		...
		if (bas == null) {
			...
			if (syncTarget == TARGETING_PALMDESKTOP) {
				...
				if (isRead) {
					...
					if (idx < 0 || (idx >= 0
									&& procSyncReadRecordByIndex(info) ==
									SYNCERR_NONE)) { // appending or overwriting. if overwriting, get the record id
						info.m_RecSize = pos;
				...
				if (isRead) {
					...
					if (recH != NULL) {
						...
						if (myDmRecordInfo(pdbHandle4ActiveSync(pdbHandle), uidx, &attr) == 0)
							myDmSetRecordInfo(pdbHandle4ActiveSync(pdbHandle), uidx,
											  attr & ~0X40); // guich@570_14: reset dirty attribute
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRPDBF_deleteRecord_i(NMParams
										  p) { // totalcross/io/sync/RemotePDBFile native public void deleteRecord(int index) throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException;
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRPDBF_listPDBs_ii(NMParams
									   p) { // totalcross/io/sync/RemotePDBFile native public static String[] listPDBs(int crtr, int type);
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
		TCObject targetAppPath = *Conduit_targetAppPath(conduit);
		WCHAR remoteAppPath[MAX_PATH];
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRF_listFiles_s(NMParams
									p) { // totalcross/io/sync/RemoteFile native public static String[] listFiles(String dir);
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
		...
		if (dir == null) {
			...
			if (syncTarget == TARGETING_PALMDESKTOP) {
				...
				CCallModuleParams* params = (*szDirA == 0) ? prepareParams(LIST_VOLUMES, null,
											0) : prepareParams(LIST_FILES, (void*) szDirA, dirLen + 1);
				if ((err = procSyncCallRemoteModule(params)) != errNone)
					; //debug("tisRF_listFiles_s error - %d, %d", err, params->m_dwResultCode);
				else if ((stringsCount = params->m_dwResultCode) > 0 &&   // no databases found?
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRF_copyToRemote_ss(NMParams
										p) { // totalcross/io/sync/RemoteFile native public static boolean copyToRemote(String srcFile, String dstFile);
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
		...
		if (srcFile == null) {
			...
			if (syncTarget == TARGETING_PALMDESKTOP) {
				...
				if ((srcRef = fopen(szSrcFile, "rb")) == NULL) {
					...
					CCallModuleParams* params = prepareParams(WRITE_FILE, buffer,
												(uint32)(dataP -
														 dstFileNameP));  // verify if file exists, return its size and transfer the first bytes
					...
					if ((err = procSyncCallRemoteModule(params)) != errNone) {
						...
						while (!result) {
							xstrcpy(offsetP, _ltoa(ofs, bufn, 10));
							dataP = offsetP + xstrlen(offsetP) + 1;
							params = prepareParams(WRITE_FILE, buffer,
												   (uint32)(dataP -
															dstFileNameP));  // verify if file exists, return its size and transfer the first bytes
							if ((read = fread(dataP, 1, (RECEIVE_BUFSIZE - (dataP - dstFileNameP)), srcRef)) <= 0) {
								result = 1;
				...
				hSrc = CreateFile(szSrcFile, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
								  0, 0);
				if (hSrc == INVALID_HANDLE_VALUE) {
					err = GetLastError();
				} else {
					hDest = procCeCreateFile(dstFileW, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
											 CREATE_ALWAYS, 0, 0);
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRF_copyFromRemote_ss(NMParams
										  p) { // totalcross/io/sync/RemoteFile native public static boolean copyFromRemote(String srcFile, String dstFile);
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
		TCObject srcFile = p->obj[0];
		TCObject dstFile = p->obj[1];
		...
		if (srcFile == null) {
			...
			if (syncTarget == TARGETING_PALMDESKTOP) {
				...
				CCallModuleParams* params = prepareParams(READ_FILE, buffer,
											(uint32)(srcFileLen + xstrlen(offsetP) +
													 2)); // verify if file exists, return its size and transfer the first bytes
				if ((err = procSyncCallRemoteModule(params)) != errNone) {
					...
					while (dstRef != NULL && read > 0) { // why boring with another if? :-)
						...
						xstrcpy(offsetP, _ltoa(ofs, bufn, 10));
						params = prepareParams(READ_FILE, buffer,
											   (uint32)(srcFileLen + xstrlen(offsetP) + 2));  // open file F at position I
						if ((err = procSyncCallRemoteModule(params)) != errNone) {
				...
				hSrc = procCeCreateFile(srcFileW, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
										OPEN_EXISTING, 0, 0);
				if (hSrc == INVALID_HANDLE_VALUE) {
					err = procCeGetLastError();
				} else {
					hDest = CreateFile(szDstFile, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
									   CREATE_ALWAYS, 0, 0);
					if (hDest == INVALID_HANDLE_VALUE) {
						err = GetLastError();
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRF_delete_s(NMParams
								 p) { // totalcross/io/sync/RemoteFile native public static boolean delete(String fileOrFolder);
		TCObject conduit = getMainClass();
		int32 syncTarget = *Conduit_syncTarget(conduit);
		...
		if (fileOrFolder == null) {
			...
			if (syncTarget == TARGETING_PALMDESKTOP) {
				CCallModuleParams* params = prepareParams(DELETE_FILE, szFileOrFolderA,
											(uint32) fileOrFolderLen + 1);
				if ((err = procSyncCallRemoteModule(params)) != errNone) {
	...
	//////////////////////////////////////////////////////////////////////////
	SYNC_API void tisRF_exec_ssib(NMParams
								  p) { // totalcross/io/sync/RemoteFile native public static int exec(String command, String args, int launchCode, boolean wait);
		...
		if (initRet == NO_ERROR || initRet == CERAPI_E_ALREADYINITIALIZED) {
			if ((createProcessRet = procCeCreateProcess(applicationName, commandLine, null, null, false, 0,
									null, null, null, &processInformation)) != 0) {
				p->retI = NO_ERROR;
