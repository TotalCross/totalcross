...
void
handleFingerTouchEvent(SDL_Event event)
{
...
handleKeyboardEvent(SDL_Event event)
{
  ...
  if (event.type == SDL_KEYDOWN) {
    ...
    // else if (key < 255)
    //    postEvent(mainContext, KEYEVENT_KEY_PRESS, event.key.keysym.unicode,
    //    0, 0, modifier);
  }
}

void
handleWheelEvent(SDL_Event event)
{
  int x, y;
  SDL_GetMouseState(&x, &y);
  if (event.wheel.y > 0) { // scroll up
    postEvent(
      mainContext, MOUSEEVENT_MOUSE_WHEEL, WHEEL_UP, x, max32(y, 0), -1);
  } else if (event.wheel.y < 0) { // scroll down
    postEvent(
      mainContext, MOUSEEVENT_MOUSE_WHEEL, WHEEL_DOWN, x, max32(y, 0), -1);
  }
  if (event.wheel.x > 0) { // scroll right
    postEvent(
      mainContext, MOUSEEVENT_MOUSE_WHEEL, WHEEL_RIGHT, x, max32(y, 0), -1);
  } else if (event.wheel.x < 0) { // scroll left
    postEvent(
      mainContext, MOUSEEVENT_MOUSE_WHEEL, WHEEL_LEFT, x, max32(y, 0), -1);
  }
}

void
handleTextInputEvent(SDL_Event event)
{
  char text[32];
  text[0] = '\0';
  int i = 0;
...
privatePumpEvent(Context currentContext)
{
  ...
  if (DEVICE_CTX->events->GetEvent(DEVICE_CTX->events, DFB_EVENT(&evt)) ==
      DFB_OK) {
    ...
    switch (evt.type) {
      ...
      case DIET_KEYPRESS:
        ...
        if (key == SK_SCREEN_CHANGE) {
          ...
          postEvent(
            mainContext, KEYEVENT_SPECIALKEY_PRESS, key, 0, 0, evt.modifiers);
        } else if (key < 255) {
  ...
  if (SDL_PollEvent(&event)) {
    ...
    if (event.type >= SDL_FINGERDOWN &&
        event.type <= SDL_FINGERMOTION) { // Finger Touch Events
      handleFingerTouchEvent(event);
    } else if (event.type >= SDL_MOUSEMOTION &&
               event.type <= SDL_MOUSEWHEEL) { // Mouse events
...
privateInitEvent()
{
  ...
  DFBResult res = DEVICE_CTX->dfb->CreateInputEventBuffer(
    DEVICE_CTX->dfb, DICAPS_ALL, DFB_TRUE, &DEVICE_CTX->events);
