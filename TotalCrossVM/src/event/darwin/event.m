...
void iphone_privatePumpEvent(Context currentContext) {
	...
	while (event = [enumerator nextObject]) {
		...
		if ([type isEqualToString: @"multitouchScale"]) {
			postEvent(currentContext, MULTITOUCHEVENT_SCALE, [[event objectForKey: @"key"] intValue],  [[event
					  objectForKey: @"x"] intValue], [[event objectForKey: @"y"] intValue], -1);
		} else if ([type isEqualToString: @"mouseDown"]) {
			postEvent(currentContext, PENEVENT_PEN_DOWN,
					  0, [[event objectForKey: @"x"] intValue], [[event objectForKey: @"y"] intValue], -1);
		} else if ([type isEqualToString: @"mouseUp"]) {
			postEvent(currentContext, PENEVENT_PEN_UP,
					  0, [[event objectForKey: @"x"] intValue], [[event objectForKey: @"y"] intValue], -1);
		} else if (([type isEqualToString: @"mouseDragged"]) || ([type isEqualToString: @"mouseMoved"])) {
			postEvent(currentContext, PENEVENT_PEN_DRAG,
					  0, [[event objectForKey: @"x"] intValue], [[event objectForKey: @"y"] intValue], -1);
...
#define SK_SCREEN_CHANGE -1030
			...
			screenChange(currentContext, [[event objectForKey: @"width"] intValue], [[event objectForKey:
						 @"height"] intValue], 0, 0, false);
		} else if ([type isEqualToString:
			@"keyPress"]) { //flsobral@tc126_59: now we support text edition directly in TotalCross controls!
...
#define SK_ENTER -1009
		...
		if ([type isEqualToString: @"updateEdit"]) {
			setEditText(currentContext, (TCObject)[[ event objectForKey: @"control"] longValue ], [ event
						objectForKey: @"value"]);
		}
		[event release];
	}
