 ...
 * Signature: (IIIIII)V
 */
void JNICALL Java_totalcross_Launcher4A_nativeOnEvent(JNIEnv* env, jobject this, jint type,
		jint key, jint x, jint y, jint modifiers, jint timestamp) {
	switch (type) {
		...
		case totalcross_Launcher4A_KEY_PRESS: {
			...
			if (key2 == x) { // no change?
				postEvent(mainContext, key == 0 ? KEYEVENT_SPECIALKEY_PRESS : KEYEVENT_KEY_PRESS,
						  key == 0 ? key2 : key, 0, 0, modifiers == 18 ? 0 :
						  modifiers);    // check if user is pressing the ALT key and pass 0, otherwise characters that are accessed using the alt key won't appear on screen
			} else {
				bool post = isEssentialKey(key2);
				if (!post && interceptedSpecialKeys !=
					null) { // guich@tc122_12: must check if post even if there's no special keys being intercepted
		...
		case totalcross_Launcher4A_SCREEN_CHANGED: {
			...
			if (starting) {
				...
				screenChange(mainContext, w, h, hRes, vRes,
							 !changed);    // guich@tc126_14: passing true here solves the problem - guich@tc130: prevent program from not recreating the mainPixels array when rotating the screen.
			}
			break;
		}
		case totalcross_Launcher4A_BARCODE_READ: {
			...
			if (scannerPostEvent == null) {
				scannerPostEvent = getMethod(loadClass(mainContext, "totalcross.io.device.scanner.Scanner", false),
											 false, "_onEvent", 1, J_INT);
			}
