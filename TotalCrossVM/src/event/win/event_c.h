...
screenChange(Context currentContext,
             ...
             bool nothingChanged);
void
getScreenSize(int32* w, int32* h); // ui/win/gfx_Graphics_c.h
...
hideWinCEStuff()
{
  ...
  if (*tcSettings.isFullScreenPtr) {
    // now make sure task bar/start icon/sip are lower down the z-order so they
    // seem to be removed
...
static bool
keysMatch(int32 tcK, int32 sysK)
{ // verifies if the given user key matches the system key
...
void
adjustWindowSizeWithBorders(int32 resizableWindow, int32* w, int32* h);
void
applyPalette();

#if defined(WP8)
static long FAR PASCAL
handleWin32Event(HWND hWnd, UINT msg, WPARAM wParam, LONG lParam)
{
  return 0L;
}
#else
...
handleWin32Event(HWND hWnd, UINT msg, WPARAM wParam, LONG lParam)
{
  ...
  // debug("msg: %X (%d), wParam: %d, lParam: %X", (int)msg, (int)msg,
  ...
  switch (msg) {
    ...
    case WM_GETMINMAXINFO:
      if (screen.pixels && *tcSettings.resizableWindow) {
        ...
        mmi->ptMinTrackSize.x =
          max32(landscape ? 320 : 240, screen.minScreenW / 2) + w;
        mmi->ptMinTrackSize.y =
          max32(landscape ? 240 : 320, screen.minScreenH / 2) + h;
    ...
    case WM_SIZE: {
      if (screen.pixels && *tcSettings.resizableWindow) {
        ...
        if (w != 0 && h != 0 && (lastW != w || lastH != h)) {
          screenChange(
            mainContext, lastW = w, lastH = h, screen.hRes, screen.vRes, false);
        }
      }
      break;
    }
...
#if defined(WIN32)
      if (HIWORD(
            wParam)) { // HIWORD(wParam) == 0 means the app is not minimized
        if (!minimized) {
    ...
    case WM_WINDOWPOSCHANGED: {
      if (((PWINDOWPOS)lParam)
            ->hwndInsertAfter) { // hwndInsertAfter == 0 means
                                 // the app window is at the top
        if (!minimized) {
          postOnMinimizeOrRestore(minimized = true);
        }
      } else if (minimized) {
    ...
    case WM_SETTINGCHANGE: {
      if (!bSipUp) { // flsobral@tc114_50: fixed the SIP keyboard button not
                     // being properly displayed on some WinCE devices.
        HWND hsipbtn = FindWindow(_T("MS_SIPBUTTON"), _T("MS_SIPBUTTON"));
        SetWindowPos(hsipbtn,
                     HWND_BOTTOM,
                     0,
                     0,
                     0,
                     ...
                     SWP_NOMOVE | SWP_NOSIZE |
                       SWP_HIDEWINDOW); // sets z-order to top but doesn't
    ...
    case WM_KILLFOCUS: {
      ...
      if ((SendMessage((HWND)wParam, WM_IME_REQUEST, IMR_ISIMEAWARE, 0) &
           IMEAF_AWARE) == IMEAF_AWARE) {
    ...
    case WM_PAINT: {
      ...
      HDC hDC;
      int32 w, h;
      hideWinCEStuff(); // in fullscreen mode, if the window looses focus, we
                        // have to hide the stuff again when we get the focus
                        // back. The problem is that there's no event telling
                        // that we recover the focus.
    ...
    case WM_LBUTTONDOWN: {
      ...
      if (yy >= 0 && keepRunning) {
        ...
        SetCapture(hWnd);
        postEvent(
          mainContext, PENEVENT_PEN_DOWN, 0, lastPenX, max32(lastPenY, 0), -1);
        isDragging = true;
      }
      break;
    }
    ...
    case WM_MOUSEHWHEEL:
    case WM_MOUSEWHEEL: {
      int32 x = lastPenX; //(int32)((int16)LOWORD(lParam)); using the last
                          // position because the ones that comes in lParam are
      ...
      if (y >= 0 && x >= 0 && keepRunning) {
        postEvent(mainContext,
                  MOUSEEVENT_MOUSE_WHEEL,
                  a > 0 ? (msg == WM_MOUSEHWHEEL ? WHEEL_RIGHT : WHEEL_UP)
                        : (msg == WM_MOUSEHWHEEL ? WHEEL_LEFT : WHEEL_DOWN),
                  x,
                  max32(y, 0),
                  -1);
      }
    ...
    case WM_KEYUP:
      scan = (lParam >> 16) & 0xFF;
      if (wParam == 134) {
        return 1; // 134 is always followed by an ENTER key
      }
      if (!keyIsDown[scan]) { // some key codes are sent only on key up, so if a
                              // key generated no keydown, we assume it is a
                              // hotkey
        isHotKey = true;
      }
      ...
      if (wParam <= 255) {
        ...
        switch (wParam) {
          case 13:
            sk_event =
              ((getTimeStamp() - actionStart) >= 800) ? SK_MENU : SK_ACTION;
            break;
          case VK_F1:
            if (!isWindowsMobile) {
              return 0L;
    ...
    case WM_HOTKEY:
      scan = (lParam >> 16) & 0xFF;
      if ((lParam & 0x1000) !=
          0) { // ignore event when key is up (process only when it is down)
        break;
      }
      if (wParam == 91 &&
          showKeyCodes) { // 91 is sent before every hotkey, so we ignore it
        break;
      }
      isHotKey = true;
    case WM_KEYDOWN: {
    ...
    cont:
      ...
      if (showKeyCodes) { // debug keys?
        alert("Key code: %d (dev: %d)\nModifier: %X\nEvent: %s",
              (int)keyDevice2Portable(key),
              (int)key,
              (int)keyGetPortableModifiers(-1),
              msg == WM_HOTKEY ? "WM_HOTKEY"
                               : msg == WM_KEYUP ? "WM_KEYUP" : "WM_KEYDOWN");
        break;
      }
      if (isHotKey && interceptedSpecialKeys != null) {
        Int32Array keys = interceptedSpecialKeys; // can store special keys (>
                                                  // 0) or totalcross keys (< 0)
          ...
          if (keysMatch(*keys, key)) {
            ...
            if (key == SK_SCREEN_CHANGE) {
              if (*tcSettings.screenWidthPtr != *tcSettings.screenHeightPtr) {
                screenChange(mainContext,
                             *tcSettings.screenHeightPtr,
        ...
        if (pkey != key && keyIsDown[scan]) {
          ...
          } else if (*tcSettings.screenWidthPtr !=
                     *tcSettings.screenHeightPtr) {
    ...
    case WM_CHAR:
      ...
      if (!dontPostOnChar) { // ignore keys that are sent with
        ...
        postEvent(mainContext,
                  ...
                  0,
                  0,
                  -1); // must pass control chars, otherwise clipboard functions
                       // won't work
      }
      break;
    ...
    case WM_CLOSE:
#ifdef WINCE // with this postQuitMessage, any MessageBox issued after it will
             // be ignored in win32
      PostQuitMessage(0);
#else
...
void
privatePumpEvent(Context currentContext)
{
#if defined(WP8)
  struct eventQueueMember q_member = eventQueuePop();
  // debug("%X - %d.event pop: %d", GetCurrentThreadId(), q_member.count,
  // q_member.type);
  if (q_member.type != 0) {
    postEvent(mainContext,
              q_member.type,
