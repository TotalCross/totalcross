...
void screenChange(Context currentContext, int32 newWidth, int32 newHeight, int hRes, int vRes,
				  bool nothingChanged);
...
static bool keysMatch(int32 tcK,
					  int32 sysK) { // verifies if the given user key matches the system key
...
static long FAR PASCAL handleWin32Event(HWND hWnd, UINT msg, WPARAM wParam, LONG lParam) {
	...
	switch (msg) {
		...
		case WM_WINDOWPOSCHANGED: {
			if (((PWINDOWPOS)
				 lParam)->hwndInsertAfter) { // hwndInsertAfter == 0 means the app window is at the top
				if (!minimized) {
					postOnMinimizeOrRestore(minimized = true);
				}
			} else if (minimized) {
		...
		case WM_SETTINGCHANGE: {
			if (!bSipUp) { //flsobral@tc114_50: fixed the SIP keyboard button not being properly displayed on some WinCE devices.
				...
				SetWindowPos(hsipbtn, HWND_BOTTOM, 0, 0, 0, 0,
							 SWP_NOMOVE | SWP_NOSIZE | SWP_HIDEWINDOW); // sets z-order to top but doesn't reposition or resize.
			}
			break;
		}
		case WM_KILLFOCUS: {
		...
		case WM_MOUSEWHEEL: {
			...
			if (y >= 0 && x >= 0 && keepRunning) {
				postEvent(mainContext, MOUSEEVENT_MOUSE_WHEEL,
						  a > 0 ? (msg == WM_MOUSEHWHEEL ? WHEEL_RIGHT : WHEEL_UP) : (msg == WM_MOUSEHWHEEL ? WHEEL_LEFT :
								  WHEEL_DOWN), x, max32(y, 0), -1);
			}
			break;
		}
#endif
		case WM_MOUSEMOVE: {
			...
			if (keepRunning && (x != lastPenX || y != lastPenY)) {
				postEvent(mainContext, isDragging ? PENEVENT_PEN_DRAG : MOUSEEVENT_MOUSE_MOVE, 0, lastPenX = x,
						  lastPenY = max32(y, 0), -1);
			}
			break;
		}
		case WM_KEYUP:
			...
			if (wParam <= 255) {
				...
				switch (wParam) {
					case 13:
						sk_event = ((getTimeStamp() - actionStart) >= 800) ? SK_MENU : SK_ACTION;
						break;
					case VK_F1:
						if (!isWindowsMobile) {
							return 0L;
		...
		cont:
			...
			if (showKeyCodes) { // debug keys?
				alert("Key code: %d (dev: %d)\nModifier: %X\nEvent: %s", (int)keyDevice2Portable(key), (int)key,
					  (int)keyGetPortableModifiers(-1),
					  msg == WM_HOTKEY ? "WM_HOTKEY" : msg == WM_KEYUP ? "WM_KEYUP" : "WM_KEYDOWN");
				break;
			}
			if (isHotKey && interceptedSpecialKeys != null) {
						...
						if (key == SK_SCREEN_CHANGE) {
							if (*tcSettings.screenWidthPtr != *tcSettings.screenHeightPtr) {
								screenChange(mainContext, *tcSettings.screenHeightPtr, *tcSettings.screenWidthPtr,
											 *tcSettings.screenHeightInDPIPtr, *tcSettings.screenWidthInDPIPtr, false);
							}
						} else {
				...
				if (pkey != key && keyIsDown[scan]) {
					...
					} else if (*tcSettings.screenWidthPtr != *tcSettings.screenHeightPtr) {
		...
		case WM_CHAR:
			...
			if (showKeyCodes) { // debug keys?
				alert("Key code: %d (dev: %d)\nModifier: %X\nEvent: %s", (int)keyDevice2Portable(key), (int)key,
					  (int)keyGetPortableModifiers(-1), "WM_CHAR");
				break;
			}
			if (!dontPostOnChar) { // ignore keys that are sent with KEYEVENT_SPECIALKEY_PRESS
				postEvent(mainContext, KEYEVENT_KEY_PRESS, key, 0, 0,
						  -1);    // must pass control chars, otherwise clipboard functions won't work
			}
			break;
		...
		case WM_CLOSE:
			...
			if (*tcSettings.closeButtonTypePtr == NO_BUTTON) {
				postEvent(mainContext, KEYEVENT_SPECIALKEY_PRESS, SK_MENU, 0, 0,
						  -1); // must pass control chars, otherwise clipboard functions won't work
				break;
			}
#endif
