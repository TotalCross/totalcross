...
void mainEventLoop(Context currentContext) {
	...
	_postEvent = getMethod(OBJ_CLASS(mainClass), true, "_postEvent", 6, J_INT, J_INT, J_INT, J_INT,
						   J_INT, J_INT);
...
#ifdef __gl2_h_
			if (markedImages >
				0) { // if another thread asked for a gc, then trigger another one now to dispose the images
...
void postEvent(Context currentContext, TotalCrossUiEvent type, int32 key, int32 x, int32 y,
			   int32 mods) {
	if (mainClass != null && _postEvent != null) {
		executeMethod(currentContext, _postEvent, mainClass, (int32)type, key, x, y,
...
void postOnMinimizeOrRestore(bool minimized) {
	...
	if (mainClass != null) {
		executeMethod(lifeContext, (isMinimized ? onMinimize : onRestore),
...
void destroyEvent() {
	if (oldAutoOffValue !=
		0) { // if user changed the state, restore the old value of the auto-off timer
		vmSetAutoOff(true);
	}
	privateDestroyEvent();
