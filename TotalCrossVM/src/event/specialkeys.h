...
extern "C"
{
  ...
  /** There are three ranges of keys:
    ...
    -1 <= k < 1000000 - hotkey. In windows, a hotkey is mapped into the ASCII
   range, so we invert the value. k <= 1000000 - portable device keys
  ...
  /// Returns the device key mapped from the portable key, or the portable key
  /// itself if it could not be mapped
  int32 keyPortable2Device(PortableSpecialKeys key);
  /// Returns the portable key mapped from the device key, or the device key
  /// itself if it could not be mapped
