...
int jpegRead(void* buff, int count, JPEGFILE* in) {
	...
	if (in->tcz != null) {
		...
		while (count > 0) {
			...
			n = executeMethod(in->currentContext, in->readBytesMethod, in->params[0].asObj, in->params[1].asObj,
							  in->params[2].asInt32, in->params[3].asInt32).asInt32;
			if (n <= 0) {
				break;
			}
...
int jpegWrite(void* buff, int count, JPEGFILE* in) {
	...
	while (current < count) {
		...
		executeMethod(in->currentContext, in->writeBytesMethod, in->params[0].asObj, in->params[1].asObj,
					  in->params[2].asInt32, in->params[3].asInt32);
		current += toCopy;
	}
	return current;
}
...
void jpegLoad(Context currentContext, TCObject imageObj, TCObject inputStreamObj, TCObject bufObj,
			  TCZFile tcz, char* first4, int32 targetWidthOrScaleNum, int32 targetHeightOrScaleDenom) {
	JPEGFILE file;
	Pixel* pixels;
	Heap heap;
	...
	if (tcz == null) {
		Method readBytesMethod = getMethod(OBJ_CLASS(file.inputStreamObj), true, "readBytes", 3, BYTE_ARRAY,
										   J_INT, J_INT);
	...
	jpeg_calc_output_dimensions(
		&cinfo); /* Calculate output image dimensions so we can allocate space */
	...
	buffer0 = (*cinfo.mem->alloc_sarray)((j_common_ptr) &cinfo, JPOOL_IMAGE,
										 (width * cinfo.output_components + 3) & ~3, (JDIMENSION)1);
...
bool rgb565_2jpeg(Context currentContext, TCObject srcStreamObj, TCObject dstStreamObj, int32 width,
				  int32 height) {
	...
	srcFile.readBytesMethod = getMethod(OBJ_CLASS(srcFile.inputStreamObj), true, "readBytes", 3,
										BYTE_ARRAY, J_INT, J_INT);
	...
	dstFile.writeBytesMethod = getMethod(OBJ_CLASS(dstFile.outputStreamObj), true, "writeBytes", 3,
										 BYTE_ARRAY, J_INT, J_INT);
	...
	while (cinfo.next_scanline < cinfo.image_height) { /* Process data */
		executeMethod(srcFile.currentContext, srcFile.readBytesMethod, srcFile.params[0].asObj,
					  srcFile.params[1].asObj, 0, scanLineIn);
...
bool image2jpeg(Context currentContext, TCObject srcImageObj, TCObject dstStreamObj,
	...
	TCObject pixObj = (Image_frameCount(srcImageObj) > 1) ? Image_pixelsOfAllFrames(
						  srcImageObj) : Image_pixels(srcImageObj);
	PixelConv* pixels = (PixelConv*)ARRAYOBJ_START(pixObj);
	int32 width = (Image_frameCount(srcImageObj) > 1) ? Image_widthOfAllFrames(
					  srcImageObj) : Image_width(srcImageObj);
