 ...
 */
SQLInsertStatement*
initSQLInsertStatement(Context context, TCObject driver, LitebaseParser* parser)
{
  ...
  SQLInsertStatement* insertStmt =
    (SQLInsertStatement*)TC_heapAlloc(heap, sizeof(SQLInsertStatement));

  // On Litebase, a table has no alias name on insert. This has no sense. So the
  // same name of the table will be used as an alias. The parser must be changed
  // to understand the alias table name. Gets the statement base table.
  Table* table = insertStmt->table = getTable(
    context, driver, insertStmt->tableName = (*parser->tableList)->tableName);
  ...
  if (((LitebaseParser*)parser)
        ->fieldNamesSize) { // Checks if it is not using the default order.
    ...
    *fields = null;
    insertStmt->storeNulls =
      TC_heapAlloc(heap, NUMBEROFBYTES(table->columnCount));
    ...
    xmemset(insertStmt->storeNulls = table->storeNulls,
            false,
  ...
  record = insertStmt->record =
    (SQLValue**)TC_heapAlloc(heap, (i = table->columnCount) * TSIZE);

  // Allocates space for the list of the parameters. Worst case: all fields are
  // parameters.
  insertStmt->paramIndexes = (uint8*)TC_heapAlloc(heap, i);
  insertStmt->paramDefined = (uint8*)TC_heapAlloc(heap, i);

  // juliana@227_9: corrected a possible crash if the number of columns of the
  // insert were greater than the one of table definition.
  ...
  *record = (SQLValue*)TC_heapAlloc(heap, sizeof(SQLValue));
  while (--i) {
    if ((value = parser->fieldValues[i - 1])) { // Only stores values that are
                                                // not null.
 ...
 */
bool
setNumericParamValueIns(Context context,
                        SQLInsertStatement* insertStmt,
                        int32 index,
                        VoidP value,
                        int32 type)
{
  ...
  if (checkInsertIndex(context,
                       insertStmt,
                       index)) { // Checks if the index is within the range of
                                 // the parameter count.
    // Checks if the column type is the same of the value type.
    if (insertStmt->table->columnTypes[i = insertStmt->paramIndexes[index]] !=
        type) {
      TC_throwExceptionNamed(context,
    ...
    setBit(insertStmt->storeNulls,
           i,
           (record = insertStmt->record[i])->isNull = false);
    switch (type) {
      case SHORT_TYPE:
 ...
 */
bool
setStrBlobParamValueIns(Context context,
                        SQLInsertStatement* insertStmt,
                        int32 index,
                        VoidP value,
                        int32 length,
                        bool isStr)
{
  ...
  if (checkInsertIndex(context,
    ...
    if ((!isStr &&
         insertStmt->table->columnTypes[i = insertStmt->paramIndexes[index]] !=
           BLOB_TYPE) ||
        (isStr &&
         insertStmt->table->columnTypes[i = insertStmt->paramIndexes[index]] ==
           BLOB_TYPE)) {
 ...
 */
bool
setNullIns(Context context, SQLInsertStatement* insertStmt, int32 index)
{
  TRACE("setNull")
  SQLValue* record;
  int32 i;
  ...
  if (checkInsertIndex(context,
                       ...
                       index)) { // Checks if the index is within the range of
                                 // the parameter count.
    // The value is null.
    (record = insertStmt->record[i = insertStmt->paramIndexes[index]])->length =
      0;
    record->asChars = null;
    record->asBlob = null;
    ...
    setBit(insertStmt->storeNulls,
           i,
           insertStmt->paramDefined[index] = record->isNull = true);
...
/**
 * Throws an exception if the index to set a parameter in the insert prepared
 * statement is invalid. If the index is correct, it erases or creates the
 * record.
 *
 * @param context The thread context where the function is being executed.
 * @param insertStmt A SQL insert statement.
 ...
 */
bool
checkInsertIndex(Context context, SQLInsertStatement* insertStmt, int32 index)
{
  int32 i;
  ...
  if (insertStmt->record[i = insertStmt->paramIndexes[index]]) {
    ...
    insertStmt->record[i] =
      (SQLValue*)TC_heapAlloc(insertStmt->heap, sizeof(SQLValue));
  }
 ...
 */
void
clearParamValuesIns(SQLInsertStatement* insertStmt)
{
  TRACE("clearParamValuesIns")
  int32 i = insertStmt->paramCount, j;
 ...
 * @param context The thread context where the function is being executed.
 * @param insertStmt A SQL insert statement.
 * return <code>true</code> if the insertion was performed successfully;
 * <code>false</code>, otherwise.
 */
bool
litebaseDoInsert(Context context, SQLInsertStatement* insertStmt)
{
  ...
  // not modified since its last opening. Verifies if the nulls do not violate a
  // null restriction and writes the record.
 ...
 * <code>false</code> otherwise.
 * @throws SQLParseException If the number of values inserted is different from
 * the table definition.
 */
bool
litebaseBindInsertStatement(Context context, SQLInsertStatement* insertStmt)
{
  TRACE("litebaseBindInsertStatement")
  int32 i = 0, valuesCount = insertStmt->nFields, paramCount = 0;
  ...
  while (++i < valuesCount) // Checks if there are undefined values.
    ...
    if (record[i] && record[i]->asChars &&
        record[i]->asChars[0] == (JChar)'?' && !record[i]->asChars[1]) {
      paramIndexes[paramCount++] = i;
    }

  // No fields: The values are ordered.
  if (fields && !reorder(context,
                         table,
                         fields,
                         record,
                         storeNulls,
  ...
  if (insertStmt->nFields !=
      table->columnCount) { // The record to be inserted size must math the
                            // table record size.
    TC_throwExceptionNamed(context,
                           "litebase.SQLParseException",
                           getMessage(ERR_NUMBER_VALUES_DIFF_TABLE_DEFINITION),
                           insertStmt->nFields - 1);
    return false;
  }

  if (!convertStringsToValues(
        context,
        table,
        record,
        insertStmt
          ->nFields)) { // Converts the string values to their right types.
    return false;
  }
