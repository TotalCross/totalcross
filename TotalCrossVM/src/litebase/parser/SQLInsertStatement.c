...
SQLInsertStatement* initSQLInsertStatement(Context context, TCObject driver,
	...
	SQLInsertStatement* insertStmt = (SQLInsertStatement*)TC_heapAlloc(heap,
									 sizeof(SQLInsertStatement));
	...
	Table* table = insertStmt->table = getTable(context, driver,
									   insertStmt->tableName = (*parser->tableList)->tableName);
	...
	if ((i = parser->fieldValuesSize + 1) > table->columnCount) {
		TC_throwExceptionNamed(context, "litebase.SQLParseException",
							   getMessage(ERR_NUMBER_VALUES_DIFF_TABLE_DEFINITION), 0);
		return null;
	}
...
bool setNumericParamValueIns(Context context, SQLInsertStatement* insertStmt, int32 index,
	...
	if (checkInsertIndex(context, insertStmt,
						 index)) { // Checks if the index is within the range of the parameter count.
...
bool litebaseBindInsertStatement(Context context, SQLInsertStatement* insertStmt) {
	...
	while (++i < valuesCount) // Checks if there are undefined values.
		...
		if (record[i] && record[i]->asChars && record[i]->asChars[0] == (JChar)'?'
			&& !record[i]->asChars[1]) {
	...
	if (fields
		&& !reorder(context, table, fields, record, storeNulls, &insertStmt->nFields, paramIndexes)) {
		return false;
	}

	if (insertStmt->nFields !=
		table->columnCount) { // The record to be inserted size must math the table record size.
		TC_throwExceptionNamed(context, "litebase.SQLParseException",
							   getMessage(ERR_NUMBER_VALUES_DIFF_TABLE_DEFINITION), insertStmt->nFields - 1);
		return false;
	}

	if (!convertStringsToValues(context, table, record,
								insertStmt->nFields)) { // Converts the string values to their right types.
		return false;
	}
