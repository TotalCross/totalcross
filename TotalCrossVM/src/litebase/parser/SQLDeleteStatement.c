 ...
 */
SQLDeleteStatement*
initSQLDeleteStatement(LitebaseParser* parser, bool isPrepared)
{
  TRACE("initSQLDeleteStatement")
  SQLDeleteStatement* deleteStmt =
    (SQLDeleteStatement*)TC_heapAlloc(parser->heap, sizeof(SQLDeleteStatement));
  ...
  if (isPrepared) { // It is only necessary to re-allocate the parser structures
    ...
    deleteStmt->rsTable =
      initSQLResultSetTable((*parser->tableList)->tableName,
                            (*parser->tableList)->aliasTableName,
                            heap);
    if (whereClause) {
      whereClause->fieldList = (SQLResultSetField**)TC_heapAlloc(
        heap, whereClause->fieldsCount * TSIZE);
      xmemmove(whereClause->fieldList,
               parser->whereFieldList,
               whereClause->fieldsCount * TSIZE);
      whereClause->paramList = (SQLBooleanClauseTree**)TC_heapAlloc(
        heap, whereClause->paramCount * TSIZE);
      xmemmove(whereClause->paramList,
               parser->whereParamList,
               whereClause->paramCount * TSIZE);
    }
  } else {
 ...
 */
bool
setNumericParamValueDel(Context context,
                        SQLDeleteStatement* deleteStmt,
                        int32 index,
                        VoidP value,
                        int32 type)
 ...
 */
bool
setParamValueStringDel(Context context,
                       SQLDeleteStatement* deleteStmt,
                       int32 index,
                       JCharP value,
                       int32 length)
 ...
 */
void
clearParamValuesDel(SQLDeleteStatement* deleteStmt)
{
 ...
 */
bool
allParamValuesDefinedDel(SQLDeleteStatement* deleteStmt)
{
 ...
 */
int32
litebaseDoDelete(Context context, SQLDeleteStatement* deleteStmt)
{
  ...
  uint8* basbuf;
  Index* index;
  int32 nn = 0, i, id, columnCount, numberComposedIndexes;
  bool hasIndexes;
  Heap heap = null;
  ...
  hasIndexes =
    (composedIndexes = table->composedIndexes) != null; // juliana@201_6
  ...
  if (!whereClause) { // Deletes the whole table.
    if (hasIndexes) { // If the whole table is being deleted, just empties all
                      // indexes.
      i = columnCount;
      while (--i >= 0)
        if (columnIndexes[i] &&
            !indexDeleteAllRows(context, columnIndexes[i])) {
          return -1;
        }

      if ((i = numberComposedIndexes)) // juliana@201_6: It now deletes the
                                       // erases the composed index when
    ...
    // guich@300: now all records are just marked as deleted instead of physical
    // removal.
    ...
    if (hasIndexes) { // Removes the value of the index from each column of this
                      // record.
      ...
      // juliana@202_3: Solved a bug that could cause a GPF when using composed
      // indices.
      ...
      i = columnCount;
      while (--i >= 0) {
        if ((index = columnIndexes[i])) { // The max size of a char column of a
                                          // simple index.
          maxSize0 = MAX(maxSize0, columnSizes[i]);
        }

        // Calculates the maximum char column size for each column index of all
        // the composed indices.
        ...
        // Allocates the char buffers if necessary. The simple index will be
        // allocated in the first record.
        if (i > 0) {
          if (maxSize > 0) {
            keys[i]->asChars = (JCharP)TC_heapAlloc(heap, (maxSize << 1) + 2);
          }
        } else {
          // Gets the greatest size between the first column of all the composed
          // indices and the max size of all the simple indices.
...
error:
  if (nn > 0 &&
      !tableSaveMetaData(context, table, TSMD_ONLY_DELETEDROWSCOUNT)) {
    return -1;
  }
  ...
  // 10 is passed to LitebaseConnection.setRowInc().
  if (!dbFile->dontFlush) { // juliana@202_23: flushs the files to disk when row
 ...
 */
bool
litebaseBindDeleteStatement(Context context,
                            TCObject driver,
                            SQLDeleteStatement* deleteStmt)
{
  TRACE("litebaseBindDeleteStatement")
  Table* table = deleteStmt->rsTable->table =
    getTable(context, driver, deleteStmt->rsTable->tableName);

  // Binds the delete statement to its table.
  if (!table || (deleteStmt->whereClause &&
                 !bindColumnsSQLBooleanClauseSimple(context,
