...
SQLSelectStatement* initSQLSelectStatement(LitebaseParser* parser, bool isPrepared) {
	...
	SQLSelectStatement* selectStmt = (SQLSelectStatement*)TC_heapAlloc(heap,
									 sizeof(SQLSelectStatement));
	...
	selectClause = selectStmt->selectClause = (SQLSelectClause*)TC_heapAlloc(heap,
				   sizeof(SQLSelectClause));
	xmemmove(selectClause, &parser->select, sizeof(SQLSelectClause));
	selectClause->fieldList = (SQLResultSetField**)TC_heapAlloc(heap,
							  count = ((selectClause->fieldsCount ? selectClause->fieldsCount : MAXIMUMS) * TSIZE));
	xmemmove(selectClause->fieldList, parser->selectFieldList, count);
	selectClause->tableList = (SQLResultSetTable**)TC_heapAlloc(heap,
							  count = (parser->select.tableListSize * TSIZE));
	...
	if (isPrepared) { // It is only necessary to re-allocate the parser structures if the statement is from a prepared statement.
		if (parser->groupBy.fieldsCount) { // Sets the group by clause.
			listClause = selectStmt->groupByClause = (SQLColumnListClause*)TC_heapAlloc(heap,
						 sizeof(SQLColumnListClause));
			xmemmove(listClause, &parser->groupBy, sizeof(SQLColumnListClause));
			listClause->fieldList = (SQLResultSetField**)TC_heapAlloc(heap,
									count = (parser->groupBy.fieldsCount * TSIZE));
			xmemmove(listClause->fieldList, parser->groupByfieldList, count);
		}

		if (parser->orderBy.fieldsCount) { // Sets the order by clause.
			listClause = selectStmt->orderByClause = (SQLColumnListClause*)TC_heapAlloc(heap,
						 sizeof(SQLColumnListClause));
			xmemmove(listClause, &parser->orderBy, sizeof(SQLColumnListClause));
			listClause->fieldList = (SQLResultSetField**)TC_heapAlloc(heap,
									count = (parser->orderBy.fieldsCount * TSIZE));
			xmemmove(listClause->fieldList, parser->orderByfieldList, count);
		}

		if (whereClause) {
			whereClause->fieldList = (SQLResultSetField**)TC_heapAlloc(heap,
									 count = (whereClause->fieldsCount * TSIZE));
			xmemmove(whereClause->fieldList, parser->whereFieldList, count);
			whereClause->paramList = (SQLBooleanClauseTree**)TC_heapAlloc(heap,
									 count = (whereClause->paramCount * TSIZE));
			xmemmove(whereClause->paramList, parser->whereParamList, count);
		}

		if (havingClause) {
			havingClause->fieldList = (SQLResultSetField**)TC_heapAlloc(heap,
									  count = (havingClause->fieldsCount * TSIZE));
			xmemmove(havingClause->fieldList, parser->havingFieldList, count);
			havingClause->paramList = (SQLBooleanClauseTree**)TC_heapAlloc(heap,
									  count = (havingClause->paramCount * TSIZE));
 ...
 */
bool setNumericParamValueSel(Context context, SQLSelectStatement* selectStmt, int32 index,
	...
	if (index < 0 || index >= (whereParamCount + (havingClause ? havingClause->paramCount : 0))) {
		TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_INVALID_PARAMETER_INDEX),
							   index);
		return false;
		...
		return setNumericParamValue(context, havingClause->paramList[index - whereParamCount], value,
									type);
	}
}
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 */
bool setParamValueStringSel(Context context, SQLSelectStatement* selectStmt, int32 index,
	...
	if (index < 0 || index >= (whereParamCount + (havingClause ? havingClause->paramCount : 0))) {
		...
		return setParamValueString(context, havingClause->paramList[index - whereParamCount], value,
								   length);
	}
}
...
bool allParamValuesDefinedSel(SQLSelectStatement* selectStmt) {
	...
	if ((clause =
			 selectStmt->havingClause)) { // Checks if all the having clause parameters are defined.
		i = clause->paramCount;
		paramList = clause->paramList;
		while (--i >= 0)
			if (!paramList[i]->isParamValueDefined) {
...
TCObject litebaseDoSelect(Context context, TCObject driver, SQLSelectStatement* selectStmt) {
	...
	while (--i >= 0) {
		if (!tableList[i]->table) {
			TC_throwExceptionNamed(context, "litebase.DriverException", getMessage(ERR_CANT_READ),
	...
	if (!selectStmt->groupByClause && !selectStmt->havingClause && !selectStmt->orderByClause
		...
		if (!*rsBaseTable->name) {
			...
			if (!remapColumnsNames2Aliases(context, rsBaseTable, fieldList, fieldListLen)
				|| !plainShrinkToSize(context,
									  &rsBaseTable->db)) { // guich@201_9: always shrink the .db and .dbo memory files.
	...
	bag->isSimpleSelect =
		isSimpleSelect; // juliana@210_1: select * from table_name does not create a temporary table anymore.
...
bool litebaseBindSelectStatement(Context context, TCObject driver, SQLSelectStatement* selectStmt) {
	...
	if (!bindSelectStatement(context,
							 selectStmt)) { // Validates and binds the select statement to the table list.
		return false;
	}
...
bool bindSelectStatement(Context context, SQLSelectStatement* selectStmt) {
	...
	if (!bindColumnsSQLSelectClause(context, selectClause)
		|| (selectStmt->whereClause
			&& !bindColumnsSQLBooleanClause(context, selectStmt->whereClause, names2Index, columnTypes,
											tableList, size, selectStmt->selectClause->heap))
		|| (selectStmt->groupByClause
			&& !bindColumnsSQLColumnListClause(context, selectStmt->groupByClause, names2Index, columnTypes,
					tableList, size))
		|| (selectStmt->orderByClause
			&& !bindColumnsSQLColumnListClause(context, selectStmt->orderByClause, names2Index, columnTypes,
					tableList, size))) {
		return false;
	}
	return true;
...
bool validateSelectStatement(Context context, SQLSelectStatement* selectStmt) {
	...
	if (groupByClause && orderByClause && !sqlcolumnlistclauseEquals(groupByClause, orderByClause)) {
		TC_throwExceptionNamed(context, "litebase.SQLParseException",
							   getMessage(ERR_ORDER_GROUPBY_MUST_MATCH), 0);
		return false;
	}

	if (groupByClause) { // Validates the group by clause if it exists.
		while (--i >=
			   0) { // Checks if all fields referenced in the HAVING clause are listed as aliased aggregated functions in the SELECT clause.
			...
			if (!field1->isAggregatedFunction && field1->isVirtual) {
				TC_throwExceptionNamed(context, "litebase.SQLParseException",
									   getMessage(ERR_VIRTUAL_COLUMN_ON_GROUPBY), 0);
				return false;
			}
			...
			if (!field1->isAggregatedFunction
				...
				TC_throwExceptionNamed(context, "litebase.SQLParseException",
									   getMessage(ERR_AGGREG_FUNCTION_ISNOT_ON_SELECT), 0);
				return false;
			}
		}
			...
			while (--i >= 0) {
				...
				while (--j >= 0) {
					if (field1->aliasHashCode == (field2 = selectFields[j])->aliasHashCode) {
						...
						TC_throwExceptionNamed(context, "litebase.SQLParseException",
											   getMessage(ERR_IS_NOT_AGGREG_FUNCTION), field1->alias);
						return false;
					}
				}
				...
				if (!found) { // The having clause fields must be in the select clause fields.
					TC_throwExceptionNamed(context, "litebase.SQLParseException",
										   getMessage(ERR_WAS_NOT_LISTED_ON_AGGREG_FUNCTION), field1->alias);
					return false;
				}
			}
		}
		...
		if (selectClause->hasRealColumns && selectClause->hasAggFunctions) {
			TC_throwExceptionNamed(context, "litebase.SQLParseException",
								   getMessage(ERR_CANNOT_MIX_AGGREG_FUNCTION), 0);
			return false;
		}
		...
		if (selectClause->hasAggFunctions && orderByClause) {
			TC_throwExceptionNamed(context, "litebase.SQLParseException",
								   getMessage(ERR_CANNOT_HAVE_AGGREG_AND_NO_GROUPBY), 0);
			return false;
		}
	}
	return true;
...
Table* generateResultSetTable(Context context, TCObject driver, SQLSelectStatement* selectStmt) {
	...
	if (selectFieldsCount == 1 && !groupByClause && (*fieldList)->sqlFunction == FUNCTION_AGG_COUNT
	...
	while (++i < selectFieldsCount) {
		...
		if (field->isVirtual) {
			...
			if ((param = field->parameter)) {
				...
				TC_htPut32(&colHashesTable, param->aliasHashCode,
						   1);  // juliana@253_1: corrected a bug when sorting if the sort field is in a function.
				...
				columnIndexes[size] =
					-1; // This is just a place holder, since this column does not map to any column in the database.
			...
			// juliana@270_24: corrected a possible application crash or exception when using order/group by with join.
			TC_htPut32(&colHashesTable, field->aliasHashCode,
					   0);  // juliana@253_1: corrected a bug when sorting if the sort field is in a function.
	...
	if (sortListClause) {
		findSortIndex(
			sortListClause); // juliana@230_29: order by and group by now use indices on simple queries.
	...
	if (!whereClause && !sortListClause && selectClause->hasAggFunctions && numTables == 1) {
		...
		if ((sortListClause && sortListClause->index == -1) || countQueryWithWhere || numTables != 1) {
			...
			if ((tempTable1 = driverCreateTable(context, driver, null, null, duplicateIntArray(columnHashes,
												size, heap_1),
				...
				totalRecords = writeResultSetToTable(context, listRsTemp, numTables, tempTable1, columnIndexes,
													 selectClause, columnIndexesTables,
			...
			if (selectClause->type == COUNT_WITH_WHERE) {
				...
				return createIntValueTable(context, driver, totalRecords, countAlias);
				   ...
				   && !sortListClause) { // A query that use index for MAX() and MIN() should not check now which rows are answered.
			...
			if (newLength > oldLength)
				if (!(tempTable1->allRowsBitmap = allRowsBitmap = xrealloc(allRowsBitmap,
												  tempTable1->allRowsBitmapLength = newLength))) {
					TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
					goto error;
				}
	...
	if (sortListClause) { // Sorts the temporary table, if required.
		if (sortListClause->index == -1) {
			...
			if ((tempTable1 = driverCreateTable(context, driver, null, null, duplicateIntArray(columnHashes,
				...
				if (!(plainDB->rowAvail = (rsTemp->rowsBitmap.items ? bitCount(rsTemp->rowsBitmap.items,
				...
				} else if (!sortRecordsDesc(context, index, &rsTemp->rowsBitmap, tempTable1, record, columnIndexes,
											heap)) {
					goto error;
				}
				...
				if (groupByClause
					&& !bindColumnsSQLColumnListClause(context, groupByClause, &tempTable1->htName2index, types, null,
													   0)) {
					goto error;
				}
	...
	if (!(tempTable2 = driverCreateTable(context, driver, null, null, duplicateIntArray(columnHashes,
										 size, heap_2),
										 duplicateByteArray(columnTypes, size, heap_2), duplicateIntArray(columnSizes, size, heap_2), null,
										 null,
	...
	if ((aggFunctionExist = selectClause->hasAggFunctions)
		...
		if (*tempTable1->name) {
			...
			while (--i >= 0) {
				aggFunctionsRealParamCols[i] = (param = fieldList[aggFunctionsParamCols[i]]->parameter) ?
											   param->tableColIndex : -1;
			}
		}
	}

	// juliana@227_12: corrected a possible bug with MAX() and MIN() with strings.
	curRecord = record1 = newSQLValues(i = (count = tempTable1->columnCount) > selectFieldsCount ?
										   count : selectFieldsCount, heap);
	...
	if (useIndex) {
		...
		if (!(tempTable1->db.rowCount - tempTable1->deletedRowsCount) || (whereClause
				&& !bitCount(rowsBitmap->items, rowsBitmap->length))) {
			heapDestroy(heap);
			return tempTable2;
		}
	...
	paramCols = (isTableTemporary = !*tempTable1->name) ? aggFunctionsParamCols :
				aggFunctionsRealParamCols;

	xmemzero(groupCountCols, aggFunctionsColsCount <<
	...
	i = count;
	while (--i >= 0)
		if ((i < selectFieldsCount && columnSizesItems1[i] > columnSizesItems2[i]) || (columnSizesItems1[i]
				&& !record1[i]->asChars)) {
	...
	while (--i >= 0) {
		...
		if (paramCols[i] >= 0 && columnSizesItems1[paramCols[i]]) {
			aggFunctionsRunTotals[i].asChars = (JCharP)TC_heapAlloc(heap,
											   (columnSizesItems1[paramCols[i]] << 1) + 2);
		}
	}
	...
	for (i = -1, groupCount = 0; ++i < totalRecords; groupCount++) {
		...
		} else if (!readRecord(context, tempTable1, curRecord, i, nullsCurRecord, null, 0, true, null,
		...
		if (groupByClause && groupCount
			&& compareRecords(prevRecord, curRecord, nullsPrevRecord, nullsCurRecord, count, groupList)) {
			if (aggFunctionExist) // Checks if there are aggregate functions and concludes any aggregated function calculation.
				endAggFunctionsCalc(prevRecord, groupCount, aggFunctionsRunTotals, aggFunctionsCodes,
									aggFunctionsParamCols, paramCols,
		...
		if (aggFunctionExist)
			performAggFunctionsCalc(context, curRecord, nullsCurRecord, aggFunctionsRunTotals,
									aggFunctionsCodes, paramCols, aggFunctionsColsCount,
	...
	if (writeDelayed && groupCount > 0) { // If there was adelayed writing, flushes the last record.
		...
		xmemmove(columnNulls0, nullsCurRecord,
				 numOfBytes); // Takes the null values for the non-aggregate fields into consideration.
		j = aggFunctionsColsCount;
		while (--j >= 0) {
			setBit(columnNulls0, aggFunctionsParamCols[j], !groupCountCols[j]);
			groupCountCols[j] = 0;
	...
	if (!(tempTable3 = driverCreateTable(context, driver, null, null, duplicateIntArray(columnHashes,
										 size, heap_3),
										 duplicateByteArray(columnTypes, size, heap_3), duplicateIntArray(columnSizes, size, heap_3), null,
										 null,
	...
	if (!remapColumnsNames2Aliases(context, tempTable3, fieldList, selectClause->fieldsCount) ||
		!bindColumnsSQLBooleanClause(context, havingClause, &tempTable3->htName2index,
									 tempTable3->columnTypes, null, 0, heap_3)) {
		goto error;
	}
	...
	if (writeResultSetToTable(context, &rsTemp, 1, tempTable3, null, selectClause, null, -1,
 ...
 */
bool createListResultSetForSelect(Context context, SQLResultSetTable** tableList, int32 size,
								  SQLBooleanClause* whereClause, ResultSet** rsList, Heap heap) {
 ...
 */
bool generateIndexedRowsMap(Context context, ResultSet** rsList, int32 size, bool hasComposedIndex,
							Heap heap) {
	...
	if (whereClause) {
		...
		} else if (!applyTableIndexes(whereClause, table->columnIndexes, table->columnCount,
									  hasComposedIndex)) {
			return true;
		}
		...
		if (!whereClause->expressionTree)
			while (--size >=
				   0) { // There is no where clause left, since all rows can be returned using the indexes.
 ...
 * @return <code>true</code> if the function executed correctly; <code>false</code>, otherwise.
 */
bool computeIndex(Context context, ResultSet** rsList, int32 size, bool isJoin,
				  int32 indexRsOnTheFly, SQLValue* value, int32 operator,
	...
	while (++i < count) {
		...
		while (--j >= 0) {
			...
			else if (booleanOp == OP_BOOLEAN_AND && i < (count - 1) && indexedCols[i + 1] == col
		...
		if (rsBag->indexCount > 1) {
			mergeBitmaps((onTheFly ? &rsBag->auxRowsBitmap : &rsBag->rowsBitmap), markBits.indexBitmap,
						 booleanOp);
		}

		rsBag->rowsBitmapBoolOp =
			booleanOp; // juliana@230_15: corrected a bug when using joins with indices which could cause an OutOfMemoryExcepion.
		if (isCI) {
			i += (size - 1);
		}
	}
 ...
 */
void endAggFunctionsCalc(SQLValue** record, int32 groupCount, SQLValue* aggFunctionsRunTotals,
						 int8* aggFunctionsCodes,
						 int32* aggFunctionsParamCols, int32* aggFunctionsRealParamCols, int32 aggFunctionsColsCount,
						 int8* columnTypes,
...
Table* createIntValueTable(Context context, TCObject driver, int32 intValue, CharP colName) {
	...
	*colType = INT_TYPE;
	...
	// juliana@223_14: solved possible memory problems.
	if ((table = driverCreateTable(context, driver, null, null, colHash, colType, colSize, null, null,
								   NO_PRIMARY_KEY, NO_PRIMARY_KEY, null, 0, 1,
 ...
 */
bool bindColumnsSQLSelectClause(Context context,
								SQLSelectClause* clause) { // guich@512_2: added columnnames
	...
	if (!fieldListSize) { // If the select clause has a wild card (is null), then expands the list using the column information from the given tables.
		...
		while (++j < tableListSize) {
			...
			while (++i < columnCount) { // guich@503_10: changed loop to exclude the rowid.
				...
				(field = fieldList[count++] = initSQLResultSetField(clause->heap))->alias =
					columnNames[i]; // guich@_512_2: stores the column name too.
		...
		while (++i < fieldListSize) { // Binds the listed colums to the table.
			...
			if (!(field = fieldList[i])->isAggregatedFunction || field->sqlFunction != FUNCTION_AGG_COUNT) {
				if (field->tableName) { // Checks the names.
					...
					if (!currentTable
						|| ((index = TC_htGet32Inv(&currentTable->htName2index,
					...
					if (field->aliasHashCode != TC_hashCodeFmt("sss", tableName = field->tableName, ".",
						...
						TC_htPut32(&htName2index, TC_hashCodeFmt("sss", tableName, ".", field->alias), i);
											 ...
											 hash1) < 0) { // Stores the name of the field once; only the first.
					...
					j = tableListSize;
					while (--j >= 0) {
						if ((auxIndex = TC_htGet32Inv(&(auxTable = (rsTableAux = tableList[j])->table)->htName2index,
													  field->tableColHashCode)) >= 0) {
							if (foundFirst) {
								TC_throwExceptionNamed(context, "litebase.SQLParseException", getMessage(ERR_AMBIGUOUS_COLUMN_NAME),
													   field->alias);
								return false;
							} else {
								foundFirst = true;
			...
			// rnovais@568_10
					 ...
					 || field->isDataTypeFunction) { // If it is an aggregated or data type function, maps its parameter.
					...
					if (field->isAggregatedFunction) { // rnovais@568_10
						...
						if (param->dataType == CHARS_TYPE
							...
							TC_throwExceptionNamed(context, "litebase.SQLParseException", getMessage(ERR_DATA_TYPE_FUNCTION),
												   ...
												   : (sqlFunction == FUNCTION_AGG_MIN) ? "min" : (sqlFunction == FUNCTION_AGG_AVG) ? "avg" :
 ...
 */
bool remapColumnsNames2Aliases(Context context, Table* table, SQLResultSetField** fieldsList,
							   int32 fieldsCount) {
	...
	if (!table->columnNames) { // previously created? don't overrite it
		...
		while (--fieldsCount >= 0) {
			...
			if (columnHashes[fieldsCount] ==
				field->aliasHashCode) { // Replaces the original mapping, if necessary.
				continue;
			}
			if (!TC_htPut32(tableName2Index, field->aliasHashCode,
 ...
 */
int32 writeResultSetToTable(Context context, ResultSet** list, int32 numTables, Table* table,
	...
	int32 countSelectedField =
		selectClause->fieldsCount, // rnovais@568_10: when it has an order by table.columnCount = selectClause.fieldsCount + 1.
		i,
		j,
		totalRecords = 0,
	...
	if (!resultSet->whereClause && !resultSet->rowsBitmap.size) {
		if (!mfGrowTo(context, memoryDB,
					  (tempDB->rowAvail = 1 + selectDB->rowCount - (*selectClause->tableList)->table->deletedRowsCount) *
					  rowSize)
	...
	if (!tempDB->rowAvail
		&& rowsBitmap.size) { // Uses the indices if the other approachs can't be useful.
	...
	if (numTables == 1) {
		...
		while (getNextRecord(context, resultSet, heap)) { // No preverify needed.
			...
			while (++i < count) { // Gets the values of the result set columns.
				...
				if (colIndex != -1 && isBitUnSet(rsNulls0, colIndex)
					&& !getTableColValue(context, resultSet, colIndex, values[i])) { // juliana@220_3
					goto error;
				}
		...
		totalRecords = performJoin(context, list, numTables, table, rs2TableColIndexes, values,
								   whereClauseType, heap);
	}
 ...
 */
int32 performJoin(Context context, ResultSet** list, int32 numTables, Table* table,
				  int16* rs2TableColIndexes, SQLValue** values,
	...
	while (currentIndexTable >= 0) {
		...
		switch (ret = getNextRecordJoin(context, currentIndexTable, verifyWhereCondition[currentIndexTable],
										numTables, whereClauseType, list, heap)) {
			...
			case VALIDATION_RECORD_INCOMPLETE: {
				...
				xmemmove(rsNulls0 = rsTable->columnNulls, rsTable->db.basbuf + rsTable->columnOffsets[rsCount],
				...
				while (--length >= 0) { // Fills the data of the current ResultSet.
					...
					if ((colIndex != -1) && !bitSet
						&& !getTableColValue(context, currentRs, colIndex, values[position])) { // juliana@220_3
						return -1;
					}
 ...
 */
int32 getNextRecordJoin(Context context, int32 rsIndex, bool verifyWhereCondition, int32 totalRs,
						int32 whereClauseType, ResultSet** rsList,
						Heap heap) {
	TRACE("getNextRecordJoin")
	ResultSet* resultSet = rsList[rsIndex];
	PlainDB* plainDB = &resultSet->table->db;
	uint8* basbuf = plainDB->basbuf;
	IntVector rowsBitmap = (resultSet->auxRowsBitmap.size > 0) ? resultSet->auxRowsBitmap :
	...
	if (rowsBitmap.size && verifyWhereCondition) {
		...
		if (resultSet->pos < rowCountLess1) {
			if (!resultSet->whereClause || resultSet->auxRowsBitmap.size) {
				...
				while ((position = findNextBitSet(&rowsBitmap, resultSet->pos + 1)) != -1
					   && position <= rowCountLess1) {
					if (plainRead(context, plainDB, resultSet->pos = position)) {
						if (recordNotDeleted(
								basbuf)) { // juliana@230_45: join should not take deleted rows into consideration.
				...
				if (resultSet->rowsBitmapBoolOp == OP_BOOLEAN_AND) {
					...
					while (resultSet->pos < rowCountLess1 && plainRead(context, plainDB, ++resultSet->pos)) {
						...
						if (recordNotDeleted(basbuf)
							&& (ret = booleanTreeEvaluateJoin(context, whereClause->expressionTree, rsList, totalRs,
															  heap)) != VALIDATION_RECORD_NOT_OK) {
							return ret;
						}
					}
				}
		...
		while (resultSet->pos < rowCountLess1 && plainRead(context, plainDB, ++resultSet->pos))
			if (recordNotDeleted(basbuf)) {
				...
				whereClause->resultSet = resultSet;
				if ((ret = booleanTreeEvaluateJoin(context, whereClause->expressionTree, rsList, totalRs,
												   heap)) == VALIDATION_RECORD_NOT_OK
 ...
 * <code>VALIDATION_RECORD_INCOMPLETE</code>, or -1 if an error occurs.
 */
int32 booleanTreeEvaluateJoin(Context context, SQLBooleanClauseTree* tree, ResultSet** rsList,
							  int32 totalRs, Heap heap) {
	...
	if (indexTree >= 0) { // AND, OR and BOOLEAN_NOT have index = -1.
		...
		if (indexTree >
			indexRs) { // juliana@211_5: solved a bug with joins which would return more answers than desired.
			if (tree->bothAreIdentifier && leftTree->indexRs == indexRs) { // Fills leftTree.value.
				...
				if (!valueJoin->asChars) {
					valueJoin->asChars = (JCharP)TC_heapAlloc(heap,
										 2 * resultSet->table->columnSizes[leftTree->colIndex] + 2);
				}
				...
				if (rightTree->hasIndex && boolOp <= 1) {
					...
					if (!computeIndex(context, rsList, totalRs, false, rightTree->indexRs, valueJoin, tree->operandType,
									  rightTree->colIndex, heap)) {
						return -1;
					}
	...
	switch (tree->operandType) { // Checks what is the operand type of the tree.
		...
		case OP_REL_LESS_EQUAL:
			switch (tree->valueType) { // Calls the right operation accordingly to the values type.
				...
				case DATETIME_TYPE:
					...
					return -1;
				case CHARS_TYPE:
					return compareStringOperands(context, tree, false,
												 heap) ? VALIDATION_RECORD_OK : VALIDATION_RECORD_NOT_OK;
				case CHARS_NOCASE_TYPE:
					return compareStringOperands(context, tree, true,
												 heap) ? VALIDATION_RECORD_OK : VALIDATION_RECORD_NOT_OK;
			}
		...
		case OP_PAT_MATCH_LIKE:
		case OP_PAT_MATCH_NOT_LIKE:
			return matchStringOperands(context, tree, tree->valueType == CHARS_NOCASE_TYPE,
									   heap) ? VALIDATION_RECORD_OK : VALIDATION_RECORD_NOT_OK;
			...
			if (leftTree && rightTree) { // Expects both trees not be null.
				switch (booleanTreeEvaluateJoin(context, leftTree, rsList, totalRs, heap)) {
					case VALIDATION_RECORD_NOT_OK:
						return VALIDATION_RECORD_NOT_OK;
					case VALIDATION_RECORD_INCOMPLETE:
						if (booleanTreeEvaluateJoin(context, rightTree, rsList, totalRs,
													heap) == VALIDATION_RECORD_NOT_OK) { // Verifies the right branch.
							return VALIDATION_RECORD_NOT_OK;
						}
					...
					case VALIDATION_RECORD_OK:
						switch (booleanTreeEvaluateJoin(context, rightTree, rsList, totalRs,
														heap)) { // The left side returned true, so verifies the right branch.
							case VALIDATION_RECORD_NOT_OK:
								return VALIDATION_RECORD_NOT_OK;
							case VALIDATION_RECORD_INCOMPLETE_OK:
							case VALIDATION_RECORD_OK:
 ...
 */
void performAggFunctionsCalc(Context context, SQLValue** record, uint8* nullsRecord,
							 SQLValue* aggFunctionsRunTotals, int8* aggFunctionsCodes,
							 int32* aggFunctionsParamCols, int32 aggFunctionsColsCount, int8* columnTypes,
							 int32* groupCountCols) {
				...
				switch (colType) { // Checks the type of the column.
					...
					case CHARS_NOCASE_TYPE:
						if (groupCountCols[i] == 1
							|| str16CompareTo(aggValue->asChars, value->asChars, aggValue->length, value->length,
											  colType) < 0) {
					...
					case DATETIME_TYPE: // rnovais@567_2
						if (groupCountCols[i] == 1) {
							...
							if (aggValue->asDate < value->asDate
								|| aggValue->asTime < value->asTime) { // The date or time is smaller.
								aggValue->asDate = value->asDate;
								aggValue->asTime = value->asTime;
							}
						}
	...
	if (table->columnIndexes[column]) { // If the field has a simple index, uses it.
		...
		while (--i >= 0)
			if (*composedIndices[i]->columns ==
				column) { // Else, if the field is the first field of a composed index, uses it.
				field->index = i;
