                       ...
                       bool isPrepared)
{
  ...
  SQLUpdateStatement* updateStmt =
    (SQLUpdateStatement*)TC_heapAlloc(heap, sizeof(SQLUpdateStatement));
  ...
  if (isPrepared) { // Some structures from the parser does not need to be
                    // reallocated when not using prepared statements.
    updateStmt->rsTable =
  ...
  if (!(updateStmt->rsTable->table = table =
          getTable(context, driver, updateStmt->rsTable->tableName))) {
    return null;
  }
  ...
  while (--i >= 0) {
    ...
    if ((value =
           parser->fieldValues[i])) { // Only stores values that are not null.
                        ...
                        int32 type)
{
  ...
  if (checkUpdateIndex(context,
                       updateStmt,
                       index)) { // Checks if the index is within the range.
    if (index <
        updateStmt->paramCount) { // The parameter is in the update clause.
      int32 i;
      SQLValue* record;

      // Checks if the column type is the same of the value type.
      if (updateStmt->rsTable->table
            ->columnTypes[i = updateStmt->paramIndexes[index]] != type) {
        TC_throwExceptionNamed(context,
      ...
      setBit(updateStmt->storeNulls,
             i,
             (record = updateStmt->record[i])->isNull = false);
      switch (type) {
        case SHORT_TYPE:
      ...
      return setNumericParamValue(
        context,
        updateStmt->whereClause->paramList[index - updateStmt->paramCount],
        value,
        type);
    }
  }
                        ...
                        bool isStr)
{
  ...
  if (checkUpdateIndex(context,
    ...
    if (index <
      ...
      if ((!isStr && updateStmt->rsTable->table
                         ->columnTypes[i = updateStmt->paramIndexes[index]] !=
                       BLOB_TYPE) ||
          (isStr && updateStmt->rsTable->table
                        ->columnTypes[i = updateStmt->paramIndexes[index]] ==
                      BLOB_TYPE)) {
        TC_throwExceptionNamed(context,
                               "litebase.DriverException",
...
setNullUpd(Context context, SQLUpdateStatement* updateStmt, int32 index)
{
  ...
  if (checkUpdateIndex(context,
    ...
    if (index <
      ...
      // The value is null.
      (record = updateStmt->record[i = updateStmt->paramIndexes[index]])
        ->asChars = null;
      record->asBlob = null;
      record->length = 0;
      setBit(updateStmt->storeNulls, i, record->isNull = true);
  ...
  if (index < 0 ||
      index >=
        updateStmt->paramCount +
          (updateStmt->whereClause ? updateStmt->whereClause->paramCount : 0)) {
    TC_throwExceptionNamed(context,
...
setUpdateRecord(SQLUpdateStatement* updateStmt, int32 index)
{
  ...
  if (updateStmt->record[i]) {
    ...
    updateStmt->record[i] =
      (SQLValue*)TC_heapAlloc(updateStmt->heap, sizeof(SQLValue));
  }
...
clearParamValuesUpd(SQLUpdateStatement* updateStmt)
{
  ...
  int32 i = updateStmt->paramCount, j;
...
allParamValuesDefinedUpd(SQLUpdateStatement* updateStmt)
{
  ...
  while (--i >=
         0) // Checks if all the parameters of the update clause are defined.
    if (!paramDefined[i]) {
      return false;
    }

  if (whereClause) { // Checks if all pararameters of the where clause are
                     // defined.
    SQLBooleanClauseTree** paramList = whereClause->paramList;
    i = whereClause->paramCount;
    while (--i >= 0)
...
litebaseDoUpdate(Context context, SQLUpdateStatement* updateStmt)
{
  ...
  // not modified since its last opening. Verifies if there are any parameters
  // missing and the nulls do not violate a null restriction. Creates the result
  // set that will be used to update the rows.
  if (!verifyNullValues(
        context, table, record, CMD_UPDATE, updateStmt->nValues) ||
      !sqlBooleanClausePreVerify(context, updateStmt->whereClause) ||
      !(rs = createSimpleResultSet(
          context, table, updateStmt->whereClause, heap)) ||
 ...
 * <code>false</code> otherwise.
 * @throws <code>SQLParseException</code> if the number of fields is greater
 * than 254.
 */
bool
litebaseBindUpdateStatement(Context context, SQLUpdateStatement* updateStmt)
{
  ...
  int32 i = -1, valuesCount = updateStmt->nValues, paramCount = 0;
  ...
  // juliana@227_17: corrected a possible crash if one tries to update more than
  // 128 fields in a table.
  ...
  while (++i < valuesCount) // Checks if there are undefined values.
    ...
    if (record[i] && (asChars = record[i]->asChars) &&
  ...
  // Converts the values to be updated into its correct type and binds the where
  // clause to its table.
