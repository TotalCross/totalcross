...
void initLitebaseMessage(void) {
	...
	errorMsgs_en[ERR_NOT_NULL_DEFAULT] =
	...
	errorMsgs_pt[ERR_TABLE_OPENED] =
		"Uma tabela aberta n�o pode ser recuperada ou convertida: %s."; // juliana@230_12
...
TESTCASE(getMessage) {
	...
	// Limit errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_MAX_NUM_FIELDS_REACHED),
				   "Maximum number of different fields was reached.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_MAX_NUM_PARAMS_REACHED),
				   "Maximum number of parameters in the 'WHERE/HAVING' clause was reached.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_MAX_COMP_INDICES),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_FIELDS_OVERFLOW),
				   "The maximum number of fields in a SELECT clause was exceeded.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_FIELD_OVERFLOW_GROUPBY_ORDERBY),
				   "Maximum number of columns exceeded in the 'ORDER BY/GROUP BY' clause.");
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_COLUMN_DOESNOT_HAVE_AN_INDEX),
				   "The following column(s) does (do) not have an associated index %s.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_AMBIGUOUS_COLUMN_NAME),
	...
	// Primary key errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_PRIMARY_KEY_ALREADY_DEFINED),
				   "A primary key was already defined for this table.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_TABLE_DOESNOT_HAVE_PRIMARY_KEY),
				   "Table does not have a primary key.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_STATEMENT_CREATE_DUPLICATED_PK),
				   "Statement creates a duplicated primary key in %s.");

	// Type errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_INCOMPATIBLE_TYPES), "Incompatible types.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_FIELD_SIZE_IS_NOT_INT),
				   "Field size must be a positive interger value.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_INVALID_NUMBER),
				   "Value %s is not a valid number for the desired type: %s.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_DATA_TYPE_FUNCTION),
				   "Incompatible data type for the function call: %s");

	// Number of fields errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_NUMBER_FIELDS_AND_VALUES_DOES_NOT_MATCH),
				   "The number of fields does not match the number of values ");
	ASSERT2_EQUALS(Sz, getMessage(ERR_NUMBER_VALUES_DIFF_TABLE_DEFINITION),
				   "The given number of values does not match the table definition %d.");

	// Default value errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_LENGTH_DEFAULT_VALUE_IS_BIGGER),
				   "Length of default value is bigger than column size.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_NOT_NULL_DEFAULT),
				   "An added column declared as NOT NULL must have a not null default value.");
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_RESULTSETMETADATA_CLOSED),
				   "ResultSetMetaData cannot be used after the ResultSet is closed.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_INVALID_CRID),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_PREPARED_STMT_CLOSED),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_WRONG_STRING_FORMAT),
				   "It is not possible to open a table within a connection with a different string format.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_WRONG_CRYPTO_FORMAT),
				   "It is not possible to open a table within a connection with a different cryptography format.");
	...
	// Prepared Statement errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_QUERY_DOESNOT_RETURN_RESULTSET),
				   "Query does not return result set.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_QUERY_DOESNOT_PERFORM_UPDATE),
				   "Query does not perform updates in the database.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_NOT_ALL_PARAMETERS_DEFINED),
				   "Not all parameters of the query had their values defined.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_PARAMETER_NOT_DEFINED),
	...
	// Rename errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_TABLE_ALREADY_EXIST),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_DUPLICATE_ALIAS),
				   "This alias is already being used in this expression: %s.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_REQUIRED_ALIAS),
				   "An alias is required for the aggregate function column.");

	// Litebase.execute() error.
	ASSERT2_EQUALS(Sz, getMessage(ERR_ONLY_CREATE_TABLE_INDEX_IS_ALLOWED),
				   "Only CREATE TABLE and CREATE INDEX can be used in Litebase.execute().");

	// Order by and group by errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_ORDER_GROUPBY_MUST_MATCH),
				   "ORDER BY and GROUP BY clauses must match.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_VIRTUAL_COLUMN_ON_GROUPBY),
				   "No support for virtual columns in SQL queries with GROUP BY clause.");
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_DROP_PRIMARY_KEY),
				   "Can't drop a primary key index withdrop index.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_INDEX_LARGE),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_RS_DEC_PLACES_START),
				   "Invalid value for decimal places: %d. It must range from -1 to 40.");
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_TABLE_NOT_CLOSED),
				   "Table not closed properly: %s."); // juliana@220_2
	ASSERT2_EQUALS(Sz, getMessage(ERR_TABLE_CLOSED),
				   "A properly closed table can't be used in recoverTable(): %s."); // juliana@222_2
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_WRONG_VERSION),
				   "The table format (%d) is incompatible with Litebase version. Please update your tables.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_WRONG_PREV_VERSION),
				   "The table format is not the previous one: %s."); // juliana@220_11
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_TABLE_OPENED),
				   "An opened table can't be recovered or converted: %s."); // juliana@230_12

	// BLOB errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_BLOB_TOO_BIG),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_BLOB_ORDER_GROUP),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_BLOBS_PREPARED),
				   "It is only possible to insert or update a BLOB through prepared statements using setBlob().");
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_MAX_TABLE_NAME_LENGTH),
	...
	// ROWID errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_ROWID_CANNOT_BE_CHANGED),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_VALUE_ISNOT_DATETIME),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_PARAM_NULL),
				   "Um par�metro em uma where clause n�o pode ser NULL.");

	// Result set errors.
	ASSERT2_EQUALS(Sz, getMessage(ERR_RS_INV_POS),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_TABLE_CLOSED),
				   "Uma tabela fechada corretamente n�o pode ser usada no recoverTable(): %s."); // juliana@222_2
	ASSERT2_EQUALS(Sz, getMessage(ERR_IDX_RECORD_DEL),
				   "N�o � poss�vel achar a posi��o de registro no �ndice na exclus�o.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_WRONG_VERSION),
				   "O formato de tabela (%d) n�o � compat�vel com a vers�o do Litebase. Por favor, atualize suas tabelas.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_WRONG_PREV_VERSION),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_DB_NOT_FOUND),
	...
	ASSERT2_EQUALS(Sz, getMessage(ERR_INVALID_MULTIPLIER),
				   "O multiplicador de tamanho n�o � v�lido.");
	ASSERT2_EQUALS(Sz, getMessage(ERR_BLOB_PRIMARY_KEY),
...
TESTCASE(initLitebaseMessage) {
	...
	ASSERT2_EQUALS(Sz, errorMsgs_en[4],
				   "Maximum number of parameters in the 'WHERE/HAVING' clause was reached.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_en[7],
				   "The maximum number of fields in a SELECT clause was exceeded.");
	ASSERT2_EQUALS(Sz, errorMsgs_en[8],
				   "Maximum number of columns exceeded in the 'ORDER BY/GROUP BY' clause.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_en[12],
				   "The following column(s) does (do) not have an associated index %s.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_en[37],
				   "It is not possible to open a table within a connection with a different string format.");
	ASSERT2_EQUALS(Sz, errorMsgs_en[38],
				   "It is not possible to open a table within a connection with a different cryptography format.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_en[58],
				   "SUM and AVG aggregation functions are not used with DATE and DATETIME type fields.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_en[68],
				   "Invalid value for decimal places: %d. It must range from -1 to 40.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_en[73],
				   "A properly closed table can't be used in recoverTable(): %s."); // juliana@222_2
	ASSERT2_EQUALS(Sz, errorMsgs_en[74], "Can't find index record position on delete.");
	ASSERT2_EQUALS(Sz, errorMsgs_en[75],
				   "The table format (%d) is incompatible with Litebase version. Please update your tables.");
	ASSERT2_EQUALS(Sz, errorMsgs_en[76],
				   "The table format is not the previous one: %s."); // juliana@220_11
	...
	ASSERT2_EQUALS(Sz, errorMsgs_en[80],
				   "An opened table can't be recovered or converted: %s."); // juliana@230_12
	...
	ASSERT2_EQUALS(Sz, errorMsgs_en[89],
				   "It is only possible to insert or update a BLOB through prepared statements using setBlob().");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[4],
				   "N�mero m�ximo da lista de par�metros na cl�usula 'WHERE/HAVING' foi alcan�ado.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[7],
				   "O n�mero m�ximo de campos na cl�usula SELECT foi excedido.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[8],
				   "O n�mero m�ximo de campos na cl�usula 'ORDER BY/GROUP BY' foi excedido.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[12],
				   "A(s) coluna(s) a seguir n�o tem (t�m) um ind�ce associado %s.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[21],
				   "O valor %s n�o � um n�mero v�lido para o tipo desejado: %s.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[22],
				   "Tipo de dados incompat�vel para a chamada de fun��o: %s");

	// Number of fields errors.
	ASSERT2_EQUALS(Sz, errorMsgs_pt[23], "O n�mero de campos � diferente do n�mero de valores ");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[24],
				   "O n�mero de valores dado n�o coincide com a defini��o da tabela %d.");

	// Default value errors.
	ASSERT2_EQUALS(Sz, errorMsgs_pt[25],
				   "Tamanho do valor padr�o � maior que o tamanho definido para a coluna.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[29],
				   "ResultSetMetaData n�o pode ser usado depois que o ResultSet estiver fechado.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[37],
				   "N�o � poss�vel abrir uma tabela com uma conex�o com um tipo de strings diferente.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[38],
				   "N�o � poss�vel abrir uma tabela com uma conex�o com um tipo de criptografia diferente.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[41],
				   "Comando SQL n�o executa uma atualiza��o no banco de dados.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[42],
				   "Nem todos os par�metros da consulta tiveram seus valores definidos.");
	...
	// Rename errors.
	ASSERT2_EQUALS(Sz, errorMsgs_pt[45],
				   "N�o � poss�vel renomear a tabela. Esta tabela j� existe: %s.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[49],
				   "Um alias � necess�rio para colunas com fun��o de agrega��o.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[57],
				   "%s n�o foi listado no SELECT. Todas as colunas da cl�usula HAVING devem ser listadas no SELECT utilizando alias.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[58],
				   "Fun��es de agrega��o SUM e AVG n�o s�o usadas com colunas do tipo DATE e DATETIME.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[62],
				   "N�o � poss�vel remover uma chave prim�ria usando drop index.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[63],
				   "�ndice muito grande. Ele n�o pode ter mais do que 65534 n�s.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[68],
				   "Valor inv�lido para casas decimais: %d. Deve ficar entre - 1 e 40.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[73],
				   "Uma tabela fechada corretamente n�o pode ser usada no recoverTable(): %s."); // juliana@222_2
	ASSERT2_EQUALS(Sz, errorMsgs_pt[74],
				   "N�o � poss�vel achar a posi��o de registro no �ndice na exclus�o.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[75],
				   "O formato de tabela (%d) n�o � compat�vel com a vers�o do Litebase. Por favor, atualize suas tabelas.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[76],
				   "O formato de tabela n�o � o anterior: %s."); // juliana@220_11
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[80],
				   "Uma tabela aberta n�o pode ser recuperada ou convertida: %s."); // juliana@230_12

	// BLOB errors.
	ASSERT2_EQUALS(Sz, errorMsgs_pt[81],
				   "O tamanho total de um BLOB n�o pode ser maior do que 10 Mb.");
	...
	ASSERT2_EQUALS(Sz, errorMsgs_pt[87],
				   "Tipos BLOB n�o podem estar em cl�usulas ORDER BY ou GROUP BY.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[88], "N�o � poss�vel comparar BLOBs.");
	ASSERT2_EQUALS(Sz, errorMsgs_pt[89],
				   "S� � poss�vel inserir ou atualizar um BLOB atrav�s prepared statements usando setBlob().");
