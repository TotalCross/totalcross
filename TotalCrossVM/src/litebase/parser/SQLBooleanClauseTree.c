...
/**
 * Internal use only. A tree structure used to evaluate a SQL boolean clause for
 * a given result set.
 */
...
/**
 * Creates a <code>SQLBooleanClauseTree</code> with the associated
 * <code>SQLBooleanClause</code>.
 *
 * @param booleanClause The associated <code>SQLBooleanClause</code>.
 * @param heap The heap to allocate the <code>SQLBooleanClauseTree</code>
 * structure.
 * @return A pointer to a <code>SQLBooleanClauseTree</code> structure.
 */
SQLBooleanClauseTree*
initSQLBooleanClauseTree(SQLBooleanClause* booleanClause, Heap heap)
{
  TRACE("initSQLBooleanClauseTree")
  SQLBooleanClauseTree* tree =
    (SQLBooleanClauseTree*)TC_heapAlloc(heap, sizeof(SQLBooleanClauseTree));
 ...
 *
 * @param booleanClauseTree A pointer to a <code>SQLBooleanClauseTree</code>
 * structure.
 * @param value The string literal value.
 */
void
setOperandStringLiteral(SQLBooleanClauseTree* booleanClauseTree, JCharP value)
{
 ...
 * @throws DriverException If the parameter type is different from the value
 * type.
 */
bool
setNumericParamValue(Context context,
                     SQLBooleanClauseTree* booleanClauseTree,
                     VoidP value,
                     int32 type)
{
  ...
  if (booleanClauseTree->valueType == UNDEFINED_TYPE) {
    ...
    TC_throwExceptionNamed(
      context, "litebase.DriverException", getMessage(ERR_INCOMPATIBLE_TYPES));
    return false;
  }
  switch (type) {
    case SHORT_TYPE:
 ...
 */
bool
setParamValueString(Context context,
                    SQLBooleanClauseTree* booleanClauseTree,
                    JCharP value,
                    int32 length)
{
  ...
  // juliana@223_4: Solved a bug that could make a prepared statement string
  // parameter in a where or having clause cause a strange exception of invalid
  // number. If the string has length greater than any type in string format,
  // throws an exception.
  ...
  switch (type) { // Converts the string to the correct type.
    ...
    case DOUBLE_TYPE:
      ...
      break;
    case DATE_TYPE: // rnovais@570_55: If the type is DATE, checks if it is
                    // valid and converts it to int.
    case DATETIME_TYPE: // If the type is DATETIME, checks if it is valid and
                        // converts it to 2 ints.
...
/**
 * Checks the if the operand value string contains pattern matching characters
 * and assigns the proper matching type.
 *
 * @param booleanClauseTree A pointer to a <code>SQLBooleanClauseTree</code>
 * structure.
 */
void
setPatternMatchType(SQLBooleanClauseTree* booleanClauseTree)
{
  ...
  if (length > 0) { // guich@512_5: checks if there is any pattern matching.
    JChar firstChar = value[0], lastChar = value[length - 1];
 ...
 */
void
weightTheTree(SQLBooleanClauseTree* booleanClauseTree)
{
  TRACE("weightTheTree")
  ...
  switch (booleanClauseTree->operandType) { // Checks the type of the operand.
    ...
    default: { // The others.
      SQLResultSetField** fieldList =
        booleanClauseTree->booleanClause->fieldList;
      Hashtable* fieldName2Index =
        &booleanClauseTree->booleanClause->fieldName2Index;
      SQLResultSetField* leftField = fieldList[TC_htGet32(
        fieldName2Index, leftTree->nameSqlFunctionHashCode)];
      SQLResultSetField* rightField = fieldList[TC_htGet32(
        fieldName2Index, rightTree->nameSqlFunctionHashCode)];
      Index* leftIndexStr =
        leftField->table->columnIndexes[leftField->tableColIndex];
      Index* rightIndexStr =
        rightField->table->columnIndexes[rightField->tableColIndex];
 ...
 */
void
setIndexRsOnTree(SQLBooleanClauseTree* booleanClauseTree)
{
  ...
  switch (booleanClauseTree->operandType) { // Checks the type of operand.
    ...
    case OP_PAT_IS:
    case OP_PAT_IS_NOT: {
      int32 leftIndex, rightIndex, fieldIndex;
      ...
      // identifier.
      if (leftTree->operandType == OP_IDENTIFIER &&
          rightTree->operandType == OP_IDENTIFIER) { // Both are identifier.
        ...
        // juliana@263_2: corrected a very old bug in a join with comparision
        // between two fields of the same table.
...
/**
 * Used for composed indices to find some properties related to a branch of the
 * expression tree.
 *
 * @param booleanClauseTree A pointer to a <code>SQLBooleanClauseTree</code>
 * structure.
 ...
 */
void
getBranchProperties(SQLBooleanClauseTree* booleanClauseTree,
                    uint8* columns,
                    uint8* operators,
                    ...
                    int32 fieldsCount)
{
  ...
  if (position >=
      fieldsCount) { // Does not let an <code>OutOfBoundsException</code>.
    return;
  }

  if (leftTree->operandType == OP_IDENTIFIER) { // One of the elements of the
                                                // branch must be an identifier.
...
/**
 * Gets a value from a result set and applies a sql function if there is one to
 * be applied.
 *
 * @param context The thread context where the function is being executed.
 * @param booleanClauseTree A pointer to a <code>SQLBooleanClauseTree</code>
 * structure.
 * @param value The value returned from the result set or the result of a
 * function on a value of the result set.
 * @return <code>true</code> if the value could be fetched correctly;
 * <code>false</code>, otherwise.
 */
bool
getOperandValue(Context context,
                SQLBooleanClauseTree* booleanClauseTree,
                SQLValue* value)
{
  TRACE("getOperandValue")

  // If the operand type is identifier, gets the value from the current row in
  // the result set. Otherwise, just returns the tree operand value.
  if (booleanClauseTree->operandType == OP_IDENTIFIER) {
    ...
    xmemmove(table->columnNulls,
             table->db.basbuf + table->columnOffsets[table->columnCount],
             NUMBEROFBYTES(table->columnCount));
    if (isBitSet(table->columnNulls,
                 booleanClauseTree->colIndex)) { // There is a null value.
      value->isNull = true;
      return true;
    }

    // rnovais@_570_1: created the last parameter for
    // <code>getTableColValue()</code>. See this function.
    if (!getTableColValue(
          context, resultSet, booleanClauseTree->colIndex, value)) {
      return false;
    }

    // rnovais@568_10: applies data type function.
    if ((field = booleanClause->fieldList[getFieldIndex(booleanClauseTree)])
          ->sqlFunction != FUNCTION_DT_NONE) {
      applyDataTypeFunction(
        value, field->sqlFunction, field->parameter->dataType);
    }
 ...
 * @param booleanClauseTree A pointer to a <code>SQLBooleanClauseTree</code>
 * structure.
 * @return <code>true</code> if the operand is null and a null value is being
 * searched or vice-versa; <code>false</code>, otherwise.
 */
bool
compareNullOperands(SQLBooleanClauseTree* booleanClauseTree)
{
 ...
 */
int32
compareNumericOperands(Context context, SQLBooleanClauseTree* booleanClauseTree)
{
  TRACE("compareNumericOperands")
  bool result = false;
  SQLBooleanClauseTree* leftTree = booleanClauseTree->leftTree;
  SQLBooleanClauseTree* rightTree = booleanClauseTree->rightTree;
  SQLValue leftValue, rightValue;
  int32 leftValueType = leftTree->valueType,
        rightValueType = rightTree->valueType,
        assignType = booleanClauseTree->isFloatingPointType
                       ? DOUBLE_TYPE
                       : leftValueType != LONG_TYPE &&
                             rightValueType != LONG_TYPE &&
                             leftValueType != DATETIME_TYPE
                           ? INT_TYPE
                           : LONG_TYPE,
        compareType =
          leftValueType == DATETIME_TYPE ? DATETIME_TYPE : assignType,
        leftValueAsInt = 0, rightValueAsInt = 0, leftValueAsTime = 0,
        rightValueAsTime = 0;
  int64 leftValueAsLong = 0, rightValueAsLong = 0;
  double leftValueAsDouble = 0, rightValueAsDouble = 0;
  ...
  if (leftValue.isNull ||
      rightValue.isNull) { // One of the values is a null value.
    return false;
  }
  ...
  switch (rightValueType) { // Getting right value.
    ...
    case CHARS_TYPE: // rnovais@567_2 : this is for DATE and DATETIME typed that
                     // are CHARS type in the sql.
  ...
  if (leftValueType == -1 ||
      rightValueType ==
        -1) { // Prevents problems when doing comparisons like 1 == 2.
    bool error;
    DoubleBuf buffer;
    ...
    // Strips off the final L, because <code>str2double()</code> does not like
    // it.
    ...
    if (rightValueAsString[last = TC_JCharPLen(rightValueAsString) - 1] ==
          'l' ||
        rightValueAsString[last] == 'L') {
    ...
    // juliana@225_12: a numeric constant in a boolean clause must have its type
    // considered to be double.
    TC_JCharP2CharPBuf(leftValueAsString, -1, buffer);
    leftTree->operandValue.asDouble = leftValue.asDouble = leftValueAsDouble =
      TC_str2double(buffer, &error);
    if (error) {
      TC_throwExceptionNamed(context,
                             "litebase.SQLParseException",
                             getMessage(ERR_INVALID_NUMBER),
    ...
    rightTree->operandValue.asDouble = rightValue.asDouble =
      rightValueAsDouble = TC_str2double(buffer, &error);
    if (error) {
      TC_throwExceptionNamed(context,
                             "litebase.SQLParseException",
                             getMessage(ERR_INVALID_NUMBER),
    ...
    booleanClauseTree->leftTree->valueType =
      booleanClauseTree->rightTree->valueType = DOUBLE_TYPE;
  ...
  switch (booleanClauseTree->operandType) { // Then perform the comparison.
    ...
    case OP_REL_DIFF:
      switch (compareType) {
        ...
        case DATETIME_TYPE:
          result = (leftValueAsInt == rightValueAsInt) &&
                   (leftValueAsTime == rightValueAsTime);
      }
    ...
    case OP_REL_LESS_EQUAL:
      switch (compareType) {
        ...
        case DATETIME_TYPE:
          result = (leftValueAsInt == rightValueAsInt)
                     ? (leftValueAsTime > rightValueAsTime)
                     : (leftValueAsInt > rightValueAsInt);
      }
 ...
 */
int32
matchStringOperands(Context context,
                    SQLBooleanClauseTree* booleanClauseTree,
                    bool ignoreCase,
                    Heap heap)
{
  ...
  JCharP leftStringStr, strToMatchStr = rightTree->strToMatch;
  int32 leftStringLen, strToMatchLen = rightTree->lenToMatch,
                       matchType = rightTree->patternMatchType, pos, strEndLen;
  bool result = false;
  JChar dateTimeBuf16[27];

  if (!leftValue.asChars) {
    leftTree->valueJoin.asChars = leftValue.asChars =
      (JCharP)TC_heapAlloc(heap,
                           (booleanClauseTree->booleanClause->resultSet->table
                              ->columnSizes[leftTree->colIndex]
                            << 1) +
                             2);
  }
  if (!getOperandValue(context, leftTree, &leftValue)) {
  ...
  if (leftValue.isNull) { // null value
    return false; // juliana@230_34: corrected possible wrong results when
  ...
  // juliana@230_3: corrected a bug of LIKE using DATE and DATETIME not
  // returning the correct result.
  if (leftTree->valueType == DATE_TYPE) {
    int32 asDate = leftValue.asInt;
    date2JCharP(asDate / 10000,
                asDate / 100 % 100,
                asDate % 100,
                leftStringStr = dateTimeBuf16);
    leftStringLen = 10;
  } else if (leftTree->valueType == DATETIME_TYPE) {
    int32 asDate = leftValue.asDate, asTime = leftValue.asTime;
    dateTime2JCharP(asDate / 10000,
                    asDate / 100 % 100,
                    asDate % 100,
                    asTime / 10000000,
                    asTime / 100000 % 100,
  ...
  switch (matchType) {
    ...
    case PAT_MATCH_STARTS_WITH:
      result = str16StartsWith(leftStringStr,
                               strToMatchStr,
                               leftStringLen,
                               strToMatchLen,
                               0,
    ...
    case PAT_MATCH_ENDS_WITH:
      result = str16StartsWith(leftStringStr,
                               ...
                               leftStringLen,
                               strToMatchLen,
                               leftStringLen - strToMatchLen,
                               ignoreCase);
      break;

    case PAT_MATCH_CONTAINS:
      result = str16IndexOf(leftStringStr,
                            strToMatchStr,
                            leftStringLen,
                            strToMatchLen,
                            ignoreCase) >= 0;
    ...
    case PAT_MATCH_MIDDLE: // rnovais@568_1
      ...
      result = str16StartsWith(leftStringStr,
                               ...
                               0,
                               ignoreCase)
                 ? str16StartsWith(leftStringStr,
                                   &strToMatchStr[pos + 1],
                                   leftStringLen,
                                   strEndLen,
                                   leftStringLen - strEndLen,
                                   ignoreCase)
                 : 0;
      break;
    ...
    case PAT_MATCH_EQUAL: // rnovais@568_1
      if (ignoreCase) {
        result = TC_JCharPEqualsIgnoreCaseJCharP(
          leftStringStr, strToMatchStr, leftStringLen, strToMatchLen);
      } else {
        result = TC_JCharPEqualsJCharP(
          leftStringStr, strToMatchStr, leftStringLen, strToMatchLen);
      }
  }
 ...
 */
int32
compareStringOperands(Context context,
                      SQLBooleanClauseTree* booleanClauseTree,
                      bool ignoreCase,
                      Heap heap)
{
  ...
  SQLBooleanClauseTree* rightTree = booleanClauseTree->rightTree;
  int32* columnSizes =
    booleanClauseTree->booleanClause->resultSet->table->columnSizes;

  SQLValue leftValue, rightValue;

  if (!rightTree->valueJoin.asChars) {
    rightTree->valueJoin.asChars =
      (JCharP)TC_heapAlloc(heap, (columnSizes[rightTree->colIndex] << 1) + 2);
  }
  ...
  if (!booleanClauseTree->bothAreIdentifier) {
    if (!leftTree->valueJoin.asChars &&
        leftTree->operandType == OP_IDENTIFIER) {
      leftTree->valueJoin.asChars =
        (JCharP)TC_heapAlloc(heap, (columnSizes[leftTree->colIndex] << 1) + 2);
    }
  ...
  result = str16CompareTo(leftValue.asChars,
                          rightValue.asChars,
                          leftValue.length,
                          rightValue.length,
                          ignoreCase);
 ...
 */
int32
booleanTreeEvaluate(Context context,
                    SQLBooleanClauseTree* booleanClauseTree,
                    Heap heap)
{
  ...
  switch (
    booleanClauseTree->operandType) { // Checks the operand type of the tree.
    ...
    case OP_REL_GREATER_EQUAL:
    case OP_REL_LESS_EQUAL: {
      switch (booleanClauseTree->valueType) { // Calls the right operation
        ...
        case BLOB_TYPE:
          TC_throwExceptionNamed(
            context, "litebase.SQLParseException", getMessage(ERR_COMP_BLOBS));
          return false;
    ...
    case OP_PAT_MATCH_LIKE:
    case OP_PAT_MATCH_NOT_LIKE:
      return matchStringOperands(context,
                                 booleanClauseTree,
                                 booleanClauseTree->valueType ==
                                   CHARS_NOCASE_TYPE,
                                 heap);
    ...
    case OP_BOOLEAN_AND: { // AND connector.
      ...
      if (booleanClauseTree->leftTree && booleanClauseTree->rightTree &&
          (result = booleanTreeEvaluate(
             context, booleanClauseTree->leftTree, heap)) == true) {
    ...
    case OP_BOOLEAN_OR: { // OR connector.
      ...
      if (booleanClauseTree->leftTree &&
          booleanClauseTree->rightTree) { // Expects both trees to be not null.
        // Short circuit: only evaluates the right tree if the left result is
        // FALSE.
        if ((result = booleanTreeEvaluate(
               context, booleanClauseTree->leftTree, heap)) == true) {
          return true;
        }
        if (!result) {
          return booleanTreeEvaluate(
            context, booleanClauseTree->rightTree, heap);
        }
        return -1;
      }
      return false;
 ...
 * @param booleanClauseTree A pointer to a <code>SQLBooleanClauseTree</code>
 * structure.
 * @return <code>false</code> If an exception is thrown; <code>true</code>,
 * otherwise.
 * @throws DriverException If there are imcompatible types or a column cannot be
 * bound.
 */
bool
bindColumnsSQLBooleanClauseTree(Context context,
                                SQLBooleanClauseTree* booleanClauseTree)
{
  ...
  if (booleanClauseTree->operandType ==
      OP_IDENTIFIER) { // If operand type is identifier, binds to a column in
                       // the table.
    int32 dtParameter, // rnovais@568_10
      fieldIndex =
        getFieldIndex(booleanClauseTree), // Stores the binding information also
                                          // in the boolean clause field list.
      colIndex;
    SQLResultSetField* field;
    SQLBooleanClause* booleanClause =
      booleanClauseTree->booleanClause; // Stores the binding information also
                                        // in the boolean clause field list.
    ...
    if (fieldIndex < 0) { // The column could not be found.
      TC_throwExceptionNamed(context,
                             ...
                             booleanClauseTree->operandName);
      return false;
    }

    booleanClauseTree->colIndex = colIndex =
      (field = booleanClause->fieldList[fieldIndex])->tableColIndex;

    if (field->sqlFunction ==
        FUNCTION_DT_NONE) { // rnovais@568_10: if it does not have a data
                            // function, stores the correct value type.
  ...
  if ((leftTree = booleanClauseTree->leftTree) &&
      !bindColumnsSQLBooleanClauseTree(context, leftTree)) {
    return false;
  }

  if ((rightTree = booleanClauseTree->rightTree) &&
      !bindColumnsSQLBooleanClauseTree(context, rightTree)) {
    return false;
  }

  // Infers the operation resulting value type.
  if (leftTree && rightTree &&
      !inferOperationValueType(context, booleanClauseTree)) {
    return false;
  }

  // rnovais@567_2: validates date and datetime in the rightTree.
  if (leftTree && rightTree &&
      (leftTree->valueType == DATE_TYPE ||
       leftTree->valueType == DATETIME_TYPE)) {
    return validateDateTime(
      context, &rightTree->operandValue, leftTree->valueType);
  }
  return true;
}

/**
 * Infers the operation value type, according to the left and right values
 ...
 * @return <code>false</code> If an exception is thrown; <code>true</code>,
 * otherwise.
 * @throws SQLParseException If left and right values have incompatible types or
 * there is a blob type in the comparison.
 */
bool
inferOperationValueType(Context context,
                        SQLBooleanClauseTree* booleanClauseTree)
{
  TRACE("inferOperationValueType")
  SQLBooleanClauseTree* leftTree = booleanClauseTree->leftTree;
  SQLBooleanClauseTree* rightTree = booleanClauseTree->rightTree;
  int32 leftOperandType, rightOperandType, leftValueType, rightValueType;
  bool leftIsParameter, rightIsParameter;
  if (!leftTree || !rightTree) {
  ...
  if (leftTree->operandName) { // rnovais@568_10: if it has a data type
                               // function, verifies if it can be applied.
    ...
    if (field->sqlFunction != FUNCTION_DT_NONE &&
        !bindFunctionDataType(field->parameter->dataType,
                              field->sqlFunction)) { // Incompatible function.
      TC_throwExceptionNamed(context,
                             "litebase.SQLParseException",
                             getMessage(ERR_DATA_TYPE_FUNCTION),
                             dataTypeFunctionsName(field->sqlFunction));
      return false;
    }
  }

  if (booleanClauseTree->operandType == OP_BOOLEAN_AND ||
      booleanClauseTree->operandType == OP_BOOLEAN_OR) { // Boolean type.
  ...
  if (leftValueType == BLOB_TYPE ||
      rightValueType == BLOB_TYPE) { // Blobs can't be compared.
    TC_throwExceptionNamed(
      context, "litebase.SQLParseException", getMessage(ERR_COMP_BLOBS));
    return false;
  }

  // In case one of them is a parameter, the tree has the type of the one that
  // is not a parameter. If both are parameters, the type is undefined (which is
  // the default, anyway).
  if (leftIsParameter || rightIsParameter) {
    ...
    bool leftIsChar =
           (leftValueType == CHARS_TYPE || leftValueType == CHARS_NOCASE_TYPE ||
            leftValueType == DATE_TYPE || leftValueType == DATETIME_TYPE),

         // juliana@201_12: both should be compared to DATE and DATETIME.
      rightIsChar =
        (rightValueType == CHARS_TYPE || rightValueType == CHARS_NOCASE_TYPE ||
         leftValueType == DATE_TYPE || leftValueType == DATETIME_TYPE);

    if (leftIsChar !=
        rightIsChar) { // Can not mix a character type with a non-character
                       // type, except for DATE and DATETIME.
      TC_throwExceptionNamed(context,
                             "litebase.SQLParseException",
                             getMessage(ERR_INCOMPATIBLE_TYPES));
      return false;
    ...
    if ((leftValueType == DATE_TYPE || leftValueType == DATETIME_TYPE) &&
        rightValueType == CHARS_TYPE) { // rnovais@567_2
      rightValueType = leftValueType;
    }

    // If one of the operands is an identifier and the other one is not, it
    // prevails the identifier operand value type.
    if (rightOperandType == OP_IDENTIFIER && leftOperandType != OP_IDENTIFIER) {
      if (!convertValue(context,
                        booleanClauseTree->leftTree,
                        booleanClauseTree->valueType = rightValueType)) {
        return false;
      }
    } else if (leftOperandType == OP_IDENTIFIER &&
               rightOperandType != OP_IDENTIFIER) {
      if (!convertValue(context,
                        booleanClauseTree->rightTree,
                        booleanClauseTree->valueType = leftValueType)) {
        return false;
      }
    } else
      switch (booleanClauseTree->operandType) {
        ...
        case OP_REL_LESS_EQUAL:
          if (leftIsChar || rightIsChar) {
            if (leftValueType == DATE_TYPE ||
                rightValueType == DATE_TYPE) { // rnovais@567_2
              booleanClauseTree->valueType = DATE_TYPE;
            } else if (leftValueType == DATETIME_TYPE ||
                       rightValueType == DATETIME_TYPE) {
              booleanClauseTree->valueType = DATETIME_TYPE;
            } else {
              // If both are identifiers, it prevails CHARS, in case one of them
              // is CHARS (case sensitive).
              if (leftOperandType == OP_IDENTIFIER &&
                  rightOperandType == OP_IDENTIFIER) {
                if (leftValueType == CHARS_TYPE ||
                    rightValueType == CHARS_TYPE) {
                  booleanClauseTree->valueType = CHARS_TYPE;
                } else {
            ...
            } else if (leftValueType == FLOAT_TYPE ||
                       rightValueType == FLOAT_TYPE) {
              booleanClauseTree->valueType = FLOAT_TYPE;
            } else if (leftValueType == LONG_TYPE ||
                       rightValueType == LONG_TYPE) {
              booleanClauseTree->valueType = LONG_TYPE;
            } else if (leftValueType == INT_TYPE ||
                       rightValueType == INT_TYPE) {
        ...
        case OP_PAT_MATCH_NOT_LIKE:
          if (!leftIsChar ||
              !rightIsChar) { // Only character types are allowed here.
            TC_throwExceptionNamed(context,
                                   "litebase.SQLParseException",
                                   getMessage(ERR_INCOMPATIBLE_TYPES));
            return false;
  ...
  booleanClauseTree->isFloatingPointType =
    booleanClauseTree->valueType == DOUBLE_TYPE ||
    booleanClauseTree->valueType == FLOAT_TYPE;
  return true;
}
             ...
             int32 type)
{
  ...
  if (type < SHORT_TYPE ||
      type > DOUBLE_TYPE) { // juliana@201_10: non-numerical types don't need to
  ...
  if ((operandValue->length = TC_JCharPLen(operandValue->asChars)) > 39) {
    CharP invalid =
      TC_JCharP2CharP(operandValue->asChars, operandValue->length);
    TC_throwExceptionNamed(context,
                           "litebase.SQLParseException",
                           getMessage(ERR_INVALID_NUMBER),
                           invalid ? invalid : "",
...
// juliana@214_4: removes not from expression trees so that indices can be used
// in more situations.
/**
 ...
 */
SQLBooleanClauseTree*
removeNots(SQLBooleanClauseTree* booleanClauseTree, Heap heap)
{
  TRACE("removeNots")
  SQLBooleanClauseTree* tree;
  SQLBooleanClauseTree* rightTree;
...
/**
 * Gets the field index given the hash code of the SQL function used or the
 * field if there is no function applied.
 *
 * @param booleanClauseTree A pointer to a <code>SQLBooleanClauseTree</code>
 * structure.
 ...
 */
int32
getFieldIndex(SQLBooleanClauseTree* booleanClauseTree)
{
  TRACE("getFieldIndex")
  return (TC_htGet32Inv(&(booleanClauseTree)->booleanClause->fieldName2Index,
                        (booleanClauseTree)->nameSqlFunctionHashCode
                          ? (booleanClauseTree)->nameSqlFunctionHashCode
                          : (booleanClauseTree)->nameHashCode));
}

/**
 * Validates a string value as a date or datetime according with its type. If it
 * is well-formed, its value is transformed into one or two ints.
 *
 * @param context The thread context where the function is being executed.
 ...
 * @return <code>true</code> if the string storing a date or a datetime is
 * well-formed; <code>false</code>, otherwise.
 * @throws SQLParseException If the string is not well-formed.
 */
bool
validateDateTime(Context context, SQLValue* value, int32 valueType)
{
...
// juliana@226_15: corrected a bug that would make a prepared statement with
// where clause and indices not work correctly after the first execution.
/**
 ...
 * @param destTree The old destination tree. If the nodes are the same of the
 * tree cloned, the node is reused in the cloned tree.
          ...
          Heap heap)
{
  ...
  if (!tree) {
    if (destTree != booleanClauseTree) {
      (tree = initSQLBooleanClauseTree(booleanClause, heap))->booleanClause =
        booleanClause;
      ...
      // juliana@227_8:  Solved a bug on select prepared statement with like not
      // returning the correct result.
  ...
  if (booleanClauseTree->leftTree) {
    (tree->leftTree = cloneTree(
       booleanClauseTree->leftTree, destTree ? destTree->leftTree : null, heap))
      ->parent = tree;
  }
  if (booleanClauseTree->rightTree) {
    (tree->rightTree = cloneTree(booleanClauseTree->rightTree,
                                 destTree ? destTree->rightTree : null,
                                 heap))
      ->parent = tree;
  }
  return tree;
