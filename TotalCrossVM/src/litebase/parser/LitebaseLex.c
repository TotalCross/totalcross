...
int32 yylex(LitebaseParser* parser) {
	...
	while (true) {
		...
		if (is[parser->yycurrent] &
			IS_ALPHA) { // Finds identifiers or reserved words. The first character must be a letter.
			initialPos = parser->yyposition - 1;
			while (parser->yycurrent <= 255
				   && is[parser->yycurrent] & IS_ALPHA_DIGIT) { // The other characters must be a letter, digit or '_'.
				CALCULATE_HASH(parser->yycurrent);
				INSERT_CHAR(parser->yycurrent);
			}
			parser->select.sqlHashCode = hash;
			return findReserved(parser,
								initialPos); // Sees if the identifier is a reserved word or just an identifier.
		}

		if (is[parser->yycurrent] &
			IS_START_DIGIT) { // Finds digits. The start of a digit can be a digit or a sign.
			if (parser->yycurrent ==
				'+') { // juliana@226a_20: now passing +number (ex. +10) will work on sql clauses.
				GET_YYCURRENT(parser->yycurrent);
			}
			...
			GET_YYCURRENT(parser->yycurrent);
			while (parser->yycurrent <= 255
				   && is[parser->yycurrent] & IS_DIGIT) { // The second part of the nunber must be digits.
				INSERT_CHAR(parser->yycurrent);
			}
			...
			if (parser->yycurrent == '.') { // Test if the number is not an integer.
				INSERT_CHAR(parser->yycurrent);
				while (parser->yycurrent <= 255
					   && is[parser->yycurrent] & IS_DIGIT) { // Gets the rest of the digits of the non-integer number.
					INSERT_CHAR(parser->yycurrent);
				}
			}

			if (parser->yycurrent <= 255
				&& is[parser->yycurrent] & IS_END_NUM) { // The number may finish with a letter indicating its type.
				INSERT_CHAR(parser->yycurrent);
			}
		...
		if (is[parser->yycurrent] & IS_RELATIONAL) { // Finds tokens with two characters, or '>' or '<'.
			...
			if ((yybefore == '!' && parser->yycurrent == '=') || (yybefore == '<'
		...
		if (parser->yycurrent == '\'') {
			...
			INSERT_CHAR(parser->yycurrent);
			str16 = parser->yylval = TC_heapAlloc(parser->heap,
												  (parser->yyposition - initialPos - 1 - counter) << 1);
			counter = 0;
...
int32 findReserved(LitebaseParser* parser, int32 initialPos) {
	...
	TC_JCharP2CharPBuf(&parser->zzReaderChars[initialPos],
					   (8 < (parser->yyposition - initialPos - 1)) ? 8 : parser->yyposition - initialPos - 1, buf);
	TC_CharPToLower(buf);
