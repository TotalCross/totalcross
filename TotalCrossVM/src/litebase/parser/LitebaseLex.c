 ...
 *
 * @param parser The parser structure, which will hold the tree resulting from
 * the parsing process.
 * @return The token code.
 */
int32
yylex(LitebaseParser* parser)
{
  TRACE("yyLex")
  int32 initialPos, yybefore, hash = parser->select.sqlHashCode;
  ...
  while (true) {
    ...
    // juliana@221_4: unicode strings were not dealt properly on Windows 32,
    // Windows CE, Palm, iPhone, and Android. Ends the lexycal analysis if the
    ...
    if (is[parser->yycurrent] &
        IS_ALPHA) { // Finds identifiers or reserved words. The first character
                    // must be a letter.
      initialPos = parser->yyposition - 1;
      while (parser->yycurrent <= 255 &&
             is[parser->yycurrent] &
               IS_ALPHA_DIGIT) { // The other characters must be a letter,
                                 // digit or '_'.
        CALCULATE_HASH(parser->yycurrent);
        INSERT_CHAR(parser->yycurrent);
      }
      parser->select.sqlHashCode = hash;
      return findReserved(parser,
                          initialPos); // Sees if the identifier is a reserved
                                       // word or just an identifier.
    }

    if (is[parser->yycurrent] &
        IS_START_DIGIT) { // Finds digits. The start of a digit can be a digit
                          // or a sign.
      if (parser->yycurrent == '+') { // juliana@226a_20: now passing +number
                                      // (ex. +10) will work on sql clauses.
        GET_YYCURRENT(parser->yycurrent);
      }
      ...
      while (parser->yycurrent <= 255 &&
             is[parser->yycurrent] &
               IS_DIGIT) { // The second part of the nunber must be digits.
        INSERT_CHAR(parser->yycurrent);
      }
      ...
      if (parser->yycurrent == '.') { // Test if the number is not an integer.
        ...
        while (parser->yycurrent <= 255 &&
               is[parser->yycurrent] &
                 IS_DIGIT) { // Gets the rest of the digits of the non-integer
                             // number.
          INSERT_CHAR(parser->yycurrent);
        }
      }

      if (parser->yycurrent <= 255 &&
          is[parser->yycurrent] & IS_END_NUM) { // The number may finish with a
                                                // letter indicating its type.
        INSERT_CHAR(parser->yycurrent);
      }

      // Copies the number to yacc.
      parser->yylval = TC_heapAlloc(
        parser->heap, (yybefore = (parser->yyposition - initialPos)) << 1);
      xmemmove(parser->yylval,
               &zzReaderChars[initialPos],
               yybefore > 2 ? (yybefore - 1) << 1 : 2);
      parser->select.sqlHashCode = hash;
      return TK_NUMBER;
    }

    if (is[parser->yycurrent] &
        IS_RELATIONAL) { // Finds tokens with two characters, or '>' or '<'.
      yybefore = parser->yycurrent;
      CALCULATE_HASH(parser->yycurrent);
      GET_YYCURRENT(parser->yycurrent);

      if ((yybefore == '!' && parser->yycurrent == '=') ||
          (yybefore == '<' &&
           parser->yycurrent == '>')) { // Sees if it is the different operator.
        CALCULATE_HASH(parser->yycurrent);
        GET_YYCURRENT(parser->yycurrent);
        parser->select.sqlHashCode = hash;
        return TK_DIFF;
      ...
      if (yybefore == '>' &&
          parser->yycurrent ==
            '=') { // Sees if it is the greater or equal operator.
        CALCULATE_HASH(parser->yycurrent);
        GET_YYCURRENT(parser->yycurrent);
        parser->select.sqlHashCode = hash;
      ...
      if (yybefore == '<' &&
          parser->yycurrent == '=') { // Sees if it is the less operator.
        CALCULATE_HASH(parser->yycurrent);
        GET_YYCURRENT(parser->yycurrent);
        parser->select.sqlHashCode = hash;
    ...
    // Sees if the tokens are arithmetic operators, '(', or ')'. In this case,
    ...
    if (parser->yycurrent == '\'') {
      ...
      // juliana@225_6: a quote was not being correctly inserted in a string
      // when not using prepared statements.
      while (parser->yycurrent != '\'') {
        if (parser->yycurrent ==
            '\\') { // Sees if there is an escape in the string.
          GET_YYCURRENT(parser->yycurrent);
          INSERT_CHAR(parser->yycurrent);
          counter++;
        }
        ...
        // Windows CE, Palm, iPhone, and Android.
        else if (parser->yycurrent == 65535) { // The string must be closed
          ...
          INSERT_CHAR(
            parser->yycurrent); // Anything else can be inside the string.
        }
      }
      INSERT_CHAR(parser->yycurrent);
      str16 = parser->yylval = TC_heapAlloc(
        parser->heap, (parser->yyposition - initialPos - 1 - counter) << 1);
      counter = 0;
...
/**
 * The initializer of the lexical analyser. It initializes the reserved words
 * hash table and the kinds of token table based on ascii code.
 *
 * @return <code>false</code> if the reserved words hash table allocation fails;
 ...
 * @param parser The parser structure, which will hold the tree resulting from
 * the parsing process.
 * @param initialPos The initial position of the current token in the SQL
 * string.
 * @return The code of a reserved word token or an identifirer token.
 */
int32
findReserved(LitebaseParser* parser, int32 initialPos)
{
  TRACE("findReserved")
  int32 found = -1;
  ...
  TC_JCharP2CharPBuf(&parser->zzReaderChars[initialPos],
                     (8 < (parser->yyposition - initialPos - 1))
                       ? 8
                       : parser->yyposition - initialPos - 1,
                     buf);
  TC_CharPToLower(buf);

  // juliana@213_7: using numbers in the identifiers may cause a colision with
  // the reserved word hash table.
  ...
  parser->yylval =
    TC_heapAlloc(parser->heap, found = parser->yyposition - initialPos);
  TC_JCharP2CharPBuf(
    &parser->zzReaderChars[initialPos], found - 1, parser->yylval);
...
/**
 * Tests if the function <code>initLex()</code> initializes all the lex global
 * variable properly.
 *
