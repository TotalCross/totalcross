...
bool sqlcolumnlistclauseEquals(SQLColumnListClause* clause1, SQLColumnListClause* clause2) {
	...
	if (length !=
		clause2->fieldsCount) { // If the length of the column lists are different, the lists are different.
		return false;
	}
		...
		while (--i >= 0) {
			...
			while (--j >= 0) {
				...
				if (xstrchr(field->alias, '.')
					&& xstrncmp(field->alias, rsTable->aliasTableName, xstrlen(rsTable->aliasTableName))) {
					index = -1;
				}
				if (index >= 0) {
