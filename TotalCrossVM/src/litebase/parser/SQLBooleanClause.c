 ...
 */
SQLBooleanClause*
initSQLBooleanClause(Heap heap)
{
  TRACE("sqlBooleanClause")
  SQLBooleanClause* sqlBooleanClause =
    (SQLBooleanClause*)TC_heapAlloc(heap, sizeof(SQLBooleanClause));
 ...
 * transform the SQL boolean tree, eliminating the branches that can be resolved
 * through the table indexes.
 *
 * @param booleanClause A pointer to a <code>SQLBooleanClause</code> structure.
 * @param tableIndices The table indices; each position in the array relates to
 * a column in the table; a <code>null</code> value indicates no index on that
 * column.
 ...
 */
bool
applyTableIndexes(SQLBooleanClause* booleanClause,
                  Index** tableIndexes,
                  int32 columnsCount,
                  bool hasComposedIndex)
{
  ...
  int32 i = columnsCount, j, curOperandType, leftOperandType, count,
        fieldsCount = booleanClause->fieldsCount, rightOperandType,
        numberComposedIndexes = table ? table->numberComposedIndexes : 0,
        countAppliedIndices = 0;
  bool appliedComposedIndex, isLeft = false;
  ...
  ComposedIndex** appliedComposedIndexes =
    booleanClause->appliedComposedIndexes;
  ComposedIndex* currCompIndex;

  if (!booleanClause->isWhereClause) { // Indices can only be applied to the
                                       // where clause.
    return false;
  }
  ...
  // Traverses the tree, from the parent to the rightmost tree until the boolean
  // operator changes. To simplify the algorithm and considering that complex
  // boolean expressions (the ones enclosed by parenthesis) are always connected
  // to the left branch), only the branches connected to the right side of the
  // tree are candidates to be replaced by table indexing.
  ...
  while (curTree) {
    ...
    switch (curOperandType =
      ...
      case OP_BOOLEAN_OR: {
        ...
        if (booleanClause->appliedIndexesBooleanOp &&
        ...
        if ((leftOperandType >= OP_REL_EQUAL &&
             leftOperandType <= OP_REL_LESS_EQUAL) ||
            ((leftOperandType == OP_PAT_MATCH_LIKE ||
              leftOperandType == OP_PAT_MATCH_NOT_LIKE) &&
          ...
          if (hasComposedIndex && curOperandType == OP_BOOLEAN_AND &&
            ...
            while (true) {
              ...
              if (count >= fieldsCount || operators[count] != OP_REL_EQUAL ||
                  operators[count] == 255) {
                count = 0;
              ...
              if ((rightOperandType = rightTree->operandType) ==
                  OP_BOOLEAN_AND) {
                curTree = rightTree;
                rightTree = curTree->rightTree;
              } else if ((leftOperandType >= OP_REL_EQUAL &&
                          leftOperandType <= OP_REL_LESS_EQUAL) ||
                         ((leftOperandType == OP_PAT_MATCH_LIKE ||
                           leftOperandType == OP_PAT_MATCH_NOT_LIKE) &&
                          leftTree->patternMatchType ==
                            PAT_MATCH_STARTS_WITH)) {
                getBranchProperties(rightTree,
                                    columns,
                                    operators,
                                    indexesValueTree,
                                    count,
            ...
            if (count >= 2) { // It has an AND operator on at least 2 fields?
              ...
              while (--i >= 0) {
                ...
                if (appliedComposedIndex) {
                  appliedComposedIndexes[booleanClause->appliedIndexesCount] =
                    currCompIndex;
                  curTree = applyComposedIndexToBranch(booleanClause,
                                                       originalTree,
                                                       columns,
                                                       operators,
                                                       indexesValueTree,
    ...
    if (booleanClause->appliedIndexesCount ==
 ...
 */
void
applyIndexToBranch(SQLBooleanClause* booleanClause,
                   ...
                   bool isLeft)
{
  ...
  if (leftIsColumn != rightIsColumn) {
    ...
    SQLBooleanClauseTree** appliedIndexesValueTree =
      booleanClause->appliedIndexesValueTree;
    SQLResultSetField** fieldList = booleanClause->fieldList;

    while (--i >=
           0) // An index cannot be applied to a function in the where clause.
      if (fieldList[i]->tableColIndex == column &&
    ...
    if (indexesMap[column]) { // Checks if the column is indexed.
      ...
      if (parent) {
        ...
        // Links the branch sibling to its grandparent, removing the branch from
        // the tree, as result.
        if (grandParent) {
          if (isLeft) {
        ...
        booleanClause->expressionTree =
          null; // The branch has no parent. So, no expression tree will be
                // left.
      }
    }
  }
...
applyComposedIndexToBranch(SQLBooleanClause* booleanClause,
                           SQLBooleanClauseTree* branch,
                           uint8* columns,
                           uint8* operators,
                           ...
                           ComposedIndex* compIndex)
{
  ...
  int32 i = -1, length = compIndex->numberColumns, n;
  uint8* appliedIndexesCols = booleanClause->appliedIndexesCols;
  uint8* appliedIndexesRelOps = booleanClause->appliedIndexesRelOps;
  SQLBooleanClauseTree** appliedIndexesValueTree =
    booleanClause->appliedIndexesValueTree;
  ...
  if (parent) {
    ...
    branch->parent = null;
    if (branch->operandType != OP_BOOLEAN_AND &&
        branch->operandType != OP_BOOLEAN_OR) { // Is the end of the root?
      branch = null;
    }
 ...
 */
bool
applyTableIndexesJoin(SQLBooleanClause* booleanClause)
{
  TRACE("applyTableIndexesJoin")
  SQLBooleanClauseTree* curTree;
  SQLBooleanClauseTree* leftTree;
  SQLBooleanClauseTree* rightTree;
  int32 curOperandType, leftOperandType, countAppliedIndices = 0;
  bool isLeft = false;

  if (!booleanClause
         ->isWhereClause) { // Indexes can only be applied to a where clause.
    return false;
  }
  ...
  while (curTree) {
    ...
    switch (curOperandType =
      ...
      case OP_BOOLEAN_OR: {
        ...
        // If so, leaves the loop, since for now, for simplicty, the algorithm
        // does not combine different boolean operators.
        if (booleanClause->appliedIndexesBooleanOp != OP_NONE &&
            booleanClause->appliedIndexesBooleanOp != curOperandType) {
          curTree = null;
          break;
        }
        ...
        // Checks if the left tree has a simple boolean operand. If so, tries to
        // apply an index on it.
        leftOperandType = leftTree->operandType;
    ...
    // If the number of indexes to be applied reached the limit, leaves the
    // loop.
 ...
 */
void
applyIndexToBranchJoin(SQLBooleanClause* booleanClause,
                       SQLBooleanClauseTree* branch,
                       bool isLeft)
{
  ...
  if (branch->bothAreIdentifier && fieldList[fieldIndex = getFieldIndex(right)]
                                     ->table->columnIndexes[right->colIndex]) {
    right->hasIndex = true;
  }
  ...
  if (leftIsColumn != rightIsColumn) {
    ...
    // juliana@285_1: solved a possible wrong result if the query had join and a
    // filter with function in a column with an index. Checks if the column is
    // indexed.
    if ((table = (field = fieldList[fieldIndex = getFieldIndex(tree)])->table)
 ...
 * @param booleanClause A pointer to a <code>SQLBooleanClause</code> structure.
 * @return <code>true</code> if all parameter are defined; <code>false</code>,
 * otherwise.
 * @throws DriverException if a parameter is not defined.
 */
bool
sqlBooleanClausePreVerify(Context context, SQLBooleanClause* booleanClause)
{
...
/**
 * Evaluates the boolean clause, accordingly to values of the current record of
 * the given <code>ResultSet</code>.
 *
 ...
 * @return 1, if the current record of the result set satisfies the boolean
 * clause; 0  if the current record of the result set does not satisfy the
 * boolean clause; -1, otherwise.
 */
int32
sqlBooleanClauseSatisfied(Context context,
 ...
 * @param heap A heap to allocate some new <code>SQLBooleanClauseTree</code>
 * nodes.
 * @return <code>true</code>, if the boolean clause was bound successfully;
 * <code>false</code>, otherwise.
 */
bool
bindColumnsSQLBooleanClause(Context context,
                            SQLBooleanClause* booleanClause,
                            Hashtable* names2Index,
                            int8* columnTypes,
                            ...
                            Heap heap)
{
  ...
  if (tableList != null) { // The having clause has already been verified.
    if (!verifyColumnNamesOnTableList(context,
                                      booleanClause->fieldList,
                                      booleanClause->fieldsCount,
                                      tableList,
                                      tableListSize)) {
      return false;
    }
    ...
    while (--i >= 0) {
      field = fieldList[i];
      field->tableColIndex = TC_htGet32Inv(
        names2Index,
        field->aliasHashCode); // Sets the column indexes of the temp table.
  ...
  booleanClause->expressionTree =
    removeNots(booleanClause->expressionTree, heap); // juliana@214_4
  return bindColumnsSQLBooleanClauseTree(
    context,
    booleanClause->expressionTree); // Binds the field information in the tree
                                    // to the table columns.
}

/**
 * Verifies if the column names are correct and belongs to the table list and is
 * used only to verify if where clause and having clause field list is the field
 * list of the where/having clause.
 *
 * @param context The thread context where the function is being executed.
 ...
 * @return <code>true</code>, if field name verification found no problems;
 * <code>false</code>, otherwise.
 * @throws SQLParseException If there is an unknown or an ambiguos column name.
 */
bool
verifyColumnNamesOnTableList(Context context,
                             ...
                             int32 tableListSize)
{
  ...
  int32 index, hashAliasTableName, j;
  bool foundFirst;
  Table* currentTable;
  SQLResultSetField* field;
    ...
    if ((field = fieldList[fieldsCount])->tableName) {
      ...
      if (!currentTable ||
          (index = TC_htGet32Inv(&currentTable->htName2index,
                                 TC_hashCode(field->tableColName))) < 0) {
        TC_throwExceptionNamed(context,
      ...
      while (++j < tableListSize) {
        index =
          TC_htGet32Inv(&(currentTable = tableList[j]->table)->htName2index,
          ...
          if (foundFirst) {
            ...
            if (field->sqlFunction == FUNCTION_DT_NONE) {
              ...
              field->parameter->dataType =
                (int8)currentTable->columnTypes[index];
            }
            field->indexRs = j;
          }
        }
 ...
 */
bool
bindColumnsSQLBooleanClauseSimple(Context context,
                                  ...
                                  Heap heap)
{
  ...
  if (!verifyColumnNamesOnTable(
        context, clause->fieldList, clause->fieldsCount, rsTable)) {
    return false;
  }

  clause->expressionTree =
    removeNots(clause->expressionTree, heap); // juliana@214_4
  return bindColumnsSQLBooleanClauseTree(
    context,
    clause->expressionTree); // Binds the field information in the tree to the
                             // table columns.
}

/**
 * Verifies if the column names are correct and belongs to the table and is used
 * only to verify if where clause field list is the field list of the where
 * clause.
 *
 * @param context The thread context where the function is being executed.
 * @param fieldList The field list of the where/having clause.
 ...
 */
bool
verifyColumnNamesOnTable(Context context,
                         SQLResultSetField** fieldList,
                         int32 fieldsCount,
                         SQLResultSetTable* rsTable)
{
  ...
  while (--fieldsCount >= 0) {
    if ((field = fieldList[fieldsCount])->tableName) {
      ...
      if (rsTable->aliasTableNameHashCode != TC_hashCode(field->tableName) ||
          (index = TC_htGet32Inv(&(currentTable = rsTable->table)->htName2index,
                                 TC_hashCode(field->tableColName))) < 0) {
        TC_throwExceptionNamed(context,
                               "litebase.SQLParseException",
                               getMessage(ERR_UNKNOWN_COLUMN),
      ...
      if ((index = TC_htGet32Inv(&(currentTable = rsTable->table)->htName2index,
