...
/**
 * Defines functions to deal with a a value which can be inserted in a column of
 * a table.
 */
...
// null values must be handled before. If the value is null, this function can't
// be called.
/**
               ...
               PlainDB* plainDB)
{
  ...
  if (isNull1 ||
      isNull2) { // A null value is always considered to be the greatest value.
    ...
    case CHARS_TYPE:
      if (!value2->length && plainDB) {
        ...
        if (!nfReadBytes(context, &plainDB->dbo, (uint8*)&length, 2) ||
            !loadString(
              context, plainDB, value2->asChars, value2->length = length)) {
          return false;
        }
        value2->asChars[length] = 0;
      }
      return str16CompareTo(value1->asChars,
                            value2->asChars,
                            value1->length,
                            value2->length,
...
{
  SQLValue value;
  int32 hour = 0, minute = 0, second = 0, time, day = 1, month = 1, year = 1000,
        date;
  bool changed = false;
  int64 asLong;
  double asDouble;
  ...
  while (hour < 24) { // Tests time functions.
    ...
    // hour
    value.asTime = time =
      1000 * (int32)getTimeLong(0, 0, 0, hour, minute, second);
...
{
  SQLValue value1, value2;
  bool changed = false;
  int32 asInt, length;
  int64 asLong;
  double asDouble;
  char bufferChar[27];
  JChar bufferJChar1[27], bufferJChar2[27];

  // Tests when one of the values is null.
  ASSERT2_EQUALS(
    I32, 0, valueCompareTo(currentContext, null, null, -1, true, true, null));
  ASSERT2_EQUALS(
    I32, 1, valueCompareTo(currentContext, null, null, -1, true, false, null));
  ASSERT2_EQUALS(
    I32, -1, valueCompareTo(currentContext, null, null, -1, false, true, null));
  ...
  while (++asInt < 32767) {
    ...
    ASSERT2_EQUALS(
      ...
      0,
      valueCompareTo(
        currentContext, &value1, &value1, SHORT_TYPE, false, false, null));
    value2.asShort = asInt + 1;
    ASSERT2_EQUALS(
      I32,
      -1,
      valueCompareTo(
        currentContext, &value1, &value2, SHORT_TYPE, false, false, null));
    value2.asShort = asInt - 1;
    ASSERT2_EQUALS(
      I32,
      1,
  ...
  while ((asInt += 131072) < 2147483647) {
    ...
    ASSERT2_EQUALS(
      ...
      0,
      valueCompareTo(
        currentContext, &value1, &value1, INT_TYPE, false, false, null));
    value2.asInt = asInt + 1;
    ASSERT2_EQUALS(
      I32,
      -1,
      valueCompareTo(
        currentContext, &value1, &value2, INT_TYPE, false, false, null));
    value2.asInt = asInt - 1;
    ASSERT2_EQUALS(
      I32,
      1,
  ...
  while ((asLong += 274877906944L) < 9223372036854775807) {
    ...
    ASSERT2_EQUALS(
      ...
      0,
      valueCompareTo(
        currentContext, &value1, &value1, LONG_TYPE, false, false, null));
    value2.asLong = asLong + 1;
    ASSERT2_EQUALS(
      I32,
      -1,
      valueCompareTo(
        currentContext, &value1, &value2, LONG_TYPE, false, false, null));
    value2.asLong = asLong - 1;
    ASSERT2_EQUALS(
      I32,
      1,
  ...
  while ((asDouble *= 10.0) < MAX_FLOAT_VALUE) {
    ...
    ASSERT2_EQUALS(
      ...
      0,
      valueCompareTo(
        currentContext, &value1, &value1, FLOAT_TYPE, false, false, null));
    value2.asFloat = (float)(asDouble * 10.0);
    ASSERT2_EQUALS(
      I32,
      -1,
      valueCompareTo(
        currentContext, &value1, &value2, FLOAT_TYPE, false, false, null));
    value2.asFloat = (float)(asDouble / 10.0);
    ASSERT2_EQUALS(
      I32,
      1,
  ...
  while ((asDouble *= 10.0) < 1.7976931348623157E308) {
    ...
    ASSERT2_EQUALS(
      ...
      0,
      valueCompareTo(
        currentContext, &value1, &value1, DOUBLE_TYPE, false, false, null));
    value2.asDouble = asDouble * 10.0;
    ASSERT2_EQUALS(
      I32,
      -1,
      valueCompareTo(
        currentContext, &value1, &value2, DOUBLE_TYPE, false, false, null));
    value2.asDouble = asDouble / 10.0;
    ASSERT2_EQUALS(
      I32,
      1,
  ...
  while (++asInt <= 'Z') {
    xmemset(bufferChar,
            asInt,
            length = value1.length = value2.length = asInt - 'A' + 1);
    ...
    ASSERT2_EQUALS(
      ...
      0,
      valueCompareTo(
        currentContext, &value1, &value2, CHARS_TYPE, false, false, null));
    ASSERT2_EQUALS(I32,
                   0,
                   valueCompareTo(currentContext,
                                  &value1,
                                  &value2,
                                  CHARS_NOCASE_TYPE,
                                  false,
    ...
    ASSERT2_EQUALS(
      ...
      +32,
      valueCompareTo(
        currentContext, &value2, &value1, CHARS_TYPE, false, false, null));
    ASSERT2_EQUALS(I32,
                   0,
                   valueCompareTo(currentContext,
                                  &value1,
 ...
 * It takes lap years into consideration.
 *
 * @param year The year.
