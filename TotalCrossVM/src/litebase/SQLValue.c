	...
	switch (type) {
		...
		case CHARS_TYPE:
			if (!value2->length && plainDB) {
				...
				if (!nfReadBytes(context, &plainDB->dbo, (uint8*)&length, 2)
					|| !loadString(context, plainDB, value2->asChars, value2->length = length)) {
					return false;
				}
				value2->asChars[length] = 0;
			}
			return str16CompareTo(value1->asChars, value2->asChars, value1->length, value2->length,
...
TESTCASE(valueCompareTo) {
	...
	while (++asInt < 32767) {
		...
		ASSERT2_EQUALS(I32, 0, valueCompareTo(currentContext, &value1, &value1, SHORT_TYPE, false, false,
											  null));
		value2.asShort = asInt + 1;
		ASSERT2_EQUALS(I32, -1, valueCompareTo(currentContext, &value1, &value2, SHORT_TYPE, false, false,
											   null));
		value2.asShort = asInt - 1;
		ASSERT2_EQUALS(I32, 1, valueCompareTo(currentContext, &value1, &value2, SHORT_TYPE, false, false,
											  null));
	...
	while ((asInt += 131072) < 2147483647) {
		...
		value1.asInt = asInt;
		ASSERT2_EQUALS(I32, 0, valueCompareTo(currentContext, &value1, &value1, INT_TYPE, false, false,
											  null));
		value2.asInt = asInt + 1;
		ASSERT2_EQUALS(I32, -1, valueCompareTo(currentContext, &value1, &value2, INT_TYPE, false, false,
											   null));
		value2.asInt = asInt - 1;
		ASSERT2_EQUALS(I32, 1, valueCompareTo(currentContext, &value1, &value2, INT_TYPE, false, false,
											  null));
	...
	while ((asLong += 274877906944L) < 9223372036854775807) {
		...
		ASSERT2_EQUALS(I32, 0, valueCompareTo(currentContext, &value1, &value1, LONG_TYPE, false, false,
											  null));
		value2.asLong = asLong + 1;
		ASSERT2_EQUALS(I32, -1, valueCompareTo(currentContext, &value1, &value2, LONG_TYPE, false, false,
											   null));
		value2.asLong = asLong - 1;
		ASSERT2_EQUALS(I32, 1, valueCompareTo(currentContext, &value1, &value2, LONG_TYPE, false, false,
											  null));
	...
	while ((asDouble *= 10.0) < MAX_FLOAT_VALUE) {
		...
		ASSERT2_EQUALS(I32, 0, valueCompareTo(currentContext, &value1, &value1, FLOAT_TYPE, false, false,
											  null));
		value2.asFloat = (float)(asDouble * 10.0);
		ASSERT2_EQUALS(I32, -1, valueCompareTo(currentContext, &value1, &value2, FLOAT_TYPE, false, false,
											   null));
		value2.asFloat = (float)(asDouble / 10.0);
		ASSERT2_EQUALS(I32, 1, valueCompareTo(currentContext, &value1, &value2, FLOAT_TYPE, false, false,
											  null));
	...
	while ((asDouble *= 10.0) < 1.7976931348623157E308) {
		...
		ASSERT2_EQUALS(I32, 0, valueCompareTo(currentContext, &value1, &value1, DOUBLE_TYPE, false, false,
											  null));
		value2.asDouble = asDouble * 10.0;
		ASSERT2_EQUALS(I32, -1, valueCompareTo(currentContext, &value1, &value2, DOUBLE_TYPE, false, false,
											   null));
		value2.asDouble = asDouble / 10.0;
		ASSERT2_EQUALS(I32, 1, valueCompareTo(currentContext, &value1, &value2, DOUBLE_TYPE, false, false,
											  null));
	...
	while (++asInt <= 'Z') {
		...
		ASSERT2_EQUALS(I32, 0, valueCompareTo(currentContext, &value1, &value2, CHARS_TYPE, false, false,
											  null));
		ASSERT2_EQUALS(I32, 0, valueCompareTo(currentContext, &value1, &value2, CHARS_NOCASE_TYPE, false,
											  false, null));
		applyDataTypeFunction(&value2, FUNCTION_DT_LOWER, -1);
		ASSERT2_EQUALS(I32, -32, valueCompareTo(currentContext, &value1, &value2, CHARS_TYPE, false, false,
												null));
		ASSERT2_EQUALS(I32, +32, valueCompareTo(currentContext, &value2, &value1, CHARS_TYPE, false, false,
												null));
		ASSERT2_EQUALS(I32, 0, valueCompareTo(currentContext, &value1, &value2, CHARS_NOCASE_TYPE, false,
											  false, null));
		applyDataTypeFunction(&value2, FUNCTION_DT_UPPER, -1);
		value2.length--;
		ASSERT2_EQUALS(I32, 1, valueCompareTo(currentContext, &value1, &value2, CHARS_TYPE, false, false,
											  null));
		ASSERT2_EQUALS(I32, -1, valueCompareTo(currentContext, &value2, &value1, CHARS_TYPE, false, false,
											   null));
		ASSERT2_EQUALS(I32, 1, valueCompareTo(currentContext, &value1, &value2, CHARS_NOCASE_TYPE, false,
											  false, null));
		ASSERT2_EQUALS(I32, -1, valueCompareTo(currentContext, &value2, &value1, CHARS_NOCASE_TYPE, false,
											   false, null));
	}
...
void getNextDate(int32* year, int32* month, int32* day) {
	(*day)++;
	if (monthDays[*month - 1] < *day && !(*day == 29 && *month == 2 && (*year % 400 == 0
										  || (*year % 100 != 0 && *year % 4 == 0)))) {
		*day = 1;
		if ((*month)++ > 12) {
			*month = 1;
