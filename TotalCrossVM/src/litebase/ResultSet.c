 ...
 * @return The result set created.
 */
ResultSet* createSimpleResultSet(Context context, Table* table, SQLBooleanClause* whereClause,
								 Heap heap) {
	...
	if (whereClause
		&& !generateIndexedRowsMap(context, &resultSet, 1, table->numberComposedIndexes > 0, heap)) {
 ...
 * @return The result set created.
 */
ResultSet* createResultSetForSelect(Context context, Table* table, SQLBooleanClause* whereClause,
									Heap heap) {
	...
	resultSet->pos = resultSet->answerCount = -1; // juliana@230_14
	resultSet->decimalPlaces = (int8*)TC_heapAlloc(resultSet->heap,
							   resultSet->columnCount = table->columnCount);
...
bool resultSetPrev(Context context, ResultSet* resultSet) {
	...
	if (table->deletedRowsCount >
		...
		while (resultSet->pos-- >
			   0) { // juliana@201_27: solved a bug in next() and prev() that would happen after doing a delete from table_name.
			if (!plainRead(context, plainDB, resultSet->pos)) {
				return false;
			}
			xmove4(&value, basbuf);
...
TCObject rsGetChars(Context context, ResultSet* resultSet, int32 column, SQLValue* value) {
	...
	if (length >
		table->columnSizes[column]) { // juliana@270_22: solved a possible crash when the table is corrupted.
...
TCObject rsGetString(Context context, ResultSet* resultSet, int32 column, SQLValue* value) {
	...
	switch (table->columnTypes[column]) {
		case SHORT_TYPE:
			xmove2(&value->asShort,
				   ptr); // juliana@227_18: corrected a possible insertion of a negative short column being recovered in the select as positive.
			break;
		case INT_TYPE:
			xmove4(&value->asInt, ptr);
			if (!column
				&& *table->name) { // juliana@213_4: rowid was not being returned correctly if the table was not temporary.
 ...
 */
void getStrings(NMParams params,
				int32 count) { // juliana@201_2: corrected a bug that would let garbage in the number of records parameter.
	TRACE("getStrings")
	...
	if (testRSClosed(context, *params->obj)) { // The driver and the result set can't be closed.
		if ((position = resultSet->pos) >= 0
			&& position <= (table = resultSet->table)->db.rowCount - 1) { // Invalid result set position.
			// juliana@230_14: removed temporary tables when there is no join, group by, order by, and aggregation.
			TCObject* strings;
			...
			int32 cols = resultSet->selectClause->fieldsCount,
				  ...
				  i,
				  column,
				  records = table->db.rowCount -
							resultSet->pos; // juliana@210_1: select * from table_name does not create a temporary table anymore.

			// juliana@210_1: select * from table_name does not create a temporary table anymore.

			if (count < -1) { // juliana@211_4: solved bugs with result set dealing.
				TC_throwExceptionNamed(context, "java.lang.IllegalArgumentException", getMessage(ERR_RS_INV_POS),
									   count);
				return;
			}
			...
			if (!(params->retO = result = TC_createArrayObject(context, "[[java.lang.String", count))
			...
			do {
				if (!(*matrixEntry = TC_createArrayObject(context, "[java.lang.String",
				...
				while (++i < cols) {
					field = fields[i];
					column = notTemporary ? (field->parameter ? field->parameter->tableColIndex : field->tableColIndex)
							 : i;
						...
						*strings = rsGetString(context, resultSet, column, &value);
						...
						if (!(*strings) || field->isDataTypeFunction) {
							if (field->isDataTypeFunction) {
								...
								createString(params, &value, columnTypes[column],
											 resultSet->decimalPlaces ? resultSet->decimalPlaces[column] : -1);
								*strings++ = params->retO;
							}
						} else {
							TC_setObjectLock(*strings++, UNLOCKED);
	...
	if (!colName) {
		...
		if ((p->i32[0] = TC_htGet32Inv(&getResultSetBag(resultSet)->intHashtable,
			...
			TC_throwExceptionNamed(p->currentContext, "java.lang.IllegalArgumentException",
								   getMessage(ERR_INVALID_COLUMN_NAME), colName);
		}
	}
}
...
void rsPrivateGetByIndex(NMParams p, int32 type) {
	...
	if (type != UNDEFINED_TYPE)
		if (!(field->isDataTypeFunction && type == SHORT_TYPE && (typeCol == DATE_TYPE
				|| typeCol == DATETIME_TYPE))
		...
		switch (typeCol) {
			...
			case CHARS_NOCASE_TYPE:
				if (type == CHARS_TYPE) {
					...
					TC_setObjectLock(p->retO = rsGetString(p->currentContext, rsBag, col, &value),
									 UNLOCKED);    // STRING
				}
				break;
			case DATE_TYPE:
...
bool verifyRSState(Context context, ResultSet* resultSet, int32 column) {
	...
	if (column <= 0
		|| column > resultSet->selectClause->fieldsCount) { // Cols given by the user range from 1 to n.
 ...
 */
void rsApplyDataTypeFunction(NMParams params, SQLValue* value, SQLResultSetField* field,
							 int32 type) {
	...
	switch (field->sqlFunction) {
		...
		case FUNCTION_DT_MILLIS: {
			if (type == UNDEFINED_TYPE) {
				IntBuf buffer;
				TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
												TC_int2str(value->asShort, buffer), -1),
								 UNLOCKED);
			} else {
			...
			switch (field->parameter->dataType) {
				...
				case INT_TYPE: {
					if (type == UNDEFINED_TYPE) {
						IntBuf buffer;
						TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
														TC_int2str(value->asInt, buffer), -1),
										 UNLOCKED);
					} else {
				...
				case LONG_TYPE: {
					if (type == UNDEFINED_TYPE) {
						LongBuf buffer;
						TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
														TC_long2str(value->asLong, buffer), -1),
										 UNLOCKED);
					} else {
				...
				case FLOAT_TYPE: {
					if (type == UNDEFINED_TYPE) {
						DoubleBuf buffer;
						TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
														TC_double2str(value->asFloat, value->length,
																buffer), -1), UNLOCKED);
					} else {
				...
				case DOUBLE_TYPE: {
					if (type == UNDEFINED_TYPE) {
						DoubleBuf buffer;
						TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
														TC_double2str(value->asDouble, value->length,
																buffer), -1), UNLOCKED);
					} else {
...
void createString(NMParams params, SQLValue* value, int32 type, int32 decimalPlaces) {
	...
	switch (type) {
		case SHORT_TYPE: {
			IntBuf buffer;
			TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
											TC_int2str(value->asShort, buffer), -1), UNLOCKED);
			break;
		}
		case INT_TYPE: {
			IntBuf buffer;
			TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
											TC_int2str(value->asInt, buffer), -1), UNLOCKED);
			break;
		}
		case LONG_TYPE: {
			LongBuf buffer;
			TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
											TC_long2str(value->asLong, buffer), -1), UNLOCKED);
			break;
		}
		case FLOAT_TYPE: {
			DoubleBuf buffer;
			TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
											TC_double2str((double)value->asFloat, decimalPlaces,
													buffer), -1), UNLOCKED);
			break;
		}
		case DOUBLE_TYPE: {
			DoubleBuf buffer;
			TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext,
											TC_double2str(value->asDouble, decimalPlaces,
													buffer), -1), UNLOCKED);
			break;
		}
		case DATE_TYPE: {
			...
			TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext, dateBuf, 10),
							 UNLOCKED);
			break;
		}
		case DATETIME_TYPE: { // rnovais@_567_2
			...
			dateTimeBuf[10] = ' ';
			TC_setObjectLock(params->retO = TC_createStringObjectFromCharP(params->currentContext, dateTimeBuf,
											23), UNLOCKED);
		}
	}
}
...
bool testRSClosed(Context context, TCObject resultSet) {
	...
	if (OBJ_ResultSetDontFinalize(resultSet)) { // Prepared Statement Closed.
		TC_throwExceptionNamed(context, "java.lang.IllegalStateException",
							   getMessage(ERR_RESULTSET_CLOSED));
		return false;
	}
	if (OBJ_LitebaseDontFinalize(getResultSetBag(
									 resultSet)->driver)) { // The connection with Litebase can't be closed.
	...
	if ((table = getTable(context, resultSet->driver, tableName))) {
		...
		switch (type) {
			...
			case DATE_TYPE: {
				...
				xstrprintf(valueCharP = buffer, "%04d/%02d/%02d", dateInt / 10000, dateInt / 100 % 100,
						   dateInt % 100);
				break;
			}
			case DATETIME_TYPE: {
				...
				xstrprintf(valueCharP = buffer, "%04d/%02d/%02d", dateInt / 10000, dateInt / 100 % 100,
						   dateInt % 100);
				xstrprintf(&buffer[11], "%02d:%02d:%02d:%03d", timeInt / 10000000, timeInt / 100000 % 100,
						   timeInt / 1000 % 100, timeInt % 1000);
				buffer[10] = ' ';
				break;
			}
