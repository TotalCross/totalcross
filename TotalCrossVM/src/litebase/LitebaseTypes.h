...
typedef bool (*readBytesFunc)(Context context,
                              XFile* xFile,
                              uint8* buffer,
                              int32 count);
typedef bool (*writeBytesFunc)(Context context,
                               XFile* xFile,
                               uint8* buffer,
                               int32 count);
...
struct XFile
{
  ...
  /**
   * A cache for the file so that the bytes do not needed to be loaded all the
   * time.
   */
  uint8* cache;
  ...
  /**
   * Indicates if the cache is dirty and its contents needs to be saved later
   * on.
   */
  uint8 cacheIsDirty;
...
/**
 * This structure represents the key of a record. It may be any of the SQL types
 * defined here.
 */
struct Key
{
...
{
  /**
   * Represents the <code>CHARS</code>, <code>VARCHAR</code>, <code>CHARS
   * NOCASE</code>, and <code>VARCHAR NOCASE</code> data types.
   */
  JCharP asChars;
...
struct SQLSelectClause
{
   ...
   * This will be better used when we implement a PLANNER for litebase. Up to
   * now this only indicates if the select clause has a count(*).
   */
  uint8 type;
  ...
  /**
   * The heap used by the parser is stored here to allocate the structures for
...
/**
 * This structure represents a SQL column list clause, like order by or group
 * by. <BR> <i>Note:</i> The select clause has a different structure
 * <code>SQLSelectClause</code>, since it has a different complexity.
 */
struct SQLColumnListClause
{
   ...
   */
  uint8 isComposed; // juliana@230_29: order by and group by now use indices on
                    // simple queries.

  /**
   * Indicates the index to use when doing a sort operation.
   */
  int16 index; // juliana@230_29: order by and group by now use indices on
               // simple queries.
...
{
  /**
   * The type of SQL command, which can be one of:
   * <b><code>CMD_CREATE_TABLE</b></code>, <b><code>CMD_CREATE_INDEX</b></code>,
   * <b><code>CMD_DROP_TABLE</b></code>, <b><code>CMD_DROP_INDEX</b></code>,
   * <b><code>CMD_ALTER_DROP_PK</b></code>,
   * <b><code>CMD_ALTER_ADD_PK</b></code>,
   * <b><code>CMD_ALTER_RENAME_TABLE</b></code>,
   * <b><code>CMD_ALTER_RENAME_COLUMN</b></code>,
   * <b><code>CMD_SELECT</b></code>, <b><code>CMD_INSERT</b></code>,
   * <b><code>CMD_UPDATE</b></code>, or <b><code>CMD_DELETE</b></code>.
   */
  uint8 command;
  ...
  /**
   * This is used to differ between a where clause and a having clause. Before
   * parsing the having clause, <code>isWhereClause</code> is set to false. So
   * the <code>getInstanceBooleanClause()</code> method will return a having
   * clause, otherwise it returns a where clause.
   */
  uint8 isWhereClause;
  ...
  /**
   * The second table name found in an update statement, which indicates an
   * error.
   */
  CharP secondFieldUpdateTableName;

  /**
   * The second table alias found in an update statement, which indicates an
   * error.
   */
  ...
  /**
   * The list of trees that contains the paramameter list of the where boolean
   * clause.
   */
  ...
  /**
   * The list of trees that contains the paramameter list of the having boolean
   * clause.
   */
  SQLBooleanClauseTree* havingParamList[MAXIMUMS];

  /**
   * A hashtable to be used on select statements to verify if it has repeated
   * table names.
   */
  Hashtable tables;
...
struct SQLBooleanClause
{
  ...
  /**
   * The original boolean clause expression tree. This is necessary when freeing
   * the tree.
   */
...
struct SQLFieldDefinition
{
  ...
  /**
   * The default value of a field can contain a string, a number or a
   * Date/Datetime value. This must be converted to the correct type later. If
   * <code>defaultValue</code> is <code>null</code>, the default value was not
   * defined.
   */
  JCharP defaultValue;

  /**
   * Only used for chars / chars no case / blob types. For other types it is
   * equal to zero.
   */
  int32 fieldSize;

  /**
   * The type of the field. It can be: <B><code>NUMBER</code></B>,
   * <B><code>UNDEFINED</code></B>, <B><code>CHARS</code></B>,
   * <B><code>SHORT</code></B>, <B><code>INT</code></B>,
   * <B><code>LONG</code></B>, <B><code>FLOAT</code></B>,
   * <B><code>DOUBLE</code></B>, <B><code>CHARS_NOCASE</code></B>,
   * <B><code>BOOLEAN</code></B>, <B><code>DATE</code></B>, or
   * <B><code>DATE_TIME</code></B>.
   */
  uint8 fieldType;
...
struct SQLResultSetField
{
  ...
  /**
   * Indicates if the field represents a virtual column (not mapped directly to
  ...
  /**
   * Indicates if the result is to be shown in ascending or decreasing order for
   * fields from order by clause.
   */
  uint8 isAscending;
  ...
  /**
   * The index of the column that this field represents in the underlying table.
   * For virtual fields, this value equals -1.
   */
  uint8 tableColIndex; // juliana@227_1: solved a problem with selecting all the
                       // columns of a 128-column table.

  /**
   * Indicates that the index to be used is composed or not.
   */
  uint8 isComposed; // juliana@230_21: MAX() and MIN() now use indices on simple
                    // queries.
   ...
   */
  int8 index; // juliana@230_21: MAX() and MIN() now use indices on simple
              // queries.
  ...
  /**
   * The name of the column that this field represents in the underlying table.
   * For virtual fields, this value equals <code>null</code>.
   */
  CharP tableColName;

  /**
   * The parameter of the function.<br>
   * <i>Note:</i> It is declared as <code>ResultSetField</code> to allow nested
...
struct SQLUpdateStatement
{
  ...
  /**
   * The number of the parameters if the update statement is a preprared
   * statement.
   */
  uint8 paramCount;
...
{
  /**
   * Indicates if a table was not correctly closed when was oppened for the last
   * time.
   */
   ...
   */
  uint8 isAscii; // juliana@210_2: now Litebase supports tables with ascii
                 // strings.
...
struct Table
{
   ...
   */
  uint8 wasUpdated; // juliana@270_27: now purge will also really purge the
   ...
   * returned.
   */
  int32 answerCount; // juliana@230_14: removed temporary tables when there is
                     // no join, group by, order by, and aggregation.

  /**
   * The maximum length of the bit map representing all table rows.
   */
  int32 allRowsBitmapLength; // juliana@230_14: removed temporary tables when
                             // there is no join, group by, order by, and
                             // aggregation.
   ...
   */
  uint8* allRowsBitmap; // juliana@230_14: removed temporary tables when there
   ...
   */
  int32* nodes; // juliana@noidr_2: the maximum number of keys of a index was
                // duplicated.
...
/**
 * Represents a set or rows resulting from a
 * <code>LitebaseConnection.executeQuery()</code> method call.
 */
struct ResultSet
{
  /**
   ...
   * not.
   */
  uint8 isSimpleSelect; // juliana@210_1: select * from table_name does not
  ...
  /**
   * Indicates that this <code>ResultSet</code> was generated by a select
   * prepared statement.
   */
  uint8 isPrepared;
  ...
  /**
   * Generates the result set indexed rows map from the associated table indexes
   * applied to the associated WHERE clause.
   */
  MarkBits* markBits;
...
{ // for B-tree
   ...
   * This children nodes.
   */
  uint16* children; // Each array has one extra component, to allow for possible
                    // overflow.
...
{ // renamed from BTree to Index
  ...
  // juliana@noidr_1: removed .idr files from all indices and changed its
  // format.
  ...
  // juliana@230_35: now the first level nodes of a b-tree index will be loaded
  // in memory.
  /**
   ...
   */
  size_t nodes[4]; // juliana@230_32: corrected a bug of searches in big indices
...
/**
 * The hash table that stores the information for each query concerning the
