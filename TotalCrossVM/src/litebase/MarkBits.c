...
onKey(Context context, Key* key, MarkBits* markBits)
{
  ...
  int32 numberColumns = index->numberColumns, leftOp = *markBits->leftOp,
        rightOp = *markBits->rightOp, length = 0, size = *index->colSizes;

  // juliana@230_2: solved a possible crash with LIKE "...%"
  if (!keys0->length && size) { // A strinhg may not be loaded.
    nfSetPos(dbo,
             keys0->asInt); // Gets and sets the string position in the .dbo.

    // Fetches the string length and the string itself.
    if (!nfReadBytes(context, dbo, (uint8*)&length, 2) ||
        !loadString(context, plainDB, keys0->asChars, keys0->length = length)) {
      return -1;
    }
  }
  ...
  if (markBits->rightKey.index) {
    int32 comp =
      keyCompareTo(null, key, &markBits->rightKey, numberColumns, null);
    if (rightOp == OP_REL_LESS_EQUAL &&
        comp > 0) { // If key <= right key, stops.
      return false;
    }
  ...
  if (leftOp == OP_REL_EQUAL || leftOp == OP_REL_GREATER_EQUAL ||
      (leftOp == OP_REL_GREATER && markBits->isNoLongerEqual)) {
    ...
    JCharP patStr, valStr;
    JChar dateTimeBuf16[24];
    int32 valLen, type = *index->types;
    ...
    if (type == DATE_TYPE) {
      ...
      date2JCharP(asDate / 10000,
                  asDate / 100 % 100,
                  asDate % 100,
                  valStr = dateTimeBuf16);
      valLen = 10;
    } else if (type == DATETIME_TYPE) {
      int32 asDate = keys0->asDate, asTime = keys0->asTime;
    ...
    if (str16StartsWith(valStr,
                        patStr,
                        valLen,
                        keys0->length,
                        0,
                        caseless)) {  // Only starts with are used with indices.
...
onValue(int32 record, MarkBits* markBits)
{
  ...
  if (record != NO_VALUE) {
    if (markBits->bitValue) {
      markBits->indexBitmap->items[record >> 5] |=
        ((int32)1 << (record & 31)); // set
    } else {
      markBits->indexBitmap->items[record >> 5] &=
        ~((int32)1 << (record & 31)); // reset
    }
  }
}
