 ...
 * index, it returns the negative number of the this old one; otherwise, it
 * returns the new number.
 * @throws AlreadyCreatedException If an index already exists.
 */
int32
verifyIfIndexAlreadyExists(Context context,
                           Table* table,
                           uint8* columnNumbers,
                           int32 indexCount)
{
  ...
  if (indexCount == 1) { // Simple index.
    ...
    uint8* columns;
    int32 size = i = table->numberComposedIndexes, j;
    ...
    while (--i >= 0) {
      ...
      // juliana@253_2: corrected a bug if a composed index with less columns
      // were created after one with more columns.
      if (j == indexCount) {
 ...
 */
bool
driverDropIndex(Context context, Table* table, int32 column)
{
  ...
  if (!index) {
    TC_throwExceptionNamed(context,
                           ...
                           getMessage(ERR_COLUMN_DOESNOT_HAVE_AN_INDEX),
  ...
  table->columnAttrs[column] &=
    ATTR_COLUMN_HAS_NO_INDEX; // Deletes the INDEX bit from the attributes.

  // Saves the meta.
  return tableSaveMetaData(context,
                           table,
                           TSMD_ATLEAST_INDEXES); // guich@560_24
}
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 * @throws DriverException If the table does not have the desired composed index
 * to be dropped.
 */
bool
driverDropComposedIndex(Context context,
                        Table* table,
                        uint8* columns,
                        int32 size,
                        int32 indexId,
                        bool saveMD)
{
  ...
  uint8* idxColumns;
  int32 indexCount = size, i = table->numberComposedIndexes, j = 0;
  ...
  if (i >= 0 && compIndex) { // Removes the index.
    ...
    // juliana@201_16: When a composed index is deleted, its information is now
    // deleted from the metadata.
    if (table->numberComposedIndexes) {
      (table->composedIndexes[i] =
         table->composedIndexes[--table->numberComposedIndexes])
        ->indexId = i + 1;
    } else {
 ...
 */
int32
deleteAllIndexes(Context context, Table* table)
{
  TRACE("deleteAllIndexes")
  int32 count = 0, i = table->columnCount, primaryKey = table->primaryKeyCol;
  bool ret = true;
  ...
  // primary key can't be deleted.
  i = table->numberComposedIndexes;
  primaryKey = table->composedPK;
  while (--i >= 0)
    if (i != primaryKey) {
      ret &= driverDropComposedIndex(context,
                                     table,
                                     composedIndexes[i]->columns,
                                     composedIndexes[i]->numberColumns,
                                     i,
                                     false);
      count++;
    }

  // juliana@230_33: corrected a bug of composed indices files returning after
 ...
 */
bool
computeColumnOffsets(Context context, Table* table)
{ // rnovais@568_10: changed from static to global.
  TRACE("computeColumnOffsets")
  uint16* offsets = table->columnOffsets;
  int8* types = table->columnTypes;
  bool notRecomputing = !offsets;
  int32 sum = 0, n = table->columnCount, i = -1;
  uint8* buffer;

  if (notRecomputing) { // Does not create the array 2 times.
    offsets = table->columnOffsets =
      (uint16*)TC_heapAlloc(table->heap, (n + 1) << 1);
  }
  ...
  // the number of bytes necessary to store the columns. Each column in a table
  // correspond to one bit. Added a number of bytes corresponding to the null
  // values and to the crc code.
  sum += NUMBEROFBYTES(n) + 4; // juliana@220_4

  plainSetRowSize(&table->db,
                  sum,
                  buffer =
                    TC_heapAlloc(table->heap, sum)); // Sets the new row size.
 ...
 */
bool
tableLoadMetaData(Context context, Table* table, bool throwException)
{ // juliana@220_5
  TRACE("tableLoadMetaData")
  TCHARP sourcePath = table->sourcePath;
  CharP tableName = table->name;
  int32 flags, columnCount = 0, i = -1, numOfBytes, version = 0, nameLength,
               indexNameLength, stringLength;
  bool exist;
  ...
  IntBuf intBuf;
  uint8* metadata =
    plainReadMetaData(context, plainDB, buffer); // Reads the meta data.
  uint8* ptr = metadata;
  uint8* columnAttrs;
  int8* columnTypes;
  int32* columnSizes;
  ...
  Heap heap = table->heap, idxHeap = null;
  NATIVE_FILE idxFile;
  ...
  if (!(((ptr[0] & USE_CRYPTO) == 0) ^ plainDB->db.useCrypto) &&
      ptr[1] == ptr[2] == ptr[3] == 0 &&
      (ptr[0] == 0 || ptr[0] == 1 || ptr[0] == 3)) {
  ...
  plainDB->dbo.finalPos =
    plainDB->dbo.size; // Gets the last position of the blobs and strings file.
  xmove2(&plainDB->headerSize, ptr + 4); // Reads the header size.
  ptr += 6;

  // juliana@226_8: a table without metadata (with an empty .db, for instance)
  // can't be recovered: it is corrupted.
  if (!plainDB->headerSize) { // The header size can't be zero.
    // juliana@222_1: the table should not be marked as closed properly if it
    // was not previously closed correctly. juliana@253_13: corrected a possible
    // crash on Palm when trying to recover a table with corrupted header.
    nfClose(context, dbFile);
    TC_throwExceptionNamed(context,
                           "litebase.DriverException",
  ...
  if ((((flags = *ptr++) & IS_ASCII) != 0) != plainDB->isAscii) {
    ...
    TC_throwExceptionNamed(
      context, "litebase.DriverException", getMessage(ERR_WRONG_STRING_FORMAT));
    goto error;
  }

  // juliana@220_2: added TableNotCreatedException which will be raised whenever
  // a table is not closed properly. If the table was not correctly closed,
  // throws an specific exception to the user.
  if (!(flags &= IS_SAVED_CORRECTLY)) {
    if (throwException) {
      ...
      nfClose(context, dbFile);
      TC_throwExceptionNamed(context,
                             "litebase.TableNotClosedException",
                             getMessage(ERR_TABLE_NOT_CLOSED),
                             &table->name[5]);
      goto error;
    } else {
  ...
  xmove4(&table->auxRowId,
         ptr + 6); // rnovais@570_61: reads the auxiliary rowid.
  table->primaryKeyCol =
    *(ptr + 10); // juliana@114_9: the simple primary key column.
  ...
  columnHashes = table->columnHashes =
    (int32*)TC_heapAlloc(heap, columnCount << 2);
  ...
  defaultValues = table->defaultValues =
    (SQLValue**)TC_heapAlloc(heap, columnCount * TSIZE);
  ...
  ptr = readStringArray(
    ptr += (columnCount << 2), &table->columnNames, columnCount, heap);
  ...
  // juliana@201_21: The null columns information must be created before
  // openning the indices when reading the table meta data.
  table->columnNulls =
    (uint8*)TC_heapAlloc(heap, numOfBytes = NUMBEROFBYTES(columnCount));

  // juliana@224_5: corrected a bug that would throw an exception when
  // re-creating an erased index file.
  ...
  while (--i >= 0) { // Loads the indices.
    if ((columnAttrs[i] & ATTR_COLUMN_HAS_INDEX)) {
      ...
      IF_HEAP_ERROR(idxHeap)
      {
        TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
        heapDestroy(idxHeap);
        table->columnIndexes[i] = null; // juliana@270_22: solved a possible
                                        // crash when the table is corrupted.
        goto error;
      }

      // rnovais@110_2: verifies if the index file exists, otherwise makes the
      // re-index.
      ...
      // juliana@224_5: corrected a bug that would throw an exception when
      // re-creating an erased index file. juliana@202_9: Corrected a bug that
      // would cause indices that have an .idr whose files were erased to be
      // built incorrectly.
      ...
      *columnTypesIdx = columnTypes[i];
      ...
      // juliana@230_8: corrected a possible index corruption if its files are
      // deleted and the application crashes after recreating it.
      if (!indexCreateIndex(context,
                            table,
                            tableName,
                            i,
                            columnSizesIdx,
  ...
  // juliana@213_6: current rowid was not being corrected fetched from the
  // table. Now the current rowid can be fetched.
  if (plainDB->rowCount) {
  ...
  while (++i < columnCount) { // Reads the default values.
    if ((columnAttrs[i] &
         ATTR_COLUMN_HAS_DEFAULT)) { // Tests if it has default values.
      ...
      switch (columnTypes[i]) {
        ...
        case CHARS_NOCASE_TYPE:
          ...
          // juliana@252_5: corrected a bug when using a default value of type
          // string which could become messed up.
          defaultValues[i]->asChars =
            (JCharP)TC_heapAlloc(heap, stringLength << 1);
          xmemmove(
            defaultValues[i]->asChars, (JCharP)(ptr + 2), stringLength << 1);

          // juliana@202_11: Corrected a bug that would create a composed index
          // when opening a table using default values.
        ...
        case SHORT_TYPE:
          ...
          ptr += 2; // juliana@202_11: Corrected a bug that would create a
                    // composed index when opening a table using default values.
          break;

        case DATE_TYPE: // Stored as int.
        case INT_TYPE:
          xmove4(&defaultValues[i]->asInt, ptr);
          ptr += 4; // juliana@202_11: Corrected a bug that would create a
                    // composed index when opening a table using default values.
          break;

        case LONG_TYPE:
          xmove8(&defaultValues[i]->asLong, ptr);
          ptr += 8; // juliana@202_11: Corrected a bug that would create a
                    // composed index when opening a table using default values.
          break;
  ...
  if ((table->numberComposedIndexes = *ptr++) >
      0) { // Reads the composed indices.
    int32 j, indexId, numColumns, size,
    ...
    while (++i < numberComposedIndexes) {
      ...
      IF_HEAP_ERROR(idxHeap)
      {
        TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
        heapDestroy(idxHeap);
        table->composedIndexes[indexId] =
          null; // juliana@270_22: solved a possible crash when the table is
                // corrupted.
        goto error;
      }
      ...
      // juliana@230_8: corrected a possible index corruption if its files are
      // deleted and the application crashes after recreating it. One of the
      // files may not exist.
      if (!indexCreateComposedIndex(context,
                                    table,
                                    table->name,
                                    columns,
                                    columnSizesIdx,
          ...
          (!exist && flags &&
           (!tableReIndex(
              context, table, -1, false, table->composedIndexes[indexId - 1]) ||
            !setModified(context, table)))) {
        heapDestroy(idxHeap);
        table->composedIndexes[indexId] =
          null; // juliana@270_22: solved a possible crash when the table is
  ...
  if ((columnCount = table->numberComposedPKCols = *ptr++) >
      0) { // Number of the composed primary key.
    xmemmove(table->composedPrimaryKeyCols =
               (uint8*)TC_heapAlloc(heap, columnCount),
             ptr,
             columnCount);
  }
 ...
 */
bool
tableSaveMetaData(Context context, Table* table, int32 saveType)
{
  // Stores the changeable information.
  TRACE("tableSaveMetaData")
  uint8 buf[SECTOR_SIZE]; // Avoids allocating memory when saving small parts -
                          // guich@570_97: increased from 30 to 512.
  uint32 n = table->columnCount, i = -1, size;
  uint8* ptr0 = null;
  uint8* ptr;
  uint8* columnAttrs = table->columnAttrs;
  ...
  if (saveType == TSMD_EVERYTHING)
    size += getStringsTotalSize(table->columnNames, table->columnCount) +
            computeDefaultValuesMetadataSize(table) +
  ...
  if (size <= SECTOR_SIZE) {
    ...
    xmemzero(ptr, size);
                 ...
                 (uint8*)xmalloc(size))) { // Must create a temporary buffer
    TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
    return false;
  }
  ...
  *ptr = (plainDB->db.useCrypto
            ? (plainDB->useOldCrypto ? 1 : USE_CRYPTO)
            : 0); // juliana@253_8: now Litebase supports weak cryptography.
  xmove2(ptr + 4, &plainDB->headerSize); // Saves the header size.
  ptr += 6;
  *ptr++ = plainDB->isAscii
             ? IS_ASCII | !table->isModified
             : !table->isModified; // juliana@226_4: table is not saved
                                   // correctly yet if modified.
  ...
  if (saveType !=
      TSMD_ONLY_DELETEDROWSCOUNT) { // More things other than the deleted rows
                                    // count must be saved.
    xmove4(ptr, &table->auxRowId); // rnovais@570_61: saves the auxiliary rowid.
    ptr += 4;

    if (saveType != TSMD_ONLY_AUXROWID) { // More things other than the
                                          // auxiliary row id must be saved.
      ...
      *ptr++ = 0;
      *ptr++ = table->composedPK; // juliana@114_9: saves the composed primary
                                  // key index.
      *ptr++ = 0;

      if (saveType !=
          TSMD_ONLY_PRIMARYKEYCOL) { // More things other than the primary key
                                     // col must be saved.
        ...
        if (saveType == TSMD_EVERYTHING) { // Stores the rest.
          ...
          ptr =
            writeStringArray(ptr += (n << 2),
                             table->columnNames,
                             table->columnCount); // Stores the column names.
              ...
              switch (columnTypes[i]) {
                ...
                case CHARS_TYPE:
                  ptr = writeString16(
                    ptr,
                    defaultValues[i]->asChars,
                    MIN((int32)defaultValues[i]->length, columnSizes[i]));
                  break;
                ...
                case DOUBLE_TYPE:
                  ptr = (uint8*)READ_DOUBLE(
                    ptr, (uint8*)&defaultValues[i]->asDouble);
                  break;
          ...
          n = *ptr++ =
            table->numberComposedIndexes; // Number of composed indices.
          i = -1;
          while (++i < n) { // Stores the composed indices.
            *ptr++ = (compIndex = table->composedIndexes[i])
                       ->indexId; // The composed index id.
            *ptr =
              compIndex
                ->numberColumns; // Number of columns on the composed index.
            ptr += 2;
            ...
            xmemmove(
              ptr,
              compIndex->columns,
              compIndex->numberColumns); // Columns of this composed index.
            ptr += compIndex->numberColumns;
          }
          n = *ptr++ =
            table->numberComposedPKCols; // Number of columns on composed
                                         // primary key. If 0, there's no
                                         // composed primary key.
          xmemmove(ptr,
                   table->composedPrimaryKeyCols,
                   n); // Stores the composed primary key.
          ptr += n;
        }
      }
    }
  ...
  if (!plainWriteMetaData(context, plainDB, ptr0, (int32)(ptr - ptr0))) {
    ...
    flushCache(context,
               &plainDB->db); // juliana@223_11: table meta data is now always
                              // flushed imediately after being changed.
  }
  if (size > SECTOR_SIZE) {
    xfree(ptr0);
  }
 ...
 */
bool
tableSetMetaData(Context context,
                 Table* table,
                 CharP* names,
                 int32* hashes,
                 int8* types,
                 ...
                 int32 composedPKColsSize)
{
  ...
  table->storeNulls =
    TC_heapAlloc(heap, numOfBytes); // Initializes the arrays for the nulls.
  ...
  if (!computeColumnOffsets(
        context, table)) { // guich@570_97: computes the column offests.
    return false;
  }

  if (*table->name) { // juliana@201_14: It is not necessary to save the meta
                      // data in the .db for memory tables.
    if (table->db.db.size) { // The table can't be already created.
      TC_throwExceptionNamed(context,
                             "litebase.AlreadyCreatedException",
                             getMessage(ERR_TABLE_ALREADY_CREATED),
                             table->name);
      return false;
    }
    ...
    table->columnIndexes = (Index**)TC_heapAlloc(
      heap, columnCount * TSIZE); // Initializes the indices.
    return tableSaveMetaData(
      context,
      table,
      TSMD_EVERYTHING); // Saves the metadata after everything was set.
  }
  return true;
}
 ...
 * @param saveType The save type of the table, which increases or decreases its
 * size.
 * @return The metadata size.
 */
int32
getTSMDSize(Table* table, int32 saveType)
{
  TRACE("getTSMDSize")
  int32 columnCount = table->columnCount,
        size =
          13; // dbo.finalPos + headerSize + flags + version + deletedRowsCount.
  ...
  if (saveType == TSMD_EVERYTHING) {
    size += columnCount + (columnCount << 2) +
            2; // columnTypes + columnSizes + columnNames.length.
  }
 ...
 */
int32
getStringsTotalSize(CharP* names, int32 count)
{
  ...
  while (--count >= 0) {
    size += xstrlen(*names++) +
            2; // Includes the two bytes used for the length of each name.
  }
  return size;
}
 ...
 */
int32
computeDefaultValuesMetadataSize(Table* table)
{
  TRACE("computeDefaultValuesMetadataSize")
  uint32 i = table->columnCount, size = 0;
  uint8* columnAttrs = table->columnAttrs;
  int8* columnTypes = table->columnTypes;
  SQLValue** defaultValues = table->defaultValues;
  ...
  while (--i) {
    if ((columnAttrs[i] & ATTR_COLUMN_HAS_DEFAULT))
      switch (columnTypes[i]) {
        ...
        case CHARS_TYPE:
          size +=
            (defaultValues[i]->length << 1) + 2; // The stringh + its length.
          break;
        case SHORT_TYPE:
          size += 2;
          break;
 ...
 */
int32
computeComposedIndicesTotalSize(Table* table)
{
  TRACE("computeComposedIndicesTotalSize")
  int32 size = 2 + table->numberComposedPKCols, // The number o composed PK +
                                                // its number of columns.
    i = table->numberComposedIndexes;
  ComposedIndex** composedIndexes = table->composedIndexes;

  while (--i >= 0) {
    size += 3 + composedIndexes[i]
                  ->numberColumns; // id + numberColumns + hasIdr + the columns.
  }
  return size;
}
 ...
 * @return <code>false</code> if there is an invalid field name;
 * <code>true</code>, otherwise.
 * @throws DriverException if there is an invalid field name.
 */
bool
reorder(Context context,
        Table* table,
        CharP* fields,
        SQLValue** record,
        uint8* storeNulls,
...
{
  TRACE("reorder")
  int32 count = table->columnCount, length = *nValues, i = -1, numParams = 0;

  // juliana@225_5: corrected a possible crash when the table has 128 columns.
  SQLValue* outRecord[MAXIMUMS + 1]; // Just to store the temporary values,
                                     // which will be copied over later.
  ...
  while (++i <
         length) { // Makes sure that the fields are in table creation order.
    ...
    if ((value = outRecord[idx] = record[i]) && (asChars = value->asChars) &&
        asChars[0] == (JChar)'?' && !asChars[1]) {
          ...
          SQLColumnListClause* orderByClause)
{
  TRACE("sortTable")
  PlainDB* plainDB = &table->db;
  int32 count = table->columnCount, totalRecords = plainDB->rowCount;
  ...
  if ((orderByClause && !bindColumnsSQLColumnListClause(context,
                                                        orderByClause,
                                                        &table->htName2index,
                                                        table->columnTypes,
                                                        null,
                                                        0)) ||
      (groupByClause && !bindColumnsSQLColumnListClause(context,
                                                        groupByClause,
                                                        &table->htName2index,
                                                        table->columnTypes,
                                                        null,
  ...
  // guich@223_16: corrected a bug that could crash the application on Windows
  // 32 when the query has an order by or group by due to a compiler error.
  sortListClause = orderByClause ? orderByClause : groupByClause;
  return quickSort(context,
                   table,
                   newSQLValues(count, heap),
                   newSQLValues(count, heap),
                   newSQLValues(count, heap),
          ...
          Heap heap)
{
  ...
  int32 low, count = fieldsCount, high = last - first + 1,
             pivotIndex, // guich@212_3: now using random partition (improves
                         // worst case 2000x).
    rowSize = plainDB->rowSize;
  uint32 size = 2, columnSize;
  StringArray** stringArray = (StringArray**)TC_heapAlloc(heap, high * TSIZE);
  StringArray* tempStringArray;

  while (--high >= 0) {
    stringArray[high] =
      (StringArray*)TC_heapAlloc(heap, sizeof(StringArray) * fieldsCount);
  }

  // juliana@268_2: solved possible crash using order by when a string order by
  // field does not appear in the select field.
  while (--count >= 0) { // Only loads columns used by the sorting process.
    pivotIndex = (field = fieldList[count])->tableColIndex;
    if ((columnSize = field->table->columnSizes[TC_htGet32(
           &field->table->htName2index, field->tableColHashCode)])) {
      pivot[pivotIndex]->asChars =
        (JCharP)TC_heapAlloc(heap, (columnSize << 1) + 2);
      someRecord1[pivotIndex]->asChars =
        (JCharP)TC_heapAlloc(heap, (columnSize << 1) + 2);
      someRecord2[pivotIndex]->asChars =
        (JCharP)TC_heapAlloc(heap, (columnSize << 1) + 2);
    }
  }
  ...
  while (size) { // guich@212_3: removed recursion (storing in a IntVector).
    ...
    if (!readRecord(context,
                    table,
                    pivot,
                    pivotIndex = (last = high) == (first = low)
                                   ? last
 ...
 * otherwise. It will return <code>MAX_INT_VALUE</code> if both records are
 ...
 */
int32
compareSortRecords(int32 recSize,
                   SQLValue** vals1,
                   SQLValue** vals2,
                   int8* types)
{ // juliana@201_3
  ...
  while (++i < recSize) // Does the comparison between the values till one of
                        // them is different from zero.
    if ((result = valueCompareTo(
           null, vals1[i], vals2[i], types[i], false, false, null)) != 0) {
      return result;
    }

  // The values are equal. Compares with the record index.
  if ((result = types[0] == DATETIME_TYPE) || result == LONG_TYPE ||
      result == DOUBLE_TYPE) {
...
// juliana@250_1: corrected a possible crash when doing ordering operations.
/**
 * Quick sort used for sorting the table to build the indices from scratch. This
 * one is simpler than the sort used for order / gropu by. Uses a stack instead
 * of a recursion.
 *
 ...
 */
void
sortRecords(SQLValue*** sortValues,
            int32 recSize,
            int8* types,
            int32 first,
            int32 last,
...
{ // juliana@201_3
  ...
  i = first;
  while (++i <= last)
    if (compareSortRecords(recSize, sortValues[i - 1], sortValues[i], types) >
        0) {
      break;
    }
  if (i > last) {
  ...
  while (size) { // guich@212_3: removed recursion (storing in a stack).
    ...
    // juliana@213_3: last can't be equal to first.
    mid = sortValues[(last = high) == (first = low) ? last
                                                    : randBetween(first, last)];

    while (true) { // Finds the partitions.
      while (high >= low &&
             compareSortRecords(recSize, mid, sortValues[low], types) > 0) {
        low++;
      }
      while (high >= low &&
             compareSortRecords(recSize, mid, sortValues[high], types) < 0) {
        high--;
      }
 ...
 * allowed (SHORT, INT, LONG). This is faster than quicksort. Also used to build
 * the indices from scratch.
 *
 * @param source The values to be sorted. Only simple records for simple indices
 * can be used.
 ...
 */
void
radixSort(SQLValue*** source, int32 length, int32 type, SQLValue*** temp)
{ // juliana@201_3
  TRACE("radixSort")
  int32 count[256];
  int32 index[256];
  ...
  if (temp !=
      source) { // If the final sorted array is not at the source, copies to it.
...
{ // juliana@201_3
  TRACE("radixPass")
  int32 i = 0, n = length, ibits = 0, b, ishift = start << 3;
  int64 lbits = 0, lshift = (int64)start << 3, lb;
  ...
  switch (type) {
    case INT_TYPE:
    case DATE_TYPE: // juliana@214_9: index creation for date types could create
                    // corrupted indices.
      if (start == 0)
        while (--n >= 0) {
  ...
  switch (type) {
    ...
    case LONG_TYPE:
      if (start == 0)
        ...
        while (--n >= 0) {
          dest[index[(int32)(((*source[i])->asLong >> lshift) + 128)]++] =
            source[i];
          i++;
        }
      else
        while (--n >= 0) {
          dest[index[(int32)(((*source[i])->asLong >> lshift) & 0xFF)]++] =
            source[i];
          i++;
        }
  }
 ...
 * @param isAscii Indicates if the table strings are to be stored in the ascii
...
{ // juliana@220_5
  ...
  if (!createPlainDB(context,
                     &table->db,
                     name,
                     create,
                     useCrypto,
                     sourcePath)) { // Creates or opens the table files.
    goto error;
  }
  ...
  if (name &&
    ...
    if (plainDB->db.size &&
        !tableLoadMetaData(context, table, throwException)) { // juliana@220_5
      goto error; // juliana@220_8: does not let the table be truncated if an
                  // error occurs when loading its metadata.
    }
  }
  return table;
 ...
 */
Table*
driverCreateTable(Context context,
                  TCObject driver,
                  CharP tableName,
                  CharP* names,
                  int32* hashes,
                  ...
                  Heap heap)
{
  ...
  if (!tableName) { // Temporary table.
    ...
    // guich@_105: First, checks if the table was already created in this
    // connection.
    ...
    if (!getDiskTableName(context,
                          appCrid,
                          tableName,
                          name)) { // Gets the table real name.
      return null;
    }
    ...
    if (!(table = tableCreate(context,
                              ...
                              sourcePath,
                              true,
                              OBJ_LitebaseIsAscii(driver),
                              OBJ_LitebaseUseCrypto(driver),
                              getLitebaseNodes(driver),
                              true,
                              heap))) {
      goto error;
    ...
    // Creates the index for the primary key or the composed index for the
    // composed primary key. // juliana@114_9
    if (!TC_htPutPtr(htTables, TC_hashCode(tableName), table) ||
        !tableSetMetaData(context,
                          table,
                          names,
                          hashes,
                          types,
                          ...
                          count,
                          composedPKColsSize) ||
        (primaryKeyCol != NO_PRIMARY_KEY &&
         !driverCreateIndex(
           context, table, &hashes[primaryKeyCol], 0, 1, null)) ||
        (composedPKColsSize > 0 &&
         !driverCreateIndex(
           context, table, null, 0, composedPKColsSize, composedPKCols))) {
      TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
      goto error;
    }
  }
...
/**
 * Renames a table. This never happens to be a temporary <code>ResultSet</code>
 * memory table.
 *
 * @param context The thread context where the function is being executed.
 ...
 */
bool
renameTable(Context context, TCObject driver, Table* table, CharP newTableName)
{ // rnovais@566_10
  TRACE("renameTable")
  ...
  while (--i >= 0) {
    if (columnIndexes[i]) {
      ...
      if (!indexRename(context,
                       columnIndexes[i],
                       result)) { // Renames the index files.
        return false;
      }
    }
  }
  ...
  while (--i >= 0) {
    ...
    if (!indexRename(context,
                     composedIndexes[i]->index,
                     result)) { // Renames the index files.
      return false;
    }
  }

  if (!TC_htPutPtr(htTables,
                   TC_hashCode(newTableName),
                   table)) { // Adds the new table name to the hash table.
    TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
    return false;
  }
  TC_htRemove(
    htTables,
    TC_hashCode(
      oldTableName)); // Removes the old table name from the hash table.
  return true;
}
 ...
 */
bool
renameTableColumn(Context context,
                  Table* table,
                  CharP oldColumn,
                  CharP newColumn,
                  bool reuseSpace)
{ // rnovais@566_17
  ...
  Hashtable* htName2index = &table->htName2index;
  int32 oldHash = TC_hashCode(oldColumn), newHash = TC_hashCode(newColumn),
        oldIdx = TC_htGet32Inv(
          htName2index, oldHash), // Gets the old column name. It must exist.
    newIdx = TC_htGet32Inv(htName2index,
                           newHash); // The new column name can't exist.
  ...
  if (newIdx >= 0) {
    TC_throwExceptionNamed(context,
                           "litebase.DriverException",
                           getMessage(ERR_COLUMN_ALREADY_EXIST),
                           newColumn);
    return false;
  }
...
// juliana@201_3: if an index is created after populating or purging the table,
// its nodes will be full in order to improve its usage and search speed.
/**
 ...
 * simple index.
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 * @throws DriverException If there is a null in the primary key or a duplicated
 * key.
 * @throws OutOfMemoryError If a heap memory allocation fails.
 */
bool
tableReIndex(Context context,
             Table* table,
             int32 column,
             bool isPKCreation,
             ComposedIndex* composedIndex)
{
  ...
  PlainDB* plainDB = &table->db;
  uint8* basbuf = plainDB->basbuf;
  Index* index = (column != -1) ? table->columnIndexes[column]
                                : composedIndex->index; // Gets the index.
  int32 n = plainDB->rowCount, i = -1;
  bool isDelayed = index->isWriteDelayed;

  if (!indexDeleteAllRows(context, index)) { // Cleans the index values.
    return false;
  }
  if (!indexSetWriteDelayed(context,
                            index,
                            true)) { // This makes the index creation faster.
    goto error1;
  }
  ...
  if (index->isOrdered && !composedIndex) { // Simple index using rowid.
    ...
    int32 k = 0,
      ...
      j = rows, columnCount = table->columnCount,
          indexCount = index->numberColumns, bytes = NUMBEROFBYTES(columnCount),
          indexSize = index->numberColumns, type, offset = 0, compare;
    bool isNull;
    SQLValue*** values;
    ...
    // juliana@270_25: solved a possible crash when an OutOfMemoryError occurs
    // when creating or recreating indices.
    ...
    while (++i < n) {
      ...
      // juliana@201_22: the null columns information wasn't being read when
      ...
      if (column != -1) {
        ...
        if (!readValue(context,
                       ...
                       *values[k],
                       ...
                       basbuf,
                       false,
                       isNull = isBitSet(columnNulls0, column),
                       false,
                       -1,
                       heap)) {
          goto error2;
        ...
        // juliana@202_10: Corrected a bug that would cause a DriverException if
        // there was a null in an index field when creating it after the table
        // is populated.
        if (isPKCreation && columnNulls0 && isNull) {
          TC_throwExceptionNamed(context,
                                 "litebase.DriverException",
                                 getMessage(ERR_PK_CANT_BE_NULL),
                                 0);
          goto error2;
        }
      } else {
        ...
        while (--j >= 0) {
          ...
          if (!readValue(context,
                         plainDB,
                         values[k][j],
                         columnOffsets[columns[j]],
                         types[j],
                         basbuf,
                         false,
                         isNull |= isBitSet(columnNulls0, columns[j]),
                         false,
                         -1,
                         heap)) {
            goto error2;
          ...
          // index. juliana@202_10: Corrected a bug that would cause a
          // DriverException if there was a null in an index field when creating
          // it after the table is populated.
          if (isPKCreation && columnNulls0 && isNull) {
            TC_throwExceptionNamed(context,
                                   "litebase.DriverException",
                                   getMessage(ERR_PK_CANT_BE_NULL));
      ...
      if (isNull) { // Do not store null records.
        ...
        // juliana@202_7: Corrected a bug that would cause long and double
        // indices to be built incorrectly. The record value is stored in an
        // empty field of the first record column value.
    ...
    if (!index->isOrdered) {
      // A radix sort is done for integer types. It is much more efficient than
      // quick sort.
      if (indexSize == 1 && (type == SHORT_TYPE || type == INT_TYPE ||
                             type == LONG_TYPE || type == DATE_TYPE)) {
    ...
    while (++k < rows) {
      ...
      if (isPKCreation && k > 0 &&
          (!(compare = compareSortRecords(
               indexSize, values[k], values[k - 1], types)) ||
           compare == MIN_SHORT_VALUE || compare == MAX_SHORT_VALUE)) {
        TC_throwExceptionNamed(context,
                               "litebase.PrimaryKeyViolationException",
                               getMessage(ERR_STATEMENT_CREATE_DUPLICATED_PK),
                               table->name);

        goto error2; // juliana@223_14: solved possible memory problems.
      }

      // juliana@202_7: Corrected a bug that would cause long and double indices
      // to be built incorrectly.
      if (((type == DATETIME_TYPE || type == LONG_TYPE ||
            type == DOUBLE_TYPE) &&
           !indexAddKey(context, index, values[k], (*values[k])->length)) ||
          (type != DATETIME_TYPE && type != LONG_TYPE && type != DOUBLE_TYPE &&
           !indexAddKey(context, index, values[k], (*values[k])->asTime))) {
        goto error2; // juliana@223_14: solved possible memory problems.
      }
    }

    if (!composedIndex ||
        column) { // An index beggining with rowid is always ordered.
      index->isOrdered = false;
    }
    heapDestroy(heap);
  }

  return indexSetWriteDelayed(
    context,
    index,
 ...
 */
bool
indexCreateIndex(Context context,
                 Table* table,
                 CharP fullTableName,
                 int32 columnIndex,
                 int32* columnSizes,
                 ...
                 Heap heap)
{
  ...
  // rnovais@113_1
  if (!(table->columnIndexes[columnIndex] = createIndex(context,
                                                        table,
                                                        columnTypes,
                                                        columnSizes,
                                                        indexName,
 ...
 */
bool
indexCreateComposedIndex(Context context,
                         Table* table,
                         CharP fullTableName,
                         uint8* columnIndexes,
                         int32* columnSizes,
                         ...
                         Heap heap)
{
  ...
  char indexName[DBNAME_SIZE];
  IntBuf intBuf;
  int32 tableLen = xstrlen(fullTableName), size = table->numberComposedIndexes,
        id = (newIndexNumber < 0) ? -newIndexNumber
                                  : newIndexNumber; // newIndexNumber < 0 means
                                                    // that this was a index.
  ComposedIndex* composedIndex;

  // The index name.
  xstrcpy(indexName, fullTableName);
  indexName[tableLen] = '&';
  xstrcpy(&indexName[tableLen + 1],
          TC_int2str(id, intBuf)); // Passes the newIndex index id.

  composedIndex =
    createComposedIndex(id, columnIndexes, numberColumns, table->heap);
  if (increaseArray &&
      table->numberComposedIndexes == MAX_NUM_INDEXES_APPLIED) {
    TC_throwExceptionNamed(
      context, "litebase.DriverException", getMessage(ERR_MAX_COMP_INDICES));
    return false;
  }

  // Creates the index of the composed index.
  if (!(composedIndex->index = createIndex(context,
                                           table,
                                           columnTypes,
                                           columnSizes,
                                           indexName,
  ...
  composedIndex->index->isOrdered =
    !*columnIndexes; // The rowid is the column 0.
  return true;
}
 ...
 * @param heap A heap to allocate the temporary strings when sorting a temporary
 * table.
 * @param stringArray A temporary string array used when sorting a temporary
 * table.
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 */
bool
readRecord(Context context,
           Table* table,
           SQLValue** record,
           int32 recPos,
           uint8* columnNulls,
           ...
           StringArray** stringArray)
{
  ...
  xmemmove(columnNulls,
           basbuf + columnOffsets[table->columnCount],
           NUMBEROFBYTES(table->columnCount));
  if (fieldList) // Reads only the columns used during sorting.
    while (--i >= 0) {
      // juliana@227_20: corrected order by or group by with strings being too
      // slow.
      j = fieldList[i]->tableColIndex;
      if ((columnTypes[j] != CHARS_TYPE &&
           columnTypes[j] != CHARS_NOCASE_TYPE) ||
          !stringArray[recPos][i].string) {
        // juliana@230_12
        if (!readValue(context,
                       plainDB,
                       record[j],
        ...
        if (record[j]->length) {
          asString = stringArray[recPos][i].string = (JCharP)TC_heapAlloc(
            heap,
            (asLength =
               ((stringArray[recPos][i].length = record[j]->length) + 1) << 1) +
              2);
          xmemmove(asString, record[j]->asChars, asLength);
        }
      } else {
        record[j]->length = stringArray[recPos][i].length;
        xmemmove(record[j]->asChars,
                 stringArray[recPos][i].string,
                 record[j]->length << 1);
      }
    }
            ...
            Heap heap)
{
  ...
  int32 columnCount = table->columnCount, i = columnCount, writePos = -1,
        rowid = 0, primaryKeyCol = table->primaryKeyCol, j,
        numberOfBytes = NUMBEROFBYTES(columnCount), type, offset,
        crc32, // juliana@230_12
    oldPos,
        k; // juliana@270_23
  bool changePos, addingNewRecord = recPos == -1, valueOk, hasIndex, isNullVOld,
                  isNull;
  int8* columnTypes = table->columnTypes;
  int32* columnSizes = table->columnSizes;
  uint16* columnOffsets = table->columnOffsets;
  ...
  xmemzero(columnNulls0,
           numberOfBytes); // First of all, clear the columnNulls used.

  while (--i) { // 0 = rowid = never is null.
    // juliana@226_11: corrected a constant Java String truncation when using it
    // with an insert or update prepared statement and its size were bigger than
    // the column definition. juliana@225_7: a PrimaryKeyViolation was not being
    // thrown when two strings with the same prefix were inserted and the field
    // definition had the size of the prefix and a primary key. juliana@214_6:
    // must trim strings during index update if they are longer than the field
    // size definition.
    if (columnSizes[i] && (tempRecord = values[i]) &&
        (int32)tempRecord->length > (j = columnSizes[i])) {
      tempRecord->length = j;
    }
      ...
      if (!definedAsNotNull(columnAttrs[i])) { // Can be null.
        if (!values[i] || values[i]->isNull) {
          if (!defaultValues[i]) {     // It doesn't have a default value.
            ...
            values[i] = defaultValues[i]; // If it doesn't have a value, stores
                                          // the default value.
          }
        }
      } else if (!values[i]) { // At this moment, if it can't be null,
                               // necessarily it has a default value.
  ...
  if (primaryKeyCol != NO_PRIMARY_KEY && values[primaryKeyCol] &&
      !checkPrimaryKey(context,
                       table,
                       &values[primaryKeyCol],
                       recPos,
                       addingNewRecord,
                       heap)) {
    return false;
  ...
  if ((i = table->numberComposedPKCols) > 0) {
    ...
    if (!checkPrimaryKey(
          context, table, vOlds, recPos, addingNewRecord, heap)) {
  ...
  if (addingNewRecord) { // Adding a record?
    ...
    (*values)->asInt = rowid =
      table->currentRowId; // Writes the rowId, marking the attribute as new.
    if (!resetAuxRowId(context, table)) {
      return false;
    }
  } else {
    if (!plainRead(context,
                   plainDB,
                   recPos)) { // May have to read the value before deleting the
                              // index value.
      return false;
    }
    writePos = recPos;
    xmove4(&rowid, basbuf);
    table->wasUpdated = true; // Indicates if the table was updated after the
  ...
  // Verifies if the value to be read is null. IMPORTANT: the array of bytes
  // can't be stored in table.columnNulls[0]. This variable is in use. Then, it
  // will be read in Table.columnNulls[1].
  xmemmove(columnNulls1, buffer, numberOfBytes);

  while (++i < columnCount) {
    isNullVOld = false;
    isNull = isBitSet(columnNulls0, i);
    idx = columnIndexes[i]; // If a new value is being written, the table index
                            // (if any) needs to be updated.
    hasIndex = (valueOk = (values[i] || isNull)) &&
               idx; // Only if this row is being updated.
    type = columnTypes[i];
    ...
    if (!addingNewRecord) { // Reads the previous value if it is an update.
      ...
      if (!readValue(context,
                     ...
                     basbuf,
                     false,
                     isNullVOld = isBitSet(columnNulls1, i),
                     false,
                     -1,
                     heap)) {
        return false;
      ...
      // juliana@202_21: Always writes the string at the end of the .dbo. This
      // removes possible bugs when doing updates. A blob in the .dbo must have
      // its position changed if the the new value is greater than the old one.
      if (valueOk && type == BLOB_TYPE && values[i] &&
          vOlds[i]->length < values[i]->length) {
        changePos = true;
      }
    }
    ...
    if (hasIndex) {
      // juliana@225_4: corrected a bug that could not build the index correctly
      // if there was the value 0 inserted in the index. Updating key? Removes
      // the old one and adds the new one.
      if (addingNewRecord ||
          valueCompareTo(
            null, vOlds[i], values[i], type, isNullVOld, isNull, null)) {
        if (!isNull) { // If it is updating a 'non-null value' to 'null value',
                       // only removes it.
          oldPos = db->position;
          tempRecord = values[i];
          ...
          if (!indexAddKey(context,
                           idx,
                           &tempRecord,
                           writePos)) { // juliana@223_14: solved possible
                                        // memory problems.
            return false;
          }
          nfSetPos(db, oldPos);
  ...
  if ((j = i = table->numberComposedIndexes)) { // Fills the composed indices.
    ...
    uint8* columns;
    int32 maxNumberColumns = 0, column;
    ...
    // Allocates the records for the composed indices just once, using the
    // maximum size.
    while (--j >= 0) {
      maxNumberColumns =
        MAX(maxNumberColumns, composedIndexes[j]->numberColumns);
    }

    if (!addingNewRecord) {
      oldVals =
        (SQLValue*)TC_heapAlloc(heap, maxNumberColumns * sizeof(SQLValue));
    }
    ...
    // least one field that is not null can be stored in the index, but the null
    // values must be handled. This implies in changing the index format. Maybe
    // using the same way like null values are handled on tables. This certainly
    // will decrease the index performance. For simplicity, the key will be
    // stored in a composed index only if all values are not null. This is a
    // project choice.
    while (--i >= 0) {
      ...
      valueOk = remove = true;
      change = false; // juliana@252_2: corrected a bug of possible composed
                      // index corruption when updating or deleting data.
      oldPos =
        db->position; // juliana@201_4: corrected a bug that could corrupt the
                      // table when updating the composed index.
      ...
      while (--j >= 0) {
        ...
        // juliana@230_43: solved a possible exception if updating a table with
        // composed indices and nulls.
        ...
        if (values[column]) { // juliana@201_18: can't reuse values. Otherwise,
                              // it will spoil the next update.
          vals[j] = values[column];
          if (addingNewRecord || valueCompareTo(null,
                                                values[column],
                                                vOlds[column],
                                                columnTypes[column],
                                                !remove,
      ...
      // juliana@230_43: solved a possible exception if updating a table with
      // composed indices and nulls. juliana@252_2: corrected a bug of possible
      // composed index corruption when updating or deleting data.
      ...
      // juliana@252_2: corrected a bug of possible composed index corruption
      // when updating or deleting data.
      if (valueOk &&
          change) { // juliana@201_4: corrected a bug that could corrupt the
                    // table when updating the composed index.
  ...
  xmemmove(
    buffer,
    columnNulls0,
    numberOfBytes); // After the columns, stores the bytes of the null values.

  // juliana@220_4: added a crc32 code for every record.
  k = basbuf[3]; // juliana@270_23: Corrected a RowIterator bug of an update
                 // changing an updated row to synced again.
  basbuf[3] = 0; // juliana@222_5: The crc was not being calculated correctly
                 // for updates.
    ...
    while (--j)
      if (columnTypes[j] == CHARS_TYPE || columnTypes[j] == CHARS_NOCASE_TYPE) {
        if (values[j] && isBitUnSet(columnNulls0, j)) {
          crc32 = updateCRC32(
            (uint8*)values[j]->asChars, values[j]->length << 1, crc32);
        } else if (!addingNewRecord && isBitUnSet(columnNulls0, j) &&
                   !vOlds[j]->isNull && vOlds[j]->asChars) {
          crc32 = updateCRC32(
            (uint8*)vOlds[j]->asChars, vOlds[j]->length << 1, crc32);
        }
      } else if (columnTypes[j] == BLOB_TYPE) {
        if (values[j] && isBitUnSet(columnNulls0, j)) {
          // juliana@253_12: corrected a possible table corruption when adding a
          // small blob. juliana@239_4: corrected a non-desired possible row
          // delete when recovering a table with blobs.
  ...
  basbuf[3] = k; // juliana@270_23: Corrected a RowIterator bug of an update
                 // changing an updated row to synced again.

  if (rowid > 0) { // Now the record's attribute has to be updated.
    int32 id = addingNewRecord ? (rowid & ROW_ID_MASK) | ROW_ATTR_NEW
                               : rowUpdated(rowid);
    xmove4(basbuf, &id);
  }

  // Writes the row.
  if (!(addingNewRecord ? plainWrite(context, plainDB)
                        : plainRewrite(context, plainDB, writePos))) {
    return false;
  }
  ...
  // juliana@270_25: corrected a possible lose of records in recover table when
  // 10 is passed to LitebaseConnection.setRowInc(). Flushs .db and .dbo.
  if (!db->dontFlush)
    if ((db->cacheIsDirty && !flushCache(context, db)) ||
        (dbo->cacheIsDirty && !flushCache(context, dbo))) {
      return false;
    }
 ...
 */
bool
writeRSRecord(Context context, Table* table, SQLValue** values)
{
  TRACE("writeRSRecord")
  ...
  xmemmove(&basbuf[offsets[n]],
           nulls,
           NUMBEROFBYTES(n)); // Writes the null values.
 ...
 */
bool
checkPrimaryKey(Context context,
                Table* table,
                SQLValue** values,
                int32 recPos,
                bool newRecord,
                Heap heap)
{
  ...
  if (primaryKeyCol == -1) {
    columns = table->composedIndexes[table->composedPK]
                ->columns; // Gets the columns of the index.
  ...
  if (!newRecord) { // An update.
    ...
    while (--i >= 0) {
      ...
      // If it is updating a record, reads the old value and checks if a primary
      ...
      if (values[i] &&
          valueCompareTo(
            null, &oldValues[i], values[i], types[i], false, false, null)) {
        hasChanged = true;
      }
 ...
 * <code>SQLElement.STMT_INSERT</code> or <code>SQLElement.CMD_UPDATE</code>.
 * @param nValues The number of values being verified.
 * @return <code>false</code> if a null violation occurs; <code>true</code>,
 * otherwise.
 * @throws DriverException If a primary key is or a <code>NOT NULL</code> field
 * is is <code>null</code>.
 */
bool
verifyNullValues(Context context,
                 Table* table,
                 SQLValue** record,
                 int32 statementType,
                 int32 nValues)
{
  ...
  if (statementType == CMD_INSERT) { // Insert statement.
    ...
    if ((i != NO_PRIMARY_KEY) &&
        (isBitSet(storeNulls, i) ||
         ((!record[i] || record[i]->isNull) && !defaultValues[i]))) {
      TC_throwExceptionNamed(
        context, "litebase.DriverException", getMessage(ERR_PK_CANT_BE_NULL));
      return false;
    }

    i = table->columnCount;
    while (--i)
      if ((!record[i] || record[i]->isNull) && !defaultValues[i] &&
          definedAsNotNull(attrs[i])) { // A not null field can't have a null.
        TC_throwExceptionNamed(context,
                               "litebase.DriverException",
                               getMessage(ERR_FIELD_CANT_BE_NULL),
                               table->columnNames[i]);
        return false;
      }
  } else { // Update statement.
    if ((i != NO_PRIMARY_KEY) &&
        (isBitSet(storeNulls, i))) { // The primary key can't be null.
      TC_throwExceptionNamed(
        context, "litebase.DriverException", getMessage(ERR_PK_CANT_BE_NULL));
      return false;
    }
    i = nValues;
    while (--i >= 0)
      if (isBitSet(storeNulls, i) &&
          definedAsNotNull(attrs[i])) { // If it is to store a null but a null
                                        // can't be stored.
        TC_throwExceptionNamed(context,
                               "litebase.DriverException",
                               getMessage(ERR_FIELD_CANT_BE_NULL),
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 * @throws SQLParseException If a conversion from string to a number or
 * date/datetime fails.
 * @throws DriverException If a blob is passed in a statement that is not
 * prepared.
 */
bool
convertStringsToValues(Context context,
                       Table* table,
                       SQLValue** record,
                       uint32 nValues)
{
  ...
  while (--nValues) { // 0 = rowid.
    ...
    // A blob can't be set in a normal statement (if the string is not null, it
    // won't be a "?".
    if ((type = columnTypes[nValues]) == BLOB_TYPE && asChars &&
        (asChars[0] != '?' || asChars[1])) {
      TC_throwExceptionNamed(
        context, "litebase.DriverException", getMessage(ERR_BLOBS_PREPARED));
      return false;
    }
 ...
 */
int32
updateCRC32(uint8* buffer, int32 length, int32 oldCRC)
{
  TRACE("computeCRC32")
  oldCRC = ~oldCRC;
 ...
 */
bool
resetAuxRowId(Context context, Table* table)
{ // rnovais@570_61
  TRACE("resetAuxRowId")
 ...
 */
bool
freeTableHT(Context context, Table* table)
{
  TRACE("freeTableHT")
 ...
 */
bool
freeTable(Context context, Table* table, bool isDelete, bool updatePos)
{
  TRACE("freeTable")
  bool ret = true;

  if (table) {
    int32 n = table->columnCount, i;
    Index** columnIndexes = table->columnIndexes;
    ComposedIndex** composedIndexes = table->composedIndexes;
    Index* idx;
    TCObject obj;
    ...
    if (*table->name &&
        (i = table->numberComposedIndexes)) { // Frees the composed indices in a
                                              // normal table.
      while (--i >= 0) {
    ...
    // juliana@221_1: solved a problem that could reduce the free memory too
    // much if many prepared statements were created and collected many times.
    if (preparedStmts) {
      Hashtable* htPS;
      TCObject sqlObj;
      do {
        ...
        if ((obj = list->value)) {
          ...
          TC_htRemove(htPS,
                      TC_JCharPHashCode(String_charsStart(sqlObj),
                                        String_charsLen(sqlObj)));
 ...
 */
bool
getTableColValue(Context context,
                 ResultSet* resultSet,
                 int32 column,
                 SQLValue* value)
{
  ...
  return readValue(context,
                   &table->db,
                   value,
                   table->columnOffsets[column],
                   table->columnTypes[column],
                   table->db.basbuf,
                   !*table->name,
                   isBitSet(table->columnNulls, column),
                   true,
                   table->columnSizes[column],
                   null);
}
 ...
 * @return <code>true</code> if the table already exists; <code>false</code>,
 * othewise.
 * @throws AlreadyCreatedException If the table is already created.
 * @throws DriverException If the path is too long.
 */
bool
tableExistsByName(Context context, TCObject driver, CharP name)
{
  ...
  // Verifies if the table exists checking if the .db exists. The name must be
  // already on lower case.
 ...
 * @return <code>false</code> if table name is too big; <code>true</code>,
 * otherwise.
 * @throws DriverException If the table name is too big.
 */
bool
getDiskTableName(Context context, int32 crid, CharP name, CharP buffer)
{
  TRACE("getDiskTableName")
  CharP to = &buffer[5];

  if (xstrlen(name) > MAX_TABLE_NAME_LENGTH) { // The table name can't be too
                                               // big because of palm os.
    TC_throwExceptionNamed(context,
                           "litebase.DriverException",
 ...
 */
Table*
getTableFromName(Context context, TCObject driver, TCObject name)
{
  TRACE("getTableFromName")
  char tableName[DBNAME_SIZE];
  int32 length = String_charsLen(name);

  if (length > MAX_TABLE_NAME_LENGTH) { // The table name can't be too big
                                        // because of palm os.
    TC_throwExceptionNamed(context,
                           "litebase.DriverException",
                           getMessage(ERR_MAX_TABLE_NAME_LENGTH));
 ...
 */
Table*
getTable(Context context, TCObject driver, CharP tableName)
{
  TRACE("getTable")
  char name[DBNAME_SIZE];
  Table* table = null;
  Hashtable* htTables = getLitebaseHtTables(driver);
  int32 length = xstrlen(tableName), appCrid = OBJ_LitebaseAppCrid(driver),
        hashCode;
  ...
  TC_CharPToLower(tableName); // The hash code must be in lower case so that the
                              // program is more efficient.

  // Opens a not already opened table.
  if (!(table =
          (Table*)TC_htGetPtr(htTables, hashCode = TC_hashCode(tableName)))) {
    // Gets the table name.
    if (getDiskTableName(context, appCrid, tableName, name)) {
      ...
      if ((table = tableCreate(context,
        ...
        if (!TC_htPutPtr(htTables,
                         hashCode,
                         table)) { // Puts the table hash code in the hash table
                                   // of opened tables.
          freeTable(context, table, false, false);
          TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
          return null;
 ...
 */
uint8*
readString(uint8* buffer, CharP* string, Heap heap)
{
  TRACE("readString")
  uint32 length = 0;
  ...
  xmemmove(
    (*string = (CharP)TC_heapAlloc(heap, length + 1)), buffer += 2, length);
 ...
 */
uint8*
readStringArray(uint8* buffer, CharP** strings, int32 count, Heap heap)
{
 ...
 */
uint8*
writeString(uint8* buffer, CharP string)
{
  TRACE("writeString")
 ...
 */
uint8*
writeStringArray(uint8* buffer, CharP* strings, int32 count)
{
 ...
 */
uint8*
writeString16(uint8* buffer, JCharP string, int32 length)
{
  TRACE("writeString16")
  xmove2(buffer, &length);
 ...
 */
bool
setModified(Context context, Table* table)
{
  TRACE("setModified")
  ...
  if (!table->isModified) {
    ...
    nfSetPos(dbFile, 6);
    if (nfWriteBytes(context, dbFile, (uint8*)&isAscii, 1) &&
...
int32
randBetween(int32 low, int32 high)
{
  TRACE("randBetween")
