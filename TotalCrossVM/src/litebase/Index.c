...
Index* createIndex(Context context, Table* table, int8* keyTypes, int32* colSizes, CharP name,
	...
	index->nodeRecSize = 2 + index->btreeMaxNodes * (index->keyRecSize = keyRecSize) + ((
							 index->btreeMaxNodes + 1) << 1);

	index->heap = heap;

	// juliana@230_35: now the first level nodes of a b-tree index will be loaded in memory.
	index->firstLevel = (Node**)TC_heapAlloc(heap,
	...
	if (!nfCreateFile(context, buffer, !exist, table->db.db.useCrypto, sourcePath, fnodes,
					  index->nodeRecSize << 1)) {
		return null;
	}
...
bool driverCreateIndex(Context context, Table* table, int32* columnHashes, bool isPKCreation,
	...
	while (--i >= 0) {
		...
		columns[i] = idx = (composedPKCols ? composedPKCols[i] : TC_htGet32Inv(&table->htName2index,
							columnHashes[i]));
	...
	if (indexCount == 1) {
		if (newIndexNumber < 0
			|| !indexCreateIndex(context, table, table->name, columns[0], columnSizes, columnTypes, false,
								 heap)) {
			goto error;
		}
		...
		if (newIndexNumber < 0
			|| !indexCreateComposedIndex(context, table, table->name, columns, columnSizes, columnTypes,
										 indexCount, newIndexNumber, true, false, heap)) {
			goto error;
		}
	...
	if (plainDB->rowCount > 0) { // The header may be created but the table may still be empty
		...
		if (indexCount == 1) {
			...
			if (!tableReIndex(context, table, -1, isPKCreation, table->composedIndexes[i - 1])) {
				driverDropComposedIndex(context, table, table->composedPrimaryKeyCols, table->numberComposedPKCols,
										i - 1, true);
				if (isPKCreation) {
...
bool indexRemoveValue(Context context, Key* key, int32 record) {
	...
	if (index->fnodes.size) {
		...
		while (true) {
			keyFound = &(currKeys = curr->keys)[pos = nodeFindIn(context, curr, key,
													false)]; // juliana@201_3 // Finds the key position.
			...
			if (--nodeCounter <
				0) { // juliana@220_16: does not let the index access enter in an infinite loop.
...
Node* indexLoadNode(Context context, Index* index, int32 idx) {
	...
	if (index->isWriteDelayed && cand->isDirty
		&& nodeSave(context, cand, false, 0, cand->size) < 0) { // Saves this one if it is dirty.
		return null;
	}
	cand->idx = idx;
...
bool indexClimbGreaterOrEqual(Context context, Node* node, int32 start, MarkBits* markBits,
	...
	if (nodeIsLeaf(node))
		...
		if (index->nodesArrayCount > 0) {
			...
			curr = createNode(
					   index);    // juliana@230_32: corrected a bug of inequality searches in big indices not returning all the results.
		}
...
bool indexSplitNode(Context context, Node* curr, int32 count) {
	...
	while (curr) {
		...
		if ((right = nodeSave(context, curr, true, medPos + 1,
							  curr->size)) < 0) { // right sibling - must be the first one to save!
			return false;
		}
...
bool indexClose(Context context, Index* index) {
	...
	index->fnodes.finalPos = index->nodeCount *
							 index->nodeRecSize; // Calculated the used space; the file will have no zeros at the end.
...
bool indexDeleteAllRows(Context context, Index* index) {
	...
	index->cacheI = 0;
	index->nodeCount = fnodes->size = fnodes->position = fnodes->finalPos = fnodes->cachePos =
										  fnodes->cacheIsDirty = 0;
	return true;
}
...
bool sortRecordsAsc(Context context, Index* index, IntVector* bitMap, Table* tempTable,
	...
	while (count) {
		...
		if (nodeIsLeaf(
				curr)) { // If the node do not have children, just process its keys in the ascending order.
			i = -1;
			while (++i < size)
...
bool sortRecordsDesc(Context context, Index* index, IntVector* bitMap, Table* tempTable,
	...
	while (count) {
		...
		if (nodeIsLeaf(
				curr)) { // If the node do not have children, just process its keys in the descending order.
			if (!writeKey(context, index, valRec, bitMap, tempTable, record, columnIndexes)) {
				return false;
			}
			i = size;
	...
	if (valRec != NO_VALUE && (!bitMap->items || IntVectorisBitSet(bitMap, valRec))) {
		...
		while (--i >= 0) { // Reads the fields for the temporary table.
			...
			if (!(isNull = isBitSet(origNulls, colIndex))
				&& !readValue(context, plainDB, record[i], offsets[colIndex], types[colIndex], basbuf, false, false,
							  true, -1, null)) {
				return false;
			}
