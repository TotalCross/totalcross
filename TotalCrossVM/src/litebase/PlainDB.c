 ...
 */
bool createPlainDB(Context context, PlainDB* plainDB, CharP name, bool create, bool useCrypto,
				   TCHARP sourcePath) {
	TRACE("createPlainDB")
...
bool plainAdd(Context context, PlainDB* plainDB) {
	...
	if (--plainDB->rowAvail <= 0) { // Checks if there are no more space pre-allocated.
		if (!plainDB->growTo(context, &plainDB->db,
							 (plainDB->rowCount + plainDB->rowInc) * plainDB->rowSize + plainDB->headerSize)) {
			return false;
		}
		plainDB->rowAvail = plainDB->rowInc;
	}
	return plainSetPos(context, plainDB,
...
bool plainRead(Context context, PlainDB* plainDB, int32 record) {
	...
	return plainSetPos(context, plainDB, record)
		   && plainDB->readBytes(context, &plainDB->db, plainDB->basbuf, plainDB->rowSize);
}
...
bool plainRewrite(Context context, PlainDB* plainDB, int32 record) {
	TRACE("plainRewrite")
	return plainSetPos(context, plainDB, record)
		   && plainDB->writeBytes(context, &plainDB->db, plainDB->basbuf, plainDB->rowSize);
}
	...
	if (plainDB) {
		if (plainDB->db.fbuf || fileIsValid(plainDB->db.file) || plainDB->db.cache) {
			if (*plainDB->name) {
				...
				*pointer++ = plainDB->isAscii ? IS_ASCII | !plainDB->wasNotSavedCorrectly :
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 */
bool readValue(Context context, PlainDB* plainDB, SQLValue* value, int32 offset, int32 colType,
			   uint8* buffer, bool isTemporary, bool isNull,
	...
	if (!isNull)
		switch (colType) {
			...
			case CHARS_TYPE: {
				...
				if (position <
					0) { // juliana@270_22: solved a possible crash when the table is corrupted on Android and possibly on other platforms.
				...
				value->asBlob = (uint8*)
								plainDB; // Holds the plainDB pointer so the string don't need to be loaded in the temporary table.
 ...
 * @return <code>false</code> if an error occurs; <code>true</code>, otherwise.
 */
bool writeValue(Context context, PlainDB* plainDB, SQLValue* value, uint8* buffer, int32 colType,
				int32 colSize, bool isValueOk, bool addingNewRecord, bool isNull, bool isTempBlob) {
	TRACE("writeValue")
		...
		switch (colType) {
			...
			case CHARS_TYPE:
				if (isTempBlob) {
					...
					int32 length =
						value->length, // juliana@225_7: the string that is bigger than its field definiton was already trimmed.
			...
			case BLOB_TYPE: {
				...
				if (isTempBlob) { // juliana@220_3: blobs are not loaded anymore in the temporary table when building result sets.
					...
					if (addingNewRecord && (dbo->finalPos + size) > (int32)dbo->size
						&& !plainDB->growTo(context, dbo, dbo->size + size * MAX(16,
											plainDB->rowInc))) { // guich@201_8: grow using rowInc instead of 16 if rowInc > 16
						return false;
					}
...
bool loadString(Context context, PlainDB* plainDB, JCharP string, int32 length) {
	...
	} else if (!plainDB->readBytes(context, &plainDB->dbo, (uint8*)string,
