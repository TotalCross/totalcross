 ...
 */
int32 str16CompareTo(JCharP string1, JCharP string2, int32 length1, int32 length2,
					 bool isCaseless) {
...
int32 str16IndexOf(JCharP charsStr, JCharP subStr, int32 charsLen, int32 subLen, bool isCaseless) {
	...
	if (isCaseless) {
		...
		while (!found) {
			while (i <= j
				   && TC_JCharToLower(*charsStr) != c) { // Searches for the next ocurrence of the first char.
				i++;
				charsStr++;
			}
...
int32 testAndPrepareTime(CharP chars) {
	...
	if (n > 0 && n <= 13) {
		...
		if ((hour = p[0]) < 0 || hour > 23 || (minutes = p[1]) < 0 || minutes > 59 || (seconds = p[2]) < 0
			|| seconds > 59 || (millis = p[3]) < 0 || millis > 999) {
			return -1;
		}
...
bool testAndPrepareDateAndTime(Context context, SQLValue* value, CharP chars, int32 type) {
	...
	if (type == DATE_TYPE && (value->asInt = testAndPrepareDate(str)) == -1) {
		TC_throwExceptionNamed(context, "litebase.SQLParseException", getMessage(ERR_VALUE_ISNOT_DATE),
							   chars);
		return false;
...
int32 compareRecords(SQLValue** record1, SQLValue** record2, uint8* nullsRecord1,
	...
	while (++i <
		   sortFieldListCount) { // Compares the records, using the sequence used by the sort column List.
		index = (field = sortFieldList[i])->tableColIndex;

		// Compares the elements checking if they are null.
		result = valueCompareTo(null, record1[index], record2[index], field->dataType,
								isBitSet(nullsRecord1, index), isBitSet(nullsRecord2, index), null);
...
int64 getTimeLong(int32 year, int32 month, int32 day, int32 hour, int32 minute, int32 second) {
	TRACE("getTimeLong")
	return (int64)year * (int64)1000000000L * (int64)10L + month * 100000000L + day * 1000000 + hour *
...
void date2JCharP(int32 year, int32 month, int32 day, JCharP buffer) {
	...
	TC_CharP2JCharPBuf(dateTimeBuf, 10, buffer,
					   false); // juliana@238_1: corrected the end quote not appearing in the log files after dates.
}
...
float str2float(CharP chars, bool* error) {
	...
	if ((value = (value < 0) ? - value : value) && (value < MIN_FLOAT_VALUE
