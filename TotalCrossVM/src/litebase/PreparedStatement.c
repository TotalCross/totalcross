...
bool psSetStringParamValue(Context context, TCObject stmt, TCObject string, int32 index,
	...
	switch (statement->type) { // Sets the parameter.
		case CMD_DELETE:
			if (!setParamValueStringDel(context, (SQLDeleteStatement*)statement, index, stringChars,
										stringLength)) {
				return false;
			}
			break;
		case CMD_INSERT:
			if (!setStrBlobParamValueIns(context, (SQLInsertStatement*)statement, index, stringChars,
										 stringLength, true)) {
				return false;
			}
			break;
		case CMD_SELECT:
			...
			break;
		case CMD_UPDATE:
			if (!setStrBlobParamValueUpd(context, (SQLUpdateStatement*)statement, index, stringChars,
										 stringLength, true)) {
				return false;
			}
	}
	...
	if (OBJ_PreparedStatementStoredParams(
		...
		if (string) { // The parameter is not null.
			if (stringLength + 2 > paramsLength[index]) { // Reuses the buffer whenever possible
				if (!(paramAsStr = paramsAsStrs[index] = (JCharP)xrealloc((uint8*)paramAsStr,
								   (stringLength + 3) << 1))) {
					TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
					return false;
				}
...
TCObject toStringBuffer(Context context, TCObject statement) {
	...
	if (OBJ_PreparedStatementStoredParams(statement)) { // There are no parameters.
		...
		while (++i < storedParams
			   ...
			   && TC_appendJCharP(context, logSBuffer, sql + paramsPos[i] + 1,
...
void resetWhereClause(SQLBooleanClause* whereClause, Heap heap) {
	...
	if (whereClause) { // guich@552_37: it may be null
		...
		whereClause->expressionTree = cloneTree(whereClause->expressionTreeBak, whereClause->expressionTree,
												heap);
