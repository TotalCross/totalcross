 ...
 */
void
freePreparedStatement(int32 unused, TCObject statement)
{
  ...
  if (!OBJ_PreparedStatementDontFinalize(
        statement)) { // The prepared statement shouldn't be finalized twice.
    ...
    switch (OBJ_PreparedStatementType(statement)) { // Destroy the statement.
      case CMD_DELETE: {
        SQLDeleteStatement* deleteStmt =
          (SQLDeleteStatement*)getPreparedStatementStatement(statement);
      ...
      case CMD_INSERT: {
        SQLInsertStatement* insertStmt =
          (SQLInsertStatement*)getPreparedStatementStatement(statement);
      ...
      case CMD_SELECT: {
        SQLSelectStatement* selectStmt =
          (SQLSelectStatement*)getPreparedStatementStatement(statement);
      ...
      case CMD_UPDATE: {
        SQLUpdateStatement* updateStmt =
          (SQLUpdateStatement*)getPreparedStatementStatement(statement);
 ...
 */
bool
psSetNumericParamValue(NMParams p, int32 type)
{
  ...
  if (testPSClosed(p)) {
    ...
    if (selectStmt) { // Only sets the parameter if the statement is not null.
      ...
      switch (selectStmt->type) { // Sets the parameter.
        case CMD_DELETE:
          if (!setNumericParamValueDel(
                context, (SQLDeleteStatement*)selectStmt, index, value, type)) {
            return false;
          }
          break;
        case CMD_INSERT:
          if (!setNumericParamValueIns(
                context, (SQLInsertStatement*)selectStmt, index, value, type)) {
            return false;
          }
          break;
        case CMD_SELECT:
          if (!setNumericParamValueSel(
                context, selectStmt, index, value, type)) {
            return false;
          }
          break;
        case CMD_UPDATE:
          if (!setNumericParamValueUpd(
                context, (SQLUpdateStatement*)selectStmt, index, value, type)) {
            return false;
          }
      }
      ...
      if (OBJ_PreparedStatementStoredParams(
        ...
        int32 length, maxLength = paramsLength[index];

        // juliana@214_2: corrected a bug that could crash the application when
        // using logger.
...
// juliana@238_1: corrected the end quote not appearing in the log files after
// dates.
/**
 ...
 */
bool
psSetStringParamValue(Context context,
                      TCObject stmt,
                      TCObject string,
                      int32 index,
                      int32 stringLength)
{
  ...
  switch (statement->type) { // Sets the parameter.
    case CMD_DELETE:
      if (!setParamValueStringDel(context,
      ...
      break;
    case CMD_INSERT:
      if (!setStrBlobParamValueIns(context,
      ...
      break;
    case CMD_SELECT:
      if (!setParamValueStringSel(
            context, statement, index, stringChars, stringLength)) {
        return false;
      }
      break;
    case CMD_UPDATE:
      if (!setStrBlobParamValueUpd(context,
  ...
  if (OBJ_PreparedStatementStoredParams(
        stmt)) { // Only stores the parameter if there are parameters to be
                 // stored.
    JCharP* paramsAsStrs = getPreparedStatementParamsAsStrs(stmt);
    ...
    if (string) { // The parameter is not null.
      if (stringLength + 2 >
          paramsLength[index]) { // Reuses the buffer whenever possible
        if (!(paramAsStr = paramsAsStrs[index] = (JCharP)xrealloc(
                (uint8*)paramAsStr, (stringLength + 3) << 1))) {
          TC_throwExceptionNamed(context, "java.lang.OutOfMemoryError", null);
          return false;
        }
...
/**
 * Returns the sql used in this statement in a string buffer. If logging is
 ...
 */
TCObject
toStringBuffer(Context context, TCObject statement)
{
  ...
  if (OBJ_PreparedStatementStoredParams(
    ...
    while (
      ++i < storedParams &&
      TC_appendJCharP(
        context, logSBuffer, paramsAsStrs[i], TC_JCharPLen(paramsAsStrs[i])) &&
      TC_appendJCharP(context,
                      logSBuffer,
                      sql + paramsPos[i] + 1,
                      (paramsPos[i + 1] - paramsPos[i] - 1)))
      ;
    ...
    if (!TC_appendJCharP(
          context, logSBuffer, String_charsStart(sql), String_charsLen(sql))) {
      return null;
    }
  }
...
/**
 * Resets a where clause because the <code>expression</code> may change between
 * runs of a prepared statement with a where clause.
 *
 ...
 */
void
resetWhereClause(SQLBooleanClause* whereClause, Heap heap)
{
  ...
  if (whereClause) { // guich@552_37: it may be null
    ...
    // gotten. guich@554_13: Use the expressionTreeBak as a condition instead of
    // expressionTree (it should always be replaced after the first try).
    whereClause->expressionTree = cloneTree(
      whereClause->expressionTreeBak, whereClause->expressionTree, heap);

    whereClause->resultSet = null;
  }
}

// juliana@226_14: corrected a bug that would make a prepared statement with
// group by not work correctly after the first execution.
/**
 * Resets an order by or group by clause because the <code>tableColIndex</code>
 * may change between runs of a prepared statement with a sort field. So, it is
 * necessary to cache the <code>tableColIndex</code> of order by fields.
 *
 * @param orderByClause the order by clause to be reseted.
 */
void
resetColumnListClause(SQLColumnListClause* columnListClause)
{
 ...
 * @param storeNulls The prepared statement field that indicates if it is to
 * store nulls in the parameters or not.
 ...
 * @param isPreparedUpdateStmt Indicates if the prepared statement is an update
 * prepared statement or not.
 */
void
rearrangeNullsInTable(Table* table,
                      SQLValue** record,
                      uint8* storeNulls,
                      uint8* paramDefined,
                      uint8* paramIndexes,
 ...
 */
bool
testPSClosed(NMParams params)
{
  TRACE("testPSClosed")
  TCObject statement = params->obj[0];

  if (OBJ_PreparedStatementDontFinalize(
        statement)) { // Tests if the prepared statement is closed.
    TC_throwExceptionNamed(params->currentContext,
                           "java.lang.IllegalStateException",
                           getMessage(ERR_PREPARED_STMT_CLOSED));
    return false;
  }
  if (OBJ_LitebaseDontFinalize(OBJ_PreparedStatementDriver(
