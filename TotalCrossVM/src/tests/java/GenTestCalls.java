...
public class GenTestCalls
{
  ...
  class TestCase
  {
    ...
    { // used in the quick sort
      return useHIN ? ("" + hardIndexNumber)
  ...
  private int find(Vector in, String what, int start)
  { // vector.find is not working
    int depIdx = -1;
    {
      int nn = in.size();
      ...
      do {
        ...
        for (i = 0; i < n; i++) {
          ...
          if (tc.dependsOn != null) {
            ...
            if (depIdx > i) { // if the dependency is in a place below us, then
                              // we have to move ourself to after it
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (tc.dependsOn != null) {
        ...
        if (!ht.exists(tc.dependsOn)) {
          error("Error! " + tc.methodName + " (" + tc.sourceFile +
                ") depends on " + tc.dependsOn + ", which was not found!");
          allFound = false;
        }
      }
    }
  ...
  private void buildOutput(TestCase[] tcs) throws Exception
  {
    ...
    for (i = 0; i < n; i++) {
      sb.setLength(0);
      TestCase tc = tcs[i];
      sb.append("void test_")
        .append(tc.methodName)
        .append("(struct TestSuite *tc, Context currentContext);");
      if (sb.length() < 67) { // pad
    ...
    for (i = 0; i < n; i++) {
      sb.setLength(0);
      ds.writeBytes(sb.append("   tests[")
                      .append(i)
                      .append("] = test_")
                      .append(tcs[i].methodName)
                      .append(";\n")
                      .toString()
                      .getBytes());
    }
  ...
  private void searchIn(String folder) throws Exception
  {
    ...
    if (files != null) {
      for (int i = 0; i < files.length; i++) {
        ...
        if (files[i].endsWith("/")) {
          searchIn(fullPath);
        } else if (files[i].endsWith("_test.h")) {
  ...
  private void parseTestCase(String line, String file)
  {
    try {
      ...
      file = (java.io.File.separator.charAt(0) == '/')
               ? file.replace('\\', '/')
               : file.replace('/', '\\');
      ...
      } else if ((open = line.indexOf('#')) >
                 0) { // does it have a hard index number
        tc.hardIndexNumber =
          totalcross.sys.Convert.toInt(line.substring(open + 1).trim());
      }

      if (ht.exists(tc))
        error("Error! Testcase " + tc.methodName + " duplicated in " +
              tc.sourceFile + " and " + ((TestCase)ht.get(tc)).sourceFile);
      else {
        ht.put(tc, tc);
      }
    } catch (Exception e) {
  ...
  public static void main(String args[]) { new GenTestCalls(args); }
}
