...
The 'finish' label must be always defined, because if a test fails, it will jump
to it so that a cleanup can be made (for sake, freeing memory allocated for the
test). The ASSERT macros will jump to the label in case of a failure.

There are three assertion macros: ASSERT<n>_EQUALS, where n is the number of
parameters it receives, excluding the extra parameter for each macro, which is
the comparision type, described below:
...
There's also the ASSERT_BETWEEN macro, used with numeric types, which can be
used to assert that a value is between two numbers (low <= x <= high).
...
The TESTCASE macro declares a test case. The first parameter is the function
name, and the second parameter is a group id, which will be used to group
several tests. This can be useful to enable a specific group to be run. The
function will be searched later by the engine that will create the program that
...
TEST_SKIP               will skip the test. Useful to state that the test was
not yet implemented TEST_OUTPUT_SOURCELINE  outputs the source line. TEST_ABORT
aborts the current and also all the other test cases TEST_CANNOT_RUN states that
the test cannot run, maybe because it must be run exclusively

The test case must not assume any call order: it must be prepared to run totally
standalone. It may, tho, declare an unique dependency with another testcase.
This is done by adding, inside a comment in the same line of the TESTCASE
declaration, a #DEPENDS(function name). The dependencies cannot be recursive
(E.G.: func1 depends on func2 depends on func3 depends on func1).

The engine that creates the tc_tests.c file is located at
P:\TotalCrossVM\src\tests\java\GenTestCalls. Here's an example of how to run it,
assuming you're at the base folder from where the tests will be recursively
searched: java -classpath
p:\TotalCrossSDK\output\eclipse;P:\TotalCrossVM\src\tests\java GenTestCalls .
...
// Function prototypes
void testTimeCreate(struct TestSuite *tc);                //
P:\SuperWabaSDKPro\src\native\waba/SW_Time.c void testVmGetTimeStamp(struct
TestSuite *tc);            // P:\SuperWabaSDKPro\src\native\waba/SW_Vm.c void
testVmSetDeviceAutoOff(struct TestSuite *tc);        //
P:\SuperWabaSDKPro\src\native\waba/SW_Vm.c void testVmSetTimeStamp(struct
TestSuite *tc);            // P:\SuperWabaSDKPro\src\native\waba/SW_Vm.c -
depends on testTimeCreate void testVmSleep(struct TestSuite *tc); //
...
void startTestSuite(Context currentContext)
{
   ...
   int testsResults[TEST_COUNT] = { 0 }; // This flag vector indicates whether a
given test had failed testFunc tests[TEST_COUNT];
...
The tests are ordered by the name, so it is easy to run all tests inside a
single file; to do this, just change the 'from' and 'to' parameters that
controls the loop inside the startTestSuite function.

To enable the tests, you have to define the ENABLE_TEST_SUITE macro. This will
make the SWVM run the test suite instead of the SW program. The suite will run
and the VM will then quit. In the release version of the program, just undefine
the ENABLE_TEST_SUITE macro; this way nobody will call the test cases and they
will be stripped by the linker.

==================  MEMORY TESTS ===================

The memory test works like this: a command is issued to the TotalCross memory
manager instructing it to return null at the first xmalloc/heapAlloc. Once the
function correctly handles this situation, the counter is incremented and
another command is issued instructing the memory manager return null at the
second xmalloc/heapAlloc. And so on, until all allocations failed and succeeded.

The routines are have the MEMORY_TEST_START and MEMORY_TEST_END requires that:
1. These routines cannot have *return* statements; just replace that to a "goto
finish", and create a label "finish: ;" right before the MEMORY_TEST_END macro
call.
2. All local variable initializations must be placed AFTER the
MEMORY_TEST_START. This includes any assignment to null and other
initializations, otherwise after the first loop iteration the variables may be
...
#define TESTCASE(name)                                                         \
  void test_##name(struct TestSuite* tc, Context currentContext)

// Macros used within the testcases, to make the assertions
#define ASSERT1_EQUALS(type, v1)                                               \
  if (!tc->assertEquals##type(tc, v1, __FILE__, __LINE__))                     \
  goto finish
#define ASSERT2_EQUALS(type, v1, v2)                                           \
  if (!tc->assertEquals##type(tc, v1, v2, __FILE__, __LINE__))                 \
  goto finish
#define ASSERT3_EQUALS(type, v1, v2, v3)                                       \
  if (!tc->assertEquals##type(tc, v1, v2, v3, __FILE__, __LINE__))             \
  goto finish
#define ASSERT_BETWEEN(type, v1, v2, v3)                                       \
  if (!tc->assertBetween##type(tc, v1, v2, v3, __FILE__, __LINE__))            \
  goto finish
#define ASSERT_ABOVE(type, v1, v2)                                             \
  if (!tc->assertAbove##type(tc, v1, v2, __FILE__, __LINE__))                  \
  goto finish
...
#define TEST_SKIP                                                              \
  do {                                                                         \
    tc->output(tc, "%s (%d): test skipped!", (char*)__FILE__, (int)__LINE__);  \
    tc->skipped++;                                                             \
    goto finish;                                                               \
  } while (0) // remove warning of finish not defined
#define TEST_OUTPUT_SOURCELINE                                                 \
  tc->output(tc, "from %s (%d)\n", (char*)__FILE__, (int)__LINE__)
#define TEST_ABORT                                                             \
  do {                                                                         \
    tc->abort = true;                                                          \
    goto finish;                                                               \
  } while (0) // abort all tests
#define TEST_CANNOT_RUN                                                        \
  do {                                                                         \
    tc->output(                                                                \
      tc, "%s (%d): test cannot run!", (char*)__FILE__, (int)__LINE__);        \
    tc->cannotRun++;                                                           \
    goto finish;                                                               \
  } while (0)

// This function must be called to start the tests. It is called by the
// GenTestCalls on the automatically generated SWTests.c
void
startTestSuite(Context currentContext);
// This function presents the test results at the proper place; if the array
// testResults isn't NULL, then print which tests have failed by their number

void
showTestResults(int* testResults);
...
#define DECLARE_ASSERT_FUNC1(name, type1)                                      \
  typedef bool (*assert##name##Func)(                                          \
    struct TestSuite * tc, type1 v1, const char* file, int line);              \
  bool assert##name(struct TestSuite* tc, type1 v1, const char* file, int line);

#define DECLARE_ASSERT_FUNC2(name, type1, type2)                               \
  typedef bool (*assert##name##Func)(                                          \
    struct TestSuite * tc, type1 v1, type2 v2, const char* file, int line);    \
  bool assert##name(                                                           \
    struct TestSuite* tc, type1 v1, type2 v2, const char* file, int line);

#define DECLARE_ASSERT_FUNC3(name, type1, type2, type3)                        \
  typedef bool (*assert##name##Func)(struct TestSuite * tc,                    \
                                     type1 v1,                                 \
                                     type2 v2,                                 \
                                     type3 v3,                                 \
                                     const char* file,                         \
                                     int line);                                \
  bool assert##name(struct TestSuite* tc,                                      \
                    type1 v1,                                                  \
                    type2 v2,                                                  \
                    type3 v3,                                                  \
                    const char* file,                                          \
...
bool
fail(struct TestSuite* tc, char* msg, ...);
typedef void (*outputFunc)(struct TestSuite* tc, char*, ...);
void
output(struct TestSuite* tc, char* msg, ...);
...
// this function is called by the startTestSuite in order to assign the
// functions and setup the variables of the SWTestSuite structure.
struct TestSuite*
createTestSuite();
