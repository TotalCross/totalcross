...
#endif
static void _long2hex(int64 b, int32 places,
					  CharP outBuf) { // placed here to remove dependency of TotalCross for Litebase testcases
...
bool assertEqualsDbl(struct TestSuite* tc, double v1, double v2, const char* file, int line) {
	...
	if (!(diff <= tc->doubleError)) {
		...
		return fail(tc, "%s (%d): assertEqualsDbl(%s == %s) (0x%s == 0x%s) failed!\n", file, line,
					TCAPI_FUNC(double2str)(v1, -1, buf1), TCAPI_FUNC(double2str)(v2, -1, buf2), xbuf1, xbuf2);
	}
	return true;
}
bool assertEqualsSz(struct TestSuite* tc, char* v1, char* v2, const char* file, int line) {
	...
	if (!v1 || !v2) {
		return fail(tc, "%s (%d): assertEqualsSz(%x == %x) failed because %s!\n", file, line, (int)v1,
					(int)v2, (!v1 && !v2) ? "both are null" : "one is null");
	}
	for (; *b1 && *b2; i++, b1++, b2++)
		if (*b1 != *b2) {
	...
	if (*b1 || *b2) {
		return fail(tc,
					"%s (%d): assertEqualsSz(%s == %s) failed because the strings have different lengths!\n", file,
					line, v1, v2);
	}
	return true;
}
...
bool assertEqualsBlock(struct TestSuite* tc, void* v1, void* v2, int32 n, const char* file,
					   int line) {
	uint8* b1 = (uint8*)v1;
	uint8* b2 = (uint8*)v2;
	int32 i = 0;
	...
	for (; i < n; i++, b1++, b2++)
		if (*b1 != *b2) {
			return fail(tc, "%s (%d): assertEqualsBlock(%x == %x) failed at position %d (%d != %d)!\n", file,
						line, (int)v1, (int)v2, (int)i, (int) * b1, (int) * b2);
		}
	return true;
}
bool assertEqualsFilled(struct TestSuite* tc, void* v1, uint32 size, uint8 filledWith,
	...
	if (!v1) {
		return fail(tc, "%s (%d): assertEqualsFill(%x,%d,%d) failed because block is null!\n", file, line,
					(int)v1, (int)size, (int)filledWith);
	}
	for (; i < size; i++, b1++)
		if (*b1 != filledWith) {
			return fail(tc, "%s (%d): assertEqualsFill(%x,%d,%d) failed at position %d (%d != %d)!\n", file,
						line, (int)v1, (int)size, (int)filledWith, (int)i, (int) * b1, (int)filledWith);
		}
	return true;
}
...
bool assertBetweenI8(struct TestSuite* tc, int8    v1, int8    v2,  int8    v3, const char* file,
					 int line) {
	return (v1 <= v2
			&& v2 <= v3) ? true : fail(tc, "%s (%d): assertBetweenI8(%d <= %d <= %d) failed!\n", file, line,
									   (int)v1, (int)v2, (int)v3);
}
bool assertBetweenI16(struct TestSuite* tc, int16   v1, int16   v2,  int16   v3, const char* file,
					  int line) {
	return (v1 <= v2
			&& v2 <= v3) ? true : fail(tc, "%s (%d): assertBetweenI16(%d <= %d <= %d) failed!\n", file, line,
									   (int)v1, (int)v2, (int)v3);
}
bool assertBetweenI32(struct TestSuite* tc, int32   v1, int32   v2,  int32   v3, const char* file,
					  int line) {
	return (v1 <= v2
			&& v2 <= v3) ? true : fail(tc, "%s (%d): assertBetweenI32(%ld <= %ld <= %ld) failed!\n", file, line,
									   (long)v1, (long)v2, (long)v3);
}
bool assertBetweenI64(struct TestSuite* tc, int64   v1, int64   v2,  int64   v3, const char* file,
					  int line) {
	if (!(v1 <= v2 && v2 <= v3)) {
		...
		;
		;
		fail(tc, "%s (%d): assertBetweenI64(%s <= %s <= %s) failed!\n", file, line, TCAPI_FUNC(long2str)(v1,
				buf1), TCAPI_FUNC(long2str)(v2, buf2), TCAPI_FUNC(long2str)(v3, buf3));
		return false;
	}
	return true;
}
bool assertBetweenU8(struct TestSuite* tc, uint8   v1, uint8   v2,  uint8   v3, const char* file,
					 int line) {
	return (v1 <= v2
			&& v2 <= v3) ? true : fail(tc, "%s (%d): assertBetweenU8(%d <= %d <= %d) failed!\n", file, line,
									   (int)v1, (int)v2, (int)v3);
}
bool assertBetweenU16(struct TestSuite* tc, uint16  v1, uint16  v2,  uint16  v3, const char* file,
					  int line) {
	return (v1 <= v2
			&& v2 <= v3) ? true : fail(tc, "%s (%d): assertBetweenU16(%d <= %d <= %d) failed!\n", file, line,
									   (uint16)v1, (uint16)v2, (uint16)v3);
}
bool assertBetweenU32(struct TestSuite* tc, uint32  v1, uint32  v2,  uint32  v3, const char* file,
					  int line) {
	return (v1 <= v2
			&& v2 <= v3) ? true : fail(tc, "%s (%d): assertBetweenU32(%ld <= %ld <= %ld) failed!\n", file, line,
									   (long)v1, (long)v2, (long)v3);
}
bool assertBetweenDbl(struct TestSuite* tc, double  v1, double  v2,  double  v3, const char* file,
					  int line) {
	if (!(v1 <= v2 && v2 <= v3)) {
		DoubleBuf buf1, buf2, buf3;
		fail(tc, "%s (%d): assertBetweenDbl(%s <= %s <= %s) failed!\n", file, line,
			 TCAPI_FUNC(double2str)(v1, -1, buf1), TCAPI_FUNC(double2str)(v2, -1, buf2),
...
bool assertAboveDbl(struct TestSuite* tc, double  v1, double  v2, const char* file, int line) {
	if (!(v1 > v2)) {
		DoubleBuf buf1, buf2;
		fail(tc, "%s (%d): assertAboveDbl(%s > %s) failed!\n", file, line, TCAPI_FUNC(double2str)(v1, -1,
				buf1), TCAPI_FUNC(double2str)(v2, -1, buf2));
		return false;
	}
	return true;
}
...
static struct TestSuite
	swtc; // making this locally because this way we don't rely on memory allocation
...
void showTestResults(int* testResults) {
	...
	if (!swtc.abort) {
		xstrprintf(buf,
				   "%ld test total\n%ld tests skipped\n%ld cannot run\n%ld tests completed\n%ld succeeded\n%ld failed\n",
				   (long)swtc.total, (long)swtc.skipped, (long)swtc.cannotRun,
				   (long)(swtc.total + swtc.failed - swtc.skipped - swtc.cannotRun),
				   (long)(swtc.total - swtc.skipped - swtc.cannotRun), (long)swtc.failed);
