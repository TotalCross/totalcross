...
HRESULT Direct3DContentProvider::PrepareResources(_In_ const LARGE_INTEGER* presentTargetTime,
...
HRESULT Direct3DContentProvider::GetTexture(_In_ const DrawingSurfaceSizeF* size,
	...
	if (m_controller->renderer->updateScreenWaiting) {
		...
		m_controller->renderer->d3dImedContext->ExecuteCommandList(m_controller->renderer->d3dCommandList,
				FALSE);
