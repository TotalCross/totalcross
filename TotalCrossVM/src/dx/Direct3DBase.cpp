...
inline bool _DxFailed(HRESULT hr, /*const char *file, */int line) {
	if (FAILED(hr)) {
		debug("DX failed: %X (line %d)",
			  hr, /*file, */line);// Platform::Exception::ex = Platform::Exception::CreateException(hr); // Set a breakpoint on this line to catch Win32 API errors.
		return true;
	}
	return false;
}
...
void Direct3DBase::setBufAndLen(Platform::Array<byte>^ fileData, byte** buf, int* len,
								int completeValue) {
	*len = fileData->Length;
	*buf = new byte[*len];
...
void Direct3DBase::initialize() {
	...
	DX::ReadDataAsync("VertexShaderGlobalColor.cso").then([this](Platform::Array<byte>^ fileData) {
		setBufAndLen(fileData, &vs1buf, &vs1len, 1);
	});
	DX::ReadDataAsync("PixelShaderGlobalColor.cso").then([this](Platform::Array<byte>^ fileData) {
		setBufAndLen(fileData, &ps1buf, &ps1len, 2);
	});
	DX::ReadDataAsync("VertexShaderTexture.cso").then([this](Platform::Array<byte>^ fileData) {
		setBufAndLen(fileData, &vs2buf, &vs2len, 4);
	});
	DX::ReadDataAsync("PixelShaderTexture.cso").then([this](Platform::Array<byte>^ fileData) {
		setBufAndLen(fileData, &ps2buf, &ps2len, 8);
	});
	DX::ReadDataAsync("VertexShaderLocalColor.cso").then([this](Platform::Array<byte>^ fileData) {
		setBufAndLen(fileData, &vs3buf, &vs3len, 16);
	});
	DX::ReadDataAsync("PixelShaderLocalColor.cso").then([this](Platform::Array<byte>^ fileData) {
...
void Direct3DBase::updateDevice(IDrawingSurfaceRuntimeHostNative* host) {
	...
	if (DxFailed(D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, creationFlags,
								   featureLevels, ARRAYSIZE(featureLevels), D3D11_SDK_VERSION, &d3dDevice, &m_featureLevel,
								   &d3dImedContext))) {
		return;
	}
	...
	CD3D11_TEXTURE2D_DESC renderTargetDesc(DXGI_FORMAT_B8G8R8A8_UNORM, appSize, appSize, 1, 1,
	...
	if (DxFailed(d3dDevice->CreateTexture2D(&renderTargetDesc, nullptr, &renderTex))) {
		return;
	}
	if (DxFailed(d3dDevice->CreateRenderTargetView(renderTex, nullptr, &renderTexView))) {
		return;
	}
	host->CreateSynchronizedTexture(renderTex, &syncTex);

	// Create a depth stencil view.
	CD3D11_TEXTURE2D_DESC depthStencilDesc(DXGI_FORMAT_D24_UNORM_S8_UINT, appSize, appSize, 1, 1,
										   D3D11_BIND_DEPTH_STENCIL);
	if (DxFailed(d3dDevice->CreateTexture2D(&depthStencilDesc, nullptr, &depthStencil))) {
		return;
	}
	CD3D11_DEPTH_STENCIL_VIEW_DESC depthStencilViewDesc(D3D11_DSV_DIMENSION_TEXTURE2D);
	if (DxFailed(d3dDevice->CreateDepthStencilView(depthStencil, &depthStencilViewDesc,
				 &depthStencilView))) {
		return;
	}
	...
	if (DxFailed(d3dDevice->CreateBuffer(&indexBufferDesc, &indexBufferData, &indexBuffer))) {
		return;
	}
	...
	if (DxFailed(d3dDevice->CreateDepthStencilState(&depthDisabledStencilDesc,
	...
	if (DxFailed(d3dDevice->CreateBlendState(&blendStateDescription, &pBlendState))) {
		return;
	}
	...
	buf = vs1buf;
	len = vs1len;
	if (DxFailed(d3dDevice->CreateVertexShader(buf, len, nullptr, &vertexShader))) {
		return;
	}
	const D3D11_INPUT_ELEMENT_DESC vertexDesc1[] = {
		{ "POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	};
	if (DxFailed(d3dDevice->CreateInputLayout(vertexDesc1, ARRAYSIZE(vertexDesc1), buf, len,
				 &inputLayout))) {
		return;
	}
	...
	if (DxFailed(d3dDevice->CreateBuffer(&constantBufferDesc, nullptr, &constantBuffer))) {
		return;
	}

	buf = vs2buf;
	len = vs2len;
	if (DxFailed(d3dDevice->CreateVertexShader(buf, len, nullptr, &vertexShaderT))) {
		return;
	}
	...
	if (DxFailed(d3dDevice->CreateInputLayout(vertexDesc2, ARRAYSIZE(vertexDesc2), buf, len,
				 &inputLayoutT))) {
		return;
	}
	...
	buf = ps2buf;
	len = ps2len;
	if (DxFailed(d3dDevice->CreatePixelShader(buf, len, nullptr, &pixelShaderT))) {
		return;
	}

	buf = vs3buf;
	len = vs3len;
	if (DxFailed(d3dDevice->CreateVertexShader(buf, len, nullptr, &vertexShaderLC))) {
		return;
	}
	...
	if (DxFailed(d3dDevice->CreateInputLayout(vertexDesc3, ARRAYSIZE(vertexDesc3), buf, len,
				 &inputLayoutLC))) {
		return;
	}
	...
	buf = ps3buf;
	len = ps3len;
	if (DxFailed(d3dDevice->CreatePixelShader(buf, len, nullptr, &pixelShaderLC))) {
		return;
	}
	...
	eventQueuePush(PENEVENT_PEN_DOWN, 0, 0, -1,
				   -1);  // fixes black screen when resuming from wp8 if the screen was not pressed before
}
...
void Direct3DBase::fillShadedRect(TCObject g, int32 x, int32 y, int32 w, int32 h, PixelConv c1,
								  PixelConv c2, bool horiz) {
	if (minimized) {
		return;
	}
	y += glShiftY;
...
void Direct3DBase::drawLines(Context currentContext, TCObject g, int32* xx, int32* yy, int32 count,
							 int32 tx, int32 ty, int color, bool fill) {
	if (minimized) {
		return;
	}
	int i;
	VertexPosition* vertices = (VertexPosition*) xmalloc(sizeof(VertexPosition) * count),
	...
	if (count > lastLinesCount) {
		...
		if (DxFailed(d3dDevice->CreateBuffer(&indexBufferDesc, &indexBufferData, &linesIndexBuffer))) {
			return;
		}
		xfree(indexes);
	...
	d3dcontext->IASetPrimitiveTopology(fill ? D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST :
...
void Direct3DBase::drawPixelColors(int32* xx, int32* yy, PixelConv* colors, int32 count) {
	...
	int i, n = count * 2;
	VertexPositionColor* vertices = (VertexPositionColor*)xmalloc(sizeof(VertexPositionColor) * n),
						 *v = vertices;// position, color
	for (i = count; --i >= 0;) {
		float x = (float) * xx++;
		float y = (float) * yy++;
	...
	d3dcontext->IASetPrimitiveTopology(
		D3D10_PRIMITIVE_TOPOLOGY_LINELIST); // POINTLIST results in points being drawn in a slightly different position
...
void Direct3DBase::drawPixels(float* glXYA, int count, int color) {
	...
	XMFLOAT4 cor = XMFLOAT4(f255[(color >> 16) & 0xFF], f255[(color >> 8) & 0xFF], f255[color & 0xFF],
							0);
	...
	D3D11_MAPPED_SUBRESOURCE ms;
	if (DxFailed(d3dcontext->Map(pBufferPixels, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &ms))) {
...
bool Direct3DBase::loadTexture(Context currentContext, TCObject img, int32* textureId,
							   Pixel* pixels,
	...
	textureSubresourceData.SysMemPitch = textureDesc.Width * (onlyAlpha ? 1 :
										 4); // Specify the size of a row in bytes
	if (FAILED(d3dDevice->CreateTexture2D(&textureDesc, &textureSubresourceData, &texture))) {
		ret = false;
		throwException(currentContext, OutOfMemoryError, "Out of texture memory for image with %dx%d",
					   width, height);
	} else {
...
int lastTexView;
void Direct3DBase::drawTexture(int32* textureId, int32 x, int32 y, int32 w, int32 h, int32 dstX,
							   int32 dstY, int32 dstW, int32 dstH, int32 imgW, int32 imgH, PixelConv* color, int32 alphaMask) {
	...
	float left = (float)x / (float)imgW, top = (float)y / (float)imgH,
		  right = (float)(x + w) / (float)imgW, bottom = (float)(y + h) / (float)imgH; // 0,0,1,1
...
void Direct3DBase::getPixels(Pixel* dstPixels, int32 srcX, int32 srcY, int32 width, int32 height,
							 int32 pitch) {
	if (minimized) {
		return;
	}
