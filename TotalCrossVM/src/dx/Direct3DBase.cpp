...
void Direct3DBase::updateDevice(IDrawingSurfaceRuntimeHostNative* host) {
	...
	if (DxFailed(d3dDevice->CreateDepthStencilView(depthStencil, &depthStencilViewDesc, &depthStencilView))) {
		return;
	}
	...
	if (DxFailed(d3dDevice->CreateDepthStencilState(&depthDisabledStencilDesc, &depthDisabledStencilState))) {
		return;
	}
	...
	if (DxFailed(d3dDevice->CreateInputLayout(vertexDesc1, ARRAYSIZE(vertexDesc1), buf, len, &inputLayout))) {
		return;
	}
	...
	if (DxFailed(d3dDevice->CreateInputLayout(vertexDesc2, ARRAYSIZE(vertexDesc2), buf, len, &inputLayoutT))) {
		return;
	}
	...
	if (DxFailed(d3dDevice->CreateInputLayout(vertexDesc3, ARRAYSIZE(vertexDesc3), buf, len, &inputLayoutLC))) {
		return;
	}
...
bool Direct3DBase::loadTexture(Context currentContext, TCObject img, int32* textureId, Pixel *pixels, int32 width, int32 height, bool onlyAlpha) {
	...
	if (!onlyAlpha) {
		for (i = width*height; --i >= 0; pf++) {
			ptemp.pixel = pf->pixel; pf->a = ptemp.r; pf->b = ptemp.g; pf->g = ptemp.b; pf->r = ptemp.a;
		}
	}
