...
Direct3DBackground::Direct3DBackground(CSwrapper ^ _cs)
  : cs(_cs)
{
  instance = this;
}
...
void
Direct3DBackground::OnPointerPressed(int x, int y)
{
  eventQueuePush(PENEVENT_PEN_DOWN, 0, x, y - glShiftY, -1);
}

void
Direct3DBackground::OnPointerMoved(int x, int y)
{
...
void
Direct3DBackground::OnPointerReleased(int x, int y)
{
...
void
Direct3DBackground::OnKeyPressed(int key)
{
  eventQueuePush(key < 32 ? KEYEVENT_SPECIALKEY_PRESS : KEYEVENT_KEY_PRESS,
                 key < 32 ? keyDevice2Portable(key) : key,
                 0,
                 0,
                 -1);
}
...
void
Direct3DBackground::OnManipulation(int type, double delta)
{
  int* pd = (int*)&delta;
  if (!renderer->updateScreenWaiting) { // boosts performance during drag
...
void
Direct3DBackground::OnScreenChanged(int newKeyboardH,
                                    int newWidth,
                                    int newHeight)
{
  if (newKeyboardH >= 0) {
...
void
Direct3DBackground::lifeCycle(bool suspending)
{
...
HRESULT
Direct3DBackground::PrepareResources(_Out_ BOOL* contentDirty)
{
...
void
Direct3DBackground::RequestNewFrame()
{
