...
{
  internal : Direct3DBase(PhoneDirect3DXamlAppComponent::CSwrapper ^ _cs);
  ...
  bool loadTexture(Context currentContext,
                   TCObject img,
                   int32* textureId,
                   Pixel* pixels,
                   int32 width,
                   int32 height,
                   bool onlyAlpha);
  void drawTexture(int32* textureId,
                   int32 x,
                   int32 y,
                   int32 w,
                   int32 h,
                   ...
                   PixelConv* color,
                   int32 alphaMask);
  void drawLines(Context currentContext,
                 TCObject g,
                 int32* x,
                 int32* y,
                 int32 n,
  ...
  void fillShadedRect(TCObject g,
                      int32 x,
                      int32 y,
                      int32 w,
                      int32 h,
  ...
  void createTexture();
  void getPixels(Pixel* dstPixels,
                 int32 srcX,
                 int32 srcY,
                 int32 width,
                 int32 height,
...
private:
  void initialize();
  void setBufAndLen(Platform::Array<byte> ^ fileData,
                    byte** buf,
                    int* len,
                    int completeValue);
  int loadCompleted;
  ...
  ID3D11Buffer *pBufferRect, *pBufferPixels, *pBufferLines, *pBufferColor,
    *texVertexBuffer, *pBufferRectLC;
