...
static int process_client_key_xchg(SSL *ssl, uint8_t *buf, int hs_len); /* fdie@ could be called recursively */
...
int do_svr_handshake(SSL *ssl, int handshake_type, uint8_t *buf, int hs_len)
{
    ...
    switch (handshake_type)
    {
        ...
        case HS_CLIENT_KEY_XCHG:
            /* fdie@ could be called as a second message, be sure to advance
             * in the buffer (JSSE optimization)
             */
            ret = process_client_key_xchg(ssl, buf, hs_len);
            break;
 ...
 * ciphers.
 */
static int process_client_key_xchg(SSL *ssl, uint8_t *buf, int hs_len)
{
    /* uint8_t *buf = ssl->bm_data; //fdie@ use the progressive buffer to be able to process a second part in a single message (JSSE optimization) */
    int pkt_size = ssl->bm_index;
...
#ifdef CONFIG_SSL_CERT_VERIFICATION
  /* fdie@ ask for RSA encrypted certificates */
  static const uint8_t g_cert_request[] = { HS_CERT_REQ, 0, 0, 4, 1, 1, 0, 0 };
