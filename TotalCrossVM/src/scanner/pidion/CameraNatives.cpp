...
bool CameraLibOpen(OpenParams params) {
	...
	_BBCameraPreviewStart   = (ProcBBCameraPreviewStart) GetProcAddress(apidll,
							  TEXT("BBCameraPreviewStart"));
	_BBCameraPreviewStop	   = (ProcBBCameraPreviewStop) GetProcAddress(apidll,
								 TEXT("BBCameraPreviewStop"));
	_BBCameraCapture    	   = (ProcBBCameraCapture) GetProcAddress(apidll, TEXT("BBCameraCapture"));
	_BBCameraPreviewZoom	   = (ProcBBCameraPreviewZoom) GetProcAddress(apidll,
								 TEXT("BBCameraPreviewZoom"));
...
//////////////////////////////////////////////////////////////////////////
SCAN_API void pC_setPreview_b(NMParams
							  p) { // pidion/Camera native public void setPreview(boolean on);
	TCObject obj = p->obj[0];
	int32 on = p->i32[0];
	DWORD ret;
	if (on && handle == 0) {
