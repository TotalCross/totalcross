...
LibOpen(OpenParams params)
{
  ...
  isOk =
    (hService = CreateFile(ServiceName,
    ...
    RegisterHotKey(
      getMainWindowHandle(), OEM_VK_BCR_TRIGGER, MOD_KEYUP, OEM_VK_BCR_TRIGGER);
...
disableKeyboardHook()
{
  ...
  DeviceIoControl(hService,
                  BCR_IOCTL_UPDATE_SETTING,
                  &status,
                  sizeof(status),
                  &Result,
                  sizeof(DWORD),
...
static bool
scannerActivate(bool activate)
{
  if (isOk) {
    DWORD Result;
    status.dwMask = MASK_MODULE_ENABLE;
    status.dwModuleEnable =
      activate ? BCRSTATUS_MOD_ENABLE : BCRSTATUS_MOD_DISABLE;
    DeviceIoControl(hService,
                    BCR_IOCTL_UPDATE_SETTING,
                    &status,
                    sizeof(status),
                    ...
                    NULL,
                    NULL);
    if (Result == BCR_SUCCESS &&
        hScanEvent == INVALID_HANDLE_VALUE) { // get the event name
      if (DeviceIoControl(hService,
                          BCR_IOCTL_GET_EVENTNAME,
                          NULL,
                          0,
                          szScanEvent,
                          sizeof(szScanEvent),
        ...
        hReadGoodThread =
          CreateThread(NULL, 0, &ScanThread, NULL, 0, &dwThreadId);
...
HandleEvent(VoidP eventP)
{
  ...
  if (we->msg == WM_HOTKEY && we->wParam == OEM_VK_BCR_TRIGGER) {
    ...
    DeviceIoControl(hService,
                    BCR_IOCTL_TRIGGER,
                    HIBYTE(nState) ? &BCR_PRESS : &BCR_RELEASE,
                    sizeof(DWORD),
                    &Result,
                    sizeof(DWORD),
                    &Result1,
...
{ // totalcross/io/device/scanner/Scanner native public static
  ...
  if (isOk) {
    DeviceIoControl(hService,
                    ...
                    null,
                    null);
    p->retO = createStringObjectFromCharP(
      p->currentContext, status.szModuleVersion, -1);
    setObjectLock(p->retO, UNLOCKED);
  }
}
//////////////////////////////////////////////////////////////////////////
