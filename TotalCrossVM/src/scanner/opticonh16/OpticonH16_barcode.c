...
SCAN_API int32 LibOpen(OpenParams params) {
	...
	isOk = (hService = CreateFile(ServiceName, GENERIC_READ | GENERIC_WRITE, 0, null, OPEN_EXISTING, 0,
...
static bool disableKeyboardHook() {
	...
	DeviceIoControl(hService, BCR_IOCTL_UPDATE_SETTING, &status, sizeof(status), &Result, sizeof(DWORD),
					NULL, NULL);
...
static bool scannerActivate(bool activate) {
	if (isOk) {
		...
		if (Result == BCR_SUCCESS && hScanEvent == INVALID_HANDLE_VALUE) { // get the event name
			if (DeviceIoControl(hService, BCR_IOCTL_GET_EVENTNAME, NULL, 0, szScanEvent, sizeof(szScanEvent),
								&Result, NULL) &&
...
SCAN_API bool HandleEvent(VoidP eventP) {
	...
	if (we->msg == WM_HOTKEY && we->wParam == OEM_VK_BCR_TRIGGER) {
		...
		DeviceIoControl(hService, BCR_IOCTL_TRIGGER, HIBYTE(nState) ? &BCR_PRESS : &BCR_RELEASE,
						sizeof(DWORD), &Result, sizeof(DWORD), &Result1, NULL);
	}
	return false;
}
