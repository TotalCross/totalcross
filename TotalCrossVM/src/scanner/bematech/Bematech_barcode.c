...
LibOpen(OpenParams params)
{
  ...
  ZBCRGetLastNotifyEventP = (ZBCRGetLastNotifyEventProc)GetProcAddress(
    bematechScanDll, TEXT("ZBCRGetLastNotifyEvent"));
  ZBCRSetPowerP =
    (ZBCRSetPowerProc)GetProcAddress(bematechScanDll, TEXT("ZBCRSetPower"));
  ZBCRGetLastBarcodeP = (ZBCRGetLastBarcodeProc)GetProcAddress(
    bematechScanDll, TEXT("ZBCRGetLastBarcode"));
...
{                         // totalcross/io/device/scanner/Scanner native
  ...
  TCHAR lastBarcode[512]; // arbitrary size of 512, hopefully nobody will ever
                          // read a barcode with more than 512 characters
