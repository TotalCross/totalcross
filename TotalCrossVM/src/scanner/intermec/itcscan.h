 ...
 *
 *  COPYRIGHT (c) 1998-2003 INTERMEC TECHNOLOGIES CORPORATION, ALL RIGHTS
 *RESERVED
 *
 ******************************************************************************/
...
#define ITCSCAN_ITC50_NOT_AVAIL_ERROR                                          \
  L"Error string retrieval failed for itc50.dll is not loaded."
...
#define ITCSCAN_DEVICES_REG_KEY L"SOFTWARE\\Intermec\\ADCDevices"
  ...
  HRESULT ITCSCAN_SyncReadEx(INT32 pHandle,
                             READ_DATA_STRUCT_EX* pReadDataBlock);
  HRESULT ITCSCAN_SyncReadWSid(INT32 pHandle,
                               READ_DATA_STRUCT* pReadDataBlock,
                               ITCSCAN_SID* pSid);
  HRESULT ITCSCAN_CancelRead(INT32 pHandle,
                             BOOL FlushBufferedData,
                             DWORD* pdwTotalDiscardedMessages,
                             DWORD* pdwTotalDiscardedBytes);
  HRESULT ITCSCAN_GetAttribute(INT32 pHandle,
                               int eAttribID,
                               BYTE rgbBuffer[],
                               DWORD dwBufferSize,
                               DWORD* pnBufferData);
  HRESULT ITCSCAN_SetAttribute(INT32 pHandle,
                               int eAttribID,
                               BYTE rgbData[],
                               DWORD dwBufferSize);
  ...
  HRESULT ITCSCAN_GetScannerFilterGrid(INT32 pHandle,
                                       LPTSTR pszGrid,
                                       INT32* Length);
  ...
  LONG ITCSCAN_GetDevice(DWORD dwIndex,
                         LPTSTR deviceNameBuffer,
                         DWORD* pBufferLength);
  LONG ITCSCAN_GetDefaultDevice(LPTSTR lpDeviceNameBuffer,
                                DWORD* pBufferLength);
  void ITCSCAN_GetErrorString(HRESULT errorCode, TCHAR* pErrorString);
  HRESULT ITCSCAN_GetBarcodeConfiguration(TCHAR* apScannerConnection,
                                          TCHAR* apCfgStr,
                                          TCHAR* apCfgValue,
                                          INT32 aMaxLen);
  HRESULT ITCSCAN_SetBarcodeConfiguration(TCHAR* apScannerConnection,
                                          TCHAR* apCfgStr);
  HRESULT ITCSCAN_SetBarcodeConfigurationFromFile(TCHAR* apCfgStr);
  HRESULT ITCSCAN_GetBarcodeMessage(INT32 pHandle,
                                    int* pMsgType,
                                    INT32* pMsgData);
  ...
  HRESULT ITCSCAN_GetDeviceDetails(INT32 pHandle,
                                   DWORD hDevice,
