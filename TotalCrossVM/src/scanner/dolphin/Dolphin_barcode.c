...
typedef Result_t (__stdcall* procDecWaitForDecode)(DWORD dwTime, BYTE* pchMessage, BYTE* pchCodeID,
		BYTE* pchSymLetter, BYTE* pchSymModifier, WORD* pnLength, BOOL (*fpCallBack)(void));
...
static bool isScanKeyDown() {
	return GetAsyncKeyState(SCAN_KEY) < 0 || GetAsyncKeyState(193) < 0 || GetAsyncKeyState(195) < 0
		   || GetAsyncKeyState(194) < 0; // last three are for dolphin 6510
}
...
LRESULT ScanMonitorThread() {
	...
	while (running) {
		...
		if (!scanning && isScanKeyDown()) {
			...
			nResult = decWaitForDecodeProc(500, (BYTE*) barcode, (BYTE*) &cCodeID, (BYTE*) &cSymLetter,
										   (BYTE*) &cSymModifier, &uBarcodeLen, CheckOnSCAN);
			if (nResult == RESULT_SUCCESS) {
				if (cCodeID == SYMID_EAN13 || cCodeID == SYMID_EAN8) {
					...
					checkDigit %= 10;
					if (checkDigit ==
						0) { //flsobral@tc114_24: fixed check digit generation for barcodes which weighted value mod is 0.
...
SCAN_API int32 LibOpen(OpenParams params) {
	...
	if ((decoderDll = LoadLibrary(TEXT("decoder.dll"))) != null) {
		decWaitForDecodeProc          = (procDecWaitForDecode)   GetProcAddress(decoderDll,
										TEXT("decWaitForDecode"));
		decConnectProc                = (procDecConnect)         GetProcAddress(decoderDll,
										TEXT("decConnect"));
		decDisconnectProc             = (procDecDisconnect)      GetProcAddress(decoderDll,
										TEXT("decDisconnect"));
		decGetDecoderRevisionProc     = (procDecGetDecoderRevision) GetProcAddress(decoderDll,
										TEXT("decGetDecoderRevision"));
		decEnableDisableSymbologyProc = (procDecEnableDisableSymbology) GetProcAddress(decoderDll,
										TEXT("decEnableDisableSymbology"));
	}
	return decWaitForDecodeProc && decConnectProc && decDisconnectProc && decGetDecoderRevisionProc
		   && decEnableDisableSymbologyProc;
}
...
SCAN_API void tidsS_scannerActivate(NMParams
	...
	Err err;
	if ((hThread = CreateThread(null, 0, (LPTHREAD_START_ROUTINE) ScanMonitorThread, null, 0,
								null)) == null) {
