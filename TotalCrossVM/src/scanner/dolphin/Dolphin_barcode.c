...
typedef enum
{
  ...
  RESULT_ERR_UNSUPPORTED_IQ_BARCODE  // Scanned barcode is not a valid IQ host
                                     // barcode
} Result_t;

typedef Result_t(__stdcall* procDecWaitForDecode)(DWORD dwTime,
                                                  BYTE* pchMessage,
                                                  BYTE* pchCodeID,
                                                  BYTE* pchSymLetter,
                                                  BYTE* pchSymModifier,
...
typedef Result_t(__stdcall* procDecEnableDisableSymbology)(int nSymId,
                                                           BOOL bEnable);
...
isScanKeyDown()
{
  return GetAsyncKeyState(SCAN_KEY) < 0 || GetAsyncKeyState(193) < 0 ||
         GetAsyncKeyState(195) < 0 ||
...
ScanMonitorThread()
{
  ...
  while (running) {
    ...
    if (!scanning && isScanKeyDown()) {
      ...
      nResult = decWaitForDecodeProc(500,
                                     (BYTE*)barcode,
                                     (BYTE*)&cCodeID,
                                     (BYTE*)&cSymLetter,
                                     (BYTE*)&cSymModifier,
      ...
      if (nResult == RESULT_SUCCESS) {
        if (cCodeID == SYMID_EAN13 || cCodeID == SYMID_EAN8) {
          ...
          checkDigit %= 10;
          if (checkDigit ==
              0) { // flsobral@tc114_24: fixed check digit generation for
                   // barcodes which weighted value mod is 0.
        ...
        // clear read-ahead - guich@tc330 - removed this because the scanner
        // will keep on while the user is looking at a barcode while
        // (decWaitForDecodeProc(50, (BYTE*) barcode2, (BYTE*) &cCodeID, (BYTE*)
        // &cSymLetter, (BYTE*) &cSymModifier, &uBarcodeLen, 0) ==
        // RESULT_SUCCESS);
...
LibOpen(OpenParams params)
{
  ...
  if ((decoderDll = LoadLibrary(TEXT("decoder.dll"))) != null) {
    decWaitForDecodeProc = (procDecWaitForDecode)GetProcAddress(
      decoderDll, TEXT("decWaitForDecode"));
    decConnectProc =
      (procDecConnect)GetProcAddress(decoderDll, TEXT("decConnect"));
    decDisconnectProc =
      (procDecDisconnect)GetProcAddress(decoderDll, TEXT("decDisconnect"));
    decGetDecoderRevisionProc = (procDecGetDecoderRevision)GetProcAddress(
      decoderDll, TEXT("decGetDecoderRevision"));
    decEnableDisableSymbologyProc =
      (procDecEnableDisableSymbology)GetProcAddress(
        decoderDll, TEXT("decEnableDisableSymbology"));
  }
  return decWaitForDecodeProc && decConnectProc && decDisconnectProc &&
         decGetDecoderRevisionProc && decEnableDisableSymbologyProc;
}
...
{ // totalcross/io/device/scanner/Scanner native public static
  ...
  if ((hThread = CreateThread(
         null, 0, (LPTHREAD_START_ROUTINE)ScanMonitorThread, null, 0, null)) ==
      null) {
...
{ // totalcross/io/device/scanner/Scanner native public static
  ...
  if ((err = decEnableDisableSymbologyProc(barcodeType, enable)) !=
      RESULT_SUCCESS) {
    p->retI = false; // throw exception on error?
  } else {
    p->retI = true;
...
{ // totalcross/io/device/scanner/Scanner native public static
  ...
  } else if ((p->retO = createStringObjectFromTCHAR(
                p->currentContext, buf, -1)) != null) {
    setObjectLock(p->retO, UNLOCKED);
  }
}
//////////////////////////////////////////////////////////////////////////
