...
typedef imgResult(__stdcall* procimgInitCamera)(LPWSTR lpFileName,
...
typedef imgResult(__stdcall* procimgCaptureImage)(HDIB hdib,
                                                  imgCaptureType CaptureType);
typedef imgResult(__stdcall* procimgDisplayImage)(HDIB hdib,
                                                  HDC hdcDest,
                                                  RECT* pDestRect,
                                                  bool stretch,
                                                  int zoom,
...
typedef imgResult(__stdcall* procimgHistogramStretchEx)(HDIB hdib,
                                                        int low,
                                                        int high,
                                                        int mode);
typedef imgResult(__stdcall* procimgGammaCorrection)(HDIB hdib, double gamma);
typedef imgResult(__stdcall* procimgSaveJPG)(HDIB hdib,
                                             LPTSTR szFile,
                                             int quality);
...
{
  if (imgapiDll == null &&
      (imgapiDll = LoadLibrary(TEXT("imgapi.dll"))) != null) {
    imgGetIllumination = (procimgGetIllumination)GetProcAddress(
      imgapiDll, TEXT("imgGetIllumination"));
    imgGetAimer =
      (procimgGetAimer)GetProcAddress(imgapiDll, TEXT("imgGetAimer"));
    imgCreateHDIB =
      (procimgCreateHDIB)GetProcAddress(imgapiDll, TEXT("imgCreateHDIB"));
    imgInitCamera =
      (procimgInitCamera)GetProcAddress(imgapiDll, TEXT("imgInitCamera"));
    imgUninitCamera =
      (procimgUninitCamera)GetProcAddress(imgapiDll, TEXT("imgUninitCamera"));
    imgDisplayImage =
      (procimgDisplayImage)GetProcAddress(imgapiDll, TEXT("imgDisplayImage"));
    imgStopCapture =
      (procimgStopCapture)GetProcAddress(imgapiDll, TEXT("imgStopCapture"));
    imgCaptureImage =
      (procimgCaptureImage)GetProcAddress(imgapiDll, TEXT("imgCaptureImage"));
    imgStartCapture =
      (procimgStartCapture)GetProcAddress(imgapiDll, TEXT("imgStartCapture"));
    ...
    imgBitCount =
      (procimgBitCount)GetProcAddress(imgapiDll, TEXT("imgBitCount"));
    imgNumColors =
      (procimgNumColors)GetProcAddress(imgapiDll, TEXT("imgNumColors"));
    imgSetAimer =
      (procimgSetAimer)GetProcAddress(imgapiDll, TEXT("imgSetAimer"));
    imgSetIllumination = (procimgSetIllumination)GetProcAddress(
      imgapiDll, TEXT("imgSetIllumination"));
    imgHistogramStretchEx = (procimgHistogramStretchEx)GetProcAddress(
      imgapiDll, TEXT("imgHistogramStretchEx"));
    imgGammaCorrection = (procimgGammaCorrection)GetProcAddress(
      imgapiDll, TEXT("imgGammaCorrection"));
  }
  return imgGammaCorrection && imgGetAimer && imgCreateHDIB && imgInitCamera &&
         imgUninitCamera && imgDisplayImage && imgStopCapture &&
         imgCaptureImage && imgStartCapture && imgSaveJPG && imgWidth &&
         imgHeight && imgBitCount && imgNumColors && imgSetAimer &&
         imgSetIllumination && imgHistogramStretchEx && imgGammaCorrection;
}
...
SCAN_API TCHAR*
showDolphinCamera(TCHAR* path, TCHAR* file, bool lowRes)
{
  MSG msg;
  saved = false;
...
initInstance()
{
  ...
  g_hInst = GetModuleHandle(
    TEXT("Dolphin.dll")); // Store instance handle in our global variable
...
updateStatus()
{
  ...
  wsprintf(szBuffer,
           TEXT("Image Information: \nPixel width:%i\tPixel height:%i\nBits "
                "per pixel:%i\tNumber of colors:%i"),
           imgWidth(g_hDib),
           imgHeight(g_hDib),
           imgBitCount(g_hDib),
           imgNumColors(g_hDib));
...
WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (message) {
    case WM_COMMAND: {
      ...
      switch (wmId) {
        ...
        case ID_OPTIONS_HIGHRES: {
          ...
          CheckMenuItem(CommandBar_GetMenu(g_hWndCommandBar, 0),
                        gHighRes ? ID_OPTIONS_HIGHRES : ID_OPTIONS_LOWRES,
        ...
        case ID_OPTIONS_ENABLEAIMER: {
          if (CheckMenuItem(CommandBar_GetMenu(g_hWndCommandBar, 0),
                            ID_OPTIONS_ENABLEAIMER,
        ...
        case ID_OPTIONS_ENABLEHISTOGRAM: {
          /* Dynamically adjust the image contrast by performing a linear
           * histogram stretch. */
        ...
        case ID_OPTIONS_INCGAMMA: {
          /* Corrects the brightness levels in an image. On a computer display,
          a small change in brightness at a low brightness level is not equal to
    ...
    case WM_CREATE: {
      ...
      hwndEdit = CreateWindow(TEXT("static"),
                              NULL,
                              WS_CHILD | WS_VISIBLE,
                              0,
                              240,
    ...
    case WM_KEYDOWN: {
      switch (wParam) {
        case 0x2A: { /* 0x2A=ON/SCAN */
          bool resshown = false;
          ///* Need to be careful, its possible that key presses have stored up
          ///and
          ...
          while (GetAsyncKeyState(0x2A) < 0) {
            if (imgCaptureImage(g_hDib, gHighRes ? imgFull : imgPreview) !=
                imgOk) {
              break; // unexpected
            }
