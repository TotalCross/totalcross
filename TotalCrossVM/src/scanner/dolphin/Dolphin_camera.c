...
typedef imgResult(__stdcall* procimgInitCamera)(LPWSTR lpFileName, LPWSTR lpProfileName,
		DWORD* pdwFlags);
...
typedef imgResult(__stdcall* procimgDisplayImage)(HDIB hdib, HDC hdcDest, RECT* pDestRect,
		bool stretch, int zoom, bool center);
...
bool ImagingLibOpen() {
	if (imgapiDll == null && (imgapiDll = LoadLibrary(TEXT("imgapi.dll"))) != null) {
		...
		imgHistogramStretchEx = (procimgHistogramStretchEx) GetProcAddress(imgapiDll,
								TEXT("imgHistogramStretchEx"));
		imgGammaCorrection = (procimgGammaCorrection) GetProcAddress(imgapiDll, TEXT("imgGammaCorrection"));
	}
	return imgGammaCorrection && imgGetAimer && imgCreateHDIB && imgInitCamera && imgUninitCamera
		   && imgDisplayImage && imgStopCapture && imgCaptureImage && imgStartCapture && imgSaveJPG
		   && imgWidth && imgHeight && imgBitCount && imgNumColors && imgSetAimer && imgSetIllumination
		   && imgHistogramStretchEx && imgGammaCorrection;
}
...
static void updateStatus() {
	TCHAR szBuffer[256];
	wsprintf(szBuffer,
			 TEXT("Image Information: \nPixel width:%i\tPixel height:%i\nBits per pixel:%i\tNumber of colors:%i"),
			 imgWidth(g_hDib), imgHeight(g_hDib), imgBitCount(g_hDib), imgNumColors(g_hDib));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
			...
			switch (wmId) {
				...
				case ID_OPTIONS_HIGHRES: {
					...
					CheckMenuItem(CommandBar_GetMenu(g_hWndCommandBar, 0),
								  gHighRes ? ID_OPTIONS_HIGHRES : ID_OPTIONS_LOWRES, MF_CHECKED | MF_BYCOMMAND);
					CheckMenuItem(CommandBar_GetMenu(g_hWndCommandBar, 0),
								  !gHighRes ? ID_OPTIONS_HIGHRES : ID_OPTIONS_LOWRES, MF_UNCHECKED | MF_BYCOMMAND);
					break;
				}
				case ID_OPTIONS_ENABLEAIMER: {
					if (CheckMenuItem(CommandBar_GetMenu(g_hWndCommandBar, 0), ID_OPTIONS_ENABLEAIMER,
									  MF_BYCOMMAND) == MF_CHECKED) {
						// UnCheck the item
						CheckMenuItem(CommandBar_GetMenu(g_hWndCommandBar, 0), ID_OPTIONS_ENABLEAIMER,
				...
				case ID_OPTIONS_ENABLEILLUMINATION: {
					if (CheckMenuItem(CommandBar_GetMenu(g_hWndCommandBar, 0), ID_OPTIONS_ENABLEILLUMINATION,
									  MF_BYCOMMAND) == MF_CHECKED) {
						// UnCheck the item
						CheckMenuItem(CommandBar_GetMenu(g_hWndCommandBar, 0), ID_OPTIONS_ENABLEILLUMINATION,
									  MF_UNCHECKED | MF_BYCOMMAND);
						// Set Illumination
		...
		case WM_CREATE: {
			...
			hwndEdit = CreateWindow(TEXT("static"), NULL, WS_CHILD | WS_VISIBLE, 0, 240, 240, 52, hWnd,
