...
typedef SCAN_RESULT(__stdcall* ScanInitProc)(HWND hWnd,
                                             UINT wmInsertion,
                                             UINT wmRemoval);
...
typedef SCAN_RESULT(__stdcall* ScanGetDevInfoProc)(HANDLE hScanner,
                                                   LPSCANDEVINFO lpScanDevInfo);
typedef SCAN_RESULT(__stdcall* ScanRequestDataEventsProc)(HANDLE hScanner,
                                                          HWND hWnd,
                                                          UINT wmScannerData);
typedef SCAN_RESULT(__stdcall* ScanTriggerProc)(HANDLE hScanner);
typedef SCAN_RESULT(__stdcall* ScanGetDataProc)(HANDLE hScanner,
                                                TCHAR* lpBuff,
                                                LPINT BufSize);
// SCANAPI_API SCAN_RESULT ScanGetStatus(HANDLE hScanner);
typedef SCAN_RESULT(__stdcall* ScanDeinitProc)(void);
// SCANAPI_API SCAN_RESULT ScanErrToText(SCAN_RESULT Err, LPTSTR lpBuff, LPINT
// BufSize); SCANAPI_API SCAN_RESULT WINAPI ScanEnableDisableSymbology (HANDLE
// hScanner, INT nSymID, BOOL flag);
...
LibOpen(OpenParams params)
{
  ...
  ScanOpenDeviceP =
    (ScanOpenDeviceProc)GetProcAddress(socketScanDll, TEXT("ScanOpenDevice"));
  ScanCloseDeviceP =
    (ScanCloseDeviceProc)GetProcAddress(socketScanDll, TEXT("ScanCloseDevice"));
  ScanGetDevInfoP =
    (ScanGetDevInfoProc)GetProcAddress(socketScanDll, TEXT("ScanGetDevInfo"));
  ScanRequestDataEventsP = (ScanRequestDataEventsProc)GetProcAddress(
    socketScanDll, TEXT("ScanRequestDataEvents"));
  ScanTriggerP =
    (ScanTriggerProc)GetProcAddress(socketScanDll, TEXT("ScanTrigger"));
  ScanGetDataP =
    (ScanGetDataProc)GetProcAddress(socketScanDll, TEXT("ScanGetData"));
  ScanDeinitP =
    (ScanDeinitProc)GetProcAddress(socketScanDll, TEXT("ScanDeinit"));

  if (!ScanInitP || !ScanOpenDeviceP || !ScanTriggerP || !ScanGetDevInfoP ||
      !ScanDeinitP || !ScanCloseDeviceP || !ScanGetDataP ||
      !ScanRequestDataEventsP) {
...
HandleEvent(VoidP eventP)
{
  ...
  switch (we->msg) {
    ...
    case WM_SCANNERDATA: {
      ...
      status(
        "ondata",
        ScanGetDataP(receivedHandle, (TCHARP)&ScannerData, &ScannerDataSize));
