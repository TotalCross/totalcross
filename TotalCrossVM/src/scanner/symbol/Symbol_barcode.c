...
// name of the default event which the pwrdll.dll sets when the unit it powered
// on This program will first look in registry setting
// HKCU\Software\SuperWaba\Scanner\pwrEvent
...
static BOOL
TranslateDecoder(int32 decoder, LPDECODER translated);
static BOOL
...
ScanClose();
static BOOL
PostErrorMessage(LPTSTR szInMsg, DWORD dwError);
...
static HANDLE hAbortEvent =
  NULL; // abort the thread which is monitoring the scanner
   ...
   if ((RCM_GetUniqueUnitId(&var) == 0))
   {
      ...
      *s++ = 'x';
      for (i = 0; i < 8; i++, s+=2)
         int2hex(var[i],2,s);
      *s = 0;
      settingsClass = loadClass(scannerContext, "totalcross.sys.Settings",
true); setObjectLock(*getStaticFieldObject(null,settingsClass,
"romSerialNumber") = createStringObjectFromCharP(scannerContext, serial, -1),
UNLOCKED);
   }
}*/
...
LibOpen(OpenParams params)
{
  ...
  scannerClass = loadClass(
    params->currentContext, "totalcross.io.device.scanner.Scanner", false);
...
//----------------------------------------------------------------------------
// superwaba/ext/xplat/NativeMethods static boolean scannerSetBarcodeParam(int
// barcodeType, boolean enable)
//
// Enable/disable a decoder
//----------------------------------------------------------------------------
DWORD
ScanToggleDecoder(LPDECODER lpDecoder, bool enable)
{
  ...
  if (dwResult == E_SCN_SUCCESS) {
    ...
    if (enable) {
      SI_SET_FIELD(&DecoderList,
                   Decoders.byList[DecoderList.Decoders.dwDecoderCount],
                   (BYTE)*lpDecoder);
      DecoderList.Decoders.dwDecoderCount++;
      dwResult = SCAN_SetEnabledDecoders(hScanner, &DecoderList);
    } else if (decoderCount >
               DecoderList.Decoders.dwDecoderCount) { // something is disabled
      dwResult = SCAN_SetEnabledDecoders(hScanner, &DecoderList);
    }
  }
  return dwResult;
...
//----------------------------------------------------------------------------
// superwaba/ext/xplat/io/scanner/NativeMethods boolean setBarcodeLength(int
// barcodeType, int lengthType, int length1, int length2 )
//----------------------------------------------------------------------------
DWORD
ScanSetDecoderLengths(LPDECODER lpDecoder, DWORD dwMinLength, DWORD dwMaxLength)
{
  ...
  return (SCAN_SetDecoderParams(hScanner, lpDecoder, &decoder_params));
}
...
TranslateDecoder(int32 decoder, LPDECODER translated)
{
  switch (decoder) {
    case 0:
      strcpy(translated, DECODER_CODE39);
      break;
    case 1:
      strcpy(translated, DECODER_UPCA);
      break;
    case 2:
      strcpy(translated, DECODER_UPCE0);
      break;
    case 3:
      strcpy(translated, DECODER_EAN13);
      break;
    case 4:
      strcpy(translated, DECODER_EAN8);
      break;
    case 5:
      strcpy(translated, DECODER_D2OF5);
      break;
    case 6:
      strcpy(translated, DECODER_I2OF5);
      break;
    case 7:
      strcpy(translated, DECODER_CODABAR);
      break;
    case 8:
      strcpy(translated, DECODER_CODE128);
      break;
    case 9:
      strcpy(translated, DECODER_CODE93);
      break;
    case 11:
      strcpy(translated, DECODER_MSI);
      break;
    case 12:
      strcpy(translated, DECODER_UPCE1);
      break;
    case 13:
      strcpy(translated, DECODER_TRIOPTIC39);
      break;
    case 15:
      strcpy(translated, DECODER_CODE11);
      break;
...
ScanStartMonitorThread()
{
  ...
  hThread = CreateThread(
    NULL, 0, (LPTHREAD_START_ROUTINE)ScanMonitorThread, NULL, 0, &dwThreadId);
  if (hThread == NULL) {
...
ScanMonitorThread()
{
  ...
  while (!bAbort) {
    ...
    if (bIsScannerActive) {
      ...
      dwResult = SCAN_ReadLabelEvent(
        hScanner, lpScanBuffer, hScannerEvent, 8460000, &dwRequestId);
      if (dwResult != E_SCN_SUCCESS) {
    ...
    switch (dwResult) {
      case WAIT_OBJECT_0: { // scanner woke us up
#if _WIN32_WCE >= 300
        // make sure the device does not go into suspended state.  The device
        // does not consider a scan to be a user activity, so may power off if
        // we don't call SystemIdleTimerReset.
        ...
        if (lpScanBuffer->dwStatus == E_SCN_SUCCESS) {
          ...
          continue;
        } else if (lpScanBuffer->dwStatus == E_SCN_DEVICEFAILURE) {
      ...
      case (WAIT_OBJECT_0 + 1): // abort/exit event woke us up
        ...
        continue;
      case (WAIT_OBJECT_0 +
            2): // deactivate scanner - do not allow reads until enabled
      ...
      case (WAIT_OBJECT_0 + 4):
        // power on event woke us up so wait to give time for the scanner & RF
        // card to power up, but still abort if abort event
        {
...
CreatePowerUpEvent()
{
  ...
  rc = RegOpenKeyEx(HKEY_CURRENT_USER,
                    TEXT("Software\\SuperWaba\\Scanner"),
                    0,
                    KEY_ALL_ACCESS,
                    &hKey);
  if (rc == ERROR_SUCCESS) {
    DWORD dwType = REG_SZ;
    RegQueryValueEx(
      hKey, TEXT("pwrEvent"), 0, &dwType, (LPBYTE)szEventName, &dwBufLen);
    RegCloseKey(hKey);
  }

  hEventPowerUp = CreateEvent(NULL, TRUE, FALSE, szEventName);
  // If we the power-up Event doesn't already exist then we can't detect power
  // on events
...
ScanSetup()
{
  ...
  //
  {
    ...
    reader_params.ReaderSpecific.laser_specific.dwRasterMode =
      RASTER_MODE_SMART;
    ...
    reader_params.ReaderSpecific.laser_specific.dwLinearSecurityLevel =
      SECURITY_ALL_THRICE;
...
{
  if (lpScanBuffer != NULL) {
    if (SCNBUF_GETSTAT(lpScanBuffer) ==
        E_SCN_READPENDING) { // if an outstanding read then cancel read
...
PostErrorMessage(LPTSTR szInMsg, DWORD dwError)
{
  ...
  ret = PostMessage(getMainWindowHandle(),
                    UM_SCAN_ERROR,
                    0,
                    (LPARAM)szMsg) != 0; // Zero means failure.
  xfree(szMsg);
...
//////////////////////////////////////////////////////////////////////////
SCAN_API void
tidsS_scannerActivate(NMParams p)
{ // totalcross/io/device/scanner/Scanner native public static
...
//////////////////////////////////////////////////////////////////////////
SCAN_API void
tidsS_setBarcodeParam_ib(NMParams p)
{ // totalcross/io/device/scanner/Scanner native public static
  // boolean setBarcodeParam(int barcodeType, boolean enable);
...
//////////////////////////////////////////////////////////////////////////
SCAN_API void
tidsS_setParam_iii(NMParams p)
{ // totalcross/io/device/scanner/Scanner native public static
  // boolean setParam(int type, int barcodeType, int value);
  int32 type = p->i32[0];
...
//////////////////////////////////////////////////////////////////////////
SCAN_API void
tidsS_setBarcodeLength_iiii(NMParams p)
{ // totalcross/io/device/scanner/Scanner native public static
  // boolean setBarcodeLength(int barcodeType, int lengthType,
  // int min, int max);
...
#define LENGTH_WITHIN_RANGE 0x03
  ...
  // Allowable decode lengths are specified by dwMinLength and dwMaxLength as
  // follows:

  // Variable length - dwMinLength = dwMaxLength = 0
  // Range (from a to b, including a and b) - dwMinLength = a, dwMaxLength = b
  // Two Discrete Lengths (either a or b, given a<b) - dwMinLength = b,
  // dwMaxLength = a One Discrete Length (only a) - dwMinLength = dwMaxLength =
  // a
...
//////////////////////////////////////////////////////////////////////////
SCAN_API void
tidsS_commitBarcodeParams(NMParams p)
{ // totalcross/io/device/scanner/Scanner native public static
  // boolean commitBarcodeParams();
  p->retI = true;
}
//////////////////////////////////////////////////////////////////////////
SCAN_API void
tidsS_getData(NMParams p)
{ // totalcross/io/device/scanner/Scanner native
  ...
  if ((dwResult = SCNBUF_GETSTAT(lpScanBuffer)) != E_SCN_SUCCESS) {
    ...
    p->retO = createStringObjectFromTCHAR(
      p->currentContext, barcode, SCNBUF_GETLEN(lpScanBuffer));
    setObjectLock(p->retO, UNLOCKED);
  }
}
//////////////////////////////////////////////////////////////////////////
SCAN_API void
tidsS_getScanManagerVersion(NMParams p)
{ // totalcross/io/device/scanner/Scanner native public static
  // String getScanManagerVersion();
  ...
  if ((dwResult = SCAN_GetVersionInfo(hScanner, &scanVersionInfo)) !=
      E_SCN_SUCCESS) {
    // error, do something about it!
  } else {
...
//////////////////////////////////////////////////////////////////////////
SCAN_API void
tidsS_getScanPortDriverVersion(NMParams p)
{ // totalcross/io/device/scanner/Scanner native public static
  // String getScanPortDriverVersion();
  SCAN_VERSION_INFO scanVersionInfo;
  DWORD dwResult;
  char buf[20];
...
//////////////////////////////////////////////////////////////////////////
SCAN_API void
tidsS_setParam_ss(NMParams p)
{ // totalcross/io/device/scanner/Scanner native public static
  // void setParam(String what, String value);
}