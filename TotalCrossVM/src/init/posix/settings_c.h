...
void
getSerialNum(char* id, int maxlen);
#if defined(darwin)
int
getRomVersion();
#endif
...
#if !defined(darwin)
// todo@ All licensed platforms have to implement this function to identify a
// device
void
getSerialNum(char* id, int maxlen)
{
...
typedef TCHAR AppHive[HIVE_MAXCHARS +
                      3]; // plus the dot, '\0' and the extra char for binary
static const TCHAR* settingsSubdir = TEXT("appSettings" PATH_SEPARATOR_STR);

static void
getAppPreferencesPath(TCHAR* buf, AppHive h)
{
  lstrcpy(buf, appPath);
  ...
  if (buf[finalpos] != '/') {
    if (buf[finalpos] == '\\') {
      ...
      lstrcat(buf, PATH_SEPARATOR_STR);
    }
  }
...
static int
ensureAppPreferencesDirectory(TCHAR* buf)
{
  lstrcpy(buf, appPath);
  if (access(buf, W_OK) != 0) {
    ...
    if (mkdir(buf, DEFAULT_DIR_PERMS) != 0) {
      return 0;
    }
  }
  int finalpos = lstrlen(buf) - 1;
...
static TCHAR*
getAppHive(AppHive h, uint32 crid, bool bin)
{
  TCHAR* cur = h;
  if (bin) {
    *cur++ = (TCHAR)'B';
...
static TCHAR*
getAppSecretHive(AppHive h, uint32 crid)
{
  TCHAR* buf = h;
  unsigned i;
...
static char*
readAppPreferences(TCHAR* hive, bool bin, UInt16* outlen)
{
  int f;
...
static void
writeAppPreferences(AppHive h, uint8* buf, UInt16 len)
{
  TCHAR path[MAXPATHLEN];
...
static void
deleteAppPreferences(AppHive h)
{
  TCHAR path[MAXPATHLEN];
  getAppPreferencesPath(path, h);
  unlink(path);
...
{ // guich@580_21: use hklm if for secret key
  ...
  TCHAR* k = isHKLM ? getAppSecretHive(hive, applicationId)
                    : getAppHive(hive, applicationId, bin);
...
{ // guich@580_21: use hklm if for secret key
  ...
  if (buf && len > 0) {
    TCObject temp = null;
    target = temp = bin ? createByteArray(currentContext, len)
                        : createCharArray(currentContext, len / 2);
    if (temp) {
...
void
fillIOSSettings(int* daylightSavingsPtr,
                int* daylightSavingsMinutesPtr,
                int* timeZonePtr,
                int* timeZoneMinutesPtr,
                char* timeZoneStrPtr,
                int sizeofTimeZoneStr); // darwin/gfx_Graphics_c.h
...
updateDaylightSavings(Context currentContext)
{
#ifdef darwin
  char nameBuf[50];
  fillIOSSettings(tcSettings.daylightSavingsPtr,
                  ...
                  sizeof(nameBuf));
  if (nameBuf[0] != 0) {
    setObjectLock(*tcSettings.timeZoneStrPtr =
                    createStringObjectFromCharP(currentContext, nameBuf, -1),
                  UNLOCKED);
  }
#else
  time_t t;
  ...
  *tcSettings.daylightSavingsPtr = localtime_r(&t, &tm)->tm_isdst;
  ...
  localtime_r(
    &t, &tm); // localtime_r is the reentrant version (multithreading aware)
...
fillSettings(Context currentContext)
{
  ...
  jmethodID method = (*env)->GetStaticMethodID(
    env, applicationClass, "requestPhoneStatePermission", "()I");
  jint result = (*env)->CallStaticIntMethod(env, applicationClass, method);
  if (result <= 0) {
    return false;
  }

  jclass jSettingsClass =
    androidFindClass(env, "totalcross/android/Settings4A");
  jmethodID fillSettingsMethod =
    (*env)->GetStaticMethodID(env, jSettingsClass, "fillSettings", "()V");
  ...
  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "lineNumber", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
    ...
    setObjectLock(
      *getStaticFieldObject(currentContext, settingsClass, "lineNumber") =
        createStringObjectFromCharP(currentContext, strTemp, -1),
      UNLOCKED);
  }
  ...
  *tcSettings.dateFormatPtr =
    (int32)(*env)->GetStaticByteField(env, jSettingsClass, jfID);

  jfID = (*env)->GetStaticFieldID(env, jSettingsClass, "dateSeparator", "C");
  *tcSettings.dateSeparatorPtr =
    (char)(*env)->GetStaticCharField(env, jSettingsClass, jfID);
  ...
  *tcSettings.is24HourPtr =
    (bool)(*env)->GetStaticBooleanField(env, jSettingsClass, jfID);
  ...
  *tcSettings.timeSeparatorPtr =
  ...
  // number format
  jfID =
    (*env)->GetStaticFieldID(env, jSettingsClass, "thousandsSeparator", "C");
  ...
  *tcSettings.fingerTouchPtr = true;
  ...
  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "deviceId", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
  ...
  *tcSettings.romVersionPtr =
    (int32)(*env)->GetStaticIntField(env, jSettingsClass, jfID);
  ...
  *tcSettings.daylightSavingsPtr =
  ...
  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "daylightSavingsMinutes", "I");
  *tcSettings.daylightSavingsMinutesPtr =
    (int32)(*env)->GetStaticIntField(env, jSettingsClass, jfID);
  ...
  *tcSettings.screenDensityPtr =
    (double)(*env)->GetStaticDoubleField(env, jSettingsClass, jfID);

  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "timeZoneStr", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
    jstring2CharP(jStringField, strTemp);
  ...
  // identification
  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "userName", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
  ...
  jfID =
    (*env)->GetStaticFieldID(env, jSettingsClass, "imei", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
  ...
  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "imei2", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
  ...
  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "iccid", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
  ...
  jfID =
    (*env)->GetStaticFieldID(env, jSettingsClass, "esn", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
    jstring2CharP(jStringField, strTemp);
    (*env)->DeleteLocalRef(env, jStringField);
    setObjectLock(*getStaticFieldObject(currentContext, settingsClass, "esn") =
                    createStringObjectFromCharP(currentContext, strTemp, -1),
                  UNLOCKED);
  }

  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "ANDROID_ID", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
    jstring2CharP(jStringField, strTemp);
    (*env)->DeleteLocalRef(env, jStringField);
    setObjectLock(
      *getStaticFieldObject(currentContext, settingsClass, "ANDROID_ID") =
        createStringObjectFromCharP(currentContext, strTemp, -1),
      UNLOCKED);
  }
  ...
  *tcSettings.virtualKeyboardPtr =
  ...
  // rom serial number
  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "serialNumber", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
    jstring2CharP(jStringField, romSerialNumber);
  }
  //    (*env)->DeleteLocalRef(env, jSettingsClass); - this is NOT a local ref.
  //    breaks on android 4.2.2

  jfID = (*env)->GetStaticFieldID(
    env, jSettingsClass, "macAddress", "Ljava/lang/String;");
  jStringField =
    (jstring)(*env)->GetStaticObjectField(env, jSettingsClass, jfID);
  if (jStringField != null) {
    jstring2CharP(jStringField, strTemp);
    ...
    if (strTemp[0]) {
      setObjectLock(
        *getStaticFieldObject(currentContext, settingsClass, "macAddress") =
          createStringObjectFromCharP(currentContext, strTemp, -1),
        UNLOCKED);
    }
  }
...
fillSettings(Context currentContext)
{
...
#if defined(darwin)
  *tcSettings.romVersionPtr =
    getRomVersion(); // flsobral@tc126_3: implemented Settings.romVersion for
                     // iPhone/iPad.
#else
  ...
  *getStaticFieldObject(currentContext, settingsClass, "platform") =
    *getStaticFieldObject(
      currentContext,
      settingsClass,
      platform); // flsobral@tc126_38: fixed implementation of
                 // Settings.platform for iPhone and iPad.
