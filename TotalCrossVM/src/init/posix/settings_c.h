...
void fillIOSSettings(int* daylightSavingsPtr, int* daylightSavingsMinutesPtr, int* timeZonePtr,
					 int* timeZoneMinutesPtr, char* timeZoneStrPtr, int sizeofTimeZoneStr); // darwin/gfx_Graphics_c.h

void updateDaylightSavings(Context currentContext) {
#ifdef darwin
	char nameBuf[50];
	fillIOSSettings(tcSettings.daylightSavingsPtr, tcSettings.daylightSavingsMinutesPtr,
					tcSettings.timeZonePtr, tcSettings.timeZoneMinutesPtr, nameBuf, sizeof(nameBuf));
	if (nameBuf[0] != 0) {
		setObjectLock(*tcSettings.timeZoneStrPtr = createStringObjectFromCharP(currentContext, nameBuf, -1),
					  UNLOCKED);
	}
#else
	time_t t;
...
bool fillSettings(Context currentContext) {
	...
	jmethodID method = (*env)->GetStaticMethodID(env, applicationClass, "requestPhoneStatePermission",
					   "()I");
	...
	jmethodID fillSettingsMethod = (*env)->GetStaticMethodID(env, jSettingsClass, "fillSettings",
								   "()V");
	...
	if (jStringField != null) {
		...
		setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
											"lineNumber") = createStringObjectFromCharP(currentContext, strTemp, -1), UNLOCKED);
	}
	...
	*tcSettings.screenDensityPtr = (double)(*env)->GetStaticDoubleField(env, jSettingsClass, jfID);
	...
	if (jStringField != null) {
		...
		(*env)->DeleteLocalRef(env, jStringField);
		setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
											"timeZoneStr") = createStringObjectFromCharP(currentContext, strTemp, -1), UNLOCKED);
	}
	...
	if (jStringField != null) {
		...
		(*env)->DeleteLocalRef(env, jStringField);
		setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
											"esn") = createStringObjectFromCharP(currentContext, strTemp, -1), UNLOCKED);
	}
	...
	if (jStringField != null) {
		...
		(*env)->DeleteLocalRef(env, jStringField);
		setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
											"ANDROID_ID") = createStringObjectFromCharP(currentContext, strTemp, -1), UNLOCKED);
	}
	...
	*tcSettings.virtualKeyboardPtr = (bool)(*env)->GetStaticBooleanField(env, jSettingsClass, jfID);
	...
	if (jStringField != null) {
		...
		if (strTemp[0]) {
			setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
												"macAddress") = createStringObjectFromCharP(currentContext, strTemp, -1), UNLOCKED);
		}
	}
...
bool fillSettings(Context currentContext) {
	...
	*tcSettings.romVersionPtr =
		getRomVersion(); //flsobral@tc126_3: implemented Settings.romVersion for iPhone/iPad.
#else
	...
	*getStaticFieldObject(currentContext, settingsClass,
						  "platform") = *getStaticFieldObject(currentContext, settingsClass,
										platform); //flsobral@tc126_38: fixed implementation of Settings.platform for iPhone and iPad.
