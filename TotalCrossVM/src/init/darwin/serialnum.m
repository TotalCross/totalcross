...
/*! Create as generic NSDictionary to be used to query and update Keychain
 * items. param1 param2
 */
 ...
 *  param2
 */
+ (OSStatus)setValue:(NSString*)value
      forKeychainKey:(NSString*)key
...
+ (NSString*)valueForKeychainKey:(NSString*)key service:(NSString*)service
{
  ...
  status = SecItemCopyMatching((__bridge CFDictionaryRef)keychainItem,
                               (CFTypeRef*)&result);
  if (status != noErr) {
    return nil;
  }
...
getSerialNum(char* id, int maxlen)
{
  ...
  NSString* nsserial =
    ver >= 6 ? [DeviceUID uid] : // new for iOS 6 or above
