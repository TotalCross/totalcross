...
TC_API DWORD TSV_Close(DWORD dwData) {
	return 0;
}
TC_API DWORD TSV_Deinit(DWORD dwData) {
	return 0;
}
TC_API DWORD TSV_IOControl(DWORD dwData, DWORD dwCode, PBYTE pBufIn, DWORD dwLenIn, PBYTE pBufOut,
						   DWORD dwLenOut, PDWORD pdwActualOut) {
	return 1;
}
TC_API DWORD TSV_Open(DWORD dwData, DWORD dwAccess, DWORD dwShareMode) {
	return 0;
}
TC_API DWORD TSV_Read(DWORD dwData, LPVOID pBuf, DWORD dwLen) {
	return 0;
}
TC_API DWORD TSV_Seek(DWORD dwData, long pos, DWORD type) {
	return 0;
}
TC_API DWORD TSV_Write(DWORD dwData, LPCVOID pInBuf, DWORD dwInLen) {
	return 0;
}
...
unsigned long __cdecl StartVMFromService(void* nnn) {
	...
	err = RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("\\Services\\TotalCrossSrv"), 0, KEY_ALL_ACCESS,
					   &handle);
	size = sizeof(buf);
	buf[0] = 0;
...
static void getWorkingDir() {
	...
	GetModuleFileName(hModuleTCVM, d1,
					  MAX_PATH); // note: passing 0 here returns the path to launcher.exe, not this dll
	TCHARP2CharPBuf(d1, vmPath);
	// get the path to the exe
	GetModuleFileName(GetModuleHandle(null), d2,
					  MAX_PATH); // note: passing 0 here returns the path to launcher.exe, not this dll
...
static void
waitUntilStarted() { // waits until the window is shown in windows ce so that the graphics test can run correctly
	MSG msg;
	int i, ret;

	for (i = *tcSettings.isFullScreenPtr ? 2 : 1; i != 0;
		) { // the first repaint is still with the taskbar; the 2nd repaint already removed the taskbar
...
static bool checkIfRunning() {
	...
	tzero(wbs);
	EnumWindows(SearchWindowProc, (long)
				&wbs); // both window text and class must be tested, otherwise an Explorer window browsing the Painter folder will be incorrectly recognized as a Painter application
	if (wbs.hWnd != null) {
...
bool wokeUp() {
	...
	last = cur;
	if (RegCreateKeyEx(HKEY_CURRENT_USER, TEXT("Software\\TotalCross"), 0, NULL, 0, KEY_ALL_ACCESS,
					   NULL, &h, &res) == 0) {
		res = 4;
