...
TC_API DWORD
TSV_Close(DWORD dwData)
{
  return 0;
}
TC_API DWORD
TSV_Deinit(DWORD dwData)
{
  return 0;
}
TC_API DWORD
TSV_IOControl(DWORD dwData,
              DWORD dwCode,
              PBYTE pBufIn,
              DWORD dwLenIn,
              PBYTE pBufOut,
...
TC_API DWORD
TSV_Open(DWORD dwData, DWORD dwAccess, DWORD dwShareMode)
{
  return 0;
}
TC_API DWORD
TSV_Read(DWORD dwData, LPVOID pBuf, DWORD dwLen)
{
  return 0;
}
TC_API DWORD
TSV_Seek(DWORD dwData, long pos, DWORD type)
{
  return 0;
}
TC_API DWORD
TSV_Write(DWORD dwData, LPCVOID pInBuf, DWORD dwInLen)
{
  return 0;
}
...
getWorkingDir()
{
  ...
  GetModuleFileName(hModuleTCVM,
                    d1,
                    MAX_PATH); // note: passing 0 here returns the path to
                               // launcher.exe, not this dll
...
static void
waitUntilStarted()
{ // waits until the window is shown in windows ce so that
  // the graphics test can run correctly
  MSG msg;
  int i, ret;

  for (i = *tcSettings.isFullScreenPtr ? 2 : 1;
       i != 0;) { // the first repaint is still with the taskbar; the 2nd
                  // repaint already removed the taskbar
...
SearchWindowProc(HWND hwnd, LPARAM lParam)
{
  TCHAR wclass[256];
...
checkIfRunning()
{
  ...
  EnumWindows(
    SearchWindowProc,
    (long)&wbs); // both window text and class must be tested, otherwise an
                 // Explorer window browsing the Painter folder will be
                 // incorrectly recognized as a Painter application
  if (wbs.hWnd != null) {
...
wokeUp()
{
  ...
  if (RegCreateKeyEx(HKEY_CURRENT_USER,
                     ...
                     &res) == 0) {
    res = 4;
    if (RegQueryValueEx(h, TEXT("PowerOn"), 0, NULL, (uint8*)&v, &res) == 0 &&
        v == 1) {
      res = 0;
      RegSetValueEx(h,
                    TEXT("PowerOn"),
...
static void
registerWake(bool reg)
{
#if defined(WINCE) && _WIN32_WCE >= 300
  TCHAR szExeName[MAX_PATH];
  GetModuleFileName(GetModuleHandle(0), szExeName, MAX_PATH);
  CeRunAppAtEvent(szExeName,
                  reg ? NOTIFICATION_EVENT_WAKEUP : NOTIFICATION_EVENT_NONE);
#endif
}

static bool
isWakeUpCall(CharP args)
{
#if defined(WINCE) && _WIN32_WCE >= 300
  if (xstrstr(args, "AppRunAfterWakeup") != 0) {
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER,
                       ...
                       &res) == 0) {
      DWORD v = 1;
      RegSetValueEx(h, TEXT("PowerOn"), 0, REG_DWORD, (byte*)&v,
                    4); // set to 1
      RegCloseKey(h);
    } // else {TCHAR buf[100]; xstrprintf(buf,"%d",(int)GetLastError());
      // MessageBox(0, buf,L"Erro na abertura",MB_OK);}
    return true;
  }
#endif
  return false;
...
static CharP
parseScreenBounds(CharP cmd, int32* xx, int32* yy, int32* ww, int32* hh)
{
  ...
  if (n != 4) {
    alert("Format: <other arguments> /scr x,y,width,height\nPass -1 to use the "
          "default and -2 to center on screen.\nEx: \"/scr "
          "-2,100,320,-1\"\nwill open a window horizontally centered "
...
screenChange(Context currentContext,
             ...
             int32 hRes,
             int32 vRes,
             bool nothingChanged); // GraphicsPrimitives_c.h
