...
static void destroyAll() { // must be in inverse order of initAll calls
	...
	destroyContexts();
	if (isMainWindow) {
		destroyGraphics();    // graphics can't be destroyed before threads are destroyed
	}
	registerWake(false);
	destroyNativeLib();
	if (tcSettings.showMemoryMessagesAtExit != NULL) {
		showMemoryMessagesAtExit =
			*tcSettings.showMemoryMessagesAtExit;    // guich@tc114: save in a global var, since tcSettings will no longer be available
	}
...
TC_API int32 startProgram(Context currentContext) {
...
#if defined (WIN32) && !(defined (WINCE) || defined(WP8)) //flsobral@tc115_64: on Win32, automatically load LitebaseLib.tcz if Litebase is installed and allowed.
	{
		...
		if (GetEnvironmentVariable(TEXT("LITEBASE_HOME"), litebasePath, MAX_PATHNAME) != 0) {
			tcscat(litebasePath,
				   TEXT("/dist/lib/LitebaseLib.tcz")); //flsobral@tc120_18: fixed path of LitebaseLib.tcz on Win32. Applications should now able to run from anywhere, as long as the Litebase and TotalCross home paths are set.
			tczLoad(currentContext, litebasePath);
		}
	}
#endif
	// 3. Load the main class (also calls its static initializer)
	c = loadClass(currentContext, mainClassName,
				  true); // some fields of totalcross.sys.Settings may be set by the programmer at the static initializer, called now
	if (c == null) {
	...
	if (currentContext->thrownException == null
		...
		if (isMainWindow) {
			mainEventLoop(currentContext);    // in the near future, MainClass apps will also receive events.
		}
		// 8. call appEnding
...
TC_API int32 startVM(CharP argsOriginal, Context* cOut) {
	...
	*cOut = currentContext = initAll(&tczName);
	...
	if (xstrstr(argsLower, "launcher")
		&& 0) { // if executing the default Launcher, instead of creating a launcher for an application, run the testsuite
	...
	if (cmdline || !*args) { // parse the command line
		...
		if (ALLOW_TEST_SUITE && (!*args || xstrstr(cmdline, "-testsuite"))) {
			...
			mainContext->OutOfMemoryErrorObj = createObject(currentContext,
											   "java.lang.OutOfMemoryError"); // now its safe to initialize the OutOfMemoryErrorObj for the main context
			...
			startTestSuite(
				currentContext); // run the testsuite before Graphics test to be able to test for Font and FontMetrics
			destroyGraphics();
#else
	...
	imageClass = loadClass(currentContext, "totalcross.ui.image.Image", false);
	loadExceptionClasses(currentContext); // guich@tc112_18
	voidTYPE    = getStaticFieldObject(currentContext, loadClass(currentContext, "java.lang.Void",
									   false), "TYPE");
	booleanTYPE = getStaticFieldObject(currentContext, loadClass(currentContext, "java.lang.Boolean",
									   false), "TYPE");
	byteTYPE    = getStaticFieldObject(currentContext, loadClass(currentContext, "java.lang.Byte",
									   false), "TYPE");
	shortTYPE   = getStaticFieldObject(currentContext, loadClass(currentContext, "java.lang.Short",
									   false), "TYPE");
	intTYPE     = getStaticFieldObject(currentContext, loadClass(currentContext, "java.lang.Integer",
									   false), "TYPE");
	longTYPE    = getStaticFieldObject(currentContext, loadClass(currentContext, "java.lang.Long",
									   false), "TYPE");
	floatTYPE   = getStaticFieldObject(currentContext, loadClass(currentContext, "java.lang.Float",
									   false), "TYPE");
	doubleTYPE  = getStaticFieldObject(currentContext, loadClass(currentContext, "java.lang.Double",
									   false), "TYPE");
	charTYPE    = getStaticFieldObject(currentContext, loadClass(currentContext, "java.lang.Character",
									   false), "TYPE");
	// used in convert
	charConverterPtr = getStaticFieldObject(currentContext, loadClass(currentContext,
											"totalcross.sys.Convert", false), "charConverter");
	...
	cloneable = loadClass(currentContext, "java.lang.Cloneable",
						  false); // Loads the Cloneable interface.

	// Create a Java thread for the main context and call it "TC Event Thread"
	mainContext->threadObj = createObjectWithoutCallingDefaultConstructor(currentContext,
							 "java.lang.Thread");
	name = createStringObjectFromCharP(currentContext, "TC Event Thread", -1);
	executeMethod(currentContext, getMethod(OBJ_CLASS(mainContext->threadObj), false, CONSTRUCTOR_NAME,
											1, "java.lang.String"), mainContext->threadObj, name);
	setObjectLock(name, UNLOCKED);

	// Create a Java thread for the GC context and call it "Finalizer"
	gcContext->threadObj = createObjectWithoutCallingDefaultConstructor(currentContext,
						   "java.lang.Thread");
	name = createStringObjectFromCharP(currentContext, "Finalizer", -1);
	executeMethod(currentContext, getMethod(OBJ_CLASS(gcContext->threadObj), false, CONSTRUCTOR_NAME, 1,
											"java.lang.String"), gcContext->threadObj, name);
	setObjectLock(name, UNLOCKED);
	...
	if ((loadedTCZ = tczLoad(currentContext, tczName)) == null) {
		...
		if (!initSettings(currentContext, mainClassName, loadedTCZ)) {
			...
			if (isMainWindow && (!initGraphicsBeforeSettings(currentContext, loadedTCZ->header->attr)
								 || !keepRunning)) {
				return exitProgram(107);
			} else {
				// 2. Retrieve the settings
				if (!retrieveSettings(currentContext, mainClassName)
					|| !keepRunning) { // Settings must always be initialized before the application
					return exitProgram(108);    // used at exit!
				} else if (isMainWindow && (!initGraphicsAfterSettings(currentContext) || !keepRunning)) {
