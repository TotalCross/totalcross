...
jclass applicationClass, jRadioDevice4A, jBluetooth4A, jConnectionManager4A,
  jSmsManager4A, jNotificationManager4A, jFirebaseInstanceId, jTcFirebaseUtils,
  jCieloPrinterManager4A;
...
#ifdef WIN32
HWND mainHWnd = NULL;
bool bSipUp = false; // flsobral@tc114_50: fixed the SIP keyboard button not
...
TCObjectArray usedList = {
  0
}; // the array with lists of used objects (allocated after the last GC)
...
// The garbage collector tests requires that no objects are created, so we cache
// the state, then restore it when the test finishes
bool canTraverse = true;
TCObjectArray freeList2 = { 0 }; // the array with lists of free objects
TCObjectArray usedList2 = {
  0
}; // the array with lists of used objects (allocated after the last GC)
...
void* deviceCtx =
  NULL; // The device context points a structure containing platform specific
        // data that have to handled in platform specific code only, that's
        // why we don't define a structure here insofar some platform specific
        // data can't be defined in plain C (such as SymbianOS C++ classes,
        // iPhone objC data structures, ...) Currently this pointer is
        // mirrored in ScreenSurface in the extension field but this may
...
// class.c
TCObject *voidTYPE, *booleanTYPE, *byteTYPE, *shortTYPE, *intTYPE, *longTYPE,
  *floatTYPE, *doubleTYPE, *charTYPE;
