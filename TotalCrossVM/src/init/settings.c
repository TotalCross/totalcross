...
static void createSettingsAliases(Context currentContext, TCZFile loadedTCZ) {
	...
	tcSettings.keyboardFocusTraversablePtr = getStaticFieldInt(settingsClass,
	...
	*tcSettings.chunksCreated = 3;
	tcSettings.appSettingsPtr              = getStaticFieldObject(currentContext, settingsClass,
			"appSettings");
	tcSettings.appSecretKeyPtr             = getStaticFieldObject(currentContext, settingsClass,
			"appSecretKey");
	tcSettings.appSettingsBinPtr           = getStaticFieldObject(currentContext, settingsClass,
			"appSettingsBin");
	tcSettings.showMemoryMessagesAtExit    = getStaticFieldInt(settingsClass,
			"showMemoryMessagesAtExit");
	tcSettings.timeZoneStrPtr              = getStaticFieldObject(currentContext, settingsClass,
			"timeZoneStr");
	tcSettings.fingerTouchPtr              = getStaticFieldInt(settingsClass, "fingerTouch");
	tcSettings.disableDebug                = getStaticFieldInt(loadClass(currentContext,
			"totalcross.sys.Vm", true), "disableDebug");
	tcSettings.fullScreenPlatformsPtr      = getStaticFieldObject(currentContext, settingsClass,
			"fullScreenPlatforms");
	...
	tcSettings.iccidPtr                    = getStaticFieldObject(currentContext, settingsClass,
			"iccid");
	...
	tcSettings.lineNumber                  = getStaticFieldObject(currentContext, settingsClass,
			"lineNumber");
	tcSettings.unmovableSIP                = getStaticFieldInt(settingsClass, "unmovableSIP");
	tcSettings.bugreportEmail              = getStaticFieldObject(currentContext, settingsClass,
			"bugreportEmail");
	tcSettings.appVersion                  = getStaticFieldObject(currentContext, settingsClass,
			"appVersion");
...
static bool
inSerialNumberExclusionList() { // empties the serial number in devices that returns it incorrectly
	int32 i;
	char deviceId[100];
...
bool retrieveSettings(Context currentContext, CharP mainClassName) {
	...
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"applicationId")   = createStringObjectFromCharP(currentContext, applicationIdStr, -1), UNLOCKED);
#if !defined darwin
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"platform")        = createStringObjectFromCharP(currentContext, platform, -1), UNLOCKED);
#endif
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"userName")        = createStringObjectFromCharP(currentContext, userName, -1), UNLOCKED);
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"romSerialNumber") = createStringObjectFromCharP(currentContext, romSerialNumber, -1), UNLOCKED);
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"deviceId")        = createStringObjectFromCharP(currentContext, deviceId, -1), UNLOCKED);
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"imei")            = createStringObjectFromCharP(currentContext, imei, -1), UNLOCKED);
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"appPath")         = createStringObjectFromCharP(currentContext, appPath, -1), UNLOCKED);
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"dataPath")        = createStringObjectFromCharP(currentContext, dataPath, -1), UNLOCKED);
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"vmPath")          = createStringObjectFromCharP(currentContext, vmPath, -1), UNLOCKED);
	setObjectLock(*getStaticFieldObject(currentContext, settingsClass,
										"iccid")           = createStringObjectFromCharP(currentContext, iccid, -1), UNLOCKED);
	...
	if (imei[0] || imei2[0]) {
		...
		*imeisArrayObj = createStringArray(currentContext, imei[0] && imei2[0] ? 2 : 1);
		if (*imeisArrayObj) {
			...
			if (imei [0]) {
				setObjectLock(*imeis++ = createStringObjectFromCharP(currentContext, imei, -1), UNLOCKED);
			}
			if (imei2[0]) {
				setObjectLock(*imeis++ = createStringObjectFromCharP(currentContext, imei2, -1), UNLOCKED);
			}
...
void retrieveSettingsChangedAtStaticInitializer(Context currentContext) {
	...
	*tcSettings.appSecretKeyPtr = getAppSettings(currentContext, getSecretKeyCreator(applicationId),
								  false, true);
...
static void updateEntry(char* name, uint32 crtr, bool bin, bool isHKLM) {
	...
	if (obj == NULL
		|| ARRAYOBJ_LEN(obj) == 0) { // if string null, delete it - guich@240_3: first condition added.
...
TC_API void tsS_refresh(NMParams
						p) { // totalcross/sys/Settings native public static void refresh();
	UNUSED(p);
	updateDaylightSavings(p->currentContext);
	storeSettings(false); // guich@tc136
}

void updateScreenSettings(int32 width, int32 height, int32 hRes, int32 vRes,
						  int32 bpp) { // will be called from initGraphicsAfterSettings
