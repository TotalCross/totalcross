...
// GoogleMaps.c
  ...
  extern jclass applicationClass, jRadioDevice4A, jBluetooth4A,
    jConnectionManager4A, jSmsManager4A, jNotificationManager4A,
    jFirebaseInstanceId, jTcFirebaseUtils, jCieloPrinterManager4A;
...
// win/gfx_Graphics_c.h
  ...
  extern bool bSipUp; // flsobral@tc114_50: fixed the SIP keyboard button not
...
// demo.c
  ...
  extern TCObjectArray usedList; // the array with lists of used objects
                                 // (allocated after the last GC)
  ...
  // The garbage collector tests requires that no objects are created, so we
  // cache the state, then restore it when the test finishes
  extern bool canTraverse;
  extern TCObjectArray freeList2; // the array with lists of free objects
  extern TCObjectArray usedList2; // the array with lists of used objects
                                  // (allocated after the last GC)
  extern TCObjectArray lockList2; // locked objects list
  extern uint32 markedAsUsed2;    // starts as 1
  extern uint32 gcCount2, objCreated2, skippedGC2,
    objLocked2; // the current gc count
...
#if !defined(WIN32)
  extern void*
    deviceCtx; // The device context points a structure containing platform
               // specific data that have to handled in platform specific code
               // only, that's why we don't define a structure here insofar
               // some platform specific data can't be defined in plain C (such
               // as SymbianOS C++ classes, iPhone objC data structures, ...)
               // Currently this pointer is mirrored in ScreenSurface in the
               // extension field but this may change sooner or later.
#endif
...
// vm.c
  ...
  // Vm_c.h
  extern int32
    oldAutoOffValue; // if not 0, the device is in NEVER-SLEEP mode, and the old
                     // value will be restored when the vm quits
#ifdef ANDROID
...
// win/Socket_c.h
...
#ifdef ANDROID
  extern jmethodID jshowCamera, jgetNativeResolutions, jgetDefaultToString,
    jzxing;
  ...
  // class.c
  extern TCObject *voidTYPE, *booleanTYPE, *byteTYPE, *shortTYPE, *intTYPE,
    *longTYPE, *floatTYPE, *doubleTYPE, *charTYPE;
  ...
  // These are set when the VM is initialized, and their values are copied into
  ...
  /***********************   METHODS THAT CAN BE USED BY LIBRARIES TO ACCESS THE
   * GLOBALS  *************************/
