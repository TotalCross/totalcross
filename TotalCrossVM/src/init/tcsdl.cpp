 ...
 */
bool
TCSDL_Init(ScreenSurface screen, const char* title, bool fullScreen)
{

  int width =
    (getenv("TC_WIDTH") == NULL) ? 640 : std::stoi(getenv("TC_WIDTH"));
  int height =
    (getenv("TC_HEIGHT") == NULL) ? 400 : std::stoi(getenv("TC_HEIGHT"));
  ...
  // Create the window
  SDL_Window* window;
  if (IS_NULL(window = SDL_CreateWindow(title,
                                        SDL_WINDOWPOS_UNDEFINED,
                                        SDL_WINDOWPOS_UNDEFINED,
                                        width,
                                        height,
                                        (getenv("TC_FULLSCREEN") == NULL)
                                          ? SDL_WINDOW_SHOWN
  ...
  if (IS_NULL(renderer = SDL_CreateRenderer(window, -1, NO_FLAGS))) {
    ...
    std::cout << '\n'
              << "HINT: try to export SDL_RENDER_DRIVER environment variable "
                 "with an available render driver!"
              << '\n';
    return false;
  }
  ...
  if (SDL_PIXELFORMAT_UNKNOWN ==
      (windowPixelFormat = SDL_GetWindowPixelFormat(window))) {
    std::cerr << "SDL_GetWindowPixelFormat(): " << SDL_GetError() << '\n';
    return false;
  }

  // MUST USE SDL_TEXTUREACCESS_STREAMING, CANNOT BE REPLACED WITH
  // SDL_CreateTextureFromSurface
  if (IS_NULL(texture = SDL_CreateTexture(renderer,
                                          windowPixelFormat,
  ...
  if (IS_NULL(screen->pixels =
                (uint8*)malloc(screen->pitch * screen->screenH))) {
    std::cerr << "Failed to alloc " << (screen->pitch * screen->screenH)
              << " bytes for pixel surface" << '\n';
    return false;
  }

  if (IS_NULL(screen->extension =
                (ScreenSurfaceEx)malloc(sizeof(TScreenSurfaceEx)))) {
    free(screen->pixels);
    std::cerr << "Failed to alloc TScreenSurfaceEx of "
              << sizeof(TScreenSurfaceEx) << " bytes" << '\n';
    return false;
  }
 ...
 */
void
TCSDL_UpdateTexture(int w, int h, int pitch, void* pixels)
{
  // Update the given texture rectangle with new pixel data.
 ...
 */
void
TCSDL_Destroy(ScreenSurface screen)
{
...
void
TCSDL_GetWindowSize(ScreenSurface screen, int32* width, int32* height)
{
