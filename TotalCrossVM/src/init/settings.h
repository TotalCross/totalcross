...
typedef enum
{
  ...
  VMTWEAK_DUMP_MEMORY_STATS, /// Collects memory usage statistics during all
                             /// program run
  VMTWEAK_MEM_PROFILER, /// Collects memory usage statistics during a piece of
                        /// code
...
#define IS_VMTWEAK_ON(x)                                                       \
  (vmTweaks & (1 << (x - 1))) // guich@tc114_19: better use this macro
...
typedef struct
{
  ...
  int32 right;
  int32 bottom;
  int32 topGap; // flsobral@tc123: gap between the top of the screen and the
                // sip.
...
hasVirtualKeyboard();
  ...
  defined darwin // some settings.h functions do not compile onder clang
bool
...
TC_API TCSettings
getSettingsPtr(); // to be called by another dll to get access
                  // to the tcSettings structure
typedef TCSettings (
  *getSettingsPtrFunc)(); // to be called by another dll to get access to the
                          // tcSettings structure

#ifndef __clang__ // some settings.h functions do not compile onder clang
TC_API bool
getDataPath(
  CharP storeInto); // stores the current Settings.dataPath value into the
                    // given buffer, or returns false if it is null
#endif
typedef bool (*getDataPathFunc)(
  CharP storeInto); // stores the current Settings.dataPath value into the
                    // given buffer, or returns false if it is null
void
updateScreenSettings(int32 width,
                     ...
                     int32 hRes,
                     int32 vRes,
                     int32 bpp); // updates Settings.screenWidth/screenHeight

TC_API void
getRomSerialNumber(CharP outBuf); // stores the rom serial number in
                                  // the given buffer of size 128.
typedef void (*getRomSerialNumberFunc)(CharP outBuf);
TC_API void
getImei(CharP outBuf); // stores the imei in the given buffer of size 128.
typedef void (*getImeiFunc)(CharP outBuf);
TC_API void
getDeviceId(
  CharP outBuf); // stores the device id in the given buffer of size 128.
