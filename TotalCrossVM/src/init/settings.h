...
TC_API TCSettings
getSettingsPtr(); // to be called by another dll to get access to the tcSettings structure
typedef TCSettings(*getSettingsPtrFunc)
();  // to be called by another dll to get access to the tcSettings structure

#ifndef __clang__ // some settings.h functions do not compile onder clang
TC_API bool getDataPath(CharP
						storeInto); // stores the current Settings.dataPath value into the given buffer, or returns false if it is null
#endif
typedef bool (*getDataPathFunc)(CharP
								storeInto); // stores the current Settings.dataPath value into the given buffer, or returns false if it is null
...
TC_API void getRomSerialNumber(CharP
							   outBuf); // stores the rom serial number in the given buffer of size 128.
...
TC_API void getDeviceId(CharP
						outBuf);        // stores the device id in the given buffer of size 128.
