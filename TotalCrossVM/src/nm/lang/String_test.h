...
TESTCASE(jlS_toUpperCase) { // java/lang/String native public String toUpperCase();
	...
	p.obj = &obj;
	obj = createStringObjectFromCharP(currentContext,
									  "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha nada.", -1);
...
TESTCASE(jlS_valueOf_d) { // java/lang/String native public static String valueOf(double d);
	...
	ASSERT2_EQUALS(Sz, "0.000000005555556", valueOfDouble(currentContext, buf, 0.000000005555555555555,
				   17));
	ASSERT2_EQUALS(Sz, "0.000000000055556", valueOfDouble(currentContext, buf,
				   0.00000000005555555555555, 17));
	ASSERT2_EQUALS(Sz, "0.000000000000556", valueOfDouble(currentContext, buf,
				   0.000000000000555555555555555, 17));
	ASSERT2_EQUALS(Sz, "0.000000000000006", valueOfDouble(currentContext, buf,
				   0.00000000000000555555555555555, 17));
	ASSERT2_EQUALS(Sz, "5.55555555555555E-17", valueOfDouble(currentContext, buf,
				   0.0000000000000000555555555555555, 20));
...
TESTCASE(jlS_equals_o) { // java/lang/String native public boolean equals(Object obj);
	...
	objs[0] = createStringObjectFromCharP(currentContext,
										  "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha nada.", -1);
	objs[1] = createStringObjectFromCharP(currentContext,
										  "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha nada!", -1);
...
TESTCASE(jlS_copyChars_CiCii) { // java/lang/String native static boolean copyChars(char []srcArray, int srcStart, char []dstArray, int dstStart, int length);
	...
	objs[1] = createStringObjectFromCharP(currentContext,
										  "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha nada!", -1);
	res     = createStringObjectFromCharP(currentContext,
										  "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha mesa!", -1);
	setObjectLock(objs[0], UNLOCKED);
	setObjectLock(objs[1], UNLOCKED);
...
TESTCASE(jlS_replace_cc) { // java/lang/String native public String replace(char oldChar, char newChar);
	...
	jlS_replace_cc(&p);
	ASSERT1_EQUALS(NotNull, p.retO);
	ASSERT2_EQUALS(I32, String_charsLen(obj2),
	...
	jlS_replace_cc(&p);
	ASSERT1_EQUALS(NotNull, p.retO);
	ASSERT2_EQUALS(I32, String_charsLen(obj),
				   String_charsLen(p.retO)); // compare with the target's length
