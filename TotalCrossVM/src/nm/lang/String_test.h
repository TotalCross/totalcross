...
TESTCASE(jlS_toUpperCase)
{ // java/lang/String native public String toUpperCase();
  TNMParams p;
  TCObject obj;
  CharP res8 = null;
  ...
  p.obj = &obj;
  obj = createStringObjectFromCharP(
    currentContext,
    "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha nada.",
    -1);
  ...
  ASSERT2_EQUALS(
    Sz, res8, "ERA UMA CASA T�O ENGRA�ADA, N�O TINHA MESA, N�O TINHA NADA.");
finish:
  xfree(res8);
}
TESTCASE(jlS_toLowerCase)
{ // java/lang/String native public String toLowerCase();
  TNMParams p;
  TCObject obj;
  CharP res8 = null;
...
static CharP
valueOfDouble(Context currentContext, CharP buf, double d, int32 size)
{
  TNMParams p;
...
TESTCASE(jlS_valueOf_d)
{ // java/lang/String native public static String
  // valueOf(double d);
  char buf[25];
  ...
  ASSERT2_EQUALS(
    Sz, "0.55555555555", valueOfDouble(currentContext, buf, 0.55555555555, 13));
  ASSERT2_EQUALS(Sz,
                 "0.0055555555555",
                 valueOfDouble(currentContext, buf, 0.0055555555555, 15));
  ASSERT2_EQUALS(Sz,
                 "0.000055555555555",
                 valueOfDouble(currentContext, buf, 0.000055555555555, 17));
  ASSERT2_EQUALS(Sz,
                 "0.000000555555556",
                 valueOfDouble(currentContext, buf, 0.00000055555555555, 17));
  ASSERT2_EQUALS(Sz,
                 "0.000000005555556",
                 valueOfDouble(currentContext, buf, 0.0000000055555555555, 17));
  ASSERT2_EQUALS(
    Sz,
    "0.000000005555556",
    valueOfDouble(currentContext, buf, 0.000000005555555555555, 17));
  ASSERT2_EQUALS(
    Sz,
    "0.000000000055556",
    valueOfDouble(currentContext, buf, 0.00000000005555555555555, 17));
  ASSERT2_EQUALS(
    Sz,
    "0.000000000000556",
    valueOfDouble(currentContext, buf, 0.000000000000555555555555555, 17));
  ASSERT2_EQUALS(
    Sz,
    "0.000000000000006",
    valueOfDouble(currentContext, buf, 0.00000000000000555555555555555, 17));
  ASSERT2_EQUALS(
    Sz,
    "5.55555555555555E-17",
    valueOfDouble(currentContext, buf, 0.0000000000000000555555555555555, 20));
  ASSERT2_EQUALS(Sz, "10000.0", valueOfDouble(currentContext, buf, 10000.0, 7));
  ASSERT2_EQUALS(
    Sz, "1000.543216", valueOfDouble(currentContext, buf, 1000.543216, 11));
  ...
  ASSERT2_EQUALS(
    Sz, "1000000.0", valueOfDouble(currentContext, buf, 1000000.0, 10));
  ASSERT2_EQUALS(
    Sz, "9999999.0", valueOfDouble(currentContext, buf, 9999999.0, 10));
  ASSERT2_EQUALS(
    Sz, "0.99999999999", valueOfDouble(currentContext, buf, 0.99999999999, 13));
  ASSERT2_EQUALS(
    Sz, "1.99999999999", valueOfDouble(currentContext, buf, 1.99999999999, 13));
  ASSERT2_EQUALS(Sz, "-1.0E308", valueOfDouble(currentContext, buf, -1e308, 8));
  ASSERT2_EQUALS(Sz,
                 "-1.432132689765E140",
                 valueOfDouble(currentContext, buf, -1.432132689765E140, 19));
  ASSERT2_EQUALS(Sz,
                 "-9.454938759257E240",
                 valueOfDouble(currentContext, buf, -9.454938759257E240, 19));
  ASSERT2_EQUALS(Sz,
                 "1.432132689765E140",
                 valueOfDouble(currentContext, buf, 1.432132689765E140, 18));
  ASSERT2_EQUALS(Sz,
                 "9.454938759257E240",
                 valueOfDouble(currentContext, buf, 9.454938759257E240, 18));
  ASSERT2_EQUALS(Sz,
                 "1.432132689765E-140",
                 valueOfDouble(currentContext, buf, 1.432132689765E-140, 19));
  ASSERT2_EQUALS(Sz,
                 "9.454938759257E-240",
                 valueOfDouble(currentContext, buf, 9.454938759257E-240, 19));
...
static JChar
valueOfChar(Context currentContext, JChar c)
{
  TNMParams p;
  int32 i;
...
static CharP
valueOfInt(Context currentContext, CharP buf, int32 i)
{
  TNMParams p;
...
{ // java/lang/String native public static String
  ...
  ASSERT2_EQUALS(Sz, "-12345678", valueOfInt(currentContext, buf, -12345678));
  ASSERT2_EQUALS(
    Sz, "305419896", valueOfInt(currentContext, buf, (int)0x12345678));
...
TESTCASE(jlS_indexOf_i)
{ // java/lang/String native public int indexOf(int c);
  TNMParams p;
  TCObject obj;
  int32 i;
  JCharP j;
...
TESTCASE(jlS_equals_o)
{ // java/lang/String native public boolean equals(Object
  // obj);
  TNMParams p;
  TCObject objs[2];
  ...
  objs[0] = createStringObjectFromCharP(
    currentContext,
    "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha nada.",
    -1);
  objs[1] = createStringObjectFromCharP(
    currentContext,
    "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha nada!",
    -1);
...
TESTCASE(jlS_compareTo_s)
{ // java/lang/String native public int
  // compareTo(String s);
  TNMParams p;
  TCObject objs[2];
...
TESTCASE(jlS_copyChars_CiCii)
{ // java/lang/String native static boolean
  // copyChars(char []srcArray, int srcStart, char
  // []dstArray, int dstStart, int length);
  TNMParams p;
  TCObject objs[2], res;
  int32 i[3];
  ...
  objs[1] = createStringObjectFromCharP(
    ...
    "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha nada!",
    -1);
  res = createStringObjectFromCharP(
    currentContext,
    "Era uma casa t�o engra�ada, n�o tinha mesa, n�o tinha mesa!",
    -1);
  setObjectLock(objs[0], UNLOCKED);
  setObjectLock(objs[1], UNLOCKED);
  ...
  ASSERT3_EQUALS(Block,
                 ARRAYOBJ_START(objs[1]),
                 String_charsStart(res),
                 2 * String_charsLen(res));
...
TESTCASE(jlS_indexOf_si)
{ // java/lang/String native public int indexOf(String
  // c, int startIndex);
  TNMParams p;
  TCObject objs[2];
  int32 s;
...
TESTCASE(jlS_hashCode)
{ // java/lang/String native public int hashCode();
  TNMParams p;
  TCObject obj;
...
TESTCASE(jlS_startsWith_si)
{ // java/lang/String native public boolean
  // startsWith(String prefix, int from);
  TNMParams p;
  TCObject objs[2];
  int32 from;
...
TESTCASE(jlS_equalsIgnoreCase_s)
{ // java/lang/String native public boolean
...
TESTCASE(jlS_replace_cc)
{ // java/lang/String native public String replace(char
  ...
  ASSERT1_EQUALS(NotNull, p.retO);
  ASSERT2_EQUALS(I32,
                 String_charsLen(obj2),
                 String_charsLen(p.retO)); // compare with the target's length
  ASSERT3_EQUALS(
    Block, String_charsStart(obj2), String_charsStart(p.retO), 13 * 2);
  ...
  ASSERT2_EQUALS(I32,
                 ...
                 String_charsLen(p.retO)); // compare with the target's length
  ASSERT3_EQUALS(
    Block, String_charsStart(obj), String_charsStart(p.retO), 13 * 2);

finish:;
}
TESTCASE(jlS_lastIndexOf_ii)
{ // java/lang/String native public int
  // lastIndexOf(int c, int startIndex);
  TNMParams p;
  TCObject obj;
  int32 is[2];
  JCharP j;
...
TESTCASE(jlS_trim)
{ // java/lang/String native public String trim();
  TNMParams p;
  TCObject obj;
  ...
  ASSERT1_EQUALS(NotNull, p.retO);
  ASSERT2_EQUALS(I32,
                 String_charsLen(p.retO),
                 13); // compare with the target's length
  ASSERT3_EQUALS(
    Block, String_charsStart(obj) + 2, String_charsStart(p.retO), 13 * 2);
