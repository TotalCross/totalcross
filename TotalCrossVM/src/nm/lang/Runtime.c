...
#else
	throwExceptionNamed(p->currentContext, "java.lang.UnsupportedOperationException",
...
TCObject createFileStream(Context context, const int streamType, int fd) {
	...
	fileStream = createObject(context,
							  streamType == FILE_STREAM_OUTPUT ? "java.io.FileOutputStream" : "java.io.FileInputStream");
	if (fileStream == NULL) {
