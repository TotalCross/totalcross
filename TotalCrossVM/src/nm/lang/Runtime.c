...
jlR_exec_SSs(NMParams p)
{
  ...
  err = cpproc_forkAndExec(
    cmdArray, envpArray, fds, pipe_count, &pid, filePathArray);
  if (err != 0) {
    // error message
    throwExceptionNamed(
      p->currentContext, "java.io.IOException", strerror(err));
    goto cleanup;
  }
  ...
  fileInputStream = createFileStream(
    p->currentContext, FILE_STREAM_INPUT, fds[CPIO_EXEC_STDIN]);
  if (fileInputStream == NULL) {
    goto cleanup;
  }

  fileOutputStream = createFileStream(
    p->currentContext, FILE_STREAM_OUTPUT, fds[CPIO_EXEC_STDOUT]);
  if (fileOutputStream == NULL) {
    goto cleanup;
  }

  fileErrInputStream = createFileStream(
    p->currentContext, FILE_STREAM_INPUT, fds[CPIO_EXEC_STDERR]);
...
{
  TCObject fileStream;
  TCObject fileChannel =
    createObject(context, "java.nio.channels.FileChannelImpl");
  if (fileChannel == NULL) {
    return NULL;
  }
  ...
  fileStream =
    createObject(context,
                 streamType == FILE_STREAM_OUTPUT ? "java.io.FileOutputStream"
