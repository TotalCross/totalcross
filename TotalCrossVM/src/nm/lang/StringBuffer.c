...
static bool
ensureCapacity(Context currentContext, TCObject obj, int32 minimumCapacity)
{
...
appendJCharP(Context currentContext, TCObject obj, JCharP srcPtr, int32 len)
{
  ...
  if ((count + len) > bufferLen &&
      !ensureCapacity(currentContext,
                      obj,
                      count + len)) { // need to increase buffer?
    return null;
  }
...
void
SB_delete(TCObject obj, int32 start, int32 end)
{
...
//////////////////////////////////////////////////////////////////////////
TC_API void
jlSB_ensureCapacity_i(NMParams p)
{ // java/lang/StringBuffer native public void
...
//////////////////////////////////////////////////////////////////////////
TC_API void
jlSB_append_s(NMParams p)
{                           // java/lang/StringBuffer native public
  ...
  if (str == null) {
    ...
    p->retO = appendJCharP(
      p->currentContext, obj, String_charsStart(str), String_charsLen(str));
  }
}
//////////////////////////////////////////////////////////////////////////
TC_API void
jlSB_append_C(NMParams p)
{                           // java/lang/StringBuffer native public
                            // StringBuffer append(char []str);
  TCObject obj = p->obj[0]; // class object
  TCObject str = p->obj[1];
  p->retO = appendJCharP(
    p->currentContext, obj, (JCharP)ARRAYOBJ_START(str), ARRAYOBJ_LEN(str));
}
//////////////////////////////////////////////////////////////////////////
TC_API void
jlSB_append_Cii(NMParams p)
{                           // java/lang/StringBuffer native public StringBuffer
...
//////////////////////////////////////////////////////////////////////////
TC_API void
SB_insertAt_sic(NMParams p)
{ // totalcross/sys/Convert native public static
  // void insertAt(StringBuffer sb, int pos, char c)
  ...
  if (obj == null) {
    ...
    throwException(p->currentContext,
                   ...
                   "Cannot insert beyond position: %d > %d",
                   pos,
                   StringBuffer_count(obj));
  } else if (pos < 0) {
    throwException(p->currentContext,
                   ArrayIndexOutOfBoundsException,
                   "Cannot insert below zero: %d",
                   pos);
  } else {
    int32 remain = StringBuffer_count(obj) - pos;
    // first we append it
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsC_append_sci(NMParams p)
{ // totalcross/sys/Convert native public static void
  // append(StringBuffer sb, char c, int count);
  TCObject obj = p->obj[0];
  int32 count = p->i32[1];
  JChar c = (JChar)p->i32[0];
  if (obj == null) {
    ...
    if ((sbcount + count) > bufferLen &&
        !ensureCapacity(p->currentContext,
                        obj,
                        sbcount + count)) { // need to increase buffer?
      return;
    }
