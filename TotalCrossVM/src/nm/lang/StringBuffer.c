...
TC_API TCObject appendJCharP(Context currentContext, TCObject obj, JCharP srcPtr, int32 len) {
	...
	if ((count + len) > bufferLen
		&& !ensureCapacity(currentContext, obj, count + len)) { // need to increase buffer?
		return null;
	}

	// append
	destPtr = ((JCharP)StringBuffer_charsStart(obj)) +
			  count; // don't cache bc the array may change in ensureCapacity
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_ensureCapacity_i(NMParams
								  p) { // java/lang/StringBuffer native public void ensureCapacity(int minimumCapacity);
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_setLength_i(NMParams
							 p) { // java/lang/StringBuffer native public void setLength(int newLength);
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_append_s(NMParams
						  p) { // java/lang/StringBuffer native public StringBuffer append(String str);
	TCObject obj = p->obj[0]; // class object
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_append_C(NMParams
						  p) { // java/lang/StringBuffer native public StringBuffer append(char []str);
	TCObject obj = p->obj[0]; // class object
	TCObject str = p->obj[1];
	p->retO = appendJCharP(p->currentContext, obj, (JCharP)ARRAYOBJ_START(str), ARRAYOBJ_LEN(str));
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_append_Cii(NMParams
							p) { // java/lang/StringBuffer native public StringBuffer append(char []str, int offset, int len);
	TCObject obj = p->obj[0]; // class object
	TCObject str = p->obj[1];
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_append_c(NMParams
						  p) { // java/lang/StringBuffer native public StringBuffer append(char c);
	TCObject obj = p->obj[0]; // class object
	JChar c = (JChar)p->i32[0];
	p->retO = appendJCharP(p->currentContext, obj, &c, 1);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_append_i(NMParams
						  p) { // java/lang/StringBuffer native public StringBuffer append(int i);
	TCObject obj = p->obj[0]; // class object
	int32 i = p->i32[0];
	IntBuf ib;
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_append_d(NMParams
						  p) { // java/lang/StringBuffer native public StringBuffer append(double d);
	TCObject obj = p->obj[0]; // class object
	double d = p->dbl[0];
	DoubleBuf db;
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_append_l(NMParams
						  p) { // java/lang/StringBuffer native public StringBuffer append(long l)
	TCObject obj = p->obj[0]; // class object
	int64 i = p->i64[0];
	LongBuf lb;
...
//////////////////////////////////////////////////////////////////////////
TC_API void SB_insertAt_sic(NMParams
							p) { // totalcross/sys/Convert native public static void insertAt(StringBuffer sb, int pos, char c)
	TCObject obj = p->obj[0], sb;
	int32 pos = p->i32[0];
	JChar c = (JChar)p->i32[1];
	if (obj == null) {
		throwNullArgumentException(p->currentContext, "sb");
	} else if (pos > StringBuffer_count(obj)) {
		throwException(p->currentContext, ArrayIndexOutOfBoundsException,
					   "Cannot insert beyond position: %d > %d", pos, StringBuffer_count(obj));
	} else if (pos < 0) {
		throwException(p->currentContext, ArrayIndexOutOfBoundsException, "Cannot insert below zero: %d",
					   pos);
	} else {
		int32 remain = StringBuffer_count(obj) - pos;
		// first we append it
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsC_append_sci(NMParams
						   p) { // totalcross/sys/Convert native public static void append(StringBuffer sb, char c, int count);
	TCObject obj = p->obj[0];
	int32 count = p->i32[1];
	JChar c = (JChar)p->i32[0];
	if (obj == null) {
		...
		if ((sbcount + count) > bufferLen
			&& !ensureCapacity(p->currentContext, obj, sbcount + count)) { // need to increase buffer?
			return;
		}

		// append
		destPtr = ((JCharP)StringBuffer_charsStart(obj)) +
				  sbcount; // don't cache bc the array may change in ensureCapacity
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlSB_delete_ii(NMParams
						   p) { // java/lang/StringBuffer native public StringBuffer delete(int start, int end);
	p->retO = p->obj[0];
