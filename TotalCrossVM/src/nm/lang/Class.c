...
void createClassObject(Context currentContext, CharP className, Type type, TCObject* ret,
					   bool* isNew) {
	...
	if (*ret == null && strEqn(className, "java.lang.", 10)) {
		char* p = className + 10;
		if (strEq(p, "Void")      && voidTYPE    != null) {
			*ret = *voidTYPE;
			return;
		}
		if (strEq(p, "Byte")      && byteTYPE    != null) {
			*ret = *byteTYPE;
			return;
		}
		if (strEq(p, "Boolean")   && booleanTYPE != null) {
			*ret = *booleanTYPE;
			return;
		}
		if (strEq(p, "Short")     && shortTYPE   != null) {
			*ret = *shortTYPE;
			return;
		}
		if (strEq(p, "Character") && charTYPE    != null) {
			*ret = *charTYPE;
			return;
		}
		if (strEq(p, "Integer")   && intTYPE     != null) {
			*ret = *intTYPE;
			return;
		}
		if (strEq(p, "Long")      && longTYPE    != null) {
			*ret = *longTYPE;
			return;
		}
		if (strEq(p, "Float")     && floatTYPE   != null) {
			*ret = *floatTYPE;
			return;
		}
		if (strEq(p, "Double")    && doubleTYPE  != null) {
			*ret = *doubleTYPE;
			return;
		}
	}
	...
	if (*ret == null) {
		if (c != null &&
			...
			setObjectLock(Class_targetName(*ret) = createStringObjectFromCharP(currentContext, className, -1),
						  UNLOCKED);
		}
		if (ptrObj != null) {
...
static void createFieldObject(Context currentContext, Field f, int32 idx, TCObject* ret) {
	...
	*ret = null;
	if ((*ret = createObject(currentContext, "java.lang.reflect.Field")) != null) {
		...
		setObjectLock(Field_name(*ret) = createStringObjectFromCharP(currentContext, f->name, -1),
					  UNLOCKED);
...
static void createMethodObject(Context currentContext, Method m, TCClass declaringClass,
							   TCObject* ret, bool isConstructor) { // also valid for Constructors
	TCObject ptrObj = null;
	*ret = null;
	if ((*ret = createObject(currentContext,
							 isConstructor ? "java.lang.reflect.Constructor" : "java.lang.reflect.Method")) != null) {
		// modifiers
		int32 mod = 0, i, n;
		if (m->flags.isFinal) {
			mod |= JFLAG_FINAL;
		...
		setObjectLock(Method_name(*ret) = createStringObjectFromCharP(currentContext,
										  isConstructor ? declaringClass->name : m->name, -1), UNLOCKED);
		createClassObject(currentContext, declaringClass->name, Type_Null, &Method_declaringClass(*ret),
						  null);
		// parameters and exceptions
		Method_parameterTypes(*ret) = createArrayObject(currentContext, "[java.lang.Class",
									  n = m->paramCount);
		if (Method_parameterTypes(*ret) && n > 0) {
			TCObject* oa = (TCObject*)ARRAYOBJ_START(Method_parameterTypes(*ret));
			for (i = 0; i < n; i++) {
				createClassObject(currentContext, declaringClass->cp->cls[m->cpParams[i]],
								  m->cpParams[i] < Type_Object ? m->cpParams[i] : Type_Null, oa++, null);
			}
		}
		Method_exceptionTypes(*ret) = createArrayObject(currentContext, "[java.lang.Class",
									  n = 0); // thrown exceptions is not stored in TCClass!
		...
		if (!isConstructor) {
			...
			createClassObject(currentContext, nn,
							  m->cpReturn == Type_Null ? Type_Void : m->cpReturn < Type_Object ? m->cpReturn : Type_Null,
							  &Method_returnType(*ret), null);
...
static void getFieldByName(NMParams p, bool onlyPublic) {
	...
	} else if ((name = String2CharP(nameObj)) == null) {
		throwException(p->currentContext, OutOfMemoryError, null);
	} else {
		o = getTargetClass(me);
		for (ff = o->i32InstanceFields, i = ARRAYLENV(ff), ff += i - 1; --i >= 0; --ff)
			if (((onlyPublic && ff->flags.isPublic) || (!onlyPublic && !ff->flags.isInherited))
				&& strEq(ff->name, name)) {
				found = ff;
				goto cont;
			}
		for (ff = o->objInstanceFields, i = ARRAYLENV(ff), ff += i - 1; --i >= 0; --ff)
			if (((onlyPublic && ff->flags.isPublic) || (!onlyPublic && !ff->flags.isInherited))
				&& strEq(ff->name, name)) {
				found = ff;
				goto cont;
			}
		for (ff = o->v64InstanceFields, i = ARRAYLENV(ff), ff += i - 1; --i >= 0; --ff)
			if (((onlyPublic && ff->flags.isPublic) || (!onlyPublic && !ff->flags.isInherited))
				&& strEq(ff->name, name)) {
				found = ff;
				goto cont;
			}
		for (ff = o->i32StaticFields, i = ARRAYLENV(ff), ff += i - 1; --i >= 0; --ff)
			if (((onlyPublic && ff->flags.isPublic) || (!onlyPublic && !ff->flags.isInherited))
				&& strEq(ff->name, name)) {
				found = ff;
				goto cont;
			}
		for (ff = o->objStaticFields, i = ARRAYLENV(ff), ff += i - 1; --i >= 0; --ff)
			if (((onlyPublic && ff->flags.isPublic) || (!onlyPublic && !ff->flags.isInherited))
				&& strEq(ff->name, name)) {
				found = ff;
				goto cont;
			}
		for (ff = o->v64StaticFields, i = ARRAYLENV(ff), ff += i - 1; --i >= 0; --ff)
			if (((onlyPublic && ff->flags.isPublic) || (!onlyPublic && !ff->flags.isInherited))
				&& strEq(ff->name, name)) {
				found = ff;
				goto cont;
...
CharP getParameterType(TCClass c, Type t) {
	switch (t) {
		...
		case Type_Double:
			return "java.lang.Double";   // if (strEq(pt,"java.lang.Float")) po = pt;
		default:
			return c->cp->cls[t];
	}
}
...
static void getFields(NMParams p, bool onlyPublic) {
	...
	if (o->flags.isInterface) {
		...
		count += ARRAYLENV(o->i32StaticFields) + ARRAYLENV(o->objStaticFields) + ARRAYLENV(
					 o->v64StaticFields);
		ret = createArrayObject(p->currentContext, "[java.lang.reflect.Field", count);
		if (ret) {
			TCObject* oa = (TCObject*)ARRAYOBJ_START(ret);
			for (ff = o->i32StaticFields, i = ARRAYLENV(ff), ff += i - 1; --i >= 0; --ff) {
				createFieldObject(p->currentContext, ff, i, oa++);
			}
			for (ff = o->objStaticFields, i = ARRAYLENV(ff), ff += i - 1; --i >= 0; --ff) {
				createFieldObject(p->currentContext, ff, i, oa++);
			}
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_forName_s(NMParams
						  p) { // java/lang/Class native public static Class forName(String className) throws java.lang.ClassNotFoundException;
	...
	if (classNameObj == NULL) {
		...
		fail:
			if (p->retO == null || (isNew && !isNew0)) {
				xfree(className);    // the classname is stored in the structure!
			}
		}
	}
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_newInstance(NMParams
							p) { // java/lang/Class native public Object newInstance() throws java.lang.InstantiationException, java.lang.IllegalAccessException;
	TCClass target;
	TCObject me = p->obj[0];
	...
	if (target->flags.isInterface || target->flags.isAbstract || target->flags.isArray) {
		...
		if (!m) { // check if the default constructor exists
			throwException(p->currentContext, InstantiationException, "Class %s has no default constructor",
						   target->name);
		} else if (!m->flags.isPublic) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_isInstance_o(NMParams
							 p) { // java/lang/Class native public boolean isInstance(Object obj);
	TCClass target;
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_isAssignableFrom_c(NMParams
								   p) { // java/lang/Class public native boolean isAssignableFrom(Class cls);
...
static bool isPrimitive(TCObject me) {
	return me == *booleanTYPE || me == *byteTYPE || me == *shortTYPE || me == *intTYPE
		   || me == *longTYPE || me == *floatTYPE || me == *doubleTYPE || me == *charTYPE;
}
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getInterfaces(NMParams
							  p) { // java/lang/Class public native java.lang.Class[] getInterfaces();
	TCClass target;
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getComponentType(NMParams
								 p) { // java/lang/Class public native Class getComponentType();
	TCClass target;
	TCObject me = p->obj[0];
	target = getTargetClass(me);
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getFields(NMParams
						  p) { // java/lang/Class public native java.lang.reflect.Field[] getFields() throws SecurityException;
	getFields(p, true);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getMethods(NMParams
						   p) { // java/lang/Class public native java.lang.reflect.Method[] getMethods() throws SecurityException;
	getMCarray(p, false, true);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getConstructors(NMParams
								p) { // java/lang/Class public native java.lang.reflect.Constructor[] getConstructors() throws SecurityException;
	getMCarray(p, true, true);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getField_s(NMParams
						   p) { // java/lang/Class public native java.lang.reflect.Field getField(String name) throws NoSuchFieldException, SecurityException;
	getFieldByName(p, true);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getMethod_sC(NMParams
							 p) { // java/lang/Class public native java.lang.reflect.Method getMethod(String name, Class []parameterTypes) throws NoSuchMethodException, SecurityException;
	TCObject nameObj = p->obj[1];
	CharP name;
	if (nameObj == NULL) {
		throwException(p->currentContext, NullPointerException, null);
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getConstructor_C(NMParams
								 p) { // java/lang/Class public native java.lang.reflect.Constructor getConstructor(Class []parameterTypes) throws NoSuchMethodException, SecurityException;
	getMCbyName(p, CONSTRUCTOR_NAME, true, true);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getDeclaredFields(NMParams
								  p) { // java/lang/Class public native java.lang.reflect.Field[] getDeclaredFields() throws SecurityException;
	getFields(p, false);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getDeclaredMethods(NMParams
								   p) { // java/lang/Class public native java.lang.reflect.Method[] getDeclaredMethods() throws SecurityException;
	getMCarray(p, false, false);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getDeclaredConstructors(NMParams
										p) { // java/lang/Class public native java.lang.reflect.Constructor[] getDeclaredConstructors() throws SecurityException;
	getMCarray(p, true, false);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getDeclaredField_s(NMParams
								   p) { // java/lang/Class public native java.lang.reflect.Field getDeclaredField(String name) throws NoSuchFieldException, SecurityException;
	getFieldByName(p, false);
}
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getDeclaredMethod_sC(NMParams
									 p) { // java/lang/Class public native java.lang.reflect.Method getDeclaredMethod(String name, Class []parameterTypes) throws NoSuchMethodException, SecurityException;
	TCObject nameObj = p->obj[1];
	CharP name;
	if (nameObj == NULL) {
		throwException(p->currentContext, NullPointerException, null);
...
//////////////////////////////////////////////////////////////////////////
TC_API void jlC_getDeclaredConstructor_C(NMParams
		p) { // java/lang/Class public native java.lang.reflect.Constructor getDeclaredConstructor(Class []parameterTypes) throws NoSuchMethodException, SecurityException;
