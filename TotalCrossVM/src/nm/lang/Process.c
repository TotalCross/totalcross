...
jlPI_waitFor(NMParams p)
{
  ...
  if (err != 0) {
    if (err == EINTR) {
      throwExceptionNamed(
        p->currentContext, "java.lang.InterruptedException", NULL);
    }
    return;
  }
...
jlPI_exitValue(NMParams p)
{
  ...
  if (err == 0 && pid == 0) {
    throwExceptionNamed(
      p->currentContext, "java.lang.IllegalThreadStateException", NULL);
    return;
  }
