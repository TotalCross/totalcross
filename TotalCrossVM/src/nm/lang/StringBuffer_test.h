...
TESTCASE(jlSB_aensureCapacity_i)
...
{ // java/lang/StringBuffer native public StringBuffer
  ...
  jlSB_append_s(&p);
  JCharP2CharPBuf(
    StringBuffer_charsStart(objs[0]), StringBuffer_charsLen(objs[0]), buf);
...
{ // java/lang/StringBuffer native public StringBuffer
  ...
  is[0] = 0;
  is[1] = 8; // ArrayIndexOutOfBoundsException
  jlSB_append_Cii(&p);
  JCharP2CharPBuf(
    StringBuffer_charsStart(objs[0]), StringBuffer_charsLen(objs[0]), buf);
...
static CharP
sbAppend(Context currentContext, CharP buf, TCObject sb, TValue v, RegType t)
{
  TNMParams p;
...
{ // java/lang/StringBuffer native public StringBuffer
  ...
  v.asInt32 = 0;
  ASSERT2_EQUALS(Sz, sbAppend(currentContext, buf, sb, v, RegI), "0");
  v.asInt32 = 1234;
  ASSERT2_EQUALS(Sz, sbAppend(currentContext, buf, sb, v, RegI), "01234");
  v.asInt32 = -5321;
  ASSERT2_EQUALS(Sz, sbAppend(currentContext, buf, sb, v, RegI), "01234-5321");
  v.asInt32 = 0xFFFFFFF;
  ASSERT2_EQUALS(
    Sz, sbAppend(currentContext, buf, sb, v, RegI), "01234-5321268435455");

finish:;
}
TESTCASE(jlSB_append_l)
{ // java/lang/StringBuffer native public StringBuffer
  // append(long l)
  char buf[50];
  TCObject sb;
  ...
  v.asInt64 = 0;
  ASSERT2_EQUALS(Sz, sbAppend(currentContext, buf, sb, v, RegL), "0");
  v.asInt64 = 1234;
  ASSERT2_EQUALS(Sz, sbAppend(currentContext, buf, sb, v, RegL), "01234");
  v.asInt64 = -5321;
  ASSERT2_EQUALS(Sz, sbAppend(currentContext, buf, sb, v, RegL), "01234-5321");
  v.asInt64 = 0xFFFFFFF;
  ASSERT2_EQUALS(
    Sz, sbAppend(currentContext, buf, sb, v, RegL), "01234-5321268435455");
  v.asInt64 = I64_CONST(0xFFFFFFFFFFFF);
  ASSERT2_EQUALS(Sz,
                 sbAppend(currentContext, buf, sb, v, RegL),
                 "01234-5321268435455281474976710655");

finish:;
}
TESTCASE(jlSB_append_d)
{ // java/lang/StringBuffer native public StringBuffer
  // append(double d);
  char buf[128];
  TCObject sb;
  ...
  ASSERT2_EQUALS(
    Sz,
    JCharP2CharPBuf(StringBuffer_charsStart(sb), StringBuffer_count(sb), buf),
    "0.01234.0-5321.00.5432424234-13132.5153513");
  ...
  ASSERT2_EQUALS(
    Sz,
    JCharP2CharPBuf(StringBuffer_charsStart(sb), StringBuffer_count(sb), buf),
    "0.01234.0-5321.00.5432424234-13132.51535131.0E300");
