...
static TCObject getMessageDigest(Context currentContext, TCObject digest, TCObject data) {
	...
	executeMethod(currentContext, getMethod(OBJ_CLASS(digest), true, "reset", 0),
				  digest); // reset digest
	if (currentContext->thrownException != null) {
		return null;
	}
	executeMethod(currentContext, getMethod(OBJ_CLASS(digest), true, "update", 1, BYTE_ARRAY), digest,
				  data); // update digest data with the message to sign
	if (currentContext->thrownException != null) {
		return null;
	}
	res = executeMethod(currentContext, getMethod(OBJ_CLASS(digest), true, "getDigest", 0),
						digest).asObj; // get the message digest
	if (currentContext->thrownException != null) {
		return null;
	}
...
static int32 createInfo(Context currentContext, TCObject digest, uint8* data, int32 dataLen,
						uint8** buf) {
	TCObject digestName;
	char name[64];
	uint8* hashInfo;
	...
	digestName = executeMethod(currentContext, getMethod(OBJ_CLASS(digest), true, "getAlgorithm", 0),
							   digest).asObj;
	if (currentContext->thrownException != null) {
		return 0;
	}
...
// packets are formed by the message digest prepended with the digest algorithm information.
static int32 retrieveInfo(Context currentContext, TCObject digest, uint8* data, int32 dataLen,
						  uint8** buf) {
	TCObject digestName;
	int32 digestLen;
	char name[64];
	...
	digestName = executeMethod(currentContext, getMethod(OBJ_CLASS(digest), true, "getAlgorithm", 0),
							   digest).asObj;
	digestLen = executeMethod(currentContext, getMethod(OBJ_CLASS(digest), true, "getDigestLength", 0),
							  digest).asInt32;
	String2CharPBuf(digestName, name);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tcsPKCS1S_init(NMParams
						   p) { // totalcross/crypto/signature/PKCS1Signature native private void init();
...
//////////////////////////////////////////////////////////////////////////
TC_API void tcsPKCS1S_finalize(NMParams
							   p) { // totalcross/crypto/signature/PKCS1Signature native protected final void finalize();
	TCObject pkcs1Obj = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tcsPKCS1S_doReset(NMParams
							  p) { // totalcross/crypto/signature/PKCS1Signature native protected final void doReset() throws totalcross.crypto.CryptoException;
	TCObject pkcs1Obj = p->obj[0];
	...
	if (operation == OPERATION_SIGN) {
		...
		RSA_priv_key_new_external(ctx, ARRAYOBJ_START(n), ARRAYOBJ_LEN(n), ARRAYOBJ_START(e),
								  ARRAYOBJ_LEN(e), ARRAYOBJ_START(d), ARRAYOBJ_LEN(d));
	} else {
		e = RSAPublicKey_e(key);
		n = RSAPublicKey_n(key);

		RSA_pub_key_new_external(ctx, ARRAYOBJ_START(n), ARRAYOBJ_LEN(n), ARRAYOBJ_START(e),
								 ARRAYOBJ_LEN(e));
	}
}
//////////////////////////////////////////////////////////////////////////
TC_API void tcsPKCS1S_doSign_B(NMParams
							   p) { // totalcross/crypto/signature/PKCS1Signature native protected final byte[] doSign(byte []data) throws totalcross.crypto.CryptoException;
	TCObject pkcs1Obj = p->obj[0];
	...
	count = createInfo(p->currentContext, digest, ARRAYOBJ_START(msgDigest), ARRAYOBJ_LEN(msgDigest),
					   &hashedBuf); // prepend digest with the hash info
	if (p->currentContext->thrownException !=
		null) { // something went wrong createInfo has already thrown an exception.
		goto cleanup;
	}
	...
	count = RSA_encrypt(ctx, hashedBuf, (uint16) count, encBuf,
...
//////////////////////////////////////////////////////////////////////////
TC_API void tcsPKCS1S_doVerify_BB(NMParams
								  p) { // totalcross/crypto/signature/PKCS1Signature native protected final boolean doVerify(byte []data, byte []signature) throws totalcross.crypto.CryptoException;
	TCObject pkcs1Obj = p->obj[0];
	TCObject dataObj = p->obj[1];
	...
	sigDigestLen = RSA_decrypt(ctx, ARRAYOBJ_START(signature), decBuf,
							   ARRAYOBJ_LEN(RSAPublicKey_n(key)), false); // get the original decrypted signature package
	if (sigDigestLen == -1) { // error decrypting
		throwException(p->currentContext, CryptoException, "Signature error");
		goto cleanup;
	}

	sigDigestLen = retrieveInfo(p->currentContext, digest, decBuf, sigDigestLen,
	...
	p->retI = (sigDigestLen == digestLen)
			  && (xmemcmp(sigDigestBuf, ARRAYOBJ_START(msgDigest), digestLen) == 0); // compare digests
