...
static uint8 MD5_HASH_INFO[] = { 0x30, 0x20, 0x30, 0x0C, 0x06, 0x08,
                                 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D,
                                 0x02, 0x05, 0x05, 0x00, 0x04, 0x10 };
static int32 MD5_HASH_INFO_LENGTH = 18;

static uint8 SHA1_HASH_INFO[] = { 0x30, 0x21, 0x30, 0x09, 0x06,
                                  0x05, 0x2b, 0x0E, 0x03, 0x02,
                                  0x1A, 0x05, 0x00, 0x04, 0x14 };
static int32 SHA1_HASH_INFO_LENGTH = 15;

static uint8 SHA256_HASH_INFO[] = { 0x30, 0x31, 0x30, 0x0d, 0x06, 0x09, 0x60,
...
static TCObject
getMessageDigest(Context currentContext, TCObject digest, TCObject data)
{
  TCObject res;

  executeMethod(currentContext,
                getMethod(OBJ_CLASS(digest), true, "reset", 0),
                digest); // reset digest
  if (currentContext->thrownException != null) {
    return null;
  }
  executeMethod(currentContext,
                getMethod(OBJ_CLASS(digest), true, "update", 1, BYTE_ARRAY),
                digest,
                data); // update digest data with the message to sign
  if (currentContext->thrownException != null) {
    return null;
  }
  res = executeMethod(currentContext,
                      getMethod(OBJ_CLASS(digest), true, "getDigest", 0),
                      digest)
          .asObj; // get the message digest
  if (currentContext->thrownException != null) {
    return null;
  }
...
// This method creates the signature packet from the message digest. Basically,
// PKCS #1 v1.5 packets are formed by the message digest prepended with the
// digest algorithm information.
static int32
createInfo(Context currentContext,
           TCObject digest,
           uint8* data,
           int32 dataLen,
           uint8** buf)
{
  ...
  digestName =
    executeMethod(currentContext,
                  getMethod(OBJ_CLASS(digest), true, "getAlgorithm", 0),
                  digest)
      .asObj;
  if (currentContext->thrownException != null) {
    return 0;
  ...
  if (xstrcmp(name, "MD5") == 0) {
    ...
    throwException(
      currentContext, RuntimeException, "Algorithm %s was not found", name);
    return 0;
  }
...
// This method retrieves the message digest from the signature packet.
// Basically, PKCS #1 v1.5 packets are formed by the message digest prepended
// with the digest algorithm information.
static int32
retrieveInfo(Context currentContext,
             TCObject digest,
             uint8* data,
             int32 dataLen,
             uint8** buf)
{
  ...
  digestLen =
    executeMethod(currentContext,
                  getMethod(OBJ_CLASS(digest), true, "getDigestLength", 0),
                  digest)
      .asInt32;
  String2CharPBuf(digestName, name);
  ...
  if (dataLen != (digestLen + expectedHashInfoLen) ||
      memcmp(data, expectedHashInfo, expectedHashInfoLen) != 0) {
    return -1;
  }
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tcsPKCS1S_doReset(NMParams p)
{ // totalcross/crypto/signature/PKCS1Signature
  // native protected final void doReset() throws
  ...
  if (ctx->m) { // already initialized?
    RSA_free_external(ctx);
    ctx->d = null; // necessary to avoid freeing ctx->d when it is not allocated
                   // (in public keys)
  }
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tcsPKCS1S_doSign_B(NMParams p)
{ // totalcross/crypto/signature/PKCS1Signature native protected
  // final byte[] doSign(byte []data) throws
  // totalcross.crypto.CryptoException;
  TCObject pkcs1Obj = p->obj[0];
  ...
  msgDigest = getMessageDigest(p->currentContext,
                               digest,
                               dataObj); // compute the message digest
  if (p->currentContext->thrownException != null) {
    return;
  }

  count = createInfo(p->currentContext,
                     digest,
                     ARRAYOBJ_START(msgDigest),
                     ARRAYOBJ_LEN(msgDigest),
                     &hashedBuf); // prepend digest with the hash info
  if (p->currentContext->thrownException !=
      null) { // something went wrong createInfo has already thrown an
              // exception.
    goto cleanup;
  }
  ...
  count =
    RSA_encrypt(ctx,
                ...
                (uint16)count,
                encBuf,
                true); // encrypt the digest, obtaining the message signature
  if (count == -1) {
    throwException(p->currentContext, CryptoException, "Signature error");
  } else if ((byteArrayResult = createByteArray(p->currentContext, count)) !=
             null) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tcsPKCS1S_doVerify_BB(NMParams p)
{ // totalcross/crypto/signature/PKCS1Signature native protected
  // final boolean doVerify(byte []data, byte []signature)
  // throws totalcross.crypto.CryptoException;
  TCObject pkcs1Obj = p->obj[0];
  TCObject dataObj = p->obj[1];
  ...
  sigDigestLen =
    RSA_decrypt(ctx,
                ARRAYOBJ_START(signature),
                decBuf,
                ARRAYOBJ_LEN(RSAPublicKey_n(key)),
                false);     // get the original decrypted signature package
  if (sigDigestLen == -1) { // error decrypting
    throwException(p->currentContext, CryptoException, "Signature error");
    goto cleanup;
  }

  sigDigestLen = retrieveInfo(p->currentContext,
                              digest,
                              decBuf,
                              sigDigestLen,
                              &sigDigestBuf); // get the original message digest
  ...
  // 3. Compare digests
  p->retI = (sigDigestLen == digestLen) &&
            (xmemcmp(sigDigestBuf, ARRAYOBJ_START(msgDigest), digestLen) ==
             0); // compare digests
