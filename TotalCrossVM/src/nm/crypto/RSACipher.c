...
//////////////////////////////////////////////////////////////////////////
TC_API void
tccRSAC_init(NMParams p)
{ // totalcross/crypto/digest/RSACipher
  // native private void init();
  TCObject rsacObj = p->obj[0];
  TCObject cipherObj;

  if ((cipherObj = createByteArray(p->currentContext, sizeof(RSA_CTX))) !=
      null) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tccRSAC_finalize(NMParams p)
{ // totalcross/crypto/digest/RSACipher native
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tccRSAC_doReset(NMParams p)
{ // totalcross/crypto/cipher/RSACipher native protected final
  // void doReset() throws totalcross.crypto.CryptoException;
  TCObject rsacObj = p->obj[0];
  TCObject cipherObj = *Cipher_cipherRef(rsacObj);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tccRSAC_process_B(NMParams p)
{ // totalcross/crypto/cipher/RSACipher native
  // protected byte[] process(byte []data) throws
  ...
  if (operation == OPERATION_ENCRYPT) {
    if ((out = xmalloc(ARRAYOBJ_LEN(RSAPublicKey_n(key)))) == null) {
      ...
      if ((count = RSA_encrypt(ctx,
                               ARRAYOBJ_START(dataObj),
                               (uint16)ARRAYOBJ_LEN(dataObj),
                               out,
                               false)) == -1) {
    ...
    if ((out = xmalloc(ARRAYOBJ_LEN(RSAPrivateKey_n(key)))) == null) {
      ...
      if ((count = RSA_decrypt(ctx,
                               ARRAYOBJ_START(dataObj),
                               out,
                               ARRAYOBJ_LEN(RSAPrivateKey_n(key)),
                               true)) == -1) {
