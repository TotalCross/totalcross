...
jncFCI_read(NMParams p)
{
  ...
  if (ret < 0) {
    throwExceptionNamed(
      p->currentContext, "java.io.IOException", strerror(errno));
    return;
  }
  p->retI = buffer;
#else
...
TC_API void
jncFCI_implCloseChannel(NMParams p)
{
#if defined(linux) && !defined(darwin)
  TCObject fileChannel = p->obj[0];
  int32 fd = FileChannelImpl_nfd(fileChannel);
