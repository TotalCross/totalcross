...
static int32 add_1(int32* dest, int32* x, int32 size, int32 y) {
	...
	while (--size >= 0) {
		carry += ((int64) * x++ & LMASK);
		*dest++ = (int32) carry;
		carry = (int64)(((uint64)carry) >>
						32); // !!!! estava carry >>= 32; diferente de todos os outros, que usava >>>
	}
	return (int32) carry;
}
...
static int64 udiv_qrnnd(int64 N, int32 D) {
	...
	int64 a0 = N & LMASK;
	if (D >= 0) {
		if (a1 < (int64)((D - a1 - ((int64)((uint64)a0 >> 31))) &
						 LMASK)) { // if (a1 < ((D - a1 - (a0 >>> 31)) & 0xffffffffL))
...
static int32 rshift(int32* dest, int32* x, int32 x_start, int32 len, int32 count) {
	...
	for (; i < len; i++) {
		...
		dest[i - 1] = (int32)((uint32)low_word >> count) | ((int32)high_word <<
					  count_2);  // dest[i - 1] = (low_word >>> count) | (high_word << count_2);
		low_word = high_word;
	}
...
static int64 rshift_long(int32* x, int32 len, int32 count) {
	...
	if (count != 0) {
		...
		w0 = (int32)(((uint32)w0 >> count) | ((int32)w1 << (32 -
											  count))); // w0 = (w0 >>> count) | (w1 << (32 - count));
		w1 = (int32)(((uint32)w1 >> count) | ((int32)w2 << (32 -
											  count))); // w1 = (w1 >>> count) | (w2 << (32 - count));
	}
...
static int32 lshift(int32* dest, int32 d_offset, int32* x, int32 len, int32 count) {
	...
	int32 retval = high_word >>
				   count_2; // int retval = high_word >>> count_2; - ps: high_word is already unsigned
	d_offset++;
	while (--i >= 0) {
...
static int32 gcd(int32* x, int32* y, int32 len) {
	int32* odd_arg, *other_arg,
		   *tmp; /* One of x or y which is odd. */ /* The other one can be even or odd. */
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_add_1_IIii(NMParams
							p) { // totalcross/util/BigInteger native static int add_1(int []dest, int []x, int size, int y);
	p->retI = add_1((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]), p->i32[0],
					p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_add_n_IIIi(NMParams
							p) { // totalcross/util/BigInteger native static int add_n(int []dest, int []x, int []y, int len);
	p->retI = add_n((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]),
					(int32*)ARRAYOBJ_START(p->obj[2]), p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_sub_n_IIIi(NMParams
							p) { // totalcross/util/BigInteger native static int sub_n(int []dest, int []x, int []y, int size);
	p->retI = sub_n((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]),
					(int32*)ARRAYOBJ_START(p->obj[2]), p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_mul_1_IIii(NMParams
							p) { // totalcross/util/BigInteger native static int mul_1(int []dest, int []x, int len, int y);
	p->retI = mul_1((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]), p->i32[0],
					p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_mul_IIiIi(NMParams
						   p) { // totalcross/util/BigInteger native static void mul(int []dest, int []x, int xlen, int []y, int ylen);
	mul((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]), p->i32[0],
		(int32*)ARRAYOBJ_START(p->obj[2]), p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_udiv_qrnnd_li(NMParams
							   p) { // totalcross/util/BigInteger native static long udiv_qrnnd(long n, int d);
	p->retL = udiv_qrnnd(p->i64[0], p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_divmod_1_IIii(NMParams
							   p) { // totalcross/util/BigInteger native static int divmod_1(int []quotient, int []dividend, int len, int divisor);
	p->retI = divmod_1((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]), p->i32[0],
					   p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_submul_1_IiIii(NMParams
								p) { // totalcross/util/BigInteger native static int submul_1(int []dest, int offset, int []x, int len, int y);
	p->retI = submul_1((int32*)ARRAYOBJ_START(p->obj[0]), p->i32[0], (int32*)ARRAYOBJ_START(p->obj[1]),
					   p->i32[1], p->i32[2]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_divide_IiIi(NMParams
							 p) { // totalcross/util/BigInteger native static void divide(int []zds, int nx, int []y, int ny);
	divide((int32*)ARRAYOBJ_START(p->obj[0]), p->i32[0], (int32*)ARRAYOBJ_START(p->obj[1]), p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_chars_per_word_i(NMParams
								  p) { // totalcross/util/BigInteger native static int chars_per_word(int radix);
	p->retI = chars_per_word(p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_count_leading_zeros_i(NMParams
									   p) { // totalcross/util/BigInteger native static int count_leading_zeros(int i);
	p->retI = count_leading_zeros(p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_set_str_IBii(NMParams
							  p) { // totalcross/util/BigInteger native static int set_str(int []dest, byte []str, int str_len, int base);
	p->retI = set_str((int32*)ARRAYOBJ_START(p->obj[0]), (int8*)ARRAYOBJ_START(p->obj[1]), p->i32[0],
					  p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_cmp_IIi(NMParams
						 p) { // totalcross/util/BigInteger native static int cmp(int []x, int []y, int size);
	p->retI = cmp((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]), p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_cmp_IiIi(NMParams
						  p) { // totalcross/util/BigInteger native static int cmp(int []x, int xlen, int []y, int ylen);
	p->retI = cmp2((int32*)ARRAYOBJ_START(p->obj[0]), p->i32[0], (int32*)ARRAYOBJ_START(p->obj[1]),
				   p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_rshift_IIiii(NMParams
							  p) { // totalcross/util/BigInteger native static int rshift(int []dest, int []x, int x_start, int len, int count);
	p->retI = rshift((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]), p->i32[0],
					 p->i32[1], p->i32[2]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_rshift0_IIiii(NMParams
							   p) { // totalcross/util/BigInteger native static void rshift0(int []dest, int []x, int x_start, int len, int count);
	rshift0((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]), p->i32[0], p->i32[1],
			p->i32[2]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_rshift_long_Iii(NMParams
								 p) { // totalcross/util/BigInteger native static long rshift_long(int []x, int len, int count);
	p->retL = rshift_long((int32*)ARRAYOBJ_START(p->obj[0]), p->i32[0], p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_lshift_IiIii(NMParams
							  p) { // totalcross/util/BigInteger native static int lshift(int []dest, int d_offset, int []x, int len, int count);
	p->retI = lshift((int32*)ARRAYOBJ_START(p->obj[0]), p->i32[0], (int32*)ARRAYOBJ_START(p->obj[1]),
					 p->i32[1], p->i32[2]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_findLowestBit_i(NMParams
								 p) { // totalcross/util/BigInteger native static int findLowestBit(int word);
	p->retI = findLowestBit(p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_findLowestBit_I(NMParams
								 p) { // totalcross/util/BigInteger native static int findLowestBit(int []words);
	p->retI = findLowestBit2((int32*)ARRAYOBJ_START(p->obj[0]));
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_gcd_IIi(NMParams
						 p) { // totalcross/util/BigInteger native static int gcd(int []x, int []y, int len);
	p->retI = gcd((int32*)ARRAYOBJ_START(p->obj[0]), (int32*)ARRAYOBJ_START(p->obj[1]), p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_intLength_i(NMParams
							 p) { // totalcross/util/BigInteger native static int intLength(int i);
	p->retI = intLength(p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuBI_intLength_Ii(NMParams
							  p) { // totalcross/util/BigInteger native static int intLength(int []words, int len);
