...
static int32 commonDeflateInflate(Context currentContext, int32 compress, int32 buffersize,
								  int32 levelOrSizeIn, int32 strategy, bool noWrap, TCObject in, TCObject out) {
	...
	if (compress == UNCOMPRESS) {
		err = inflateInit2(&c_stream, 32 +
						   MAX_WBITS);    // windowBits can also be greater than 15 for optional gzip decoding. Add 32 to windowBits to enable zlib and gzip decoding with automatic header detection, or add 16 to decode only the gzip format(the zlib format will return a Z_DATA_ERROR).
	} else {
		int32 bits = compress == COMPRESS_GZIP ? (compress + MAX_WBITS) : (noWrap ? -MAX_WBITS :
					 MAX_WBITS); // flsobral@tc123b_68: GZIP negative window bits is for "no wrap", GZIP uses its own value.
	...
	if (compress != UNCOMPRESS) {
		levelOrSizeIn =
			-1;    // if compressing, level was already used, so we reset it to avoid confusing with sizeIn
	}
	...
	do {
		if (c_stream.avail_out == 0) {
			...
			do {
				wrote += executeMethod(currentContext, writeMethod, out, outByteArray, wrote,
									   count - wrote).asInt32;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZL_deflate_ssiib(NMParams
								p) { // totalcross/util/zip/ZLib native public static int deflate(totalcross.io.Stream in, totalcross.io.Stream out, int compressionLevel, int strategy, boolean noWrap) throws IOException;
	...
	if (!streamIn) {
		...
		if (level < -1
			|| level > 9) { //flsobraltc123a_64: removed else that caused GZip to jump over the deflate method.
			throwIllegalArgumentExceptionI(p->currentContext, "compressionLevel", level);
		} else {
			p->retI = commonDeflateInflate(p->currentContext, compressionType, BUFSIZE, level, strategy, noWrap,
										   streamIn, streamOut);
		}
	}
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZL_inflate_ssib(NMParams
							   p) { // totalcross/util/zip/ZLib native public static int inflate(totalcross.io.Stream in, totalcross.io.Stream out, int sizeIn, boolean noWrap) throws IOException, ZipException;
	TCObject streamIn = p->obj[0];
	TCObject streamOut = p->obj[1];
	...
	if (!streamIn) {
		...
		p->retI = commonDeflateInflate(p->currentContext, UNCOMPRESS, BUFSIZE, sizeIn, 0, noWrap, streamIn,
									   streamOut);
	}
}
