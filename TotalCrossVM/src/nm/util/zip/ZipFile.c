...
static bool initializeZipNative(Context currentContext, ZipNativeP zipNativeP, TCObject streamObj) {
	zipNativeP->streamRead  = getMethod(OBJ_CLASS(streamObj), true, "readBytes", 3, BYTE_ARRAY, J_INT,
										J_INT);
	zipNativeP->streamWrite = getMethod(OBJ_CLASS(streamObj), true, "writeBytes", 3, BYTE_ARRAY, J_INT,
										J_INT);
...
TC_API void tuzZF_createZipFile_f(NMParams
								  p) { // totalcross/util/zip/ZipFile native private totalcross.util.zip.ZipFile createZipFile(totalcross.io.File file) throws totalcross.io.IOException;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZF_close(NMParams
						p) { // totalcross/util/zip/ZipFile native public void close() throws totalcross.io.IOException;
	TCObject zipFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZF_entries(NMParams
						  p) { // totalcross/util/zip/ZipFile native public String[] entries();
	TCObject zipFile = p->obj[0];
	TCObject nativeFile = ZipFile_nativeFile(zipFile);
	unzFile* unzipFile = (unzFile*) ARRAYOBJ_START(nativeFile);
	...
	if (list != null) {
		if ((p->retO = arrayObj = createArrayObject(p->currentContext, "[totalcross.util.zip.ZipEntry",
								  count)) != null) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZF_getEntry_s(NMParams
							 p) { // totalcross/util/zip/ZipFile native public String getEntry(String name);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZF_getEntryStream_s(NMParams
								   p) { // totalcross/util/zip/ZipFile native public totalcross.io.Stream getEntryStream(String name) throws IOException;
	TCObject zipFile = p->obj[0];
	TCObject nativeFile = ZipFile_nativeFile(zipFile);
	unzFile* unzipFile = (unzFile*) ARRAYOBJ_START(nativeFile);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZS_createInflate_s(NMParams
								  p) { // totalcross/util/zip/ZipStream native protected Object createInflate(totalcross.io.Stream stream);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZS_createDeflate_si(NMParams
								   p) { // totalcross/util/zip/ZipStream native protected Object createDeflate(totalcross.io.Stream stream, int compressionType);
	TCObject zipStream = p->obj[0];
	TCObject streamObj = p->obj[1];
	TCObject zipNativeObj;
	ZipNativeP zipNativeP;
	...
	if (streamObj == null) {
		...
		if (!initializeZipNative(p->currentContext, zipNativeP, streamObj)) {
			...
			if ((zipNativeP->zipFile = zipOpen2((char*) streamObj, APPEND_STATUS_CREATE, null,
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZS_available(NMParams
							p) { // totalcross/util/zip/ZipStream native public int available() throws IOException;
	TCObject zipStream = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZS_getNextEntry(NMParams
							   p) { // totalcross/util/zip/ZipStream native public totalcross.util.zip.ZipEntry getNextEntry() throws IOException;
	TCObject zipStream = p->obj[0];
	...
	if (mode != 2) { // INFLATE
		...
		} else if ((zipEntryObj = createObject(p->currentContext,
											   "totalcross.util.zip.ZipEntry")) == null) {
			throwException(p->currentContext, OutOfMemoryError, null);
		} else if ((zipEntryNameObj = createStringObjectFromCharP(p->currentContext, zipEntryName,
									  (int)file_info.size_filename)) == null) {
			throwException(p->currentContext, OutOfMemoryError, null);
		} else if (file_info.size_file_comment > 0
				   && (zipEntryCommentP = (CharP)xmalloc((int)file_info.size_file_comment)) == null) {
			throwException(p->currentContext, OutOfMemoryError, null);
		} else if (file_info.size_file_extra > 0
				   && (zipEntryExtraObj = createByteArray(p->currentContext,
										  (int)file_info.size_file_extra)) == null) {
			throwException(p->currentContext, OutOfMemoryError, null);
		} else if ((err = unzGetCurrentFileInfo(zipNativeP->zipFile, &file_info, zipEntryName, MAX_PATHNAME,
												zipEntryCommentP, file_info.size_file_comment,
												file_info.size_file_comment > 0 ? (CharP)ARRAYOBJ_START(zipEntryCommentObj) : null,
												file_info.size_file_extra)) != UNZ_OK) { // guich@tc126_31: check for comment length
			throwException(p->currentContext, IOException, "Failed to retrieve details for the next entry");
		} else if ((zipEntryCommentObj = createStringObjectFromCharP(p->currentContext, zipEntryCommentP,
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZS_putNextEntry_z(NMParams
								 p) { // totalcross/util/zip/ZipStream native public void putNextEntry(totalcross.util.zip.ZipEntry zipEntry) throws IOException;
	TCObject zipStream = p->obj[0];
	TCObject zipNativeObj = *ZipStream_nativeZip(zipStream);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZS_closeEntry(NMParams
							 p) { // totalcross/util/zip/ZipStream native public void closeEntry() throws IOException;
	TCObject zipStream = p->obj[0];
	TCObject zipNativeObj = *ZipStream_nativeZip(zipStream);
	TCObject lastEntryObj = *ZipStream_lastEntry(zipStream);
	ZipNativeP zipNativeP = (ZipNativeP) ARRAYOBJ_START(zipNativeObj);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZS_readBytes_Bii(NMParams
								p) { // totalcross/util/zip/ZipStream native public int readBytes(byte []buf, int start, int count) throws IOException;
	TCObject zipStream = p->obj[0];
	TCObject zipNativeObj = *ZipStream_nativeZip(zipStream);
	ZipNativeP zipNativeP = (ZipNativeP) ARRAYOBJ_START(zipNativeObj);
	int32 mode = CompressedStream_mode(zipStream);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZS_writeBytes_Bii(NMParams
								 p) { // totalcross/util/zip/ZipStream native public int writeBytes(byte []buf, int start, int count) throws IOException;
	TCObject zipStream = p->obj[0];
	TCObject zipNativeObj = *ZipStream_nativeZip(zipStream);
	ZipNativeP zipNativeP = (ZipNativeP) ARRAYOBJ_START(zipNativeObj);
	int32 mode = CompressedStream_mode(zipStream);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzZS_close(NMParams
						p) { // totalcross/util/zip/ZipStream native public void close() throws IOException;
	TCObject zipStream = p->obj[0];
	TCObject zipNativeObj = *ZipStream_nativeZip(zipStream);
	TCObject lastEntryObj = *ZipStream_lastEntry(zipStream);
	ZipNativeP zipNativeP = (ZipNativeP) ARRAYOBJ_START(zipNativeObj);
