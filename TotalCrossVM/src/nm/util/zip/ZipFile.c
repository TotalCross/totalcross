...
static bool
initializeZipNative(Context currentContext,
                    ZipNativeP zipNativeP,
                    TCObject streamObj)
{
  zipNativeP->streamRead = getMethod(
    OBJ_CLASS(streamObj), true, "readBytes", 3, BYTE_ARRAY, J_INT, J_INT);
  zipNativeP->streamWrite = getMethod(
    OBJ_CLASS(streamObj), true, "writeBytes", 3, BYTE_ARRAY, J_INT, J_INT);
  zipNativeP->streamTell = getMethod(OBJ_CLASS(streamObj), true, "getPos", 0);
  zipNativeP->streamSeek =
    getMethod(OBJ_CLASS(streamObj), true, "setPos", 2, J_INT, J_INT);
  zipNativeP->streamClose = getMethod(OBJ_CLASS(streamObj), true, "close", 0);

  if (zipNativeP->streamRead == null || zipNativeP->streamWrite == null ||
      zipNativeP->streamTell == null || zipNativeP->streamSeek == null ||
      zipNativeP->streamClose == null) {
    return false;
  }
...
//////////////////////////////////////////////////////////////////////////
// TC_API void tuzZF_createZipFile_s(NMParams p) // totalcross/util/zip/ZipFile
// native private totalcross.util.zip.ZipFile createZipFile(String name) throws
// totalcross.io.IOException;
TC_API void
tuzZF_createZipFile_f(NMParams p)
{ // totalcross/util/zip/ZipFile native
  // private totalcross.util.zip.ZipFile
  // createZipFile(totalcross.io.File file)
  ...
  if ((zipNativeObj = createByteArray(p->currentContext, sizeof(ZipNative))) ==
      null) {
    ...
    if (!initializeZipNative(p->currentContext, zipNativeP, streamObj)) {
      throwException(
        p->currentContext, IOException, "Failed to initialize zip context");
    } else {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZF_close(NMParams p)
{ // totalcross/util/zip/ZipFile native public void
...
{ // totalcross/util/zip/ZipFile native
  ...
  for (i = 0; i < size; i++) {
    ...
    err = unzGetCurrentFileInfo(
      *unzipFile, &file_info, filename, MAX_PATHNAME, null, 0, null, 0);
    if (err != UNZ_OK) {
      break;
    }
  ...
  if (list != null) {
    if ((p->retO = arrayObj = createArrayObject(p->currentContext,
      ...
      for (; start < end;
        ...
        TCObject name =
          createStringObjectFromCharP(p->currentContext, list->value, -1);
        *start =
          createObject(p->currentContext, "totalcross.util.zip.ZipEntry");
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZF_getEntry_s(NMParams p)
{ // totalcross/util/zip/ZipFile native public
  // String getEntry(String name);
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZF_getEntryStream_s(NMParams p)
{ // totalcross/util/zip/ZipFile native public totalcross.io.Stream
  // getEntryStream(String name) throws IOException;
  TCObject zipFile = p->obj[0];
  TCObject nativeFile = ZipFile_nativeFile(zipFile);
  unzFile* unzipFile = (unzFile*)ARRAYOBJ_START(nativeFile);
  ...
  if (unzLocateFile(*unzipFile, szName, 0) == UNZ_OK) {
    if (unzOpenCurrentFile(*unzipFile) == UNZ_OK) {
      TCObject zipStream =
        createObject(p->currentContext, "totalcross.util.zip.ZipStream");
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZS_createInflate_s(NMParams p)
{ // totalcross/util/zip/ZipStream native protected Object
  // createInflate(totalcross.io.Stream stream);
  ...
  } else if ((zipNativeObj = createByteArray(p->currentContext,
                                             sizeof(ZipNative))) == null) {
    throwException(p->currentContext, OutOfMemoryError, null);
  } else {
    zipNativeP = (ZipNativeP)ARRAYOBJ_START(zipNativeObj);
    if (!initializeZipNative(p->currentContext, zipNativeP, streamObj)) {
      ...
      if ((zipNativeP->zipFile =
             unzOpen2((char*)streamObj, &zipNativeP->zlib_def)) == null) {
        throwException(
          p->currentContext, IOException, "Failed to start the zip file");
      }
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZS_createDeflate_si(NMParams p)
{ // totalcross/util/zip/ZipStream native protected Object
  // createDeflate(totalcross.io.Stream stream, int compressionType);
  TCObject zipStream = p->obj[0];
  TCObject streamObj = p->obj[1];
  TCObject zipNativeObj;
  ZipNativeP zipNativeP;
  ...
  if (streamObj == null) {
    ...
    if (!initializeZipNative(p->currentContext, zipNativeP, streamObj)) {
      ...
      if ((zipNativeP->zipFile = zipOpen2((char*)streamObj,
                                          APPEND_STATUS_CREATE,
                                          null,
                                          &zipNativeP->zlib_def)) == null) {
        throwException(
          p->currentContext, IOException, "Failed to start the zip file");
      }
      *ZipStream_nativeZip(zipStream) = zipNativeObj;
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZS_available(NMParams p)
{ // totalcross/util/zip/ZipStream native public int
  ...
  if ((err = unzGetCurrentFileInfo(
         *unzipFile, &file_info, null, 0, null, 0, null, 0)) != UNZ_OK) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZS_getNextEntry(NMParams p)
{ // totalcross/util/zip/ZipStream native public
  // totalcross.util.zip.ZipEntry getNextEntry()
  // throws IOException;
  TCObject zipStream = p->obj[0];
  ...
  if (mode != 2) { // INFLATE
    throwException(p->currentContext,
                   IOException,
                   "This operation can only be performed in INFLATE mode.");
  } else {
    ...
    if (err == UNZ_END_OF_LIST_OF_FILE) {
      ...
      throwException(
        p->currentContext, IOException, "Failed to retrieve the next entry");
    } else if ((err = unzGetCurrentFileInfo(zipNativeP->zipFile,
                                            &file_info,
                                            zipEntryName,
                                            MAX_PATHNAME,
                                            null,
      ...
      throwException(p->currentContext,
                     IOException,
                     "Failed to retrieve details for the next entry");
    } else if ((zipEntryObj = createObject(
                  p->currentContext, "totalcross.util.zip.ZipEntry")) == null) {
      throwException(p->currentContext, OutOfMemoryError, null);
    } else if ((zipEntryNameObj = createStringObjectFromCharP(
                  p->currentContext,
                  zipEntryName,
                  (int)file_info.size_filename)) == null) {
      throwException(p->currentContext, OutOfMemoryError, null);
    } else if (file_info.size_file_comment > 0 &&
               (zipEntryCommentP =
                  (CharP)xmalloc((int)file_info.size_file_comment)) == null) {
      throwException(p->currentContext, OutOfMemoryError, null);
    } else if (file_info.size_file_extra > 0 &&
               (zipEntryExtraObj = createByteArray(
                  p->currentContext, (int)file_info.size_file_extra)) == null) {
      throwException(p->currentContext, OutOfMemoryError, null);
    } else if ((err = unzGetCurrentFileInfo(
                  zipNativeP->zipFile,
                  &file_info,
                  zipEntryName,
                  MAX_PATHNAME,
                  ...
                  file_info.size_file_comment > 0
                    ? (CharP)ARRAYOBJ_START(zipEntryCommentObj)
                    : null,
                  file_info.size_file_extra)) !=
               UNZ_OK) { // guich@tc126_31: check for comment length
      throwException(p->currentContext,
                     IOException,
                     "Failed to retrieve details for the next entry");
    } else if ((zipEntryCommentObj = createStringObjectFromCharP(
                  p->currentContext,
                  zipEntryCommentP,
                  (int)file_info.size_file_comment)) == null) {
      throwException(p->currentContext, OutOfMemoryError, null);
    } else if (unzOpenCurrentFile(zipNativeP->zipFile) != UNZ_OK) {
      throwException(
        p->currentContext, IOException, "Failed to open the next entry");
    } else {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZS_putNextEntry_z(NMParams p)
{ // totalcross/util/zip/ZipStream native public void
  // putNextEntry(totalcross.util.zip.ZipEntry zipEntry) throws
  // IOException;
  TCObject zipStream = p->obj[0];
  TCObject zipNativeObj = *ZipStream_nativeZip(zipStream);
  ...
  if (method != Z_DEFLATED) {
    ...
    if (ZipEntry_size(zipEntryObj) < 0) {
      throwException(
        p->currentContext, ZipException, "Method STORED, but size not set");
      return;
    }
    if (ZipEntry_crc(zipEntryObj) < 0) {
      throwException(
        p->currentContext, ZipException, "Method STORED, but crc not set");
      return;
    }
  }
  ...
  if (lastEntryObj != null && zipNativeP->method != Z_DEFLATED) {
    ...
    *ZipStream_lastEntry(zipStream) = null;
    ...
    if (err == ZIP_PARAMERROR) {
      throwException(
        p->currentContext, IOException, "No open entry to be closed");
      return;
    }
    if (err != ZIP_OK) {
      throwException(p->currentContext, IOException, null);
  ...
  if ((err = zipOpenNewFileInZip(
         zipNativeP->zipFile,
         zipEntryNameP,
         &fileInfo,
         zipEntryExtraP,
         (zipEntryExtraObj != null ? ARRAYOBJ_LEN(zipEntryExtraObj) : 0),
         null,
         0,
         zipEntryCommentP,
         method,
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZS_readBytes_Bii(NMParams p)
{ // totalcross/util/zip/ZipStream native public
  // int readBytes(byte []buf, int start, int
  ...
  } else if (mode != 2) { // INFLATE
    throwException(p->currentContext,
                   IOException,
                   "This operation can only be performed in INFLATE mode.");
  } else if ((ret = unzReadCurrentFile(
                zipNativeP->zipFile, bufP + start, count)) < 0) {
    throwException(
      p->currentContext, IOException, "Internal zip error: %d", ret);
  } else {
    p->retI = ret == 0 ? -1 : ret;
  }
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tuzZS_writeBytes_Bii(NMParams p)
{ // totalcross/util/zip/ZipStream native
  // public int writeBytes(byte []buf, int
  // start, int count) throws IOException;
  TCObject zipStream = p->obj[0];
  TCObject zipNativeObj = *ZipStream_nativeZip(zipStream);
  ZipNativeP zipNativeP = (ZipNativeP)ARRAYOBJ_START(zipNativeObj);
  int32 mode = CompressedStream_mode(zipStream);
  ...
  if (zipNativeObj == null) {
    ...
    throwException(p->currentContext,
                   IOException,
                   "This operation can only be performed in DEFLATE mode.");
  } else if ((err = zipWriteInFileInZip(
                zipNativeP->zipFile, bufP + start, count)) != ZIP_OK) {
    throwException(
      p->currentContext, IOException, "Internal zip error: %d", err);
  } else {
    p->retI = count;
  }
}
...
{ // totalcross/util/zip/ZipStream native
  ...
  executeMethod(p->currentContext,
                zipNativeP->streamTell,
                CompressedStream_streamRef(zipStream));
  ...
  if (mode == 2) { // INFLATE
    if ((err = unzClose(zipNativeP->zipFile)) != UNZ_OK) {
      throwException(p->currentContext,
                     IOException,
                     "An error ocurred while closing the ZipStream");
    }
