...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzCS_createInflate_s(NMParams
								  p) { // totalcross/util/zip/CompressedStream native protected Object createInflate(totalcross.io.Stream stream) throws IOException;
	...
	if ((zstreamRefObj = createByteArray(p->currentContext, sizeof(TZLibStreamRef))) != null) {
		...
		if ((err = inflateInit2(&zstreamRef->c_stream, 32 + MAX_WBITS)) != Z_OK) {
			...
			zstreamRef->rwMethod  = getMethod((TCClass) OBJ_CLASS(stream), true, "readBytes", 3, BYTE_ARRAY,
											  J_INT, J_INT);
		}
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzCS_createDeflate_si(NMParams
								   p) { // totalcross/util/zip/CompressedStream native protected Object createDeflate(totalcross.io.Stream stream, int compressionType) throws IOException;
	TCObject stream = p->obj[1];
	...
	if ((zstreamRefObj = createByteArray(p->currentContext, sizeof(TZLibStreamRef))) != null) {
		...
		if ((err = deflateInit2(&zstreamRef->c_stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, bits,
								DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY)) != Z_OK) { //flsobral@tc114_82: now supports GZip compression.
			throwException(p->currentContext, IOException, zstreamRef->c_stream.msg);
		} else {
			zstreamRef->rwMethod  = getMethod((TCClass) OBJ_CLASS(stream), true, "writeBytes", 3, BYTE_ARRAY,
											  J_INT, J_INT);
		}
		setObjectLock(p->retO = zstreamRefObj, UNLOCKED);
	}
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzCS_readBytes_Bii(NMParams
								p) { // totalcross/util/zip/CompressedStream native public int readBytes(byte []buf, int start, int count) throws IOException;
	...
	if (mode != INFLATE) {
		...
		while (start < end && err != Z_STREAM_END) {
			if (zstreamRef->c_stream.avail_in == 0) {
				zstreamRef->c_stream.avail_in = executeMethod(p->currentContext, zstreamRef->rwMethod, stream,
												streamBuffer, 0, ARRAYOBJ_LEN(streamBuffer)).asInt32;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzCS_writeBytes_Bii(NMParams
								 p) { // totalcross/util/zip/CompressedStream native public int writeBytes(byte []buf, int start, int count) throws IOException;
	TCObject zstreamRefObj = CompressedStream_streamRef(p->obj[0]);
	TCObject streamBuffer = CompressedStream_streamBuffer(p->obj[0]);
	int32 mode = CompressedStream_mode(p->obj[0]);
	ZLibStreamRef zstreamRef = (ZLibStreamRef) ARRAYOBJ_START(zstreamRefObj);
	...
	if (mode != DEFLATE) {
		...
		do {
			...
			result += executeMethod(p->currentContext, zstreamRef->rwMethod, stream, streamBuffer, 0,
									streamBufferLen - zstreamRef->c_stream.avail_out).asInt32;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tuzCS_close(NMParams
						p) { // totalcross/util/zip/CompressedStream native public void close() throws IOException;
	TCObject zstreamRefObj = CompressedStream_streamRef(p->obj[0]);
			...
			case DEFLATE: {
				do {
					...
					if ((int32) zstreamRef->c_stream.avail_out < streamBufferLen) {
						executeMethod(p->currentContext, zstreamRef->rwMethod, stream, streamBuffer, 0,
									  streamBufferLen - zstreamRef->c_stream.avail_out);
					}
				} while (zstreamRef->c_stream.avail_out == 0);
				if (err != Z_STREAM_END) {
					throwException(p->currentContext, IOException, zstreamRef->c_stream.msg);
				} else if ((err = deflateEnd(&zstreamRef->c_stream)) != Z_OK) {
					throwException(p->currentContext, IOException, zstreamRef->c_stream.msg);
				}
			}
			break;
