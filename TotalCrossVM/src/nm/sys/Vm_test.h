...
#else
	p.obj[0] = createStringObjectFromCharP("C:/Arquivos de programas/Internet Explorer/iexplore.exe",
										   -1);
...
TESTCASE(tsV_clipboardPaste) { // totalcross/sys/Vm native public static String clipboardPaste();
	...
	p1.obj = &copied;
	copied = createStringObjectFromCharP(currentContext,
										 "Testing tsV_clipboardCopy_s and tsV_clipboardPaste", -1);
	...
	ASSERT3_EQUALS(Block, String_charsStart(pasted), String_charsStart(copied),
