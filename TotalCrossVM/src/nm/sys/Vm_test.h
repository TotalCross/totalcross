...
TC_API void
tsT_update(NMParams p);

TESTCASE(tsV_arrayCopy_oioii)
{ // totalcross/sys/Vm native public static boolean
  // arrayCopy(Object srcArray, int srcStart, Object
  ...
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  {
    ...
    int i;
    for (i = 0; i < 4; i++) {
      char* sz = JCharP2CharP(String_charsStart(pStrings2[i]),
                              String_charsLen(pStrings2[i]));
...
TESTCASE(tsV_getTimeStamp)
{ // totalcross/sys/Vm native public static int
  // getTimeStamp();
  TNMParams p;
...
{ // totalcross/sys/Vm native public static void
  ...
  ASSERT_BETWEEN(I32,
                 Time_second(testTime),
                 Time_second(checkTime),
                 Time_second(testTime) + 5);
...
TESTCASE(tsV_exec_ssib)
{ // totalcross/sys/Vm native public static int exec(String
  // command, String args, int launchCode, boolean wait);

#if 1
  TEST_CANNOT_RUN;
  // Vm.exec testcase cannot be created because there are no ways to sublaunch a
  // process in most devices
...
#else
  p.obj[0] = createStringObjectFromCharP(
    "C:/Arquivos de programas/Internet Explorer/iexplore.exe", -1);
...
TESTCASE(tsV_setAutoOff_b)
{ // totalcross/sys/Vm native public static void
  // setAutoOff(boolean enable);
  TEST_SKIP;
finish:;
}
TESTCASE(tsV_sleep_i)
{ // totalcross/sys/Vm native public static void sleep(int
  // millis);
  TNMParams p;
  int32 i32buf[1];
  int32 start, end;
...
TESTCASE(tsV_interceptSpecialKeys_I)
{ // totalcross/sys/Vm native public static
  // void interceptSpecialKeys(int []keys);
  //   TNMParams p;
  //   int32 i32buf[1];
...
TESTCASE(tsV_clipboardPaste)
{ // totalcross/sys/Vm native public static String
  // clipboardPaste();
  TNMParams p1, p2;
  ...
  p1.obj = &copied;
  copied = createStringObjectFromCharP(
    currentContext, "Testing tsV_clipboardCopy_s and tsV_clipboardPaste", -1);
  ...
  ASSERT3_EQUALS(Block,
                 String_charsStart(pasted),
                 String_charsStart(copied),
                 String_charsLen(pasted));
#else
  TEST_CANNOT_RUN;
#endif
finish:;
}
TESTCASE(tsV_attachLibrary_s)
{ // totalcross/sys/Vm native public static
  // boolean attachLibrary(String name);
  TEST_SKIP;
finish:;
}
TESTCASE(tsV_privateAttachNativeLibrary_s)
{ // totalcross/sys/Vm native private
  // static boolean
  // privateAttachNativeLibrary(String
  // name);
  TEST_SKIP;
finish:;
}
TESTCASE(tsV_isKeyDown_i)
{ // totalcross/sys/Vm native public static boolean
  // isKeyDown(int key);
  TNMParams p;
  int32 i = 0;
...
TESTCASE(tsV_getFile_s)
{ // totalcross/sys/Vm native public static byte[]
  // getFile(String name);
  TNMParams p;
  TCObject obj[3];
  int32 i32buf[3];
  int32 ret;
  CharP pRet;
  char startBytes[23] = { (char)0xFF, (char)0xD8, (char)0xFF, (char)0xE0,
                          (char)0x00, (char)0x10, (char)0x4A, (char)0x46,
                          (char)0x49, (char)0x46, (char)0x00, (char)0x01,
                          (char)0x01, (char)0x01, (char)0x00, (char)0x48,
                          (char)0x00, (char)0x48, (char)0x00, (char)0x00,
                          (char)0xFF, (char)0xE1, 0x1D };

  char endBytes[37] = {
    (char)0x43, (char)0xBB, (char)0x9B, (char)0xC5, (char)0x8A, (char)0xD7,
    (char)0xA8, (char)0xCB, (char)0x74, (char)0x02, (char)0x83, (char)0x8A,
    (char)0xE5, (char)0xCA, (char)0x74, (char)0xE9, (char)0x4B, (char)0xB7,
    (char)0x96, (char)0x3B, (char)0xA4, (char)0x7C, (char)0x91, (char)0xD0,
    (char)0x76, (char)0xAF, (char)0x5E, (char)0x56, (char)0x03, (char)0x1B,
    (char)0xB0, (char)0x2A, (char)0x58, (char)0xBB, (char)0x4F, (char)0xFF,
    (char)0xD9
  };
...
TESTCASE(tsV_getRemainingBattery)
{ // totalcross/sys/Vm native public static
...
TESTCASE(tsV_tweak_ib)
{ // totalcross/sys/Vm native public static void
  // tweak(int param, boolean set);
  TEST_SKIP;
finish:;
}
TESTCASE(tsV_getStackTrace_t)
{ // totalcross/sys/Vm native public static String
