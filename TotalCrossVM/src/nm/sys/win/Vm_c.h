...
{
#ifdef WINCE
  typedef BOOL(__stdcall * KernelIoControlProc)(
    DWORD, LPVOID, DWORD, LPVOID, DWORD, LPDWORD);
  KernelIoControlProc procKernelIoControl =
    (KernelIoControlProc)GetProcAddress(coreDll, TEXT("KernelIoControl"));
  ...
  SetSystemPowerStateProc SetSystemPowerState =
  (SetSystemPowerStateProc)GetProcAddress(dll, TEXT("SetSystemPowerState"));
  SetSystemPowerState(null, 0x00800000, 4096);*/
#endif
}

typedef HANDLE(__stdcall* RegisterServiceProc)(LPCWSTR lpszType,
                                               DWORD dwIndex,
                                               LPCWSTR lpszLib,
                                               DWORD dwInfo);
typedef BOOL(__stdcall* DeregisterServiceProc)(HANDLE hDevice);
typedef HANDLE(__stdcall* GetServiceHandleProc)(LPWSTR szPrefix,
                                                LPWSTR szDllName,
                                                DWORD pdwDllBuf);
...
vmExec(TCHARP szCommand, TCHARP szArgs, int32 launchCode, bool wait)
{
  ...
  if (lstrcmp(szCommand, TEXT("unregister service")) == 0) {
    ...
    DeregisterServiceProc deregisterService =
      (DeregisterServiceProc)GetProcAddress(dll, TEXT("DeregisterService"));
    GetServiceHandleProc getServiceHandle =
      (GetServiceHandleProc)GetProcAddress(dll, TEXT("GetServiceHandle"));
    ret = 0;
  ...
  if (lstrcmp(szCommand, TEXT("register service")) == 0) {
    ...
    RegisterServiceProc registerService =
      (RegisterServiceProc)GetProcAddress(dll, TEXT("RegisterService"));
    char dllpath[255];
    HANDLE srv;
  ...
  if (strEq(szCommand, "running process")) {
    ...
    return exists;
  } else if (strEq(szCommand, "viewer") || strEq(szCommand, "url")) {
  ...
  ok = CreateProcess(
    ...
    null,
    startInfo,
    &processInfo); // guich@tc100b5_16: iexplore requires this mode.
  err = GetLastError();

  if (!ok) {
    lstrcpy(pathargs,
            szCommand); // concats the parameters. not sure why this works on
                        // most features but does not work when passing the
...
#ifndef WINCE
  if (!ok && xstrchr(szCommand, '/') == null &&
      xstrchr(szCommand, '\\') == null) { // if the user didn't specified a path
    ...
    if (
      ...
      if (err != 0) {
        xstrprintf(
          fullname, "Applications\\%s\\shell\\new\\command", szCommand);
        err = RegOpenKeyEx(HKEY_CLASSES_ROOT, fullname, 0, KEY_READ, &handle);
      }
      if (err == 0) {
        err =
          RegQueryValueEx(handle, null, null, null, (uint8*)fullname, &size);
        if (err == 0) {
          // returns something like "%ProgramFiles%\Windows
          // NT\Acess�rios\WORDPAD.EXE" "%1" get the first part
          char* f = fullname;
          if (*f == '"') {
            f++;
          }
...
void
vmSetAutoOff(bool enable)
{
#ifdef WINCE
  if (!enable && oldAutoOffValue == 0) {
    SystemParametersInfo(SPI_GETBATTERYIDLETIMEOUT,
                         0,
                         &oldAutoOffValue,
                         0); // get the previous value
    if (oldAutoOffValue == 0) {
      oldAutoOffValue =
        -1; // guich@tc113_22: some systems returns 0, so we change it to -1
    } else {
      SystemParametersInfo(SPI_SETBATTERYIDLETIMEOUT,
                           0,
                           null,
                           0); // set to 0 (2nd parameter)
    }
  } else if (enable && oldAutoOffValue != 0) {
...
// XXX: O que s�o hot keys? N�o fa�o a menor id�ia do que fazer aqui e acho que
// nem faz sentido
void
registerHotkeys(Int32Array keys, bool isRegister)
{
  if (mainHWnd != null) {
#ifdef WINCE
    int32 n;
    ...
    if ((hCoreDll = LoadLibrary(TEXT("coredll.dll"))) != null) {
      UnregisterFunc1Proc procUnregisterFunc =
        (UnregisterFunc1Proc)GetProcAddress(hCoreDll, _T("UnregisterFunc1"));
    ...
    // throwException(currentContext, RuntimeException, "Could not find entry
    // point for hotkeys registration");
#elif !defined WP8
    int32 n;
    for (n = ARRAYLEN(keys); n-- > 0; keys++)
      if (isRegister) {
...
static void
vmInterceptSpecialKeys(int32* keys, int32 len)
{
...
static void
vmClipboardCopy(CharP string, int32 stringLen)
{
  // XXX
#if !defined WP8
  HANDLE hClipData;
...
static TCObject
vmClipboardPaste(Context currentContext)
{
  // XXX
#if !defined WP8
  HANDLE hClipData;
  ...
  } else if ((hClipData = GetClipboardData(CF_TCHARTEXT)) == null ||
             (lpClipData = ClipboardLock(hClipData)) == null) {
    CloseClipboard();
    o = createStringObjectFromCharP(currentContext, "", 0);
  } else {
    o = createStringObjectFromTCHAR(
      currentContext, lpClipData, tcslen(lpClipData));
    ClipboardUnlock(hClipData);
    CloseClipboard();
  }
  return o;
...
vmGetRemainingBattery()
{
  ...
  if (GetSystemPowerStatusEx(&p, true)) {
    ...
    if (p.BackupBatteryLifePercent <
        100) { // even if not present, it returns 100, so we will only consider
               // values less than 100
      if (p.BatteryLifePercent ==
          0xFF) { // sometimes, right after the device is undocked, the main
                  // battery is at an unknown state
        ret = 0;
      }
...
vmTurnScreenOn(bool on)
{
  ...
  gdc = GetDC(NULL);
  if (ExtEscape(gdc, QUERYESCSUPPORT, sizeof(int), (LPCSTR)&iESC, 0, NULL) ==
      0) {
    ret = false;
  } else {
    ...
    ret =
      on ? (vpm.PowerState == VideoPowerOn) : (vpm.PowerState == VideoPowerOff);
    ReleaseDC(NULL, gdc);
  }
  return ret;
#endif
...
vmVibrate(int32 ms)
{
  ...
  if (vibtype == VIB_NONE) {
    if (coreDll) {
      NLedSetDevice =
        (NLedSetDeviceProc)GetProcAddress(coreDll, TEXT("NLedSetDevice"));
      NLedGetDeviceInfo = (NLedGetDeviceInfoProc)GetProcAddress(
        coreDll, TEXT("NLedGetDeviceInfo"));
  ...
  if (vibThread == null && (vibtype == VIB_AYG || vibtype == VIB_CORE)) {
    vibThread = CreateThread(
      null, 0, (LPTHREAD_START_ROUTINE)VibrateThread, (int*)ms, 0, null);
  }
