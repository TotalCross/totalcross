...
void rebootDevice() {
	...
	KernelIoControlProc procKernelIoControl = (KernelIoControlProc)GetProcAddress(coreDll,
			TEXT("KernelIoControl"));
...
typedef HANDLE(__stdcall* RegisterServiceProc)(LPCWSTR lpszType,  DWORD dwIndex,  LPCWSTR lpszLib,
		DWORD dwInfo);
...
static int32 vmExec(TCHARP szCommand, TCHARP szArgs, int32 launchCode, bool wait) {
	...
	if (lstrcmp(szCommand, TEXT("unregister service")) == 0) {
		...
		DeregisterServiceProc deregisterService = (DeregisterServiceProc)GetProcAddress(dll,
				TEXT("DeregisterService"));
		GetServiceHandleProc getServiceHandle = (GetServiceHandleProc)GetProcAddress(dll,
												TEXT("GetServiceHandle"));
		ret = 0;
	...
	if (lstrcmp(szCommand, TEXT("register service")) == 0) {
		...
		RegisterServiceProc registerService = (RegisterServiceProc)GetProcAddress(dll,
...
#endif
	ok = CreateProcess(szCommand, szArgs, null, null, false, 0, null, null, startInfo,
					   &processInfo); // guich@tc100b5_16: iexplore requires this mode.
	err = GetLastError();

	if (!ok) {
		lstrcpy(pathargs,
				szCommand); // concats the parameters. not sure why this works on most features but does not work when passing the parameters separately
...
void registerHotkeys(Int32Array keys, bool isRegister) {
	if (mainHWnd != null) {
		...
		if ((hCoreDll = LoadLibrary(TEXT("coredll.dll"))) != null) {
			UnregisterFunc1Proc procUnregisterFunc = (UnregisterFunc1Proc)GetProcAddress(hCoreDll,
					_T("UnregisterFunc1"));
...
static int32 vmGetRemainingBattery() {
	...
	if (GetSystemPowerStatusEx(&p, true)) {
		...
		if (p.BackupBatteryLifePercent <
			100) { // even if not present, it returns 100, so we will only consider values less than 100
			if (p.BatteryLifePercent ==
				0xFF) { // sometimes, right after the device is undocked, the main battery is at an unknown state
				ret = 0;
			}
