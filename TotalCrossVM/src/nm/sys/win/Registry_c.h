...
static void privateGetInt(NMParams p, int32 hk, TCHARP key, TCHARP value) {
	...
	notFound:
		...
		RegCloseKey(handle);
		if (ret ==
			ERROR_FILE_NOT_FOUND) { // very strange: a key that does not exist does not return an error in RegOpenKeyEx!
			goto notFound;
		}
		if (ret != 0) {
	...
	notFound:
		...
		if (ret == 0) {
			...
			if (buf == null) {
				...
				if (ret == 0) {
					p->retO = createStringObjectFromTCHARP(p->currentContext, buf,
														   size / sizeof(TCHAR) - 1); // guich@tc100b5_37: in wince its a TCHAR. - guich@tc112_16
					if (p->retO) {
...
static TCObject privateList(Context currentContext, int32 hk, TCHARP key) {
	...
	if ((ret = RegOpenKeyEx((HKEY)hk, key, 0, KEY_ENUMERATE_SUB_KEYS | KEY_QUERY_VALUE,
							&handle)) != NO_ERROR) {
		return null;
	}
