...
privateGetInt(NMParams p, int32 hk, TCHARP key, TCHARP value)
{
  ...
  notFound:
    throwException(
      p->currentContext, ElementNotFoundException, "Key not found");
  } else {
    ret = RegQueryValueEx(handle, value, NULL, NULL, (uint8*)&p->retI, &size);
    RegCloseKey(handle);
    if (ret ==
        ERROR_FILE_NOT_FOUND) { // very strange: a key that does not exist does
                                // not return an error in RegOpenKeyEx!
      goto notFound;
    }
    if (ret != 0) {
      throwException(
        p->currentContext, ExceptionClass, "Cannot read value. Error: %d", ret);
    }
  }
}
...
privateGetString(NMParams p, int32 hk, TCHARP key, TCHARP value)
{
  ...
  notFound:
    ...
    if (ret == 0) {
      ...
      if (buf == null) {
        throwException(
          p->currentContext, OutOfMemoryError, "Can't allocate char buffer");
      } else {
        ret = RegQueryValueEx(handle, value, NULL, NULL, (char*)buf, &size);
        if (ret == 0) {
          p->retO = createStringObjectFromTCHARP(
            p->currentContext,
            buf,
            size / sizeof(TCHAR) - 1); // guich@tc100b5_37: in wince its a
                                       // TCHAR. - guich@tc112_16
          if (p->retO) {
  ...
  notFound:
    ...
    if (ret == 0) {
      if ((p->retO = createByteArray(p->currentContext, size)) != null) {
        setObjectLock(p->retO, UNLOCKED);
        ret = RegQueryValueEx(
          handle, value, NULL, NULL, ARRAYOBJ_START(p->retO), &size);
      }
    }
    RegCloseKey(handle);
    if (ret ==
...
static TCObject
privateList(Context currentContext, int32 hk, TCHARP key)
{
  HKEY handle;
  LONG ret;
  DWORD count = 0, i;
