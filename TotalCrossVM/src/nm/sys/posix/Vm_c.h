...
{
  //   if ((!enable && oldAutoOffValue == 0) || (enable && oldAutoOffValue !=
  //   0))
...
#ifdef darwin
void
vmClipboardCopy(JCharP string, int32 sLen); // in mainview.m
unsigned short*
ios_ClipboardPaste();
TCObject
vmClipboardPaste(Context currentContext)
{
  ...
  return !chars ? null
                : createStringObjectFromJCharP(currentContext, chars, -1);
}
#else
static void
...
#if 0
static void* loadLib(UtfString lname,
					 int libPrefix/*, int syspath*/) { // TODO support client native libs in the app path
	void* h;
