...
static int32 vmExec(TCObject command, TCObject args, int32 launchCode, bool wait) {
	JNIEnv* env = getJNIEnv();
	jstring jcommand = (*env)->NewString(env, (jchar*) String_charsStart(command),
										 String_charsLen(command));
	jstring jargs = !args ? null : (*env)->NewString(env, (jchar*) String_charsStart(args),
					String_charsLen(args));
	int32 ret = (*env)->CallStaticIntMethod(env, applicationClass, jvmExec, jcommand, jargs, launchCode,
											wait);
...
static bool vmTurnScreenOn(bool on) {
	JNIEnv* env = getJNIEnv();
	return !env ? false : (*env)->CallStaticIntMethod(env, applicationClass, jvmFuncI, TURN_SCREEN_ON,
			(int32)on) == 1;
}
