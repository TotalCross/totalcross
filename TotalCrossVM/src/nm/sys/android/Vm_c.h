...
rebootDevice()
{
  ...
  (*env)->CallStaticIntMethod(
    env, applicationClass, jvmFuncI, REBOOT_DEVICE, 0);
}

static int32
vmExec(TCObject command, TCObject args, int32 launchCode, bool wait)
{
  JNIEnv* env = getJNIEnv();
  jstring jcommand = (*env)->NewString(
    env, (jchar*)String_charsStart(command), String_charsLen(command));
  jstring jargs = !args ? null
                        : (*env)->NewString(env,
                                            (jchar*)String_charsStart(args),
                                            String_charsLen(args));
  int32 ret = (*env)->CallStaticIntMethod(
    env, applicationClass, jvmExec, jcommand, jargs, launchCode, wait);
...
vmSetAutoOff(bool enable)
{
  JNIEnv* env = getJNIEnv();
  (*env)->CallStaticIntMethod(
    env, applicationClass, jvmFuncI, SET_AUTO_OFF, (int32)enable);
}
...
vmShowKeyCodes(bool show)
{
  JNIEnv* env = getJNIEnv();
  (*env)->CallStaticIntMethod(
    env, applicationClass, jvmFuncI, SHOW_KEY_CODES, (int32)show);
}
...
{ // NOT IMPLEMENTED!
  JNIEnv* env = getJNIEnv();
  jstring src = (jstring)(*env)->CallStaticObjectMethod(
    env, applicationClass, jclipboard, 0);
  TCObject o = null;
  if (src != null) {
    const jchar* str = (*env)->GetStringChars(env, src, 0);
    if (str) {
      o = createStringObjectFromJCharP(
        currentContext, (JCharP)str, (*env)->GetStringLength(env, src));
    }
...
vmIsKeyDown(int32 key)
{
  ...
  return (*env)->CallStaticIntMethod(
           env, applicationClass, jvmFuncI, IS_KEY_DOWN, devkey) == 1;
}
...
vmGetRemainingBattery()
{
  JNIEnv* env = getJNIEnv();
  return (*env)->CallStaticIntMethod(
    env, applicationClass, jvmFuncI, GET_REMAINING_BATTERY, 0);
}
...
vmTurnScreenOn(bool on)
{
  ...
  return !env
           ? false
           : (*env)->CallStaticIntMethod(
               env, applicationClass, jvmFuncI, TURN_SCREEN_ON, (int32)on) == 1;
}
