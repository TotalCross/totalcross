...
#ifdef darwin
int32
vmExec(TCHARP szCommand, TCHARP szArgs, int32 launchCode, bool wait);
#endif

CompatibilityResult
areArraysCompatible(Context currentContext, TCObject array, CharP ident)
{
  // this function searches for the first non-null element in array and checks
  // if it is compatible with ident
  ...
  return COMPATIBLE; // if all elements are null, then it is safe to move null
                     // to any kind other of array,
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_arrayCopy_oioii(NMParams p)
{ // totalcross/sys/Vm native public static boolean
  // arrayCopy(Object srcArray, int srcStart, Object dstArray,
  ...
  if (!srcArray) {
    ...
    if (isObjectArray &&
        p->currentContext !=
          gcContext) { // prevent another thread from calling the gc while
                       // running this method, which can change objects in
                       // memory and result in a crash
      LOCKVAR(omm);
    }
    if (dstStart < 0) {
      ...
      result = true;
    } else if (!checkArrayRange(
                 p->currentContext, srcArray, srcStart, length) ||
               !checkArrayRange(p->currentContext,
                                dstArray,
                                dstStart,
                                length)) { // Check the array's range
      result = false;
    } else if (OBJ_CLASS(srcArray) != OBJ_CLASS(dstArray) && // quick test
               areClassesCompatible(p->currentContext,
                                    OBJ_CLASS(srcArray),
                                    OBJ_CLASS(dstArray)->name) !=
                 COMPATIBLE && // Check if srcArray and dstArray are
                               // compatible
               (OBJ_CLASS(srcArray)->name[1] == '&' ||
                OBJ_CLASS(dstArray)->name[1] ==
                  '&' || // make sure the arrays are not primitive arrays
                areArraysCompatible(p->currentContext,
                                    srcArray,
                                    OBJ_CLASS(dstArray)->name + 1) !=
                  COMPATIBLE)) { // check if the first elements are compatible
      throwException(p->currentContext,
                     ArrayStoreException,
                     "srcArray and dstArray are not compatible");
    } else {
      TCClass c = OBJ_CLASS(srcArray);
      src = ARRAYOBJ_START(srcArray);
      dst = ARRAYOBJ_START(dstArray);
      if (isObjectArray) { // copy pointers directly to prevent a partial
                           // assigned value from being considered a valid
                           // object
        TCObjectArray psrc = (TCObjectArray)src;
        TCObjectArray pdst = (TCObjectArray)dst;
        if (src == dst && srcStart < dstStart) // copy arrays overlap?
          for (psrc += srcStart + length - 1, pdst += dstStart + length - 1;
               --length >=
               0;) { // must go backwards to allow copy into overlapping array
            *pdst-- = *psrc--;
          }
        else
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_getTimeStamp(NMParams p)
{ // totalcross/sys/Vm native public static int getTimeStamp();
  p->retI = getTimeStamp();
}
//////////////////////////////////////////////////////////////////////////
void
...
TC_API void
tsV_setTime_t(NMParams p)
{ // totalcross/sys/Vm native public static void
  // setTime(totalcross.sys.Time t);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_exitAndReboot(NMParams p)
{ // totalcross/sys/Vm native public
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_exec_ssib(NMParams p)
{ // totalcross/sys/Vm native public static int exec(String
  // command, String args, int launchCode, boolean wait);
...
#else
    if ((szCommand = JCharP2TCHARP(String_charsStart(command),
                                   String_charsLen(command))) != null) {
    if (!args || (szArgs = JCharP2TCHARP(String_charsStart(args),
                                         String_charsLen(args))) != null) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_setAutoOff_b(NMParams p)
{ // totalcross/sys/Vm native public static void
  // setAutoOff(boolean enable);
  vmSetAutoOff(p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_sleep_i(NMParams p)
{ // totalcross/sys/Vm native public static
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_getFreeMemory(NMParams p)
{ // totalcross/sys/Vm native public static int getFreeMemory();
  p->retI = getFreeMemory(false);
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_interceptSpecialKeys_I(NMParams p)
{ // totalcross/sys/Vm native public static void
...
{ // totalcross/sys/Vm native public static
  ...
  } else if ((s = String2CharP(strObj)) !=
             null) { // just ignore if no more memory
    p->retI = debugStr(s);
    xfree(s);
  }
}
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_clipboardCopy_s(NMParams p)
{ // totalcross/sys/Vm native public static void
  ...
  if (!string) {
    ...
    if ((s = JCharP2CharP(String_charsStart(string),
                          (sLen = String_charsLen(string)))) == null) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_attachLibrary_s(NMParams p)
{ // totalcross/sys/Vm native public static
  // boolean attachLibrary(String name);
  TCObject nameObj;
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_privateAttachNativeLibrary_s(NMParams p)
{ // totalcross/sys/Vm native private static boolean
  // privateAttachNativeLibrary(String name);
  TCObject nameObj;
...
TC_API void
tsV_gc(NMParams p)
{ // totalcross/sys/Vm native public static void gc();
  if (!runningGC) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_getFile_s(NMParams p)
{ // totalcross/sys/Vm native public
  // static byte[] getFile(String name);
  ...
  if (!name) {
    ...
    if ((file = tczGetFile((CharP)szName, false)) != null &&
        (p->retO = createByteArray(p->currentContext,
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_getRemainingBattery(NMParams p)
{ // totalcross/sys/Vm native public static
...
TC_API void
tsV_tweak_ib(NMParams p)
{ // totalcross/sys/Vm native public static
  // void tweak(int param, boolean set);
  int32 param = p->i32[0];
  int32 bit = param - 1;
  int32 on = p->i32[1];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_getStackTrace_t(NMParams p)
{ // totalcross/sys/Vm native public static
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_turnScreenOn_b(NMParams p)
{ // totalcross/sys/Vm native public static
  // boolean turnScreenOn(boolean on);
  p->retI = vmTurnScreenOn(p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_vibrate_i(NMParams p)
{ // totalcross/sys/Vm native public
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_identityHashCode_o(NMParams p)
{ // totalcross/sys/Vm native public static
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tsV_preallocateArray_oi(NMParams p)
{ // totalcross/sys/Vm native public static void
  // preallocateArray(Object sample, int length);
  TCObject t = p->obj[0];
  int32 len = p->i32[0];
  if (!t) {
