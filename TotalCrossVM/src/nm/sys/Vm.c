...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_arrayCopy_oioii(NMParams
								p) { // totalcross/sys/Vm native public static boolean arrayCopy(Object srcArray, int srcStart, Object dstArray, int dstStart, int length);
	...
	if (!srcArray) {
		...
		if (isObjectArray
			&& p->currentContext !=
			gcContext) { // prevent another thread from calling the gc while running this method, which can change objects in memory and result in a crash
			LOCKVAR(omm);
		}
		if (dstStart < 0) {
			...
			result = true;
		} else if (!checkArrayRange(p->currentContext, srcArray, srcStart, length)
				   || !checkArrayRange(p->currentContext, dstArray, dstStart, length)) { // Check the array's range
			result = false;
		} else if (OBJ_CLASS(srcArray) != OBJ_CLASS(dstArray) && // quick test
				   areClassesCompatible(p->currentContext, OBJ_CLASS(srcArray),
										OBJ_CLASS(dstArray)->name) != COMPATIBLE && // Check if srcArray and dstArray are compatible
				   (OBJ_CLASS(srcArray)->name[1] == '&' || OBJ_CLASS(dstArray)->name[1] == '&'
					|| // make sure the arrays are not primitive arrays
					areArraysCompatible(p->currentContext, srcArray,
										OBJ_CLASS(dstArray)->name + 1) != COMPATIBLE)) { // check if the first elements are compatible
			...
			if (isObjectArray) { // copy pointers directly to prevent a partial assigned value from being considered a valid object
				...
				if (src == dst && srcStart < dstStart) // copy arrays overlap?
					for (psrc += srcStart + length - 1, pdst += dstStart + length - 1;  --length >= 0;
						) { // must go backwards to allow copy into overlapping array
						*pdst-- = *psrc--;
					} else
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_getTimeStamp(NMParams
							 p) { // totalcross/sys/Vm native public static int getTimeStamp();
	p->retI = getTimeStamp();
}
//////////////////////////////////////////////////////////////////////////
void updateDaylightSavings(Context currentContext);

TC_API void tsV_setTime_t(NMParams
						  p) { // totalcross/sys/Vm native public static void setTime(totalcross.sys.Time t);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_exitAndReboot(NMParams
							  p) { // totalcross/sys/Vm native public static void exitAndReboot();
	UNUSED(p);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_exec_ssib(NMParams
						  p) { // totalcross/sys/Vm native public static int exec(String command, String args, int launchCode, boolean wait);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_setAutoOff_b(NMParams
							 p) { // totalcross/sys/Vm native public static void setAutoOff(boolean enable);
	vmSetAutoOff(p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_sleep_i(NMParams
						p) { // totalcross/sys/Vm native public static void sleep(int millis);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_getFreeMemory(NMParams
							  p) { // totalcross/sys/Vm native public static int getFreeMemory();
	p->retI = getFreeMemory(false);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_interceptSpecialKeys_I(NMParams
									   p) { // totalcross/sys/Vm native public static void interceptSpecialKeys(int []keys);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_debug_s(NMParams
						p) { // totalcross/sys/Vm native public static void debug(String s);
	TCObject strObj = p->obj[0];
	CharP s;

	if (!strObj) {
		p->retI = debugStr("null");
	} else if ((s = String2CharP(strObj)) != null) { // just ignore if no more memory
		p->retI = debugStr(s);
		xfree(s);
	}
}
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_alert_s(NMParams
						p) { // totalcross/sys/Vm native public static void alert(String s);
	TCObject strObj = p->obj[0];
	CharP s;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_clipboardCopy_s(NMParams
								p) { // totalcross/sys/Vm native public static void clipboardCopy(String s);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_clipboardPaste(NMParams
							   p) { // totalcross/sys/Vm native public static String clipboardPaste();
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_attachLibrary_s(NMParams
								p) { // totalcross/sys/Vm native public static boolean attachLibrary(String name);
	TCObject nameObj;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_privateAttachNativeLibrary_s(NMParams
		p) { // totalcross/sys/Vm native private static boolean privateAttachNativeLibrary(String name);
	TCObject nameObj;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_isKeyDown_i(NMParams
							p) { // totalcross/sys/Vm native public static boolean isKeyDown(int key);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_getFile_s(NMParams
						  p) { // totalcross/sys/Vm native public static byte[] getFile(String name);
	...
	if (!name) {
		throwNullArgumentException(p->currentContext, "name");
	} else if ((nameLen = String_charsLen(name)) > 0) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_getRemainingBattery(NMParams
									p) { // totalcross/sys/Vm native public static int getRemainingBattery();
	p->retI = vmGetRemainingBattery();
}
//////////////////////////////////////////////////////////////////////////
int tweakSSL;

TC_API void tsV_tweak_ib(NMParams
						 p) { // totalcross/sys/Vm native public static void tweak(int param, boolean set);
	int32 param = p->i32[0];
	int32 bit = param - 1;
	int32 on = p->i32[1];
	...
	if (param == VMTWEAK_TRACE_METHODS) {
		traceOn = on;
	} else if (param == VMTWEAK_DISABLE_GC && !on) { // guich@tc114_18
		gc(p->currentContext);
	} else if (param == VMTWEAK_MEM_PROFILER) { // guich@tc111_4
		if (profilerMaxMem == 0) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_getStackTrace_t(NMParams
								p) { // totalcross/sys/Vm native public static String getStackTrace(Throwable t);
	TCObject t = p->obj[0];
	if (!t) {
		throwNullArgumentException(p->currentContext, "t");
	} else {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_showKeyCodes_b(NMParams
							   p) { // totalcross/sys/Vm native public static void showKeyCodes(boolean on);
	vmShowKeyCodes(showKeyCodes = p->i32[0] == 1);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_turnScreenOn_b(NMParams
							   p) { // totalcross/sys/Vm native public static boolean turnScreenOn(boolean on);
	p->retI = vmTurnScreenOn(p->i32[0]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_vibrate_i(NMParams
						  p) { // totalcross/sys/Vm native public static void vibrate(int millis);
#if defined(WIN32) || defined(ANDROID) || defined(darwin)
	vmVibrate(p->i32[0]);
#endif
}
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_identityHashCode_o(NMParams
								   p) { // totalcross/sys/Vm native public static int identityHashCode(Object object);
	jlO_nativeHashCode(p);
}
//////////////////////////////////////////////////////////////////////////
TC_API void tsV_preallocateArray_oi(NMParams
									p) { // totalcross/sys/Vm native public static void preallocateArray(Object sample, int length);
	TCObject t = p->obj[0];
	int32 len = p->i32[0];
	if (!t) {
