...
TCObject
iso88591bytes2chars(Context currentContext, uint8* bytes, int32 length)
{
  JCharP chars;
...
TCObject
iso88591chars2bytes(Context currentContext, JCharP chars, int32 length)
{
  ...
  if (byteArray != null) {
      ...
      if (c <= 255) { // guich@tc123_42: '\377' (octal) is 255 (decimal)
        *bytes++ = (uint8)c;
        ...
        if (0xD800 <= c && c <= 0xDBFF &&
            chars < end) { // invalid range? skip one byte
          skip++;
          chars++;
        }
        *bytes++ = '?';
    ...
    if (skip != 0) { // just adjust the final length - this will waste memory
                     // but will prevent fragmentation
...
utf8bytes2chars(Context currentContext, uint8* bytes, int32 length)
{
  ...
  if (charArray) {
    ...
    if (resultingLength !=
        length) { // just adjust the final length - this will waste memory but
...
TCObject
utf8chars2bytes(Context currentContext, JCharP chars, int32 length)
{
  TCObject byteArray = createByteArray(
    currentContext,
    utf8len(chars,
            length)); // find the exact length, since it can be max 3*length
  if (byteArray != null) {
    uint8* bytes = ARRAYOBJ_START(byteArray);
...
TCObject
chars2bytes(Context currentContext, JCharP chars, int32 length)
{
  ...
  if (*charConverterPtr != lastCharConverter) {
    chars2bytesMtd = getMethod(OBJ_CLASS(*charConverterPtr),
                               true,
                               "chars2bytes",
                               3,
                               CHAR_ARRAY,
  ...
  return executeMethod(
           currentContext, chars2bytesMtd, *charConverterPtr, chars, 0, length)
    .asObj;
}

//////////////////////////////////////////////////////////////////////////
TC_API void
tsCC_bytes2chars_Bii(NMParams p)
{ // totalcross/sys/CharacterConverter native public
  ...
  if (checkArrayRange(p->currentContext, bytes, offset, len)) {
    p->retO = iso88591bytes2chars(
      p->currentContext, ((uint8*)ARRAYOBJ_START(bytes)) + offset, len);
  }
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tsCC_chars2bytes_Cii(NMParams p)
{ // totalcross/sys/CharacterConverter native public byte[]
  // chars2bytes(char []chars, int offset, int length);
  TCObject chars = p->obj[1];
  int32 offset = p->i32[0];
  int32 len = p->i32[1];
  if (checkArrayRange(p->currentContext, chars, offset, len)) {
    p->retO = iso88591chars2bytes(
      p->currentContext, ((JCharP)ARRAYOBJ_START(chars)) + offset, len);
  }
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tsUTF8CC_bytes2chars_Bii(NMParams p)
{ // totalcross/sys/UTF8CharacterConverter native public
  // char[]bytes2chars(byte []bytes, int offset, int length);
  TCObject bytes = p->obj[1];
  int32 offset = p->i32[0];
  int32 len = p->i32[1];
  if (checkArrayRange(p->currentContext, bytes, offset, len)) {
    p->retO = utf8bytes2chars(
      p->currentContext, ((uint8*)ARRAYOBJ_START(bytes)) + offset, len);
  }
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tsUTF8CC_chars2bytes_Cii(NMParams p)
{ // totalcross/sys/UTF8CharacterConverter native public byte[]
  // chars2bytes(char []chars, int offset, int length);
  TCObject chars = p->obj[1];
  int32 offset = p->i32[0];
  int32 len = p->i32[1];
  if (checkArrayRange(p->currentContext, chars, offset, len)) {
    p->retO = utf8chars2bytes(
      p->currentContext, ((JCharP)ARRAYOBJ_START(chars)) + offset, len);
  }
}
