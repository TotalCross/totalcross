...
TCObject iso88591chars2bytes(Context currentContext, JCharP chars, int32 length) {
	...
	if (byteArray != null) {
		...
		if (skip !=
			0) { // just adjust the final length - this will waste memory but will prevent fragmentation
...
TCObject utf8bytes2chars(Context currentContext, uint8* bytes, int32 length) {
	...
	if (charArray) {
		...
		if (resultingLength !=
			length) { // just adjust the final length - this will waste memory but will prevent fragmentation
...
TCObject utf8chars2bytes(Context currentContext, JCharP chars, int32 length) {
	TCObject byteArray = createByteArray(currentContext, utf8len(chars,
...
TCObject chars2bytes(Context currentContext, JCharP chars, int32 length) {
	...
	if (*charConverterPtr != lastCharConverter) {
		chars2bytesMtd = getMethod(OBJ_CLASS(*charConverterPtr), true, "chars2bytes", 3, CHAR_ARRAY, J_INT,
								   J_INT);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsCC_bytes2chars_Bii(NMParams
								 p) { // totalcross/sys/CharacterConverter native public char[]bytes2chars(byte []bytes, int offset, int length);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsCC_chars2bytes_Cii(NMParams
								 p) { // totalcross/sys/CharacterConverter native public byte[] chars2bytes(char []chars, int offset, int length);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsUTF8CC_bytes2chars_Bii(NMParams
									 p) { // totalcross/sys/UTF8CharacterConverter native public char[]bytes2chars(byte []bytes, int offset, int length);
	TCObject bytes = p->obj[1];
	int32 offset = p->i32[0];
	int32 len = p->i32[1];
	if (checkArrayRange(p->currentContext, bytes, offset, len)) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tsUTF8CC_chars2bytes_Cii(NMParams
									 p) { // totalcross/sys/UTF8CharacterConverter native public byte[] chars2bytes(char []chars, int offset, int length);
	TCObject chars = p->obj[1];
	int32 offset = p->i32[0];
	int32 len = p->i32[1];
	if (checkArrayRange(p->currentContext, chars, offset, len)) {
