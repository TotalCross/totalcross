...
TC_API bool validatePath(TCHARP path) {
	...
	if (auxP && (auxP[2] == 0 || auxP[2] == '/' || (auxP[2] == '.' && (auxP[3] == 0
				 || auxP[3] == '/')))) {
		return false;
	}
	return true;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_getDeviceAlias(NMParams
							   p) { // totalcross/io/File native private static String getDeviceAlias();
#if defined (darwin)
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_isCardInserted_i(NMParams
								 p) { // totalcross/io/File native public static boolean isCardInserted(int slot);
	int32 slot = p->i32[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_create_sii(NMParams
						   p) { // totalcross/io/File native private void create(String path, int mode, int slot) throws totalcross.io.IOException;
	...
	if (mode != DONT_OPEN) {
		...
		if (mode != CREATE && mode != CREATE_EMPTY && !fileExists(szPath, File_slot(file))) {
			throwFileNotFoundException(p->currentContext, szPath);
											  ...
											  sizeof(NATIVE_FILE))) != null) { // created fileRef will be unlocked only in native close
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_close(NMParams
					  p) { // totalcross/io/File native public void close() throws totalcross.io.IOException;
...
TC_API void tiF_createDir(NMParams
						  p) { // totalcross/io/File native public void createDir() throws totalcross.io.IOException;
	TCObject file = p->obj[0];
...
TC_API void tiF_delete(NMParams
					   p) { // totalcross/io/File native public void delete() throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	TCObject path = File_path(file);
	...
	if (mode == INVALID || mode == CLOSED) {
		...
		if (!fileExists(szPath, slot)) {
			...
			if (IS_DEBUG_CONSOLE(
					szPath)) { // guich@tc100b5_39: close the debug console if we're trying to delete it
				closeDebug();
			}
			fref = (NATIVE_FILE*) ARRAYOBJ_START(fileRef);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_getSize(NMParams
						p) { // totalcross/io/File native public int getSize() throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	TCObject path = File_path(file);
	TCObject fileRef = File_fileRef(file);
	int32 mode = File_mode(file);
	...
	if (mode == INVALID || mode == CLOSED) {
		...
		if (len > 0 && szPath[len - 1] == '/') {
			...
			throwException(p->currentContext, IOException,
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_listFiles(NMParams
						  p) { // totalcross/io/File native public String []listFiles() throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	TCObject path = File_path(file);
	int32 mode = File_mode(file);
	int32 slot = File_slot(file);
	...
	if (mode == INVALID || mode == CLOSED) {
		...
		if (!fileExists(szPath,
						slot)) { //flsobral@tc125_27: should now throw a FileNotFoundException if the given path is not found.
			throwFileNotFoundException(p->currentContext, szPath);
		} else if ((err = listFiles(szPath, slot, &list, &count, h, LF_NONE)) != NO_ERROR) {
			throwExceptionWithCode(p->currentContext, IOException, err);
		} else if (list != null) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_readBytes_Bii(NMParams
							  p) { // totalcross/io/File native public int readBytes(byte []b, int off, int len) throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	...
	} else if (!checkArrayRange(p->currentContext, bytes, off,
								len)) // off < 0 || len < 0 || off + len > (int32) ARRAYOBJ_LEN(bytes))
		;
	else {
		fref = (NATIVE_FILE*) ARRAYOBJ_START(fileRef);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_rename_s(NMParams
						 p) { // totalcross/io/File native public boolean rename(String path) throws totalcross.io.IOException;
	TCObject file = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_setPos_i(NMParams
						 p) { // totalcross/io/File native public void setPos(int pos) throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	int32 pos = p->i32[0];
	TCObject path = File_path(file);
	TCObject fileRef = File_fileRef(file);
	...
	if (mode == INVALID || mode == CLOSED) {
		...
		if ((err = fileGetSize(*fref, szPath, &size)) != NO_ERROR) {
			throwExceptionWithCode(p->currentContext, IOException, err);
				   ...
				   && (err = fileSetSize(fref, pos + 1)) !=
				   NO_ERROR) { // guich@tc125_5: grow the file instead of throwing an exception
			throwExceptionWithCode(p->currentContext, IOException, err);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_writeBytes_Bii(NMParams
							   p) { // totalcross/io/File native public int writeBytes(byte []b, int off, int len) throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	TCObject bytes = p->obj[1];
	int32 off = p->i32[0];
	int32 len = p->i32[1];
	...
	if (mode == INVALID || mode == CLOSED) {
		...
		fref = (NATIVE_FILE*) ARRAYOBJ_START(fileRef);
		if ((err = fileWriteBytes(*fref, (char*)ARRAYOBJ_START(bytes), off, len,
								  &bytesWritten)) != NO_ERROR) {
			throwExceptionWithCode(p->currentContext, IOException, err);
		} else {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_setAttributes_i(NMParams
								p) { // totalcross/io/File native public void setAttributes(int attr) throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	int32 attr = p->i32[0];
	TCObject path = File_path(file);
	TCObject fileRef = File_fileRef(file);
	...
	if (mode == INVALID || mode == CLOSED) {
		...
		throwException(p->currentContext, IOException, "Operation cannot be used in READ_ONLY mode");
	} else if (attr < 0 || attr > 15) { // the user may reset all attributes, so 0 is a valid number
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_getAttributes(NMParams
							  p) { // totalcross/io/File native public int getAttributes() throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	int32 attr = p->i32[0];
	TCObject path = File_path(file);
	TCObject fileRef = File_fileRef(file);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_setTime_bt(NMParams
						   p) { // totalcross/io/File native public void setTime(byte whichTime, totalcross.sys.Time time) throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	...
	} else if (whichTime != 0x1 && whichTime != 0x2 && whichTime != 0x4 && whichTime != 0xF) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_getTime_b(NMParams
						  p) { // totalcross/io/File native public totalcross.sys.Time getTime(byte whichTime) throws totalcross.io.IOException;
	TCObject file = p->obj[0];
	char whichTime = (char) p->i32[0];
	TCObject path = File_path(file);
	TCObject fileRef = File_fileRef(file);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_setSize_i(NMParams
						  p) { // totalcross/io/File native public void setSize(int newSize) throws totalcross.io.IOException;
	TCObject file = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_getCardSerialNumber_i(NMParams
									  p) { // totalcross/io/File native public static String getCardSerialNumber(int slot) throws totalcross.io.IOException;
	int32 slot = p->i32[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_listRoots(NMParams
						  p) { // totalcross/io/File native public static String []listRoots()
#if defined (WIN32)
	TCHARPs* list = null;
	int32 count = 0;
	volatile TCObject arrayObj = null;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_isEmpty(NMParams
						p) { // totalcross/io/File native public boolean isEmpty() throws IOException
	TCObject file = p->obj[0];
	TCObject path = File_path(file);
	int32 mode = File_mode(file);
	int32 slot = File_slot(file);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiF_chmod_i(NMParams
						p) { // totalcross/io/File native public int chmod(int mod) throws IOException
	TCObject file = p->obj[0];
	int32 mod = p->i32[0];
	TCObject path = File_path(file);
	int32 mode = File_mode(file);
	...
	if (mode == INVALID || mode == CLOSED) {
		...
		if (fileExists(szPath, slot)) {
			throwFileNotFoundException(p->currentContext,
									   szPath);    //flsobral@tc126: throw exception if the file does not exist.
		} else {
			if (mode != DONT_OPEN) {
				fref = (NATIVE_FILE*) ARRAYOBJ_START(file);
			}
