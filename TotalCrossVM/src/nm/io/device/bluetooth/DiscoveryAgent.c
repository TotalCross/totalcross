...
//////////////////////////////////////////////////////////////////////////
TC_API void tidbDA_nativeDiscoveryAgent(NMParams
										p) { // totalcross/io/device/bluetooth/DiscoveryAgent native private void nativeDiscoveryAgent();
	...
	if ((inquiryNativeFields = createByteArray(p->currentContext, sizeof(TDeviceSearch))) == null) {
		...
		deviceSearchP->inquiryStatus =
			INQUIRY_COMPLETED; // initialize with a status different from OPERATION_IN_PROGRESS
...
//////////////////////////////////////////////////////////////////////////
TC_API void tidbDA_cancelInquiry_d(NMParams
								   p) { // totalcross/io/device/bluetooth/DiscoveryAgent native public boolean cancelInquiry(totalcross.io.device.bluetooth.DiscoveryListener listener);
#if !defined WP8 && (defined (WIN32) || defined (WINCE))
...
//////////////////////////////////////////////////////////////////////////
TC_API void tidbDA_cancelServiceSearch_i(NMParams
		p) { // totalcross/io/device/bluetooth/DiscoveryAgent native public boolean cancelServiceSearch(int transID);
}
//////////////////////////////////////////////////////////////////////////

TC_API void tidbDA_retrieveDevices_i(NMParams
									 p) { // totalcross/io/device/bluetooth/DiscoveryAgent native public totalcross.io.device.bluetooth.RemoteDevice []retrieveDevices(int option);
#ifdef ANDROID
	setObjectLock(p->retO = nativeRetrieveDevices(p->currentContext, p->i32[0] == PREKNOWN), UNLOCKED);
#endif
}
//////////////////////////////////////////////////////////////////////////
TC_API void tidbDA_nativeSearchServices_IUrd(NMParams
		p) { // totalcross/io/device/bluetooth/DiscoveryAgent native public int nativeSearchServices(int []attrSet, totalcross.io.device.bluetooth.UUID []uuidSet, totalcross.io.device.bluetooth.RemoteDevice btDev, totalcross.io.device.bluetooth.DiscoveryListener discListener) throws IOException;
#if !defined WP8 && (defined (WIN32) || defined (WINCE))
	...
	if (serviceSearchP->searchStatus == OPERATION_IN_PROGRESS) {
		...
		serviceSearchP->servicesDiscovered = getMethod(OBJ_CLASS(discListener), true, "servicesDiscovered",
											 2, J_INT, "[totalcross.io.device.bluetooth.ServiceRecord");
		serviceSearchP->serviceSearchCompleted = getMethod(OBJ_CLASS(discListener), true,
				"serviceSearchCompleted", 2, J_INT, J_INT);
		...
		if (serviceSearchP->servicesDiscovered == null || serviceSearchP->serviceSearchCompleted == null) {
			...
			if ((err = nativeSearchServices(p->currentContext, remoteDevice, serviceSearchP, attrSet, uuidSet,
											&searchStarted)) != NO_ERROR) {
				if (p->currentContext->thrownException !=
					null) { // check if an exception was already thrown by nativeSearchServices
...
//////////////////////////////////////////////////////////////////////////
TC_API void tidbDA_selectService_uib(NMParams
									 p) { // totalcross/io/device/bluetooth/DiscoveryAgent native public String selectService(totalcross.io.device.bluetooth.UUID uuid, int security, boolean master) throws IOException;
}
//////////////////////////////////////////////////////////////////////////
TC_API void tidbDA_startInquiry_id(NMParams
								   p) { // totalcross/io/device/bluetooth/DiscoveryAgent native public boolean startInquiry(int accessCode, totalcross.io.device.bluetooth.DiscoveryListener listener) throws IOException;
#if !defined WP8 && (defined (WIN32) || defined (WINCE))
	...
	if (accessCode != LIAC && accessCode != GIAC && (accessCode < 0x9E8B00 || accessCode > 0x9E8B3F)) {
		...
		if (deviceSearchP->inquiryStatus == OPERATION_IN_PROGRESS) {
			...
			deviceSearchP->deviceDiscovered = getMethod(OBJ_CLASS(listener), true, "deviceDiscovered", 2,
											  "totalcross.io.device.bluetooth.RemoteDevice", "totalcross.io.device.bluetooth.DeviceClass");
			deviceSearchP->inquiryCompleted = getMethod(OBJ_CLASS(listener), true, "inquiryCompleted", 1,
											  J_INT);

			if (deviceSearchP->deviceDiscovered == null
				|| deviceSearchP->inquiryCompleted ==
				null) { // || servicesDiscovered == null || serviceSearchCompleted == null)
				throwException(p->currentContext, NoSuchMethodError, null);    // methods not found?
			} else {
				...
				if ((err = nativeStartInquiry(deviceSearchP, &inquiryStarted)) != NO_ERROR) {
					...
					deviceSearchP->inquiryStatus =
						OPERATION_IN_PROGRESS; // inquiryStarted return true, set status to OPERATION_IN_PROGRESS.
