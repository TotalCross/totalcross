...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidbSPC_createSerialPortClient_s(NMParams p)
{ // totalcross/io/device/bluetooth/SerialPortClient native
  // private void createSerialPortClient(String address, int
  // port, String []params) throws totalcross.io.IOException;
  ...
  if ((nativeHandleObj =
    ...
    if ((err = btsppClientCreate(nativeHandle, address, channel)) != NO_ERROR) {
      ...
      throwException(p->currentContext,
                     IOException,
                     err == BT_INVALID_PASSWORD ? "Invalid password"
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidbSPC_readBytes_Bii(NMParams p)
{ // totalcross/io/device/bluetooth/SerialPortClient native
  // public int readBytes(byte []b, int offset, int count)
  ...
  if ((err = btsppClientRead(
         nativeHandle, byteArrayP, offset, count, &bytesRead)) != NO_ERROR) {
    throwExceptionWithCode(p->currentContext, IOException, err);
  }
  p->retI = bytesRead;
#else
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidbSPC_writeBytes_Bii(NMParams p)
{ // totalcross/io/device/bluetooth/SerialPortClient native
  // public int writeBytes(byte []b, int offset, int count)
  // throws totalcross.io.IOException;
#if !defined WP8 && (defined(WIN32) || defined(WINCE) || defined(ANDROID))
  TCObject serialPortClientObj = p->obj[0];
  TCObject byteArrayObj = p->obj[1];
  ...
  if ((err = btsppClientWrite(
         nativeHandle, byteArrayP, offset, count, &bytesWritten)) != NO_ERROR)
#ifdef ANDROID
    throwException(
      p->currentContext, IOException, "Error connecting to device");
#else
    throwExceptionWithCode(p->currentContext, IOException, err);
#endif
  p->retI = bytesWritten;
