...
//////////////////////////////////////////////////////////////////////////
TC_API void tidbSPC_createSerialPortClient_s(NMParams
		p) { // totalcross/io/device/bluetooth/SerialPortClient native private void createSerialPortClient(String address, int port, String []params) throws totalcross.io.IOException;
	...
	if ((nativeHandleObj = createByteArray(p->currentContext, sizeof(NATIVE_HANDLE))) != null) {
		...
		if ((err = btsppClientCreate(nativeHandle, address, channel)) != NO_ERROR) {
			...
			throwException(p->currentContext, IOException,
						   err == BT_INVALID_PASSWORD ? "Invalid password" : "Error connecting to device");
#else
...
//////////////////////////////////////////////////////////////////////////
TC_API void tidbSPC_readBytes_Bii(NMParams
								  p) { // totalcross/io/device/bluetooth/SerialPortClient native public int readBytes(byte []b, int offset, int count) throws totalcross.io.IOException;
#if !defined WP8 && (defined (WIN32) || defined (WINCE) || defined (ANDROID))
	TCObject serialPortClientObj = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tidbSPC_writeBytes_Bii(NMParams
								   p) { // totalcross/io/device/bluetooth/SerialPortClient native public int writeBytes(byte []b, int offset, int count) throws totalcross.io.IOException;
#if !defined WP8 && (defined (WIN32) || defined (WINCE) || defined (ANDROID))
	TCObject serialPortClientObj = p->obj[0];
	TCObject byteArrayObj = p->obj[1];
	int32 offset = p->i32[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tidbSPC_close(NMParams
						  p) { // totalcross/io/device/bluetooth/SerialPortClient native public void close() throws totalcross.io.IOException;
#if !defined WP8 && (defined (WIN32) || defined (WINCE) || defined (ANDROID))
	TCObject serialPortClientObj = p->obj[0];
	TCObject nativeHandleObj = SerialPortClient_nativeHandle(serialPortClientObj);
	NATIVE_HANDLE* nativeHandle;
