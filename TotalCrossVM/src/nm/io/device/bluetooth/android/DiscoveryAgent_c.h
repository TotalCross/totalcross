...
static TCObject nativeRetrieveDevices(Context currentContext, bool isPaired) {
	...
	jmethodID m = (*env)->GetStaticMethodID(env, jBluetooth4A,
											isPaired ? "getPairedDevices" : "getUnpairedDevices", "()[Ljava/lang/String;");
		...
		if (outArray != null) {
			...
			for (i = 0; i < len; i++, out++) {
				...
				if (addr != null && name != null) {
					*out = createObjectWithoutCallingDefaultConstructor(currentContext,
