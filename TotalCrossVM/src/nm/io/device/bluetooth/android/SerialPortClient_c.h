...
static Err btsppClientReadWrite(bool isRead, NATIVE_HANDLE* nativeHandle, uint8* byteArrayP,
								int32 offset, int32 count, int32* bytesRead) {
	JNIEnv* env = getJNIEnv();
	...
	ret = (*env)->CallStaticIntMethod(env, jBluetooth4A, isRead ? jread : jwrite, jaddress, jbytesP, 0,
									  (jint)count);
...
static Err btsppClientRead(NATIVE_HANDLE* nativeHandle, uint8* byteArrayP, int32 offset,
						   int32 count, int32* bytesRead) {
	return btsppClientReadWrite(true, nativeHandle, byteArrayP, offset, count, bytesRead);
}

static Err btsppClientWrite(NATIVE_HANDLE* nativeHandle, uint8* byteArrayP, int32 offset,
							int32 count, int32* bytesWritten) {
