...
static Err nativeRetrieveDevices(VoidPs** devices, Heap h, int32* count) {
	...
	*count = 0;
	...
	do {
		...
		if ((err = WSALookupServiceNext(hLookup, LUP_RETURN_NAME | LUP_RETURN_ADDR, &queryBufLen,
										queryBufP)) == NO_ERROR) {
...
DWORD WINAPI ThreadStartInquiry(LPVOID lpParameter) {
	...
	while (keepRunning) {
		queryBufLen = QUERY_BUF_SIZE;
		if ((err = WSALookupServiceNext(*hLookupP, LUP_RETURN_NAME | LUP_RETURN_ADDR | LUP_RETURN_BLOB,
										&queryBufLen, queryBufP)) != NO_ERROR) {
			err = WSAGetLastError();
			break; // only case we break out of the loop.
		} else if ((remoteDevice = createObjectWithoutCallingDefaultConstructor(
									   deviceSearchP->currentContext, "totalcross.io.device.bluetooth.RemoteDevice")) == null) {
			keepRunning = false;
		} else {
			if ((deviceClass = createObjectWithoutCallingDefaultConstructor(deviceSearchP->currentContext,
							   "totalcross.io.device.bluetooth.DeviceClass")) == null) {
				keepRunning = false;
			} else {
				if (deviceSearchP->remoteDeviceConstructor == null
					&& (deviceSearchP->remoteDeviceConstructor = getMethod(OBJ_CLASS(remoteDevice), false,
							CONSTRUCTOR_NAME, 1, "java.lang.String")) == null) {
					keepRunning = false;
				} else if (deviceSearchP->deviceClassConstructor == null
						   && (deviceSearchP->deviceClassConstructor = getMethod(OBJ_CLASS(deviceClass), false,
								   CONSTRUCTOR_NAME, 1, J_INT)) == null) {
					keepRunning = false;
				} else {
					...
					if (queryBufP->lpszServiceInstanceName == null) {
						...
						friendlyName = createStringObjectFromTCHARP(deviceSearchP->currentContext,
									   queryBufP->lpszServiceInstanceName, -1);
					}

					if (friendlyName == null) {
						keepRunning = false;
					} else {
						if ((address = createStringObjectFromTCHARP(deviceSearchP->currentContext, szAddress,
									   -1)) == null) {
							keepRunning = false;
						} else {
							// Call RemoteDevice constructor and set the object's fields
							executeMethod(deviceSearchP->currentContext, deviceSearchP->remoteDeviceConstructor, remoteDevice,
										  address);
							RemoteDevice_friendlyName(remoteDevice) = friendlyName;
							setObjectLock(address, UNLOCKED);

							// Call DeviceClass constructor
							executeMethod(deviceSearchP->currentContext, deviceSearchP->deviceClassConstructor, deviceClass,
										  getCOD(bthInquiry));
							executeMethod(deviceSearchP->currentContext, deviceSearchP->deviceDiscovered,
										  deviceSearchP->listener, remoteDevice, deviceClass);
						}
	...
	executeMethod(deviceSearchP->currentContext, deviceSearchP->inquiryCompleted,
				  deviceSearchP->listener, deviceSearchP->inquiryStatus);
...
void convertUUIDBytesToGUID(uint8* bytes, GUID* uuid) {
	...
	uuid->Data1 = bytes[0] << 24 & 0xff000000 | bytes[1] << 16 & 0x00ff0000 | bytes[2] << 8 & 0x0000ff00
				  | bytes[3] & 0x000000ff;
...
Err getServicesHandle(Context currentContext, TCObject uuidSet, BTH_ADDR btAddress,
					  ULONG** serviceRecordHandleArray, int32* serviceRecordHandleArrayLen) {
	...
	*serviceRecordHandleArrayLen = 0;
	...
	if (WSALookupServiceBegin(&querySet, 0, &hLookupSearchServices) != NO_ERROR) {
		...
		if ((pwsaResults = (LPWSAQUERYSET) xmalloc(wsaResultSize)) == null) {
			...
			if ((err = WSALookupServiceNext(hLookupSearchServices, 0, &wsaResultSize,
											pwsaResults)) != NO_ERROR) {
				err = WSAGetLastError();
...
Err getServiceAttributes(Context currentContext, ULONG serviceHandle, BTH_ADDR btAddress,
						 TCObject attrSet, TCObject* byteArray) {
	...
	if ((queryService = (BTHNS_RESTRICTIONBLOB*) xmalloc(sizeof(BTHNS_RESTRICTIONBLOB) + sizeof(
							SdpAttributeRange))) == null) {
		throwException(currentContext, OutOfMemoryError, null);
	} else {
		...
		queryService->pRange[0].maxAttribute = *((int32*) ARRAYOBJ_START(attrSet) + (ARRAYOBJ_LEN(
				attrSet) -
											   1)); //SDP_ATTRIB_PROTOCOL_DESCRIPTOR_LIST; //(USHORT)ints[env->GetArrayLength(attrIDs)-1];
...
// Mutex will be already locked by tidbDA_startInquiry_id, we'll always have exclusive access to the search structure from here.
Err nativeSearchServices(Context currentContext, TCObject remoteDevice,
						 ServiceSearch serviceSearchP, TCObject attrSet, TCObject uuidSet, boolean* searchStarted) {
	...
	if (ARRAYOBJ_LEN(uuidSet) > MAX_UUIDS_IN_QUERY) {
		throwException(currentContext, IllegalArgumentException,
					   "Maximum number of service attributes to be retrieved per service record exceeded.");
		...
		err = getServicesHandle(currentContext, uuidSet, *btAddr, &serviceRecordHandleArray,
		...
		if (err != NO_ERROR && currentContext->thrownException == null) {
			...
			if (serviceRecordHandleArrayLen == 0) { // no service records found, just finish the search.
				executeMethod(serviceSearchP->currentContext, serviceSearchP->serviceSearchCompleted,
							  serviceSearchP->listener, 0, SERVICE_SEARCH_NO_RECORDS);
			} else {
				int32 result;
				TCObject byteArray;
				TCObject serviceRecordArray = createArrayObject(serviceSearchP->currentContext,
											  "[totalcross.io.device.bluetooth.ServiceRecord", serviceRecordHandleArrayLen);
				if (serviceRecordArray != null) {
					Method readSDP = getMethod(loadClass(serviceSearchP->currentContext,
														 "totalcross.io.device.bluetooth.ServiceRecord", false), false, "readSDP", 3,
											   "totalcross.io.device.bluetooth.RemoteDevice", BYTE_ARRAY, INT_ARRAY);
					if (readSDP != null) {
						for (i = 0 ; i < serviceRecordHandleArrayLen ; i++) {
							TCObject serviceRecord = createObject(serviceSearchP->currentContext,
							...
							if (serviceRecord != null) {
								...
								result = executeMethod(serviceSearchP->currentContext, readSDP, serviceRecord, remoteDevice,
													   byteArray, attrSet).asInt32;
						...
						executeMethod(serviceSearchP->currentContext, serviceSearchP->servicesDiscovered,
									  serviceSearchP->listener, 0, serviceRecordArray);
						executeMethod(serviceSearchP->currentContext, serviceSearchP->serviceSearchCompleted,
									  serviceSearchP->listener, 0, SERVICE_SEARCH_COMPLETED);
					}
