...
DWORD WINAPI
ThreadStartInquiry(LPVOID lpParameter);
DWORD WINAPI
ThreadStartSearch(LPVOID lpParameter);

static Err
nativeRetrieveDevices(VoidPs** devices, Heap h, int32* count)
{
  WSADATA wsaData;
  ...
  *count = 0;
  ...
  if ((err = WSALookupServiceBegin(&querySet, LUP_CONTAINERS, &hLookup)) !=
      NO_ERROR) {
    err = WSAGetLastError();
    WSACleanup();
    return err;
  ...
  do {
    ...
    if ((err = WSALookupServiceNext(hLookup,
      ...
      btAddr =
        ((SOCKADDR_BTH*)queryBufP->lpcsaBuffer->RemoteAddr.lpSockaddr)->btAddr;
      _stprintf(
        btDeviceP->address, TEXT("%04x%08x"), GET_NAP(btAddr), GET_SAP(btAddr));
...
// Mutex will be already locked by tidbDA_cancelInquiry_d, we'll always have
// exclusive access to the search structure from here.
void
nativeCancelInquiry(DeviceSearch deviceSearchP)
{
  HANDLE* hLookupP = &((NATIVE_FIELDS)deviceSearchP->nativeFields)->hLookup;
  WSALookupServiceEnd(*hLookupP);
}

// Mutex will be already locked by tidbDA_startInquiry_id, we'll always have
// exclusive access to the search structure from here.
Err
nativeStartInquiry(DeviceSearch deviceSearchP, boolean* inquiryStarted)
{
  WSADATA wsaData;
  WSAQUERYSET querySet;
  HANDLE* hLookupP = &((NATIVE_FIELDS)deviceSearchP->nativeFields)->hLookup;
  ...
  *inquiryStarted = false;
  ...
  if ((err = WSALookupServiceBegin(&querySet, LUP_CONTAINERS, hLookupP)) !=
      NO_ERROR) {
    err = WSAGetLastError();
    WSACleanup();
    return err;
  ...
  if ((inquiryThread = CreateThread(
         null, 0, ThreadStartInquiry, deviceSearchP, 0, null)) == null) {
    err = GetLastError();
    WSALookupServiceEnd(*hLookupP);
    WSACleanup();
    return err;
...
ThreadStartInquiry(LPVOID lpParameter)
{
  ...
  while (keepRunning) {
    queryBufLen = QUERY_BUF_SIZE;
    if ((err = WSALookupServiceNext(*hLookupP,
                                    LUP_RETURN_NAME | LUP_RETURN_ADDR |
                                      LUP_RETURN_BLOB,
                                    &queryBufLen,
                                    queryBufP)) != NO_ERROR) {
    ...
    } else if ((remoteDevice = createObjectWithoutCallingDefaultConstructor(
                  deviceSearchP->currentContext,
                  "totalcross.io.device.bluetooth.RemoteDevice")) == null) {
      keepRunning = false;
    } else {
      if ((deviceClass = createObjectWithoutCallingDefaultConstructor(
             deviceSearchP->currentContext,
             "totalcross.io.device.bluetooth.DeviceClass")) == null) {
        keepRunning = false;
      } else {
        if (deviceSearchP->remoteDeviceConstructor == null &&
            (deviceSearchP->remoteDeviceConstructor =
               getMethod(OBJ_CLASS(remoteDevice),
                         false,
                         CONSTRUCTOR_NAME,
                         1,
                         "java.lang.String")) == null) {
          keepRunning = false;
        } else if (deviceSearchP->deviceClassConstructor == null &&
                   (deviceSearchP->deviceClassConstructor =
                      getMethod(OBJ_CLASS(deviceClass),
                                false,
                                CONSTRUCTOR_NAME,
                                1,
                                J_INT)) == null) {
          ...
          // Copy bluetooth device address
          btAddr =
            ((SOCKADDR_BTH*)queryBufP->lpcsaBuffer->RemoteAddr.lpSockaddr)
              ->btAddr;
          _stprintf(
            szAddress, TEXT("%04x%08x"), GET_NAP(btAddr), GET_SAP(btAddr));

          // Copy device name
          if (queryBufP->lpszServiceInstanceName == null) {
            friendlyName =
              createStringObjectWithLen(deviceSearchP->currentContext, 0);
          } else {
            friendlyName =
              createStringObjectFromTCHARP(deviceSearchP->currentContext,
                                           queryBufP->lpszServiceInstanceName,
                                           -1);
          }
          ...
          if (friendlyName == null) {
            ...
            if ((address = createStringObjectFromTCHARP(
                   deviceSearchP->currentContext, szAddress, -1)) == null) {
              keepRunning = false;
            } else {
              // Call RemoteDevice constructor and set the object's fields
              executeMethod(deviceSearchP->currentContext,
                            deviceSearchP->remoteDeviceConstructor,
                            remoteDevice,
                            address);
              ...
              executeMethod(deviceSearchP->currentContext,
                            ...
                            getCOD(bthInquiry));
              executeMethod(deviceSearchP->currentContext,
                            deviceSearchP->deviceDiscovered,
                            deviceSearchP->listener,
                            remoteDevice,
                            deviceClass);
            }
  ...
  executeMethod(deviceSearchP->currentContext,
                deviceSearchP->inquiryCompleted,
                deviceSearchP->listener,
                deviceSearchP->inquiryStatus);
  UNLOCKVAR(deviceSearchP->deviceInquiry);

  // the thread returns the error value - which may be retrieved with
  // GetExitCodeThread
  return err;
}

void
convertUUIDBytesToGUID(uint8* bytes, GUID* uuid)
{
  int32 i;
...
Err
getServicesHandle(Context currentContext,
                  TCObject uuidSet,
                  BTH_ADDR btAddress,
  ...
  *serviceRecordHandleArrayLen = 0;
  ...
  if (WSALookupServiceBegin(&querySet, 0, &hLookupSearchServices) != NO_ERROR) {
    ...
    if ((pwsaResults = (LPWSAQUERYSET)xmalloc(wsaResultSize)) == null) {
      ...
      if ((err = WSALookupServiceNext(
             hLookupSearchServices, 0, &wsaResultSize, pwsaResults)) !=
          NO_ERROR) {
        err = WSAGetLastError();
        if (err == WSANO_DATA || err == WSA_E_NO_MORE ||
            err == WSAENOMORE) { // NO MORE RECORDS
          err = NO_ERROR;
        }
      } else if (((*serviceRecordHandleArray) =
                    (ULONG*)xmalloc(pwsaResults->lpBlob->cbSize)) == null) {
        throwException(currentContext, OutOfMemoryError, null);
      } else {
        *serviceRecordHandleArrayLen =
          pwsaResults->lpBlob->cbSize / sizeof(ULONG);
        xmemmove(*serviceRecordHandleArray,
...
Err
getServiceAttributes(Context currentContext,
                     ULONG serviceHandle,
                     BTH_ADDR btAddress,
                     TCObject attrSet,
                     TCObject* byteArray)
{
  ...
  if ((queryService = (BTHNS_RESTRICTIONBLOB*)xmalloc(
         sizeof(BTHNS_RESTRICTIONBLOB) + sizeof(SdpAttributeRange))) == null) {
    throwException(currentContext, OutOfMemoryError, null);
  } else {
    ...
    queryService->pRange[0].minAttribute = *((int32*)ARRAYOBJ_START(
      attrSet)); // SDP_ATTRIB_PROTOCOL_DESCRIPTOR_LIST; //(USHORT)ints[0];
    queryService->pRange[0].maxAttribute =
      *((int32*)ARRAYOBJ_START(attrSet) +
        (ARRAYOBJ_LEN(attrSet) -
         1)); // SDP_ATTRIB_PROTOCOL_DESCRIPTOR_LIST;
              // //(USHORT)ints[env->GetArrayLength(attrIDs)-1];
    ...
    if (WSALookupServiceBegin(&queryset, 0, &hLookupServiceAttributes) !=
      ...
      if ((pwsaResults = (LPWSAQUERYSET)xmalloc(pwsaResultsSize)) == null) {
        ...
        if (WSALookupServiceNext(
              hLookupServiceAttributes, 0, &pwsaResultsSize, pwsaResults) !=
            NO_ERROR) {
          err = WSAGetLastError();
        } else {
          *byteArray =
            createByteArray(currentContext, pwsaResults->lpBlob->cbSize);
          xmemmove(ARRAYOBJ_START(*byteArray),
                   pwsaResults->lpBlob->pBlobData,
                   pwsaResults->lpBlob->cbSize);
        }
        xfree(pwsaResults);
...
// exclusive access to the search structure from here.
Err
nativeSearchServices(Context currentContext,
                     TCObject remoteDevice,
                     ServiceSearch serviceSearchP,
                     TCObject attrSet,
                     TCObject uuidSet,
                     boolean* searchStarted)
{
  ...
  if (ARRAYOBJ_LEN(uuidSet) > MAX_UUIDS_IN_QUERY) {
    throwException(currentContext,
                   ...
                   "Maximum number of service attributes to be retrieved per "
                   "service record exceeded.");
  } else if (radix2long(serviceSearchP->address, 16, btAddr) == false) {
    throwException(
      currentContext, IllegalArgumentException, "Device address is not valid.");
  } else {
    ULONG* serviceRecordHandleArray;
    int32 serviceRecordHandleArrayLen;

    err = getServicesHandle(currentContext,
                            uuidSet,
                            *btAddr,
                            &serviceRecordHandleArray,
                            &serviceRecordHandleArrayLen);
    if (err != NO_ERROR && currentContext->thrownException == null) {
      throwExceptionWithCode(currentContext, IOException, err);
    } else if (err == NO_ERROR) {
      if (serviceRecordHandleArrayLen ==
          0) { // no service records found, just finish the search.
        executeMethod(serviceSearchP->currentContext,
                      serviceSearchP->serviceSearchCompleted,
                      serviceSearchP->listener,
                      0,
                      SERVICE_SEARCH_NO_RECORDS);
      } else {
        int32 result;
        TCObject byteArray;
        TCObject serviceRecordArray =
          createArrayObject(serviceSearchP->currentContext,
                            "[totalcross.io.device.bluetooth.ServiceRecord",
                            serviceRecordHandleArrayLen);
        if (serviceRecordArray != null) {
          Method readSDP =
            getMethod(loadClass(serviceSearchP->currentContext,
                                "totalcross.io.device.bluetooth.ServiceRecord",
                                false),
                      false,
                      "readSDP",
          ...
          if (readSDP != null) {
            for (i = 0; i < serviceRecordHandleArrayLen; i++) {
              TCObject serviceRecord =
                createObject(serviceSearchP->currentContext,
                             "totalcross.io.device.bluetooth.ServiceRecord");
              if (serviceRecord != null) {
                getServiceAttributes(currentContext,
                                     serviceRecordHandleArray[i],
                                     *btAddr,
                                     attrSet,
                                     &byteArray);
                result = executeMethod(serviceSearchP->currentContext,
                                       readSDP,
                                       serviceRecord,
                                       remoteDevice,
                                       byteArray,
                ...
                *((TCObjectArray)ARRAYOBJ_START(serviceRecordArray) + i) =
                  serviceRecord;
            ...
            executeMethod(serviceSearchP->currentContext,
                          serviceSearchP->servicesDiscovered,
                          serviceSearchP->listener,
                          0,
                          serviceRecordArray);
            executeMethod(serviceSearchP->currentContext,
