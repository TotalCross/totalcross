...
static GUID _SerialPortServiceClassID_UUID16 = {
  0x00001101,
  0x0000,
  0x1000,
...
// auxiliary functions
static Err
RegisterBtService(GUID* pguid, GUID* pguid2, byte bChannel, ULONG* pRecord)
{
  // SDP dummy record
  // GUID goes at offset 8
  // Channel goes in last byte of record.
  static BYTE bSDPRecord[] = { 0x35, 0x27, 0x09, 0x00, 0x01, 0x35, 0x11,
                               0x1C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                               0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                               0x00, 0x00, 0x00, 0x09, 0x00, 0x04, 0x35,
                               0x0C, 0x35, 0x03, 0x19, 0x01, 0x00, 0x35,
                               0x05, 0x19, 0x00, 0x03, 0x08, 0x00 };
  ...
  return PublishRecord(
    bSDPRecord, pguid2, sizeof(bSDPRecord), pRecord, (BTHNS_SETBLOB*)&setBlob);
}

// interface functions
static Err
btsppServerCreate(NATIVE_HANDLE* nativeHandle, GUID guid)
{
  WSADATA wsaData;
  ...
  *nativeHandle = INVALID_SOCKET;
  ...
  if ((*nativeHandle = socket(AF_BTH, SOCK_STREAM, BTHPROTO_RFCOMM)) ==
      INVALID_SOCKET) {
    goto Error;
  }
  ...
  sockAddrP =
    (struct sockaddr*)&sockAddrBth; // sockaddr* to the bluetooth struct to
                                    // use with winsock functions.
  ...
  //
  if ((err = RegisterBtService(&_SerialPortServiceClassID_UUID16,
                               &guid,
                               (byte)sockAddrBth.port,
                               &recordHandle)) != NO_ERROR) {
    goto Error;
  }

  //
  // listen() call indicates winsock2 to listen on a given socket for any
  // incoming connection.
  //
...
static Err
btsppServerAccept(NATIVE_HANDLE* nativeHandle, NATIVE_HANDLE* clientHandle)
{
  WSADATA wsaData;
  Err err;
  //
  ...
  if ((*clientHandle = accept(*nativeHandle, null, null)) == INVALID_SOCKET) {
    err = WSAGetLastError();
    if (err == WSAEINTR) {
      // WSAEINTR is returned when the socket is closed during a blocking IO
      // operation. In this case, we'll set the handle to null to notify the
      // caller.
      *clientHandle = null;
      return NO_ERROR;
    }
  ...
  if ((err = WSAStartup(MAKEWORD(2, 2),
                        &wsaData)) !=
      NO_ERROR) { // we must call WSAStartup again for each client, because they
                  // will call WSACleanup on close.
    return err;
  }
  return NO_ERROR;
}
...
static Err
btsppServerClose(NATIVE_HANDLE* nativeHandle)
{
  Err err = NO_ERROR;
