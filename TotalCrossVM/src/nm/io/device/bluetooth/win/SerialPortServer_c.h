...
// $Id: SerialPortServer_c.h,v 1.10 2011-01-04 13:31:07 guich Exp $
...
// auxiliary functions
Err RegisterBtService(GUID* pguid, GUID* pguid2, byte bChannel, ULONG *pRecord);
Err PublishRecord(PBYTE pSDPRec, GUID* pguid2, int32 nRecSize, ULONG *pRecord, BTHNS_SETBLOB* pSetBlob);
...
static Err btsppServerAccept(NATIVE_HANDLE* nativeHandle, NATIVE_HANDLE* clientHandle)
{
   ...
   {
      if ((err = WSAGetLastError()) == WSAEINTR)
      {
