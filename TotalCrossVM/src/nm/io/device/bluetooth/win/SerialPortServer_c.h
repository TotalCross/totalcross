...
static Err PublishRecord(PBYTE pSDPRec, GUID* pguid2, int32 nRecSize, ULONG* pRecord,
...
static Err btsppServerCreate(NATIVE_HANDLE* nativeHandle, GUID guid) {
	...
	*nativeHandle = INVALID_SOCKET;
	...
	sockAddrP = (struct sockaddr*)
				&sockAddrBth; // sockaddr* to the bluetooth struct to use with winsock functions.
	...
	//
	if ((err = RegisterBtService(&_SerialPortServiceClassID_UUID16, &guid, (byte) sockAddrBth.port,
...
static Err btsppServerAccept(NATIVE_HANDLE* nativeHandle, NATIVE_HANDLE* clientHandle) {
	...
	if ((err = WSAStartup(MAKEWORD(2, 2),
						  &wsaData)) !=
		NO_ERROR) { // we must call WSAStartup again for each client, because they will call WSACleanup on close.
		return err;
	}
	return NO_ERROR;
}
