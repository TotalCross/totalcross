...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidbSPS_createSerialPortServer_s(NMParams p)
{ // totalcross/io/device/bluetooth/SerialPortServer native
  // private void createSerialPortServer(String uuid, String
  // []params) throws totalcross.io.IOException;
  ...
  if (!String2GUID(uuidObj, &guid)) {
    throwException(
      p->currentContext, IllegalArgumentException, "Invalid UUID.");
  } else if ((nativeHandleObj = createByteArray(
                p->currentContext, sizeof(NATIVE_HANDLE))) != null) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidbSPS_accept(NMParams p)
{ // totalcross/io/device/bluetooth/SerialPortServer
  // native public totalcross.io.Stream accept()
  ...
  if ((serialPortClientObj = createObject(
         p->currentContext,
         "totalcross.io.device.bluetooth.SerialPortClient")) != null &&
      (clientHandleObj =
         createByteArray(p->currentContext, sizeof(NATIVE_HANDLE))) != null) {
    ...
    if ((err = btsppServerAccept(nativeHandle, clientHandle)) != NO_ERROR) {
      ...
      // the handle is null, that means the socket was closed during this IO
      // blocking operation. we'll throw a different exception in this case.
    ...
    setObjectLock(clientHandleObj,
                  UNLOCKED); // it will be unlocked by the client's close.
  }
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidbSPS_close(NMParams p)
{ // totalcross/io/device/bluetooth/SerialPortServer
  // native public void close() throws
  // throws totalcross.io.IOException;
#if !defined WP8 && (defined(WIN32) || defined(WINCE) || defined(ANDROID))
  TCObject serialPortServerObj = p->obj[0];
  TCObject nativeHandleObj = SerialPortServer_nativeHandle(serialPortServerObj);
