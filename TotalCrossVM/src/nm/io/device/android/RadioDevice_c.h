...
static bool RdIsSupported(Context currentContext, int32 type) {
	...
	jboolean result = (*env)->CallStaticBooleanMethod(env, jRadioDevice4A, isSupportedMethod,
					  (jint) type);
	return result != 0;
}
...
static int32 RdGetState(Context currentContext, int32 type) {
	JNIEnv* env = getJNIEnv();
	jmethodID getStateMethod = ggetStateMethod ? ggetStateMethod : (ggetStateMethod =
								   (*env)->GetStaticMethodID(env, jRadioDevice4A, "getState", "(I)I"));
