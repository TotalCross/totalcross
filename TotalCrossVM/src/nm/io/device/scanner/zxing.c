...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidsS_readBarcode_s(NMParams p)
{ // totalcross/io/device/scanner/Scanner native public static
  // String readBarcode(String mode);
#ifdef ANDROID
  ...
  if (result > 0) {
    jstring jmode = mode == null
                      ? null
                      : (*env)->NewString(env,
                                          (jchar*)String_charsStart(mode),
                                          String_charsLen(mode));
    jstring result =
      (*env)->CallStaticObjectMethod(env, applicationClass, jzxing, jmode);
    (*env)->DeleteLocalRef(env, jmode);
    if (result != null) {
      const jchar* str = (*env)->GetStringChars(env, result, 0);
      if (str) {
        o = createStringObjectFromJCharP(
          p->currentContext, (JCharP)str, (*env)->GetStringLength(env, result));
      }
