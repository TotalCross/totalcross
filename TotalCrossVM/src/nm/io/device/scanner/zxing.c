...
//////////////////////////////////////////////////////////////////////////
TC_API void tidsS_readBarcode_s(NMParams
								p) { // totalcross/io/device/scanner/Scanner native public static String readBarcode(String mode);
#ifdef ANDROID
	JNIEnv* env = getJNIEnv();
	TCObject mode = p->obj[0];
	TCObject o = null;

	jmethodID method = (*env)->GetStaticMethodID(env, applicationClass, "requestCameraPermission",
					   "()I");
	jint result = (*env)->CallStaticIntMethod(env, applicationClass, method);
	if (result > 0) {
		jstring jmode = mode == null ? null : (*env)->NewString(env, (jchar*) String_charsStart(mode),
						String_charsLen(mode));
		...
		if (result != null) {
			...
			if (str) {
				o = createStringObjectFromJCharP(p->currentContext, (JCharP)str, (*env)->GetStringLength(env,
												 result));
			}
