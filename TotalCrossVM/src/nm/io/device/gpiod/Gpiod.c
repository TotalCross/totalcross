...
tidgGC_open_i(NMParams p)
{
  ...
  gpiodChipObj =
    createObject(p->currentContext, "totalcross.io.device.gpiod.GpiodChip");
  if (gpiodChipObj == null) {
    throwExceptionWithCode(p->currentContext, IOException, -1);
  } else if ((handleObj = createByteArray(p->currentContext,
                                          sizeof(NATIVE_HANDLE))) != null) {
...
tidgGL_open_gi(NMParams p)
{
  ...
  gpiodLineObj =
    createObject(p->currentContext, "totalcross.io.device.gpiod.GpiodLine");
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidgGL_requestOutput_si(NMParams p)
{
#if defined(HEADLESS) && defined(__arm__)
  TCObject gpiodLineObj = p->obj[0];
  NATIVE_HANDLE* lineHandle =
    (NATIVE_HANDLE*)ARRAYOBJ_START(GpiodLine_handle(gpiodLineObj));
  TCObject consumerObj = p->obj[1];
  int32 defaultValue = p->i32[0];
  char* consumer = String2CharP(consumerObj);

  p->retI =
    gpiod_line_request_output(lineHandle->handle, consumer, defaultValue);
  xfree(consumer);
#else
  UNUSED(p);
#endif
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidgGL_requestInput_s(NMParams p)
{
#if defined(HEADLESS) && defined(__arm__)
  TCObject gpiodLineObj = p->obj[0];
  NATIVE_HANDLE* lineHandle =
