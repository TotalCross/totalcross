...
/** Maximum size, in characters, of prefixes used for CreateFile device names.
 * For example, this is the maximum size of the szGPSDriverPrefix and
 * szGPSMultiplexPrefix fields in the GPS_POSITION structure. */
#define GPS_MAX_PREFIX_NAME 16
/** Maximum size, in characters, of data like the friendly name of an input
 * source. For example, this is the maximum size of the szGPSFriendlyName field
...
// Actually an enumeration, but its definition is not public and we won't be
// using it.
...
typedef HANDLE(__stdcall* GPSOpenDeviceProc)(HANDLE hNewLocationData,
                                             HANDLE hDeviceStateChange,
                                             const WCHAR* szDeviceName,
                                             DWORD dwFlags);

typedef DWORD(__stdcall* GPSCloseDeviceProc)(HANDLE hGPSDevice);

typedef DWORD(__stdcall* GPSGetPositionProc)(HANDLE hGPSDevice,
                                             GPS_POSITION* pGPSPosition,
                                             DWORD dwMaximumAge,
                                             DWORD dwFlags);

typedef DWORD(__stdcall* GPSGetDeviceStateProc)(GPS_DEVICE* pGPSDevice);
...
nativeStartGPS()
{
  ...
  GPSOpenDevice =
    (GPSOpenDeviceProc)GetProcAddress(gpsDll, TEXT("GPSOpenDevice"));
  GPSCloseDevice =
    (GPSCloseDeviceProc)GetProcAddress(gpsDll, TEXT("GPSCloseDevice"));
  GPSGetPosition =
    (GPSGetPositionProc)GetProcAddress(gpsDll, TEXT("GPSGetPosition"));
  GPSGetDeviceState =
    (GPSGetDeviceStateProc)GetProcAddress(gpsDll, TEXT("GPSGetDeviceState"));

  if (!GPSOpenDevice || !GPSCloseDevice || !GPSGetPosition ||
      !GPSGetDeviceState) {
    goto onError;
  }
...
static Err
nativeUpdateLocation(Context currentContext, TCObject gpsObject, int32* flags)
{
