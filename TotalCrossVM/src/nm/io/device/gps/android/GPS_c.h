...
static Err
nativeStartGPS(TCObject gpsObject)
{
  ...
  jmethodID method = (*env)->GetStaticMethodID(
    env, applicationClass, "requestLocationPermission", "()I");
  ...
  jgpsData =
    (*env)->CallStaticObjectMethod(env,
                                   ...
                                   jgpsFunc,
                                   GPSFUNC_START,
                                   GPS_precision(gpsObject)); // guich@tc125_1
  if (jgpsData != null) {
    (*env)->DeleteLocalRef(env, jgpsData);
  }
...
nativeStopGPS()
{
  ...
  if (env) {
    jobject ret = (*env)->CallStaticObjectMethod(
      env, applicationClass, jgpsFunc, GPSFUNC_STOP, -1);
    if (ret != null) {
...
static void
splitStr(char* what, char sep, char** into)
{
  int32 i = 0;
  into[i++] = what;
  for (; *what; what++)
...
nativeUpdateLocation(Context currentContext, TCObject gpsObject, int32* flags)
{
  ...
  jgpsData = (*env)->CallStaticObjectMethod(
    env, applicationClass, jgpsFunc, GPSFUNC_GETDATA, -1);
