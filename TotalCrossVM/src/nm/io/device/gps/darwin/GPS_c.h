...
nativeUpdateLocation(Context currentContext, TCObject gpsObject, int32* flags)
{
  ...
  int ret = iphone_gpsUpdateLocation(
    flags, &date, &time, &sat, &veloc, &pdop, &dir, &lat, &lon);
  ...
  if (ret == 0 && *flags != 0) {
    ...
    Time_day(lastFix) = date % 100;
    date /= 100;
    Time_month(lastFix) = date % 100;
    date /= 100;
    Time_year(lastFix) = date;
    Time_second(lastFix) = time % 100;
    time /= 100;
    Time_minute(lastFix) = time % 100;
    time /= 100;
