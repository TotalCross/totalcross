...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidPC_create_iiiii(NMParams p)
{ // totalcross/io/device/PortConnector native void create(int
  // number, int baudRate, int bits, int parity, int stopBits);
#if !defined WP8
  ...
  if (portConnectorRef != null && receiveBufferObj != null) {
    ...
    if ((err = portConnectorCreate(portConnectorHandle,
                                   *receiveBuffer,
                                   number,
                                   baudRate,
                                   bits,
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidPC_nativeClose(NMParams p)
{ // totalcross/io/device/PortConnector native
  ...
  PortHandle* portConnectorHandle =
    (PortHandle*)ARRAYOBJ_START(portConnectorRef);
  ...
  if ((err = portConnectorClose(
         *portConnectorHandle, *receiveBuffer, portNumber)) != NO_ERROR) {
    throwExceptionWithCode(p->currentContext, IOException, err);
  }
  invalidate(portConnector);
#endif
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidPC_setFlowControl_b(NMParams p)
{ // totalcross/io/device/PortConnector native public void
  ...
  if (portConnectorRef == null) {
    ...
    portConnectorHandle = (PortHandle*)ARRAYOBJ_START(portConnectorRef);
    if ((err = portConnectorSetFlowControl(*portConnectorHandle, flowOn)) !=
        NO_ERROR) {
      throwExceptionWithCode(p->currentContext, IOException, err);
    }
  }
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidPC_readWriteBytes_Biib(NMParams p)
{
#if !defined WP8
  TCObject portConnector = p->obj[0];
  ...
  bool stopWriteCheckOnTimeout =
    PortConnector_stopWriteCheckOnTimeout(portConnector);
  int32 timeout;
  int32 retCount;
  Err err;
  ...
  if ((err = portConnectorReadWriteBytes(*portConnectorHandle,
                                         portNumber,
                                         stopWriteCheckOnTimeout,
                                         timeout,
                                         ARRAYOBJ_START(byteArray),
                                         start,
                                         count,
                                         &retCount,
                                         isRead)) != NO_ERROR) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tidPC_readCheck(NMParams p)
{ // totalcross/io/device/PortConnector
  // native public int readCheck();
#if !defined WP8
  TCObject portConnector = p->obj[0];
  ...
  if (portConnectorRef == null) {
    ...
    portConnectorHandle = (PortHandle*)ARRAYOBJ_START(portConnectorRef);
    if ((err = portConnectorReadCheck(*portConnectorHandle, &inQueue)) !=
        NO_ERROR) {
      throwExceptionWithCode(p->currentContext, IOException, err);
    } else {
      p->retI = inQueue;
