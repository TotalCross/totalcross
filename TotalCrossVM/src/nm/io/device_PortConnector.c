...
//////////////////////////////////////////////////////////////////////////
TC_API void tidPC_create_iiiii(NMParams
							   p) { // totalcross/io/device/PortConnector native void create(int number, int baudRate, int bits, int parity, int stopBits);
#if !defined WP8
	...
	if (portConnectorRef != null && receiveBufferObj != null) {
		...
		if ((err = portConnectorCreate(portConnectorHandle, *receiveBuffer, number, baudRate, bits, parity,
									   stopBits, timeout)) != NO_ERROR) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tidPC_nativeClose(NMParams
							  p) { // totalcross/io/device/PortConnector native private void nativeClose();
#if !defined WP8
	TCObject portConnector = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tidPC_setFlowControl_b(NMParams
								   p) { // totalcross/io/device/PortConnector native public void setFlowControl(boolean on);
#if !defined WP8
	TCObject portConnector = p->obj[0];
...
TC_API void tidPC_readWriteBytes_Biib(NMParams p) {
	...
	if ((err = portConnectorReadWriteBytes(*portConnectorHandle, portNumber, stopWriteCheckOnTimeout,
										   timeout, ARRAYOBJ_START(byteArray), start, count, &retCount, isRead)) != NO_ERROR) {
		throwExceptionWithCode(p->currentContext, IOException, err);
	} else {
		p->retI = retCount;
	}
...
//////////////////////////////////////////////////////////////////////////
TC_API void tidPC_readCheck(NMParams
							p) { // totalcross/io/device/PortConnector native public int readCheck();
#if !defined WP8
	TCObject portConnector = p->obj[0];
