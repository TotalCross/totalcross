...
#define DEBUG_PORTCONNECTOR                                                    \
  false // MUST CONVERT IT TO THE NEW OBJECT ALLOCATION FORMAT!!!!

TESTCASE(tidPC_create_iiiii)
{ // totalcross/io/device/PortConnector native void
  // create(int number, int baudRate, int bits, int
  // parity, int stopBits);
...
TESTCASE(tidPC_close)
{ // totalcross/io/device/PortConnector native public void
  // close();
  TEST_SKIP;
finish:;
}
TESTCASE(tidPC_isOpen)
{ // totalcross/io/device/PortConnector native public
  // boolean isOpen();
  TEST_SKIP;
finish:;
}
TESTCASE(tidPC_setFlowControl_b)
{ // totalcross/io/device/PortConnector native
  // public boolean setFlowControl(boolean on);
  TEST_SKIP;
finish:;
}
TESTCASE(tidPC_readBytes_Bii)
{ // totalcross/io/device/PortConnector native public
  // int readBytes(byte []buf, int start, int count);
  TEST_SKIP;
finish:;
}
TESTCASE(tidPC_readCheck)
{ // totalcross/io/device/PortConnector native public
  // int readCheck();
  TEST_SKIP;
finish:;
}
TESTCASE(tidPC_writeBytes_Bii)
{ // totalcross/io/device/PortConnector native public
