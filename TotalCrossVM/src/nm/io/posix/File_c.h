...
static Err fileCreate(NATIVE_FILE* fref, TCHARP path, int32 mode, int32* slot) {
	...
	switch (mode) {
		...
		case CREATE:
			...
			break; //flsobral@tc123_19: fixed behaviour of File.CREATE mode on Linux based platforms.
		case CREATE_EMPTY:
			rwMode = TEXT("wb+");
			break;
		case READ_ONLY:
...
error:
	...
	if (errno == EACCES) {
		...
		JNIEnv* env = getJNIEnv();
		jmethodID method = (*env)->GetStaticMethodID(env, applicationClass, "requestStoragePermission",
						   "()I");
...
#if defined(ANDROID)
	JNIEnv* env = getJNIEnv();
	jmethodID method = (*env)->GetStaticMethodID(env, applicationClass, "fileGetFreeSpace",
					   "(Ljava/lang/String;)I");
...
static Err fileGetSize(NATIVE_FILE fref, TCHARP szPath, int32* size) {
	if (fref.handle != INVALID_HANDLE_VALUE) {
		struct stat statData;
		fileFlush(
			fref); // flsobral: must flush before getSize, otherwise the value returned may not be accurate - this fix a bug in ZipStream introduced with the forced flush in setSize.
		if (!fstat(fileno(fref.handle), &statData)) {
...
static Err fileGetTime(Context currentContext, NATIVE_FILE fref, TCHARP path, int32 whichTime,
					   TCObject* time) {
	struct stat statData;
