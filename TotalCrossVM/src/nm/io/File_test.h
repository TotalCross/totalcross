...
TCHAR buf[128];
TCObject
createFile(Context currentContext,
           const TCHARP dir,
           const TCHARP fileName,
           TCObject* outPath)
{
  TCObject obj;
  ...
  if (obj != null) {
    ...
    *outPath = File_path(obj) =
      createStringObjectFromTCHAR(currentContext, buf, -1);
 ...
 */
TCObject
createTime(Context currentContext,
           int32 year,
           int32 month,
           int32 day,
           int32 hour,
...
TCObject
CharP2Buf(Context currentContext, CharP bytes, int len)
{
  TCObject arrayObj;
  CharP array;
...
TESTCASE(tiF_create_sii)
{ // totalcross/io/File native private void
  // create(String path, int mode, int slot);
  // #DEPENDS(tiF_isCardInserted_i)
  ...
  // ASSERT1_EQUALS(Null, currentContext->thrownException); - the folder may
  // already exist.
  // **** //
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_create_sii.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);

  // #1 - Invalid path
  p.obj[1] = createStringObjectFromCharP(
    currentContext, "/thisfolderdoesnotexists/file.test", -1);
  ...
  // If p.retI is true, then the test has failed, besaucse "create" should not
  // return true
  if (p.retI) {
    tiF_delete(&p);
    ASSERT1_EQUALS(Null, currentContext->thrownException);
  ...
  ASSERT2_EQUALS(Sz,
                 ...
                 throwableAsCharP[FileNotFoundException]);
  currentContext->thrownException = null;
  ...
  tiF_close(&p);
  ASSERT1_EQUALS(Null,
                 currentContext->thrownException); // assert file was closed
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_create_sii"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
...
TESTCASE(tiF_close)
{ // totalcross/io/File native private void close();
  // #DEPENDS(tiF_create_sii)
  TNMParams p;
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_close.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  ...
  // DIRECTORY
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_nativeCloseDir"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
...
TESTCASE(tiF_createDir)
{ // totalcross/io/File native public boolean
  // createDir(); #DEPENDS(tiF_create_sii)
  TNMParams p;
  TCObject objArray[2], path;
  int32 i32Array[2];
  p.obj = objArray;
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_createDir"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = DONT_OPEN;
  p.i32[1] = lastVolume;
  ...
  tiF_exists(&p);
  ASSERT1_EQUALS(Null, currentContext->thrownException);
  if (!p.retI) { // directory exists? skip the test, because we would have to
                 // recursively delete the directory, what isn't allowed yet
    p.obj[1] = path;
    p.i32[0] = DONT_OPEN;
    p.i32[1] = lastVolume;
    tiF_createDir(&p); // create dir
...
{ // totalcross/io/File native public void delete();
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_delete.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = CREATE;
  p.i32[1] = lastVolume;
...
{ // totalcross/io/File native public boolean exists();
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_exists.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = CREATE;
  p.i32[1] = lastVolume;
...
{ // totalcross/io/File native public int getSize();
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_getSize.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = CREATE_EMPTY;
  p.i32[1] = lastVolume;
...
{ // totalcross/io/File native public boolean isDir();
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_isDir.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = CREATE;
  p.i32[1] = lastVolume;
...
TESTCASE(tiF_listFiles)
{ // totalcross/io/File native public String
  // []listFiles();  #DEPENDS(tiF_create_sii)
  TNMParams p;
  TCObject objArray[2], path;
  int32 i32Array[2];
  TCObject* list;
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_ListFiles.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = CREATE;
  p.i32[1] = lastVolume;
  ...
  found = false;
  while (--count >= 0 && !found) {
    s = JCharP2TCHARP(String_charsStart(list[count]),
                      String_charsLen(list[count]));
...
TESTCASE(tiF_rename_s)
{ // totalcross/io/File native public boolean
  // rename(String path);  #DEPENDS(tiF_create_sii)
  TNMParams p;
  TCObject objArray[2], path;
  int32 i32Array[2];
  TCObject renPath;
  ...
  p.obj[0] = createFile(
    currentContext, TEMP_PATH, TEXT("tiF_rename_s.test.renamed"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_rename_s.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = CREATE_EMPTY;
  p.i32[1] = lastVolume;
...
{ // totalcross/io/File native public int
  ...
  wbP.obj[0] = createFile(
    currentContext, TEMP_PATH, TEXT("tiF_writeBytes_Bii.test"), &path);
...
TESTCASE(tiF_setAttributes_i)
{ // totalcross/io/File native public void
  // setAttributes(int attr); #DEPENDS(tiF_create_sii)
  TNMParams p;
  TCObject objArray[2], path;
  int32 i32Array[2];
  p.obj = objArray;
  ...
  p.obj[0] = createFile(
    currentContext, TEMP_PATH, TEXT("tiF_setAttributes_i.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = CREATE;
  p.i32[1] = lastVolume;
  ...
  for (p.i32[0] = 0; p.i32[0] < 8;
       p.i32[0]++) { // do not test ATTR_SYSTEM (may fail in some platforms)
    tiF_setAttributes_i(&p);
    ASSERT1_EQUALS(Null, currentContext->thrownException);
    tiF_getAttributes(&p);
    ASSERT1_EQUALS(Null, currentContext->thrownException);
#if !defined(WIN32) &&                                                         \
  !defined(PALMOS) // POSIX does not support HIDDEN and ARCHIVE attribues
    if (p.i32[0] & 1) {
      p.retI |= 1;
    }
    if (p.i32[0] & 2) {
...
TESTCASE(tiF_setTime_bt)
{ // totalcross/io/File native public void setTime(byte
  // whichTime, totalcross.sys.Time time);
  // #DEPENDS(tiF_create_sii)
  TNMParams p;
  TCObject objArray[2], path;
  int32 i32Array[2];
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_setTime_bt.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = CREATE_EMPTY;
  p.i32[1] = lastVolume;
...
#if defined(WIN32) ||                                                          \
  defined(PALMOS) // POSIX does not support creation time of files
  // time created
...
TESTCASE(tiF_setSize_i)
{ // totalcross/io/File native public void setSize(int
  // newSize); #DEPENDS(tiF_create_sii)
  TNMParams p;
  TCObject objArray[2], path;
  int32 i32Array[2];
  p.obj = objArray;
  ...
  p.obj[0] =
    createFile(currentContext, TEMP_PATH, TEXT("tiF_setSize_i.test"), &path);
  ASSERT1_EQUALS(NotNull, p.obj[0]);
  p.obj[1] = path;
  p.i32[0] = CREATE_EMPTY;
  p.i32[1] = lastVolume;
