...
static Err fileCreate(NATIVE_FILE* fref, TCHARP path, int32 mode, int32* slot) {
	...
	LPSECURITY_ATTRIBUTES lpSecurityAttributes =
		null; // Not used in WinCE, may be null in Win32 since we won't be using it.
	DWORD dwCreationDisposition;
	DWORD dwFlagsAndAttributes =
		FILE_ATTRIBUTE_NORMAL; // file attributes are ignored while opening an existing obj, it is set to FILE_ATTRIBUTE_NORMAL for obj creation.
	...
	hFile = CreateFile(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition,
					   dwFlagsAndAttributes, hTemplateFile);
	if (hFile == INVALID_HANDLE_VALUE && mode == CREATE) {
		hFile = CreateFile(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes, OPEN_EXISTING,
						   dwFlagsAndAttributes, hTemplateFile);
	}
...
#else
{
	bool result;
	SetErrorMode(
		SEM_FAILCRITICALERRORS); // flsobral@tc115_25: no longer displays error message to user when the removable disk is not available.
...
static Err fileGetFreeSpace(TCHAR* path, int32* freeSpace, int32 slot) {
	...
	if (GetDiskFreeSpaceEx(path, null, null, &freeSize)) {
		if (freeSize.HighPart > 0
			|| freeSize.LowPart >
			2147483647) { //flsobral@tc122_25: the low part may be 64 bits in some systems for no particular reason.
		...
		*isEmpty = true;
		if ((hFind = FindFirstFile(searchPath, &findData)) != INVALID_HANDLE_VALUE) {
			do {
				...
				if (findData.cFileName[0] != '.' || (tcscmp(findData.cFileName, TEXT("."))
													 && tcscmp(findData.cFileName, TEXT("..")))) // first check is just for speedup
#endif
				{
					*isEmpty = false;
					break;
...
static Err fileReadBytes(NATIVE_FILE fref, CharP bytes, int32 offset, int32 length,
						 int32* bytesRead) {
	return (ReadFile(fref.handle, (bytes + offset), length, bytesRead,
...
static Err fileWriteBytes(NATIVE_FILE fref, CharP bytes, int32 offset, int32 length,
						  int32* bytesWritten) {
	return (WriteFile(fref.handle, (bytes + offset), length, bytesWritten,
					  null) ? NO_ERROR : GetLastError());
}
...
static Err fileSetTime(NATIVE_FILE fref, TCHARP path, char whichTime, TCObject time) {
	 ...
	 */
#ifndef WP8
	return (SetFileTime(fref.handle, creationTime, lastAccessTime,
						lastWriteTime) ? NO_ERROR : GetLastError());
#else
	{
		char buff[40];
		...
		return SetFileInformationByHandle(fref.handle, FileBasicInfo, finfo,
										  sizeof(buff)) ? NO_ERROR : GetLastError();
	}
#endif
}
...
static Err fileGetTime(Context currentContext, NATIVE_FILE fref, TCHARP path, char whichTime,
					   TCObject* time) {
	FILETIME systemFileTime, localFileTime;
	SYSTEMTIME fileTime;
...
static Err fileSetSize(NATIVE_FILE* fref, int32 newSize) {
	...
	if (SetEndOfFile(fref->handle) == 0) {
		...
		if (error == ERROR_INVALID_PARAMETER && newSize == 0) {
			CloseHandle(fref->handle);
			fref->handle = CreateFile(fref->path, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, null,
...
static Err fileGetCardSerialNumber(int32 slot, CharP serialNumber) {
	...
	typedef BOOL (__stdcall * procFindNextFlashCard)(HANDLE hFlashCard,
	...
	FindFirstFlashCardProc = (procFindFirstFlashCard) GetProcAddress(note_prjDll,
							 TEXT("FindFirstFlashCard"));
	FindNextFlashCardProc = (procFindNextFlashCard) GetProcAddress(note_prjDll,
							TEXT("FindNextFlashCard"));
	...
	if ((hFoundCard = CreateFile(buf, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, null, OPEN_EXISTING, 0,
	...
	if (!DeviceIoControl(hFoundCard, IOCTL_DISK_GET_STORAGEID, null, 0, storageIDP, 256, &dwNumReturned,
						 null)) {
		CloseHandle(hFoundCard);
		goto error;
	}

	xstrncpy(serialNumber, (((CharP)storageIDP) + storageIDP->dwSerialNumOffset),
...
static Err fileListRoots(TCHARPs** list, int32* count, Heap h) {
	...
	*list = TCHARPsAdd(*list, logicalDrives, h); // add entry to list
	...
	if ((hFind = FindFirstFile(searchPath, &findData)) == INVALID_HANDLE_VALUE) {
		...
		return errCode != ERROR_NO_MORE_FILES
			   && errCode != ERROR_FILE_NOT_FOUND ? errCode :
			   NO_ERROR; // guich@tc113_17: "file not found" is thrown in empty folders
	}
#if WIN32_WCE <= 400
	errCodeBug =
		GetLastError(); // some old implementations return a strange error code in SUCCESS situations. So we store it here to compare with it again below.
#endif
	do {
		if ((findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			...
			fileName = (TCHARP)heapAlloc(h,
										 sizeof(TCHAR) * (fileNameSize +
												 pathlen)); // note: if an heap error occurs, the FindClose will never be called; TODO change the implementation to allow that.
