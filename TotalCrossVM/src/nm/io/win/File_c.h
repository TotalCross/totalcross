...
#ifdef UNICODE
#define IS_DEBUG_CONSOLE(path)                                                 \
  (JCharPIndexOfJCharP(path, TEXT("DebugConsole"), 0, -1, 12) >= 0)
#else
...
static Err
fileCreate(NATIVE_FILE* fref, TCHARP path, int32 mode, int32* slot)
{
  HANDLE hFile;
  DWORD dwDesiredAccess;
  DWORD dwShareMode =
    FILE_SHARE_READ; // file is read-only for other applications while in use.
  LPSECURITY_ATTRIBUTES lpSecurityAttributes =
    null; // Not used in WinCE, may be null in Win32 since we won't be using
          // it.
  DWORD dwCreationDisposition;
  DWORD dwFlagsAndAttributes =
    FILE_ATTRIBUTE_NORMAL; // file attributes are ignored while opening an
                           // existing obj, it is set to FILE_ATTRIBUTE_NORMAL
                           // for obj creation.
  HANDLE hTemplateFile = null; // Not used in WinCE, may be null in Win32 since
...
static Err
fileCreateDir(TCHARP path, int32 slot)
{
  TCHARP c;
...
static Err
fileDelete(NATIVE_FILE* fref, TCHARP path, int32 slot, bool isOpen)
{
...
static bool
fileExists(TCHARP path, int32 slot)
#if defined WINCE
{
...
{
  bool result;
  SetErrorMode(SEM_FAILCRITICALERRORS); // flsobral@tc115_25: no longer displays
                                        // error message to user when the
...
static Err
fileGetFreeSpace(TCHAR* path, int32* freeSpace, int32 slot)
{
  ULARGE_INTEGER freeSize = { 0 };

  if (GetDiskFreeSpaceEx(path, null, null, &freeSize)) {
    if (freeSize.HighPart > 0 ||
        freeSize.LowPart >
          2147483647) { // flsobral@tc122_25: the low part may be 64 bits in
                        // some systems for no particular reason.
      *freeSpace =
        2147483647; // If there is more than 2GB available, returns 2GB.
    } else {
...
static Err
fileGetSize(NATIVE_FILE fref, TCHARP szPath, int32* size)
{
#ifndef WP8
  return (((*size = GetFileSize(fref.handle, null)) != 0xFFFFFFFF)
            ? NO_ERROR
            : GetLastError());
#else
  ...
  *size = 0xFFFFFFFF;
  if (GetFileInformationByHandleEx(
        fref.handle, FileStandardInfo, &finfo, sizeof(finfo)) == 0) {
    return GetLastError();
  }
...
static bool
fileIsDir(TCHARP path, int32 slot)
{
#ifndef WP8
  DWORD fileAttributes;
  ...
  return ((fileAttributes != INVALID_ATTR_VALUE) &&
          (fileAttributes & FILE_ATTRIBUTE_DIRECTORY));
}

static Err
fileIsEmpty(NATIVE_FILE* fref, TCHARP path, int32 slot, int32* isEmpty)
{
  Err err = NO_ERROR;
#ifndef WP8
  ...
  if ((fileAttributes != INVALID_ATTR_VALUE) &&
      (fileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
    ...
    *isEmpty = true;
    if ((hFind = FindFirstFile(searchPath, &findData)) !=
        INVALID_HANDLE_VALUE) {
      do {
#if defined(WIN32) && !defined(WINCE)
        if (findData.cFileName[0] != '.' ||
            (tcscmp(findData.cFileName, TEXT(".")) &&
             tcscmp(findData.cFileName,
                    TEXT("..")))) // first check is just for speedup
#endif
        {
          *isEmpty = false;
          break;
...
static Err
fileReadBytes(NATIVE_FILE fref,
              CharP bytes,
              int32 offset,
              int32 length,
              int32* bytesRead)
{
  return (ReadFile(fref.handle, (bytes + offset), length, bytesRead, null)
            ? NO_ERROR
            : GetLastError());
}
...
static Err
fileSetPos(NATIVE_FILE fref, int32 position)
{
  Err err;
  // Must use SetFilePointerEx when running on the WP8 emulator, but not on
  // device
#if defined WP8
  ...
  return ((SetFilePointerEx(fref.handle, off, &cur, FILE_BEGIN) != 0)
            ? NO_ERROR
            : (((err = GetLastError()) == NO_ERROR) ? NO_ERROR : err));
#else
  return ((SetFilePointer(fref.handle, position, null, FILE_BEGIN) !=
           INVALID_FILEPTR_VALUE)
            ? NO_ERROR
            : (((err = GetLastError()) == NO_ERROR) ? NO_ERROR : err));
#endif
}
...
static Err
fileWriteBytes(NATIVE_FILE fref,
               CharP bytes,
               int32 offset,
               int32 length,
               int32* bytesWritten)
{
  return (WriteFile(fref.handle, (bytes + offset), length, bytesWritten, null)
            ? NO_ERROR
            : GetLastError());
}
...
static Err
fileSetAttributes(NATIVE_FILE fref, TCHARP path, int32 tcAttributes)
{
...
static Err
fileGetAttributes(NATIVE_FILE fref, TCHARP path, int32* attributes)
{
  DWORD fileAttributes = 0;
#ifndef WP8
...
static Err
fileSetTime(NATIVE_FILE fref, TCHARP path, char whichTime, TCObject time)
{
   ...
   */
#ifndef WP8
  return (SetFileTime(fref.handle, creationTime, lastAccessTime, lastWriteTime)
            ? NO_ERROR
            : GetLastError());
#else
  {
    ...
    if (!GetFileInformationByHandleEx(
          fref.handle, FileBasicInfo, finfo, sizeof(buff))) {
      return GetLastError();
    }
...
static Err
fileGetTime(Context currentContext,
            NATIVE_FILE fref,
            TCHARP path,
            char whichTime,
            TCObject* time)
 ...
 *************************************/
static Err
fileSetSize(NATIVE_FILE* fref, int32 newSize)
{
  DWORD posHigh = 0;
  DWORD fileSize;
...
#else
  if (SetFilePointer(fref->handle, newSize, &posHigh, FILE_BEGIN) ==
      INVALID_FILEPTR_VALUE) {
    return GetLastError();
  }
#endif
  if (SetEndOfFile(fref->handle) == 0) {
    ...
    if (error == ERROR_INVALID_PARAMETER && newSize == 0) {
      CloseHandle(fref->handle);
      fref->handle = CreateFile(fref->path,
 ...
 *************************************/
static Err
fileGetCardSerialNumber(int32 slot, CharP serialNumber)
{
#if defined(WINCE) && _WIN32_WCE >= 300 && defined(WIN32_PLATFORM_PSPC)
  typedef HANDLE(__stdcall *
                 procFindFirstFlashCard)(LPWIN32_FIND_DATA lpFindFlashData);
  typedef BOOL(__stdcall * procFindNextFlashCard)(
    HANDLE hFlashCard, LPWIN32_FIND_DATA lpFindFlashData);
  ...
  FindFirstFlashCardProc = (procFindFirstFlashCard)GetProcAddress(
    note_prjDll, TEXT("FindFirstFlashCard"));
  FindNextFlashCardProc = (procFindNextFlashCard)GetProcAddress(
    note_prjDll, TEXT("FindNextFlashCard"));

  if ((hFlashCard = FindFirstFlashCardProc(&findFlashData)) ==
      INVALID_HANDLE_VALUE) {
    goto error;
  }
  ...
  xstrncpy(serialNumber,
           (((CharP)storageIDP) + storageIDP->dwSerialNumOffset),
           dwNumReturned - storageIDP->dwSerialNumOffset);
 ...
 *************************************/
static Err
fileFlush(NATIVE_FILE fref)
{
  return (FlushFileBuffers(fref.handle) == 0) ? GetLastError() : NO_ERROR;
}

static Err
fileListRoots(TCHARPs** list, int32* count, Heap h)
{
  ...
  if ((logicalDrives = heapAlloc(h, 8 * sizeof(TCHAR))) ==
      null) { /* "device/".length = 7 */
    (*count) = -1;
    return NO_ERROR;
  }
  ...
  *list = TCHARPsAdd(*list, logicalDrives, h); // add entry to list
  ...
  if ((hFind = FindFirstFile(searchPath, &findData)) == INVALID_HANDLE_VALUE) {
    ...
    return errCode != ERROR_NO_MORE_FILES && errCode != ERROR_FILE_NOT_FOUND
             ? errCode
             : NO_ERROR; // guich@tc113_17: "file not found" is thrown in
                         // empty folders
  }
#if WIN32_WCE <= 400
  errCodeBug = GetLastError(); // some old implementations return a strange
                               // error code in SUCCESS situations. So we store
                               // it here to compare with it again below.
#endif
  do {
    if ((findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
        (findData.dwFileAttributes & FILE_ATTRIBUTE_TEMPORARY) &&
        lstrcmpi(findData.cFileName, TEXT("NETWORK"))) {
      fileNameSize = tcslen(findData.cFileName) +
                     2; // One for null and one extra in case it is a directory.
      fileName = (TCHARP)heapAlloc(
        h,
        sizeof(TCHAR) *
          (fileNameSize + pathlen)); // note: if an heap error occurs, the
                                     // FindClose will never be called; TODO
                                     // change the implementation to allow that.
  ...
  if ((logicalDrives = heapAlloc(h, (logicalDrivesLen++) * sizeof(TCHAR))) ==
      null) {
    (*count) = -1;
    return NO_ERROR;
  }
  ...
  if ((logicalDrivesLen =
         GetLogicalDriveStrings(logicalDrivesLen, logicalDrives)) == 0) {
    return GetLastError();
  }
...
static Err
fileChmod(NATIVE_FILE* fref, TCHARP path, int32 slot, int32* mod)
{
  *mod = -1;
  return NO_ERROR;
}
