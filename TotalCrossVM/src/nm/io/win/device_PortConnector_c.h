...
static Err portConnectorCreate(PortHandle* portConnectorRef, VoidP receiveBuffer, int32 number,
							   int32 baudRate, int32 bits, int32 parity, int32 stopBits, int32 writeTimeOutValue) {
	...
	switch (number) { // guich@320_42
		case SERIAL_IRCOMM:
			isIR = 1;
			if ((err = queryRegistry(HKEY_LOCAL_MACHINE, TEXT("Drivers\\BuiltIn\\IrCOMM"), TEXT("Index"),
									 (CharP) &i32, 4)) == NO_ERROR) {
				number = i32;
			} else {
		...
		case SERIAL_SIR:
			isIR = 1;
			if ((err = queryRegistry(HKEY_LOCAL_MACHINE, TEXT("ExtModems\\GenericInfraredModem"), TEXT("Port"),
									 (CharP) buf, 32)) != NO_ERROR) {
				return err;
			}
		...
		case SERIAL_USB: // fabio: as far as we know this type of connection is no longer used on pdas, so I'll leave the implementation as it is.
			if ((err = queryRegistry(HKEY_LOCAL_MACHINE, TEXT("Drivers\\BuiltIn\\SerialUSB"), TEXT("Index"),
									 (CharP) &i32, 4)) == NO_ERROR) {
				number = i32;
			} else {
				return err;
		...
		case SERIAL_BLUETOOTH:
			for (i = 10; i >= 0;
				 i--) { // guich@tc112_4: loop through the serial ports to find the correct bluetooth one - wierd: there are 2 bluetooth ports, but only one works (usually com6)
				xprintf(buf, TEXT("Drivers\\BuiltIn\\Serial%d"), i);
				if ((err = queryRegistry(HKEY_LOCAL_MACHINE, buf, TEXT("FriendlyName"), (CharP) buf2,
										 sizeof(buf2))) == NO_ERROR) {
					TCHARP2CharPBuf(buf2, buf8);
					CharPToLower(buf8);
					if (xstrstr(buf8, "bluetooth")
						&& (err = queryRegistry(HKEY_LOCAL_MACHINE, buf, TEXT("Index"), (CharP) &i32, 4)) == NO_ERROR) {
						number = i32;
						buf[0] = 0;
						break;
					}
			...
			break;
		case SERIAL_DEFAULT:
			if ((err = queryRegistry(HKEY_LOCAL_MACHINE, TEXT("Drivers\\BuiltIn\\Serial"), TEXT("Index"),
									 (CharP) &i32, 4)) == NO_ERROR) {
				number = i32;
			} else {
				return err;    //number = 1;
		...
		default: // guich@570_106: the user can now pass something like TCP1 in the port number
			xprintf(buf, TEXT("%c%c%c%c:"), (TCHAR)((number >> 24) & 0xFF), (TCHAR)((number >> 16) & 0xFF),
					(TCHAR)((number >> 8) & 0xFF), (TCHAR)(number & 0xFF));
	}
	...
	if ((h = CreateFile(buf, GENERIC_READ | GENERIC_WRITE, 0, null, OPEN_EXISTING,
						FILE_ATTRIBUTE_SYSTEM | FILE_FLAG_WRITE_THROUGH, null)) == INVALID_HANDLE_VALUE) {
		return GetLastError();
	}
	...
	dcb.Parity = (parity == SERIAL_PARITY_NONE) ? NOPARITY : (parity == SERIAL_PARITY_EVEN) ? EVENPARITY
				 : ODDPARITY; // guich@450_29: added oddparity
	if (stopBits == 1) {
...
static inline Err portConnectorClose(PortHandle portConnectorRef, VoidP receiveBuffer,
									 int32 portNumber) {
...
typedef BOOL (__stdcall* ReadWriteFileFunc)(HANDLE hFile, LPVOID lpBuffer,
		DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped);

static Err portConnectorReadWriteBytes(PortHandle portConnectorRef, int32 portNumber,
									   bool stopWriteCheckOnTimeout, int32 timeout, uint8* bytes, int32 start, int32 count,
									   int32* retCount, bool isRead) {
	...
	if (isRead) {
		...
		commtimeouts.ReadTotalTimeoutConstant =
			timeout; //flsobral@tc122: don't need to set the multiplier, we can just use the constant field.
