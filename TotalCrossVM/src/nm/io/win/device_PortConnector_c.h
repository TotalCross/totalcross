...
static Err
queryRegistry(HKEY key, TCHARP subkey, TCHARP name, CharP buf, int32 size)
{
  HKEY handle;
  int32 type;
  Err err = NO_ERROR;
  ...
  type = REG_DWORD;
  if ((err = RegQueryValueEx(handle, name, null, &type, buf, &size)) !=
      NO_ERROR) {
    RegCloseKey(handle);
    return err;
  }
...
static Err
portConnectorCreate(PortHandle* portConnectorRef,
                    VoidP receiveBuffer,
                    int32 number,
                    int32 baudRate,
                    int32 bits,
                    ...
                    int32 writeTimeOutValue)
{
  ...
  switch (number) { // guich@320_42
    case SERIAL_IRCOMM:
      isIR = 1;
      if ((err = queryRegistry(HKEY_LOCAL_MACHINE,
    ...
    case SERIAL_SIR:
      isIR = 1;
      if ((err = queryRegistry(HKEY_LOCAL_MACHINE,
                               TEXT("ExtModems\\GenericInfraredModem"),
                               TEXT("Port"),
                               (CharP)buf,
                               32)) != NO_ERROR) {
        return err;
      }
      // fabio: Couldn't test if the above also works for Qtek, so I'm leaving
      // the alternative to test that later. if ((err =
      // queryRegistry(HKEY_LOCAL_MACHINE, TEXT("Drivers\\BuiltIn\\SerialIR"),
      // TEXT("Index"), (CharP) &i32, 4)) == NO_ERROR) // guich@570_105: in qtek
      // its SerialIR
      //   number = i32;
      break;
    case SERIAL_USB: // fabio: as far as we know this type of connection is no
                     // longer used on pdas, so I'll leave the implementation as
                     // it is.
      if ((err = queryRegistry(HKEY_LOCAL_MACHINE,
                               TEXT("Drivers\\BuiltIn\\SerialUSB"),
                               TEXT("Index"),
                               (CharP)&i32,
                               4)) == NO_ERROR) {
        number = i32;
    ...
    case SERIAL_BLUETOOTH:
      for (i = 10; i >= 0;
           i--) { // guich@tc112_4: loop through the serial ports to find the
                  // correct bluetooth one - wierd: there are 2 bluetooth ports,
                  // but only one works (usually com6)
        ...
        if ((err = queryRegistry(HKEY_LOCAL_MACHINE,
          ...
          if (xstrstr(buf8, "bluetooth") &&
              (err = queryRegistry(
                 HKEY_LOCAL_MACHINE, buf, TEXT("Index"), (CharP)&i32, 4)) ==
                NO_ERROR) {
            number = i32;
            buf[0] = 0;
            break;
    ...
    case SERIAL_DEFAULT:
      ...
      break;
    default: // guich@570_106: the user can now pass something like TCP1 in the
             // port number
      xprintf(buf,
              TEXT("%c%c%c%c:"),
              (TCHAR)((number >> 24) & 0xFF),
              (TCHAR)((number >> 16) & 0xFF),
              (TCHAR)((number >> 8) & 0xFF),
              (TCHAR)(number & 0xFF));
  }
  ...
  dcb.Parity = (parity == SERIAL_PARITY_NONE)
                 ? NOPARITY
...
static inline Err
portConnectorClose(PortHandle portConnectorRef,
                   VoidP receiveBuffer,
                   int32 portNumber)
{
...
static Err
portConnectorSetFlowControl(PortHandle portConnectorRef, bool flowOn)
{
  DCB dcb;
...
typedef BOOL(__stdcall* ReadWriteFileFunc)(HANDLE hFile,
                                           LPVOID lpBuffer,
...
static Err
portConnectorReadWriteBytes(PortHandle portConnectorRef,
                            int32 portNumber,
                            bool stopWriteCheckOnTimeout,
                            int32 timeout,
                            uint8* bytes,
                            int32 start,
                            int32 count,
                            ...
                            bool isRead)
{
  ...
  if (isRead) {
    ...
    commtimeouts.ReadTotalTimeoutConstant =
      timeout; // flsobral@tc122: don't need to set the multiplier, we can
               // just use the constant field.
...
static Err
portConnectorReadCheck(PortHandle portConnectorRef, int32* inQueue)
{
  COMSTAT Stat;
  int32 dwErrors;
  /*
