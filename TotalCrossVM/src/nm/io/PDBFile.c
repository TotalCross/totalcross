...
static Err releaseRecord(TCObject obj) {
	...
	if (dbPObj == null) {
		...
		if (hvRecordPos != -1) { // release the current record
			...
			if (!dbP) {
				err = -1;
			} else if (DmReleaseRecord(dbP, (UInt16) hvRecordPos, (bool) hvRecordChanged) == errNone) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_create_sssi(NMParams
							   p) { // totalcross/io/PDBFile native private void create(String name, int mode);
	...
	dbId = DmFindDatabase(p->currentContext, szName,
						  dbId);  // ATENCAO: TEM QUE VER SE O DMFINDDATABASE FAZ UM LOCK DO OBJETO NOVAMENTE! ISSO CORROMPERIA O GC

	if (dbId) { // check if the creator id and type are those expected
		err = DmDatabaseInfo(dbId, null, &attr, null, null, null, null, null, null, null, &typeId2,
							 &creatorId2);
		if (err != errNone) {
	...
	if (p->currentContext->thrownException) { // So far, so good.
		...
		switch (mode) {
			...
			case CREATE:
				...
				if ((err = DmSetDatabaseInfo(dbId, null, &attr, &version, null, null, null, &modificationNumber,
											 null, null, null, null)) != errNone) {
					throwExceptionWithCode(p->currentContext, IOException, err);
					goto error;
				}
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_rename_s(NMParams
							p) { // totalcross/io/PDBFile native public void rename(String newName) throws totalcross.io.IOException;
	TCObject pdbFile = p->obj[0];
	...
	if (dbPObj == null) {
		...
		if (newName == null) {
			...
			if (!splitName(szNewPath, &szNewName, &szCreator,
				...
				if ((err = DmSetDatabaseInfo(dbId, szNewName, null, null, null, null, null, null, null, null, &type,
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_addRecord_ii(NMParams
								p) { // totalcross/io/PDBFile native public void addRecord(int size, int pos) throws totalcross.io.IOException;
	TCObject pdbFile = p->obj[0];
	int32 size = p->i32[0];
	int32 pos =  p->i32[1];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_addRecord_i(NMParams
							   p) { // totalcross/io/PDBFile native public int addRecord(int size) throws totalcross.io.IOException;
	int32 i32[2];
	int32* i32P;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_resizeRecord_i(NMParams
								  p) { // totalcross/io/PDBFile native public void resizeRecord(int size) throws totalcross.io.IOException;
	TCObject pdbFile = p->obj[0];
	int32 size = p->i32[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_nativeClose(NMParams
							   p) { // totalcross/io/PDBFile native private void nativeClose();
	TCObject pdbFile = p->obj[0];
	TCObject dbPObj = PDBFile_openRef(pdbFile);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_delete(NMParams
						  p) { // totalcross/io/PDBFile native public void delete() throws totalcross.io.IOException;
	TCObject pdbFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_listPDBs_ii(NMParams
							   p) { // totalcross/io/PDBFile native public static String []listPDBs(int creatorId, int type);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_deleteRecord(NMParams
								p) { // totalcross/io/PDBFile native public void deleteRecord() throws totalcross.io.IOException;
	TCObject pdbFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_getRecordCount(NMParams
								  p) { // totalcross/io/PDBFile native public int getRecordCount();
	TCObject pdbFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_setRecordPos_i(NMParams
								  p) { // totalcross/io/PDBFile native public boolean setRecordPos(int pos) throws totalcross.io.IOException;
	TCObject pdbFile = p->obj[0];
	int32 pos = p->i32[0];
...
TC_API void tiPDBF_readWriteBytes_Biib(NMParams p) {
	...
	if (isRead) {
		...
		DmWrite(recPtr, (uint32) hvRecordOffset, bufP + start,
				(uint32) count); // Palm OS requires use of DmWrite is we don't use the semaphore trick
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_inspectRecord_Bii(NMParams
									 p) { // totalcross/io/PDBFile native public int inspectRecord(byte []buf, int recPosition) throws totalcross.io.IOException;
	TCObject pdbFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_getRecordAttributes_i(NMParams
		p) { // totalcross/io/PDBFile native public byte getRecordAttributes(int recordPos);
	TCObject pdbFile = p->obj[0];
	int32 recordPos = p->i32[0];
	...
	if (dbPObj == null) {
		...
		if (recordPos < 0 || recordPos >= DmNumRecords(dbP)) {
			throwIllegalArgumentIOException(p->currentContext, "recordPos", int2str(recordPos, intBuf));
		} else if ((err = DmRecordInfo(dbP, (uint16)recordPos, &attr, 0, null)) != errNone) {
			throwExceptionWithCode(p->currentContext, IOException, err);
		} else {
			p->retI = (int8) attr;
		}
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_setRecordAttributes_ib(NMParams
		p) { // totalcross/io/PDBFile native public void setRecordAttributes(int recordPos, byte attr);
	TCObject pdbFile = p->obj[0];
	int32 recordPos = p->i32[0];
	int32 attr = p->i32[1];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_getAttributes(NMParams
								 p) { // totalcross/io/PDBFile native public int getAttributes();
	TCObject pdbFile = p->obj[0];
	...
	if (dbPObj == null) {
		...
		if ((err = DmDatabaseInfo(dbId, null, &attr, null, null, null, null, null, null, null, null,
								  null)) != errNone) {
			throwExceptionWithCode(p->currentContext, IOException, err);
		} else {
			p->retI = (int32) attr;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_setAttributes_i(NMParams
								   p) { // totalcross/io/PDBFile native public void setAttributes(int i);
	TCObject pdbFile = p->obj[0];
	...
	if (dbPObj == null) {
		...
		if ((err = DmSetDatabaseInfo(dbId, null, &attr, null, null, null, null, null, null, null, null,
									 null)) != errNone) {
			throwExceptionWithCode(p->currentContext, IOException, err);
		}
		SysNotifyBroadcast(null);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tiPDBF_searchBytes_Bii(NMParams
								   p) { // totalcross/io/PDBFile native public int searchBytes(byte []toSearch, int length, int offsetInRec);
	TCObject pdbFile = p->obj[0];
