...
releaseRecord(TCObject obj)
{
  ...
  if (dbPObj == null) {
    ...
    if (hvRecordPos != -1) { // release the current record
      ...
      if (!dbP) {
        err = -1;
      } else if (DmReleaseRecord(dbP,
                                 (UInt16)hvRecordPos,
                                 (bool)hvRecordChanged) == errNone) {
...
static bool
splitName(TCHARP fullName, TCHARP* name, TCHARP* creator, TCHARP* type)
{
  TCHARP strP;
  int32 nameLen;
  int32 pathLen;
  ...
  *name = fullName;
  for (strP = *creator; strP != fullName && *strP != '/' && *strP != '\\';
       strP--)
    ;
  if (strP == fullName) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_create_sssi(NMParams p)
{ // totalcross/io/PDBFile native private void
  ...
  JCharP2TCHARPBuf(
    String_charsStart(creator), String_charsLen(creator), szCreator);
  JCharP2TCHARPBuf(String_charsStart(type), String_charsLen(type), szType);
  creatorId = chars2int(szCreator);
  typeId = chars2int(szType);

  dbId =
    DmFindDatabase(p->currentContext,
                   szName,
                   dbId); // ATENCAO: TEM QUE VER SE O DMFINDDATABASE FAZ UM
                          // LOCK DO OBJETO NOVAMENTE! ISSO CORROMPERIA O GC

  if (dbId) { // check if the creator id and type are those expected
    err = DmDatabaseInfo(dbId,
                         null,
                         &attr,
                         null,
                         null,
  ...
  if (p->currentContext->thrownException) { // So far, so good.
    ...
    switch (mode) {
      case CREATE_EMPTY:
        ...
        dbId = 0;
        attr = 0; // flsobral@tc120_29: keep original attribute value if mode is
                  // CREATE, set to 0 on CREATE_EMTPY.
      case CREATE:
        if (!dbId && (err = DmCreateDatabase(
                              szName, creatorId, typeId, false) != errNone)) {
          throwExceptionWithCode(p->currentContext, IOException, err);
          goto error;
        }
        dbId = dbP;
        ...
        if (!dbId) {
          throwExceptionWithCode(
            p->currentContext, IOException, DmGetLastErr());
          goto error;
        }
        version = 1;
        modificationNumber = 0;
        if ((err = DmSetDatabaseInfo(dbId,
                                     null,
                                     &attr,
                                     &version,
                                     null,
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_rename_s(NMParams p)
{ // totalcross/io/PDBFile native public void rename(String
  // newName) throws totalcross.io.IOException;
  TCObject pdbFile = p->obj[0];
  ...
  if (dbPObj == null) {
    ...
    if (newName == null) {
      ...
      char tbuf[256];
      JCharP2TCHARPBuf(
        String_charsStart(newName), String_charsLen(newName), szNewPath);
      if (!splitName(szNewPath,
                     &szNewName,
                     &szCreator,
                     &szType)) { // split full name into NAME, CREATOR and TYPE
        throwIllegalArgumentIOException(
          p->currentContext, "newName", TCHARP2CharPBuf(szNewPath, tbuf));
      } else {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_addRecord_ii(NMParams p)
{ // totalcross/io/PDBFile native public void addRecord(int
  // size, int pos) throws totalcross.io.IOException;
  TCObject pdbFile = p->obj[0];
  int32 size = p->i32[0];
  int32 pos = p->i32[1];
  ...
  if (dbPObj == null) {
    ...
    if (size < 0 || size > 65535) {
      throwIllegalArgumentIOException(
        p->currentContext, "size", int2str(size, intBuf));
    } else if (pos != 65535 && (pos < 0 || pos > DmNumRecords(dbP))) {
      throwIllegalArgumentIOException(
        p->currentContext, "pos", int2str(pos, intBuf));
    } else if ((err = releaseRecord(pdbFile)) != errNone) {
      throwExceptionWithCode(p->currentContext, IOException, err);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_addRecord_i(NMParams p)
{ // totalcross/io/PDBFile native public int addRecord(int size)
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_resizeRecord_i(NMParams p)
{ // totalcross/io/PDBFile native public void resizeRecord(int
  // size) throws totalcross.io.IOException;
  TCObject pdbFile = p->obj[0];
  int32 size = p->i32[0];
  ...
  if (dbPObj == null) {
    ...
    if (size < 0 || size > 65535) {
      throwIllegalArgumentIOException(
        p->currentContext, "size", int2str(size, intBuf));
    } else if ((hvRecordPos = PDBFile_hvRecordPos(pdbFile)) == -1) {
      throwException(p->currentContext,
                     IOException,
                     "No record selected for this operation.");
    } else if (!(handle =
                   DmResizeRecord(dbP, (UInt16)hvRecordPos, (UInt32)size))) {
      throwExceptionWithCode(p->currentContext, IOException, DmGetLastErr());
    } else {
      PDBFile_hvRecordLength(pdbFile) = size;
      PDBFile_hvRecordHandle(pdbFile) = handle;
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_nativeClose(NMParams p)
{ // totalcross/io/PDBFile native private void nativeClose();
  TCObject pdbFile = p->obj[0];
  TCObject dbPObj = PDBFile_openRef(pdbFile);

  if (dbPObj == null) {
    throwException(p->currentContext, IOException, "The pdb file is closed.");
  } else if (!destroy(pdbFile)) {
    throwException(
      p->currentContext, IOException, "Could not close the database");
  }
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_delete(NMParams p)
{ // totalcross/io/PDBFile native public void delete()
  // throws totalcross.io.IOException;
  TCObject pdbFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_listPDBs_ii(NMParams p)
{ // totalcross/io/PDBFile native public static
  // String []listPDBs(int creatorId, int type);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_deleteRecord(NMParams p)
{ // totalcross/io/PDBFile native public void deleteRecord()
  // throws totalcross.io.IOException;
  TCObject pdbFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_getRecordCount(NMParams p)
{ // totalcross/io/PDBFile native public int getRecordCount();
  TCObject pdbFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_setRecordPos_i(NMParams p)
{ // totalcross/io/PDBFile native public boolean
  // setRecordPos(int pos) throws totalcross.io.IOException;
  TCObject pdbFile = p->obj[0];
  int32 pos = p->i32[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_readWriteBytes_Biib(NMParams p)
{
  ...
  if (isRead) {
    ...
    DmWrite(recPtr,
            (uint32)hvRecordOffset,
            bufP + start,
            (uint32)count); // Palm OS requires use of DmWrite is we don't use
                            // the semaphore trick
  ...
  p->retI =
    count == 0 && isRead ? -1 : count; // guich@tc110_65: return -1 if eof
}
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_inspectRecord_Bii(NMParams p)
{ // totalcross/io/PDBFile native public int inspectRecord(byte
  // []buf, int recPosition) throws totalcross.io.IOException;
  TCObject pdbFile = p->obj[0];
  ...
  if (dbPObj == null) {
    ...
    } else if (offsetInRec < 0 || offsetInRec > 65535) {
      throwIllegalArgumentIOException(
        p->currentContext, "offsetInRec", int2str(offsetInRec, intBuf));
    } else if (recordPos < 0 || recordPos >= DmNumRecords(dbP)) {
      throwIllegalArgumentIOException(
        p->currentContext, "recordPos", int2str(recordPos, intBuf));
    } else {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_getRecordAttributes_i(NMParams p)
{ // totalcross/io/PDBFile native public byte
  ...
  if (dbPObj == null) {
    ...
    if (recordPos < 0 || recordPos >= DmNumRecords(dbP)) {
      throwIllegalArgumentIOException(
        p->currentContext, "recordPos", int2str(recordPos, intBuf));
    } else if ((err = DmRecordInfo(dbP, (uint16)recordPos, &attr, 0, null)) !=
               errNone) {
      throwExceptionWithCode(p->currentContext, IOException, err);
    } else {
      p->retI = (int8)attr;
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_setRecordAttributes_ib(NMParams p)
{ // totalcross/io/PDBFile native public void
  // setRecordAttributes(int recordPos, byte attr);
  TCObject pdbFile = p->obj[0];
  int32 recordPos = p->i32[0];
  int32 attr = p->i32[1];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_getAttributes(NMParams p)
{ // totalcross/io/PDBFile native public int getAttributes();
  TCObject pdbFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_setAttributes_i(NMParams p)
{ // totalcross/io/PDBFile native public void
  // setAttributes(int i);
  TCObject pdbFile = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tiPDBF_searchBytes_Bii(NMParams p)
{ // totalcross/io/PDBFile native public int searchBytes(byte
  // []toSearch, int length, int offsetInRec);
  TCObject pdbFile = p->obj[0];
  ...
  if (dbPObj == null) {
    ...
    if (toSearch == null) {
    ...
    } else if (offsetInRec < 0) {
      throwIllegalArgumentIOException(
        p->currentContext, "offsetInRec", int2str(offsetInRec, intBuf));
    } else if (length <= 0 || length > (toSearchLen = ARRAYOBJ_LEN(toSearch))) {
      ...
      for (i = max32(0, PDBFile_hvRecordPos(pdbFile)); i < n && found == -1;
           i++) {
        ...
        if (handle != null && MemHandleSize(handle) >= minRecSize) {
          ...
          if (*recPtr == *toSearchBuf && *(recPtr + lenM1) == *toSearchLast) {
            if (length <= 2) { // block has one or two bytes, so we have already
                               // compared the whole block
              found = i;
