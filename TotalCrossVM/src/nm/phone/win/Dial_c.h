...
static HCALL
hCall;            // Handle to the open line device on which the call is to be originated(lineMakeCall)
...
static void CALLBACK lineCallbackFunc(DWORD hDevice, DWORD dwMsg, DWORD dwCallbackInstance,
			...
			switch (dwParam1) {
				...
				case LINEDEVSTATE_REINIT: {
					...
					switch (dwParam2) {
						...
						case 0:
							throwDialException("Tapi line configuration has been changed.", 0);
							lineShutdown(hLineApp);
							hLineApp = null;
							break;
						case LINE_CREATE:
						case LINE_LINEDEVSTATE:
							lineCallbackFunc(hDevice, dwParam2, dwCallbackInstance, dwParam3, 0, 0);
							break;
					}
				}
			}
...
static bool InitializeTAPI() {
	...
	while ((dwReturn = lineInitialize(&hLineApp, hModuleTCVM, (LINECALLBACK)lineCallbackFunc,
									  TEXT("TotalCross"), &dwNumDevs)) == LINEERR_REINIT) {
		Sleep(50);
		// Bring up the message box if 5 seconds have passed.
		if (GetTickCount() > 5000 + dwTimeCount) {
			throwDialException("Cannot initialize tapi.dll. Please quit all other phone programs and try again.",
							   0);
			return false;
		}
	}
	...
	for (dwLineID = 0; dwLineID < dwNumDevs; ++dwLineID) {
		...
		dwReturn = lineNegotiateAPIVersion(hLineApp, dwLineID, TAPI_VERSION_1_0, TAPI_VERSION_2_0,
										   &VoiceLineInfo.dwAPIVersion, NULL);
		if (dwReturn == 0) {
			tzero(LineDevCaps);
			LineDevCaps.dwTotalSize = sizeof(LINEDEVCAPS_v3);
			dwReturn = lineGetDevCaps(hLineApp, dwLineID, VoiceLineInfo.dwAPIVersion, 0,
...
static VOID MakePhoneCall(LPCTSTR lpszPhoneNum) {
	...
	if (dwReturn = lineOpen(hLineApp, dwCurrentLineID, &VoiceLineInfo.hLine, VoiceLineInfo.dwAPIVersion,
							0, 0, LINECALLPRIVILEGE_NONE, 0, NULL)) {
		goto exit;
	}
	...
	while (true) {
		...
		if (dwReturn = lineTranslateAddress(hLineApp, dwCurrentLineID, VoiceLineInfo.dwAPIVersion,
											lpszPhoneNum, 0, 0, lpTransOutput)) {
			goto exit;
		}
	...
	lstrcpy(szDialablePhoneNum, (LPTSTR)((LPBYTE) lpTransOutput +
	...
	MakeCallRequestID = lineMakeCall(VoiceLineInfo.hLine, &hCall, szDialablePhoneNum, 0,
