...
static HCALL hCall; // Handle to the open line device on which the call is to be
                    // originated(lineMakeCall)
...
typedef struct linedevcaps_tag_v3
{ // we're targetting windows mobile 5 and
  // beyond
  DWORD dwTotalSize;
  DWORD dwNeededSize;
  DWORD dwUsedSize;
...
// This is a callback function invoked to determine status and events on the
// line device, addresses, or calls.
static void CALLBACK
lineCallbackFunc(DWORD hDevice,
                 DWORD dwMsg,
    ...
    case LINE_CALLSTATE: // Sent after change of call state
      ...
      switch (dwParam1) {
        ...
        case LINECALLSTATE_PROCEEDING:
          status = "Dialing completed, call proceeding";
          break;
        ...
        case LINECALLSTATE_SPECIALINFO:
          status = "Special Information, couldn't dial number";
          CurrentLineClose();
          break;
        case LINECALLSTATE_DISCONNECTED: {
          switch (dwParam2) {
            case LINEDISCONNECTMODE_NORMAL:
              status = "Disconnected by remote party";
              break;
            case LINEDISCONNECTMODE_REJECT:
              status = "Remote Party rejected call";
              break;
            case LINEDISCONNECTMODE_PICKUP:
              status = "Disconnected: Local phone picked up";
              break;
            case LINEDISCONNECTMODE_FORWARDED:
              status = "Disconnected: Forwarded";
              break;
            case LINEDISCONNECTMODE_BUSY:
              status = "Disconnected: Busy";
              break;
            case LINEDISCONNECTMODE_NOANSWER:
              status = "Disconnected: No Answer";
              break;
            case LINEDISCONNECTMODE_BADADDRESS:
              status = "Disconnected: Bad address";
              break;
            case LINEDISCONNECTMODE_UNREACHABLE:
              status = "Disconnected: Unreachable";
              break;
            case LINEDISCONNECTMODE_CONGESTION:
              status = "Disconnected: Congestion";
              break;
            case LINEDISCONNECTMODE_INCOMPATIBLE:
              status = "Disconnected: Incompatible";
              break;
            case LINEDISCONNECTMODE_UNAVAIL:
              status = "Disconnected: Unavailable";
              break;
            case LINEDISCONNECTMODE_NODIALTONE:
              status = "Disconnected: No dial tone";
              break;
            default:
              status = "Disconnected";
              break;
    ...
    case LINE_LINEDEVSTATE:
      switch (dwParam1) {
        ...
        case LINEDEVSTATE_OUTOFSERVICE:
          status = "The line selected is out of service";
          CurrentLineClose();
          break;
        case LINEDEVSTATE_DISCONNECTED:
          status = "The line selected is disconnected";
          CurrentLineClose();
          break;
        case LINEDEVSTATE_MAINTENANCE:
          status = "The line selected is out for maintenance";
          CurrentLineClose();
          break;
        case LINEDEVSTATE_REMOVED:
          status = "The Line device has been removed; no action taken";
          break;
        case LINEDEVSTATE_REINIT: {
          // This usually means that a service provider has changed in such a
          // way that requires TAPI to REINIT. Note that there are both soft
          // REINITs and hard REINITs. Soft REINITs do not require a full
          // shutdown but an informational change that historically required a
          // REINIT to force the application to deal with.  TAPI API Version 1.3
          // applications require a full REINIT for both hard and soft REINITs.
          switch (dwParam2) {
            // This is the hard REINIT. TAPI is waiting for everyone to shut
            // down. Our response is to immediately shut down any  calls, shut
            // down our use of TAPI and notify the user.
            case 0:
              throwDialException("Tapi line configuration has been changed.",
                                 0);
            ...
            case LINE_LINEDEVSTATE:
              lineCallbackFunc(
                hDevice, dwParam2, dwCallbackInstance, dwParam3, 0, 0);
              break;
          }
        }
      }
      ...
      /*         if (dwParam1 >= dwNumDevs)
               {
                  ...
                  if (!(lpLineInfo =(LPLINEINFO) LocalAlloc(LPTR,
         sizeof(LINEINFO)
         * dwNumDevs))) break;
...
InitializeTAPI()
{
  ...
  while ((dwReturn = lineInitialize(&hLineApp,
                                    hModuleTCVM,
                                    (LINECALLBACK)lineCallbackFunc,
                                    TEXT("TotalCross"),
                                    &dwNumDevs)) == LINEERR_REINIT) {
    Sleep(50);
    // Bring up the message box if 5 seconds have passed.
    if (GetTickCount() > 5000 + dwTimeCount) {
      throwDialException("Cannot initialize tapi.dll. Please quit all other "
                         "phone programs and try again.",
                         0);
      return false;
    }
  }
  ...
  for (dwLineID = 0; dwLineID < dwNumDevs; ++dwLineID) {
    ...
    // TAPI registration handle, Line device to be queried, Least recent API
    // version, Most recent API version, Negotiated API version, Negotiated API
    // versionMust be NULL; the provider-specific extension is not supported on
    // Windows CE
    dwReturn = lineNegotiateAPIVersion(hLineApp,
                                       dwLineID,
                                       TAPI_VERSION_1_0,
                                       TAPI_VERSION_2_0,
                                       &VoiceLineInfo.dwAPIVersion,
                                       NULL);
    if (dwReturn == 0) {
      tzero(LineDevCaps);
      LineDevCaps.dwTotalSize = sizeof(LINEDEVCAPS_v3);
      dwReturn = lineGetDevCaps(hLineApp,
                                dwLineID,
                                VoiceLineInfo.dwAPIVersion,
                                0,
                                (LPLINEDEVCAPS)&LineDevCaps);
      if (dwReturn == 0 &&
          LineDevCaps.dwMediaModes & LINEMEDIAMODE_INTERACTIVEVOICE) {
...
typedef struct linecallparams_tag_v3
{                            // Defaults:
  DWORD dwTotalSize;         // ---------
...
static VOID
MakePhoneCall(LPCTSTR lpszPhoneNum)
{
#ifdef WINCE
  DWORD dwReturn, dwSizeOfTransOut = sizeof(LINETRANSLATEOUTPUT),
  ...
  // Usage handle for TAPI, Cannot use the LINEMAPPER value, Line handle, Must
  // set to zero for Windows CE, No data passed back, Can only make an outgoing
  // call, Media mode, Must set to NULL for Windows CE
  if (dwReturn = lineOpen(hLineApp,
                          dwCurrentLineID,
                          &VoiceLineInfo.hLine,
                          VoiceLineInfo.dwAPIVersion,
                          0,
  ...
  while (true) {
    ...
    // Usage handle for TAPI, Line device identifier, Address to be translated,
    // Must be 0 for Windows CE, No associated operations, Result of the address
    // translation
    if (dwReturn = lineTranslateAddress(hLineApp,
                                        dwCurrentLineID,
                                        VoiceLineInfo.dwAPIVersion,
                                        lpszPhoneNum,
                                        0,
  ...
  if (!(lpCallParams =
          (LPLINECALLPARAMS_v3)LocalAlloc(LPTR, dwSizeOfCallParams))) {
    goto exit;
  }
  ...
  MakeCallRequestID = lineMakeCall(VoiceLineInfo.hLine,
                                   &hCall,
                                   szDialablePhoneNum,
                                   0,
  ...
  if (MakeCallRequestID <= 0) {
    throwDialException("Failed in making the call. Error: %X",
                       MakeCallRequestID);
    CurrentLineClose();
  }
exit:
