...
static bool initSMS(Context currentContext, const DWORD mode) {
	...
	if (!smsDll) {
		...
		if (!SmsOpen || !SmsSendMessage || !SmsClose || !SmsReadMessage || !SmsGetMessageSize) {
			throwException(currentContext, IOException,
...
static void SmsSend(Context currentContext, TCHARP szMessage, TCHARP szDestination) {
	if (isWindowsMobile && *tcSettings.romVersionPtr >= 300 && initSMS(currentContext, SMS_MODE_SEND)) {
		...
		if (dwReturn != WAIT_ABANDONED && dwReturn != WAIT_TIMEOUT) {
			...
			// send the message
			err = SmsSendMessage(smsHandle, NULL, &destination, NULL, (PBYTE)szMessage, lstrlen(szMessage) * 2,
								 (PBYTE) &tpsd, sizeof(TEXT_PROVIDER_SPECIFIC_DATA), SMSDE_OPTIMAL, SMS_OPTION_DELIVERY_NONE,
								 &smsmidMessageID);
			if (err != NO_ERROR) {
				throwExceptionWithCode(currentContext, IOException, err);
			}
		} else {
			throwException(currentContext, IOException, "SMS event was not activated.");
		}
			...
			if (err != NO_ERROR) {
				...
				if (msg != null) {
					...
					if (pMessage == null) {
						...
						err = SmsReadMessage(smsHandle, NULL, &smsaDestination, NULL, (PBYTE)pMessage, dwSize, (PBYTE)&tpsd,
											 sizeof(TEXT_PROVIDER_SPECIFIC_DATA), &dwRead);
						if (err != NO_ERROR) {
							throwExceptionWithCode(currentContext, IOException, err);
						} else {
