...
typedef HRESULT(__stdcall* SmsOpenProc)(const LPCTSTR ptsMessageProtocol,
                                        const DWORD dwMessageModes,
                                        SMS_HANDLE* const psmshHandle,
                                        HANDLE* const phMessageAvailableEvent);

typedef HRESULT(__stdcall* SmsCloseProc)(const SMS_HANDLE smshHandle);
...
typedef HRESULT(__stdcall* SmsGetMessageSizeProc)(const SMS_HANDLE smshHandle,
                                                  DWORD* const pdwDataSize);

// Read an SMS message (the appropriate size of the buffer can be found via a
// call to SmsGetMessageSize)
...
typedef struct text_provider_specific_data_tag
{
  ...
  BYTE pbHeaderData[SMS_DATAGRAM_SIZE]; // For concatenated messages, only the
                                        // header from the first segment is
                                        // returned.
  BOOL fMessageContainsEMSHeaders;      // At least one segment of this message
                                        // contains EMS headers.
  // Only set if EMS handler installed.
  DWORD dwProtocolID; // PID of incoming message, or desired PID of outgoing
                      // message.
  // Applies only to GSM.  Set to SMS_MSGPROTOCOL_UNKNOWN if psReplaceOption
  // is not PSRO_NONE.
  DWORD dwExtParams; // Bitfield of valid additional structure parameters (all
                     // structure
  // values above are considered always valid).
  TEXT_PROVIDER_SPECIFIC_PRIORITY_TYPE
  tpsPriority;              // Applies only to CDMA IS637. Priority indicator.
...
static bool
initSMS(Context currentContext, const DWORD mode)
{
  smsDll = LoadLibrary(TEXT("sms.dll"));
  if (!smsDll) {
    throwException(
      currentContext, RuntimeException, "Could not load the SMS library");
  } else {
    SmsOpen = (SmsOpenProc)GetProcAddress(smsDll, TEXT("SmsOpen"));
    SmsSendMessage =
      (SmsSendMessageProc)GetProcAddress(smsDll, TEXT("SmsSendMessage"));
    SmsClose = (SmsCloseProc)GetProcAddress(smsDll, TEXT("SmsClose"));
    SmsGetMessageSize =
      (SmsGetMessageSizeProc)GetProcAddress(smsDll, TEXT("SmsGetMessageSize"));
    SmsReadMessage =
      (SmsReadMessageProc)GetProcAddress(smsDll, TEXT("SmsReadMessage"));

    if (!SmsOpen || !SmsSendMessage || !SmsClose || !SmsReadMessage ||
        !SmsGetMessageSize) {
      throwException(
        currentContext,
        IOException,
        "Could not load the required procedures from the SMS library");
    } else {
...
static void
SmsSend(Context currentContext, TCHARP szMessage, TCHARP szDestination)
{
  if (isWindowsMobile && *tcSettings.romVersionPtr >= 300 &&
      initSMS(currentContext, SMS_MODE_SEND)) {
    Err err;
    ...
    if (dwReturn != WAIT_ABANDONED && dwReturn != WAIT_TIMEOUT) {
      ...
      throwException(
        currentContext, IOException, "SMS event was not activated.");
    }
...
static void
SmsReceive(Context currentContext, TCObject* out)
{
  if (isWindowsMobile && *tcSettings.romVersionPtr >= 300 &&
      initSMS(currentContext, SMS_MODE_RECEIVE)) {
    Err err;
    ...
    if (dwReturn != WAIT_ABANDONED && dwReturn != WAIT_TIMEOUT) {
      ...
      if (err != NO_ERROR) {
        ...
        if (msg != null) {
          ...
          if (pMessage == null) {
            throwException(
              currentContext, OutOfMemoryError, "To allocate the SMS message");
          } else {
            memset(&tpsd, 0, sizeof(tpsd));
            err = SmsReadMessage(smsHandle,
                                 NULL,
                                 &smsaDestination,
                                 NULL,
                                 (PBYTE)pMessage,
            ...
            if (err != NO_ERROR) {
              ...
              *out = createArrayObject(currentContext, "[java.lang.String", 2);
              if (*out != null) {
                ...
                nm[0] = createStringObjectFromTCHARP(
                  currentContext, smsaDestination.ptsAddress, -1);
                nm[1] =
                  createStringObjectFromCharP(currentContext, pMessage, dwSize);
