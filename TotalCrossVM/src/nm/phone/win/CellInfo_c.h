...
CALLBACK RilCallback(DWORD dwCode, HRESULT hrCmdID, const void* lpData, DWORD cbdata,
					 DWORD dwParam);
...
static void CellInfoLoadResources(Context currentContext) {
	if (*tcSettings.romVersionPtr >= 420) {
		...
		if (!rilDll) {
			...
			_RIL_Initialize            = (RIL_InitializeProc)           GetProcAddress(rilDll,
										 TEXT("RIL_Initialize"));
			_RIL_Deinitialize          = (RIL_DeinitializeProc)         GetProcAddress(rilDll,
										 TEXT("RIL_Deinitialize"));
			_RIL_GetRegistrationStatus = (RIL_GetRegistrationStatusProc)GetProcAddress(rilDll,
										 TEXT("RIL_GetRegistrationStatus"));
			_RIL_GetCellTowerInfo      = (RIL_GetCellTowerInfoProc)     GetProcAddress(rilDll,
										 TEXT("RIL_GetCellTowerInfo"));
			_RIL_GetSignalQuality      = (RIL_GetSignalQualityProc)     GetProcAddress(rilDll,
										 TEXT("RIL_GetSignalQuality"));

			if (!_RIL_Initialize || !_RIL_Deinitialize || !_RIL_GetRegistrationStatus || !_RIL_GetCellTowerInfo
