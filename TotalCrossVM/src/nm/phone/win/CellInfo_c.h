...
DWORD
RilCallbackResult(HRESULT hrCmdID,
                  VoidP* lpData,
                  DWORD* cbData,
                  DWORD* dwParam);
CALLBACK
RilCallback(DWORD dwCode,
            HRESULT hrCmdID,
            const void* lpData,
            DWORD cbdata,
            DWORD dwParam);
...
static void
CellInfoLoadResources(Context currentContext)
{
  if (*tcSettings.romVersionPtr >= 420) {
    ...
    if (!rilDll) {
      throwException(currentContext,
                     RuntimeException,
                     "Could not load library for CellInfo");
    } else {
      _RIL_Initialize =
        (RIL_InitializeProc)GetProcAddress(rilDll, TEXT("RIL_Initialize"));
      _RIL_Deinitialize =
        (RIL_DeinitializeProc)GetProcAddress(rilDll, TEXT("RIL_Deinitialize"));
      _RIL_GetRegistrationStatus =
        (RIL_GetRegistrationStatusProc)GetProcAddress(
          rilDll, TEXT("RIL_GetRegistrationStatus"));
      _RIL_GetCellTowerInfo = (RIL_GetCellTowerInfoProc)GetProcAddress(
        rilDll, TEXT("RIL_GetCellTowerInfo"));
      _RIL_GetSignalQuality = (RIL_GetSignalQualityProc)GetProcAddress(
        rilDll, TEXT("RIL_GetSignalQuality"));

      if (!_RIL_Initialize || !_RIL_Deinitialize ||
          !_RIL_GetRegistrationStatus || !_RIL_GetCellTowerInfo ||
          !_RIL_GetSignalQuality) {
        throwException(currentContext,
                       RuntimeException,
                       "Could not load the procedures required for CellInfo");
      } else {
        HRESULT err = S_FALSE;
        int32 i;
        ...
        if (err != S_OK) {
          throwException(currentContext,
                         RuntimeException,
                         "RIL_Initialize falhou, o que fazer aqui?");
        } else {
...
static void
CellInfoUpdate(int32* mcc, int32* mnc, int32* lac, int32* cellid, int32* signal)
{
  HRESULT hrCmdID;
  VoidP data;
  DWORD cbData;
...
DWORD
RilCallbackResult(HRESULT hrCmdID, VoidP* lpData, DWORD* cbData, DWORD* dwParam)
{
            ...
            DWORD dwParam)
{
  ...
  if (lpData != null &&
      (*(callbackResultP->lpData) = xmalloc(cbData)) != null) {
