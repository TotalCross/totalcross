...
static void statusChange(CharP msg) {
	...
	if (listener == null || *listener != lastListener) {
		TCClass dial = loadClass(currentContext, "totalcross.phone.Dial",
...
//////////////////////////////////////////////////////////////////////////
TC_API void tpD_number_s(NMParams
						 p) { // totalcross/phone/Dial native public static void number(String number);
...
#if !defined WP8
		char number[100];
		JCharP2CharPBuf(String_charsStart(numberObj), min32(String_charsLen(numberObj), sizeof(number) - 1),
						number);
		dialNumber(number);
#else
		JChar number[100];
		JCharPDupBuf(String_charsStart(numberObj), min32(String_charsLen(numberObj), sizeof(number) - 1),
					 number);
