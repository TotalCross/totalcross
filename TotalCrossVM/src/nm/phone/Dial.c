...
#ifdef WINCE
static void
throwDialException(CharP msg, uint32 param);
static Context currentContext;
static void
statusChange(CharP msg);
#endif
...
statusChange(CharP msg)
{
  ...
  if (listener == null || *listener != lastListener) {
    TCClass dial =
      loadClass(currentContext,
                "totalcross.phone.Dial",
                true);  // flsobral@tc114_75: fixed Dial's full qualified name.
    ...
    if (listener == null) {
      throwException(currentContext,
                     NoSuchFieldError,
                     "Can't find Dial.listener static field.");
      return;
    }
    dialStatusChange = getMethod(
      OBJ_CLASS(*listener), true, "dialStatusChange", 1, "java.lang.String");
    if (dialStatusChange == null) {
      throwException(currentContext,
                     NoSuchMethodError,
                     "Dial.Listener.dialStatusChange(String)");
      return;
    }
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tpD_number_s(NMParams p)
{ // totalcross/phone/Dial native public
...
#if !defined WP8
    char number[100];
    JCharP2CharPBuf(String_charsStart(numberObj),
                    min32(String_charsLen(numberObj), sizeof(number) - 1),
                    number);
    dialNumber(number);
#else
    JChar number[100];
    JCharPDupBuf(String_charsStart(numberObj),
                 min32(String_charsLen(numberObj), sizeof(number) - 1),
                 number);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tpD_hangup(NMParams p)
{ // totalcross/phone/Dial native public static void hangup();
#if defined(WINCE)
  currentContext = p->currentContext;
  hangup();
#else
