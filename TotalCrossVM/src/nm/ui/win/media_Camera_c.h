...
HRESULT
SHCameraCapture(PSHCAMERACAPTURE pshcc, bool useThread)
{
  int32 id;
  ...
  if (_SHCameraCapture == NULL) {
    ...
    CameraThreadParams* p =
      (CameraThreadParams*)xmalloc(sizeof(CameraThreadParams));
    if (p == NULL) {
      return -3;
    } else {
      HRESULT ret;
      ...
      if (useThread) {
        CreateThread(NULL,
                     ...
                     p,
                     0,
                     &id); // guich@tc122_1: must be in a thread
      } else {
        privateThreadFunc(p);
      }
      while (!p->done) {
...
{
  return str
           ? JCharP2TCHARPBuf(String_charsStart(str), String_charsLen(str), buf)
           : null;
}
...
*/
char* pictureResolutionValues[] = { "480x640", "1E0",   "280",       "9DE00",
                                    "4DE00",   "7800",  "1024x1280", "400",
                                    "500",     "ABC00", "4BC00",     "7000",
                                    0 };
char* pictureResolutionNames[] = { "ItemString",
                                   "Width",
                                   "Height",
                                   "HighQualityFileSize",
                                   "NormalQualityFileSize",
                                   "LowQualityFileSize",
                                   0 };

static void
updatePicturesRegistry(int32 resW, int32 resH)
{ // guich@tc111_10
  HKEY handle = (HKEY)0;
  ...
  for (i = 1; found == -1; i++) {
    size = 4;
    wsprintf(keyname,
             TEXT("Software\\Microsoft\\Pictures\\Camera\\OEM\\PictureResolutio"
                  "n\\%d"),
             i);
    ...
    ww = hh = -1;
    err =
      RegQueryValueEx(handle, TEXT("Width"), null, null, (uint8*)&ww, &size);
    err =
      RegQueryValueEx(handle, TEXT("Height"), null, null, (uint8*)&hh, &size);
    if (ww == resW && hh == resH) {
      found = i;
    }
    RegCloseKey(handle);
  ...
  if (found ==
      -1) { // if key was not found, add a new entry with that resolution
    // increase OptionNum count
    RegOpenKeyEx(
      HKEY_LOCAL_MACHINE,
      TEXT("Software\\Microsoft\\Pictures\\Camera\\OEM\\PictureResolution"),
      0,
      KEY_READ | KEY_WRITE,
      &handle);
    size = 4;
    RegQueryValueEx(
      handle, TEXT("OptionNum"), null, null, (uint8*)&dvalue, &size);
    dvalue++;
    RegSetValueEx(
      handle, TEXT("OptionNum"), null, REG_DWORD, (uint8*)&dvalue, 4);
    RegCloseKey(handle);
    ...
    for (values = pictureResolutionValues; *values; values += 6)
      if (xstrcmp(*values, option1) == 0) { // found such resolution?
        ...
        found = i;
        wsprintf(keyname,
                 TEXT("Software\\Microsoft\\Pictures\\Camera\\OEM\\PictureResol"
                      "ution\\%d"),
                 found);
        err = RegCreateKeyEx(
          HKEY_LOCAL_MACHINE, keyname, 0, 0, 0, KEY_WRITE, 0, &handle, &size);
        if (err != 0) {
          return;
        }
        names = pictureResolutionNames;
        // set the ItemString (the only SZ)
        err = RegSetValueEx(handle,
                            CharP2TCHARPBuf(*names++, keyname),
                            0,
                            REG_SZ,
                            (uint8*)CharP2TCHARPBuf(*values, keyvalue),
                            xstrlen(*values) * 2 + 2);
        values++;
  ...
  if (found != -1 &&
      RegOpenKeyEx(HKEY_CURRENT_USER,
                   TEXT("\\Software\\Microsoft\\Pictures\\Camera\\USER"),
                   0,
                   KEY_WRITE,
                   &handle) == 0) { // update the currently selected resolution
    dvalue = found - 1;
    RegSetValueEx(
      handle, TEXT("Resolution"), null, REG_DWORD, (uint8*)&dvalue, 4);
    RegCloseKey(handle);
  }
}
...
changeArcSoftAPI()
{
  ...
  err = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
                     TEXT("System\\Pictures\\Camera\\OEM"),
                     0,
                     KEY_ALL_ACCESS,
                     &handle);
  if (err) {
  ...
  buf[0] = 0;
  RegQueryValueEx(
    handle, TEXT("OEMCAMERACAPTUREDLL"), null, null, (uint8*)buf, &size);
  if (wcsicmp(buf, TEXT("\\windows\\oemcameracapture.dll")) == 0) {
    RegSetValueEx(handle,
                  TEXT("OEMCAMERACAPTUREDLL"),
                  0,
                  REG_SZ,
                  (uint8*)TEXT("\\windows\\_oemcameracapture.dll"),
                  62);
  } else if (wcsicmp(buf, TEXT("\\windows\\_oemcameracapture.dll")) == 0) {
    RegSetValueEx(handle,
                  TEXT("OEMCAMERACAPTUREDLL"),
                  0,
                  REG_SZ,
                  (uint8*)TEXT("\\windows\\oemcameracapture.dll"),
                  60);
  }
  ...
  if (wcsicmp(buf, TEXT("\\windows\\camera.exe")) == 0) {
    RegSetValueEx(handle,
                  ...
                  0,
                  REG_SZ,
                  (uint8*)TEXT("\\windows\\pimg.exe"),
                  36);
  } else if (wcsicmp(buf, TEXT("\\windows\\pimg.exe")) == 0) {
    RegSetValueEx(handle,
                  TEXT("CameraApp"),
                  0,
                  REG_SZ,
                  (uint8*)TEXT("\\windows\\camera.exe"),
                  40);
  }
...
typedef TCHAR*(__stdcall* procshowDolphinCamera)(TCHAR* path,
                                                 TCHAR* file,
                                                 bool lowRes);
...
cameraClick(NMParams p)
{
  ...
  if (wcsstr(deviceId, TEXT("Intermec")) !=
      null) { // intermec devices don't like capturing on a thread.
    useThread = false;
  }
  ...
  if (wcsstr(deviceId, TEXT("Hand Held")) != null) {
    TCHAR* ret;
    bool lowRes =
      Camera_resolutionWidth(obj) != 640 || Camera_resolutionHeight(obj) != 480;
    ...
    if (dolphinDll == null && (dolphinDll = loadLibrary("Dolphin")) == null) {
      throwException(
        p->currentContext, IOException, "Error: cannot find Dolphin.dll");
    } else {
      if (showDolphinCamera == null) {
        showDolphinCamera = (procshowDolphinCamera)GetProcAddress(
          dolphinDll, TEXT("showDolphinCamera"));
      }
      ret = showDolphinCamera(
        initialDir[0] ? initialDir : null, defFN[0] ? defFN : null, lowRes);
      p->retO =
        ret ? createStringObjectFromTCHAR(p->currentContext, ret, tcslen(ret))
            : null;
    }
    return;
  }
  ...
  if (Camera_captureMode(obj) == 0) { // photo?
    updatePicturesRegistry(Camera_resolutionWidth(obj),
                           Camera_resolutionHeight(obj));
  }
  ...
  if (hResult != S_OK && hResult != 1) {
    throwException(p->currentContext,
                   IOException,
                   "Error when using the camera: %d (0x%X)",
                   (int)hResult,
                   (int)hResult);
  } else {
    p->retO = hResult == 1 ? null
                           : createStringObjectFromTCHAR(p->currentContext,
                                                         shcc.szFile,
                                                         tcslen(shcc.szFile));
  }
  if (wcsstr(deviceId, TEXT("Motorola")) != null) {
