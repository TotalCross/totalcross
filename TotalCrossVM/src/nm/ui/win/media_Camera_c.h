...
static void updatePicturesRegistry(int32 resW, int32 resH) { // guich@tc111_10
	...
	if (found == -1) { // if key was not found, add a new entry with that resolution
		...
		RegOpenKeyEx(HKEY_LOCAL_MACHINE,
					 TEXT("Software\\Microsoft\\Pictures\\Camera\\OEM\\PictureResolution"), 0, KEY_READ | KEY_WRITE,
					 &handle);
		size = 4;
		...
		for (values = pictureResolutionValues; *values; values += 6)
			if (xstrcmp(*values, option1) == 0) { // found such resolution?
				...
				err = RegSetValueEx(handle, CharP2TCHARPBuf(*names++, keyname), 0, REG_SZ,
									(uint8*)CharP2TCHARPBuf(*values, keyvalue), xstrlen(*values) * 2 + 2);
				values++;
	...
	if (found != -1
		&& RegOpenKeyEx(HKEY_CURRENT_USER, TEXT("\\Software\\Microsoft\\Pictures\\Camera\\USER"), 0,
						KEY_WRITE, &handle) == 0) { // update the currently selected resolution
		dvalue = found - 1;
...
static void changeArcSoftAPI() {
	...
	err = RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("System\\Pictures\\Camera\\OEM"), 0, KEY_ALL_ACCESS,
					   &handle);
	if (err) {
		return;
	}
	...
	if (wcsicmp(buf, TEXT("\\windows\\oemcameracapture.dll")) == 0) {
		RegSetValueEx(handle, TEXT("OEMCAMERACAPTUREDLL"), 0, REG_SZ,
					  (uint8*)TEXT("\\windows\\_oemcameracapture.dll"), 62);
	} else if (wcsicmp(buf, TEXT("\\windows\\_oemcameracapture.dll")) == 0) {
		RegSetValueEx(handle, TEXT("OEMCAMERACAPTUREDLL"), 0, REG_SZ,
					  (uint8*)TEXT("\\windows\\oemcameracapture.dll"), 60);
	}
...
static void cameraClick(NMParams p) {
	...
	if (hResult != S_OK && hResult != 1) {
		throwException(p->currentContext, IOException, "Error when using the camera: %d (0x%X)",
					   (int)hResult, (int)hResult);
	} else {
		p->retO = hResult == 1 ? null : createStringObjectFromTCHAR(p->currentContext, shcc.szFile,
				  tcslen(shcc.szFile));
	}
	if (wcsstr(deviceId, TEXT("Motorola")) != null) {
		changeArcSoftAPI();
