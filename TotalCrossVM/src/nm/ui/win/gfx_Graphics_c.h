...
adjustWindowSizeWithBorders(int32 resizableWindow, int32* w, int32* h)
{
  ...
  *w +=
    GetSystemMetrics(resizableWindow ? SM_CXSIZEFRAME : SM_CXFIXEDFRAME) * 2;
  *h +=
    GetSystemMetrics(resizableWindow ? SM_CYSIZEFRAME : SM_CYFIXEDFRAME) * 2 +
...
restoreTaskbar()
{
  ...
  if (!isWindowsMobile) {
    ...
    SystemParametersInfo(
      ...
      0,
      &defaultWorkingArea,
      SPIF_SENDCHANGE); // flsobral@tc113_25: fixed bug - instead of restoring
                        // the taskbar, it was changing the default window
                        // size to the size of the taskbar.
  }
#endif
}

void
getScreenSize(int32* w, int32* h)
{
#ifdef WINCE
  RECT rect;
...
graphicsStartup(ScreenSurface screen, int16 appTczAttr)
{
  ...
  if (appTczAttr & ATTR_WINDOWSIZE_320X480) {
    ...
    defScrX = defScrY = -2;
    width = 600;
    height = min32(800, rect.bottom - rect.top);
  } else {
  ...
  rect.left =
    defScrX == -1
      ? 0
      : defScrX == -2 ? (rect.left + (rect.right - width) / 2) : defScrX;
  rect.top =
    defScrY == -1
      ? 0
      : defScrY == -2 ? (rect.top + (rect.bottom - height) / 2) : defScrY;
  rect.bottom = height;
  rect.right = width;
  ...
  CharP2TCHARPBuf(dot ? dot + 1 : mainClassName,
                  main); // remove the package from the name
  mainHWnd =
    CreateWindow(exeName,
                 main,
                 style,
                 rect.left,
                 rect.top,
                 ...
                 NULL,
                 instance,
                 NULL); // guich@400_62: move window to desired user position
  if (!mainHWnd) {
    return false;
  }
...
graphicsCreateScreenSurface(ScreenSurface screen)
{
  ...
  dibInfo.bi.bmiHeader.biCompression =
    (screen->bpp == 16) ? BI_BITFIELDS : BI_RGB;
  ...
  dibInfo.bi.bmiHeader.biSizeImage =
    screen->screenW * screen->screenH * screen->bpp / 8;
  if (screen->bpp == 16) {
    // setup the bit masks
    ...
    ptr[1] = 0x07e0;
    ptr[2] = 0x001F;
  } else if (screen->bpp == 8) { // apply our 485 palette to the screen
  ...
  SCREEN_EX(screen)->hbmp = CreateDIBSection(SCREEN_EX(screen)->dc,
                                             &dibInfo.bi,
                                             DIB_RGB_COLORS,
                                             (void**)&screen->pixels,
                                             NULL,
...
inline static void
drawImageLine(ScreenSurface screen,
              HDC targetDC,
              int32 minx,
              int32 miny,
              int32 maxx,
...
{ // screen's already locked
  ...
  BitBlt(SCREEN_EX(screen)->dc,
         ...
         currentContext->dirtyX2 - currentContext->dirtyX1,
         currentContext->dirtyY2 - currentContext->dirtyY1,
         targetDC,
         currentContext->dirtyX1,
         currentContext->dirtyY1,
         SRCCOPY);
