...
void adjustWindowSizeWithBorders(int32 resizableWindow, int32* w, int32* h) {
	...
	*h += GetSystemMetrics(resizableWindow ? SM_CYSIZEFRAME : SM_CYFIXEDFRAME) * 2 + GetSystemMetrics(
			  SM_CYCAPTION);
#endif
}
...
void restoreTaskbar() {
	...
	if (!isWindowsMobile) {
		...
		SystemParametersInfo(SPI_SETWORKAREA, 0, &defaultWorkingArea,
							 SPIF_SENDCHANGE); // flsobral@tc113_25: fixed bug - instead of restoring the taskbar, it was changing the default window size to the size of the taskbar.
	}
#endif
}
...
bool graphicsStartup(ScreenSurface screen, int16 appTczAttr) {
	...
	mainHWnd = CreateWindow(exeName, main, style, rect.left, rect.top, rect.right, rect.bottom, NULL,
							NULL, instance, NULL);  // guich@400_62: move window to desired user position
	if (!mainHWnd) {
		return false;
	}
...
bool graphicsCreateScreenSurface(ScreenSurface screen) {
	...
	SCREEN_EX(screen)->hbmp = CreateDIBSection(SCREEN_EX(screen)->dc, &dibInfo.bi, DIB_RGB_COLORS,
							  (void**)&screen->pixels, NULL, 0);
...
inline static void drawImageLine(ScreenSurface screen, HDC targetDC, int32 minx, int32 miny,
...
void graphicsUpdateScreen(Context currentContext, ScreenSurface screen) { // screen's already locked
	...
	BitBlt(SCREEN_EX(screen)->dc, currentContext->dirtyX1, currentContext->dirtyY1,
		   currentContext->dirtyX2 - currentContext->dirtyX1,
		   currentContext->dirtyY2 - currentContext->dirtyY1, targetDC, currentContext->dirtyX1,
		   currentContext->dirtyY1, SRCCOPY);
