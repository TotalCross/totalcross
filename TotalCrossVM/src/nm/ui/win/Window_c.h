...
static void windowSetSIP(int32 sipOption, bool numeric) {
...

	{
		...
		if (vkSettings.bottom > scrH || vkSettings.right > scrW) { // guich@tc110_99
			int32 w = min32(scrW,
							scrH); // if the screen has been rotated, then the width is surely the width in portrait (mininum value)
			int32 h = scrH - scrW;
			...
			case SIP_SHOW:
				...
				SipShowIM(SIPF_ON);
				{
					...
					SetWindowPos(hsipbtn, HWND_TOP, 0, 0, 0, 0,
								 SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW); // sets z-order to top but doesn't reposition or resize
				}
				break;
		}
#endif
...
static void windowSetDeviceTitle(TCObject titleObj) {
	TCHAR buf[30];
	JCharP2TCHARPBuf(String_charsStart(titleObj), min32(String_charsLen(titleObj), 29),
