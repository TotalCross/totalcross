...
#define EM_SETINPUTMODE                                                        \
  0x00DE // Sets default input mode when control gets focus. lParam should be be
         // EIM_*|EIMMF_*.
...
static void
windowSetSIP(int32 sipOption, bool numeric)
{
  isShown = sipOption != SIP_HIDE && sipOption != SIP_DISABLE_NUMERICPAD;
  if (numeric) {
    sipOption =
      sipOption == SIP_HIDE ? SIP_DISABLE_NUMERICPAD : SIP_ENABLE_NUMERICPAD;
  }

  {
    ...
    if (vkSettings.bottom > scrH || vkSettings.right > scrW) { // guich@tc110_99
      int32 w = min32(scrW,
                      scrH); // if the screen has been rotated, then the width
                             // is surely the width in portrait (mininum value)
      int32 h = scrH - scrW;
    ...
    switch (sipOption) {
      ...
      case SIP_HIDE:
        ...
        SipShowIM(SIPF_OFF);
        {
          // flsobral@tc114_50: fixed the SIP keyboard button not being properly
          // displayed on some WinCE devices.
          HWND hsipbtn;
          bSipUp = false;
      ...
      case SIP_BOTTOM:
        if (sipOption == SIP_TOP) {
          sipRect.bottom = screen.screenY +
                           (vkSettings.bottom - vkSettings.top) +
                           vkSettings.topGap;
          sipRect.top = screen.screenY + vkSettings.topGap;
        } else {
          if (!isWindowsMobile) {
            // flsobral@tc113_24: SIP bottom position is always relative to the
            // screen, not to its original position.
            sipRect.bottom = screen.screenY + screen.screenH;
            sipRect.top = sipRect.bottom - (vkSettings.bottom - vkSettings.top);
          } else {
            // flsobral@tc113_42: SIP is relative to the screen only on
            // WindowsCE, WM devices use the original SIP position.
        ...
        SipShowIM(SIPF_ON);
        {
          ...
          SetWindowPos(hsipbtn,
                       ...
                       SWP_NOMOVE | SWP_NOSIZE |
                         SWP_SHOWWINDOW); // sets z-order to top but doesn't
...
windowSetDeviceTitle(TCObject titleObj)
{
  TCHAR buf[30];
  JCharP2TCHARPBuf(
    String_charsStart(titleObj),
    min32(String_charsLen(titleObj), 29),
    buf); // guich@tc113_32: limit to buf's size (and reduced to 30 chars)
