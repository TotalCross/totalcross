...
static bool mediaClipPlay(Context currentContext, TCObject mediaClip, MediaData media) {
	...
	media->byteBuffer = createByteArray(currentContext,
										(media->waveBuffer.blockSize + sizeof(WAVEHDR)) * media->waveBuffer.blockCount);
	media->waveBuffer.blocks =  ARRAYOBJ_START(media->byteBuffer);
	media->waveBuffer.headers = (WAVEHDR*)(ARRAYOBJ_START(media->byteBuffer) +
	...
	if ((err = waveOutOpen(&media->waveOUT, WAVE_MAPPER, (LPWAVEFORMATEX) &waveFormat,
						   (DWORD) media->evtDone, (DWORD) media, CALLBACK_EVENT)) != MMSYSERR_NOERROR) {
		goto end;
	}
...
static Err mediaClipReset(TCObject mediaClip, MediaData media) {
	...
	executeMethod(media->currentContext, media->setPosMethod, MediaClip_mediaClipStream(mediaClip),
				  media->dataPos);
...
static bool mediaClipClose(TCObject mediaClip, MediaData media) {
	...
	if (!MediaClip_isRecording(mediaClip)) {
		...
		executeMethod(media->currentContext, media->writeMethod, MediaClip_mediaClipStream(mediaClip),
					  MediaClip_mediaHeader(mediaClip), 0, 46);
	}
...
static char* getMMErrorMessage(int32 res) {
	switch (res) {
		...
		case WAVERR_UNPREPARED:
			return "The buffer pointed to by the wave header parameter hasn't been prepared";
		case WAVERR_SYNC:
			return "The device is synchronous but waveOutOpen was called without using the WAVE_ALLOWSYNC flag";
		case WAVERR_STILLPLAYING:
			return "The buffer pointed to by the wave header parameter is still in the queue";
...
static void mediaClipRecord(Context currentContext, TCObject mediaClip, MediaData media,
							int32 samplesPerSecond, int32 bitsPerSample, bool stereo) {
	WAVEFORMATEX waveFormat;
	...
	media->byteBuffer = createByteArray(currentContext,
										(media->waveInBuffer.blockSize + sizeof(WAVEHDR)) * media->waveInBuffer.blockCount);
	media->waveInBuffer.blocks = ARRAYOBJ_START(media->byteBuffer);
	media->waveInBuffer.headers = (WAVEHDR*)(ARRAYOBJ_START(media->byteBuffer) +
	...
	if ((err = waveInOpen(&media->waveIN, WAVE_MAPPER, (LPWAVEFORMATEX) &waveFormat,
						  (DWORD) media->evtDone, (DWORD) media, CALLBACK_EVENT)) != MMSYSERR_NOERROR) {
		goto error;
	}
	...
	for (i = media->waveInBuffer.blockCount - 1; i >= 0 ; i--) {
		...
		media->waveInBuffer.headers[i].lpData = media->waveInBuffer.blocks + media->waveInBuffer.blockSize *
												i;
...
start:
	while (!media->stop) {
		...
		buf = current->lpData;
		bytesRead = executeMethod(media->currentContext, media->readMethod,
								  MediaClip_mediaClipStream(media->mediaStream), media->byteBuffer,
								  media->waveBuffer.blockSize * media->waveBuffer.currentBlock, media->waveBuffer.blockSize).asInt32;
		if (bytesRead <= 0) {
...
DWORD WINAPI ThreadInProc(LPVOID lpParameter) {
	...
	while (!media->stop) {
		...
		bytesRead = executeMethod(media->currentContext, media->writeMethod,
								  MediaClip_mediaClipStream(media->mediaStream), media->byteBuffer,
								  media->waveInBuffer.blockSize * currentBlock, current->dwBytesRecorded).asInt32;
