...
DWORD WINAPI
ThreadInProc(LPVOID lpParameter);

static bool
mediaClipPlay(Context currentContext, TCObject mediaClip, MediaData media)
{
  ...
  media->byteBuffer =
    createByteArray(currentContext,
                    (media->waveBuffer.blockSize + sizeof(WAVEHDR)) *
                      media->waveBuffer.blockCount);
  media->waveBuffer.blocks = ARRAYOBJ_START(media->byteBuffer);
  media->waveBuffer.headers =
    (WAVEHDR*)(ARRAYOBJ_START(media->byteBuffer) +
               media->waveBuffer.blockSize * media->waveBuffer.blockCount);
  for (i = media->waveBuffer.blockCount - 1; i >= 0; i--) {
    ...
    media->waveBuffer.headers[i].lpData =
      media->waveBuffer.blocks + media->waveBuffer.blockSize * i;
  }
  ...
  if ((err = waveOutOpen(&media->waveOUT,
                         WAVE_MAPPER,
...
static Err
mediaClipPause(MediaData media)
{
  return waveOutPause(media->waveOUT);
}

static Err
mediaClipResume(MediaData media)
{
  return waveOutRestart(media->waveOUT);
}

static Err
mediaClipStop(MediaData media, bool pause)
{
  Err err;
...
static Err
mediaClipReset(TCObject mediaClip, MediaData media)
{
  waveOutReset(media->waveOUT);
  executeMethod(media->currentContext,
                media->setPosMethod,
                MediaClip_mediaClipStream(mediaClip),
                media->dataPos);
...
static bool
mediaClipClose(TCObject mediaClip, MediaData media)
{
  int32 dataSize;
  Err err;
  ...
  if (!MediaClip_isRecording(mediaClip)) {
    ...
    executeMethod(media->currentContext,
                  media->writeMethod,
                  MediaClip_mediaClipStream(mediaClip),
                  MediaClip_mediaHeader(mediaClip),
                  0,
                  46);
  }
...
getMMErrorMessage(int32 res)
{
  switch (res) {
    ...
    case WAVERR_UNPREPARED:
      return "The buffer pointed to by the wave header parameter hasn't been "
             "prepared";
    case WAVERR_SYNC:
      return "The device is synchronous but waveOutOpen was called without "
             "using "
             "the WAVE_ALLOWSYNC flag";
    case WAVERR_STILLPLAYING:
      return "The buffer pointed to by the wave header parameter is still in "
             "the "
             "queue";
    case MMSYSERR_NOTSUPPORTED:
      return "Specified device is synchronous and does not support pausing";
    case 999:
      return "Not enough memory to create recording buffer";
    default:
      return null;
  }
}
...
static void
throwMMException(Context currentContext, uint32 res)
{
...
static void
mediaClipRecord(Context currentContext,
                TCObject mediaClip,
                MediaData media,
                ...
                bool stereo)
{
  ...
  media->waveHeader->BlockAlign =
    media->waveHeader->NumChannels * bitsPerSample / 8;
  media->waveHeader->ByteRate =
    samplesPerSecond * media->waveHeader->BlockAlign;
  ...
  // flsobral@tc112_17: The section below was using two different buffers,
  // waveBuffer and waveInBuffer.
  media->waveInBuffer.blockCount = 5;
  media->waveInBuffer.blockSize = media->waveHeader->BlockAlign * 1024;
  media->byteBuffer =
    createByteArray(currentContext,
                    (media->waveInBuffer.blockSize + sizeof(WAVEHDR)) *
                      media->waveInBuffer.blockCount);
  media->waveInBuffer.blocks = ARRAYOBJ_START(media->byteBuffer);
  media->waveInBuffer.headers =
    (WAVEHDR*)(ARRAYOBJ_START(media->byteBuffer) +
               media->waveInBuffer.blockSize * media->waveInBuffer.blockCount);
  ...
  if ((err = waveInOpen(&media->waveIN,
                        WAVE_MAPPER,
                        (LPWAVEFORMATEX)&waveFormat,
                        (DWORD)media->evtDone,
                        (DWORD)media,
  ...
  for (i = media->waveInBuffer.blockCount - 1; i >= 0; i--) {
    media->waveInBuffer.headers[i].dwBufferLength =
      media->waveInBuffer.blockSize;
    media->waveInBuffer.headers[i].dwFlags = 0;
    media->waveInBuffer.headers[i].lpData =
      media->waveInBuffer.blocks + media->waveInBuffer.blockSize * i;
    err = waveInPrepareHeader(
      media->waveIN, &media->waveInBuffer.headers[i], sizeof(WAVEHDR));
  }
  err = waveInAddBuffer(
    media->waveIN, &media->waveInBuffer.headers[0], sizeof(WAVEHDR));
  ...
  return; // NO_ERROR;
	...
	err = waveInOpen(&media->waveIN, (UINT)WAVE_MAPPER, &PCMfmt, (DWORD)SoundWaveInProc, (DWORD)params,
					 CALLBACK_FUNCTION);
...
start:
  while (!media->stop) {
    ...
    buf = current->lpData;
    bytesRead = executeMethod(media->currentContext,
                              media->readMethod,
                              MediaClip_mediaClipStream(media->mediaStream),
                              media->byteBuffer,
                              media->waveBuffer.blockSize *
...
ThreadInProc(LPVOID lpParameter)
{
  ...
  while (!media->stop) {
    ...
    bytesRead = executeMethod(media->currentContext,
                              ...
                              MediaClip_mediaClipStream(media->mediaStream),
                              media->byteBuffer,
                              media->waveInBuffer.blockSize * currentBlock,
                              current->dwBytesRecorded)
                  .asInt32;
