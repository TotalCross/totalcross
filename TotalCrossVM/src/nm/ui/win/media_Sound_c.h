...
BOOL WaveBeeperHoneywell(unsigned frequency, unsigned duration) {
	...
	HANDLE gFrontSpeakerDriverHandle = CreateFile(L"FSP1:", GENERIC_READ | GENERIC_WRITE,
									   FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, 0);
	if (gFrontSpeakerDriverHandle != INVALID_HANDLE_VALUE) {
		rc = DeviceIoControl(gFrontSpeakerDriverHandle, kFSPSetTone, kFSPPBYTE, (DWORD) frequency,
							 kFSPPBYTE, (DWORD) duration, kFSPPDWORD,  NULL);
...
static void soundSetEnabled(bool enableSound) {
	...
	for (DeviceID = 0; DeviceID < numDevices; DeviceID++) {
		...
		mixerGetLineInfo((HMIXEROBJ) hMixer, &Line,
		...
		for (ControlID = 1; ControlID <= NumControls ; ControlID++) { //IDs are NOT zero-based
			...
			mixerGetControlDetails((HMIXEROBJ) hMixer, &Details,
...
void soundTone(int32 frequency, int32 duration) {
	...
	if (xstrstr(buf, "Hand")) { // Hand Held Products
		...
		if (waveOutOpen(&hWaveOut, WAVE_MAPPER, (LPWAVEFORMATEX) &waveMidi, (DWORD) hEvent, 0,
