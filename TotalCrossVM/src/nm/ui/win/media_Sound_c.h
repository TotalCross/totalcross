...
BOOL
WaveBeeperHoneywell(unsigned frequency, unsigned duration)
{
  DWORD rc = 0;

  HANDLE gFrontSpeakerDriverHandle =
    CreateFile(L"FSP1:",
  ...
  if (gFrontSpeakerDriverHandle != INVALID_HANDLE_VALUE) {
    rc = DeviceIoControl(gFrontSpeakerDriverHandle,
                         kFSPSetTone,
                         kFSPPBYTE,
                         (DWORD)frequency,
                         kFSPPBYTE,
 ...
 *    WAVECAPS_SYNC           Specifies that the driver is synchronous and
 *blocks while playing a buffer. WAVECAPS_VOLUME         Supports volume
 *control. WAVECAPS_SAMPLEACCURATE Returns sample-accurate position information.
 *
 * waveOutGetNumDevs
 * waveOutGetDevCaps
 * waveOutSetVolume
 ...
 ************************************/
static void
soundSetEnabled(bool enableSound)
{
#if defined(WINCE)
  ...
  if (waveOutGetDevCaps(WAVE_MAPPER, &tWaveoutCaps, sizeof(tWaveoutCaps)) !=
      MMSYSERR_NOERROR) {
    return;
  }
  ...
  for (DeviceID = 0; DeviceID < numDevices; DeviceID++) {
    ...
    for (ControlID = 1; ControlID <= NumControls;
         ControlID++) { // IDs are NOT zero-based
      ...
      Value.dwValue =
        enableSound
          ? 0
          : ((MIXERCONTROLDETAILS_UNSIGNED*)Details.paDetails)->dwValue;
...
soundTone(int32 frequency, int32 duration)
{
  ...
  if (xstrstr(buf, "Hand")) { // Hand Held Products
    ...
    /// These fields adjust the interpretation of DeltaTicks, and thus the rate
    /// of playback
    // Set to 1 second. Note driver will default to 500000 if set to 0.
    waveMidi.USecPerQuarterNote = 1000000;
    // Set to 1000 so duration is expressed in milliseconds. Note driver will
    // default to 96 if set to 0
    ...
    if (waveOutOpen(&hWaveOut,
      ...
      if (waveOutPrepareHeader(hWaveOut, &waveHdr, sizeof(waveHdr)) ==
        ...
        if (waveOutWrite(hWaveOut, &waveHdr, sizeof(waveHdr)) ==
          ...
          if (WaitForSingleObject(hEvent, (uint32)(duration * 1.5)) ==
              WAIT_TIMEOUT) {
...
#if defined(WINCE)
bool
SndGetSoundWM5(SND_EVENT sndEvent, SNDFILEINFO* soundFileInfo)
{
...
bool
SndSetSoundWM5(SND_EVENT sndEvent, SNDFILEINFO* soundFileInfo)
{
  if (!isWindowsMobile || *tcSettings.romVersionPtr < 500 ||
      _SndSetSound == NULL) {
    return false;
