...
#define SETPIXEL24(t, f)                                                       \
  ...
  typedef struct TScreenSurface
  {                // represents a device-dependant surface,
                   // there's only ONE per application
    uint8* pixels; // pixels in native format
    __unsafe_unretained TCObject
      mainWindowPixels; // pixels in 888 format, read directly from
                        // totalcross.ui.gfx.Graphics class
   ...
   * defined in plain C (such as SymbianOS C++ classes, iPhone objC data
   * structures, ...) Currently this pointer is mirrored in ScreenSurface's
