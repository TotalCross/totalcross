...
void setupLookupBuffers() {
	int32 i;
	for (i = 0; i < 14; i++) {
		ftransp[i + 1] = (float)(i << 4) / (float)
						 255;    // make it lighter. since ftransp[0] is never used, shift it to [1]
	}
	ftransp[15] = 1;
	for (i = 0; i <= 255; i++) {
...
static Pixel* getSurfacePixels(TCObject surf) {
	...
	if (surf != null) {
		...
		if (name[0] == 't'
			&& strEq(name,
					 "totalcross.ui.gfx.Graphics")) { // if the surface is a Graphics, get the target surface
...
void screenChange(Context currentContext, int32 newWidth, int32 newHeight, int32 hRes, int32 vRes,
				  bool nothingChanged) { // rotate the screen
...
#ifndef SKIA_H
static void drawSurface(Context currentContext, TCObject dstSurf, TCObject srcSurf, int32 srcX,
						int32 srcY, int32 width, int32 height,
	...
	if (Surface_isImage(srcSurf)) {
		...
		if (forcedSmoothScale && (Image_hwScaleW(srcSurf) != 1 || Image_hwScaleH(srcSurf) != 1)) {
			...
			if (mGetScaledInstance == null) {
				mGetScaledInstance       = getMethod(OBJ_CLASS(srcSurf), false, "getScaledInstance", 2, J_INT,
													 J_INT);
				mGetSmoothScaledInstance = getMethod(OBJ_CLASS(srcSurf), false, "getSmoothScaledInstance", 2, J_INT,
													 J_INT);
			}
			disableGC = true; // the gc may collect the image before we lock it here (*)
			newSurf = executeMethod(currentContext, Image_hwScaleW(srcSurf) < 1
									&& Image_hwScaleH(srcSurf) < 1 ? mGetSmoothScaledInstance : mGetScaledInstance, srcSurf, srcWidth,
									srcHeight).asObj;
	...
	if (isSrcScreen) {
		...
		/*img*/ glDrawTexture(Image_textureId(srcSurf), srcX + frame * srcPitch, srcY, width, height, dstX,
							  dstY, 0, 0, fc > 1 ? (int32)(Image_widthOfAllFrames(srcSurf) * Image_hwScaleW(srcSurf)) : srcWidth,
							  srcHeight, null, alphaMask);
	} else
#endif
...
static void drawSurface(Context currentContext, TCObject dstSurf, TCObject srcSurf, int32 srcX,
	...
	if (Surface_isImage(srcSurf)) {
		...
		skia_drawSurface(0, Image_textureId(srcSurf), srcX, srcY, w, h, w, h, dstX, dstY,
						 Image_alphaMask(srcSurf), doClip);
		skia_restoreClip();
	} else {
...
static int32 getPixel(TCObject g, int32 x, int32 y) {
	...
	x += Graphics_transX(g);
	y += Graphics_transY(g);
	if (Graphics_clipX1(g) <= x && x < Graphics_clipX2(g) && Graphics_clipY1(g) <= y
		&& y < Graphics_clipY2(g)) {
		PixelConv p;
#ifdef SKIA_H
...
static bool surelyOutsideClip(TCObject g, int32 x1, int32 y1, int32 x2, int32 y2) {
	...
	return (x1 < cx1 && x2 < cx1) || (x1 > cx2 && x2 > cx2) || (y1 < cy1 && y2 < cy1) || (y1 > cy2
			&& y2 > cy2);
}

//   Device specific routine.
//   Draws a horizontal line from x to x+w-1, translating and clipping
static void drawHLine(Context currentContext, TCObject g, int32 x, int32 y, int32 width,
					  Pixel pixel1, Pixel pixel2) {
	x += Graphics_transX(g);
	y += Graphics_transY(g);
	/*
	...
	*/
	if (Graphics_clipY1(g) <= y && y < Graphics_clipY2(g) && Graphics_clipX1(g) <= (x + width)
		&& x < Graphics_clipX2(g)) { // NOPT
		Pixel* pTgt;
...
static void drawVLine(Context currentContext, TCObject g, int32 x, int32 y, int32 height,
					  Pixel pixel1, Pixel pixel2) {
	x += Graphics_transX(g);
	y += Graphics_transY(g);
	/*
	...
	*/
	if (Graphics_clipX1(g) <= x && x < Graphics_clipX2(g) && Graphics_clipY1(g) <= (y + height)
		&& y < Graphics_clipY2(g)) { // NOPT
		Pixel* pTgt;
...
static void drawDottedLine(Context currentContext, TCObject g, int32 x1, int32 y1, int32 x2,
						   int32 y2, Pixel pixel1, Pixel pixel2) {
	...
	if (dX == 0 && dY == 0) {
		...
		Pixel* row;
		int32 on = 1;
		int32 clipX1 = Graphics_clipX1(g), clipY1 = Graphics_clipY1(g), clipX2 = Graphics_clipX2(g),
			  clipY2 = Graphics_clipY2(g);
		bool dontClip =
			true; // the most common will be draw lines that do not cross the clip bounds, so we may speedup a little
		...
		row = getGraphicsPixels(g) + (pyInc > 0 ? yMin : (yMin + dY)) * Graphics_pitch(g) +
			  (xInc > 0 ? xMin : (xMin + dX)); // currentX    | else start from the max value

		yInc = pyInc > 0 ? Graphics_pitch(g) : -Graphics_pitch(
				   g); // now that we have active, correctly assign the y incrementing value
		...
		if (dX >= dY) { // if X is the independent variable
			...
			if (pixel1 == pixel2) // quick optimization
				for (; dX >= 0;
					 dX--) {                     // process each point in the line one at a time (just use dX)
					if (dontClip || (clipX1 <= currentX && currentX < clipX2 && clipY1 <= currentY
									 && currentY < clipY2)) {
#ifdef __gl2_h_
						if (Graphics_useOpenGL(g)) {
			...
			if (pixel1 == pixel2) // quick optimization
				for (; dY >= 0;
					 dY--) {                     // process each point in the line one at a time (just use dY)
					if (dontClip || (clipX1 <= currentX && currentX < clipX2 && clipY1 <= currentY
									 && currentY < clipY2)) {
#ifdef __gl2_h_
						if (Graphics_useOpenGL(g)) {
...
#ifndef __gl2_h_
		if (!currentContext->fullDirty && !Graphics_isImageSurface(g)) {
			markScreenDirty(currentContext, xMin, yMin, (x2 > x1) ? (x2 - x1) : (x1 - x2),
							(y2 > y1) ? (y2 - y1) : (y1 - y2));
		}
#else
		if (Graphics_isImageSurface(g)) {
...
static void drawLine(Context currentContext, TCObject g, int32 x1, int32 y1, int32 x2, int32 y2,
					 Pixel pixel) {
...
#ifndef SKIA_H
static void drawRect(Context currentContext, TCObject g, int32 x, int32 y, int32 width,
					 int32 height, Pixel pixel) {
...
#else
static void drawRect(Context currentContext, TCObject g, int32 x, int32 y, int32 w, int32 h,
					 Pixel pixel) {
	x += Graphics_transX(g);
	y += Graphics_transY(g);
	skia_setClip(Get_Clip(g));
...
#ifndef SKIA_H
static void fillRect(Context currentContext, TCObject g, int32 x, int32 y, int32 width,
					 int32 height, Pixel pixel) {
...
#else
static void fillRect(Context currentContext, TCObject g, int32 x, int32 y, int32 w, int32 h,
					 Pixel pixel) {
	x += Graphics_transX(g);
	y += Graphics_transY(g);
	skia_setClip(Get_Clip(g));
...
static void drawText(Context currentContext, TCObject g, JCharP text, int32 chrCount, int32 x0,
					 int32 y0, Pixel foreColor, int32 justifyWidth) {
	...
	int32 rowWIB, offset, xMin, xMax, yMin, yMax, x, y, yDif, width, width0, height, spaceW = 0, k,
																					 clipX1, clipX2, clipY1, clipY2, pitch;
	...
	for (k = 0; k < chrCount; k++) { // guich@402
		...
		if (uf == null || ch < first || ch > last) {
			...
			if (uf == null || ch < uf->fontP.firstChar
				|| ch > uf->fontP.lastChar) { // invalid char - guich@tc122_23: must also check the font's range
...
#else
static void drawText(Context currentContext, TCObject g, JCharP text, int32 chrCount, int32 x,
					 int32 y, Pixel foreColor, int32 justifyWidth) {
	TCObject fontObj = Graphics_font(g);
	...
	y += Graphics_transY(g);
	skia_setClip(Get_Clip(g));
	skia_drawText(0, text, chrCount * sizeof(JChar), x, y + fontSize, foreColor | Graphics_alpha(g),
				  justifyWidth, fontSize, typefaceIndex);
	skia_restoreClip();

	markDirty(currentContext, g, x, y, skia_stringWidth(text, chrCount * sizeof(JChar), typefaceIndex,
			  fontSize), fontSize);
}
#endif
...
static SurfaceType getSurfaceType(Context currentContext, TCObject surface) {
	...
	return (surface != NULL
			&& areClassesCompatible(currentContext, OBJ_CLASS(surface),
									"totalcross.ui.image.Image") == 1) == COMPATIBLE ? SURF_IMAGE : SURF_CONTROL;
}

#ifndef SKIA_H
////////////////////////////////////////////////////////////////////////////
static void quadPixel(Context currentContext, TCObject g, int32 xc, int32 yc, int32 x, int32 y,
					  Pixel c) {
...
static void quadLine(Context currentContext, TCObject g, int32 xc, int32 yc, int32 x, int32 y,
					 Pixel c) {
...
static void ellipseDrawAndFill(Context currentContext, TCObject g, int32 xc, int32 yc, int32 rx,
							   int32 ry, Pixel pc1, Pixel pc2, bool fill, bool gradient) {
	int32 numSteps = 0, startRed = 0, startGreen = 0, startBlue = 0, endRed = 0, endGreen = 0,
		  endBlue = 0, redInc = 0, greenInc = 0, blueInc = 0, red = 0, green = 0, blue = 0;
	PixelConv c, c1, c2;
...
static void ellipseDrawAndFill(Context currentContext, TCObject g, int32 xc, int32 yc, int32 rx,
	...
	skia_setClip(Get_Clip(g));
	skia_ellipseDrawAndFill(0, xc, yc, rx, ry, pc1 | Graphics_alpha(g), pc2 | Graphics_alpha(g), fill,
							gradient);
	skia_restoreClip();
...
static void qsortInts(int32* items, int32 first, int32 last) {
	...
	while (true) {
		while (high >= low
			   && items[low] <
			   mid) { // guich@566_25: added "high > low" here and below - guich@568_5: changed to >=
			low++;
		}
...
static TCObject growIntArray(Context currentContext, TCObject oldArrayObj,
							 int32 newLen) { // must unlock the returned obj
...
static void fillPolygon(Context currentContext, TCObject g, int32* xPoints1, int32* yPoints1,
						int32 nPoints1, int32* xPoints2, int32* yPoints2, int32 nPoints2, int32 tx, int32 ty, Pixel c1,
						Pixel c2, bool gradient, bool isPie) {
	int32 x1, y1, x2, y2, y, n = 0, temp, i, j, miny, maxy, a, numSteps = 0, startRed = 0,
							 startGreen = 0, startBlue = 0, endRed = 0, endGreen = 0, endBlue = 0, redInc = 0, greenInc = 0,
							 blueInc = 0, red = 0, green = 0, blue = 0;
	int32* yp;
	...
	if (!gradient && (nPoints1 == 0
					  || isConvexAndInsideClip(g, tx, ty, xPoints1, yPoints1, nPoints1, isPie)) && (nPoints2 == 0
		...
		if (nPoints1 > 0) {
			glDrawLines(currentContext, g, xPoints1, yPoints1, nPoints1, tx + Graphics_transX(g),
						ty + Graphics_transY(g), c1, true);
		}
		if (nPoints2 > 0) {
			glDrawLines(currentContext, g, xPoints2, yPoints2, nPoints2, tx + Graphics_transX(g),
						ty + Graphics_transY(g), c1, true);
		}
		return;
	}
...
static void fillPolygon(Context currentContext, TCObject g, int32* xPoints1, int32* yPoints1,
						...
						Pixel c2, bool gradient, bool isPie) {
	skia_setClip(Get_Clip(g));
	skia_fillPolygon(0, xPoints1, yPoints1, nPoints1, Graphics_transX(g), Graphics_transY(g),
					 c1 | Graphics_alpha(g), c2 | Graphics_alpha(g), gradient, isPie);
	skia_restoreClip();

	// to avoid computing the polygon's bounds, we mark dirty the current clip
	markDirty(currentContext, g, Graphics_clipX1(g), Graphics_clipY1(g),
			  Graphics_clipX2(g) - Graphics_clipX1(g), Graphics_clipY2(g) - Graphics_clipY1(g));
}
#endif
...
#ifndef SKIA_H
static void drawPolygon(Context currentContext, TCObject g, int32* xPoints1, int32* yPoints1,
						int32 nPoints1, int32* xPoints2, int32* yPoints2, int32 nPoints2, int32 tx, int32 ty, Pixel pixel) {
	if (xPoints1 && yPoints1 && nPoints1 >= 2) {
#if defined __gl2_h_ && !defined WP8
		if (Graphics_useOpenGL(g) && (nPoints1 == 0
									  || isInsideClip(g, tx, ty, xPoints1, yPoints1, nPoints1)) && (nPoints2 == 0
											  || isInsideClip(g, tx, ty, xPoints2, yPoints2, nPoints2))) {
			if (nPoints1 > 0) {
				glDrawLines(currentContext, g, xPoints1, yPoints1, nPoints1, tx + Graphics_transX(g),
							ty + Graphics_transY(g), pixel, false);
			}
			if (nPoints2 > 0) {
				glDrawLines(currentContext, g, xPoints2, yPoints2, nPoints2, tx + Graphics_transX(g),
							ty + Graphics_transY(g), pixel, false);
...
#endif
		{
			...
			for (i = 1; i < nPoints1; i++) {
				drawLine(currentContext, g, tx + xPoints1[i - 1], ty + yPoints1[i - 1], tx + xPoints1[i],
						 ty + yPoints1[i], pixel);
			}
			for (i = 1; i < nPoints2; i++) {
				drawLine(currentContext, g, tx + xPoints2[i - 1], ty + yPoints2[i - 1], tx + xPoints2[i],
						 ty + yPoints2[i], pixel);
			}
		}
	}
...
static void drawPolygon(Context currentContext, TCObject g, int32* xPoints1, int32* yPoints1,
						int32 nPoints1, int32* xPoints2, int32* yPoints2, int32 nPoints2, int32 tx, int32 ty, Pixel pixel) {
	skia_setClip(Get_Clip(g));
	skia_drawPolygon(0, xPoints1, yPoints1, nPoints1, Graphics_transX(g), Graphics_transY(g),
...
#ifdef SKIA_H
static void arcPiePointDrawAndFill(Context currentContext, TCObject g, int32 xc, int32 yc, int32 rx,
								   int32 ry, double startAngle, double endAngle, Pixel c, Pixel c2, bool fill, bool pie,
								   bool gradient) {
	xc += Graphics_transX(g);
	yc += Graphics_transY(g);
	skia_setClip(Get_Clip(g));
	skia_arcPiePointDrawAndFill(0, xc, yc, rx, ry, startAngle, endAngle, c | Graphics_alpha(g),
								c2 | Graphics_alpha(g), fill, pie, gradient);
	skia_restoreClip();
...
static void arcPiePointDrawAndFill(Context currentContext, TCObject g, int32 xc, int32 yc, int32 rx,
	...
	double ppd;
	int32 startIndex, endIndex, index, i, nq, size = 0, oldX1 = 0, oldY1 = 0, last, oldX2 = 0,
											  oldY2 = 0;
	...
	int32 clipFactor = Graphics_minX(g) * 1000000000 + Graphics_maxX(g) * 10000000 + Graphics_minY(
						   g) * 100000 + Graphics_maxY(g);
	...
	if (!sameClipFactor || !sameR) {
		...
		if (xPoints == null
			|| ARRAYOBJ_LEN(*xPointsObj) != (uint32)
			size) { // guich@tc304: changed < to != to fix a glytch when drawing two pies with different radius
	...
	if (startIndex >
		endIndex) { // drawing from angle -30 to +30 ? (startIndex = 781, endIndex = 73, size=854)
		int p1 = last - startIndex;
		if (fill) {
			fillPolygon(currentContext, g, xPoints + startIndex, yPoints + startIndex, p1, xPoints, yPoints,
						endIndex, xc, yc, gradient ? c : c2, c2, gradient, true);    // lower half, upper half
		}
		if (!gradient) {
			drawPolygon(currentContext, g, xPoints + startIndex, yPoints + startIndex, p1 - 1, xPoints + 1,
						yPoints + 1, endIndex - 1, xc, yc, c);
		}
	} else {
		int32 arc = pie ? 0 : 1;
		if (fill) {
			fillPolygon(currentContext, g, xPoints + startIndex, yPoints + startIndex, endIndex - startIndex, 0,
						0, 0, xc, yc, gradient ? c : c2, c2, gradient, true);
		}
		if (!gradient) {
			drawPolygon(currentContext, g, xPoints + startIndex + arc, yPoints + startIndex + arc,
						endIndex - startIndex - arc, 0, 0, 0, xc, yc, c);
		}
	}
...
////////////////////////////////////////////////////////////////////////////
#ifndef SKIA_H
static void drawRoundRect(Context currentContext, TCObject g, int32 x, int32 y, int32 width,
						  int32 height, int32 r, Pixel c) {
...
#else
static void drawRoundRect(Context currentContext, TCObject g, int32 x, int32 y, int32 w, int32 h,
						  int32 r, Pixel c) {
	x += Graphics_transX(g);
	y += Graphics_transY(g);
	skia_setClip(Get_Clip(g));
...
#ifndef SKIA_H
////////////////////////////////////////////////////////////////////////////
static void setPixelA(Context currentContext, TCObject g, int32 x, int32 y, PixelConv color,
					  int32 alpha);

////////////////////////////////////////////////////////////////////////////
static void fillRoundRect(Context currentContext, TCObject g, int32 xx, int32 yy, int32 width,
						  int32 height, int32 r, Pixel c) {
	int32 px1, px2, py1, py2, xm, ym, x, y = 0, i, x2, e2, err;
	PixelConv color;
	if (r > (width / 2) || r > (height / 2)) {
		r = min32(width / 2, height / 2);    // guich@200b4_6: correct bug that crashed the device.
	}
...
#else
static void fillRoundRect(Context currentContext, TCObject g, int32 x, int32 y, int32 w, int32 h,
						  int32 r, Pixel c) {
	x += Graphics_transX(g);
	y += Graphics_transY(g);
	skia_setClip(Get_Clip(g));
	...
	*/
	...
	if ((*shiftY + *shiftH) < appHeightOnSipOpen) { // don't shift the screen if above
		*shiftY = 0;
		...
		int32 diffBetweenShiftAndAppHOSOpen = appHeightOnSipOpen - (*shiftY -
											  (screen.screenH - *shiftY)); // difference between shift and app height on sip open.
	...
	if ((*env)->GetStaticBooleanField(env, applicationClass, jhardwareKeyboardIsVisible)) {
		*shiftY = *shiftH = 0;
		...
		if (sipVisible && (screen.screenH > screen.screenW || *tcSettings.romVersionPtr >= 11)) {
			int32 appHeightOnSipOpen = screen.screenH - (*env)->CallStaticIntMethod(env, applicationClass,
									   jgetHeight);
...
static bool updateScreenBits(Context currentContext) { // copy the 888 pixels to the native format
	...
	if (screen.mainWindowPixels == null
		|| ARRAYOBJ_LEN(screen.mainWindowPixels) < (uint32)(screen.screenW * screen.screenH)) {
		return false;
	}
	...
	if (shiftYfield == null
		&& (window = loadClass(currentContext, "totalcross.ui.Window", false)) != null) {
	...
	if (!currentContext->fullDirty && shiftY != 0) { // *1* clip dirty Y values to screen shift area
		if (shiftY !=
			lastShiftY) { // the first time a shift is made, we must paint everything, to let the gray part be painted
			lastShiftY = shiftY;
			markWholeScreenDirty(currentContext);
		} else {
			...
			currentContext->dirtyY1 -= shiftY;
			currentContext->dirtyY2 = currentContext->dirtyY1 + min32(currentContext->dirtyY2 -
									  (currentContext->dirtyY1 + shiftY), shiftH);
		}
	}
	...
	if (screen.bpp == 16) {
		...
		if (currentContext->fullDirty
			&& IS_PITCH_OPTIMAL(screenW, screen.pitch,
								screen.bpp)) { // fairly common: the MainWindow is often fully repainted, and Palm OS and Windows always have pitch=width
			...
			PixelConv* f = ((PixelConv*)ARRAYOBJ_START(screen.mainWindowPixels)) +
						   (currentContext->dirtyY1 + shiftY) * screenW + currentContext->dirtyX1, *rowf, *pf;
			Pixel565* t = ((Pixel565*)BITMAP_PTR(screen.pixels, currentContext->dirtyY1,
												 screen.pitch)) + currentContext->dirtyX1, *rowt, *pt;
			for (pf = rowf = f, pt = rowt = t, y = currentContext->dirtyY1; y < currentContext->dirtyY2;
				 y++, pt = (rowt = (Pixel565*)(((uint8*)rowt) + screen.pitch)), pf = (rowf += screenW))
		...
		if (currentContext->fullDirty
			...
			PixelConv* f = ((PixelConv*)ARRAYOBJ_START(screen.mainWindowPixels)) +
						   (currentContext->dirtyY1 + shiftY) * screenW + currentContext->dirtyX1, *rowf, *pf;
			PixelPal* t = ((PixelPal*)BITMAP_PTR(screen.pixels, currentContext->dirtyY1,
												 screen.pitch)) + currentContext->dirtyX1, *rowt, *pt;
			for (pf = rowf = f, pt = rowt = t, y = currentContext->dirtyY1; y < currentContext->dirtyY2;
				 y++, pt = (rowt = (PixelPal*)(((uint8*)rowt) + screen.pitch)), pf = (rowf += screenW))
				if (shiftY != 0 && y >= shiftH) {
					if (currentContext->fullDirty) // draw gray area only if first time (full dirty, set above *1*)
						for (count = currentContext->dirtyX2 - currentContext->dirtyX1; count != 0; count--) {
							*pt++ = grayp;
...
#define SETPIXEL_32(p) SETPIXEL32((p)->r, (p)->g, (p)->b)
		...
		if (currentContext->fullDirty
			...
			PixelConv* f = ((PixelConv*)ARRAYOBJ_START(screen.mainWindowPixels)) +
						   (currentContext->dirtyY1 + shiftY) * screenW + currentContext->dirtyX1, *rowf, *pf = f;
			Pixel32* t = ((Pixel32*)BITMAP_PTR(screen.pixels, currentContext->dirtyY1,
											   screen.pitch)) + currentContext->dirtyX1, *rowt, *pt = t;
			for (pf = rowf = f, pt = rowt = t, y = currentContext->dirtyY1; y < currentContext->dirtyY2;
				 y++, pt = (rowt = (Pixel32*)(((uint8*)rowt) + screen.pitch)), pf = (rowf += screenW))
				if (shiftY != 0 && y >= shiftH) {
					if (currentContext->fullDirty) // draw gray area only if first time (full dirty, set above *1*)
						for (count = currentContext->dirtyX2 - currentContext->dirtyX1; count != 0; count--) {
							*pt++ = grayp;
		...
		if (currentContext->fullDirty
			...
			PixelConv* f = ((PixelConv*)ARRAYOBJ_START(screen.mainWindowPixels)) +
						   (currentContext->dirtyY1 + shiftY) * screenW + currentContext->dirtyX1, *rowf, *pf = f;
			Pixel24* t = ((Pixel24*)BITMAP_PTR(screen.pixels, currentContext->dirtyY1,
											   screen.pitch)) + currentContext->dirtyX1, *rowt, *pt = t;
			for (pf = rowf = f, pt = rowt = t, y = currentContext->dirtyY1; y < currentContext->dirtyY2;
				 y++, pt = (rowt = (Pixel24*)(((uint8*)rowt) + screen.pitch)), pf = (rowf += screenW))
...
static void drawFadedPixel(Context currentContext, TCObject g, int32 xx, int32 yy,
						   int32 c) { // guich@tc124_4
#ifdef __gl2_h_
	if (Graphics_useOpenGL(g)) {
...
static void drawRoundGradient(Context currentContext, TCObject g, int32 startX, int32 startY,
							  int32 endX, int32 endY, int32 topLeftRadius, int32 topRightRadius, int32 bottomLeftRadius,
							  int32 bottomRightRadius, int32 startColor, int32 endColor, bool vertical) {
	int32 numSteps = max32(1, vertical ? abs32(endY - startY) : abs32(endX -
						   startX)); // guich@tc110_11: support horizontal gradient - guich@gc114_41: prevent div by 0 if numsteps is 0
	...
	int32 clipX1 = Graphics_clipX1(g) - Graphics_transX(g),
		  clipY1 = Graphics_clipY1(g) - Graphics_transY(g), clipX2 = Graphics_clipX2(g) - Graphics_transX(g),
		  clipY2 = Graphics_clipY2(g) - Graphics_transY(g);
	bool optimize = Graphics_useOpenGL(g) && (startX >= clipX1 && startY >= clipY1 && endX < clipX2
					&& endY < clipY2) && topLeftRadius == topRightRadius && bottomLeftRadius == bottomRightRadius
	...
	for (i = 0; i < numSteps; i++) {
		...
		if (!optimize || leftOffset != 0 || rightOffset != 0) {
			if (vertical) {
				...
				if (drawFadedPixels
					&& rightOffset != 0) { // since there's no fading of pixels in opengl, we can safely ignore this
	...
	if (optimize) {
		...
		if (vertical) {
			glFillShadedRect(g, startX + tx, startY + ty + r - 1, endX - startX, endY - startY - r * 2 + 1, pf,
							 pi, false);
		} else {
			glFillShadedRect(g, startX + tx + r - 1, startY + ty, endX - startX - r * 2 + 1, endY - startY, pf,
							 pi, true);
		}
	}
#endif
...
static void drawRoundGradient(Context currentContext, TCObject g, int32 startX, int32 startY,
	...
	skia_setClip(Get_Clip(g));
	skia_drawRoundGradient(0, startX, startY, endX, endY, topLeftRadius, topRightRadius,
						   bottomLeftRadius, bottomRightRadius, startColor | Graphics_alpha(g), endColor | Graphics_alpha(g),
						   vertical);
	skia_restoreClip();
...
//#ifndef SKIA_H
#if 1
static int getsetRGB(Context currentContext, TCObject g, TCObject dataObj, int32 offset, int32 x,
					 int32 y, int32 w, int32 h, bool isGet) {
	if (dataObj == null) {
...
static void addError(PixelConv* pixel, int32 x, int32 y, int32 w, int32 h, int32 errR, int32 errG,
					 int32 errB, int32 j, int32 k) {
	int32 r, g, b;
	if (x >= w || y >= h || x < 0) {
		return;
	}
...
static bool createScreenSurface(Context currentContext, bool isScreenChange) {
	...
	if (graphicsCreateScreenSurface(&screen)) {
		TCObject* screenObj;
		screenObj = getStaticFieldObject(currentContext, loadClass(currentContext,
										 "totalcross.ui.gfx.Graphics", false), "mainWindowPixels");
#ifdef darwin // in darwin, the pixels buffer is pre-initialized and never changed
		if (controlEnableUpdateScreenPtr == null) {
			controlEnableUpdateScreenPtr = getStaticFieldInt(loadClass(currentContext, "totalcross.ui.Control",
										   false), "enableUpdateScreen");
		}
		ret = true;
#else
		...
		*screenObj = screen.mainWindowPixels = createArrayObject(currentContext, INT_ARRAY,
...
void updateScreen(Context currentContext) {
	...
	if (keepRunning && checkScreenPixels() && controlEnableUpdateScreenPtr
		&& *controlEnableUpdateScreenPtr && (currentContext->fullDirty
				|| (currentContext->dirtyX1 != screen.screenW && currentContext->dirtyX2 != 0
					&& currentContext->dirtyY1 != screen.screenH && currentContext->dirtyY2 != 0))) {
		if (updateScreenBits(currentContext)) { // move the temporary buffer to the real screen
#ifdef darwin
			UNLOCKVAR(
				screen); // without this, a deadlock can occur in iOS if the user minimizes the application, since another thread can trigger a markScreenDirty
#endif
