...
#ifndef __gl2_h_
		if (!currentContext->fullDirty && !Graphics_isImageSurface(g)) {
			markScreenDirty(currentContext, xMin, yMin, (x2>x1) ? (x2-x1) : (x1-x2), (y2>y1) ? (y2-y1) : (y1-y2));
		}
#else
		if (Graphics_isImageSurface(g)) {
			Image_changed(Graphics_surface(g)) = true;
...
static void arcPiePointDrawAndFill(Context currentContext, TCObject g, int32 xc, int32 yc, int32 rx, int32 ry, double startAngle, double endAngle, Pixel c, Pixel c2, bool fill, bool pie, bool gradient) {
	...
	if (!sameClipFactor || !sameR) {
		...
		if (pie) {
			xPoints++; yPoints++;
		}                      // make sure that startIndex-1 is at a valid pointer
	...
	if (startIndex > endIndex) { // drawing from angle -30 to +30 ? (startIndex = 781, endIndex = 73, size=854)
		...
		if (!gradient) {
			drawPolygon(currentContext, g, xPoints+startIndex, yPoints+startIndex, p1-1, xPoints+1, yPoints+1, endIndex-1, xc,yc, c);
		}
	} else {
		int32 arc = pie ? 0 : 1;
		if (fill) {
		...
		if (!gradient) {
			drawPolygon(currentContext, g, xPoints+startIndex+arc, yPoints+startIndex+arc, endIndex-startIndex-arc, 0,0,0, xc,yc, c);
		}
	}
...
static void fillWith8bppPalette(uint32* ptr) {
	...
	// color values  16 - 255
	for (r = 1; r <= R; r++) {
		rr = r * 256/R; if (rr > 255) {
			rr = 255;
		}
		for (g = 1; g <= G; g++) {
			gg = g * 256/G; if (gg > 255) {
				gg = 255;
			}
			for (b = 1; b <= B; b++) {
				bb = b * 256/B; if (bb > 255) {
					bb = 255;
				}
