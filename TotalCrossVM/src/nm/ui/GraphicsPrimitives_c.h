...
glClearClip();
void
glSetClip(int32 x1, int32 y1, int32 x2, int32 y2);
void
glDrawDots(int32 x1, int32 y1, int32 x2, int32 y2, int32 rgb1, int32 rgb2);
...
setupLookupBuffers()
{
  ...
  for (i = 0; i < 14; i++) {
    ftransp[i + 1] =
      (float)(i << 4) / (float)255; // make it lighter. since ftransp[0] is
                                    // never used, shift it to [1]
  }
  ftransp[15] = 1;
  for (i = 0; i <= 255; i++) {
...
/*
static void glDrawPixelG(TCObject g, int32 xx, int32 yy, int32 color, int32
alpha)
{
   xx += Graphics_transX(g);
   yy += Graphics_transY(g);
   if (Graphics_clipX1(g) <= xx && xx <= Graphics_clipX2(g) &&
Graphics_clipY1(g) <= yy && yy <= Graphics_clipY2(g))
...
bool
graphicsStartup(ScreenSurface screen, int16 appTczAttr);
bool
graphicsCreateScreenSurface(ScreenSurface screen);
void
graphicsUpdateScreen(Context currentContext, ScreenSurface screen);
void
graphicsDestroy(ScreenSurface screen, bool isScreenChange);
#if !defined(NO_GRAPHICS_LOCK_NEEDED)
bool
graphicsLock(ScreenSurface screen, bool on);
#endif
void
graphicsDestroyPrimitives();

static bool
createScreenSurface(Context currentContext, bool isScreenChange);

void
updateScreen(Context currentContext);
void
privateScreenChange(int32 w, int32 h);
void
markWholeScreenDirty(Context currentContext);
static bool
translateAndClip(TCObject g,
                 int32* pX,
                 int32* pY,
                 int32* pWidth,
                 int32* pHeight);
...
#define Get_Clip(g)                                                            \
  Graphics_clipX1(g), Graphics_clipY1(g), Graphics_clipX2(g), Graphics_clipY2(g)

// >>>>>>>>>
// DO NOT LOCK THE SCREEN ON THESE METHODS. THE CALLER MUST DO THAT.
static Pixel*
getSurfacePixels(TCObject surf)
{
  TCObject pix;
  bool isImage = false;
  if (surf != null) {
    ...
    if (name[0] == 't' &&
        strEq(name,
              "totalcross.ui.gfx.Graphics")) { // if the surface is a Graphics,
...
void
repaintActiveWindows(Context currentContext)
{
  static Method repaintActiveWindows;
  if (repaintActiveWindows == null && mainClass != null) {
    repaintActiveWindows =
      getMethod(OBJ_CLASS(mainClass), true, "repaintActiveWindows", 0);
  }
...
void
screenChange(Context currentContext,
             int32 newWidth,
             int32 newHeight,
             int32 hRes,
             int32 vRes,
...
{ // rotate the screen
  ...
  if (mainClass != null) {
    postEvent(currentContext,
              KEYEVENT_SPECIALKEY_PRESS,
              SK_SCREEN_CHANGE,
              0,
              0,
              -1); // XXX
...
Pixel
makePixelA(int32 a, int32 r, int32 g, int32 b)
{
  PixelConv p;
  p.a = (uint8)(a & 0xFF);
  p.r = (uint8)(r & 0xFF);
...
Pixel
makePixel(int32 r, int32 g, int32 b)
{
  PixelConv p;
  p.a = 0xFF;
  p.r = (uint8)(r & 0xFF);
...
Pixel
makePixelRGB(int32 rgb)
{ // from Java's big endian to native format
  PixelConv p;
  p.a = 0xFF;
...
static void
markScreenDirty(Context currentContext, int32 x, int32 y, int32 w, int32 h)
{
  LOCKVAR(screen);
  if (w > 0 && h > 0 && x < screen.screenW && y < screen.screenH) {
    ...
    x2 = x + w;
    y2 = y + h;
    if (x2 > screen.screenW) {
      x2 = screen.screenW;
    }
    if (y2 > screen.screenH) {
      y2 = screen.screenH;
    }
    ...
    } else if (currentContext->dirtyX2 > x2) {
    ...
    } else if (currentContext->dirtyY2 > y2) {
...
#define LOGD(...)                                                              \
  __android_log_print(ANDROID_LOG_DEBUG, "TotalCross", __VA_ARGS__)
#else
#define LOGD(...) debug(__VA_ARGS__)
#endif

void
markDirty(Context currentContext, TCObject surface, int x, int y, int w, int h)
{
...
// This is the main routine that draws a surface (a Control or an Image) in the
// destination GfxSurface. Destination is always a Graphics object.
#ifndef SKIA_H
static void
drawSurface(Context currentContext,
            TCObject dstSurf,
            TCObject srcSurf,
            int32 srcX,
            int32 srcY,
            ...
            int32 doClip)
{
...
#ifdef __gl2_h_ // for opengl, we will use the smoothScaled only if we will draw
                // on an image. for win32, we will always use smoothScale
    bool forcedSmoothScale = Graphics_isImageSurface(
      dstSurf); // the destination is always a Graphics object
#else
    bool forcedSmoothScale = true;
#endif
    srcPitch = srcWidth =
      (int32)(Image_width(srcSurf) * Image_hwScaleW(srcSurf));
    srcHeight = (int32)(Image_height(srcSurf) * Image_hwScaleH(srcSurf));
    if (forcedSmoothScale &&
        (Image_hwScaleW(srcSurf) != 1 || Image_hwScaleH(srcSurf) != 1)) {
      static Method mGetScaledInstance, mGetSmoothScaledInstance;
      TCObject newSurf;
      if (mGetScaledInstance == null) {
        mGetScaledInstance = getMethod(
          OBJ_CLASS(srcSurf), false, "getScaledInstance", 2, J_INT, J_INT);
        mGetSmoothScaledInstance = getMethod(OBJ_CLASS(srcSurf),
                                             false,
      ...
      disableGC =
        true; // the gc may collect the image before we lock it here (*)
      newSurf =
        executeMethod(currentContext,
                      Image_hwScaleW(srcSurf) < 1 && Image_hwScaleH(srcSurf) < 1
  ...
  if (!doClip) {
    ...
    */
    if (srcX <= -width || srcX >= srcWidth || srcY <= -height ||
        srcY >= srcHeight) {
      goto end;
    }
  ...
  if (isSrcScreen) {
  ...
  } else if (Graphics_useOpenGL(dstSurf)) {
    int32 fc;
    int frame;
    ...
    /*img*/ glDrawTexture(Image_textureId(srcSurf),
                          srcX + frame * srcPitch,
                          srcY,
                          width,
                          height,
                          ...
                          0,
                          0,
                          fc > 1 ? (int32)(Image_widthOfAllFrames(srcSurf) *
                                           Image_hwScaleW(srcSurf))
                                 : srcWidth,
                          srcHeight,
                          null,
                          alphaMask);
...
#endif
    for (i = 0; i < (uint32)height;
         i++) { // in opengl, only case of image drawing on image
...
#ifndef __gl2_h_
  if (!currentContext->fullDirty && !Graphics_isImageSurface(dstSurf)) {
...
static int32
getPixel(TCObject g, int32 x, int32 y)
{
  int32 ret = -1;
  x += Graphics_transX(g);
  y += Graphics_transY(g);
  if (Graphics_clipX1(g) <= x && x < Graphics_clipX2(g) &&
      Graphics_clipY1(g) <= y && y < Graphics_clipY2(g)) {
    PixelConv p;
#ifdef SKIA_H
...
#ifndef SKIA_H
static PixelConv
getPixelConv(TCObject g, int32 x, int32 y)
{
  PixelConv p;
  p.pixel = -1;
  x += Graphics_transX(g);
...
#ifndef SKIA_H
static void
setPixel(Context currentContext, TCObject g, int32 x, int32 y, Pixel pixel)
{
  x += Graphics_transX(g);
  y += Graphics_transY(g);
  if (Graphics_clipX1(g) <= x && x < Graphics_clipX2(g) &&
...
#endif
    {
      ...
      if (!currentContext->fullDirty && !Graphics_isImageSurface(g)) {
        markScreenDirty(currentContext, x, y, 1, 1);
      }
    }
  }
}
...
static PixelConv
interpolatePC(PixelConv c, PixelConv d, int32 factor)
{
  int m = 255 - factor;
...
static int32
interpolate(PixelConv c, PixelConv d, int32 factor)
{
  return interpolatePC(c, d, factor).pixel;
}

static bool
surelyOutsideClip(TCObject g, int32 x1, int32 y1, int32 x2, int32 y2)
{
...
static void
drawHLine(Context currentContext,
          TCObject g,
          int32 x,
          int32 y,
          int32 width,
          ...
          Pixel pixel2)
{
  ...
  */
  if (Graphics_clipY1(g) <= y && y < Graphics_clipY2(g) &&
      Graphics_clipX1(g) <= (x + width) && x < Graphics_clipX2(g)) { // NOPT
    Pixel* pTgt;
    ...
    if (Graphics_useOpenGL(g)) {
      ...
      } else if (checkGLfloatBuffer(currentContext, width / 2 + 2)) {
        float* xya = glXYA;
        int32 xx, ww, nn = 0;
...
#endif
    {
      ...
      if (!currentContext->fullDirty && !Graphics_isImageSurface(g)) {
        markScreenDirty(currentContext, x, y, width, 1);
      }
...
static void
drawVLine(Context currentContext,
          TCObject g,
          int32 x,
          int32 y,
          int32 height,
          ...
          Pixel pixel2)
{
  ...
  */
  if (Graphics_clipX1(g) <= x && x < Graphics_clipX2(g) &&
      Graphics_clipY1(g) <= (y + height) && y < Graphics_clipY2(g)) { // NOPT
    Pixel* pTgt;
    ...
    if (Graphics_useOpenGL(g)) {
      ...
      } else if (checkGLfloatBuffer(currentContext, height / 2 + 2)) {
        float* xya = glXYA;
        int32 yy, hh, nn = 0;
...
#endif
    {
      ...
      if (!currentContext->fullDirty && !Graphics_isImageSurface(g)) {
        markScreenDirty(currentContext, x, y, 1, height);
      }
    }
  }
}
...
static void
drawDottedLine(Context currentContext,
               TCObject g,
               int32 x1,
               int32 y1,
               int32 x2,
               ...
               Pixel pixel2)
{
  ...
  if (dX == 0 && dY == 0) {
    ...
    drawHLine(
      currentContext, g, min32(x1, x2), min32(y1, y2), dX + 1, pixel1, pixel2);
  } else if (dX == 0) { // vertical line?
    drawVLine(
      currentContext, g, min32(x1, x2), min32(y1, y2), dY + 1, pixel1, pixel2);
  } else { // guich@566_43: removed the use of drawH/VLine to make sure that it
           // will draw the same of desktop
    int32 currentX, currentY; // saved for later markScreenDirty
    Pixel* row;
    int32 on = 1;
    int32 clipX1 = Graphics_clipX1(g), clipY1 = Graphics_clipY1(g),
          clipX2 = Graphics_clipX2(g), clipY2 = Graphics_clipY2(g);
    bool dontClip = true; // the most common will be draw lines that do not
                          // cross the clip bounds, so we may speedup a little

    xMin += tX;
    yMin += tY;

    if (xMin < clipX1 || (xMin + dX) >= clipX2 || yMin < clipY1 ||
        (yMin + dY) >= clipY2) {
      dontClip = false;
    }
    ...
    row =
      getGraphicsPixels(g) +
      (pyInc > 0 ? yMin : (yMin + dY)) * Graphics_pitch(g) +
      (xInc > 0 ? xMin
                : (xMin + dX)); // currentX    | else start from the max value

    yInc = pyInc > 0 ? Graphics_pitch(g)
                     : -Graphics_pitch(g); // now that we have active, correctly
                                           // assign the y incrementing value
    ...
    if (dX >= dY) { // if X is the independent variable
      ...
      if (pixel1 == pixel2)     // quick optimization
        for (; dX >= 0; dX--) { // process each point in the line one at a time
                                // (just use dX)
          if (dontClip || (clipX1 <= currentX && currentX < clipX2 &&
                           clipY1 <= currentY && currentY < clipY2)) {
#ifdef __gl2_h_
            if (Graphics_useOpenGL(g)) {
        ...
        for (; dX >= 0; dX--) { // process each point in the line one at a time
          ...
          if (dontClip || (clipX1 <= currentX && currentX < clipX2 &&
            ...
            if (Graphics_useOpenGL(g)) {
              glDrawPixel(
                currentX, currentY, (on++ & 1) ? pixel1 : pixel2, 255);
            } else
#endif
...
#ifndef __gl2_h_
    if (!currentContext->fullDirty && !Graphics_isImageSurface(g)) {
      markScreenDirty(currentContext,
                      xMin,
                      yMin,
                      (x2 > x1) ? (x2 - x1) : (x1 - x2),
                      (y2 > y1) ? (y2 - y1) : (y1 - y2));
               ...
               Pixel pixel2)
{
  ...
  skia_setClip(Get_Clip(g));
  skia_drawDottedLine(
    0, x1, y1, x2, y2, pixel1 | Graphics_alpha(g), pixel2 | Graphics_alpha(g));
  skia_restoreClip();

  markDirty(currentContext,
            g,
            min32(x1, x2),
            min32(y1, y2),
            abs(x2 - x1),
...
static void
drawLine(Context currentContext,
         TCObject g,
         int32 x1,
         int32 y1,
         int32 x2,
...
#ifndef SKIA_H
static void
drawRect(Context currentContext,
         TCObject g,
         int32 x,
         int32 y,
         int32 width,
...
#ifndef SKIA_H
static void
fillRect(Context currentContext,
         TCObject g,
         int32 x,
         int32 y,
         int32 width,
...
// Darkens the screen
static void
fadeScreen(Context currentContext, int32 amount)
{
...
uint8*
getResizedCharPixels(Context currentContext,
                     UserFont uf,
                     JChar ch,
                     int32 w,
                     int32 h);
...
static void
drawText(Context currentContext,
         TCObject g,
         JCharP text,
         int32 chrCount,
         int32 x0,
         ...
         int32 justifyWidth)
{
  ...
  int32 rowWIB, offset, xMin, xMax, yMin, yMax, x, y, yDif, width, width0,
    height, spaceW = 0, k, clipX1, clipX2, clipY1, clipY2, pitch;
  ...
  if (!text || chrCount == 0 || fontObj == null) {
    return;
  }
  ...
  if (justifyWidth > 0) {
    ...
    rem =
      justifyWidth - getJCharPWidth(currentContext, fontObj, text, chrCount);
    if (rem > 0) {
  ...
  for (k = 0; k < chrCount; k++) { // guich@402
    ...
    if (uf == null || ch < first || ch > last) {
      ...
      if (uf == null || ch < uf->fontP.firstChar ||
          ch > uf->fontP.lastChar) { // invalid char - guich@tc122_23: must also
                                     // check the font's range
        x0 += spaceW ? spaceW
                     : (spaceW = getJCharWidth(currentContext, fontObj, ' ')) +
                         extraPixelsPerChar;
        if (k <= extraPixelsRemaining) {
          x0++;
        }
...
#ifdef __gl2_h_
    if (!checkGLfloatBuffer(currentContext,
                            uf->fontP.maxHeight * uf->fontP.maxWidth)) {
      return;
    }
#endif
    ...
    switch (aaType) {
      case AA_NO: {
        ...
        if (isGL) {
          ...
          xya = glXYA;
          for (; r < rmax;
               start += rowWIB, r++, row += pitch, y++) { // draw each row
            current = start;
...
#endif
          for (row = row0; r < rmax;
               start += rowWIB, r++, row += pitch) { // draw each row
            current = start;
            ands = ands8 + (currentBit = startBit);
            for (x = x0; x < xMax; x++) {
              if ((*current & *ands++) != 0 && x >= xMin) {
      ...
      case AA_4BPP: {
        ...
        if (isGL) {
          ...
          for (; r < rmax; start += rowWIB, r++, y++) { // draw each row
            ...
            for (x = x0; x < xMax; x++) {
              transparency =
                isLowNibble ? (*current++ & 0xF) : ((*current >> 4) & 0xF);
      ...
      case AA_8BPP: { // textured font files
        ...
        if (isGL) {
          if (!isClipped &&
              getCharPosInTexture(currentContext, uf->ubase, ch, charXY))
            /*text*/
            glDrawTexture(uf->ubase->textureId,
                          charXY[0],
                          charXY[1],
                          width0,
                          uf->ubase->fontP.maxHeight, // source char position
                          x0,
                          y,
                          width,
                          height, // target bitmap position
                          uf->ubase->maxW,
                          uf->ubase->maxH,
                          &fc,
                          255); // total bitmap size
          else {
            uint8* alpha = getResizedCharPixels(
              currentContext, uf->ubase, ch, width + diffW, height);
            if (alpha) {
              int32 nn = 0;
              rowWIB = width + diffW;
...
#ifndef __gl2_h_
  if (!currentContext->fullDirty && !Graphics_isImageSurface(g)) {
    markScreenDirty(currentContext, xMin, yMin, (xMax - xMin), (yMax - yMin));
  }
#else
  if (Graphics_isImageSurface(g)) {
    Image_changed(Graphics_surface(g)) = true;
         ...
         int32 justifyWidth)
{
  ...
  skia_drawText(0,
                text,
                chrCount * sizeof(JChar),
                x,
                y + fontSize,
                foreColor | Graphics_alpha(g),
                justifyWidth,
                fontSize,
                typefaceIndex);
  skia_restoreClip();
  ...
  markDirty(
    ...
    x,
    y,
    skia_stringWidth(text, chrCount * sizeof(JChar), typefaceIndex, fontSize),
    fontSize);
}
#endif

static SurfaceType
getSurfaceType(Context currentContext, TCObject surface)
{
  // cache class pointers for performance
  return (surface != NULL &&
          areClassesCompatible(currentContext,
                               OBJ_CLASS(surface),
                               "totalcross.ui.image.Image") == 1) == COMPATIBLE
           ? SURF_IMAGE
           : SURF_CONTROL;
}
...
////////////////////////////////////////////////////////////////////////////
static void
quadPixel(Context currentContext,
          TCObject g,
          int32 xc,
          int32 yc,
          int32 x,
...
static void
quadLine(Context currentContext,
         TCObject g,
         int32 xc,
         int32 yc,
         int32 x,
...
static void
ellipseDrawAndFill(Context currentContext,
                   TCObject g,
                   int32 xc,
                   int32 yc,
                   int32 rx,
                   ...
                   bool gradient)
{
  int32 numSteps = 0, startRed = 0, startGreen = 0, startBlue = 0, endRed = 0,
                   ...
                   bool gradient)
{
  ...
  skia_setClip(Get_Clip(g));
  skia_ellipseDrawAndFill(0,
                          xc,
                          yc,
                          rx,
                          ry,
                          pc1 | Graphics_alpha(g),
                          pc2 | Graphics_alpha(g),
                          fill,
                          gradient);
  skia_restoreClip();
...
static void
qsortInts(int32* items, int32 first, int32 last)
{
  int32 low = first;
  int32 high = last, mid;
  if (first >= last) {
  ...
  while (true) {
    while (high >= low &&
           items[low] < mid) { // guich@566_25: added "high > low" here and
                               // below - guich@568_5: changed to >=
      low++;
    }
...
static TCObject
growIntArray(Context currentContext, TCObject oldArrayObj, int32 newLen)
{ // must unlock the returned obj
...
static void
fillPolygon(Context currentContext,
            TCObject g,
            int32* xPoints1,
            int32* yPoints1,
            int32 nPoints1,
...
{
  int32 x1, y1, x2, y2, y,
    n = 0, temp, i, j, miny, maxy, a, numSteps = 0, startRed = 0,
    startGreen = 0, startBlue = 0, endRed = 0, endGreen = 0, endBlue = 0,
    redInc = 0, greenInc = 0, blueInc = 0, red = 0, green = 0, blue = 0;
  int32* yp;
  ...
  if (!gradient &&
      (nPoints1 == 0 ||
       isConvexAndInsideClip(g, tx, ty, xPoints1, yPoints1, nPoints1, isPie)) &&
      (nPoints2 == 0 ||
       isConvexAndInsideClip(g, tx, ty, xPoints2, yPoints2, nPoints2, isPie)) &&
      Graphics_useOpenGL(g)) { // opengl doesnt fills non-convex polygons well
    if (nPoints1 > 0) {
      glDrawLines(currentContext,
                  g,
                  xPoints1,
                  yPoints1,
                  nPoints1,
  ...
  if (ints == null) {
    *intsObj = createArrayObject(currentContext,
                                 INT_ARRAY,
                                 2); // 2 is the most used length
    if (*intsObj == null) {
      return;
    }
  ...
  for (y = miny; y <= maxy; y++) {
    ...
    for (a = 0; a < 2; a++) {
      ...
      for (i = 0; i < nPoints; j = i, i++) {
        ...
        if ((y1 <= y && y < y2) || (y == maxy && y1 < y && y <= y2)) {
          if (n ==
              (int32)ARRAYOBJ_LEN(*intsObj)) { // have to grow the ints array?
    ...
    if (n >= 2) {
      ...
      if (n == 2) { // most of the times
        if (ints[1] > ints[0]) {
          drawHLine(
            currentContext, g, ints[0], y, ints[1] - ints[0], c.pixel, c.pixel);
        } else {
          drawHLine(
            currentContext, g, ints[1], y, ints[0] - ints[1], c.pixel, c.pixel);
        }
      } else {
        qsortInts(ints, 0, n - 1);
        for (n >>= 1, yp = ints; --n >= 0; yp += 2) {
          drawHLine(
            currentContext, g, yp[0], y, yp[1] - yp[0], c.pixel, c.pixel);
        }
      }
    }
  }
            ...
            bool isPie)
{
  ...
  markDirty(currentContext,
            ...
            Graphics_clipX1(g),
            Graphics_clipY1(g),
            Graphics_clipX2(g) - Graphics_clipX1(g),
            Graphics_clipY2(g) - Graphics_clipY1(g));
}
#endif
...
#ifndef SKIA_H
static void
drawPolygon(Context currentContext,
            TCObject g,
            int32* xPoints1,
            int32* yPoints1,
            int32 nPoints1,
            ...
            Pixel pixel)
{
  if (xPoints1 && yPoints1 && nPoints1 >= 2) {
    ...
    if (Graphics_useOpenGL(g) &&
        (nPoints1 == 0 ||
         isInsideClip(g, tx, ty, xPoints1, yPoints1, nPoints1)) &&
        (nPoints2 == 0 ||
         isInsideClip(g, tx, ty, xPoints2, yPoints2, nPoints2))) {
      if (nPoints1 > 0) {
        glDrawLines(currentContext,
                    g,
                    xPoints1,
...
#ifdef SKIA_H
static void
arcPiePointDrawAndFill(Context currentContext,
                       TCObject g,
                       int32 xc,
                       int32 yc,
                       int32 rx,
                       ...
                       bool gradient)
{
  ...
  double ppd;
  int32 startIndex, endIndex, index, i, nq, size = 0, oldX1 = 0, oldY1 = 0,
                                            last, oldX2 = 0, oldY2 = 0;
  ...
  int32 clipFactor = Graphics_minX(g) * 1000000000 +
                     Graphics_maxX(g) * 10000000 + Graphics_minY(g) * 100000 +
                     Graphics_maxY(g);
  ...
  if (startAngle < 0.1 &&
    ...
    if (fill) {
      ellipseDrawAndFill(
        currentContext, g, xc, yc, rx, ry, c, c2, true, gradient);
    }
    ellipseDrawAndFill(
      currentContext, g, xc, yc, rx, ry, c, c, false, gradient);
    return;
  }
  ...
  if (!sameClipFactor || !sameR) {
    // step 1: computes how many points the circle has (computes only 45 degrees
    // and mirrors the rest) intermediate terms to speed up loop
    int64 t1 = (int64)rx * (int64)rx, t2 = t1 << 1, t3 = t2 << 1;
    int64 t4 = (int64)ry * (int64)ry, t5 = t4 << 1, t6 = t5 << 1;
    int64 t7 = (int64)rx * t5, t8 = t7 << 1, t9 = 0L;
    int64 d1 = t2 - t7 + (t4 >> 1); // error terms
    ...
    if (xPoints == null ||
        ARRAYOBJ_LEN(*xPointsObj) !=
          (uint32)size) { // guich@tc304: changed < to != to fix a glytch when
                          // drawing two pies with different radius
      *xPointsObj =
        createArrayObject(currentContext, INT_ARRAY, max32(3, size));
      if (*xPointsObj == null) {
        return;
      }
      *yPointsObj =
    ...
    if (pie) {
      xPoints++; // make sure that startIndex-1 is at a valid pointer
      yPoints++;
    }
  ...
  if (startIndex > endIndex) { // drawing from angle -30 to +30 ? (startIndex =
                               // 781, endIndex = 73, size=854)
    int p1 = last - startIndex;
    if (fill) {
      fillPolygon(currentContext,
                  g,
                  xPoints + startIndex,
                  yPoints + startIndex,
                  p1,
                  ...
                  c2,
                  gradient,
                  true); // lower half, upper half
    }
    if (!gradient) {
      drawPolygon(currentContext,
                  g,
                  xPoints + startIndex,
                  yPoints + startIndex,
                  p1 - 1,
    ...
    if (!gradient) {
      drawPolygon(currentContext,
                  g,
                  xPoints + startIndex + arc,
                  yPoints + startIndex + arc,
                  endIndex - startIndex - arc,
                  0,
                  0,
                  0,
                  xc,
  ...
  if (pie) { // restore saved points
    ...
    if (!gradient && endAngle == 360) {
      drawLine(currentContext,
               ...
               xc,
               yc,
               xc + xPoints[endIndex - 1],
               yc + yPoints[endIndex - 1],
               c);
    }
#endif
  }
...
#ifndef SKIA_H
static void
drawRoundRect(Context currentContext,
              TCObject g,
              int32 x,
              int32 y,
              int32 width,
...
////////////////////////////////////////////////////////////////////////////
static void
setPixelA(Context currentContext,
          TCObject g,
          int32 x,
          int32 y,
          PixelConv color,
...
////////////////////////////////////////////////////////////////////////////
static void
fillRoundRect(Context currentContext,
              TCObject g,
              int32 xx,
              int32 yy,
              int32 width,
              ...
              Pixel c)
{
  ...
  if (r > (width / 2) || r > (height / 2)) {
    r =
      min32(width / 2,
            height / 2); // guich@200b4_6: correct bug that crashed the device.
  }
  ...
  do {
    ...
    if (i < 256 && i > 0) {
      xm = px2;
      ym = py2;
      setPixelA(currentContext, g, xm - x, ym + y, color, i); // br
      xm = px1;
      ym = py2;
      setPixelA(currentContext, g, xm - y, ym - x, color, i); // bl
      xm = px1;
      ym = py1;
      setPixelA(currentContext, g, xm + x, ym - y, color, i); // tl
      xm = px2;
      ym = py1;
      setPixelA(currentContext, g, xm + y, ym + x, color, i); // tr
    }
    e2 = err;
    x2 = x;
    if (err + y > 0) {
      ...
      if (i < 256 && i > 0) {
        xm = px2;
        ym = py2;
        setPixelA(currentContext, g, xm - x, ym + y + 1, color, i);
        xm = px1;
        ym = py2;
        setPixelA(currentContext, g, xm - y - 1, ym - x, color, i);
        xm = px1;
        ym = py1;
        setPixelA(currentContext, g, xm + x, ym - y - 1, color, i);
        xm = px2;
    ...
    if (e2 + x2 <= 0) {
      ...
      if (i < 256 && i > 0) {
        xm = px2;
        ym = py2;
        setPixelA(currentContext, g, xm - x2 - 1, ym + y, color, i);
        xm = px1;
        ym = py2;
        setPixelA(currentContext, g, xm - y, ym - x2 - 1, color, i);
        xm = px1;
        ym = py1;
        setPixelA(currentContext, g, xm + x2 + 1, ym - y, color, i);
        xm = px2;
...
static void
createGfxSurface(int32 w, int32 h, TCObject g, SurfaceType stype)
{
...
int32 *shiftYfield, *shiftHfield, *lastShiftYfield, *needsPaint, lastShiftY,
  screenY = 0;
#ifndef SKIA_H
#define BITMAP_PTR(p, dline, pitch) (((uint8*)p) + (dline * pitch))
#define IS_PITCH_OPTIMAL(w, pitch, bpp)                                        \
  (((uint32)w * (uint32)bpp / 8) == (uint32)pitch) // 240 * 32 / 8 == 960 ?
...
static void
checkKeyboardAndSIP(Context currentContext, int32* shiftY, int32* shiftH)
{
  ...
  */
  ...
  if ((*shiftY + *shiftH) <
      appHeightOnSipOpen) { // don't shift the screen if above
    *shiftY = 0;
  } else {
    int32 diffBetweenShiftAndAppHOSOpen =
      appHeightOnSipOpen -
      (*shiftY -
       (screen.screenH -
        *shiftY)); // difference between shift and app height on sip open.
    *shiftY += diffBetweenShiftAndAppHOSOpen -
               *shiftH; // add remaining space between sip and component
  }
  /*
     if (oldShiftY != *shiftY)
     {
...
checkKeyboardAndSIP(Context currentContext, int32* shiftY, int32* shiftH)
{
  ...
  if ((*env)->GetStaticBooleanField(
        env, applicationClass, jhardwareKeyboardIsVisible)) {
    *shiftY = *shiftH = 0;
  } else {
    bool sipVisible =
      (*env)->GetStaticBooleanField(env, applicationClass, jsipVisible);
    // we know the height if:
    // 1. Portrait and Android 2.x
    // 2. Portrait/landscape and Android 3.x
    if (sipVisible &&
        (screen.screenH > screen.screenW || *tcSettings.romVersionPtr >= 11)) {
      int32 appHeightOnSipOpen =
        screen.screenH -
        (*env)->CallStaticIntMethod(env, applicationClass, jgetHeight);
      int32 appTitleH =
        (*env)->GetStaticIntField(env, applicationClass, jappTitleH);
      // when application is in full screen, this function would erase a
      // possibly valid shiftY value; so, here i store the desired shiftY and
      // restore it when the application is really not full screen
      bool isFullScreen =
        appTitleH != 0 && (appTitleH + appHeightOnSipOpen) == screen.screenH;
      if (!isFullScreen && desiredShiftY != -1) {
        *shiftY = desiredShiftY;
        desiredShiftY = -1;
      } else if (isFullScreen && desiredShiftY == -1) {
...
void
setShiftYgl(int32 shiftY);

#ifndef SKIA_H
// not used with opengl
static bool
updateScreenBits(Context currentContext)
{ // copy the 888 pixels to the native format
  ...
  if (screen.mainWindowPixels == null ||
      ARRAYOBJ_LEN(screen.mainWindowPixels) <
        (uint32)(screen.screenW * screen.screenH)) {
    return false;
  }
  ...
  if (shiftYfield == null &&
      (window = loadClass(currentContext, "totalcross.ui.Window", false)) !=
        null) {
  ...
  if (!currentContext->fullDirty &&
      shiftY != 0) {            // *1* clip dirty Y values to screen shift area
    if (shiftY != lastShiftY) { // the first time a shift is made, we must paint
                                // everything, to let the gray part be painted
      lastShiftY = shiftY;
      markWholeScreenDirty(currentContext);
    } else {
      if (currentContext->dirtyY1 < shiftY) {
        currentContext->dirtyY1 = shiftY;
      }
      if (currentContext->dirtyY2 >= (shiftY + shiftH)) {
        currentContext->dirtyY2 = shiftY + shiftH;
      }
      currentContext->dirtyY1 -= shiftY;
      currentContext->dirtyY2 =
        currentContext->dirtyY1 +
        min32(currentContext->dirtyY2 - (currentContext->dirtyY1 + shiftY),
              shiftH);
    }
  }
...
#ifdef __gl2_h_
  desiredScreenShiftY =
    shiftY; // will be set with glScreenShiftY in updateScreen
#else
  ...
  if (screen.bpp == 16) {
    ...
    if (currentContext->fullDirty &&
        IS_PITCH_OPTIMAL(screenW,
                         screen.pitch,
                         screen.bpp)) { // fairly common: the MainWindow is
                                        // often fully repainted, and Palm OS
                                        // and Windows always have pitch=width
      ...
      if (shiftY == 0)
        ...
        for (count = shiftH * screenW, f += shiftY * screenW; count != 0;
             f++, count--)
#if defined(WIN32) && !defined(WP8)
          SETPIXEL565_(t, f->pixel)
#else
      ...
      PixelConv *f = ((PixelConv*)ARRAYOBJ_START(screen.mainWindowPixels)) +
                     (currentContext->dirtyY1 + shiftY) * screenW +
                     currentContext->dirtyX1,
                *rowf, *pf;
      Pixel565 *t = ((Pixel565*)BITMAP_PTR(
                      screen.pixels, currentContext->dirtyY1, screen.pitch)) +
                    currentContext->dirtyX1,
               *rowt, *pt;
      for (pf = rowf = f, pt = rowt = t, y = currentContext->dirtyY1;
           y < currentContext->dirtyY2;
           y++,
          pt = (rowt = (Pixel565*)(((uint8*)rowt) + screen.pitch)),
          pf = (rowf += screenW))
        if (shiftY != 0 && y >= shiftH) {
          if (currentContext->fullDirty) // draw gray area only if first time
                                         // (full dirty, set above *1*)
            for (count = currentContext->dirtyX2 - currentContext->dirtyX1;
                 count != 0;
                 count--) {
              *pt++ = grayp;
            }
    ...
    if (currentContext->fullDirty &&
      ...
      PixelConv *f = ((PixelConv*)ARRAYOBJ_START(screen.mainWindowPixels)) +
                     ...
                     currentContext->dirtyX1,
                *rowf, *pf;
      PixelPal *t = ((PixelPal*)BITMAP_PTR(
                      screen.pixels, currentContext->dirtyY1, screen.pitch)) +
                    currentContext->dirtyX1,
               *rowt, *pt;
      for (pf = rowf = f, pt = rowt = t, y = currentContext->dirtyY1;
           y < currentContext->dirtyY2;
           y++,
          pt = (rowt = (PixelPal*)(((uint8*)rowt) + screen.pitch)),
          pf = (rowf += screenW))
        if (shiftY != 0 && y >= shiftH) {
          if (currentContext->fullDirty) // draw gray area only if first time
                                         // (full dirty, set above *1*)
...
#define SETPIXEL_32(p) SETPIXEL32((p)->r, (p)->g, (p)->b)
    ...
    if (currentContext->fullDirty &&
      ...
      for (pf = rowf = f, pt = rowt = t, y = currentContext->dirtyY1;
           y < currentContext->dirtyY2;
           y++,
          pt = (rowt = (Pixel32*)(((uint8*)rowt) + screen.pitch)),
          pf = (rowf += screenW))
        if (shiftY != 0 && y >= shiftH) {
          if (currentContext->fullDirty) // draw gray area only if first time
                                         // (full dirty, set above *1*)
    ...
    if (currentContext->fullDirty &&
      ...
      for (pf = rowf = f, pt = rowt = t, y = currentContext->dirtyY1;
           y < currentContext->dirtyY2;
           y++,
          pt = (rowt = (Pixel24*)(((uint8*)rowt) + screen.pitch)),
          pf = (rowf += screenW))
        for (count = currentContext->dirtyX2 - currentContext->dirtyX1;
             count != 0;
             pf++, pt++, count--) {
...
#ifndef SKIA_H
static void
fillWith8bppPalette(uint32* ptr)
{
...
#ifndef SKIA_H
static int
getOffset(int radius, int y)
{
  return radius - (int32)sqrt((double)radius * radius - y * y);
}

static void
drawFadedPixel(Context currentContext, TCObject g, int32 xx, int32 yy, int32 c)
{ // guich@tc124_4
#ifdef __gl2_h_
  if (Graphics_useOpenGL(g)) {
...
static void
drawRoundGradient(Context currentContext,
                  TCObject g,
                  int32 startX,
                  int32 startY,
                  int32 endX,
                  ...
                  bool vertical)
{
  int32 numSteps = max32(
    1,
    vertical
      ? abs32(endY - startY)
      : abs32(endX -
              startX)); // guich@tc110_11: support horizontal gradient -
...
#ifdef __gl2_h_
  int32 clipX1 = Graphics_clipX1(g) - Graphics_transX(g),
        clipY1 = Graphics_clipY1(g) - Graphics_transY(g),
        clipX2 = Graphics_clipX2(g) - Graphics_transX(g),
        clipY2 = Graphics_clipY2(g) - Graphics_transY(g);
  bool optimize =
    Graphics_useOpenGL(g) &&
    (startX >= clipX1 && startY >= clipY1 && endX < clipX2 && endY < clipY2) &&
  ...
  for (i = 0; i < numSteps; i++) {
    if (hasRadius) {
      ...
      if (topRightRadius > 0 && i < topRightRadius) {
        ...
        rightOffset =
          getOffset(bottomRightRadius, bottomRightRadius - (numSteps - i + 1)) -
          1;
      }
    ...
    if (!optimize || leftOffset != 0 || rightOffset != 0) {
      if (vertical) {
        ...
        drawLine(currentContext,
                 ...
                 p);
        if (drawFadedPixels &&
            rightOffset != 0) { // since there's no fading of pixels in opengl,
                                // we can safely ignore this
          drawFadedPixel(
            currentContext, g, endX - rightOffset + 1, startY + i, fc);
        }
        if (drawFadedPixels && leftOffset != 0) {
          drawFadedPixel(
            currentContext, g, startX + leftOffset - 1, startY + i, fc);
        }
      } else {
        drawLine(currentContext,
                 g,
                  ...
                  bool vertical)
{
  ...
  skia_drawRoundGradient(0,
                         ...
                         bottomLeftRadius,
                         bottomRightRadius,
                         startColor | Graphics_alpha(g),
                         endColor | Graphics_alpha(g),
                         vertical);
  skia_restoreClip();
...
#if 1
static int
getsetRGB(Context currentContext,
          TCObject g,
          TCObject dataObj,
          int32 offset,
          int32 x,
          ...
          bool isGet)
{
  if (dataObj == null) {
    throwException(
      currentContext, NullPointerException, "Argument 'data' can't be null");
  } else if (!checkArrayRange(currentContext, dataObj, offset, w * h))
    ;
  else if (translateAndClip(g, &x, &y, &w, &h)) {
...
#endif
    setPixel(currentContext,
             g,
             x,
             y,
             interpolate(color, getPixelConv(g, x, y), alpha));
}
#endif

static void
addError(PixelConv* pixel,
         int32 x,
         int32 y,
         int32 w,
         int32 h,
...
static void
dither(Context currentContext, TCObject g, int32 x0, int32 y0, int32 w, int32 h)
{
  PixelConv* pixels;
  int32 oldR, oldG, oldB, newR, newG, newB, errR, errG, errB, pitch, x, y,
    yf = y0 + h, xf = x0 + w;
  pitch = Graphics_pitch(g);
  ...
  if (!currentContext->fullDirty && !Graphics_isImageSurface(g)) {
    markScreenDirty(currentContext, x0, y0, w, h);
  }
}

/////////////// Start of Device-dependant functions ///////////////
static bool
startupGraphics(int16 appTczAttr)
{ // there are no threads running at this point
  return graphicsStartup(&screen, appTczAttr);
}

#ifdef SKIA_H
static void
shiftScreen(Context currentContext)
{
  ...
  //     checkKeyboardAndSIP(currentContext, &shiftY, &shiftH); // Adjust shift
  //     according to the keyboard int32 delta = shiftY - screen.screenH;
  // #ifdef ANDROID
  //     delta = 0;
  // #endif
...
createScreenSurface(Context currentContext, bool isScreenChange)
{
  ...
  if (graphicsCreateScreenSurface(&screen)) {
    TCObject* screenObj;
    screenObj = getStaticFieldObject(
      currentContext,
      loadClass(currentContext, "totalcross.ui.gfx.Graphics", false),
      "mainWindowPixels");
#ifdef darwin // in darwin, the pixels buffer is pre-initialized and never
              // changed
    if (controlEnableUpdateScreenPtr == null) {
      controlEnableUpdateScreenPtr = getStaticFieldInt(
        loadClass(currentContext, "totalcross.ui.Control", false),
        "enableUpdateScreen");
    }
    ret = true;
#else
    ...
    *screenObj = screen.mainWindowPixels = createArrayObject(
      currentContext, INT_ARRAY, screen.screenW * screen.screenH);
    setObjectLock(*screenObj, UNLOCKED);
    ret =
      screen.mainWindowPixels != null && controlEnableUpdateScreenPtr != null;
#endif
  }
  return ret;
}
...
#endif
  LOCKVAR(screen);
  if (keepRunning && checkScreenPixels() && controlEnableUpdateScreenPtr &&
      *controlEnableUpdateScreenPtr &&
      (currentContext->fullDirty ||
       (currentContext->dirtyX1 != screen.screenW &&
        currentContext->dirtyX2 != 0 &&
        currentContext->dirtyY1 != screen.screenH &&
        currentContext->dirtyY2 != 0))) {
    if (updateScreenBits(
          currentContext)) { // move the temporary buffer to the real screen
#ifdef darwin
      UNLOCKVAR(screen); // without this, a deadlock can occur in iOS if the
                         // user minimizes the application, since another thread
                         // can trigger a markScreenDirty
#endif
