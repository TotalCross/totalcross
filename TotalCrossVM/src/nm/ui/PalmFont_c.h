...
bool fontInit(Context currentContext) {
	...
	JCharP2CharPBuf(String_charsStart(*defaultFontNameObj), String_charsLen(*defaultFontNameObj),
					defaultFontName);
	maxFontSize = *maxfs;
	minFontSize = *minfs;
...
uint8* getResizedCharPixels(Context currentContext, UserFont uf, JChar ch, int32 newWidth,
							int32 newHeight) { // access directly the font bits and return an array of alpha only
	bool fSuccess = false;
	// font bits
	int32 offset = uf->bitIndexTable[ch];
	int32 width = uf->bitIndexTable[ch + 1] - offset - (uf->ubase
				  && uf->ubase->fontP.antialiased == AA_8BPP);
	...
	i = (uf->fontP.maxWidth * newHeight / uf->fontP.maxHeight + 1) * height + 2 * s * maxContribsXY *
		sizeof(int32) + 2 * s * sizeof(int32);
	...
	tempbuf += s * sizeof(int32); /* how many contributions for the target pixel */
...
Cleanup: /* CLEANUP */
	if (!fSuccess) {
		throwException(currentContext, OutOfMemoryError, "Cannot create font buffers");
	}
	UNLOCKVAR(fonts);
...
#ifdef __gl2_h_
bool lowmemDevice;
bool glLoadTexture(Context currentContext, TCObject img, int32* textureId, Pixel* pixels,
	...
	for (w = 0; w < widthsCount; offset += widths[w++])
		for (y = 0; y < fontH; y++) {
			xmemmove(&uf->textureAlphas[(y + w * fontH) * maxW],
					 &uf->bitmapTable[y * uf->rowWidthInBytes + offset], widths[w]);
		}
	uf->maxW = maxW;
	uf->maxH = maxH;
	return true;
...
bool getCharPosInTexture(Context currentContext, UserFont uf, JChar ch, int32* ret) {
	...
	if (uf->textureId[0] == 0 && (uf->maxW != 0 || buildFontTexture(currentContext, uf))) {
		b = glLoadTexture(currentContext, null, uf->textureId, (Pixel*)uf->textureAlphas, uf->maxW,
						  uf->maxH, true);
	}
...
tryAgain:
	nlen = 0;
	vsize = (size == -1) ? normalFontSize : ((int32)(max32(size,
			minFontSize) * fontSizeFactor));  // guich@tc122_15: don't check for the maximum font size here
	...
	uf->rowWidthInBytes = 2 * (uint32)uf->fontP.rowWords * (uf->fontP.antialiased == AA_NO ? 1 :
	...
	uf->bitIndexTable -=
		uf->fontP.firstChar; // instead of doing "bitIndexTable[ch-firstChar]", this trick will allow use "bitIndexTable[ch]
	if (uf->fontP.antialiased == AA_8BPP) { // glfont - create the texture
		uf->charPixels = newPtrArrayOf(Int32, (uf->fontP.maxWidth + 1) * (uf->fontP.maxHeight + 1),
									   fontsHeap);
	}
