...
{
#if defined(ANDROID) || (defined(WINCE) && _WIN32_WCE >= 300) || defined(darwin) //flsobral@tc115_56: fixed Camera API support for WinCE. (nothing was changed, for some reason the compiler was ignoring the WinCE code)
   cameraClick(p);
#else
   UNUSED(p);
#endif
