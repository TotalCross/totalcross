...
#define WAVEFORMAT_MIDI_EXTRASIZE                                              \
  (sizeof(WAVEFORMAT_MIDI) - sizeof(WAVEFORMATEX))
#endif

#ifndef _SOUNDFILE_H_
typedef enum tagSND_SOUNDTYPE
{
  SND_SOUNDTYPE_ON, // If sound is currently Vibrate or None this will restore
                    // the
  ...
  SND_SOUNDTYPE_VIBRATE, // Sound is vibration.  SNDFILEINFO.szPathName is
                         // ignored.
  SND_SOUNDTYPE_NONE, // No sound (silence).  SNDFILEINFO.szPathName is ignored.
  SND_SOUNDTYPE_DISABLE, // Only applies to SND_EVENTs for KNOWNCALLER and
                         // ROAMING
...
typedef enum tagSND_EVENT
{
  ...
  SND_EVENT_KNOWNCALLERRINGTONELINE1, // The ringtone to play for a caller in
                                      // the contact list who has no
                                      // contact-specific ring tone.
  // This event does not support the "vibrate" and "none" sound types
  // Currently applies to both lines without distinction
  SND_EVENT_ROAMINGRINGTONELINE1, // The ringtone to play while roaming.  This
                                  // has higher priority than other ring tones.
  // This event does not support the "vibrate" and "none" sound types
  // Currently applies to both lines without distinction
