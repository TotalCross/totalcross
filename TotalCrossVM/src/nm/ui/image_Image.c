...
void
jpegLoad(Context currentContext,
         TCObject imageInstance,
         ...
         int32 scale_denom);
void
pngLoad(Context currentContext,
        TCObject imageInstance,
        TCObject inputStreamObj,
        TCObject bufObj,
        TCZFile tcz,
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_imageLoad_s(NMParams p)
{ // totalcross/ui/image/Image native private void
  ...
  if (tcz != null) {
    char magic[4]; // read the magic to find if its a png or a jpeg (note that
                   // jpeg has no magic)
    tczRead(tcz, magic, 4);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_imageParse_sB(NMParams p)
{ // totalcross/ui/image/Image native private void
  // imageParse(totalcross.io.Stream in, byte []buf);
  TCObject imageObj = p->obj[0];
  ...
  char magic[4];
  xmove4(magic,
         buf); // buf already comes filled from Java with the first 4 bytes
  if ((magic[0] & 0xFF) == 0x89 && magic[1] == 'P' && magic[2] == 'N' &&
      magic[3] == 'G') {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_changeColors_ii(NMParams p)
{ // totalcross/ui/image/Image native public
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_getPixelRow_Bi(NMParams p)
{ // totalcross/ui/image/Image native protected
...
TC_API void
tuiI_getModifiedInstance_iiiiiii(NMParams p)
{ // totalcross/ui/image/Image native private void
  // getModifiedInstance(totalcross.ui.image.Image newImg, int
  // angle, int percScale, int color, int brightness, int
  // contrast, int type);
  TCObject thisObj = p->obj[0];
  ...
  switch (type) {
    ...
    case ROTATED_SCALED_INSTANCE:
      getRotatedScaledInstance(
        thisObj, newObj, percScale, angle, color, p->i32[3], p->i32[4]);
      break;
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_setCurrentFrame_i(NMParams p)
{ // totalcross/ui/image/Image native public
...
{ // totalcross/ui/image/Image native public void
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_nativeEquals_i(NMParams p)
{ // totalcross/ui/image/Image native private boolean
  // nativeEquals(totalcross.ui.image.Image other);
  TCObject thisObj = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_setTransparentColor_i(NMParams p)
{ // totalcross/ui/image/Image native public
  // totalcross.ui.image.Image setTransparentColor(int color);
  TCObject thisObj = p->obj[0];
  Pixel color = makePixelRGB(p->i32[0]);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_applyChanges(NMParams p)
{ // totalcross/ui/image/Image native
  ...
  TCObject pixelsObj =
    frameCount == 1 ? Image_pixels(img) : Image_pixelsOfAllFrames(img);
  Pixel* pixels = (Pixel*)ARRAYOBJ_START(pixelsObj);
  int32 width = (Image_frameCount(img) > 1) ? Image_widthOfAllFrames(img)
                                            : Image_width(img);
...
//////////////////////////////////////////////////////////////////////////
bool
image2jpeg(Context currentContext,
           TCObject srcImageObj,
           TCObject dstStreamObj,
           int32 quality); // JpegLoader.c
TC_API void
tuiI_createJpg_si(NMParams p)
{ // totalcross/ui/image/Image native public void
  // createJpg(totalcross.io.Stream s, int quality);
  TCObject thisObj = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_nativeResizeJpeg_ssi(NMParams p)
{ // totalcross/ui/image/Image native public static void
  // nativeResizeJpeg(String inputPath, String outputPath, int
  // maxPixelSize);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_getJpegBestFit_sii(NMParams p)
{ // totalcross/ui/image/Image native public static
  // totalcross.ui.image.Image getJpegBestFit(String path, int
  // targetWidth, int targetHeight) throws java.io.IOException,
  ...
  if ((imageObj = createObject(p->currentContext,
                               "totalcross.ui.image.Image")) != NULL &&
      (initMethod = getMethod(OBJ_CLASS(imageObj), false, "init", 0)) != NULL) {
    if (tcz != null) {
      jpegLoad(p->currentContext,
               imageObj,
               null,
               null,
               tcz,
               ...
               targetWidth,
               targetHeight);
    } else if ((fileObj = createObject(p->currentContext,
                                       "totalcross.io.File")) != NULL) {
      fileConstructor = getMethod(OBJ_CLASS(fileObj),
                                  false,
                                  CONSTRUCTOR_NAME,
                                  2,
                                  "java.lang.String",
      ...
      if (fileConstructor != null) {
        executeMethod(
          p->currentContext, fileConstructor, fileObj, pathObj, READ_ONLY);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tuiI_getJpegScaled_sii(NMParams p)
{ // totalcross/ui/image/Image native public static
  // totalcross.ui.image.Image getJpegScaled(String path, int
  // scaleNumerator, int scaleDenominator) throws
  // java.io.IOException, totalcross.ui.image.ImageException;
  TCObject pathObj = p->obj[0];
