...
//////////////////////////////////////////////////////////////////////////
TC_API void
tufF_fontCreate(NMParams p)
{ // totalcross/ui/font/Font native void fontCreate();
  char name[128];
  ...
  if (!(nameTTF[len - 4] == '.' && nameTTF[len - 3] == 't' &&
        nameTTF[len - 2] == 't' && nameTTF[len - 1] == 'f')) {
    xstrcat(nameTTF, ".ttf");
  }
  ...
  ff = name[0] == '$'
         ? null
         : loadFontFile(
             name); //  bruno@tc114_37: native fonts always start with '$'
  if (ff == null) {
    ...
    Font_name(obj) =
      createStringObjectFromCharP(p->currentContext, defaultFontName, 6);
