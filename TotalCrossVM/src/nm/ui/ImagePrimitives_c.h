...
static void
setCurrentFrame(TCObject obj, int32 nr)
{
  int32 y, width, widthOfAllFrames;
  Pixel* pixelsOfAllFrames =
    (Pixel*)ARRAYOBJ_START(Image_pixelsOfAllFrames(obj));
  Pixel* pixels = (Pixel*)ARRAYOBJ_START(Image_pixels(obj));
  int32 frameCount = Image_frameCount(obj), mw;

  if (frameCount <= 1 || nr == Image_currentFrame(obj)) {
    return;
  }
  if (nr < 0) {
    nr = frameCount - 1;
  ...
  for (y = Image_height(obj); --y >= 0;
       pixelsOfAllFrames += widthOfAllFrames, mw = width)
    while (mw-- > 0) {
...
static void
applyColor(TCObject obj, Pixel color)
{ // guich@tc112_24
  int32 frameCount = Image_frameCount(obj);
  TCObject pixelsObj =
    frameCount == 1 ? Image_pixels(obj) : Image_pixelsOfAllFrames(obj);
...
static bool
getSmoothScaledInstance(TCObject thisObj, TCObject newObj)
{ // guich@tc130: changed area-averaging to Catmull-Rom
  // resampling
  bool fSuccess = false;
  ...
  TCObject pixelsObj = (frameCount == 1) ? Image_pixels(thisObj)
                                         : Image_pixelsOfAllFrames(thisObj);
  ...
  v_pixel = (int32*)xmalloc(s * maxContribsXY *
                            sizeof(int32)); /* the contributing pixels */
  v_count = (int32*)xmalloc(
    s * sizeof(int32)); /* how may contributions for the target pixel */
  v_wsum = (int32*)xmalloc(
    s * sizeof(int32)); /* sum of the weights for the target pixel */

  if (!tb || !v_weight || !v_pixel || !v_count || !v_wsum) {
    goto Cleanup;
  }
...
static void
changeColors(TCObject obj, Pixel from, Pixel to)
{
  int32 frameCount = Image_frameCount(obj);
  TCObject pixelsObj =
    frameCount == 1 ? Image_pixels(obj) : Image_pixelsOfAllFrames(obj);
...
static void
getScaledInstance(TCObject thisObj, TCObject newObj)
{
  Pixel* dstImageData = (Pixel*)ARRAYOBJ_START(Image_pixels(newObj));
  int32 frameCount = Image_frameCount(thisObj);
  TCObject pixelsObj =
    frameCount == 1 ? Image_pixels(thisObj) : Image_pixelsOfAllFrames(thisObj);
...
static void
getRotatedScaledInstance(TCObject thisObj,
                         TCObject newObj,
                         int32 percScale,
                         int32 angle,
                         Pixel color,
...
{
  int32 frameCount = Image_frameCount(thisObj);
  Pixel *pixelsIn = (Pixel*)ARRAYOBJ_START(Image_pixels(thisObj)),
        *pixelsIn0 = pixelsIn;
  Pixel *pixelsOut = (Pixel*)ARRAYOBJ_START(Image_pixels(newObj)),
        *pixelsOut0 = pixelsOut;
  ...
  int32 sine = 0;
  int32 cosine = 0;
  int32 i, u, v, x, y, x00 = x0, y00 = y0, j, newHeight0 = newHeight,
                       widthOfAllFrames;
  ...
  for (j = 0; j < frameCount; j++) {
    ...
    if (frameCount > 1) {
      ...
      pixelsOfAllFrames =
        (Pixel*)ARRAYOBJ_START(Image_pixelsOfAllFrames(newObj));
      pixels = (Pixel*)ARRAYOBJ_START(Image_pixels(newObj));
      pixelsOfAllFrames += j * newWidth;
      for (y = newHeight0; --y >= 0;
           pixelsOfAllFrames += widthOfAllFrames, n = newWidth)
        while (n-- > 0) {
...
// Generates 128 points from a kind of symmetrical yn = sum{1 to
// inf}(e-xt/tn)/dt, n being proportional to 'level'. The contrast level must be
// in the range -128 ... + 127
static void
computeContrastTable(uint8* table, int32 level)
{
  double factor;
  int32 i, v;
...
static void
getTouchedUpInstance(TCObject thisObj,
                     TCObject newObj,
                     int32 iBrightness,
                     int32 iContrast)
{
...
static void
getFadedInstance(TCObject thisObj, TCObject newObj, int32 backColor)
{ // guich@tc110_50
...
static void
getAlphaInstance(TCObject thisObj, TCObject newObj, int32 delta)
{ // guich@tc110_50
  PixelConv *in, *out;
  int32 len;
  int32 frameCount = Image_frameCount(thisObj);
...
static void
getPixelRow(Context currentContext, TCObject obj, TCObject outObj, int32 y)
{
  TCObject pixObj = (Image_frameCount(obj) > 1) ? Image_pixelsOfAllFrames(obj)
                                                : Image_pixels(obj);
  PixelConv* pixels = (PixelConv*)ARRAYOBJ_START(pixObj);
  int8* out = (int8*)ARRAYOBJ_START(outObj);
  int32 width = (Image_frameCount(obj) > 1) ? Image_widthOfAllFrames(obj)
                                            : Image_width(obj);
...
static void
applyColor2(TCObject obj, Pixel color)
{
  int32 frameCount = Image_frameCount(obj);
  TCObject pixelsObj =
    frameCount == 1 ? Image_pixels(obj) : Image_pixelsOfAllFrames(obj);
  ...
  for (len = len0, pixels = pixels0; len-- > 0; pixels++) {
    ...
    if (changeA) {
      int32 a = pixels->r > pixels->g ? pixels->r : pixels->g;
      if (pixels->b > a) {
        a = pixels->b;
      }
      pixels->a = a * 255 / hi;
...
void
setTransparentColor(TCObject obj, Pixel color);
void
setTransparentColor(TCObject obj, Pixel color)
{
  int32 frameCount = Image_frameCount(obj);
  ...
  if ((int32)color == -1) // no transparent pixels?
    ...
    for (; len-- > 0; pixels++) {
      ...
      *pixels = (p == color)
                  ? color
                  : p | 0xFF000000; // if is the transparent color, set the
                                    // alpha to 0, otherwise, set to full bright
    }
  if (frameCount != 1) {
    Image_currentFrame(obj) = 2;
    setCurrentFrame(obj, 0);
...
void
onImage(int32 it, VoidP ptr)
{
  TCObject img = (TCObject)ptr;
  if (!Surface_isImage(ptr)) { // Image can be created and an exception be
                               // thrown before the constructor is called
    return;
  }

  Image_changed(img) = true; // applyChanges(lifeContext, img); - update only
                             // when the image is going to be painted
}

static bool
nativeEquals(TCObject thisObj, TCObject otherObj)
{
  Pixel *p1, *p2;
  int32 len;
  int32 frameCount = Image_frameCount(thisObj);
...
static void
applyFade(TCObject obj, int32 fadeValue)
{
  int32 frameCount = Image_frameCount(obj);
  TCObject pixelsObj =
    frameCount == 1 ? Image_pixels(obj) : Image_pixelsOfAllFrames(obj);
