...
static bool getSmoothScaledInstance(TCObject thisObj,
									TCObject newObj) { // guich@tc130: changed area-averaging to Catmull-Rom resampling
	bool fSuccess = false;
...
static void getRotatedScaledInstance(TCObject thisObj, TCObject newObj, int32 percScale,
									 int32 angle, Pixel color, int32 x0, int32 y0) {
	int32 frameCount = Image_frameCount(thisObj);
...
static void getTouchedUpInstance(TCObject thisObj, TCObject newObj, int32 iBrightness,
								 int32 iContrast) {
	enum {
		NO_TOUCHUP,
		BRITE_TOUCHUP,
...
void onImage(int32 it, VoidP ptr) {
	...
	if (!Surface_isImage(
			ptr)) { // Image can be created and an exception be thrown before the constructor is called
		return;
	}

	Image_changed(img) =
		true; //applyChanges(lifeContext, img); - update only when the image is going to be painted
}
