...
void
createTempFileName(char* dest, char* ext);
 ...
 */
void JNICALL
Java_totalcross_Launcher4A_pictureTaken(JNIEnv* env, jclass _class, jint jcode)
{
  code = jcode;
  takingPicture = false;
}
...
static TCObject
Camera_getNativeResolutions(Context currentContext)
{
  JNIEnv* env = getJNIEnv();
  TCObject ret = null;

  jmethodID method = (*env)->GetStaticMethodID(
    env, applicationClass, "requestCameraPermission", "()I");
  jint result = (*env)->CallStaticIntMethod(env, applicationClass, method);
  if (result > 0) {
    jstring src = (*env)->CallStaticObjectMethod(
      env, applicationClass, jgetNativeResolutions);
    if (src != null) {
...
cameraClick(NMParams p)
{
  ...
  if (env) {
    ...
    if (defaultFN == null) {
      ...
      JCharP2CharPBuf(
        String_charsStart(defaultFN), String_charsLen(defaultFN), fileName);
      if (xstrchr(fileName, '/') == null &&
          xstrchr(fileName, '\\') == null) { // no path specified:
    ...
    if (!isPhoto && len > 4 &&
        fileName[len - 3] !=
          '3') { // is a movie and filename is .jpg? make it 3gp
      fileName[len - 3] = '3';
      fileName[len - 2] = 'g';
      fileName[len - 1] = 'p';
    }
    ...
    (*env)->CallStaticVoidMethod(env,
                                 ...
                                 cameraType);
    for (takingPicture = true;
         takingPicture;) { // block vm until the picture is taken
      Sleep(250);
    }
    switch (code) {
      case 0:
        p->retO = createStringObjectFromCharP(
          p->currentContext, fileName, xstrlen(fileName));
