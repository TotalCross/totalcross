...
#ifdef ANDROID
void JNICALL
Java_totalcross_Launcher4A_nativeInitSize(JNIEnv* env,
                                          jobject this,
                                          jobject surface,
                                          jint width,
                                          jint height)
{ // called only once
  ...
  if (lastWindow && lastWindow != window) {
    ...
    //      invalidateTextures(INVTEX_INVALIDATE); // now we set the changed
...
initGLES(ScreenSurface screen)
{
  ...
  const EGLint surfaceAttribs[] = { EGL_RENDER_BUFFER,
                                    EGL_BACK_BUFFER,
                                    EGL_NONE };
  ...
  if (!(surface =
          eglCreateWindowSurface(display, config, window, surfaceAttribs))) {
...
#ifdef SKIA_H
  initSkia(
    screen->screenW, screen->screenH, NULL, screen->pitch, screen->pixelformat);
#endif
