...
int skia_makeBitmap(int32 id, void* data, int32 w, int32 h) {
	...
	for (int i = 0; i < w * h; i++) {
		...
		converted[i] = (((pixel >> 24) & 0xFF)) | ((((pixel >> 16) & 0xFF) << 8) | (((
...
void skia_drawSurface(int32 skiaSurface, int32 id, int32 srcX, int32 srcY, int32 srcW, int32 srcH,
					  int32 w, int32 h, int32 dstX, int32 dstY, int32 alphaMask, int32 doClip) {
	SKIA_TRACE()
...
Pixel skia_getPixel(int32 skiaSurface, int32 x, int32 y) {
	...
	return (((pixel >> 24) & 0xFF) << 24) | (((pixel & 0xFF) << 16) | (((pixel >> 8) & 0xFF) << 8) | ((
				pixel >> 16) & 0xFF));
}
...
void skia_drawDottedLine(int32 skiaSurface, int32 x1, int32 y1, int32 x2, int32 y2, Pixel pixel1,
						 Pixel pixel2) {
	SKIA_TRACE()
	float intervals[] = {5, 5};
...
void skia_drawText(int32 skiaSurface, const void* text, int32 chrCount, int32 x0, int32 y0,
				   Pixel foreColor, int32 justifyWidth, int32 fontSize, int32 typefaceIndex) {
	SKIA_TRACE()
...
void skia_ellipseDrawAndFill(int32 skiaSurface, int32 xc, int32 yc, int32 rx, int32 ry, Pixel pc1,
							 Pixel pc2, bool fill, bool gradient) {
	SKIA_TRACE()
	if (fill) {
		if (gradient) {
			SkPoint points[3] = {
...
void skia_drawPolygon(int32 skiaSurface, int32* xPoints, int32* yPoints, int32 nPoints, int32 tx,
					  int32 ty, Pixel pixel) {
	SKIA_TRACE()
	forePaint.setColor(pixel);
...
void skia_fillPolygon(int32 skiaSurface, int32* xPoints, int32* yPoints, int32 nPoints, int32 tx,
					  int32 ty, Pixel c1, Pixel c2, bool gradient, bool isPie) {
	SKIA_TRACE()
...
SkPath _skia_makeArcPath(const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle,
						 bool useCenter) {
...
void skia_arcPiePointDrawAndFill(int32 skiaSurface, int32 xc, int32 yc, int32 rx, int32 ry,
								 double startAngle, double endAngle, Pixel c, Pixel c2, bool fill, bool pie, bool gradient) {
	...
	if (fill) {
		...
		if (gradient) {
			SkPath arcPath = _skia_makeArcPath(SkRect::MakeXYWH(xc - rx, yc - ry, rx * 2, ry * 2), start,
											   sweepAngle, pie);
			...
			canvas->drawArc(SkRect::MakeXYWH(xc - rx, yc - ry, rx * 2, ry * 2), start, sweepAngle, pie,
							backPaint);
...
void skia_drawRoundGradient(int32 skiaSurface, int32 startX, int32 startY, int32 endX, int32 endY,
							int32 topLeftRadius, int32 topRightRadius, int32 bottomLeftRadius, int32 bottomRightRadius,
							int32 startColor, int32 endColor, bool vertical) {
	SKIA_TRACE()
	int32 w = endX - startX;
	int32 h = endY - startY;
	SkPoint points[2];
	...
	canvas->drawRRect(SkRRect::MakeRectXY(SkRect::MakeXYWH(startX, startY, w, h), topLeftRadius,
										  topLeftRadius), backPaint);
	backPaint.setShader(nullptr);
}
int skia_getsetRGB(int32 skiaSurface, void* pixels, int32 offset, int32 x, int32 y, int32 w,
				   int32 h,
				   bool isGet) {
	SKIA_TRACE()
...
int32 colorType(uint32 pixelformat) {
	...
	debug("Unsupported pixel format %s, try mapping your color format on %s - %s",
		  SDL_GetPixelFormatName(pixelformat), __FILE__, __FUNCTION__);
