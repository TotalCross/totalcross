...
void
eventQueuePush(int type, int key, int x, int y, int modifiers)
{
...
extern "C"
{
  void screenChange(Context currentContext,
                    int32 newWidth,
                    int32 newHeight,
                    int32 hRes,
                    int32 vRes,
...
void
windowSetDeviceTitle(TCObject titleObj)
{}
...
vmExec(TCHARP szCommand, TCHARP szArgs, int32 launchCode, bool wait)
{
  ...
  if (comm->Equals("url") || comm->Equals("viewer")) {
    Windows::System::Launcher::LaunchUriAsync(
      ref new Windows::Foundation::Uri(args));
  }
  return 1;
}

void
vibrate(DWORD32 milliseconds)
{
  VibrationDevice ^ vib = VibrationDevice::GetDefault();
  if (vib != nullptr) {
    TimeSpan time;
    time.Duration = min(milliseconds * 10000,
                        50000000); // The time unit is 100ns and the limit is 5
                                   // s. More than that, boom!
    vib->Vibrate(time);
  }
}
...
void
alertCPP(JCharP jCharStr)
{
  Direct3DBase::getLastInstance()->csharp->privateAlertCS(
    ref new Platform::String((wchar_t*)jCharStr), eventsInitialized);
  while (Direct3DBase::getLastInstance()->csharp->isAlertVisible()) {
    Sleep(10);
  }
}

void
vmSetAutoOffCPP(bool enable)
{
  Direct3DBase::getLastInstance()->csharp->vmSetAutoOffCS(enable);
}

void
dialNumberCPP(JCharP number)
{
  Direct3DBase::getLastInstance()->csharp->dialNumberCS(
    ref new Platform::String((wchar_t*)number));
}

void
smsSendCPP(JCharP szMessage, JCharP szDestination)
{
  Direct3DBase::getLastInstance()->csharp->smsSendCS(
    ref new Platform::String((wchar_t*)szMessage),
    ref new Platform::String((wchar_t*)szDestination));
}

int
rdGetStateCPP(int type)
{
  PhoneDirect3DXamlAppComponent::CSwrapper ^ cs =
    Direct3DBase::getLastInstance()->csharp;
  cs->rdGetStateCS(type);
  return cs->getTurnedState();
}

bool
isAvailableCPP(int type)
{
...
int
nativeUpdateLocationCPP(Context context, TCObject gpsObject)
{
  PhoneDirect3DXamlAppComponent::CSwrapper ^ cs =
    Direct3DBase::getLastInstance()->csharp;
  ...
  setObjectLock(
    GPS_messageReceived(gpsObject) = createStringObjectFromJCharP(
      context, (JCharP)messageReceived->Data(), messageReceived->Length()),
    UNLOCKED);

  lowSignalReason = cs->getLowSignalReason();
  setObjectLock(
    GPS_lowSignalReason(gpsObject) = createStringObjectFromJCharP(
      context, (JCharP)lowSignalReason->Data(), lowSignalReason->Length()),
    UNLOCKED);
...
dxGetPixels(Pixel* dstPixels,
            ...
            int32 pitch)
{
  Direct3DBase::getLastInstance()->getPixels(
    dstPixels, srcX, srcY, width, height, pitch);
}
...
bool
dxLoadTexture(Context currentContext,
              TCObject img,
              int32* textureId,
              Pixel* pixels,
              int32 width,
              ...
              bool onlyAlpha)
{
  return Direct3DBase::getLastInstance()->loadTexture(
    currentContext, img, textureId, pixels, width, height, onlyAlpha);
}

void
dxDeleteTexture(TCObject img, int32* textureId)
{
  Direct3DBase::getLastInstance()->deleteTexture(img, textureId);
}

void
dxDrawTexture(int32* textureId,
              int32 x,
              int32 y,
              int32 w,
              int32 h,
              ...
              int32 alphaMask)
{
  Direct3DBase::getLastInstance()->drawTexture(textureId,
                                               x,
                                               y,
                                               w,
                                               h,
...
void
dxDrawLines(Context currentContext,
            TCObject g,
            int32* x,
            int32* y,
            int32 n,
            ...
            bool fill)
{
  Direct3DBase::getLastInstance()->drawLines(
    currentContext, g, x, y, n, tx, ty, color, fill);
}

void
dxDrawLine(int x1, int y1, int x2, int y2, int color)
{
  Direct3DBase::getLastInstance()->drawLine(x1, y1, x2, y2, color);
}

void
dxFillShadedRect(TCObject g,
                 int32 x,
                 int32 y,
                 int32 w,
                 int32 h,
...
void
dxFillRect(int32 x, int32 y, int32 w, int32 h, int color)
{
  Direct3DBase::getLastInstance()->fillRect(x, y, w, h, color);
}

void
dxDrawPixelColors(int32* x, int32* y, PixelConv* colors, int32 n)
{
  Direct3DBase::getLastInstance()->drawPixelColors(x, y, colors, n);
}

void
dxDrawPixels(float* glXYA, int count, int color)
{
...
void
privateWindowSetSIP(bool visible)
{
  isShown = visible;
...
void
cameraClick(NMParams p)
{
  Direct3DBase::getLastInstance()->csharp->cameraClick();
  int status;
  while ((status = Direct3DBase::getLastInstance()->csharp->cameraStatus()) ==
         -1) {
    Sleep(100);
  }
  if (status == 0) { // cancelled?
    ...
    Platform::String ^ name =
      Direct3DBase::getLastInstance()->csharp->cameraFilename();
    ...
    WideCharToMultiByte(CP_ACP,
                        ...
                        NULL,
                        NULL);
    setObjectLock(p->retO = createStringObjectFromCharP(
                    p->currentContext, ret, name->Length()),
                  UNLOCKED);
    xfree(ret);
  }
}
...
void
nativeSoundPlayCPP(char* filename)
{
...
void
getDeviceIdCPP(CharP ret)
{
  Platform::String ^ name =
    Direct3DBase::getLastInstance()->csharp->getDeviceId();
  WideCharToMultiByte(CP_ACP,
                      0,
                      name->Data(),
                      name->Length(),
...
void
getRomSerialNumberCPP(CharP ret)
{
  Platform::String ^ name =
    Direct3DBase::getLastInstance()->csharp->getSerialNumber();
  WideCharToMultiByte(CP_ACP,
                      0,
                      name->Data(),
                      name->Length(),
...
bool
showMap(JCharP originStr,
        int32 originLen,
        JCharP destinationStr,
        int32 destinationLen)
{
  return Direct3DBase::getLastInstance()->csharp->showMap(
    ref new Platform::String((wchar_t*)originStr, originLen),
    ref new Platform::String((wchar_t*)destinationStr, destinationLen));
}
