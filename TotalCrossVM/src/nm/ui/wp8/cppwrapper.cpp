...
void vibrate(DWORD32 milliseconds) {
	...
	if (vib != nullptr) {
		...
		time.Duration = min(milliseconds * 10000,
							50000000); // The time unit is 100ns and the limit is 5 s. More than that, boom!
		vib->Vibrate(time);
	}
}
...
void alertCPP(JCharP jCharStr) {
	Direct3DBase::getLastInstance()->csharp->privateAlertCS(ref new Platform::String((
				wchar_t*)jCharStr), eventsInitialized);
...
void smsSendCPP(JCharP szMessage, JCharP szDestination) {
	Direct3DBase::getLastInstance()->csharp->smsSendCS(ref new Platform::String((wchar_t*)szMessage),
...
int nativeUpdateLocationCPP(Context context, TCObject gpsObject) {
	...
	setObjectLock(GPS_messageReceived(gpsObject) = createStringObjectFromJCharP(context,
				  (JCharP)messageReceived->Data(), messageReceived->Length()), UNLOCKED);

	lowSignalReason = cs->getLowSignalReason();
	setObjectLock(GPS_lowSignalReason(gpsObject) = createStringObjectFromJCharP(context,
				  (JCharP)lowSignalReason->Data(), lowSignalReason->Length()), UNLOCKED);
...
bool dxLoadTexture(Context currentContext, TCObject img, int32* textureId, Pixel* pixels,
				   int32 width,
				   int32 height, bool onlyAlpha) {
	return Direct3DBase::getLastInstance()->loadTexture(currentContext, img, textureId, pixels, width,
			height, onlyAlpha);
}
...
void dxDrawTexture(int32* textureId, int32 x, int32 y, int32 w, int32 h, int32 dstX, int32 dstY,
	...
	Direct3DBase::getLastInstance()->drawTexture(textureId, x, y, w, h, dstX, dstY, dstW, dstH, imgW,
			imgH, color, alphaMask);
}
...
void cameraClick(NMParams p) {
	...
	if (status == 0) { // cancelled?
		...
		setObjectLock(p->retO = createStringObjectFromCharP(p->currentContext, ret, name->Length()),
					  UNLOCKED);
		xfree(ret);
	}
}
...
bool showMap(JCharP originStr, int32 originLen, JCharP destinationStr, int32 destinationLen) {
	return Direct3DBase::getLastInstance()->csharp->showMap(ref new Platform::String((
				wchar_t*)originStr, originLen), ref new Platform::String((wchar_t*)destinationStr, destinationLen));
}
