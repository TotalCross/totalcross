...
static bool analyzeApplyColor2(NativeImageBackingRecord* source,
    ...
    source->applyColor2HighestRed = static_cast<uint8_t>(highestRed);
    source->applyColor2HighestGreen = static_cast<uint8_t>(highestGreen);
    source->applyColor2HighestBlue = static_cast<uint8_t>(highestBlue);
    source->applyColor2HighestChannel = static_cast<uint8_t>(highestChannel);
    source->applyColor2AnalysisGeneration = source->generation;
    source->applyColor2AnalysisValid = true;
    analysis->highestRed = highestRed;
    analysis->highestGreen = highestGreen;
    analysis->highestBlue = highestBlue;
    analysis->highestChannel = highestChannel;
    return true;
}
...
static bool transform(std::vector<uint8_t>* pixels, int32 width, int32 height, int32 operation,
                      ...
                      int32 currentFrame, NativeImageBackingRecord* source) {
    ...
    case SKIA_IMAGE_COLOR_APPLY_COLOR2: {
        ...
        ApplyColor2Analysis analysis;
        if (!analyzeApplyColor2(source, *pixels, &analysis)) {
            return false;
        }
        for (size_t i = 0; i < pixels->size(); i += 4) {
            if ((*pixels)[i + 3] == 0) {
            ...
            const int red = clampChannel((*pixels)[i] * targetRed / analysis.highestRed);
            const int green = clampChannel((*pixels)[i + 1] * targetGreen / analysis.highestGreen);
            const int blue = clampChannel((*pixels)[i + 2] * targetBlue / analysis.highestBlue);
            if (changeAlpha) {
                const int brightest = std::max((*pixels)[i], std::max((*pixels)[i + 1], (*pixels)[i + 2]));
                (*pixels)[i + 3] = static_cast<uint8_t>(clampChannel(brightest * 255 / analysis.highestChannel));
            }
...
static bool transformedPixels(int64_t handle, int32 operation, int32 parameter1, int32 parameter2,
    ...
    *source = findBacking(handle);
    ...
    return transform(pixels, (*source)->width, (*source)->height, operation, parameter1, parameter2,
        frameCount, visibleWidth, currentFrame, *source);
}
