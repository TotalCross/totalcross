...
{ // totalcross/ui/image/Image native private void
  ...
  intArray = FIELD_OBJ(imgObj, OBJ_CLASS(imgObj), 5) =
    createIntArray(currentContext, 1);
...
TESTCASE(tuiI_imageParse_sB)
{ // totalcross/ui/image/Image native private void
  // imageParse(totalcross.io.Stream in, byte
  // []buf);  #DEPENDS(tuiI_imageLoad_s)
  TEST_SKIP;
finish:;
}
TESTCASE(tuiI_changeColors_ii)
{ // totalcross/ui/image/Image native public void
  // changeColors(int from, int to);
  // #DEPENDS(tuiI_imageParse_sB)
  TEST_SKIP;
finish:;
}
TESTCASE(tuiI_getPixelRow_Bi)
...
TESTCASE(tuiI_getModifiedInstance_iiiiiii)
{ // totalcross/ui/image/Image native
  // private void
  // getModifiedInstance(totalcross.ui.image.Image4D
  // newImg, int angle, int percScale, int
  // color, int brightness, int contrast,
  // int type);
  // #DEPENDS(tuiI_imageParse_sB)
  TEST_SKIP;
finish:;
