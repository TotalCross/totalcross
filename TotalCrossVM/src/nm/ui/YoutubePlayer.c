...
TC_API void tumYP_play_scbii(NMParams p) {
	...
	(*env)->CallStaticObjectMethod(env, applicationClass, jplayYoutube, jUrl,
								   autoPlay == 1 ? JNI_TRUE : JNI_FALSE, start, end);
#endif
}
#if defined(ANDROID)
void JNICALL Java_totalcross_Launcher4A_nativeYoutubeCallback(JNIEnv* env, jclass class,
		jint message) {
	if (callback == null) {
		return;
	}
