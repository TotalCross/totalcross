...
cameraClick(NMParams p)
{
  ...
  if (defaultFileName == null) { // if filename not provided, use a default one
    ...
    if (xstrchr(fileName, '/')) {
      xstrcpy(tempPictureName,
              fileName); // if path was passed with the filename, use it.
    } else {
      xstrcat(tempPictureName,
              fileName); // else, just the name, append after the path
    }
  }

  if (iphone_cameraClick(Camera_resolutionWidth(cameraObj),
                         Camera_resolutionHeight(cameraObj),
                         Camera_cameraType(cameraObj),
                         tempPictureName)) {
    setObjectLock(p->retO = createStringObjectFromCharP(
                    p->currentContext, tempPictureName, -1),
                  UNLOCKED);
  }
#endif
}
