...
static void cameraClick(NMParams p) {
	...
	if (iphone_cameraClick(Camera_resolutionWidth(cameraObj), Camera_resolutionHeight(cameraObj),
						   Camera_cameraType(cameraObj), tempPictureName)) {
		setObjectLock(p->retO = createStringObjectFromCharP(p->currentContext, tempPictureName, -1),
					  UNLOCKED);
	}
#endif
}
