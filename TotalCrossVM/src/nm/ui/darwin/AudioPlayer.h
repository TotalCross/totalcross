...
@interface AudioPlayer : AudioQueueObject {
  ...
  AudioQueueBufferRef
    buffers[kNumberAudioDataBuffers]; // the audio queue buffers for the audio
                                      // queue

  UInt32
    bufferByteSize; // the number of bytes to use in each audio queue buffer
  UInt32 numPacketsToRead; // the number of audio data packets to read into each
                           // audio queue buffer
...
- (void)copyMagicCookieToQueue:(AudioQueueRef)queue
                      fromFile:(AudioFileID)playbackFileID;
