...
- (id) init: (UIViewController*)ctrl {
	...
	firstCall = true;
	controller = ctrl;
	taskbarHeight = MIN([UIApplication sharedApplication].statusBarFrame.size.width, [UIApplication
...
- (CGRect) getBounds {
	...
	if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7) {
		if (firstCall
			&& taskbarHeight ==
			0) { // fix a problem that, if there's no statusbar, the viewDidLayoutSubviews is not called. So we force it by using at setup a smaller size, which will be fixed at a later call
			r.size.height--;
			firstCall = false;
		} else
...
- (void) processEvent: (NSSet*)touches withEvent: (UIEvent*)event {
	if ([ touches count ] == 1) {
		...
		if (touch != nil && (touch.phase == UITouchPhaseBegan || touch.phase == UITouchPhaseMoved
			...
			if (touch.phase == UITouchPhaseMoved
				&& [(MainViewController*)controller
					hasEvents]) { // ignore move events if the last one was not yet consumed
				return;
			}
			CGPoint point = [touch locationInView: self];
			[(MainViewController*)controller addEvent:
											 [[NSDictionary alloc] initWithObjectsAndKeys:
											  touch.phase == UITouchPhaseBegan ? @"mouseDown" : touch.phase == UITouchPhaseMoved ? @"mouseMoved" :
											  @"mouseUp", @"type",
