...
- (BOOL)isRunning
{
  ...
  result = AudioQueueGetProperty(
    queueObject, kAudioQueueProperty_IsRunning, &isRunning, &propertySize);
...
- (void)enableLevelMetering
{
  ...
  self.audioLevels = (AudioQueueLevelMeterState*)calloc(
    sizeof(AudioQueueLevelMeterState), audioFormat.mChannelsPerFrame);

  UInt32 trueValue = true;

  AudioQueueSetProperty(self.queueObject,
...
- (void)getAudioLevels:(Float32*)levels peakLevels:(Float32*)peakLevels
{
  ...
  UInt32 propertySize =
    audioFormat.mChannelsPerFrame * sizeof(AudioQueueLevelMeterState);
