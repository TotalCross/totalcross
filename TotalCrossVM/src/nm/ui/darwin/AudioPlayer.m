...
static void
playbackCallback(void* inUserData,
                 AudioQueueRef inAudioQueue,
                 AudioQueueBufferRef bufferReference)
{
  // This callback, being outside the implementation block, needs a reference to
  ...
  // This callback is called when the playback audio queue object has an audio
  // queue buffer available for filling with more data from the file being
  // played
  ...
  if (numPackets > 0) {
    ...
    [player setDonePlayingFile:YES]; // 'donePlayingFile' used by this callback
                                     // and by setupAudioQueueBuffers

    // if playback is stopping because file is finished, call AudioQueueStop
    // here; if user tapped Stop, then the AudioViewController calls
    // AudioQueueStop
...
static void
propertyListenerCallback(void* inUserData,
                         ...
                         AudioQueuePropertyID propertyID)
{
  ...
  if (player.audioPlayerShouldStopImmediately == YES) {
    ...
    // if the file reached the end, update the UI after the run loop has
    // finished.
    ...
    //[player.notificationDelegate	performSelector: @selector
    //(updateUserInterfaceOnAudioQueueStateChange:)
    //withObject: player
    // afterDelay: 0];
  }
}
...
//	so this app still works if you change the recording format to one that
// uses 	magic cookies.
...
- (void)setupPlaybackAudioQueueObject
{
  ...
  AudioQueueAddPropertyListener([self queueObject],
...
- (void)setupAudioQueueBuffers
{
  ...
  int bufferIndex;
  bool isFormatVBR =
    (audioFormat.mBytesPerPacket == 0 || audioFormat.mFramesPerPacket == 0);

  for (bufferIndex = 0; bufferIndex < kNumberAudioDataBuffers; ++bufferIndex) {

    // if you intend to support *only* constant bit-rate formats, you can
    // instead
    //	use AudioQueueAllocateBuffers. In this case, the playback callback
    // function 	needs to change; you need to change the arguments to the
    ...
    playbackCallback(
      (__bridge void*)self, [self queueObject], buffers[bufferIndex]);
...
- (void)play
{
  ...
  AudioQueueStart(self.queueObject,
...
- (void)calculateSizesFor:(Float64)seconds
{
  ...
  if (audioFormat.mFramesPerPacket) {
    ...
    // relationship between packets and time -- so we return a default buffer
    // size
    [self setBufferByteSize:maxBufferSize > maxPacketSize ? maxBufferSize
                                                          : maxPacketSize];
  }
  ...
  if (bufferByteSize > maxBufferSize && bufferByteSize > maxPacketSize) {
    ...
    // also make sure we're not too small - we don't want to go the disk for too
    // small chunks
