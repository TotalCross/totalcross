...
bool
initGLES(ScreenSurface screen)
{
  deviceCtx = screen->extension =
    (TScreenSurfaceEx*)malloc(sizeof(TScreenSurfaceEx));
  ...
  window.rootViewController =
    [(DEVICE_CTX->_mainview = [MainViewController alloc]) init];
...
{ // ios 6
  ...
  //  [self destroySIP]; - commented out because its the reason why the keyboard
  //  wont close on app launch
...
- (void)viewDidLayoutSubviews
{
  ...
  if (orientation != lastOrientationSentToVM) {
    ...
    [self addEvent:[[NSDictionary alloc]
                     initWithObjectsAndKeys:@"screenChange",
                                            @"type",
                                            [NSNumber numberWithInt:res.width],
                                            @"width",
                                            [NSNumber numberWithInt:res.height],
                                            @"height",
                                            nil]];
  }
}
...
- (void)loadView
{
  self.view = DEVICE_CTX->_childview = child_view =
    [[ChildView alloc] init:self];
...
- (void)showSIP:(SipArguments*)args
{
  ...
  if (options == SIP_HIDE) {
    ...
    if (wasNumeric != [args values].numeric) {
      [self destroySIP]; // Destroy to repaint keybaord with different type
    }
    wasNumeric = [args values].numeric;
    if ([args values].numeric) {
      kbd.keyboardType = UIKeyboardTypeDecimalPad;
    } else {
      kbd.keyboardType = UIKeyboardTypeDefault;
    }
          ...
          replacementText:(NSString*)text
{
  if ([text isEqualToString:@" "]) {
    [self addEvent:[[NSDictionary alloc]
                     initWithObjectsAndKeys:@"keyPress",
                                            @"type",
  ...
  if ([text isEqualToString:@"\n"]) { // Be sure to test for equality using the
  ...
  if ([text length] == 0) {
    lastRange.length = -1; // guich: fixed bug when pressing backspace and the
                           // next key was being ignored
    [self addEvent:[[NSDictionary alloc]
                     initWithObjectsAndKeys:@"keyPress",
                                            @"type",
                                            [NSNumber numberWithInt:(int)'\b'],
                                            @"key",
                                            nil]];
  } else if (lastRange.location <= 0 ||
             !NSEqualRanges(range, lastRange)) { // flsobral@tc126: avoid adding
                                                 // the same character twice.
    lastRange.location = range.location;
    lastRange.length = range.length;
    unsigned short* chars =
      (unsigned short*)[text cStringUsingEncoding:NSUnicodeStringEncoding];
    if (chars != null) {
      [self addEvent:[[NSDictionary alloc]
                       initWithObjectsAndKeys:@"keyPress",
                                              @"type",
                                              [NSNumber numberWithInt:chars[0]],
                                              @"key",
                                              nil]];
    }
  }
...
- (void)keyboardDidHide:(NSNotification*)notif
{
  keyboardH = 0;
  [self addEvent:[[NSDictionary alloc]
                   initWithObjectsAndKeys:@"sipClosed",
                                          @"type",
  ...
  for (AVMetadataObject* metadata in metadataObjects) {
    for (NSString* type in barCodeTypes) {
      if ([metadata.type isEqualToString:type]) {
        barCodeObject = (AVMetadataMachineReadableCodeObject*)[_prevLayer
        ...
        NSString* currBar = [NSString stringWithCString:barcode
                                               encoding:NSASCIIStringEncoding];
        if (![detectionString isEqualToString:currBar]) {
          timeSpentReadingTheSameBarCode =
            ([[NSDate date] timeIntervalSince1970] * 1000);
          strncpy(barcode,
                  [detectionString cStringUsingEncoding:NSASCIIStringEncoding],
                  MIN([detectionString length], sizeof(barcode)));
        } else {
...
- (BOOL)cameraClick:(NSString*)fileName width:(int)w height:(int)h type:(int)t
{
  ...
  if (t != 3) {
    if (authStatus == AVAuthorizationStatusAuthorized) {
      ...
      completionHandler:^(BOOL granted) { if(granted){ NSLog(@"Granted access to
      %@", AVMediaTypeVideo); } else { NSLog(@"Not granted access to %@",
      AVMediaTypeVideo);
          }
      }];
      */
  ...
  if (t == 3 || authStatus == AVAuthorizationStatusAuthorized) {
    ...
    if (o != UIDeviceOrientationLandscapeLeft &&
        o != UIDeviceOrientationLandscapeRight) { // when the camera comes back
                                                  // from landscape and we call
                                                  // updateLayout, the screen
                                                  // gets painted as if it was
                                                  // in portrait. this hack
                                                  // makes the screen a bit
  ...
  didFinishPickingMediaWithInfo:(NSDictionary*)info
{
  ...
  if (finalImage == NULL) {
    ...
    if (h > w &&
        imageW > imageH) { // if user selected a landscape resolution and the
                           // photo was taken in portrait, swap the resolution
      int t = imageW;
      imageW = imageH;
      imageH = t;
    }
...
- (BOOL)mapsShowAddress:(NSString*)address flags:(int)flags;
{
  ...
  if ((flags & USE_WAZE) != 0 &&
    ...
    stringURL =
      [NSString stringWithFormat:@"waze://?q=%@&navigate=yes", address];
  } else if ([address length] == 0) { // not working yet
    //      CLLocationCoordinates2D cl = [self getCurrentLocation];
    stringURL = [NSString
      stringWithFormat:
        @"http://maps.google.com/maps?q=Current%%20Location&z=14%@", type];
  } else if (c == '@') {
    stringURL =
      [NSString stringWithFormat:@"http://maps.google.com/maps?q=%@%%&z=14%@",
                                 [address substringFromIndex:1],
                                 type];
  } else {
    stringURL = [NSString
      stringWithFormat:@"http://maps.google.com/maps?q=%@%%&z=14%@",
           ...
           fromLocation:(CLLocation*)oldLocation
{
  ...
  if (locationCount > 0 && locationAge > 5.0) {
    return;
  }
  ...
  NSDateComponents* components = [[NSCalendar currentCalendar]
    components:NSDayCalendarUnit | NSMonthCalendarUnit | NSYearCalendarUnit
      fromDate:newLocation.timestamp];
  locationDate = (int)([components day] + [components month] * 100 +
                       [components year] * 10000);
  locationTime = (int)([components second] + [components minute] * 100 +
                       [components hour] * 10000);
}
...
fillIOSSettings(int* daylightSavingsPtr,
                ...
                char* timeZoneStrPtr,
                int sizeofTimeZoneStr)
{
  NSTimeZone* tz = [NSTimeZone systemTimeZone];
  *daylightSavingsPtr = [tz isDaylightSavingTime];
  *daylightSavingsMinutesPtr = (int)([tz daylightSavingTimeOffset] / 60);
  *timeZoneMinutesPtr =
    (int)(tz.secondsFromGMT / 60) -
    *daylightSavingsMinutesPtr; // as of 22/10/2013, android returns -180/60
                                // and ios returns -120/60
...
bool
iphone_mapsShowAddress(char* addr, int flags)
{
...
int
iphone_cameraClick(int w, int h, int t, char* fileName)
{
...
int
iphone_gpsUpdateLocation(BOOL* flags,
                         int* date,
                         int* time,
                         int* sat,
                         double* veloc,
...
void
vmClipboardCopy(JCharP string, int32 sLen)
{ // from Vm.c
  UIPasteboard* pb = [UIPasteboard generalPasteboard];
  [pb setString:[[[NSString alloc] initWithCharacters:string
                                               length:sLen] autorelease]];
}
unsigned short*
ios_ClipboardPaste()
{
