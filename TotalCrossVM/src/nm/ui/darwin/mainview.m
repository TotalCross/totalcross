...
- (BOOL) textView: (UITextView*)textView shouldChangeTextInRange: (NSRange)range replacementText:
	...
	if ([text isEqualToString: @" "]) {
		[self addEvent: [[NSDictionary alloc] initWithObjectsAndKeys: @"keyPress",
						 @"type", [NSNumber numberWithInt: ' '], @"key", nil]];
		return NO;
	}
	...
	if ([text isEqualToString:
		...
		[self addEvent: [[NSDictionary alloc] initWithObjectsAndKeys: @"keyPress",
						 @"type", [NSNumber numberWithInt: (int)'\n'], @"key", nil]];
		return FALSE; // Return FALSE so that the final '\n' character doesn't get added
	}
	if ([text length] == 0) {
		lastRange.length =
			-1; // guich: fixed bug when pressing backspace and the next key was being ignored
		[self addEvent: [[NSDictionary alloc] initWithObjectsAndKeys: @"keyPress",
						 @"type", [NSNumber numberWithInt: (int)'\b'], @"key", nil]];
	} else if (lastRange.location <= 0
			   || !NSEqualRanges(range, lastRange)) { //flsobral@tc126: avoid adding the same character twice.
		...
		if (chars != null) {
			[self addEvent: [[NSDictionary alloc] initWithObjectsAndKeys: @"keyPress",
							 @"type", [NSNumber numberWithInt: chars[0]], @"key", nil]];
		}
	}
		...
		for (NSString * type in barCodeTypes) {
			if ([metadata.type isEqualToString: type]) {
				...
				if (![detectionString isEqualToString: currBar]) {
					...
					strncpy(barcode, [detectionString cStringUsingEncoding: NSASCIIStringEncoding],
...
- (BOOL) cameraClick: (NSString*)fileName width: (int)w height: (int)h type: (int)t {
	...
	if (t == 3 || authStatus == AVAuthorizationStatusAuthorized) {
		...
		if (o != UIDeviceOrientationLandscapeLeft
			&& o != UIDeviceOrientationLandscapeRight) { // when the camera comes back from landscape and we call updateLayout, the screen gets painted as if it was in portrait. this hack makes the screen a bit better, but still buggy.
			[self updateLayout];
		}
	}
...
- (void) imagePickerController: (UIImagePickerController*)picker didFinishPickingMediaWithInfo:
	...
	if (finalImage == NULL) {
		...
		if (h > w && imageW >
			imageH) { // if user selected a landscape resolution and the photo was taken in portrait, swap the resolution
			int t = imageW;
			imageW = imageH;
			imageH = t;
		}
...
- (BOOL) mapsShowAddress: (NSString*)address flags: (int)flags;
{
	...
	if ((flags & USE_WAZE) != 0
		&& [[UIApplication sharedApplication] canOpenURL: [NSURL URLWithString: @"waze://"]]) {
		...
		stringURL = [NSString stringWithFormat: @"http://maps.google.com/maps?q=Current%%20Location&z=14%@",
							  type];
	} else if (c == '@') {
		stringURL = [NSString stringWithFormat: @"http://maps.google.com/maps?q=%@%%&z=14%@", [address
							  substringFromIndex: 1], type];
	} else {
		stringURL = [NSString stringWithFormat: @"http://maps.google.com/maps?q=%@%%&z=14%@", [address
							  stringByReplacingOccurrencesOfString: @" " withString: @"%20"], type];
	}
	dispatch_sync(dispatch_get_main_queue(), ^ {
...
- (void) locationManager: (CLLocationManager*)manager
	...
	NSDateComponents* components = [[NSCalendar currentCalendar] components: NSDayCalendarUnit |
																 NSMonthCalendarUnit | NSYearCalendarUnit fromDate: newLocation.timestamp];
...
void fillIOSSettings(int* daylightSavingsPtr, int* daylightSavingsMinutesPtr, int* timeZonePtr,
					 int* timeZoneMinutesPtr, char* timeZoneStrPtr, int sizeofTimeZoneStr) {
	NSTimeZone* tz = [NSTimeZone systemTimeZone];
	*daylightSavingsPtr = [tz isDaylightSavingTime];
	*daylightSavingsMinutesPtr = (int)([tz daylightSavingTimeOffset] / 60);
	*timeZoneMinutesPtr = (int)(tz.secondsFromGMT / 60) -
						  *daylightSavingsMinutesPtr; // as of 22/10/2013, android returns -180/60 and ios returns -120/60
...
int iphone_gpsUpdateLocation(BOOL* flags, int* date, int* time, int* sat, double* veloc,
							 double* pdop, double* dir, double* lat, double* lon) {
