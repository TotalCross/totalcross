...
- (void)presentYoutubeVideo:(char*)videoId
                      ...
                      start:(int)start
{
  ...
  //    YTPlaybackQuality qualityObj = [self getQualityByName:[NSString
  //    stringWithUTF8String:quality]];
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    //                           if(hlObjCString != null && ![hlObjCString
    //                           isEqualToString:@""]){
    //                               [playVars addEntriesFromDictionary:@{@"hl":
    //                               [NSString stringWithUTF8String:hl]}];
    //                           }

    [playerView loadWithVideoId:videoIdString
                     playerVars:(NSDictionary*)playVars];
  });
}

- (YTPlaybackQuality)getQualityByName:(NSString*)qualityStr
{
  if ([qualityStr isEqualToString:@"small"]) {
    return kYTPlaybackQualitySmall;
  }
  if ([qualityStr isEqualToString:@"medium"]) {
    return kYTPlaybackQualityMedium;
  }
  if ([qualityStr isEqualToString:@"large"]) {
    return kYTPlaybackQualityLarge;
  }
  if ([qualityStr isEqualToString:@"hd720"]) {
    return kYTPlaybackQualityHD720;
  }
  if ([qualityStr isEqualToString:@"hd1080"]) {
    return kYTPlaybackQualityHD1080;
  }
  if ([qualityStr isEqualToString:@"highres"]) {
    return kYTPlaybackQualityHighRes;
  }
  if ([qualityStr isEqualToString:@"auto"]) {
    return kYTPlaybackQualityAuto;
  }
  if ([qualityStr isEqualToString:@"default"]) {
    return kYTPlaybackQualityDefault;
  }
  if ([qualityStr isEqualToString:@"unknown"]) {
    return kYTPlaybackQualityUnknown;
  }
  return null;
}
...
- (void)playerView:(nonnull YTPlayerView*)playerView
     receivedError:(YTPlayerError)error
{
  ...
  Method onStateChange =
    getMethod(OBJ_CLASS(callback), false, "onStateChange", 1, J_INT);
...
- (void)addLoadingView
{
  ...
  spinnerView.backgroundColor = [UIColor colorWithRed:0.5
                                                green:0.5
                                                 blue:0.5
                                                alpha:0.5];
...
void
playYoutube(NMParams p)
{
  char url[128];
  if (p->obj[1] != null) {
    String2CharPBuf(p->obj[1], url);
  }
  ...
  [[[YoutubeController alloc] initWithCallBack:&p->obj[2]]
    presentYoutubeVideo:url
               autoPlay:autoPlay
                    end:end
                  start:start];
}
