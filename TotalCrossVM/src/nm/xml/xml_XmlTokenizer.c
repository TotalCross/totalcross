...
static void foundStartTagName(Context currentContext, TCObject xml, TCObject input, int32 offset,
							  int32 count) {
...
static void foundEndTagName(Context currentContext, TCObject xml, TCObject input, int32 offset,
							int32 count) {
	BoundMethods b = getBoundMethods(xml);
...
static void foundCharacterData(Context currentContext, TCObject xml, TCObject input, int32 offset,
							   int32 count) {
	BoundMethods b = getBoundMethods(xml);
...
static void foundAttributeName(Context currentContext, TCObject xml, TCObject input, int32 offset,
							   int32 count) {
	BoundMethods b = getBoundMethods(xml);
...
static void foundAttributeValue(Context currentContext, TCObject xml, TCObject input, int32 offset,
								int32 count, uint8 dlm) {
	BoundMethods b = getBoundMethods(xml);
...
static void foundComment(Context currentContext, TCObject xml, TCObject input, int32 offset,
						 int32 count) {
	BoundMethods b = getBoundMethods(xml);
...
static void foundProcessingInstruction(Context currentContext, TCObject xml, TCObject input,
...
static void foundDeclaration(Context currentContext, TCObject xml, TCObject input, int32 offset,
							 int32 count) {
	BoundMethods b = getBoundMethods(xml);
...
static void foundReference(Context currentContext, TCObject xml, TCObject input, int32 offset,
						   int32 count) {
	BoundMethods b = getBoundMethods(xml);
...
static void foundInvalidData(Context currentContext, TCObject xml, TCObject input, int32 offset,
							 int32 count) {
	BoundMethods b = getBoundMethods(xml);
...
static JChar resolveCharacterReference(Context currentContext, TCObject inputObj, int32 offset,
									   int32 count) {
...
static void throwSyntaxException(Context currentContext, int32 code, int32 offset) {
	TCObject exception = currentContext->thrownException = createObjectWithoutCallingDefaultConstructor(
...
static void tellReference(Context currentContext, TCObject xml, TCObject input, int32 offset,
						  int32 count) {
	if (XmlTokenizer_resolveCharRef(xml)) {
		...
		if (XmlTokenizer_strictlyXml(xml) && res == 0xFFFF) {
			throwSyntaxException(currentContext, XmlTokenizer_state(xml),
								 XmlTokenizer_ofsCur(xml) + XmlTokenizer_readPos(xml));
			return;
		}
...
TC_API void txXT_nativeCreate(NMParams
							  p) { // totalcross/xml/XmlTokenizer native private void nativeCreate();
	...
	if ((XmlTokenizer_bag(xml) = createByteArray(p->currentContext, sizeof(TBoundMethods))) != null) {
		...
		b->foundAttributeValue = getMethod(c, true, "foundAttributeValue", 4, BYTE_ARRAY, J_INT, J_INT,
										   J_BYTE);
		b->foundComment = getMethod(c, true, "foundComment", 3, BYTE_ARRAY, J_INT, J_INT);
		b->foundProcessingInstruction = getMethod(c, true, "foundProcessingInstruction", 3, BYTE_ARRAY,
										J_INT, J_INT);
...
//////////////////////////////////////////////////////////////////////////
TC_API void txXT_resolveCharacterReference_B(NMParams
		p) { // totalcross/xml/XmlTokenizer native public static JChar resolveCharacterReference(uint8 []input, int32 offset, int32 count);
	p->retI = resolveCharacterReference(p->currentContext, p->obj[0], p->i32[0], p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void txXT_setCdataContents_Bii(NMParams
									  p) { // totalcross/xml/XmlTokenizer native protected void setCdataContents(uint8 []input, int32 offset, int32 count);
	TCObject xml = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void txXT_endTokenize_B(NMParams
							   p) { // totalcross/xml/XmlTokenizer native private void endTokenize(uint8 []input) throws SyntaxException;
	endTokenize(p->currentContext, p->obj[0], p->obj[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void txXT_tokenizeBytes_B(NMParams
								 p) { // totalcross/xml/XmlTokenizer native private void tokenizeBytes(uint8 []input) throws SyntaxException;
	TCObject xml      = p->obj[0];
	TCObject inputObj = p->obj[1];
	...
	while (input < inputEnd) {
		...
		switch (state) {
			...
			case 1:
				...
				} else if (!endTokenize(currentContext, xml, inputObj)) { // strictly XML: give up
					goto end;
				}
				break;
			case 2:
			...
			case 7:
				...
				++ofsStart;
				foundAttributeValue(currentContext, xml, inputObj, ofsStart, _ofsCur - ofsStart,
