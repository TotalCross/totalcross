...
static BoundMethods
getBoundMethods(TCObject xml)
{
  return (BoundMethods)ARRAYOBJ_START(XmlTokenizer_bag(xml));
}

static void
foundStartTagName(Context currentContext,
                  TCObject xml,
                  TCObject input,
                  int32 offset,
                  int32 count)
{
  ...
  if (b->foundStartTagName != null) {
    executeMethod(
      currentContext, b->foundStartTagName, xml, input, offset, count);
  }
}

static void
foundEndTagName(Context currentContext,
                TCObject xml,
                TCObject input,
                int32 offset,
                int32 count)
{
  ...
  if (b->foundEndTagName != null) {
    executeMethod(
      currentContext, b->foundEndTagName, xml, input, offset, count);
  }
}

static void
foundEndEmptyTag(Context currentContext, TCObject xml)
{
  BoundMethods b = getBoundMethods(xml);
...
static void
foundCharacterData(Context currentContext,
                   TCObject xml,
                   TCObject input,
                   int32 offset,
                   int32 count)
{
  ...
  if (b->foundCharacterData != null) {
    executeMethod(
      currentContext, b->foundCharacterData, xml, input, offset, count);
  }
}

static void
foundCharacter(Context currentContext, TCObject xml, JChar charFound)
{
  BoundMethods b = getBoundMethods(xml);
...
static void
foundAttributeName(Context currentContext,
                   TCObject xml,
                   TCObject input,
                   int32 offset,
                   int32 count)
{
  ...
  if (b->foundAttributeName != null) {
    executeMethod(
      currentContext, b->foundAttributeName, xml, input, offset, count);
  }
}

static void
foundAttributeValue(Context currentContext,
                    TCObject xml,
                    TCObject input,
                    int32 offset,
                    int32 count,
                    uint8 dlm)
{
  ...
  if (b->foundAttributeValue != null) {
    executeMethod(
      currentContext, b->foundAttributeValue, xml, input, offset, count, dlm);
  }
}

static void
foundComment(Context currentContext,
             TCObject xml,
             TCObject input,
             int32 offset,
             int32 count)
...
static void
foundProcessingInstruction(Context currentContext,
                           TCObject xml,
                           TCObject input,
                           int32 offset,
                           int32 count)
{
  ...
  if (b->foundProcessingInstruction != null) {
    executeMethod(
      currentContext, b->foundProcessingInstruction, xml, input, offset, count);
  }
}

static void
foundDeclaration(Context currentContext,
                 TCObject xml,
                 TCObject input,
                 int32 offset,
                 int32 count)
{
  ...
  if (b->foundDeclaration != null) {
    executeMethod(
      currentContext, b->foundDeclaration, xml, input, offset, count);
  }
}

static void
foundReference(Context currentContext,
               TCObject xml,
               TCObject input,
               int32 offset,
               int32 count)
...
static void
foundInvalidData(Context currentContext,
                 TCObject xml,
                 TCObject input,
                 int32 offset,
                 int32 count)
{
  ...
  if (b->foundInvalidData != null) {
    executeMethod(
      currentContext, b->foundInvalidData, xml, input, offset, count);
  }
}

static void
foundEndOfInput(Context currentContext, TCObject xml, int32 count)
{
  BoundMethods b = getBoundMethods(xml);
...
static JChar
hex2char(uint8* input, int32 count)
{
  JChar res = 0;
  if (1 <= count && count <= 4)
    while (true) {
...
static JChar
dec2char(uint8* input, int32 count)
{
  JChar res = 0;
  if (count > 0)
    while (true) {
...
static JChar
ref2char(uint8* input, int32 count)
{
...
static JChar
resolveCharacterReference(Context currentContext,
                          TCObject inputObj,
                          int32 offset,
                          int32 count)
{
...
static void
throwSyntaxException(Context currentContext, int32 code, int32 offset)
{
  TCObject exception = currentContext->thrownException =
    createObjectWithoutCallingDefaultConstructor(
      currentContext, "totalcross.xml.SyntaxException");
  if (exception) {
    Method m =
      getMethod(OBJ_CLASS(exception), false, CONSTRUCTOR_NAME, 2, J_INT, J_INT);
    if (m) {
...
static void
tellReference(Context currentContext,
              TCObject xml,
              TCObject input,
              int32 offset,
              int32 count)
{
  if (XmlTokenizer_resolveCharRef(xml)) {
    ...
    if (XmlTokenizer_strictlyXml(xml) && res == 0xFFFF) {
      throwSyntaxException(currentContext,
                           XmlTokenizer_state(xml),
                           XmlTokenizer_ofsCur(xml) +
                             XmlTokenizer_readPos(xml));
      return;
    }
...
static bool
endTokenize(Context currentContext, TCObject xml, TCObject input)
{
  ...
  switch (XmlTokenizer_state(xml)) {
    ...
    case 10:
      if (dif > 0) {
        foundCharacterData(
          currentContext, xml, input, XmlTokenizer_ofsStart(xml), dif);
      }
      break;
    case 11:
  ...
  foundEndOfInput(
    currentContext, xml, XmlTokenizer_ofsCur(xml) + XmlTokenizer_readPos(xml));
  return true;
}

//////////////////////////////////////////////////////////////////////////
#define REMOVE_IF_ONLY_RETURNS_VOID(m)                                         \
  if (m->code[0].op.op == RETURN_void)                                         \
    m = null;
TC_API void
txXT_nativeCreate(NMParams p)
{ // totalcross/xml/XmlTokenizer native private
  // void nativeCreate();
  TCObject xml = p->obj[0];
  if (!xmlInitialized) {
    initialize();
  }
  if ((XmlTokenizer_bag(xml) =
         createByteArray(p->currentContext, sizeof(TBoundMethods))) != null) {
    TCClass c = OBJ_CLASS(xml);
    BoundMethods b = getBoundMethods(xml);
    b->foundStartTagName =
      getMethod(c, true, "foundStartTagName", 3, BYTE_ARRAY, J_INT, J_INT);
    b->foundEndTagName =
      getMethod(c, true, "foundEndTagName", 3, BYTE_ARRAY, J_INT, J_INT);
    b->foundCharacterData =
      getMethod(c, true, "foundCharacterData", 3, BYTE_ARRAY, J_INT, J_INT);
    b->foundAttributeName =
      getMethod(c, true, "foundAttributeName", 3, BYTE_ARRAY, J_INT, J_INT);
    b->foundAttributeValue = getMethod(
      c, true, "foundAttributeValue", 4, BYTE_ARRAY, J_INT, J_INT, J_BYTE);
    b->foundComment =
      getMethod(c, true, "foundComment", 3, BYTE_ARRAY, J_INT, J_INT);
    b->foundProcessingInstruction = getMethod(
      c, true, "foundProcessingInstruction", 3, BYTE_ARRAY, J_INT, J_INT);
    b->foundDeclaration =
      getMethod(c, true, "foundDeclaration", 3, BYTE_ARRAY, J_INT, J_INT);
    b->foundReference =
      getMethod(c, true, "foundReference", 3, BYTE_ARRAY, J_INT, J_INT);
    b->foundInvalidData =
      getMethod(c, true, "foundInvalidData", 3, BYTE_ARRAY, J_INT, J_INT);
    ...
    // now an optimization. We check if the method contains only a "return;". If
    // so, we null out the reference and then the method won't be called
...
//////////////////////////////////////////////////////////////////////////
TC_API void
txXT_resolveCharacterReference_B(NMParams p)
{ // totalcross/xml/XmlTokenizer native public static JChar
  // resolveCharacterReference(uint8 []input, int32 offset,
  // int32 count);
  p->retI = resolveCharacterReference(
    p->currentContext, p->obj[0], p->i32[0], p->i32[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void
txXT_setCdataContents_Bii(NMParams p)
{ // totalcross/xml/XmlTokenizer native protected void
  // setCdataContents(uint8 []input, int32 offset, int32 count);
  TCObject xml = p->obj[0];
  ...
  if ((XmlTokenizer_endTagToSkipTo(xml) =
         createByteArray(p->currentContext, count)) != null) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
txXT_endTokenize_B(NMParams p)
{ // totalcross/xml/XmlTokenizer native private void
  // endTokenize(uint8 []input) throws SyntaxException;
  endTokenize(p->currentContext, p->obj[0], p->obj[1]);
}
//////////////////////////////////////////////////////////////////////////
TC_API void
txXT_tokenizeBytes_B(NMParams p)
{ // totalcross/xml/XmlTokenizer native private void
  // tokenizeBytes(uint8 []input) throws SyntaxException;
  TCObject xml = p->obj[0];
  TCObject inputObj = p->obj[1];
  ...
  while (input < inputEnd) {
    ...
    switch (state) {
      ...
      case 1:
        ...
        } else if (!endTokenize(currentContext,
                                xml,
                                inputObj)) { // strictly XML: give up
          goto end;
        }
        break;
      case 2:
        ...
        foundStartTagName(
          currentContext, xml, inputObj, ofsStart + 1, _ofsCur - ofsStart - 1);
        break;
      case 3:
      ...
      case 4:
        ...
        foundAttributeName(
          currentContext, xml, inputObj, ofsStart, _ofsCur - ofsStart);
        break;
      case 5:
        while ((_is[ch] & ISSPACE) != 0) {
          if (++input >= inputEnd) {
      ...
      case 7:
        ...
        ++ofsStart;
        foundAttributeValue(currentContext,
                            xml,
                            inputObj,
                            ofsStart,
                            _ofsCur - ofsStart,
                            (uint8)XmlTokenizer_quote(xml));
        state = 8;
        break;
      case 8:
        if (ch == '>') {
      ...
      case 11:
        ...
        if (ch == ';') {
          ...
          if (ch == '<') {
            state = 1;
          } else if (ch != '&') { // spaces (else '&' again, stay here)
            state = 10;
          }
        } else if (!endTokenize(currentContext,
                                xml,
      ...
      case 13:
        ...
        foundEndTagName(
          currentContext, xml, inputObj, ofsStart + 2, _ofsCur - ofsStart - 2);
        break;
      case 14:
        while ((_is[ch] & ISSPACE) != 0) {
          if (++input >= inputEnd) {
      ...
      case 20:
        switch (substate) {
          ...
          case 1:
            if (ch == '>') {
              foundProcessingInstruction(currentContext,
                                         xml,
                                         inputObj,
                                         ofsStart + 2,
                                         _ofsCur - ofsStart - 3);
      ...
      case 24:
        if (ixEndTagToSkipTo ==
            (int32)ARRAYOBJ_LEN(XmlTokenizer_endTagToSkipTo(xml))) {
          ...
          foundCharacterData(
            currentContext, xml, inputObj, ofsStart, ofsTemp - ofsStart);
          ofsStart = ofsTemp;
          foundEndTagName(
            currentContext, xml, inputObj, ofsTemp + 2, ixEndTagToSkipTo);
          XmlTokenizer_endTagToSkipTo(xml) = null;
        } else {
          uint8* endTagToSkipTo =
            (uint8*)ARRAYOBJ_START(XmlTokenizer_endTagToSkipTo(xml));
          if ('a' <= ch) {
            ch -= ('a' - 'A'); // fast toUpper
          }
