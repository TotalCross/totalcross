...
static Err sendTextMessage(TCObject destinationAddress, TCObject scAddress, TCObject text) {
	...
	jmethodID sendTextMessageMethod = (*env)->GetStaticMethodID(env, jSmsManager4A, "sendTextMessage",
									  "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V");
	jstring jDestinationAddress = (*env)->NewString(env, String_charsStart(destinationAddress),
								  String_charsLen(destinationAddress));
	jstring jScAddress = (scAddress != null) ? (*env)->NewString(env, String_charsStart(scAddress),
						 String_charsLen(scAddress)) : null;
	jstring jText = (*env)->NewString(env, String_charsStart(text), String_charsLen(text));

	(*env)->CallStaticObjectMethod(env, jSmsManager4A, sendTextMessageMethod, jDestinationAddress,
								   jScAddress, jText);
...
static Err registerSmsReceiver(TCObject smsReceiver, int32 port) {
	JNIEnv* env = getJNIEnv();
	jmethodID enableSmsReceiverMethod = (*env)->GetStaticMethodID(env, jSmsManager4A,
										"enableSmsReceiver", "(ZI)V");

	(*env)->CallStaticObjectMethod(env, jSmsManager4A, enableSmsReceiverMethod,
								   (jboolean)(smsReceiver != null), (jint) port);
	return NO_ERROR;
}

void JNICALL Java_totalcross_Launcher4A_nativeSmsReceived(JNIEnv* env, jclass _class,
		jstring jDisplayOriginatingAddress, jstring jDisplayMessageBody, jbyteArray jUserData) {
	...
	TCObject displayOriginatingAddress = createStringObjectFromCharP(mainContext,
										 szDisplayOriginatingAddress, -1);
	TCObject displayMessageBody = szDisplayMessageBody == null ? null : createStringObjectFromCharP(
									  mainContext, szDisplayMessageBody, -1);
	...
	Method getDefaultMethod = getMethod(loadClass(mainContext, "totalcross.telephony.SmsManager",
										false), false, "getDefault", 0);
		...
		if (smsManager != null) {
			...
			if (smsReceiver != null) {
				...
				if (smsMessage != null) {
					...
					Method onReceiveMethod = getMethod(OBJ_CLASS(smsReceiver), true, "onReceive", 1,
...
static Err sendDataMessage(TCObject destinationAddress, TCObject scAddress, int32 port,
						   TCObject data) {
	JNIEnv* env = getJNIEnv();
	jmethodID sendDataMessageMethod = (*env)->GetStaticMethodID(env, jSmsManager4A, "sendDataMessage",
	...
	(*env)->CallStaticObjectMethod(env, jSmsManager4A, sendDataMessageMethod, jDestinationAddress,
								   jScAddress, port, jData);
	(*env)->DeleteLocalRef(env, jDestinationAddress);
	if (scAddress != null) {
		(*env)->DeleteLocalRef(env, jScAddress);
	}
