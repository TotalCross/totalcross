...
TC_API void tfiFII_getToken(NMParams p) {
	...
	jmethodID getToken = (*env)->GetStaticMethodID(env, jTcFirebaseUtils,
						 "getTokenFromRegisteredFirebaseApp", "()Ljava/lang/String;");
...
void JNICALL Java_totalcross_Launcher4A_nativeOnMessageReceived(JNIEnv* env, jclass _class,
		jstring messageId, jstring messageType,
	...
	if (keys != null) {
		...
		for (i = 0; i < size; ++i) {
			...
			CharP value = (*env)->GetStringUTFChars(env, (jstring)(*env)->GetObjectArrayElement(env, values, i),
													0);
			*((TCObjectArray) ARRAYOBJ_START(keysArray) + i) = createStringObjectFromCharP(mainContext, key,
					-1);
			*((TCObjectArray) ARRAYOBJ_START(valuesArray) + i) = createStringObjectFromCharP(mainContext, value,
					-1);
		}
	}
	...
	if (fmInstance != null) {
		...
		if (onMsgRcvd != null) {
			...
			if (messageId != null) {
				messageIdObj = createStringObjectFromCharP(mainContext, (*env)->GetStringUTFChars(env, messageId,
							   0), -1);
			}
			TCObject messageTypeObj = null;
			if (messageType != null) {
				messageTypeObj = createStringObjectFromCharP(mainContext, (*env)->GetStringUTFChars(env,
								 messageType, 0), -1);
			}
			TCObject collapsedKeyObj = null;
			if (collapsedKey != null) {
				collapsedKeyObj = createStringObjectFromCharP(mainContext, (*env)->GetStringUTFChars(env,
								  collapsedKey, 0), -1);
			}
