...
static Err CmGprsConfigure(Context currentContext, TCHARP szConnCfg) {
	...
	if (!isWindowsMobile || *tcSettings.romVersionPtr < 300) {
		...
		if (!(*apn == *username == *password == *domain == 0)) {
			_stprintf(xml,
					  TEXT("<wap-provisioningdoc><characteristic type=\"CM_GPRSEntries\"><characteristic type=\"%s\"><parm name=\"DestId\" value=\"{436EF144-B4FB-4863-A041-8F905A62C572}\"/><parm name=\"UserName\" value=\"%s\"/><parm name=\"Password\" value=\"%s\"/><parm name=\"Domain\" value=\"%s\"/><characteristic type=\"DevSpecificCellular\"><parm name=\"GPRSInfoValid\" value=\"1\"/><parm name=\"GPRSInfoAccessPointName\" value=\"%s\"/></characteristic></characteristic></characteristic></wap-provisioningdoc>"),
					  TEXT("TotalCrossGPRS"), username, password, domain, apn);

			if ((err = _DMProcessConfigXML((LPCWSTR) xml, 0x0001, &out)) == S_OK &&
				(err = _DMProcessConfigXML((LPCWSTR)
										   TEXT("<wap-provisioningdoc><characteristic type=\"CM_Planner\"><characteristic type=\"PreferredConnections\"><parm name=\"{436EF144-B4FB-4863-A041-8F905A62C572}\" value=\"TotalCrossGPRS\"/></characteristic></characteristic></wap-provisioningdoc>"),
										   0x0001, &out)) == S_OK &&
				(err = _DMProcessConfigXML((LPCWSTR)
										   TEXT("<wap-provisioningdoc><characteristic type=\"CM_ProxyEntries\"><characteristic type=\"HTTP-{ADB0B001-10B5-3F39-27C6-9742E785FCD4}\"><parm name=\"SrcId\" value=\"{ADB0B001-10B5-3F39-27C6-9742E785FCD4}\"/><parm name=\"DestId\" value=\"{436EF144-B4FB-4863-A041-8F905A62C572}\"/><parm name=\"Enable\" value=\"1\"/><parm name=\"Proxy\" value=\"new-inet:1159\"/><parm name=\"Type\" value=\"0\"/></characteristic></characteristic></wap-provisioningdoc>"),
										   0x0001, &out)) == S_OK) {
				err = NO_ERROR;
			}
		}
...
static Err CmGprsOpen(Context currentContext, NATIVE_CONNECTION* connHandle, int32 timeout,
					  bool* wasSuccessful) {
#if defined (WINCE)
	...
	if (isWindowsMobile && *tcSettings.romVersionPtr >= 300) {
		...
		*wasSuccessful = false;
		...
		if ((err = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
								TEXT("Comm\\ConnMgr\\Providers\\{7C4B7A38-5FF7-4bc1-80F6-5DA7870BB1AA}\\Connections"), 0, 0,
								&regKey)) != NO_ERROR)
			if ((err = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
									TEXT("SOFTWARE\\Microsoft\\ConnMgr\\Providers\\{7C4B7A38-5FF7-4bc1-80F6-5DA7870BB1AA}\\Connections"),
									0, 0, &regKey)) != NO_ERROR) {
				goto regError;
			}
		...
		for (i = 0 ; err == NO_ERROR ; i++) {
			...
			if ((err = RegEnumKeyEx(regKey, i, subkeyName, &subkeyNameLen, null, null, null,
									null)) == NO_ERROR) {
				if (((err = RegOpenKeyEx(regKey, subkeyName, 0, 0, &subKey)) == NO_ERROR)
					&& ((err = RegQueryValueEx(subKey, TEXT("Enabled"), null, null, (LPBYTE) &value,
											   &valueSize)) == NO_ERROR)
					&& ((err = RegQueryValueEx(subKey, TEXT("DestId"), null, null, (LPBYTE) &destid,
											   &destidSize)) == NO_ERROR)
					&& (value == 1)) {
					...
					if (tcscmp(destid, TEXT("{0DAEA92E-2917-4C6C-9E23-F2BCAA13DA07}")) != 0 || (_ConnMgrMapConRef
		...
		*wasSuccessful = _ConnMgrEstablishConnectionSync(&connectionInfo, connHandle, timeout,
						 &status) == S_OK;
		if (!*wasSuccessful) {
...
static Err CmOpen(Context currentContext, NATIVE_CONNECTION* connHandle, int32 timeout,
				  bool* wasSuccessful) {
#if defined (WINCE)
	RASCONN rasConn;
...
static Err CmClose(Context currentContext, NATIVE_CONNECTION* hConnection) {
	...
	for (i = dNumber ; i >= 0 ;
		 i--) //flsobral@tc115_52: fixed to properly search for an active modem connection and close it.
...
static void CmLoadResources(Context currentContext) {
	...
	if (isWindowsMobile && *tcSettings.romVersionPtr >= 300) {
		if (!cellcoreDll) {
			throwException(currentContext, RuntimeException,
						   "Could not load the required libraries for the ConnectionManager");
		} else {
			_ConnMgrEstablishConnectionSync = (ConnMgrEstablishConnectionSyncProc) GetProcAddress(cellcoreDll,
											  TEXT("ConnMgrEstablishConnectionSync"));
			_ConnMgrMapURL = (ConnMgrMapURLProc) GetProcAddress(cellcoreDll, TEXT("ConnMgrMapURL"));
			_ConnMgrReleaseConnection = (ConnMgrReleaseConnectionProc) GetProcAddress(cellcoreDll,
										TEXT("ConnMgrReleaseConnection"));
	...
	switch (type) {
		case CM_CRADLE: {
			...
			if (isWindowsMobile) { // first check the registry for windows mobile
				...
				if ((err = RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("System\\State\\Hardware"), 0, 0,
										&regKey)) != NO_ERROR) {
					goto finish;
				}
				if ((err = RegQueryValueEx(regKey, TEXT("Cradled"), null, null, (LPBYTE) &value,
										   &valueSize)) != NO_ERROR) {
					goto finish;
				}
...
boolean isWifiActive() {
	...
	TCHAR adapterGUID[MAX_PATHNAME] =
		TEXT("{98C5250D-C29A-4985-AE5F-AFE5367E5006}"); // {98C5250D-C29A-4985-AE5F-AFE5367E5006} is the GUID for power-managed NDIS miniports. That usually includes Wi-Fi driver.
	...
	if ((err = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
							TEXT("Software\\Microsoft\\WZCSVC\\Parameters\\Interfaces"), 0, 0, &regKey)) == NO_ERROR) {
	...
	if ((err = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
							TEXT("System\\CurrentControlSet\\Control\\Power\\State"), 0, 0, &regKey)) != NO_ERROR) {
		goto returnFalse;
	}
	if (tcslen(adapterName) > 0) {
		...
		for (i = 0 ;
			 (err = RegEnumValue(regKey, i, adapterState, &nameLen, null, null, null,
								 null)) != ERROR_NO_MORE_ITEMS ; i++) {
			if (err != NO_ERROR) {
				goto returnFalse;
			}
	...
	if ((err = RegQueryValueEx(regKey, adapterState, null, &valueType, (LPBYTE) &adapterStateValue,
							   &valueSize)) != NO_ERROR) {
		goto returnFalse;
	}
	RegCloseKey(regKey);
	...
	if (err != NO_ERROR && !isWindowsMobile) { // flsobral@tc123_27: WinCE specific check.
		...
		if ((err = RegQueryValueEx(regKey, TEXT("UpperBind"), null, &valueType, (LPBYTE) upperBind,
								   &valueSize)) != NO_ERROR) {
			goto returnFalse;
		}
		RegCloseKey(regKey);
		...
		if ((err = RegQueryValueEx(regKey, TEXT("DhcpIPAddress"), null, &valueType, null,
								   null)) != NO_ERROR) {
			goto returnFalse;
		}
		RegCloseKey(regKey);
