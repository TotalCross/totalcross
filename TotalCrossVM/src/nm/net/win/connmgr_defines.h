...
#ifndef _connmgr_defines_h_
#define _connmgr_defines_h_
...
extern "C"
{
  ...
  // DEFINE_GUID(IID_DestNetInternet,     0x436ef144, 0xb4fb, 0x4863, 0xa0,
  // 0x41, 0x8f, 0x90, 0x5a, 0x62, 0xc5, 0x72);
  ...
  // DEFINE_GUID(IID_DestNetCorp,         0xa1182988, 0x0d73, 0x439e, 0x87,
  // 0xad, 0x2a, 0x5b, 0x36, 0x9f, 0x80, 0x8b);
  const GUID IID_DestNetCorp = {
    0xa1182988,
    0x0d73,
    0x439e,
    { 0x87, 0xad, 0x2a, 0x5b, 0x36, 0x9f, 0x80, 0x8b }
  };

  // {7022E968-5A97-4051-BC1C-C578E2FBA5D9}
  // DEFINE_GUID(IID_DestNetWAP,          0x7022e968, 0x5a97, 0x4051, 0xbc,
  // 0x1c, 0xc5, 0x78, 0xe2, 0xfb, 0xa5, 0xd9);
  const GUID IID_DestNetWAP = {
    0x7022e968,
    0x5a97,
    0x4051,
    { 0xbc, 0x1c, 0xc5, 0x78, 0xe2, 0xfb, 0xa5, 0xd9 }
  };

  // {F28D1F74-72BE-4394-A4A7-4E296219390C}
  // DEFINE_GUID(IID_DestNetSecureWAP,    0xf28d1f74, 0x72be, 0x4394, 0xa4,
  // 0xa7, 0x4e, 0x29, 0x62, 0x19, 0x39, 0x0c);
  const GUID IID_DestNetSecureWAP = {
    0xf28d1f74,
    0x72be,
    0x4394,
    { 0xa4, 0xa7, 0x4e, 0x29, 0x62, 0x19, 0x39, 0x0c }
  };
...
//
// @comm The following flags in the dwParams field of the CONNMGR_CONNECTIONINFO
// structure define which optional parameters are valid.
//
// -----------------------------------------------------------------------------
#define CONNMGR_PARAM_GUIDDESTNET                                              \
  (0x1)                              // @constdefine guidDestNet field is valid
...
#define CONNMGR_PARAM_MAXCONNLATENCY                                           \
  (0x8) // @constdefine MaxConnLatency field is valid
...
//
// @comm The following flags in the dwFlags field of the CONNMGR_CONNECTIONINFO
// structure define special connection properties supported by client apps,
// typically which proxies are supported by each app. For example, each
// CONNMGR_FLAG_PROXY_* specifies that this connection can be specified by
// configuring an appropriate proxy server.  e.g. HTTP this is appropriate for
// Internet Explorer and other HTTP based protocols, including HTTP-DAV
// synchronization. If none of these flags are specified, then only a direct IP
// connection (or tunnel) will be attempted.
//
// -----------------------------------------------------------------------------
#define CONNMGR_FLAG_PROXY_HTTP (0x1) // @constdefine HTTP Proxy supported
#define CONNMGR_FLAG_PROXY_WAP                                                 \
  (0x2) // @constdefine WAP Proxy (gateway) supported
...
//
// -----------------------------------------------------------------------------
#define CONNMGR_FLAG_SUSPEND_AWARE                                             \
  (0x10) // @constdefine suspended connections supported
#define CONNMGR_FLAG_REGISTERED_HOME                                           \
  (0x20) // @constdefine only dial out if we're registered on the home network
#define CONNMGR_FLAG_NO_ERROR_MSGS                                             \
  (0x40) // @constdefine don't show any error messages for failed connections

  // -----------------------------------------------------------------------------
  //
  // @doc EXTERNAL
  //
  // @constants Priority | Defines which priority of the applications
  // connections
  //
  // @comm The following flags in the dwPriority field of the
  // CONNMGR_CONNECTIONINFO structure define the priority of the connection.
  // Connection manager arbitrates among multiple connections, satisfying as
  // many as possible at a time but giving preference to higher priorities
  //
  // -----------------------------------------------------------------------------
...
#define CONNMGR_PRIORITY_USERIDLE 0x0800
  ...
  // the user uses the application.  This helps ConnectionManager optimize
  // responsiveness to the interactive application, while sharing the connection
...
#define CONNMGR_PRIORITY_ALWAYS_ON 0x0001
  ...
  // applications, and other applications with an interest in a connection to
  // this network will be notified that the connection is available.  If true,
  // then this connection can not be shared with other applications, and no
  // other applications will be notified, and any application requesting a
  // connection to the same network will be treated as a contender for the same
  // resource, and not permitted to share the existing connection.  A decision
  // will be made between this connection and the others based on connection
  // priority.
  ...
  HRESULT WINAPI ConnMgrEstablishConnection(
    CONNMGR_CONNECTIONINFO*
      pConnInfo,         // @parm Params describing requested connection
...
#define CONNMGR_STATUS_SUSPENDED                                               \
  0x11 // @constdefine Connection is up but suspended

#define CONNMGR_STATUS_DISCONNECTED                                            \
  0x20 // @constdefine Connection is disconnected
#define CONNMGR_STATUS_CONNECTIONFAILED                                        \
  0x21 // @constdefine Connection failed and cannot not be reestablished
#define CONNMGR_STATUS_CONNECTIONCANCELED                                      \
  0x22 // @constdefine User aborted connection
#define CONNMGR_STATUS_CONNECTIONDISABLED                                      \
  0x23 // @constdefine Connection is ready to connect but disabled
#define CONNMGR_STATUS_NOPATHTODESTINATION                                     \
  0x24 // @constdefine No path could be found to destination
#define CONNMGR_STATUS_WAITINGFORPATH                                          \
  0x25 // @constdefine Waiting for a path to the destination
#define CONNMGR_STATUS_WAITINGFORPHONE                                         \
  0x26 // @constdefine Voice call is in progress
#define CONNMGR_STATUS_PHONEOFF                                                \
  0x27 // @constdefine Phone resource needed and phone is off
#define CONNMGR_STATUS_EXCLUSIVECONFLICT                                       \
  0x28 // @constdefine the connection could not be established because it would
       // multi-home an exclusive connection
#define CONNMGR_STATUS_NORESOURCES                                             \
  0x29 // @constdefine Failed to allocate resources to make the connection.
#define CONNMGR_STATUS_CONNECTIONLINKFAILED                                    \
  0x2A // @constdefine Connection link disconnected prematurely.
#define CONNMGR_STATUS_AUTHENTICATIONFAILED                                    \
  0x2B // @constdefine Failed to authenticate user.

#define CONNMGR_STATUS_WAITINGCONNECTION                                       \
  0x40 // @constdefine Attempting to connect
#define CONNMGR_STATUS_WAITINGFORRESOURCE                                      \
  0x41 // @constdefine Resource is in use by another connection
#define CONNMGR_STATUS_WAITINGFORNETWORK                                       \
  0x42 // @constdefine Network is used by higher priority thread or device is
       // roaming.

#define CONNMGR_STATUS_WAITINGDISCONNECTION                                    \
  0x80 // @constdefine Connection is being brought down
#define CONNMGR_STATUS_WAITINGCONNECTIONABORT                                  \
  0x81 // @constdefine Aborting connection attempt
  ...
  typedef HRESULT(__stdcall* ConnMgrConnectionStatusProc)
    ...
    (HANDLE hConnection, // @parm Handle to connection, returned from
                         // ConnMgrEstablishConnection
     DWORD* pdwStatus    // @parm Returns current connection status, one of
                         // CONNMGR_STATUS_*
    );
  ...
  HRESULT WINAPI ConnMgrProviderMessage(
    ...
    DWORD* pdwIndex, // @parm Optional index, used to address multiple providers
...
#define CONNMGR_MAX_DESC 128 // @constdefine Max size of a network description
  ...
  HRESULT WINAPI ConnMgrEnumDestinations(
    ...
    CONNMGR_DESTINATION_INFO*
      pDestInfo // @param ptr to structure to fill in with info about network
  );
  ...
  typedef struct _SCHEDULEDCONNECTIONINFO
  {
    ...
    TCHAR
    szToken[32]; // @field Unique token identifying this scheduled connection
    BOOL
      bPiggyback; // @field If true, execute app whenever network is available
  ...
  HRESULT WINAPI ConnMgrRegisterScheduledConnection(
    SCHEDULEDCONNECTIONINFO*
      pSCI // @param Ptr to struct describing scheduled connection
  );
  ...
  // API is ready to be used. Caller is responsible for calling CloseHandle on
  // the returned event.
  //
  // @comm none
  //
