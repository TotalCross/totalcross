...
static Err serverSocketCreate(SERVER_SOCKET* serverSocketHandle, int32 port, int32 backlog,
							  CharP addr) {
...
static Err serverSocketAccept(SERVER_SOCKET serverSocketHandle, SOCKET* socketHandle,
							  int32 timeoutMillis) {
	FD_SET fdReadSet;
	WSADATA WSAData;
	Err err;
