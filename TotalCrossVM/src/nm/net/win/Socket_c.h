 ...
 *
 *************************************/
static Err socketCreate(SOCKET* socketHandle, CharP hostname, int32 port, int32 timeout,
						bool noLinger, bool* isUnknownHost, bool* timedOut) {
 ...
 */
static Err socketReadWriteBytes(SOCKET socketHandle, int32 timeoutMillis, CharP buf, int32 start,
								int32 count, int32* retCount, bool isRead) {
	FD_SET fdSet;
	TIMEVAL timeout;
	int32 result;
	int32 timestamp;
	...
	*retCount = 0; // clear bytes count
	...
	if (result == 0) { // select timed out, update retCount and return
		*retCount = -2; // indicating it was a timeout.
		...
		goto Error;
						 ...
						 &fdSet)) { // select returned but socket is not set? return the error code.
		goto Error;
	}
	...
	do { // flsobral@tc113_33: loop back only on WSAEWOULDBLOCK, respecting the timeout.
		if (isRead) {
			...
			if (result ==
				0) { // flsobral@tc110_2: if result is 0, the connection was gracefully closed by the remote host.
				return NO_ERROR;
			}
		} else {
	...
	} while (result == SOCKET_ERROR && (err = WSAGetLastError()) == WSAEWOULDBLOCK
