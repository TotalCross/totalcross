...
static Err
socketClose(SOCKET* socketHandle);
 ...
 *************************************/
static Err
socketCreate(SOCKET* socketHandle,
             CharP hostname,
             int32 port,
             int32 timeout,
             bool noLinger,
             ...
             bool* timedOut)
{
  ...
  if ((destination_sin.sin_addr.s_addr = inet_addr(hostname)) == INADDR_NONE) {
    if ((phostent = gethostbyname(hostname)) == null) {
      ...
      xmemmove(&(destination_sin.sin_addr.s_addr),
               phostent->h_addr,
               phostent->h_length);
    }
  }
  ...
  if (connect(hostSocket,
    ...
    if (!FD_ISSET(hostSocket, &fdWriteSet)) {
      err = WSAETIMEDOUT; // flsobral@tc100b5_7: select sets the last error to
                          // S_OK, even when the connection times out.
      *timedOut = true;
      goto Finish;
    }
  }
...
/**
 * This function implements a timeout for reading and writing bytes using
 * timestamp because this timeout for these operations is only supported by
 * WindowsCE .NET, Windows 2000 and later versions. The function should be
 * improved for these devices, and use winsock2 instead of winsock when
 * available. For now, this will work for all Windows platforms.
 */
static Err
socketReadWriteBytes(SOCKET socketHandle,
                     int32 timeoutMillis,
                     CharP buf,
                     int32 start,
                     int32 count,
  ...
  *retCount = 0; // clear bytes count
  ...
  if (result == 0) { // select timed out, update retCount and return
    *retCount = -2;  // indicating it was a timeout.
    ...
    goto Error;
  } else if (!FD_ISSET(socketHandle,
                       &fdSet)) { // select returned but socket is not set?
                                  // return the error code.
    goto Error;
  }

  do { // flsobral@tc113_33: loop back only on WSAEWOULDBLOCK, respecting the
       // timeout.
    if (isRead) {
      result = recv(socketHandle,
                    buf + start + *retCount,
                    count - *retCount,
                    0);  // Read
      if (result == 0) { // flsobral@tc110_2: if result is 0, the connection was
                         // gracefully closed by the remote host.
        return NO_ERROR;
      }
    } else {
      result = send(socketHandle,
                    buf + start + *retCount,
                    count - *retCount,
                    0); // Write
    }
    *retCount += result; // update the number of bytes write/read
    Sleep(1);            // guich@tc122_5: tell the system that we're alive
           ...
           (getTimeStamp() - timestamp < timeoutMillis));
