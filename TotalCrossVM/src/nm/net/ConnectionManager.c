...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_loadResources(NMParams
							   p) { // totalcross/net/ConnectionManager native private void loadResources();
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_setDefaultConfiguration_is(NMParams
		p) { // totalcross/net/ConnectionManager native public static void setDefaultConfiguration(int type, String cfg) throws totalcross.io.IOException;
	int32 type = p->i32[0];
	...
	jmethodID setDefaultConfMethod = (*env)->GetStaticMethodID(env, jConnectionManager4A,
									 "setDefaultConfiguration", "(ILjava/lang/String;)V");
	jstring szConnCfg = !connCfg ? null : (*env)->NewString(env, String_charsStart(connCfg),
						String_charsLen(connCfg));
	...
	if (szConnCfg) {
		(*env)->DeleteLocalRef(env, szConnCfg);
	}
#else
	UNUSED(connCfg);
	UNUSED(type);
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_open(NMParams
					  p) { // totalcross/net/ConnectionManager native public static void open() throws totalcross.io.IOException;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_open_i(NMParams
						p) { // totalcross/net/ConnectionManager native public static void open(int type) throws totalcross.io.IOException;
#if defined (WINCE)
	int type = p->i32[0];
	TCObject* connRef;
	NATIVE_CONNECTION* connHandle;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_nativeClose(NMParams
							 p) { // totalcross/net/ConnectionManager native private static void nativeClose() throws totalcross.io.IOException;
#if defined (WINCE)
	Err err;
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_getHostAddress_s(NMParams
								  p) { // totalcross/net/ConnectionManager native public static String getHostAddress(String host) throws totalcross.net.UnknownHostException;
	...
	*szHostAddress = 0;
	...
	if (hostName == null) {
		...
		if (!(szHostName = String2CharP(hostName))) {
			...
			throwExceptionWithCode(p->currentContext, UnknownHostException,
								   err);    // flsobral@tc120: now we throw an exception
		} else if (xstrlen(szHostAddress) >
				   0) { //flsobral@tc115_43: must return null if the host address is not found.
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_getHostName_s(NMParams
							   p) { // totalcross/net/ConnectionManager native public static String getHostName(String host) throws totalcross.net.UnknownHostException;
#if defined (WIN32) || defined (ANDROID)
	...
	*szHostName = 0;
	...
	if (hostAddress == null) {
		...
		if (!(szHostAddress = String2CharP(hostAddress))) {
			...
			throwExceptionWithCode(p->currentContext, UnknownHostException,
								   err);    // flsobral@tc120_XX: now we throw an exception
		} else {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_getLocalHost(NMParams
							  p) { // totalcross/net/ConnectionManager native public static String getLocalHost() throws totalcross.net.UnknownHostException;
#if defined (WIN32) || defined (ANDROID)
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_getLocalHostName(NMParams
								  p) { // totalcross/net/ConnectionManager native public static String getLocalHostName() throws totalcross.net.UnknownHostException;
	char hostname[256];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_isAvailable_i(NMParams
							   p) { // totalcross/net/ConnectionManager native public static boolean isAvailable(int type) throws totalcross.io.IOException;
	int32 type = p->i32[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnCM_releaseResources(NMParams
								  p) { // totalcross/net/ConnectionManager native private void releaseResources();
#if defined (WINCE)
	CmReleaseResources();
#endif
}
