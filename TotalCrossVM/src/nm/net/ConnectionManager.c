...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_loadResources(NMParams p)
{ // totalcross/net/ConnectionManager native
  // private void loadResources();
  if (connMgrClass == null) {
    connMgrClass =
      loadClass(p->currentContext, "totalcross.net.ConnectionManager", true);
  }
#if defined(WINCE)
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_setDefaultConfiguration_is(NMParams p)
{ // totalcross/net/ConnectionManager native public static void
  // setDefaultConfiguration(int type, String cfg) throws
  ...
  jmethodID setDefaultConfMethod =
    (*env)->GetStaticMethodID(env,
                              jConnectionManager4A,
                              "setDefaultConfiguration",
                              "(ILjava/lang/String;)V");
  jstring szConnCfg = !connCfg ? null
                               : (*env)->NewString(env,
                                                   String_charsStart(connCfg),
                                                   String_charsLen(connCfg));
  (*env)->CallStaticVoidMethod(
    env, jConnectionManager4A, setDefaultConfMethod, type, szConnCfg);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_open(NMParams p)
{     // totalcross/net/ConnectionManager native public static
      // void open() throws totalcross.io.IOException;
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_open_i(NMParams p)
{ // totalcross/net/ConnectionManager native public static void
  // open(int type) throws totalcross.io.IOException;
#if defined(WINCE)
  int type = p->i32[0];
  TCObject* connRef;
  NATIVE_CONNECTION* connHandle;
    ...
    case CM_CELLULAR: {
      ...
      *connRef = createByteArray(p->currentContext, sizeof(NATIVE_CONNECTION));
      if (*connRef != null) {
        connHandle = (NATIVE_CONNECTION*)ARRAYOBJ_START(*connRef);
        if ((err =
               CmGprsOpen(p->currentContext, connHandle, -1, &wasSuccessful)) !=
            NO_ERROR) {
          throwExceptionWithCode(p->currentContext, IOException, err);
        }
        setObjectLock(*connRef, UNLOCKED);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_nativeClose(NMParams p)
{ // totalcross/net/ConnectionManager native private static void
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_getHostAddress_s(NMParams p)
{ // totalcross/net/ConnectionManager native public static
  // String getHostAddress(String host) throws
  ...
  *szHostAddress = 0;
  ...
  if (hostName == null) {
    ...
    } else if ((err = CmGetHostAddress(szHostName, szHostAddress)) !=
               NO_ERROR) {
      throwExceptionWithCode(p->currentContext,
                             UnknownHostException,
                             err); // flsobral@tc120: now we throw an exception
    } else if (xstrlen(szHostAddress) >
               0) { // flsobral@tc115_43: must return null if the host address
                    // is not found.
      p->retO =
        createStringObjectFromCharP(p->currentContext, szHostAddress, -1);
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_getHostName_s(NMParams p)
{ // totalcross/net/ConnectionManager native public static
  // String getHostName(String host) throws
  // totalcross.net.UnknownHostException;
#if defined(WIN32) || defined(ANDROID)
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_getLocalHost(NMParams p)
{ // totalcross/net/ConnectionManager native
  // public static String getLocalHost() throws
  // totalcross.net.UnknownHostException;
#if defined(WIN32) || defined(ANDROID)
...
#elif defined                                                                  \
  POSIX // https://stackoverflow.com/questions/33125710/how-to-get-ipv6-interface-address-using-getifaddr-function
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_getLocalHostName(NMParams p)
{ // totalcross/net/ConnectionManager native public static String
  // getLocalHostName() throws totalcross.net.UnknownHostException;
  char hostname[256];
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnCM_releaseResources(NMParams p)
{ // totalcross/net/ConnectionManager native
