...
static Err CmGetHostAddress(CharP hostName, CharP hostAddress) {
	...
	jmethodID getHostAddressMethod = (*env)->GetStaticMethodID(env, jConnectionManager4A,
	...
	jstring jString = (jstring)(*env)->CallStaticObjectMethod(env, jConnectionManager4A,
					  getHostAddressMethod, jHostName);
	xfree(jhostName);
	if (jString != null) {
...
static Err CmGetHostName(CharP hostAddress, CharP hostName) {
	JNIEnv* env = getJNIEnv();
	jmethodID getHostNameMethod = (*env)->GetStaticMethodID(env, jConnectionManager4A, "getHostName",
	...
	jstring jString = (jstring)(*env)->CallStaticObjectMethod(env, jConnectionManager4A,
					  getHostNameMethod, jHostAddress);
	xfree(jhostAddress);
	if (jString != null) {
...
static Err CmGetLocalHost(CharP address) {
	JNIEnv* env = getJNIEnv();
	jmethodID getLocalHostMethod = (*env)->GetStaticMethodID(env, jConnectionManager4A, "getLocalHost",
								   "()Ljava/lang/String;");
	jstring jString = (jstring)(*env)->CallStaticObjectMethod(env, jConnectionManager4A,
					  getLocalHostMethod);
	if (jString != null) {
