...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnSS_serversocketCreate_iiis(NMParams p)
{ // totalcross/net/ServerSocket native void
  // serversocketCreate(int port, int backlog, String host)
  ...
  if ((serverSocketRef =
         createByteArray(p->currentContext, sizeof(SERVER_SOCKET))) != null) {
    ServerSocket_serverRef(serverSocket) = serverSocketRef;
    serverSocketHandle = (SERVER_SOCKET*)ARRAYOBJ_START(serverSocketRef);
    if ((err = serverSocketCreate(
           serverSocketHandle, port, backlog, szAddress)) != NO_ERROR) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnSS_nativeAccept(NMParams p)
{ // totalcross/net/ServerSocket native public void
  // nativeAccept() throws totalcross.io.IOException;
  TCObject serverSocket = p->obj[0];
  ...
  if (newSocket != null && (socketRef = createByteArray(
                              p->currentContext, sizeof(SOCKET))) != null) {
    Socket_socketRef(newSocket) = socketRef;
    serverSocketHandle = (SERVER_SOCKET*)ARRAYOBJ_START(serverSocketRef);
    socketHandle = (SOCKET*)ARRAYOBJ_START(socketRef);
    *socketHandle = INVALID_SOCKET;

    if ((err = serverSocketAccept(
           *serverSocketHandle, socketHandle, timeout)) != NO_ERROR) {
      throwExceptionWithCode(p->currentContext, IOException, err);
      invalidateSocket(&newSocket);
    } else if (*socketHandle == INVALID_SOCKET) {
      invalidateSocket(&newSocket);
    }
  }
  if (newSocket) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnSS_nativeClose(NMParams p)
{ // totalcross/net/ServerSocket native private void
