...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnS_socketCreate_siib(NMParams p)
{ // totalcross/net/Socket native void socketCreate(final String
  // host, final int port, final int timeout, final boolean
  // noLinger);
  ...
  if ((socketRef = createByteArray(p->currentContext, sizeof(SOCKET))) !=
    ...
    if ((err = socketCreate(socketHandle,
      ...
      if (isUnknownHost) {
        ...
        throwException(
          p->currentContext, IOException, "Socket creation timed out.");
      } else {
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnS_nativeClose(NMParams p)
{ // totalcross/net/Socket native private void
...
//////////////////////////////////////////////////////////////////////////
TC_API void
tnS_readWriteBytes_Biib(NMParams p)
{ // totalcross/net/Socket native private int
  // readWriteBytes(byte []buf, int start, int count, boolean
  // isRead) throws totalcross.io.IOException;
  TCObject socket = p->obj[0];
  TCObject socketRef = Socket_socketRef(socket);
  SOCKET* socketHandle;
  ...
  socketHandle = (SOCKET*)ARRAYOBJ_START(socketRef);
  if ((err = socketReadWriteBytes(*socketHandle,
                                  timeout,
                                  (CharP)ARRAYOBJ_START(buf),
                                  start,
                                  count,
                                  &retCount,
                                  isRead)) != NO_ERROR) {
    ...
    throwException(
      p->currentContext, SocketTimeoutException, "Operation timed out");
...
int
tcSocketReadWrite(int fd, CharP buf, int32 count, bool isRead)
{
  int32 written = 0;
  int32 retCount;
  TCObject socket;
  ...
  do {
    err = socketReadWriteBytes(
      fd, timeout, buf, 0 + written, count - written, &retCount, isRead);
    if (retCount ==
        0) { // Gracefully closed by the remote, just break the loop.
      break;
    }
    if (retCount > 0) {
      written += retCount;
