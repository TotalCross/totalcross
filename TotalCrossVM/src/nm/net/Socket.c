...
//////////////////////////////////////////////////////////////////////////
TC_API void tnS_socketCreate_siib(NMParams
								  p) { // totalcross/net/Socket native void socketCreate(final String host, final int port, final int timeout, final boolean noLinger);
	...
	if ((socketRef = createByteArray(p->currentContext, sizeof(SOCKET))) != null) {
		...
		if ((err = socketCreate(socketHandle, szHost, port, timeout, noLinger, &isUnknownHost,
								&timedOut)) != NO_ERROR) {
			if (isUnknownHost) {
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnS_nativeClose(NMParams
							p) { // totalcross/net/Socket native private void nativeClose() throws totalcross.io.IOException;
	TCObject socket = p->obj[0];
...
//////////////////////////////////////////////////////////////////////////
TC_API void tnS_readWriteBytes_Biib(NMParams
									p) { // totalcross/net/Socket native private int readWriteBytes(byte []buf, int start, int count, boolean isRead) throws totalcross.io.IOException;
	TCObject socket = p->obj[0];
	TCObject socketRef = Socket_socketRef(socket);
	SOCKET* socketHandle;
	...
	socketHandle = (SOCKET*) ARRAYOBJ_START(socketRef);
	if ((err = socketReadWriteBytes(*socketHandle, timeout, (CharP)ARRAYOBJ_START(buf), start, count,
									&retCount, isRead)) != NO_ERROR) {
		throwExceptionWithCode(p->currentContext, IOException, err);
