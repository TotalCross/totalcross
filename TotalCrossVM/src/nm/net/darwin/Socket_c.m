...
int
iphoneSocket(char* hostname, struct sockaddr_in6* in_addr)
{
  ...
  hostnameStr =
    CFStringCreateWithCString(NULL, hostname, kCFStringEncodingASCII);
  if (hostnameStr == NULL) {
    return -2;
  }
  ...
  if (!success) {
    debug("error in CFHostStartInfoResolution: %d %d",
          (int)error.domain,
          (int)error.error);
    return -4;
  }
  ...
  //[(id)addresses autorelease]; - this causes SAV to abort the thread after
  // synchronizing the photos

  if (addresses != NULL) {
    count = CFArrayGetCount(addresses);
    for (index = 0; index < count; index++) {
      addr = (struct sockaddr_in6*)CFDataGetBytePtr(
