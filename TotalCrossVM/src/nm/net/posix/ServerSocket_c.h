...
static Err
nativeClose(SERVER_SOCKET serverSocketHandle)
{
...
static Err
serverSocketCreate(SERVER_SOCKET* serverSocketHandle,
                   int32 port,
                   int32 backlog,
                   CharP addr)
{
  ...
  if (setsockopt(
        serverSocketRef, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) == -1) {
    goto Error;
  }
  ...
  } else if ((internetAddr.sin_addr.s_addr = inet_addr(addr)) == -1) {
    if ((phostent = gethostbyname(addr)) == null) {
      goto Error;
    } else {
      internetAddr.sin_family = phostent->h_addrtype;
      xmemmove(
        &(internetAddr.sin_addr.s_addr), phostent->h_addr, phostent->h_length);
    }
  }
  ...
  if (bind(serverSocketRef,
           (struct sockaddr*)&internetAddr,
           sizeof(internetAddr)) < 0) {
    goto Error;
  }
...
static Err
serverSocketAccept(SERVER_SOCKET serverSocketHandle,
                   SOCKET* socketHandle,
                   int32 timeoutMillis)
{
  fd_set fdReadSet;
  ...
  timeout.tv_usec =
    (timeoutMillis < 1000 ? timeoutMillis : timeoutMillis % 1000) * 1000;
