             ...
             bool* timedOut)
{
...
#if defined(darwin)
  for (currentAddrInfo = addrInfoList; currentAddrInfo;
    ...
    if (portPtr != NULL &&
        ((*portPtr) == 0 || ((*portPtr) != htons((uint16)port)))) {
      (*portPtr) = htons((uint16)port);
    }

    if ((hostSocket = socket(currentAddrInfo->ai_family,
    ...
    res = connect(
      hostSocket, currentAddrInfo->ai_addr, currentAddrInfo->ai_addrlen);
    ...
    if (res < 0) {
      ...
      if ((res = select(
             hostSocket + 1, NULL, &fdWriteSet, NULL, &timeout_val)) < 0) {
        goto Timeout;
      }
      if (res == 0) {
        err = ETIMEDOUT;
  ...
  res = connect(
    hostSocket, (struct sockaddr*)&destination_sin, sizeof(destination_sin));
  if (res < 0) {
    if (errno != EINPROGRESS) {
      goto Error;
    }
    ...
    FD_ZERO(&fdWriteSet);
    FD_SET(hostSocket, &fdWriteSet);
    if ((res = select(hostSocket + 1, NULL, &fdWriteSet, NULL, &timeout_val)) <
        0) {
      goto Error;
    }
    if (res == 0) {
  ...
  if (result == 0) { // select timed out, update retCount and return
    *retCount = -2;  // indicating it was a timeout.
    return NO_ERROR; // do not display platform specific error.
             ...
             !FD_ISSET(socketHandle,
                       &fdSet)) { // select failed or socket is not set, return
                                  // the error code.
    goto Error;
  }

  do { // flsobral@tc113_33: loop back only on EWOULDBLOCK, respecting the
       // timeout.
    if (isRead) {
      result = (int)recv(
        socketHandle, buf + start + *retCount, count - *retCount, 0); // Read
      ...
      result = (int)send(
        socketHandle, buf + start + *retCount, count - *retCount, 0); // Write
    }
