...
#define INIT_MUTEX_VAR(x)                                                      \
  do { /*debug("INIT_MUTEX_VAR %s", #x);*/                                     \
    InitializeCriticalSectionEx(&(x), 1, 0);                                   \
  } while (0)
#else
#define INIT_MUTEX_VAR(x) InitializeCriticalSection(&(x))
#endif
#define RESERVE_MUTEX_VAR(x)                                                   \
  do { /*debug("LOCKVAR %s", #x);*/                                            \
...
void
threadDestroy(ThreadHandle h,
              bool threadDestroyingItself); // must be used when exiting the
                                            // application or the thread itself
void
threadDestroyAll(); // destroy all threads

void
freeMutex(int32 hash, VoidP mutex);
bool
