...
//#if defined(WIN32) && defined(_DEBUG) // use default libc on Visual C, because
// its easier to find corruption problems #define FORCE_LIBC_ALLOC #endif
...
TC_API TValue
executeMethod(Context currentContext, Method method, ...);
typedef TValue (*executeMethodFunc)(Context currentContext, Method method, ...);
/// check if the given range is inside the bounds of the array (start < x <=
/// end)
TC_API bool
checkArrayRange(Context currentContext, TCObject obj, int32 start, int32 count);
typedef bool (*checkArrayRangeFunc)(Context currentContext,
                                    TCObject obj,
                                    int32 start,
                                    int32 count);
