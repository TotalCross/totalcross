...
static VoidP getSFieldFromInterfaces(TCClass ext0, CharP fieldName, RegType t) {
	...
	TCClass ext;
	for (ext = ext0; ext;
		 ext = ext->superClass) // if didn't find in the classes, search in the interfaces
		for (i = ARRAYLENV(ext->interfaces) - 1; i >= 0; i--)
			for (f = fields = ext->interfaces[i]->staticFields[(int32)t], len = ARRAYLENV(fields); len-- > 0;
				 f++)
				if (strEq(f->name, fieldName)) {
...
uint16 getInstanceFieldIndex(CharP fieldName, CharP fieldClassName, TCObject o, RegType t) {
	...
	while (ext) {
		for (fields = ext->instanceFields[(int32)t], f = fields + ARRAYLENV(fields); --f >= fields;
			) // guich@tc110_101: must go backwards
			if (fields) {
				if (strEq(f->name, fieldName) && (found || (found = (f->sourceClassName == fieldClassName
												  || strEq(f->sourceClassName,
														  fieldClassName))))) { // guich@tc110_101: once the class was found, we can search all superclasses
...
double* getInstanceFieldDouble(TCObject instance, CharP fieldName,
							   CharP className) { // class name is the class where the field is declared, or null if from current class
...
uint16 getIField_Index(ConstantPool cp, TCObject o, int32 sym, RegType t) {
	if (sym == 0) {
		...
		uint16 idx = getInstanceFieldIndex(fieldName, className, o,
										   t); // the strEq is needed to avoid a NoSuchField that occurs if you try to get the "width" field in a class that extends MainWindow from inside the class that extends MainWindow
