...
static int32
getStaticFieldIndex(CharP fieldName, FieldArray fields)
{
  FieldArray f = fields;
...
int32*
getStaticFieldInt(TCClass c, CharP fieldName)
{
...
double*
getStaticFieldDouble(TCClass c, CharP fieldName)
{
...
int64*
getStaticFieldLong(TCClass c, CharP fieldName)
{
  int32 idx = getStaticFieldIndex(fieldName, c->v64StaticFields);
...
TCObject*
getStaticFieldObject(Context context, TCClass c, CharP fieldName)
{
  int32 idx = getStaticFieldIndex(fieldName, c->objStaticFields);
  if (idx >= 0) {
    TCObject* ret = &c->objStaticValues[idx];
    Field f = &c->objStaticFields[idx];
    if (strEq(f->targetClassName, "java.lang.Class") &&
        OBJ_CLASS(*ret)->flags.isString) {
      TNMParams params;
      tzero(params);
...
static VoidP
getSFieldFromInterfaces(TCClass ext0, CharP fieldName, RegType t)
{
  int32 i, len;
  FieldArray fields = null, f;
  TCClass ext;
  for (ext = ext0; ext; ext = ext->superClass) // if didn't find in the classes,
                                               // search in the interfaces
    for (i = ARRAYLENV(ext->interfaces) - 1; i >= 0; i--)
      for (f = fields = ext->interfaces[i]->staticFields[(int32)t],
          len = ARRAYLENV(fields);
           len-- > 0;
           f++)
        if (strEq(f->name, fieldName)) {
          ...
          switch (t) {
            case RegI:
              return &ext->interfaces[i]->i32StaticValues[idx];
            case RegO:
              return &ext->interfaces[i]->objStaticValues[idx];
            default:
              return &ext->interfaces[i]->v64StaticValues[idx];
          }
        }
  return null;
}
...
VoidP
getSField_Ref(Context currentContext, TCClass c, int32 sym, RegType t)
{
  ...
  FieldArray fields = null, f;
  TCClass ext = strEq(c->name, className)
                  ? c
                  : loadClass(currentContext, className, false),
          ext0 = ext;
  while (ext) {
    for (f = fields = ext->staticFields[(int32)t], len = ARRAYLENV(fields);
         len-- > 0;
         f++)
      if (strEq(f->name, fieldName) && strEq(f->sourceClassName, className)) {
        ...
        switch (t) {
          case RegI:
            return c->cp->boundSField[sym] = &ext->i32StaticValues[idx];
          case RegO:
            return c->cp->boundSField[sym] = &ext->objStaticValues[idx];
          default:
            return c->cp->boundSField[sym] = &ext->v64StaticValues[idx];
        }
      }
    ext = ext->superClass;
    if (ext) {
...
void
getSField_Names(ConstantPool cp, int32 sym, CharP* fieldName, CharP* className)
{
...
// used by both
uint16
getInstanceFieldIndex(CharP fieldName,
                      CharP fieldClassName,
                      TCObject o,
                      RegType t)
{
  bool found = false;
  ...
  while (ext) {
    for (fields = ext->instanceFields[(int32)t], f = fields + ARRAYLENV(fields);
         --f >= fields;) // guich@tc110_101: must go backwards
      if (fields) {
        if (strEq(f->name, fieldName) &&
            (found ||
             (found = (f->sourceClassName == fieldClassName ||
                       strEq(f->sourceClassName,
                             fieldClassName))))) { // guich@tc110_101: once the
                                                   // class was found, we can
                                                   // search all superclasses
...
int32*
getInstanceFieldInt(TCObject instance, CharP fieldName, CharP className)
{
...
double*
getInstanceFieldDouble(TCObject instance, CharP fieldName, CharP className)
{ // class name is the class where the field is declared,
  // or null if from current class
  uint16 idx = getInstanceFieldIndex(fieldName, className, instance, RegD);
  return idx < UNBOUND_ERROR ? &FIELD_DBL(instance, OBJ_CLASS(instance), idx)
                             : null;
}

int64*
getInstanceFieldLong(TCObject instance, CharP fieldName, CharP className)
{
  uint16 idx = getInstanceFieldIndex(fieldName, className, instance, RegL);
  return idx < UNBOUND_ERROR ? &FIELD_I64(instance, OBJ_CLASS(instance), idx)
                             : null;
}

TCObject*
getInstanceFieldObject(TCObject instance, CharP fieldName, CharP className)
{
  uint16 idx = getInstanceFieldIndex(fieldName, className, instance, RegO);
  return idx < UNBOUND_ERROR ? &FIELD_OBJ(instance, OBJ_CLASS(instance), idx)
                             : null;
}
...
void
getIField_Names(ConstantPool cp, int32 sym, CharP* fieldName, CharP* className)
{
...
uint16
getIField_Index(ConstantPool cp, TCObject o, int32 sym, RegType t)
{
  if (sym == 0) {
    return UNBOUND_FIELD_ERROR;
  } else {
    ...
    uint16 idx = getInstanceFieldIndex(
      fieldName,
      className,
      o,
      t); // the strEq is needed to avoid a NoSuchField that occurs if you try
          // to get the "width" field in a class that extends MainWindow from
          // inside the class that extends MainWindow
