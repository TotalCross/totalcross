...
/* These are the structures for Class, Method and Field that are persisted to
 * disk. The structures that will be kept into memory are available at tcvm.h.
 ...
 * below decreases performance on 32 bit systems. The standard is 4-byte
 * alignment.
 */
...
{ // guich@tc110_21
  ...
  params1 += 2; // params1 have the class+method+params, while params2 have only
                // the params
  while (--n1 >= 0) {
    s1 = *params1++;
    s2 = *params2++;
    if (((cp1 == cp2) ||
         (s1 < DEFAULT_CONSTANTS_LEN && s2 < DEFAULT_CONSTANTS_LEN)) &&
        s1 ==
          s2) { // fast comparison: same constant pools or default constants?
      continue;
    }
    p1 = cp1->cls[s1];
    p2 = cp2->cls[s2];
...
int32
getIndexInCP(ConstantPool cp, CharP s)
{ // currently used only by the test cases
  int32 i;
  if (cp) {
...
void
readConstantPool(Context currentContext, ConstantPool t, TCZFile tcz, Heap heap)
{
  ...
  if (t->sfieldCount > 0) {
    ...
    for (u = t->sfieldField + 1, i = t->sfieldCount; --i > 0; u++) {
      *u += *(u - 1);
    }
  ...
  if (t->ifieldCount > 0) {
    ...
    for (u = t->ifieldField + 1, i = t->ifieldCount; --i > 0; u++) {
      *u += *(u - 1);
    }
  ...
  if (t->mtdCount > 0) {
    uint8* lens;
    uint16* bunch;
    UInt16Array* ua = t->mtd = (UInt16Matrix)newArray(
      sizeof(UInt16Array),
      t->mtdCount,
      heap); // caution! this is not an uint16 array, but an uint16 matrix!
    lens = t->mtdLens = newPtrArrayOf(UInt8, t->mtdCount, heap);
    if (t->mtdCount > 1) { // guich@tc123_24: zero-length mtd has no written
    ...
    t->boundNormal =
      (MethodPtrArray)newArray(sizeof(Method), t->mtdCount, heap);
    t->boundVirtualMethod =
      (MethodAndClassPtrArray)newArray(TSIZE, t->mtdCount, heap);
  }
  ...
  if (t->strCount > 0) { // String constants must be the last loaded one!
    ...
    for (oa++, i = t->strCount; --i > 0; oa++) {
      ...
      if (mark < 254) {       // can && l < 254
        ...
        chars[mark] = 0;
        if ((*oa = createStringObjectFromCharP(currentContext, chars, mark)) ==
            null) {
  ...
  // now we can generate the constants for mtd, because the constants were
  // already loaded
  if (t->mtdCount > 0) {
    ...
    for (i = t->mtdCount; --i > 0; ua++) {
      methodHashCode(t->mtdfld[(*ua)[1]],
                     (*ua) + 2,
                     *lens++,
                     t,
                     ++hashNames,
                     ++hashParams); // generate a hash for quicker search
    }
  }
...
static RegType
getRegType(ConstantPool cp, uint16 idx)
{
  if (idx >= 10) {
    return RegO;
  }
...
static UInt8Array
genParamRegs(ConstantPool cp, UInt16Array params, Heap heap)
{
...
static CharP
appendFirstLetter(CharP fn, CharP pack)
{
  CharP next;
  while (true) {
    next = xstrchr(pack, '.');
    if (next != null) { // is this the part of a package name?
      *fn++ = *pack;
      ...
      for (; *pack;
           pack++) // this is the class name, so add all uppercase letters
...
static CharP
createMethodSignature(Method m, Heap h)
{
  char fn[100];
  CharP c = fn, mn;
  int32 n;
    ...
    for (p = m->cpParams; (c - fn) < 32 && n-- > 0; p++, c++) {
      ...
      if (*p < 10) { // primitive type?
        *c = toLower(*pname);
        ...
        bool isUp = false;
        for (; *pname == '['; pname++) { // use uppercase for arrays - stop the
                                         // for only when the [ sequence ends
          isUp = true;
        }
        if (isUp && *pname == '&') { // array of primitive?
          *c = *++pname;
          ...
          for (*c = *pname++; *pname;
               pname++) // guich@20131224: fix support for
                        // totalcross.db.sqlite.DB$ProgressObserver
            if (*pname == '.' || *pname == '$') {
...
static void
readMethod(ConstantPool cp, TCZFile tcz, Method m, TCClass c)
{
  ...
  methodHashCode(
    m->name, m->cpParams, m->paramCount, c->cp, &m->hashName, &m->hashParams);
  // read the opcodes
  if (ti.opcodeCount > 0) {
  ...
  if (ti.exceptionHandlersCount > 0) {
    m->exceptionHandlers =
      newArrayOf(Exception, ti.exceptionHandlersCount, tcz->tempHeap);
    for (i = ti.exceptionHandlersCount, ex = m->exceptionHandlers; i-- > 0;
         ex++) {
      tczRead(tcz, &exi, 10);
  ...
  if (ti.lineNumberInfoCount > 0) {
    ...
    uint8* t;
    m->lineNumberStartPC =
      newPtrArrayOf(UInt16,
                    ti.lineNumberInfoCount,
                    tcz->tempHeap); // guich@tc110_69: now the line number is
                                    // placed in two uint16 arrays.
    m->lineNumberLine =
      newPtrArrayOf(UInt16, ti.lineNumberInfoCount, tcz->tempHeap);
    tczRead(tcz, &first, 1);
    if (ti.lineNumberInfoCount == 1) {
      ...
      if (first == ALL_1) // all1 means pcs = 0,1,2,3,4,... p[0] is already 0
        ...
        *++p = first; // p[0] is always 0, so we skip it; put in p[1] the first
        ...
        if (ti.lineNumberInfoCount > 2) {
          ...
          tczRead(tcz,
                  t + 1,
                  ti.lineNumberInfoCount -
                    2); // read the byte array into the short array
          for (i = ti.lineNumberInfoCount - 2; i >= 1;
               i--) { // transform the byte array into a short array. p[0] and
                      // p[1] are already filled - guich@tc114_83: correct is
                      // -2, not -1
            p[i] = t[i];
            t[i] = 0;
          }
      ...
      if (t[2] == ALL_1)
        ...
        if (ti.lineNumberInfoCount > 2) {
          tczRead(tcz,
                  ...
                  ti.lineNumberInfoCount -
                    2); // read the byte array into the short array
          for (i = ti.lineNumberInfoCount - 1; i >= 2;
               i--) { // transform the byte array into a short array. p[0] and
                      // p[1] are already filled
            p[i] = t[i + 1];
            t[i + 1] = 0;
          }
        }
...
static FieldArray
readFields(ConstantPool cp,
           int32 len,
           TCZFile tcz,
           FieldArray super,
           CharP sourceClass)
{ // sourceClass: numeric fields must pass the
  // source class, object fields must pass null
  TFieldInfo ci;
  ...
  for (; len-- > 0; f++) {
    ...
    if (ci.cpIdxType) {
      f->targetClassName = cp->cls[ci.cpIdxType];
    }
  }
  return f0;
}
...
static TCClass
readClass(Context currentContext, ConstantPool cp, TCZFile tcz)
{
  ...
  {
    heapDestroy(heap);
    return !heap || heap->ex.errorCode == HEAP_MEMORY_ERROR
             ? CLASS_OUT_OF_MEMORY
             : null;
  }
  ...
  if (ci.interfacesCount > 0) {
    ...
    for (i = 0; i < ci.interfacesCount; i++) {
      ...
      if ((c->interfaces[i] = loadClass(currentContext, cp->cls[u16], true)) ==
          null) {
        HEAP_ERROR(heap, 11);
      }
    }
  ...
  if (ci.cpIdxSuperClass) { // java.lang.Object does not have a superclass
    c->superClass =
      loadClass(currentContext, cp->cls[ci.cpIdxSuperClass], true);
    if (c->superClass == null) {
      alert("readClass - Superclass not found: %s",
            cp->cls[ci.cpIdxSuperClass]); // TODO throw an exception
      HEAP_ERROR(heap, 10);
    }
    ...
    if (ci.i32StaticFieldsCount > 0) {
      c->i32StaticFields =
        readFields(cp, ci.i32StaticFieldsCount, tcz, null, c->name);
      c->i32StaticValues = newPtrArrayOf(Int32, ci.i32StaticFieldsCount, heap);
    }
    if (ci.objStaticFieldsCount > 0) {
      c->objStaticFields =
        readFields(cp, ci.objStaticFieldsCount, tcz, null, c->name);
      c->objStaticValues =
        newPtrArrayOf(TCObject, ci.objStaticFieldsCount, heap);
    }
    if (ci.v64StaticFieldsCount > 0) {
      c->v64StaticFields =
        readFields(cp, ci.v64StaticFieldsCount, tcz, null, c->name);
    ...
    if (ci.i32InstanceFieldsCount > 0 || superI32 > 0) {
      c->i32InstanceFields = readFields(cp,
                                        ci.i32InstanceFieldsCount,
                                        tcz,
                                        c->superClass->i32InstanceFields,
                                        c->name);
    }
    if (ci.objInstanceFieldsCount > 0 || superObj > 0) {
      c->objInstanceFields = readFields(cp,
                                        ci.objInstanceFieldsCount,
                                        tcz,
                                        c->superClass->objInstanceFields,
                                        c->name);
    }
    if (ci.v64InstanceFieldsCount > 0 || superV64 > 0) {
      c->v64InstanceFields = readFields(cp,
                                        ci.v64InstanceFieldsCount,
                                        tcz,
                                        c->superClass->v64InstanceFields,
                                        c->name);
    }
    ...
    // The fields are placed sequentialy: int1, int2, ..., obj1, obj2, ...,
    // double1/long1, double2/long2, ... In each type, this class instance
    // fields go first and non-private Inherited fields go LAST.
    c->objOfs = totalI32 * 4;
    c->v64Ofs = totalI32 * 4 + totalObj * TSIZE;

    // create a matrix of Fields so it can be easily accessed given the register
    // type
  ...
  if (c->finalizeMethod == null &&
      c->superClass != null) { // if no finalize methods are defined in this
                               // class, inherit it from the super class
    c->finalizeMethod = c->superClass->finalizeMethod;
  }
  // if (c->dontFinalize == null && c->superClass != null) c->dontFinalize =
  // c->superClass->dontFinalize;
  return c;
}
...
static void
freeClass(int32 i32, VoidP ptr)
{
  TCClass c = (TCClass)ptr;
  UNUSED(i32);
#ifdef TRACE_OBJCREATION
  debug("Destroying class %s, alloc count: %d, alloc total: %d, avail: %d, "
        "block count: %d",
        c->name,
        c->heap->numAlloc,
        c->heap->totalAlloc,
...
static int32
getArrayElementSize(CharP type)
{
  if (*type == '[') {
    type++;
  }
...
TCClass
loadClass(Context currentContext, CharP className, bool throwClassNotFound)
{ // do NOT throw error messages from here
  ...
  if (ret == null) {
    ...
    isArray = *className == '['; // if we're "loading" an array, then load the
                                 // java.lang.Array instead
    if (isArray) {
    ...
    if (tcz != null) {
      ...
      tczClose(tcz);
#ifdef TRACE_OBJCREATION
      debug("**** putting class %s in hashtable. Consumed: %d ****",
            realClassName,
            getFreeMemory(false) - fr);
#endif
      // call static initializer
      if (ret != null && ret != CLASS_OUT_OF_MEMORY) {
        TCClass ret2 = (TCClass)htGetPtr(
          &htLoadedClasses,
          hc); // guich@tc110_92: the class may have a reference to itself,
               // which was loaded in the readClass above. So we double-check
               // for it and use the previous version
        if (ret2 != null ||
            !htPutPtr(
              &htLoadedClasses,
              hc,
              ret)) { // class must be placed in the loaded classes before any
                      // method runs because the GC can be triggered and this
                      // class' objects may be collected
          freeClass(0, ret);
          ret = ret2;
        } else {
    ...
    if (ret != null && ret != CLASS_OUT_OF_MEMORY) {
      if (isArray) {
        ret->name = className;
        ret->flags.bits2shift =
          (uint16)getArrayElementSize(className);       // store the bit shift
  ...
  } else if (ret == null && throwClassNotFound) {
...
bool
isSuperClass(TCClass s, TCClass t)
{ // s instanceof t
  int32 i;
  for (; s != null; s = s->superClass)
    if (s == t) {
      return true;
    } else if (s->interfaces != null)
...
CompatibilityResult
areClassesCompatible(Context currentContext, TCClass s, CharP ident)
{ // S instanceof idenT ?
  TCClass t = null;
...
tryAgain:
  if (ident == null || className == null) {
    debug("areClassesCompatible: %X (%s) / %X (%s)",
          ident,
          ident ? ident : "null",
          className,
  ...
  if (strEq(ident, className)) { // quick test
    result = COMPATIBLE;
             ...
             *className == '&') { // if comparing a [String with a [&S, at the
                                  // second iteration will remain String &S,
                                  // which results to false.
    result = NOT_COMPATIBLE;
  } else if (!tIsArray &&
             (t = loadClass(currentContext,
                            ident,
                            true)) ==
               null) { // not an array but class was not found? - note: we
                       // should throw a ClassNotFoundException here
    result = TARGET_CLASS_NOT_FOUND;
  } else if (s == t) { // another quick test
    result = COMPATIBLE;
  } else if (strEq(className, "java.lang.String") &&
             strEq(ident, "java.lang.Class")) {
    result = COMPATIBLE;
  } else
    ...
    if (!t) {
      result = tIsArray
                 ? 0
                 : TARGET_CLASS_NOT_FOUND; // s not array but t is array ? not
                                           // compatible; otherwise, goto
    ...
    //  no else here - if T is an interface, check also if S has any subclasses
    //  that implements T
    ...
    if (!t) {
      ...
      // If T is an interface type, then T must be the same interface as S or
      // a superinterface of S (2.13.2).
      if (t->flags.isInterface) {
      result = ((s == t) || isSuperClass(s, t)) ? COMPATIBLE : NOT_COMPATIBLE;
    } else
    ...
    if (tIsArray) {
      ...
      if (sc < Type_Object && tc < Type_Object) {
        ...
        // TC and SC are reference types (2.4.6), and type SC can be cast to TC
    ...
    } else if (!t) { // a piece of an array?
      result = NOT_COMPATIBLE;
    } else
      //   If T is an interface type, T must be one of the interfaces
      //   implemented by arrays (2.15).
      if (t->flags.isInterface) {
      result = (strEq("java.lang.Cloneable", t->name) ||
                strEq("java.io.Serializable", t->name))
                 ? COMPATIBLE
                 : NOT_COMPATIBLE;
    }
    //   If T is a class type, then T must be Object (2.4.7).
