				...
				if (isUp && (*pname == '&') ) { // array of primitive?
					*c = *++pname;
					...
					for (*c = *pname++; *pname; pname++) // guich@20131224: fix support for totalcross.db.sqlite.DB$ProgressObserver
						if ((*pname == '.') || (*pname == '$') ) {
							*c = *(pname+1);
						}// don't break! we want the last piece
				}
...
tryAgain:
	...
	if (strEq(ident,className)) { // quick test
		result = COMPATIBLE;
	} else if ((*ident == '&') || (*className == '&') ) { // if comparing a [String with a [&S, at the second iteration will remain String &S, which results to false.
		result = NOT_COMPATIBLE;
	} else if (!tIsArray && ((t = loadClass(currentContext, ident, true)) == null) ) { // not an array but class was not found? - note: we should throw a ClassNotFoundException here
		result = TARGET_CLASS_NOT_FOUND;
