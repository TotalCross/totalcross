...
bool paramsEq(ConstantPool cp1, UInt16Array params1, int32 n1, ConstantPool cp2,
			  UInt16Array params2) { // guich@tc110_21
	int32 n2, s1, s2;
	CharP p1, p2;
	...
	while (--n1 >= 0) {
		s1 = *params1++;
		s2 = *params2++;
		if (((cp1 == cp2) || (s1 < DEFAULT_CONSTANTS_LEN && s2 < DEFAULT_CONSTANTS_LEN))
			&& s1 == s2) { // fast comparison: same constant pools or default constants?
			continue;
		}
		p1 = cp1->cls[s1];
		p2 = cp2->cls[s2];
...
void methodHashCode(CharP name, uint16* cpParams, int32 n, ConstantPool cp, int32* hashName,
					int32* hashParam) {
...
void readConstantPool(Context currentContext, ConstantPool t, TCZFile tcz, Heap heap) {
	...
	if (t->mtdCount > 0) {
		uint8* lens;
		uint16* bunch;
		UInt16Array* ua = t->mtd = (UInt16Matrix)newArray(sizeof(UInt16Array), t->mtdCount,
	...
	if (t->mtdCount > 0) {
		...
		for (i = t->mtdCount; --i > 0; ua++) {
			methodHashCode(t->mtdfld[(*ua)[1]], (*ua) + 2, *lens++, t, ++hashNames,
		...
		for (p = m->cpParams; (c - fn) < 32 && n-- > 0; p++, c++) {
			...
			if (*p < 10) { // primitive type?
				*c = toLower(*pname);
				...
				bool isUp = false;
				for (; *pname == '[';
					 pname++) { // use uppercase for arrays - stop the for only when the [ sequence ends
					isUp = true;
				}
				if (isUp && *pname == '&') { // array of primitive?
					*c = *++pname;
					...
					for (*c = *pname++; *pname;
						 pname++) // guich@20131224: fix support for totalcross.db.sqlite.DB$ProgressObserver
						if (*pname == '.' || *pname == '$') {
...
static void readMethod(ConstantPool cp, TCZFile tcz, Method m, TCClass c) {
	...
	if (ti.lineNumberInfoCount > 0) {
		...
		uint16* p, *pend;
		uint8* t;
		m->lineNumberStartPC = newPtrArrayOf(UInt16, ti.lineNumberInfoCount,
											 tcz->tempHeap); // guich@tc110_69: now the line number is placed in two uint16 arrays.
				...
				*++p = first; // p[0] is always 0, so we skip it; put in p[1] the first that was already read
				if (ti.lineNumberInfoCount > 2) {
					...
					for (i = ti.lineNumberInfoCount - 2; i >= 1;
						 i--) { // transform the byte array into a short array. p[0] and p[1] are already filled - guich@tc114_83: correct is -2, not -1
						p[i] = t[i];
						t[i] = 0;
					}
				}
			...
			if (t[2] == ALL_1)
				...
				if (ti.lineNumberInfoCount > 2) {
					...
					for (i = ti.lineNumberInfoCount - 1; i >= 2;
						 i--) { // transform the byte array into a short array. p[0] and p[1] are already filled
						p[i] = t[i + 1];
						t[i + 1] = 0;
					}
				}
...
static FieldArray readFields(ConstantPool cp, int32 len, TCZFile tcz, FieldArray super,
							 CharP sourceClass) { // sourceClass: numeric fields must pass the source class, object fields must pass null
	TFieldInfo ci;
...
static TCClass readClass(Context currentContext, ConstantPool cp, TCZFile tcz) {
	...
	if (ci.cpIdxSuperClass) { // java.lang.Object does not have a superclass
		...
		if (ci.i32InstanceFieldsCount > 0 || superI32 > 0) {
			c->i32InstanceFields = readFields(cp, ci.i32InstanceFieldsCount, tcz,
											  c->superClass->i32InstanceFields, c->name);
		}
		if (ci.objInstanceFieldsCount > 0 || superObj > 0) {
			c->objInstanceFields = readFields(cp, ci.objInstanceFieldsCount, tcz,
											  c->superClass->objInstanceFields, c->name);
		}
		if (ci.v64InstanceFieldsCount > 0 || superV64 > 0) {
			c->v64InstanceFields = readFields(cp, ci.v64InstanceFieldsCount, tcz,
											  c->superClass->v64InstanceFields, c->name);
		}
	...
	if (c->finalizeMethod == null
		&& c->superClass !=
		null) { // if no finalize methods are defined in this class, inherit it from the super class
...
static void freeClass(int32 i32, VoidP ptr) {
	...
	UNUSED(i32);
#ifdef TRACE_OBJCREATION
	debug("Destroying class %s, alloc count: %d, alloc total: %d, avail: %d, block count: %d", c->name,
		  c->heap->numAlloc, c->heap->totalAlloc, c->heap->totalAvail, c->heap->blocksAlloc);
#endif
	heapDestroy(c->heap);
}
...
TCClass loadClass(Context currentContext, CharP className,
				  bool throwClassNotFound) { // do NOT throw error messages from here
	...
	if (ret == null) {
		...
		if (tcz != null) {
			...
			tczClose(tcz);
#ifdef TRACE_OBJCREATION
			debug("**** putting class %s in hashtable. Consumed: %d ****", realClassName,
				  getFreeMemory(false) - fr);
#endif
			// call static initializer
			if (ret != null && ret != CLASS_OUT_OF_MEMORY) {
				TCClass ret2 = (TCClass)htGetPtr(&htLoadedClasses,
												 hc); // guich@tc110_92: the class may have a reference to itself, which was loaded in the readClass above. So we double-check for it and use the previous version
				if (ret2 != null
					|| !htPutPtr(&htLoadedClasses, hc,
								 ret)) { // class must be placed in the loaded classes before any method runs because the GC can be triggered and this class' objects may be collected
					freeClass(0, ret);
					ret = ret2;
				} else {
...
CompatibilityResult areClassesCompatible(Context currentContext, TCClass s,
		CharP ident) { // S instanceof idenT ?
	TCClass t = null;
...
tryAgain:
	if (ident == null || className == null) {
		debug("areClassesCompatible: %X (%s) / %X (%s)", ident, ident ? ident : "null", className,
	...
	if (strEq(ident, className)) { // quick test
		result = COMPATIBLE;
			   ...
			   || *className ==
			   '&') { // if comparing a [String with a [&S, at the second iteration will remain String &S, which results to false.
		result = NOT_COMPATIBLE;
	} else if (!tIsArray
			   && (t = loadClass(currentContext, ident,
								 true)) == null) { // not an array but class was not found? - note: we should throw a ClassNotFoundException here
		result = TARGET_CLASS_NOT_FOUND;
	} else if (s == t) { // another quick test
		result = COMPATIBLE;
	} else if (strEq(className, "java.lang.String") && strEq(ident, "java.lang.Class")) {
		result = COMPATIBLE;
	} else
		...
		if (!sIsArray) {
			if (!t) {
				result = tIsArray ? 0 :
						 TARGET_CLASS_NOT_FOUND;    // s not array but t is array ? not compatible; otherwise, goto throwClassNotFoundException
			} else
					...
					if (tIsArray) {
						...
						if (t->flags.isInterface) {
							result = (strEq("java.lang.Cloneable", t->name)
									  || strEq("java.io.Serializable", t->name)) ? COMPATIBLE : NOT_COMPATIBLE;
						}
					//   If T is a class type, then T must be Object (2.4.7).
						else {
							result = strEq("java.lang.Object", t->name) ? COMPATIBLE : NOT_COMPATIBLE;
