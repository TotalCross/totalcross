...
typedef struct {
uint16 bits2shift     :
	2; /// the number of bits to shift to compute the array's size for the object/primitive type that this class represents.
...
union TCode {
	...
	struct {
		...
		uint32 reg  : 8; // 6
	uint32 sym  :
		SYM16; // cp.sfield - index in constant pool for the full definition of the field (class and name)
	...
	struct {
		...
		uint32 reg   : 8; // 6
	uint32 sym   :
		SYM16; // cp.i32 / cp.obj / cp.dbl / cp.i64 / cp.identObj
	} reg_sym; // reg <-> sym
	struct {
		uint32 op    : 8;
		uint32 reg   : 6;
	...
	struct {
		...
		uint32 unused: 8;
	uint32 sym   :
		SYM16; // cp.i32 / cp.obj / cp.dbl / cp.i64
	} sym;
	struct {
		uint32 op    : 8;
	uint32 sym   :
		SYM12; // cp.cls or cp.identPrimitive
		uint32 regO  : 6;
...
typedef CompatibilityResult(*areClassesCompatibleFunc)(Context currentContext, TCClass c,
		CharP className);
