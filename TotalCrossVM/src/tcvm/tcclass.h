...
extern "C"
{
  ...
  /** IMPORTANT: there's a main difference between the XXX and XXXInfo
   * structures: the XXXInfo are PACKED at 2 bytes, which make it use less
   * memory main memory, but it makes the C compiled code bigger and slower. Due
   * to this, we decided to keep the structures separetely, even if most of them
   * are the same.
   */
...
#pragma pack(2) // make sure structure members are aligned at 2 bytes
  ...
  typedef struct
  {
    uint16 bits2shift : 2; /// the number of bits to shift to compute the
                           /// array's size for the object/primitive type that
    ...
    uint16 isStrict : 1;
    uint16 __available : 4; // prefer to fill the gap to prevent structure size
                            // optimization
  ...
  typedef struct
  {
    ...
    uint16 __available : 8; // prefer to fill the gap to prevent structure size
                            // optimization
  ...
  typedef struct
  {
    ...
    uint16 __available : 3; // prefer to fill the gap to prevent structure size
                            // optimization
...
#pragma pack() // restore structure member alignment to default
  ...
  typedef enum
  { // this order can't be changed, it impacts the compiler and the
    // ObjectMemoryManager.
    Type_Null,
    Type_Void,
    Type_Boolean,
...
#define TYPE_IS_PRIMITIVE(t) (Type_Boolean <= t && t <= Type_Double)
  ...
  typedef struct TObjectProperties
    TObjectProperties; // An object is prefixed with its properties
  typedef TObjectProperties* ObjectProperties;

  typedef ValueArray TCObject;     // an TCObject is an array of TValues
  typedef TCObject* TCObjectArray; // different from the other arrays! this one
                                   // is a pointer array
...
#define SYM16 16
  ...
  union TCode
  {
    ...
    struct
    {
      uint32 op : 8;
      uint32 reg : 8;     // 6
      uint32 sym : SYM16; // cp.sfield - index in constant pool for the full
  ...
  /// Definition of a NativeMethod: returns void and receives a NMParams
  /// structure.
  ...
  {            // guich@tc110_67: create a linked list of
               // bound methods
    TCClass c; // in
    Method m;  // out
   ...
   * . obj    stores the constants of type String (char16). These are real
   * String objects and can be directly assigned by the code; no conversion is
   * needed. These objects are never collected by the garbage collector. . cls
   * stores the class package+name identifiers, which are char arrays . mf
   * stores the method names and field names . mtd    stores the fully qualified
   * names of methods . sfield stores the fully qualified names of static fields
   * . ifield stores the fully qualified names of instance fields Important:
   * position 0 of the Contant Heap must not be used. Symbols must start from
   * index 1.
   */
  struct TConstantPool
  {
    UInt16Array boundIField;
    VoidPArray boundSField; // will store a pointer directly to the static field
                            // inside the class
...
#define FIELD_I64(o, c, idx)                                                   \
  (((int64*)(FIELD_V64_OFFSET(                                                 \
    o, c)))[idx]) // i64 and dbl point to the same structure
#define FIELD_DBL(o, c, idx) (((double*)(FIELD_V64_OFFSET(o, c)))[idx])

  /** This structure represents a Java class. */
  struct TTCClass
  {
    // The offsets to the object where each instance field type starts in an
    // TCObject
    uint16 objOfs,
      v64Ofs; // there's no i32Ofs because int32's offset is always 0
    ...
    CharP name;
    // The access flags of this class (e.g.: if this class is public, static,
    // etc)
    ClassFlags flags;
    // The fields that will be copied into each instance (an TCObject) of this
    // class.
    ...
    // The superclass of this class. The only class that has a null superClass
    // is java.lang.Object
    TCClass superClass;
  ...
  struct TMethod
  {
    ...
    TCClass class_; // 8
    // Signature of this method, containing indexes to the CP: name, number of
    // parameters, and the parameters
    CharP name; // 0xC
    uint16 paramCount;
    UInt16Array
      cpParams; // indexes to the constant pool of the parameters - 0x14
    UInt8Array
      paramRegs; // the RegType to where the parameters must go on - two bits
                 // would be enough, but we use more to improve speed
    int32 hashName, hashParams; // hash for the name and parameters
    // The return type, as an index to the constant pool. Zero for constructors
    // and methods that return void.
    uint16 cpReturn;
  ...
  /** This structure represents a Java int, double, long and TCObject class
   * field.
   */
  struct TField
  {
    // The field's name
    CharP name;
    // The access flags of this field (isPublic, isPrivate, isObject, isArray,
    // etc) and the Type
    FieldFlags flags;
    // The fully qualified class name
    CharP sourceClassName,
      targetClassName; // class where the field is, class type of the field
  };
...
#define J_BOOLEAN "b"
  ...
  TC_API TCClass loadClass(Context currentContext,
                           CharP className,
                           bool throwClassNotFound);
  typedef TCClass (*loadClassFunc)(Context currentContext,
                                   CharP className,
                                   bool throwClassNotFound);
  ...
  TC_API CompatibilityResult areClassesCompatible(Context currentContext,
                                                  TCClass c,
                                                  CharP className);
  typedef CompatibilityResult (*areClassesCompatibleFunc)(
    Context currentContext,
    TCClass c,
    CharP className);

  /// Checks if the methods have the same parameters
  bool paramsEq(ConstantPool cp1,
                UInt16Array params1,
                int32 n1,
                ConstantPool cp2,
                UInt16Array params2);
