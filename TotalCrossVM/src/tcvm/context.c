...
Context newContext(ThreadHandle thread, TCObject threadObj, bool bigContextSizes) {
	...
	if (mainContext != null && c != null) { // for the first context, it will be created later
		c->OutOfMemoryErrorObj = createObject(c,
											  "java.lang.OutOfMemoryError");    // create the exception and prevent the exception from being collected. Note that there's no need to lock the msg and trace strings inside of it.
	}
	return c;
}
...
void deleteContext(Context c, bool destroyThread) {
	...
	if (c->OutOfMemoryErrorObj != null) {
		setObjectLock(c->OutOfMemoryErrorObj, UNLOCKED);
	}
	UNLOCKVAR(omm);
...
static bool contextIncrease(Heap h, uint8** start, uint8** end, uint8** current, uint8** r,
							uint32 ssize, uint32 inc) {
	uint8* a;
	int32 oldLen, newLen;
...
bool contextIncreaseRegI(Context c, int32** r) {
	return contextIncrease(c->heap, (uint8**)&c->regIStart, (uint8**)&c->regIEnd, (uint8**)&c->regI,
						   (uint8**)r, sizeof(*c->regI), STARTING_REGI_SIZE / 4);
}
bool contextIncreaseRegO(Context c, TCObject** r) {
	return contextIncrease(c->heap, (uint8**)&c->regOStart, (uint8**)&c->regOEnd, (uint8**)&c->regO,
						   (uint8**)r, sizeof(*c->regO), STARTING_REGO_SIZE / 4);
}
bool contextIncreaseReg64(Context c, Value64* r) {
	return contextIncrease(c->heap, (uint8**)&c->reg64Start, (uint8**)&c->reg64End, (uint8**)&c->reg64,
						   (uint8**)r, sizeof(*c->reg64), STARTING_REG64_SIZE / 4);
}
bool contextIncreaseCallStack(Context c) {
	return contextIncrease(c->heap, (uint8**)&c->callStackStart, (uint8**)&c->callStackEnd,
						   (uint8**)&c->callStack, null, sizeof(*c->callStack), STARTING_STACK_SIZE / 4);
}
