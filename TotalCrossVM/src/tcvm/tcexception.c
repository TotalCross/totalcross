...
TC_API void throwException(Context currentContext, Throwable t, CharP message,
	...
	if (currentContext->thrownException !=
		null) { // do not overwrite a first exception, maybe the second one is caused by the first.
		return;
	}

	exception = t == OutOfMemoryError ? null : createObject(currentContext,
				exceptionClassName); // guich@tc110_40: use the already-created OOME object
	if (exception == null) {
...
TC_API void throwExceptionWithCode(Context currentContext, Throwable t, int32 errorCode) {
	...
	if (text != null) {
		...
		createException(currentContext, t, true, errorCode < -10000 ? "Error Code: %X" : "Error Code: %d",
						errorCode);
	}
}

TC_API void throwIllegalArgumentExceptionI(Context currentContext, CharP argName,
		int32 illegalValue) {
	char msg[128];
...
static CharP dumpMethodInfo(CharP c, Method m, int32 line,
							CharP end) { // waba.applet.Applet.registerMainWindow(Applet.java:441)
	CharP s, b;
	// totalcross.Launcher
	s = m->class_->name;
	while (c < end && *s) {
...
void showUnhandledException(Context context, bool useAlert) {
	...
	printf("Unhandled exception:\n%s:\n %s\n\nStack trace:\n%s\nAborting %s.",
		   OBJ_CLASS(thrownException)->name, msg == null ? "" : msg,
