...
void
printStackTraceFromObj(TCObject traceObj);

TC_API void
throwException(Context currentContext, Throwable t, CharP message, ...)
{ // throw an exception based on the Throwable enumeration
  TCObject exception;
  // note: code is duplicated here because the ... cannot be passed along
  // different routines
  CharP exceptionClassName = throwableAsCharP[(int32)t];

  if (currentContext->thrownException !=
      null) { // do not overwrite a first exception, maybe the second one is
              // caused by the first.
    return;
  }
  ...
  exception =
    t == OutOfMemoryError
      ...
      : createObject(currentContext,
                     exceptionClassName); // guich@tc110_40: use the
                                          // already-created OOME object
  if (exception == null) {
    currentContext->thrownException = exception =
      currentContext->OutOfMemoryErrorObj;
    //      debug("invalidating exception due to OutOfMemory (1). Stack
    //      trace:"); printStackTraceFromObj(*Throwable_trace(exception));
  ...
  if (message) {
    ...
    va_end(args);
    *Throwable_msg(exception) =
      createStringObjectFromCharP(currentContext, currentContext->exmsg, -1);
...
TC_API TCObject
createException(Context currentContext,
                Throwable t,
                bool fillStack,
                CharP message,
                ...)
{
  ...
  if (exception == null) {
    currentContext->thrownException = exception =
      currentContext->OutOfMemoryErrorObj;
    //      debug("invalidating exception due to OutOfMemory (2). Stack
    //      trace:"); printStackTraceFromObj(*Throwable_trace(exception));
    *Throwable_trace(exception) = null; // let a new trace be generated
  } else {
    currentContext->thrownException = exception;
...
TC_API void
throwExceptionNamed(Context currentContext,
                    CharP exceptionClassName,
                    CharP message,
                    ...)
{ // throw an exception
  TCObject exception;
...
TC_API void
throwExceptionWithCode(Context currentContext, Throwable t, int32 errorCode)
{
  CharP text;
  char errMsg[256];
  ...
  if (text != null) {
    createException(
      currentContext, t, true, "Error Code: %d - %s", errorCode, text);
  } else {
    createException(currentContext,
                    t,
                    true,
                    errorCode < -10000 ? "Error Code: %X" : "Error Code: %d",
                    errorCode);
  }
}

TC_API void
throwIllegalArgumentExceptionI(Context currentContext,
                               CharP argName,
                               int32 illegalValue)
{
  char msg[128];
...
TC_API void
throwIllegalArgumentException(Context currentContext, CharP argName)
{
  char msg[128];
...
TC_API void
throwIllegalArgumentIOException(Context currentContext,
                                CharP argName,
                                CharP argValue)
{
  char msg[128];
...
TC_API void
throwFileNotFoundException(Context currentContext, TCHARP path)
{
...
TC_API void
throwNullArgumentException(Context currentContext, CharP argName)
{
  char msg[128];
...
static CharP
dumpMethodInfo(CharP c, Method m, int32 line, CharP end)
{ // waba.applet.Applet.registerMainWindow(Applet.java:441)
  CharP s, b;
  // totalcross.Launcher
  s = m->class_->name;
  while (c < end && *s) {
...
void
fillStackTrace(Context currentContext,
               TCObject exception,
               int32 pc0,
               VoidPArray callStack)
{
  ...
  while (callStack > currentContext->callStackStart) {
    callStack -= 2;
    // int2hex((int32)callStack, 6, c); c += 6; *c++ = ' '; - used when
    // debugging
    m = (Method)callStack[0];
    im = (size_t)m;
    if (im < 1000 || (im & 3) != 0) {
      debug("breaking fillStackTrace due to invalid memory addresses");
      break; // trying to handle crash on addresses 0x33 and 0x36 and odd
             // addresses
    }
    oldpc = (Code)callStack[1];
    line = (m->lineNumberLine != null)
             ? locateLine(m, first ? pc0 : ((int32)(oldpc - m->code)))
             : -1;
  ...
  if (exception != null) {
    ...
    if (c != c0) { // was something filled in?
      if (currentContext != gcContext &&
          exception == currentContext->OutOfMemoryErrorObj) {
        debug("OutOfMemory:\n%s", c0);
      }
      *trace = createStringObjectFromCharP(currentContext, c0, (int32)(c - c0));
      if (*trace) {
        setObjectLock(*trace, UNLOCKED);
      } else if (currentContext != gcContext) {
        debug("Not enough memory to create the stack trace string. Dumping to "
              "here: %s\n%s",
              OBJ_CLASS(exception)->name,
              c0);
      } else if (exception != currentContext->OutOfMemoryErrorObj) {
        debug("Exception thrown in finalize: %s\n%s",
              OBJ_CLASS(exception)->name,
              c0); // guich@tc126_63
      }
    } else {
      *trace = null; // the trace may not be null if we're reusing
                     // OutOfMemoryErrorObj
    }
  }
#ifdef _DEBUG
...
void
printStackTrace(Context currentContext)
{
...
void
showUnhandledException(Context context, bool useAlert)
{
  TCObject o;
  ...
  printf("Unhandled exception:\n%s:\n %s\n\nStack trace:\n%s\nAborting %s.",
         OBJ_CLASS(thrownException)->name,
         msg == null ? "" : msg,
         throwableTrace == null ? "" : throwableTrace,
         useAlert ? "program" : "thread"); // always dump to the console
#endif
  if (useAlert) {
    alert(
      "Unhandled exception:\n%s:\n %s\n\nStack trace:\n%s\nAborting program.",
...
initException()
{
  ...
  throwableAsCharP[IllegalArgumentException] =
    "java.lang.IllegalArgumentException";
  throwableAsCharP[ImageException] = "totalcross.ui.image.ImageException";
  throwableAsCharP[IndexOutOfBoundsException] =
  ...
  throwableAsCharP[IllegalArgumentIOException] =
    "totalcross.io.IllegalArgumentIOException";
  throwableAsCharP[UnknownHostException] =
    "totalcross.net.UnknownHostException";
  throwableAsCharP[SocketTimeoutException] =
  ...
  throwableAsCharP[InvalidNumberException] =
    "totalcross.sys.InvalidNumberException";
  throwableAsCharP[ElementNotFoundException] =
    "totalcross.util.ElementNotFoundException";
  ...
  throwableAsCharP[NegativeArraySizeException] =
    "java.lang.NegativeArraySizeException";
  throwableAsCharP[InvocationTargetException] =
  ...
  throwableAsCharP[NotInstalledException] =
    "totalcross.util.NotInstalledException";
  throwableAsCharP[GPSDisabledException] =
