	...
	if (o) {
		...
		if (oSize > size) {
			...
			if (objectBytesRemaining >= MIN_SPACE_LEFT) { // is there enough space to create another minimum object?
				...
				if (_TRACE_OBJCREATION) {
					debug("G Allocd: %5d. Remaining object: %X with size %d at index %d. lock: %d. context: %X",size,oremain,objectBytesRemaining,ridx,OBJ_ISLOCKED(oremain),currentContext);
				}
...
static TCObject privateCreateObject(Context currentContext, CharP className, bool callDefaultConstructor) {
	...
	if (_TRACE_OBJCREATION) {
		debug("G %X obj created %s of size %d at %d. lock: %d. mark: %d. context: %X", o, className, objectSize, size2idx(objectSize), OBJ_ISLOCKED(o), markedAsUsed, currentContext);
	}
...
TCObject createArrayObject(Context currentContext, CharP type, int32 len) {
	...
	if (_TRACE_OBJCREATION) {
		debug("G %X array obj created %s len %d, size = %d at %d. lock: %d", o, c->name,len, objectSize, size2idx(objectSize), OBJ_ISLOCKED(o));
	}
end:
	return o;
}
...
static bool joinAdjacentObjects(uint8* block, uint32 size) {
	...
	if (COMPUTETIME) {
		int32 tt = size-sizeof(TObjectProperties), uu = usedSize > 0 ? (usedSize-sizeof(TObjectProperties)) : 0; debug("G Chunk %X size %d has %d used objects with %d bytes (%d%%)",block0, tt, usedCount, uu, uu * 100 / tt);
	}
	return false;
}
