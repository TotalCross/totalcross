...
static void removeNodeFromDblList(TCObject headObj, TCObject nodeObj) {
	...
	if (node->next) { // not the last node?
		...
		head->prev = node->prev == headObj ? null :
					 node->prev;    // if this is the last node being removed, we must set to null; otherwise, store the pre-last as last
	}
	...
	if (nEnd > 0 && nEnd != (nIni - 1)) {
		debug("M @@@@@@@ Error while removing node %X! Size should be %d, but is %d (idx = %d)", nodeObj,
			  nIni - 1, nEnd, size2idx(OBJ_SIZE(nodeObj)));
	}
#endif
}
...
static void insertNodeInDblList(TCObject headObj, TCObject nodeObj) { // insert at the head
	...
	if (!head->prev) {
		head->prev =
			nodeObj;    // make the prev in head point to the last: since all nodes are inserted at the start, the last item is the first inserted
	}
	...
	nEnd = countObjectsInList(headObj, false, -1, null);
	if (nEnd != (nIni + 1)) {
		debug("M @@@@@@@ Error while inserting node %X! Size should be %d, but is %d", nodeObj, nIni + 1,
			  nEnd);
	}
#endif
}
...
static bool createChunk(uint32 size) {
	...
	debug("M +++ %2d Created chunk %X size %d, first obj: %X",
		  tcSettings.chunksCreated ? *tcSettings.chunksCreated : 1, chunk, size, o);
#endif
	return true;
}
...
void destroyObjectMemoryManager() {
	if (IS_VMTWEAK_ON(VMTWEAK_DUMP_MEMORY_STATS)) {
		debug("M Times gc was called: %d. Total gc time: %d. Chunks created: %d. Max allocated: %d",
			  tcSettings.gcCount ? *tcSettings.gcCount : 0, tcSettings.gcTime ? *tcSettings.gcTime : 0,
			  tcSettings.chunksCreated ? *tcSettings.chunksCreated : 1, maxAllocated);
	}
...
static TCObject allocObjWith(uint32 size) {
	...
	if (idx == OBJARRAY_MAX_INDEX
		...
		null) { // if this is the max index, or there's no free objects in the current index, try on the highest index
	...
	if (o) {
		...
		if (oSize > size) {
			...
			if (objectBytesRemaining >=
				MIN_SPACE_LEFT) { // is there enough space to create another minimum object?
				...
				if (_TRACE_OBJCREATION) {
					debug("G Allocd: %5d. Remaining object: %X with size %d at index %d. lock: %d. context: %X", size,
						  oremain, objectBytesRemaining, ridx, OBJ_ISLOCKED(oremain), currentContext);
				}
				// and put it in the used list
			} else {
				size = oSize;    // not enough memory remains in the object, so keep the old size
			}
		}
		OBJ_SIZE(o) = size;
...
static TCObject privateCreateObject(Context currentContext, CharP className,
	...
	if (_TRACE_OBJCREATION) {
		debug("G %X obj created %s of size %d at %d. lock: %d. mark: %d. context: %X", o, className,
			  objectSize, size2idx(objectSize), OBJ_ISLOCKED(o), markedAsUsed, currentContext);
	}
...
TCObject createArrayObject(Context currentContext, CharP type, int32 len) {
	...
	if (_TRACE_OBJCREATION) {
		debug("G %X array obj created %s len %d, size = %d at %d. lock: %d", o, c->name, len, objectSize,
			  size2idx(objectSize), OBJ_ISLOCKED(o));
	}
end:
	return o;
}
...
TCObject createArrayObjectMulti(Context currentContext, CharP type, int32 count, uint8* dims,
								int32* regI) {
	uint32 len;
	TCObject o;
	TCObject* oa;
	...
	if (o != null && count > 1) {
		for (oa = (TCObject*)ARRAYOBJ_START(o); len-- > 0; oa++)
			if ((*oa = createArrayObjectMulti(currentContext, type + 1, count - 1,
											  dims == null ? null : dims + 1, dims == null ? regI + 1 : regI)) == null) {
				return null;
			} else {
...
TCObject createStringObjectWithLen(Context currentContext, int32 len) {
	TCObject str;
	str = createObjectWithoutCallingDefaultConstructor(currentContext,
			"java.lang.String"); // do not call default constructor, we'll set our own char array (2 lines below)
	if (str) {
...
TC_API void setObjectLock(TCObject o, LockState lock) {
	...
	if (lock == LOCKED) {
		if (OBJ_ISLOCKED(o)) {
			alert("FATAL ERROR: OBJECT %X (%s) IS BEING LOCKED BUT IT IS ALREADY LOCKED!", o,
				  OBJ_CLASS(o)->name);
		}
		OBJ_SETLOCKED(o);
		...
		if (!OBJ_ISLOCKED(o)) {
			alert("FATAL ERROR: OBJECT %X (%s) IS BEING UNLOCKED BUT IT IS ALREADY UNLOCKED!", o,
				  OBJ_CLASS(o)->name);
		}
		OBJ_SETUNLOCKED(o);
...
static void markSingleObject(TCObject o,
							 bool dump) { // NEVER call this directly, unless the Object has no instance fields nor is an array
	TCClass c;
	...
	if (!OBJ_ISLOCKED(
			o)) { // locked objects can't be revived, since they are not in the used/free lists
		// "revive" the object
		int32 size, idx;
		...
		insertNodeInDblList(usedList[idx], o);
		if (_TRACE_OBJCREATION) {
			debug("G Object revived: %X (%s). mark: %d", o, OBJ_CLASS(o)->name, OBJ_MARK(o));
		}
	}
	...
	if (c->flags.isObjectArray) { // array of objects or array of arrays?
		...
		if (c->objInstanceFields != null) {
			...
			objs.n = (int32)((TCObjectArray)FIELD_V64_OFFSET(o,
							 c) - objs.start); // the object fields ends where the 64-bit ones start.
			stackPush(objStack, &objs);
		}
}
...
static void markObjects(TCObject o, bool dump) {
	...
	if (!o) {
		return;    // can occurr if concorrent threads are accessing the structure where this object is
	}
	...
	for (o = OBJ_PROPERTIES(o)->next; o != null; o = OBJ_PROPERTIES(o)->next) {
		...
		if (mark >= 0)
			if ((int32)op->mark != mark) {
				debug("G %X: mark is %d but should be %d (%s)", o, (int)op->mark, (int)mark,
					  OBJ_CLASS(o) ? OBJ_CLASS(o)->name : "null class");
				alert("G %X: mark is %d but should be %d (%s)", o, (int)op->mark, (int)mark,
					  OBJ_CLASS(o) ? OBJ_CLASS(o)->name : "null class");
			}
#endif
		n++;
...
static bool joinAdjacentObjects(uint8* block, uint32 size) {
	...
	while (block < blockEnd) {
		...
		if (/*op->next == null || */op->class_ !=
									null) { // not empty object? skip  - guich@tc113_19: commented first part. since the blocks are added FIFO, retaining that block is not good. the idea here was to keep the first memory block created, but since the block was created for the constant pool strings, and these are locked, this comparison is uneeded.
			usedCount++;
			if (COMPUTETIME) {
			...
			if (newSize != op0->size) { // chunks were merged?
				...
				if (_DEBUG_OMM_LIST) {
					debug("G %X merged: %4d -> %4d (%3d->%3d)", o, op0->size, newSize, oldIdx, newIdx);
				}
				op0->size = newSize;
				if (oldIdx != newIdx) {
					removeNodeFromDblList(freeList[oldIdx], o);
	...
	if (usedCount == 0) {
		...
		if (COMPUTETIME) {
			debug("G --- Removed chunk %X size %d", block00, size - sizeof(TObjectProperties));
		}
	...
	if (COMPUTETIME) {
		...
		debug("G Chunk %X size %d has %d used objects with %d bytes (%d%%)", block0, tt, usedCount, uu,
			  uu * 100 / tt);
	}
	return false;
}
	...
	for (i = 0, usedL = usedList; i <= OBJARRAY_MAX_INDEX; i++, usedL++)
		if (*usedL)
			for (o = OBJ_PROPERTIES(*usedL)->next; o != null; o = OBJ_PROPERTIES(o)->next)
				if ((c = OBJ_CLASS(o)) != null && c->finalizeMethod != null && (c->dontFinalizeFieldIndex == 0
						|| FIELD_I32(o, (c->dontFinalizeFieldIndex - 1)) ==
						false)) { // if user defined a dontFinalize field and set it to true, don't call finalize
...
void gc2(Context currentContext, bool lockOMM) {
	...
	int32 total0 = totalAllocated / 1024 / 1024, free0 = getFreeMemory(USE_MAX_BLOCK) / 1024 / 1024,
	...
	if (oldAutoOffValue !=
		0) { // guich@450_33: since the autooff timer function don't work on wince, we must keep resetting the idle timer so that the device will never go sleep - guich@554_7: reimplemented this feature
		SystemIdleTimerReset();
	}
#endif
#if !defined(ENABLE_TEST_SUITE) // this scrambles the test
	freemem = getFreeMemory(USE_MAX_BLOCK);
	if (disableGC || (IS_VMTWEAK_ON(VMTWEAK_DISABLE_GC)
					  && freemem >
					  CRITICAL_SIZE)) { // use an agressive gc if memory is under 2MB - guich@tc114_18: let user control gc runs
		skippedGC++;
		if (COMPUTETIME) {
			debug("G ====  GC SKIPPED");
		}
	...
	if (COMPUTETIME) {
		...
		debug("G ====  GC INI : %d (skipped: %d) free: %d, used: %d, mark: %d, chunks: %d, objs created: %d (%d ms ago), locked objs: %d, context: %X, free mem: %d (max: %d). context: %X",
			  tcSettings.gcCount ? *tcSettings.gcCount : 0, skippedGC, nfree, nused, markedAsUsed,
			  tcSettings.chunksCreated ? *tcSettings.chunksCreated : 1, objCreated, iniT - lastGC, objLocked,
			  currentContext, getFreeMemory(false), getFreeMemory(true), currentContext);
...
#ifdef __gl2_h_
		if (currentContext !=
			mainContext) { // in opengl, an image can only be freed in the main context, otherwise the texture will not be released
	...
	markedAsUsed =
		!markedAsUsed; // otherwise, objects allocated in this executeMethod will have problems when being collected
	...
	if (COMPUTETIME || traceObjsCreatedBetween2GCs) {
		...
		debug("GC %d : free: %d, used: %d (dif: %d), chunks: %d, elapsed: %4d (compact: %3d)",
			  tcSettings.gcCount ? *tcSettings.gcCount : 0, nfree, nused, rdif,
			  tcSettings.chunksCreated ? *tcSettings.chunksCreated : 1, endT - iniT, endT - compIni);
		lastUsed = nused;
	}
	...
	{
		int32 totalf = totalAllocated / 1024 / 1024, freef = getFreeMemory(USE_MAX_BLOCK) / 1024 / 1024,
		...
		if ((lastT != totalf) || (lastF != freef) || (lastC != chunksf)) {
			debug("%d gc u:%d->%d f:%d->%d, c:%d->%d tex:%d t:%X (%c) %dms",
				  tcSettings.gcCount ? *tcSettings.gcCount : 0, total0, lastT = totalf, free0, lastF = freef, chunks0,
				  lastC = chunksf, totalTextureLoaded, currentContext,
				  currentContext == mainContext ? 'M' : currentContext == gcContext ? 'G' : 'T', endT - iniT);
		}
#endif /* DEBUG */
	}
}
