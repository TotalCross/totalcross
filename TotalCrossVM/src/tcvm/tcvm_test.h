...
static TCObject newTestTypesInstance(Context currentContext) {
	...
	o = createObjectWithoutCallingDefaultConstructor(currentContext,
...
static Method initMethod(Context currentContext, int op) {
	...
	currentContext->regO[0] =
		testTypesInstance; // instance fields assume that the class instance is in regO[0]
	code[0].s24.op = op;
...
static bool arCheck(Context currentContext, struct TestSuite* tc, int op) {
	...
	ASSERT2_EQUALS(Sz, OBJ_CLASS(currentContext->thrownException)->name,
...
static TCObject initExtTest(Context currentContext, struct TestSuite* tc, Method m, RegType t,
							bool isStatic) {
	static TCObject ext;
	...
	if (!isStatic) {
		...
		ASSERT1_EQUALS(NotNull, currentContext->thrownException);
		ASSERT2_EQUALS(Sz, OBJ_CLASS(currentContext->thrownException)->name,
					   throwableAsCharP[isStatic ? ClassNotFoundException : NullPointerException]);
		currentContext->thrownException = null;
	}
...
TESTCASE(VM_z3_RETURN_reg64) {
	...
	Context c = currentContext;
	// double
	REGD(c->reg64)[m->code[0].reg.reg = 2] =
	...
	REGL(c->reg64)[m->code[0].reg.reg = 1] =
...
TESTCASE(VM_THROW) { // throw new Exception() -> newObj regO, java.lang.Exception; throw regO;
	...
	ASSERT1_EQUALS(NotNull, currentContext->thrownException);
	executeMethod(currentContext, testException);
	ASSERT2_EQUALS(Ptr, currentContext->thrownException,
...
TESTCASE(VM_INSTANCEOF) { // if (other instanceof Rect) -> mov regI, regO instanceof sym; jeq regI,1;
	...
	// byte array
	currentContext->regO[m->code[0].instanceof.regO = 1] = FIELD_OBJ(testTypesInstance, testTypesClass,
			1);
...
TESTCASE(VM_SWITCH) {
	...
	*/
	...
	m->code[12].s24.desloc = exit -
							 12; // note: jump is always relative to the current instruction pointer, which is the switch opcode.
