...
saveRestoreOMM(bool save)
{
  if (save) {
    ...
    lockList2 = lockList;
    lockList = null;
    markedAsUsed2 = markedAsUsed;
    markedAsUsed = 0;
    gcCount2 = *tcSettings.gcCount;
    *tcSettings.gcCount = 0;
    ommHeap2 = ommHeap;
    ommHeap = null;
    chunksHeap2 = chunksHeap;
    chunksHeap = null;
    objCreated2 = objCreated;
    objCreated = 0;
    skippedGC2 = skippedGC;
    skippedGC = 0;
    objLocked2 = objLocked;
    objLocked = 0;
    objStack2 = objStack;
    objStack = null;
...
{ // #3
  ...
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 13);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 12);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 11);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 10);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 9);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 8);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 7);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 6);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 5);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 4);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 3);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 2);
  ASSERT1_EQUALS(True, stackPop(s, &v));
  ASSERT2_EQUALS(I32, v, 1);
  ASSERT1_EQUALS(False, stackPop(s, &v));
...
static TCObject
allocAndFillObj(Context currentContext, uint32 size, uint8 fillWith)
{
...
// This test will implement the explanation in the top of the file
// objectmemorytest.c MUST COME RIGHT AFTER ArrayFileCreation TEST CASE !!!
TESTCASE(GarbageCollector)
{ // #5
  int32 nfree, nused;
  TCObjectArray held =
    currentContext
      ->regOStart; // objects that will be held (in use) after the GC
  TCObjectArray held0 = held;
  TCObject rel1, rel2, rel3, rel4, rel5, rel13,
    rel14; // objects that will be released (ready to be reused later)
  TCObject rel6, rel7, rel8, rel9, rel10, rel11,
    rel12; // objects that will be released (ready to be reused later)
  TCObject free1,
    free2; // objects that will be freed (returned to the system)
  ...
  rel1 = allocAndFillObj(
    currentContext,
    DEFAULT_CHUNK_SIZE - 4 - 8 - sizeof(TObjectProperties),
    1); // big object 1 - compute the size to let allocate held[0] below
  free1 = allocAndFillObj(currentContext,
                          DEFAULT_CHUNK_SIZE - 4,
                          2); // big object 2 - will have exactly
                              // DEFAULT_CHUNK_SIZE bytes. * WILL BE COLLECTED
  *held++ = allocAndFillObj(
    currentContext,
    4,
    3); // this will be allocated in the remaining block of huge object 1
  ...
  *held++ = allocAndFillObj(currentContext, 24, 9);
  ...
  free2 = allocAndFillObj(currentContext,
                          DEFAULT_CHUNK_SIZE + 10,
                          13); // huge object 1 - * WILL BE COLLECTED
  ...
  ASSERT2_EQUALS(I32,
                 8,
                 nfree); // rel1-rel5 + first chunk - two objects were freed
  ASSERT2_EQUALS(I32, 6, nused); // held objects
  // 3. allocate 12 objects
  held = currentContext->regO /*Protected*/ -
         2; // simulate the release of a two held objects. note that these are
            // in the longlive list, so they won't be freed in the next gc
  ...
  *held++ = allocAndFillObj(currentContext, 16, 23);
  ...
  held = currentContext->regO /*Protected*/ -
         1; // simulate the release of 1 held object.
  *held++ = allocAndFillObj(currentContext, 34, 25); // take space of rel7
  ...
  held = currentContext->regO /*Protected */ -
         3; // simulate the release of 3 held objects.
