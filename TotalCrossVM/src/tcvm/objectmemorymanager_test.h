...
TESTCASE(GarbageCollector) { // #5
	...
	rel1    = allocAndFillObj(currentContext, DEFAULT_CHUNK_SIZE - 4 - 8 - sizeof(TObjectProperties),
							  1); // big object 1 - compute the size to let allocate held[0] below
	free1   = allocAndFillObj(currentContext, DEFAULT_CHUNK_SIZE - 4,
							  2); // big object 2 - will have exactly DEFAULT_CHUNK_SIZE bytes. * WILL BE COLLECTED
	*held++ = allocAndFillObj(currentContext, 4,
	...
	*held++ = allocAndFillObj(currentContext, 24, 9);
	...
	free2   = allocAndFillObj(currentContext, DEFAULT_CHUNK_SIZE + 10,
	...
	held = currentContext->regO/*Protected*/ -
		   2; // simulate the release of a two held objects. note that these are in the longlive list, so they won't be freed in the next gc
