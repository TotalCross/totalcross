	...
	if (run != null) {
		...
		if (cc == null) {
			...
			setObjectLock(thread,
						  UNLOCKED); // now is safe to unlock, because the context will mark the threadObj
...
void threadCreateJava(Context currentContext, TCObject this_) {
	TCObject a;
	setObjectLock(this_,
				  LOCKED); // prevent the java.lang.Thread object from being collected, because another thread may collect it before the thread is started
