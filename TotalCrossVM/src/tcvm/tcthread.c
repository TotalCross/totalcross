...
void
executeThreadRun(Context context, TCObject thread);
...
executeThreadRun(Context context, TCObject thread)
{
  ...
  if (run != null) {
    ...
    if (cc == null) {
      ...
      Sleep(1);
      setObjectLock(thread,
                    UNLOCKED); // now is safe to unlock, because the context
                               // will mark the threadObj
...
ThreadHandle
threadCreateNative(Context context, ThreadFunc t, VoidP args)
{
...
void
threadCreateJava(Context currentContext, TCObject this_)
{
  TCObject a;
  setObjectLock(this_,
                LOCKED); // prevent the java.lang.Thread object from being
                         // collected, because another thread may collect it
                         // before the thread is started
  a = Thread_taskID(this_) =
    createByteArray(currentContext, sizeof(TThreadArgs));
  if (a != null) {
    ThreadHandle h;
...
void
threadDestroy(ThreadHandle h, bool threadDestroyingItself)
{
  threadCount--;
...
void
freeMutex(int32 hash, VoidP pmutex)
{
