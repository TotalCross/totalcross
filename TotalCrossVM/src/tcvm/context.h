...
struct TContext
{
  ...
  Code code;
  Heap heap;
  ThreadHandle thread; // the thread handle for this thread or null if its the
                       // main thread
  TCObject threadObj;
  TNMParams nmp;
  ...
  // IMPORTANT: ALL IFDEFS MUST BE PLACED AT THE END, otherwise, other native
  // libraries that use this header that do not define the same #defines, will
  // have problems.
...
Context
newContext(ThreadHandle thread,
           TCObject threadObj,
           bool bigContextSizes); // if bigContextSize is false, use
                                  // STARTING_xxx_SIZE/10
void
