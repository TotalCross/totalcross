...
#define CONVERT_PRIORITY(p, v)                                                 \
  sched_get_priority_min(p) +                                                  \
    (sched_get_priority_max(p) - sched_get_priority_min(p)) * (v - 1) /        \
      9; // java 1=min, 10=max

static ThreadHandle
privateThreadCreateNative(Context context, ThreadFunc t, VoidP this_)
{
  pthread_t h = 0;
  pthread_attr_t attr;
  ...
  pthread_create(
    ...
    NULL,
    t,
    targs); // Create, but thread will suspend until we send a resume signal
  ...
  if (h != null) {
    ...
    throwException(context, OutOfMemoryError, "Can't create thread (2)");
  }
  return h;
}

static ThreadHandle
privateThreadGetCurrent()
{
  return pthread_self();
}

static void
privateThreadDestroy(ThreadHandle h, bool threadDestroyingItself)
{
#if !defined(ANDROID)
...
privateThreadFunc(VoidP argP)
{
  ...
  {
    JNIEnv* env;
    (*androidJVM)->AttachCurrentThreadAsDaemon(androidJVM, &env, NULL);
  }
#endif
