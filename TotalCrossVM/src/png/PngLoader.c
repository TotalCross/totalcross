...
void pngLoad(Context currentContext, TCObject imageObj, TCObject inputStreamObj, TCObject bufObj,
			 TCZFile tcz, char* first4) {
	Heap heap;
	int32 count;
	uint8 buffer[512];
	int32 transp = -1;
	...
	png_ptr = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, heap, error_callback, null, heap,
									   usermalloc, userfree);
	if (png_ptr == NULL) {
		HEAP_ERROR(heap, 999);
	}
	...
	if (tcz == null) {
		Method readBytesMethod = getMethod(OBJ_CLASS(userData.inputStreamObj), true, "readBytes", 3,
	...
	if (info_ptr->text && strEq("Comment", info_ptr->text->key)) {
		Image_comment(imageObj) = createStringObjectFromCharP(currentContext, info_ptr->text->text,
								  (int)info_ptr->text->text_length);
	...
	if (png_ptr->color_type == 6) {
		...
		if (png_ptr->color_type == PNG_COLOR_TYPE_PALETTE) { // palettized?
			...
			for (i = png_ptr->num_trans; --i >= 0;
				) // guich@tc120_60: must find the entry that has 0 in trans array
			...
			transp = (info_ptr->trans_color.red << 16) | (info_ptr->trans_color.green << 8) |
	...
	if (userData.upixels) {
		png_free(png_ptr, userData.upixels);
	}
...
static void info_callback(png_structp png_ptr, png_infop info_ptr) {
	...
	*/
	...
	if (color_type == PNG_COLOR_TYPE_RGB_ALPHA || color_type == PNG_COLOR_TYPE_PALETTE || bit_depth < 8
	...
	if (png_ptr->color_type != PNG_COLOR_TYPE_PALETTE
		&& png_ptr->num_trans != 0) { // we don't support transparent palettes
	...
	Image_pixels(userData->imageObj) = userData->pixelsObj = createIntArray(userData->currentContext,
									   (int32)(width * height));
...
static void row_callback(png_structp png_ptr, png_bytep new_row, png_uint_32 row_num, int pass) {
	...
	if (pass == userData->lastPass) {
		...
		int32 x;
		if (png_ptr->channels == 4 || (png_ptr->color_type == PNG_COLOR_TYPE_PALETTE
									   && png_ptr->num_trans > 6))
			for (x = 0; x < userData->width; x++, buffer += 4) {
				*userData->pixels++ = makePixelA((uint8)buffer[3], (uint8)buffer[0], (uint8)buffer[1],
												 (uint8)buffer[2]);
			} else
