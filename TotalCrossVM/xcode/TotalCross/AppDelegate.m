...
- (void) initApp {
	...
	NSString* pkgDirectory = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:
													 @"pkg"];
	if ([[NSFileManager defaultManager] fileExistsAtPath:
										pkgDirectory]) { // First check if the pkg directory exists
		NSString* documentsDirectory = [NSHomeDirectory() stringByAppendingPathComponent: @"Documents"];
		NSError* error = nil;
		NSArray* fileList = [[NSFileManager defaultManager] contentsOfDirectoryAtPath: pkgDirectory error:
															&error];
		for (NSString * s in fileList) {
			...
			NSDate* sourceDate = [[[NSFileManager defaultManager] attributesOfItemAtPath: sourceFilePath error:
																   &error] objectForKey: NSFileCreationDate];
			NSDate* targetDate = !targetExists ? nil : [[[NSFileManager defaultManager] attributesOfItemAtPath:
																 targetFilePath error: &error] objectForKey: NSFileCreationDate];
			if (!targetExists
				|| [targetDate compare: sourceDate] == NSOrderedAscending) { // pkg date is more recent than target?
				if (targetExists) { //File exist, delete it
					[[NSFileManager defaultManager] removeItemAtPath: targetFilePath error: &error];
				}
				[[NSFileManager defaultManager] copyItemAtPath: sourceFilePath toPath: targetFilePath error:
												&error];
			}
		}
	}
	...
	const char* name =
		...
		[[[[[NSBundle mainBundle] infoDictionary] objectForKey: @"CFBundleName"]
								   stringByTrimmingCharactersInSet: [NSCharacterSet whitespaceCharacterSet]] cStringUsingEncoding:
...
- (void) userNotificationCenter: (UNUserNotificationCenter*)center
	...
	// Print message ID.
	[tcvm privateOnMessageReceived: userInfo[kGCMMessageIDKey] messageType: NULL keys: keyArray values:
		  valueArray collapsedKey: NULL ttl: 0];
...
// [START refresh_token]
- (void) messaging: (nonnull FIRMessaging*)messaging didRefreshRegistrationToken:
...
- (void) messaging: (nonnull FIRMessaging*)messaging
	...
	[tcvm privateOnMessageReceived: NULL messageType: NULL keys: keyArray values: valueArray
		  collapsedKey: NULL ttl: 0];
}
