...
// $Id: SOAP.java,v 1.56 2011-03-24 18:41:42 guich Exp $
...
public class SOAP // guich@570_34
{
   ...
   private class ParseAnswer extends XmlTokenizer
   {
      ...
      private void storeAnswer()
      {
         ...
         switch (type)
         {
            ...
            case FLOAT_TYPE:
            case BOOLEAN_TYPE:
               if (answer == null) // is this the first answer?
                  answer = tag;
               else if (answer instanceof Vector) // is answer already a Vector?
   ...
   public void setObjectParam(String paramName, String[] fieldNames, String[] fieldValues)
   {
      ...
      sb.append('<').append(paramName).append('>');
      for (int i = 0; i < len; i++)
         sb.append("<").append(fieldNames[i]).append('>').append(fieldValues[i]).append("</").append(fieldNames[i]).append('>');
      sb.append("</").append(paramName).append('>');
      paramIndex++;
   }
   ...
   public void execute() throws SOAPException
   {
      try
      {
         ...
         if (!disableEncoding) // flsobral@tc110_72: SOAP will now ask the server for encoded content by default. (either gzip or zlib)
         ...
         if (!ok)
         {
            if (hs.responseCode == 500) // guich@582_1: wait until we get the response message if error 500
         ...
         receivedStream.close();
         if (!ok && hs.responseCode == 500) // guich@582_1: now we have the message, throw the exception
