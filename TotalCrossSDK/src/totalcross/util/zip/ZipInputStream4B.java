...
// $Id: ZipInputStream4B.java,v 1.4 2011-01-04 13:19:09 guich Exp $
 ...
 * Replaced EOFException by IOException with the message "EOFException", because this exception is not supported by
 * TotalCross.<br>
 * Replaced Integer.toHexString by Convert.toString(value, 16).<br>
 * 
 * @author Jochen Hoenicke
 */
public class ZipInputStream4B extends InflaterInputStream4B implements ZipConstants4B
{
    ...
    */
   public ZipInputStream4B(Stream in)
   {
      super(in, new Inflater4B(true));
   }
   ...
   private int readBuf(byte[] out, int offset, int length) throws IOException
   {
      ...
      if (length > avail)
         length = avail;
      Vm.arrayCopy(buf, len - avail, out, offset, length);
      avail -= length;
      return length;
   }
   ...
   public ZipEntry getNextEntry() throws IOException
   {
      ...
      if (method == ZipOutputStream4B.STORED && csize != size)
      ...
      if (method == ZipOutputStream4B.DEFLATED && avail > 0)
      {
         Vm.arrayCopy(buf, len - avail, buf, 0, avail);
         len = avail;
         avail = 0;
   ...
   public void closeEntry() throws IOException
   {
      ...
      if (method == ZipOutputStream4B.DEFLATED)
      {
         if ((flags & 8) != 0)
         {
      ...
      else
      {
         ...
         while (csize != 0)
         {
            long skipped = in.skipBytes(csize);
            if (skipped <= 0)
   ...
   public int read(byte[] b, int off, int len) throws IOException
   {
      ...
      switch (method)
      {
         case ZipOutputStream4B.DEFLATED:
