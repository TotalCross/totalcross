...
// $Id: OutputWindow4B.java,v 1.3 2011-01-04 13:19:09 guich Exp $
...
class OutputWindow4B
{
   ...
   public void repeat(int len, int dist)
   {
      ...
      if (rep_start <= border && window_end < border)
      {
         if (len <= dist)
         {
            Vm.arrayCopy(window, rep_start, window, window_end, len);
            window_end += len;
         }
         else
         {
   ...
   public int copyStored(StreamManipulator4B input, int len)
   {
   ...
   public void copyDict(byte[] dict, int offset, int len)
   {
      ...
      Vm.arrayCopy(dict, offset, window, 0, len);
   ...
   public int copyOutput(byte[] output, int offset, int len)
   {
      ...
      if (tailLen > 0)
      {
         Vm.arrayCopy(window, WINDOW_SIZE - tailLen,
               output, offset, tailLen);
         offset += tailLen;
         len = copy_end;
      }
      Vm.arrayCopy(window, copy_end - len, output, offset, len);
      window_filled -= copied;
      if (window_filled < 0)
         throw new IllegalStateException();
      return copied;
