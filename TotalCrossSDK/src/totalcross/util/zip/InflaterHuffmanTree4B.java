 ...
 */
public class InflaterHuffmanTree4B
{
   private final static int MAX_BITLEN = 15;
   private short[] tree;

   public static InflaterHuffmanTree4B defLitLenTree, defDistTree;
   ...
   static
   {
      try
      {
         ...
         while (i < 288)
            codeLengths[i++] = 8;
         defLitLenTree = new InflaterHuffmanTree4B(codeLengths);

         codeLengths = new byte[32];
         i = 0;
         while (i < 32)
            codeLengths[i++] = 5;
         defDistTree = new InflaterHuffmanTree4B(codeLengths);
      }
    ...
    */
   public InflaterHuffmanTree4B(byte[] codeLengths) throws ZipException
   {
      buildTree(codeLengths);
   }

   private void buildTree(byte[] codeLengths) throws ZipException
   {
      ...
      for (int bits = MAX_BITLEN; bits >= 10; bits--)
      {
         ...
         {
            tree[DeflaterHuffman4B.bitReverse(i)] = (short) ((-treePtr << 4) | bits);
            treePtr += 1 << (bits - 9);
         }
      }
      ...
      for (int i = 0; i < codeLengths.length; i++)
      {
         ...
         if (bits == 0)
            continue;
         code = nextCode[bits];
         int revcode = DeflaterHuffman4B.bitReverse(code);
         if (bits <= 9)
         {
            do
            {
    ...
    */
   public int getSymbol(StreamManipulator4B input) throws ZipException
   {
