 ...
 * Commented debugging code.<br>
 * System.arraycopy replaced by Vm.arrayCopy.
 * 
 * @author Jochen Hoenicke
 * @date Jan 5, 2000
 */

class PendingBuffer4B
{
   protected byte[] buf;
   int start;
   int end;
   ...
   public PendingBuffer4B()
   {
      this(4096);
   }

   public PendingBuffer4B(int bufsize)
   {
   ...
   {
      //    if (DeflaterConstants4B.DEBUGGING && start != 0)
   ...
   public final void writeBlock(byte[] block, int offset, int len)
   {
      //    if (DeflaterConstants4B.DEBUGGING && start != 0)
      //      throw new IllegalStateException();
      Vm.arrayCopy(block, offset, buf, end, len);
      end += len;
   }
   ...
   public final int flush(byte[] output, int offset, int length)
   {
      ...
      {
         length = end - start;
         Vm.arrayCopy(buf, start, output, offset, length);
         start = 0;
         end = 0;
      }
      else
   ...
   public final byte[] toByteArray()
   {
      ...
      Vm.arrayCopy(buf, start, ret, 0, ret.length);
      start = 0;
      end = 0;
      return ret;
   }
