 ...
 * Disabled method checkEmpty, which was only used for debugging.
 * 
 * @author Jochen Hoenicke
 * @date Jan 6, 2000
 */
class DeflaterHuffman4B
{
   private static final int BUFSIZE = 1 << (DeflaterConstants4B.DEFAULT_MEM_LEVEL + 6);
   ...
   class Tree
   {
      ...
      public void buildCodes()
      {
         ...
         //         if (DeflaterConstants4B.DEBUGGING && code != 65536)
   ...
   static short bitReverse(int value)
   {
      return (short) (bit4Reverse.charAt(value & 0xf) << 12
            ...
            | bit4Reverse.charAt((value >> 8) & 0xf) << 4 | bit4Reverse.charAt(value >> 12));
   }
   ...
   public DeflaterHuffman4B(PendingBuffer4B pending)
   {
         ...
         boolean lastBlock)
   {
      ...
      pending.writeBits((DeflaterConstants4B.STORED_BLOCK << 1)
            + (lastBlock ? 1 : 0), 3);
      pending.alignToByte();
         ...
         boolean lastBlock)
   {
      ...
      else if (opt_len == static_len)
      {
         ...
         pending.writeBits((DeflaterConstants4B.STATIC_TREES << 1)
               + (lastBlock ? 1 : 0), 3);
      ...
      else
      {
         ...
         pending.writeBits((DeflaterConstants4B.DYN_TREES << 1)
               + (lastBlock ? 1 : 0), 3);
