...
// $Id: Base64.java,v 1.21 2011-01-04 13:19:06 guich Exp $
 ...
 * The base 64 is used to convert binaries into text so that
 * they can be sent over a stream. It may then be converted
 * back to binary form.<p>
 * Note that 3 bytes base 16 (hexadecimal) are encoded into 4 bytes base 64,
 * so the final length is 33% bigger.
 * @since SuperWaba 5.1
 */
...
public class Base64
{
    ...
    * 
    * @param bytes
    *           the base 10 byte array to be encoded into base 64
    * @param start
    ...
    *
    * @param in The Base 10 output stream. Its position must be set to the current size of the input data.
    * @param out The Base 64 input stream. When returning, its position will be set with the output size in bytes.
    */
