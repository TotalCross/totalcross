...
// $Id: KeyboardBox.java,v 1.9 2011-01-06 16:39:58 guich Exp $
...
public class KeyboardBox extends Window // guich@102
{
   ...
   {
      {"123 +$&() αΰγΰ456 -#@[] βικθ789 *%|{} νστυ.0, /=\\<> φϊηρ               qwertyuiop'_  asdfghjkl:~^  zxcvbnm!?;\"` "}, // will be parsed later
      {"Caps","Shift"},
      {" "},
      {"«","Done"},
   };
               ...
               if (sel >= 0) // guich@200b4_105: fix ArrayIndexOutOfBounds exception
               {
                  ...
                  if (pb.appId == SPECIAL_PAD || pb.appId == CAPS_PAD || pb.appId == SPACE_PAD) // special char?
                  {
                     ...
                     switch (st.charAt(0))
                     {
                        ...
                        case '«': // guich@200b4: this char is not being recognized correctly in the SPT1700.
                        default : key = SpecialKeys.BACKSPACE; break;
                     }
                     if (key != -1) insertKey(key); else
                     if (pb.appId == CAPS_PAD) // guich@564_8
