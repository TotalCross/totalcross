...
// $Id: Style.java,v 1.44 2011-03-24 18:41:40 guich Exp $
...
public class Style
{
   ...
   private static final int tagStyles[] =
   {
      ...
      /* 12 BLOCKQUOTE*/ type(TYPE_BLOCK) | CHG_STYLE | DISJOINT | P_AFTER | top(5) | indent(+2),
      /* 13 BODY      */ 0,
      /* 14 BR        */ type(TYPE_INLINE) | CHG_STYLE | DISJOINT | indent(-1),
      /* 15 BUTTON    */ 0,
      /* 16 CAPTION   */ 0,
      /* 17 CENTER    */ type(TYPE_BLOCK) | CHG_STYLE | DISJOINT | align(ALIGN_CENTER),
      /* 18 CITE      */ 0,
      ...
      /* 53 LABEL     */ 0,
      /* 54 LEGEND    */ 0,
      /* 55 LI        */ type(TYPE_BLOCK) | CHG_STYLE | DISJOINT | HEADED | indent(+3),
      /* 56 LINK      */ 0,
      /* 57 LISTING   */
      /* 58 MAP       */ 0,
      /* 59 MARQUEE   */
  ...
  /**
   ...
   private Style(int tagHashId, AttributeList atts, Style parent)
   {
      ...
      switch (tagHashId)
      {
         ...
         case TagDereferencer.FONT:
            int v = atts.getAttributeValueAsInt("size",-1); if (v != -1) fontSize = v; 
   ...
   static Style tagStartFound(Style current, int tagHashId, AttributeList atts)
   {
      ...
      if ((current.tagHashId == TagDereferencer.HR) || (current.tagHashId == TagDereferencer.IMG) || (current.tagHashId == TagDereferencer.INPUT))
      ...
      if (isStyleChanged(tagHashId))
      {
         switch (tagHashId)
         {
            ...
            default:
               ...
               if (tagHashId != TagDereferencer.P && originalAlign == ALIGN_NONE) // guich@tc112_34: inherit <pre> when <b> is found
                  current.alignment = ALIGN_NONE;
               else if (tagHashId == TagDereferencer.BR && originalAlign != ALIGN_LEFT) //flsobral@tc126: <br> no longer overwrites the inherited alignment
                  current.alignment = ALIGN_NONE;
               break;
         }
      }
