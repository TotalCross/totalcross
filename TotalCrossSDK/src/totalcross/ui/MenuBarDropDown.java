...
// $Id: MenuBarDropDown.java,v 1.45 2011-03-21 19:10:19 guich Exp $
...
public class MenuBarDropDown extends Window
{
   ...
   public static MenuItem[] strings2items(String[] strings)
   {
      ...
      for (int i = mi.length-1; i > 0; i--)
      {
         ...
         {
            case '*': m = new MenuItem(caption.substring(1)); m.isEnabled = false; break; // disabled
            case '!': m = new MenuItem(caption.substring(1),true); break;  // checked
            case '?': m = new MenuItem(caption.substring(1),false); break; // unchecked
            case '-': m = new MenuItem(); break; // separator
            default:
   ...
   public int setSelectedIndex(int index)
   {
      ...
      if (index == 1000) // Don't update contents on the current screen when unpop.
         selected = -1;
      else
   ...
   protected boolean onClickedOutside(PenEvent event)
   {
      ...
      if (event.x < this.x || event.y < this.y || event.x >= (this.x+this.width) || event.y >= (this.y+this.height)) // guich@300_39: added the last condition to fix a problem when this is used not with a menubar.
      {
         if (getTopMost() instanceof MenuBar) // guich@tc114_29 
   ...
   protected void loadBehind() // guich@tc120_38
   {
      try
      {
         if (!(zStack.peek() instanceof MenuBar)) // only update screen if our parent window is not a MenuBar
            super.loadBehind();
      }
   ...
   public void onEvent(Event event)
   {
      switch (event.type)
      {
         case KeyEvent.SPECIAL_KEY_PRESS:
            ...
            if (handleFocusChangeKeys((KeyEvent)event)) // guich@tc100b4_8
               break;
         case KeyEvent.KEY_PRESS:
            ...
            if (w instanceof MenuBar) ((MenuBar)w).close(); // close all menu windows
            break;
   ...
   protected boolean handleFocusChangeKeys(KeyEvent ke) // guich@512_1: transfer focus on tab keys - fdie@550_15 : transfer also on arrow keys
   {
      ...
      if (ke.isUpKey() || ke.isDownKey())
      {
         ...
         if (newSelected != selected && newSelected != -1) // guich@tc115_19: only if changed and not -1
            setSelectedIndex(newSelected);
      }
      else
      if (ke.key == SpecialKeys.LEFT || ke.key == SpecialKeys.RIGHT || ke.key == SpecialKeys.TAB)
      {
