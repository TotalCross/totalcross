...
public class ColumnChart extends Chart
{
   ...
   public void onPaint(Graphics g)
   {
      ...
      for (int i = columns.size(); i < numCols; i ++) // add missing columns rectangles - guich@tc115_82: changed <= to <
      ...
      for (int i = 0; i < cCount; i ++) // for each category
      {
         ...
         for (int j = forward ? 0 : sCount-1; forward ? (j < sCount) : (j >= 0); j+=(forward?1:-1), x += (forward?colW:-colW), col++) // for each series
         {
            ...
            int c = s.color;
            if (selection == col) // this column is selected
               c = Color.darker(c);
            ...
            if ((type & (GRADIENT_HORIZONTAL|GRADIENT_VERTICAL)) != 0)
            {
               ...
               if ((type & GRADIENT_VERTICAL) == 0)
                  ...
                  g.fillShadedRect(x,y,colW, yAxisY1-y,invertGradient,false,c,fade,100); // 3x faster
            }
            else
            {
               g.backColor = c;
