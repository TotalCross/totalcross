...
public class LineData extends Container
{
    ...
    */
   public LineData(Chart chart, String[] title, int[] colors, String[][] data)
   {
      this.chart = chart;
      this.title = title;
      this.data = data;
      this.colors = colors;
      backs = new Image[title.length];
   }
      ...
      for (int i = 0; i < data.length; i++)
      {
         ...
         for (int j = 0, n = data[i].length; j < n; j++, x0 += inc)
         {
            ...
            if (d.length() > 0)
            {
               ...
               try
               {
                  if (backs[i] == null || backs[i].getWidth() != backw || backs[i].getHeight() != ystep-2)
                     createBacks(backw,ystep-2);
                  g.drawImage(back = backs[i],xt,yt-2);
                  xt++;
               }
               catch (ImageException e)
               {
               ...
               if (back != null)
               {
                  lastX = xt + back.getWidth() -1;
                  xt += (back.getWidth()-sw)/2;
               }
               g.drawText(d,xt,yt);
               lastV = d;
            }
   ...
   private void createBacks(int w, int h) throws ImageException
   {
      for (int i = 0; i < backs.length; i++)
         backs[i] = NinePatch.getInstance().getNormalInstance(NinePatch.BUTTON,w,h,colors[i],false,true);
   }
      ...
      switch (e.type)
      {
         case PenEvent.PEN_UP:
            if (enabled && !hadParentScrolled())
            {
               ...
               int xstep = chart.columnW;
               int wdim = xstep/3;
               if (chart.xAxisX1-wdim <= pex && pex <= chart.xAxisX2+wdim && chart.yAxisY2 <= pey && pey <= chart.yAxisY1)
               {
                  int ystep = this.height / data.length;
                  int yxtra = this.height % ystep;

                  for (int i = 0, n = data[0].length, xx = chart.xAxisX1; i <= n; i++,xx += xstep) // horizontal lines
                  {
                     if (xx-wdim <= pex && pex < xx+wdim)
                     {
                        selectedCol = i;
                        break;
                     }
