...
// $Id: PDBFile.java,v 1.54 2011-02-24 16:44:56 fabio Exp $
...
public class PDBFile extends Stream
{
   ...
   */
   ...
   final public void rename(String newName) throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
   {
      ...
      if (path != null && path.length() > 0) // guich@561_11: if the file was not yet dumped to disk, path will return ""
   ...
   */
   ...
   {
      if (!deleted && (modificationNumber != originalModificationNumber || (modificationNumber == 0 && getRecordCount() == 0))) // guich@tc112_19: only write when == 0 if there's no record at all
         writeCB();
   }
            ...
            if (v.indexOf(s[i]) == -1 && (/*s[i].toLowerCase().endsWith("prc") ||*/ s[i].toLowerCase().endsWith("pdb")))
               try
               {
                  ...
                  if (buf.length == readLength)
                  {
                     ...
                     if (includeVersion)
                        fullName += "#" + version;
                     if (v.indexOf(fullName) == -1) // guich@300_18
   ...
   private int readWriteBytes(byte buf[], int start, int count, boolean isRead) throws totalcross.io.IOException
   {
      ...
      try
      {
         ...
         else
         {
            ...
            if (_attrs != null) setRecordAttributes(_hvRecordPos, (byte)(getRecordAttributes(_hvRecordPos) | PDBFile.REC_ATTR_DIRTY)); // set dirty
            modificationNumber++;
         }
   ...
   */
   ...
   private String removePDBFileFromDisk()
   {
      ...
      if (path == null || path.length() == 0) // guich@564_20: added path.length in case path is "". occurs after a driver.closeAll
  ...
  */
  ...
  /** reads a pdb file and returns a vector of records. */
         ...
         throws totalcross.io.IOException
   {
      ...
      if (all.length >= 8 && new String(all, 0, 8).trim().equalsIgnoreCase("<HTML>")) // for some reason we got a webpage?
     ...
     if (_attrs != null) _attrs.removeAllElements();
     for (int i=0; i < numRecords; i++)
     {
        recOffsets[i] = is.readInt();  // offset
        recAttributes = is.readByte();
        is.readBytes(recUniqueID);
        if (_attrs != null) _attrs.addElement(new Byte(recAttributes));
     }
