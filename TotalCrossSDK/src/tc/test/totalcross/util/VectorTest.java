...
// $Id: VectorTest.java,v 1.6 2011-01-04 13:19:10 guich Exp $
...
public class VectorTest extends TestCase
{
   ...
   private void push_pop_peek(/*Object obj*/)
   {
      ...
      v.push("three");
      assertEquals(3,v.size());
      try {assertEquals("three",v.peek());} catch (ElementNotFoundException e) {fail();}
      assertEquals(3,v.size()); // make sure that peek does not remove the element
      try {assertEquals("three",v.pop());} catch (ElementNotFoundException e) {fail();}
      try {assertEquals("two",v.pop());} catch (ElementNotFoundException e) {fail();}
      try {assertEquals("one",v.pop());} catch (ElementNotFoundException e) {fail();}
      try {assertNull(v.pop()); fail(); } catch (ElementNotFoundException e) {}
      try {assertNull(v.peek()); fail(); } catch (ElementNotFoundException e) {}
   }
   private void clear()
   {
      Vector v = new Vector(3);
