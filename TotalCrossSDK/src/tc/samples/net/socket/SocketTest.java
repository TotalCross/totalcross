...
// $Id: SocketTest.java,v 1.29 2011-01-04 13:19:31 guich Exp $

package tc.samples.net.socket;
...
public class SocketTest extends MainWindow
{
   ...
   public void initUI()
   {
      boolean crad = false, wifi = false, cell = false;
      try
      {
         crad = ConnectionManager.isAvailable(ConnectionManager.CRADLE);
      }
      catch (IOException e) {}
      ...
      if (!crad && !wifi && !cell) // no connection available
      {
   ...
   private void openSocket() throws totalcross.net.UnknownHostException,  totalcross.io.IOException
   {
      ...
      else if (network.equals("CRADLE"))
         ConnectionManager.open(ConnectionManager.CRADLE);
