 ...
 *  Copyright (C) 2004-2011 SuperWaba Ltda.                                      *
 *  All Rights Reserved                                                          *
 *                                                                               *
...
// $Id: HtmlBrowser.java,v 1.39 2011-01-04 13:19:26 guich Exp $

package tc.samples.app.htmlbrowser;
 ...
 */
public class HtmlBrowser extends MainWindow
{
   ...
   public void initUI()
   {
      ...
      add(btnMinus, RIGHT, 0, PREFERRED,getTitleFont().fm.height);
      add(btnPlus, BEFORE-2,0,PREFERRED,SAME);
      add(new Label("Font: "),BEFORE-2,0,PREFERRED,SAME);
      repaintNow(); // update background color
      String cmd = getCommandLine();
      if (cmd != null && cmd.startsWith("/s"))
         try {loadSample(Convert.toInt(cmd.substring(2).trim()));} catch (Exception e) {MessageBox.showException(e, true);}
      else
         popupMenuBar();
      ...
      switch(event.type)
      {
         case ControlEvent.PRESSED:
            try
            {
               ...
               {
                  Vm.debug("Link: "+htmlCnr.pressedLink);
               }
               else
