...
public class Utils
{
   ...
   public static File waitForFile(String file) throws Exception
   {
      ...
      if (count <= 0) throw new DeployerException("Error in script: expected file "+file+" was never written!");
      return in;
   }
   /////////////////////////////////////////////////////////////////////////////////////
   public static String findPath(String fileName, boolean ignoreFileWithoutSlash)
   {
      ...
      try
      {
         if ((!ignoreFileWithoutSlash || fileName.replace('/','\\').indexOf('\\') >= 0) && new File(p=fileName).exists())
            return p;
   ...
   {
      if (path != null) // guich@tc100b5_50: path may be null
   ...
   public static byte[] findAndLoadFile(String fileName, boolean showFile)
   {
      ...
      try
      {
         ...
         if (bytes == null && DeploySettings.classPath != null) // search in the classpath
            for (int i =0; i < DeploySettings.classPath.length && bytes == null; i++)
            {
               path = DeploySettings.classPath[i]; // guich@340_19: show the path
               if (path.toLowerCase().indexOf(".jar") < 0) // not a jar file?
                  bytes = loadFile(path = path+"/"+fileName,false);
               else
                  try
                  {
                     String lowerfile = fileName.toLowerCase();
                     if (path.toLowerCase().indexOf("jdeb") >= 0) // without this, deploying the samples takes 10s more
                        continue;
                     ...
                     for (ZipEntry zEntry = zIn.getNextEntry(); zEntry != null && bytes == null; zEntry =zIn.getNextEntry())
                     {
                        ...
                        if (name.toLowerCase().endsWith(lowerfile)) // sample for name: "stubs\\ce\\PocketPC\\arm\\CEStub.exe"
                           bytes = readJavaInputStream(zIn);
                        zIn.closeEntry();
                     }
                     zIn.close();
         ...
         if (bytes == null && fileName.toLowerCase().endsWith(".png") && DeploySettings.mainPackage != null && DeploySettings.mainPackage.indexOf("samples/") >= 0) // guich@tc126_65: use the default icon with transparency if deploying a sample
     ...
     for (i=start; i < len; i++)
        if (src[i] == b) // first letter matches?
        {
           ...
           for (j = 1; j < what.length && found; j++)
              if (skipStrange && src[i+j+plus] < 0) // is it a strange codewarrior symbol?
                 plus++; else
              if (src[i+j+plus] != what[j]) found = false; // ps: cannot use continue here since were insice 2 loops
  ...
  public static int uniIndexOf(byte[] src, byte[] what, int i) // guich@340_60: added skipStrange
  {
     ...
     for (; i < len; i++)
        if (src[i] == b) // first letter matches?
        {
           boolean found = true;
           for (j = 1; j < what.length && found; j++)
              if (src[i+(j<<1)] != what[j]) found = false; // ps: cannot use continue here since were insice 2 loops
           if (found) return i; // all matches!
        }
     return -1;
  }
  ...
  public static String removeWhiteSpaces(String s, boolean upperCaseAfterSpace)
  {
   ...
   {
      if (!DeploySettings.quiet) System.out.println(s);
   }
   public static void println(String s, boolean force)
   {
      if (!DeploySettings.quiet || force) System.out.println(s);
   }
   /////////////////////////////////////////////////////////////////////////////////////
   ...
   public static String getBaseFolder(String currentDir, String passed, String className)
   {
      ...
      if (!currentDir.endsWith(""+DeploySettings.SLASH))
   ...
   public static void jarSigner(String jar, String targetDir) throws Exception
   {
      ...
      String cmd = DeploySettings.pathAddQuotes(jarsignerExe)+" -digestalg SHA1 -sigalg MD5withRSA -keystore "+DeploySettings.pathAddQuotes(keystore)+" -storepass @ndroid$w -keypass @ndroidsw "+DeploySettings.pathAddQuotes(jar)+" tcandroidkey";
