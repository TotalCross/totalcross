...
public class RegAllocation
{
   ...
   private RegAllocation(CFG cfg)
   {
      ...
      {
         stack = new Stack();
         cfg.livenessAnalysis(TCConstants.RegI);
         maxColor = 0;
         if (allocateRegisters(cfg, TCConstants.RegI))
         {
      ...
      {
         stack = new Stack();
         cfg.livenessAnalysis(TCConstants.RegD);
         maxColor = 0;
         if (allocateRegisters(cfg, TCConstants.RegD))
         {
            adjListD = createCopyOf(adjList);
            maxColorD = maxColor + 1;
         }
         else
            throw new ConverterException("The maximum number of floating registers has been reached. Please split your method and try again.");
         cleanData();
      }
      ...
      {
         stack = new Stack();
         cfg.livenessAnalysis(TCConstants.RegO);
         maxColor = 0;
         if (allocateRegisters(cfg, TCConstants.RegO))
         {
            adjListO = createCopyOf(adjList);
            maxColorO = maxColor + 1;
         }
         else
            throw new ConverterException("The maximum number of object registers has been reached. Please split your method and try again.");
      }

      if (adjListI == null) adjListI = defaultAdjList();
      if (adjListD == null) adjListD = defaultAdjList();
      if (adjListO == null) adjListO = defaultAdjList();
   ...
   public boolean allocateRegisters(CFG cfg, int regType)
   {
      ...
      switch (regType)
      {
         case TCConstants.RegI: pruneGraph(cfg.iCount()); break;
         case TCConstants.RegD: pruneGraph(cfg.v64Count()); break;
         case TCConstants.RegO: pruneGraph(cfg.oCount()); break;
      }
      return assignRegs();
   }
   ...
   public void buildAdjMtx(CFG cfg, int regType)
   {
      switch (regType)
      {
         case TCConstants.RegI: createMatrix(cfg.iCount()); break;
         case TCConstants.RegD: createMatrix(cfg.v64Count()); break;
         case TCConstants.RegO: createMatrix(cfg.oCount()); /*reserveRegOZero();*/ break;
      }
      ...
      for(int i=0;i<cfg.order.length;i++)
      {
         ...
         if (insts != null)
         {
            for(int j=0;j<insts.length;j++)
            {
               ...
               switch (regType)
               {
                  case TCConstants.RegI: def = ud[3]; break;
                  case TCConstants.RegD: def = ud[4]; break;
                  case TCConstants.RegO: def = ud[5];
               }               
   ...
   public void buildAdjList(CFG cfg, int regType)
   {
      ...
      switch (regType)
      {
         case TCConstants.RegI: len = cfg.iCount(); break;
         case TCConstants.RegD: len = cfg.v64Count(); break;
         case TCConstants.RegO: len = cfg.oCount(); break;
      }
      createAdjList(len);
      for (int i=0;i<len;i++)
      {
