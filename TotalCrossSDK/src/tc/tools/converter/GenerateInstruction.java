...
// $Id: GenerateInstruction.java,v 1.74 2011-01-04 13:19:25 guich Exp $
...
public class GenerateInstruction implements TCConstants
{
      ...
      switch (toKind)
      {
         ...
         case opr_reg16:
         {
            if (OperandKind.isConstant(opr.kind))
            {
               ...
               switch (toKind)
               {
                  case opr_regIb: r = new OperandRegIb(); break;
                  case opr_regIc: r = new OperandRegIs(); break;
                  case opr_regIs: r = new OperandRegIc(); break;
                  case opr_reg16: r = new OperandReg16(); break;
               }
               newInstruction(vcode, pref_MOV, r, opr, line);
               return r;
            }
   ...
   public static final Instruction newInstruction(Vector vcode, int prefix, int line)
   {
      ...
      switch (prefix)
      {
         case pref_RETURNV: c = new Instruction(RETURN_void, line); break;
         default:
   ...
   public static final Instruction newInstruction(Vector vcode, int prefix, Operand opr, int line)
   {
      ...
      switch (opcode)
      {
         ...
         {
            c = new Reg(opcode, line);
            OperandReg r = OperandKind.isTypeL(opr.kind)
   ...
   public static final Instruction newInstruction(Vector vcode, int prefix, Operand opr1, Operand opr2, int line)
   {
      ...
      switch (opcode)
      {
         ...
         {
            c = new Reg_reg(opcode, line);
            OperandReg op = OperandKind.isTypeL(opr2.kind)
   ...
   public static final Instruction newInstruction(Vector vcode, int prefix, Operand opr1, Operand opr2, Operand opr3, int line)
   {
      boolean swap = false;
   ...
   public static final Instruction newInstruction(Vector vcode, OperandReg regO, OperandSym sym, Operand dims[], int line)
   {
   ...
   public static final Instruction newInstruction(Vector vcode, int opcode, OperandSym sym, OperandReg _this, Operand retAndParams[], boolean isVoid, int line)
   {
   ...
   public static void GEN_CALL(Vector vcode, Instruction c, OperandSym sym, OperandReg _this, Operand retAndParams[], int line)
   {
      Call i = (Call) c;
      i.sym = sym.index;
      i._this = _this.index;
      ...
      for (int j=1, k=0; j<len; j++)
      {
         ...
         if (retAndParams[j] instanceof OperandConstant)
         {
         ...
         else
         {
            ...
            value = reg.index;
            if (OperandKind.isTypeI(reg.kind)) type = type_Int;
            else
            if (OperandKind.isTypeO(reg.kind)) type = type_Obj;
         }
         switch (j & 3)
         {
            case 1:
