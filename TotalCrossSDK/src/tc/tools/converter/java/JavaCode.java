...
// $Id: JavaCode.java,v 1.22 2011-01-04 13:19:07 guich Exp $
...
public final class JavaCode implements JConstants
{
   ...
   public JavaCode(JavaMethod method, DataStream ds, JavaConstantPool cp) throws totalcross.io.IOException
   {
      ...
      if (dump) System.out.println(method.signature);
      // code
      int n = ds.readInt();
      if (n > 0)
      {
         ...
         for (; ByteCode.pc < n; i++)
         {
            ...
            {
               System.out.println("Error in "+method.classOfMethod+"."+method.signature+"\nInvalid bytecode index: "+bcode);
               for (int j =0; j < i; j++)
            ...
            bc.bc = bcode;
            bc.jc = this;
            if (false) System.out.println(tempBcs[i]);
      ...
      for (int i =0; i < n; i++)
      {
         ...
         {
            if (dump) System.out.println("Skipping attribute "+name+" in method "+method.signature);
            int len = ds.readInt();
