...
// $Id: BitSet.java,v 1.8 2011-01-04 13:19:30 guich Exp $
...
final public class BitSet
{
   ...
   {
      if (elems == null) allocate();
      if (bits.elems == null) bits.allocate();
   ...
   public int first()
   {
      if (elems == null) 
         return -1;
      int i=0,n=nBits;  
      for (; i < n; i += 8) // find the first byte not 0
         if (elems[i>>3] != 0)
            break;
      for (; i < n; i++)
         if (isOn(i))
