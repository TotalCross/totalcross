...
// $Id: BasicBlock.java,v 1.20 2011-01-04 13:19:30 guich Exp $
...
final public class BasicBlock
{
   ...
   public Instruction[] vector2Array()
   {
      ...
      {
         int i = 0;
         while ((Instruction) instructions.items[i] != first) i++;
         int startIndex = i;
         while ((Instruction) instructions.items[i] != last) i++;
         int endIndex = i;
   ...
   public void computeLiveUsesAtInstr(int regType)
   {
      ...
      switch (regType)
      {
         case TCConstants.RegI : currout = outI; break;
         case TCConstants.RegD : currout = outD; break;
         case TCConstants.RegO : currout = outO;
      }
      ...
      for (int k=block.length-1; k>=0; k--)
      {
         ...
         switch (regType)
         {
            case TCConstants.RegI : u = ud[0]; d = ud[3]; break;
            case TCConstants.RegD : u = ud[1]; d = ud[4]; break;
            case TCConstants.RegO : u = ud[2]; d = ud[5]; break;
         }
