   ...
   public BitSet[] defsAndUses(BasicBlock bb, int iCount, int dCount, int oCount)
   {
      ...
      switch (op)
      {
         ...
         case CALL_virtual:
         {
            ...
            {
               case type_Int   : bits[inst.isVoid ? 0 : 3].on(inst.retOrParam); break;
               case type_Double:
               case type_Long  : bits[inst.isVoid ? 1 : 4].on(inst.retOrParam); break;
               case type_Obj   : bits[inst.isVoid ? 2 : 5].on(inst.retOrParam); break;
            }
            if (inst.params != null)
            {
               for (int i=0; i<inst.params.length; i++)
               {
                  ...
                  {
                     case type_Int   : bits[0].on(p.param1); break;
                     case type_Double:
                     case type_Long  : bits[1].on(p.param1); break;
                     case type_Obj   : bits[2].on(p.param1); break;
                  }
                  switch (p.typeOfParam2)
                  {
                     case type_Int   : bits[0].on(p.param2); break;
                     case type_Double:
                     case type_Long  : bits[1].on(p.param2); break;
                     case type_Obj   : bits[2].on(p.param2); break;
                  }
                  switch (p.typeOfParam3)
                  {
                     case type_Int   : bits[0].on(p.param3); break;
                     case type_Double:
                     case type_Long  : bits[1].on(p.param3); break;
                     case type_Obj   : bits[2].on(p.param3); break;
                  }
                  switch (p.typeOfParam4)
                  {
                     case type_Int   : bits[0].on(p.param4); break;
                     case type_Double:
                     case type_Long  : bits[1].on(p.param4); break;
                     case type_Obj   : bits[2].on(p.param4); break;
                  }
               }
            }
            break;
      ...
      if (belongsTry)
      {
         ...
         if (def>=0 && bb.outI.isOn(def)) bits[0].on(bits[3]);
         def = bits[4].first();
         if (def>=0 && bb.outD.isOn(def)) bits[1].on(bits[4]);
         def = bits[5].first();
         if (def>=0 && bb.outO.isOn(def)) bits[2].on(bits[5]);
      }
      return bits;
   }
   ...
   public void virtualRegs2PhysicalRegs(AdjListNode[] adjListI, AdjListNode[] adjListD, AdjListNode[] adjListO)
   {
      ...
      switch (op)
      {
         ...
         case MOV_regI_regI:
         {
            ...
            if (inst.r0 == inst.r1) // if the move is to the same register, zero it to make the code smaller here
         ...
         case MOV_aru_reg16:
         {
            ...
            if (inst.idx > -1) inst.idx = adjListI[inst.idx].color;
            break;
         }
         case MOV_regO_aru:
         case MOV_regO_arc:
         ...
         case NEWARRAY_multi:
         {
            ...
            for (int i=0; i<inst.params.length; i++)
            {
               ...
               if (p.typeOfParam1 == type_Int) p.param1 = adjListI[p.param1].color;
               if (p.typeOfParam2 == type_Int) p.param2 = adjListI[p.param2].color;
               if (p.typeOfParam3 == type_Int) p.param3 = adjListI[p.param3].color;
               if (p.typeOfParam4 == type_Int) p.param4 = adjListI[p.param4].color;
            }
            break;
         }
         ...
         case CALL_virtual:
         {
            ...
            switch (inst.retOrParamType)
            {
               case type_Int   : inst.retOrParam = adjListI[inst.retOrParam].color; break;
               case type_Double:
               case type_Long  : inst.retOrParam = adjListD[inst.retOrParam].color; break;
               case type_Obj   : inst.retOrParam = adjListO[inst.retOrParam].color; break;
            }
            if (inst.params != null)
            {
               for (int i=0; i<inst.params.length; i++)
               {
                  ...
                  switch (p.typeOfParam1)
                  {
                     case type_Int   : p.param1 = adjListI[p.param1].color; break;
                     case type_Double:
                     case type_Long  : p.param1 = adjListD[p.param1].color; break;
                     case type_Obj   : p.param1 = adjListO[p.param1].color; break;
                  }
                  switch (p.typeOfParam2)
                  {
                     case type_Int   : p.param2 = adjListI[p.param2].color; break;
                     case type_Double:
                     case type_Long  : p.param2 = adjListD[p.param2].color; break;
                     case type_Obj   : p.param2 = adjListO[p.param2].color; break;
                  }
                  switch (p.typeOfParam3)
                  {
                     case type_Int   : p.param3 = adjListI[p.param3].color; break;
                     case type_Double:
                     case type_Long  : p.param3 = adjListD[p.param3].color; break;
                     case type_Obj   : p.param3 = adjListO[p.param3].color; break;
                  }
                  switch (p.typeOfParam4)
                  {
                     case type_Int   : p.param4 = adjListI[p.param4].color; break;
                     case type_Double:
                     case type_Long  : p.param4 = adjListD[p.param4].color; break;
                     case type_Obj   : p.param4 = adjListO[p.param4].color; break;
                  }
               }
            }
            break;
