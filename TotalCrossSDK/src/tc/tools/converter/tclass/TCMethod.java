...
// $Id: TCMethod.java,v 1.47 2011-01-04 13:19:25 guich Exp $
...
public final class TCMethod implements TCConstants
{
   ...
   public void write(DataStreamLE ds) throws totalcross.io.IOException
   {
      ...
      if (tc.tools.converter.J2TC.dump)
      {
         ...
         System.out.println(") i="+iCount+",o="+oCount+",v64="+v64Count+" - "+(lineNumbers==null?"lines=0":("lines="+lineNumbers.length)));
      }
      else
      if (iCount >= 64 || oCount >= 64 || v64Count >= 64) System.err.println("Warning: "+tcclass.className+"."+GlobalConstantPool.getMethodFieldName(cpName)+" has reached 64 registers. i="+iCount+",o="+oCount+",v64="+v64Count);
      ...
      if (lineNumberDebugInfoCount > 0 && !name.equals("<S>"))
      {
         ...
         if (lineNumberDebugInfoCount > 2 && lineNumbers[0].lineNumber == 0) // fix some invalid lines
         ...
         for (i=1; i < lineNumberDebugInfoCount; i++)
         {
            ...
            {
               System.out.print("Cannot write line numbers for "+tcclass.className+"."+name+" with "+paramCount+" parameters ("+p+"/"+l+"). ");
               if (l >= ALL_1) 
                  System.out.println("Move statement at line "+lineNumbers[i].lineNumber+" to above line "+lineNumbers[0].lineNumber);
               else
                  System.out.println();
      ...
      else lineNumberDebugInfoCount = 0;
      ...
      if (lineNumberDebugInfoCount > 1)
      {
         ...
         if (allLN1)
            ...
            for (i=1; i < lineNumberDebugInfoCount; i++)
            {
               if (lineNumbers[i].lineNumber < lineNumbers[i-1].lineNumber) // make sure it always goes forward
