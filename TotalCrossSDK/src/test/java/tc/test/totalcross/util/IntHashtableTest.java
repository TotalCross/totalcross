...
/*      ByteArrayStream bas = new ByteArrayStream(1000);
      ...
      ht.saveTo(ds);
*/      // get some keys
      try {assertEquals(1000,ht.get(1));} catch (ElementNotFoundException e) {fail();}
      try {assertEquals(2000,ht.get(2));} catch (ElementNotFoundException e) {fail();}
      try {assertEquals(7000,ht.get(7));} catch (ElementNotFoundException e) {fail();}
      try {assertEquals(11000,ht.get(11));} catch (ElementNotFoundException e) {fail();}
      try {ht.get(0); fail(); } catch (ElementNotFoundException e) {/* ok */}
      ...
      for (int i =0; i < 11; i++)
      {
         ...
         try {assertEquals(k*1000,ht.get(k));} catch (ElementNotFoundException e) {fail();}
      }
      // remove some
      for (int i =1; i <= 11; i++)
         if ((i&1) == 1)
            try {ht.remove(i);} catch (ElementNotFoundException e) {fail();}
      // check if they were successfully removed
      for (int i =1; i <= 11; i++)
         if ((i&1) == 0)
            try {ht.get(i);} catch (ElementNotFoundException e) {fail();}
      // remove everything and make sure they were removed
      ht.clear();
      assertEquals(0, ht.size());
      for (int i =1; i <= 11; i++)
         try {ht.get(i); fail(); } catch (ElementNotFoundException e) {}
