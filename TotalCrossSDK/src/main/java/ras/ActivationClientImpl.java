...
final class ActivationClientImpl extends ActivationClient {
  ...
  public ActivationClientImpl() {
    ...
    if (htDump != null) {
      ...
      if ((field = htVmParams.getString("iosCertDate")) != null) {
        try {
          Settings.iosCertDate = new Time(field);
        } catch (Exception e) {
        }
      }
  ...
  public boolean isActivated() throws ActivationException {
    ...
    try {
      ...
      Settings.activationId = Convert
          .bytesToHexString(generateActivationCode(key, Convert.hexStringToBytes((String) deviceInfo.get("HASH"))));
      ...
      if (!checkRegistration) {
        checkRegistration = (deviceIMEI != null) && (activationIMEI != null) && (deviceIMEI.length() > 0)
            && deviceIMEI.equals(activationIMEI);
      }
      ...
      if (!Settings.WIN32.equals(activationPlatform)
          && !isValidActivationCode(Convert.hexStringToBytes(request.getActivationCode()),
      ...
      if (expiration < compilationDate) {
        throw new Exception("Please purchase renewal credits or use a VM version released before "
            + new totalcross.util.Date(expiration) + ".");
      }
  ...
  public void activate() throws ActivationException {
    ...
    try {
      ...
      ActivationRequest request = new ActivationRequest(productInfo, deviceInfo,
          Convert.bytesToHexString(activationCode));
  ...
  private byte[] aesEncrypt(byte[] dec) throws IOException, CryptoException {
    aesCipher.reset(Cipher.OPERATION_ENCRYPT, aesKey, Cipher.CHAINING_CBC, null, Cipher.PADDING_PKCS5);
  ...
  private byte[] getSignerRSAKeyN() {
    return Convert.ints2bytes(new int[] { 0xC0699C00, 0x0A92507E, 0x33020F10, 0xE6CB2A47, 0xE3A18FC9, 0xAA75AEDA,
        0x7EDFA963, 0x14112344, 0x34711273, 0x89DD103E, 0xB8C82856, 0x407CBBB9, 0x529A8F2A, 0xCEE404DE, 0x963C835C,
        0xF9B479C7, 0x388689C3, 0x76BF22E8, 0x6408059E, 0x4507B358, 0x02D991BE, 0x9FA869B5, 0x2C705528, 0x214EB1EB,
        0x5BCBFCE5, 0xC81E5C4F, 0xA92A6489, 0xBAA7ACA1, 0xA0FD4C74, 0x8B90AB3A, 0x42E7B3F0, 0x4258CE77, 0x00000067 },
        129);
  }
}
