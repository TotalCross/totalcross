...
{
  /**
   * Set this to a classpath that will have precedence over the classpath
   * passed by Java. This allows you to call tc.Deploy directly from an
   * application.
   */
  public static String bootClassPath;

  public static void main(String[] args) throws Exception { new Deploy(args); }
  ...
  public Deploy(String[] args) throws Exception
  {
    try {
      ...
      if (DeploySettings.testClass) {
        ...
        System.out.println(
          "TCZ file created, but no target platforms specified. Type \"java tc.Deploy\" for help.");
      } else {
        if (DeploySettings.etcDir == null ||
            !new File(DeploySettings.etcDir).exists()) {
        ...
        if (DeploySettings.filePrefix == null) {
          throw new DeployerException(
            "Error: MainWindow or library not found!");
        }
        ...
        if ((options & BUILD_WINCE) != 0) {
          ...
          new Deployer4WinCE(
            false); // there's no need to build for winmo if built for wince
        }
        ...
        if ((options & BUILD_IPHONE) != 0) {
          if (Deployer4IPhoneIPA.certStorePath == null) { // Use default
            Deployer4IPhoneIPA.certStorePath =
              DeploySettings.etcDir + "tools" + File.separator + "ipa";
            Deployer4IPhoneIPA.buildIPA = true;
            Deployer4IPhoneIPA.mobileProvision = new File(
              Deployer4IPhoneIPA.certStorePath, "dummy.mobileprovision");
        ...
        if (!DeploySettings.testClass && (options & BUILD_APPLET) != 0 &&
        ...
        if (!DeploySettings.testClass && DeploySettings.showBBPKGName != null) {
          System.out.println(
            ...
            ".cod package was created. The files will be copied to the folder \"" +
            DeploySettings.showBBPKGRoot +
        ...
        if (DeploySettings.mainClassName == null && DeploySettings.isJarOrZip) {
          ...
          System.out.println(
            "\nThe file '" + fileName + "' does not contain a class named '" +
            name +
          ...
          if (!DeploySettings.filePrefix.equals("TCBase") &&
              !DeploySettings.filePrefix.equals("TCUI") &&
              !DeploySettings.filePrefix.toLowerCase().endsWith("lib")) {
            System.out.println(
              "If this file is really a library, you must name it " +
              DeploySettings.filePrefix + "Lib." + ext +
              ", or it will NOT be loaded in the device.");
          }
        }
  ...
  {
    // flsobral@tc210: dynamically load some useful libs for handling files and
    // compression
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/bouncycastle/bcprov-jdk15on-1.56.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/bouncycastle/bcpkix-jdk15on-1.56.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/commons/commons-io-2.2.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/commons/commons-compress-1.4.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/truezip/truezip-driver-file-7.5.1.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/truezip/truezip-driver-zip-7.5.1.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/truezip/truezip-file-7.5.1.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/truezip/truezip-kernel-7.5.1.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/truezip/truezip-swing-7.5.1.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "tools/ipa/dd-plist.jar");
    JarClassPathLoader.addFile(DeploySettings.etcDir +
                               "libs/totalcross-annotations.jar");
  ...
  {
    try { // guich@500_8: verify if the tc.jar is in the classpath (IntHashtable
          // is needed by Bitmap)
      Class.forName("totalcross.util.IntHashtable");
    } catch (ClassNotFoundException cd) {
      throw new DeployerException(
        "You must also add /TotalCross3/lib/TotalCross.jar to the classpath!");
    }
  }
  ...
  private int parseOptions(String[] args) throws Exception
  {
    String arg0 = args[0];
    if (arg0.startsWith("-") ||
        (arg0.startsWith("/") && !new File(arg0).exists())) {
      throw new DeployerException(
        "The first parameter must be the class or package name!");
    }
        ...
        switch (op.charAt(1)) {
          ...
          case 'n':
            ...
            if (DeploySettings.filePrefix.toLowerCase().endsWith(".tcz")) {
              DeploySettings.filePrefix = DeploySettings.filePrefix.substring(
                0, DeploySettings.filePrefix.length() - 4);
            }
            DeploySettings.isTotalCrossJarDeploy =
              DeploySettings.filePrefix.equals("TCBase") ||
              DeploySettings.filePrefix.equals("TCUI");
            break;
          case 'x':
            DeploySettings.excludeOptionSet = true;
            String[] exc =
              totalcross.sys.Convert.tokenizeString(args[++i], ',');
          ...
          case 'p':
            ...
            // keep compatibility with old sdk by ignoring the parameter after
            // /p
