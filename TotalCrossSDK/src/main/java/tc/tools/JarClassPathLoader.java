...
/**
 * Utility class that dynamically loads a jar file into the Deploy
 * classpath.<br> It uses reflection to grant access to the loaded jar, a little
 * hackish but that's the easiest way of doing it.<br> Another option would be
 * to iterate through the jar entries and load classes one by one, which is not
 * much better than this IMHO. So I decided to go with the easiest-to-implement
 * solution.
 *
 * @author Fabio Sobral
