...
public class FontGenerator {
	...
	private void convertFont(Vector v, java.awt.Font f, String fileName, List<Range> newRanges,
							 boolean isMono) {
		java.awt.FontMetrics fm = comp.getFontMetrics(f);
		...
		for (int ri = 0; ri < n; ri++) {
			...
			for (i = ini; i <= end; i++) {
				...
				if (r == null) { // blank char?
					...
					gaps[i] = (byte)
							  gap;//<=0 ? 1 : 0; // most chars Java puts 1 pixel away; some chars Java puts one pixel near; for those chars, we make them one pixel right
		...
		for (int ri = 0; ri < n; ri++) {
			...
			println((ri + 1) + " of " + n + ". Original height: " + fm.getHeight() + ", ascent: " +
					fm.getMaxAscent()
	...
	static class PalmFont {
		...
		public TCZ.Entry save() {
			try {
				...
				ds.writeShort(
					antialiased); // note that writeShort already writes an unsigned short (its the same method)
