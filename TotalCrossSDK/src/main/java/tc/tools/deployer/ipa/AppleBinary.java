...
/**
 * Abstract class that represents an apple binary, currently supports ARM, ARM64
 * and FAT (Universal) binaries. Subclasses should not be instantiated directly,
 * use instead the method {@link #create(byte[])}.
 *
 * @author Fabio Sobral
 */
public abstract class AppleBinary
{
  enum Type
  {
    ...
    Type(long magic) { this.magic = magic; }
  ...
  public static AppleBinary create(byte[] data)
  ...
  protected AppleBinary(byte[] data) { this.data = data; }

  abstract public byte[] resign(KeyStore ks,
                                X509Store certStore,
