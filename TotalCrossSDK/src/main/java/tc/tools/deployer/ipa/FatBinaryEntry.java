...
public class FatBinaryEntry
{
  ...
  public void resign(ElephantMemoryWriter writer,
                     KeyStore ks,
                     X509Store certStore,
                     ...
                     byte[] info,
                     byte[] sourceData)
    throws UnrecoverableKeyException, CertificateEncodingException,
           KeyStoreException, NoSuchAlgorithmException,
           OperatorCreationException, IOException, CMSException
  {
    writer.align((int)offset);
    byte[] resignedFile = file.resign(
      ks, certStore, bundleIdentifier, entitlementsBytes, info, sourceData);
