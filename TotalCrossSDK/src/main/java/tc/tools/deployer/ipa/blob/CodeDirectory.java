...
public class CodeDirectory extends BlobCore
{
  ...
  /**
   * Types of hashes supported. Actually, right now, only SHA1 is really
   * supported.
   */
  ...
  /**
   * Special hash slot values. In a CodeDirectory, these show up at negative
   * slot indices. This enumeration is also used widely in various internal
   * APIs, and as type values in embedded SuperBlobs.
   */
  ...
  public CodeDirectory() { super(CSMAGIC_CODEDIRECTORY); }
  ...
  public CodeDirectory(String identifier, long codeLimit)
  {
    ...
    this.nCodeSlots =
      (long)((this.codeLimit + actualPageSize - 1) / actualPageSize);
    ...
    this.hashes =
      new byte[(int)((this.nSpecialSlots + this.nCodeSlots) * this.hashSize)];
  }

  public void setSpecialSlotsHashes(byte[] info,
                                    byte[] requirements,
                                    byte[] resourceDir,
                                    byte[] application,
                                    byte[] entitlements)
  ...
  public void setCodeSlotsHashes(byte[] data)
  {
    for (int i = 0; i < this.nCodeSlots; i++) {
      ...
      hashDigest.doFinal(this.hashes,
                         (int)((this.nSpecialSlots + i) * this.hashSize));
    }
  }
  ...
  protected void writeToStream(ElephantMemoryWriter writer) throws IOException
  {
    ...
    // hashOffset starts after the identifier and the hashes of the special
    // slots
    this.hashOffset = this.identOffset + (identifier.length() + 1) +
                      (this.hashSize * this.nSpecialSlots);
  ...
  protected void readFromStream(ElephantMemoryReader reader) throws IOException
  {
    ...
    for (long i = 0L; i < num4; i += 1L) {
      ...
      reader.read(b);
      System.arraycopy(
        b, 0, this.hashes, (int)(i * this.hashSize), (int)this.hashSize);
    }
    reader.moveBack();
  }
}
