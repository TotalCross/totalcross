...
public class BlobWrapper extends BlobCore {
	...
	throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException,
		...
		X509CertificateHolder cert = new X509CertificateHolder(keyStore.getCertificate(
					firstAlias).getEncoded());
		ContentSigner sha1Signer = new JcaContentSignerBuilder("SHA1withRSA").setProvider("BC").build(priv);
		DigestCalculatorProvider digestCalculator = new
		JcaDigestCalculatorProviderBuilder().setProvider("BC").build();
		SignerInfoGenerator signerGenerator = new JcaSignerInfoGeneratorBuilder(digestCalculator).build(
			sha1Signer, cert);
