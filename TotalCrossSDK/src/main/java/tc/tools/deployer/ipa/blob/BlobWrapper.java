...
public class BlobWrapper extends BlobCore
{
  ...
  public BlobWrapper() { super(CSMAGIC_BLOB_WRAPPER); }

  public BlobWrapper(KeyStore keyStore,
                     X509Store certStore,
           ...
           OperatorCreationException, IOException, CMSException
  {
    ...
    PrivateKey priv =
      (PrivateKey)(keyStore.getKey(firstAlias, "".toCharArray()));

    X509CertificateHolder cert = new X509CertificateHolder(
      keyStore.getCertificate(firstAlias).getEncoded());
    ContentSigner sha1Signer =
      new JcaContentSignerBuilder("SHA1withRSA").setProvider("BC").build(priv);
    DigestCalculatorProvider digestCalculator =
      new JcaDigestCalculatorProviderBuilder().setProvider("BC").build();
    SignerInfoGenerator signerGenerator =
      new JcaSignerInfoGeneratorBuilder(digestCalculator)
  ...
  public void sign() throws IOException, CMSException
  {
    ...
    CMSProcessableByteArray content =
      (rawData != null) ? new CMSProcessableByteArray(rawData) : null;
