...
public class MachObjectFile extends AppleBinary
{
  ...
  protected MachObjectFile(byte[] data)
    throws IOException, InstantiationException, IllegalAccessException
  {
    ...
    for (int i = 0; i < ncmds; i++) {
      ...
      if (command != null) {
        ...
        if (lc_signature == null && command instanceof
    ...
    if (lc_segment == null || lc_signature == null ||
        (lc_signature.blobFileOffset + lc_signature.blobFileSize) !=
          (lc_segment.fileoff + lc_segment.filesize)) {
      throw new RuntimeException(
        "Template IPA files appears to be corrupted, please reinstall the SDK and try again");
    }
  }
  ...
  public void setEmbeddedSignature(EmbeddedSignature signature)
    throws IOException
  {
    ...
    // original size - size of the original signature + size of the new
    // signature
    lc_segment.updateFileSize(writer,
                              lc_segment.filesize - lc_signature.blobFileSize +
                                signatureTemplate.length);
  ...
  @Override
  public byte[] resign(KeyStore ks,
                       X509Store certStore,
                       String bundleIdentifier,
                       byte[] entitlementsBytes,
                       byte[] info,
                       byte[] sourceData)
    throws IOException, CMSException, UnrecoverableKeyException,
           CertificateEncodingException, KeyStoreException,
           NoSuchAlgorithmException, OperatorCreationException
  {
    // create a new codeDirectory with the new identifier, but keeping the same
    // codeLimit
    CodeDirectory codeDirectory = new CodeDirectory(
      bundleIdentifier, lc_signature.signature.codeDirectory.codeLimit);
    ...
    EmbeddedSignature newSignature = new EmbeddedSignature(
    ...
    // recalculate hashes
    codeDirectory.setSpecialSlotsHashes(
      info, requirements.getBytes(), sourceData, null, entitlements.getBytes());
    ...
    if (signatureTemplate.length != resignedData.length) {
      throw new IllegalStateException(
        "Failed to resign the file, please try again.");
    }
    ...
    if (actualSize < expectedSize) {
      throw new IllegalStateException(
        "Generated file appears to be missing data, please try again.");
