...
public class MachObjectFile extends AppleBinary {
	...
	protected MachObjectFile(byte[] data) throws IOException, InstantiationException,
		...
		if (lc_segment == null || lc_signature == null
			|| (lc_signature.blobFileOffset + lc_signature.blobFileSize) != (lc_segment.fileoff +
					lc_segment.filesize)) {
	...
	public void setEmbeddedSignature(EmbeddedSignature signature) throws IOException {
		...
		lc_segment.updateFileSize(writer,
								  lc_segment.filesize - lc_signature.blobFileSize + signatureTemplate.length);
	...
	@Override
	public byte[] resign(KeyStore ks, X509Store certStore, String bundleIdentifier,
						 byte[] entitlementsBytes,
						 byte[] info,
						 byte[] sourceData) throws IOException, CMSException, UnrecoverableKeyException,
		...
		CodeDirectory codeDirectory = new CodeDirectory(bundleIdentifier,
				lc_signature.signature.codeDirectory.codeLimit);
		...
		EmbeddedSignature newSignature = new EmbeddedSignature(codeDirectory, entitlements, requirements,
				blobWrapper);
		...
		// recalculate hashes
		codeDirectory.setSpecialSlotsHashes(info, requirements.getBytes(), sourceData, null,
											entitlements.getBytes());
