...
public class MobileProvision
{
  ...
  public MobileProvision(String content) throws Exception
  {
    ...
    NSDictionary rootDictionary =
      (NSDictionary)PropertyListParser.parse(content.getBytes("UTF-8"));

    // ApplicationIdentifierPrefix
    NSArray array =
      (NSArray)rootDictionary.objectForKey("ApplicationIdentifierPrefix");
    applicationIdentifierPrefix =
      array.count() > 0 ? array.objectAtIndex(0).toString() : null;
    ...
    developerCertificates =
      array != null ? new Certificate[array.count()] : null;
    if (developerCertificates != null && developerCertificates.length > 0) {
      ...
      for (int i = 0; i < certificates.length; i++) {
        ByteArrayInputStream certificateData =
          new ByteArrayInputStream(((NSData)certificates[i]).bytes());
    ...
    NSNumber number =
      (NSNumber)rootDictionary.objectForKey("ProvisionsAllDevices");
  ...
  public static MobileProvision readFromFile(File input) throws Exception
  {
    ...
    int length = ((inputData[startIdx - 2] & 0xFF) << 8) |
                 (inputData[startIdx - 1] & 0xFF);
