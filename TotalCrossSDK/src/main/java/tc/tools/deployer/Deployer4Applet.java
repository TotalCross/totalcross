...
public class Deployer4Applet
{
  ...
  private void addZipEntry(ZipOutputStream zos, String name, byte[] bytes)
    throws Exception
  {
  ...
  private void addHashtable2zip(Hashtable ht, ZipOutputStream zos)
    throws Exception
  {
    ...
    for (int i = 0; i < n; i++) {
      ...
      byte[] bytes =
        (o instanceof JavaClass) ? ((JavaClass)o).bytes : (byte[])o;
  ...
  private void writeFile(String targetDir, String name) throws Exception
  {
    ...
    String out =
      ...
      DeploySettings.filePrefix + ".jar" +
      (DeploySettings.isJarOrZip ? ",tc.jar" : "") + "\" width=" + w +
      " height=" + h + ">\n"
