...
public class Bitmaps {
	...
	public class Bmp {
		...
		public Bmp(byte[] bytes) {
			...
			if (palette[0] == 0xFFFFFF && width != 22 && width != 15 && width != 30 && width != 44
				&& width != 20) {
	 ...
	 */
	private void copyBits(byte[] src, int[] paletteFrom, int[] paletteTo, int srcWidthInBytes,
						  byte[] dest,
	...
	public void saveWinCEIcons(byte[] bytes, int bitmap16x16x8_Offset, int bitmap32x32x8_Offset,
	...
	throws ImageException, IOException {
		...
		if (bmp16x16x8 != null && bitmap16x16x8_Offset != -1) {
			copyBits(bmp16x16x8.pixels, bmp16x16x8.palette, loadPalette(bytes, bitmap16x16x8_Offset + 40, 256),
					 16, bytes,
		...
		if (bmp32x32x8 != null && bitmap32x32x8_Offset != -1) {
			copyBits(bmp32x32x8.pixels, bmp32x32x8.palette, loadPalette(bytes, bitmap32x32x8_Offset + 40, 256),
					 32, bytes,
		...
		if (bmp48x48x8 != null && bitmap48x48x8_Offset != -1) {
			copyBits(bmp48x48x8.pixels, bmp48x48x8.palette, loadPalette(bytes, bitmap48x48x8_Offset + 40, 256),
					 48, bytes,
	...
	public void saveWin32Icon(byte[] bytes, int iconOffset) throws ImageException, IOException {
		...
		if (bmp32x32x8 != null && iconOffset != -1) {
			copyBits(bmp32x32x8.pixels, bmp32x32x8.palette, loadPalette(bytes, iconOffset + 40, 256), 32, bytes,
					 iconOffset + 40 + 1024, 32, -32, 8, iconOffset + 32 * 32 + 40 + 1024, false,
					 bmp32x32x8.shouldInvertY);
		}
	}
	...
	static class Image4iOS {
		...
		public byte[] getImage() {
			byte[] b = null;
			try {
				Image img = width == height ? IconStore.getSquareIcon(width) : IconStore.getSplashImage(width,
							height);
...
class IconStore extends Hashtable {
	...
	public static Bmp getBmp(int width, int height, int bpp) throws IOException, ImageException {
		...
		return b != null ? DeploySettings.bitmaps.new Bmp(b) : DeploySettings.bitmaps.new Bmp(width,
				height);
	}
	...
	public static Image getSplashImage(int width, int height) throws ImageException, IOException {
		...
		if (img.getHeight() != height || img.getWidth() != width) {
			throw new ImageException("splash" + img.getHeight() + "x" + img.getWidth() + " must be " + width +
									 "x" + height);
		}
		return img;
	}
	...
	public static Image getSquareIcon(int size) throws ImageException, IOException {
		...
		if (img.getHeight() != size || img.getWidth() != size) {
			throw new ImageException("icon" + img.getHeight() + "x" + img.getWidth() + " must be " + size + "x"
									 + size);
		}
		return img;
	}
	...
	public static Image getIcon(int width, int height) throws ImageException, IOException {
		...
		if (img.getHeight() != height || img.getWidth() != width) {
			throw new ImageException("icon" + img.getHeight() + "x" + img.getWidth() + " must be " + height +
									 "x" + width);
		}
		return img;
	}
