...
public class DeploySettings {
	...
	public static List<java.io.File> getDefaultTczs() {
		if (defaultTczs == null) {
			...
			defaultTczs.add(new java.io.File(Convert.appendPath(DeploySettings.etcDir, "fonts"),
											 "Material Icons.tcz"));
		}
		return defaultTczs;
	}
	...
	public static void init() throws Exception {
		...
		isBuggyJDKVersionForSynchronizedKeyword = dJavaVersion < 1.7 && dJavaVersion > 1.5
				&& subver <= 6; // 1.6 and <= 6 ?
		...
		String cp0;
		String cp = cp0 = tc.Deploy.bootClassPath != null ? tc.Deploy.bootClassPath :
		...
		System.out.println("TotalCross SDK version " + Settings.versionStr + "." + Settings.buildNumber +
						   " running on "
						   + osName + " with JDK " + javaVersion);
		System.out.println("Current folder: " + currentDir);
		System.out.println("Etc directory: " + (etcDir != null ? etcDir :
												"not found")); // keep this always visible, its a very important information
		...
		while (true) {
			try {
				...
				try
					...
					try
						(StringReader stringReader = new StringReader(ht.getKeyValuePairs("=").toString("\n"));
