...
public class DeploySettings
{
  ...
  public static boolean quiet =
    true; // to set to false, pass /v(erbose) to tc.Deploy
  ...
  public static byte[] defaultArgument =
  ...
  public static boolean
    isBuggyJDKVersionForSynchronizedKeyword; // guich@tc120_0
  ...
  public static HashMap<String, String> hmMappedClasses =
  ...
  public static List<java.io.File> getDefaultTczs()
  {
    if (defaultTczs == null) {
      ...
      defaultTczs.add(
        new java.io.File(DeploySettings.folderTotalCross3DistVM, "TCBase.tcz"));
      defaultTczs.add(
        new java.io.File(DeploySettings.folderTotalCross3DistVM, "TCUI.tcz"));
      defaultTczs.add(
        new java.io.File(DeploySettings.folderTotalCross3DistVM, "TCFont.tcz"));
      defaultTczs.add(new java.io.File(DeploySettings.folderTotalCross3DistVM,
                                       "LitebaseLib.tcz"));
      defaultTczs.add(
        new java.io.File(Convert.appendPath(DeploySettings.etcDir, "fonts"),
                         "Material Icons.tcz"));
    }
    return defaultTczs;
  }
  ...
  public static void init() throws Exception
  {
    ...
    if (dJavaVersion < 1.6) {
      throw new DeployerException(
        "Error: the Deployer requires JDK 1.6 or above!");
    }
    // guich@tc120_0: check the minor version and make sure no one uses 1.6.0_06
    int subver = 100;
    try {
      subver = Integer.parseInt(
        completeVersion.substring(completeVersion.indexOf('_') + 1));
    } catch (Exception e) {
    }
    isBuggyJDKVersionForSynchronizedKeyword =
      dJavaVersion < 1.7 && dJavaVersion > 1.5 && subver <= 6; // 1.6 and <= 6 ?
    ...
    String cp0;
    String cp = cp0 = tc.Deploy.bootClassPath != null
                        ? tc.Deploy.bootClassPath
    ...
    } else if (new File(System.getenv("GIT_HOME") +
                        "/TotalCross/TotalCross3/etc")
                 .exists()) {
    ...
    System.out.println("TotalCross SDK version " + Settings.versionStr + "." +
                       Settings.buildNumber + " running on " + osName +
                       " with JDK " + javaVersion);
    System.out.println("Current folder: " + currentDir);
    System.out.println(
      "Etc directory: " +
      (etcDir != null ? etcDir : "not found")); // keep this always visible, its
    ...
    if (folderTotalCross3DistVM == null ||
        !new File(folderTotalCross3DistVM).exists()) {
      folderTotalCross3DistVM = null;
    }

    Utils.fillExclusionList(); // flsobral@tc115: exclude files contained in jar
  ...
  private static void handleTCAppProp()
  {
    ...
    while (true) {
      try {
        ...
        try (File f = new File(path, File.READ_WRITE);
             InputStreamReader fileReader =
               new InputStreamReader(f.asInputStream());
             StringWriter stringWriter = new StringWriter();) {
          IOUtils.copy(fileReader, stringWriter);
          Hashtable ht = new Hashtable(stringWriter.toString());
          appBuildNumber =
            Convert.toInt((String)ht.get("build.number", "0"), 0) + 1;
          ht.put("build.number", appBuildNumber);
          try (StringReader stringReader =
                 new StringReader(ht.getKeyValuePairs("=").toString("\n"));
               OutputStreamWriter outputWriter =
                 new OutputStreamWriter(f.asOutputStream());) {
            f.setSize(0);
      ...
      if (tcappProp == null) {
        Utils.println("File " + TCAPP_PROP +
                      " not found; build number could not be generated.");
      }
    }
  }
  ...
  public static boolean isWindows() { return osName.indexOf("windows") >= 0; }
  ...
  public static boolean isMac() { return osName.indexOf("mac") >= 0; }

  public static String appendDotExe(String s)
  { // guich@tc115_85
    return isUnix() || isMac() ? s : (s + ".exe");
  }

  public static boolean isFullScreenPlatform(String plat)
  { // guich@tc120_59
    if (fullScreenPlatforms == null) {
      return isFullScreen; // guich@tc126_32: return what user decided for all
                           // platforms
    }
