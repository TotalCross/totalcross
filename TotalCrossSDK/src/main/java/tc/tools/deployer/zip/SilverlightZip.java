 ...
 * Silverlight can only handle zip files whose contents' headers include
 * compressed size and crc, but most zip implementations for Java only include
 * this information on the extended local header. This class uses a
 * {@link BlackHoleOutputStream} to fill the ZipEntry fields before writing to
 * the actual output stream.<br> Also includes an implementation of {@link
 * FilenameFilter} to copy the contents of another zip file directly using
 * TrueZip.
 *
 * @author Fabio Sobral
 *
...
public class SilverlightZip
{
  ...
  private Hashtable<String, ZipEntry> entries =
    new Hashtable<String, ZipEntry>();
   ...
   * @param name
   * @param content
   * @return true if successful, false if there's already a file with the given
   *     name on the zip file.
   * @throws IOException
   */
