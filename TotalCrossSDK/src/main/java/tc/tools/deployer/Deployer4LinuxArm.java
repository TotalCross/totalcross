...
public class Deployer4LinuxArm
{
  ...
  public Deployer4LinuxArm() throws Exception
  {
    String linuxArguments =
      DeploySettings.commandLine; // the name of the tcz will be the same as
                                  // the executable
    byte[] args = linuxArguments.trim().getBytes();
    if (args.length > DeploySettings.defaultArgument.length) {
      throw new IllegalArgumentException(
        "Error: command line for Linux too long. It has " + args.length +
    ...
    if (DeploySettings.mainClassName != null) {
      ...
      byte[] buf = Utils.findAndLoadFile(
        DeploySettings.etcDir + "launchers/linux_arm/Launcher", false);
      if (buf == null) {
      ...
      if (ofs < 0) {
        throw new DeployerException(
          "Can't find offset for command line on linux!");
      }
      Vm.arrayCopy(args,
                   0,
                   buf,
