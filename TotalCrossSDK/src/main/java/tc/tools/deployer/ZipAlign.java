...
public class ZipAlign
{
  ...
  private void openFiles() throws IOException
  {
    ...
    mOutputStream = new FilterOutputStreamEx(
      new BufferedOutputStream(new FileOutputStream(mOutputFile), 32 * 1024));
  } // openFiles()
  ...
  private void copyAllEntries() throws IOException
  {
    ...
    while (entries.hasMoreElements()) {
      ...
      final int inputEntryHeaderSize =
        ZIP_ENTRY_HEADER_LEN +
      ...
      final XEntry xentry =
        new XEntry(entry, outputEntryHeaderOffset, flags, padding);
      ...
      if (sizeToCopy > 0) {
        ...
        while (totalSizeCopied < sizeToCopy) {
          int read = mRafInput.read(
            buf, 0, (int)Math.min(32 * 1024, sizeToCopy - totalSizeCopied));
          if (read <= 0) {
            break;
          }
  ...
  private void buildCentralDirectory() throws IOException
  {
    ...
    for (int i = 0, n = mXEntries.size(); i < n; i++) {
      ...
      mOutputStream.writeShort(entry.getExtra() != null
      ...
      if (entry.getExtra() != null) {
        mOutputStream.write(
          entry.getExtra(), 0, entry.getExtra().length - xentry.padding);
      }
      if (commentBytes != null) {
        mOutputStream.write(
          commentBytes, 0, Math.min(commentBytes.length, 0xffff));
      }
    } // for xentry
