...
public class ZipAlign {
	...
	private void openFiles() throws IOException {
		...
		mOutputStream = new FilterOutputStreamEx(new BufferedOutputStream(new FileOutputStream(mOutputFile),
				32 * 1024));
	}// openFiles()
	...
	private void copyAllEntries() throws IOException {
		...
		while (entries.hasMoreElements()) {
			...
			final int inputEntryHeaderSize = ZIP_ENTRY_HEADER_LEN + (entry.getExtra() != null ?
											 entry.getExtra().length : 0)
