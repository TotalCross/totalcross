...
public class Deployer4Win32
{
  ...
  public Deployer4Win32() throws Exception
  {
    String winArguments =
      DeploySettings
        .commandLine; // the name of the tcz will be the same of the .exe
    byte[] args = winArguments.trim().getBytes();
    if (args.length > DeploySettings.defaultArgument.length) {
      throw new IllegalArgumentException(
        "Error: command line for Win32 too long. It has " + args.length +
        ", while the maximum allowed is " +
        DeploySettings.defaultArgument.length);
    }
    ...
    if (DeploySettings.mainClassName != null) {
      ...
      byte[] buf = Utils.findAndLoadFile(
        DeploySettings.etcDir + "launchers/win32/Launcher.exe", false);
      if (buf == null) {
      ...
      if (ofs < 0) {
        throw new DeployerException(
          "Can't find offset for command line on win32!");
      }
      Vm.arrayCopy(args,
                   0,
                   buf,
                   ofs,
                   args.length); // no "write taking care" needed
      ...
      byte[] empty_icon = Utils.findAndLoadFile(
        DeploySettings.etcDir + "images/empty_icon.ico", false);
      if (empty_icon == null) {
      ...
      if (iconOffset == -1) {
        throw new DeployerException(
          "Can't find offset of empty_icon.ico in Launcher.exe");
      }
      ...
      FileUtils.copyFileToDirectory(
        new java.io.File(DeploySettings.folderTotalCross3DistVM +
                         "/win32/TCVM.dll"),
