...
public class LinuxBuildNatives
{
  ...
  public static void main(String args[]) throws Exception
  {
      ...
      if (cmd.charAt(0) == '-') {
        switch (cmd.charAt(1)) {
          ...
          case 'q':
            ...
            break;
          default:
            throw new IllegalArgumentException("Error: bad option: -" +
                                               cmd.charAt(1));
        }
      } else {
    ...
    if (category == null) {
      throw new DeployerException(
        "Error: missing -c for the library category!");
    }
    if (version == null) {
    ...
    if (targetDir == null) {
      throw new DeployerException(
        "Error: missing -t for the target directory!");
    }
  ...
  public static void createLinuxPackage(String name,
                                        String category,
                                        String description,
                                        String location,
                                        String url,
                                        ...
                                        Vector files) throws Exception
  {
    ...
    DataOutputStream dos =
      new DataOutputStream(new FileOutputStream(controlDir + outFile));
    dos.writeBytes("Package: " + name + "\n"
                   ...
                   + "Depends: " +
                   (vmPkg
                      ? "libdirectfb-1.2-0 (>= 1.2.7)"
                      : "totalcross (>= " +
                          getTCVersion(totalcross.sys.Settings.version) + ")") +
                   "\n");
    dos.close();
    ...
    Mapper mapper = new Mapper() {
      //@Override
      @Override public TarEntry map(TarEntry entry)
      {
        if (entry.getName().contains("/lib") &&
            entry.getName().contains(".so")) {
    ...
    dataProd[0] = new DataProducerDirectory(
      new java.io.File("install/linux/"),
    ...
    try {
      PackageDescriptor pdsc =
        debProc.createDeb(ctrlFiles, dataProd, debOut, "bzip2");
    ...
    Utils.copyFile(
      "install/linux/" + debName, targetDir + "/" + debName, false);
    totalcross.io.File.deleteDir("install");

    //      int ret = Runtime.getRuntime().exec("dpkg-deb -b cydia/" +
    //      name).waitFor(); if (ret != 0)
    //      {
    ...
    //      byte[] data = Utils.findAndLoadFile("etc/"+"images/" + iconfile,
    //      false); if (data != null)
    //      {
    ...
    //      }
    //      else Utils.println("...didn't found
    //      "+DeploySettings.etcDir+"images/" + iconfile);
    ...
    //         install_commands.append(shellScript("/bin/ln -s " + appPath(name)
    //         + fname + " " + appPath("TotalCross") + fname));
    //         // remove symbolic link at uninstall
    //         uninstall_commands.append(shellScript("/bin/rm -f " +
    //         appPath("TotalCross") + fname));
    //      }
    ...
    //         FileOutputStream(target)); Utils.println("...writing zipfile
    //         "+target);
    //
  ...
  {
    //@Override
    @Override public void println(String s) { Utils.println(s); }
  }
}

/*
      createCommon(vFiles, vExtras, baseDir, version, name, cmdLine, uriBase,
iconfile, FIRMWARE_2x);
...
//      +Description: an example of using APT Every day people use Cydia, but
with the instructions embodied in this package, people can publish for it as
well.
