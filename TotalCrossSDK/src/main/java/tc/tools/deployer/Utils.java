...
public class Utils
{
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  public static void copyFile(String source, String dest, boolean deleteSource)
    throws Exception
  {
        ...
        if (lines[i].length() > 0) {
          String capStart =
            lines[i]
              .substring(0,
                         lines[i].indexOf(']') + 1)
              .toUpperCase(); // switch on the first 8 capitalized letters
          if (capStart.startsWith(INST_FILE_GLOBALS) ||
              capStart.startsWith(INST_FILE_LOCALS)) {
            ...
            capStart = null;
          } else if (capStart.startsWith(INST_FILE_PKEY) ||
                     capStart.startsWith(INST_FILE_CERT) ||
            ...
            String[] iPhoneAttr =
              new String[] { INST_FILE_CATEGORY,    INST_FILE_LOCATION,
                             INST_FILE_URL,         INST_FILE_URI_BASE,
                             INST_FILE_DESCRIPTION, INST_FILE_ICON_FILE };
            for (int a = 0; a < iPhoneAttr.length; a++) {
              if (capStart.startsWith(iPhoneAttr[a])) {
                ht.put(iPhoneAttr[a],
                       lines[i].substring(iPhoneAttr[a].length()).trim());
                capStart = null;
                break;
              }
            }
  ...
  private static final String pdbtczError =
    "You can't add TCZ files to the installer using the palm.pkg file; they must be converted to .pdb using \"java tc.Deploy mytczfile.tcz -palm\"; then reference the pdb in the palm.pkg file.";

  public static String[] joinGlobalWithLocals(Hashtable ht,
                                              Vector more,
                                              boolean allowTCZ)
    throws IllegalArgumentIOException, IOException
  { // guich@tc114_87: changed dontAllowTCZ into allowTCZ
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  public static void preprocessPKG(Vector v, boolean acceptsPath)
    throws IllegalArgumentIOException, IOException
  {
    Vector vextra = new Vector(100);
    for (int i = v.size(); --i >= 0;) {
      String[] pathnames =
        totalcross.sys.Convert.tokenizeString((String)v.items[i], ',');
      ...
      if (pathname.endsWith("/")) { // a folder?
        ...
        if (ff != null) {
          for (int j = 0; j < ff.length; j++) {
            vextra.addElement(
              pathname + ff[j] +
              (pathnames.length > 1 && acceptsPath ? "," + pathnames[1] : ""));
          }
        }
        ...
        if (ff.exists()) {
          for (int idx = 1; true; idx++) {
            final String pathname2 =
              ff.getPath().substring(0, pathname.length() - 4) + "_" + idx +
              "lib.tcz";
  ...
  public static void copyEntry(String s, String targetDir) throws Exception
  {
    ...
    String to = Convert.appendPath(
      targetDir, Convert.appendPath(tpath, Utils.getFileName(s)));
    try {
  ...
  public static File waitForFile(String file) throws Exception
  {
    ...
    if (count <= 0) {
      throw new DeployerException("Error in script: expected file " + file +
                                  " was never written!");
    }
    return in;
  }
  ...
  public static String findPath(String fileName, boolean ignoreFileWithoutSlash)
  {
    ...
    try {
      if ((!ignoreFileWithoutSlash ||
           fileName.replace('/', '\\').indexOf('\\') >= 0) &&
          new File(p = fileName).exists()) {
        return p;
      }
      if (DeploySettings.classPath != null &&
          (p = searchIn(DeploySettings.classPath, fileName)) != null) {
        return p;
      }
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  public static String searchIn(String[] searchPath, String fileName)
  { // guich@tc111_19
    if (searchPath != null) {
      try {
        String p;
        for (int i = 0; i < searchPath.length; i++) {
          ...
          // check if its a jar file. If it is, then use its path to try to find
          // the file
  ...
  public static String getParentFolder(String path)
  {
    if (path != null) {
      for (int i = 0; i < 2;
           i++) { // p:/totalcross/dist/tc.jar -> p:/totalcross
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  public static byte[] loadZipEntry(String zip, String file)
    throws java.io.IOException
  {
    ...
    byte[] bytes = null;
    for (ZipEntry zEntry = zIn.getNextEntry(); zEntry != null && bytes == null;
    ...
    zIn.close();
    if (bytes == null) {
      throw new DeployerException("Error: \"" + file + "\" not found inside " +
                                  zip);
    }
    return bytes;
  }
  ...
  public static byte[] findAndLoadFile(String fileName, boolean showFile)
  {
    ...
    try {
      ...
      bytes = loadFile(path = DeploySettings.currentDir + "/" + fileName,
                       false); // first, in currentDir
      if (bytes == null) {
        bytes = loadFile(path = fileName,
                         false); // second, search in the current path
      }
      if (bytes == null) {
        bytes = loadFile(path = DeploySettings.baseDir + "/" + fileName,
                         false); // third, search in the base path
      }
      if (bytes == null && DeploySettings.mainClassDir != null) {
        bytes = loadFile(path = DeploySettings.mainClassDir + "/" + fileName,
                         false); // fourth, search in the directory where the
                                 // MainWindow is placed
      }
      if (bytes == null && DeploySettings.classPath != null) {
        for (int i = 0; i < DeploySettings.classPath.length && bytes == null;
             i++) {
          ...
          if (path.toLowerCase().indexOf(".jar") < 0) {
            ...
            try {
              ...
              ZipInputStream zIn =
                new ZipInputStream(new java.io.BufferedInputStream(
                  new java.io.FileInputStream(f)));
              for (ZipEntry zEntry = zIn.getNextEntry();
                   zEntry != null && bytes == null;
                   zEntry = zIn.getNextEntry()) {
                String name = zEntry.getName();
                if (name.endsWith("/")) {
                  continue;
            ...
            } // guich@tc115_45: ignore if the file is in the classpath but does
              // not exist
          }
        }
      }
      if (bytes == null && fileName.toLowerCase().endsWith(".png") &&
          DeploySettings.mainPackage != null &&
          DeploySettings.mainPackage.indexOf("samples/") >= 0) {
        bytes =
          loadFile(path = DeploySettings.etcDir + "images/" + fileName, false);
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public static void fillExclusionList()
  {
    ...
    if (DeploySettings.etcDir != null) {
      ...
      if (java.io.File.separatorChar != '/') {
        etcDir =
          etcDir.replace('/', '\\'); // use the platform default separator
      }
    }
    try {
      if (DeploySettings.classPath != null) {
        for (int i = 0; i < DeploySettings.classPath.length; i++) {
          ...
          if (path.endsWith(".jar") &&
              (etcDir == null || !path.startsWith(etcDir))) {
            try {
              java.io.File f = new java.io.File(path);
              ZipInputStream zIn =
                new ZipInputStream(new java.io.FileInputStream(f));
              for (ZipEntry zEntry = zIn.getNextEntry(); zEntry != null;
                   zEntry = zIn.getNextEntry()) {
                String name = zEntry.getName();
                if (name.endsWith("/")) {
                  continue;
  ...
  public static byte[] loadFile(String path, boolean throwEx) throws IOException
  {
    try {
      ...
      try {
        ...
        fis = new File(DeploySettings.pathAddQuotes(path),
                       File.READ_ONLY); // now try with quotes
      }
      int len = fis.getSize();
   ...
   * @param zeroTerminate true to add a zero byte after the end of the string,
   *     false to add nothing
   */
   ...
   * @param pad the character to pad at the end if the string doesn't use all
   *     the allocated space
   */
  public static void writeString(byte[] bytes,
                                 String s,
  ...
  public static void writeStringUnicode(byte[] bytes,
                                        String title,
                                        int ofs,
                                        int size)
  {
  ...
  public static int indexOf(byte[] src, byte[] what, boolean skipStrange)
  { // guich@340_60: added skipStrange
      ...
      if (src[i] == b) { // first letter matches?
        ...
        for (j = 1; j < what.length && found; j++) {
          if (skipStrange && src[i + j + plus] < 0) {
            ...
            found =
              false; // ps: cannot use continue here since were insice 2 loops
          }
        }
        if (found) {
  ...
  public static int uniIndexOf(byte[] src, byte[] what, int i)
  { // guich@340_60: added skipStrange
    if (src == null || src.length == 0 || what == null || what.length == 0) {
      return -1;
    }
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  /**
   * Removes all white spaces from a string and change the first char after
   * every white space to upper case.
   * @param s The string to remove the spaces from.
   * @param upperCaseAfterSpace Flag indicating if the next char after a space
   *     must be
  ...
  public static String palm2epocCrid(String s)
  {
    ...
    return "0x" + totalcross.sys.Convert.unsigned2hex(c[0], 2) +
      totalcross.sys.Convert.unsigned2hex(c[1], 2) +
      totalcross.sys.Convert.unsigned2hex(c[2], 2) +
      totalcross.sys.Convert.unsigned2hex(c[3], 2);
  }
  ...
  public static void writeFile(String fileName,
                               String[] contents,
                               String[] extraContents) throws Exception
  {
  ...
  /**
   * "Agenda" -> "Agenda"    "Agenda 1.0" -> "Agenda"     "--== Agenda ==--" ->
   * "Agenda"
   */
  public static String stripNonLetters(String s)
  {
    String[] parts = totalcross.sys.Convert.tokenizeString(
      s,
      NON_LETTERS); // guich@tc115_84: exclude other chars
    if (parts.length > 1) {
      s = "";
  ...
  public static void copyTCZFile(String targetDir) throws Exception
  {
    for (int i = 0; i < DeploySettings.tczs.length; i++) {
      copyFile(DeploySettings.tczs[i],
               targetDir + "/" + Utils.getFileName(DeploySettings.tczs[i]),
               false);
    }
  }
  ...
  public static String getBaseFolder(String currentDir,
                                     String passed,
                                     String className)
  {
    // currentDir: /home/raphael/Documentos/projetos/totalcross/marte/trunk/n
    // passed    :
    // /home/raphael/Documentos/projetos/totalcross/marte/trunk/n/build/classes/main/Carregar.class
    // className : main/Carregar
    String className2 =
      className.endsWith(".class") ? className : className + ".class";
    if (passed.startsWith(currentDir) && passed.endsWith(className2)) {
      return passed.substring(0, passed.length() - className2.length());
    }
    // normalize the slashes
    currentDir = currentDir.replace('/', DeploySettings.SLASH)
                   .replace('\\', DeploySettings.SLASH);
    if (passed.indexOf(':') >= 0) {
      currentDir = "";
    } else if (!currentDir.endsWith("" + DeploySettings.SLASH)) {
      currentDir += DeploySettings.SLASH;
    }
    passed = passed.replace('/', DeploySettings.SLASH)
               .replace('\\', DeploySettings.SLASH);
    className = className.replace('/', DeploySettings.SLASH)
                  .replace('\\', DeploySettings.SLASH);
    ...
    currentDir = idx > 0
                   ? currentDir.substring(0,
                                          idx - 1)
                   : null; // do not change to >= 0 !   - remove the last slash
    return currentDir;
  }
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  // Calls runtime.exec, returning the output string which can be used to debug
  // a problem.
  public static String exec(String[] command, String path) throws Exception
  {
    Process process =
      Runtime.getRuntime().exec(command, null, new java.io.File(path));
  ...
  public static void jarSigner(String jar, String targetDir) throws Exception
  {
    ...
    // 0D:79:8E:42:A9:CD:50:AC:29:72:85:F8:12:3C:22:0E jarsigner -keystore
    // P:\TotalCross3\etc\security\tcandroidkey.keystore -storepass @ndroid$w
    // -keypass @ndroidsw UIGadgets.apk tcandroidkey
    String jarsignerExe = Utils.searchIn(
      DeploySettings.path, DeploySettings.appendDotExe("jarsigner"));
    if (jarsignerExe == null) {
      throw new DeployerException(
        "Could not find the file " + DeploySettings.appendDotExe("jarsigner") +
    ...
    String keystore = Utils.findPath(
      DeploySettings.etcDir + "security/tcandroidkey.keystore", false);
    if (keystore == null) {
      throw new DeployerException(
        "File security/tcandroidkey.keystore not found!");
    }
    ...
    if (DeploySettings.dJavaVersion >= 1.7) {
      v.addElements(
        new String[] { "-digestalg", "SHA1", "-sigalg", "MD5withRSA" });
    }
    ...
    if (out != null && !out.startsWith("INPUT:jar signed")) {
      throw new DeployerException(
        "An error occured when signing the APK. The output is " + out);
    }
  }
  ...
  public static int version2int(String v)
  {
    int i;
    try {
      if (DeploySettings.appBuildNumber !=
          -1) { // 2.9 -> 209, 2.10 -> 210, 2.123 -> 2123
  ...
  {
    java.util.HashMap<String, String> ret =
      new java.util.HashMap<String, String>(5);
    String[] kv = sp.split("\\|");
