...
public class Utils
{
  ...
  public static String[] joinGlobalWithLocals(Hashtable ht, Vector more, boolean allowTCZ) throws IllegalArgumentIOException, IOException // guich@tc114_87: changed dontAllowTCZ into allowTCZ
  {
    ...
    vLocals  = (Vector)ht.get("[L]"); if (vLocals == null){
      vLocals  = new Vector();
    }
    vGlobals = (Vector)ht.get("[G]"); if (vGlobals== null){
      vGlobals = new Vector();
    }
    if (more != null){
  ...
  public static byte[] findAndLoadFile(String fileName, boolean showFile)
  {
    ...
    try
    {
      ...
      if (bytes == null && fileName.toLowerCase().endsWith(".png") && DeploySettings.mainPackage != null && DeploySettings.mainPackage.indexOf("samples/") >= 0) {
  ...
  public static byte []loadFile(String path, boolean throwEx) throws IOException
  {
    ...
    catch (IOException e) {if (throwEx){
      throw e;
    }else {
      return null;
    }}
