...
public class Utils {
	...
	public static void processInstallFile(String file, Hashtable ht) {
		...
		if (lines != null) {
			for (int i = 0; i < lines.length; i++) {
				if (lines[i].length() > 0) {
					String capStart = lines[i].substring(0,
														 lines[i].indexOf(']') + 1).toUpperCase(); // switch on the first 8 capitalized letters
	...
	throws IllegalArgumentIOException,
		IOException { // guich@tc114_87: changed dontAllowTCZ into allowTCZ
	...
	/////////////////////////////////////////////////////////////////////////////////////
	public static void preprocessPKG(Vector v, boolean acceptsPath) throws IllegalArgumentIOException,
		IOException {
			...
			if (pathname.endsWith("/")) { // a folder?
				...
				if (ff != null) {
					for (int j = 0; j < ff.length; j++) {
						vextra.addElement(pathname + ff[j] + (pathnames.length > 1
															  && acceptsPath ? "," + pathnames[1] : ""));
					}
				}
	...
	public static byte[] loadZipEntry(String zip, String file) throws java.io.IOException {
		...
		for (ZipEntry zEntry = zIn.getNextEntry(); zEntry != null
			 && bytes == null; zEntry = zIn.getNextEntry()) {
		...
		try {
			...
			if (bytes == null && DeploySettings.mainClassDir != null) {
				bytes = loadFile(path = DeploySettings.mainClassDir + "/" + fileName,
								 false); // fourth, search in the directory where the MainWindow is placed
			}
	 ...
	 */
	public static void writeString(byte[] bytes, String s, int pos, boolean zeroTerminate, int len,
								   byte pad) {
		byte[] b = s.getBytes();
	...
	public static int indexOf(byte[] src, byte[] what,
							  boolean skipStrange) { // guich@340_60: added skipStrange
		return indexOf(src, what, skipStrange, 0);
	}

	public static int indexOf(byte[] src, byte[] what, boolean skipStrange,
							  int start) { // guich@340_60: added skipStrange
	...
	public static String palm2epocCrid(String s) {
		...
		return "0x" + totalcross.sys.Convert.unsigned2hex(c[0],
				2) + totalcross.sys.Convert.unsigned2hex(c[1], 2)
	...
	public static String stripNonLetters(String s) {
		String[] parts = totalcross.sys.Convert.tokenizeString(s,
	...
	public static void copyTCZFile(String targetDir) throws Exception {
		for (int i = 0; i < DeploySettings.tczs.length; i++) {
			copyFile(DeploySettings.tczs[i], targetDir + "/" + Utils.getFileName(DeploySettings.tczs[i]),
					 false);
		}
	}
	...
	public static String getBaseFolder(String currentDir, String passed, String className) {
		...
		currentDir = idx > 0 ? currentDir.substring(0,
					 idx - 1) : null; // do not change to >= 0 !   - remove the last slash
		return currentDir;
	}
