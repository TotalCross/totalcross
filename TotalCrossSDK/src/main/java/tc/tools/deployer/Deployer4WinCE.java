...
public class Deployer4WinCE {
	...
	public Deployer4WinCE(boolean allTargets) throws Exception {
		...
		String ceArguments =
			DeploySettings.commandLine.trim(); // the name of the tcz will be the same of the .exe
		...
		if (!keepExe) {
			try {
				new File(Convert.appendPath(targetDir, DeploySettings.filePrefix + ".exe"),
						 File.DONT_OPEN).delete();
	...
	public void createInstallFiles() throws Exception {
		...
		if (infFileGiven) {
			...
			cabName = infFileName.substring(0, Math.min(8,
			...
			String inf = "[Version]\n" +
						 ...
						 + (DeploySettings.companyInfo != null ? Utils.stripNonLetters(DeploySettings.companyInfo) :
							"TotalCross")
		...
		// create the cab
		path2Cabwiz = Utils.findPath(Convert.appendPath(DeploySettings.etcDir, "tools/makecab/Cabwiz.exe"),
									 false);
		if (path2Cabwiz == null) {
		...
		batFile = new File(Convert.appendPath(targetDir, DeploySettings.filePrefix + "_Install.bat"),
						   File.CREATE_EMPTY);
		bat = "@echo off\r\n" + "\r\n"
		...
		iniFile = new File(Convert.appendPath(targetDir, DeploySettings.filePrefix + "_Install.ini"),
						   File.CREATE_EMPTY);
		ini = "[CEAppManager]\r\n" + "Version        = 1.0\r\n" + "Component      = App\r\n" + "[App]\r\n"
