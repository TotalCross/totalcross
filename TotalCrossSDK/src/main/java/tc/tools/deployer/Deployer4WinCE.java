...
public class Deployer4WinCE
{
  ...
  // guich@200 - to make modifications easier, we now read the prc bytes from
  // disk and modify it.
  byte bytes[];

  String sEx =
    "The CEStub.exe file has been modified. some of the placeholder strings could not be found. Reinstall TotalCross. - "; // string Exception - sEx - good name, isnt? ;-)
  boolean allTargets;
  ...
  public Deployer4WinCE(boolean allTargets) throws Exception
  {
    ...
    String ceArguments =
      DeploySettings.commandLine
        .trim(); // the name of the tcz will be the same of the .exe
    if (ceArguments.length() > DeploySettings.defaultArgument.length / 2) {
      throw new IllegalArgumentException("ERROR - launch string too long: " +
                                         ceArguments);
    }
    ...
    if (DeploySettings.mainClassName != null) {
      try {
        String fileName = Convert.appendPath(DeploySettings.etcDir,
                                             "launchers/wince/Launcher.exe");
        bytes = Utils.findAndLoadFile(fileName, false);
        if (bytes == null) {
          throw new DeployerException("Could not find " + fileName +
                                      ". Please add tc.jar to the classpath!");
        }
        findOffsets(DeploySettings.bitmaps);
        storeInformation(ceArguments, versionOffsetARM);
        DeploySettings.bitmaps.saveWinCEIcons(bytes,
                                              bitmap16x16x8_Offset,
                                              bitmap32x32x8_Offset,
    ...
    if (!keepExe) {
      try {
        new File(
          Convert.appendPath(targetDir, DeploySettings.filePrefix + ".exe"),
          File.DONT_OPEN)
          .delete();
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  private void saveStub(String userPath, String exeName, boolean show)
    throws Exception
  {
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  byte[] icon48x48x8_header = {
    (byte)0x28, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x30, (byte)0x00,
    (byte)0x00, (byte)0x00, (byte)0x60, (byte)0x00, (byte)0x00, (byte)0x00,
    (byte)0x01, (byte)0x00, (byte)0x08, (byte)0x00
  };
  byte[] icon32x32x8_header = {
    (byte)0x28, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x20, (byte)0x00,
    (byte)0x00, (byte)0x00, (byte)0x40, (byte)0x00, (byte)0x00, (byte)0x00,
    (byte)0x01, (byte)0x00, (byte)0x08, (byte)0x00
  };
  byte[] icon16x16x8_header = {
  ...
  private void deleteCabs() throws totalcross.io.IOException
  {
    try {
      new File(getCabName())
        .delete(); // guich@341_14 replaced '/' by java.io.File.separatorChar
    } catch (FileNotFoundException e) {
    }
  }
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  private String toString(Vector v, String post, boolean copyFile)
    throws Exception
  {
    String s = "\n";
    int n = v.size();
  ...
  public void createInstallFiles() throws Exception
  {
    ...
    if (infFileGiven) {
      ...
      cabName = infFileName.substring(
        0,
        Math.min(8,
                 infFileName.length() - 4)); // strip the .inf - guich@421_75
      deleteCabs();
      File oFile =
        new File(Convert.appendPath(targetDir, infFileName), File.CREATE_EMPTY);
      ...
      if (DeploySettings.packageVM) { // include the vm?
        if (!hasExe) {
          System.out.println(
            "Warning: /p ignored since package has no binary files");
        } else {
          ...
          lbFolder =
            Convert.appendPath(DeploySettings.folderTotalCross3DistVM, "wince");
          tcFolder =
            Convert.appendPath(DeploySettings.folderTotalCross3DistVM, "wince");
          // copy binary files
          try {
          ...
          File.copy(Convert.appendPath(tcFolder, name),
                    Convert.appendPath(targetDir, name));
          if (lbFolder != null) {
            name = "/Litebase.dll";
            File.copy(Convert.appendPath(lbFolder, name),
                      Convert.appendPath(targetDir, name));
          }
        }
      }
      ...
      String installDir =
        "\\TotalCross\\" +
        DeploySettings.filePrefix; // guich@568_7: removed extra \"
      String inf =
        "[Version]\n"
        +
        ...
        + "Provider           = \"" +
        (DeploySettings.companyInfo != null
           ? Utils.stripNonLetters(DeploySettings.companyInfo)
           : "TotalCross") +
        "\"\n"
        ...
        "[DefaultInstall]\n"
        + "CopyFiles   = " + (hasExe ? "Binaries," : "") +
        "GlobalFiles,LocalFiles\n"
        ...
        // local libraries (and other files) goes to the default instalation
        // directory
    ...
    // create the cab
    path2Cabwiz = Utils.findPath(
      Convert.appendPath(DeploySettings.etcDir, "tools/makecab/Cabwiz.exe"),
      false);
    if (path2Cabwiz == null) {
    ...
    // since exec don't allow us to change the current path, we create a batch
    // file that will cd to the current folder
    try {
    ...
    String[] callCabWiz =
      allTargets
        ? new String[] { path2Cabwiz.replace('/', DeploySettings.SLASH),
                         infFileName } // old wince devices
        : new String[] { path2Cabwiz.replace('/', DeploySettings.SLASH),
                         infFileName,
                         "/compress" };
    out = Utils.exec(callCabWiz, targetDir.replace('/', DeploySettings.SLASH));

    // now we need to wait the process finish. For some reason, the
    ...
    // the cab files were created, now we need to create the bat file to start
    // the instalation
    batFile = new File(
      Convert.appendPath(targetDir, DeploySettings.filePrefix + "_Install.bat"),
      File.CREATE_EMPTY);
    bat =
      "@echo off\r\n"
      + "\r\n"
      ...
      + ":ok\r\n"
      + "\"%ProgramFiles%\\Microsoft ActiveSync\\CeAppMgr.exe\" \".\\" +
    ...
    iniFile = new File(
      Convert.appendPath(targetDir, DeploySettings.filePrefix + "_Install.ini"),
      File.CREATE_EMPTY);
    ini = "[CEAppManager]\r\n"
          + "Version        = 1.0\r\n"
          + "Component      = App\r\n"
