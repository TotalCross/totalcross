...
Global constant pool:
   ...
   htMtdRef - each entry is stored as uint16*  - references for methods:
                                                 class name index (=), method
name index (pushed into htMtd), index for each parameter (taken from htCls or
htConst - if class or primitive type, respectively). htSF - each entry is stored
as uint32       - references for static fields, stored as (class << 16) | field:
                                                 class name index (pushed into
htCls) and field name index (pushed into htMtd) htIF - each entry is stored as
uint32       - references for instance fields, stored as (class << 16) | field:
                                                 class name index (pushed into
htCls) and field name index (pushed into htMtd)

Important: position 0 of the Contant Pool must not be used. Symbols must start
from index 1.

 */

public class GlobalConstantPool
  implements tc.tools.converter.tclass.TClassConstants
{
  ...
  private static Hashtable htMtdFld =
    new Hashtable(2000); // method and field names
  ...
  private static Hashtable htCls =
    new Hashtable(2000); // class and primitive references/names
  ...
  private static int idxI32, idxI64, idxDbl, idxStr, idxMtdFld, idxMtd, idxSF,
    idxIF, idxCls, svdCount, svlCount;
  ...
  static String javaPrimitiveType2TCType(String value)
  {
    switch (value.charAt(0)) {
      ...
      case 'J':
        return "&L";
      case 'F':
        return "&D"; // guich@tc100b3_2: changed F to D. without this change,
                     // "float f = 0.1f; String s = "test"+f;" results in
                     // calling "StringBuffer.append(F)", which does not exist
                     // on device, since all floats are converted to double
      case 'D':
        return "&D";
    }
    return null;
  ...
  {
    value = value.replace(
      '/', '.'); // java use java/lang/String, we use java.lang.String
  ...
  // for a method: putMtd("java.lang.String","substring",{"I","I"},
  // "substring(II)") method's parameters must be in Java's format (L...; for
  // class)
  public static int putMethod(String className,
                              String name,
                              String[] params,
                              String signature)
  { // puts an reference for a method
  ...
  public static int putStaticField(String className, String name)
  { // puts a reference for a static field
    return putField(htSF, vSF, POOL_SF, className, name);
  }

  public static int putInstanceField(String className, String name)
  { // puts a reference for a instance field
    return putField(htIF, vIF, POOL_IF, className, name);
  }

  private static int putField(Hashtable ht,
                              Vector v,
                              int type,
                              String className,
                              String name)
  { // puts a reference for a field
    ...
    int f = putMethodOrFieldName(
      name); // field name was already obfuscated during field convertion
             // (which comes before method convertion)
    i.asInt = (c << 16) | f;
    return i.index;
  }

  public static int putClsOrParam(String value)
  { // value is sure a Java type, but it may be an
    // array or a class without the Lxxx;
    return value.charAt(0) == '[' ? putParam(value) : putCls(value);
  }

  public static int putCls(String value)
  { // pure class name: value must be a single class, no
    // arrays, no Lxxx; !
    value = value.replace('/', '.');
    int index = value.indexOf("4D");
    if (index >= 0) {
    ...
    if (((value.charAt(0) == 'L' && value.charAt(value.length() - 1) == ';') ||
         value.charAt(0) == '[')) {
  ...
  public static int putParam(String value)
  { // part of a method signature - value = (Lxxx; or [zzz)
  ...
  public static int putMethodOrFieldName(String value)
  { // puts the name of a field or method in
  ...
  public static int putStr(String value)
  { // mainly Strings
  ...
  private static int put(String value, int type, Hashtable ht, Vector v)
  {
    if (v == vMtdFld && value.length() > 255) {
      throw new ConverterException(
        "Identifier '" + value +
        "' too long. Size is limited to 255 characters, but it has " +
        value.length());
    }
    temp.set(value, type);
  ...
  public static int getClassIndex(String ident)
  {
    try {
      ...
      if (ident.startsWith("totalcross.lang.")) {
        ident = totalcross.sys.Convert.replace(
          ident, "totalcross.lang.", "java.lang.");
      }
  ...
  public static void write(DataStreamLE ds) throws totalcross.io.IOException
  {
    ...
    // note that the first element of each array is always zero, so it isnt
    // written
    ...
    for (i = 1; i < mtdCount; i++) {
      ...
      int[] s = (int[])v.asObj;
      size += ds.writeByte(
        s.length - 2); // min length is 2, subtracting 2 improves compression
      partSize += s.length * 2;
    }
    if (mtdCount > 1) {
    ...
    for (i = 1; i < clsCount; i++) {
      ...
      // convert sql classes
      String s = v.asStr.charAt(0) == '&'
                   ? v.asStr.substring(1)
                   : v.asStr; // remove the & from the primitives
      partSize += 1 + s.length();
    }
    ds.writeInt(partSize);
    for (i = 1; i < clsCount; i++) {
    ...
    // Strings
    for (i = 1; i < strCount;
         i++) { // String constants must be the last written
    ...
    if (J2TC.dump) {
      System.out.println("\nConstant pool size: " + size + " (int=" + si +
                         ",long=" + sl + ",double=" + sd +
                         ",static field=" + ssf + ",instance field=" + sif +
                         ",method ref=" + sm + ",method/field names=" + smf +
                         ",class names=" + sc + ",string=" + ss + ")\n");
    }
    if (clsCount > 1000 || mtdCount > 1000 || SFCount > 1000 ||
        IFCount > 1000) {
      System.out.println(
        "Total References - Classes: " + clsCount + ", Methods: " + mtdCount +
        ", instance fields: " + IFCount + " (each one limited to 4095)");
    }
  }

  public static boolean isEmpty()
  { // guich@tc111_2: returns if it contains at least one user class
