...
public class GlobalConstantPool implements tc.tools.converter.tclass.TClassConstants {
	...
	private static int idxI32, idxI64, idxDbl, idxStr, idxMtdFld, idxMtd, idxSF, idxIF, idxCls,
			svdCount, svlCount;
	...
	public static int putMethod(String className, String name, String[] params,
								String signature) { // puts an reference for a method
	...
	public static int putStaticField(String className,
									 String name) { // puts a reference for a static field
		return putField(htSF, vSF, POOL_SF, className, name);
	}

	public static int putInstanceField(String className,
									   String name) { // puts a reference for a instance field
		return putField(htIF, vIF, POOL_IF, className, name);
	}

	private static int putField(Hashtable ht, Vector v, int type, String className,
								String name) { // puts a reference for a field
		...
		int f = putMethodOrFieldName(
					name); // field name was already obfuscated during field convertion (which comes before method convertion)
		i.asInt = (c << 16) | f;
		return i.index;
	}

	public static int putClsOrParam(String
									value) { // value is sure a Java type, but it may be an array or a class without the Lxxx;
		return value.charAt(0) == '[' ? putParam(value) : putCls(value);
	}

	public static int putCls(String
							 value) { // pure class name: value must be a single class, no arrays, no Lxxx; !
		value = value.replace('/', '.');
		int index = value.indexOf("4D");
		if (index >= 0) {
			value = value.substring(0, index) + value.substring(index + 2);
	...
	public static int putMethodOrFieldName(String
										   value) { // puts the name of a field or method in the identifiers array
	...
	private static int put(String value, int type, Hashtable ht, Vector v) {
		if (v == vMtdFld && value.length() > 255) {
			throw new ConverterException(
				"Identifier '" + value + "' too long. Size is limited to 255 characters, but it has " +
				value.length());
		}
		temp.set(value, type);
	...
	public static void write(DataStreamLE ds) throws totalcross.io.IOException {
		...
		for (i = 1; i < SFCount; i++) { // class
			...
			if (dump) {
				System.out.println("SF[" + v.index + "] : " + ((v.asInt >> 16) & 0xFFFF) + " " +
								   (v.asInt & 0xFFFF));
			}
		}
		ssf = size - ssf;
		...
		for (i = 1; i < IFCount; i++) { // class
			...
			size += ds.writeShort(now);
			if (dump) {
				System.out.println("IF[" + v.index + "] : " + ((v.asInt >> 16) & 0xFFFF) + " " +
								   (v.asInt & 0xFFFF));
			}
		}
		sif = size - sif;
		...
		for (i = 1; i < clsCount; i++) {
			...
			// convert sql classes
			String s = v.asStr.charAt(0) == '&' ? v.asStr.substring(1) :
		...
		if (J2TC.dump) {
			System.out.println("\nConstant pool size: " + size + " (int=" + si + ",long=" + sl + ",double=" + sd
							   + ",static field=" + ssf + ",instance field=" + sif + ",method ref=" + sm + ",method/field names=" +
							   smf
							   + ",class names=" + sc + ",string=" + ss + ")\n");
		}
		if (clsCount > 1000 || mtdCount > 1000 || SFCount > 1000 || IFCount > 1000) {
			System.out.println("Total References - Classes: " + clsCount + ", Methods: " + mtdCount +
							   ", instance fields: "
