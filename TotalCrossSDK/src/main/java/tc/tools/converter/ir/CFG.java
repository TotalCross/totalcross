...
final public class CFG implements TCConstants
{
  ...
  public CFG(TCMethod mt)
  {
    ...
    if (methodCode != null) {
      ...
      for (int i = 0, ind = 0; i < len; i++) {
        ...
        switch (op) {
          case SWITCH: {
            ...
            BasicBlock bb =
              new BasicBlock("SWITCH", this, first, tc, null, null);
          ...
          case JGE_regD_regD: {
            ...
            BasicBlock bb =
              new BasicBlock("NORMAL", this, first, tc, instLeft, instRigth);
            first = instRigth;
            v.addElement(bb);
            break;
          }
          ...
          case JUMP_s24: {
            ...
            Instruction instRigth = getInstruction(methodCode, ind + 1);
            BasicBlock bb =
              new BasicBlock("NORMAL", this, first, tc, instLeft, null);
            first = instRigth;
            v.addElement(bb);
            break;
          }
          ...
          case JUMP_regI: {
            BasicBlock bb =
              new BasicBlock("NORMAL", this, first, tc, null, null);
      ...
      for (int i = 0; i < len; i++) {
        ...
        if (bb.last.opcode == JUMP_regI) { // this basic block ends with the
                                           // JUMP_regI instruction ?
  ...
  private static void DFS(int sorted[],
                          boolean mark[],
                          BasicBlock bb,
                          TCValue nNodes)
  {
  ...
  public static void handlesExceptions(TCMethod m)
  {
    if (m.exceptionHandlers != null) {
      for (int i = 0; i < m.exceptionHandlers.length; i++) {
        ...
        BasicBlock bbCatch =
          getBasicBlockOfInstruction(inst, m.cfg.blocks, m.insts);
  ...
  public BasicBlock getBasicBlock(Instruction code,
                                  Vector blocks,
                                  Vector methodCode)
  {
    if (code != null) {
  ...
  public static BasicBlock getBasicBlock(Instruction code,
                                         BasicBlock[] blocks,
                                         Vector methodCode)
  {
    if (code != null) {
  ...
  public static BasicBlock getBasicBlockOfInstruction(Instruction code,
                                                      BasicBlock[] blocks,
                                                      Vector methodCode)
  {
    if (code != null) {
  ...
  public BasicBlock newBasicBlock(Vector blocks,
                                  Vector methodCode,
                                  Instruction code)
  {
    Instruction tc = null;
  ...
  public static void printAllMethods(TCMethod[] methods)
  {
    int len = methods.length;
    for (int i = 0; i < len; i++) {
      System.out.println("\nMethod: " + GlobalConstantPool.getMethodFieldName(
                                          methods[i].cpName));
      methods[i].cfg.print();
    }
  }

  public BasicBlock DFSOrderNode(int i) { return blocks[order[i]]; }
  ...
  public int iCount() { return method.iCount; }

  public int oCount() { return method.oCount; }

  public int v64Count() { return method.v64Count; }
  ...
  public void computeInsOuts()
  {
    ...
    while (change) {
      ...
      for (int i = 0; i < order.length; i++) {
        ...
        bb.inD = useD.on(outD);
        bb.inO = useO.on(outO);
        if (!bb.inI.equals(oldinI) || !bb.inD.equals(oldinD) ||
            !bb.inO.equals(oldinO)) {
          change = true;
        }
      }
    }
