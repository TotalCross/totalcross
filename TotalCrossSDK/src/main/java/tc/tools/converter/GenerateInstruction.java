...
public class GenerateInstruction implements TCConstants {
	...
	public static final Instruction newInstruction(Vector vcode, int prefix, Operand opr, int line) {
		...
		switch (opcode) {
			...
			case RETURN_reg64: {
				c = new Reg(opcode, line);
				OperandReg r = OperandKind.isTypeL(opr.kind) ? (OperandReg) promoteOperand(vcode, opr, opr_regL,
							   line)
	...
	public static final Instruction newInstruction(Vector vcode, int prefix, Operand opr1, Operand opr2,
			int line) {
		...
		switch (opcode) {
			...
			case MOV_reg64_reg64: {
				c = new Reg_reg(opcode, line);
				OperandReg op = OperandKind.isTypeL(opr2.kind) ? (OperandReg) promoteOperand(vcode, opr2, opr_regL,
								line)
	...
	public static final Instruction newInstruction(Vector vcode, OperandReg regO, OperandSym sym,
			Operand dims[],
			int line) {
	...
	public static final Instruction newInstruction(Vector vcode, int opcode, OperandSym sym,
			OperandReg _this,
	...
	public static final void newInstruction(Vector vcode, int defAddr, Operand k, BC171_lookupswitch ji,
											int line) {
	...
	public static final void newInstruction(Vector vcode, int defAddr, Operand index,
											BC170_tableswitch ji, int line) {
		Switch_reg sswitch = new Switch_reg(SWITCH, line);
	...
	public static void GEN_CALL(Vector vcode, Instruction c, OperandSym sym, OperandReg _this,
	...
	public static void GEN_SWITCH(Vector vcode, Switch_reg sswitch, int defAddr, BC171_lookupswitch ji,
								  int line) {
	...
	public static void GEN_SWITCH(Vector vcode, Switch_reg sswitch, int defAddr, BC170_tableswitch ji,
								  int line) {
		// add the default address
		Two16 tc1 = new Two16(line);
		tc1.v1 = Bytecode2TCCode.setGotoIndex(defAddr);
