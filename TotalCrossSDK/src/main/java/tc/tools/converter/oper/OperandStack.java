...
public class OperandStack implements TCConstants
{
  ...
  public OperandStack(int length) { elems = new Operand[length]; }

  public void push(Operand op) { elems[++sp] = op; }
  ...
  public Operand top() { return elems[sp]; }

  public int count() { return (sp + 1); }
  ...
  {
    for (int i = 0; i <= sp; i++) {
      if (elems[i] instanceof OperandReg &&
          (elems[i].kind == kind ||
           elems[i].kind == opr_regD && kind == opr_regL)) {
