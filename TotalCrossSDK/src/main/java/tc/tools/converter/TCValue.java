...
public class TCValue
  implements JConstants, tc.tools.converter.tclass.TClassConstants
{
  public int type;
  ...
  {
    //      return
    //      typeAsString[type]+"(I:"+asInt+",L:"+asLong+",D:"+asDouble+",O:\""+(asObj==null?"":asObj)+"\"";
    if (index >= 0) {
      switch (type) {
        case POOL_I32:
  ...
  public TCValue(int val) { set(val); }
  ...
  public TCValue(long val) { set(val); }
  ...
  public TCValue(double val) { set(val); }
  ...
  public TCValue(String val, int type) { set(val, type); }
