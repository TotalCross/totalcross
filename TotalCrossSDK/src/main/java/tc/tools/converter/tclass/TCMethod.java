...
public final class TCMethod implements TCConstants {
	...
	paramRegs; // the RegType to where the parameters must go on - two bits would be enough, but we use more to improve speed
	...
	public void write(DataStreamLE ds) throws totalcross.io.IOException {
		...
		if (tc.tools.converter.J2TC.dump) {
			...
			System.err.println("Warning: " + tcclass.className + "." + GlobalConstantPool.getMethodFieldName(
								   cpName)
		...
		if (lineNumberDebugInfoCount > 0 && !name.equals("<S>")) {
			...
			for (i = 1; i < lineNumberDebugInfoCount; i++) {
				...
				if (p >= ALL_1 || l >= ALL_1) { // 255 is reserved for mark telling that diff is always 1
					System.out.print("Cannot write line numbers for " + tcclass.className + "." + name + " with " +
									 paramCount
									 + " parameters (" + p + "/" + l + "). ");
					if (l >= ALL_1) {
						System.out.println(
							"Move statement at line " + lineNumbers[i].lineNumber + " to above line " +
		...
		if (opcodeCount > 0) {
			Storage.writeIntArray(ds,
								  opcode); // note that the bit fields are inverted when comparing to Java, so we store them in inverse order
	...
	public void printCode() {
		System.out.print("\n" + tcclass.className + "." + GlobalConstantPool.getMethodFieldName(
							 cpName) + "(");
