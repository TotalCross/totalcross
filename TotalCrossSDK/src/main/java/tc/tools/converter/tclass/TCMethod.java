...
public final class TCMethod implements TCConstants
{
  ...
  public int[] /*Uint16Array*/ cpParams; // indexes to the constant pool of the
                                         // parameters
  ...
  // The method index on VM (Virtual Machine). Used by the compiler in the
  ...
  public void write(DataStreamLE ds) throws totalcross.io.IOException
  {
    ...
    if (tc.tools.converter.J2TC.dump) {
      ...
      for (i = 0; i < paramCount; i++) {
        System.out.print(GlobalConstantPool.getClassName(cpParams[i]) +
                         (i < paramCount - 1 ? "," : ""));
      }
      System.out.println(
        ") i=" + iCount + ",o=" + oCount + ",v64=" + v64Count + " - " +
        (lineNumbers == null ? "lines=0" : ("lines=" + lineNumbers.length)));
    } else if (iCount >= 64 || oCount >= 64 || v64Count >= 64) {
      System.err.println("Warning: " + tcclass.className + "." +
                         GlobalConstantPool.getMethodFieldName(cpName) +
                         " has reached 64 registers. i=" + iCount +
                         ",o=" + oCount + ",v64=" + v64Count);
    }

    int[] opcode = getCode();
    int opcodeCount = code != null ? code.length : 0;
    int exceptionHandlersCount =
      exceptionHandlers != null ? exceptionHandlers.length : 0;
    ...
    if (lineNumberDebugInfoCount > 0 && !name.equals("<S>")) {
      ...
      for (i = 1; i < lineNumberDebugInfoCount; i++) {
        ...
        if (p >= ALL_1 || l >= ALL_1) { // 255 is reserved for mark telling that
                                        // diff is always 1
          System.out.print("Cannot write line numbers for " +
                           tcclass.className + "." + name + " with " +
                           paramCount + " parameters (" + p + "/" + l + "). ");
          if (l >= ALL_1) {
            System.out.println("Move statement at line " +
                               lineNumbers[i].lineNumber + " to above line " +
                               lineNumbers[0].lineNumber);
          } else {
            System.out.println();
          }
          // for (i=0; i < lineNumberDebugInfoCount; i++) System.out.println(i+"
          // "+lineNumbers[i].startPC+" "+lineNumbers[i].lineNumber);
    ...
    if (opcodeCount > 0) {
      Storage.writeIntArray(
        ds,
        opcode); // note that the bit fields are inverted when comparing to
                 // Java, so we store them in inverse order
    ...
    /*System.out.println(tcclass.className+"."+name);
      ...
      for (i=0; i < lineNumberDebugInfoCount; i++)
      System.out.println(lineNumbers[i].startPC); System.out.println("line:");
      for (i=0; i < lineNumberDebugInfoCount; i++)
      System.out.println(lineNumbers[i].lineNumber);*/
    ...
    if (lineNumberDebugInfoCount == 1) {
      ...
      if (allLN1) {
        ...
        for (i = 1; i < lineNumberDebugInfoCount; i++) {
          ...
          ds.writeByte(lineNumbers[i].lineNumber -
                       lineNumbers[i - 1].lineNumber);
        }
      }
    }
  }
  ...
  private int[] getCode()
  {
    ...
    if (tc.tools.converter.J2TC.dump) {
      ...
      for (int j = 0, i = 0; i < code.length; j++, i++) {
        System.out.println(Convert.zeroPad(Convert.toString(j), 3) + " " +
                           code[i].toString());
      }
    }
  ...
  public void modifyCode(AdjListNode[] adjListI,
                         AdjListNode[] adjListD,
                         AdjListNode[] adjListO)
  {
    if (insts != null) {
  ...
  private static String beAware =
    " Be aware that the classes and methods you use from the java.lang package at desktop are automatically transformed by tc.Deploy into the classes and methods available in totalcross.lang. Thus, you must use only the classes and methods described in the javadocs.";
  private static totalcross.util.Hashtable htAlreadyChecked =
    new totalcross.util.Hashtable(40);

  // checks if this is a java class that exists at device
  private void checkIfAllowed(int[] params)
  { // guich@tc100b5_26
    ...
    // System.out.println(J2TC.currentClass+" "+J2TC.currentMethod+" checking
    // "+className+" "+method);
    if (J2TC.inProhibitedList(className, false) &&
        !htAlreadyChecked.exists(params)) {
      ...
      try {
        ...
        if (index < 0) {
          ...
          c4D = Class.forName(tcClassName.substring(0, index) + "4D" +
                              tcClassName.substring(index));
        }
      ...
      boolean found = false;
      if (method.equals(
            TClassConstants.CONSTRUCTOR_NAME)) { // is this a constructor?
        java.lang.reflect.Constructor<?>[] consts =
          c4D.getDeclaredConstructors();
        ...
        for (int i = 0; i < methods.length; i++) {
          ...
          if (name.equals(method) &&
              areParametersCompatible(params, methods[i].getParameterTypes())) {
            found = true;
            break;
          }
        }
      ...
      if (!found) {
        throw new InvalidClassException(
          toHumanReadable(className, method, params) +
          " is not available at the device! To find the available ones, see the Javadocs for " +
          java2totalcross(className) + " class." +
          (className.startsWith("java.lang") ? beAware : ""));
      }
    }
  }
  ...
  {
    return name.startsWith("java.") ? name.replace("java.", "totalcross.")
                                    : name;
  }
  ...
  // jiargs is what the user is calling; args is from the method retrieved with
  // introspection in the totalcross/xxx4D class
    ...
    for (int i = 0; i < jiargsLen && found; i++) {
      ...
      if (js.charAt(0) == '&') {
        ...
        found &= name.equals(js) || name.equals(js + "4D") ||
                 Convert.replace(name, "4D", "").equals(js);
      }
    }
    return found;
  }
