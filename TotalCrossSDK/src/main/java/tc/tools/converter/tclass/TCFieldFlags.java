...
public final class TCFieldFlags
{ // uint16 structure
  // Java access flags
  ...
  {
    int v = ((isArray ? 1 : 0) << 0) | (type << 1) | ((isPublic ? 1 : 0) << 5) |
            ((isPrivate ? 1 : 0) << 6) | ((isProtected ? 1 : 0) << 7) |
            ((isStatic ? 1 : 0) << 8) | ((isVolatile ? 1 : 0) << 9) |
  ...
  {
    return (isPublic ? "public " : "") + (isPrivate ? "private " : "") +
      (isProtected ? "protected " : "") + (isStatic ? "static " : "") +
      (isVolatile ? "volatile " : "") + (isTransient ? "transient " : "") +
      (isFinal ? "final " : "");
  }
}
