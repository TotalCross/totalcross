...
public final class TCClassFlags {
	public int
	bits2shift;// 2; // the number of bits to shift to compute the array's size for the object/primitive type that this class represents.
	...
	public void write(DataStreamLE ds) throws totalcross.io.IOException {
		int v = bits2shift | ((isArray ? 1 : 0) << 2) | ((isObjectArray ? 1 : 0) << 3) | ((
					isString ? 1 : 0) << 4)
				| ((isPublic ? 1 : 0) << 5) | ((isStatic ? 1 : 0) << 6) | ((isFinal ? 1 : 0) << 7) | ((
							isAbstract ? 1 : 0) << 8)
				| ((isSynthetic ? 1 : 0) << 9) | ((isInterface ? 1 : 0) << 10) | ((isStrict ? 1 : 0) << 11) |
				0; // this is set by the vm
		ds.writeShort(v);
	}
}
