...
public final class JavaCode implements JConstants
{
  ...
  public JavaCode(JavaMethod method, DataStream ds, JavaConstantPool cp)
    throws totalcross.io.IOException
  {
    ...
    if (n > 0) {
      ...
      ByteCode[] tempBcs =
        new ByteCode[n]; // we don't know how many bytecodes will exist, but
                         // surely not more than the number of bytes in this
                         // method
      ...
      for (; ByteCode.pc < n; i++) {
        ...
        if (bcode >= bcClassNames.length) {
          System.out.println("Error in " + method.classOfMethod + "." +
                             method.signature +
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (name.equals("LineNumberTable")) {
        ...
        if (dump) {
          System.out.println("Skipping attribute " + name + " in method " +
                             method.signature);
        }
        int len = ds.readInt();
    ...
    if (!lineNumberWarned && lineNumberLine == null &&
        !className.startsWith("totalcross.") &&
  ...
  {
    sb.setLength(0);
    sb.append(method.classOfMethod)
      .append('.')
