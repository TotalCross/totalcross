...
public final class JavaMethod
{
  ...
  public boolean isPublic, isPrivate, isProtected, isStatic, isFinal,
    isVolatile, isTransient, isNative, isAbstract, isStrict, isSynchronized;
  // method signature
  ...
  public static String[] splitParams(String s, StringBuffer ret)
  { // return the parameters array and the
    // return type in the string buffer
    Vector v = vec;
    v.removeAllElements();
    ret.setLength(0);
    int n = s.length();
  ...
  public JavaMethod(JavaClass jc, DataStream ds, JavaConstantPool cp)
    throws totalcross.io.IOException
  {
    ...
    signature = name + parameters.substring(
                         0, parameters.length() - ret.length()); // (I)V -> (I)
    ...
    if (params != null) {
      for (int i = 0; i < params.length - 1; i++) {
        if (params[i].equals("F")) {
          ...
          System.out.println("Caution! Method " + jc.className + "." + name +
  ...
  public JavaMethod(JavaClass jc, MethodNode methodNode)
  {
    ...
    signature =
      methodNode.name + Arrays.toString(Type.getArgumentTypes(methodNode.desc))
                          .replace(", ", "")
                          .replaceFirst("^\\[(.*)\\]$", "($1)");

    if (methodNode.invisibleAnnotations != null &&
        methodNode.invisibleAnnotations.size() > 0) {
      for (AnnotationNode annotation :
           (List<AnnotationNode>)methodNode.invisibleAnnotations) {
        if (Type.getDescriptor(ReplacedByNativeOnDeploy.class)
