...
public final class JavaClass
{
  ...
  public JavaClass(byte[] bytes, boolean onlyHeader)
    throws totalcross.io.IOException
  {
    ...
    if (!onlyHeader &&
        majorVersion >
          52) { // check if we're using the correct class file format
  ...
  public JavaClass parse(byte[] bytes, boolean onlyHeader)
    throws totalcross.io.IOException
  {
    int i, n;
    this.bytes = bytes;
  ...
  public byte[] JavaClassToBytes()
  {
    ...
    try {
      stream.writeInt(
        0xCAFEBABE); // write magic number that identifies it as a java class
