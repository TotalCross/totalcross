...
public class ChooseOpcode implements TCConstants {
	...
	private static int shift(int prefix, int target, int src1, int src2) {
		switch (target) {
			case opr_regI: {
				if (src2 == opr_s6I || src2 == opr_s12I) {
					return (prefix == pref_SHL ? SHL_regI_regI_s12 : prefix == pref_SHR ? SHR_regI_regI_s12 :
							USHR_regI_regI_s12);
				}

				return (prefix == pref_SHL ? SHL_regI_regI_regI : prefix == pref_SHR ? SHR_regI_regI_regI :
						USHR_regI_regI_regI);
			}
			case opr_regL: {
				return (prefix == pref_SHL ? SHL_regL_regL_regL : prefix == pref_SHR ? SHR_regL_regL_regL :
						USHR_regL_regL_regL);
			}
			default:
				;
