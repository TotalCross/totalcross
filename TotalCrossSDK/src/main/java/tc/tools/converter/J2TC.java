...
{
  public static Hashtable htAddedClasses = new Hashtable(
    0xFF); // will also be used to check if there are files ending with 4D
  public static Hashtable htExcludedClasses = new Hashtable(
    0xFF); // will also be used to check if there are files ending with 4D
  ...
  private static Vector vFinalVar =
    new Vector(4); // initialization of static final fields
  ...
  {
    jc.className =
      Bytecode2TCCode.replaceTotalCrossLangToJavaLang(jc.className);
    //  if xxx is the current class, compile it if and only if not exist a class
    //  xxx4D
    boolean has4D = htAddedClasses.exists(jc.className + "4D.class");
    if (!has4D || jc.className.equals("totalcross/util/Vector") ||
        jc.className.equals("totalcross/util/Hashtable")) {
      if (isInnerClassOfNon4DClass(jc.className) &&
          !jc.className.equals("totalcross/util/Hashtable$Entry")) {
        return;
      }
      ...
      // if (dump) {System.out.println(jc.className); byte[] bytes =
      // tcbas.toByteArray();
      // System.out.println(TCZ.toString(bytes,0,bytes.length));}
      tc.tools.converter.Storage.compressAndWrite(tcbas,
                                                  new DataStream(tcbasz));
  ...
  private static boolean isInnerClassOfNon4DClass(String className)
  { // guich@tc100b5_27
    // check if there are inner classes of the class that has 4D and "remove"
    // them
    ...
    int dollar;
    if (!is4D &&
        (dollar = className.indexOf('$')) >= 0) { // is this not a 4D class?
  ...
  // This constructor was created solely to perform the test cases of conversion
  // Bytecode2TCCode. After that, it could be removed.
      ...
      switch (bcs[j].bc) {
        case 18: // ldc
          if (DeploySettings.appTitle == null &&
              j < 7) { // guich@tc100br_14: restrict it to the very first
                       // begining of the constructor
            ...
            if (bcs[j + 1].bc == 183) { // invoke special?
              ...
              if (className.equals("java/lang/StringBuffer") &&
                  signature.equals("<init>(Ljava/lang/String;)")) {
                BC018_ldc ldc = (BC018_ldc)bcs[j];
                DeploySettings.appTitle = Utils.stripNonLetters(
                  (String)ldc.val.asObj); // if the user wrote "Agenda 1.0",
                                          // break at the "Agenda"
                continue;
              }
            }
            // else - commented out for classes that don't extend MainWindow
            // directly super("Agenda",TAB_ONLY_BORDER)
            //    0    0:aload_0
            //    1    1:ldc1            #1   <String "Agenda">
            //    2    3:iconst_4
            //    3    4:invokespecial   #13  <Method void MainWindow(String,
            //    byte)>
            if ((j + 2) < bcs.length &&
                (bcs[j + 1].bc == 183 || bcs[j + 2].bc == 183)) {
              BC183_invokespecial invoke =
                (BC183_invokespecial)bcs[bcs[j + 2].bc == 183 ? j + 2 : j + 1];
              String className = invoke.className;
              String signature = invoke.signature;
              if ((className.equals(totalcrossUiMainWindow) ||
        ...
        case 179: { // putstatic?
          ...
          if (p.className.equals("totalcross/sys/Settings")) {
            ...
            if (field.equals("resizableWindow")) {
              DeploySettings.resizableWindow =
                bcs[j - 1] instanceof BC004_iconst_1;
            } else if (field.equals("isFullScreen")) {
              DeploySettings.isFullScreen =
                bcs[j - 1] instanceof BC004_iconst_1;
            ...
            } else if (18 <= bcs[j - 1].bc &&
                       bcs[j - 1].bc <=
                         20) { // guich@tc113_14: supports ldc_w and lcd2_w too
              String value;
              if (bcs[j - 1].bc == 18) {
              ...
              if (field.equals("fullScreenPlatforms")) {
                ...
                DeploySettings.applicationId =
                  totalcross.sys.Settings.applicationId = value;
              } else if (field.equals("appVersion")) {
                for (int i = 0, n = value.length(); i < n; i++) {
                  if (APPVER_VALID.indexOf(value.charAt(i)) == -1) {
                    throw new IllegalArgumentException(
                      "Settings.versionStr '" + value +
                      "' can only have digits and a dot.");
                  }
                }
                if (value.endsWith(".") &&
                    DeploySettings.appBuildNumber != -1) {
                  value += DeploySettings.appBuildNumber;
                }
                DeploySettings.appVersion = totalcross.sys.Settings.appVersion =
                  value;
  ...
  private void convertMethods(JavaClass jc, TCClass tc) throws Exception
  {
    ...
    for (int i = 0; i < methodCount; i++) {
      ...
      if (jm.name.equals("<clinit>")) {
        ...
        if (Bytecode2TCCode.hasMethodWith4D(jc, sign)) {
          ...
          newMethodCount--;
          Utils.println("removing method " + jm.signature + " of class " +
                        jc.className);
        }
      }
      currentMethod = null;
    ...
    // If the class does not have static initializer, but it has final fields,
    // even if not declared, the method static initializer is created It is
    // inserted last position of the array of methods.
    ...
    for (int i = 0, k = 0; i < methodCount; i++) {
      ...
      if (jm.name.endsWith("4D")) {
        ...
        jms[i].signature = name.substring(0, name.length() - 2) +
                           jm.signature.substring(name.length());
      }
      ...
      for (int j = 0; j < jm.paramCount; j++) {
        ...
        if (isClass) {
          tcm.cpParams[j] =
            GlobalConstantPool.putParam(p); // already in java's format
        } else {
    ...
    // now that the headers all methods were generated, we can convert their
    // instructions.
    for (int i = 0, k = 0; i < methodCount; i++) {
      JavaMethod jm = jms[i];
      ...
      if (jcode != null) {
        ...
        if (jm.name.equals("<clinit>")) { // guich@tc110: must check the full
                                          // name, not the cpname
        ...
        for (int j = 0; j < bc.length; j++) {
          Instruction tccode = Bytecode2TCCode.convert(
            bc, bc[j], stack, vcode, isStaticInitializer, jm.signature);
          if (dumpBytecodes) {
            System.out.println(bc[j] + " -> " + tccode); // dump
          }
        }
        Bytecode2TCCode.updateBranchs(vcode);
        tcm.exceptionHandlers =
          Bytecode2TCCode.updatePCsOfExceptionHandler(tcm.exceptionHandlers);
        tcm.insts = vcode;
      }
  ...
  private TCException[] convertExceptions(JavaException[] jexs)
  {
    ...
    for (int i = 0; i < jexs.length; i++) {
      ...
      t.className =
        j.catchType != null
          ? j.catchType
          : "java/lang/Throwable"; // guich@tc111_15: base exception class
                                   // is throwable, not Exception
    ...
    for (int i = 0; i < jfs.length; i++) {
      ...
      switch (flags.type) {
        ...
        case SHORT: {
          ...
          t.cpType = GlobalConstantPool.getPrimitiveTypeIndex(
            GlobalConstantPool.javaPrimitiveType2TCType(f.type));
          o = t;
          ...
          if (flags.isStatic) {
            ...
            if (flags.isFinal) {
              ...
              GenerateInstruction.newInstruction(
                vFinalVar, pref_MOV, target, value, -999);
            }
            sI32.addElement(o);
          } else {
        ...
        case DOUBLE: {
          ...
          if (f.constantValue != null) {
            if (flags.type == LONG) {
              ...
              t.dvalue = flags.type == DOUBLE
                           ? ((Double)f.constantValue).doubleValue()
              ...
              if (flags.isFinal && flags.isStatic) {
                ...
                Operand value =
                  new OperandSymD64(GlobalConstantPool.put(t.dvalue));
                GenerateInstruction.newInstruction(
                  vFinalVar, pref_MOV, target, value, -999);
              }
            }
        ...
        default: { // OBJECT
           ...
           * is referenced as: Ljava/lang/Class; -
           * class$totalcross$ui$html$Table$Row$EmptyCell
           */
          if (f.type.equals("Ljava/lang/Class;") &&
              f.name.startsWith("class$")) {
          ...
          if (f.constantValue != null) {
            ...
            idx = GlobalConstantPool.putStr(
              (String)t.value); // add the value to the temporary constant pool
          }
          if (flags.isStatic) {
            index = GlobalConstantPool.putStaticField(jc.className, f.name);
            ...
            /*if (flags.isFinal && idx != -1)
                    {
                       ...
                       target = new OperandExternal(regO, (OperandSym) target);
                       Operand value = new OperandSymO(idx);
                       GenerateInstruction.newInstruction(vFinalVar, pref_MOV,
               target, value, 0);
                    }*/
            iObj.addElement(o);
          }
      ...
      // set the index to access the field in the constant pool (class name +
      // field name)
  ...
  {
    // class$totalcross$ui$html$Table$Row$EmptyCell ->
    // totalcross.ui.html.Table$Row$EmptyCell
  ...
  private TCObjectField[] v2aObj(Vector obj)
  { // vector to array
  ...
  private static void expandZip(Map<String, JavaClass> classes,
                                Vector vin,
                                String fName) throws Exception
  {
    ...
    ZipInputStream zIn = new ZipInputStream(
      new java.io.BufferedInputStream(new java.io.FileInputStream(f), 65000));
    try {
      ...
      // then we place it at the begining of the list, to ensure that it will be
      // in the first tcz if they are splitted.
      ArrayList<NameBytes> all = new ArrayList<NameBytes>(500);
      for (java.util.zip.ZipEntry zEntry = zIn.getNextEntry(); zEntry != null;
           zEntry = zIn.getNextEntry()) {
        // if the name is a path, exit.
        String name = zEntry.getName();
        if (name.endsWith("/") || name.toLowerCase().startsWith("meta-inf") ||
            name.endsWith(".java")) {
          continue;
        }
  ...
  private static int processFiles(Vector vin, Vector vout) throws Exception
  {
    ...
    for (int i = 0; i < vin.size();) {
      try {
        ...
        if (nameLow.endsWith(".class")) { // is this a class file?
          ...
          if (j2.converted == null || DeploySettings.testClass) {
            ...
            if (!DeploySettings.testClass) {
              vout.addElement(
                new TCZ.Entry(bytes,
                              j2.converted.className,
                              len)); // note that all files must be added -
                                     // use the real package name
            }
          }
        } else if (!DeploySettings.testClass) {
          ByteArrayStream basz = new ByteArrayStream(bytes);
          print = !name.equals("tcparms.bin") &&
                  !name.equals(DeploySettings.TCAPP_PROP);

          if (print) {
            System.out.print("Adding " + name);
          }
          tcbasz.reset();
          if (nameLow.endsWith(".bmp") ||
              nameLow.endsWith(".gif")) { // convert gif and bmp to png
            new Image(bytes).createPng(basz = new ByteArrayStream(8192));
            len = basz.getPos();
            System.out.print(" (converted to PNG)");
          }

          tc.tools.converter.Storage.compressAndWrite(basz,
                                                      new DataStream(tcbasz));
          bytes = tcbasz.toByteArray();
          vout.addElement(new TCZ.Entry(
            bytes, name, len)); // note that all files must be added
        }
        if (print && len != 0) {
        ...
        System.out.println("Limit reached for " + cplre.getMessage() +
                           ". Splitting tcz...");
    ...
    if (!DeploySettings.isJarOrZip) {
      ...
      for (int i = n - 1; i >= 0;
           i--) { // first, remove the ones that were already processed
      ...
      if (n > 0) {
        System.out.println("Adding " + n +
                           " classes specified with Class.forName");
        String[] classes = (String[])callForName.toObjectArray();

        Vector temp =
          new Vector(50); // process callForName on this temporary vector
  ...
  private static void addAndExpand(Vector vin,
                                   String name,
                                   boolean throwErrorIfInexistant)
    throws Exception
  {
    JavaClass jc;
    if (htVisited.exists(name) || name.startsWith("tc/tools") ||
        name.startsWith("tc/Deploy") || name.startsWith("net/rim")) {
      return;
    }
    htVisited.put(name, name);
    byte[] bytes = Utils.findAndLoadFile(name, false);
    boolean isClass =
      name.toLowerCase().endsWith(".class") && !isPrimitiveClass(name);
    if (bytes == null) {
      if (isClass) {
  ...
  {
    return name.equals("byte.class") || name.equals("short.class") ||
      name.equals("int.class") || name.equals("boolean.class") ||
      name.equals("char.class") || name.equals("long.class") ||
      name.equals("float.class") || name.equals("double.class") ||
      name.equals("void.class");
  }
        ...
        switch (jci.type) {
          ...
          case 8: // string
            if (!inProhibitedList(c, false) && isValidFile(c) &&
                !htAddedClasses.exists(c) /* && !inExclusionList(c)*/
                /* && !htExcludedClasses.exists(c)*/) {
  ...
  {
    c = c.replace('.', '/');
    // return c.length() == 0 || c.startsWith("java/") || c.charAt(0) == '[' ||
    // c.startsWith("sun/") || c.startsWith("com/sun/") ||
    // c.startsWith("javax/");
    return c.length() == 0 ||
      (blockTC && !DeploySettings.isTotalCrossJarDeploy &&
       c.startsWith("totalcross/")) ||
      c.startsWith("java/") || c.charAt(0) == '[' || c.startsWith("sun/") ||
      c.startsWith("com/sun/") || c.startsWith("javax/");
  }
  ...
  public static void process(String fName, int options) throws Exception
  {
    ...
    if (fLow.endsWith(".tcz")) { // if the user passed a tcz file, use it to
                                 // create the installers
      DeploySettings.inputFileWasTCZ =
        true; // guich@tc100b5_53: don't delete the tcz file if it was the
              // input file.
      ...
      DeploySettings.targetDir = Convert.appendPath(
        DeploySettings.targetDir != null ? DeploySettings.targetDir
                                         : DeploySettings.currentDir,
        "/install/");
      ...
      // flsobral@tc123_56: fixed Deploy to correctly handle using a tcz file as
      // input.
      ...
      DeploySettings.isJarOrZip =
        fLow.endsWith(".zip") || fLow.endsWith(".jar");
      ...
      if (DeploySettings.isJarOrZip) { // process bat of files?
              ...
              if (files[i].endsWith(".class")) {
                try {
                  ...
                  if (isMain(jc) && !jc.className.contains("$")) {
                    if (found) {
                      throw new IllegalArgumentException(
                        "More than one MainWindow/MainClass found in the given folder: " +
                        fName + " and " + files[i] +
        ...
        if (fLow.endsWith(".class") ||
            (fName.indexOf('.') < 0 &&
             new File(fName + ".class")
               .exists())) { // if the user passed a .class or a class without
                             // .class...
          if (!fLow.endsWith(".class")) {
            fName += ".class";
            fLow += ".class";
          ...
          setApplicationProperties(jc);
          // if (jc.className.indexOf('/') > 0) // does it have a package? -
          // guich@tc114_84: support class without package
          {
            DeploySettings.baseDir = Utils.getBaseFolder(
              DeploySettings.currentDir, fName, jc.className);
            if (DeploySettings.baseDir == null) {
              throw new IllegalArgumentException(
                "Class " + jc.className +
                " is at a directory that does not correspond to the package declaration!");
            }
            DeploySettings.mainClassDir =
              Utils.getParent(DeploySettings.baseDir + "/" + jc.className);
      ...
      if (DeploySettings.mainClassName != null &&
          !DeploySettings.isTotalCrossJarDeploy &&
          DeploySettings
            .isMainWindow) { // flsobral@tc126: test for mainClassName instead
                             // of mainClassDir. The later is always null when
                             // deploy is used with a zip/jar file. This fixes
                             // third-party server activation when the
                             // application is deployed from a jar file. -
                             // guich@tc310 only if not deploying the sdk and
                             // if its not a library
        Hashtable htVmParams = new Hashtable(4);
        if (totalcross.sys.Settings.activationServerURI != null) {
          htVmParams.put("activationServerURI",
                         totalcross.sys.Settings.activationServerURI);
        }
        if (totalcross.sys.Settings.activationServerNamespace != null) {
          htVmParams.put("activationServerNamespace",
                         totalcross.sys.Settings.activationServerNamespace);
        }
        if (totalcross.sys.Settings.applicationId != null) {
          htVmParams.put("applicationId",
        ...
        if (totalcross.sys.Settings.pushTokenAndroid != null) {
          htVmParams.put("pushTokenAndroid",
                         totalcross.sys.Settings.pushTokenAndroid);
        }
        if (totalcross.sys.Settings.iosCertDate != null) {
          htVmParams.put("iosCertDate",
                         totalcross.sys.Settings.iosCertDate.toIso8601());
        }
        byte[] htDump =
          htVmParams.getKeyValuePairs("=").toString("\n").getBytes();
        vin.addElement(new TCZ.Entry(htDump, "tcparms.bin", htDump.length));
      }
      if (DeploySettings.tcappProp != null) {
        vin.addElement(new TCZ.Entry(DeploySettings.tcappProp,
                                     DeploySettings.TCAPP_PROP,
                                     DeploySettings.tcappProp.length));
      }
      ...
      for (int part = 0; vin.size() > 0; part++) {
        ...
        if (!GlobalConstantPool.isEmpty()) { // guich@tc111_2: only store if
                                             // there's at least one user class
          ...
          vout.addElement(
            new TCZ.Entry(tcbasz.toByteArray(), "ConstantPool", orig));
        }
        // now that all files were processed, put everything in a single tcz
        // file. set the TCZ attributes
        short attr = 0;
        ...
        if (cn != null) {
          ...
          if (part == 0) {
            attr |= (DeploySettings.isMainWindow ? TCZ.ATTR_HAS_MAINWINDOW
                                                 : TCZ.ATTR_HAS_MAINCLASS);
          }
        } else {
          cn = fName;
        }
        ...
        if (DeploySettings.isJarOrZip && (cn.toLowerCase().endsWith(".zip") ||
                                          cn.toLowerCase().endsWith(".jar"))) {
          cn = cn.substring(0, cn.length() - 4);
        }
        if (DeploySettings.filePrefix == null) {
          DeploySettings.filePrefix = cn;
        }
        DeploySettings.tczFileName =
          ((DeploySettings.targetDir != null ? DeploySettings.targetDir
                                             : DeploySettings.currentDir) +
           "/" + DeploySettings.filePrefix + ".tcz")
            .replace('\\', '/');

        if (vin.size() == 0) {
          if (DeploySettings.targetDir != null &&
              DeploySettings.targetDir.length() > 0) {
    ...
    if (!DeploySettings.testClass) {
      ...
      if (DeploySettings.appTitle != null &&
          DeploySettings.appTitle.trim().length() > 0) {
        System.out.println("Application title: " + DeploySettings.appTitle);
      } else {
        System.out.println("Application title not provided, using: " +
                           (DeploySettings.appTitle = cn));
      }
      if (DeploySettings.companyInfo != null) {
        System.out.println("Company information: " +
                           DeploySettings.companyInfo);
      }
      if (DeploySettings.applicationId != null) {
        ...
        System.out.println("Application id not provided. Created from \"" +
                           name + "\": " + DeploySettings.applicationId);
      }
  ...
  {
    if ("totalcross/ui/MainWindow".equals(jc.superClass) ||
        "totalcross/game/GameEngineMainWindow".equals(jc.superClass) ||
        "totalcross/game/GameEngine".equals(jc.superClass) ||
        "totalcross/unit/TestSuite".equals(jc.superClass) ||
        "totalcross/io/sync/Conduit".equals(jc.superClass) ||
        "totalcross/Service".equals(jc.superClass)) {
      return true;
    }
    if (jc.interfaces != null) {
      for (int i = 0; i < jc.interfaces.length; i++) {
  ...
  public static Map<String, JavaClass> getJavaClassFromJar(JarFile jarFile)
    throws Exception
  {
