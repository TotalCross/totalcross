...
public final class J2TC implements JConstants, TCConstants
{
  ...
  {
    JavaClass jc;
    if (htVisited.exists(name) || name.startsWith("tc/tools") || name.startsWith("tc/Deploy") || name.startsWith("net/rim")){
      return;
    }
    ...
    for (int i =1; i < jcp.numConstants; i++) {
      if (jcp.constants[i] instanceof JavaConstantInfo)
      {
        ...
        case 7: // class identifier
          c += ".class";
        case 8: // string
          if (!inProhibitedList(c,false) && isValidFile(c) && !htAddedClasses.exists(c)/* && !inExclusionList(c)*//* && !htExcludedClasses.exists(c)*/) {
