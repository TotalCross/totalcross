...
public final class J2TC implements JConstants, TCConstants {
			...
			switch (bcs[j].bc) {
				case 18: // ldc
					if (DeploySettings.appTitle == null
						&& j < 7) { // guich@tc100br_14: restrict it to the very first begining of the constructor
						...
						if (bcs[j + 1].bc == 183) { // invoke special?
							...
							if (className.equals("java/lang/StringBuffer") && signature.equals("<init>(Ljava/lang/String;)")) {
								...
								DeploySettings.appTitle = Utils.stripNonLetters((String)
														  ldc.val.asObj); // if the user wrote "Agenda 1.0", break at the "Agenda"
								continue;
							}
						}
						...
						if (field.equals("resizableWindow")) {
							...
							if (bcs[j - 1] instanceof BC004_iconst_1) {
								...
								DeploySettings.windowSize = Settings.WINDOWSIZE_600X800;
								   ...
								   && bcs[j - 1].bc <= 20) { // guich@tc113_14: supports ldc_w and lcd2_w too
							String value;
							if (bcs[j - 1].bc == 18) {
		...
		for (int i = 0, k = 0; i < methodCount; i++) {
			...
			if (jcode != null) {
				...
				for (int j = 0; j < bc.length; j++) {
					Instruction tccode = Bytecode2TCCode.convert(bc, bc[j], stack, vcode, isStaticInitializer,
										 jm.signature);
					if (dumpBytecodes) {
	...
	private TCException[] convertExceptions(JavaException[] jexs) {
		...
		for (int i = 0; i < jexs.length; i++) {
			...
			j = jexs[i];
			t.className = j.catchType != null ? j.catchType :
						  "java/lang/Throwable"; // guich@tc111_15: base exception class is throwable, not Exception
		...
		for (int i = 0; i < jfs.length; i++) {
			...
			switch (flags.type) {
				...
				case SHORT: {
					...
					t.cpType = GlobalConstantPool.getPrimitiveTypeIndex(GlobalConstantPool.javaPrimitiveType2TCType(
								   f.type));
					o = t;
	...
	private static void expandZip(Map<String, JavaClass> classes, Vector vin,
		...
		ZipInputStream zIn = new ZipInputStream(new java.io.BufferedInputStream(new java.io.FileInputStream(
				f), 65000));
		try {
			...
			try {
				...
				if (nameLow.endsWith(".class")) { // is this a class file?
					...
					if (j2.converted == null || DeploySettings.testClass) {
						...
						if (!DeploySettings.testClass) {
							vout.addElement(new TCZ.Entry(bytes, j2.converted.className,
														  len)); // note that all files must be added - use the real package name
						}
					}
	...
	private static void addAndExpand(Vector vin, String name,
									 boolean throwErrorIfInexistant) throws Exception {
		JavaClass jc;
	...
	public static boolean inProhibitedList(String c, boolean blockTC) {
		...
		return c.length() == 0 || (blockTC && !DeploySettings.isTotalCrossJarDeploy
	...
	public static void process(String fName, int options) throws Exception {
		...
		if (fLow.endsWith(".tcz")) { // if the user passed a tcz file, use it to create the installers
			DeploySettings.inputFileWasTCZ =
				true; // guich@tc100b5_53: don't delete the tcz file if it was the input file.
			...
			DeploySettings.targetDir = Convert.appendPath(
										   DeploySettings.targetDir != null ? DeploySettings.targetDir : DeploySettings.currentDir,
										   "/install/");
			...
			if (DeploySettings.isJarOrZip) { // process bat of files?
				...
				if (fLow.endsWith(".class") || (fName.indexOf('.') < 0
												&& new File(fName +
														".class").exists())) { // if the user passed a .class or a class without .class...
			...
			if (DeploySettings.mainClassName != null && !DeploySettings.isTotalCrossJarDeploy
				&& DeploySettings.isMainWindow) { //flsobral@tc126: test for mainClassName instead of mainClassDir. The later is always null when deploy is used with a zip/jar file. This fixes third-party server activation when the application is deployed from a jar file. - guich@tc310 only if not deploying the sdk and if its not a library
			...
			if (DeploySettings.tcappProp != null) {
				vin.addElement(
					new TCZ.Entry(DeploySettings.tcappProp, DeploySettings.TCAPP_PROP,
		...
		if (!DeploySettings.testClass) {
			...
			if (DeploySettings.applicationId != null) {
				...
				System.out
				.println("Application id not provided. Created from \"" + name + "\": " +
