...
public class RegAllocation
{
  ...
  {
    J2TC.currentMethod =
      GlobalConstantPool.getMethodFieldName(cfg.method.cpName);
    adjListI = null;
    adjListD = null;
    adjListO = null;
  ...
  public void updateRegOExceptionHandler(TCException[] tces,
                                         AdjListNode[] adjList)
  {
    TCException e;
    if (tces != null) {
    ...
    for (int i = 0; i < cfg.order.length; i++) {
      ...
      if (insts != null) {
        for (int j = 0; j < insts.length; j++) {
          BitSet[] ud = insts[j].defsAndUses(
            bb, cfg.method.iCount, cfg.method.v64Count, cfg.method.oCount);
          BitSet def = null;
          switch (regType) {
            case TClassConstants.RegI:
              def = ud[3];
        ...
        if (k != -1) {
          for (; k < in; k++) {
            if (use.isOn(k)) {
              int regLive = k;
              if (regDef != regLive) {
                adjMatrix[Math.max(regDef, regLive)].on(
                  Math.min(regDef, regLive));
              }
            }
          }
        }
  ...
  public void createAdjList(int len) { adjList = new AdjListNode[len]; }
  ...
  public void pruneGraph(int len)
  {
    ...
    while (!sucess) {
      ...
      for (int i = TCConstants.PHYSICALREGCOUNT; i < len; i++) {
        if (!adjList[i].pushed &&
            adjList[i].nInts() < TCConstants.PHYSICALREGCOUNT) {
          sucess = false;
          stack.push(i);
          adjustNeighbors(i);
    ...
    if (nodes > 0) {
      throw new ConverterException(
        "This method is too big and must be splitted into two or more parts.");
