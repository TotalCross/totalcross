...
public class RegAllocation
{
  ...
  public boolean allocateRegisters(CFG cfg, int regType)
  {
    ...
    switch (regType)
    {
    case TClassConstants.RegI: pruneGraph(cfg.iCount()); break;
    case TClassConstants.RegD: pruneGraph(cfg.v64Count()); break;
    case TClassConstants.RegO: pruneGraph(cfg.oCount()); break;
    }
    return assignRegs();
  }
  ...
  public void buildAdjMtx(CFG cfg, int regType)
  {
    switch (regType)
    {
    case TClassConstants.RegI: createMatrix(cfg.iCount()); break;
    case TClassConstants.RegD: createMatrix(cfg.v64Count()); break;
    case TClassConstants.RegO: createMatrix(cfg.oCount()); /*reserveRegOZero();*/ break;
    }
    ...
    for(int i=0;i<cfg.order.length;i++)
    {
      ...
      if (insts != null)
      {
        for(int j=0;j<insts.length;j++)
        {
          ...
          switch (regType)
          {
          case TClassConstants.RegI: def = ud[3]; break;
          case TClassConstants.RegD: def = ud[4]; break;
          case TClassConstants.RegO: def = ud[5];
          }               
  ...
  public void buildAdjList(CFG cfg, int regType)
  {
    ...
    switch (regType)
    {
    case TClassConstants.RegI: len = cfg.iCount(); break;
    case TClassConstants.RegD: len = cfg.v64Count(); break;
    case TClassConstants.RegO: len = cfg.oCount(); break;
    }
    createAdjList(len);
    for (int i=0;i<len;i++)
    {
