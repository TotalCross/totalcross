...
{
  public int[] jumps;
  public int def; // pc for the "default" instruction. if there's no default, it
                  // points to the end of the switch.
  public int paramStackPos;

  public Switch(int paramStackPos) { this.paramStackPos = paramStackPos; }
  /*
  ...
  Which proves that is impossible to find where the switch ends. (marked by *)
   ...
   {
      // here we scan the code from the first case to the last one and find the
  farest goto ByteCode []bcs = jc.bcs; int pcAfterSwitch = pcInMethod + pcInc;
