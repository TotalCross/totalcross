...
public class ByteCode implements JConstants
{
  ...
  /**
   * Current program counter being interpreted. Must be incremented by the
   * interpreter.
   */
  ...
  public ByteCode getNext() { return jc.bcs[posInMethod + 1]; }

  public ByteCode getPrev() { return jc.bcs[posInMethod - 1]; }

  public ByteCode() { pcInMethod = pc; }
  ...
  public static void initClasses() throws ClassNotFoundException
  {
    if (bcClasses == null) { // init only once
      ...
      // by directly assigning the real class, we can safely obfuscate these
      // classes
  ...
  public static void initLocal(int max) { local = allocTCValues(local, max); }
  ...
  {
    return (code[p + 3] & 0xFF) | ((code[p + 2] & 0xFF) << 8) |
      ((code[p + 1] & 0xFF) << 16) | ((code[p + 0] & 0xFF) << 24);
  }
  ...
  protected static int readUInt8(int p) { return code[p] & 0xFF; }
  ...
  {
    sbuf.setLength(0);
    return sbuf.append(posInMethod)
      .append("  ")
      .append(pcInMethod)
      .append(": ")
      .append(bcClassNames[bc].substring(6))
      .toString();
  }
}
