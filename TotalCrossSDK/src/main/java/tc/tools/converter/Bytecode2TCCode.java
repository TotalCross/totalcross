...
{
  public static int[] isTargetOfGoto; // if zero, it is not target of goto.
                                      // Otherwise, it is target of goto.
  ...
  private static Instruction processIINC(ByteCode[] bcs,
                                         ByteCode i,
                                         OperandStack stack,
                                         Vector vcode,
                                         boolean isStaticInitializer)
  {
    ...
    return GenerateInstruction.newInstruction(
      vcode, pref_ADD, target, target, opr, lineOfPC);
  }

  private static void processILOAD(ByteCode[] bcs,
                                   ByteCode i,
                                   OperandStack stack,
                                   Vector vcode,
                                   boolean isStaticInitializer)
  ...
  private static void processLLOAD(ByteCode[] bcs,
                                   ByteCode i,
                                   OperandStack stack,
                                   Vector vcode,
                                   boolean isStaticInitializer)
  ...
  private static void processFLOAD(ByteCode[] bcs,
                                   ByteCode i,
                                   OperandStack stack,
                                   Vector vcode,
                                   boolean isStaticInitializer)
  ...
  private static void processDLOAD(ByteCode[] bcs,
                                   ByteCode i,
                                   OperandStack stack,
                                   Vector vcode,
                                   boolean isStaticInitializer)
  ...
  private static void processALOAD(ByteCode[] bcs,
                                   ByteCode i,
                                   OperandStack stack,
                                   Vector vcode,
                                   boolean isStaticInitializer)
  ...
  private static Instruction processISTORE(ByteCode[] bcs,
                                           ByteCode i,
                                           OperandStack stack,
                                           Vector vcode,
                                           boolean isStaticInitializer)
  {
    ...
    return GenerateInstruction.newInstruction(
      vcode, pref_MOV, target, opr, lineOfPC);
  }

  private static Instruction processLSTORE(ByteCode[] bcs,
                                           ByteCode i,
                                           OperandStack stack,
                                           Vector vcode,
                                           boolean isStaticInitializer)
  ...
  private static Instruction processFSTORE(ByteCode[] bcs,
                                           ByteCode i,
                                           OperandStack stack,
                                           Vector vcode,
                                           boolean isStaticInitializer)
  ...
  private static Instruction processDSTORE(ByteCode[] bcs,
                                           ByteCode i,
                                           OperandStack stack,
                                           Vector vcode,
                                           boolean isStaticInitializer)
  ...
  private static Instruction processASTORE(ByteCode[] bcs,
                                           ByteCode i,
                                           OperandStack stack,
                                           Vector vcode,
                                           boolean isStaticInitializer)
  {
    ...
    if (regIndex != -1) {
      ...
      if (setExceptionHandler(currentExceptionHandlers,
                              isTargetOfGoto[indexOfCurrentBytecode],
                              target.index)) {
        return GenerateInstruction.newInstruction(
          vcode, pref_MOV, target, target, lineOfPC);
      } else {
        // even excluding the handler of exception, the java compiler can leave
        // dirt, which should be discarded.
        if (!stack.empty()) {
          Operand opr = stack.pop();
          return GenerateInstruction.newInstruction(
            vcode, pref_MOV, target, opr, lineOfPC);
  ...
  private static Instruction processRET(ByteCode[] bcs,
                                        ByteCode i,
                                        OperandStack stack,
                                        Vector vcode,
                                        boolean isStaticInitializer)
  ...
  private static Instruction processWIDE(ByteCode[] bcs,
                                         ByteCode i,
                                         OperandStack stack,
                                         Vector vcode,
                                         boolean isStaticInitializer)
  ...
  public static Instruction convert(ByteCode[] bcs,
                                    ByteCode i,
                                    OperandStack stack,
                                    Vector vcode,
                                    boolean isStaticInitializer,
                                    String signature)
  {
    ...
    if (isTargetOfGoto[indexOfCurrentBytecode] != 0) {
      htBytecodeIndex.put(isTargetOfGoto[indexOfCurrentBytecode],
                          countOfTCCode(vcode));
    }
    ...
    switch (op) {
      ...
      case LDC_W: { // 19
        ...
        if (v.type == INT) {
          ...
          stack.push(
            new OperandSymO(GlobalConstantPool.putStr((String)v.asObj)));
        }
        break;
      }
      case LDC2_W: { // 20
      ...
      case SALOAD: { // 53
        ...
        OperandReg regIndex = (OperandReg)GenerateInstruction.promoteOperand(
          vcode, index, opr_regI, lineOfPC);
        OperandArrayAccess base =
          new OperandArrayAccess(opr_arcI, regO, regIndex);
        ...
        tc = GenerateInstruction.newInstruction(
          vcode, pref_MOV, reg, base, lineOfPC);
        stack.push(reg);
        break;
      }
      case LALOAD: { // 47
        ...
        OperandReg regIndex = (OperandReg)GenerateInstruction.promoteOperand(
          vcode, index, opr_regI, lineOfPC);
        OperandArrayAccess base =
          new OperandArrayAccess(opr_arcL, regO, regIndex);
      ...
      case DALOAD: { // 49
        ...
        OperandReg regIndex = (OperandReg)GenerateInstruction.promoteOperand(
          vcode, index, opr_regI, lineOfPC);
        OperandArrayAccess base =
          new OperandArrayAccess(opr_arcD, regO, regIndex);
      ...
      case AALOAD: { // 50
        ...
        OperandReg regIndex = (OperandReg)GenerateInstruction.promoteOperand(
          vcode, index, opr_regI, lineOfPC);
        OperandArrayAccess base =
          new OperandArrayAccess(opr_arcO, regO, regIndex);
      ...
      case SASTORE: { // 86
        ...
        tc = GenerateInstruction.newInstruction(
          vcode, pref_MOV, base, value, lineOfPC);
        break;
      }
      case FASTORE:   // 80
      case DASTORE: { // 82
      ...
      case IADD: { // 96
        ...
        tc = GenerateInstruction.newInstruction(
          vcode, pref_ADD, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LADD: { // 97
      ...
      case ISUB: { // 100
        ...
        if (opr2.isConstantInt()) {
          ...
          if (c.value >= -2047 && c.value <= 2048) {
            ...
            c.kind = opr_s12I;
            tc = GenerateInstruction.newInstruction(
              vcode, pref_ADD, target, opr1, c, lineOfPC);
            ...
            tc = GenerateInstruction.newInstruction(
              vcode, pref_SUB, target, opr1, c, lineOfPC);
          }
        } else {
          tc = GenerateInstruction.newInstruction(
            vcode, pref_SUB, target, opr1, opr2, lineOfPC);
        }
        stack.push(target);
        break;
      }
      ...
      case IMUL: { // 104
        ...
        OperandReg target = new OperandRegI();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_MUL, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LMUL: { // 105
      ...
      case IDIV: { // 108
        ...
        OperandReg target = new OperandRegI();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_DIV, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LDIV: { // 109
      ...
      case IREM: { // 112
        ...
        OperandReg target = new OperandRegI();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_MOD, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LREM: { // 113
      ...
      case ISHL: { // 120
        ...
        OperandReg target = new OperandRegI();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_SHL, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LSHL: { // 121
      ...
      case ISHR: { // 122
        ...
        OperandReg target = new OperandRegI();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_SHR, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LSHR: { // 123
      ...
      case IUSHR: { // 124
        ...
        OperandReg target = new OperandRegI();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_USHR, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LUSHR: { // 125
      ...
      case IAND: { // 126
        ...
        OperandReg target = new OperandRegI();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_AND, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LAND: { // 127
      ...
      case IOR: { // 128
        ...
        OperandReg target = new OperandRegI();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_OR, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LOR: { // 129
      ...
      case IXOR: { // 130
        ...
        OperandReg target = new OperandRegI();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_XOR, target, opr1, opr2, lineOfPC);
        stack.push(target);
        break;
      }
      case LXOR: { // 131
      ...
      case D2L: { // 143
        ...
        OperandReg target = new OperandRegL();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_CONV, target, opr, lineOfPC);
        stack.push(target);
        break;
      }
      case I2F:   // 134
      ...
      case IFLE: { // 158
        ...
        OperandConstant target =
          new OperandConstant32(isTargetOfGoto[ji.jumpIfTrue], type_Int);
        int prefix = 0;
        switch (op) {
          case IFEQ:
            prefix = pref_JEQ;
        ...
        tc = GenerateInstruction.newInstruction(
          vcode, prefix, v1, v2, target, lineOfPC);
        break;
      }
      case IF_ICMPEQ:   // 159
      case IF_ICMPNE:   // 160
      ...
      case IF_ACMPNE: { // 166
        ...
        stackOfBranch(vcode, bcs, stack, ji.jumpIfTrue);
        tc = GenerateInstruction.newInstruction(
          vcode, prefix, value1, value2, target, lineOfPC);
        break;
      }
      case GOTO:     // 167
      case GOTO_W: { // 200
        Branch ji = (Branch)i;
        OperandConstant branch =
          new OperandConstant32(isTargetOfGoto[ji.jumpTo], type_Int);
        stackOfBranch(vcode, bcs, stack, ji.jumpTo);
        tc = GenerateInstruction.newInstruction(
          vcode, pref_JUMP, branch, lineOfPC);
        break;
      }
      case JSR:     // 168
      case JSR_W: { // 201
      ...
      case LOOKUPSWITCH: { // 171
        ...
        GenerateInstruction.newInstruction(
          vcode, ji.def + indexOfCurrentBytecode, key, ji, lineOfPC);
        break;
      }
      case IRETURN: { // 172
        Operand value = stack.pop();
        GenerateInstruction.newInstruction(
          vcode, pref_RETURNI, value, lineOfPC);
        break;
      }
      case LRETURN: { // 173
        Operand value = stack.pop();
        GenerateInstruction.newInstruction(
          vcode, pref_RETURNL, value, lineOfPC);
        break;
      }
      case FRETURN: { // 174
        Operand value = stack.pop();
        GenerateInstruction.newInstruction(
          vcode, pref_RETURND, value, lineOfPC);
        break;
      }
      case DRETURN: { // 175
        Operand value = stack.pop();
      ...
      case ARETURN: { // 176
        Operand value = stack.pop();
        GenerateInstruction.newInstruction(
          vcode, pref_RETURNO, value, lineOfPC);
        break;
      }
      case RETURN: { // 177
      ...
      case GETSTATIC: { // 178
        ...
        if (currentJClass.className.equals(ji.className) &&
            !existsField(currentJClass.fields, ji.fieldName)) {
        ...
        int index = GlobalConstantPool.putStaticField(
          ji.className,
          ji.fieldName); // external fields require the class and the field name
        if (!isStaticInitializer) {
          htStaticFieldIndexes.put(index, index);
        }
        Operand source = new OperandSym(kind, index);
        tc = GenerateInstruction.newInstruction(
          vcode, pref_MOV, target, source, lineOfPC);
        stack.push(target);
        break;
      }
      case PUTSTATIC: { // 179
        ...
        int index =
          GlobalConstantPool.putStaticField(ji.className, ji.fieldName);
        if (!isStaticInitializer) {
          htStaticFieldIndexes.put(index, index);
        }
        ...
        Operand value = stack.pop();
        tc = GenerateInstruction.newInstruction(
          vcode, pref_MOV, target, value, lineOfPC);
        break;
      }
      case GETFIELD: { // 180
        ...
        int index =
          GlobalConstantPool.putInstanceField(ji.className, ji.fieldName);
      ...
      case INVOKEINTERFACE: { // 185
        ...
        if (ji.className.equals("java/lang/Object") &&
            ji.signature.equals("<init>()")) {
          stack.pop();
          break;
        }
        ...
        if (op == INVOKESTATIC) {
          ...
          _this = (OperandRegO)GenerateInstruction.promoteOperand(
            vcode, obj, opr_regO, lineOfPC);
        }

        int idx = GlobalConstantPool.putMethod(
          ji.className, ji.name, ji.jargs, ji.signature);
        OperandSym sym = new OperandSymO(idx);
        if (ji.name.equals(
              "forName")) { // keep track of the list of classes used in forName
        ...
        Call call = (Call)GenerateInstruction.newInstruction(
          vcode,
          op == INVOKEVIRTUAL ? CALL_virtual : CALL_normal,
          sym,
          _this,
          retAndParams,
          retType == type_Void,
      ...
      case NEW: { // 187 (186 is not used)
        ...
        tc = GenerateInstruction.newInstruction(
          vcode, pref_NEWOBJ, regO, sym, lineOfPC);
        stack.push(regO);
        break;
      }
      case NEWARRAY: { // 188
        ...
        tc = GenerateInstruction.newInstruction(
          vcode, pref_NEWARRAY, reg, sym, len, lineOfPC);
        stack.push(reg);
        break;
      }
      case ANEWARRAY: { // 189
        BC189_anewarray ji = (BC189_anewarray)i;
        String classType = ji.classType.charAt(0) == '['
                             ? "[" + ji.classType
                             : "[L" + ji.classType + ";";
      ...
      case ARRAYLENGTH: { // 190
        ...
        OperandArrayAccess base =
          new OperandArrayAccess(opr_arlen, regBase, null);
      ...
      case ATHROW: { // 191
        /*
               ...
               Eclipse has an optimization that removes an extra move of the
           lock from the local variables to the stack. This is the code it
           generates:
               ...
               At line 14, the stack holds the exception at top, and the lock at
           local variable 1. However, the TC converter isn't aware that the
           exception is at the top of the stack, so we check it and provide that
           extra operand that tells it.
         */
        OperandReg regO =
          stack.empty() ? new OperandRegO() : (OperandReg)stack.pop();
      ...
      case JCHECKCAST: { // 192
        ...
        OperandSymO sym = new OperandSymO(index);
        tc = GenerateInstruction.newInstruction(
          vcode, pref_CHECKCAST, regO, sym, lineOfPC);
        stack.push(regO);
        break;
      }
      case JINSTANCEOF: { // 193
        ...
        OperandSymO sym = new OperandSymO(index);
        tc = GenerateInstruction.newInstruction(
          vcode, pref_INSTANCEOF, regI, regO, sym, lineOfPC);
        stack.push(regI);
        break;
      }
      ...
      case MULTIANEWARRAY: { // 197
        ...
        tc =
          GenerateInstruction.newInstruction(vcode, reg, sym, dims, lineOfPC);
        stack.push(reg);
        break;
      }
      case IFNULL:      // 198
        ...
        switch (op) {
          ...
          case JNE_regI_sym: {
            ...
            if (desloc < -32 || desloc > 31) {
              ...
              GenerateInstruction.newInstruction(
                vc, pref_MOV, target, sym, tc.line);
              int reg = i.reg;
              switch (op) {
                case JEQ_regI_sym:
                  op = JEQ_regI_regI;
              ...
              Reg_reg_s12 i2 =
                new Reg_reg_s12(op, i.line, reg, target.index, desloc);
              tc = i2;
            } else {
              i.desloc = desloc;
            }
    ...
    for (int j = 0; j < vcode.size(); j++) {
      ...
      switch (op) {
        ...
        case JGE_regD_regD: {
          ...
          int branch = i.s12;
          i.s12 =
            newInstructionsCount(newInsts, tccodeCount, tccodeCount + branch) +
            branch;
          break;
        }
        ...
        case SWITCH: {
          ...
          int v1 = p1.v1;
          int target = v1;
          p1.v1 = newInstructionsCount(
                    newInsts, tccodeCount + 1, tccodeCount + 1 + target) +
                  target;
          ...
          for (int x = 0; x < n; x++) {
            if ((x & 1) == 1) {
              p1 = (Two16)i.params[k];
              target = p1.v2;
              p1.v2 = newInstructionsCount(
                        newInsts, tccodeCount + k, tccodeCount + k + target) +
                      target;
            } else {
              p1 = (Two16)i.params[++k];
              target = p1.v1;
  ...
  public static TCException[] updatePCsOfExceptionHandler(TCException[] tces)
    throws Exception
  {
    if (tces != null) {
      ...
      for (int j = 0; j < tces.length; j++) {
        ...
        e.endPC = pc +
                  newInstructionsCount(newInsts,
                                       0,
                                       pc) -
                  1; // sub "-1" because the endPC is exclusive ==> interval
                     // [startPC, endPC)
  ...
  public static void CheckIfIsGotoTarget(int BCIndex,
                                         ByteCode bc,
                                         OperandStack stack,
                                         Vector vcode)
  {
  ...
  private static void stackOfBranch(Vector vcode,
                                    ByteCode[] bcs,
                                    OperandStack stack,
                                    int BCIndex)
  {
    ...
    if (!stack.empty()) {
      ...
      if (s == null) {
        do {
          ...
          GenerateInstruction.newInstruction(
            vcode, pref_MOV, reg, opr, lineOfPC);
          stack.push(reg);
        } while (!s2.empty());
