...
public class Bytecode2TCCode implements JConstants, TCConstants {
	public static int[]
	isTargetOfGoto; // if zero, it is not target of goto. Otherwise, it is target of goto.
	...
	private static Instruction processISTORE(ByteCode[] bcs, ByteCode i, OperandStack stack,
			Vector vcode,
			boolean isStaticInitializer) {
	...
	private static Instruction processLSTORE(ByteCode[] bcs, ByteCode i, OperandStack stack,
			Vector vcode,
			boolean isStaticInitializer) {
		StoreLocal ji = (StoreLocal) i;
		Operand opr = stack.pop();
	...
	private static Instruction processFSTORE(ByteCode[] bcs, ByteCode i, OperandStack stack,
			Vector vcode,
			boolean isStaticInitializer) {
		StoreLocal ji = (StoreLocal) i;
		Operand opr = stack.pop();
	...
	private static Instruction processDSTORE(ByteCode[] bcs, ByteCode i, OperandStack stack,
			Vector vcode,
			boolean isStaticInitializer) {
		StoreLocal ji = (StoreLocal) i;
		Operand opr = stack.pop();
	...
	private static Instruction processASTORE(ByteCode[] bcs, ByteCode i, OperandStack stack,
			Vector vcode,
			boolean isStaticInitializer) {
		StoreLocal ji = (StoreLocal) i;
		int regIndex = -1;
		...
		if (regIndex != -1) {
			...
			if (setExceptionHandler(currentExceptionHandlers, isTargetOfGoto[indexOfCurrentBytecode],
									target.index)) {
	...
	public static Instruction convert(ByteCode[] bcs, ByteCode i, OperandStack stack, Vector vcode,
		...
		switch (op) {
			...
			case SALOAD: { //53
				...
				OperandReg regIndex = (OperandReg) GenerateInstruction.promoteOperand(vcode, index, opr_regI,
									  lineOfPC);
			...
			case GETSTATIC: { //178
				...
				int index = GlobalConstantPool.putStaticField(ji.className,
			...
			case INVOKEINTERFACE: { //185
				...
				Call call = (Call) GenerateInstruction.newInstruction(vcode,
	...
	public static TCException[] updatePCsOfExceptionHandler(TCException[] tces) throws Exception {
		if (tces != null) {
			...
			for (int j = 0; j < tces.length; j++) {
				...
				e.endPC = pc + newInstructionsCount(newInsts, 0,
													pc) - 1; // sub "-1" because the endPC is exclusive ==> interval [startPC, endPC)
