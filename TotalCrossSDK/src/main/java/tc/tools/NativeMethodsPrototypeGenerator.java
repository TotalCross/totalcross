...
/**
 * Created by guich to help the implementation of native methods in the
 * TotalCross vm.
 */
...
public class NativeMethodsPrototypeGenerator
{
  ...
  public static void main(String[] argv)
  {
    try {
      if (argv.length < 1) { // guich@330_10
        System.out.println(
          "Format: NativeMethodsPrototypeGenerator <input-file> <testcase>");
        System.out.println("<input-file>: the file to be parsed");
        System.out.println(
          "<testcase>: if true, creates the stubs for the test cases");
      } else {
        ...
        if (argv[0].equals("makeNativeHT")) {
          ...
          makeTestCases =
            argv.length >= 2 && (Boolean.valueOf(argv[1]).booleanValue() ||
                                 argv[1].equalsIgnoreCase("yes"));
        }
        int n = v.size();
        if (n == 0) {
        ...
        for (int i = 0; i < n; i++) {
          ...
          if (i4d >= 0 && !Character.isLetter(line.charAt(i4d + 2))) {
            throw new Exception("Invalid signature with 4D suffix detected: " +
                                line);
          }
  ...
  private static Hashtable<String, String> htNames =
    new Hashtable<String, String>(1023);
  ...
  //////////////////////////////////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////////////////
  private static Hashtable<String, String> htTypes =
    new Hashtable<String, String>(31);
  static
  {
    htTypes.put("int[", "[I");
    htTypes.put("long[", "[J");
    ...
    htTypes.put("object",
                "Ljava/lang/Object;"); // guich@200b4: include the object class
  ...
  //////////////////////////////////////////////////////////////////////////////////
  private static void createFunctionPrototype(String funcName,
                                              String originalFuncDesc)
  {
    String s = "";
    ...
    iosarray.addElement("htPutPtr(&htNativeProcAddresses, hashCode(\"" +
                        funcName + "\"), &" + funcName + ");\n");
   ...
   * in objects as parameters, you MUST suply the whole class method (eg:
   * totalcross.ui.gfx.Rect) the only exceptions are String, that it recognizes
   * alone (no need to specify the package) and the same class name, which it
   * prepends the package. specify if the method is private static or not so the
   * skeleton of the implementation prototype can be built correctly. If you
   * need to specify the function name to avoid conflicts, use this method.
   * Otherwise, to get the function name automatically generated, use the method
   * parseNative(className,method) defined above.
   */
  private static void parseNative(String className, String method)
  {
    ...
    StringTokenizer st =
      new StringTokenizer(method, " (),;]"); // DONT ADD '[' !
    String methodName = "";
    String methodDesc = "(";
    String methodRet = "";
    // parse the return type
    String tok = st.nextToken();
    while (tok.equalsIgnoreCase("private") || tok.equalsIgnoreCase("public") ||
           tok.equalsIgnoreCase("protected") ||
           tok.equalsIgnoreCase("static") || tok.equalsIgnoreCase("native") ||
           tok.equalsIgnoreCase("final")) {
      tok = st.nextToken();
    }
    if (tok.indexOf(".") != -1) { // does it have a package?
      ...
      if (methodRet == null) {
        System.out.println(errorCount++ + " - unrecognized return token: " +
                           tok + "    in " + method + " (" + className + ")");
      }
    }
    ...
    while (st.hasMoreTokens()) {
      ...
      if (count == 1 && htTypes.get(tok.toLowerCase()) == null &&
          methodName.toLowerCase().startsWith(
            tok.toLowerCase())) { // guich@200b4: is the first parameter the
                                  // class name? windowXXXXX(Window yyyy)
      ...
      if (tok.indexOf('.') != -1) { // does it have a package?
        ...
        methodDesc += last;
                 ...
                 -1) { // is (the last parameter) an array? case int []a
        if (tok.indexOf("[") > 0) {
          System.out.println(
            errorCount++ +
            " - Please write arrays like 'char []c' instead of 'char[] c'. Error in " +
            method);
          break;
        }
        int len = last.length();
        ...
        methodDesc =
          new String(methodDesc.toCharArray(), 0, methodDesc.length() - len);
        methodDesc += "[" + last;
      } else {
        String s = (String)htTypes.get(tok.toLowerCase());
        if (s != null && s.length() > 0 &&
            !s.equals("V")) { // void is valid only in return
          last = s;
          methodDesc += s;
        } else if (Character.isUpperCase(tok.charAt(0))) {
          System.out.println(errorCount++ + " - unrecognized token: " + tok +
    ...
    // java/lang/StringBuffer|StringBuffer append(char []str, int offset, int
    // len)
    // -> jls_append_cii
    String functionName =
      createMethodSignature(className, methodName, methodDesc);

    // check if there's already another function with this signature
    String lastfn = (String)htNames.get(functionName);
    if (lastfn != null) {
      System.out.println(errorCount++ + " - duplicate function name " +
                         functionName + ": \n   " + lastfn + "\n   " +
                         className + " " + method);
    } else {
  ...
  private static String createMethodSignature(String className,
                                              String methodName,
                                              String d)
  {
  ...
  private static void appendFirstLetter(StringBuffer fn, String[] pack)
  {
    ...
    for (i = 0; i < pack.length; i++) { // take the first letter of each name
      ...
      n = s.length();
      if (i < pack.length - 1) {
        fn.append(Character.toLowerCase(s.charAt(
          0))); // for the middle package names, use only the first letter
      } else {
        for (int j = 0; j < n; j++) {
          c = s.charAt(j);
          if (('A' <= c && c <= 'Z') || ('0' <= c && c <= '9')) {
      ...
      if (fos == null) {
        if (inputFilePath != null) {
          fos = new FileOutputStream(
            new File(new File(inputFilePath).getParentFile(),
    ...
    if (errorCount > 0) {
      ...
      if (makeNativeHT) {
        FileWriter fw = new FileWriter(new File(
          nativeHTPath, "nativeProcAddresses" + nativeHTSuffix + ".c"));
        ...
        System.out.println(
          "\n\noutput sent to " + System.getProperty("user.dir") +
          System.getProperty("file.separator") + "NativeMethodsPrototypes.txt");
      }
    }
    System.exit(0);
  }
