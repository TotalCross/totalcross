...
public class NativeMethodsPrototypeGenerator {
		...
		while (st.hasMoreTokens()) {
			...
			if (count == 1 && htTypes.get(tok.toLowerCase()) == null
				&& methodName.toLowerCase().startsWith(
					tok.toLowerCase())) { // guich@200b4: is the first parameter the class name? windowXXXXX(Window yyyy)
		...
		try {
			if (fos == null) {
				if (inputFilePath != null) {
					fos = new FileOutputStream(new File(new File(inputFilePath).getParentFile(),
	...
	private static void go() throws Exception {
		if (errorCount > 0) {
			...
			// ios prototypes
			if (makeNativeHT) {
				FileWriter fw = new FileWriter(new File(nativeHTPath,
														"nativeProcAddresses" + nativeHTSuffix + ".c"));
