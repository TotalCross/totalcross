...
final class CommandLineParser {
  ...
  static LaunchOptions parse(SimulatorConfiguration config, boolean application, int fallbackWidth, int fallbackHeight)
      throws InvalidArgumentException {
    String[] args = config.getLauncherArgs();
    LaunchOptions result = new LaunchOptions(config.getMainWindowClass());
    int n = args.length;
    int i = 0;
    try {
      for (i = 0; i < n; i++) {
  ...
  private static void parseScreen(LaunchOptions result, String value) throws Exception {
