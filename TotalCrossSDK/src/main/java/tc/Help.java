...
public class Help extends MainWindow {
	...
	private void reload(boolean load) {
		try {
			...
			c0.add(new Label(x("Helper application to Run/Deploy",
							   "Aplicação de ajuda para Executar / Empacotar"), CENTER,
							 Color.BLACK, true), LEFT, AFTER + 50, ic);

			c0.add(btHtml = new ShadedButton("Javadocs", fimg, Color.WHITE, Orange), RIGHT, TOP,
				   PARENTSIZE + 30,
				   PREFERRED + 50);
			...
			c0.add(btPdf = new ShadedButton("Companion", fimg, Color.WHITE, Orange), SAME, AFTER + 50, SAME,
				   SAME);
			...
			tip(rEn, "Click here to set the user interface to English",
			...
			c0.add(btpath = new ShadedButton(x(" Select ", " Selecionar "), fimg, Color.BLACK, GREEN), RIGHT,
				   SAME);
			...
			switch (e.type) {
				case ControlEvent.PRESSED:
					if (e.target == btpath) {
						...
						java.awt.Desktop.getDesktop().open(new java.io.File(getTCPath() +
														   "/docs/TotalCross Companion.pdf"));
	...
	private boolean checkFilled() {
		...
		if (!error.isEmpty()) {
			Toast.show(
				x("The following fields must be filled: " + error,
	...
	class RunContainer extends ScrollContainer {
		...
		public void initUI() {
			...
			rdI.appId = 4;
			tip(rdI, "Width=640, height=960, bpp=24, scale=0.75",
			...
			bc2.add(cbSc = new ComboBox(new String[] { "0.25", "0.5", "0.75", "1", "2", "4", "6", "8" }), AFTER,
					SAME);
			tip(cbSc,
			...
			bc2.finish();
			c.finish();
			add(new Label(x("Command line to pass to application: ", "Linha de comandos para aplicação")),
				LEFT, AFTER + 50);
			...
			tip(chM, "Show the mouse position on window's title area",
				"Mostra a posição do mouse no título da janela");
			btRun = new ShadedButton(x("Run the application", "Executar a aplicação"), fimg, Color.WHITE,
									 COLOR);
	...
	class DepContainer extends ScrollContainer {
		...
		public void initUI() {
			...
			c.add(all = new Check(x("All the ones listed below", "Todas as listadas abaixo")), LEFT + 100,
				  AFTER);
			all.appId = 1;
			...
			add(btpath = new ShadedButton(x(" Select ", " Selecionar "), fimg, Color.BLACK, GREEN), RIGHT,
				SAME);
			...
			add(pack = new Check(x("Package the vm with application", "Empacota a VM com a aplica��o")),
				LEFT, AFTER + 50);
			tip(pack,
			...
			btDep = new ShadedButton(x("Deploy the application", "Empacotar a aplica��o"), fimg,
									 Color.WHITE, COLOR);
			...
			tip(btfol, "Opens the path where the packages were created",
			...
			switch (e.type) {
				case ControlEvent.PRESSED:
					if (e.target == all && all.isChecked()) {
						...
						selectPath(edpathd, x("Select the Mobile Provision's path",
