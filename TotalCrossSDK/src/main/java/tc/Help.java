...
public class Help extends MainWindow
{
  ...
  private void reload(boolean load)
  {
    try {
      ...
      c0.add(new Label(x("Helper application to Run/Deploy",
                         "Aplicação de ajuda para Executar / Empacotar"),
                       CENTER,
                       Color.BLACK,
                       true),
             LEFT,
      ...
      c0.add(btHtml = new ShadedButton("Javadocs", fimg, Color.WHITE, Orange),
             RIGHT,
             TOP,
             PARENTSIZE + 30,
             PREFERRED + 50);
      tip(btHtml,
          "Opens the html javadocs in your browser",
          "Abre os javadocs em html no seu navegador");
      c0.add(btBlog = new ShadedButton("Blog", fimg, Color.WHITE, Orange),
             SAME,
             AFTER + 50,
             SAME,
             SAME);
      ...
      tip(
        btBlog,
        "Opens the TotalCross blog, a very useful technical source",
        "Abre o blog do TotalCross, uma fonte extremamente útil de informações");
      c0.add(btPdf = new ShadedButton("Companion", fimg, Color.WHITE, Orange),
             SAME,
             AFTER + 50,
             SAME,
             SAME);
      ...
      tip(btPdf,
          "Opens the TotalCross Companion.pdf",
          "Abre o TotalCross Companion.pdf");
      ...
      tip(rPt,
          "Click here to set the user interface to Portuguese",
          "Clique aqui para mudar a linguagem para português");
      c0.add(rEn = new Radio("English", rgLang), BEFORE - 50, SAME);
      tip(rEn,
          "Click here to set the user interface to English",
          "Clique aqui para mudar a linguagem para inglês");
      c0.add(l = new Label(x("Language: ", "Linguagem: ")), BEFORE - 100, SAME);
      l.transparentBackground = rPt.transparentBackground =
        rEn.transparentBackground = true;

      Spinner.spinnerType = Spinner.IPHONE;
      c0.add(spin = new Spinner(),
             LEFT + 50,
             SAME - 50,
             FONTSIZE + 150,
             FONTSIZE + 150);
      ...
      c0.add(
        new Label(x("Class name: ", "Nome da classe: ")), LEFT, AFTER + 25);
      ...
      c0.add(l = new Label(x("Class folder: ", "Pasta dos .class: ")),
             LEFT,
             AFTER + 25);
      c0.add(btpath = new ShadedButton(
               x(" Select ", " Selecionar "), fimg, Color.BLACK, GREEN),
             RIGHT,
             SAME);
      tip(
        btpath,
        "Press this button to select the folder where the .class is located",
      ...
      tip(edpath,
          "Type the .class folder or press the Select button",
      ...
      tip(edkey,
          "Type the 24-characters registration key that you received by email",
      ...
      tc = new TabbedContainer(new String[] {
        x("Run", "Executar"), x("Deploy", "Empacotar"), "Console" });
      tc.useBorder2 = true;
  ...
  private void tip(Control c, String en, String pt)
  {
    ...
    c.addMouseListener(new MouseListener() {
      ...
      @Override public void mouseOut(MouseEvent e) { lstatus.setText(""); }

      @Override public void mouseIn(MouseEvent e) { lstatus.setText(s); }
    });
  }

  @Override public void onExit() { saveConfig(); }
        ...
        case ControlEvent.PRESSED:
          if (e.target == btpath) {
            if (edclass.getTrimmedLength() == 0) {
              Toast.show(
                x("Please type the class name before selecting the path",
                  "Digite o nome da classe antes de selecionar a pasta"),
                3000);
            } else {
              selectPath(edpath,
                         x("Select the .class' path",
                           "Selecione a pasta dos arquivos .class"));
            }
            ...
            java.awt.Desktop.getDesktop().open(
              new java.io.File(getTCPath() + "/docs/TotalCross Companion.pdf"));
  ...
  class Config
  {
    ...
    private String getKey()
    {
      return key.startsWith("%")
        ? System.getenv(key.substring(1, key.length() - 1))
        : key;
    }
    ...
    private void deploy()
    {
      try {
        fromUI();
        // to run: classname=tc.samples.api.TotalCrossAPI, class
        // folder=G:\TotalCross\TotalCrossSDK to deploy: classname =
        // TotalCrossAPI, class
        // folder=G:\TotalCross\TotalCrossSDK\tc\samples\api
  ...
  public void exec(final String java,
    ...
    new Thread() { // thread is needed to let spin run
      @Override public void run()
      {
        try {
          ...
          Process process =
            Runtime.getRuntime().exec(cmd, null, new java.io.File(path));
  ...
  private void dump(java.io.InputStream inputStream,
                    java.io.InputStream errorStream) throws Exception
  {
    String lineIn;
    if (inputStream.available() > 0) {
      while (inputStream.available() > 0 &&
             (lineIn = readStream(inputStream)) != null) {
        println(lineIn);
      }
    }
    if (errorStream.available() > 0) {
      while (errorStream.available() > 0 &&
             (lineIn = readStream(errorStream)) != null) {
  ...
  private boolean checkFilled()
  {
    ...
    if (!error.isEmpty()) {
      Toast.show(x("The following fields must be filled: " + error,
                   "Os seguintes campos devem ser preenchidos: " + error),
                 4000);
    }
    boolean ok = error.isEmpty();
    if (ok) { // check if filename exists
      String fn = Convert.appendPath(edpath.getText(),
                                     edclass.getText().replace('.', '/'));
      try {
      ...
      if (!ok) {
        new MessageBox(
          "Error",
          x("The class folder + class name does not lead to an existing filename: " +
              fn +
            ...
            "A pasta da classe + nome da classe não resulta em um arquivo existente: " +
              fn +
  ...
  class ConsoleContainer extends ScrollContainer
  {
    ...
    public ConsoleContainer() { super(false, true); }
  ...
  class RunContainer extends ScrollContainer
  {
    ...
    public RunContainer() { super(false, true); }

    @Override public void initUI()
    {
      BorderContainer c =
        new BorderContainer(x("Screen settings", "Configurações da janela"));
      c.borderColor = COLOR;
      c.fillW = true;
      ...
      tip(
        edX,
        "Type the X position for the window. Leave blank to let the system set it",
      ...
      tip(
        edY,
        "Type the Y position for the window. Leave blank to let the system set it",
      ...
      rd32.appId = 1;
      tip(rd32,
          "Width=240, height=320, bpp=24",
          "Largura=240, altura=320, bpp=24");
      bc2.add(rdCE = new Radio("WinCE", rg), SAME, AFTER + 25);
      rdCE.appId = 2;
      tip(
        rdCE, "Width=240, height=320, bpp=8", "Largura=240, altura=320, bpp=8");
      bc2.add(rdA = new Radio("Android", rg), SAME, AFTER + 25);
      rdA.appId = 3;
      tip(rdA,
          "Width=320, height=480, bpp=24",
          "Largura=320, altura=480, bpp=24");
      bc2.add(rdI = new Radio("iOS", rg), SAME, AFTER + 25);
      rdI.appId = 4;
      tip(rdI,
          "Width=640, height=960, bpp=24, scale=0.75",
          "Largura=640, altura=960, bpp=24, escala=0.75");

      Label l1, l2, l3,
        l4; // store labels to make sure all fields are in the same X position
      bc2.add(
        l1 = new Label(x("Width: ", "Largura: ")), AFTER + 100, SAME, rdC);
      ...
      bc2.add(l3 = new Label(x("BitsPerPixel: ", "BitsPorPixel: ")),
              SAME,
              AFTER + 25,
              l2);
      bc2.add(cbBpp = new ComboBox(new String[] { "8", "16", "24", "32" }),
              AFTER,
              SAME);
      tip(
        cbBpp,
      ...
      bc2.add(l4 = new Label(x("Font size: ", "Tam da letra: ")),
              SAME,
              AFTER + 25,
              l3);
      bc2.add(edFS = new Edit("99999"), AFTER, SAME);
      tip(edFS,
          "Type the font size. Leave it blank to use the default one",
          "Digite o tamanho da fonte. Deixe vazio pra usar o tamanho padrão");

      bc2.add(new Label(x("Scale: ", "Escala: ")), SAME, AFTER + 25, l4);
      bc2.add(cbSc = new ComboBox(new String[] {
                "0.25", "0.5", "0.75", "1", "2", "4", "6", "8" }),
              AFTER,
              SAME);
      tip(
        cbSc,
      ...
      tip(
        swSc,
        "Scale slow has a good appearance, while fast scale has a worst appearance",
      ...
      bc2.finish();
      c.finish();
      add(new Label(x("Command line to pass to application: ",
                      "Linha de comandos para aplicação")),
          LEFT,
          AFTER + 50);
      add(edCmd = new Edit(), LEFT, AFTER);
      tip(
        edCmd,
        "You can pass extra arguments to application and retrieve it using MainWindow.getCommandLine()",
        "Você pode passar argumentos extras pra aplicação e recuperá-los usando MainWindow.getCommandLine()");
      add(chM = new Check(x("Show mouse position", "Mostrar posição do mouse")),
          LEFT,
          AFTER + 25);
      tip(chM,
          "Show the mouse position on window's title area",
          "Mostra a posição do mouse no título da janela");
      btRun = new ShadedButton(x("Run the application", "Executar a aplicação"),
                               fimg,
                               Color.WHITE,
                               COLOR);
      ...
      tip(
        btRun,
        "Fill the fields above and press this button to run the application",
  ...
  class DepContainer extends ScrollContainer
  {
    ...
    public DepContainer() { super(false, true); }
    ...
    @Override public void initUI()
    {
      ...
      c.add(all = new Check(
              x("All the ones listed below", "Todas as listadas abaixo")),
            LEFT + 100,
            AFTER);
      all.appId = 1;
      tip(all,
          "Deploys for all platforms",
          "Faz o deploy para todas as plataformas");
      ...
      w32.appId = 2;
      tip(w32,
          "Deploys for Windows 32 desktop",
          "Faz o deploy para Windows 32 desktop");
      ...
      apl.appId = 2;
      tip(apl,
          "Deploys for applet to run in a browser",
          "Faz o deploy para executar no navegador");
      b.finish();
      ...
      wmo.appId = 2;
      tip(wmo,
          "Deploys for Windows Mobile 5, 6 and 7",
          "Faz o deploy para Windows Mobile 5, 6 e 7");
      b2.add(and = new Check("Android"), SAME, BEFORE - 25);
      and.appId = 2;
      tip(and,
          "Deploys for Android 2.3 or greater",
          "Faz o deploy para Android 2.3 ou superior");
      b2.finish();
      c.finish();

      add(l = new Label(
            x("Mobile provision folder: ", "Pasta do mobile provision: ")),
          LEFT,
          AFTER + 50);
      add(btpath = new ShadedButton(
            x(" Select ", " Selecionar "), fimg, Color.BLACK, GREEN),
          RIGHT,
          SAME);
      tip(
        btpath,
        "Selects the folder where the iOS mobile provision is (required for iOS)",
      ...
      add(inst = new Check(x("Install on device (android)",
                             "Instalar no equipamento (android)")),
          LEFT,
          AFTER + 50);
      tip(
        inst,
        "Install the package if device is connected. You must only select android",
        "Instala o pacote se o equipamento estiver conectado. Voc� dever� selecionar somente android");

      add(pack = new Check(x("Package the vm with application",
                             "Empacota a VM com a aplica��o")),
          LEFT,
          AFTER + 50);
      tip(
        pack,
      ...
      btDep =
        new ShadedButton(x("Deploy the application", "Empacotar a aplica��o"),
                         fimg,
                         Color.WHITE,
                         COLOR);
      ...
      tip(
        btDep,
        "Fill the fields above and press this button to deploy the application",
        "Preencha os campos acima e clique nesse bot�o para empacotar a aplica��o");

      btfol = new ShadedButton(
        x("Install path", "Pasta da instala��o"), fimg, Color.WHITE, GREEN);
      btfol.setFont(font.asBold());
      btfol.setBackColor(0xAAFF00);
      add(btfol, RIGHT, SAME, PARENTSIZE + 30, PREFERRED + 50);
      tip(btfol,
          "Opens the path where the packages were created",
          "Abre a pasta onde est�o os pacotes para instalar");
    }
      ...
      switch (e.type) {
        case ControlEvent.PRESSED:
          if (e.target == all && all.isChecked()) {
            ...
            and.clear();
            wmo.clear();
          } else if (e.target instanceof Check &&
                     ((Check)e.target).appId == 2) {
            if (((Check)e.target).isChecked()) {
              all.clear();
            }
            if (e.target == ios && edpathd.getLength() == 0) {
              Toast.show(x("You must set the Mobile Provision path",
                           "Voc� deve especificar a pasta do Mobile Provision"),
                         3000);
            }
          } else if (e.target == btpath) {
            selectPath(edpathd,
                       x("Select the Mobile Provision's path",
                         "Selecione a pasta do Mobile Provision"));
          } else if (e.target == btDep && checkFilled()) {
            configs[sel].deploy();
          } else if (e.target == btfol && !edpath.getText().isEmpty()) {
            Vm.exec("viewer",
                    edpath.getText().replace('\\', '/') + "/install",
                    0,
                    true);
          }
          break;
  ...
  {
    FileChooserBox fb = new FileChooserBox(
      tit,
      new String[] { x("Select", "Selecionar"), x("Cancel", "Cancelar") },
      new FileChooserBox.Filter() {
        @Override public boolean accept(File f) throws IOException
        {
          return f.isDir();
        }
      });
