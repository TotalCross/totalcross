...
public class Help extends MainWindow
{
  ...
  class DepContainer extends ScrollContainer
  {
    ...
    public void initUI()
    {
      ...
      add(btpath = new ShadedButton(x(" Select ", " Selecionar "),fimg,Color.BLACK,GREEN), RIGHT,SAME); tip(btpath, "Selects the folder where the iOS mobile provision is (required for iOS)", "Seleciona a pasta onde est�o os arquivos do mobile provision (requerido para o iOS)");
      add(edpathd = new Edit(), AFTER,SAME,FIT-25,PREFERRED,l); 

      add(inst = new Check(x("Install on device (android or wp8)", "Instalar no equipamento (android ou wp8)")), LEFT,AFTER+50); tip(inst, "Install the package if device is connected. You must only select android or wp8","Instala o pacote se o equipamento estiver conectado. Voc� dever� selecionar somente android ou wp8");

      add(pack = new Check(x("Package the vm with application", "Empacota a VM com a aplica��o")),LEFT, AFTER+50);  tip(pack, "Inserts the VM inside the application's package (android and Windows Mobile); always true for other platforms", "Embute a VM no pacote da aplica��o (android e Windows Mobile); sempre embute para as outras plataformas");
      ...
      tip(btDep, "Fill the fields above and press this button to deploy the application", "Preencha os campos acima e clique nesse bot�o para empacotar a aplica��o");
        ...
        if (e.target == all && all.isChecked()) {w32.clear(); lin.clear(); apl.clear(); ios.clear(); and.clear(); wp8.clear(); wmo.clear(); }
          ...
          if (e.target instanceof Check && ((Check)e.target).appId == 2)
          {
            ...
            if (e.target == ios && edpathd.getLength() == 0) {
              Toast.show(x("You must set the Mobile Provision path", "Voc� deve especificar a pasta do Mobile Provision"), 3000);
            }
          }
          else
            if (e.target == btpath) {
