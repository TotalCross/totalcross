...
public class Calculator extends MainWindow {
  ...
  private String opers1[] = { "7", "8", "9", "/", "A", "B", "4", "5", "6", "*", "C", "D", "1", "2", "3", "-", "E", "F",
      "0", " +-", ".", "+", "=", "clr" };
  private String opers2[] = { "e^x", "Ln", "Log", "X^2", "X^3", "X^Y", "1/x", "Sqr", "N!", "Mod", "Rand", "Sin", "Cos",
      "Tan", "Int", "Shl", "Shr", "Rol", "Ror", "clr", "And", "Or", "Xor", "Not", "=" };
  ...
  public void initUI() {
    ...
    // add a menubar
    MenuItem col0[] = { new MenuItem(" File "), new MenuItem("Set precision"), new MenuItem(),
        miSaveHist = new MenuItem("Save history", false), new MenuItem("Clear history"), new MenuItem(),
        new MenuItem("Exit"), };
    MenuItem col1[] = { new MenuItem(" Edit "), new MenuItem("Copy"), new MenuItem("Paste"), };
    MenuItem col2[] = { new MenuItem("   ?   "), new MenuItem("Instructions"), new MenuItem("About"), };
    ...
    case ControlEvent.PRESSED: // anything pressed?
      try {
        ...
        if (event.target instanceof PushButtonGroup
            && (ind = ((PushButtonGroup) event.target).getSelectedIndex()) != -1) {
          char c;
        ...
        // base conversion ?
        if ((event.target == rbDec || event.target == rbHex || event.target == rbBin)
            && base != ((Control) event.target).appId) // appId stores the base that is represented by the control
        {
  ...
  private void handleMenuEvent(int item) {
    ...
    case 1: // set precision
      InputBox id = new InputBox("Set Precision", "Please enter the number\nof decimal places to be used\n(2 to 12):",
          "" + places);
  ...
  private String compute(String oper1, String oper2, char op) {
    ...
    try {
      ...
      if (op <= 14 && op != 8) {
        ...
        long op2 = oper2 == null ? 0
            : oper2.indexOf('.') != -1 ? (long) Convert.toDouble(oper2) : Convert.toLong(oper2, base);
        switch ((int) op) {
        case 15: // Shl
          r = op1 << 1;
          break;
