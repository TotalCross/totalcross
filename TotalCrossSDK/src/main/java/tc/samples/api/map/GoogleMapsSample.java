...
public class GoogleMapsSample extends BaseContainer {
  ...
  public void initUI() {
    try {
      ...
      sc.add(r = new Radio(" TotalCross MGP locatoin", rg), LEFT + g, AFTER + g, FILL, PREFERRED);
      r.leftJustify = true;
      sc.add(new Ruler(), LEFT + g, AFTER + g, FILL - g, PREFERRED);
      sc.add(r = new Radio(" Current location", rg), LEFT + g, AFTER + g, FILL, PREFERRED);
      r.leftJustify = true;
      sc.add(chGPS = new Check("Use GPS if activated."), LEFT + 2 * g, AFTER + g, FILL, PREFERRED);
      sc.add(r = new Radio(" Address", rg), LEFT + g, AFTER + 2 * g, FILL, PREFERRED);
      r.leftJustify = true;
      ...
      sc.add(r = new Radio(" Coordinates", rg), LEFT + g, AFTER + 2 * g, FILL, PREFERRED);
      r.leftJustify = true;
      ...
      chWaze.addPressListener(new PressListener() {
        ...
        public void controlPressed(ControlEvent e) {
          if (chWaze.isChecked()) {
            Toast.show("The address you specify will be used as the destination address. Dont use \"To (optional)\".",
                3000);
          }
        }
      });
    ...
    try {
      ...
      case ControlEvent.PRESSED:
        if (e.target instanceof Radio) {
          enableControls();
        } else if (e.target == mbgps && mbgps.getPressedButtonIndex() == 0) {
          ...
          case 0:
            if (!isAndroid) {
              GoogleMaps.showAddress(toLatLon("Av Norte 2920, Luciano Cavalcante, Fortaleza, CE, Brasil"),
                  chSat.isChecked());
            } else {
              // text
          ...
          case 1:
            ...
            if (chGPS.isChecked()) {
              ...
              try {
                ...
                mbgps.unpop();
                mbgps = null;
                MessageBox mb = new MessageBox("Error",
                    "An error occured while activating GPS: " + ioe.getMessage() + ".", null);
                mb.popupNonBlocking();
                Vm.safeSleep(2000);
                mb.unpop();
              } finally {
          ...
          if (addr != null) {
            ...
            try {
              ...
              if (chWaze.isChecked() || (to != null && to.length() > 0)) {
                ok = totalcross.map.GoogleMaps.showRoute(addr, to, null,
                    (chSat.isChecked() ? GoogleMaps.SHOW_SATELLITE_PHOTOS : 0)
                        | (chWaze.isChecked() ? GoogleMaps.USE_WAZE : 0));
              } else {
