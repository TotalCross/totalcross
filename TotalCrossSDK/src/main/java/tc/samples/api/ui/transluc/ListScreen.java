...
public class ListScreen extends TranslucentBaseContainer {
  ...
  public static ListScreen getInstance() {
    return inst == null ? inst = new ListScreen() : inst;
  }
  ...
  public void reload() {
    ...
    try {
      for (int i = 0, n = bil.arqs.length; i < n;) {
        clist.add(new Product(bil.new DynImage(i), "Cushion " + i, 100 + i), PARENTSIZE + 25, AFTER + 50,
            PARENTSIZE + 46, PREFERRED);
        i++;
        if (i < n) {
