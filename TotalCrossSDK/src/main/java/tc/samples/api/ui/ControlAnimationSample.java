...
public class ControlAnimationSample extends BaseContainer {
  ...
  public void initUI() {
    ...
    add(btnSwp = new Button("SWAP"), LEFT, TOP + fmH / 4);
    btnSwp.appId = 5;
    add(btnSH = new Button("SHOW/HIDE"), CENTER, TOP + fmH / 4);
    btnSH.appId = 6;
    add(btnRot = new Button("ROTATE"), RIGHT, TOP + fmH / 4);
    btnRot.appId = 7;
  ...
  public void onEvent(Event e) {
    if (e.type == ControlEvent.PRESSED) {
      ...
      case 5: {
        ...
        PathAnimation.create(btnR, xr, yr, xl, yl, null, -1).then(PathAnimation.create(btnR, xl, yl, xr, yr, null, -1))
            .start();
        PathAnimation.create(btnL, xl, yl, xr, yr, null, -1).then(PathAnimation.create(btnL, xr, yr, xl, yl, null, -1))
            .start();
        break;
      }
      case 6:
        (FadeAnimation.create(c, false, null, -1).then(FadeAnimation.create(c, true, null, -1))).start();
        break;
      case 7: {
        int xr = btnR.getX(), yr = btnR.getY(), xl = btnL.getX(), yl = btnL.getY();
