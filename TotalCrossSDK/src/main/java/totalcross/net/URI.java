...
/**
 * An URI represents a Uniform Resource Identifier, or an address of something
 * in the Internet.
 *
...
public class URI
{
  ...
  static private final char[] noEncode = { '\u0000', '\u0000', '\u6401',
                                           '\u03ff', '\ufffe', '\u87ff',
                                           '\ufffe', '\u07ff' };
  ...
  /**
   * Constructor from a byte array, and a base URI missing components derive
   * from.
   *
   * @param spec byte array that holds the specified URI
   * @param start where the URI starts in the byte array
                    ...
                    boolean needCopy)
  {
    ...
    // Find out if the spec URI is relative. This should just be: | "if ((scheme
    // == null) && (baseURI != null))", | however, if the scheme of the spec URI
    // matches the scheme of the | base URI, and the base URI is a hierarchical
    // URI scheme, | then maintain backwards compatibility and treat it as if |
    // the spec didn't contain the scheme; see 5.2.3 of RFC2396
    if ((baseURI != null) &&
        ((scheme == null) ||
         ((scheme.equalsIgnoreCase(baseURI.scheme)) && (baseURI.path != null) &&
          (baseURI.path.len > 0) &&
          (baseURI.path.base[baseURI.path.pos] == '/')))) {
    ...
    // Given the logic above, this section is hit when spec is one of: | - a
    // relative URI (no scheme) starting with
    // '/' or '?' or something else. | - an absolute URI with '/' or '?'
    // following the scheme | Strip off the query part
    for (i = start; i < end; ++i) {
      if (spec[i] == '?') {
    ...
    // Given the logic above, this section is hit when spec is one of: | - a
    // relative URI (no scheme) starting with
    // '/', | - an absolute URI with '/' following the scheme | It is an
    // absolute path. | Parse the authority part if any
    parseRelative();
    if (start == end) {
    ...
    if (b1 && b2) {
      ...
      if (host.len > 0) {
        ...
        if (host.base[host.pos] == '[') {
          // If the host is surrounded by [ and ] | then it is an IPv6 literal
          // address as specified in RFC2732.
          i = host.indexOf((byte)']', 0);
          if ((i < 0) ||
              ((++i < host.len) && (host.base[host.pos + i] != ':'))) {
            i = -1;
          }
        } else {
          i = host.indexOf((byte)':', 0);
  ...
  private boolean isAbsolute()
  {
    ...
    if ((end == start) || (spec[start] != '/')) {
      // Not a slash. | The spec is either an absolute URL with no path (just a
      // query), | or the spec is a relative URI. | Take care of the former. |
      // Then, resolve a relative-path according to RFC2396, 5.2.6. | Try to
      // remove the last segment from the Base URI path | and recompose as said
      // in RFC2396, 5.2.6a and 5.2.6b. | If the Base URI path has only one
      // segment (no '/'), | a leading slash is needed to end the authority. |
  ...
  {
    if (start == end) {
      // The path is empty, and the authority and query are undefined. | If the
      // scheme is also undefined, the RFC2396 at 5.2.2 implies | that the query
      // and fragment are inheritated from the baseURI | if undefined.
      if (isRelative) {
  ...
  private void stripURL()
  {
    ...
    if (((end - start) >= 4) && (spec[start + 3] == ':') &&
        ((spec[start] == 'u') || ((spec[start] == 'U'))) &&
  ...
  public static class Path extends ByteString
  {
    ...
    private static final byte[] sdds = { (byte)'/',
                                         (byte)'.',
                                         (byte)'.',
                                         (byte)'/' };
     ...
     */
    Path(int pos, int len, byte[] spec) { super(pos, len, spec); }
     ...
     */
    Path(Path src) { super(src.pos, src.len, src.base); }
    ...
    final void collapsePath()
    {
      ...
      for (int found = 0; (found = indexOf(sds, found)) >= 0;) {
        this.len -= 2;
        Vm.arrayCopy(
          base, pos + found + 2, base, pos + found, this.len - found);
      }
      ...
      for (int found = 0; (found = indexOf(sdds, found)) >= 0;) {
        for (int ppos = pos + found - 1;; --ppos) {
          ...
          if ((base[ppos] == (byte)'/') && (base[ppos + 1] != (byte)'.')) {
            Vm.arrayCopy(
              base, pos + found + 3, base, ppos, this.len - (found + 3));
            ppos -= pos; // relative
      ...
      if (base[this.len + pos - 1] == (byte)'.') {
        ...
        if ((ppos > pos) &&
            ((base[ppos] == (byte)'/') && (base[ppos + 1] == (byte)'.'))) {
          do {
            if (base[--ppos] == (byte)'/') {
              this.len = ppos - pos + 1;
              break;
