 ...
 * sockets can be used by the same application code just by passing it different
 * kinds of factories.
 *
 ...
 * java.net.Socket, so that they can directly expose new APIs for features such
 * as compression, security, record marking, statistics collection, or firewall
 * tunneling.
 * </UL>
 *
 * Factory classes are specified by environment-specific configuration
 * mechanisms. For example, the getDefault method could return a factory that
 * was appropriate for a particular user or applet, and a framework could use a
