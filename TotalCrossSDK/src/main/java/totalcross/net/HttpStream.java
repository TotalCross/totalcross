...
public class HttpStream extends Stream {
	 ...
	 * @see Options
	 */
	public HttpStream(URI uri, Options options) throws totalcross.net.UnknownHostException,
	...
	public static class Options {
		...
		public Hashtable requestHeaders = new Hashtable(
			13); // flsobral@tc110_104: hash table for request headers.
		/**
		...
		public Options() {
			requestHeaders.put("User-Agent",
							   Settings.platform + " / " +
							   Settings.deviceId); // flsobral@tc110_104: this is a request header, not a post header.
		...
		public void setContent(Multipart
							   multipart) { //flsobral@tc125_37: created the method setContent which receives a MIME multipart to be used as the HTTP POST data.
			if (partContent == null) {
	...
	private static final byte[] contentEncodingFieldName =
		"Content-Encoding:".getBytes(); // // flsobral@tc110_102: content encoding support.
	...
	private static final ByteString bsContentEncodingFieldName = new ByteString(0,
			contentEncodingFieldName.length,
			contentEncodingFieldName); // flsobral@tc110_102: content encoding support.
	private static final ByteString bsContentTypeFieldName = new ByteString(0,
			contentTypeFieldName.length,
			contentTypeFieldName);
	private static final ByteString bsContentLengthFieldName = new ByteString(0,
			contentLengthFieldName.length,
			contentLengthFieldName);
	private static final ByteString bsConnectionFieldName = new ByteString(0,
			connectionFieldName.length,
			connectionFieldName);
	private static final ByteString bsCookiesFieldName = new ByteString(0, cookiesFieldName.length,
			cookiesFieldName);
	private static final ByteString bsLocationFieldName = new ByteString(0, locationFieldName.length,
			locationFieldName);
	...
	public boolean
	badResponseCode; // flsobral@tc115_65: Must be an instance field, otherwise the HttpStream will always return ok.
	...
	public int writeBytes(byte buf[], int start, int count) throws totalcross.io.IOException {
		...
		while (bytesLeft > 0) {
			int ret = socket.writeBytes(buf, startPos,
	 ...
	 */
	protected void init(URI uri, Options options) throws totalcross.net.UnknownHostException,
		totalcross.io.IOException {
		int port;
		String strUri;
	...
	private void getResponse(Options options) throws totalcross.io.IOException {
		...
		String serverPath = useProxy ? uri.toString() :
							uri.path.toString(); //flsobral@tc126: had problems with a proxy that would only reply if we send the whole url on the post/get line.
		...
		if (!options.requestHeaders.exists("Host")) {
			options.requestHeaders.put("Host",
									   uri.host != null ? uri.host.toString() :
									   ""); //flsobral@tc126: Host must always be provided, empty if not available. // guich@570_32: check if its already set
		}
		...
		options.postHeaders.dumpKeysValues(sb, ": ",
										   Convert.CRLF); //flsobral@tc126: send post headers also on GET. Temporary fix to fix proxy and authorization support for both GET and POST.
		sb.append(Convert.CRLF);
		...
		if (options.partContent == null) {
			writeResponseRequest(sb,
								 options); //flsobral@tc120_17: fixed bug with HttpStream connection over BIS transport on BlackBerry.
		} else {
			byte[] bytes = cc.chars2bytes(sb.toString().toCharArray(), 0, sb.length());
			writeBytes(bytes, 0, bytes.length);
			try {
				options.partContent.writeTo(
					socket); //flsobral@tc125_XX: added support to set a MIME part as the POST data.
		...
		if (state != 6 && Settings.onJavaSE) {
			Vm.debug("HTTP: " +
					 getStatus()); // flsobral@tc110_95: No longer stop reading the header when a bad response code is found, so we can get the error cause.
		}
	}
	...
	protected void writeResponseRequest(StringBuffer sb,
										Options options) throws totalcross.io.IOException {
		byte[] bytes = cc.chars2bytes(sb.toString().toCharArray(), 0, sb.length());
		writeBytes(bytes, 0, bytes.length);
		bytes = null;
	...
	public String getStatus() {
		switch (state) {
			...
			case 6:
				return (socket != null) ? "OK." :
					   "Socket closed"; // guich@553_12: is nonsense return "Ok" if the socket is closed
			case 7:
	 ...
	 */
	public Image makeImage() throws ImageException,
		IOException { // flsobral@tc100: throw the specific exceptions instead of the generic Exception
	...
	void setFields() {
		...
		if ((contentType == UNKNOWN_TYPE)
			...
			start += bsConnectionFieldName.len;
				   ...
				   && bsCookiesFieldName.equalsIgnoreCase(buffer, ofsStart, bsCookiesFieldName.len)) {
			type = 2;
			...
			start += bsContentLengthFieldName.len;
				   ...
				   && bsLocationFieldName.equalsIgnoreCase(buffer, ofsStart, bsLocationFieldName.len)) {
			type = 4;
		...
		switch (type) {
			...
			case 4:
				...
				break;
			case 5:
				contentEncoding = new String(buffer, start,
