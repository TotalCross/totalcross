 ...
 * It starts reading (in a buffer) at the message-body. If you are having read
 * problems, try to increase the timeout with Options.readtimeout.
 *
 * <p>
 * Here is an example showing a rest consumer class managing data from an
 * restful webservice:
 *
 * <pre>
 * import totalcross.io.IOException;
 ...
 *
 * 		String getString =
 * "https://jsonplaceholder.typicode.com/posts/1"; String postString =
 * "https://jsonplaceholder.typicode.com/posts"; String putString =
 * "https://jsonplaceholder.typicode.com/posts/1"; String deleteString =
 * "https://jsonplaceholder.typicode.com/posts/1"; String patchString =
 * "https://jsonplaceholder.typicode.com/posts/1";
 *
 * 		HttpStream httpStream		;
 * 		HttpStream.Options options = new HttpStream.Options();
 * 		options.socketFactory = new SSLSocketFactory(); // In case https
 ...
 * 		options.data = "{\"title\": \"foo\", \"body\": \"bar\",
 * \"userId\": 1}";
 *
 ...
 * 		options.data = "{\"id\": 1, \"title\": \"new title\", \"body\":
 * \"bar\", \"userId\": 1}"; httpStream = new HttpStream(new URI(putString),
 * options); printResponse(httpStream);
 *
 * 		Vm.debug("=============  PATCH  ==============");
 *
 ...
 *
 * See also the <a
 * href="https://github.com/TotalCross/tc-utilities/blob/master/src/main/java/com/tc/utils/utilities/io/HttpConn.java">HttpConn</a>
 * wrapper and the <a
 * href="https://github.com/TotalCross/tc-utilities/blob/master/src/main/java/com/tc/utils/utilities/io/HttpMethod.java">enumeration
 * of common HTTP methods</a>. HttpConn abstracts dozens of stuff and is totally
...
public class HttpStream extends Stream
{
   ...
   * @see Options
   */
  public HttpStream(URI uri, Options options)
    throws totalcross.net.UnknownHostException, totalcross.io.IOException
  {
    init(uri, options);
  }
  ...
  public String
    contentEncoding; // flsobral@tc110_102: Content encoding support.
  /**
   * READ-ONLY the size of the returned data (-1 if unknown). Used in the
   * response.
   */
  public int contentLength;
  /**
   * READ-ONLY number of bytes read from the response's content. Initialized
   * with 0 and incremented whenever the readBytes method is executed.
   */
  public int contentRead; // flsobral@tc110_103: May be useful when content
  ...
  /**
   * This Hashtable contains all headers that came in the response that don't
   * belong to the public fields.
  ...
  public static class Options
  {
    ...
    public Hashtable requestHeaders =
      new Hashtable(13); // flsobral@tc110_104: hash table for request headers.
    /**
     * Set this with your post commands to send a post. The final data string
     * will be the concatenation of postPrefix+postDataSB+postData+postSuffix.
     * @see #postPrefix
     * @see #postSuffix
     * @see #postDataSB
     */
    @Deprecated public String postData;
    /**
     * Set this with your post commands to send a post. The final data string
     * will be the concatenation of postPrefix+postDataSB+postData+postSuffix.
     * @see #postPrefix
     ...
     */
    @Deprecated public StringBuffer postDataSB;
    /**
     * Set this with your post commands to send a post. The final data string
     * will be the concatenation of postPrefix+postDataSB+postData+postSuffix.
     * @see #postSuffix
     ...
     * @since TotalCross 1.23
     */
    @Deprecated public String postPrefix; // guich@tc123_39
    /**
     * Set this with your post commands to send a post. The final data string
     * will be the concatenation of postPrefix+postDataSB+postData+postSuffix.
     * @see #postPrefix
     ...
     * @since TotalCross 1.23
     */
    @Deprecated public String postSuffix; // guich@tc123_39

    /**
     * Set this with your POST/PUT/PATCH/DELETE commands. The final data string
     * will be the concatenation of dataPrefix+dataSB+data+dataSuffix.
     * @see #dataPrefix
     * @see #postSuffix
     * @see #dataSB
     */
    ...
    /**
     * The write timeout. The default value is to be the same value of
     * readTimeOut.
     */
     ...
     */
    @Deprecated public boolean doGet = true;

    /**
       Set to true to issue a post instead of a get. Note that the doGet is
       automatically set to false. <br>Here's a sample code: <pre> String conn =
       hostUrl+"survey/RequestArticle.aspx"; HttpStream.Options options = new
       HttpStream.Options(); options.readTimeOut = 60000; // 1 minute
         options.writeTimeOut = 50000; // 50 seconds
         options.openTimeOut = 120000; // 2 minutes
         options.doPost = true;
         options.postData = "xml=<root><update login=\""+state.login+"\"
       password=\""+state.pass+"\" dayofsync=\""+state.dayOfSync+"\"/></root>";
         XmlReadableSocket stream = new XmlReadableSocket(new
       URI(conn),options);
         </pre>
         @deprecated Use httpType instead
     */
     ...
     * You can also define a custom type, like if you want to use restful
     * services. In this case, the header will be set to what you store in the
     * httpType String. Note that, to use another http method, append a space.
     *
     * <pre>
     * HttpStream.Options options = new HttpStream.Options();
    ...
    /**
     * Socket factory used to create the underlying connection. You may replace
     * it with a SSLSocketFactory to make a HTTPS connection over a secure
     * socket, or with your own subclass of SocketFactory.
     *
    ...
    /**
     * Constructs a new Options class, from where you change the behaviour of
     * an Http connection. Sets the <code>postHeaders</code> to: <pre>
     * Content-Type: application/x-www-form-urlencoded
     * User-Agent: platform / deviceId
     * </pre>
     * You can override these if you want, just put the new values in the
     * Hashtable or use other methods available in this class.
     */
    public Options()
    {
      requestHeaders.put(
        "User-Agent",
        Settings.platform + " / " +
          Settings.deviceId); // flsobral@tc110_104: this is a request
                              // header, not a post header.
     ...
     *
     * @param encoding
     *           the cjarset encoding to be set. Must be either "ISO-8859-1" or
     * "UTF-8".
     */
     ...
     */
    public String getCharsetEncoding() { return this.encoding; }

    /**
     * Replaces the default Content-Type
     * (<code>application/x-www-form-urlencoded</code>) by the given one.
     */
    ...
    { // guich@570_32
      postHeaders.put(
        "Cookie",
        cookies.dumpKeysValues(new StringBuffer(512), "=", "; ").toString());
    }
    ...
    /**
     * Encodes the given username and password in Base64 for basic proxy
     * authorization
     *
     * @param user
     *           the username for the proxy. Passing null disables proxy
     * authorization.
     * @param password
     *           the password for the proxy. Passing null disables proxy
     * authorization.
    ...
    public void setBasicProxyAuthorization(String user, String password)
    {
      if (user == null || password == null) {
        ...
        postHeaders.put("Proxy-Authorization",
                        createBasicAuthString(user, password));
      }
    }
     ...
     * This method may not be used with post data fields. These fields are
     * ignored by the HttpStream after this method is used.
     *
     * @param multipart the multipart to be set as POST data
     * @since TotalCross 1.25
     */
    public void setContent(Multipart multipart)
    { // flsobral@tc125_37: created the method
      // setContent which receives a MIME multipart to
      // be used as the HTTP POST data.
      if (partContent == null) {
    ...
    public boolean mustSendData() { return sendData; }
  }

  /**
   * This makes a sleep during the send of a file.
   * Important: when using softick, you must set this to 500(ms) or more, or
   * softick will starve to death.
   */
  public int sendSleep; // guich@568_6

  /**
   * The delimiter used when reading data using readTokens.
   * Once the readTokens method is called, changing the delimiter field will be
   * useless.
   * @since TotalCross 1.25
   * @see #readTokens
   */
  ...
  private static final byte[] contentEncodingFieldName =
    "Content-Encoding:"
      .getBytes(); // // flsobral@tc110_102: content encoding support.
  private static final byte[] contentTypeFieldName = "Content-Type:".getBytes();
  private static final byte[] contentLengthFieldName =
  ...
  private static final ByteString bsContentEncodingFieldName =
    new ByteString(0,
                   contentEncodingFieldName.length,
                   contentEncodingFieldName); // flsobral@tc110_102: content
                                              // encoding support.
  private static final ByteString bsContentTypeFieldName =
    new ByteString(0, contentTypeFieldName.length, contentTypeFieldName);
  private static final ByteString bsContentLengthFieldName =
    new ByteString(0, contentLengthFieldName.length, contentLengthFieldName);
  private static final ByteString bsConnectionFieldName =
    new ByteString(0, connectionFieldName.length, connectionFieldName);
  private static final ByteString bsCookiesFieldName =
    new ByteString(0, cookiesFieldName.length, cookiesFieldName);
  private static final ByteString bsLocationFieldName =
    new ByteString(0, locationFieldName.length, locationFieldName);
  ...
  public boolean
    badResponseCode; // flsobral@tc115_65: Must be an instance field,
                     // otherwise the HttpStream will always return ok.

  private AbstractCharacterConverter cc =
    (CharacterConverter)Convert.charsetForName("ISO-8859-1");
    ...
    throws totalcross.io.IOException
  {
    ...
    return (bytesRead == 0 && lastRead == IOUtils.EOF) ? IOUtils.EOF
                                                       : bytesRead;
  }
    ...
    throws totalcross.io.IOException
  {
    ...
    while (bytesLeft > 0) {
      int ret = socket.writeBytes(
        buf,
        startPos,
        (bytesLeft >= writeBytesSize ? writeBytesSize : bytesLeft));
      sentBytes += ret;
      bytesLeft -= ret;
      startPos += ret;
      if (sendSleep > 0) {
   ...
   */
  protected void init(URI uri, Options options)
    throws totalcross.net.UnknownHostException, totalcross.io.IOException
  {
    int port;
    String strUri;
    ...
    if (options.proxyAddress != null) { // hsoares@421_63
      ...
      if (uri.host == null) {
        throw new UnknownHostException(
          "Please prefix the host with http:// or the correct protocol");
      }
    ...
    socket =
      options.socketFactory.createSocket(strUri, port, options.openTimeOut);
    socket.readTimeout = options.readTimeOut;
    socket.writeTimeout =
      options.writeTimeOut == -1 ? options.readTimeOut : options.writeTimeOut;
  ...
  private void getResponse(Options options) throws totalcross.io.IOException
  {
    ...
    if (options.partContent != null) {
      ...
      if (contentType != null && contentType.indexOf("charset") != -1) {
        options.postHeaders.put("Content-Type",
    ...
    String serverPath =
      useProxy ? uri.toString()
               : uri.path.toString(); // flsobral@tc126: had problems with a
                                      // proxy that would only reply if we send
                                      // the whole url on the post/get line.
    ...
    if (!options.requestHeaders.exists("Host")) {
      options.requestHeaders.put(
        "Host",
        uri.host != null
          ? uri.host.toString()
          : ""); // flsobral@tc126: Host must always be provided, empty if
                 // not available. // guich@570_32: check if its already set
    }
    ...
    if (options.partContent == null && shouldSendData(options)) {
      ...
      String prefix =
        options.dataPrefix != null ? options.dataPrefix : options.postPrefix;
      String suffix =
        options.dataSuffix != null ? options.dataSuffix : options.postSuffix;
      String dataAux = options.data != null ? options.data : options.postData;
      StringBuffer dataSBAux =
        options.dataSB != null ? options.dataSB : options.postDataSB;
      ...
      if (dataSBAux != null) {
        len += cc.chars2bytes(
                   dataSBAux.toString().toCharArray(), 0, dataSBAux.length())
                 .length;
      }
      if (dataAux != null) {
        len +=
          cc.chars2bytes(dataAux.toCharArray(), 0, dataAux.length()).length;
      }
      if (suffix != null) {
    ...
    options.postHeaders.dumpKeysValues(
      sb,
      ": ",
      Convert.CRLF); // flsobral@tc126: send post headers also on GET.
                     // Temporary fix to fix proxy and authorization support
    ...
    if (options.partContent == null) {
      writeResponseRequest(
        sb,
        options); // flsobral@tc120_17: fixed bug with HttpStream connection
                  // over BIS transport on BlackBerry.
    } else {
      byte[] bytes =
        cc.chars2bytes(sb.toString().toCharArray(), 0, sb.length());
      writeBytes(bytes, 0, bytes.length);
      try {
        options.partContent.writeTo(
          socket); // flsobral@tc125_XX: added support to set a MIME part as
                   // the POST data.
    ...
    if (state != 6 && Settings.onJavaSE) {
      Vm.debug("HTTP: " +
               getStatus()); // flsobral@tc110_95: No longer stop reading the
                             // header when a bad response code is found, so we
                             // can get the error cause.
    }
  }
    ...
    throws totalcross.io.IOException
  {
    ...
    if (shouldSendData(options)) {
      ...
      // postPrefix, postSuffix, postData and postDataSB are deprecated but they
      // still work
      String prefix =
        options.dataPrefix != null ? options.dataPrefix : options.postPrefix;
      String suffix =
   ...
   *
   * @return true, if this HttpStream is functionning properly and the socket is
   *     open;
   *         false otherwise.
   */
  public boolean isOk()
  {
    return (state == 6 &&
            socket !=
              null); // guich@570_70: now it also depends on socket be open
  }
  ...
  public String getStatus()
  {
    switch (state) {
      ...
      case 6:
        return (socket != null)
          ? "OK."
          : "Socket closed"; // guich@553_12: is nonsense return "Ok" if the
                             // socket is closed
      case 7:
  ...
  { // flsobral@tc100: throw the specific exceptions
  ...
  void setFields()
  {
    ...
    if ((contentType == UNKNOWN_TYPE) &&
        bsContentTypeFieldName.equalsIgnoreCase(
          buffer, ofsStart, bsContentTypeFieldName.len)) {
      type = 0;
      start += bsContentTypeFieldName.len;
    } else if ((connection == null) &&
               bsConnectionFieldName.equalsIgnoreCase(
                 buffer, ofsStart, bsConnectionFieldName.len)) {
      type = 1;
      start += bsConnectionFieldName.len;
    } else if ((cookies == null) &&
               bsCookiesFieldName.equalsIgnoreCase(
                 buffer, ofsStart, bsCookiesFieldName.len)) {
      type = 2;
      start += bsCookiesFieldName.len;
    } else if ((contentLength == -1) &&
               bsContentLengthFieldName.equalsIgnoreCase(
                 buffer, ofsStart, bsContentLengthFieldName.len)) {
      type = 3;
      start += bsContentLengthFieldName.len;
    } else if ((location == null) &&
               bsLocationFieldName.equalsIgnoreCase(
                 buffer, ofsStart, bsLocationFieldName.len)) {
      type = 4;
      start += bsLocationFieldName.len;
    } else if ((contentEncoding == null) &&
               bsContentEncodingFieldName.equalsIgnoreCase(
                 buffer, ofsStart, bsContentEncodingFieldName.len)) {
    ...
    switch (type) {
      ...
      case 5:
        contentEncoding = new String(
          buffer,
          start,
          end - start); // flsobral@tc110_102: content encoding support.
        break;
    }
  }
   ...
   * Note that both readLine and readTokens cannot be used at the same time.
   * The delimiter must be set using the delimiter field, prior to using this
   * method.
   *
   * @return the read line or <code>null</code> if nothing was read.
   * @since TotalCross 1.25
  ...
  { // guich@tc125_16
    if (tr == null) {
      tr = new TokenReader(
        socket, readTokensDelimiter, buffer, ofsCur, ofsEnd - ofsCur);
