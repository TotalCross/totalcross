...
{
  protected POP3Folder(Store store) { super(store); }

  @Override public void open() throws MessagingException
  {
    if (messageCount != -1) {
      return;
    }
  ...
  @Override public Message getMessage(int msgNumber) throws MessagingException
  {
    if (messageCount == -1) {
      return null;
    }
    ...
    try {
      ...
      if (msgRet == null) {
        ...
        int msgSize =
          Convert.toInt(reply.substring(reply.lastIndexOf(' ') + 1)) + 3;
        int top = msgSize <= HEADER_BUFFER_SIZE ? msgSize : 0;
        store.connection.writeBytes("TOP " + msgNumber + " " + top +
                                    Convert.CRLF);
        reply = store.connection.readLine();
        ...
        try {
          do {
            ...
            bytesRead = store.connection.readBytes(
              msgHeaderBuffer, totalRead, msgHeaderBuffer.length - totalRead);
            if (bytesRead > 0) {
              totalRead += bytesRead;
            }
          } while (
            !new String(msgHeaderBuffer, totalRead - 5, 5).equals("\r\n.\r\n"));
        } catch (SocketTimeoutException e) {
          if (!new String(msgHeaderBuffer, totalRead - 5, 5)
                 .equals("\r\n.\r\n")) {
            throw new MessagingException(e);
          }
        }
        ...
        if (top > 0) {
          ...
          msgRet.parseContent(
            s.substring(msgIndex + 4)); // +4 to skip double CRLF
        } else {
  ...
  @Override public Message getMessage(String uidl) throws MessagingException
  {
    if (messageCount == -1) {
      return null;
    }
  ...
  @Override public void reset() throws MessagingException
  {
    try {
      String reply;
  ...
  @Override void deleteMessage(Message message) throws MessagingException
  {
    try {
      String reply;
