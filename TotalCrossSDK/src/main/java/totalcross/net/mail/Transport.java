...
{
  protected Transport(MailSession session) { super(session); }

  /**
   * Send a message. The message will be sent to all recipient addresses
   * specified in the message, using message transports appropriate to each
   * address, as specified on the static instance of MailSession.
   *
  ...
  public static void send(Message message)
    throws MessagingException, AuthenticationException
  {
   ...
   * Send a message. The message will be sent to all recipient addresses
   * specified in the message, using message transports appropriate to each
   * address, as specified on the given instance of MailSession.
   *
   * NOTE: Only SMTP transport is currently supported.
   *
   * @param message
    ...
    throws MessagingException, AuthenticationException
  {
    ...
    int connectionTimeout =
      ((Properties.Int)session.getNotNull(MailSession.SMTP_CONNECTIONTIMEOUT))
        .value;
    int timeout =
      ((Properties.Int)session.getNotNull(MailSession.SMTP_TIMEOUT)).value;
    boolean tlsEnabled =
      ((Properties.Boolean)session.getNotNull(MailSession.SMTP_STARTTLS)).value;
    ...
    try {
      ...
      if (!tlsEnabled) {
        ...
        Properties.Str sslSocketFactoryClass = (Properties.Str)session.get(
          MailSession.SMTP_SSL_SOCKET_FACTORY_CLASS);
        if (sslSocketFactoryClass != null &&
            sslSocketFactoryClass.value != null) {
          sf = (SocketFactory)Class.forName(sslSocketFactoryClass.value)
                 .newInstance();
        } else {
      ...
      SMTPTransport smtp =
        (SMTPTransport)session.getTransport(tlsEnabled ? "smtps" : "smtp");
