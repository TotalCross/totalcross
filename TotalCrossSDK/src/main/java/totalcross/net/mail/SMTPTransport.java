...
public class SMTPTransport extends Transport {
	...
	public void sendMessage(Message message) throws MessagingException {
		try {
			...
			connection.readTimeout =
				40000; //flsobral: some SMTP servers are really slow to reply the message terminator, so we wait a little longer here. This is NOT related to the device connection.
	...
	public void issueCommand(String cmd, int expect) throws MessagingException {
		...
		if (expect != -1 && responseCode != expect) {
			throw new MessagingException("Unexpected response code. Expected " + expect + ", but received " +
										 responseCode);
		}
	}
