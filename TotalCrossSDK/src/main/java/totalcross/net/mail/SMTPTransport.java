...
public class SMTPTransport extends Transport
{
  ...
  protected SMTPTransport(MailSession session) { super(session); }

  @Override
  protected void protocolConnect(String host,
                                 int port,
                                 String user,
                                 String password)
    throws AuthenticationException, MessagingException
  {
    ...
    switch (authSupported) {
      ...
      case 2: // auth with PLAIN
        ...
        issueCommand(
          "AUTH PLAIN " + Base64.encode(auth.getBytes()) + Convert.CRLF, 235);
        break;
  ...
  @Override public void sendMessage(Message message) throws MessagingException
  {
    try {
      // WRITE RETURN PATH
      Properties.Str smtpFrom =
        (Properties.Str)session.get(MailSession.SMTP_FROM);
      ...
      for (int i = message.recipients.size() - 1; i >= 0; i--) {
        issueCommand("RCPT TO:<" + ((String)message.recipients.items[i]) + ">" +
                       Convert.CRLF,
                     250);
      }
      // START DATA
      ...
      connection.readTimeout =
        40000; // flsobral: some SMTP servers are really slow to reply the
               // message terminator, so we wait a little longer here. This is
               // NOT related to the device connection.
  ...
  {
    boolean tlsEnabled =
      ((Properties.Boolean)session.get(MailSession.SMTP_STARTTLS)).value;
  ...
  public void issueCommand(String cmd, int expect) throws MessagingException
  {
    ...
    if (expect != -1 && responseCode != expect) {
      throw new MessagingException("Unexpected response code. Expected " +
                                   expect + ", but received " + responseCode);
    }
  }
  ...
  public boolean getRequireStartTLS() { return requiresTLS; }
  ...
  /*
   * This method MUST ensure the service is completely invalidated and closed,
   * even when an exception is thrown. That's why each command is issued inside
   * a try/catch block. (non-Javadoc)
   *
  ...
  /**
   * Issue the STARTTLS command and switch the socket to TLS mode if it
   * succeeds.
   *
