...
public class Message extends Part
{
  ...
  /**
   * Package constructor used by subclasses of Folder to create a message
  ...
  @Override public void addHeader(String name, String value)
  {
    if (name.length() > 7) {
      ...
      throw new IllegalArgumentException();
                 ...
                 RecipientType.recipientPrefix[RecipientType.TO])) {
      throw new IllegalArgumentException();
    } else if (name.equalsIgnoreCase(
                 RecipientType.recipientPrefix[RecipientType.CC])) {
      throw new IllegalArgumentException();
    } else if (name.equalsIgnoreCase(
                 RecipientType.recipientPrefix[RecipientType.BCC])) {
      throw new IllegalArgumentException();
   ...
   * the person(s) who wished this message to be sent. In certain
   * implementations, this may be different from the entity that actually sent
   * the message.
   *
  ...
  @Override public void writeTo(Stream stream) throws MessagingException
  {
    try {
      // flsobral@tc123_49: moved SMTP commands from Message.writeTo to
      // SMTPTransport.sendMessage - Message.writeTo now correctly supports
      // writing to any type of stream.
      // FROM
      if (vFrom != null) {
      ...
      if (!vRecipients[RecipientType.TO].isEmpty()) {
        stream.writeBytes("To: " +
                          vRecipients[RecipientType.TO].toString(", "));
        stream.writeBytes(Convert.CRLF_BYTES);
      }
      if (!vRecipients[RecipientType.CC].isEmpty()) {
        stream.writeBytes("Cc: " +
                          vRecipients[RecipientType.CC].toString(", "));
        stream.writeBytes(Convert.CRLF_BYTES);
      }
      if (!vRecipients[RecipientType.BCC].isEmpty()) {
        stream.writeBytes("Bcc: " +
                          vRecipients[RecipientType.BCC].toString(", "));
        stream.writeBytes(Convert.CRLF_BYTES);
      }
   ...
   * new message object will be empty, that is, it will not have a "content".
   * These will have to be suitably filled in by the client.
   *
   * If replyToAll is set, the new Message will be addressed to all recipients
   * of this message. Otherwise, the reply will be addressed to only the sender
   * of this message.
   *
   ...
   *
   * NOTE: The replyToAll feature is NOT supported yet, messages returned by
   * this method will contain only the original sender as recipient.
   *
   * @param replyToAll
  ...
  public Message reply(boolean replyToAll) throws AddressException
  {
    ...
    reply.setFrom(new Address(
      MailSession.getDefaultInstance().get(MailSession.SMTP_USER).toString(),
      null));
    ...
    if (result.startsWith("=?iso-8859-1?Q?")) {
      ...
      while ((end = encoded.indexOf('=', start)) != -1) {
        ...
        sb.append(new String(
          Convert.hexStringToBytes(encoded.substring(end + 1, start))));
      }
  ...
  private void parseRecipient(int type, String[] tokens) throws AddressException
  {
    ...
    for (int i = 0; i < len; i++) {
      if (tokens[i].length() > 0) {
        ...
        if (start != -1) {
          String address =
            tokens[i].substring(start + 1, tokens[i].indexOf('>'));
