...
/**
 * This class represents a MIME body part, which are contained by Multipart
 * objects.
 *
...
public class Part
{
  ...
  /**
   * Defines if this part's content should be inlined in the body of the
   * message, or attached as a file.<br> Its value MUST be set to either INLINE
   * (default value) or ATTACHMENT, using the constants defined by the class
   * Part.
   */
   ...
   * Content-Type could not be determined.<br>
   *
   ...
   * available, otherwise the content will be handled by the
   * BinaryContentHandler, which writes the content as a base 64 encoded
   * String.<br>
   *
  ...
  public void setContent(Object content, String mimeType)
  {
    if (content instanceof Part) {
      throw new IllegalArgumentException(
        "A body part cannot be set as content of another body part");
    }
  ...
  public void setContent(Multipart multipart)
  {
    ...
    contentHandler = null;
    this.mimeType = MULTIPART + multipart.subType + "; boundary=\"" +
                    new String(multipart.boundary) + "\"";
  }

  /**
   * A convenience method for setting this part's content based on the type of
   * the passed object.<br> If the given object type is Multipart, it's MIME
   * type is set to "multipart/mixed".<br> If the given object type is String,
   * it's MIME type is set to "text/plain".<br> Otherwise, the given object will
   * be handled as "application/octet-stream" by the BinaryContentHandler.
   *
   * @param content
   *           this part's content
   * @throws IllegalArgumentException
   ...
   * aggregation of the Part attributes and an appropriately encoded byte stream
   * from its 'content'.
   *
   * Classes that extends the Part class decide on the appropriate encoding
   * algorithm to be used.
   *
   * The received Stream is typically used for sending.
   *
   * @param stream
   *           the output stream that will receive this part's encoded
   * representation
   * @throws IOException
  ...
  public void writeTo(Stream stream) throws IOException, MessagingException
  {
    addHeader("Content-Type",
              mimeType); // flsobral@tc125: It is not an error to use "type",
  ...
  {
    return content; // flsobral@tc124_26: now we return the content, regardless
                    // of its type.
  }
}
...
class PartHeaders
{
  ...
  PartHeaders() { headers = new Vector(); }
