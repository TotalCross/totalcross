...
public class DataHandler
{
  ...
  /**
   * May be used to add an user-defined DataContentHandler, associated to a
   * specific MIME type.
   *
   ...
   * @since TotalCross 1.13
   */
  public static void addDataContentHandler(
  ...
  /**
   * Retrieves the DataContentHandler associated with the given MIME type,
   * returning a BinaryContentHandler if the given MIME type is not mapped to
   * any handler.
   *
   * @param mimeType
   *           MIME type of the content that we need to be handled
   * @return the requested DataContentHandler, or a BinaryContentHandler if the
   *     given MIME type is not mapped to any
   *         handler.
   * @since TotalCross 1.13
   */
