...
/**
 * Implementation of DataContentHandler that handles MIME types handled as
 * base64 encoded byte arrays.<br> It also handle Streams, reading from the
 * input stream on demand to avoid excessive memory load. This will usually be
 * slower than reading the whole content of the stream to a byte array and using
 * it as the Part content.
 *
 * @since TotalCross 1.13
 */
public class BinaryContentHandler extends DataContentHandler
{
  static byte[] ContentTransferEncoding =
    ...
    throws IOException
  {
    ...
    String disposition =
      (fileName != null ? Part.ATTACHMENT : part.disposition);
  ...
  private void writeEncoded(Stream outputStream, Object input)
    throws IOException
  {
    byte[] inputBytes;
    if (input instanceof byte[]) {
      ...
      outputStream.writeBytes(Convert.CRLF_BYTES);
               ...
               Stream) { // flsobral@tc123_45: now we use ByteArrayStream with
                         // fixed length, which should GREATLY reduce the memory
                         // usage when processing large files.
      DataStream inputStream = (input instanceof DataStream)
                                 ? ((DataStream)input)
      ...
      outputStream.writeBytes(Convert.CRLF_BYTES);
    } else if (input != null) { // flsobral@tc122_40: handle unknown types using
                                // toString().getBytes()
