 ...
 * protocols.<br>
 *
 * The initial handshake on this connection is initiated by calling
 * startHandshake which explicitly begins handshakes. If handshaking fails for
...
public class SSLSocket extends Socket
{
  ...
  /**
   * Constructs an SSL connection to a named host at a specified port, with the
   * specified connection timeout, binding the client side of the connection a
   ...
   */
  public SSLSocket(String host, int port, int timeout)
  ...
  /**
   * Creates a new SSLClient to be used by this instance of SSLSocket during the
   * handshake. The default implementation does not perform any kind of
   * validation. Subclasses may override this method to use their own
   * implementation of SSLClient.
   *
   * @return a SSLClient initialized with the objects required to perform the
   *     validation for this socket.
  ...
  public void startHandshake() throws IOException
  {
    try {
      ...
      int status;
      for (int elapsedTime = 0; (status = sslConnection.handshakeStatus()) ==
                                  Constants.SSL_HANDSHAKE_IN_PROGRESS &&
                                elapsedTime < super.readTimeout;
           elapsedTime += 25) {
        Vm.sleep(25);
      }
  ...
  @Override public int readBytes(byte[] buf) throws IOException
  {
