...
public class SSL
{
  ...
  {
    synchronized (cache) { return (SSL)cache.get(s); }
  }
  ...
  protected SSL(Object ssl, Socket socket)
  {
    ...
    ((javax.net.ssl.SSLSocket)ssl)
      .addHandshakeCompletedListener(new HandshakeCompletedListener() {
        @Override public void handshakeCompleted(HandshakeCompletedEvent arg)
        {
   ...
   */
  final public int handshakeStatus() { return status; }
    ...
    if (ssl != null) {
      ...
      switch (component) {
        ...
        case Constants.SSL_X509_CA_CERT_ORGANIZATIONAL_NAME:
          principal = ((java.security.cert.X509Certificate)chain[0])
  ...
  final public int verifyCertificate() throws CryptoException
  {
    for (int i = 0; i < _trustMgrs.length; i++) {
      try {
        Certificate[] certs =
          ((javax.net.ssl.SSLSocket)ssl).getSession().getPeerCertificates();
        java.security.cert.X509Certificate[] _certs =
          new java.security.cert.X509Certificate[certs.length];
        for (int c = 0; c < certs.length; c++) {
          _certs[c] = (java.security.cert.X509Certificate)certs[c];
        }

        javax.net.ssl.X509TrustManager x509_tm =
          (javax.net.ssl.X509TrustManager)_trustMgrs[i];
   ...
   */
  final public Exception getLastException() { return lastException; }
}
