...
public class SSLCTX
{
   ...
   * could not be found.
   */
  final public SSL find(Socket s) { return SSL.cacheGetSSL(s); }

  /**
   * Load security material (CA, Cert or private key) in binary DER or ASCII PEM
   * format. These information are used during the SSL protocol to
   * authentication of SSL parts and cyphering/uncyphering of the data exchanged
   * between the two parts.
   * @param obj_type [in] The format of the file. Can be one of:
   * - SSL_OBJ_X509_CERT (no password required).
   * - SSL_OBJ_X509_CACERT (no password required).
   * - SSL_OBJ_RSA_KEY (AES128/AES256 PEM encryption supported). (not supported
   * on Desktop)
   * - SSL_OBJ_P8 (RC4-128 encrypted data supported). (password protection not
   * supported on Desktop)
   * - SSL_OBJ_P12 (RC4-128 encrypted data supported).
   * PEM encoded files are automatically detected and may contain several
   * material, whereas DER encoding only support one single material.
   ...
   * @throws NoSuchAlgorithmException
   */
  final public int objLoad(int obj_type,
                           totalcross.io.Stream material,
                           String password)
  ...
  private final int objLoadOne(int obj_type,
                               byte[] data,
                               int len,
                               String password)
    throws NoSuchAlgorithmException, CryptoException
  {
    try {
      if (obj_type == Constants.SSL_OBJ_PKCS8) {
        ...
        ks12.load(new ByteArrayInputStream(data, 0, len),
        ...
        throw new UnsupportedOperationException(
          "Only pkcs8/pkcs12 encoding are supported for private keys");
      } else if (obj_type == Constants.SSL_OBJ_X509_CACERT ||
                 obj_type == Constants.SSL_OBJ_X509_CERT) {
  ...
  /**
   * Load security material (CA, Cert or private key) in binary DER or ASCII PEM
   * format. These information are used during the SSL protocol to authenticate
   * an SSL part and cyphering/uncyphering of the data exchanged between the two
   * parts.
   ...
   * supported on Desktop)
   * - SSL_OBJ_P12 (RC4-128 encrypted data supported).
   * PEM encoded data is automatically detected and may contain several
   * material, whereas DER encoding only support one single material.
    ...
    throws IOException, NoSuchAlgorithmException, CryptoException
  {
    ...
    try {
      ...
      ssl_socket = (SSLSocket)sf.createSocket(
        ns, inet.getHostName(), inet.getPort(), false);
      if (ssl_socket != null) {
        ssl_socket.setSoTimeout(socket.readTimeout < 0 ? 0
                                                       : socket.readTimeout);
    ...
    throws IOException, NoSuchAlgorithmException, CryptoException
  {
    ...
    try {
      ...
      if (ssl_socket != null) {
        ...
        ssl_socket.setNeedClientAuth(
          (options & Constants.SSL_CLIENT_AUTHENTICATION) != 0);
        ssl.renegotiate();
        return ssl;
      }
    } catch (java.io.IOException e) {
    ...
    try {
      ...
      for (int i = CACerts.size() - 1; i >= 0; i--) {
        java.security.cert.X509Certificate caCert =
          (java.security.cert.X509Certificate)(CACerts.items[i]);
      ...
      for (int i = Certs.size() - 1; i >= 0; i--) {
        java.security.cert.X509Certificate cert =
          (java.security.cert.X509Certificate)(Certs.items[i]);
      ...
      if ((options & Constants.SSL_SERVER_VERIFY_LATER) != 0) {
        tm = new TrustManager[] {
          ...
          new X509TrustManager(){
            /**
             * Doesn't throw an exception, so this is how it approves a
             *certificate.
             * @see
...
findIssuer(X509Certificate cert, Vector all, Vector chain)
{
  for (int i = all.size() - 1; i >= 0; i--) {
    java.security.cert.X509Certificate issuerCert =
      (java.security.cert.X509Certificate)all.items[i];
...
private static Certificate[] buildChain(PrivateKey pkey,
                                        Vector cacerts,
                                        Vector certs)
  throws NoSuchAlgorithmException, CryptoException
{
...
finalize()
{ // flsobral@tc114_36: finalize support.
  try {
