 ...
 * </pre>
 *
 * Important: you cannot open a server socket before the main event loop. In
 * other words, you cannot open a socket in the app's constructor, but CAN in
 * the initUI method. <p>
 */
public class ServerSocket
{
  ...
  /**
   * Passing a value of 0 to the constructor causes any accept operation to
   * wait indefinitely.
   */
   ...
   * @throws totalcross.io.IOException
   */
  public ServerSocket(int port, int timeout, String addr)
    throws totalcross.io.IOException
  {
    ...
    throws totalcross.io.IOException
  {
    ...
    if (backlog <= 0) {
      throw new java.lang.IllegalArgumentException(
        "Invalid value for argument 'backlog': " + backlog);
    }
   ...
   * is unbound.
   */
  public String getHost() { return addr; }

  /**
   * Returns the local TCP port on which this socket is listening, passed in the
   * constructor.
   ...
   * returning a Socket representing the connection established with the client.
   * This method blocks until a connection is made or the operation times out.
   *
  ...
  final private totalcross.net.Socket nativeAccept()
    throws totalcross.io.IOException
  {
