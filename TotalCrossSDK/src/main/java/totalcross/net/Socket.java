...
public class Socket extends Stream
{
  ...
  /**
   * Opens a socket with the given parameters. This method establishes a socket
   * connection by looking up the given host and performing the 3 way TCP/IP
   * handshake.
   *
   * @param host the host name or IP address to connect to
   * @param port the port number to connect to
   ...
   *           'apnuser=[value]', 'apnpass=[value]', 'directtcp=[true|false]'
   * and 'nolinger=[true|false]'. On Palm OS devices, the only valid parameter
   * is 'nolinger=[true| false]'. If true, the socket is closed immediately, and
   * no ack is waited from the server. Note that this must be done for the very
   * first socket creation per application.
    ...
    throws totalcross.net.UnknownHostException, totalcross.io.IOException
  {
    if (port < 0 || port > 65535) {
      throw new java.lang.IllegalArgumentException(
        "Invalid value for argument 'port': " + port);
    }
    if (timeout < 0) {
      throw new java.lang.IllegalArgumentException(
        "Invalid value for argument 'timeout': " + timeout);
    }
   ...
   * @param port the port number to connect to
   * @param timeout the specified timeout, in milliseconds.
   * @param noLinger if true, the socket is closed immediately, and no ack is
   *     waited
   ...
   * @throws totalcross.io.IOException
   */
  final private void socketCreate(final String host,
                                  final int port,
                                  final int timeout)
    throws totalcross.net.UnknownHostException, totalcross.io.IOException
  {
    ...
    if (isRead) {
      ...
      try {
        ((java.net.Socket)socketRef)
          .setSoTimeout(readTimeout < 0 ? 0 : readTimeout);
  ...
  private byte[] rlbuf; // guich@tc123_48: use a byte[] buffer instead of a
                        // StringBuffer
   ...
   * with \\n or \\r\\n.
   * <br><br>Note: this method is VERY slow since it reads a single character
   * per time. Consider using new LineReader(socket) or new
   * BufferedStream(socket) instead.
   *
  ...
  public String readLine() throws totalcross.io.IOException
  {
    ...
    while ((r = readWriteBytes(buf, pos, 1, true)) == 1) {
      if (buf[pos] == '\n') { // guich@tc123_47
        ...
        // note that pos must be same of length, otherwise the String will be
        // constructed with one less character
        break;
      }
    ...
    return (pos > 0 || r == 1)
      ? new String(Convert.charConverter.bytes2chars(buf,
                                                     0,
                                                     pos))
      : null; // brunosoares@582_11
  }
  ...
  public Object getNativeSocket() { return socketRef; }

  /**
   * Returns the remote host which this socket is connected to, passed in the
   * constructor.
   */
  public String getHost() { return host; }

  /**
   * Returns the remote port which this socket is connected to, passed in the
   * constructor.
   */
