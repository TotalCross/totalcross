...
public class Socket4D extends Stream
{
  ...
  native void socketCreate(final String host,
                           final int port,
                           final int timeout,
  ...
  native private int readWriteBytes(byte buf[],
                                    int start,
                                    int count,
                                    boolean isRead)
    ...
    throws totalcross.net.UnknownHostException, totalcross.io.IOException
  {
    ...
    boolean noLinger =
      params != null && params.toLowerCase().indexOf("nolinger=true") >= 0;
    this.host = host;
    this.port = port;
  ...
  public String readLine() throws totalcross.io.IOException
  {
    ...
    return (pos > 0 || r == 1)
      ...
      : null; // brunosoares@582_11 - charConverter is already used
  }
