...
public class Base64 {
	 ...
	 */
	public static String encode(byte[] bytes, int start,
								int count) { // flsobral@tc113_40: created to be used by totalcross.mail.BinaryContentHandler. - guich@tc123_40: no longer used there
	...
	public static void encode(ByteArrayStream in, ByteArrayStream out) {
		...
		if (from == len - 2) {
			int d = ((((int) fromBytes[from]) & 0xFF) << 16) | ((((int) fromBytes[from + 1]) & 0xFF) <<
					8); //fdie@511_6
