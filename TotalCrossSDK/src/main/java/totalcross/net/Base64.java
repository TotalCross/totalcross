...
/**
 * Contains methods to encode and decode to base 64. The base 64 format is used
 * to convert binaries into text so that they can be sent over a stream. It may
 * then be converted back to binary form.<p> Note that 3 bytes base 16
 * (hexadecimal) are encoded into 4 bytes base 64, so the final length is 33%
 * bigger.
 *
 * @since SuperWaba 5.1
 */
public class Base64
{
   ...
   * Important: error detection is NOT made, so you must be sure that no invalid
   * characters are given!
   *
   * @param in The base 64 input stream. Its position must be set to the current
   *     size of the input data.
   * @param out The base 10 output stream. When returning, its position will be
   *     set with the output size in bytes.
   */
   ...
   * Important: error detection is NOT made, so you must be sure that no invalid
   * characters are given!<br> Important: it does not check if the destiny has
   * the proper size
   *
                           ...
                           int destStart)
  {
    ...
    while (tuples-- > 0) {
      int tri = (toBase10[src[from++]] << 18) + (toBase10[src[from++]] << 12) +
                (toBase10[src[from++]] << 6) + (toBase10[src[from++]]);
   ...
   */
  public static String encode(byte[] bytes, int start, int count)
  { // flsobral@tc113_40: created to be used by
    // totalcross.mail.BinaryContentHandler. - guich@tc123_40:
   ...
   *
   * @param in The Base 10 intput stream. Its position must be set to the
   *     current size of the input data.
   * @param out The Base 64 ouput stream. When returning, its position will be
   *     set with the output size in bytes.
   */
  public static void encode(ByteArrayStream in, ByteArrayStream out)
  {
    int len = in.getPos();
    int size = len * 3 / 2;
    size = ((size + 4) >> 2) << 2; // guich@552_3: make sure that the computed
