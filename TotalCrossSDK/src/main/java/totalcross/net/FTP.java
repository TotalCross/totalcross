 ...
 * <p>
 * Note: if you're experiencing slowness in the server, try to disable the
 reverse-ip resolution in your FTP server:
 ...
 * </pre>
 *
 * You can use the CompressedByteArrayStream to transfer and receive big files
 to/from the server (check the class for more
 ...
 * <pre>
 ...
 &nbsp   ftp.sendFile(f, &quot;michelle.txt&quot;, false); // last parameter
 depends on what you want to do
 * </pre>
 *
 * It is currently impossible to transfer a whole PDBFile to/from the server.
 The solution for this would be to send
 * each record in pieces, and store it in separate files in the server. Then a
 routine written in TotalCross in the server could
 ...
 * <pre>
 ...
 &nbsp         throw new RuntimeException(&quot;PDBFile is in use
 elsewhere!&quot;); &nbsp      else &nbsp         ftp.sendFile(cat, name +
 &quot;#&quot; + i, false); &nbsp   }
 ...
 &nbsp   PDBFile cat = new PDBFile(name + &quot;.crtr.type&quot;,
 PDBFile.CREATE); &nbsp   for (int i = 0;; i++) &nbsp   { &nbsp      File f =
 new File(name + &quot;#&quot; + i, File.READ_WRITE); &nbsp      if
 (!f.exists()) &nbsp         break; // no more records &nbsp      int size =
 f.getSize(); &nbsp      f.readBytes(buf, 0, size); &nbsp      f.delete(); //
 could be also: f.close();
 ...
 * Here is a list of error codes that can thrown if an Exception occurs: <p>
 ...
 bgcolor="#FFFFFF"> <TR bgcolor="#CCCCCC" align="CENTER"> <TD><B>Code</B></TD>
        ...
        <TR VALIGN="TOP" bgcolor="#FFFFCC">
          ...
          <TD align=left><B>The requested action has been successfully
 completed.</B></TD>
        </TR>
        <TR VALIGN="TOP">
        ...
        <TR VALIGN="TOP">
          ...
          <TD>Service closing control connection. (Logged out if
 appropriate.)</TD>
        </TR>
        <TR VALIGN="TOP">
        ...
        <TR VALIGN="TOP" bgcolor="#FFFFCC">
          ...
          <TD align=left><B>The command has been accepted, but the requested
              action is being held pending receipt of further
 information.</B></TD>
        </TR>
        <TR VALIGN="TOP">
        ...
        <TR VALIGN="TOP">
          ...
          <TD>Requested file action pending further information. <br>If you're
 using "list *.txt", try "list *.*" and filter localy.</TD>
        </TR>
        <TR VALIGN="TOP" bgcolor="#FFFFCC">
          <TD align=CENTER><B>400 Codes</B></TD>
          <TD align=left><B>The command was not accepted and the requested
 action did not take place. <BR> Tthe error condition is temporary, however, and
 the action may be requested again.</B></TD>
        </TR>
        <TR VALIGN="TOP">
        ...
        <TR VALIGN="TOP">
          ...
          <TD>Requested file action not taken. File unavailable (e.g., file
 busy).</TD>
        </TR>
        <TR VALIGN="TOP">
        ...
        <TR VALIGN="TOP">
          ...
          <TD>Requested action not taken. Insufficient storage space in
 system.</TD>
        </TR>
        <TR VALIGN="TOP" bgcolor="#FFFFCC">
          <TD align=CENTER><B>500 Codes</B></TD>
          <TD align=left><B>The command was not accepted and the requested
 action did not take place.</B></TD>
        </TR>
        <TR VALIGN="TOP">
        ...
        <TR VALIGN="TOP">
          ...
          <TD>Requested action not taken. File unavailable (e.g., file not
 found, no access).</TD>
        </TR>
        <TR VALIGN="TOP">
...
{
  /**
   * Assign the progressInfo member to an instance of this interface to receive
   * information of how many bytes were transfered. Note that there's no way to
   * get the total size of the file, but only what has been transfered so far.
   * @see #progressInfo
   */
  ...
  private StringBuffer sbsc = new StringBuffer(
    100); // guich@562_4: now we use a string buffer to avoid gc
  private byte buffer[];
  ...
  /**
   * Assign the ProgressInformation member to receive information about the
   * file transfer. This way, you can show it in a ProgressBar.
   * @since TotalCross 1.14
   */
  public ProgressInformation progressInfo;

  /**
   * This makes a sleep during the send of a file. Important: when using
   * softick, you must set this to 500(ms) or more, or softick will starve to
   * death.
   */
  ...
  private LineReader
    myTCPConnectionReader; // flsobral@tc123_54: FTP now uses LineReader to
                           // process the server responses - greatly improving
                           // its performance.
   ...
   * @param url The url as an ip or a full address to the server. The connection
   *     port is always 21
  ...
  /**
   * Opens a socket to the given URL, user, password, port and the default
   * timeouts.
   *
  ...
  /**
   * Opens a socket to the given URL, user, password, logging control and the
   * default timeouts.
   *
   * @param url  The url as an ip or a full address to the server. The
   *     connection port is always 21
   * @param user The user name for login
   * @param pass The password for login
   * @param loggingControl The ListBox or ComboBox where the logging will be
   *     sent to.
   *
   * @see #FTP(String, String, String, int, int, int, int, Control)
   */
  ...
  /**
   * Opens a socket to the given URL, user, password, timeouts and logging
   * control
   *
   * @param url  The url as an ip or a full address to the server. The
   *     connection port is always 21
  ...
  /**
   * Opens a socket to the given URL, user, password, port, timeouts and logging
   * control.
   *
   * @param url  The url as an ip or a full address to the server.
   * @param user The user name for login
    ...
    throws UnknownHostException, IOException, FTPConnectionClosedException
  {
    ...
    try {
      myTCPConnection = new Socket(url,
                                   port,
                                   openTimeout,
                                   true); // guich@561_14: use  for Palm OS
    ...
    throws FTPConnectionClosedException, totalcross.io.IOException
  {
    if (myTCPConnection != null) {
      try {
        ...
        // The server might close the connection without sending an
        // acknowledgement, so we catch and ignore IOExceptions at this point.
      } finally {
   ...
   * information. Transfer parameters are similarly unchanged. The argument is a
   * pathname specifying a directory or other system dependent file group
   * designator.
   *
   * @param pathName Specifies a directory or other system dependent file group
   *     designator.
   */
  public void setCurrentDir(String pathName)
    throws FTPConnectionClosedException, totalcross.io.IOException
  {
    ...
    throws FTPConnectionClosedException, totalcross.io.IOException
  {
    if (type == null) {
      throw new NullPointerException(
        "Argument 'type' cannot have a null value.");
    }
   ...
   * fields are separated by commas. A port command would be: <pre> PORT
   * h1,h2,h3,h4,p1,p2
   * </pre>
    ...
    throws FTPConnectionClosedException, totalcross.io.IOException
  {
    if (port == null) {
      throw new NullPointerException(
        "Argument 'port' cannot have a null value.");
    }
  ...
  /**
   * Causes the directory specified in the pathname to be removed as a directory
   * (if the pathname is absolute) or as a subdirectory of the current working
   ...
   * @throws NullPointerException if any of the arguments, oldPathName and
   *     newPathName, is null.
   */
    ...
    throws FTPConnectionClosedException, totalcross.io.IOException
  {
    if (oldPathName == null) {
      throw new NullPointerException(
        "Argument 'oldPathName' cannot have a null value.");
    }
    if (newPathName == null) {
      throw new NullPointerException(
        "Argument 'newPathName' cannot have a null value.");
    }
  ...
  /**
   * Sends a file to be stored at the server site. If the file specified in the
   * pathname exists at the server site, then its contents shall be replaced by
   * the data being transferred. A new file is created at the server site if the
   * file specified in the pathname does not already exist.
   *
   * @param inputStream The Stream from where the data will be read (using
   *     readBytes method)
   * @param pathName The name of the destination file in the server
   * @return Returns the total bytes sent. Its up to the user to check if it was
   *     the desired amount.
   * @throws NullPointerException if any of the arguments, inputStream or
   *     pathName, is null.
   */
    ...
    throws FTPConnectionClosedException, totalcross.io.IOException
  {
    if (inputStream == null) {
      throw new NullPointerException(
        "Argument 'inputStream' cannot have a null value.");
    }
    if (pathName == null) {
      throw new NullPointerException(
        "Argument 'pathName' cannot have a null value.");
    ...
    setType(BINARY);
    /*
     * Important: linger must be set to true. in a gprs connection, the data is
     * not completely sent until the socket.close is sent. In this case, we MUST
     * wait for the acknowledge from the server, otherwise the file will not be
     * flushed.
     */
      ...
      while (totalSent < count) { // this may happen when using softick
        ...
        if (sent == 0 && tries-- == 0) {
          throw new totalcross.io.IOException(
            "Cannot write to server.|Transmitted " + total + " bytes");
        }
        if (sent > 0) {
          totalSent += sent;
        }
    ...
    try {
      ...
      // if the socket was correctly closed, we can safely ignore any timeout
      // exceptions
      ...
      try {
        ...
        if (msg != null &&
            (msg.startsWith("No response") || msg.indexOf("450") >= 0)) {
          /*
           * This is a case where, given to the long wait for file flushing, the
           * ack from the server was lost. Since the socket had closed fine, we
           * can ensure that the file was transfered and just ignore the ack.
           */
  ...
  /**
   * Transfers a copy of the file specified in the pathname from the server. The
   * status and contents of the file at the server site shall be unaffected.
   *
   * @param pathName Specifies the file to be retrieved.
   * @param outputStream The stream to where the file will be written.
   * @return number of bytes read.
   * @throws NullPointerException if any of the arguments, pathName or
    ...
    throws FTPConnectionClosedException, totalcross.io.IOException
  {
    ...
    if (progressInfo != null) {
      try {
        ...
        progressInfo.reportSize(
          Convert.toInt(fsize.substring(fsize.lastIndexOf(' ') + 1)));
      } catch (Exception e) {
      }
    }
    ...
    while ((count = socket.readBytes(buffer)) >=
           0) { // guich@561_8: != -1 -> > 0
      if (count > 0) {
        tries = 3;
        total += count;
    ...
    try {
      checkResponse(COMPLETED); // flsobral@tc123_50: always check for the
                                // server response, even when count is -1 (EOF).
    } catch (totalcross.io.IOException e) {
   ...
   *
   * This command causes a list to be sent from the server passive DTP. If the
   * pathname specifies a directory or other group of files, the server should
   ...
   * file. A null argument implies the user's current working or default
   * directory. The data transfer is over the data connection in type ASCII or
   * type EBCDIC. (The user must ensure that the TYPE is appropriately ASCII or
   * EBCDIC). Since the information on a file may vary widely from system to
   * system, this information may be hard to use automatically in a program, but
   * may be quite useful to a human user.
   *
   * @param pathName Specifies a directory, a group of files or a single file. A
   *     null value lists the user's current
   ...
   * site. The pathname should specify a directory or other system-specific file
   * group descriptor; a null argument implies the current directory. The server
   * will return a stream of names of files and no other information. The data
   * will be transferred in ASCII or EBCDIC type over the data connection as
   * valid pathname strings separated by <CRLF> or <NL>. (The user must ensure
   * that the TYPE is correct.) This command is intended to return information
   * that can be used by a program to further process the files automatically.
   * For example, in the implementation of a "multiple get" function.
   *
   * @param pathName Specifies a directory or a file group descriptor. A null
   *     value implies the user's current directory.
  ...
  private Socket openReturningPath(boolean linger)
    throws FTPConnectionClosedException, totalcross.net.UnknownHostException,
           totalcross.io.IOException
  {
    String line = sendCommand(
      "PASV",
      "",
      COMPLETED); // 227 Entering Passive Mode (219,112,241,81,68,33)
    // get the host and port
    if (line == null || line.length() == 0) {
      throw new totalcross.io.IOException(
        "Nothing read from PASV|connection closed?");
    }
    line = line.substring(line.indexOf('(') + 1);
    line = line.substring(
      0,
      line.lastIndexOf(
        ')')); // guich@561_6: some servers return "33)" and others "33)."
    ...
    try {
      ...
      log(ine.getMessage());
      throw new totalcross.io.IOException("Invalid port number: " + p[4] +
                                          p[5]);
    }
      ...
      try {
        socket =
          new Socket(host,
                     port,
                     openTimeout,
                     linger); // guich@561_14: 15: use nolinger for Palm OS
        ...
        if (i < 4) {
          ...
          throw e; // flsobral@tc100b3: failed all 5 times, time to give up and
  ...
  private String readAck() throws totalcross.io.IOException
  {
    try {
      ...
      } while (!isDigit(s.charAt(0)) || !isDigit(s.charAt(1)) ||
               !isDigit(s.charAt(2)) || s.charAt(3) != ' ');
      return s;
  ...
  private String checkResponse(byte expectedReturnType)
    throws FTPConnectionClosedException, totalcross.io.IOException
  {
    String line = readAck();
    if (line != null) {
      ...
      try {
        ...
        // This exception will NEVER be thrown by Conver.toInt because readAck()
        // ALWAYS returns strings starting with 3 digits.
      ...
      if (ack == 421 || ack == 425 || ack == 426) { // guich@561_8
        ...
        throw new FTPConnectionClosedException(
          "Connection timed out. This FTP|instance is no longer valid.");
      }
      throw new totalcross.io.IOException("Not acknowledged|Expected of type " +
                                          expectedReturnType +
                                          "|Received: " + ack);
    }
    ...
    throw new totalcross.io.IOException(
      "No response|Maybe the ftp was closed?");
  }

  private String sendCommand(String s1, String s2, byte expectedReturnType)
    throws FTPConnectionClosedException, totalcross.io.IOException
  { // guich@562_4: now the 2nd command is
    // passed separately
    if (s1 != null) {
      ...
      sb.append(Convert.CRLF); // guich@562_4: seems that we must send all the
                               // string at once
      // Let's avoid a few method calls and call directly the writeBytes with 3
      // arguments.
  ...
  /**
   * Sets the control where the log will be displayed. Only ComboBox and ListBox
   * are allowed.
   */
  ...
  /**
   * Closes the control connection without issuing any quit or abort command to
   * the server. Avoid using this method unless strictly necessary, use the quit
   * method instead.
   */
