...
public class FTP {
	...
	private LineReader
	myTCPConnectionReader; //flsobral@tc123_54: FTP now uses LineReader to process the server responses - greatly improving its performance.
	...
	throws UnknownHostException, IOException, FTPConnectionClosedException {
		this(url, user, pass, defaultPort, defaultOpenTimeout, defaultReadTimeout, defaultWriteTimeout,
			 null);
	}
	 ...
	 * @param loggingControl The ListBox or ComboBox where the logging will be sent to.
	 */
	public FTP(String url, String user, String pass, int port, int openTimeout, int readTimeout,
			   int writeTimeout,
			   Control loggingControl) throws UnknownHostException, IOException, FTPConnectionClosedException {
	 ...
	 */
	public void setCurrentDir(String pathName) throws FTPConnectionClosedException,
	 ...
	 *            if pathName is null.
	 */
	public void delete (String pathName) throws FTPConnectionClosedException,
		totalcross.io.IOException {
		if (pathName == null) {
	 ...
	 */
	public void createDir(String pathName) throws FTPConnectionClosedException,
		totalcross.io.IOException {
		if (pathName == null) {
			throw new NullPointerException("Argument 'pathName' cannot have a null value");
		}
	 ...
	 * @throws NullPointerException if pathName is null.
	 */
	public void removeDir(String pathName) throws FTPConnectionClosedException,
		totalcross.io.IOException {
		if (pathName == null) {
			throw new NullPointerException("Argument 'pathName' cannot have a null value");
		}
	...
	throws FTPConnectionClosedException, totalcross.io.IOException {
		...
		try {
			checkResponse(
				COMPLETED); //flsobral@tc123_50: always check for the server response, even when count is -1 (EOF).
		} catch (totalcross.io.IOException e) {
	 ...
	 * @throws IOException
	 */
	public String[] list(String pathName) throws FTPConnectionClosedException,
		totalcross.io.IOException {
		Socket socket = openReturningPath(true);
	 ...
	 * @throws totalcross.io.IOException
	 */
	public String[] listNames(String pathName) throws FTPConnectionClosedException,
		totalcross.io.IOException {
		Socket socket = openReturningPath(true);
	...
	throws FTPConnectionClosedException, totalcross.net.UnknownHostException,
		...
		line = line.substring(0,
							  line.lastIndexOf(')')); // guich@561_6: some servers return "33)" and others "33)."
	...
	private String readAck() throws totalcross.io.IOException {
		try {
			...
			} while (!isDigit(s.charAt(0)) || !isDigit(s.charAt(1)) || !isDigit(s.charAt(2))
					 || s.charAt(3) != ' ');
			return s;
	...
	private String checkResponse(byte expectedReturnType) throws FTPConnectionClosedException,
		totalcross.io.IOException {
		String line = readAck();
		if (line != null) {
			int ack = -1;
	...
	throws FTPConnectionClosedException,
		totalcross.io.IOException { // guich@562_4: now the 2nd command is passed separately
		if (s1 != null) {
