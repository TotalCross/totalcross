...
public class GoogleMaps
{
  ...
  /**
   * Used in the flags argument of showRoute: use waze to show the route of the
  ...
  private static int toCoordI(double v) { return (int)(v * 1e6); }
  ...
  public static class Circle extends MapItem
  {
    ...
    /**
     * The radius; if > 0, its computed as meters; if < 0, its computed as
     * delta of the coordinates
     */
    public double rad;
    ...
    @Override void serialize(StringBuffer sb)
    {
      sb.append("*C*,")
        .append(toCoordI(lat))
        .append(",")
  ...
  public static class Shape extends MapItem
  {
    ...
    @Override void serialize(StringBuffer sb)
    {
      ...
      for (int i = 0; i < lats.length; i++) {
        sb.append(toCoordI(lats[i]))
          .append(",")
          .append(toCoordI(lons[i]))
          .append(",");
      }
  ...
  /**
   * A map item that represents a place in the map. It shows with a pin, and
   * above it, a balloon with a text. <pre> GoogleMaps.Place p1 = new
   * GoogleMaps.Place(); p1.lat = -3.778284; p1.lon = -38.482617; p1.backColor =
   * Color.WHITE; p1.capColor = Color.BLACK; p1.detColor = 0x444444; p1.pinColor
   * = Color.RED; p1.caption = "TotalCross MGP"; p1.fontPerc = 150; p1.detail =
   * "Av Norte 2920\nLuciano Cavalcante\nCeará - Brazil";
   * </pre>
   */
  public static class Place extends MapItem
  {
    ...
    {
      sb.append("*P*,\"")
        .append(caption == null ? null : caption.replace('"', '\''))
        .append("\",\"")
   ...
   *
   * @param address The address (WITHOUT ACCENTUATION) to show (E.G.: "rua
   *     tonelero 10, copacabana, rio de janeiro, brasil", or "22030,brasil").
   * The address is resolved and its latitude and logitude coordinates are
   * retrieved. Careful with the address given, be sure to make some tests
   * before. To remove accentuation, you can use Convert.removeAccentuation.
   *
   * If you want to pass the lat/lon coordinates, pass them in the form:
   * "@lat,lon" (E.G.: "@-22.966000,-43.185000"). Note that, due to Android's
   * restrictions, only the first 6 decimal digits in the coordinates are used.
   *
   * Passing an empty string will make the engine search for a valid "last known
   * position". If there's no such position, it will return false.
   *
   * @param showSatellitePhotos If true, an image of the satellite is combined
   *     with the map. May require more
   * bandwidth and internet resources.
   *
   * @return true if the map was shown, false otherwise. False is often returned
  ...
  /**
   * Shows the route between two points. The traversed points is a sequence of
   * lat,lon coordinates comma-separated.
   * @deprecated
   * @see #showRoute(String, String, String, int)
   */
  @Deprecated
  @ReplacedByNativeOnDeploy
  public static boolean showRoute(String addressI,
                                  String addressF,
                                  String traversedPoints,
                                  boolean showSatellitePhotos)
    throws NotInstalledException
  {
    return showRoute(
      addressI, addressF, traversedPoints, SHOW_SATELLITE_PHOTOS);
  }

  /**
   * Shows the route between two points. The traversed points is a sequence of
   * lat,lon coordinates comma-separated. If flags is USE_WAZE, the addressF is
   * not used and can be null, and a NotInstalledException is thrown if WAZE is
   * not installed.
   *
   * Note that you must remove accentuation from addressI and addressF; you may
   * use Convert.removeAccentuation.
   *
   * @see Convert#removeAccentuation(String)
   */
  @ReplacedByNativeOnDeploy
  public static boolean showRoute(String addressI,
                                  String addressF,
                                  String traversedPoints,
                                  int flags) throws NotInstalledException
  {
    return true;
  }

  /**
   * Shows an array of MapItem elements in the map. The map is zommed in a way
   * that all coordinates are visible. <p/> If one may pass multiple overlapping
   * items (let's say, items[0], items[1] and items[2]), one item will cover all
   * the previous items (item[1] will be over item[0]; item[2] will be over
   * item[1] and item[0]).
   *
   * @see {@link Circle}
   * @see {@link Shape}
  ...
  /**
   * Returns the location after searching Google. Requires internet connection
   * and waits up to 20 seconds for an answer. Returns the lat/lon pair, or null
   * if a bad response code was returned.
   */
    ...
    throws IOException, InvalidNumberException
  {
    ...
    String url =
      ...
      Convert.removeAccentuation(address).replace("  ", " ").replace(' ', '+') +
      "&sensor=false";
    HttpStream hs = new HttpStream(new URI(url));
    if (hs.badResponseCode) {
      Vm.debug("getLocation \"" + url +
               "\" returned response code: " + hs.responseCode);
      return null;
    }
    ...
    if (s.contains("<location_type>approximate</location_type>")) {
      Vm.debug("getLocation \"" + url +
               "\" returned a bad location from google maps");
      return null;
    }
