...
public class Resources {
	...
	// SWITCH
	public static Image switchBrdAnd, switchBack, switchBtnAnd, switchBtnIos, switchBrdIos, switchBtn,
	...
	public static void uiStyleChanged() {
		try {
			...
			try
				(File test = new File("device/chime.mp3")) {
				if (!test.exists()) {
					try
						(File writeTo = new File("device/chime.mp3", File.CREATE_EMPTY)) {
						try
