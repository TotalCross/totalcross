...
public class TristateImage
{
  ...
  public Image getCopy() throws ImageException { return base.getCopy(); }

  public Image getNormalInstance(int width, int height, int backColor)
    throws ImageException
  {
    return getNormalInstance(width, height, backColor, true);
  }

  private Image getNormalInstance(int width,
                                  int height,
                                  int backColor,
                                  boolean apply) throws ImageException
  {
    int hash;
    StringBuffer sb = new StringBuffer(20);
    hash = Convert.hashCode(
      sb.append((width << 16) | height).append('|').append(backColor));
  ...
  public Image getDisabledInstance(int width, int height, int backColor)
    throws ImageException
  {
    int hash;
    StringBuffer sb = new StringBuffer(20);
  ...
  public Image getPressedInstance(int width,
                                  int height,
                                  int backColor,
                                  int pressColor,
                                  boolean enabled) throws ImageException
  {
    int hash;
    StringBuffer sb = new StringBuffer(20);
    hash = Convert.hashCode(sb.append((width << 16) | height)
                              .append('|')
                              .append(backColor)
                              .append(enabled)
                              .append(pressColor));
      ...
      if (pressColor != -1) {
        ...
        ret = getNormalInstance(width, height, backColor, false)
                .getTouchedUpInstance(
                  Color.getBrightness(backColor) > (256 - 32) ? (byte)-64
                                                              : (byte)32,
                  (byte)0);
      }
      if (!enabled) {
