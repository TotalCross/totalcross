 ...
 * methods of this class. For instance, getSelectedIndex returns just the last
 * selected index; to retrieve all indexes, use getSelectedIndexes. <br><br> In
 * penless devices, there will be a cursor which will be used to highlight an
 * item; to select or unselect it, you must press the left key. <br><br>
...
public class MultiListBox extends ListBox
{
  ...
  /**
   * Fill this IntVector with the values that will be selected when the clear
   * method is called.
   */
  public IntVector clearValues = new IntVector(1); // guich@tc112_33

  /**
   * Set to false to don't show the number of selected elements as they are
   * clicked.
   ...
   * Note that this implies that setOrderIsImportant(true) and also
   * setMaxSelections were both called, otherwise it has no effects.
   */
  public boolean unselectFirstWhenMaxIsReached;

  /**
   * Global suffix used to display the number of items in a ComboBox. Defaults
   * to " items".
   */
  public static String itemsText = " items";

  /**
   * Local suffix to display the number of items in a ComboBox. Defaults to "
   * items".
   */
  ...
  /**
   * Call this method if you want to keep track of the order in which the items
   * were selected. Note that this makes the listbox slower. Calling this method
  ...
  public boolean isOrderImportant() { return order != null; }

  /**
   * Returns the selected index. If more than one item is selected, returns the
   * last one.
   */
  @Override public int getSelectedIndex()
  {
    try {
      return /*Settings.keyboardFocusTraversable ? selectedIndex : */ order !=
          null
        ? order.peek()
        : selectedIndexes.size() > 0
            ? selectedIndexes.getKey(0)
            : -1; // guich@tc110_100: fixed problem in penless mode: a
                  // highlighted item was shown in the combobox instead of
                  // the selected item
  ...
  /**
   * Returns the last selected item if you had set <i>order is important</i>,
  ...
  /**
   * Return a vector with the indexes which have been selected.  The elements of
   * the Vector are the indexes in the Vector of the items.  So if this Vector
   * is holding [3, 12, 5] it means that the items 3, 5 and 12 have been
   * selected. If order is important, then the order intvector is returned
   * (caution: do not change the returned array!). Note that the indexes are not
   * in order; to order them, call <code>qsort</code>.
   */
  ...
  public boolean isSelected(int index) { return selectedIndexes.exists(index); }
  ...
  @Override protected void drawSelectedItem(Graphics g, int from, int to)
  {
    drawingSel = true;
  ...
  @Override protected int getCursorColor(int index)
  {
  ...
  /**
   * In this MultiListBox, inverts the status of the given index, or clears all
   * if i is -1.
  ...
  @Override public void setSelectedIndex(int index)
  {
    ...
    if (index == -1) {
      oldSelection =
        NO_OLD_SELECTION; // no old selection after unselecting everything
      selectedIndexes.clear();
      if (order != null) {
        order.removeAllElements();
      }
  ...
  @Override protected void leftReached() { handleClick(selectedIndex); }

  /**
   * Sets or clear an index. Can also be used to set or clear all indexes,
   * passing -1 as the index. Both operations are limited by the defined max
   * selections.
   */
  @Override public void setSelectedIndex(int index, boolean set)
  {
    if (0 <= index && index < itemCount) {
      ...
      if (!set) {
        ...
        if (unselectFirstWhenMaxIsReached && order != null &&
            selectedIndexes.size() == maxSelections) {
  ...
  @Override protected void handleSelection(int newSelection)
  {
    if (newSelection != oldSelection &&
        newSelection <
          itemCount) { // then they dragged outside the current selection
      handleClick(newSelection);
      oldSelection = selectedIndex = newSelection;
      drawCursor(
        getGraphics(), newSelection, selectedIndexes.exists(newSelection));
    }
  }

  /** Called by ComboBoxDropDown when its being popped. */
  protected void cbddOnPopup() { changed = false; }

  /**
   * Called by ComboBoxDropDown when its being unpopped. Sends a PRESSED event
   * if a change was made in the selected indexes.
   */
  ...
  {
    if (Settings.keyboardFocusTraversable ||
        !(parent instanceof ComboBoxDropDown)) {
      super.postPressedEvent();
    }
  }

  @Override protected void endSelection()
  {
  ...
  /**
   * Returns the String with the selected item (if single) or a string with the
   * number of selected items. You can change the suffix itemsText or
   * localItemsText to another one. If order is important, returns the last
   * selected item.
   */
  @Override public String getText()
  {
    ...
    return Convert.toString(size) +
      (localItemsText != null ? localItemsText : itemsText);
  }
  ...
  @Override public void onEvent(Event e)
  {
    if (e.target == this) {
      if (e instanceof PenEvent &&
          Settings.keyboardFocusTraversable) { // if in kft and the user clicked
                                               // in the control, handle the
                                               // event as if we were not in kft
