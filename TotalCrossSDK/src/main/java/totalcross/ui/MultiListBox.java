...
public class MultiListBox extends ListBox {
	...
	public int getSelectedIndex() {
		try {
			return /*Settings.keyboardFocusTraversable ? selectedIndex : */order != null ? order.peek()
					: selectedIndexes.size() > 0 ? selectedIndexes.getKey(0) :
					-1; // guich@tc110_100: fixed problem in penless mode: a highlighted item was shown in the combobox instead of the selected item
	...
	protected void handleSelection(int newSelection) {
		if (newSelection != oldSelection
			&& newSelection < itemCount) { // then they dragged outside the current selection
	...
	public void onEvent(Event e) {
		if (e.target == this) {
			if (e instanceof PenEvent
				&& Settings.keyboardFocusTraversable) { // if in kft and the user clicked in the control, handle the event as if we were not in kft
