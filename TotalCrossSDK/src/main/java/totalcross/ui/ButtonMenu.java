...
public class ButtonMenu extends ScrollContainer implements PressListener {
	...
	public void initUI() {
		...
		if (disposition == MULTIPLE_VERTICAL
			&& height / imageH >
			rowsPerPage) { // guich: prevent problem when the gap is too high and just a few buttons are shown
			vgap = height % imageH;
			rowsPerPage++;
			pageH = height - vgap;
			imageH = imageH0 + vgap;
		...
		if (isVertical) {
			...
			if (v > 0) {
				add(spacer = new Spacer(v, 1), maxX2,
					TOP); // in a multipage, make sure that the last page has a full width
			}
		}
