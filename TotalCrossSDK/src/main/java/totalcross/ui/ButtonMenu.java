...
/**
 * This class adds a multi-button menu that can be scrolled horizontally
 * (single-row) or vertically (multiple-rows), using a scrollbar or flicking.
 *
 ...
 * So, a value of 25 means 25% of the font's height, or 1/4; 150 means 150% of
 * the font's height, or 1.5x; and so on. This enabled the gaps be constant in
 * physical inches no matter the screen DPI or resolution.
 *
...
public class ButtonMenu extends ScrollContainer implements PressListener
{
  ...
  /**
   * The gap between the image and the text, in percentage of the font's
  ...
  /**
   * The gap between two horizontal buttons, in percentage of the font's
   * height. Also used as gap between the button and the ButtonMenu's border.
   * Defaults to 100 (%).
   */
  ...
  /**
   * The gap between the text or image and the button borders, in percentage of
   * the font's height. Defaults to 10 (%).
   */
  ...
  /**
   * Used in the pagePositionDisposition. Don't use a PagePosition, use the
   * ScrollPosition instead.
   */
  public final static int NO_PAGEPOSITION = 2;

  /**
   * If disposition is a MULTIPLE_HORIZONTAL, set how the PagePosition will
   * replace the ScrollPosition control.
  ...
  /**
   * Where to place the text (supports only LEFT, TOP, RIGHT, BOTTOM, CENTER -
   * no adjustments!). Also supports RIGHT_OF (relativeToText is computed
   * automatically). Defaults to CENTER.
   */
  ...
  /**
   * Used in the disposition member of the constructor. The menu will have a
   * single column and multiple rows and will scroll vertically.
   */
  public static final int SINGLE_COLUMN = 1;
  /**
   * Used in the disposition member of the constructor. The menu will have a
   * single row and multiple columns and will scroll horizontally.
   */
  public static final int SINGLE_ROW = 2;
  /**
   * Used in the disposition member of the constructor. The menu will have
   * multiple columns and rows and will scroll horizontally.
   */
  public static final int MULTIPLE_HORIZONTAL = 3;
  /**
   * Used in the disposition member of the constructor. The menu will have
   * multiple columns and rows and will scroll vertically.
   */
  ...
  /**
   * Changes all the buttons to the given parameters. If you don't want to set
   * the images or the names, pass null in the proper place. Calls onFontChanged
   * and initUI to reset the buttons.
   ...
   */
  public Button getButton(int idx) { return btns[idx]; }

  /**
   * Creates and resizes all Button and images. For better performance, call
   * setFont for this control BEFORE calling add or setRect (this is a general
   * rule for all other controls as well).
   */
  @Override public void onFontChanged()
  {
    ...
    for (int i = n; --i >= 0;) {
      ...
      if (img != null && imageSize != -1 && img.getHeight() != imageS) {
        try {
          img = img.getHwScaledInstance(
            img.getWidth() * imageS / img.getHeight(), imageS);
      ...
      btn.setBorder(borderType); // setBorder uses borderColor3DG and resets the
                                 // other values to default.
      btn.borderColor3DG = borderColor3DG;
  ...
  @Override public void initUI()
  {
    if (super.sbH != null && super.sbH instanceof ScrollPosition) {
      ((ScrollPosition)super.sbH).barColor =
        pressedColor != -1 ? pressedColor : foreColor;
    }
    if (super.sbV != null && super.sbV instanceof ScrollPosition) {
      ((ScrollPosition)super.sbV).barColor =
        pressedColor != -1 ? pressedColor : foreColor;
    }
    if (btns != null && btns[0].isChildOf(this)) {
      for (int i = btns.length; --i >= 0;) {
    ...
    if (disposition == MULTIPLE_VERTICAL &&
        height / imageH >
          rowsPerPage) { // guich: prevent problem when the gap is too high
                         // and just a few buttons are shown
      vgap = height % imageH;
      rowsPerPage++;
      pageH = height - vgap;
      imageH = imageH0 + vgap;
    ...
    for (int i = 0, c = 0; i < n; i++) {
      ...
      if (disposition == MULTIPLE_HORIZONTAL) {
        ...
        y += (i % colsPerPage) == 0
               ? (i % itemsPerPage) == 0 ? divY + remY : divY
               : 0;
      }
    ...
    boolean isVertical =
      disposition == SINGLE_COLUMN || disposition == MULTIPLE_VERTICAL;
    // checks if there's enough space to fit all buttons in our height, and if
    // there is, prevent it from scrolling
    int top = btns[0].y - 1;
    int bot = 0;
    for (int i = Math.max(0, n - 2 * colsPerPage * rowsPerPage); i < n;
         i++) { // guich@tc153: correctly find the max y2
      int yy = btns[i].getY2();
      if (yy > bot) {
        bot = yy;
      }
    ...
    if ((bot - top) < height) {
      // check how many space we have at top and bottom, and change the buttons
      // y so they are centered vertically
    ...
    boolean hasPagePosition = pagePositionDisposition != NO_PAGEPOSITION &&
                              disposition == MULTIPLE_HORIZONTAL &&
    ...
    if (isVertical) {
      ...
      if (v > 0) {
        add(spacer = new Spacer(v, 1),
            maxX2,
            TOP); // in a multipage, make sure that the last page has a full
                  // width
      }
    }
    ...
    if (Settings.fingerTouch) {
      if (disposition == SINGLE_ROW || disposition == MULTIPLE_HORIZONTAL) {
        ...
        if (hasPagePosition) {
          ...
          pagepos.setRect(
            CENTER,
            pagePositionDisposition == PAGEPOSITION_AT_BOTTOM ? BOTTOM : TOP,
            PREFERRED,
            PREFERRED);
        ...
        flick.setDistanceToAbortScroll(
          0); // we deliberably disable the scroll abort on vertical scrolls
  ...
  public int getPreferredWidth(int cols)
  {
    ...
    int sb = Settings.fingerTouch || sbV == null || !sbV.isVisible()
               ? 0
  ...
  @Override public int getPreferredHeight() { return getPreferredHeight(1); }
  ...
  public int getPreferredHeight(int rows)
  {
    ...
    int sb = Settings.fingerTouch || sbH == null || !sbH.isVisible()
               ? 0
