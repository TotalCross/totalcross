 ...
 * <pre>
 * add(gc = new
 GridContainer(GridContainer.HORIZONTAL_ORIENTATION),LEFT,TOP,FILL,FILL);
...
public class GridContainer extends Container
{
  ...
  /**
   * A percentage that defines the heihgt of the arrow buttons. Defaults to 200
   * in penless devices, 100 otherwise.
   */
  public int buttonsHeight =
    Settings.fingerTouch ? Control.isTablet ? 200 : 150 : 100;
  ...
  public GridContainer(int orientation)
  {
    ...
    sc = new ScrollContainer(orientation == HORIZONTAL_ORIENTATION,
                             orientation == VERTICAL_ORIENTATION) {
      @Override public int getScrollDistance()
      {
        return GridContainer.this.orientation == HORIZONTAL_ORIENTATION &&
            cells != null && cells.length >= cols
          ? cells[cols - 1].getX2() + 1
          : 0;
      }
    };
  ...
  public Flick getFlick() { return sc.flick; }
  ...
  public static class Cell extends Container
  {
    ...
    public Cell() { focusTraversable = true; }
  ...
  @Override public void onFontChanged() { sc.setFont(font); }
  ...
  @Override public void initUI()
  {
    ...
    if (isHoriz) {
      ...
      sc.flick.forcedFlickDirection = orientation == HORIZONTAL_ORIENTATION
                                        ? Flick.HORIZONTAL_DIRECTION_ONLY
  ...
  /**
   * Sets the rows per page. Changing this value changes the font size
   * dynamically.
   */
  public void setRowsPerPage(int rpp) { this.rpp = rpp; }
  ...
  /**
   * Sets the cells of this GridContainer. Note that you cannot delete or add
   * cells, only change the whole set of cells. You must call setRowsPerPage
   * and/or setPageSize before calling this method.
   */
  public void setCells(Cell[] cells)
  {
    ...
    if (rpp != 0) {
      setFont(
        Font.getFont(font.isBold(), Math.min(height, width) / rows / rpp));
    }
    this.cells = cells;
    ...
    if (orientation == VERTICAL_ORIENTATION) {
      ...
      // fill with spacers the rest of the columns if the last page has less
      // than one column filled
