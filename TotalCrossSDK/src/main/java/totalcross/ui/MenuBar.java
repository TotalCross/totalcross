 ...
 * MenuItem col0[] =
 * {
 *   new MenuItem("Record"),                               // caption for the
 * MenuBar miNewEvent = new MenuItem("NewEvent",false),          // checked
 * item, starting unchecked miDeleteEvent = new MenuItem("Delete
 * Event...",true), // checked item, starting checked new MenuItem("Attach
 * Note"), new MenuItem("Delete Note..."), new MenuItem("Purge..."), beamEvent =
 ...
 * </pre>
 * The menu can be closed by a click on a valid item or clicking outside of its
 * bounds. A PRESSED event will be thrown when the menu is closed and a menu
 * item was selected. To discover which item was selected, see method
 * getSelectedIndex, which returns -1 if none, or the matrix index otherwise.
 * <p>
 * Note that the separator dotted line doesn't generate events and can't be
 * selected. <p> To convert the old menu form (using a string matrix) into the
 * new form using a MenuItem matrix, you can use this idea: <br>Old format:
 * <pre>
 * // the string arrays menuArq, menuUtil and menuSobre are initialized
 * somewhere else setMenuBar(mbar = new MenuBar(new
 * String[][]{menuArq,menuUtil,menuSobre})); mbar.setChecked(106,true);
 ...
 *
 * // at initUI
 * setMenuBar(mbar = new MenuBar(MenuBar.strings2items(new
 * String[][]{menuArq,menuUtil,menuSobre}))); MenuItem[][] mis =
 * mbar.getMenuItems(); mi4 = mis[0][4]; mi106 = mis[1][6]; mi106.isChecked =
 * true; mi106.isEnabled = false; mi4.isChecked = true;
 * </pre>
 * Note that, after changing the isChecked and isEnabled states, there's no need
 * to call repaint, bacause they will show up only the next time the menu bar
 * opens.
 */
...
public class MenuBar extends Window
{
  ...
  /**
   * Note: if you want to change the spacement between the menu items (maybe to
   * make more items fit in the row), change this gap value. Default is 3 (3
   * pixels at left and 3 at right).
   */
  ...
  public MenuItem[][] getMenuItems() { return this.items; }
  ...
  {
    if (b) {
      setRect(0, 0, FILL, fmH + 4 + titleGap); // update the bounds, because the
                                               // screen may have been changed
      popupNonBlocking();
    }
  }
   ...
   * @return -1 if no item was selected, or a number with format <i>xyy</i>,
   *     where x is the index of the item
   * selected at the MenuBar and yy is the index+1 ( = the index of the String
   * array that defines the items) of the item selected in the MenuBarDropDown.
   * EG: suppose that the col1/"Cut" of the
   * example at the top of this page was clicked, then 102 is returned.
   */
  public int getSelectedIndex() { return menuItemSelected; }
  ...
  @Override protected boolean onClickedOutside(PenEvent event)
  {
    close();
    return true;
  }
  ...
  protected void switchTo(int index)
  {
    ...
    if (index != -1) {
      ...
      pop.titleGap = this.titleGap; // propagate to the drop down since user may
                                    // change our's
      pop.setTextShadowColor(textShadowColor);
      pop.setFont(
        this.font); // guich@350_8: added a fix when the user changes the font
      if (borderStyle == NO_BORDER) {
        pop.setBorderStyle(NO_BORDER);
      }
      pop.setBackForeColors(popBack != -1 ? popBack : backColor,
                            popFore != -1 ? popFore : foreColor);
      if (popCursor != -1) {
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (selected != -1) {
      if (!uiAndroid && uiVista && borderStyle == BORDER_NONE) {
        ...
        g.fillRect(
          xpos[selected], 1, xpos[selected + 1] - xpos[selected], height - 2);
      } else {
        g.backColor = cursorColor != -1
                        ? cursorColor
                        : Color.getCursorColor(bColor); // guich@220_49
        g.fillRect(
          xpos[selected], 1, xpos[selected + 1] - xpos[selected], height - 2);
      }
    }
  ...
  /**
   * Moves to the MenuItem array at left or right depending on the passed value
   * (-1 or +1).
   */
  public void moveBy(int i)
  {
    int newSelected;
    if (i > 0) {
      newSelected = Settings.circularNavigation
  ...
  @Override protected boolean handleFocusChangeKeys(KeyEvent ke)
  ...
  /**
   Sets the style of this menubar to an alternative style (no borders,
   ...
   * The colors are set using a combination of the given back and fore colors.
   You can have a more
   * flexible color selection by using the code below, instead of calling this
   method. Under most
   ...
   * <pre>
          ...
          mbar.setPopColors(0x0078FF, Color.CYAN, -1); // use the default cursor
   color for the popup menu (last -1 param)
   * </pre>
  ...
  public void setAlternativeStyle(int back, int fore)
  {
    ...
    setPopColors(c1 = Color.brighter(back, 32),
                 c2 = Color.darker(fore, 32),
                 Color.interpolate(c1,
                                   c2)); // use the default cursor color for the
                                         // popup menu (last null param)
  }
}