...
public class MenuBar extends Window {
	...
	public void setVisible(boolean b) {
		if (b) {
			setRect(0, 0, FILL, fmH + 4 +
					titleGap); // update the bounds, because the screen may have been changed
			popupNonBlocking();
		}
	}
	...
	public void setAlternativeStyle(int back, int fore) {
		...
		setPopColors(c1 = Color.brighter(back, 32), c2 = Color.darker(fore, 32), Color.interpolate(c1,
					 c2)); // use the default cursor color for the popup menu (last null param)
	}
}