 ...
 * but you need to separate the text with \n. A Label never receives focus
 * neither dispatches events. <p> Note: this class does not do automatic parse
 * of the text. However, you can use a handy method to parse the text that you
 * want to display: see the totalcross.sys.Convert.insertLineBreak method. <p>
...
public class Label extends Control implements TextControl
{
  ...
  protected int alpha;
  /**
   * The text alignment; possible values are LEFT, RIGHT, CENTER, FILL
   * (justifies the text). If align is CENTER and the text is wider than the
   * Label, only the right portion will be lost.
   */
  public int align; // guich@400_71: made public instead of protected
  /**
   * Set to false if you want to scroll the text a line at a time instead of a
   * page at a time.
   */
  ...
  /**
   * Set automatically to true when an empty string is passed in the
   * constructor.
   */
  ...
  /**
   * By default, the getPreferredWidth uses the current text to compute the
   * width. However, if you create a Label with a predefined text that will be
   * changed later, in the advent of a reposition, the preferred width will be
   * recomputed again using the current text and not the predefined one.
   *
  ...
  /**
   * Set to a color to let this Label have a border with that color. Defaults
   * to -1, which means no border. Note that the border affects the Label's size
   * (width and height are increased by 4), so you must set it this field before
   * setting the rect. If you want a bigger gap between border and text, you can
   * use setInsets.
  ...
  /**
   * Specifies a horizontal 3d-gradient background for this Label. Used in the
   * setBorder method.
   * @see #VERTICAL_GRADIENT_BACKGROUND
   * @see #SOLID_BACKGROUND
   * @since TotalCross 1.15
  ...
  /**
   * Set to true to let the label split its text based on the width every time
   * its width changes. If the height is PREFERRED, the Label will change its
   * size accordingly. You may change the height again calling setRect.
  ...
  /**
   * Creates a label displaying the given text. Alignment is set to LEFT by
   * default.<br> Supports inverted text, multiple lines and is scrollable by
   * default.
   *
   * @param text
  ...
  /**
   * Creates a label with the given text, alignment, foreground color, and if
  ...
  public Label(String text, int align)
  {
    if (align != LEFT && align != RIGHT && align != CENTER && align != FILL) {
      throw new IllegalArgumentException(
        "Argument 'align' value must be either LEFT, RIGHT, CENTER or FILL");
    }
  ...
  /**
   * Shows this label as a horizontal marquee. The text must have a single
   * line. To stop the marquee, just call setText. When a window covers the
   * marquee, it is halted, and resumed when the window closes.
   * @param text the text to be displayed
   * @param delay the timer delay in ms used to scroll the marquee. 100 is a
   *     good value.
   * @param loopCount the number of times the text will loop. Set to -1 to loop
   *     forever.
   * When the loop count is reached, the text is cleared.
   * @param step the step in pixels in which the text will be scrolled. If &gt;
   *     0, will
   ...
   * @since TotalCross 1.0 beta 4
   */
  public boolean isMarqueeRunning() { return marqueeTimer != null; }
  ...
  @Override public void onEvent(Event e)
  {
    if (marqueeTimer != null && marqueeTimer.triggered) {
      ...
      if (w != null &&
          w.isTopMost()) { // update only if our window is the one being shown
        marqueeX += marqueeStep;
        repaintNow(); // using this instead of repaint prevents an Edit to be
                      // painted too often, because repaint affects all controls
                      // on screen, while repaintNow affects only this control
      }
    }
  }
  ...
  /**
   * Highlights the text, i.e., paints the text in all directions with a
   * brighter color, then centered, with the foreground color.
  ...
  { // guich@tc110_14
    ...
    highlightColor =
      userHighlightColor != -1 ? userHighlightColor : Color.brighter(backColor);
  }

  /**
   * The color used when highlighting is on. Defaults to a brighter foreground
   * color.
   * @param c The color to be used as highlighted color. Pass -1 to use the
   *     default one.
  ...
  /**
   * Splits the text to the given width. Remember to set the font (or add the
   * Label to its parent) before calling this method.
   * @since TotalCross 1.14
   * @see #autoSplit
   */
  ...
  {                             // guich@tc114_73
    ...
    setText(Convert.insertLineBreak(
      maxWidth,
      fm,
      text)); // guich@tc126_18: text cannot be assigned here or originalText
              // will be overwritten
    originalText = text;
  }
  ...
  @Override public void setText(String text)
  {
    ...
    this.text = autoSplit && width > 0
                  ? Convert.insertLineBreak(this.width, fm, text)
                  : text;
    lines = this.text.equals("")
              ? new String[] { "" }
              : Convert.tokenizeString(this.text,
  ...
  {
    int ret = useFillAsPreferred
                ? FILL
                : preferredWidthText != null
                    ? fm.stringWidth(preferredWidthText)
                    : (getMaxTextWidth() +
                       (insets == null ? 0 : insets.left + insets.right)) +
                        (borderColor == -1 ? 0 : 4);
  ...
  {
    return fmH * lines.length + Edit.prefH + (highlighted ? 2 : 0) +
      (insets == null ? 0 : insets.top + insets.bottom) +
      (borderColor == -1
         ? 0
         : 4); // if inverted, make sure the string is surrounded by the
               // black box - guich@401_18: added commonVGap
  }
  ...
  {
    fColor = Color.interpolateA(
      foreColor,
      backColor,
      alpha); // sholtzer@450_21: added support for setEnabled(false)
    if (is3d) {
      dColor = Color.darker(backColor);
    }
    if (highlighted) {
      highlightColor = userHighlightColor != -1 ? userHighlightColor
  ...
  {
    if (autoSplit && this.width > 0 &&
        this.width !=
          lastASW) { // guich@tc114_74 - guich@tc120_5: only if PREFERRED was
                     // choosen in first setRect - guich@tc126_35
      lastASW = this.width;
  ...
  /**
   * Returns if the label can scroll in the given direction. A Label can scroll
   * if the number of text lines is greater than the actual height.
   */
  public boolean canScroll(boolean down)
  { // guich@200b4_142
    if (lines.length > linesPerPage) {
      return down ? (currentLine + linesPerPage < lines.length)
  ...
  public void scrollToBegin() { currentLine = 0; }

  /** Scroll the text to the end. */
  public void scrollToEnd() { currentLine = lines.length - linesPerPage; }

  /**
   * Scroll to the given line.
   * Can be used to scroll the Label using a ScrollBar. Here's a sample,
   * assuming that sbVert has been added to the container: <pre>
  ...
  {
    if (lines.length > linesPerPage && line != linesPerPage && 0 <= line &&
        line < lines.length) {
      currentLine = line;
      Window.needsPaint = true;
    }
   ...
   * @since TotalCross 1.2
   */
  public int getLinesPerPage() { return this.linesPerPage; }
    ...
    if (drawTranslucentBackground(g, alphaValue)) {
      ...
      if (backgroundType == SOLID_BACKGROUND || !isEnabled()) {
        g.fillRect(0,
                   0,
                   width,
                   height); // guich@200b4_120: make sure the label is painted
        ...
        g.fillShadedRect(0,
                         ...
                         height,
                         true,
                         backgroundType == HORIZONTAL_GRADIENT_BACKGROUND,
                         firstGradientColor,
                         secondGradientColor,
                         100);
    ...
    if (text.length() > 0) {
      ...
      switch (vAlign) {
        case TOP:
          ...
          break;
        case BOTTOM:
          y = this.height - fmH * Math.min(lines.length, linesPerPage) -
              (insets == null ? 0 : insets.bottom);
          break;
        default:
          y = ((this.height - fmH * Math.min(lines.length, linesPerPage)) >>
               1) /* + (insets == null ? 0 : insets.top)*/;
          break; // guich@tc115_34: min of lines.length and linesPerPage
      }
      if (marqueeStep != 0) {
        int shadow = textShadowColor != -1 ? textShadowColor
                                           : highlighted ? highlightColor : -1;
