...
public class Label extends Control implements TextControl {
	...
	public int getPreferredWidth() {
		int ret = useFillAsPreferred ? FILL
				  ...
				  : (getMaxTextWidth() + (insets == null ? 0 : insets.left + insets.right)) +
				  (borderColor == -1 ? 0 : 4);
	...
	public int getPreferredHeight() {
		return fmH * lines.length + Edit.prefH + (highlighted ? 2 : 0) + (insets == null ? 0 : insets.top +
				insets.bottom)
			   + (borderColor == -1 ? 0 :
				  4); // if inverted, make sure the string is surrounded by the black box - guich@401_18: added commonVGap
	}
	...
	protected void onColorsChanged(boolean colorsChanged) {
		fColor = Color.interpolateA(foreColor, backColor,
		...
		if (drawTranslucentBackground(g, alphaValue)) {
			...
			if (backgroundType == SOLID_BACKGROUND || !isEnabled()) {
				g.fillRect(0, 0, width,
						   height); // guich@200b4_120: make sure the label is painted with the correct color
			} else {
		...
		if (text.length() > 0) {
			...
			switch (vAlign) {
				...
				case BOTTOM:
					...
					break;
				default:
					y = ((this.height - fmH * Math.min(lines.length,
													   linesPerPage)) >> 1)/* + (insets == null ? 0 : insets.top)*/;
			...
			if (marqueeStep != 0) {
				...
				for (int i = currentLine; i < n; i++, y += fmH) {
					int justify = align == FILL && (i < lines.length - 1)
								  ? this.width - 1 - (insets == null ? 0 : insets.left + insets.right) :
