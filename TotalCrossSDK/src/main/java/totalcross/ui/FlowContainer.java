...
/**
 * This class is a Container that will place controls one after the other and,
 * once the width has been reached, it wraps to next line.
 *
 * Example:
 * <pre>
 ...
 * platform?"); l.autoSplit = true; add(l, LEFT,AFTER,PARENTSIZE+100,PREFERRED);
 *
 ...
 *
 * When calling setRect for this control, the height must be PREFERRED (with
 * adjustments, if needed).
 *
 * Also, if initUI is overriden, be sure to call <code>super.initUI()</code>.
 *
 * <b>IT IS VERY IMPORTANT THAT YOU USE PARENTSIZE+100 INSTEAD OF FILL IN THE
 * WIDTH PARAMETER!</b>
 *
...
public class FlowContainer extends Container
{
   ...
   * You <b>must</b> add all children controls before calling setRect for this
   * container.
   */
  ...
  @Override public void initUI()
  {
    ...
    int g = Settings.uiAdjustmentsBasedOnFontHeight &&
  ...
  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    if (setW == FILL) {
      throw new RuntimeException(
        "For FlowContainer subclasses, please use PARENTSIZE+100 instead of FILL");
    }
    if (this.width > 0 && this.width != lastASW &&
        ((PREFERRED - RANGE) <= setH && setH <= (PREFERRED + RANGE))) {
      lastASW = this.width;
      initUI();
