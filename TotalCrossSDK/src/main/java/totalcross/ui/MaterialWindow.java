...
public class MaterialWindow extends SlidingWindow
{
    ...
    if (smc != null) {
      ...
      for (Control control2 : children2) {
        if (control2 instanceof Bar) {
          bar.setBackForeColors(control2.getBackColor(),
                                control2.getForeColor());
        }
      }
    }
  ...
  @Override public void setForeColor(int color) { bar.setForeColor(color); }

  @Override public void setBackColor(int color) { bar.setBackColor(color); }
  ...
  public String getTitle() { return bar.getTitle(); }
