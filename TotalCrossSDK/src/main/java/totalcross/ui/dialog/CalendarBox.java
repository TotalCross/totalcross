...
/**
 The Calendar class displays a calendar with buttons used to advance the month
 and the year. It uses the Date class for all operations.<br> Instead of create
 a new instance (which consumes memory), you may use the Edit's static field
 <i>calendar</i> <p> If there is something in the edit box which poped up the
 calendar, the clear button will clear it. Cancel will leave whatever was in
 there. <p> The month can be changed via keyboard using the left/right keys, and
 the year can be changed using up/down keys.
 */
...
public class CalendarBox extends Window
{
  ...
  /**
   * Defines the array length of year window. If array length is 20, and
   * current year is 2010, the list goes from 2000 to 2019.
  ...
  public static String[] weekNames = { "Sun", "Mon", "Tue", "Wed",
                                       "Thu", "Fri", "Sat" }; // guich#573_39
  /** The 12 month names painted in the control.*/
  public static String[] monthNames = { "Jan", "Feb", "Mar", "Apr",
                                        "May", "Jun", "Jul", "Aug",
                                        "Sep", "Oct", "Nov", "Dec" };
  ...
  public CalendarBox()
  {
    ...
    lDay = new Label();
    lDay.setText(weekNames[new Date().getDayOfWeek()] + ", " +
    ...
    pbgDays = new PushButtonGroup(defCaps,
                                  false,
                                  -1,
                                  -1,
                                  fm.charWidth('@') - 2,
  ...
  public CalendarBox(Date date)
  {
    ...
    String[] defCaps = new String[42];
    lDay = new Label();
    lDay.setText(weekNames[date.getDayOfWeek()] + ", " +
                 monthNames[date.getMonth() - 1] + " " + date.getDay());
    for (int i = 0; i < 42; i++) {
      defCaps[i] = Convert.toString(i + 10);
    }
  ...
  { // guich@tc100b5_28
    if (!showYearCalendar) {
      ...
      if (Settings.screenHeight > Settings.screenWidth) {
        ...
        lDay.setFont(
          Font.getFont(true, Font.NORMAL_SIZE + 30 + fontAmountToBeAdjusted));
        lDay.setForeColor(Color.WHITE);
        calendarTitle.add(titleYear,
                          LEFT + leftGap,
                          TOP + Settings.screenHeight * 8 / 664,
                          PREFERRED,
                          PREFERRED);
        calendarTitle.add(lDay,
                          SAME,
                          TOP + Settings.screenHeight * 35 / 664,
                          PREFERRED,
                          PREFERRED);
        boolean adjustFont = false;
        while (lDay.fm.height >
               calendarTitle.getHeight() -
                 (Settings.screenHeight * 8 / 664 + titleYear.fm.height)) {
        ...
        if (adjustFont) {
          lDay.setFont(
            Font.getFont(lDay.getFont().name, true, lDay.getFont().size));
        }
        ...
        lowerCalendar.add(btnMonthPrev,
                          LEFT + mButtonGap,
                          TOP + Settings.screenHeight * 10 / 664,
                          DP + 48,
                          DP + 48);

        displayedMonth = new Label(
          Date.getMonthName(new Date().getMonth()) + " " + year, CENTER);
        displayedMonth.setForeColor(Color.BLACK);
        lowerCalendar.add(
          btnMonthNext, RIGHT - mButtonGap, SAME, DP + 48, DP + 48);
        lowerCalendar.add(
          displayedMonth, AFTER, CENTER_OF, FIT, PREFERRED, btnMonthPrev);
        ...
        lowerCalendar.add(btnOk,
                          RIGHT - Settings.screenWidth * 12 / 664,
                          BOTTOM - Settings.screenHeight * 4 / 664,
                          DP + 64,
                          DP + 48);
        lowerCalendar.add(btnCancel, BEFORE, SAME, DP + 77, DP + 48);
        lowerCalendar.add(pbgDays,
                          CENTER,
                          TOP + Settings.screenHeight * 98 / 664,
                          PARENTSIZE + 86,
                          Settings.screenHeight * 240 / 664,
                          displayedMonth);
        ...
        for (int i = 0; i < 7; i++) {
          ...
          lowerCalendar.add(l,
                            xx + r.x + (r.width - l.getPreferredWidth()) / 2,
                            pbgDays.getY() - l.getPreferredHeight());
        }
      } else {
        lDay.autoSplit = true;
        leftGap = Settings.screenWidth * 16 / 688;
        setRect(
          CENTER, CENTER, SCREENSIZE + (51800 / 688), SCREENSIZE + (3180 / 36));
        add(calendar, LEFT, TOP, FILL, FILL);
        ...
        calendarTitle.add(titleYear,
                          LEFT + leftGap,
                          TOP + Settings.screenHeight * 8 / 360,
                          PREFERRED,
                          PREFERRED);
        calendarTitle.add(lDay,
                          SAME,
                          TOP + Settings.screenHeight * 35 / 360,
        ...
        lowerCalendar.add(btnMonthPrev,
                          LEFT + mButtonGap,
                          TOP - Settings.screenHeight * 4 / 360,
                          DP + 48,
                          DP + 48);
        ...
        lowerCalendar.add(btnOk,
                          RIGHT - Settings.screenWidth * 12 / 664,
                          BOTTOM - Settings.screenHeight * 4 / 688,
                          DP + 64,
                          DP + 48);
        lowerCalendar.add(btnCancel, BEFORE, SAME, DP + 77, DP + 48);
        lowerCalendar.add(pbgDays,
                          CENTER,
                          TOP + Settings.screenHeight * 80 / 360,
                          PARENTSIZE + 90,
                          Settings.screenHeight * 192 / 360,
                          displayedMonth);
      ...
      if (Settings.screenHeight > Settings.screenWidth) {
        ...
        calendar.add(calendarTitle, LEFT, TOP, FILL, SCREENSIZE + (9300 / 664));
        calendar.add(
          buttonsCtnr, LEFT, BOTTOM, FILL, Settings.screenHeight * 56 / 688);
        ...
        int aux = years.size();
        for (int i = 0; i++ < aux;) {
          lowerCalendarSC.add(years.get(i - 1),
                              CENTER,
                              i == 0 ? TOP : AFTER,
                              PARENTSIZE,
                              DP + 57);
          ...
          years.get(i - 1).addPenListener(new PenListener() {
            ...
            @Override public void penDragStart(DragEvent e) { isDrag = true; }
        ...
        calendar.add(calendarTitle, LEFT, TOP, SCREENSIZE + (1680 / 72), FILL);
        calendar.add(
          lowerCalendarSC, AFTER, TOP, FILL, Settings.screenHeight * 246 / 360);
        calendar.add(
          buttonsCtnr, SAME, AFTER, FILL, Settings.screenHeight * 56 / 360);
  ...
  private void updateDays()
  {
    try {
      ...
      for (int i = 0; i < pbgDays.names.length; i++) {
        if ((pbgDays.names[i].equals(sentDay + "") &&
             sentMonth == date.getMonth() && sentYear == date.getYear()) &&
            !pbgDays.names[i].equals("")) {
      ...
      switch (event.type) {
        case KeyEvent.SPECIAL_KEY_PRESS:
          ...
          if (ke.key == SpecialKeys.KEYBOARD_ABC ||
              ke.key == SpecialKeys.KEYBOARD_123) { // closes this window
                                                    // without selecting any day
        ...
        case ControlEvent.PRESSED:
          if (event.target == btnOk) {
            ...
            canceled = true;
            unpop();
          } else if (event.target == pbgDays &&
                     pbgDays.getSelectedIndex() >= 0) {
            try {
              Date date =
                new Date(Convert.toInt(pbgDays.getSelectedItem()), month, year);
              day = date.getDay();
              lDay.setText(weekNames[date.getDayOfWeek()] + ", " +
  ...
  private void incMonth(boolean inc)
  {
    ...
    displayedMonth.setText(
      sb.append(Date.getMonthName(month)).append(' ').append(year).toString());
    updateDays();
  }
  ...
  @Override protected void onUnpop() { setFocus(this); }
