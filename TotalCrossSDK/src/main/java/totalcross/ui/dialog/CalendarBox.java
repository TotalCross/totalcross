...
public class CalendarBox extends Window {
	...
	public CalendarBox() {
		...
		lDay = new Label();
		lDay.setText(weekNames[new Date().getDayOfWeek()] + ", " + monthNames[new Date().getMonth() - 1] +
					 " " + new Date().getDay());
		for (int i = 0; i < 42; i++) {
		...
		pbgDays = new PushButtonGroup(defCaps, false, -1, -1, fm.charWidth('@') - 2, 6, true,
	...
	public CalendarBox(Date date) {
		...
		String[] defCaps = new String[42];
		lDay = new Label();
		lDay.setText(weekNames[date.getDayOfWeek()] + ", " + monthNames[date.getMonth() - 1] + " " +
					 date.getDay());
		for (int i = 0; i < 42; i++) {
			defCaps[i] = Convert.toString(i + 10);
		}
	...
	private void setupUI() { // guich@tc100b5_28
		if (!showYearCalendar) {
			...
			if (Settings.screenHeight > Settings.screenWidth) {
				...
				calendarTitle.add(titleYear, LEFT + leftGap, TOP + Settings.screenHeight * 8 / 664, PREFERRED,
								  PREFERRED);
				calendarTitle.add(lDay, SAME, TOP + Settings.screenHeight * 35 / 664, PREFERRED, PREFERRED);
				boolean adjustFont = false;
				while (lDay.fm.height > calendarTitle.getHeight() - (Settings.screenHeight * 8 / 664 +
						titleYear.fm.height)) {
				...
				lowerCalendar.add(btnMonthPrev, LEFT + mButtonGap, TOP + Settings.screenHeight * 10 / 664, DP + 48,
								  DP + 48);
				...
				lowerCalendar.add(btnOk, RIGHT - Settings.screenWidth * 12 / 664,
								  BOTTOM - Settings.screenHeight * 4 / 664, DP + 64, DP + 48);
				lowerCalendar.add(btnCancel, BEFORE, SAME, DP + 77, DP + 48);
				lowerCalendar.add(pbgDays, CENTER, TOP + Settings.screenHeight * 98 / 664, PARENTSIZE + 86,
								  Settings.screenHeight * 240 / 664, displayedMonth);
				...
				for (int i = 0; i < 7; i++) {
					...
					lowerCalendar.add(l, xx + r.x + (r.width - l.getPreferredWidth()) / 2,
				...
				lDay.setForeColor(Color.WHITE);
				calendarTitle.add(titleYear, LEFT + leftGap, TOP + Settings.screenHeight * 8 / 360, PREFERRED,
								  PREFERRED);
				calendarTitle.add(lDay, SAME, TOP + Settings.screenHeight * 35 / 360, Settings.screenWidth / 6,
								  DP + 80);
				calendarTitle.setBackColor(Color.getRGB(66, 133, 244));
				lowerCalendar.setBackColor(Color.WHITE);
				...
				lowerCalendar.add(btnMonthPrev, LEFT + mButtonGap, TOP - Settings.screenHeight * 4 / 360, DP + 48,
								  DP + 48);
				...
				btnCancel.setBackColor(Color.WHITE);
				lowerCalendar.add(btnOk, RIGHT - Settings.screenWidth * 12 / 664,
								  BOTTOM - Settings.screenHeight * 4 / 688, DP + 64, DP + 48);
				lowerCalendar.add(btnCancel, BEFORE, SAME, DP + 77, DP + 48);
				lowerCalendar.add(pbgDays, CENTER, TOP + Settings.screenHeight * 80 / 360, PARENTSIZE + 90,
								  Settings.screenHeight * 192 / 360, displayedMonth);
			...
			if (Settings.screenHeight > Settings.screenWidth) {
				...
				btnCancel.setForeColor(Color.getRGB(66, 133, 244));
				btnCancel.setBackColor(Color.WHITE);
				buttonsCtnr.add(btnOk, RIGHT - Settings.screenWidth * 12 / 664,
								BOTTOM - Settings.screenHeight * 4 / 688, DP + 64, DP + 48);
	...
	private void updateDays() {
		try {
			...
			for (int i = 0; i < pbgDays.names.length; i++) {
				if ((pbgDays.names[i].equals(sentDay + "") && sentMonth == date.getMonth()
					 && sentYear == date.getYear()) && !pbgDays.names[i].equals("")) {
			...
			switch (event.type) {
				case KeyEvent.SPECIAL_KEY_PRESS:
					...
					if (ke.key == SpecialKeys.KEYBOARD_ABC
						|| ke.key == SpecialKeys.KEYBOARD_123) { // closes this window without selecting any day
