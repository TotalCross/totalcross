...
/**
 * A popup window that displays any Control, given as parameter to the
 * constructor.
 */
...
public class ControlBox extends Window
{
  ...
  /**
   * Constructs a ControlBox with the given parameters, and an Ok and a Cancel
   * buttons. The control may have at least one item, which will be used to
   ...
   * The control may have at least one item, which will be used to determine the
   * preferred size.
   * @param title The window's title.
   * @param text The text that will be displayed in a Label above the control.
   * @param cb The control that will be used to get input from the user.
   ...
   *     PushButtonGroup.
   */
  public ControlBox(String title,
                    String text,
                    Control cb,
   ...
   * @param cb The control that will be used to get input from the user.
   * @param buttonCaptions The button captions that will be used in the
   *     PushButtonGroup, or null to hide them.
   * @param prefW The preferred width for the control. You can also use FILL or
   *     PREFERREED
  ...
  { // guich@tc100b5_28
    ...
    int w = Convert.max(wb,
                        wm,
                        we,
                        titleFont.fm.stringWidth(title != null ? title : "")) +
            6; // guich@200b4_29
    w = Math.min(w,
    ...
    cb.setRect(
      we == FILL ? LEFT + gap : CENTER, AFTER + fmH / 2, we - gap, he, msg);
  }
  ...
  public Control getControl() { return cb; }
