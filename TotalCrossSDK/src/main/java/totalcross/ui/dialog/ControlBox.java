...
public class ControlBox extends Window {
	 ...
	 */
	public ControlBox(String title, String text, Control cb, int prefW, int prefH,
	...
	public ControlBox(String title, String text, Control cb, int prefW, int prefH,
		...
		if (buttonCaptions != null) {
			btns = new PushButtonGroup(buttonCaptions, false, -1, uiAndroid ? fmH / 2 : 4, 6, buttonRows,
									   uiAndroid,
	...
	protected void onPopup() { // guich@tc100b5_28
		...
		int w = Convert.max(wb, wm, we,
							titleFont.fm.stringWidth(title != null ? title : "")) + 6; // guich@200b4_29
		w = Math.min(w,
