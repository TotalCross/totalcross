...
public class FileChooserBox extends Window {
	 ...
	 */
	public boolean showInitialPathOnly = !Settings.onJavaSE
	...
	protected void onPopup() { // guich@tc100b5_28
		...
		tree.setFont(font);
		add(tap = new Container(), LEFT + 2, btRefresh == null ? TOP + 2 : AFTER + 2, FILL - 2, FIT - 5,
			btRefresh);
		if (showPreview) {
	...
	private static void qsort(String thisDir, String[] items, int first,
							  int last) { // guich@tc310: newest first - do not handle folders
		if (first >= last) {
			return;
		}
		int low = first;
	...
	private static String getSortName(String
									  s) { // guich@tc126_9: give priority to folders and case insensitive sort
		s = s.toLowerCase(); // case insensitive
		if (s.endsWith("/")) {
			s = "\1".concat(s);
		}
