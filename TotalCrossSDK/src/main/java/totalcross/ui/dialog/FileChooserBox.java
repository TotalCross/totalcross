...
/**
 A class that shows all folders from a startup one to allow the user select a
 file or a folder.
 ...
 * <pre>
 ...
 This one  "," Cancel "}, new FileChooserBox.Filter()
         {
            public boolean accept(File f) throws IOException
            {
               return f.isDir(); // will only list folders. you may filter by
 ...
 * <ul>
 * <li> You can set a path to be selected initially by setting the
 <code>initialPath</code> property.
 * <li> Set the defaultButton property to allow the selection of an item doing a
 double-click on it.
 * </ul>
 */
...
public class FileChooserBox extends Window
{
  ...
  protected Filter ff;
  /**
   * The default button captions: " Select " and " Cancel ". You can localize
   * them if you want.
   */
  public static String[] defaultButtonCaptions = { " Select ", " Cancel " };
  /**
   * The "Volume: " label that's placed before the volume combo. You can
   * localize it if you want.
   */
  public static String msgVolume = "Volume: ";
  /**
   * The " Refresh " button that's placed after the volume combo. You can
   * localize it if you want.
   */
  public static String msgRefresh = " Refresh ";
  /**
   * The title of a message box that appears if the user tries to access a
   * volume and an error is issued by the operating system. Defaults to "Error".
  ...
  /**
   * The body of a message box that appears if the user tries to access a
   * volume and an error is issued by the operating system. Defaults to "Unable
   * to read the contents of the selected volume. Make sure the volume is
   * mounted and you have enough privileges to query its contents.". You can
   * localize it if you want.
   */
  public static String msgInvalidVolumeMessage =
    "Unable to read the contents of the selected volume. Make sure the volume is mounted and you have enough privileges to query its contents.";
  ...
  /**
   * On Win32, we show by default the root drive and expand to the default
   * path. Set this to false to show the initial path only.
   * @since TotalCross 3.2
   */
  public boolean showInitialPathOnly =
    !Settings.onJavaSE && !Settings.platform.equals(Settings.WIN32);
  ...
  /**
   * The button index that will be choosen if the user makes a double-click in
   * an item. Usually you set this to the index of the "Ok" button.
   */
  ...
  /**
   * Set to true to sort the list showing the newest files first, instead of
   * alphabetical order
   */
  ...
  {
    public LoadOnDemandTree() { super(tmodel); }

    /** Checks if a node was already loaded and load it otherwise */
    @Override public boolean expand(Node node)
    {
  ...
  /**
   * Interface used if you want to filter the files that will be added to the
   * tree.
   */
   ...
   * @param buttonCaptions The button captions that will be used in the
   *     PushButtonGroup
  ...
  public FileChooserBox(String caption, String[] buttonCaptions, Filter ff)
  {
    ...
    if (Settings.isWindowsCE() || isAndroid ||
  ...
  /**
   * Constructs a file chooser with "Select a file" as the window title, and
  ...
  @Override protected void onPopup()
  { // guich@tc100b5_28
    if (children != null) {
    ...
    tree.setFont(font);
    add(tap = new Container(),
        LEFT + 2,
        btRefresh == null ? TOP + 2 : AFTER + 2,
        FILL - 2,
        FIT - 5,
        btRefresh);
    if (showPreview) {
      add(btnPreview = new Button(previewTitle), LEFT + 2, BOTTOM - 2);
      tap.add(preview = new ImageControl(),
              LEFT,
              BOTTOM,
              FILL,
              PARENTSIZE + PREVIEW_HEIGHT);
    ...
    tap.add(
      tree, LEFT, TOP, FILL, showPreview ? PARENTSIZE + 69 : FILL, btRefresh);
  ...
  private void reload(String initialPath)
  {
    try {
      if (showInitialPathOnly) {
        ...
        boolean cut =
          initialPath.length() > 3 && dp != -1; // dont cut if is "c:\"
  ...
  /**
   * Call this method to mount the tree, starting from the given path and
   * volume.
  ...
  private static void qsort(String[] items, int first, int last)
  { // guich@tc126_9: quick sort method
    if (first >= last) {
      return;
    }
    int low = first;
  ...
  private static void qsort(String thisDir, String[] items, int first, int last)
  { // guich@tc310: newest first - do not handle folders
    if (first >= last) {
      return;
    }
    int low = first;
    ...
    while (true) {
      while (high >= low &&
             mid.compareTo(getSortName(thisDir, items[low])) < 0) {
        low++;
      }
      while (high >= low &&
             mid.compareTo(getSortName(thisDir, items[high])) > 0) {
        high--;
      }
      if (low <= high) {
        String temp = items[low];
  ...
  private static String getSortName(String thisDir, String s)
  {
    ...
    try {
      ...
      // ignore. usually trying to access hidden or system files, like
  ...
  {                      // guich@tc126_9: give priority to folders and case
                         // insensitive sort
    s = s.toLowerCase(); // case insensitive
    if (s.endsWith("/")) {
      s = "\1".concat(s);
      ...
      for (int i = 0; i < files.length; i++) {
        ...
        if (ff == null || ff.accept(f)) {
          ...
          if (!isDir) {
            ...
            try {
              if (!f.isEmpty()) {
                nod.add(new Node(f)); // store the file for easy call on
                                      // mountTree when expanding
              }
    ...
    try {
      switch (e.type) {
        case PenEvent.PEN_UP:
          if (defaultButton >= 0 && !(e.target instanceof ScrollBar) &&
        ...
        case ControlEvent.PRESSED:
          if (showPreview && e.target == btnPreview) {
            ...
            tree.setRect(
              KEEP, KEEP, KEEP, PARENTSIZE + (b ? 100 - PREVIEW_HEIGHT : 100));
              ...
              if (showPreview && preview.isVisible()) {
                try {
                  if (!isImage(lastSelected.getNodeName())) {
                    ...
                    byte[] bytes =
                      new File(sbp.toString(), File.READ_ONLY).readAndClose();
            ...
            try {
              ...
              new MessageBox(msgInvalidVolumeTitle, msgInvalidVolumeMessage)
                .popupNonBlocking();
  ...
  public int getPressedButtonIndex() { return selectedIndex; }

  /**
   * Returns the path choosen by the user. If using the default captions and
   * the cancel button was pressed, returns null. The filename is returned with
   * normal (/) slashes; the path also ends with a slash. If multipleSelections
   * is on, a list of paths, separated by comma (,) is returned.
   */
