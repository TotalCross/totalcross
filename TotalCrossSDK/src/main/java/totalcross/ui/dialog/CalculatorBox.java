...
public class CalculatorBox extends Window {
	...
	private void setupUI(boolean isReposition) { // guich@tc100b5_28
		setBackColor(showOperations ? UIColors.calculatorBack :
		...
		numericPad.setRect(SAME, AFTER + 4, SAME,
						   Settings.screenHeight > hh * 8 ? hh * 6 : hh * 4); // guich@571_9
		...
		if (showOperations && pbgOp == null) {
			...
			pbgOp2 = new PushButtonGroup(new String[] { "%" }, false, -1, 2, 12, 1, true,
										 PushButtonGroup.BUTTON);
			pbgOp2.setFont(ff);
			...
			pbgEq = new PushButtonGroup(new String[] { "=" }, false, -1, 2, 12, 1, true,
										PushButtonGroup.BUTTON);
			pbgEq.setFont(ff);
	...
	public void onPopup() {
		...
		cOrig = cOrigDefault != null ? cOrigDefault : c instanceof Edit
				|| c instanceof SpinList ? (Control) c : null;
		setupUI(false);
		clear();
		if (cOrig != null) {
