 ...
 * displays a calculator with six basic operations and a numeric pad.
 */

public class CalculatorBox extends Window
{
  /**
   * Implement this interface to check if the given value is a valid value to
   * be returned. Return null if the value is correct, or a message that will be
   * displayed to the user if not.
   *
  ...
  /**
   * The RangeCheck instance that will be called before the user closes the
   * box.
   */
  ...
  private PushButtonGroup pbgAction, numericPad, pbgArrows, pbgOp, pbgOp2,
    pbgEq;
  ...
  /**
   * Strings used to display the action messages. You can localize these
   * strings if you wish.
   */
  public static String[] actions = {
    ...
    "Ok",
    "Cancel"
  }; // guich@320_44: added reuse button

  /** The default title. */
  public static String defaultTitle = "Numeric Pad";

  /**
   * Defines an optional character to be used in the NumericBox. Replaces the
   * decimal separator / 00 char.
  ...
  /**
   * Set to true to don't replace the original value in the Edit if user
   * pressed Ok.
   */
   ...
   * the next field. The user can still clean the edit by clicking the backspace
   * &lt;&lt; button.
   *
   * The default behaviour calls moveFocusToNextControl. You can change it by
   * overriding the method <code>gotoNext</code>.
   *
  ...
  private void setupUI(boolean isReposition)
  { // guich@tc100b5_28
    setBackColor(showOperations
                   ? UIColors.calculatorBack
                   : UIColors.numericboxBack); // before control definitions!
    ...
    if (!isReposition || edNumber == null) {
      ...
      edNumber = cOrig != null && cOrig instanceof Edit
                   ? ((Edit)cOrig).getCopy()
                   : new Edit();
    ...
    edNumber.setFont(f);
    edNumber.setRect(LEFT + 2,
                     TOP + 4,
                     Math.min(hh * 5, Settings.screenWidth - 20),
                     PREFERRED);

    // positioning arrows
    if (pbgArrows == null) {
      pbgArrows = new PushButtonGroup(new String[] { "<", ">", "<<" },
                                      false,
                                      -1,
                                      2,
                                      12,
    ...
    if (numericPad == null) {
      add(
        numericPad = new PushButtonGroup(
          new String[] {
    ...
    numericPad.setRect(SAME,
                       AFTER + 4,
                       SAME,
                       Settings.screenHeight > hh * 8 ? hh * 6
                                                      : hh * 4); // guich@571_9
    String[] names = numericPad.names;
    ds = Convert.toString(Settings.decimalSeparator);
    names[9] = optionalValue != null ? optionalValue
                                     : edNumber.getMode() == Edit.CURRENCY &&
                                           edNumber.getDecimalPlaces() > 0
                                         ? "00"
                                         : ds;
    numericPad.setNames(names);

    if (pbgAction == null) {
      pbgAction = new PushButtonGroup(
        actions, false, -1, 2, 12, 1, true, PushButtonGroup.BUTTON);
    ...
    if (showOperations && pbgOp == null) {
      ...
      String[] opers = { "+", "-", "*", "÷" };
      pbgOp = new PushButtonGroup(
        opers, false, -1, 2, 12, opers.length, true, PushButtonGroup.NORMAL);
      pbgOp.setFont(ff);
      ...
      pbgOp2 = new PushButtonGroup(new String[] { "%" },
                                   false,
                                   -1,
                                   2,
                                   12,
      ...
      pbgEq = new PushButtonGroup(new String[] { "=" },
                                  false,
                                  -1,
                                  2,
                                  12,
    ...
    numericPad.setBackColor(showOperations ? UIColors.calculatorBack
                                           : UIColors.numericboxBack);
    pbgAction.setBackColor(showOperations ? UIColors.calculatorAction
                                          : UIColors.numericboxAction);
    pbgArrows.setBackColor(showOperations ? UIColors.calculatorAction
                                          : UIColors.numericboxAction);
    edNumber.setBackColor(backColor);
    if (pbgOp != null) {
      pbgEq.setBackColor(showOperations ? UIColors.calculatorAction
                                        : UIColors.numericboxAction);
    }
  }
   ...
   * It can be the first operator, the total computed or null if the user
   * canceled.
   */
  public String getAnswer()
  { // guich@200b4_193: get the 'pasted' answer
    return answer;
  }
  ...
  @Override public void onUnpop() { setFocus(this); }

  @Override public void onPopup()
  {
    Control c = topMost.getFocus();
    cOrig = cOrigDefault != null
              ? cOrigDefault
              : c instanceof Edit || c instanceof SpinList ? (Control)c : null;
    setupUI(false);
    clear();
    if (cOrig != null) {
      String s = cOrig instanceof Edit ? ((Edit)cOrig).getTextWithoutMask()
                                       : ((SpinList)cOrig).getSelectedItem();
  ...
  @Override public void postPopup() { setFocus(edNumber); }
    ...
    try {
      switch (event.type) {
        ...
        case ControlEvent.PRESSED:
          if (pbgEq != null && event.target == pbgEq &&
              pbgEq.getSelectedIndex() != -1) {
            compute(-2);
          } else if (pbgOp != null && event.target == pbgOp &&
                     pbgOp.getSelectedIndex() != -1) {
            compute(pbgOp.getSelectedIndex());
          } else if (pbgOp2 != null && event.target == pbgOp2 &&
                     pbgOp2.getSelectedIndex() != -1) {
            compute(-3);
          } else if (event.target == pbgArrows &&
                     pbgArrows.getSelectedIndex() != -1) {
          ...
          } else if (event.target == pbgAction &&
                     pbgAction.getSelectedIndex() != -1) {
            ...
            if (s != null) {
              if (s.equals(ds) && (clearNext || edNumber.getLength() == 0 ||
                                   edNumber.getText().indexOf(ds) != -1)) {
                return;
              }
              if (s.equals("±")) {
  ...
  @Override public void reposition() { setupUI(true); }
      ...
      switch (selectedIndex) {
        case -3: // %
          if (edNumber.getLength() > 0) {
            double d2 =
              Convert.toDouble(unformat(edNumber.getTextWithoutMask()));
            if (last == null) {
  ...
  private String showResult(double res)
  {
    ...
    if (edNumber.getMode() != Edit.CURRENCY && p != -1 &&
