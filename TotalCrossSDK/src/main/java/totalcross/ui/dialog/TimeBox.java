...
public class TimeBox extends Window
{
  ...
  /**
   * Set to false to disable the buttons instead of hiding them if they are not
   * a valid number to appear at the current position. By default, the buttons
   * are hidden, but setting this to false can disable them instead.
   */
  public static boolean hideIfInvalid = true;

  /** Constructs a TimeBox with time set to midnight. */
  public TimeBox() { this(new Time(0, 0, 0, 0, 0, 0, 0)); }
  ...
  {
    visor.setFont(
      Font.getFont(this.font.name, this.font.style == 1, this.font.size + 2));
  }
  ...
  private class Visor extends Container
  {
    ...
    public Visor()
    {
      ...
      setBackColor(
        TimeBox.uiMaterial
          ? (UIColors.timeboxVisorBack == -1 ? Color.getRGB("9fffd7")
                                             : UIColors.timeboxVisorBack)
          : (UIColors.timeboxVisorBack == -1 ? Color.WHITE
                                             : UIColors.timeboxVisorBack));
    ...
    @Override public void onPaint(Graphics g)
    {
      ...
      g.backColor = UIColors.timeboxVisorCursor == -1
                      ? Color.darker(backColor)
                      : UIColors.timeboxVisorCursor;
      g.fillCircle(pos * w0 + xx - 2 + w0 / 2,
                   ((this.height + fmH) / 2),
                   this.height / 26);
      ...
      for (int i = 0; i < chars.length; i++, xx += w0) {
        g.drawText(
          chars, i, 1, i == 2 || i == 5 ? xx + wp : xx, yy, sh != -1, sh);
      }
    }

    @Override public int getPreferredHeight() { return fmH + 4; }
  }
  ...
  @Override protected void onPopup()
  {
    ...
    setRect(CENTER,
            ...
            Settings.isLandscape()
              ? (Settings.is24Hour ? SCREENSIZE + 85 : SCREENSIZE + 90)
    ...
    int btnSize =
      Settings.is24Hour ? (int)(this.height * 0.14) : (int)(this.height * 0.12);
    ...
    add(btNumbers[2] = new Button("2", Button.BORDER_NONE),
        AFTER + gapW,
        SAME,
        SAME,
        SAME);
    add(btNumbers[3] = new Button("3", Button.BORDER_NONE),
        AFTER + gapW,
        SAME,
        SAME,
        SAME);

    add(btNumbers[4] = new Button("4", Button.BORDER_NONE),
        LEFT + gapW,
        AFTER + gapH,
        SAME,
        SAME);
    add(btNumbers[5] = new Button("5", Button.BORDER_NONE),
        AFTER + gapW,
        SAME,
        SAME,
        SAME);
    add(btNumbers[6] = new Button("6", Button.BORDER_NONE),
        AFTER + gapW,
        SAME,
        SAME,
        SAME);

    add(btNumbers[7] = new Button("7", Button.BORDER_NONE),
        LEFT + gapW,
        AFTER + gapH,
        SAME,
        SAME);
    add(btNumbers[8] = new Button("8", Button.BORDER_NONE),
        AFTER + gapW,
        SAME,
        SAME,
        SAME);
    add(btNumbers[9] = new Button("9", Button.BORDER_NONE),
        AFTER + gapW,
        SAME,
        SAME,
        SAME);

    add(btBack = new Button("<", Button.BORDER_NONE),
        LEFT + gapW,
        AFTER + gapH,
        SAME,
        SAME);
    add(btNumbers[0] = new Button("0", Button.BORDER_NONE),
        AFTER + gapW,
        SAME,
        SAME,
        SAME);
    add(btForw = new Button(">", Button.BORDER_NONE),
        AFTER + gapW,
        SAME,
        SAME,
        SAME);
    ...
    add(btOK = new Button(okCaption, Button.BORDER_NONE),
        LEFT + gapW,
        SAME,
        FIT - gapW,
        PARENTSIZE + 9);
    add(btClear = new Button(clearCaption, Button.BORDER_NONE),
        AFTER + gapW,
        SAME,
        FILL - gapW,
        PARENTSIZE + 9,
    ...
    btOK.setForeColor(
      uiMaterial
        ? (UIColors.timeboxOk == -1 ? Color.darker(visor.getBackColor())
                                    : UIColors.timeboxOk)
    ...
    if (Settings.is24Hour) {
      tabOrder = new Vector(new Control[] { btNumbers[1],
                                            btNumbers[2],
                                            btNumbers[3],
                                            btNumbers[4],
                                            btNumbers[5],
    ...
    String s = Convert.zeroPad(sentTime.hour, 2) + ":" +
               Convert.zeroPad(sentTime.minute, 2) + ":" +
  ...
  @Override public void setForeColor(int c)
  {
    UIColors.timeboxFore = c;
    super.setForeColor(c);
  }

  /**Changing this method will also change the UIColors.timeboxBack and
   * UIColors.timeboxFore attributes*/
  @Override public void setBackForeColors(int back, int fore)
  {
  ...
  @Override protected void postUnpop() { postPressedEvent(); }
