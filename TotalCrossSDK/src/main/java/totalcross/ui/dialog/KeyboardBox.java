...
public class KeyboardBox extends Window
{ // guich@102
  ...
  /**
   * Event issued by this keybard on the onUnpop method. The edit must call
   * pushPosState.
   */
  public static final int KEYBOARD_ON_UNPOP = 1001; // guich@320_34
  /**
   * Event issued by this keybard on the postUnpop method. The edit must call
   * popPosState.
   */
  ...
  public static String names[][] = {
    // guich@400_4: made static and public
  ...
  @Override protected void onPopup()
  {
      ...
      for (int i = 0; i < pbs.length; i++) {
        add(pbs[i] =
              new PushButtonGroup(names[i],
                                  ...
                                  i == TEXT_PAD ? 8 : 1,
                                  i != SPECIAL_PAD,
                                  (i != CAPS_PAD) ? PushButtonGroup.BUTTON
                                                  : PushButtonGroup.CHECK));
        pbs[i].setBackColor(i < CAPS_PAD ? UIColors.keyboardFore
    ...
    add(destControl);
    // repositions the window and the controls based in the control's size. Note
    // that a MultiEdit has a different height of an Edit, so this must be
    // dinamically computed
    int hh =
      pbs[TEXT_PAD].getPreferredHeight() + pbs[CAPS_PAD].getPreferredHeight() +
      destControl.getPreferredHeight() + getPreferredHeight() + 4 * 3 + fmH / 2;
    int ww = Math.min(Settings.screenWidth, Settings.screenHeight) - 4;
    setRect(CENTER, CENTER, ww, hh);
    destControl.setRect(
      CENTER, TOP + 2, Math.min(destRect.width, width - 10), PREFERRED);
  ...
  @Override protected void onUnpop()
  {
    destControl._onEvent(
      new ControlEvent(KEYBOARD_ON_UNPOP, destControl)); // guich@320_34
    destControl.setRect(
  ...
  {
    destControl._onEvent(
      new ControlEvent(KEYBOARD_POST_UNPOP, destControl)); // guich@320_34
    ...
    switch (event.type) {
      ...
      case ControlEvent.PRESSED:
        if (event.target instanceof PushButtonGroup) {
          ...
          if (sel >=
              0) { // guich@200b4_105: fix ArrayIndexOutOfBounds exception
            String st = pb.getSelectedItem();
            if (pb.appId == SPECIAL_PAD || pb.appId == CAPS_PAD ||
                pb.appId == SPACE_PAD) { // special char?
              int key = -1;
              switch (st.charAt(0)) {
                case 'D':
                ...
                case 'C':
                  ...
                  isShift = false;
                  break;
                case '\u00AB': // guich@200b4: char « is not being recognized
                               // correctly in the SPT1700.
                default:
  ...
  { // guich@564_8
    // we assume that the uppercase will have almost the same width of the
    // lowercase
