	...
	protected void onPopup() {
		...
		if (pbs[0] == null) {
			...
			for (int i = 0; i < pbs.length; i++) {
				add(pbs[i] = new PushButtonGroup(names[i], false, -1, i == TEXT_PAD ? glue : 1,
												 i >= CAPS_PAD ? 8 : 4,
												 i == TEXT_PAD ? 8 : 1, i != SPECIAL_PAD,
		...
		int hh = pbs[TEXT_PAD].getPreferredHeight() + pbs[CAPS_PAD].getPreferredHeight() +
				 destControl.getPreferredHeight()
