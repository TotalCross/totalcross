   ...
   This class pops up a Window with a Label, an Edit and some buttons. Good
    ...
    // on some event...
      if (renameDialog == null)
         renameDialog = new InputBox("Project Rename","Please enter the new name
   which will be used for the project:",""); renameDialog.setValue(projectName);
      renameDialog.popup();
      // when window closes...
...
public class InputBox extends Window
{
   ...
   * </pre>
   * Note that if you use the default Ok/Cancel buttons, this mapping is already
   * done.
   * @since TotalCross 1.27
   */
   ...
   * The text used in a Label can be multi-line. If the text is too big, it will
   * be splitted.
   */
  public InputBox(String title, String text, String defaultValue)
  {
    this(
      title, text, defaultValue, new String[] { "Ok", "Cancel" }, false, 4, 6);
  ...
  @Override protected void onPopup()
  {
    ...
    if (text.indexOf('\n') < 0 &&
        fm.stringWidth(text) > Settings.screenWidth - 6) {
      text = Convert.insertLineBreak(
        Settings.screenWidth - 6, fm, text.replace('\n', ' '));
    }
    msg = new Label(text, labelAlign);
    msg.setFont(font);
    ed.setFont(font);
    btns = new PushButtonGroup(buttonCaptions,
                               false,
                               -1,
                               gap,
                               insideGap,
                               1,
                               allSameWidth || uiAndroid,
                               PushButtonGroup.BUTTON);
    btns.setFont(font);
    int wb = btns.getPreferredWidth();
    if (wb > Settings.screenWidth - 10) { // guich@tc123_38: buttons too large?
                                          // place them in a single column
      btns = new PushButtonGroup(buttonCaptions,
                                 false,
                                 -1,
                                 gap,
                                 insideGap,
                                 buttonCaptions.length,
                                 true,
                                 PushButtonGroup.BUTTON);
      btns.setFont(font);
    ...
    int wm = Math.min(msg.getPreferredWidth() + (uiAndroid ? fmH : 1),
    ...
    int w =
      Convert.max(wb, wm, we, fm2.stringWidth(title != null ? title : "")) +
      6; // guich@200b4_29
    w = Math.min(w,
    ...
    if (uiAndroid) {
      btns.setRect(buttonCaptions.length > 1 ? LEFT + 3 : CENTER,
                   AFTER + 3,
                   buttonCaptions.length > 1 ? FILL - 3 : Math.max(w / 3, wb),
                   FILL - 3);
    } else {
    ...
    if (btns != null) {
      btns.setBackForeColors(
        UIColors.inputboxAction,
        Color.getBetterContrast(UIColors.inputboxAction,
                                foreColor,
                                backColor)); // guich@tc123_53
    }
  }
   ...
   */
  public void setTextAlignment(int align) { labelAlign = align; }
  ...
  public String getValue() { return ed.getText(); }

  /** Sets the default value on the Edit field */
  public void setValue(String value) { ed.setText(value); }
  ...
  public void setEdit(Edit ed) { this.ed = ed; }
}