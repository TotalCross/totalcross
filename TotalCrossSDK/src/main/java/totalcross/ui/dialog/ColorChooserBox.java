...
/**
 * Shows a color dialog that can be used to select a color. There are several
 * ways to choose a color: <ul> <li> Using a color matrix <li> Using a websafe
 * palette <li> Writting the red, green and blue components <li> Writting the
 * color in hexdecimal.
 * </ul>
...
public class ColorChooserBox extends Window implements PenListener
{
  ...
  @Override public void initUI()
  {
    ...
    alc = new AlignedLabelsContainer(
      new String[] { "Palette: ", "Red: ", "Green: ", "Blue: ", "Hex: " });
    alc.foreColors =
      new int[] { 0xAA00AA, 0xAA0000, 0x007700, 0x0000AA, 0x222222 };
    add(alc, LEFT, TOP + 2, WILL_RESIZE, WILL_RESIZE);
    alc.add(cbColors = new ComboBox(new ColorList()),
            LEFT,
            alc.getLineY(0),
            PREFERRED,
            fmH + Edit.prefH);
    ...
    alc.setRect(RIGHT - 2,
                KEEP,
                KEEP,
                KEEP); // right can't be used with WILL_RESIZE

    addColorMatrix();

    add(pbgNum = new PushButtonGroup(
    ...
    add(pbgAlpha = new PushButtonGroup(
          new String[] { "A", "B", "C", "D", "E", "F" }, 2, 1),
        SAME,
        AFTER + 2,
        SAME,
        PREFERRED + extra / 2);
    ...
    add(pbgAction = new PushButtonGroup(new String[] { "Ok", "Cancel" }, 2, 1),
        SAME,
        BOTTOM - 2,
        SAME,
        PREFERRED + extra / 2);
  ...
  private void addColorMatrix()
  {
    try {
      ...
      add(ic = new ImageControl(colorMatrix),
          LEFT + 2,
          TOP + 2,
          colorMatrix.getWidth(),
          colorMatrix.getHeight());
  ...
  @Override protected void reposition(boolean recursive)
  {
  ...
  @Override public void onEvent(Event e)
  {
    switch (e.type) {
      case ControlEvent.PRESSED:
        ...
        } else if (e.target instanceof PushButtonGroup && _focus != null &&
                   _focus instanceof Edit) {
      ...
      case ControlEvent.FOCUS_IN:
        ...
        pbgNum.setVisible(e.target == edRGB || e.target == edR ||
                          e.target == edG || e.target == edB);
        break;
  ...
  @Override public void penDrag(DragEvent e) { penDown(e); }
