...
public class ColorChooserBox extends Window implements PenListener {
	...
	public void initUI() {
		...
		alc.add(cbColors = new ComboBox(new ColorList()), LEFT, alc.getLineY(0), PREFERRED,
				fmH + Edit.prefH);
		...
		add(pbgNum = new PushButtonGroup(new String[] { "1", "2", "3", "4", "5", "6", "7", "8", "9", "0" },
										 2, 2), SAME,
			AFTER + 4, SAME, PREFERRED + extra, alc);
		pbgNum.setFocusLess(true);
		pbgNum.setVisible(false);
		add(pbgAlpha = new PushButtonGroup(new String[] { "A", "B", "C", "D", "E", "F" }, 2, 1), SAME,
			AFTER + 2, SAME,
			PREFERRED + extra / 2);
	...
	private void addColorMatrix() {
		try {
			...
			add(ic = new ImageControl(colorMatrix), LEFT + 2, TOP + 2, colorMatrix.getWidth(),
