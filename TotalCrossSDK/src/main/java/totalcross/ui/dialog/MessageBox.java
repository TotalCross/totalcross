...
public class MessageBox extends Window {
	 ...
	 */
	private boolean
	allSameWidth; //flsobral@tc126_50: set to make all buttons to always have the same width.
	...
	private MessageBox() {
		...
		setRect(CENTER, CENTER, Math.min(PREFERRED_WIDTH,
										 Settings.screenWidth - paddingLeft - paddingRight), PREFERRED);
	}
	...
	public MessageBox(Image image, String title, String text, String[] buttonCaptions, int gap,
					  int insideGap) {
	...
	public MessageBox(Image image, String title, String text, String[] buttonCaptions,
					  boolean allSameWidth,
					  int gap, int insideGap) { // andrew@420_5
	...
	public void onPopup() {
		...
		if (title[0] != null) {
			...
			if (uiMaterial && hasTitle) {
				add(title[0], paddingLeft, paddingTop + (image == null ? 0 : AFTER), FILL - paddingRight,
					title[0].fm.height, image == null ? null : imageControl);
		...
		int w = lgap + Convert.max(wb, wm,
								   (iconW > 0 ? iconW + fmH : 0) + fm2.stringWidth(hasTitle ? title[0].getText() : "")) +
				7; // guich@200b4_29 - guich@tc100: +7 instead of +6, to fix 565_11
		w = Math.min(w,
					 Settings.screenWidth); // guich@200b4_28: dont let the window be greater than the screen size
		...
		if (!msg.getText().equals("") || usingOwnCont) {
			add(baseContainer, LEFT + paddingLeft, image != null
				|| hasTitle ? AFTER + titleContGap : TOP + paddingTop, FILL - paddingRight, WILL_RESIZE);
			if (!usingOwnCont) {
		...
		if (btns != null && btns[0] != null) {
			...
			if (uiMaterial) {
				btns[multiRow ? 0 : btns.length - 1].setRect(RIGHT - buttonMargin,
						AFTER + buttonMargin + contButtonGap,
						btnWidth > btnMaxW ? btnMaxW : PREFERRED, PREFERRED);
				for (int i = multiRow ? 1 : btns.length - 2; multiRow ? i < btns.length : i >= 0;
					 i = multiRow ? i + 1 : i - 1) {
					btnWidth = btns[i].getPreferredWidth();
					btns[i].setRect(multiRow ? RIGHT - buttonMargin : BEFORE - buttonMargin,
		...
		xa = r.x + r.width - (wa << 1);
		ya = btns != null ? (btns[0].getY() + (btns[0].getHeight() - ha) / 2) :
			 (r.y2() + 3); // guich@570_52: vertically center the arrow buttons if the ok button is present
		msg.setBackForeColors(backColor,
							  messageForeColor == -1 ? UIColors.messageboxMsgFore : messageForeColor);
		if (btns != null) {
			for (Button button : btns) {
	...
	public void onPaint(Graphics g) {
		if (hasScroll) {
			g.drawArrow(xa, ya, ha, Graphics.ARROW_UP, false,
						msg.canScroll(false) ? foreColor : Color.getCursorColor(
							foreColor)); // guich@200b4_143: msg.canScroll
			...
			case KeyEvent.SPECIAL_KEY_PRESS: // guich@200b4_42
				...
				if (ke.isUpKey()) { // guich@330_45
					...
					Window.needsPaint = true; // guich@300_16: update the arrow's state
						   ...
						   && ke.isActionKey()) { // there's a single button and the enter key was pressed?
					selected = 0;
					unpop();
				} else if (buttonKeys != null && captionCount > 0) {
					int k = ke.key;
					for (int i = buttonKeys.length; --i >= 0;) {
						if (buttonKeys[i] == k || (buttonKeys[i] == SpecialKeys.ENTER
	...
	private String[] breakText(String text, FontMetrics fm, int width) {
		...
		labels[1] = StringUtils.shortText(labels[1], font.fm,
