...
/**
 * This class implements a scrollable message box window with customized
 * buttons, delayed unpop and scrolling text. <br>for example, to create an
 * automatic unpop after 5 seconds, do: <pre> MessageBox mb = new
 * MessageBox("TotalCross","TotalCross is the most exciting tool for developing
 * totally cross-platform programs.",null); mb.setUnpopDelay(5000);
 *   mb.popup(mb);
 * </pre>
 */
...
public class MessageBox extends Window
{
  ...
  protected int lgap;
  /**Message box fonts. You can change it the way you want to control the font
   * sizes.*/
  ...
  /**
   * Set at the object creation. if true, all the buttons will have the same
   * width, based on the width of the largest one.<br> Default value is false.
   *
   * @since TotalCross 1.27
   */
  private boolean allSameWidth; // flsobral@tc126_50: set to make all buttons to
                                // always have the same width.

  /**
   * Defines the y position on screen where this window opens. Can be changed
   * to TOP or BOTTOM. Defaults to CENTER.
   * @see #CENTER
   * @see #TOP
   * @see #BOTTOM
   */
  ...
  /**
   * If you set the buttonCaptions array in the construction, you can also set
   * this public field to an int array of the keys that maps to each of the
   * buttons. For example, if you set the buttons to {"Ok","Cancel"}, you can
   * map the enter key for the Ok button and the escape key for the Cancel
   * button by assigning: <pre> buttonKeys = new
   * int[]{SpecialKeys.ENTER,SpecialKeys.ESCAPE};
   * </pre>
   * Note that ENTER is also handled as ACTION, since the ENTER key is mapped to
   * ACTION under some platforms.
   * @since TotalCross 1.27
   */
  ...
  private MessageBox()
  {
    ...
    setRect(CENTER,
            CENTER,
            Math.min(PREFERRED_WIDTH,
                     Settings.screenWidth - paddingLeft - paddingRight),
            PREFERRED);
  }

  /**
   * Constructs a message box with the text and one "Ok" button. The text may be
   * separated by '\n' as the line delimiters; otherwise, it is automatically
   * splitted if its too big to fit on screen.
   */
   ...
   * The text may be separated by '\n' as the line delimiters; otherwise, it is
   * automatically splitted if its too big to fit on screen. if buttonCaptions
   * is null, no buttons are displayed and you must dismiss the dialog by
   * calling unpop or by setting the delay using setUnpopDelay method
   */
  public MessageBox(String title, String text, String[] buttonCaptions)
  {
    this(null,
         ...
         false,
         4,
         UnitsConverter.toPixels(12 + DP));
  }

  public MessageBox(Image image,
                    String title,
                    String text,
   ...
   * automatically splitted if its too big to fit on screen. If buttonCaptions
   * is null, no buttons are displayed and you must dismiss the dialog by
   * calling unpop or by setting the delay using setUnpopDelay method. The
   * parameters allSameWidth is the same as in the constructor for
   * PushButtonGroup.
   *
   * @since TotalCross 1.27
   */
   ...
   * automatically splitted if its too big to fit on screen. If buttonCaptions
   * is null, no buttons are displayed and you must dismiss the dialog by
   * calling unpop or by setting the delay using setUnpopDelay method. The new
   * parameters gap and insideGap are the same as in the constructor for
   * PushButtonGroup.
   *
   ...
   * is null, no buttons are displayed and you must dismiss the dialog by
   * calling unpop or by setting the delay using setUnpopDelay method. The
   * parameters allSameWidth, gap and insideGap are the same as in the
   * constructor for PushButtonGroup.
   *
   * @since TotalCross 1.27
   */
  public MessageBox(Image image,
  ...
  { // andrew@420_5
    ...
    ha = 6 * Settings.screenHeight /
         160;        // guich@450_24: increase arrow size if screen size change
  ...
  public static class Builder
  {
    ...
    /**After you make all of your process of build, call this to return the
     * result.*/
    public MessageBox build() { return mb; }
    ...
    /**This is the content that will be placed between the title and the
     * buttons. You can put anything here. <br>
     * <b>Note: </b>After you set this, the message will not appear since you're
     * putting a Container above it.*/
    ...
    /**Sets the insets of the Container (The container that will be your
     * content).
     * @see setBaseContainer(Container baseContainer)*/
    public Builder setBaseContainerInsets(int left,
                                          int right,
                                          int top,
                                          int bottom)
    {
    ...
    /**Sets the gap between the Container (MessageBox's content) and the
     * buttons.
     * @see setBaseContainer(Container baseContainer)*/
  ...
  public Image getImage(Image image) { return this.image; }

  @Override public void onPopup()
  {
    removeAll();
    int maxW = Math.min(PREFERRED_WIDTH,
                        Settings.screenWidth - paddingLeft - paddingRight);
    ...
    if (uiMaterial && updateFonts && title != null) {
      ...
      title[1].setText(
        StringUtils.shortText(titles[1], titleFont.fm, textWidth));
      updateFonts = false;
    }
    ...
    if (title[0] != null) {
      ...
      if (uiMaterial && hasTitle) {
        add(title[0],
            paddingLeft,
            paddingTop + (image == null ? 0 : AFTER),
            FILL - paddingRight,
            title[0].fm.height,
            image == null ? null : imageControl);
    ...
    if (buttonCaptions == null) {
      ...
      hb = btns[0].getPreferredHeight() +
           (multiRow ? insideGap * buttonCaptions.length : insideGap);
    }
    ...
    boolean removeTitleLine =
      uiAndroid && borderStyle == ROUND_BORDER && !hasTitle;
    if (removeTitleLine) {
      titleGap = 0;
    } else if (uiAndroid) {
      hm += fmH;
    ...
    int captionH =
      (removeTitleLine ? 0 : Math.max(iconH, fm2.height) + titleGap) + 8;
    int ly = captionH - 6;
    ...
    int w = lgap +
            Convert.max(wb,
                        wm,
                        (iconW > 0 ? iconW + fmH : 0) +
                          fm2.stringWidth(hasTitle ? title[0].getText() : "")) +
            7; // guich@200b4_29 - guich@tc100: +7 instead of +6, to fix 565_11
    w = Math.min(w,
                 Settings.screenWidth); // guich@200b4_28: dont let the window
                                        // be greater than the screen size
    ...
    if (!msg.getText().equals("") || usingOwnCont) {
      add(baseContainer,
          LEFT + paddingLeft,
          image != null || hasTitle ? AFTER + titleContGap : TOP + paddingTop,
          FILL - paddingRight,
          WILL_RESIZE);
      if (!usingOwnCont) {
    ...
    if (btns != null && btns[0] != null) {
      ...
      if (uiMaterial) {
        btns[multiRow ? 0 : btns.length - 1].setRect(
          RIGHT - buttonMargin,
          AFTER + buttonMargin + contButtonGap,
          btnWidth > btnMaxW ? btnMaxW : PREFERRED,
          PREFERRED);
        for (int i = multiRow ? 1 : btns.length - 2;
             multiRow ? i < btns.length : i >= 0;
             i = multiRow ? i + 1 : i - 1) {
          btnWidth = btns[i].getPreferredWidth();
          btns[i].setRect(multiRow ? RIGHT - buttonMargin
                                   : BEFORE - buttonMargin,
                          multiRow ? AFTER + buttonMargin : SAME,
                          btnWidth > btnMaxW ? btnMaxW : PREFERRED,
                          PREFERRED);
        }
        if (multiRow) {
        ...
        int end = btns.length;
        if (multiRow) {
          btns[0].setRect(RIGHT - buttonMargin,
                          AFTER + buttonMargin + contButtonGap,
                          btnWidth > btnMaxW ? btnMaxW : PREFERRED,
                          PREFERRED);
          for (int i = 1; i < btns.length; i = i + 1) {
            btnWidth = btns[i].getPreferredWidth();
            btns[i].setRect(RIGHT - buttonMargin,
                            AFTER + buttonMargin,
                            btnWidth > btnMaxW ? btnMaxW : PREFERRED,
                            PREFERRED);
          }
          ...
          for (int i = 0; i < end; i++) {
            if (i == 0) {
              btns[i].setRect(
                LEFT + gap, AFTER + buttonMargin, PREFERRED, PREFERRED);
            } else {
    ...
    xa = r.x + r.width - (wa << 1);
    ya = btns != null
           ? (btns[0].getY() + (btns[0].getHeight() - ha) / 2)
           : (r.y2() + 3); // guich@570_52: vertically center the arrow
                           // buttons if the ok button is present
    msg.setBackForeColors(backColor,
                          messageForeColor == -1 ? UIColors.messageboxMsgFore
                                                 : messageForeColor);
    if (btns != null) {
      for (Button button : btns) {
        button.setForeColor(buttonForeColor == -1 ? UIColors.messageboxAction
                                                  : buttonForeColor);
      }
  ...
  @Override public void reposition() { onPopup(); }

  /**
   * Set an icon to be shown in the MessageBox's title, at left.
   * It only works if there's a title. If you really need an empty title, pass
   * as title a String with a couple of spaces, like " ".
   *
  ...
  {
    this.icon =
      icon.getSmoothScaledInstance(titleFont.fm.ascent, titleFont.fm.ascent);
  }
  ...
  public void setUnpopDelay(int unpopDelay)
  {
    if (unpopDelay <= 0) {
      throw new IllegalArgumentException(
        "Argument 'unpopDelay' must have a positive value");
    }
    if (unpopTimer != null) {
  ...
  @Override public void onPaint(Graphics g)
  {
    if (hasScroll) {
      g.drawArrow(
        ...
        msg.canScroll(false)
          ? foreColor
          : Color.getCursorColor(foreColor)); // guich@200b4_143: msg.canScroll
      g.drawArrow(xa + wa,
                  ya,
                  ha,
                  Graphics.ARROW_DOWN,
                  false,
                  msg.canScroll(true) ? foreColor
                                      : Color.getCursorColor(foreColor));
    }
  }
    ...
    switch (e.type) {
      ...
      case PenEvent.PEN_DOWN:
        if (hasScroll) {
          ...
          if (ya <= py && py <= ya + ha && xa <= px && px < xa + (wa << 1) &&
              msg.scroll((px - xa) / wa != 0)) {
            Window.needsPaint = true;
          } else if (msg.isInsideOrNear(px, py) &&
                     msg.scroll(py > msg.getHeight() / 2)) {
            Window.needsPaint = true;
          }
        }
        break;
      case KeyEvent.SPECIAL_KEY_PRESS: // guich@200b4_42
        ...
        if (ke.isUpKey()) { // guich@330_45
          ...
          msg.scroll(true);
          Window.needsPaint = true; // guich@300_16: update the arrow's state
        } else if (!Settings.keyboardFocusTraversable && captionCount == 1 &&
                   ke.isActionKey()) { // there's a single button and the enter
                                       // key was pressed?
          selected = 0;
          unpop();
          ...
          for (int i = buttonKeys.length; --i >= 0;) {
            if (buttonKeys[i] == k ||
                (buttonKeys[i] == SpecialKeys.ENTER &&
                 k == SpecialKeys.ACTION)) { // handle ENTER as ACTION too
              selected = i;
              unpop();
              break;
            }
  ...
  public int getPressedButtonIndex() { return selected; }

  @Override protected void postPopup()
  {
    if (Settings.keyboardFocusTraversable) { // guich@570_39: use this instead
                                             // of pen less
  ...
  @Override protected void postUnpop()
  {
  ...
  /**
   * Shows the exception, with its name, message and stack trace in a new
   * MessageBox.
  ...
  public static void showException(Throwable t, boolean dumpToConsole)
  {
    ...
    mb.originalText =
      Convert.insertLineBreak(Settings.screenWidth - mb.fmH, mb.font.fm, msg);
    mb.labelAlign = LEFT;
    mb.popup();
  }

  @Override protected void onFontChanged() { updateFonts = true; }
   ...
   */
  public void setDelayToShowButton(int ms) { buttonTimer = addTimer(ms); }
  ...
  private String[] breakText(String text, FontMetrics fm, int width)
  {
    ...
    labels[1] = StringUtils.shortText(
      labels[1],
      font.fm,
      Math.min(PREFERRED_WIDTH - paddingLeft - paddingRight,
               Settings.screenWidth - paddingLeft - paddingRight));
    return labels;
  }
}