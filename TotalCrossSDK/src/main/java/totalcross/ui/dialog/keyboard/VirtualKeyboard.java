...
public class VirtualKeyboard extends Window
{
  ...
  /**
   * This method returns the last created instance of the VirtualKeyboard. In
   * case there isn't one it will return null (This method should only be used
   * to retrieve an instance, not create one).
   *
   * @return
   */
  public static VirtualKeyboard getInstance() { return keyboard; }
  ...
  {
    // This title is big to allow the window to be dragged even if there's no
    // title
    super.titleGap = 0;
    ...
    // Places the keyboard outside the screen with valid width and height for
    // placing children
    setRect(
      100000, 100000, SCREENSIZE, (int)(this.fm.height * 16.5), null, true);
    ...
    valueEdit.setRect(LEFT + ((int)X),
                      TOP + (int)(Y),
                      (int)((WIDTH_BUTTON * 3) + (2 * X)),
                      HEIGHT_BUTTON);
    ...
    clrIcon.setRect(valueEdit.getWidth() - (buttonWidth),
                    TOP + (int)(1.5 * Y) + btLimparY,
                    buttonWidth,
                    buttonWidth);

    btOk.setRect(
      AFTER + ((int)X), SAME, WIDTH_BUTTON, HEIGHT_BUTTON, valueEdit);
  ...
  protected void setRect(boolean screenResized)
  {
    ...
    switch (animDir) {
      ...
      case RIGHT:
        ...
        break;
      default:
        resetSetPositions(); // required to make sure the height gets updated by
                             // the following setRect
  ...
  @Override public void popup()
  {
    setRect(false);
    super.popup();
  }

  @Override protected void postPopup() { isOpen = true; }

  @Override public void postUnpop()
  {
    Window.getTopMost().repaintNow();
    isOpen = false;
  }

  @Override public void unpop()
  {
    if (currentAnimation != null) {
      return;
    }
    ...
    if (animDir == CENTER) {
      ...
      if (fadeOnPopAndUnpop) {
        currentAnimation.with(
          FadeAnimation.create(this, false, null, totalTime));
      }
    }
    currentAnimation.setAnimationFinishedAction(new AnimationFinished() {
      @Override public void onAnimationFinished(ControlAnimation anim)
   ...
   *
   * @return returns a boolean which signals if the letters keyboard should
   *     appear or not
   */
  protected boolean organizarVisibilidadeTecladoAlfa()
  {
    ...
    return (q || w || e || r || t || yTmp || u || i || o || p || a || s || d ||
            f || g || h || j || k || l || z || xTmp || c || v || b || n || m);
  }

  /**
   * Organizes the visible buttons in accordance with the valid characters.
   *
   * @return returns a boolean which signals if the numeric keyboard should
   *     appear or not
   */
  protected boolean organizeNumericKeyboardVisibility()
  {
    ...
    return (zero || um || dois || tres || quatro || cinco || seis || sete ||
            oito || nove);
  }
   ...
   */
  public String getText() { return valueEdit.getText(); }
   ...
   */
  public void setText(String text) { this.valueEdit.setText(text); }

  /**
   * Sets the possible characters to be written on the keyboard. If the value is
   * null, any character can be used.
   */
   ...
   * @param maxLength2
   */
  public void setMaxLength(int maxLength) { valueEdit.setMaxLength(maxLength); }

  public void setMode(byte mode) { valueEdit.setMode(mode); }

  @Override public void setFocus(Control c)
  {
  ...
  @Override public void onEvent(Event event)
  {
    ...
    switch (event.type) {
      ...
      case ControlEvent.PRESSED:
        ...
        else if (target == btCancel || target == alfaKeyboard.btCancel ||
        ...
        else if (target == btDel || target == alfaKeyboard.btDel ||
  ...
  /**
   * Method called when the Ok Button is pressedMétodo chamado ao clicar no
   * botão OK
   */
  ...
  @Override public void screenResized()
  {
    setRect(true);
    reposition(true);
  }
