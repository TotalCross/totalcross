...
public class Slider extends ScrollBar {
		...
		switch (Settings.uiStyle) {
			...
			case Settings.Flat: {
				...
				if (verticalBar) {
					...
					g.draw3dRect(p, 0, s, height, uiMaterial ? Graphics.R3D_FILL : Graphics.R3D_RAISED, false, false,
								 fourColors);
					g.backColor = invertDirection ? empty : filled;
					...
					g.backColor = !invertDirection ? empty : filled;
					g.draw3dRect(uiMaterial ? midBarSize : 0, p, barSize, s,
								 uiMaterial ? Graphics.R3D_FILL : Graphics.R3D_RAISED, false, false, fourColors);
					g.backColor = invertDirection ? empty : filled;
					if (dragBarPos > 0 && drawFilledArea) {
						g.fillRect(k + (uiMaterial ? midBarSize / 2 : 0), p + k,
								   drawFilledArea ? dragBarPos : size - (uiMaterial ? midBarSize * 2 : 0), uiMaterial ? s : s - k - k);
					}
					g.translate(dragBarPos, 0);
				}
				if (drawTicks) {
	...
	public int getPreferredWidth() {
		int ret = uiMaterial ? verticalBar ? 2 * midBarSize + insets.top + insets.bottom
				  ...
				  super.getPreferredWidth(); // guich@300_70: vertical bar always use width; horizontal always use height
	...
	public int getPreferredHeight() {
		int ret = uiMaterial ? verticalBar ? UnitsConverter.toPixels((int)(maximum * valuesPerPixel) + DP)
				  ...
				  super.getPreferredHeight(); // guich@300_70: vertical bar always use width; horizontal always use height
		if (uiMaterial && (ret % 2) == 0) {
			ret++;
		}
		return ret;
