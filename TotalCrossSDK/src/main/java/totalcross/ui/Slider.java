 ...
 * You can set some properties of the slider, like drawTicks, invertDirection
 * and drawFilledArea; read the javadocs of each for more information. You can
 * change the thumb size by setting the <code>minDragBarSize</code> public field
 * and then call setValues or setMaximum or setMinimum method (value must always
 * be ODD!).
 *
...
public class Slider extends ScrollBar
{
  ...
  /**
   * Inverts the direction of the marker. You must set this property before
   * calling setValues (or the other min/max/value/visibleItems set methods) for
   * the first time. For material UI, inverts the color area.
   */
  public boolean invertDirection;
  /**
   * Set to true to draw the ticks. Should be set only when maximum-minimum is
   * much smaller than width. You must set this property before calling
   * setValues (or the other min/max/value/visibleItems set methods) for the
   * first time.
   */
  public boolean drawTicks;
  /**
   * Set to false to don't draw the filled area. You must set this property
   * before calling setValues (or the other min/max/value/visibleItems set
  ...
  @Override protected void recomputeParams(boolean justValue)
  {
    if (size <= 0) {
      return;
    }
    if (!justValue) {
      ...
      dragBarMax = size;
      valuesPerPixel =
        (double)(size - dragBarSize - (uiMaterial ? midBarSize + 1 : 0)) /
        (double)delta;
  ...
  @Override public void onFontChanged() { super.onFontChanged(); }

  /** Returns the drag bar position. */
  public int getDragBarPos(int value)
  {
    return Math.min(dragBarMax,
                    (int)(valuesPerPixel * (value - minimum) + 0.5d)) +
      midBarSize;
  }
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    s = uiMaterial ? barHeight
                   : Math.max(4, verticalBar ? (width / 2) : (height / 2));
    p = verticalBar
          ? (width - s) / 2
          : (height - s) / 2; // guich@tc126_72: center based on bar size
    switch (Settings.uiStyle) {
      ...
      case Settings.Vista: {
        ...
        g.backColor =
          isEnabled() ? sliderColor != -1 ? sliderColor : fourColors[0] : bc;
      ...
      case Settings.Flat: {
        ...
        int filled =
          isEnabled()
            ? (uiMaterial && sliderColor != -1 ? sliderColor : fourColors[0])
            : bc;
        ...
        if (verticalBar) {
          ...
          g.draw3dRect(p,
                       ...
                       s,
                       height,
                       uiMaterial ? Graphics.R3D_FILL : Graphics.R3D_RAISED,
                       false,
                       false,
                       fourColors);
          g.backColor = invertDirection ? empty : filled;
          if (dragBarPos > 0 && drawFilledArea) {
            g.fillRect(p + k,
                       k,
                       uiMaterial ? s : s - k - k,
                       drawFilledArea ? dragBarPos : size);
          }
          g.translate(0, dragBarPos);
        } else {
          g.backColor = !invertDirection ? empty : filled;
          g.draw3dRect(uiMaterial ? midBarSize : 0,
                       p,
                       barSize,
                       s,
                       uiMaterial ? Graphics.R3D_FILL : Graphics.R3D_RAISED,
          ...
          g.backColor = invertDirection ? empty : filled;
          if (dragBarPos > 0 && drawFilledArea) {
            g.fillRect(k + (uiMaterial ? midBarSize / 2 : 0),
                       p + k,
                       drawFilledArea
                         ? dragBarPos
                         : size - (uiMaterial ? midBarSize * 2 : 0),
                       uiMaterial ? s : s - k - k);
          }
          g.translate(dragBarPos, 0);
        }
        ...
        g.backColor =
          isEnabled() ? uiMaterial ? sliderColor : fourColors[0] : bc;
        g.foreColor = isEnabled() ? uiMaterial ? sliderColor : fourColors[1]
                                  : getForeColor();
        if (uiMaterial) {
          boolean f = hasFocus();
          int r = f ? midBarSize : fmH / 4;
          g.drawCircle(
            verticalBar ? width / 2 : 0, verticalBar ? 0 : height / 2, r);
        } else {
          g.fillPolygon(barX, barY, 5);
          g.drawPolygon(barX, barY, 5);
        }
  ...
  @Override public int getPreferredWidth()
  {
    int ret =
      uiMaterial
        ? verticalBar
            ? 2 * midBarSize + insets.top + insets.bottom
            : UnitsConverter.toPixels((int)(maximum * valuesPerPixel) + DP)
        : super.getPreferredWidth(); // guich@300_70: vertical bar always use
    ...
    int ret =
      uiMaterial
        ? verticalBar
            ? UnitsConverter.toPixels((int)(maximum * valuesPerPixel) + DP)
            : 2 * midBarSize + insets.top + insets.bottom
        : super.getPreferredHeight(); // guich@300_70: vertical bar
                                      // always use width; horizontal
                                      // always use height
    if (uiMaterial && (ret % 2) == 0) {
      ret++;
    }
  ...
  @Override protected void updateValue(int pos)
  {
    if (pos > dragBarMax) {
      pos = dragBarMax;
