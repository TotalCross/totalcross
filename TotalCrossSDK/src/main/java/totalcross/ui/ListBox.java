 ...
 *   lb.setRect(LEFT,TOP,PREFERRED,PREFERRED); // use control's preferred width
 ...
 *          if (event.target == lb)
 *             Object element = lb.getSelectedItem(); // in most cases, this is
 * just a String and may be casted to such
 *    }
 * }
 * }
 * </pre>
...
public class ListBox extends Container implements Scrollable
{
  ...
  /**
   * The gap between the icon and the text. Used in IconItem. Defaults to
   * fmH*4/3. If you plan to change this value, do it after calling setFont (if
   * you call it).
   *
  ...
  @Deprecated public boolean drawBorder = true;

  /**
   * Used to show an icon or two and a text. You can mix IconItem with other
   * item types in the ListBox. <br> Tip: The icons are automatically resized by
   * default. If you don't want this, please set autoResizeIcon to false.
   *
   * <br>
   * Example:
   *
  ...
  public static class IconItem
  {
    ...
    /**
     * This receives the item text and left icon. It is highly recommended the
     * icon size be 24DP. <br> Tip: The icons are automatically resized by
     * default. If you don't want this, please set autoResizeIcon to false. <br>
     * @param text     The item text.
     * @param iconLeft The left icon of the item.
     */
    public IconItem(String text, Image iconLeft) { this(text, iconLeft, null); }

    /**
     * This receives the item text and the item left and right icons. It is
     * highly recommended the icons sizes be 24DP. <br> Tip: The icons are
     * automatically resized by default. If you don't want this, please set
     * autoResizeIcon to false. <br>
     * @param text      The item text.
     * @param iconLeft  The left icon of the item.
  ...
  /**
   * When the ListBox has horizontal buttons and its height divided by the
   * button height is greater than this value (10), the horizontal button
   ...
   * ihtBackColors.put(10, 0xAABBCC); // will make line number 10 with back
   * color 0xAABBCC.
   * </pre>
   *
   * Specify a null value if you want to use the default back color (this also
   * makes drawing faster). Note that its up to you to update the hashtable if
   ...
   * ihtForeColors.put(10, Color.RED); // will make line number 10 with fore
   * color RED.
   * </pre>
   *
  ...
  /**
   * The extra height of the horizontal scroll buttons. Defaults 2 in 160x160 or
   * a multiple of it in other resolutions.
   *
   * @see #EXTRA_HEIGHT_FACTOR
   * @see #enableHorizontalScroll()
   */
  public int extraHorizScrollButtonHeight =
    Settings.screenHeight * 2 / 160; // guich@560_11: now depends on the
  // resolution
  ...
  public static double itemHeightFactor =
    uiAndroid ? DEFAULT_ITEM_HEIGHT_FACTOR : 1;
   ...
   *
   * <ul>
   * <li>You can edit the paddings of this control by changing the
  ...
  public boolean scrollContent(int xDelta, int yDelta, boolean fromFlick)
  {
    ...
    if (hFlick) {
      if ((xDelta < 0 && xOffset >= 0) ||
        ...
        if (dragDistanceX <= -itemH || dragDistanceX >= itemH) {
          ...
          xOffset +=
            -offsetDelta *
            itemH; // invert signal to follow weird onPaint implementation
    ...
    if (vFlick) {
      ...
      if ((yDelta < 0 && cur <= sbar.getMinimum()) ||
          (yDelta > 0 && cur >= sbar.getMaximum())) {
        vFlick = false;
      } else {
   ...
   * below the vertical scrollbar. The add, replace and remove operations will
   * be a bit slower because the string's width will have to be computed in
   * order to correctly set the max horizontal scroll.
   *
   * @since SuperWaba 5.6
   * @see #extraHorizScrollButtonHeight
   */
  ...
  public void add(Object[] moreItems)
  {
    ...
    if (itemCount ==
        0) { // guich@310_5: directly assign the array if this listbox is empty
   ...
   */
  @Deprecated public void add(Object[] moreItems, int startAt, int size)
  {
    int realSize =
      moreItems.length < startAt + size ? moreItems.length - startAt : size;
    if (itemCount ==
        0) { // guich@310_5: directly assign the array if this listbox is empty
  ...
  /**
   * Adds the given text to this ListBox, breaking the text if it goes beyond
   * the ListBox' limits, and also breaking if it contains \n. Returns the
   * number of lines. Note that each part of the text is considered a new item.
   * This method is slower than the other <code>add</code> methods.
   *
   * @since TotalCross 1.24
   */
  public int addWrapping(String text)
  { // guich@tc124_21
    ...
    String[] lines =
      Convert.tokenizeString(Convert.insertLineBreak(btnX, fm, text), '\n');
    add(lines);
    return lines.length;
  }
   ...
   * <code>null</code> so they can be garbage collected. <b>Attention!</b> If
   * you used the same object array to initialize two ListBoxes (or ComboBoxes),
   * this method will null both ListBoxes (because they use the same array
   * reference), and you'll get a null pointer exception!
   */
  ...
  { // guich@200final_12: new method
    if (0 <= itemIndex && itemIndex < itemCount) {
      ...
      if (ivWidths != null) { // guich@560_9
        ...
        if (old == -xOffsetMin) { // was this the max offset? recompute the
                                  // remaining ones
          int m = 0;
  ...
  /**
   * Returns the selected item of the Listbox or an empty String Object if none
   * is selected
   */
  public Object getSelectedItem()
  {
    int sel = getSelectedIndex();
    return sel >= 0 ? items.items[sel]
                    : ""; // guich@200b4: handle no selected index yet.
  }

  /**
   * Returns the position of the selected item of the Listbox or -1 if the
   * listbox has no selected index yet.
   */
   ...
   * Used internally
   */
  protected Object[] getItemsArray() { return items.items; }

  /**
   * Returns the index of the item specified by the name, or -1 if not found.
   */
  public int indexOf(Object name) { return items.indexOf(name); }
  ...
  {
    if (0 <= i && i < itemCount &&
        i != selectedIndex /* && height != 0*/) { // guich@tc100: commented
                                                  // height!=0 otherwise Watch's
      ...
      if (offset + vi > ma) {
        offset = Math.max(ma - vi,
                          0); // guich@220_4: fixed bug when the listbox is
                              // greater than the current item count
      }
   ...
   * @since SuperWaba 5.6
   */
  public void selectLast() { selectLast(true); }
  ...
  {
    if (uiMaterial) {
      return (hasIconItemLeft || hasIconItemRight
                ? UnitsConverter.toPixels(112 - biggestIconWidth + DP)
                : UnitsConverter.toPixels(112 + DP)) +
        (hasIconItemLeft ? iconGap + biggestIconWidth : 0) +
        (hasIconItemRight ? iconGap + biggestIconWidth : 0) + paddingLeft +
        paddingRight;
    }
  ...
  @Override public int getPreferredHeight()
  {
    if (uiMaterial) {
      int n = visibleLines == -1
                ? itemCount
                : visibleLines; // guich@tc113_11: use visibleLines if set
    ...
    int lineH = getItemHeight(0);
    int h =
      Math.max(lineH * n, sbar.getPreferredHeight()) + (simpleBorder ? 4 : 6);
    if (ivWidths != null && h < 4 * lineH) {
      h = 4 * lineH;
    }
  ...
  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    ...
    if (ivWidths != null) { // guich@560_9: handle horiz scroll?
      if (btnRight == null) {
        int hh = 3 * fmH / 11;
        super.add(btnRight =
                    new ArrowButton(Graphics.ARROW_RIGHT, hh, foreColor));
        super.add(btnLeft =
                    new ArrowButton(Graphics.ARROW_LEFT, hh, foreColor));
        btnRight.focusTraversable = btnLeft.focusTraversable = false;
        tabOrder.removeElement(
          btnRight); // guich@572_6: remove them from the tabOrder, otherwise
                     // it will block the control navigation in some
                     // situations (AllTests)
        tabOrder.removeElement(btnLeft);
        onColorsChanged(true); // guich@tc111_6
      }
      n =
        (btnRight.getPreferredHeight() + extraHorizScrollButtonHeight + extraHB)
        << 1;
      if (uiFlat) {
    ...
    if (ivWidths != null) { // guich@560_9: handle horiz scroll?
      ...
      btnLeft.setRect(SAME,
                      AFTER - n,
                      SAME,
                      PREFERRED + extraHorizScrollButtonHeight + extraHB,
                      null,
                      screenChanged);
      btnRight.setRect(SAME,
                       AFTER - n,
                       SAME,
                       PREFERRED + extraHorizScrollButtonHeight + extraHB,
                       null,
  ...
  {
    int i;
    c = Convert.toUpperCase(
      c); // dbeers@570_103: make sure that the letter is uppercased.
    int foundIndex = -1; // guich@450_30: fix search when exist repeating
                         // letters (cat, chicken, cow - pressing C 3 times)
    // first search from the next item
    for (i = selectedIndex + 1; i < itemCount; i++) {
      String s = items.items[i].toString(); // guich@220_37
      if (s.length() > 0 &&
          Convert.toUpperCase(s.charAt(0)) == c) { // first letter matches?
        foundIndex = i;
        break;
      }
    }
      ...
      switch (event.type) {
        case ControlEvent.PRESSED:
          if (event.target == sbar) {
            int newOffset = sbar.getValue();
            if (newOffset !=
                offset) { // guich@200final_3: avoid unneeded repaints
              offset = newOffset;
              Window.needsPaint = true;
            }
        ...
        case KeyEvent.SPECIAL_KEY_PRESS:
          ...
          if (Settings.keyboardFocusTraversable && ke.isActionKey()) {
            postPressedEvent();
                     ...
                     ke.isNextKey()) { // guich@220_19 - guich@330_45
            if (Settings.keyboardFocusTraversable) {
              if (ke.isUpKey()) {
                setSelectedIndex(
        ...
        case PenEvent.PEN_UP:
          if (event.target == this &&
              !isScrolling) { // if scrolling, do not end selection
            pe = (PenEvent)event;
            if (Settings.fingerTouch) {
              handleSelection(
                ((pe.y - (simpleBorder ? 3 : 4)) / getItemHeight(0)) +
            ...
            if (isInsideOrNear(pe.x, pe.y) && pe.x < btnX &&
                newSelection < itemCount) {
              postPressedEvent();
            }
            endSelection();
          }
        ...
        case PenEvent.PEN_DOWN:
          scScrolled = false;
          pe = (PenEvent)event;
          if (event.target == this && pe.x < btnX &&
              isInsideOrNear(pe.x, pe.y)) {
            int sel =
              ((pe.y - (simpleBorder ? 3 : 4)) / getItemHeight(0)) + offset;
            if (Settings.fingerTouch) {
  ...
  protected void handleSelection(int newSelection)
  {
    if (newSelection != selectedIndex && newSelection < itemCount) {
      if (transparentBackground) { // guich@tc115_18: on transparent
                                   // backgrounds, we must repaint everything
  ...
  @Override protected void onColorsChanged(boolean colorsChanged)
  {
    ...
    back1 = customCursorColor != -1
              ? customCursorColor
              : (back0 != Color.WHITE)
                  ? backColor
                  : Color.getCursorColor(
                      back0); // guich@300_20: use backColor instead of:
                              // back0.getCursorColor(); // guich@210_19
    if (fColor == back1) {
      fColor = foreColor;
    }
    if (!uiAndroid) {
    ...
    if (uiMaterial) {
        ...
        if (npParts != null && npback == null) {
          try {
            npback = NinePatch.getInstance().getNormalInstance(
              ...
              width,
              height,
              isEnabled() ? back0 : Color.interpolate(back0, parent.backColor),
              false);
      ...
      if (!transparentBackground) {
        g.fillRect(0,
                   0,
                   width,
                   height); // guich@tc115_77: fill till end because the
                            // scrollbar may not being shown
      }
      if (drawBorder) {
        if (uiAndroid) {
          if (npback == null) {
        ...
        if (!uiAndroid) {
          g.draw3dRect(
            0, 0, width, height, Graphics.R3D_CHECK, false, false, fourColors);
        }
      }
      g.foreColor = fColor;
  ...
  @Override public int getEffectW() { return width - (getEffectX() + xOffset); }

  @Override public int getEffectH() { return getItemHeight(tempSelectedIndex); }
                           ...
                           int greatestVisibleItemIndex)
  {
    if (uiMaterial) {
      ...
      for (int i = offset; i < greatestVisibleItemIndex;
           dy += itemHeight, i++) {
        drawItem(g, i, dx, dy); // guich@200b4: let the user extend ListBox and
                                // draw the items himself
      }
  ...
  { // guich@tc100b4_5: use a common routine to prevent errors
    int yy = dy;
    if (uiMaterial) {
      g.setClip(dx + 1,
                yy,
                width,
                Math.min(height - 2 - yy, getItemHeight(0) * visibleItems + 2));
    } else {
      g.setClip(
        dx + 1,
        yy,
        btnX - dx - 2,
        Math.min(
          height - (uiAndroid ? 2 : 1) - yy,
          getItemHeight(0) * visibleItems +
            (uiAndroid ? 1 : 2))); // guich@tc100b5_20: don't let get over the
                                   // border - guich@tc115_77: if scrollbar is
                                   // not shown, use the whole area
    }
  }
  ...
  protected void drawItem(Graphics g, int index, int dx, int dy)
  {
    if (uiMaterial) {
      ...
      if (obj instanceof CustomDrawingItem) {
        ((CustomDrawingItem)obj)
          .onItemPaint(g, dx, dy, width, getItemHeight(index));
        return;
      }
      if (isIconItem) {
        try {
          ...
          if (iconItem.icon != null) {
            if (autoResizeIcon) {
              Image newIcon =
                iconItem.icon.getSmoothScaledInstance(iconSize, iconSize);
              g.drawImage(newIcon,
                          dx,
                          dy + getItemHeight(index) / 2 -
                            newIcon.getHeight() / 2);
            } else {
              g.drawImage(iconItem.icon,
                          dx,
                          dy + getItemHeight(index) / 2 -
                            iconItem.icon.getHeight() / 2);
            }
          }
        } catch (ImageException e) {
          g.drawImage(iconItem.icon,
        ...
        try {
          if (autoResizeIcon) {
            Image newIcon =
              iconItem.iconRight.getSmoothScaledInstance(iconSize, iconSize);
            g.drawImage(newIcon,
                        width - right,
                        dy + getItemHeight(index) / 2 -
                          newIcon.getHeight() / 2);
          } else
            g.drawImage(iconItem.iconRight,
                        width - right,
                        dy + getItemHeight(index) / 2 -
                          iconItem.iconRight.getHeight() / 2);
        } catch (ImageException e) {
          g.drawImage(iconItem.iconRight,
                      width - right,
                      dy + getItemHeight(index) / 2 -
      ...
      if (ihtBackColors != null) {
        ...
        g.fillRect(dx,
                   dy,
                   useFullWidthOnSelection ? btnX : fm.stringWidth(s),
                   getItemHeight(index));
        g.backColor = b;
      }
      ...
      g.drawText(s,
                 dx,
                 dy + (!uiAndroid ? 0 : (getItemHeight(index) - fmH) / 2),
                 textShadowColor != -1,
                 textShadowColor); // guich@402_31: don't test for index out of
                                   // bounds. this will be catched in the caller
      g.foreColor = f;
    } else {
      Object obj = items.items[index];
      if (obj == null) {
  ...
  protected void drawSelectedItem(Graphics g, int index, int dx, int dy, int w)
  {
    ...
    //	            (this.captionIcon == null ? 0 : iconGap +
    // this.captionIcon.getWidth());
    //	    g.drawText(StringUtils.shortText(pop.lb.getText(), font.fm,
    // textWidth), paddingLeft + (this.captionIcon == null ? 0 : iconGap +
    // this.captionIcon.getWidth()), 	            height/2 + paddingTop -
    // paddingBottom - fmH/2);

    g.drawText(uiMaterial ? StringUtils.shortText(getText(), font.fm, w)
                          : getText(),
               dx,
               dy,
               textShadowColor != -1,
  ...
  protected int getItemWidth(int index)
  {
    ...
    Object obj = items.items[index];
    return obj == null ? 0
                       : fm.stringWidth(obj.toString()) +
                           (obj instanceof IconItem ? iconGap : 0);
  }
  ...
  protected void drawCursor(Graphics g, int sel, boolean on)
  {
    if (uiMaterial) {
      if (offset <= sel && sel < visibleItems + offset &&
        ...
        g.foreColor = fColor; // guich@520_15: by using fillRect we must set to
                              // the textcolor
        ...
        g.fillRect(dx - 1,
                   dy + paddingTop - 1,
                   width + 1,
                   ih + 2); // pgr@520_4: if this is an image or an
        ...
        // if (on && getParentWindow() instanceof ComboBoxDropDown && !(this
        // instanceof MultiListBox)) Window.updateScreen(); // guich@tc114_80:
        // update screen before the combobox closes. not comparing with
        // ComboBoxDropDown results in screen FLICKERing - guich@tc115_89:
        // prevent flicker in MultiListBox
      }
    } else {
      if (offset <= sel && sel < visibleItems + offset &&
          sel < itemCount) {  // guich@555_10: fixed in all ui styles
        ...
        int dx = 3; // guich@580_41: cursor must be drawn at 3 or will overwrite
                    // the border on a combobox with PalmOS style
        int dy = 3;
        if (uiFlat) {
          dy--;
        }
        ...
        int ih = getItemHeight(sel);
        dx += xOffset; // guich@552_24: added this to make scroll apply to the
                       // item
        dy += (sel - offset) * ih;
        int sw;
        ...
        g.fillRect(
          ...
          dy - 1,
          sw + 2,
          ih + 2); // pgr@520_4: if this is an image or an antialiased font,
                   // using eraseRect will make it ugly. - guich@552_7: added
                   // -1 to fix cursor not overwriting border.
  ...
  protected boolean hasIconItemLeft()
  {
    for (int i = 0; i < items.size(); i++)
      if (items.items[i] instanceof IconItem &&
          ((IconItem)items.items[i]).icon != null) {
  ...
  protected boolean hasIconItemRight()
  {
    for (int i = 0; i < items.size(); i++)
      if (items.items[i] instanceof IconItem &&
          ((IconItem)items.items[i]).iconRight != null) {
        paddingBorder = IconItem.marginBorder;
        paddingLeft = paddingRight = paddingBorder;
        getPreferredWidth();
        return true;
  ...
  protected int getCursorColor(int index) { return back1; }
   ...
   * Sorts the elements of this ListBox. The current selection is cleared.
   *
   * @param caseless Pass true to make a caseless sort, if the items are
   *     Strings.
   */
  public void qsort(boolean caseless)
  { // guich@tc113_5
    if (size() > 0) {
      Convert.qsort(items.items,
                    0,
                    items.size() - 1,
                    (caseless && items.items[0] instanceof String)
  ...
  /**
   * Check if it is needed to change the xOffsetMin based in the given item's
   * text width
   */
  ...
  private boolean horizontalScroll(boolean toLeft)
  {
    ...
    int newOffset = toLeft ? Math.min(xOffset + step, 0)
                           : Math.max(xOffset - step, xOffsetMin);
  ...
  { // any change here must synchronize with MultiListBox'
    if ((ke.isPrevKey() && !ke.isUpKey()) ||
        (ke.isNextKey() && !ke.isDownKey())) {
      int oldXOffset = xOffset;
      _onEvent(ke);
      return (oldXOffset != xOffset) ? this : null;
    }
    if ((ke.isUpKey() && selectedIndex <= 0) ||
        (ke.isDownKey() && selectedIndex == itemCount - 1)) {
      return null;
    }
    _onEvent(ke);
    return this;
  ...
  /**
   * This method hides the scrollbar if its not needed, i.e., if horizontal
   * scroll is disabled and the preferred height is smaller than the actual
   * height. You may have to call <code>reposition</code> if this method returns
   * true. You can call this method after all items were added.
   *
