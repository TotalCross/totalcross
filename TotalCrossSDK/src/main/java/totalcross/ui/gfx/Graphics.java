/*********************************************************************************
 * TotalCross Software Development Kit * Copyright (C) 1998, 1999 Wabasoft <www.wabasoft.com> * Copyright (C) 2000-2012
 * SuperWaba Ltda. * All Rights Reserved * * This library and virtual machine is distributed in the hope that it will *
 * be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of * MERCHANTABILITY or FITNESS FOR A
 * PARTICULAR PURPOSE. * * This file is covered by the GNU LESSER GENERAL PUBLIC LICENSE VERSION 3.0 * A copy of this
 * license is located in file license.txt at the root of this * SDK or can be downloaded here: *
...
public final class Graphics
{
  ...
  {
    arcPiePointDrawAndFill(xc, yc, rx, ry, startAngle, endAngle, foreColor | alpha, foreColor | alpha, false, true, false); // guich@402_57:
    // was
    // true,false
  }
  ...
  {
    arcPiePointDrawAndFill(xc, yc, rx, ry, startAngle, endAngle, foreColor | alpha, backColor | alpha, true, true, false); // guich@402_57:
    // was
    // true,false
  }
  ...
  {
    arcPiePointDrawAndFill(xc, yc, rx, ry, startAngle, endAngle, foreColor | alpha, backColor | alpha, true, true, true); // guich@402_57:
    // was
    // true,false
  }
   ...
   * @since TotalCross 1.0
   */
  public void drawRoundGradient(int startX, int startY, int endX, int endY, int topLeftRadius, int topRightRadius, int bottomLeftRadius, int bottomRightRadius,
      int startColor, int endColor, boolean vertical)
  {
    if (startX > endX)
    {
      int temp = startX;
  ...
  public void getAnglePoint(int xc, int yc, int rx, int ry, int angle, Coord out)
  {
    ...
    case 2:
      ...
      break;
    case 3:
      int idx = size-(index-3*size)-1; if (idx < 0) {
        idx = 0;
      }
      x = + xPoints[idx];
      y = + yPoints[idx];
  ...
  //////////////////// dstX,dstY
  private void drawSurface(int[] pixels, Object srcSurface, int x, int y, int width, int height, int dstX, int dstY, boolean doClip, int bmpX, int bmpY,
      int bmpW, int bmpH)
  {
  ...
  private void fillPolygon(int[] xPoints1, int[] yPoints1, int base1, int nPoints1, int[] xPoints2, int[] yPoints2, int nPoints2, int c1, int c2,
      boolean gradient)
  {
    int x1, y1, x2, y2, n = 0, temp, numSteps = 0, startRed = 0, startGreen = 0, startBlue = 0, endRed = 0, endGreen = 0, endBlue = 0, redInc = 0,
        greenInc = 0, blueInc = 0, red = 0, green = 0, blue = 0, c = 0, j;
  ...
  {
    int numSteps = 0, startRed = 0, startGreen = 0, startBlue = 0, endRed = 0, endGreen = 0, endBlue = 0, redInc = 0, greenInc = 0, blueInc = 0, red = 0,
        green = 0, blue = 0, c = 0;
    // intermediate terms to speed up loop
    long t1 = (long) rx * rx, t2 = t1 << 1, t3 = t2 << 1;
    long t4 = (long) ry * ry, t5 = t4 << 1, t6 = t5 << 1;
  ...
  private void arcPiePointDrawAndFill(int xc, int yc, int rx, int ry, double startAngle, double endAngle, int c, int c2, boolean fill, boolean pie,
      boolean gradient)
  {
  ...
  static final int[][][] windowBorderAlpha = {
      { // thickness 1
        ...
        { IN, IN, IN, -11, 255, 245, 62 }, { IN, IN, IN, IN, -110, 255, 81 }, { IN, IN, IN, IN, -26, 255, 152 }, { IN, IN, IN, IN, IN, 255, 190 }, },
      { // thickness 2
          { 255, 229, 163, 95, OUT, OUT, OUT }, { 255, 255, 255, 255, 215, OUT, OUT }, { IN, -36, -102, -197, 255, 215, OUT },
          { IN, IN, IN, -36, -191, 255, 122 }, { IN, IN, IN, IN, -77, 255, 179 }, { IN, IN, IN, IN, -32, 255, 229 }, { IN, IN, IN, IN, IN, 255, 255 }, },
      { // thickness 3
            { 255, 199, 128, 10, OUT, OUT, OUT }, { 255, 255, 255, 223, 59, OUT, OUT }, { 255, 255, 255, 255, 255, 81, OUT },
            { IN, -79, -234, 255, 255, 245, 16 }, { IN, IN, -32, -215, 255, 255, 133 }, { IN, IN, IN, -77, 255, 255, 207 },
  ...
  public void drawWindowBorder(int xx, int yy, int ww, int hh, int titleH, int footerH, int borderColor, int titleColor, int bodyColor, int footerColor,
      int thickness, boolean drawSeparators)
  {
    int kx, ky, a, c;
    ...
    {
      drawLine(rectX1, ty, rectX2, ty, interpolate(borderColorR, borderColorG, borderColorB, titleColorR, titleColorG, titleColorB, 64));
      ty++;
      footerH--;
    }
    // footer
  ...
  public void plotLineAA(int x0, int y0, int x1,
      int y1) /* draw a black (0) anti-aliased line on white (255) background */
  {
