 ...
 * <p>
 * Surfaces are objects that implement the GfxSurface interface. MainWindow and
 * Image are both examples of surfaces. <p> Here is an example that uses
 * Graphics to draw a line:
 *
 * <pre>
...
public final class Graphics
{
  ...
  /**
   * Set to true to use antialiase in all drawing operations that draw a
   * diagonal line, such as drawPoligon and drawLine.
   *
  ...
  private int lastXC, lastYC, lastRX, lastRY,
    lastSize; // used by arcPiePointDrawAndFill

  private int pitch;

  /**
   * Alpha value used on operations, shifted by 24 bits left. E.G.: to use an
   * alpha value of 0x80, set this with 0x80000000, or 0x80<<24. The default
   * value is 0xFF000000 and is used only when drawing on Images.
   */
  public int alpha;

  /**
   * States that the next calls to drawText will draw it vertically. We decided
   * to do this using a boolean instead of duplicating all the 10 drawText
   * method calls, to keep it simple.
   *
  ...
  /**
   * used in the draw3dRect method. Note that the <i>simple</i> parameter acts
   * in anothe way: if is true, a menu shaded is drawn; if is false, a listbox
   * shaded is drawn. Also note that the fore color must have been set.
   */
  ...
  private static int ands8Mask[] = { 0x80, 0x40, 0x20, 0x10,
                                     0x08, 0x04, 0x02, 0x01 };

  private int gxPoints[], gyPoints[], axPoints[][], ayPoints[][], anPoints[],
    aBase[];
   ...
   * surface. <p> Only totalcross.ui.Control and totalcross.ui.image.Image
   * classes can have a graphics. Trying to create it with another class will
   * result in a runtime exception. <p> If you are trying to create a graphics
   * object for drawing in a subclass of control, use the getGraphics() method
  ...
  @ReplacedByNativeOnDeploy private void create(GfxSurface surface)
  {
    if (surface instanceof Image) {
      ...
      if (mainWindowPixels == null ||
          mainWindowPixels.length <
            Settings.screenWidth * Settings.screenHeight) {
        mainWindowPixels = new int[Settings.screenWidth *
                                   Settings.screenHeight]; // create the pixels
      }
  ...
  /**
   * Returns true if the source surface is from a Control (if false, its an
   * Image).
   */
  public boolean isControlSurface() { return isControlSurface; }

  /**
   * Fades all window pixels with the given value. The window is not repainted,
   * so, if you fade it to bright, you will have to repaint the window to get
   * the original pixels back. Also, don't forget to call updateWindow after
   * this method. Here's a sample of how to fade the screen to black:
   *
   * <pre>
   * for (int i = 0; i < 256; i++) {
   ...
   *       // repaints every window, from the nearest with the MainWindow size
   * to last parent
   ...
   * </pre>
   *
   * You may want to add a delay in the fade out, since its much faster than the
   * fade in. <br> <br> CAUTION: the code above does not work on Android.
   *
   * @since TotalCross 1.2
   * @see Window#fadeOtherWindows
   * @see Window#fadeValue
   */
  @ReplacedByNativeOnDeploy public static void fadeScreen(int fadeValue)
  {
  ...
  {
    return surface instanceof Image ? ((Image)surface).getPixels()
                                    : mainWindowPixels;
  }
  ...
  public void refresh(int sx, int sy, int sw, int sh, int tx, int ty, Font f)
  {
    int scrW, scrH;
    if (isControlSurface) {
      if (mainWindowPixels.length <
          Settings.screenHeight * Settings.screenWidth) {
        mainWindowPixels = null; // let the gc collect the old array before
                                 // allocating the new one if necessary.
        mainWindowPixels =
          new int[Settings.screenHeight * Settings.screenWidth];
      }
  ...
  /**
   * Returns the palette used when the screen has 8 bpp. You can view the
   * palette online <a href='http://www.superwaba.org/tc/pal685_values.png'
   * target=_blank>here</a>. There's no need to convert your colors to these
   * ones; this will be done on-the-fly by the vm.
   *
   * @since TotalCross 1.0
   */
  ...
  {
    ellipseDrawAndFill(
      xc, yc, rx, ry, foreColor | alpha, foreColor | alpha, false, false);
  }
  ...
  {
    ellipseDrawAndFill(
      xc, yc, rx, ry, backColor | alpha, backColor | alpha, true, false);
  }

  /**
   * Fills a gradient Ellipse, using a gradient from the foreground color to the
   * background color.
   *
   * @param xc x coordinate of the center of the ellipse
   * @param yc y coordinate of the center of the ellipse
  ...
  {
    ellipseDrawAndFill(
      xc, yc, rx, ry, foreColor | alpha, backColor | alpha, true, true);
  }

  /**
   * Draws an arc, using the current foreground color as the outline color. If
   * startAngle < 0 and endAngle > 359 a whole circle is drawn. A pie differs
   * from an arc so the pie connects the end points of the arc to the center,
   * thus making a closed figure.
   *
   ...
   * @param startAngle starting angle of the arc. It must be between 0 and 360
   *     (degrees). 0º is at 3 o'clock.
   * @param endAngle ending angle of the arc. It must be between 0 and 360
   *     (degrees). 0º is at 3 o'clock.
   */
  @ReplacedByNativeOnDeploy
  public void drawArc(int xc, int yc, int r, double startAngle, double endAngle)
  {
    arcPiePointDrawAndFill(xc,
                           yc,
                           r,
                           r,
                           startAngle,
   ...
   * If startAngle < 0 and endAngle > 359 a whole circle is drawn. A pie differs
   * from an arc so the pie connects the end points of the arc to the center,
   * thus making a closed figure.
   *
   * @param xc x coordinate of the center of the circle that contains the arc.
  ...
  /**
   * Fills a pie slice, using the current background color as the fill color and
   * the current foreground color as the outline color. If startAngle < 0 and
   * endAngle > 359 a whole circle is filled.
   *
   * @param xc x coordinate of the center of the circle that contains the arc.
   * @param yc y coordinate of the center of the circle that contains the arc.
   * @param r radix of the circle that contains the pie
  ...
  /**
   * Fills a pie slice, using a gradient from the foreground color to the
   * background color. To draw the outline, you must call drawPie. If startAngle
   * < 0 and endAngle > 359 a whole circle is filled.
   *
   * @param xc x coordinate of the center of the circle that contains the arc.
   * @param yc y coordinate of the center of the circle that contains the arc.
   * @param r radix of the circle that contains the pie
  ...
  /**
   * Draws an elliptical arc, using the current foreground color as the outline
   * color. If startAngle < 0 and endAngle > 359 a whole circle is drawn. A pie
   * differs from an arc so the pie connects the end points of the arc to the
  ...
  /**
   * Draws an elliptical pie slice, using the current foreground color as the
   * outline color. If startAngle < 0 and endAngle > 359 a whole circle is
   * drawn. A pie differs from an arc so the pie connects the end points of the
   * arc to the center, thus making a closed figure.
   *
   * @param xc x coordinate of the center of the circle that contains the arc.
   * @param yc y coordinate of the center of the circle that contains the arc.
   * @param rx x radix of the circle that contains the pie
  ...
  /**
   * Fills an elliptical pie slice, using the current background color as the
   * fill color and the current foreground color as the outline color. If
   * startAngle < 0 and endAngle > 359 a whole circle is filled.
   *
   * @param xc x coordinate of the center of the circle that contains the arc.
   * @param yc y coordinate of the center of the circle that contains the arc.
   * @param rx x radix of the circle that contains the pie
  ...
  /**
   * Fills an elliptical pie slice, using a gradient from the foreground color
   * to the background color. To draw the outline, you must call
   * drawEllipticalPie. If startAngle < 0 and endAngle > 359 a whole circle is
   * filled.
   *
   * @param xc x coordinate of the center of the circle that contains the arc.
   * @param yc y coordinate of the center of the circle that contains the arc.
   ...
   */
  @ReplacedByNativeOnDeploy public void drawCircle(int xc, int yc, int r)
  {
    ellipseDrawAndFill(
      xc, yc, r, r, foreColor | alpha, foreColor | alpha, false, false);
  }
   ...
   * @param r radix of the circle
   */
  @ReplacedByNativeOnDeploy public void fillCircle(int xc, int yc, int r)
  {
    ellipseDrawAndFill(
      xc, yc, r, r, backColor | alpha, backColor | alpha, true, false);
  }

  /**
   * Fills a circle, using a gradient from the foreground color to the
   * background color. To draw the outline, you must call drawCircle.
   *
   * @param xc x coordinate of the center of the circle
   * @param yc y coordinate of the center of the circle
   * @param r radix of the circle
  ...
  {
    ellipseDrawAndFill(
      xc, yc, r, r, foreColor | alpha, backColor | alpha, true, true);
  }

  /**
   * Gets the pixel color at given position in the 0x00RRGGBB format. Note that
   * if the image has an alpha channel, the format is 0xAARRGGBB, where AA is
   * the alpha value; to get the RRGGBB, use <code>pixel & 0xFFFFFF</code>.
   *
   * @param x x coordinate of the pixel
   * @param y y coordinate of the pixel
   * @return the color of the pixel or -1 if the pixel lies outside the clip
   *     bounds.
   */
  @ReplacedByNativeOnDeploy public int getPixel(int x, int y)
  {
    x += transX;
    y += transY;
  ...
  /**
   * Sets the pixel at the specified position, using the current foreground
   * color as the pixel color.
   *
   * @param x x coordinate of the pixel
   * @param y y coordinate of the pixel
   */
  @ReplacedByNativeOnDeploy public void setPixel(int x, int y)
  {
  ...
  private boolean surelyOutsideClip(int x1, int y1, int x2, int y2)
  {
    ...
    y1 += transY;
    y2 += transY;
    return (x1 < cx1 && x2 < cx1) || (x1 > cx2 && x2 > cx2) ||
      (y1 < cy1 && y2 < cy1) || (y1 > cy2 && y2 > cy2);
  }

  /**
   * Draws a line at any direction, using the current foreground color as the
   * line color.
   *
   ...
   */
  @ReplacedByNativeOnDeploy public void drawLine(int Ax, int Ay, int Bx, int By)
  {
    drawLine(Ax, Ay, Bx, By, foreColor | alpha);
  }

  /**
   * Draws a dotted line in any direction. Uses the current foreground color as
   * the 1st, 3rd, ... pixels and the background color as the 2nd, 4th, ...
   * pixels.
   *
   * @param Ax x coordinate of the start of the line.
   * @param Ay y coordinate of the start of the line.
   ...
   * @param By y coordinate of the end of the line.
   */
  @ReplacedByNativeOnDeploy public void drawDots(int Ax, int Ay, int Bx, int By)
  {
   ...
   */
  @ReplacedByNativeOnDeploy public void drawRect(int x, int y, int w, int h)
  {
   ...
   */
  @ReplacedByNativeOnDeploy public void fillRect(int x, int y, int w, int h)
  {
  ...
  /**
   * Fills a polygon, using the current background color as the fill color. The
  ...
  public void fillPolygon(int[] xPoints, int[] yPoints, int nPoints)
  {
    if (nPoints > xPoints.length || nPoints > yPoints.length) {
      throw new ArrayIndexOutOfBoundsException(
        "array index out of range at fillPolygon: " + nPoints);
    } else {
      fillPolygon(xPoints,
                  yPoints,
                  0,
  ...
  /**
   * Fills a vertical gradient polygon, mixing from the foreground color to the
   * background color. The polygon can be convex or concave.
   *
   * @param xPoints the array with the x coordinate points
   * @param yPoints the array with the y coordinate points
   * @param nPoints the total number of point to be drawn
  ...
  public void drawPolygon(int[] xPoints, int[] yPoints, int nPoints)
  {
    if (nPoints > xPoints.length || nPoints > yPoints.length) {
      throw new ArrayIndexOutOfBoundsException(
        "array index out of range at drawPolygon: " + nPoints);
    } else {
  ...
  /**
   * Draws a dotted rectangle. Use the current foreground color as the 1st, 3rd,
   * ... pixels and the background color as the 2nd, 4th, ... pixels.
   *
   * @param x left coordinate of the rectangle
   * @param y top coordinate of the rectangle
   * @param w width of the rectangle. the rectangle is drawn from x to x+w-1.
  ...
  /**
   * Draws a sequence of connected lines, using the current foreground color as
   * the outline color. The lines are <b>not</b> closed.
   *
   * @param xPoints the array with the x coordinate points
   * @param yPoints the array with the y coordinate points
   * @param nPoints the total number of point to be drawn
  ...
  public void drawPolyline(int[] xPoints, int[] yPoints, int nPoints)
  {
    if (nPoints > xPoints.length || nPoints > yPoints.length) {
      throw new ArrayIndexOutOfBoundsException(
        "array index out of range at drawPolyline: " + nPoints);
    } else {
      drawPolygon(xPoints, yPoints, 0, nPoints, foreColor | alpha);
    }
  }
  ...
  {
    drawText(
      new String(chars, chrStart, chrCount), x, y, 0, shadow, shadowColor);
  }

  public void drawText(StringBuffer sb,
                       int chrStart,
                       int chrCount,
                       int x,
                       int y,
  ...
  {
    try {
      drawText(sb.toString().substring(chrStart, chrStart + chrCount),
               x,
               y,
               0,
               shadow,
                       ...
                       int shadowColor)
  {
    try {
      ...
      drawText(sb.toString().substring(chrStart),
               x,
               y,
               justifyWidth,
               shadow,
  ...
  /**
   * Draws a text with the current font and the current foregrount color. On
   * BlackBerry devices, this method is slower than drawText(String,x,y) because
   * it convert the chars to a string.
   *
   ...
   */
  public void drawText(String text, int x, int y) { drawText(text, x, y, 0); }

  /**
   * Draws a VERTICAL text with the current font and the current foregrount
   * color. This is a shorthand of:
   *
   * <pre>
  ...
  /**
   * Draws a text with the current font and the current foregrount color,
   * justifying it to the given width. The text can be vertical if you set the
   * isVerticalText property. Note that vertical text does not allow
   * justification. You can use the justifyWidth to adjust the kerning;
   * depending on the characters you're using, you can decrease the distance
  ...
  public void drawText(String text, int x, int y, int justifyWidth)
  {
    ...
    int rowWIB;
    // speed-up
    totalcross.Launcher.UserFont font =
      (totalcross.Launcher.UserFont)this.font.hv_UserFont;
    ...
    if (justifyWidth > 0 && !isVert) {
      ...
      rem = justifyWidth -
            this.font.fm.stringWidth(text.toCharArray(), 0, chrCount);
      if (rem > 0) {
      ...
      if (ch <= ' ' || ch == 160) {
        if (ch == 160 || ch == ' ' || ch == '\t') {
          if (isVert) {
            ...
            x0 += Launcher.instance.getCharWidth(this.font, ch) +
                  extraPixelsPerChar;
      ...
      switch (font.antialiased) {
        case AA_NO: {
          ...
          for (; r < rmax; start += rowWIB, r++, y++) {
            ...
            for (x = x0; x < xMax; x++) {
              // draw each column
              if (bitmapTable != null &&
                  (bitmapTable[current] & ands8[currentBit]) != 0 &&
                  x >= xMin) {
        ...
        case AA_4BPP: {
          ...
          for (; r < rmax; start += rowWIB, r++, y++) {
            ...
            for (x = x0; x < xMax; x++) {
              // draw each column
              transparency = isLowNibble ? (bitmapTable[current++] & 0xF)
                                         : ((bitmapTable[current] >> 4) & 0xF);
              ...
              if (transparency == 0xF) {
                ...
                i &= 0x00FF00;
                pixels[yy + x] =
                  ((j + (((pxRB - j) * transparency) >> 4)) & 0xFF00FF) |
          ...
          for (; r < rmax; start += rowWIB, r++, y++) {
            ...
            for (x = x0; x < xMax; x++) {
              ...
              if (transparency == 0xFF) {
                ...
                i &= 0x00FF00;
                pixels[yy + x] =
                  ((j + (((pxRB - j) * transparency) >> 8)) & 0xFF00FF) |
  ...
  /**
   * Draws a rectangle with rounded corners, using the current foreground color
   * as the outline color.
   *
   * @param x left coordinate of the rectangle
   * @param y top coordinate of the rectangle
   * @param width width of the rectangle. the rectangle is drawn from x to
   *     x+w-1.
   * @param height height of the rectangle. the rectangle is drawn from y to
   *     y+h-1.
   * @param r radix of the circle at the corners. If its greater than width/2 or
   *     greater than height/2, it will be
  ...
  public void drawRoundRect(int x, int y, int width, int height, int r)
  {
    if (r > (width >> 1) || r > (height >> 1)) {
      r = Math.min(width >> 1,
                   height >>
                     1); // guich@200b4_6: correct bug that crashed the device.
    }
    int w = width - (r << 1);
    int h = height - (r << 1);
  ...
  /**
   * Fills a rectangle with rounded corners, using the current background color
   * as the fill color.
   *
   * @param xx left coordinate of the rectangle
   * @param yy top coordinate of the rectangle
   * @param width width of the rectangle. the rectangle is filled from x to
   *     x+w-1.
   * @param height height of the rectangle. the rectangle is filled from y to
   *     y+h-1.
   * @param r radix of the circle at the corners. If its greater than width/2 or
   *     greater than height/2, it will be
   *           adjusted to the minimum of both values.
  ...
  public void fillRoundRect(int xx, int yy, int width, int height, int r)
  {
    ...
    if (r > (width / 2) || r > (height / 2)) {
      r = Math.min(width / 2,
                   height /
                     2); // guich@200b4_6: correct bug that crashed the device.
    }
   ...
   */
  public void setClip(Rect r) { setClip(r.x, r.y, r.width, r.height); }

  /**
   * Gets the current clipping rectangle.
   *
   * @param r a Rect object where the clip coordinates will be stored and
   *     returned.
   */
  public Rect getClip(Rect r)
  {
    r.x = clipX1 - transX; // guich@200b4_38: fixed getClip - and guich@241_5:
                           // changed + to -
    r.y = clipY1 - transY;
    r.width =
      clipX2 - clipX1; // guich@300_36: removed +1 here and in line below
    r.height = clipY2 - clipY1;
    return r;
  }

  /** Returns the clip's width */
  public int getClipWidth() { return clipX2 - clipX1; }

  /** Returns the clip's height */
  public int getClipHeight() { return clipY2 - clipY1; }

  /**
   * Clips the specified rectangle to the clipping bounds.
   *
   * @param r The coordinates of the rectangle that are to be adjusted to match
   *     the clipping bounds.
   * @return false if the specified rectangle does not intersects the clipping
   *     bounds or the new coordinates of the
   *         clipped rectangle.
   */
  ...
  /**
   * Translates the origin of the of the current coordinate system by the given
   * dx and dy. The final translation coords cannot be lesser than 0.
   *
   * @param dx the delta x of the x coordinate translation. the new x origin
   *     coordinate will be x + dx
   * @param dy the delta y of the y coordinate translation. the new y origin
   *     coordinate will be y + dy
   */
  ...
  /**
   * Copies a rectangular area from a surface to the given coordinates on the
   * current surface. The copy operation is performed by combining pixels
   * according to the setting of the current drawing operation. The same surface
   * can be used as a source and destination to implement quick scrolling.
   *
   ...
   */
  public Coord getTranslation() { return new Coord(transX, transY); }
   ...
   * @param topLeftRadius Radius (in pixels) of the top-left corner. A value of
   *     zero means that this corner will not be
   *           rounded
   * @param topRightRadius Radius (in pixels) of the top-right corner. A value
   *     of zero means that this corner will not
   *           be rounded
   * @param bottomLeftRadius Radius (in pixels) of the bottom-left corner. A
   *     value of zero means that this corner will
   *           not be rounded
   * @param bottomRightRadius Radius (in pixels) of the bottom-right corner. A
   *     value of zero means that this corner
   *           will not be rounded
   ...
   * @param vertical True to be a vertical gradient, false to be a horizontal
   *     one
   * @since TotalCross 1.0
   */
  @ReplacedByNativeOnDeploy
                                ...
                                boolean vertical)
  {
    ...
    int numSteps = Math.max(
      ...
      vertical
        ? (endY - startY)
        : (endX - startX)); // guich@tc110_11: support horizontal gradient -
                            // guich@gc114_41: prevent div by 0 if numsteps is 0
    ...
    int leftOffset = 0;
    int rightOffset = 0;
    boolean hasRadius = (topLeftRadius + topRightRadius + bottomLeftRadius +
                         bottomRightRadius) > 0;
    for (int i = 0; i < numSteps; i++) {
      if (hasRadius) {
        ...
        if (topLeftRadius > 0 && i < topLeftRadius) {
          ...
          leftOffset =
            getOffset(bottomLeftRadius, bottomLeftRadius - (numSteps - i + 1)) -
            1;
        }
        ...
        if (topRightRadius > 0 && i < topRightRadius) {
          ...
          rightOffset = getOffset(bottomRightRadius,
                                  bottomRightRadius - (numSteps - i + 1)) -
                        1;
        }
      ...
      int fc = foreColor =
        (red & 0xFF0000) | ((green >> 8) & 0x00FF00) | ((blue >> 16) & 0xFF);
      if (vertical) {
        drawLine(
          startX + leftOffset, startY + i, endX - rightOffset, startY + i);
        if (rightOffset != 0) {
        ...
        drawLine(
          startX + i, startY + leftOffset, startX + i, endY - rightOffset);
  ...
  {
    foreColor =
      color | 0x14000000; // Color.interpolate(color,getPixel(xx,yy),20);
    setPixel(xx, yy);
  }
  ...
  /**
   * Draws the image in the given position with the given draw operation and
   * back color. Note that the current draw operation is not changed, neither
   * the current back color. <P> Deprecated use: backColor forces the
   * transparentColor of the Image, and is ignored when drawOp is
   * DRAW_PAINT.<BR> <P> Preferred use: when <code>backColor</code> is -1, the
   * transparentColor is the one which has been defined for this Image, using
   * <code>Image.transparentColor</code>. Note that for a GIF Image, the
   * transparent pixel is automatically set from the image's information. <P>
   * The parameter doClip must be used with caution. If false, it will not do
   * any clipping checks (what may improve drawings in 8%). Set it to true only
   * when you're sure that the image will not pass the current surface bounds.
   * Otherwise, be prepared to even <b>hard-reset</b> your device!
   *
   * @since SuperWaba 3.3
   */
  @ReplacedByNativeOnDeploy
  public void drawImage(totalcross.ui.image.Image image,
                        int x,
                        int y,
                        boolean doClip)
  {
  ...
  /**
   * Copies a part of the given source image to here at the given position with
   * the given draw operation and back color. Note that the current draw
   * operation is not changed, neither the current back color. <P> Deprecated
   * use: backColor forces the transparentColor of the Image, and is ignored
   * when drawOp is DRAW_PAINT.<BR> <P> Preferred use: when
   * <code>backColor</code> is -1, the transparentColor is the one which has
   * been defined for this Image, using <code>Image.transparentColor</code>.
   * Note that for a GIF Image, the transparent pixel is automatically set from
   * the image's information. <P> The parameter doClip must be used with
   * caution. If false, it will not do any clipping checks (what may improve
   * drawings in 8%). Set it to true only when you're sure that the image will
   * not pass the current surface bounds. Otherwise, be prepared to even
   * <b>hard-reset</b> your device!
   *
   * @since SuperWaba 3.3
   */
  @ReplacedByNativeOnDeploy
  public void copyImageRect(totalcross.ui.image.Image src,
                            int x,
                            int y,
                            int width,
                            int height,
  ...
  /**
   * Draws an image at the given absolute x and y coordinates. The x and y
   * coordinates must be absolute values; LEFT/RIGHT/TOP/BOTTOM/CENTER are not
   * allowed.
   *
   * @see #copyRect
   */
   ...
   * everytime you change the enabled state or the fore/back colors. This can be
   * easily achieved if you extend the onColorsChanged method (but don't forget
   * to call super.onColorsChanged).
   */
                                     ...
                                     int fourColors[])
  {
    ...
    if (four == null) {
      ...
      four[1] =
        Color.brighter(backColor, Color.LESS_STEP); // inside topLeft - WHITE
      if (four[0] == four[1]) {
        four[1] = Color.darker(four[0], Color.LESS_STEP);
      }
      four[0] = Color.darker(
        four[0], Color.HALF_STEP); // in 16 colors, make the buttons more 3d
      four[2] = Color.brighter(
        foreColor,
        enabled
          ? Color.LESS_STEP
          : Color.FULL_STEP); // inside bottomRight - DARK - guich@tc122_48:
                              // use full_step if not enabled
  ...
  /**
   * Fills a shaded rectangle. Used to draw many Vista user interface style
   * controls
   */
                            ...
                            boolean rotate)
  {
    ...
    if (rotate) {
      ...
      if (invert) {
        ...
        fillShadedRect(
          x, y + mid, width, height - mid, true, rotate, end1, ini1, 100);
      } else {
        fillShadedRect(x, y, width, mid, true, rotate, ini1, end1, 100);
        fillShadedRect(
          x, y + mid, width, height - mid, true, rotate, ini2, end2, 100);
      }
    }
  }

  /**
   * Fills a shaded rectangle. Used to draw many Android user interface style
   * controls
   *
                         ...
                         int[] fourColors)
  {
    if (type == R3D_FILL) {
      ...
      drawLine(
        menu ? 0 : 1, 0, menu ? width - 1 : width - 3, 0, foreColor | alpha);
      drawLine(0, 1, 0, height - 3, foreColor | alpha);
      drawLine(width - 2, 1, width - 2, height - 3, foreColor | alpha);
      drawLine(
        width - 1, menu ? 1 : 2, width - 1, height - 3, foreColor | alpha);
          ...
          switch (type) {
            ...
            case R3D_LOWERED:
              drawVistaRect(x,
                            ...
                            fourColors[3],
                            fourColors[1],
                            fourColors[1]); // guich@tc122_10; 2113 -> 2311
              break;
          }
          break;
      }
  ...
  @Override public String toString()
  {
    Rect r = new Rect();
    getClip(r);
    return super.toString() + ", Clip:" + r +
      ", translation from origin: " + getTranslation();
  }
   ...
   * @param h is the height (or width, if left/right arrows) in pixels. 1st line
   *     has 1 pixel, 2nd line has 3, 3rd line
   ...
   * @param foreColor the foreground color to be used. This Graphic's fore color
   *     is changed after this method is
   *           called.
   *           <p>
   *           Example:
   *
  ...
  /**
   * Gets the coordinates where the specified point lies. This is useful if
   * you're drawing a pie chart and want to know where to draw the label. The
   * computed values are cached to speedup the process.
   *
   * @param xc x coordinate of the center of the circle
   * @param yc y coordinate of the center of the circle
   * @param rx x radix of the circle that contains the point. Maximum rx value
   *     is 8000; above this, the result is
   *           unpredictable.
   * @param ry y radix of the circle that contains the point. Maximum ry value
   *     is 8000; above this, the result is
   *           unpredictable.
   * @param angle angle of the point. It should be between 0 and 360 (degrees).
   *     0º is at 3 o'clock.
                            ...
                            Coord out)
  {
    ...
    if (!sameR) {
      ...
      if (sameR) {
        ...
        // step 1: computes how many points the circle has (computes only 45
        // degrees and mirrors the rest) intermediate terms to speed up loop
      ...
      // step 4: stores all the 1/4 circle in the array. the odd arcs are drawn
      // in reverse order intermediate terms to speed up loop
      t2 = t1 << 1;
      t3 = t2 << 1;
      t8 = t7 << 1;
      t9 = 0;
    ...
    // step 5: computes the start and end indexes that will become part of the
    // arc
  ...
  /**
   * Gets raw RGB data from a rectangular region of the surface this graphic s
   * drawing to and stores it in the provided array.
   *
   ...
   * @throws ArrayIndexOutOfBoundsException If the data array is not big enough
  ...
  /**
   * Sets raw RGB data of a rectangular region of the surface this graphic s
   * drawing to copying it from the provided array. <p> Important: the data is
   * in RAW format, so this method only works if the data was retrieved using
   * getRGB.
   *
   ...
   * @throws ArrayIndexOutOfBoundsException If the data array has not enough RGB
   *     values.
   */
  @ReplacedByNativeOnDeploy
  ...
  /////////////
  // copy the area x,y,width,height of the bitmap bmp with dimensions bmpW,bmpH
  // to the (current active) screen location
  //////////////////// dstX,dstY
  private void drawSurface(int[] pixels,
                           Object srcSurface,
                           int x,
                           int y,
                           int width,
                           ...
                           int bmpH)
  {
    ...
    if (srcSurface instanceof Image) {
      ...
      if (img.hwScaleH != 1 || img.hwScaleW != 1) {
        try {
          img = img.hwScaleW < 1 && img.hwScaleH < 1
                  ? img.smoothScaledBy(img.hwScaleW, img.hwScaleH)
    ...
    try {
      ...
      if (!doClip) {
        /*
         * | Although doClip is not set, we make sure that the area of the
         * bitmap | that we want to copy IS inside its area | If doClip is set,
         * then the clipping computation below ensures | the sanity of the
         * operation (and is quite as fast). | DoClip is important for game
         * sprites where programmers ensure that the | sprite will never pass
         * screen boundaries.
         */
      ...
      int scrPitch = pixels == mainWindowPixels
                       ? Settings.screenWidth
                       : bmpW; // if we're copying from a control, use the
                               // real width instead of the control's width
      int psrc = (bmpY + y) * scrPitch + bmpX + x;
      int pdst = dstY * pitch + dstX;
      int alphaMask =
        srcSurface instanceof Image ? ((Image)srcSurface).alphaMask : 255;
        ...
        if (isSrcScreen) {
          ...
          for (i = width; --i >= 0; dstIdx++) {
            ...
            if (a == 0xFF) {
              ...
              b = (b + 1 + (b >> 8)) >> 8;
              dst[dstIdx] =
                (dst[dstIdx] & 0xFF000000) | (r << 16) | (g << 8) | b;
            }
          }
        }
      }
      ...
      if (!isScaled) {
        Vm.warning("Exception in drawBitmap\n"
                   ...
                   " -> " + dstX + ',' + dstY + ',' + doClip + ")\n"
                   + "clip: " + clipX1 + "," + clipY1 + "," + clipX2 + "," +
                   clipY2 + " - trans: " + transX + "," + transY +
                   ", isScaled: " + isScaled);
        e.printStackTrace();
      }
    }
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////
  // took from gd 2.0.1 beta (http://www.boutell.com/gd/manual2.0.html) from
  // Thomas Boutell
  private void fillPolygon(int[] xPoints1,
                           int[] yPoints1,
                           int base1,
                           int nPoints1,
                           int c1,
                           ...
                           boolean gradient)
  {
    int x1, y1, x2, y2, n = 0, temp, numSteps = 0, startRed = 0, startGreen = 0,
                        startBlue = 0, endRed = 0, endGreen = 0, endBlue = 0,
                        redInc = 0, greenInc = 0, blueInc = 0, red = 0,
                        green = 0, blue = 0, c = 0, j;
    ...
    for (int y = miny; y <= maxy; y++) {
      ...
      if (n >= 2) {
        if (gradient) {
          c = (red & 0xFF0000) | ((green >> 8) & 0x00FF00) |
              ((blue >> 16) & 0xFF);
          red += redInc;
          green += greenInc;
          blue += blueInc;
  ...
  private void drawPolygon(int[] xPoints1,
                           int[] yPoints1,
                           int base1,
                           int nPoints1,
                           int c)
  ...
  // Bresenham algorithm to draw lines (modified by guich to improve performance
  // in vertical and horizontal lines)
  @ReplacedByNativeOnDeploy
  private void drawLine(int Ax, int Ay, int Bx, int By, int c)
  {
    ...
    int dX =
      Math.abs(Bx - Ax); // store the change in X and Y of the line endpoints
    ...
    if (dX == 0 && dY == 0) {
      setPixel(Ax, Ay, c);
    } else if (dY == 0) { // guich: if they are vertical or horizontal lines,
      ...
      if (dX >= dY) { // if X is the independent variable
        int dPr =
          dY << 1; // amount to increment decision if right is chosen (always)
        int dPru =
          dPr - (dX << 1); // amount to increment decision if up is chosen
        ...
        int dPr =
          dX << 1; // amount to increment decision if right is chosen (always)
        int dPru =
          dPr - (dY << 1); // amount to increment decision if up is chosen
  ...
  ////////////////////////////////////////////////////////////////////////////
  // Bresenham algorithm to draw lines. draws one pixel with color c1 and one
  // pixel with color c2
  ...
  // transltates the given coords and returns the intersection between the clip
  // rect and the coords passed. returns 0 if the coords are outside the clip
  // rect
  private boolean translateAndClip(int x, int y, int w, int h)
  {
    ...
    if (x < 0 || y < 0 || h <= 0 || w <= 0) {
      return false; // guich@566_42: check the resulting w/h - guich@tc112_34:
                    // check also x and y
    }
  ...
  {
    return ((x + transX) < clipX1)
      ? (clipX1 - transX)
      : ((x + transX) >= clipX2) ? (clipX2 - transX)
                                 : x; // guich@401_2: added TRANSX/Y
  }

  private int doClipY(int y)
  {
    return ((y + transY) < clipY1)
      ? (clipY1 - transY)
      : ((y + transY) >= clipY2) ? (clipY2 - transY) : y;
  }

  // draws an ellipse incrementally
  private void ellipseDrawAndFill(int xc,
                                  int yc,
                                  int rx,
                                  int ry,
                                  int c1,
                                  ...
                                  boolean gradient)
  {
    int numSteps = 0, startRed = 0, startGreen = 0, startBlue = 0, endRed = 0,
        endGreen = 0, endBlue = 0, redInc = 0, greenInc = 0, blueInc = 0,
  ...
  // draw an elliptical arc from startAngle to endAngle. c is the fill color and
  // c2 is the outline color (if in fill mode - otherwise, c = outline color)
  private void arcPiePointDrawAndFill(int xc,
                                      int yc,
                                      int rx,
                                      int ry,
                                      double startAngle,
                                      ...
                                      boolean gradient)
  {
    ...
    // this algorithm was created by Guilherme Campos Hazan
    double ppd;
    int startIndex, endIndex, index, i, oldX1 = 0, oldY1 = 0, oldX2 = 0,
                                        oldY2 = 0;
    int nq, size = 0;
    boolean checkClipX =
      (xc + rx + transX) > clipX2 ||
      (xc - rx + transX) < clipX1; // guich@340_3 - guich@401_2:
    // added TRANSX/Y
    boolean checkClipY =
      (yc + ry + transY) > clipY2 || (yc - ry + transY) < clipY1;
    if (rx < 0 || ry < 0) {
      return;
    }
    // step 0: correct angle values
    if (startAngle < 0.1 &&
        endAngle > 359.9) { // full circle? use the fastest routine instead
      if (fill) {
        ellipseDrawAndFill(
          xc,
          yc,
          ...
          c2,
          true,
          gradient); // guich@201_2: corrected colors. - guich@401_3: changed
                     // c->c2 and vice versa (line below)
      }
    ...
    // step 0: if possible, use cached results
    int clipFactor =
      clipX1 * 1000000000 + clipX2 * 10000000 + clipY1 * 100000 + clipY2;
    ...
    if (!sameC || !sameR) {
      ...
      // step 4: stores all the circle in the array. the odd arcs are drawn in
      // reverse order intermediate terms to speed up loop
      if (!sameR) {
        t2 = t1 << 1;
        t3 = t2 << 1;
        t8 = t7 << 1;
    ...
    // drawing from angle -30 to +30 ? (startIndex = 781, endIndex = 73,
    // size=854)
    if (startIndex > endIndex) {
      ...
      if (fill) {
        fillPolygon(xPoints,
                    ...
                    gradient ? c : c2,
                    c2,
                    gradient); // lower half, upper half
        fillPolygon(xPoints,
                    yPoints,
                    0,
                    endIndex,
  ...
  private static int interpolate(int color1r,
                                 int color1g,
                                 int color1b,
                                 int color2,
                                 int factor)
                               ...
                               boolean drawSeparators)
  {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      drawLine(
        ...
        x2r,
        yy + i,
        interpolate(borderColorR, borderColorG, borderColorB, c, a)); // top
      ...
      drawLine(
        ...
        xx + i,
        y2r,
        interpolate(borderColorR, borderColorG, borderColorB, c, a)); // left
      ...
      drawLine(
        ...
        x2 - i,
        y2r,
        interpolate(borderColorR, borderColorG, borderColorB, c, a)); // right
    }
    // round corners
    for (int j = 0; j < 7; j++) {
                                 ...
                                 int h)
  {
    ...
    int numSteps = Math.max(
      1,
      Math.min((endY - startY) / 2,
               (endX - startX) /
                 2)); // guich@tc110_11: support horizontal gradient -
                      // guich@gc114_41: prevent div by 0 if numsteps is 0
    int startRed = (startColor >> 16) & 0xFF;
    int startGreen = (startColor >> 8) & 0xFF;
    int startBlue = startColor & 0xFF;
  ...
  /**
   * Draws a set of connected lines from the given x,y coordinates array. You
   * must provide at least 2 points (4 int numbers).
   */
   ...
   * Don't use dithering if Settings.screenBPP is not equal to 16, like on
   * desktop computers. In OpenGL platforms, does not work if the surface is the
   * screen (in other words, works only for images).
   *
   * @since TotalCross 1.53
   */
  @ReplacedByNativeOnDeploy public void dither(int x, int y, int w, int h)
  {
    if (!translateAndClip(x, y, w, h)) {
      return;
    }
  ...
  private void addError(int[] pixel,
                        int x,
                        int y,
                        int w,
                        int h,
