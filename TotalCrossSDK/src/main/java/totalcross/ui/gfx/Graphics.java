...
public final class Graphics {
	...
	private void create(GfxSurface surface) {
		if (surface instanceof Image) {
			...
			if (mainWindowPixels == null
				|| mainWindowPixels.length < Settings.screenWidth * Settings.screenHeight) {
	...
	public void refresh(int sx, int sy, int sw, int sh, int tx, int ty, Font f) {
		...
		if (isControlSurface) {
			if (mainWindowPixels.length < Settings.screenHeight * Settings.screenWidth) {
				mainWindowPixels =
					null; // let the gc collect the old array before allocating the new one if necessary.
	...
	public void drawArc(int xc, int yc, int r, double startAngle, double endAngle) {
		arcPiePointDrawAndFill(xc, yc, r, r, startAngle, endAngle, foreColor | alpha, foreColor | alpha,
							   false, false, false);
	}
	...
	public void fillPie(int xc, int yc, int r, double startAngle, double endAngle) {
		arcPiePointDrawAndFill(xc, yc, r, r, startAngle, endAngle, foreColor | alpha, backColor | alpha,
							   true, true, false);
	}
	...
	public void drawEllipticalArc(int xc, int yc, int rx, int ry, double startAngle, double endAngle) {
		arcPiePointDrawAndFill(xc, yc, rx, ry, startAngle, endAngle, foreColor | alpha, foreColor | alpha,
							   false, false,
							   false);
	}
	...
	public void fillEllipticalPie(int xc, int yc, int rx, int ry, double startAngle, double endAngle) {
		arcPiePointDrawAndFill(xc, yc, rx, ry, startAngle, endAngle, foreColor | alpha, backColor | alpha,
							   true, true,
							   false); // guich@402_57: was true,false
	}
	 ...
	 */
	@ReplacedByNativeOnDeploy
	public void fillEllipticalPieGradient(int xc, int yc, int rx, int ry, double startAngle,
										  double endAngle) {
		arcPiePointDrawAndFill(xc, yc, rx, ry, startAngle, endAngle, foreColor | alpha, backColor | alpha,
							   true, true,
	...
	public void drawText(char chars[], int chrStart, int chrCount, int x, int y, boolean shadow,
						 int shadowColor) {
		drawText(new String(chars, chrStart, chrCount), x, y, 0, shadow, shadowColor);
	}

	public void drawText(StringBuffer sb, int chrStart, int chrCount, int x, int y, boolean shadow,
						 int shadowColor) {
		try {
	...
	public void drawText(StringBuffer sb, int chrStart, int chrCount, int x, int y, int justifyWidth,
						 boolean shadow, int shadowColor) {
		try {
			drawText(sb.toString().substring(chrStart, chrStart + chrCount), x, y, justifyWidth, shadow,
					 shadowColor);
		} catch (StringIndexOutOfBoundsException e) {
	...
	public void copyRect(GfxSurface surface, int x, int y, int width, int height, int dstX, int dstY) {
		...
		if (srcPixels != null) {
			drawSurface(srcPixels, surface, x, y, width, height, dstX, dstY, true, surface.getX(),
						surface.getY(),
	 ...
	 */
	@ReplacedByNativeOnDeploy
	public void drawRoundGradient(int startX, int startY, int endX, int endY, int topLeftRadius,
								  int topRightRadius,
		...
		int numSteps = Math.max(1, vertical ? (endY - startY) : (endX -
	...
	public void drawImage(totalcross.ui.image.Image image, int x, int y, boolean doClip) {
		...
		if (srcPixels != null) {
			drawSurface(srcPixels, image, 0, 0, image.getWidth(), image.getHeight(), x, y, doClip, 0, 0,
						image.getWidth(),
						image.getHeight());
		}
	}
	 ...
	 */
	@ReplacedByNativeOnDeploy
	public void copyImageRect(totalcross.ui.image.Image src, int x, int y, int width, int height,
							  boolean doClip) {
		int[] srcPixels = (int[]) src.getPixels();
		if (srcPixels != null) {
			drawSurface(srcPixels, src, x, y, width, height, 0, 0, doClip, 0, 0, src.getWidth(),
						src.getHeight());
		}
	}
	 ...
	 */
	public static void compute3dColors(boolean enabled, int backColor, int foreColor,
									   int fourColors[]) {
		...
		if (four == null) {
			...
			four[2] = Color.brighter(foreColor,
									 enabled ? Color.LESS_STEP :
									 Color.FULL_STEP); // inside bottomRight - DARK - guich@tc122_48: use full_step if not enabled
	 ...
	 */
	public void drawVistaRect(int x, int y, int width, int height, int topColor, int rightColor,
							  int bottomColor,
							  int leftColor) {
		int x1 = x + 1;
		int y1 = y + 1;
	 ...
	 */
	public void fillVistaRect(int x, int y, int width, int height, int back, boolean invert,
							  boolean rotate) {
		...
		if (rotate) {
			...
			fillShadedRect(x + mid, y, width - mid, height, !invert, rotate, invert ? ini1 : ini2,
						   invert ? end1 : end2, 100);
		} else {
			if (invert) {
	 ...
	 */
	public void fillShadedRect(int x, int y, int width, int height, boolean invert, boolean rotate,
							   int c1, int c2,
							   int factor) {
	 ...
	 * @see #R3D_SHADED
	 */
	public void draw3dRect(int x, int y, int width, int height, byte type, boolean yMirror,
						   boolean simple,
						   int[] fourColors) {
		if (type == R3D_FILL) {
				...
				case Settings.Material:
					...
					switch (type) {
						...
						case R3D_RAISED:
							...
							break;
						case R3D_LOWERED:
							drawVistaRect(x, y, width, height, fourColors[2], fourColors[3], fourColors[1],
	...
	//////////////////// dstX,dstY
	private void drawSurface(int[] pixels, Object srcSurface, int x, int y, int width, int height,
							 int dstX,
							 int dstY,
		...
		try {
			...
			int scrPitch = pixels == mainWindowPixels ? Settings.screenWidth :
						   bmpW; // if we're copying from a control, use the real width instead of the control's width
	...
	private void fillPolygon(int[] xPoints1, int[] yPoints1, int base1, int nPoints1, int c1, int c2,
							 boolean gradient) {
		int x1, y1, x2, y2, n = 0, temp, numSteps = 0, startRed = 0, startGreen = 0, startBlue = 0,
							endRed = 0,
							endGreen = 0, endBlue = 0, redInc = 0, greenInc = 0, blueInc = 0, red = 0, green = 0, blue = 0,
							c = 0, j;
	...
	private int doClipX(int x) {
		return ((x + transX) < clipX1) ? (clipX1 - transX) : ((x + transX) >= clipX2) ?
			   (clipX2 - transX) : x; // guich@401_2: added TRANSX/Y
	}

	private int doClipY(int y) {
		return ((y + transY) < clipY1) ? (clipY1 - transY) : ((y + transY) >= clipY2) ?
			   (clipY2 - transY) : y;
	}


	// draws an ellipse incrementally
	private void ellipseDrawAndFill(int xc, int yc, int rx, int ry, int c1, int c2, boolean fill,
									boolean gradient) {
		int numSteps = 0, startRed = 0, startGreen = 0, startBlue = 0, endRed = 0, endGreen = 0,
			endBlue = 0, redInc = 0,
	...
	private void arcPiePointDrawAndFill(int xc, int yc, int rx, int ry, double startAngle,
										double endAngle, int c,
										int c2,
		...
		int nq, size = 0;
		boolean checkClipX = (xc + rx + transX) > clipX2
		...
		if (startAngle < 0.1 && endAngle > 359.9) { // full circle? use the fastest routine instead
			if (fill) {
				ellipseDrawAndFill(xc, yc, rx, ry, c, c2, true,
								   gradient); // guich@201_2: corrected colors. - guich@401_3: changed c->c2 and vice versa (line below)
			}
		...
		if (startIndex > endIndex) {
			...
			if (fill) {
				fillPolygon(xPoints, yPoints, startIndex, p1, gradient ? c : c2, c2,
							gradient); // lower half, upper half
				fillPolygon(xPoints, yPoints, 0, endIndex, gradient ? c : c2, c2,
							gradient); // lower half, upper half
			}
			if (!gradient) {
	...
	private static int interpolate(int color1r, int color1g, int color1b, int color2r, int color2g,
								   int color2b,
								   int factor) {
		int m = 255 - factor;
		int r = (color1r * factor + color2r * m) / 255;
	...
	public void drawWindowBorder(int xx, int yy, int ww, int hh, int titleH, int footerH,
								 int borderColor,
								 int titleColor,
		...
		for (int i = 0; i < 3; i++) {
			...
			ky = y2 - i;
			c = getPixel(kx, ky);
			drawLine(kx, ky, x2r, y2 - i, interpolate(borderColorR, borderColorG, borderColorB, c,
					 a)); // bottom
			...
			for (int i = 0; i < 7; i++) {
				...
				if (a != OUT) {
					if (a <= 0) {
						...
						setPixel(right, top, interpolate(borderColorR, borderColorG, borderColorB, getPixel(right, top),
														 a));
					}
				}
				// bottom left
				...
				if (a != OUT) {
					if (a <= 0) {
						setPixel(left, bot,
								 interpolate(borderColorR, borderColorG, borderColorB, footerColorR, footerColorG, footerColorB,
											 -a));
					} else {
				...
				if (a != OUT) {
					if (a <= 0) {
						...
						setPixel(right, bot, interpolate(borderColorR, borderColorG, borderColorB, getPixel(right, bot),
														 a));
					}
				}
			}
	...
	public void drawCylindricShade(int startColor, int endColor, int x, int y, int w, int h) {
		...
		int numSteps = Math.max(1, Math.min((endY - startY) / 2,
											(endX - startX) /
											2)); // guich@tc110_11: support horizontal gradient - guich@gc114_41: prevent div by 0 if numsteps is 0
		int startRed = (startColor >> 16) & 0xFF;
		int startGreen = (startColor >> 8) & 0xFF;
		int startBlue = startColor & 0xFF;
		int endRed = (endColor >> 16) & 0xFF;
	...
	private void addError(int[] pixel, int x, int y, int w, int h, int errR, int errG, int errB, int j,
						  int k) {
		if (x >= w || y >= h || x < 0) {
			return;
		}
