...
/**
 * The Color class is just an utility class used to do operations in a color,
 * which is actually represented by an <code>int</code> in the format
 * 0x00RRGGBB. <p> A color is defined as a mixture of red, green and blue color
 * values. Each value is in the range of 0 to 255 where 0 is darkest and 255 is
 * brightest. For example, Color(255, 0, 0) is the color red. <p> Here are some
 * more examples: <ul> <li>Color(0, 0, 0) is black <li>Color(255, 255, 255) is
 * white <li>Color(255, 0, 0 ) is red <li>Color(0, 255, 0) is green <li>Color(0,
 * 0, 255) is blue
 * </ul>
 */
...
public final class Color
{
  ...
  /**
   * Increase or decrease all RGB values by 96. To be used in the darker(step)
   * and brighter(step) methods.
   */
  public static final int FULL_STEP = 96;
  /**
   * Increase or decrease all RGB values by 32. To be used in the darker(step)
   * and brighter(step) methods.
   */
  public static final int LESS_STEP = 32;
  /**
   * Increase or decrease all RGB values by 48. To be used in the darker(step)
   * and brighter(step) methods.
   */
  public static final int HALF_STEP = 48;

  private static int lastDarkerSrc = -1, lastDarkerDst, lastBrighterSrc = -1,
                     lastBrighterDst;

  /**
   * Returns a color integer value with the given red, green and blue
   * components.
  ...
  /**
   * Returns a color integer value with the given red, green and blue
   * components, ensuring that the values are within the 0-255 range.
   * @param red the red value in any range
   * @param green the green value in any range
   * @param blue the blue value in any range
   * @since SuperWaba 5.8
   */
  public static int getRGBEnsureRange(int red, int green, int blue)
  { // guich@580_28
  ...
  /**
   * Returns a color integer value parsing the given rgb. Note that this method
   * is rather slow.
   * @param rrggbb a string in the format RRGGBB (colors in hex).
   * @since SuperWaba 5.8
   */
  public static int getRGB(String rrggbb)
  { // guich@580_28
    try {
  ...
  /**
   * Returns the alpha channel (brightness) of the color value in a value from
   * 0 to 255.
   * @deprecated Use {@link #getBrightness(int)}
   */
  @Deprecated public static int getAlpha(int rgb)
  {
    return ((((rgb >> 16) & 0xFF) << 5) + (((rgb >> 8) & 0xFF) << 6) +
            ((rgb & 0xFF) << 2)) /
      100;
  }
  ...
  public static int brighter(int rgb)
  {
    ...
    lastBrighterSrc = rgb;
    return lastBrighterDst = getRGBEnsureRange(((rgb >> 16) & 0xFF) + FULL_STEP,
                                               ((rgb >> 8) & 0xFF) + FULL_STEP,
                                               (rgb & 0xFF) + FULL_STEP);
  }
  ...
  public static int darker(int rgb)
  {
    ...
    lastDarkerSrc = rgb;
    return lastDarkerDst = getRGBEnsureRange(((rgb >> 16) & 0xFF) - FULL_STEP,
                                             ((rgb >> 8) & 0xFF) - FULL_STEP,
                                             (rgb & 0xFF) - FULL_STEP);
  }
  ...
  public static int getCursorColor(int rgb)
  {
    ...
    return getBrightness(rgb) >= 127
      ? getRGBEnsureRange(((rgb >> 16) & 0xFF) - step,
                          ((rgb >> 8) & 0xFF) - step,
                          (rgb & 0xFF) - step)
      : getRGBEnsureRange(((rgb >> 16) & 0xFF) + step,
                          ((rgb >> 8) & 0xFF) + step,
                          (rgb & 0xFF) +
                            step); // guich@220_45 - guich@340_51: in color
                                   // devices, use normal step
  }
  ...
  {
    return getRGBEnsureRange(((rgb >> 16) & 0xFF) + step,
                             ((rgb >> 8) & 0xFF) + step,
                             (rgb & 0xFF) + step);
  }
