...
public class Container extends Control {
	...
	*/
	...
	protected boolean
	finishedStart; // guich@450_36: avoid repaints while we're creating our controls for the first time
	...
	public Container() {
		asContainer = this;
		focusTraversable =
			false; // kmeehl@tc100: Container is now not focusTraversable by default. Controls extending Container will set focusTraversable explicitly.
	}
	...
	public void setInsets(int left, int right, int top, int bottom) { // guich@tc110_87
		int gap = borderStyle == BORDER_NONE
				  || borderStyle == BORDER_TOP ? 0 : borderStyle == BORDER_SIMPLE ? 1 : 2;
	...
	public void add(Control control) {
		...
		if (control.width > 0 && finishedStart) {
			Window.needsPaint =
				true; // guich@450_36: only repaint here if the setRect was already called; otherwise, repaint will be called on setRect
		}
	...
	public Control findNearestChild(int x, int y, int minDistance) { // guich@tc120_48
		...
		while (true) {
			...
			Control child = container.tail;
			while (child !=
				   null) { // guich@240_8: fixed when adding two controls in the same location but one of them was not visible (!child.visible)
				if (child.visible) {
	...
	protected void getClientRect(Rect r) { // guich@450_36
		r.set(insets.left, insets.top, width - insets.left - insets.right,
	...
	public void onPaint(Graphics g) {
		...
		if (drawTranslucentBackground(g, alphaValue)) {
				   ...
				   && (parent != null && (b != parent.backColor || parent.asWindow != null
			...
			switch (borderStyle) {
				...
				case BORDER_ROUNDED:
					if (Settings.onJavaSE) { // used on simulator
						g.drawWindowBorder(0, 0, width, height, 0, 0, borderColor != -1 ? borderColor : getForeColor(), b,
										   b, b, 2,
										   false);
					} else {
		...
		for (int i = 0; i < numChildren; i++, child = child.next) {
			if (child.focusTraversable && !child.focusLess) {
				if (child.asContainer != null && !child.focusHandler) {
					((Container) child).getFocusableControls(
						v); // Note that calling this function directly on the focus handler in question will yield its focusable controls.
				}
	...
	public Control findNextFocusControl(Control c, int direction) { // kmeehl@tc100
		...
		while (childControl != null) {
			...
			if (isChildPast) {
				...
				} else if (isCloser(childCoordValue, closestControlCoordValue, closestControl, vertical,
									childRect.x,
	...
	private boolean isCloser(int childCoordValue, int nextControlCoordValue, Control nextControl,
							 boolean vertical,
		...
		if (childCoordValue == nextControlCoordValue) {
			...
			if ((toPrev && childCoordValue < nextControlCoordValue) || (!toPrev
					&& childCoordValue > nextControlCoordValue)) {
	...
	public void resizeWidth() { // guich@tc114_53
		...
		int hborder = borderStyle == BORDER_NONE
					  || borderStyle == BORDER_TOP ? 0 : borderStyle == BORDER_SIMPLE ? 1 : 2;
	...
	private boolean changeTo(Control c) {
		if (c != null && c != this && c.isEnabled() && c.visible && (c instanceof Edit
				&& ((Edit) c).editable)
			|| (c instanceof MultiEdit
				&& ((MultiEdit)
					c).editable)) { // guich@tc100b4_12: also check for enabled/visible/editable - guich@tc120_49: skip ourself
			c.requestFocus();
			if (Settings.virtualKeyboard && Settings.enableVirtualKeyboard) {
