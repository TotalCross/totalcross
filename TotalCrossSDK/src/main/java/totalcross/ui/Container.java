...
public class Container extends Control
{
  ...
  /**
   * Sets the type of background of this Container. To disable the background,
   * set the <code>transparentBackground</code> of the Control class to true.
   * This field is used when transparentBackground is set to false (default).
   *
  ...
  /**
   * used in the backgroundStyle field. The bright color must be the fore
   * color, and the darker color, the back color
   * @since TotalCross 2.0
   */
  ...
  protected boolean finishedStart; // guich@450_36: avoid repaints while we're
                                   // creating our controls for the first time
  /**
   * Holds the controls that will be used to transfer focus when the tab key is
   * pressed. You can add or remove controls from here, but be careful not to
   * add repeated controls.
   * @since SuperWaba 5.5
   */
  public Vector tabOrder = new Vector(); // guich@550_15
  protected int lastScreenWidth;
  /**
   * The insets of this container. Never change it directly, otherwise some
   * controls may not work correctly; use setInsets instead.
   * @see #setInsets
   */
  ...
  {
    asContainer = this;
    focusTraversable = false; // kmeehl@tc100: Container is now not
                              // focusTraversable by default. Controls extending
                              // Container will set focusTraversable explicitly.
  }
  ...
  public void setPressColor(int color)
  {
    ...
    if (color != -1 && pe == null) {
      ...
      addPenListener(pe = new PenListener() {
        @Override public void penUp(PenEvent e)
        {
          if (e.type == PenEvent.PEN_UP && isEnabled() &&
              !hadParentScrolled()) {
            setPressed(!cpressed);
            postPressedEvent();
          }
        ...
        @Override public void penDown(PenEvent e) {}

        @Override public void penDrag(DragEvent e) {}

        @Override public void penDragStart(DragEvent e) {}

        @Override public void penDragEnd(DragEvent e) {}
      });
    }
  }
  ...
  public boolean isPressed() { return cpressed; }
  ...
  { // guich@tc110_87
    int gap = borderStyle == BORDER_NONE || borderStyle == BORDER_TOP
                ? 0
  ...
  /**
   * Copy the current insets values into the given insets. If you call this
   * method often, create an Insets field and reuse it.
   * @param copyInto The created object where the insets values will be copied
   *     into.
   * @since TotalCross 1.01
   */
  ...
  /**
   * Adds the control to this container, using the given bounds, relative to
   * the last control added Same of <pre> add(control); control.setRect(x,y,w,h,
   * null,false);
   * </pre>
  ...
  /**
   * Adds the control to this container, using the given bounds, relative to
   * the given control. Same of <pre> add(control); control.setRect(x,y,w,h,
   * relative,false);
   * </pre>
   * @see Control#setRect(int, int, int, int, Control, boolean)
   */
   ...
   * <b>Important</b>: If you're swapping containers from the MainWindow, be
   * sure that you set the focus on the new container after calling this add
   * method. Otherwise, a MenuBar will not work. Or, use the handy method
   * Window.swap
   */
  public void add(Control control)
  {
    if (control.uiAdjustmentsBasedOnFontHeightIsSupported ==
        Settings.uiAdjustmentsBasedOnFontHeight) {
      control.uiAdjustmentsBasedOnFontHeightIsSupported =
        this.uiAdjustmentsBasedOnFontHeightIsSupported;
    }
    ...
    if (control.asWindow != null) {
      throw new RuntimeException(
        "A Window can't be added to a container: use popup instead.");
    }
    ...
    if (control.foreColor <
			0/* || control.foreColor == UIColors.controlsFore - if the user set the container's color to something else and the control's color to black, this test overrides the black color*/) {
      control.foreColor = this.foreColor; // guich@200b4_125
    }
    if (control.backColor <
        0 /* || control.backColor == UIColors.controlsBack*/) {
      control.backColor = this.backColor; // guich@200b4_125
    }
    if (this.font != MainWindow.defaultFont &&
        control.font == MainWindow.defaultFont) {
      control.setFont(this.font);
    }
    control.onColorsChanged(true);
    if (control.width > 0 && finishedStart) {
      Window.needsPaint =
        true; // guich@450_36: only repaint here if the setRect was already
              // called; otherwise, repaint will be called on setRect
    }
    if (control.asContainer != null && !control.asContainer.ignoreOnAddAgain &&
        control.asContainer.started) {
      control.asContainer.onAddAgain(); // guich@402_5
    }
    if (control.asContainer != null ||
        (control.focusTraversable && !control.focusLess)) {
  ...
  public void remove(Control control)
  {
    ...
    numChildren--;
    Window.needsPaint =
      true; // guich@200b4_16: invalidate the hole container's area
    control.parent = null;
    if (control.asContainer != null && !control.asContainer.ignoreOnRemove) {
      control.asContainer.onRemove(); // guich@402_5
    }
    tabOrder.removeElement(control); // kmeehl@tc100: remove the element from
                                     // the Vector, if it is there
  }
  ...
  { // guich@tc120_48
    ...
    while (true) {
      ...
      while (
        child !=
        null) { // guich@240_8: fixed when adding two controls in the same
                // location but one of them was not visible (!child.visible)
        if (child.visible) {
          int dist =
            (int)(Convert.getDistancePoint2Rect(x,
                                                y,
                                                child.x,
                                                child.y,
                                                child.x + child.width,
  ...
  public Control getFirstChild() { return children; }

  public int getChildrenCount() { return numChildren; }
  ...
  /**
   Posts an event to the children of this container and to all containers
   inside this containers; recursively.
   @since SuperWaba 2.0 beta 4
 */
  public void broadcastEvent(Event e)
  {
    _onEvent(
      e); // guich@200b4_110: make sure this container receive this event.
    for (Control child = children; child != null; child = child.next) {
      child._onEvent(e);
      if (child.asContainer != null) {
  ...
  {
    for (Control child = children; child != null; child = child.next) {
      if (child.visible) { // guich@200: ignore hidden controls - note: a window
                           // added to a container may not be painted correctly
  ...
  /**
   * Sets the border for this container. The insets are changed after this
   * method is called. The BORDER_ROUNDED sets the background to transparent.
   * @see #BORDER_NONE
   * @see #BORDER_LOWERED
   * @see #BORDER_RAISED
   * @see #BORDER_SIMPLE
  ...
  { // guich@200final_16
    int gap = border == BORDER_NONE || borderStyle == BORDER_TOP
                ? 0
                : borderStyle == BORDER_SIMPLE ? 1 : 2;
   ...
   * The client rectangle usually excludes the title (if applicable) and the
   * border. If you call this method often, consider using the other version,
   * where a cached Rect is used; it avoids the creation of a Rect.
  ...
  /**
   * Returns the client rect for this Container, in relative coords, excluding
   * the insets. In this version, you provide the instantiated Rect to be filled
   * with the coords.
   */
  ...
  @Override protected void onColorsChanged(boolean colorsChanged)
  {
    if (borderStyle != BORDER_NONE && borderStyle != BORDER_SIMPLE &&
        borderStyle != BORDER_TOP && borderStyle != BORDER_ROUNDED) {
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (drawTranslucentBackground(g, alphaValue)) {
               ...
               (parent != null &&
                (b != parent.backColor || parent.asWindow != null ||
      ...
      switch (borderStyle) {
        ...
        case BORDER_ROUNDED:
          if (Settings.onJavaSE) { // used on simulator
            g.drawWindowBorder(0,
                               0,
                               width,
                               height,
                               0,
                               0,
                               borderColor != -1 ? borderColor : getForeColor(),
                               b,
                               b,
                               b,
                               2,
        ...
        default:
          g.draw3dRect(
            0, 0, width, height, borderStyle, false, false, fourColors);
      }
    }
  }

  /**
   * When the container is added for the first time, the method initUI is
   * called, so the user interface can be initialized. From the second time and
   * up that the container is added, the onAddAgain method is called instead.
   * <br><br>
   * If this container is a window, this method is not called when the window is
   * popped. <br><br> If you don't want this method to be called, you must set
   * <code>ignoreOnAddAgain = true;</code> inside the method initUI or the class
   * constructor. Doing so makes the user interface initialize faster.
   *
   * @since SuperWaba 4.1
   */
   ...
   * container is a window, this method is not called when the window is
   * unpopped. <br><br> If you don't want this method to be called, you must set
   * <code>ignoreOnRemove = true;</code> inside the method initUI or the class
   * constructor. Doing so makes the user interface initialize faster.
   *
   * @since SuperWaba 4.1
   */
  ...
  /**
   * Called to initialize the User Interface of this container. This differs
   * from the onAddAgain method by that this method is called only once, at the
   * first time the control is added to the parent. When the container is being
   * setup, the initUI method is called; then, the onAddAgain is called every
   * time the container is added again.
   * @since SuperWaba 3.4
   */
  public void initUI()
  { // guich@340_15
   ...
   */
  public void setHighlighting() { isHighlighting = true; }

  /**
   * Call this method to swap this Container to the topmost window. Note that
   * since we have no idea who is our future parent window, we just use the
   * topmost window. The topmost window is the one that is currently visible.
   * <br>This method is useful if you have containers that are SINGLETONS and
   * wish to set them as the current container. E.G.: <pre> public class
   * MainMenu extends Container // create a Singleton from MainMenu
   * {
  ...
  /**
   * Clears all children controls that are <code>focusTraversable</code>,
   * recursively.
   */
  @Override public void clear()
  { // guich@572_19
    for (Control child = children; child != null; child = child.next) {
      if (child.focusTraversable || (child.asContainer != null &&
                                     child.asContainer.tabOrder.size() > 0)) {
        child.clear();
      }
    }
  }
        ...
        if (child.asContainer != null && !child.focusHandler) {
          ((Container)child)
            .getFocusableControls(
              v); // Note that calling this function directly on the focus
                  // handler in question will yield its focusable controls.
        }
        if (child.isVisible() && child.isEnabled() && child.height > 0 &&
            child.width > 0) {
          v.addElement(child);
        }
  ...
  /**
   * Finds the next control that should receive focus based on the direction
   * with respect to c.
   * @param c The reference control from which to find the next control.
   * @param direction The direction in which to look from c. One of:
   *     SpecialKeys.LEFT, SpecialKeys.RIGHT, SpecialKeys.UP, SpecialKeys.DOWN
  ...
  { // kmeehl@tc100
    ...
    boolean vertical =
      (direction == SpecialKeys.UP || direction == SpecialKeys.DOWN);
    if (vertical) {
    ...
    while (childControl != null) {
      ...
      if (isChildPast) {
        ...
        boolean childOverlaps = (vertical && childRect.x <= controlRight &&
                                 childRight >= controlRect.x) ||
                                (!vertical && childRect.y <= controlBottom &&
                                 childBottom >= controlRect.y);
        if (closestControl == null) {
          closestControl = childControl;
          closestControlCoordValue = childCoordValue;
        } else if (isCloser(childCoordValue,
                            closestControlCoordValue,
                            closestControl,
                            vertical,
                            childRect.x,
                            childRect.y,
        ...
        if (childOverlaps &&
            (closestOverlap == null || isCloser(childCoordValue,
                                                closestOverlapCoordValue,
                                                closestOverlap,
                                                vertical,
                                                childRect.x,
                                                childRect.y,
    ...
    if (closestControl == null &&
        (direction == SpecialKeys.LEFT || direction == SpecialKeys.RIGHT)) {
      return findNextFocusControl(
        c, direction == SpecialKeys.LEFT ? SpecialKeys.UP : SpecialKeys.DOWN);
    }
    return closestControl;
  }

  private boolean isCloser(int childCoordValue,
                           int nextControlCoordValue,
                           Control nextControl,
                           boolean vertical,
                           int childX,
                           int childY,
  ...
  { // kmeehl@tc100
    if (childCoordValue == nextControlCoordValue) {
      ...
      //  check if this child control is more to the left/top than the previous
      //  child.
      ...
      boolean toPrev =
        direction == SpecialKeys.DOWN || direction == SpecialKeys.RIGHT;
      // is new closest control?
      if ((toPrev && childCoordValue < nextControlCoordValue) ||
          (!toPrev && childCoordValue > nextControlCoordValue)) {
  ...
  public byte getBorderStyle() { return borderStyle; }
  ...
  public void incLastX(int n) { lastX += n; }

  /** Increments the lastY, used in relative positioning. */
  public void incLastY(int n) { lastY += n; }

  /**
   * This method resizes the Container's width and height to the needed bounds,
   * based on added childs. You can add spaces at right and bottom using the
   * insets.right/bottom properties. Sample: <pre>
   ...
   *   c.resize();
   * </pre>
   * Note: differently of Window.resize, this method does not call setRect
   * again, it only changes the width and height by direct assignment.
  ...
  /**
   * This method resizes the Container's width to the needed bounds, based on
   * added childs. You can add spaces at right using the insets.right property.
   * Sample:
   * <pre>
   *   // this sample will center two buttons of different sizes on screen
   *   Container c = new Container();
   ...
   * </pre>
   * Note: differently of Window.resize, this method does not call setRect
   * again, it only changes the width by direct assignment.
   * @since TotalCross 1.14
   * @see #resize
   * @see #resizeHeight
   */
  ...
  { // guich@tc114_53
    ...
    int hborder = borderStyle == BORDER_NONE || borderStyle == BORDER_TOP
                    ? 0
                    : borderStyle == BORDER_SIMPLE ? 1 : 2;
  ...
  /**
   * This method resizes the Container's width to the needed bounds, based on
   * added childs. You can add spaces at bottom using the insets.bottom
   * property. Sample: <pre>
   *   // this sample will center two buttons of different sizes on screen
   *   Container c = new Container();
   *   add(c, CENTER,BOTTOM,1000,1000);
   ...
   * </pre>
   * Note: differently of Window.resize, this method does not call setRect
   * again, it only changes the height by direct assignment.
   * @since TotalCross 1.14
   * @see #resizeWidth
   * @see #resize
   */
  ...
  { // guich@tc125_26
      ...
      for (int i = n - 1; i >= 0; i--) {
        if (forward && ++idx == n) {
          if (Settings.virtualKeyboard && Settings.enableVirtualKeyboard) {
            if (parent != null &&
                (control instanceof Edit && ((Edit)control).editable)) {
    ...
    return parent != null ? parent.moveFocusToNextEditable(control, forward)
                          : null;
  }

  private boolean changeTo(Control c)
  {
    if (c != null && c != this && c.isEnabled() && c.visible &&
          (c instanceof Edit && ((Edit)c).editable) ||
        (c instanceof MultiEdit &&
         ((MultiEdit)c).editable)) { // guich@tc100b4_12: also check for
                                     // enabled/visible/editable -
                                     // guich@tc120_49: skip ourself
      c.requestFocus();
      if (Settings.virtualKeyboard && Settings.enableVirtualKeyboard) {
  ...
  /**
   * Moves the focus to the next control, which can be an Edit, a MultiEdit, or
   * another control type. It does not show the keyboard.
   * @return The selected control or null if none was found.
  ...
  /**
   * Changes the focusTraversable property for this container and all controls,
   * recursively
   */
  ...
  /**
   * Called when this container has been swapped into the Window and the swap
   * is done.
   * @since TotalCross 2.0
   */
  ...
  /**
   * get border radius for border style @see BORDER_ROUNDED. Default value is
   * 12dp. (Not used on JAVA simulator)
   * @return
   */
  public int getBorderRadius() { return borderRadius; }

  /**
   * Set border radius for border style @see BORDER_ROUNDED. Default value is
   * 12dp. (Not used on JAVA simulator)
