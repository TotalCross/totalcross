...
/* A container that checks if the sibling is within the visible area before
 * calling paint on it.
 *
 * Used internally.
 */
...
public class ClippedContainer extends Container
{
  ...
  @Override public void paintChildren()
  {
    ...
    if (verticalOnly) {
      ...
      // check if the mid container of the last search is still visible, and
      // restart the search using it
      if (lastMid != -1 && lastMid < n &&
          ((Control)items[lastMid]).isVisibleAndInside(bagClipY0, bagClipYf)) {
        first = lastMid;
      } else {
        first = findOneVisible(bagClipY0, bagClipYf, 0, n);
      }
      // found the first (or second) visible, go back to find the really first
      // visible
      while (
        first > 0 &&
        ((Control)items[first - 1]).isVisibleAndInside(bagClipY0, bagClipYf)) {
        first--;
      }
      ...
      for (Control child = children; child != null; child = child.next) {
        if (child.isVisibleAndInside(
              bagClipX0, bagClipY0, bagClipXf, bagClipYf) &&
            (pw == null || !child.isObscured(pw))) {
          if (child.asContainer != null &&
              child.asContainer.offscreen != null) {
