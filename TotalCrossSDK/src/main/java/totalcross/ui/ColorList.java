 ...
 * The only functional methods are setColors, getSelectedItem and
 * getSelectedColor. Next an example of how to use this class as a combobox
 * color chooser: <pre> add(foreCombo = new ComboBox(new ColorList()), CENTER,
 * BOTTOM);
 * </pre>
...
public class ColorList extends ListBox
{
  ...
  public static class Item
  {
    ...
    public Item(int value) { set(value); }
  ...
  static { defaultColors = convert(Graphics.getPalette()); }
  private Item[] colors;
  ...
  @Override protected void drawItem(Graphics g, int index, int dx, int dy)
  {
    if (0 <= index && index <= colors.length) {
      ...
      g.foreColor = Color.getBrightness(colors[index].value) >= 127
                      ? Color.BLACK
                      : Color.WHITE;
      g.drawText(colors[index].toString(),
                 dx + 4,
                 dy + (hh - fmH) / 2,
                 textShadowColor != -1,
                 textShadowColor);
  ...
  @Override protected void drawCursor(Graphics g, int sel, boolean on)
  {
    if (offset <= sel && sel < visibleItems + offset && sel < itemCount) {
      ...
      g.setClip(dx - 1,
                yy,
                btnX - dx + 1,
                Math.min(ih * visibleItems,
                         this.height - dy)); // guich@200b4_83: fixed selection
                                             // overflowing paint area
      ...
      g.drawRect(dx - 1,
                 yy,
                 btnX - 1,
                 ih); // only select the Object - guich@200b4_130
    }
  }

  @Override public int getPreferredWidth()
  {
    return w + (simpleBorder ? 4 : 6) + sbar.getPreferredWidth() + insets.left +
      insets.right;
  }

  @Override protected int getItemWidth(int index) { return w; }

  /**
   * Returns the selected Item object, or a black Item representing the black
   * color if none was selected. From the Item you can retrieve the color as
   * int. Use the getSelectedColor method to retrieve the color directly.
   */
