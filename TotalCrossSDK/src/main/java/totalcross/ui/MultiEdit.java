...
public class MultiEdit extends Container implements Scrollable, TextControl, TimerListener
{
  ...
  public boolean scrollContent(int xDelta, int yDelta, boolean fromFlick)
  {
    ...
    if (lastFirstToDrawLine <= 0 || (yDelta < 0 && firstToDraw == 0) || (yDelta > 0 && firstToDraw >= lastFirstToDrawLine)){
      return false;
    }
  ...
  protected void onWindowPaintFinished()
  {
    ...
    {
      _onEvent(new Event(ControlEvent.FOCUS_IN,this,0)); // this event is called on the focused control of the parent window. so, if we are not in FOCUS state, set it now. --original - guich@350_7: added the editable check
    }
  }
