...
/**
 * MultiEdit is an Edit with support for multiple lines. A static scrollbar is
 * added, but disabled/enabled as needed. <p> Here is an example showing an Edit
 * control being used:
 *
 * <pre>
 ...
 * }
 * </pre>
 * If the MultiEdit is not editable, the user can scroll the edit a page at a
 * time just by clicking in the middle upper or middle lower.
 *
 * <p><b>IN MATERIAL UI, IS VERY IMPORTANT THAT THE CAPTION IS SET BEFORE ADDING
 * THE CONTROL.</b></p>
 *
...
public class MultiEdit
  extends Container implements Scrollable, TextControl, HasValue<String>
{
  ...
  private UpdateListener blinkListener = new UpdateListener() {
    @Override public void updateListenerTriggered(int elapsedMilliseconds)
    {
      ...
      if (blinkTime > 350) {
        if (!isTopMost()) { // must check here and not in the onPaint method,
                            // otherwise it results in a problem: show an edit
                            // field, then popup a window and move it: the edit
                            // field of the other window is no longer being
                            // drawn
          focusOut();
        }
  ...
  /**
   * Set to true if you want the control to decide whether to gain/lose focus
   * automatically, without having to press ACTION.
   */
  ...
  protected IntVector first =
    new IntVector(5); // JR@0.4.  indices of first character of each line. the
                      // value of last is len(text)+1
  ...
  /**
   * While using geographical focus, editMode is toggled using the Action Key
   * to allow navigation and editing of the text inside the MultiEdit *
   */
  ...
  int lastPenDown = -1;
  private static KeyEvent backspaceEvent =
    new KeyEvent(KeyEvent.SPECIAL_KEY_PRESS, SpecialKeys.BACKSPACE, 0);
  // changes for material design
  private int iconX0;

  private boolean scrollBarsAlwaysVisible;
  /**
   * The mask used to infer the preferred width. Unlike the Edit class, the
   * MultiEdit does not support real masking.
   */
  public String mask;

  /**
   * Used to set the number of rows of this MultiEdit; used as parameter to
   * compute the preferred height. You must call setRect after changing this to
   * resize the control in height.
   */
  public int rowCount; // guich@320_28

  /**
   * Sets the capitalise settings for this MultiEdit. Text entered will made as
  ...
  public int gap =
    1; // guich@320_28: made public to remove 2 almost unused methods
  ...
  /**
   * Constructs a MultiEdit with 1 pixel as space between lines and with no
   * lines. You must set the bounds using FILL or FIT.
   */
  public MultiEdit() { this(1, 1); }

  /**
    Constructor for a text Edit with a vertical scroll Bar, gap is 1
    by default and control's bounds must be specified with a setRect. Space
    between lines may be 0.
  */
  public MultiEdit(int rowCount, int spaceBetweenLines)
  { // vertical space between 2 lines in pixel
    blinkTime = 0;
    ...
    onFontChanged();
    this.clearPosState();
    add(this.sb = Settings.fingerTouch ? new ScrollPosition(ScrollBar.VERTICAL)
                                       : new ScrollBar(ScrollBar.VERTICAL));
    if (!Settings.fingerTouch) {
      sb.setLiveScrolling(true);
      // don't let the scrollbar steal focus from us
      sb.setEnabled(false); // gao - leave this disabled for visual effect until
                            // we know we need it
  ...
  /**
   * Constructor for a text Edit with a vertical scroll Bar, gap is 1
   * by default and control's bounds must be given with a setRect. Space between
   * lines may be 0. The mask is used to compute the PREFERRED width of the
   * control. Note that the mask does not <i>masks</i> the input. If mask is "",
   * the FILL width is choosen.
   */
  ...
  public boolean scrollContent(int xDelta, int yDelta, boolean fromFlick)
  {
    ...
    dragDistance += yDelta;
    if ((dragDistance < 0 && dragDistance > -hLine) ||
        (dragDistance >= 0 && dragDistance < hLine)) {
      return true;
    }
  ...
  @Override public int getPreferredHeight()
  {
    ...
    int ret = (hLine * rowCount0 + (uiFlat ? 2 : 4) + 2 * gap) + insets.top +
              insets.bottom; //+2= minimal space between 2 lines
  ...
  {
    return (mask == null
              ? (totalcross.sys.Settings.screenWidth >> 2)
              : (mask.length() == 0) ? FILL : (fm.stringWidth(mask) + 10)) +
      insets.left + insets.right; // guich@200b4_202: from 2 -> 4 is PalmOS
  ...
  {
    this.lastKbdType = this.kbdType =
      kbd == Edit.KBD_NONE ? Edit.KBD_NONE : Edit.KBD_KEYBOARD; // vik@421_24
  }

  /**
   * sets the valid chars that can be entered in this edit. if null is passed,
   * any char can be entered. (case insensitive).
   */
   ...
   */
  @Override public String getText() { return chars.toString(); }
  ...
  */
  ...
  public int getNumberOfTextLines() { return numberTextLines; }

  /**
   * Set to true to hide the vertical scrollbar when it isn't needed (instead
   * of disabling it). This must be done right after the constructor.
   * @since TotalCross 1.0
   */
  ...
  {
    if (kbdType == Edit.KBD_NONE || !editable || !isEnabled() ||
        !Settings.enableVirtualKeyboard) {
      return;
    }
    if (Settings.virtualKeyboard) {
      _onEvent(new ControlEvent(ControlEvent.FOCUS_IN,
                                this)); // simulate a focus in event.
    } else {
  ...
  private void showInputWindow(Window w)
  {
    ...
    // guich@320: modify and restore later our state, bc the Keyboard needs a
    // shrinked control
    tempGap = gap;
  ...
  private int zToCharPos(Coord z)
  {
    ...
    z.x =
      Math.max(0,
               Math.min(fm.sbWidth(chars,
                                   first.items[line],
                                   first.items[line + 1] - first.items[line]),
                        z.x));
  ...
  private void charPosToZ(int n, Coord z)
  {
    ...
    for (i = 0; end - i >
                1;) { // kmeehl@tc100: compute the new line using binary search
      mid = i + (end - i) / 2;
    ...
    // if (n == first.items[i+1]) i++; else - if char is at last space, put it
    // in the next line - note that this doesn't work bc when pressing down key
    // it skips 2 lines
  ...
  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    ...
    boardRect = new Rect(zOffset,
                         zOffset,
                         this.width - 2 * zOffset -
                           (Settings.fingerTouch ? 0 : sb.getPreferredWidth()),
                         this.height - 2 * zOffset); // JR @0.5
    textRect = boardRect.modifiedBy(
      leftTextGap, topTextGap, -2 * rightTextGap, -2 * bottomTextGap);
    rowCount =
      textRect.height / this.hLine; // kambiz@350_5: update rowCount according
                                    // to the new size of the text area
    sb.setRect(RIGHT - (Settings.fingerTouch ? 1 : 0),
               TOP,
               PREFERRED,
               FILL,
               null,
    ...
    if (materialCaption != null) {
      materialCaption.xcap0 = materialCaption.xcap =
        chars.length() == 0 ? zOffset : 0;
      materialCaption.ycap0 = materialCaption.ycap =
        chars.length() == 0 ? textRect.y : 0;
    }
  ...
  {                                  // guich@320_28: completely redesigned -
                                     // guich@581_6: highly optimized
    ...
    int pos = 0;
    for (; pos < n; pos++) {
      int pos0 = pos == 0 || chars.charAt(pos - 1) < ' '
                   ? pos
                   : pos - 1; // guich@tc113_37: when parsing "Update of
                              // /pcvsroot/LitebaseSDK/src/native/parser", it
                              // was breaking in the first /, but in the next
                              // loop iteration, it was skipping the first /,
                              // and, thus, computing a character less
      first.addElement(
        pos = Convert.getBreakPos(fm,
                                  chars,
                                  pos0,
                                  tw,
                                  true)); // guich@tc166: we'll take care of the
                                          // initial space/ENTER during drawing
    }
    first.addElement(n);
    numberTextLines = first.size() - 1;
    // try {for (i =0; i <= numberTextLines; i++) Vm.debug("first["+i+"]:
    // "+first.items[i]+" '"+chars.charAt(first.items[i])+"'");} catch
    // (Exception e) {Vm.debug("first["+i+"]: "+first.items[i]);}
    ...
    if (numberTextLines != originalLineCount) {
      ...
      if (!Settings.fingerTouch) {
        sb.setEnabled(needScroll); // gao always visually enable / disable based
                                   // on needScroll
    ...
    int mid;
    for (i = 0;
         end - i > 1;) { // kmeehl - compute the new line using binary search
      mid = i + (end - i) / 2;
    ...
    if (i < firstToDraw) {
      firstToDraw = i;
    } else if (i >= firstToDraw +
                      rowCount) { // bruno@tc114_47: fixed scrolling - when
                                  // typing, the last line was being omitted
  ...
  private void focusOut()
  {
    ...
    if ((Settings.keyboardFocusTraversable || Settings.geographicalFocus) &&
        w != null &&
        w == Window.getTopMost()) { // guich@tc110_81: remove highlight from us.
                                    // - guich@tc120_39: only if we're in the
                                    // topmost window
      ...
      switch (event.type) {
        case ControlEvent.FOCUS_IN:
          ...
          if (Settings.geographicalFocus) {
            editMode =
              editModeValue || improvedGeographicalFocus; // kmeehl@tc100
          }
          // guich@300_43: this is needed bc when popupKCC is called, the focus
          // comes back to here; also, when the popped up window is closed, the
          // focus comes back again, so we could enter in an infinite loop
        ...
        case KeyEvent.SPECIAL_KEY_PRESS:
          if (editable && isEnabled()) {
            ...
            if ((ke.key == SpecialKeys.ACTION ||
              ...
              // isHighlighting = true; // kmeehl@tc100: set isHighlighting
              // first, so that Window.removeFocus() wont trample
              // Window.highlighted - guich@tc110_81: commented out. this will
              // be done in focusOut().
            ...
            if (editable) {
              if (ke.key == 0) {
                return; // guich@402_41: sometimes, the left key causes a zero
                        // key being entered, crashing everything
              }
              if (ke.key == LINEFEED) {
                break;
              }
              ...
              boolean moveFocus =
                !Settings.geographicalFocus && ke.key == SpecialKeys.TAB;
              if (event.target == this && moveFocus) { // guich@tc125_26
                if (parent != null && parent.moveFocusToNextEditable(
                                        this, ke.modifiers == 0) != null) {
                  return;
                }
              }
              // if ((Settings.keyboardFocusTraversable ||
              // Settings.geographicalFocus) && (ke.key == SpecialKeys.ESCAPE ||
              // ke.key == SpecialKeys.MENU))

              boolean isControl =
                (ke.modifiers & SpecialKeys.CONTROL) !=
                0; // guich@320_46 - guich@tc100b4_25: also check for the type
                   // of event, otherwise the arrow keys won't work
              ...
              if (del1 >= 0 && del2 < len) {
                if (len > del2 - 1) {
                  chars.delete(del1, del2 + 1); // Vm.arrayCopy(chars, del2 + 1,
                                                // chars, del1, numOnRight);
                }
                newInsertPos = del1;
                clearSelect = true;
              }
              ...
              switch (ke.key) {
                ...
                case SpecialKeys.PAGE_UP:
                  if (!editable && !hasCursorWhenNotEditable) {
                    ...
                    z1.x = z3.x; // kmeehl@tc100: remember the previous
                                 // horizontal position
                    if (z1.y <= textRect.y &&
                        firstToDraw >
                          0) { // guich@550_22: check firstToDraw, otherwise it
                               // will insert blanks at the top
                      int ii = sb.getValue();
                      if (ii > sb.getMinimum()) {
                        firstToDraw--;
                      }
                    ...
                    if (line >= 0) {
                      if (chars.charAt(first.items[line]) == ENTER &&
                ...
                case SpecialKeys.LEFT:
                  ...
                  charPosToZ(newInsertPos, z3); // kmeehl@tc100: remember the
                ...
                case SpecialKeys.PAGE_DOWN:
                  if (!editable && !hasCursorWhenNotEditable) {
                    sb.onEvent(event);
                  } else if (numberTextLines - 1 > 0 &&
                             newInsertPos <=
                               first
                                 .items[numberTextLines -
                                        2]) { // -1 guich@573_44: check if > 0
                    charPosToZ(newInsertPos, z1);
                    z1.x = z3.x; // kmeehl@tc100: remember the previous
                                 // horizontal position
                    ...
                    int line = firstToDraw + (z1.y - textRect.y) / hLine;
                    if (newInsertPos < len &&
                        chars.charAt(newInsertPos) == ENTER &&
                        first.items[line + 1] - first.items[line] == 1) {
                      newInsertPos++;
                    } else {
                      newInsertPos = zToCharPos(z1);
        ...
        case PenEvent.PEN_UP: { // kmeehl@tc100
          ...
          if (captionPress != null && caption != null &&
            ...
            firstPenDown = false;
            if (!editable &&
                !Settings.fingerTouch) { // guich@tc100: allow the user to
                                         // scroll by just clicking in the ME
              event.target = sb;
              ((PenEvent)event).y =
                ((PenEvent)event).y < height / 2 ? 0 : height;
              sb.onEvent(event);
              break;
            } else if (popupVKbd) {
              showSip();
            ...
            charPosToZ(
              newInsertPos,
              z3); // kmeehl@tc100: remember the previous horizontal position
            isScrolling = false;
          }
          break;
        }
        case PenEvent.PEN_DOWN: {
          ...
          if (!editable &&
              !Settings.fingerTouch) { // guich@tc100: allow the user to scroll
                                       // by just clicking in the ME
            event.target = sb;
          ...
          z1.x = pe.x;
          z1.y = pe.y;
          newInsertPos = firstPenDown && Settings.moveCursorToEndOnFocus
                           ? chars.length()
                           : zToCharPos(z1);
        ...
        case PenEvent.PEN_DRAG: {
          ...
          if (Settings.fingerTouch) {
            if (isScrolling) {
              ...
              event.consumed = true;
              if (canScrollContent(direction, de.target) &&
                  scrollContent(-de.xDelta, -de.yDelta, true)) {
        ...
        case KeyEvent.ACTION_KEY_PRESS:
          try {
            ...
            if (Settings.geographicalFocus && ke.key == SpecialKeys.ENTER &&
                editMode) {
              event.type = KeyEvent.KEY_PRESS;
              _onEvent(event);
              break;
  ...
  private void clipboardCut()
  {
    ...
    if (sel1 != -1) {
      Vm.clipboardCopy(chars.toString().substring(
        sel1,
        sel2)); // brunosoares@tc100: BlackBerry does not support
  ...
  private void clipboardPaste()
  {
    ...
    if (pasted == null || pasted.length() == 0) {
      ...
      int n = pasted.length();
      if (maxLength > 0) {
        pasted = pasted.substring(
          0,
          Math.min(pasted.length(), Math.max(0, maxLength - chars.length())));
        n = pasted.length();
      }
  ...
  { // guich@tc126_21
    if (kbdType != Edit.KBD_NONE && Settings.virtualKeyboard && editMode &&
        editable && !hadParentScrolled() && !Window.isScreenShifted() &&
        Settings
          .enableVirtualKeyboard) { // if running on a PocketPC device, set
                                    // the bounds of Sip in a way to not cover
                                    // the edit - kmeehl@tc100: added check
                                    // for editMode and !dragScroll
      if (Settings.customKeyboard != null) {
        ...
        if (!Window.isSipShown() || Settings.isWindowsCE()) {
          Window.setSIP(
            onBottom ? Window.SIP_BOTTOM : Window.SIP_TOP, this, false);
        }
  ...
  protected void draw(Graphics g)
  {
      ...
      if (npParts != null && npback == null) {
        try {
          npback = NinePatch.getInstance().getNormalInstance(
            ...
            isEnabled() ? back0
                        : Color.interpolate(
                            back0 == parent.backColor ? Color.BRIGHT : back0,
                            parent.backColor),
            false);
        ...
        if (uiMaterial) {
          ...
          if (isEnabled()) {
            if (fillColor != -1) {
              ...
              g.fillRect(0,
                         caption != null ? materialCaption.getExtraHeight() : 0,
                         width,
                         height);
            }
            g.backColor = c;
          ...
          g.clearClip();
          int x2 =
            this.width - (Settings.fingerTouch ? 0 : sb.getPreferredWidth());
          if (alphaValue == 255) {
    ...
    if (startSelectPos != -1 && editable) { // guich@tc113_38: only if editable
      ...
      charPosToZ(sel1, z1);
      charPosToZ(sel2, z2);
      g.backColor = g.backColor =
        back1 == backColor ? Color.brighter(back1) : back1;
      ;
      if (z1.y == z2.y) {
        ...
        if (z2.y > z1.y) {
          g.fillRect(
            textRect.x, z1.y + hLine, textRect.width, z2.y - z1.y - hLine);
        }
    ...
    if (drawCaption && !uiMaterial) {
      ...
      for (; i <= last && h < maxh; i++, h += hLine, dh += hLine) {
        // if (!forceDrawAll) g.fillRect(boardRect.x + 1, h, boardRect.width -
        // 2, hLine); // erase drawing area
        int k = first.items[i];
        ...
        if (len > 0 && k < len && k != k2) {
          ...
          if (k2 > k) {
            g.drawText(
              ...
              textRect.x,
              h,
              (!editable && justify && i < last && k2 < len &&
               chars.charAt(k2) >= ' ')
                ? textRect.width
                : 0,
              textShadowColor != -1,
              textShadowColor); // don't justify if the line ends with <enter>
          }
        }
        if (drawDots) {
          g.drawDots(textRect.x,
                     dh,
                     textRect.x2(),
                     dh); // guich@320_28: draw the dotted line
          g.backColor = back0;
        }
      }
    }
  ...
  @Override protected void onWindowPaintFinished()
  {
    if (editable && !hasFocus) {
      _onEvent(new ControlEvent(
        ControlEvent.FOCUS_IN,
        this)); // this event is called on the focused control of the parent
                // window. so, if we are not in FOCUS state, set it now.
                // --original - guich@350_7: added the editable check
    }
  }
   ...
   * in the value for rowCount given in the constructor if the given height is
   * PREFERRED
   */
  @Override
  ...
  @Override public Control handleGeographicalFocusChangeKeys(KeyEvent ke)
  {
    boolean processEvent =
      editable &&
      (!improvedGeographicalFocus || (!ke.isPrevKey() && !ke.isNextKey()) ||
       (ke.key == SpecialKeys.LEFT && insertPos > 0) ||
       (ke.key == SpecialKeys.RIGHT && insertPos < chars.length()) ||
       ((ke.key == SpecialKeys.UP || ke.key == SpecialKeys.PAGE_UP) &&
        numberTextLines > 1 && insertPos >= first.items[1]) ||
       ((ke.key == SpecialKeys.DOWN || ke.key == SpecialKeys.PAGE_DOWN) &&
        numberTextLines > 1 && insertPos < first.items[numberTextLines - 1]));

    if (editMode && processEvent) {
      Object o = ke.target; // guich@tc115_6: otherwise, we will not process
                            // arrow events
  ...
  { // guich@tc115_6: user requested focus directly,
    // so enable editMode by default
    editModeValue = true;
    super.requestFocus();
    editModeValue = false;
  }
  ...
  {
    MultiEdit ed = mask == null
                     ? new MultiEdit(rowCount, spaceBetweenLines)
  ...
  {
    return editable &&
      (kbdType == Edit.KBD_DEFAULT || kbdType == Edit.KBD_KEYBOARD);
  }

  /**
   * Sets the cursor position, ranging from 0 to the text' length.
   * You can use this with <code>lastInsertPos</code> to recover cursor
   * position.
   */
  ...
  { // guich@400_18
    if (0 <= start && start <= chars.length() && 0 <= end &&
        end <= chars.length() + 1) {
   ...
   * */
  public void setTextGap(int leftTextGap,
                         int rightTextGap,
                         int topTextGap,
                         int bottomTextGap)
  {
