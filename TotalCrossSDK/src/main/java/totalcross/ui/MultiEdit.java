...
public class MultiEdit extends Container implements Scrollable, TextControl, HasValue<String> {
	...
	protected IntVector first = new IntVector(
		5); //JR@0.4.  indices of first character of each line. the value of last is len(text)+1
	...
	int lastPenDown = -1;
	private static KeyEvent backspaceEvent = new KeyEvent(KeyEvent.SPECIAL_KEY_PRESS,
			SpecialKeys.BACKSPACE, 0);
	...
	public MultiEdit(int rowCount, int spaceBetweenLines) { // vertical space between 2 lines in pixel
		...
		onFontChanged();
		this.clearPosState();
		add(this.sb = Settings.fingerTouch ? new ScrollPosition(ScrollBar.VERTICAL) : new ScrollBar(
						  ScrollBar.VERTICAL));
	...
	public int getPreferredWidth() {
		return (mask == null ? (totalcross.sys.Settings.screenWidth >> 2)
				: (mask.length() == 0) ? FILL : (fm.stringWidth(mask) + 10)) + insets.left +
			   insets.right; // guich@200b4_202: from 2 -> 4 is PalmOS style - guic@300_52: empty mask means FILL
	}
	...
	public void setKeyboard(byte kbd) {
		this.lastKbdType = this.kbdType = kbd == Edit.KBD_NONE ? Edit.KBD_NONE :
										  Edit.KBD_KEYBOARD; //vik@421_24
	}
	...
	private int zToCharPos(Coord z) {
		...
		z.x = Math.max(0, Math.min(fm.sbWidth(chars, first.items[line],
											  first.items[line + 1] - first.items[line]), z.x));
	...
	protected void onBoundsChanged(boolean screenChanged) {
		...
		boardRect = new Rect(zOffset, zOffset,
							 this.width - 2 * zOffset - (Settings.fingerTouch ? 0 : sb.getPreferredWidth()),
							 this.height - 2 * zOffset); //JR @0.5
		textRect = boardRect.modifiedBy(leftTextGap, topTextGap, -2 * rightTextGap, -2 * bottomTextGap);
		rowCount = textRect.height /
				   this.hLine; // kambiz@350_5: update rowCount according to the new size of the text area
	...
	calculateFirst() { // guich@320_28: completely redesigned - guich@581_6: highly optimized
		...
		for (; pos < n; pos++) {
			int pos0 = pos == 0
					   || chars.charAt(pos - 1) < ' ' ? pos : pos -
					   1; // guich@tc113_37: when parsing "Update of /pcvsroot/LitebaseSDK/src/native/parser", it was breaking in the first /, but in the next loop iteration, it was skipping the first /, and, thus, computing a character less
			first.addElement(pos = Convert.getBreakPos(fm, chars, pos0, tw,
								   true)); // guich@tc166: we'll take care of the initial space/ENTER during drawing
		}
		first.addElement(n);
		...
		if (i < firstToDraw) {
			firstToDraw = i;
				   ...
				   rowCount) { //bruno@tc114_47: fixed scrolling - when typing, the last line was being omitted
	...
	private void focusOut() {
		...
		if ((Settings.keyboardFocusTraversable || Settings.geographicalFocus) && w != null
			&& w == Window.getTopMost()) { // guich@tc110_81: remove highlight from us. - guich@tc120_39: only if we're in the topmost window
			...
			switch (event.type) {
				...
				case KeyEvent.SPECIAL_KEY_PRESS:
					if (editable && isEnabled()) {
						...
						if (ke.key == SpecialKeys.ACTION && (Settings.isWindowsCE()
						...
						if (editable) {
							...
							boolean isControl = (ke.modifiers & SpecialKeys.CONTROL) !=
												0; // guich@320_46 - guich@tc100b4_25: also check for the type of event, otherwise the arrow keys won't work
							...
							switch (ke.key) {
								...
								case SpecialKeys.PAGE_UP:
									if (!editable && !hasCursorWhenNotEditable) {
										...
										if (z1.y <= textRect.y
											&& firstToDraw > 0) { // guich@550_22: check firstToDraw, otherwise it will insert blanks at the top
											int ii = sb.getValue();
											if (ii > sb.getMinimum()) {
												firstToDraw--;
											}
								...
								case SpecialKeys.PAGE_DOWN:
									if (!editable && !hasCursorWhenNotEditable) {
										sb.onEvent(event);
											   ...
											   && newInsertPos <= first.items[numberTextLines - 2]) { // -1 guich@573_44: check if > 0
										charPosToZ(newInsertPos, z1);
										z1.x = z3.x; // kmeehl@tc100: remember the previous horizontal position
				...
				case PenEvent.PEN_UP: { // kmeehl@tc100
					...
					if (captionPress != null && caption != null && pe.y <= materialCaption.getExtraHeight()) {
						...
						firstPenDown = false;
						if (!editable
							&& !Settings.fingerTouch) { // guich@tc100: allow the user to scroll by just clicking in the ME
							event.target = sb;
	...
	private void clipboardCut() {
		...
		if (sel1 != -1) {
			Vm.clipboardCopy(chars.toString().substring(sel1,
							 sel2)); // brunosoares@tc100: BlackBerry does not support StringBuffer.substring()
	...
	private void showSip() { // guich@tc126_21
		if (kbdType != Edit.KBD_NONE && Settings.virtualKeyboard && editMode && editable
			&& !hadParentScrolled()
			&& !Window.isScreenShifted()
			&& Settings.enableVirtualKeyboard) { // if running on a PocketPC device, set the bounds of Sip in a way to not cover the edit - kmeehl@tc100: added check for editMode and !dragScroll
	...
	protected void onWindowPaintFinished() {
		if (editable && !hasFocus) {
			_onEvent(new ControlEvent(ControlEvent.FOCUS_IN,
									  this)); // this event is called on the focused control of the parent window. so, if we are not in FOCUS state, set it now. --original - guich@350_7: added the editable check
		}
	}
	...
	protected void onColorsChanged(boolean colorsChanged) {
		...
		back1 = back0 != Color.WHITE ? backColor : Color.getCursorColor(
					back0);//guich@300_20: use backColor instead of: back0.getCursorColor();
		if (!uiAndroid) {
	...
	public Control handleGeographicalFocusChangeKeys(KeyEvent ke) {
		boolean processEvent = editable && (!improvedGeographicalFocus || (!ke.isPrevKey()
											&& !ke.isNextKey())
											|| (ke.key == SpecialKeys.LEFT && insertPos > 0) || (ke.key == SpecialKeys.RIGHT
													&& insertPos < chars.length())
