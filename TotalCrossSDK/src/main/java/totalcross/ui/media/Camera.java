...
/**
 * This class is used to enable the camera of the underlying device. The
 * following platforms are supported: Windows Mobile devices, Blackberry, Palm
 * OS, Android and iOS.
 *
 ...
 *
 * <p>Note that you can easily rotate the image to put it in portrait mode,
 * using the <code> Image.getRotatedScaledInstance</code> method, after
 * retrieving the image.
 *
 * <p>To use this class with the Hand Held Dolphin barcode scanners, you must
 * install TotalCross3/dist/vm/wince/POCKETPC/ARM/Dolphin.dll. Only 640x480
 * photos are supported, and you may change the following options:
 *
 * <ul>
 *   <li>initialDir
 *   <li>defaultFileName (must end with jpg)
 *   <li>resolutionWidth x resolutionHeight: possible values are 640x480 and
 * 212x160 (different values defaults to 640x480)
 * </ul>
 *
 * All other options are ignored.
 *
 * <p>In Android you can set the defaultFileName, stillQuality, resolutionWidth
 * and resolutionHeight. All other options are ignored.
 *
 * <p>You can call the getSupportedResolutions method to see the resolutions
 * that are available at the device.
 *
 * <p>In iOS there's no way to return the supported resolutions; it will take a
 * photo using the default camera's resolution, and then will resize to the
 * resolution defined in resolutionWidth/resolutionHeight, keeping the camera's
 * aspect ratio. In iOS you can specify the defaultFileName with a path or just
 * the name, or use a system-generated name.
 *
 * <p>In Android, you can use the internal camera by setting the cameraType
 * field.
 *
...
public class Camera
{
  ...
  /**
   * This field is false by default so that the default camera orientation is
   * still landscape. If this is set to true, the camera orientation will follow
   * the device orientation. Used on Android only.
   */
  ...
  /**
   * The native camera application; the original image is deleted and a copy of
   * it is returned.
   */
   ...
   *
   * @return The String with the file name where the image is located or null if
   *     the user canceled.
  ...
  public String click() throws IOException
  {
    if (Settings.onJavaSE || Settings.isWindowsCE() ||
        Settings.platform.equals(Settings.WIN32) || Settings.isIOS() ||
      ...
      if (Settings.platform.equals(Settings.ANDROID) &&
          cameraType == CAMERA_FROM_GALLERY) {
        try {
          File f = new File(output, File.READ_WRITE);
          if (f.getSize() < 500) { // If less than 500 bytes long it is likely a
                                   // path, else a image
  ...
  @ReplacedByNativeOnDeploy private String nativeClick()
  {
    FileChooserBox fcb =
      new FileChooserBox(fcbTitle, FileChooserBox.defaultButtonCaptions, null);
    fcb.showPreview = fcb.newestFirst = true;
    fcb.popup();
    return fcb.getAnswer();
  }

  @Override protected void finalize() throws Throwable { nativeFinalize(); }

  @ReplacedByNativeOnDeploy private void nativeFinalize() {}

  /**
   * Returns the resolutions that are supported by the device. Works for Windows
   * CE, Blackberry and Android.
   *
   * <p>When the information is not available, a default list is returned and
   * the first element is set to "Default values:".
   *
   * <p>If you take a photo and an image with a different resolution is
   * returned, its because the selected resolution is not supported by the
   * device.
   *
  ...
  public static String[] getSupportedResolutions()
  {
    ...
    if (Settings.platform.equals(Settings.ANDROID)) {
      ...
      String[] folders =
        Registry.list(Registry.HKEY_LOCAL_MACHINE, regPictureResolution);
      for (String folder : folders) {
        final String fullKey = regPictureResolution + "\\" + folder;
        try {
          final int w =
            Registry.getInt(Registry.HKEY_LOCAL_MACHINE, fullKey, "Width");
          final int h =
            Registry.getInt(Registry.HKEY_LOCAL_MACHINE, fullKey, "Height");
          v.add(w + "x" + h);
        } catch (Exception e) {
          // key not found
        }
    ...
    if (ret == null) {
      ret = new String[] {
        "default resolution", "320x240", "640x480", "1024x768", "2048x1536"
      };
    }
    return ret;
  }
  ...
  @ReplacedByNativeOnDeploy private static String getNativeResolutions()
  {
    return null;
  }
}
