 ...
 * where the underlying platform supports generating tones. Tones aren't
 * supported under Java. <p> Here is an example that beeps the speaker and plays
 * a tone:
 *
 * <pre>
 * Sound.beep();
...
public final class Sound
{
   ...
   * is played asynchronously, so you must do a Vm.sleep(ms) for each tone
   * played if you play more than one.
   *
   ...
   */
  public static void setEnabled(boolean on) { soundEnabled = on; }
   ...
   * <p>Note: The smaller the midi number, the greater the error in the
   * frequency, due to the fact that the frequency calculated must be an
   * integer.
   ...
   */
  @Deprecated public static void midiTone(int midiNoteNumber, int duration)
  { // guich@300_33
    int freq = (int)Math.round(
      440.0 * Math.pow(2, ((double)midiNoteNumber - 69.0) / 12));
    tone(freq, duration);
  }

  /**
   * Plays the given short wav or mp3 file. Make sure that the sounds are
   * enabled, or you will not hear it! Works on Android, iOS and Win32, but does
   * not work on JavaSE. On WinCE, supports playing only WAV files.
   *
   * The file must be located in the file system.
   * If you store a mp3 file in the TCZ, you can get it out using this code:
   * <pre>
   * new File("device/mysound.mp3",
   * File.CREATE_EMPTY).writeAndClose(Vm.getFile("mysound.mp3"));
   * </pre>
   * Then you play it using:
   * <pre>
   * Sound.play("device/mysound.mp3");
   * </pre>
   * The last sound is cached, so playing it again is fast. If you want to
   * unload it, just call <code>Sound.play("");</code>, but this is not needed
   * since small mp3/wav files consumes just a few memory. Cache is not done in
   * Win32.
   */
   ...
   * The params can be: <pre> title:title to show to user timeout: timeout in
   * millis that will be waiten to return after dictation finishes
   * </pre>
   ...
   * </pre>
   */
  public static String toText(String params) { return ""; }

  /**
   * Activates speech and let the user hear the given text.
   * The very first time it is used in the device, Android will ask you to
   * install a high definition language package (and you should install it
   * because the difference is impressive).
   *
   * The first time it is called, you can pass some parameters:
   * <pre>
   * languages: shows in adb's logcat the available languages. For example, in
   * the Samsung S4 mini, it shows: [spa_MEX_f00, tur_TUR_l02, jpn_JPN_f00,
   * kor_KOR_f00, por_PRT_f00, eng_USA_f00, rus_RUS_f00, eng_GBR_f00,
   * por_BRA_f00, fra_FRA_f00, por_BRA_l01, ita_ITA_f00, deu_DEU_f00,
   * zho_CHN_f00, spa_ESP_f00] locale: the locale to be used. You can pass, for
   * example, spa_ESP or spa_ESP_f00 speech: the speech speed. The standard
   * is 1.0. A lower value will decrease the speed, a higher value will increase
   * it. quit: quits the speech engine.
   * </pre>
