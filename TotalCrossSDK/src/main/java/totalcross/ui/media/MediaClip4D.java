...
public class MediaClip4D
{
  ...
  final public void start() throws IOException
  {
    if (currentState == PREFETCHED) {
      ...
      do {
        ...
        subChunkId =
          (((mediaHeader[41] & 0xFF) << 24) | ((mediaHeader[40] & 0xFF) << 16) |
           ((mediaHeader[39] & 0xFF) << 8) | (mediaHeader[38] & 0xFF));
        dataPos += mediaStream.readBytes(mediaHeader, 42, 4);
        subChunkSize =
          (((mediaHeader[45] & 0xFF) << 24) | ((mediaHeader[44] & 0xFF) << 16) |
  ...
  native public void record(int samplesPerSecond,
                            int bitsPerSample,
                            boolean stereo) throws totalcross.io.IOException;
