...
public class MaterialEffect extends UIEffects implements PenListener, TimerListener {
	...
	private static final int X_WAS_UNKNOWN = X_UNKNOWN +
			2; // x was set but was unknown, dont ask to set again until animation stops
	...
	public void penDown(PenEvent e) {
		if (!sideEffectsOnly
			&& Flick.currentFlick ==
			null) { // guich@20171004 - if material is applied during a flick, it halts the flick making a strange effect
			if (isRunning()) {
				postEvent();
			}
			x = X_UNSET;
