...
public class MaterialEffect
  extends UIEffects implements PenListener, TimerListener
{
  ...
  private static final int X_WAS_UNKNOWN =
    X_UNKNOWN + 2; // x was set but was unknown, dont ask to set again until
                   // animation stops
  ...
  public MaterialEffect(Control target)
  {
    ...
    darkSideOnPress =
      target instanceof Button || target instanceof PushButtonGroup;
  }

  @Override public boolean isRunning() { return te != null; }

  @Override protected void finalize() { target.removePenListener(this); }

  @Override public void paintEffect(Graphics g)
  {
    if (te != null && enabled) {
      ...
      } else if ((penUpTimestamp - penDownTimestamp) <
                 duration) { // reached max radius and if there is still
        // time to fade out...
        alpha = (duration - curDn) * alphaValue / duration; // fading out
      } else {
        alpha = (penUpTimestamp + duration - ts) * alphaValue /
                duration; // fading out
      }
      if (alpha < 0) {
        alpha = 0;
      ...
      if (!sideEffectsOnly) {
        ...
        g.backColor = color != -1 && color != bc
                        ? color
                        : Color.getBrightness(bc) < 127 ? Color.brighter(bc, 64)
                                                        : Color.darker(bc, 64);

        g.backColor =
          Color.PremultiplyAlpha(target.getBackColor(), g.backColor, g.alpha);
        g.fillCircle(px, py, rad - 2);
        g.alpha = olda;
        //        if (isPenDown && darkSideOnPress) // make darker area at sides
        //        and bottom
        //        {
        //          g.alpha = 0x40000000;
        //          g.backColor = 0;
        //          g.backColor = Color.PremultiplyAlpha(target.getBackColor(),
        //          g.backColor, g.alpha); g.fillRect(2, h - 2, w - 4, 2);
  ...
  @Override public void penDown(PenEvent e)
  {
    if (!sideEffectsOnly &&
        Flick.currentFlick ==
          null) { // guich@20171004 - if material is applied during a flick,
                  // it halts the flick making a strange effect
      if (isRunning()) {
        postEvent();
      }
      x = X_UNSET;
  ...
  {
    if (savedPenUp != null && sideEffect == null &&
        !target.hadParentScrolled()) {
