...
class Table extends ScrollContainer implements CustomLayout, StopLayout, SizeDelimiter {
	...
	Container startCell(AttributeList atts, Style style) {
		...
		int pad = padding;
		if (borderWidth >= 1) {
			cell.setBorderStyle(borderWidth == 1 ? BORDER_LOWERED : borderWidth == 2 ? BORDER_SIMPLE :
								BORDER_RAISED);
			pad++;
		}
			...
			for (int c = 0; c < maxColspan; c++) {
				...
				if (control instanceof CellSpan) {
					...
					if (c == 0) {
						...
						ww = cell1 == cell ? 0 :
							 cell.getWidth(); // if its the same owner, the width was already computed, so assign 0 to ww
					}
					hh = cell.getHeight();
				} else {
					...
					if (cell != null) { // guich@tc114_22: google.com.br?
						...
						if (cell.rowspan > 1) {
							...
							hh = Math.max(cell.getPreferredHeight(),
										  hh); // if the cell has a height bigger than the sum of the other rows, use it
						}
