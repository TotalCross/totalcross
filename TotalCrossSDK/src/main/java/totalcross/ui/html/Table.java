...
class Table
  extends ScrollContainer implements CustomLayout, StopLayout, SizeDelimiter
{
  int colCount;
  int borderWidth;
    ...
    if (s != null) {
      try {
        ...
        if (Settings.onJavaSE) {
          Vm.debug("Exception on Table's constructor: " + e + " " +
                   e.getMessage() + "");
        }
      }
    }
  ...
  Container startCell(AttributeList atts, Style style)
  {
    ...
    int pad = padding;
    if (borderWidth >= 1) {
      cell.setBorderStyle(borderWidth == 1
                            ? BORDER_LOWERED
                            : borderWidth == 2 ? BORDER_SIMPLE : BORDER_RAISED);
      pad++;
    }
  ...
  static class CellSpan extends Control
  {
    ...
    CellSpan(Cell owner) { this.owner = owner; }
  }
  ...
  private Control[][] adjustSpan()
  {
    ...
    for (int r = 0, i = cs.length, jj = 0; r < rows; r++, jj++) {
      for (int c = 0, ii = 0, maxCols = colsPerRow.items[r]; c < maxCols;
           c++, ii++) {
    ...
    /* now, rowspan: shift columns to right when a rowspan is found (C=Cell,
       S=CellSpan, 0=null - see Sample6 in HtmlBrowser) C C C C      C C C C C C
       C 0      S C C C C S C 0  =>  S C S C C C C C      C C C C C S C 0      S
       C S C C 0 0 0      S S S C      */
    for (int c = 0; c < maxColspan; c++) {
      for (int r = 0; r < maxRowspan; r++) {
        Control control = (Control)matrix[r][c];
        Cell cell = control instanceof CellSpan ? ((CellSpan)control).owner
                                                : (Cell)control;
        if (cell == null) {
          continue;
        }
    ...
    // dump - for (int i = 0; i < rows; i++) for (int j =0; j < maxColspan; j++)
    // System.out.print((matrix[i][j] == null ? "0 " : matrix[i][j] instanceof
    // Cell ? "C " : "S ")+(j==maxColspan-1?"\n":""));
    return matrix;
  }
      ...
      for (int c = 0; c < maxColspan; c++) {
        ...
        if (control instanceof CellSpan) {
          ...
          if (c == 0) {
            ...
            Cell cell1 =
              c1 instanceof CellSpan ? ((CellSpan)c1).owner : (Cell)c1;
            ww = cell1 == cell
                   ? 0
                   : cell.getWidth(); // if its the same owner, the width was
                                      // already computed, so assign 0 to ww
          }
          hh = cell.getHeight();
        } else {
          ...
          if (cell != null) { // guich@tc114_22: google.com.br?
            ...
            if (cell.rowspan > 1) {
              ...
              hh = Math.max(cell.getPreferredHeight(),
                            hh); // if the cell has a height bigger than the sum
                                 // of the other rows, use it
            }
