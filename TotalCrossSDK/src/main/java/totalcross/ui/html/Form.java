...
public class Form {
		...
		for (int i = 0; i < n; i++) {
			...
			if (name != null) {
				...
				if (val != null && listeners != null
					&& ((Listener) listeners.get(0)).listener instanceof Document.SubmitReset
	...
	public static String getValue(Control c) {
		...
		if (c instanceof ComboBox) {
			...
			return (sel >= 0
					|| (sel = cb.clearValueInt) >= 0) ? ((Document.Entry) cb.getItemAt(sel)).key : null;
		}
		...
		if (c instanceof ListBox) {
			return ((ListBox) c).getSelectedIndex() >= 0 ? ((Document.Entry)((ListBox)
					c).getSelectedItem()).key : null;
		}
		return cp.value;
	}
