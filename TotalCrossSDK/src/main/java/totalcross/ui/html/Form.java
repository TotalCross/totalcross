...
public class Form
{
  ...
  public String buildURL(String buttonName)
  {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (name != null) {
        ...
        if (val != null && listeners != null &&
            ((Listener)listeners.get(0)).listener instanceof
              Document.SubmitReset &&
        ...
        if (!ht.exists(name)) {
          ...
          ht.put(name,
                 val == null
                   ? old
                   : (old.length() > 0 ? old + "=" : "") + URI.encode(val));
        }
      }
    }
  ...
  public void submit(String buttonValue)
  {
    ...
    if (htmlCnr != null) {
      ...
      Vm.warning(
        "Form can't be submitted because there are no inputs whose parent is a HtmlContainer.");
    }
  }
  ...
  private HtmlContainer getHtmlContainer()
  {
    for (int i = inputs.size(); --i >= 0;) {
      for (Control control = (Control)inputs.items[i]; control != null;
  ...
  /**
   * Returns the value of the given Control. Used by this Form to build the url
   * with parameters.
   */
  public static String getValue(Control c)
  {
    ...
    if (c instanceof Check) {
      return ((Check)c).isChecked() ? (cp.value == null ? "on" : cp.value)
                                    : null;
    }
    ...
    if (c instanceof ComboBox) {
      ...
      return (sel >= 0 || (sel = cb.clearValueInt) >= 0)
        ? ((Document.Entry)cb.getItemAt(sel)).key
        : null;
    }
    if (c instanceof MultiListBox) {
      ...
      if (n > 0) {
        for (int i = 0, k = 0; i < n; i++) {
          if (mlb.isSelected(i)) {
            sb.append(k++ > 0 ? "," : "")
              .append(((Document.Entry)mlb.getItemAt(i)).key);
          }
        }
      }
      return sb.toString();
    ...
    if (c instanceof ListBox) {
      return ((ListBox)c).getSelectedIndex() >= 0
        ? ((Document.Entry)((ListBox)c).getSelectedItem()).key
        : null;
    }
    return cp.value;
  }
