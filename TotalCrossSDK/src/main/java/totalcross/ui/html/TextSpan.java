...
class TextSpan extends Control implements Document.CustomLayout, Document.SizeDelimiter {
		...
		do {
			...
			if (lc.x + newWidth <= lc.maxWidth) {
				...
				if (curWidth > 0) {
					...
					if (DEBUG) {
						Vm.debug("1. \"" + new String(text, lineStart,
													  wordEnd - lineStart) + "\": " + curWidth + " " + isLastLine);
					}
					addLine(lineStart, wordEnd, true, lc, glue);
					curWidth = 0;
					isLastLine = false; // must recompute the last line, since there's at least a word left.
				} else if (
					!lc.atStart()) { // case of "this is a text at the end <b>oftheline</b>" -> oftheline will overflow the screen
					if (++tries == 2) {
						break;
					}
					if (DEBUG) {
	 ...
	 */
	private void addLine(int textStart, int textEnd, boolean isDisjoin, LayoutContext lc,
						 boolean glue) {
		//String s = new String(text, textStart,textEnd-textStart);
		int xx = style.getControlAlignment(
					 true); //flsobral@tc126: use the new method to get the line's alignment.
		...
		if (style.alignment == Style.ALIGN_CENTER || style.alignment == Style.ALIGN_RIGHT) {
			l.setRect(xx, KEEP, KEEP, KEEP,
					  lc.parentContainer); //flsobral@tc126: make line relative to the layout context parent container when aligned with center or right.
		}
		if (isDisjoin) {
			lc.disjoin();
		} else {
