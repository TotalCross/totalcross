  ...
  extends Control implements Document.CustomLayout, Document.SizeDelimiter
{
  ...
  private class TextLine extends totalcross.ui.Control
  {
    ...
    @Override public int getPreferredHeight() { return fmH + Edit.prefH; }
    ...
    @Override public void onPaint(Graphics g)
    {
      ...
      if ((style.fontBits & (Style.STRIKETHROUGH | Style.UNDERLINE)) != 0) {
        int ypos = (style.fontBits & (Style.STRIKETHROUGH)) != 0
                     ? fm.height / 2
                     : fm.ascent + 2;
  ...
  private int computeMaxWidthPerLine(String input, char[] inputChars)
  {
    ...
    while ((newPosition = input.indexOf('\n', position)) >= 0) {
      maxWidth = Math.max(
        maxWidth, fm.stringWidth(inputChars, position, newPosition - position));
  ...
  @Override public int getMaxWidth() { return maxWordWidth; }
  ...
  @Override public void layout(LayoutContext lc)
  {
    ...
    boolean wasDisjoin =
      lc.lastStyle != null && lc.lastStyle.alignment == Style.ALIGN_NONE;
    lc.lastStyle = style;
  ...
  private void makeLines(LayoutContext lc)
  {
    ...
    int tries = 0;
    int curWidth, wordWidth, lineStart, wordEnd, wordStart, newWidth, textLen,
      beg, maxM2, spaceW;
    ...
    do {
          ...
          do {
            if (++wordEnd >= textLen) {
              ...
              break;
                   ...
                   text[wordEnd] !=
                     '/'); // loop until the next space/slash char
          // use slashes as word delimiters (useful for URL addresses).
          if (maxM2 > wordEnd && text[wordEnd] == '/' &&
              text[wordEnd + 1] != '/') {
            wordEnd++;
          }
          break;
      ...
      if (lc.x + newWidth <= lc.maxWidth) {
        ...
        if (curWidth > 0) {
          // At here, wordStart and wordEnd refer to the word that overflows.
          // So, we have to stop at the previous word
          wordEnd = wordStart;
          if (text[wordEnd - 1] == ' ') {
            wordEnd--;
          }
          if (DEBUG) {
            Vm.debug("1. \"" +
                     new String(text, lineStart, wordEnd - lineStart) +
                     "\": " + curWidth + " " + isLastLine);
          }
          addLine(lineStart, wordEnd, true, lc, glue);
          curWidth = 0;
          isLastLine = false; // must recompute the last line, since there's at
                              // least a word left.
        } else if (!lc.atStart()) { // case of "this is a text at the end
                                    // <b>oftheline</b>" -> oftheline will
                                    // overflow the screen
          if (++tries == 2) {
            break;
          }
          ...
          // Nothing was gathered in, but the current line has characters left
          // by a previous TextSpan.  This occurs only once.
          addLine(0, 0, false, lc, glue);
          curWidth = 0;
          isLastLine = false; // must recompute the last line, since there's at
                              // least a word left.
        } else {
          // Rare case where we both have nothing gathered in, and the physical
          // line is empty.  Had this not been made, then we woud have generated
          // an extra-line at the top of the block.
          if (DEBUG) {
            Vm.debug("3. \"" +
                     new String(text, lineStart, wordEnd - lineStart) + '"');
          }
    ...
    if (wordEnd != lineStart) {
      ...
      boolean split = lc.x + curWidth > lc.maxWidth &&
                      style.hasInitialValues() && style.isDisjoint;
      if (DEBUG) {
        Vm.debug("4. \"" + new String(text, lineStart, wordEnd - lineStart) +
                 "\" " + split);
      }
   ...
   */
  private void addLine(int textStart,
                       int textEnd,
                       boolean isDisjoin,
                       LayoutContext lc,
                       boolean glue)
  {
    ...
    int xx =
      style.getControlAlignment(true); // flsobral@tc126: use the new method
                                       // to get the line's alignment.
    ...
    if (style.alignment == Style.ALIGN_CENTER ||
        style.alignment == Style.ALIGN_RIGHT) {
      l.setRect(xx,
                KEEP,
                KEEP,
                KEEP,
                lc.parentContainer); // flsobral@tc126: make line relative to
                                     // the layout context parent container when
                                     // aligned with center or right.
    }
    if (isDisjoin) {
      lc.disjoin();
    } else {
  ...
  /*   static int conta;
   ...
   {
      System.err.println(conta++ +" "+positionName(lc.nextX + ((lc.nextX ==
   LEFT) ? style.indent : 0))+" "+positionName(lc.nextY + ((lc.nextY == AFTER) ?
   style.topMargin : 0))+" "+style.alignment+"/"+lc.x+" "+text);
   }
