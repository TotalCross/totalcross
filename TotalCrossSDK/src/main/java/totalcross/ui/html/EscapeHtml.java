...
{ // guich@571_10
  ...
  /**
   * Converts special chars into escaped sequences. Note that the space is NOT
   * converted.
   */
  public static final String escape(String s)
  {
    ...
    for (int i = 0, n = s.length(); i < n; i++) {
      ...
      switch (c) {
        case 0:
          break; // flsobral@tc120_58: ignore character of value 0, because it
  ...
  /**
   * Converts escaped sequences into special chars. Note that the space IS
   * converted if it appears escaped.
   */
  public static final String unescape(String escaped)
  {
    ...
    while (true) {
      ...
      if (2 <= len && len <= 6) { // is this a valid symbol?
        ...
        // String token = escaped.substring(s0+1,sf); int hash =
        // token.hashCode();
        try {
          int to = ht.get(hash);
          if (s0 > last) {
