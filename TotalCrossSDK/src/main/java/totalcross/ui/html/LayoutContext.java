...
final class LayoutContext
{
  ...
  /**
   * Parent container with a graphical representation which this context is
   * relative to. Controls should have their bounds set relative to this
   * container whenever a layout context is available. <br> <br> Created to
   * avoid using the controls' parent, which is often a container with no
  ...
  public LayoutContext(int maxWidth, Container parentContainer)
  {
    if (parentContainer == null) {
      throw new NullPointerException(
        "Argument 'parentContainer' cannot be null");
    }
  ...
  public boolean atStart() { return x == gap; }

  /**
   * Verify if there's enough space to add this control and do a disjoin
   * otherwise.
   */
