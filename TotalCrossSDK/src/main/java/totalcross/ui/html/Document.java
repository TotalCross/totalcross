...
public class Document extends ScrollContainer {
	 ...
	 * @throws totalcross.io.IOException
	 */
	protected void renderDoc(XmlReadable doc) throws totalcross.io.IOException,
		SyntaxException { // guich@510_22
	...
	private class Img extends Control implements Document.CustomLayout, Document.SizeDelimiter {
		...
		Img(Style currStyle, AttributeList atts, URI baseURI) {
			...
			if (src != null) {
				try {
					img = Document.this.loadImage(src,
												  baseURI); // let our caller handle the image - maybe it comes from another source?
					if (img == null) {
	...
	static class Entry {
		...
		public boolean equals(Object
							  arg0) { //flsobral@tc114_92: added method equals to class totalcross.ui.html.Document.Entry, to compare only the entry value. This fixes an issue with Form.setValue.
	...
	private class Builder extends XmlReader {
		...
		private class BodyContentHandler extends ContentHandler {
			...
			public void characters(String s) {
				if (!isDataCDATA()
					&& s.trim().length() > 0) { //mike@570_59 Don't create textspan tiles for empty texts
	...
	static void layout(Control control, LayoutContext lc) {
		if (control instanceof CustomLayout) {
			...
			if (control instanceof BR) {
				...
				int dif = (control.getFont().fm.height + Edit.prefH - control.getPreferredHeight()) /
	...
	private int getMaxWidth(Control control, int maxWidth) {
		maxWidth = Math.max(maxWidth,
							control instanceof SizeDelimiter ? ((SizeDelimiter) control).getMaxWidth() :
