...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//!!!!  REMEMBER THAT ANY CHANGE YOU MAKE IN THIS CODE MUST BE SENT BACK TO
//! SUPERWABA COMPANY     !!!!
//!!!!  LEMBRE-SE QUE QUALQUER ALTERACAO QUE SEJA FEITO NESSE CODIGO DEVERÁ SER
//! ENVIADA PARA NOS  !!!!
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 * Here is a sample of how to load an image of a document instead of downloading
 * it from a URL: <pre>
 * // Extends Document so that we can provide another way to load images, from
 * our internal database class ArticleDocument extends Document
 * {
 *    PDBFile cat;
 *    DataStream ds;
 *
 *    /** The cat parameter should be an already open PDBFile which contains all
 * the images.
 *      * Note that the image is in the form "<rec number>.gif" or "<rec
 * number>.jpeg".
 *      * /
 ...
 *    {
 *       int dot = src.indexOf('.');
 *       if (dot == -1) throw new RuntimeException("src image not in expected
 * format: "+src); int idx = Convert.toInt(src.substring(0,dot)); String ext =
 * src.substring(dot+1).toLowerCase(); if (cat.setRecordPos(idx))
 *       {
 ...
 * if (!s.substring(0,10).toLowerCase().startsWith("<body")) // make sure we are
 * between a body tag s = "<body>"+s+"</body>";
 * // Displays the xml
 * hcView.setDocument(new ArticleDocument(new XmlReadableString(s),cat)); // cat
 * must be already instantiated
 * </pre>
 * To change the font size, you must change Style.defaultFontSize. To change the
 * default colors, change UIColors.htmlXXX fields, and don't forget to call
 * htmlContainer.setBackForeColors with these colors.
...
public class Document extends ScrollContainer
{
  ...
  public Document(XmlReadable doc)
    throws totalcross.io.IOException, SyntaxException
  {
    renderDoc(doc);
  }
  ...
  /**
   * Can be overridden by the caller in order to load an image from a different
   * place. Must return null if not handled/not found. By default, searches for
   * the given file at the loaded libraries (TCZ files).
   * @param src The image name
   * @param baseURI The base URI
   * @return an Image
   */
  protected Image loadImage(String src, URI baseURI)
  { // guich@510_20
    try {
  ...
  static class Hidden extends Control
  {
    ...
    @Override public int getPreferredWidth() { return 0; }

    @Override public int getPreferredHeight() { return 0; }
  }

  private class Img
    extends Control implements Document.CustomLayout, Document.SizeDelimiter
  {
    /**
    ...
    Img(Style currStyle, AttributeList atts, URI baseURI)
    {
      ...
      this.align = v == null ? currStyle.getControlAlignment(false)
                             : v.equalsIgnoreCase("center")
                                 ? CENTER
                                 : v.equalsIgnoreCase("right") ? RIGHT : LEFT;
      v = atts.getAttributeValue("valign");
      this.valign =
        v == null ? TOP
                  : v.equalsIgnoreCase("middle") || v.equalsIgnoreCase("center")
                      ? CENTER
      ...
      if (src != null) {
        try {
          img = Document.this.loadImage(
            src,
            baseURI); // let our caller handle the image - maybe it comes from
                      // another source?
          if (img == null) {
          ...
          if (img != null) {
            ...
            if ((imgWidth = img.getWidth()) <= 0 ||
                (imgHeight = img.getHeight()) <= 0) {
              img = null;
    ...
    @Override public int getMaxWidth() { return img.getWidth(); }

    @Override public int getPreferredWidth() { return img.getWidth(); }

    @Override public int getPreferredHeight() { return img.getHeight(); }

    @Override public void layout(LayoutContext lc)
    {
      ...
      // mike@570_59 implemented layouting accordingly to the alignment of the
      // image
      switch (align) {
        case CENTER:
    ...
    @Override public void onPaint(Graphics g) { g.drawImage(img, 0, 0); }
    ...
    { // mike@570_59
      ...
      if ((e.type == PenEvent.PEN_DOWN ||
           e.type == KeyEvent.ACTION_KEY_PRESS) &&
  ...
  static class Entry
  {
    ...
    { // flsobral@tc114_92: added method equals to class
      // totalcross.ui.html.Document.Entry, to compare only the
      // entry value. This fixes an issue with Form.setValue.
  ...
  private class Builder extends XmlReader
  {
    ...
    @Override protected void foundReference(byte input[], int offset, int count)
    {
    ...
    @Override protected int getTagCode(byte b[], int offset, int count)
    {
    ...
    private class HeadContentHandler extends ContentHandler
    {
      ...
      HeadContentHandler() { setContentHandler(this); }

      @Override public void startElement(int tagHashId, AttributeList atts)
      {
        switch (tagHashId) {
          ...
          case TagDereferencer.BODY:
            // flsobral@tc126_36: added support to bgcolor attribute in body
            // tag.
            String bgcolor = atts.getAttributeValue("bgcolor");
            if (bgcolor != null) {
              currStyle.backColor =
                Style.getColor(bgcolor, currStyle.backColor);
            }
      ...
      @Override public void endElement(int tagHashId)
      {
    ...
    private class BodyContentHandler extends ContentHandler
    {
      ...
      @Override public void startElement(int tagHashId, AttributeList atts)
      {
        ...
        switch (tagHashId) {
          ...
          case TagDereferencer.INPUT:
            ...
            if (type == null || type.equalsIgnoreCase(
                                  "password")) { // Edit is the default input
              ...
              control = new Button(
                value == null ? (isReset ? "Reset" : "Submit") : value);
              ((Button)control)
                .addPressListener(new SubmitReset(currForm, isReset, value));
        ...
        if (control != null) {
          ...
          if (currForm != null && !(control instanceof BR)) {
            currForm.inputs.addElement(
              control); // add this control to the form.
          }
          currTile.add(control);
          control.setBackForeColors(UIColors.htmlContainerControlsBack,
                                    UIColors.htmlContainerControlsFore);
        }
      }
      ...
      @Override public void endElement(int tagHashId)
      {
        switch (tagHashId) {
          ...
          default:
            currStyle = Style.tagEndFound(currStyle, tagHashId);
            if ((Style.getStyle(tagHashId) & Style.P_AFTER) !=
                0) { // add a <p> after a <Hn>
              currTile.add(new BR(false));
              currTile.add(new BR(false));
            }
            break;
      ...
      @Override public void characters(String s)
      {
        if (!isDataCDATA() &&
            s.trim().length() >
              0) { // mike@570_59 Don't create textspan tiles for empty texts
    ...
    private class TableContentHandler extends BodyContentHandler
    {
      ...
      {
        // super() set currTile to the Document.rootTile this is a fall-back in
        // case TextSpan are created while no table cells exist.  This garbaged
        // text will show up after the end of the table.
        super();
        this.table = table;
      }
    ...
    private class TextAreaContentHandler extends BodyContentHandler
    {
      ...
      @Override public void characters(String s) { sb.append(s); }
    }
  }
  ...
  static void layout(Control control, LayoutContext lc)
  {
    if (control instanceof CustomLayout) {
      ...
      if (control instanceof BR) {
        ...
        int dif = (control.getFont().fm.height + Edit.prefH -
                   control.getPreferredHeight()) /
                  2; // vertically align the controls with their texts
        if (dif < 0) {
          dif = 0;
        }
        ...
        switch (style.alignment) {
          case Style.ALIGN_CENTER:
            alignedPosX +=
              (lc.parentContainer.getWidth() - control.getPreferredWidth()) / 2;
            break;
          case Style.ALIGN_RIGHT:
            alignedPosX +=
              lc.parentContainer.getWidth() - control.getPreferredWidth();
            break;
          case Style.ALIGN_LEFT:
            // default, left aligned
            break;
        }
        control.setRect(alignedPosX,
                        lc.nextY + dif,
                        Control.PREFERRED,
                        Control.PREFERRED,
                        lc.lastControl);
  ...
  private int getMaxWidth(Control control, int maxWidth)
  {
    maxWidth = Math.max(maxWidth,
                        control instanceof SizeDelimiter
                          ? ((SizeDelimiter)control).getMaxWidth()
  ...
  @Override public void initUI()
  {
    ...
    layout(
      this,
      new LayoutContext(getClientRect().width - sbV.getPreferredWidth(), this));
    resize();
  }
  ...
  { // guich@tc114_28
    ...
    for (int i = vForms.size(); --i >= 0;) {
      ...
      for (int j = inputs.size(); --j >= 0;) {
        ...
        if (name != null && (v = (String)ht.get(name)) != null &&
            v.length() > 0) {
