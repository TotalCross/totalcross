...
public class Node extends Vector implements totalcross.util.Comparable
{
   ...
   *
   * The userObject can also be a Control (or Container); in this case, all
   controls inside of
   * it are drawn. The control itself is added to the Tree at an invisible
   location and is painted
   * at the right position. This means that events are somewhat limited. You
   should change the lineH
  ...
  /**
   * Default constructor to create a tree node that has no parent and no
   * children, but which allows children.
   */
  public Node() { this(""); }

  /**
   * Default constructor to create a tree node with no parent, no children, but
   * which allows children, and initializes it with the specified user object.
   *
   * @param userObject
   *           the user object.
   */
  ...
  /**
   * Method to remove newChild from its parent and makes it a child of this node
   * by adding it to the end of this node's child vector.
   *
   * @param newChild
  ...
  /**
   * Method to returns the child in this node's child array that immediately
   * precedes aChild, which must be a child of this node; otherwise, retrun
   * null.
   *
   ...
   */
  public Node getFirstChild() { return count == 0 ? null : (Node)items[0]; }
  ...
  /**
   * Method to return the number of levels above this node -- the distance from
   * the root to this node.
   *
   * @return the number of levels above this node -- the distance from the root
   *     to this node
   */
  public int getLevel()
  {
   ...
   *     Returns null if this node has no parent or
  ...
  /**
   * Method to return the previous sibling of this node in the parent's children
   ...
   *
   * @param newChild
   *           the new child to remove from this subtree and add to this node
   * children vector at the specified index.
   * @param childIndex
  ...
  public int insert(Node newChild, int childIndex)
  {
    ...
    try {
      ...
      newChild.parent = this;
      return childIndex < 0 || childIndex >= count ? indexOf(newChild)
                                                   : childIndex;
    } catch (Exception e) {
      addElement(newChild);
      newChild.parent = this;
  ...
  /**
   * Method to return true if this node is a root. Root node is node that has a
   * null parent node.
   */
   ...
   * as) this node.
   *
   * @return true if anotherNode is a sibling of (has the same parent as) this
   *     node.
   */
  ...
  /**
   * Method to remove the child at the specified index from this node's children
  ...
  /**
   * Method to remove aChild from this node's child array, giving it a null
   * parent.
   *
   * @param aChild
  ...
  /**
   * Method to remove all of this node's children, setting their parents to
   * null. If you don't want to set their parents to null, call
  ...
  /**
   * Method to remove the subtree rooted at this node from the tree, giving this
   * node a null parent.
   */
   ...
   *
   * @return the result of sending toString() to this node's user object, or
   *     null if this node has no user object.
   */
  @Override public String toString() { return getNodeName(); }

  @Override public int compareTo(Object o)
  {
    Node other = (Node)o;
    if (other.userObject instanceof totalcross.util.Comparable) {
      return ((totalcross.util.Comparable)userObject)
        .compareTo((totalcross.util.Comparable)other.userObject);
    }
    return this.equals(other) ? 0 : -1;
  }

  @Override public boolean equals(Object o) { return o.equals(userObject); }
}
