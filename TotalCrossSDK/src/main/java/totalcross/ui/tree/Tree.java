 ...
 * natural to render the tree in rows, this class borrows most of the code from
 * ListBox. Features: <ul> <li> similar to Microsoft Windows Explorer tree <li>
 * horizontal and vertical scrolling <li> allows setting of folder and leaf
 * icons. <li> expands and collapse of folder <li> allowsChildren flag to
 * determine if the node is a leaf or folder <li> delete, insert, and modify
 * (user object) of a node <li> clicking on leaf node will swap leaf icon (like
 * hyperlink) <li> allows creation of tree to show or hide root node.
 * </ul>
 ...
 * </pre>
 * You can also see the FileChooserBox control and FileChooserTest (in UIGadgets
 * sample).
 *
 * @see Node#userObject
 */
  ...
  implements PressListener, PenListener, KeyListener, Scrollable
{
  ...
  /**
   * Set to true to allow multiple selections using a Check drawn before the
   * nodes.
  ...
  protected int
    itemCount; // the vertical scrollbar maximum (size of items vector)
  ...
  private boolean allowsChildren =
    true; // flag to use the node's allowsChildren to determine
  ...
  private static final int hline =
    3; // number of pixel used to draw horizontal line (from
  // plus icon to gap between plus icon and folder or leaf icon)
  private static final int gap =
    1; // the number of space(in pixel) between the plus icon
  ...
  private int lastV = -10000000, lastH =
                                   -10000000; // eliminate duplicate events
  ...
  {
    super.add(vbar = Settings.fingerTouch
                       ? new ScrollPosition(ScrollBar.VERTICAL)
                       : new ScrollBar(ScrollBar.VERTICAL));
    super.add(hbar = Settings.fingerTouch
                       ? new ScrollPosition(ScrollBar.HORIZONTAL)
  ...
  @Override public boolean canScrollContent(int direction, Object target)
  {
    if (flickDirection == NONE) {
      flickDirection = direction == DragEvent.UP || direction == DragEvent.DOWN
                         ? VERTICAL
                         : HORIZONTAL;
    }
    if (Settings.fingerTouch) {
  ...
  @Override public boolean scrollContent(int dx, int dy, boolean fromFlick)
  {
  ...
  /**
   * Method to set the tree root node with the new root node. If the new root
   * node is null, the tree is unchanged.
   *
   * @param root
  ...
  /**
   * Method to load icons use for the tree. You can change the icon by using the
   * setIcon(int iconType, Filename imageFilename).
   */
  ...
  /**
   * Method to set the icon of the tree based on the icon type. Note: You should
   * not change the plus and minus icons. You can set the open/close to null
   * (setting one will null out the other).
   *
   * @param iconType
  ...
  {
    if (iconType > 1) {
      img = img.smoothScaledFixedAspectRatio(lineH > fmH ? fmH : lineH * 8 / 10,
                                             true); // guich@tc110_19
   ...
   * fontmetrics. Note: if you override this class you must implement this
   * method.
   */
  ...
  /**
   * Method to empties this Tree, setting all elements of the array to null, so
  ...
  /**
   * Method to insert the items to the tree (For internal uses) Note: this
   * method does not reset the scroll bar, you need to call this
  ...
  /**
   * Method to remove the given index from the Tree items vector. This method
   * will not remove the node from the original node.
   *
   * @param index
  ...
  public void remove(int index)
  {
    ...
    do {
      ...
      index < itemCount &&
      level <
        ((Node)items.items[index]).level); // guich@tc126_42: inverted test
  ...
  private void resetHBar()
  {
    ...
    max +=
      vbar.getPreferredWidth(); // remember to take into account of the pixels
                                // used to draw the icons and scrollbar

    hbar.setMaximum(max);
    if (hbarPolicy == SCROLLBAR_ALWAYS ||
        (width - vbar.getPreferredWidth()) < max) {
  ...
  /**
   * Method to rest the vertical and horizontal scrollbars properties. Note:
   * there's still a bug in resetting the horizontal scroll bar.
   */
  ...
  {
    int index, n;
    if (!node.expanded && (index = indexOf(node)) != -1 &&
        !node.isLeaf(allowsChildren) &&
        (n = node.size()) > 0) { // guich@tc126_5: added last test to prevent
                                 // that a root-only tree sets expand to true
      node.expanded = true;
  ...
  /**
   * Method to get the Object at the given Index. Returns an empty string in
   * case of error.
   *
   * @param i
   *           the index.
  ...
  /**
   * Method to return the selected item of the Tree or an empty String if no
  ...
  /**
   * Method to return the position of the selected item of the Tree or -1 if the
   * Tree has no selected index yet.
   *
  ...
  /**
   * Method to return all items in the items vector as an array of object. The
   * objects are of the class Node.
   *
  ...
  /**
   * Method to return the index of the item specified by the name, or -1 if not
   * found.
   *
   * @param name
   *           the object to find.
  ...
  /**
   * Method to select the given index and scroll to it if necessary. Note:
   * select must be called only after the control has been added to the
   * container and its rect has been set.
   *
   * @param i
  ...
  public void setSelectedIndex(int i)
  {
    if (0 <= i && i < itemCount && i != selectedIndex && height != 0) {
      ...
      if (i < offset || i >= offset + vi) { // guich@tc125_20: change offset
                                            // only if the item is not visible
  ...
  /**
   * Method to search this Tree for an item with the first letter matching the
   * given char. The search is made case insensitive. Note: if you override this
  ...
  @Override public void setBackColor(int c)
  {
  ...
  @Override protected void onColorsChanged(boolean colorsChanged)
  {
    ...
    bgColor1 = cursorColor != -1
                 ? cursorColor
                 : (bgColor0 != Color.WHITE) ? backColor
                                             : Color.getCursorColor(bgColor0);
  ...
  /**
   * Method to recalculate the box size for the selected item if the control is
   * resized by the main application .
   */
  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    ...
    vbar.setRect(Settings.fingerTouch ? RIGHT - 2 : RIGHT,
                 0,
                 PREFERRED,
                 FILL,
                 null,
                 screenChanged);
    hbar.setRect(0,
                 Settings.fingerTouch ? BOTTOM - 2 : BOTTOM,
                 btnW == 0 ? FILL : FIT,
                 PREFERRED,
                 null,
                 screenChanged);
  ...
  /**
   * Method to notify the tree that a node has been removed from the tree model
   * and to repaint the tree to reflect the changes, if necessary
   *
   * @param node
   *           the node that has been removed from the tree model
   */
  public void nodeRemoved(Node node) { remove(indexOf(node)); }
  ...
  /**
   * Method to notify the tree that a node in the tree model has been modified
  ...
  @Override public void penDrag(DragEvent de)
  {
    if (Settings.fingerTouch) {
      int dx = -de.xDelta;
      int dy = -de.yDelta;
      ...
      de.consumed = true; // guich@tc166: if inside a TabbedContainer, prevent
                          // it from scrolling
    }
  }
  ...
  @Override public void penDown(PenEvent pe)
  {
    if (pe.target != this) {
      return;
    }
  ...
  @Override public void penUp(PenEvent pe)
  {
    if (pe.target != this) {
      return;
    }
    ...
    if (isInsideOrNear(pe.x, pe.y) && pe.x < btnX && sel < itemCount) {
      ...
      if (isLeaf && pe.x >= xstart) {
        node.visited = true;
      } else if (((!scScrolled || !Settings.fingerTouch) &&
                  expandClickingOnText) ||
                 (!scScrolled && pe.x < xstart)) {
        // call expand and collapse or change the leaf icon on when clicked
        // anywhere
        if (!node.expanded) {
          expand(node);
        } else {
  ...
  @Override public void actionkeyPressed(KeyEvent e)
  {
  ...
  @Override public void keyPressed(KeyEvent e)
  {
    ...
    } else if (multipleSelection && selectedIndex >= 0 &&
               ((KeyEvent)e).key == ' ') {
  ...
  @Override public void specialkeyPressed(KeyEvent e)
  {
    if (lastControl != null) {
      e.target = lastControl;
      lastControl.onEvent(e);
  ...
  private boolean handleKeys(KeyEvent ke)
  {
    if (ke.modifiers != 0 && (ke.isDownKey() || ke.isUpKey())) {
      vbar.onEvent(ke);
               ...
               (ke.key == SpecialKeys.LEFT || ke.key == SpecialKeys.RIGHT)) {
      hbar.onEvent(ke);
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    // draw scrollbar border (why is it disappear in the first place? or is
    // there a border for the scrollbar class??)
    g.drawRect(btnX - 1, 0, vbar.getPreferredWidth() + 1, height);
    if (!Settings.fingerTouch && hbar.isVisible()) {
      g.drawRect(0,
                 height - hbar.getPreferredHeight() - 1,
                 width - vbar.getPreferredWidth() + 1,
  ...
  protected void drawNode(Graphics g, int index, int dx, int dy)
  {
    ...
    if (index > 0) {
      drawConnector(g,
                    ...
                    dx,
                    dy,
                    node); // draw the line that connect the nodes
    }
    boolean expand = node.expanded;
    int x =
      dx + (imgPlusSize + hline + gap + imgOpenW / 2 - imgPlusSize / 2) * level;
    int y = dy;
  ...
  private int getTextX(int level)
  { // guich@tc125_24
    int dx = multipleSelection ? x0 + imgPlusSize + 2 : x0;
    dx -= hsOffset;
    level--;
  ...
  protected void drawConnector(Graphics g, int index, int dx, int dy, Node node)
  {
    ...
    if (level == 0) {
      ...
      x += imgPlusSize + hline + gap + imgOpenW / 2 +
           (imgPlusSize / 2 + hline + gap + imgOpenW / 2 + 1) * (level - 1);
    }
    ...
    if (level == 0 && next == null && prev != null &&
        items.items[index] == node) {
    ...
    if (offset <= sel && sel < visibleItems + offset && sel < itemCount) {
      ...
      if (level == 1) {
        ...
        dx += imgPlusSize + hline + gap + imgOpenW +
              (imgPlusSize + hline + gap + imgOpenW / 2 - imgPlusSize / 2) *
                (level - 1);
      }
      ...
      dy += (sel - offset) * lineH;
      g.setClip(useFullWidthOnSelection ? 2 : dx - 1,
                dy - 1,
                btnX - (useFullWidthOnSelection ? 2 : dx),
                Math.min(lineH * visibleItems, this.height - dy));
      int oldb = g.backColor;
      g.backColor = bgColor1;
      int extraH = n.userObject != null && n.userObject instanceof Control
                     ? 0
                     : fm.descent - 1;
  ...
  /**
   * Method to set the cursor color for this Tree. The default is equal to the
  ...
  /**
   * Method to reload the tree. Use this method when the tree model has made a
   * drastic change.
   */
  public void reload()
  {
