...
public class Tree extends Container implements PressListener, PenListener, KeyListener, Scrollable {
	...
	public Tree(TreeModel model, boolean showRoot) {
		super.add(vbar = Settings.fingerTouch ? new ScrollPosition(ScrollBar.VERTICAL) : new ScrollBar(
							 ScrollBar.VERTICAL));
		super.add(
			hbar = Settings.fingerTouch ? new ScrollPosition(ScrollBar.HORIZONTAL) : new ScrollBar(
	...
	public void remove(int index) {
		...
		do {
			...
			itemCount--;
				 ...
				 && level < ((Node) items.items[index]).level); // guich@tc126_42: inverted test
	...
	public boolean expand(Node node) {
		...
		if (!node.expanded && (index = indexOf(node)) != -1 && !node.isLeaf(allowsChildren)
			&& (n = node.size()) >
			0) { // guich@tc126_5: added last test to prevent that a root-only tree sets expand to true
			node.expanded = true;
	...
	public void setSelectedIndex(int i) {
		if (0 <= i && i < itemCount && i != selectedIndex && height != 0) {
			...
			if (i < offset
				|| i >= offset + vi) { // guich@tc125_20: change offset only if the item is not visible
	...
	protected void onColorsChanged(boolean colorsChanged) {
		...
		bgColor1 = cursorColor != -1 ? cursorColor : (bgColor0 != Color.WHITE) ? backColor :
	...
	protected void onBoundsChanged(boolean screenChanged) {
		...
		hbar.setRect(0, Settings.fingerTouch ? BOTTOM - 2 : BOTTOM, btnW == 0 ? FILL : FIT, PREFERRED, null,
					 screenChanged);
	...
	public void penUp(PenEvent pe) {
		...
		if (isInsideOrNear(pe.x, pe.y) && pe.x < btnX && sel < itemCount) {
			...
			if (isLeaf && pe.x >= xstart) {
				node.visited = true;
			} else if (((!scScrolled || !Settings.fingerTouch) && expandClickingOnText) || (!scScrolled
					   && pe.x < xstart)) {
	...
	protected void drawConnector(Graphics g, int index, int dx, int dy, Node node) {
		...
		if (level == 0) {
			...
			x += imgPlusSize + hline + gap + imgOpenW / 2 + (imgPlusSize / 2 + hline + gap + imgOpenW / 2 + 1)
				 * (level - 1);
		}
