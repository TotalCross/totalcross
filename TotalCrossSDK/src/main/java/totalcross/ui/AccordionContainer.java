 ...
 * </pre>
 *
 * Note that when the container is changing its height, it calls
 * <code>parent.reposition</code> to open space for its growth.
 */

public class AccordionContainer extends ClippedContainer
  implements PathAnimation.SetPosition, AnimationFinished
{
   ...
   * If defined as a negative value, the minimum height will be computed as
   * -minH * font_height
   */
  ...
  public class Caption extends Container
  {
    ...
    @Override public void initUI()
    {
      if (btExpanded == null) {
        ...
        btExpanded.transparentBackground = true;
      }
      if (btCollapsed == null) {
        btCollapsed = new ArrowButton(Graphics.ARROW_RIGHT, fmH / 2, foreColor);
        btCollapsed.setBorder(Button.BORDER_NONE);
        btCollapsed.transparentBackground = true;
      }
      add(btExpanded, LEFT, TOP, height, FILL);
      add(btCollapsed, SAME, SAME, SAME, SAME);
      add(lCaption,
          uiAdjustmentsBasedOnFontHeightIsSupported ? AFTER + 50
                                                    : AFTER + fmH / 2,
          CENTER);
