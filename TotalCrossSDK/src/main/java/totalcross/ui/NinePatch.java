 ...
 *
 * Corner are drawn unscaled, sides are resized in a single direction, and the
 * middle is resized, colorized and then dithered.
 *
...
public class NinePatch
{
  ...
  /**
   * Defines if we will use the Image.applyColor (1) or Image.applyColor2 (2)
   * algorithms when getting the pressed instance.
   */
  ...
  private NinePatch()
  {
    switch (Settings.uiStyle) {
      case Settings.Holo:
        parts = new Parts[] {
          load(Resources.button, 7, 4),     load(Resources.edit, 16, 2),
          load(Resources.combobox, 9, 5),   load(Resources.listbox, 10, 5),
          load(Resources.multiedit, 9, 4),  load(Resources.progressbarv, 9, 4),
          load(Resources.scrollposh, 3, 2), load(Resources.scrollposv, 3, 2),
          load(Resources.tab, 10, 4),       load(Resources.grid, 5, 3),
          load(Resources.tab2, 18, 3),      load(Resources.multibutton, 5, 2),
        };
        break;
      case Settings.Material:
        parts = new Parts[] {
          load(Resources.button, 7, 3),
          load(Resources.combobox, 9, 5),
          load(Resources.combobox, 9, 5),
          load(Resources.listbox, 10, 5),
          null,
          ...
          load(Resources.multibutton, 5, 2),
        };
        break;
      default:
        parts = new Parts[] {
          load(Resources.button, 7, 1),     load(Resources.edit, 5, 3),
          load(Resources.combobox, 9, 5),   load(Resources.listbox, 5, 3),
          load(Resources.multiedit, 9, 4),  load(Resources.progressbarv, 9, 4),
          load(Resources.scrollposh, 3, 2), load(Resources.scrollposv, 3, 2),
          load(Resources.tab, 10, 4),       load(Resources.grid, 5, 3),
  ...
  public class Parts
  {
    Image imgLT, imgT, imgRT, imgL, imgC, imgR, imgLB, imgB,
      imgRB; // left top right bottom
    int scalableAreaStartWidth, scalableAreaEndWidth, scalableAreaStartHeight,
      scalableAreaEndHeight;
  }
  ...
  private void copyPixels(int[] buf,
                          Image dst,
                          Image src,
                          int dstX,
                          int dstY,
  ...
  private Image getImageArea(int[] buf, Image orig, int x, int y, int w, int h)
  ...
  public Parts load(Image original)
  {
    ...
    Parts p = null;
    try {
      p = load(getImageArea(buf, original, 1, 1, w - 2, h - 2),
               scalableAreas[0],
               scalableAreas[1],
               scalableAreas[2],
               scalableAreas[3]);
  ...
  private int[] getScalableArea(Image image)
  {
    ...
    for (int i = 0, j = 0; i < w; i++, j = i * 4) {
      c2[i] = (colors[j + 3] << 24) | (colors[j] << 16) | (colors[j + 1] << 8) |
              colors[j + 2];
    }
    ...
    if (startW < endW && startH < endH) {
      ...
      System.out.println(
        "Failed to find guide points for this ninepatch. Printing the stack trace");
      Vm.printStackTrace();
      scalableArea[0] = 1;
      scalableArea[1] = w - 2;
      scalableArea[2] = 1;
                    ...
                    int scalableAreaEndHeight)
  {
    try {
      ...
      p.imgLT = getImageArea(
        buf, original, 0, 0, scalableAreaStartWidth, scalableAreaStartHeight);
      p.imgRT = getImageArea(buf,
                             original,
                             scalableAreaEndWidth,
                             0,
                             w - scalableAreaEndWidth,
                             scalableAreaStartHeight);
      p.imgLB = getImageArea(buf,
                             original,
                             0,
                             scalableAreaEndHeight,
                             scalableAreaStartWidth,
                             h - scalableAreaEndHeight);
      p.imgRB = getImageArea(buf,
                             original,
                             scalableAreaEndWidth,
                             scalableAreaEndHeight,
                             w - scalableAreaEndWidth,
                             h - scalableAreaEndHeight);
      p.imgT = getImageArea(buf,
                            original,
                            scalableAreaStartWidth,
                            0,
                            scalableAreaEndWidth - scalableAreaStartWidth,
                            scalableAreaStartHeight);
      p.imgB = getImageArea(buf,
                            original,
                            scalableAreaStartWidth,
                            scalableAreaEndHeight,
                            scalableAreaEndWidth - scalableAreaStartWidth,
                            h - scalableAreaEndHeight);
      p.imgL = getImageArea(buf,
                            original,
                            0,
                            scalableAreaStartHeight,
                            scalableAreaStartWidth,
                            scalableAreaEndHeight - scalableAreaStartHeight);
      p.imgR = getImageArea(buf,
                            original,
                            scalableAreaEndWidth,
                            scalableAreaStartHeight,
                            w - scalableAreaEndWidth,
                            scalableAreaEndHeight - scalableAreaStartHeight);
      p.imgC = getImageArea(buf,
                            original,
                            scalableAreaStartWidth,
                            scalableAreaStartHeight,
                            scalableAreaEndWidth - scalableAreaStartWidth,
                    ...
                    int scalableAreaHeight)
  {
    try {
      ...
      p.imgLT = getImageArea(
        buf, original, 0, 0, scalableAreaWidth, scalableAreaHeight);
      p.imgRT = getImageArea(buf,
                             original,
                             w - scalableAreaWidth,
                             0,
  ...
  public Image getNormalInstance(Parts p,
                                 int width,
                                 int height,
                                 int color,
                                 boolean rotate) throws ImageException
  {
    ...
    if (s > 0) {
      ...
      copyPixels(
        buf, ret, c, scalableAreaStartWidth, 0, 0, 0, s, c.getHeight());
    ...
    if (s > 0) {
      ...
      copyPixels(
        buf, ret, c, 0, scalableAreaStartHeight, 0, 0, c.getWidth(), s);
    ...
    try {
      copyPixels(
        buf, ret, p.imgLT, 0, 0, 0, 0, p.imgLT.getWidth(), p.imgLT.getHeight());
      copyPixels(buf,
                 ret,
                 p.imgRT,
                 width - p.imgRT.getWidth(),
                 0,
                 0,
                 0,
                 p.imgRT.getWidth(),
      ...
      copyPixels(buf,
                 ret,
                 p.imgRB,
                 width - p.imgRB.getWidth(),
                 height - p.imgRB.getHeight(),
                 0,
                 0,
                 p.imgRB.getWidth(),
                 p.imgRB.getHeight());
    ...
    if (cW > 0 && cH > 0) {
      c = p.imgC.getScaledInstance(cW,
                                   cH); // smoothscale generates a worst result
                                        // because it enhances the edges
      copyPixels(buf,
                 ret,
                 c,
                 scalableAreaStartWidth,
  ...
  public Image getNormalInstance(int type,
                                 int width,
                                 int height,
                                 int color,
                                 boolean rotate) throws ImageException
  {
    ...
    synchronized (imageLock)
    {
      int hash = 0;
      sbBtn.setLength(0);
      hash = Convert.hashCode(sbBtn.append(type)
                                .append('|')
                                .append(width)
                                .append('|')
                                .append(height)
  ...
  public Image getPressedInstance(Image img, int backColor, int pressColor)
    throws ImageException
  {
    Image pressed = null;
    sbBtn.setLength(0);
    int hash = Convert.hashCode(
      sbBtn.append(img).append('|').append(backColor).append('|').append(
        pressColor));
    ...
    if (pressed == null) {
      synchronized (imageLock)
      {
        if (pressColor != -1) {
          ...
          pressed = img.getTouchedUpInstance(
            Color.getBrightness(backColor) > (256 - 32) ? (byte)-64 : (byte)32,
            (byte)0);
        }
