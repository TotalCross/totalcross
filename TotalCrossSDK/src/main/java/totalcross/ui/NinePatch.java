...
public class NinePatch {
  ...
  public Image getNormalInstance(Parts p, int width, int height, int color, boolean rotate) throws ImageException {
    ...
    s = width - corner * 2;
    if (s > 0) {
      c = p.imgT.getScaledInstance(s, corner);
      copyPixels(buf, ret, c, corner, 0, 0, 0, s, corner);
      c = p.imgB.getScaledInstance(s, corner);
      copyPixels(buf, ret, c, corner, height - corner, 0, 0, s, corner);
    }
    s = height - corner * 2;
    if (s > 0) {
      c = p.imgL.getScaledInstance(side, s);
      copyPixels(buf, ret, c, 0, corner, 0, 0, side, s);
      c = p.imgR.getScaledInstance(side, s);
      copyPixels(buf, ret, c, width - side, corner, 0, 0, side, s);
    }
    // corners
    try {
