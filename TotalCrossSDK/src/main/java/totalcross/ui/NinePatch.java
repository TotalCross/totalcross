...
public class NinePatch {
	...
	private void copyPixels(int[] buf, Image dst, Image src, int dstX, int dstY, int srcX, int srcY,
							int srcW,
							int srcH) {
	...
	private Image getImageArea(int[] buf, Image orig, int x, int y, int w,
							   int h) throws ImageException {
	...
	public Parts load(Image original) {
		...
		Parts p = null;
		try {
			p = load(getImageArea(buf, original, 1, 1, w - 2, h - 2), scalableAreas[0], scalableAreas[1],
					 scalableAreas[2], scalableAreas[3]);
	...
	public Parts load(Image original, int scalableAreaStartWidth, int scalableAreaEndWidth,
					  int scalableAreaStartHeight, int scalableAreaEndHeight) {
		try {
			int w = original.getWidth();
			int h = original.getHeight();
			int[] buf = new int[w > h ? w : h];
			...
			p.imgRT = getImageArea(buf, original, scalableAreaEndWidth, 0, w - scalableAreaEndWidth,
								   scalableAreaStartHeight);
			p.imgLB = getImageArea(buf, original, 0, scalableAreaEndHeight, scalableAreaStartWidth,
								   h - scalableAreaEndHeight);
			p.imgRB = getImageArea(buf, original, scalableAreaEndWidth, scalableAreaEndHeight,
								   w - scalableAreaEndWidth, h - scalableAreaEndHeight);
			p.imgT = getImageArea(buf, original, scalableAreaStartWidth, 0,
								  scalableAreaEndWidth - scalableAreaStartWidth, scalableAreaStartHeight);
			p.imgB = getImageArea(buf, original, scalableAreaStartWidth, scalableAreaEndHeight,
								  scalableAreaEndWidth - scalableAreaStartWidth, h - scalableAreaEndHeight);
			p.imgL = getImageArea(buf, original, 0, scalableAreaStartHeight, scalableAreaStartWidth,
								  scalableAreaEndHeight - scalableAreaStartHeight);
			p.imgR = getImageArea(buf, original, scalableAreaEndWidth, scalableAreaStartHeight,
								  w - scalableAreaEndWidth, scalableAreaEndHeight - scalableAreaStartHeight);
			p.imgC = getImageArea(buf, original, scalableAreaStartWidth, scalableAreaStartHeight,
								  scalableAreaEndWidth - scalableAreaStartWidth, scalableAreaEndHeight - scalableAreaStartHeight);
			return p;
		} catch (Exception e) {
	...
	public Parts load(Image original, int scalableAreaWidth, int scalableAreaHeight) {
		try {
			...
			p.imgRT = getImageArea(buf, original, w - scalableAreaWidth, 0, scalableAreaWidth,
								   scalableAreaHeight);
			p.imgLB = getImageArea(buf, original, 0, h - scalableAreaHeight, scalableAreaWidth,
								   scalableAreaHeight);
			p.imgRB = getImageArea(buf, original, w - scalableAreaWidth, h - scalableAreaHeight,
								   scalableAreaWidth, scalableAreaHeight);
			p.imgT = getImageArea(buf, original, scalableAreaWidth, 0, w - scalableAreaWidth * 2,
								  scalableAreaHeight);
			p.imgB = getImageArea(buf, original, scalableAreaWidth, h - scalableAreaHeight,
								  w - scalableAreaWidth * 2, scalableAreaHeight);
			p.imgL = getImageArea(buf, original, 0, scalableAreaHeight, scalableAreaWidth,
								  h - scalableAreaHeight * 2);
			p.imgR = getImageArea(buf, original, w - scalableAreaWidth, scalableAreaHeight, scalableAreaWidth,
								  h - scalableAreaHeight * 2);
			p.imgC = getImageArea(buf, original, scalableAreaWidth, scalableAreaHeight,
								  w - scalableAreaWidth * 2, h - scalableAreaHeight * 2);
			return p;
		} catch (Exception e) {
			throw new RuntimeException(e + " " + e.getMessage());
		}
	...
	public Image getNormalInstance(Parts p, int width, int height, int color,
								   boolean rotate) throws ImageException {
		...
		try {
			...
			copyPixels(buf, ret, p.imgRT, width - p.imgRT.getWidth(), 0, 0, 0, p.imgRT.getWidth(),
					   p.imgRT.getHeight());
			copyPixels(buf, ret, p.imgLB, 0, height - p.imgLB.getHeight(), 0, 0, p.imgLB.getWidth(),
					   p.imgLB.getHeight());
			copyPixels(buf, ret, p.imgRB, width - p.imgRB.getWidth(), height - p.imgRB.getHeight(), 0, 0,
					   p.imgRB.getWidth(), p.imgRB.getHeight());
		...
		if (cW > 0 &&  cH > 0) {
			c = p.imgC.getScaledInstance(cW,
										 cH); // smoothscale generates a worst result because it enhances the edges
	...
	public Image getNormalInstance(int type, int width, int height, int color,
								   boolean rotate) throws ImageException {
		Image ret = null;
		synchronized (imageLock) {
			int hash = 0;
			sbBtn.setLength(0);
			hash = Convert.hashCode(sbBtn.append(type).append('|').append(width).append('|').append(
										height).append('|')
	...
	public Image getPressedInstance(Image img, int backColor, int pressColor) throws ImageException {
		Image pressed = null;
		sbBtn.setLength(0);
		int hash = Convert.hashCode(sbBtn.append(img).append('|').append(backColor).append('|').append(
										pressColor));
		...
		if (pressed == null) {
			synchronized (imageLock) {
				if (pressColor != -1) {
					...
					pressed = img.getTouchedUpInstance(Color.getBrightness(backColor) > (256 - 32) ? (byte) - 64 :
													   (byte) 32,
													   (byte) 0);
				}
