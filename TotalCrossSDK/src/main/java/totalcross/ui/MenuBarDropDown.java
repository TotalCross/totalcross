 ...
 * // is closed when the user clicks outside or selects an item. Use the
 * getSelectedIndex
 * // method to discover the index of the MenuItem array that was selected (-1
 * if none).
 * </pre>
...
public class MenuBarDropDown extends Window
{
  ...
  /**
   * Constructs a MenuBarDropDown that will show at the given x,y position the
   * given items.
   */
  public MenuBarDropDown(int x, int y, MenuItem[] items)
  {
    focusTraversable = true; //	timo@tc100b3 - must be focusTraversable for
                             // geographical focus to work
    this.items = items;
    canDrag = false;
    popX = x;
    popY = y;
  ...
  @Override public void screenResized() { onFontChanged(); }
  ...
  public int getYPos(int index) { return ypos[index]; }
  ...
  @Override protected void onPopup()
  {
    ...
    highlighted = this; //	timo@tc100b3 - if highlighted != _focus,
                        // geographical focus doesn't work
  }
  ...
  @Override protected boolean onClickedOutside(PenEvent event)
  {
    ...
    selected = -1;
    if (event.y < this.y &&
        ((w = (Window)zStack.items[zStack.size() - 2]) instanceof
         totalcross.ui
           .MenuBar)) { // clicked on MenuBar? propagate the event to it
    ...
    } else if (event.x < this.x || event.y < this.y ||
               event.x >= (this.x + this.width) ||
               event.y >=
                 (this.y + this.height)) { // guich@300_39: added the last
                                           // condition to fix a problem when
                                           // this is used not with a menubar.
      if (getTopMost() instanceof MenuBar) {
        enableUpdateScreen =
          false; // avoids that the screen is updated in next event tick
      }
      unpop();
    }
    return true;
  ...
  @Override protected void loadBehind()
  { // guich@tc120_38
    try {
  ...
  @Override public void onEvent(Event event)
  {
    switch (event.type) {
      ...
      case KeyEvent.KEY_PRESS:
        ...
        int idx = ((KeyEvent)event).key -
                  '0'; // guich@tc112_27: allow select using numbers
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (borderStyle == -1) {
      g.draw3dRect(
        0, 0, width, height, Graphics.R3D_SHADED, false, true, fourColors);
    }
    ...
    for (int i = 1; i < items.length; i++) {
      ...
      if (mi.isSeparator) {
        ...
        if (mi.isChecked) {
          // draws the check
          int x =
            width - 9 - fmH / 11; // guich@580_8: space a bit on highres devices
          int y = ypos[i - 1] + 5;
          int my = 2;
          int mx = 7;
        ...
        g.drawText(
          mi.caption, 3, ypos[i - 1], textShadowColor != -1, textShadowColor);
        if (!mi.isEnabled) {
          g.foreColor = fColor;
        }
      }
  ...
  /**
   * Returns the index of the next menu item that is to be selected based on the
   * direction of the KeyEvent.
   * @param ke The key event
   * @return   The index of the next menu item based on the direction of the key
   *     event.
   */
  protected int getNextSelectionIndex(KeyEvent ke)
  {
    ...
    for (int i = items.length; i > 0;
         i--) { // to avoid infinite loop if all items are non-clickable
      if (isDown) {
        if (++newSelected == items.length) {
          newSelected =
            Settings.circularNavigation ? 1 : newSelected - 1; // guich@tc115_19
        }
      } else {
        if (--newSelected <= 0) {
          newSelected = Settings.circularNavigation ? items.length - 1
                                                    : 1; // guich@tc115_19
        }
      }
