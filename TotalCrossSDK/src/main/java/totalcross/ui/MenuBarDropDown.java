	...
	protected boolean onClickedOutside(PenEvent event) {
		...
		if (event.y < this.y
			&& ((w = (Window) zStack.items[zStack.size() - 2]) instanceof
				totalcross.ui.MenuBar)) { // clicked on MenuBar? propagate the event to it
			...
			if (topMost == this) {
				requestFocus(); // we must get back the focus
				   ...
				   || event.y >= (this.y +
								  this.height)) { // guich@300_39: added the last condition to fix a problem when this is used not with a menubar.
