...
public class Control extends GfxSurface {
	...
	protected int
	x; // guich@200: VERY IMPORTANT: DONT CHANGE THE LOCATION OF THIS VARIABLE! it must be the # 1
	/** The control's y location */
	protected int
	y; // guich@200: VERY IMPORTANT: DONT CHANGE THE LOCATION OF THIS VARIABLE! it must be the # 2
	/** The control's width */
	protected int
	width; // guich@200: VERY IMPORTANT: DONT CHANGE THE LOCATION OF THIS VARIABLE! it must be the # 3
	/** The control's height */
	protected int
	height; // guich@200: VERY IMPORTANT: DONT CHANGE THE LOCATION OF THIS VARIABLE! it must be the # 4
	...
	pressedEvent; // guich@tc100: share the same event across all controls - guich@tc114_42: no longer share
	...
	private Control
	dragTarget; // holds the Control that handled the last dragEvent sent to this control.
	...
	protected Control() {
		if (MainWindow.defaultFont ==
			null) { // guich@tc130: moved from static block at MainWindow to here, so user can define if he wants to use the old or new font
	...
	void paint2shot(Graphics g, boolean shift) {
		if (!transparentBackground && parent != null
			&& !(parent.parent != null && parent.parent instanceof ScrollContainer
				 && parent.parent.transparentBackground)) { // last clause prevents a white background on SAV's menu
	...
	public void setRect(int x, int y, int width, int height, Control relative, boolean screenChanged) {
		...
		if (x + y + width + height >= MAXABSOLUTECOORD) { // are there any relative coords?
			...
			if (parent != null) {
				...
				if (relative != null) {
					...
					parent.lastH = relative.height;
						   ...
						   -999999) { // first control being added? - guich@450_36: only one check is enough
					parent.lastX = cli.x;
					parent.lastY = cli.y;
				}
			} else {
			...
			if (Settings.onJavaSE
				&& Settings.showUIErrors) { // guich@450_36: do these checks only if running on desktop
				if (cli.width == 0 || cli.height == 0) {
					...
					if (!zeroIsValid) { // when a Control is inside a AccordionContainer, it can reach size 0, so we just ignore the exception
						throw new RuntimeException(parent + " must have its bounds set before calling " + this +
					...
					if (!error.isEmpty()) {
						throw new RuntimeException("You are using constant positions " + error.substring(0,
												   error.length() - 1)
												   + " in a wrong place for control " + toString());
					} else {
						throw new RuntimeException(
							"To use AFTER/BEFORE/SAME you must add first the control " + toString() +
							" to the parent container.");
					}
				} else if (x + y < -RANGE) { // guich@300_27
					if (x < -RANGE) {
						throw new RuntimeException("You can't use FILL with BEFORE, CENTER or RIGHT for control " +
												   toString());
					} else {
						throw new RuntimeException("You can't use FILL with BEFORE, CENTER or BOTTOM for control " +
												   toString());
					}
				}
				if (height < 0 && width < 0) {
					...
					throw new RuntimeException("Invalid resulting values in width for control " + toString() + ": " +
											   width);
				} else if (height < 0) {
					throw new RuntimeException("Invalid resulting values in height for control " + toString() + ": " +
											   height);
				}
			}
		}
		...
		if (asContainer != null) {
			...
			asContainer.lastScreenWidth =
				Settings.screenWidth; // save the last screen resolution so we can be repositioned if a rotation occured at a time that the container was not on screen
		}
	}
	...
	private boolean isOnlyForX(int k) {
		return ((LEFT - RANGE) <= k && k <= (LEFT + RANGE)) || ((RIGHT - RANGE) <= k
				&& k <= (RIGHT + RANGE))
			   || ((RIGHT_OF - RANGE) <= k && k <= (RIGHT_OF + RANGE));
	}

	private boolean isOnlyForY(int k) {
		return ((TOP - RANGE) <= k && k <= (TOP + RANGE)) || ((BOTTOM - RANGE) <= k
				&& k <= (BOTTOM + RANGE))
			   || ((BOTTOM_OF - RANGE) <= k && k <= (BOTTOM_OF + RANGE));
	}
	...
	public void repaintNow() {
		...
		if (w != null
			&& Window.zStack.indexOf(w, 0) >= 0) { // guich@560_12: if we're not visible, this is nonsense
			if (Settings.isOpenGL
				|| !w.isTopMost()) { // guich@issue#80: if control is not topmost, must repaint all windows
	...
	Graphics refreshGraphics(Graphics g, int expand, Control topParent, int tx0, int ty0) {
		...
		g.refresh(sx + tx0 - expand, sy - expand + ty0, sw + expand + expand, sh + expand + expand,
				  tx + tx0, ty + ty0,
				  font);
		return g;
	}
	...
	public <H extends EventHandler> void postEvent(Event<H> event) {
		...
		if (Settings.keyboardFocusTraversable && event.type == ControlEvent.PRESSED
			&& !(event.target instanceof Edit)) {
		...
		if (dragTarget !=
			null) { // improve drag performance by sending the event directly to the drag handler control
		...
		if (!event.consumed) {
			...
			if (!eventTargetCalled
				...
				Control) { // guich@tc110_52: call any listeners of the target control - guich@tc112_3: if not yet called
	...
	private Control getNextHighlighted(Container p, boolean forward) {
		...
		if (p == this) {
			idx = forward ? -1 : last +
				  1; // return (Control)v.items[forward?0:last]; // return the first/last control - guich@573_22: can't just go to first or last - they may be disabled!
		} else if (idx == -1) {
			return null;
		}
			...
			if (c.visible && c.isEnabled() && !c.focusLess) {
				if (c.focusTraversable) {
					return c;
						   ...
						   null) { // kmeehl@tc100: look through child containers for the next focusable control
	...
	public void changeHighlighted(Container p, boolean forward) {
		Control c = getNextHighlighted(p,
		...
		if (c != null) {
			...
			if (this == p) {
				...
				parent.changeHighlighted(parent.parent != null ? parent.parent : parent,
										 forward); // return to the parent of the parent and continues - p.p!=null: if our parent is a window (E.G. UIGadgets' scrollbar), his parent is null; so we restart from the window
			}
				...
				if (e.target == l.target || (callListenersOnAllTargets
					...
					switch (e.type) {
						...
						case EnabledStateChangeEvent.ENABLED_STATE_CHANGE:
						 ...
						 ee) { // prevent totalcross.ui.event.PenEvent is not compatible with totalcross.ui.event.DragEvent
				if (Settings.onJavaSE) {
					throw ee;
				}
			}
	...
	public boolean isVisibleAndInside(int x0, int y0, int xf, int yf) { // guich@tc115_40
		return this.visible && this.y < yf && (this.y + this.height) > y0 && this.x < xf
			   && (this.x + this.width) >
			   x0; // guich@200: ignore hidden controls - note: a window added to a container may not be painted correctly
	}

	boolean isVisibleAndInside(int y0, int yf) { // guich@tc115_40
		return this.visible && this.y < yf
			   && (this.y + this.height) >
			   y0; // guich@200: ignore hidden controls - note: a window added to a container may not be painted correctly
	}

	public int getGap(int gap) {
		return Settings.uiAdjustmentsBasedOnFontHeight
			   && uiAdjustmentsBasedOnFontHeightIsSupported ? gap * fmH / 100 : gap;
	}
