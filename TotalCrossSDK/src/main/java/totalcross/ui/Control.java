...
public class Control extends GfxSurface
{
  ...
  public static enum TranslucentShape { NONE, RECT, ROUND, LESS_ROUND, CIRCLE }
  ;

  /** The type of surface. */
  int surfaceType; // don't move from here! must be at position 0
  /** The control's x location */
  protected int x; // guich@200: VERY IMPORTANT: DONT CHANGE THE LOCATION OF
                   // THIS VARIABLE! it must be the # 1
  /** The control's y location */
  protected int y; // guich@200: VERY IMPORTANT: DONT CHANGE THE LOCATION OF
                   // THIS VARIABLE! it must be the # 2
  /** The control's width */
  protected int width; // guich@200: VERY IMPORTANT: DONT CHANGE THE LOCATION OF
                       // THIS VARIABLE! it must be the # 3
  /** The control's height */
  protected int height; // guich@200: VERY IMPORTANT: DONT CHANGE THE LOCATION
                        // OF THIS VARIABLE! it must be the # 4
  ...
  /**
   * Foreground color of this control. When the control is added, its
   * foreground is set to be the same of the parent's.
   */
  protected int foreColor = -1;
  /**
   * Background color of this control. When the control is added, its
   * background is set to be the same of the parent's.
   */
  protected int backColor = -1;
  /**
   * Application defined constant. This constant is not used; its only a
   * placeholder for the application so it can set to any value it wants
   */
  public int appId;
  /**
   * Application defined object. This field is not used; its only a placeholder
   * for the application so it can set to any value it wants
   */
  public Object appObj; // guich@580_38
  /**
   * Default value when calling clear. When the control will use a numeric
   * value or a String, depends on the type of control. Defaults to an empty
   * string.
   */
  public String clearValueStr = ""; // guich@572_19
  /**
   * Default value when calling clear. When the control will use a numeric
   * value or a String, depends on the type of control. Defaults to zero.
   */
  ...
  /**
   * Constant used in params width and height in setRect. You can use this
   * constant added to a number to specify a increment/decrement to the
   * calculated size. EG: PREFERRED+2 or PREFERRED-1.
   */
  public static final int PREFERRED = 1 * UICONST;
  /**
   * Constant used in param x in setRect. You can use this constant added to a
   * number to specify a increment/decrement to the calculated size. EG: LEFT+2
   * or LEFT-1.
   */
  public static final int LEFT = 2 * UICONST;
  /**
   * Constant used in params x and y in setRect.  You can use this constant
   * added to a number to specify a increment/decrement to the calculated size.
   * EG: CENTER+2 or CENTER-1.
   */
  public static final int CENTER = 3 * UICONST;
  /**
   * Constant used in param x in setRect. You can use this constant added to a
   * number to specify a increment/decrement to the calculated size. EG: RIGHT+2
   * or RIGHT-1.
   */
  public static final int RIGHT = 4 * UICONST;
  /**
   * Constant used in param y in setRect. You can use this constant added to a
   * number to specify a increment/decrement to the calculated size. EG: TOP+2
   * or TOP-1.
   */
  public static final int TOP = 5 * UICONST;
  /**
   * Constant used in param y in setRect. You can use this constant added to a
   * number to specify a increment/decrement to the calculated size. EG:
   * BOTTOM+2 or BOTTOM-1.
   */
  public static final int BOTTOM = 6 * UICONST;
  /**
   * Constant used in params width and height in setRect. You can use this
   * constant added to a number to specify a increment/decrement to the
   * calculated size. EG: FILL+2 or FILL-1. Note that FILL cannot be used with
   * other x/y positioning constants.
   */
  public static final int FILL = 7 * UICONST;
  /**
   * Constant used in param x/y in setRect. You can use this constant added to
   * a number to specify a increment/decrement to the calculated size. EG:
   * BEFORE+2 or BEFORE-1.
   */
  public static final int BEFORE = 8 * UICONST; // guich@200b4_100
  /**
   * Constant used in params x/y/width/height in setRect. You can use this
   * constant added to a number to specify a increment/decrement to the
   * calculated size. EG: SAME+2 or SAME-1.
   */
   ...
   * Constant used in params width and height in setRect. You can use this
   * constant added to a number to specify a increment/decrement to the
   * calculated size. EG: FIT+2 or FIT-1. Note that FIT cannot be used with
   * other x/y positioning constants. FIT will make the control fit between the
   * desired position and the last added control position.
   */
  public static final int FIT = 11 * UICONST; // guich@330_4
  /**
   * Constant used in param x/y in setRect. You can use this constant added to
   * a number to specify a increment/decrement to the calculated size. EG:
   * CENTER_OF+2 or CENTER_OF-1. CENTER_OF is related to a control, while CENTER
   * is related to the screen. CENTER_OF cannot be used with FILL/FIT in the
   * widths.
   */
   ...
   * Constant used in param x/y in setRect. You can use this constant added to
   * a number to specify a increment/decrement to the calculated size. EG:
   * RIGHT_OF+2 or RIGHT_OF-1. RIGHT_OF is related to a control, while RIGHT is
   * related to the screen. RIGHT_OF cannot be used with FILL/FIT in the widths.
   */
  public static final int RIGHT_OF = 13 * UICONST; // guich@tc110_97
  /**
   * Constant used in param x/y in setRect. You can use this constant added to
   * a number to specify a increment/decrement to the calculated size. EG:
   * BOTTOM_OF+2 or BOTTOM_OF-1. BOTTOM_OF is related to a control, while BOTTOM
   * is related to the screen. BOTTOM_OF cannot be used with FILL/FIT in the
   * widths.
   */
  public static final int BOTTOM_OF = 14 * UICONST; // guich@tc110_97
  /**
   * Constant used in param width (will use screen's width) and height (will
   * use screen's height) in setRect. You can use this constant added or
   * subtracted to a number to specify a increment to the calculated size. There
   * are two ways to use it:<br>
   * 1. SCREENSIZE + constant: it will use as a PERCENTAGE of the screen's size.
   * For example, SCREENSIZE+20 in width will result in 20% of screen's
   * size.<br>
   * 2. SCREENSIZE - constant: it will use as a FRACTION of the screen's size.
   * For example, SCREENSIZE-4 in width will result in 1/4 of screen's size.<br>
   *
   * If there are no constant number, size will be 100% of the screen's
   * width/height.
   * @since TotalCross 1.3
   */
  public static final int SCREENSIZE = 15 * UICONST;
  /**
   * Constant used in params x/y/width/height in setRect. It keeps the current
   * value. Note that it does NOT support increment nor decrement. KEEP differs
   * from SAME in the manner that KEEP applies to the coordinates of this
   * control, while SAME applies to the coordinates of another control.
   * @since TotalCross 1.14
   */
  public static final int KEEP = 16 * UICONST; // guich@tc114_68
  /**
   * Constant used in param width (will use parent's width) and height (will
   * use parent's height) in setRect. It can also be used in X or Y,
   * representing the position where to draw it. You can use this constant added
   * or subtracted to a number to specify a increment to the calculated size.
   * There are two ways to use it:<br>
   * 1. PARENTSIZE + constant: it will use as a PERCENTAGE of the parent's size.
   * For example, PARENTSIZE+20 in width will result in 20% of parent's
   * size.<br>
   * 2. PARENTSIZE - constant: it will use as a FRACTION of the parent's size.
   * For example, PARENTSIZE-4 in width will result in 1/4 of parent's size.<br>
   *
   * If there are no constant number, size will be 100% of the parent's
   * width/height.
   *
  ...
  /**
   * Constant used in param width or height (will use screen's minimum size
   * between width and height) in setRect. You can use this constant added or
   * subtracted to a number to specify a increment to the calculated size. There
   * are two ways to use it:<br>
   * 1. SCREENSIZEMIN + constant: it will use as a PERCENTAGE of the screen's
   * size. For example, SCREENSIZEMIN+20 in width will result in 20% of screen's
   * minimum size.<br>
   * 2. SCREENSIZEMIN - constant: it will use as a FRACTION of the screen's
   * size. For example, SCREENSIZEMIN-4 in width will result in 1/4 of screen's
   * minimum size.<br>
   *
   * If there are no constant number, size will be 100% of the screen's
   * width/height.
  ...
  /**
   * Constant used in param width or height (will use screen's maximum size
   * between width and height) in setRect. You can use this constant added or
   * subtracted to a number to specify a increment to the calculated size. There
   * are two ways to use it:<br>
   * 1. SCREENSIZEMAX + constant: it will use as a PERCENTAGE of the screen's
   * size. For example, SCREENSIZEMAX+20 in width will result in 20% of screen's
   * maximum size.<br>
   * 2. SCREENSIZEMAX - constant: it will use as a FRACTION of the screen's
   * size. For example, SCREENSIZEMAX-4 in width will result in 1/4 of screen's
   * maximum size.<br>
   *
   * If there are no constant number, size will be 100% of the screen's
   * width/height.
  ...
  /**
   * Constant used in param width or height (will use parent's minimum size
   * between width and height) in setRect. You can use this constant added or
   * subtracted to a number to specify a increment to the calculated size. There
   * are two ways to use it:<br>
   * 1. PARENTSIZEMIN + constant: it will use as a PERCENTAGE of the parent's
   * size. For example, PARENTSIZEMIN+20 in width will result in 20% of parent's
   * size.<br>
   * 2. PARENTSIZEMIN - constant: it will use as a FRACTION of the parent's
   * size. For example, PARENTSIZEMIN-4 in width will result in 1/4 of parent's
   * size.<br>
   *
   * If there are no constant number, size will be 100% of the parent's
   * width/height.
  ...
  /**
   * Constant used in param width or height (will use parent's maximum size
   * between width and height) in setRect. You can use this constant added or
   * subtracted to a number to specify a increment to the calculated size. There
   * are two ways to use it:<br>
   * 1. PARENTSIZEMAX + constant: it will use as a PERCENTAGE of the parent's
   * size. For example, PARENTSIZEMAX+20 in width will result in 20% of parent's
   * size.<br>
   * 2. PARENTSIZEMAX - constant: it will use as a FRACTION of the parent's
   * size. For example, PARENTSIZEMAX-4 in width will result in 1/4 of parent's
   * size.<br>
   *
   * If there are no constant number, size will be 100% of the parent's
   * width/height.
  ...
  /**
   * Constant used in param width (will use parent's width) and height (will
   * use the current font size/height) in setRect. You can use this constant
   * added or subtracted to a number to specify a increment to the calculated
   * size. There are two ways to use it:<br>
   * 1. FONTSIZE + constant: it will use as a PERCENTAGE of the parent's size.
   * For example, FONTSIZE+20 in width will result in 20% of font's size.<br>
   * 2. FONTSIZE - constant: it will use as a FRACTION of the parent's size. For
   * example, FONTSIZE-4 in width will result in 1/4 of font's size.<br>
   *
   * If there are no constant number, size will be 100% of the font's
   * width/height. So, FONTSIZE and FONTSIZE+100 is the same.
   * @since TotalCross 1.52
   */
  ...
  /**
   * Constant used in params width/height in setRect. It informs that the
   * parent's last width/height should not be updated now, because it will be
   * resized later. Note that it does NOT support increment nor decrement.
   * Sample:
   * <pre>
   * Container c;
  ...
  private ControlEvent
    pressedEvent; // guich@tc100: share the same event across all controls -
                  // guich@tc114_42: no longer share

  /**
   * Allows to disable the ui adjustments based on font height for a single
   * control. Set this flag in the constructor. It is propagated to all child
   * controls.
   * @since TotalCross 1.3
   */
  ...
  /**
   * Stores the control that should handle the focus change key for this
   * control
   */
   ...
   * one is selected. This will cause the KeyEvent to be intercepted and handled
   * by the method changeHighlighted. When the user press the ACTION (or ENTER)
   * key to use the control, this flag is set to false and the focus will be set
   * to the control, so it be able to use the arrows to navigate inside it. The
   * control must then set this to true when the finish using it or press the
   * ACTION button again (which then sets the flag to true).
   * @since SuperWaba 5.5
   */
  ...
  /**
   * Shortcuts to test the UI style. Use the setUIStyle method to change them
   * accordingly.
   */
  ...
  /**
   * If true, this control will receive pen and key events but will never gain
   * focus. This is useful to create keypads. See totalcross.ui.Calculator.
   */
  ...
  /**
   * Set the background to be transparent, by not filling the control's area
   * with the background color.
   * @since TotalCross 1.0
   */
  public boolean transparentBackground;
  /**
   * True means this control expects to get focus on a PEN_DOWN event. If
   * focusOnPenDown is false, focus will be set on PEN_UP instead. This can be
   * used for things like drag-scrollable list controls that contain controls as
   * list items, to avoid setting focus to an item (and therefore changing the
   * selection) during a drag-scroll.
   */
  public boolean focusOnPenDown = true;

  /**
   * True means that EventListeners for PenEvent and KeyEvent will be called
   * without verifying that the event target is this.
   */
  public boolean callListenersOnAllTargets;

  private Control dragTarget; // holds the Control that handled the last
                              // dragEvent sent to this control.

  /**
   * The offscreen image taken with takeScreenShot. The onPaint will use this
   * shot until the user calls releaseScreenShot.
   */
  ...
  {
    npParts =
      img == null ? null : NinePatch.getInstance().load(img, corner, side);
  }
  /**
   * Set the NinePatch of the control. This is used to draw the background
   * image.
  ...
  protected Control()
  {
    if (MainWindow.defaultFont ==
        null) { // guich@tc130: moved from static block at MainWindow to here,
                // so user can define if he wants to use the old or new font
      MainWindow.defaultFont =
        Font.getFont(Font.DEFAULT, false, Font.NORMAL_SIZE);
      ...
      isTablet = Math.max(Settings.screenWidth, Settings.screenHeight) /
                   Font.NORMAL_SIZE >
                 30;
    }
   ...
   * <code>offscreen0</code>.
   */
  public void takeInitialScreenShot() { takeScreenShot(0); }

  /**
   * Take a screen shot of this container and stores it in
   * <code>offscreen</code>.
   */
  public void takeScreenShot() { takeScreenShot(1); }

  void paint2shot(Graphics g, boolean shift)
  {
    if (!transparentBackground && parent != null &&
        !(parent.parent != null && parent.parent instanceof ScrollContainer &&
          parent.parent
            .transparentBackground)) { // last clause prevents a white
                                       // background on SAV's menu
  ...
  void paint2shot(Graphics g, Control top, boolean shift)
  {
    ...
    if (asContainer != null) {
      for (Control child = asContainer.children; child != null;
           child = child.next) {
        if (child.visible) {
   ...
   * The shadow is made drawing the button in (x-1,y-1), (x+1,y-1), (x-1,y+1),
   * (x+1,y+1) positions. Defaults to -1, which means no shadow. You can set
   * pass BRIGHTER_BACKGROUND or DARKER_BACKGROUND as parameter, AFTER calling
   * setBackColor or setForeColor, to compute the color based on the background.
   * <br><br> Example:
   * <pre>
   * c = new Label(....);
   * c.setBackColor(Color.BLUE);
   * c.setTextShadowColor(DARKER_BACKGROUND);
   * // you may also set it directly to a color:
  ...
  public void setTextShadowColor(int color)
  {
    this.textShadowColor =
      color == BRIGHTER_BACKGROUND
        ? Color.brighter(backColor)
   ...
   * @since TotalCross 1.27
   */
  public int getTextShadowColor() { return textShadowColor; }
  ...
  { // guich@tc120_48
    ...
    int d =
      (int)(Convert.getDistancePoint2Rect(x, y, 0, 0, width, height) + 0.5);
    return d <= Settings.touchTolerance;
  }

  /** Shows a message using a global tip shared by all controls. */
  static void showTip(Control c, String s, int duration, int y)
  { // guich@tc100b4_27
   ...
   * @since TotalCross 1.14
   */
  public void postPressedEvent() { postEvent(getPressedEvent(this)); }
  ...
  public int getPreferredHeight() { return fmH; }
   ...
   * Note that in this case, only the SAME,BEFORE,AFTER are affected by the
   * given control. Here is an example of relative positioning: <p> Important
   * note: you can't use FILL/FIT with BEFORE/RIGHT/BOTTOM (for x,y). <pre>
   ...
   *  43
   * </pre>
   * Note: add(control, x,y) does: <code>add(control);
   * control.setRect(x,y,PREFERRED,PREFERRED);</code> <p> <b>Important! Always
   * add the control to the container before doing a setRect.</b> <p>The
   * relative positioning does not work well if the control is placed outside
   * screen bounds.
   * @param x One of the relative positioning constants: LEFT, RIGHT, SAME,
   *     BEFORE, AFTER, CENTER, with a small adjustment. You can also use an
   *     absolute value, but this is strongly discouraged.
   * @param y One of the relative positioning constants: TOP, BOTTOM, SAME,
   *     BEFORE, AFTER, CENTER, with a small adjustment. You can also use an
   *     absolute value, but this is strongly discouraged.
   * @param width  One of the relative positioning constants: PREFERRED, FILL,
   *     FIT, SAME. You can also use an absolute value, but this is strongly
   *     discouraged.
   * @param height One of the relative positioning constants: PREFERRED, FILL,
   *     FIT, SAME. You can also use an absolute value, but this is strongly
   *     discouraged.
   * @param relative To whom the position should be relative to; or null to be
   *     relative to the last control.
   * @param screenChanged Indicates that a screen change (resize, collapse)
   *     occured and the <code>reposition</code> method is calling this method.
   *     Set by the system. If you call this method directly, always pass false
   *     to it.
   * @see #LEFT
   * @see #TOP
   * @see #RIGHT
                      ...
                      boolean screenChanged)
  {
    ...
    if (x + y + width + height >=
        MAXABSOLUTECOORD) { // are there any relative coords?
      if (x == KEEP) {
        x = this.x;
      }
      if (y == KEEP) {
      ...
      int lpx = 0, lpy = 0;
      Container parent =
        this.parent;       // guich@450_36: use local var instead of field
      ...
      if (parent != null) {
        ...
        } else if (parent.lastX ==
                   -999999) { // first control being added? - guich@450_36: only
                              // one check is enough
          parent.lastX = cli.x;
          parent.lastY = cli.y;
        }
      ...
      if (Settings.uiAdjustmentsBasedOnFontHeight &&
        ...
        } else if ((SCREENSIZEMIN - RANGE) <= width &&
                   width <= (SCREENSIZEMIN + RANGE)) {
          width -= SCREENSIZEMIN;
          if (width < 0) {
            width =
              Math.min(Settings.screenWidth, Settings.screenHeight) / -width;
          } else if (width == 0) {
            width = Math.min(Settings.screenWidth, Settings.screenHeight);
          } else {
            width = width *
                    Math.min(Settings.screenWidth, Settings.screenHeight) / 100;
          }
        } else if ((SCREENSIZEMAX - RANGE) <= width &&
                   width <= (SCREENSIZEMAX + RANGE)) {
          width -= SCREENSIZEMAX;
          if (width < 0) {
            width =
              Math.max(Settings.screenWidth, Settings.screenHeight) / -width;
          } else if (width == 0) {
            width = Math.max(Settings.screenWidth, Settings.screenHeight);
          } else {
            width = width *
                    Math.max(Settings.screenWidth, Settings.screenHeight) / 100;
          }
        } else if ((PARENTSIZE - RANGE) <= width &&
                   width <= (PARENTSIZE + RANGE)) {
          width -= PARENTSIZE;
          if (width < 0) {
        ...
        } else if ((PARENTSIZEMIN - RANGE) <= width &&
                   width <= (PARENTSIZEMIN + RANGE)) {
          width -= PARENTSIZEMIN;
          if (width < 0) {
        ...
        } else if ((PARENTSIZEMAX - RANGE) <= width &&
                   width <= (PARENTSIZEMAX + RANGE)) {
          width -= PARENTSIZEMAX;
          if (width < 0) {
        ...
        } else if ((PREFERRED - RANGE) <= height &&
                   height <= (PREFERRED + RANGE)) {
          height = getPreferredHeight() + (height - PREFERRED) * fmH / 100;
        } else if ((SAME - RANGE) <= height && height <= (SAME + RANGE) &&
                   parent != null) {
          height = parent.lastH + (height - SAME) * fmH / 100;
        } else // can't be moved from here!
          if ((SCREENSIZE - RANGE) <= height &&
              height <= (SCREENSIZE + RANGE)) {
          height -= SCREENSIZE;
          if (height < 0) {
        ...
        } else if ((SCREENSIZEMIN - RANGE) <= height &&
                   height <= (SCREENSIZEMIN + RANGE)) {
          height -= SCREENSIZEMIN;
          if (height < 0) {
            height =
              Math.min(Settings.screenWidth, Settings.screenHeight) / -height;
          } else if (height == 0) {
        ...
        } else if ((SCREENSIZEMAX - RANGE) <= height &&
                   height <= (SCREENSIZEMAX + RANGE)) {
          height -= SCREENSIZEMAX;
          if (height < 0) {
            height =
              Math.max(Settings.screenWidth, Settings.screenHeight) / -height;
          } else if (height == 0) {
            height = Math.max(Settings.screenWidth, Settings.screenHeight);
          } else {
            height = height *
                     Math.max(Settings.screenWidth, Settings.screenHeight) /
                     100;
          }
        } else if ((PARENTSIZE - RANGE) <= height &&
                   height <= (PARENTSIZE + RANGE)) {
          height -= PARENTSIZE;
          if (height < 0) {
        ...
        } else if ((PARENTSIZEMIN - RANGE) <= height &&
                   height <= (PARENTSIZEMIN + RANGE)) {
        ...
        } else if ((PARENTSIZEMAX - RANGE) <= height &&
                   height <= (PARENTSIZEMAX + RANGE)) {
        ...
        } else if ((FONTSIZE - RANGE) <= height &&
                   height <= (FONTSIZE + RANGE)) {
          height -= FONTSIZE;
          if (height < 0) {
            height = fmH / -height;
          } else if (height == 0) {
        ...
        if (x < MAXABSOLUTECOORD) {
          ...
          x -= width / 2;
        } else if ((PARENTSIZEMIN - RANGE) <= x &&
                   x <= (PARENTSIZEMIN + RANGE)) {
          x -= PARENTSIZEMIN;
          if (x < 0) {
            x = cli.width / -x;
          } else if (x == 0) {
          ...
          x -= width;
        } else if ((PARENTSIZEMAX - RANGE) <= x &&
                   x <= (PARENTSIZEMAX + RANGE)) {
          x -= PARENTSIZEMAX;
          if (x < 0) {
            x = cli.width / -x;
          } else if (x == 0) {
          ...
          x = parent.lastX + (parent.lastW - width) / 2 +
              (x - CENTER_OF) * fmH / 100;
        } else // guich@tc110_88
          if ((RIGHT_OF - RANGE) <= x && x <= (RIGHT_OF + RANGE)) {
          x = parent.lastX + (parent.lastW - width) +
              (x - RIGHT_OF) * fmH / 100; // guich@tc110_97
        }
        // y
        if (y <= MAXABSOLUTECOORD) {
        } else if ((AFTER - RANGE) <= y && y <= (AFTER + RANGE) &&
                   parent != null) {
          y = parent.lastY + parent.lastH + (y - AFTER) * fmH / 100;
        } else // guich@450_36: test parent only after testing the relative type
          if ((BEFORE - RANGE) <= y && y <= (BEFORE + RANGE) &&
              parent != null) {
          y = parent.lastY - height + (y - BEFORE) * fmH / 100;
        } else if ((SAME - RANGE) <= y && y <= (SAME + RANGE) &&
                   parent != null) {
          ...
          y -= height / 2;
        } else if ((PARENTSIZEMIN - RANGE) <= y &&
                   y <= (PARENTSIZEMIN + RANGE)) {
          y -= PARENTSIZEMIN;
          if (y < 0) {
            y = cli.height / -y;
          } else if (y == 0) {
          ...
          y -= height;
        } else if ((PARENTSIZEMAX - RANGE) <= y &&
                   y <= (PARENTSIZEMAX + RANGE)) {
          y -= PARENTSIZEMAX;
          if (y < 0) {
            y = cli.height / -y;
          } else if (y == 0) {
          ...
          y = parent.lastY + (parent.lastH - height) / 2 +
              (y - CENTER_OF) * fmH / 100;
        } else // guich@tc110_88
          if ((BOTTOM_OF - RANGE) <= y && y <= (BOTTOM_OF + RANGE)) {
          y = parent.lastY + (parent.lastH - height) +
              (y - BOTTOM_OF) * fmH / 100; // guich@tc110_97
        }
        // width that depends on x
        if (width > MAXABSOLUTECOORD) {
          if ((FILL - RANGE) <= width && width <= (FILL + RANGE)) {
            width = cli.width - x + cli.x + (width - FILL) * fmH / 100;
          } else if ((FIT - RANGE) <= width && width <= (FIT + RANGE) &&
                     parent != null) {
        ...
        if (height > MAXABSOLUTECOORD) {
          ...
          } else if ((FIT - RANGE) <= height && height <= (FIT + RANGE) &&
                     parent != null) {
        ...
        if (x < MAXABSOLUTECOORD) {
          ...
          x += cli.x - LEFT;
        } else if ((BEFORE - RANGE) <= x && x <= (BEFORE + RANGE) &&
                   parent != null) {
          ...
          if ((RIGHT_OF - RANGE) <= x && x <= (RIGHT_OF + RANGE)) {
          x +=
            parent.lastX + (parent.lastW - width) - RIGHT_OF; // guich@tc110_97
        }
        // y
        if (y <= MAXABSOLUTECOORD) {
        } else if ((AFTER - RANGE) <= y && y <= (AFTER + RANGE) &&
      ...
      if (Settings.onJavaSE &&
          Settings.showUIErrors) { // guich@450_36: do these checks only if
                                   // running on desktop
        if (cli.width == 0 || cli.height == 0) {
          ...
          if (!zeroIsValid) { // when a Control is inside a AccordionContainer,
                              // it can reach size 0, so we just ignore the
                              // exception
            throw new RuntimeException(
              parent + " must have its bounds set before calling " + this +
              ".setRect"); // guich@300_28
          }
          ...
          if (!error.isEmpty()) {
            throw new RuntimeException("You are using constant positions " +
                                       error.substring(0, error.length() - 1) +
                                       " in a wrong place for control " +
                                       toString());
          } else {
            throw new RuntimeException(
              "To use AFTER/BEFORE/SAME you must add first the control " +
              toString() + " to the parent container.");
          }
        } else if (x + y < -RANGE) { // guich@300_27
          if (x < -RANGE) {
            throw new RuntimeException(
              "You can't use FILL with BEFORE, CENTER or RIGHT for control " +
              toString());
          } else {
            throw new RuntimeException(
              "You can't use FILL with BEFORE, CENTER or BOTTOM for control " +
              toString());
          }
        }
        if (height < 0 && width < 0) {
          throw new RuntimeException(
            "Invalid resulting values in width,height for control " +
            toString() + ": " + width + "," + height);
        } else if (width < 0) {
          throw new RuntimeException(
            "Invalid resulting values in width for control " + toString() +
            ": " + width);
        } else if (height < 0) {
          throw new RuntimeException(
            "Invalid resulting values in height for control " + toString() +
            ": " + height);
        }
      }
    }
    ...
    if (asContainer != null) {
      ...
      asContainer.lastScreenWidth =
        Settings.screenWidth; // save the last screen resolution so we can be
                              // repositioned if a rotation occured at a time
                              // that the container was not on screen
    }
  }
  ...
  {
    return ((AFTER - RANGE) <= k && k <= (AFTER + RANGE)) ||
  ...
  {
    return ((LEFT - RANGE) <= k && k <= (LEFT + RANGE)) ||
      ((RIGHT - RANGE) <= k && k <= (RIGHT + RANGE)) ||
      ((RIGHT_OF - RANGE) <= k && k <= (RIGHT_OF + RANGE));
  }
  ...
  {
    return ((TOP - RANGE) <= k && k <= (TOP + RANGE)) ||
      ((BOTTOM - RANGE) <= k && k <= (BOTTOM + RANGE)) ||
      ((BOTTOM_OF - RANGE) <= k && k <= (BOTTOM_OF + RANGE));
  }

  /**
   * Resets the original points that are set by the first setRect, so if you
   * call setRect again, the old positions are replaced by the new ones. The set
   * positions are used when a rotation occurs.
   * @since TotalCross 1.25
   */
  public void resetSetPositions() { setX = SETX_NOT_SET; }
  ...
  public Coord getSize() { return new Coord(width, height); }

  /** Returns the current position (x,y) of this control */
  public Coord getPos() { return new Coord(x, y); }

  /**
   * Shows or "hides" this control. Note that it remains attached to its
   * container. Calls repaint.
   */
  ...
  /**
   * Returns a copy of the control's rectangle, relative to its parent. A
   * control's rectangle defines its location and size.
   */
  public Rect getRect()
  {
  ...
  /**
   * Returns the absolute coordinates of this control relative to the
   * MainWindow.
   */
  public Rect getAbsoluteRect()
  { // guich@102: changed name from
    // getRelativeRect to getAbsoluteRect.
    Rect r = getRect();
    Control c = parent;
    while (c != null) {
      r.x += c.x;
   ...
   * (eg: the control still not added to any container). If this control is a
   * window, will return itself.
   */
  ...
  public Control getNext() { return next; }

  /** Returns the previous child in the parent's list of controls. */
  public Control getPrev() { return prev; }
   ...
   * Important note: when you call repaint, a flag is set indicating that the
   * screen must be repainted; then, the next time a event (a keypress, a timer,
   * a pen event) occurs, the screen is updated. If you call repaint and the
   * control isn't effectively repainted, you can use the Control.repaintNow
   * method. <br><br> If you want to avoid a method call, you can do <pre>
   * Window.needsPaint = true;
   * </pre>
   * @see #repaintNow()
   */
  public static void repaint() { Window.needsPaint = true; }

  /**
   * Redraws the control immediately. If this control is a Window, the whole
   * window area is marked for repaint (useful if you're removing some controls
   * from a container). This method affects only this control, while the repaint
   * method affects the whole screen.
   *
  ...
  public void repaintNow()
  {
    ...
    if (w != null &&
        Window.zStack.indexOf(w, 0) >=
          0) { // guich@560_12: if we're not visible, this is nonsense
      if (Settings.isOpenGL ||
          !w.isTopMost()) { // guich@issue#80: if control is not topmost, must
                            // repaint all windows
        Window.needsPaint =
          true; // make sure the whole area is marked to be repainted
        if (MainWindow.isMainThread()) {
          ...
          Thread.yield();
                 ...
                 null) { // guich@200b4: if this is a Window, paint everything
        Window.needsPaint =
          true; // make sure the whole area is marked to be repainted
        asWindow._doPaint(); // doPaint already calls updateScreen here
      } else if (transparentBackground) {
        parent.repaintNow(); // guich@tc100: for transparent backgrounds we have
                             // to force paint everything
      } else {
  ...
  /**
   * Sets the given Point to the absolute coordinate relative to the origin
   * Window.
   * @since SuperWaba 5.5
   */
   ...
   * It sets a clipping rectangle on the graphics, clipping it against all
   * parent areas.
   */
  public Graphics getGraphics() { return refreshGraphics(gfx, 0, null, 0, 0); }

  Graphics refreshGraphics(Graphics g,
                           int expand,
                           Control topParent,
                           int tx0,
                           int ty0)
  ...
  public <H extends EventHandler> void postEvent(Event<H> event)
  {
    ...
    if (Settings.keyboardFocusTraversable &&
        event.type == ControlEvent.PRESSED && !(event.target instanceof Edit)) {
      (asContainer != null ? asContainer : parent.asContainer)
        .setHighlighting();
    }

    // don't dispatch events when disabled except TIMER events or (fingertouch
    // and pen events)
    if ((!isEnabled() &&
    ...
    if (dragTarget != null) { // improve drag performance by sending the event
                              // directly to the drag handler control
    ...
    if (!event.consumed) {
      ...
      if (!eventTargetCalled &&
          event.target instanceof
            Control) { // guich@tc110_52: call any listeners of the target
                       // control - guich@tc112_3: if not yet called
    ...
    event.consumed =
      false; // set to false again bc some controls reuse event objects
  }
   ...
   */
  public void setEnabled(boolean enabled) { internalSetEnabled(enabled, true); }
  ...
  public boolean internalSetEnabled(boolean enabled, boolean post)
  {
    if (enabled != this.enabled) {
      ...
      Window.needsPaint =
        true; // now the controls have different l&f for disabled states
      return true;
    }
    return false;
  }
   ...
   * @param screenChanged If the bounds were changed due to a screen change
   *     (rotation, collapse)
   */
  protected void onBoundsChanged(boolean screenChanged) {}

  /**
   * Called after a setEnabled, setForeColor and setBackColor and when a
   * control has been added to a Container. If colorsChanged is true, it was
   * called from setForeColor/setBackColor/Container.add; otherwise, it was
  ...
  /**
   * Called after the window has finished a paint. Only called to the focused
   * control and the parent's window.
   */
   ...
   * Calling this method is faster than calling setBackColor and setForeColor
   * separately.
   */
 ...
 */
  public int getForeColor()
  {
    return isEnabled()
      ? foreColor
      : uiMaterial ? Color.getGray(foreColor) : Color.brighter(foreColor);
  }
 ...
 */
  public int getBackColor()
  {
    ...
    return (isEnabled() || parent == null) ? backColor
                                           : Color.darker(backColor);
  }

  /**
   * Return true if the parent of this Control is added to somewhere.
   * Some containers, like the TabPanel, has n child containers, but only one is
   * added at a time. With this method, you can discover if your container is
   * the one being shown.
   */
  ...
  /**
   * Sets the focus to this control. Note that in penless devices its also
   * needed to set <code>isHighlighting = false</code>.
   */
  public void requestFocus()
  {
    ...
    if (w != null) {
      w.setFocus(this);
      // does not work - test on UIGadgets for example - isHighlighting = false;
      // // guich@570_39: if the user decided to place the focus here, we should
      // disable highlighting traversals
    }
  }
   ...
   * <pre>
         ...
         add(numericPad=new PushButtonGroup(numerics, false, -1, -1, 6, 4, true,
                                             PushButtonGroup.BUTTON), RIGHT-2,
   TOP+2); numericPad.setFocusLess(true);

         // in the onEvent method
         Control focus;
         if (e.target == numericPad && (focus=getParentWindow().getFocus())
   instanceof Edit)
         {
  ...
  /** Internal use Only. */
  public static void resetStyle() { uiStyleAlreadyChanged = false; }
  ...
  public static void uiStyleChanged()
  {
    if (!uiStyleAlreadyChanged) {
      ...
      uiAndroid = Settings.uiStyle == Settings.Android ||
                  Settings.uiStyle == Settings.Holo || uiMaterial;
      ...
      throw new RuntimeException(
        "The user interface style can be changed only once, in the MainWindow's constructor.");
    }
  }
  ...
  private Control getNextHighlighted(Container p, boolean forward)
  {
    ...
    if (p == this) {
      idx = forward
              ? -1
              : last + 1; // return (Control)v.items[forward?0:last]; // return
                          // the first/last control - guich@573_22: can't just
                          // go to first or last - they may be disabled!
    } else if (idx == -1) {
      return null;
    }
    ...
    while (true) {
      ...
      if (c.visible && c.isEnabled() && !c.focusLess) {
        if (c.focusTraversable) {
          return c;
        } else if (c.asContainer !=
                   null) { // kmeehl@tc100: look through child containers for
  ...
  {
    Control c = getNextHighlighted(
      p,
      forward); // allow the control to find the next control to highlight
    if (c != null) {
      Window w = getParentWindow();
      if (w != null) {
        w.setHighlighted(c);
      ...
      if (this == p) {
        changeHighlighted(
          parent, forward); // look in parent for the next focusable control
      } else {
        parent.changeHighlighted(
          parent.parent != null ? parent.parent : parent,
          forward); // return to the parent of the parent and continues -
                    // p.p!=null: if our parent is a window (E.G. UIGadgets'
                    // scrollbar), his parent is null; so we restart from the
                    // window
      }
    } else if (p != this.asWindow) {
      changeHighlighted(this.asWindow,
                        forward); // we're the window - restart everything
    }
  }

  /**
   * Placeholder for the clear method. The class that inherits control is
   ...
   * @since SuperWaba 5.0
   */
  @ReplacedByNativeOnDeploy public static void updateScreen()
  {
   ...
   * the screen is updated immediatly. If not, the updated is schedulled to
   ...
   * @since TotalCross 1.14
   */
  public int getX2() { return x + width - 1; }

  /**
   * Returns y+height-1
   * @since TotalCross 1.14
   */
  public int getY2() { return y + height - 1; }

  /**
   * Repositions this control, and dives into other controls if this is a
   * container and recursive is true.
   */
  ...
  /**
   * Repositions the children controls. This is usually called from reposition,
   * but you can call by yourself if you are changing this control's position
   * and size by yourself (for example during rotaton)
   */
   ...
   * parameters.
   */
  public void reposition() { reposition(true); }
  ...
  public <H extends EventHandler> void _onEvent(Event<H> e)
  {
    ...
    if (!e.consumed && listeners.size() > 0 &&
        (callListenersOnAllTargets || e.target == this)) {
  ...
  {
    // although this code is not much eficient, the number of listeners for a
    // single control will be only one, most of the times.
    for (int i = 0; listeners != null && i < listeners.size() && !e.consumed;
         i++) {
      try {
        Listener l = listeners.get(i);
        if (e.target == l.target ||
            (callListenersOnAllTargets &&
             (e instanceof ListContainerEvent || e instanceof KeyEvent ||
              e instanceof PenEvent))) {
          if (e instanceof ValueChangeEvent && l.listener instanceof
                                                 ValueChangeHandler) {
            ((ValueChangeEvent)e).dispatch((ValueChangeHandler)l.listener);
            continue;
          } else if (e instanceof FontChangeEvent && l.listener instanceof
                                                       FontChangeHandler) {
            ((FontChangeEvent)e).dispatch((FontChangeHandler)l.listener);
            continue;
          } else if (e instanceof SizeChangeEvent && l.listener instanceof
                                                       SizeChangeHandler) {
          ...
          switch (e.type) {
            ...
            case PushNotificationEvent.MESSAGE_RECEIVED:
              if (l.type == Listener.PUSHNOTIFICATION) {
                ((PushNotificationListener)l.listener)
            ...
            case ListContainerEvent.ITEM_SELECTED_EVENT:
              if (l.type == Listener.LISTCONTAINER) {
                ((ListContainerListener)l.listener)
                  .itemSelected((ListContainerEvent)e);
              }
              break;
            case ListContainerEvent.LEFT_IMAGE_CLICKED_EVENT:
              if (l.type == Listener.LISTCONTAINER) {
                ((ListContainerListener)l.listener)
                  .leftImageClicked((ListContainerEvent)e);
              }
              break;
            case ListContainerEvent.RIGHT_IMAGE_CLICKED_EVENT:
              if (l.type == Listener.LISTCONTAINER) {
                ((ListContainerListener)l.listener)
                  .rightImageClicked((ListContainerEvent)e);
              }
              break;
            case EnabledStateChangeEvent.ENABLED_STATE_CHANGE:
              if (l.type == Listener.ENABLED) {
                ((EnabledStateChangeListener)l.listener)
                  .enabledStateChange((EnabledStateChangeEvent)e);
              }
              break;
          }
        }
      } catch (ClassCastException
                 ee) { // prevent totalcross.ui.event.PenEvent is not
                       // compatible with totalcross.ui.event.DragEvent
        if (Settings.onJavaSE) {
          throw ee;
        }
      }
  ...
  /**
   * Used by the main event loop to give the currently focused control an
   * opportunity to act directly on the KeyEvent.
   * @param ke The KeyEvent to be processed
   * @return The control that should get focus as a result of this KeyEvent.
   *     Null if this control did not
   * handle the KeyEvent.
   ...
   * @see Listener
   */
  public List<Listener> getEventListeners() { return listeners; }
  ...
  { // guich@tc115_40
    return this.visible && this.y < yf && (this.y + this.height) > y0 &&
      this.x < xf &&
      (this.x + this.width) >
        x0; // guich@200: ignore hidden controls - note: a window added to a
            // container may not be painted correctly
  }

  boolean isVisibleAndInside(int y0, int yf)
  { // guich@tc115_40
    return this.visible && this.y < yf &&
      (this.y + this.height) >
        y0; // guich@200: ignore hidden controls - note: a window added to a
            // container may not be painted correctly
  }

  public int getGap(int gap)
  {
    return Settings.uiAdjustmentsBasedOnFontHeight &&
        uiAdjustmentsBasedOnFontHeightIsSupported
      ? gap * fmH / 100
      : gap;
  }
  ...
  /**
   * Returns true if the parent of this control is a Scrollable and had
   * scrolled since the last pen down.
   *
  ...
  /**
   * Returns true if this is a MultiEdit or an Edit that has a standard
   * keyboard.
   */
  protected boolean willOpenKeyboard() { return false; }

  /** Returns if this event should be handled as an action. */
  protected <H extends EventHandler> boolean isActionEvent(Event<H> event)
  {
    return (Settings.fingerTouch && event.type == PenEvent.PEN_UP &&
            !hadParentScrolled()) ||
      (!Settings.fingerTouch && event.type == PenEvent.PEN_DOWN);
  }

  /**
   * Returns true if this control is added to the given container at some
   * higher level.
  ...
  public boolean isObscured(Window parentWindow)
  {
    ...
    int cx1 = Window.topMost.x, cy1 = Window.topMost.y,
        cx2 = cx1 + Window.topMost.width, cy2 = cy1 + Window.topMost.height;
  ...
  /**
   * Make this control a translucent one by setting the desired shape instead
   * of NONE. Calling this resets the backColor to BLACK, foreColor to WHITE,
   * and the textShadowColor to 0x444444, but you may change that value later.
  ...
  public void setTranslucent(TranslucentShape shape)
  {
    ...
    if (doEffect && effect != null) {
      effect.enabled = translucentShape == TranslucentShape.NONE ||
                       translucentShape == TranslucentShape.RECT;
    }
  }
  ...
  public boolean drawTranslucentBackground(Graphics g, int alphaValue)
  {
    if (translucentShape != TranslucentShape.NONE) {
      if (transback == null || transback.getWidth() != width ||
          transback.getHeight() != height) {
        try {
          ...
          switch (translucentShape) {
            ...
            case CIRCLE:
              gg.fillRoundRect(
                (width - height) / 2, 0, height, height, height / 2);
              break;
          }
        } catch (Throwable t) {
        }
  ...
  public boolean getDoEffect() { return doEffect; }

  /** Set to false to make this control not to do it's effect.*/
  public void setDoEffect(boolean doEffect) { this.doEffect = doEffect; }
}
