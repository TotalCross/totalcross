 ...
 *
 * <pre>
 * String[] items = { "Always", "Never", "Only in Silent mode", "Only when not
 * in Silent mode", "Non of the answers above", }; PopupMenu pm = new
 * PopupMenu("Vibrate", items); pm.popup();
 * </pre>
 *
...
public class PopupMenu extends Window
{
  ...
  /**
   * If the items is a String matrix (String[][]), this field sets the column
   * that will be shown.
   */
  public int dataCol;

  /**
   * Sets the number of elements should be used from the items array passed in
   * the constructor. Defaults to <code>items.length</code>.
   */
  ...
  @Override public boolean onClickedOutside(PenEvent event)
  {
    selected = -1;
    unpop();
    return false;
   ...
   * selection support.
   */
  public PopupMenu(String caption, Object[] items)
  ...
  public PopupMenu(String caption, Object[] items, boolean multipleSelection)
    throws IOException, ImageException
  {
    ...
    this.caption.setText(caption =
                           StringUtils.shortText(caption, font.fm, width));
  ...
  //				StringUtils.shortText(caption.getText(), font.fm,
  //width
  //- menu.paddingLeft - menu.paddingRight)); 		if (title != null &&
  //! title.equals("")) 			add(caption, CENTER, TOP + fmH /
  //! 2);
  //
  //		add(menu, LEFT, title == null || title.equals("") ? TOP : AFTER
  //+ menu.paddingTop, FILL, FILL);
  //	}
  ...
  public void initUI()
  {
    try {
      caption.transparentBackground = true;
      this.caption.setText(
        StringUtils.shortText(caption.getText(),
                              font.fm,
                              width - menu.paddingLeft - menu.paddingRight));
      ...
      if (multipleSelection || !uiMaterial) {
        ...
        layout.defaultRightImage2 =
          getSelectedImage(checkColor == -1 ? foreColor : checkColor);
        layout.imageGap = 50;
      ...
      for (int i = 0; i < itemCount; i++) {
        ...
        containers[i] = c;
        String s = items[i] instanceof String
                     ? (String)items[i]
                     : (items[i] instanceof String[])
                         ? ((String[])items[i])[dataCol]
                         : items[i].toString();
        ...
        if (cw == -1) {
          cw = getClientRect().width - Math.abs(c.getLeftControlX()) -
               Math.abs(c.getRightControlX());
        }
        ...
        if (sw <= cw) {
          ...
          if (uiMaterial) {
            String[] parts =
              Convert.tokenizeString(Convert.insertLineBreak(cw, fm, s), '\n');
            c.items = new String[] { "", "", "" };
            for (int j = 0, n = Math.min(parts.length, c.items.length); j < n;
                 j++) {
              c.items[j] = parts[j];
            }
          } else {
      ...
      if (enableSearch) {
        ...
        add(sc2 = new ScrollContainer(true, false),
            ...
            TOP + UnitsConverter.toPixels(DP + 8),
            FILL - UnitsConverter.toPixels(DP + 2),
            DP + 52);
        sc2.disableFlick();
        for (int i = 0; i < v.size(); i++) {
          ...
          sc2.add(charFilterB,
                  AFTER + UnitsConverter.toPixels(DP + 8),
                  TOP + UnitsConverter.toPixels(DP + 4),
                  DP + 48,
                  DP + 48);
        }
      }
      ...
      add(list,
          LEFT + UnitsConverter.toPixels(DP + 2),
          enableSearch || this.caption != null ? AFTER + fmH / 5 : TOP,
          FILL - UnitsConverter.toPixels(DP + 2),
          (enableCancel ? FIT : FILL) - fmH / 2,
  ...
  @Override public void onEvent(Event event)
  {
    switch (event.type) {
      ...
      case ListContainerEvent.RIGHT_IMAGE_CLICKED_EVENT: {
        ListContainerEvent lce = (ListContainerEvent)event;
        // if (lce.isImage2) since tc 1.5, when this event is sent the image 2
        // was already replaced by image 1
        {
          int idx;
      ...
      if (npback == null && npParts != null) {
        try {
          npback = NinePatch.getInstance().getNormalInstance(
            npParts, width, height, backColor, false);
  ...
  {
    return uiMaterial ? (caption == null || caption.getText().equals("")
                           ? 0
                           : menu.paddingTop * 2 + fmH) +
                          Math.min(menu.getPreferredHeight(), maxH)
                      : super.getPreferredHeight();
  }

  /**
   * Returns the ListBox used to show the menu. You can edit the paddings the
   * way you want calling this method.
   */
  public ListBox getMenu() { return menu; }

  public Object getSelectedItem() { return menu.getSelectedItem(); }
  ...
  public void popupAt(int x, int y) { popupAt(x, y, PREFERRED, PREFERRED); }
  ...
  @Override protected void onPopup()
  {
    ...
    if (list == null) {
      int maxW = Math.max(!enableCancel ? 0 : fm.stringWidth(cancelString),
                          title == null ? 0 : titleFont.fm.stringWidth(title)) +
                 fmH * 4;
      for (int i = 0; i < itemCount; i++) {
        String s = items[i] instanceof String
                     ? (String)items[i]
  ...
  public void setCursorColor(int c) { cursorColor = c; }
}
