 ...
 * On Windows and Linux desktop, the tooltip is also shown when the mouse stays
 *over a control. <br><br> The default popup delay is 1000ms and the amount of
 *time it will be displayed is 2000 ms; a pen up also hides the tip. <p> You can
 *change some properties of the tooltip: borderColor, insideGap, distX, distY,
 ...
 * t.setBackColor(Color.getRGB(250,0,0));  // same as control's container by
 *default
 * </pre>
 * The tooltip can have multiple lines, just split them using the char '\n' in
 *your message.<br> Example : <pre> ToolTip t = new ToolTip(control, "Hi!\nIt's
 *Me");
 * </pre>
 * A ControlEvent.PRESSED event will be dispatched to the attached control right
 *before the text is shown. You can then set the tip to a new value using
 *setText; setting to an empty string will disable the tooltip at that moment.
 *Calling setControlRect also changes the rectangle around which the tooltip
 *will be displayed.
 *
...
public class ToolTip extends Label implements PenListener, MouseListener
{
  ...
  /**
   * The x distance between the tip and the control, 0 by default. Common to
   * all tooltips.
   */
  public static int distX;
  /**
   * The y distance between the tip and the control, 0 by default. Common to
   * all tooltips.
   */
  public static int distY;

  /**
   * The amount of time that the pen must be down until the tip pops up (by
   * default, 1000ms)
   */
   ...
   * If null, you must call <code>setControlRect</code> and <code>show</code> by
   *your own.
  ...
  public ToolTip(Control control, String message)
  {
    ...
    if (control != null) {
      control.addPenListener(
        this); // i must admit: listeners simplified a LOT the ToolTip class
  ...
  /**
   * Use this handy method to split the text in order to correctly fit the
   * window. Here's a sample: <pre> String msg = "a very long text that will be
   * split to fit in the window"; new ToolTip(control, ToolTip.split(msg, fm));
   * </pre>
   * Make sure that fm will be the font's FontMetrics (if you plan to change the
   * font after calling the constructor).
   * @deprecated The split is done automatically
   * @since TotalCross 1.2
   */
  @Deprecated public static String split(String msg, FontMetrics fm)
  {
    return Convert.insertLineBreak(
      Settings.screenWidth - insideGap * 2, fm, msg);
  }

  /**
   * Change the control rect to the given one. By default, its used the
   * absolute rectangle of the control passed in the constructor. The placement
   * of the tooltip will be defined based on it, in a way that the control is
   * not obscured.
   */
  public void setControlRect(Rect r)
  {
    ...
    w.add(this); // guich@tc100b4_19: must always be (re)added to the parent
                 // container to make sure we will be the last control that will
                 // be painted
    Coord size = w.getSize();

    int ww =
      msg0lines != null
        ? fm.getMaxWidth(msg0lines,
                         0,
                         msg0lines.length)
        : fm.stringWidth(msg0); // guich@tc120_2: moved to after w.add(this)
    if (ww == 0) {
      ww = getMaxTextWidth() + insideGap;
    }
    if (ww > Settings.screenWidth) {
      super.setText(Convert.insertLineBreakBalanced(
        Settings.screenWidth * 9 / 10, fm, msg0));
    ...
    if (r.y2() + distY + hh < size.y && r.height != 0) {
      ...
      yy = Math.max(
        0, r.y - hh - distY - w.y); // guich@tc126_48: decrease window's y
    }
    ...
    setRect(xx,
            yy,
            ww + (super.lines.length == 1 ? insideGap : fmH),
            hh,
            null,
            false); // careful: xx,yy are relative to window position, but the
                    // control's rect passed as parameter is ABSOLUTE
  }
    ...
    if (e.type == TimerEvent.TRIGGERED) {
      if (delayTimer != null && delayTimer.triggered) {
        ...
        if (control != null) {
          ...
          control._onEvent(
            getPressedEvent(this)); // tell the parent that we'll popup
        }
        if (text.length() > 0) {
          show();
        }
  ...
  {
    outside.absoluteX = outside.x = outside.absoluteY = outside.y =
      -(Settings.touchTolerance +
        1); // use a small value because some controls (like Grid) may behave
            // incorrectly if we use big values
  ...
  @Override public void mouseIn(MouseEvent e) { penDown(e); }

  @Override public void mouseOut(MouseEvent e) { penUp(e); }
  ...
  @Override public void penUp(PenEvent e)
  {
    ...
    if (delayTimer != null ||
        displayTimer !=
          null) { // guich@503_1: if the user removes the pen before the
                  // popup, do not show the tooltip anymore - guich@570_93: if
                  // the user removes the pen before timeout, remove the
                  // timers as well
  ...
  @Override public void onPaint(Graphics g)
  {
    if (displayTimer != null) { // only draw if we're visible
      ...
      if (uiAndroid) {
        ...
        g.fillRect(0, 0, dx, height); // since g will be translated, fill the
                                      // part that the Label won't
      }
