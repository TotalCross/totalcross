...
public class ToolTip extends Label implements PenListener, MouseListener {
	...
	public void setControlRect(Rect r) {
		...
		int ww = msg0lines != null ? fm.getMaxWidth(msg0lines, 0,
				 msg0lines.length) : fm.stringWidth(msg0); // guich@tc120_2: moved to after w.add(this)
		if (ww == 0) {
		...
		setRect(xx, yy, ww + (super.lines.length == 1 ? insideGap : fmH), hh, null,
				false); // careful: xx,yy are relative to window position, but the control's rect passed as parameter is ABSOLUTE
	}
	...
	private void postOutside() {
		outside.absoluteX = outside.x = outside.absoluteY = outside.y = -(Settings.touchTolerance +
										1); // use a small value because some controls (like Grid) may behave incorrectly if we use big values
	...
	public void penUp(PenEvent e) {
		...
		if (delayTimer != null
			|| displayTimer !=
			null) { // guich@503_1: if the user removes the pen before the popup, do not show the tooltip anymore - guich@570_93: if the user removes the pen before timeout, remove the timers as well
