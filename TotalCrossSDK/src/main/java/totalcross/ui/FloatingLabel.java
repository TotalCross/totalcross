...
public class FloatingLabel<T extends Control & HasValue<?>>
{
  ...
  public FloatingLabel(T target)
  {
    ...
    testPercentage = 0;
    executedTime = 0;
    captionFontSmall = Font.getFont(
      target.getFont().name, false, target.getFont().size * 75 / 100);
    fcap = target.getFont();

    target.addFontChangeHandler(new FontChangeHandler() {
      @Override public void onFontChange(FontChangeEvent event)
      {
        captionFontSmall =
          Font.getFont(event.font.name, false, event.font.size * 75 / 100);
        fcap = event.font;
      }
    });
    ...
    if (target instanceof TextControl) {
      target.addFocusListener(new FocusListener() {
        @Override public void focusOut(ControlEvent e)
        {
        ...
        @Override public void focusIn(ControlEvent e)
        {
          if (target.getValue() == null & isExpanded) {
            animateMaterial(true);
          }
  ...
  private void singleStep(int ellapsedMilliseconds)
  {
    ...
    if (target instanceof OutlinedEdit) {
      ...
      float timeT =
        1.0f / ellapsedMilliseconds + (float)executedTime / totalTime;
      if (timeT > 1) {
        timeT = 1;
      } else if (timeT < 0) {
        timeT = 0;
      ...
      if (isExpanded ? testPercentage < timeT : testPercentage > 1 - timeT) {
        ...
        float lastStep = (isExpanded ? (textSizeDifference - 1) : 1) /
                         (float)textSizeDifference;
        float lastStepLastFortyPercent =
          isExpanded ? lastStep + 6 / 10 / textSizeDifference
                     : 4 / 10 * lastStep;
        if (isExpanded ? timeT < lastStepLastFortyPercent
                       : 1 - timeT > lastStepLastFortyPercent &&
                           testPercentage == lastStep) {
          fcap = fcap.adjustedBy(-inccap);
          testPercentage -= -inccap / (float)textSizeDifference;
        }
      }
      ...
      if (fcap.size ==
          (isExpanded ? captionFontSmall.size : target.font.size)) {
        timeT = 1;
      }
      ...
      fcap = fcap.adjustedBy(inccap);
      // Label Position Animation
      ycap = ycap0 * (fcap.fm.height - fmHmin) /
             (target.getFont().fm.height - fmHmin);
      xcap = xcap0 * (fcap.fm.height - fmHmin) /
             (target.getFont().fm.height - fmHmin);
    } else {
      ...
      float timeT =
        1.0f / ellapsedMilliseconds + (float)executedTime / animationTime;
      if (timeT > 1) {
        timeT = 1;
      } else if (timeT < 0) {
        timeT = 0;
      ...
      yA = isExpanded
             ? ycap0
             : UnitsConverter.toPixels(Control.DP + 20) -
                 captionFontSmall.fm.ascent - captionFontSmall.fm.descent;
      yB = !isExpanded
             ? ycap0
             : UnitsConverter.toPixels(Control.DP + 20) -
                 captionFontSmall.fm.ascent - captionFontSmall.fm.descent;
  ...
  {
    // Label Font size Animation
    fcap = fcap.adjustedBy(isExpanded
                             ? captionFontSmall.size - target.getFont().size
  ...
  public int getExtraHeight() { return captionFontSmall.fm.height; }

  public Font getCaptionFontSmall() { return captionFontSmall; }

  public Font getFcap() { return fcap; }

  public boolean isBig() { return isExpanded; }

  public int getTopX() { return topX; }
  public void setTopY(int variable1) { this.topX = variable1; }
  public int getTopY() { return topY; }
  public void setTopX(int variable2) { this.topY = variable2; }
}
