 ...
 * It implements auto scroll when pressing and holding a button or the gap area
 * of the scrollBar. Here is an example of how to use it. <pre>
 *  // declarations
 *  ScrollBar sb1;
 *  ScrollBar sb2;
 *  ScrollBar sb3;
 ...
 * totalcross.sys.Settings.screenHeight/2); add(sb2 = new
 * ScrollBar(ScrollBar.VERTICAL), BEFORE, SAME, PREFERRED, SAME);
 *     sb2.setLiveScrolling(true);
 *     add(sb3 = new ScrollBar(ScrollBar.HORIZONTAL),
 * LEFT,CENTER,totalcross.sys.Settings.screenWidth/2, PREFERRED); add(sb4 = new
 * ScrollBar(ScrollBar.HORIZONTAL), SAME, AFTER, SAME, PREFERRED);
 *  }
 *
  ...
  implements PathAnimation.SetPosition, ControlAnimation.AnimationFinished
{
  ...
  /**
   * Set to true to disable block increments, which occurs when the user clicks
   * outside the bar and buttons.
  ...
  /**
   * The minimum dragbar size in pixels. By default, 5. This has no effect if
   * the ui style is Palm OS.
   */
  public int minDragBarSize = 5 * Settings.screenHeight / 160; // guich@510_26

  /* luciana@570_22: Implements auto scroll when pressing and holding button or
   * scrollBar */
  ...
  /**
   * The extra size (width for vertical ScrollBars or height for horizontal
   * ones) used in all ScrollBars. Note that this member is static so it will
   * affect all ScrollBars created afterwards, unless you reset it to 0.
   ...
   */
  public ScrollBar() { this(VERTICAL); }
  ...
  public ScrollBar(byte orientation)
  {
    ...
    btnDec =
      new ArrowButton(verticalBar ? Graphics.ARROW_UP : Graphics.ARROW_LEFT,
                      fmH * 3 / 11 + extra,
                      Color.BLACK);
    btnInc =
      new ArrowButton(verticalBar ? Graphics.ARROW_DOWN : Graphics.ARROW_RIGHT,
                      fmH * 3 / 11 + extra,
                      Color.BLACK);
                        ...
                        int newMaximum)
  {
    ...
    if (visibleItems + newValue + minimum <= maximum) {
      ...
      value =
        maximum -
        visibleItems; // msicotte@502_5: don't let value get more than maximum
    }
    if (value < minimum) {
   ...
   */
  public void setBlockIncrement(int i) { blockIncrement = i; }
  ...
  public void setValue(int i)
  {
    if (i != value) { // guich@320_30
      value =
        (visibleItems + i /*+minimum*/ <= maximum)
          ? i
          : (maximum - visibleItems); // guich@510_25: if greater than the
                                      // maximum, set to the max. guich@tc100:
                                      // removed +minimum, because if
                                      // min=6,max=22,vis.items=1,value=18, it
                                      // would fail
      if (value < minimum) {
        value =
          minimum; // guich@tc100: don't let the value get under the minimum
      }
  ...
  /**
   * Set the count of visible items for the scrollbar. This value cannot be
   * zero. It also sets the blockIncrement to be equal to the given value.
   */
  ...
  /**
   * Set the live scrolling. If "true" an event is thrown during dragging or
  ...
  protected void recomputeParams(boolean justValue)
  {
    ...
    if (!justValue) {
      ...
      dragBarSize = thumbSize != 0
                      ? thumbSize
                      : Math.max(minDragBarSize,
                                 Math.min(barArea,
                                          (int)(valuesPerPixel * visibleItems) +
                                            1)); // guich@300_13
    ...
    dragBarPos =
      Math.min(dragBarMax,
               dragBarMin + (int)(valuesPerPixel * (value - minimum) +
                                  0.5d)); // round value - guich@512_12:
  ...
  @Override public void onEvent(Event event)
  {
    ...
    switch (event.type) {
      ...
      case ControlEvent.PRESSED:
        ...
        // mustPostEvent = true; afarine@350_15: avoid extra 2 events when
        // pressing a button - the event will be posted in the penUp event
        break;
      ...
      case PenEvent.PEN_DRAG_END:
        ...
        if (disableBlockIncrement &&
            (pos < dragBarPos || pos > dragBarPos + dragBarSize)) {
          event.consumed = true;
        }
        return;
      case PenEvent.PEN_DOWN:
        ...
        if (event.target == this) { // can be the buttons
          ...
          if (!directMove && pos < dragBarPos - margin) {
            ...
            startDragPos =
              pos - dragBarPos + midBarSize; // point inside drag bar
            Window.needsPaint = true;
          }
        } else if (enableAutoScroll &&
                   (event.target == btnInc ||
                    event.target == btnDec)) { // luciana@570_22
          autoScrollTimer = addTimer(INITIAL_DELAY);
      ...
      case PenEvent.PEN_DRAG:
        oldPos = -1;
        if (event.target == btnDec ||
            event.target == btnInc) { // kmeehl@tc100: cancel the scroll timer
                                      // when the user drags off of the buttons
          Control btn = (Control)event.target;
          PenEvent pe = (PenEvent)event;
          if (!btn.isInsideOrNear(pe.x, pe.y) &&
              autoScrollTimer !=
                null) { // guich@tc114_51: must add control's coordinates
            disableAutoScroll();
            break;
          }
        }
        ...
        autoScrollBarPos =
          verticalBar ? ((PenEvent)event).y : ((PenEvent)event).x;
        event.consumed = true;
      ...
      case PenEvent.PEN_UP:
        ...
        if (directMove && oldPos >= 0) {
          ...
          pathanim = PathAnimation.create(
            this, oldPos, 0, pos, 0, this, UIEffects.duration);
        ...
        recomputeParams(true); // guich@tc100: otherwise, Slider may fall into a
                               // wrong position on pen up
        break;
      case KeyEvent.SPECIAL_KEY_PRESS:
        KeyEvent ke = (KeyEvent)event;
        if (ke.isPrevKey()) { // guich@330_45
          if (btnDec.isEnabled()) {
            value -= Settings.keyboardFocusTraversable ? unitIncrement
                                                       : blockIncrement;
          }
          event.consumed = true;
  ...
  protected void updateValue(int pos)
  {
    ...
    if (dragBarPos == dragBarMax) {
      value = Math.max(
        0, maximum - visibleItems); // guich@556_7: fixed the correct value,
                                    // subtracting by visibleItems
    } else {
      value = (int)((dragBarPos - dragBarMin) / valuesPerPixel);
      if (unitIncrement != 1) {
        value = ((int)value / unitIncrement) * unitIncrement;
      }
      value += minimum; // msicotte@502_5: fixes problem when minimum is
                        // different of zero
    }
  }
  ...
  private void setNewValue(int oldValue, boolean mustPostEvent)
  {
    ...
    // else - guich@tc113_16: commented out this else to make sure we're not
    // below min if value is changed in the if above
    if (value <= minimum) {
      value = minimum;
      removeTimer(autoScrollTimer);
    }
  ...
  { // luciana@570_22
    ...
    if (!b && (foc == btnDec ||
               foc == btnInc)) { // guich@572_16: only if the focus is one of
                                 // the buttons - bruno@tc114: avoid
                                 // auto-setting the focus to a multi-edit
      requestFocus();
    ...
    if (verticalBar) {
      if (uiVista && isEnabled()) {
        g.fillVistaRect(0,
                        ...
                        backColor,
                        startDragPos != -1,
                        true); // guich@tc110_51: press the bar if dragging.
      } else {
      ...
      g.draw3dRect(0,
                   ...
                   width,
                   dragBarSize,
                   uiVista ? Graphics.R3D_CHECK : Graphics.R3D_RAISED,
                   false,
                   false,
      ...
      if (uiVista && isEnabled()) {
        g.fillVistaRect(dragBarPos,
                        0,
                        dragBarSize,
                        height,
                        backColor,
                        startDragPos != -1,
                        false); // guich@tc110_51: press the bar if dragging.
      } else {
        g.fillRect(dragBarPos, 0, dragBarSize, height);
      }
      g.draw3dRect(dragBarPos,
                   0,
                   dragBarSize,
                   height,
                   uiVista ? Graphics.R3D_CHECK : Graphics.R3D_RAISED,
  ...
  /**
   * If this is a vertical scroll bar, i strongly suggest you use PREFERRED in
   * your control's width (with small adjustments).
   */
  @Override public int getPreferredWidth()
  {
    return (verticalBar ? btnDec.getPreferredWidth() + extraSize
                        : (btnDec.getPreferredHeight() << 1)) +
      insets.left + insets.right; // guich@240_18 - guich@300_70
  }

  /**
   * If this is a horizontal scroll bar, i strongly suggest you use PREFERRED
   * in your control's height (with small adjustments)
   */
  @Override public int getPreferredHeight()
  {
    return (verticalBar ? (btnDec.getPreferredWidth() << 1)
                        : btnDec.getPreferredHeight() + extraSize) +
      insets.top + insets.bottom; // guich@300_70: vertical bar always use
                                  // width; horizontal always use height
  }
  ...
  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    ...
    btnInc.setRect(verticalBar ? 0 : (size - btnWH),
                   verticalBar ? (size - btnWH) : 0,
                   btnWH,
                   btnWH,
                   null,
                   screenChanged);
  ...
  @Override public void setHighlighting() {}
