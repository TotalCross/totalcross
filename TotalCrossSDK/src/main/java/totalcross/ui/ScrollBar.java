...
public class ScrollBar extends Container implements PathAnimation.SetPosition,
	...
	public ScrollBar(byte orientation) {
		...
		btnDec = new ArrowButton(verticalBar ? Graphics.ARROW_UP : Graphics.ARROW_LEFT,
								 fmH * 3 / 11 + extra, Color.BLACK);
		btnInc = new ArrowButton(verticalBar ? Graphics.ARROW_DOWN : Graphics.ARROW_RIGHT,
								 fmH * 3 / 11 + extra,
								 Color.BLACK);
	...
	public void setValue(int i) {
		if (i != value) { // guich@320_30
			value = (visibleItems + i/*+minimum*/ <= maximum) ? i : (maximum -
					visibleItems); // guich@510_25: if greater than the maximum, set to the max. guich@tc100: removed +minimum, because if min=6,max=22,vis.items=1,value=18, it would fail
			if (value < minimum) {
	...
	protected void recomputeParams(boolean justValue) {
		...
		if (!justValue) {
			...
			dragBarSize = thumbSize != 0 ? thumbSize
						  : Math.max(minDragBarSize, Math.min(barArea,
									 (int)(valuesPerPixel * visibleItems) + 1));  // guich@300_13
		...
		dragBarPos = Math.min(dragBarMax,
							  dragBarMin + (int)(valuesPerPixel * (value - minimum) +
									  0.5d));  // round value - guich@512_12: subtract minimum from value
		enableButtons();
	}
	...
	public void onEvent(Event event) {
		...
		switch (event.type) {
			...
			case PenEvent.PEN_DOWN:
				...
				} else if (enableAutoScroll && (event.target == btnInc
			...
			case PenEvent.PEN_DRAG:
				...
				if (event.target == btnDec
					|| event.target ==
					btnInc) { // kmeehl@tc100: cancel the scroll timer when the user drags off of the buttons
					Control btn = (Control) event.target;
					PenEvent pe = (PenEvent) event;
					if (!btn.isInsideOrNear(pe.x, pe.y)
						&& autoScrollTimer != null) { // guich@tc114_51: must add control's coordinates
						disableAutoScroll();
						break;
					}
				}
	...
	protected void updateValue(int pos) {
		...
		if (dragBarPos == dragBarMax) {
			value = Math.max(0, maximum -
							 visibleItems); // guich@556_7: fixed the correct value, subtracting by visibleItems
		} else {
	...
	private void enableButtons() { // luciana@570_22
		...
		if (!b && (foc == btnDec
				   || foc == btnInc)) { // guich@572_16: only if the focus is one of the buttons - bruno@tc114: avoid auto-setting the focus to a multi-edit
			requestFocus();
	...
	public void onPaint(Graphics g) {
		...
		if (verticalBar) {
			if (uiVista && isEnabled()) {
				g.fillVistaRect(0, dragBarPos, width, dragBarSize, backColor, startDragPos != -1,
								true); // guich@tc110_51: press the bar if dragging.
			} else {
				g.fillRect(0, dragBarPos, width, dragBarSize);
			}
			g.draw3dRect(0, dragBarPos, width, dragBarSize, uiVista ? Graphics.R3D_CHECK : Graphics.R3D_RAISED,
						 false, false,
			...
			if (uiVista && isEnabled()) {
				g.fillVistaRect(dragBarPos, 0, dragBarSize, height, backColor, startDragPos != -1,
								false); // guich@tc110_51: press the bar if dragging.
			} else {
				g.fillRect(dragBarPos, 0, dragBarSize, height);
			}
			g.draw3dRect(dragBarPos, 0, dragBarSize, height, uiVista ? Graphics.R3D_CHECK : Graphics.R3D_RAISED,
						 false, false,
						 fourColors); // guich@tc110_51: press the bar if dragging.
			if (uiFlat || uiVista) {
				g.foreColor = fourColors[2];
	...
	public int getPreferredWidth() {
		return (verticalBar ? btnDec.getPreferredWidth() + extraSize : (btnDec.getPreferredHeight() << 1)) +
			   insets.left
	...
	public int getPreferredHeight() {
		return (verticalBar ? (btnDec.getPreferredWidth() << 1) : btnDec.getPreferredHeight() + extraSize) +
			   insets.top
	...
	protected void onBoundsChanged(boolean screenChanged) {
		...
		btnInc.setRect(verticalBar ? 0 : (size - btnWH), verticalBar ? (size - btnWH) : 0, btnWH, btnWH,
					   null,
					   screenChanged);
