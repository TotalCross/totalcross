...
public class ComboBoxEditable extends ComboBox implements PressListener,
	KeyListener { // guich@tc113_3
	...
	public void specialkeyPressed(KeyEvent ke) {
		if (ke.isUpKey() || ke.isDownKey()) {
			...
			if (Settings.circularNavigation) {
				...
				pop.lb.setSelectedIndex(ke.isDownKey() ? Math.min((idx + 1), pop.lb.itemCount - 1) : Math.max(0,
										idx - 1));
			}
			ke.consumed = true;
			return;
