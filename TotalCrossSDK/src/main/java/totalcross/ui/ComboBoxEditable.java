 ...
 * <pre>
   String[] items =
 {"Ana","Barbara","Raul","Marcelo","Eduardo","Denise","Michelle","Guilherme","Vera","Dulce","Leonardo","Andre","Gustavo","Anne","Renato","Zelia","Helio"};
...
public class ComboBoxEditable
  extends ComboBox implements PressListener, KeyListener
{ // guich@tc113_3
  ...
  public ComboBoxEditable() { this((Object[])null); }

  public ComboBoxEditable(Object[] items) { this(new ListBox(items)); }
   ...
   * @param keepSorted If the list must be sorted after a new item is added by
   *     the autoAdd.
   */
  ...
  @Override protected void drawSelectedItem(Graphics g)
  {
    // do nothing
  }
  ...
  {
    if (edit.getLength() == 0 ||
        !pop.lb.setSelectedItemStartingWith(edit.getText(), true)) {
  ...
  @Override public void actionkeyPressed(KeyEvent e) { setTextFromList(); }

  @Override public void keyPressed(KeyEvent e) {}

  @Override public void specialkeyPressed(KeyEvent ke)
  {
    if (ke.isUpKey() || ke.isDownKey()) {
      int idx = pop.lb.getSelectedIndex();
      if (Settings.circularNavigation) {
        pop.lb.setSelectedIndex(ke.isDownKey()
                                  ? (idx + 1) % pop.lb.itemCount
                                  : idx <= 0 ? pop.lb.itemCount - 1 : idx - 1);
      } else {
        pop.lb.setSelectedIndex(ke.isDownKey()
                                  ? Math.min((idx + 1), pop.lb.itemCount - 1)
                                  : Math.max(0, idx - 1));
      }
      ke.consumed = true;
      return;
