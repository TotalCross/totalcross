...
/**
 * A horizontal flow layout component. Use this layout to place components
 * sequentially in the horizontal axis. For custom placements, use the
 * specialized constructor or the 'setLayout' method. Using this component, you
 * can easily stack, distribute or fill a area with components.
 * @author João, Ygor
 */
public class HBox extends LinearBox
{
  ...
  public HBox() { super(LAYOUT_STACK_LEFT, ALIGNMENT_CENTER); }
  /** Creates a new HBox component with the default layout options */
  public HBox(int mode, int aligment) { super(mode, aligment); }
  ...
  @Override protected void resizeElements(int layout, int alignment)
  {
    ...
    this.internalAdd(
      elements[elements.length - 1], xpos_first, ypos, width, height, null);
    for (int i = elements.length - 2; i >= 0; i--) {
      this.internalAdd(
        elements[i], xpos + spacing, ypos, width, height, elements[i + 1]);
    }
  }
