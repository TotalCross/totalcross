...
/**
 * Base class for HBox and VBox components. Contains most of the functionality
 * needed for these layouts to work. This class should not be used explicitly or
 * inherited.
 * @author João, Ygor
 */
public abstract class LinearBox extends Container
{
  ...
  /**
   * Distribute and scale each element to fill the entire width of this
   * component
   */
  ...
  public LinearBox() { this(LAYOUT_STACK_CENTER, ALIGNMENT_CENTER); }
   ...
   */
  @Override public void add(Control control, int w, int h)
  {
  ...
  {
    if (this.suspendLayout) { // If the user manually asked to suspend layout,
                              // do not interfere
  ...
  /**
   * This layout does not supports this action. Use the add(control) or
   * add(control, w, h) instead.
   */
  @Override public void add(Control control, int x, int y, int w, int h)
  {
    throw new RuntimeException(
      "This layout does not supports this action. Use the add(control), add(control, w, h) or pin(control, x, y, w, h) instead.");
  }
  /**
   * This layout does not supports this action. Use the add(control) or
   * add(control, w, h) instead.
  ...
  /**
   * Suspends all layout operations from 'add' calls until a 'resumeLayout'
   * call.
   */
  public void suspendLayout() { this.suspendLayout = true; }
  ...
  public void resumeLayout()
  {
    if (!this.suspendLayout) {
      throw new RuntimeException(
        "Calling 'resumeLayout' without first calling 'suspendLayout'");
    }
