...
/**
 * A vertical flow layout component. Use this layout to place components sequentially
...
public class VBox extends LinearBox {
	...
	public VBox() {
		super(LAYOUT_STACK_TOP, ALIGNMENT_CENTER);
	}
	/** Creates a new VBox component with the given layout options */
	public VBox(int mode, int aligment) {
		super(mode, aligment);
	}
	...
	protected void resizeElements(int layout, int alignment) {
		...
		this.internalAdd(elements[elements.length - 1], xpos, ypos_first, width, height, null);
		for (int i = elements.length - 2; i >= 0; i--) {
			this.internalAdd(elements[i], xpos, ypos + spacing, width, height, elements[i + 1]);
		}
	}
