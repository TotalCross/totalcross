 ...
 */
public class Switch
  extends Control implements PathAnimation.SetPosition, AnimationFinished
{
  ...
  /**
   * Text to draw when on background or foreground, and when this switch is on
   * or off
   */
  public String textBackOn, textForeOn, textBackOff, textForeOff;

  /**
   * Text color to draw when on background or foreground, and when this switch
   * is on or off
   */
  ...
  /**
   * Constructs a switch of the given type. In material UI, there is only one
   * type
   */
  ...
  /**
   * Returns true if this switch is ON. Note that, if this Switch has
   * animations, you must call this method only after the animation finishes
   */
  public boolean isOn() { return dragBarPos + dragBarSize / 2 >= width / 2; }
      ...
      if (dragBarPos != destPos) {
        try {
          PathAnimation p = PathAnimation.create(
            this, dragBarPos, 0, destPos, 0, this, ANIMATION_TIME);
  ...
  @Override public void onPaint(Graphics g)
  {
    try {
      ...
      if (on && textBackOn != null) { // text at left
        ...
        g.drawText(textBackOn,
                   centerText ? (width - ww) / 2
                              : (width - dragBarSize - ww) / 2,
                   (height - fmH) / 2);
      } else if (!on && textBackOff != null) { // text at right
        g.foreColor = colorBackOff;
        int ww = fm.stringWidth(textBackOn);
        g.drawText(textBackOff,
                   centerText ? (width - ww) / 2
                              : (width - dragBarSize - ww) / 2 + dragBarSize,
                   (height - fmH) / 2);
      }
      // ball
      if (!uiMaterial) {
      ...
      if (on && textForeOn != null) { // text at left
        ...
        g.drawText(textForeOn,
                   dragBarPos + (dragBarSize - fm.stringWidth(textForeOn)) / 2,
                   (height - fmH) / 2);
      } else if (!on && textForeOff != null) { // text at right
        g.foreColor = colorForeOff;
        g.drawText(textForeOff,
                   dragBarPos + (dragBarSize - fm.stringWidth(textForeOff)) / 2,
                   (height - fmH) / 2);
      }
    } catch (Throwable t) {
      t.printStackTrace();
  ...
  private void buildBall() throws Exception
  {
    ...
    ballOn =
      (uiMaterial ? Resources.switchBtn
                  : isIos ? Resources.switchBtnIos : Resources.switchBtnAnd)
  ...
  private void buildBar(boolean on) throws Exception
  {
    ...
    int barY = (height - h) / 2;
    int fillB =
      !isEnabled() ? Color.interpolate(backColor, parent.backColor) : backColor;
