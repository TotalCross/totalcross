...
public class Switch extends Control implements PathAnimation.SetPosition, AnimationFinished {
	...
	public void onPaint(Graphics g) {
		try {
			...
			if (on && textBackOn != null) { // text at left
				...
				g.drawText(textBackOn, centerText ? (width - ww) / 2 : (width - dragBarSize - ww) / 2,
						   (height - fmH) / 2);
			} else if (!on && textBackOff != null) { // text at right
				g.foreColor = colorBackOff;
				int ww = fm.stringWidth(textBackOn);
				g.drawText(textBackOff, centerText ? (width - ww) / 2 : (width - dragBarSize - ww) / 2 +
						   dragBarSize,
						   (height - fmH) / 2);
			}
			// ball
			...
			if (on && textForeOn != null) { // text at left
				...
				g.drawText(textForeOn, dragBarPos + (dragBarSize - fm.stringWidth(textForeOn)) / 2,
						   (height - fmH) / 2);
			} else if (!on && textForeOff != null) { // text at right
				g.foreColor = colorForeOff;
				g.drawText(textForeOff, dragBarPos + (dragBarSize - fm.stringWidth(textForeOff)) / 2,
						   (height - fmH) / 2);
			}
		} catch (Throwable t) {
			t.printStackTrace();
	...
	private void buildBall() throws Exception {
		...
		ballOn = (uiMaterial ? Resources.switchBtn : isIos ? Resources.switchBtnIos :
				  Resources.switchBtnAnd)
