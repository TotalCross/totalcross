...
/**
  A control that can show an image bigger than its area and that can be
  dragged using a pen to show the hidden parts. Note that, by default, events
  (and dragging) are disabled. You must call setEventsEnabled to allow dragging.
 */
...
public class ImageControl extends Control
{
   ...
   * Note: starting on TotalCross 3.1, the border is drawn around the color and
   * no longer around the image, because it was not working on OpenGL devices.
   */
  ...
  /**
   * Constructs with no initial image. You must set the image with the setImage
   * method.
   */
  public ImageControl()
  {
  ...
  /**
   * Pass true to enable this ImageControl to send Press events. Note that it
   * will disable drag and resize of the image.
   */
  ...
  /**
   * Sets the image to the given one. If the image size is different, you must
   * explicitly call setRect again if you want to resize the control.
   */
  public void setImage(Image img) { setImage(img, true); }

  /**
   * Sets the image to the given one, optionally resetting the image position.
   * If the image size is different, you must explicitly call setRect again if
  ...
  @Override public void onEvent(Event event)
  {
    ...
    if (event instanceof SizeChangeEvent && !event.consumed) {
      if (target != null) {
        ...
        scaleImage();
        if (centerImage && img != null) { // guich@100_1: reset the image's
                                          // position if bounds changed
          lastX = (width - getImageWidth()) / 2;
          lastY = (height - getImageHeight()) / 2;
        } else {
          lastX = lastY = 0;
    ...
    PenEvent pe;
    if (isPressedEventsEnabled && event.type == PenEvent.PEN_UP &&
        !hadParentScrolled() &&
      ...
      switch (event.type) {
        ...
        case PenEvent.PEN_DOWN:
          pe = (PenEvent)event;
          startX =
            pe.x - lastX; // save the start relative to the last point clicked
          startY = pe.y - lastY;
          break;
        case PenEvent.PEN_DRAG:
          if (getImageWidth() > this.width || getImageHeight() > this.height ||
              allowBeyondLimits) {
            pe = (PenEvent)event;
  ...
  /**
   * Moves to the given coordinates, respecting the current moving policy
   * regarding <i>allowBeyondLimits</i>.
    ...
    if (allowBeyondLimits) {
      ...
      if (getImageWidth() > width) {
        lastX = Math.max(
          width - getImageWidth(),
          Math.min(newX,
                   0)); // don't let it move the image beyond its bounds
      }
  ...
  @Override public void onPaint(Graphics g) { paint(g, true); }

  private void paint(Graphics g, boolean drawBack)
  {
    g.backColor =
      isEnabled() ? backColor : Color.interpolate(backColor, parent.backColor);
  ...
  private void drawImage(Graphics g, boolean isBack)
  {
    ...
    if (!isBack &&
        tempHwScale !=
          NOTEMP) { // scale the original image instead of the resized one
    ...
    if (tempHwScale != NOTEMP) {
      try {
         ...
         * However hwScale is not working correctly on totalcross skia version
         * yet. This might consumes more memory to scale
         */
  ...
  public int getImageWidth()
  {
    return img == null
      ? 0
      : tempHwScale != NOTEMP ? (int)(img.getWidth() * tempHwScale)
                              : img.getWidth();
  }
  ...
  public int getImageHeight()
  {
    return img == null
      ? 0
      : tempHwScale != NOTEMP ? (int)(img.getHeight() * tempHwScale)
                              : img.getHeight();
  }
  ...
  {
    return img != null
      ? Math.min(getImageWidth(), Settings.screenWidth)
      : imgBack != null ? imgBack.getWidth()
                        : Settings.screenWidth; // guich@tc115_35
  }

  @Override public int getPreferredHeight()
  {
    return img != null
      ? Math.min(getImageHeight(), Settings.screenHeight)
      : imgBack != null ? imgBack.getHeight()
                        : Settings.screenHeight; // guich@tc115_35
  }
  ...
  /**
   * Returns the original image assigned to this ImageControl. Note that
   * getImage() returns the image scaled, while getOriginalImage() returns the
   * unscaled image.
   */
  public Image getOriginalImage() { return img0; }

  /** Sets the given image as a freezed background of this image control. */
  public void setBackground(Image img) { imgBack = img; }

  /** Returns the background image set with setBackground */
  public Image getBackground() { return imgBack; }

  /**
   * Gets an image representing the portion being shown. If all image is being
   * shown, returns the currently assigned image.
   */
  public Image getVisibleImage(boolean includeBackground) throws ImageException
  {
    ...
    rArea.x = rArea.y = 0;
    Image ret = img;
    if (!rArea.contains(rImg.x, rImg.y) ||
        !rArea.contains(rImg.x2(), rImg.y2())) {
