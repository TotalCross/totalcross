...
public class ImageControl extends Control {
	...
	public boolean moveTo(int newX, int newY) {
		...
		if (allowBeyondLimits) {
			...
			if (getImageWidth() > width) {
				lastX = Math.max(width - getImageWidth(), Math.min(newX,
	...
	public int getImageWidth() {
		return img == null ? 0 : tempHwScale != NOTEMP ? (int)(img.getWidth() * tempHwScale) :
			   img.getWidth();
	}

	/** Returns the image's height; when scaling, returns the scaled height. */
	public int getImageHeight() {
		return img == null ? 0 : tempHwScale != NOTEMP ? (int)(img.getHeight() * tempHwScale) :
			   img.getHeight();
	}
