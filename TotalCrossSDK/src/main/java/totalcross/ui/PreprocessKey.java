...
/**
 Class used to customize the way that Edit and MultiEdit handle keys. Useful,
 for example,
 * to handle devices with special keypads and translate them into standard
 characters.
 *
 * You have to extend the preprocess method and change the key to one you may
 want to use.
 * Then, assign an instance of the class to the <code>instance</code> member,
 and the Edit and MultiEdit will use them
 *
 * It has two handy methods, to be used to map char sequences. Below, an
 implementation for the the MC67.
 * <pre>
   PreprocessKey.instance =
      new PreprocessKey()
      {
         ...
         {
            if (e.type == KeyEvent.KEY_PRESS && (Settings.onJavaSE ||
 (e.modifiers & SpecialKeys.SYSTEM) != 0)) // use only if key came from physical
 keyboard
            {
...
public abstract class PreprocessKey
{
  ...
  /**
   * Maximum millis delay to consider that the user is repeating the current
   * key
   */
  ...
  protected char[] getKeySet(Control target, KeyEvent e) { return null; }
