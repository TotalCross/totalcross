...
public class ShadedButton extends Button
{
  ...
  @Override public void onBoundsChanged(boolean changed)
  {
    super.onBoundsChanged(changed);
    if (width > 0) {
      try {
        back =
          back0.getSmoothScaledInstance(width - shadeDist, height - shadeDist);
        fore =
          back0.getSmoothScaledInstance(width - shadeDist, height - shadeDist);
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    g.foreColor = fcolor;
    g.drawText(text,
               (width - fm.stringWidth(text) - shadeDist) / 2 + k,
               (height - fmH) / 2 - shadeDist + k);
  }
}
