...
/**
 * This class represents a velocimeter gauge. The background and pointer can be
 * customized. The text, max and min values can be drawn or not. The pointer's
...
public class Velocimeter extends Container
{
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    try {
      ...
      if (drawValue) {
        String s =
          String.valueOf(value > max ? max : value < min ? min : value);
