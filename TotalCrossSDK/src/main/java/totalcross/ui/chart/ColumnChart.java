...
public class ColumnChart extends Chart
{
   ...
   */
  public ColumnChart(String[] categories) { this(0, categories); }
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    for (int i = 0; i < cCount; i++) { // for each category
      int x = getXValuePos(i) + colS +
              (forward ? 0 : colW * (sCount - 1) - perspectiveH);
      for (int j = forward ? 0 : sCount - 1; forward ? (j < sCount) : (j >= 0);
           j += (forward ? 1 : -1),
               x += (forward ? colW : -colW),
               col++) { // for each series
        ...
        if ((type & (GRADIENT_HORIZONTAL | GRADIENT_VERTICAL)) != 0) {
          int fade = (type & GRADIENT_DARK) != 0 ? Color.darker(c, 128)
                                                 : Color.brighter(c, 128);
          ...
          g.fillShadedRect(
            ...
            c,
            fade,
            100); // note: original method was drawRoundGradient, which draws
                  // from c2 to c1, that's why we use !invert here
        } else {
          g.backColor = c;
    ...
    if (selection >= 0) { // there is a selection
      ...
      String text = Convert.toCurrencyString(
        ((Series)series.items[selection % sCount]).yValues[selection / sCount],
        xDecimalPlaces);
      ...
      for (int i = 0; i < numCols; i++) {
        ...
        String text = Convert.toCurrencyString(
          ((Series)series.items[z]).yValues[j], xDecimalPlaces);
        drawTextBox(g, r.x, r.y, text);
        z++;
      }
    }
    ...
    switch (e.type) {
      case PenEvent.PEN_UP:
        if (!hadParentScrolled()) {
          ...
          if (pe.x >= xAxisX1 && pe.x <= xAxisX2 && pe.y >= yAxisY2 &&
              pe.y <= yAxisY1) {
            selection = -1;
            ...
            if (i < len) {
              if ((type & IS_3D) != 0 &&
                  i < len - 1) { // check overlaping with next column if 3D
