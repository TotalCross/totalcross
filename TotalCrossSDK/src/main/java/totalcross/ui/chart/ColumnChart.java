...
public class ColumnChart extends Chart {
	...
	public void onPaint(Graphics g) {
		...
		for (int i = 0; i < cCount; i++) { // for each category
			...
			for (int j = forward ? 0 : sCount - 1; forward ? (j < sCount)
				...
				if ((type & (GRADIENT_HORIZONTAL | GRADIENT_VERTICAL)) != 0) {
					...
					g.fillShadedRect(x, y, colW, yAxisY1 - y, !invertGradient, !vertical, c, fade,
									 100); // note: original method was drawRoundGradient, which draws from c2 to c1, that's why we use !invert here
				} else {
					g.backColor = c;
		...
		if (selection >= 0) { // there is a selection
			...
			String text = Convert.toCurrencyString(((Series) series.items[selection %
													sCount]).yValues[selection / sCount],
												   xDecimalPlaces);
