 ...
 * on the series values. If the user clicks on the slice, a popup shows the
 * corresponding value. By setting the legendValueSuffix to "%", the value
 * displayed will be the percentage instead of the serie's value.
 */

public class PieChart extends Chart
{
  /**
   * Specifies the distance that the selected pie will be placed from the rest
   * of the pie. Defaults to fmH.
   */
  public int distanceOfSelectedPie = Font.NORMAL_SIZE;
  /** Specifies the selected pie. */
  public int selectedSeries = -1;
  /**
   * The suffix used in the legend to display the values. E.G.: "%". Defaults
   * to blank.
   */
  ...
  /**
   * GAO: if true, then offset selected pie slice, for visual indicator that
   * its been selected
   */
   ...
   * The yDecimalPlaces defines the number of decimal places used to display the
   * value in the legend.
   */
  public PieChart()
  {
  ...
  @Override protected void getCustomInsets(Insets r)
  {
    if ((type & IS_3D) != 0) {
      if (perspectiveV > 0) {
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (showValuesOnLegend &&
        (legendValues == null || legendValues.length != sCount)) {
    ...
    for (int i = 0; i < sCount; i++) { // for each series
      ...
      if (showValuesOnLegend) {
        legendValues[i] =
          " " + Convert.toCurrencyString(v, yDecimalPlaces) + legendValueSuffix;
      }
    }
    ...
    int rr =
      Math.min(clientRect.width, clientRect.height) / 2 - distanceOfSelectedPie;
    if (rr > 0) {
      this.rr = rr;
      if ((type & IS_3D) != 0) {
  ...
  private void drawPie(Graphics g, int xx, int yy, int rr, boolean is3d)
  {
    ...
    for (int i = 0; i < sCount; i++) { // for each series
      // juliana@268: it is necessary to save the old positions to correctly
      // offset the selected pie.
      xx = this.xx;
      yy = this.yy;

      Series s = (Series)series.items[i];
      int color = i == currentSelection
                    ? Color.darker(s.color, 32)
                    : is3d ? Color.darker(s.color) : s.color;
      ...
      if (i == selectedSeries) {
        ...
        g.getAnglePoint(
          xx, yy, distanceOfSelectedPie, distanceOfSelectedPie, half, c);
        xx = c.x;
        yy = c.y;
      }
      if (last == current) {
        ...
        int fade = (type & GRADIENT_DARK) != 0 ? Color.darker(color, 128)
                                               : Color.brighter(color, 128);
        g.backColor =
          is3d ? color : ((type & GRADIENT_INVERT) != 0) ? color : fade;
        g.foreColor =
          is3d ? g.backColor : ((type & GRADIENT_INVERT) != 0) ? fade : color;
        ...
        g.foreColor = is3d || sum == v
                        ? color
                        : 0; // fixed color when only 1 serie has value > 0
        g.backColor = color;
      ...
      if (showTextBox) {
        String text =
          Convert.toCurrencyString(legendValueSuffix.indexOf('%') >= 0
                                     ? (s.yValues[0] / sum * 100)
                                     : s.yValues[0],
                                   yDecimalPlaces) +
          legendValueSuffix;
              ...
              if (last <= degree && degree <= current) {
                ...
                if (r < distance) {
                  ...
                  if (offsetSelectedSlice) {
                    this.selectedSeries =
                      i; // make the slice user tapped on be the 'selected' one
                  }
                  selectedSlice = i; // field so after receiving a Pie event,
                                     // can retrieve selected slice
                }
                break;
              }
              last = current;
  ...
  {
    if (!showTextBox) {
      tip.setText(Convert.toCurrencyString(legendValueSuffix.indexOf('%') >= 0
                                             ? (s.yValues[0] / sum * 100)
                                             : s.yValues[0],
                                           yDecimalPlaces) +
                  legendValueSuffix);
  ...
  @Override public void onFontChanged() { tip.setFont(this.font); }
}
