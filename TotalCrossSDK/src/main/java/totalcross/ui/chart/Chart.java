...
public class Chart extends Control {
	...
	protected double yAxisMaxValue =
		100; // guich@tc100b5_56: initialize to something if the user don't call setYAxis
	...
	protected boolean draw(Graphics g) {
		...
		if (drawLegend) {
			sMaxLen += lr ? fm.getMaxWidth(seriesNames, 0,
										   sCount) : (sqWH + 4) * sCount + getTotalSize(seriesNames);
		...
		for (int i = 0; i <= ySteps; i++, val += incY) {
			...
			if (i == 0 && !showFirstYValue) {
				...
				if (showYValues) {
					...
					g.foreColor = axisText;
					g.drawText(s, xAxisX1 - fm.stringWidth(s) - 3, pos - fmH / 2, textShadowColor != -1,
							   textShadowColor);
					g.foreColor = axisForeColor;
				}
			}
		...
		if (drawTitle) {
			...
			g.setFont(fb);
			g.drawText(title, (width - fb.fm.stringWidth(title)) / 2, 0, textShadowColor != -1,
					   textShadowColor);
		...
		if (drawLegend) {
			...
			for (int i = 0; i < sCount; i++) {
				...
				g.drawText(s, x + halfBlankWidth + sqWH + halfBlankWidth, y, textShadowColor != -1,
						   textShadowColor);
				if (lr) {
					y += fmH;
				} else {
	...
	public int getXValuePos(double value) {
		return xAxisX1 + (int) Math.round((value - xAxisMinValue) / (xAxisMaxValue - xAxisMinValue) *
										  (xAxisX2 - xAxisX1));
	}
	...
	public int getYValuePos(double value) {
		return yAxisY1 - (int) Math.round((value - yAxisMinValue) / (yAxisMaxValue - yAxisMinValue) *
										  (yAxisY1 - yAxisY2));
	}
}
