...
{
  /**
   * Sample gray color to be used in the chart, taken from <a
   * href='http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization'>here</a>.
   */
  public static final int COLOR1 = 0x4D4D4D; // (gray)
  /**
   * Sample blue color to be used in the chart, taken from <a
   * href='http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization'>here</a>.
   */
  public static final int COLOR2 = 0x5DA5DA; // (blue)
  /**
   * Sample orange color to be used in the chart, taken from <a
   * href='http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization'>here</a>.
   */
  public static final int COLOR3 = 0xFAA43A; // (orange)
  /**
   * Sample green color to be used in the chart, taken from <a
   * href='http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization'>here</a>.
   */
  public static final int COLOR4 = 0x60BD68; // (green)
  /**
   * Sample pink color to be used in the chart, taken from <a
   * href='http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization'>here</a>.
   */
  public static final int COLOR5 = 0xF17CB0; // (pink)
  /**
   * Sample brown color to be used in the chart, taken from <a
   * href='http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization'>here</a>.
   */
  public static final int COLOR6 = 0xB2912F; // (brown)
  /**
   * Sample purple color to be used in the chart, taken from <a
   * href='http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization'>here</a>.
   */
  public static final int COLOR7 = 0xB276B2; // (purple)
  /**
   * Sample yellow color to be used in the chart, taken from <a
   * href='http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization'>here</a>.
   */
  public static final int COLOR8 = 0xDECF3F; // (yellow)
  /**
   * Sample red color to be used in the chart, taken from <a
   * href='http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization'>here</a>.
   */
  ...
  /**
   * The chart type. The values can be OR'ed together. It applies to Column and
   * Pie charts. For example: <pre> chart.type = Chart.IS_3D |
   * Chart.GRADIENT_HORIZONTAL | Chart.GRADIENT_DARK;
   * </pre>
   * @see #IS_3D
  ...
  protected double yAxisMaxValue =
    100; // guich@tc100b5_56: initialize to something if the user don't call
         // setYAxis
  ...
  /**
   * Flag to indicate whether the categories must be painted on the next
   * tick.<br> The field {@link #showCategories} must also be set to true.<br>
   * <br>
  ...
  /**
   * The number of decimal places to show the data of the X axis. Defaults
   * to 3.
   */
  public int xDecimalPlaces = 3;

  /**
   * The number of decimal places to show the data of the Y axis. Defaults
   * to 3.
   */
  public int yDecimalPlaces = 3;

  /**
   * The position of the legend. Can be one of: RIGHT (default), LEFT, TOP,
   * BOTTOM.
   */
  ...
  protected boolean draw(Graphics g)
  {
    ...
    if (drawLegend) {
      sMaxLen += lr ? fm.getMaxWidth(seriesNames, 0, sCount)
                    : (sqWH + 4) * sCount + getTotalSize(seriesNames);
      if (legendValues != null) {
        sMaxLen += lr ? fm.getMaxWidth(legendValues, 0, sCount)
                      : getTotalSize(legendValues);
      }
      int ww = sMaxLen + 32;
      if (is3d) {
    ...
    if (showYValues) {
      ...
      for (double v = yAxisMinValue; v <= yAxisMaxValue; v += incY) {
        yvalW = Math.max(
          yvalW, fm.stringWidth(Convert.toCurrencyString(v, yDecimalPlaces)));
      }
      left += yvalW + 3;
    ...
    if (!onlyShowCategories &&
        (yAxisY2 < 0 || yAxisY2 >= height || yAxisY2 >= yAxisY1)) {
      return false;
    }
    ...
    if (fillColor2 != -1) {
      ...
      for (int j = 1, n = xAxisSteps; j <= n;
           j += 2, x0 += inc * use2ndColorEveryXColumns * 2) {
        g.fillRect(xx = getXValuePos(x0),
                   yAxisY2,
                   getXValuePos(x0 + inc * use2ndColorEveryXColumns) - xx,
                   yAxisY1 - yAxisY2);
      }
    }
    val = xAxisMinValue;
    ...
    for (int i = 0; i <= xAxisSteps; i++, val += inc) {
      ...
      if (drawCategories && i < xAxisCategories.length) { // draw category
        final int tW =
          (xAxisCategories[i] == null || xAxisCategories[i].length() == 0)
            ? 0
        ...
        if (tW > 0) {
          if (showCategoriesOnTick) {
            ...
            if ("|".equals(xAxisCategories[i])) {
              ...
              g.drawText(xAxisCategories[i],
                         p,
                         onlyShowCategories ? (height - fmH) / 2 : yAxisY1,
                         textShadowColor != -1,
                         textShadowColor);
              g.foreColor = axisForeColor;
              lastPos = p + tW + fmH;
    ...
    while (ySteps > 1 && showYValues &&
           (getYValuePos(incY) - getYValuePos(incY * 2)) < fm.ascent) {
      ySteps--;
    ...
    if (drawTitle) {
      ...
      g.setFont(fb);
      g.drawText(title,
                 (width - fb.fm.stringWidth(title)) / 2,
                 0,
                 textShadowColor != -1,
                 textShadowColor);
    ...
    if (drawLegend) {
      ...
      if (lr) {
        ...
        if (legendPosition == TOP) {
          ...
          y =
            this.height - (fmH + 2 + (is3d ? Math.abs(legendPerspective) : 0));
        }
      }
      ...
      if (is3d) {
        ...
        g.fillRect(
          x + legendPerspective, y + Math.abs(legendPerspective), w, h);
      }
      g.backColor = backColor;
      g.fillRect(x, y, w, h);
      g.drawRect(x, y, w, h);
      ...
      for (int i = 0; i < sCount; i++) {
        ...
        if (se.dot != null) {
          if (se.legendDot == null) {
            try {
              se.legendDot = se.dot.smoothScaledFixedAspectRatio(
                sqWH - halfBlankWidth, true);
            } catch (Exception e) {
              se.legendDot = se.dot;
            }
          }
        ...
        g.drawText(s,
                   x + halfBlankWidth + sqWH + halfBlankWidth,
                   y,
                   textShadowColor != -1,
                   textShadowColor);
        if (lr) {
    ...
    clientRect.set(
      left, top, this.width - right - left, this.height - top - bottom);
  ...
  protected void drawTextBox(Graphics g, int refX, int refY, String text)
  {
    ...
    g.drawText(
      text, boxX + 2, boxY + 1, textShadowColor != -1, textShadowColor);
  }
  ...
  {
    return xAxisX1 +
      (int)Math.round((value - xAxisMinValue) /
                      (xAxisMaxValue - xAxisMinValue) * (xAxisX2 - xAxisX1));
  }
  ...
  {
    return yAxisY1 -
      (int)Math.round((value - yAxisMinValue) /
                      (yAxisMaxValue - yAxisMinValue) * (yAxisY1 - yAxisY2));
  }
}
