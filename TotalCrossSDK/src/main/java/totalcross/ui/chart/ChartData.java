...
/**
    The ChartData class represents a table with data that would be displayed in
   the chart. Here's a sample: <pre> double[] xAxis = new double[0]; double[]
   pressureyAxis = new double[0]; Series pressureSeries = new Series("Pression",
   xAxis, pressureyAxis, Color.BLUE); int cols = 24, rows = 5; XYChart chart =
   new XYChart(); chart.showHGrids = chart.showVGrids = true; // Shows grids.
      chart.showTitle = true; // Shows title.
      chart.showYValues = true; // Shows the Y axis values.
      chart.yDecimalPlaces = 0; // No decimal places.
      chart.setXAxis(0, 240, 24); // The X axis is time in minutes. It has an
   interval of 5 minutes and a maximum of 2 hours. chart.setYAxis(0, 240, 24);
   // The Y axis is different for each graph.
...
public class ChartData extends Container
{
  ...
  /**
   * The height of the cell when using PREFERRED, defined as a % of the
   * control's font height. Default value is 100(%).
   */
   ...
   */
  public ChartData(Chart chart) { this.chart = chart; }
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (rowsLength > 0) {
      ...
      if (fillColor2 != -1) {
        ...
        // vertical lines
        for (int j = 1, n = getRow(0).columns.length; j <= n;
             j += 2, x0 += inc * use2ndColorEveryXColumns * 2) {
          xx = chart.getXValuePos(x0);
          g.fillRect(xx,
                     0,
                     chart.getXValuePos(x0 + inc * use2ndColorEveryXColumns) -
                       xx,
                     height);
        }
      }
      ...
      if (lineColor != -1) {
        ...
        // vertical lines
        for (int j = fillColor2 != -1 ? 1 : 0, n = getRow(0).columns.length;
             j <= n;
             j++, x0 += inc) {
        ...
        yy = 0;
        int hh = 0;
        for (int i = snapToBottom ? 1 : 0, n = rowsLength; i <= n;
             i++, yy += hh) { // horizontal lines
    ...
    switch (e.type) {
      case PenEvent.PEN_UP:
        if (isEnabled() && !hadParentScrolled()) {
          ...
          int rowsLength = rows.size();
          if (rowsLength > 0 && chart.xAxisX1 <= pex && pex <= chart.xAxisX2 &&
              chart.yAxisY2 <= pey && pey <= chart.yAxisY1) {
            int ystep = this.height / rowsLength;
            int yxtra = this.height % ystep;
            selectedCol = (pex - chart.xAxisX1) / chart.columnW;
            for (int i = 0, hh, yy = chart.yAxisY2; i <= rowsLength;
                 i++, yy += hh) { // horizontal lines
              hh = ystep;
              if (i < yxtra) {
                hh++;
