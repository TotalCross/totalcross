  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (!showLines || showPoints || hasFocus || Settings.fingerTouch) {
      ...
      if (selectedSeries != -1 && showValue) {
        ...
        String text =
          Convert.toCurrencyString(s.yValues[selectedValue], yDecimalPlaces);
        if (s.xValues != null) {
          text =
            "(" +
            Convert.toCurrencyString(s.xValues[selectedValue], xDecimalPlaces) +
            "," + text + ")";
        }

        Coord c =
          (Coord)((Vector)points.items[selectedSeries]).items[selectedValue];
        ...
        for (int i = 0; i < numCols; i++) {
          ...
          if (s.xValues != null) {
            text = "(" +
                   Convert.toCurrencyString(s.xValues[z], xDecimalPlaces) +
                   "," + text + ")";
          }
  ...
  @Override public void onEvent(Event e)
  {
    switch (e.type) {
      ...
      case PenEvent.PEN_DOWN: {
        ...
        if (xx < (xAxisX1 - pointR) || xx > (xAxisX2 + pointR) ||
            yy > (yAxisY1 + pointR) || yy < (yAxisY2 - pointR)) {
          hasFocus = false;
          selectedSeries = -1;
          selectedValue = -1;
      ...
      case KeyEvent.SPECIAL_KEY_PRESS: {
        ...
        if (ke.isActionKey()) { // release focus
          ...
          if (selectedValue <
              ((Series)series.items[selectedSeries]).yValues.length - 1) {
            selectedValue++;
            Window.needsPaint = true;
