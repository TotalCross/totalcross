...
public class ArcChart extends Chart {
	...
	private void drawPie(Graphics g, int xx, int yy, int rr, boolean is3d) {
		...
		for (int i = 0; i < sCount; i++) { // for each series
			...
			int color = i == currentSelection ? Color.darker(s.color,
						32) : is3d ? Color.darker(s.color) : s.color;
			...
			if (showTextBox) {
				String text = Convert.toCurrencyString(legendValueSuffix.indexOf('%') >= 0 ?
													   (s.yValues[0] / sum * 100) : s.yValues[0],
	...
	private void setTipText(Series s) {
		if (!showTextBox) {
			tip.setText(
				Convert.toCurrencyString(legendValueSuffix.indexOf('%') >= 0 ? (s.yValues[0] / sum * 100) :
										 s.yValues[0],
										 yDecimalPlaces) + legendValueSuffix);
