 ...
 * </pre>
 * If you don't add at least one image before calling add/setRect, you must
 * compute the preferred size yourself.
 */
...
{ // guich@tc126_2
  ...
  public ImageList(Object[] items) { super(items); }
  ...
  @Override protected void drawCursor(Graphics g, int sel, boolean on)
  {
    if (offset <= sel && sel < visibleItems + offset && sel < itemCount) {
      ...
      dx +=
        xOffset; // guich@552_24: added this to make scroll apply to the item
      dy += (sel - offset) * ih;
      if (on) {
  ...
  @Override public int getPreferredWidth()
  {
    ...
    return max + (simpleBorder ? 4 : 6) +
      (sbar.visible ? sbar.getPreferredWidth() : 0) + insets.left +
      insets.right;
  }
  ...
  @Override protected int getItemHeight(int index)
  {
    if (itemCount == 0) {
      return 0;
    }
