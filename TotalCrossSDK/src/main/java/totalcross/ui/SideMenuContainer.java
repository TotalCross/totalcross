...
public class SideMenuContainer extends Container implements PenListener {
	...
	public SideMenuContainer(int direction, String title, Control... items) {
		...
		if (direction == BOTTOM || direction == TOP) {
			...
			topMenu.widthInPixels = (int)(Math.min(320,
												   screenWidthInDp - BAR_HEIGHT_IN_DP) * Settings.screenDensity);
		}
	...
	public void initUI() {
		...
		add(content = new Container(), LEFT, direction == BOTTOM ? TOP : AFTER, FILL,
	...
	public static class Item extends TopMenu.Item {
		...
		public Item(String caption, IconType icon, int iconColor, boolean showTitle,
	...
	public void penDrag(DragEvent e) {
		...
		if (direction == RIGHT && e.direction == DragEvent.LEFT && e.xTotal < -150
	...
	public static class Sub extends ClippedContainer implements PathAnimation.SetPosition,
		AnimationFinished {
		...
		public void initUI() {
			...
			if (items != null) {
				for (Control control : items) {
					...
					add(control, (int)((Settings.screenWidth < 320 ? 4 : 8) * Settings.screenDensity), AFTER, FILL,
						DP + 48);
				}
			}
		}
		...
		public void collapse(boolean showAnimation) {
			if (showAnimation) {
				PathAnimation p = PathAnimation.create(this, 0, this.height, 0, getPreferredHeight(), this,
													   ANIMATION_TIME);
				p.useOffscreen = false;
				p.setpos = this;
				showUIErrorsOld = Settings.showUIErrors;
