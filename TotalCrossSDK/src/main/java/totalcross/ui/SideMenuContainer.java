...
public class SideMenuContainer extends Container implements PenListener
{
  ...
  public SideMenuContainer(int direction, String title, Control... items)
  {
    ...
    bar.setFont(medium != null ? medium
                               : Font.getFont(bar.getFont().name, false, 20));
    ...
    topMenu = new TopMenu(
      items, direction, title != null ? Window.RECT_BORDER : Window.NO_BORDER);
    ...
    if (direction == BOTTOM || direction == TOP) {
      ...
      int screenWidthInDp =
        (int)(Settings.screenWidth / Settings.screenDensity);
      topMenu.widthInPixels =
        (int)(Math.min(320, screenWidthInDp - BAR_HEIGHT_IN_DP) *
  ...
  @Override public void initUI()
  {
    ...
    add(content = new Container(),
        LEFT,
        direction == BOTTOM ? TOP : AFTER,
        FILL,
        direction == BOTTOM ? FIT : FILL);
  ...
  public void setBarFont(Font f) { bar.setFont(f); }
  ...
  @Override public void setForeColor(int color)
  {
  ...
  @Override public void setBackColor(int color)
  {
  ...
  public static class Item extends TopMenu.Item
  {
    ...
    public Item(String caption,
                Image icon,
                final Presenter<Container> presenter)
    {
      super(caption, icon);
                ...
                final Presenter<Container> presenter)
    {
      super(caption, new Container() {
        @Override public void initUI()
        {
          // 16dp left spacing -
          // https://material.io/guidelines/patterns/navigation-drawer.html#navigation-drawer-specs
          this.setInsets((int)((Settings.screenWidth < 320 ? 8 : 16) *
                               Settings.screenDensity),
                         0,
                         0,
                         0);
  ...
  {
    double margin = 0.20;
    if (direction == LEFT && e.direction == DragEvent.RIGHT && e.xTotal > 150 &&
        (e.x - e.xTotal) < width * (margin)) {
      topMenu.popup();
    }
    if (direction == TOP && e.direction == DragEvent.DOWN && e.yTotal > 150 &&
        (e.y - e.yTotal) < height * (margin)) {
      topMenu.popup();
    }
    if (direction == RIGHT && e.direction == DragEvent.LEFT &&
        e.xTotal < -150 && (e.x - e.xTotal) > width * (1 - margin)) {
      topMenu.popup();
    }
    if (direction == BOTTOM && e.direction == DragEvent.UP && e.yTotal < -150 &&
  ...
  public static class Sub extends ClippedContainer
    implements PathAnimation.SetPosition, AnimationFinished
  {
    ...
    @Override public void initUI()
    {
      ...
      if (items != null) {
        for (Control control : items) {
          ...
          add(control,
              (int)((Settings.screenWidth < 320 ? 4 : 8) *
                    Settings.screenDensity),
              AFTER,
              FILL,
              DP + 48);
    ...
    private int getMaxHeight()
    {
      ...
      return maxH +
        minH; // use the distance of the first container as a gap at bottom
    }
    ...
    public void expand(boolean showAnimation)
    {
      ...
      if (showAnimation) {
        PathAnimation p = PathAnimation.create(
          this, 0, this.height, 0, maxH, this, ANIMATION_TIME);
    ...
    public void collapse(boolean showAnimation)
    {
      if (showAnimation) {
        PathAnimation p = PathAnimation.create(
          this, 0, this.height, 0, getPreferredHeight(), this, ANIMATION_TIME);
        p.useOffscreen = false;
        p.setpos = this;
        showUIErrorsOld = Settings.showUIErrors;
        Settings.showUIErrors = false;
    ...
    public void collapse() { this.collapse(true); }

    public boolean isExpanded() { return this.height != getPreferredHeight(); }
    ...
    public static class Group
    {
      ...
      public void collapseAll() { this.collapseAll(true); }
    }
    ...
    public static class Caption extends Container
    {
      ...
      {
        this(caption,
             MaterialIcons._ARROW_DROP_DOWN,
             MaterialIcons._ARROW_DROP_UP);
      }
      ...
      {
        this.setInsets(
          (int)((Settings.screenWidth < 320 ? 8 : 16) * Settings.screenDensity),
          0,
          0,
          0);
        ...
        add(new Container() {
          @Override public void initUI()
          {
             ...
             */
            this.setInsets(
              (int)((Settings.screenWidth < 320 ? 64 - 8 : 72 - 16) *
                    Settings.screenDensity),
              (int)((Settings.screenWidth < 320 ? 8 : 16) *
                    Settings.screenDensity),
              0,
              0);
