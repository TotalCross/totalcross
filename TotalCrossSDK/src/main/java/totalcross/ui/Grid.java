...
public class Grid extends Container implements Scrollable {
	 ...
	 */
	public static int columnResizeMargin = Settings.fingerTouch ? Font.NORMAL_SIZE / 2 +
										   Font.NORMAL_SIZE % 2 : 3; // the %2 to keep it ODD
	...
	public Grid(String[] captions, int[] widths, int[] aligns, boolean checkEnabled) {
		...
		if (captions.length != widths.length || aligns.length != captions.length) {
			throw new RuntimeException("The caption (" + captions.length + "), widths (" + widths.length +
									   ") and align ("
		...
		bag = new Container() {
			...
			protected boolean willOpenKeyboard() {
				...
				if (col != -1 && controls[col] != null && selectedLine >= 0
					...
					int row0 = ds != null ? lastStartingRow :
		...
		if (sbVert == null) {
			sbVert = Settings.fingerTouch ? new ScrollPosition() : new
					 ScrollBar(); // guich@580_15: instantiate the scrollbar before the grid is added to the container.
		}
		if (useHorizontalScrollBar || Settings.fingerTouch) {
			sbHoriz = Settings.fingerTouch ? new ScrollPosition(ScrollBar.HORIZONTAL) : new ScrollBar(
	...
	public void replace(String[] item, int row) {
		if (vItems != null && 0 <= row && row < itemsCount) {
			Vm.arrayCopy(item, 0, vItems.items[row], 0,
						 item.length); // guich@557_6: just copy the new item over the current one
		}
	}
			...
			if (w > 0) {
				...
				if (i > 0 || !this.checkEnabled) {
					...
					if (captionImages != null && captionImages[i] != null) {
						...
						g.drawText(data[i], kx + 2 + (greater ? 0 : (w - capw) / 2), yy, textShadowColor != -1,
								   textShadowColor);
						if (greater) {
							g.setClip(2, 0, width - 4, height); // don't allow draw over the borders
						}
	...
	private void paint(Graphics g) {
		...
		g.clearClip();
		if (!uiAndroid) {
			g.drawRect(0, lineH, width + 1,
					   height - lineH); // guich@555_8: removed +1 bc on 3d it overrides scrollbar
	...
	public void initUI() {
		...
		if (Settings.fingerTouch || uiAndroid) {
			if (uiAndroid) {
				add(bag, 0, 0, FILL - (Settings.fingerTouch ? 0 : sbVert.getPreferredWidth()),
		...
		if (sbHoriz != null) {
			...
			by = (btnRight.getPreferredHeight() + extraHorizScrollButtonHeight + extraHB) <<
		...
		if (!Settings.fingerTouch && !uiAndroid) {
			add(bag, 0, 0, FILL - (Settings.fingerTouch ? 0 : sbVert.getWidth()),
				FILL - (!Settings.fingerTouch
						&& sbHoriz != null ? sbHoriz.getPreferredHeight() : 0)); // guich@554_31: +1
		}
	...
	public int getPreferredHeight() {
		return (useHorizontalScrollBar ? sbHoriz.getPreferredHeight() : 0)
			   + (visibleLines > 0 ? (visibleLines + 1) * lineH : Settings.screenHeight >> 1) + insets.top +
			   insets.bottom
	...
	private void showControl(int row, int col) {
		...
		if (c instanceof Edit) {
			...
			ed.setBackColor(
				Color.darker((isStriped() ? ((selectedLine & 1) == 0 ? firstStripeColor : secondStripeColor) :
							  backColor)));
	...
	private void onTip() { // is the tooltip being shown? set the text to the cell one if needed
		...
		int col = ge.col;
		if (!isDisplayed() || (lastPE != null && lastPE.y <= lineH) || (ge != null && ge.row >= itemsCount)
			|| col < 0
			|| col >= widths.length || selectedLine < 0
			|| controls[col] !=
			null) { // guich@557_5: check all bounds - guich@563_6: added isDisplayed - guich@570_92: don't show tip when there's a control assigned - guich@tc115_67: ge.row, not col must be > itemsCount
			tip.setText("");
			if (resizingLine >= 0) {
			...
			int x1 = Convert.sum(widths, 0,
								 ge.col) + xOffset; // guich@tc122_2: if this is the last column and it is not completely visible, show the tooltip
			...
			if (fmw <= cellw) {
				...
				tip.setEnabled(true);
				rTemp.set(absRect.x + 2, absRect.y + (ge.row - gridOffset + 1) * lineH, absRect.width - 10,
						  lineH); // guich@557_10: sub gridOffset in case the grid was scrolled.
			...
			switch (e.type) {
				case ControlEvent.FOCUS_IN:
					if (e.target instanceof Edit && itemsCount > 0
						&& lastShownControl ==
						null) { // guich@573_14: when the Calendar closes, a requestFocus is called for the control, so we handle it
				...
				case ControlEvent.PRESSED:
					if (e.target instanceof ComboBoxDropDown && Settings.keyboardFocusTraversable) {
						...
						if (gridOffset !=
							newOffset) { // guich@555_8: this will be called twice (when button is pressed and when released) due to liveScrolling
				...
				case KeyEvent.KEY_PRESS: {
					...
					if (key == ' ' && checkEnabled && canClickSelectAll) {
						invertAllMarks();
							   ...
							   && key <= '9') { // guich@573_15: allow edit by using a shortcut from 0 to 9
						int col = key - '0';
	...
	private void clickedOnData(int pex, int px, int py) {
		...
		int newSel = line +
					 gridOffset; // guich@tc114_55: consider the DataSource's starting row - guich@tc162: don't consider it
		...
		if (checkEnabled && pex <= widths[0]) {
			if (0 <= newSel
				&& newSel <
				itemsCount) { // just in case you did not click on a line first - guich@580_31: verify if the user can change this check state
				if (cc == null
					|| cc.isEnabled(newSel, 0)) { // guich@tc120_54: don't let the check be down, but post the event
	...
	private int getColFromX(int px, boolean doGap) {
		...
		while (true) {
			if (w[i] > 0) {
				int gap = doGap ? 3 :
						  0; // if doGap is true (for quicksort), we ignore clicks near the col divisions - guich@tc120_23: only 3 pixels is enough
	...
	private void qsortInt(int col, int first, int last,
						  boolean ascending) throws InvalidNumberException { // guich@220_34
		if (first >= last) {
			return;
		}
		int low = first;
	...
	private void qsortDouble(int col, int first, int last,
							 boolean ascending) throws InvalidNumberException { // guich@220_34
		if (first >= last) {
			return;
		}
		...
		while (true) {
			if (ascending) {
				while (high >= low
					   && mid > getDoubleValue(((String[]) items[low])[col], Convert.MIN_DOUBLE_VALUE)) {
					low++;
				}
				while (high >= low
					   && mid < getDoubleValue(((String[]) items[high])[col], Convert.MAX_DOUBLE_VALUE)) {
					high--;
				}
			} else {
				while (high >= low
	...
	private void qsortDate(int col, int first, int last,
						   boolean ascending) throws InvalidDateException { // guich@220_34
		if (first >= last) {
			return;
		}
		int low = first;
	...
	private void postGridEvent(int col, int row, boolean isTextChangedEvent) {
		if (enableSelectDisabledCell || cc == null || (row == -1
				|| cc.isEnabled(row, col))) { // guich@580_31 - guich@tc120_54: commented out
			ge.touch();
			ge.target = this;
			ge.row = row;
			ge.col = col;
	...
	public void setSelectedIndex(int row) { // guich@555_7
		...
		if (0 <= row
			&& row < itemsCount) { // guich@555_7: removed Math.min(itemsCount, linesPerPage), otherwise it will only select the first items
