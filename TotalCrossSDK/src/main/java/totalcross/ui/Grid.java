 ...
 * <li>Horizontal scrolling in case the columns widths are greater than the
 * parent container's width <li>An easy to use interface for adding/removing
 * information to the grid <li>An optional check column, which is a column that
 * is clickable, marking an specific line as checked/unchecked. This is usefull
 * if you want the user to be able to mark multiple lines displayed on the grid
 * <li>Columns can be resized so that the user can see all the information
 * displayed in a given column <li>Style configuration, you can set the color of
 * captions boxes all the way thru the stripes colors and vertical line types.
 * The grid is compatible with all user interface styles (Flat, WinCE, etc... )
 * <li>The GridEvent class contains the events generated by the grid.
 * <li>A column may be marked as editable. In this case, an Edit will be placed
 * in the currently focused cell. A column can also have choices (ComboBox).
 * <li>A column can also have a ComboBox associated.
 * <li>The columns can be sorted by clicking in the title.
 * <li>If the text is bigger than the size of the column, you can click and hold
 * in the cell to display a tooltip with the full text. <li>You can customize
 * cell back and fore colors, enabled state and choices by using the
 * CellController class.
 * <li>Clicking in the column's caption will sort in ascending order; clicking
 * on it again sorts on descending order. <li>It is possible to assign a
 * DataSource to a Grid, making the elements be fetched on demand. <li>A column
 * with size 0 is not displayed, so you can use it to store in the grid
 * important information, for example the rowid of a table.
 * </ul>
 *
 *  Here's an example:
 * <pre>
 ...
 * alignment and a check column. <p> Don't forget to take a look at
 * tc.samples.ui.grid.GridTest. <p> Also consider using the ListContainer
 * instead of Grid if you're using devices with big screens. See the UIGadgets
 * sample, click menu / Tests2 / ListContainer.
 *
 * @since SuperWaba 5.5
 */
...
{
  /**
   * Abstract class that must be implemented if you want to get a fine control
   * of each cell in the grid. Note that you must keep track of the state of
   * each cell in your code. How to proceed: <ul> <li> Create a class that
   * extends CellController and implement each of the methods. Then call
   * grid.setCellController passing an instance of the created class. <li> Each
   * time a cell is drawn, the getForeColor, getBackColor and getFont methods
   * are called. If you want to use the default values, return
   * <code>null</code>. <li> If a column has choices activated (with
   * setColumnChoices), each time the ComboBox will be opened, the getChoices
   * method is called. This way, you can dynamically change the choices for each
   * cell. If you want to use the default values, return <code>null</code>. <li>
   * Each time a cell is clicked, the isEnabled method is called. If you return
   * false to it, the event is not generated; also, when trying to open the
   * ComboBox or the Edit assigned to the column, the control will not be
   * activated. This is valid for a check column too.
   * </ul>
   * Note that using a CellControler makes the grid drawings a bit slower. Also,
   * if the user sorts the columns, you may get scrambled. You can disable sort
   * using <code>grid.disableSort = true</code>.
   */
  ...
  { // guich@580_31
    ...
    /**
     * Must return the background color for the given cell, or -1 if none. Note
     * that if the grid has a check, the back color of the check cell will be
     * requested passing col value as -1.
     */
    public abstract int getBackColor(int row, int col);

    /**
     * Must return the choices for a given line and column, if setColumnChoices
     * was set to this column.
     */
    public abstract String[] getChoices(int row, int col);

    /**
     * Must return if the given cell is enabled, ie, can have input and can be
     * selected. If the grid has a check column, returning false will disable
     * the check state change.
     */
    public abstract boolean isEnabled(int row, int col);

    /**
     * Must return the font to be used in this cell, or null to use the default
     * control's font. Note: if you plan to use a font bigger than in the rest
     * of the grid, be sure to follow these steps: <ol> <li> Construct the grid
     * and set the grid's font to be the one with bigger size <li> Call
     * add/setRect <li> Restore the grid's default font
     * </ol>
  ...
  /**
   Interface that can be used to fetch data on demmand. This makes the grid
   slower but
  ...
  /**
   * When the user clicks on the header check to check all, a
   * CHECK_CHANGED_EVENT is dispatched, and the row is set as ALL_CHECKED.
   */
  public static final int ALL_CHECKED = -900; // guich@580_16

  /**
   * When the user clicks on the header check, to uncheck all, a
   * CHECK_CHANGED_EVENT is dispatched, and the row is set as ALL_UNCHECKED.
   */
  public static final int ALL_UNCHECKED = -901; // guich@580_16

  /**
   * Set this to false if you dont want the check column to have the rect
   * underneath the check mark
   */
  ...
  /**
   * How many pixels are used to define the place where the column will be
   * resized. Defaults to 3 on pen devices, and 5 on touch devices (must be
   * ODD!)
   * @since TotalCross 1.22
   */
  public static int columnResizeMargin =
    Settings.fingerTouch ? Font.NORMAL_SIZE / 2 + Font.NORMAL_SIZE % 2
                         : 3; // the %2 to keep it ODD

  /**
   * The column captions. Can be directly assigned, but always make sure it has
   * the same number of elements of the widths array, set with the
   * <code>setWidths</code> method.
   */
  ...
  /**
   * The tooltip used to display a column's text when it exceeds the column
   * width. You can directly customize the tooltip.
   */
  public ToolTip tip;

  /**
   * The extra height of the horizontal scroll buttons. Defaults 2 in 160x160
   * or a multiple of it in other resolutions.
   * @see ListBox#EXTRA_HEIGHT_FACTOR
   */
  public int extraHorizScrollButtonHeight =
    Settings.screenHeight * 2 /
    160; // guich@556_11 - guich@560_11: now depends on the resolution

  /**
   * Stores the last line that was requested from the current data source.
   * The last lines retrieved will be cached to prevent requesting the same
   * data. If you want to ensure that the data will be fetched again, set this
   * member to -1 in getItems.
   * @since TotalCross 1.0 beta 5
   */
  public int lastStartingRow = -1; // guich@tc100b5_24
  /**
   * Defines the sort type for each column of the grid. Specify -2 to disable
   * sort for that column. The array is created in the constructor setting all
   * columns to Convert.SORT_AUTODETECT; so you can just change a specific
   * column with the sort type you want. <br><br>The column index always
   * excludes the check column. So a grid with 3 columns and a check, the
   * sortTypes length will be 3, which will be the same length of a grid with 3
   * columns without a check.
  ...
  /**
   * Set to true before constructing the Grids to use an horizontal scrollbar
   * instead of the two left/right buttons. Note that this member is static so
   * it will affect all Grids created afterwards, unless you reset it to false.
   * @since TotalCross 1.01
   */
  public static boolean useHorizontalScrollBar; // guich@tc110_38

  /**
   * Set to true before calling setDataSource to enable live scrolling when
   * using a DataSource.
   * @see #setDataSource
   */
   ...
   * wanted number of decimal places, or -1 if you don't want to format it. Note
   * that the original value must be already formatted with the wanted number of
   * decimal places, and also that only the displayed value is changed; the
   * internal value will remain without formatting. Example: <pre> Grid g = new
   * Grid(new String[]{"Name","Age","Salary"},false); g.currencyDecimalPlaces =
   * new int[]{-1,-1,2}; // ignore name and age, and format salary with 2
   * decimal places.
   * </pre>
   */
  ...
  /**
   * Set to false to disallow the selection of a row if the selected cell is
   * disabled (via CellController).
   * @since TotalCross 1.2
   */
  public boolean enableSelectDisabledCell = true;

  /**
   * Set to true to enable navigation per line instead of per page when in
   * non-penless mode. By default, in pen mode (devices with touch-screen), when
   * the user press up/down, the grid is scrolled one page at a time. Setting
   * this flag to true makes it scroll a line at a time.
   * @since TotalCross 1.23
   */
  public boolean lineScroll; // guich@tc122_55

  /**
   * Defines the border gap when using CellController.getBackColor. Setting it
   * to something bigger than 0 will make a space between the cells.
   * @since TotalCross 1.24
   */
  public int borderGap; // guich@tc123_52

  /**
   * The aligns that are used in the constructor. You may change them after the
   * constructor is called.
   */
  public int[] aligns; // guich@tc125_8: field aligns is now public, as
                       // requested by users.

  /**
   * When the width of a title is greater than the width you specified for a
   * the column, the title's width is used instead. Set this flag to true to
  ...
  /**
   * A way to mix Images and Strings at the caption. The array's length must be
   * the same of the caption's passed in the constructor, but the indices that
   * are an image will have it drawn. For example: <pre> String[] tits =
   * {"","Add","Minus",""}; captionImages = {img1, null,null, img2};
   * </pre>
  ...
  protected Control[] controls; // guich@570_81: now a column may have edit OR a
                                // poplist.
  ...
  private boolean recomputeDefaultCaptionWidths; // check if widths was passed
                                                 // as null on constructor
  ...
  /**
   * This will create a grid with the given captions, column widths, information
   * alignment and an optional multi-selection check column so that the user can
   * select multiple lines of the grid
   *
   * @param captions
   *           Captions for the columns. Cannot be null!
   * @param widths
   *           Widths of the columns. If the total width is less than the grid's
   * width, the last column will fill until the grid width. If null, the caption
   * widths will be computed and used as the row width. If a negative value is
   * passed, it will be computed as a percentage against the Grid's width.
   * @param aligns
   *           Alignment of information on the given column. If null, all aligns
   * will be LEFT.
   * @param checkEnabled
   *           True if you want the multi-selection check column, false
   * otherwise
   */
  public Grid(String[] captions,
              int[] widths,
              int[] aligns,
              boolean checkEnabled)
  {
    ...
    if (captions.length != widths.length || aligns.length != captions.length) {
      throw new RuntimeException(
        "The caption (" + captions.length + "), widths (" + widths.length +
        ") and align (" + aligns.length + ") arrays must have the same length");
    }
    ...
    bag = new Container() {
      @Override public void onPaint(Graphics g) { paint(g); }
      ...
      @Override protected boolean willOpenKeyboard()
      {
        ...
        if (col != -1 && controls[col] != null && selectedLine >= 0 &&
            controls[col] instanceof Edit) { // show the edit
          int row0 =
            ds != null
              ? lastStartingRow
              : 0; // guich@tc114_55: consider the DataSource's starting row
    ...
    if (sbVert == null) {
      sbVert =
        Settings.fingerTouch
          ? new ScrollPosition()
          : new ScrollBar(); // guich@580_15: instantiate the scrollbar
                             // before the grid is added to the container.
    }
    if (useHorizontalScrollBar || Settings.fingerTouch) {
      sbHoriz = Settings.fingerTouch ? new ScrollPosition(ScrollBar.HORIZONTAL)
   ...
   * The widths will be computed as the width of the grid captions and the
   * alignment will be all LEFT.
   *
   * @param captions
  ...
  @Override public boolean canScrollContent(int direction, Object target)
  {
    ...
    if (Settings.fingerTouch) {
      switch (direction) {
        ...
        case DragEvent.DOWN:
          return sbVert != null &&
            (sbVert.getValue() + sbVert.getVisibleItems()) <
              sbVert.getMaximum();
        case DragEvent.LEFT:
          return sbHoriz != null && sbHoriz.getValue() > sbHoriz.getMinimum();
        case DragEvent.RIGHT:
          return sbHoriz != null &&
            (sbHoriz.getValue() + sbHoriz.getVisibleItems()) <
              sbHoriz.getMaximum();
      }
    }
    flickDirection = NONE;
  ...
  /**
   * Sets the CellController instance for this grid. Read the javadocs of the
  ...
  /**
   * Sets the column width of the given column. Positive values are used as
   * pixels, negative values represent percentage of the grid's width and 0
   * hides the column.
  ...
  /**
   * Returns an array with the current column widths, not the original ones
   * passed in the constructor. Changing these values will NOT change the
   * column's width
  ...
  /**
   * Sets the given column as an editable one. Note that setting an editable
   * column as not editable will remove all its formats. Returns the previous
   * Edit or the new one, so you can easily change its formats. Important: this
   * must be called AFTER the grid has set its bounds.
   */
  public Edit setColumnEditable(int col, boolean editable)
  {
    ...
    if (!editable) {
      ...
      add(ed);
      ed.setVisible(false);
      tabOrder.removeAllElements(); // guich@580_55: don't let the focus go to
                                    // the edit.
    }
    return ed;
  }
   ...
   * @since TotalCross 1.39
   */
  public boolean isColumnEditable(int col) { return controls[col] != null; }

  /**
   * Makes the given column ComboBox-like. The given choice array is used to
   * create a PopList that is shown when the user clicks on the column. Note
   * that calling this method removes any Edit assigned to this column. You can
   * change dynamically the choices by extending the CellController class.
   * @param col The column to set as a ComboBox column
   * @param choices The choices that will be displayed. Passing a null value
   *     removes any ComboBox
   * assigned to the column.
   * @see #setCellController
   * @returns The created ComboBoxDropDown (so you can customize) or null if
   *     choices is null
   * @since SuperWaba 5.7
   */
  public ComboBoxDropDown setColumnChoices(int col, String[] choices)
  { // guich@570_81
    if (col < 0 || col >= captions.length) {
  ...
  private void getColRect(Rect temp, int col, int row, boolean absolute)
  {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (i == col) {
        ...
        temp.set(xx,
                 (row + 1) * lineH + (absolute ? 0 : (lineH - fmH) / 2),
                 ww,
                 absolute ? lineH : fmH);
        break;
      }
  ...
  @Override protected void onFontChanged()
  {
    ...
    rBox =
      uiAndroid ? new Rect(1, 1, k - 2, k - 2) : new Rect(0, 0, k / 2, k / 2);
  ...
  /**
   * Sets the grid items to be displayed. Note that it needs to be conforming to
   * the numbers of columns that the grid currently have. This method removes
   * any assigned datasource. The strings inside the matrix cannot be null, If
   * items is null, the grid will remain empty.
   */
  ...
  /**
   * Sets the data source of this grid to be the given one. Note that when
   * using data sources, the add, remove, insert, etc methods CANNOT BE USED. A
   * data source is mostly used to assign a ResultSet to it, so its nonsense any
   * data modification. Note that the scroll is made <b>not</b> <i>live
   * scrolling</i> to speedup data retrieval.
   * @since SuperWaba 5.7
   */
  ...
  /**
   * Add a new line to the end of the grid. Its up to the user to call
   * Window.needsPaint = true afterwards. This method does not work if there's a
   * datasource assigned.
   ...
   * Its up to the user to call Window.needsPaint = true afterwards. This method
   * does not work if there's a datasource assigned.
   *
   * @param items
  ...
  { // flsobral@tc120_35: added convenience method.
  ...
  public void replace(String[] item, int row)
  {
    if (vItems != null && 0 <= row && row < itemsCount) {
      Vm.arrayCopy(item,
                   ...
                   vItems.items[row],
                   0,
                   item.length); // guich@557_6: just copy the new item over the
                                 // current one
    }
  }
  ...
  public int getSelectedIndex() { return selectedLine; }

  /**
   * Checks or unchecks the given index based on the second argument.
   * This method only works with a grid that has checks and repaint must be
   * called.
   */
  ...
  {
    return checkEnabled && row >= 0 &&
      ivChecks.items[row] == 1; // guich@556_6: added lineIndex check
  }

  /**
   * Return a vector containing all information inside the grid. Note that if a
   * datasource is assigned, this method returns null.
   */
  public Vector getItemsVector() { return vItems; }

  /**
   * Returns the line corresponding to the given index. This method works with
   * or without a datasource.
   */
  public String[] getItem(int row)
  {
    ...
    return vItems != null ? (String[])vItems.items[row]
                          : getDataSourceItems(row, 1)[0];
  }
  ...
  private String[][] getDataSourceItems(int startingRow, int count)
  { // guich@tc100b5_24
  ...
  private void drawCaptions(Graphics g)
  {
    ...
    /**
     * create rect around the captions, fill it with an specified color, draws
     * the vertical line all the way thru the component, and then draws the
     * captions text inside of it -- vlima
     */
    g.clearClip();
    /**
     * fill up the unused space in case the user drags the last column to the
     * left
     */
    ...
    if (!uiAndroid) {
      ...
      try {
        if (npPartsCap != null && npcapt == null) {
          npcapt = NinePatch.getInstance().getNormalInstance(
            npPartsCap, width, lineH + 5, captionsBackColor, false);
        }
        // draw top
      ...
      if (w > 0) {
        ...
        if (!uiAndroid) {
          if (uiVista && isEnabled()) {
            g.fillVistaRect(
              kx, 0, w + 1, lineH, captionsBackColor, true, false);
          } else {
        ...
        if (i > 0) {
          switch (this.verticalLineStyle) {
            case VERT_DOT:
              ...
              break;
            case VERT_NONE:
              // doesn't draw the vertical line. Note that the columns are still
              // resizable
              break;
            default:
            case VERT_LINE:
        ...
        if (i > 0 || !this.checkEnabled) {
          ...
          if (captionImages != null && captionImages[i] != null) {
            g.drawImage(
              captionImages[i], kx + (w - captionImages[i].getWidth()) / 2, yy);
          } else {
            ...
            if (greater) {
              g.setClip(2,
                        0,
                        width - 4,
                        height); // don't allow draw over the borders
            }
          }
        }
  ...
  private void drawCheck(Graphics g, int y, boolean checked)
  {
    ...
    if (uiAndroid) {
      try {
        if (drawCheckBox) {
          if (npCheckBack == null) {
            npCheckBack = Resources.checkBkg.getNormalInstance(
              rBox.height, rBox.height, foreColor);
          }
        ...
        if (checked) {
          ...
          if (npCheck == null) {
            npCheck = Resources.checkSel.getPressedInstance(
              hh,
              hh,
              backColor,
          ...
          g.drawImage(npCheck,
                      xOffset + (drawCheckBox ? rBox.x : 1),
                      y + (drawCheckBox ? rBox.y : 0));
        }
      } catch (Exception e) {
        if (Settings.onJavaSE) {
          e.printStackTrace();
  ...
  private void paint(Graphics g)
  {
    ...
    if (!transparentBackground) { // guich@tc115_18
      ...
      if (isStriped()) {
        ...
        g.fillRect(0, 0, width, height); // guich@566_9: erase the background if
                                         // drawStripes is false
      }
    }
    g.foreColor = fc;
    drawCaptions(g);
    ...
    if (itemsCount > 0) {
      ...
      for (int j = 0; j < cols; j++) {
        ...
        ty = lineH;
        if (w > 0 && cx + w > 0 &&
            cx <= maxX) { // ignore columns not being shown
          int cw = Math.min(w - 1, width - cx + 1);
          g.setClip(cx - 1,
                    uiAndroid ? ty : ty + 1,
                    cw,
                    lineH * linesPerPage); // guich@580_32: fixed clip
          // rect based on
          ...
          if (checkDrawn) {
            ...
            for (int i = i0; i < rows; i++, ty += lineH) {
              ...
              Image columnImg =
                htImages != null ? (Image)htImages.get(columnText) : null;
              if (columnText == null) {
                columnText = "";
              }
              if (currencyDecimalPlaces != null &&
                  currencyDecimalPlaces[j] >= 0) {
                columnText = Convert.toCurrencyString(columnText,
                                                      currencyDecimalPlaces[j]);
              }
              ...
              if (columnImg != null) {
                ...
                tx =
                  cx + (align == LEFT
                          ? 0
                          : align == CENTER
                              ? (w - f.fm.stringWidth(columnText)) / 2
                              : w - 3 - f.fm.stringWidth(columnText)); // RIGHT
              }
              cf = -1;
              boolean isSelectedLine =
                drawHighlight && currentRow == selectedLine;
              if (cc != null || isSelectedLine) { // guich@580_31
                cb = -1;
                if (isSelectedLine |
                    (cc != null &&
                     (cb = cc.getBackColor(currentRow, j)) != -1)) // have
                                                                   // to
                                                                   // compute
                                                                   // both
                                                                   // arguments
                {
                  if (!isSelectedLine) {
                    ...
                    g.backColor = Color.interpolate(highlightColor,
                                                    cb); // guich@tc126_20;
                  } else {
                    g.backColor = highlightColor;
                  }
                  g.fillRect(cx - 1 + borderGap,
                             ty + borderGap,
                             w - 1 - borderGap - borderGap,
                ...
                if (cc != null && (cf = cc.getForeColor(currentRow, j)) != -1) {
                  g.foreColor =
                    isEnabled()
                      ? cf
                      : Color.interpolate(cf, g.backColor); // guich@tc139:
                                                            // shade
                                                            // color if
                                                            // not
                                                            // enabled
            ...
            for (int i = i0; i < rows; i++, ty += lineH) {
              ...
              if (currentRow == selectedLine |
                ...
                if (currentRow != selectedLine) {
                  ...
                  g.backColor =
                    Color.interpolate(highlightColor, cb); // guich@tc126_20;
                } else {
                  g.backColor = highlightColor;
                }
    ...
    if (!uiAndroid) {
      g.drawRect(0,
                 lineH,
                 width + 1,
                 height - lineH); // guich@555_8: removed +1 bc on 3d it
                                  // overrides scrollbar box - guich@tc115_2:
                                  // moved to here, after the items were drawn
    } else if (!uiMaterial) {
      // draw bottom
  ...
  /**
   * We need to add the check column information to widths, aligns, and captions
  ...
  private void setWidths(int[] newWidths)
  {
    ...
    for (i = 0; i <= n; i++) {
      if (newWidths[i] > 10000) {
        throw new RuntimeException(
          "FILL is no longer supported as a column width! Width of column " +
          i + " set to " + newWidths[i]);
      }
      if (newWidths[i] == 0) {
        continue;
      }
      ...
      totalW += widths[i];
      if (i == lastVisibleCol) {
        if (totalW < availW) { // make sure that the last col will have the
                               // grid's width if needed
      ...
      if (enableColumnResize && (!checkEnabled || i > 0)) {
        int l =
          (totalW << 16) | i; // store the real position with the line index
        for (int step = (columnResizeMargin - 1) / 2, z = -step; z <= step;
             z++) {
  ...
  @Override public void initUI()
  {
    ...
    if (Settings.fingerTouch || uiAndroid) {
      if (uiAndroid) {
        add(bag,
            0,
            0,
            FILL - (Settings.fingerTouch ? 0 : sbVert.getPreferredWidth()),
            FILL - 4); // guich@554_31: +1
      } else {
    ...
    if (sbHoriz != null) {
      ...
      if ((this.height / btnRight.getPreferredHeight() >
      ...
      by =
        ...
        << 1; // get the height of the two buttons together
      if (uiAndroid) {
        by += 4;
      }
    }
    ...
    sbVert.setValues(0,
                     linesPerPage,
                     0,
                     itemsCount); // guich@580_15: set the current itemsCount
    sbVert.setLiveScrolling(true);
    sbVert.setBackForeColors(backColor, foreColor);
    if (sbHoriz != null) {
      add(sbHoriz,
          LEFT,
          BOTTOM - (uiAndroid ? 4 : 0),
          Settings.fingerTouch ? FILL : FIT + 1,
          PREFERRED);
    } else {
      // add the two horizontal scroll buttons below the scrollbar
      btnLeft.setRect(
        RIGHT, AFTER, SAME, PREFERRED + extraHorizScrollButtonHeight + extraHB);
      btnRight.setRect(
        RIGHT, AFTER, SAME, PREFERRED + extraHorizScrollButtonHeight + extraHB);
    }
    if (!Settings.fingerTouch && !uiAndroid) {
      add(bag,
          0,
          0,
          FILL - (Settings.fingerTouch ? 0 : sbVert.getWidth()),
          FILL - (!Settings.fingerTouch && sbHoriz != null
                    ? sbHoriz.getPreferredHeight()
                    : 0)); // guich@554_31: +1
    }
  ...
  /**
   * If all widths (passed in constructor) are positive (ie, not a percentage),
   * then it will use the sum as the preferred width.
   */
  @Override public int getPreferredWidth()
  {
    // guich@tc114_52: if all values are positive, use them to compute a
    // preferred width
    int sum = 0;
    ...
    if (sum > 0) {
      sum += (Settings.fingerTouch ? 0 : new ScrollBar().getPreferredWidth()) +
             (checkEnabled ? defaultCheckWidth : 0);
    } else {
      sum = Settings.screenWidth >>
            1; // else, use the default, which is screen width / 2
    }
  ...
  @Override public int getPreferredHeight()
  {
    return (useHorizontalScrollBar ? sbHoriz.getPreferredHeight() : 0) +
      (visibleLines > 0 ? (visibleLines + 1) * lineH
                        : Settings.screenHeight >> 1) +
      insets.top + insets.bottom + (uiAndroid ? 4 : 0); // guich@tc126_
  }

  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    int sbh = (!Settings.fingerTouch && sbHoriz != null)
                ? sbHoriz.getPreferredHeight()
                : 0;
    int lh = height - lineH + 1 -
             sbh; // height of the vertical grid line (captions excluded)
    if (uiAndroid) {
      lh -= 4;
    }
    linesPerPage = lh / lineH;
    if (sbVert != null) {
      sbVert.setVisibleItems(
        linesPerPage); // guich@556_8: fixed problem when linesPerPage changes
    }
    ...
    if (asContainer.finishedStart) { // luciana@570_18: fixed problem when
                                     // setRect is called more than once
      sbVert.reposition();
      if (sbHoriz != null) {
        sbHoriz.reposition();
      } else {
  ...
  /**
   * Remove all elements from the grid, leaving it blank. Removes any assigned
   * datasource.
   */
  ...
  private void invertAllMarks()
  {
    markAll(!allChecked);
    postGridEvent(0,
                  allChecked ? ALL_CHECKED : ALL_UNCHECKED,
                  false); // guich@580_16
    Window.needsPaint = true;
  }
  ...
  public boolean horizontalScroll(boolean toLeft)
  {
    ...
    int newOffset = toLeft ? Math.min(xOffset + step, 0)
                           : Math.max(xOffset - step, maxOffset);
  ...
  private void hideControl()
  {
    ...
    if (c instanceof Edit) {
      ...
      if (lb.getSelectedIndex() >= 0 &&
          !oldCellText.equals(sel = (String)lb.getSelectedItem())) {
        Window.needsPaint = true;
  ...
  /**
   * Sets the text of a column. If the grid has a check column, col must start
   * from 1.
   * @since SuperWaba 5.8
   */
  ...
  {
    return firstStripeColor != secondStripeColor ||
      firstStripeColor != backColor;
  }
  ...
  private void showControl(int row, int col)
  {
    ...
    c.setRect(rTemp);
    c.appId =
      (col << 24) | selectedLine; // guich@557_11: replaced line by selectedLine
    if (c instanceof Edit) {
      Edit ed = (Edit)c;
      if (cc != null) {
      ...
      ed.setBackColor(Color.darker(
        (isStriped()
           ? ((selectedLine & 1) == 0 ? firstStripeColor : secondStripeColor)
           : backColor)));
      ...
      if (cc != null) {
        ...
        if ((newItems = cc.getChoices(row + row0, col)) !=
            null) { // guich@580_31
          oldItems = pl.lb.getItems();
          pl.lb.itemCount = 0; // can't call removeAll bc the string array will
                               // have all elements set to null
          pl.lb.add(newItems);
  ...
  private void onTip()
  { // is the tooltip being shown? set the text to the cell
    // one if needed
    // uses the last event to find the clicked col
    int col = ge.col;
    if (!isDisplayed() || (lastPE != null && lastPE.y <= lineH) ||
        (ge != null && ge.row >= itemsCount) || col < 0 ||
        col >= widths.length || selectedLine < 0 ||
        controls[col] !=
          null) { // guich@557_5: check all bounds - guich@563_6: added
                  // isDisplayed - guich@570_92: don't show tip when there's a
                  // control assigned - guich@tc115_67: ge.row, not col must
                  // be > itemsCount
      tip.setText("");
      if (resizingLine >= 0) {
      ...
      int x1 = Convert.sum(widths,
                           0,
                           ge.col) +
               xOffset; // guich@tc122_2: if this is the last column and it is
                        // not completely visible, show the tooltip
      ...
      if (fmw <= cellw) {
        ...
        tip.setEnabled(true);
        rTemp.set(absRect.x + 2,
                  absRect.y + (ge.row - gridOffset + 1) * lineH,
                  absRect.width - 10,
                  lineH); // guich@557_10: sub gridOffset in case the grid was
                          // scrolled.
      ...
      switch (e.type) {
        case ControlEvent.FOCUS_IN:
          if (e.target instanceof Edit && itemsCount > 0 &&
              lastShownControl == null) { // guich@573_14: when the Calendar
                                          // closes, a requestFocus is called
                                          // for the control, so we handle it
        ...
        case ControlEvent.PRESSED:
          if (e.target instanceof ComboBoxDropDown &&
              Settings.keyboardFocusTraversable) {
            isHighlighting = false;
          } else if (e.target == sbVert) {
            int newOffset = sbVert.getValue();
            if (gridOffset !=
                newOffset) { // guich@555_8: this will be called
                             // twice (when button is pressed and
                             // when released) due to liveScrolling
        ...
        case PenEvent.PEN_DOWN:
          ...
          if (e.target == this) {
             ...
             * accordingly. One click mark all, one click after all marked
             * dismark all.
             */
            if (checkEnabled && canClickSelectAll && py <= lineH &&
                px <= widths[0]) {
              invertAllMarks();
            ...
            if (line >= 0) { // clicked in a row? resize it
              ...
              e.consumed = true; // don't let the tooltip timer be started
                       ...
                       itemsCount > 1) { // guich@555b: quicksort the column
          ...
          // else ignoreNextEvent = true; // when the user press a button, a
          // repaint is propagated to the parent (we), resulting on an
          // undesirable flicker
          break;
        case PenEvent.PEN_DRAG_END:
          if (flick != null && Flick.currentFlick == null &&
              itemsCount > linesPerPage) {
            e.consumed = true;
          }
          break;
        case PenEvent.PEN_DRAG: {
          ...
          if (e.target == this && rl != -1) {
            ...
            widths[rl] =
              resizingOrigWidth + dx; // guich@tc110_47: update in realtime
        ...
        case KeyEvent.KEY_PRESS: {
          ...
          if (isHighlighting || ke.target instanceof Edit) {
            break; // let the Edit work - guich@582_16: check if isHighlighting,
                   // and, in this case, just exit.
          }
          int key = ke.key;
          if (key == ' ' && checkEnabled && canClickSelectAll) {
            invertAllMarks();
          } else if (selectedLine >= 0 && '0' <= key &&
                     key <= '9') { // guich@573_15: allow edit by using a
                                   // shortcut from 0 to 9
            int col = key - '0';
            int row0 = ds != null ? lastStartingRow : 0;
            if (cc == null || cc.isEnabled(selectedLine + row0, col)) {
              if (checkEnabled &&
                  col == 0) { // guich@580_31: verify if the user
                              // can change this check state
                setChecked(selectedLine, !isChecked(selectedLine));
                postGridEvent(checkEnabled ? 0 : 1, selectedLine, false);
                getParentWindow()
                  .repaintNow(); // this is the only way it worked in the 6600.
                                 // at no other ways the screen was updated
                                 // after pressing 0.
                     ...
                     // guich@582_
                      {
                         ...
                         getColRect(rTemp, col, selectedLine, true); //
                     guich@tc100: not sure if is "true" here for (int i =0; i <
                     4; i++)
                         {
        ...
        case KeyEvent.SPECIAL_KEY_PRESS: {
          ...
          if ((Settings.keyboardFocusTraversable || lineScroll) &&
            ...
            if (ke.isUpKey()) { // guich@550_15: added support for navigate
                                // using all arrows
              if (itemsCount == 0) {
                return;
              }
              ...
              if (--line < 0) {
                line = Settings.circularNavigation ? itemsCount - 1
                                                   : 0; // guich@tc115_19
              }
              if (line != selectedLine) {
                setSelectedIndex(line);
                // postGridEvent(1,selectedLine,false); guich@572_21: now the
                // SELECTED event is dispatched only when a key is pressed.
              }
              ...
              int line = selectedLine;
              if (++line >= itemsCount) {
                line = Settings.circularNavigation
                         ? 0
                         : itemsCount - 1; // guich@tc115_19
              }
              if (line != selectedLine) {
                setSelectedIndex(line);
                // postGridEvent(1,selectedLine,false); guich@572_21: now the
              ...
              horizontalScroll(key == SpecialKeys.LEFT);
                     ...
                     !lineScroll) { // guich@555_12
            if (ke.isUpKey() &&
                itemsCount > linesPerPage) { // guich@571_8: do only when needed
              ...
              if (v != gridOffset) {
                ...
                return;
                       ...
                       itemsCount >
                         linesPerPage) { // guich@571_8: do only when needed
           ...
           * guich@573_45
              if (ke.isActionKey())
              {
                 if (checkEnabled && selectedLine != -1) // guich@572_21: now
           the enter key always dispatch a SELECTED event, not only when
           checkEnabled.
           - guich@573_5: check if there's a line selected
                    setChecked(selectedLine, !isChecked(selectedLine));
                 postGridEvent(checkEnabled ? 0 : 1,selectedLine, false);
              }*/
          Window.needsPaint = true;
  ...
  private int getLine(int py) { return py / lineH - 1; }
  ...
  private void clickedOnData(int pex, int px, int py)
  {
    ...
    int newSel =
      line + gridOffset; // guich@tc114_55: consider the DataSource's starting
                         // row - guich@tc162: don't consider it

    if (selectedLine != newSel &&
        (enableSelectDisabledCell || cc == null || cc.isEnabled(newSel, 0))) {
    ...
    if (checkEnabled && pex <= widths[0]) {
      if (0 <= newSel &&
          newSel < itemsCount) { // just in case you did not click on a line
                                 // first - guich@580_31: verify if the user can
                                 // change this check state
        if (cc == null ||
            cc.isEnabled(newSel, 0)) { // guich@tc120_54: don't let the check be
                                       // down, but post the event
  ...
  private int getColFromX(int px, boolean doGap)
  {
    ...
    while (true) {
      if (w[i] > 0) {
        int gap = doGap ? 3 : 0; // if doGap is true (for quicksort), we ignore
                                 // clicks near the col divisions -
                                 // guich@tc120_23: only 3 pixels is enough
  ...
  { // guich@tc115_28
    ...
    if (itemsCount > 1 && vItems != null) {
      ...
      do {
        ...
        if (sortType == Convert.SORT_DOUBLE) {
          containsDouble = true;
        }
        if (containsDouble && sortType == Convert.SORT_INT) {
          sortType = Convert.SORT_DOUBLE;
        }
          ...
          try {
            switch (sortType) {
              ...
              case Convert.SORT_DATE:
                ...
                break;
              default:
                return; // get out - dont know what kind of exceptions can be
                        // thrown with String
            }
          } catch (Exception ee) {
            if (ascending) {
  ...
  /**
   * Performs a quicksort in the items of the given column. This method does
   * not work if there's a datasource assigned.
   */
  ...
  private void qsortInt(int col, int first, int last, boolean ascending)
  ...
  { // guich@220_34
    ...
    while (true) {
      if (ascending) {
        while (high >= low &&
               mid > Convert.toInt(((String[])items[low])[col])) {
          low++;
        }
        while (high >= low &&
               mid < Convert.toInt(((String[])items[high])[col])) {
          high--;
        }
      } else {
        while (high >= low &&
  ...
  private double getDoubleValue(String s, double def)
  {
    return s.equals("")
      ? -1
      : Convert.toDouble(s, def); // guich@tc210: support empty cells
  }

  private void qsortDouble(int col, int first, int last, boolean ascending)
    throws InvalidNumberException
  { // guich@220_34
    if (first >= last) {
      return;
    ...
    double mid = getDoubleValue(((String[])items[(first + last) >> 1])[col],
                                Convert.MIN_DOUBLE_VALUE);
    while (true) {
      if (ascending) {
        while (high >= low && mid > getDoubleValue(((String[])items[low])[col],
                                                   Convert.MIN_DOUBLE_VALUE)) {
          low++;
        }
        while (high >= low && mid < getDoubleValue(((String[])items[high])[col],
                                                   Convert.MAX_DOUBLE_VALUE)) {
          high--;
        }
      } else {
        while (high >= low && mid < getDoubleValue(((String[])items[low])[col],
  ...
  private void qsortDate(int col, int first, int last, boolean ascending)
  ...
  private void qsortString(int col, int first, int last, boolean ascending)
  { // guich@220_34
    if (first >= last) {
      return;
    }
  ...
  private void qsortStringNocase(int col,
                                 int first,
                                 int last,
                                 boolean ascending)
  { // guich@220_34
    ...
    while (true) {
      if (ascending) {
        while (high >= low &&
               mid.compareTo(((String[])items[low])[col].toLowerCase()) > 0) {
          low++;
        }
        while (high >= low &&
               mid.compareTo(((String[])items[high])[col].toLowerCase()) < 0) {
          high--;
        }
      } else {
        while (high >= low &&
  ...
  {
    if (enableSelectDisabledCell || cc == null ||
        (row == -1 ||
         cc.isEnabled(row,
                      col))) { // guich@580_31 - guich@tc120_54: commented out
      ge.touch();
      ge.target = this;
      ge.row = row;
      ge.col = col;
      ge.checked = isChecked(selectedLine);
      ge.type = isTextChangedEvent
                  ? GridEvent.TEXT_CHANGED_EVENT
                  : (col == 0 && checkEnabled) ? GridEvent.CHECK_CHANGED_EVENT
                                               : GridEvent.SELECTED_EVENT;
      postEvent(ge);
      ge.target = null;
    }
  }
  ...
  /**
   * Sets the selected row index to the given one. Note: if this grid has
   * checks, the selected line is used to scroll to mark the line that may be
   * checked; if the grid does not have checks, then it acts like a normal
   * selected line.
   */
  ...
  { // guich@555_7
    ...
    if (0 <= row &&
        row < itemsCount) { // guich@555_7: removed Math.min(itemsCount,
                            // linesPerPage), otherwise it will only select the
                            // first items
  ...
  public int size() { return itemsCount; }
  ...
  @Override public Control handleGeographicalFocusChangeKeys(KeyEvent ke)
  {
    if (!ke.isUpKey() && !ke.isDownKey()) {
      return (ke.key == SpecialKeys.LEFT || ke.key == SpecialKeys.RIGHT) &&
          horizontalScroll(ke.key == SpecialKeys.LEFT)
        ? this
        : null;
    }
    if ((ke.isUpKey() && selectedLine <= 0) ||
        (ke.isDownKey() && selectedLine == itemsCount - 1)) {
      return null;
    }
    _onEvent(ke);
    return this;
  ...
  /**
   * Sets the number of visible lines (excluding the caption), used to make
   * PREFERRED height return the given number of lines as the grid height. This
   * method must be called before setRect.
   * @since TotalCross 1.0
   */
  ...
  /**
   Sets an image to be used in the Grid. You must use the same tag in the
   grid's item.
  ...
  @Override public Control moveFocusToNextControl(Control c, boolean forward)
  { // guich@tc125_26
    if (c instanceof Edit) {
      int col = (c.appId >> 24 & 127);
      int row = (c.appId & 0xFFFFFF) + (forward ? 1 : -1);
      if (0 <= row && row < itemsCount &&
          (cc == null || cc.isEnabled(row, col))) {
        new NextEdit(col, row).start();
        return c;
      } else {
        col += (forward ? 1 : -1);
        if (0 <= col && col < widths.length && controls[col] != null &&
            controls[col] instanceof Edit) {
