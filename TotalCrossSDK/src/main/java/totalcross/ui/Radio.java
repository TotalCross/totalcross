 ...
 *    {
 *       if (event.type == ControlEvent.PRESSED && (event.target instanceof
 * Radio) && ((Radio)event.target).getRadioGroup() == rgGender)
 *       {
  ...
  extends Control implements TextControl, MaterialEffect.SideEffect
{
  ...
  /**
   * Set to true to left justify this control if the width is above the
   * preferred one. <p><b>Attention:</b> You can change the gap between the
   * radio and the text by changing the variable "radioTextGap" by calling
   * <i>setRadioTextGap(int gap)</i></p>
   */
  ...
  /**
   * Creates a radio control with the given text attached to the given
  ...
  /**
   * Returns the RadioGroupController that this radio belongs to, or null if
   * none.
   */
  public RadioGroupController getRadioGroup() { return radioGroup; }
  ...
  /**
   * "Merge" the colors between the original grayscale image and the current
   * foreground.
   */
  private Image getImage(boolean isSelected) throws Exception
  {
    ...
    String key = (isSelected ? "*" : "") + foreColor + "|" + backColor + "|" +
                 fmH +
                 (isEnabled() ? "*" : ""); // guich@tc110a_110: added backColor.
    Image img;
    if (imgs == null) {
    ...
    img = isSelected ? vistaSelected.getFrameInstance(0)
                     : vistaUnselected.getFrameInstance(0);
  ...
  {
    return getFont().fm.stringWidth(text) + /*Radio symbol width*/ fmH +
      Edit.prefH + 2;
  }
  ...
  private final static int coords2[] = {
    5,  0,  9,  0,  3,  1,  4,  1,
    10, 1,  11, 1,  12, 2,  12, 2, // dark grey top
    0,  5,  0,  9,  1,  3,  1,  4,
    1,  10, 1,  11, 2,  2,  2,  2, // dark grey left
    5,  1,  9,  1,  3,  2,  4,  2,
    10, 2,  11, 2,  12, 3,  12, 3, // black top
    1,  5,  1,  9,  2,  3,  2,  4,
    2,  10, 2,  11, 2,  10, 2,  11, // black left
    3,  12, 4,  12, 5,  13, 9,  13,
    10, 12, 11, 12, 10, 12, 11, 12, // light grey bottom
    12, 10, 12, 11, 13, 5,  13, 9,
    12, 4,  12, 4,  12, 4,  12, 4, // light grey right
    2,  12, 2,  12, 3,  13, 4,  13,
    5,  14, 9,  14, 10, 13, 11, 13, // bottom white
    12, 12, 12, 12, 13, 10, 13, 11,
    14, 5,  14, 9,  13, 3,  13, 4 // right white
  };
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    int hh = Math.min(width - (textW + (lines.length > 1 ? 2 : -6)),
                      uiMaterial
                        ? (fmH - UnitsConverter.toPixels(4 + DP)) * lines.length
                        : height);
    if (hh == height) {
      hh -= Edit.prefH;
    }
    if (uiAndroid) {
      try {
        Image img =
          uiMaterial
            ? Resources.radioBkg.getPressedInstance(
                hh,
                hh,
                backColor,
                ...
                enabled)
            : enabled
                ? Resources.radioBkg.getNormalInstance(hh, hh, foreColor)
        ...
        if (checked || animating) {
          img = Resources.radioSel.getPressedInstance(
            hh,
            hh,
            backColor,
            checkColor != -1 ? checkColor : foreColor,
      ...
      g.drawImage(checked ? imgSel : imgUnsel,
                  0,
                  (height - imgSel.getHeight()) / 2); // guich@tc122_50: /2
    } else {
    ...
    g.foreColor =
      textColor != -1
        ? (enabled ? textColor : Color.interpolate(textColor, backColor))
        : cColor;
    for (int i = 0; i < lines.length; i++, yy += fmH) {
      int textMaxWidth = this.width - xx;
      String text = textMaxWidth > 0 && leftJustify
                      ? StringUtils.shortText(lines[i], font.fm, textMaxWidth)
                      : lines[i];
  ...
  private void drawBack(boolean big, boolean enabled, Graphics g)
  {
    ...
    int xx = 0; // guich@tc100: can't be -1, now we have real clipping that will
                // cut out if draw out of bounds
  ...
  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    if (autoSplit && this.width > 0 &&
        this.width !=
          lastASW) { // only if PREFERRED was choosen in first setRect
      lastASW = this.width;
  ...
  {
    displayedText =
      Convert.insertLineBreak(maxWidth,
                              fm,
                              text); // text cannot be assigned here or
                                     // originalText will be overwritten
    lines = text.equals("") ? new String[] { "" }
                            : Convert.tokenizeString(displayedText, '\n');
  }
  ...
  /**The gap between the radio and the text when the <i>leftJustify</i> is set
   * to true.*/
  public void setRadioTextGap(int gap) { this.radioTextGap = gap; }

  /**The gap between the radio and the text.*/
  public int getRadioTextGap() { return this.radioTextGap; }
}
