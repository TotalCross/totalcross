...
public class Radio extends Control implements TextControl, MaterialEffect.SideEffect {
	...
	private Image getImage(boolean isSelected) throws Exception {
		...
		String key = (isSelected ? "*" : "") + foreColor + "|" + backColor + "|" + fmH +
	...
	public void onPaint(Graphics g) {
		...
		int hh = Math.min(width - (textW + (lines.length > 1 ? 2 : -6)),
						  uiMaterial ? (fmH - UnitsConverter.toPixels(4 + DP)) * lines.length : height);
		if (hh == height) {
			hh -= Edit.prefH;
		}
		if (uiAndroid) {
			try {
				Image img = uiMaterial
							? Resources.radioBkg.getPressedInstance(hh, hh, backColor,
				...
				if (checked || animating) {
					img = Resources.radioSel.getPressedInstance(hh, hh, backColor,
							checkColor != -1 ? checkColor : foreColor,
							enabled);
			...
			g.drawImage(checked ? imgSel : imgUnsel, 0,
		...
		g.foreColor = textColor != -1 ? (enabled ? textColor : Color.interpolate(textColor,
										 backColor)) : cColor;
		for (int i = 0; i < lines.length; i++, yy += fmH) {
			int textMaxWidth = this.width - xx;
			String text = textMaxWidth > 0
						  && leftJustify ? StringUtils.shortText(lines[i], font.fm, textMaxWidth) : lines[i];
	...
	protected void onBoundsChanged(boolean screenChanged) {
		if (autoSplit && this.width > 0
			&& this.width != lastASW) { // only if PREFERRED was choosen in first setRect
			lastASW = this.width;
	...
	public void split(int maxWidth) {
		displayedText = Convert.insertLineBreak(maxWidth, fm,
												text); // text cannot be assigned here or originalText will be overwritten
