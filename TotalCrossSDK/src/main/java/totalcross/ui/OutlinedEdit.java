...
public class OutlinedEdit extends Edit implements UpdateListener {
	...
	protected void draw(Graphics g) {
		...
		g.drawText(caption, materialCaption.xcap , materialCaption.ycap - materialCaption.getFcap().fm.descent);
	}
	...
	public void setText(String s) {
		...
		lineRight = new Line((textSpace >> 1) + UnitsConverter.toPixels(DP + 10), holeY, textSpace >> 1, borderHeight);
	}
	...
	private void setupBorder() {
		...
		if (lineColor == -1) {
			...
			if (colors[position * 4 + 3] < 0 || colors[position * 4 + 3] > 10) { // When the alpha isn't between 0 and 10, inclusive,  there is some color a this pixel, which
			...
			if(colors[position * 4 + 3] < 0 || colors[position * 4 + 3] > 10) {            // When the alpha is low enough it gets the parent's color to give the trasnsparency. 
	...
	public void updateListenerTriggered(int elapsedMilliseconds) {
		...
		if (totalTime >= executedTime) {
			if (isPlaying) {
				lineLeft = new Line((textSpace >> 1) + UnitsConverter.toPixels(DP + 10) - lineWidth, holeY, lineWidth,
						borderHeight);
