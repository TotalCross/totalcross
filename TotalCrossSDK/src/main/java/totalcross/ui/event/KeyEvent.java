...
{
  /**
   * The event type for a key press event. Device keys are handled in the
   * SPECIAL_KEY_PRESS event.
   */
  ...
  public static final int ACTION_KEY_PRESS =
    EventType.ACTION_KEY_PRESS; // guich@550_33
  /**
   * The event type for a device key press event. Note that some keys are
   * posted only when they are released.
   */
  ...
  /**
   * Creates a new instance of KeyEvent with the specified type, key and
   * modifiers.
   *
   * @param type
   *           the KeyEvent type, must be either <code>KEY_PRESS</code>,
   * <code>ACTION_KEY_PRESS</code> or <code>SPECIAL_KEY_PRESS</code>
   * @param key
   *           the input key
   * @param modifiers
   *           state of the modifier keys for this event
   * @throws IllegalArgumentException
   *            if the type argument is not equal to <code>KEY_PRESS</code>,
   * <code>ACTION_KEY_PRESS</code> or <code>SPECIAL_KEY_PRESS</code>.
  ...
  /**
   * Returns true if the key press is any kind of the possible ones that means
   * UP.
   * @since SuperWaba 5.5
   */
  ...
  /**
   * Returns true if the key press is any kind of the possible ones that means
   * forward (TAB, PAGE_DOWN, DOWN, RIGHT, etc).
   * @since SuperWaba 5.5
   */
  ...
  @Override public void dispatch(KeyListener listener)
  {
    switch (this.type) {
      case KEY_PRESS:
