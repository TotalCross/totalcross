...
public class ListContainerEvent extends Event<ListContainerListener>
{
  ...
  public static final int ITEM_SELECTED_EVENT =
    EventType.LIST_CONTAINER_ITEM_SELECTED_EVENT;
  /**
  ...
  public static final int LEFT_IMAGE_CLICKED_EVENT =
    EventType.LIST_CONTAINER_LEFT_IMAGE_CLICKED_EVENT;
  /**
   * Event generated when the right image was clicked.
   * Verify the isImage2 member to determine the current image that's displayed.
   */
  public static final int RIGHT_IMAGE_CLICKED_EVENT =
    EventType.LIST_CONTAINER_RIGHT_IMAGE_CLICKED_EVENT;

  /**
   * True if the new image is the second one. If there are two images, one
   * as an unselected image and a second one as the selected, using this field
   * you can find which one is being displayed.
   */
  public boolean isImage2;

  /**
   * The control that originated the event. When the user clicks on an Image,
   * the <code>target</code> is the Image, and the source is the control where
   * the image is.
   */
  ...
  @Override public void dispatch(ListContainerListener listener)
  {
    switch (this.type) {
      case ITEM_SELECTED_EVENT:
