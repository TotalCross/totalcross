...
/**
 * Check is a control with a box and a check inside of it when the state is
 * checked. <p> Here is an example showing a check being used:
 *
 * <pre>
...
public class Check
  extends Control implements TextControl, MaterialEffect.SideEffect
{
  private String text;
  ...
  /**
   * Set to true to left-justify the text in the control. The default is
   * right-justified, if the control's width is greater than the preferred one.
  ...
  /**
   * Set to the color of the check, if you want to make it different of the
   * foreground color.
  ...
  /**
   * Set to true to let the Check split its text based on the width every time
  ...
  /**Sets the size of the check. Changing its value will only have effect if the
   * variable "autoResizeCheckSize" is false.
   * @since TotalCross 5.0*/
  ...
  /**
   * Creates a check control. The size is controlled by the height of the
   * element. You can change it by using the "setRect" or "add" methods.
   */
  public Check() { this(""); }

  /**
   * Creates a check control displaying the given text. The size is controlled
   * by the height of the element. You can change it by using the "setRect" or
   * "add" methods.
   */
  ...
  /**
   * Sets the checked state of the control, and send the press event if
   * desired.
   */
  ...
  @Override public int getPreferredWidth()
  {
    ...
    return Math.max(1,
                    checkSize +
                      (maxTextWidth == 0 ? 0 : maxTextWidth + textLeftGap) +
                      insets.left * 3 / 2 + insets.right * 3 / 2);
  }

  /**Returns the preferred height of this control.*/
  @Override public int getPreferredHeight()
  {
    return (uiMaterial ? checkSize
                       : (fmH * lines.length > checkSize ? fmH * lines.length
                                                         : checkSize)) +
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (!uiAndroid && uiVista && enabled) {
      ...
      if (uiMaterial) {
        g.fillRect(insets.left,
                   yy + insets.top - insets.bottom,
                   checkSize,
                   checkSize); // guich@220_28
      } else if (!uiAndroid) {
    ...
    if (uiAndroid) {
      ...
      try {
        Image img = uiMaterial
                      ? Resources.checkBkg.getPressedInstance(
                          checkSize,
                          checkSize,
                          backColor,
                          enabled ? (checkColor != -1 ? checkColor : foreColor)
                                  : disabledColor,
                          enabled)
                      : enabled ? Resources.checkBkg.getNormalInstance(
                                    checkSize,
                                    checkSize,
                                    checkColor != -1 ? checkColor : foreColor)
                                : Resources.checkBkg.getDisabledInstance(
                                    checkSize, checkSize, disabledColor);
        img.alphaMask = alphaValue;
        if (!uiMaterial || !checked || animating) {
          NinePatch.tryDrawImage(
            g, img, insets.left, yy + insets.top - insets.bottom);
        }
        img.alphaMask = 255;
        if (checked || animating) {
          img = Resources.checkSel.getPressedInstance(
            checkSize,
            checkSize,
            backColor,
            enabled ? (checkColor != -1 ? checkColor : foreColor)
      ...
      g.draw3dRect(0,
                   ...
                   false,
                   false,
                   fourColors); // guich@220_28//
    }
    g.foreColor =
      checkColor != -1 ? checkColor : uiAndroid ? foreColor : cfColor;
    ...
    for (int i = 0; i < lines.length; i++, yy += fmH) {
      String text = StringUtils.shortText(
        lines[i], font.fm, this.width - xx - insets.right);
      g.drawText(text, xx, yy, textShadowColor != -1, textShadowColor);
    }
  }

  /**
   * Paints a check in the given coordinates. The g must have been translated
   * to destination x,y coordinates.
   * @since SuperWaba 5.5
   * @param g The desired Graphics object where to paint. The forecolor must
   *     already be set.
   * @param fmH The fmH member
   * @param height The height of the control. The check will be vertical aligned
  ...
  { // guich@550_29
    if (uiAndroid) {
      try {
        g.drawImage(Resources.checkSel.getPressedInstance(
                      height, height, 0, g.foreColor, true),
                    0,
                    0);
  ...
  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    ...
    //    int preferredWidth = insets.left + this.height - insets.top -
    //    insets.bottom + (getMaxTextWidth() == 0 ? 0 : textLeftGap +
    //    getMaxTextWidth()) + insets.right; if(this.width < preferredWidth)
    //    	this.width = preferredWidth;
  }

  @Override public void sideStart() { animating = true; }
