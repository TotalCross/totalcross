...
public class Check extends Control implements TextControl, MaterialEffect.SideEffect {
	...
	public int getPreferredWidth() {
		...
		return Math.max(1, checkSize + (maxTextWidth == 0 ? 0 : maxTextWidth + textLeftGap) + insets.left *
						3 / 2 + insets.right * 3 / 2);
	}

	/**Returns the preferred height of this control.*/
	@Override
	public int getPreferredHeight() {
		return (uiMaterial ? checkSize : (fmH * lines.length > checkSize ? fmH * lines.length : checkSize))
		...
		if (uiAndroid) {
			...
			try {
				Image img = uiMaterial
							? Resources.checkBkg.getPressedInstance(checkSize, checkSize, backColor,
									enabled ? (checkColor != -1 ? checkColor : foreColor) : disabledColor,
									enabled)
							: enabled ? Resources.checkBkg.getNormalInstance(checkSize, checkSize,
				...
				if (checked || animating) {
					img = Resources.checkSel.getPressedInstance(checkSize, checkSize, backColor,
							enabled ? (checkColor != -1 ? checkColor : foreColor) : disabledColor,
							enabled);
			...
			g.draw3dRect(0, 0, checkSize, checkSize, Graphics.R3D_CHECK, false, false,
	...
	public void split(int maxWidth) { // guich@tc114_73
		...
		setText(Convert.insertLineBreak(maxWidth, fm,
										text)); // guich@tc126_18: text cannot be assigned here or originalText will be overwritten
		originalText = text;
	}
	...
	protected void onBoundsChanged(boolean screenChanged) {
		if (autoSplit && this.width > 0
			&& this.width !=
			lastASW) { // guich@tc114_74 - guich@tc120_5: only if PREFERRED was choosen in first setRect - guich@tc126_35
			lastASW = this.width;
