	...
	private Image getHandleImage() {
		...
		try {
			if (npPartsHandle == null) {
				...
				img = NinePatch.getInstance().getNormalInstance(npPartsHandle, fmH * 2, dragBarSize, backColor,
						false);
		...
		if (isEnabled() || !autoHide) {
			...
			if (uiAndroid) {
				...
				if (isHandle() && showHandle) {
					NinePatch.tryDrawImage(getGraphics(), handle, 0,
										   dragBarPos); // when the button is pressed, the graphic's clip becomes invalid
				} else if (npback != null) {
