 ...
 * finger-touched devices. This special scrollbar is just a small position
 * indicator that appears when the area is dragged. The ScrollPosition does not
 * take an area of the control, since it appears and disappears automatically.
 *
 * All Scrollable controls change their ScrollBar by the ScrollPosition when
 * Settings.fingerTouch is true.
 *
 * If the back color and the bar color are the same, the bar is not drawn; this
 * is how the ButtonMenu class hides this control.
 *
...
public class ScrollPosition
  extends ScrollBar implements Scrollable, PenListener, TimerListener
{
  ...
  /**
   * Set to false to make the PositionBar always show (instead of the default
   * auto-hide behaviour).
   */
  public static boolean AUTO_HIDE = true;

  /**
   * By default, the ScrollPosition is shown during 1 second to let the user
   * know that he can scroll. This value is a number that will be multiplied by
   * 500. Defaults to 2, set to 0 to disable this feature.
   * @since TotalCross 3.07
   */
  public static int VISIBLE_COUNT = 2;

  /**
   * The bar color. Defaults to UIColors.positionbarColor but can be changed to
   * something else.
   */
  public int barColor = UIColors.positionbarColor;

  /**
   * Defines the height multiplier that must be reached to show the handle
   * while scrolling. The handle speeds up scrolling since the user can drag it
   * (like the bar in a ProgressBar). It is very useful for long lists.
   *
   * Setting this to 10 will make the handle appear when the height of the
   * item's list exceeds 10 times the height of the ScrollContainer.
   *
   * Set it to 0 will always show the handle, set to something very big (like
   * Convert.MAX_INT) to never show the handle.
   *
   * Defaults to 7.
   *
  ...
  public ScrollPosition() { this(VERTICAL); }
  ...
  @Override public void onColorsChanged(boolean b)
  {
  ...
  private Image getHandleImage()
  {
    ...
    try {
      if (npPartsHandle == null) {
        img = Resources.progressHandle.getSmoothScaledInstance(fmH * 2,
                                                               dragBarSize);
        img.applyColor(barColor);
      } else
        img = NinePatch.getInstance().getNormalInstance(
          npPartsHandle, fmH * 2, dragBarSize, backColor, false);
    ...
    if (isEnabled() || !autoHide) {
      ...
      if (uiAndroid) {
        ...
        if (verticalBar) {
          if ((Flick.currentFlick != null || startDragPos != -1) &&
            ...
            if (dragBarSize == minDragBarSize) {
              ...
              setRect(RIGHT,
                      KEEP,
                      fmH * 2,
                      KEEP); // parameters will be recomputed
              return;
            }
           ...
           * 1. makes the drag of a ListBox cause a repaint even if the drag is
           below the height of a line
        ...
        if (npback == null ||
            ((verticalBar ? npback.getHeight() : npback.getWidth()) !=
             dragBarSize)) {
          if (npParts == null) {
            setNinePatch(
              verticalBar ? Resources.scrollposv : Resources.scrollposh, 3, 2);
          }
          try {
            if (verticalBar) {
              npback = NinePatch.getInstance().getNormalInstance(
                npParts, width, dragBarSize, barColor, false);
            } else {
              npback = NinePatch.getInstance().getNormalInstance(
                npParts, dragBarSize, height, barColor, false);
            }
        ...
        if (isHandle() && showHandle) {
          NinePatch.tryDrawImage(getGraphics(),
                                 handle,
                                 0,
                                 dragBarPos); // when the button is pressed, the
                                              // graphic's clip becomes invalid
        } else if (npback != null) {
          NinePatch.tryDrawImage(g,
                                 npback,
                                 verticalBar ? 0 : dragBarPos,
                                 verticalBar ? dragBarPos : 0);
        }
      } else {
        if (verticalBar) {
  ...
  {
    if (!atPenDown && internalVisibility && autoHide &&
        verticalBar == verticalScroll) {
      internalVisibility = false;
  ...
  @Override public int getScrollPosition(int direction) { return 0; }

  @Override public Flick getFlick() { return null; }
  ...
  {
    verticalScroll =
      e.direction == DragEvent.DOWN || e.direction == DragEvent.UP;
  ...
  public void setHandleVisibility(boolean visible) { showHandle = visible; }
  ...
  @Override public void timerTriggered(TimerEvent e)
  {
    if (timer != null && timer.triggered) {
      ...
      if (internalVisibility && autoHide && Flick.currentFlick == null &&
          !isPenDown) {
        resetHandle();
      }
      if (!isDisplayed()) {
