 ...
 *
 *  Check the ListContainerSample in the sdk for a bunch of ideas of what can be
 done with this component.
 *
 *  The ListContainer supports navigation using the keys up/down, page-up/down,
 and enter. The left and right
...
{
  /**
   * A set of fields and default fields that will be used to define the layout
   * of a ListContainer's Item.
   */
  public class Layout
  {
    /**
     * Specify what to do if the left or right controls are images.
     * There are two situations that occurs when the image has a different
     * height of the ListContainer's Item: <ol> <li> The image is smaller than
     * the ListContainer's Item height. It can be enlarged or vertically
     * centered. If the flag is false (default), the image will be centered.
     * Otherwise, a "smooth upscaled" image will be created, however the image
     * will mostly have a bad appearance. This is the worst choice. The best
     *  choice is always to have a big image (for the biggest possible
     * resolution of the device) that will always be scaled down. <li> The image
     * is bigger than the ListContainer's Item height. It will always be scaled
     * down, using a "smooth scale" algorithm. This is the best choice.
     * </ol>
     * In general way, the image never defines the height of the Item; its the
     * opposite: the number of Item lines is that defines the image height and
     * size.
     */
    public boolean leftImageEnlargeIfSmaller, rightImageEnlargeIfSmaller;
    /**
     * If the left and/or right control is a fixed Image, set it here and it
     * will be replicated on all lines.
     */
    public Image defaultLeftImage, defaultRightImage;
    protected int defaultLeftImageW, defaultLeftImageH, defaultRightImageW,
      defaultRightImageH;
    /**
     * If the left and/or right control is a fixed Image, set it here and it
     * will be replicated on all lines. These images can be set only if the
     * default image was set. The image size must be the same of the default
     * one.
     */
    ...
    /**
     * The default relative font sizes. You can specify a delta compared to the
     * font size of this ListContainer.
     * For example, specifying -1 will make the item have a font 1 size less
     * than the standard one.
     */
    public int[] relativeFontSizes;
    /**
     * The x position of the label, relative to the column's width.
     * Can be AFTER (default for all items), CENTER (relative to container's
     * width), CENTER_OF (relative to the space available after the last String
     * added), RIGHT (adjustments are NOT allowed!), BEFORE. The number of lines
     * of the Item is computed based on the column count. Note that this field
     * cannot be changed after the first Item is created, since the internal
     * computation of number of lines is done only once.
     */
    public int[] positions;

    /**
     * The font names to be used when creating the fonts. If the member is
     * null, it will use the parent's name.
     */
    public String[] fontNames;

    /**
     * The gap between the left/right controls and the text.
     * This gap is a <b>percentage</b> based in the control font's height. So,
     * if you pass 100 (default), it will be 100% of the font's height, 50 will
     * be 50% of the height, 150 will be 1.5x the font height, and so on.
     */
    public int controlGap = 100;
    /**
     * The gap between the image and the item's top/bottom. This is useful if
     * your item has 3 lines but you want to decrase the image's height.
     */
    public int imageGap;
    /**
     * The gap between the Item and the borders can be set using this field. The
     * values stored are not absolute pixels, but a percentage. Defaults to 0
     * (%) for all items.
     */
    public Insets insets;

    /**
     * The line spacing between two consecutive lines. Again, a percentage of
     * the font's height is used. Defaults to 0.
     * @see #controlGap
     */
    public int lineGap;

    /**
     * Set to true to center the labels vertically if there are empty lines
     * ("") before or after the items array. Only works if the items form a
     * single column.
     */
    ...
    public Layout(int itemCount, int itemsPerLine)
    {
      ...
      insets = new Insets((int)(Settings.screenDensity * 16),
    ...
    /**
     * After you set the properties, you must call this method to setup the
     * internal coordinates. If you create a Item before calling this method, a
    ...
    public void setup()
    {
      ...
      for (int i = 0, lineH = 0, col = 0, last = itemCount - 1; i <= last;
           i++) {
        Font f = fonts[i] =
          Font.getFont(fontNames[i] != null ? fontNames[i] : font.name,
                       boldItems[i],
        ...
        if (++col == itemsPerLine || i == last) {
          // adjust the y so that different font heights at the same line are
          // bottom-aligned
  ...
  public static class Item extends Container
  {
    ...
    /**
     * The Strings that will be displayed in the container. Individual items
     * cannot be null; pass "" instead to not display it.
     */
    public String[] items;
    /** The colors of all items. */
    private int[] itemColors;
    /**
     * When leftControl or rightControl is an Image, set this to false to don't
     * show it (and also disable controls)
     */
    ...
    /**
     * Constructs an Item based in the given layout. You must set the items
     * array with the strings that will be displayed. You may also set the
     * leftControl/rightControl and individual itemColors and boldItems.
     */
    ...
    /**
     * Returns the colors used on this Item. You can then set the individual
     * colors if you wish.
    ...
    {
      if (itemColors ==
          layout.defaultItemColors) { // still the default? create a new array
                                      // so user can change
    ...
    @Override public void initUI()
    {
      ...
      if (items.length != layout.itemCount) {
        throw new IllegalArgumentException(
          "Items have " + items.length + " but itemCount was specified as " +
      ...
      if (leftControl != null && leftControl instanceof Control) {
        ...
        c.setRect(
          LEFT + layout.insets.left * fmH / 100, CENTER, PREFERRED, PREFERRED);
      ...
      if (rightControl != null && rightControl instanceof Control) {
        ...
        c.setRect(RIGHT - layout.insets.right * fmH / 100,
                  CENTER,
                  PREFERRED,
                  PREFERRED);
        c.focusTraversable = true;
      ...
      if (e.type == PenEvent.PEN_DOWN || e.type == PenEvent.PEN_UP) {
        ...
        if (leftControl != null && leftControlVisible &&
          ...
          handleButtonClick(e.target, true);
                   ...
                   (e.target == rightControl ||
                    ((PenEvent)e).x >= getRightControlX())) {
    ...
    private void handleButtonClick(Object target, boolean isLeft)
    {
      ...
      // change images
      Image cur = c instanceof Image
                    ? (Image)c
                    : c instanceof Button ? ((Button)c).getImage() : null;
      if (cur != null) {
        Image img1 =
          isLeft ? layout.defaultLeftImage : layout.defaultRightImage;
        Image img2 =
          isLeft ? layout.defaultLeftImage2 : layout.defaultRightImage2;
        if (img1 != null &&
            img2 != null) { // if there are both images, swap them
      ...
      // flsobral: the target of the pen down event may be the left control
      // instead of an item from this list.
      if (!(target instanceof Item)) { // this if is just an optimization to
                                       // avoid starting an unnecessary loop.
      ...
      if (target instanceof
          Item) { // although unlikely, it is possible for the loop above to set
                  // target to an unknown type, this is a last check to avoid a
                  // ClassCastException
        Window.needsPaint = true;
        // change to the selected line so user can correctly find who was
        // pressed
        ListContainer lc = getLC();
        if (lc != null) {
          lc.setSelectedItem((Container)target);
        }
        postListContainerEvent(c,
                               target,
                               isLeft
                                 ? ListContainerEvent.LEFT_IMAGE_CLICKED_EVENT
                                 : ListContainerEvent.RIGHT_IMAGE_CLICKED_EVENT,
                               is2);
      }
    }
    ...
    public void postListContainerEvent(Object target,
                                       Object source,
                                       int type,
                                       boolean is2)
    {
    ...
    {
      return layout.itemH +
        (layout.insets.top + layout.insets.bottom) * fmH / 100;
    }
    ...
    public int getLeftControlX()
    {
      ...
      if (leftControl != null) {
        x1 +=
          (leftControl instanceof Control ? ((Control)leftControl).getWidth()
                                          : layout.defaultLeftImageW) +
          (int)(Settings.screenDensity * 16);
      }
      return x1;
    }
    ...
    public int getRightControlX()
    {
      ...
      if (rightControl != null) {
        x2 -=
          (rightControl instanceof Control ? ((Control)rightControl).getWidth()
                                           : layout.defaultRightImageW) +
    ...
    @Override public void onPaint(Graphics g)
    {
      ...
      // compute the area available for the text, excluding the left/right
      // controls
      ...
      if (leftControl != null && leftControl instanceof Image &&
          leftControlVisible) {
        g.drawImage((Image)leftControl,
                    layout.insets.left * fmH / 100,
                    (height - layout.defaultLeftImageH) / 2);
      }
      if (rightControl != null && rightControl instanceof Image &&
          rightControlVisible) {
        g.drawImage((Image)rightControl,
                    width - layout.defaultRightImageW -
                      layout.insets.right * fmH / 100,
      ...
      if (layout.centerVertically) {
        ...
        if (first != last) {
          deltaY += (firstH + lastH +
                     (first - 1 + last - 1) * layout.lineGap * fmH / 100) /
                    2;
        }
      }
  ...
  protected Container
    lastSel; // flsobral@tc126_65: ListContainer is no longer restricted to
             // accept only subclasses of ScrollContainer, any subclass of
             // Container may be added to a ListContainer.
  ...
  @Override public void onColorsChanged(boolean colorsChanged)
  {
  ...
  public void addContainer(Container c)
  {
    ...
    if (isSC) {
      ...
      if (sc.sbH != null || sc.sbV != null) {
        throw new RuntimeException("The given ScrollContainer " + c +
                                   " must have both ScrollBars disabled.");
      }
      sc.shrink2size = true;
    }
    if (drawHLine &&
        !(c instanceof
          Item)) { // make sure that the container has a gap for the border
      if (scInsets == null) {
        scInsets = new Insets();
      }
    ...
    add(c,
        ...
        AFTER,
        FILL,
        Math.max(c.getPreferredHeight(), (int)(Settings.screenDensity * 56)));
    if (isSC) {
      c.resize();
    }
    int n = tabOrder.size();
    if (n >= 2 && ((Control)tabOrder.items[n - 1]).y <
                    ((Control)tabOrder.items[n - 2]).y) {
  ...
  /**
   * Adds an array of Containers to this list. Adding hundreds of containers is
   * hundred times faster using this method instead of adding one at a time.
   *
   * Consider also to increase the value of
   * Flick.defaultLongestFlick BEFORE creating the ListContainer, otherwise the
   * user may take forever to flick. You can set it to 3 * all.length, if
   * all.length is above 1000.
  ...
  public void addContainers(Container[] all)
  {
    ...
    if (all.length > 0) {
      ...
      boolean isSC =
        all[0] instanceof
        ScrollContainer; // assume that if one is a ScrollContainer, all are.
      boolean isItem = all[0] instanceof Item;
      int s = bag.tabOrder.size();
      bag.tabOrder.setSize(
        s + all.length); // increase taborder's size to the final one
      if (!isItem) {
      ...
      for (int i = 0; i < all.length; i++) {
        ...
        c.y = AFTER;
        c.width = FILL;
        c.height = Math.max(c.getPreferredHeight(),
                            (int)(Settings.screenDensity *
                                  56)); // positions will be set later on resize
        if (isSC) {
          c.resize();
        }
        if (drawHLine && c.borderStyle == BORDER_NONE) {
    ...
    resize(); // all lines except this one take 10% of the method's time, and
              // this one takes 90%
  }
  ...
  @Override public void onEvent(Event e)
  {
    ...
    switch (e.type) {
      ...
      case PenEvent.PEN_UP:
        // case ControlEvent.FOCUS_IN: - will not work now that it can use keys
        // to traverse
          ...
          while (c != null) {
            if (c.asContainer != null && c.asContainer.containerId != 0) {
              ...
              if (c instanceof Item) {
                ((Item)c).postListContainerEvent(
                  this,
                  e.target,
                  ListContainerEvent.ITEM_SELECTED_EVENT,
                  false);
              } else {
                postPressedEvent();
              }
      ...
      case KeyEvent.SPECIAL_KEY_PRESS:
        if (vc.size() > 1) {
          Item sel =
            lastSelIndex != -1 && vc.items[lastSelIndex] instanceof Item
              ? (Item)vc.items[lastSelIndex]
              : null;
          KeyEvent ke = (KeyEvent)e;
          if (sel != null && ke.isActionKey()) {
            sel.postListContainerEvent(
              sel, e.target, ListContainerEvent.ITEM_SELECTED_EVENT, false);
          } else {
            switch (ke.key) {
              case SpecialKeys.UP:
                scroll(-1);
  ...
  public Container getSelectedItem() { return lastSel; }

  /** Returns the selected index, or -1 if none is selected. */
  public int getSelectedIndex() { return lastSelIndex; }
  ...
  public void setSelectedItem(Container c)
  {
    ...
    c.setBackColor(
      hc); // flsobral@tc126_70: highlight the whole selected container
    Window.needsPaint = true;
  ...
  /**
   * Changes the color of all controls inside the given container that matches
   * the background color of this ListContainer.
   */
  public void setBackColor(Container c, int back)
  {
    ...
    for (Control child = c.children; child != null; child = child.next) {
      if (child.asContainer == null &&
          (child.backColor == lastSelBack || child.backColor == hc)) {
  ...
  @Override public void resize()
  {
    ...
    if (isItem) { // if Item, we know that all of them have the same width and
                  // height
  ...
  /**
   * Positions the given Container (that should be a control added to this
   * ListContainer) at the top of the list.
   */
  @Override public void scrollToControl(Control c)
  { // kmeehl@tc100
    if (c != null && sbV != null) {
      int yy = c.y;
      Control f = c.parent;
      while (f.parent != this) {
        ...
        f = f.parent;
        if (f == null) {
          return; // either c is not in this container, or it has since been
                  // removed from the UI
        }
      }
      // vertical
