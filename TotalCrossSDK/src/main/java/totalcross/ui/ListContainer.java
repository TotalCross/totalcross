...
public class ListContainer extends ScrollContainer {
	...
	public static class Item extends Container {
		...
		public int[] getColors() {
			if (itemColors ==
				layout.defaultItemColors) { // still the default? create a new array so user can change
		...
		private void handleButtonClick(Object target, boolean isLeft) {
			...
			if (!(target instanceof
				  Item)) { // this if is just an optimization to avoid starting an unnecessary loop.
			...
			if (target instanceof
				Item) { // although unlikely, it is possible for the loop above to set target to an unknown type, this is a last check to avoid a ClassCastException
				...
				postListContainerEvent(c, target,
									   isLeft ? ListContainerEvent.LEFT_IMAGE_CLICKED_EVENT : ListContainerEvent.RIGHT_IMAGE_CLICKED_EVENT,
									   is2);
			}
		}
		...
		public int getLeftControlX() {
			int x1 = 0;
			if (leftControl != null) {
				x1 += (leftControl instanceof Control ? ((Control) leftControl).getWidth() :
		...
		public int getRightControlX() {
			...
			if (rightControl != null) {
				x2 -= (rightControl instanceof Control ? ((Control) rightControl).getWidth() :
		...
		public void onPaint(Graphics g) {
			...
			if (leftControl != null && leftControl instanceof Image && leftControlVisible) {
				g.drawImage((Image) leftControl, layout.insets.left * fmH / 100,
			...
			if (rightControl != null && rightControl instanceof Image && rightControlVisible) {
				g.drawImage((Image) rightControl,
							width - layout.defaultRightImageW - layout.insets.right * fmH / 100,
	...
	protected Container
	lastSel; //flsobral@tc126_65: ListContainer is no longer restricted to accept only subclasses of ScrollContainer, any subclass of Container may be added to a ListContainer.
		...
		if (all.length > 0) {
			...
			for (int i = 0; i < all.length; i++) {
				...
				c.height = Math.max(c.getPreferredHeight(),
									(int)(Settings.screenDensity * 56));  // positions will be set later on resize
				if (isSC) {
					c.resize();
				}
				if (drawHLine && c.borderStyle == BORDER_NONE) {
	...
	public void onEvent(Event e) {
		...
		switch (e.type) {
			...
			case KeyEvent.SPECIAL_KEY_PRESS:
				if (vc.size() > 1) {
					Item sel = lastSelIndex != -1
							   && vc.items[lastSelIndex] instanceof Item ? (Item) vc.items[lastSelIndex] : null;
