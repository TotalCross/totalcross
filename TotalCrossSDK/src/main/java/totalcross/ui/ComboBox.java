...
/**
 * ComboBox is an implementation of a ComboBox, with the drop down window
 * implemented by the ComboBoxDropDown class. <p> Note: the color used in the
 * setBackground method will be used in the button only. The background color of
 * the control will be a lighter version of the given color.
 */

public class ComboBox
  extends Container implements MaterialEffect.SideEffect, HasValue<Object>
{
  ...
  private boolean wasOpen; // guich@tc152: fixed ComboBox problem of clicking on
                           // an open combobox was making it open again
  ...
  /**The padding between the left border and the text (or icon, if it's
   * available).
   *
   * @since TotalCross 5.0*/
   ...
   * Usually this value is 0, but sometimes you may wish set it to -1, to
   * unselect the ComboBox when clear is called.
   */
  public static int defaultClearValueInt;

  /**
   * The check color used to fill the radio button used in Android. Defaults to
   * the #757575 color while using UI Material.
   *
  ...
  /**
   * Set to false to don't use the PopupMenu when the user interface style is
   * Android. This affects all ComboBoxes. If you want to change a particular
   * ComboBox to use the standard popup list, but keep others with the
   * PopupMenu, you can do something like: <pre>
  ...
  public ComboBox() { this((Object[])null); }

  /** Creates a ComboBox with the given items */
  public ComboBox(Object[] items) { this(new ListBox(items)); }
  ...
  { // guich@340_36
    ...
    if (!uiMaterial) {
      if (uiAndroid) {
        ...
        btn =
          new ArrowButton(Graphics.ARROW_DOWN,
                          uiMaterial ? getArrowWidth() / 2 : getArrowWidth(),
                          Color.BLACK);
        btn.setBorder(Button.BORDER_NONE);
        if (uiVista) {
  ...
  void add(Control control, boolean dummy) { super.add(control); }
   ...
   */
  @Deprecated public void add(Object[] items, int startAt, int size)
  {
  ...
  /**
   * Adds an Object to the Listbox. This method is very slow if used in loop;
   * use the <code>add(Object[])</code> to add a bunch of objects instead.
   */
  ...
  public Object getItemAt(int i) { return pop.lb.getItemAt(i); }

  /** Returns the selected item of the ListBox */
  public Object getSelectedItem() { return pop.lb.getSelectedItem(); }

  /** Returns the position of the selected item of the ListBox */
  public int getSelectedIndex() { return pop.lb.selectedIndex; }

  /** Returns all items in this ComboBox */
  public Object[] getItems() { return pop.lb.getItems(); }
   ...
   * @since SuperWaba 4.01
   */
  public void setSelectedItem(Object name, boolean sendPress)
  { // guich@401_25
  ...
  /**
   * Select the given index, and optionally sends a PRESSED event. Choose "-1"
   * if you want to blank the ComboBox view box.
   */
  ...
  public int size() { return pop.lb.itemCount; }

  @Override public int getPreferredWidth()
  {
    if (uiMaterial) {
      return UnitsConverter.toPixels(104 + DP) + paddingLeft + paddingRight +
        insets.left + insets.right +
        (captionIcon != null && captionIcon.getWidth() >= preferredIconSize
           ? captionIcon.getWidth()
           : 0);
    }
    return Math.max(fm.stringWidth(caption == null ? "" : caption),
                    pop.getPreferredWidth()) +
      1 + insets.left + insets.right +
      (uiMaterial ? fmH * 3 / 2
                  : Settings.fingerTouch ? btn.getPreferredWidth() + 4 : 0);
  }

  @Override public void setNinePatch(Image img, int corner, int side)
  {
  ...
  {
    Image img;
    Image img0 = arrowStyle == ARROWSTYLE_PAGEFLIP ? Resources.comboArrow2
                                                   : Resources.comboArrow;
    int s = fmH;
    try {
      img = arrowStyle == ARROWSTYLE_PAGEFLIP
              ? img0.smoothScaledFixedAspectRatio(s, false)
  ...
  {
    if (uiMaterial) {
      return UnitsConverter.toPixels(32 + DP) + paddingTop + paddingBottom +
        insets.top + insets.bottom +
        (captionIcon != null && captionIcon.getHeight() >= preferredIconSize
           ? captionIcon.getHeight()
           : 0);
    }
    int ret =
      (pop.lb.itemCount > 0 && !isSupportedListBox() ? pop.lb.getItemHeight(0)
                                                     : fmH) +
      Edit.prefH + insets.top + insets.bottom;
    ;
  ...
  {
    boolean isString =
      pop.lb.itemCount > 0 && !(pop.lb.items.items[0] instanceof Image);
  ...
  /**
   * Sets the ihtForeColors and ihtBackColors for the ListBox used with this
   * ComboBox. Note that null is a valid value, used when you always want to use
   * the default color.
    ...
    if (uiMaterial) {
      ...
      switch (Settings.uiStyle) {
        ...
        default: // guich@573_6: both Flat and Vista use this
          btn.setRect(
            width - btnW - 3, 1, btnW + 2, height - 2, null, screenChanged);
          break;
      }
    }
      ...
      switch (event.type) {
        case PenEvent.PEN_DRAG:
          ...
          if (event.target == this && inside != armed && pop.lb.itemCount > 0) {
            armed = inside;
            if (uiAndroid) {
              Window.needsPaint = true; // guich@580_25: just call repaint
                                        // instead of drawing the cursor
            } else if (!uiMaterial) {
              btn.press(armed);
            }
          }
        ...
        case PenEvent.PEN_UP:
          ...
          if (event.target == this && !wasOpen &&
              (armed || isActionEvent(event))) {
            if (captionPress != null && caption != null &&
              ...
              armed = false;
              inside =
                isInsideOrNear(pe.x, pe.y); //  is a method of class Control
                                            //  that uses absolute coords
        ...
        case ControlEvent.WINDOW_CLOSED:
          if (event.target == pop) { // an item was selected?
            ...
            if (pop.lb.selectedIndex >= 0 &&
                ((!isMulti && (!Settings.sendPressEventOnChange ||
                               pop.lb.selectedIndex != selOnPopup)) ||
        ...
        case ControlEvent.PRESSED:
          if (!uiMaterial && event.target == btn && pop.lb.itemCount > 0) {
            btn.appId = this.appId; // guich@502_3: make the button have the
                                    // same appid than us.
            event.consumed = true; // kevinsmotherman@450_22: avoid passing this
  ...
  private boolean isSupportedListBox()
  {
    ...
    return cl.equals("totalcross.ui.ListBox") ||
      cl.equals("litebase.ui.DBListBox");
  }

  /** Pops up the ComboBoxDropDown */
  public void popup()
  {
    requestFocus(); // guich@240_6: avoid opening the combobox when its popped
                    // up and the user presses the arrow again - guich@tc115_36:
                    // moved from the event handler to here
    boolean isMultiListBox = pop.lb instanceof MultiListBox;
    if (_usePopupMenu && !isMultiListBox &&
        isSupportedListBox()) { // we don't support yet user-defined ListBox
                                // types yet
      if (pop.lb.itemCount == 0) {
        ...
        try {
          ...
          pm.popup();
          Event.clearQueue(
            PenEvent.PEN_UP); // prevent problem when user selects an item
                              // that is at the top of this ComboBox
      ...
      if (pop.lb.itemCount == 0) {
        ...
        pop.lb.requestFocus();
        isHighlighting = false; // kmeehl@tc100: allow immediate keyboard
  ...
  @Override protected void onColorsChanged(boolean colorsChanged)
  {
    ...
    if (colorsChanged) {
      if (uiAndroid && btn != null) {
        ...
        btn.setBackForeColors(uiVista ? Color.darker(bColor, 32)
                                      : uiFlat ? bColor : backColor,
                              foreColor);
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (!uiMaterial) {
        ...
        try {
          if (npback == null) {
            npback = NinePatch.getInstance().getNormalInstance(
              ...
              width,
              height,
              enabled ? bColor : Color.interpolate(bColor, parent.backColor),
              false);
          }
          ...
          g.setClip(2,
                    2,
                    width - btnW - (arrowStyle == ARROWSTYLE_PAGEFLIP ? 0 : 8),
                    height - 4);
        } catch (ImageException e) {
          e.printStackTrace();
        }
        ...
        try {
          if (npParts != null && npback == null) {
            npback = NinePatch.getInstance().getNormalInstance(
              ...
              width,
              height - 3,
              enabled ? (fillColor == -1 ? backColor : fillColor)
                      : disabledColor,
              false);
      ...
      g.setClip(paddingLeft,
                caption == null ? paddingTop : 1,
                width - paddingRight,
      ...
      g.foreColor = hasFocus ? (captionColor != -1 ? captionColor : foreColor)
                             : Color.getGray(foreColor);
      ...
      g.drawArrow(width - paddingRight - arrowSize / 2 - btnW / 2,
                  dy - btnW / 2,
                  btnW,
                  opened ? Graphics.ARROW_UP : Graphics.ARROW_DOWN,
                  false,
                  checkColor != -1 && hasFocus() ? checkColor
                                                 : getArrowColor());
  ...
  private int getX0()
  {
    if (uiMaterial) {
      return paddingLeft +
        (captionIcon == null ? 0 : captionIcon.getWidth() + iconGap);
    }
  ...
  protected void drawSelectedItem(Graphics g)
  {
    ...
    int textWidth =
      width - paddingLeft - paddingRight - arrowSize -
      (this.captionIcon == null ? 0 : iconGap + this.captionIcon.getWidth());
    //	g.drawText(StringUtils.shortText(pop.lb.getText(), font.fm, textWidth),
    // paddingLeft + (this.captionIcon == null ? 0 : iconGap +
    // this.captionIcon.getWidth()), 			height/2 + paddingTop -
    // paddingBottom
    // - fmH/2);

    pop.lb.drawSelectedItem(
      g,
      pop.lb.selectedIndex,
      getX0(),
      getTextY(),
      textWidth); // guich@200b4: let the listbox draw the item
    if (trickW) {
      pop.lb.width = 0;
    }
  }
  ...
  /**
   * Clears this control, selecting index clearValueInt (0 by default); uses
   * clearValueStr if set, instead.
   */
  @Override public void clear()
  { // guich@572_19
    final int clearValueStrIdx =
      clearValueStr != null ? indexOf(clearValueStr) : -1;
  ...
  public ListBox getListBox() { return pop.lb; }
  ...
  { // guich@tc113_2
    return setSelectedItemStartingWith(
      text, caseInsensitive, Settings.sendPressEventOnChange);
  }
    ...
    if (o != null && o instanceof String[]) {
      ...
      if (v.size() != s.length) {
        ...
        for (int i = s.length; --i >= 0;) {
          if ((s[i] == null && s[i] != v.items[i]) ||
              (s[i] != null && !s[i].equals(v.items[i]))) {
            return false;
          }
        }
        return true;
  ...
  @Override public void sideStart() {}

  @Override public void sideStop()
  {
    if (cancelPopup) {
      cancelPopup = false;
    } else {
  ...
  @Override public void sidePaint(Graphics g, int alpha) {}
  ...
  @Override public void setValue(Object value) { this.setSelectedItem(value); }
}
