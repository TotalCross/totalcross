...
public class ComboBox extends Container implements MaterialEffect.SideEffect, HasValue<Object> {
	...
	private boolean
	wasOpen; // guich@tc152: fixed ComboBox problem of clicking on an open combobox was making it open again
	...
	public ComboBox(ComboBoxDropDown userPopList) { // guich@340_36
		...
		if (!uiMaterial) {
			if (uiAndroid) {
				...
				btn = new ArrowButton(Graphics.ARROW_DOWN, uiMaterial ? getArrowWidth() / 2 : getArrowWidth(),
									  Color.BLACK);
				btn.setBorder(Button.BORDER_NONE);
				if (uiVista) {
	...
	public int getPreferredWidth() {
		...
		return Math.max(fm.stringWidth(caption == null ? "" : caption),
						pop.getPreferredWidth()) + 1 + insets.left
			   + insets.right + (uiMaterial ? fmH * 3 / 2 : Settings.fingerTouch ? btn.getPreferredWidth() + 4 :
								 0);
	}
	...
	public int getPreferredHeight() {
		...
		int ret = (pop.lb.itemCount > 0
				   && !isSupportedListBox() ? pop.lb.getItemHeight(0) : fmH) + Edit.prefH + insets.top
				  + insets.bottom;
		;
	...
	public void popup() {
		...
		if (_usePopupMenu && !isMultiListBox
			&& isSupportedListBox()) { // we don't support yet user-defined ListBox types yet
			if (pop.lb.itemCount == 0) {
				...
				try {
					...
					pm.popup();
					Event.clearQueue(
						PenEvent.PEN_UP); // prevent problem when user selects an item that is at the top of this ComboBox
	...
	public void onPaint(Graphics g) {
		...
		if (!uiMaterial) {
			...
			g.setClip(paddingLeft, caption == null ? paddingTop : 1, width - paddingRight,
			...
			g.drawArrow(width - paddingRight - arrowSize / 2 - btnW / 2, dy - btnW / 2, btnW,
						opened ? Graphics.ARROW_UP : Graphics.ARROW_DOWN, false,
	...
	protected void drawSelectedItem(Graphics g) {
		...
		pop.lb.drawSelectedItem(g, pop.lb.selectedIndex, getX0(), getTextY(),
	 ...
	 * @since TotalCross 1.13
	 */
	public boolean setSelectedItemStartingWith(String text, boolean caseInsensitive,
			boolean sendPress) { // guich@tc310
		int idx = pop.lb.selectedIndex;
