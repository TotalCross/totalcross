...
/**
 MultiButton is a control that displays a single line button with a set of
 titles.
...
public class MultiButton extends Control
{
  ...
  public MultiButton(String[] captions)
  {
    ...
    setNinePatch(Resources.multibutton, 5, 2);
  }
  ...
  @Override public void onBoundsChanged(boolean colorsChanged)
  {
    npback = null;
  }

  /**
   * Sets the selected index, or -1 to unset it. Only an enabled index can be
   * selected.
   */
  public void setSelectedIndex(int sel)
  {
    if (sel == -1 || (0 <= sel && sel < tits.length &&
                      (disabled == null || !disabled[sel]))) {
      this.sel = sel;
      repaintNow();
      postPressedEvent();
      if (!isSticky) {
  ...
  public int getSelectedIndex() { return sel; }
    ...
    try {
      ...
      if (npParts != null && npback == null) {
        ...
        npback = NinePatch.getInstance().getNormalInstance(
          npParts, width, height, c, false);
        npsel = NinePatch.getInstance().getPressedInstance(
          npback, backColor, selColor);
  ...
  @Override public int getEffectH() { return height; }
  ...
  @Override public int getEffectY() { return 0; }
  ...
  @Override public void onEvent(Event e)
  {
    switch (e.type) {
      case PenEvent.PEN_DOWN: {
        ...
        tempIndex =
          isInsideOrNear(pe.x, pe.y) ? pe.x / (width / tits.length) : -1;
        break;
      }
      case PenEvent.PEN_UP:
        if (isEnabled() && !hadParentScrolled()) {
          ...
          int sel =
            isInsideOrNear(pe.x, pe.y) ? pe.x / (width / tits.length) : -1;
          if (sel != this.sel && (sel == -1 || disabled == null ||
                                  (sel < disabled.length && !disabled[sel]))) {
