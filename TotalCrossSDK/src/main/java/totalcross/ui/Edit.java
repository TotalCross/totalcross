 ...
 *    }
 * </pre>
 * Important: if you wish to open a popup window after a FOCUS_OUT event has
 occured,
 ...
 * </pre>
 * A long click on an Edit will result in a menu with copy/paste options to be
 displayed.
 * @see #clipboardDelay
 */
...
public class Edit extends Control implements TextControl, HasValue<String>
{
    ...
    @Override public void updateListenerTriggered(int elapsedMilliseconds)
    {
      ...
      if (blinkTime > 350) {
        if (!isTopMost()) {
          ...
          /*                  if (editable && enabled && lastPenDown != -1 &&
             clipboardDelay != -1 && (Vm.getTimeStamp() - lastPenDown) >=
             clipboardDelay) if (showClipboardMenu())
                   {
                      event.consumed = true;
                      break;
                   }
  ...
  private static int xMins[] = {
    4, 1, 3, 3, 4, 4, UnitsConverter.toPixels(DP + 12)
  };
  ...
  /**
   * Sets the alignment of this Edit, which can be LEFT (default), CENTER or
   * RIGHT. Note that it will always edit at left, but on focus lost, it will
   * drawn aligned.
   ...
   * In Material UI, you must set the caption BEFORE you add this control to the
   * container; also remember to set the height to at least PREFERRED, it is 75%
   * taller.
   * @see #captionColor
   */
  ...
  /**
   * The padding between the captionIcon and the left border. If there's no
   * captionIcon this will do nothing.
   */
  ...
  /**
   * Set to true on Android devices to use the native numeric pad when mode is
   * set to CURRENCY. Note that the numeric keybaord will probably appear only
  ...
  private boolean wasFocusIn,
    wasFocusInOnPenDown; // jairocg@450_31: used to verify if the event was
                         // focusIn
  ...
  /**
   * Used to inform that a <i>copy</i> operation has been made. You can
   * localize this message if you wish.
   */
  public static String copyStr = "copy";
  /**
   * Used to inform that a <i>cut</i> operation has been made. You can localize
   * this message if you wish.
   */
  public static String cutStr = "cut";
  /**
   * Used to inform that a <i>paste</i> operation has been made. You can
   * localize this message if you wish.
   */
  public static String pasteStr = "paste";
  /**
   * Used to inform that a <i>replace</i> operation has been made. You can
   * localize this message if you wish.
   */
  public static String replaceStr = "replace";
  /**
   * Used to inform that a <i>command</i> operation has been made. You can
   * localize this message if you wish.
   */
  ...
  /**
   * Defines an optional value to be used in the CalculatorBox when the
   * keyboard type is KBD_NUMERIC or KBD_CALCULATOR. Replaces the decimal
   * separator / 00 char.
  ...
  /**
   * Defines the time that the user will have to press to see a popup menu with
   * copy/paste options. Set to -1 to disable it; defaults to 1500 (1.5 seconds)
   * in pen devices, -1 in finger devices (because in these devices . Also
   * affects MultiEdit.
  ...
  /**
   * used only to compute the preferred width of this edit. If the mask is
   * empty, the edit fills to width.
   */
  protected char[] mask;
  /**
   * Sets the capitalise settings for this Edit. Text entered will made as is,
  ...
  public static boolean removeFocusOnAction =
    true; // as per MBertrand - guich@580

  /**
   * Color to apply to the Edit when it has focus (works only on Android user
   * interface style). By default, there's only a blinking cursor.
   * @since TotalCross 1.3
   */
  public int focusColor = -1;

  /**
   * Use the NumericBox instead of the Calculator in all Edits that have mode
   * equal to CURRENCY. Note that you can set for each control by calling
   * <code>ed.setKeyboard(Edit.KBD_NUMERIC)</code>.
   * @since TotalCross 1.3
   */
  ...
  /**
   * Set to false if you don't want the cursor to blink when the edit is not
   * editable
   */
  ...
  /**
   * Keep the selection persistent; otherwise, it is reset if you change the
   * letter
   */
  ...
  /**
   * The Keyboard class (or the internal virtual keyboard) will be used for
   * this Edit.
   * To change the window colors that may open, make sure to see the UIColors
   * static attributes.
   */
  ...
  public static int cursorThickness =
    Math.max(Settings.screenWidth, Settings.screenHeight) > 2000
      ? 4
      : Math.max(Settings.screenWidth, Settings.screenHeight) > 1500
          ? 3
  ...
  /**
   * Construct an Edit with FILL as preferred width. Note that you cannot use
   * RIGHT or CENTER at the x coordinate if you use this constructor.
   */
  public Edit()
  {
    blinkTime = 0;
    ...
    if (uiAndroid) {
      try {
        setNinePatch(Resources.edit.scaledBy(Settings.screenDensity / 4,
                                             Settings.screenDensity / 4));
   ...
   * For example enable a 'numeric pad' on devices that has the 1 in the u
   * character, you can use this: <pre> ed.mapKeys("uiojklnm!.","1234567890");
   * </pre>
   * To make sure that lowercase characters are also handled, you should also
   * change the capitalise mode: <pre> ed.capitalise = Edit.ALL_LOWER;
   * </pre>
   * If you want to disable a set of keys, use the setValidChars method. Note
   * that mapKeys have precendence over setValidChars.
   * @param from The source keys. Must have the same length of <code>to</code>.
   *     Set to null to disable mapping.
   * @param to The destination keys. Must have the same length of
   *     <code>from</code>
   ...
   * */
  public FloatingLabel<Edit> getMaterialCaption() { return materialCaption; }

  /**
   * Set the number of decimal placed if a masked edit with CURRENCY mode.
   * Default is 2 decimal places. It cannot be used with masked Edits; pass the
   * number of decimal places in the mask itself. The only exception is when you
   * want to use the default CURRENCY mask, passing a null mask in the
   * constructor; in this situation, you can call setDecimalPlaces before
   * calling setMode, and a mask will be constructed with the given number of
   * decimals.
   */
  public void setDecimalPlaces(int count)
  {
    ...
    if (isMaskedEdit) {
      throw new RuntimeException(
        "Edit.setDecimalPlaces can't be used after the mask is applied using setMode.");
    }
   ...
   * To change the window colors that may open, make sure to see the UIColors
   * static attributes. Note that setMode calls setKeyboard(KBD_DEFAULT), so be
   * sure to set the mode before calling setKeyboard.
   * @see #KBD_NONE
   * @see #KBD_DEFAULT
   * @see #KBD_KEYBOARD
   * @see #KBD_CALCULATOR
  ...
  { // guich@310_19
    ...
    if (kbd == KBD_DEFAULT) {
      switch (mode) {
        ...
        case CURRENCY:
          kbdType =
            useNumericBoxInsteadOfCalculator ? KBD_NUMERIC : KBD_CALCULATOR;
          break;
        default:
  ...
  @Override protected void onFontChanged() { wildW = fm.charWidth('*'); }

  /** Returns the mask passed on the constructor. */
  public String getMask() { return mask == null ? "" : new String(mask); }

  /**
   * Used to set the valid characters that can be entered by using one of the
   * mode constants, <b>without</b> masking. To enable masking the input, you
   * have to call the setMode method passing the mode and <code>true</code> as
   * parameter. Note that setMode calls setKeyboard(KBD_DEFAULT), so be sure to
   * set the mode before calling setKeyboard.
   * @see #NORMAL
   * @see #DATE
   * @see #CURRENCY
   * @see #PASSWORD
   * @see #PASSWORD_ALL
   */
  public void setMode(byte mode) { setMode(mode, false); }
  ...
  /**
   * Used to set the valid characters that can be entered by using one of the
   * mode constants, optionally enabling the mask to be applied to the input.
   * Note that setMode calls setKeyboard(KBD_DEFAULT), so be sure to set the
   * mode before calling setKeyboard.
   * @see #NORMAL
   * @see #DATE
   * @see #CURRENCY
   * @see #PASSWORD
  ...
  public void setMode(byte mode, boolean maskedEdit)
  {
    ...
    switch (mode) {
      case DATE:
        setValidChars(maskedEdit ? numbersSet
                                 : (numbersSet + Settings.dateSeparator));
        if (maskedEdit) {
          maxLength = 8;
          mask = Settings.dateFormat == Settings.DATE_YMD
                   ? ("9999" + Settings.dateSeparator + "99" +
                      Settings.dateSeparator + "99")
                       .toCharArray()
                   : ("99" + Settings.dateSeparator + "99" +
                      Settings.dateSeparator + "9999")
                       .toCharArray();
        }
        break;
      case CURRENCY:
  ...
  {
    String s = decimalPlaces == 0
                 ? "999a999a999a999a999"
                 : ("999a999a999a999a999b" + Convert.dup('9', decimalPlaces));
   ...
   * If you pass "" (empty string), no chars will be able to be inputted, and
   * movement, delete and copy/paste operations will also be disabled.
   * @see #mapKeys
   */
  ...
  /**
   * Return true if the given char exists in the set of valid characters for
   * this Edit
   */
  protected boolean isCharValid(char c)
  {
    return validChars == null ||
      validChars.indexOf(Convert.toUpperCase(c)) != -1;
  }
 ...
 */
  ...
  protected void popPosState()
  {
    ...
    insertPos =
      Math.min(len,
               pushedInsertPos); // guich@571_5: make sure the insert position
                                 // isn't bigger than the size of the text.
  ...
  protected int getX0()
  {
    ...
    switch (alignment) {
      case RIGHT:
        ...
        if (captionIcon != null) {
          textStartX -=
            (captionIcon == null ? 0 : captionIcon.getWidth() + fmH / 4);
        }
        break;
      case CENTER:
        textStartX = (this.width - fm.sbWidth(s, 0, s.length())) / 2 - xOffset;
  ...
  protected int charPos2x(int n)
  {
    ...
    switch (mode) {
      ...
      case PASSWORD:
        return extra + xOffset + wildW * (n - 1) +
          fm.charWidth(chars, chars.length() - 1);
      case CURRENCY:
        ...
        if (masked.length() > 0) {
          ...
          return extra + xOffset +
            fm.sbWidth(
              ...
              Math.min(
                pos,
                masked.length())); // Math.min(pos,masked.length())); //
                                   // guich@tc152: changed mask to masked,
                                   // otherwise, using old font and 1's will
                                   // make the cursor appear incorrectly
        }
    }
  ...
  /**
   * Returns the text displayed in the edit control. If masking is enabled, the
   * text with the mask is returned; to get the text without the mask, use the
   * getTextWithoutMask method.
  ...
  /**
   * Returns the text without the mask. For non-currency mode, only chars whose
   * corresponding mask is '9' are returned.
   * @see #getText()
   */
    ...
    if (isMaskedEdit) {
      if (mode == CURRENCY) {
        if (!hasSignificantDigits()) {
          ...
          if (decimalPlaces > 0) { // for currency mode, remove the , and . and
                                   // put it in Java's format (xxxx.yy)
            int k =
              str.length() - decimalPlaces; // get the number of decimal places
            if (k <= 0) {
        ...
        if (mask.length ==
            str.length()) { // totally formatted? faster algorithm
          // 25/03/1970 -> 25031970
          for (int i = 0; i < mask.length; i++) {
            if (mask[i] == '9') {
  ...
  /**
   * Returns the text's buffer. Do NOT change the buffer contents, since
   * changing it will not affect the char widths array, thus, leading to a wrong
   * display.
   * @since TotalCross 1.0
   */
  public StringBuffer getTextBuffer() { return chars; }
  ...
  public void setText(String s, boolean postPressed)
  {
    ...
    if (s != null && (len = s.length()) > 0) {
      chars.append(s);
      if (mode == CURRENCY &&
          isMaskedEdit) { // correct the number if this is a numeric edit
        ...
        if (isNegative) {
          ...
          chars.setLength(0);
          chars.append(s);
        } // guich@tc168 - if user sends a negative value, remove it from start
          // and set the flag
        ...
        if (decimalPlaces == 0) { // setText("12.34") -> "12"
          ...
          chars.delete(delpos,
                       delpos + 1); // remove the . - guich@tc100b4_11: added +1
        }
      } else if (mode != CURRENCY && isMaskedEdit &&
  ...
  void fireValueChangeEvent()
  {
    ValueChangeEvent<String> e =
      new ValueChangeEvent<String>(this, this.getValue());
    e.target = this;
    postEvent(e);
  }
   ...
   */
  public void setEditable(boolean on) { focusTraversable = editable = on; }

  /** Gets if the control accepts input from the user */
  public boolean isEditable() { return editable; }

  public Rect oldBounds;
  @Override protected void onBoundsChanged(boolean screenChanged)
  { // guich
    xMin = xMins[Settings.uiStyle];
    if (captionIconHeightFactor != 0 && captionIcon != null) {
      try {
        captionIcon = captionIcon.smoothScaledFixedAspectRatio(
          height * captionIconHeightFactor / 100, true);
      } catch (ImageException e) {
        e.printStackTrace();
      }
    }
    if (captionIconHeightFactor != 0 && captionIcon != null) {
      try {
        captionIcon = captionIcon.hwScaledFixedAspectRatio(
          height * captionIconHeightFactor / 100, true);
        if (captionIcon.getWidth() >
            this.width - fm.stringWidth(chars.toString())) {
          captionIcon = captionIcon.hwScaledFixedAspectRatio(
            width - fm.stringWidth(chars.toString()), false);
        }
      } catch (Throwable t) {
      }
    }
    ...
    if (this.getRect().equals(oldBounds)) {
      ...
      if (materialCaption != null) {
        materialCaption.xcap0 = materialCaption.xcap =
          chars.length() == 0 ? xMin : 0;
        materialCaption.ycap0 = materialCaption.ycap =
          uiMaterial ? this.height / 2 - this.fmH / 2
                     : chars.length() == 0 ? getTextY() : 0;
        if (this instanceof OutlinedEdit) {
          OutlinedEdit oe = (OutlinedEdit)this;
          int labelAscentMiddleY =
            (materialCaption.getCaptionFontSmall().fm.ascent -
             oe.borderHeight) /
            2;
  ...
  @Override public int getPreferredWidth()
  {
    ...
    return (mask == null || useFillAsPreferred)
      ? FILL
      : (fm.stringWidth(new String(mask)) +
         (uiAndroid ? 10
                    : (uiFlat || uiVista)
                        ? 8
                        : 4)); // guich@200b4_202: from 2 -> 4 is PalmOS
                               // style - guic@300_52: empty mask means FILL
                               // - guich@570_88: fixed width when uiFlat
  }
  ...
  @Override public int getPreferredHeight()
  {
    ...
    if (uiMaterial) {
      if (caption != null) {
        ret = fmH + prefH > UnitsConverter.toPixels(DP + 56)
                ? ret + materialCaption.getExtraHeight()
                : UnitsConverter.toPixels(DP + 56);
      } else {
  ...
  @Override protected void onColorsChanged(boolean colorsChanged)
  {
    npback = null;
    fColor = getForeColor();
    back0 = UIColors.sameColors
              ? backColor
              : Color.brighter(getBackColor()); // guich@572_15
    back1 =
      back0 != Color.WHITE
        ? (UIColors.sameColors ? Color.darker(getBackColor()) : backColor)
        : Color.getCursorColor(back0); // guich@300_20: use backColor
                                       // instead of: back0.getCursorColor();
    if (!uiAndroid) {
  ...
  protected void draw(Graphics g)
  {
    ...
    if (drawLine) {
      if (uiMaterial) {
        final int lineHeight =
          Math.max(UnitsConverter.toPixels(DP + (hasFocus ? 2 : 1)), 1);
        int h = fmH / 10;
        if (h < 2) {
          h = 2;
        }
        ...
        if (isEnabled()) {
          ...
          if (npback != null) {
            ...
            npback.getPixelRow(
              colors,
              height - UnitsConverter.toPixels(DP + (hasFocus ? 2 : 1)));
    ...
    if (len > 0 || drawCaption || captionIcon != null) {
      ...
      if (captionIcon != null) {
        xx += getX0();
        g.drawImage(captionIcon,
                    uiMaterial ? captionIconPadding : fmH,
                    captionIconHeightFactor == 0
                      ? y
                      : height / 2 - captionIcon.getHeight() / 2);
      }
          ...
          case PASSWORD: // password fields usually have small text, so this
            ...
            if (len > 0) {
              g.drawText(Convert.dup('*', len - 1) + chars.charAt(len - 1),
                         xx,
                         y + (uiMaterial ? materialCaption.ycap0 - y : 0),
                         textShadowColor != -1,
                         textShadowColor);
            }
            break;
          ...
          default:
            ...
            if (masked.length() > 0) {
              ...
              g.drawText(chars,
                         ...
                         textY,
                         textShadowColor != -1,
                         textShadowColor); // xx wrong in this case?
            }
        }
      }
      if (hasBorder) {
      ...
      if (xMin <= cursorX && cursorX <= xMax) { // guich@200b4_155
        if (cursorShowing) {
          ...
          g.fillRect(cursorX - 1 +
                     ...
                     uiMaterial ? materialCaption.ycap0 + font.fm.descent : y,
                     cursorThickness,
                     fmH - font.fm.descent);
        }
      }
  ...
  private void applyMaskToInput()
  {
    ...
    if (len == 0) {
      isNegative = false;
    } else if ((mode == DATE || mode == NORMAL) &&
               isMaskedEdit) { // date must go forward
      ...
      if (pos < mask.length && mask[pos] != '9') {
        masked.append(mask[pos]);
               ...
               len > 0) { // currency must go backward
      ...
      if (hasSignificantDigits()) {
        if (decimalPlaces > 0) {
          int k =
            masked.length() - decimalPlaces; // get the number of decimal places
          if (k <= 0) {
            masked.append(Convert.zeroPad("", -k))
              .append(Settings.decimalSeparator)
              .append('0');
          }
        }
        if (isNegative) {
  ...
  /**
   * Sets the selected text of this Edit (if start != end). Can be used to set
   * the cursor position, if start equals end. Start must be less or equal to
   * end, and both must be >= 0. It can also be used to clear the selectedText,
   * calling <code>setCursorPos(-1,0)</code>. Note: if you're setting the cursor
   * position before the edit is drawn for the first time, the edit will not be
   * scrolled if the end position goes beyond the limits. Important! No bounds
  ...
  public void popupKCC()
  {
    ...
    if (!popupsHidden()) {
      ...
      if (Settings.fingerTouch && kbdType != KBD_TIME &&
          kbdType != KBD_CALENDAR && kbdType != KBD_CALCULATOR &&
          kbdType != KBD_NUMERIC) {
        return;
      }
    }
    ...
    if (w != null) {
      w.swapFocus(this); // requestFocus(); // guich@200b4: bring focus back -
                         // guich@401_15: changed to swapFocus
    }
    ...
    switch (kbdType) {
      case KBD_TIME:
        ...
        try {
          time.setTime(
            new Time(getText(), false, false, false, true, true, true));
        } catch (Exception e) {
      ...
      case KBD_CALENDAR:
        try {
          calendar = new CalendarBox(
            this.foreColor,
            new Date(getText())); // Reinitializing the window every time with
                                  // the actual date
          calendar.tempTitle = keyboardTitle;
          calendar.setSelectedDate(new Date(getText()));
        } catch (InvalidDateException ide) {
          calendar =
            new CalendarBox(this.foreColor); // if the date is invalid, start
                                             // the date with the system date
    ...
    if (!Settings.enableVirtualKeyboard) {
      ;
               ...
               (Settings.platform.equals(Settings.ANDROID) ||
                Settings.isIOS() || Settings.isWindowsCE())) {
      if (Settings.customKeyboard != null) {
        ...
        if (!Window.isSipShown() || lastWasNumeric != isNumeric) {
          ...
          Window.setSIP(
            onBottom ? Window.SIP_BOTTOM : Window.SIP_TOP,
            this,
            isNumeric); // if running on a PocketPC device, set the bounds of
                        // Sip in a way to not cover the edit
        }
      }
    } else {
  ...
  protected void shiftScreen(boolean force)
  {
    if (Settings.unmovableSIP &&
        (force || !Window.isSipShown())) { // guich@tc126_21
  ...
  {
    if (calendar != null && event.type == ControlEvent.WINDOW_CLOSED &&
        event.target ==
          calendar) { // called from the keyboard and from the calendar
      Date d = null;
    ...
    switch (event.type) {
      ...
      case ControlEvent.FOCUS_IN:
        isHighlighting = false; // guich@573_28: after closing a KCC, don't let
                                // the focus move from here.
        ...
        if (len > 0) { // guich@550_20: autoselect the text
          if (autoSelect &&
              !ignoreSelect) { // guich@570_112: changed to !ignoreSelect
            startSelectPos = 0;
            newInsertPos = len;
      ...
      case ControlEvent.FOCUS_OUT:
        if (cursorShowing) {
          Window.needsPaint = true; // draw(drawg=getGraphics(), true); // erase
                                    // cursor at old insert position
        }
        newInsertPos = 0;
        focusOut();
        break;
      ...
      case KeyEvent.SPECIAL_KEY_PRESS:
        if (editable && isEnabled()) {
          ...
          if (event.type == KeyEvent.SPECIAL_KEY_PRESS &&
              ke.key == SpecialKeys.ESCAPE) {
            event.consumed =
              true; // don't let the back key be passed to the parent
          }
          if (insertPos == 0 && ke.key == ' ' &&
              (mode == CURRENCY || mode == DATE)) { // guich@tc114_34
            popupKCC();
            break;
          }
          ...
          if (event.target == this &&
              moveFocus) { // guich@tc100b2: move to the next edit in the same
                           // container
            Control next;
            if (parent != null && (next = parent.moveFocusToNextEditable(
                                     this, ke.modifiers == 0)) != null) {
              shiftTo(next);
              return;
            }
          }
          ...
          if (event.target == this && loseFocus) {
            // isHighlighting = true; // kmeehl@tc100: set isHighlighting first,
            // so that Window.removeFocus() wont trample Window.highlighted
            if (removeFocusOnAction) {
              Window w = getParentWindow(); // guich@tc114_32: restore the
                                            // highlight to this control...
          ...
          // print state
          if ((ke.key == SpecialKeys.KEYBOARD_ABC ||
               ke.key == SpecialKeys.KEYBOARD_123) &&
              popupsHidden()) { // guich@102: pop's up the keyboard
            popupKCC();
            break;
          }
          boolean isControl =
            (ke.modifiers & SpecialKeys.CONTROL) != 0; // guich@320_46
          if (Settings
                .onJavaSE) { // guich@tc100b4_26: if the user pressed the
                             // command and then a key, assume is a control
            if (lastCommand > 0 && (Vm.getTimeStamp() - lastCommand) < 2500) {
              ke.modifiers |= SpecialKeys.CONTROL;
              isControl = true;
              lastCommand = 0;
            } else if (ke.key ==
                       SpecialKeys.COMMAND) { // just a single COMMAND? break
          ...
          boolean isPrintable = ke.key > 0 &&
                                event.type == KeyEvent.KEY_PRESS &&
                                (ke.modifiers & SpecialKeys.ALT) == 0 &&
          ...
          if (del1 >= 0 && del2 < len) {
            if (cursorShowing) {
              Window.needsPaint =
                true; // draw(drawg == null ? (drawg = getGraphics()) : drawg,
                      // true); // erase cursor at old insert position
            }
            if (len > del2 - 1) {
          ...
          if (isPrintable) {
            if (maxLength == 0 || len < maxLength ||
                clearSelect) { // guich@tc125_34
              char c = (char)ke.key;
              boolean append = true;
              if (isMaskedEdit &&
                  masked.length() >
                    0) { // put or remove '-' at the beginning of a string
                char first = masked.charAt(0);
                if (c == '+' || c == '-') {
                  if (first == '-') {
                    isNegative =
                      false; // typed + and is negative (or neg of neg)?
                  } else if (c == '+') {
        ...
        if (reapplyMask && mask != null &&
            (mode == CURRENCY || mode == DATE || mode == NORMAL)) {
          applyMaskToInput();
        }
        fireValueChangeEvent();
        break;
      case PenEvent.PEN_DOWN: {
        ...
        if (!autoSelect ||
            !wasFocusIn) { // jairocg@450_31: if the event was focusIn, do not
                           // change the selected text
          for (newInsertPos = 0; newInsertPos < chars.length() &&
                                 charPos2x(newInsertPos) < pe.x - 3;
               newInsertPos++) {
          }
          ...
          wasFocusIn = false; // guich@570_98: let the user change cursor
                              // location after the first focus_in event.
        }
        break;
      }
      case PenEvent.PEN_DRAG: {
        PenEvent pe = (PenEvent)event;
        for (newInsertPos = 0;
             newInsertPos < chars.length() && charPos2x(newInsertPos) <= pe.x;
             newInsertPos++) {
        }
      ...
      case PenEvent.PEN_UP: {
        PenEvent pe = (PenEvent)event;
        if (captionPress != null && caption != null &&
            pe.y <= materialCaption.getExtraHeight()) {
          captionPress.onCaptionPress();
        } else if (captionPress != null && captionIcon != null &&
                   pe.x <= captionIcon.getWidth()) {
          captionPress.onIconPress();
        } else {
          if (kbdType != KBD_NONE && virtualKeyboard && !hadParentScrolled()) {
            if (!autoSelect && clipboardDelay != -1 && startSelectPos != -1 &&
      ...
      case KeyboardBox.KEYBOARD_ON_UNPOP: // guich@320_34
        pushPosState();
        ignoreSelect =
          true; // guich@570_112: don't autoselect when keyboard is called
        return;
      case KeyboardBox.KEYBOARD_POST_UNPOP:
        popPosState();
        if (oldTabIndex !=
            -1) { // reinsert this control in the previous position
        ...
        ignoreSelect =
          false;            // guich@570_112: after the keyboard is definetively
                            // gone, its safe to set this to false.
        wasFocusIn = false; // guich@570_112: fix first click on edit being
                            // ignored after keyboard is closed.
        startSelectPos = -1;
        return;
      default:
        return;
    ...
    if (!clearSelect && wasFocusIn && startSelectPos != -1 &&
        insertPos > startSelectPos) {
      wasFocusIn = false;
    ...
    boolean insertChanged =
      event.type == ControlEvent.CURSOR_CHANGED || (newInsertPos != insertPos);
    if (reapplyMask && mask != null &&
        (mode == CURRENCY || mode == DATE || mode == NORMAL)) {
      applyMaskToInput();
    }
    ...
    if (isTopMost()) {
      Window.needsPaint =
        true; // must repaint everything due to a possible background image
    }
  }

  protected void shiftTo(Control next)
  {
    for (Container p = parent; p != null;
         p = p.parent) { // if next is inside a ScrollContainer, scroll to it
    ...
    if (next instanceof Edit) {
      ((Edit)next)
        .shiftScreen(
          true); // update screen shift position when user press ENTER
    }
  }
  ...
  private boolean showClipboardMenu()
  {
    ...
    if (0 <= idx && idx <= 3) {
      if (idx != 3 &&
          startSelectPos == -1) { // if nothing was selected, select everything
        startSelectPos = 0;
  ...
  {
    @Override public void controlPressed(ControlEvent e)
    {
  ...
  static int showClipboardMenu(Control host)
  {
    try {
      if (clipboardMenu == null) {
        ...
        clipboardMenu = new PushButtonGroup(
          names, false, -1, 0, 3, 2, true, PushButtonGroup.BUTTON) {
          @Override protected boolean willOpenKeyboard() { return true; }
        };
      ...
      w.add(clipboardMenu,
            LEFT + 2,
            host instanceof MultiEdit
              ? cli.y + cli.height - ph
              : (cli.y > w.height / 2 ? cli.y - ph : cli.y + cli.height),
            PREFERRED + 4,
            PREFERRED + 4,
            host);
      clipboardMenu.setBackForeColors(UIColors.clipboardBack,
                                      UIColors.clipboardFore);
      ...
      int end =
        Vm.getTimeStamp() + 3000; // make sure we will elapse only 3 seconds
  ...
  @Override protected void onWindowPaintFinished()
  {
    if (!hasFocus) {
      _onEvent(
        new ControlEvent(ControlEvent.FOCUS_IN,
                         this)); // this event is called on the focused control
                                 // of the parent window. so, if we are not in
                                 // FOCUS state, set it now. - guich@tc112_
    }
  }

  /** Called by the system to draw the edit control. */
  @Override public void onPaint(Graphics g) { draw(g); }
  ...
  /**
   * Clears this control, settings the text to clearValueStr. Note that if the
   * Edit
   * is not editable, you will have to explicitly call the clear method of this
   * Edit.
   */
  ...
  @Override public Control handleGeographicalFocusChangeKeys(KeyEvent ke)
  { // kmeehl@tc100
    if (ke.isUpKey() || ke.isDownKey() ||
        (ke.isNextKey() && insertPos == chars.length()) ||
  ...
  { // guich@tc114_66
    ...
    if (sel1 != -1) { // cut/copy
      Vm.clipboardCopy(chars.toString().substring(
        sel1,
        sel2)); // brunosoares@tc100: Changed from chars.substring to
                // chars.toString().substring
  ...
  { // guich@tc114_66
    ...
    if (pasted == null || pasted.length() == 0) {
      ...
      KeyEvent ke = new KeyEvent();
      if (startSelectPos !=
          insertPos) { // if a text is selected, replace the value
  ...
  /**
   * Returns a copy of this Edit with almost all features. Used by Keyboard and
   * SIPBox classes.
   * @since TotalCross 1.27
   */
  public Edit getCopy()
  {
    ...
    boolean maskState =
      isMaskedEdit; // Saving the isMaskedEdit state so it can be set to false
                    // and setDecimalPlaces can be called
  ...
  @Override protected boolean willOpenKeyboard()
  {
  ...
  @Override public void setValue(String value) { this.setText(value); }

  public int getCaptionIconHeightFactor() { return captionIconHeightFactor; }
  ...
  public int getCaptionIconPadding() { return captionIconPadding; }
