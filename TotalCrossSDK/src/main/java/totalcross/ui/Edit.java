...
public class Edit extends Control implements TextControl, HasValue<String> {
	...
	private boolean wasFocusIn,
			wasFocusInOnPenDown; // jairocg@450_31: used to verify if the event was focusIn
	...
	public static char[] getDefaultCurrencyMask(int decimalPlaces) {
		String s = decimalPlaces == 0 ? "999a999a999a999a999" : ("999a999a999a999a999b" + Convert.dup('9',
				   decimalPlaces));
	...
	protected void popPosState() {
		...
		insertPos = Math.min(len,
							 pushedInsertPos); // guich@571_5: make sure the insert position isn't bigger than the size of the text.
	...
	protected int getX0() {
		...
		switch (alignment) {
			...
			case CENTER:
				...
				if (getTotalCharWidth() > xMax - xMin - (captionIcon == null ? 0 : captionIcon.getWidth() + fmH /
						4)) {
					textStartX = this.width - fm.sbWidth(s, 0, s.length()) - 2 * xOffset;
					if (captionIcon != null) {
						textStartX -= (captionIcon == null ? 0 : captionIcon.getWidth() + fmH / 4);
		...
		switch (mode) {
			...
			default://case DATE:
				if (masked.length() > 0) {
					...
					return extra + xOffset + fm.sbWidth(masked, 0, Math.min(pos,
														masked.length()));//Math.min(pos,masked.length())); // guich@tc152: changed mask to masked, otherwise, using old font and 1's will make the cursor appear incorrectly
				}
		}
		...
		if (isMaskedEdit) {
			if (mode == CURRENCY) {
				if (!hasSignificantDigits()) {
					...
					if (decimalPlaces >
						0) { // for currency mode, remove the , and . and put it in Java's format (xxxx.yy)
	...
	protected void onBoundsChanged(boolean screenChanged) { // guich
		...
		if (captionIconHeightFactor != 0 && captionIcon != null) {
			try {
				captionIcon = captionIcon.smoothScaledFixedAspectRatio(height * captionIconHeightFactor / 100,
							  true);
			} catch (ImageException e) {
				e.printStackTrace();
			}
		...
		if (this.getRect().equals(oldBounds)) {
			...
			if (materialCaption != null) {
				...
				materialCaption.ycap0 = materialCaption.ycap = uiMaterial ? this.height / 2  - this.fmH / 2 :
	...
	public int getPreferredWidth() {
		...
		return (mask == null || useFillAsPreferred) ? FILL
			   : (fm.stringWidth(new String(mask)) + (uiAndroid ? 10 : (uiFlat
					   || uiVista) ? 8 :
					   4)); // guich@200b4_202: from 2 -> 4 is PalmOS style - guic@300_52: empty mask means FILL - guich@570_88: fixed width when uiFlat
	}
	...
	public int getPreferredHeight() {
		...
		if (uiMaterial) {
			if (caption != null) {
				ret = fmH + prefH > UnitsConverter.toPixels(DP + 56) ? ret + materialCaption.getExtraHeight() :
	...
	protected void draw(Graphics g) {
				...
				switch (mode) {
					...
					default:
						...
						if (masked.length() > 0) {
							...
							g.drawText(chars, 0, len, xx, textY, textShadowColor != -1,
		...
		if (hasBorder && !uiAndroid) {
			g.draw3dRect(0, 0, this.width, this.height, Graphics.R3D_EDIT, false, false,
		...
		if (uiMaterial) {
			...
			g.drawText(caption, materialCaption.xcap + (captionIcon == null ? 0 : captionIcon.getWidth() + fmH /
					   4), materialCaption.ycap);
		}
	}
	...
	public void popupKCC() {
		...
		if (!popupsHidden()) {
			...
			if (Settings.fingerTouch && kbdType != KBD_TIME && kbdType != KBD_CALENDAR
				&& kbdType != KBD_CALCULATOR
				&& kbdType != KBD_NUMERIC) {
				return;
			}
		}
		...
		if (w != null) {
			w.swapFocus(
				this);//requestFocus(); // guich@200b4: bring focus back -  guich@401_15: changed to swapFocus
		}
			...
			case KBD_CALENDAR:
				try {
					calendar = new CalendarBox(this.foreColor,
											   new Date(getText())); //Reinitializing the window every time with the actual date
					calendar.tempTitle = keyboardTitle;
					calendar.setSelectedDate(new Date(getText()));
				} catch (InvalidDateException ide) {
					calendar = new CalendarBox(
						this.foreColor);	//if the date is invalid, start the date with the system date
		...
		if (!Settings.enableVirtualKeyboard) {
			...
			if (Settings.customKeyboard != null) {
				...
				if (!Window.isSipShown() || lastWasNumeric != isNumeric) {
					...
					Window.setSIP(onBottom ? Window.SIP_BOTTOM : Window.SIP_TOP, this,
								  isNumeric); // if running on a PocketPC device, set the bounds of Sip in a way to not cover the edit
				}
			}
		} else {
	...
	public void onEvent(Event event) {
		if (calendar != null && event.type == ControlEvent.WINDOW_CLOSED
			&& event.target == calendar) { // called from the keyboard and from the calendar
			Date d = null;
		...
		switch (event.type) {
			...
			case KeyEvent.SPECIAL_KEY_PRESS:
				if (editable && isEnabled()) {
					...
					if (event.target == this
						&& moveFocus) { // guich@tc100b2: move to the next edit in the same container
						Control next;
					...
					boolean isPrintable = ke.key > 0 && event.type == KeyEvent.KEY_PRESS
					...
					if (del1 >= 0 && del2 < len) {
						if (cursorShowing) {
							Window.needsPaint =
								true; //draw(drawg == null ? (drawg = getGraphics()) : drawg, true); // erase cursor at old insert position
						}
						if (len > del2 - 1) {
			...
			case PenEvent.PEN_DOWN: {
				...
				if (!autoSelect
					|| !wasFocusIn) { // jairocg@450_31: if the event was focusIn, do not change the selected text
					for (newInsertPos = 0; newInsertPos < chars.length()
						 && charPos2x(newInsertPos) < pe.x - 3; newInsertPos++) {
					}
					...
					wasFocusIn =
						false; // guich@570_98: let the user change cursor location after the first focus_in event.
				}
				break;
			}
			case PenEvent.PEN_DRAG: {
				PenEvent pe = (PenEvent) event;
				for (newInsertPos = 0; newInsertPos < chars.length()
					 && charPos2x(newInsertPos) <= pe.x; newInsertPos++) {
				}
			...
			case KeyboardBox.KEYBOARD_POST_UNPOP:
				...
				ignoreSelect =
					false; // guich@570_112: after the keyboard is definetively gone, its safe to set this to false.
				wasFocusIn =
					false; // guich@570_112: fix first click on edit being ignored after keyboard is closed.
				startSelectPos = -1;
				return;
			default:
				return;
	...
	protected void onWindowPaintFinished() {
		if (!hasFocus) {
			_onEvent(new ControlEvent(ControlEvent.FOCUS_IN,
									  this)); // this event is called on the focused control of the parent window. so, if we are not in FOCUS state, set it now. - guich@tc112_
		}
	}
	...
	public void clipboardCut() { // guich@tc114_66
		...
		if (sel1 != -1) { // cut/copy
			Vm.clipboardCopy(chars.toString().substring(sel1,
							 sel2)); // brunosoares@tc100: Changed from chars.substring to chars.toString().substring
	...
	public Edit getCopy() {
		...
		boolean maskState =
			isMaskedEdit; // Saving the isMaskedEdit state so it can be set to false and setDecimalPlaces can be called
