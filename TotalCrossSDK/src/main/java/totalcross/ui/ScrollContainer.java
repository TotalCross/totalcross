...
public class ScrollContainer extends Container implements Scrollable, UpdateListener {
	...
	public Rect getClientRect() { // guich@200final_15
		...
		int sbHsize = showScroll && sbH != null
					  && !sbH.transparentBackground ? sbH.getPreferredHeight() : 0;
		r.width -= sbVsize;
		r.height -= sbHsize;
		return r;
	}
	...
	protected void setScrollBars(boolean allowHScrollBar, boolean allowVScrollBar) {
		...
		if (allowHScrollBar) {
			sbH = Settings.fingerTouch ? new ScrollPosition(ScrollBar.HORIZONTAL) : new ScrollBar(
					  ScrollBar.HORIZONTAL);
			sbH.setLiveScrolling(true);
			sbH.setMaximum(0);
		}
		if (allowVScrollBar) {
			sbV = Settings.fingerTouch ? new ScrollPosition(ScrollBar.VERTICAL) : new ScrollBar(
					  ScrollBar.VERTICAL);
	...
	public void resize() {
		...
		for (Control child = bag.children; child != null; child = child.next) {
			...
			if (!hasFillH && sbV != null && (FILL - RANGE) <= child.setH
				&& child.setH <= (FILL +
								  RANGE)) { // if control has fill on the height, don't take it into consideration
				hasFillH = true;
				hh = 0;
			}
			m = child.y + hh;
			...
			for (Control child = bag.children; child != null; child = child.next) {
				if ((FILL - RANGE) <= child.setH && child.setH <= (FILL + RANGE)) {
					child.height = maxY - child.y
								   + (uiAdjustmentsBasedOnFontHeightIsSupported ? (child.setH - FILL) * fmH / 100 :
									  (child.setH - FILL));
	...
	public void resize(int maxX, int maxY) {
		...
		int availX = r.width;
		int availY = r.height;
		boolean finger = ((sbH != null && sbH instanceof ScrollPosition) || (sbV != null
						  && sbV instanceof ScrollPosition));
		...
		boolean showScroll = !Settings.fingerTouch || !ScrollPosition.AUTO_HIDE;
		int sbVsize = needY && showScroll && sbV != null
					  && !sbV.transparentBackground ? sbV.getPreferredWidth() : 0;
		int sbHsize = needX && showScroll && sbH != null
					  && !sbH.transparentBackground ? sbH.getPreferredHeight() : 0;
		...
		if (needX && sbH != null && canShowScrollBars(false)) {
			...
			sbH.setUnitIncrement(flick != null
								 && flick.scrollDistance > 0 ? flick.scrollDistance : fm.charWidth('@'));
			lastH = 0;
		} else if (sbH != null) {
		...
		if (needY && sbV != null && canShowScrollBars(true)) {
			...
			sbV.setUnitIncrement(flick != null
								 && flick.scrollDistance > 0 ? flick.scrollDistance : fmH + Edit.prefH);
			lastV = 0;
		} else if (sbV != null) {
		...
		if (curPage != 0) {
			...
			if (sbV != null) {
				sbV.setValue(sbV.maximum == 0 ? 0 :
							 -vy); // if we're scrolled but we don't need scroll, move to origin
				bag.y = -sbV.getValue();
			}
		}
	}
		...
		switch (event.type) {
			...
			case PenEvent.PEN_DRAG_START:
				...
				if (Settings.optimizeScroll && ((DragEvent) event).direction != 0
					&& isFirstBag((Control) event.target)
