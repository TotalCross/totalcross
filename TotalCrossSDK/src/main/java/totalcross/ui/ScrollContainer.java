...
/**
 * ScrollContainer is a container with a horizontal only, vertical only, both or
 no
 ...
 * <p>
 * <b>Caution</b>: you must not use RIGHT, BOTTOM, CENTER and FILL when setting
 the control bounds,
 * unless you disable the corresponding ScrollBar! The only exception to this is
 to use FILL on the control's height,
 * which is allowed.
 * <p>
...
public class ScrollContainer
  extends Container implements Scrollable, UpdateListener
{
  /**
   ...
   * by the ScrollPosition. If you set
   * <code>sbV.transparentBackground=true</code> (or for sbH), you will expand
   * the client area to over the ScrollPosition's area.
   */
   ...
   * ...
   * </pre>
   * This is useful if such class usually requires more than one press to have a
   * value defined.
   * @see #autoScroll
   */
  ...
  /**
   * Standard constructor for a new ScrollContainer, with both scrollbars
   * enabled.
   */
  public ScrollContainer() { this(true); }
  ...
  { // guich@200final_15
    ...
    int sbVsize = showScroll && sbV != null && !sbV.transparentBackground
                    ? sbV.getPreferredWidth()
                    : 0;
    int sbHsize = showScroll && sbH != null && !sbH.transparentBackground
                    ? sbH.getPreferredHeight()
                    : 0;
    r.width -= sbVsize;
    r.height -= sbHsize;
    return r;
   ...
   * By disabling the horizontal scrollbar, you can use RIGHT and CENTER on the
   * x parameter of a control that is added. By disabling the vertical
   * scrollbar, you can use BOTTOM and CENTER on the y parameter of a control
   * that is added.
  ...
  public ScrollContainer(boolean allowHScrollBar, boolean allowVScrollBar)
  {
    ...
    bag.setRect(0,
                0,
                getBagInitialWidth(),
                getBagInitialHeight()); // set an arbitrary size
  ...
  protected void setScrollBars(boolean allowHScrollBar, boolean allowVScrollBar)
  {
    ...
    if (allowHScrollBar) {
      sbH = Settings.fingerTouch ? new ScrollPosition(ScrollBar.HORIZONTAL)
                                 : new ScrollBar(ScrollBar.HORIZONTAL);
      sbH.setLiveScrolling(true);
      sbH.setMaximum(0);
    }
    if (allowVScrollBar) {
      sbV = Settings.fingerTouch ? new ScrollPosition(ScrollBar.VERTICAL)
  ...
  protected int getBagInitialHeight() { return 20000; }

  /** Overwrite this method to define a custom width for the scrolling area */
  protected int getBagInitialWidth() { return 4000; }
  ...
  @Override public boolean scrollContent(int dx, int dy, boolean fromFlick)
  {
    ...
    if (isFromFlick) { // In case of flick, scroll instantaneously
      ...
      resetTarget();
    } else { // In case of a drag, compute the target position and move to it
             // smoothly
      ...
      if (sbH != null) {
        ...
        if (targetX < sbH.getMinimum()) {
          ...
          if (dx > 0) {
            postEvent(new ScrollEvent(ScrollEvent.SCROLL_RIGHT));
          } else if (dx < 0) {
            postEvent(new ScrollEvent(ScrollEvent.SCROLL_LEFT));
          }
        }
      } else {
        willScroll = false;
      ...
      if (sbV != null) {
        ...
        if (targetY < sbV.getMinimum()) {
          ...
          if (dy > 0) {
            postEvent(new ScrollEvent(ScrollEvent.SCROLL_DOWN));
          } else if (dy < 0) {
            postEvent(new ScrollEvent(ScrollEvent.SCROLL_UP));
          }
        }
  ...
  @Override public void updateListenerTriggered(int elapsedMilliseconds)
  {
    ...
    if (!isFromFlick) {
      ...
      // Each millisecond, the screen scrolls 1.66% of the way towards the
      // target point
  ...
  {
    return direction == DragEvent.LEFT || direction == DragEvent.RIGHT ? lastH
                                                                       : lastV;
  }
  ...
  void addToSC(Control c) { bag0.add(c); }
  ...
  /**
   * This method resizes the control to the needed bounds, based on added
   * childs.
   * Must be called if you're controlling reposition by your own, after you
  ...
  @Override public void resize()
  {
    ...
    for (Control child = bag.children; child != null; child = child.next) {
      ...
      int hh = child.height;
      if (!hasFillH && sbV != null && (FILL - RANGE) <= child.setH &&
          child.setH <= (FILL + RANGE)) { // if control has fill on the height,
  ...
  /**
   * This method resizes the control to the needed bounds, based on the given
   * maximum width and heights.
   */
  public void resize(int maxX, int maxY)
  {
      ...
      do {
        ...
        if (!needX && maxX > availX) { // do we need an horizontal scrollbar?
          ...
          if (finger && sbV != null && sbH != null) {
            availY -= sbH.getPreferredHeight(); // remove the horizbar area from
                                                // the avail Y area
          }
        }
      } while (changed);
    ...
    boolean showScroll = !Settings.fingerTouch || !ScrollPosition.AUTO_HIDE;
    int sbVsize =
      needY && showScroll && sbV != null && !sbV.transparentBackground
        ? sbV.getPreferredWidth()
        : 0;
    int sbHsize =
      needX && showScroll && sbH != null && !sbH.transparentBackground
        ? sbH.getPreferredHeight()
        : 0;
    ...
    if (needX && sbH != null && canShowScrollBars(false)) {
      ...
      sbH.setUnitIncrement(flick != null && flick.scrollDistance > 0
                             ? flick.scrollDistance
                             : fm.charWidth('@'));
      lastH = 0;
    } else if (sbH != null) {
      sbH.setMaximum(0); // kmeehl@tc100: drag-scrolling depends on this to
    ...
    if (needY && sbV != null && canShowScrollBars(true)) {
      ...
      sbV.setUnitIncrement(flick != null && flick.scrollDistance > 0
                             ? flick.scrollDistance
                             : fmH + Edit.prefH);
      lastV = 0;
    } else if (sbV != null) {
      sbV.setMaximum(0); // kmeehl@tc100: drag-scrolling depends on this to
                         // determine the bounds
    }
    Window.needsPaint = true;
  }

  protected boolean canShowScrollBars(boolean vertical) { return true; }

  /**
   * Override this method to return the correct scroll distance. Defaults to
   * the container's width.
   */
  public int getScrollDistance() { return this.width; }

  @Override public void reposition()
  {
    int vx = bag.x, vy = bag.y; // keep position when changing size
    int curPage =
      flick != null && flick.pagepos != null ? flick.pagepos.getPosition() : 0;
    super.reposition();
    resize();
    ...
    if (curPage != 0) {
      ...
      if (sbV != null) {
        sbV.setValue(sbV.maximum == 0 ? 0
                                      : -vy); // if we're scrolled but we don't
                                              // need scroll, move to origin
        bag.y = -sbV.getValue();
      }
    }
  }
  ...
  /**
   * Returns the preferred width AFTER the resize method was called. If the
   * ScrollBars are disabled, returns the maximum size of the container to hold
   * all controls.
   */
  @Override public int getPreferredWidth()
  {
    ...
    return sbV == null
      ? bag.width
      : horizontalMax + (sbV.maximum == 0 ? 0 : sbV.getPreferredWidth());
  }

  /**
   * Returns the preferred height AFTER the resize method was called. If the
   * ScrollBars are disabled, returns the maximum size of the container to hold
   * all controls.
   */
  @Override public int getPreferredHeight()
  {
    ...
    return sbH == null
      ? bag.height
      : verticalMax + (sbH.maximum == 0 ? 0 : sbH.getPreferredWidth());
  }
  ...
  @Override public void onEvent(Event event)
  {
    switch (event.type) {
      ...
      case PenEvent.PEN_DRAG_START:
        hasScrolled = true;
        if (Settings.optimizeScroll && ((DragEvent)event).direction != 0 &&
            isFirstBag((Control)event.target) && bag.offscreen == null &&
            Settings.fingerTouch && bag.width < 4096 && bag.height < 4096) {
          bag.takeScreenShot();
        }
        break;
      ...
      case PenEvent.PEN_UP:
        isScrolling = false;
        if (autoScroll && event.target instanceof Control &&
            event.target != lastScrolled &&
            !hsIgnoreAutoScroll.contains(event.target.getClass()) &&
            ((Control)event.target).isChildOf(this) &&
    ...
    if (event instanceof ScrollEvent) {
      event.consumed = true;
      for (ScrollEventHandler handler : scrollEventHandlerList) {
  ...
  /**
   * Scrolls to the given page, which is the flick's scrollDistance (if set),
   * or the control's height.
   * @since TotalCross 1.53
   */
  public void scrollToPage(int p)
  {
    int pageH = flick != null && flick.scrollDistance != 0
                  ? flick.scrollDistance
                  : this.height;
    int val = (p - 1) * pageH;
    if (sbH != null) {
  ...
  /**
   * Scrolls a page to left or right. Works only if it has a flick and a page
   * position.
   */
  ...
  { // kmeehl@tc100
    if (c != null && (sbH != null || sbV != null)) {
      ...
      if (sbH != null && (r.x < 0 || r.x2() > bag0.width)) {
        ...
        int val =
          lastH +
          (r.x <= 0 || r.width > bag0.width ? r.x : (r.x2() - bag0.width));
        setHValue(val);
      }
      // vertical
      if (sbV != null && (r.y < 0 || r.y2() > bag0.height)) {
        ...
        int val =
          lastV +
          (r.y <= 0 || r.height > bag0.height ? r.y : (r.y2() - bag0.height));
        setVValue(val);
      }
    }
    resetTarget();
  ...
  @Override public void setBorderStyle(byte border)
  {
    if (shrink2size) {
  ...
  @Override public boolean wasScrolled() { return hasScrolled; }

  /**
   * Removes all controls from the ScrollContainer.
   */
  @Override public void removeAll() { bag.removeAll(); }

  /**
   * Returns the children of the bag. If you call ScrollContainer.getChildren,
   * it will not return the controls added to the ScrollContainer, since they
   * are actually added to the bag.
   * @since TotalCross 1.5
   */
  public Control[] getBagChildren() { return bag.getChildren(); }

  @Override public void onFontChanged() { bag.setFont(font); }
  ...
  private int getSafeSbhValue() { return (sbH != null) ? sbH.value : 0; }
  private int getSafeSbvValue() { return (sbV != null) ? sbV.value : 0; }

  /**
   * Disables the 'flick' behaviour from this scroll container. This is useful
   * when you have small scrollable containers and are facing usability
   * problems. We recommend leaving the flick behavior on for large scrollable
   * surfaces, with many items. All scroll containers have the flick behaviour
   * on by default. If disabled, this behavior can be reenabled using the
   * 'enableFlick' method.
   * @see enableFlick
   */
   ...
   */
  public void setHandlersVisibility(boolean horizontalHandlerVisiblle,
