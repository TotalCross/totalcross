...
/**
 * Group or matrix of pushbuttons in a single control. Is one of the most
 * versatiles controls of TotalCross. Here is an example of constructor: <pre>
 * new PushButtonGroup(new
 * String[]{"Button1","Button2","Button3"},false,-1,-1,4,0,false,PushButtonGroup.NORMAL)
 * </pre>
 * <p>The width of each button is calculated based on its caption size plus the
 * insideGap, if you use PREFERRED as the width; otherwise, it uses the size you
 * specified (E.G.: FILL, FIT, etc). The height is calculated based on the
 * font's size or on the height you specified.
 */
...
public class PushButtonGroup extends Control
{
  ...
  /**
   * The button will be selected and unselected immediately, acting like a real
   * button
   */
  public static final byte BUTTON = 1;
  /**
   * One click in the button will select it and another click will unselect it.
   * However, only one button can be selected at a time
   */
  public static final byte CHECK = 2;
  /**
   * Set to true to make the CHECK mode appear as raised by default (default
   * is: button lowered when not selected).
   */
  ...
  private int gap;
  /**
   * Space between the text and the button border. The ideal is 4. If
   * allSameWidth is true, it is only used to compute the preferred width and
   * may be overriden; otherwise, it is used as the internal gap.
   */
   ...
   * event will be sent while this button is held. Works only when the type is
   * BUTTON.
   * @see #INITIAL_DELAY
   * @see #AUTO_DELAY
  ...
  /**
   * The boolean array that defines which buttons are hidden. If you want to
   * hide a button, just access this and set an array index to true. Note that
   * you must also explicitly call the repaint function to update the control.
   * Sample: <pre> pbg.hidden[5] = true; // hides button 5 repaint();
   * </pre>
   * @since SuperWaba 5.0
   */
  ...
  /**
   Span across multiple columns and rows. These cells that will be overriden
   must be null and the allSameWidth must be true.
   * This sample:
   * <pre>
      String []numerics =
   {"1","2","3","4","5","6","7","clear",null,"0",null,null}; PushButtonGroup
   pbg=new PushButtonGroup(numerics,false,-1,4,0,4,true,PushButtonGroup.BUTTON);
      pbg.colspan[7] = 2;
      pbg.rowspan[7] = 2;
  ...
  /**
     Create the button matrix.
       @param names captions of the buttons. You can specify some names as null
         so the button is not displayed. This is good if you're creating a
         button matrix and want to hide some buttons definetively (you can hide
         some buttons temporarily setting the hiden array). You can also use the
         <code>hidden</code> property to dynamically show/hide buttons.
       ...
       @param insideGap Space between the text and the button border. The ideal
         is 4. If allSameWidth is true, it is only used to compute the preferred
         width and may be overriden; otherwise, it is used as the internal gap.
       @param rows if > 1, creates a button matrix
       @param allSameWidth if true, all the buttons will have the width of the
         most large one.
  ...
  /**
     Create the button matrix, with insideGap = 4, selected = -1, atLeastOne =
     false, allSameWidth = true and type = BUTTON.
      @param names captions of the buttons. You can specify some names as null
         so the button is not displayed. This is good if you're creating a
         button matrix and want to hide some buttons definetively (you can hide
         some buttons temporarily setting the hiden array). You can also use the
  ...
  /**
     Create the button matrix, with selected = -1, atLeastOne = false,
     allSameWidth = false and type = BUTTON.
      @param names captions of the buttons. You can specify some names as null
         so the button is not displayed. This is good if you're creating a
         button matrix and want to hide some buttons definetively (you can hide
         some buttons temporarily setting the hiden array). You can also use the
  ...
  /**
   * Sets the names. Note that it must have the same number of elements passed
   * in the constructor, and that the bounds are NOT recomputed. Repaint is
   * called.
   */
  ...
  /**
   * Sets a button's index color. The other buttons will remain with the
   * default color. Pass -1 to restore the default color.
   */
  ...
  { // guich@573_37
    if (0 <= index && index < count) {
      if (foreColor != -1 &&
          btnFColors ==
            null) { // only initialize the colors array after the first use
  ...
  public void setSimpleBorder(boolean simple)
  {
    this.simpleBorder =
      simple || uiFlat ||
      uiVista; // guich@552_22: added uiFlat - // guich@573_6: added uiVista
  }
  ...
  /**
   * Returns the caption of the selected button, or <code>null</code> if no
   * button is selected
  ...
  /**
   * Sets the cursor color for this PushButtonGroup. The default is equal to
  ...
  {
    return (fmH + (simpleBorder ? 0 : 2) + gap) * rows -
      gap; // remove the last gap
  }
  ...
  @Override protected void onFontChanged()
  {
    ...
    // guich@200b4: search again for possible null names and set its width as
    // the average width
    if (nullNames > 0) {
      avg /= count - nullNames;
      for (i = count - 1; i >= 0; i--) {
  ...
  @Override protected void onColorsChanged(boolean colorsChanged)
  {
    ...
    if (colorsChanged) {
      dColor = userCursorColor >= 0 ? userCursorColor
                                    : Color.getCursorColor(backColor);
    }
    fColor = getForeColor();
  ...
  {
    if (g == null) {
      return; // guich@tc152: not sure when this happens, but a user sent a
              // stack trace with such occurence.
    }
    ...
    boolean drawEachBack =
      nullNames > 0 ||
      (btnBColors != null || uiAndroid || (uiVista && isEnabled())) ||
      (gap > 0 && parent != null &&
       backColor != parent.backColor); // guich@230_34 - guich@tc110_16:
                                       // consider nullNames
    if (uiMaterial && Settings.onJavaSE && getDoEffect() && effect != null &&
        effect.isRunning()) {
      ...
      if ((r = rects[i]) != null && !hidden[i]) {
        if (drawEachBack && !transparentBackground) {
          try {
            ...
            if (uiAndroid) {
              if (border) {
                Image img = getAndroidButton(
                  r.width,
                  r.height,
                  isEnabled() ? back
                              : Color.interpolate(back, parent.backColor),
                  i == sel && !uiMaterial);
              ...
              g.fillVistaRect(
                r.x, r.y, r.width, r.height, back, i == sel, false);
            } else {
              g.backColor = back;
        ...
        if (!uiAndroid) {
          if (simpleBorder) {
            ...
            g.draw3dRect(r.x,
                         ...
                         r.width,
                         r.height,
                         actLikeCheck && !checkAppearsRaised
                           ? (uiVista && i == sel) ? Graphics.R3D_RAISED
                                                   : Graphics.R3D_CHECK
                           : (uiVista && i == sel) ? Graphics.R3D_LOWERED
                                                   : Graphics.R3D_RAISED,
                         false,
                         false,
                         fourColors);
    ...
    for (i = 0; i < n; i++) {
      if ((r = rects[i]) != null && !hidden[i]) {
        ...
        boolean useCustomColor =
          btnFColors != null && btnFColors[i] >= 0; // guich@573_37
        ...
        if (highlightSelected) {
          if (i == selectedIndex) {
            if (highlightEmptyValues) {
              ...
              g.fillCircle(rects[i].x + rects[i].width / 2,
        ...
        if (uiFlat || i != sel) {
          g.drawText(names[i],
                     ...
                     r.y + ty,
                     textShadowColor != -1,
                     textShadowColor); // tX[i]: if allSameWidth, center the
                                       // label in the button
        } else {
  ...
  private Image getAndroidButton(int w, int h, int color, boolean selected)
    throws ImageException
  {
    Image img = null;
    try {
      img = NinePatch.getInstance().getNormalInstance(
        NinePatch.getInstance().load(new Image(Resources.button.getPath())),
        w,
        h,
        color,
        false);
  ...
  /**
   * Sets the selected button index. Note. if there are any null or hidden
   * buttons, you must consider them too to compute the correct index.
   */
  ...
  private int findButtonAt(int px, int py)
  {
    ...
    if (Settings.fingerTouch || inside) {
      ...
      if (inside) {
        ...
        int e = colspan == null && rowspan == null ? Math.min(count, s + cols)
                                                   : count;
        for (int i = s; i < e; i++) {
        ...
        for (int i = 0; i < rects.length; i++) {
          if ((r = rects[i]) != null && !hidden[i]) {
            int d = (int)(Convert.getDistancePoint2Rect(
                            px, py, r.x, r.y, r.x + r.width, r.y + r.height) +
                          0.5);
            if (d < minDist) {
              minDist = d;
              sel = i;
    ...
    switch (event.type) {
      ...
      case KeyEvent.SPECIAL_KEY_PRESS:
        ...
        if (ke.key == '0' && !atLeastOne) {
          ...
          int max = count; // guich@tc123_1: limit the loop to the number of
                           // items in the PBG
          if (newIndex == -1) {
            ke.key = SpecialKeys.RIGHT;
          }
          if (ke.isUpKey()) { // guich@550_15: added support for navigate using
                              // all arrows
            do {
              newIndex -= cols;
              if (newIndex < 0) {
            ...
            } while (max-- > 0 && (names[newIndex] == null ||
                                   names[newIndex].length() ==
                                     0)); // guich@573_40: also ignores (here
                                          // and below) if name == ""
  ...
  private void setSelectedIndex(int sel, boolean selectIt)
  {
    ...
    if (actLikeCheck) {
      ...
      // Vm.safeSleep(150); - guich@tc130: with this, clicking fast on buttons
      // will make them laggy
      setSelectedIndex(-1);
    }
  }

  public boolean getHighlightSelected() { return highlightSelected; }
  ...
  public boolean getHighlightEmptyValues() { return highlightEmptyValues; }
  ...
  /**
   * Clears this control, selecting index clearValueInt. Note that if
   * atLeastOne is true, setting clearValueInt to -1 will have no effect.
   */
  ...
  @Override public Control handleGeographicalFocusChangeKeys(KeyEvent ke)
  {
    if ((ke.isUpKey() && selectedIndex - cols < 0) ||
        ...
        (ke.isNextKey() && !ke.isDownKey() && selectedIndex != -1 &&
         (selectedIndex + 1) % cols == 0) ||
