...
public class PushButtonGroup extends Control {
	 ...
	 */
	public PushButtonGroup(String[] names, boolean atLeastOne, int selected, int gap, int insideGap,
						   int rows,
	...
	public void setSimpleBorder(boolean simple) {
		this.simpleBorder = simple || uiFlat
	...
	public void onPaint(Graphics g) {
		...
		g.foreColor = fColor;
		boolean drawEachBack = nullNames > 0 || (btnBColors != null || uiAndroid || (uiVista
							   && isEnabled()))
							   || (gap > 0 && parent != null
								   && backColor != parent.backColor); // guich@230_34 - guich@tc110_16: consider nullNames
				...
				if (!uiAndroid) {
					if (simpleBorder) {
						...
						g.draw3dRect(r.x, r.y, r.width, r.height,
									 actLikeCheck && !checkAppearsRaised ? (uiVista
											 && i == sel) ? Graphics.R3D_RAISED : Graphics.R3D_CHECK
		...
		for (i = 0; i < n; i++) {
			if ((r = rects[i]) != null && !hidden[i]) {
				...
				if (highlightSelected) {
					if (i == selectedIndex) {
						if (highlightEmptyValues) {
							...
							g.fillCircle(rects[i].x + rects[i].width / 2, rects[i].y + rects[i].height / 2,
										 rects[i].width < rects[i].height ? rects[i].width / 2 : rects[i].height / 2);
							g.backColor = bkg;
						} else {
							if (!names[i].equals("")) {
								int bkg = g.backColor;
				...
				if (uiFlat || i != sel) {
					g.drawText(names[i], tX[i], r.y + ty, textShadowColor != -1,
							   textShadowColor); // tX[i]: if allSameWidth, center the label in the button
				} else {
	...
	private Image getAndroidButton(int w, int h, int color, boolean selected) throws ImageException {
		Image img = null;
		try {
			img = NinePatch.getInstance().getNormalInstance(NinePatch.getInstance().load(new Image(
						Resources.button.getPath())), w, h, color, false);
		} catch (IOException e) {
			e.printStackTrace();
		}
		if (selected) {
						...
						do {
							...
							if (newIndex < 0) {
								newIndex = count + newIndex;
											   ...
											   || names[newIndex].length() == 0)); // guich@573_40: also ignores (here and below) if name == ""
