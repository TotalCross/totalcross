...
public class Button extends Control implements TextControl
{
   ...
   * will be recolorized and resized to the text's size. To create the image
   * that will work with this border type, do this: <ol> <li>Get an empty image
   * from somewhere in the web
   * (<a href='http://www.aaa-buttons.com/'>this site</a> is a good place;
   * choose a light background image to prevent problems described in step 5).
   ...
   * <li>Images with a smooth round border should have the background colorized
   * to the target background color; otherwise, the results will not be perfect.
   * </ol>
   * <p>
   * Important: you must set the button colors before calling setBorder. Here's
   * a sample:
   *
   * <pre>
   * Button btn = new Button("Bárbara\nHazan", new Image("button1.png"), CENTER,
   * 8); btn.setBackColor(backColor); btn.borderColor3DG = 0x008800; // used to
   * colorize the image btn.setFont(font.asBold());
  ...
  /**
   * Set to true to enable auto-repeat feature for this button. The PRESSED
   ...
   * // another option: btn.pressedImage =
   * btn.getImage().getFadedInstance(Color.RED);
   * </pre>
   *
   * @since TotalCross 1.15
   */
   ...
   * make the button have a vista-like background. Otherwise, it will have a
   * flat background.
   *
  ...
  /**
   * The top color of this button when BORDER_3D_GRADIENT border style is
   * active. Set to 0xDCDCFF when setBorder is called.
   */
  ...
  /**
   * Change to a color value to highlight the button's text. The highlight is
   * made drawing the button in x-1,y-1; x+1,y-1; x-1,y+1; x+1,y+1 positions.
   * Defaults to -1.
   *
   * @since TotalCross 1.12
   * @deprecated Fix the typo, use Control.textShadowColor instead.
   */
  @Deprecated public int hightlightColor = -1; // guich@tc112_29
  ...
  /**
   * Used when the textPosition is RIGHT_OF. Specifies the text that will be
   * used as reference to center this button's text with that one. Usually, you
   * find the biggest text of a set of buttons and place it here.
   *
  ...
  /**
   * The height of the image based on the button's height, ranging from 1 to
   * 100. Used when an image is passed as parameter in one of the constructors.
   */
   ...
   * @param textPosition Where to place the text (supports only LEFT, TOP,
   *     RIGHT,
   *                     BOTTOM, CENTER - no adjustments!). Also supports
   *                     RIGHT_OF, which uses the relativeToText field as
   * another button to be used as reference to be able to center the title among
   * a set of buttons; the text will be placed at the right)
  ...
  public Button(String text, Image img, int textPosition, int gap)
  {
    if (uiMaterial) {
      ...
      if (text != null) {
        ...
        String auxText = text;
        for (int i = 0; textWidth > Settings.screenWidth -
  ...
  public Button(String text) { this(text, null, 0, 0); }
   ...
   */
  public Button(Image img) { this(null, img, 0, 0); }
  ...
  /**
   * Sets the image that is displayed in the button. The transparentColor
   * property of the Image must be set before calling this method.<br> Note:
   * this method does not resize the button to fit the image.
   */
  ...
  /**
   * Sets the color that the button's background will go ('armed color') when
   * the button gets a PENDOWN event. The default is the cursor color for the
   * background.
   *
  ...
  @Override public String getText() { return text != null ? text : "image"; }
  ...
  @Override public int getPreferredWidth()
  {
    ...
    int iw = (this.border == BORDER_GRAY_IMAGE || img == null)
               ? 0
               : img.getWidth(); // guich@tc120_1: using a gray
    ...
    return prefW + ((localCommonGap + border) << 1) +
      (img != null && text == null ? 1 : 0); // guich@tc100b4_16:
                                             // add an extra
                                             // pixel if
                                             // image-only
  }
    ...
    if (this.text != null) {
      if (this.img != null) {
        ...
        if (this.border == BORDER_NONE) {
          this.paddingLeft = paddingLeft == UnitsConverter.toPixels(DP + 16)
      ...
      for (int i = 0;
           textWidth > Settings.screenWidth - (paddingRight + paddingLeft);
           i++) {
      ...
      this.text = auxText;
      final int textWithPadding =
        this.paddingLeft + this.paddingRight + textWidth +
        (this.img != null
           ? this.img.getWidth() +
               ((txtPos == RIGHT || txtPos == LEFT) ? this.tiGap : 0)
           : 0);
      ...
      this.paddingLeft = this.paddingLeft != UnitsConverter.toPixels(DP + 16)
                           ? this.paddingLeft
                           : UnitsConverter.toPixels(DP + 12);
      this.paddingRight = this.paddingRight != UnitsConverter.toPixels(DP + 16)
                            ? this.paddingRight
                            : UnitsConverter.toPixels(DP + 12);
  ...
  @Override public int getPreferredHeight()
  {
    ...
    int ih = (this.border == BORDER_GRAY_IMAGE || img == null)
               ? 0
               : img.getHeight(); // guich@tc120_1: using a gray
    // image does not take the
    // image into consideration
    switch (txtPos) {
      case BOTTOM:
    ...
    return prefH + ((localCommonGap + border) << 1) +
      (img != null && text == null ? 1 : 0); // guich@tc100b4_16:
                                             // add an extra
                                             // pixel if
                                             // image-only
  ...
  private int getPreferredMaterialHeight()
  {
    ...
    if (this.text != null) {
      this.paddingTop = this.paddingTop != UnitsConverter.toPixels(DP + 11)
                          ? this.paddingTop
      ...
      if (this.img != null) {
        if (this.txtPos == TOP || this.txtPos == BOTTOM) {
          returnValue = this.img.getHeight() + this.getFont().fm.height +
                        paddingTop + paddingBottom + tiGap;
        } else {
          returnValue =
            Math.max(this.img.getHeight(), this.getFont().fm.height) +
  ...
  private void disableAutoRepeat()
  { // luciana@570_22
  ...
  public void press(boolean pressed)
  {
    if (transparentBackground &&
        !isAndroidStyle) // guich@tc114_77: repaint now the parent's background
                         // otherwise
    ...
    armed = pressed; // some drawing routines does not receive the armed
                     // parameter, so we must set it
    // here.
    repaintNow();
  }
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if ((uiMaterial || uiAndroid) && this.border == BORDER_3D) {
      int bbackColor =
        isEnabled()
          ? backColor
          : (uiMaterial ? disabledColor
                        : Color.interpolate(parent.backColor, backColor));
      if (!transparentBackground) {
      ...
      if (npParts != null) {
        try {
          if (!drawTranslucentBackground(
                g,
                armed ? alphaValue >= 80 ? alphaValue / 2 : alphaValue * 2
                      : alphaValue)) {
            if (npback == null) {
              npback = NinePatch.getInstance().getNormalInstance(
                ...
                width,
                height,
                isEnabled() ? transparentBackground ? borderColor : bbackColor
                            : bbackColor,
                false);
            }
            this.originalForeColor = this.originalForeColor == -1
                                       ? this.foreColor
                                       : this.originalForeColor;
            this.foreColor =
              this.isEnabled() ? this.originalForeColor : Color.BLACK;
          }
          if (npback != null) {
            NinePatch.tryDrawImage(
              g,
              armed && (isSticky || effect == null)
                ? NinePatch.getInstance().getPressedInstance(
                    npback, backColor, pressColor)
                : npback,
              0,
              0);
          }
      ...
      if (isAndroidStyle) {
        if (translucentShape == TranslucentShape.NONE && !uiMaterial &&
            !Settings.isOpenGL) {
          g.getClip(clip);
          g.backColor =
            Settings.isOpenGL ? parent.backColor : g.getPixel(clip.x, clip.y);
          g.fillRect(0, 0, width, height);
        }
      } else if (!isBorderRound &&
                 (!transparentBackground ||
                  (armed && fillWhenPressedOnTransparentBackground) ||
      ...
      if (isBorderRound) {
        ...
        g.fillRoundRect(0,
                        ...
                        width,
                        height,
                        uiMaterial ? UnitsConverter.toPixels(DP + 4)
                                   : height / roundBorderFactor);
      ...
      int border =
        txtPos == CENTER ? 0 : Math.min(2, this.border); // guich@tc112_31
      g.setClip(border,
                border,
                width - (border << 1),
      ...
      if (armed && !isAndroidStyle && shiftOnPress &&
          (isBorder3D || uiVista ||
           (img != null &&
            text == null))) { // guich@tc100: if this is an image-only button,
    ...
    if (getDoEffect() && effect != null) {
      if (this.border == BORDER_OUTLINED) {
        effect.color = Color.getBrightness(fColor) < 127
                         ? Color.brighter(fColor, 64)
                         : Color.darker(fColor, 64);
  ...
  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    ...
    if (imageHeightFactor != 0 && img0 != null) {
      try {
        img =
          img0.hwScaledFixedAspectRatio(height * imageHeightFactor / 100, true);
      ...
      if (img != null) {
        ...
        switch (txtPos) {
          ...
          case CENTER:
            ...
            break;
          case CENTRALIZE:
            ix0 = (this.width - img.getWidth() - this.tiGap -
                   fm.stringWidth(text)) >>
                  1;
            iy0 = (this.height - img.getHeight()) >> 1;
          ...
          case RIGHT_OF:
            ...
            int ix0 =
              (this.width - img.getWidth() - fm.stringWidth(relativeToText)) >>
              1;
        ...
        switch (txtPos) {
          ...
          case RIGHT_OF: { // guich@tc126_28
            ...
            if (relativeToText.indexOf('\n') == -1) {
              ...
              String[] relToLines =
                Convert.tokenizeString(relativeToText, '\n');
  ...
  @Override protected void onColorsChanged(boolean colorsChanged)
  {
    ...
    if (!fixPressColor) {
      pressColor =
        Color.getCursorColor(backColor); // guich@450_35: only assign a new
                                         // color if none was set. -
  ...
  protected void paintBackground(Graphics g)
  {
    ...
    if (!transparentBackground ||
      ...
      if (border == BORDER_3D_HORIZONTAL_GRADIENT ||
          border == BORDER_3D_VERTICAL_GRADIENT) {
        g.drawRoundGradient(0,
                            0,
                            width - 1,
                            height - 1,
                            cornerRadius3DG,
        ...
        if (armed) {
          g.drawRoundGradient(
            ...
            height - 1,
            cornerRadius3DG,
            cornerRadius3DG - borderWidth3DG,
            cornerRadius3DG - borderWidth3DG,
            cornerRadius3DG,
            armed && fixPressColor ? pressColor : topColor3DG,
            enabled ? bottomColor3DG : topColor3DG,
            border == BORDER_3D_VERTICAL_GRADIENT); // guich@tc120_42: use
                                                    // pressColor if defined
        } else {
          g.drawRoundGradient(0,
                              0,
                              width - borderWidth3DG - 1,
                              height - borderWidth3DG - 1,
                              cornerRadius3DG,
                              cornerRadius3DG - borderWidth3DG,
                              cornerRadius3DG - borderWidth3DG,
                              cornerRadius3DG,
                              ...
                              enabled ? bottomColor3DG : topColor3DG,
                              border ==
                                BORDER_3D_VERTICAL_GRADIENT); // guich@tc120_42
        }
        return;
      }
      switch (Settings.uiStyle) {
        case Settings.Flat:
          if (border != BORDER_OUTLINED) {
            g.backColor =
              img == null && armed
                ? pressColor
                : backColor; // guich@tc100b4_13: also check if img is
            // null
            g.fillRect(0, 0, width, height);
          }
          break;
        ...
        case Settings.Vista: {                           // guich@573_6
          if (border == BORDER_NONE && flatBackground) { // guich@582_14
            g.backColor =
              armed && fixPressColor && effect == null ? pressColor : backColor;
            g.fillRect(0, 0, width, height);
          } else if (this.border == BORDER_OUTLINED) {
            break;
          } else if (enabled) {
          ...
          // guich@582_14 - commented if (border != BORDER_NONE)
          {
            ...
            if (border != BORDER_NONE) {
              g.drawVistaRect(
                0, 0, width, height, fColor, fColor, fColor, fColor);
            }
          }
          break;
        }
    ...
    if (border != BORDER_NONE && border != BORDER_OUTLINED && !isAndroidStyle &&
        !(uiVista && !enabled)) {
      g.draw3dRect(0,
                   0,
                   width,
                   height,
                   armed ? Graphics.R3D_LOWERED : Graphics.R3D_RAISED,
                   false,
  ...
  protected void paintText(Graphics g, int tx, int ty)
  {
    int shade = !isEnabled()
                  ? -1
                  : textShadowColor != -1 ? textShadowColor : hightlightColor;
    if (underlinedText) {
