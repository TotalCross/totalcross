...
/**
 * <code>AlignedLabelsContainer</code> is a <code>Container</code> used to align
 * all controls to the maximum width of a set of labels. You can define the
 * label alignment. The controls that you add to this container are placed at
 * the right of the labels. Here's a sample of how to use it:
 *
 * <pre>
 * String[] labels =
 * {
 ...
 * c.labelAlign = RIGHT;
 * c.foreColors = new
 * int[]{Color.RED,Color.BLACK,Color.BLACK,Color.BLACK,Color.BLACK,Color.BLACK,Color.BLACK,};
 * c.setInsets(2,2,2,2);
...
{ // guich@tc110_86
  ...
  /**
   * Sets this member to the font you want to set to the controls that are added
   * to this container.
   *
  ...
  /**
   * Sets an array with the same number of labels and the colors you want to
   * show for each label. If the number of labels differs, you will get an
   * exception. It can be used to hide a label: just set its foreground color to
  ...
  /**
   * Creates a new <code>AlignedLabelsContainer</code> without labels. You may
   * call <code>setLabels()</code> to set the labels.
   *
   * @see #AlignedLabelsContainer(String[])
   * @see #AlignedLabelsContainer(String[], int)
   * @see #setLabels(String[], int)
   */
  public AlignedLabelsContainer() { setLabels(null, 0); }

  /**
   * Creates a new <code>AlignedLabelsContainer</code> with the given labels.
   *
   * @param labels The strings that represents the labels. You may pass
   *     <code>null</code>, but be sure to call <code>setLabels()</code> to set
   *     the
   * labels.
   *
   * @see #AlignedLabelsContainer(String[], int)
   * @see #setLabels(String[], int)
   */
  public AlignedLabelsContainer(String[] labels) { setLabels(labels, 0); }
   ...
   *
   * @param labels The strings that represents the labels. You may pass
   *     <code>null</code>, but be sure to call <code>setLabels()</code> again
   *     to
  ...
  @Override public void onFontChanged()
  {
    ...
    super.setInsets(maxw + ileft,
                    insets.right,
                    insets.top,
                    insets.bottom); // guich@tc114_3: set the insets again
  }

  /**
   * Since this is an <code>AlignedLabelsContainer</code>, use this to add a
   * label.
   */
  @Override public void add(Control c)
  {
    super.add(c);
   ...
   */
  @Override public void setInsets(int left, int right, int top, int bottom)
  {
    ileft = left;
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    for (int i = 0; i < labels.length; i++, yy += inc) {
      ...
      switch (labelAlign) {
        case LEFT:
          g.drawText(
            labels[i], left, yy, textShadowColor != -1, textShadowColor);
          break;
        case CENTER:
          g.drawText(labels[i],
                     left + (maxw - widths[i]) / 2,
                     yy,
                     textShadowColor != -1,
                     textShadowColor);
          break;
  ...
  /**
   * Given a line (staring from 0), returns the y position. Can be used to
   * easily align the controls. Note that the line number may be greater than
   ...
   */
  public int getLeftInset() { return maxw; }
}
