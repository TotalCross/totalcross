...
public class SpinList extends Control
{
  ...
  /**
   * Number of ticks of the timer interval that will be waiten until the scroll
   * starts.
   */
  ...
  /**
   * Set to true if there are only numbers in the SpinList and you want to open
   * a NumericBox to let the user enter a value. The SpinList is divided into 3
   * areas: left (decrease), right (increase), middle (opens the NumericBox).
  ...
  /**
   * Set to false to disallow the wrap around that happens when the user is at
   * the first or last items.
   * @since TotalCross 2.0
   */
  public boolean wrapAround = true;

  /**
   * By default, equals the choices' length. You can define its length and then
   * create a single array shared by a set of SpinLists with different lengths
   * on each SpinList.
   */
  ...
  /**
   * Constructs a vertical SpinList with the given choices, selecting index 0
   * by default.
   * @see #setChoices
   */
  ...
  /**
   * Sets the choices to the given ones. Searches for [i0,if] and then expands
   * the items. For example, passing some string as "Day [1,31]" will expand
   * that to an array of <code>"Day 1","Day 2",...,"Day 31"</code>.
   */
  ...
  /**
   * Expands the items in the format "prefix [start,end] suffix", where prefix
   * and suffix are optional. For example, passing some string as "Day [1,31]"
   * will expand that to an array of <code>"Day 1","Day 2",...,"Day 31"</code>.
   */
  public static void expand(Vector v, String str) throws InvalidNumberException
  {
    ...
    int j;
    int start =
      Convert.toInt(str.substring(ini + 1, j = str.indexOf(',', ini + 1)));
  ...
  /**
   * Selects the given item. If the item is not found, the selected index
   * remains unchanged.
   */
  public void setSelectedItem(String item) { setSelectedIndex(indexOf(item)); }
  ...
  /**
   * Inserts the given element in order (based in the assumption that the
  ...
  private int getArrowHeight() { return isVertical ? 4 * fmH / 11 : fmH / 2; }
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (isVertical) {
      ...
      g.drawArrow(
        0, yoff + height / 2, wArrow, Graphics.ARROW_DOWN, false, fore);
      if (choicesLen > 0) {
        g.drawText(s,
                   hAlign == LEFT
                     ? wArrow * 2
                     : hAlign == RIGHT ? width - fm.stringWidth(s)
      ...
      g.drawArrow(
        width - wArrow, yoff, wArrow, Graphics.ARROW_RIGHT, false, fore);
      if (choicesLen > 0) {
        g.drawText(s,
                   hAlign == LEFT
                     ? wArrow
                     : hAlign == RIGHT ? width - fmH / 2 - 1 - fm.stringWidth(s)
                                       : (width - fm.stringWidth(s)) / 2,
                   yoff - 1,
                   textShadowColor != -1,
                   textShadowColor);
    ...
    switch (event.type) {
      ...
      case PenEvent.PEN_UP: {
        ...
        if (Settings.fingerTouch && !hadParentScrolled()) {
          if (!isVertical && (useNumericBox || useCalculatorBox) &&
              width / 3 <= pe.x && pe.x <= 2 * width / 3) {
            CalculatorBox nb = new CalculatorBox(useCalculatorBox);
            nb.cOrigDefault = this;
            if (useNumericBox) {
              nb.maxLength =
                Math.max(choices[0].length(), choices[choicesLen - 1].length());
            }
            nb.popup();
          } else {
            doScroll((PenEvent)event);
  ...
  @Override public void clear() { setSelectedIndex(clearValueInt); }
