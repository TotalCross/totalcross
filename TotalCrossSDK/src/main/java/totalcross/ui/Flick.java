 ...
 *
 * The animations conditionally start on the PenUp after drag events. The
 * animations simulate friction using the constant acceleration formula:
 *
 * x = x0 + v0 * (t - t0) + a * (t - t0)^2 / 2
 *
 * The position at t -= t0 is: position = x0 + v0 * t + a * t^2 / 2;
 *
 * This class is for internal use. You should use the ScrollContainer class
 * instead.
 */
public class Flick implements PenListener, TimerListener
{
  ...
  /**
   * Shortest flick animation allowed to start in milliseconds. Used to compute
   * the minimum initial velocity. If a flick animation will come to rest in a
   * shorter time than this it isn't done.
   */
  public static int defaultShortestFlick = 300;
  public int shortestFlick = defaultShortestFlick;

  /**
   * Longest flick animation allowed in milliseconds. The maximum initial
   * velocity is limited so that no flick animation takes longer than this to
   * come to rest.
   *
   * Changing this property will affect the flick object of all Controls. If you
   * have access to a flick control, you can use the longestFlick property
   * instead. Otherwise, set this property to a new value before constructing
   * the control and then set it back to the original value (2500) after the
  ...
  /**
   * Flick acceleration in inches/second^2. This value simulates friction to
   * slow the flick motion. Defaults to 2.95 for screen height > 320, or 1.6
   * otherwise.
   */
  public static double defaultFlickAcceleration =
    Math.max(Settings.screenWidth, Settings.screenHeight) / Font.NORMAL_SIZE /
    (Settings.platform.equals(Settings.ANDROID) ? 10.0 : 5.0);
  ...
  // Signed acceleration used during a flick animation. Negative when motion is
  // positive.
  private double a;
  ...
  /**
   * True if the user is dragging a control that owns a Flick but the flick
   * didn't started yet (in that case, currentFlick would not be null).
   */
  public static boolean isDragging;

  /**
   * The maximum accelleration that can be applied when the user keep dragging
   * the container. Defaults to 5.
   */
  public int maximumAccelerationMultiplier = 5;

  /**
   * When a ScrollContainer is inside another one, by default, both can be
   * scrolled at the same time. Set this flag to false at the topmost one to
  ...
  /**
   * Call this method to set the PagePosition control that will be updated with
   * the current page as the flick occurs. It must have all properties already
   * set, since the Flick will only change the current position.
   */
  public void setPagePosition(PagePosition pp) { this.pagepos = pp; }

  /**
   * Used in page scrolls, defines the distance that should be scrolled.
   * Recomputes the time and the initial velocity to ensure that this is the
   * amount that will be scrolled. Also updates distanceToAbortScroll.
  ...
  /**
   * The distance used to abort the scroll. Set to 0 to make it always scroll a
   * page, even if it dragged just a bit. Defaults to scrollDistance/5.
   *
   * Be sure to call the method setScrollDistance before calling this one.
   * @see #setScrollDistance(int)
   */
  public void setDistanceToAbortScroll(int v) { distanceToAbortScroll = v; }
  ...
  public void addEventSource(Control c) { addEvents(c); }

  private void addEvents(Control c)
  {
    c.addPenListener(this);
    c.addTimerListener(this);
    // So a container event listener can listen to events targeting the
  ...
  private void initialize(int dragId, int x, int y, int t)
  {
    ...
    // Adjust resolutions, which can change during rotation. some devices don't
    // report properly.
  ...
  {
    boolean isHoriz =
      direction == DragEvent.LEFT || direction == DragEvent.RIGHT;
  ...
  @Override public void penDrag(DragEvent e)
  {
    ...
    if (absDeltaY >= absDeltaX &&
        forcedFlickDirection == HORIZONTAL_DIRECTION_ONLY) {
      return; // deltaY = absDeltaY = 0;
    } else if (absDeltaX >= absDeltaY &&
               forcedFlickDirection == VERTICAL_DIRECTION_ONLY) {
    ...
    if (direction != 0) {
      if ((flickDirection != 0 && direction != flickDirection) ||
          (-v / a) < shortestFlick) { // if flick direction changed or movement
                                      // was too slow, reset flick start
        dragT0 = t;
        dragX0 = x;
        dragY0 = y;
      }
  ...
  @Override public void penDragEnd(DragEvent e)
  {
    ...
    int absDeltaX = deltaX < 0 ? -deltaX : deltaX;
    int absDeltaY = deltaY < 0 ? -deltaY : deltaY;
    // if (absDeltaX <= Settings.touchTolerance && absDeltaY <=
    ...
    if (flickDirection == 0) {
      a = 0;
      cancelFlick = (absDeltaY >= absDeltaX &&
                     forcedFlickDirection == HORIZONTAL_DIRECTION_ONLY) ||
                    (absDeltaX >= absDeltaY &&
                     forcedFlickDirection == VERTICAL_DIRECTION_ONLY);
    ...
    if (scrollDistance != 0) {
      boolean isHorizontal =
        flickDirection == DragEvent.RIGHT || flickDirection == DragEvent.LEFT;
      boolean forward =
        flickDirection == DragEvent.RIGHT || flickDirection == DragEvent.DOWN;
      // case where a flick was started in a single direction but the user made
      // a new drag in a non-valid direction e.g.: was flicking to left but user
      // moved to up
      if (a != 0) {
        lastA = a;
      ...
      int dragged =
        e.direction == DragEvent.LEFT || e.direction == DragEvent.RIGHT
          ? e.xTotal
          : e.yTotal;
      if (dragged < 0) {
        dragged = -dragged;
      ...
      // not enough to move?
      if (cancelFlick ||
          (consecutiveDragCount <= 1 && distanceToAbortScroll > 0 &&
           dragged < distanceToAbortScroll)) {
        int s0 = initialRealPos < 0 ? -initialRealPos : initialRealPos;
        int sf = rpos < 0 ? -rpos : rpos;

        if ((s0 % scrollDistance) != 0) {
          scrollDistanceRemaining =
            forward ? runnedDistance : remainingDistance;
        } else {
          if (s0 == sf) {
            ;
          } else if (s0 < sf) {
            ...
            forward = false;
            lastFlickDirection = flickDirection =
              isHorizontal ? DragEvent.LEFT : DragEvent.UP;
            ...
            forward = true;
            lastFlickDirection = flickDirection =
              isHorizontal ? DragEvent.RIGHT : DragEvent.DOWN;
      ...
      if (consecutiveDragCount > 1) {
        scrollDistanceRemaining +=
          (consecutiveDragCount - 1) * scrollDistance; // acceleration
      }
      ...
      // When the flick ends. No rounding is done, the maximum rounding error is
      // 1 millisecond.
      t1 = (int)(-v0 / a);
   ...
   */
  @Override public void timerTriggered(TimerEvent e)
  {
    if (e == timer && !totalcross.unit.UIRobot.abort) {
      ...
      if (scrollDistance != 0 && absNewFlickPos > scrollDistanceRemaining) {
        newFlickPos =
          newFlickPos < 0 ? -scrollDistanceRemaining : scrollDistanceRemaining;
      }
      ...
      if (!endReached) {
        switch (flickDirection) {
          ...
          case DragEvent.DOWN:
            if (listeners != null) {
              for (int i = listeners.size(); --i >= 0;) {
                ((Scrollable)listeners.items[i])
                  .scrollContent(0, -flickMotion, true);
              }
            }
          ...
          case DragEvent.RIGHT:
            if (listeners != null) {
              for (int i = listeners.size(); --i >= 0;) {
                ((Scrollable)listeners.items[i])
                  .scrollContent(-flickMotion, 0, true);
              }
            }
