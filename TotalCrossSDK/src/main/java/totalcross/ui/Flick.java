...
public class Flick implements PenListener, TimerListener {
	 ...
	 */
	public static double defaultFlickAcceleration = Math.max(Settings.screenWidth,
	...
	public void penDrag(DragEvent e) {
		...
		if (direction != 0) {
			if ((flickDirection != 0 && direction != flickDirection)
				|| (-v / a) <
				shortestFlick) { // if flick direction changed or movement was too slow, reset flick start
				dragT0 = t;
				dragX0 = x;
				dragY0 = y;
			}
	...
	public void penDragEnd(DragEvent e) {
		...
		if (scrollDistance != 0) {
			...
			// not enough to move?
			if (cancelFlick || (consecutiveDragCount <= 1 && distanceToAbortScroll > 0
