...
public class Whiteboard extends Control
{
  ...
  /**
   * Set to true to enable antialiase on the line drawing. It must be set right
  ...
  /**
   * Now that we know our bounds, we can create the image that will hold the
   * drawing
   */
  @Override public void onBoundsChanged(boolean screenChanged)
  {
    // if (!screenChanged)
    if (isEmpty) {
      try {
  ...
  @Override public int getPreferredHeight() { return FILL; }
  ...
  public int getPenColor() { return desiredPenColor; }

  @Override public void onPaint(Graphics g)
  {
    if (!Settings.isOpenGL && gScr == null) {
      gScr = getGraphics(); // create the graphics object that will be used to
                            // repaint the image
  ...
  @Override public void onEvent(Event event)
  {
    ...
    switch (event.type) {
      ...
      case PenEvent.PEN_DOWN:
        ...
        getParentWindow().setGrabPenEvents(
          this); // guich@tc100: redirect all pen events to here, bypassing
                 // other processings
