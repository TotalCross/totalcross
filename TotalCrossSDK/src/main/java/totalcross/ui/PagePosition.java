...
public class PagePosition extends Control {
	...
	public void onPaint(Graphics g) {
		...
		for (int i = rightArrow ? n : exactFraction ? visibleCount : count % visibleCount; --i >= 0;
			 x += k) {
			g.drawCircle(x + r, y + r, r);
		}
		// draw the current position
		int p = n == 1 ? mid : leftArrow
				&& rightArrow ? pos % visibleCount : rightArrow ? pos : pos % visibleCount;
		x = x0 + (n == 1 ? 0 : p * k);
