 ...
 * <li> count: the number of balls that will be displayed. Can be less, equal or
 * greater than visible count <li> position: the current position of the filled
 * ball
 * </ol>
 *
...
public class PagePosition extends Control
{
  ...
  /**
   * Sets the current position (starting at 1). Invalid values are forced to a
   * valid range.
   */
  ...
  /**
   * Increments the current position. If wrap is true, when position gets above
   * count, it wraps to 1.
   */
  ...
  /**
   * Decrements the current position. If wrap is true, when position gets below
   * 1, it wraps to count.
   */
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    boolean leftArrow =
      count > visibleCount && (n == 1 ? position > 1 : pos >= visibleCount);
    boolean rightArrow =
      count > visibleCount && (n == 1 ? position < count : curPage < pageCount);
    ...
    for (int i =
           rightArrow ? n : exactFraction ? visibleCount : count % visibleCount;
         --i >= 0;
         x += k) {
    ...
    int p = n == 1
              ? mid
              : leftArrow && rightArrow ? pos % visibleCount
                                        : rightArrow ? pos : pos % visibleCount;
    x = x0 + (n == 1 ? 0 : p * k);
  ...
  @Override public int getPreferredHeight() { return fmH / 2; }
}
