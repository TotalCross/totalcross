 ...
 * <p>
 * You can create a horizontal endless ProgressBar, always going from left to
 * right, by setting the given parameters: <ul> <li> call setEndless() <li>
 * max-min: used to compute the width of the bar <li> prefix and suffix:
 * displayed, but the current value is not displayed <li> setValue(n): n used to
 * increment the current value, not to set the value to n.
 * </ul>
...
public class ProgressBar extends Control
{
  ...
  /**
   * The string prefix. The displayed label will be prefix+value+sufix. The
  ...
  /**
   * The string prefix. The displayed label will be prefix+value+sufix. The
   * default is the percentage symbol.
   */
  ...
  /**
   * Set to false to don't let the value be drawn. Note that the prefix and
   * suffix will still be drawn.
   * @since TotalCross 1.0
   */
  ...
  /**
   * Creates a progress bar, with minimum and maximum set as 0 and 100,
   * respectively.
   */
  public ProgressBar() { this(0, 100); }

  /**
   * Creates a progress bar, String is set to <code>null</code>, which is the
   * default. Uses the specified minimum for the initial value of the progress
   * bar.
  ...
  /**
   * Call this method to make this ProgressBar a horizontal endless
   * progressbar; just keep calling <code>setValue(step)</code> to increase the
  ...
  { // guich@tc125_19
    if (vertical) {
      throw new RuntimeException(
        "Endless progressbar cannot be vertical, only horizontal)");
    }
    suffix = "";
    endless = true;
  }
  ...
  {
    if (vertical) {
      return UnitsConverter.toPixels(4 + DP) +
        (!endless && drawText ? fm.stringWidth(max + prefix + suffix) : 0);
    }
   ...
   *
   * If you call this method and the bar isnt updated, you can try to call
   * <code>MainWindow.pumpEvents()</code>.
   *
  ...
  /**
   * Sets the current value and the prefix and suffix. Note that, due to
   * performance reasons, min and max are not verified. This does not check if
   * the value had changed; it always repaint the progress bar immediately.
   *
  ...
  @Override public void onColorsChanged(boolean b) { npback = npfore = null; }
  ...
  /**
   * Paint the Progress Bar. The filled part of the bar is painted with the
   * foreground color; the empty part of the bar is painted with the background
   * color; the text is painted with the defined textColor color; no border is
   * drawn.
   */
  @Override public void onPaint(Graphics g)
  {
    ...
    int s = (dif == 0 || value == max)
              ? size
              : (value == 0)
                  ? 0
                  : (int)((long)size * (long)(value - min) / (long)dif);
    if (s > size) {
      s = size;
    }
      ...
      try {
        if (npback == null) {
          npback = NinePatch.getInstance().getNormalInstance(
            npParts, width, height, bc, false);
          npfore = NinePatch.getInstance().getNormalInstance(
            npParts, width, height, fc, false);
        }
        ...
        if ((uiMaterial || uiVista) && isEnabled()) { // guich@573_6
          if (uiMaterial) {
            ...
            if (vertical) {
              ...
              g.fillVistaRect(value - filledBarSize,
                              0,
                              filledBarSize,
                              height,
                              fc,
    ...
    if (drawText) {
      ...
      int shadow = uiMaterial
                     ? -1
                     : textShadowColor != -1
                         ? textShadowColor
                         : !highlight ? -1
                                      : highlightColor == -1
                                          ? Color.getCursorColor(textColor)
                                          : highlightColor;
