...
/**
 * Spinner is a control that shows an image indicating that something is
 * running in the background.
 *
 * To start the spin call the start method, and to stop it call the stop method.
 *
 * If you try to run a spinner inside a tight loop, be sure to call
 * <code>spinner.update()</code> or the spinner will not be updated. Without
 * this, it will work in Java but not on devices.
 *
...
public class Spinner extends Control implements TimerListener, UpdateListener
{
  ...
  /**
   * Defines the type of spinner for all instances. Defaults for IPHONE when
   * running in iPhone and ANDROID for all other platforms.
   */
  ...
  public Spinner() { this(spinnerType); }
  ...
  public void setType(int t)
  {
    ...
    try {
      this.setImage(loaded[t] == null ? loaded[t] = new Image(files[t])
                                      : loaded[t]);
    } catch (Exception e) {
   ...
   * You can download additional animations from: <a
   * href='http://preloaders.net/en'>here</a>. Change only the given settings:
   * <ul>
   *  <li> Image type: GIF
   *  <li> Transparent background: Yes
   *  <li> Foreground color: FFFFFF if the animation is only black, 000000 if it
   * has fade. <li> Background color: 000000 <li> Keep size 128 x 128
   * </ul>
   * Then press Generate preloader and download the gif file that will appear at
   * the right pane. If the spinner is moving counterclockwise, you can make it
   * go clickwise by changing also, under the  Advanced options: <ul> <li> Flip
   * image: Hor <li> Reverse animation: Yes
   * </ul>
   * The image is colorized with the foreground color.
   * If it appears not filled, try selecting the "Invert colors" option, and use
  ...
  @Override public void onBoundsChanged(boolean screenChanged) { anim = null; }

  @Override public void onColorsChanged(boolean changed) { anim = null; }
  ...
  @Override public void onPaint(Graphics g)
  {
    ...
    if (anim != null) {
      g.drawImage(
        anim, (width - anim.getWidth()) / 2, (height - anim.getHeight()) / 2);
    }
  }
  ...
  {
    try {
      anim = anim0.smoothScaledFixedAspectRatio(width < height ? width : height,
                                                true);
      if (type != -1) {
  ...
  @Override public void timerTriggered(TimerEvent e) { step(); }
