...
public class ComboBoxDropDown extends Window {
	...
	public boolean dontHideParent =
		true; // guich@200b4_205: let the user choose to not hide the parent control
	...
	public void setRect(int x, int y, int width, int height, Control relative, boolean screenChanged) {
		if (lb.size() > 0) {
			...
			if (fullWidth) {
				...
				width = Math.min(Math.max(uiFlat ? width : (width - 2), lb.getPreferredWidth()),
								 Settings.screenWidth - 10);
				if (x + width >
					Settings.screenWidth) { // guich@220_24: make sure the pop doesnt go beyond the screen
			...
			if (prefH <= remainsAtBottom) { // can fit at bottom?
				...
				if (dontHideParent) {
					...
					y += height - prefH -
						 1; // put window below parent's - guich@210_5: fix popping up a PalmOS Combo placed in the LEFT,BOTTOM.
				}
				height = prefH;
	...
	protected boolean onClickedOutside(PenEvent event) {
		if (event.type ==
			PenEvent.PEN_DOWN) { // kmeehl@tc100: only unpop for pen_down, to make it easier to manipulate
	...
	public void onEvent(Event event) {
		switch (event.type) {
			case KeyEvent.SPECIAL_KEY_PRESS:
				if (event.target == lb
					&& ((KeyEvent) event).key ==
					SpecialKeys.ESCAPE) { // brunos@tc120: ESCAPE means: cancel selection and close dropdown
