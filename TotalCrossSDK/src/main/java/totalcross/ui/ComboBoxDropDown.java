...
/**
 * ComboBoxDropDown is a simple implementation of a PopUp Listbox. Used by the
 * ComboBox class.
 */
...
public class ComboBoxDropDown extends Window
{
  ...
  public ListBox lb;
  /**
   * Set to true if want to make the control popup below or above always, and
   * not only if WinCE
   */
  public boolean dontHideParent = true; // guich@200b4_205: let the user choose
                                        // to not hide the parent control
  ...
  /**
   * Creates a ComboBoxDropDown with coordinates that will be set later via the
   * setRect method.
   */
  public ComboBoxDropDown() { this(new ListBox()); }

  /**
   * Creates a ComboBoxDropDown with the given ListBox. You can extend the
   * ListBox to draw the items by yourself and use this constructor so the
   * ComboBoxDropDown will use your class and not the default ListBox one. Note
   * that this constructor forces the ListBox.simpleBorder to true.
   */
                      ...
                      boolean screenChanged)
  {
    if (lb.size() > 0) {
      ...
      if (fullWidth) {
        ...
        int origW = width;
        // width fit to screen - keep same size of parent control if possible,
        // and less than the screen width
        width = Math.min(
          Math.max(uiFlat ? width : (width - 2), lb.getPreferredWidth()),
          Settings.screenWidth - 10);
        if (x + width >
            Settings.screenWidth) { // guich@220_24: make sure the pop doesnt go
                                    // beyond the screen
      ...
      if (prefH <= remainsAtBottom) { // can fit at bottom?
        ...
        if (dontHideParent) {
          ...
          y += height - prefH -
               1; // put window below parent's - guich@210_5: fix popping up a
                  // PalmOS Combo placed in the LEFT,BOTTOM.
        }
        height = prefH;
        ...
        if (fullHeight /* && prefH > Settings.screenHeight*/) { // guich@330_52
                                                                ...
                                                                // 2nd part
        height =
          Math.min(prefH, ((int)((Settings.screenHeight - 6) / ih)) * ih + 6);
        ...
        if (!dontHideParent) { // guich@220_24: hide parent? add one more row to
                               // the height
          height += ih;
          y += ih;
        }
  ...
  {
    if (event.type ==
        PenEvent.PEN_DOWN) { // kmeehl@tc100: only unpop for pen_down, to make
                             // it easier to manipulate
      setHighlighting();     // kmeehl@tc100: set highlighting back to allow
                             // navigation through controls again
      unpop();
    }
    return true;
  }
  ...
  @Override public void onEvent(Event event)
  {
    switch (event.type) {
      case KeyEvent.SPECIAL_KEY_PRESS:
        if (event.target == lb &&
            ((KeyEvent)event).key ==
              SpecialKeys.ESCAPE) {      // brunos@tc120: ESCAPE means: cancel
                                         // selection and close dropdown
          lb.setSelectedIndex(selected); // restore original selection, since
                                         // the user is canceling
          requestFocus(); // guich@tc123_15
          unpop();
        }
        break;
      // case KeyEvent.ACTION_KEY_PRESS: // kmeehl@tc100 - guich@tc113_9:
      // commented out
