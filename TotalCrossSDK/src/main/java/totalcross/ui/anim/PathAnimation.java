...
public class PathAnimation extends ControlAnimation
{
  ...
  /**
   * Creates a path animation, moving the control to the given x and y
   * positions.
   * @param c The control to be moved
   * @param toX The destination X coordinate
   * @param toY The destination Y coordinate
   * @param animFinish An interface method to be called when the animation
   *     finished, or null if none.
   * @param totalTime The total time in millis that the animation will take, or
   *     -1 to use the default value (800ms).
   */
   ...
   * @param c The control to be moved
   * @param direction One of BOTTOM, -BOTTOM, TOP, -TOP, LEFT, -LEFT, RIGHT,
   *     -RIGHT. Any other value will return null.
   * @param animFinish An interface method to be called when the animation
   *     finished, or null if none.
   * @param totalTime The total time in millis that the animation will take, or
   *     -1 to use the default value (800ms).
                                     ...
                                     int slack)
  {
    ...
    int x0, y0, xf, yf;
    int pw =
      c instanceof Window ? Settings.screenWidth : c.getParent().getWidth();
    int ph =
      c instanceof Window ? Settings.screenHeight : c.getParent().getHeight();
    int cw = c.getWidth();
    int ch = c.getHeight();
    xf = x0 = (pw - cw) / 2;
    y0 = yf = (ph - ch) / 2;
