...
/**
 * This class provides a title area and a button area (at right). The title and
 * the button are optional, although it doesn't make sense to have a
 * <code>Bar</code> without title and buttons.
 *
 * You can add or remove buttons, and change the title text; the title text can
 * have an icon at left.
 *
 ...
 * </pre>
 *
 * A <code>ControlEvent.PRESSED</code> is sent to the caller, and the button
 * index can be retrieved using the <code>getSelectedIndex()</code> method.
 *
...
public class Bar extends Container
{
  ...
  /**
   * The title horizontal alignment (<code>LEFT</code>, <code>CENTER</code>, or
  ...
  public class BarButton extends Control
  {
    ...
    public BarButton(String title, Image icon)
    { // title or icon
      this(title);
      this.icon0 = icon;
    }

    public BarButton(String title, Icon icon)
    { // title or icon
      this(title);
      this.realIcon = icon;
    }
    ...
    @Override public void onFontChanged()
    {
      if (title != null) {
        ...
        if (leftIcon0 != null) {
          try {
            leftIcon = null;
            leftIcon = leftIcon0.getSmoothScaledInstance(
              leftIcon0.getWidth() * fmH / leftIcon0.getHeight(), fmH);
          } catch (Exception e) {
            icon = icon0;
          }
        }
        ...
        try {
          icon = null;
          if (icon0 != null) {
            icon = icon0.getSmoothScaledInstance(
              icon0.getWidth() * fmH / icon0.getHeight(), fmH);
          }
        } catch (Exception e) {
          icon = icon0;
        }
    ...
    @Override public void onBoundsChanged(boolean b)
    {
      onFontChanged();
      if (title != null) {
        int a = buttonTitleAlign != -1 ? buttonTitleAlign : titleAlign;
        px = a == LEFT ? gap + 1
                       : a == CENTER ? 2 + (width - fm.stringWidth(title)) / 2
                                     : (width - fm.stringWidth(title) - gap);
        py = (height - fmH) / 2;
      } else if (icon != null) {
      ...
      if (drawBorders && (uiAndroid || uiVista || uiFlat)) {
        ...
        if (backgroundStyle == BACKGROUND_SHADED) {
          g.fillShadedRect(
            ...
            fc,
            c2,
            30); // drawLine causes an unexpected effect on shaded backgrounds
        } else {
          g.drawLine(0, 0, 0, h);
        }
      }
    ...
    {
      if ((!buttonCanSelectTitle && title != null) ||
  ...
  /**
   * Constructs a <code>Bar</code> object without a title. Note that if you call
   * the <code>setTitle()</code> method, a <code>RuntimeException</code> will be
   * thrown.
   *
  ...
  /**
   * An image icon that can be placed at the left of the title. It only shows if
   * there's a title set. Pass <code>null</code> to remove the icon if it was
   * previously set.
   *
  ...
  /**
   * Returns the icon set (and possibly resized) with setIcon, or null if none
   * was set
   */
  public Image getIcon() { return title != null ? title.leftIcon : null; }
  ...
  public void setTitle(String newTitle)
  {
    if (this.title == null) {
      throw new RuntimeException(
        "You can only set a title if you set one in the Bar's constructor.");
    }
   ...
   */
  public String getTitle() { return this.title == null ? "" : title.title; }
   ...
   */
  public int addButton(Image icon) { return addButton(icon, true); }
   ...
   *
   * @param icon The image to the add to a button in the bar.
   * @param atRight if true, button is added at right; if false, button is added
   *     at left.
   * @return The button index
   */
  ...
  /**
   * Sets the given button with an auto-repeat interval in the given
   * milliseconds.
   *
  ...
  /**
   * Adds a control to the bar at right. Not all types of controls are
   * supported.
   *
   * @param c The control to be added.
   * @return The button index
   */
  public int addControl(Control c) { return addControl(c, true); }
  ...
  public int addControl(Control c, boolean atRight)
  {
    ...
    for (int i = icons.size(); --i >= 0;) {
      ...
      cc.appId =
        cc.appId == 0
          ? (atRight ? 1000 : 0)
          : (cc.appId > 1000 ? 1000 : 0); // update appId used for selection
      cc.appId += i + 1;
    }
    if (initialized) {
      initUI();
   ...
   *
   * The title always has index 0 (even if there's no title), and the button'
  ...
  @Override public void initUI()
  {
    ...
    if (n == 1 && !(icons.items[0] instanceof BarButton)) {
    ...
    } else if (title == null) { // if there's no title, make the icons take the
                                // whole size of the container
      for (int i = n; --i > 0;) {
        add((Control)icons.items[i],
            i == n - 1 ? RIGHT : BEFORE,
            TOP,
            PARENTSIZE - n,
            FILL);
      ...
      for (int i = n; --i >= 0;) {
        ...
        if (uiMaterial) {
          ...
          add(c,
              posX,
              TOP,
              getButtonWidth(c.appId > 1000 ? c.appId - 1000 : c.appId),
              FILL,
              rel);
        }
      }
      if (n == 0) {
        ...
        Settings.showUIErrors =
          false; // ignore title if there is not enough space
        add(title, AFTER, TOP, FIT, FILL, spl);
        Settings.showUIErrors = old;
      }
      if (spinner != null) {
        add(spinner,
            RIGHT_OF - (n == 0 ? fmH / 2 : height),
            CENTER_OF,
            FONTSIZE,
            FONTSIZE,
            this.title);
  ...
  /**
   * Called after a <code>setEnabled()</code>, <code>setForeColor()</code>, or
   * <code>setBackColor()</code>; or when a control has been added to a
   * container. If <code>colorsChanged</code> is <code>true</code>, it was
   * called from <code>setForeColor()</code>/<code>setBackColor()</code>/
   * <code>Container.add()</code>; otherwise, it was called from
   * <code>setEnabled()</code>.
   *
   * @param colorsChanged Indicates if the control colors have changed, which
   *     happens after a <code>setForeColor()</code>,
  ...
  @Override public int getPreferredHeight()
  {
    if (uiMaterial) {
      ...
      return Settings.isLandscape()
        ? (landscapePrefH != 0 ? landscapePrefH : fmH * 2)
  ...
  /**
   * Stops and hides the spinner (if createSpinner or setSpinner was called
   * before)
   *
   * @see #spinner
   */
  ...
  /**
   * Repositions this control, calling again <code>setRect()</code> with the
   * original parameters.
   */
  ...
  /**
   * Assigns the BACK key on Android (mapped to <code>SpecialKeys.ESCAPE</code>)
   * to the given button. This can only be called after the bar has been added
   * to a container.
   *
   * For example, if button 1 is assigned with
   * <code>totalcross.res.Resources.back</code>, call
   * <code>assignBackKeyToButton(1);</code>.
   *
  ...
  /**
   * Creates a spinner with the following color. The spinner will be placed at
   * the right of the title (it only works if there's a title).
   *
   ...
   */
  public void setSpinner(Spinner s) { spinner = s; }

  public int getTitleWidth() { return title == null ? 0 : title.getWidth(); }
}
