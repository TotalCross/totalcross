...
public class Bar extends Container {
		...
		public void onFontChanged() {
			if (title != null) {
				...
				if (leftIcon0 != null) {
					try {
						leftIcon = null;
						leftIcon = leftIcon0.getSmoothScaledInstance(leftIcon0.getWidth() * fmH / leftIcon0.getHeight(),
								   fmH);
					} catch (Exception e) {
						icon = icon0;
					}
		...
		public void onPaint(Graphics g) {
			...
			if (drawBorders && (uiAndroid || uiVista || uiFlat)) {
				...
				if (backgroundStyle == BACKGROUND_SHADED) {
					g.fillShadedRect(0, 1, 1, h - 2, true, false, fc, c2,
	...
	public int addControl(Control c, boolean atRight) {
		...
		for (int i = icons.size(); --i >= 0;) {
			...
			cc.appId = cc.appId == 0 ? (atRight ? 1000 : 0) : (cc.appId > 1000 ? 1000 :
					   0); // update appId used for selection
			cc.appId += i + 1;
		}
		if (initialized) {
			initUI();
	...
	public void initUI() {
		...
		if (n == 1 && !(icons.items[0] instanceof BarButton)) {
			add((Control) icons.items[0], LEFT, TOP, FILL, FILL);
				   ...
				   null) { // if there's no title, make the icons take the whole size of the container
			for (int i = n; --i > 0;) {
