...
public class Image4D extends GfxSurface {
	...
	public Image4D(int width, int height) throws ImageException {
		...
		try {
			...
			throw new ImageException("Out of memory: cannot allocated " + width + "x" + height +
									 " offscreen image.");
		}
		init();
	}
	...
	native private void getModifiedInstance(totalcross.ui.image.Image4D newImg, int angle,
											int percScale, int color,
											int brightness, int contrast, int type);

	private totalcross.ui.image.Image4D getModifiedInstance(int newW, int newH, int angle,
			int percScale, int color,
			int brightness, int contrast, int type) throws totalcross.ui.image.ImageException {
		if (type != ALPHA_INSTANCE && type != FADED_INSTANCE && newW == width && newH == height
			&& (angle % 360) == 0
	...
	public totalcross.ui.image.Image4D smoothScaledFixedAspectRatio(int newSize,
	...
	public Image4D scaledBy(double scaleX,
							double scaleY) throws totalcross.ui.image.ImageException { // guich@402_6
	...
	public Image4D smoothScaledBy(double scaleX, double scaleY) throws
		totalcross.ui.image.ImageException { // guich@402_6
		return ((scaleX == 1 && scaleY == 1) || scaleX <= 0 || scaleY <= 0) ? this
	 ...
	 * @since TotalCross 1.12
	 */
	final public Image4D smoothScaledFromResolution(int originalRes) throws
	...
	@Deprecated
	public Image4D getSmoothScaledInstance(int newWidth, int newHeight,
	...
	/** @deprecated TotalCross 2 no longer uses the backColor parameter. */
	@Deprecated
	public Image4D smoothScaledFixedAspectRatio(int newSize, boolean isHeight,
			int backColor) throws ImageException { // guich@402_6
		return smoothScaledFixedAspectRatio(newSize, isHeight);
	}
