...
public class Image4D extends GfxSurface
{
  ...
  public Image4D(int width, int height) throws ImageException
  {
    ...
    try {
      ...
      throw new ImageException("Out of memory: cannot allocated " + width +
                               "x" + height + " offscreen image.");
    }
    init();
  }
  ...
  public Image4D(Stream s) throws ImageException, totalcross.io.IOException
  {
    ...
    // the buffer must go initially filled so that the native parser can
    // discover if this is a jpeg or png image
  ...
  public Image4D(byte[] fullDescription, int len) throws ImageException
  {
    ...
    bas.skipBytes(
      4); // first 4 bytes are read directly from the fullDescription buffer
  ...
  private Image4D(Image4D src)
  {
    ...
    if (instanceCount[0] == 0) {
      this.master =
        new Image4D[] { src }; // must keep a copy of the original image
    } else {
  ...
  public void createPng(Stream s)
    throws ImageException, totalcross.io.IOException
  {
    try {
  ...
  native private void getModifiedInstance(totalcross.ui.image.Image4D newImg,
                                          int angle,
                                          int percScale,
                                          int color,
                                          int brightness,
  ...
  private totalcross.ui.image.Image4D getModifiedInstance(int newW,
                                                          int newH,
                                                          int angle,
                                                          int percScale,
                                                          int color,
  ...
  {
    if (type != ALPHA_INSTANCE && type != FADED_INSTANCE && newW == width &&
        newH == height && (angle % 360) == 0 && brightness == 0 &&
        contrast == 0) {
      return this;
    }
    ...
    getModifiedInstance(
      imageOut, angle, percScale, color, brightness, contrast, type);
    if (type != ROTATED_SCALED_INSTANCE && frameCount > 1) {
      imageOut.setFrameCount(frameCount);
    }
    return imageOut;
  ...
  { // guich@tc110_50
    return getModifiedInstance(
      width, height, 0, 0, backColor, 0, 0, FADED_INSTANCE);
  }
  ...
  {
    return getModifiedInstance(
      width, height, 0, 0, delta, 0, 0, ALPHA_INSTANCE);
  }

  public totalcross.ui.image.Image4D smoothScaledFixedAspectRatio(
    int newSize,
    boolean isHeight) throws ImageException
  { // guich@402_6
  ...
  public totalcross.ui.image.Image4D getScaledInstance(int newWidth,
                                                       int newHeight)
    throws totalcross.ui.image.ImageException
  { // guich@350_22
    return getModifiedInstance(
      newWidth, newHeight, 0, 0, -1, 0, 0, SCALED_INSTANCE);
  }

  public totalcross.ui.image.Image4D getSmoothScaledInstance(int newWidth,
                                                             int newHeight)
    throws totalcross.ui.image.ImageException
  { // guich@350_22
    return getModifiedInstance(
      newWidth, newHeight, 0, 0, 0, 0, 0, SMOOTH_SCALED_INSTANCE);
  }

  public totalcross.ui.image.Image4D getRotatedScaledInstance(int percScale,
                                                              int angle,
                                                              int fillColor)
    throws totalcross.ui.image.ImageException
  {
    ...
    int y0 = ((hIn << 16) -
             ...
             2;
    return getModifiedInstance(
      wOut, hOut, percScale, angle, 0, x0, y0, ROTATED_SCALED_INSTANCE);
  }

  public totalcross.ui.image.Image4D getTouchedUpInstance(byte brightness,
                                                          byte contrast)
    throws totalcross.ui.image.ImageException
  {
    return getModifiedInstance(
      width, height, 0, 0, 0, brightness, contrast, TOUCHEDUP_INSTANCE);
  }

  public Image4D scaledBy(double scaleX, double scaleY)
    throws totalcross.ui.image.ImageException
  { // guich@402_6
  ...
  public Image4D smoothScaledBy(double scaleX, double scaleY)
    throws totalcross.ui.image.ImageException
  { // guich@402_6
    return ((scaleX == 1 && scaleY == 1) || scaleX <= 0 || scaleY <= 0)
      ? this
  ...
  native public void createJpg(Stream s, int quality)
    throws ImageException, IOException;
  ...
  public Image4D hwScaledFixedAspectRatio(int newSize, boolean isHeight)
    throws ImageException
  {
    return smoothScaledFixedAspectRatio(newSize, isHeight);
  }

  public Image4D getHwScaledInstance(int width, int height)
    throws ImageException
  {
   ...
   * @since TotalCross 1.12
   */
  final public Image4D smoothScaledFromResolution(int originalRes)
    throws ImageException
  { // guich@tc112_23
  ...
  @ReplacedByNativeOnDeploy private boolean nativeEquals(Image4D other)
  {
    Image4D img = other;
  ...
  @Deprecated
  public Image4D getSmoothScaledInstance(int newWidth,
                                         int newHeight,
  ...
  /** @deprecated TotalCross 2 no longer uses the backColor parameter. */
  @Deprecated
  public Image4D smoothScaledBy(double scaleX, double scaleY, int backColor)
    throws ImageException
  { // guich@402_6
    return smoothScaledBy(scaleX, scaleY);
  }
  ...
  /** @deprecated TotalCross 2 no longer uses the backColor parameter. */
  @Deprecated
  public Image4D smoothScaledFixedAspectRatio(int newSize,
                                              boolean isHeight,
                                              int backColor)
    throws ImageException
  { // guich@402_6
   ...
   * @since TotalCross 3.1
   */
  public Image4D getCopy() throws ImageException { return getFrameInstance(0); }
   ...
   */
  public Image4D getClippedInstance(int x, int y, int w, int h)
    throws ImageException
  {
  ...
  private static boolean hasNativeResizeJpeg() { return Settings.isIOS(); }

  native public static void nativeResizeJpeg(String inputPath,
                                             String outputPath,
                                             int maxPixelSize);

  public static native Image getJpegBestFit(String path,
                                            int targetWidth,
                                            int targetHeight)
    throws java.io.IOException, ImageException;

  public static native Image getJpegScaled(String path,
                                           int scaleNumerator,
