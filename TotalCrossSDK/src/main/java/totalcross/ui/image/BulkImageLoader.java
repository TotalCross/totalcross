...
/**
 * This class is used to load and release lots of images when used for a book,
 * for example. It loads the images in a thread and release old images to keep
 * memory usage low. Use it with DynImage. See the ImageBook and the Product
 * Store samples.
 */
...
public class BulkImageLoader extends Thread
{
  ...
  public class DynImage extends Control
  {
    ...
    public DynImage(int idx) { this.idx = idx; }
    ...
    @Override public void onPaint(Graphics g)
    {
      ...
      try {
        ...
        if (img != null) {
          ...
          g.drawImage(
            img, (width - img.getWidth()) / 2, (height - img.getHeight()) / 2);
          img.hwScaleH = oldH;
          img.hwScaleW = oldW;
        }
      } catch (Exception ioe) {
                         ...
                         String[] arqs)
  {
    ...
    if (pagesInMemory > 6) {
      Vm.debug(
        "Caution: use a smaller value for pagesInMemory (up to 6) because it takes lot of memory!");
    }
  }
  ...
  @Override public void run()
  {
    ...
    while (true) {
      ...
      lastEnd = lastMaxIdx;
      ex = null;
      // Vm.tweak(Vm.TWEAK_DISABLE_GC,true); -- uncommenting these lines will
      // load faster but will take much more memory
