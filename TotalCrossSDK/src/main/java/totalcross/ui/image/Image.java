 ...
 * object. Images are always 24bpp, and TotalCross supports PNG and JPEG formats
 at device,
 * and PNG/JPEG/GIF/BMP at desktop (the last two are converted to png when
 deploying).
 * The deployed png may contain transparency information which is correctly
 handled.
 * <p>
 * This is a code sample of how to run a multi-framed image. Note that it will
 work
 ...
 * </pre>
 * Some transformation methods returns a new instance of this image and other
 apply to the current instance.
 * To preserve an image with a single frame, use
 <code>getFrameInstance(0)</code>.
 *
 * Note: TotalCross does not support grayscale PNG with alpha-channel. Convert
 the image to true-color with
 * alpha-channel and it will work fine (the only backdraw is that the new image
 will be bigger).
 *
 * The hwScale methods should not be used in images that are shown using
 transition effects.
 *
 * @see Graphics
 */
public class Image extends GfxSurface
{
  ...
  public int surfaceType =
    1; // don't move from here! must be static at position 0
  protected int width;
  ...
  /**
   * A global alpha mask to be applied to the whole image when drawing it,
   ...
   * If you set this in non-opengl environments, nothing will happen; you should
   * use the hwScaledBy, getHwScaledInstance and hwScaledFixedAspectRatio
   * methods.
   *
   ...
   * @since TotalCross 2.0
   */
  public Image hwScaledFixedAspectRatio(int newSize, boolean isHeight)
  ...
  public Image(int width, int height) throws ImageException
  {
    ...
    try {
      ...
      throw new ImageException("Out of memory: cannot allocate " + width + "x" +
                               height + " offscreen image.");
    }
    init();
  }
  ...
  /**
   * Loads and constructs an image from a file. The path given is the path to
   * the image file. The file must be in 2, 16, 256, 24bpp color compressed
   * (RLE) or uncompressed BMP bitmap format, or a PNG file, or a GIF file, or a
   * JPEG file. If the image cannot be loaded, an ImageException will be thrown.
  ...
  /**
   * Loads a BMP, JPEG, GIF or PNG image from a totalcross.io.Stream. Note that
   * Gif and BMP are supported only at desktop. Note that all the bytes of the
   * given stream will be fetched, even those bytes that may follow this Image.
  ...
  public Image(Stream s) throws ImageException, totalcross.io.IOException
  {
    ...
    if (width == 0) {
      throw new ImageException("Error on bmp with " + bas.getPos() +
  ...
  /**
   * Returns the path used to create the Image. For constructors that don't
   * receive a path, returns null
   */
  ...
  {
    int[] pixels =
      (int[])((frameCount == 1) ? this.pixels
                                : this.pixelsOfAllFrames); // guich@tc100b5_40
    for (int i = pixels.length; --i >= 0;) {
      int p = pixels[i] & 0xFFFFFF;
      pixels[i] =
        (p == color)
          ? color
          : p | 0xFF000000; // if is the transparent color, set the alpha to
                            // 0, otherwise, set to full bright
    }
    return this;
  }
   ...
   * specify the full JPEG/PNG image, with headers (Gif/Bmp are supported at
   * desktop only). Here is a code example: <pre>
   ...
   * // parse the saved png
   * Image im = new Image(bas.getBuffer()); // Caution! the buffer may be
   * greater than totalBytesWritten, but when parsing theres no problem. if
   * (im.getWidth() > 0) // successfully parsed?
   * {
   *    getGraphics().drawImage(im,CENTER,CENTER);
   *    Vm.sleep(2000);
   * }
   * </pre>
   * Caution: if reading a JPEG file, the original array contents will be
   * changed!
   * @throws totalcross.ui.image.ImageException Thrown when something was wrong
   *     with the image.
   */
  ...
  /**
   * Parses an image from the given byte array with the specified length. Note
   * that the byte array must specify the full JPEG/PNG image, with headers
   * (Gif/Bmp are supported at desktop only). Here is a code example: <pre>
   * // create the image and fill it with something
   * Image img = new Image(160,160);
   * Graphics g = img.getGraphics();
   * for (int i =0; i < 16; i++)
  ...
  public Image(byte[] fullDescription, int length) throws ImageException
  {
    ...
    if (width == 0) {
      throw new ImageException(fullDescription == null
                                 ? "Description is null"
                                 : ("Error on image with " +
  ...
  /**
   * Sets the frame count for this image. The width may be a multiple of the
   * frame count. After the frame count is set, it cannot be changed.
   *
    ...
    throws IllegalArgumentException, IllegalStateException, ImageException
  {
    ...
    if (n < 1) {
      throw new IllegalArgumentException(
        "Argument 'n' must have a positive value");
    }
    ...
    if (n != frameCount && n > 1 && frameCount <= 1) {
      try {
        ...
        throw new ImageException(
          "Not enough memory to create the single frame");
      }
    }
  }
  ...
  final public void setCurrentFrame(int nr)
  {
    ...
    for (int y = height - 1; y >= 0; y--) {
      Vm.arrayCopy(pixelsOfAllFrames,
                   nr * width + y * widthOfAllFrames,
                   pixels,
                   y * width,
                   width);
    }
  ...
  /**
   * Returns the height of the image. You can check if the image is ok
   * comparing this with zero.
   */
  @Override public int getHeight() { return (int)(height * hwScaleH); }

  /**
   * Returns the width of the image. You can check if the image is ok comparing
   * this with zero.
   */
  @Override public int getWidth() { return (int)(width * hwScaleW); }
  ...
  public Graphics getGraphics()
  {
    if (Launcher.instance != null && Launcher.instance.mainWindow != null) {
      gfx.setFont(MainWindow.getDefaultFont()); // avoid loading the font if
                                                // running from tc.Deploy
    }
    gfx.refresh(0, 0, width, height, 0, 0, null);
    return gfx;
  }
   ...
   * In Open GL platforms, creates a texture for this image. This is already
   * done, lazily, when the image is going to be painted. If you want to speedup
   * paint, call this method as soon as any changes in the image are finished.
   *
   ...
   *
   * Note this replaces a single solid color by another solid color. If you want
   * to change a gradient, or colorize an image, use the applyColor method
   * instead.
   *
   * You must pass the color with the alpha channel (usually, 0xFF).
   * For example, to change a red to green, use from=0xFFFF0000 (0xFF0000 with
   * alpha=0xFF), to=0xFF00FF00.
   *
   * @see #applyColor(int)
   * @see #applyColor2(int)
   */
  ...
  {
    int[] pixels =
      (int[])(frameCount == 1 ? this.pixels : this.pixelsOfAllFrames);
   ...
   * <ul>
   * <li>The stored image size is limited to near 64Kb. Note that a stored image
   * size has no relation to its size in pixels. For example, a 1300x1200
   * completely-white PNG file takes 7Kb of storage size but 6MB of RAM when
   * loaded. <li>The PDBFile can save multiple images, but the record must be
   * prefixed with the image's name and must be sorted. <li>This method finds
   * the exact place where to insert the png and puts it there. <li>If you want
   * to create a png to be transfered by a stream to serial or socket then you
   * must use the method createPng instead. <li>If a record with this name
   * already exists, it will be replaced. <li>The name is always converted to
   * lowercase and the method makes sure that .png is appended to it. <li>To get
   * the list of images in a PDBFile, just do a readString at the beginning of
   * each record.
   ...
   * // create the PDBFile to save the image. You must change CRTR to match your
   * apps creator ID String pdbName = "images.CRTR.TYPE"; PDBFile pdb = new
   * PDBFile(pdbName, PDBFile.CREATE); img.saveTo(pdb, "boxcircle.png");
   * pdb.close();
   ...
   */
  public void saveTo(PDBFile cat, String name)
    throws ImageException, IOException
  {
    ...
    ResizeRecord rs = new ResizeRecord(
      cat,
      Math.min(65500,
               width * height * 3 +
                 200)); // guich@tc114_17: make sure is not bigger than 64k
  ...
  /**
   * Loads an image from a PDB file, if it was previously saved using saveTo
   * method.
   * @see #saveTo(PDBFile, String)
   * @since TotalCross 1.22
   */
  public static Image loadFrom(PDBFile cat, String name)
    throws IOException, ImageException
  {
    ...
    if (idx == -1) {
      throw new IOException("The image " + name + " is not inside " +
                            cat.getName());
    }
  ...
  private static int findPosition(PDBFile cat, String name, boolean isWrite)
    throws IOException
  {
    DataStream ds = new DataStream(cat);
    // guich@200b4_45: fixed the insert_in_order routine
    int n = cat.getRecordCount();
    for (int i = 0; i < n; i++) { // find the correct position to insert the
                                  // record. the records must be sorted
      cat.setRecordPos(i);
   ...
   * @param quality The quality of the image; 100 = no compression, 90 = medium
   *     compression,
   * 80 = high compression. Anything below 80 may greatly redude the image's
   * quality. 85 is a common value. In JavaSE, the quality argument is ignored.
   * @throws ImageException
   * @throws IOException
   */
  public void createJpg(Stream s, int quality)
    throws ImageException, IOException
  {
    try {
      java.awt.image.MemoryImageSource screenMis =
      ...
      java.awt.Image screenImg =
        java.awt.Toolkit.getDefaultToolkit().createImage(screenMis);
      screenMis.newPixels();

      java.awt.image.BufferedImage dest = new java.awt.image.BufferedImage(
      ...
      java.io.ByteArrayOutputStream bos =
        new java.io.ByteArrayOutputStream(width);
  ...
  public void createJpg4B(Stream s, int quality)
    throws ImageException, IOException
  {}

  /**
   * Saves this image as a 24 BPP .png file format (if useAlpha is true, it
   * saves as 32 BPP), to the given stream. If you're sending the png through a
   * stream but not saving to a PDBFile, you can use this method. If you're
   * going to save it to a PDBFile, then you must use the saveTo method.
   * @throws ImageException
   * @throws IOException
  ...
  public void createPng(Stream s) throws ImageException, IOException
  {
    try {
      ...
      byte[] pngIdBytes = { (byte)-119, (byte)80, (byte)78, (byte)71,
  ...
  /**
   * Returns the scaled instance for this image. The algorithm used is the
   * replicate scale: not good quality, but fast.
   *
   * @since SuperWaba 3.5
   */
  public Image getScaledInstance(int newWidth, int newHeight)
    throws ImageException
  { // guich@350_22
    // Based on the ImageProcessor class on "KickAss Java Programming" (Tonny
    // Espeset)
    ...
    int fw =
      frameCount == 1 ? this.width : this.widthOfAllFrames; // guich@tc100b5_40
  ...
  /**
   * Returns the scaled instance using the area averaging algorithm for this
   * image. Example: <pre> Image img2 = img.getSmoothScaledInstance(200,200);
   * </pre>
   * In device and JavaSE it uses a Catmull-rom resampling, and in Blackberry it
   * uses an area-average resampling. The reason is that the Catmull-rom
   * consumes more memory and is also slower than the area-average, although the
   * final result is much better.
   * @since TotalCross 1.0
   */
  public Image getSmoothScaledInstance(int newWidth, int newHeight)
    throws ImageException
  { // guich@350_22
    // image preparation
    ...
    int maxContribs, maxContribsXY; // Almost-const: max number of contribution
                                    // for current sampling
    double scaledRadius, scaledRadiusY; // Almost-const: scaled radius for
                                        // downsampling operations
    double
      filterFactor; // Almost-const: filter factor for downsampling operations
    ...
    for (i = 0; i < newWidth; i++) {
      ...
      for (n = 0; n < height; n++) {
        ...
        for (j = 0; j < count; j++) {
          ...
          val = pixels[v_pixel[p_pixel++] +
                       n * width]; /* Using val as temporary storage */
        ...
        tb[i + n * newWidth] =
          (a << 24) | (r << 16) | (g << 8) | b; /* Temporary buffer */
      }
    }
    ...
    for (i = 0; i < newHeight; i++) {
      ...
      for (n = 0; n < newWidth; n++) {
        ...
        for (j = 0; j < count; j++) {
          int iweight = v_weight[p_weight++];
          val = tb[n + newWidth * v_pixel[p_pixel++]]; /* Using val as temporary
                                                          storage */
          /* Acting on color components */
          a += ((val >> 24) & 0xFF) * iweight;
          r += ((val >> 16) & 0xFF) * iweight;
  ...
  /**
   * Returns the scaled instance for this image, given the scale arguments. The
   * algorithm used is the replicate scale, not good quality, but fast. Given
   * values must be &gt; 0.
   * @since SuperWaba 4.1
   */
  public Image scaledBy(double scaleX, double scaleY) throws ImageException
  { // guich@402_6
    return ((scaleX == 1 && scaleY == 1) || scaleX <= 0 || scaleY <= 0)
      ? this
      : getScaledInstance(
          (int)(width * scaleX),
          (int)(height *
                scaleY)); // guich@400_23: now test if the width/height are
                          // the same, what returns the original image
  }

  /**
   * Returns the scaled instance for this image, given the scale arguments.
   * Given values must be &gt; 0. The backColor replaces the transparent pixel
   * of the current image to produce a smooth border. Example: <pre> Image img2
   * = img.smoothScaledBy(0.75,0.75, getBackColor());
   * </pre>
   * @since TotalCross 1.0
   */
  public Image smoothScaledBy(double scaleX, double scaleY)
    throws ImageException
  { // guich@402_6
    return ((scaleX == 1 && scaleY == 1) || scaleX <= 0 || scaleY <= 0)
      ? this
   ...
   * the scale arguments. Given values must be &gt; 0. This method is useful to
   * resize an image, specifying only one of its sides: the width or the height.
   * The other side is computed to keep the aspect ratio.
   *
   * @param newSize The new size (width or height) for the image
   * @param isHeight If true, the newSize is considered as the new height of the
   *     image. If false, the newSize is considered the new width of the image.
   *
   * Example: <pre>
   ...
   */
  public Image smoothScaledFixedAspectRatio(int newSize, boolean isHeight)
    throws ImageException
  { // guich@402_6
    int w = !isHeight ? newSize : (newSize * width / height);
    int h = isHeight ? newSize : (newSize * height / width);
   ...
   * @param fillColor the fill color; -1 indicates the transparent color of this
   *     image or
   * Color.WHITE if the transparentColor was not set; use 0 for a transparent
   * background, or 0xFF000000 for the BLACK color.
   */
    ...
    throws ImageException
  {
    ...
    for (int f = 0; f < frameCount; f++) {
      ...
      /* center */
      int x0 = ((wIn << 16) -
                (((xMax - xMin) * rawCosine) - ((yMax - yMin) * rawSine)) - 1) /
               2;
      int y0 = ((hIn << 16) -
                (((xMax - xMin) * rawSine) + ((yMax - yMin) * rawCosine)) - 1) /
               2;
      /* and draw! */
      ...
      if (frameCount != 1) {
        for (int y = imageOut.height - 1; y >= 0; y--) {
          Vm.arrayCopy(imageOut.pixels,
                       y * imageOut.width,
                       imageOut.pixelsOfAllFrames,
                       f * imageOut.width + y * imageOut.widthOfAllFrames,
                       imageOut.width);
        }
      }
    }
  ...
  /**
   * Creates a faded instance of this image, interpolating all pixels with the
  ...
  { // guich@tc110_50
    ...
    int[] to =
      (int[])(frameCount > 1 ? imageOut.pixelsOfAllFrames : imageOut.pixels);
    for (int i = from.length; --i >= 0;) {
      to[i] = (from[i] & 0xFF000000) |
              Color.interpolate(backColor,
                                from[i]); // keep the alpha channel unchanged
    }
    if (frameCount != 1) {
  ...
  /**
   * Creates a faded instance of this image, decreasing the alpha-channel by
   * 128 for all pixels.
   * @since TotalCross 2.0
   * @see #FADE_VALUE
   */
  ...
  /**
   * Creates a touched-up version of this Image with the specified brightness
   * and contrast. A new <code>Image</code> object is returned which will render
   * the image at the specified <code>brigthness</code>and the specified
   * <code>contrast</code>.
   *
   * @param brightness
   *           a number between -128 and 127 stating the desired level of
   * brightness.&nbsp; 127 is the highest brightness level (white image), -128
   * is no brightness (darkest image).
   * @param contrast
   *           a number between -128 and 127 stating the desired level of
   * contrast.&nbsp; 127 is the highest contrast level, -128 is no contrast.
   */
    ...
    switch (touchup) {
      ...
      case BRITE_TOUCHUP:
        for (int i = w * h - 1; i >= 0; i--) {
          ...
          int g = (p >> 8) & 0xFF;
          int b = p & 0xFF;
          out[i] = a | Color.getRGBEnsureRange(((m * r) + k) >> 16,
                                               ((m * g) + k) >> 16,
                                               ((m * b) + k) >> 16);
        }
        break;
      case CONTRAST_TOUCHUP:
        for (int i = w * h - 1; i >= 0; i--) {
          ...
          int b = p & 0xFF;
          out[i] =
            a | Color.getRGBEnsureRange(table[r], table[g], table[b] & 0xFF);
        }
        break;
  ...
  private void imageParse(byte[] fullBmpDescription, int length)
    throws ImageException
  {
  ...
  {
    // totalcross.JavaBridge.print("reading " + (doDither ? "and dithering " :
    // "") + "rgb " + bpp + "bpp");
    ...
    int pitch = ((width + div - 1) / div) *
                div; // make sure are in a 4 byte boundary - those extra pixels
                     // will be stripped off by the current clip
    int dif = pitch - width;
    // Start at the bottom of the pixel array and work up
    switch (bpp) {
      case 16: // guich@tc111_1
        pitch = (width * 2 + 3) &
                ~3; // guich@tc114_30: bmp with w=41 has 84 bytes per row
        for (dif = pitch - width * 2, y = height - 1, row = y * width; y >= 0;
             y--, offset += dif, row -= width + width) {
          for (x = width; x > 0; x--) {
      ...
      case 32: // guich@tc114_15
        for (y = height - 1, row = y * width; y >= 0;
             y--, row -= width + width) {
          for (x = width; x > 0; x--) {
            pix[row++] = 0xFF000000 | ((in[offset++] & 0xFF) /*<< 0*/) |
                         ((in[offset++] & 0xFF) << 8) |
                         ((in[offset++] & 0xFF) << 16) |
                         ((in[offset++] & 0xFF) << 24);
          }
        ...
        break;
      case 24:
        pitch = (width * 3 + 3) & ~3; // guich@tc110_107: must consider the
                                      // width in bytes, not in pixels
        for (dif = pitch - width * 3, y = height - 1, row = y * width; y >= 0;
             y--, offset += dif, row -= width + width) {
          for (x = width; x > 0; x--) {
            pix[row++] =
              0xFF000000 |
              (((in[offset++] & 0xFF) /*<< 0*/) | ((in[offset++] & 0xFF) << 8) |
               ((in[offset++] & 0xFF) << 16)); // guich@tc114:20: fixed order
          }
        }
        break;
        ...
        for (y = height - 1, row = y * width; y >= 0;
          ...
          for (x = 0; x < pitch; x++) {
            ...
            if (x < width) {
              pix[row++] =
                0xFF000000 |
                colorTable[(currByte >> bitShifts[whichBit]) & bitMask];
            }
            // If the current bit position is past the number of pixels in a
            // byte, advance to the next byte
  ...
  private void readRLE(int width,
                       int height,
                       byte[] in,
                       int offset,
                       boolean rle8)
  {
    ...
    while (true) {
      esc = in[offset++] & 0xFF;
      // encoded mode starts with a run length, and then a byte with two colour
      // indexes to alternate between for the run
      if (esc != 0) {
        if (rle8) {
          ...
          for (len = 1, r = y * width + x; len <= esc; len++, r++, x++) {
            if (x < width) {
              pix[r] =
                ((len & 1) == 1) ? colors0 : colors1; // odd count, low nybble
            }
          }
        }
      } else
      // A leading zero is an escape; it may signal the end of the bitmap, a
      // cursor move, or some absolute data.
      {
  ...
  private void ImageLoadBMPCompressed(byte[] p, int length)
    throws ImageException
  {
    ...
    if (this.width > 65535 || this.height > 65535 || this.width <= 0 ||
        this.height <= 0) {
    ...
    colorTable =
      bmpBPP >= 16 ? null : new int[1 << bmpBPP]; // guich@340_59: in some
                                                  // bitmaps, colorsUsed may be
    ...
    if (bmpBPP == 1 &&
        colorTable[0] ==
          0xFFFFFF) { // the bitmap is monochrome, are the colors iverted?
      colorTable[1] = 0xFFFFFF;
      colorTable[0] = 0x000000;
    }
    /*
     * if (storePalette) { GRHANDLE_PaletteSize(imgHandle) = 256;
     * GRHANDLE_Palette(imgHandle) = colorTable; }
     */

    // Create space for the pixels
    this.pixels = new int[this.height * this.width];

    // Read the pixels from the stream based on the compression type directly
    // into the selected offscreen image
    if (compression == BI_RGB) {
      readRGB(this.width, this.height, bmpBPP, p, bitmapOffset);
    } else if (bmpBPP == 16) {
      throw new ImageException(
        "16-bpp BMP compressed RLE images is not supported! Use 24-bpp instead.");
    } else {
      readRLE(this.width, this.height, p, bitmapOffset, compression == BI_RLE8);
    }
    setTransparentColor(
      Color.WHITE); // every bmp image has white as default transparent color
  }

  /**
   * Fill the required fields of the GifImage <code>image</code> from reading
   * the stream <code>input</code>
   *
   * @param image
   ...
   * @param input
   *           totalcross.io.Stream that contains the GIF encoded bytes of the
   * image
   * @param buffer
   *           array of bytes used as a working buffer - its size should be
   * between 512 and 2048 bytes
   * @param imageNo
   *           position of the image in a multi-image file must start (and
   * default to) zero.
   */
  ...
  static class ImageLoader
  {
    ...
    private int[] convertBufferedImageToPixels(BufferedImage image,
                                               int[] pixelsOut,
                                               final int width,
                                               final int height)
    {
      ...
      if (transparentColor == -3) { // guich@tc130: not already changed?
        if ((colorModel instanceof java.awt.image.IndexColorModel) &&
            (-1 != (index = ((java.awt.image.IndexColorModel)colorModel)
                              .getTransparentPixel()))) {
    ...
    public Image load(byte[] input, int len) throws ImageException
    {
      ...
      try {
        final ImageInputStream stream = ImageIO.createImageInputStream(
          new ByteArrayInputStream(input, 0, len));
        final ImageReader reader = ImageIO.getImageReaders(stream).next();
        reader.setInput(stream);

        if (new String(input, 0, 3).equals("GIF")) {
          /*
           * The width and height of each frame may differ, so we must first get
           * the dimensions of the entire file.
           */
          ...
          try {
            ...
            for (int index = 0; /* index < count */; index++) {
              ...
              image.pixels = convertBufferedImageToPixels(
                frame, image.pixels, width, height);
              frames.add(image);
            }
          } catch (IndexOutOfBoundsException e) {
            /*
             * Getting the number of frames may require a full scan of the file.
             * It's better just to keep reading and stop at the exception.
             */
          }
          ...
          image.pixels =
            convertBufferedImageToPixels(frame, image.pixels, width, height);
        }
    ...
    private void fillPNGInformations(byte[] input, Image img)
      throws ImageException
    {
        ...
        while (true) {
          ...
          if (id.equals("IHDR")) {
            ...
            switch (len) {
              case 6: // RGB
                transparentColor =
                  Color.getRGBEnsureRange(ds.readUnsignedShort(),
              ...
              case 256: // palettized? find the color that is transparent (0)
                ...
                for (int i = 0, pos = bas.getPos(); i < 256; i++, pos++) {
                  if (input[pos] == 0) {
                    if (plteLen == 768) {
                      transparentColor =
                        Color.getRGB(input[pltePos + i * 3] & 0xFF,
                                     input[pltePos + i * 3 + 1] & 0xFF,
            ...
            if (type.equals("Comment")) {
              ...
              bas.skipBytes(
                -type.length() -
                1); // guich@tc100b5_31: go back if its not our comment
            }
          }
    ...
    private Image joinImages(List<Image> images)
    {
      ...
      if (n > 1) {
        try {
          ...
          resultImage =
            images.get(0); // if an error occurs, we assume only the first frame
        }
      }
      return resultImage;
    }
  ...
  /**
   * Returns true if the given filename is a supported image: Png or Jpeg. Gif
   * and Bmp are supported on JavaSE only.
   * @since TotalCross 1.0
   */
   ...
   * @since TotalCross 1.12
   */
  @ReplacedByNativeOnDeploy final public void applyColor(int color)
  { // guich@tc112_24
   ...
   * MAKE the image). The target size is computed as
   * <code>image_size*min(screen_size)/original_resolution</code>
   * @param originalRes The original resolution that the image was developed
   *     for. Its a good idea to create images for 320x320 and then scale them
   *     down.
   * @since TotalCross 1.12
   */
  final public Image smoothScaledFromResolution(int originalRes)
    throws ImageException
  { // guich@tc112_23
    int k = Math.min(Settings.screenWidth, Settings.screenHeight);
    return getSmoothScaledInstance(width * k / originalRes,
                                   height * k / originalRes);
  }

  /**
   * Returns true if the given Image object has the same size and RGB pixels of
   * this one. The alpha-channel is ignored.
  ...
  @ReplacedByNativeOnDeploy private boolean nativeEquals(Image other)
  {
    Image img = other;
   ...
   *
   * If the color's alpha is 0xAA, the image's alpha will also be changed. This
   * is used by the Spinner.
   *
   * @param color The color to be applied
   * @since TotalCross 1.3
   */
  @ReplacedByNativeOnDeploy final public void applyColor2(int color)
  {
    int r2 = Color.getRed(color);
    int g2 = Color.getGreen(color);
    int b2 = Color.getBlue(color);
    ...
    // the given color argument will be equivalent to the brighter color of this
    // image. Here we search for that color
    int hi = 0, hip = 0;
    for (int n = 0; n < pixels.length; n++) {
      // for (int n = pixels.length; --n >= 0;) {
      if (((p = pixels[n]) & 0xFF000000) ==
          0xFF000000) { // consider only opaque pixels
        p &= 0x00FFFFFF;
  ...
  public Image getSmoothScaledInstance(int newWidth,
                                       int newHeight,
                                       int backColor) throws ImageException
  { // guich@350_22
  ...
  /** @deprecated TotalCross 2 no longer uses the backColor parameter. */
  @Deprecated
  public Image smoothScaledBy(double scaleX, double scaleY, int backColor)
    throws ImageException
  { // guich@402_6
    return smoothScaledBy(scaleX, scaleY);
  }
  ...
  /** @deprecated TotalCross 2 no longer uses the backColor parameter. */
  @Deprecated
  final public Image smoothScaledFromResolution(int originalRes, int backColor)
    throws ImageException
  { // guich@tc112_23
  ...
  /**
   * Applies the given fade value to r,g,b of this image while preserving the
   * alpha value.
   */
  @ReplacedByNativeOnDeploy public void applyFade(int fadeValue)
  {
  ...
  /**
   * Utility method used to change the frame count of an image. This method
   * exists in java only, not on device. The frame count of a TotalCross' Image
   * is stored in a comment inside the PNG file. If you create a PNG with many
   * frames and don't want to keep calling the setFrameCount manually, you can
   * call this method like: <pre>
   * Image.writeFrameCount("c:/project/src/images/people.png",2);
   * </pre>
   * Be careful that this must be done once only; this method does not exist in
   * the device and will abort the vm if you try to call it there!
    ...
    try {
      ...
      if (img.getFrameCount() == count) {
        throw new RuntimeException(
          "The image " + filePath + " already has " + count +
  ...
  /**
   * Gets a copy of this image; if the image is multi-framed, returns a copy of
   * the first frame.
   * @since TotalCross 3.1
   */
  public Image getCopy() throws ImageException { return getFrameInstance(0); }

  /**
   * Returns a clipped image from the current position. Note that you must
   * ensure that the values are correct or an exception will be thrown
   */
  public Image getClippedInstance(int x, int y, int w, int h)
    throws ImageException
  {
  ...
  public static void resizeJpeg(String inputPath,
                                String outputPath,
                                int maxPixelSize)
  {
    try {
  ...
  public static Image getJpegBestFit(String path,
                                     int targetWidth,
                                     int targetHeight)
  ...
  public static Image getJpegScaled(String path,
                                    int scaleNumerator,
