...
public class Image extends GfxSurface {
	...
	public Image(int width, int height) throws ImageException {
		...
		try {
			...
			throw new ImageException("Out of memory: cannot allocate " + width + "x" + height +
									 " offscreen image.");
		}
		init();
	}
	...
	public Image setTransparentColor(int color) {
		int[] pixels = (int[])((frameCount == 1) ? this.pixels :
							   this.pixelsOfAllFrames);  // guich@tc100b5_40
		for (int i = pixels.length; --i >= 0;) {
			int p = pixels[i] & 0xFFFFFF;
			pixels[i] = (p == color) ? color : p |
						0xFF000000; // if is the transparent color, set the alpha to 0, otherwise, set to full bright
		}
		return this;
	}
	...
	public void saveTo(PDBFile cat, String name) throws ImageException, IOException {
		...
		ResizeRecord rs = new ResizeRecord(cat, Math.min(65500,
										   width * height * 3 + 200)); // guich@tc114_17: make sure is not bigger than 64k
	...
	private static int findPosition(PDBFile cat, String name, boolean isWrite) throws IOException {
		...
		int n = cat.getRecordCount();
		for (int i = 0; i < n;
			 i++) { // find the correct position to insert the record. the records must be sorted
			cat.setRecordPos(i);
	...
	public void createJpg(Stream s, int quality) throws ImageException, IOException {
		try {
			java.awt.image.MemoryImageSource screenMis = new java.awt.image.MemoryImageSource(width, height,
					new java.awt.image.DirectColorModel(32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0), (int[]) pixels, 0,
					width);
	...
	public Image getScaledInstance(int newWidth, int newHeight) throws ImageException { // guich@350_22
		...
		int[] srcImageData = (int[])((frameCount == 1) ? this.pixels :
									 this.pixelsOfAllFrames);  // guich@tc100b5_40
	...
	public Image getSmoothScaledInstance(int newWidth,
										 int newHeight) throws ImageException { // guich@350_22
		// image preparation
	...
	public Image scaledBy(double scaleX, double scaleY) throws ImageException { // guich@402_6
		return ((scaleX == 1 && scaleY == 1) || scaleX <= 0 || scaleY <= 0) ? this
			   : getScaledInstance((int)(width * scaleX),
								   (int)(height *
										 scaleY));   // guich@400_23: now test if the width/height are the same, what returns the original image
	}
	...
	public Image smoothScaledBy(double scaleX, double scaleY) throws ImageException { // guich@402_6
		return ((scaleX == 1 && scaleY == 1) || scaleX <= 0 || scaleY <= 0) ? this
			   : getSmoothScaledInstance((int)(width * scaleX),
										 (int)(height *
											   scaleY));   // guich@400_23: now test if the width/height are the same, what returns the original image
	}
	...
	public Image smoothScaledFixedAspectRatio(int newSize,
			boolean isHeight) throws ImageException { // guich@402_6
		int w = !isHeight ? newSize : (newSize * width / height);
		int h = isHeight ? newSize : (newSize * height / width);
	...
	public Image getFadedInstance(int backColor) throws ImageException { // guich@tc110_50
		...
		for (int i = from.length; --i >= 0;) {
			to[i] = (from[i] & 0xFF000000) | Color.interpolate(backColor,
	...
	private void readRGB(int width, int height, int bpp, byte[] in, int offset) {
		...
		int pitch = ((width + div - 1) / div) *
					div; // make sure are in a 4 byte boundary - those extra pixels will be stripped off by the current clip
		int dif = pitch - width;
	...
	private void ImageLoadBMPCompressed(byte[] p, int length) throws ImageException {
		...
		colorTable = bmpBPP >= 16 ? null : new int[1 <<
					 bmpBPP]; // guich@340_59: in some bitmaps, colorsUsed may be
	...
	static class ImageLoader {
		...
		public Image load(byte[] input, int len) throws ImageException {
			...
			try {
				final ImageInputStream stream = ImageIO.createImageInputStream(new ByteArrayInputStream(input, 0,
												len));
	 ...
	 * @since TotalCross 1.12
	 */
	final public Image smoothScaledFromResolution(int originalRes) throws
		ImageException { // guich@tc112_23
	...
	public Image getSmoothScaledInstance(int newWidth, int newHeight,
										 int backColor) throws ImageException { // guich@350_22
	...
	public Image smoothScaledBy(double scaleX, double scaleY,
								int backColor) throws ImageException { // guich@402_6
		return smoothScaledBy(scaleX, scaleY);
	}
	...
	final public Image smoothScaledFromResolution(int originalRes,
			int backColor) throws ImageException { // guich@tc112_23
	...
	throws java.io.IOException, ImageException {
		...
		if (path != null) {
			...
			if (b != null) {
				...
				try
					(File f = new File(path, File.READ_ONLY)) {
