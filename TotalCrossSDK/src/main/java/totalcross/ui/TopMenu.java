...
public class TopMenu extends Window
{
  ...
  public static class Item extends Container
  {
    ...
    private TopMenu getTopMenu() { return (TopMenu)getParentWindow(); }
    ...
    @Override public void initUI()
    {
      ...
      if (icon == null) {
        ...
        if (icon instanceof Control) {
          ...
          try {
            c =
              new ImageControl(((Image)icon).getHwScaledInstance(itemH, itemH));
            ((ImageControl)c).centerImage = true;
          } catch (ImageException e) {
          }
        }
        add(c == null ? (Control) new Spacer(itemH, itemH) : c,
            LEFT,
            TOP,
            PARENTSIZE + percIcon,
            FILL);
      ...
      add(new Container() {
        @Override public void initUI()
        {
           ...
           */
          this.setInsets((int)((Settings.screenWidth < 320 ? 64 : 72) *
    ...
    @Override public void onEvent(Event e)
    {
      if (e.type == PenEvent.PEN_UP && !hadParentScrolled()) {
        ...
        if (highlight) {
          ...
          int highlight = Color.getBrightness(original) < 200
                            ? Color.brighter(original)
                            : Color.darker(original);
  ...
  {
    int ww = setW = widthInPixels != 0
                      ? widthInPixels
                      : SCREENSIZE + (percWidth > 0 ? percWidth : 50);
    switch (animDir) {
      case LEFT:
      ...
      default:
        ...
        setRect(LEFT,
                animDir,
                ww,
                (int)Math.min(Settings.screenHeight * 3 / 4,
                              Settings.screenDensity * items.length * 50),
                null,
                screenResized);
        break;
    }
  ...
  @Override public void initUI()
  {
    ...
    if (showElevation && animDir == LEFT) {
      ...
      try {
        ImageControl ic = new ImageControl(
          new Image("totalcross/res/mat/drawer_shadow.9-hdpi.png"));
    ...
    add(sc,
        LEFT + scInsets.left,
        TOP + scInsets.top,
        (showElevation && animDir == LEFT ? FIT : FILL) - scInsets.right,
        FILL);
    ...
    if (header != null) {
      ...
      sc.add(header,
             ...
             TOP,
             FILL,
             widthInPixels > 0 ? (widthInPixels * 9 / 16)
                               : (PARENTSIZE + (percWidth * 9 / 16)));
    }
    for (int i = 0; i < n; i++) {
      Control tmi = items[i];
      tmi.appId = i + 1;
      sc.add(tmi,
             ...
             AFTER,
             FILL,
             tmi instanceof TopMenu.Item ? DP + 48 : PREFERRED);
      if (i == n - 1) {
        break;
      }
      if (drawSeparators) {
  ...
  @Override public void onEvent(Event e)
  {
    switch (e.type) {
      case ControlEvent.PRESSED:
        if (autoClose && e.target != this &&
            ((Control)e.target).isChildOf(this) &&
   ...
   */
  @Override public void unpop() { unpop(null); }
  ...
  @Override public void onPopup()
  {
    ...
    if (animDir == CENTER) {
      ...
      if (fadeOnPopAndUnpop) {
        currentAnimation.with(
          FadeAnimation.create(this, true, null, totalTime));
      }
    }
    currentAnimation.setAnimationFinishedAction(new AnimationFinished() {
      @Override public void onAnimationFinished(ControlAnimation anim)
  ...
  @Override protected void postPopup() { super.postPopup(); }

  public int getSelectedIndex() { return selected; }

  /** Returns if this control fades in/out on pop/unpop */
  public boolean isFadeOnPopAndUnpop() { return fadeOnPopAndUnpop; }
