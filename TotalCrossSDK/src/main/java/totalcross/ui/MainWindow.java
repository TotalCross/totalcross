...
public class MainWindow extends Window implements totalcross.MainClass {
	...
	public MainWindow(String title, byte style) { // guich@112
		...
		setBackColor(UIColors.controlsBack =
						 Color.WHITE); // guich@200b4_39 - guich@tc100: set the controlsBack to this color
	 ...
	 */
	@Override
	final public void appStarting(int
								  timeAvail) { // guich@200b4_107 - guich@tc126_46: added timeAvail parameter to show MessageBox from inside here.
	...
	@Override
	final public
	void appEnding() { // guich@200final_11: fixed when switching apps not calling killThreads.
	...
	final public void _onTimerTick(boolean canUpdate) {
		if (startTimer != null) { // guich@567_17
			...
			startTimer =
				null; // removeTimer calls again onTimerTick, so we have to null out this before calling it
			removeTimer(t);
	...
	void setTimerInterval(int n) {
		totalcross.Launcher.instance.setTimerInterval(n < Settings.minimalUpdateInterval ? n :
				Settings.minimalUpdateInterval);
	}
	...
	public void setRect(int x, int y, int width, int height, Control relative,
						boolean screenChanged) { // guich@567_19
