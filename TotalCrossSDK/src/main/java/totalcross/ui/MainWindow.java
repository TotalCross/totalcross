 ...
 * <p>
 * All TotalCross programs with an user-interface must have <b>one and only
 * one</b> main window. <p> Here is an example showing a basic application:
 *
 * <pre>
...
public class MainWindow extends Window implements totalcross.MainClass
{
  ...
  public MainWindow() { this(null, NO_BORDER); }
  ...
  { // guich@112
    ...
    setBackColor(UIColors.controlsBack =
                   Color.WHITE); // guich@200b4_39 - guich@tc100: set the
                                 // controlsBack to this color
    ...
    FirebaseManager.getInstance().registerFirebaseInstanceIdService(
      initFirebaseInstanceIdService());
    FirebaseManager.getInstance().setMessagingService(
  ...
  /**
   * Sets the default font used in all controls created. To change the default
   * font, assign it to this member in the MainWindow constructor, making it the
   * FIRST LINE in the constructor; you'll not be able to use
   * super(title,border): change by setBorderStyle and setTitle, after the
   * defaultFont assignment. Example: <pre> public MyApp()
   * {
   ...
   * This method must be called in the MainWindow's constructor, and only once.
   * E.g.: <pre> public class Foo extends MainWindow
   * {
   *    public Foo()
   *    {
   ...
   * If you don't like such behaviour in non finger devices, set this property
   * to false after calling setUIStyle.
   *
  ...
  public void setUIStyle(byte style)
  {
    ...
    if (uiAndroid) {
      androidBorderThickness =
        Settings.screenWidth <= 320 ? 1 : Settings.screenWidth <= 640 ? 2 : 3;
   ...
   * Note 2: On Android, you can exit softly by using SOFT_EXIT as the exit
   * code. <p>If you want your code to be called when the VM exits, extend the
   * onExit method.
   * @see #onExit
   */
  @ReplacedByNativeOnDeploy public static final void exit(int exitCode)
  {
   ...
   * to the background. Whenever the application is minimized, the following
   * call back function will be called: {@link #onMinimize()}. Note: On Android,
   * calling {@link #minimize()} will pause the application execution and it can
   * only be restored manually by the user. This method is also supported on
   * Windows 32.
   * @see #onMinimize
   * @see #onRestore
   * @since TotalCross 1.10
   */
  @ReplacedByNativeOnDeploy public static void minimize()
  { // bruno@tc110_89
   ...
   * to the foreground.
   * Whenever the application is restored, the following call back function will
   * be called:
   * {@link #onRestore()}. Note: This method is supported on Android but the
   * user must restore the application manually. This method is also supported
   * on Windows 32.
   * @since TotalCross 1.10
   */
  @ReplacedByNativeOnDeploy public static void restore()
  { // bruno@tc110_89
    totalcross.Launcher.instance.restore();
  }

  /**
   * Returns the instance of the current main window. You can use it to get
   * access to methods of the <code>MainWindow</code> class from outside the
   * class. It is also possible to cast the returned class to the class that is
   * extending <code>MainWindow</code> (this is a normal Java behavior). So, if
   * UiGadgets is running, it is correct to do: <pre> UIGadgets instance =
   * (UIGadgets)MainWindow.getMainWindow();
   * </pre>
   */
  public static MainWindow getMainWindow() { return mainWindowInstance; }
   ...
   * the Control class. The Timer event will be issued to the target every
   * millis milliseconds.
   */
  ...
  /**
   * Adds the timer t to the target control. This method is protected, the
   * public method to add a timer to a control is the addTimer() method in the
   * Control class. The Timer event will be issued to the target every millis
   * milliseconds.
   */
                          ...
                          boolean append)
  {
    ...
    setTimerInterval(
      1); // forces a call to _onTimerTick inside the TC Event Thread
  }

  /**
   * Removes the given timer from the timers queue. This method returns true if
   * the timer was found and removed and false if the given timer could not be
   * found. The <code>target</code> member is set to null.
   */
  @Override public boolean removeTimer(TimerEvent timer)
  {
    if (timer == null) {
      return false;
    }
   ...
   */
  @Override final public void appStarting(int timeAvail)
  { // guich@200b4_107 - guich@tc126_46: added timeAvail
    // parameter to show MessageBox from inside here.
   ...
   * to prevent the compiler from removing it during optimization.
   */
  @Override final public void appEnding()
  { // guich@200final_11: fixed when switching
    // apps not calling killThreads.
    if (!Settings.onJavaSE) {
      try {
        new File(Convert.appendPath(Settings.appPath, "appCr4shed"),
                 File.READ_WRITE)
          .delete();
      } catch (Throwable t) {
      } // finished fine
    ...
    quittingApp = true;
    // guich@tc100: do this at device side - if (resetSipToBottom)
    // setStatePosition(0, Window.VK_BOTTOM); // fixes a problem of the window
    // of the sip not correctly being returned to the bottom

    // guich@tc126_46: don't call app's onExit if time expired, since initUI was
    // not called.
    if (initUICalled) {
   ...
   * You should return from this method as soon as possible, because the OS can
   * kill the application if it takes too much to return.
   *
   ...
   * corrupted (actually, no data is lost, but a TableNotClosedException will be
   * issued). So, its a good thing to call LitebaseConnection.closeAll in your
   * litebase instances and recover them in the onRestore method. <br><br> When
   * the onMinimize is called, the screen will only be able to be updated after
   * it resumes (in other words, calling repaint or repaintNow from the
   * onMinimize method has no effect).
   *
   * On Windows Phone, the onMinimize is called and, if the user don't call the
   * application again within 10 seconds, the application is KILLED without
   * notifications. So, you should save all your application's state in this
   * method and restore it in the onRestore method.
   * @see #minimize()
   * @since TotalCross 1.10
   */
  public void onMinimize()
  { // bruno@tc110_89 - bruno@tc122_31: now supported
    // on wince and win32
  }
  ...
  { // bruno@tc110_89 - bruno@tc122_31: now supported on
    // wince and win32
  }
  ...
  private void startProgram()
  {
    ...
    started = true; // guich@567_17: moved this from appStarting to here to let
                    // popup check if the screen was already painted
   ...
   * to prevent the compiler from removing it during optimization.
   */
  @Override final public void _onTimerTick(boolean canUpdate)
  {
    if (startTimer != null) { // guich@567_17
      TimerEvent t = startTimer;
      startTimer = null; // removeTimer calls again onTimerTick, so we have to
                         // null out this before calling it
      removeTimer(t);
      if (timeAvailable >= 0 && Settings.platform.equals(Settings.WIN32) &&
          (Settings.romSerialNumber == null ||
           Settings.romSerialNumber.length() == 0)) {
        new MessageBox(
          "Fatal Error",
    ...
    if (elapsedMilliseconds >= Settings.minimalUpdateInterval) {
      // Reverse-for allows for the safe removal of updateListeners during the
      // update event
    ...
    if (Window.needsPaint) {
      ...
      // guich@tc100: make sure that any pending screen update is committed. -
      // if not called from addTimer/removeTimer (otherwise, an open combobox
      // will flicker)
      safeUpdateScreen();
    }
  }

  @ReplacedByNativeOnDeploy void setTimerInterval(int n)
  {
    totalcross.Launcher.instance.setTimerInterval(
      n < Settings.minimalUpdateInterval ? n : Settings.minimalUpdateInterval);
  }

  /**
   * Returns the command line passed by the application that called this
   * application in the Vm.exec method.
   *
   * In Android, you can start an application using adb:
   * <pre>
   * adb shell am start -a android.intent.action.MAIN -n
   * totalcross.app.uigadgets/.UIGadgets -e cmdline "Hello world"
   * </pre>
   * In the sample above, we're starting UIGadgets. Your app should be:
   * totalcross.app.yourMainWindowClass/.yourMainWindowClass
   *
   * Note: When you click on the application's icon, there's no command line.
   */
  @ReplacedByNativeOnDeploy final public static String getCommandLine()
  { // guich@tc120_8: now is static
  ...
  /**
   * Takes a screen shot of the current screen. Since TotalCross 3.06, it uses
   * Control.takeScreenShot. Here's a sample: <pre> Image img =
   * MainWindow.getScreenShot(); File f = new File(Settings.appPath +
   * "/screen.png", File.CREATE_EMPTY); img.createPng(f); f.close();
   * </pre>
   * Note that the font varies from device to device and even to desktop. So, if
   * you want to compare a device's screen shot with one taken at desktop, be
   * sure to set the default font in both to the same, like using
   ...
   * Note that this
   * */
  public void runOnMainThread(Runnable r) { runOnMainThread(r, true); }

  /**
   Runs the given code in the main thread. As of TotalCross 2.0, a thread
   cannot update the screen.
   * So, asking the code to be called in the main thread solves the problem. Of
   course, this call is asynchronous, ie,
   * the thread may run again before the screen is updated. This method is
   thread-safe.
   * If singleInstance is true and the array contains an element of this class,
   it is replaced by the given one.
