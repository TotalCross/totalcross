...
public class Toast
{
  ...
  /**Gap between the text and the button, vertically. Used when the button text
   * is too big.*/
  public static int buttonVGap = UnitsConverter.toPixels(18 + Control.DP);
  private static final int MAX_HEIGHT =
    UnitsConverter.toPixels(344 + Control.DP);
  ...
  /**
   * Shows a toast message with a button using the given parameters on the
   * given window.
   */
  ...
  /**
   * Shows a toast message with a button using the given parameters on the main
   * window.
   ...
   * Toast.show("\n"+message+"\n", 2000);
   * </pre>
   * See the public static fields of this class to show how you can customize
   * the appearance. Calling with a nulled message will make the last toast
   * disappear.
   *
   * The text is splitted if wider than the screen.
   *
   * If delay is INFINITE, it will wait forever until you call stopShow().
   * If delay is INFINITE_NOANIM, it will wait forever until you call stopShow()
   * and will not use animation.
   */
  public static void show(final String message,
                          final int delay,
                          final Window parentWindow)
      ...
      MainWindow.getMainWindow().runOnMainThread(new Runnable() {
        @Override public void run()
        {
          ...
          if (message != null) {
            ...
            cnt.setRect(posX,
                        posY,
                        Math.min(Settings.screenWidth - 5, MAX_HEIGHT),
                        48 + Control.DP);
            ...
            if (hasButton) {
              ...
              if (bigButton) {
                paddingTop = paddingLeft =
                  UnitsConverter.toPixels(16 + Control.DP);
                paddingRight = paddingBottom =
                  UnitsConverter.toPixels(8 + Control.DP);
                textWidth = cnt.getWidth() - paddingLeft - paddingRight;
              } else {
                paddingTop = paddingBottom =
                  UnitsConverter.toPixels(6 + Control.DP);
                paddingRight = UnitsConverter.toPixels(8 + Control.DP);
                textWidth = cnt.getWidth() - paddingLeft - paddingRight -
                            actionWidth - buttonHGap;
              }
              ...
              if (twoLines && !bigButton) {
                ...
                paddingRight = UnitsConverter.toPixels(8 + Control.DP);
                cnt.setRect(posX,
                            posY - (int)(Font.getDefaultFontSize() * 2 *
              ...
              cnt.add(labels[0],
                      paddingLeft,
                      twoLines || bigButton ? paddingTop : Control.CENTER,
                      textWidth,
                      cnt.fmH);
              if (twoLines) {
                cnt.add(labels[1],
                        paddingLeft,
                        Control.AFTER + cnt.fmH / 2,
                        textWidth,
                        cnt.fmH,
                        labels[0]);
              }
              ...
              if (bigButton)
                cnt.add(action,
                        Control.RIGHT - paddingRight,
                        Control.AFTER + buttonVGap,
                        twoLines ? labels[1] : labels[0]);
              else {
                cnt.add(action,
                        Control.RIGHT - paddingRight,
                        Control.CENTER,
              ...
              cnt.add(padding,
                      ...
                      1,
                      paddingBottom,
                      bigButton ? action : twoLines ? labels[1] : labels[0]);
              cnt.resizeHeight();
              parent.reposition();
            } else {
              ...
              cnt.add(labels[0],
                      paddingLeft,
                      twoLines ? paddingTop : Control.CENTER,
                      textWidth,
                      cnt.fmH);
              if (twoLines) {
                cnt.add(labels[1],
            ...
            if (delay != INFINITE_NOANIM) {
              ...
              animation
                .then(FadeAnimation.create(
                  cnt,
                  false,
                  new ControlAnimation.AnimationFinished() {
                    @Override
                    public void onAnimationFinished(ControlAnimation anim)
                    {
                      ...
                      if (thisCnt == cnt) { // button may change if user
                                            // tries to show several Toasts
                        cnt = null;
                        action = null;
                        animation = null;
                        canShow = true;
  ...
  public static void stopShow() { stopShow(true); }
  ...
  private static void addActionButton(String text,
                                      int buttonForeColor,
