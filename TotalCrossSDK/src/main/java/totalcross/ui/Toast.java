...
public class Toast {
	...
	public static void show(final String message, final int delay, final Window parentWindow,
							...
							final int buttonForeColor, PressListener btnPressListener) {
	 ...
	 */
	public static void show(final String message, final int delay, final String buttonCaption,
				...
				public void run() {
					...
					if (message != null) {
						...
						if (hasButton) {
							...
							cnt.add(labels[0], paddingLeft, twoLines
									|| bigButton ? paddingTop : Control.CENTER, textWidth, cnt.fmH);
							if (twoLines) {
								cnt.add(labels[1], paddingLeft, Control.AFTER + cnt.fmH / 2, textWidth, cnt.fmH, labels[0]);
							}

							if (bigButton)
								cnt.add(action, Control.RIGHT - paddingRight, Control.AFTER + buttonVGap,
