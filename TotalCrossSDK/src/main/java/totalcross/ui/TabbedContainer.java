...
public class TabbedContainer extends ClippedContainer implements Scrollable, AnimationFinished {
			...
			if (flick == null) {
				...
				if (!firstTabChange && animationTime > 0 && animate) {
					try {
						...
						PathAnimation p = PathAnimation.create(this, 0, 0, dif * width - correction, 0, this,
															   animationTime);
	...
	public int getPreferredWidth() {
		...
		if (count > 0) {
			...
			sum += maxw -
				   rNotSel[maxi].width; // add the diff between the bold and the plain fonts of the biggest title
		}
	...
	private boolean mustScroll() {
		if (!allSameWidth) {
			return count > 1
				   && getPreferredWidth() >
				   this.width; // guich@564_10: support scroll - guich@573_2: only add arrows if there's more than one tab
		}
	...
	private void addArrows() {
		...
		if (scroll && showArrows) {
			...
			if (btnLeft == null) {
				int hh = Settings.fingerTouch ? fmH * 3 / 4 : Math.max(fmH / 2,
						 tabH / 4); // guich@tc110_90: use tab height if its larger than font's height
	...
	public void onFontChanged() { // guich@564_11
		...
		for (int i = count; --i >= 0;) {
			...
			wbolds[i] = isText
						&& !uiAndroid ? fmb.stringWidth(strCaptions[i]) : wplains[i]; // in uiandroid there's no bold font
		}
	...
	protected void onColorsChanged(boolean colorsChanged) {
		if (uiAndroid && parent != null
			&& backColor == parent.backColor) { // same background color in uiandroid does not look good.
		...
		if (colorsChanged && btnLeft != null) {
			...
			btnRight.backColor = btnLeft.backColor = parent != null ? parent.backColor :
	...
	public void onPaint(Graphics g) {
		...
		int containerColor =
			containers[activeIndex].backColor; // guich@580_26: use current container's backcolor instead of TabbedContainer's backcolor
		...
		if (tabsType != TABS_NONE) {
			...
			if (!transparentBackground && (uiAndroid || useOnTabTheContainerColor || tabsBackColor != null
				...
				for (int i = 0; i < n; i++) {
					...
					g.backColor = back = getTabColor(
											 i); // guich@580_7: use the container's color if containersColor was not set - guich@tc110_59: use default back color if container was not yet shown.
			...
			for (int i = 0; i < n; i++) {
				...
				if (isText) {
					...
					if (imgCaptions != null && imgCaptions[i] != null) {
						g.drawImage(i == activeIndex
									&& activeIcon != null ? activeIcon : disabled[i] ? imgDis[i] : imgCaptions[i],
			...
			if (Settings.keyboardFocusTraversable
				&& focusMode ==
				FOCUSMODE_CHANGING_TABS) { // draw the focus around the current tab - guich@580_52: draw the cursor only when changing tabs
	...
	public int getTabColor(int tab) {
		return tabsBackColor != null && tabsBackColor[tab] != -1 ? tabsBackColor[tab]
			   ...
			   && containers[tab].backColor != -1 ? containers[tab].backColor : backColor;
	}
	...
	public void onEvent(Event event) {
		...
		switch (event.type) {
			...
			case PenEvent.PEN_DOWN:
				...
				if (pe.x < btnX && (flick != null || (rects[0].y <= pe.y
													  && pe.y <=
													  rects[0].y2()))) { // guich@tc100b4_7 - guich@tc120_48: when fingerTouch, the y position may be below the tabbed container
					int sel = -1;
					if (flick != null) { // guich@tc120_48
						int minDist = Settings.touchTolerance;
						for (int i = count - 1; i >= 0; i--) {
							Rect r = rects[i];
							int d = (int)(Convert.getDistancePoint2Rect(pe.x, pe.y, r.x, r.y, r.x + r.width,
										  r.y + r.height) + 0.5);
							if (d <= minDist) {
								minDist = d;
								sel = i;
	...
	public Control handleGeographicalFocusChangeKeys(KeyEvent ke) {
		...
		if (MainWindow.mainWindowInstance._focus == this) {
			...
			if ((ke.isNextKey() && activeIndex == containers.length - 1) || (ke.isPrevKey()
					&& activeIndex == 0)) {
				return null;
			}
			ke.target = this;
	...
	public boolean canScrollContent(int direction, Object target) { // called when
		return getPositionedTab(true) == -1
			   || (direction == DragEvent.LEFT && activeIndex > 0 && (flickIntoDisabledTabs
