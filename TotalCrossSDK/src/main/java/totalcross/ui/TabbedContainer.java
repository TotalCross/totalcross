 ...
 * different widths. <br> A scroll is automatically added when the total width
 * of the titles is bigger than the control's width. <br> The containers are
 * created automatically and switched when the user press the corresponding tab.
 * <p>
 ...
 *       tab.setContainer(0,new Edition()); // replace container 1 by a class
 * that extends Container. tab.getContainer(1).add(new Label("Not
 * implemented"),CENTER,CENTER);
 *    }
 *
 ...
 * }
 * </pre>
 * Here's another sample that will show two TabbedContainers, one with images
 * and another one with scrolling tabs. Note that you must create img1.png and
 * img2.png. <pre> TabbedContainer tp1 = new TabbedContainer(new Image[]{new
 * Image("img1.png"), new Image("img2.png")}, null); add(tp1);
 * tp1.setRect(LEFT,TOP,Settings.screenWidth/2,Settings.screenHeight/2);
 * tp1.activeTabBackColor = Color.getRGB(222,222,222);
 *
 * TabbedContainer tp2 = new TabbedContainer(new
 * String[]{"verinha","marcelo","denise","guilherme","renato","michelle","rafael","barbara","lucas","ronaldo","nenem",});
 * add(tp2);
 * tp2.setRect(LEFT,AFTER+2,FILL,FILL);
 * </pre>
 *
 * When the user interface is Android, the tabs do not look good if the
 * background is the same of the parent's. In this case, we force the background
 * to be slighly darker. There are a few fields that you can use to change the
 * color, like activeTabBackColor, useOnTabTheContainerColor and pressedColor.
 *
 * Important: starting in TotalCross 1.3, with Settings.fingerTouch=true, you
 * CANNOT call setRect in your container. Otherwise, the flick and drag will not
 * work and your container will be positioned incorrectly.
 */

public class TabbedContainer
  extends ClippedContainer implements Scrollable, AnimationFinished
{
  ...
  /**
   * Set the arrows color right after the constructor and after calling
   * setCaptionsColor, which also change this property.
   */
   ...
   * Works only for String-based captions.
   */
  ...
  /**
   * Sets the colors used on each tab. You must create and set the array with
   * the colors. Pass -1 to keep the original color. This array has precedence
   * over the other ways that changes colors, except activeTabBackColor.
   * @see #setBackColor
   * @see #useOnTabTheContainerColor
  ...
  public int lastActiveTab =
    -1; // guich@421_30: changed name to conform with getActiveIndex
  private int materialLastActiveIndex = -1;

  /**
   * In finger touch devices, the user still can flick into a disabled tab. To
   * disable this behaviour, set this flag to false; so when a disabled tab is
   * reached, the user will not be able to flick into it, and will have to click
   * on an enabled tab to continue flicking.
   * @since TotalCross 1.3
   */
  public boolean flickIntoDisabledTabs = true;

  /**
   * To be used on the setType method: specifies that the tabs will be placed
   * on the top.
   */
   ...
   * and you will be responsible to change them.
   */
  ...
  /**
   * The color used for the text of unselected tabs. Defaults to the foreground
   * color.
   */
  ...
  /**
   * The Flick object listens and performs flick animations on PenUp events
   * when appropriate.
   */
  protected Flick flick;

  /**
   Set to false to disable flicking between tabs. You can still switch
   between the tabs by clicking on them.
   * Sample:
   * <pre>
  ...
  @Override protected void computeClipRect()
  {
  ...
  @Override public void initUI() { onBoundsChanged(false); }

  /**
   * Returns the number of tabs.
   * @since TotalCross 1.15
   */
  public int getTabCount() { return count; }

  /**
   * Sets the given tab index as enabled or not. When a tab is disabled, it is
   * displayed faded, and if the user clicks on it, nothing happens. However,
   * you still can activate it by calling setActiveTab. If there are no tabs
   * enabled, the current tab will be made active and the controls will also be
   * enabled. So, if you plan to disable all tabs, better disable the
   * TabbedContainer control instead.
  ...
  { // guich@tc110_58
    ...
    if (!on && (!isTextCaption || imgCaptions != null) &&
        (imgDis[tabIndex] == null)) {
      try {
   ...
   * @since TotalCross 1.01
   */
  public boolean isEnabled(int tabIndex) { return !disabled[tabIndex]; }
  ...
  /**
   * Constructor to keep compilation compatibility with TC 1; transparentColor
   * is ignored.
   */
  ...
  /**
   * Constructs a tab bar control with images as captions, using the given
   * color as transparent color. If you don't want to use transparent colors,
   * just pass -1 to the color.
   */
  ...
  /**
   * Set the given icons to appear at the top (or bottom, if TABS_BOTTOM) of a
   * text TabbedContainer. The icon images must be squared. You must also set
   * the extraTabHeight, because the icons will be resized to
   * (extraTabHeight-fmH) in both directions.
   * @since TotalCross 1.3.4
   */
  public void setIcons(Image[] icons)
  {
    if (icons.length != count) {
      throw new RuntimeException(
        "Image array passed in setIcons must have the same length of the captions.");
    }
    imgCaptions0 = icons;
   ...
   * text TabbedContainer. The icon images must be squared. You must also set
   * the extraTabHeight, because the icons will be resized to
   * (extraTabHeight-fmH) in both directions. Also, sets the active icon.
   */
  ...
  private void setupImageProps() { imgDis = new Image[count]; }
  ...
  public int getType() { return tabsType; }

  /** Returns the Container for tab i */
  public Container getContainer(int i) { return containers[i]; }

  /**
   * Sets the type of border. Currently, only the Window.NO_BORDER and
   * Window.RECT_BORDER are supported. NO_BORDER only draws the line under the
   * tabs.
   */
  @Override public void setBorderStyle(byte style) { this.style = style; }

  /**
   * Replaces the default created Container with the given one. This way you
   * can avoid adding a container to a container and, as such, waste memory.
   * Note that you must do this before the first setRect for this
   * TabbedContainer; otherwise, you must explicitly call setRect again to
   * update the added container bounds
   */
  public void setContainer(int i, Container container)
  {
    if (containers != null && i >= 0 && i < containers.length) {
      ...
      if (i == activeIndex) { // guich@300_34: fixed problem when the current
                              // tab was changed
        remove(old);
        if (flick != null) {
        ...
        add(container);
        tabOrder.removeAllElements(); // don't let the cursor keys get into our
                                      // container
  ...
  /**
   * Sets the currently active tab, animating it. A PRESSED event will be posted
   * to the given tab if it is not the currently active tab; then, the
   * containers will be switched.
   */
  public void setActiveTab(int tab) { setActiveTab(tab, true); }

  /**
   * Sets the currently active tab. A PRESSED event will be posted to
   * the given tab if it is not the currently active tab; then, the containers
   * will be switched. The animation is optional and can also be defined with
   * @see
   */
      ...
      if (flick == null) {
        ...
        if (!firstTabChange && animationTime > 0 && animate) {
          try {
            ...
            PathAnimation p = PathAnimation.create(
              this, 0, 0, dif * width - correction, 0, this, animationTime);
          ...
          for (int xx = -activeIndex * width + clientRect.x, i = 0;
               i < containers.length;
               i++, xx += width) {
            containers[i].x = xx;
          }
  ...
  @Override public void onAnimationFinished(ControlAnimation anim)
  {
    tabOrder.removeAllElements(); // don't let the cursor keys get into our
                                  // container
    computeTabsRect();
  ...
  public int getActiveTab() { return activeIndex; }
  ...
  /**
   * Returns the caption height for this TabbedContainer. Note that it is not
   * possible to compute the correct height of each container, since they will
   * be added AFTER this TabbedContainer has their bounds set. So, you should
   * actually use some
   * other way to specify the bounds, like FILL or FIT; using PREFERRED in the
   * height of setRect will make your application abort.
   */
  ...
  /**
   * Returns the minimum width (based on the sizes of the captions) for this
   * TabbedContainer
   */
  @Override public int getPreferredWidth()
  {
    int sum = 0;
    if (count > 0) {
      // the max size is the size of the biggest bolded title plus the size of
      // the plain titles
      int maxw = 0, maxi = 0;
      for (int i = count; --i >= 0;) {
        int w = rSel[i].width;
      ...
      sum += maxw - rNotSel[maxi].width; // add the diff between the bold and
                                         // the plain fonts of the biggest title
    }
    return sum + getExtraSize(); // guich@573_11: changed from 3 to 2
  }

  @Override public void setNinePatch(Image img, int side, int corner)
  {
    super.setNinePatch(img, side, corner);
    canUseBorder2 = false;
  }

  private int getExtraSize() { return 2 + insets.left + insets.right; }

  /**
   * Returns the index of the next/prev enabled tab, or the current tab if
   * there's none.
   */
  private int nextEnabled(int from, boolean forward)
  {
    for (int i = 0; i < count; i++) {
      boolean limitsReached =
        (forward && from == containers.length - 1) || (!forward && from == 0);
      if (limitsReached) {
  ...
  /**
   * Used internally. resizes all the containers and add the arrows if scroll
   * is needed.
   */
  @Override protected void onBoundsChanged(boolean screenChanged)
  {
    ...
    if (autoShrinkCaptions) {
      Vm.arrayCopy(strCaptions0,
                   0,
                   strCaptions = new String[strCaptions0.length],
                   0,
                   strCaptions.length);
      onFontChanged();
      int idx = 0;
    ...
    if (getDoEffect() && effect != null) {
      effect.color = Color.brighter(
        pressedColor != -1 ? pressedColor : activeTabBackColor, 32);
    }
    if (flick != null) {
  ...
  private boolean mustScroll()
  {
    if (!allSameWidth) {
      return count > 1 &&
        getPreferredWidth() >
          this.width; // guich@564_10: support scroll - guich@573_2: only
                      // add arrows if there's more than one tab
    }
  ...
  private void addArrows()
  {
    ...
    if (scroll && showArrows) {
      int c = parent != null ? parent.backColor
                             : UIColors.controlsBack; // guich@573_4
      if (btnLeft == null) {
        int hh = Settings.fingerTouch
                   ? fmH * 3 / 4
                   : Math.max(fmH / 2,
                              tabH / 4); // guich@tc110_90: use tab height if
                                         // its larger than font's height
        ...
        super.add(btnRight,
                  RIGHT,
                  tabsType == TABS_TOP ? (tabH - yy) : (this.height - yy),
                  PREFERRED + (Settings.fingerTouch ? fmH : 0),
                  PREFERRED);
  ...
  public void computeTabsRect()
  {
    ...
    if (tabsType == TABS_NONE) {
      ;
    } else if (!allSameWidth &&
               transparentBackground) { // using balls? center on screen
      int ww = 0;
      for (int i = 0; i < n; i++) {
    ...
    if (btnLeft != null && mustScroll()) {
      ...
      if (canGoLeft || canGoRight) {
        ...
        if (toIdx == 0) {
          ...
          xOfs = Settings.fingerTouch
                   ? fmH * 2
                   : 7 * fmH / 11; // keep part of the previous tab on screen
   ...
   */
  @Override public void onFontChanged()
  {                   // guich@564_11
    ...
    int extraTabHeight = tabsType == TABS_NONE
                           ? 0
                           : this.extraTabHeight + (uiMaterial ? lineH * 3 : 0);
    tabH = tabsType == TABS_NONE
             ? 0
             : isText ? uiAndroid ? (fmH + 8 + extraTabHeight) : (fmH + 4)
                      : (imgCaptions[0].getHeight() + 4);
    ...
    for (int i = count; --i >= 0;) {
      wplains[i] =
        isText ? fm.stringWidth(strCaptions[i]) : imgCaptions[i].getWidth();
      wbolds[i] = isText && !uiAndroid
                    ? fmb.stringWidth(strCaptions[i])
                    : wplains[i]; // in uiandroid there's no bold font
    }
    ...
    for (int i = count; --i >= 0;) {
      if (tabsType == TABS_NONE) {
        ...
        rNotSel[i] =
          imgCaptions == null
            ? new Rect(0,
                       tabsType == TABS_TOP ? extraTabHeight / 2 : 0,
                       allSameWidth ? wp : wplains[i] + 12,
                       tabH - extraTabHeight / 2)
            : new Rect(0, 0, allSameWidth ? wp : wplains[i] + 12, tabH);
      } else {
        rSel[i] = new Rect(0, 0, allSameWidth ? wb : wbolds[i] + 5, tabH);
        rNotSel[i] =
          new Rect(0, y0, allSameWidth ? wp : wplains[i] + 4, tabH - 2);
      }
    }
    if (isText && imgCaptions != null) {
      if (extraTabHeight == 0) {
        ...
        try {
          ...
          for (int i = 0; i < count; i++) {
            imgCaptions[i] =
              imgCaptions0[i].getSmoothScaledInstance(size, size);
          }
          if (activeIcon0 != null) {
  ...
  {
    if (uiAndroid && parent != null &&
        backColor == parent.backColor) { // same background color in uiandroid
                                         // does not look good.
    ...
    fColor =
      (isEnabled() || !brightBack) ? getForeColor() : Color.darker(foreColor);
    cColor = (isEnabled() || !brightBack) ? getCaptionColor()
                                          : Color.darker(captionColor);
    if (colorsChanged && btnLeft != null) {
      btnRight.arrowColor = btnLeft.arrowColor = arrowsColor;
      btnRight.backColor = btnLeft.backColor =
  ...
  @Override public void onPaint(Graphics g)
  {
    if (activeIndex == -1) {
      return;
    }
    ...
    int containerColor =
      containers[activeIndex]
        .backColor; // guich@580_26: use current container's backcolor
                    // instead of TabbedContainer's backcolor
    ...
    // draw the tabs
    if (tabsType != TABS_NONE) {
      boolean drawSelectedTabAlone =
        !transparentBackground && (activeTabBackColor >= 0 || uiAndroid);

      if (!transparentBackground &&
          (uiAndroid || useOnTabTheContainerColor || tabsBackColor != null ||
        ...
        for (int i = 0; i < n; i++) {
          ...
          r = rects[i];
          g.backColor = back = getTabColor(
            i); // guich@580_7: use the container's color if containersColor
                // was not set - guich@tc110_59: use default back color if
                // container was not yet shown.
          ...
          if (npParts != null) {
            try {
              ...
              npback = NinePatch.getInstance().getNormalInstance(
                ...
                r.width,
                r.height,
                i == tempSelected && pressedColor != -1 ? pressedColor : back,
                !atTop);
            ...
            g.fillVistaRect(
              r.x + 1, r.y + 1, r.width - 2, r.height - 2, back, atTop, false);
          } else if (!uiMaterial) {
            g.fillRect(r.x, r.y, r.width, r.height); // (*)
          }
        }
      ...
      if (drawSelectedTabAlone) { // draw again for the selected tab if we want
                                  // to use a different color
        int b = containers[activeIndex].backColor;
        if (tabsBackColor == null && useOnTabTheContainerColor &&
            activeTabBackColor != -1) {
          g.backColor = b == backColor ? activeTabBackColor : b;
        } else {
          boolean dontUseTabs =
            tabsBackColor == null || tabsBackColor[activeIndex] == -1;
          g.backColor = activeTabBackColor != -1 && dontUseTabs
                          ? activeTabBackColor
                          : activeTabBackColor != -1 && !dontUseTabs
                              ? Color.interpolate(activeTabBackColor,
                                                  tabsBackColor[activeIndex])
                              : getTabColor(activeIndex);
        }
        r = rects[activeIndex];
        if (npParts != null) {
          try {
            ...
            npback = NinePatch.getInstance().getNormalInstance(
              npParts, r.width, r.height, g.backColor, !atTop);
            NinePatch.tryDrawImage(g, npback, r.x, r.y);
          } catch (ImageException ie) {
            if (Settings.onJavaSE) {
              ie.printStackTrace();
      ...
      if (uiMaterial && activeIcon == null) {
        ...
        if (materialLastActiveIndex != -1 && lastActiveTab != activeIndex) {
          int iold =
            materialLastActiveIndex == -1 ? lastActiveTab : activeIndex;
          int inew = materialLastActiveIndex == -1 ? activeIndex
                                                   : materialLastActiveIndex;
      ...
      for (int i = 0; i < n; i++) {
        r = rects[i];
        int xx =
          r.x + (r.width - (i == activeIndex ? wbolds[i] : wplains[i])) / 2;
        int yy = r.y + (r.height - fmH) / 2;
        // if (uiMaterial) yy -= lineH*3;
        if (isText) {
          g.foreColor = disabled[i]
                          ? Color.getCursorColor(cColor)
                          : i != activeIndex && unselectedTextColor != -1
                              ? unselectedTextColor
                              : cColor; // guich@200b4_156
          if (uiAndroid) {
            g.drawText(strCaptions[i],
                       xx,
                       atTop
                         ? (extraTabHeight > 0 ? (r.y + r.height - fmH - 7 -
                                                  (uiMaterial ? lineH * 3 : 0))
                                               : yy - 2)
            ...
            g.drawText(
              strCaptions[i], xx, yy, textShadowColor != -1, textShadowColor);
          } else {
          ...
          if (imgCaptions != null && imgCaptions[i] != null) {
            g.drawImage(
              i == activeIndex && activeIcon != null
                ? activeIcon
                : disabled[i] ? imgDis[i] : imgCaptions[i],
          ...
          g.drawImage(i == activeIndex && activeIcon != null
                        ? activeIcon
                        : disabled[i] ? imgDis[i] : imgCaptions[i],
                      r.x + (r.width - imgCaptions[i].getWidth()) / 2,
                      r.y + 1);
      ...
      if (!uiAndroid) {
        g.foreColor = getTabColor(
          activeIndex); // guich@580_7: use the container's back color
      ...
      if (Settings.keyboardFocusTraversable &&
          focusMode ==
            FOCUSMODE_CHANGING_TABS) { // draw the focus around the current
                                       // tab - guich@580_52: draw the cursor
                                       // only when changing tabs
  ...
  {
    return tabsBackColor != null && tabsBackColor[tab] != -1
      ? tabsBackColor[tab]
      : useOnTabTheContainerColor && containers[tab].backColor != -1
          ? containers[tab].backColor
          : backColor;
  }
  ...
  /**
   * Gets the text color of the captions. return a grayed value if this control
   * is not enabled.
   */
  ...
  /**
   * Returns the area excluding the tabs and borders for this TabbedContainer.
   * In this version, you provide the created Rect to be filled with the
   * coords.
   */
  ...
  @Override public void onEvent(Event event)
  {
    ...
    switch (event.type) { // material: when the container is dragged and we have
    ...
    if (event.target != this) {
      ...
      if (!(flick != null && (event.type == PenEvent.PEN_DRAG ||
    ...
    switch (event.type) {
      ...
      case PenEvent.PEN_DRAG:
        ...
        break;
      case ControlEvent.FOCUS_IN: // guich@580_53: when focus is set, activate
                                  // tab changing mode.
        if (Settings.keyboardFocusTraversable) {
      ...
      case PenEvent.PEN_DOWN:
        ...
        if (pe.x < btnX &&
            (flick != null ||
             (rects[0].y <= pe.y &&
              pe.y <= rects[0].y2()))) { // guich@tc100b4_7 - guich@tc120_48:
                                         // when fingerTouch, the y position may
                                         // be below the tabbed container
          int sel = -1;
          if (flick != null) { // guich@tc120_48
            ...
            for (int i = count - 1; i >= 0; i--) {
              Rect r = rects[i];
              int d =
                (int)(Convert.getDistancePoint2Rect(
                        pe.x, pe.y, r.x, r.y, r.x + r.width, r.y + r.height) +
                      0.5);
              if (d <= minDist) {
                minDist = d;
                sel = i;
      ...
      case KeyEvent.ACTION_KEY_PRESS:
        focusMode = FOCUSMODE_CHANGING_TABS;
        // guich@573_23 - super.drawHighlight(); // remove the highlight around
        // the TabbedContainer
        ...
        if (Settings.keyboardFocusTraversable) {
          ...
          if (focusMode == FOCUSMODE_CHANGING_TABS) {
            if (key == SpecialKeys.LEFT || key == SpecialKeys.RIGHT) {
              ...
              containers[activeIndex].changeHighlighted(containers[activeIndex],
                                                        ke.isDownKey());
              isHighlighting = true;
            }
          }
  ...
  @Override public void changeHighlighted(Container p, boolean forward)
  {
    Window w = getParentWindow();
    if (w != null) {
      switch (focusMode) {
        case FOCUSMODE_OUTSIDE: // focus just got here
          ...
          break;
        case FOCUSMODE_INSIDE_CONTAINERS: // was changing a control and the
  ...
  @Override public void setHighlighting() { isHighlighting = false; }

  @Override public void reposition()
  {
    super.reposition();
    computeTabsRect();
    addArrows(); // this is needed because the btnX was not yet repositioned
                 // when onBounds called addArrows.
    if (mustScroll()) {
      scrollTab(activeIndex);
    }
    if (Settings.fingerTouch) {
  ...
  @Override public void getFocusableControls(Vector v)
  {
  ...
  @Override public Control handleGeographicalFocusChangeKeys(KeyEvent ke)
  {
    ...
    if (MainWindow.mainWindowInstance._focus == this) {
      ...
      if ((ke.isNextKey() && activeIndex == containers.length - 1) ||
          (ke.isPrevKey() && activeIndex == 0)) {
        return null;
      }
      ke.target = this;
      _onEvent(ke);
    ...
    Control c =
      findNextFocusControl(MainWindow.mainWindowInstance._focus, direction);
    if (c == null) {
      boolean prev =
        direction == SpecialKeys.UP || direction == SpecialKeys.LEFT;
      c =
        (prev == atTop)
          ? this
          : MainWindow.mainWindowInstance.findNextFocusControl(this, direction);
    }
    return c;
  }
   ...
   */
  @Deprecated public boolean isAtTop() { return tabsType == TABS_TOP; }

  /**
   * Resizes the height of each added container and sets the height of this
   * TabbedContainer to the maximum height of the containers.
   */
  ...
  @Override public boolean flickStarted()
  {
    flickTimerStarted = true;
    return isScrolling;
  }

  @Override public void flickEnded(boolean atPenDown)
  {
  ...
  @Override public boolean canScrollContent(int direction, Object target)
  { // called when
  ...
  public boolean scrollContent(int xDelta, int yDelta, boolean fromFlick)
  {
    ...
    int maxX =
      -(containers[0].width * (containers.length - 1) + containers.length) - 1;
    int minX = 1;
  ...
  @Override public int getScrollPosition(int direction)
  {
  ...
  @Override public boolean wasScrolled() { return scScrolled; }

  /**
   * Changes the tab captions. The new array must have the same length or an
   * exception is thrown.
   */
  public void setCaptions(String[] caps)
  {
    if (strCaptions == null || caps.length != strCaptions.length) {
      throw new IllegalArgumentException(
        "The TabbedContainer's captions does not match the given ones.");
    }
