...
public class Window extends Container {
	...
	protected boolean beepIfOut =
		true; // guich@102: cancels the event if the user type outside the window area. to be used in a future window manager
	...
	/** @deprecated */
	@Deprecated
	protected boolean highResPrepared = Settings.platform == null ? false : !Settings.platform.equals(
											Settings.PALMOS); // guich@400_35: as default for WinCE, highres is true - use indexOf to support PalmOS/SDL - guich@552_6: added the ! - guich@553_6: check if null to let retroguard run
	...
	protected Rect
	rTitle; // guich@200b4_52: the area where the title is located - guich@tc120_61: now its protected
	...
	private static int repeatedEventMinInterval = Settings.isIOS()
			|| Settings.ANDROID.equals(Settings.platform) ? 40 : 0;
	...
	public void setFocus(Control c) {
		...
		while (c != null) {
			if (!c.focusLess
				&& c.focusTraversable) { // kmeehl@tc100: focusTraversable is now a reliable way to determine if a control should get focus
				removeFocus();
				if (_focus ==
					null) { // guich@tc100: maybe the user changed the focus to a new control in the FOCUS_OUT event
					_focus = c;
	...
	public void removeFocus() { // kmeehl@tc100
		...
		if (highlighted != null && (Settings.geographicalFocus
									|| Settings.keyboardFocusTraversable)) { // kmeehl@tc100: prevent highlighted from being trampled in taborder focus mode
			_controlEvent.type = ControlEvent.HIGHLIGHT_OUT;
			_controlEvent.update(highlighted);
			highlighted.postEvent(
				_controlEvent); // kmeehl@tc100: send the currently highlighted control a HIGHLIGHT_OUT event
			highlighted = null;
	 ...
	 * @since SuperWaba 1.2 */
	protected boolean onClickedOutside(PenEvent
									   event) { // kmeehl@tc100: changed signature to pass the event, to give the window more information
		return false;
	}
	...
	final public void _postEvent(int type, int key, int x, int y, int modifiers, int timeStamp) {
		...
		if (timeStamp == 0) {
			timeStamp =
				currentTime; // guich@401_13: get the timestamp - bruno@tc115: must come before setting lastInteractionTime
		}
		...
		if (isPenEvent) { // do all the pen event filtering here
			if (grabPenEvents !=
				null) { // guich@tc100 - guich@tc168: moved to here to prevent gaps when Whiteboard is inside a window
				PenEvent pe = type == PenEvent.PEN_DOWN
							  || type == PenEvent.PEN_UP ? _penEvent :
							  _dragEvent; // guich@tc130: fix ClassCastException when a WhiteBoard had a ToolTip attached
				Control c = _focus;
				while (c != null) {
					x -= c.x;
					y -= c.y;
			...
			if (type == PenEvent.PEN_DRAG
				&& firstDrag) { // discard first PEN_DRAG unless it exceeds the drag threshold
			...
			if (type == lastType && ((x == lastX && y == lastY)
		...
		if (this !=
			topMost) { // if this isnt the top-most window, pass control to it. i must do this because the vm calls MainWindow._postEvent
			if (topMost != null) {
		...
		if (isPenEvent || type == ControlEvent.SIP_CLOSED) {
			...
			if (shiftY != 0) { // is the screen shifted?
				if (y >= shiftH
					&& type ==
					PenEvent.PEN_DOWN) { // if screen is shifted and user clicked below the visible area, unshift screen
					lastY = lastShiftY = 0;
					...
					lastY = y = y + shiftY; // shift the y coordinate to the place that the component "thinks" it is.
					   ...
					   0) { // if the user clicked in a button (like in a Cancel button of a Window), we have to keep shifting the coordinate until the pen_up occurs
		...
		if (Settings.debugEvents) {
			Vm.debug(this + " event: type=" + type + ", key=" + key + " (" + (char) key + "), x=" + x + ", y=" +
					 y + ", mods="
		...
		if (type < 300) {
			Settings.lastInteractionTime =
				currentTime; // don't use the event timestamp, since it can be wrong! it's better to get the current timestamp instead.
		}
		...
		if (Settings.scrollDistanceOnMouseWheelMove != 0 && type == MouseEvent.MOUSE_WHEEL
			&& contains(x, y)) {
		...
		if (isPenEvent
			|| type ==
			MouseEvent.MOUSE_MOVE) { // guich@102: user clicked outside the window? - guich@tc126_45: send MOUSE_IN/OUT when the window bounds are crossed
			boolean inside = contains(x, y);
			if (type == MouseEvent.MOUSE_MOVE) {
				if (inside !=
					lastInside) { // if the mouse passed against the window bounds (we only have to post events to the topmost window)
					lastInside = inside;
					...
					return;
					   ...
					   && !Flick.isDragging) { // guich@tc130: must check if user is dragging before a flick, otherwise, in a paged-flick, the process would end too early
				_penEvent.type = type;
				_penEvent.x = x;
				_penEvent.y = y;
			...
			if (!Settings.geographicalFocus && Settings.keyboardFocusTraversable
				...
				if (_focus != null && highlighted == _focus
					...
					if (c == this || c == null || !c.focusTraversable
						...
						if (firstFocus == null || firstFocus.focusLess || !firstFocus.focusTraversable
							|| !firstFocus.visible
							|| !firstFocus.isEnabled()) { // kmeehl@tc100: if firstfocus is set and focusable, set it as the first control to get focus
							if (fakeControl ==
								null) { // create a fake control and find the closest control to the top-left corner
		...
		if (menubar != null && (invokeMenu || (isKeyEvent
											   && key == SpecialKeys.MENU))) { // guich@200b4: popup the menu if was have a title and it was clicked
			popupMenuBar();
			return;
		}
		...
		if (_focus != null && _focus != this && (!_focus.isEnabled()
				|| _focus.parent ==
				null)) { // guich@300_55: make always sure that the focused control is enabled; added 2nd condition
		...
		if (!isKeyEvent) {
			...
			if (type == PenEvent.PEN_UP) {
				...
				if (firstDrag) {
					...
					de.dragId =
						currentDragId++; // set the drag id, which should be used until the end of the physical drag sequence
					...
					de.absoluteY = de.y = ptPenDownY + shiftY -
										  shiftYAtPenDownY; // guich@tc138: fix the y position if the window was shifted since the last pen down
					for (Control c = target; c != null; c = c.parent) { // translate x, y to coordinate system of target
						de.x -= c.x;
						de.y -= c.y;
					}
		...
		if (event.type ==
			PenEvent.PEN_UP) { // guich@320_31: release tempFocus - bruno@tc126: release tempFocus BEFORE posting PEN_UP event
			focusOnPenUp = _focus != null && (_focus instanceof Edit
											  || _focus instanceof MultiEdit) ? _focus : null;
			tempFocus = null;
		}
	...
	protected void paintTitle(String title, Graphics gg) {
		if (title != null
			|| borderStyle >
			NO_BORDER) { // guich@220_48: changed = NO_BORDER by > NO_BORDER to let MenuBar set borderStyle to -1 and thus we don't interfere with its paint
			if (title == null) {
				title = uiAndroid ? "" : " ";
			}
			String tit = title;
			...
			if (borderStyle != NO_BORDER) {
				...
				gg.foreColor = titleColor == -1 ? headerColor == b ? f : b :
			...
			if (rTitle == null && borderStyle != NO_BORDER) {
				rTitle = new Rect(xx - 2, 0, ww + 4, hh == 0
								  && tit.length() > 0 ? titleFont.fm.height : hh + 1); // guich@200b4_52
			}
		}
	}
	...
	private void popup(Window newWin) { // anodos@320_9
		if (!started
			&& this instanceof
			MainWindow) { // guich@567_17: if the user call this method in the initUI method, repaintNow so that the back of the screen is stored correctly
	...
	public void unpop() {
		...
		if (topMost != null) {
			...
			lastTopMost = topMost;
			topMost.focusOnPopup.postEvent(new ControlEvent(ControlEvent.WINDOW_CLOSED,
										   this)); // tell last control that we closed
			if (topMost == lastTopMost) {
				topMost.setFocus(topMost.focusOnPopup);
			}
			postUnpop();
			if (topMost.focusOnPopup != null && !(topMost.focusOnPopup instanceof TextControl)
	...
	public void swap(Container newContainer) { // guich@300_57: make easier to swap containers
		...
		Control firstTarget = (_focus != null && _focus.getParentWindow() == this) ? _focus
							  : newContainer.tabOrder.size() > 0 ? (Control) newContainer.tabOrder.items[0] :
							  newContainer; // guich@573_19: set focus to the first control, instead of the new container. - guich@tc100: only if the focus was not already set in the initUI method of the newContainer
		if (Toast.cnt != null) {
			try {
		...
		topMost.focusOnPopup =
			firstTarget; // guich@550_15: otherwise, the ContainerSwitch app won't work for Sub3 when using pen less.
	...
	public static void repaintActiveWindows() {
		...
		try {
			...
			for (n = zStack.size(); i < n;
				 i++) { // repaints every window, from the nearest with the MainWindow size to last parent
				if (i == lastFade) {
	 ...
	 * @since SuperWaba 5.5
	 */
	protected boolean handleFocusChangeKeys(KeyEvent
											ke) { // guich@512_1: transfer focus on tab keys - fdie@550_15 : transfer also on arrow keys
		if (isHighlighting) {
			...
			if (isAction && highlighted != null) {
				highlighted.requestFocus();
				isHighlighting =
					false; // kmeehl@tc100: set isHighlighting after requesting focus so that the focus_out will not set it back
				...
				if (highlighted == null && firstFocus != null && !firstFocus.focusLess
					&& firstFocus.focusTraversable
	...
	public void setHighlighted(Control c) {
		if (Settings.keyboardFocusTraversable
			&& c != highlighted) { // kmeehl@tc100: removed check for children. focusTraversable is now a reliable way to tell if a control should be focusable and highlightable; check if c is null
			if (highlighted != null) {
				_controlEvent.type = ControlEvent.HIGHLIGHT_OUT;
				_controlEvent.update(highlighted);
				highlighted.postEvent(
			...
			if (c != null && c.focusTraversable) {
				...
				c.postEvent(
					_controlEvent); // kmeehl@tc100: send the currently highlighted control a HIGHLIGHT_IN event
				highlighted = c;
			}
			if (Settings.isOpenGL) {
				needsPaint = true;
	...
	void drawHighlight(Control c) {
		...
		if (c != null && c.visible && c.isDisplayed() && Settings.keyboardFocusTraversable
			&& c.asWindow == null) { // guich@tc112_34: don't draw highlight of a Window (as in MenuBarDropDown)
	...
	public static int getDefaultDragThreshold() {
		double threshold = (Settings.fingerTouch ? DEFAULT_DRAG_THRESHOLD_IN_INCHES_FINGER
							: DEFAULT_DRAG_THRESHOLD_IN_INCHES_PEN) * (Settings.screenWidthInDPI + Settings.screenHeightInDPI) /
						   2;
	...
	public void shiftScreen(Control c, int deltaY) {
		if (c == null) { // unshift the screen?
			...
			if (newShiftY != shiftY) {
				...
				shiftH = Settings.onJavaSE ? Settings.screenHeight / 2 : (1 + 1 + 2) *
						 c.fmH; // one line above and two below control, plus control's line
			}
			needsPaint = true;
		}
	}
