 ...
 * <p>
 * A window cannot be added to a container; use popup or popupNonBlocking to
 *show it on screen. Trying to add it to a container will raise a
 *RuntimeException. <P> The following example creates a popup window class:
 *<pre>
 ...
 *  }
 *</pre>
 * To use it <b>non-blocking</b> (the execution continues right after the popup
 *command, even with the window still open): <pre> public class Launcher extends
 *MainWindow
 *  {
 *     TestWindow tw;
 *     public void initUI()
 ...
 *  }
 *</pre>
 * Blocking popup may be use in InputBox/MessageBox classes, while non-blocking
 *popup is used in MenuBar and other classes. <p> Important note: you can't use
 *popup with a delay to unpop it. In this case, the correct would be to use
 *popupNonBlocking: <pre> mb = new MessageBox(...); mb.popupNonBlocking();
...
public class Window extends Container
{
  ...
  /**
   * When this window is the top most and the user clicks outside, a beep is
   * thrown. Set this to false do disable the beep.
   */
  protected boolean beepIfOut =
    true; // guich@102: cancels the event if the user type outside the window
          // area. to be used in a future window manager
   ...
   *  If false (default), the Window is doubled size (and centered) to make
   * controls fit.
   */
  /** @deprecated */
  @Deprecated
  protected boolean highResPrepared =
    Settings.platform == null
      ? false
      : !Settings.platform.equals(
          Settings.PALMOS); // guich@400_35: as default for WinCE, highres
                            // is true - use indexOf to support PalmOS/SDL
                            // - guich@552_6: added the ! - guich@553_6:
                            // check if null to let retroguard run

  /**
   * A temporary title that will be displayed when this Windows pops up. It
   * will be replaced by the original title when it is closed.
   * @since TotalCross 1.53
   */
  public String tempTitle;

  /**
   * @deprecated Flick is now enabled by default; just remove the reference to
   *     it.
   */
  @Deprecated public static boolean flickEnabled;

  static int[] borderGaps = {
    0, 1, 2, 1, 0, 0, 0
  }; // guich@200final_14 - guich@400_77 - guich@564_16
  protected Control _focus, focusOnPenUp;
  private Control
    focusOnPopup; // last control that had focus when popup was called.
  /**
  ...
  protected Rect rTitle; // guich@200b4_52: the area where the title is located
                         // - guich@tc120_61: now its protected
  ...
  private static int repeatedEventMinInterval =
    Settings.isIOS() || Settings.ANDROID.equals(Settings.platform) ? 40 : 0;
  ...
  /**
   * If true, the next pen_up event will be ignored. This is used when a
   * pen_down cancels a flick, or if a drag-scrollable control needs to cancel
   * the next pen_up during a drag-scrolling interaction.
   */
  ...
  /**
   * The title color. The title color depends on the border type: it will be
   * the foreground color if NO_BORDER is set, otherwise will be the background
   * color.
   */
  public int titleColor = -1; // guich@tc110_13

  /**
   * A vertical gap used to increase the title area. Defaults to fmH/2 in
   * Android, 0 on other user interface styles.
   ...
   * It can be LEFT, CENTER or RIGHT, and you can use an adjustment on the value
   * (E.G.: LEFT+5).
  ...
  /**
   * Set the header and the footer colors when in Android style and border type
   * is ROUND_BORDER. Not used in other styles.
   * @since TotalCross 1.3
   */
  ...
  /**
   * During a swap, the container is repainted. If the swap is being done under
   * the presence of a (wait) Window, the container will be drawn but the window
   * dont. To fix this, set this flag to false.
   */
  ...
  /**
   * Stack of poped up windows. Dont mess with this, unless you want to crash
   * the VM!
   */
  public static totalcross.util.Vector zStack =
    new totalcross.util.Vector(5); // guich@102
  ...
  private static final double DEFAULT_DRAG_THRESHOLD_IN_INCHES_PEN =
    1.0 * 0.0393700787; // 0.5mm
  private static final double DEFAULT_DRAG_THRESHOLD_IN_INCHES_FINGER =
    1.0 * 0.0393700787; // 1.0mm
  ...
  public Window()
  {
    ...
    titleGap =
      uiAndroid && borderStyle != NO_BORDER ? titleFont.fm.height / 2 : 0;
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Set to a control to redirect all pen events directly to it. This speeds up
   * pen event processing. Used in Whiteboard class.
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Force the orientation during runtime. Use one of ORIENTATION_PORTRAIT /
   * ORIENTATION_LANDSCAPE. Can be ored with ORIENTATION_INVERTED flag.
   */
   ...
   * @see #VERTICAL_GRADIENT
   */
  @Override public void setBorderStyle(byte borderStyle)
  {
    this.borderStyle = borderStyle;
    rTitle = null;
    needsPaint = true;
  ...
  @Override public byte getBorderStyle() { return borderStyle; }

  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Makes this window static, i.e., the user will not be able to move it
   * around the screen by dragging the title area. It just set the canDrag
   * member to false.
   */
  ...
  public void setFocus(Control c)
  {
    ...
    while (c != null) {
      if (!c.focusLess &&
          c.focusTraversable) { // kmeehl@tc100: focusTraversable is now a
                                // reliable way to determine if a control should
                                // get focus
        removeFocus();
        if (_focus == null) { // guich@tc100: maybe the user changed the focus
                              // to a new control in the FOCUS_OUT event
          _focus = c;
          if (c.getParentWindow() != getTopMost()) {
            focusOnPopup = c;
          }
          if (c.isEnabled()) { // guich@tc152: disabled controls can't send
                               // focus events
  ...
  { // kmeehl@tc100
    ...
    if (highlighted != null &&
        (Settings.geographicalFocus ||
         Settings
           .keyboardFocusTraversable)) { // kmeehl@tc100: prevent highlighted
                                         // from being trampled in taborder
                                         // focus mode
      _controlEvent.type = ControlEvent.HIGHLIGHT_OUT;
      _controlEvent.update(highlighted);
      highlighted.postEvent(
        _controlEvent); // kmeehl@tc100: send the currently highlighted
                        // control a HIGHLIGHT_OUT event
      highlighted = null;
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Changes the focus to the desired control, without posting FOCUS_IN and
   * FOCUS_OUT events
   ...
   * <control>focusedControl</control>, unless the last one has been changed
   * manually.
   * @see totalcross.ui.Window#setFocus
   */
  public Control getFocus() { return _focus; }

  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * called when the user clicks outside the bounds of this window. must return
   * true if the event was handled, false otherwise. If false is returned and
   * <code>beepIfOut</code> is true, then a beep is played and nothing more
   * happens.
   * @since SuperWaba 1.2
   */
  protected boolean onClickedOutside(PenEvent event)
  { // kmeehl@tc100: changed signature to pass the event, to
    // give the window more information
    return false;
  }
   ...
   */
  final public void _postEvent(int type,
                               int key,
                               int x,
                               int y,
                               int modifiers,
  ...
  {
    boolean isPenEvent =
      !multiTouching && PenEvent.PEN_DOWN <= type && type <= PenEvent.PEN_DRAG;
    boolean isKeyEvent =
      type == KeyEvent.KEY_PRESS || type == KeyEvent.SPECIAL_KEY_PRESS;
    if (isKeyEvent && Settings.optionalBackspaceKey != 0 &&
        key == Settings.optionalBackspaceKey) {
      key = SpecialKeys.BACKSPACE;
    }
    if (isKeyEvent && Settings.deviceRobotSpecialKey != 0 &&
        Settings.deviceRobotSpecialKey == key) {
      onRobotKey();
      return;
    }
    ...
    if (timeStamp == 0) {
      timeStamp = currentTime; // guich@401_13: get the timestamp - bruno@tc115:
                               // must come before setting lastInteractionTime
    }

    if (robot != null && UIRobot.status == UIRobot.RECORDING &&
        this == topMost) {
    ...
    if (isPenEvent) { // do all the pen event filtering here
      if (grabPenEvents !=
          null) { // guich@tc100 - guich@tc168: moved to here to prevent gaps
                  // when Whiteboard is inside a window
        PenEvent pe =
          type == PenEvent.PEN_DOWN || type == PenEvent.PEN_UP
            ? _penEvent
            : _dragEvent; // guich@tc130: fix ClassCastException when a
                          // WhiteBoard had a ToolTip attached
        Control c = _focus;
        while (c != null) {
          x -= c.x;
          y -= c.y;
        ...
        grabPenEvents._onEvent(
          pe.update(grabPenEvents, x, x + gpeX, y, y + gpeY, type, modifiers));
        return;
      }
      if (type == PenEvent.PEN_DRAG &&
          firstDrag) { // discard first PEN_DRAG unless it exceeds the drag
                       // threshold
        int absDeltaX = x > ptPenDownX ? x - ptPenDownX : ptPenDownX - x;
        int absDeltaY = y > ptPenDownY ? y - ptPenDownY : ptPenDownY - y;
        if (absDeltaX < Settings.touchTolerance &&
            absDeltaY < Settings.touchTolerance) {
          return;
        }
      }
      if (type == lastType &&
          ((x == lastX && y == lastY) ||
           (timeStamp - lastTime) < repeatedEventMinInterval)) {
        return;
      }
    ...
    if (type == PushNotificationEvent.TOKEN_RECEIVED) { // single message
      broadcastEvent(
        new PushNotificationEvent(type, PushNotification.readToken()));
      return;
    }
    ...
    if (key == SpecialKeys.SCREEN_CHANGE) { // dont move from here!
      ...
      for (int i = 0; i < zStack.size(); i++) {
        ((Window)zStack.items[i])
          .screenResized(); // guich@tc123_21: only call screenResized here.
      }
      repaintActiveWindows(); // guich@tc123_21: let this method handle the
                              // screen update
      return;
    }
    if (this !=
        topMost) { // if this isnt the top-most window, pass control to it. i
                   // must do this because the vm calls MainWindow._postEvent
      if (topMost != null) {
    ...
    if (isPenEvent || type == ControlEvent.SIP_CLOSED) {
      ...
      if (shiftY != 0) { // is the screen shifted?
        if (y >= shiftH &&
            type ==
              PenEvent.PEN_DOWN) { // if screen is shifted and user clicked
                                   // below the visible area, unshift screen
          lastY = lastShiftY = 0;
          ...
          lastY = y = y + shiftY; // shift the y coordinate to the place that
                                  // the component "thinks" it is.
        }
      } else if (lastShiftY !=
                 0) { // if the user clicked in a button (like in a Cancel
                      // button of a Window), we have to keep shifting the
                      // coordinate until the pen_up occurs
    ...
    if (Settings.debugEvents) {
      Vm.debug(this + " event: type=" + type + ", key=" + key + " (" +
               (char)key + "), x=" + x + ", y=" + y + ", mods=" + modifiers +
               ", time=" + timeStamp);
    }

    if (type < 300) {
      Settings.lastInteractionTime =
        currentTime; // don't use the event timestamp, since it can be wrong!
                     // it's better to get the current timestamp instead.
    }

    Event event = null;
    boolean invokeMenu = false;
    if (_focus == null) {
      _focus = this; // guich@200b4: make sure that there is always one control
                     // with focus. this test was being made in // 1 and // 2
    }
    ...
    if (type == MultiTouchEvent.SCALE) {
      ...
      if (needsPaint) {
        repaintActiveWindows(); // guich@tc100: paint the topMost, not
                                // ourselves.
      }
      return;
    }
    ...
    if (Flick.currentFlick == null &&
        (isMoving || (isPenEvent && rTitle != null &&
                      rTitle.contains(x - this.x, y - this.y)))) {
      if (shiftY != 0) {
        return;
      }
      switch (type) {
        ...
        case PenEvent.PEN_UP:
          invokeMenu =
            ptMoving != null && (!isMoving || this instanceof MainWindow);
          isMoving = false;
          ptMoving = null;
          break;
      }
    ...
    if (Settings.scrollDistanceOnMouseWheelMove != 0 &&
        type == MouseEvent.MOUSE_WHEEL && contains(x, y)) {
      Control c = findChild(x - this.x, y - this.y);
      for (; c != null && !(c instanceof Scrollable); c = c.parent) {
      }
      if (c != null && c instanceof Scrollable &&
          !(c instanceof TabbedContainer)) {
        int k = Settings.scrollDistanceOnMouseWheelMove;
        Scrollable sc = (Scrollable)c;
        boolean canScrollVert = sc.canScrollContent(DragEvent.UP, sc) ||
                                sc.canScrollContent(DragEvent.DOWN, sc);
        boolean canScrollHoriz = sc.canScrollContent(DragEvent.LEFT, sc) ||
                                 sc.canScrollContent(DragEvent.RIGHT, sc);
        int kx = 0, ky = 0;
        ...
        if ((ky != 0 && sc.scrollContent(0, ky, false)) ||
            (kx != 0 && sc.scrollContent(kx, 0, false))) {
          repaintNow();
        }
        return;
      }
    ...
    if (isPenEvent ||
        type == MouseEvent
                  .MOUSE_MOVE) { // guich@102: user clicked outside the window?
                                 // - guich@tc126_45: send MOUSE_IN/OUT when the
                                 // window bounds are crossed
      boolean inside = contains(x, y);
      if (type == MouseEvent.MOUSE_MOVE) {
        if (inside !=
            lastInside) { // if the mouse passed against the window bounds (we
                          // only have to post events to the topmost window)
          lastInside = inside;
          _mouseEvent.target = this;
          _mouseEvent.consumed = false;
          _mouseEvent.timeStamp = timeStamp;
          _mouseEvent.type =
            inside ? MouseEvent.MOUSE_IN : MouseEvent.MOUSE_OUT;
          postEvent(_mouseEvent);
          return;
        }
      } else if (!inside &&
                 !Flick.isDragging) { // guich@tc130: must check if user is
                                      // dragging before a flick, otherwise, in
                                      // a paged-flick, the process would end
                                      // too early
        _penEvent.type = type;
        _penEvent.x = x;
        _penEvent.y = y;
        ...
        _penEvent.touch();
        if (_focus != null && _focus != this &&
            (_focus == _dragEvent.target || x == 10000) &&
    ...
    if (isKeyEvent) {
      ...
      if (!Settings.geographicalFocus && Settings.keyboardFocusTraversable &&
          (_keyEvent.isPrevKey() || _keyEvent.isNextKey() ||
           _keyEvent.isActionKey())) {
        ...
        // geographical focus
        Control c;
        if (_focus != null && highlighted == _focus &&
            (c = bubbleHandleFocusChangeKey(_focus, _keyEvent)) != null) {
          setFocus(c);
          if (needsPaint) {
            repaintActiveWindows(); // guich@tc100: paint the topMost, not
                                    // ourselves.
          ...
          if (c == this || c == null || !c.focusTraversable ||
              c.focusLess) { // find a new control to set focus to
            if (firstFocus == null || firstFocus.focusLess ||
                !firstFocus.focusTraversable || !firstFocus.visible ||
                !firstFocus.isEnabled()) { // kmeehl@tc100: if firstfocus is set
                                           // and focusable, set it as the first
                                           // control to get focus
              if (fakeControl ==
                  null) { // create a fake control and find the closest control
    ...
    if (menubar != null &&
        (invokeMenu ||
         (isKeyEvent &&
          key == SpecialKeys.MENU))) { // guich@200b4: popup the menu if was
                                       // have a title and it was clicked
      popupMenuBar();
      return;
    }
    ...
    if (!isKeyEvent && type == PenEvent.PEN_DOWN) {
      ...
      if (!controlFound && Settings.fingerTouch) { // guich@tc120_48
        Control cn =
          findNearestChild(x - this.x, y - this.y, Settings.touchTolerance);
        if (cn != null) {
          c = cn;
        }
      }
      ...
      tempFocus = c;
      forceShifted = false;
      if (Flick.currentFlick == null && c.focusOnPenDown &&
          !c.focusLess) { // if flicking, do not transfer focus
        forceShifted =
          c == _focus ||
          (c.isEnabled() && (c instanceof Edit || c instanceof MultiEdit));
        if (c != _focus) {
          setFocus(c);
        }
      }
    ...
    if (_focus != null && _focus != this &&
        (!_focus.isEnabled() ||
         _focus.parent ==
           null)) { // guich@300_55: make always sure that the focused control
                    // is enabled; added 2nd condition
      if (_focus.isEnabled()) { // guich@tc152: disabled controls can't send
                                // focus events
        _controlEvent.type = ControlEvent.FOCUS_OUT;
        _controlEvent.target = _focus;
        _controlEvent.touch();
    ...
    if (!isKeyEvent) {
      ...
      mouseMove = type == MouseEvent.MOUSE_MOVE
                    ? findChild(x - this.x, y - this.y)
                    : null;
      Control target =
        mouseMove != null ? mouseMove : tempFocus != null ? tempFocus : _focus;
      PenEvent pe = type == PenEvent.PEN_DOWN || type == PenEvent.PEN_UP
                      ? _penEvent
                      : _dragEvent;
      pe.type = type;
      ...
      pe.absoluteX = pe.x = x;
      pe.absoluteY = pe.y = y;
      for (Control c = target; c != null;
           c = c.parent) { // translate x, y to coordinate system of target
        pe.x -= c.x;
        pe.y -= c.y;
      }
      ...
      if (type == PenEvent.PEN_UP) {
        if (Settings.unmovableSIP && (isScreenShifted() || isSipShown())) {
          boolean keepShifted =
            forceShifted || (tempFocus != null && tempFocus.willOpenKeyboard());
          if (!keepShifted && tempFocus != null) {
            ...
            if (c != null && c instanceof Scrollable &&
                ((Scrollable)c).wasScrolled()) {
              keepShifted = true;
            }
          }
          if (!keepShifted) {
            // pe.y -= lastShiftY; with this line, clicking in a control when
            // the screen is shifted makes the pe.y to an invalid value
            shiftScreen(null, 0);
            lastShiftY = 0;
            if (isSipShown()) {
        ...
        if (!firstDrag) {
          DragEvent de = _dragEvent.update(
            pe); // PEN_DRAG_END has the same coordinates as the PEN_UP
        ...
        if (tempFocus != null && _focus != tempFocus &&
            !tempFocus.focusOnPenDown && !tempFocus.focusLess) {
        ...
        if (firstDrag) {
          ...
          de.dragId =
            currentDragId++; // set the drag id, which should be used until
                             // the end of the physical drag sequence
          de.type = PenEvent.PEN_DRAG_START;
          de.modifiers = modifiers;
          de.absoluteX = de.x = ptPenDownX; // PEN_DRAG_START has the same
                                            // coordinates as the PEN_DOWN
          de.absoluteY = de.y =
            ptPenDownY + shiftY -
            shiftYAtPenDownY; // guich@tc138: fix the y position if the window
                              // was shifted since the last pen down
          for (Control c = target; c != null;
               c = c.parent) { // translate x, y to coordinate system of target
            de.x -= c.x;
            de.y -= c.y;
        ...
        de.direction =
          getDirection(ptDraggingX, ptDraggingY, x, y); // guich@tc122_11
    ...
    event.target = (type == MouseEvent.MOUSE_MOVE && mouseMove != null)
                     ? mouseMove
                     : tempFocus != null ? tempFocus : _focus;
    event.timeStamp = timeStamp;

    if (event.type ==
        PenEvent.PEN_UP) { // guich@320_31: release tempFocus - bruno@tc126:
                           // release tempFocus BEFORE posting PEN_UP event
      focusOnPenUp = _focus != null &&
                         (_focus instanceof Edit || _focus instanceof MultiEdit)
                       ? _focus
                       : null;
      tempFocus = null;
    }
  ...
  void postPushNotifications()
  {
    for (String msg : PushNotification.readMessages()) {
      broadcastEvent(
        new PushNotificationEvent(PushNotificationEvent.MESSAGE_RECEIVED, msg));
    }
  }
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Returns the client rect, ie, the rect minus the border and title area, in
   * relative coords In this version, you provide the created Rect to be filled
   * with the coords.
   */
  @Override protected void getClientRect(Rect r)
  { // guich@450_36
    int m = borderGaps[borderStyle];
    boolean onlyBorder =
      borderStyle == NO_BORDER || ((title == null || title.isEmpty()) &&
                                   (borderStyle == ROUND_BORDER && uiAndroid));
   ...
   * relative coords
   */
  @Override public Rect getClientRect()
  { // guich@200final_15
    Rect r = new Rect();
    getClientRect(r);
    return r;
  ...
  protected void paintTitle(String title, Graphics gg)
  {
    if (title != null ||
        borderStyle >
          NO_BORDER) { // guich@220_48: changed = NO_BORDER by > NO_BORDER to
                       // let MenuBar set borderStyle to -1 and thus we don't
                       // interfere with its paint
      if (title == null) {
        title = uiAndroid ? "" : " ";
      }
      String tit = title;
      ...
      if (ww > this.width - 6) {
        int idx = Convert.getBreakPos(
          titleFont.fm, new StringBuffer(tit), 0, this.width - 6, false);
        tit = tit.substring(0, idx);
        ww = titleFont.fm.stringWidth(tit);
      }
      int hh = borderStyle == NO_BORDER && tit.length() == 0
                 ? 0
      ...
      } else if ((RIGHT - RANGE) <= titleAlign &&
                 titleAlign <= (RIGHT + RANGE)) {
      ...
      if (borderStyle != NO_BORDER) {
        ...
        if (borderStyle != TAB_ONLY_BORDER && borderStyle != ROUND_BORDER &&
            borderStyle != HORIZONTAL_GRADIENT &&
        ...
        switch (borderStyle) {
          ...
          case VERTICAL_GRADIENT:
            gg.fillShadedRect(
              ...
              gradientTitleEndColor == -1 ? b : gradientTitleEndColor,
              100);
            break;
          case TAB_BORDER:
          case TAB_ONLY_BORDER:
          ...
          case ROUND_BORDER:
            ...
            //            int titlecolor = hasTitle ? headerColor != -1 ?
            //            headerColor : c : b; int footercolor = footerH > 0 ?
            //            footerColor != -1 ? footerColor : c : b; int bodycolor
            //            = b; if (sameBackgroundColor) {
            //              bordercolor = titlecolor = footercolor = bodycolor;
            //            }
            //            gg.drawWindowBorder(0, 0, width, height, hasTitle ? hh
            //            : 0, footerH, bordercolor, titlecolor, bodycolor,
            //                footercolor, borderGaps[ROUND_BORDER], hasTitle ||
            //                footerH > 0);
            //            if (!hasTitle) {
            //              return;
            //            } else {
        ...
        gg.foreColor = titleColor == -1
                         ? headerColor == b ? f : b
                         : titleColor; // draws the text with inversed color
        gg.backColor = f;
      ...
      if (rTitle == null && borderStyle != NO_BORDER) {
        rTitle =
          new Rect(xx - 2,
                   0,
                   ww + 4,
                   hh == 0 && tit.length() > 0 ? titleFont.fm.height
                                               : hh + 1); // guich@200b4_52
      }
    }
  }
  ...
  public void paintWindowBackground(Graphics gg)
  {
    if (!transparentBackground &&
        (borderStyle != ROUND_BORDER || this instanceof MainWindow)) {
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Popup a modal window, and make it child of this one. All events in the
   * behind window are deactivated.
   */
  private void popupNonBlocking(Window newWin)
  {
    ...
    if (this != topMost) {
      ...
      setFocus(
        topMost); // guich@567_4: changed from setFocus to swapFocus to fix
                  // 566_18 problem - guich@568_17: changed back to setFocus
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
     Popup a modal window, blocking the program execution, and make it child of
     this one. All events in the behind window are deactivated. Important! You
     can't use this method in the application's constructor or in the initUI
     method! Calling this
   */
  private void popup(Window newWin)
  { // anodos@320_9
    if (!started &&
        this instanceof
          MainWindow) { // guich@567_17: if the user call this method in the
                        // initUI method, repaintNow so that the back of the
                        // screen is stored correctly
      started =
        true; // don't let this repaintNow be called again if more than one
              // popup is called in sequence in the initUI of the MainWindow
      // repaintNow(); - guich@tc210 - prevent an empty white background on
      // startup.
    }
    if (!enableUpdateScreen) { // guich@tc114_57: if we need interaction, make
                               // sure that the screen was updated.
      enableUpdateScreen = true;
      repaintActiveWindows();
    }
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Hides this window. Calling unpop when only the MainWindow is active does
   * nothing.
   */
  public void unpop()
  {
    ...
    if (topMost != null) {
      ...
      if (topMost.focusOnPopup instanceof totalcross.ui.MenuBar) {
        topMost.focusOnPopup =
          topMost; // make sure that the focus is not on the closed menu bar
      } else if (topMost.focusOnPopup instanceof totalcross.ui.Window &&
                 !topMost.focusOnPopup.asWindow.isVisible()) {
      ...
      lastTopMost = topMost;
      topMost.focusOnPopup.postEvent(
        new ControlEvent(ControlEvent.WINDOW_CLOSED,
                         this)); // tell last control that we closed
      if (topMost == lastTopMost) {
        topMost.setFocus(topMost.focusOnPopup);
      }
      postUnpop();
      if (topMost.focusOnPopup != null &&
          !(topMost.focusOnPopup instanceof TextControl) &&
          Settings.virtualKeyboard && Window.isSipShown()) {
   ...
   * </pre>
   */
  @ReplacedByNativeOnDeploy public static void pumpEvents()
  {
  ...
  @Override public boolean isTopMost() { return this == topMost; }

  ////////////////////////////////////////////////////////////////////////////////////
  /** Returns the topmost window */
  static public Window getTopMost() { return topMost; }

  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Placeholder called imediatly before the popup began. The default
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Placeholder called after the popup is done and after the repaint of this
   * window. The default implementation does nothing.
   */
  protected void postPopup() {}

  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Placeholder called imediatly before the unpop began. The default
   * implementation does nothing.
   */
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Placeholder called after the unpop is done and after the repaint of the
   * other window. The default implementation does nothing.
   */
  ...
  public void setMenuBar(Control menubar) { this.menubar = menubar; }
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * This method repaints all windows from bottom to top. It is automaticaly
   * used in the unpop method.
   * @since SuperWaba 2.0 beta 4
   */
  protected void loadBehind() { repaintActiveWindows(); }

  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * This method is for internal use only. calling it directly will cause the
   * VM to crash.
   */
  public static void destroyZStack() { zStack.removeAllElements(); }

  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Used to swap containers from the Window. To restore the first
   * container, pass null as parameter. Note that null cannot be used for the
   * first swap. See samples/ui/ContainerSwitch. This method correctly handles
   * screen rotation for containers that were already swapped.
   */
  public void swap(Container newContainer)
  { // guich@300_57: make easier to swap containers
    ...
    Control firstTarget =
      (_focus != null && _focus.getParentWindow() == this)
        ? _focus
        : newContainer.tabOrder.size() > 0
            ? (Control)newContainer.tabOrder.items[0]
            : newContainer; // guich@573_19: set focus to the first
                            // control, instead of the new container. -
                            // guich@tc100: only if the focus was not
                            // already set in the initUI method of the
                            // newContainer
    if (Toast.cnt != null) {
      try {
    ...
    if (repaintOnSwap) {
      newContainer.repaintNow(); // guich@503_7: fixed problem when this swap
                                 // was being called from inside a Menu.
    }
    firstTarget.requestFocus(); // guich@tc153: put this after repaintNow to fix
                                // transition effect problems
    topMost.focusOnPopup =
      firstTarget; // guich@550_15: otherwise, the ContainerSwitch app won't
                   // work for Sub3 when using pen less.
  ...
  @Override public int getPreferredWidth()
  {
    ...
    int wborder =
      (borderStyle == NO_BORDER) ? 0 : (borderStyle == ROUND_BORDER ? 4 : 2);
  ...
  public static void repaintActiveWindows()
  {
    ...
    try {
      ...
      Rect mainWindowRect =
        MainWindow.mainWindowInstance.getRect(); // size of the MainWindow
      ...
      for (n = zStack.size(); i < n;
           i++) { // repaints every window, from the nearest with the MainWindow
                  // size to last parent
        if (i == lastFade) {
      ...
      if (neededPaint) {
        ...
        if (topMost._focus != null &&
            topMost._focus.getParentWindow() == topMost) {
          topMost._focus
            .onWindowPaintFinished(); // guich@200b4: test if the last focused
                                      // control belongs to this window; this
                                      // corrects the painted control after a
                                      // window is poped up
        }
    ...
    // guich@tc125_18: there's no need to paint the highlight here because it
    // was already painted in the repaintNow() method called above.
  ...
  { // guich@512_1: transfer focus on tab keys - fdie@550_15 :
    // transfer also on arrow keys
    if (isHighlighting) {
      ...
      if (isAction && highlighted != null) {
        highlighted.requestFocus();
        isHighlighting =
          false; // kmeehl@tc100: set isHighlighting after requesting focus so
                 // that the focus_out will not set it back
        ...
        if (c == null || c == this) {
          ...
          p = (c.asContainer != null && c.asContainer.tabOrder.size() > 0)
                ? c.asContainer
                : c.parent;
        }
        if (p == null) {
        ...
        if (highlighted == null && firstFocus != null &&
            !firstFocus.focusLess && firstFocus.focusTraversable &&
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  /**
   * Sets the currently highlighted control which will gain focus if the ACTION
   * button is pressed. It may be the same of the one that holds focus.
   * @since SuperWaba 5.5
   */
  public void setHighlighted(Control c)
  {
    if (Settings.keyboardFocusTraversable &&
        c != highlighted) { // kmeehl@tc100: removed check for children.
                            // focusTraversable is now a reliable way to tell if
                            // a control should be focusable and highlightable;
                            // check if c is null
      if (highlighted != null) {
        _controlEvent.type = ControlEvent.HIGHLIGHT_OUT;
        _controlEvent.update(highlighted);
      ...
      if (c != null && c.focusTraversable) {
        ...
        c.postEvent(_controlEvent); // kmeehl@tc100: send the currently
                                    // highlighted control a HIGHLIGHT_IN event
        highlighted = c;
      }
      if (Settings.isOpenGL) {
        needsPaint = true;
   ...
   * @since SuperWaba 5.5
   */
  public Control getHighlighted() { return highlighted; }
   ...
   * before the window is dismissed.
   * @since SuperWaba 5.5
   */
  public void popupNonBlocking() { topMost.popupNonBlocking(this); }
   ...
   * @since SuperWaba 5.5
   */
  public void popup() { topMost.popup(this); }
   ...
   * @since SuperWaba 5.8
   */
  @Override public void postPressedEvent()
  { // guich@580_27
  ...
  public void screenResized()
  {
    ...
    requestFocus();
    enableUpdateScreen =
      true;        // if resize occured in an edit, remove the focus from it.
    rTitle = null; // guich@tc120_37
    reposition();
    MainWindow.getMainWindow().runOnMainThread(new Runnable() {
      @Override public void run() { repaintActiveWindows(); }
    });
  }

  ////////////////////////////////////////////////////////////////////////////////////
  /**
   *  Walks up from control to each parent giving each focus handler an
   * opportunity to handle the KeyEvent. The Control passed in is given a chance
   * to handle the focus change key regardless of its status as a focus handler.
   * @return The next control which should get focus, or null.
   */
  private Control bubbleHandleFocusChangeKey(Control c, KeyEvent ke)
  { // kmeehl@tc100
  ...
  void drawHighlight(Control c)
  {
    ...
    if (c != null && c.visible && c.isDisplayed() &&
        Settings.keyboardFocusTraversable &&
        c.asWindow == null) { // guich@tc112_34: don't draw highlight of a
                              // Window (as in MenuBarDropDown)
  ...
  private void drawHighlight(Control c, boolean highlighted)
  {
    ...
    if (g != null) {
      int offset = 0;
      int x = -n - 1, y = -n - 1, w = c.width + n + n + 2,
          h = c.height + n + n + 2;
      ...
      if (highlighted) {
        ...
        int old = g.foreColor; // since the graphics is now shared among all
                               // controls, save and restore the fore color
        while (n-- > 0) {
  ...
  /**
   * This method resizes the Window to the needed bounds, based on added
   * childs. It changes only the size, keeping the x,y coordinates passed on
   * setRect. You can add spaces at right and bottom using the
   * insets.right/bottom properties. Sample: <pre> public class TestWindow
   * extends Window
   * {
   *    public TestWindow()
   *    {
   *       ... call constructor
   *       setRect(CENTER,CENTER,Screen.width-40,1000); // height will be
   * resized later
   *    }
   *    public void initUI()
   *    {
   ...
   * }
   * </pre>
   * In this sample, since the height is variable, you can't use BOTTOM on
   * <code>y</code> nor FILL on <code>height</code>, otherwise resize will not
   * work as expected. Same counts if you have a variable width.
  ...
  { // guich@tc114_53
    ...
    setRect(setX,
            setY,
            maxX + insets.right + hborder / 2,
            maxY + insets.bottom + hborder / 2);
  }

  @Override public void resizeWidth()
  {
    int maxX = 0;
    for (Control child = children; child != null; child = child.next) {
      maxX = Math.max(maxX, child.x + child.width);
  ...
  @Override public void resizeHeight()
  {
    int maxY = 0;
    for (Control child = children; child != null; child = child.next) {
      maxY = Math.max(maxY, child.y + child.height);
  ...
  {
    double threshold =
      (Settings.fingerTouch ? DEFAULT_DRAG_THRESHOLD_IN_INCHES_FINGER
                            : DEFAULT_DRAG_THRESHOLD_IN_INCHES_PEN) *
      (Settings.screenWidthInDPI + Settings.screenHeightInDPI) / 2;
    return (int)Math.round(threshold);
  }

  /**
   * Returns the number of windows that are popped up. If there's only a
   * MainWindow, returns 0.
   ...
   * Don't call this method directly, use Settings.deviceRobotSpecialKey
   * instead.
  ...
  public void shiftScreen(Control c, int deltaY)
  {
    if (c == null) { // unshift the screen?
      ...
      shiftY = shiftH = 0;
      boolean wasPenEvent =
        PenEvent.PEN_DOWN <= lastType && lastType <= PenEvent.PEN_DRAG;
      ...
      if (newShiftY != shiftY) {
        ...
        shiftH = Settings.onJavaSE
                   ? Settings.screenHeight / 2
                   : (1 + 1 + 2) * c.fmH; // one line above and two below
                                          // control, plus control's line
      }
      needsPaint = true;
    }
  }

  public static boolean isScreenShifted() { return shiftY != 0; }

  public static boolean isSipShown() { return false; }
