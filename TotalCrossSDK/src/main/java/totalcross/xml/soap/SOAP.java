 ...
 * try
 * {
 *    Convert.setDefaultConverter("UTF8"); // disable this if you don't need to
 * support UNICODE SOAP s = new SOAP(&quot;getClientStatus&quot;,
 * &quot;http://localhost:8080/axis/ClientHandler.jws&quot;);
 *    s.setParam(&quot;João Pedro&quot;, &quot;name&quot;);
 *    s.setParam(&quot;227887227-2&quot;, &quot;id&quot;);
 *    s.execute();
 *    String status = (String) s.getAnswer();
 *    Convert.setDefaultConverter(""); // disable this if you don't need to
 * support UNICODE
 * }
...
{ // guich@570_34
  ...
  /**
   * The SOAP request will ask the server for GZip or ZLib encoded response by
   * default.<br> To disable encoding, set this field to true.
   */
  public static boolean
    disableEncoding; // flsobral@tc110_72: SOAP will now ask the server for
                     // encoded content by default. (either gzip or zlib)
  ...
  /**
   * A flag that indicates if the SOAP connection was using either GZip or
   * ZLib. This is a ready-only flag, set during the execute method, and
   ...
   * receives the basic parameters (URI, method, and vector to receive the
   * answer) and initialize these attributes. Now the user can create a SOAP
   * object, sets its parameters and then call execute() to handle the SOAP
   * request. There are setParam methods that receives only the parameter value
   * and others that receives the parameter value and parameter name. This is
  ...
  private class ParseAnswer extends XmlTokenizer
  {
    ...
    private StringBuffer sb =
      new StringBuffer(2048); // guich@571_12: now all references and characters
                              // are appended until an end-tag is found

    private boolean isReturn(String tag)
    {
      // flsobral@tc100b5: split the test in three, so we can compare
      // alternativeReturnTag with tag before creating a new instance in lower
      // case to compare with the default.
      if (tag == null) {
        return false;
      }
      if ((alternativeReturnTag != null &&
           tag.endsWith(alternativeReturnTag))) {
        return true;
      }
      String lowerTag = tag.toLowerCase(); // flsobral@tc100b5_44: comparison
                                           // was case sensitive.
      return (lowerTag.endsWith("return") || lowerTag.endsWith("result"));
    }

    public ParseAnswer(Stream stream)
      throws SyntaxException, totalcross.io.IOException
    {
      tokenize(stream);
    }

    @Override public void foundCharacter(char charFound)
    {
      sb.append(charFound);
    }
    ...
    {
      String tag = lastTag = new String(Convert.charConverter.bytes2chars(
        buffer,
        offset,
        count)); // flsobral@tc100b5_46: all String constructors now use the
                 // CharacterConverter.bytes2chars.
    ...
    @Override public void foundEndTagName(byte buffer[], int offset, int count)
    {
      if (sb.length() >=
          0) { // flsobral@tc100: replaced > by >= so empty tags are not ignored
        if (isInReturn) {
          storeAnswer();
        }
        sb.setLength(0);
    ...
    @Override public void foundEndEmptyTag()
    { // case <ValidateUserResult />
      if (isReturn(lastTag)) {
        isInReturn = false;
      }
    }
    ...
    public void foundAttributeName(byte buffer[], int offset, int count)
    {
      if (isInReturn) {
        String tag =
          new String(Convert.charConverter.bytes2chars(buffer, offset, count));
      ...
      switch (type) {
        ...
        case BOOLEAN_TYPE:
          if (answer == null) {
            ...
            // otherwise, more than one answer was found; create a vector and
    ...
    public void foundCharacterData(byte buffer[], int offset, int count)
    {
      if (errorReasonState == 2) { // guich@582_1: get the error message
        ...
        int i = offset + count;
        while (buffer[i] != '<') {
          // ok, here we'll trick the parser: find where the message ends. this
          // avoid problems when the message has strange chars on it. it is
          // safe, since the message is below 1k, the size of the buffer
          i++;
        }
        errorReason = new String(
          Convert.charConverter.bytes2chars(buffer, offset, i - offset));
      }
      if (isInReturn) {
        sb.append(
          new String(Convert.charConverter.bytes2chars(buffer, offset, count)));
  ...
  public static String suffix = "</soapenv:Body>"
                                + "</soapenv:Envelope>";
   ...
   * will be used, along with an open timeout of 25 seconds, and a read and
   * write timeout of 60 seconds.
   *
   * @param mtd
  ...
  {
    setParam(
      Convert.toString(param), DEFAULT_PARAM_NAME + paramIndex, "boolean");
  }
   ...
   * will be <code>paramType</code> and the name is <code>paramName</code>.
   * Important: unicode characters are not accepted because the default header
   * uses UTF-8.
   *
   * @param param
   * @param paramName
   ...
   * it as <code>arg+index</code>. Important: unicode characters are not
   * accepted because the default header uses UTF-8.
   *
   * @param param
   */
  public void setParam(String[] param)
  ...
  /**
   * Sets a String array parameter in the order of the method call, identifying
   * it as <code>paramName</code>. Important: unicode characters are not
   * accepted because the default header uses UTF-8.
   *
   * @param param
   * @param paramName
  ...
  /**
   * Sets an object param identifying it as <code>paramName</code>. The object
   * fields names and values must be informed as the String arrays <fieldNames>
   * and <fieldValues>. Important: unicode characters are not accepted because
   ...
   * @param fieldValues
   */
  public void setObjectParam(String paramName,
                             String[] fieldNames,
                             String[] fieldValues)
  {
    StringBuffer sb = sbuf;
  ...
  public void setObjectArrayParam(String paramName,
                                  String[] fieldNames,
                                  Vector fieldValues)
  {
    StringBuffer sb = sbuf;
  ...
  /**
   * Set the proxy settings to be used by this SOAP connection. You may
   * optionally set the username and password for basic proxy authorization.<br>
   ...
   *           the proxy port
   * @param username
   *           the username for basic proxy authorization. Passing a null value
   * disables proxy authorization.
   * @param password
   *           the password for basic proxy authorization. Passing a null value
   * disables proxy authorization.
   *
   * @since TotalCross 1.27
   */
  public void useProxy(String address,
                       int port,
                       String username,
                       String password)
  {
  ...
  public void execute() throws SOAPException
  {
    try {
      ...
      if (!disableEncoding) {
        httpOptions.postHeaders.put(
          "Accept-Encoding",
          "deflate;q=1.0, gzip;q=0.5"); // flsobral@tc110_77: zlib encoding is
                                        // preferred over gzip encoding.
      }
      httpOptions.postHeaders.put("Content-Type", "text/xml; charset=utf-8");
      httpOptions.postHeaders.put(
        "SOAPAction",
        "\"" + namespace + (!namespace.endsWith("/") ? "/" : "") + mtd +
          "\""); // flsobral@tc100b5_48: only add a trailing slash if the
                 // namespace does not have one already.
      httpOptions.postPrefix =
        "<?xml version=\"1.0\" encoding=\"" + httpOptions.getCharsetEncoding() +
        "\"?>" + prefix +
        (namespaceId == null
           ? "<" + mtd + " xmlns=\"" + namespace + "\">"
           : "<" + namespaceId + ":" + mtd + " xmlns:" + namespaceId + "=\"" +
               namespace + "\">"); // guich@tc123_39: don't concatenate the args
                                   // with the prefix and suffix
      httpOptions.postDataSB = sbuf;
      httpOptions.postSuffix =
        namespaceId == null ? "</" + mtd + ">" + suffix
                            : "</" + namespaceId + ":" + mtd + ">" + suffix;
      if (debug) {
        Vm.debug("post: " + httpOptions.postPrefix + httpOptions.postDataSB +
                 httpOptions.postSuffix);
      }
      ...
      if (hs.contentEncoding != null) {
        ...
        if (hs.contentEncoding.equalsIgnoreCase("deflate")) {
          ...
          GZipStream zs = new GZipStream(
            hs,
            CompressedStream.INFLATE); // flsobral@tc112_35: Better performance
                                       // with GZipStream instead of GZip.
          receivedStream = zs;
          wasCompressionUsed = true;
        } else {
          throw new SOAPException("Unsupported encoding: " +
                                  hs.contentEncoding);
        }
      } else {
        // flsobral@tc110_73: Use ByteArrayStream if the content is already
        // encoded, if the length is unknown, or if the length is known and
        // lower than 70k. (zlib requires at least 65k to run, so we'll only use
        // it when reading more than 70k.)
        boolean useBBAS =
          (hs.contentLength >= -1 && hs.contentLength <= 70000) ||
          Vm.getFreeMemory() < 1024 * 1024;
        if (useBBAS) {
      ...
      if (debug) {
        ...
        if (receivedStream instanceof ByteArrayStream) {
          ((ByteArrayStream)receivedStream)
            .mark(); // flsobral@tc110_73: use mark instead of reset.
        } else {
          ((CompressedByteArrayStream)receivedStream)
            .setMode(CompressedByteArrayStream
                       .READ_MODE); // reset the buffer so it can be re-read
        }
      }
      ...
      if (!ok && hs.responseCode == 500) {
        throw new SOAPException(
          "Error 500 on HttpStream. Reason: " +
          (errorReasonState == 3
             ? EscapeHtml.unescape(errorReason)
             : "could not detect error in response")); // guich@tc114_9:
                                                       // unescape the error
                                                       // message
      }
