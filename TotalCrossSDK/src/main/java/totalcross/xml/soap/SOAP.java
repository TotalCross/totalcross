...
public class SOAP { // guich@570_34
	...
	disableEncoding; // flsobral@tc110_72: SOAP will now ask the server for encoded content by default. (either gzip or zlib)
	...
	private class ParseAnswer extends XmlTokenizer {
		...
		private StringBuffer sb = new StringBuffer(
			2048); // guich@571_12: now all references and characters are appended until an end-tag is found
		...
		public void foundStartTagName(byte buffer[], int offset, int count) {
			String tag = lastTag = new String(Convert.charConverter.bytes2chars(buffer, offset,
											  count)); // flsobral@tc100b5_46: all String constructors now use the CharacterConverter.bytes2chars.
	...
	public void setParam(String param, String paramName, String paramType) {
		sbuf.append('<').append(paramName).append(" xsi:type=\"xsd:").append(
			paramType).append("\">").append(param)
	...
	public void setParam(String[] param, String paramName, String paramType) {
		...
		sb.append('<').append(paramName).append(" soapenc:arrayType=\"xsd:").append(
			paramType).append("[]\" ")
		.append("xsi:type=\"soapenc:Array\" xmlns:soapenc=\"http://schemas.xmlsoap.org/soap/encoding/\">");
		for (int i = 0; i < len; i++) {
			sb.append('<').append(paramType).append(" xsi:type=\"xsd:").append(paramType).append("\">").append(
				param[i])
	...
	public void execute() throws SOAPException {
		try {
			...
			if (!disableEncoding) {
				httpOptions.postHeaders.put("Accept-Encoding",
											"deflate;q=1.0, gzip;q=0.5"); // flsobral@tc110_77: zlib encoding is preferred over gzip encoding.
			}
			httpOptions.postHeaders.put("Content-Type", "text/xml; charset=utf-8");
			httpOptions.postHeaders.put("SOAPAction",
										"\"" + namespace + (!namespace.endsWith("/") ? "/" : "") + mtd +
										"\""); // flsobral@tc100b5_48: only add a trailing slash if the namespace does not have one already.
			httpOptions.postPrefix = "<?xml version=\"1.0\" encoding=\"" + httpOptions.getCharsetEncoding() +
									 "\"?>" + prefix
									 + (namespaceId == null ? "<" + mtd + " xmlns=\"" + namespace + "\">"
										: "<" + namespaceId + ":" + mtd + " xmlns:" + namespaceId + "=\"" + namespace +
										"\">"); // guich@tc123_39: don't concatenate the args with the prefix and suffix
			...
			if (hs.contentEncoding != null) {
				...
				if (hs.contentEncoding.equalsIgnoreCase("deflate")) {
					...
					GZipStream zs = new GZipStream(hs,
												   CompressedStream.INFLATE); // flsobral@tc112_35: Better performance with GZipStream instead of GZip.
					receivedStream = zs;
					wasCompressionUsed = true;
				} else {
				...
				boolean useBBAS = (hs.contentLength >= -1 && hs.contentLength <= 70000)
			...
			if (debug) {
				...
				if (receivedStream instanceof ByteArrayStream) {
					...
					((CompressedByteArrayStream) receivedStream).setMode(
						CompressedByteArrayStream.READ_MODE); // reset the buffer so it can be re-read
				}
			}
			...
			if (!ok && hs.responseCode == 500) {
				throw new SOAPException("Error 500 on HttpStream. Reason: "
										+ (errorReasonState == 3 ? EscapeHtml.unescape(errorReason) :
										   "could not detect error in response")); // guich@tc114_9: unescape the error message
			}
