 ...
 * takes a byte[] array with offset and count; another one for an HTML document
 * which is embedded within an HTTP stream; and the last takes a (byte) Stream.
 * <P>
 ...
 *       {
 *          Vm.debug(&quot;StartTagName: &quot; + new String(buffer, offset,
 * count));
 *       }
 *
 *       public void foundEndTagName(byte buffer[], int offset, int count)
 *       {
 *          Vm.debug(&quot;EndTagName: &quot; + new String(buffer, offset,
 * count));
 *       }
 *
 ...
 *       {
 *          Vm.debug(&quot;AttributeName: &quot; + new String(buffer, offset,
 * count));
 *       }
 *
 ...
 * byte dlm)
 *       {
 *          Vm.debug(&quot;AttributeValue: &quot; + new String(buffer, offset,
 * count));
 *       }
 *
 ...
 *    {
 *       String input =
 * &quot;&lt;p&gt;Hello&lt;i&gt;World!&lt;/i&gt;&lt;/p&gt;&quot;; MyXmlTokenizer
 * xtk = new MyXmlTokenizer(); try
 *       {
...
public class XmlTokenizer
{
  ...
  protected XmlTokenizer() { resolveCharRef = true; }
   ...
   */
  public final void tokenize(byte input[], int offset, int count)
    ...
    while (ofsCur < ofsEnd) {
      ...
      if (ofsEnd == buffer.length) {
        ...
        if (ofsStart > 0) {
          ...
          int newSize = oldBuffer.length * 15 /
                        10; // guich@510_17: instead of double, grow 50%...
   ...
   *         reported event.
   */
  public final int getAbsoluteOffset() { return ofsStart + readPos; }
   ...
   * Character Data)
   */
  public final boolean isDataCDATA() { return (endTagToSkipTo != null); }
   ...
   *           constructs.
   */
  public final void setStrictlyXml(boolean toSet) { strictlyXml = toSet; }
   ...
   * have been reported. The implied tag name is the one of the starting tag
   * (<i>e.g.:</i> the most recently reported start tag.) <P> Derived class may
   * override this method. <P> Example: <PRE>
   *
   *   &lt;FOO A=B&gt; generates:
  ...
  /**
   * Method called when a character has been found in the contents, which is
   * resulting from a character reference resolution. <P> Derived class may
   ...
   * been called first, with <code>disable</code> set to
   * <code>true</code>.&nbsp; If not, then <code>foundReference</code> is
   * <U>never called</U>, and
            ...
            case 1:
              if (ch == '>') {
                foundProcessingInstruction(
                  input, ofsStart + 2, ofsCur - ofsStart - 3);
                ofsStart = ofsCur;
                state = 0;
              } else {
                substate = 0;
   ...
   *           number of bytes of the reference
   */
  private void tellReference(byte[] input, int offset, int count)
    throws SyntaxException
  {
    if (resolveCharRef) {
