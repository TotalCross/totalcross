...
public class XmlReader extends XmlTokenizer {
	...
	protected void foundDeclaration(byte[] input, int offset, int count) {
		...
		if (count > 7
			&& new String(totalcross.sys.Convert.charConverter.bytes2chars(input, offset,
						  7)).equals("[CDATA[")) {
	...
	private void storeData(byte input[], int offset, int count, boolean stripLeadingSpaces) {
		if (newlineSignificant > 0) {
			...
			if (count > 0) {
				pcdata.append(totalcross.sys.Convert.charConverter.bytes2chars(input, offset,
							  count)); // kcchan@554_39
			}
		} else {
			int from = offset - 1;
			...
			while (--count > 0) {
				if ((input[++from] & 0xFF) > ' ') {
					...
					pcdata.append(totalcross.sys.Convert.charConverter.bytes2chars(input, fromOrig,
