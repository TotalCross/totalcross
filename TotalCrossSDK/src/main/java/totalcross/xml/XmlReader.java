...
public class XmlReader extends XmlTokenizer
{
  ...
  /**
   * Set to true if you want the get/set methods of the AttributeList to be
   * case insensitive.
   */
   ...
   */
  public AttributeList.Filter setAttributeListFilter(
    AttributeList.Filter filter)
  {
   ...
   */
  public ContentHandler getContentHandler() { return cntHandler; }
   ...
   * @exception SyntaxException
   */
  public final void parse(byte[] input, int offset, int count)
   ...
   * as the contents of the &lt;PRE&gt; tag.&nbsp; When the parsing process
   * starts, newlines are not significant.&nbsp; Hence, setNewLineSignificant
   * must be called <b>after</b> the parsing has started.&nbsp; For example, to
   * make all newlines significant:
   *
   * <PRE>
   ...
   * setNewlineSignificant(false); // newlines are still significant - stack is
   * 1 setNewlineSignificant(false); // newlines are no more significant again -
   * stack is 0
   *
   *
   * </PRE>
   ...
   */
  @Override public void foundStartTagName(byte buffer[], int offset, int count)
  {
    switch (state) {
   ...
   * Override of XmlTokenizer
   */
  @Override public final void foundEndEmptyTag()
  {
    reportStartTag();
  ...
  public final void foundCharacterData(byte buffer[], int offset, int count)
  {
    ...
    storeData(buffer,
              offset,
              count,
              (state == START_TAG_STATE) || (state == SPRING_STATE));
   ...
   */
  @Override public final void foundCharacter(char charFound)
  {
    if (state == START_TAG_STATE) {
      reportStartTag();
    }
  ...
  /** Override of XmlTokenizer */
  @Override public final void foundComment(byte buffer[], int offset, int count)
  {
    switch (state) {
      case START_TAG_STATE:
        reportStartTag();
  ...
  /** Override of XmlTokenizer */
  @Override public final void foundEndOfInput(int count)
  {
    switch (state) {
      case START_TAG_STATE:
        reportStartTag();
   ...
   */
  @Override protected void foundDeclaration(byte[] input, int offset, int count)
  {
    flushAttribute();
    ...
    if (count > 7 &&
        new String(
          totalcross.sys.Convert.charConverter.bytes2chars(input, offset, 7))
          .equals("[CDATA[")) {
      cntHandler.cdata(
        tagNameHashId,
        new String(totalcross.sys.Convert.charConverter.bytes2chars(
          input, offset + 7, count - 9)));
    }
  }
   ...
   * <PRE>
   * Examples: &quot; AB\r\nC &quot; -&gt; &quot; AB C &quot; with
   * stripLeadingSpace set to false: &quot; AB\r\nC &quot; -&gt; &quot;AB C
   * &quot; with stripLeadingSpace set to true; &quot;A\tB CD&quot; -&gt;
   * &quot;A B CD&quot;
   * </PRE>
   *
   * @param input
   ...
   */
  private void storeData(byte input[],
                         int offset,
                         int count,
                         boolean stripLeadingSpaces)
  {
    if (newlineSignificant > 0) {
      ...
      if (count > 0) {
        pcdata.append(totalcross.sys.Convert.charConverter.bytes2chars(
          input,
          offset,
          count)); // kcchan@554_39
      }
   ...
   *           if false, the data will be followed by a space had the source
   * data ended with one or more white spaces; if true, the data ends at the
   ...
   * <P>
   * Note that an attribute name not followed by a value assignment (Ex:
   * <code>&gt;dl compact&gt;</code> is perfectly legal HTML/SGML.
   */
