...
public class XmlTokenizer4D
{
  ...
  {
    substate += ixEndTagToSkipTo + quote + (strictlyXml ? 0 : 1) +
                (resolveCharRef ? 0 : 1); // remove warnings
    resolveCharRef = true;
    nativeCreate();
  }
  ...
  native public static char resolveCharacterReference(byte input[],
                                                      int offset,
                                                      int count);
