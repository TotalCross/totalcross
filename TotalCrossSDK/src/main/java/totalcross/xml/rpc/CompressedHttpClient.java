...
public class CompressedHttpClient extends StandardHttpClient {
	 ...
	 */
	public CompressedHttpClient(String hostname, int port, String uri, int openTimeout, int readTimeout,
								int writeTimeout)
	throws XmlRpcException, UnknownHostException {
		super(hostname, port, uri, openTimeout, readTimeout, writeTimeout);
	}

	public CompressedHttpClient(String hostname, int port, String uri) throws XmlRpcException,
	...
	private Object readCompressedResponse(boolean asBytes) throws XmlRpcException, IOException {
		...
		if (contentRead != contentLength) {
			throw new XmlRpcException("Could not read answer: " + contentRead + " is less than " +
									  contentLength);
		}
	...
	private Object privateExecute(byte[] requestBody, boolean asBytes) throws XmlRpcException {
		try {
			...
			return inflateResults ? readCompressedResponse(asBytes) : asBytes ? readResponseBytes() :
				   readResponse();
		} catch (IOException e) {
