...
public class CompressedHttpClient extends StandardHttpClient
{
  ...
  public CompressedHttpClient(String hostname, int port, String uri)
    throws XmlRpcException, UnknownHostException
  {
    super(hostname, port, uri);
  }

  private Object readCompressedResponse(boolean asBytes)
    throws XmlRpcException, IOException
  {
    ...
    if (contentRead != contentLength) {
      throw new XmlRpcException("Could not read answer: " + contentRead +
                                " is less than " + contentLength);
    }
    ByteArrayStream is = new ByteArrayStream(contentBuff);
    ByteArrayStream os = out;
    os.reset();
    int inflateSize = ZLib.inflate(is, os);
    return asBytes ? (Object)os.toByteArray()
                   : new String(os.getBuffer(), 0, inflateSize);
  }
   ...
   */
  @Override public String execute(byte[] requestBody) throws XmlRpcException
  {
  ...
  private Object privateExecute(byte[] requestBody, boolean asBytes)
    throws XmlRpcException
  {
    try {
      // compress the body
      ...
      // read the answer
      boolean inflateResults =
        "deflate".equals(htHeader.get("content-encoding"));
      return inflateResults ? readCompressedResponse(asBytes)
                            : asBytes ? readResponseBytes() : readResponse();
    } catch (IOException e) {
  ...
  /**
   * Writes the headers for a HTTP request, adding the deflate method as
   * content-encoding.
   */
  @Override protected StringBuffer writeRequestHeader(int requestLength)
  {
