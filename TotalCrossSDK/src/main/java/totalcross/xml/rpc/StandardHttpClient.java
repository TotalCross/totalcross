...
public class StandardHttpClient
{
  ...
  /**
   * Sets the keep-alive flag. If this is false, the connection to the server
   * will be closed at each execution.
   */
   ...
   */
  public StandardHttpClient(String hostname,
                            int port,
                            String uri,
                            int openTimeout,
                            int readTimeout,
   ...
   *            If the connection to the server could not be made
   */
  public StandardHttpClient(String hostname, int port, String uri)
    throws UnknownHostException, XmlRpcException
  {
    this(hostname,
         port,
  ...
  protected void writeRequest(byte[] requestBody, int len) throws IOException
  {
    ...
    do {
      writeRet = socket.writeBytes(
        requestHeader, 0 + written, requestHeaderLen - written);
      if (writeRet > 0) {
        written += writeRet;
      }
    ...
    written = 0;
    do {
      writeRet = socket.writeBytes(requestBody,
                                   0 + written,
                                   len - written); // write the given body
      if (writeRet > 0) {
        written += writeRet;
      }
    } while (written < len);
  ...
  protected void checkResponse() throws XmlRpcException, IOException
  {
    ...
    if (!"200".equals(statusCode)) {
      throw new XmlRpcException("Unexpected Response from Server: " +
                                statusMsg);
    }
  }
  ...
  protected void parseHeader() throws IOException
  {
    ...
    keepAlive &=
      ht.exists("keep-alive") || "keep-alive".equals(ht.get("connection"));
    htHeader = ht;
  }

  /** Used internally by readResponse and readResponseBytes. */
  protected StringBuffer privateReadResponse()
   ...
   * @param returnAsBytes If true, the result can be casted to
   *     <code>byte[]</code>, otherwise, it can be casted to
   *     <code>String</code>.
   * @throws IOException
   */
