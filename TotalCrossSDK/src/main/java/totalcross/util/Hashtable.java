 ...
 *        // "two = " + Convert.toInt(n);
 * </pre>
 * This Hashtable class does not support Generics; use the HashMap class
 instead.
...
public class Hashtable
{
  ...
  /**
   * Computes the number of collisions for a set of inserts. You must zero this
   * each time you want to compute it. Here's a sample of how to determine the
   * best values. Keep in mind that the lower collisions is better, but don't
  ...
  /**
   * Setting this to true will allow the hashtable to have more than one key
   * with the same value. In this case, the methods will always return the first
   * matching key.
   ...
   *
   * @param initialCapacity The number of elements you think the hashtable will
   end with. The hashtable will grow if necessary, but using
   * a number near or above the final size can improve performance.

   */
  public Hashtable(int initialCapacity) { init(initialCapacity, 0.75f); }
   ...
   * tcz file. <br> Now, when your program starts, you can do: <pre> String txt
   * = idiom == EN ? "EN.txt" : "PT.txt"; byte[] b = Vm.getFile(txt); Hashtable
   * res = new Hashtable(new String(b,0,b.length)); new
   * MessageBox(res.get("Message"), res.get("TestMsg"), new
   * String[]{res.get("Exit")}).popupNonBlocking();
   * </pre>
   * Note that the keys are <i>case sensitive</i>, and that all strings are
   * trimmed.
   *
   ...
   * <ol>
   * <li> An Object array (<code>Object[]</code>). In this case, the number of
   * keys and values must match. <li> A single Object. This object is set as
   * value to all keys.
   * </ol>
  ...
  { // guich@tc114_27
    ...
    initialCapacity = (int)(initialCapacity / loadFactor +
                            1); // guich@tc100: since most users just pass the
                                // number of element, compute the desired
                                // initial capacity based in the load factor
  ...
  /**
   * Returns the value to which the specified key is mapped in this hashtable as
   * a String. If the item is a String, a cast is made, otherwise, the toString
   * method is called.
   * @param   key   a key in the hashtable.
   * @return  the value to which the key is mapped in this hashtable;
   *          <code>null</code> if the key is not mapped to any value in
   ...
   * @since TotalCross 1.15
   */
  public Object get(int hash, Object defaultValue)
  { // guich@tc115_47
    Object r = get(hash);
   ...
   * <p>
   * <b>Caution</b>: since you're passing an integer instead of an object, if
   * there are two objects that map to the same key, this method will always
  ...
  /**
   * Return a Vector of the keys in the Hashtable. The order is the same of the
   * getValues method.
   * @see #getValues
   * @see #getKeyValuePairs
   */
  public Vector getKeys() { return getKeysValues(GET_KEYS, null); }

  /**
   * Return a Vector of the values in the Hashtable. The order is the same of
   * the getKeys method.
   * @since SuperWaba 5.1
   * @see #getKeys
   * @see #getKeyValuePairs
  ...
  /**
   * Return a Vector with pairs in the form <code>key=value</code> from the
   * Hashtable. Each vector's element can safely be casted to a String.
   * @param separator the separator between the key and the value. Should be ":
   *     ","=", etc.
   * @since SuperWaba 5.1
   */
  ...
  {
    // dgecawich 5/16/01 - fix so that all keys are returned rather than just
    // the last one the sympton for this was that getCount() always returned 1
    // regardless of how many items were added
    Object[] v = new Object[count];
    if (table != null) {
      for (int i = 0, n = 0; i < table.length; i++) {
        for (Entry entry = table[i]; entry != null; entry = entry.next) {
          v[n++] = getType == GET_KEYS
                     ? entry.key
                     : getType == GET_VALUES ? entry.value
                                             : (entry.key + sep + entry.value);
        }
      }
    }
    return new Vector(v);
   ...
   * Note that the target Hashtable is not cleared; you should do that by
   * yourself.
   * @since TotalCross 1.15
   */
  public void copyInto(Hashtable target)
  { // guich@tc115_17
    if (table != null) {
  ...
  public Object put(Object key, Object value)
  {
    ...
    if (value == null) {
      throw new NullPointerException(
        "Argument 'value' cannot have a null value");
    }
    int hash =
      key.hashCode(); // flsobral@tc100b4_23: this operation throws NPE if key
                      // is null, no need to explicitly test that.
   ...
   * with a key that is equal to the original key.
   * <p>
   * This method receives a hashcode instead of the object. You MUST use the
   * get(int) method to retrieve the value, otherwise you will get a
   * NullPointerException, because no key is stored using this method.
   *
  ...
  protected void rehash()
  {
    ...
    int newCapacity =
      (((oldCapacity << 1) + oldCapacity) >> 1) +
      1; // guich@120 - grows 50% instead of 100% - guich@200b4_198: added
         // Peter Dickerson and Andrew Chitty changes to correct the
         // optimization i made with << and >>
  ...
  { // flsobral@tc100b4: Added method remove(int hash)
    Entry tab[] = table;
    int index = (hash & 0x7FFFFFFF) % tab.length;
    for (Entry e = tab[index], prev = null; e != null; prev = e, e = e.next) {
      if (e.hash == hash) {
   ...
   */
  public StringBuffer dumpKeysValues(StringBuffer sb,
