...
public class Vector {
	 ...
	 */
	public void setSize(int newSize) throws
		ArrayIndexOutOfBoundsException { // flsobral@tc112: Throw AIOOBE instead of IllegalArgument to match the Java implementation.
		if (newSize < 0) {
	 ...
	 */
	public void addElements(Object[]
							objects) { // flsobral@tc113_39: new method to add array of objects.
		int newSize = count + objects.length;
		if (items.length < newSize) {
			ensureCapacity(newSize);
		}
		Vm.arrayCopy(objects, 0, items, count,
					 objects.length); //flsobral@tc120_33: inserting elements one position higher than it should.
	...
	public void ensureCapacity(int minCapacity) {
		if (minCapacity > 0 && items.length < minCapacity) {
			...
			int newSize = (Settings.onJavaSE ? minCapacity * 2 : minCapacity * 12 / 10) +
						  1; // flsobral@tc110_5: new size is >= current size + 1.- guich@tc112_6: +1 in both cases
