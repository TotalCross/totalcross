...
public class Vector
{
  ...
  public Vector(int size)
  {
    if (size < 0) {
      throw new IllegalArgumentException(
        "The argument 'size' must be non-negative");
    }
    items = new Object[size];
  }

  /**
   * Constructs a vector starting with the given elements. The vector can grow
   * after this. Note that the given array must not have null elements.
   *
   * @param startingWith
   */
  public Vector(Object[] startingWith)
  {                              // guich@200b4_31
    count = startingWith.length; // flsobral@tc100b4_21: fail-fast policy -
                                 // throws NPE if startingWith is null.
    items = startingWith;
  }
   ...
   * Note that if the elements are Strings, you can cast the result to a
   * String[] array.
   */
   ...
   *
   */
  public Object peek(Object def) { return count > 0 ? items[count - 1] : def; }
  ...
  { // flsobral@tc112: Throw AIOOBE
    // instead of IllegalArgument to
    // match the Java implementation.
    if (newSize < 0) {
      throw new ArrayIndexOutOfBoundsException(
        "Argument 'newSize' must be positive.");
    }
  ...
  /**
   * Finds the index of the given object. The list is searched using a O(n)
   * linear search through all the objects in the vector.
   */
  public int indexOf(Object elem) { return indexOf(elem, 0); }

  /**
   * Finds the index of the given object. The list is searched using a O(n)
   * linear search starting in startIndex up through all the objects in the
   * vector.
   */
  ...
  /**
   * Inserts the object at the given index. If index is less than 0 or above
   * the number of elements, it is inserted at the end.
   */
  ...
  { // flsobral@tc113_39: new method
    // to add array of objects.
    int newSize = count + objects.length;
    if (items.length < newSize) {
      ensureCapacity(newSize);
    }
    Vm.arrayCopy(objects,
                 0,
                 items,
                 count,
                 objects.length); // flsobral@tc120_33: inserting elements one
                                  // position higher than it should.
   ...
   */
  public void addElementsNotNull(Object[] objects)
  { // guich@tc124_8
    int newSize = count + objects.length;
    if (items.length < newSize) {
      ensureCapacity(newSize);
  ...
  /**
   * Clears all elements in this vector and sets the count to 0. Note that this
   * method sets all items in this vector to <code>null</code>, so, if you had
   * directly assigned an array to this vector, all items inside it will be
   * nulled.
   */
   ...
   */
  public void qsort()
  { // flsobral@tc100b4_22: changed return type to void.
    if (count > 0) {
  ...
  /**
   Sorts the elements of this Vector, in the given order. If they are
   Strings, the sort will be much faster because a cast to String is done; if
   they are not strings, Convert.qsort will try to discover the type.
 */
  ...
  /**
   * Copies the items of this vector into the given array, which must have at
   * least the current size of this vector. If the out vector is greater than
   * the current size, the remaining positions will remain unchanged.
   * @since TotalCross 1.0 beta 4
   */
  public void copyInto(Object[] out) { Vm.arrayCopy(items, 0, out, 0, count); }
  ...
  public void ensureCapacity(int minCapacity)
  {
    if (minCapacity > 0 && items.length < minCapacity) {
      ...
      int newSize =
        (Settings.onJavaSE ? minCapacity * 2 : minCapacity * 12 / 10) +
        1; // flsobral@tc110_5: new size is >= current size + 1.-
           // guich@tc112_6: +1 in both cases
   ...
   * In a vector with n elements, the element of index 0 is moved to the index
   * n-1, the element of index 1 is moved to the index n-2, and so on.
   *
   * @since TotalCross 1.15
   */
  public void reverse()
  ...
  public Object elementAt(int i) { return items[i]; }
}