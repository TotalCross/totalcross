 ...
 * ...
 * </pre>
 * <br>For efficiency, get and set is made directly through the <items> public
 * array. <b>Please use the add and remove methods to manipulate the
 * IntVector.</b>
 */
public class IntVector
{
  ...
  /**
   * Constructs a vector by directly assigning the given int array. Changes on
   * the original array will reflect the items of this array and vice-versa.
   * @since SuperWaba 5.11
   */
  public IntVector(int[] items)
  {                       // guich@511_11
    count = items.length; // flsobral@tc100b4: fail-fast policy - throws NPE if
                          // items is null.
    this.items = items;
  }

  /**
   * Useful method to use when this IntVector will act like a bit vector,
   * through the methods <code>isBitSet</code> and <code>setBit</code>. Just
   * call it with the maximum bit index what will be used (starting from 0);
   * then you can safely use the two methods. This must be done because those
   * methods does not check the bounds of the array.
   */
   ...
   * @return true if the bit specified is set. you must guarantee that the index
  ...
  public void insertElementAt(int obj, int index)
  {
    ...
    if (count == items.length) {
      ...
      int newSize =
        (Settings.onJavaSE ? items.length * 2 : items.length * 12 / 10) +
  ...
  { // flsobral@tc120_34: new method to
  ...
  public void removeElement(int obj) { removeElementAt(indexOf(obj, 0)); }
  ...
  public void qsort() { qsort(0, count - 1); }
   ...
   * size, new items are added to the end of the vector. If the new size is less
   * than the current size, all components at index newSize and greater are
   * discarded.
   *
   * @param newSize
  ...
  { // flsobral@tc112_11: removed
    // boolean argument "copyOldData".
    // The original content is always
    // kept now.
    if (newSize < 0) {
   ...
   */
  public void copyInto(int[] out) { Vm.arrayCopy(items, 0, out, 0, count); }
   ...
   */
  public boolean contains(int v) { return indexOf(v) >= 0; }
