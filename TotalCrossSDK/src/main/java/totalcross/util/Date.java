...
/**
 ...
 built in to other languages. It supports all days from January 1st, 1000 D.C.
 through December 31st, 2999. It checks to make sure that the dates that are
 instanciated or changed exist and if they don't an exception is thrown.  It
 provides methods to advance the date backwards and forwards by increments of
 day, week, and month.  It provides comparisons =,>,<.
...
public class Date implements Comparable
{
  ...
  /**
   * Defines the minimum valid year. Any year before this date throws an
   * InvalidDateException. Defaults to 1000 DC
   */
  ...
  public Date(totalcross.sys.Time t) throws InvalidDateException
  { // guich@350_24
    this.day = t.day;
    this.month = t.month;
    this.year = t.year;
  ...
  /**
   * Sets the date fields by parsing the given String, and using the
   * dateFormat. If you want to use the default date format, use
   * <code>Settings.dateFormat</code>. Trailing spaces are skipped.
   * @since TotalCross 1.24
   * @return The date in the format YYYYMMDD
   */
  public int set(String strDate, byte dateFormat) throws InvalidDateException
  {
    if (strDate == null || strDate.length() == 0) {
      throw new InvalidDateException(strDate == null
                                       ? "Error: date cannot be null"
                                       : "Error: date cannot have length 0");
    }
    try {
  ...
  { // guich@552_13: wasDateValid must be set
    // after calling setToday. Also, this code
    // was very replicated.
    if (0 <= year && year < 20) {
      year += 2000;
    ...
    if (!(day > 0 && month >= 1 && month <= 12 && day <= getDaysInMonth() &&
          year >= minValidYear && year < 3000)) {
      throw new InvalidDateException("Invalid date. Day: " + day +
                                     ", month: " + month + ", year: " + year);
    }
  }

  /**
    Constructs a Date object set to a passed string in the format specified in
    the dateFormat parameter. The constructor auto-detects the separator.

    @param strDate string that should have the format specified in the
        Settings.dateFormat. Note: does not have to be separated by
  ...
  /**
    Constructs a Date object set to a passed string in the format specified in
    the current date format. The constructor auto-detects the separator. If an
    invalid date is passed, sets to the current date;
  ...
  public Date(int sentDate) throws InvalidDateException
  {
    if (sentDate < 10000101 || sentDate > 29991231) {
      throw new InvalidDateException("Argument sentDate (" + sentDate +
  ...
  /**
   ...
   @param sentDay - an integer that must be between 1 and the last day in the
       month.
  ...
  { // guich@566_29: removed setDayOfWeek and placed code here.
    int yy = year;
    int mm = month;
    int a = (yy - 1582) * 365;
    int b = (int)((yy - 1581) / 4);
  ...
  /**
     ...
     @return integer representation of ordinal value of the week within the set
         year.
   */
  public int getWeek()
  {
    ...
    try {
      ...
      int gregorian = getGregorianDay() - firstDay.getGregorianDay() +
                      1;        // Jan 1 = 1, Jan 2 = 2, etc...
      ...
      if (weekNum > 53 ||
          (weekNum > 52 && !(dowJan1 > (isLeapYear(this.year) ? 4 : 5)))) {
        weekNum = 1;
      }
  ...
  /**
   * Formats the day/month specified with the Settings.dateFormat, zero
   * padded.
   */
  public String formatDayMonth()
  {
    ...
    return Convert.zeroPad(Convert.toString(i1), 2) + Settings.dateSeparator +
      Convert.zeroPad(Convert.toString(i2), 2);
  }

  /**
    Formats the date specified with the dateFormat parameter, zero padded, and
  ...
  { // guich@tc115_22
    ...
    return Convert.zeroPad(i1, 2) + dateSeparator + Convert.zeroPad(i2, 2) +
      dateSeparator + Convert.zeroPad(i3, 2);
  }

  /**
   * Formats the date specified with the Settings.dateFormat, zero padded, and
   * using the given separator.
   * @since TotalCross 1.15
   */
  public static String formatDate(int day,
  ...
  {
    return formatDate(
      day, month, year, dateFormat, Convert.toString(Settings.dateSeparator));
  }
   ...
   */
  @Deprecated public String getDate() { return formatDate(day, month, year); }

  /**
   Returns the date in a string format.
   @return string representation of the date in the current Settings.dateFormat.
   */
  @Override public String toString() { return formatDate(day, month, year); }

  /**
   Returns the date in a string format and using the given format and
   separator.
   @return string representation of the date in the current Settings.dateFormat.
   @since TotalCross 1.15
   */
  public String toString(byte dateFormat, String separator)
  { // guich@tc115_22
  ...
  /**
   ...
   @return integer representation of the date (year * 10000) + (month *100) +
       day
   */
  public int getDateInt() { return (year * 10000) + (month * 100) + day; }

  /**
    Returns number of days in the set month.
    @return integer containing number of days in set month.
   */
  public int getDaysInMonth() { return getDaysInMonth(month); }
  ...
  /**
    Checks to see if the Date object passed occurs before the existing Date
    object.
  ...
  /**
    Checks to see if the Date object passed occurs after the existing Date
    object.
    @param sentDate object to compare with existing.
  ...
  /**
    Checks to see if the Date object passed occurs at the same time as the
    existing Date object.
    @param sentDate object to compare with existing.
    @return boolean stating whether or not it occurs at the same time as
        existing date.
   */
  @Override public boolean equals(Object sentDate)
  { // guich@341_11
  ...
  public void advanceWeek() { advanceWeek(FORWARD); }

  /**
    Advances the date to the beginning of the next or previous week.
    @param direction - static variables FORWARD or BACKWARD instructs the method
        to either move to the next
    or previous week
   */
  ...
  public void advanceMonth() { advanceMonth(FORWARD); }
   ...
   *
   * @param numberDays integer containing number of days that the date should
   *     change can be positive or negative
   */
  public void advance(int numberDays)
  { // flsobral@tc100: advance was calculating leap
    // years based on the julian calendar, created the
    // method isLeapYear to fix it.
  ...
  { // flsobral@tc100b4_17: renamed getJulianDay to
    // getGregorianDay.
    // dhs@340_29
    int numberofDays = (year - epochYear) * 365 + getNumLeapDays(year) -
                       getNumLeapDays(epochYear);
  ...
  /**
   * Returns the difference in days from this date and the given one (other -
   * this).
  ...
  { // flsobral@tc100b4_18: made it static and public.
    if (year % 400 == 0) {
      return true;
    }
   ...
   * @since TotalCross 2.0
   */
  public String getSQLString() { return getSQLString(new StringBuffer(20)); }

  /**
   * Returns this date in the Time.getTimeLong format, with
   * hour/minute/second/millis equal to 0.
   * @since TotalCross 2.0
   */
