...
public class Date implements Comparable {
	...
	private void verifyDate() throws
		InvalidDateException { // guich@552_13: wasDateValid must be set after calling setToday. Also, this code was very replicated.
		if (0 <= year && year < 20) {
			year += 2000;
		} else if (20 <= year && year < 100) {
			year += 1900;
		}
		if (!(day > 0 && month >= 1 && month <= 12 && day <= getDaysInMonth() && year >= minValidYear
			  && year < 3000)) {
			throw new InvalidDateException("Invalid date. Day: " + day + ", month: " + month + ", year: " +
										   year);
		}
	}
	...
	public String formatDayMonth() {
		...
		return Convert.zeroPad(Convert.toString(i1),
							   2) + Settings.dateSeparator + Convert.zeroPad(Convert.toString(i2), 2);
	}
	...
	public static String formatDate(int day, int month, int year, byte dateFormat,
									String dateSeparator) { // guich@tc115_22
		...
		return Convert.zeroPad(i1, 2) + dateSeparator + Convert.zeroPad(i2,
				2) + dateSeparator + Convert.zeroPad(i3, 2);
	}
	...
	public void advance(int
						numberDays) { // flsobral@tc100: advance was calculating leap years based on the julian calendar, created the method isLeapYear to fix it.
