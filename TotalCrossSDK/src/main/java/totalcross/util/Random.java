...
/**
   This is a simple Linear Congruential Generator which
    ...
    <pre>
    Random r = new Random(0x1234); // the use of the same key is sometimes
   desirable for (...)
    {
...
public class Random
{
  ...
  public Random()
  {
    ...
    seed0 = seed = t.hour * 60 * 60 + t.minute * 60 + t.second +
                   100 * t.millis; // hope this value never gets < 0 !
  }
  ...
  public double nextDouble() { return next(24) / (double)(1 << 24); }
   ...
   * inclusive. Note that if s == e, it will return in the range s and e+1.
  ...
  public int getSeed() { return seed0; }
}