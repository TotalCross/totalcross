...
public class LangTag {
	...
	private static final String language             = languageRE_1 + "|" + languageRE_2 + "|" +
			languageRE_3 ;
	...
	private static final String langtag              = "^(" + language + ")(?:-(" + script + "))?(?:-("
			+
			region + "))?(?:-(" + variant + "))?(?:-(" + extension + "))?$";
	...
	public static String[] parse(String languageTag) {
		...
		parts[idxLanguage] = parts[idxLanguage].toLowerCase() ; // Copyright 2018 Totalcross/Italo Yeltsin
	...
	private static String titlecase(String string) {
		...
		string = string.substring(1).toLowerCase() ; // Copyright 2018 Totalcross/Italo Yeltsin
		return ch1 + string ;
	}
}