...
public class ThreadPool
{
   ...
   * new thread or in an existing pooled thread. If the task cannot be submitted
   * for execution, either because this executor has been shutdown or because
   ...
   */
  public void shutdown() { this.keepRunning = false; }

  /**
   * Invokes shutdown when this executor is no longer referenced and it has no
   * threads.
   */
  @Override protected void finalize() throws Throwable { this.shutdown(); }
  ...
  private class PoolWorker extends Thread
  {
    public void run()
    {
      while (true) {
        Runnable r = null;
        synchronized (lock) { r = queue.poll(); }
        if (r != null) {
          try {
            r.run();
          } catch (Exception e) {
