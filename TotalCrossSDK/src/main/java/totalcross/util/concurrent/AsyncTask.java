 ...
 * BufferedStream.WRITE, 4096); int counter = 0; while(true) { int size =
 * p.readBytes(buff, 0, buff.length); counter += size; progress =
 * (int)((counter/(double)totalSize)*100); if(size <= 0) break;
 ...
 * (elapsed) -> { progressBar.setValue(progress);
 *             });
 *         }
 *
 *         &#64;Override
...
public abstract class AsyncTask<Params, Progress, Result>
{
  ...
  /**
   * A single instance responsible for creating an unique thread that will
   * receive Runnable instances to execute their run methods and will sleep in
   * ausence of instance to be executed.
   */
  static class Executor implements Runnable
  {
    ...
    {
      synchronized (lock) { requests.addLast(request); }
    }
    ...
    @Override public void run()
    {
      while (true) {
        Runnable r = null;
        synchronized (lock) { r = requests.poll(); }
        if (r != null) {
          try {
            r.run();
          } catch (Exception e) {
  ...
  @SafeVarargs protected final void publishProgress(Progress... values)
  {
    MainWindow.getMainWindow().runOnMainThread(new Runnable() {
      @Override public void run() { onProgressUpdate(values); }
    });
  }
