 ...
 *
 * A logger can be created or retrieved (if it already exists) by calling
 * getLogger.
 *
 * To dispose a logger after using it, just call dispose and it will be
...
public class Logger
{
  ...
  static
  {
    ...
    getLogger(
      "totalcross", Logger.WARNING | Logger.SEVERE, Logger.DEBUG_CONSOLE);
    getLogger(
      "totalcross.event", Logger.WARNING | Logger.SEVERE, Logger.DEBUG_CONSOLE);
    getLogger(
      "totalcross.net", Logger.WARNING | Logger.SEVERE, Logger.DEBUG_CONSOLE);
    getLogger(
      "totalcross.sys", Logger.WARNING | Logger.SEVERE, Logger.DEBUG_CONSOLE);
  }
   ...
   * if the logger is set to log a specific type of message (SEVERE, WARNING,
   * etc), just check if the level OR'ed with the message level is different
   * than zero.
   */
   ...
   * @param level The logger level to set. This should be a composition of one
   *     or more
   * of the message types constants (FINEST, FINER, FINE, CONFIG, INFO, WARNING,
   * SEVERE) or OFF, if you want to disable all logging, or ALL, if you want to
   * enable all logging.
   */
  public void setLevel(int level) { this.level = level; }

  /**
   * Sets the initial logger level, which will be used when a new logger is
   * created.
   * @param level The logger level to set. This should be a composition of one
   *     or more
   * of the message types constants (FINEST, FINER, FINE, CONFIG, INFO, WARNING,
   ...
   * enable all logging.
   */
  public static void setDefaultLevel(int level) { defaultLevel = level; }

  /**
   * Returns the initial logger level, which is used when a new logger is
   * created.
   * @return An integer number representing the default logger level.
   */
  public static int getDefaultLevel() { return defaultLevel; }
   ...
   */
  public static String getDefaultSeparator() { return defaultSeparator; }

  /**
   * Adds an output stream to the logger's output handler set. This means that
   * every logged message (depending on the current logger level) will be
   ...
   * @param prependInfo A flag indicating whether this log message must be
   *     prepended
  ...
  public void log(int level, String message, boolean prependInfo)
  {
    if ((this.level & level) != 0) {
      ...
      byte[] b1 = Convert.getBytes(sb); // guich@tc123_35: don't append the
                                        // message into the StringBuffer...
  ...
  { // guich@tc123_62
    if ((this.level & INFO) != 0) {
      byte[] b2 =
        message == null
          ? NULL_BYTES
          : Convert.getBytes(message); // ... just use its bytes directly
      for (int i = outputHandlers.size() - 1; i >= 0; i--) {
        try {
          Stream s = (Stream)outputHandlers.items[i];
   ...
   * message level then the given message is written to all the registered
   * output handler streams.
   * @param message The message to log.
   * @throws NullPointerException if one or more registered output handler
   * streams are null or this logger has been disposed.
   */
  public void finest(String message) { log(FINEST, message, true); }
   ...
   * streams are null or this logger has been disposed.
   */
  public void finer(String message) { log(FINER, message, true); }
   ...
   * streams are null or this logger has been disposed.
   */
  public void fine(String message) { log(FINE, message, true); }
   ...
   * streams are null or this logger has been disposed.
   */
  public void config(String message) { log(CONFIG, message, true); }
   ...
   * streams are null or this logger has been disposed.
   */
  public void info(String message) { log(INFO, message, true); }
   ...
   * streams are null or this logger has been disposed.
   */
  public void warning(String message) { log(WARNING, message, true); }
   ...
   * streams are null or this logger has been disposed.
   */
  public void severe(String message) { log(SEVERE, message, true); }
  ...
  {
    log(FINER,
        "ENTRY " + sourceClass + "." + sourceMethod + "(" + params + ")",
        true);
  }
  ...
  /**
   * Log throwing an exception. This is a convenience method to log that a
   * method is terminating by throwing an exception. The logging is done using
   * the FINER level. If the logger is currently enabled for the given message
   * level then the record is forwarded to all registered output handlers. The
   * record's message is set to "THROW".
  ...
  {
    log(SEVERE,
        "THROW " + sourceClass + "." + sourceMethod + ": " +
          Vm.getStackTrace(thrown),
        true);
  }
        ...
        for (int i = outputHandlers.size() - 1; i >= 0; i--) {
          try {
            ...
            if (s instanceof ResizeRecord) {
              ((ResizeRecord)s)
                .getStream()
                .close(); // close the PDB associated to it, since
                          // ResizeRecord no longer closes the underlying
                          // stream
            } else {
              s.close();
            }
