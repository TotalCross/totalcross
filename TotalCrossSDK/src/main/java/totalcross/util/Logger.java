...
public class Logger {
	...
	public void log(int level, String message, boolean prependInfo) {
		if ((this.level & level) != 0) {
			...
			byte[] b1 = Convert.getBytes(
							sb); // guich@tc123_35: don't append the message into the StringBuffer...
	...
	public void logInfo(StringBuffer message) { // guich@tc123_62
		if ((this.level & INFO) != 0) {
			byte[] b2 = message == null ? NULL_BYTES : Convert.getBytes(
							message); // ... just use its bytes directly
			for (int i = outputHandlers.size() - 1; i >= 0; i--) {
				try {
					Stream s = (Stream) outputHandlers.items[i];
				...
				for (int i = outputHandlers.size() - 1; i >= 0; i--) {
					try {
						...
						if (s instanceof ResizeRecord) {
							((ResizeRecord)
							 s).getStream().close(); // close the PDB associated to it, since ResizeRecord no longer closes the underlying stream
						} else {
							s.close();
						}
