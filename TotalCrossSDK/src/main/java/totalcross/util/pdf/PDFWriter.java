...
public class PDFWriter {
	...
	private void renderCatalog() {
		mCatalog
		.setDictionaryContent("  /Type /Catalog\n  /Pages " +
	...
	public void addImage(int fromLeft, int fromBottom, Image Image,
		...
		mCurrentPage.addImage(fromLeft, fromBottom, xImage.getWidth(), xImage.getHeight(), xImage,
							  transformation);
	}

	public void addImage(int fromLeft, int fromBottom, int toLeft, int toBottom,
						 Image Image) throws IOException {
		addImage(fromLeft, fromBottom, toLeft, toBottom, Image, Transformation.DEGREES_0_ROTATION);
	}

	public void addImage(int fromLeft, int fromBottom, int toLeft, int toBottom, Image Image,
						 String transformation)
	throws IOException {
		mCurrentPage.addImage(fromLeft, fromBottom, toLeft, toBottom, new XObjectImage(mDocument, Image),
							  transformation);
	}

	public void addImageKeepRatio(int fromLeft, int fromBottom, int width, int height,
								  Image Image) throws IOException {
		addImageKeepRatio(fromLeft, fromBottom, width, height, Image, Transformation.DEGREES_0_ROTATION);
	}

	public void addImageKeepRatio(int fromLeft, int fromBottom, int width, int height, Image Image,
								  String transformation)
	throws IOException {
		final XObjectImage xImage = new XObjectImage(mDocument, Image);
