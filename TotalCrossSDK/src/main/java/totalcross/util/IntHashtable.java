 ...
 *     int i = numbers.get(2);
      // "two = " + i;
 * </pre>
 * Don't forget to catch the ElementNotFoundException in the methods it is
 thrown.
 */
public class IntHashtable
{
  ...
  {
    public DuplicatedKeyException(String s) { super(s); }
  }
  ...
  /**
   * Set to false to throw a IntHashtable.DuplicatedKeyException if you add a
   * key that already exists. Its very rare to have two Objects with same key,
   * but it could occur. This is good to improve program's correctness.
   ...
   *
   * @param   initialCapacity The number of elements you think the hashtable
   *     will end with. The hashtable will grow if necessary, but using
   ...
   * @throws totalcross.util.ElementNotFoundException When the key was not
   *     found.
   * @see #get(int, int)
   */
  ...
  /**
   * Return an IntVector of the keys in the IntHashtable. The order is the same
   * of the getValues method. Added ds@120. corrected by dgecawich@200
   */
  public IntVector getKeys() { return getKeysOrValues(true); }
  ...
  private IntVector getKeysOrValues(boolean isKeys)
  {
    ...
    int[] array =
      new int[count]; // guich@511_10: optimized to avoid method calls
   ...
   * @throws IntHashtable.DuplicatedKeyException if allowDuplicateKeys is set to
   *     false and another key is already added.
   */
   ...
   * @param   key   a key in the hashtable.
   * @return  the value to which the key is mapped in this hashtable;
   *          or <code>defaultValue</code> if the key is not mapped to any value
   * in this hashtable.
  ...
  /**
   * Returns the key at the given position, or throws ArrayIndexOutOfBounds if
   * the given position does not exist. Note that the first key has no relation
  ...
  /**
   * Increments the value of a key by the given amount. If the key doesn't
   * exist, a new one is created with the amount. Otherwise, its value is
   * changed by the amount. This method is useful to use an IntHashtable as a
   * multi counter.
