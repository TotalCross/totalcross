...
final public class ZLib {
	 ...
	 */
	@ReplacedByNativeOnDeploy
	public static int inflate(Stream in, Stream out, int sizeIn, boolean noWrap) throws IOException,
		ZipException {
		if (in == null) {
			throw new NullPointerException("Argument 'in' cannot have a null value");
		}
		...
		try {
			while (true) {
				...
				if (tor > 0) {
					r = in.readBytes(bin, 0,
									 tor); // if tor == 0 and the stream does not quietly accept requests of size 0 (such as File stream) this call will throw exception
				}
				if (r > 0) {
