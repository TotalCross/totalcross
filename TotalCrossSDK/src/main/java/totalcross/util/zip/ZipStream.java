 ...
 * ZIP file format. Currently supports only compressed entries.<br> <br> See the
 * sammple TotalCross3/src/tc/samples/util/zip/zip.
 *
...
public class ZipStream extends CompressedStream
{
  ...
  /**
   * Creates a new ZIP stream that may be used to read compressed data from the
   * given stream, or to write compressed data to the given stream.<br>
   *
   * @param stream
   *           input stream.
   * @param mode
   ...
   * @since TotalCross 1.20
   */
  public ZipStream(RandomAccessStream stream, int mode) { super(stream, mode); }

  @Override protected Object createDeflate(Stream stream)
  {
    Launcher.S2OS os = new Launcher.S2OS(stream, false);
    return new java.util.zip.ZipOutputStream(os);
  }

  @Override protected Object createInflate(Stream stream)
  {
   ...
   * always return 1.<br> Programs should not count on this method to return the
  ...
  public ZipEntry getNextEntry() throws IOException
  {
    ...
    java.util.zip.ZipInputStream zis =
      (java.util.zip.ZipInputStream)this.compressedStream;
    try {
   ...
   * of the entry data. Closes the current entry if still active.
   *
   * @param entry
  ...
  public void putNextEntry(ZipEntry entry) throws IOException, ZipException
  {
    ...
    java.util.zip.ZipOutputStream zos =
      (java.util.zip.ZipOutputStream)this.compressedStream;
    try {
  ...
  /**
   * Closes the current ZIP entry and positions the stream for reading (INFLATE)
   * or writing (DEFLATE) the next entry.
   *
   * @throws IOException
   *            if an I/O error has occurred
   *
