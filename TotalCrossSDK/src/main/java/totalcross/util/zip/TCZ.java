...
/**
    ...
    <ul>
    <li> length      (4 bytes)
    <li> offsets array (length+1) - offset[i+1]-offset[i] gives the compressed
   size <li> uncompressed sizes array (length) <li> names array (length)
    </ul>

    The header is compressed to save space.
    The first record is the class that implements totalcross.MainClass or
   extends totalcross.ui.MainWindow.
 */
...
public class TCZ
{
  ...
  public static class Entry
  {
    ...
    public Entry(byte[] bytes, String name, int uncompressedSize)
      throws Exception
    {
      this(bytes, name, uncompressedSize, null);
    }

    public Entry(byte[] bytes, String name, int uncompressedSize, Object extra)
      throws Exception
    {
      ...
      if (name2write.indexOf('.') < 0) {
        name2write =
          name2write.replace('/', '.'); // assuming not ending with .class
      }
    }

    /**
     * Returns a String representing this Entry. Used in the Vector.qsort
     * method
     */
    @Override public String toString()
    {
      if (mainClassName != null && name.equals(mainClassName)) {
        return '\1' +
          name2write; // make sure this name will go first when sorting
      }
      return name2write;
    }
  }
  ...
  /**
   * Defines that the tcz file has a MainClass at the first record. If false,
   * it is a library-only module
   */
   ...
   * @throws ZipException
   */
  public TCZ(Vector vout, String outName, short attr)
    throws IOException, ZipException
  {
    ...
    for (int i = 0; i <= n; i++) {
      dsh.writeInt(
        offsets[i]); // the offsets are stored first to ensure alignment.
    }
    for (int i = 0; i < n; i++) {
    ...
    for (int i = 0; i < n; i++) { // strings are stored as UTF8
      ...
      int l = s.length();
      if (l > 255) {
        throw new ZipException("Error: Name too long: " + s + ". (" + l +
                               " chars). Maximum allowed: 255 chars");
      }
      dsh.writeSmallString(s);
    }

    // write the first part: the version, the attribute and the compressed
    // header
    ...
    ZLib.deflate(
      header,
      bc,
      9); // the smaller the compression level, the slower is the vm's startup
    size +=
      dsf.writeInt(bc.getPos() + 8); // base offset = compressed header size + 4
  ...
  /**
   * Reads a TCZ file and fill the public members available in this class:
   * version, attr, baseOffset, offsets, uncompressedSizes, names, chunks,
   * numberOfChunks. To preserve memory, you must request each chunk using
   * readNextChunk <b>right after</b> you call this constructor.
   * @throws IOException
   */
  ...
  public int getNextChunkSize() { return uncompressedSizes[idx]; }

  /**
   * Fills the given stream with the next available chunk.
   * The size of the chunk can be retrieved with getNextChunkSize.
   * If out is a ByteArrayStream, don't forget to call reset before starting to
   * read from it!
   * @throws IOException
   */
