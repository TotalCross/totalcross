...
/**
 * This class implements stream compression using the GZip algorithm provided
 * with the ZLib library.
 *
 * <blockquote><i><q>gzip (GNU zip) is a compression utility designed to be a
 * replacement for compress. Its main advantages over compress are much better
 * compression and freedom from patented algorithms. It has been adopted by the
...
{
  /**
   * Deflates the given stream 'in', writing the compressed data to the given
   * stream 'out'.
   *
   * @param in
  ...
  /**
   * Attempts to fully read the given stream 'in', inflate its content and write
   * the uncompressed data to the given stream 'out'.
   *
   * @param in
   ...
   *           Inflated output stream
   * @param sizeIn
   *           How many bytes to read, or -1 to read until <code>in</code>'s
   * end.
   * @return Size of the inflated stream
   * @throws IOException
   * @throws ZipException
  ...
  private GZip() {} // cannot instantiate
}
