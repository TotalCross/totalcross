...
/**
 * Largely based on <a
 * href="https://commons.apache.org/proper/commons-io/javadocs/api-2.5/src-html/org/apache/commons/io/IOUtils.html">
 * Apache IOUtils</a> <p/> Only needed methods copied, like copy InputStream ->
 * OutoutStream and copy Reader -> Writer <p/> Javadoc maintained where
 * possible, but the "since" tag was largely removed
 */
public final class IOUtils
{
   ...
   * <code>OutputStream</code> using an internal buffer of the given size. <p>
   ...
   * to an <code>OutputStream</code>, optionally skipping input bytes. <p> This
   ...
   * This means that the method may be considerably less efficient than using
   * the actual skip implementation, this is done to guarantee that the correct
  ...
  {
    return copyLarge(
      input, output, inputOffset, length, new byte[DEFAULT_BUFFER_SIZE]);
  }
   ...
   * Copies some or all bytes from a large (over 2GB) <code>InputStream</code>
   * to an <code>OutputStream</code>, optionally skipping input bytes. <p> This
   * method uses the provided buffer, so there is no need to use a
   * <code>BufferedInputStream</code>.
   * </p>
   * <p>
   * Note that the implementation uses {@link #skip(InputStream, long)}.
    ...
    throws IOException
  {
    ...
    if (skipped != toSkip) {
      throw new EOFException("Bytes to skip: " + toSkip +
                             " actual: " + skipped);
    }
  }
   ...
   * <p>
   * Note that the implementation uses {@link InputStream#read(byte[], int,
   * int)} rather than delegating to {@link InputStream#skip(long)}. This means
    ...
    throws IOException
  {
    ...
    long remain = toSkip;
    while (remain > 0) {
      // See https://issues.apache.org/jira/browse/IO-203 for why we use read()
      // rather than delegating to skip()
      final long n = input.read(
        SKIP_BYTE_BUFFER, 0, (int)Math.min(remain, SKIP_BUFFER_SIZE));
      if (n < 0) { // EOF
        break;
      }
      remain -= n;
   ...
   * <code>Writer</code>. <p> This method buffers the input internally, so there
   * is no need to use a <code>BufferedReader</code>. <p> The buffer size is
   * given by {@link #DEFAULT_BUFFER_SIZE}.
   *
   * @param input the <code>Reader</code> to read from
   * @param output the <code>Writer</code> to write to
  ...
  /**
   * Copies chars from a large (over 2GB) <code>Reader</code> to a
   * <code>Writer</code>. <p> This method uses the provided buffer, so there is
   * no need to use a <code>BufferedReader</code>. <p>
   *
   * @param input the <code>Reader</code> to read from
   * @param output the <code>Writer</code> to write to
   ...
   * @since 2.2
   */
  public static long copyLarge(final Reader input,
                               final Writer output,
