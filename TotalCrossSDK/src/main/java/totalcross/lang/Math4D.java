 ...
 * <P>
 * The Math class used in the device is not the same one used at Java SE. Be
 * sure to refer to this documentation to see what you can use. <P> Note that
 * angles are specified in radians.  Conversion functions are provided for your
 * convenience. <br><br> IMPORTANT: the totalcross.lang package is the java.lang
 * that will be used in the device. You CANNOT use nor import totalcross.lang
 * package in desktop. When tc.Deploy is called, all references to java.lang are
 * replaced by totalcross.lang automatically. Given this, you must use only the
 * classes and methods that exists BOTH in java.lang and totalcross.lang. For
 * example, you can't use java.lang.ClassLoader because there are no
...
{
  static public final double NaN =
    Convert.longBitsToDouble(0x7ff8000000000000L);
  static public final double POSITIVE_INFINITY =
    Convert.longBitsToDouble(0x7ff0000000000000L);
  static public final double NEGATIVE_INFINITY =
    Convert.longBitsToDouble(0xfff0000000000000L);
  ...
  private static final int[] SQRT_PARMS = {
    0,     1024,  3062,  5746,  9193,  13348, 18162, 23592, 29598, 36145, 43202,
    50740, 58733, 67158, 75992, 85215, 83599, 71378, 60428, 50647, 41945, 34246,
    27478, 21581, 16499, 12183, 8588,  5674,  3403,  1742,  661,   130
  };
   ...
   * The range of angles you will get are from -pi/2 to pi/2 radians (-90 to 90
   * degrees)
  ...
  public static double asin(double x)
  {
    ...
    // |x| < 2e-27
      ...
      double p =
        x2 *
        (pS0 + x2 * (pS1 + x2 * (pS2 + x2 * (pS3 + x2 * (pS4 + x2 * pS5)))));
   ...
   * degrees), the whole spectrum of angles.  That is what makes this function
   * so much more useful than the other <code>atan()</code>.
  ...
  public static double log(double x)
  {
    ...
    double f = (Convert.longBitsToDouble(Convert.doubleToLongBits(x) -
                                         (((long)k) << 52)) -
                1.0);
    double s = f / (2.0 + f);
    double z = s * s;
    double w = z * z;
    double r = w * (Lg2 + w * (Lg4 + w * Lg6)) +
  ...
  public static double sqrt(double x)
  {
    ...
    double y =
      Convert.longBitsToDouble(((long)(k - SQRT_PARMS[(k >> 15) & 31])) << 32);
  ...
  public static double pow(double x, double y)
  {
    ...
    if (x < 0.0) {
      ...
      if (y == 0.0) {
        ...
        long temp =
          ((((lly & 0x000fffffffffffffL) + 0x0010000000000000L) << expY) &
           0x001fffffffffffffL);
        if (temp == 0L) {
          yIntType = 2;
  ...
  private static double kernelSin(double x)
  {
    ...
    return x *
      (1.0 +
       x2 * (S1 + x2 * (S2 + x2 * (S3 + x2 * (S4 + x2 * (S5 + x2 * S6))))));
  }
  ...
  private static double kernelCos(double x)
  {
    ...
    return 1.0 +
      x2 *
        (-0.5 +
         x2 * (C1 + x2 * (C2 + x2 * (C3 + x2 * (C4 + x2 * (C5 + x2 * C6))))));
  }
  ...
  private static double kernelTan(double x)
  {
    ...
    return (
      ...
      x * x2 *
        (T1 +
         x4 *
           (T3 + x4 * (T5 + x4 * (T7 + x4 * (T9 + x4 * (T11 + x4 * T13)))))) +
      x * x4 *
        (T2 + x4 * (T4 + x4 * (T6 + x4 * (T8 + x4 * (T10 + x4 * T12))))));
  }
