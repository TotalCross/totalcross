 ...
 * <br><br>
 * IMPORTANT: the totalcross.lang package is the java.lang that will be used in
 * the device. You CANNOT use nor import totalcross.lang package in desktop.
 * When tc.Deploy is called, all references to java.lang are replaced by
 * totalcross.lang automatically. Given this, you must use only the classes and
 * methods that exists BOTH in java.lang and totalcross.lang. For example, you
 * can't use java.lang.ClassLoader because there are no
 * totalcross.lang.ClassLoader. Another example, you can't use
 * java.lang.String.indexOfIgnoreCase because there are no
 * totalcross.lang.String.indexOfIgnoreCase method. Trying to use a class or
 * method from the java.lang package that has no correspondence with
 * totalcross.lang will make the tc.Deploy program to abort, informing where the
 * problem occured. A good idea is to always refer to this javadoc to know what
 * is and what isn't available.
 */
...
public final class String4D implements Comparable<String4D>, CharSequence
{
  ...
  public String4D() { chars = new char[0]; }

  public String4D(CharSequence cs) { this(charSequence2charArryay(cs)); }

  /** Creates a copy of the given string. */
  public String4D(String4D s) { chars = s.chars; }
  ...
  public String4D(char c[])
  {
    ...
    // prevent null access error if len = 0 or ac is null. (added by dgecawich
    // 7/9/01)
    if (count > 0) {
  ...
  /**
   * Creates a string from the given byte array. The bytes are converted to
   * char using the CharacterConverter associated in the charConverter member of
  ...
  public String4D(byte[] value, int offset, int count)
  {
    try {
      ...
      throw new StringIndexOutOfBoundsException(
        "value: " + value.length + " bytes length, offset: " + offset +
        ", count: " + count);
    }
  }

  public String4D(byte[] value, int offset, int count, String encoding)
    throws UnsupportedEncodingException
  {
    try {
      Charset charset = getCharset(encoding);
      if (charset instanceof AbstractCharacterConverter) {
        chars = ((AbstractCharacterConverter)charset)
                  .bytes2chars(value, offset, count);
      }
      chars = Convert.charConverter.bytes2chars(value, offset, count);
    } catch (ArrayIndexOutOfBoundsException aioobe) {
      throw new IndexOutOfBoundsException("value: " + value.length +
   ...
   */
  private String4D(char value[], boolean dummy)
  { // dummy is just to distinguish from the
  ...
  {
    // changed by dgecawich on 5/14/01 because a = a + "something" or a +=
    // "something" caused a stack overflow
  ...
  /** Returns this string. */
  public String4D toString4D() { return this; }
  ...
  /**
   * Returns true if the given string is equal to this string using caseless
  ...
  /**
   * Returns the last index of the specified char in this string, or -1 if not
   * found
   */
  native public int lastIndexOf(int c, int startIndex);

  /**
   * Returns the last index of the specified char in this string starting from
   * length-1, or -1 if not found
   */
  native public int lastIndexOf(int c);

  /**
   * Returns the last index of the specified string in this string starting
   * from length-1, or -1 if not found. <p>CAUTION: this method does not exist
   * in BlackBerry, so if you use it, your program will fail in Blackberry.
   */
  native public int lastIndexOf(String4D s);

  /**
   * Returns the last index of the specified string in this string starting
   * from the given starting index, or -1 if not found.
   * <p>CAUTION: this method does not exist in BlackBerry, so if you use it,
   * your program will fail in Blackberry.
   */
  native public int lastIndexOf(String4D s, int startIndex);

  /**
   * Removes characters less than or equal to ' ' (space) from the beginning
  ...
  /**
   * Return this String as bytes. The chars are converted to byte using the
   * CharacterConverter associated in the charConverter member of
   * totalcross.sys.Convert.
   * @see totalcross.sys.Convert#setDefaultConverter(String)
   * @see totalcross.sys.CharacterConverter
   * @see totalcross.sys.UTF8CharacterConverter
  ...
  {
    if (charset instanceof AbstractCharacterConverter) {
      AbstractCharacterConverter converter =
  ...
  /**
   * Returns the index of the specified string in this string, or -1 if not
  ...
  /**
   * Returns the index of the specified char in this string starting from 0, or
   * -1 if not found
   */
   ...
   */
  @Override native public boolean equals(Object obj);
   ...
   */
  @Override native public int compareTo(String4D s);

  /** Copies the specified srcArray to dstArray */
  native static boolean copyChars(char[] srcArray,
                                  int srcStart,
                                  char[] dstArray,
                                  int dstStart,
                                  int length);
  ...
  /**
   * Returns the index of the specified string in this string starting from the
   * given index, or -1 if not found.
   */
  native public int indexOf(String4D c, int startIndex);

  /** Returns the hashcode for this string */
  @Override native public int hashCode();

  public boolean contains(String4D part) { return indexOf(part) != -1; }
  ...
  public boolean regionMatches(boolean ignoreCase,
                               ...
                               int len)
  {
    if (toffset < 0 || ooffset < 0 || toffset + len > chars.length ||
        ooffset + len > other.chars.length) {
      return false;
    }
    while (--len >= 0) {
      ...
      if (c1 != c2 &&
          (!ignoreCase ||
           (Convert.toLowerCase(c1) != Convert.toLowerCase(c2) &&
  ...
  /*public String replaceFirst(String regex, String replacement)
   {
     return
   Pattern.compile(regex).matcher(this.toString()).replaceFirst(replacement); //
   TODO JEFF
   }*/
  ...
  public String replaceFirst(String regex, String replacement)
  {
    ...
    // return
    // Pattern.compile(regex).replacer(replacement).replace(this.toString());
  }
   ...
   * @since 1.6
   */
  public boolean isEmpty() { return chars.length == 0; }
  ...
  { // NOTE: the original version uses CharSequence
