...
public class Character4D
{
  ...
  public Character4D(char v) { this.v = v; }

  public char charValue() { return v; }

  @Override public boolean equals(Object o)
  {
    return o != null && o instanceof Character4D &&
      ((Character4D)o).v == this.v;
  }

  @Override public int hashCode() { return v; }

  public static Character4D valueOf(char c) { return new Character4D(c); }
  ...
  /*
   ...
   31: 0
         ...
         try
         {
            ...
            File f = new
   File("G:\\TotalCross\\TotalCross3\\src\\totalcross\\chartypes.bin",
   File.CREATE_EMPTY); f.writeBytes(v,0,v.length); f.close();
         }
         catch (IOException e)
         {
            e.printStackTrace();
      ...
      synchronized (lock)
      {
        if (types == null) {
          genTypes = new TypesSupplier<byte[]>() {
            @Override public byte[] get() { return types; }
          };
  ...
  public static int getType(int i) { return genTypes.get()[i]; }

  public static int getType(char i) { return genTypes.get()[i]; }
  ...
  public static char toLowerCase(char c) { return Convert.toLowerCase(c); }

  public static char toUpperCase(char c) { return Convert.toUpperCase(c); }

  public static char toTitleCase(char ch) { return Convert.toTitleCase(ch); }
  ...
  public static boolean isJavaIdentifierStart(char ch)
  {
    ...
    return ((1 << genTypes.get()[codePoint]) &
            ((1 << UPPERCASE_LETTER) | (1 << LOWERCASE_LETTER) |
             ...
             (1 << OTHER_LETTER) | (1 << LETTER_NUMBER) |
             (1 << CURRENCY_SYMBOL) | (1 << CONNECTOR_PUNCTUATION))) != 0;
  }
  ...
  public static boolean isJavaIdentifierPart(char ch)
  {
    ...
    return ((1 << category) &
            ((1 << UPPERCASE_LETTER) | (1 << LOWERCASE_LETTER) |
             (1 << TITLECASE_LETTER) | (1 << MODIFIER_LETTER) |
             (1 << OTHER_LETTER) | (1 << NON_SPACING_MARK) |
             (1 << COMBINING_SPACING_MARK) | (1 << DECIMAL_DIGIT_NUMBER) |
             (1 << LETTER_NUMBER) | (1 << CURRENCY_SYMBOL) |
  ...
  public static boolean isLetterOrDigit(char codePoint)
  {
    return ((1 << getType(codePoint)) &
            ((1 << UPPERCASE_LETTER) | (1 << LOWERCASE_LETTER) |
             (1 << TITLECASE_LETTER) | (1 << MODIFIER_LETTER) |
             (1 << OTHER_LETTER) | (1 << DECIMAL_DIGIT_NUMBER))) != 0;
  }
