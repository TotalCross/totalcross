 ...
 * </pre>
 * You can also call the update method to update the time with the current
 * system values.
 */
...
public final class Time
{
   ...
   */
  @ReplacedByNativeOnDeploy public void update()
  { // guich@tc123_65: renamed timeCreate to update and made it public
  ...
  /**
   * Constructs a time object from a Date, zeroing the hour/minute/second and
   * millis
   * @since TotalCross 2.0
   */
  public Time(Date d) { this(d.getDateInt(), 0); }
   ...
   * <pre>YYYYMMDDTHH:MM:SS</pre>.
   * @since SuperWaba 5.61
   */
  public Time(String iso8601) throws InvalidNumberException
  { // guich@561_4
  ...
  /**
   * Creates a Time object from an SQL String. It accepts any character as
   * separator and also with less fields, based that the sequence is always
  ...
  public Time(char[] sqlTime) throws InvalidDateException
  {
    try {
      ...
      InvalidDateException ide = new InvalidDateException(
        "Invalid date: \"" + new String(sqlTime) + "\"");
      ide.initCause(e);
      throw ide;
    }
  }
   ...
   * @param b Here only to differ from the other constructor that receives a
   *     long and its ignored.
  ...
  {
    return new Date(this).getTime() + hour * 60L * 60L * 1000L +
      (minute - Settings.timeZoneMinutes) * 60L * 1000L + second * 1000L +
      millis;
  }

  /**
   * Constructs a Time object, parsing the String and placing the fields
   * depending on the flags that were set, using the Settings.timeSeparator as
   * spliter. The number of parts must match the number of true flags, or an
   ...
   * depending on the flags that were set, using the Settings.timeSeparator as
   * spliter. The number of parts must match the number of true flags, or an
   * ArrayIndexOutOfBoundsException will be thrown. The dateFormat is only
   * considered if hasYear, hasMonth and hasDay are all true.
   *
   * AM/PM is supported.
   * @since TotalCross 2.1
   */
  public Time(String time,
              ...
              boolean hasMinute,
              boolean hasSeconds,
              byte dateFormat) throws InvalidNumberException
  {
    ...
    try {
      StringBuffer seps =
        new StringBuffer(2); // guich@sqlite: accept any kind of separator
      ...
      String[] parts =
        Convert.tokenizeString(time, seps.toString().toCharArray());
      int idx = 0;
  ...
  public long getSQLLong()
  {
    return year * 10000000000000L + month * 100000000000L + day * 1000000000L +
      hour * 10000000L + minute * 100000L + second * 1000L + millis;
  }
  ...
  /**
   * Returns the time in format specified in totalcross.sys.Settings (does NOT
   * include the date neither the millis). To return the date, use class
   * totalcross.util.Date. So, to get a String with the date and time, use:
   * <pre>
   * Time t = new Time();
   ...
   */
  public String toString(String timeSeparator)
  { // guich@tc115_22
  ...
  { // guich@561_4
    StringBuffer sb = new StringBuffer(20);
    sb.setLength(0);
    // flsobral@tc100b4: calculate first part, instead of creating a Date object
    // to call getDateInt.
  ...
  /**
   * Returns true if the time is valid. Note that the date part is NOT checked,
   * only hour, minute, second and millis are checked against valid ranges.
   * @since TotalCross 1.22
   */
  public boolean isValid()
  {
    return 0 <= hour && hour <= 23 && 0 <= minute && minute <= 59 &&
      0 <= second && second <= 59 && 0 <= millis && millis <= 999;
  }

  /**
   * Increments or decrements the fields below. Note that this method DOES
   * update the day/month/year fields, constructing a Date object and
   * manipulating it; consider reusing a Time object if you use this method.
   * <br><br>Parameters can be positive (to increment), zero (to keep it), or
