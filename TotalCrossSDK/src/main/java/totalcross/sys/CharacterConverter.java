 ...
 * The default character scheme converter is the 8859-1. If you want to use a
 * different one, you must extend this class, implementing the bytes2chars and
 * chars2bytes methods, and then assign the public member of
 * totalcross.sys.Convert.charConverter to use your class instead of this
 * default one. You can also use the method Convert.setDefaultConverter to
 * change it, passing, as parameter, the prefix of your CharacterConverter
 * class. For example, if you created a class named Iso88592CharacterConverter,
 * call <code>Convert.setDefaultConverter("Iso88592");</code> <p> To find out
 * which <code>sun.io.CharacterEncoder</code> you're using, do:
...
public class CharacterConverter extends AbstractCharacterConverter
{
  ...
  public byte[] chars2bytes(char chars[], int offset, int length)
  {
    ...
    if (i != length) { // will never be greater, always smaller, if unicode
                       // chars were found
