...
public interface SpecialKeys {
  ...
  /**
   * modifier used in Android that states the key came from a trusted part of
   * the system (ie, physical keyboard)
   */
  ...
  /**
   * Valid only on WindowsCE. If not registered by the user, it is replaced by
   * MENU.
   */
   ...
   * Settings.screenWidth/Height are already changed when this key event is
   * posted.
   */
  public static final int SCREEN_CHANGE = -1030;
  /**
   * The device was turned off and now on while the application was running.
   * This is not a hardware key and cannot be intercepted; all you can do is
   * handle the SPECIAL_KEY_PRESS event and check if this key was typed.
