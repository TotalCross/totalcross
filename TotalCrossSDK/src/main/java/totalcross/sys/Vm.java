...
public final class Vm {
	 ...
	 */
	public static boolean arrayCopy(Object srcArray, int srcStart, Object dstArray, int dstStart,
									int length) {
	...
	public static int exec(String command, String args, int launchCode, boolean wait) {
		...
		try {
			if (launchCode == -1) { // guich@120
				...
				Launcher.instance.setNewMainWindow((totalcross.ui.MainWindow) Class.forName(command).newInstance(),
												   args);
				status = 0;
				...
				try {
					...
					java.io.BufferedReader input = new java.io.BufferedReader(new java.io.InputStreamReader(
								p.getInputStream()));
	...
	public static void debug(String s) {
		...
		if (!ERASE_DEBUG.equals(s)) {
			System.err.println(Settings.showDebugTimestamp ? (getTimeStamp() + " - " + s) :
							   s); // alexmuc@400_63 - guich@567_16: now using getTimeStamp - guich@tc115_50: don't display timestamp if user don't want
		} else {
			System.err.println(
	...
	public static String getStackTrace(Throwable t) { // guic@582_6
		...
		java.io.StringWriter(); // guich@tc100b4_6: with StringWriter it always works
