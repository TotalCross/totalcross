...
/**
 * This class is used to correctly handle UTF8 byte to UCS-2 chracter
 * conversions. <P> To use this class, you can call
...
public class UTF8CharacterConverter extends AbstractCharacterConverter
{
  ...
  {
    super("UTF-8", new String[] { "unicode-1-1-utf-8", "UTF8" });
  }
  ...
  public byte[] chars2bytes(char chars[], int start, int length)
  {
    ...
    byte[] bytes = new byte[length + length +
                            length]; // guich@566_5: worst case is all chars >
                                     // 0x800, which leads to 3 x length
