...
public class RSACipher extends Cipher
{
   ...
   * @return "RSA".
   */
  @Override public final String getAlgorithm() { return "RSA"; }
   ...
   */
  @Override public final int getBlockLength() { return 0; }
    ...
    try {
      ...
      if (operation == OPERATION_ENCRYPT) {
        ...
        keyRef = factory.generatePublic(
          new RSAPublicKeySpec(new BigInteger(pubKey.getModulus()),
                               new BigInteger(pubKey.getPublicExponent())));
      } else { // DECRYPT
        RSAPrivateKey privKey = (RSAPrivateKey)key;
        keyRef = factory.generatePrivate(
          new RSAPrivateKeySpec(new BigInteger(privKey.getModulus()),
                                new BigInteger(privKey.getPrivateExponent())));
      }
  ...
  @Override @ReplacedByNativeOnDeploy protected final void finalize() {}
}
