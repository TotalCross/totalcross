...
public class PKCS1Signature extends Signature
{
   ...
   */
  public PKCS1Signature(Digest digest)
    throws NoSuchAlgorithmException, CryptoException
  {
    ...
    if (digest instanceof MD5Digest) {
      ...
      throw new CryptoException("Invalid or unsupported signature digest: " +
                                digest.getAlgorithm());
    }
    init();
  }
  ...
  @Override protected boolean isKeySupported(Key key, int operation)
  {
  ...
  @ReplacedByNativeOnDeploy private void init() throws CryptoException
  {
    try {
