...
public abstract class Signature
{
  ...
  /**
   * Initializes this message signature algorithm to sign or verify. Calling
   * this method will also reset the input data buffer.
   *
   * @param operation The operation mode of this signature algorithm
   *     (<code>OPERATION_SIGN</code> or <code>OPERATION_VERIFY</code>).
   * @param key The key.
   *
   * @throws CryptoException If one or more initialization parameters are
   *     invalid or the signature algorithm fails to initialize with the given
   * parameters.
   */
  public final void reset(int operation, Key key) throws CryptoException
  {
    if (operation < OPERATION_SIGN || operation > OPERATION_VERIFY) {
      throw new CryptoException("Invalid or unsupported signature operation: " +
                                operation);
    }
  ...
  /**
   * Finalizes the sign operation by processing all the accumulated input data
  ...
  /**
   * Finalizes the verify operation by processing all the accumulated input data
   * and returning the signature comparison result.
   *
