...
public abstract class Digest
{
   ...
   */
  @Override public String toString() { return getAlgorithm(); }
  ...
  /**
   * Initializes this message digest. Calling this method will also reset the
   * input data buffer.
   */
  public final void reset() { input.reset(); }

  /**
   * Updates the input data that will be processed by this message digest
   * algorithm. The data will be accumulated in an input buffer to be processed
  ...
  /**
   * Finalizes the message digest operation by processing all the accumulated
   * input data and returning the result in a new buffer.
   *
   * @return The operation result in a new buffer.
   */
  public final byte[] getDigest() { return process(input.toByteArray()); }
