...
{
  public static Logger getLogWriter() { return null; }
  ...
  public static Connection getConnection(String url) throws SQLException
  {
    if (url.startsWith("jdbc:sqlite:")) { // :sample.db
      ...
      if ((Settings.isIOS() || Settings.platform.equals(Settings.ANDROID)) &&
          dbname.indexOf("/") == -1 && dbname.indexOf(":memory:") == -1 &&
          dbname.indexOf("mode=memory") ==
            -1) { // dont use this for memory databases
        // in ios and android its required that the user specify a valid path.
        // if he don't, we put the database in the app path
  ...
  static Connection newConnection(String url, String dbname) throws SQLException
  {
    ...
    return new SQLite4JConnection(
      new org.sqlite.SQLiteConnection(url, dbname, props));
  }

  static Connection newConnection4D(String url, String dbname)
    throws SQLException
  {
  ...
  public static Driver[] getDrivers() { return null; }
