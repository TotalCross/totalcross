 ...
 *
 * The Initial Developer of the Original Code is Bruno Lowagie. Portions created
 *by the Initial Developer are Copyright (C) 1999, 2000, 2001, 2002 by Bruno
 *Lowagie. All Rights Reserved. Co-Developer of the code is Paulo Soares.
 *Portions created by the Co-Developer are Copyright (C) 2000, 2001, 2002 by
 *Paulo Soares. All Rights Reserved.
 *
 * Derived Code is 'jbars, a free JAVA barcode generation library'
 * The initial developer of jbars is Andrés Ederra. Portions created by the
 *initial Developer of jbars are Copyright (C) 2003 by Andrés Ederra
 **/
 ...
 * </pre>
 *
 * This class is based on iText "A Free Java-PDF library by Bruno Lowagie and
 * Paulo Soares" using the Mozilla Public License(MPL)
 *
...
public class Barcode93 extends Barcode
{
   ...
   */
  static byte BARS[][] = {
    { 1, 3, 1, 1, 1, 2 }, { 1, 1, 1, 2, 1, 3 }, { 1, 1, 1, 3, 1, 2 },
    { 1, 1, 1, 4, 1, 1 }, { 1, 2, 1, 1, 1, 3 }, { 1, 2, 1, 2, 1, 2 },
    { 1, 2, 1, 3, 1, 1 }, { 1, 1, 1, 1, 1, 4 }, { 1, 3, 1, 2, 1, 1 },
    { 1, 4, 1, 1, 1, 1 }, { 2, 1, 1, 1, 1, 3 }, { 2, 1, 1, 2, 1, 2 },
    { 2, 1, 1, 3, 1, 1 }, { 2, 2, 1, 1, 1, 2 }, { 2, 2, 1, 2, 1, 1 },
    { 2, 3, 1, 1, 1, 1 }, { 1, 1, 2, 1, 1, 3 }, { 1, 1, 2, 2, 1, 2 },
    { 1, 1, 2, 3, 1, 1 }, { 1, 2, 2, 1, 1, 2 }, { 1, 3, 2, 1, 1, 1 },
    { 1, 1, 1, 1, 2, 3 }, { 1, 1, 1, 2, 2, 2 }, { 1, 1, 1, 3, 2, 1 },
    { 1, 2, 1, 1, 2, 2 }, { 1, 3, 1, 1, 2, 1 }, { 2, 1, 2, 1, 1, 2 },
    { 2, 1, 2, 2, 1, 1 }, { 2, 1, 1, 1, 2, 2 }, { 2, 1, 1, 2, 2, 1 },
    { 2, 2, 1, 1, 2, 1 }, { 2, 2, 2, 1, 1, 1 }, { 1, 1, 2, 1, 2, 2 },
    { 1, 1, 2, 2, 2, 1 }, { 1, 2, 2, 1, 2, 1 }, { 1, 2, 3, 1, 1, 1 },
    { 1, 2, 1, 1, 3, 1 }, { 3, 1, 1, 1, 1, 2 }, { 3, 1, 1, 2, 1, 1 },
    { 3, 2, 1, 1, 1, 1 }, { 1, 1, 2, 1, 3, 1 }, { 1, 1, 3, 1, 2, 1 },
    { 2, 1, 1, 1, 3, 1 }, { 1, 2, 1, 2, 1, 1 }, { 3, 1, 2, 1, 1, 1 },
   ...
   */
  public static String ASCII_CHARS =
    "&U" +
    ...
    " %" + //%		It's also valid to use "#E" but it uses an extra
           // char
    "#F" + //&
    "#G" + //'
    "#H" + //(
    "#I" + //)
    "#J" + //*
    " +" + //+		It's also valid to use "#K" but it uses an extra
           // char
    "#L" + //
    " -" + //-		It's also valid to use "#M" but it uses an extra
           // char
    " ." + //.		It's also valid to use "#N" but it uses an extra
           // char
    " /" + ///		It's also valid to use "#O" but it uses an extra
           /// char
    " 0" + // 0		It's also valid to use "#P" but it uses an extra
           // char
    " 1" + // 1		It's also valid to use "#Q" but it uses an extra
           // char
    " 2" + // 2		It's also valid to use "#R" but it uses an extra
           // char
    " 3" + // 3		It's also valid to use "#S" but it uses an extra
           // char
    " 4" + // 4		It's also valid to use "#T" but it uses an extra
           // char
    " 5" + // 5		It's also valid to use "#U" but it uses an extra
           // char
    " 6" + // 6		It's also valid to use "#V" but it uses an extra
           // char
    " 7" + // 7		It's also valid to use "#W" but it uses an extra
           // char
    " 8" + // 8		It's also valid to use "#X" but it uses an extra
           // char
    " 9" + // 9		It's also valid to use "#Y" but it uses an extra
           // char
    "#Z" + //:
    "&F" + //;
    "&G" + //<
  ...
  public Barcode93()
  {
    ...
    // As we are working in a discrete environment, we can only use 2:1 or 3:1
    // ratio We choose 3:1 ratio because it is valid for all sizes, and it is
    // less error prone when doing image scalation operations        setFont(new
   ...
   */
  @Override public int getBarcodeWidth()
  {
  ...
  /**
   * Creates the bars.
   * @param text the text to create the bars. This text does not include the
   *     start and
   * stop characters
   * @return the bars
   */
  static byte[] getBarsCode93(String text)
  {
    ...
    for (k = 0; k < text.length(); ++k) {
      ...
      if (idx < 0) {
        throw new IllegalArgumentException("The character '" + text.charAt(k) +
    ...
    for (int k = checksum.length() - 1; k >= 0; k--) {
      ...
      if (idx < 0) {
        ...
        throw new IllegalArgumentException(
          "The character '" + checksum.charAt(k) + "' is illegal in code 93.");
      }
      sum += (idx) * (weight + 1);
      weight = (weight + 1) % 20;
    }
   ...
   *    <TD><P>bars painted with current color<br>text painted with
   * <CODE>textColor</CODE></TD>
   *    </TR>
   * <TR>
   *    <TD><P><CODE>barColor</CODE></TD>
   *    <TD><P><CODE>textColor</CODE></TD>
   *    <TD><P>bars painted with <CODE>barColor</CODE><br>text painted with
   * <CODE>textColor</CODE></TD>
   *    </TR>
   * </TABLE>
   ...
   */
  @Override public void placeBarcode(Image i, int barColor, int textColor)
  {
    ...
    for (int k = 0; k < bars.length; ++k) {
      ...
      if (print) {
        ...
        g.fillRect(quietZone + scale(imageX, fullWidth, barStartX),
