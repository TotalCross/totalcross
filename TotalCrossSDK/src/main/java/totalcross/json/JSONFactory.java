...
/**
   ...
   reflection.
    ...
    <pre>
    Car cc = JSONFactory.parse("{\"carro\":{\"id\":-1,\"descricao\":\"GOL\"}}",
   Carro.class);
    </pre>
...
public class JSONFactory
{
           ...
           NoSuchMethodException, SecurityException
  {
    if (classOfT.isArray()) {
      ...
      try {
        JSONArray jsonArray = new JSONArray(json);
        array = classOfT.cast(
          Array.newInstance(classOfT.getComponentType(), jsonArray.length()));
        for (int i = jsonArray.length() - 1; i >= 0; i--) {
          Array.set(
            array,
            i,
            parse(jsonArray.getJSONObject(i), classOfT.getComponentType()));
        }
      } catch (JSONException e) {
        array =
          classOfT.cast(Array.newInstance(classOfT.getComponentType(), 1));
        Array.set(
          array, 0, parse(new JSONObject(json), classOfT.getComponentType()));
      }
      return array;
    }
  ...
  private static <T> T parse(Object outerObject,
                             JSONArray jsonArray,
                             Class<T> classOfT)
    throws InstantiationException, IllegalAccessException,
           IllegalArgumentException, InvocationTargetException, JSONException,
        ...
        for (int i = jsonArray.length() - 1; i >= 0; i--) {
          Array.set(array,
                    ...
                    parse(outerObject,
                          ...
                          classOfT.getComponentType()));
        }
      } catch (JSONException e) {
        array =
          classOfT.cast(Array.newInstance(classOfT.getComponentType(), 1));
        Array.set(
          array, 0, parse(outerObject, jsonArray, classOfT.getComponentType()));
      }
      return array;
    }
           ...
           NoSuchMethodException, SecurityException
  {
    ...
    try {
      ...
      if (outerObject != null &&
          classOfT.getName().indexOf(outerObject.getClass().getName()) != -1) {
        Constructor<T> constructorOfT =
          classOfT.getDeclaredConstructor(outerObject.getClass());
    ...
    for (Method method : methods) {
      ...
      if (paramTypes != null && paramTypes.length == 1 &&
        ...
        if (jsonObject.has(originalName)) {
          ...
          // not found as-is or lowercased? try replacing camel case with
          // underscore
          /*
           * originally done using regex, but totalcross implementation has some
           * bugs and until they are fixed this is done looping through the
           * characters originalName.replaceAll("(.)(\\p{Upper})",
           * "$1_$2").toLowerCase();
           */
        ...
        if (!jsonObject.isNull(name)) {
          ...
          if (parameterType.isPrimitive()) {
            ...
            method.invoke(
              object,
              parse(object, jsonObject.getJSONArray(name), parameterType));
          } else {
            method.invoke(
              object,
              parse(object, jsonObject.getJSONObject(name), parameterType));
          }
        }
      }
    }
