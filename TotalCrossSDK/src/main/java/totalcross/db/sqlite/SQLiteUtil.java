...
public class SQLiteUtil {
	...
	public boolean tableExists(String tab) throws SQLException {
		return isNotEmpty("SELECT name FROM sqlite_master WHERE type='table' AND lower(name)='" +
						  tab.toLowerCase() + "'");
	}
	...
	throws SQLException {
		...
		try {
			prepareStatement =
				connection.prepareStatement("SELECT name FROM sqlite_master WHERE type='index' AND name='"
