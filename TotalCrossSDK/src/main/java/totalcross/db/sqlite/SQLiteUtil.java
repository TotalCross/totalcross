 ...
 * </ul>
 *
 * When using 'between' or any other date/time comparation, the arguments MUST
 * MATCH the type form. So, in "select borndate from test where borndate between
 * '2014-12-10' and '2014-12-14'": <ul> <li> If borndate is a DATE, the
 * comparison will succeed. <li> If borndate is a TIME, the comparison will
 * fail. To make it work, use "select borndate from test where borndate between
 * '2014-12-10 00:00:00.000' and '2014-12-14 00:00:00.000'".
 * </ul>
...
public class SQLiteUtil
{
  ...
  public SQLiteUtil(String table) throws SQLException { this.fullPath = table; }
  ...
  public SQLiteUtil() throws SQLException { this(""); }

  /** Use the given connection in all operations */
  public SQLiteUtil(Connection con) { this.con = con; }
  ...
  /**
   * SQLite has a problem (not sure if its a bug) where it returns DATE for
   * both DATE and DATETIME types, so this method returns the correct
   * correspondence: DATE for date and TIME for DATETIME (remember that in
   * TotalCross, a Time object also contains the date).
  ...
  public int getColumnType(ResultSetMetaData md, int col) throws SQLException
  {
    ...
    return s.equals("DATE")
      ? Types.DATE
      : s.equals("DATETIME") ? Types.TIME : md.getColumnType(col);
  }
  ...
  public String[][] getStrings(ResultSet rs, Vector v, int[] decimalPlaces)
    throws SQLException
  {
    ...
    while (rs.next()) {
      ...
      for (int i = 0; i < cols; i++) {
        switch (types[i]) {
          ...
          case Types.DOUBLE:
            linha[i] =
              Convert.toString(rs.getDouble(i + 1),
                               decimalPlaces == null ? -1 : decimalPlaces[i]);
            break;
          default:
  ...
  {
    return toSqlite ? Convert.replace(s, "'", "''")
                    : Convert.replace(s, "''", "'");
  }

  /**
   * Changes a date in format 2014-02-19 00:00:00:000 to a
   * totalcross.util.Date.
   */
  public static Date fromSqlDate(String sqldate) throws InvalidDateException
  {
    int sp = sqldate.indexOf(' ');
    return new Date(sp == -1 ? sqldate : sqldate.substring(0, sp),
                    Settings.DATE_YMD);
  }

  /**
   * Rebuild and shrink the entire database, like the old Litebase's
   * <code>purge</code> method, but in this case it applies to all tables.
   */
   ...
   * @throws SQLException
   */
  public static final boolean existsIndex(String tableName,
                                          String indexName,
