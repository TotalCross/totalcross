 ...
 * <p>
 * Note: the color used in the setBackground method will be used in the
 * scrollbar only. The background color of the control will be a lighter version
 * of the given color. <p> Note: this is a special version for the
 * LitebaseConnection. It accepts a String matrix as the input, and does not
 * allow add/remove/set of elements, since it reflects the database and must be
 * filled again after some row is updated. <p> Here is an example showing how it
 * can be used:
 *
 * <pre>
 * import litebase.ui.*;
 ...
 * public void initUI()
 * {
 *   ResultSet rs = driver.executeQuery("select rowid,name from person where
 * name > 20"); rs.first(); lb = new DBListBox(rs.getStrings(-1,true,false),1);
 * // display column 1 add(lb,LEFT,TOP);
 * }
 *
 ...
 *          if (event.target == lb)
 *             String[] element = (String[])lb.getSelectedItem(); // in most
 * cases, this is just a String and may be casted to such
 *    }
 * }
 * }
 * </pre>
...
public class DBListBox extends ListBox
{
   ...
   * elements.
   */
  public void setDisplayCol(int displayCol) { this.dataCol = displayCol; }
  ...
  @Override public void add(Object[] items)
  {
  ...
  /** Does nothing */
  @Override public void insert(Object item, int index) {}
  ...
  /**
     ...
     this method will null both ListBoxes
  ...
  /** Does nothing */
  @Override public void remove(int itemIndex)
  ...
  /** Does nothing */
  @Override public void setItemAt(int i, Object s) {}

  /**
   * Get the Object at the given Index. Returns a String array, or null if i is
   * out of range.
   */
  @Override public Object getItemAt(int i)
  {
    if (0 <= i && i < itemCount) {
      return sitems[i];
    }
   ...
   */
  @Override public Object[] getItems() { return sitems; }

  @Override protected Object[] getItemsArray() { return sitems; }
  ...
  /**
   * Select an item and scroll to it if necessary. Note: select must be called
   * only after the control has been added to the container and its rect has
   * been set.
   */
  @Override public void setSelectedIndex(int i)
  {
    if (0 <= i && i < itemCount &&
        i != selectedIndex /* && height != 0*/) { // flsobral@220_14: commented
                                                  // height!=0 otherwise
                                                  // combobox are not properly
                                                  // set. (same problem observed
                                                  // with listbox)
      offset = i;
      ...
      selectedIndex = i;
      sbar.setValue(
        offset); // guich@210_9: fixed scrollbar update when selecting items
      repaint();
  ...
  /** Does nothing */
  @Override protected void find(char c) {}

  /** You can extend ListBox and overide this method to draw the items */
  @Override protected void drawItem(Graphics g, int index, int dx, int dy)
  {
    // Vm. debug(this+" index: "+index+", items.size: "+items.size()+", dx,dy =
    // "+dx+","+dy);
   ...
   */
  @Override protected int getItemWidth(int index)
  {
    if (sitems == null) {
      return 0;
    }
