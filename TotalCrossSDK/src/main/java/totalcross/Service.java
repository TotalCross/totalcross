...
public abstract class Service implements MainClass {
  ...
  public void start() {
    if (ANDROID) {
      String path = serviceApplicationId != null ? "totalcross.app" + serviceApplicationId.toLowerCase()
          : Settings.appPath.substring(Settings.appPath.lastIndexOf('/') + 1); // /data/data/totalcross.appkfcb
