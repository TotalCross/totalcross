...
public abstract class Service implements MainClass
{
    ...
    if (ANDROID) {
      String path =
        serviceApplicationId != null
          ? "totalcross.app" + serviceApplicationId.toLowerCase()
          : Settings.appPath.substring(Settings.appPath.lastIndexOf('/') +
                                       1); // /data/data/totalcross.appkfcb
  ...
  public boolean registerService()
  {
    ...
    try {
      ...
      try {
        Registry.set(Registry.HKEY_LOCAL_MACHINE,
                     regkey,
                     "Dll",
                     "\\" + serviceName + "\\tcvm.dll");
        Registry.set(Registry.HKEY_LOCAL_MACHINE, regkey, "Context", 1);
        Registry.set(
          Registry.HKEY_LOCAL_MACHINE, regkey, "FriendlyName", "TotalCrossSrv");
        ...
        Registry.set(
          Registry.HKEY_LOCAL_MACHINE, regkey, "TCZ", serviceName + ".tcz");
      } catch (Exception ee) {
    ...
    int ret =
      Vm.exec("register service", null, 0, true); // register the service
    return ret == 1;
  }
  ...
  @Override public void appEnding() {}

  @Override public void _onTimerTick(boolean canUpdate) {}
}
