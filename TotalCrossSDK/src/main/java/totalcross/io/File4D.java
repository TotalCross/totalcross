...
public class File4D extends RandomAccessStream implements FileStates
{
           ...
           totalcross.io.FileNotFoundException, totalcross.io.IOException
  {
    ...
    if (mode < DONT_OPEN || mode > CREATE_EMPTY) {
      throw new totalcross.io.IllegalArgumentIOException(
        "mode", Convert.toString(mode));
    }
    if (slot < -1) {
      throw new totalcross.io.IllegalArgumentIOException(
        "slot", Convert.toString(slot));
    }

    path = Convert.normalizePath(path);
    if (path.startsWith("device/")) { // flsobral@tc110_108: added support for
                                      // the alias "device/".
      ...
      slot = 1;
    } else if (isAndroid && !path.startsWith("/") &&
               path.indexOf("data/data") < 0) {
      path = Settings.appPath + "/" + path;
  ...
  public File4D(String path)
    throws IllegalArgumentIOException, totalcross.io.IOException
  {
    this(path, DONT_OPEN, -1);
  }

  @Override native public void close() throws totalcross.io.IOException;
  ...
  {
    if (mode <= DONT_OPEN) {
      throw new IOException(mode == INVALID
                              ? "Invalid file handler"
                              : "Operation cannot be used in mode DONT_OPEN");
    }
  ...
  native public static boolean isCardInserted(int slot)
    throws totalcross.io.IllegalArgumentIOException;

  native public void createDir()
    throws totalcross.io.IOException; // guich@tc122_24: void, not boolean
  ...
  @Override
  native public int readBytes(byte b[], int off, int len)
    throws totalcross.io.IOException;

  native public void rename(String path)
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException;

  @Override native public void setPos(int pos) throws totalcross.io.IOException;

  @Override
  native public int writeBytes(byte b[], int off, int len)
    throws totalcross.io.IOException;

  native public void setAttributes(int attr)
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException;
  ...
  {
    if (Settings.isWindowsCE()) { // flsobral@tc112_10: Fixed to also work on
                                  // devices recognized as WindowsMobile.
      String cardName = null;
      try {
        cardName =
          Registry.getString(Registry.HKEY_LOCAL_MACHINE,
                             "System\\StorageManager\\Profiles\\SDMemory",
                             "Folder"); // flsobral@tc112_15: Attempt to
      ...
      if (cardName != null) {
        cardName = cardName.replace(
          '\\', '/'); // flsobral@tc113_15: Always wrap the path with slashes.
