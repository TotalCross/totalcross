...
public class File4D extends RandomAccessStream implements FileStates {
	...
	public void setPos(int offset, int origin) throws IOException {
		if (mode <= DONT_OPEN) {
			throw new IOException(mode == INVALID ? "Invalid file handler" :
	...
	native public static boolean isCardInserted(int slot) throws
		totalcross.io.IllegalArgumentIOException;

	native public void createDir() throws
		totalcross.io.IOException; // guich@tc122_24: void, not boolean
	...
	native public void rename(String path) throws totalcross.io.IllegalArgumentIOException,
			   totalcross.io.IOException;
	...
	native public void setAttributes(int attr) throws totalcross.io.IllegalArgumentIOException,
			   totalcross.io.IOException;
	...
	public static File getCardVolume() throws totalcross.io.IOException {
		if (Settings.isWindowsCE()) { //flsobral@tc112_10: Fixed to also work on devices recognized as WindowsMobile.
			...
			try {
				cardName = Registry.getString(Registry.HKEY_LOCAL_MACHINE,
