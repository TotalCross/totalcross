 ...
 * 		ByteArrayStream bas = new ByteArrayStream(img64.getBytes()); //
 * get a Stream to read the String InputStream stringInput =
 * bas.asInputStream(); // use ByteArrayStream as a java.io.InputStream
 * compliant object
 *
 * 		Base64decoderStream decoder = new
 * Base64decoderStream(stringInput);
 * // now we can read the original bytes Stream imgSource =
 * Stream.asStream(decoder); // use Base64decoderStream as a
 * totalcross.io.Stream compliant object return new Image(imgSource); // passing
 * the stream directly to the image for the sake of memory
 * }
 * </pre>
 *
...
public class Base64decoderStream extends InputStream
{
  ...
  @Override public long skip(long n) throws IOException
  {
    throw new IOException(new UnsupportedOperationException(
      "totalcross.io.Base64Decoder.skip use is not supported"));
  ...
  private void ensureFetch() throws IOException
  {
    ...
    if (decodedReadPos == decodedSize) {
      ...
      int readFromStream = base64encodedStream.read(
        bytesEncodedRead, oldOverflowSize, BYTES_ENCODED_SIZE);
      ...
      if (readFromStream < 0) {
        ...
        if (!hasPreviousOverflow) {
          ...
          // We still have some leftover from a previous overflow, let's use
          // it...
          overflowSize = 0;
          ...
          decodedReadPos = 0;
          decodedSize =
            Base64.decode(bytesEncodedRead, 0, 4, bytesDecodedRead, 0);
        }
      } else {
        ...
        if (overflowSize != 0) {
          Vm.arrayCopy(bytesEncodedRead,
                       consideredRead - overflowSize,
                       overflowEncodedRead,
                       0,
                       overflowSize);
        }
        ...
        if (usableReadData > 0) {
          try {
            decodedSize = Base64.decode(
              bytesEncodedRead, 0, usableReadData, bytesDecodedRead, 0);
