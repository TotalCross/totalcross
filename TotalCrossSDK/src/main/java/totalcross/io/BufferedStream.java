 ...
 * amounts of data. It works like the CompressedByteArrayStream, however it does
 * not compresses the data like that one.
 *
 * Here's a sample code:
 * <pre>
 *   public void writeLargeFile(String path, byte[] largeData) throws
 * IOException
 *   {
...
public class BufferedStream extends Stream
{
  ...
  /**
   * Creates a new buffered stream given the underlying stream and the mode to
   * use. This constructor will use the default buffer size: 2048 bytes.
   * @param stream The underlying stream.
   * @param mode The mode to use - READ or WRITE.
   * @throws IllegalArgumentIOException if the mode is invalid.
   */
  public BufferedStream(Stream stream, int mode)
  ...
  /**
   * Creates a new buffered stream given the underlying stream, the mode to use
   * and the buffer size.
   * @param stream The underlying stream.
   * @param mode The mode to use - READ or WRITE.
   * @param bufferSize The buffer size.
   * @throws IllegalArgumentIOException if the mode is invalid or the bufferSize
   *     is not
   * a positive number.
    ...
    throws IllegalArgumentIOException, NullPointerException
  {
    ...
    if (bufferSize < 0) {
      throw new IllegalArgumentIOException("bufferSize",
                                           Convert.toString(bufferSize));
    }
    if (stream == null) {
   ...
   */
  public void setStream(Stream f) throws IOException, NullPointerException
  { // guich@tc123_34
    if (f == null) {
   ...
   * per time. Consider using LineReader instead.
   *
