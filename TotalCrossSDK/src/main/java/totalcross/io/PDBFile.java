...
public class PDBFile extends Stream {
	...
	throws totalcross.io.IllegalArgumentIOException, totalcross.io.FileNotFoundException,
		...
		if (st == null || st.length != 3 || st[0].length() > 31 || st[1].length() != 4
			|| st[2].length() != 4) {
	...
	throws totalcross.io.FileNotFoundException, totalcross.io.IOException {
		...
		if (fileBytes == null) { // PDBFile not found?
			if (mode != PDBFile.CREATE
				&& mode != PDBFile.CREATE_EMPTY) { // if not creating, the file was not found.
	 ...
	 */
	final public void rename(String newName) throws totalcross.io.IllegalArgumentIOException,
		totalcross.io.IOException {
		if (openRef == null) {
		...
		if (st == null || st.length != 3 || st[0].length() > 31 || st[1].length() != 4
			|| st[2].length() != 4) {
			throw new totalcross.io.IllegalArgumentIOException("Invalid value for argument 'newName': " +
					newName);
		}
	 ...
	 */
	final public void addRecord(int size) throws totalcross.io.IllegalArgumentIOException,
		totalcross.io.IOException {
		if (openRef == null) {
			throw new totalcross.io.IOException("The pdb file is closed.");
		}
	 ...
	 */
	final public void resizeRecord(int size) throws totalcross.io.IllegalArgumentIOException,
		totalcross.io.IOException {
		if (openRef == null) {
			throw new totalcross.io.IOException("The pdb file is closed.");
		}
	...
	final private void nativeClose() throws totalcross.io.IOException {
		if (!deleted
			&& (modificationNumber != originalModificationNumber || (modificationNumber == 0
					&& getRecordCount() == 0))) {
			writeCB();
		}
	}
	...
	final public static String[] listPDBs(int creatorIdWild, int typeWild) { // guich@330_17
		...
		Vector v = (keys != null ? new Vector(keys) : new
					Vector()); //flsobral@tc111_20a: Avoid NPE if keys is null.
		...
		String path = Launcher.instance.getDataPath() != null ? Launcher.instance.getDataPath() :
		...
		if (s != null) {
			...
			for (int i = 0; i < s.length; i++) {
				if (v.indexOf(s[i]) == -1
					&& (/*s[i].toLowerCase().endsWith("prc") ||*/ s[i].toLowerCase().endsWith("pdb"))) {
					try {
		...
		return (String[])
			   v.toObjectArray(); // guich@200b4_27: fix array cast exception - guich@220_32: now we can use the cast.
	}
	 ...
	 */
	final public void setRecordPos(int pos) throws totalcross.io.IllegalArgumentIOException,
		totalcross.io.IOException {
		if (openRef == null) {
			throw new totalcross.io.IOException("The pdb file is closed.");
		}
	...
	private int readWriteBytes(byte buf[], int start, int count,
		...
		try {
			...
			if (isRead) {
				...
				if (_attrs != null) {
					setRecordAttributes(_hvRecordPos,
										(byte)(getRecordAttributes(_hvRecordPos) | PDBFile.REC_ATTR_DIRTY)); // set dirty
				}
				modificationNumber++;
			}
			...
			System.out
			.println("Exception " + ee + " in _readWriteBytes: rec.length: " + (rec == null ? "null" : "" +
					 rec.length)
					 + ", buf.length: " + buf.length + ", start: " + start + ", count: " + count + ", isRead? " +
					 isRead);
			count = -1;
		}
		return count;
	 ...
	 */
	@Override
	public int skipBytes(int count) throws totalcross.io.IllegalArgumentIOException,
		totalcross.io.IOException {
		if (openRef == null) {
			throw new totalcross.io.IOException("The pdb file is closed.");
		}
		...
		if (off < 0) {
			throw new totalcross.io.IllegalArgumentIOException("Offset cannot underflow the record size: " +
					off);
		}
		if (off > _hvRecordLength) {
			throw new totalcross.io.IllegalArgumentIOException("Offset cannot overflow the record size: " +
					off);
		}
		_hvRecordOffset += count;
		return count;
	 ...
	 * @since SuperWaba 5.5
	 */
	public void setRecordOffset(int ofs) throws totalcross.io.IllegalArgumentIOException,
		totalcross.io.IOException {
		if (openRef == null) {
			throw new totalcross.io.IOException("The pdb file is closed.");
		}
	...
	throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException {
		...
		if (offsetInRec < 0 || offsetInRec > 65535) {
			throw new totalcross.io.IllegalArgumentIOException("Invalid value for argument 'offsetInRec': " +
					offsetInRec);
		}

		Vector records = (Vector) openRef;
		if (recordPos < 0 || recordPos >= records.size()) {
			throw new totalcross.io.IllegalArgumentIOException("Invalid value for argument 'recordPos': " +
					recordPos);
		}
	...
	throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException {
		...
		int lenM1 = length - 1;
		for (int i = Math.max(_hvRecordPos, 0); i < n
			 && found == -1; i++) { // guich@tc110_34: consider recordPos in the search
			int bytesRead;
			try {
	...
	private short attributes =
		PDBFile.DB_ATTR_BACKUP; // guich@330_44: don't force the Attrbackup bc the user may want to clear it
	...
	private void toPDB(OutputStream outStream, Vector records, String creatorId, String dbName,
					   String typeId,
		...
		int numRecords = records == null ? 0 :
						 records.size(); // guich@tc110_35: delete may call writeCB, which then calls toPDB. but records is nulled by delete.
		...
		for (int i = 0; i < numRecords; i++) {
			...
			if (recSize > 65520) {
				Launcher.print("CAUTION! RECORD " + i +
	...
	private Vector fromPDB(byte[] all, String creatorId, String typeId,
						   Vector _attrs) throws totalcross.io.IOException {
		...
		// DatabaseHdrType
		byte name[] = new
		byte[32]; // ps: the written string is in c++ format, so this routine doesnt loads the name correctly (comes trash with it)
		...
		int toSkip = (offset2 -
					  offset); // pad - guich@580_3: fixed sf_1432481  - guich@tc112_22: moved to after the app and sort blocks are read
		is.skipBytes(toSkip);
