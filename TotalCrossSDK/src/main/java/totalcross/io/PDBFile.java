...
/**
 * PDBFile is a collection of records commonly referred to as a database on
 * small devices. <p> Here is an example showing data being read from records in
 * a PDBFile:
 *
 * <pre>
 * try
 ...
 * }
 * // since the exceptions are being treated the same way, we could just catch
 ...
 *
 * <p>
 * Note: a PDBFile cannot be opened twice. You must close one instance before
 * opening the other or share the same instance. <p> In Windows, you can specify
 * the path of the pdb file, either absolute ("\\My Documents\\Test") or
 * relative
 * ("..\\..\\Test"). Note that the Settings.dataPath, if set, is also used as
...
public class PDBFile extends Stream
{
  ...
  /**
   * Non-destructive create open mode. If the database doesn't exist, it is
   * created and stays in READ_WRITE mode. Otherwise, if it does exists, it is
   * <b>not</b> erased; it just stays in READ_WRITE mode.
   * @see #PDBFile(String, int)
   */
  public static final int CREATE = 4;
  /**
   * Destructive-create open mode. If the file exists, it is deleted, then a
   * new file is created, and the mode is changed to READ_WRITE.
   * @see #PDBFile(String, int)
   */
  ...
  /**
   * Set if Application Info block is dirty. Optionally supported by an App's
   * conduit
   * @see #setAttributes(int)
   */
  ...
  /**
   * This tells the backup conduit that it's OK for it to install a newer
   * version of this database with a different name if the current database is
   * open. This mechanism is used to update the Graffiti Shortcuts database, for
   * example.
   * @see #setAttributes(int)
   */
  ...
  /**
   * This database is used for file stream implementation. This attribute may
   * not be used.
   * @see #setAttributes(int)
   */
  ...
  /**
   * Use this in order to explicitly release a record through the
   * setRecordAttributes method. This is a hack for a bug in the NVFS of Palm
   * OS 5.4.9, which does not release locked records during the boot (reset).
   * You can use the following code in order to clear the locked records: <pre>
   ...
   * }
   * </pre>
   * Note that this code assume that the PDBFile was NOT being used previously
   * and that there are no records being used. You must also close the PDBFile
   * after and open it again. Ignoring these restrictions will likely reset the
   * device.
  ...
  /**
   * Record atribute: Deleted. Note that after a record has it delete attribute
   * set you don't have access to it anymore.
   * @see #setRecordAttributes(int, byte)
   */
   ...
   * <p>
   * Note that some file types are blocked from opening on Palm OS. These types
   * are appl, psys and rsrc ones, and also the resource databases. Hacking the
   * system to allow it to access these files may simply reboot the device.
   * @param name PDBFile name, allowing only letters, digits and '_'. Must
   *     contain the creator id and type.
           ...
           totalcross.io.FileNotFoundException, totalcross.io.IOException
  {
    if (name == null) {
      throw new java.lang.NullPointerException(
        "Argument 'name' cannot have a null value.");
    }
    if (mode < 3 || mode > 5) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Invalid value for argument 'mode': " + mode);
    }

    String[] st = Convert.tokenizeString(name, '.');
    if (st == null || st.length != 3 || st[0].length() > 31 ||
        st[1].length() != 4 || st[2].length() != 4) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Invalid value for argument 'name': " + name +
    ...
    throws totalcross.io.FileNotFoundException, totalcross.io.IOException
  {
    ...
    if (fileBytes == null) { // PDBFile not found?
      if (mode != PDBFile.CREATE &&
          mode !=
            PDBFile.CREATE_EMPTY) { // if not creating, the file was not found.
        openRef = null; // don't let the finalizer close an empty db!
        throw new totalcross.io.FileNotFoundException(
          "Could not find the pdb file: " + this.name);
      }
    } else {
    ...
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    ...
    if (newName == null) {
      throw new java.lang.NullPointerException(
        "Argument 'newName' cannot have a null value.");
    }
    ...
    if (st == null || st.length != 3 || st[0].length() > 31 ||
        st[1].length() != 4 || st[2].length() != 4) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Invalid value for argument 'newName': " + newName);
    }

    // update the htOpenedAt with the new name, otherwise the file will be saved
    // in a different location
    String path = removePDBFileFromDisk(); // guich@400_2: putted the delete
                                           // routine in a method.
   ...
   */
  final public void addRecord(int size)
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    if (openRef == null) {
      throw new totalcross.io.IOException("The pdb file is closed.");
    ...
    if (size < 0 || size > 65535) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Invalid value for argument 'size': " + size);
    }
   ...
   * <p><i>Historic notes: this was the first change implemented by Guich in
   * Waba in 06/30/2000.</i>
   *
   * @param size The size in bytes of the record to add
   * @param pos The position where to add the record. You can specify
   *     getRecordCount() to add it to the end.
   * Cannot be lower than 0.
   * @throws totalcross.io.IOException If the operation could not be completed.
   * @throws totalcross.io.IllegalArgumentIOException If size or pos have
   *     invalid values.
   */
    ...
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    ...
    if (pos < 0 || pos > records.size()) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Invalid value for argument 'pos': " + pos);
    }
  ...
  /**
   * Resizes a record. This method changes the size (in bytes) of the current
   * record, which is specified using setRecordPos. The contents of the existing
   * record are preserved if the new size is larger than the existing size. If
   * the new size is less than the existing size, the contents of the record are
   * also preserved but truncated to the new size.
   * @param size the new size of the record
   * @throws totalcross.io.IOException When the method fails.
   */
  final public void resizeRecord(int size)
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    if (openRef == null) {
      throw new totalcross.io.IOException("The pdb file is closed.");
    ...
    if (_hvRecordPos == -1) {
      throw new totalcross.io.IOException(
        "No record selected for this operation.");
    }
  ...
  /**
   * Closes the PDBFile. If you don't close the PDBFile, it will be closed when
   * it gets garbage collected. Note that in desktop the PDBFile is written to
   * disk only after it is closed; during the program's execution, it is held in
   * main memory.
   * @throws IOException If the PDBFile was already closed.
   */
  @Override public void close() throws totalcross.io.IOException
  {
    if (openRef == null) {
      throw new totalcross.io.IOException("The pdb file is closed.");
    }
  ...
  {
    if (!deleted && (modificationNumber != originalModificationNumber ||
                     (modificationNumber == 0 && getRecordCount() == 0))) {
      writeCB();
    }
  }

  private void writeCB() throws totalcross.io.IOException
  { // guich@552_19
    OutputStream os = Launcher.instance.openOutputStream(
      fileName); // guich@552_19: write directly to the stream
    if (os != null) {
  ...
  /**
   * Returns the complete list of existing PDBFiles. <b>If no PDBFiles exist,
   * this method returns null.</b> In the desktop, it searches in the already
   * opened files and also the pdb files in the current "." folder or at
   * Settings.dataPath folder, if it was set.
   * @return A String array with the names
   */
  public static String[] listPDBs() { return listPDBs(0, 0); }

  /**
   * Returns the list of existing PDBFiles with the given creator id and/or
   * type. If no PDBFiles exist, this method returns null. In the desktop, it
   * searches in the already opened files and also the pdb files in the current
   * "." directory. To convert the four characters of a creator id or type into
   * an int, use, you can use the function Convert.chars2int.
   ...
   */
  final public static String[] listPDBs(int creatorIdWild, int typeWild)
  {                                                       // guich@330_17
    ...
    Vector v =
      (keys != null
         ? new Vector(keys)
         : new Vector()); // flsobral@tc111_20a: Avoid NPE if keys is null.
    ...
    if (creatorIdWild != 0 || typeWild != 0) {
      for (int i = v.size(); --i >= 0;) {
        ...
        if ((creatorIdWild != 0 && creatorIdWild != crtrI) ||
            (typeWild != 0 && typeWild != typeI)) {
          v.removeElementAt(i);
    ...
    String path = Launcher.instance.getDataPath() != null
                    ? Launcher.instance.getDataPath()
                    : "."; // guich@570_21: use datapath if assigned
    ...
    if (s != null) {
      ...
      for (int i = 0; i < s.length; i++) {
        if (v.indexOf(s[i]) == -1 &&
            (/*s[i].toLowerCase().endsWith("prc") ||*/ s[i]
               .toLowerCase()
               .endsWith("pdb"))) {
          try {
            // get the type and creator for this file
            java.io.FileInputStream fis =
              new java.io.FileInputStream(new java.io.File(path, s[i]));
            int readLength = fis.read(buf);
            if (buf.length == readLength) {
              int typeI =
                makeInt(buf[60], buf[61], buf[62], buf[63]); // guich@330_17
              ...
              String type = "" + (char)buf[60] + (char)buf[61] + (char)buf[62] +
                            (char)buf[63];
              String crtr = "" + (char)buf[64] + (char)buf[65] + (char)buf[66] +
                            (char)buf[67];
              String version = "" + ((buf[34] << 8) | buf[35]);
              String fullName =
                s[i].substring(0, s[i].indexOf('.')) + '.' + crtr + '.' + type;
              if (includeVersion) {
                fullName += "#" + version;
              }
    ...
    return (String[])
      v.toObjectArray(); // guich@200b4_27: fix array cast exception -
                         // guich@220_32: now we can use the cast.
  }
  ...
  /**
   * Sets the current record position and locks the given record. The value -1
   * can be passed to unset and unlock the current record. If the operation is
   * succesful, the read/write cursor is set to the beggining of the given
   * record.
   *
   * @param pos Record to be locked for use. Must be between 0 and the current
   *     number of records, or -1.
   * @throws totalcross.io.IllegalArgumentIOException If the argument pos has an
   *     invalid value.
   * @throws totalcross.io.IOException If the method fails
   */
  final public void setRecordPos(int pos)
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    if (openRef == null) {
      throw new totalcross.io.IOException("The pdb file is closed.");
    ...
    throws totalcross.io.IOException
  {
    ...
    try {
      ...
      if (isRead) {
        ...
        if (_attrs != null) {
          setRecordAttributes(_hvRecordPos,
                              (byte)(getRecordAttributes(_hvRecordPos) |
                                     PDBFile.REC_ATTR_DIRTY)); // set dirty
        }
        modificationNumber++;
      }
      ...
      System.out.println("Exception " + ee +
                         " in _readWriteBytes: rec.length: " +
                         (rec == null ? "null" : "" + rec.length) +
                         ", buf.length: " + buf.length + ", start: " + start +
                         ", count: " + count + ", isRead? " + isRead);
      count = -1;
    }
    return count;
  }
   ...
   * the cursor in the current record (where read and write operations start
   * from) is advanced by the number of bytes read.
   * @param buf the byte array to read data into
   * @param start the start position in the <code>buf</code> array
   * @param count the number of bytes to read
   * @return The number of bytes read
   * @throws IOException If you request more bytes than available or if the
   *     PDBFile is closed.
   */
  @Override
  final public int readBytes(byte buf[], int start, int count)
    throws totalcross.io.IOException
  {
    if (openRef == null) {
      throw new totalcross.io.IOException("The pdb file is closed.");
    ...
    if (buf == null) {
      throw new java.lang.NullPointerException(
        "Argument 'buf' cannot have a null value.");
    }
  ...
  /**
   * Changes the internal read/write cursor of the current record in a number of
   * bytes. It can be negative but cannot underflow neither overflow the
   * record's size.
   *
    ...
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    ...
    if (off < 0) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Offset cannot underflow the record size: " + off);
    }
    if (off > _hvRecordLength) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Offset cannot overflow the record size: " + off);
    }
    _hvRecordOffset += count;
    return count;
  }
    ...
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    ...
    if (ofs < 0) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Invalid value for argument 'ofs': " + ofs);
    }
    if (_hvRecordPos == -1) {
      throw new totalcross.io.IOException(
        "No record selected for this operation.");
   ...
   */
  @Override
  final public int writeBytes(byte buf[], int start, int count)
    throws totalcross.io.IOException
  {
    if (openRef == null) {
      throw new totalcross.io.IOException("The pdb file is closed.");
  ...
  /**
   * Inspects a record, even if its locked. The cursor is not advanced, neither
   * the current record position. This method can be used to load a locked
   * record. The number of bytes read will be <i>buf.length</i> or the record's
   ...
   * @return the number of bytes read, which can be different of buf.length if
   *     buf.length is greater than the record size.
   *
   * @since SuperWaba 1.1
   * @throws totalcross.io.IOException When the method fails.
   */
    ...
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    ...
    if (offsetInRec < 0 || offsetInRec > 65535) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Invalid value for argument 'offsetInRec': " + offsetInRec);
    }

    Vector records = (Vector)openRef;
    if (recordPos < 0 || recordPos >= records.size()) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Invalid value for argument 'recordPos': " + recordPos);
    }
   ...
   *
   * @param recordPos the record position from where the record attributes will
   *     be retrieved.
  ...
  /**
   * Sets the attributes of the given record. Note that the current record, if
   * modified after you call this method, will have its attributes changed.
   *
   * @param recordPos the record position from where the record attributes will
   *     be retrieved.
   ...
   * original attributes must be retrieved prior applying this value. If not,
   * you can loose your database and your app will crash.
   *
   ...
   * Note that its not possible to search using case insensitive, due to the
   * obvious reason that the byte array may not represent a String.
   *
   * @param toSearch The byte array used to compare with the contents of each
   *     record.
   * @param length   How many bytes will be searched inside
   *     <code>toSearch</code>
    ...
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    ...
    if (toSearch == null) {
      throw new java.lang.NullPointerException(
        "Argument 'toSearch' cannot have a null value.");
    }
    if (offsetInRec < 0) {
      throw new totalcross.io.IllegalArgumentIOException(
        "Invalid value for argument 'offsetInRec': " + offsetInRec);
    ...
    int lenM1 = length - 1;
    for (int i = Math.max(_hvRecordPos, 0); i < n && found == -1;
         i++) { // guich@tc110_34: consider recordPos in the search
      int bytesRead;
      try {
        bytesRead = inspectRecord(buf, i, offsetInRec);
        if (bytesRead >= length && buf[lenM1] == toSearch[lenM1] &&
            buf[0] == toSearch[0]) {
          found = i;
          if (length > 2) {
  ...
  private String removePDBFileFromDisk()
  {
    ...
    if (!f.exists()) {
      ...
      if (!f.exists() && Launcher.instance.getDataPath() !=
                           null) { // guich@450_: test also with dataPath
    ...
    // guich@400_3: if the file was created in the same session it is being
    // deleted, it may not have been written to disk, so we can simply return as
    // if it was created.
    if (f.exists()) {
      try {
        f.delete();
   ...
   * . categoryLabel An array of 16 null-terminated category labels, each of
   * which is 16 bytes long.<br> . categoryID An array of 16 category ID values,
   * each of which is one byte long.<br> . lastUniqID The last unique category
   * ID assigned.<br> . padding Unused.<br>
   *
   */
  private byte[] appInfoBlock; // guich@200b4: store here the appinfoblock (with
                               // the categories)
  ...
  private short attributes =
    PDBFile.DB_ATTR_BACKUP; // guich@330_44: don't force the Attrbackup bc the
   ...
   * start of the optional, application-specific appInfo block. This value is
   * set to NULL for databases that do not include an appInfo block.
   */
   ...
   * NULL for databases that do not include a sortInfo block.
   */
  private int sortInfoOffset;
  /**
   * Used internally by the Palm OS to generate unique identifiers for records
   * on the Palm device when the database is loaded into the device. For PRC
   * databases, this value is normally not used and is set to 0. For PQA
   * databases, this value is not used, and is set to 0.
   */
   ...
   * <b>Important!</b> In SuperWaba, this type of database is not supported!
   */
  ...
  /**
   * converts the records to a pdb file. @param records a vector of array of
   * bytes
   */
  private void toPDB(OutputStream outStream,
                     Vector records,
                     String creatorId,
                     String dbName,
                     String typeId,
                     Vector _attrs) throws totalcross.io.IOException
  {
    ...
    int slash = -1;
    if ((slash = dbName.lastIndexOf('\\')) != -1 ||
        (slash = dbName.lastIndexOf('/')) != -1) {
    ...
    int numRecords =
      records == null
        ? 0
        : records.size(); // guich@tc110_35: delete may call writeCB, which
                          // then calls toPDB. but records is nulled by delete.
    ByteArrayStream bas = new ByteArrayStream(
      8192); // guich@552_19: write the header to a temp bytearray
    ...
    for (int i = 0; i < numRecords; i++) {
      ...
      if (_attrs == null || i >= _attrs.size()) {
        ...
        os.writeByte(((Byte)_attrs.items[i]).byteValue() &
                     ~0x20); // guich@300_64: disable all busy attributes
      }
      ...
      if (recSize > 65520) {
        Launcher.print(
          "CAUTION! RECORD " + i +
          " HAS A SIZE GREATER THAN 65520 AND WILL BE REJECTED BY PALM OS!"); // guich@230_12
      }
      offset += recSize;
    }
    os.writeShort(0); // pad
  ...
  private Vector fromPDB(byte[] all,
                         String creatorId,
                         String typeId,
                         Vector _attrs) throws totalcross.io.IOException
  {
    // ps: i had to read everything to an array because reading one record at a
    // time was reading trash.
    DataStream is = new DataStream(new ByteArrayStream(all));

    if (all.length == 0) {
      throw new totalcross.io.IOException(
        "Invalid or corrupted PDBFile with length 0.");
    }
    // DatabaseHdrType
    byte name[] = new byte[32]; // ps: the written string is in c++ format, so
                                // this routine doesnt loads the name correctly
                                // (comes trash with it)
    ...
    if (all.length >= 8 &&
        new String(all, 0, 8).trim().equalsIgnoreCase("<HTML>")) {
    ...
    if (!creatorOk && !typeOk) {
      throw new totalcross.io.IOException(
        "Database already exists with different creator and type.");
    }
    if (!creatorOk) {
      throw new totalcross.io.IOException(
        "Database already exists with different creator.");
    }
    if (!typeOk) {
      throw new totalcross.io.IOException(
        "Database already exists with different type.");
    }
    ...
    recOffsets[numRecords] = all.length; // add the total size so we can compute
    ...
    // guich@200b4: read the appInfoBlock and sortInfoBlock and store it in a
    // safe place
    if (appInfoOffset > 0) {
      int len = sortInfoOffset > 0 ? (sortInfoOffset - offset)
                                   : (recOffsets[0] - offset);
    ...
    int toSkip =
      (offset2 -
       offset); // pad - guich@580_3: fixed sf_1432481  - guich@tc112_22:
                // moved to after the app and sort blocks are read
    is.skipBytes(toSkip);
    ...
    int size = 0;
    for (int i = 0; i < numRecords; i++) {
      if (_attrs != null && (((Byte)_attrs.items[i]).byteValue() &
                             PDBFile.REC_ATTR_DELETE) != 0) {
        _attrs.removeElementAt(i);
      } else {
        size = recOffsets[i + 1] - recOffsets[i];
        if (size <= 0) {
          throw new totalcross.io.IOException("Invalid record " + i +
                                              "! Size <= 0 (" + size + ")");
        }
