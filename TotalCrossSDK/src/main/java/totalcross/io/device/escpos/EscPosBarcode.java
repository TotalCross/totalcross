...
public class EscPosBarcode implements EscPosPrintObject
{
  ...
  public EscPosBarcode(final EscPosBarcodeType type, final CharSequence data)
  {
    ...
    if (type.lengthMin == type.lengthMax &&
        this.data.length != type.lengthMin) {
      throw new IllegalArgumentException("The length of " + type.toString() +
                                         " barcode data must be " +
                                         type.lengthMin + " symbols");
    } else if (this.data.length < type.lengthMin ||
               this.data.length > type.lengthMax) {
      throw new IllegalArgumentException(
        "The length of barcode data must be between " + type.lengthMin +
        " and " + type.lengthMax + " symbols");
    }
  }
   ...
   *
   * <p>The original ESC/POS Manual defines the bar code height in dots, but the
   * actual size of a dot and the default value for n and may differ for each
   * device.
   *
   * <p>For instance, the default bar code height for devices with dot size of
   * 0.125mm (1/203 inches) is 162.
   *
   ...
   *
   * <p>The units for n depends on the printer model - usually 2 ≤ n ≤ 6 with
   * default value 3.
   *
