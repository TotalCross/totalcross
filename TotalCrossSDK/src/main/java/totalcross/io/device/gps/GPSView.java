...
/**
 * Control that display GPS coordinates read from the COM (or Bluetooth, or IR)
 * port. In Windows Mobile and Android, it uses the native API instead of
 ...
 * &amp; Location / Enable GPS satellites. The other platforms may require that
 * as well.
 *
 ...
 *
 * This class uses the GPS class to show the values on Labels. You can use the
 * GPS class standalone if you wish.
 *
...
{
  /**
   * String that will be printed with there longitude can't be acquired. You
   * can localize this string.
   */
  ...
  public GPSView() throws IOException { this(2000); }

  /**
   * Constructs a GPSView using the given read interval.
   *
   * @param readInterval
   *           The interval used to fetch data, in milliseconds. A
   * ControlEvent.PRESSED is posted each time the TRIGGERED event occurs.
   * @throws IOException
   */
  ...
  private void showGPSData()
  {
    ...
    text[0].setText("lat: " + degrees + " " + minutes + " " + seconds +
                    (lat < 0 ? " S" : " N"));
    ...
    text[1].setText("lon: " + degrees + " " + minutes + " " + seconds +
                    (lon < 0 ? " W" : " E"));
  ...
  @Override public int getPreferredWidth() { return FILL; }
