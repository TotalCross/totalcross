...
/**
 * Class that retrieves GPS coordinates read from the COM (or Bluetooth, or IR)
 * port. Windows Mobile, Android and iOS use the native API instead of reading
 * from the COM port.
 *
 * This class only retrieves data updating the internal fields. If you want to
 * display that data, you may use the GPSView class.
 *
 * For example:
 *
 * <pre>
 ...
 * </pre>
 *
 * If the GPS fails connecting to the satellites, and the phone has signal, you
 * can use the cell tower location as a rough location. The precision is vary
 * between 50m to 3km, depending where the phone is. You can get the latitude
 * and longitude using CellInfo.toCoordinates.
 *
...
{
  /**
   * Stores the location - latitude on first index (0) and longitude on second
   * index (1). On low signal, it contains the value <CODE>INVALID</code>.
   */
   ...
   * if HIGH, only the GPS is used, if LOW, then Google Play Services is also
   * used. Be aware that Google Play Services may return wifi and radio antenna
   * values, with pdop ranging from 30m to 1500m or more; always check the pdop
   * value and discard or store it.
   *
   * Note that this is set at the constructor, and changing it after will have
   * no effect; you must create a new GPS instance.
   *
  ...
  private static boolean nativeAPI =
    Settings.isWindowsCE() || Settings.platform.equals(Settings.ANDROID) ||
    Settings.isIOS();
  private static boolean isOpen;
  boolean dontFinalize;

  /**
   * Returns the Windows CE GPS COM port, which can be used to open a
   * PortConnector. Sample:
   *
   * <pre>
   * String com;
   * if (Settings.isWindowsDevice() && (com = GPS.getWinCEGPSCom()) != null)
   *    sp = new PortConnector(Convert.chars2int(com), 9600, 7,
   * PortConnector.PARITY_EVEN, 1);
   * </pre>
   *
   * @return A string like "COM3", or null if no keys with GPS was found under
   *     HKLM\Drivers\BuildIn.
   */
  ...
  { // guich@tc100b5_38
    try {
      try { // guich@tc120_51
        String key =
          "System\\CurrentControlSet\\GPS Intermediate Driver\\Multiplexer\\ActiveDevice";
        String prefix =
          Registry.getString(Registry.HKEY_LOCAL_MACHINE, key, "Prefix");
      ...
      String[] keys =
        Registry.list(Registry.HKEY_LOCAL_MACHINE, "Drivers\\BuiltIn");
      for (int i = 0; i < keys.length; i++) {
        ...
        if (k.indexOf("serial") >= 0) {
          ...
          try {
            prefix =
              Registry.getString(Registry.HKEY_LOCAL_MACHINE, key, "Prefix");
            name = Registry.getString(
              Registry.HKEY_LOCAL_MACHINE, key, "FriendlyName");
            if (prefix.equals("COM") &&
                name.toLowerCase().indexOf("gps") >= 0) {
              return "COM" +
                Registry.getInt(Registry.HKEY_LOCAL_MACHINE, key, "Index");
            }
          } catch (ElementNotFoundException enfe) {
            // ignore if a key was not found
          }
   ...
   * automatically scans the Windows CE registry searching for the correct GPS
   * COM port.
   *
   * Under Windows Mobile and Android, uses the internal GPS api.
   *
   * @throws GPSDisabledException If GPS is disabled
   * @throws IOException If something goes wrong or if there's already an open
   *     instance of the GPS class
   */
  public GPS() throws IOException { this(HIGH_GPS_PRECISION); }

  /**
   * Constructs a GPS control, with the given precision. Already prepared for
   * PIDION scanners. It automatically scans the Windows CE registry searching
   * for the correct GPS COM port.
   *
   * Under Windows Mobile and Android, uses the internal GPS api.
   *
   * @throws GPSDisabledException If GPS is disabled
  ...
  public GPS(int precision) throws IOException
  {
    ...
    if (!nativeAPI || !testStartGPS()) {
      ...
      if ("PIDION".equals(Settings.deviceId)) {
        sp = new PortConnector(
          Convert.chars2int("COM4"), 9600, 7, PortConnector.PARITY_EVEN, 1);
      } else if (Settings.isWindowsCE() && (com = getWinCEGPSCom()) != null) {
        sp = new PortConnector(
          Convert.chars2int(com), 9600, 7, PortConnector.PARITY_EVEN, 1);
      } else {
  ...
  @ReplacedByNativeOnDeploy private boolean startGPS() throws IOException
  {
    return false;
  }
  ...
  private void checkOpen() throws IOException
  {
    if (isOpen) {
      throw new IOException(
        "There's already an open instance of the GPS class");
    }
    isOpen = true;
  }
  ...
  @ReplacedByNativeOnDeploy private void stopGPS() {}
      ...
      try {
        deg = (double)Convert.toInt(s.substring(0, i - 2)) +
              ((double)Convert.toInt(s.substring(i - 2, i)) +
               (double)Convert.toInt(s.substring(i + 1, size)) / divider) /
                60;
  ...
  public static String toCoordinateGGMMSS(String coord, char dir)
  ...
  @ReplacedByNativeOnDeploy private int updateLocation() { return 0; }
  ...
  private boolean processNative()
  {
    ...
    return (result & 3) !=
      0; // latitude and longitude: one doesn't make sense without the other
  }
    ...
    while ((message = nextMessage()) != null) {
      ...
      if (sp[0].equals("$GPGGA")) { // guich@tc115_71
        ...
        if (sp.length > 7 && sp[7].length() > 0) {
          satellites =
            Convert.toInt(sp[7]); // flsobral@tc120_66: new field, satellites.
        }
   ...
   *
   * @param message
   * @return the given message without the ending CRLF if valid, otherwise null
   *     is returned.
   *
   * @since TotalCross 1.20
   */
  private String validateMessage(String message)
  {
    ...
    if (msgChars[0] != '$' || msgChars[msgChars.length - 2] != '\r' ||
        msgChars[msgChars.length - 1] != '\n') {
      return null;
    }
    ...
    if (msgChars[msgChars.length - 5] == '*') {
      int checksum2 =
        Convert.hex2signed(new String(msgChars, msgChars.length - 4, 2));
  ...
  @Override protected void finalize() { this.stop(); }
}
