...
/**
 * This class is defined by the JSR-82 specification <em>Java&trade; APIs for
 * Bluetooth&trade; Wireless Technology, Version 1.1.</em>
 */
public class DataElement
{
   ...
   *
   * This field also keeps the value for the type DATALT and DATSEQ. In this
   * case it's a Vector. The access to the Vector elements is synchronized in
   * cldc (according the source code). But, this is not documented, so we make a
  ...
  public DataElement(int valueType, Object value)
  {
    ...
    switch (valueType) {
      ...
      case U_INT_16:
        isCorrectValue =
          value instanceof byte[] && ((byte[])value).length == 16;
        break;
      case U_INT_8:
  ...
  {
    /*
     * We can't optimize this by invoking the this.insertElementAt(elem,
     * getSize()), because the ClassCastException may be thrown from getSize()
     * which gives us improper stack trace.
     */

    if (valueType != DATSEQ && valueType != DATALT) {
      throw new ClassCastException("Invalid element type for this method: " +
                                   valueType);
    }
    if (elem == null) {
  ...
  public synchronized void insertElementAt(DataElement elem, int index)
  {
    ...
    /*
     * We can't use the Vector.insertElementAt check for out of bounds, because
     * Vector throws ArrayIndexOutOfBoundsException in this case.
     */
  ...
  public boolean removeElement(DataElement elem)
  {
    ...
    /*
     * The Bluetooth spec says the two DataElement equals if their references
     * are equal. According to cldc1.1 ref impl sources, the Vector uses
     * 'equals' call, and the Object.equls uses a references compare, so we may
     * not care about doing this here.
     */
  ...
  public synchronized Object getValue()
  {
    ...
    /*
     * According to cldc & bluetooth specifications, the String and UUID are
     * immutable, so we may not return a clone object to safe the stored one.
     *
     * The Vector.elements() returns an Enumeration, which does not allow to
     * break the Vector either.
     *
