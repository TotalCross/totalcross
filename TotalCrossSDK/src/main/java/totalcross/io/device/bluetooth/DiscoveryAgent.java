...
/**
 * The <code>DiscoveryAgent</code> class provides methods to perform device and
 * service discovery. A local device must have only one
 * <code>DiscoveryAgent</code> object. This object must be retrieved by a call
 * to <code>getDiscoveryAgent()</code> on the <code>LocalDevice</code> object.
 *
...
{
  /**
   * Used with the <code>retrieveDevices()</code> method to return those devices
   * that were found via a previous inquiry. If no inquiries have been started,
   * this will cause the method to return <code>null</code>. <p> The value of
   * <code>CACHED</code> is 0x00 (0).
   * </p>
   */
   ...
   * This is used to specify the type of inquiry to complete or respond to. <p>
  ...
  /**
   * Used with the <code>retrieveDevices()</code> method to return those devices
   * that are defined to be pre-known devices. Pre-known devices are specified
   * in the BCC. These are devices that are specified by the user as devices
   ...
   * <code>INQUIRY_TERMINATED</code> as a result of calling this method. After
   * receiving this event, no further <code>deviceDiscovered()</code> events
   ...
   *     <code>false</code> if the inquiry was not
   *         canceled or if the inquiry was not started using
   * <code>listener</code>
   ...
   * event with a discovery type of <code>SERVICE_SEARCH_TERMINATED</code> will
   * occur when this method is called. After receiving this event, no further
   * <code>servicesDiscovered()</code> events will occur as a result of this
   * search.
   *
   * Not implemented on Android and Windows CE.
   *
   * @param transID
   *           the ID of the service search transaction to cancel; returned by
   * <code>searchServices()</code>
   * @return <code>true</code> if the service search transaction is terminated,
   *     else <code>false</code> if
   *         <code>transID</code> does not represent an active service search
   * transaction
   * @since TotalCross 1.2
   */
   ...
   * detail.) A device can be set as a pre-known device in the Bluetooth Control
   * Center.
   *
   * Works on Android. Pass CACHED to list the unpaired devices, and PREKNOWN to
   * list the paired devices. Note that paired devices may not be at reach at
   * the moment.
   *
   * @param option
   *           <code>option</code> - <code>CACHED</code> if previously found
   * devices should be returned; <code>PREKNOWN</code> if pre-known devices
   * should be returned
   * @return an array containing the Bluetooth devices that were previously
   *     found if <code>option</code> is
   *         <code>CACHED</code>; an array of devices that are pre-known devices
   * if <code>option</code> is <code>PREKNOWN</code>; <code>null</code> if no
  ...
  /**
   * Searches for services on a remote Bluetooth device that have all the UUIDs
   * specified in <code>uuidSet</code>. Once the service is found, the
   * attributes specified in <code>attrSet</code> and the default attributes are
   * retrieved. The default attributes are ServiceRecordHandle (0x0000),
   * ServiceClassIDList (0x0001), ServiceRecordState (0x0002), ServiceID
   * (0x0003), and ProtocolDescriptorList (0x0004). If <code>attrSet</code> is
   * <code>null</code> then only the default attributes will be retrieved.
   * <code>attrSet</code> does not have to be sorted in increasing order, but
   ...
   * services which have the UUIDs specified in <code>uuidSet</code>
   * @param uuidSet
   ...
   * @return the transaction ID of the service search, which is a positive
   *     number.
   * @throws NullPointerException
   *            if <code>uuidSet</code>, <code>btDev</code>, or
   * <code>discListener</code> is <code>null</code>; if an element in
   * <code>uuidSet</code> array is <code>null</code>
   * @throws IllegalArgumentException
   *            if <code>attrSet</code> has an illegal service attribute ID or
   * exceeds the property <code>bluetooth.sd.attr.retrievable.max</code> defined
   * in the class <code>LocalDevice</code>; if <code>attrSet</code> or
   * <code>uuidSet</code> is of length 0; if <code>attrSet</code> or
   *            <code>uuidSet</code> contains duplicates
   * @throws IOException
   *            if the number of concurrent service search transactions exceeds
   * the limit specified by the <code>bluetooth.sd.trans.max</code> property
   * obtained from the class <code>LocalDevice</code> or the system is unable to
                            ...
                            DiscoveryListener discListener) throws IOException
  {
    ...
    if (attrSet == null) {
      ...
      if (attrSetVector.items[0] < 0 || attrSetVector.items[0] > maxAttrValue) {
        throw new IllegalArgumentException(
          "attrSet values must be in the range [0 - (2^16 - 1)]");
      }
      for (int i = attrSetLen - 1; i > 0; i--) {
        ...
        if (attrSetVector.items[i] < 0 ||
            attrSetVector.items[i] > maxAttrValue) {
          throw new IllegalArgumentException(
            "attrSet values must be in the range [0 - (2^16 - 1)]");
        }
      }

      // not pretty, but this way we avoid some extra method calls and a short
      // loop.
      ...
      attrSetVector.qsort();
      attrSet = attrSetVector.toIntArray(); // final array, after adding the
                                            // default attributes and sorting.
    }

    // uuidSet cannot have duplicated values
    UUID[] uuidSet2 = new UUID[uuidSetLen];
    Vm.arrayCopy(uuidSet, 0, uuidSet2, 0, uuidSetLen);
    Convert.qsort(uuidSet2,
                  0,
                  uuidSetLen - 1,
                  Convert.SORT_OBJECT,
                  true); // we don't have to check uuidSet for null values,
                         // qsort already does that;
    ...
    throw new IOException(
      "Bluetooth API is not supported on JDK"); // not supported on JDK,
                                                // always throws IOException.
  }

  /**
   * Attempts to locate a service that contains <code>uuid</code> in the
   * ServiceClassIDList of its service record. This method will return a string
   * that may be used in <code>Connector.open()</code> to establish a connection
   * to the service. How the service is selected if there are multiple services
   * with <code>uuid</code> and which devices to search is implementation
   * dependent.
   *
   * Not implemented on Android.
   *
   ...
   * @param master
   *           determines if this client must be the master of the connection;
   * <code>true</code> if the client must be the master; <code>false</code> if
   * the client can be the master or the slave
   * @return the connection string used to connect to the service with a UUID of
   *     <code>uuid</code>; or
   *         <code>null</code> if no service could be found with a UUID of
   * <code>uuid</code> in the ServiceClassIDList
   * @throws NullPointerException
   ...
   * <code>ServiceRecord.AUTHENTICATE_ENCRYPT</code>
   * @throws IOException
   *            if the Bluetooth system cannot start the request due to the
   * current state of the Bluetooth system
   * @since TotalCross 1.2
   */
   ...
   * returned to the application via the method <code>deviceDiscovered()</code>
   * of the interface <code>DiscoveryListener</code>. The
   * <code>cancelInquiry()</code> method is called to stop the inquiry.
   *
   * Not implemented on Android.
   *
   * @param accessCode
   ...
   * @return <code>true</code> if the inquiry was started; <code>false</code> if
   *     the inquiry was not started because
   ...
   *            if <code>listener</code> is <code>null</code>
   * @throws IOException
   *            if the Bluetooth device does not allow an inquiry to be started
   * due to other operations that are being performed by the device
   * @since TotalCross 1.2
   */
  public boolean startInquiry(int accessCode, DiscoveryListener listener)
    throws IOException
  {
    if (accessCode != LIAC || accessCode != GIAC || accessCode < 0x9E8B00 ||
        accessCode > 0x9E8B3F) {
      throw new IllegalArgumentException();
    }
    if (listener == null) {
