...
{
  final private static UTF8CharacterConverter converterUTF8 =
    (UTF8CharacterConverter)Convert.charsetForName("UTF-8");
  final private static CharacterConverter converterASCII =
    ...
    switch (type) {
      ...
      case 4: { // STRING
        ...
        return new DataElement(
          DataElement.STRING,
          new String(converterUTF8.bytes2chars(bytes, 0, bytes.length)));
      }
      case 5: // BOOL
      ...
      case 6: { // DATSEQ
        ...
        if ((started + length) != inputStream.getPos()) {
          throw new IOException("DATSEQ size corruption " +
                                (started + length - inputStream.getPos()));
        }
        return element;
      }
      case 7: { // DATALT
      ...
      case 8: { // URL
        ...
        return new DataElement(
          DataElement.URL,
          new String(converterASCII.bytes2chars(bytes, 0, bytes.length)));
      }
      default:
