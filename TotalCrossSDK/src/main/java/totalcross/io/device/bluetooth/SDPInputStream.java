...
class SDPInputStream extends DataStream {
	final private static UTF8CharacterConverter converterUTF8 = (UTF8CharacterConverter)
			Convert.charsetForName("UTF-8");
	final private static CharacterConverter converterASCII = (CharacterConverter)
			Convert.charsetForName("ISO-8859-1");
	...
	public DataElement readElement() throws IOException {
		...
		switch (type) {
			...
			case 4: { // STRING
				...
				return new DataElement(DataElement.STRING, new String(converterUTF8.bytes2chars(bytes, 0,
									   bytes.length)));
			}
			case 5: // BOOL
			...
			case 8: { // URL
				...
				byte[] bytes = readBytes(length);
				return new DataElement(DataElement.URL, new String(converterASCII.bytes2chars(bytes, 0,
									   bytes.length)));
			}
			default:
