...
public class ServiceRecord4D
{
  ...
  public RemoteDevice getHostDevice() { return hostDevice; }
  ...
  public String getConnectionURL(int requiredSecurity, boolean mustBeMaster)
  {
    if (baseURL == null) {
      ...
      DataElement protocolDescriptor =
        getAttributeValue(BluetoothConsts4D.ProtocolDescriptorList);
      if ((protocolDescriptor == null) ||
          (protocolDescriptor.getDataType() != DataElement.DATSEQ)) {
        return null;
      }
      ...
      DataElement[] protocolsSeqEnum =
        (DataElement[])protocolDescriptor.getValue();
      for (int i = 0; i < protocolsSeqEnum.length; i++) {
        ...
        if (BluetoothConsts4D.OBEX_PROTOCOL_UUID.equals(uuid)) {
          ...
          isRFCOMM = false;
          isL2CAP = false;
        } else if (elementSeqEnum.length > 1 &&
                   (BluetoothConsts4D.RFCOMM_PROTOCOL_UUID.equals(uuid))) {
          ...
          switch (protocolPSMElement.getDataType()) {
            ...
            case DataElement.INT_8:
              ...
              if ((val >= BluetoothConsts4D.RFCOMM_CHANNEL_MIN) &&
                  (val <= BluetoothConsts4D.RFCOMM_CHANNEL_MAX)) {
                commChannel = (int)val;
                isRFCOMM = true;
                isL2CAP = false;
              }
              break;
                   ...
                   (BluetoothConsts4D.L2CAP_PROTOCOL_UUID.equals(uuid))) {
          DataElement protocolPSMElement = (DataElement)elementSeqEnum[1];
          switch (protocolPSMElement.getDataType()) {
            case DataElement.U_INT_1:
            case DataElement.U_INT_2:
            ...
            case DataElement.INT_8:
              ...
              if ((pcm >= BluetoothConsts4D.L2CAP_PSM_MIN) &&
                  (pcm <= BluetoothConsts4D.L2CAP_PSM_MAX)) {
                commChannel = (int)pcm;
                isL2CAP = true;
              }
              break;
      ...
      urlBuffer.append(isOBEX ? BluetoothConsts4D.PROTOCOL_SCHEME_BT_OBEX
                              : (isRFCOMM
                                   ? BluetoothConsts4D.PROTOCOL_SCHEME_RFCOMM
                                   : BluetoothConsts4D.PROTOCOL_SCHEME_L2CAP));
  ...
  boolean readSDP4D(RemoteDevice device, byte[] input, int[] attrIDs)
    throws IOException
  {
