 ...
 * <p>
 * {@code ServiceRecords} are made available to a client application via an
 * argument of the {@link
 * totalcross.io.device.bluetooth.DiscoveryListener#servicesDiscovered} method
 ...
 *
 * <TABLE BORDER=1>
 * <TR><TH>Attribute Name</TH><TH>Attribute ID</TH><TH>Attribute Value
 * Type</TH></TR> <TR><TD>ServiceRecordHandle</TD><TD>0x0000</TD><TD>32-bit
 * unsigned integer</TD></TR>
 * <TR><TD>ServiceClassIDList</TD><TD>0x0001</TD><TD>DATSEQ of UUIDs</TD></TR>
 * <TR><TD>ServiceRecordState</TD><TD>0x0002</TD><TD>32-bit unsigned
 * integer</TD></TR> <TR><TD>ServiceID</TD><TD>0x0003</TD><TD>UUID</TD></TR>
 * <TR><TD>ProtocolDescriptorList</TD><TD>0x0004</TD><TD>DATSEQ of DATSEQ of
 * UUID and optional parameters</TD></TR>
 * <TR><TD>BrowseGroupList</TD><TD>0x0005</TD><TD>DATSEQ of UUIDs</TD></TR>
 * <TR><TD>LanguageBasedAttributeIDList</TD><TD>0x0006</TD><TD>DATSEQ of DATSEQ
 * triples</TD></TR> <TR><TD>ServiceInfoTimeToLive</TD><TD>0x0007</TD><TD>32-bit
 * unsigned integer</TD></TR>
 * <TR><TD>ServiceAvailability</TD><TD>0x0008</TD><TD>8-bit unsigned
 * integer</TD></TR>
 * <TR><TD>BluetoothProfileDescriptorList</TD><TD>0x0009</TD><TD>DATSEQ of
 * DATSEQ pairs</TD></TR>
 ...
 * <TR><TD>VersionNumberList</TD><TD>0x0200</TD><TD><code>DATSEQ</code> of
 * 16-bit unsigned integers</TD></TR>
 * <TR><TD>ServiceDatabaseState</TD><TD>0x0201</TD><TD>32-bit unsigned
 * integer</TD></TR>
 * </TABLE>
 * <p>
 * The following table lists the common string-valued attribute ID offsets used
 * in a
 * {@code ServiceRecord}. These offsets must be added to a base value to obtain
 * the actual service ID. (For more information, see the Service Discovery
 * Protocol Specification located in the Bluetooth Core Specification. <p>
 * <TABLE BORDER=1>
 * <TR><TH>Attribute Name</TH><TH>Attribute ID Offset</TH><TH>Attribute Value
 * Type</TH></TR> <TR><TD>ServiceName</TD><TD>0x0000</TD><TD>String</TD></TR>
...
public class ServiceRecord
{
   ...
   * service. Used with <code>getConnectionURL()</code> method. <p>
   * <code>NOAUTHENTICATE_NOENCRYPT</code> is set to the constant value 0x00
   * (0).
   * </p>
   */
   ...
   * connection. Used with <code>getConnectionURL()</code> method. <p>
   ...
   * Used with <code>getConnectionURL()</code> method. <p>
  ...
  ServiceRecord(Object nativeInstance) { this.nativeInstance = nativeInstance; }

  /**
   * Returns the remote Bluetooth device that populated the service record with
   * attribute values. It is important to note that the Bluetooth device that
   * provided the value might not be reachable anymore, since it can move, turn
   * off, or change its security mode denying all further transactions.
   *
   * @return the remote Bluetooth device that populated the service record, or
   *     null if the local device populated this
   *         ServiceRecord
  ...
  /**
   * Returns the value of the service attribute ID provided it is present in the
   * service record, otherwise this method returns null.
   *
   * @param attrID
  ...
  /**
   * Returns a string including optional parameters that can be used by a client
   * to connect to the service described by this ServiceRecord. The return value
   * can be used as the first argument to Connector.open(). In the case of a
   * Serial Port service record, this string might look like
   * "btspp://0050CD00321B:3;authenticate=true;encrypt=false;master=true", where
   * "0050CD00321B" is the Bluetooth address of the device that provided this
   * ServiceRecord, "3" is the RFCOMM server channel mentioned in this
   * ServiceRecord, and there are three optional parameters related to security
   * and master/slave roles.
   *
   * @return a string that can be used to connect to the service or null if the
   *     ProtocolDescriptorList in this
   ...
   *           determines whether authentication or encryption are required for
   * a connection
   * @param mustBeMaster
   *           true indicates that this device must play the role of master in
   * connections to this service; false indicates that the local device is
   * willing to be either the master or the slave
   * @return a string that can be used to connect to the service or null if the
   *     ProtocolDescriptorList in this
   *         ServiceRecord is not formatted according to the Bluetooth
   * specification
   ...
   * NOAUTHENTICATE_NOENCRYPT, AUTHENTICATE_NOENCRYPT, or AUTHENTICATE_ENCRYPT
   * @since TotalCross 1.27
