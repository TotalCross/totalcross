 ...
 * <p>
 * This works on the devices and on JDK (in this case, it uses javax.comm, which
 * must be installed separately) <p> When a serial port is created, an attempt
 * is made to open the port. If the open attempt is successful, the port will
 * remain open until close() is called. If close() is never called, the port
 ...
 * port.close();
 * </pre>
 * On JDK, you can define a file called serial.properties with the number and
 * ports to be used:<br> DEFAULT=a<br> IRCOMM=b<br> SIR=c<br> USB=d<br> If the
 * file is not found, it will use COM1, COM2, COM3, and COM4, respectively.
 */
...
public class PortConnector extends Stream
{
   ...
   * SIR (aka raw IR); see this:
   * http://www.alanjmcf.me.uk/comms/infrared/IrDA%20FAQ.html and
   * http://news.palmos.com/read/messages?id=146145#146145. Note also that
   * there's no handshaking support on SIR, and that not all devices work with
   * it. Use IRCOMM instead when possible.
   */
   ...
   * must be one of the constants PARITY_NONE, PARITY_EVEN, PARITY_ODD. For Palm
   * OS, PARITY_ODD turns on XON/XOFF mode.
   *
   * @param number port number. In Windows, this is the number of the COM port.
   *     On Windows and Palm OS, you can pass
   * a 4-letter that indicates the connection that will be used. For example, on
   * Palm OS you could use <code>Convert.chars2int("rfcm")</code> or in Windows
   * CE use <code>Convert.chars2int("COM4")</code>.
   * @param baudRate baud rate
   * @param bits bits per char [5 to 8]
   * @param parity of the constants PARITY_NONE, PARITY_EVEN, PARITY_ODD. For
   *     Palm OS, PARITY_ODD turns on XonXoff mode.
   * @param stopBits number of stop bits
   * @see #setFlowControl
   * @since SuperWaba 4.5
   */
  ...
  private void create(int number,
                      ...
                      int bits,
                      int parity,
                      int stopBits) throws totalcross.io.IOException
  {
    try {
      // guich@330_38: use introspection to be able to compile everything
      // without the comm.jar javax.comm.CommPortIdentifier portId =
      // javax.comm.CommPortIdentifier.getPortIdentifier(getPortName(number));
      Class<?> cpi =
        Class.forName("javax.comm.CommPortIdentifier"); // get the class

      String portName = getPortName(number);
      System.out.println("Port found: " + portName);

      Method mGetPortIdentifier = cpi.getMethod(
        "getPortIdentifier",
        new Class[] {
          String.class }); // get the method getPortIdentifier(String)
      Object portId = mGetPortIdentifier.invoke(
        null,
        new Object[] {
          portName }); // invoke the
                       // getPortIdentifier(getPortName(number)) method

      // javax.comm.SerialPort port =
      // (javax.comm.SerialPort)portId.open("TotalCross", 1000);
      Method mOpen = portId.getClass().getMethod(
        "open",
        new Class[] { String.class,
                      int.class }); // get the method getPortIdentifier(String)
      Object port = mOpen.invoke(
        portId,
        new Object[] { "TotalCross",
                       new Integer(1000) }); // invoke the portId.open method

      // port.setSerialPortParams(baudRate, bits, stopBits,
      // parity?javax.comm.SerialPort.PARITY_EVEN:javax.comm.SerialPort.PARITY_NONE);
      Class<?> cSerialPort = port.getClass().getSuperclass();
      Method mSetSerialPortParams = cSerialPort.getMethod(
        "setSerialPortParams",
        new Class[] { int.class,
                      int.class,
                      int.class,
                      int.class }); // get the method setSerialPortParams
      String[] parities = { "PARITY_NONE", "PARITY_EVEN", "PARITY_ODD" };
      Field parityField = cSerialPort.getField(parities[parity]);
      mSetSerialPortParams.invoke(
        port,
        new Object[] { new Integer(baudRate),
                       new Integer(bits),
                       new Integer(stopBits),
                       new Integer(parityField.getInt(port)) });

      // port.enableReceiveTimeout(100);
      Method mEnableReceiveTimeout = cSerialPort.getMethod(
        "enableReceiveTimeout", new Class[] { int.class });
      ...
      throw new totalcross.io.IOException(
        ule.getTargetException().getClass().getName() + " - " +
      ...
      throw new totalcross.io.IOException(t.getClass().getName() + " - " +
                                          t.getMessage());
    }
  }
  ...
  final public void setFlowControl(boolean on) throws totalcross.io.IOException
  {
    try {
      ...
      Field fRTSCTS_IN = portConnectorRef.getClass().getSuperclass().getField(
        "FLOWCONTROL_RTSCTS_IN");
      Field fRTSCTS_OUT = portConnectorRef.getClass().getSuperclass().getField(
        "FLOWCONTROL_RTSCTS_OUT");
      Field fNONE = portConnectorRef.getClass().getSuperclass().getField(
        "FLOWCONTROL_NONE");
      Method m = portConnectorRef.getClass().getSuperclass().getMethod(
        "setFlowControlMode", new Class[] { int.class });
      m.invoke(
        portConnectorRef,
        new Object[] { on ? new Integer(fRTSCTS_IN.getInt(portConnectorRef) |
                                        fRTSCTS_OUT.getInt(portConnectorRef))
  ...
  final public int readCheck() throws totalcross.io.IOException
  {
    try {
      ...
      Method m =
        thisInputStream.getClass().getSuperclass().getMethod("available");
  ...
  /**
   * Writes to the port. Returns the number of bytes written or throws an
   * <code>IOException</code> if an error prevented the write operation from
   * occurring. If data can't be written to the port and flow control is on, the
   * write operation will time out and fail after approximately 2 seconds.
  ...
  final private int readWriteBytes(byte buf[],
                                   int start,
                                   int count,
                                   boolean isRead)
    throws totalcross.io.IOException
  {
    if (isRead) {
      try {
        ...
        Method mEnableReceiveTimeout =
          portConnectorRef.getClass().getSuperclass().getMethod(
            "enableReceiveTimeout", new Class[] { int.class });
        mEnableReceiveTimeout.invoke(portConnectorRef,
                                     new Object[] { new Integer(readTimeout) });

        int tries = readTries;
        Method m = thisInputStream.getClass().getSuperclass().getMethod(
          "read", new Class[] { byte[].class, int.class, int.class });
        // guich@330_42: make sure we have read everything
        int total = count;
        do {
          // return ((java.io.InputStream)thisInputStream).read(buf, start,
          // count);
          Integer i = (Integer)m.invoke(
            thisInputStream,
            new Object[] { buf, new Integer(start), new Integer(count) });
          int read = i.intValue();
          if (read <= 0) {
            break;
          }
      ...
      try {
        ...
        Method m = thisOutputStream.getClass().getSuperclass().getMethod(
          "write", new Class[] { byte[].class, int.class, int.class });
        m.invoke(thisOutputStream,
                 new Object[] { buf, new Integer(start), new Integer(count) });
        return count;
      } catch (Throwable t) {
        throw new totalcross.io.IOException(t.getMessage() +
  ...
  /**
   * Set this to a value that will be used to keep trying to read further while
   * something is available. If a series of bytes is returned in very few steps,
   * this will make the read stop after readTries has reached zero.
  ...
  /**
   * @return When number == Logical port number defined in
  ...
  static private String getPortName(int number)
  {
    ...
    try {
      // java.util.Enumeration portIds =
      // javax.comm.CommPortIdentifier.getPortIdentifiers();
      Class<?> cpi =
        Class.forName("javax.comm.CommPortIdentifier"); // get the class
      Method mGetPortIdentifiers = cpi.getMethod("getPortIdentifiers");
      java.util.Enumeration<?> portIds =
        (java.util.Enumeration<?>)mGetPortIdentifiers.invoke(null);

      if (number == 0) {
        if (portIds.hasMoreElements()) {
          // return
          // ((javax.comm.CommPortIdentifier)portIds.nextElement()).getName();
