...
public class PortConnector extends Stream {
	...
	private void create(int number, int baudRate, int bits, int parity,
	...
	final private int readWriteBytes(byte buf[], int start, int count,
									 boolean isRead) throws totalcross.io.IOException {
		if (isRead) {
			try {
				// guich@tc: now the readTimeout is a field
				Method mEnableReceiveTimeout =
					portConnectorRef.getClass().getSuperclass().getMethod("enableReceiveTimeout",
							new Class[] { int.class });
