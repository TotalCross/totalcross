 ...
 * a popup dialog will ask you for a barcode symbol which will be returned every
 * time the <code>getData</code> method is called. <p> Since there is only one
 * scanner per device, the scanner class is static. Activating the scanner
 * causes the physical scanner to be powered, and enables the trigger.
 * Deactivating the scanner removes the power from the scanner and prevents
 * scans from taking place. The scanner should always be deactivated at the end
 * of processing to prevent excessive battery drain. <p> When the scanner is
 * activated, scan events will appear in the MainWindow's onEvent method.  The
 * scan events will contain a string describing either the item scanned or a
 * battery error. Deactivating the scanner prevents scan events from being
 * delivered. <p> Since barcodes can have many formats, this class includes a
 * method to register the barcode types with the scanner. These types must each
 * be individually set as parameters to the scanner.  After all the parameter
 ...
 *    return;
 * </pre>
 * Note: to generate an installation for the SYMBOL MC32, you must deploy using
 * -WINCE (creates cab without compression), and not -WINMO (creates cab with
 * compression).
 */
...
public class Scanner
{
  public static boolean
    isActive; // guich@200b4: avoid activate the scanner again
  private static String barcode;
  private static int tries;
  private static boolean driverLoaded;
  /** The scanner version. */
  public static String scanManagerVersion =
    Settings.onJavaSE ? "Scanner emulation" : "1.0";

  /** Set this listener to send all Scanner events to its onEvent method. */
  public static Control
    listener; // maybe theres another better way to do this...
    ...
    if (Settings.isWindowsCE()) {
      ...
      if (!driverLoaded && tries++ == 0) {
        throw new RuntimeException(
          "Cannot find the native implementation for the scanner library.");
      }
    }
  };
...
public static void
proprietaryScanLoad(Runnable scanLoad)
{
  synchronized (lock)
  {
    if (scanLoad != null) {
 ...
 *
 * <p><b>Note</b>: On Android Intermec, you can't activate the scanner if it is
 * off in the settings. Moreover, you should not set the scanner and virtual
 * wedge settings off and then try to use the scanner. Even though your app
 * won't crash or hang, the device might not behave properly. There is no way to
 * check if the device settings are off in the settings or in the app.
 */
public static boolean
activate()
{
...
{
  InputBox id =
    new InputBox("Barcode emulation", "Please enter the barcode:", "");
  id.popup();
  barcode = id.getValue();
 ...
 *
 * <p><b>Note</b>: On Android Intermec, you CAN'T set enable a barcode type the
 * scanner is not activate. In this case, this method will return false.
 */
@ReplacedByNativeOnDeploy
public static boolean
setBarcodeParam(int barcodeType, boolean enable)
{
  return true;
}
 ...
 * <p>
 * With Datalogic scanners, the SECOND parameter is not used (always pass 0 to
 * it); and don't forget to call the commitBarcodeParams method. <p> Values
 * valid for SYMBOL scanners: <pre>
 *    // triggering modes
 *    #define LEVEL                                       0x00
 *    #define PULSE                                       0x02
 *    #define HOST                                        0x08
...
@ReplacedByNativeOnDeploy
public static boolean
setParam(int type, int barcodeType, int value)
{ // guich@330_43
  return true;
}
 ...
 * @param lengthType 0 (variables length, min, and max are ignored), 1 (length =
 *     min), 2 (length = min || length = max), 3 (min <= length <= max).
 *     Careful: in Motorola Scanners, min and max must be used in inverted order
 *     due to a bug in the MOTOROLA API.
 * @param min The minimum value
 * @param max The maximum value
 */
@ReplacedByNativeOnDeploy
public static boolean
setBarcodeLength(int barcodeType, int lengthType, int min, int max)
{ // guich@330_43
  return true;
}
 ...
 *
 * <p><b>Note</b>: On Android Intermec, you should first set some parameters
 * before fetching data or else it might crash after 10 or 15 reads.
 */
@ReplacedByNativeOnDeploy
...
/**
 * Get the scan manager version as an hexadecimal String. If an error occurs or
 * if this method is called before the Scanner is initialized, a null String
 * will be returned.
 */
@ReplacedByNativeOnDeploy
public static String
...
/**
 * Get the Scanner Port Driver version as an hexadecimal string. If an error
 * occurs or if this method is called before the Scanner is initialized, a null
...
_onEvent(int type)
{
  ...
  int id =
    type % 1100 == 2
      ? ScanEvent.BATTERY_ERROR
      : type % 1100 == 1 ? ScanEvent.SCANNED /* 0 */ : ScanEvent.TRIGGERED;
...
@ReplacedByNativeOnDeploy
public static void
setParam(String what, String value)
{}
}