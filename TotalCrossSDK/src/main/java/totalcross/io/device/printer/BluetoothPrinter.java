...
/**
 * Used as interface to printers that uses Bluetooth to communicate with the
 * device. See the CitizenPrinter javadocs if you plan to use one. <p> Note:
 * during tests, we found that some printers require to fill their buffer to
 * start printing. So, if you have problems printing, try the following code
 * after you instantiate this class: <pre> cp = new CitizenPrinter(); // or new
 * BluetoothPrinter(); cp.write(new byte[2048]); // fill the buffer
 * cp.newLine();
 * ...
 * </pre>
 */
...
public class BluetoothPrinter
{
  ...
  /**
   * Creates a new BluetoothPrinter instance, using PortConnector.BLUETOOTH
   * port at 57600 baud rate.
   */
  ...
  /**
   * Creates a new BluetoothPrinter instance, using the given PortConnector as
   ...
   */
  public BluetoothPrinter(Stream con) throws IOException { this.con = con; }
  ...
  public void print(String str) throws IOException { write(str.getBytes()); }
   ...
   */
  @Deprecated
  public void print(MonoImage img, byte imageMode)
  ...
  /**
   * Prints the given MonoImage.
   * In the image, only black pixels are written. The maximum width is 576
   * pixels for single density is 192, and for double density is 384; the image
   * is trimmed to fit these values.
   */
  ...
  public void newLine() throws IOException { write(ENTER); }
