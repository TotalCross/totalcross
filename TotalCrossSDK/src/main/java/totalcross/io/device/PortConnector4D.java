...
public class PortConnector4D extends Stream
{
  ...
  {
    this(
      number, baudRate, bits, (parity ? PARITY_EVEN : PARITY_NONE), stopBits);
  }

  public PortConnector4D(int number, int baudRate)
  ...
  native public void setFlowControl(boolean on)
    throws totalcross.io.IOException;

  native public int readWriteBytes(byte buf[],
                                   int start,
                                   int count,
                                   boolean isRead)
    throws totalcross.io.IOException;
