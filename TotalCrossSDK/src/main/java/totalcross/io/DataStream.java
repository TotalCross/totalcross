...
public class DataStream extends Stream {
	...
	public long readUnsignedInt() throws EOFException, totalcross.io.IOException {
		byte[] b = buffer;
		readBytesInternal(b, 0, 4, true);
		return ((((long)(b[0] & 0xFF)) << 24) | (((long)(b[1] & 0xFF)) << 16) | ((b[2] & 0xFF) << 8) |
				(b[3] & 0xFF));
	}
	...
	public String readString() throws EOFException, totalcross.io.IOException {
		...
		if (buffer.length < len) {
			buffer = new byte[len +
							  16]; // guich@200 grows the buffer if necessary - corrected by mike mcroberts
		}

		readBytesInternal(buffer, 0, len, true);
		return new String(totalcross.sys.Convert.charConverter.bytes2chars(buffer, 0,
						  len)); // eisvogel@450_17: make sure that desktop and device use the same convertion algorithm
	}
	...
	public int writeString(String s) throws totalcross.io.IOException {
		int n = 0, l;
		if (s != null
			&& (l = s.length()) > 0) { // eisvogel@421_70: don't let write more than the pdb can handle
	 ...
	 */
	public int readChars(char[] chars, int start, int count) throws EOFException,
	...
	public int writeChars(char[] chars, int start, int len,
						  int lenSize) throws totalcross.io.IOException {
		int n = 0, c;
		if (len < 0) {
	 ...
	 * @see Storable
	 */
	public Object readObject() throws ClassNotFoundException, InstantiationException,
	...
	protected int writeBytesInternal(byte[] buf, int start,
									 int count) throws totalcross.io.IOException {
	...
	throws EOFException, totalcross.io.IOException {
		...
		if (r != -1) {
			while (bytesRead < count) {
				r = stream.readBytes(buf, start + bytesRead,
