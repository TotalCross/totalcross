...
public class DataStream extends Stream
{
  ...
  protected byte[] buffer =
    new byte[256]; // starts with 256 bytes since readSmallString uses it
   ...
   */
  public DataStream(Stream stream) { this(stream, false); }
   ...
   * @param ensureWrite
   *           if true, write operations are blocked until all the requested
   ...
   * thus closing it. Usually, this method may never be called. Remember that
   * closing a stream twice might throw an IOException, so if you call this
   * close method, don't call the base Stream's.
   *
  ...
  {
    if (zeros.length < n) {
      zeros = new byte[n + 16]; // guich@200 grows the buffer if necessary -
                                // corrected by mike roberts
    }
   ...
   * @throws totalcross.io.IOException
   */
  final public boolean readBoolean()
   ...
   * @throws totalcross.io.IOException  If an error prevented the read operation
   *     from occurring.
   */
  ...
  public String readString() throws EOFException, totalcross.io.IOException
  {
    ...
    if (buffer.length < len) {
      buffer = new byte[len + 16]; // guich@200 grows the buffer if necessary -
                                   // corrected by mike mcroberts
    }

    readBytesInternal(buffer, 0, len, true);
    return new String(totalcross.sys.Convert.charConverter.bytes2chars(
      buffer,
      0,
      len)); // eisvogel@450_17: make sure that desktop and device use the
             // same convertion algorithm
  }
  ...
  {
    int n = 0, l;
    if (s != null && (l = s.length()) > 0) { // eisvogel@421_70: don't let write
                                             // more than the pdb can handle
      char[] ac = s.toCharArray();
      if (l > 65535) {
        throw new IOException("String size " + l +
                              " is too big to use with writeString!");
      }
      byte[] c = totalcross.sys.Convert.charConverter.chars2bytes(
        ac, 0, ac.length); // eisvogel@450_17
  ...
  {
    int n = 0;
    if (s != null && s.length() > 0) { // eisvogel@421_70: don't let write more
                                       // than the pdb can handle
      char[] ac = s.toCharArray();
      byte[] c = totalcross.sys.Convert.charConverter.chars2bytes(
        ac, 0, ac.length); // eisvogel@450_17
      n += writeInt(c.length);
  ...
  final public int writeCString(String s) throws totalcross.io.IOException
  {
    ...
    int n = 0;
    char[] ac = s.toCharArray();
    byte[] ab = totalcross.sys.Convert.charConverter.chars2bytes(
      ac, 0, ac.length); // eisvogel@450_17
   ...
   */
  public char readChar() throws EOFException, totalcross.io.IOException
  { // guich@421_31
    byte[] b = buffer;
    readBytesInternal(b, 0, 2, true);
   ...
   * @since SuperWaba 4.21
   */
  public int writeChar(char c) throws totalcross.io.IOException
  { // guich@421_31
    byte[] b = buffer;
    b[1] = (byte)c;
    c >>= 8;
   ...
   */
  public void readChars(char[] chars, int len)
    throws EOFException, totalcross.io.IOException
  {
    byte[] bytes = buffer;
  ...
  /**
   * Reads 'count' chars from the stream to the given char array, starting from
   * index 'start'.
   *
   * @param chars
   *           a char array
   ...
   */
  public int readChars(char[] chars, int start, int count)
    throws EOFException, totalcross.io.IOException
  {
    byte[] bytes = buffer;
   ...
   * <p>The char array size is limited to 65535 characters.
   *
   * @param len   the length to be written or -1 if it is to write the whole
   *     char array
   ...
   * @since SuperWaba 3.5
   */
  public int writeChars(char[] chars, int start, int len)
    throws totalcross.io.IOException
  {
   ...
   * @since TotalCross 1.0 beta3
   */
  public int writeBigChars(char[] chars, int start, int len)
    throws totalcross.io.IOException
  {
  ...
  /**
   * Writes the given char array, writting the length as an int or as a short
   * or as a byte or don't
   * writting the length, depending on the number of bytes given (4,2,1,0).
   */
  public int writeChars(char[] chars, int start, int len, int lenSize)
    throws totalcross.io.IOException
  {
    int n = 0, c;
    if (len < 0) {
    ...
    if (lenSize == 2) {
      if (len > 65535) {
        throw new IOException("String size " + len +
                              " is too big to use with writeChars!");
      }
      n = writeShort(len);
    } else if (lenSize == 4) {
      n = writeInt(len);
  ...
  /**
   * Writes the String as a char array. The chars are read using the charAt
   * method from the String class. This method is faster than the other
   * writeChars method on blackberry, but slower on other devices. <p>The char
   * array size is limited to 65535 characters.
   * @param s The String to be written. Must not be null!
   * @param len The maximum number of chars to be written. Must be less than the
   *     String's length.
    ...
    throws EOFException, totalcross.io.IOException
  {
    ...
    return new String(totalcross.sys.Convert.charConverter.bytes2chars(
      buffer,
      0,
      length)); // guich@tc100b5_25: use the byte array instead
  }
  ...
  public Object readObject()
    throws ClassNotFoundException, InstantiationException,
           IllegalAccessException, EOFException, totalcross.io.IOException
  {
  ...
  public String readSmallString() throws EOFException, totalcross.io.IOException
  {
    ...
    readBytesInternal(buffer, 0, len, true);
    return new String(
      totalcross.sys.Convert.charConverter.bytes2chars(buffer, 0, len));
  }

  /**
   * Write a small String comprised of only ASCII chars (each char is casted to
   * byte). The length is written as a single byte before the byte array. <p>The
   * String size is limited to 255 characters.
   * @since TotalCross 1.0
   */
  public int writeSmallString(String s) throws totalcross.io.IOException
  {
    ...
    if (len > 255) {
      throw new IOException("String size " + s.length() +
                            " is too big to use with writeSmallString!");
    }
    writeByte(len);
    int ret = len + 1;
    if (len > 0) {
  ...
  /**
   * Write a small String taking each char as a byte. To read it, use
   * readString. The maximum allowed length is 65536.
  ...
  public int writeSmallString8(String s) throws IOException
  {
    ...
    if (len > 65536) {
      throw new IOException("String size " + s.length() +
                            " is too big to use with writeSmallString8!");
    }
    writeShort(len);
    int ret = len + 2;
    if (len > 0) {
  ...
  /**
   * Blocks until the requested number of bytes is read from the underlying
   * stream, only returning less than count if the end of the stream is reached.
   *
   * @param buf
   * @param start
   * @param count
   * @return The number of bytes read, which will usually be equal to count. It
   *     may return a positive value smaller
   *         than count if the end of the stream is reached during the read
   * operation, or -1 if the end of stream and nothing was read.
   * @throws EOFException
   * @throws totalcross.io.IOException
   */
    ...
    if (r != -1) {
      while (bytesRead < count) {
        r = stream.readBytes(
          buf,
          start + bytesRead,
          count - bytesRead); // stop after we finish reading or at eos
        if (r == -1) {
          break;
        }
        bytesRead += r;
  ...
  @Deprecated public int skip(int n) throws IOException { return skipBytes(n); }
}
