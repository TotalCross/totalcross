...
/**
 * Used to make a PDBFile act as a single Stream, reading all records in
 * sequence. Usually you use this to open a photo taken with the Palm's camera.
...
public class PDBStream extends Stream
{
  ...
  /**
   * Constructs a new PDBStream opening a PDB with the given name in READ_WRITE
   * mode.
   * @param name The pdb name of the file to open in the format Name.CRTR.TYPE.
   */
  public PDBStream(String name)
  ...
  @Override public void close() throws IOException { pdb.close(); }
  ...
  public int readBytes(byte[] buf, int start, int count) throws IOException
  {
    ...
    // could not read enough? move to next record and keep reading until we fill
    // the buffer
