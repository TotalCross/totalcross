...
public class ByteArrayStream extends RandomAccessStream
{
   ...
   * @param buffer The initial buffer from where data will be read or written
   *     into.
   */
  ...
  public ByteArrayStream(byte[] buffer, int len)
  {
    ...
    if (len < 0) {
      throw new IllegalArgumentException(
        "Argument 'len' must be greater or equal than 0");
    }
    if (len > buffer.length) {
      throw new IllegalArgumentException(
        "Argument 'len' must not be greater than 'buffer.length'");
    }
    this.len = len;
    this.buffer = buffer;
    pos = 0;
  ...
  public ByteArrayStream(int size)
  {
    if (size < 0) {
      throw new IllegalArgumentException(
        "Argument 'size' must be greater or equal than 0");
    }
  ...
  /**
   * Returns the number of bytes available in the buffer from the actual read
   * position.
   * @since SuperWaba 4.02
   */
  public int available() { return len - pos; }
   ...
   * The actual read or written data may differ from the buffer's length; use
   * the count method to get the correct value.
   * @see #count()
   */
   ...
   */
  @Deprecated public int count() { return pos; }
  ...
  /**
   * Resets the position to 0 so the buffer can be reused, and sets the mark to
   * the buffer real limits.
   * @see #mark()
   */
  public void reset()
  {
  ...
  /**
   * Moves the cursor n bytes from the current position, moving backwards if n
   * is negative, or forward if n is positive.<br> The cursor cannot be placed
   * outside the stream limits, stopping at position 0 when moving backwards, or
   * at the last position of the stream, when moving forward.
   *
   * @param n
  ...
  @Override public int skipBytes(int n)
  {
    ...
    if (off < 0) {
      ...
      off = len - pos; // jeffque@tc200: skip to the end of the buffer, not the
                       // last readable byte
    } else {
      off = n;
    }
  ...
  @Override public int writeBytes(byte buf[], int start, int count)
  {
    if (len < (count + pos)) { // grow the buffer
      int size =
        (count + pos) * 12 / 10; // grows 20% above the new needed capacity
  ...
  /**
   * Sets the size of the current byte array. If the current size is smaller
   * then the given one, a new byte array is created with the given size. If
   * there's already enough room for the given size, nothing is made. Note that
   * this method does not reset the current position.
   * @param newSize the new array size
   * @param copyOldData If true, the old data up to <code>pos</code> is copied
   *     into the new buffer.
   * @since SuperWaba 5.1
   */
  public void setSize(int newSize, boolean copyOldData)
  { // guich@510_15: added method -
    // guich@512_9: added copyOldData
    if (len < newSize) {
      byte[] buf = buffer;
  ...
  /**
   * Returns a copy of the data inside this buffer. The returned buffer will
   * have the exact size of the stored data
   * @since SuperWaba 5.1
   */
  ...
  @Override public void setPos(int offset, int origin) throws IOException
  {
    int newPos;
    ...
    if (newPos >= this.len) {
      this.pos = this.len; // flsobral@tc120: ensure the current data is copy by
                           // setSize.
  ...
  @Override public void setPos(int newPos) throws IOException
  { // flsobral@tc120: now may throw an IOException
    if (newPos < 0) {
      throw new IOException();
    }
  ...
  { // guich@570_31
    byte[] buf =
      (writeBuf != null && writeBuf.length >= bufSize)
        ? writeBuf
        : (writeBuf = new byte[bufSize]); // flsobral@tc110_71: readFully
                                          // now uses an internal buffer to
