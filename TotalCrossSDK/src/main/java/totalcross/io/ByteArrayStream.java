...
public class ByteArrayStream extends RandomAccessStream {
	 ...
	 * @since SuperWaba 5.1
	 */
	public void setSize(int newSize, boolean
						copyOldData) { // guich@510_15: added method - guich@512_9: added copyOldData
		if (len < newSize) {
			byte[] buf = buffer;
	 ...
	 * @since TotalCross 1.0
	 */
	public void readFully(Stream inputStream, int retryCount,
						  int bufSize) throws totalcross.io.IOException { // guich@570_31
		byte[] buf = (writeBuf != null
					  && writeBuf.length >= bufSize) ? writeBuf : (writeBuf = new
		byte[bufSize]); // flsobral@tc110_71: readFully now uses an internal buffer to read data before writing.
		reset();
