...
/**
 * This class allows you to automatically synchronize files between your device
 * and the desktop. Currently it supports only WinCE and PalmOS devices, and
 * desktops with Windows 2000+. It is also required that you have installed the
 * synchronization software provided by the device manufacturer. (Refer to the
 * TotalCross Companion for more details)
 *
 * <p>
 * This is the conduit's MainWindow. Two methods will be called at different
 * times: <ul> <li>doSync: when a synchronization begins. <li>doConfig: when a
 * setup (preferences) is asked by the user.
 * </ul>
 * <p>
 */
public abstract class Conduit extends totalcross.ui.MainWindow
{
   ...
   * that your MainWindow class may call.
   */
                 ...
                 byte style)
  {
    ...
    if (targetApplicationId == null) {
      throw new NullPointerException(
        "Argument 'targetApplicationId' cannot be null");
    }
  ...
  /**
   * A commandline parameter must be passed in order to invoke actions on this
   * conduit:<br> /r{target platform} register conduit.<br> /u{target platform}
   * unregister conduit.<br> /s{target platform} synchronize.<br> /c
   * configure.<br>
   *
   ...
   */
  @Override public final void initUI()
  {
      ...
      if (syncTarget != 0) {
        switch (lcmd.charAt(1)) {
          ...
          case 's': { // sync
            ...
            finishSync(); // flsobral@tc114_46: always clean up and exit after
                          // initSync.
            exit(0);
            return;
          }
  ...
  @ReplacedByNativeOnDeploy private boolean initSync() { return true; }

  @ReplacedByNativeOnDeploy private boolean finishSync() { return true; }

  /**
   * When synchronizing Palm OS devices, logs the given text on the HotSync
   * log.<br> When synchronizing Windows CE devices, the text is logged on the
   * debug console instead.
   */
  @ReplacedByNativeOnDeploy public static void log(String text)
  {
    Vm.debug(text);
  }

  /**
   * This method lets the conduit manager <i>take a breath</i>. This is
   * important, because the connection between the device and the desktop might
   * be lost during long operations.
   */
  @ReplacedByNativeOnDeploy public static void yield() { Vm.sleep(10); }

  /**
   * Called when the synchronization is started. The whole synchronization done
   * from this method.
   */
  ...
  /**
   * This method must be used so that a conduit will be called when the
   * synchronization occurs. Note that you cannot call it directly, use the
   * command line argument instead. Here is a sample:
   *
   * <pre>
   * TIConduit.exe /rp
   * </pre>
   *
   * <p>
   * Note that, to the conduit be called by HotSync, the corresponding program
   * must be installed on the PDA.
   */
  @ReplacedByNativeOnDeploy private boolean register(int priority)
  {
    return true;
  }

  /**
   * This method must be called to remove a the conduit from the
   * synchronization. Note that you cannot call it directly, use the command
   * line argument instead. Here is a sample:
   *
   * <pre>
   * TIConduit.exe /up
   * </pre>
   *
   */
  @ReplacedByNativeOnDeploy private boolean unregister() { return true; }
  ...
  {
    try {
      return Registry.getInt(Registry.HKEY_CURRENT_USER,
                             "Software\\TotalCross\\EnableSync",
                             getClassName()) == 1;
    } catch (Exception e) {
      return false;
   ...
   * You should call this method in the onRegister method, inherited by your
   * conduit. <br>This is a sample that sets the screen size to be the same of
   * "/scr palmhi" when running on the desktop: <pre>
   * setConduitRect(-2,-2,320,320,false);
   * </pre>
   * @param x Absolute position on screen, or -1 to use default, or -2 to
   *     center.
   * @param y Absolute position on screen, or -1 to use default, or -2 to
   *     center.
   * @param w The width of the window (usually 240 or 320)
   * @param h The height of the window (usually 320)
   * @param config If true, you are passing the bounds for when the config is
   *     being called; if false, is the bounds used when synchronizing.
  ...
  { // guich@tc110_27
    try {
      Registry.set(Registry.HKEY_CURRENT_USER,
                   config ? "Software\\TotalCross\\ConduitCfgRect"
  ...
  /**
   * Called when the conduit is registered. Placeholder, does nothing by
   * default.
   * @since TotalCross 1.01
   */
  ...
  /**
   * Called when the conduit is unregistered. Placeholder, does nothing by
   * default.
   * @since TotalCross 1.01
   */
