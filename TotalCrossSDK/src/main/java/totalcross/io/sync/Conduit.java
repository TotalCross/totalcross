...
public abstract class Conduit extends totalcross.ui.MainWindow {
	...
	public static void setSyncingEnabled(boolean enable) {
		try {
			Registry.set(Registry.HKEY_CURRENT_USER, "Software\\TotalCross\\EnableSync", getClassName(),
						 enable ? 1 : 0);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static boolean isSyncingEnabled() {
		try {
			return Registry.getInt(Registry.HKEY_CURRENT_USER, "Software\\TotalCross\\EnableSync",
								   getClassName()) == 1;
		} catch (Exception e) {
			return false;
		}
	...
	public static void setConduitRect(int x, int y, int w, int h, boolean config) { // guich@tc110_27
		try {
			Registry.set(Registry.HKEY_CURRENT_USER,
						 config ? "Software\\TotalCross\\ConduitCfgRect" : "Software\\TotalCross\\ConduitSyncRect",
						 getClassName(),
						 x + "," + y + "," + w + "," + h);
		} catch (Exception e) {
			e.printStackTrace();
