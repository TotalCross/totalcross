...
public final class RemotePDBFile {
	protected String
	name; // guich@572_4: made all fields protected so they are not removed by the obfuscator.
	protected int mode;
	...
	private static ByteArrayStream rwbas = new ByteArrayStream(
		65500); // the buffer used on read and write operations - max possible size
	...
	throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException {
		...
		if (read) {
			...
			// if ((idx < 0 && >= 0) || (idx >= 0 && cat.setRecordPos(idx) && cat.resizeRecord(size)))
			{
				...
				pdbFile.setRecordAttributes(idx,
	 ...
	 */
	@ReplacedByNativeOnDeploy
	final public void deleteRecord(int index) throws totalcross.io.IllegalArgumentIOException,
		totalcross.io.IOException {
