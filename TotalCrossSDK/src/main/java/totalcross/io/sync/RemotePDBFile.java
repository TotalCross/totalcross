...
/**
 * Allows you to access a pdb file on the device from the desktop during the
 * conduit synchronization.<br> On PalmOS it may only be used to handle files on
 * the device's internal memory.
 */
public final class RemotePDBFile
{
  protected String name; // guich@572_4: made all fields protected so they are
                         // not removed by the obfuscator.
  protected int mode;
  ...
  private static ByteArrayStream rwbas =
    new ByteArrayStream(65500); // the buffer used on read and write
                                // operations - max possible size
    ...
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    ...
    if ((rec.size = rwRecord(index, rwbas, true)) >
        0) { // fetch and fill our buffer
    ...
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
    ...
    if (read) {
      ...
      // if ((idx < 0 && >= 0) || (idx >= 0 && cat.setRecordPos(idx) &&
      // cat.resizeRecord(size)))
      {
        pdbFile.writeBytes(bas.getBuffer(), 0, size);
        // guich@570_14: reset the dirty flag
        pdbFile.setRecordAttributes(
          idx,
          (byte)(pdbFile.getRecordAttributes(idx) & ~PDBFile.REC_ATTR_DIRTY));
        return size;
      }
    }
  }
   ...
   */
  public int getRecordPos() { return recIndex; }
   ...
   */
  @ReplacedByNativeOnDeploy
  final public void deleteRecord(int index)
    throws totalcross.io.IllegalArgumentIOException, totalcross.io.IOException
  {
   ...
   */
  public int skipBytes(int n) { return rwbas.skipBytes(n); }
  ...
  {
    idle =
      true; // guich@tc114_95: set it here before close, because if close
            // throws an exception the user will never be able to sync again.
   ...
   */
  @ReplacedByNativeOnDeploy public static String[] listPDBs(int crtr, int type)
  {
    // get the list
    String[] list = PDBFile.listPDBs(crtr, type);
    // now strip the first _ so that the user gets the expected name of a
    // RemotePDBFile
    if (list != null) {
