 ...
 * stream. Consecutive newlines are skipped. This class does not work well with
 * multi-byte characters when the second byte contains the delimiter or
 * enter/linefeed. <p> Here's a sample:
 *
 * <pre>
 * LineReader reader = new LineReader(new
 * File(&quot;text.txt&quot;,File.READ_WRITE)); String line; while ((line =
 ...
 * newline. Don't use LineReader with a BufferedStream, it's nonsense and it
 * will throw a warning on the desktop.
 *
...
{
  protected Stream f;
  protected ByteArrayStream readBuf = new ByteArrayStream(
    !Settings.onJavaSE ? 4096
                       : 32768); // guich@554_10: increase buffer size on applet
  protected int ofs;
  /**
   ...
   */
  public LineReader(Stream f, byte[] buffer, int start, int len)
    throws totalcross.io.IOException
  { // guich@tc125_16
    this.f = f;
   ...
   * @since TotalCross 1.23
   */
  public Stream getStream() { return f; }
  ...
  protected boolean readMore() throws totalcross.io.IOException
  {
    ...
    if (readBuf.available() == 0) {
      readBuf.setSize(readBuf.getPos() + 1024,
                      true); // grow the buffer if needed
    }
    int r =
      f.readBytes(readBuf.getBuffer(), readBuf.getPos(), readBuf.available());
    if (r < 0) {
      for (int i = maxTries - 1; i >= 0; i--) {
        Thread.yield();
        r = f.readBytes(
          readBuf.getBuffer(), readBuf.getPos(), readBuf.available());
        if (r > 0) {
          break;
        }
      }
  ...
  public String readLine() throws totalcross.io.IOException
  {
    ...
    while (true) {
      ...
      for (i = ofs; i < size; i++) {
        if (buf[i] == '\n') { // found an enter? - guich@tc123_31
          ...
          if (len > 0 || returnEmptyLines) {
            ...
            String s =
              new String(Convert.charConverter.bytes2chars(buf, ofs, len));
            ofs = i;
            return s;
          }
      ...
      if (!foundMore) {
        ...
        if (len > 0 ||
            (foundMore &&
             returnEmptyLines)) { // any remaining string on the buffer?
          if (foundEnter) {
            ofs = len;
          }
          int len0 = len;
