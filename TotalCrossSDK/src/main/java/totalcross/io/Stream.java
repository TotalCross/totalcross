...
public abstract class Stream extends Connection
{
  ...
  /**
   * Reads bytes from the stream. Returns the number of bytes actually read or
   * -1 if the end of the stream was reached. (if applicable to the stream)
   *
   * @param buf
  ...
  /**
   * Writes bytes to the stream. Returns the number of bytes actually written or
   * throws an <code>IOException</code> if an error prevented the write
  ...
  /**
   * Writes the string to the stream as a byte array. Returns the number of
   * bytes actually written or throws an <code>IOException</code> if an error
   * prevented the write operation from occurring.<br> The String is written
   * as-is, nothing is placed before or after it.
   *
   * @param string
  ...
  /**
   * Writes the given StringBuffer as a byte array, retrieved using
   * Convert.getBytes.<br> The String is written as-is, nothing is placed before
   * or after it.
   *
   * @param sb
   ...
   */
  final public int writeBytes(StringBuffer sb) throws totalcross.io.IOException
  {                                  // guich@tc123_44
  ...
  /**
   * Skips over and discards n bytes of data from this stream. The skip method
   ...
   * negative, no bytes are skipped, but this may vary among some
   * implementations of skipBytes in classes that inherit them (like File and
   * PDBFile). <br> <br> The skip method of this class uses a static byte array,
   * repeatedly reading into it until n bytes have been read or the end of the
   ...
   * @throws totalcross.io.IOException
   *            if the stream does not support skip, or if some other I/O error
   * occurs.
   */
   ...
   */
  @Deprecated public InputStream wrapInputStream() { return asInputStream(); }

  public InputStream asInputStream() { return new WrapInputStream(this); }

  public OutputStream asOutputStream() { return new WrapOutputStream(this); }
...
class WrapInputStream extends InputStream
{
  ...
  WrapInputStream(Stream stream) { this.stream = stream; }
...
class WrapOutputStream extends OutputStream
{
  ...
  WrapOutputStream(Stream stream) { this.stream = stream; }
...
class WrapFromInputStream extends Stream
{
    ...
    throws totalcross.io.IOException
  {
    throw new totalcross.io.IOException(
      "Can't write to a wrapped input stream");
  }

  @Override public int writeBytes(byte[] buf) throws totalcross.io.IOException
  {
    throw new totalcross.io.IOException(
      "Can't write to a wrapped input stream");
  }
  ...
  @Override public int skipBytes(int n) throws totalcross.io.IOException
  {
    try {
  ...
  @Override public InputStream asInputStream() { return inputStream; }
...
class WrapFromOutputStream extends Stream
{
  ...
  {
    throw new totalcross.io.IOException(
      "Can't read from a wrapped output stream");
  }

  @Override public int skipBytes(int n) throws totalcross.io.IOException
  {
    throw new totalcross.io.IOException(
      "Can't skip in a wrapped output stream");
  }

  @Override public OutputStream asOutputStream() { return outputStream; }
