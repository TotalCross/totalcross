 ...
 * It is used in most cases with a DataStream. For example:
   ...
   PS: if you dont call startRecord, writeBytes will simply call
 PDBFile.writeBytes and will not resize the record!
   @since SuperWaba 5.8
 */
...
{ // guich@580_19
  ...
  private boolean isRecordClosed =
    true; // flsobral@tc113_30: flag to avoid the execution of endRecord twice
          // for the same record.
   ...
   *
   * @param pos
   *           The position where to insert the record. If greater or equal to
   * the record count, the record is appended.
   * @throws IOException
   ...
   * @see #endRecord()
   */
  public void startRecord(int pos) throws IOException
  { // flsobral@tc100b5_49: returning boolean is no
    // longer necessary.
   ...
   * @param pos The position to overwrite. If pos is lower than 0 or greater
   *     than the
  ...
  public boolean restartRecord(int pos) throws IOException
  {
    if (pos < 0 || pos >= cat.getRecordCount()) {
      ...
      size = cat.getRecordSize(); // guich@300_35: initialize to the current
                                  // record size
    }
    return true;
  }
  ...
  public Stream getStream() { return cat; }
