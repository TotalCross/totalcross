...
public class File extends RandomAccessStream implements FileStates {
	 ...
	 */
	@Deprecated
	public File(String path, int mode, int slot) throws IllegalArgumentIOException,
		...
		if (path.startsWith("device/")) {
			path = Convert.appendPath(Settings.appPath,
									  path.substring(6)); // guich@tc310: in desktop was using the root folder of current drive
		}
	 ...
	 */
	public File(String path, int mode) throws IllegalArgumentIOException, FileNotFoundException,
		IOException {
		this(path, mode, -1);
	}
	...
	final private void create() throws FileNotFoundException, IOException {
		...
		if (mode != DONT_OPEN) {
			...
			try {
				...
				mode = INVALID;
				if (wasCreate) {
					throw new IOException("Folder not found or there's already a folder with the same name of the file: "
										  + path);
				}
				throw new FileNotFoundException(path);
	...
	final public String[] listFiles() throws IOException {
		...
		try {
			...
			String pathWithSlash = path.endsWith("/") ? path : (path +
								   '/'); // guich@564_5: check if the path ends with /
			if (files != null) {
		...
		try {
			if (pos > fileEx4Java.length()) {
				fileEx4Java.setLength(pos +
									  1); // suppose the file is empty. seeking to pos 8 makes the file with 9 bytes (0-8).
			}
	 ...
	 * @see #TIME_MODIFIED
	 */
	final public void setTime(byte whichTime,
							  totalcross.sys.Time time) throws IllegalArgumentIOException, IOException {
		if (mode == INVALID || mode == CLOSED) {
			throw new IOException("Invalid file handle");
		}
		if (mode == DONT_OPEN) {
	 ...
	 * @see #TIME_MODIFIED
	 */
	final public totalcross.sys.Time getTime(byte whichTime) throws IllegalArgumentIOException,
		IOException {
		if (mode == INVALID || mode == CLOSED) {
			throw new IOException("Invalid file handle");
		}
	...
	final public void setSize(int newSize) throws IOException {
		...
		try {
			fileEx4Java.setLength(
				newSize); // suppose the file is empty. seeking to pos 8 makes the file with 9 bytes (0-8).
	 ...
	 */
	final public static String getCardSerialNumber(int slot) throws IllegalArgumentIOException,
		IOException {
		if (slot < -1) {
			throw new IllegalArgumentIOException("slot", Convert.toString(slot));
		}
	...
	private static void listFiles(String dir, Vector files,
								  boolean recursive) throws IOException { // guich@tc115_92
		try
			(File f = new File(dir)) {
	...
	public static void copy(String src, String dst) throws IOException { // guich@tc126_43
		try
			(File fin = new File(src, READ_ONLY)) {
			try
				(File fout = new File(dst, CREATE_EMPTY)) {
				fin.copyTo(fout);
			}
		}
	}
	...
	public static void move(String src, String dst) throws IOException { // guich@tc126_43
		try
			(File fin = new File(src, READ_WRITE)) {
			try
				(File fout = new File(dst, CREATE_EMPTY)) {
				fin.moveTo(fout);
			}
