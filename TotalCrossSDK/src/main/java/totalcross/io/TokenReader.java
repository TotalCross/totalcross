 ...
 * from a stream. Consecutive newlines are skipped. This class does not work
 * well with multi-byte characters when the second byte contains the delimiter
 * or enter/linefeed. <br><br>The usual way to parse a CSV file is: <pre> do
 *   line = readline
 *   Convert.tokenizeString(line)
 * </pre>
 * Using this class takes less memory, because the line is read in tokens. For
 * example, suppose a line contains 200 chars, and splitting them contains 10
 * tokens of 20 chars each. Using the first approach (readline/tokenizeString),
 * the readline will create a string with 200 chars, then that String will be
 * tokenized into 10 smaller strings of 20 chars each.<br><br> Using this class,
 * it will read the 10 tokens of 20 chars each directly, no longer having to
 * create the temporary string of 200 chars. <br><br> The delimiter can be any
 * character except for \r and \n. Note that two consecutive delimiters are
 * considered a single token. So <code>;a;;</code> is returned as
 * <code>{"","a","",""}</code>. <br><br> Here's a sample that parses the input
 * from a file:
 *
 * <pre>
 * TokenReader reader = new TokenReader(new
 * File(&quot;text.csv&quot;,File.READ_WRITE), ','); String[] tokens; while
 * ((tokens = reader.readTokens()) != null)
 * {
 ...
 * 0: 'b'
 * </pre>
 * Note that this class already uses a buffer for faster detection of the
 * newline and delimiters. Don't use TokenReader with a BufferedStream, it's
 * nonsense and it will throw a warning on the desktop.
 *
...
public class TokenReader extends LineReader
{
   ...
   * Constructs a new TokenReader and sets maxTries accordingly to the type of
   * class: 10 if its a Socket or a PortConnector, 0 otherwise.
   * The given buffer contents are added to the internal buffer to start reading
   * from them.
   *
   * @throws totalcross.io.IOException
   * @since TotalCross 1.25
   */
  public TokenReader(Stream f,
                     char delimiter,
                     byte[] buffer,
                     int start,
                     int len) throws totalcross.io.IOException
  { // guich@tc125_16
    ...
    while (true) {
      ...
      for (i = ofs; i < size; i++) {
        if (buf[i] == delimiter ||
          ...
          int ii = ofs + len;
          if (doTrim && len > 0 &&
              (buf[ofs] <= ' ' || buf[ii - 1] <= ' ')) { // guich@tc123_37
      ...
      if (!foundMore) {
        ...
        lastOfs = 0;
        if (doTrim && len > 0 &&
            (buf[0] <= ' ' || buf[len - 1] <= ' ')) { // guich@tc123_37
