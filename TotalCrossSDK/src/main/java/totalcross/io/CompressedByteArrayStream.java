 ...
 * writting huge amount of data. Arrays of 16000 bytes will be created and each
 * byte array will be compressed once filled and will be automatically
 * decompressed on read. This saves space but adds a slowdown to the process. It
 * is useful when transferring FTP files to/from the server. <p> This class
 * cannot be used for output AND input, but only for output OR input, in an
 * absolutely sequential mode (the skipBytes method is NOT implemented): you
 * must write everything, then read everything. To change the mode, use the
 ...
 * mode is WRITE_MODE for (int i = 0; i &lt; 50000; i++)
 ...
 * String g = &quot;1234567890&quot;;
 * CompressedByteArrayStream cbas = new CompressedByteArrayStream(9); // default
 * mode is WRITE_MODE for (i = 0; i &lt; 50000; i++) cbas.writeLine(g); //
 * already appends \r\n cbas.flush(); Vm.debug(&quot;size: &quot; +
 * cbas.getCompressedSize() + &quot; -&gt; &quot; + cbas.getSize()); String s;
 ...
 * </pre>
 *
 * Note that, although the samples above use writeLine and readLine, you can
 * store any kind of data. By attaching a DataStream it's possible to read any
 * data type from the stream.
 *
 * <pre>
...
{
  /**
   * Implements a CharacterConverter that converts from char[] to byte[] which
  ...
  {
    /** Just casts the char to byte; thus, ignoring any non-ASCII character. */
    @Override public byte[] chars2bytes(char[] chars, int offset, int length)
    {
      offset += length;
   ...
   * each buffer, discards it, releasing memory. CompressedByteArrayStream will
   * not be able to read the buffer again. This is useful when you download data
   * and then want to read from it, releasing memory on-demand.
   */
  public static final int DESTRUCTIVE_READ_MODE = 2; // guich@570_28

  /**
   * Defines the line terminator, which is by default \r\n. To change it to a
   * single \n use <code>CompressedByteArrayStream.crlf = new
   * byte[]{'\n'};</code>
   */
  ...
  private ByteArrayStream buf =
    new ByteArrayStream(SIZE); // current read/write buffer
  private byte[] bufbytes =
    buf.getBuffer(); // since buf size won't change, this is safe
  private static ByteArrayStream temp =
    new ByteArrayStream(SIZE); // used to compress/uncompress
  private StringBuffer sbuf;   // used in readLine
  private byte[] writeBuf;     // used in readFully

  /**
   * Creates a new CompressedByteArrayStream, using the given compression level
    ...
    throws IllegalArgumentException
  {
    if (compressionLevel < 0 || compressionLevel > 9) {
      throw new IllegalArgumentException(
        "Argument 'compressionLevel' must be >= 0 and <= 9");
    }
    this.compressionLevel = compressionLevel;
  }

  /**
   * Creates a new CompressedByteArrayStream using the maximum compression level
   * (9)
   */
  public CompressedByteArrayStream() { this(9); }

  /**
   * After everything was written, call this method to flush the internal
   * buffers and prepare the CompressedByteArrayStream for read. It is already
   * called by setMode when it changes the modes.
   * @throws IOException
   * @see #setMode(int)
   */
  ...
  /**
   * Changes the mode to the given one, calling <code>flush</code> if in write
   * mode.
  ...
  {
    // flsobral@tc100b5_45: Stream was not being reseted when the new mode was
  ...
  {
    buf.mark(); // note: unless for the last buffer, all the others will have
                // SIZE bytes
    temp.reset();
  ...
  public int readBytes(byte buffer[], int start, int count) throws IOException
  {
    if (start < 0) {
      throw new IllegalArgumentException(
        "Argument 'start' cannot be less than 0");
    }
    if (count < 0) {
      throw new IllegalArgumentException(
        "Argument 'count' cannot be less than 0");
    }
   ...
   * @return the number of bytes written. If an error occurred, -1 is returned
   *     and
   * @throws IOException, IllegalArgumentException
   * @since SuperWaba 2.0 beta 2
   */
  @Override
  public int writeBytes(byte buffer[], int start, int count)
    throws IOException, IllegalArgumentException
  {
    if (start < 0) {
      throw new IllegalArgumentException(
    ...
    while (true) {
      ...
      saveCurrentBuffer(); // if failed, this will just abort the program with a
                           // OutOfMemoryError
    }
    orig -= count;
    rSize += orig;
  ...
  /**
   * Reads a String until the next control character (newline, enter, tab, etc)
   * is read.
   * @return A line of text read from internal buffer or null if no more lines
   *     are available.
   * @throws IOException
   */
  public String readLine() throws IOException
  {
    ...
    while (!stop) {
      ...
      while (a > 0 &&
             (b[p] == '\r' ||
              b[p] == '\n')) { // skip starting enters - guich@565_10: discard
                               // negative values - guich@tc123_31: use only \r
                               // and \n as delimiters
        if (sb.length() > 0) {
          return sb.toString();
        }
      ...
      // search for the \r\n
      for (; a > 0; i++, a--) {
        if (b[i] == '\r' || b[i] == '\n') { // guich@565_10: discard negative
                                            // values - guich@tc123_31
          stop = true;
          break;
        }
      }
   ...
   * @param retryCount  The number of times to retry if no data is read. In
   *     remote connections,
   ...
   * @since SuperWaba 5.7
   */
  public void readFully(Stream inputStream, int retryCount, int bufSize)
    throws IOException
  { // guich@570_31
    byte[] buf = (writeBuf != null && writeBuf.length >= bufSize)
                   ? writeBuf
  ...
  public String readUntilNextChar(char c) throws IOException
  {
    ...
    while (!stop) {
      ...
      byte[] b = bufbytes;
      while (a > 0 &&
             (b[p] & 0xFF) == c) { // skip starting enters - guich@565_10:
