...
public class CompressedByteArrayStream extends Stream {
	...
	public void readFully(Stream inputStream, int retryCount,
						  int bufSize) throws IOException { // guich@570_31
		byte[] buf = (writeBuf != null
					  && writeBuf.length >= bufSize) ? writeBuf : (writeBuf = new byte[bufSize]);
		setMode(WRITE_MODE);
		while (true) {
	...
	public String readUntilNextChar(char c) throws IOException {
		...
		while (!stop) {
			...
			byte[] b = bufbytes;
			while (a > 0
				   && (b[p] & 0xFF) == c) { // skip starting enters - guich@565_10: discard negative values
				if (sb.length() > 0) {
					return sb.toString();
				}
				p++;
