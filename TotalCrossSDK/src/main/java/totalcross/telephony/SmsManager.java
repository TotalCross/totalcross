...
public class SmsManager
{
   ...
   *
   * @param receiver the receiver that will handle incoming sms messages or null
   *     to stop listening incoming messages
   * @param port the port to listen for incoming sms messages - required for
   *     data sms messages, pass -1 to listen on default port for text messages
   */
   ...
   *
   * @param receiver the receiver that will handle incoming text sms messages or
   *     null to stop listening incoming messages
   */
   ...
   * @param scAddress the service center address or null to use the current
   *     default SMSC
                              ...
                              String text)
  {
    if (destinationAddress.length() == 0) {
      throw new IllegalArgumentException(
        "Argument destinationAddress cannot be empty");
    }
    if (text.length() == 0) {
      throw new IllegalArgumentException("Argument text cannot be empty");
    }
    System.out.println("To: " + destinationAddress + ", From: " + scAddress +
                       ", Text: " + text);
  }
