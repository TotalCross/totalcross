...
class GameEngineMainWindow extends MainWindow
{
  ...
  protected GameEngineMainWindow() { paintBackground = false; }
   ...
   */
  @Override public final void onExit()
  {
    engine.onGameExit();
    shutdown();
  }
   ...
   */
  TextRenderer createTextRenderer(Font font,
                                  int foreColor,
                                  String text,
  ...
  { // fdie@420_27
    return new TextRenderer(
      font, foreColor, backColor, text, maxDigits, zeroPadding);
  }
  ...
  void refresh()
  {
    if (!engine.gameHasUI) {
      if (flicker && bgSurface == null) {
        ...
        paintBackground =
          flicker; // continue background display if "clearscreen" is enabled
      }
      engine.onPaint(graf);
   ...
   *           event that occurs.
   */
  @Override public final void onEvent(Event evt)
  {
    switch (evt.type) {
