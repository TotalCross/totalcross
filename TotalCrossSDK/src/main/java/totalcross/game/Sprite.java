 ...
 * with collision detection feature and background saving/restoring
 * capability.<p> The sprite attributes are:<br> <p> <li>position<br> The sprite
 * position <U>centerX,centerY</U> is in fact its center position.<br> <br>
 * <li>size<br>
 * <U>width/height</U> contains respectively the image horizontal/vertical
 * dimensions.<br> <br> <li>half size<br> <U>halfWidth/halfHeight</U> contains
 * respectively the half horizontal/vertical dimensions.<br> <br> <li>valid
 * region<br> The valid region of a sprite are relative to the sprite center.
 * The valid region can be provided in the Sprite <i>constructor</i> or by the
 * <i>setRegion</i> call. Both are totalcross.ui.gfx.Rect objects. If null is
 * passed in the constructor the default region is computed to prevent the
 * sprite living even partially the screen.<br> Once set the region can be
 * modified by incrementing/decrementing the values of <U>regionMinx</U>,
 * <U>regionMiny</U>, <U>regionMaxx</U> and <U>regionMaxy</U><br> <br> <li>draw
 * operation<br> The default value of <U>drawOp</U> is DRAW_PAINT if its value
 * is DRAW_PAINT the image is copied "as is" to the buffer, and<br> if its value
 * is DRAW_SPRITE the transparency color specified in the <i>constructor</i> is
 * used to prevent the copy of image pixels of this color in order to preserve
 * the background. <p> You can find a complete game API sample named 'Scape' in
 * the TotalCross samples folder.<br> Here is some sample code:
 *
 * <pre>
 * // Ball is a the Sprite bouncing on the screen.
 * // The ball bounces on the left, the top and the bottom border and
 * // must be hit by the racket on the right border to keep the ball inside the
 * screen.
 ...
 *   private Ping game;
 *
 *   // other important sprite the ball sprite must know about for
 * interaction... private Racket racket;
 *
 *  // ball movement speed in both directions in double values for acceleration
 * precision private double speedx,speedy;
 *
 ...
 *
 *     // setup the sprite by loading the bitmap, defining the transparency
 * color.
 *     // The ball element does not fill the whole bitmap, the corners are
 * filled
 *     // with WHITE pixels. By setting WHITE as the transparency color and
 * selecting the
 *     // DRAW_SPRITE draw mode, the bitmap white pixels are not shown and the
 * background
 *     // is not affected by the bitmap display. The background saving feature
 * is disabled
 *     // because the whole screen is cleared and redrawn at each frame. No
 * valid region
 *     // (null) means, default valid region is right. Except that the miny
 * value must be
 ...
 *     // see the "Scape" sample code for more details about position
 * validation.
 *     // this towardPos() moves the sprite toward a point, the third parameter
 *     // enables position validation which is needed for each position change
 * to
...
public class Sprite
{
   ...
   * The sprite cannot leave this area if the default position validation
  ...
  /**
   * set to false if this sprite never reaches the screen boundaries. Default
   * is true.
   */
   ...
   * @param saveBckgd
   *           true if the background should be saved each time the sprite is
   * drawn to restore it once the sprite moves.
   * @param region
   *           defines the sprite valid area.<br>
   *           If null, a default region is set to prevent the sprite to leave
   * even partially the screen.
  ...
  {
    return new Rect(regionMinx,
                    regionMiny,
                    regionMaxx - regionMinx + 1,
                    regionMaxx - regionMiny + 1);
  }
   ...
   * This function can be overloaded to define your own sprite position
   * validation or collision detection.<br> The overloaded function may use the
   * sprite data members and should update the sprite position centerX and
   * centerY if needed.
   *
   * @return true if the (centerX,centerY) is a valid position, false if it's
   *     not and the position has been corrected.<br>
   *         <B>NOTE</B>: false returns will stop the movements of the towardPos
   * function.
   * @see #towardPos
   */
   ...
   * Note: if doValidate is false, you may consider just setting centerX and
   * centerY attributes directly (it is 60% faster).
   *
   * @param x position.
   * @param y position.
   * @param doValidate if true the position is validated which means that the
   *     onPositionChange() function is called.
   ...
   * <B>NOTE</B>: should be called cautiously due to a Coord object alloc. You
   * may also consider accessing the centerX and centerY directly.
   *
   * @return sprite's center position
   */
  public final Coord getPos() { return new Coord(centerX, centerY); }

  /**
   * Moves the sprite toward the specified position. <br>
   * This function is typically used when the user points the end position with
   * the pen. The object position is computed to move smoothly from it's current
   * position to the target position.
   *
   * @param x position.
   * @param y position.
   * @param doValidate if true the position is validated which means that the
   *     onPositionChange() function is called.<br>
   *           <B>NOTE</B>: a false return of onPositionChange() will stop the
   * towardPos function.
   * @return true if the defined position has been set
   * @see #onPositionChange
   */
  public boolean towardPos(int x, int y, boolean doValidate)
  {
    ...
    if (dx == 0) { // vertical move
      ...
      dx =
        dx >= 0 ? dx : -dx; // store the change in X and Y of the line endpoints
      dy = dy >= 0 ? dy : -dy;
      ...
      if (dx >= dy) { // if X is the independent variable
        ...
        for (; dx >= 0 && steps > 0; dx--) { // process each point in the line
      ...
      // if Y is the independent variable
      {
        ...
        for (; dy >= 0 && steps > 0; dy--) { // process each point in the line
   ...
   * If the Sprite has been created with enabled background saving (usefull only
   * when the screen is not cleared before the new frame display) the previously
   * stored background will be restored first and the sprite is displayed in a
   * second step.<br>
   * <B>NOTE</B>: if several sprite moves simultaneously and may overlap the
   * background restoring may erase a previously drawn sprite. In this case, the
   * only solution is to call explicitly the hide() function on all the sprites
   * in the DRAWN REVERSE ORDER.<br> By doing so, you always restore a clean
   * image without sprites before starting a new draw cycle of all the sprites.
   */
  public void show()
  {
    int w2 = width >> 1;
   ...
   * <B>NOTE</B>: the position may have change, the saved background position
   * has been memorized also, to restore it at the right place.
   */
  public void hide()
  {
    ...
    gfx.copyRect(background,
                 0,
                 0,
                 width,
                 height,
                 bgX - width / 2,
                 bgY - height / 2); // buffer -> screen
    bgX = INVALID;
  }
  ...
  public boolean collide(Sprite s)
  {
    ...
    int sh2 = s.height >> 1;
    return !(
      (s.centerX + sw2 <= centerX - w2) || (s.centerY + sh2 <= centerY - h2) ||
      (s.centerX - sw2 >= centerX + w2) || (s.centerY - sh2 >= centerY + h2));
  }
}
