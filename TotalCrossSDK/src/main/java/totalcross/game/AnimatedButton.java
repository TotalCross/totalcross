 ...
 * <br>
 * This control displays an animated button which can take <code>'S'</code>
 * different states and each state is fades in or out in <code>'F'</code>
 * frames. <code>'S'</code> and <code>'F'</code> represent the two first
 * constructor arguments. The frames of this special animation have to be
 * ordered to be supported by this class. The states are numbered from
 * <code>0</code> to <code>'S'-1</code> and the frames order is the following
 ...
 * </pre>
 *
 * where <code>S</code> stands for state, <code>F</code> for frame; and where
 * <code>S0F0</code>, <code>S1F0</code>, and <code>S2F0</code> are the full
 * states and the others are transition frames. Open the <code>onOff.bmp</code>
 * in the Scape game sample and you will understand ;-) .
 *
...
{
  /**
   * Defines the frames animation order. In the case of <code>S</code> states
   * button of <code>F</code> frames per state, <code>FADE_OUT_LAYOUT</code>
   * means that the frames are a <code>S</code> set of <code>F</code> frames
   * that are fading out the state, which means the first frame of each set is
   * the full state image. In the <code>FADE_IN_LAYOUT</code> layout, it's the
   * opposite, namely the last frame of each set represents the state ending
   * position. Finally the <code>FADE_OUT_IN_LAYOUT</code> is a mix of the two
   * others, because inter-frames represent successively fading out from one
   * state to fading in to next state.
   */
   ...
   * @param layoutType <code>FADE_OUT_LAYOUT</code>,
   *     <code>FADE_IN_LAYOUT</code>, or <code>FADE_OUT_IN_LAYOUT</code>.
  ...
  public AnimatedButton(Image frames,
                        ...
                        int framesPerState,
                        int layoutType,
                        int framePeriod) throws ImageException
  { // fdie@341_2
    ...
    for (int s = 0; s < states; s++) {
      statesIndexes[s] = (layoutType == FADE_IN_LAYOUT)
                           ? ((s + 1) * framesPerState) - 1
                           : s * framesPerState;
    }
   ...
   *
   * @param up Boolean with a <code>true</code> value to increase the value;
   *     decrease otherwise.
   */
