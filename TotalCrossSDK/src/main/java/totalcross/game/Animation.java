 ...
 * (one frame per image) or by a multi-frames BMP. This kind of BMP file
...
{
  /**
   * Sets to true to stop the animation if its parent window is not the top
   * most.
   */
  ...
  /**
   * Event notification mask, whose value is <code>eventFinish<code>, which
   * means that an event is posted only when the animation finishes.
   */
  protected int eventsMask =
    eventFinish; // fdie@341_3 new notification mechanism
  ...
  // fdie@341_6+
  /**
   * <code>start()</code> method <code>loops</code> argument special value to
   * loop endlessly.
   */
   ...
   */
  protected Animation() { super(); }

  /**
   * Animation constructor.
   *
   * @param frames Single image containing all frames. The number of frames and
   *     the transparent colors are fetched from the image.
   ...
   * @throws ImageException If an internal method throws it.
   */
  public Animation(Image frames, int frameCount, int framePeriod)
    throws ImageException
  { // fdie@341_2 : direct multi-frame image
    // constructor
    super();
   ...
   * @throws ImageException If an internal method throws it.
   */
  public void setImage(Image frames, int frameCount, int framePeriod)
    throws ImageException
  {
  ...
  @Override public void onEvent(Event e)
  {
    if (animTimer != null && animTimer.triggered &&
        !(pauseIfNotVisible &&
          (getParentWindow() == null || !getParentWindow().isVisible()))) {
      paintNextFrame();
    } else {
      super.onEvent(e);
    }
   ...
   */
  public int size() { return framesBuffer.getFrameCount(); }
   ...
   */
  @Override public int getPreferredWidth() { return width; }
   ...
   */
  @Override public int getPreferredHeight() { return height; }
   ...
   */
  @Override public void onPaint(Graphics gfx)
  {
    // fdie@400_51 : save animation background - no need in OpenGL, since the
    // screen is fully painted at each frame
    if (!Settings.isOpenGL && gfx.isControlSurface()) {
      if (background == null) {
        // guich@tc100: on a screen rotation, we would have to re-get the
        // background!
        try {
    ...
    // flsobral@tc100b5_6: argument doClip is now true, this avoids exceptions
    // when the image is larger than the screen.
    gfx.drawImage(framesBuffer, 0, 0, true);
  }

  /**
   * Enable the posting of events. By default the posting of events are
   * disabled.
   *
   */
  ...
  /**
   * Pauses a running animation. If the animation is not playing, this call has
   * no effect.
   */
  public void pause()
  {
  ...
  /**
   * Resumes a paused animation. If the animation is not playing, this call has
   * no effect.
   */
  public void resume()
  {
  ...
  /**
   * Stops the animation. If the animation is not playing, this call has no
   * effect.
   */
  public void stop()
  {
  ...
  private void paintNextFrame()
  {
    if (isPlaying && !isPaused) {
      ...
      if (curFrame != endFrame) {
        ...
        framesBuffer.setCurrentFrame(curFrame);
      } else if (loopCount == LOOPS_UNLIMITED ||
                 --loopCount >
                   0) { // fdie@341_6 will be the only loop management once
                        // the above boolean loop is removed
        curFrame = startFrame;
        framesBuffer.setCurrentFrame(curFrame);
  ...
  /**
   * Starts the animation with a frame range. This method starts an animation by
   * specifying the frame range and a loop flag. If the application is already
   * playing, this call has no effect.
   *
  ...
  // fdie@341_6+
  /**
   * Starts the animation. This method starts the animation and loops the
   * specified amount of time. If the application is already playing, this call
   * has no effect.
   *
   * @param loops Integer value specifying the number of loops or
   *     <code>LOOPS_UNLIMITED</code> for an infinite loop.
   *
   * @see #LOOPS_UNLIMITED
   */
  public void start(int loops) { start(0, frameCount - 1, 1, loops); }
}
