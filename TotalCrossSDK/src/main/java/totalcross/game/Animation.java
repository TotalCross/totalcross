...
public class Animation extends Control {
	 ...
	 * @throws ImageException If an internal method throws it.
	 */
	public Animation(Image frames, int frameCount,
					 int framePeriod) throws ImageException { // fdie@341_2 : direct multi-frame image constructor
		super();
		...
		if (isPlaying && !isPaused) {
			...
			if (curFrame != endFrame) {
				...
				framesBuffer.setCurrentFrame(curFrame);
					   ...
					   || --loopCount >
					   0) { // fdie@341_6 will be the only loop management once the above boolean loop is removed
				curFrame = startFrame;
				framesBuffer.setCurrentFrame(curFrame);
