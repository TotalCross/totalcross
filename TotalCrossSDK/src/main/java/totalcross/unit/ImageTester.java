 ...
 * {
 *    private Graphics g;
 *    private boolean learning = false; // change this to true to generate the
 * strings
 *
 *    // generated string during learning stage
 *    private String drawLine =
 * "";
 *
...
{ // guich@565_7
    ...
    if (originalObj instanceof String) { // created with the toString below?
      ...
      for (j = 0; j < h; j++) {
        ...
        for (i = 0; i < w; i++) {
          ...
          if (test != orig) {
            new CompareBox(
              title, rgbs, orig, test, i, j, n, originalbytes, this)
              .popup();
  ...
  public static void hex2bytes(String str, ByteArrayStream bas)
  {
    ...
    for (int i = 0; i < size; i++) {
      out[i] = (byte)(Convert.digitOf((char)in[ofs++], 16) << 4 |
                      Convert.digitOf((char)in[ofs++], 16));
    }
    bas.reset();
    bas.skipBytes(size);
    bas.mark();
