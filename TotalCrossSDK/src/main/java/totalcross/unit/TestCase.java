...
public abstract class TestCase {
	...
	protected void run() { // guich@565_6
		try {
			...
			testRun();
					 ...
					 e) { // this error must be catched here, otherwise, the vm will issue an "Uncaugh Exception" and then halt.
	...
	protected static void assertEquals(byte[] b1, byte[] b2) {
		...
		for (int i = 0; i < b1.length; i++) {
			if (b1[i] != b2[i]) {
				assertFailed(
					"Mismatch at " + i + ": " + Convert.unsigned2hex(b1[i], 2) + " != " + Convert.unsigned2hex(b2[i],
							2));
			}
		}
	}
	...
	protected static void assertGreater(char left,
										char right) { // guich@565_5: added Greater/GreaterOrEqual/Lower/LowerOrEqual
		assertionCounter++;
