...
/**
 * This class represents a testcase. Just extend this class and implement the
 * testRun method, calling then the assert methods. The testcase must be added
 * to a TestSuite class using the addTestCase method. See the Litebase/alltests
 * sample.
 */
...
public abstract class TestCase
{
  ...
  /**
   * Set this to true when your code is <i>learning</i> the right answers. It
   * will not halt at each assertion failure, allowing you to know all the
   * failures at once. When the test ends, it will display "success" even if
   * there were failures.
   */
  ...
  { // guich@565_6
    try {
      ...
      testRun();
               ...
               e) { // this error must be catched here, otherwise, the vm will
                    // issue an "Uncaugh Exception" and then halt.
  ...
  /**
   * Shows the text, in a limited (half screen width) region of the TestSuite
   * screen.
   */
  protected static void status(String s) { TestSuite.status(s); }

  /** Updates the label with the available memory. */
  protected static void updateMem() { TestSuite.updateMem(); }

  protected static void fail(Throwable e)
  {
    fail("\nOriginal exception: " + e.getClass().getName() + " - " +
         e.getMessage() + "\nOriginal stackTrace:\n" + Vm.getStackTrace(e));
  }
  ...
  protected static void fail() { fail("Failed"); }
  ...
  protected static void assertNotSame(java.lang.Object expected,
  ...
  protected static void assertSame(java.lang.Object expected,
                                   java.lang.Object actual)
  {
    assertionCounter++;
    boolean ok = (expected == null && actual == null) ||
  ...
  protected static void assertEquals(java.lang.Object expected,
                                     java.lang.Object actual)
  {
    assertionCounter++;
    boolean ok = (expected == null && actual == null) ||
  ...
  protected static void assertEquals(byte[] b1, byte[] b2)
  {
    ...
    for (int i = 0; i < b1.length; i++) {
      if (b1[i] != b2[i]) {
        assertFailed("Mismatch at " + i + ": " +
                     Convert.unsigned2hex(b1[i], 2) +
                     " != " + Convert.unsigned2hex(b2[i], 2));
      }
    }
  }
  ...
  protected static void assertEquals(double expected,
                                     double actual,
                                     double delta)
  {
    assertionCounter++;
  ...
  protected static void assertNotEquals(double expected,
                                        double actual,
                                        double delta)
  {
    assertionCounter++;
