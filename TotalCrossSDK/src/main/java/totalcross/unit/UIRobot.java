...
/**
 * This class permits the control of the User Interface, playing back events
 * recorded by the user.
 *
 * The robot is comprised of some dialogs that are invoked using a special key
 * defined by the application. The special key running in Java SE (eclipse,
 * netbeans) is control+1. The code below defines the Find special key to be
 * used at the device (you can choose any special key you want): <pre>
 ...
 *
 * When this key is pressed, it opens a window with three options: record,
 * playback and cancel.
 *
 * Clicking on the "record" button opens another window asking for the robot's
 * name. Type the name and press "Start record" to start recording. Do the
 * events smoothly and slowly. When done, press the special key again.
 *
 * Clicking in the playback button opens a screen with a list of recorded
 * robots.
 *
 * After selecting the robots, press one of these buttons:
 * <ul>
 * <li> Play selected: plays the robots in the same order you selected (the
 * order is recorded as you select). <li> Play all: plays all robots in the
 * list, in the order they appear. <li> Play random: randomizes the selected
 * robots order and reproduce that. A new window is opened asking you to enter
 * the number of times that exact sequence will be run. <li> Dump contents:
 * dumps the contents of the selected robots, so you can see the events that are
 * played back. <li> Delete selected: delete the selected robots.
 * </ul>
 * Robots are saved in a file with ".robot" extension at the application's path.
 * Since the robot saves absolute pen events at a specific time, they are not
 * portable among different device resolutions and/or devices with different
 * processors.
 *
 * You can start a specific robot passing its name as commandline to the
 * application. For example, in: <ul> <li> Android: adb shell am start -a
 * android.intent.action.MAIN -n totalcross.app.uigadgets/.UIGadgets -e cmdline
 * test1.robot <li> Windows 32: UIGadgets.exe /cmdline test1.robot <li> J2SE:
 * /cmdline flick.robot tc.samples.ui.gadgets.UIGadgets
 * </ul>
 *
 * Note that no other commandline parameters must be passed to the application;
 * the UIRobot expects that the only parameter will be the robot's name.
 *
 * In JavaSE (desktop), you can skip the robot start by launching the
 * application and pressing the SHIFT key while the "Starting ..." MessageBox is
 * displayed. This is useful if you want to record the robot again whithout
 * having to remove it from the commandline. The shift key also aborts the robot
 * during its execution.
 *
 * When the robot finishes, it takes a screenshot of the application. When it
 * plays back, it compares the screen with the saved one and sends one of these
 * two UIRobotEvent to the MainWindow: ROBOT_SUCCEED (if comparison succeeds) or
 * ROBOT_FAILED (if comparison fails).
 *
 * You can create a log file with the results by putting this in the onEvent of
 * your MainWindow class: <pre> if (event.type == UIRobotEvent.ROBOT_FAILED ||
 * event.type == UIRobotEvent.ROBOT_SUCCEED)
 *  {
 *     try
 *     {
 *        File f = new File(Settings.appPath+"/robot.log",File.CREATE_EMPTY);
 *        String s = event.type == UIRobotEvent.ROBOT_FAILED ? "FAILED" :
 * "SUCCEED"; f.writeBytes("Robot "+UIRobot.robotFileName+" "+s+". Running time:
 * "+UIRobot.totalTime); f.close();
 *     }
...
public class UIRobot
{
  ...
  /**
   * Set to true to abort the run of the current UIRobot. Useful if you set a
   * breakpoint at a code and want to abort the run.
   *
   * You can also abort the UIRobot by pressing the shift key during execution
   * and at the starting message box.
   */
  ...
  public UIRobot() throws Exception
  {
    ...
    switch (showMessage("Please select the action:",
                        new String[] { "Record", "Playback", "Cancel" },
                        0,
                        false)) {
      case 0:
        record();
  ...
  private void record() throws Exception
  {
    InputBox ib = new InputBox(TITLE,
                               "Please type the robot name:",
                               "",
                               new String[] { "Start record", "Cancel" });
    ...
    flog = new File(robotFileName = Settings.appPath + "/" + name,
                    File.CREATE_EMPTY);
  ...
  private void playback() throws Exception
  {
    ...
    ControlBox cb =
      new ControlBox(TITLE,
                     "Select the robots in the\nsequence you want to run.",
                     lb,
                     Control.FILL,
                     Control.FIT,
                     new String[] { "Play selected",
                                    "Play all",
                                    "Play random",
                                    "Dump contents",
                                    "Delete selected",
    ...
    switch (sel) {
      ...
      case 3: // dump
        ...
        break;
      case 4: // delete
        if (showMessage("Do you want to delete the selected robots?",
                        new String[] { "No", "Yes" },
                        0,
                        true) == 1) {
          for (int i = 0; i < n; i++) {
  ...
  {
    try {
      synchronized (tpLock) { return (PostThread)threadPool.pop(); }
  ...
  {
    synchronized (tpLock) { threadPool.push(t); }
  }
  ...
  private void play(final int[] items,
                    final int n,
                    final boolean dump,
                    final int repeat)
  {
    ...
    new Thread() {
      @Override public void run()
      {
        ...
        try {
          ...
          abort = false;
          for (int r = 1; !abort && r <= repeat && (dump || status == PLAYBACK);
               r++) {
            for (int i = 0; i < n && (dump || status == PLAYBACK); i++) {
              ...
              File f = new File(fileName.indexOf('/') <= 0
                                  ? Settings.appPath + "/" + fileName
                                  : fileName,
                                File.READ_WRITE);
              ...
              if (!dump && !abort) {
                ...
                try {
                  ...
                  showMessage("Finished " + fileName +
                              ...
                              false);
                  Vm.debug(
                    "One of the image files was not found during robot test comparison.");
                }
              } else if (dump) {
                lb.add("====================");
              }
  ...
  private void robotFailed(String fileName, String reason)
  {
    ...
    UIRobotEvent ev =
      new UIRobotEvent(UIRobotEvent.ROBOT_FAILED, fileName, reason);
  ...
  private void robotSucceed(String fileName)
  {
    ...
    UIRobotEvent ev =
      new UIRobotEvent(UIRobotEvent.ROBOT_SUCCEED, fileName, null);
    MainWindow.getMainWindow().postEvent(ev);
    if (!ev.consumed && listeners != null) {
      for (int i = listeners.size(); --i >= 0 && !ev.consumed;) {
  ...
  private void saveScreenShot(String fileName, boolean recording)
  ...
  private String getScreenShotName(String fileName, boolean recording)
  {
    return fileName.substring(0, fileName.length() - 6) +
      (recording ? "_rec" : "_play") + ".png";
  }

  private static byte[] cmpbuf1, cmpbuf2;

  private boolean compareScreenShots(String fileName)
  ...
  private String
  dumpEvent(int j, int type, int key, int x, int y, int mods, int delay)
  {
    switch (type) {
      case UIRobotEvent.ROBOT_EOF:
        return "ROBOT FINISHED"
          + " @ " + delay + "ms - " + formatTime(totalTime);
      case PenEvent.PEN_DOWN:
        return "PEN_DOWN  " + x + "," + y + " @ " + delay + "ms - " +
          formatTime(totalTime);
      case PenEvent.PEN_UP:
        return "PEN_UP    " + x + "," + y + " @ " + delay + "ms - " +
          formatTime(totalTime);
      case PenEvent.PEN_DRAG:
        return "PEN_DRAG  " + x + "," + y + " @ " + delay + "ms - " +
          formatTime(totalTime);
      case KeyEvent.KEY_PRESS:
        return "KEY_PRESS " + (key < 10 ? "  " : key < 100 ? " " : "") + key +
          " '" + (char)key + "'" + (mods == 0 ? " @ " : " - " + mods + " @ ") +
          delay + "ms - " + formatTime(totalTime);
      case KeyEvent.SPECIAL_KEY_PRESS:
        return "SPECIAL_KEY_PRESS " + key +
          (mods == 0 ? " @ " : " (" + mods + ") @ ") + delay + "ms - " +
          formatTime(totalTime);
    }
    return "";
  }
  ...
  private int showMessage(String msg, String[] btns, int delay, boolean error)
  {
    MessageBox mb = delay > 0 ? new MessageBox(TITLE, msg, null)
                              : btns == null ? new MessageBox(TITLE, msg)
                                             : new MessageBox(TITLE, msg, btns);
    mb.setBackForeColors(
      error ? Color.interpolate(Color.RED, Color.ORANGE) : Color.ORANGE, 1);
    if (delay == 0) {
      mb.popup();
    } else {
      mb.popupNonBlocking();
    ...
    if (list != null && list.length > 0) {
      ...
      for (int i = 0; i < list.length; i++) {
        if (list[i].endsWith(".robot")) {
          ...
          f.close();
          v.addElement(
            new StrTime(list[i] + " (" + new Date(t) + " " + t + ")", t));
        }
      }
      int n = v.size();
      if (n > 0) {
