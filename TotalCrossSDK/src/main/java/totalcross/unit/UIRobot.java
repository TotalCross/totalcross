...
public class UIRobot {
	...
	private void playback() throws Exception {
		...
		ControlBox cb = new ControlBox(TITLE, "Select the robots in the\nsequence you want to run.", lb,
									   Control.FILL,
									   Control.FIT,
									   new String[] { "Play selected", "Play all", "Play random", "Dump contents", "Delete selected", "Cancel" },
									   2);
		...
		switch (sel) {
			...
			case 3: // dump
				...
				break;
			case 4: // delete
				if (showMessage("Do you want to delete the selected robots?", new String[] { "No", "Yes" }, 0,
								true) == 1) {
					for (int i = 0; i < n; i++) {
						try {
						...
						for (int i = 0; i < n && (dump || status == PLAYBACK); i++) {
							...
							if (!dump && !abort) {
								...
								try {
									...
									showMessage("Finished " + fileName + ".\nScreenshot comparison " + (ok ? "succeed" : "FAILED"),
												null,
												ok ? 1500 : 2500, !ok);
	...
	private boolean compareScreenShots(String fileName) throws FileNotFoundException, IOException,
		ImageException {
	...
	public void onEvent(int type, int key, int x, int y, int modifiers) {
		...
		try {
			...
			if (Settings.onJavaSE) {
				Vm.debug((counter++) + " " + type + " " + key + " " + x + " " + y + " " + modifiers + " " +
						 elapsed);
			}
		} catch (Exception ee) {
