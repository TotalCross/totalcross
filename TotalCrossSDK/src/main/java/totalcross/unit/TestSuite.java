...
public abstract class TestSuite extends MainWindow {
	...
	private void updateMsg() {
		byte[] s =
			Settings.appSettingsBin; // guich@580_9: use appSettingsBin instead of appSecretKey because AllTests use the last
		int skip = 0;
		if (s != null) {
	...
	private static String getLineNumber(Throwable e) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (!l.startsWith("totalcross.") && !l.startsWith("litebase.") && (l.indexOf(' ') >= 0
					|| l.indexOf(':') >= 0)) {
				return l;
			}
		}
		...
		public void onEvent(Event e) {
			if (e.type == ControlEvent.PRESSED && e.target == btn) {
				...
				for (int i = 0; i < n; i++) {
					if (grid.isChecked(i)) {
						try {
							String num = grid.getCellText(i,
														  1); // get the real index, because the user may have sorted the grid
				...
				Settings.appSettingsBin = checked == n ? null :
										  c; // if everyone is checked, then we just store null - run everybody
				updateMsg();
				unpop();
			}
		}
