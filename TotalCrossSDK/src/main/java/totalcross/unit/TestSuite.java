...
/**
 JUnit implementation for TotalCross, to be used in the device (or in the
 desktop).
 * It simulates the same output that you get when using JUnit under Eclipse.
 * To use it, you must extend this class and add the test cases using the
 <code>addTestCase</code> method.
 * See an example in the Litebase SDK, in samples/sys/testcases folder.
 * <br><br>
 * You can also create a test suite without this user interface. Here's a
 sample:
 * <pre>
...
public abstract class TestSuite extends MainWindow
{
  ...
  public TestSuite(String title) { super(title, TAB_ONLY_BORDER); }

  /** Add a new testcase. */
  public void addTestCase(Class<?> c) { v.addElement(c); }
  ...
  {
    byte[] s =
      Settings.appSettingsBin; // guich@580_9: use appSettingsBin instead of
                               // appSecretKey because AllTests use the last
    int skip = 0;
    if (s != null) {
  ...
  protected void runTests()
  {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (!canRun(i)) {
        ...
        // show the test in the msg area. aligning to the right makes it always
        // show the class name, and maybe cut the package.
        if (hasUI) {
          lMsg.align = RIGHT;
        ...
        try { // guich@tc110_3: this strange code is to deal with Blackberry's
              // odd behaviour on "catch Throwable".
          try {
            updateMem();
            Object o = c.newInstance();
            ((TestCase)o)
              .run(); // guich@565_6: don't start the tests on the constructor
            error = false;
          } catch (Throwable e) { // was another exception but
      ...
      TestCase.learning = false; // reset after each testcase so that one do not
                                 // change other's behaviour
    }
  ...
  { // rnovais@570_77 : Now is not final anymore
    ...
    if (Settings.appSettings == null || Settings.appSettings.length() < 3 ||
        Settings.appSettings.equals("yes")) {
    ...
    MenuItem[] menu0 = {
      new MenuItem("File"),
      new MenuItem("Select tests"),
      miDump = new MenuItem("Dump to console", s.charAt(0) == 'y'),
      new MenuItem(),
      miLoop = new MenuItem("Loop " + loopCount + "x", s.charAt(1) == 'y'),
      new MenuItem("Select loop count"),
      new MenuItem(),
      new MenuItem("Exit"),
    };
    bar.prefix = miLoop.isChecked ? "0/" + loopCount + " - " : "";

    setMenuBar(mbar = new MenuBar(new MenuItem[][] {
                 menu0 })); // if a new vertical menu is added, AllTests must
                            // be changed!
    ...
    if ("/autorun".equals(getCommandLine())) {
      runTests(); // guich@570_90: now is safe to run from here, because initUI
                  // is already started from a thread.
    } else {
      btn.requestFocus();
    }
  }
  ...
  private static String getLineNumber(Throwable e)
  {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (!l.startsWith("totalcross.") && !l.startsWith("litebase.") &&
  ...
  {
    Settings.appSettings = (miDump.isChecked ? "y" : "n") +
                           (miLoop.isChecked ? "y" : "n") + loopCount;
    bar.prefix = miLoop.isChecked ? "0/" + loopCount + " - " : "";
  ...
  class TestChooser extends Window
  { // guich@567_8
    Grid grid;
    Button btn;
    ...
    @Override public void initUI()
    {
      ...
      int n = v.size();
      for (int i = 0; i < n; i++) {
        grid.add(new String[] { Convert.toString(i + 1),
                                prepareName(v.items[i].toString()) });
      }
    }
    ...
    @Override public void onEvent(Event e)
    {
      if (e.type == ControlEvent.PRESSED && e.target == btn) {
        ...
        for (int i = 0; i < n; i++) {
          if (grid.isChecked(i)) {
            try {
              String num =
                grid.getCellText(i,
                                 1); // get the real index, because the user
                                     // may have sorted the grid
        ...
        Settings.appSettingsBin =
          checked == n ? null : c; // if everyone is checked, then we just
                                   // store null - run everybody
        updateMsg();
        unpop();
      }
    }
