...
public class CompareBox extends Window {
	...
	public CompareBox(String title, int[] rgbs, int me, int ot, int i, int j, int n,
					  byte[] originalBytes,
					  ImageTester testImg) {
		super(title + " failed", ROUND_BORDER);
		setBackColor(0xF0F0F0);
		Label msg = new Label(Convert.unsigned2hex(me, 6) + " != " + Convert.unsigned2hex(ot,
							  6) + " at " + i + "," + j);
		int scale = (Settings.screenWidth - 20) / testImg.getWidth() / 2;
		Image imgo, imgi;
		try {
			imgo = reconstroyImage(testImg, rgbs, n, originalBytes, testImg.getWidth(),
								   testImg.getHeight()).scaledBy(scale,
										   scale);
