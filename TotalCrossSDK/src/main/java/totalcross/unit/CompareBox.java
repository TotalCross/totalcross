...
/**
 * This class is used to show two images, side by side, and show the first
...
public class CompareBox extends Window
{
  ...
  public CompareBox(String title,
                    int[] rgbs,
                    int me,
                    int ot,
                    int i,
                    ...
                    ImageTester testImg)
  {
    ...
    Label msg = new Label(Convert.unsigned2hex(me, 6) + " != " +
                          Convert.unsigned2hex(ot, 6) + " at " + i + "," + j);
    int scale = (Settings.screenWidth - 20) / testImg.getWidth() / 2;
    Image imgo, imgi;
    try {
      imgo = reconstroyImage(testImg,
                             rgbs,
                             n,
                             originalBytes,
                             testImg.getWidth(),
  ...
  private Image reconstroyImage(Image testImg,
                                int[] rgbs,
                                int n,
                                byte[] originalBytes,
                                int k,
                                int l) throws ImageException
  {
