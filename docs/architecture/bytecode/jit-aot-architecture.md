...
An additional macOS checkpoint is available through the off-by-default `TC_BUILD_IR_BENCHMARKS` option and `run-tcir-jit-benchmark` target. It requires Release plus SLJIT, warms each path five times, records 60 samples, rotates through all six backend orders ten times, excludes no outliers, and validates every batch against the `executeMethod` checksum. The paired JSON/CSV validator independently recomputes statistics and checks all 720 execution rows. At revision `e49acf6a5` on the recorded Apple M1 Pro/Low Power Mode host, SLJIT mean execution was 46.282% slower than `executeMethod` for `add`, 42.698% slower for `abs`, and 18.454% faster for `sumTo(65537)` (1.226x mean speedup). This is a revision-keyed standalone-API baseline that includes current per-invocation verification and scratch allocation, not production-dispatch or application performance evidence; the ExecPlan records the raw artifact paths, hashes, complete conditions, compilation statistics, and unfiltered scheduler variance.
