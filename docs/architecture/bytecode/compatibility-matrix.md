...
| macOS arm64 | current CI job | POC passed for three integer fixtures | forced POC passed | compile/link/run passed | 1,179 four-way comparisons and W^X mapping inspection passed; production dispatch remains |
| Android arm64-v8a | root `TotalCrossVM/CMakeLists.txt` with Android toolchain | compile path passed | compile path passed | generated object passed | NDK `28.2.13676358`, API 23, arm64-v8a; no device execution, runtime selection, or publication |
...
Seven focused CTest entries are available when `TC_BUILD_IR_TESTS=ON`, `TC_ENABLE_SLJIT_JIT=ON`, and `TC_ENABLE_C_AOT=ON`. `tcir-core` verifies exact TCode words/source lines generated by the production converter; repeated frontend goldens; twenty stable verifier/frontend diagnostics; all 160 registry dispositions; reference execution; bounded-loop handling; typed homes; and pre-execution rejection. `tcir-opcode-sources` runs `scripts/validate-tcir-opcodes.py` across the C, Java, runtime-dispatch, matrix, and bytecode-reference sources. `tcir-aot` covers deterministic order, semantic invalidation, exact lookup mismatch, and unsupported valid input. `tcir-aot-determinism` performs two clean file generations, while `tcir-aot-manifest` independently validates schema and generated artifacts. `tcir-differential` links the built runtime and generated-C artifacts and compares fresh `executeMethod`, TCIR, forced SLJIT, and AOT states over 1,179 deterministic inputs. `tcir-jit` retains W^X/lifecycle coverage. `TCIRConverterFixtureTest` independently regenerates slots through javac and production `J2TC`.

On macOS arm64, Debug and Release passed all 7/7 backend-enabled entries and the named fixtures therefore reach both `jit_target_passed` and `aot_target_passed`. AOT-only passed 6/6 without SLJIT. ASan passed the full differential graph, ASan/UBSan passed the generator test with unsupported Apple leak detection disabled, and focused Clang analysis reported no new-code bug. The broad VM scan produced 119 reports in pre-existing sources and is not used as focused AOT evidence. A default-off build exposed neither optional backend and passed the original 3/3 tests. Android arm64-v8a/API 23 and iPhoneOS arm64 compiled the same host-generated C object; neither result includes device execution or production publication.

The optional Release benchmark matrix was repeated after Milestone 6 at revision `1aa428b74012`. Its mandatory sequential profiles used 5/10/20 warmups and 60/200/1,000 samples, exercised every backend order with near-equal counts without outlier removal, validated all batches against `executeMethod`, and cross-checked 720/2,400/12,000 CSV rows. Under the recorded Apple M1 Pro/Low Power Mode conditions, every profile found SLJIT slower for short `add` and `abs` but 1.401x–1.440x faster for `sumTo(65537)`. This intentionally preserves the three-way historical regime and contains no AOT performance result; generated-C performance needs a separately named protocol. It does not promote any additional opcode/platform cell. Future checkpoints must repeat all three profiles. Device execution, exception handlers, object state, GC, helper calls, production selection, application benchmarks, Linux/Windows execution, and Apple distribution entitlements remain unvalidated.
