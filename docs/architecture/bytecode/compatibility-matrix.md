...
The exact per-opcode semantics are in `totalcross-bytecode-reference.md`. Every numeric TotalCross opcode 0–159 appears below; `OPCODE_LENGTH` is excluded because it is a count. The table remains a backend roadmap. Milestone 5 implemented forced SLJIT execution for the exact instruction combinations emitted by the converter's `add`, `abs`, and `sumTo` fixtures. Other cells labeled POC remain requirements rather than broad family-level execution claims. C AOT and production runtime dispatch remain unimplemented.
...
The first POC is intentionally method-atomic: if any operation in a method is outside the POC subset, that method is interpreted from its first instruction. Production-converter `add`, `abs`, and `sumTo` cover `BREAK`, `MOV_regI_s18`, `INC_regI`, integer add/subtract, `JGE` register/immediate branches, `JUMP_s24`, and `RETURN_regI`. Those exact fixture combinations reached `jit_target_passed` on macOS arm64 in Milestone 5, including all 1,179 Milestone 4 boundary, overflow, loop, and fixed-seed inputs. This does not promote every operation in the surrounding family rows: other registry entries marked supported have explicit lowering/validation code but still need focused converter fixtures and differential execution before a family can be called complete.
...
| macOS arm64 | current CI job | POC passed for three integer fixtures | forced POC passed | required | 1,179 three-way comparisons and W^X mapping inspection passed; Hardened Runtime/distribution gate remains |
| Android arm64-v8a | root `TotalCrossVM/CMakeLists.txt` with Android toolchain | compile path passed | compile path passed | required | NDK `28.2.13676358`, API 23, arm64-v8a passed; no device execution or runtime selection; legacy `Android.mk` is out of scope |
...
Four focused CTest entries are available when `TC_BUILD_IR_TESTS=ON` and `TC_ENABLE_SLJIT_JIT=ON`. `tcir-core` verifies exact TCode words/source lines generated by the production converter; repeated frontend goldens; twenty stable verifier/frontend diagnostics; all 160 registry dispositions; reference execution; bounded-loop handling; typed homes; and pre-execution rejection. `tcir-opcode-sources` runs `scripts/validate-tcir-opcodes.py` across the C, Java, runtime-dispatch, matrix, and bytecode-reference sources. `tcir-differential` links the built runtime artifact and compares fresh `executeMethod`, TCIR, and forced SLJIT states for the three converter fixtures over 1,179 deterministic inputs. `tcir-jit` covers W^X permissions, rejection, emission failure cleanup, repeated disposal, concurrent publication, interpreter progress, and shutdown. With the JIT option omitted, the original three tests remain and no JIT targets are generated. `TCIRConverterFixtureTest` independently regenerates the slots through javac and the production `J2TC` path.

On macOS arm64, normal and Release builds passed all 4/4 JIT-enabled CTest entries. The standalone backend passed ASan/UBSan, and Clang static analysis emitted no reports. Forced finalized-code mapping inspection required executable permission without writable permission. The named fixture combinations therefore reach `jit_target_passed` on this host. A default-off build produced no JIT targets and passed the original 3/3 tests. Android arm64-v8a cross-compilation passed with NDK `28.2.13676358`, target API 23, compile/target SDK 35, and W^X allocator definitions.

The optional Release benchmark checkpoint at revision `77d179edc` also passed on macOS arm64. Its mandatory sequential matrix used 5/10/20 warmups and 60/200/1,000 samples, exercised every backend order with near-equal occurrence counts without outlier removal, validated all batches against `executeMethod`, and cross-checked 720/2,400/12,000 raw CSV rows against JSON. Under its recorded Apple M1 Pro/Low Power Mode conditions, every profile found SLJIT mean execution slower for the short `add` and `abs` fixtures but 1.390x–1.438x faster for `sumTo(65537)`. This measures the current standalone APIs, including per-invocation verifier and scratch-allocation costs. It does not promote any additional opcode or platform cell to `jit_target_passed`. Future checkpoints must repeat all three profiles. Android device execution, exception handlers, object state, GC, helper calls, production runtime selection, application benchmarks, Windows/Linux execution, and Apple distribution entitlements remain unvalidated.
