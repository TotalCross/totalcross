...
The optional Release benchmark checkpoint at revision `e49acf6a5` also passed on macOS arm64. It used five warmups and 60 samples, exercised every backend order ten times without outlier removal, validated all batches against `executeMethod`, and cross-checked 720 raw CSV rows against JSON. Under its recorded Apple M1 Pro/Low Power Mode conditions, SLJIT mean execution was slower for the short `add` and `abs` fixtures but 1.226x faster for `sumTo(65537)`. This measures the current standalone APIs, including per-invocation verifier and scratch-allocation costs. It does not promote any additional opcode or platform cell to `jit_target_passed`. Android device execution, exception handlers, object state, GC, helper calls, production runtime selection, application benchmarks, Windows/Linux execution, and Apple distribution entitlements remain unvalidated.
