...
The exact per-opcode semantics are in `totalcross-bytecode-reference.md`. Every numeric TotalCross opcode 0–159 appears below; `OPCODE_LENGTH` is excluded because it is a count. The table remains a backend roadmap: Milestone 3 implemented the frontend for the registry's static-integer POC subset, but the combined frontend/interpreter cells labeled POC are not execution claims. The TCIR interpreter, SLJIT, C AOT, and runtime dispatch remain unimplemented.
...
The first POC is intentionally method-atomic: if any operation in a method is outside the POC subset, that method is interpreted from its first instruction. Milestone 3 frontend evidence is deliberately narrower than each family row: production-converter `add`, `abs`, and `sumTo` cover `BREAK`, `MOV_regI_s18`, `INC_regI`, integer add/subtract, `JGE` register/immediate branches, `JUMP_s24`, and `RETURN_regI`. Other registry entries marked supported have explicit lowering/validation code but still need focused converter fixtures and, beginning in Milestone 4, differential execution before any family can be called complete.
...
Two focused CTest entries remain behind `TC_BUILD_IR_TESTS`. In Milestone 3, `tcir-core` verifies exact TCode words/source lines generated by the production converter for `add`, `abs`, and `sumTo`; compares repeated frontend runs with canonical CFG/TCIR goldens; checks twenty stable verifier/frontend diagnostics including malformed continuations and interpreter fallback; and validates that all 160 registry rows have a disposition. `tcir-opcode-sources` runs `scripts/validate-tcir-opcodes.py`, which cross-checks C definitions, Java numeric constants, runtime dispatch, this matrix, and the bytecode reference. `TCIRConverterFixtureTest` regenerates the inputs through javac and `J2TC` and fails on drift. The observed macOS arm64 Debug and ASan/UBSan builds passed 2/2 CTest entries, and the focused SDK test passed. This establishes `focused_test_run_passed` for the named translation fixtures only; no opcode has yet reached `differential_ir_passed`.
