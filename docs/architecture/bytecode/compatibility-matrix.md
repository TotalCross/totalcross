...
The exact per-opcode semantics are in `totalcross-bytecode-reference.md`. Every numeric TotalCross opcode 0–159 appears below; `OPCODE_LENGTH` is excluded because it is a count. The table remains a backend roadmap. Milestone 4 implemented the TCIR reference interpreter and established differential execution for the exact instruction combinations emitted by the converter's `add`, `abs`, and `sumTo` fixtures. Other cells labeled POC remain requirements rather than broad family-level execution claims. SLJIT, C AOT, and runtime dispatch remain unimplemented.
...
The first POC is intentionally method-atomic: if any operation in a method is outside the POC subset, that method is interpreted from its first instruction. Production-converter `add`, `abs`, and `sumTo` cover `BREAK`, `MOV_regI_s18`, `INC_regI`, integer add/subtract, `JGE` register/immediate branches, `JUMP_s24`, and `RETURN_regI`. Those exact fixture combinations reached `differential_ir_passed` on macOS arm64 in Milestone 4, including boundary, overflow, loop, and fixed-seed inputs. This does not promote every operation in the surrounding family rows: other registry entries marked supported have explicit lowering/validation code but still need focused converter fixtures and differential execution before a family can be called complete.
...
| macOS arm64 | current CI job | POC passed for three integer fixtures | opt-in only | required | Milestone 4 host evidence; Hardened Runtime/JIT entitlement gate remains |
...
Three focused CTest entries remain behind `TC_BUILD_IR_TESTS`. `tcir-core` verifies exact TCode words/source lines generated by the production converter; repeated frontend goldens; twenty stable verifier/frontend diagnostics; all 160 registry dispositions; reference execution; bounded-loop handling; typed homes; and pre-execution rejection. `tcir-opcode-sources` runs `scripts/validate-tcir-opcodes.py` across the C, Java, runtime-dispatch, matrix, and bytecode-reference sources. `tcir-differential` links the built runtime artifact and compares fresh `executeMethod` and TCIR states for the three converter fixtures over 1,179 deterministic inputs. `TCIRConverterFixtureTest` independently regenerates the slots through javac and the production `J2TC` path.

On macOS arm64, normal and Release builds passed all 3/3 CTest entries. ASan/UBSan passed the standalone TCIR core and source validator, and ASan passed all differential comparisons against the runtime. Apple leak detection was disabled because the platform sanitizer reports it unsupported. UBSan signed-overflow instrumentation was not applied to the differential runtime because the existing `executeMethod` arithmetic uses signed C operations for TotalCross/Java wrap behavior; the TCIR interpreter itself uses defined modular arithmetic and passed UBSan. Strict C99 compilation and Clang static analysis passed all seven TCIR sources, and the focused SDK fixture test passed. The named fixture combinations therefore reach `differential_ir_passed`; exception handlers, object state, GC, helper calls, native backends, runtime selection, and non-host platforms do not.
