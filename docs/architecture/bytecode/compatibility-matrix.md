...
| Shifts | `70 SHR_regI_regI_s12`; `71 SHR_regI_regI_regI`; `72 SHR_regL_regL_regL`; `73 SHL_regI_regI_s12`; `74 SHL_regI_regI_regI`; `75 SHL_regL_regL_regL`; `76 USHR_regI_regI_s12`; `77 USHR_regI_regI_regI`; `78 USHR_regL_regL_regL` | Current | Integer POC; long planned | Integer POC; long fallback | Integer POC; long planned | i32 Java shift-count masking/sign passed four-way differential execution; long remains. |
| Bitwise | `79 AND_regI_regI_s12`; `81 AND_regI_regI_regI`; `82 AND_regL_regL_regL`; `83 OR_regI_regI_s12`; `84 OR_regI_regI_regI`; `85 OR_regL_regL_regL`; `86 XOR_regI_regI_s12`; `87 XOR_regI_regI_regI`; `88 XOR_regL_regL_regL` | Current | Integer POC; long planned | Integer POC; long fallback | Integer POC; long planned | immediate/register i32 forms passed four-way differential execution; long remains. |
...
| Conversions | `124 CONV_regI_regL`; `125 CONV_regI_regD`; `126 CONV_regIb_regI`; `127 CONV_regIc_regI`; `128 CONV_regIs_regI`; `129 CONV_regL_regI`; `130 CONV_regL_regD`; `131 CONV_regD_regI`; `132 CONV_regD_regL` | Current | i8/i16 POC; rest planned | i8/i16 POC; rest fallback | i8/i16 POC; rest planned | byte/short sign extension and char zero extension passed four-way differential execution; long/double conversions remain. |
...
The first POC is intentionally method-atomic: if any operation in a method is outside the selected backend subset, that method is rejected before execution. Production-converter `add`, `abs`, `sumTo`, and `pureI32` cover `BREAK`, `MOV_regI_s18`, `INC_regI`, integer add/subtract, i32 shifts/bitwise/narrowing conversions, `JGE` register/immediate branches, `JUMP_s24`, and `RETURN_regI`. Those exact fixture combinations reached both `jit_target_passed` and `aot_target_passed` on macOS arm64, including all 1,701 boundary, overflow, loop, and fixed-seed comparisons. Checked i32 division/remainder has focused TCIR/reference-runtime tests but is intentionally not eligible for SLJIT or generated C yet. This does not promote every operation in the surrounding family rows: other registry entries marked supported have explicit lowering/validation code but still need focused converter fixtures and differential execution before a family can be called complete.
...
| macOS arm64 | current CI job | POC/runtime dispatch passed for four pure integer fixtures plus checked div/rem | forced and runtime-dispatch pure-i32 POC passed | compile/link/runtime-dispatch pure-i32 POC passed | 1,701 four-way pure comparisons plus checked-div/rem IR tests, mixed-mode ABI tests, W^X, lifecycle, sanitizers, and benchmark checkpoints passed; broader helper/GC semantics remain |
