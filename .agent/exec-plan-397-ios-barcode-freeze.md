...
Current state: milestones 1 through 4 are complete. `TCBarcodeSession` now owns a serial capture queue; camera authorization continues asynchronously, configuration validates input/output and supported metadata types, and `startRunning` runs off the main queue. The overlay and completion remain on the main queue, and every permission/configuration error uses the centralized finalizer. The user explicitly waived physical-device reproduction and thread capture, so this plan does not claim device validation. The standalone Objective-C syntax check is blocked before the modified source by the absent `YTPlayerView.h` CocoaPods dependency; no Xcode build was run.
