...
At the Milestone 7 checkpoint, eligible methods contained no managed-reference, helper, allocation, or exception-handler TCIR operation. Its runtime ABI tests could prove pending-exception status transfer, frame/usage ownership, concurrency, and shutdown, but could not honestly prove live-root spilling, arena-base reload after growth, forced GC at helper boundaries, handler selection, or stack traces. The later reference and static-call slices add spill/reload and native-bridge evidence, while forced/moving GC, arbitrary-native suspension, handlers, and stack traces remain open.
...
Milestone 8 is now partially implemented in twelve deliberately separated commits. The first seven commits cover the pure and checked i32/i64/f64 slices described above. Commit `a57eace1a` then promotes pure reference moves, null constants, identity comparisons, branches, and returns across the frontend, verifier, TCIR interpreter, SLJIT, generated C, production fixtures, and four-way differential corpus. Commit `0feb15e29` follows separately with the effectful `TEST_regO` null check, exact `may_throw | may_gc` effects, precise TC PC, live-reference GC-home metadata/materialization, and conditional `NullPointerException` delivery. Commit `891eb84bc` adds pure keyed/default `SWITCH` edges to every pure engine and converter-backed coverage. Commits `4115f2f95` and `155c74ebd` add typed static `CALL_normal` operations, compiled invoke support, pre-bound call-site preflight, and the real `executeMethod` native/interpreted/compiled bridge. Effectful division and null checking remain intentionally ineligible for SLJIT and generated C, so backend choice occurs before execution and cannot leave partial effects; `THROW` remains fallback until stack-trace allocation and handler selection can be lowered together.

The expanded partial Milestone 8 host configuration passed all 8 focused CTest entries on macOS arm64 in Release and ASan builds. Its fourteen production-converter fixtures produced 6,398 successful legacy-VM/TCIR/SLJIT/AOT comparisons, including null, aliasing, distinct opaque reference identities, sparse/default switch paths, and static-call arguments/results. The focused SDK fixture test and UBSan runtime test passed; a default-off build passed 7/7 and exposed neither a runtime target nor `tcirRuntime*` VM symbols. Android NDK `28.2.13676358`, arm64-v8a/API 23, compiled the expanded `tcir`, `tcir_jit`, `tcir_aot`, `tcir_runtime`, and conditional `tcvm.c` object. Runtime tests execute the converter-produced call through IR, SLJIT, and portable C into a real native `NMParams` target, while absent or signature-incompatible bindings fall back before any TCIR invocation. This proves static pre-bound call transport, not lazy symbol resolution, virtual/interface dispatch, forced/moving GC inside arbitrary native code, arena growth, or handler behavior. Floating-to-integer conversion, double remainder, and `THROW` retain explicit fallback; allocation, fields, arrays, handlers, virtual calls, monitors, and other legacy/special cases remain open, so Milestone 8 is not complete.
...
Partial status through commit `155c74ebd`: selected pure i32, i64, normalized-f64, reference, switch, and pre-bound static-call operations execute through TCIR, portable-C AOT, and 64-bit SLJIT. Fourteen production-converter fixtures pass 6,398 legacy-VM/TCIR/SLJIT/AOT comparisons, including exact floating bits, opaque reference null/alias/distinct identities, keyed/default switch paths, and typed static-call arguments/results. Checked integer/double division and `TEST_regO` execute through TCIR with exact `may_throw | may_gc` contracts and conditional VM exception hooks; SLJIT and generated C reject those effectful functions before emission, so `auto` selects TCIR and forced compiled policies preserve whole-method fallback. Static calls carry the full conservative effect set, spill/reload declared reference homes, preflight all caller bindings, and reuse `executeMethod` for interpreted, compiled, or native callees. Reference/call safepoint tests do not yet prove forced/moving GC, arena reload, arbitrary native suspension behavior, or lazy resolution. `THROW` remains explicit fallback pending the combined stack-trace/helper/handler slice. Remaining conversions, allocation, fields, arrays, handlers, virtual calls, monitors, and legacy/special cases still gate Milestone 8 completion.
...
The implemented runtime ABI is version 4. `TCCompiledFrame` appends a `TCCompiledRuntime` pointer with opaque runtime context, caller method key, a typed direct-dispatch thunk, and a typed call-site invoke thunk; compiled scratch, edge, and call-argument storage use runtime values that can carry i32, i64/f64 bits, or opaque references. `tcir_runtime.h` exposes explicit bounded registration, backend policy, dispatch, diagnostics/statistics, IR output, shutdown, and reset. Generated entries never receive private `Context` or `Method` layouts; pre-bound static calls cross the invoke thunk and re-enter `executeMethod`. A future helper table for lazy resolution, allocation, fields, arrays, monitors, and other effectful operations must extend this versioned contract deliberately rather than overloading the call thunk.
