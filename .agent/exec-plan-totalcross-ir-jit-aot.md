...
- [x] (2026-07-18T17:08:16Z) Added converter-backed static `CALL_normal` lowering in `4115f2f95`: typed method symbols, packed register/immediate arguments, runtime ABI version 4, full conservative call effects, GC-home spill/reload contracts, and callback-based TCIR/SLJIT/portable-C execution now cover a fourteenth fixture and 6,398 four-way comparisons.
- [x] (2026-07-18T17:08:16Z) Connected lowered calls to the real VM in `155c74ebd`: the caller's pre-bound `boundNormal` target is checked for static kind, arity, parameter types, and return type before backend entry, then invoked through the existing `executeMethod` bridge so interpreted, compiled, and native callees preserve one call path.
- [x] (2026-07-18T17:08:16Z) Validated the call slice on macOS arm64 with 8/8 Release and ASan focused tests, focused UBSan runtime execution, the SDK converter fixture test, and a dispatch-disabled 7/7 build with no runtime target/symbols; Android NDK r28c/API 23 compiled `tcir`, `tcir_jit`, `tcir_aot`, `tcir_runtime`, and the conditional `tcvm.c` object for arm64-v8a.
- [x] (2026-07-18T17:08:16Z) Repeated and independently validated the mandatory macOS 60/200/1,000-sample profiles at revision `155c74ebd40d`, preserving six artifacts under `build/m8-call-benchmark/results`; the unchanged workloads remain longitudinal `add`/`abs`/`sumTo` evidence and make no static-call performance claim.
- [x] (2026-07-18T17:49:39Z) Added converter-backed `NEWOBJ` semantics in `e7ea5cb14`: class-symbol validation, full conservative allocation effects, explicit live-reference homes, and a publication-before-return callback contract now cover a fifteenth fixture without changing production dispatch.
- [x] (2026-07-18T17:49:39Z) Connected object allocation to TCIR, SLJIT, portable C, and the conditional VM adapter in `051800dcd`, advancing the experimental runtime ABI to version 5; the adapter uses `createObjectWithoutCallingDefaultConstructor`, publishes the locked object in its destination `regO` home, then unlocks it, and method-atomic class preflight retains fallback before entry.
- [x] (2026-07-18T17:49:39Z) Validated the allocation slice on macOS arm64 with 8/8 Release and ASan focused tests, focused UBSan runtime execution, the SDK converter fixture test, 6,398 unchanged legacy four-way comparisons plus 16 TCIR/SLJIT/AOT allocation-contract comparisons, and a dispatch-disabled 7/7 build with no runtime target/symbols; Android NDK r28c/API 23 compiled all four TCIR libraries and the conditional `tcvm.c` object for arm64-v8a.
- [x] (2026-07-18T17:49:39Z) Repeated and independently validated the mandatory macOS 60/200/1,000-sample profiles at revision `051800dcd6e6`, preserving six artifacts under `build/m8-allocation-benchmark/results`; the unchanged workloads remain longitudinal arithmetic evidence and make no allocation-performance claim.
...
At the Milestone 7 checkpoint, eligible methods contained no managed-reference, helper, allocation, or exception-handler TCIR operation. Its runtime ABI tests could prove pending-exception status transfer, frame/usage ownership, concurrency, and shutdown, but could not honestly prove live-root spilling, arena-base reload after growth, forced GC at helper boundaries, handler selection, or stack traces. The later reference, static-call, and allocation slices add spill/reload, native-bridge, and destination-publication evidence, while real object-memory-manager forced/moving GC, arbitrary-native suspension, handlers, and stack traces remain open.
...
`CALL_normal` has a variable continuation width determined by the resolved callee signature, while each packed byte is interpreted against the callee parameter type and may name a typed register or a small immediate. The static-call slice therefore cannot infer types from the caller instruction alone: the bounded view supplies owner/name/descriptor, parameter types, return type, and call kind. Once lowered, an arbitrary method call conservatively carries heap read/write, throw, GC, lock, symbol-resolution, and unknown-call effects even when the fixture's target happens to be a pure integer native function.

Whole-method fallback for calls requires more than backend preflight. A generated method could otherwise execute earlier effects before discovering an unresolved symbol at the call site. The runtime now scans every `method.call` before backend entry and accepts only compatible pre-bound static `boundNormal` targets; the invoke thunk rechecks the stable binding and routes it through `executeMethod`. This deliberately postpones lazy linking inside compiled execution until the linker/resolver itself has a complete effect, exception, and GC-root contract.
...
- Decision: Advance `TC_RUNTIME_ABI_VERSION` to 4 with a caller method key and typed call-site invoke thunk shared by TCIR, SLJIT, and portable C. Rationale: compiled code must identify the caller constant pool and transfer typed arguments/results without importing `Context`, `Method`, or native-bridge layouts into either backend. Date: 2026-07-18.
- Decision: Initially lower only static `CALL_normal` sites whose `boundNormal` target is already resolved and signature-compatible, assigning the full conservative method-call effect set and preflighting all sites before backend entry. Rationale: reusing `executeMethod` preserves interpreted/compiled/native callee behavior, while pre-binding avoids partial execution followed by fallback and defers lazy resolution until its effects can be modeled honestly. Date: 2026-07-18.
- Decision: Advance `TC_RUNTIME_ABI_VERSION` to 5 with an allocation descriptor and thunk that receive the caller constant-pool index, all live reference homes, the destination home, and the exact TC PC. Rationale: allocation can load and initialize a class, allocate or collect, grow arenas, lock, throw, and write heap state, so the helper needs the full conservative effect set and must reload reference homes after return. Date: 2026-07-18.
- Decision: Require the allocation helper to publish a successful object in its destination `regO` home before returning to generated code, while the production adapter publishes the still-locked object before `setObjectLock(..., UNLOCKED)`. Rationale: the destination home is the collector-visible root during helper return/unlock, and accepting an un-published result would create a collection window that opaque standalone tests could not detect. Date: 2026-07-18.
- Decision: Treat the standalone opaque-token allocation comparisons as ABI/publication/status evidence, not as proof of the real object-memory manager, class loader, class initializer, or moving/forced GC. Rationale: the isolated differential harness does not initialize a real TCZ/class-loader/OMM environment; the production adapter is compiled and preflighted there, while full runtime allocation stress remains an explicit later gate. Date: 2026-07-18.
...
At the Milestone 7 boundary, `TC_RUNTIME_ABI_VERSION` was 2. `TCCompiledFrame` carried a `TCCompiledRuntime` containing an opaque context and call thunk. A generated entry passed stable method identity, typed values, and optional receiver to that thunk; it converted values to the existing internal `TValue` form and re-entered `executeMethod`, preserving its native bridge and recursive compiled selection. Focused tests covered top-level and nested interpreter-to-compiled dispatch, generated-entry forwarding to unregistered interpreted, registered generated-C, and native targets, primitive results, frame/usage restoration, forced fallback, explicit IR output, AOT-first `auto`, eight concurrent JIT callers with exactly one compile, pending-exception status and TC PC, and shutdown during a deliberately blocked active AOT invocation.
...
Milestone 7 acceptance is intentionally bounded to its static-i32 eligible subset. Build-time isolation, method-atomic registration/fallback, mixed-mode call mechanics, primitive result transfer, exception-status handoff, frame/usage ownership, concurrency, lifecycle, diagnostics, and host/Android compilation have direct evidence. No Milestone 7 eligible operation could allocate, carry a managed reference, invoke a `may_gc` helper, enter a real handler, or grow an arena. Reference results, forced GC, arena-base reload, stack traces/handler selection, production class-loader registration, dead-strip-safe AOT publication, and device/platform execution therefore remained explicit Milestone 8/9 gates rather than being inferred from those ABI tests.

Milestone 8 is now partially implemented in fourteen deliberately separated commits. The first seven commits cover the pure and checked i32/i64/f64 slices described above. Commit `a57eace1a` then promotes pure reference moves, null constants, identity comparisons, branches, and returns across the frontend, verifier, TCIR interpreter, SLJIT, generated C, production fixtures, and four-way differential corpus. Commit `0feb15e29` follows separately with the effectful `TEST_regO` null check, exact `may_throw | may_gc` effects, precise TC PC, live-reference GC-home metadata/materialization, and conditional `NullPointerException` delivery. Commit `891eb84bc` adds pure keyed/default `SWITCH` edges to every pure engine and converter-backed coverage. Commits `4115f2f95` and `155c74ebd` add typed static `CALL_normal` operations, compiled invoke support, pre-bound call-site preflight, and the real `executeMethod` native/interpreted/compiled bridge. Commits `e7ea5cb14` and `051800dcd` add `NEWOBJ`, a publication-before-unlock allocation contract, compiled allocation thunks, class preflight, and the real conditional VM allocation adapter. Effectful division and null checking remain intentionally ineligible for SLJIT and generated C, so backend choice occurs before execution and cannot leave partial effects; `THROW` remains fallback until stack-trace allocation and handler selection can be lowered together.

The expanded partial Milestone 8 host configuration passed all 8 focused CTest entries on macOS arm64 in Release and ASan builds. Its fifteen production-converter fixtures retain 6,398 successful legacy-VM/TCIR/SLJIT/AOT comparisons and add 16 TCIR/SLJIT/AOT allocation-contract comparisons for publication, thrown status, allocation failure, and live-root spill/reload. The focused SDK fixture test and UBSan runtime test passed; a default-off build passed 7/7 and exposed neither a runtime target nor `tcirRuntime*` VM symbols. Android NDK `28.2.13676358`, arm64-v8a/API 23, compiled the expanded `tcir`, `tcir_jit`, `tcir_aot`, `tcir_runtime`, and conditional `tcvm.c` object. Runtime tests execute the converter-produced call through IR, SLJIT, and portable C into a real native `NMParams` target, while allocation tests enforce publication-before-return and class-symbol preflight. This proves static pre-bound call transport and the allocation ABI contract, not lazy symbol resolution, virtual/interface dispatch, real TCZ/class initialization, forced/moving GC in the object-memory manager, arena growth, or handler behavior. Floating-to-integer conversion, double remainder, and `THROW` retain explicit fallback; fields, arrays, handlers, virtual calls, monitors, and other legacy/special cases remain open, so Milestone 8 is not complete.
...
The unchanged benchmark was repeated again after the static-call slice at revision `155c74ebd40d`. The fresh Release configuration used AC power, battery at 100%, Low Power Mode on, no intentional background workload, observed configure-time load averages 4.16/3.99/4.49, all six execution-order permutations, no exclusions, and the mandatory 5/10/20 warmups. The independent validator accepted all 720/2,400/12,000 CSV rows. Mean results were:

| Samples | Workload | `executeMethod` ns | TCIR ns | SLJIT ns | SLJIT vs `executeMethod` | Compile us |
| ---: | --- | ---: | ---: | ---: | ---: | ---: |
| 60 | `add` | 54.136 | 232.829 | 79.773 | 0.679x | 3.834 |
| 60 | `abs` | 58.668 | 485.522 | 85.860 | 0.683x | 4.630 |
| 60 | `sumTo(65537)` | 773,228.994 | 5,666,729.038 | 638,617.361 | 1.211x | 5.752 |
| 200 | `add` | 54.096 | 236.451 | 79.988 | 0.676x | 3.756 |
| 200 | `abs` | 58.661 | 480.240 | 85.834 | 0.683x | 5.030 |
| 200 | `sumTo(65537)` | 776,798.483 | 5,669,533.398 | 642,269.994 | 1.209x | 5.473 |
| 1,000 | `add` | 53.186 | 219.227 | 77.179 | 0.689x | 4.151 |
| 1,000 | `abs` | 58.230 | 454.176 | 83.359 | 0.699x | 4.603 |
| 1,000 | `sumTo(65537)` | 761,563.439 | 5,600,501.225 | 629,127.857 | 1.211x | 6.371 |

Code sizes remained 84, 256, and 424 bytes. `sumTo(65537)` retains a 1.209x-1.211x mean advantage through SLJIT; short `add` and `abs` remain slower. Because the harness has no call workload, these files are continuity evidence rather than a static-call performance measurement:

- 60 samples: `build/m8-call-benchmark/results/tcir-jit-benchmark-155c74ebd40d-s60.json` (11,970 bytes, SHA-256 `16c0e75fa3f158508194d1c7d2501297397ed3b43a4fc525462f01b2ae9297d7`) and `.csv` (77,176 bytes, SHA-256 `939dba9916c59a9c7c88069f2068ffd18f23fd242cc5edd399d4d53e1437039a`).
- 200 samples: `build/m8-call-benchmark/results/tcir-jit-benchmark-155c74ebd40d-s200.json` (25,015 bytes, SHA-256 `4870bf408a63173bb5c8e2a02143cd31dbe142afbce2ed289d72ce71d3de116e`) and `.csv` (258,435 bytes, SHA-256 `9ac725972987c75ee2d8e87417b8245ac4cebc7c5f9fdff066f6067ee7edf14a`).
- 1,000 samples: `build/m8-call-benchmark/results/tcir-jit-benchmark-155c74ebd40d-s1000.json` (99,440 bytes, SHA-256 `ddd223bdcb7bec7a88967744b3e744d330a27db3b7585977d9e19b59df21636e`) and `.csv` (1,296,862 bytes, SHA-256 `f3a8e2db82a9fa5c163b01409ba50c017e09b445df582fcd000d1f9287f78041`).

The unchanged benchmark was repeated after the allocation slice at revision `051800dcd6e6`. The fresh Release configuration used AC power, battery at 100%, Low Power Mode on, no intentional background workload, observed configure-time load averages 3.67/3.29/3.00, all six execution-order permutations, no exclusions, and the mandatory 5/10/20 warmups. The independent validator accepted all 720/2,400/12,000 CSV rows. Mean results were:

| Samples | Workload | `executeMethod` ns | TCIR ns | SLJIT ns | SLJIT vs `executeMethod` | Compile us |
| ---: | --- | ---: | ---: | ---: | ---: | ---: |
| 60 | `add` | 53.259 | 232.792 | 80.627 | 0.661x | 7.153 |
| 60 | `abs` | 57.833 | 444.919 | 86.144 | 0.671x | 5.104 |
| 60 | `sumTo(65537)` | 761,055.662 | 5,498,258.222 | 631,892.730 | 1.204x | 7.471 |
| 200 | `add` | 53.326 | 233.888 | 109.903 | 0.485x | 3.446 |
| 200 | `abs` | 57.949 | 446.535 | 86.098 | 0.673x | 4.813 |
| 200 | `sumTo(65537)` | 761,250.489 | 5,537,271.244 | 632,271.672 | 1.204x | 6.955 |
| 1,000 | `add` | 53.259 | 229.191 | 79.523 | 0.670x | 5.462 |
| 1,000 | `abs` | 58.256 | 442.053 | 85.153 | 0.684x | 4.575 |
| 1,000 | `sumTo(65537)` | 777,200.264 | 5,534,884.971 | 645,178.595 | 1.205x | 5.287 |

Code sizes remained 84, 256, and 424 bytes. `sumTo(65537)` retains a 1.204x-1.205x mean advantage through SLJIT; short `add` and `abs` remain slower, and the unfiltered 200-sample `add` distribution is preserved. Because the harness has no allocation workload and does not initialize the object-memory manager, these files are continuity evidence rather than an allocation-performance or GC measurement:

- 60 samples: `build/m8-allocation-benchmark/results/tcir-jit-benchmark-051800dcd6e6-s60.json` (11,942 bytes, SHA-256 `f638d6d83b671fedcd73c9308cb3351b391b5d040e575796bb717583a8183f09`) and `.csv` (77,179 bytes, SHA-256 `020918c7609adcc70837c22643ef14e8b20eae1c9f25a4348335c08309734e32`).
- 200 samples: `build/m8-allocation-benchmark/results/tcir-jit-benchmark-051800dcd6e6-s200.json` (24,981 bytes, SHA-256 `2f88b58400d9acca58e3b1f09b3ee2d647d87e1c42ea380c4f661f5eb91f15ee`) and `.csv` (258,434 bytes, SHA-256 `54e3e77bbf632da79c6c7ccad0f0cbf175e5412c01eaeb4fe53ca6c719f11947`).
- 1,000 samples: `build/m8-allocation-benchmark/results/tcir-jit-benchmark-051800dcd6e6-s1000.json` (99,405 bytes, SHA-256 `6fc4897e48202a78d3bd784862b174f773bb7f7b8cbd398abdef23a0558aa891`) and `.csv` (1,296,851 bytes, SHA-256 `7d58c3bc0ec82b086d1f0379cf2cc2e4f4dfc85105bbd67c55fa5f11fde036f5`).
...
This is an interim factual report through the controlled mixed-i32/i64/f64/reference, switch, pre-bound static-call, and object-allocation runtime-integration proof of concept and partial Milestone 8 coverage. It is not the final report required by `.agent/PLANS.md`; Milestone 9 must reconcile it with broader semantic/platform coverage and final performance/release evidence.

### Editorial Summary

The project now has a source-grounded map from Java class parsing through TotalCross bytecode execution and GC, an implemented backend-neutral TCIR version 1 contract, a bounded frontend for the current numeric/reference/control-flow/static-call/allocation POC subset, a verified reference interpreter, an opt-in SLJIT baseline, an opt-in deterministic portable-C generator, and an independently default-off runtime adapter. Developers can take fifteen canonical converter fixtures through the applicable legacy, TCIR, native SLJIT, and compiled generated-C paths using one versioned frame ABI, then explicitly register compatible methods for top-level/nested VM dispatch. Pre-bound static calls re-enter the same facade, while allocation crosses a publication-checked runtime thunk whose VM adapter publishes the object before unlock. Disabling `TC_ENABLE_COMPILED_DISPATCH` removes the hooks and runtime library/symbols, so the default production execution path remains unchanged.

### Original Plan versus Actual Outcome

Milestone 1 produced only the ExecPlan and architecture documents. Milestone 2 then implemented the planned C contract, verifier, printer, registry, fixtures, and focused tests. The optional text parser was omitted because stable one-way golden output met acceptance without creating another input format. Milestone 3 implemented the partial decoder/frontend through an explicit bounded method view and generated its integration fixtures through the production converter. Milestone 4 implemented reference execution and an isolated real-`executeMethod` oracle. Milestone 5 implemented the pinned, optional SLJIT backend and standalone artifact cache, including Android cross-compilation but not Android execution. Milestone 6 implemented deterministic generated C, an exact-identity registry and manifest, a native POC host tool, and four-way functional comparison. Milestone 7 implemented explicit runtime side-table registration, policy/lifecycle/observability, conditional `executeMethod` dispatch, and mixed-mode call mechanics. Milestone 8 has so far expanded numeric operations, pure reference identity/transport, switch control flow, pre-bound static calls, and object allocation, plus interpreter-only checked arithmetic and null testing. General TCZ/class-loader registration, real object-memory-manager/forced-GC stress, fields, arrays, handlers, lazy/virtual calls, production AOT publication, and an AOT performance regime remain later work.

### What Changed

The initial eight documents under `docs/architecture/bytecode` and this ExecPlan remain the design record. Milestone 2 added the owned TCIR implementation, verifier, printer, registry, focused tests, CMake option, and opcode source validator. Milestone 3 added the bounded frontend, converter-backed fixtures/goldens, and SDK regeneration check. Milestone 4 added `tcir_interp.h`, `tcir_interp.c`, reference execution/rejection tests in `tcir_tests.c`, and the real-runtime differential harness. Milestone 5 added the `tcir_jit` library, backend/cache/memory interfaces, forced JIT test, depot-tools pin, and Android integration. Milestone 6 extracted `tcir_compiled.h`, added `tcir_aot`, `tcaot`, generated C/header/manifest output, deterministic and manifest validators, exact registry tests, four-way differential execution, and CMake fixture targets. Milestone 7 added `tcir_runtime.h`/`.c`, runtime ABI version 2, a call thunk, the conditional VM hooks, `tcir-runtime` tests, backend/off isolation checks, and a benchmark-driven atomic fast path. The current Milestone 8 slices add opaque reference values and results across all pure engines, an interpreter safepoint/null-check path with GC-home materialization and conditional VM exception delivery, explicit switch terminators lowered by both compiled backends, and runtime ABI version 5 with typed pre-bound calls plus publication-checked object allocation through every execution engine.

### Decisions and Trade-offs

TCIR version 1 uses opaque owned C structures, simplified-SSA values and block arguments, typed homes, explicit source metadata/effects, stable diagnostics, and a deterministic one-way printer. Builders copy temporary arrays but the verifier—not the builder—is the canonical malformed-graph rejection boundary. Reference, SLJIT, AOT, and runtime entries share `tcir_compiled.h` without importing private `Context` layout and use explicit result/rejection status. Generated C uses a portable state machine and stable lexical order instead of compiler extensions; semantic FNV-1a hashes provide deterministic identity, not cryptographic trust. Runtime state remains outside `TMethod`; generated calls and allocation helpers cross opaque version-5 thunks; backend build and VM dispatch flags remain separate. Static call targets and allocation classes must pass method-atomic preflight before backend entry. All engines materialize declared `regO` homes around conservative helper safepoints, and successful allocation is accepted only after the helper publishes the destination home. Future work still requires a real class/TCZ adapter, object-memory-manager forced-GC and arena-reload coverage, lazy resolution, virtual calls, further effectful helpers, and production AOT publication.

### Unexpected Problems and Discoveries

The existing converter IR is opcode-shaped, float identity is normalized, the class record is unversioned inside TCZ, and one Java opcode-name table omits values 158/159. Root CMake resolves the full native dependency graph before focused targets run, and its legacy PUBLIC source list propagates all VM sources to normal target consumers; the differential harness therefore links the built artifact directly. Milestone 3 found that `TMethod` discards its serialized code count, call width depends on resolved signature metadata, and converter control flow uses `BREAK` padding. Milestone 4 exposed the legacy oracle's signed-C overflow under UBSan. Milestone 5 confirmed depot-tools SLJIT/W^X and Android API 23 compilation. Milestone 6 showed that negative backend fixtures must first be valid TCIR, that cross builds need C generated by a native host tool, and that broad VM static analysis surfaces legacy findings which must be separated from focused new-code analysis. Milestone 7 showed that functional correctness does not expose disabled-policy transition cost: the first full matrix found a mutex tripling short `executeMethod` time, and the required repetition proved the atomic bypass fix. It also separated successful Android compilation of changed objects from a later legacy `fadeScreen` full-build failure. The current Milestone 8 slices showed that opaque reference identity can reach every pure backend without object-layout knowledge, while the first reference safepoint must remain TCIR-only until the compiled helper ABI has forced-GC evidence. They also showed that switch payload slots need an explicit instruction-start map, that `THROW` cannot be separated from stack-trace allocation and handler selection without losing observable VM behavior, and that method-atomic call fallback requires resolving every target before execution rather than lazily at the generated call site. Allocation further showed that the helper result alone is not a safe root: the still-locked object must be published in its destination `regO` home before unlock, and an opaque-token harness cannot substitute for a fully initialized class loader and object-memory manager.

### Validation and Measurable Results

The current partial Milestone 8 checkpoint passes 8/8 focused host CTest entries in Release and ASan configurations, 6,398 retained legacy four-way comparisons from the prior fourteen converter fixtures, 16 allocation-contract comparisons for the fifteenth fixture across TCIR/SLJIT/generated C, the production SDK converter fixture test, focused UBSan runtime execution, and the default-off 7/7 VM build with no runtime target/symbols. Android NDK r28c/API 23 cross-compiles all four TCIR libraries plus the conditional VM object for arm64-v8a. The mandatory 60/200/1,000 matrix validates all 720/2,400/12,000 rows with no samples excluded; at revision `051800dcd6e6`, the unchanged short workloads remain slower through SLJIT, while `sumTo(65537)` measures a 1.204x-1.205x mean advantage. Generated-C AOT, reference, switch, static-call, and allocation performance were intentionally not measured. Pure reference transport/identity, switch control flow, null-check spill, pre-bound static calls, and allocation publication/status contracts now have evidence; lazy/virtual calls, real class initialization, object-memory-manager moving/forced GC, arena growth, fields, arrays, handlers, automatic production registration, Linux/GCC, Windows/MSVC, full iOS linkage, Android full link/device execution, and product workloads remain unvalidated.

### Useful Evidence and Examples

The bytecode reference enumerates all 160 opcodes. `tcir_converter_fixtures.h` records exact converter words/lines; the reference, call, and `frontend-newObject.tcir` goldens show identity, merges, results, static-call effects, and the full allocation contract; and `tcir_differential_tests.c` contains the real-runtime legacy corpus plus isolated allocation-contract comparisons. `tcir_tests.c` contains GC-home/TC-PC/exception/publication evidence, while `tcir_runtime_tests.c` proves actual lowered calls to a native target and method-atomic call/class preflight fallback. Commits `e7ea5cb14` and `051800dcd` are the allocation semantic/backend and conditional VM-adapter slices. The generated artifacts and all repeated benchmark families/hashes above identify the accepted checkpoints; the compatibility matrix distinguishes exact fixture evidence from family-level roadmap labels.

### Limitations, Remaining Work, and Open Questions

No TCIR text parser, general TCZ-to-AOT input adapter, automatic runtime `Method` adapter, static publication model, or non-host platform execution test exists yet. The frontend/interpreter/SLJIT/AOT paths cover selected i32/i64/f64/reference operations, pre-bound static calls, and object allocation; fields, arrays, monitors, handlers, virtual/interface calls, and legacy/special methods remain fallback, as do checked arithmetic/null checks in compiled backends. The benchmark measures the historical standalone interpreter/JIT APIs with compiled dispatch present but policy off, includes current per-invocation verification/allocation overhead, ran with Low Power Mode enabled, covers only three synthetic integer fixtures, and does not measure AOT, reference, call, switch, or object-allocation operations. Handler behavior, real heap mutation/class initialization, object-memory-manager moving/forced GC, arena growth, thread suspension, unloading/redefinition, volatile/atomic semantics, dead stripping/signing, and real-world legacy `JUMP_regI` require later evidence.
...
Acceptance was observed for the controlled static-i32 integration slice in commits `35b14388b` and `3cdfd6974`. The separate default-off build option removes both VM hooks and the runtime target/symbols; the enabled runtime explicitly registers only fully verified compatible methods and retains whole-method interpreter fallback. ABI tests exercise all four dispatch directions with primitive results, pending-exception TC-PC handoff, frame/usage restoration, lazy-JIT concurrency, and active-call shutdown. The mandatory benchmark found and then verified the fix for a disabled-policy mutex regression. Because the Milestone 7 eligible subset had no managed-reference, real call, helper, allocation, or handler operation, the reference-result, forced-GC, arena-growth, handler/stack-trace, and helper-effect clauses remained required acceptance for the corresponding Milestone 8 slices rather than being falsely claimed there.
...
Partial status through commit `051800dcd`: selected pure i32, i64, normalized-f64, reference, switch, pre-bound static-call, and object-allocation operations execute through TCIR, portable-C AOT, and 64-bit SLJIT. Fifteen production-converter fixtures retain 6,398 legacy-VM/TCIR/SLJIT/AOT comparisons and add 16 TCIR/SLJIT/AOT allocation-contract comparisons. Checked integer/double division and `TEST_regO` execute through TCIR with exact `may_throw | may_gc` contracts and conditional VM exception hooks; SLJIT and generated C reject those effectful functions before emission, so `auto` selects TCIR and forced compiled policies preserve whole-method fallback. Static calls carry the full conservative effect set, preflight all caller bindings, and reuse `executeMethod`; allocation carries the same full conservative boundary, preflights its class symbol, spills/reloads declared reference homes, and requires destination publication before helper return/unlock. These allocation tests prove the version-5 ABI and status/publication contract but do not initialize the real TCZ/class-loader/object-memory-manager path or prove forced/moving GC, arena reload, or class-initializer behavior. `THROW` remains explicit fallback pending the combined stack-trace/helper/handler slice. Remaining conversions, fields, arrays, handlers, virtual calls, monitors, and legacy/special cases still gate Milestone 8 completion.
...
The implemented runtime ABI is version 5. `TCCompiledFrame` appends a `TCCompiledRuntime` pointer with opaque runtime context, caller method key, a typed direct-dispatch thunk, typed call-site invoke thunk, and typed allocation thunk; compiled scratch, edge, call-argument, and allocation-root storage use runtime values that can carry i32, i64/f64 bits, or opaque references. `tcir_runtime.h` exposes explicit bounded registration, backend policy, dispatch, diagnostics/statistics, IR output, shutdown, and reset. Generated entries never receive private `Context` or `Method` layouts; pre-bound static calls cross the invoke thunk and re-enter `executeMethod`, while allocation passes constant-pool/class identity, live reference homes, destination home, and TC PC to the helper and verifies publication before continuing. Future helpers for lazy resolution, fields, arrays, monitors, and other effectful operations must extend this versioned contract deliberately rather than overloading existing thunks.
...
Which runtime helpers can directly or transitively allocate or trigger GC on every platform, including native methods, class initialization, resolution, monitors, string operations, and exception creation? The static-call slice conservatively marks every call with the full effect set and proves the existing native bridge with pre-bound targets, but it has not forced GC, arena relocation, or thread suspension inside arbitrary native code. Each later helper-bearing slice still requires a specific effect review and stress test.
...
Revision note (2026-07-18, partial Milestone 8 static-call slice): committed typed static `CALL_normal` semantics, full conservative effects, runtime ABI version 4, and TCIR/SLJIT/portable-C callback lowering in `4115f2f95`, then connected pre-bound compatible call sites to the real `executeMethod` interpreted/compiled/native bridge with method-atomic preflight in `155c74ebd`. Fourteen converter fixtures pass 6,398 four-way comparisons. macOS Release and ASan passed 8/8, focused UBSan runtime and the SDK converter test passed, and dispatch-disabled Release passed 7/7 with no runtime target/symbols. Android NDK r28c/API 23 compiled all four TCIR libraries plus the conditional VM object. The mandatory 60/200/1,000 profiles were repeated from fresh `build/m8-call-benchmark` artifacts and independently validated. Lazy resolution, virtual calls, arbitrary-native forced GC/suspension, allocation, fields, arrays, handlers, monitors, and remaining opcode families keep Milestone 8 open.

Revision note (2026-07-18, partial Milestone 8 allocation slice): committed converter-backed `NEWOBJ`, full conservative effects, class symbols, root-home publication contracts, and reference-interpreter coverage in `e7ea5cb14`, then advanced the compiled runtime ABI to version 5 and connected TCIR/SLJIT/portable-C allocation thunks to the conditional VM adapter in `051800dcd`. Fifteen converter fixtures retain 6,398 legacy four-way comparisons and add 16 allocation-contract comparisons. macOS Release and ASan passed 8/8, focused UBSan runtime and the SDK converter test passed, and dispatch-disabled Release passed 7/7 with no runtime target/symbols. Android NDK r28c/API 23 compiled all four TCIR libraries plus the conditional VM object. The mandatory 60/200/1,000 profiles were repeated from fresh `build/m8-allocation-benchmark` artifacts and independently validated. The isolated harness proves publication/status/root transport but does not initialize the real TCZ/class loader/object-memory manager or force moving GC; fields, arrays, handlers, virtual calls, monitors, and remaining opcode families keep Milestone 8 open.
