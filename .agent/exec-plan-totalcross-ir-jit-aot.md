...
Milestone 8 is now partially implemented in seven deliberately separated commits. Commit `da38b7278` adds pure i32 shifts, bitwise operations, and byte/char/short conversions across the frontend, verifier, TCIR interpreter, SLJIT, generated C, converter fixture, and four-way differential corpus. Commit `a7af114e8` adds checked i32 division/remainder with precise TC PC, Java `INT_MIN / -1` behavior, zero-divisor exception outcome, and a conditional `ArithmeticException` runtime hook. Commit `e305e0d89` expands the same pure pipeline to long constants, moves, arithmetic, shifts, bitwise operations, comparisons, branches, returns, and int/long conversions, advancing the mixed-value compiled ABI to version 3. Commit `fedb32c8a` adds checked long division/remainder with the same exact effect and exception contract. Commit `eade23ce0` adds bit-preserving normalized-float/double values and pure operations across all four execution paths, `cc78ad5aa` adds stable int/long-to-double conversions, and `64a8bec8c` adds checked double division. Effectful division remains intentionally ineligible for SLJIT and generated C, so backend choice occurs before execution and cannot leave partial effects.

The expanded partial Milestone 8 host configuration passed all 8 focused CTest entries on macOS arm64; its nine pure converter fixtures produced 4,311 successful legacy-VM/TCIR/SLJIT/AOT comparisons from curated IEEE-754/integer boundary cases and fixed seeds `0x4d595df4`/`0x8a5cd789`/`0x31f2a8c7`/`0xc42b91e5`. The focused SDK fixture test passed, and ASan/UBSan passed `tcir-core` with Apple leak detection disabled. Android NDK `28.2.13676358`, arm64-v8a/API 23, compiled the expanded `tcir`, `tcir_jit`, `tcir_aot`, and `tcir_runtime` targets. Floating-to-integer conversion and double remainder now have explicit retained-fallback rationale; calls, references, allocation, fields, arrays, handlers, monitors, and other legacy/special cases remain open, so Milestone 8 is not complete and no new performance checkpoint is yet claimed.
...
Partial status as of commits `da38b7278`, `a7af114e8`, `e305e0d89`, and `fedb32c8a`: immediate/register i32 shifts and bitwise operations plus byte/char/short conversions execute on all three TCIR engines. Pure long constants, moves, arithmetic, shifts, bitwise operations, comparisons, branches/returns, and int/long conversions do the same on 64-bit SLJIT targets, with method-atomic SLJIT fallback on 32-bit targets. The five production-converter fixtures pass 2,223 legacy-VM/TCIR/SLJIT/AOT comparisons. Checked i32 and i64 division/remainder execute through TCIR with an exact `may_throw | may_gc` contract and conditional `ArithmeticException` hook. SLJIT and generated C reject those effectful pairs before execution, so `auto` selects TCIR and forced compiled policies preserve whole-method fallback. Double/normalized-float semantics, remaining conversions, calls, managed references, allocation, fields, arrays, handlers, monitors, and legacy/special cases still gate Milestone 8 completion.
