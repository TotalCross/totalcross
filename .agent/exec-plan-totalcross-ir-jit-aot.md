...
- [x] (2026-07-17T22:20:54Z) Added the off-by-default `TC_BUILD_IR_BENCHMARKS` harness and revision-keyed JSON/CSV validator in `e49acf6a5`, with five warmups, 60 samples, all six execution-order permutations, oracle checksums, raw samples, descriptive statistics, and host/build metadata.
- [x] (2026-07-17T22:20:54Z) Captured and validated the post-Milestone-5 macOS arm64 performance checkpoint under `build/m5-sljit-benchmark/results`, preserving raw artifacts and recording their hashes and interpretation below for future comparisons.
...
Short standalone methods expose transition costs more strongly than generated-code throughput. Both `tcirInterpretFunction` and `tcirJitInvoke` currently allocate invocation scratch, and the TCIR interpreter also verifies and preflights each call; `add` and `abs` therefore remained slower than `executeMethod` in the first structured checkpoint. `sumTo(65537)` amortized those costs and made the generated loop observable. The first 30-sample attempt also exposed scheduler variance in millisecond-scale batches, so the accepted protocol increased to 60 samples and larger batches rather than filtering outliers.
...
The post-Milestone-5 measurement checkpoint added a reproducible standalone comparison without changing that acceptance boundary. `tcir_jit_benchmark` reuses prepared contexts, frames, functions, and compiled artifacts; every warmup and measured batch must reproduce the `executeMethod` checksum. Five warmups precede 60 recorded samples. The six possible backend orders each occur ten times, no samples are removed, and `validate-tcir-jit-benchmark.py` recomputes statistics and cross-checks all 720 CSV rows against JSON. Compilation is separately sampled 60 times after five warmups, with the interpreter baseline explicitly marked not applicable.

The accepted artifact was measured at revision `e49acf6a5049f77a22eb1997a02fb5fce98461e4` on an Apple M1 Pro with 10 logical CPUs and 16 GiB RAM, Darwin 25.5.0 arm64, AC power at 100% battery with Low Power Mode on, no affinity, and no intentional background workload. It used AppleClang 21.0.0.21000101, Ninja, Release `-O3 -DNDEBUG -Wall -Wextra -Werror`, and the ARM-64 SLJIT backend. Mean execution nanoseconds per invocation were: `add` 50.973 (`executeMethod`), 239.775 (TCIR), and 74.565 (SLJIT); `abs` 59.489, 475.345, and 84.889; `sumTo(65537)` 1,017,824.022, 5,509,191.776, and 829,996.093. Relative to `executeMethod`, SLJIT was 46.282% slower for `add`, 42.698% slower for `abs`, and 18.454% faster for `sumTo`, a 1.226x mean speedup only for that loop workload. Mean JIT compilation times were 7.272, 4.551, and 5.246 microseconds, with code sizes 84, 256, and 424 bytes. The raw distributions contain scheduler outliers, which remain included in these means and in the independently recomputable artifacts. These are checkpoint measurements of the current standalone APIs under Low Power Mode, not production-VM or general performance claims.

The raw baseline is `build/m5-sljit-benchmark/results/tcir-jit-benchmark-e49acf6a5049.json` (11,977 bytes, SHA-256 `5a8287812a43941fe9fbc9be86b69b9b2568e8883b519326696357aaa77082ec`) and the paired CSV is `build/m5-sljit-benchmark/results/tcir-jit-benchmark-e49acf6a5049.csv` (77,181 bytes, SHA-256 `e8c75a92ee796ecb46ed349f454a6dd0e52efa022ba26a37e3ab38b65b69358c`). JSON preserves host/build/protocol metadata, all raw compile/execution samples, means, medians, sample standard deviations, ranges, deltas, percentages, and ratios; CSV preserves the paired raw observations and execution position. Future comparisons must retain this scope or label any changed dispatcher, scratch, power, or workload regime explicitly.
...
An additional accepted measurement checkpoint now follows Milestone 5 without expanding backend semantics. Commit `e49acf6a5` adds the optional `TC_BUILD_IR_BENCHMARKS`/`run-tcir-jit-benchmark` path and independent JSON/CSV validation. The accepted macOS artifact uses five warmups, 60 samples, every execution-order permutation ten times, interpreter-oracle validation for every batch, and zero exclusions. Its detailed conditions, raw artifact hashes, and results are recorded in Outcomes. Future milestones should rerun the same target into their revision-keyed filenames before changing the benchmark scope; when scope changes, retain both result families and document the new regime instead of overwriting or directly combining unlike measurements.
...
Configure and run the optional standalone performance checkpoint only in an optimized build. Supply honest power/load/dirty-path notes; CMake replaces semicolons in those values with commas before invoking the harness. Result filenames include the first twelve characters of the configured repository revision, so later revisions coexist in the same results directory:
    ...
    python3 scripts/validate-tcir-jit-benchmark.py \
      --json build/m5-sljit-benchmark/results/tcir-jit-benchmark-<revision>.json \
      --csv build/m5-sljit-benchmark/results/tcir-jit-benchmark-<revision>.csv
...
Revision note (2026-07-17, post-Milestone-5 benchmark): added the optional revision-keyed benchmark/validator in `e49acf6a5`; captured five warmups and 60 raw samples across all six backend permutations on macOS arm64; recorded compile latency, code size, execution statistics, host/build/power/dirty metadata, artifact hashes, and the standalone-API scope. The accepted checkpoint found SLJIT slower for short `add`/`abs` calls but 1.226x faster on mean for `sumTo(65537)` under the recorded Low Power Mode conditions. Milestones 6–9 remain unstarted.
