...
Milestone 8 is now partially implemented in two deliberately separate slices. Commit `da38b7278` adds pure i32 shifts, bitwise operations, and byte/char/short conversions across the frontend, verifier, TCIR interpreter, SLJIT, generated C, converter fixture, and four-way differential corpus. Commit `a7af114e8` adds checked i32 division/remainder with precise TC PC, Java `INT_MIN / -1` behavior, zero-divisor exception outcome, and a conditional `ArithmeticException` runtime hook. Division/remainder remains intentionally ineligible for SLJIT and generated C, so backend choice occurs before execution and cannot leave partial effects.

The partial Milestone 8 host configuration passed all 8 focused CTest entries on macOS arm64; its four pure converter fixtures produced 1,701 successful legacy-VM/TCIR/SLJIT/AOT comparisons. The focused SDK fixture regeneration/test passed, and ASan/UBSan passed `tcir-core` with Apple leak detection disabled. Android NDK `28.2.13676358`, arm64-v8a/API 23, compiled the expanded `tcir`, `tcir_jit`, `tcir_aot`, and `tcir_runtime` targets. Long, double, remaining conversions, calls, references, allocation, fields, arrays, handlers, monitors, and legacy/special cases remain open, so Milestone 8 is not complete and no new performance checkpoint is yet claimed.
...
Partial status as of commits `da38b7278` and `a7af114e8`: immediate/register i32 shifts and bitwise operations plus byte/char/short conversions execute on all three TCIR engines, with a production-converter fixture and 1,701 four-way comparisons. Checked i32 division/remainder executes through TCIR with an exact `may_throw | may_gc` contract and conditional `ArithmeticException` hook. SLJIT and generated C reject that effectful pair before execution, so `auto` selects TCIR and forced compiled policies preserve whole-method fallback. This is the first slice only; long, double, remaining conversions, calls, managed references, allocation, fields, arrays, handlers, monitors, and legacy/special cases still gate Milestone 8 completion.
