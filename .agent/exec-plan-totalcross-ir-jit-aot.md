...
Milestone 8 is now partially implemented in four deliberately separated commits. Commit `da38b7278` adds pure i32 shifts, bitwise operations, and byte/char/short conversions across the frontend, verifier, TCIR interpreter, SLJIT, generated C, converter fixture, and four-way differential corpus. Commit `a7af114e8` adds checked i32 division/remainder with precise TC PC, Java `INT_MIN / -1` behavior, zero-divisor exception outcome, and a conditional `ArithmeticException` runtime hook. Commit `e305e0d89` expands the same pure pipeline to long constants, moves, arithmetic, shifts, bitwise operations, comparisons, branches, returns, and int/long conversions, advancing the mixed-value compiled ABI to version 3. Commit `fedb32c8a` adds checked long division/remainder with the same exact effect and exception contract. Division/remainder remains intentionally ineligible for SLJIT and generated C, so backend choice occurs before execution and cannot leave partial effects.

The expanded partial Milestone 8 host configuration passed all 8 focused CTest entries on macOS arm64; its five pure converter fixtures produced 2,223 successful legacy-VM/TCIR/SLJIT/AOT comparisons from curated boundary cases and fixed seeds `0x4d595df4`/`0x8a5cd789`. The focused SDK fixture test passed, and ASan/UBSan passed `tcir-core` plus the opcode-source check with Apple leak detection disabled. Android NDK `28.2.13676358`, arm64-v8a/API 23, compiled the expanded `tcir`, `tcir_jit`, `tcir_aot`, and `tcir_runtime` targets. Double/normalized-float semantics, remaining numeric conversions, calls, references, allocation, fields, arrays, handlers, monitors, and legacy/special cases remain open, so Milestone 8 is not complete and no new performance checkpoint is yet claimed.
...
This is an interim factual report through the completed controlled static-i32 runtime-integration proof of concept. It is not the final report required by `.agent/PLANS.md`; Milestone 9 must reconcile it with broader semantic/platform coverage and final performance/release evidence.
