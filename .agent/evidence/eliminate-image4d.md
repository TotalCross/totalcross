...
- 2026-08-27 — final revision `c8f5d5f99`; focused converter/image tests and `dist -x test` passed; `git diff --check` and scoped copyright validation passed; provenance audit from `df4903620` to `c8f5d5f99` for `Image.java` reported no material or interval-specific review lineage; source search found no remaining `Image4D` references in production/source trees and generic `4D` conversion remains.
