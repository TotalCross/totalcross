...
- [x] (2026-07-25T22:00:00-03:00) Repeated the cached SDK 7.2.2 test on the connected physical `2312DRA50G` (Android 13, 1220x2712); `mInputShown=true`, `shiftY=1800`/`shiftH=200`, and the lower Edit was visible above the keyboard. The device's `screenrecord` encoder failed with `err=-38`, so `screencap` is the visual evidence.
- [x] (2026-07-25T22:05:00-03:00) Generated the current-branch Standard Release AAB, deployed the same two-Edit app, and repeated the test on the physical `2312DRA50G`; `mInputShown=true`, `shiftY=1800`/`shiftH=200`, and the lower Edit was visible above the keyboard.
- [x] (2026-07-25T22:15:00-03:00) Repeated the current-branch test on the newly started `emulator-5554` AVD (Android 17, 1080x2424); `mInputShown=true`, `shiftY=1806`/`shiftH=176`, and the lower Edit remained covered by the keyboard in both screencap and screenrecord evidence.
- [x] (2026-07-25T22:20:00-03:00) Closed the M6 follow-up as a documented emulator limitation: Android 14/17 AVDs failed the lower-Edit visibility criterion, while the same SDK 7.2.2 and current-branch APKs passed on the physical Android 13 device. No runtime fix was included in this plan.
- [x] M6 follow-up and ExecPlan: finalize with the emulator limitation recorded; native `Window.shiftY` investigation is deferred outside this plan.
...
- Decision: finalize the plan with the AVD behavior recorded as an environment-specific limitation. Rationale: Android 14/17 AVDs failed with the base, checkpoint, SDK 7.2.2, and current-branch APKs, while the same SDK 7.2.2 and current-branch APKs passed on the physical Android 13 device; no uniform product regression was established. Date: 2026-07-25.
...
Current state: the ExecPlan is finalized. The base Milestone 6 validation is complete, and the two-Edit follow-up is closed as an environment-specific emulator limitation rather than a proven plan regression. The final Standard Release AAB was rebuilt with `:app:bundleStandardRelease`, copied to `TotalCrossSDK/dist/vm/android/TotalCross.aab`, deployed through `tc.Deploy`, and installed as `totalcross.apprtst`. On the Android 14 and Android 17 AVD sessions, the lower Edit remained covered despite computed `Window.shiftY` with `mInputShown=true`; the same SDK 7.2.2 and current-branch APKs showed the lower Edit above the keyboard on the physical Android 13 device. The final idle/load rotation evidence, lifecycle checks, navigation checks, and stale-package removal remain as recorded above. The compact trace remains disabled by default; no runtime fix or temporary diagnostic source was added to the repository. Native `Window.shiftY` investigation, broader devices, iOS, Windows, and full distribution validation are deferred outside this finalized plan.

### Editorial Summary

The Android rotation path now drops exact duplicates and obsolete queued work, avoids redundant closed-keyboard SIP events, performs one useful repaint, and retains the existing EGL/Skia ownership because runtime evidence did not show duplicate recreation. Final 20-rotation idle/load runs completed without crash or ANR on the available AVD. The two-Edit test exposed an AVD-specific rendering limitation: Android 14/17 AVDs kept the lower Edit covered, while the same SDK and branch passed on a physical Android 13 device.
...
The Standard Release AAB build, SDK-template deploy, APK install, focused unit tests, 20 idle rotations, 20 load rotations, lifecycle checks, keyboard checks, two navigation modes, and the two-Edit shift test were exercised. Load p95 improved from `424.110 ms` to `211.629 ms` against the preserved M1 baseline. The two-Edit visibility criterion failed on Android 14/17 AVDs and passed on the physical Android 13 device.

### Useful Evidence and Examples

Final traces: `/tmp/tc-rotation-m6-final-idle-summary.json` and `/tmp/tc-rotation-m6-final-load-fixed-summary.json`. Visual evidence: `/tmp/tc-rotation-m6-final-load-fixed.mp4`. Lifecycle evidence: `/tmp/tc-rotation-m6-lifecycle-explicit.log`. Shift evidence: `/tmp/tc-current-branch-avd-shift-ime-frame.png`, `/tmp/tc-current-branch-avd-shift-ime.mp4`, and `/tmp/tc-current-branch-shift-ime-retry.png`.

### Limitations, Remaining Work, and Open Questions

The AVD limitation was observed on Android 14 and Android 17 emulator sessions, while one physical Android 13 device passed with the same SDK and branch. The physical device's `screenrecord` encoder was unavailable (`err=-38`), so its result relies on `screencap`. No Samsung/other physical devices, iOS, Windows, or full distribution validation was performed. The lower-Edit native OpenGL path remains a separate investigation if emulator support is required.
...
Milestone 6 follow-up note (2026-07-25): added a temporary diagnostic app with top and bottom `Edit` controls, executed the keyboard-visible test, and observed `Window.shiftY=1804`/`shiftH=184` while the lower control remained covered in OpenGL output on the Android 14 emulator. A regression across the pre-plan base (`b7c25d776`), M4 SIP (`c72ca44a9`), and M4 repaint (`5a773cebc`) AABs reproduced the emulator result; the current-branch AAB also failed on the Android 17 AVD, while both the cached SDK 7.2.2 AAB and the current-branch AAB passed on the connected physical Android 13 device with `shiftY=1800`/`shiftH=200` and the lower Edit visible above the keyboard. The emulator limitation is recorded and the ExecPlan is finalized; no runtime fix was attempted.
...
2026-07-25: The requested two-Edit keyboard-shift follow-up was executed after the M6 closure. The Android 14 and newly started Android 17 AVDs kept the lower Edit covered despite computed VM shift state, while both the cached SDK 7.2.2 and current-branch AABs shifted the Edit above the keyboard on the physical Android 13 device. The evidence points to a device/rendering-path difference rather than a uniform regression in the plan's changes; no runtime code was changed.
2026-07-25: The ExecPlan was finalized after recording the Android 14/17 AVD limitation and the passing physical Android 13 result. No later milestone or runtime investigation was executed.
