...
Milestone 6 follow-up note (2026-07-25): added a temporary diagnostic app with top and bottom `Edit` controls, executed the keyboard-visible test, and observed `Window.shiftY=1804`/`shiftH=184` while the lower control remained covered in OpenGL output on the Android 14 emulator. A regression across the pre-plan base (`b7c25d776`), M4 SIP (`c72ca44a9`), and M4 repaint (`5a773cebc`) AABs reproduced the same emulator result, while the cached SDK 7.2.2 AAB passed on the connected physical Android 13 device with `shiftY=1800`/`shiftH=200` and the lower Edit visible above the keyboard. No runtime fix was attempted; this follow-up remains a device/rendering-path investigation, blocked pending authorization.
...
2026-07-25: The requested two-Edit keyboard-shift follow-up was executed after the M6 closure. The Android 14 emulator kept the lower Edit covered despite computed VM shift state, while the same cached SDK 7.2.2 test on the physical Android 13 device shifted the Edit above the keyboard. The evidence now points to a device/rendering-path difference rather than a uniform regression in the plan's changes; no runtime code was changed.
