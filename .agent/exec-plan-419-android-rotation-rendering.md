...
Current state: Milestone 3 is complete. Accepted resize requests now carry an immutable generation snapshot, and the event-thread runnable exits before JNI when its generation is obsolete or its captured `Surface` is invalid. Sentinel calls `-999`, `-998`, and `-997` remain outside the resize coordinator. `RotationRequestCoordinatorTest` has 11 focused cases, including rapid portrait-landscape-portrait sequencing, old-task invalidation after a new surface, final-request preservation, and sentinel isolation; `:app:testStandardDebugUnitTest` passed. The updated Standard Release AAB was built with `:app:bundleStandardRelease`, copied to `TotalCrossSDK/dist/vm/android/TotalCross.aab`, deployed with `tc.Deploy`, and installed as `totalcross.apprtst`. The rapid-load evidence `/tmp/tc-rotation-m3-rapid.log` reports 3 stale tasks and 1/1 final rotation callback completed, with no native-init stages for the stale generations. Closure evidence on Android 14 emulator `sdk_gphone64_arm64` (`emulator-5554`): idle `/tmp/tc-rotation-m3-idle.log` had 10 accepted callbacks, 6 completed, 7 stale tasks, p50/p95 752.037/2034.659 ms; event-thread load `/tmp/tc-rotation-m3-load.log` had 10 accepted callbacks, 8 completed, 5 stale tasks, p50/p95 1057.959/2564.655 ms. In both runs the newest stable generation reached `SCREEN_CHANGED`, first swap, and final dimensions `1080x2220`; no crash or ANR was observed and the test activity remained foreground. The M1 screen-recording artifacts remain the visual evidence for the unchanged diagnostic application. The next action belongs to Milestone 4 and is intentionally not executed in this turn.
