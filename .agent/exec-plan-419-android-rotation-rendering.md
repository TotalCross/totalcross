...
- [ ] (blocked: 2026-07-25T22:30:00-03:00) Capture the physical/emulator baseline. The available emulator rejects the debug APK with `INSTALL_FAILED_UID_CHANGED` because the installed `totalcross.android` package uses a different shared UID; installing the instrumented APK would require uninstalling that package and deleting its data.
- [ ] Milestone 1: complete the reproducible idle and event-thread-load baseline after the device-install blocker is resolved.
- [ ] Milestone 2: create focused coordinator tests and remove only exact duplicate resize requests.
...
- Observation: the available emulator cannot install the instrumented debug APK over its existing `totalcross.android` package. Evidence: `adb install -r TotalCrossVM/android/app/build/outputs/apk/standard/debug/app-standard-debug.apk` returned `INSTALL_FAILED_UID_CHANGED` because the installed package has a different shared UID; no package was removed.
...
Current state: Milestone 1 instrumentation is implemented and the Android debug module builds successfully. The summarizer is proven on synthetic trace input, but no runtime baseline was captured because the available emulator rejects the APK's shared-UID change; no performance or visual conclusion has been drawn. The next action is to resolve package installation safely, enable the trace tag, and capture the two prescribed workloads before starting Milestone 2.
...
Milestone 1 continuation note (2026-07-25): disabled-by-default structured tracing and a deterministic summarizer were added, the focused Android debug build passed, and device capture stopped because the available emulator has an incompatible installed shared UID. No optimization milestone was started.
