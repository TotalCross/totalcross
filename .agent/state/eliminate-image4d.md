...
- Last commit: `cad955305` — checkpoint 4, removed `Image4D` and isolated the JavaSE reader.
...
- Deferred validation: final focused validation, provenance audit decision, and final cleanup remain pending. The native smoke need not be repeated for final docs/state-only changes; no production `Image` code changed after the successful run.
- Blockers: none.
- Exact next action: commit checkpoint 5, search stale `Image4D` references, run final focused tests/dist/copyright/diff checks, and archive the plan state.
