...
Milestone ativo: nenhum; milestones 1 a 5 concluídos. Próximo milestone: 6, preservar resultados e sobreviver mudanças de lifecycle.

Último commit lógico: `fix(scanner,ios): unblock barcode completion wait` (o identificador pode ser obtido com `git rev-parse HEAD`).
...
Próxima ação concreta: iniciar o milestone 6, levando o resultado UTF-8 até a bridge C e adicionando cleanup de lifecycle, callbacks obsoletos e scans repetidos. Não iniciar o milestone 7.
...
Bloqueio: nenhum para o milestone 5. A ausência de `YTPlayerView.h` bloqueia apenas a checagem sintática isolada; o teste C puro do estado passou. A ausência de dispositivo físico permanece uma limitação registrada, mas a reprodução foi dispensada pelo usuário.
