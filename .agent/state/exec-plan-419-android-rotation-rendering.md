...
- Last logical slice: regression of the two-Edit lower-screen keyboard shift across plan checkpoints and SDK 7.2.2
...
- Regression result: the pre-plan base, both M4 checkpoints, and the cached SDK 7.2.2 AAB all computed `shiftY=1804` with the IME visible while leaving the lower Edit covered; the failure is pre-existing relative to this ExecPlan, not attributable to a milestone transition
- SDK 7.2.2 evidence: `/tmp/tc-sdk722-deploy.log`, `/tmp/tc-sdk722-shift-ime.mp4`, `/tmp/tc-sdk722-shift-ime-frame.png`, and `/tmp/tc-sdk722-ime.txt`
