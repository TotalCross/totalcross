...
- Last logical slice: repeat of the SDK 7.2.2 two-Edit lower-screen keyboard shift on the connected physical device
...
- Regression result: the pre-plan base, both M4 checkpoints, and cached SDK 7.2.2 AAB failed on the Android 14 emulator; the same SDK 7.2.2 APK passed on the physical Android 13 device with `shiftY=1800`/`shiftH=200` and the lower Edit visible above the IME. This is device/rendering-path dependent, not a uniform milestone regression.
