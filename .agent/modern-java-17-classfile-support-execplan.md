...
| 8 | Default and static interface methods | Partial | The aggregate smoke app compiles and deploys default/static interface method fixtures. | Runtime execution currently fails for both default and static interface method calls in `Java8FeatureSmokeTest`. |
...
| 9 | Private interface methods and private static interface methods | Partial | The aggregate smoke app compiles and deploys private/private static interface method fixtures. | Runtime execution currently fails for both private and private static interface method calls in `Java9FeatureSmokeTest`. |
...
| 16 | Records | Partial | `Java16FeatureSmokeTest`, `Java17ClassFileTest`, and `Java17FeatureTest` cover record metadata, construction, accessors, custom methods, and generated object methods. Runtime smoke passes construction, accessors, generated `toString`, generated `hashCode`, and custom methods. | Runtime smoke fails generated `equals` because private record fields cannot be accessed by reflection. Reflection over record components and annotations remains normal runtime API compatibility work. |
...
- [ ] Review Java 8 runtime gaps found by smoke validation, especially default/static interface method dispatch, `Predicate.and`, and serializable lambda deserialization, and either implement or document precise unsupported diagnostics.
...
2026-07-06 / Codex: Runtime smoke execution was captured from `TotalCrossSDK/build/feature-smoke/classes/install/macos/FeatureSmokeApp` with stdout in `/tmp/tc-feature-smoke-app.log`. The requested Gradle task could not regenerate the app during this run because Gradle's journal cache was locked by another process, so the existing generated app was executed. Runtime passed Java 1.4 assertions; Java 5 enums, generics, enhanced for, autoboxing, varargs, covariant return, and annotation metadata; Java 6 interface `@Override`; Java 7 diamond, string switch, try-with-resources, multi-catch, binary literals, and numeric separators; Java 8 lambdas, method references, `altMetafactory`, reference/primitive adaptations, type annotations, and repeatable annotations; Java 9 string concat, diamond anonymous classes, effectively-final try-with-resources, and private `@SafeVarargs`; Java 10 `var`; Java 11 string concat, nestmates, and lambda `var`; Java 14 switch expressions; Java 15 text blocks; Java 16 record construction/accessors/`toString`/`hashCode` and instanceof pattern matching; and Java 17 sealed metadata-only support. Runtime failures found: Java 8 default interface method call, Java 8 static interface method call, Java 9 private interface method call, Java 9 private static interface method call, Java 16 record `equals`, and Java 16 record equals mismatch. Record equality failures reported `java.lang.IllegalAccessException: Private fields cannot be accessed by reflection`.
