...
WINDOW_STARTUP = ROOT / "TotalCrossVM/src/nm/ui/WindowStartup.c"
WINDOW_STARTUP_H = ROOT / "TotalCrossVM/src/nm/ui/WindowStartup.h"
...
class SDLDesktopContractTests(unittest.TestCase):
    ...
    def test_window_startup_size_precedence_and_defaults(self):
        window = WINDOW_STARTUP.read_text()
        header = WINDOW_STARTUP_H.read_text()
        self.assertIn("bool windowResolveStartupConfiguration(", header)
        self.assertIn('#ifdef __cplusplus\nextern "C" {', header)
        self.assertIn("ATTR_WINDOWSIZE_320X480", window)
        self.assertIn("*width = 320", window)
        self.assertIn("*height = 480", window)
        self.assertIn("*width = 480", window)
        self.assertIn("*height = 640", window)
        self.assertIn("*width = 600", window)
        self.assertIn("display->usableHeight < 800", window)
        self.assertIn("options->screenSpecified", window)
        self.assertIn("environmentWidth > 0", window)
        self.assertIn("display->width / 2", window)
        self.assertIn("display->height / 2", window)
    ...
    def test_sdl_startup_size_attributes_and_hidpi_state(self):
        ...
        self.assertIn("windowResolveStartupConfiguration(&options, &display, &startupConfiguration)", source)
        self.assertIn("windowLoadStartupEnvironment(&options)", source)
        self.assertIn("display.width = 800", source)
        self.assertIn("display.height = 600", source)
        self.assertIn("options.legacyFullscreen = fullScreen", source)
        self.assertIn("if (startupConfiguration.resizable)", source)
        ...
        self.assertIn("sdlWindowPosition(startupConfiguration.xMode, startupConfiguration.x)", source)
        self.assertNotIn("TC_WIDTH", source)
        self.assertNotIn("TC_HEIGHT", source)

    def test_native_startup_uses_shared_size_policy_and_client_geometry(self):
        native = WIN_GFX.read_text()
        self.assertIn("windowResolveStartupConfiguration(&startupOptions, &display, &configuration)", native)
        ...
        self.assertIn("startupOptions.legacyFullscreen = *tcSettings.isFullScreenPtr", native)
        self.assertIn("windowLoadStartupEnvironment(&startupOptions)", native)
    ...
    def test_screen_bounds_parser_tracks_explicit_size_request(self):
        ...
        self.assertIn("TCWindowStartupOptions desktopWindowStartupOptions", globals_source)
        self.assertIn("extern TCWindowStartupOptions desktopWindowStartupOptions", globals_header)
        self.assertIn("desktopWindowStartupOptions.screenSpecified = true", source)
        self.assertIn("windowResetCommandLineOptions(&desktopWindowStartupOptions)", source)

        tests = (ROOT / "TotalCrossVM/src/tests/tc_tests.c").read_text()
        self.assertIn("test_windowResolveStartupConfiguration", tests)
        self.assertNotIn("defScrX", source)
        self.assertNotIn("initialWindowState", source)
